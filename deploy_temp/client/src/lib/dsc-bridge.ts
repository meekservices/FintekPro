...
export type DSCTokenStatus = 'not_detected' | 'connected' | 'pin_required' | 'authenticated' | 'error';
...
class DSCBridge {
  ...
  private async checkMiddleware(): Promise<void> {
    try {
      const response = await fetch(`http://localhost:${this.middlewarePort}/status`, {
        method: 'GET',
  ...
  private connectWebSocket(): void {
    ...
    try {
      this.websocket = new WebSocket(`ws://localhost:${this.middlewarePort}/dsc`);
  ...
  async signData(dataToSign: string, algorithm: 'SHA256withRSA' | 'SHA384withRSA' | 'SHA512withRSA' | 'SHA256withECDSA' = 'SHA256withRSA'): Promise<DSCSignResult> {
    if (!this.selectedCertificate) {
      return { success: false, error: 'No certificate selected' };
    }

    if (this.status !== 'authenticated') {
      return { success: false, error: 'Token not authenticated. Please enter PIN.' };
    }
