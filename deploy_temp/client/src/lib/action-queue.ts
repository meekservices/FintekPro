...
export class ActionQueueService {
  ...
  async queueAction(
    ...
    if (isBlockedAction(endpoint)) {
      console.warn('[ActionQueue] Blocked action queuing for execution endpoint:', endpoint);
      return null;
    }
  ...
  async processQueue(userId: string): Promise<SyncResult[]> {
    ...
    this.processingPromise = (async () => {
      try {
        ...
        console.log(`[ActionQueue] Processing ${pendingActions.length} pending actions`);
  ...
  private async processAction(action: QueuedAction): Promise<SyncResult> {
    ...
    if (action.retryCount >= action.maxRetries) {
      await draftStorage.updateActionStatus(action.id, 'failed', 'Max retries exceeded');
      return {
        success: false,
        action,
    ...
    try {
      ...
      if (this.isRetryableError(error)) {
        await draftStorage.updateActionStatus(action.id, 'pending', errorMessage);
        console.log(`[ActionQueue] Action will retry: ${action.id}`);
      } else {
        await draftStorage.updateActionStatus(action.id, 'failed', errorMessage);
        console.error(`[ActionQueue] Action failed permanently: ${action.id}`, error);
      }
