...
import { 
  TDS1Entry, TDS2Entry, TCSEntry, Section234FDetails, Section87ADetails 
} from "../types";
...
export const TDSSchedulesSection: React.FC = (): React.ReactElement => {
  ...
  const totalTDS1 = tds1Entries.reduce((acc: number, curr: TDS1Entry): number => acc + (curr.tdsDeducted || 0), 0);
  const totalTDS2 = tds2Entries.reduce((acc: number, curr: TDS2Entry): number => acc + (curr.tdsDeducted || 0), 0);
  const totalTCS = tcsEntries.reduce((acc: number, curr: TCSEntry): number => acc + (curr.tcsCollected || 0), 0);
  ...
  const compute234F = (): void => {
    ...
    setSection234F((p: Section234FDetails): Section234FDetails => ({ ...p, isApplicable: isLate, lateFee: fee, isSmallTaxpayer: section234F.totalIncome <= 500000 }));
  };

  const compute87A = (): void => {
    const threshold = taxRegime === "old" ? section87A.incomeThresholdOld : section87A.incomeThresholdNew;
    const isElig = section87A.taxableIncome <= threshold && residentialStatus === "resident";
    let rebate = 0;
    if (isElig) {
    ...
    setSection87A((p: Section87ADetails): Section87ADetails => ({ ...p, isEligible: isElig, rebateAmount: rebate, taxAfterRebate: Math.max(0, p.normalTaxLiability - rebate) }));
  };
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              {tds1Entries.length === 0 ? (
                ...
                tds1Entries.map((entry: TDS1Entry, idx: number): React.ReactElement => (
                  <Card key={idx} className="border-dashed">
                    <CardContent className="pt-4 space-y-3">
                      <div className="flex items-center justify-between">
                        <Badge variant="outline" className="text-xs">Employer {idx + 1}</Badge>
                        <Button variant="ghost" size="sm" onClick={(): void => setTds1Entries((prev: TDS1Entry[]) => prev.filter((_e: TDS1Entry, i: number): boolean => i !== idx))}>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                          ...
                          <Input value={entry.employerTAN} onChange={(e: React.ChangeEvent<HTMLInputElement>): void => setTds1Entries((p: TDS1Entry[]) => p.map((x: TDS1Entry, i: number): TDS1Entry => i === idx ? { ...x, employerTAN: e.target.value.toUpperCase() } : x))} placeholder="AAAA00000A" maxLength={10} />
                        </div>
                        <div>
                          <Label className="text-xs">Employer Name</Label>
                          <Input value={entry.employerName} onChange={(e: React.ChangeEvent<HTMLInputElement>): void => setTds1Entries((p: TDS1Entry[]) => p.map((x: TDS1Entry, i: number): TDS1Entry => i === idx ? { ...x, employerName: e.target.value } : x))} />
                        </div>
                        <div>
                          <Label className="text-xs">Income Credited (₹)</Label>
                          <CurrencyInput id={`tds1-income-${idx}`} value={entry.incomeCredited} onChange={(v: number): void => setTds1Entries((p: TDS1Entry[]) => p.map((x: TDS1Entry, i: number): TDS1Entry => i === idx ? { ...x, incomeCredited: v } : x))} />
                        </div>
                        <div>
                          <Label className="text-xs">TDS Deducted (₹)</Label>
                          <CurrencyInput id={`tds1-tds-${idx}`} value={entry.tdsDeducted} onChange={(v: number): void => setTds1Entries((p: TDS1Entry[]): TDS1Entry[] => p.map((x: TDS1Entry, i: number): TDS1Entry => i === idx ? { ...x, tdsDeducted: v } : x))} />
                        </div>
                      </div>
                    </CardContent>
                  </Card>
              ...
              <Button variant="outline" size="sm" onClick={(): void => setTds1Entries((p: TDS1Entry[]): TDS1Entry[] => [...p, { employerTAN: "", employerName: "", salaryUnderSection: "17(1)" as const, incomeCredited: 0, tdsDeducted: 0, tdsClaimedCurrentYear: 0 }])}>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              {tds2Entries.length === 0 ? (
                ...
                tds2Entries.map((entry: TDS2Entry, idx: number): React.ReactElement => (
                  <Card key={idx} className="border-dashed">
                    <CardContent className="pt-4 space-y-3">
                      <div className="flex items-center justify-between">
                        <Badge variant="outline" className="text-xs">Entry {idx + 1}</Badge>
                        <Button variant="ghost" size="sm" onClick={(): void => setTds2Entries((prev: TDS2Entry[]): TDS2Entry[] => prev.filter((_: TDS2Entry, i: number): boolean => i !== idx))}>
                          <XCircle className="h-4 w-4 text-red-500" />
                        </Button>
                      </div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                          ...
                          <Input value={entry.deductorTAN} onChange={(e: React.ChangeEvent<HTMLInputElement>): void => setTds2Entries((p: TDS2Entry[]): TDS2Entry[] => p.map((x: TDS2Entry, i: number): TDS2Entry => i === idx ? { ...x, deductorTAN: e.target.value.toUpperCase() } : x))} placeholder="AAAA00000A" maxLength={10} />
                        </div>
                        <div>
                          <Label className="text-xs">Deductor Name</Label>
                          <Input value={entry.deductorName} onChange={(e: React.ChangeEvent<HTMLInputElement>): void => setTds2Entries((p: TDS2Entry[]) => p.map((x: TDS2Entry, i: number): TDS2Entry => i === idx ? { ...x, deductorName: e.target.value } : x))} />
                        </div>
                        <div>
                          <Label className="text-xs">Income Type</Label>
                          <Select value={entry.incomeType} onValueChange={(v: "interest" | "dividend" | "rent" | "professional_fees" | "commission" | "winnings" | "sale_of_property" | "other"): void => setTds2Entries((p: TDS2Entry[]): TDS2Entry[] => p.map((x: TDS2Entry, i: number): TDS2Entry => i === idx ? { ...x, incomeType: v } : x))}>
                        ...
                        <div>
                          ...
                          <Input value={entry.section} onChange={(e: React.ChangeEvent<HTMLInputElement>): void => setTds2Entries((p: TDS2Entry[]) => p.map((x: TDS2Entry, i: number): TDS2Entry => i === idx ? { ...x, section: e.target.value } : x))} placeholder="e.g. 194A" />
                        </div>
                        <div>
                          <Label className="text-xs">Income Credited (₹)</Label>
                          <CurrencyInput id={`tds2-income-${idx}`} value={entry.incomeCredited} onChange={(v: number): void => setTds2Entries((p: TDS2Entry[]) => p.map((x: TDS2Entry, i: number): TDS2Entry => i === idx ? { ...x, incomeCredited: v } : x))} />
                        </div>
                        <div>
                          <Label className="text-xs">TDS Deducted (₹)</Label>
                          <CurrencyInput id={`tds2-tds-${idx}`} value={entry.tdsDeducted} onChange={(v: number): void => setTds2Entries((p: TDS2Entry[]) => p.map((x: TDS2Entry, i: number): TDS2Entry => i === idx ? { ...x, tdsDeducted: v } : x))} />
                        </div>
                      </div>
                    </CardContent>
                  </Card>
              ...
              <Button variant="outline" size="sm" onClick={(): void => setTds2Entries((p: TDS2Entry[]): TDS2Entry[] => [...p, { deductorTAN: "", deductorName: "", incomeType: "interest" as const, section: "194A", dateOfPayment: "", incomeCredited: 0, tdsDeducted: 0, tdsClaimedCurrentYear: 0 }])}>
        ...
        <TabsContent value="tcs">
          <Card>
            ...
            <CardContent className="space-y-4">
                  ...
                  <Card key={idx} className="border-dashed">
                    <CardContent className="pt-4 space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        <Button variant="ghost" size="sm" onClick={(): void => setTcsEntries((prev: TCSEntry[]): TCSEntry[] => prev.filter((_: TCSEntry, i: number): boolean => i !== idx))}>
                          <XCircle className="h-4 w-4 text-red-500" />
                        </Button>
                      </div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                          ...
                          <Input value={entry.collectorTAN} onChange={(e: React.ChangeEvent<HTMLInputElement>): void => setTcsEntries((p: TCSEntry[]): TCSEntry[] => p.map((x: TCSEntry, i: number): TCSEntry => i === idx ? { ...x, collectorTAN: e.target.value.toUpperCase() } : x))} />
                        </div>
                        <div>
                          <Label className="text-xs">Collector Name</Label>
                          <Input value={entry.collectorName} onChange={(e: React.ChangeEvent<HTMLInputElement>): void => setTcsEntries((p: TCSEntry[]) => p.map((x: TCSEntry, i: number): TCSEntry => i === idx ? { ...x, collectorName: e.target.value } : x))} />
                        </div>
                        <div>
                          <Label className="text-xs">Amount Paid (₹)</Label>
                          <CurrencyInput id={`tcs-amt-${idx}`} value={entry.amountPaid} onChange={(v: number): void => setTcsEntries((p: TCSEntry[]) => p.map((x: TCSEntry, i: number): TCSEntry => i === idx ? { ...x, amountPaid: v } : x))} />
                        </div>
                        <div>
                          <Label className="text-xs">TCS Collected (₹)</Label>
                          <CurrencyInput id={`tcs-tcs-${idx}`} value={entry.tcsCollected} onChange={(v: number): void => setTcsEntries((p: TCSEntry[]) => p.map((x: TCSEntry, i: number): TCSEntry => i === idx ? { ...x, tcsCollected: v } : x))} />
                        </div>
                      </div>
                    </CardContent>
                  </Card>
              ...
              <Button variant="outline" size="sm" onClick={(): void => setTcsEntries((p: TCSEntry[]): TCSEntry[] => [...p, { collectorTAN: "", collectorName: "", amountPaid: 0, tcsCollected: 0, tcsClaimedCurrentYear: 0 }])}>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <Input type="date" value={section234F.filingDueDate} onChange={(e: React.ChangeEvent<HTMLInputElement>): void => setSection234F((p: Section234FDetails) => ({ ...p, filingDueDate: e.target.value }))} />
                </div>
                <div>
                  <Label className="text-xs">Actual Filing Date</Label>
                  <Input type="date" value={section234F.actualFilingDate} onChange={(e: React.ChangeEvent<HTMLInputElement>): void => setSection234F((p: Section234FDetails) => ({ ...p, actualFilingDate: e.target.value }))} />
                </div>
                <div>
                  <Label className="text-xs">Total Income (₹)</Label>
                  <CurrencyInput id="s234f-income" value={section234F.totalIncome} onChange={(v: number): void => setSection234F((p: Section234FDetails) => ({ ...p, totalIncome: v }))} />
                </div>
              </div>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <CurrencyInput id="s87a-income" value={section87A.taxableIncome} onChange={(v: number): void => setSection87A((p: Section87ADetails) => ({ ...p, taxableIncome: v }))} />
                </div>
                <div>
                  <Label className="text-xs">Normal Tax Liability (₹)</Label>
                  <CurrencyInput id="s87a-tax" value={section87A.normalTaxLiability} onChange={(v: number): void => setSection87A((p: Section87ADetails) => ({ ...p, normalTaxLiability: v }))} />
                </div>
              </div>
