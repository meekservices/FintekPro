...
export default function ITRPrefilledPage() {
  ...
  const [selectedAssessmentYear, setSelectedAssessmentYear] = useState('2025-26');
  ...
  const { data: itrData, isLoading: itrLoading, refetch: refetchITR } = useQuery<ITRPrefilledData>({
    queryKey: ['/api/itr/prefilled', userId, selectedAssessmentYear],
    queryFn: async () => {
      const response = await fetch(`/api/itr/prefilled/${userId}?assessmentYear=${selectedAssessmentYear}`);
      const result = await response.json();
      return result.data;
    }
  });
  ...
  const autoPopulateMutation = useMutation({
    mutationFn: async (data: {
      ...
      const response = await apiRequest('POST', '/api/itr/auto-populate', { body: data });
  ...
  const syncDataSourceMutation = useMutation({
    mutationFn: async (sourceId: string) => {
      const response = await apiRequest('POST', `/api/itr/sync-source/${sourceId}`, { body: { userId } });
      return response.json();
    },
  ...
  const validateITRMutation = useMutation({
    mutationFn: async () => {
      const response = await apiRequest('POST', `/api/itr/validate/${itrData?.id}`, { body: {} });
      return response.json();
    },
    onSuccess: (data) => {
      toast({
  ...
  const generateITRMutation = useMutation({
    mutationFn: async () => {
      const response = await apiRequest('POST', `/api/itr/generate/${itrData?.id}`, { body: {} });
      return response.json();
    },
    onSuccess: (data) => {
      toast({
        title: "ITR Generated Successfully",
        description: "Your ITR is ready for filing. Download the JSON/XML file.",
      });
      refetchITR();
    },
  });
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Intelligent Income Tax Return preparation with auto-population from multiple sources
          </p>
        </div>
        <div className="flex gap-3">
          <Button
            onClick={handleAutoPopulate}
            disabled={autoPopulateMutation.isPending || dataSources.filter(s => s.status === 'connected').length === 0}
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="space-y-2">
              ...
              <Select value={selectedAssessmentYear} onValueChange={setSelectedAssessmentYear}>
            ...
            <div className="space-y-2">
              ...
              <Select value={selectedTaxRegime} onValueChange={setSelectedTaxRegime}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <Card>
              <CardContent className="p-6">
                ...
                <Progress value={itrData?.completionPercentage || 0} className="mt-4" />
              </CardContent>
            </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="flex flex-wrap gap-3">
                ...
                >
                  {validateITRMutation.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                ...
                <Button
                  ...
                  disabled={generateITRMutation.isPending || !itrData?.readyForFiling}
                  variant="outline"
                  data-testid="button-generate-itr"
                >
                  {generateITRMutation.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                ...
                <Button
                  variant="outline"
                  disabled={!itrData}
                  onClick={() => window.open(`/api/itr/download/${itrData?.id}/json`, '_blank')}
                    ...
                    <div key={source.id} className="flex items-center justify-between p-4 border rounded-lg">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            {source.recordsCount} records • Last sync: {source.lastSync || 'Never'}
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center gap-3">
                        ...
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => syncDataSourceMutation.mutate(source.id)}
                ...
                <CardContent>
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-semibold">{formatCurrency(itrData.taxComputation?.totalIncome || 0)}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span>Total Tax</span>
                      <span className="font-semibold">{formatCurrency(itrData.taxComputation?.totalTax || 0)}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span>TDS Deducted</span>
                      <span className="font-semibold text-green-600">{formatCurrency(itrData.tdsDetails?.totalTds || 0)}</span>
                    </div>
                    <Separator />
                    <div className="flex justify-between items-center text-lg font-bold">
                      <span>Refund/Tax Payable</span>
                      <span className={itrData.taxComputation?.refundDue > 0 ? 'text-green-600' : 'text-red-600'}>
              ...
              {itrData ? (
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant={itrData.validationStatus === 'validated' ? 'default' : 'secondary'}>
                  ...
                  {itrData.validationErrors && itrData.validationErrors.length > 0 && (
                    <div className="space-y-2">
                      ...
                      <ul className="space-y-1">
                        {itrData.validationErrors.map((error: string, index: number) => (
