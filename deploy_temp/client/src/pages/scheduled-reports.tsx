...
import { 
  FileText, Plus, Calendar, Clock, Mail, Trash2, 
  Play, Pause, Download, BarChart3, PieChart, TrendingUp
} from "lucide-react";
...
const REPORT_TYPES = [
  ...
  { value: "performance_analysis", label: "Performance Analysis", icon: TrendingUp }
];
...
export default function ScheduledReports() {
  ...
  const { data, isLoading } = useQuery<{ success: boolean; reports: ScheduledReport[] }>({
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      toast({ title: "Report Scheduled", description: "Your report has been scheduled successfully." });
      setIsDialogOpen(false);
      setFormData({ reportType: "", reportName: "", frequency: "monthly", dayOfWeek: 1, dayOfMonth: 1, deliveryEmail: "" });
      queryClient.invalidateQueries({ queryKey: ["/api/features/reports/scheduled"] });
    },
    onError: () => {
  ...
  const handleSubmit = () => {
    if (!formData.reportType || !formData.reportName || !formData.deliveryEmail) {
        ...
        <Card className="text-center py-12">
          <CardContent>
            ...
            <p className="text-muted-foreground mb-4">
              Set up automated reports to receive financial insights on a regular schedule.
            </p>
