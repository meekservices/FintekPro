...
export default function CommoditiesPage() {
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      case "Precious Metals": return <Gem className="h-5 w-5" />;
      case "Energy": return <Zap className="h-5 w-5" />;
      case "Base Metals": return <Package className="h-5 w-5" />;
      case "Agriculture": return <Wheat className="h-5 w-5" />;
      case "Spices": return <Droplet className="h-5 w-5" />;
    ...
    <div className="container mx-auto py-6 px-4 max-w-7xl">
      <div className="mb-6">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h1 className="text-3xl font-bold" data-testid="commodities-title">Commodities Trading</h1>
            <p className="text-muted-foreground">Trade on MCX and NCDEX exchanges</p>
          </div>
        </div>
      </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-600">{marketStats.advancers}</p>
              </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-red-600">{marketStats.decliners}</p>
              </div>
        ...
        <Card>
          <CardContent className="pt-6">
            <div>
              ...
              <p className="text-green-600 font-bold">+{marketStats.topGainer.percent}%</p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="pt-6">
            <div>
              ...
              <p className="text-red-600 font-bold">{marketStats.topLoser.percent}%</p>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Tabs defaultValue="market" className="space-y-6">
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>Real-time prices from MCX and NCDEX</CardDescription>
                </div>
            ...
            <CardContent>
              <div className="space-y-3">
                {filteredCommodities.map((commodity) => (
                  <div key={commodity.id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-accent transition-colors cursor-pointer" data-testid={`commodity-${commodity.id}`}>
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 text-sm text-muted-foreground">
                          ...
                          <Badge variant="outline" className="text-xs">{commodity.exchange}</Badge>
                    ...
                    <div className="text-right">
                      <p className="text-xl font-bold">₹{commodity.price.toLocaleString()}</p>
                      <div className="flex items-center gap-2 justify-end">
                        <p className={`text-sm font-medium ${commodity.change >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                          {commodity.change >= 0 ? '+' : ''}{commodity.change}
                        </p>
                        <Badge variant={commodity.changePercent >= 0 ? "default" : "destructive"}>
                          {commodity.changePercent >= 0 ? '+' : ''}{commodity.changePercent}%
                        </Badge>
                      </div>
                      <p className="text-xs text-muted-foreground mt-1">Vol: {commodity.volume} • /{commodity.unit}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            <Card className="lg:col-span-2">
              <CardHeader>
                ...
                <CardDescription>Trade commodities on MCX and NCDEX</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      <Button variant="default" className="flex-1" data-testid="button-buy-commodity">Buy</Button>
                      <Button variant="outline" className="flex-1" data-testid="button-sell-commodity">Sell</Button>
                    </div>
                  </div>

                  <div className="space-y-2">
                    <Label>Quantity (Lot)</Label>
                    <Input type="number" defaultValue="1" data-testid="input-commodity-quantity" />
                  </div>
                </div>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <Label>Price</Label>
                    <Input type="number" placeholder="Market Price" data-testid="input-commodity-price" />
                  </div>
                ...
                <div className="pt-4 space-y-2">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-medium text-green-600">₹5,00,000</span>
                  </div>
                </div>

                <Button className="w-full" size="lg" data-testid="button-place-commodity-order">
                  Place Order
                </Button>
              </CardContent>
            </Card>
        ...
        <TabsContent value="metals" className="space-y-4">
          <div className="grid md:grid-cols-2 gap-6">
            {commodities.filter(c => c.category === "Precious Metals").map((metal) => (
              <Card key={metal.id}>
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <CardDescription>{metal.exchange} • {metal.unit}</CardDescription>
                      </div>
                    </div>
                    <Badge variant={metal.changePercent >= 0 ? "default" : "destructive"}>
                      {metal.changePercent >= 0 ? '+' : ''}{metal.changePercent}%
                    </Badge>
                  </div>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                    <div>
                      <p className="text-3xl font-bold">₹{metal.price.toLocaleString()}</p>
                      <p className={`text-sm ${metal.change >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                    ...
                    <Button className="w-full" data-testid={`button-trade-${metal.symbol.toLowerCase()}`}>
                      Trade {metal.name}
                    </Button>
                  </div>
                </CardContent>
        ...
        <TabsContent value="energy" className="space-y-4">
          <div className="grid md:grid-cols-2 gap-6">
            {commodities.filter(c => c.category === "Energy").map((energy) => (
              <Card key={energy.id}>
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <CardDescription>{energy.exchange} • {energy.unit}</CardDescription>
                      </div>
                    </div>
                    <Badge variant={energy.changePercent >= 0 ? "default" : "destructive"}>
                      {energy.changePercent >= 0 ? '+' : ''}{energy.changePercent}%
                    </Badge>
                  </div>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                    <div>
                      <p className="text-3xl font-bold">₹{energy.price.toLocaleString()}</p>
                      <p className={`text-sm ${energy.change >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                    ...
                    <Button className="w-full" data-testid={`button-trade-${energy.symbol.toLowerCase()}`}>
                      Trade {energy.name}
                    </Button>
                  </div>
                </CardContent>
        ...
        <TabsContent value="agri" className="space-y-4">
          <div className="grid md:grid-cols-2 gap-6">
            {commodities.filter(c => c.category === "Agriculture" || c.category === "Spices").map((agri) => (
              <Card key={agri.id}>
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <CardDescription>{agri.exchange} • {agri.unit}</CardDescription>
                      </div>
                    </div>
                    <Badge variant={agri.changePercent >= 0 ? "default" : "destructive"}>
                      {agri.changePercent >= 0 ? '+' : ''}{agri.changePercent}%
                    </Badge>
                  </div>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                    <div>
                      <p className="text-3xl font-bold">₹{agri.price.toLocaleString()}</p>
                      <p className={`text-sm ${agri.change >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                    ...
                    <Button className="w-full" data-testid={`button-trade-${agri.symbol.toLowerCase()}`}>
                      Trade {agri.name}
                    </Button>
                  </div>
                </CardContent>
