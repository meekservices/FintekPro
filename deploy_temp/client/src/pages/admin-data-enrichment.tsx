...
export default function AdminDataEnrichment() {
  ...
  const { data: mfStats, isLoading: mfLoading } = useQuery<{ success: boolean; stats: MFStats }>({
    queryKey: ["/api/admin/enrichment/mf/stats"],
    refetchInterval: 30000,
  });

  const { data: stockStats, isLoading: stockLoading } = useQuery<{ success: boolean; stats: StockStats }>({
    queryKey: ["/api/admin/enrichment/stocks/stats"],
    refetchInterval: 30000,
  });

  const { data: mfProgress } = useQuery<{ success: boolean; progress: EnrichmentProgress }>({
  ...
  const runMfEnrichment = useMutation({
    ...
    onSuccess: () => {
      toast({ title: "MF Enrichment Started", description: "Processing funds in the background..." });
      queryClient.invalidateQueries({ queryKey: ["/api/admin/enrichment/mf/progress"] });
    },
    onError: (error: any) => {
      toast({ title: "Error", description: error.message, variant: "destructive" });
    },
  ...
  const runStockEnrichment = useMutation({
    ...
    onSuccess: () => {
      toast({ title: "Stock Enrichment Started", description: "Processing stocks in the background..." });
      queryClient.invalidateQueries({ queryKey: ["/api/admin/enrichment/stocks/progress"] });
    },
    onError: (error: any) => {
      toast({ title: "Error", description: error.message, variant: "destructive" });
    },
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Fill NULL database columns with real, dynamic data from AMFI and market sources
          </p>
        </div>
      </div>
        ...
        <TabsContent value="overview" className="space-y-4">
              ...
              <CardContent>
                {mfLoading ? (
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-2xl font-bold">{mf.totalFunds.toLocaleString()}</span>
                      <Badge variant={mf.percentEnriched > 50 ? "default" : "secondary"}>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      ...
                      <div className="flex items-center gap-1 col-span-2">
                        ...
                        <span>Both NULL: {mf.withBothNull.toLocaleString()}</span>
                      </div>
                    </div>
            ...
            <Card>
              ...
              <CardContent>
                {stockLoading ? (
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-2xl font-bold">{stock.totalStocks.toLocaleString()}</span>
                      <Badge variant={stock.percentEnriched > 50 ? "default" : "secondary"}>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      ...
                      <div className="flex items-center gap-1">
                        <CheckCircle2 className="h-3 w-3 text-green-500" />
                        <span>Book Value: {stock.withBookValue.toLocaleString()}</span>
                      </div>
                      <div className="flex items-center gap-1">
                        <AlertCircle className="h-3 w-3 text-amber-500" />
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Run enrichment jobs to fill NULL columns with real data</CardDescription>
            </CardHeader>
        ...
        <TabsContent value="mutual-funds" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Fetches TER (expense ratio) and AUM data from AMFI and GitHub sources
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              {mf && (
                <div className="grid gap-4 md:grid-cols-4">
                  <div className="bg-muted/50 rounded-lg p-4">
                    ...
                    <div className="text-2xl font-bold">{mf.totalFunds.toLocaleString()}</div>
                  </div>
                  <div className="bg-muted/50 rounded-lg p-4">
                    <div className="text-sm text-muted-foreground">With TER</div>
                    <div className="text-2xl font-bold text-green-600">{mf.withTer.toLocaleString()}</div>
                    <div className="text-xs text-muted-foreground">
                      {mf.totalFunds > 0 ? Math.round((mf.withTer / mf.totalFunds) * 100) : 0}%
                    </div>
                  </div>
                  <div className="bg-muted/50 rounded-lg p-4">
                    <div className="text-sm text-muted-foreground">With AUM</div>
                    <div className="text-2xl font-bold text-green-600">{mf.withAum.toLocaleString()}</div>
                    <div className="text-xs text-muted-foreground">
                      {mf.totalFunds > 0 ? Math.round((mf.withAum / mf.totalFunds) * 100) : 0}%
                    </div>
                  </div>
                  <div className="bg-muted/50 rounded-lg p-4">
                    <div className="text-sm text-muted-foreground">Missing Both</div>
                    <div className="text-2xl font-bold text-amber-600">{mf.withBothNull.toLocaleString()}</div>
                  </div>
                </div>
              )}
              ...
              {isMfRunning && mfProg && (
                <div className="border rounded-lg p-4 space-y-3">
                  ...
                  <p className="text-sm text-muted-foreground">{mfProg.currentStep}</p>
                  <Progress 
                    value={mfProg.totalFunds ? (mfProg.processedFunds! / mfProg.totalFunds) * 100 : 0} 
                    className="h-2" 
                  />
                  <div className="flex gap-4 text-sm text-muted-foreground">
                    <span>Processed: {mfProg.processedFunds?.toLocaleString() || 0}</span>
                    <span>TER Updated: {mfProg.terUpdated?.toLocaleString() || 0}</span>
                    <span>AUM Updated: {mfProg.aumUpdated?.toLocaleString() || 0}</span>
                  </div>
                </div>
              )}
              ...
              <div className="text-sm text-muted-foreground space-y-1">
                ...
                <ul className="list-disc list-inside ml-2">
                  <li>TER: Inferred from SEBI category-based averages (Direct vs Regular plans)</li>
                  <li>AUM: GitHub Mutual_Fund_Data repository (daily updated)</li>
                </ul>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="space-y-6">
              {stock && (
                <div className="grid gap-4 md:grid-cols-4">
                  <div className="bg-muted/50 rounded-lg p-4">
                    ...
                    <div className="text-2xl font-bold">{stock.totalStocks.toLocaleString()}</div>
                  </div>
                  <div className="bg-muted/50 rounded-lg p-4">
                    <div className="text-sm text-muted-foreground">With P/E Ratio</div>
                    <div className="text-2xl font-bold text-green-600">{stock.withPe.toLocaleString()}</div>
                    <div className="text-xs text-muted-foreground">
                      {stock.totalStocks > 0 ? Math.round((stock.withPe / stock.totalStocks) * 100) : 0}%
                    </div>
                  </div>
                  <div className="bg-muted/50 rounded-lg p-4">
                    <div className="text-sm text-muted-foreground">With EPS</div>
                    <div className="text-2xl font-bold text-green-600">{stock.withEps.toLocaleString()}</div>
                    <div className="text-xs text-muted-foreground">
                      {stock.totalStocks > 0 ? Math.round((stock.withEps / stock.totalStocks) * 100) : 0}%
                    </div>
                  </div>
                  <div className="bg-muted/50 rounded-lg p-4">
                    <div className="text-sm text-muted-foreground">With Book Value</div>
                    <div className="text-2xl font-bold text-green-600">{stock.withBookValue.toLocaleString()}</div>
                  </div>
                </div>
              )}
              ...
              {isStockRunning && stockProg && (
                <div className="border rounded-lg p-4 space-y-3">
                  ...
                  <p className="text-sm text-muted-foreground">{stockProg.currentStep}</p>
                  <Progress 
                    value={stockProg.totalStocks ? (stockProg.processedStocks! / stockProg.totalStocks) * 100 : 0} 
                    className="h-2" 
                  />
                  <div className="flex gap-4 text-sm text-muted-foreground">
                    <span>Processed: {stockProg.processedStocks?.toLocaleString() || 0}</span>
                    <span>Enriched: {stockProg.enriched?.toLocaleString() || 0}</span>
                  </div>
                </div>
              )}
