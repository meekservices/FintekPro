...
const PLATFORM_REPORTS: PlatformReport[] = [
  {
    id: 'aum_report',
    name: 'AUM Report',
    description: 'Total assets under management across all clients and products',
    category: 'business',
    icon: Wallet,
...
export default function AdminReportsHub() {
  ...
  const getStatusConfig = (status: string) => {
    switch (status) {
      case 'ready': return { color: 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30', label: 'Ready', icon: CheckCircle };
  ...
  const getCategoryStats = (category: string) => {
    const categoryReports = PLATFORM_REPORTS.filter(r => r.category === category);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {(['business', 'compliance', 'revenue', 'operations'] as const).map((category) => {
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
        <TabsList className="bg-card border-border">
          ...
          <TabsTrigger value="operations" className="data-[state=active]:bg-purple-600" data-testid="tab-operations">
                  ...
                  <Card key={report.id} className="bg-card/50 border-border hover:border-border transition-colors" data-testid={`report-card-${report.id}`}>
                    ...
                    <CardContent>
                      ...
                      >
                        ...
                        {report.status === 'ready' ? 'Download Report' : 
                         report.status === 'generating' ? 'Generating...' :
                         report.status === 'scheduled' ? 'Scheduled' : 'Unavailable'}
                      </Button>
                    </CardContent>
                  </Card>
                );
