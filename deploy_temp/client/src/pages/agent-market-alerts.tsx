...
import { BellRing, RefreshCw, TrendingUp, TrendingDown, MessageCircle, ExternalLink, Activity } from "lucide-react";
...
export default function AgentMarketAlerts() {
  ...
  const allClients = Array.from(
    new Map(alerts.flatMap((a) => a.clients.map((c) => [c.clientId, c.clientName]))).entries()
  ).map(([id, name]) => ({ id, name }));

  const filteredAlerts =
    filterClient === "all"
      ? alerts
      : alerts.filter((a) => a.clients.some((c) => c.clientId === filterClient));

  const filteredGainers = filteredAlerts.filter((a) => a.direction === "up");
  const filteredDecliners = filteredAlerts.filter((a) => a.direction === "down");
  ...
  return (
    <div className="container max-w-7xl mx-auto p-4 sm:p-6 space-y-6">
      ...
      {isLoading ? (
        <div className="py-16 text-center text-muted-foreground">Scanning client holdings for market moves...</div>
        ...
        <Card>
          <CardContent className="py-16 text-center">
            ...
            <p className="text-muted-foreground text-sm max-w-sm mx-auto">
              {alerts.length === 0
                ? "No holdings in your clients' portfolios have moved more than " + minMove + "% today. Markets may be closed."
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-3">
            <h3 className="font-semibold flex items-center gap-2 text-green-700 dark:text-green-400">
              <TrendingUp className="h-4 w-4" /> Gainers ({filteredGainers.length})
            </h3>
            {filteredGainers.length === 0 ? (
              <p className="text-sm text-muted-foreground py-4 text-center">No gainers with current filter</p>
            ) : (
              filteredGainers.map((alert) => (
                <AlertCard key={alert.symbol} alert={alert} filterClient={filterClient} />
              ))
            )}
          </div>
          ...
          <div className="space-y-3">
            <h3 className="font-semibold flex items-center gap-2 text-red-700 dark:text-red-400">
              <TrendingDown className="h-4 w-4" /> Decliners ({filteredDecliners.length})
            </h3>
            {filteredDecliners.length === 0 ? (
              <p className="text-sm text-muted-foreground py-4 text-center">No decliners with current filter</p>
            ) : (
...
function AlertCard({ alert, filterClient }: { alert: MarketAlert; filterClient: string }) {
  const isUp = alert.direction === "up";
  const borderColor = isUp ? "border-l-green-500" : "border-l-red-500";
  const changeColor = isUp ? "text-green-700 dark:text-green-400" : "text-red-700 dark:text-red-400";
  const bgColor = isUp ? "bg-green-50/50 dark:bg-green-950/20" : "bg-red-50/50 dark:bg-red-950/20";

  const displayedClients =
    filterClient === "all" ? alert.clients : alert.clients.filter((c) => c.clientId === filterClient);
      ...
      <CardContent className="p-4 space-y-3">
        <div className="flex items-start justify-between">
          ...
          <div className="text-right">
            ...
            <div className="text-xs text-muted-foreground">₹{alert.currentPrice.toFixed(2)}</div>
          </div>
        </div>

        <div className="space-y-1.5">
          <div className="text-xs text-muted-foreground font-medium">Affected clients:</div>
              ...
              <div className="flex gap-1">
                {client.phone && (
                  <Button
                    ...
                    onClick={() => window.open(whatsappLink(client, alert), "_blank")}
                  >
                ...
                <Button
                  ...
                  onClick={() => window.open(`/agent/crm/clients/${client.clientId}`, "_blank")}
                >
