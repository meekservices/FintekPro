    ...
    <div className="container mx-auto p-4 sm:p-6 space-y-4 max-w-5xl" data-testid="page-itr-self">
            ...
            <div key={step.id} className="flex items-center">
              <button
                ...
                className={`flex flex-col items-center gap-1 px-3 py-2 rounded-lg transition-all min-w-[80px] ${
                  ...
                  isAccessible ? "text-muted-foreground hover:bg-muted" : "text-muted-foreground/40 cursor-not-allowed"
                }`}
              >
                <div className={`p-1.5 rounded-full ${isActive ? "bg-primary text-white" : "bg-muted text-muted-foreground"}`}>
                  <Icon className="h-3.5 w-3.5" />
                </div>
                <span className="text-[10px] font-medium whitespace-nowrap">{step.title}</span>
                {hasErrors && <XCircle className="h-2.5 w-2.5 text-destructive mt-0.5" />}
                {isCompleted && !hasErrors && <CheckCircle className="h-2.5 w-2.5 text-green-500 mt-0.5" />}
              </button>
      ...
      <div className="relative min-h-[500px] flex flex-col">
        <div className="absolute top-0 left-0 right-0 h-1 bg-muted rounded-full overflow-hidden">
          <Progress value={progress} className="h-full transition-all duration-500" />
        </div>
