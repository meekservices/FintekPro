...
import { Search, TrendingUp, TrendingDown, Star, Plus, X, BarChart3, PieChart, Target, AlertCircle, CheckCircle, ArrowUpRight, ArrowDownRight, Shuffle } from "lucide-react";
import { useQuery, useMutation, useQueryClient } from "@tanstack/react-query";
import { apiRequest } from "@/lib/queryClient";
import { useMutualFunds, useSearchMutualFunds, type MutualFundData } from "@/hooks/use-mutual-funds";
...
function FundSearchInput({ onFundSelect, selectedFunds }: { 
  ...
  const filteredResults = searchResults?.filter(fund => 
        ...
        <div className="absolute z-10 w-full mt-1 bg-card border border-border rounded-md shadow-lg max-h-64 overflow-y-auto">
          {isLoading ? (
            ...
            filteredResults.map((fund) => (
              ...
              >
                <div className="font-medium text-foreground">{fund.schemeName}</div>
                <div className="text-sm text-muted-foreground">{fund.fundHouse} • NAV: ₹{fund.nav}</div>
              </button>
            ))
          ) : (
...
function SelectedFundCard({ fund, onRemove }: { fund: MutualFundData; onRemove: () => void }) {
  ...
  return (
    <Card className="border border-border" data-testid={`selected-fund-${fund.schemeCode}`}>
...
function ComparisonResults({ comparison }: { comparison: FundComparisonResult }) {
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="bg-gradient-to-br from-green-50 to-green-100 dark:from-green-900/20 dark:to-green-800/20 border-green-200 dark:border-green-700">
          <CardContent className="p-4">
            ...
            <p className="text-sm font-semibold text-green-800 dark:text-green-200">{comparison.summary.bestPerformer}</p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20 border-blue-200 dark:border-blue-700">
          <CardContent className="p-4">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs font-medium text-blue-700 dark:text-blue-300">Most Stable</span>
            </div>
            <p className="text-sm font-semibold text-blue-800 dark:text-blue-200">{comparison.summary.mostStable}</p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-900/20 dark:to-purple-800/20 border-purple-200 dark:border-purple-700">
          <CardContent className="p-4">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs font-medium text-purple-700 dark:text-purple-300">Highest Sharpe</span>
            </div>
            <p className="text-sm font-semibold text-purple-800 dark:text-purple-200">{comparison.summary.highestSharpe}</p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gradient-to-br from-orange-50 to-orange-100 dark:from-orange-900/20 dark:to-orange-800/20 border-orange-200 dark:border-orange-700">
          <CardContent className="p-4">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs font-medium text-orange-700 dark:text-orange-300">Lowest Expense</span>
            </div>
            <p className="text-sm font-semibold text-orange-800 dark:text-orange-200">{comparison.summary.lowestExpense}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="bg-gradient-to-br from-finance-blue/5 to-finance-blue/10 border-finance-blue/20">
        ...
        <CardContent>
          <p className="text-muted-foreground leading-relaxed">{comparison.aiInsights}</p>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border">
                  ...
                  {comparison.funds.map((fund, index) => (
                    <th key={fund.schemeCode} className="text-center p-2 font-medium text-muted-foreground">
                      Fund {index + 1}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                ...
                <tr className="border-b border-border">
                  ...
                  {comparison.funds.map((fund) => (
                    <td key={fund.schemeCode} className="p-2 text-center">
                      <span className={fund.returns["1Y"] >= 0 ? 'text-green-600' : 'text-red-600'}>
                ...
                <tr className="border-b border-border">
                  ...
                  {comparison.funds.map((fund) => (
                    <td key={fund.schemeCode} className="p-2 text-center">
                      <span className={fund.returns["3Y"] >= 0 ? 'text-green-600' : 'text-red-600'}>
                ...
                <tr className="border-b border-border">
                  ...
                  {comparison.funds.map((fund) => (
                    <td key={fund.schemeCode} className="p-2 text-center">
                      {fund.aum ? `₹${(fund.aum / 10000000).toFixed(0)} Cr` : 'N/A'}
                    </td>
                  ))}
                </tr>
                <tr>
...
export default function FundComparison() {
  ...
  const [comparisonResult, setComparisonResult] = useState<FundComparisonResult | null>(null);
  const { toast } = useToast();
  const queryClient = useQueryClient();

  const compareMutation = useMutation({
    mutationFn: async (data: { fundCodes: string[]; timePeriod: string; comparisonType: string }) => {
    ...
    onSuccess: (data) => {
      ...
      toast({
        ...
        description: "Fund comparison analysis has been generated successfully."
      });
    },
    onError: (error) => {
      toast({
  ...
  const handleFundRemove = (fundCode: string) => {
    setSelectedFunds(selectedFunds.filter(fund => fund.schemeCode !== fundCode));
  };
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <div className="text-center space-y-2">
        ...
        <p className="text-muted-foreground">Compare mutual funds side-by-side with AI-powered insights</p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Search and select up to 5 mutual funds for detailed comparison analysis
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <FundSearchInput onFundSelect={handleFundSelect} selectedFunds={selectedFunds} />
          
          {selectedFunds.length > 0 && (
            <div>
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-medium text-foreground">Selected Funds ({selectedFunds.length}/5)</h3>
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    ...
                    disabled={selectedFunds.length < 2 || compareMutation.isPending}
                    className="bg-finance-blue hover:bg-finance-blue/90"
                    data-testid="button-compare-funds"
                  >
                    {compareMutation.isPending ? "Comparing..." : "Compare Funds"}
                  </Button>
                </div>
              </div>
      ...
      {selectedFunds.length === 0 && !comparisonResult && (
        <Card className="text-center py-12">
          <CardContent>
            ...
            <h3 className="text-lg font-medium text-foreground mb-2">Start Comparing Funds</h3>
