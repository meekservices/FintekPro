...
const defaultCommissionRates: CommissionRate[] = [
  { productType: "Mutual Funds", trailCommission: 0.5, upfrontCommission: 1.0, minInvestment: 5000, description: "Equity & Debt MFs" },
  { productType: "Bonds", trailCommission: 0.25, upfrontCommission: 0.5, minInvestment: 10000, description: "Corporate & Government Bonds" },
  { productType: "Insurance", trailCommission: 2.0, upfrontCommission: 15.0, minInvestment: 25000, description: "Life & Health Insurance" },
  { productType: "Unlisted Stocks", trailCommission: 0.0, upfrontCommission: 2.5, minInvestment: 100000, description: "Pre-IPO & Private Equity" },
  { productType: "REITs", trailCommission: 0.3, upfrontCommission: 0.75, minInvestment: 50000, description: "Real Estate Investment Trusts" },
];
...
export default function AgentCommissionCalculator() {
  ...
  const formatInputCurrency = (value: string) => {
    ...
    if (!numValue) return '';
    return parseInt(numValue).toLocaleString('en-IN');
  };
  ...
  const scenarioCalculations = useMemo(() => {
    return scenarios.map(scenario => {
      const rate = rates.find(r => r.productType === scenario.productType) || rates[0];
                    ...
                    <TableCell className="text-foreground font-medium">
                      <div className="flex items-center gap-2">
                        ...
                        {rate.productType === productType && (
                          <Badge className="bg-emerald-600 text-xs">Selected</Badge>
                        )}
                      </div>
                    </TableCell>
