...
import {
  Bookmark, Plus, Trash2, X, RefreshCw, Star, TrendingUp,
  ChevronRight, ArrowLeft, AlertTriangle, Search,
} from "lucide-react";
...
export default function WatchlistsPanel({ accountId }: WatchlistsPanelProps) {
  ...
  const { data, isLoading, error, refetch } = useQuery<{ success: boolean; watchlists: Watchlist[] }>({
    queryKey: qKey,
    queryFn: () => fetch(`/api/broker/accounts/${accountId}/watchlists`).then(r => r.json()),
    enabled: !!accountId,
    staleTime: 30_000,
  });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (res: any) => {
      toast({ title: "Watchlist created", description: `"${res.watchlist?.name}" is ready.` });
  ...
  const deleteMutation = useMutation({
    mutationFn: (watchlistId: string) =>
      apiRequest(`/api/broker/accounts/${accountId}/watchlists/${watchlistId}`, "DELETE", {}),
    onSuccess: () => {
  ...
  const addSymbolMutation = useMutation({
    mutationFn: ({ watchlistId, symbol }: { watchlistId: string; symbol: string }) =>
      apiRequest(`/api/broker/accounts/${accountId}/watchlists/${watchlistId}/symbols`, "POST", { symbol }),
    onSuccess: (res: any) => {
      toast({ title: "Symbol added", description: `${addSymbolInput.toUpperCase()} added to watchlist.` });
      queryClient.invalidateQueries({ queryKey: qKey });
      if (res.watchlist) setSelectedWatchlist(res.watchlist);
      setAddSymbolInput("");
    },
    onError: (err: any) => {
      toast({ title: "Add symbol failed", description: err.message, variant: "destructive" });
    },
  });
  ...
  const removeSymbolMutation = useMutation({
    ...
    onSuccess: (_, vars) => {
      toast({ title: "Symbol removed", description: `${vars.symbol} removed from watchlist.` });
      queryClient.invalidateQueries({ queryKey: qKey });
      setSelectedWatchlist(prev =>
        prev ? { ...prev, assets: prev.assets.filter(a => a.symbol !== vars.symbol) } : null,
      );
    },
    onError: (err: any) => {
  ...
  function handleAddSymbol() {
    ...
    addSymbolMutation.mutate({ watchlistId: selectedWatchlist.id, symbol: sym });
  }

  const filteredAssets = selectedWatchlist
    ? selectedWatchlist.assets.filter(a =>
        symbolFilter ? a.symbol.includes(symbolFilter.toUpperCase()) || a.name.toLowerCase().includes(symbolFilter.toLowerCase()) : true,
      )
    : [];
  ...
  if (error) {
    return (
      <Alert variant="destructive">
        ...
        <AlertDescription>Failed to load watchlists. {(error as any).message}</AlertDescription>
      </Alert>
    );
  }
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="sm" onClick={() => { setSelectedWatchlist(null); setSymbolFilter(""); }}>
            <ArrowLeft className="h-4 w-4 mr-1" /> Back
          </Button>
          <div className="flex-1 min-w-0">
            <h3 className="font-semibold text-base truncate">{selectedWatchlist.name}</h3>
            <p className="text-xs text-muted-foreground">{selectedWatchlist.assets.length} symbol{selectedWatchlist.assets.length !== 1 ? "s" : ""}</p>
          </div>
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  "{selectedWatchlist.name}" and all its symbols will be permanently deleted.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
          ...
          <CardContent>
            <div className="flex gap-2">
              ...
              <Button
                ...
                size="sm"
              >
                {addSymbolMutation.isPending ? <RefreshCw className="h-4 w-4 animate-spin" /> : <Plus className="h-4 w-4" />}
        ...
        <Card className="border-dashed">
          <CardContent className="flex flex-col items-center justify-center py-14 gap-3">
            ...
            <div className="text-center">
              <p className="font-medium text-muted-foreground">No watchlists yet</p>
              ...
              <CardContent className="p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">
                      {wl.assets.length} symbol{wl.assets.length !== 1 ? "s" : ""}
                    </p>
                    {wl.assets.length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-2">
                        {wl.assets.slice(0, 5).map(a => (
                          <Badge key={a.id} variant="secondary" className="text-xs font-mono px-1.5 py-0">
                            {a.symbol}
                          </Badge>
                        ))}
                        {wl.assets.length > 5 && (
      ...
      <Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
        <DialogContent>
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-1.5">
              <label className="text-sm font-medium">Initial Symbols <span className="text-muted-foreground font-normal">(optional)</span></label>
              <Input
                placeholder="AAPL, MSFT, NVDA"
                value={newWatchlistSymbols}
                onChange={e => setNewWatchlistSymbols(e.target.value.toUpperCase())}
              />
              <p className="text-xs text-muted-foreground">Comma or space separated. Max 30 symbols.</p>
            </div>
          </div>
          <DialogFooter>
