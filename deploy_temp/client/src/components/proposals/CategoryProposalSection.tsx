...
interface CategoryProposalSectionProps {
  category: 'mutual_fund' | 'equity' | 'bond' | 'ipo' | 'unlisted' | 'insurance' | 'loan' | 'aif' | 'pms';
  title?: string;
  showViewAll?: boolean;
}

const sourceConfig: Record<ProposalSource, { label: string; icon: any; color: string }> = {
  ai_rebalancing: { label: "AI", icon: Brain, color: "bg-purple-100 text-purple-700 dark:bg-purple-900 dark:text-purple-300" },
...
export default function CategoryProposalSection({ 
  ...
  const { data: proposals = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(`/api/unified-proposals/by-category/${category}`, { credentials: 'include' });
  ...
  const addToCartMutation = useMutation({
    mutationFn: async (proposalId: string) => {
      const response = await apiRequest('POST', `/api/unified-proposals/${proposalId}/add-to-cart`, { orderType: 'LUMPSUM' });
      return response.json();
    },
    onSuccess: () => {
      toast({ title: "Added to cart" });
      queryClient.invalidateQueries({ queryKey: ['/api/unified-proposals/by-category', category] });
  ...
  const acceptedProposals = proposals.filter(p => p.status === 'accepted' && !p.addedToCart);
    ...
    <Card className="border-primary/20 bg-gradient-to-br from-primary/5 to-transparent">
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>
              {pendingProposals.length} pending, {acceptedProposals.length} ready for cart
            </CardDescription>
          </div>
          {showViewAll && (
            <Button 
      ...
      <CardContent className="space-y-3">
        {proposals.slice(0, 3).map((proposal) => {
          ...
          const canAddToCart = proposal.status === 'accepted' && !proposal.addedToCart;
