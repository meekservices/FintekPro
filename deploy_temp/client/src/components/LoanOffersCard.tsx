...
import { CreditCard, TrendingDown, Calendar, CheckCircle2, ArrowRight } from "lucide-react";
...
export function LoanOffersCard() {
  ...
  const activeOffers = offers.filter(offer => 
    offer.status === 'pending' || offer.status === 'in_progress'
  );
    ...
    <Card className="w-full" data-testid="card-loan-offers">
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="text-2xl flex items-center gap-2" data-testid="text-loan-offers-title">
              <CreditCard className="h-6 w-6 text-primary" />
              Your Pre-Approved Loan Offers
            </CardTitle>
            <CardDescription data-testid="text-loan-offers-description">
              Compare personalized loan offers from leading lenders and choose the best option
            </CardDescription>
          </div>
          <Badge variant="secondary" className="text-sm" data-testid="badge-offers-count">
            {activeOffers.length} {activeOffers.length === 1 ? 'Offer' : 'Offers'} Available
          </Badge>
        </div>
      </CardHeader>
      <CardContent>
        <div className="overflow-x-auto">
          <table className="w-full border-collapse" data-testid="table-loan-offers">
            <thead>
                ...
                >
                  <td className="p-3">
                    <div className="flex flex-col">
                      <span className="font-medium" data-testid={`text-lender-${offer.id}`}>
                        {offer.lenderName}
                      </span>
                  ...
                  <td className="p-3 text-right font-semibold text-lg" data-testid={`text-amount-${offer.id}`}>
                    {formatCurrency(offer.offerAmount)}
                  </td>
                  <td className="p-3 text-right" data-testid={`text-rate-${offer.id}`}>
                    <div className="flex items-center justify-end gap-1">
                      <TrendingDown className="h-4 w-4 text-green-600" />
                      <span className="font-medium">{offer.interestRate}%</span>
                    </div>
                    <span className="text-xs text-muted-foreground">per annum</span>
                  </td>
                  <td className="p-3 text-right" data-testid={`text-tenure-${offer.id}`}>
                    <div className="flex items-center justify-end gap-1">
                      <Calendar className="h-4 w-4 text-muted-foreground" />
                      <span>{offer.tenureMonths} months</span>
                    </div>
                  </td>
                  <td className="p-3 text-right font-medium" data-testid={`text-emi-${offer.id}`}>
                    {formatCurrency(offer.emi)}
                    <div className="text-xs text-muted-foreground">per month</div>
                  </td>
                  <td className="p-3 text-right" data-testid={`text-fee-${offer.id}`}>
                    {formatCurrency(offer.processingFee)}
                  </td>
                  <td className="p-3">
                    <ul className="space-y-1" data-testid={`list-features-${offer.id}`}>
                  ...
                  <td className="p-3 text-center">
                    <Button
                      ...
                      disabled={processingOfferId === offer.id || proceedMutation.isPending}
                      size="sm"
                      className="gap-2"
        ...
        <div className="mt-4 p-4 bg-muted/30 rounded-lg">
          <p className="text-sm text-muted-foreground">
            <strong>Note:</strong> These are pre-approved offers based on your credit profile. 
            Final approval is subject to the lender's verification process. Clicking "Proceed" will 
            redirect you to the lender's application portal.
          </p>
        </div>
      </CardContent>
    </Card>
