...
export function ABTestingDashboard({ className = "" }: { className?: string }) {
  ...
  const killSwitchMutation = useMutation({
    mutationFn: async (data: { action: "activate" | "deactivate"; reason?: string }) => {
      const response = await apiRequest("POST", "/api/admin/recommendations/kill-switch", data);
      return response.json();
    },
    onSuccess: (data, variables) => {
      toast({
        title: variables.action === "activate" ? "Kill Switch Activated" : "Kill Switch Deactivated",
        description: variables.action === "activate" 
          ? "Growth-Optimized mode has been disabled." 
          : "Growth-Optimized mode is now available.",
      });
      queryClient.invalidateQueries({ queryKey: ["/api/admin/recommendations/kill-switch"] });
  ...
  const getSignificanceBadge = (significance?: number) => {
    ...
    if (significance >= 95) {
      return <Badge className="bg-green-500">Highly Significant ({significance}%)</Badge>;
    }
    if (significance >= 90) {
      return <Badge className="bg-blue-500">Significant ({significance}%)</Badge>;
    }
      ...
      <div className="p-4 border rounded-lg">
        ...
        <div className="mt-2 pt-2 border-t">
          <p className="text-xs text-muted-foreground">
            Difference: {diff > 0 ? "+" : ""}{formatValue(diff)} ({diffPercent}%)
          </p>
        </div>
      </div>
    );
    ...
    <div className={`space-y-6 ${className}`}>
          ...
          <DialogContent>
            ...
            <DialogFooter>
              ...
              <Button
                ...
                disabled={killSwitchMutation.isPending || (!killSwitchActive && !killSwitchReason)}
      ...
      {killSwitchActive && (
        <Alert variant="destructive" data-testid="kill-switch-active-alert">
          ...
          <AlertDescription>
            Growth-Optimized mode is currently disabled. Reason: {killSwitchData?.status?.reason}
          </AlertDescription>
        </Alert>
      )}
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-sm text-muted-foreground">Group A (Balanced)</p>
                <p className="text-2xl font-bold">{summary?.groupACount || 0}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-sm text-muted-foreground">Group B (Growth)</p>
                <p className="text-2xl font-bold">{summary?.groupBCount || 0}</p>
              </div>
            </div>
          </CardContent>
        </Card>
              ...
              <div className="space-y-4">
                <div className="p-4 border rounded-lg">
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <Badge variant="outline">{thresholdsData?.thresholds?.maxDrawdown || 15}%</Badge>
                  </div>
                  <p className="text-sm text-muted-foreground">
                    Kill switch activates if portfolio drawdown exceeds this threshold
                  </p>
                </div>
                ...
                <div className="p-4 border rounded-lg">
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <Badge variant="outline">{thresholdsData?.thresholds?.maxComplaintRate || 2}%</Badge>
                  </div>
                  <p className="text-sm text-muted-foreground">
                    Kill switch activates if client complaint rate exceeds this threshold
                  </p>
                </div>

                <div className="p-4 border rounded-lg">
                  <div className="flex justify-between items-center mb-2">
                    <span className="font-medium">Max Restricted Asset Exposure</span>
                    <Badge variant="outline">{thresholdsData?.thresholds?.maxRestrictedAssetExposure || 30}%</Badge>
                  </div>
                  <p className="text-sm text-muted-foreground">
                    Kill switch activates if exposure to restricted assets exceeds this threshold
                  </p>
                </div>
              </div>
            </CardContent>
