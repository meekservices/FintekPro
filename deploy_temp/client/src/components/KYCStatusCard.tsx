...
export function KYCStatusCard() {
  ...
  const getStatusBadge = () => {
    if (status.requiresReKYC) {
      return (
        <Badge variant="destructive" className="gap-1" data-testid="kyc-badge-expired">
    ...
    if (status.isActive) {
      return (
        <Badge variant="default" className="gap-1 bg-green-600 hover:bg-green-700" data-testid="kyc-badge-active">
    ...
    return (
      <Badge variant="outline" className="gap-1" data-testid="kyc-badge-incomplete">
    ...
    <Card data-testid="kyc-status-card" className="border-2">
      ...
      <CardContent className="space-y-6">
        ...
        {status.dueDate && (
          <div className="rounded-lg border p-3 space-y-1">
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm text-muted-foreground" data-testid="kyc-due-date">
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between p-2 rounded-lg border">
            ...
            {status.canTradeMutualFunds ? (
              <Badge variant="default" className="bg-green-600" data-testid="permission-mf">
          ...
          <div className="flex items-center justify-between p-2 rounded-lg border">
            ...
            {status.canTradeBroking ? (
              <Badge variant="default" className="bg-green-600" data-testid="permission-broking">
                <CheckCircle2 className="h-3 w-3 mr-1" />
                Enabled
              </Badge>
            ) : (
          ...
          <div className="flex items-center justify-between p-2 rounded-lg border">
            ...
            {status.canTradeInternational ? (
              <Badge variant="default" className="bg-green-600" data-testid="permission-international">
                <CheckCircle2 className="h-3 w-3 mr-1" />
                Enabled
              </Badge>
            ) : (
        ...
        <div className="text-xs text-muted-foreground space-y-1">
          <p>Risk Category: <span className="font-medium capitalize">{status.riskCategory}</span></p>
          <p>Review Frequency: <span className="font-medium">{status.reviewFrequency.replace("_", " ")}</span></p>
          {status.lastUpdated && (
            <p>Last Updated: {new Date(status.lastUpdated).toLocaleDateString()}</p>
          )}
        </div>
      </CardContent>
    </Card>
