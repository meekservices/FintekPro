...
import { 
  FileText, Edit3, MessageSquare, Check, X, Clock, Send, 
  Eye, History, Lock, Unlock, AlertTriangle, Users, CheckCircle
} from 'lucide-react';
...
export default function DocumentEditor({ workflowId, proposalId, canEdit = false, canComment = true, canApprove = false }: DocumentEditorProps) {
  ...
  const { data: editableFields } = useQuery<{ success: boolean; editableFields: EditableField[] }>({
    queryKey: ['/api/proposal-esign/agreements', proposalId, 'fields'],
    enabled: !!proposalId,
  });

  const recordEditMutation = useMutation({
    mutationFn: async (data: { fieldName: string; previousValue?: string; newValue: string; changeType: 'modify' }) => {
  ...
  const approveEditMutation = useMutation({
    mutationFn: async (editId: string) => {
      return apiRequest(`/api/proposal-esign/edits/${editId}/approve`, { method: 'POST' });
    },
    onSuccess: () => {
  ...
  const addCommentMutation = useMutation({
    mutationFn: async (data: { content: string; commentType: string }) => {
      return apiRequest(`/api/proposal-esign/workflows/${workflowId}/comments`, {
        method: 'POST',
        body: JSON.stringify(data),
      });
    },
  ...
  const resolveCommentMutation = useMutation({
    mutationFn: async (commentId: string) => {
      return apiRequest(`/api/proposal-esign/comments/${commentId}/resolve`, { method: 'POST' });
    },
    onSuccess: () => {
          ...
          <Card>
                  ...
                  <div key={field.id} className="space-y-2">
                    ...
                    {editingField === field.id ? (
                      <div className="flex gap-2">
                        ...
                        <Button
                          size="sm"
                          onClick={() => handleFieldEdit(field.id, fieldValues[field.id])}
                        ...
                        <Button
                          ...
                          onClick={() => {
                            setEditingField(null);
                            setFieldValues({ ...fieldValues, [field.id]: field.currentValue });
                          }}
                        >
                          <X className="h-4 w-4" />
                        </Button>
                      ...
                      >
                        <span className={field.type === 'currency' ? 'font-mono' : ''}>
                          {field.currentValue || '-'}
                        </span>
                        {canEdit && field.isEditable && !isLocked && (
            ...
            <CardFooter>
              <Button
                variant="outline"
                className="w-full"
                onClick={() => window.open(`/api/proposal-esign/agreements/${proposalId}/preview`, '_blank')}
              >
                    ...
                    {pendingEdits.map((edit: FieldEdit) => (
                      <div key={edit.id} className="border rounded-lg p-4 space-y-3">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <Badge variant={
                              edit.approvalStatus === 'pending' ? 'secondary' :
                              edit.approvalStatus === 'approved' ? 'default' : 'destructive'
                            }>
                              {edit.approvalStatus}
                            </Badge>
                            ...
                            <Dialog>
                              ...
                              <DialogContent>
                                ...
                                <form onSubmit={(e) => {
                                  ...
                                  rejectEditMutation.mutate({ editId: edit.id, reason });
                                }}>
                                  <Textarea name="reason" placeholder="Reason for rejection..." required />
                                  <DialogFooter className="mt-4">
                                    <Button type="submit" variant="destructive">Reject Edit</Button>
                                  </DialogFooter>
                                </form>
                              </DialogContent>
                            </Dialog>
                ...
                <div className="space-y-3 border-b pb-4">
                  <div className="flex gap-2">
                    ...
                    <div className="flex-1 flex gap-2">
                      ...
                      />
                      <Button
                        onClick={() => addCommentMutation.mutate({ content: newComment, commentType })}
                    ...
                    {comments.map((comment: Comment) => (
                      <div 
                        ...
                        className={`p-3 rounded-lg border ${
                          comment.threadResolved ? 'bg-muted/30 opacity-60' : 'bg-card'
                        }`}
                      >
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center gap-2">
                            <Badge variant={
                              comment.commentType === 'issue' ? 'destructive' :
                              comment.commentType === 'question' ? 'secondary' :
                              comment.commentType === 'suggestion' ? 'outline' : 'default'
                            }>
                              {comment.commentType}
                            </Badge>
                            <span className="text-sm font-medium">{comment.authorName || 'Anonymous'}</span>
                          </div>
                          <div className="flex items-center gap-2">
                    ...
                    {versions.map((version: any, index: number) => (
                      <div key={version.id} className="flex gap-4">
                        ...
                        <div className="flex-1 pb-4">
                          <div className="flex items-center justify-between">
                            <div>
                              <span className="font-medium">{version.versionLabel || `Version ${version.versionNumber}`}</span>
                              {version.isLocked && (
                          ...
                          <p className="text-xs text-muted-foreground mt-1">
                            Round {version.negotiationRound} • By {version.createdByName || 'System'}
                          </p>
                        </div>
                      </div>
                    ))}
