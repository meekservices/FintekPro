...
import { Settings, Save, X, Banknote, Building2, CreditCard, CheckCircle2, Info } from "lucide-react";
...
const PRODUCT_TYPES = [
  { value: "mutual_fund", label: "Mutual Funds", requiresDemat: false, icon: <Banknote className="h-4 w-4" /> },
  { value: "ipo", label: "IPO Applications", requiresDemat: true, icon: <Building2 className="h-4 w-4" /> },
  { value: "bond", label: "Bonds", requiresDemat: true, icon: <CreditCard className="h-4 w-4" /> },
  { value: "equity", label: "Equity Trading", requiresDemat: true, icon: <Building2 className="h-4 w-4" /> },
...
export function ProductAccountPreferences() {
  ...
  const [preferences, setPreferences] = useState<Record<string, { bankAccountId: string | null; dematAccountId: string | null }>>({});
  ...
  useEffect(() => {
    if (existingPreferences) {
      const prefsMap: Record<string, { bankAccountId: string | null; dematAccountId: string | null }> = {};
  ...
  const savePreferenceMutation = useMutation({
    mutationFn: async ({ productType, bankAccountId, dematAccountId }: { 
      productType: string; 
      bankAccountId: string | null; 
      dematAccountId: string | null;
    }) => {
      const existingPref = existingPreferences?.find(p => p.productType === productType);
      
      if (existingPref) {
        // Update existing preference
        return apiRequest("PUT", `/api/product-account-preferences/${existingPref.id}`, {
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/product-account-preferences"] });
      toast({
  ...
  const handleBankAccountChange = (productType: string, bankAccountId: string) => {
    const newPrefs = {
      ...preferences,
      [productType]: {
  ...
  const handleDematAccountChange = (productType: string, dematAccountId: string) => {
    const newPrefs = {
      ...preferences,
      [productType]: {
        ...preferences[productType],
  ...
  const handleSave = (productType: string) => {
    const pref = preferences[productType] || { bankAccountId: null, dematAccountId: null };
  ...
  if (!bankAccounts?.length && !dematAccounts?.length) {
    return (
      <Alert data-testid="alert-no-accounts">
        ...
        <AlertDescription>
          Please add bank and demat accounts first before configuring product preferences.
        </AlertDescription>
      </Alert>
    );
  }
  ...
  return (
    <Card data-testid="card-product-preferences">
      <CardHeader>
        ...
        <CardDescription>
          Configure which bank and demat accounts to use for each product type. This helps streamline your transactions.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        {PRODUCT_TYPES.map((product) => {
          const pref = preferences[product.value] || { bankAccountId: null, dematAccountId: null };
          const isChanged = hasUnsavedChanges(product.value);
          
          return (
            <div key={product.value} className="border rounded-lg p-4 space-y-4" data-testid={`product-pref-${product.value}`}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  {product.icon}
                  <h3 className="font-semibold">{product.label}</h3>
                  {product.requiresDemat && (
                    <Badge variant="secondary" className="text-xs">Requires Demat</Badge>
                  )}
                </div>
                {isChanged && (
                  <Button 
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) => handleBankAccountChange(product.value, value)}
                ...
                {product.requiresDemat && (
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) => handleDematAccountChange(product.value, value)}
