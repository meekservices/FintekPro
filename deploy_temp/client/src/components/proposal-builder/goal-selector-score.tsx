...
type InvestmentGoal = "WEALTH_CREATION" | "RETIREMENT" | "CHILD_EDUCATION" | "INCOME";
...
const goalConfig: Record<InvestmentGoal, { label: string; icon: typeof Target; description: string }> = {
  WEALTH_CREATION: {
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        <div>
          <label className="text-sm font-medium mb-2 block">Investment Goal</label>
          <Select value={selectedGoal} onValueChange={(v) => handleGoalChange(v as InvestmentGoal)}>
            <SelectTrigger>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between p-4 bg-muted/30 rounded-lg">
              <div className="flex items-center gap-3">
                <div className="relative w-16 h-16">
                  <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                    ...
                    <circle
                      ...
                      className={cn(
                        scoreData.score >= 75 ? "text-green-500" :
                        scoreData.score >= 60 ? "text-blue-500" :
                        scoreData.score >= 40 ? "text-amber-500" : "text-red-500"
                      )}
                    />
                  </svg>
                ...
                <div>
                  ...
                  <Badge variant="outline" className={cn("mt-1", gradeColors[scoreData.grade])}>
                    {scoreData.grade}
                  </Badge>
                </div>
              </div>
              <div className="text-right">
                ...
                <Badge variant="outline" className={cn("mt-1", gradeColors[scoreData.riskAlignment])}>
            ...
            <div className="p-3 bg-blue-50 dark:bg-blue-950/30 rounded-lg">
              <p className="text-xs text-blue-700 dark:text-blue-300">
                <strong>Score adjusted for {scoreData.goal} goal:</strong>{" "}
                Stock overlap penalties are multiplied by {scoreData.goalAdjustments.stockOverlapMultiplier}x,
                and sector concentration penalties by {scoreData.goalAdjustments.sectorPenaltyMultiplier}x.
              </p>
            </div>
                ...
                <div className="max-h-24 overflow-y-auto space-y-1">
                  {scoreData.penalties.slice(0, 3).map((penalty, idx) => (
                    <div key={idx} className="flex items-center justify-between text-xs p-2 bg-muted/50 rounded">
                      ...
                      <span className="text-red-600 font-medium">{penalty.impact}</span>
                    </div>
                  ))}
                </div>
              </div>
