...
const SOURCE_CONFIG: Record<SipSource, { label: string; color: string; description: string }> = {
  rebalancing: { 
    label: 'Rebalancing', 
...
export function SipAttribution({ proposalId }: SipAttributionProps) {
  ...
  const { data: summary, isLoading } = useQuery<SipSummary>({
    ...
    queryFn: async () => {
      const response = await fetch(`/api/proposal-builder/sips/${proposalId}/summary`);
  ...
  const convertLumpsum = useMutation({
    mutationFn: async () => {
      return apiRequest(`/api/proposal-builder/sips/${proposalId}/convert-lumpsum`, {
        method: 'POST',
        body: JSON.stringify({
          instrumentName: selectedInstrument,
          lumpsumAmount: parseFloat(lumpsumAmount),
          sipDurationMonths: parseInt(sipDuration)
        })
      });
    },
    onSuccess: (data: any) => {
      queryClient.invalidateQueries({ queryKey: ['/api/proposal-builder/sips', proposalId] });
  ...
  const deleteSip = useMutation({
    mutationFn: async (instrumentName: string) => {
      return apiRequest(`/api/proposal-builder/sips/${proposalId}/${encodeURIComponent(instrumentName)}`, {
        method: 'DELETE'
      });
    },
    onSuccess: () => {
          ...
          <>
            <div className="grid grid-cols-4 gap-4">
              <div className="text-center p-3 bg-muted rounded-lg">
                <div className="text-2xl font-bold">{formatCurrency(summary.totalMonthlyAmount)}</div>
                <div className="text-xs text-muted-foreground">Total Monthly</div>
              </div>
              <div className="text-center p-3 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
                <div className="text-lg font-bold text-purple-600">{summary.rebalancingSipCount}</div>
                <div className="text-xs text-muted-foreground">Rebalancing</div>
                <div className="text-sm">{formatCurrency(summary.rebalancingSipAmount)}</div>
              </div>
              <div className="text-center p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
                <div className="text-lg font-bold text-green-600">{summary.freshSipCount}</div>
                <div className="text-xs text-muted-foreground">Fresh</div>
                <div className="text-sm">{formatCurrency(summary.freshSipAmount)}</div>
              </div>
              <div className="text-center p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                <div className="text-lg font-bold text-blue-600">{summary.hybridSipCount}</div>
                <div className="text-xs text-muted-foreground">Hybrid</div>
                <div className="text-sm">{formatCurrency(summary.hybridSipAmount)}</div>
              </div>
            </div>
                  ...
                  {summary.recommendations.map((sip) => {
                    ...
                    return (
                      <TableRow key={sip.instrumentName}>
                        <TableCell>
                          <div>
                            <div className="font-medium">{sip.instrumentName}</div>
                            {sip.convertedFromLumpsum && (
                              <div className="text-xs text-muted-foreground">
                                Converted from {formatCurrency(sip.originalLumpsumAmount || 0)} lumpsum
                              </div>
                            )}
                          </div>
                        </TableCell>
                        <TableCell className="text-center">
                          <TooltipProvider>
                            <Tooltip>
                              ...
                              <TooltipContent>
                                <p className="max-w-xs">{sip.sourceRationale || config.description}</p>
                              </TooltipContent>
                            </Tooltip>
                          </TooltipProvider>
                        </TableCell>
                              ...
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <Button
                                    variant="ghost"
                                    size="icon"
                                    onClick={() => handleConvertClick(sip.instrumentName)}
                                  >
                                    <ArrowRightLeft className="h-4 w-4" />
                                  </Button>
                                </TooltipTrigger>
                                <TooltipContent>Convert Lumpsum to SIP</TooltipContent>
                              </Tooltip>
                            </TooltipProvider>
                            <Button
                              variant="ghost"
                              size="icon"
                              onClick={() => deleteSip.mutate(sip.instrumentName)}
                            >
        ...
        <Dialog open={showConvertDialog} onOpenChange={setShowConvertDialog}>
          <DialogContent>
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium">SIP Duration (months)</label>
                <Input
                  type="number"
                  value={sipDuration}
              ...
              {lumpsumAmount && sipDuration && (
                <div className="p-3 bg-muted rounded-lg">
                  <div className="text-sm text-muted-foreground">Monthly SIP Amount:</div>
                  <div className="text-xl font-bold">
                    {formatCurrency(parseFloat(lumpsumAmount) / parseInt(sipDuration))}
                  </div>
                </div>
              )}
            </div>
            <DialogFooter>
              ...
              >
                {convertLumpsum.isPending && <Loader2 className="h-4 w-4 mr-2 animate-spin" />}
                Convert to SIP
              </Button>
            </DialogFooter>
          </DialogContent>
