...
export function SmartReportBuilder({ proposalId }: SmartReportBuilderProps) {
  ...
  const { data: sectionsData, isLoading } = useQuery<{ proposalId: string; sections: ReportSectionStatus[] }>({
    queryKey: ['/api/proposal-builder/report-sections', proposalId],
    queryFn: async () => {
      const response = await fetch(`/api/proposal-builder/report-sections/${proposalId}`);
  ...
  const toggleSection = useMutation({
    mutationFn: async ({ sectionCode, enabled }: { sectionCode: string; enabled: boolean }) => {
      return apiRequest(`/api/proposal-builder/report-sections/${proposalId}/toggle`, {
        method: 'POST',
        body: JSON.stringify({ sectionCode, enabled, byAgent: true })
      });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/proposal-builder/report-sections', proposalId] });
    },
    onError: (error: any) => {
      toast({
        title: 'Toggle Failed',
  ...
  const autoSelect = useMutation({
    mutationFn: async () => {
      return apiRequest(`/api/proposal-builder/report-sections/${proposalId}/auto-select`, {
        method: 'POST'
      });
    },
    onSuccess: (data: any) => {
                  ...
                  {!section.dependencyMet ? (
                    <Tooltip>
                      ...
                      <TooltipContent side="left" className="max-w-xs">
                        <p className="font-medium text-destructive">Missing Data</p>
                    ...
                    <Switch
                      ...
                      onCheckedChange={(checked) => 
                        toggleSection.mutate({ sectionCode: section.code, enabled: checked })
                      }
                    />
                  )}
                </div>
