...
interface ConsentHook {
  checkConsent: (panNumber: string, schemeType: SchemeType) => Promise<ConsentStatus>;
  grantConsent: (panNumber: string, schemeType: SchemeType, purpose?: string) => Promise<void>;
...
export function useConsent(): ConsentHook {
  ...
  const checkConsentQuery = async (panNumber: string, schemeType: SchemeType): Promise<ConsentStatus> => {
    const response = await fetch(`/api/government-schemes/consent/${panNumber}/${schemeType}`, {
      credentials: 'include'
    });
  ...
  const grantConsentMutation = useMutation({
    mutationFn: async ({ panNumber, schemeType, purpose }: { 
      panNumber: string; 
      schemeType: SchemeType; 
      purpose?: string 
    }) => {
  ...
  const revokeConsentMutation = useMutation({
    mutationFn: async ({ panNumber, schemeType }: { panNumber: string; schemeType: SchemeType }) => {
      const response = await fetch(`/api/government-schemes/consent/${panNumber}/${schemeType}`, {
        method: "DELETE",
        credentials: 'include'
      });
  ...
  return {
    ...
    grantConsent: async (panNumber: string, schemeType: SchemeType, purpose?: string) => {
      await grantConsentMutation.mutateAsync({ panNumber, schemeType, purpose });
    },
