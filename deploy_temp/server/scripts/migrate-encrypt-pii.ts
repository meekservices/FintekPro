  ...
  try {
    ...
    for (const user of allUsers) {
      try {
        ...
        const isAadhaarEncrypted = user.aadharNumber && user.aadharNumber.length > 50;
        const isBankEncrypted = user.bankAccountNumber && user.bankAccountNumber.length > 50;

        // Skip if all data is already encrypted or empty
        if ((!user.panNumber || isPanEncrypted) && 
            (!user.aadharNumber || isAadhaarEncrypted) && 
            (!user.bankAccountNumber || isBankEncrypted)) {
          stats.skipped++;
          console.log(`⏭️  Skipped user ${user.userId} - already encrypted or no PII data`);
          continue;
        }
        ...
        if (encryptedData.aadhaar) updateData.aadharNumber = encryptedData.aadhaar;
        if (encryptedData.bankAccount) updateData.bankAccountNumber = encryptedData.bankAccount;
