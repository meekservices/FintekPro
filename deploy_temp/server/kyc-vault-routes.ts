...
export function registerKYCVaultRoutes(app: Express) {
   ...
   */
  app.post('/api/kyc-vault/initiate', kycRateLimiter, async (req: Request, res: Response) => {
    try {
      ...
      const result = await kycWorkflowOrchestrator.initiateOKYC(aadhaarNumber);
   ...
   */
  app.post('/api/kyc-vault/verify-otp', kycRateLimiter, async (req: Request, res: Response) => {
    try {
   ...
   */
  app.post('/api/kyc-vault/complete', kycRateLimiter, async (req: any, res: Response) => {
    try {
      const { 
        panNumber, 
        aadhaarNumber, 
   ...
   */
  app.post('/api/kyc-vault/token/generate', kycTokenRateLimiter, async (req: any, res: Response) => {
    try {
      // Require authentication
      if (!req.user || !req.user.id) {
        return res.status(401).json({
   ...
   */
  app.post('/api/kyc-vault/token/validate', kycTokenRateLimiter, async (req: Request, res: Response) => {
    try {
  ...
  app.get('/api/kyc-vault/status', async (req: any, res: Response) => {
    try {
      ...
      res.json({
        ...
        needsRenewal: vault.kycNextRenewalDate ? new Date() >= new Date(vault.kycNextRenewalDate) : false
      });
    } catch (error: any) {
