...
export class YieldTrackerService {
  ...
  static async updateTrackerPrice(trackerId: string, currentPrice: number, marketData: any = {}): Promise<YieldTracker | undefined> {
  ...
  static calculateYieldMetrics(tracker: YieldTracker): YieldMetrics {
    ...
    const downDeviation = this.calculateDownsideDeviation(returns);
  ...
  static calculateBenchmarkComparison(tracker: YieldTracker): BenchmarkComparison {
    ...
    return {
      ...
      upCaptureRatio: trackerReturn > 0 ? (trackerReturn / Math.max(benchmarkReturn, 0.01)) * 100 : 0,
      downCaptureRatio: trackerReturn < 0 ? (Math.abs(trackerReturn) / Math.max(Math.abs(benchmarkReturn), 0.01)) * 100 : 0
    };
  }

  // Generate comprehensive performance analysis
  static async generatePerformanceAnalysis(userId: string, period: string = '1Y'): Promise<PerformanceAnalysis[]> {
  ...
  static async calculatePortfolioYield(userId: string, portfolioId?: string): Promise<{
    totalValue: number;
    ...
    Object.keys(sectorAllocation).forEach(sector => {
      sectorAllocation[sector] = (sectorAllocation[sector] / totalInvestment) * 100;
    });
  ...
  private static calculateBeta(returns: number[], benchmarkReturn: number): number {
  ...
  static async generateOptimizationSuggestions(userId: string): Promise<Array<{
