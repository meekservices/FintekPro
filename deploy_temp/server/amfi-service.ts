...
class AMFIService {
   ...
   */
  private findNAVForDate(navData: NAVData[], targetDate: Date, maxDaysBack: number = 7): NAVData | null {
    ...
    for (const nav of sortedData) {
      ...
      const daysDiff = Math.abs(targetTime - navDate.getTime()) / (1000 * 60 * 60 * 24);
   ...
   */
  private calculateAnnualizedReturn(startNav: number, endNav: number, years: number): number {
   ...
   */
  async calculateFundPerformance(schemeCode: string): Promise<FundPerformance | null> {
    try {
      ...
      const currentNav = parseFloat(currentNAV.nav);
      ...
      const navThreeYear = this.findNAVForDate(fundData.data, threeYearsAgo, 14); // Allow 14 days back for longer periods
      ...
      const returns: CalculatedReturns = {
        '1M': navOneMonth ? this.calculateSimpleReturn(parseFloat(navOneMonth.nav), currentNav) : null,
        '6M': navSixMonth ? this.calculateSimpleReturn(parseFloat(navSixMonth.nav), currentNav) : null,
        '1Y': navOneYear ? this.calculateAnnualizedReturn(parseFloat(navOneYear.nav), currentNav, 1) : null,
        '3Y': navThreeYear ? this.calculateAnnualizedReturn(parseFloat(navThreeYear.nav), currentNav, 3) : null,
        '5Y': navFiveYear ? this.calculateAnnualizedReturn(parseFloat(navFiveYear.nav), currentNav, 5) : null,
      };

      // Format return strings
      const returnStrings = {
        '1M': returns['1M'] !== null ? `${returns['1M'] >= 0 ? '+' : ''}${returns['1M'].toFixed(2)}%` : 'N/A',
        '6M': returns['6M'] !== null ? `${returns['6M'] >= 0 ? '+' : ''}${returns['6M'].toFixed(2)}%` : 'N/A',
        '1Y': returns['1Y'] !== null ? `${returns['1Y'] >= 0 ? '+' : ''}${returns['1Y'].toFixed(2)}%` : 'N/A',
        '3Y': returns['3Y'] !== null ? `${returns['3Y'] >= 0 ? '+' : ''}${returns['3Y'].toFixed(2)}%` : 'N/A',
        '5Y': returns['5Y'] !== null ? `${returns['5Y'] >= 0 ? '+' : ''}${returns['5Y'].toFixed(2)}%` : 'N/A',
      };
    ...
    try {
      ...
      const searchPromises = matchingSchemeCodes.map(async (code) => {
        try {
          ...
          if (fundData.meta.scheme_name.toLowerCase().includes(query.toLowerCase()) ||
              fundData.meta.fund_house.toLowerCase().includes(query.toLowerCase()) ||
              code === query) {
      ...
      return results
        .filter((result): result is PromiseFulfilledResult<FundPerformance> => 
   ...
   */
  async getFundCategories(): Promise<Array<{name: string, description: string, riskLevel: string, funds: FundPerformance[]}>> {
  ...
  private normalizeCategoryName(category: string): string {
    ...
    } else if (normalized.includes('multi cap') || normalized.includes('flexi cap')) {
      return 'Multi Cap Funds';
    } else if (normalized.includes('elss') || normalized.includes('tax saver')) {
      return 'ELSS Funds';
    } else if (normalized.includes('debt') || normalized.includes('bond')) {
      return 'Debt Funds';
    } else if (normalized.includes('hybrid') || normalized.includes('balanced')) {
      return 'Hybrid Funds';
    }
  ...
  private getCategoryDescription(categoryName: string): string {
    const descriptions: Record<string, string> = {
      ...
      'Small Cap Funds': 'Invest in companies ranked beyond 250th by market cap',
      'Multi Cap Funds': 'Flexible allocation across large, mid & small cap stocks',
