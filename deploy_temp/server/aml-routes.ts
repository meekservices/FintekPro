  ...
  try {
    ...
    if (alerts.length > 0) {
      console.log('[AML] Transaction alerts generated', {
        ...
        alerts: alerts.map(a => ({ alertId: a.alertId, type: a.alertType, risk: a.riskScore }))
      });
    }
...
router.get('/api/aml/alerts/:userId', requireAuth, async (req: Request, res: Response) => {
  try {
    const { userId } = req.params;
...
router.get('/api/aml/risk-profile/:userId', requireAuth, async (req: Request, res: Response) => {
  try {
    const { userId } = req.params;
...
router.post('/api/aml/trigger-edd', requireAuth, async (req: Request, res: Response) => {
  try {
...
router.put('/api/aml/alerts/:alertId/status', requireAuth, async (req: Request, res: Response) => {
  try {
...
router.post('/api/aml/batch-screen', requireAuth, async (req: Request, res: Response) => {
  try {
