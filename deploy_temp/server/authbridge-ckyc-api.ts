...
class AuthBridgeCKYCService {
  ...
  constructor() {
    ...
    if (this.adapter.isInMockMode()) {
      console.log(`[TruthScreen CKYC] Running in mock mode (TRUTHSCREEN_USERNAME / TRUTHSCREEN_PASSWORD not set)`);
    } else {
      console.log(`✅ [TruthScreen CKYC] Initialized — credentials loaded from TRUTHSCREEN_USERNAME`);
    }
  }
  ...
  async fetchCKYC(request: AuthBridgeCKYCRequest): Promise<AuthBridgeCKYCResponse> {
    ...
    return {
      ...
      data: {
        ...
        kyc_type: result.verificationLevel === 'simplified' ? 'Simplified' : 'Normal',
  ...
  async ckycExists(pan: string): Promise<boolean> {
    try {
      const response = await this.fetchCKYC({ pan, full_name: 'Check', date_of_birth: '01/01/1990' });
