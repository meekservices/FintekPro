  ...
  private async refreshPopularFunds(): Promise<void> {
    ...
    try {
      ...
      if (popularFunds && popularFunds.length > 0) {
        console.log(`✅ Refreshed ${popularFunds.length} popular mutual funds in database`);
        ...
        const staleFunds = allDbFunds.filter(fund => {
          const lastUpdated = fund.lastUpdated ? new Date(fund.lastUpdated).getTime() : 0;
          return lastUpdated < sixHoursAgo;
        });

        console.log(`📊 Database status: ${allDbFunds.length} total funds, ${staleFunds.length} stale (>6 hours old)`);
