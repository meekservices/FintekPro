...
export function requireMFA(req: Request, res: Response, next: NextFunction): void {
  ...
  // Get user role(s)
  const userRoles: string[] = (req.user as any).roles || [(req.user as any).role || 'user'];
  ...
  res.status(403).json({
    code: 'MFA_REQUIRED',
    message: 'Multi-factor authentication is required for your role before accessing this resource.',
    mfaMethods: ['webauthn', 'totp'],
    verifyEndpoint: '/api/auth/webauthn/authenticate',
    regulatoryBasis: 'SEBI Cybersecurity & Cyber Resilience Framework 2023, Clause 4.3',
  });
}
