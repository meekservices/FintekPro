...
export class CamsApiService {
  ...
  constructor() {
    this.config = {
      ...
      environment: (process.env.CAMS_ENVIRONMENT as 'production' | 'staging' | 'sandbox') || 'sandbox'
    };
  ...
  async getInvestorPortfolio(pan: string, folio?: string): Promise<FolioDetails[]> {
    const requestData = {
      PortfolioRequest: {
  ...
  async getTransactionHistory(
    ...
    const response = await this.makeRequest('transactions/history', requestData);
    
    const transactions = response?.TransactionResponse?.TransactionDetails || [];
  ...
  async createPurchaseTransaction(transaction: {
    ...
    const response = await this.makeRequest('transactions/purchase', requestData);
  ...
  async createRedemptionTransaction(redemption: {
    ...
    const response = await this.makeRequest('transactions/redemption', requestData);
    ...
    const requestData = {
      SIPRequest: {
        ...
        SIPDetails: {
          ...
          ...(sipDetails.installments && { NoOfInstallments: sipDetails.installments }),
  ...
  // Cancel SIP
  async cancelSip(sipId: string, pan: string): Promise<{ status: string; message: string }> {
    const requestData = {
      SIPCancelRequest: {
        MemberCode: this.config.memberId,
        Password: this.config.password,
    ...
    return {
      ...
      message: response?.SIPCancelResponse?.Message || 'SIP cancellation initiated'
    };
  }
  ...
  // Get NAV data
  async getNavData(schemeCode: string, date?: string): Promise<{ nav: number; navDate: string }> {
    const requestData = {
      NAVRequest: {
        MemberCode: this.config.memberId,
        Password: this.config.password,
    ...
    return {
      ...
      navDate: response?.NAVResponse?.NAVDate || new Date().toISOString().split('T')[0]
    };
  }
  ...
  async getConsolidatedStatement(
    ...
    return {
      ...
      fileName: response?.StatementResponse?.FileName || `statement_${pan}_${Date.now()}.${format.toLowerCase()}`
    };
  }
}
