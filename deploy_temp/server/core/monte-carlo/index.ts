...
export class MonteCarloSimulator {
  ...
  public simulatePortfolio(config: MonteCarloConfig): MonteCarloResult {
    ...
    const drift = (config.expectedAnnualReturn - 0.5 * config.portfolioVolatility ** 2) * dt;
    ...
    const p50Index = Math.floor(this.NUM_SIMULATIONS * 0.50);
    ...
    const avgMaxDrawdown = maxDrawdowns.reduce((a, b) => a + b, 0) / this.NUM_SIMULATIONS;
