...
const adapterLoaders: Map<string, () => Promise<ICkycProviderAdapter>> = new Map();

adapterLoaders.set('truthscreen', async () => {
  const { TruthScreenCkycAdapter } = await import('./adapters/truthscreen-ckyc-adapter');
...
adapterLoaders.set('offline_aadhaar', async () => {
  const { OfflineAadhaarCkycAdapter } = await import('./adapters/offline-aadhaar-ckyc-adapter');
...
export async function getAdapter(providerCode: string): Promise<ICkycProviderAdapter> {
  ...
  if (!loader) {
    throw new AppError(`Unknown CKYC provider: ${providerCode}`, 400, 'UNKNOWN_PROVIDER');
  }
