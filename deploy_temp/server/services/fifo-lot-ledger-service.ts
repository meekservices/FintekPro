import { CASHolding, CASTransaction, parseCASDate } from './cas-statement-service';
...
class FIFOLotLedgerService {
  ...
  normalizeTransaction(txn: CASTransaction): NormalizedTransaction {
    ...
    const normalizedType = this.classifyTransactionType(txn.transactionType, txn.description);
   ...
   */
  private classifyTransactionType(type: string, description: string): NormalizedTransactionType {
    ...
    if (txnType === 'sip' || /systematic investment/i.test(desc) || /sip/i.test(desc)) {
      return 'sip_purchase';
    }
    if (txnType === 'purchase' || /purchase/i.test(desc)) {
      return 'purchase';
    }
    if (txnType === 'switch in' || /switch[\s-]*in/i.test(desc) || /transfer[\s-]*in/i.test(desc)) {
      return 'switch_in';
    }
    if (txnType === 'bonus' || /bonus/i.test(desc)) {
      return 'bonus';
    }
    if (txnType === 'reinvestment' || /reinvest/i.test(desc) || /dividend.*reinvest/i.test(desc)) {
      return 'reinvestment';
    }
    if (txnType === 'redemption' || /redemption/i.test(desc)) {
      return 'redemption';
    }
    if (txnType === 'switch out' || /switch[\s-]*out/i.test(desc) || /transfer[\s-]*out/i.test(desc)) {
      return 'switch_out';
    }
  ...
  buildLotLedger(holding: CASHolding): LotLedgerResult {
    ...
    try {
      ...
      result.totalActiveLots = lots.filter(l => l.status !== 'fully_sold').length;
      result.totalRemainingUnits = lots.reduce((sum, l) => sum + l.remainingUnits, 0);
      ...
      console.log(`[FIFO Ledger] ${holding.isin}: ${lots.length} lots, ${result.totalActiveLots} active, ${result.totalRemainingUnits.toFixed(3)} units remaining`);
  ...
  private isPurchaseType(type: NormalizedTransactionType): boolean {
    return ['purchase', 'sip_purchase', 'switch_in', 'bonus', 'reinvestment'].includes(type);
  }
  ...
  private consumeLotsForRedemption(
    ...
    for (const lot of activeLots) {
      ...
      const holdingPeriodDays = Math.floor(
        (txn.date.getTime() - lot.purchaseDate.getTime()) / (1000 * 60 * 60 * 24)
      );
      
      // Determine capital gains type (equity: 365 days for LTCG)
      const capitalGainsType: 'stcg' | 'ltcg' = holdingPeriodDays >= 365 ? 'ltcg' : 'stcg';
      ...
      if (lot.remainingUnits <= 0.001) {  // Small tolerance for floating point
  ...
  private reconcileClosingBalance(
    ...
    const deltaPercent = expectedBalance > 0 ? (delta / expectedBalance) * 100 : 0;

    // If opening balance exists and calculated is 0, we don't have full transaction history
    const hasPartialHistory = openingBalance > 0 && calculatedBalance < expectedBalance;
    ...
    console.log(`[FIFO Ledger] Reconciliation: Calculated ${calculatedBalance.toFixed(3)} vs Expected ${expectedBalance.toFixed(3)} (delta: ${delta.toFixed(6)})`);
  ...
  async persistLotLedger(
    ...
    for (const result of results) {
      if (result.success) {
        const storageLots = this.convertToStorageFormat(result, userId, portfolioId);
