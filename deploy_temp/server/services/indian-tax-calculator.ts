...
export class IndianTaxCalculator {
  ...
  private isTrustOrInstitution(entityType: EntityType): boolean {
    return ['trust', 'political_party', 'institution', 'research_association', 'news_agency'].includes(entityType);
  }
  ...
  private computeDeductionsNewRegimeWizard(wizardData: WizardData, hasSalary: boolean): number {
  ...
  private computeSurchargeIndividual(
    ...
    if (rate > 0) {
      ...
      if (totalCurrent - totalAtThreshold > incomeAboveThreshold) {
        surcharge = Math.max(0, totalAtThreshold + incomeAboveThreshold - basicTax - (basicTax * 0.04));
      }
    }
  ...
  private computeSurchargeFirm(basicTax: number, taxableIncome: number): number {
  ...
  private computeSurchargeCompany(
    ...
    const isConcessional = companySection === '115BAA' || companySection === '115BAB';
    ...
    if (isConcessional) {
      ...
      if (taxableIncome > 100000000) return basicTax * 0.12;
      if (taxableIncome > 10000000) return basicTax * 0.10;
    }
    return 0;
  }
  ...
  private computeFull(
    ...
    if (this.isIndividualOrHUF(entityType)) {
      ...
      basicTax = this.computeCompanyTax(taxableIncome, options.companySection, options.companyTurnover);
      surcharge = this.computeSurchargeCompany(basicTax, taxableIncome, options.companySection);
  ...
  calculateTaxFromWizard(wizardData: WizardData): ITRCalculationResponse {
    try {
      ...
      if (this.isIndividualOrHUF(entityType) || this.isAOPBOI(entityType)) {
        if (regime === 'old') {
          totalDeductions = this.computeDeductionsOldRegimeWizard(wizardData, hasSalary);
        } else {
          totalDeductions = this.computeDeductionsNewRegimeWizard(wizardData, hasSalary);
        }
      }
      ...
      let relief89 = wizardData.reliefUs89 || 0;
      let foreignTaxCredit = wizardData.foreignTaxCredit || 0;
      ...
      if (data) {
        ...
        if (totalRelief > 0) {
          ...
          data.effectiveTaxRate = grossIncome > 0
      ...
      console.error('[IndianTaxCalculator] calculateTaxFromWizard error:', error);
      return {
        success: false,
  ...
  getOptimalRegime(
    ...
    if ('capitalGainsSTCG' in incomeDetailsOrWizard) {
      ...
      const oldResult = this.calculateTaxFromWizard({
        ...
        isSuperSeniorCitizen: options?.isSuperSeniorCitizen || wizardData.isSuperSeniorCitizen,
      });
    ...
    const oldDeductions = this.computeDeductionsOldRegime(ded, income, isSenior, isSuperSenior, hasSalary);
    const newDeductions = this.computeDeductionsNewRegime(hasSalary);

    const oldData = this.computeFull(grossIncome, oldDeductions, 0, entityType, 'old', {
      isSeniorCitizen: isSenior,
      isSuperSeniorCitizen: isSuperSenior,
    });

    const newData = this.computeFull(grossIncome, newDeductions, 0, entityType, 'new', {
      isSeniorCitizen: isSenior,
      isSuperSeniorCitizen: isSuperSenior,
    });
