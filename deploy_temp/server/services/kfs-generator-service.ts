...
class KFSGeneratorService {
  ...
  private readonly REGULATORY_REFERENCE = 'RBI/2022-23/111 DOR.FIN.REC.65/03.10.038/2022-23';
  private readonly COOLING_OFF_DAYS = 3;
  
  generateKFS(loanDetails: KFSLoanDetails, borrowerName: string, borrowerPan: string): KeyFactsStatement {
    const now = new Date();
    ...
    const emiSchedule = this.generateEMISchedule(
      ...
      loanDetails.interestRatePerAnnum,
      loanDetails.tenureMonths,
      loanDetails.firstEmiDate || new Date(now.getTime() + 45 * 24 * 60 * 60 * 1000)
    );
  ...
  private calculateEMI(principal: number, annualRate: number, tenureMonths: number): { emi: number; totalInterest: number; totalRepayable: number } {
    const monthlyRate = annualRate / 12 / 100;
    
    if (monthlyRate === 0) {
      const emi = principal / tenureMonths;
      return { emi: Math.round(emi * 100) / 100, totalInterest: 0, totalRepayable: principal };
    }
  ...
  private calculateFeesBreakdown(loanDetails: KFSLoanDetails): KeyFactsStatement['feesAndCharges'] {
    const processingFeeAmount = loanDetails.processingFeeFixed || 
      (loanDetails.loanAmount * loanDetails.processingFeePercent / 100);
    
    const stampDuty = loanDetails.stampDuty || Math.round(loanDetails.loanAmount * 0.001);
    ...
    const totalUpfrontCharges = processingFeeAmount + stampDuty + gst + insurancePremium + documentationCharges + otherChargesTotal;
  ...
  private calculateAPR(loanDetails: KFSLoanDetails, totalUpfrontCharges: number): KeyFactsStatement['annualPercentageRate'] {
    const processingFeeImpact = (totalUpfrontCharges / loanDetails.loanAmount) * (12 / loanDetails.tenureMonths) * 100;
    ...
    return {
      ...
      aprBreakdown: {
        ...
        insuranceImpact: loanDetails.insurancePremium ? 
          Math.round((loanDetails.insurancePremium / loanDetails.loanAmount) * 100 * 100) / 100 : 0,
        otherChargesImpact: 0,
      },
      aprCalculationMethod: 'Internal Rate of Return (IRR) method as per RBI Guidelines',
    };
  }
  
  private generateEMISchedule(principal: number, annualRate: number, tenureMonths: number, firstEmiDate: Date) {
    const monthlyRate = annualRate / 12 / 100;
    ...
    const samplesToShow = [1, 2, 3, Math.ceil(tenureMonths / 2), tenureMonths - 1, tenureMonths];
  ...
  private getPrepaymentTerms(loanType: string): KeyFactsStatement['prepaymentTerms'] {
