...
class OfflineAadhaarService {
  ...
  async verifyOfflineAadhaar(
    ...
    try {
      ...
      if (!decryptedData) {
        return {
          success: false,
          error: 'Failed to decrypt Aadhaar data. Please check your share code.',
      ...
      if (isExpired) {
        validationErrors.push(`XML is expired. Generated on ${decryptedData.generatedAt.toISOString()}`);
      }
      ...
      if (!decryptedData.address.pincode) validationErrors.push('Pincode is missing');

      console.log(`✅ [Offline Aadhaar] Verified, reference: ${decryptedData.referenceId}`);
   ...
   */
  private async verifySignature(xmlContent: string): Promise<{ isValid: boolean; issuer: string }> {
    // In production, use proper XML signature verification with UIDAI certificates
    // This is a simplified check
    
    const hasSignature = xmlContent.includes('<ds:Signature') || xmlContent.includes('<Signature');
    const hasUIDAICert = xmlContent.includes(this.UIDAI_ISSUER);

    console.log('🔐 [Offline Aadhaar] Signature check:', { hasSignature, hasUIDAICert });
   ...
   */
  private async decryptData(parsed: any, shareCode: string): Promise<OfflineAadhaarData | null> {
    try {
      // Extract data from parsed XML
      // The actual structure depends on UIDAI's XML schema
      const uidData = parsed.OfflinePaperlessKyc || parsed.PrintLetterBarcodeData;
      ...
      const referenceId = this.generateReferenceId(attrs.uid || 'UNKNOWN', shareCode);
      ...
      const data: OfflineAadhaarData = {
        ...
        photo: attrs.photo,
        referenceId,
        generatedAt: new Date(attrs.gd || attrs.GeneratedDateTime || Date.now()),
        validUntil: new Date(Date.now() + this.VALIDITY_DAYS * 24 * 60 * 60 * 1000),
        signature: {
