...
class DataEnrichmentScheduler {
  ...
  initialize(): void {
    ...
    console.log(`[DataEnrichmentScheduler] Initialized - next run at ${this.nextRunTime?.toISOString()}`);
  }
  ...
  async runEnrichmentJobs(): Promise<void> {
    ...
    try {
      ...
      try {
        const { mfExtendedEnrichmentService } = await import('./mf-extended-enrichment-service');
        const mfResult = await mfExtendedEnrichmentService.enrichAllFunds({ 
          forceRefresh: false,
          batchSize: 500,
        });
        stats.mfEnriched = mfResult.fundsEnriched;
        console.log(`[DataEnrichmentScheduler] MF enrichment: ${stats.mfEnriched} funds enriched`);
      } catch (error: any) {
        console.error('[DataEnrichmentScheduler] MF enrichment failed:', error.message);
      }
      ...
      try {
        ...
        console.error('[DataEnrichmentScheduler] Stock enrichment failed:', error.message);
      }
      
      // Run NAV-based metrics sync
      try {
        const { mfReturnsSyncService } = await import('./mf-returns-sync-service');
        const navResult = await mfReturnsSyncService.runBatchSync(100);
        stats.navMetricsUpdated = navResult.successful;
        console.log(`[DataEnrichmentScheduler] NAV metrics: ${stats.navMetricsUpdated} funds updated`);
      } catch (error: any) {
