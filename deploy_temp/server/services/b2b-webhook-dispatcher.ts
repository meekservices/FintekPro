...
export class B2BWebhookDispatcher {
   ...
   */
  public async dispatchPayload(clientId: string, eventType: string, payload: any): Promise<boolean> {
    try {
      const [client] = await db.select().from(b2bClients).where(eq(b2bClients.id, clientId)).limit(1);

      if (!client || !client.webhookUrl) {
         console.warn(`[WFIA Webhook] Webhook URL missing for Tenant ${clientId}. Payload dropped.`);
         return false;
      }
      ...
      const signature = client.webhookSecret 
            ? createHmac("sha256", client.webhookSecret).update(stringifiedPayload).digest("hex") 
