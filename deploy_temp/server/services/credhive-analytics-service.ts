...
export class CredhiveAnalyticsService {
  ...
  async checkProspectingThresholds(
    ...
    try {
      ...
      for (const lead of leads) {
        const revenue = lead.annualRevenue ? parseFloat(String(lead.annualRevenue)) : null;
        const profit = lead.netProfit ? parseFloat(String(lead.netProfit)) : null;
        const score = lead.leadScore ?? 0;

        const meetsRevenue = !thresholds.minRevenue || (revenue && revenue >= thresholds.minRevenue);
        const meetsProfit = !thresholds.minProfit || (profit && profit >= thresholds.minProfit);
        const meetsScore = !thresholds.minLeadScore || score >= thresholds.minLeadScore;

        if (meetsRevenue && meetsProfit && meetsScore) {
          const priority: 'high' | 'medium' | 'low' =
            lead.leadQuality === 'hot' ? 'high' :
            lead.leadQuality === 'warm' ? 'medium' : 'low';
      ...
      console.error('[CredhiveAnalytics] checkProspectingThresholds error:', error.message);
      return [];
    }
  }
  ...
  async calculateSmartLeadScore(cin: string): Promise<SmartLeadScore | null> {
    try {
      ...
      const totalScore = Math.round(
        financialStrength * 0.4 + complianceHealth * 0.35 + directorProfile * 0.25
      );

      const leadGrade: 'hot' | 'warm' | 'cold' =
        totalScore >= 65 ? 'hot' :
        totalScore >= 35 ? 'warm' : 'cold';
      ...
      console.error(`[CredhiveAnalytics] calculateSmartLeadScore error for ${cin}:`, error.message);
      return null;
    }
  }
  ...
  private scoreFinancialStrength(company: any): number {
    ...
    if (financials.debtToEquityRatio != null && financials.debtToEquityRatio < 1) score += 15;
  ...
  private scoreDirectorProfile(company: any): number {
    const directorCount = Array.isArray(company.directors) ? company.directors.length : 0;
