import { db } from "../db";
import { holdingLotsV2, HoldingLotV2, InsertHoldingLotV2 } from "@shared/schema";
...
class HoldingLotsStorageService {
  ...
  async insertLots(lots: LotStorageInput[]): Promise<{ inserted: number; errors: string[] }> {
    const errors: string[] = [];
    let inserted = 0;
    ...
    console.log(`[HoldingLotsStorage] Inserted ${inserted}/${lots.length} lots`);
  ...
  async getActiveLotsByIsin(userId: string, isin: string): Promise<HoldingLotV2[]> {
    return db.select()
      .from(holdingLotsV2)
      .where(and(
        eq(holdingLotsV2.userId, userId),
  ...
  async processRedemption(
  ...
  ): Promise<{ processedLots: { lotId: string; unitsRedeemed: number }[]; remainingUnits: number }> {
    ...
    for (const lot of activeLots) {
      ...
      const lotRemainingUnits = parseFloat(lot.remainingUnits || lot.units);
  ...
  async deleteLotsByPortfolio(portfolioId: string): Promise<number> {
    const result = await db.delete(holdingLotsV2)
      .where(eq(holdingLotsV2.portfolioId, portfolioId));
  ...
  private aggregateLots(lots: HoldingLotV2[]): LotQueryResult {
    ...
    for (const lot of lots) {
      if (lot.status === 'active' || lot.status === 'partial') {
        ...
        totalUnits += units;
        totalCost += parseFloat(lot.totalCost) * (units / parseFloat(lot.units));
        if (lot.currentValue) {
          totalCurrentValue += parseFloat(lot.currentValue) * (units / parseFloat(lot.units));
        }
      }
    }
...
export const holdingLotsStorageService = HoldingLotsStorageService.getInstance();
