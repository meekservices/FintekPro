...
export class CAAssignmentService {
  async findBestCA(criteria: CAAssignmentCriteria): Promise<CACandidate[]> {
    ...
    for (const ca of availableCAs) {
      ...
      const rating = parseFloat(ca.caAverageRating || '4.0');
      matchScore += rating * 5;
      ...
      if (criteria.caseType === 'itr' && criteria.itrFormType) {
        const feeField = `baseFeeItr${criteria.itrFormType.replace('ITR-', '')}` as keyof typeof ca;
        baseFee = 0;
      }
  ...
  async assignCAToCaseFromPartners(caseId: string, caPartnerId: string): Promise<{ success: boolean; message: string }> {
    try {
      const [ca] = await db
        .select()
        .from(partners)
      ...
      if (currentCases >= maxCases) {
        return { success: false, message: 'CA has reached maximum case capacity' };
      }
  ...
  async autoAssignCA(caseId: string, criteria: CAAssignmentCriteria): Promise<{ success: boolean; assignedCA?: CACandidate; message: string }> {
    const candidates = await this.findBestCA(criteria);
    
    if (candidates.length === 0) {
      return { success: false, message: 'No available CA found matching the criteria' };
    }
    ...
    if (result.success) {
      return { success: true, assignedCA: bestCA, message: 'CA auto-assigned successfully' };
    }
    
    for (let i = 1; i < candidates.length; i++) {
      const fallbackResult = await this.assignCAToCaseFromPartners(caseId, candidates[i].id);
      if (fallbackResult.success) {
        return { success: true, assignedCA: candidates[i], message: 'CA auto-assigned successfully (fallback)' };
      }
    }
  ...
  async markCaseCompleted(caseId: string, caPartnerId: string): Promise<{ success: boolean }> {
    try {
      await db.transaction(async (tx) => {
        await tx
          .update(agentItrCases)
        ...
        if (ca) {
          await tx
            .update(partners)
            .set({
              caCurrentActiveCases: Math.max(0, (ca.caCurrentActiveCases || 0) - 1),
  ...
  async updateCARating(caPartnerId: string, rating: number): Promise<void> {
    ...
    const currentRating = parseFloat(ca.caAverageRating || '5.0');
  ...
  async getCADashboardStats(caPartnerId: string): Promise<{
    ...
    return {
      ...
      pendingCases: 0,
      avgRating: parseFloat(ca?.caAverageRating || '5.0'),
      totalEarnings: parseFloat(ca?.totalCommissionsEarned || '0'),
      thisMonthEarnings: 0,
      casesByType: {
        itr: 0,
        gst: 0,
