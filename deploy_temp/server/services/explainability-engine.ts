...
interface RecommendationReason {
  category: 'risk_match' | 'return_potential' | 'goal_alignment' | 'diversification' | 
            'tax_efficiency' | 'liquidity' | 'regulatory' | 'market_conditions' | 'historical_performance';
...
interface SuitabilityExplanation {
  overallSuitability: 'highly_suitable' | 'suitable' | 'moderately_suitable' | 'not_suitable';
...
interface Disclosure {
  type: 'sebi_mandatory' | 'risk_disclosure' | 'tax_disclosure' | 'liquidity_disclosure' | 
        'past_performance' | 'conflict_of_interest' | 'exit_load' | 'expense_ratio';
...
export class ExplainabilityEngine {
   ...
   */
  explainProductRecommendation(request: ExplainabilityRequest): ProductExplanation {
    const { clientProfile, product, investmentAmount, goalId, includeAlternatives } = request;
    ...
    const goalAlignment = this.analyzeGoalAlignment(clientProfile, product, investmentAmount, goalId);
    const riskReturnDelta = this.calculateRiskReturnDelta(clientProfile, product, investmentAmount);
    ...
    if (includeAlternatives) {
      explanation.alternatives = this.suggestAlternatives(clientProfile, product, suitability);
    }
   ...
   */
  private analyzeSuitability(client: ClientProfile, product: ProductData): SuitabilityExplanation {
    ...
    if (!horizonMatch) {
      warnings.push(`Lock-in period of ${product.lockInPeriod} months exceeds your investment horizon`);
    }
    ...
    const kycMatch = kycLevels[client.kycTier] >= kycLevels[product.kycRequirement];
    factors.push({
    ...
    if (!kycMatch) {
      warnings.push(`KYC upgrade to ${product.kycRequirement} level required before investing`);
    }
    ...
    const matchedWeight = factors.filter(f => f.match).reduce((sum, f) => sum + f.weight, 0);
   ...
   */
  private determineRecommendation(suitability: SuitabilityExplanation): { action: ProductExplanation['recommendation']; confidence: number } {
    const score = suitability.suitabilityScore;
    const hasWarnings = suitability.warnings.length > 0;
    const criticalFactorsFailed = suitability.factors.filter(f => !f.match && f.weight >= 20).length;
    
    if (score >= 85 && !hasWarnings) {
      return { action: 'buy', confidence: Math.min(95, score) };
    } else if (score >= 70 && criticalFactorsFailed === 0) {
   ...
   */
  private generateReasons(client: ClientProfile, product: ProductData, suitability: SuitabilityExplanation): RecommendationReason[] {
    ...
    reasons.push({
      ...
      impact: returnDelta > 0 ? 'positive' : returnDelta < -2 ? 'negative' : 'neutral',
      dataPoints: {
    ...
    const existingAllocation = client.currentAssetAllocation[product.productType] || 0;
    ...
    // Tax efficiency
    const taxEfficient = product.taxTreatment.toLowerCase().includes('exempt') || 
    ...
    reasons.push({
      category: 'liquidity',
      weight: 10,
      description: product.liquidityScore >= 70
      ...
      impact: product.liquidityScore >= 70 ? 'positive' : product.liquidityScore >= 40 ? 'neutral' : 'negative',
      dataPoints: {
    ...
    reasons.push({
      ...
      weight: 15,
      description: oneYearReturn > 0 && threeYearReturn > 0
      ...
      impact: oneYearReturn > 5 && threeYearReturn > 8 ? 'positive' : oneYearReturn < 0 ? 'negative' : 'neutral',
      dataPoints: {
        '1Y': oneYearReturn,
        '3Y': threeYearReturn,
    ...
    const primaryGoal = client.goals.find(g => g.priority === 'high') || client.goals[0];
    const goalName = primaryGoal?.name || 'General Wealth Creation';
    const productHorizonYears = product.lockInPeriod / 12;
    const goalYearsRemaining = primaryGoal 
      ? Math.max(0, (new Date(primaryGoal.targetDate).getTime() - Date.now()) / (365.25 * 24 * 60 * 60 * 1000))
    ...
    reasons.push({
      ...
      impact: horizonDiff >= 3 ? 'positive' : horizonMatch ? 'neutral' : 'negative',
      dataPoints: {
        goalName,
  ...
  private analyzeGoalAlignment(
    ...
    const goalYearsRemaining = goal 
      ? Math.max(0, (new Date(goal.targetDate).getTime() - Date.now()) / (365.25 * 24 * 60 * 60 * 1000))
      : client.investmentHorizon;
    const productHorizonYears = product.lockInPeriod / 12;
    ...
    const projectedValue = investmentAmount * Math.pow(1 + product.expectedReturn / 100, goalYearsRemaining);
    const projectedProgress = Math.min(100, ((currentProgress * targetAmount / 100) + projectedValue) / targetAmount * 100);
    
    // Alignment score
    const horizonScore = matchQuality === 'excellent' ? 100 : matchQuality === 'good' ? 80 : matchQuality === 'fair' ? 60 : 40;
    ...
    const portfolioRiskBefore = this.calculatePortfolioRisk(client.currentAssetAllocation);
    const newAllocation = { ...client.currentAssetAllocation };
    newAllocation[product.productType] = (newAllocation[product.productType] || 0) + 
      (investmentAmount / (client.currentPortfolioValue + investmentAmount)) * 100;
    ...
    return {
      ...
      targetProgress: {
        ...
        progressDelta: Math.round((projectedProgress - currentProgress) * 10) / 10
      },
      riskContribution: {
        portfolioRiskBefore,
        portfolioRiskAfter,
        riskChange: portfolioRiskAfter > portfolioRiskBefore + 2 ? 'increased' : 
                   portfolioRiskAfter < portfolioRiskBefore - 2 ? 'decreased' : 'unchanged'
      }
    };
  }
  ...
  private calculateRiskReturnDelta(
    ...
    const compounded5Year = Math.pow(1 + expectedReturn / 100, 5) - 1;
    const compounded10Year = Math.pow(1 + expectedReturn / 100, 10) - 1;
    
    // Portfolio impact
    const currentPortfolioReturn = 10; // Assume 10% baseline
    const newWeight = investmentAmount / (client.currentPortfolioValue + investmentAmount);
    const oldWeight = 1 - newWeight;
    const newPortfolioReturn = (currentPortfolioReturn * oldWeight) + (expectedReturn * newWeight);
    ...
    const newSharpe = (newPortfolioReturn - riskFreeRate) / ((15 * oldWeight) + (product.volatility * newWeight));
    
    // Diversification benefit
    const correlationBenefit = 0.7; // Assume 70% correlation
    const diversificationBenefit = Math.round((1 - correlationBenefit) * 100 * newWeight);
    ...
    return {
      ...
      riskMetrics: {
        ...
        sharpeRatio: Math.round((expectedReturn - riskFreeRate) / product.volatility * 100) / 100,
        sortinoRatio: Math.round((expectedReturn - riskFreeRate) / (product.volatility * 0.7) * 100) / 100,
        beta: product.riskLevel / 3 // Simplified beta calculation
      },
      portfolioImpact: {
        returnDelta: Math.round((newPortfolioReturn - currentPortfolioReturn) * 100) / 100,
        riskDelta: Math.round((product.volatility * newWeight - 15 * newWeight) * 100) / 100,
   ...
   */
  private generateDisclosures(product: ProductData, suitability: SuitabilityExplanation): Disclosure[] {
    ...
    disclosures.push({
      ...
      content: 'Past performance does not guarantee future results. The investment value can go down as well as up, and you may get back less than you invested.'
    });
   ...
   */
  explainPortfolio(clientProfile: ClientProfile, products: ProductData[]): PortfolioExplanation {
    const allocationExplanations = this.generateAllocationExplanations(clientProfile, products);
    const diversification = this.analyzeDiversification(products);
    const riskBudget = this.analyzeRiskBudget(clientProfile, products);
    const goalContributions = this.analyzeGoalContributions(clientProfile, products);
    const compliance = this.checkCompliance(clientProfile, products);
    
    return {
      portfolioId: `PORTFOLIO-${clientProfile.clientId}`,
      clientId: clientProfile.clientId,
      overallRationale: this.generatePortfolioRationale(clientProfile, products),
  ...
  generateNarrativeSummary(explanation: ProductExplanation): string {
    const action = explanation.recommendation === 'buy' ? 'recommend' : 
                   explanation.recommendation === 'hold' ? 'suggest reviewing' :
                   explanation.recommendation === 'avoid' ? 'advise against' : 'suggest holding';
    ...
    const warnings = explanation.suitabilityExplanation.warnings.length > 0
  ...
  private generateAllocationExplanations(client: ClientProfile, products: ProductData[]): AllocationExplanation[] {
    const assetClasses = Array.from(new Set(products.map(p => p.productType)));
    return assetClasses.map(assetClass => {
      const classProducts = products.filter(p => p.productType === assetClass);
  ...
  private analyzeDiversification(products: ProductData[]): DiversificationAnalysis {
    const assetClasses = Array.from(new Set(products.map(p => p.productType)));
    return {
      ...
      concentrationRisk: assetClasses.length < 3 ? 'High - consider adding more asset classes' : 'Moderate - well diversified',
      geographicDiversification: 'Primarily domestic exposure',
      sectorDiversification: 'Diversified across multiple sectors'
    };
  }
  
  private analyzeRiskBudget(client: ClientProfile, products: ProductData[]): RiskBudgetAnalysis {
    const totalRiskBudget = this.mapRiskToleranceToLevel(client.riskTolerance) * 10;
    const utilizedRisk = products.reduce((sum, p) => sum + p.riskLevel * 2, 0) / products.length;
    ...
    return {
      ...
      recommendation: utilizedRisk < totalRiskBudget 
  ...
  private analyzeGoalContributions(client: ClientProfile, products: ProductData[]): GoalContribution[] {
    return client.goals.map(goal => {
      const avgReturn = products.reduce((sum, p) => sum + p.expectedReturn, 0) / products.length;
      const yearsToGoal = Math.max(1, (new Date(goal.targetDate).getTime() - Date.now()) / (365.25 * 24 * 60 * 60 * 1000));
      const projectedValue = goal.currentProgress * goal.targetAmount / 100 * Math.pow(1 + avgReturn / 100, yearsToGoal);
      ...
      return {
        ...
        probability: Math.min(95, Math.round((projectedValue / goal.targetAmount) * 100)),
        shortfall: projectedValue < goal.targetAmount ? goal.targetAmount - projectedValue : undefined,
        recommendation: projectedValue < goal.targetAmount 
  ...
  private checkCompliance(client: ClientProfile, products: ProductData[]): ComplianceStatus {
    const issues: string[] = [];
    
    // Check KYC compliance
    const kycLevels = { basic: 1, enhanced: 2, accredited: 3 };
    const kycIssues = products.filter(p => kycLevels[client.kycTier] < kycLevels[p.kycRequirement]);
    if (kycIssues.length > 0) {
      issues.push(`${kycIssues.length} product(s) require higher KYC tier`);
    }
    
    // Check suitability
    const avgProductRisk = products.reduce((sum, p) => sum + p.riskLevel, 0) / products.length;
    const clientRiskLevel = this.mapRiskToleranceToLevel(client.riskTolerance);
  ...
  private generatePortfolioRationale(client: ClientProfile, products: ProductData[]): string {
    const avgReturn = products.reduce((sum, p) => sum + p.expectedReturn, 0) / products.length;
    const avgRisk = products.reduce((sum, p) => sum + p.riskLevel, 0) / products.length;
    const assetClasses = Array.from(new Set(products.map(p => p.productType)));
