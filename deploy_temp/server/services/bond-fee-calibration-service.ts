import { db } from "../db";
import { bondFeeProfiles, bondFeeOverrides, bondCatalog, type BondFeeProfile, type InsertBondFeeProfile } from "@shared/schema";
...
export const REGULATORY_FEE_CAPS = {
  ...
  sgb: {
    maxBrokerage: 0.50,       // 0.50% distribution commission
    maxPlatformFee: 0,        // No platform fee (RBI subsidized)
    stampDuty: false,
    notes: "Sovereign Gold Bonds - Distribution commission allowed, no platform fee"
  },
  corporate_bond: {
    maxBrokerage: 0.50,       // 0.50% for corporate bonds
  ...
  ncd: {
    maxBrokerage: 0.50,       // 0.50% for NCDs
    maxPlatformFee: 0.25,     // 0.25% platform fee
    stampDuty: true,
  ...
  infrastructure_bond: {
    maxBrokerage: 0.50,       // 0.50% for infra bonds
    maxPlatformFee: 0.25,     // 0.25% platform fee
    stampDuty: true,
    stampDutyRate: 0.0001,
  ...
  unlisted_bond: {
    maxBrokerage: 1.00,       // 1.00% for unlisted bonds
    maxPlatformFee: 0.50,     // 0.50% platform fee
    stampDuty: true,
    stampDutyRate: 0.00015,   // 0.015% stamp duty
    notes: "Unlisted/Private Bonds - Higher risk, higher fees allowed"
  },
  tax_free_bond: {
    maxBrokerage: 0.50,       // 0.50% for tax-free bonds
    maxPlatformFee: 0.25,     // 0.25% platform fee
...
class BondFeeCalibrationService {
    ...
    for (const [instrumentType, caps] of Object.entries(REGULATORY_FEE_CAPS)) {
      ...
      if (existing.length === 0) {
        ...
        await db.insert(bondFeeProfiles).values({
          instrumentType,
          instrumentLabel: this.getInstrumentLabel(instrumentType as InstrumentType),
          ...
          stampDutyRate: caps.stampDuty ? String((caps as any).stampDutyRate || 0) : '0',
          retailMultiplier: '1.00',
          hniMultiplier: '1.00',
          institutionalMultiplier: '0.50',
          buyFeeMultiplier: '1.00',
          sellFeeMultiplier: '1.00',
          regulatoryReference: this.getRegulatoryReference(instrumentType as InstrumentType),
  ...
  async getAllProfiles(): Promise<BondFeeProfile[]> {
    return await db.select().from(bondFeeProfiles).orderBy(bondFeeProfiles.instrumentType);
  }
  
  // Get fee profile by instrument type
  async getProfileByInstrumentType(instrumentType: string): Promise<BondFeeProfile | null> {
  ...
  // Update fee profile
  async updateProfile(id: string, updates: Partial<InsertBondFeeProfile>, updatedBy?: string): Promise<BondFeeProfile | null> {
    // Validate against regulatory caps
    if (updates.brokerageFeeValue || updates.platformFeeValue) {
      const profile = await db.select().from(bondFeeProfiles).where(eq(bondFeeProfiles.id, id)).limit(1);
      if (profile[0]) {
        const caps = REGULATORY_FEE_CAPS[profile[0].instrumentType as InstrumentType];
        if (caps) {
          if (updates.brokerageFeeValue && parseFloat(updates.brokerageFeeValue) > caps.maxBrokerage) {
            throw new Error(`Brokerage fee exceeds regulatory cap of ${caps.maxBrokerage}% for ${profile[0].instrumentType}`);
          }
          if (updates.platformFeeValue && parseFloat(updates.platformFeeValue) > caps.maxPlatformFee) {
            throw new Error(`Platform fee exceeds regulatory cap of ${caps.maxPlatformFee}% for ${profile[0].instrumentType}`);
          }
        }
      }
    }
    
    const result = await db.update(bondFeeProfiles)
  ...
  async calculateFees(input: FeeCalculationInput): Promise<FeeBreakdown> {
    const { instrumentType, transactionAmount, grossYield, investorSegment, transactionType, feeProfileId, feeOverrideId } = input;
    ...
    if (profile) {
      txnMultiplier = transactionType === 'buy' 
        ? parseFloat(profile.buyFeeMultiplier || '1.00')
        : parseFloat(profile.sellFeeMultiplier || '1.00');
    }
    ...
    if (brokerageRate > caps.maxBrokerage) {
      violations.push(`Brokerage ${brokerageRate}% exceeds regulatory cap of ${caps.maxBrokerage}%`);
      brokerageRate = caps.maxBrokerage;
    }
    if (platformFeeRate > caps.maxPlatformFee) {
      violations.push(`Platform fee ${platformFeeRate}% exceeds regulatory cap of ${caps.maxPlatformFee}%`);
    ...
    const transactionCharges = (transactionAmount * transactionChargesRate) / 100;
  ...
  async createFeeOverride(data: {
    ...
    if (data.brokerageFeeOverride && data.brokerageFeeOverride > caps.maxBrokerage) {
      throw new Error(`Brokerage override ${data.brokerageFeeOverride}% exceeds regulatory cap of ${caps.maxBrokerage}%`);
    }
    if (data.platformFeeOverride && data.platformFeeOverride > caps.maxPlatformFee) {
      throw new Error(`Platform fee override ${data.platformFeeOverride}% exceeds regulatory cap of ${caps.maxPlatformFee}%`);
    }
    
    const result = await db.insert(bondFeeOverrides).values({
      isin: data.isin,
  ...
  private getRegulatoryReference(type: InstrumentType): string {
    const refs: Record<InstrumentType, string> = {
      ...
      corporate_bond: 'SEBI (Issue and Listing of Non-Convertible Securities) Regulations, 2021',
