...
type MassiveEvent = MassiveTradeEvent | MassiveQuoteEvent | MassiveAggregateEvent | MassiveStatusEvent;
...
class MassiveWebSocketService extends EventEmitter {
  ...
  connect(feed?: FeedType): void {
    if (!this.isConfigured()) {
      console.warn("⚠️ Massive WebSocket: API key not configured (POLYGON_API_KEY)");
      return;
    }
    ...
    const wsUrl = this.feedType === "realtime" ? MASSIVE_WS_REALTIME : MASSIVE_WS_DELAYED;
    console.log(`🔌 [Massive WS] Connecting to ${this.feedType} feed: ${wsUrl}`);
    ...
    this.ws.on("close", (code: number, reason: Buffer) => {
      ...
      this.clearHeartbeat();
      console.log(`🔌 [Massive WS] Disconnected (code: ${code}, reason: ${reason.toString()})`);
  ...
  private handleStatus(msg: MassiveStatusEvent): void {
    ...
    if (msg.status === "auth_success") {
      ...
      console.log("ℹ️ [Massive WS] WebSocket access requires a paid Massive plan. REST API and Flat Files remain available.");
  ...
  subscribeMinuteAggs(symbols: string[]): void {
    ...
    this.sendSubscribe(channels);
    console.log(`📊 [Massive WS] Subscribed to minute aggregates: ${symbols.join(", ")}`);
  }
  ...
  subscribeSecondAggs(symbols: string[]): void {
    ...
    this.sendSubscribe(channels);
    console.log(`📊 [Massive WS] Subscribed to second aggregates: ${symbols.join(", ")}`);
  }
  ...
  private attemptReconnect(): void {
    if (this.reconnectAttempts >= this.maxReconnectAttempts) {
      console.error(`❌ [Massive WS] Max reconnect attempts (${this.maxReconnectAttempts}) reached`);
    ...
    console.log(`🔄 [Massive WS] Reconnecting in ${delay}ms (attempt ${this.reconnectAttempts}/${this.maxReconnectAttempts})`);
