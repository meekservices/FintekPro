...
export class AIModelRegistry implements ModelRegistryManager {
  ...
  async getEligibleModels(requiredCapabilities: string[] = []): Promise<ModelRegistryEntry[]> {
    await this.refreshCacheIfNeeded();
    
    // Convert cached iterables to active array list
    const activeModels = Array.from(this.registryCache.values()).filter(m => m.status === "active");
  ...
  async updateModelScore(modelId: string, metricType: "accuracy" | "compliance" | "latency", value: number): Promise<void> {
    try {
      const records = await db.select().from(amseModelRegistry).where(eq(amseModelRegistry.modelId, modelId));
      ...
      if (metricType === "accuracy") {
        ...
        const oldScore = parseFloat(record.avgScore as string || "0");
        updates.avgScore = ((oldScore * 9) + value) / 10;
      } else if (metricType === "compliance") {
        const oldScore = parseFloat(record.complianceScore as string || "100");
      ...
      await db.update(amseModelRegistry).set(updates).where(eq(amseModelRegistry.modelId, modelId));
  ...
  async deactivateModel(modelId: string, reason: string): Promise<void> {
    try {
      await db.update(amseModelRegistry)
        .set({ status: "inactive", lastUpdated: new Date() })
        .where(eq(amseModelRegistry.modelId, modelId));
      
      this.invalidateCache();
      console.warn(`[AMSE FAILSAFE] Model ${modelId} deactivated by Registry Manager. Reason: ${reason}`);
    } catch (e) {
  ...
  private async refreshCacheIfNeeded(): Promise<void> {
    const now = Date.now();
    if (this.registryCache.size === 0 || (now - this.cacheLastUpdated) > this.CACHE_TTL_MS) {
      ...
      for (const r of records) {
        this.registryCache.set(r.modelId, {
          ...
          avg_score: parseFloat(r.avgScore as string || "0"),
          latency_ms: r.latencyMs || 0,
          cost_per_call: parseFloat(r.costPerCall as string || "0"),
          compliance_score: parseFloat(r.complianceScore as string || "100"),
          specialization_weights: r.specializationWeights as any,
          status: r.status as any,
          last_updated: r.lastUpdated?.toISOString() || new Date().toISOString()
        });
      }
