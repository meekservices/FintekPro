import { db } from "../db";
import { payoutClaims, proofUploads, bankerConfirmationEmails, leadRegistry, leadAuditLogs, progressiveCommissionLedger, reversalLedger, partnerWallets, PayoutClaim } from "@shared/schema";
...
export class PayoutClaimService {
  ...
  static getInstance(): PayoutClaimService {
    if (!this.instance) this.instance = new PayoutClaimService();
  ...
  async submitClaim(data: {
    ...
    if (lead.firstTouchTimestamp > disbursementDate) {
      return { success: false, error: "Lead was registered after disbursement date — anti-fraud check failed" };
    }

    if (lead.firstAgentId !== data.agentId) {
      return { success: false, error: "Agent does not match lead's first agent" };
    }

    const [claim] = await db.insert(payoutClaims).values({
      leadId: data.leadId,
    ...
    await db.insert(leadAuditLogs).values({
      ...
      details: { disbursementAmount: data.disbursementAmount, financierName: data.financierName },
    });

    return { success: true, claim };
  }

  async addProof(claimId: string, data: {
    fileName: string;
    fileType: string;
    fileSize: number;
    fileHash: string;
    ...
    const [claim] = await db.select().from(payoutClaims)
    ...
    const [proof] = await db.insert(proofUploads).values({
      claimId,
    ...
    await db.insert(leadAuditLogs).values({
      ...
      details: { fileName: data.fileName, fileType: data.fileType, fileSize: data.fileSize },
    });

    return { success: true, proof };
  }

  async sendBankerConfirmation(claimId: string, data: {
    bankerEmail: string;
    seniorEmail?: string;
    ccAdminEmail?: string;
    emailSubject: string;
    ...
    const [email] = await db.insert(bankerConfirmationEmails).values({
      claimId,
    ...
    await db.update(payoutClaims).set({
    ...
    await db.insert(leadAuditLogs).values({
      ...
      details: { bankerEmail: data.bankerEmail, emailSubject: data.emailSubject },
    });

    return { success: true, email };
  }

  async adminConfirmBankerReply(claimId: string, adminId: string, emailContent?: string): Promise<{ success: boolean; claim?: PayoutClaim; decisionApplied?: string; error?: string }> {
    const [claim] = await db.select().from(payoutClaims)
      .where(eq(payoutClaims.claimId, claimId));
    ...
    if (!claim.bankerConfirmationEmailId) {
      return { success: false, error: "No banker confirmation email associated with this claim" };
    }

    await db.update(bankerConfirmationEmails).set({
      replyReceived: true,
      replyReceivedAt: new Date(),
      replyContent: emailContent,
      taggedByAdminId: adminId,
      taggedAt: new Date(),
    }).where(eq(bankerConfirmationEmails.emailId, claim.bankerConfirmationEmailId));
    ...
    } else if (claim.pddStatus === 'PENDING' && claim.pddExceptionAllowedByFinancier) {
      finalStatus = 'APPROVED';
      decisionApplied = 'PDD pending but exception allowed by financier — claim approved';
    ...
    const [updatedClaim] = await db.update(payoutClaims).set(updateData)
      .where(eq(payoutClaims.claimId, claimId)).returning();
  ...
  async confirmPddClearance(claimId: string, actorId: string, ipAddress?: string): Promise<{ success: boolean; claim?: PayoutClaim; error?: string }> {
    const [claim] = await db.select().from(payoutClaims)
      .where(eq(payoutClaims.claimId, claimId));
    ...
    if (claim.status !== 'ON_HOLD_PDD') {
      return { success: false, error: "Claim is not on hold for PDD clearance" };
    }

    const [updatedClaim] = await db.update(payoutClaims).set({
  ...
  async rejectClaim(claimId: string, adminId: string, reason: string): Promise<{ success: boolean; claim?: PayoutClaim; error?: string }> {
    const [claim] = await db.select().from(payoutClaims)
      .where(eq(payoutClaims.claimId, claimId));
  ...
  async executeClawback(claimId: string, actorId: string, reason: string): Promise<{ success: boolean; claim?: PayoutClaim; error?: string }> {
    const [claim] = await db.select().from(payoutClaims)
      .where(eq(payoutClaims.claimId, claimId));
    ...
    if (claim.status !== 'APPROVED') {
      return { success: false, error: "Only approved claims can be clawed back" };
    }
    ...
    if (claim.commissionLedgerId) {
      const [ledgerEntry] = await db.select().from(progressiveCommissionLedger)
        .where(eq(progressiveCommissionLedger.ledgerId, claim.commissionLedgerId));
  ...
  async getClaimsByAgent(agentId: string): Promise<PayoutClaim[]> {
    return db.select().from(payoutClaims)
  ...
  async getProofsByClaimId(claimId: string) {
    return db.select().from(proofUploads)
  ...
  async getAuditLogs(leadId?: string, claimId?: string) {
    ...
    const query = conditions.length > 0
      ? db.select().from(leadAuditLogs).where(and(...conditions))
