...
interface DeviceFingerprint {
  ...
  canvasHash?: string;
  createdAt: Date;
  action: 'kyc_start' | 'pan_verify' | 'aadhaar_otp' | 'aadhaar_verify' | 'compliance_signoff' | 'login' | 'transaction';
}
...
class DeviceFingerprintService {
  ...
  generateFingerprint(
    ...
    console.log(`🔐 [Device Fingerprint] Captured for user ${userId.substring(0, 8)}...: ${fingerprintId.substring(0, 8)}... | Action: ${action}`);
  ...
  detectSuspiciousPatterns(userId: string): {
    ...
    const recentFingerprints = history.filter(fp => 
      Date.now() - fp.createdAt.getTime() < 60 * 60 * 1000 // Last hour
    );
    ...
    const vpnIndicators = recentFingerprints.filter(fp => 
      fp.ip?.startsWith('10.') || fp.ip?.startsWith('192.168.')
    );
    ...
    const automationIndicators = recentFingerprints.filter(fp =>
   ...
   */
  getAuditLog(userId: string, fromDate?: Date, toDate?: Date): DeviceFingerprint[] {
    const history = this.fingerprints.get(userId) || [];
  ...
  exportForCompliance(userId: string): {
    ...
    return {
      ...
      lastSeen: history.length > 0 ? history[history.length - 1].createdAt : null,
