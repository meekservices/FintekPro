  ...
  private async _send(payload: NotifyPayload): Promise<void> {
    ...
    try {
      ...
      const adminUsers = await db.select({ id: schema.users.id })
        .from(schema.users)
        .where(sql`${schema.users.roles} @> '["admin"]'::jsonb OR ${schema.users.roles} @> '["superadmin"]'::jsonb OR ${schema.users.roles} @> '["compliance_officer"]'::jsonb`)
        .limit(20);

      for (const admin of adminUsers) {
        await db.insert(schema.userNotifications).values({
          userId: admin.id,
          type: priority === 'critical' || priority === 'high' ? 'alert' : 'warning',
          title,
          message: body,
      ...
      logger.warn('[AdminParallelNotifier] DB insert failed, continuing with email notifications', {
        taskType,
    ...
    for (const email of recipients) {
      try {
        const prefix = priority === 'critical' ? '🚨 CRITICAL: ' : priority === 'high' ? '⚠️ ' : '';
        ...
        logger.warn('[AdminParallelNotifier] Email send failed', { email, taskType });
      }
    }
    ...
    if (priority === 'critical' || priority === 'high') {
      ...
      if (complianceMobile) {
        try {
          ...
          logger.warn('[AdminParallelNotifier] WhatsApp send failed', { taskType });
        }
      }
    }

    logger.info('[AdminParallelNotifier] Task dispatched', { taskType, priority, recipients: recipients.length });
  }
}
...
export function notifyGatewayNotConfigured(params: {
  ...
  adminParallelNotifier.dispatch({
    ...
    metadata: { instrumentType: params.instrumentType, provider: params.provider, missingKeys: params.missingKeys },
  });
}
