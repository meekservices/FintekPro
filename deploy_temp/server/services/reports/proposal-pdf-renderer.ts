...
export class ProposalPDFRenderer {
  ...
  async generateProposal(config: ProposalConfig, clientData: ClientData): Promise<Buffer> {
    ...
    if (config.sections.termsConditions || config.settings.includeSEBIDisclosure) {
      this.addNewPage();
  ...
  private addFooter(): void {
    ...
    this.pdf.text(`Page ${this.pageNumber}`, this.pageWidth - this.margin - 15, this.pageHeight - 10);
    this.pdf.text('FintekPro Financial Services', this.margin, this.pageHeight - 10);
  }

  private renderCoverPage(config: ProposalConfig, clientData: ClientData): void {
    this.pageNumber++;
    ...
    this.pdf.text(config.coverPage.title || 'Investment Proposal', this.margin, 50);

    this.pdf.setFontSize(16);
    this.pdf.setFont('helvetica', 'normal');
    this.pdf.text(`Prepared for: ${config.coverPage.clientName || clientData.fullName}`, this.margin, 70);
    ...
    const details = [
      ...
      ['Prepared By:', config.coverPage.preparedBy || 'FintekPro Financial Advisor'],
      ['Date:', config.coverPage.date || new Date().toLocaleDateString('en-IN')],
  ...
  private renderExecutiveSummary(config: ProposalConfig, clientData: ClientData): void {
...
Our recommended asset allocation is designed to balance growth potential with risk management, ensuring your investments align with your financial objectives and comfort level.`;
    ...
    const lines = this.pdf.splitTextToSize(summary, this.pageWidth - 2 * this.margin);
  ...
  private renderAssetAllocation(config: ProposalConfig): void {
    ...
    const allocations = [
      { name: 'Equity', value: config.assetAllocation.equity, color: COLORS.equity, description: 'Stocks, equity mutual funds' },
      { name: 'Debt', value: config.assetAllocation.debt, color: COLORS.debt, description: 'Bonds, FDs, debt funds' },
      { name: 'Gold', value: config.assetAllocation.gold, color: COLORS.gold, description: 'Gold ETFs, SGBs' },
      { name: 'Real Estate', value: config.assetAllocation.realestate, color: COLORS.realestate, description: 'REITs, property' },
      { name: 'Cash', value: config.assetAllocation.cash, color: COLORS.cash, description: 'Liquid funds, savings' },
    ].filter(a => a.value > 0);
    ...
    const rationaleLines = this.pdf.splitTextToSize(rationale, this.pageWidth - 2 * this.margin);
  ...
  private renderProjectedReturns(config: ProposalConfig): void {
    ...
    const years = this.getYearsFromHorizon(config.investmentGoals.investmentHorizon);
    
    const projections = [];
    let accumulated = 0;
    
    for (let year = 1; year <= years; year++) {
      const yearlyContribution = config.investmentGoals.monthlyContribution * 12;
  ...
  private renderFeeDisclosure(): void {
    ...
    const fees = [
      ...
      ['Exit Load (if applicable)', 'Percentage', 'As per fund', 'On redemption'],
    ...
    const lines = this.pdf.splitTextToSize(disclosure, this.pageWidth - 2 * this.margin);
    this.pdf.text(lines, this.margin, this.currentY);
  }
    ...
    const sections = [
      {
        ...
        content: 'This proposal is for informational purposes only and should not be construed as investment advice. Investment decisions should be made based on your individual circumstances, risk tolerance, and financial goals. Past performance is not indicative of future results.',
      },
      {
        title: 'SEBI Registration',
        content: 'FintekPro is a SEBI registered investment advisor. All investment recommendations are made in compliance with SEBI (Investment Advisers) Regulations, 2013.',
      },
      {
      ...
      },
      {
        ...
        content: 'We may receive commissions from mutual fund houses for investments made through our platform. However, our recommendations are made solely based on what we believe is in the best interest of the client.',
      },
      {
        title: 'Privacy Policy',
        content: 'Your personal and financial information is kept strictly confidential and is used only for the purpose of providing investment services. We do not share your information with third parties without your consent.',
      },
    ];
  ...
  private formatGoalName(goal: string): string {
    ...
    return names[goal] || goal.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase());
  }
  ...
  private getRecommendations(config: ProposalConfig): Array<{category: string; instruments: string; allocation: string; risk: string}> {
...
export async function generateProposalPDF(config: any, clientData: any): Promise<Buffer> {
  const renderer = new ProposalPDFRenderer(config.settings?.orientation || 'portrait');
