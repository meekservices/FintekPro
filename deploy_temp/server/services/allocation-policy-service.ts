...
export const AssetClassBandSchema = z.object({
  target: z.number().min(0).max(100),
  lowerBand: z.number().min(0).max(100),
  upperBand: z.number().min(0).max(100)
}).refine(data => data.lowerBand <= data.target && data.target <= data.upperBand, {
...
export class AllocationPolicyService {
  ...
  static getInstance(): AllocationPolicyService {
    if (!this.instance) {
      this.instance = new AllocationPolicyService();
    }
  ...
  normalizeAllocations(holdings: Array<{ assetType: string; currentValue: number }>): Record<string, number> {
    ...
    for (const holding of holdings) {
      const normalizedClass = assetClassMapping[holding.assetType.toLowerCase()] || 'alternates';
      allocations[normalizedClass] = (allocations[normalizedClass] || 0) + holding.currentValue;
    }
