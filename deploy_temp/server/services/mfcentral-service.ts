...
class MFCentralService {
  ...
  private buildRequestEnvelope(payload: object): { signature: string; request: string } {
  ...
  async initiateCASRequest(
    ...
    if (!this.isConfigured) {
      return { requestId: `stub-${Date.now()}`, message: "OTP sent (stub mode)" };
    }
    ...
    return { requestId: result.requestId || result.req_id, message: result.message || "OTP sent" };
  }
    ...
    const folios: CASHolding[] = (raw.folios || raw.holdings || []).map(
      (f: any) => {
        ...
        return {
          ...
          marketValue: parseFloat(f.currentValue || f.market_value || "0"),
          investedValue: parseFloat(f.investedValue || f.invested_value || "0"),
          gainLoss: parseFloat(f.gainLoss || f.gain_loss || "0"),
          gainLossPercent: parseFloat(f.gainLossPercent || f.gain_loss_pct || "0"),
          sipFlag: !!(sip.sipAmount || f.sipFlag),
          sipAmount: sip.sipAmount ? parseFloat(sip.sipAmount) : undefined,
