...
interface W9FormData {
  name: string;
  businessName?: string;
  federalTaxClassification: 'individual' | 'c_corporation' | 's_corporation' | 'partnership' | 'trust_estate' | 'llc' | 'other';
...
class W8W9FormService {
   ...
   * Create W-8BEN form
   */
  createW8BEN(userId: string, data: Omit<W8FormData, 'formType' | 'expiresAt'>): W8FormData {
    ...
    console.log(`📋 [W8/W9] Created W-8BEN for user ${userId.substring(0, 8)}..., expires: ${expiresAt.toISOString()}`);
   ...
   * Create W-9 form
   */
  createW9(userId: string, data: Omit<W9FormData, 'certificationDate'>): W9FormData {
  ...
  getForm(userId: string): (W8FormData | W9FormData) | null {
    return this.forms.get(`W8-${userId}`) || this.forms.get(`W9-${userId}`) || null;
  }
      ...
      if (key.startsWith('W8-') && 'expiresAt' in form) {
        ...
        if (w8Form.expiresAt <= cutoffDate && w8Form.expiresAt > now) {
          const daysUntilExpiry = Math.ceil(
            (w8Form.expiresAt.getTime() - now.getTime()) / (24 * 60 * 60 * 1000)
          );
          expiring.push({
   ...
   * Validate form data
   */
  validateW8BEN(data: Partial<W8FormData>): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];

    if (!data.beneficialOwnerName) errors.push('Beneficial owner name is required');
    if (!data.countryOfCitizenship) errors.push('Country of citizenship is required');
    if (!data.countryOfResidence) errors.push('Country of residence is required');
    if (!data.permanentResidenceAddress?.line1) errors.push('Permanent residence address is required');
    if (!data.permanentResidenceAddress?.country) errors.push('Country in address is required');
    
    // Check for US person indicators
    if (data.countryOfCitizenship?.toLowerCase() === 'usa' || 
  ...
  exportForCompliance(userId: string): {
    ...
    return {
      ...
      countryOfResidence: isW8 ? (form as W8FormData).countryOfResidence : 'United States'
    };
  }
}
