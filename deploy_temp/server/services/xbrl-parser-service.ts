...
const INDIAN_XBRL_NAMESPACES = [
  'in-gaap', 'in-as', 'in-bse', 'in-nse',
  'http://www.infosys.com/', 'http://www.tatamotors.com/',
];

const CANONICAL_METRIC_MAP: Record<string, { canonical: string; priority: number }> = {
  ...
  'EarningsBeforeInterestTaxDepreciationAmortization': { canonical: 'ebitda', priority: 2 },
  ...
  'DepreciationAndAmortisationExpense': { canonical: 'depreciation', priority: 1 },
...
class XBRLParserService {
  ...
  async parseXBRL(content: string, sourceUrl?: string): Promise<XBRLParseResult> {
    ...
    try {
      const isIXBRL = content.includes('ix:') || content.includes('<html') || content.includes('<HTML');
      ...
      result.overallConfidence = this.calculateOverallConfidence(result.metrics);
      result.success = result.metrics.length > 0;

      console.log(`[XBRL] Parsed ${result.metrics.length} metrics with ${(result.overallConfidence * 100).toFixed(1)}% confidence`);
    } catch (error: any) {
  ...
  private extractXBRLFromIXBRL(content: string): string {
    ...
    const ixNonNumericPattern = /<ix:nonNumeric[^>]*name="([^"]+)"[^>]*>([^<]*)<\/ix:nonNumeric>/gi;
    const ixNumericPattern = /<ix:nonFraction[^>]*name="([^"]+)"[^>]*>([^<]*)<\/ix:nonFraction>/gi;
  ...
  private extractContexts(xbrlRoot: any): Record<string, any> {
    ...
    const contextArray = Array.isArray(contextElements) ? contextElements : [contextElements];
  ...
  private extractCompanyInfo(xbrlRoot: any): {
    ...
    const companyNameTags = [
      'NameOfCompany', 'CompanyName', 'in-gaap:NameOfCompany',
  ...
  private extractMetrics(xbrlRoot: any, contexts: Record<string, any>): XBRLExtractedMetric[] {
    ...
    const flattenObject = (obj: any, prefix = ''): Record<string, any> => {
      ...
      for (const key of Object.keys(obj)) {
        if (key.startsWith('@_') || key === 'context' || key === 'unit') continue;
    ...
    for (const [tag, value] of Object.entries(flatElements)) {
      ...
      if (!existing || mapping.priority < (CANONICAL_METRIC_MAP[existing.metric]?.priority || 99)) {
  ...
  private parseNumericValue(value: any): number | null {
    ...
    if (!strValue || strValue === '-' || strValue.toLowerCase() === 'nil') return null;
  ...
  getSupportedMetrics(): string[] {
    const canonicals = new Set(Object.values(CANONICAL_METRIC_MAP).map(m => m.canonical));
  ...
  async extractAndPersistMetrics(
    ...
    for (const metric of parseResult.metrics) {
      try {
        ...
        const hashCurrent = crypto.createHash('sha256').update(hashData).digest('hex').slice(0, 16);
        ...
        result.errors.push(`Failed to insert ${metric.canonicalName}: ${error.message}`);
      }
    }
