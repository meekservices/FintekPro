...
class UnlistedStockMetricsService {
   ...
   */
  async calculateMetricsForCompany(companyId: string): Promise<UnlistedMetricsResult | null> {
    try {
      ...
      const [latestFinancial] = await db.select()
      ...
      const netProfit = parseFloat(latestFinancial.netProfit || latestFinancial.pat || '0');
      const networth = parseFloat(latestFinancial.networth || '0');
      const totalDebt = parseFloat(latestFinancial.totalDebt || '0');
      const totalAssets = parseFloat(latestFinancial.totalAssets || '0');
      ...
      const longTermDebt = parseFloat(latestFinancial.longTermDebt || '0');
      const shareCapital = parseFloat(latestFinancial.shareCapital || company.paidUpCapital || '0');
      
      const currentPrice = parseFloat(company.currentPrice || '0');
      const faceValue = parseFloat(company.faceValue || '10');
      ...
      this.auditLog.push({
        ...
        companyName: company.name,
        ratios: result,
        formula: 'ROE=PAT/Networth, ROCE=EBIT/CE, P/E=Price/EPS, P/B=Price/BVPS',
        dataSource: 'mca_financials',
      ...
      console.error(`[UnlistedMetrics] Error calculating for ${companyId}:`, error.message);
      return null;
    }
  }
  ...
  async updateCompanyRatios(companyId: string): Promise<boolean> {
    try {
      ...
      console.log(`[UnlistedMetrics] Updated ${metrics.companyName}: ROE=${metrics.roe?.toFixed(2)}%, P/E=${metrics.peRatio?.toFixed(2) || 'N/A'}`);
      return true;
    } catch (error: any) {
      console.error(`[UnlistedMetrics] Failed to update ${companyId}:`, error.message);
      return false;
    }
  }
  ...
  async batchRefreshAllRatios(): Promise<BatchRefreshResult> {
    console.log('[UnlistedMetrics] Starting batch refresh of all unlisted company ratios...');
    ...
    try {
      ...
      console.log(`[UnlistedMetrics] Found ${companies.length} active unlisted companies`);

      for (const company of companies) {
        const hasFinancials = await db.select({ id: companyFinancials.id })
          .from(companyFinancials)
    ...
    return {
      ...
      formulas: {
        ...
        ROA: {
          ...
          unit: 'percentage',
          description: 'Return on Assets - measures efficiency in using assets to generate profits',
        },
        ROCE: {
          formula: '(EBIT / Capital Employed) × 100',
          capitalEmployed: 'Networth + Long-term Debt',
          unit: 'percentage',
          description: 'Return on Capital Employed - measures returns on all capital invested',
        },
        PE_Ratio: {
          ...
          unit: 'ratio',
          description: 'Price to Earnings ratio - valuation relative to earnings',
        },
        PB_Ratio: {
        ...
        EBITDA_Margin: {
          ...
          unit: 'percentage',
          description: 'Operating profitability before depreciation and amortization',
        },
      },
      compliance: {
