...
async function pushToRetryQueue(
  ...
  if (existing.length > 0) {
    ...
    if (entry.retryCount >= entry.maxRetries) {
      await logJob(jobType, instrumentId, symbol, 'MAX_RETRIES_EXCEEDED', `Retry cap (${entry.maxRetries}) reached: ${error}`);
      return;
    }
    const backoffMinutes = Math.pow(2, entry.retryCount) * 15;
    await db
...
export async function runDailyPriceUpdate(): Promise<DailyUpdateResult> {
  const result: DailyUpdateResult = { processed: 0, skipped: 0, errors: 0, retried: 0, apiCalls: 0 };
  ...
  console.log(`[DailyPriceUpdater] Found ${instruments.length} instruments needing update`);

  for (const instrument of instruments) {
    try {
      const { result: quote } = await registry.getQuote(`${instrument.symbol}.NS`);
      result.apiCalls++;
      ...
      await logJob('DAILY_UPDATE', instrument.id, instrument.symbol, 'SUCCESS', null, 1);
      ...
      result.errors++;
      await logJob('DAILY_UPDATE', instrument.id, instrument.symbol, 'FAILED', err.message);
      await pushToRetryQueue(instrument.id, instrument.symbol, 'DAILY_UPDATE', err.message);
    }
  }

  await processRetryQueue('DAILY_UPDATE', result, registry);

  console.log(`[DailyPriceUpdater] Complete: ${result.processed} updated, ${result.skipped} skipped, ${result.errors} errors, ${result.retried} retries`);
  await logJob('DAILY_UPDATE_BATCH', null, null, 'COMPLETE', 
  ...
  for (const item of retryItems) {
    try {
      ...
      if (quote && quote.price) {
        ...
        result.retried++;
        await logJob(jobType, item.instrumentId, item.symbol, 'RETRY_SUCCESS', `Resolved on retry ${item.retryCount}`);
      }
    } catch (err: any) {
      await pushToRetryQueue(item.instrumentId, item.symbol || '', jobType, err.message);
    }
  }
}
