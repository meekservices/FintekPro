...
export class CachedVerificationService {
   ...
   */
  async verifyPAN(pan: string, name: string, requestedBy?: string): Promise<CachedVerificationResult> {
    const panUpper = pan.toUpperCase();
    
    // 1. Check cache first
    const cached = await dataCacheService.getVerificationResult('pan', panUpper);
    if (cached) {
      console.log(`[CachedVerification] Cache HIT for PAN: ${panUpper.slice(0, 4)}****`);
      return {
        ...
        additionalData: typeof cached.additional_data === 'string' 
    ...
    console.log(`[CachedVerification] Cache MISS for PAN: ${panUpper.slice(0, 4)}****, calling Cashfree API`);
   ...
   */
  async verifyGSTIN(gstin: string, requestedBy?: string): Promise<CachedVerificationResult> {
    const gstinUpper = gstin.toUpperCase();
    
    // 1. Check cache first
    const cached = await dataCacheService.getVerificationResult('gstin', gstinUpper);
    if (cached) {
      console.log(`[CachedVerification] Cache HIT for GSTIN: ${gstinUpper.slice(0, 4)}****`);
      return {
        verified: cached.verified,
        fromCache: true,
        registeredName: cached.registered_name,
    ...
    console.log(`[CachedVerification] Cache MISS for GSTIN: ${gstinUpper.slice(0, 4)}****, calling Sandbox API`);
   ...
   */
  async verifyCompanyByCIN(cin: string, requestedBy?: string): Promise<CachedVerificationResult> {
    ...
    console.log(`[CachedVerification] Cache MISS for CIN: ${cinUpper}, calling Sandbox API`);
    
    // 2. Call Sandbox MCA API
    try {
      const apiResult = await this.sandboxService.getMCACompanyDetails(cinUpper);
      ...
      await dataCacheService.saveCompanyToCache({
        ...
        dateOfIncorporation: apiResult.dateOfIncorporation ? new Date(apiResult.dateOfIncorporation) : undefined,
        registrationNumber: apiResult.registrationNumber,
        registeredAddress: apiResult.registeredAddress,
        authorizedCapital: parseFloat(apiResult.authorizedCapital?.replace(/[^0-9.]/g, '')) || undefined,
        paidUpCapital: parseFloat(apiResult.paidUpCapital?.replace(/[^0-9.]/g, '')) || undefined,
