...
import { PriceSuggestionService, type PriceSuggestion } from './price-suggestion';
import { moneyControlScraper } from './moneycontrol-scraper';
import type { UnlistedCompany, SellListing, BuyRequest, UnlistedDeal, UnlistedPriceHistory } from '@shared/schema';
...
class UnlistedPriceAggregationService {
   ...
   */
  async getAggregatedPriceSuggestion(companyId: string): Promise<AggregatedPriceSuggestion> {
   ...
   */
  private async getMoneyControlPrice(company: UnlistedCompany): Promise<MoneyControlPrice> {
    try {
      ...
      if (moneyControlPrices.length > 0) {
        ...
        const change = previousPrice 
          ? price - parseFloat(previousPrice.price) 
          : null;
        const changePercent = previousPrice && parseFloat(previousPrice.price) > 0
          ? ((price - parseFloat(previousPrice.price)) / parseFloat(previousPrice.price)) * 100
          : null;
      ...
      const searchResult = await moneyControlScraper.searchISINByCompanyName(company.name);
  ...
  private async getInternalCalculation(companyId: string): Promise<InternalCalculation> {
    try {
      const suggestion = await this.priceSuggestionService.calculateSuggestedPrice(companyId);
      ...
      console.error('[PriceAggregation] Internal calculation error:', error.message);
      return {
        suggestedPrice: null,
        minPrice: null,
        maxPrice: null,
   ...
   */
  private async getMarketplacePrice(companyId: string): Promise<MarketplacePrice> {
    try {
      ...
      if (activeRequests.length > 0) {
        const sortedBids = activeRequests.sort((a: BuyRequest, b: BuyRequest) => 
          parseFloat(b.maxPrice) - parseFloat(a.maxPrice)
        );
        bestBid = parseFloat(sortedBids[0].maxPrice);
        // Sum volume at or near best bid (within 2%)
        bidVolume = sortedBids
          .filter((r: BuyRequest) => parseFloat(r.maxPrice) >= bestBid! * 0.98)
      ...
      if (activeListings.length > 0) {
        const sortedAsks = activeListings.sort((a: SellListing, b: SellListing) => 
          parseFloat(a.landingPrice || a.askPrice) - parseFloat(b.landingPrice || b.askPrice)
        );
        bestAsk = parseFloat(sortedAsks[0].landingPrice || sortedAsks[0].askPrice);
        // Sum volume at or near best ask (within 2%)
        askVolume = sortedAsks
          .filter((l: SellListing) => parseFloat(l.landingPrice || l.askPrice) <= bestAsk! * 1.02)
      ...
      const recentDeals = deals.filter((d: UnlistedDeal) => {
        return d.status === 'completed' && new Date(d.createdAt || now) >= ninetyDaysAgo;
      });
      ...
      if (recentDeals.length > 0) {
        ...
        const totalValue = recentDeals.reduce((sum: number, d: UnlistedDeal) => 
          sum + parseFloat(d.agreedPrice) * d.quantity, 0);
        const totalQuantity = recentDeals.reduce((sum: number, d: UnlistedDeal) => 
          sum + d.quantity, 0);
        recentClearingPrice = Math.round(totalValue / totalQuantity);
        
        // Get most recent deal date
        const sortedDeals = recentDeals.sort((a: UnlistedDeal, b: UnlistedDeal) => 
      ...
      return {
        ...
        lastDealDate,
        lastUpdated: new Date(),
        available: bestBid !== null || bestAsk !== null || recentClearingPrice !== null,
      };
    } catch (error: any) {
  ...
  private calculateRecommendedPrices(
    ...
    marketplace: MarketplacePrice
  ): { buyPrice: number | null; sellPrice: number | null; confidence: 'high' | 'medium' | 'low' } {
   ...
   */
  async getBatchPriceSuggestions(companyIds: string[]): Promise<AggregatedPriceSuggestion[]> {
    const results = await Promise.all(
      companyIds.map(async (id) => {
        try {
          return await this.getAggregatedPriceSuggestion(id);
        } catch (error: any) {
          console.error(`[PriceAggregation] Failed for company ${id}:`, error.message);
          return null;
        }
      })
    );
   ...
   */
  async refreshMoneyControlPrice(companyId: string): Promise<MoneyControlPrice> {
    const company = await storage.getUnlistedCompanyById(companyId);
    if (!company) {
      throw new Error('Company not found');
    }
