...
class MFTaxService {
  private detectFundType(category: string, schemeName: string): string {
    ...
    if (lowerName.includes('gold') || lowerName.includes('silver') || lowerCategory.includes('gold')) {
      return 'gold_etf';
    }
    if (lowerCategory.includes('equity') || lowerCategory.includes('large cap') || 
        lowerCategory.includes('mid cap') || lowerCategory.includes('small cap') ||
        lowerCategory.includes('flexi') || lowerCategory.includes('multi cap')) {
      return 'equity';
    }
    if (lowerCategory.includes('debt') || lowerCategory.includes('liquid') || 
        lowerCategory.includes('money market') || lowerCategory.includes('gilt') ||
        lowerCategory.includes('corporate bond') || lowerCategory.includes('banking')) {
      return 'debt';
    }
  ...
  async getTaxRules(fundType: string): Promise<Array<{
    ...
    const rules = await db
      ...
      .where(
        and(
          eq(mfTaxRules.fundType, fundType),
          isNull(mfTaxRules.effectiveTo)
        )
      );
    ...
    return rules.map(r => ({
      ...
      exemptionLimit: r.exemptionLimit ? parseFloat(r.exemptionLimit) : null,
      cessRate: parseFloat(r.cessRate || '4'),
  ...
  async calculateTax(
    ...
    let applicableRule = rules.find(r => 
    ...
    const holdingPeriod = applicableRule.holdingPeriodType as 'short_term' | 'long_term';
    const exemptionUsed = applicableRule.exemptionLimit ? 
      Math.min(gain, applicableRule.exemptionLimit) : 0;
  ...
  async getExitLoadForScheme(schemeCode: string, holdingDays: number): Promise<{
    exitLoadPercent: number;
    description: string;
    daysToZeroExitLoad: number | null;
  ...
  async calculateExitLoad(
    ...
    const exitLoadInfo = await this.getExitLoadForScheme(schemeCode, holdingDays);
    const exitLoadAmount = redemptionAmount * exitLoadInfo.exitLoadPercent / 100;
    ...
    return {
      ...
      netRedemption: Math.round((redemptionAmount - exitLoadAmount) * 100) / 100,
  ...
  async calculateWithdrawalSummary(
    ...
    const exitLoad = await this.calculateExitLoad(schemeCode, currentValue, holdingDays);
    
    const taxableGain = absoluteGain > 0 ? absoluteGain : 0;
    const tax = await this.calculateTax(taxableGain, holdingDays, category, schemeName, slabRate);

    const netProceeds = exitLoad.netRedemption - tax.totalTax;
    const effectiveReturn = ((netProceeds - investmentAmount) / investmentAmount) * 100;
  ...
  getExitLoadTimeline(schemeCode: string): Promise<Array<{
    tier: number;
    minDays: number;
    maxDays: number | null;
    exitLoadPercent: number;
    ...
    return db
      ...
      .then(rows => rows.map(r => ({
        ...
        minDays: r.minDays,
        maxDays: r.maxDays,
        exitLoadPercent: parseFloat(r.exitLoadPercent),
        description: r.description || ''
      })));
  }
