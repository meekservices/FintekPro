...
class RequestDeduplicationService {
  ...
  async dedupe<T>(
    ...
    if (existing) {
      ...
      console.log(`[RequestDedupe] Deduplicated request for: ${key} (${existing.callers} callers)`);
   ...
   */
  createKey(service: string, operation: string, ...identifiers: string[]): string {
  ...
  getMetrics() {
    return {
      ...
      savingsPercentage: this.metrics.totalRequests > 0
        ? ((this.metrics.apiCallsSaved / this.metrics.totalRequests) * 100).toFixed(2)
        : '0.00',
    };
  }
