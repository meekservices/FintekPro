...
import { db } from '../db';
import { mfNavHistory, mfMonthlyReturns, mfSchemeExitLoads, mutualFunds } from '@shared/schema';
...
class MFDataSyncService {
  ...
  async syncNavHistoryForScheme(schemeCode: string, maxDays: number = 1825): Promise<number> {
    try {
      ...
      console.log(`[MFSync] Synced ${inserted} NAV records for scheme ${schemeCode}`);
      return inserted;
    } catch (error: any) {
      console.error(`[MFSync] Error syncing NAV for ${schemeCode}:`, error.message);
      return 0;
    }
  }

  async syncNavHistoryForAllFunds(limit: number = 100): Promise<{ total: number; synced: number }> {
    ...
    console.log(`[MFSync] Synced NAV history for ${synced}/${funds.length} funds`);
  ...
  async calculateMonthlyReturnsForScheme(schemeCode: string): Promise<number> {
    try {
      ...
      const monthlyData = new Map<string, { first: { date: string; nav: number }; last: { date: string; nav: number } }>();
      ...
      for (const [monthYear, data] of monthlyData) {
        const returnPercent = ((data.last.nav - data.first.nav) / data.first.nav) * 100;
      ...
      console.error(`[MFSync] Error calculating monthly returns for ${schemeCode}:`, error.message);
      return 0;
    }
  }
  ...
  async calculateReturnsFromLocalHistory(schemeCode: string): Promise<{
    ...
    try {
      ...
      const currentNav = parseFloat(latestNav[0].nav);
      ...
      const getNavNearDate = async (targetDate: Date): Promise<number | null> => {
        ...
        const result = await db
          ...
          .orderBy(sql`ABS(nav_date - ${this.formatDateForDB(targetDate)}::date)`)
          .limit(1);

        return result.length ? parseFloat(result[0].nav) : null;
      };
      ...
      const returns3y = nav3yAgo ? (Math.pow(currentNav / nav3yAgo, 1/3) - 1) * 100 : 0;
      const returns5y = nav5yAgo ? (Math.pow(currentNav / nav5yAgo, 1/5) - 1) * 100 : 0;
      ...
      console.error(`[MFSync] Error calculating returns for ${schemeCode}:`, error.message);
      return null;
    }
  }
  ...
  async updateMutualFundReturns(schemeCode: string): Promise<boolean> {
    ...
    try {
      ...
      console.error(`[MFSync] Error updating returns for ${schemeCode}:`, error.message);
      return false;
    }
  }

  async batchUpdateAllReturns(limit: number = 500): Promise<{ updated: number; failed: number }> {
    const funds = await db
      .select({ schemeCode: mutualFunds.schemeCode })
      .from(mutualFunds)
      .limit(limit);
    ...
    console.log(`[MFSync] Updated returns: ${updated} success, ${failed} failed`);
  ...
  async fetchExitLoadFromCaptnemo(isin: string): Promise<{
    ...
    try {
      const response = await axios.get<CaptnemoResponse>(`${this.CAPTNEMO_BASE_URL}/${isin}.json`, {
        timeout: 10000
      });
      ...
      if (response.data?.exit_load) {
        ...
        if (daysMatch) {
          ...
          else if (exitLoadStr.toLowerCase().includes('month')) days = value * 30;
          else days = value;
        }

        return {
          exitLoadPercent: percentMatch ? parseFloat(percentMatch[1]) : 0,
          exitLoadDays: days,
      ...
      console.error(`[MFSync] Error fetching exit load for ${isin}:`, error.message);
      return null;
    }
  }

  async syncExitLoadForScheme(schemeCode: string, isin: string): Promise<boolean> {
    ...
    try {
      ...
      console.error(`[MFSync] Error saving exit load for ${schemeCode}:`, error.message);
      return false;
    }
  }
  ...
  async getExitSignals(schemeCode: string): Promise<{
    ...
    try {
      ...
      for (const r of recentReturns) {
        if (parseFloat(r.returnPercent || '0') < 0) consecutiveNegative++;
        else break;
      }

      const trailing6M = recentReturns.reduce((sum, r) => sum + parseFloat(r.returnPercent || '0'), 0);
      ...
      console.error(`[MFSync] Error getting exit signals for ${schemeCode}:`, error.message);
      return null;
    }
  }
