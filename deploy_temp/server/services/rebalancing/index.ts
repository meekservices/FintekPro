...
export class RebalancePlanner {
  ...
  public async evaluatePortfolio(
    ...
    let auditTraceContext = { trigger: "drift_threshold", governanceDecision: "PENDING" };
    ...
    try {
      ...
      if (marketRegimeDetector.detectBlackSwanEvent()) {
        return { 
          ...
          governance_reasoning: ["SYSTEM SUSPENSION: 10σ Black Swan Trigger Active. Automated Rebalancing Disabled for Capital Protection."] 
        };
      }

      // 1. Structural Drift Detection
      const driftMatrix = driftEngine.calculateDrift(targetModel, currentAllocation);
      
      if (!driftMatrix.has_drifted) {
        return { status: "BLOCK", audit_id: rebalanceAuditId, governance_reasoning: ["No structural drift threshold breached. Avoiding unnecessary churn."] };
      }

      // 2. Instantiate Theoretical Optimize Plan & Tax Estimation
      const { plan, taxContexts } = rebalanceOptimizer.generateOptimizedPlan(driftMatrix, portfolioValuation);
      const taxEstimation = taxEstimationEngine.estimateRebalanceImpact(taxContexts);
      ...
      const simulatedStressCheck = stressTestingInterceptor.evaluateSimulationViolations(simulatedState, stressTestRules);

      if (simulatedStressCheck.status === "BLOCK") {
         this.persistAudit(rebalanceAuditId, targetModel.portfolio_id, plan, simulatedState, "BLOCK", auditTraceContext.trigger);
         return { status: "BLOCK", audit_id: rebalanceAuditId, governance_reasoning: [simulatedStressCheck.reason || "Severe Simulation Override"] };
      }
      ...
      if (aageCheck.decision === "BLOCK") {
         this.persistAudit(rebalanceAuditId, targetModel.portfolio_id, plan, simulatedState, "BLOCK", auditTraceContext.trigger);
         return { status: "BLOCK", audit_id: rebalanceAuditId, governance_reasoning: ["AAGE Compliance Block: Policy Overreach."] };
      }

      // 5. Success Path: Escalate to User/Advisor approval
      this.persistAudit(rebalanceAuditId, targetModel.portfolio_id, plan, simulatedState, "APPROVE", auditTraceContext.trigger);
      ...
      return { status: "BLOCK", audit_id: rebalanceAuditId, governance_reasoning: ["Internal Orchestration Error: Safe Mode Enacted."] };
    }
  }

  private persistAudit(id: string, portfolioId: string, plan: any, sim: any, decision: string, trigger: string) {
    db.insert(apreAuditLogs).values({
      id: id,
      ...
      approvalStatus: decision === "APPROVE" ? "pending_user_approval" : "blocked",
      executionStatus: "not_started"
    }).execute().catch(e => console.error("Failed to commit APRE audit state", e));
  }
}
