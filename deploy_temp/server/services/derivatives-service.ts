...
const LOT_SIZES: Record<string, number> = {
  ...
  'AXISBANK': 600, 'MARUTI': 100, 'TITAN': 175, 'BAJFINANCE': 125,
  'ASIANPAINT': 200, 'TATAMOTORS': 1425, 'SUNPHARMA': 350, 'HCLTECH': 350,
  'WIPRO': 1500, 'TATASTEEL': 5500, 'ONGC': 3850, 'NTPC': 2250,
  'POWERGRID': 2700, 'COALINDIA': 2100, 'JSWSTEEL': 675, 'HINDALCO': 1075,
};

// Baseline prices used as fallback when all live sources fail
const REFERENCE_PRICES: Record<string, number> = {
  'NIFTY': 24500, 'BANKNIFTY': 52000, 'FINNIFTY': 23500, 'MIDCPNIFTY': 12500,
  'RELIANCE': 2950, 'TCS': 4200, 'INFY': 1850, 'HDFCBANK': 1750,
  'ICICIBANK': 1280, 'SBIN': 840, 'BHARTIARTL': 1680, 'ITC': 485,
  'KOTAKBANK': 1850, 'LT': 3650, 'AXISBANK': 1180, 'MARUTI': 12500,
  'TITAN': 3750, 'BAJFINANCE': 7200, 'ASIANPAINT': 2350, 'TATAMOTORS': 780,
  'SUNPHARMA': 1920, 'HCLTECH': 1950, 'WIPRO': 295, 'TATASTEEL': 145,
  'ONGC': 255, 'NTPC': 385, 'POWERGRID': 345, 'COALINDIA': 420,
  'JSWSTEEL': 920, 'HINDALCO': 665,
};
...
class NseSession {
  ...
  private async refresh(): Promise<void> {
    try {
      ...
      this.cookies = raw.split(',').map(c => c.split(';')[0].trim()).filter(Boolean).join('; ');
...
class DerivativesService {
  ...
  constructor() {
    console.log('✅ Derivatives Service initialized (NSE + FMP + Python/yfinance tiers)');
  }
  ...
  private async fetchNSEOptionChain(symbol: string, expiry?: string): Promise<any | null> {
  ...
  private async fetchNSEFutures(symbol: string): Promise<any | null> {
    return nseSession.get(`https://www.nseindia.com/api/quote-derivative?symbol=${symbol}`);
  }
    ...
    try {
      ...
      return data
        ...
        .map(d => ({
          ...
          previousClose: d.previousClose ? parseFloat(d.previousClose) : undefined,
          change:        d.change        ? parseFloat(d.change)        : undefined,
          changePercent: d.changesPercentage ? parseFloat(d.changesPercentage) : undefined,
          dayHigh:       d.dayHigh ? parseFloat(d.dayHigh) : undefined,
          dayLow:        d.dayLow  ? parseFloat(d.dayLow)  : undefined,
          dataSource: 'fmp',
        }));
    } catch {
      return [];
  ...
  private async fetchPythonGlobalFutures(): Promise<GlobalFuture[]> {
    try {
      const resp = await callPython<{ futures: any[]; count: number }>('/derivatives/global-futures', 'GET');
      ...
      return resp.futures.map(f => ({
        ...
        previousClose: f.previousClose ? parseFloat(f.previousClose) : undefined,
        change:        f.change        ? parseFloat(f.change)        : undefined,
        changePercent: f.changePercent ? parseFloat(f.changePercent) : undefined,
        dayHigh:       f.dayHigh ? parseFloat(f.dayHigh) : undefined,
        dayLow:        f.dayLow  ? parseFloat(f.dayLow)  : undefined,
  ...
  private async fetchPythonNseSpot(symbols: string[]): Promise<Map<string, number>> {
    const prices = new Map<string, number>();
    if (!symbols.length) return prices;
    try {
      const resp = await callPython<{ results: Record<string, any> }>(
        '/derivatives/nse-spot', 'POST', { symbols },
      );
      if (!resp?.results) return prices;
      for (const [sym, data] of Object.entries(resp.results)) {
        if (data?.price > 0) prices.set(sym.toUpperCase(), parseFloat(data.price));
      }
    } catch { /* Python unavailable — fall through */ }
    return prices;
  }
  ...
  async getOptionsChain(symbol: string, expiryDate?: string): Promise<OptionsChain> {
    ...
    try {
      ...
      if (nseData?.records?.data?.length) {
        ...
        const underlying = nseData.records.underlyingValue || nseData.filtered?.IVs?.[0] || 0;
        ...
        const strikePrices = [...new Set<number>(filtered.map((d: any) => d.strikePrice))].sort((a, b) => a - b);
  ...
  async getOIAnalysis(symbol: string): Promise<OIAnalysis> {
    ...
    const spotPrice = chain.underlyingValue || await this.getSpotPrice(symbol);
    ...
    const allStrikes = [...new Set([...calls, ...puts].map(o => o.strikePrice))].sort((a, b) => a - b);
    let maxPainStrike = allStrikes[0] || spotPrice;
    let maxPainLoss = -Infinity;

    for (const strike of allStrikes) {
      const callLoss = calls.reduce((s, c) => s + c.openInterest * Math.max(0, c.strikePrice - strike), 0);
      const putLoss  = puts.reduce( (s, p) => s + p.openInterest * Math.max(0, strike - p.strikePrice), 0);
      const totalLoss = callLoss + putLoss;
      if (totalLoss > maxPainLoss) { maxPainLoss = totalLoss; maxPainStrike = strike; }
    }
  ...
  async getFuturesData(symbol: string): Promise<FuturesData[]> {
    ...
    try {
      ...
      if (futureRecords?.length) {
        ...
        const result: FuturesData[] = futureRecords.slice(0, 3).map((r: any) => {
          const m = r.metadata;
          const d = r.marketDeptOrderBook?.tradeInfo || {};
          const lastPrice = parseFloat(m.lastPrice || m.closePrice || '0');
          ...
          return {
            ...
            pChange:              parseFloat(m.pChange || '0'),
            openInterest:         parseInt(d.openInterest || m.openInterest || '0'),
            changeinOpenInterest: parseInt(d.changeinOpenInterest || '0'),
            totalTradedVolume:    parseInt(m.totalTradedVolume || '0'),
    ...
    const result: FuturesData[] = monthly.map((expiry, i) => {
      ...
      return {
        ...
        change:               Math.round((Math.random() - 0.5) * spotPrice * 0.01 * 100) / 100,
  ...
  async getGlobalFutures(): Promise<{
    ...
    if (fmpFutures.length >= 5) {
      const result = { futures: fmpFutures, count: fmpFutures.length, dataSource: 'fmp', timestamp: new Date().toISOString() };
      this.setCached(cacheKey, result);
      return result;
    }
    ...
    if (pyFutures.length >= 3) {
      ...
      const result = { futures: merged, count: merged.length, dataSource: 'python-yfinance', timestamp: new Date().toISOString() };
      this.setCached(cacheKey, result);
      return result;
    }

    // Tier 3: empty (let frontend show loading state)
    return { futures: [], count: 0, dataSource: 'unavailable', timestamp: new Date().toISOString() };
  }

  /** Merge two futures arrays, preferring FMP data when both have the same symbol */
  private mergeFutures(primary: GlobalFuture[], secondary: GlobalFuture[]): GlobalFuture[] {
  ...
  calculateGreeks(
    ...
    const S = spotPrice, K = strikePrice, r = riskFreeRate, sigma = volatility;

    if (T <= 0) return {
      delta: optionType === 'call' ? (S > K ? 1 : 0) : (S < K ? -1 : 0),
      gamma: 0, theta: 0, vega: 0, rho: 0, impliedVolatility: volatility * 100,
    };

    const d1 = (Math.log(S / K) + (r + sigma ** 2 / 2) * T) / (sigma * Math.sqrt(T));
    const d2 = d1 - sigma * Math.sqrt(T);
    const Nd1 = this.normalCDF(d1), Nd2 = this.normalCDF(d2), nd1 = this.normalPDF(d1);
  ...
  calculateStrategyPayoff(legs: StrategyLeg[], spotPrice: number, priceRange?: { min: number; max: number }): StrategyPayoff {
    ...
    for (let price = range.min; price <= range.max; price += step) {
      ...
      for (const leg of legs) {
        ...
        if (leg.type === 'call')  totalProfit += (Math.max(0, price - (leg.strikePrice || 0)) - (leg.premium || 0)) * mult * qty;
        else if (leg.type === 'put') totalProfit += (Math.max(0, (leg.strikePrice || 0) - price) - (leg.premium || 0)) * mult * qty;
  ...
  getMarginRequirement(symbol: string, legs: StrategyLeg[]) {
    ...
    let spanMargin = 0, exposureMargin = 0, premium = 0;
    for (const leg of legs) {
  ...
  getPopularStrategies() {
    return [
      { name: 'Long Call',          description: 'Buy a call option expecting price to rise',          outlook: 'bullish', legs: [{ type: 'call', action: 'buy' }], riskReward: 'Limited risk, Unlimited reward' },
      { name: 'Long Put',           description: 'Buy a put option expecting price to fall',           outlook: 'bearish', legs: [{ type: 'put', action: 'buy' }], riskReward: 'Limited risk, Limited reward' },
      { name: 'Covered Call',       description: 'Own stock and sell call to generate income',         outlook: 'neutral', legs: [{ type: 'stock', action: 'buy' }, { type: 'call', action: 'sell' }], riskReward: 'Limited upside, Stock risk' },
      { name: 'Bull Call Spread',   description: 'Buy lower strike call, sell higher strike call',     outlook: 'bullish', legs: [{ type: 'call', action: 'buy' }, { type: 'call', action: 'sell' }], riskReward: 'Limited risk, Limited reward' },
      { name: 'Bear Put Spread',    description: 'Buy higher strike put, sell lower strike put',       outlook: 'bearish', legs: [{ type: 'put', action: 'buy' }, { type: 'put', action: 'sell' }], riskReward: 'Limited risk, Limited reward' },
      { name: 'Long Straddle',      description: 'Buy call and put at same strike',                    outlook: 'volatile', legs: [{ type: 'call', action: 'buy' }, { type: 'put', action: 'buy' }], riskReward: 'Limited risk, Unlimited reward' },
      { name: 'Long Strangle',      description: 'Buy OTM call and OTM put',                          outlook: 'volatile', legs: [{ type: 'call', action: 'buy' }, { type: 'put', action: 'buy' }], riskReward: 'Limited risk, Unlimited reward' },
      { name: 'Iron Condor',        description: 'Sell OTM call spread and OTM put spread',           outlook: 'neutral', legs: [{ type: 'put', action: 'buy' }, { type: 'put', action: 'sell' }, { type: 'call', action: 'sell' }, { type: 'call', action: 'buy' }], riskReward: 'Limited risk, Limited reward' },
      { name: 'Iron Butterfly',     description: 'Sell ATM straddle, buy OTM strangle',              outlook: 'neutral', legs: [{ type: 'put', action: 'buy' }, { type: 'put', action: 'sell' }, { type: 'call', action: 'sell' }, { type: 'call', action: 'buy' }], riskReward: 'Limited risk, Limited reward' },
    ] as const;
  }
  ...
  private async buildSyntheticChain(symbol: string, expiryDate?: string): Promise<OptionsChain> {
    const spotPrice = await this.getSpotPrice(symbol);
    const expiries  = await this.getExpiryDates(symbol);
    const selected  = expiryDate || expiries[0];
    const isIndex   = NSE_INDICES.has(symbol.toUpperCase());
    const interval  = isIndex ? (symbol === 'BANKNIFTY' ? 100 : 50) : this.strikeInterval(spotPrice);
    const atm       = Math.round(spotPrice / interval) * interval;
    const strikes   = Array.from({ length: 31 }, (_, i) => atm + (i - 15) * interval);
    ...
    for (const strike of strikes) {
      ...
      const base = (price: number, type: 'CE' | 'PE'): OptionData => ({
        strikePrice: strike, expiryDate: selected, optionType: type,
        openInterest: oi(), changeinOpenInterest: Math.floor((Math.random() - 0.5) * 50_000),
        totalTradedVolume: Math.floor(Math.random() * 100_000),
        impliedVolatility: iv * 100, lastPrice: price,
        change: (Math.random() - 0.5) * price * 0.1, pChange: (Math.random() - 0.5) * 10,
        totalBuyQuantity: Math.floor(Math.random() * 10_000), totalSellQuantity: Math.floor(Math.random() * 10_000),
        bidQty: Math.floor(Math.random() * 1000), bidPrice: price * 0.99,
        askQty: Math.floor(Math.random() * 1000), askPrice: price * 1.01,
        underlyingValue: spotPrice,
      });
    ...
    const chain: OptionsChain = {
      symbol, underlyingValue: spotPrice, expiryDates: expiries,
      strikePrices: strikes, options: { calls, puts },
      timestamp: new Date().toISOString(), dataSource: 'synthetic',
    };
  ...
  private bsPrice(S: number, K: number, dte: number, sigma: number, type: 'call' | 'put'): number {
    const T = dte / 365;
    const r = 0.065;
  ...
  private categorizeFuture(symbol: string): string {
    ...
    if (/^(HG|ALI|ZN|ZB|ZT)=F/.test(symbol)) return symbol.startsWith('HG') || symbol.startsWith('ALI') ? 'industrial_metal' : 'bond';
  ...
  private marketOfFuture(symbol: string): string {
    if (/^(ES|NQ|YM|RTY|ZN|ZB|ZT|GC|SI|PL|PA|CL|HG|NG|HO|ALI|ZW|ZC|ZS|KC|CT|6[EJBAC])=F/.test(symbol)) return 'US/CME';
  ...
  private identifyStrategy(legs: StrategyLeg[]): string {
    const cb = legs.filter(l => l.type === 'call' && l.action === 'buy').length;
    const cs = legs.filter(l => l.type === 'call' && l.action === 'sell').length;
    const pb = legs.filter(l => l.type === 'put'  && l.action === 'buy').length;
    const ps = legs.filter(l => l.type === 'put'  && l.action === 'sell').length;
    const hs = legs.some(l => l.type === 'stock' || l.type === 'future');
    if (cb === 1 && cs === 0 && pb === 0 && ps === 0 && !hs) return 'Long Call';
    if (cb === 0 && cs === 1 && pb === 0 && ps === 0 && !hs) return 'Short Call';
    ...
    if (cb === 1 && cs === 0 && pb === 1 && ps === 0) return 'Long Straddle/Strangle';
    if (cb === 0 && cs === 1 && pb === 0 && ps === 1) return 'Short Straddle/Strangle';
    if (cb === 1 && cs === 1 && pb === 1 && ps === 1) return 'Iron Condor/Butterfly';
