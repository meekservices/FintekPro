...
const SCHEME_SCOPES: Record<SchemeType, string[]> = {
  epf: ['account_number', 'uan', 'employer_name', 'balance', 'contributions', 'interest', 'nominee_details'],
...
class GovernmentSchemeConsentOrchestrator {
  ...
  async initiateConsent(request: OTPRequest): Promise<{
    ...
    await this.logAuditEvent({
      ...
      details: {
        channel: request.channel,
        destination: request.mobile ? `XXXXXX${request.mobile.slice(-4)}` : request.email ? `${request.email.slice(0, 3)}***` : 'registered'
      }
    });
  ...
  async verifyOTPAndGrantConsent(verification: OTPVerification): Promise<{
    ...
    if (!consent) {
      return { success: false, message: 'Invalid or expired challenge. Please request a new OTP.' };
    }

    if (consent.userId !== verification.userId) {
      return { success: false, message: 'Unauthorized consent verification attempt.' };
    }

    if (consent.status !== 'pending') {
      return { success: false, message: `Consent is already ${consent.status}. Please request a new OTP.` };
    }
  ...
  async hasActiveConsent(userId: string, schemeType: SchemeType): Promise<{
    hasConsent: boolean;
    consent?: any;
  }> {
    const now = new Date();
  ...
  async revokeConsent(userId: string, schemeType: SchemeType, reason?: string, ipAddress?: string): Promise<{
    success: boolean;
    message: string;
  }> {
    const consents = await db.select()
    ...
    return { success: true, message: 'Consent revoked successfully. Your data will be retained as per regulatory requirements.' };
  }
  ...
  private async logAuditEvent(event: {
    ...
    const retentionExpiresAt = new Date(Date.now() + RETENTION_PERIOD_YEARS * 365 * 24 * 60 * 60 * 1000);
    ...
    console.log(`[SCHEME_AUDIT] ${event.eventType}: ${event.schemeType} for user ${event.userId.substring(0, 8)}...`);
  }

  async getAuditLog(userId: string, schemeType?: SchemeType, limit = 50): Promise<any[]> {
    let query = db.select()
  ...
  async cleanupExpiredRecords(): Promise<{ removedCount: number }> {
    ...
    const expiredRecords = await db.select({ id: schema.governmentSchemeAudit.id })
      .from(schema.governmentSchemeAudit)
    ...
    if (removedCount > 0) {
      await db.delete(schema.governmentSchemeAudit)
        .where(lt(schema.governmentSchemeAudit.retentionExpiresAt, now));
      console.log(`[SCHEME_AUDIT] Cleaned up ${removedCount} expired audit records`);
    }

    return { removedCount };
  }

  async getConsentStatus(userId: string): Promise<Record<SchemeType, {
    hasConsent: boolean;
    grantedAt?: Date;
    expiresAt?: Date;
    lastRefreshAt?: Date;
    ...
    for (const scheme of schemes) {
      const { hasConsent, consent } = await this.hasActiveConsent(userId, scheme);
      status[scheme] = {
        hasConsent,
...
export const governmentSchemeConsentOrchestrator = new GovernmentSchemeConsentOrchestrator();
