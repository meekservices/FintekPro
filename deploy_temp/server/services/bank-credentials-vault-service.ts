...
import { db } from '../db';
import { bankCredentialsVault, type BankCredentialsVault, type InsertBankCredentialsVault } from '@shared/dsa-loan-schema';
...
class BankCredentialsVaultService {
   ...
   */
  async storeCredential(params: StoreCredentialParams): Promise<{ success: boolean; credentialId?: string; error?: string }> {
    try {
      const { bankCode, credentialType, value, environment, description, expiresAt, createdBy } = params;
      ...
      if (existing.length > 0) {
        ...
        await db.update(bankCredentialsVault)
          .set({ 
            isActive: false, 
            updatedAt: new Date(),
            metadata: {
      ...
      const [inserted] = await db.insert(bankCredentialsVault)
        .values({
          bankCode,
          credentialType,
          encryptedValue,
          environment,
          keyVersion: existing.length > 0 ? (existing[0].keyVersion || 1) + 1 : 1,
          metadata: {
            description,
      ...
      console.log(`[Vault] Stored credential: ${bankCode}/${credentialType}/${environment}`);
  ...
  async getCredential(
    ...
    try {
      ...
      // Decrypt the value
      const decryptedValue = encryptionService.decrypt(credential.encryptedValue);
      if (!decryptedValue) {
        console.error(`[Vault] Failed to decrypt credential: ${bankCode}/${credentialType}`);
        return null;
      }
   ...
   */
  async listBankCredentials(bankCode: string, environment?: Environment): Promise<Array<{
    ...
    try {
      ...
      .where(
        and(
          eq(bankCredentialsVault.bankCode, bankCode),
          eq(bankCredentialsVault.isActive, true),
          environment ? eq(bankCredentialsVault.environment, environment) : undefined
        )
      );
  ...
  async revokeCredential(
    ...
    try {
      ...
      console.log(`[Vault] Revoked credential: ${bankCode}/${credentialType}/${environment}`);
      return true;
    } catch (error: any) {
   ...
   */
  async getExpiringCredentials(daysThreshold: number = 30): Promise<Array<{
    bankCode: string;
    credentialType: string;
    environment: string;
    expiresAt: Date;
    ...
    try {
      ...
      console.error('[Vault] Error checking expiring credentials:', error.message);
      return [];
    }
  }
   ...
   */
  private logAccess(bankCode: string, credentialType: string, environment: string) {
   ...
   */
  getAccessStats(bankCode: string, credentialType: string, environment: string): {
