...
class ABTestingService {
  ...
  constructor() {
    this.experiments.set(this.defaultExperiment.experimentId, this.defaultExperiment);
    console.log("✅ A/B Testing Service initialized");
  }

  assignClient(
    clientId: string,
    clientProfile: { risk_category: string; client_category: string; created_at?: Date }
    ...
    console.log(`[A/B TEST] Client ${clientId} assigned to Group ${group} (${mode})`);
  ...
  private isClientEligible(
    ...
    if (eligibilityCriteria.minRiskLevel) {
      const riskHierarchy = { 'conservative': 1, 'moderate': 2, 'aggressive': 3, 'very_aggressive': 4 };
      const clientRisk = riskHierarchy[clientProfile.risk_category as keyof typeof riskHierarchy] || 1;
      const minRisk = riskHierarchy[eligibilityCriteria.minRiskLevel as keyof typeof riskHierarchy] || 2;
    ...
    if (eligibilityCriteria.clientCategories) {
      if (!eligibilityCriteria.clientCategories.includes(clientProfile.client_category)) {
        return false;
      }
    }
  ...
  recordMetric(
    ...
    const experimentMetrics = this.metrics.get(assignment.experimentId) || { groupA: [], groupB: [] };
    const groupMetrics = assignment.group === 'A' ? experimentMetrics.groupA : experimentMetrics.groupB;
  ...
  private calculateGroupMetrics(metrics: any[]): GroupMetrics {
    ...
    const acceptanceMetrics = metrics.filter(m => m.metricType === 'acceptance');
    const allocationMetrics = metrics.filter(m => m.metricType === 'allocation');
    const timeMetrics = metrics.filter(m => m.metricType === 'time_to_decision');
    const engagementMetrics = metrics.filter(m => m.metricType === 'engagement');

    return {
      clientCount: uniqueClients.size,
      recommendationAcceptanceRate: acceptanceMetrics.length > 0 
        ? acceptanceMetrics.reduce((sum, m) => sum + m.value, 0) / acceptanceMetrics.length 
        : 0,
      avgAllocationToGrowthAssets: allocationMetrics.length > 0
        ? allocationMetrics.reduce((sum, m) => sum + m.value, 0) / allocationMetrics.length
        : 0,
      avgTimeToDecision: timeMetrics.length > 0
        ? timeMetrics.reduce((sum, m) => sum + m.value, 0) / timeMetrics.length
        : 0,
      aiExplanationEngagement: engagementMetrics.length > 0
        ? engagementMetrics.reduce((sum, m) => sum + m.value, 0) / engagementMetrics.length
        : 0,
    };
  }

  private calculateSignificance(rawMetrics: { groupA: any[]; groupB: any[] }): number {
    const minSampleSize = 30;
    if (rawMetrics.groupA.length < minSampleSize || rawMetrics.groupB.length < minSampleSize) {
      return 0;
    }

    const aAcceptance = rawMetrics.groupA.filter(m => m.metricType === 'acceptance');
    const bAcceptance = rawMetrics.groupB.filter(m => m.metricType === 'acceptance');

    if (aAcceptance.length < 10 || bAcceptance.length < 10) {
      return 0;
    }

    const pA = aAcceptance.reduce((sum, m) => sum + m.value, 0) / aAcceptance.length;
    const pB = bAcceptance.reduce((sum, m) => sum + m.value, 0) / bAcceptance.length;
    const pooledP = (pA * aAcceptance.length + pB * bAcceptance.length) / (aAcceptance.length + bAcceptance.length);
    
    const se = Math.sqrt(pooledP * (1 - pooledP) * (1/aAcceptance.length + 1/bAcceptance.length));
  ...
  checkSafetyThresholds(metrics: {
    ...
    if (metrics.drawdown !== undefined && metrics.drawdown > this.safetyThresholds.maxDrawdown) {
      violations.push(`Drawdown ${metrics.drawdown}% exceeds threshold ${this.safetyThresholds.maxDrawdown}%`);
    }

    if (metrics.complaintRate !== undefined && metrics.complaintRate > this.safetyThresholds.maxComplaintRate) {
      violations.push(`Complaint rate ${metrics.complaintRate}% exceeds threshold ${this.safetyThresholds.maxComplaintRate}%`);
    }

    if (metrics.restrictedAssetExposure !== undefined && metrics.restrictedAssetExposure > this.safetyThresholds.maxRestrictedAssetExposure) {
      violations.push(`Restricted asset exposure ${metrics.restrictedAssetExposure}% exceeds threshold ${this.safetyThresholds.maxRestrictedAssetExposure}%`);
    }
  ...
  createExperiment(config: Omit<ExperimentConfig, 'experimentId'>): ExperimentConfig {
