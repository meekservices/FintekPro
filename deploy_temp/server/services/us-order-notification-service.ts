...
class UsOrderNotificationService {
  ...
  private initializeEmailService() {
    ...
    if (emailHost && emailUser && emailPass) {
      ...
      console.log('⚠️ US Order Notification email not configured - missing EMAIL_HOST, EMAIL_USER, or EMAIL_PASS');
    }
  }
  ...
  private async createInAppNotification(
    ...
    try {
      await db.insert(userNotifications).values({
        ...
        title,
        message,
        actionUrl: entityType === 'order' ? '/us-trading?tab=orders' : undefined,
        priority: type.includes('rejected') ? 'high' : 'medium',
      });
      console.log(`📱 Created in-app notification for user ${userId}: ${title}`);
    } catch (error) {
  ...
  async notifyOrderFilled(context: UsOrderNotificationContext): Promise<void> {
    ...
    if (prefs.pushEnabled) {
      await this.createInAppNotification(
        ...
        'order',
        context.orderId,
        { symbol: context.symbol, side: context.side, quantity: context.quantity, price: context.priceUsd }
      );
    }
    ...
    if (prefs.emailEnabled && user.email) {
      const html = this.generateOrderEmailHtml({
        ...
        details: [
          ...
          { label: 'Total Value', value: `$${(context.quantity * context.priceUsd).toFixed(2)}` },
  ...
  async notifyOrderCancelled(context: UsOrderNotificationContext): Promise<void> {
    const prefs = await this.getUserPreferences(context.userId);
  ...
  async notifyOrderRejected(context: UsOrderNotificationContext, reason?: string): Promise<void> {
    const prefs = await this.getUserPreferences(context.userId);
  ...
  async notifyRebalancingSuggestion(context: RebalanceNotificationContext): Promise<void> {
    const prefs = await this.getUserPreferences(context.userId);
  ...
  async getUnreadCount(userId: string): Promise<number> {
    try {
      const notifications = await db.select({ id: userNotifications.id })
