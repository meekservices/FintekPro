...
export class MutualFundStrategy extends BaseStrategy {
  ...
  async generate(context: StrategyContext): Promise<DailyPickData | null> {
    try {
      ...
      const investableFunds = nonEtfFunds.filter(fund => isFundInvestable({
        schemeName: fund.schemeName,
        category: fund.category || undefined,
        purchaseAllowed: true
      }).investable);
      
      const freshFunds = this.filterRecentPicks(investableFunds, context.recentIds, f => f.schemeCode);
      ...
      return {
        ...
        suitableFor: this.deriveSuitableFor(topFund.riskLevel || 'medium', 'mutual_funds'),
  ...
  async getLivePrice(instrumentId: string): Promise<number | null> {
    const row = await db.select({ nav: mutualFunds.nav })
      .from(mutualFunds).where(eq(mutualFunds.schemeCode, instrumentId)).limit(1);
