...
export class ComplianceValidator implements GovernanceValidator {
  async validate(input: GovernanceInput): Promise<ValidationResult> {
    ...
    const suggested_modifications: any = { add_disclaimers: [], enforce_language: [] };
    ...
    if (!recommendationStr.includes("not investment advice") && !recommendationStr.includes("risk disclosure")) {
      violations.push({
        module: "ComplianceValidator",
        severity: "MINOR",
      ...
      suggested_modifications.add_disclaimers.push("Not investment advice. Market volatility warning: Investments are subject to market risks.");
    }
