...
class AmfiNavScheduler {
  ...
  static getInstance(): AmfiNavScheduler {
    if (!this.instance) {
      this.instance = new AmfiNavScheduler();
    }
  ...
  initialize(): void {
    ...
    this.cronJob = cron.schedule('0 18 * * *', async () => {
      ...
      try {
        ...
        console.log(`[AMFI NAV Scheduler] Scheduled sync completed: ${result.updatedFunds} funds updated`);
      } catch (error: any) {
        console.error('[AMFI NAV Scheduler] Scheduled sync failed:', error.message);
      }
    }, {
      timezone: 'UTC'
    });
    ...
    console.log(`[AMFI NAV Scheduler] Initialized - daily sync at 11:30 PM IST`);
    console.log(`[AMFI NAV Scheduler] Next scheduled run: ${nextRun.toISOString()}`);
  }
