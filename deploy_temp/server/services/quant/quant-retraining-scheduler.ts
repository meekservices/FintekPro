...
class QuantRetrainingScheduler {
  ...
  private log(level: LogLevel, action: string, lockKey?: string, detail?: Record<string, any>): void {
  ...
  async runHealthCheck(): Promise<Record<string, any>> {
    ...
    try {
      ...
      if (modelsNeedingRetrain.length > 0) {
        this.log('INFO', 'Models needing retraining (scheduled interval will handle)', undefined, { models: modelsNeedingRetrain });
      } else {
      ...
      this.log('ERROR', 'Health check failed', lockKey, { error: error.message });
      return {};
    } finally {
    ...
    try {
      ...
      for (const s of states) {
        stateMap[s.lockKey] = {
          ...
          isInBackoff: s.backoffUntil ? new Date(s.backoffUntil) > new Date() : false,
        };
      }
  ...
  private async acquireDbLock(lockKey: string): Promise<boolean> {
    try {
      ...
      await db.delete(quantSchedulerLocks)
        .where(lt(quantSchedulerLocks.expiresAt, now));

      const existing = await db.select().from(quantSchedulerLocks)
        .where(eq(quantSchedulerLocks.lockKey, lockKey))
        .limit(1);

      if (existing.length > 0) {
        const lock = existing[0];
        const heartbeatAge = lock.heartbeatAt ? now.getTime() - new Date(lock.heartbeatAt).getTime() : Infinity;
        if (heartbeatAge > LOCK_TTL_MS) {
          ...
          await db.delete(quantSchedulerLocks)
            .where(sql`${quantSchedulerLocks.lockKey} = ${lockKey} AND ${quantSchedulerLocks.lockedBy} = ${lock.lockedBy}`);
        } else {
          this.log('DEBUG', 'DB lock already held', lockKey, { heldBy: lock.lockedBy });
          return false;
        }
      }

      await db.insert(quantSchedulerLocks).values({
        lockKey,
      ...
      const verify = await db.select().from(quantSchedulerLocks)
        .where(eq(quantSchedulerLocks.lockKey, lockKey))
        .limit(1);
      ...
      this.log('WARN', 'DB lock acquisition error, falling back to in-memory', lockKey, { error: e.message });
      return true;
    }
  }
  ...
  private async releaseDbLock(lockKey: string): Promise<void> {
    try {
      await db.delete(quantSchedulerLocks)
        .where(sql`${quantSchedulerLocks.lockKey} = ${lockKey} AND ${quantSchedulerLocks.lockedBy} = ${this.instanceId}`);
  ...
  private async refreshLockHeartbeat(lockKey: string): Promise<boolean> {
    try {
      ...
      const result = await db.update(quantSchedulerLocks)
        .set({ heartbeatAt: now, expiresAt: newExpiry })
        .where(sql`${quantSchedulerLocks.lockKey} = ${lockKey} AND ${quantSchedulerLocks.lockedBy} = ${this.instanceId}`);
      return true;
    } catch (e: any) {
      this.log('WARN', 'Heartbeat refresh failed', lockKey, { error: e.message });
      return false;
    }
  }
  ...
  private async getOrCreateState(lockKey: string): Promise<{
    ...
    try {
      const rows = await db.select().from(quantSchedulerState)
        .where(eq(quantSchedulerState.lockKey, lockKey))
        .limit(1);

      if (rows.length === 0) {
        await db.insert(quantSchedulerState).values({
          lockKey,
          dailyCount: 0,
        ...
        return { dailyCount: 0, dailyCountDate: today, consecutiveFailures: 0, lastAttemptAt: null, lastSuccessAt: null, backoffUntil: null };
      }

      const state = rows[0];
      if (state.dailyCountDate !== today) {
        await db.update(quantSchedulerState)
          .set({ dailyCount: 0, dailyCountDate: today, updatedAt: new Date() })
          .where(eq(quantSchedulerState.lockKey, lockKey));
        return { ...state, dailyCount: 0, dailyCountDate: today, lastAttemptAt: state.lastAttemptAt, lastSuccessAt: state.lastSuccessAt, backoffUntil: state.backoffUntil };
      }
      ...
      this.log('WARN', 'State fetch failed, using defaults', lockKey, { error: e.message });
      return { dailyCount: 0, dailyCountDate: today, consecutiveFailures: 0, lastAttemptAt: null, lastSuccessAt: null, backoffUntil: null };
    }
  }

  private async recordAttempt(lockKey: string, success: boolean): Promise<void> {
    const now = new Date();
    try {
      if (success) {
        await db.update(quantSchedulerState)
      ...
      this.log('WARN', 'Failed to record attempt', lockKey, { error: e.message });
    }
  }
  ...
  private async safeRun(label: string, fn: () => Promise<any>): Promise<any> {
    ...
    try {
      this.log('INFO', 'Starting retrain', label, { dailyCount: state.dailyCount + 1, cap: DAILY_RETRAIN_CAP });
      const startMs = Date.now();
      const result = await fn();
      const durationMs = Date.now() - startMs;

      await this.recordAttempt(label, true);
      this.lastRunResults[label] = { timestamp: new Date(), result, status: 'SUCCESS', durationMs };

      this.log('INFO', 'Retrain completed', label, { durationMs, status: 'SUCCESS' });
      return result;
    } catch (error: any) {
