...
type WebhookHandler = (event: any) => Promise<{ success: boolean; error?: string }>;
...
class KycWebhookService {
  ...
  async receiveEvent(params: WebhookEventParams): Promise<{
    ...
    try {
      const [event] = await db.insert(kycWebhookEvents).values({
      ...
      this.processEvent(event.id).catch(err => {
        console.error(`[WebhookService] Error processing event ${event.id}:`, err);
      });
  ...
  private async processEvent(eventId: string): Promise<void> {
    try {
      ...
      if (!handler) {
        await db.update(kycWebhookEvents)
          .set({
            status: 'FAILED',
      ...
      console.error(`[WebhookService] Error processing event ${eventId}:`, error);
    }
  }
