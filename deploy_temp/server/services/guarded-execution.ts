...
export async function guardedExecution<T>(
  ...
  try {
    ...
    if (ctx.validator !== undefined && result !== null && result !== undefined) {
      const validated = ctx.validator(result);
      await recordFeedback({ ...ctx, durationMs: Date.now() - start, success: true });
      return validated;
    }
    ...
    console.error(`[GuardedExecution] ${ctx.module}.${ctx.operation} failed (${riskLevel} risk): ${errorMsg}`);
...
export function validateStockPrice(price: number, symbol: string): number {
  ...
  if (price > 10_000_000) {
    throw new Error(`Suspiciously high price for ${symbol}: ${price} — possible schema mismatch`);
  }
  return price;
}
 ...
 */
export function validateChangePercent(changePct: number | null | undefined, symbol: string): number | undefined {
  ...
  if (Math.abs(changePct) > 75) {
    throw new Error(`Implausible change% for ${symbol}: ${changePct}% — possible bad data`);
  }
  return changePct;
}
 ...
 */
export function validateQuoteSchema(quote: Record<string, unknown>, requiredFields: string[], context: string): void {
  for (const field of requiredFields) {
    if (!(field in quote) || quote[field] === null || quote[field] === undefined) {
      throw new Error(`Missing required field "${field}" in ${context} response — schema may have changed`);
    }
  }
}
...
export function validateProspectData<T extends Record<string, unknown>>(
  ...
  for (const field of requiredFields) {
    if (!(field in data) || data[field] === null || data[field] === undefined) {
      throw new Error(`Prospect data missing required field "${field}" in ${context} — schema drift?`);
    }
  }
  return data;
}
 ...
 */
export function validateICAIResult(result: Record<string, unknown>): Record<string, unknown> {
  if (!result || typeof result !== 'object') {
    throw new Error('ICAI result is not an object — scraper returned unexpected type');
  }
 ...
 */
export function validateCredhiveProfile(data: Record<string, unknown>, cin: string): Record<string, unknown> {
  ...
  if (!data['company_name'] && !data['companyName'] && !data['name']) {
    throw new Error(`Credhive profile for ${cin} missing company name — API response changed?`);
  }
  return data;
}
 ...
 */
export function routeFixStrategy(module: GuardedModule): 'AUTO_FIX' | 'SCRAPER_HEALING' | 'PARTIAL_AUTO' | 'MANUAL_REVIEW' {
  switch (module) {
...
export async function getFeedbackStats(hours = 24): Promise<{
  ...
  try {
    ...
    const totalFailures = parseInt(summary?.total_failures ?? '0', 10);
    ...
    return {
      byModule: rows.map((r) => ({
        ...
        failureRate:  totalCalls > 0
          ? `${((parseInt(r.failures, 10) / parseInt(r.total_calls, 10)) * 100).toFixed(1)}%`
          : '0%',
        fallbacksUsed: parseInt(r.fallback_used, 10),
      ...
      summary: {
        totalCalls,
        totalFailures,
        overallFailureRate: totalCalls > 0
    ...
    return {
      byModule: [],
      recentFailures: [],
      summary: { totalCalls: 0, totalFailures: 0, overallFailureRate: '0%', hoursWindow: hours },
    };
  }
}
