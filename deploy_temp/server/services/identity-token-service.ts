...
class IdentityTokenService {
  async getOrCreateProfile(userId: string): Promise<IdentityProfile> {
    try {
      ...
      console.log(`${LOG_PREFIX} Created new identity profile for user ${userId} with token ${identityTokenId}`);
      return newProfile;
    } catch (error: any) {
      console.error(`${LOG_PREFIX} Error in getOrCreateProfile for user ${userId}:`, error?.message || error);
      throw error;
    }
  }
  ...
  async updateVerificationStatus(
    ...
    data: { verified: boolean; provider: string; details?: Record<string, any> }
    ...
    try {
      ...
      switch (verificationType) {
        case "pan":
          updateFields = {
            ...
            ...(data.details?.panNumber && { panNumber: data.details.panNumber }),
          };
          break;
        case "aadhaar":
          updateFields = {
            ...
            ...(data.details?.aadhaarLastFour && { aadhaarLastFour: data.details.aadhaarLastFour }),
          };
          break;
        case "ckyc":
          updateFields = {
            ...
            ...(data.details?.ckycNumber && { ckycNumber: data.details.ckycNumber }),
          };
          break;
        case "bank":
          updateFields = {
      ...
      console.log(`${LOG_PREFIX} Updated ${verificationType} verification for user ${userId} - KYC level: ${level}, status: ${status}`);
      return finalProfile;
    } catch (error: any) {
      console.error(`${LOG_PREFIX} Error updating verification status for user ${userId}:`, error?.message || error);
      throw error;
    }
  }

  calculateKycLevel(profile: Partial<IdentityProfile>): { level: string; status: string } {
  ...
  async declareFatca(userId: string): Promise<void> {
    try {
      ...
      console.error(`${LOG_PREFIX} Error declaring FATCA for user ${userId}:`, error?.message || error);
      throw error;
    }
  }

  async assessRisk(userId: string, riskData: { category: string; score: number }): Promise<void> {
    try {
      await db
        .update(identityProfiles)
        .set({
      ...
      console.log(`${LOG_PREFIX} Risk assessed for user ${userId} - category: ${riskData.category}, score: ${riskData.score}`);
    } catch (error: any) {
      console.error(`${LOG_PREFIX} Error assessing risk for user ${userId}:`, error?.message || error);
      throw error;
    }
  }
  ...
  async getProfileByToken(tokenId: string): Promise<IdentityProfile | null> {
    try {
      ...
      console.error(`${LOG_PREFIX} Error fetching profile by token ${tokenId}:`, error?.message || error);
      throw error;
    }
  }
  ...
  async checkKycEligibility(
    ...
    try {
      ...
      if (productConfigs.length === 0) {
        console.log(`${LOG_PREFIX} No product configuration found for ${productType}, defaulting to BASIC`);
        return {
        ...
        for (const step of requiredSteps) {
          switch (step) {
            case "pan":
              ...
              break;
            case "aadhaar":
              if (!profile.aadhaarVerified) missingSteps.push("aadhaar_verification");
              break;
            case "ckyc":
            ...
            case "bank":
              ...
              break;
            case "address":
              if (!profile.addressVerified) missingSteps.push("address_verification");
              break;
            case "fatca":
              if (!profile.fatcaDeclared) missingSteps.push("fatca_declaration");
              break;
          }
        }
      } else {
      ...
      const eligible = currentLevelIndex >= requiredLevelIndex && missingSteps.length === 0;

      console.log(`${LOG_PREFIX} KYC eligibility check for user ${userId}, product ${productType}: eligible=${eligible}, missing=${missingSteps.join(",")}`);
      ...
      console.error(`${LOG_PREFIX} Error checking KYC eligibility for user ${userId}:`, error?.message || error);
      throw error;
    }
  }
}
