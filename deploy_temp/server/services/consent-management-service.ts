...
import { db } from '../db';
import { dataSourceConsents, type InsertDataSourceConsent, type DataSourceConsent } from '@shared/schema';
...
export class ConsentManagementService {
  ...
  async grantConsent(request: ConsentRequest): Promise<DataSourceConsent> {
    ...
    const consentText = this.getConsentText(request.dataSource, request.provider);
   ...
   */
  async checkConsent(userId: string, dataSource: DataSourceType): Promise<ConsentStatus> {
    const consent = await db
      .select()
    ...
    const now = new Date();
    const isExpired = consentRecord.expiresAt && new Date(consentRecord.expiresAt) < now;

    // Auto-revoke expired consents
    if (isExpired && consentRecord.isActive) {
      await this.revokeConsent(consentRecord.id, 'Consent expired automatically');
    }
    ...
    return {
      ...
      lastSyncedAt: consentRecord.lastSyncedAt ? new Date(consentRecord.lastSyncedAt) : null,
      isExpired
    };
  }
   ...
   */
  private getConsentText(dataSource: DataSourceType, provider?: string): string {
  ...
  async grantAllConsents(
    ...
    console.log(`✅ All consents granted for user ${userId} (${consents.length} sources)`);
    return consents;
  }
}
