...
const CATEGORY_KEYWORD_MAP: Record<string, string[]> = {
  equity: ['equity', 'elss', 'nifty', 'sensex', 'bse', 'nse', 'growth', 'bluechip', 'multicap', 'midcap', 'smallcap', 'largecap', 'flexi', 'focussed', 'focused', 'contra', 'value', 'dividend', 'thematic', 'sectoral', 'esg', 'momentum', 'alpha'],
  debt: ['debt', 'bond', 'duration', 'credit', 'gilt', 'liquid', 'overnight', 'money market', 'floater', 'banking', 'psu', 'income', 'fixed income', 'treasury', 'corporate'],
  hybrid: ['hybrid', 'balanced', 'arbitrage', 'equity savings', 'multi asset', 'dynamic asset', 'blended', 'asset allocation'],
...
const THEMATIC_SUBCATEGORIES = ['sectoral/thematic', 'sectoral', 'thematic', 'eq_sectoral_thematic'];
...
class MfNamingComplianceService {
  ...
  static getInstance(): MfNamingComplianceService {
    if (!this.instance) {
      this.instance = new MfNamingComplianceService();
    }
  ...
  validateSchemeName(
    ...
    // Check if exempt
    const isExempt = KEYWORD_EXEMPT_CATEGORIES.some(exemptCat => catLower.includes(exemptCat));

    // Check if thematic/sectoral (exempt from equity keyword)
    const isThematic = THEMATIC_SUBCATEGORIES.some(sub => catLower.includes(sub));

    if (!isExempt && !isThematic) {
      const matchedGroup = Object.entries(CATEGORY_KEYWORD_MAP).find(([key]) => catLower.includes(key));
      if (matchedGroup) {
  ...
  async validateAndPersist(
    ...
    const result = this.validateSchemeName(schemeCode, schemeName, category, lifecycleMetadata);
    ...
    try {
      ...
      console.error(`[NamingCompliance] DB update error for ${schemeCode}:`, e.message);
    }

    return result;
  }

  async runNamingValidationForAll(batchSize = 500): Promise<BulkValidationSummary> {
    console.log('[NamingCompliance] Starting bulk naming validation...');

    let offset = 0;
    let total = 0, passed = 0, failed = 0;
    const failures: Array<{ schemeCode: string; schemeName: string; reason: string }> = [];
      ...
      for (const fund of batch) {
        ...
        if (result.status === 'PASSED') {
          ...
          if (failures.length < 200) {
            failures.push({ schemeCode: fund.schemeCode, schemeName: fund.schemeName || '', reason: result.reason || '' });
          }
        }
      }

      offset += batchSize;
      console.log(`[NamingCompliance] Progress: ${total} processed (${passed} passed, ${failed} failed)`);
    }

    console.log(`[NamingCompliance] Complete: ${total} total, ${passed} passed, ${failed} failed`);
    return { total, passed, failed, failures };
  }
}

export const mfNamingComplianceService = MfNamingComplianceService.getInstance();
