...
const API_COST_PER_CALL = {
  PAN: 0.50,      // INR per PAN verification
  AADHAAR: 1.00,  // INR per Aadhaar verification
...
class OnboardingCacheService {
  ...
  private cleanupCache(cache: Map<string, { result: any; timestamp: number }>, ttl: number, now: number): void {
  ...
  async batchCheckVerificationStatus(
    userIds: number[]
  ): Promise<Map<number, { panVerified: boolean; kraVerified: boolean; bankVerified: boolean }>> {
    const results = new Map<number, { panVerified: boolean; kraVerified: boolean; bankVerified: boolean }>();
  ...
  prewarmForUser(pan: string, existingPanData?: any): void {
    if (existingPanData) {
      const key = this.hashKey(pan);
      this.cache.panVerified.set(key, { result: existingPanData, timestamp: Date.now() });
  ...
  invalidateForUser(pan: string, accountNumber?: string, ifsc?: string): void {
    ...
    console.log(`[OnboardingCache] Invalidated cache for ${pan.slice(0, 4)}****`);
  }
   ...
   */
  getMetrics() {
    const totalHits = this.metrics.panCacheHits + this.metrics.aadhaarCacheHits +
                      this.metrics.bankCacheHits + this.metrics.kraCacheHits;
    const totalMisses = this.metrics.panCacheMisses + this.metrics.aadhaarCacheMisses +
                        this.metrics.bankCacheMisses + this.metrics.kraCacheMisses;
