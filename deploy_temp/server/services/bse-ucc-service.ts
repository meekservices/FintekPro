...
class BSEUCCService {
  ...
  constructor() {
    ...
    if (!this.hasCredentials) {
      if (IS_PRODUCTION) {
        throw new Error('BSE API credentials (BSE_USER_ID, BSE_PASSWORD, BSE_MEMBER_ID, BSE_PASS_KEY) must be configured in production');
      } else {
  ...
  async createUCC(request: UCCCreationRequest): Promise<UCCCreationResponse> {
    try {
      ...
      throw new Error(`BSE UCC creation failed: ${error.message || 'Unknown error'}`);
    }
  }
  ...
  private prepareBSERequest(request: UCCCreationRequest): any {
    ...
    return {
      ...
      NomineeDOB: request.nomineeDob ? this.formatDateForBSE(request.nomineeDob) : '',
    };
  }
  ...
  private mockUCCCreation(request: UCCCreationRequest): UCCCreationResponse {
    ...
    const panHash = request.panNumber.split('').reduce((sum, char) => sum + char.charCodeAt(0), 0);
