...
class CkycEnvironmentService {
  ...
  private detectEnvironmentMode(): CkycEnvironmentMode {
    const explicitMode = process.env.CKYC_ENV_MODE?.toUpperCase() as CkycEnvironmentMode | undefined;
  ...
  assertMockAllowed(context?: { userId?: string; panNumber?: string }): void {
    if (this.mode === 'PROD') {
      ...
      console.error(`[CKYC SECURITY] 🚫 Mock provider blocked in PROD:`, attempt);
  ...
  blockMockProviderInvocation(provider: string, context?: { userId?: string; panNumber?: string }): boolean {
    if (this.mode === 'PROD' && provider === 'mock') {
      ...
      this.blockedAttempts.push(attempt);
      console.error(`[CKYC SECURITY] 🚫 Blocked mock provider in PROD:`, attempt);
  ...
  runStartupComplianceCheck(): EnvironmentComplianceResult {
    ...
    if (this.mode === 'PROD') {
      console.log('[CKYC Environment] ✅ Running production compliance assertion...');
      
      const hasTruthScreenCreds = !!(process.env.TRUTHSCREEN_USERNAME && process.env.TRUTHSCREEN_PASSWORD);
      const hasCkycApiCreds = !!(process.env.CKYC_API_KEY && process.env.CKYC_API_SECRET);
      
      if (!hasTruthScreenCreds && !hasCkycApiCreds) {
        result.warnings.push('No CKYC API credentials configured. Manual CKYC will be the only option.');
        console.warn('[CKYC Environment] ⚠️ No CKYC API credentials - Manual CKYC only');
      }
      
      if (process.env.CKYC_ENV_MODE?.toUpperCase() === 'DEV' && process.env.NODE_ENV === 'production') {
        result.violations.push('CKYC_ENV_MODE set to DEV but NODE_ENV is production - potential misconfiguration');
        result.isCompliant = false;
      }
    }

    console.log(`[CKYC Environment] Compliance check: ${result.isCompliant ? '✅ PASSED' : '❌ FAILED'}`);
    if (result.warnings.length > 0) {
      console.log(`[CKYC Environment] Warnings: ${result.warnings.join(', ')}`);
    }
    if (result.violations.length > 0) {
      console.error(`[CKYC Environment] Violations: ${result.violations.join(', ')}`);
    }
  ...
  getSlaAgingBucket(slaStartedAt: Date): '<24h' | '24-72h' | '>72h' {
    const hoursElapsed = (Date.now() - slaStartedAt.getTime()) / (1000 * 60 * 60);
