...
export class SFTPBankConnector extends BaseBankConnector {
  ...
  async submitApplication(payload: BankSubmissionPayload): Promise<BankSubmissionResponse> {
    ...
    try {
      ...
      console.log(`[${this.config.bankCode}] Submitting loan application via SFTP`, {
      ...
      console.error(`[${this.config.bankCode}] SFTP submission failed:`, error.message);
      return {
        success: false,
        error: error.message,
      };
  ...
  async checkStatus(bankReference: string): Promise<BankStatusResponse> {
    console.log(`[${this.config.bankCode}] Checking SFTP response files for reference:`, bankReference);
  ...
  private buildCSVContent(application: any): string {
    const headers = [
      'ReferenceNumber', 'ApplicantName', 'Mobile', 'Email', 'PAN',
      'EmploymentType', 'MonthlyIncome', 'LoanType', 'LoanAmount', 'Tenure',
    ];
