...
export class ICICITransformer implements PayloadTransformer {
  ...
  transform(canonical: CanonicalLoanPayload): BankTransformedPayload {
    ...
    const employmentMapping = mapEmploymentType(canonical.applicant.employment_type);
...
export class PayloadTransformerFactory {
  ...
  transform(bankCode: string, canonical: CanonicalLoanPayload): BankTransformedPayload | null {
    const transformer = this.getTransformer(bankCode);
    if (!transformer) {
      console.error(`[PayloadTransformer] No transformer found for bank: ${bankCode}`);
      return null;
    }
