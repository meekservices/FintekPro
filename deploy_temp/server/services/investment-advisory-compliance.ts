...
export interface ConsentRecord {
  consentId: string;
  clientId: string;
  consentType: 'risk_disclosure' | 'investment_advice' | 'data_processing' | 'commission_disclosure' | 'suitability_waiver';
  consentText: string;
  grantedAt: Date;
  expiresAt?: Date;
  ipAddress: string;
...
export class InvestmentAdvisoryComplianceEngine {
  ...
  logRecommendation(params: {
    ...
    const auditLog: RecommendationAuditLog = {
      ...
      retentionExpiry: new Date(now.getTime() + retentionYears * 365 * 24 * 60 * 60 * 1000)
    };
  ...
  private generateMandatoryDisclosures(): RecommendationAuditLog['disclosures'] {
    return {
      conflictOfInterest: 'FintekPro may receive commissions from product providers. This potential conflict has been disclosed as per SEBI (Investment Advisers) Regulations, 2013.',
      commissionDisclosure: 'Commission structure: Trail commission of 0.5-1% p.a. on assets under advice. Upfront commissions disclosed in product-specific documentation.',
      riskDisclosure: 'Investment in securities market is subject to market risks. Past performance is not indicative of future returns. Read all scheme related documents carefully before investing.',
      pastPerformanceDisclaimer: 'Past performance is not necessarily indicative of future results and no representation is being made that any investment will or is likely to achieve profits or losses similar to those achieved in the past.'
    };
  }
  ...
  recordConsent(params: {
    ...
    const regulatoryRefs: Record<ConsentRecord['consentType'], string> = {
      ...
      investment_advice: 'SEBI (Investment Advisers) Regulations, 2013 - Regulation 17',
      ...
      suitability_waiver: 'SEBI (Investment Advisers) Regulations, 2013 - Regulation 17(2)'
    };
  ...
  createSuitabilityDeclaration(params: {
    ...
    const allMatch = params.riskProfileMatch && params.horizonMatch && 
                     params.financialCapacityMatch && params.objectivesMatch && 
                     params.kycComplete;
  ...
  generateComplianceReport(params: {
    ...
    const periodLogs = Array.from(auditLogs.values()).filter(
      log => log.timestamp >= params.periodStart && log.timestamp <= params.periodEnd
    );
    
    const totalRecommendations = periodLogs.length;
    const suitableRecommendations = periodLogs.filter(l => l.recommendation.suitabilityScore >= 70).length;
    const unsuitableOverrides = periodLogs.filter(
      l => l.recommendation.suitabilityScore < 50 && l.recommendation.action === 'buy'
    ).length;
    const acknowledgedLogs = periodLogs.filter(l => l.clientAcknowledgment?.acknowledged).length;
    const avgSuitability = totalRecommendations > 0
      ? periodLogs.reduce((sum, l) => sum + l.recommendation.suitabilityScore, 0) / totalRecommendations
      : 0;
    const compliantLogs = periodLogs.filter(l => l.regulatoryCompliance.sebiRegulation17Compliant).length;
    
    const violations: ComplianceReport['violations'] = [];
    
    periodLogs.forEach(log => {
      if (log.recommendation.suitabilityScore < 50 && log.recommendation.action === 'buy') {
        violations.push({
      ...
      if (!log.clientAcknowledgment?.acknowledged && log.recommendation.action === 'buy') {
        violations.push({
          violationType: 'MISSING_ACKNOWLEDGMENT',
          description: 'Client acknowledgment not obtained for investment recommendation',
          severity: 'medium',
          clientId: log.clientId,
          recommendationId: log.auditId,
          dateOccurred: log.timestamp,
    ...
    const report: ComplianceReport = {
      ...
      summary: {
        ...
        unsuitableOverrides,
        clientAcknowledgmentRate: totalRecommendations > 0 ? (acknowledgedLogs / totalRecommendations) * 100 : 100,
        averageSuitabilityScore: Math.round(avgSuitability * 100) / 100,
        complianceRate: totalRecommendations > 0 ? (compliantLogs / totalRecommendations) * 100 : 100
      },
      violations,
      regulatoryStatus: {
        sebiCompliant: violations.filter(v => v.severity === 'high' || v.severity === 'critical').length === 0,
        pendingItems: violations.filter(v => !v.resolved).map(v => v.description),
        upcomingDeadlines: [
          {
  ...
  getClientConsents(clientId: string): ConsentRecord[] {
    return Array.from(consentRecords.values())
      .filter(record => record.clientId === clientId && !record.revoked);
  }
    ...
    return {
      ...
      sections: [
        ...
        },
        {
          ...
          content: 'FintekPro is a SEBI Registered Investment Adviser (Registration No: XXXXXXXXX). The advice provided is based on your stated investment objectives, risk tolerance, and financial situation as assessed through our KYC process.',
          regulatoryReference: 'SEBI (Investment Advisers) Regulations, 2013'
        },
        {
          title: 'Conflict of Interest Statement',
          content: 'FintekPro may receive trail commissions from product manufacturers. We endeavor to recommend products that are suitable for your profile regardless of commission structure.',
        ...
        },
        {
          ...
          content: 'For any grievances, please contact our compliance officer at compliance@fintekpro.com. You may also file complaints at SEBI SCORES portal (https://scores.gov.in).',
  ...
  getComplianceStats(): {
    ...
    const avgSuitability = totalLogs > 0
      ? logs.reduce((sum, l) => sum + l.recommendation.suitabilityScore, 0) / totalLogs
      : 0;
    const acknowledged = logs.filter(l => l.clientAcknowledgment?.acknowledged).length;
    const compliant = logs.filter(l => l.regulatoryCompliance.sebiRegulation17Compliant).length;
    ...
    return {
      ...
      complianceRate: totalLogs > 0 ? Math.round((compliant / totalLogs) * 100) : 100
    };
  }
}

export const investmentAdvisoryCompliance = new InvestmentAdvisoryComplianceEngine();
