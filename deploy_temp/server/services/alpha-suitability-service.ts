...
export class AlphaSuitabilityService {
  ...
  static getInstance(): AlphaSuitabilityService {
    if (!this.instance) {
      this.instance = new AlphaSuitabilityService();
    }
   ...
   */
  async getRecommendedAlphaPicks(userId: string, limit: number = 5): Promise<any[]> {
    try {
      // 1. Get user risk category
      const [profile] = await db.select({ riskCategory: userProfiles.riskCategory })
        .from(userProfiles)
      ...
      logger.error('[AlphaSuitability] Failed to fetch recommendations', { userId, error: err.message });
      return [];
    }
  }
  ...
  async generateInformedConsent(userId: string, pickId: number): Promise<any> {
    ...
    const [profile] = await db.select({ riskCategory: userProfiles.riskCategory }).from(userProfiles).where(eq(userProfiles.userId, userId)).limit(1);
    ...
    return {
      ...
      disclosures: [
        ...
        },
        {
          ...
          content: "AI-driven Alpha picks use predictive models that can be volatile. Historical backtesting ('Alpha') does not guarantee future results.",
        },
        {
          title: "Regulatory Acknowledgment",
          content: "By proceeding, you acknowledge that you have read the suitability warning and are making this investment based on your independent judgment, overriding the default suitability filter.",
        }
      ],
      auditTrail: {
   ...
   */
  async logInformedConsent(userId: string, pickId: number, consentData: any): Promise<void> {
    ...
    logger.info('[Audit] User completed informed consent for suitability override', { userId, pickId });
  }
}
