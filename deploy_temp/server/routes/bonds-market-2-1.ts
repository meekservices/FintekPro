...
export function registerBondsMarkPart2Part1Routes(app: Express): void {
  app.post("/api/products/:id/refresh", async (req: Request<{ id: string }>, res: Response): Promise<void> => {
    try {
      const { id } = req.params;
  ...
  app.get("/api/loans/rates", async (_req: Request, res: Response): Promise<void> => {
    try {
  ...
  app.post("/api/clients", async (req: Request<any, any, { name?: string; panNumber?: string; email?: string; mobile?: string }>, res: Response): Promise<void> => {
    try {
  ...
  app.post("/api/portfolios/:portfolioId/holdings", async (req: Request<{ portfolioId: string }, any, { holdings: PortfolioHoldingInput[] }>, res: Response): Promise<void> => {
    try {
      const { portfolioId } = req.params;
      const { holdings } = req.body;
      
      if (!Array.isArray(holdings)) {
        res.status(400).json({ status: "error", error: "Holdings must be an array" });
        return;
      }
  ...
  app.post("/api/partners/load-amcs", async (_req: Request, res: Response): Promise<void> => {
    try {
  ...
  // Get all partners
  app.get("/api/partners", async (_req: Request, res: Response): Promise<void> => {
    try {
  ...
  app.get("/api/rates/current", async (_req: Request, res: Response): Promise<void> => {
    try {
