...
router.get('/portfolio/:clientId', async (req: Request, res: Response) => {
  try {
    const { clientId } = req.params;
    const portfolio = await unifiedAdvisoryService.getPortfolioSummary(clientId);
...
router.post('/recommendations/:clientId', async (req: Request, res: Response) => {
  try {
    const { clientId } = req.params;
    ...
    const validProductTypes = productTypes.filter(
      (p): p is ProductType => [
        'STOCK', 'MF', 'BOND', 'UNLISTED', 'MLD', 'PMS', 'AIF', 'CFD', 'TREASURY'
      ].includes(p)
    );
    ...
    const recommendations = await unifiedAdvisoryService.generateRecommendations(
      clientId,
      validProductTypes,
    ...
    console.error('[Unified Advisory] Recommendation generation failed:', error);
    res.status(500).json({
      success: false,
...
router.get('/disclosures/:productType', async (req: Request, res: Response) => {
  try {
    ...
    const disclosures = unifiedAdvisoryService.getDisclosuresForProduct(productType as ProductType);
    const executionChannel = unifiedAdvisoryService.getExecutionChannel(productType as ProductType);
...
router.post('/create-proposal', async (req: Request, res: Response) => {
  try {
    ...
    const result = await unifiedAdvisoryService.createProposalFromRecommendations(
      clientId,
      agentId,
      decisions
    );
...
router.get('/audit-trail/:clientId', async (req: Request, res: Response) => {
  try {
    ...
    const auditTrail = await unifiedAdvisoryService.getAuditTrail(clientId, limit);
...
router.get('/execution-channel/:productType', async (req: Request, res: Response) => {
  try {
    const { productType } = req.params;
    ...
    const channel = unifiedAdvisoryService.getExecutionChannel(productType as ProductType);
