...
router.post('/checkout', requireAuth, async (req: any, res) => {
  try {
    ...
    if (tier === 'elite' && cycle === 'monthly') {
      return res.status(400).json({ error: 'Elite is only available on annual billing' });
    }
    ...
    const order = await createCheckoutOrder(userId, tier, cycle, {
      ...
      name: [user.firstName, user.lastName].filter(Boolean).join(' ') || 'Investor',
    });
...
router.post('/verify', requireAuth, async (req: any, res) => {
  try {
    ...
    if (status.orderStatus === 'EXPIRED' || status.orderStatus === 'CANCELLED') {
      ...
      return res.json({ success: false, status: status.orderStatus.toLowerCase() });
    }
  ...
  try {
    ...
    if (event?.type === 'PAYMENT_SUCCESS_WEBHOOK') {
      ...
      if (pending) {
        ...
        logger.info(`[Subscription] Webhook activated subscription for order ${orderId}`);
      }
    }
