...
router.post("/analyze", async (req: Request, res: Response) => {
  try {
    ...
    const totalWeight = portfolioFunds.reduce((sum, f) => sum + f.portfolioWeight, 0);
    if (totalWeight < 1) {
      const equalWeight = 100 / portfolioFunds.length;
      portfolioFunds.forEach(f => f.portfolioWeight = equalWeight);
    }

    const result = await stockIntersectionAnalysisService.analyzePortfolio(portfolioFunds);
