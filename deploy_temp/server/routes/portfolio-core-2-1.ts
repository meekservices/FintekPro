...
export function registerPortfolioCorPart2Part1Routes(app: Express): void {
  app.get("/api/loans/applications/:id", requireAuth, async (req: Request, res: Response): Promise<void> => {
    try {
  ...
  app.post("/api/loans/applications", requireAuth, async (req: Request, res: Response): Promise<void> => {
    try {
      const userId = (req as AuthRequest).user!.id;
  ...
  app.post("/api/government-schemes/:schemeType/refresh", requireAuth, async (req: Request, res: Response): Promise<void> => {
    try {
      const userId = (req as AuthRequest).user!.id;
  ...
  app.post("/api/government-schemes/:schemeType/otp/verify", requireAuth, async (req: Request, res: Response): Promise<void> => {
    try {
      const userId = (req as AuthRequest).user!.id;
      const { schemeType } = req.params;
  ...
  app.get("/api/insurance-holdings", requireAuth, async (req: Request, res: Response): Promise<void> => {
    if (process.env.INSURANCE_SERVICE_URL) return proxyToInsurance(req, res, '/insurance-holdings');
    try {
      const insuranceHoldings = await storage.getInsuranceHoldings((req as AuthRequest).user!.id);
  ...
  app.post("/api/insurance-holdings", requireAuth, async (req: Request, res: Response): Promise<void> => {
    if (process.env.INSURANCE_SERVICE_URL) return proxyToInsurance(req, res, '/insurance-holdings');
    try {
      const holdingData = { ...req.body, userId: (req as AuthRequest).user!.id };
  ...
  app.patch("/api/insurance-holdings/:id", requireAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  app.get("/api/portfolios/:userId", async (req: Request<{ userId: string }>, res: Response): Promise<void> => {
    try {
