...
export class ZohoCommerceAPI {
  ...
  private async makeRequest(endpoint: string, options: any = {}): Promise<any> {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Zoho Commerce API error: ${response.status} - ${errorText}`);
    }
   ...
   */
  async updateProduct(productId: string, updates: Partial<ZohoProduct>): Promise<any> {
    return this.makeRequest(`/products/${productId}`, {
      method: 'PUT',
   ...
   */
  async updateCategory(categoryId: string, updates: Partial<ZohoCategory>): Promise<any> {
    return this.makeRequest(`/categories/${categoryId}`, {
      method: 'PUT',
      body: JSON.stringify(updates)
    });
   ...
   */
  async updateInventory(productId: string, variantId: string, quantity: number): Promise<any> {
    return this.makeRequest(`/products/${productId}/variants/${variantId}/inventory`, {
      method: 'PUT',
   ...
   */
  async bulkUpdateInventory(updates: Array<{productId: string, variantId: string, quantity: number}>): Promise<any> {
   ...
   * Process refund
   */
  async processRefund(orderId: string, refundAmount: number, reason?: string): Promise<any> {
  ...
  async getProductAnalytics(productId?: string): Promise<any> {
    const endpoint = productId ? `/analytics/products/${productId}` : '/analytics/products';
   ...
   */
  getTokens(): { accessToken?: string; refreshToken?: string; tokenExpiry?: number } {
    return {
...
export { ZohoCommerceConfig, ZohoProduct, ZohoOrder, ZohoCategory };