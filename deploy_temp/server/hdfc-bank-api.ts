...
export class HDFCBankAPI {
  ...
  constructor(config: HDFCBankConfig) {
    ...
    this.client = axios.create({
      baseURL: config.baseUrl || baseUrls[config.environment] || baseUrls.sandbox,
      timeout: 30000,
      headers: {
  ...
  private async authenticate(): Promise<void> {
    ...
    try {
      ...
      console.error('HDFC Bank OAuth authentication error:', error.response?.data || error.message);
      throw error;
    }
  }

  async getAccountBalance(accountNumber: string): Promise<HDFCBankResponse<HDFCAccountBalance>> {
    try {
      const response = await this.client.get(`/api/v1/accounts/${accountNumber}/balance`);
      ...
      return {
        ...
        data: {
          ...
          ledgerBalance: parseFloat(balanceData.ledgerBalance || '0'),
      ...
      console.error('HDFC Bank account balance error:', error.response?.data || error.message);
      return {
        success: false,
        error: error.response?.data?.message || 'Failed to fetch account balance',
  ...
  async getTransactionHistory(
    ...
    try {
      ...
      const response = await this.client.get(`/api/v1/accounts/${accountNumber}/transactions`, { params });
      
      const transactions = response.data.transactions || [];
      
      const formattedTransactions: HDFCTransaction[] = transactions.map((txn: any) => ({
        transactionId: txn.transactionId || txn.id,
        accountNumber: accountNumber,
        amount: parseFloat(txn.amount || '0'),
        transactionType: txn.type === 'C' || txn.type === 'CREDIT' ? 'CREDIT' : 'DEBIT',
      ...
      console.error('HDFC Bank transaction history error:', error.response?.data || error.message);
      return {
        success: false,
  ...
  async initiatePayment(paymentRequest: HDFCPaymentRequest): Promise<HDFCBankResponse<HDFCPaymentResponse>> {
    try {
      const payload = {
      ...
      const response = await this.client.post('/api/v1/payments/transfer', payload);
      ...
      console.error('HDFC Bank payment error:', error.response?.data || error.message);
      return {
        success: false,
  ...
  async validateAccount(accountNumber: string, ifscCode: string): Promise<HDFCBankResponse<HDFCAccountValidation>> {
    try {
      ...
      console.error('HDFC Bank account validation error:', error.response?.data || error.message);
      return {
        success: false,
  ...
  async generateStatement(request: HDFCStatementRequest): Promise<HDFCBankResponse<{ downloadUrl: string }>> {
    try {
      ...
      return {
        success: true,
        data: {
          downloadUrl: statementData.downloadUrl || statementData.fileUrl || '#'
        }
      };
    } catch (error: any) {
      console.error('HDFC Bank statement generation error:', error.response?.data || error.message);
      return {
        success: false,
  ...
  async getPaymentStatus(transactionId: string): Promise<HDFCBankResponse<HDFCPaymentResponse>> {
    try {
      const response = await this.client.get(`/api/v1/payments/status/${transactionId}`);
      ...
      return {
        ...
        data: {
          ...
          charges: parseFloat(statusData.charges || '0'),
          message: statusData.message || statusData.remarks,
          utr: statusData.utr
        }
      };
    } catch (error: any) {
      console.error('HDFC Bank payment status error:', error.response?.data || error.message);
      return {
        success: false,
  ...
  async healthCheck(): Promise<HDFCBankResponse<{ status: string; timestamp: string }>> {
    try {
