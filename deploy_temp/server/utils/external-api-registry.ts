...
import { CircuitBreaker, CircuitBreakerOptions, circuitBreakerRegistry, CircuitState } from './circuitBreaker';
import { logger } from '../logger';

export interface ExternalServiceConfig {
  name: string;
  baseUrl?: string;
  category: 'kyc' | 'payment' | 'market_data' | 'communication' | 'analytics' | 'compliance';
...
const EXTERNAL_SERVICES: ExternalServiceConfig[] = [
  ...
  },
  {
    ...
    category: 'kyc',
    options: { failureThreshold: 3, timeout: 90_000, monitoringWindow: 120_000 },
  },
  {
    name: 'ckyc_registry',
  ...
  },
  {
    ...
    category: 'market_data',
    options: { failureThreshold: 5, timeout: 60_000, monitoringWindow: 120_000 },
  },
  // ── Communication ─────────────────────────────────────────────────────────
  {
    name: 'twilio',
...
class ExternalApiRegistry {
  ...
  constructor() {
      ...
      circuitBreakerRegistry.get(svc.name, {
        ...
        onStateChange: (state, name) => {
          if (state === CircuitState.OPEN) {
            logger.warn(`[ExternalAPI] Circuit OPEN for ${name} — blocking requests`, { service: name });
          } else if (state === CircuitState.CLOSED) {
            logger.info(`[ExternalAPI] Circuit CLOSED for ${name} — service recovered`, { service: name });
          }
        },
        onFailure: (err, name) => {
    ...
    logger.info(`[ExternalAPI] Registry initialized with ${EXTERNAL_SERVICES.length} services`);
  }
   ...
   */
  getHealth(): Record<string, { state: string; failures: number; category: string }> {
    const result: Record<string, { state: string; failures: number; category: string }> = {};
