...
function calculateSettlementDate(
	escrowedAt: string | undefined,
	cycle: number = SETTLEMENT_CYCLE.T_PLUS_2,
): Date {
	const baseDate = escrowedAt ? new Date(escrowedAt) : new Date();
	return addBusinessDays(baseDate, cycle);
}

function getSettlementProgress(deal: Deal): {
	percentage: number;
...
function DealSettlementCard({
	deal,
	userRole,
}: { deal: Deal; userRole: string }) {
			...
			<CardHeader className="pb-2">
				<div className="flex justify-between items-start">
					<div>
						...
						<CardDescription>
							{deal.quantity} shares @ ₹
							{Number.parseFloat(deal.agreedPrice).toLocaleString()}
						</CardDescription>
					</div>
					...
					<div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
						...
						<div>
							...
							<p className="font-semibold text-green-600">
								...
								{Number.parseFloat(
									userRole === "seller"
										? deal.sellerPayout || deal.totalValue
						...
						<div>
							...
							<p className="font-medium">
								₹{Number.parseFloat(deal.platformFee || "0").toLocaleString()}
							</p>
						</div>
						<div>
							...
							<p className="font-medium">
								{expectedSettlement
									? format(expectedSettlement, "MMM dd, yyyy")
									: "Pending"}
							</p>
						</div>
					</div>
...
export function SettlementDashboard({
	...
	const stats = {
		...
		totalValue: activeDeals.reduce(
			(sum, d) => sum + Number.parseFloat(d.totalValue),
			0,
		),
	};
			...
			<div className="flex justify-between items-center">
				<div>
					...
					<p className="text-muted-foreground">
						Track your unlisted share settlements with T+2 cycle
					</p>
				</div>
					...
					{delayedDeals.length === 0 ? (
						<Card>
							<CardContent className="py-8 text-center">
								...
								<p className="text-muted-foreground">
									No delayed settlements - all on track!
								</p>
							</CardContent>
						</Card>
					) : (
