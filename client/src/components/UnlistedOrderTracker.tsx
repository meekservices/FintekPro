import { ClipboardList, Handshake, CreditCard, Truck, CheckCircle2, AlertTriangle, Clock, XCircle } from "lucide-react";
...
export function UnlistedOrderTracker({ 
  ...
  const stages = [
    { key: 'created', label: 'Request Created', icon: ClipboardList, date: createdAt },
    { key: 'matched', label: 'Deal Matched', icon: Handshake, date: matchedAt },
    { key: 'payment', label: 'Payment Confirmed', icon: CreditCard, date: paymentAt },
    { key: 'transfer', label: 'Transfer in Progress', icon: Truck, date: transferAt },
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4">
          ...
          {statusMessage && (
            <p className="text-sm text-red-500 dark:text-red-400 mt-2">{statusMessage}</p>
          )}
        </div>
      </div>
    );
      ...
      <div className="relative">
        ...
        <div className="relative flex justify-between">
          {stages.map((stage, index) => {
            ...
            return (
              <div key={stage.key} className="flex flex-col items-center" data-testid={`stage-${stage.key}`}>
                <div
