...
export function IdleTimeoutManager({
	...
	const handleLogout = useCallback(async () => {
		...
		try {
			...
			console.warn(
				"[IdleTimeout] Logout request failed, continuing with redirect",
			);
		}
				...
				<div className="space-y-4">
					...
					<div className="rounded-md bg-amber-50 dark:bg-amber-900/20 p-3 border border-amber-200 dark:border-amber-800">
						<p className="text-xs text-amber-800 dark:text-amber-200">
							<span className="font-semibold">Security Notice:</span> As per RBI
							Digital Lending Guidelines, sessions are automatically locked
							after {timeoutMinutes} minutes of inactivity for your security.
						</p>
					</div>
