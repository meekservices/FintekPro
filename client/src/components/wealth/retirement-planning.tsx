...
export function RetirementPlanning() {
	...
	useEffect(() => {
		...
		const futureMonthlyExpenses =
			...
			(1 + plan.inflationRate / 100) ** yearsToRetirement;
		...
		const futureValueOfCurrentSavings =
			plan.currentSavings *
			(1 + plan.expectedReturn / 100) ** yearsToRetirement;
		const remainingCorpus = requiredCorpus - futureValueOfCurrentSavings;

		const sipAmount =
			remainingCorpus /
			((((1 + monthlyRate) ** monthsToRetirement - 1) / monthlyRate) *
				(1 + monthlyRate));
	...
	const getRetirementReadiness = () => {
		...
		if (progress >= 80)
			return {
				status: "excellent",
				color: "green",
				message: "On track for comfortable retirement",
			};
		if (progress >= 60)
			return {
				status: "good",
				color: "blue",
				message: "Good progress, minor adjustments needed",
			};
		if (progress >= 40)
			return {
				status: "moderate",
				color: "yellow",
				message: "Moderate progress, increase contributions",
			};
		return {
			status: "needs attention",
			color: "red",
			message: "Urgent action needed for retirement goals",
		};
	};
		...
		<div className="space-y-6">
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-muted-foreground">
						Plan your retirement with personalized investment strategies
					</p>
				</div>
				<Button
					variant="outline"
						...
						<div className="flex items-center justify-between">
							<div>
								...
								<p className="text-xl font-bold">
									{formatCurrency(plan.retirementCorpus)}
								</p>
							</div>
					...
					<CardContent className="p-6">
						<div className="flex items-center justify-between">
							<div>
								...
								<p className="text-xl font-bold">
									{formatCurrency(plan.monthlyInvestmentNeeded)}
								</p>
							</div>
			...
			{showCalculation && (
				<Card data-testid="card-retirement-calculator">
					<CardHeader>
						...
						<CardDescription>
							Adjust your parameters to see the impact on your retirement
							planning
						</CardDescription>
					</CardHeader>
					<CardContent>
							...
							<div className="space-y-2">
								...
								<Input
									...
									onChange={(e) =>
										setPlan({
											...plan,
											currentAge: Number.parseInt(e.target.value) || 30,
										})
									}
								/>
							</div>
							...
							<div className="space-y-2">
								...
								<Input
									...
									onChange={(e) =>
										setPlan({
											...plan,
											retirementAge: Number.parseInt(e.target.value) || 60,
										})
									}
								/>
							</div>
							...
							<div className="space-y-2">
								...
								<Input
									...
									onChange={(e) =>
										setPlan({
											...plan,
											currentSalary: Number.parseInt(e.target.value) || 0,
										})
									}
								/>
							</div>
							...
							<div className="space-y-2">
								...
								<Select
									...
									onValueChange={(value: any) =>
										setPlan({ ...plan, riskProfile: value })
									}
								>
			...
			<Card data-testid="card-retirement-progress">
				...
				<CardContent>
					<div className="space-y-4">
						...
						>
							<p className={`font-medium text-${readiness.color}-700`}>
								{readiness.message}
							</p>
						</div>
					</div>
			...
			<Tabs defaultValue="allocation" className="space-y-4">
				<ScrollableTabsList className="grid w-full grid-cols-3">
					<TabsTrigger value="allocation" data-testid="tab-asset-allocation">
						Asset Allocation
					</TabsTrigger>
					<TabsTrigger
						value="instruments"
						data-testid="tab-investment-instruments"
					>
						Investment Instruments
					</TabsTrigger>
					<TabsTrigger value="schemes" data-testid="tab-retirement-schemes">
						Retirement Schemes
					</TabsTrigger>
				</ScrollableTabsList>
				...
				<TabsContent value="allocation">
					<Card data-testid="card-recommended-allocation">
						<CardHeader>
							...
							<CardDescription>
								Based on your age ({plan.currentAge}) and risk profile (
								{plan.riskProfile})
							</CardDescription>
						</CardHeader>
						<CardContent>
				...
				<TabsContent value="instruments">
					<Card data-testid="card-recommended-instruments">
						<CardHeader>
							...
							<CardDescription>
								Curated investment options based on your retirement timeline and
								risk profile
							</CardDescription>
						</CardHeader>
						<CardContent>
								...
								{recommendations.instruments.map((instrument, index) => (
									...
									>
										...
										<Button
											size="sm"
											variant="outline"
											data-testid={`button-invest-${index}`}
										>
											Invest
										</Button>
									</div>
								...
								<div className="space-y-3">
									<div className="flex items-center justify-between p-3 bg-green-50 dark:bg-green-950/30 rounded-lg">
										<div>
											...
											<p className="text-sm text-muted-foreground">
												Currently earning 8.25% interest
											</p>
										</div>
									...
									<div className="flex items-center justify-between p-3 bg-purple-50 dark:bg-purple-950/30 rounded-lg">
										<div>
											...
											<p className="text-sm text-muted-foreground">
												Market-linked returns, low cost
											</p>
										</div>
				...
				<CardContent>
					<div className="space-y-4">
							...
							<div className="space-y-3">
								...
								<div className="space-y-2">
									...
									<div className="flex items-center gap-2 p-2 bg-yellow-50 dark:bg-yellow-950/30 rounded">
										...
										<span className="text-sm">
											Gradually shift to conservative allocation near retirement
										</span>
									</div>
									<div className="flex items-center gap-2 p-2 bg-pink-50 dark:bg-pink-950/30 rounded">
										...
										<span className="text-sm">
											Consider partial withdrawal options post 60
										</span>
									</div>
								</div>
							</div>
						...
						<div className="mt-6 p-4 bg-muted rounded-lg">
							<p className="text-sm text-muted-foreground">
								<strong>Note:</strong> This is a basic retirement projection.
								Actual returns may vary based on market conditions. Consider
