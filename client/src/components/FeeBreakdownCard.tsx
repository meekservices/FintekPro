...
export function FeeBreakdownCard({
  ...
  const categoryGroups = fees.reduce((acc, fee) => {
  ...
  return (
    <div className="space-y-2" data-testid="fee-breakdown-card">
      <Collapsible open={isOpen} onOpenChange={setIsOpen}>
        <CollapsibleTrigger className="flex items-center justify-between w-full text-sm hover:bg-gray-50 dark:hover:bg-gray-800 p-2 rounded transition-colors" data-testid="button-toggle-fee-details">
        ...
        <CollapsibleContent className="space-y-3 pt-2">
          {Object.entries(categoryGroups).map(([category, categoryFees]) => {
            const categoryTotal = categoryFees.reduce((sum, f) => sum + f.netAmount, 0);
