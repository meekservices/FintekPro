	...
	const { data, isLoading, isError } = useQuery<{
		...
		queryFn: async () => {
			const response = await fetch(
				`/api/fee-mode/calculate-fees?orderValue=${orderValueInr}`,
				{
					credentials: "include",
				},
			);
				...
				<Tooltip>
					<TooltipTrigger asChild>
						<div
							...
							data-testid="fee-breakdown-compact"
						>
			...
			<CardContent className="space-y-3">
				<div className="space-y-2 text-sm">
					<div className="flex justify-between text-muted-foreground">
						...
						<span>
							₹
							{orderValueInr.toLocaleString("en-IN", {
							...
							<span className="flex items-center gap-1">
								...
								<TooltipProvider>
									<Tooltip>
										...
										<TooltipContent>
											{fees.advisoryFeeBps} basis points (
											{(fees.advisoryFeeBps / 100).toFixed(2)}%)
										</TooltipContent>
									</Tooltip>
								</TooltipProvider>
							</span>
						...
						<span className="flex items-center gap-1">
							...
							<TooltipProvider>
								<Tooltip>
									...
									<TooltipContent>
										{fees.platformFeeBps} basis points (
										{(fees.platformFeeBps / 100).toFixed(2)}%)
									</TooltipContent>
								</Tooltip>
							</TooltipProvider>
						</span>
				...
				{!isAdvisory && (
					<p className="text-xs text-muted-foreground bg-muted p-2 rounded">
						You are in Platform-Only mode. No advisory fees apply. AI
