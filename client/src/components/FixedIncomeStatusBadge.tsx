...
export function FixedIncomeStatusBadge({
	status,
	showLabel = true,
}: FixedIncomeStatusBadgeProps) {
	switch (status) {
		case "SELLABLE":
			return (
		...
		case "VISIBLE":
			return (
				<Badge
					variant="secondary"
					className="bg-amber-500/10 text-amber-600 border-amber-200 dark:border-amber-800 gap-1"
				>
...
export function VisibleInstrumentWarning({
	instrumentName,
	compact = false,
}: VisibleInstrumentWarningProps) {
	if (compact) {
		return (
			<div className="flex items-center gap-2 px-3 py-2 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-md text-sm text-amber-700 dark:text-amber-300">
				...
				<span>
					This instrument is shown for reference only and cannot be recommended
					or transacted.
				</span>
			</div>
		);
	...
	return (
		<Alert className="bg-amber-50 dark:bg-amber-950/30 border-amber-200 dark:border-amber-800">
			...
			<AlertDescription className="text-amber-700 dark:text-amber-300">
				<strong>{instrumentName || "This instrument"}</strong> is displayed for
				reference only. It cannot be recommended or transacted at this time due
				to liquidity, regulatory, or credit constraints.
			</AlertDescription>
		</Alert>
	);
}
