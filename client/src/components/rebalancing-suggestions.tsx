...
export function RebalancingSuggestions({
	...
	const {
		...
		isLoading,
		error,
	} = useQuery<RebalancingSuggestionsData>({
			...
			<CardContent className="space-y-4">
				...
				{suggestions?.summary?.rebalanceNeeded ? (
					<Alert>
						...
						<AlertDescription>
							Your portfolio has{" "}
							{suggestions?.summary?.highPrioritySuggestions || 0} high-priority
							rebalancing opportunities worth ₹
							{suggestions?.summary?.totalRebalanceAmount?.toLocaleString() ||
								"0"}
							.
						</AlertDescription>
					</Alert>
					...
					<Alert>
						...
						<AlertDescription>
							Your portfolio allocation is well balanced. Minor adjustments may
							still optimize returns.
						</AlertDescription>
					</Alert>
				)}
						...
						>
							...
							{suggestion.action !== "maintain" &&
								suggestion.priority !== "low" && (
									<Button
										variant={
											suggestion.action === "buy" ? "default" : "outline"
										}
										size="sm"
										className="w-full"
										data-testid={`action-${suggestion.assetType}`}
									>
										{suggestion.action === "buy" ? "Add to" : "Reduce"}{" "}
				...
				<p className="text-xs text-muted-foreground text-center pt-4 border-t">
					Last updated:{" "}
					{suggestions?.summary?.lastUpdated
						? new Date(suggestions.summary.lastUpdated).toLocaleTimeString()
						: "Never"}
				</p>
			</CardContent>
		</Card>
