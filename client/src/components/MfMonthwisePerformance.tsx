...
export function MfMonthwisePerformance({
	schemeCode,
	schemeName,
	months = 24,
}: MonthwisePerformanceProps) {
	...
	const { data, isLoading, error, refetch } = useQuery<{
		...
		queryFn: async () => {
			const response = await fetch(
				`/api/mutual-funds/${schemeCode}/monthwise-performance?months=${months}`,
			);
			if (!response.ok)
				throw new Error("Failed to fetch monthwise performance");
	...
	if (monthlyData.length === 0) {
		return (
			<Card>
				...
				<CardContent>
					<p className="text-muted-foreground text-sm">
						No monthly performance data available yet. Click "Calculate" to
						generate performance data from historical NAV.
					</p>
				</CardContent>
			</Card>
		);
	...
	const positiveMonths = monthlyData.filter(
		(m) => m.returnPercent !== null && m.returnPercent > 0,
	).length;
	const negativeMonths = monthlyData.filter(
		(m) => m.returnPercent !== null && m.returnPercent < 0,
	).length;
	const avgReturn =
		monthlyData.reduce((acc, m) => acc + (m.returnPercent || 0), 0) /
		monthlyData.length;
		...
		<Card>
			...
			<CardContent>
						...
						<TableBody>
							{monthlyData.map((month) => (
								<TableRow key={month.monthYear}>
									...
									<TableCell
										className={`text-right font-medium ${getReturnColor(month.returnPercent)}`}
									>
									...
									<TableCell
										className={`text-right ${getReturnColor(month.benchmarkReturn)}`}
									>
										{formatReturn(month.benchmarkReturn)}
									</TableCell>
									<TableCell
										className={`text-right font-medium ${getReturnColor(month.excessReturn)}`}
									>
				...
				<p className="text-xs text-muted-foreground mt-4">
					* Benchmark comparison coming soon | Returns calculated from
					historical NAV data
				</p>
			</CardContent>
		</Card>
