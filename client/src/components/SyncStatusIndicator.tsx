...
export function QueuedActionsBadge({ count }: { count: number }) {
	...
	return (
		<Tooltip>
			<TooltipTrigger asChild>
				<div
					className="inline-flex items-center gap-1.5 px-2 py-1 rounded-full text-xs font-medium bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400"
...
export function ComplianceDisclosure({
	...
	const disclosures = {
		execution:
			"Execution will occur only after internet connectivity is restored and confirmed.",
		offline:
			"Actions performed while offline are drafts only. No transactions will be executed until you are online.",
		advisory:
