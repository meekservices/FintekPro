...
export function USFundingCard() {
	...
	const linkTokenMutation = useMutation({
		mutationFn: async () => {
			const res = await apiRequest("/api/alpaca/funding/plaid/link-token", {
				method: "POST",
			});
	...
	const linkBankMutation = useMutation({
		...
		onSuccess: () => {
			...
			toast({
				title: "Bank Linked!",
				description:
					"Your bank account has been successfully linked via Plaid.",
			});
		},
	});
	...
	return (
		<Card className="border-primary/20 bg-gradient-to-br from-background to-primary/5">
			<CardHeader>
				...
				<CardDescription>
					Fund your US brokerage account via ACH transfer.
				</CardDescription>
			</CardHeader>
