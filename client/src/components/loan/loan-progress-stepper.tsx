				...
				{stages.map((stage, index) => {
					...
					return (
						...
						>
							...
							>
								{stage.key === "decision" && isSuccess
									? "Approved"
									: stage.key === "decision" && isRejected
										? "Rejected"
										: stage.key === "decision" && status === "withdrawn"
											? "Withdrawn"
											: stage.key === "decision" && status === "expired"
												? "Expired"
												: stage.label}
							</span>
						</div>
...
export const loanTypeProcessingTime: Record<
	string,
...
export function ProcessingTimeDisplay({
	loanType,
	className = "",
	...
	return (
		...
		>
			...
			<span>
				...
				<strong className="text-foreground">{timing.label}</strong>
			</span>
		</div>
	);
}
