...
export function GlobalSearch() {
	...
	const { data, isLoading } = useQuery<{
		...
		queryFn: async () => {
			const res = await fetch(
				`/api/features/search?q=${encodeURIComponent(query)}&category=${category}`,
			);
			return res.json();
		},
	...
	const getCategoryPlaceholder = () => {
		switch (category) {
			case "stocks":
				return "Search stocks... (min 3 chars)";
			case "mutualFunds":
				return "Search mutual funds... (min 3 chars)";
			case "bonds":
				return "Search bonds... (min 3 chars)";
			case "goals":
				return "Search goals... (min 3 chars)";
			case "orders":
				return "Search orders... (min 3 chars)";
			default:
				return "Search... (min 3 chars, ⌘K)";
		}
	};
		...
		? {
				stocks:
					category === "all" || category === "stocks"
						...
						: [],
				mutualFunds:
					category === "all" || category === "mutualFunds"
						? data.results.mutualFunds
						: [],
				bonds:
					category === "all" || category === "bonds" ? data.results.bonds : [],
				goals:
					category === "all" || category === "goals" ? data.results.goals : [],
				orders:
	...
	return (
		...
		>
			<div className="flex gap-2">
				<Select
					value={category}
					onValueChange={(val) => setCategory(val as SearchCategory)}
				>
			...
			{isOpen && query.length >= 3 && (
				<Card
					className="absolute top-full mt-2 w-full z-50 max-h-96 overflow-auto shadow-lg"
