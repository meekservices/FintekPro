...
export function ClientTransactionHistory({ 
  ...
  const { data: transactionsData, isLoading } = useQuery<{
    ...
    queryFn: async () => {
      ...
      const response = await fetch(`/api/client/transactions?${params}`, { credentials: 'include' });
      ...
      >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <span className="font-medium text-sm truncate">
                {tx.productName || categoryLabels[tx.productCategory] || tx.productCategory}
              </span>
        ...
        <div className="flex items-center gap-3 ml-3">
          <div className="text-right">
            <div className="font-semibold text-sm">{formatAmount(tx.amount)}</div>
            {tx.quantity && <div className="text-xs text-muted-foreground">{tx.quantity} units</div>}
          </div>
        ...
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="flex items-center gap-2">
              ...
              {category ? `${categoryLabels[category] || category} Transactions` : "Transaction History"}
            </CardTitle>
            {!compact && (
              <CardDescription>
          ...
          <div className="flex flex-wrap gap-2 mt-3">
            {summary.map(s => (
              <Badge key={s.category} variant="secondary" className="py-1 px-2">
                {categoryLabels[s.category] || s.category}: {formatAmount(s.totalAmount)} ({s.count})
              </Badge>
            ))}
          </div>
        )}
