...
function ScoreIndicator({ score, label }: { score: number; label: string }) {
  const getScoreColor = (s: number) => {
    if (s >= 90) return "text-green-600 bg-green-100 dark:text-green-400 dark:bg-green-900/30";
    if (s >= 75) return "text-blue-600 bg-blue-100 dark:text-blue-400 dark:bg-blue-900/30";
    if (s >= 60) return "text-yellow-600 bg-yellow-100 dark:text-yellow-400 dark:bg-yellow-900/30";
    return "text-muted-foreground bg-muted/30";
  };

  return (
    <div className={cn("px-2 py-1 rounded-md text-sm font-medium", getScoreColor(score))}>
      {label}: {score}
    </div>
  );
}
...
function AIRecommendationCard({ 
  ...
  const { score_breakdown, key_factors, risk_factors } = product.rationale_inputs;
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <CardDescription className="text-xs">{product.issuer}</CardDescription>
            </div>
          </div>
          <div className="flex items-center gap-1">
            <Sparkles className="h-4 w-4 text-amber-500" />
            <span className="text-sm font-bold text-amber-600">{product.suitability_score}</span>
          </div>
        </div>
      </CardHeader>
      <CardContent className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2 text-sm">
          <div>
            ...
            <p className="font-medium">₹{product.min_investment.toLocaleString()}</p>
          </div>
          <div>
            <span className="text-muted-foreground">Expected Return</span>
            <p className="font-medium text-green-600">
              {product.expected_return_band.min}% - {product.expected_return_band.max}%
            </p>
          </div>
        </div>
    ...
    return (
      <div className={cn("p-4 border rounded-lg bg-muted/50", className)}>
        <div className="flex items-center gap-2 text-muted-foreground">
          ...
          <span>Unable to load AI recommendations. Please try again later.</span>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className={cn("p-4 border rounded-lg bg-muted/50", className)}>
        <div className="flex items-center gap-2 text-muted-foreground">
          ...
          <span>No recommendations available for your profile at this time.</span>
        </div>
      </div>
    );
  }
