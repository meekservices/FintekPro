    ...
    <Card className="border-dashed border-2 bg-gradient-to-br from-muted/30 to-muted/10">
      <CardHeader className="text-center pb-4">
        ...
        <CardDescription className="text-base">
          {message || `We're working hard to bring you ${categoryName}. Register your interest to be notified when it launches.`}
        </CardDescription>
        {expectedDate && (
          <div className="flex items-center justify-center gap-2 mt-3 text-sm text-muted-foreground">
            <Calendar className="h-4 w-4" />
            <span>Expected: {new Date(expectedDate).toLocaleDateString('en-IN', { month: 'long', year: 'numeric' })}</span>
          </div>
        )}
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4 max-w-md mx-auto">
          ...
          <div className="space-y-2">
            <Label htmlFor="inquiry-message">What interests you about {categoryName}?</Label>
            <Textarea
              id="inquiry-message"
          ...
          <p className="text-xs text-center text-muted-foreground">
            We'll contact you when {categoryName} products become available. Your data is safe with us.
          </p>
        </form>
      </CardContent>
    </Card>
