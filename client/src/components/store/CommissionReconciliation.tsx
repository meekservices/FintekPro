...
export function CommissionReconciliation() {
	...
	const { data: zohoStatus, isLoading: statusLoading } =
		useQuery<ZohoSyncStatus>({
		...
		useQuery<{
			...
			queryFn: async () => {
				...
				if (selectedSyncStatus !== "all")
					params.append("syncStatus", selectedSyncStatus);
				const response = await fetch(
					`/api/admin/commission-reconciliation?${params}`,
					{ credentials: "include" },
				);
				if (!response.ok)
	...
	const syncAllMutation = useMutation({
		...
		onSuccess: (data: any) => {
			...
			queryClient.invalidateQueries({
				queryKey: ["/api/admin/commission-reconciliation"],
			});
			queryClient.invalidateQueries({
				queryKey: ["/api/admin/zoho-books/sync/status"],
			});
	...
	const syncSingleMutation = useMutation({
		...
		onSuccess: () => {
			toast({
				...
				description: "Successfully synced to Zoho Books",
			});
			queryClient.invalidateQueries({
				queryKey: ["/api/admin/commission-reconciliation"],
			});
	...
	const approvePayoutMutation = useMutation({
		mutationFn: async ({
			payoutId,
			forceLocalApproval = false,
		}: { payoutId: string; forceLocalApproval?: boolean }) => {
			const response = await fetch(
		...
		<div className="space-y-6">
			{!zohoStatus?.connected && (
				<Alert variant="destructive">
					...
					<AlertDescription>
						Zoho Books is not connected. Please configure the integration to
						enable commission reconciliation.
					</AlertDescription>
				</Alert>
			)}
						...
						<div className="flex items-center justify-between">
							<div>
								...
								<p className="text-2xl font-bold">
									{reconciliationData?.summary?.totalTransactions || 0}
								</p>
							</div>
						...
						<div className="flex items-center justify-between">
							<div>
								...
								<p className="text-2xl font-bold text-green-600">
									{reconciliationData?.summary?.matched || 0}
								</p>
							</div>
					...
					<CardContent className="p-4">
						<div className="flex items-center justify-between">
							<div>
								...
								<p className="text-2xl font-bold">
									{formatCurrency(
										reconciliationData?.summary?.totalAmount || "0",
									)}
								</p>
							</div>
						...
						<CardHeader className="pb-3">
							<div className="flex items-center justify-between">
								<div>
									...
									<CardDescription>
										Match store transactions with Zoho Books entries
									</CardDescription>
								</div>
								<div className="flex items-center gap-2">
									<Select
									...
									<Select
										...
										onValueChange={setSelectedSyncStatus}
									>
										<SelectTrigger
											className="w-[150px]"
											data-testid="select-sync-status"
										>
										...
										{reconciliationData?.items?.map((item) => (
											...
											>
												...
												<TableCell>
													<Badge
														className={
															syncStatusColors[item.matchStatus] ||
												...
												<TableCell>
													{item.zohoInvoiceId ? (
														<div className="flex items-center gap-1 text-xs">
															...
															<span>
																INV: {item.zohoInvoiceId.substring(0, 10)}...
															</span>
														</div>
													) : item.zohoBillId ? (
														<div className="flex items-center gap-1 text-xs">
															...
															<span>
																BILL: {item.zohoBillId.substring(0, 10)}...
															</span>
														</div>
													) : (
													...
													{item.matchStatus === "pending" &&
														zohoStatus?.connected && (
															<Button
																...
																variant="outline"
																onClick={() =>
																	syncSingleMutation.mutate({
																		id: item.id,
																		type:
																			item.productType === "mutual_fund"
																				? "mutual-fund"
																				: item.productType,
																	})
																}
				...
				<TabsContent value="payouts" className="space-y-4">
								...
								<div className="flex items-center justify-between">
									<div>
										...
										<p className="text-xl font-bold text-yellow-600">
											{formatCurrency(
												payoutsData?.summary?.totalPending || "0",
											)}
										</p>
									</div>
								...
								<div className="flex items-center justify-between">
									<div>
										...
										<p className="text-xl font-bold text-blue-600">
											{formatCurrency(
												payoutsData?.summary?.totalApproved || "0",
											)}
										</p>
									</div>
								...
								<div className="flex items-center justify-between">
									<div>
										...
										<p className="text-xl font-bold">
											{payoutsData?.summary?.pendingCount || 0}
										</p>
									</div>
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Agent and partner commission payout reconciliation with Zoho
								Books
							</CardDescription>
						</CardHeader>
						<CardContent>
												...
												<TableCell>
													{payout.zohoBillId ? (
														<div className="flex items-center gap-1 text-xs">
															<Link2 className="h-3 w-3 text-green-500" />
															<span>
																{payout.zohoBillId.substring(0, 10)}...
															</span>
														</div>
													) : (
