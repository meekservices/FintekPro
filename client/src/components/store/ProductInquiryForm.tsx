			...
			<Card className="bg-background border-border max-w-lg mx-auto">
				<CardContent className="pt-8 pb-8 text-center">
					...
					<p className="text-muted-foreground mb-6">
						Your inquiry about{" "}
						<strong className="text-foreground">{itemName}</strong> has been
						submitted successfully. Our team will contact you within 24-48
						hours.
					</p>
		...
		<Card className="bg-background border-border">
			<CardHeader>
				<div className="flex items-center gap-3 mb-2">
					...
					>
						{type === "product" ? (
							...
							<FolderTree
								className={`w-6 h-6 ${type === "subcategory" ? "text-blue-400" : "text-amber-400"}`}
							/>
						)}
					</div>
					<div>
				...
				<CardDescription className="text-muted-foreground">
					{description ||
						`This ${type} is currently not available for direct access. Submit an inquiry and our team will assist you.`}
				</CardDescription>
			</CardHeader>
			...
			<CardContent>
				<Alert className="mb-6 bg-amber-500/10 border-amber-500/30">
					...
					<AlertDescription className="text-amber-200">
						...
						Please submit your details and we'll reach out to discuss your
						requirements.
					</AlertDescription>
				</Alert>
		...
		<Card className="bg-background/50 border-border border-dashed">
			<CardContent className="p-6 text-center">
				...
				{productType && (
					<Badge
						variant="outline"
						className="mb-3 text-muted-foreground border-border"
					>
						{productType}
					</Badge>
				)}
				<p className="text-sm text-muted-foreground mb-4">
					This product is currently unavailable. Click below to express your
					interest.
				</p>
				<Button
					variant="outline"
		...
		<Alert className="bg-amber-500/10 border-amber-500/30 mb-6">
			...
			<div className="flex-1 ml-3">
				<h4 className="font-medium text-amber-200">
					{categoryName} - Currently Unavailable
				</h4>
				<AlertDescription className="text-amber-200/80 text-sm mt-1">
					{message ||
						`This ${type} is temporarily unavailable. Our team is working on bringing it back.`}
				</AlertDescription>
			</div>
			{onInquiry && (
				<Button
