...
export function CkycDeferredDashboard() {
	...
	const {
		...
		success: boolean;
		data: { cases: DeferredCase[]; total: number };
	}>({
	...
	const checkSlaMutation = useMutation({
		mutationFn: () =>
			apiRequest("/api/admin/ckyc-deferred/check-sla-breaches", {
				method: "POST",
			}),
		onSuccess: () => {
	...
	const getStatusBadge = (status: string) => {
		switch (status) {
			case "ckyc_deferred":
				return (
					<Badge variant="destructive" data-testid="badge-status-deferred">
						Deferred
					</Badge>
				);
			case "manual_review_in_progress":
				return (
					<Badge variant="secondary" data-testid="badge-status-review">
						In Review
					</Badge>
				);
			case "resolved":
	...
	return (
		<div className="space-y-6" data-testid="ckyc-deferred-dashboard">
			...
			{stats && (
				<Alert
					variant={stats.environmentMode === "PROD" ? "default" : "destructive"}
					data-testid="alert-environment"
				>
			...
			<div
				...
				data-testid="aging-buckets-grid"
			>
				<Card data-testid="card-less-than-24h">
					...
					<CardContent>
						<div className="text-3xl font-bold text-green-600">
							{stats?.agingBuckets.lessThan24h || 0}
						</div>
				...
				<Card data-testid="card-24-72h">
					...
					<CardContent>
						<div className="text-3xl font-bold text-yellow-600">
							{stats?.agingBuckets.between24And72h || 0}
						</div>
				...
				<Card data-testid="card-more-than-72h">
					...
					<CardContent>
						<div className="text-3xl font-bold text-red-600">
							{stats?.agingBuckets.moreThan72h || 0}
						</div>
				...
				<div className="flex gap-2">
					...
					>
						...
						<SelectContent>
							...
							<SelectItem value="manual_review_in_progress">
								In Review
							</SelectItem>
