...
export default function DocumentWorkflowDashboard() {
	...
	const stats = {
		...
		pending: allWorkflows.filter((w: any) =>
			["pending_edit", "pending_approval", "pending_signature"].includes(
				w.status,
			),
		).length,
											...
											<TableRow key={workflow.id}>
												...
												<TableCell>
													<Badge variant="outline">
														{workflow.proposalType || "Investment"}
													</Badge>
												</TableCell>
												<TableCell>{getStatusBadge(workflow.status)}</TableCell>
												<TableCell>
													<span className="text-sm">
														v{workflow.currentVersion} (Round{" "}
														{workflow.negotiationRound})
													</span>
												</TableCell>
												<TableCell>
													<div className="flex items-center gap-1">
														...
														<span>
															{workflow.signedCount || 0}/
															{workflow.participantCount || 0}
														</span>
													</div>
												</TableCell>
																	...
																	<TabsContent value="details">
																		<div className="grid grid-cols-2 gap-4 py-4">
																			<div>
																				...
																				<div className="mt-1">
																					{getStatusBadge(
																						workflowDetails?.workflow?.status ||
																							workflow.status,
																					)}
																				</div>
																			</div>
																			...
																			<div>
																				...
																				<div className="mt-1 font-medium">
																					{workflowDetails?.workflow?.deadline
																						? formatDate(
																								workflowDetails.workflow
																									.deadline,
																							)
																	...
																	<TabsContent value="audit">
																		<ScrollArea className="h-[300px]">
																			<div className="space-y-2 py-4">
																				{(auditLog?.auditLog || []).map(
																					(log) => (
																						<div
																							key={log.id}
																							className="flex gap-3 p-2 text-sm border-b"
																						>
						...
						<div className="space-y-3">
							{(pendingApprovals as any)?.pending?.map((item: any) => (
								...
								>
									<div>
										<div className="font-medium">
											{item.workflow.documentName}
										</div>
										<div className="text-sm text-muted-foreground">
											{item.participant.role === "signer"
												? "Signature required"
												: "Approval required"}
										</div>
									</div>
