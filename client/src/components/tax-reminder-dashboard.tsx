...
export default function TaxReminderDashboard() {
		...
		return (
			<Card className="border-dashed" data-testid="card-no-subscription">
				<CardHeader className="text-center">
					...
					<CardDescription>
						Subscribe to our quarterly tax reminder service to stay on top of
	...
	const daysUntilNext = upcomingReminder
		? Math.ceil(
				(new Date(upcomingReminder.dueDate).getTime() - new Date().getTime()) /
					(1000 * 60 * 60 * 24),
			)
		: null;
	...
	return (
		<div className="space-y-6" data-testid="tax-reminder-dashboard">
			<Card
				className="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-950 dark:to-indigo-900 border-blue-200 dark:border-blue-800"
							...
							<div className="space-y-4">
								<div>
									...
									<div className="text-2xl font-bold text-blue-600">
										{upcomingReminder.quarter} - FY{" "}
										{upcomingReminder.financialYear}
									</div>
								</div>
								<div>
									<div className="text-sm text-muted-foreground">Due Date</div>
									<div className="text-lg font-semibold">
										{new Date(upcomingReminder.dueDate).toLocaleDateString(
											"en-IN",
											{
												day: "numeric",
												month: "long",
									...
									<div className="grid grid-cols-2 gap-4">
										<div>
											...
											<div className="text-lg font-bold text-orange-600">
												₹
												{Number.parseFloat(
													upcomingReminder.estimatedSTCG,
												).toLocaleString()}
											</div>
										</div>
										<div>
											...
											<div className="text-lg font-bold text-green-600">
												₹
												{Number.parseFloat(
													upcomingReminder.estimatedLTCG,
												).toLocaleString()}
											</div>
										</div>
									</div>
									<div className="mt-3 pt-3 border-t">
										<div className="flex items-center justify-between">
											...
											<span className="text-xl font-bold text-blue-600">
												₹
												{Number.parseFloat(
													upcomingReminder.totalTaxLiability,
												).toLocaleString()}
											</span>
										</div>
									</div>
								...
								>
									...
									<div className="text-right">
										<div className="font-semibold text-foreground">
											₹
											{Number.parseFloat(
												reminder.totalTaxLiability,
											).toLocaleString()}
										</div>
										<Badge
											variant={
												reminder.status === "paid" ? "default" : "secondary"
											}
											className="text-xs"
										>
											{reminder.status}
