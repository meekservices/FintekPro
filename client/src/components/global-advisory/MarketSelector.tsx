...
export function MarketSelector({
	...
	const selectedMarket =
		markets.find((m) => m.marketCode === preferences?.selectedMarket) ||
	...
	if (marketsLoading || preferencesLoading) {
		return (
			<Button
				...
				disabled
				data-testid="market-selector-loading"
			>
	...
	return (
		<DropdownMenu>
				...
				>
					...
					{!compact && (
						<span className="hidden sm:inline">
							{selectedMarket?.marketName || "Select Market"}
						</span>
					)}
			...
			<DropdownMenuContent
				align="end"
				className="w-64"
				data-testid="market-selector-menu"
			>
				...
				{Object.entries(groupedMarkets).map(([region, regionMarkets]) => (
					<div key={region}>
						...
						{regionMarkets.map((market) => {
							...
							const isSelected =
								selectedMarket?.marketCode === market.marketCode;
