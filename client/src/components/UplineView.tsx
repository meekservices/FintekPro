			...
			<Card>
				...
				<CardContent>
					<p className="text-sm text-muted-foreground">
						You are at the top of your reporting chain.
					</p>
				</CardContent>
			</Card>
		);
			...
			<CardContent>
				<div className="space-y-2">
					{upline.map((agent, index) => {
						const levelLabel = agent.hierarchyLevel
							? LEVEL_LABELS[agent.hierarchyLevel]
							: `Level ${index + 1}`;
						return (
							<div key={agent.id} className="flex items-center gap-3">
								...
								<div
									className={`flex items-center gap-3 flex-1 rounded-lg border px-3 py-2 ${index === 0 ? "bg-primary/5 border-primary/20" : "bg-muted/30"}`}
								>
				...
				<p className="text-xs text-muted-foreground mt-3">
					Commissions from your sales flow through {upline.length} level
					{upline.length > 1 ? "s" : ""} above you.
				</p>
			</CardContent>
		</Card>
	);
