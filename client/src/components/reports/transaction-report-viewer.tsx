...
export function TransactionReportViewer() {
  ...
  const fetchFromNSDLMutation = useMutation({
    mutationFn: async (params: { userId: string; financialYear: string; clientId: string }) => {
  ...
  const fetchFromCDSLMutation = useMutation({
    mutationFn: async (params: { userId: string; financialYear: string; dpId: string; clientId: string }) => {
  ...
  const handleDownload = async (reportId: string, format: 'csv' | 'pdf' | 'json' = 'csv') => {
    try {
      const response = await fetch(`/api/transaction-reports/${reportId}/download?format=${format}`);
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'processing':
        return <Badge className="bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-200">Processing</Badge>;
      case 'failed':
        return <Badge className="bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-200">Failed</Badge>;
      default:
