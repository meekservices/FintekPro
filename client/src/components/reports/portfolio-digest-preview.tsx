...
export function PortfolioDigestPreview({
	userName = "Investor",
	period = "weekly",
}: PortfolioDigestPreviewProps) {
	const mockData = {
			...
			<CardContent className="pt-6 space-y-6">
				<div className="text-center p-6 bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-950 dark:to-emerald-950 rounded-lg">
					...
					<p className="text-4xl font-bold">
						{formatCurrency(mockData.portfolioValue)}
					</p>
					<div
						className={`flex items-center justify-center gap-2 mt-2 ${mockData.change >= 0 ? "text-green-600" : "text-red-600"}`}
					>
						{mockData.change >= 0 ? (
						...
						<span className="font-semibold">
							{mockData.change >= 0 ? "+" : ""}
							{formatCurrency(mockData.change)} ({mockData.changePercent}%)
						</span>
				...
				<div className="grid grid-cols-2 gap-4">
					<div className="p-4 border rounded-lg">
						...
						<p className="text-2xl font-bold text-green-600">
							{formatCurrency(mockData.dividendsReceived)}
						</p>
					</div>
					<div className="p-4 border rounded-lg">
						<div className="flex items-center gap-2 text-muted-foreground mb-1">
						...
						<p className="text-2xl font-bold">
							{formatCurrency(mockData.sipExecuted)}
						</p>
					</div>
				</div>
