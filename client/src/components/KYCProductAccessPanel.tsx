...
const PRODUCT_META: Record<
	string,
	{ icon: React.ElementType; category: string; color: string }
> = {
	MUTUAL_FUNDS: {
		icon: TrendingUp,
...
function StatusPill({
	...
	pct,
	expired,
}: { canProceed: boolean; pct: number; expired: boolean }) {
	if (expired) {
		return (
...
function ProductCard({ product }: { product: SufficiencyResult }) {
	...
	const meta = PRODUCT_META[product.productCode] ?? {
		icon: Globe,
		category: "Other",
	...
	const missingVerifications = product.verifications.filter(
		(v) => v.mandatory && !v.satisfied,
	);
	const missingData = product.dataFields.filter(
		(f) => f.mandatory && !f.satisfied,
	);
	const satisfiedVerifications = product.verifications.filter(
		(v) => v.satisfied,
	);
		...
		>
			<CardHeader className="pb-2">
				<div className="flex items-center justify-between">
					<div className="flex items-center gap-3">
						...
						>
							<Icon
								className={cn(
									"h-5 w-5",
									product.canProceed ? meta.color : "text-muted-foreground",
								)}
							/>
						</div>
						<div>
					...
					<div className="flex items-center gap-2">
						<StatusPill
							canProceed={product.canProceed}
							pct={product.completionPercentage}
							expired={product.kycIsExpired}
						/>
						{expanded ? (
			...
			{expanded && (
				...
				>
					...
					{(satisfiedVerifications.length > 0 || satisfiedData.length > 0) && (
						<div>
							<p className="text-xs font-semibold text-green-700 dark:text-green-400 mb-2 flex items-center gap-1">
								<CheckCircle2 className="h-3 w-3" /> Already verified from your
								KYC
							</p>
							<div className="flex flex-wrap gap-1.5">
								{satisfiedVerifications.map((v) => (
									<span
										key={v.key}
										className="inline-flex items-center gap-1 text-xs bg-green-50 dark:bg-green-950 text-green-700 dark:text-green-300 px-2 py-0.5 rounded-full border border-green-200 dark:border-green-800"
									>
								...
								{satisfiedData.map((f) => (
									...
									>
										...
										{f.label}
										{f.prefilledValue && (
											<span className="text-[10px] text-green-500 max-w-[80px] truncate">
												· {f.prefilledValue}
											</span>
										)}
									</span>
										...
										<div>
											...
											{v.upgradeAvailable === false && (
												<span className="block text-[10px] text-rose-500 mt-0.5">
													Requires actual video session (SEBI mandate)
												</span>
											)}
										</div>
									</div>
					...
					{!product.canProceed &&
						!product.kycIsExpired &&
						(missingVerifications.length > 0 || missingData.length > 0) && (
							<div className="pt-1">
								{/* Only show verifiable-via-KYC-wizard items (exclude manual-only like demat, video KYC) */}
								{missingVerifications.some(
									(v) => v.upgradeAvailable !== false,
								) ||
								missingData.some((f) =>
									["pan_api", "aadhaar_okyc", "ckyc", "user_input"].includes(
										f.source,
									),
								) ? (
									<Button
										...
										onClick={(e) => {
											e.stopPropagation();
											setLocation(
												`/profile?tab=kyc-dashboard&product=${product.productCode}`,
											);
										}}
									>
									...
									<p className="text-[11px] text-muted-foreground text-center py-1">
										Remaining steps require dedicated verification sessions
										(e.g., video KYC, demat setup).
									</p>
								)}
							</div>
						)}
					...
					{product.canProceed && (
						<div className="flex items-center gap-2 p-2.5 bg-green-50 dark:bg-green-950 rounded-lg border border-green-200 dark:border-green-800">
							...
							<p className="text-xs text-green-700 dark:text-green-300">
								Your KYC is complete for this product. No additional
								verification needed.
								{product.kycExpiresAt && (
									<span className="block text-[10px] mt-0.5 text-green-600">
										Valid until{" "}
										{new Date(product.kycExpiresAt).toLocaleDateString(
											"en-IN",
...
export function KYCProductAccessPanel() {
	...
	const filteredProducts =
		activeCategory === "All"
			? products
			: products.filter(
					(p) => PRODUCT_META[p.productCode]?.category === activeCategory,
				);
				...
				<div className="flex items-start justify-between">
					<div>
						...
						<CardDescription className="mt-1">
							Your KYC data is reused automatically. Only missing details are
				...
				{totalCount > 0 && readyCount < totalCount && (
					<div className="pt-2 border-t">
						<p className="text-xs text-muted-foreground text-center">
							Tap any product to see what's already verified and what else is
							needed. Your verified data is pre-filled — you only enter what's
							genuinely missing.
						</p>
					</div>
				)}
