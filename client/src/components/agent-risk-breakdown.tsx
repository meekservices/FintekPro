...
export function AgentRiskBreakdown({ clientId, clientPan }: Props) {
  const { data: assessmentResponse, isLoading, error } = useQuery<{ success: boolean; data: RiskAssessment }>({
    queryKey: ["/api/sebi-risk-profiling/assessment", clientId],
    enabled: !!clientId,
  });
  const assessment = assessmentResponse?.data;

  const { data: historyResponse } = useQuery<{ success: boolean; data: AssessmentHistory[] }>({
    queryKey: ["/api/sebi-risk-profiling/history", clientId],
    enabled: !!clientId,
  });
  const history = historyResponse?.data;

  const { data: eligibilityResponse } = useQuery<{ success: boolean; data: Array<{ productType: string; isEligible: boolean; reason: string }> }>({
    queryKey: ["/api/sebi-risk-profiling/eligibility/matrix", assessment?.profileCode],
      ...
      <Card>
        <CardContent className="py-8">
          <Alert>
            ...
            <AlertDescription>
              This client has not completed their risk assessment. Please guide them to complete the SEBI risk profiling questionnaire.
            </AlertDescription>
          </Alert>
        </CardContent>
      </Card>
  ...
  const tierConfig = RISK_TIER_CONFIG[assessment.profileCode] || RISK_TIER_CONFIG.RP3;
  const isExpired = new Date(assessment.expiresAt) < new Date();
  const daysUntilExpiry = Math.ceil((new Date(assessment.expiresAt).getTime() - Date.now()) / (1000 * 60 * 60 * 24));
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                PAN: {clientPan || assessment.pan} | Last assessed: {new Date(assessment.createdAt).toLocaleDateString()}
              </CardDescription>
            </div>
            <div className="text-right">
              <Badge className={`text-lg px-4 py-2 ${tierConfig.color}`} data-testid="badge-client-risk-tier">
                {assessment.profileCode} - {tierConfig.label}
              </Badge>
              {isExpired ? (
                <p className="text-xs text-red-500 mt-1">Profile expired - revalidation required</p>
              ) : daysUntilExpiry <= 30 ? (
                <p className="text-xs text-amber-500 mt-1">Expires in {daysUntilExpiry} days</p>
              ) : null}
            </div>
          </div>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="grid grid-cols-3 gap-4">
            <div className="p-4 bg-muted/50 rounded-lg text-center">
              <p className="text-2xl font-bold">{Math.round(assessment.rawScore)}</p>
              <p className="text-sm text-muted-foreground">Raw Score</p>
            </div>
            <div className="p-4 bg-muted/50 rounded-lg text-center">
              <p className="text-2xl font-bold">{Math.round(assessment.adjustedScore || assessment.rawScore)}</p>
              <p className="text-sm text-muted-foreground">Adjusted Score</p>
            </div>
            <div className="p-4 bg-muted/50 rounded-lg text-center">
              <p className="text-2xl font-bold">{assessment.categoryScores?.length || 0}</p>
              <p className="text-sm text-muted-foreground">Categories Assessed</p>
            </div>
          </div>
            ...
            <Alert className="border-orange-200 bg-orange-50 dark:bg-orange-900/20">
              ...
              <AlertDescription className="text-orange-700 dark:text-orange-300">
                ...
                {assessment.originalProfileCode && (
                  <p><strong>Original Profile:</strong> {assessment.originalProfileCode} → {assessment.profileCode}</p>
                )}
              </AlertDescription>
            </Alert>
          )}
                    ...
                    return (
                      <div key={cat.categoryCode} className="space-y-2">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="font-medium">{Math.round(cat.rawScore)}/100</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <Progress value={cat.rawScore} className="h-2 flex-1" />
              ...
              <AccordionContent>
                <div className="grid grid-cols-2 gap-2 pt-2">
                  {eligibility?.map((product) => (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <span className="font-medium text-sm">{product.productType}</span>
                        {product.isEligible ? (
            ...
            <AccordionItem value="history">
              ...
              <AccordionContent>
                {history && history.length > 0 ? (
                    ...
                    <TableBody>
                      {history.map((h) => (
                        <TableRow key={h.id}>
                          ...
                          <TableCell className="capitalize">{h.assessmentType}</TableCell>
                          <TableCell>
                            {h.hasOverride ? (
                  ...
                  <p className="text-sm text-muted-foreground py-4">No previous assessments</p>
                )}
              </AccordionContent>
            </AccordionItem>
                  ...
                  {assessment.answers?.map((answer, idx) => (
                    <div key={idx} className="flex items-center justify-between p-2 bg-muted/30 rounded text-sm">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-muted-foreground">Score: {answer.score}</span>
                      </div>
                    </div>
                  ))}
                </div>
