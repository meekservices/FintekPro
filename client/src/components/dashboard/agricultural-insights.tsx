...
export function AgriculturalInsights() {
	...
	const marketMetrics = useMemo(() => {
		...
		const totalVolume = filteredCommodities.reduce(
			(sum, c) => sum + c.volume,
			0,
		);
		const totalValue = filteredCommodities.reduce(
			(sum, c) => sum + c.ltp * c.volume,
			0,
		);
		const avgPrice =
			filteredCommodities.reduce((sum, c) => sum + c.ltp, 0) /
		...
		// Top performer
		const topGainer = filteredCommodities.reduce(
			(max, c) => (c.pchange > max.pchange ? c : max),
			filteredCommodities[0],
		);

		// Worst performer
		const topLoser = filteredCommodities.reduce(
			(min, c) => (c.pchange < min.pchange ? c : min),
			filteredCommodities[0],
		);
	...
	const categoryPerformance = useMemo(() => {
		...
		return Object.values(categoryStats).map((cat) => ({
			...
			avgChange:
				cat.commodities.reduce(
					(sum: number, c: NCDEXCommodity) => sum + c.pchange,
					0,
				) / cat.commodities.length,
		}));
	}, [commoditiesData]);
				...
				<div className="flex items-center space-x-3">
					...
					<div>
						...
						<p className="text-muted-foreground">
							Comprehensive NCDEX commodity analysis and trends
						</p>
					</div>
				</div>
				<Button
					...
					<Card>
						...
						<CardContent>
							<div className="flex items-center space-x-2">
								<Badge
									className={
										marketMetrics.marketSentiment === "Bullish"
								...
								<CardContent>
									<div className="flex justify-between items-center">
										<div>
											...
											<p className="text-sm text-muted-foreground">
												{marketMetrics.topGainer.category}
											</p>
											<p className="text-2xl font-bold">
												₹{marketMetrics.topGainer.ltp.toFixed(2)}
											</p>
										</div>
								...
								<CardContent>
									<div className="flex justify-between items-center">
										<div>
											...
											<p className="text-sm text-muted-foreground">
												{marketMetrics.topLoser.category}
											</p>
											<p className="text-2xl font-bold">
												₹{marketMetrics.topLoser.ltp.toFixed(2)}
											</p>
										</div>
										<div className="text-right">
										...
										>
											...
											<div className="text-right">
												<p className="font-medium">
													{(category.value / 1000).toFixed(1)}K MT
												</p>
												<p
													className={`text-sm ${category.avgChange >= 0 ? "text-green-600" : "text-red-600"}`}
												>
				...
				<TabsContent value="trends" className="space-y-4">
					<Card>
						<CardHeader>
							...
							<p className="text-sm text-muted-foreground">
								Historical price movements over the last 30 days
							</p>
						</CardHeader>
						<CardContent>
							<div className="h-96">
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
								<div>
									<h4 className="font-semibold mb-3">
										Peak Season Commodities
									</h4>
									<div className="space-y-3">
										{filteredCommodities
											.filter((c) => c.category === "Spices")
											.slice(0, 3)
											.map((commodity) => (
												<div
													key={commodity.symbol}
													className="p-3 bg-yellow-50 dark:bg-yellow-950/30 rounded-lg border-l-4 border-yellow-400"
												>
													<div className="flex justify-between">
														...
														<div className="text-right">
															<p className="font-bold">
																₹{commodity.ltp.toFixed(2)}
															</p>
															<p
																className={`text-sm ${commodity.change >= 0 ? "text-green-600" : "text-red-600"}`}
															>
																{commodity.change >= 0 ? "+" : ""}
																{commodity.pchange.toFixed(2)}%
															</p>
														</div>
													</div>
												</div>
								...
								<div>
									...
									<div className="space-y-3">
										{filteredCommodities
											.filter((c) => c.category === "Grains")
											.slice(0, 3)
											.map((commodity) => (
												<div
													key={commodity.symbol}
													className="p-3 bg-blue-50 dark:bg-blue-950/30 rounded-lg border-l-4 border-blue-400"
												>
													<div className="flex justify-between">
														<div>
															<p className="font-medium">{commodity.name}</p>
