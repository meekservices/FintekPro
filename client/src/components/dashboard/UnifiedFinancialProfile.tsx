...
export function UnifiedFinancialProfile() {
	...
	const netWorth = Number.parseFloat(profile.netWorth || "0");
	const totalAssets = Number.parseFloat(profile.totalAssets || "0");
	const totalLiabilities = Number.parseFloat(profile.totalLiabilities || "0");
	const creditUtilization = Number.parseFloat(profile.creditUtilization || "0");
	const riskScore = Number.parseFloat(profile.riskScore || "0");
	...
	return (
		<Card className="bg-gradient-to-r from-blue-900 via-indigo-900 to-purple-900 border-none text-white shadow-xl overflow-hidden relative">
			...
			<div className="absolute top-0 right-0 -mt-10 -mr-10 h-40 w-40 bg-white opacity-5 rounded-full blur-3xl" />
			<div className="absolute bottom-0 left-0 -mb-10 -ml-10 h-40 w-40 bg-blue-400 opacity-10 rounded-full blur-3xl" />
						...
						<div className="bg-white/5 backdrop-blur-sm rounded-xl p-5 border border-white/10 hover:bg-white/10 transition-all cursor-default">
							...
							<div className="w-full bg-white/10 h-1.5 rounded-full overflow-hidden">
								<div
									...
									style={{
										width: `${Math.min((totalLiabilities / (totalAssets || 1)) * 100, 100)}%`,
									}}
								/>
							</div>
						</div>
