									...
									<tbody>
										{commoditiesData.map((commodity) => (
											<tr
												...
												data-testid={`commodity-row-${commodity.symbol}`}
											>
												<td className="py-2">
													<div>
														<p className="font-medium">{commodity.name}</p>
														<p className="text-xs text-muted-foreground">
															{commodity.symbol} • {commodity.expiry}
														</p>
													</div>
												</td>
												<td className="py-2">
												...
												<td
													...
													data-testid={`price-${commodity.symbol}`}
												>
													{formatPrice(commodity.ltp)}
												</td>
												<td
													className={`py-2 text-right ${commodity.change >= 0 ? "text-green-600" : "text-red-600"}`}
												>
													{commodity.change >= 0 ? "+" : ""}
													{formatChange(commodity.change)}
												</td>
												<td
													className={`py-2 text-right ${commodity.pchange >= 0 ? "text-green-600" : "text-red-600"}`}
												>
													{commodity.pchange >= 0 ? "+" : ""}
													{formatPercentage(commodity.pchange)}
												</td>
							...
							<div className="space-y-3">
								{gainersData.map((commodity, index) => (
									<div
										...
										data-testid={`gainer-${commodity.symbol}`}
									>
							...
							<div className="space-y-3">
								{losersData.map((commodity, index) => (
									<div
										...
										data-testid={`loser-${commodity.symbol}`}
									>
										<div className="flex items-center space-x-3">
