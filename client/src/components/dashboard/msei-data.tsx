				...
				<Card data-testid="msei-market-status">
					...
					<CardContent>
						...
						<div className="mt-4 text-xs text-muted-foreground">
							...
							<p>
								Last Updated:{" "}
								{new Date(statusData.lastUpdated).toLocaleTimeString()}
							</p>
						</div>
					</CardContent>
				</Card>
									...
									<tbody>
										{equitiesData.map((equity) => (
											<tr
												...
												data-testid={`equity-row-${equity.symbol}`}
											>
												<td className="py-2">
													<div>
												...
												<td
													...
													data-testid={`price-${equity.symbol}`}
												>
													{formatPrice(equity.ltp)}
												</td>
												<td
													className={`py-2 text-right ${equity.change >= 0 ? "text-green-600" : "text-red-600"}`}
												>
													{equity.change >= 0 ? "+" : ""}
													{formatChange(equity.change)}
												</td>
												<td
													className={`py-2 text-right ${equity.pchange >= 0 ? "text-green-600" : "text-red-600"}`}
												>
													{equity.pchange >= 0 ? "+" : ""}
													{formatPercentage(equity.pchange)}
												</td>
							...
							<div className="space-y-4">
								{currenciesData.map((currency) => (
									<div
										...
										data-testid={`currency-${currency.symbol}`}
									>
										<div className="flex items-center justify-between">
											...
											<div className="text-right">
												...
												<p
													className={`text-sm ${currency.change >= 0 ? "text-green-600" : "text-red-600"}`}
												>
													{currency.change >= 0 ? "+" : ""}
													{formatChange(currency.change)} (
													{currency.pchange >= 0 ? "+" : ""}
													{formatPercentage(currency.pchange)})
												</p>
											</div>
										</div>
									...
									<tbody>
										{derivativesData.map((derivative) => (
											<tr
												...
												className="border-b hover:bg-muted"
												data-testid={`derivative-row-${derivative.symbol}`}
											>
												<td className="py-2">
													<div>
												...
												<td className="py-2 text-right text-muted-foreground">
													{derivative.strike
														? derivative.strike.toLocaleString()
														: "-"}
												</td>
												...
												<td
													className={`py-2 text-right ${derivative.change >= 0 ? "text-green-600" : "text-red-600"}`}
												>
													{derivative.change >= 0 ? "+" : ""}
													{formatChange(derivative.change)}
												</td>
												<td className="py-2 text-right text-muted-foreground">
									...
									>
										...
										<div className="text-right">
											...
											<p className="text-sm text-green-600">
												...
												{formatPercentage(stock.pchange)})
											</p>
										</div>
									</div>
								))}
