...
export function AssetAllocation({ portfolioId }: AssetAllocationProps) {
	...
	const riskProfiles = riskProfilesData?.success
		? riskProfilesData.data
	...
	const chartData = hasAllocation
		? allocation.map((item) => ({
				name:
					ASSET_TYPE_LABELS[item.assetType as keyof typeof ASSET_TYPE_LABELS] ||
					item.assetType,
				current: Number.parseFloat(item.currentPercentage || "0"),
				target: Number.parseFloat(item.targetPercentage || "0"),
				currentValue: Number.parseFloat(item.currentValue || "0"),
				targetValue: Number.parseFloat(item.targetValue || "0"),
				rebalanceAmount: Number.parseFloat(item.rebalanceAmount || "0"),
				color:
					ASSET_COLORS[item.assetType as keyof typeof ASSET_COLORS] ||
					"#8b5cf6",
			}))
		: [];
				...
				<CardContent>
					<div className="text-center py-12">
						...
						<p className="text-muted-foreground text-sm max-w-md mx-auto">
							Add investments to your portfolio to see your asset allocation
							breakdown and receive rebalancing suggestions.
						</p>
					</div>
				</CardContent>
			</Card>
				...
				<div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
					...
					<div>
						<h3
							...
							data-testid="current-target-title"
						>
							Current vs Target Allocation
						</h3>
								...
								>
									<div className="flex items-center space-x-3">
										...
										<span
											...
											data-testid={`asset-name-${item.name.toLowerCase()}`}
										>
											{item.name}
										</span>
									</div>
									<div className="flex items-center space-x-4">
										<div className="text-right">
											<p
												...
												data-testid={`current-percent-${item.name.toLowerCase()}`}
											>
												Current: {item.current}%
											</p>
											<p
												className="text-sm text-muted-foreground"
												data-testid={`target-percent-${item.name.toLowerCase()}`}
											>
												Target: {item.target}%
											</p>
										</div>
						...
						<div
							...
							data-testid="rebalance-suggestions"
						>
								...
								{chartData.map((item) => {
									...
									return (
										<p
											...
											data-testid={`suggestion-${item.name.toLowerCase()}`}
										>
											• {deviation > 0 ? "Reduce" : "Increase"} {item.name} by{" "}
					...
					<div>
						<h3
							className="font-bold text-foreground mb-4"
							data-testid="performance-chart-title"
						>
							Current Allocation
						</h3>
						<div
								...
								{riskProfiles.map((profile: any) => (
									<div
										...
										className={`text-center p-3 rounded-lg border-2 transition-all cursor-pointer ${
											selectedRiskProfile === (profile.id || profile.name)
												? "border-primary ring-1 ring-primary"
												: "border-transparent hover:border-border"
										}`}
										style={{
											backgroundColor: `${profile.color || "#3b82f6"}15`,
										}}
										onClick={() => {
											setSelectedRiskProfile(profile.id || profile.name);
											optimizeMutation.mutate({
												riskScore: profile.scoreRange
													? (profile.scoreRange[0] + profile.scoreRange[1]) / 2
													: 50,
												segment: "retail",
												investmentHorizon: 5,
												existingAllocations: Object.fromEntries(
													chartData.map((d) => [
														d.name.toLowerCase(),
														d.current,
													]),
									...
									>
										...
										<div className="text-[10px] space-y-0.5 text-muted-foreground">
											{profile.equity !== undefined && (
												<p>Equity: {profile.equity}%</p>
											)}
											{profile.debt !== undefined && (
							...
							{optimizeMutation.isPending && (
								<div className="mt-4 p-4 border rounded-lg animate-pulse bg-muted/50">
									<p className="text-sm text-center">
										Optimizing portfolio for {selectedRiskProfile} profile...
									</p>
								</div>
							)}
							...
							{optimizeMutation.data?.success && (
								<div className="mt-4 p-4 border border-green-200 bg-green-50 dark:bg-green-950/20 rounded-lg">
									...
									<div className="flex flex-wrap gap-4">
										{Object.entries(
											optimizeMutation.data.data.targetAllocation,
										).map(([asset, weight]: [string, any]) => (
									...
									<p className="text-xs text-green-700 dark:text-green-400 mt-2">
										{optimizeMutation.data.data.rationale ||
											"AI-optimized based on mean-variance analysis and historical risk-adjusted returns."}
									</p>
								</div>
							)}
						</div>
