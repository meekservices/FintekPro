...
const defaultYieldCurveData: YieldCurveData = {
	...
	historicalDate: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)
		.toISOString()
		.split("T")[0],
	data: [],
	summary: {
	...
	const { data: yieldData, isLoading } = useQuery<YieldCurveData>({
		...
		queryFn: async () => {
			const res = await fetch(
				`/api/bonds/yield-curve/public?timeRange=${timeRange}`,
			);
		...
		if (active && payload && payload.length) {
			...
			return (
				<div className="bg-card p-4 rounded-lg shadow-lg border border-border">
					<p className="font-semibold text-foreground mb-2">{data.benchmark}</p>
					<div className="space-y-1 text-sm">
						...
						{showHistorical && (
							<div className="flex justify-between gap-4">
								...
								<span className="font-medium text-muted-foreground">
									{(data.historicalYield ?? 0).toFixed(2)}%
								</span>
							</div>
						)}
						<div className="flex justify-between gap-4 pt-1 border-t border-border">
							...
							<span
								className={`font-medium flex items-center gap-1 ${(data.change ?? 0) >= 0 ? "text-green-600" : "text-red-600"}`}
							>
								{(data.change ?? 0) >= 0 ? (
			...
			<CardHeader className="pb-4">
				<div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
					...
					<div className="flex flex-wrap items-center gap-2">
						<Badge className={curveShapeColor}>{curveShapeLabel}</Badge>
				...
				<div className="flex flex-wrap items-center gap-2 mt-4">
					...
					<div className="ml-auto">
						<Button
							...
							className={
								showHistorical ? "text-blue-600" : "text-muted-foreground"
							}
				...
				<div className="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
					<div className="flex items-start gap-2">
						...
						<p className="text-sm text-blue-700 dark:text-blue-300">
							<strong>Yield Curve Analysis:</strong> A{" "}
							{chartData.summary.curveShape} yield curve indicates
							{chartData.summary.curveShape === "normal" &&
								" healthy economic conditions with higher returns for longer-term investments."}
							{chartData.summary.curveShape === "inverted" &&
								" potential economic slowdown with short-term yields exceeding long-term yields."}
							{chartData.summary.curveShape === "flat" &&
								" economic uncertainty with similar yields across maturities."}
						</p>
					</div>
				</div>
			</CardContent>
