...
export function PortfolioSuggestions({ portfolioId }: PortfolioSuggestionsProps) {
  const { data: suggestions, isLoading, error } = usePortfolioRebalancingSuggestions(portfolioId);
              ...
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  >
                    ...
                    <span className="ml-1">{suggestion.type.replace('_', ' ')}</span>
                  </Badge>
                </div>
              </div>
                  ...
                  <div className="space-y-1">
                    {suggestion.actions.map((action: any, index: number) => (
                      ...
                      >
                        ...
                        {action.assetType && (
                          <span className="ml-1">
                            {action.percentage && `${action.percentage.toFixed(1)}%`} in {action.assetType}
                          </span>
                        )}
                        {action.target && (
