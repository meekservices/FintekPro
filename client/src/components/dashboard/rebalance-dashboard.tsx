...
export function RebalanceDashboard({
	portfolioId,
	totalValue,
}: RebalanceDashboardProps) {
	...
	const getTotalPercentage = () => {
		return targetAllocations.reduce(
				...
				<div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
					...
					<div>
						<h3
							...
							data-testid="target-allocation-title"
						>
								...
								>
									<div
										...
										style={{
											backgroundColor:
												ASSET_COLORS[
													allocation.assetType as keyof typeof ASSET_COLORS
												],
										}}
									/>
									<Label className="w-20 text-sm font-medium">
										{
											ASSET_TYPE_LABELS[
												allocation.assetType as keyof typeof ASSET_TYPE_LABELS
											]
										}
									</Label>
									<Input
					...
					<div>
						<h3
							className="font-bold text-foreground mb-4"
							data-testid="rebalance-results-title"
						>
							Rebalance Analysis
						</h3>
						...
						{rebalanceResults ? (
							<div className="space-y-4" data-testid="rebalance-results">
								<div className="bg-blue-50 dark:bg-blue-950/30 p-4 rounded-lg">
									...
									<div className="space-y-3">
										{rebalanceResults.rebalanceCalculations?.map(
											(calc: any, index: number) => (
												...
												>
													<div className="flex items-center space-x-2">
														<div
															...
															style={{
																backgroundColor:
																	ASSET_COLORS[
																		calc.assetType as keyof typeof ASSET_COLORS
																	],
															}}
														/>
														<span className="font-medium">
															{
																ASSET_TYPE_LABELS[
																	calc.assetType as keyof typeof ASSET_TYPE_LABELS
																]
															}
														</span>
													</div>
													<div className="text-right">
														<div
															className={`font-bold ${calc.action === "BUY" ? "text-finance-green" : "text-finance-red"}`}
														>
															{calc.action} ₹
															{Math.abs(calc.rebalanceAmount).toLocaleString()}
														</div>
														<div className="text-xs text-muted-foreground">
															{calc.currentValue.toLocaleString()} →{" "}
															{calc.targetValue.toLocaleString()}
														</div>
													</div>
												</div>
											),
								...
								<div className="bg-green-50 dark:bg-green-950/30 p-4 rounded-lg">
									...
									<div className="text-sm text-muted-foreground">
										...
										<p>
											Actions required:{" "}
											{rebalanceResults.rebalanceCalculations?.filter(
												(c: any) => Math.abs(c.rebalanceAmount) > 1000,
											).length || 0}
										</p>
										<p className="text-finance-green font-medium mt-2">
											Your portfolio will be optimally balanced after these
											changes.
										</p>
									</div>
								</div>
							...
							>
								...
								<p className="text-muted-foreground">
									Set your target allocation and click calculate to see
