						...
						<Badge
							className={
								Array.isArray(marketStatus.marketState)
									? marketStatus.marketState.some(
											(m: any) => m.marketStatus === "Open",
										)
										? "bg-finance-green"
										: "bg-finance-red"
				...
				<CardContent>
					<Tabs defaultValue="indices" className="w-full">
						<ScrollableTabsList className="grid w-full grid-cols-3">
							<TabsTrigger value="indices" data-testid="indices-tab">
								Indices
							</TabsTrigger>
							<TabsTrigger value="gainers" data-testid="gainers-tab">
								Top Gainers
							</TabsTrigger>
							<TabsTrigger value="losers" data-testid="losers-tab">
								Top Losers
							</TabsTrigger>
						</ScrollableTabsList>
						...
						<TabsContent value="indices" className="space-y-4">
							<div className="grid gap-4">
								{indices.length > 0 ? (
									indices.slice(0, 8).map((index: NSEData, i: number) => (
										<div
											key={i}
											...
											<div className="text-right">
												...
												>
													...
													<span className="font-semibold">
														{index.chng ? index.chng.toFixed(2) : "0.00"} (
														{index.per_chng
						...
						<TabsContent value="gainers" className="space-y-4">
							<div className="grid gap-4">
								{gainers.length > 0 ? (
									gainers.slice(0, 10).map((stock: NSEData, i: number) => (
										<div
											key={i}
											className="flex items-center justify-between p-4 border rounded-lg"
										>
											...
											<div className="text-right">
												<div className="flex items-center text-finance-green">
													<TrendingUp className="h-4 w-4 mr-1" />
													<span className="font-semibold">
														+{stock.chng ? stock.chng.toFixed(2) : "0.00"} (+
														{stock.per_chng
						...
						<TabsContent value="losers" className="space-y-4">
							<div className="grid gap-4">
								{losers.length > 0 ? (
									losers.slice(0, 10).map((stock: NSEData, i: number) => (
										<div
											key={i}
											className="flex items-center justify-between p-4 border rounded-lg"
										>
