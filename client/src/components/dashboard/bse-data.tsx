                  ...
                  <div key={i} className="flex items-center justify-between p-4 border rounded-lg">
                    ...
                    <div className="text-right">
                      <div className={`flex items-center ${
                        (index.change || 0) >= 0 ? 'text-finance-green' : 'text-finance-red'
                      }`}>
                        {(index.change || 0) >= 0 ? <TrendingUp className="h-4 w-4 mr-1" /> : <TrendingDown className="h-4 w-4 mr-1" />}
                        <span className="font-semibold">
                          {index.change ? index.change.toFixed(2) : '0.00'} ({index.pchange ? index.pchange.toFixed(2) : '0.00'}%)
                        </span>
                      </div>
                      {index.volume && (
                  ...
                  <div key={i} className="flex items-center justify-between p-4 border rounded-lg">
                    ...
                    <div className="text-right">
                      <div className="flex items-center text-finance-green">
                        ...
                        <span className="font-semibold">
                          +{stock.change ? stock.change.toFixed(2) : '0.00'} (+{stock.pchange ? stock.pchange.toFixed(2) : '0.00'}%)
                        </span>
                      </div>
                      {stock.volume && (
              ...
              <div className="grid gap-4">
                {turnovers.length > 0 ? turnovers.slice(0, 10).map((stock: BSEData, i: number) => (
                  <div key={i} className="flex items-center justify-between p-4 border rounded-lg">
                    ...
                    <div className="text-right">
                      <div className={`flex items-center ${
                        (stock.change || 0) >= 0 ? 'text-finance-green' : 'text-finance-red'
                      }`}>
                        {(stock.change || 0) >= 0 ? <TrendingUp className="h-4 w-4 mr-1" /> : <TrendingDown className="h-4 w-4 mr-1" />}
                        <span className="font-semibold">
                          {stock.change ? stock.change.toFixed(2) : '0.00'} ({stock.pchange ? stock.pchange.toFixed(2) : '0.00'}%)
                        </span>
                      </div>
