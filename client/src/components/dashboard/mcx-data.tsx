		...
		<div className="space-y-6" data-testid="mcx-data">
					...
					<CardContent>
						<div className="space-y-2">
							...
							<div className="flex flex-wrap gap-2">
								{marketStatus.tradingSegments?.map(
			...
			<Card>
				<CardHeader className="flex flex-row items-center justify-between">
					...
					>
						<RefreshCw
							className={`h-4 w-4 mr-2 ${commoditiesLoading ? "animate-spin" : ""}`}
						/>
						Refresh
					</Button>
				</CardHeader>
				<CardContent>
					<Tabs defaultValue="commodities" className="w-full">
						<ScrollableTabsList className="grid w-full grid-cols-3">
							<TabsTrigger
								value="commodities"
								data-testid="mcx-commodities-tab"
							>
								All Commodities
							</TabsTrigger>
							<TabsTrigger value="gainers" data-testid="mcx-gainers-tab">
								Top Gainers
							</TabsTrigger>
							<TabsTrigger value="losers" data-testid="mcx-losers-tab">
								Top Losers
							</TabsTrigger>
						</ScrollableTabsList>
						...
						<TabsContent value="commodities" className="space-y-4">
							<div className="grid gap-4">
								{commodities.length > 0 ? (
									commodities.map((commodity: MCXData, i: number) => (
										<div
											key={i}
						...
						<TabsContent value="gainers" className="space-y-4">
							<div className="grid gap-4">
								{gainers.length > 0 ? (
									gainers.map((commodity: MCXData, i: number) => (
										<div
											key={i}
											className="flex items-center justify-between p-4 border rounded-lg"
										>
						...
						<TabsContent value="losers" className="space-y-4">
							<div className="grid gap-4">
								{losers.length > 0 ? (
									losers.map((commodity: MCXData, i: number) => (
										<div
											key={i}
											className="flex items-center justify-between p-4 border rounded-lg"
										>
