...
export function TrendingInvestments() {
  const { data, isLoading } = useQuery<{ success: boolean; investments: TrendingItem[] }>({
    ...
    <Card data-testid="trending-investments">
      ...
      <CardContent>
          ...
          {trending.slice(0, 5).map((item, index) => (
            ...
            >
              ...
              <div className="flex items-center gap-3">
                ...
                <Badge 
                  variant={item.category === 'top_gainers' ? 'default' : 'secondary'}
                  className="text-xs"
                >
                  {item.category === 'top_gainers' ? 'Top Gainer' : 
                   item.category === 'most_traded' ? 'Most Traded' : 'Popular'}
                </Badge>
              </div>
            </div>
          ))}
        ...
        <Link href="/domestic-trading">
          <Button variant="ghost" className="w-full mt-4" data-testid="view-all-trending">
            View All Trending
