          ...
          {actions.map((action) => (
            <Link key={action.name} href={action.href}>
              ...
              >
                ...
                <span className={`text-sm font-medium ${action.color}`}>{action.name}</span>
                <span className="text-xs text-muted-foreground mt-1 line-clamp-1">{action.description}</span>
              </Button>
            </Link>
          ))}
        </div>
