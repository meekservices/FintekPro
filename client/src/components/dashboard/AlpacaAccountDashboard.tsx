	...
	return (
		<Card>
			<CardContent className="p-5">
				...
				{sub && !loading && (
					<div
						className={`text-xs mt-1 font-medium ${positive === undefined ? "text-muted-foreground" : positive ? "text-green-600 dark:text-green-400" : "text-red-500"}`}
					>
						{sub}
					</div>
				)}
...
function MarketClockBadge({
	clock,
	loading,
}: { clock?: MarketClock; loading: boolean }) {
	if (loading) return <Skeleton className="h-6 w-24" />;
	if (!clock) return null;
	const nextEvent = clock.is_open
		? new Date(clock.next_close).toLocaleTimeString("en-US", {
				hour: "2-digit",
				minute: "2-digit",
			})
		: new Date(clock.next_open).toLocaleTimeString("en-US", {
				hour: "2-digit",
				minute: "2-digit",
			});
	...
	return (
		<div className="flex items-center gap-2">
			<div
				className={`h-2 w-2 rounded-full ${clock.is_open ? "bg-green-500 animate-pulse" : "bg-gray-400"}`}
			/>
...
function PortfolioChart({
	...
	const { data, isLoading } = useQuery<{
		...
		queryFn: () =>
			fetch(
				`/api/us-trading/alpaca/portfolio/history?period=${period}&timeframe=${PERIOD_TIMEFRAME[period]}`,
			).then((r) => r.json()),
		staleTime: 60000,
	});
	...
	const lastEquity =
		chartData.length > 0 ? chartData[chartData.length - 1].equity : 0;
					...
					<div>
						<CardTitle className="text-base flex items-center gap-2">
							...
							{isPaper && (
								<Badge
									variant="outline"
									className="text-xs ml-1 border-amber-400 text-amber-600 dark:text-amber-400"
								>
									Paper
								</Badge>
							)}
						...
						{!isLoading && chartData.length > 0 && (
							<p
								className={`text-sm font-semibold mt-0.5 ${isPositive ? "text-green-600 dark:text-green-400" : "text-red-500"}`}
							>
								{fmtUSD(lastEquity)} &nbsp;
								<span className="font-normal text-xs">
									{isPositive ? "▲" : "▼"}{" "}
									{fmtUSD(Math.abs(lastEquity - baseValue))} (
									{fmtPct(((lastEquity - baseValue) / baseValue) * 100)})
								</span>
							</p>
						)}
					</div>
...
function PositionsTable({ isPaper }: { isPaper: boolean }) {
	...
	const closeMutation = useMutation({
		mutationFn: (symbol: string) =>
			apiRequest(`/api/us-trading/alpaca/positions/${symbol}`, {
				method: "DELETE",
			}),
		onSuccess: () => {
			...
			toast({
				...
				description: "The position has been closed.",
			});
		},
		onError: (e: any) => {
						...
						<TableBody>
							{positions.map((p) => (
								<TableRow key={p.symbol}>
									...
									<TableCell
										className={`text-right ${p.gainLoss >= 0 ? "text-green-600 dark:text-green-400" : "text-red-500"}`}
									>
										{fmtUSD(p.gainLoss)}
									</TableCell>
									<TableCell
										className={`text-right ${p.gainLossPercent >= 0 ? "text-green-600 dark:text-green-400" : "text-red-500"}`}
									>
										...
										<AlertDialog>
											...
											<AlertDialogContent>
												<AlertDialogHeader>
													...
													<AlertDialogDescription>
														This will place a market sell order for{" "}
														{fmt(p.quantity)} shares of {p.symbol} at market
														price.
														{isPaper &&
...
function OrdersTable({
	isPaper,
	accountId,
}: { isPaper: boolean; accountId?: string }) {
	const { toast } = useToast();
	...
	const { data, isLoading, refetch } = useQuery<{
		...
		queryFn: () =>
			fetch(
				`/api/us-trading/alpaca/orders?status=${statusFilter}&limit=50`,
			).then((r) => r.json()),
		staleTime: 15000,
	});
	...
	const cancelOneMutation = useMutation({
		mutationFn: (orderId: string) =>
			apiRequest(`/api/us-trading/alpaca/orders/${orderId}`, {
				method: "DELETE",
			}),
		onSuccess: () => {
			queryClient.invalidateQueries({
				queryKey: ["/api/us-trading/alpaca/orders"],
			});
	...
	const replaceOrderMutation = useMutation({
		mutationFn: ({ orderId, updates }: { orderId: string; updates: any }) =>
			apiRequest(
				`/api/us-trading/broker/accounts/${accountId}/orders/${orderId}`,
				"PATCH",
				updates,
			),
		onSuccess: () => {
	...
	function handleModify() {
		...
		if (modifyLimitPrice && modifyLimitPrice !== modifyOrder.limit_price)
			updates.limit_price = modifyLimitPrice;
		if (modifyStopPrice && modifyStopPrice !== modifyOrder.stop_price)
			updates.stop_price = modifyStopPrice;
		replaceOrderMutation.mutate({ orderId: modifyOrder.id, updates });
	}

	const cancelAllMutation = useMutation({
		mutationFn: () =>
			apiRequest("/api/us-trading/alpaca/orders", { method: "DELETE" }),
		onSuccess: (res: any) => {
			queryClient.invalidateQueries({
				queryKey: ["/api/us-trading/alpaca/orders"],
			});
	...
	const openOrders = orders.filter((o) =>
		["new", "partially_filled", "pending_new", "accepted", "held"].includes(
			o.status,
		),
	);
							...
							<AlertDialog>
								<AlertDialogTrigger asChild>
									<Button
										variant="outline"
										size="sm"
										className="h-8 text-xs border-red-300 text-red-600 dark:text-red-400"
									>
								...
								<AlertDialogContent>
									<AlertDialogHeader>
										...
										<AlertDialogDescription>
											This will cancel all {openOrders.length} open order
							...
							{orders.map((o) => {
								...
								return (
									<TableRow key={o.id}>
										...
										<TableCell>
											<Badge
												className={`text-xs ${o.side === "buy" ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" : "bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400"}`}
												variant="outline"
											>
												{o.side.toUpperCase()}
											</Badge>
										...
										<TableCell className="text-right">
											{o.qty ?? o.notional ? o.qty ?? "$" + o.notional : "—"}
										</TableCell>
										<TableCell className="text-right">
											{o.filled_qty || "0"}
										</TableCell>
										<TableCell className="text-right">
											{o.filled_avg_price
												? fmtUSD(Number.parseFloat(o.filled_avg_price))
												: "—"}
										</TableCell>
										<TableCell>
											<span
												className={`text-xs px-1.5 py-0.5 rounded font-medium ${statusColor[o.status] ?? "bg-gray-100 text-gray-600"}`}
											>
												{o.status.replace("_", " ")}
											</span>
										</TableCell>
				...
				<DialogContent className="max-w-sm">
					...
					<div className="space-y-3 py-2">
						<p className="text-xs text-muted-foreground">
							Update the qty, limit price, or stop price. Leave fields unchanged
						...
						{["limit", "stop_limit"].includes(
							modifyOrder?.order_type || "",
						) && (
							<div className="space-y-1">
...
function WalletTab({ accountId }: { accountId: string }) {
	...
	const {
		...
		refetch: refetchAch,
	} = useQuery<{ relationships: AchRelationship[] }>({
		queryKey: [
			"/api/us-trading/broker/accounts",
			accountId,
			"ach-relationships",
		],
		queryFn: () =>
			fetch(
				`/api/us-trading/broker/accounts/${accountId}/ach-relationships`,
			).then((r) => r.json()),
		staleTime: 60000,
	});
	...
	} = useQuery<{ transfers: Transfer[] }>({
		queryKey: ["/api/us-trading/broker/accounts", accountId, "transfers"],
		queryFn: () =>
			fetch(`/api/us-trading/broker/accounts/${accountId}/transfers`).then(
				(r) => r.json(),
			),
		staleTime: 30000,
	});
	...
	const deleteAchMutation = useMutation({
		mutationFn: (relId: string) =>
			apiRequest(
				`/api/us-trading/broker/accounts/${accountId}/ach-relationships/${relId}`,
				{ method: "DELETE" },
			),
		onSuccess: () => {
			queryClient.invalidateQueries({
	...
	const verifyAchMutation = useMutation({
		mutationFn: ({
			...
			amount1,
			amount2,
		}: { achId: string; amount1: number; amount2: number }) =>
			apiRequest(
				`/api/us-trading/broker/accounts/${accountId}/ach/${achId}/verify`,
				"POST",
				{ amount1, amount2 },
			),
		onSuccess: () => {
			toast({
				title: "ACH Verified",
				description: "Bank account successfully verified via micro-deposits.",
			});
			setVerifyOpen(false);
			setVerifyAmt1("");
			setVerifyAmt2("");
	...
	const transferMutation = useMutation({
		...
		onSuccess: () => {
			...
			toast({
				...
				description: `$${amount} ${direction === "INCOMING" ? "deposit" : "withdrawal"} submitted.`,
			});
		...
		<div className="space-y-5">
								...
								{relationships.map((r) => (
									<TableRow key={r.id}>
										<TableCell className="font-medium">
											{r.nickname || r.account_owner_name}
										</TableCell>
										<TableCell className="font-mono text-xs">
											****{r.bank_account_number.slice(-4)}
										</TableCell>
										<TableCell className="font-mono text-xs">
											{r.bank_routing_number}
										</TableCell>
										...
										<TableCell>
											<span
												className={`text-xs px-1.5 py-0.5 rounded font-medium ${statusColors[r.status] ?? "bg-gray-100 text-gray-600"}`}
											>
												{r.status}
											</span>
										</TableCell>
										<TableCell>
											<div className="flex items-center gap-1">
												{(r.status === "PENDING" ||
													r.status === "PENDING_MICRO_DEPOSITS") && (
													<Button
														variant="ghost"
														size="sm"
			...
			<Dialog open={verifyOpen} onOpenChange={setVerifyOpen}>
				<DialogContent className="max-w-sm">
					...
					<div className="space-y-3 py-2">
						<p className="text-xs text-muted-foreground">
							Alpaca sent two small deposits to your bank account (usually
							$0.01–$0.99). Enter both amounts below to verify ownership.
						</p>
					...
					<DialogFooter>
						...
						<Button
							size="sm"
							onClick={() =>
								verifyAchMutation.mutate({
									achId: verifyRelId,
									amount1: Number.parseFloat(verifyAmt1),
									amount2: Number.parseFloat(verifyAmt2),
								})
							}
							disabled={
								!verifyAmt1 || !verifyAmt2 || verifyAchMutation.isPending
							}
						>
							{verifyAchMutation.isPending ? (
								<>
									<RefreshCw className="h-3.5 w-3.5 mr-1 animate-spin" />
									Verifying…
								</>
								...
								{transfers.map((t) => (
									<TableRow key={t.id}>
										...
										<TableCell className="text-right font-semibold">
											{fmtUSD(Number.parseFloat(t.amount))}
										</TableCell>
										<TableCell className="text-xs capitalize">
											{t.type}
										</TableCell>
										<TableCell>
											<span
												className={`text-xs px-1.5 py-0.5 rounded font-medium ${statusColors[t.status] ?? "bg-gray-100 text-gray-600"}`}
											>
												{t.status}
											</span>
										</TableCell>
...
function ActivitiesTab({ accountId }: { accountId: string }) {
	...
	const { data, isLoading, refetch } = useQuery<{
		...
		queryFn: () => {
			...
			return fetch(
				`/api/us-trading/broker/accounts/${accountId}/activities${params}`,
			).then((r) => r.json());
		},
		staleTime: 30000,
	});
	...
	const ACTIVITY_TYPES: Array<{
		...
		{ value: "FILL", label: "Fills", tooltip: "Order executions (buys/sells)" },
		{
			value: "DIV",
			label: "Dividends",
			tooltip: "Cash dividends for US residents",
		},
		{
			value: "DIVNRA",
			label: "Dividends (NRA)",
			tooltip: "25% withholding tax on dividends for non-US residents (DIVNRA)",
		},
		{
			value: "INT",
			label: "Interest",
			tooltip:
				"Cash interest earned on uninvested cash via the High-Yield program",
		},
		{
			value: "CSD",
			label: "Cash Deposits",
			tooltip: "ACH or wire transfers into the account",
		},
		{
			value: "CSW",
			label: "Cash Withdrawals",
			tooltip: "Cash withdrawn to external bank",
		},
		{
			value: "JNLC",
			label: "Journal Cash",
			tooltip: "Internal cash journal between Alpaca accounts",
		},
		{
			value: "JNLS",
			label: "Journal Securities",
			tooltip: "Internal securities journal between Alpaca accounts",
		},
		{
			value: "ACATC",
			label: "ACAT Cash",
			tooltip: "Automated Customer Account Transfer — cash component",
		},
		{
			value: "ACATS",
			label: "ACAT Securities",
			tooltip: "Automated Customer Account Transfer — securities component",
		},
		{
			value: "SSO",
			label: "Stock Settlement",
			tooltip: "Stock settlement out of account",
		},
		{
			value: "SSOI",
			label: "Stock Settlement In",
			tooltip: "Stock settlement into account",
		},
		{
			value: "REORG",
			label: "Reorganisation",
			tooltip: "Corporate reorganisation event (spin-off, merger, split)",
		},
		{
			value: "PTC",
			label: "Pass-through Charge",
			tooltip: "Exchange or regulatory pass-through fee",
		},
		{
			value: "NC",
			label: "Name Change",
			tooltip: "Security name/symbol change",
		},
	];
						...
						<TableBody>
							{activities.slice(0, 50).map((a) => (
								<TableRow key={a.id}>
									<TableCell>
										<span
											className={`text-xs px-1.5 py-0.5 rounded font-medium ${activityTypeColor[a.activity_type] ?? "bg-gray-100 text-gray-600"}`}
										>
											{a.activity_type}
										</span>
									</TableCell>
									<TableCell className="text-xs text-muted-foreground">
										{a.date ??
											(a.transaction_time
												? new Date(a.transaction_time).toLocaleDateString(
														"en-US",
									...
									<TableCell className="text-right text-xs">
										{a.price
											? fmtUSD(Number.parseFloat(a.price))
											: a.per_share_amount
												? fmtUSD(Number.parseFloat(a.per_share_amount))
												: "—"}
									</TableCell>
									<TableCell
										className={`text-right font-semibold text-sm ${Number.parseFloat(a.net_amount ?? "0") >= 0 ? "text-green-600 dark:text-green-400" : "text-red-500"}`}
									>
										{a.net_amount
											? fmtUSD(Number.parseFloat(a.net_amount))
											: "—"}
									</TableCell>
								</TableRow>
							))}
...
const REPORT_TYPES = [
	{
		...
		desc: "Monthly or quarterly account summary",
	},
	{
		value: "trade_confirmation",
		label: "Trade Confirmations",
		desc: "Executed order confirmations",
	},
	{
		value: "tax_1099b",
		label: "1099-B (Capital Gains)",
		desc: "For US tax reporting; informational for India ITR Schedule FA/FSI",
	},
	{
		value: "tax_1099div",
		label: "1099-DIV (Dividends)",
		desc: "Dividend income report incl. DIVNRA withholding",
	},
	{
		value: "tax_1099int",
		label: "1099-INT (Interest)",
		desc: "Cash interest earned (report in India ITR Schedule FSI)",
	},
	{
		value: "pnl",
		label: "Realized P&L",
		desc: "Realized gains/losses — use for India Capital Gains computation",
	},
	{
		value: "schedule_c_gain_loss",
		label: "P&L / Schedule C",
		desc: "Detailed gain/loss breakdown per security",
	},
];

function DocumentsTab({ accountId }: { accountId: string }) {
	const { toast } = useToast();
	const [docFilter, setDocFilter] = useState<"all" | "tax" | "kyc" | "trade">(
		"all",
	);
	...
	const { data, isLoading, refetch } = useQuery<{
		...
		queryFn: () =>
			fetch(`/api/us-trading/broker/accounts/${accountId}/documents`).then(
				(r) => r.json(),
			),
		staleTime: 120000,
	});
	...
	const downloadMutation = useMutation({
		mutationFn: (docId: string) =>
			fetch(
				`/api/us-trading/broker/accounts/${accountId}/documents/${docId}/download`,
			).then((r) => r.json()),
		onSuccess: (data: any) => {
			if (data?.url) window.open(data.url, "_blank");
				...
				toast({
					...
					description: "Document URL opened in new tab.",
				});
		},
		onError: (e: any) =>
			toast({
	...
	const generateMutation = useMutation({
		...
		onSuccess: () => {
			toast({
				...
				description: "The document will appear in your list shortly.",
			});
	...
	const docs =
		docFilter === "all"
			? allDocs
			: allDocs.filter(
					(d) => (DOC_CATEGORY[d.document_type] ?? "trade") === docFilter,
				);
	const taxDocs = allDocs.filter(
		(d) => DOC_CATEGORY[d.document_type] === "tax",
	);
	const kycDocs = allDocs.filter(
		(d) => DOC_CATEGORY[d.document_type] === "kyc",
	);
		...
		<div className="space-y-4">
			...
			<Card className="border-amber-200">
				<CardHeader className="pb-3">
					...
					<CardDescription className="text-xs">
						Generate tax documents for Indian ITR filing. Use P&L / 1099 data
						for Schedule FA, FSI, and Capital Gains schedules.
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-3">
					...
					<div>
						<p className="text-xs font-medium mb-2 flex items-center gap-1">
							<FilePlus className="h-3.5 w-3.5" /> Generate Report
						</p>
			...
			<Alert className="border-amber-200 bg-amber-50/50 py-2">
				...
				<AlertDescription className="text-xs text-amber-700">
					<strong>India Tax Filing Tip:</strong> US capital gains go in{" "}
					<strong>Schedule CG → FA</strong>. Dividend income (with DIVNRA
					withholding) goes in <strong>Schedule FSI</strong>. Use Form 67 before
					filing to claim Foreign Tax Credit. Maintain position records for
					Schedule FA (foreign assets) if portfolio value exceeds ₹5L at
					year-end.
				</AlertDescription>
			</Alert>
								...
								{docs.map((d) => (
									<TableRow key={d.id}>
										...
										<TableCell>
											<Badge
												className={
													DOC_CATEGORY[d.document_type] === "tax"
														? "bg-amber-100 text-amber-700 text-[10px]"
														: DOC_CATEGORY[d.document_type] === "kyc"
...
function LrsTrackerCard() {
	const { data, isLoading } = useQuery<{
		...
		queryFn: () =>
			fetch("/api/global-advisory/lrs/status").then((r) => r.json()),
		staleTime: 120_000,
	});

	const d = data?.data;
	const usedPct = d
		? Math.min((d.totalRemittedUsd / d.lrsLimitUsd) * 100, 100)
		: 0;
	...
	return (
		<Card className="border-indigo-200">
			<CardHeader className="pb-2">
				...
				<CardDescription className="text-xs">
					Liberalised Remittance Scheme — $250,000/FY cap
				</CardDescription>
			</CardHeader>
			<CardContent className="space-y-3">
				{isLoading ? (
						...
						<div className="flex items-end justify-between">
							<div>
								<div className="text-2xl font-bold">
									${(d?.totalRemittedUsd ?? 0).toLocaleString()}
								</div>
								<div className="text-xs text-muted-foreground">
									of $250,000 used ({usedPct.toFixed(1)}%)
								</div>
							</div>
							<div className="text-right">
								<div className="text-base font-semibold text-green-600">
									${(d?.remainingLimitUsd ?? 250000).toLocaleString()}
								</div>
						...
						<div className="grid grid-cols-2 gap-2 text-xs">
							<div className="bg-amber-50 dark:bg-amber-900/20 rounded p-2 border border-amber-200">
								...
								<div className="text-muted-foreground">
									₹7 Lakh ({d?.taxImplications?.tcsRate ?? 20}% TCS above)
								</div>
							</div>
							<div className="bg-blue-50 dark:bg-blue-900/20 rounded p-2 border border-blue-200">
								...
								<div className="text-muted-foreground">
									{d?.transactionCount ?? 0} remittances this FY
								</div>
							</div>
						</div>
		...
		<div className="max-w-xl mx-auto space-y-5">
			<div className="text-center mb-2">
				...
				<p className="text-sm text-muted-foreground mt-1">
					Enter your Alpaca API credentials to enable live account data,
			...
			<Card>
				<CardContent className="p-6 space-y-5">
					...
					<div className="space-y-1.5">
						...
						<p className="text-xs text-muted-foreground">
							Broker Sandbox:{" "}
							<code className="bg-muted px-1 rounded">{DEFAULT_BASE_URL}</code>
						</p>
					</div>
					...
					<div className="space-y-1.5">
						...
						/>
						<p className="text-xs text-muted-foreground">
							Broker API keys start with{" "}
							<code className="bg-muted px-1 rounded">CK</code> (e.g.{" "}
							<code className="bg-muted px-1 rounded">CK7IO…</code>)
						</p>
					</div>
					...
					<Alert className="border-blue-200 bg-blue-50 dark:bg-blue-950/20 dark:border-blue-800 py-3">
						...
						<AlertDescription className="text-xs text-blue-700 dark:text-blue-300 space-y-1">
							<p>
								<strong>Broker API</strong> credentials (keys starting with{" "}
						...
						<Alert className="border-red-200 bg-red-50 dark:bg-red-950/20 dark:border-red-800 py-3">
							...
							<AlertDescription className="text-xs text-red-700 dark:text-red-300">
								...
								{(saveMutation.error as any)?.message ??
									"Check that your Broker API credentials are correct and the sandbox account is active."}{" "}
								<a
...
export default function AlpacaAccountDashboard() {
	...
	const equity = Number.parseFloat(account?.equity ?? "0");
	const cash = Number.parseFloat(account?.cash ?? "0");
	const buyingPower = Number.parseFloat(account?.buying_power ?? "0");
	const unrealizedPL = Number.parseFloat(account?.unrealized_pl ?? "0");
	const unrealizedPLPC = Number.parseFloat(account?.unrealized_plpc ?? "0");
	const longMarketValue = Number.parseFloat(account?.long_market_value ?? "0");
		...
		<div className="space-y-5">
							...
							<div className="flex items-center gap-4">
								...
								<div>
									<h3 className="font-semibold text-base">
										Open Your US Brokerage Account
									</h3>
									<p className="text-sm text-muted-foreground mt-0.5">
										Trade US stocks directly via our Alpaca FINRA/SEC licensed
										broker-dealer. Complete the 5-step application — takes under
										3 minutes.
									</p>
									{prefillData?.compliance &&
										!prefillData.compliance.eligible && (
											<p className="text-xs text-red-500 mt-1">
												⚠{" "}
												{prefillData.compliance.blockers?.[0] ||
				...
				<Card className="border-dashed border-2 border-blue-200 dark:border-blue-800 bg-gradient-to-br from-blue-50/50 to-purple-50/50 dark:from-blue-950/20 dark:to-purple-950/20">
					<CardContent className="p-6 text-center">
						...
						<h3 className="font-semibold text-lg mb-1">
							US Trading — Pro & Elite Feature
						</h3>
						<p className="text-sm text-muted-foreground mb-4 max-w-sm mx-auto">
							Direct US stock trading via Alpaca's FINRA/SEC licensed
							infrastructure is available on Pro (₹999/mo) and Elite (₹25K/yr)
							plans. Upgrade to unlock unlimited trades at 0.5% FX spread.
						</p>
				...
				brokerAccount.alpacaStatus !== "ACTIVE" && (
					...
					>
						...
						<span>
							...
							<strong>{brokerAccount.alpacaStatus?.replace(/_/g, " ")}</strong>
							{brokerAccount.alpacaStatus === "SUBMITTED" &&
								" — Alpaca is reviewing your application (1-2 business days)."}
							{brokerAccount.alpacaStatus === "ACTION_REQUIRED" &&
								" — Additional documents needed. Check your email."}
						</span>
						<Button
							variant="link"
							size="sm"
			...
			<div className="grid grid-cols-2 md:grid-cols-4 gap-3">
				<StatCard
					...
					sub={
						account
							? `${account.currency} · ${isPaper ? "Paper" : "Live"}`
							: undefined
					}
					icon={Wallet}
				...
				<div className="grid grid-cols-3 gap-3">
					...
					<Card>
						<CardContent className="p-4">
							...
							<div
								className={`font-semibold ${Number.parseFloat(account.realized_pl) >= 0 ? "text-green-600 dark:text-green-400" : "text-red-500"}`}
							>
								{fmtUSD(Number.parseFloat(account.realized_pl))}
							</div>
						</CardContent>
					</Card>
					<Card>
			...
			<Tabs defaultValue="overview" className="mt-2">
				<TabsList className="mb-4 flex-wrap h-auto gap-y-1">
					...
					<TabsTrigger value="tax-lots" className="flex items-center gap-1.5">
