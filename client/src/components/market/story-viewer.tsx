...
export function StoryViewer({ story, onRefresh, isRefreshing = false }: StoryViewerProps) {
  ...
  const getConfidenceLevel = () => {
    if (story.confidence >= 0.8) return { label: 'High', color: 'text-green-600' };
    if (story.confidence >= 0.6) return { label: 'Medium', color: 'text-yellow-600' };
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm font-medium text-blue-600">AI-Generated Market Story</span>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span className={confidenceInfo.color}>
                    {confidenceInfo.label} Confidence ({Math.round(story.confidence * 100)}%)
                  </span>
                </div>
              </div>
            </div>
