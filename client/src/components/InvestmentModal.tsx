			...
			try {
				...
				await addToUnifiedCart({
					mutualFundSchemeCode:
						variables.investmentId?.toString() ||
				...
				toast({
					...
					description:
						"Added to cart but unified tracking failed. Please try again.",
					variant: "default",
				});
			}
	...
	const handleAddToCart = () => {
		...
		const cartItem = {
			...
			metadata: {
				...
				duration:
					investmentType === "SIP" ? Number.parseInt(duration) : undefined,
				fundHouse: fund.fundHouse,
				category: fund.category,
				nav: fund.nav,
				schemeCode: fund.schemeCode,
				...
				<div className="space-y-6">
						...
						<div className="flex items-start gap-3">
							...
							<div className="flex-1">
								<h3
									...
									data-testid="modal-fund-name"
								>
									{fund.schemeName}
								</h3>
								<p
									className="text-sm text-muted-foreground"
									data-testid="modal-fund-house"
								>
									{fund.fundHouse}
								</p>
					...
					<div className="space-y-3">
						...
						<RadioGroup
							...
							onValueChange={(value) =>
								setInvestmentType(value as "SIP" | "LUMPSUM")
							}
									...
									<>
										...
										<div className="flex justify-between border-t border-green-300 dark:border-green-700 pt-2 mt-2">
											...
											<span className="font-bold text-green-600 dark:text-green-400">
												₹
												{(
													Number.parseFloat(amount) * Number.parseInt(duration)
												).toLocaleString()}
											</span>
										</div>
									</>
