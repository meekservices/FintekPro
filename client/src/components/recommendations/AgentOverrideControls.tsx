		...
		queryFn: async () => {
			...
			if (!response.ok) {
				if (response.status === 401)
					return { success: true, clientId, overrides: [], count: 0 };
	...
	const createOverrideMutation = useMutation({
		...
		onSuccess: () => {
			toast({
				...
				description:
					"The override has been recorded and will apply to future recommendations.",
			});
	...
	const handleSubmit = () => {
		if (!overrideType || !overrideReason || overrideReason.length < 10) {
			toast({
				...
				description:
					"Please select an override type and provide a reason (min 10 characters).",
				variant: "destructive",
			});
			return;
		}
				...
				<CardHeader className="pb-2">
					<CollapsibleTrigger asChild>
						<div
							...
							data-testid="override-controls-header"
						>
					...
					<CardContent className="space-y-4">
						<Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
							...
							<DialogContent>
								<DialogHeader>
									...
									<DialogDescription>
										Override the default recommendation behavior for this
										client. All overrides are logged for compliance purposes.
									</DialogDescription>
								</DialogHeader>
								...
								<div className="space-y-4 py-4">
									...
									{overrideType === "asset_class_lock" && (
										<div className="space-y-2">
											<Label htmlFor="assetClasses">
												Asset Classes to Lock
											</Label>
											<Input
												id="assetClasses"
											...
											<p className="text-xs text-muted-foreground">
												These asset classes will be excluded from
												recommendations
											</p>
										</div>
									)}
						...
						<Alert className="bg-amber-50 border-amber-200 dark:bg-amber-950/20 dark:border-amber-800">
							...
							<AlertDescription className="text-amber-800 dark:text-amber-200 text-sm">
								All overrides are immutably logged for compliance and audit
								purposes. Overrides cannot be deleted once applied.
							</AlertDescription>
						</Alert>
					</CardContent>
				</CollapsibleContent>
