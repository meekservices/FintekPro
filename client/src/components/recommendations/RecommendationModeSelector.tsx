...
export function RecommendationModeSelector({
	...
	const getWeightingDisplay = (weightings: {
		suitability: number;
		upside: number;
	}) => {
		return (
			...
			<CardContent>
				{data.killSwitchStatus.active && (
					...
					>
						...
						<AlertDescription>
							Growth-Optimized mode is temporarily disabled:{" "}
							{data.killSwitchStatus.reason}
						</AlertDescription>
					</Alert>
				)}
				...
				>
                    ...
                    `}
										onClick={() =>
											mode.available && !disabled && onModeChange(mode.id)
										}
										...
										<div className="flex-1 space-y-1">
											<div className="flex items-center gap-2">
												...
												{mode.requiresDisclosure && (
													<Badge
														variant="outline"
														className="text-xs text-orange-600 border-orange-300 dark:border-orange-700"
													>
														Requires Disclosure
													</Badge>
												)}
				...
				{selectedMode === "growth_optimized" && (
					<Alert
						className="mt-4 border-orange-200 bg-orange-50 dark:bg-orange-950/20"
						data-testid="growth-mode-warning"
					>
						<AlertTriangle className="h-4 w-4 text-orange-500" />
						<AlertDescription className="text-orange-800 dark:text-orange-200">
							Growth-Optimized mode emphasizes upside potential within the
							client's risk profile. A disclosure will be shown to the client
