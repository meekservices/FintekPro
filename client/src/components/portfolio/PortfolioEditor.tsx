...
const WORKFLOW_STEPS = [
	{
		id: 1,
		name: "Enter Portfolio",
		description: "Add or import your holdings",
		icon: FileSpreadsheet,
	},
	{
		id: 2,
		name: "Validate",
		description: "Check for errors and issues",
		icon: FileCheck,
	},
	{
		id: 3,
...
function detectBrokerFormat(
	headers: string[],
): "zerodha" | "upstox" | "generic" | null {
	const normalizedHeaders = headers.map((h) => h.toLowerCase().trim());

	if (
		normalizedHeaders.includes("tradingsymbol") &&
...
function mapCSVRowToHolding(
	...
	if (format === "zerodha") {
		...
		quantity = Number.parseFloat(getField("quantity")) || 0;
		buyPrice = Number.parseFloat(getField("average_price")) || 0;
		currentPrice = Number.parseFloat(getField("last_price")) || buyPrice;
	} else if (format === "upstox") {
		isin = getField("isin");
		symbol = getField("symbol");
		quantity = Number.parseFloat(getField("quantity")) || 0;
		buyPrice = Number.parseFloat(getField("buy_avg")) || 0;
		currentPrice = Number.parseFloat(getField("ltp")) || buyPrice;
	} else {
		isin = getField("isin");
		symbol = getField("symbol") || getField("name");
		quantity = Number.parseFloat(getField("quantity") || getField("qty")) || 0;
		buyPrice =
			Number.parseFloat(
				getField("buy_price") || getField("avg_price") || getField("price"),
			) || 0;
		currentPrice =
			Number.parseFloat(getField("current_price") || getField("ltp")) ||
			buyPrice;
	}
...
export function PortfolioEditor({
	...
	const handleCSVPreview = useCallback(() => {
		...
		if (!format) {
			toast({
				...
				description:
					"CSV must contain an ISIN column. Supported formats: Zerodha, Upstox, or generic with ISIN column.",
				variant: "destructive",
			});
			return;
		}
		...
		if (parsed.length === 0) {
			toast({
				...
				description:
					"Check that your CSV has valid ISIN codes and quantity values.",
				variant: "destructive",
			});
		}
	}, [csvText, toast]);
			...
			for (const partial of importPreview) {
				...
				try {
					const res = await fetch(
						`/api/instruments/search?q=${encodeURIComponent(partial.isin)}&limit=1`,
					);
				...
				const investmentValue =
					(partial.quantity || 0) * (partial.buyPrice || 0);
				...
				newHoldings.push({
					...
					securityName:
						instrument?.name || partial.securityName || partial.isin,
	...
	const fetchAIInsights = useCallback(async () => {
		...
		try {
			...
			setAiInsights(
				res.insights || "Unable to generate insights at this time.",
			);
		} catch (error: any) {
	...
	const { data: searchResults, isLoading: isSearching } = useQuery({
		...
		queryFn: async () => {
			...
			const res = await fetch(
				`/api/instruments/search?q=${encodeURIComponent(searchQuery)}&limit=15`,
			);
			return res.json();
		},
			...
			setHoldings((prev) =>
				prev.map((h) => {
					...
					if (
						...
						const buyPx =
							field === "buyPrice" ? Number(value) : updated.buyPrice;
						const curPx =
							field === "currentPrice" ? Number(value) : updated.currentPrice;
						...
						updated.unrealizedGainLossPercent =
							investmentValue > 0
								? ((updated.currentValue - investmentValue) / investmentValue) *
									100
								: 0;
					}
	...
	const validateHoldings = useCallback((): string[] => {
		...
		holdings.forEach((h, i) => {
			...
			if (h.assetClass === "pms" && h.currentValue < 5000000) {
				errors.push(
					`Row ${rowNum}: PMS typically requires minimum ₹50 Lakhs investment`,
				);
			}

			if (h.assetClass === "aif" && h.currentValue < 10000000) {
				errors.push(
					`Row ${rowNum}: AIF typically requires minimum ₹1 Crore investment`,
				);
			}
		});
		...
		try {
			if (proposalId) {
				...
				queryClient.invalidateQueries({
					queryKey: ["/api/proposals", proposalId, "holdings"],
				});
			}
							...
							return (
								<div key={step.id} className="flex items-center">
									<div className="flex flex-col items-center">
										...
										<span
											className={`text-xs mt-2 font-medium ${isActive ? "text-indigo-600" : "text-muted-foreground"}`}
										>
											{step.name}
										</span>
									...
									{index < WORKFLOW_STEPS.length - 1 && (
										<div
											className={`h-0.5 w-16 mx-2 hidden sm:block ${
							...
							<Dialog open={showImportModal} onOpenChange={setShowImportModal}>
								...
								<DialogContent className="max-w-2xl">
									<DialogHeader>
										...
										<DialogDescription>
											Paste your broker's holdings CSV data. Supports Zerodha,
											Upstox, or any CSV with ISIN column.
										</DialogDescription>
									</DialogHeader>
										...
										{importFormat && (
											<div className="flex items-center gap-2">
												<Badge variant="outline">
													{importFormat.toUpperCase()} format detected
												</Badge>
											</div>
										)}
										...
										{aiInsights.split("\n").map((line, i) => {
											if (line.startsWith("## ")) {
												return (
													<h2
														key={i}
														className="text-lg font-bold mt-2 mb-3 text-purple-800 dark:text-purple-300"
													>
														{line.replace("## ", "")}
													</h2>
												);
											...
											if (line.startsWith("### ")) {
												return (
													<h3
														key={i}
														className="text-md font-semibold mt-4 mb-2 text-indigo-700 dark:text-indigo-400"
													>
														{line.replace("### ", "")}
													</h3>
												);
											...
											<TableBody>
												{filteredHoldings.length === 0 ? (
													<TableRow>
														...
														>
															No holdings added yet. Click "Add Holding" to
															start.
														</TableCell>
													</TableRow>
												) : (
														...
														>
																		...
																		<Command>
																			...
																			<CommandList>
																				...
																				{!isSearching &&
																					...
																					searchResults?.instruments?.length ===
																						0 && (
																						<CommandEmpty>
																							<div className="py-2 px-3 space-y-3">
																								<p className="text-sm text-muted-foreground text-center">
																									No instruments found for "
																									{searchQuery}"
																								</p>
																								<div className="text-xs text-muted-foreground text-center">
																									Add manually for stocks, PMS,
																									AIF, MLDs not in database
																								</div>
																										...
																										onClick={() => {
																											...
																											const userIsin =
																												isinInput?.value
																													...
																													.toUpperCase();
																											const generatedId =
																												`MAN${Math.random().toString(36).substr(2, 9).toUpperCase()}`.substring(
																													0,
																													12,
																												);
																											updateRow(
															...
															<TableCell>
																<Select
																	...
																	onValueChange={(v) =>
																		updateRow(holding.id, "assetClass", v)
																	}
																	disabled={readOnly}
																>
																	<SelectTrigger
																		className="h-8 text-xs"
																		data-testid={`select-asset-class-${holding.id}`}
																	>
																		<SelectValue />
																	</SelectTrigger>
																	<SelectContent>
