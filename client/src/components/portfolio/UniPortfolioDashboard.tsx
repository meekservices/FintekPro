...
export function UniPortfolioDashboard({
	...
	const [activeTab, setActiveTab] = useState<
		"overview" | "drift" | "rebalance" | "holdings"
	>("overview");
	...
	return (
		<div style={styles.container}>
			...
			<div style={styles.header}>
				<div>
					<div style={styles.portfolioId}>
						🔗 Portfolio ID:{" "}
						<code style={styles.code}>{portfolio.portfolioId}</code>
					</div>
					<div style={styles.netWorth}>
						{fmt(portfolio.summary.totalValueInr)}
					</div>
					<div
						style={{
							fontSize: 14,
							color:
								portfolio.summary.unrealizedPnlInr >= 0 ? "#10b981" : "#ef4444",
							marginTop: 4,
						}}
					>
			...
			{hasDrift && (
				<div
					style={{ ...styles.alertBanner, borderColor: urgencyColor(urgency) }}
				>
					...
					<span style={{ marginLeft: 12, color: "#94a3b8", fontSize: 13 }}>
						Max drift: {portfolio.analysis.drift.largest_drift.toFixed(1)}% ·{" "}
						{portfolio.analysis.rebalancing.summary.numberOfTrades} trades
						recommended
					</span>
					<button
				...
				{portfolio.summary.brokerBreakdown.map((b) => (
					<div key={b.brokerId} style={styles.brokerBadge}>
						<div
							style={{
								...styles.brokerDot,
								background: BROKER_COLORS[b.brokerId] ?? "#94a3b8",
							}}
						/>
						<div>
							<div style={styles.brokerName}>{b.brokerId}</div>
							<div style={styles.brokerMeta}>
								{b.count} holdings · {fmt(b.valueInr)}
							</div>
						</div>
					</div>
				))}
			...
			<div style={styles.tabBar}>
				{(["overview", "drift", "rebalance", "holdings"] as const).map(
					(tab) => (
						<button
							key={tab}
						...
						<div key={i} style={styles.concRow}>
							<div style={styles.concLeft}>
								<div
									style={{
										...styles.brokerDot,
										background: BROKER_COLORS[c.brokerId] ?? "#94a3b8",
									}}
								/>
								<div>
			...
			<div style={styles.cardTitle}>
				...
				<span
					style={{
						marginLeft: 12,
						fontSize: 12,
						color: drift.has_drifted ? "#ef4444" : "#10b981",
					}}
				>
				...
				>
					...
					<div
						style={{
							...
							padding: "4px 12px",
							borderRadius: 20,
							background: urgencyColor(r.urgency) + "22",
		...
		<div style={styles.card}>
			...
			<div style={{ overflowX: "auto" }}>
				<table style={styles.table}>
					<thead>
						<tr>
							{["Name", "Broker", "Type", "Qty", "Value (INR)", "P&L"].map(
								(h) => (
							...
							>
								...
								<td style={styles.td}>
									<span
										style={{
											...styles.brokerPill,
											background:
												(BROKER_COLORS[h.brokerId] ?? "#64748b") + "22",
											color: BROKER_COLORS[h.brokerId] ?? "#64748b",
										}}
									>
										{h.brokerId}
									</span>
								...
								<td
									style={{
										...styles.td,
										color:
											(h.unrealizedPnlInr ?? 0) >= 0 ? "#10b981" : "#ef4444",
									}}
								>
									{h.unrealizedPnlInr != null
										? `${h.unrealizedPnlInr >= 0 ? "+" : ""}${fmt(h.unrealizedPnlInr)}`
										: "—"}
								</td>
							</tr>
						))}
...
function CountryBar({
	country,
}: { country: { IN: number; US: number; OTHER: number } }) {
	const bars = [
...
function TradeCard({ trade }: { trade: RebalanceTrade }) {
	...
	return (
		<div style={styles.tradeCard}>
			...
			>
				<span>
					Δ {trade.allocationDiff > 0 ? "+" : ""}
					{trade.allocationDiff.toFixed(1)}%
				</span>
				<span>
					Tax:{" "}
					{trade.taxImpact.taxEfficiency === "high"
						? "🟢"
						: trade.taxImpact.taxEfficiency === "medium"
							? "🟡"
							: "🔴"}{" "}
					{fmt(trade.taxImpact.estimatedTax)}
				</span>
	...
	return (
		...
		>
			...
			<div style={{ color: "#94a3b8", marginTop: 16 }}>
				Loading portfolio from all brokers…
			</div>
		</div>
	);
}
...
function ErrorState({ onRetry }: { onRetry: () => void }) {
	return (
		<div style={{ ...styles.container, textAlign: "center", padding: 40 }}>
			...
			<div style={{ color: "#ef4444", fontSize: 16, marginTop: 12 }}>
				Failed to load portfolio
			</div>
			<button onClick={onRetry} style={{ ...styles.refreshBtn, marginTop: 16 }}>
				Try Again
			</button>
		</div>
	);
