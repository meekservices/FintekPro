...
export function PortfolioConfetti({ portfolioId, enabled = true }: PortfolioConfettiProps) {
  ...
  const triggerAdvancedConfetti = useCallback((type: string, milestone: string) => {
    const duration = 3000;
  ...
  useEffect(() => {
    ...
    if (previous) {
      ...
      const profitMilestones = [100000, 500000, 1000000, 2500000, 5000000, 10000000, 25000000, 50000000, 100000000];
      ...
      for (const milestoneAmount of profitMilestones) {
        if (currentReturns >= milestoneAmount && previousReturns < milestoneAmount) {
      ...
      if (!shouldCelebrate) {
        ...
        for (const percentMilestone of percentageMilestones) {
          if (currentPercent >= percentMilestone && previousPercent < percentMilestone) {
            shouldCelebrate = true;
      ...
      if (!shouldCelebrate && performance.totalValue > (previous.totalValue || 0)) {
      ...
      if (!shouldCelebrate && performance.todaysGain > 0) {
        const significantDailyGain = performance.todaysGain >= 10000 || performance.todaysGainPercentage >= 2;
        
        if (significantDailyGain && performance.todaysGain > (previous.todaysGain || 0)) {
          shouldCelebrate = true;
...
export const triggerCelebrationConfetti = (type: "profit" | "percentage" | "milestone" = "profit") => {
  const duration = 2000;
  const defaults = { startVelocity: 30, spread: 360, ticks: 60, zIndex: 0 };
