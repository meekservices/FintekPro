...
export function PortfolioHero({
	...
	if (isLoading) {
		return (
			<Card
				className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-foreground border-0 shadow-2xl"
	...
	return (
		<Card
			className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-foreground border-0 shadow-2xl overflow-hidden relative"
				...
				<div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
						...
						<div className="flex flex-wrap items-center gap-4">
							...
							>
								...
								<span
									className={cn(
										"text-sm",
										isDayPositive ? "text-green-400/80" : "text-red-400/80",
									)}
								>
									({isDayPositive ? "+" : ""}
							...
							>
								...
								<span
									className={cn(
										"font-semibold",
										isTotalPositive ? "text-green-400" : "text-red-400",
									)}
								>
									{isValueHidden
										? "••••"
										: `${isTotalPositive ? "+" : ""}₹${Math.abs(totalGain).toLocaleString("en-IN")}`}
								</span>
								<span
									className={cn(
										"text-sm",
										isTotalPositive ? "text-green-400/80" : "text-red-400/80",
									)}
								>
									({isTotalPositive ? "+" : ""}
					...
					<div className="grid grid-cols-2 gap-4 lg:gap-6">
						<div
							...
							data-testid="invested-value-card"
						>
						...
						<div
							className="bg-card/50 backdrop-blur-sm rounded-xl p-4 border border-border/50"
							data-testid="current-value-card"
						>
							<p className="text-muted-foreground text-xs font-medium mb-1">
								Current Value
							</p>
						...
						<div
							className="bg-card/50 backdrop-blur-sm rounded-xl p-4 border border-border/50"
							data-testid="holdings-count-card"
						>
							<p className="text-muted-foreground text-xs font-medium mb-1">
								Holdings
							</p>
