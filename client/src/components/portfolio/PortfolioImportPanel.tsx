...
export function PortfolioImportPanel({
	...
	const [mode, setMode] = useState<"choose" | "smart" | "manual" | "iris">(
		"choose",
	);
		...
		onSuccess: (data) => {
			if (!data.holdings || data.holdings.length === 0) {
				toast({
					...
					description:
						"IRIS returned zero holdings for your PAN. Try manual upload instead.",
					variant: "destructive",
				});
				return;
			}
	...
	const isLoading =
		smartImport.isPending || saveHoldings.isPending || irisFetch.isPending;

	const handleParseSuccess = useCallback(
		(result: ImportResult, source: ImportSource) => {
			if (result.success && result.holdings?.length > 0) {
				...
				toast({
					...
					description:
						result.errors?.[0] ||
						"Could not extract any holdings from the file.",
					variant: "destructive",
				});
			}
		},
	...
	const handleSaveHoldings = useCallback(async () => {
		...
		const totalValue = previewHoldings.reduce((sum, h) => {
			return sum + (Number.parseFloat(h.currentValue as string) || 0);
		}, 0);
			...
			},
			{
				onSuccess: (result) => {
					toast({
						...
						description: `Successfully saved ${result.savedCount} holdings.`,
					});
					...
					if (prospectId) {
						recordHistory.mutate({
							...
							data: {
								...
								confidenceScore:
									holdingsWithIsin.length / Math.max(previewHoldings.length, 1),
								status:
									holdingsWithIsin.length === previewHoldings.length
										? "success"
										: "partial",
								errors: [],
							},
	...
	const updateHolding = useCallback(
		(index: number, updates: Partial<ImportedHolding>) => {
			setPreviewHoldings((prev) =>
	...
	if (previewMode) {
		const totalValue = previewHoldings.reduce((sum, h) => {
			return sum + (Number.parseFloat(h.currentValue || "0") || 0);
		}, 0);
				...
				<CardHeader>
					<div className="flex items-center justify-between">
						<div>
							...
							<CardDescription>
								Review the parsed holdings below. You can edit, add, or remove
				...
				<CardContent className="space-y-4">
							...
							<div className="border-l pl-4">
								...
								<div className="flex items-center gap-2 mt-0.5">
									...
									<span className="text-sm text-amber-600">
										{previewHoldings.filter((h) => !h.isin && h.symbol).length}{" "}
										partial
									</span>
									<span className="text-sm text-muted-foreground">/</span>
						...
						<Table>
							...
							<TableBody>
								{previewHoldings.map((holding, index) => {
									...
									const value =
										Number.parseFloat(holding.currentValue || "0") || 0;
										...
										<TableRow key={holding.id || index}>
											...
											<TableCell>
												{holding.isin ? (
													...
													<Badge
														variant="secondary"
														className="text-xs bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 hover:bg-amber-200 dark:hover:bg-amber-800/30"
													>
											...
											<TableCell className="text-right">
												{isEditing ? (
													<Input
														...
														onChange={(e) =>
															updateHolding(index, { avgPrice: e.target.value })
														}
														className="h-8 w-24 text-right"
													/>
												) : (
					...
					<div className="flex items-center gap-4 pt-2">
						...
						<Button
							...
							disabled={saveHoldings.isPending || previewHoldings.length === 0}
						>
							{saveHoldings.isPending ? (
								...
								<>
									...
									{previewHoldings.length} Holdings
								</>
							)}
						</Button>
					</div>
						...
						{showIrisImport && (
							...
							>
								<div className="absolute top-2 right-2">
									<Badge className="text-[10px] px-1.5 py-0 bg-violet-600 text-white">
										KFintech
									</Badge>
								</div>
								...
								<div>
									...
									<p className="text-xs text-muted-foreground mt-1">
										Live fetch from KFintech/IRIS using your PAN — no upload
										needed
									</p>
								</div>
					...
					<div className="space-y-5">
						...
						<div className="rounded-xl border border-violet-200 dark:border-violet-800 bg-violet-50 dark:bg-violet-950/20 p-6 space-y-4">
							<div className="flex items-center gap-3">
								...
								<div>
									<p className="font-semibold">
										Live Fetch from KFintech CAS Registry
									</p>
									<p className="text-xs text-muted-foreground mt-0.5">
										Your Mutual Fund portfolio will be fetched directly using
										your KYC-verified PAN. No file upload required.
									</p>
								</div>
							</div>
							...
							<p className="text-[11px] text-muted-foreground text-center">
								Powered by IRIS KFintech API. Data fetched is read-only and
								requires your explicit save to store.
							</p>
						</div>
					</div>
				)}
				...
				{mode === "smart" && (
					<div className="space-y-5">
						...
						<Button
							...
							disabled={
								(!selectedFile && !urlInput.trim()) || smartImport.isPending
							}
						>
										...
										>
											...
											<div className="flex items-center gap-3 text-muted-foreground text-xs">
												...
												<span className="text-green-600">
													{entry.isinMatchedCount} matched
												</span>
												<span>
