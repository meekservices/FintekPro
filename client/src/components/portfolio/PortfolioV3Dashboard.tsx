...
export function PortfolioV3Dashboard({
	...
	const totalValue = Number.parseFloat(performance?.totalCurrentValue || "0");
	const totalInvested = Number.parseFloat(
		performance?.totalInvestedValue || "0",
	);
	const totalGainLoss = Number.parseFloat(performance?.totalGainLoss || "0");
	const gainLossPercent = Number.parseFloat(
		performance?.totalGainLossPercent || "0",
	);
	const xirr = Number.parseFloat(performance?.xirr || "0");
	const dailyChange = Number.parseFloat(performance?.dailyChange || "0");
	const dailyChangePercent = Number.parseFloat(
		performance?.dailyChangePercent || "0",
	);

	const riskScore = useMemo(() => {
		const volatility = Number.parseFloat(performance?.volatility || "15");
		if (volatility < 10)
			return { score: 2, label: "Low", color: "text-emerald-600" };
		if (volatility < 20)
			return { score: 5, label: "Moderate", color: "text-amber-600" };
		if (volatility < 30)
			return { score: 7, label: "High", color: "text-orange-600" };
	...
	const assetAllocation = useMemo(() => {
		...
		holdings.forEach((holding: any) => {
			...
			const value = Number.parseFloat(holding.currentValue || "0");
	...
	const performanceData = useMemo(() => {
		const getStartDate = () => {
			...
			switch (selectedPeriod) {
				...
				case "CUSTOM":
					return customDateRange.from || subMonths(now, 1);
				default:
					return subMonths(now, 1);
			}
		};
		...
		const endDate =
			selectedPeriod === "CUSTOM" && customDateRange.to
				? customDateRange.to
				: new Date();
		...
		for (let i = 0; i <= dataPoints; i++) {
			const date = new Date(
				startDate.getTime() +
					(i * (endDate.getTime() - startDate.getTime())) / dataPoints,
			);
			const progress = i / dataPoints;
			const portfolioValue =
				baseValue *
				(1 + (gainLossPercent / 100) * progress + (Math.random() - 0.5) * 0.02);
			const benchmarkValue =
				baseValue * (1 + 0.12 * progress + (Math.random() - 0.5) * 0.01);
	...
	const aiInsights = [
		{
			...
			description: "₹15,000 potential tax savings identified",
			icon: Lightbulb,
		},
		{
			type: "alert",
			title: "Portfolio Drift",
			description: "Equity allocation 5% above target",
			icon: AlertTriangle,
		},
		{
			type: "suggestion",
			title: "Rebalance Due",
			description: "Last rebalanced 45 days ago",
			icon: RefreshCw,
		},
	];
					...
					<Card>
						<CardContent className="p-6">
							...
							<p
								className={cn(
									"text-3xl font-bold",
									xirr >= 0 ? "text-purple-600" : "text-red-600",
								)}
							>
								{formatPercent(xirr)}
							</p>
					...
					<Card>
						<CardContent className="p-6">
							...
							<div className="flex items-center gap-3">
								<p className={cn("text-3xl font-bold", riskScore.color)}>
									{riskScore.score}/10
								</p>
								<Badge variant="outline" className={riskScore.color}>
									{riskScore.label}
								</Badge>
							</div>
							<p className="text-sm text-muted-foreground mt-1">
									...
									period.value === "CUSTOM" ? (
										<Popover key={period.value}>
											<PopoverTrigger asChild>
												<Button
													variant={
														selectedPeriod === "CUSTOM" ? "default" : "outline"
													}
													size="sm"
													className="h-8"
												>
									...
									<TooltipTrigger asChild>
										<div className="flex items-center justify-between p-3 rounded-lg bg-muted/50 cursor-help">
											...
											<span className="text-sm font-bold">
												{(performance?.volatility || 15.2).toFixed(1)}%
											</span>
										</div>
									</TooltipTrigger>
									<TooltipContent>
								...
								<Tooltip>
									<TooltipTrigger asChild>
										<div className="flex items-center justify-between p-3 rounded-lg bg-muted/50 cursor-help">
											...
											<span className="text-sm font-bold">
												{(performance?.sharpeRatio || 1.25).toFixed(2)}
											</span>
										</div>
									</TooltipTrigger>
									<TooltipContent>
									...
									<TooltipTrigger asChild>
										<div className="flex items-center justify-between p-3 rounded-lg bg-muted/50 cursor-help">
											...
											<span className="text-sm font-bold text-red-600">
												-{(performance?.maxDrawdown || 12.5).toFixed(1)}%
											</span>
										</div>
									</TooltipTrigger>
									<TooltipContent>
								...
								<Tooltip>
									<TooltipTrigger asChild>
										<div className="flex items-center justify-between p-3 rounded-lg bg-muted/50 cursor-help">
											...
											<span className="text-sm font-bold">
												{(performance?.beta || 0.95).toFixed(2)}
											</span>
										</div>
									</TooltipTrigger>
									<TooltipContent>
