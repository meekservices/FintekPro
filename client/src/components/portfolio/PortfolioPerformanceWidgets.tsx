...
export function LivePerformanceTicker({
	portfolioId,
}: { portfolioId?: string }) {
	...
	if (isLoading) {
		return (
			<Card
				className="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-card dark:to-background border-blue-200 dark:border-blue-800"
				data-testid="widget-live-performance-loading"
			>
	...
	return (
		<Card
			className="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-card dark:to-background border-blue-200 dark:border-blue-800 overflow-hidden"
			data-testid="widget-live-performance"
		>
			<CardHeader className="pb-2">
				<div className="flex items-center justify-between">
					<div className="flex items-center gap-2">
						...
						<CardTitle
							...
							data-testid="text-live-performance-title"
						>
			...
			<CardContent className="space-y-4">
				<div className="flex items-end justify-between">
					<div>
						<div
							...
							data-testid="text-total-portfolio-value"
						>
							₹
							{displayData.totalValue.toLocaleString("en-IN", {
						...
						>
							...
							<span>
								{isPositive ? "+" : ""}₹
								{Math.abs(displayData.dayChange).toLocaleString("en-IN", {
									maximumFractionDigits: 0,
								})}
							</span>
							<span className="text-muted-foreground">
								({isPositive ? "+" : ""}
								{displayData.dayChangePercent.toFixed(2)}%)
							</span>
				...
				<div className="grid grid-cols-4 gap-3 pt-2 border-t border-border">
					{[
						{
							label: "1W",
							value: displayData.weekChangePercent,
							testId: "text-change-1w",
						},
						{
							label: "1M",
							value: displayData.monthChangePercent,
							testId: "text-change-1m",
						},
						{
							label: "1Y",
							value: displayData.yearChangePercent,
							testId: "text-change-1y",
						},
						{
							label: "All",
							value: displayData.allTimeReturnPercent,
							testId: "text-change-all",
						},
					].map((item) => (
					...
					<div className="flex items-center gap-3">
						<Tooltip>
							<TooltipTrigger
								...
								data-testid="tooltip-xirr"
							>
								<Target className="h-3 w-3" />
								<span data-testid="text-xirr-value">
									XIRR: {displayData.xirr.toFixed(1)}%
								</span>
							</TooltipTrigger>
							<TooltipContent>
								Extended Internal Rate of Return - Your actual returns
						...
						<Tooltip>
							<TooltipTrigger
								className="flex items-center gap-1"
								data-testid="tooltip-cagr"
							>
								<BarChart3 className="h-3 w-3" />
								<span data-testid="text-cagr-value">
									CAGR: {displayData.cagr.toFixed(1)}%
								</span>
							</TooltipTrigger>
							<TooltipContent>
								Compound Annual Growth Rate - Average yearly growth of your
								portfolio
							</TooltipContent>
						</Tooltip>
					</div>
...
export function TopMoversWidget({ portfolioId }: { portfolioId?: string }) {
	const {
		...
		isLoading,
		isError,
	} = useQuery<{ gainers: TopMover[]; losers: TopMover[] }>({
	...
	const currentMovers = showGainers
		? displayMovers.gainers
		: displayMovers.losers;
			...
			<CardHeader className="pb-2">
				<div className="flex items-center justify-between">
					<div className="flex items-center gap-2">
						...
						<CardTitle
							className="text-lg text-foreground"
							data-testid="text-top-movers-title"
						>
							Top Movers
						</CardTitle>
					</div>
					...
					>
						<div className="flex items-center gap-3">
							...
							<div>
								<div
									...
									data-testid={`text-mover-symbol-${mover.symbol.toLowerCase()}`}
								>
									{mover.symbol}
								</div>
								<div
									className="text-xs text-muted-foreground truncate max-w-[120px]"
									data-testid={`text-mover-name-${mover.symbol.toLowerCase()}`}
								>
									{mover.name}
								</div>
							</div>
						...
						<div className="text-right">
							<div
								...
								data-testid={`text-mover-price-${mover.symbol.toLowerCase()}`}
							>
								₹
								{mover.currentPrice.toLocaleString("en-IN", {
									maximumFractionDigits: 0,
								})}
							</div>
							<div
...
const requiredIndices = [
	{
		symbol: "^NSEI",
		name: "NIFTY 50",
		defaultValue: 24567.8,
		defaultChange: 128.45,
		defaultChangePercent: 0.52,
	},
	{
		symbol: "^BSESN",
		name: "SENSEX",
		defaultValue: 80892.35,
		defaultChange: 412.3,
		defaultChangePercent: 0.51,
	},
	{
		symbol: "^NSEBANK",
		name: "BANK NIFTY",
		defaultValue: 52148.9,
		defaultChange: -156.75,
		defaultChangePercent: -0.3,
	},
	{
		symbol: "^CNXIT",
		name: "NIFTY IT",
		defaultValue: 42356.15,
		defaultChange: 285.6,
		defaultChangePercent: 0.68,
	},
];
	...
	const buildDisplayIndices = (): MarketIndex[] => {
		...
		return requiredIndices.map((reqIndex) => {
			...
			if (apiData && typeof apiData.price === "number" && apiData.price > 0) {
				return {
					...
					sparklineData: generateSparklineData(
						apiData.changePercent >= 0 ? "up" : "down",
						15,
					),
				};
			}
			return {
				...
				sparklineData: generateSparklineData(
					reqIndex.defaultChangePercent >= 0 ? "up" : "down",
					15,
				),
			};
		});
			...
			<CardHeader className="pb-2">
				<div className="flex items-center justify-between">
					<div className="flex items-center gap-2">
						...
						<CardTitle
							className="text-lg text-foreground"
							data-testid="text-market-pulse-title"
						>
							Market Pulse
						</CardTitle>
					</div>
			...
			<CardContent>
				<div className="grid grid-cols-2 gap-3">
					{displayIndices.map((index) => {
						...
						const sparkData = index.sparklineData.map((v, i) => ({
							index: i,
							value: v,
						}));
							...
							>
								<div className="flex items-center justify-between mb-1">
									<span
										...
										data-testid={`text-index-name-${indexId}`}
									>
										{index.name}
									</span>
									<div
										className="h-6 w-12"
										data-testid={`chart-index-${indexId}`}
									>
										<ResponsiveContainer width="100%" height="100%">
								...
								<div
									...
									data-testid={`text-index-value-${indexId}`}
								>
								...
								>
									...
									{isPositive ? "+" : ""}
									{index.change.toFixed(0)} ({isPositive ? "+" : ""}
...
export function PortfolioAlertsWidget({
	portfolioId,
}: { portfolioId?: string }) {
	const {
		data: alerts,
	...
	const defaultAlerts: PortfolioAlert[] = [
		{
			...
			type: "price",
			symbol: "RELIANCE",
			message: "Price crossed ₹2,850 resistance level",
		...
		},
		{
			...
			type: "target",
			symbol: "TCS",
			message: "52-week high achieved at ₹4,285",
		...
		},
		{
			...
			type: "dividend",
			symbol: "INFY",
			message: "Dividend of ₹18/share declared",
		...
		},
		{
			...
			type: "news",
			symbol: "HDFCBANK",
			message: "Quarterly results announced - Beat estimates",
		...
		},
		{
			...
			type: "stop_loss",
			symbol: "ICICIBANK",
			message: "Approaching stop-loss at ₹1,000",
			...
			<CardHeader className="pb-2">
				<div className="flex items-center justify-between">
					...
					<Badge
						variant="secondary"
						className="text-xs"
						data-testid="badge-active-alerts-count"
					>
						...
						<div className="flex-1 min-w-0">
							<div className="flex items-center gap-2">
								<span
									...
									data-testid={`text-alert-symbol-${alert.id}`}
								>
									{alert.symbol}
								</span>
								<span
...
export function AssetPerformanceWidget({
	...
	const displayAssets =
		isError || !assetPerformance ? defaultAssetPerformance : assetPerformance;
		...
		<Card data-testid="widget-asset-performance">
			<CardHeader className="pb-2">
				<div className="flex items-center gap-2">
					...
					<CardTitle
						className="text-lg text-foreground"
						data-testid="text-asset-performance-title"
					>
						Asset Performance
					</CardTitle>
				</div>
				...
				{displayAssets.map((asset) => (
					...
					>
						<div className="flex items-center justify-between">
							<div className="flex items-center gap-2">
								...
								<span
									...
									data-testid={`text-asset-name-${asset.assetType}`}
								>
									{asset.name}
								</span>
							</div>
							<div className="flex items-center gap-3">
								<span
									...
									data-testid={`text-asset-value-${asset.assetType}`}
								>
						...
						<div className="flex items-center justify-between text-xs text-muted-foreground">
							<span data-testid={`text-asset-allocation-${asset.assetType}`}>
								{asset.allocation}% of portfolio
							</span>
							<span data-testid={`text-asset-week-change-${asset.assetType}`}>
								1W: {asset.weekChange >= 0 ? "+" : ""}
								{asset.weekChange.toFixed(1)}%
							</span>
						</div>
					</div>
				))}
...
export function PortfolioPerformanceWidgets({
	portfolioId,
}: { portfolioId?: string }) {
	return (
		<div
			className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4"
			data-testid="container-portfolio-widgets"
		>
