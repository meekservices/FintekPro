...
export function RebalancingAlerts({ portfolioId }: RebalancingAlertProps) {
			...
			<Card className="border-dashed" data-testid="rebalancing-alerts-widget">
				<CardContent className="pt-6 text-center">
					...
					<p className="text-sm text-muted-foreground">
						Login to view rebalancing alerts
					</p>
				</CardContent>
			</Card>
		);
	...
	return (
		<Card
			data-testid="rebalancing-alerts-widget"
			className={
				needsRebalancing ? "border-orange-300 dark:border-orange-700" : ""
			}
		>
			<CardHeader className="pb-3">
				<div className="flex items-center justify-between">
					<div className="flex items-center gap-2">
						<div
							className={`p-2 rounded-full ${needsRebalancing ? "bg-orange-100 dark:bg-orange-900" : "bg-green-100 dark:bg-green-900"}`}
						>
							{needsRebalancing ? (
			...
			<CardContent className="space-y-4">
					...
					<div className="p-4 bg-muted rounded-lg space-y-4 mb-4">
						...
						<div className="space-y-2">
							...
							<p className="text-xs text-muted-foreground">
								Alert when any asset class drifts more than {driftThreshold}%
								from target
							</p>
						</div>
					</div>
							...
							<div key={idx} className="space-y-1">
								<div className="flex items-center justify-between text-sm">
									<span className="font-medium flex items-center gap-2">
										<div
											className={`w-3 h-3 rounded-full ${allocation.color}`}
										/>
				...
				{needsRebalancing && (
					<div className="flex gap-2 pt-2">
						...
						<Button className="flex-1" data-testid="auto-rebalance-btn">
