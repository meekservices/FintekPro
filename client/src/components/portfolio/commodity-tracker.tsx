			...
			<CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
				<div>
					...
					<p className="text-sm text-muted-foreground">
						Real-time global commodity benchmarks
					</p>
				</div>
				<Button
					variant="ghost"
			...
			<CardContent>
							...
							>
								<div className="flex items-center space-x-3">
									...
									<div>
										<div className="flex items-center space-x-2">
											...
											<Badge
												className={getCategoryColor(commodity.category)}
												variant="secondary"
											>
								...
								<div className="text-right">
									...
									<div className="flex items-center space-x-1">
										...
										>
											{Number(commodity.changePercent) >= 0 ? "+" : ""}
											{Number(commodity.changePercent).toFixed(2)}%
										</span>
									</div>
									<div className="text-xs text-muted-foreground">
										{Number(commodity.changePercent) >= 0 ? "+" : ""}₹
										{Number(commodity.change).toFixed(2)}
									</div>
								</div>
							</div>
						))}
				...
				<div className="mt-4 pt-4 border-t">
					<div className="flex items-center justify-between text-xs text-muted-foreground">
						...
						<span>
							Last update:{" "}
							{commodities?.[0]?.lastUpdated
								? new Date(commodities[0].lastUpdated).toLocaleTimeString()
								: "N/A"}
						</span>
					</div>
				</div>
