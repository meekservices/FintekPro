...
const INTERACTION_TYPE_BADGES: Record<
	string,
...
export function ProposalInteractionThread({
	...
	const addInteractionMutation = useMutation({
		mutationFn: async (data: {
			...
			return await apiRequest(
				`/api/agent/proposals/${proposalId}/interactions`,
				{
					method: "POST",
	...
	const unreadCount =
		interactions?.filter((i) => !i.isRead && i.senderType === "client")
			.length || 0;
			...
			<CardContent className="space-y-4">
				{isLoading ? (
					...
					<div className="space-y-4 max-h-[400px] overflow-y-auto">
						{interactions.map((interaction) => {
							const typeInfo =
								INTERACTION_TYPE_BADGES[interaction.type] ||
								INTERACTION_TYPE_BADGES.question;
								...
								>
									...
									>
										...
										<p className="text-xs text-muted-foreground mt-1">
											{isAgent ? agentName : prospectName} •{" "}
					...
					<div className="text-center py-8 text-muted-foreground">
						...
						<p className="text-sm">
							Client questions and your responses will appear here
						</p>
					</div>
				)}
				...
				<div className="space-y-3">
					<div className="flex gap-2">
						...
						<Button
							variant={
								messageType === "revision_completed" ? "default" : "outline"
							}
							size="sm"
