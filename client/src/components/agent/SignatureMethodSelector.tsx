...
import {
	SignatureCanvas,
	SignatureData,
} from "@/components/esign/SignatureCanvas";
...
const SIGNATURE_METHODS: MethodInfo[] = [
	{
		...
		name: "Aadhaar eSign",
		description:
			"Authenticate with Aadhaar OTP for legally valid digital signature",
		icon: Fingerprint,
		legalValidity: "High",
...
const METHODS_REQUIRING_VISUAL_SIGNATURE: SignatureMethod[] = [
	"zoho_sign",
	"otp",
	"user_signature",
];
...
export default function SignatureMethodSelector({
	...
	const [selected, setSelected] = useState<SignatureMethod | undefined>(
		selectedMethod,
	);
	...
	const availableMethods = SIGNATURE_METHODS.filter((m) =>
		allowedMethods.includes(m.id),
	);

	const needsVisualSignature = (method: SignatureMethod) =>
		requireVisualSignature &&
		METHODS_REQUIRING_VISUAL_SIGNATURE.includes(method);
					...
					<CardContent className="space-y-4">
						{signaturesLoading ? (
							...
							<Alert>
								...
								<AlertDescription>
									You don't have any saved signatures. Create one to proceed
									with signing.
								</AlertDescription>
							</Alert>
						) : (
	...
	const getValidityBadge = (validity: "High" | "Medium" | "Standard") => {
		switch (validity) {
			...
			case "Standard":
				return (
					<Badge className="bg-muted text-foreground">Standard Validity</Badge>
				);
		}
	};
		...
		<Card>
			...
			<CardContent className="space-y-4">
				<Alert>
					...
					<AlertDescription>
						All signature methods are SEBI-compliant and legally binding for
				...
				<RadioGroup
					value={selected}
					onValueChange={(v) => setSelected(v as SignatureMethod)}
				>
					...
					>
						{isLoading ? (
							...
							<>
								...
								Proceed with{" "}
								{selected
									? SIGNATURE_METHODS.find((m) => m.id === selected)?.name
									: "Selected Method"}
							</>
						)}
					</Button>
