...
export function ClientApprovalWorkflow({
  ...
  const submitApprovalMutation = useMutation({
    ...
    onSuccess: (data) => {
      if (data.status === 'approved') {
        toast({ title: "Proposal Approved", description: "Client has approved the proposal for execution" });
        onApproved?.();
      } else if (data.status === 'rejected') {
        toast({ title: "Proposal Rejected", description: "Client has declined this proposal" });
        onRejected?.();
      }
      queryClient.invalidateQueries({ queryKey: ['/api/agent/proposals', proposalId, 'approval'] });
    },
    onError: (error: any) => {
  ...
  const completionSteps = [
    { label: "Disclosure Acknowledged", completed: approval?.disclosureAcknowledged || disclosureAck },
    { label: "Risk Acknowledged", completed: approval?.riskAcknowledged || riskAck },
    { label: "Execution Consent", completed: approval?.executionConsent || executionConsent },
    ...
    return (
      <Card className="border-green-200 dark:border-green-800">
        <CardHeader>
          ...
          <CardDescription>
            {prospectName} approved this proposal on {approval?.approvedAt ? format(new Date(approval.approvedAt), 'MMM d, yyyy h:mm a') : 'N/A'}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="p-3 bg-green-50 dark:bg-green-950 rounded-lg">
              ...
              <p className="text-lg font-bold capitalize">{approval?.signatureType || 'Digital'}</p>
            </div>
          </div>
          {approval?.clientNotes && (
            <div className="mt-4 p-3 bg-muted rounded-lg">
              <p className="text-sm font-medium">Client Notes</p>
              <p className="text-sm text-muted-foreground">{approval.clientNotes}</p>
            </div>
          )}
          <Alert className="mt-4 bg-green-50 dark:bg-green-950 border-green-200">
            ...
            <AlertDescription>
              All consents received. You can now proceed with executing the transactions on FintekPro.
            </AlertDescription>
          </Alert>
        </CardContent>
      </Card>
    ...
    return (
      <Card className="border-red-200 dark:border-red-800">
        <CardHeader>
          ...
          <CardDescription>
            {prospectName} declined this proposal on {approval?.rejectedAt ? format(new Date(approval.rejectedAt), 'MMM d, yyyy h:mm a') : 'N/A'}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {approval?.rejectionReason && (
            <div className="p-3 bg-red-50 dark:bg-red-950 rounded-lg">
              ...
              <p className="text-sm text-muted-foreground">{approval.rejectionReason}</p>
            </div>
          )}
          <Alert className="mt-4">
            <AlertCircle className="h-4 w-4" />
            <AlertDescription>
              Consider revising the proposal based on client feedback and resharing.
            </AlertDescription>
          </Alert>
        </CardContent>
      </Card>
      ...
      <CardContent className="space-y-6">
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-sm text-muted-foreground">{completedCount}/{completionSteps.length}</span>
          </div>
          ...
          <div className="flex justify-between mt-2">
            {completionSteps.map((step, idx) => (
              <div key={idx} className="flex flex-col items-center">
                ...
                <span className="text-[10px] text-muted-foreground mt-1 text-center max-w-[60px]">{step.label}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex items-start space-x-3 p-3 border rounded-lg">
            <Checkbox
              ...
              onCheckedChange={(checked) => setDisclosureAck(checked as boolean)}
          ...
          <div className="flex items-start space-x-3 p-3 border rounded-lg">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-muted-foreground mt-1">
                I understand that investments are subject to market risks and past performance is not indicative of future results. I accept the risk level of this proposal.
              </p>
            </div>
          ...
          <div className="flex items-start space-x-3 p-3 border rounded-lg">
            <Checkbox
              ...
              onCheckedChange={(checked) => setExecutionConsent(checked as boolean)}
              disabled={!disclosureAck || !riskAck}
              data-testid="checkbox-execution"
            />
            <div className="flex-1">
              <Label htmlFor="execution" className={`font-medium cursor-pointer ${(!disclosureAck || !riskAck) ? 'text-muted-foreground' : ''}`}>
                Execution Consent
              </Label>
              <p className="text-sm text-muted-foreground mt-1">
                I authorize FintekPro and my advisor to execute the transactions as outlined in this proposal on my behalf.
              </p>
          ...
          <div className="p-4 border border-red-200 rounded-lg bg-red-50 dark:bg-red-950">
            ...
            <div className="flex gap-2 mt-3">
              <Button
                ...
                disabled={!rejectionReason.trim() || submitApprovalMutation.isPending}
              >
                {submitApprovalMutation.isPending ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <XCircle className="h-4 w-4 mr-2" />}
                Confirm Rejection
              </Button>
      ...
      {!showRejectForm && (
        <CardFooter className="flex justify-between">
          ...
          <Button
            ...
            disabled={!disclosureAck || !riskAck || !executionConsent || submitApprovalMutation.isPending}
