...
export function ProposalComparisonView({
	...
	const calculateRiskScore = (allocation: Record<string, number>): number => {
		const equityWeight =
			(allocation.equity || 0) + (allocation.mutual_fund || 0) * 0.6;
		const debtWeight =
			(allocation.bond || 0) + (allocation.debt || 0) + (allocation.fd || 0);
									...
									{Array.from(allAssetClasses).map((asset) => {
										...
										return (
											<div key={asset} className="flex items-center gap-2">
												<div
													className={`w-3 h-3 rounded ${ASSET_COLORS[asset] || ASSET_COLORS.other}`}
												/>
						...
						<div className="space-y-4">
							<div className="text-center p-4 bg-primary/10 rounded-lg">
								<p className="text-2xl font-bold text-primary">
									₹{proposedTotal.toLocaleString("en-IN")}
								</p>
				...
				<Card>
					...
					<CardContent>
						...
						{Math.abs(riskChange) > 15 && (
							<Alert
								className={`mt-4 ${riskChange > 0 ? "border-amber-200 dark:border-amber-800" : "border-green-200 dark:border-green-800"}`}
							>
...
export function GuidedWalkthroughPanel({
	...
	const talkingPoints: TalkingPoint[] = [
		...
		...recommendations.slice(0, 5).map((rec) => ({
			...
			content:
				rec.selectionReason ||
				rec.rationale ||
				`Allocated ${rec.allocationPercentage}% (₹${rec.recommendedAmount.toLocaleString("en-IN")}) to ${rec.productType.replace(/_/g, " ")}`,
			type: "rationale" as const,
		})),
		{
			title: "Risk Disclosure",
			content:
				"All investments carry risk. Past performance is not indicative of future results. The recommendations are based on current market conditions and may need periodic review.",
			type: "disclosure",
		},
		{
			title: "Regulatory Disclosure",
			content:
				"This proposal is advisory in nature. Final investment decisions are made by you with my assistance as a licensed agent. I am registered with relevant regulatory bodies.",
			type: "disclosure",
		},
		{
			title: "Next Steps",
			content:
				"If you're comfortable with this proposal, we can proceed with the documentation and execution. You'll need to acknowledge the risk disclosures before we execute any transactions.",
			type: "benefit",
		},
	];
	...
	const getTypeIcon = (type: TalkingPoint["type"]) => {
		switch (type) {
			...
			case "disclosure":
				return <LucideShield className="h-4 w-4 text-amber-500" />;
			case "benefit":
		...
		<Card>
			...
			<CardContent className="space-y-3">
				<Alert className="bg-blue-50 dark:bg-blue-950 border-blue-200">
					...
					<AlertDescription>
						Use these talking points during your client meeting. Each point
						includes rationale and required disclosures.
					</AlertDescription>
				</Alert>
				...
				{talkingPoints.map((point, idx) => (
					<div
						key={idx}
						className={`p-3 rounded-lg border ${getTypeBg(point.type)}`}
					>
					...
					<div className="flex items-start gap-2">
						...
						<div>
							...
							<p className="text-xs text-muted-foreground mt-1">
								...
								investment decisions are made by you with the assistance of a
								licensed agent."
							</p>
						</div>
					</div>
