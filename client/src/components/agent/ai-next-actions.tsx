...
const NEXT_ACTIONS: NextAction[] = [
	{
		...
		description:
			"Stock reached target price of ₹1,750. Consider booking profits.",
		clientName: "Rajesh Sharma",
		clientId: "1",
		impact: "Protect ₹2.4L gains",
		suggestedAction:
			"Call client to discuss profit booking before price correction",
		deadline: "Today",
		potentialValue: 240000,
		reasoning:
			"AI detected target price breach with overbought RSI (78). Historical pattern suggests 15% correction probability in next 2 weeks.",
	},
	{
		id: "2",
		type: "tax_harvest",
		...
		suggestedAction:
			"Schedule call to discuss selling Wipro, reinvesting in TCS after 30 days",
		deadline: "3 days",
		potentialValue: 32000,
		reasoning:
			"Portfolio has ₹2.5L STCG. Wipro showing ₹1.1L loss can offset, saving 30% tax.",
	},
	{
		id: "3",
		type: "rebalance",
		...
		reasoning:
			"Client risk profile is Moderate Conservative. Current allocation exposes to unnecessary volatility. Suggest SDP to debt over 3 months.",
	},
	{
		id: "4",
		type: "upsell",
		...
		reasoning:
			"Client in 31.2% tax bracket with no NPS. ₹50K contribution provides additional deduction beyond 80C limit.",
	},
	{
		id: "5",
		type: "follow_up",
		...
		reasoning:
			"High-value proposal awaiting decision. 7-day follow-up window optimal for conversion.",
	},
	{
		id: "6",
		type: "kyc",
		...
		deadline: "10 days",
		reasoning:
			"Approaching KYC expiry. Early renewal prevents service disruption and demonstrates proactive care.",
	},
	{
		id: "7",
		type: "exit_alert",
		...
		reasoning:
			"Stock down 8.2% from purchase. Technical indicators bearish. Suggest exit to protect capital.",
	},
	{
		id: "8",
		type: "review",
		...
		reasoning:
			"Regular reviews improve retention. Client AUM ₹45L warrants quarterly engagement.",
	},
];
...
export default function AINextActions({
	maxItems = 8,
	compact = false,
}: AINextActionsProps) {
	const [dismissedIds, setDismissedIds] = useState<string[]>([]);

	const activeActions = NEXT_ACTIONS.filter(
		(action) => !dismissedIds.includes(action.id),
	).slice(0, maxItems);
				...
				<div className="flex items-center justify-between">
					<div>
						...
						<CardDescription className="text-muted-foreground mt-1">
							Personalized recommendations based on client data and market
							signals
						</CardDescription>
					</div>
					<div className="flex items-center gap-2">
										...
										<div className="flex items-start gap-3">
											...
											<div className="flex-1">
												...
												<div className="flex items-center gap-4 mt-2 text-sm">
													...
													{action.deadline && (
														<span
															className={`flex items-center gap-1 ${action.deadline === "Today" ? "text-red-400" : "text-amber-400"}`}
														>
													...
													{action.potentialValue && (
														<span
															className={`flex items-center gap-1 ${action.potentialValue > 0 ? "text-emerald-400" : "text-red-400"}`}
														>
															<IndianRupee className="h-3 w-3" />
															{action.potentialValue > 0 ? "+" : ""}
															{formatValue(action.potentialValue)}
														</span>
													)}
												</div>
