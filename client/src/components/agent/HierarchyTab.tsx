...
import { 
  Building2, ChevronDown, ChevronUp, Pencil, Plus, 
  Users, TrendingUp, Network, IndianRupee, PieChart, Activity 
} from "lucide-react";
...
export function HierarchyTab({ isAdmin }: { isAdmin: boolean }) {
  ...
  const addEmp = useMutation({
    ...
    onSuccess: () => { 
      toast({ title: "Employee added", description: "The sub-broker has been successfully added to your hierarchy." }); 
  ...
  const updateEmp = useMutation({
    ...
    onSuccess: () => { 
      toast({ title: "Employee updated", description: "Sub-broker details improved." }); 
      setEmpDialogOpen(false); 
      qc.invalidateQueries({ queryKey: ["/api/iris/hierarchy/sub-brokers"] }); 
    },
      ...
      <div className="bg-gradient-to-r from-primary/10 via-background to-primary/5 rounded-xl border p-6 flex flex-col md:flex-row md:items-center justify-between gap-4 relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <p className="text-sm text-muted-foreground mt-1 max-w-md">
            Manage your downline sub-brokers, view their performance metrics, and track your organizational hierarchy.
          </p>
        </div>

        <div className="flex items-center gap-4 relative z-10">
          <div className="bg-background/80 backdrop-blur-sm border rounded-lg p-3 text-center min-w-[120px] shadow-sm">
            <h4 className="text-2xl font-bold text-primary">{sbL ? "-" : brokers.length}</h4>
            <p className="text-[11px] uppercase tracking-wider font-semibold text-muted-foreground mt-0.5">Total Partners</p>
          </div>
          <div className="bg-background/80 backdrop-blur-sm border rounded-lg p-3 text-center min-w-[120px] shadow-sm">
            <h4 className="text-2xl font-bold text-emerald-600">{sbL ? "-" : activeCount}</h4>
            <p className="text-[11px] uppercase tracking-wider font-semibold text-muted-foreground mt-0.5">Active</p>
          </div>
        </div>
      </div>

      <div className="flex items-center justify-between">
        <h4 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">Hierarchy Tree</h4>
        {isAdmin && (
          <Button onClick={openAdd} className="shadow-md hover:shadow-lg transition-all rounded-full px-5">
          ...
          {sbL ? (
            ...
            <div className="grid grid-cols-1 gap-4">
              {brokers.map((b, i) => {
                ...
                return (
                  <div key={i} className={`rounded-xl border transition-all duration-300 ease-in-out ${isExpanded ? 'border-primary/50 shadow-lg bg-card/50 backdrop-blur-sm' : 'border-border/60 hover:border-primary/30 hover:shadow-md bg-card/30'}`}>
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div className={`h-12 w-12 rounded-full flex items-center justify-center transition-colors ${isExpanded ? 'bg-primary text-primary-foreground shadow-md' : 'bg-primary/10 text-primary'}`}>
                          <Building2 className="h-6 w-6" />
                        </div>
                        <div>
                          <div className="flex items-center gap-2">
                            <h4 className="text-lg font-semibold tracking-tight">{b.name ?? b.subBrokerName ?? "—"}</h4>
                            {b.status && (
                              <Badge variant={b.status === "ACTIVE" ? "default" : "secondary"} className={`text-[10px] uppercase font-bold tracking-wider rounded-sm ${b.status === "ACTIVE" ? 'bg-emerald-500 hover:bg-emerald-600' : ''}`}>
                                {b.status}
                              </Badge>
                            )}
                          </div>
                          <div className="flex items-center gap-3 text-sm text-muted-foreground mt-1">
                            ...
                            {b.mobile && <span className="flex items-center gap-1">• {b.mobile}</span>}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2 mt-4 sm:mt-0">
                        ...
                        <div className={`h-8 w-8 rounded-full flex items-center justify-center border transition-all ${isExpanded ? 'bg-background shadow-inner' : 'bg-background hover:bg-muted'}`}>
                        ...
                        <div className="mt-5">
                          {aumL ? (
                            ...
                            <div className="space-y-6">
                              ...
                              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                <div className="bg-background rounded-lg p-4 border shadow-sm flex items-center gap-4 hover:border-primary/30 transition-colors">
                                  ...
                                  <div>
                                    ...
                                    <p className="text-xl font-bold mt-1 text-foreground">{fmt(aum.totalAum)}</p>
                                  </div>
                                </div>
                                ...
                                <div className="bg-background rounded-lg p-4 border shadow-sm flex items-center gap-4 hover:border-primary/30 transition-colors">
                                  ...
                                  <div>
                                    <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">SIP Book</p>
                                    <p className="text-xl font-bold mt-1 text-foreground">{fmt(aum.sipBook)}</p>
                                  </div>
                                </div>
                                ...
                                <div className="bg-background rounded-lg p-4 border shadow-sm flex items-center gap-4 hover:border-primary/30 transition-colors">
                                  ...
                                  <div>
                                    <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Investors</p>
                                    <p className="text-xl font-bold mt-1 text-foreground">{aum.investorCount ?? "—"}</p>
                                  </div>
                                </div>
                                ...
                                <div className="bg-background rounded-lg p-4 border shadow-sm flex items-center gap-4 hover:border-primary/30 transition-colors">
                                  ...
                                  <div>
                                    <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Schemes</p>
                                    <p className="text-xl font-bold mt-1 text-foreground">{aum.topSchemes?.length ?? 0}</p>
                                  </div>
                                </div>
                              </div>
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                {aum.amcWise && aum.amcWise.length > 0 && (
                                  <div className="bg-background/50 rounded-xl border p-5">
                                    <h5 className="flex items-center gap-2 text-sm font-semibold text-foreground mb-4">
                                      <Building2 className="h-4 w-4 text-primary" /> AMC Distribution
                                    </h5>
                                    <div className="space-y-3">
                                      {aum.amcWise.slice(0, 5).map((a, j) => {
                                        const pct = aum.totalAum ? ((a.aum ?? 0) / aum.totalAum) * 100 : 0;
                                        return (
                                          <div key={j} className="group">
                                            <div className="flex justify-between text-sm mb-1">
                                              <span className="font-medium text-foreground">{a.amcName ?? a.amc ?? "—"}</span>
                                              <span className="font-bold text-foreground">{fmt(a.aum)}</span>
                                            </div>
                                ...
                                {aum.topSchemes && aum.topSchemes.length > 0 && (
                                  <div className="bg-background/50 rounded-xl border p-5">
                                    <h5 className="flex items-center gap-2 text-sm font-semibold text-foreground mb-4">
                                      <PieChart className="h-4 w-4 text-primary" /> Top Schemes
                                    </h5>
                                    <div className="space-y-3">
                                      {aum.topSchemes.slice(0, 5).map((s, j) => {
                                        return (
                                          <div key={j} className="flex items-center justify-between border-b border-border/50 pb-2 last:border-0 last:pb-0">
                                            <div className="max-w-[70%]">
                                              <p className="text-sm font-medium text-foreground truncate" title={s.schemeName ?? s.scheme ?? "—"}>
                            ...
                            <div className="flex flex-col items-center justify-center py-8 text-muted-foreground border border-dashed rounded-lg bg-background/50">
                              ...
                              <p className="text-sm">AUM analytics will appear once this partner registers investments.</p>
                            </div>
                          )}
                        </div>
                      </div>
            ...
            <div className="flex flex-col items-center justify-center p-12 text-center border rounded-xl border-dashed bg-muted/10">
              ...
              <h3 className="text-xl font-semibold mb-2">No Sub-Brokers Found</h3>
              <p className="text-muted-foreground max-w-sm">
                Your organizational hierarchy is currently empty. Click the button below to add your first partner.
              </p>
              {isAdmin && (
          ...
          <DialogContent className="sm:max-w-md p-0 overflow-hidden">
            <div className="bg-gradient-to-r from-primary/10 to-transparent p-6 pb-4 border-b">
              <DialogTitle className="text-xl flex items-center gap-2">
                {editingEuin ? <Pencil className="h-5 w-5 text-primary" /> : <Plus className="h-5 w-5 text-primary" />}
                {editingEuin ? "Update Partner Details" : "Add New Partner"}
              </DialogTitle>
              <DialogDescription className="mt-1">
                {editingEuin ? `Editing EUIN mapping: ${editingEuin}` : "Register a new sub-broker downline in your hierarchy."}
              </DialogDescription>
            </div>
            ...
            <div className="p-6 space-y-5 bg-background">
              {!editingEuin && (
                <div className="space-y-1.5">
                  <Label className="text-xs uppercase tracking-wider font-semibold text-muted-foreground">EUIN Code <span className="text-destructive">*</span></Label>
                  <Input 
                    className="focus-visible:ring-primary/50" 
                    value={empForm.euinCode} 
                    onChange={e => setEmpForm(f => ({ ...f, euinCode: e.target.value.toUpperCase() }))} 
              ...
              <div className="space-y-1.5">
                <Label className="text-xs uppercase tracking-wider font-semibold text-muted-foreground">Full Name <span className="text-destructive">*</span></Label>
                <Input 
                  className="focus-visible:ring-primary/50" 
                  value={empForm.name} 
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <Label className="text-xs uppercase tracking-wider font-semibold text-muted-foreground">Mobile</Label>
                  <Input 
                    className="focus-visible:ring-primary/50" 
            ...
            <div className="p-4 bg-muted/50 border-t flex items-center justify-end gap-3">
              <Button variant="ghost" className="hover:bg-background" onClick={() => setEmpDialogOpen(false)}>
                Cancel
              </Button>
              <Button 
                ...
                onClick={() => {
                  if (editingEuin) { 
                    updateEmp.mutate({ euinCode: editingEuin, body: { name: empForm.name, mobile: empForm.mobile, email: empForm.email } }); 
                  } else { 
                    addEmp.mutate({ euinCode: empForm.euinCode, name: empForm.name, mobile: empForm.mobile, email: empForm.email }); 
                  }
                }}
                disabled={addEmp.isPending || updateEmp.isPending || (!editingEuin && !empForm.euinCode) || !empForm.name}
              >
                {(addEmp.isPending || updateEmp.isPending) ? (
                  <span className="flex items-center gap-2"><Activity className="h-4 w-4 animate-pulse" /> Saving...</span>
                ) : (
                  editingEuin ? "Save Changes" : "Create Partner"
                )}
              </Button>
            </div>
          </DialogContent>
