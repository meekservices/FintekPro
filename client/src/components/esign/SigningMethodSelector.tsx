...
const SIGNING_METHODS: SigningMethodOption[] = [
  ...
  },
  {
    ...
    icon: CreditCard,
    features: ['Class 2/3 DSC', 'No OTP', 'Offline Capable', 'Highest Security'],
    legalValidity: 'high',
    costPerSign: 0,
    available: true,
  ...
  },
  {
    ...
    icon: MessageSquare,
    features: ['Mobile First', 'Instant Delivery', 'High Open Rate', 'Convenient'],
    legalValidity: 'medium',
    costPerSign: 0.5,
    available: true,
  },
...
export function SigningMethodSelector({
  ...
  const [hoveredMethod, setHoveredMethod] = useState<SigningMethod | null>(null);
  ...
  const getLegalBadge = (level: string) => {
    switch (level) {
      case 'high':
        return <Badge className="bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400">High Legal Validity</Badge>;
      case 'medium':
  ...
  if (compact) {
    return (
      <RadioGroup value={selectedMethod} onValueChange={(v) => onSelect(v as SigningMethod)}>
    ...
    <div className="space-y-4">
              ...
              <CardHeader className="pb-2">
                <div className="flex items-center gap-3">
                  <div className={cn(
                    ...
                    selected ? "bg-emerald-100 dark:bg-emerald-900/30" : "bg-muted"
                  )}>
                  ...
                  <div>
                    ...
                    {method.costPerSign !== undefined && (
                      <span className="text-xs text-muted-foreground">
                        {method.costPerSign === 0 ? 'Free' : `₹${method.costPerSign}/sign`}
                      </span>
                    )}
                  </div>
                </div>
      ...
      {selectedMethod && (
        <Alert>
          ...
          <AlertDescription>
            Selected: <strong>{SIGNING_METHODS.find(m => m.id === selectedMethod)?.name}</strong>
            {' - '}
