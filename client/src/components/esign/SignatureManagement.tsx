...
export function SignatureManagement() {
	...
	const [editingSignature, setEditingSignature] =
		useState<UserSignature | null>(null);
	...
	const handleSaveSignature = (signatureData: SignatureData) => {
		const name =
			newName.trim() ||
			`${signatureData.type === "upload" ? "Uploaded" : signatureData.type === "draw" ? "Drawn" : "Typed"} Signature`;
				...
				<CardContent>
					{signatures.length === 0 ? (
						<Alert>
							...
							<AlertDescription>
								You haven't created any signatures yet. Click "Add Signature" to
