...
const agentNavCategories: NavCategory[] = [
	{
		...
		items: [
			...
			},
			{
				...
				href: "/agent/tracker",
				icon: Activity,
				description: "MFCentral AUM, SIP & trail tracker",
				badge: "NEW",
			},
			{
				title: "My Profile",
	...
	},
	{
		...
		items: [
			...
			{
				title: "HNI Prospects",
				href: "/agent/hni-leaderboard",
				icon: UserCog,
				description: "Prospects ranked by wealth score",
				badge: "AI",
			},
			{
				title: "Deal Matcher",
				href: "/agent/deal-matcher",
				icon: Crosshair,
				description: "Match AIF/PMS deals to best-fit prospects",
				badge: "AI",
			},
			{
				title: "Zoho CRM",
				href: "/agent/zoho-crm",
				icon: BarChart3,
				description: "All leads from Zoho",
			},
			{
				title: "Tasks",
				href: "/agent/tasks",
	...
	},
	{
		...
		items: [
			...
			},
			{
				...
				href: "/agent/onboard-client",
				icon: UserPlus,
				description: "New client KYC",
			},
			{
				title: "KYC Rejections",
				href: "/agent/kyc-rejections",
				icon: ShieldAlert,
				description: "Retry failed KYC sessions",
			},
		],
	},
	{
		...
		items: [
			{
				...
				href: "/agent/proposal-builder",
				icon: Plus,
				description: "Multi-product proposals",
			},
			{
				title: "My Proposals",
		...
		items: [
			{
				...
				href: "/agent/loan-marketplace",
				icon: Landmark,
				description: "Browse available loan products",
			},
			{
				title: "Submit Loan Lead",
				href: "/agent/loan-apply",
				icon: Send,
				description: "Submit client loan application",
			},
			{
				title: "Track Applications",
				href: "/agent/loan-applications",
				icon: ClipboardList,
				description: "Monitor your loan leads",
			},
			{
				title: "DSA Performance",
				href: "/agent/dsa-performance",
				icon: Target,
				description: "Your loan metrics",
			},
		],
	},
	{
		...
		items: [
			{
				...
				href: "/agent/iris",
				icon: Landmark,
				description: "MF, AIF, PMS, FD, NPS & SIF via KFintech",
				badge: "NEW",
			},
			{
				title: "Alpaca Hub",
				...
				icon: Briefcase,
				description: "Full Alpaca US equity management",
			},
			{
				title: "US Client Accounts",
				href: "/agent/us-client-accounts",
				icon: Wallet,
				description: "Broker-managed US trading accounts",
			},
			{
				title: "Open US Account",
				href: "/agent/us-client-accounts",
				icon: UserPlus,
				description: "Onboard client for US equity",
			},
			{
				title: "Treasury",
				href: "/agent/treasury",
				icon: Coins,
				description: "Treasury & fixed income instruments",
			},
		],
	},
	{
		...
		items: [
			{
				...
				href: "/agent/revenue",
				icon: DollarSign,
				description: "AUM and commissions overview",
			},
			{
				title: "My Payouts",
				href: "/agent/payouts",
			...
			{
				title: "Payout Claims",
				href: "/agent/payout-claims",
				icon: FileCheck,
				description: "Submit & track payout claims",
			},
			{
				title: "Commission Calculator",
				href: "/agent/commission-calculator",
				icon: Calculator,
				description: "Estimate commissions",
			},
			{
				title: "Reports",
				href: "/agent/reports",
				icon: FileText,
				description: "Client and portfolio reports",
			},
			{
				title: "SIP Health Monitor",
				href: "/agent/sip-health",
				icon: HeartPulse,
				description: "Client SIP status dashboard",
			},
			{
				title: "Market Alert Center",
				href: "/agent/market-alerts",
				icon: BellRing,
				description: "Significant moves in client holdings",
			},
			{
				title: "Portfolio Drift",
				href: "/agent/portfolio-drift",
				icon: Activity,
				description: "Detect allocation drift across clients",
			},
		],
	},
	{
		...
		items: [
			{
				title: "AI Advisory",
				href: "/agent/investment-advisory",
				icon: Brain,
			...
			},
			{
				...
				icon: ClipboardList,
				description: "ITR filing & tax services",
			},
			{
				title: "Festival Greetings",
				href: "/agent/festival-greetings",
				icon: Sparkles,
				description: "Send festival wishes to clients",
			},
			{
				title: "Bulk Communication",
				href: "/agent/bulk-communication",
				icon: Mail,
				description: "Email/SMS campaigns",
			},
			{
				title: "Marketing Profile",
				href: "/agent/advisor-profile",
				icon: Briefcase,
				description: "Public microsite & credentials",
			},
		],
	},
	{
		...
		items: [
			{
				...
				icon: ListChecks,
				description: "Curated instrument lists",
			},
			{
				title: "Research Note",
				href: "/agent/research/generate",
				icon: FileEdit,
				description: "Institutional PPT & PDF reports",
				badge: "AI",
			},
			{
				title: "Screener",
				href: "/agent/screener",
				icon: Filter,
				description: "Filter instruments by criteria",
			},
			{
			...
			{
				title: "Analytics",
				href: "/agent/research-analytics",
				icon: BarChart3,
				description: "Research performance analysis",
			},
		],
	},
	{
		...
		items: [
			{
				title: "Knowledge Hub",
				href: "/agent/knowledge-hub",
				icon: BookOpen,
				description: "Market briefs & product insights",
			},
			{
				title: "Training",
				href: "/agent/training",
		...
		items: [
			{
				...
				href: "/agent/settings",
				icon: Settings,
				description: "Your preferences & account settings",
			},
			{
				title: "Theme & Accessibility",
				href: "/agent/theme-settings",
				icon: Palette,
		...
		<nav className="fixed bottom-0 left-0 right-0 z-50 md:hidden bg-slate-900/95 backdrop-blur-md border-t border-border/50 safe-area-bottom">
			<div className="flex items-center justify-around h-14">
				{mobileNavItems.map((item) => {
					...
					const isActive =
						location === item.href ||
						(item.href !== "/agent" && location.startsWith(item.href));
					return (
						<button
							key={item.href}
...
export function AgentLayout({ children }: AgentLayoutProps) {
		...
		agentNavCategories.forEach((category) => {
			...
			if (isActive) {
				setExpandedCategories(
					(prev) => new Set(Array.from(prev).concat(category.title)),
				);
			}
		});
	}, [location]);
	...
	const isAgent =
		actualUser?.roles?.includes("agent") ||
		actualUser?.roles?.includes("admin") ||
		actualUser?.roles?.includes("superadmin") ||
		actualUser?.roles?.includes("partner");
					...
					<div className="bg-background/80 backdrop-blur-xl border border-red-100 dark:border-red-900/30 rounded-2xl shadow-2xl p-8 max-w-md w-full text-center relative overflow-hidden">
						...
						{actualUser?.email && (
							<p className="text-sm text-muted-foreground mb-6">
								...
								<strong className="text-foreground">{actualUser.email}</strong>,
								which does not have agent access.
							</p>
						)}
	...
	return (
		<div className="h-screen bg-background text-foreground flex flex-col overflow-hidden">
				...
				<div className="flex items-center justify-between px-2 sm:px-4 py-1.5 sm:py-2.5">
					...
					<div className="flex items-center gap-1 sm:gap-2">
						...
						>
							...
							<PopoverContent
								className="w-[calc(100vw-1rem)] sm:w-96 max-w-96 p-0 bg-background border-border"
								align="end"
								sideOffset={8}
							>
								...
								<ScrollArea className="max-h-96">{notificationList}</ScrollArea>
							</PopoverContent>
						</Popover>
								...
								<DropdownMenuTrigger asChild>
									...
									>
										...
										<div className="text-left hidden md:block">
											<p className="text-xs font-medium text-foreground leading-tight max-w-[90px] truncate">
									...
									<DropdownMenuGroup>
										...
										<DropdownMenuItem asChild>
											<a
												...
												className="flex items-center gap-2.5 px-3 py-2 cursor-pointer text-blue-500"
											>
			...
			<CommandDialog open={searchOpen} onOpenChange={setSearchOpen}>
				<CommandInput
					placeholder="Search clients, proposals, tasks..."
					className="border-0"
				/>
				<CommandList>
