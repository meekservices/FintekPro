...
function getKycChipProps(user: Record<string, unknown>): {
	label: string;
	className: string;
} {
	...
	if (panOk && aadhaarOk)
		return {
			label: "KYC Verified",
			className: "bg-emerald-600 hover:bg-emerald-600 text-white border-0",
		};
...
export function AppLayout({ children }: AppLayoutProps) {
	...
	const bellBadgeCount = (activeAlerts ?? []).filter(
		(a) =>
			...
			new Date(a.lastTriggeredAt).getTime() > dayAgo,
	).length;
	...
	// KYC status chip
	const kycChip = user
		? getKycChipProps(user as unknown as Record<string, unknown>)
		: null;
	...
	return (
		...
		>
			<div
				className={
					isMobile ? "flex flex-col flex-1" : "h-full grid grid-cols-[auto_1fr]"
				}
			>
