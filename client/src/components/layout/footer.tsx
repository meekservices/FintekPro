...
export function Footer() {
	...
	useEffect(() => {
		...
		if (savedScore) {
			setCreditScore(Number.parseInt(savedScore));
		}
	}, []);
			...
			<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
				<div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-8 gap-8">
					...
					<div>
						...
						>
							Complete financial services platform for investments, trading,
							loans, tax filing, and wealth management.
						</p>
					...
					<div>
						<h4
							...
							data-testid="footer-getting-started-title"
						>
								...
								<li key={link.name}>
									<Link href={link.href}>
										<span
											...
											data-testid={`footer-start-${link.name.toLowerCase().replace(/\s+/g, "-")}`}
										>
											{link.name}
										</span>
									</Link>
					...
					<div>
						<h4
							className="font-semibold mb-4 flex items-center gap-2"
							data-testid="footer-research-title"
						>
								...
								<li key={link.name}>
									<Link href={link.href}>
										<span
											...
											data-testid={`footer-research-${link.name.toLowerCase().replace(/\s+/g, "-")}`}
										>
											{link.name}
										</span>
									</Link>
					...
					<div>
						<h4
							className="font-semibold mb-4 flex items-center gap-2"
							data-testid="footer-premium-title"
						>
								...
								<li key={link.name}>
									<Link href={link.href}>
										<span
											...
											data-testid={`footer-premium-${link.name.toLowerCase().replace(/\s+/g, "-")}`}
										>
											{link.name}
										</span>
									</Link>
						...
						<h4
							className="font-semibold mb-4 mt-6 flex items-center gap-2"
							data-testid="footer-ai-title"
						>
								...
								<li key={link.name}>
									<Link href={link.href}>
										<span
											...
											data-testid={`footer-ai-${link.name.toLowerCase().replace(/\s+/g, "-")}`}
										>
											{link.name}
										</span>
									</Link>
					...
					<div>
						<h4
							className="font-semibold mb-4 flex items-center gap-2"
							data-testid="footer-investing-title"
						>
								...
								<li key={link.name}>
									<Link href={link.href}>
										<span
											...
											data-testid={`footer-investing-${link.name.toLowerCase().replace(/\s+/g, "-")}`}
										>
											{link.name}
										</span>
									</Link>
								...
								<li key={link.name}>
									<Link href={link.href}>
										<span
											...
											data-testid={`footer-tax-${link.name.toLowerCase().replace(/\s+/g, "-")}`}
										>
											{link.name}
										</span>
									</Link>
							...
							{familyAlertsLinks.map((link) => (
								<li key={link.name}>
									<Link href={link.href}>
										<span
											className="hover:text-indigo-400 transition-colors cursor-pointer"
											data-testid={`footer-family-${link.name.toLowerCase().replace(/\s+/g, "-")}`}
										>
											{link.name}
										</span>
									</Link>
					...
					<div className="border-t border-border mt-8 pt-6">
						<div className="flex flex-col md:flex-row justify-between items-center">
							<div className="flex items-center space-x-4 mb-4 md:mb-0">
								...
								<div className="flex items-center space-x-2">
									<span
										className={`text-2xl font-bold ${getCreditScoreColor(creditScore)}`}
									>
										{creditScore}
									</span>
				...
				<div className="border-t border-border mt-8 pt-6">
						...
						<div className="flex flex-wrap justify-center md:justify-start gap-4 mb-4 md:mb-0">
							{legalLinks.map((link, index) => (
								<span key={link.name} className="flex items-center">
									<Link href={link.href}>
										<span
											className="text-muted-foreground hover:text-blue-400 transition-colors cursor-pointer text-sm"
					...
					<div className="text-center text-muted-foreground text-sm">
						<p data-testid="footer-copyright">
							&copy; 2026 FintekPro Financial Services LLP All rights reserved.
							| SEBI Registered Investment Advisor | AMFI Registered Mutual Fund
							Distributor
						</p>
						<p className="text-xs text-muted-foreground mt-2">
							Investments are subject to market risks. Read all related
