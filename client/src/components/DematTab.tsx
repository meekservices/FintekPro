...
const dematAccountSchema = z.object({
	dematAccountNumber: z
		.string()
		.min(8, "Demat account number must be at least 8 digits")
	...
	depositoryType: z.enum(["NSDL", "CDSL"], {
		error: "Please select depository type",
	}),
...
export function DematTab() {
	...
	const addAccountMutation = useMutation({
		mutationFn: (data: DematAccountForm) =>
			apiRequest("POST", "/api/demat-accounts", data),
		onSuccess: () => {
	...
	const deleteAccountMutation = useMutation({
		mutationFn: (id: string) =>
			apiRequest("DELETE", `/api/demat-accounts/${id}`),
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: ["/api/demat-accounts"] });
			toast({
				title: "Success",
	...
	const setDefaultMutation = useMutation({
		mutationFn: ({
			accountId,
			defaultType,
		}: { accountId: string; defaultType: "equity" | "mutualFunds" }) =>
			apiRequest("PUT", `/api/demat-accounts/${accountId}/set-default`, {
				defaultType,
			}),
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: ["/api/demat-accounts"] });
					...
					<CardHeader>
						...
						<CardDescription>
							{editingAccountId
								...
								: "Enter your demat account information"}
						</CardDescription>
					</CardHeader>
					<CardContent>
						<Form {...form}>
											...
											<FormItem>
												...
												<FormControl>
													<Input
														...
														{...field}
														data-testid="input-demat-account-number"
													/>
												</FormControl>
												<FormMessage />
											</FormItem>
										...
										render={({ field }) => (
											<FormItem>
												...
												<Select
													...
													value={field.value}
													data-testid="select-depository-type"
												>
													<FormControl>
														<SelectTrigger>
								...
								<FormField
									...
									render={({ field }) => (
										<FormItem>
											...
											<FormControl>
												<Input
													placeholder="Enter DP name (e.g., ICICI Securities)"
													{...field}
													data-testid="input-dp-name"
												/>
											</FormControl>
											<FormMessage />
										</FormItem>
									...
									<FormField
										...
										render={({ field }) => (
											<FormItem>
												...
												<FormControl>
													<Input
														placeholder="Enter NSDL Client ID (16 digits)"
														{...field}
														data-testid="input-nsdl-client-id"
													/>
												</FormControl>
												<FormMessage />
											</FormItem>
									...
									<FormField
										...
										render={({ field }) => (
											<FormItem>
												...
												<FormControl>
													<Input
														placeholder="Enter CDSL BO ID (16 digits)"
														{...field}
														data-testid="input-cdsl-bo-id"
													/>
												</FormControl>
												<FormMessage />
											</FormItem>
												...
												<FormItem>
													...
													<FormControl>
														<Input
															...
															{...field}
															data-testid="input-trading-account-number"
														/>
													</FormControl>
													<FormMessage />
												</FormItem>
						...
						<Card key={account.id} className="relative">
								...
								<div className="flex items-start justify-between">
									<div className="space-y-1">
										<CardTitle className="flex items-center gap-2 text-lg">
											...
											{account.isVerified && (
												<Badge
													variant="secondary"
													className="text-green-700 dark:text-green-300 bg-green-50 dark:bg-green-950/30"
												>
									...
									<div className="flex gap-2">
										<Button
											...
											data-testid={`button-edit-demat-${account.id}`}
										>
										...
										<Button
											...
											data-testid={`button-delete-demat-${account.id}`}
										>
							...
							<CardContent className="space-y-4">
								<div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
									...
									{account.depositoryType === "NSDL" &&
										account.nsdlClientId && (
											<div>
									...
									<div className="flex flex-wrap gap-4">
										<div className="flex items-center space-x-2">
											<Checkbox
												...
												onCheckedChange={() =>
													handleSetDefault(account.id, "equity")
												}
											...
											>
												...
												{account.isDefaultForEquityTransactions && (
										...
										<div className="flex items-center space-x-2">
											...
											>
												...
												{account.isDefaultForMutualFundTransactions && (
													<Star className="h-3 w-3 fill-yellow-500 text-yellow-500" />
												)}
											</label>
										</div>
				...
				<Card>
					<CardContent className="flex flex-col items-center justify-center py-12">
						...
						<p className="text-center text-muted-foreground mb-4 max-w-md">
							You haven't added any demat accounts yet. Add your first demat
							account to start securities trading.
						</p>
						<Button
							onClick={() => setIsAddingAccount(true)}
							data-testid="button-add-first-demat-account"
						>
