...
export function ComplianceStatusBadge() {
  ...
  if (isLoading || !status) {
    return (
      <div className="h-6 w-32 animate-pulse bg-muted rounded-full" />
    );
  }
        ...
        <TooltipContent side="bottom" className="w-64 p-3 shadow-xl">
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              ...
              <Badge variant={isCompliant ? "outline" : "destructive"} className="text-[10px] uppercase h-4 px-1.5">
            ...
            <div className="grid grid-cols-2 gap-2 pt-1 border-t text-[11px]">
              ...
              <div>
                ...
                <p className="font-medium">{status.auditStats?.retentionPeriod || '7 Years'}</p>
              </div>
              <div className="col-span-2 pt-1 border-t">
                <p className="text-muted-foreground">Active Alerts</p>
                <p className={cn(
                  "font-medium",
                  status.alerts && status.alerts.length > 0 ? "text-amber-600 dark:text-amber-400" : "text-green-600 dark:text-green-400"
                )}>
                  {status.alerts && status.alerts.length > 0 ? `${status.alerts.length} Pending Actions` : 'System Clear'}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-1 text-[10px] text-muted-foreground pt-2 border-t mt-2">
              ...
              <span>Forensic Heartbeat Active • {status.heartbeat?.lastPulse ? new Date(status.heartbeat.lastPulse).toLocaleTimeString() : 'Live'}</span>
            </div>
          </div>
        </TooltipContent>
      </Tooltip>
