...
export function ForensicAuditTrail() {
	...
	const handleExport = async () => {
		...
		window.open(
			`/api/compliance/audit-log/export?riskLevel=${riskFilter}`,
			"_blank",
		);
	};
	...
	const getRiskBadge = (level: string) => {
		switch (level) {
			case "critical":
				return (
					<Badge className="bg-red-600 text-white border-none">CRITICAL</Badge>
				);
			case "high":
				return (
					<Badge className="bg-orange-500 text-white border-none">HIGH</Badge>
				);
			case "medium":
				return (
					<Badge className="bg-amber-500 text-white border-none">MEDIUM</Badge>
				);
			case "low":
				return (
					<Badge className="bg-blue-500 text-white border-none">LOW</Badge>
				);
			default:
					...
					<CardContent className="pt-6">
						<div className="flex items-center justify-between">
							<div className="flex items-center gap-2">
								<Heart
									className={`w-5 h-5 ${heartbeat?.status === "healthy" ? "text-rose-500 animate-pulse" : "text-muted-foreground"}`}
								/>
						...
						<p className="text-xs text-muted-foreground mt-2">
							...
							{heartbeat?.lastVerified
								? format(new Date(heartbeat.lastVerified), "HH:mm:ss")
								: "N/A"}
						</p>
					</CardContent>
				</Card>
				...
				<Card className="bg-muted/30 border-dashed">
					<CardContent className="pt-6">
						<div className="flex items-center justify-between">
							...
							<Badge
								variant={heartbeat?.integrityCheck ? "default" : "destructive"}
								className={
									heartbeat?.integrityCheck
										? "bg-emerald-500 hover:bg-emerald-600 text-white border-none"
										: ""
								}
							>
									...
									entries.map((entry) => (
										...
										>
											...
											<TableCell className="text-right">
												...
												>
													<ChevronRight
														className={`w-4 h-4 transition-transform ${showDetails === entry.id ? "rotate-90" : ""}`}
													/>
												</Button>
											</TableCell>
										</TableRow>
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
							<div>
								...
								<div className="bg-muted p-3 rounded-md font-mono text-[11px] overflow-auto max-h-[300px]">
									<pre>
										{JSON.stringify(
											entries.find((e) => e.id === showDetails)?.details,
											null,
											2,
										)}
									</pre>
									...
									<div className="space-y-2 p-3 bg-muted rounded-md text-xs">
										<div className="flex justify-between">
											...
											<span className="font-mono text-indigo-600 truncate ml-4">
												{
													entries.find((e) => e.id === showDetails)?.metadata
														.hash
												}
											</span>
										</div>
