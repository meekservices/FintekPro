...
export function KeyFactsStatement({
	kfs,
	onAcknowledge,
	showAcknowledgement = true,
}: KeyFactsStatementProps) {
	...
	return (
		<Card
			...
			data-testid="key-facts-statement"
		>
			...
			<CardContent className="p-6 space-y-6">
				<Alert className="bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-800">
					...
					<AlertDescription className="text-amber-800 dark:text-amber-200">
						...
						<br />
						Please read this document carefully before proceeding with your loan
						application.
					</AlertDescription>
				</Alert>
						...
						<CardContent className="space-y-2 text-sm">
							...
							<div className="flex justify-between">
								...
								<span className="font-mono text-xs">
									{kfs.lenderDetails?.rbiRegistrationNumber}
								</span>
							</div>
						</CardContent>
					</Card>
						...
						<CardContent className="space-y-2 text-sm">
							...
							<div className="flex justify-between">
								...
								<span className="font-bold text-green-600">
									{formatCurrency(kfs.loanSummary?.loanAmountSanctioned || 0)}
								</span>
							</div>
							<div className="flex justify-between">
								<span className="text-muted-foreground">Tenure</span>
								<span className="font-medium">
									{kfs.loanSummary?.tenureMonths} months
								</span>
							</div>
							<div className="flex justify-between">
								<span className="text-muted-foreground">Interest Rate</span>
								<span className="font-medium">
									{kfs.loanSummary?.interestRatePerAnnum}% p.a. (
									{kfs.loanSummary?.interestRateType})
								</span>
							</div>
						</CardContent>
					</Card>
					...
					<CardContent>
						...
						<div className="grid grid-cols-2 gap-4 text-sm">
							<div className="flex justify-between">
								...
								<span>
									{kfs.annualPercentageRate?.aprBreakdown?.baseInterestRate}%
								</span>
							</div>
							<div className="flex justify-between">
								<span className="text-muted-foreground">
								...
								<span>
									{kfs.annualPercentageRate?.aprBreakdown?.processingFeeImpact}%
								</span>
							</div>
						</div>
						<p className="text-xs text-muted-foreground mt-3">
							Calculation Method:{" "}
							{kfs.annualPercentageRate?.aprCalculationMethod}
						</p>
					</CardContent>
				</Card>
					...
					<Card>
						...
						<CardContent className="space-y-2 text-sm">
							<div className="flex justify-between">
								...
								<span>
									{formatCurrency(
										kfs.feesAndCharges?.processingFee?.amount || 0,
									)}{" "}
									({kfs.feesAndCharges?.processingFee?.percent}%)
								</span>
							</div>
							<div className="flex justify-between">
								<span className="text-muted-foreground">Stamp Duty</span>
								<span>
									{formatCurrency(kfs.feesAndCharges?.stampDuty || 0)}
								</span>
							</div>
							<div className="flex justify-between">
							...
							<div className="flex justify-between">
								...
								<span>
									{formatCurrency(
										kfs.feesAndCharges?.documentationCharges || 0,
									)}
								</span>
							</div>
							...
							<div className="flex justify-between font-bold">
								...
								<span className="text-red-600">
									{formatCurrency(kfs.feesAndCharges?.totalUpfrontCharges || 0)}
								</span>
							</div>
							<div className="flex justify-between font-bold text-green-600">
								...
								<span>
									{formatCurrency(
										kfs.feesAndCharges?.netDisbursementAmount || 0,
									)}
								</span>
							</div>
						</CardContent>
						...
						<CardContent className="space-y-2 text-sm">
							<div className="flex justify-between">
								...
								<span className="text-2xl font-bold text-blue-600">
									{formatCurrency(kfs.loanSummary?.emiAmount || 0)}
								</span>
							</div>
							<div className="flex justify-between">
							...
							<div className="flex justify-between">
								...
								<span className="text-orange-600">
									{formatCurrency(kfs.loanSummary?.totalInterestPayable || 0)}
								</span>
							</div>
							<div className="flex justify-between font-bold">
								<span>Total Amount Repayable</span>
								<span>
									{formatCurrency(kfs.loanSummary?.totalAmountRepayable || 0)}
								</span>
							</div>
						</CardContent>
					</Card>
				...
				<Card className="bg-purple-50 dark:bg-purple-900/20 border-purple-200 dark:border-purple-800">
					...
					<CardContent>
						<Alert className="bg-card">
							...
							<AlertDescription>
								<strong>
									You have {kfs.coolingOffPeriod?.lookUpPeriodDays} days
								</strong>{" "}
								from the date of disbursement to exit this loan without any
								penalty.
								{kfs.coolingOffPeriod?.exitOptionAvailable && (
									<span>
										{" "}
										Principal amount plus proportionate interest will be
										refunded within {kfs.coolingOffPeriod?.refundProcessDays}{" "}
										working days.
									</span>
								)}
							</AlertDescription>
					...
					<Card>
						...
						<CardContent className="space-y-2 text-sm">
							<div className="flex justify-between">
								...
								<Badge
									variant={
										kfs.prepaymentTerms?.prepaymentAllowed
											? "default"
											: "secondary"
									}
								>
							...
							<div className="flex justify-between">
								...
								<span>
									{formatCurrency(
										kfs.prepaymentTerms?.partPrepaymentMinAmount || 0,
									)}
								</span>
							</div>
							<div className="flex justify-between">
					...
					<Card>
						...
						<CardContent className="space-y-2 text-sm">
							<div className="flex justify-between">
								...
								<span>
									{kfs.penalCharges?.latePaymentChargePercent}% or{" "}
									{formatCurrency(
										kfs.penalCharges?.latePaymentChargeFixed || 0,
									)}
								</span>
							</div>
							<div className="flex justify-between">
								...
								<span>
									{formatCurrency(kfs.penalCharges?.bounceCharges || 0)}
								</span>
							</div>
						</CardContent>
					</Card>
						...
						<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
							<div className="p-4 bg-muted rounded-lg">
								...
								<div className="text-sm space-y-1 text-muted-foreground">
									...
									<div className="text-xs">
										Response:{" "}
										{kfs.grievanceRedressal?.level1?.responseTimeBusinessDays}{" "}
										business days
									</div>
								</div>
							</div>
							...
							<div className="p-4 bg-muted rounded-lg">
								...
								<div className="text-sm space-y-1 text-muted-foreground">
									...
									<div className="text-xs">
										Response:{" "}
										{kfs.grievanceRedressal?.level2?.responseTimeBusinessDays}{" "}
										business days
									</div>
								</div>
							</div>
							...
							<div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
								...
								<div className="text-sm space-y-1 text-muted-foreground">
									<div className="flex items-center gap-2">
										...
										<a
											href={kfs.grievanceRedressal?.rbiOmbudsman?.portalLink}
											target="_blank"
									...
									<div className="text-xs">
										Escalate after:{" "}
										{
											kfs.grievanceRedressal?.rbiOmbudsman
												?.escalationTimelineDays
										}{" "}
										days
									</div>
								</div>
					...
					<Card className="border-2 border-green-200 dark:border-green-800">
						...
						<CardContent className="space-y-4">
							<div className="grid grid-cols-2 gap-4 text-sm">
								<div>
									...
									<span className="ml-2 font-medium">
										{kfs.acknowledgementRequired?.borrowerName}
									</span>
								</div>
								<div>
									<span className="text-muted-foreground">PAN:</span>
									<span className="ml-2 font-mono">
										{kfs.acknowledgementRequired?.borrowerPan}
									</span>
								</div>
							</div>
							...
							<div className="flex items-center space-x-2">
								...
								>
									I have read and understood all the terms in this Key Facts
									Statement
								</label>
							</div>
						</CardContent>
