...
export function TreasuryDashboard() {
	...
	const { data: forecastResponse, isLoading: isLoadingForecast } =
	...
	const syncMutation = useMutation({
		mutationFn: async () => {
			const response = await fetch(`/api/treasury/entities/${entityId}/sync`, {
				method: "POST",
			});
		...
		onSuccess: () => {
			...
			toast({
				title: "Sync Complete",
				description:
					"Your treasury balances have been updated across all banks.",
			});
			setIsSyncing(false);
		},
		onError: () => {
			toast({
				title: "Sync Failed",
				description:
					"Could not synchronize with banking providers. Please try again.",
				variant: "destructive",
			});
			setIsSyncing(false);
		},
	...
	const allocationData = positionData?.data?.breakdown?.bankBalances?.map(
		(b) => ({
			name: b.bankName,
			value: b.balance,
			color: b.bankName.includes("HDFC")
				? "#1e40af"
				: b.bankName.includes("ICICI")
					? "#ea580c"
					: "#2563eb",
		}),
	) || [
	...
	const consolidatedCash = positionData?.data?.totalBalance
		? `₹${(positionData.data.totalBalance / 10000000).toFixed(2)} Cr`
		: "₹12.45 Cr";
	const aiInsight =
		aiAnalysisResponse?.data ||
		"Analyzing your cash flow patterns for risks and opportunities...";
		...
		>
			...
			<div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
				<div>
					...
					<p className="text-slate-500 dark:text-slate-400">
						Real-time liquidity & corporate cash management
					</p>
				</div>
			...
			<Tabs defaultValue="overview" className="space-y-6">
				<TabsList className="bg-white/80 dark:bg-slate-900/80 border shadow-sm p-1 backdrop-blur-md sticky top-0 z-10">
					...
					<TabsTrigger value="liquidity" className="px-6">
						Liquidity Forecast
					</TabsTrigger>
					<TabsTrigger value="banks" className="px-6">
						Bank Accounts
					</TabsTrigger>
					<TabsTrigger value="payouts" className="px-6">
						Payout History
					</TabsTrigger>
				</TabsList>
					...
					>
						...
						<Card className="border-none shadow-sm bg-white dark:bg-slate-900">
							<CardHeader>
								...
								<CardDescription>
									Consolidated balance distribution
								</CardDescription>
							</CardHeader>
							<CardContent>
									...
									{allocationData.map((bank) => (
										...
										>
											<div className="flex items-center gap-2">
												<div
													className={`w-3 h-3 rounded-full ${getBankColorClass(bank.color)}`}
												/>
							...
							<CardHeader className="flex flex-row items-center justify-between">
								<div>
									<CardTitle className="text-lg">
										Recent Treasury Payouts
									</CardTitle>
									<CardDescription>
										Monitor your corporate disbursements
									</CardDescription>
								</div>
						...
						<Card className="lg:col-span-3 border-none shadow-sm bg-white dark:bg-slate-900">
							<CardHeader className="flex flex-row items-center justify-between">
								<div>
									...
									<CardDescription>
										30-day liquidity projection based on historical patterns
									</CardDescription>
								</div>
								<Badge
									variant="outline"
				...
				<TabsContent value="banks">{/* Bank Management Content */}</TabsContent>
