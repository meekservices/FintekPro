...
import { Building2, CreditCard, CheckCircle2, AlertCircle, ChevronDown, Edit2 } from "lucide-react";
...
export function AccountSelectionWidget({
  ...
  const [dematAccountId, setDematAccountId] = useState<string>(selectedDematAccountId || "");
  ...
  useEffect(() => {
    if (requiresDemat && !selectedDematAccountId && dematAccounts?.length) {
      const preferredDematId = preference?.dematAccountId || dematAccounts[0]?.id;
      if (preferredDematId) {
  ...
  const verifiedBankAccounts = bankAccounts?.filter(acc => acc.isVerified) || [];
  const verifiedDematAccounts = dematAccounts?.filter(acc => acc.isVerified) || [];
  ...
  if (compact) {
    return (
      <div className={cn("space-y-3", className)} data-testid="account-selection-compact">
        <div className="flex items-center gap-2">
          <Building2 className="h-4 w-4 text-muted-foreground" />
          <Select value={bankAccountId} onValueChange={handleBankAccountChange} data-testid="select-bank-compact">
        ...
        {requiresDemat && (
          <div className="flex items-center gap-2">
            ...
            <Select value={dematAccountId} onValueChange={handleDematAccountChange} data-testid="select-demat-compact">
              <SelectTrigger className="h-9">
              ...
              <SelectContent>
                {verifiedDematAccounts.map((account) => (
                  <SelectItem key={account.id} value={account.id}>
                    {account.dematDpName} - {account.dematAccountNumber.slice(-4)}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
  ...
  return (
    <Card className={cn("bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-950/20 dark:to-indigo-950/20", className)} data-testid="account-selection-card">
      <CardContent className="p-4">
        <div className="space-y-3">
          {showLabels && (
            <div className="flex items-center justify-between">
              <h3 className="font-semibold text-sm">Payment & Holding Accounts</h3>
              <Button 
                variant="ghost" 
                size="sm" 
              ...
              >
                {isEditing ? "Done" : <><Edit2 className="h-3 w-3 mr-1" />Edit</>}
              </Button>
            </div>
          )}
          ...
          <div className="space-y-2">
            <label className="text-xs font-medium text-muted-foreground">Bank Account (for payment)</label>
            {isEditing ? (
              <Select value={bankAccountId} onValueChange={handleBankAccountChange} data-testid="select-bank-edit">
                <SelectTrigger>
                  <SelectValue placeholder="Select bank account" />
                </SelectTrigger>
                <SelectContent>
              ...
              <div className="flex items-center justify-between p-3 bg-white dark:bg-gray-900 rounded-lg border" data-testid="display-selected-bank">
                <div className="flex items-center gap-2">
              ...
              <Alert className="py-2" data-testid="alert-no-bank-selected">
                ...
                <AlertDescription className="text-xs">No bank account selected</AlertDescription>
              </Alert>
            )}
          </div>
          ...
          {requiresDemat && (
            <div className="space-y-2">
              <label className="text-xs font-medium text-muted-foreground">Demat Account (for holdings)</label>
              {isEditing ? (
                <Select value={dematAccountId} onValueChange={handleDematAccountChange} data-testid="select-demat-edit">
                  <SelectTrigger>
                    <SelectValue placeholder="Select demat account" />
                  </SelectTrigger>
                  <SelectContent>
                    {verifiedDematAccounts.map((account) => (
                      <SelectItem key={account.id} value={account.id}>
                        <div className="flex items-center justify-between w-full">
                          <span>{account.dematDpName} - {account.dematAccountNumber.slice(-4)}</span>
                          <CheckCircle2 className="h-3 w-3 text-green-600 ml-2" />
                        </div>
                      </SelectItem>
                    ))}
                ...
                <div className="flex items-center justify-between p-3 bg-white dark:bg-gray-900 rounded-lg border" data-testid="display-selected-demat">
                  <div className="flex items-center gap-2">
                    <CreditCard className="h-4 w-4 text-indigo-600" />
                    <div>
                      <p className="font-medium text-sm">{selectedDemat.dematDpName}</p>
                      <p className="text-xs text-muted-foreground">
                        {selectedDemat.depositoryType} - ****{selectedDemat.dematAccountNumber.slice(-4)}
                      </p>
                    </div>
                  </div>
                ...
                <Alert className="py-2" data-testid="alert-no-demat-selected">
                  <AlertCircle className="h-3 w-3" />
                  <AlertDescription className="text-xs">No demat account selected</AlertDescription>
                </Alert>
              )}
            </div>
          )}
