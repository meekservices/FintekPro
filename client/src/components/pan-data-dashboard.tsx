...
export function PANDataDashboard({ className }: PANDataDashboardProps) {
  ...
  const { data: panStatus, isLoading: statusLoading, error: statusError } = useQuery({
    queryKey: ['/api/pan-consent/my-data', refreshKey],
    retry: 2
  }) as { data?: { success: boolean; data: any } | undefined; isLoading: boolean; error: any };
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                      <code 
                        className="text-lg font-mono bg-gray-100 dark:bg-gray-800 px-3 py-2 rounded-md"
                        data-testid="text-masked-pan"
                      >
                        {panStatus?.data?.maskedPan || 'N/A'}
                      </code>
                      <Badge 
                        variant={panStatus?.data?.consentDetails?.panVerified ? "default" : "secondary"}
                  ...
                  <div className="space-y-2">
                    ...
                    <Badge 
                      variant={panStatus?.data?.consentDetails?.kycVerified ? "default" : "outline"}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                  <div>
                    <p className="text-gray-500 dark:text-gray-400">Consent Given</p>
              ...
              <CardContent className="space-y-4">
                {panStatus?.data?.compliance?.isCompliant ? (
                  <Alert className="border-green-200 bg-green-50 dark:bg-green-900/10">
                    ...
                    <AlertDescription className="text-green-800 dark:text-green-200">
                      Your PAN consent is fully compliant with all regulatory requirements.
                    </AlertDescription>
                  </Alert>
                ) : (
                  <Alert variant="destructive">
                    <AlertTriangle className="h-4 w-4" />
                    <AlertDescription>
                      Compliance issues detected. Please review the details below.
                    </AlertDescription>
                  </Alert>
                )}
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant={panStatus?.data?.compliance?.needsRenewal ? "destructive" : "default"}>
