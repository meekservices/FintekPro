		...
		switch (status) {
			case "offline":
				return {
					...
					message:
						"Some features are unavailable. Changes will sync when connected.",
					dismissible: false,
				};
			case "server-error":
				return {
			...
			<div className="flex items-center gap-3 flex-1">
				<Icon
					className={cn(
						...
						isServerError && retryCount > 0 && "animate-pulse",
					)}
				/>
				<div className="flex flex-col sm:flex-row sm:items-center sm:gap-2">
					<span
						className="font-semibold text-sm"
						data-testid="network-status-title"
					>
						{config.title}
					</span>
			...
			<div className="flex items-center gap-2">
				...
				>
					<RefreshCw
						className={cn(
							"h-4 w-4",
							(isChecking || (isServerError && retryCount > 0)) &&
								"animate-spin",
						)}
					/>
...
export function OfflineGuard({
	...
	if (isOffline) {
		...
		return (
			<div
				className="flex flex-col items-center justify-center p-6 text-center bg-muted/50 rounded-lg border border-dashed"
