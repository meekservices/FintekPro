...
export function BankingTab() {
	...
	const addAccountMutation = useMutation({
		mutationFn: (data: BankAccountForm) =>
			apiRequest("/api/bank-accounts", "POST", { body: data }),
		onSuccess: () => {
	...
	const deleteAccountMutation = useMutation({
		mutationFn: (id: string) =>
			apiRequest(`/api/bank-accounts/${id}`, "DELETE"),
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: ["/api/bank-accounts"] });
			toast({
				title: "Success",
	...
	const setDefaultMutation = useMutation({
		mutationFn: ({
			accountId,
			defaultType,
		}: { accountId: string; defaultType: "mutualFunds" }) =>
			apiRequest(`/api/bank-accounts/${accountId}/set-default`, "PUT", {
				body: { defaultType },
			}),
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: ["/api/bank-accounts"] });
	...
	const verifyAccountMutation = useMutation({
		mutationFn: (accountId: string) =>
			apiRequest("/api/bank-accounts/verify-penny-drop", "POST", {
				body: { accountId },
			}),
		onSuccess: (data: any) => {
			queryClient.invalidateQueries({ queryKey: ["/api/bank-accounts"] });
		...
		<div className="space-y-6">
				...
				<CardHeader>
					<div className="flex items-center justify-between">
						<div>
							...
							<CardDescription>
								Manage up to 5 bank accounts. Set default accounts for mutual
								fund transactions.
							</CardDescription>
						</div>
			...
			{bankAccounts && bankAccounts.length >= 5 && (
				<Alert>
					...
					<AlertDescription>
						You have reached the maximum limit of 5 bank accounts. Remove an
						existing account to add a new one.
					</AlertDescription>
				</Alert>
			)}
										...
										<div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-green-50 dark:bg-green-950 rounded-lg border border-green-200 dark:border-green-800">
											...
											<div className="md:col-span-2">
												...
												<p className="text-sm text-muted-foreground mt-1">
													{ifscDetails.address}, {ifscDetails.city},{" "}
													{ifscDetails.state}
												</p>
											</div>
										</div>
						...
						{bankAccounts.map((account) => (
							<Card key={account.id} className="relative">
								...
								<CardContent className="pt-0">
									...
									{account.verifiedAccountHolderName &&
										...
										account.nameMatchScore < 80 && (
											<Alert className="mb-4">
												...
												<AlertDescription>
													<strong>Name Mismatch:</strong> Bank shows "
													{account.verifiedAccountHolderName}" but you provided
													"{account.accountHolderName}" (Match:{" "}
													{account.nameMatchScore}%). Please update the name or
													contact support.
												</AlertDescription>
											</Alert>
										)}
									...
									{account.verificationAttempts &&
										account.verificationAttempts >= 3 &&
										!account.isVerified && (
											<Alert className="mb-4">
												<AlertCircle className="h-4 w-4" />
												<AlertDescription>
													Maximum verification attempts reached. Please contact
										...
										(!account.verificationAttempts ||
											...
											<div className="mb-4">
												<Button
													...
													size="sm"
													onClick={() =>
														verifyAccountMutation.mutate(account.id)
													}
												...
												<p className="text-xs text-muted-foreground mt-2">
													₹1 will be deposited to verify account.{" "}
													{account.verificationAttempts
														? `${3 - account.verificationAttempts} attempts remaining`
