    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <Label htmlFor="sip-sim-amount" className="text-sm">Monthly SIP</Label>
          ...
          <div>
            ...
            <Tabs value={horizonMonths} onValueChange={(v) => setHorizonMonths(v as "6" | "12" | "24")} className="mt-1">
              <TabsList className="grid w-full grid-cols-3">
                <TabsTrigger value="6" className="text-xs">6 mo</TabsTrigger>
                <TabsTrigger value="12" className="text-xs">12 mo</TabsTrigger>
                <TabsTrigger value="24" className="text-xs">24 mo</TabsTrigger>
              </TabsList>
            </Tabs>
          </div>
        </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="p-3 bg-muted/50 rounded-lg text-center">
                ...
                <div className="text-lg font-bold">{simulation.horizonMonths}mo</div>
                <div className="text-xs text-muted-foreground">Horizon</div>
              </div>
              <div className="p-3 bg-muted/50 rounded-lg text-center">
                <IndianRupee className="h-4 w-4 mx-auto mb-1 text-muted-foreground" />
                <div className="text-lg font-bold">₹{(simulation.totalInvested / 100000).toFixed(1)}L</div>
                <div className="text-xs text-muted-foreground">Total SIP</div>
              </div>
              <div className="p-3 bg-green-50 dark:bg-green-950/30 rounded-lg text-center">
                <TrendingUp className="h-4 w-4 mx-auto mb-1 text-green-600" />
                <div className="text-lg font-bold text-green-600">+{simulation.scoreImprovement}</div>
            ...
            <div className="p-4 border rounded-lg space-y-3">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className={cn("text-2xl font-bold", getScoreColor(simulation.diversificationScoreStart))}>
                ...
                <div className="text-right">
                  ...
                  <p className={cn("text-2xl font-bold", getScoreColor(simulation.diversificationScoreEnd))}>
            ...
            <div>
              ...
              <div className="flex items-end gap-1 h-20">
                {simulation.monthlySnapshots.map((snapshot, idx) => {
                  const height = ((snapshot.diversificationScore - simulation.diversificationScoreStart) /
                    Math.max(simulation.scoreImprovement, 1)) * 100;
                  return (
                    <div
                      key={idx}
