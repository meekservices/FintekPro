        ...
        <div className="flex items-center justify-between py-3 px-4 hover:bg-muted/50 cursor-pointer border-b border-border/50 transition-colors">
          <div className="flex items-center gap-3 flex-1">
            ...
            <div className="flex-1">
              ...
              {overlap.sector && (
                <div className="text-xs text-muted-foreground">{overlap.sector}</div>
              )}
            </div>
          </div>
          <div className="flex items-center gap-4">
            <div className="text-right">
              <div className="font-semibold text-sm">{overlap.totalExposure.toFixed(2)}%</div>
              <div className="text-xs text-muted-foreground">{overlap.fundCount} funds</div>
            </div>
          ...
          <div className="space-y-2">
            {overlap.funds.map((fund, idx) => (
              <div key={idx} className="flex items-center justify-between text-sm">
                <span className="text-muted-foreground truncate max-w-[60%]">{fund.name}</span>
                <div className="flex items-center gap-3">
                  <span className="text-xs text-muted-foreground">
                    ({fund.portfolioWeight.toFixed(1)}% weight)
                  </span>
                  <span className="font-medium">{fund.contribution.toFixed(2)}%</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="flex h-4 rounded-full overflow-hidden bg-muted">
        {sectors.slice(0, 8).map((sector, idx) => (
          <div
            ...
            style={{ width: `${(sector.exposure / Math.max(totalExposure, 100)) * 100}%` }}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
        {sectors.slice(0, 8).map((sector, idx) => (
          <div key={sector.sector} className="flex items-center gap-2 text-xs">
            ...
            <span className="font-medium ml-auto">{sector.exposure.toFixed(1)}%</span>
          </div>
        ))}
      </div>
    </div>
...
export function StockOverlapAnalysis({
  ...
  const {
    ...
    queryKey: ["/api/stock-intersection/analyze", funds.map((f) => f.mfIsin).join(",")],
    queryFn: async () => {
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Import portfolio holdings to analyze stock overlap across mutual funds
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex items-center gap-3 p-4 bg-muted/50 rounded-lg">
            ...
            <p className="text-sm text-muted-foreground">
              No mutual fund holdings found with ISIN data. Import holdings to enable overlap analysis.
            </p>
          </div>
        </CardContent>
      </Card>
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>Analyzing stock holdings across {funds.length} mutual funds...</CardDescription>
        </CardHeader>
          ...
          <div className="flex items-center justify-between p-4 bg-red-50 dark:bg-red-950/30 rounded-lg">
            <div className="flex items-center gap-3">
              ...
              <p className="text-sm text-red-600 dark:text-red-400">
                Failed to analyze stock overlap. Holdings data may be unavailable.
              </p>
            </div>
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>
              Analyzing {analysis.totalFundsAnalyzed} funds with {analysis.totalStocksFound} unique stocks
            </CardDescription>
          </div>
          <Button
            variant="outline"
      ...
      <CardContent className="space-y-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="bg-muted/50 rounded-lg p-4 text-center">
            <div className="text-3xl font-bold">{analysis.overlappingStocksCount}</div>
            <div className="text-xs text-muted-foreground mt-1">Overlapping Stocks</div>
          </div>
          <div className="bg-red-50 dark:bg-red-950/30 rounded-lg p-4 text-center">
            <div className="text-3xl font-bold text-red-600">{analysis.highRiskStocksCount}</div>
          ...
          <div className="bg-amber-50 dark:bg-amber-950/30 rounded-lg p-4 text-center">
            <div className="text-3xl font-bold text-amber-600">{analysis.mediumRiskStocksCount}</div>
            <div className="text-xs text-amber-600/80 mt-1">Medium Risk Stocks</div>
          </div>
        </div>
        ...
        {analysis.overlappingStocksCount === 0 && (
          <div className="flex items-center gap-3 p-4 bg-green-50 dark:bg-green-950/30 rounded-lg">
            ...
            <p className="text-sm text-green-700 dark:text-green-300">
              Excellent! No significant stock overlap detected across your mutual funds.
            </p>
          </div>
        )}
      </CardContent>
