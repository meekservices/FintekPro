...
const PHASE_CONFIG: Record<ProposalPhase, { label: string; icon: React.ReactNode; description: string }> = {
  risk_profile: { 
    label: 'Risk Profile', 
...
export function PhaseLockStepper({ proposalId, onPhaseClick, currentActivePhase }: PhaseLockStepperProps) {
  const { data: flowStatus, isLoading, error } = useQuery<ProposalFlowStatus>({
    queryKey: ['/api/proposal-builder/flow-state', proposalId],
    queryFn: async () => {
      const response = await fetch(`/api/proposal-builder/flow-state/${proposalId}`);
                        ...
                        <div className="mt-2">
                          ...
                          <ul className="text-xs text-muted-foreground">
                            {validation.missingPrerequisites.map(prereq => (
                              <li key={prereq}>• {PHASE_CONFIG[prereq].label}</li>
                            ))}
                          </ul>
                        </div>
                      )}
