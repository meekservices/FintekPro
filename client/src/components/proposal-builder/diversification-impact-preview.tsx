...
export function DiversificationImpactPreview({
	impact,
	compact = false,
}: DiversificationImpactPreviewProps) {
	const { currentScore, projectedScore, netImprovement, changesApplied } =
		impact;
	...
	const ImprovementIcon =
		netImprovement > 0 ? TrendingUp : netImprovement < 0 ? TrendingDown : Minus;
	...
	if (compact) {
		return (
			<div className="flex items-center gap-2 p-2 bg-muted/50 rounded-lg">
				...
				<Badge
					variant="outline"
					className={cn("text-xs ml-auto", improvementColor)}
				>
				...
				{netImprovement > 0 && (
					<div className="mt-4 p-3 bg-green-50 dark:bg-green-950/30 rounded-lg">
						<p className="text-sm text-green-700 dark:text-green-300">
							The proposed changes improve your diversification score by{" "}
							{netImprovement} points without changing your risk profile.
						</p>
					</div>
				)}
				...
				{netImprovement < 0 && (
					<div className="mt-4 p-3 bg-red-50 dark:bg-red-950/30 rounded-lg">
						<p className="text-sm text-red-700 dark:text-red-300">
							Warning: The proposed changes would reduce your diversification
							score by {Math.abs(netImprovement)} points. Consider alternative
							options.
						</p>
					</div>
				)}
