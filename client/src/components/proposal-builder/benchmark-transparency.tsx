...
export function BenchmarkTransparency({
	...
	const { data, isLoading, error } = useQuery<{
		...
		queryKey: [
			"/api/proposal-builder/benchmarks/select",
			{ goalType, riskProfile, horizonYears },
		],
		queryFn: async () => {
			...
			const response = await fetch(
				`/api/proposal-builder/benchmarks/select?${params}`,
			);
	...
	const overrideBenchmark = useMutation({
		...
		onSuccess: () => {
			queryClient.invalidateQueries({
				queryKey: ["/api/proposal-builder/benchmarks/select"],
			});
