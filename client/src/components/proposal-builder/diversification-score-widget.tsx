...
export function DiversificationScoreWidget({ data, compact = false }: DiversificationScoreWidgetProps) {
  ...
  const penaltySummary = useMemo(() => {
    const totalPenalty = penalties.reduce((sum, p) => sum + Math.abs(p.impact), 0);
    const stockPenalties = penalties.filter(p => p.type === "STOCK_OVERLAP").length;
    const sectorPenalties = penalties.filter(p => p.type === "SECTOR_CONCENTRATION").length;
    const crowdingPenalties = penalties.filter(p => p.type === "FUND_CROWDING").length;
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <div className={cn(
              ...
              <span className={cn("font-bold text-sm", config.color)}>{score}</span>
          ...
          <TooltipContent side="bottom" className="max-w-xs">
            ...
            <p className="text-xs text-muted-foreground">
              Score reflects diversification after accounting for overlapping stocks and sectors.
            </p>
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
    ...
    <Card className={cn("border-2", config.border)}>
      <CardHeader className={cn("pb-3", config.bg)}>
        <div className="flex items-center justify-between">
          ...
          <Badge variant="outline" className={cn("text-lg font-bold px-3 py-1", config.color, config.bg, config.border)}>
            {grade}
          </Badge>
        </div>
      </CardHeader>
      <CardContent className="pt-4 space-y-4">
          ...
          <div className="flex-1 space-y-2">
            ...
            {penaltySummary.totalPenalty > 0 && (
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium text-red-600">-{penaltySummary.totalPenalty}</span>
              </div>
            )}
              ...
              {penalties.slice(0, 5).map((penalty, idx) => (
                ...
                >
                  <div className="flex items-center gap-2 flex-1">
                    ...
                    {penalty.exposure && (
                      <span className="text-muted-foreground">({penalty.exposure.toFixed(1)}%)</span>
                    )}
                  </div>
                  <span className="font-medium text-red-600 ml-2">{penalty.impact}</span>
                </div>
              ))}
              {penalties.length > 5 && (
        ...
        <div className="grid grid-cols-3 gap-2 pt-2">
          <div className="text-center p-2 bg-red-50 dark:bg-red-950/30 rounded">
            <div className="text-lg font-bold text-red-600">{penaltySummary.stockPenalties}</div>
          ...
          <div className="text-center p-2 bg-purple-50 dark:bg-purple-950/30 rounded">
            <div className="text-lg font-bold text-purple-600">{penaltySummary.sectorPenalties}</div>
          ...
          <div className="text-center p-2 bg-amber-50 dark:bg-amber-950/30 rounded">
            <div className="text-lg font-bold text-amber-600">{penaltySummary.crowdingPenalties}</div>
