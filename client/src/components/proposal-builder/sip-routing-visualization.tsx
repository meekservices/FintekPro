      ...
      <CardContent className="space-y-4">
        <div className="flex items-end gap-4">
          <div className="flex-1">
            <Label htmlFor="sip-amount" className="text-sm">Monthly SIP Amount</Label>
          ...
          >
            <RefreshCw className={cn("h-4 w-4 mr-2", (isFetching || isOptimizing) && "animate-spin")} />
            Optimize
          </Button>
        </div>
          ...
          <>
            <div className="space-y-3">
              ...
              {routingResult.sipRouting.map((routing, idx) => {
                const percentage = (routing.amount / routingResult.totalAllocated) * 100;
                return (
                  <div key={idx} className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        <span className="text-sm font-bold">₹{routing.amount.toLocaleString()}</span>
                    ...
                    <div className="relative h-2 bg-muted rounded-full overflow-hidden">
                      <div
                        className={cn("h-full transition-all", getOverlapColor(routing.overlapScore))}
                ...
                <TooltipTrigger asChild>
                  <div className="flex items-start gap-2 p-3 bg-blue-50 dark:bg-blue-950/30 rounded-lg cursor-help">
                    ...
                    <p className="text-xs text-blue-700 dark:text-blue-300">
                      <strong>Why this split?</strong> {routingResult.explanation}
                    </p>
                  </div>
                </TooltipTrigger>
                <TooltipContent side="bottom" className="max-w-sm">
                  <p className="text-xs">
                    Allocation is calculated to minimize incremental stock overlap with your existing holdings.
                    Lower overlap funds receive proportionally higher SIP amounts.
                  </p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
