			...
			<CardHeader className="pb-3">
				<div className="flex items-center justify-between">
					<div>
						...
						<CardDescription>Stock concentration across funds</CardDescription>
					</div>
					<TooltipProvider>
						<Tooltip>
							...
							<TooltipContent className="max-w-xs">
								<p className="text-xs">
									Each cell shows portfolio exposure (%) to a stock through a
									fund. Darker colors = higher concentration risk.
								</p>
							</TooltipContent>
						</Tooltip>
					</TooltipProvider>
					...
					<div className="min-w-max">
						...
						<div className="flex">
							...
							{fundNames.map((name, idx) => (
								<div key={idx} className="w-16 px-1 text-center" title={name}>
								...
								{matrix[rowIdx].map((value, colIdx) => (
									<TooltipProvider key={colIdx}>
										<Tooltip>
											...
											<TooltipContent>
												<p className="text-xs">
													<strong>{stock}</strong> via{" "}
													<strong>{fundNames[colIdx]}</strong>
													<br />
