...
export function SEBIAuditExport({
	proposalId,
	isAdmin = false,
}: SEBIAuditExportProps) {
	...
	const handleExportCSV = async () => {
		try {
			const response = await fetch(
				`/api/sebi-audit/export?proposalId=${proposalId}`,
			);
	...
	const StatusIcon = summary
		? statusConfig[summary.complianceStatus].icon
		: Clock;
					...
					<>
						...
						<div>
							...
							<div className="flex flex-wrap gap-2">
								{Object.entries(summary.actionBreakdown).map(
									([action, count]) => (
						...
						{filteredLogs && filteredLogs.length > 10 && (
							<p className="text-xs text-center text-muted-foreground">
								Showing 10 of {filteredLogs.length} logs. Export for full audit
								trail.
							</p>
						)}
					</>
