...
export function VerdictEnforcement({
	proposalId,
	onVerdictComplete,
}: VerdictEnforcementProps) {
	...
	const { data: validation } = useQuery<VerdictValidation>({
		...
		queryFn: async () => {
			const response = await fetch(
				`/api/proposal-builder/verdicts/${proposalId}/validate`,
			);
	...
	const { data: summary, isLoading } = useQuery<VerdictSummary>({
		...
		queryFn: async () => {
			const response = await fetch(
				`/api/proposal-builder/verdicts/${proposalId}/summary`,
			);
	...
	const assignVerdict = useMutation({
		...
		onSuccess: () => {
			queryClient.invalidateQueries({
				queryKey: ["/api/proposal-builder/verdicts", proposalId],
			});
			setShowDialog(false);
					...
					<div className="grid grid-cols-3 gap-4">
						<div className="text-center p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
							...
							<div className="text-sm font-medium">
								{formatCurrency(summary.buyTotal)}
							</div>
						</div>
						<div className="text-center p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
							...
							<div className="text-sm font-medium">
								{formatCurrency(summary.holdTotal)}
							</div>
						</div>
						<div className="text-center p-3 bg-red-50 dark:bg-red-900/20 rounded-lg">
							...
							<div className="text-sm font-medium">
								{formatCurrency(summary.sellTotal)}
							</div>
						</div>
					</div>
				)}
				...
				<Dialog open={showDialog} onOpenChange={setShowDialog}>
					<DialogContent>
						<DialogHeader>
							...
							<DialogDescription>
								Select a verdict for this instrument
							</DialogDescription>
						</DialogHeader>
						<div className="space-y-4 py-4">
							...
							<div className="space-y-2">
								...
								<Select
									value={verdictValue}
									onValueChange={(v) => setVerdictValue(v as Verdict)}
								>
									<SelectTrigger>
										<SelectValue />
									</SelectTrigger>
