...
export function WhatIfSimulator({ proposalId }: WhatIfSimulatorProps) {
	...
	const { data: scenarios, isLoading } = useQuery<{
		...
		queryFn: async () => {
			const response = await fetch(
				`/api/proposal-builder/what-if/${proposalId}/scenarios`,
			);
		...
		mutationFn: async () => {
			return apiRequest(`/api/proposal-builder/what-if/${proposalId}`, {
				...
				body: JSON.stringify({
					mode,
					assumptions:
						mode === "interactive" ? { returnDelta, volatility } : undefined,
				}),
			});
		},
		onSuccess: () => {
			queryClient.invalidateQueries({
				queryKey: ["/api/proposal-builder/what-if", proposalId],
			});
			toast({
	...
	const toggleReportInclusion = useMutation({
		mutationFn: async ({
			scenarioName,
			include,
		}: { scenarioName: string; include: boolean }) => {
			return apiRequest(
				`/api/proposal-builder/what-if/${proposalId}/toggle-report`,
				{
					method: "POST",
	...
	return (
		<Card>
			...
			<CardContent className="space-y-6">
				<Tabs
					value={mode}
					onValueChange={(v) => setMode(v as "static" | "interactive")}
				>
					...
					<TabsContent value="static" className="space-y-4">
						<p className="text-sm text-muted-foreground">
							Generate standard market scenarios: Base Case, Bull (+10%), Bear
							(-10%), and Bear (-20%).
						</p>
						<Button
						...
						<div className="space-y-4">
							<div className="space-y-2">
								<div className="flex items-center justify-between">
									...
									<span
										className={`font-mono text-sm ${returnDelta > 0 ? "text-green-600" : returnDelta < 0 ? "text-red-600" : ""}`}
									>
										{returnDelta > 0 ? "+" : ""}
										{returnDelta}%
									</span>
						...
						<Table>
							...
							<TableBody>
								{scenarios.scenarios.map((scenario) => {
									const config =
										SCENARIO_LABELS[scenario.scenarioName] ||
										SCENARIO_LABELS.custom;
									return (
