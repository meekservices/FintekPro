...
export function PortfolioIntelligencePanel({
	...
	const [impactPreview, setImpactPreview] =
		useState<DiversificationImpact | null>(null);
	const [editableTalkingPoints, setEditableTalkingPoints] = useState<
		AdvisorTalkingPoint[]
	>([]);
	...
	const simulateImpactMutation = useMutation({
		mutationFn: async (
			changes: { action: string; fundIsin: string; replacementIsin?: string }[],
		) => {
		...
		onError: (error: any) => {
			toast({
				...
				description:
					error.message || "Could not simulate the diversification impact.",
				variant: "destructive",
			});
		},
	});
	...
	const handleViewImpact = (
		...
		alternative?: AlternativeFund,
	) => {
		if (alternative) {
			simulateImpactMutation.mutate([
				{
					action: "REPLACE",
					fundIsin: suggestion.fundIsin,
					replacementIsin: alternative.isin,
				},
			]);
		}
	};
		...
		return (
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Analyzing portfolio for overlap and diversification...
					</CardDescription>
				</CardHeader>
				...
				<Tabs value={activeTab} onValueChange={setActiveTab}>
					...
					<TabsContent value="overview" className="mt-0">
						<DiversificationScoreWidget
							data={intelligence.diversificationScore}
						/>
					</TabsContent>
						...
						{intelligence.replaceFundSuggestions.length > 0 ? (
							...
							<div className="p-6 bg-green-50 dark:bg-green-950/30 rounded-lg text-center">
								...
								<p className="text-sm text-green-600/80 dark:text-green-400/80 mt-1">
									Your portfolio funds have healthy diversification with minimal
									overlap.
								</p>
							</div>
						)}
						...
						{impactPreview ? (
							...
							<div className="p-6 bg-muted/50 rounded-lg text-center">
								...
								<p className="text-sm text-muted-foreground mt-1">
									Select a replacement option in the "Replace" tab to see the
