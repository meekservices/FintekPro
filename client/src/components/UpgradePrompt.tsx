		...
		<div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-muted-foreground/30 bg-gradient-to-br from-muted/20 to-muted/40 p-10 text-center gap-4">
			...
			<div>
				...
				<p className="text-sm text-muted-foreground max-w-sm">
					{description ||
						`This feature is available on the ${meta.badge} plan and above.`}
				</p>
			</div>
			<div className="flex flex-col items-center gap-2">
				<Badge
					className={`${requiredTier === "elite" ? "bg-yellow-100 text-yellow-800 border-yellow-200" : "bg-blue-100 text-blue-800 border-blue-200"} text-sm px-3 py-1`}
				>
					{meta.badge} — Starting at {meta.price}
				</Badge>
				<Button onClick={() => setLocation("/pricing")} className="mt-1">
