...
export function ConfettiTestPanel({
	confettiEnabled,
	onToggleConfetti,
}: ConfettiTestPanelProps) {
	const [lastTriggered, setLastTriggered] = useState<string | null>(null);

	const handleTriggerConfetti = (
		type: "profit" | "percentage" | "milestone",
		label: string,
	) => {
		if (confettiEnabled) {
			...
			<CardContent className="space-y-4">
				...
				<div className="grid grid-cols-1 md:grid-cols-3 gap-3">
					...
					<Button
						...
						size="sm"
						onClick={() =>
							handleTriggerConfetti("percentage", "Percentage Gain")
						}
						disabled={!confettiEnabled}
					...
					<div className="flex items-start gap-2">
						...
						<div>
							...
							<p className="text-xs text-blue-700 dark:text-blue-300 mt-1">
								The portfolio confetti system automatically triggers
								celebrations when your portfolio hits positive milestones.
								Real-time data updates every 2 seconds, watching for profit
								thresholds, percentage gains, and significant value increases.
							</p>
						</div>
					</div>
				</div>
