...
const steps = [
	...
	},
	{
		...
		icon: Landmark,
		description: "Bank & Demat accounts",
	},
				...
				<div className="space-y-2">
					...
					<div className="flex gap-2">
						<Input
							...
							onChange={(e) =>
								onChange("corporatePan", e.target.value.toUpperCase())
							}
							maxLength={10}
							className="uppercase"
						/>
			...
			<div className="space-y-4">
				...
				<div className="space-y-2">
					...
					<Input
						...
						type="file"
						onChange={(e) =>
							onChange("boardResolutionUrl", e.target.files?.[0])
						}
...
function SignatoryVerificationStep({
	data,
	onChange,
	onNext,
	onBack,
	...
	const verifySignatory = useMutation({
		mutationFn: async () => {
			const response = await apiRequest(
				"POST",
				"/api/kyc/corporate/verify-signatory",
				{
					body: {
			...
			<div className="space-y-4">
				...
				<div className="space-y-2">
					...
					<Select
						...
						onValueChange={(value) => onChange("signatoryDesignation", value)}
					>
...
function AccountDiscoveryStep({ data, onChange, onNext, onBack }: StepProps) {
	...
	const discoverAccounts = useMutation({
		mutationFn: async () => {
			const response = await apiRequest(
				"POST",
				"/api/kyc/corporate/discover-accounts",
				{
		...
		onSuccess: (result) => {
			...
			toast({
				title: `✅ Found ${result.bankAccountsFound} bank accounts and ${result.dematAccountsFound} demat accounts`,
			});
			onNext();
		},
		onError: (error: any) => {
	...
	return (
		<div className="space-y-6 animate-in fade-in-50 duration-500">
			<div className="space-y-4">
				<p className="text-sm text-muted-foreground">
					We'll search for corporate bank and demat accounts linked to your PAN
					number
				</p>
				<Button
					onClick={() => discoverAccounts.mutate()}
					className="w-full"
				...
				<CardContent className="space-y-4">
					...
					<div>
						...
						<p className="font-medium">
							{data.signatoryName} - {data.signatoryDesignation}
						</p>
					</div>
				</CardContent>
			</Card>
...
export function CorporateKYCWizard() {
	...
	const currentStepComponent = () => {
		switch (currentStep) {
			case 1:
				return <CorporatePANStep {...stepProps} />;
			case 2:
				return <CompanyDocumentsStep {...stepProps} />;
			case 3:
				return <SignatoryVerificationStep {...stepProps} />;
			case 4:
				return <AccountDiscoveryStep {...stepProps} />;
			case 5:
