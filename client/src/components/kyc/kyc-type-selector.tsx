...
export function KYCTypeSelector() {
	...
	useEffect(() => {
		if (
			individualProgress &&
			!individualProgress.isCompleted &&
			(individualProgress.currentStep ?? 0) > 0
		) {
			setSelectedType("individual");
			setIsStarted(true);
		} else if (
			corporateProgress &&
			!corporateProgress.isCompleted &&
			(corporateProgress.currentStep ?? 0) > 0
		) {
			setSelectedType("corporate");
			setIsStarted(true);
		} else if (
			nriProgress &&
			!nriProgress.isCompleted &&
			(nriProgress.currentStep ?? 0) > 0
		) {
			setSelectedType("nri");
			setIsStarted(true);
		}
	...
	const kycTypes = [
		...
		},
		{
			...
			inProgress:
				(nriProgress?.currentStep ?? 0) > 0 && !nriProgress?.isCompleted,
		},
	];
			...
			<div className="container max-w-6xl mx-auto py-12 px-4">
				...
				{selectedType && (
					<div className="flex justify-center">
						...
						>
							Start {kycTypes.find((k: any) => k.type === selectedType)?.title}{" "}
							KYC
