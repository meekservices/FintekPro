...
export function KycGuardModal({
  ...
  const { productLabel, allRequiredSteps, missingSteps, sebiRef, currentLevel, requiredLevel } = checkResult;

  const completedSteps = allRequiredSteps.filter(s => !missingSteps.includes(s));
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <div className="flex items-center gap-3 mb-1">
            ...
            <div>
              <DialogTitle className="text-base">KYC Verification Required</DialogTitle>
              <DialogDescription className="text-xs mt-0.5">
                To transact in <span className="font-semibold text-foreground">{productLabel}</span>
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
        ...
        {sebiRef && (
          <div className="flex items-start gap-2 rounded-lg bg-blue-50 dark:bg-blue-950/30 px-3 py-2 text-xs text-blue-700 dark:text-blue-400">
            ...
            <span>
              Mandated under <span className="font-medium">{sebiRef}</span>. We only collect the minimum information required by Indian regulations.
            </span>
          </div>
        )}
        ...
        <div className="flex flex-col gap-2 pt-1">
          <Button onClick={onProceedToKyc} className="w-full gap-2">
            ...
            Complete {missingSteps.length === 1 ? missingSteps[0] : "Verification"}
