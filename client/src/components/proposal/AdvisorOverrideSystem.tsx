...
const OVERRIDE_CATEGORIES = [
	{
		...
		description: "Client specifically requested this change",
	},
	{
	...
	},
	{
		...
		description: "To better match client risk profile",
	},
	{
		value: "tax_optimization",
		label: "Tax Optimization",
		description: "For tax efficiency benefits",
	},
	{
		value: "other",
		label: "Other",
		description: "Other professional judgment",
	},
];
...
export function AdvisorOverrideSystem({
	...
	const [overrideData, setOverrideData] = useState({
		...
		newAmount:
			recommendation.changeAmount ?? recommendation.suggestedAmount ?? 0,
		reason: "",
	...
	const overrideMutation = useMutation({
		mutationFn: async (data: typeof overrideData) => {
			return await apiRequest(
				`/api/agent-wizard/proposals/${proposalId}/override-recommendation`,
				{
					method: "POST",
					body: JSON.stringify({
		...
		onSuccess: (data) => {
			...
			queryClient.invalidateQueries({
				queryKey: ["/api/agent-wizard/proposals", proposalId],
			});
		},
		onError: (error: any) => {
			toast({
	...
	const revertMutation = useMutation({
		mutationFn: async () => {
			return await apiRequest(
				`/api/agent-wizard/proposals/${proposalId}/revert-override`,
				{
					method: "POST",
					body: JSON.stringify({
						recommendationId: recommendation.id || recommendation.productName,
	...
	if (recommendation.isOverridden && recommendation.override) {
		return (
			<div className="relative">
				<Badge className="absolute -top-2 -right-2 z-10 bg-amber-100 text-amber-800 border-amber-200 dark:bg-amber-900/30 dark:text-amber-400 dark:border-amber-700">
						...
						<div className="space-y-2">
							<div className="flex items-center justify-between">
								...
								<span className="font-semibold text-green-600">
									₹
									{(recommendation.override.newAmount ?? amount).toLocaleString(
										"en-IN",
									)}
								</span>
							</div>
							...
							<div className="text-xs text-muted-foreground bg-card/50/50 rounded p-2 space-y-1">
								...
								<div className="flex items-center gap-3 pt-1 border-t border-amber-200 dark:border-amber-800/50">
									...
									<span className="flex items-center gap-1">
										...
										{new Date(
											recommendation.override.overriddenAt,
										).toLocaleDateString()}
									</span>
								</div>
							</div>
			...
			<Dialog open={showOverrideDialog} onOpenChange={setShowOverrideDialog}>
				<DialogContent className="max-w-md">
					<DialogHeader>
						...
						<DialogDescription>
							Modify this recommendation with your professional judgment. All
							overrides are tracked for compliance.
						</DialogDescription>
					</DialogHeader>
					...
					<div className="space-y-4 py-4">
						...
						<div className="grid grid-cols-2 gap-3">
							<div className="space-y-1.5">
								...
								<Select
									...
									onValueChange={(value) =>
										setOverrideData((prev) => ({ ...prev, newAction: value }))
									}
								>
								...
								<Input
									...
									onChange={(e) =>
										setOverrideData((prev) => ({
											...prev,
											newAmount: Number.parseFloat(e.target.value) || 0,
										}))
									}
									className="h-9"
								/>
						...
						<div className="space-y-1.5">
							...
							<Select
								...
								onValueChange={(value: any) =>
									setOverrideData((prev) => ({ ...prev, category: value }))
								}
							>
								<SelectTrigger>
									<SelectValue />
						...
						<div className="space-y-1.5">
							...
							/>
							<p className="text-xs text-muted-foreground">
								This will be visible on the proposal and stored for compliance
								records.
							</p>
						</div>
						...
						<div className="flex items-start gap-2 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg p-3">
							...
							<p className="text-xs text-amber-800 dark:text-amber-300">
								Overrides are tracked for regulatory compliance. The proposal
								will show "Advisor Modified" badge.
							</p>
						</div>
					</div>
					...
					<DialogFooter>
						...
						<Button
							...
							disabled={
								!overrideData.reason.trim() || overrideMutation.isPending
							}
						>
...
export function AdvisorModifiedBadge({
	override,
}: { override?: RecommendationOverride }) {
