...
export function ProductVerificationStatus({
	...
	const products: ProductVerificationItem[] = [
		...
		},
		{
			...
			icon: Globe,
			description:
				"Enhanced KYC with FATCA/CRS compliance for international investments",
			entityTypes: ["all"],
		},
	];
	...
	const getStatusIcon = (verified: boolean, required: string) => {
		...
		if (currentKYCLevel === "none" || currentKYCLevel === "basic") {
	...
	const getStatusBadge = (verified: boolean, required: string) => {
		...
		if (required === "enhanced" && currentKYCLevel === "full") {
			return (
				<Badge
					variant="outline"
					className="border-yellow-600 text-yellow-700 dark:text-yellow-300"
					data-testid={`badge-upgrade-required`}
				>
					...
					{products.map((product) => {
						...
						return (
							...
							>
								...
								<div>{getStatusBadge(product.verified, product.required)}</div>
							</div>
						);
					})}
				</div>
