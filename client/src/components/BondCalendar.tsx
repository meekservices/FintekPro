...
function SourceBadge({ source }: { source: string }) {
	...
	return (
		<Badge
			className={`${srcConfig.color} text-foreground text-xs flex items-center gap-1`}
		>
...
function EventCard({
	event,
	compact = false,
}: { event: BondCalendarEvent; compact?: boolean }) {
	...
	const instrConfig = instrumentTypeConfig[event.instrumentType] || {
		label: event.instrumentType,
		color: "bg-muted",
	};
	...
	if (compact) {
		return (
			<div
				className={`p-2 rounded-lg ${config.bgColor} border cursor-pointer hover:shadow-md transition-shadow`}
			>
			...
			<CardContent className="p-4">
				<div className="flex items-start justify-between gap-3">
					...
					<div className="flex-1 min-w-0">
						...
						<div className="flex flex-wrap gap-1.5">
							...
							<Badge className={`${instrConfig.color} text-foreground text-xs`}>
								{instrConfig.label}
							</Badge>
							{event.creditRating && (
					...
					<div className="text-right flex-shrink-0">
						...
						<p className="text-xs text-muted-foreground">
							{format(parseISO(event.eventDate), "yyyy")}
						</p>
						{event.couponRate && (
							<p className="text-xs font-medium text-green-600 mt-1">
								{Number.parseFloat(event.couponRate).toFixed(2)}%
							</p>
						)}
					</div>
				</div>
				...
				{(event.issueSize || event.minInvestment || event.issuerName) && (
					<div className="mt-3 pt-3 border-t flex items-center gap-4 text-xs text-muted-foreground">
						{event.issueSize && Number.parseFloat(event.issueSize) > 0 && (
							<span className="flex items-center gap-1">
								<IndianRupee className="h-3 w-3" />
								{Number.parseFloat(event.issueSize).toLocaleString()} Cr
							</span>
						)}
						{event.minInvestment && (
							<span>
								Min: ₹{Number.parseFloat(event.minInvestment).toLocaleString()}
							</span>
						)}
						{event.issuerName && (
...
function EventDetailDialog({
	event,
	children,
}: { event: BondCalendarEvent; children: React.ReactNode }) {
	const config = eventTypeConfig[event.eventType] || {
		label: event.eventType,
		icon: Calendar,
		color: "text-muted-foreground",
						...
						{event.yieldIndicative && (
							<div>
								...
								<p className="font-semibold text-blue-600">
									{Number.parseFloat(event.yieldIndicative).toFixed(2)}%
								</p>
							</div>
						)}
						{event.faceValue && (
							<div>
								...
								<p className="font-medium">
									₹{Number.parseFloat(event.faceValue).toLocaleString()}
								</p>
							</div>
						)}
					</div>
						...
						<div className="grid grid-cols-3 gap-4 pt-2 border-t">
							...
							{event.minInvestment && (
								<div>
									...
									<p className="font-medium">
										₹{Number.parseFloat(event.minInvestment).toLocaleString()}
									</p>
								</div>
							)}
							{event.retailQuota && (
								<div>
									...
									<p className="font-medium">
										{Number.parseFloat(event.retailQuota).toFixed(0)}%
									</p>
								</div>
							)}
						</div>
		...
		<div className="border rounded-lg overflow-hidden">
			...
			<div className="grid grid-cols-7">
				{paddingDays.map((_, i) => (
					<div
						key={`pad-${i}`}
						className="min-h-[100px] border-t border-l bg-muted/30"
					/>
				))}
				{days.map((day) => {
	...
	return (
		<Card className="border-yellow-200 bg-gradient-to-br from-yellow-50 to-amber-50 dark:from-yellow-950 dark:to-amber-950">
			<CardHeader className="pb-2">
				...
				<CardDescription>
					Don't miss these highlighted investment opportunities
				</CardDescription>
			</CardHeader>
...
function StatsCards({ stats }: { stats: CalendarStats }) {
	const items = [
		...
		{
			label: "Maturities",
			value: stats.upcomingMaturities,
			icon: Coins,
			color: "text-amber-600",
...
export function BondCalendar() {
	...
	const handleExportIcal = () => {
		...
		toast({
			...
			description:
				"Downloading iCal file. You can import it into your calendar app.",
		});
	};
	...
	const handleAddToGoogleCalendar = async (eventId: string) => {
		try {
			const res = await fetch(
				`/api/bond-calendar/events/${eventId}/google-calendar`,
			);
	...
	const { data: highlightedData, isLoading: highlightedLoading } = useQuery<{
		success: boolean;
		events: BondCalendarEvent[];
	}>({
	...
	const syncExternalMutation = useMutation({
		mutationFn: async () => {
			const res = await apiRequest("/api/bond-calendar/sync/external", {
				method: "POST",
			});
			return res.json();
		},
	...
	const filteredEvents = useMemo(() => {
		...
		return eventsData.events.filter((event) => {
			...
			if (
				...
				event.instrumentType !== instrumentTypeFilter
			)
				return false;
			if (sourceFilter !== "all") {
				if (sourceFilter === "external" && !event.source.includes("external"))
					return false;
				if (sourceFilter === "internal" && event.source.includes("external"))
					return false;
				if (
					sourceFilter !== "external" &&
					sourceFilter !== "internal" &&
					!event.source.includes(sourceFilter)
				)
					return false;
			}
			return true;
	...
	return (
		<div className="space-y-6">
			{statsLoading ? (
				...
				statsData?.stats && <StatsCards stats={statsData.stats} />
			)}

			{!highlightedLoading &&
				highlightedData?.events &&
				highlightedData.events.length > 0 && (
					...
					<div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
						<div className="flex items-center gap-3">
							...
							<div>
								...
								<CardDescription>
									Track upcoming issuances, auctions, maturities, and coupon
									payments
								</CardDescription>
							</div>
						</div>
						...
						<div className="flex items-center gap-2 flex-wrap">
							...
							>
								<RefreshCw
									className={`h-4 w-4 mr-1.5 ${syncExternalMutation.isPending ? "animate-spin" : ""}`}
								/>
									...
									<DropdownMenuItem
										onClick={() => {
											navigator.clipboard.writeText(
												`${window.location.origin}/api/bond-calendar/export/ical`,
											);
											toast({
												title: "Calendar URL Copied",
												description:
													"Subscribe to this URL in your calendar app for live updates",
											});
										}}
					...
					<div className="flex flex-wrap gap-4 mb-6">
						...
						>
							...
							<SelectContent>
								...
								<SelectItem value="infrastructure_bond">
									Infrastructure Bond
								</SelectItem>
								<SelectItem value="capital_gains_bond">
									54EC Capital Gains
								</SelectItem>
