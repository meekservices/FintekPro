...
function getRoleLabel(roles: string[]): string {
  ...
  if (roles.some(r => ['agent', 'sub_agent', 'associate'].includes(r))) return 'Agent';
  if (roles.some(r => ['compliance_officer', 'regulatory_auditor'].includes(r))) return 'Compliance / Audit';
  if (roles.some(r => ['admin', 'superadmin'].includes(r))) return 'Administrator';
  if (roles.some(r => ['bd_head', 'bd_team'].includes(r))) return 'Business Development';
      ...
      <div className="max-w-2xl w-full space-y-6">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-orange-200 dark:border-orange-800/50 shadow-sm p-6 space-y-5">
          ...
          <div className="flex flex-wrap gap-2 items-center">
            <Badge variant="outline" className="text-xs border-blue-200 text-blue-700 dark:text-blue-400">
              Role: {roleLabel}
            </Badge>
            <Badge variant="outline" className="text-xs border-gray-200 text-gray-600 dark:text-gray-400">
