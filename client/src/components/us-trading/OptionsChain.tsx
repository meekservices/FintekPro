...
export default function OptionsChain({
	...
	const { data, isLoading, refetch } = useQuery<{
		...
		queryKey: [
			"/api/us-trading/options/contracts",
			underlying,
			expiryFilter,
			typeFilter,
		],
		queryFn: () => {
			...
			return fetch(`/api/us-trading/options/contracts?${params}`).then((r) =>
				r.json(),
			);
		},
	...
	const orderMutation = useMutation({
		mutationFn: () =>
			apiRequest("POST", "/api/us-trading/orders", {
				symbol: selectedContract?.symbol,
				side: orderSide,
				type: orderType,
				time_in_force: "day",
				quantity: Number.parseFloat(orderQty),
				limitPrice:
					orderType === "limit" ? Number.parseFloat(limitPrice) : undefined,
			}),
		onSuccess: () => {
			toast({
				title: "Options order placed!",
				description: `${orderSide.toUpperCase()} ${orderQty}x ${selectedContract?.symbol}`,
			});
	...
	function ContractRow({
		contract,
		side,
	}: { contract: AlpacaOptionContract; side: "call" | "put" }) {
		return (
			<TableRow
				key={contract.id}
				className={`text-xs ${!contract.tradable ? "opacity-40" : ""}`}
			>
				...
				<TableCell>
					{contract.close_price
						? `$${Number.parseFloat(contract.close_price).toFixed(2)}`
						: "—"}
				</TableCell>
					...
					{contract.tradable && (
						<div className="flex gap-1">
							<Button
								...
								onClick={() => openOrder(contract, "buy")}
							>
								Buy
							</Button>
							<Button
								...
								onClick={() => openOrder(contract, "sell")}
							>
								Sell
							</Button>
						</div>
		...
		<div className="space-y-4">
			<Alert className="border-purple-200 bg-purple-50 dark:bg-purple-950/20">
				...
				<AlertDescription className="text-sm text-purple-700 dark:text-purple-300">
					<strong>
						Options Disclosure Document (ODD) — FINRA Rule 2360 / SEC Rule 9b-1:
					</strong>{" "}
					Before trading options, you must read the{" "}
					<a
					...
					published by OCC. Options trading involves substantial risk of loss.
					Options may expire worthless. Each options contract represents 100
					shares. Your account must be approved for the appropriate options
					level (1–3) and you must have received and read the ODD prior to
					trading.
				</AlertDescription>
			</Alert>
			<Alert className="border-blue-100 bg-blue-50/60 dark:bg-blue-950/10 py-2">
				<AlertDescription className="text-xs text-blue-700 dark:text-blue-300">
					<strong>Indian Resident Notice:</strong> Gains from US options are
					treated as capital gains in India (STCG/LTCG based on holding period
					of the underlying). Report in Schedule FA and Schedule FSI of your
					ITR. FEMA/LRS limits apply to the premium paid.
				</AlertDescription>
			</Alert>
					...
					<div className="flex gap-3 items-end flex-wrap">
						<div>
							...
							<div className="flex gap-2 mt-1">
								<Input
									...
									onKeyDown={(e) =>
										e.key === "Enter" && setUnderlying(searchInput)
									}
									placeholder="AAPL"
								...
								<Button
									size="sm"
									className="h-8 gap-1"
									onClick={() => setUnderlying(searchInput)}
								>
						...
						<div>
							...
							<Select
								...
								onValueChange={(v) =>
									setTypeFilter(v === "all" ? "" : (v as any))
								}
							>
								<SelectTrigger className="h-8 text-xs w-24 mt-1">
									<SelectValue />
								</SelectTrigger>
								<SelectContent>
						...
						<div>
							...
							<Select
								value={expiryFilter || "all"}
								onValueChange={(v) => setExpiryFilter(v === "all" ? "" : v)}
							>
								<SelectTrigger className="h-8 text-xs w-36 mt-1">
			...
			{isLoading ? (
				...
				<div className="text-center py-12 text-sm text-muted-foreground">
					No option contracts found for <strong>{underlying}</strong>. Try a
					different symbol or expiry filter.
				</div>
			) : (
								...
								<Table>
									...
									<TableBody>
										{calls.slice(0, 50).map((c) => (
											<ContractRow key={c.id} contract={c} side="call" />
										))}
									</TableBody>
								</Table>
							</CardContent>
						...
						<Card>
							...
							<CardContent className="p-0">
								<Table>
									...
									<TableBody>
										{puts.slice(0, 50).map((c) => (
											<ContractRow key={c.id} contract={c} side="put" />
										))}
									</TableBody>
								</Table>
							</CardContent>
					...
					<div className="space-y-3 py-2 text-sm">
						<div className="rounded-lg bg-muted/40 p-3 text-xs space-y-1">
							<div className="flex justify-between">
								...
								<span className="font-mono">{selectedContract?.symbol}</span>
							</div>
							<div className="flex justify-between">
							...
							<div className="flex justify-between">
								...
								<span>${selectedContract?.strike_price}</span>
							</div>
							<div className="flex justify-between">
								<span className="text-muted-foreground">Expiry</span>
								<span>
									{selectedContract
										? formatExpiry(selectedContract.expiration_date)
										: "—"}
								</span>
							</div>
							<div className="flex justify-between">
								...
								<span>
									{selectedContract?.close_price
										? `$${Number.parseFloat(selectedContract.close_price).toFixed(2)}`
										: "—"}
								</span>
							</div>
						</div>
					...
					<DialogFooter>
						...
						>
							{orderMutation.isPending
								? "Placing…"
								: `${orderSide === "buy" ? "Buy" : "Sell"} ${orderQty} Contract${Number.parseInt(orderQty) !== 1 ? "s" : ""}`}
						</Button>
					</DialogFooter>
				</DialogContent>
			</Dialog>
