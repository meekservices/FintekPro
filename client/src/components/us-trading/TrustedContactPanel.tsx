...
export default function TrustedContactPanel({
	...
	const { data, isLoading, refetch } = useQuery<{
		...
		queryFn: () =>
			fetch(
				`/api/us-trading/broker/accounts/${accountId}/trusted-contact`,
			).then((r) => r.json()),
		enabled: !!accountId,
		staleTime: 60_000,
	});
	...
	const saveMutation = useMutation({
		...
		onSuccess: () => {
			toast({
				title: contact ? "Trusted contact updated" : "Trusted contact added",
			});
	...
	function handleSave() {
		...
		if (form.email_address?.trim())
			payload.email_address = form.email_address.trim();
		...
		<div className="space-y-4">
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-xs text-muted-foreground mt-0.5">
						Required under FINRA Rule 4512 — a person the broker may contact to
			...
			<Alert className="border-blue-200 bg-blue-50/50 py-2">
				...
				<AlertDescription className="text-xs text-blue-700">
					<strong>India note:</strong> Under FINRA Rule 4512, US brokers must
					attempt to obtain a trusted contact for each retail account. This
					person may be contacted in cases of possible financial exploitation,
					incapacitation, or when the broker cannot reach the account holder.
					The trusted contact does not receive any authority over the account.
				</AlertDescription>
			</Alert>
							...
							<AlertDialog>
								...
								<AlertDialogContent>
									<AlertDialogHeader>
										...
										<AlertDialogDescription>
											Remove {contact.given_name} {contact.family_name} as the
											trusted contact for this account? The account will no
											longer have a trusted contact on file.
										</AlertDialogDescription>
									</AlertDialogHeader>
									<AlertDialogFooter>
					...
					<CardContent className="py-10 flex flex-col items-center gap-3 text-center">
						...
						<div>
							...
							<p className="text-xs text-muted-foreground mt-1">
								Adding a trusted contact helps protect the account holder under
								FINRA Rule 4512.
							</p>
						</div>
			...
			<Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
				<DialogContent className="max-w-md">
					<DialogHeader>
						...
						<DialogDescription>
							Provide at least a name and one contact method (email or phone).
							This person has no authority over the account.
						</DialogDescription>
					</DialogHeader>
						...
						<div className="grid grid-cols-2 gap-3">
							<div className="space-y-1">
								...
								<Input
									...
									onChange={(e) =>
										setForm((f) => ({ ...f, given_name: e.target.value }))
									}
								/>
							</div>
							<div className="space-y-1">
