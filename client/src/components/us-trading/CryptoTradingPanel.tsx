...
export default function CryptoTradingPanel({ accountId }: Props) {
	...
	const {
		...
		queryFn: () =>
			fetch(
				`/api/us-trading/crypto/positions${accountId ? `?account_id=${accountId}` : ""}`,
			).then((r) => r.json()),
		staleTime: 30_000,
	...
	const orderMutation = useMutation({
		...
		onSuccess: (data: any) => {
			...
			queryClient.invalidateQueries({
				queryKey: ["/api/us-trading/crypto/positions"],
			});
		},
		onError: (e: any) => {
			toast({
	...
	function handleOrder() {
		if (!symbol)
			return toast({ title: "Select a symbol", variant: "destructive" });
		const payload: any = {
			symbol,
			side,
			type: orderType,
		...
		if (qtyMode === "qty") {
			if (!qty)
				return toast({ title: "Enter quantity", variant: "destructive" });
			payload.qty = Number.parseFloat(qty);
		} else {
			if (!notional)
				return toast({ title: "Enter USD amount", variant: "destructive" });
			payload.notional = Number.parseFloat(notional);
		}
		if (orderType === "limit") {
			if (!limitPrice)
				return toast({ title: "Enter limit price", variant: "destructive" });
			payload.limit_price = Number.parseFloat(limitPrice);
		}
		...
		<div className="space-y-4">
			<Alert className="border-blue-200 bg-blue-50 dark:bg-blue-950/20">
				...
				<AlertDescription className="text-blue-700 dark:text-blue-300 text-xs">
					<strong>India / LRS note:</strong> Crypto trading via Alpaca US
					brokerage is treated as a foreign investment under LRS (₹7L/yr cap,
					TCS 20% above ₹7L). These are crypto assets held in your US brokerage
					account — not Indian crypto exchanges. FEMA compliance applies. Gains
					are taxable in India as per IT Act.
				</AlertDescription>
			</Alert>
			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
				...
				<Card>
					...
					<CardContent className="space-y-3">
								...
								<SelectContent>
									...
									{loadingAssets
										...
										: allAssets
												.filter(
													(a) =>
														!POPULAR_CRYPTO.find((p) => p.symbol === a.symbol),
												)
												.slice(0, 20)
												.map((a: any) => (
													<SelectItem
						...
						>
							...
							{orderMutation.isPending
								? "Submitting…"
								: `${side === "buy" ? "Buy" : "Sell"} ${selectedCrypto?.name || symbol}`}
						</Button>
					</CardContent>
				</Card>
				...
				<Card>
					<CardHeader className="pb-3">
						...
						<CardDescription className="text-xs">
							Alpaca supports{" "}
							{loadingAssets ? "…" : allAssets.length || POPULAR_CRYPTO.length}+
			...
			<Card>
				<CardHeader className="pb-3 flex flex-row items-center justify-between">
					<div>
						...
						<CardDescription className="text-xs">
							Current holdings in your US account
						</CardDescription>
					</div>
					<Button
						size="sm"
				...
				<CardContent>
					{loadingPositions ? (
						...
						<Table>
							...
							<TableBody>
								{positions.map((pos) => {
									const pl = Number.parseFloat(pos.unrealized_pl);
									const plPct = Number.parseFloat(pos.unrealized_plpc) * 100;
									const isPos = pl >= 0;
									return (
										<TableRow key={pos.symbol}>
											...
											<TableCell className="text-xs text-right">
												<span
													className={isPos ? "text-green-600" : "text-red-600"}
												>
													{isPos ? "+" : ""}
