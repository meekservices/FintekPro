...
export default function AlpacaEventFeed({
	alpacaAccountId,
	maxEvents = 100,
}: AlpacaEventFeedProps) {
	...
	function formatData(data: any): string {
		...
		const parts = [
			...
			qty && `${qty} shares`,
			notional && `$${Number.parseFloat(notional).toFixed(2)}`,
					...
					{events.length === 0 ? (
						<div className="text-center py-10 text-sm text-muted-foreground">
							...
							<span className="text-xs">
								Place an order or change account status to see live events here.
							</span>
						</div>
					) : (
								...
								>
									...
									<div className="min-w-0">
										<div className="flex items-center gap-1.5 flex-wrap">
											<Badge
												variant="outline"
												className={`text-xs px-1.5 py-0 ${EVENT_COLORS[ev.event] || "text-foreground"}`}
											>
												{ev.event}
											</Badge>
