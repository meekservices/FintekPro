...
export default function EnhancedOrderForm({
	...
	const [qtyMode, setQtyMode] = useState<
		"shares" | "notional_usd" | "notional_inr"
	>("shares");
	...
	const etHour = clockData?.timestamp
		? new Date(
				new Date(clockData.timestamp).toLocaleString("en-US", {
	...
	const { data: quoteData, isLoading: quoteLoading } = useQuery<{
		...
		queryFn: () =>
			fetch(`/api/us-trading/market/quote/${symbol.toUpperCase()}`).then((r) =>
				r.json(),
			),
	...
	const orderMutation = useMutation({
		mutationFn: () => {
			...
			if (qtyMode === "shares") {
				payload.quantity = Number.parseFloat(qty);
			} else if (qtyMode === "notional_usd") {
				payload.notionalUsd = Number.parseFloat(qty);
			} else {
				payload.notionalUsd = Number.parseFloat(inrAmount) / fxRate;
			}

			if (orderType === "limit" || orderType === "stop_limit")
				payload.limitPrice = Number.parseFloat(limitPrice);
			if (orderType === "stop" || orderType === "stop_limit")
				payload.stopPrice = Number.parseFloat(stopPrice);
			if (orderType === "trailing_stop") {
				if (trailBy === "price")
					payload.trailPrice = Number.parseFloat(trailValue);
				else payload.trailPercent = Number.parseFloat(trailValue);
			}
		...
		onSuccess: () => {
			toast({
				title: "Order placed!",
				description: `${side.replace("_", " ").toUpperCase()} ${symbol.toUpperCase()} order submitted`,
			});
						...
						{clockData &&
							(isMarketOpen ? (
								<Badge className="bg-green-100 text-green-700 text-xs gap-1">
									<CheckCircle2 className="h-3 w-3" /> Market Open
								</Badge>
							) : isExtendedWindow ? (
								...
								<Badge className="bg-gray-100 text-gray-600 text-xs gap-1">
									<XCircle className="h-3 w-3" /> Market Closed
								</Badge>
							))}
						{extendedHours && (
							<Badge className="bg-indigo-100 text-indigo-700 text-xs gap-1">
								<Moon className="h-3 w-3" /> Extended Hours On
							</Badge>
						)}
					</CardTitle>
				</CardHeader>
				<CardContent className="space-y-4">
					...
					{pdtRisk && (
						<Alert className="border-red-200 bg-red-50 dark:bg-red-950/20">
							...
							<AlertDescription className="text-sm text-red-700 dark:text-red-300">
								<strong>Pattern Day Trader (PDT) Restriction:</strong> Your
								account is flagged as a PDT and equity ($
								{equity.toLocaleString()}) is below $25,000 — the FINRA minimum.
								Day trades will be rejected until equity is restored. Use GTC
								orders or consider adding funds. Day trades this week:{" "}
								{daytradeCount}/4.
							</AlertDescription>
						</Alert>
					)}
					{isPdt && !pdtRisk && (
						<Alert className="border-amber-200 bg-amber-50 dark:bg-amber-950/20 py-2">
							...
							<AlertDescription className="text-xs text-amber-700">
								<strong>PDT Account:</strong> Your account is flagged as a
								Pattern Day Trader. You can place unlimited day trades since
								equity exceeds $25,000. Day trades this week: {daytradeCount}.
							</AlertDescription>
						</Alert>
					)}
					{/* Symbol */}
					<div>
						...
						<div className="flex gap-2 mt-1">
							...
							/>
							{quoteLoading && (
								<RefreshCw className="h-4 w-4 mt-2.5 animate-spin text-muted-foreground" />
							)}
							{livePrice > 0 && (
						...
						<Tabs value={qtyMode} onValueChange={(v: any) => setQtyMode(v)}>
							<TabsList className="h-8 w-full">
								...
								<TabsTrigger
									...
									className="flex-1 text-xs gap-1"
								>
									<BadgeIndianRupee className="h-3 w-3" />₹ Amount
								</TabsTrigger>
							</TabsList>
								...
								{qty &&
									...
									Number.parseFloat(qty) < 1 && (
										<p className="text-xs text-amber-600 mt-1">
											Fractional quantity — asset must be fractionable. Only
											market orders with time_in_force=day are supported.
										</p>
									)}
							</TabsContent>
							<TabsContent value="notional_usd" className="mt-2">
								...
								<p className="text-xs text-muted-foreground mt-1">
									Notional orders require the asset to be fractionable. Market
									orders only. Min $1.
								</p>
							</TabsContent>
							<TabsContent value="notional_inr" className="mt-2">
								...
								{inrAmount && (
									<p className="text-xs text-muted-foreground mt-1">
										≈ ${(Number.parseFloat(inrAmount) / fxRate).toFixed(2)} at ₹
										{fxRate.toFixed(2)}/USD · Notional = fractionable assets
										only
									</p>
								)}
							</TabsContent>
					...
					{orderType === "trailing_stop" && (
						<div className="space-y-2">
							...
							<div className="flex gap-2">
								<Button
									size="sm"
									variant={trailBy === "percent" ? "default" : "outline"}
									className="h-8 text-xs"
									onClick={() => setTrailBy("percent")}
								>
									% Percent
								</Button>
								<Button
									size="sm"
									variant={trailBy === "price" ? "default" : "outline"}
									className="h-8 text-xs"
									onClick={() => setTrailBy("price")}
								>
									$ Price
								</Button>
							</div>
					...
					<div className="flex items-center justify-between rounded-lg border px-3 py-2">
						<div>
							<div className="flex items-center gap-1.5 text-sm font-medium">
								<Clock className="h-3.5 w-3.5 text-indigo-500" /> Extended Hours
								Trading
							</div>
							<p className="text-xs text-muted-foreground mt-0.5">
								Pre-market (4–9:30 AM ET) &amp; after-hours (4–8 PM ET)
							</p>
						</div>
						<Switch
					...
					{extendedHours && (
						<Alert className="border-amber-200 bg-amber-50/50 py-2">
							...
							<AlertDescription className="text-xs text-amber-700 space-y-0.5">
								<p>
									<strong>Extended Hours Risk Disclosure (Alpaca):</strong>
								</p>
								<p>
									Overnight 8 PM–4 AM ET · Pre-market 4–9:30 AM ET · After-hours
									4–8 PM ET. Lower liquidity, wider bid-ask spreads, and higher
									price volatility than regular hours. Only limit orders
									accepted. Fractional orders are also supported during extended
									hours. By enabling this, you acknowledge Alpaca's Extended
					...
					{canSubmit && (
						<div className="rounded-lg bg-muted/40 px-3 py-2.5 text-xs space-y-1">
							...
							<div className="flex justify-between">
								...
								<span
									className={`font-medium ${side === "buy" ? "text-green-600" : "text-red-600"}`}
								>
							...
							{notionalUsd > 0 && (
								<>
									...
									<div className="flex justify-between">
										...
										<span className="font-medium">
											₹
											{notionalInr.toLocaleString("en-IN", {
					...
					{side === "buy" && (
						<Alert className="border-blue-200 bg-blue-50/50 py-2">
							...
							<AlertDescription className="text-xs text-blue-700">
								<strong>Wash Sale Rule (US IRC §1091):</strong> Buying a
								substantially identical security within 30 days before/after a
								loss sale disallows the capital loss. Consult your tax advisor
								before placing this order. As an Indian resident, corresponding
								DTAA treatment may apply.
							</AlertDescription>
						</Alert>
					)}
					...
					<Alert className="border-violet-200 bg-violet-50/50 py-2">
						...
						<AlertDescription className="text-xs text-violet-700">
							<strong>ITR Disclosure:</strong> Gains/losses from US equities are
							taxable in India as capital gains. Report foreign assets in{" "}
							<strong>Schedule FA</strong> and foreign income in{" "}
							<strong>Schedule FSI</strong> of ITR-2/ITR-3. TCS deducted by your
							AD bank (20% above ₹7 lakh/FY) is creditable against tax
							liability.
						</AlertDescription>
					</Alert>
					...
					<Button
						className="w-full"
						onClick={() =>
							guardAction("us_equity", () => orderMutation.mutate())
						}
						disabled={!canSubmit || orderMutation.isPending || isChecking}
					>
						{orderMutation.isPending || isChecking ? (
