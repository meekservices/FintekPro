...
export default function AccountRestrictionsPanel({
	accountId,
	accountStatus,
}: Props) {
	...
	const [pendingRestrictions, setPendingRestrictions] =
		useState<Restrictions | null>(null);
	...
	const isSuspended =
		accountStatus === "ACCOUNT_SUSPENDED" || restrictions.suspend_trading;
	...
	const updateMutation = useMutation({
		...
		onSuccess: () => {
			toast({
				...
				description: "Account restrictions saved successfully.",
			});
	...
	const suspendMutation = useMutation({
		...
		onSuccess: () => {
			toast({
				...
				description: "All trading has been halted on this account.",
			});
			setSuspendReason("");
			refetch();
		},
	...
	function saveRestrictions() {
		...
		if (marginMultiplier)
			payload.max_margin_multiplier = Number.parseFloat(marginMultiplier);
	...
	if (!accountId) {
		return (
			<Alert>
				...
				<AlertDescription>
					Connect your Alpaca account to manage restrictions.
				</AlertDescription>
			</Alert>
		);
	}
		...
		<div className="space-y-4">
			...
			>
				...
				>
					...
					{isSuspended
						? "SUSPENDED — All trading is halted."
								...
								<div className="flex items-center justify-between">
									<div>
										...
										<p className="text-[10px] text-muted-foreground">
											Prevent all buy/sell orders
										</p>
									</div>
									<Switch
										checked={!!current.restrict_trading}
										onCheckedChange={(v) =>
											toggleRestriction("restrict_trading", v)
										}
									/>
								</div>
								<div className="flex items-center justify-between">
									<div>
										...
										<p className="text-[10px] text-muted-foreground">
											Disable shorting on this account
										</p>
									</div>
									<Switch
										checked={
											!!current.restrict_short_selling || !!current.no_shorting
										}
										onCheckedChange={(v) =>
											toggleRestriction("restrict_short_selling", v)
										}
									/>
								</div>
								<div className="flex items-center justify-between">
									<div>
										<Label className="text-xs font-medium">
											Restrict Options Trading
										</Label>
										<p className="text-[10px] text-muted-foreground">
											Prevent options orders
										</p>
									</div>
									<Switch
										checked={!!current.restrict_options_trading}
										onCheckedChange={(v) =>
											toggleRestriction("restrict_options_trading", v)
										}
									/>
								</div>
								<div className="flex items-center justify-between">
									<div>
										...
										<p className="text-[10px] text-muted-foreground">
											Disable margin borrowing
										</p>
									</div>
									<Switch
										checked={!!current.restrict_margin}
										onCheckedChange={(v) =>
											toggleRestriction("restrict_margin", v)
										}
									/>
								</div>
							...
							<>
								<div className="space-y-2">
									<div className="flex items-center justify-between py-1.5 border-b">
										...
										<Badge
											variant={
												restrictions.pattern_day_trader
													? "destructive"
													: "outline"
											}
											className="text-[10px]"
									...
									<div className="flex items-center justify-between py-1.5 border-b">
										...
										<Badge
											variant={
												restrictions.no_shorting ? "secondary" : "outline"
											}
											className="text-[10px]"
										>
								...
								<Alert className="border-amber-200 bg-amber-50 dark:bg-amber-950/20 mt-2">
									...
									<AlertDescription className="text-[10px] text-amber-700 dark:text-amber-300">
										<strong>India note:</strong> PDT rule requires $25K min
										balance. Indian users with &lt;$25K equity in a margin
										account are PDT-restricted to 3 day trades per 5 days. Use a
										cash account or maintain $25K+ to avoid PDT.
									</AlertDescription>
								</Alert>
							</>
						)}
							...
							<AlertDialog>
								<AlertDialogTrigger asChild>
									<Button
										size="sm"
										variant="outline"
										className="border-green-600 text-green-600 hover:bg-green-50 text-xs shrink-0"
									>
								...
								<AlertDialogContent>
									<AlertDialogHeader>
										...
										<AlertDialogDescription>
											This will re-activate the account and allow trading. Only
											do this after compliance review confirms the issue is
											resolved.
										</AlertDialogDescription>
									</AlertDialogHeader>
									<AlertDialogFooter>
							...
							<AlertDialog>
								...
								<AlertDialogContent>
									<AlertDialogHeader>
										...
										<AlertDialogDescription>
											This will immediately halt all trading. The user will not
											be able to place or cancel orders. Reason:{" "}
											<em>{suspendReason}</em>
										</AlertDialogDescription>
									</AlertDialogHeader>
									<AlertDialogFooter>
