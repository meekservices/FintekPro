...
export default function JournalsPanel() {
	...
	const { data, isLoading, error, refetch } = useQuery<{
		...
		queryKey: qKey,
		queryFn: () =>
			fetch("/api/us-trading/broker/journals").then((r) => r.json()),
		staleTime: 30_000,
	});
	...
	const createMutation = useMutation({
		...
		onSuccess: () => {
			toast({
				...
				description: "Journal entry is pending settlement.",
			});
	...
	const reverseMutation = useMutation({
		mutationFn: (journalId: string) =>
			apiRequest(
				`/api/us-trading/broker/journals/${journalId}/reverse`,
				"POST",
				{},
			),
		onSuccess: () => {
			toast({
				...
				description: "A reversing entry has been created.",
			});
			queryClient.invalidateQueries({ queryKey: qKey });
		},
		onError: (err: any) => {
	...
	const {
		...
		queryFn: () => {
			...
			if (transferDirection !== "all")
				params.set("direction", transferDirection);
			return fetch(`/api/us-trading/broker/transfers?${params}`).then((r) =>
				r.json(),
			);
		},
		staleTime: 30_000,
	...
	function handleCreate() {
		...
		if (form.entry_type === "JNLC") {
			...
			payload.amount = Number.parseFloat(form.amount);
		} else {
			if (!form.symbol.trim() || !form.qty) return;
			payload.symbol = form.symbol.trim().toUpperCase();
			payload.qty = Number.parseFloat(form.qty);
		}
	...
	const isFormValid =
		form.entry_type === "JNLC"
			? !!form.to_account.trim() &&
				!!form.amount &&
				Number.parseFloat(form.amount) > 0
			: !!form.to_account.trim() &&
				!!form.symbol.trim() &&
				!!form.qty &&
				Number.parseFloat(form.qty) > 0;
		...
		<div className="space-y-4">
			<div className="flex items-center justify-between flex-wrap gap-2">
				<div>
					...
					<p className="text-xs text-muted-foreground mt-0.5">
						JNLC = cash transfer · JNLS = securities transfer between
						sub-accounts
					</p>
				</div>
			...
			<Alert className="border-amber-200 bg-amber-50 dark:border-amber-800 dark:bg-amber-950">
				...
				<AlertDescription className="text-xs text-amber-800 dark:text-amber-200">
					<strong>Broker-level operation:</strong> Journals move cash (JNLC) or
					securities (JNLS) between Alpaca sub-accounts instantly. Irreversible
					once executed. Requires valid Alpaca account IDs.
				</AlertDescription>
			</Alert>
				...
				<CardContent className="p-0">
					{isLoading ? (
						...
						<Alert variant="destructive" className="m-4">
							...
							<AlertDescription>
								Failed to load journals. {(error as any).message}
							</AlertDescription>
						</Alert>
					) : journals.length === 0 ? (
								...
								{journals.map((j) => (
									<TableRow key={j.id}>
										...
										<TableCell className="font-mono text-sm">
											{j.entry_type === "JNLC"
												? j.net_amount
													? `$${Number.parseFloat(j.net_amount).toFixed(2)}`
													: "—"
												: j.qty
													? `${j.qty} ${j.symbol || ""}`
													: "—"}
										...
										<TableCell>
											<Badge
												className={`text-xs flex items-center gap-1 w-fit ${STATUS_STYLES[j.status] ?? "bg-gray-100 text-gray-600"}`}
											>
													...
													<AlertDialog>
														...
														<AlertDialogContent>
															<AlertDialogHeader>
																...
																<AlertDialogDescription>
																	Cancel this pending {j.entry_type} journal
													...
													<AlertDialog>
														...
														<AlertDialogContent>
															<AlertDialogHeader>
																...
																<AlertDialogDescription>
																	Create a reversing entry for this executed{" "}
																	{j.entry_type} journal? This will move the
																	funds/securities back to the originating
																	account.
																</AlertDialogDescription>
															</AlertDialogHeader>
															<AlertDialogFooter>
				...
				<div className="flex items-center justify-between flex-wrap gap-2 mb-3">
					...
					<div className="flex items-center gap-2">
						<Select
							...
							onValueChange={setTransferDirection}
						>
							<SelectTrigger className="h-8 w-36 text-xs">
								<SelectValue />
							</SelectTrigger>
											...
											<TableRow key={t.id}>
												<TableCell>
													<div className="flex items-center gap-1.5">
														...
														<span
															className={`text-xs font-medium ${isIn ? "text-emerald-700 dark:text-emerald-400" : "text-red-600 dark:text-red-400"}`}
														>
