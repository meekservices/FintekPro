...
export default function MarketCalendarPanel() {
	...
	const { data, isLoading, refetch } = useQuery<{
		...
		queryFn: () =>
			fetch(
				`/api/us-trading/broker/calendar?start=${toISO(today)}&end=${toISO(in60)}`,
			).then((r) => r.json()),
			...
			<div className="flex items-center justify-between">
				<div>
					<h3 className="text-sm font-medium">
						NYSE Market Calendar — Next 60 Days
					</h3>
					<p className="text-xs text-muted-foreground">
						All times Eastern (ET). India is 9.5–10.5 hrs ahead.
					</p>
				</div>
				<Button
					variant="ghost"
						...
						{[
							{
								...
								et: "4:00 AM ET",
								ist: "2:30 PM IST / 1:30 PM IST (DST)",
							},
							{
								label: "Regular session opens",
								et: "9:30 AM ET",
								ist: "8:00 PM IST / 7:00 PM IST (DST)",
							},
							{
								label: "Regular session closes",
								et: "4:00 PM ET",
								ist: "2:30 AM IST / 1:30 AM IST (DST)",
							},
							{
								label: "After-hours closes",
								et: "8:00 PM ET",
								ist: "6:30 AM IST / 5:30 AM IST (DST)",
							},
						].map(({ label, et, ist }) => (
							<div key={label}>
							...
							<div className="flex items-center gap-1">
								...
								{type === "normal" && (
									<span className="text-muted-foreground">
										{day.open || day.session_open
											? `${day.open || day.session_open} – ${day.close || day.session_close}`
											: "Open"}
									</span>
								)}
							</div>
