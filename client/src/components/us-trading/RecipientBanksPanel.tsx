...
export default function RecipientBanksPanel({
	alpacaAccountId,
}: RecipientBanksPanelProps) {
	...
	const [form, setForm] = useState({
		...
		bank_name: "",
		bank_account_number: "",
		bank_account_type: "INTERNATIONAL" as
			| "INTERNATIONAL"
			| "CHECKING"
			| "SAVINGS",
		bank_swift_code: "",
	...
	const { data, isLoading, refetch } = useQuery<{
		...
		queryFn: () =>
			fetch(
				`/api/us-trading/broker/accounts/${alpacaAccountId}/recipient-banks`,
			).then((r) => r.json()),
		staleTime: 60_000,
	});
	...
	const deleteMutation = useMutation({
		mutationFn: (bankId: string) =>
			apiRequest(
				"DELETE",
				`/api/us-trading/broker/accounts/${alpacaAccountId}/recipient-banks/${bankId}`,
			),
		onSuccess: () => {
	...
	const withdrawMutation = useMutation({
		mutationFn: () =>
			apiRequest(
				"POST",
				`/api/us-trading/broker/accounts/${alpacaAccountId}/wire-withdrawal`,
				{
		...
		onSuccess: () => {
			toast({
				title: "Wire withdrawal initiated",
				description: `$${withdrawAmount} withdrawal queued`,
			});
		...
		<div className="space-y-5">
			<Alert className="border-amber-200 bg-amber-50 dark:bg-amber-950/20">
				...
				<AlertDescription className="text-sm text-amber-700 dark:text-amber-300">
					<strong>FEMA / LRS Notice:</strong> Wire withdrawals back to India are
					subject to RBI FEMA regulations. Capital gains must be declared per
					Indian tax law. Ensure your bank supports inward SWIFT remittances.
				</AlertDescription>
			</Alert>
				...
				<Card className="border-dashed">
					<CardContent className="py-10 text-center">
						...
						<p className="text-xs text-muted-foreground mb-4">
							Add your Indian bank account to withdraw USD proceeds.
						</p>
						<Button
							size="sm"
							onClick={() => setAddOpen(true)}
						...
						<div className="grid grid-cols-2 gap-3">
							...
							<div>
								...
								<Select
									...
									onValueChange={(v: any) =>
										setForm((f) => ({ ...f, bank_account_type: v }))
									}
								>
			...
			<Dialog open={withdrawOpen} onOpenChange={setWithdrawOpen}>
				<DialogContent className="max-w-sm">
					<DialogHeader>
						<DialogTitle className="flex items-center gap-2">
							<Send className="h-4 w-4" /> Initiate Wire Withdrawal
						</DialogTitle>
					</DialogHeader>
					<div className="space-y-3 py-2">
						<div>
					...
					<DialogFooter>
						...
						<Button
							...
							disabled={
								withdrawMutation.isPending || !selectedBankId || !withdrawAmount
							}
						>
