...
function isVoluntary(a: CorporateAction): boolean {
	return (
		a.is_voluntary === true ||
		VOLUNTARY_TYPES.has(getActionType(a).toLowerCase())
	);
}

function ActionBadge({ action }: { action: CorporateAction }) {
	const type = getActionType(action);
	const info = ACTION_LABELS[type.toLowerCase()] ?? {
		label: type.replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase()),
		color: "bg-gray-100 text-gray-600",
	};
	return (
		<span className={`text-xs px-2 py-0.5 rounded font-medium ${info.color}`}>
			{info.label}
		</span>
	);
}
...
function ElectionDialog({ action, accountId, onClose }: ElectionDialogProps) {
	...
	const mutation = useMutation({
		mutationFn: () =>
			apiRequest(
				`/api/us-trading/broker/accounts/${accountId}/corporate-actions/${action.id}/elections`,
				"POST",
				{
		...
		onSuccess: () => {
			toast({
				...
				description: `Your ${electionType} election for ${action.initiating_symbol ?? action.id} has been recorded.`,
			});
		...
		onError: (e: any) => {
			toast({
				...
				description: e?.message ?? "Unable to submit election. Try again.",
				variant: "destructive",
			});
		},
	});
	...
	const isMerger =
		type.toLowerCase() === "merger" || type.toLowerCase() === "tender_offer";
		...
		<Dialog open onOpenChange={onClose}>
			<DialogContent className="sm:max-w-md">
				<DialogHeader>
					...
					<DialogDescription>
						<strong>{action.initiating_symbol ?? "Unknown"}</strong> —{" "}
						{ACTION_LABELS[type.toLowerCase()]?.label ?? type}
						{action.election_deadline && (
							<span className="ml-1 text-orange-600 font-medium">
								(Deadline:{" "}
								{new Date(action.election_deadline).toLocaleDateString(
									"en-IN",
				...
				<div className="space-y-4 py-2">
					<RadioGroup
						value={electionType}
						onValueChange={(v) => setElectionType(v as any)}
						className="space-y-3"
					>
						<div className="flex items-start gap-3 p-3 border rounded-md hover:bg-muted/30 cursor-pointer">
							...
							<Label htmlFor="el-cash" className="cursor-pointer space-y-0.5">
								...
								<p className="text-xs text-muted-foreground">
									Receive{" "}
									{action.cash
										? `$${Number.parseFloat(action.cash).toFixed(4)}/share`
										: "cash consideration"}{" "}
									for your shares.
									{isMerger &&
										" India: cash received = capital gain (LTCG/STCG based on holding period). Report on Schedule CG of ITR-2/3."}
								</p>
							</Label>
						</div>
						<div className="flex items-start gap-3 p-3 border rounded-md hover:bg-muted/30 cursor-pointer">
							...
							<Label htmlFor="el-stock" className="cursor-pointer space-y-0.5">
								...
								<p className="text-xs text-muted-foreground">
									...
									{isMerger &&
										" India: stock exchange in a merger is generally treated as a deemed transfer — capital gains arise. DTAA relief may apply."}
									{isRights &&
										" India: rights allotment at a discount; cost basis = issue price; LTCG/STCG on later sale."}
								</p>
							</Label>
						</div>
						<div className="flex items-start gap-3 p-3 border rounded-md hover:bg-muted/30 cursor-pointer">
							...
							<Label htmlFor="el-mixed" className="cursor-pointer space-y-0.5">
								...
								<p className="text-xs text-muted-foreground">
									Partial cash + partial stock consideration. Both legs are
									taxable separately under Indian income tax rules.
								</p>
							</Label>
						</div>
						<div className="flex items-start gap-3 p-3 border rounded-md hover:bg-muted/30 cursor-pointer">
							...
							<Label htmlFor="el-none" className="cursor-pointer space-y-0.5">
								...
								<p className="text-xs text-muted-foreground">
									Accept the default outcome set by the issuer. If you do not
									elect, this is automatically applied.
								</p>
							</Label>
						</div>
					</RadioGroup>
					...
					<Alert className="border-amber-200 bg-amber-50/50 py-2">
						...
						<AlertDescription className="text-xs text-amber-700">
							<strong>India Tax:</strong> Both cash and stock elections may
							trigger capital gains under Section 45. Report under{" "}
							<strong>Schedule FA + Schedule CG</strong> of ITR-2/3. Consult a
							CA before electing on high-value corporate actions.
						</AlertDescription>
					</Alert>
				</div>
...
export default function CorporateActionsPanel({
	...
	const [electionTarget, setElectionTarget] = useState<CorporateAction | null>(
		null,
	);
	...
	const { data, isLoading, refetch } = useQuery<{
		success: boolean;
		announcements: CorporateAction[];
	}>({
		queryKey: [
			"/api/us-trading/broker/corporate-actions/announcements",
			filterType,
		],
		queryFn: () => {
			...
			return fetch(
				`/api/us-trading/broker/corporate-actions/announcements?${params}`,
			).then((r) => r.json());
		},
		staleTime: 300_000,
	...
	const upcoming = announcements.filter(
		(a) => (a.payable_date ?? a.ex_date ?? a.declaration_date ?? "") >= now,
	);
	...
	const portfolioActions = announcements.filter(
		(a) =>
			holdingSymbols.has(a.initiating_symbol ?? "") ||
			holdingSymbols.has(a.target_symbol ?? ""),
	);
	...
	function filterRows(rows: CorporateAction[]) {
		return rows.filter((a) => {
			if (
				filterSymbol !== "all" &&
				a.initiating_symbol !== filterSymbol &&
				a.target_symbol !== filterSymbol
			)
				return false;
			return true;
		});
				...
				<TableBody>
					{filtered.map((a) => (
						<TableRow key={a.id}>
							...
							<TableCell className="text-xs text-muted-foreground">
								{a.ex_date
									? new Date(a.ex_date).toLocaleDateString("en-IN", {
											month: "short",
											day: "numeric",
											year: "numeric",
										})
							...
							<TableCell className="text-xs text-muted-foreground">
								{a.payable_date
									? new Date(a.payable_date).toLocaleDateString("en-IN", {
											month: "short",
											day: "numeric",
											year: "numeric",
										})
							...
							<TableCell className="text-right text-xs">
								{a.cash
									? `$${Number.parseFloat(a.cash).toFixed(4)}/sh`
	...
	return (
		<div className="space-y-4">
					...
					<div className="flex items-center justify-between flex-wrap gap-2">
						<div>
							...
							<CardDescription className="text-xs mt-0.5">
								Dividends, splits, spinoffs, mergers — past 3 months and next 3
								months
							</CardDescription>
						</div>
								...
								<TabsList className="h-8 flex-wrap">
									{voluntary.length > 0 && (
										<TabsTrigger value="voluntary" className="text-xs gap-1">
											<Vote className="h-3 w-3" />
											Elect
											<Badge className="bg-amber-100 text-amber-700 text-[10px] h-4 px-1 ml-1">
												{voluntary.length}
											</Badge>
										</TabsTrigger>
									)}
									{portfolioActions.length > 0 && (
										<TabsTrigger value="portfolio" className="text-xs gap-1">
											My Holdings
											<Badge className="bg-blue-100 text-blue-700 text-[10px] h-4 px-1 ml-1">
			...
			<Alert className="border-amber-200 bg-amber-50/50 py-2">
				<Info className="h-3.5 w-3.5 text-amber-600" />
				<AlertDescription className="text-xs text-amber-700">
					<strong>India Tax Note:</strong> Cash dividends from US stocks attract
					25% withholding (DIVNRA) under IRS rules. Claim credit in India under{" "}
					<strong>India–US DTAA</strong> via <strong>Schedule FSI</strong>{" "}
					(ITR-2/3) and Form 67. For voluntary corporate actions (mergers,
					rights, tender offers) — capital gains arise on the date of
					receipt/allotment under Section 45. Consult a CA before electing on
