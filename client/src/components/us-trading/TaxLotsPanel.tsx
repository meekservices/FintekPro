...
function holdingDays(purchaseDate: string): number {
	return Math.floor(
		(Date.now() - new Date(purchaseDate).getTime()) / 86_400_000,
	);
}
...
export default function TaxLotsPanel({ accountId }: TaxLotsPanelProps) {
	...
	const { data, isLoading, refetch, isFetching } = useQuery<{
		...
		queryFn: () =>
			fetch(
				`/api/us-trading/broker/accounts/${accountId}/positions/tax-lots`,
			).then((r) => r.json()),
		enabled: !!accountId,
		staleTime: 60_000,
	});
	...
	rawLots.forEach((item: any) => {
		...
		if (item.lots) {
			item.lots.forEach((l: TaxLot) =>
				grouped[sym].push({ ...l, symbol: sym }),
			);
		} else {
	...
	// Summary totals
	const totalCost = symbols.reduce(
		(sum, s) =>
			sum +
			grouped[s].reduce((a, l) => a + (l.cost_basis ?? l.price * l.qty), 0),
		0,
	);
	const totalPl = symbols.reduce(
		(sum, s) =>
			sum + grouped[s].reduce((a, l) => a + (l.unrealized_pl ?? 0), 0),
		0,
	);
	const ltcgLots = symbols.flatMap((s) =>
		grouped[s].filter((l) => isLtcgIndia(l.purchase_date)),
	);
	const stcgLots = symbols.flatMap((s) =>
		grouped[s].filter((l) => !isLtcgIndia(l.purchase_date)),
	);
		...
		<div className="space-y-4">
			...
			<div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
				{[
					...
					{
						label: "LTCG Lots (India)",
						value: ltcgLots.length.toString(),
						sub: "Held >24 months",
					},
					{
						label: "STCG Lots (India)",
						value: stcgLots.length.toString(),
						sub: "Held ≤24 months",
					},
				].map((item) => (
					<Card key={item.label} className="p-3">
						...
						<p
							className={`text-lg font-bold mt-0.5 ${item.positive === true ? "text-emerald-600" : item.positive === false && totalPl < 0 ? "text-red-500" : ""}`}
						>
							{item.value}
						</p>
					...
					<div className="flex items-center justify-between">
						<div>
							...
							<CardDescription className="text-xs mt-0.5">
								Individual purchase lots — showing LTCG/STCG status for India
								and US tax rules
							</CardDescription>
						</div>
						<Button
				...
				<CardContent className="p-0">
					{isLoading ? (
						...
						<div className="py-10 text-center text-sm text-muted-foreground">
							No tax lot data available. You may not have any open positions, or
							the broker does not expose lot-level data for this account type.
						</div>
					) : (
						<div className="divide-y">
							{symbols.map((symbol) => {
								...
								const totalCostSym = lots.reduce(
									(s, l) => s + (l.cost_basis ?? l.price * l.qty),
									0,
								);
								const totalPlSym = lots.reduce(
									(s, l) => s + (l.unrealized_pl ?? 0),
									0,
								);
								...
								return (
									<Collapsible
										key={symbol}
										open={expanded}
										onOpenChange={() => toggleSymbol(symbol)}
									>
												...
												<div className="flex items-center gap-4 text-right">
													...
													<div>
														...
														<p className="text-sm font-medium">
															$
															{totalCostSym.toLocaleString("en-US", {
													...
													<div>
														...
														<p
															className={`text-sm font-medium ${totalPlSym > 0 ? "text-emerald-600" : totalPlSym < 0 ? "text-red-500" : ""}`}
														>
													...
													<TableBody>
														{lots
															...
															.map((lot, idx) => {
																...
																const costBasis =
																	lot.cost_basis ?? lot.price * lot.qty;
																...
																return (
																	<TableRow key={lot.id ?? idx}>
																		...
																		<TableCell
																			className={`text-xs py-2 text-right font-medium ${pl > 0 ? "text-emerald-600" : pl < 0 ? "text-red-500" : ""}`}
																		>
																			{pl !== 0 ? (
			...
			<Alert className="border-blue-200 bg-blue-50/50 py-2">
				...
				<AlertDescription className="text-xs text-blue-700 space-y-0.5">
					<p>
						<strong>India Tax on US Equity (Section 112 / 112A):</strong> Gains
						from US stocks held <strong>≤24 months</strong> = Short-Term (slab
						rate, up to 30%). Held <strong>&gt;24 months</strong> = Long-Term
						(20% with indexation, or 10% without above ₹1L).
					</p>
					<p>
						<strong>US Tax:</strong> ≤1 yr = ordinary income; &gt;1 yr = 0–20%
						LTCG (based on income bracket). Claim credit in India under{" "}
						<strong>India–US DTAA Article 13</strong> via Schedule FSI + Form 67
						of ITR.
					</p>
					<p>
						<strong>Tax-Loss Harvesting:</strong> Realising STCG losses before
						31 March offsets STCG gains. Use specific-lot selection to close
