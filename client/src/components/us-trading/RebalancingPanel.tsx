...
export default function RebalancingPanel({ accountId }: RebalancingPanelProps) {
  ...
  const [weightsText, setWeightsText] = useState("AAPL:25\nMSFT:25\nGOOGL:25\nAMZN:25");
  const [runningPortfolioId, setRunningPortfolioId] = useState<string | null>(null);

  const { data: portfolioData, isLoading, refetch } = useQuery<{ success: boolean; portfolios: RebalancingPortfolio[] }>({
    queryKey: ["/api/us-trading/broker/rebalancing/portfolios"],
    queryFn: () => fetch("/api/us-trading/broker/rebalancing/portfolios").then(r => r.json()),
    staleTime: 60_000,
  });
  ...
  const createMutation = useMutation({
    mutationFn: () => {
      const weights = weightsText.split("\n").filter(Boolean).map(line => {
        ...
        return { symbol: symbol.trim().toUpperCase(), percent: parseFloat(percent ?? "0") };
      });
      const totalPct = weights.reduce((s, w) => s + w.percent, 0);
      if (Math.abs(totalPct - 100) > 0.01) throw new Error(`Weights must sum to 100% (currently ${totalPct.toFixed(2)}%)`);
      return apiRequest("/api/us-trading/broker/rebalancing/portfolios", {
        method: "POST",
        body: JSON.stringify({ name: portfolioName, description: portfolioDesc, weights }),
      });
    },
    onSuccess: () => {
      toast({ title: "Portfolio created", description: "Your rebalancing portfolio has been created." });
  ...
  const subscribeMutation = useMutation({
    mutationFn: (portfolioId: string) => {
      ...
      return apiRequest(`/api/us-trading/broker/rebalancing/portfolios/${portfolioId}/subscriptions`, {
        method: "POST",
        body: JSON.stringify({ account_id: accountId }),
      });
    },
    onSuccess: () => {
      toast({ title: "Subscribed", description: "Account subscribed to portfolio. Rebalancing will run as scheduled." });
      refetch();
    },
  ...
  const runMutation = useMutation({
    mutationFn: (portfolioId: string) => {
      ...
      return apiRequest(`/api/us-trading/broker/rebalancing/portfolios/${portfolioId}/runs`, {
        method: "POST",
        body: JSON.stringify({ type: "full_rebalance" }),
      });
    },
    onSuccess: () => {
      toast({ title: "Rebalancing triggered", description: "Rebalancing run initiated. Orders will be placed shortly." });
    ...
    <div className="space-y-4">
          ...
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div>
              ...
              <CardDescription className="text-xs mt-0.5">
                Define target allocations. Alpaca automatically places orders to match your target weights.
              </CardDescription>
            </div>
            <div className="flex items-center gap-2">
              ...
              <Button size="sm" onClick={() => setNewPortfolioOpen(true)} className="h-8 gap-1 text-xs">
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="py-10 text-center text-sm text-muted-foreground">
              ...
              <p className="text-xs mt-1">Create a portfolio to define target allocations.</p>
            </div>
          ) : (
            <Table>
              <TableHeader>
                ...
                {portfolios.map(p => (
                  <TableRow key={p.id}>
                    ...
                    <TableCell className="text-xs text-muted-foreground max-w-[160px]">
                      {p.weights?.slice(0, 4).map(w => `${w.symbol} ${w.percent}%`).join(", ")}
                      {p.weights?.length > 4 && ` +${p.weights.length - 4} more`}
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center gap-1">
                        ...
                        <Button
                          ...
                          disabled={runMutation.isPending && runningPortfolioId === p.id}
                        >
      ...
      <Alert className="border-blue-200 bg-blue-50/50 py-2">
        ...
        <AlertDescription className="text-xs text-blue-700">
          <strong>How it works:</strong> Alpaca's rebalancing engine places buy/sell orders to bring your portfolio in line with target weights. Orders are market orders placed during regular hours. Fractional shares are used when needed. Tax implications (capital gains) apply — consult your CA before running rebalances frequently.
        </AlertDescription>
      </Alert>
          ...
          <div className="space-y-4 py-2">
            ...
            <div>
              <Label className="text-xs flex items-center justify-between">
                ...
                <span className={totalPercent > 100.01 ? "text-red-500" : totalPercent < 99.99 ? "text-amber-500" : "text-green-600"}>
              ...
              {Math.abs(totalPercent - 100) > 0.01 && (
                <p className="text-xs text-amber-600 mt-1">Weights must sum to exactly 100%</p>
              )}
            </div>
          </div>
          <DialogFooter>
            ...
            <Button
              ...
              disabled={!portfolioName || Math.abs(totalPercent - 100) > 0.01 || createMutation.isPending}
            >
