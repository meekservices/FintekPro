...
export function KycUpgradeBanner() {
  ...
  const { currentTier, percentComplete, missingSteps, blockedProducts, urgencyLevel } = kycData;
...
export function KycBlockedProductAlert({ productName }: { productName: string }) {
  const [, setLocation] = useLocation();

  return (
    <Alert variant="destructive" className="mb-4" data-testid="kyc-blocked-alert">
      <Lock className="h-4 w-4" />
      <AlertTitle>KYC Required</AlertTitle>
      <AlertDescription className="flex items-center justify-between">
        <span>
          Complete your KYC verification to access {productName}.
        </span>
        <Button 
          size="sm" 
          variant="outline"
...
export function KycProgressWidget() {
  ...
  const { currentTier, percentComplete, missingSteps, blockedProducts } = kycData;
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-3">
        <h3 className="font-semibold text-gray-900 dark:text-gray-100">KYC Progress</h3>
