...
const SCHEME_DESCRIPTIONS: Record<SchemeType, string> = {
	...
	insurance:
		"View your insurance policies, premium details, and coverage information",
};
	...
	const {
		...
		queryFn: async () => {
			...
			const response = await fetch(
				`/api/government-schemes/consent/${user.panNumber}/${schemeType}`,
				{
					credentials: "include",
				},
			);
				...
				queryClient.invalidateQueries({
					predicate: (query) => {
						...
						if (Array.isArray(key)) {
							...
							return (
								keyStr.includes(`government-schemes/${schemeType}`) ||
					...
					<CardContent>
						<div className="text-center py-8">
							...
							<p className="text-muted-foreground mb-4">
								To access {SCHEME_NAMES[schemeType]} data, please complete your
								KYC by adding your PAN card in your profile.
							</p>
							<Button
								variant="outline"
					...
					<CardContent>
						<div className="text-center py-8">
							...
							<p className="text-muted-foreground mb-4 max-w-md mx-auto">
								{SCHEME_DESCRIPTIONS[schemeType]} for PAN number{" "}
								{user.panNumber}.
							</p>
								...
								<div className="flex items-start gap-3">
									...
									<div className="text-left">
										...
										<p className="text-sm text-blue-800 dark:text-blue-200">
											Your data is accessed through secure government APIs. We
											only retrieve the information necessary for portfolio
											management and never store sensitive data.
										</p>
									</div>
								</div>
							</div>
	...
	return (
		<div className="space-y-8">
			<div className="flex items-center justify-between">
				<div className="bg-green-50 dark:bg-green-950/20 border border-green-200 dark:border-green-800 rounded-lg p-3 flex-1 mr-4">
					<div className="flex items-center gap-2 text-green-800 dark:text-green-200">
						...
						<span className="text-sm font-medium">
							Authorized access to {SCHEME_NAMES[schemeType]} data for PAN{" "}
							{user.panNumber}
						</span>
					</div>
				...
				>
					<RefreshCw
						className={`h-4 w-4 ${isRefreshing || initiateRefreshMutation.isPending ? "animate-spin" : ""}`}
					/>
					Refresh Data
				</Button>
			</div>
			...
			<Dialog open={showOtpDialog} onOpenChange={setShowOtpDialog}>
				<DialogContent className="sm:max-w-md">
					<DialogHeader>
						...
						<DialogDescription>
							To refresh your {SCHEME_NAMES[schemeType]} data from{" "}
							{SCHEME_SOURCES[schemeType]}, please enter the OTP sent to your
						...
						<div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 rounded-lg p-3">
							<div className="flex items-start gap-2">
								...
								<div className="text-sm text-amber-800 dark:text-amber-200">
									...
									<p className="text-xs">
										This verification will be recorded in our audit log as per
										RBI/PMLA compliance requirements. Data retention period: 8
										years.
									</p>
								</div>
							</div>
