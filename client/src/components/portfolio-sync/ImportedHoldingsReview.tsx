...
export function ImportedHoldingsReview({
	userId,
	onSyncComplete,
	onCancel,
}: ImportedHoldingsReviewProps) {
	...
	const [editingHolding, setEditingHolding] = useState<StagedHolding | null>(
		null,
	);
	...
	const filteredHoldings = useMemo(() => {
		return holdings.filter((holding) => {
			...
			const matchesSource =
				sourceFilter === "all" || holding.source === sourceFilter;
	...
	const approveHoldingsMutation = useMutation({
		...
		onSuccess: () => {
			toast({
				title: "Holdings Approved",
				description: "Selected holdings marked for sync",
			});
			refetch();
		},
		onError: (error: any) => {
	...
	const rejectHoldingsMutation = useMutation({
		...
		onSuccess: () => {
			toast({
				title: "Holdings Rejected",
				description: "Selected holdings will not be synced",
			});
			refetch();
		},
		onError: (error: any) => {
	...
	const handleFinalSync = () => {
		const approvedCount = holdings.filter(
			(h) => h.status === "approved" || h.status === "modified",
		).length;
		if (approvedCount === 0) {
			toast({
				...
				<CardContent className="py-12">
					<div className="text-center">
						...
						<p className="text-muted-foreground mt-2">
							Import your portfolio via Account Aggregator or upload a CAS
							statement to see holdings here.
						</p>
					</div>
				</CardContent>
			</Card>
				...
				<div className="flex flex-wrap gap-4 items-center justify-between">
					...
					<div className="flex items-center gap-2">
						<Button
							...
							disabled={
								selectedHoldings.size === 0 || approveHoldingsMutation.isPending
							}
						>
						...
						<Button
							...
							disabled={
								selectedHoldings.size === 0 || rejectHoldingsMutation.isPending
							}
				...
				{holdings.some((h) => h.validationErrors?.length) && (
					<Alert className="bg-amber-50 dark:bg-amber-950/30 border-amber-200 dark:border-amber-800">
						...
						<AlertDescription className="text-amber-800 dark:text-amber-200">
							Some holdings have validation issues. Review and correct before
							syncing.
						</AlertDescription>
					</Alert>
				)}
						...
						<TableHeader>
							<TableRow>
								<TableHead className="w-[40px]">
									<Checkbox
										checked={
											selectedHoldings.size === filteredHoldings.length &&
					...
					<div className="flex items-center justify-between">
						<div className="space-y-1">
							<div className="text-sm text-muted-foreground">
								Total Portfolio Value (Approved)
							</div>
						...
						<div className="flex items-center gap-3">
							<div className="text-right">
								...
								<div className="text-lg font-semibold">
									{summary.approved + summary.modified} of {summary.total}{" "}
									holdings
								</div>
							</div>
						</div>
			...
			<CardFooter className="flex justify-between">
				...
				<div className="flex gap-2">
					<Button
						variant="outline"
						onClick={() => {
							approveHoldingsMutation.mutate(
								holdings.filter((h) => h.status === "pending").map((h) => h.id),
							);
						}}
						disabled={
							summary.pending === 0 || approveHoldingsMutation.isPending
						}
					>
					...
					<Button
						...
						disabled={
							...
							summary.approved + summary.modified === 0
						}
					>
						{finalSyncMutation.isPending ? (
					...
					{editingHolding && (
						<div className="space-y-4 py-4">
							...
							<div className="grid grid-cols-2 gap-4">
								<div>
									<label className="text-sm font-medium text-foreground">
										{editingHolding.assetType === "mutual_fund"
											? "Units"
											: "Quantity"}
									</label>
									<Input
								...
								<div>
									...
									<Input
										...
										onChange={(e) =>
											setEditForm({
												...editForm,
												averageCost: Number.parseFloat(e.target.value),
											})
										}
										className="mt-1"
									/>
