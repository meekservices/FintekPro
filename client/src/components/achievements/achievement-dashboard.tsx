	...
	const filteredAchievements =
		selectedCategory === "all"
			...
			: userAchievements.filter(
					(ua) => ua.achievement.categoryId === selectedCategory,
				);

	const completedAchievements = filteredAchievements.filter(
		(ua) => ua.isCompleted,
	);
				...
				<Card className="bg-gradient-to-br from-blue-50 dark:from-blue-950/30 to-indigo-50 dark:to-indigo-950/30 border-blue-200 dark:border-blue-800">
					<CardContent className="p-6">
						...
						{userStats && (
							<div className="mt-3">
								<Progress
									value={(userStats.pointsToNextLevel / 1000) * 100}
									className="h-2"
								/>
					...
					<CardContent className="p-6">
						<div className="flex items-center justify-between">
							<div>
								...
								<Badge
									className={`${getRankBadge(userStats?.rank || "Bronze")} text-sm px-2 py-1`}
								>
							...
							<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
								{completedAchievements.map((userAchievement) => (
									<Card
										...
										className="border-2 border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-950/30/50"
										data-testid={`achievement-${userAchievement.achievementId}`}
									>
											...
											<div className="flex items-start justify-between">
												<div className="flex items-center space-x-3">
													<div className="p-2 bg-green-100 dark:bg-green-900/30 rounded-lg">
														{getCategoryIcon(
															userAchievement.achievement.category?.name || "",
														)}
													</div>
													<div>
														...
														<div className="flex items-center space-x-2 mt-1">
															...
															<Badge
																className={`text-xs ${getDifficultyColor(userAchievement.achievement.difficulty)}`}
															>
																{userAchievement.achievement.difficulty}
															</Badge>
														</div>
						...
						<div>
							...
							<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
								{inProgressAchievements.map((userAchievement) => (
									<Card
										key={userAchievement.id}
										className="border-2 border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-950/30/50"
									>
										<CardHeader className="pb-3">
											<div className="flex items-start justify-between">
												<div className="flex items-center space-x-3">
						...
						<div>
							...
							<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
								{lockedAchievements.slice(0, 6).map((userAchievement) => (
									<Card
										key={userAchievement.id}
										className="border-border bg-muted/50 opacity-75"
									>
										<CardHeader className="pb-3">
											<div className="flex items-start justify-between">
												<div className="flex items-center space-x-3">
						...
						<div className="space-y-3">
							{leaderboard.map((entry: any, index: number) => (
								...
								>
									<div className="flex items-center space-x-3">
										<Badge
											className={`${index < 3 ? "bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-200" : "bg-muted text-white"}`}
										>
											#{index + 1}
										</Badge>
										<div>
