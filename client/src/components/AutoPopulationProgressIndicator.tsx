...
export function AutoPopulationProgressIndicator({
	...
	if (!statusData?.status) {
		return (
			<Alert
				variant="destructive"
				className={className}
				data-testid="progress-indicator-error"
			>
				<AlertDescription>
	...
	return (
		<Card
			className={cn("w-full", className)}
			data-testid="progress-indicator-card"
		>
			<CardHeader>
			...
			<CardContent className="space-y-6">
				...
				<div className="space-y-2">
					<div className="flex justify-between text-sm">
						...
						>
							{summary.successfulSources} of {summary.totalSources} sources
							completed
						</span>
					</div>
					<Progress
					...
					<div className="flex justify-between text-xs text-muted-foreground">
						<span data-testid="text-records-fetched">
							{summary.totalRecordsFetched} records fetched
						</span>
									...
									<div className="flex items-center gap-3">
										...
										<div>
											<p
												...
												data-testid={`text-source-label-${sourceStatus.source}`}
											>
												{config.label}
											</p>
											{sourceStatus.recordsFetched !== undefined &&
												sourceStatus.recordsFetched > 0 && (
													<p
														className="text-xs text-muted-foreground"
														data-testid={`text-records-${sourceStatus.source}`}
													>
											...
											{sourceStatus.error && (
												<p
													...
													data-testid={`text-error-${sourceStatus.source}`}
												>
													{sourceStatus.error}
												</p>
											)}
				...
				{workflowStatus === "completed" && (
					<Alert
						...
						data-testid="alert-completed"
					>
						<CheckCircle2 className="h-4 w-4 text-green-600 dark:text-green-400" />
						<AlertDescription className="text-green-900 dark:text-green-100">
							Auto-population completed successfully! Your financial data has
							been synced.
						</AlertDescription>
					</Alert>
				)}
				...
				{workflowStatus === "failed" && (
					<Alert variant="destructive" data-testid="alert-failed">
						...
						<AlertDescription>
							Auto-population failed. Please check the errors above and try
							again.
						</AlertDescription>
					</Alert>
				)}
