...
export function DSCSigningModal({
  ...
  const [selectedCert, setSelectedCert] = useState<DSCCertificateInfo | null>(null);
  ...
  const detectToken = async () => {
    ...
    try {
      ...
      if (info) {
        ...
        setError('No DSC token detected. Please insert your USB token and try again.');
      }
    } catch (err) {
      setError('Failed to detect DSC token. Please check if the token is properly connected.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSign = async () => {
    ...
    try {
      ...
      if (!initiateResponse.success) {
        throw new Error(initiateResponse.message || 'Failed to initiate signing session');
      }

      setTransactionId(initiateResponse.transactionId);

      const signResult = await dscBridge.signData(initiateResponse.dataToSign, 'SHA256withRSA');
      setProgress(85);
      ...
      <div className="flex flex-col items-center justify-center py-8">
        {isLoading ? (
          ...
          <>
            ...
            <p className="text-muted-foreground">Please insert your DSC USB token</p>
            ...
            return (
              <Card 
                ...
                className={`cursor-pointer transition-all hover:border-primary ${
                  selectedCert?.serialNumber === cert.serialNumber ? 'border-primary bg-primary/5' : ''
                } ${displayInfo.isExpired ? 'opacity-50' : ''}`}
                onClick={() => !displayInfo.isExpired && handleCertificateSelect(cert)}
                  ...
                  <div className="flex items-start justify-between">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        ...
                        <Badge variant={
                          cert.certificateClass === 'Class3' ? 'default' :
                          cert.certificateClass === 'Class2' ? 'secondary' : 'outline'
                        }>
                      ...
                      <div className="flex items-center gap-2 text-sm">
                        ...
                        <span className={displayInfo.isExpiringSoon ? 'text-orange-500' : ''}>
                          Valid until: {displayInfo.validUntil}
                          {displayInfo.isExpiringSoon && ` (${displayInfo.daysRemaining} days remaining)`}
                        </span>
                      </div>
                    </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          <div className="flex justify-between">
            ...
            <span className="font-medium">{selectedCert?.subject.commonName}</span>
          </div>
          <div className="flex justify-between">
      ...
      {isLoading && (
        <div className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground text-center">
            {progress < 70 ? 'Preparing signature...' : 
             progress < 90 ? 'Signing with DSC token...' : 
    ...
    <Dialog open={open} onOpenChange={(isOpen) => !isOpen && onClose()}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Use your Digital Signature Certificate (USB Token) to sign the document
          </DialogDescription>
        </DialogHeader>
