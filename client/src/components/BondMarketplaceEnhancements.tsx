...
export function EligibilityBadge({
	...
	if (eligible) {
		return (
			<Badge
				className="bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 border-green-200 dark:border-green-800 flex items-center gap-1"
				variant="outline"
			>
...
export function DataFreshnessIndicator({
	lastUpdated,
	source,
}: { lastUpdated: Date | null; source: string }) {
	const now = new Date();
	...
	return (
		<TooltipProvider>
			<Tooltip>
				<TooltipTrigger asChild>
					<div
						className={`flex items-center gap-1 text-xs ${config.color} ${config.bgColor} px-2 py-1 rounded-full`}
					>
				...
				<TooltipContent>
					<div className="text-xs">
						<p>
							Source:{" "}
							{source === "government_securities" ? "RBI/ReDIES" : "NSE/BSE"}
						</p>
						{lastUpdated && (
							<p>
								Updated:{" "}
	...
	return (
		<div className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 p-3 rounded-lg border border-blue-200 dark:border-blue-700">
			...
			<div className="flex items-baseline gap-2">
				...
				<Badge
					variant="outline"
					className="text-xs bg-red-50 dark:bg-red-950/30 text-red-600 border-red-200 dark:border-red-800"
				>
...
export function RiskDisclosureModal({
	...
	const allAcknowledged =
		disclosures?.every((d) => acknowledged[d.id]) ?? false;
		...
		<div className="space-y-4">
			<Alert className="bg-amber-50 dark:bg-amber-950/30 border-amber-200 dark:border-amber-800">
				...
				<AlertDescription className="text-amber-700 dark:text-amber-300 text-sm">
					Before proceeding with your investment in {bondName}, you must
					acknowledge the following risks as mandated by SEBI regulations.
				</AlertDescription>
			</Alert>
							...
							<div className="flex-1 space-y-2">
								...
								{disclosure.regulatoryReference && (
									<p className="text-xs opacity-60">
										Reference: {disclosure.regulatoryReference}
									</p>
								)}
							</div>
...
const getFilterCreditRatingColors = (
	rating: string,
	isSelected: boolean,
): string => {
	...
	if (r === "SOV" || r === "AAA")
		return "bg-emerald-50 dark:bg-emerald-950/30 text-emerald-700 dark:text-emerald-300 border-emerald-200 dark:border-emerald-800 hover:bg-emerald-100 dark:bg-emerald-900/30";
	if (r.startsWith("AA"))
		return "bg-green-50 dark:bg-green-950/30 text-green-700 dark:text-green-300 border-green-200 dark:border-green-800 hover:bg-green-100 dark:bg-green-900/30";
	if (r.startsWith("A"))
		return "bg-blue-50 dark:bg-blue-950/30 text-blue-700 dark:text-blue-300 border-blue-200 dark:border-blue-800 hover:bg-blue-100 dark:bg-blue-900/30";
	if (r.startsWith("BBB"))
		return "bg-yellow-50 dark:bg-yellow-950/30 text-yellow-700 dark:text-yellow-300 border-yellow-200 dark:border-yellow-800 hover:bg-yellow-100 dark:bg-yellow-900/30";
...
const getProgressiveColorLight = (
	value: number,
	min: number,
	max: number,
): string => {
...
const getProgressiveLabelColor = (
	value: number,
	min: number,
	max: number,
): string => {
...
const getFilterInstrumentTypeColors = (
	type: string,
	isSelected: boolean,
): string => {
	...
	if (isSelected) {
		// Selected state - darker, more prominent colors
		if (t === "gsec" || t === "government")
			return "bg-blue-600 text-white border-blue-700 hover:bg-blue-700";
		if (t === "sdl" || t === "state")
			return "bg-indigo-600 text-white border-indigo-700 hover:bg-indigo-700";
		if (t === "tbill")
			return "bg-sky-600 text-white border-sky-700 hover:bg-sky-700";
		if (t === "sgb" || t === "gold")
	...
	if (t === "gsec" || t === "government")
		return "bg-blue-50 dark:bg-blue-950/30 text-blue-700 dark:text-blue-300 border-blue-200 dark:border-blue-800 hover:bg-blue-100 dark:bg-blue-900/30";
	if (t === "sdl" || t === "state")
		return "bg-indigo-50 dark:bg-indigo-950/30 text-indigo-700 dark:text-indigo-300 border-indigo-200 dark:border-indigo-800 hover:bg-indigo-100 dark:bg-indigo-900/30";
	if (t === "tbill")
		return "bg-sky-50 dark:bg-sky-950/30 text-sky-700 dark:text-sky-300 border-sky-200 dark:border-sky-800 hover:bg-sky-100 dark:bg-sky-900/30";
	if (t === "sgb" || t === "gold")
		return "bg-amber-50 dark:bg-amber-950/30 text-amber-700 dark:text-amber-300 border-amber-200 dark:border-amber-800 hover:bg-amber-100 dark:bg-amber-900/30";
	if (t === "corporate")
		return "bg-purple-50 dark:bg-purple-950/30 text-purple-700 dark:text-purple-300 border-purple-200 dark:border-purple-800 hover:bg-purple-100 dark:bg-purple-900/30";
	if (t === "ncd")
		return "bg-orange-50 dark:bg-orange-950/30 text-orange-700 dark:text-orange-300 border-orange-200 dark:border-orange-800 hover:bg-orange-100 dark:bg-orange-900/30";
	if (t === "infrastructure")
		return "bg-teal-50 dark:bg-teal-950/30 text-teal-700 dark:text-teal-300 border-teal-200 dark:border-teal-800 hover:bg-teal-100 dark:bg-teal-900/30";
	if (t === "tax_free")
		return "bg-green-50 dark:bg-green-950/30 text-green-700 dark:text-green-300 border-green-200 dark:border-green-800 hover:bg-green-100 dark:bg-green-900/30";
	return "bg-muted text-muted-foreground border-border hover:bg-muted";
};
			...
			<div className="space-y-4">
				<div>
					...
					<div className="flex flex-wrap gap-2">
						{creditRatings.map((rating) => {
							const isSelected =
								filters.creditRating?.includes(rating) || false;
							return (
								<Badge
									key={rating}
									variant="outline"
				...
				<div>
					<Label
						...
						style={{
							color: getProgressiveLabelColor(
								filters.maturityRange?.[1] || 30,
								0,
								30,
							),
						}}
					>
						...
						<span className="font-bold">
							{filters.maturityRange?.[0] || 0} -{" "}
							{filters.maturityRange?.[1] || 30} years
						</span>
					</Label>
					<div
						...
						style={
							{
								"--slider-track-bg": getProgressiveColorLight(
									filters.maturityRange?.[1] || 30,
									0,
									30,
								),
								"--slider-range-bg": getProgressiveColor(
									filters.maturityRange?.[1] || 30,
									0,
									30,
								),
								"--slider-thumb-border": getProgressiveColor(
									filters.maturityRange?.[1] || 30,
									0,
									30,
								),
					...
					>
						<Slider
							...
							onValueChange={(value) =>
								onFiltersChange({
									...filters,
									maturityRange: value as [number, number],
								})
							}
							min={0}
							max={30}
				...
				<div>
					...
					>
						Yield Range:{" "}
						<span className="font-bold">
							{filters.yieldRange?.[0] || 0}% - {filters.yieldRange?.[1] || 15}%
						</span>
					</Label>
					<div
						className="py-4 px-2 rounded-lg"
				...
				<div>
					...
					<Select
						...
						onValueChange={(v) =>
							onFiltersChange({
								...filters,
								minInvestment: v === "any" ? undefined : Number.parseInt(v),
							})
						}
					>
				...
				<div>
					...
					<div className="flex flex-wrap gap-2">
						{instrumentTypes.map((type) => {
							const isSelected =
								filters.instrumentType?.includes(type) || false;
							return (
								<Badge
									key={type}
									variant="outline"
				...
				<div className="flex items-center gap-2">
					<Checkbox
						...
						onCheckedChange={(checked) =>
							onFiltersChange({ ...filters, taxFree: !!checked })
						}
				...
				<div>
					...
					>
						...
						<SelectContent>
							...
							<SelectItem value="min_investment_asc">
								Lowest Investment
							</SelectItem>
						</SelectContent>
					</Select>
									...
									<p className="text-xs text-muted-foreground mt-1">
										...
										{(
											bucket.bonds.reduce(
												(sum, b) =>
													sum +
													(Number.parseFloat(b.yieldToMaturity || "0") || 0),
												0,
											) / bucket.bonds.length
										).toFixed(2)}
										%
...
export function BondComparisonTable({
	...
	const attributes = [
		...
		},
		{
			...
			key: "yearsToMaturity",
			format: (v: number) => (v ? `${v.toFixed(1)} yrs` : "-"),
		},
		{
		...
		},
		{
			label: "Tax Status",
			key: "isTaxFree",
			format: (v: boolean) => (v ? "Tax Free" : "Taxable"),
		},
						...
						<tr key={attr.key} className="border-t border-border">
							...
							{selectedBonds.map((bond) => {
								...
								const formatted = attr.format
									? (attr.format as (v: unknown) => string)(value)
									: value ?? "-";
								return (
...
export function useEnhancedBondMarketplace(userId?: string) {
	...
	const isWatched = (bondId: string) =>
		watchlist?.some((w) => w.bondId === bondId) ?? false;
	const hasAlert = (bondId: string) =>
		alerts?.some((a) => a.bondId === bondId && a.isActive) ?? false;
