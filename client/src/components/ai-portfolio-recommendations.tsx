...
export default function AIPortfolioRecommendations({
	portfolioId,
}: AIPortfolioRecommendationsProps) {
	...
	const investmentProposalMutation = useMutation({
		...
		onSuccess: (data: any) => {
			if (data?.success) {
				toast({
					...
					description:
						"AI has analyzed your portfolio and generated personalized recommendations.",
				});
				queryClient.invalidateQueries({
					queryKey: [`/api/ai/portfolios/${portfolioId}/investment-proposal`],
				});
			} else {
				toast({
					title: "Generation Failed",
					description:
						data?.message || "Unable to generate investment proposal",
					variant: "destructive",
				});
			}
		},
		onError: (error) => {
			toast({
				title: "Error",
				description:
					"Failed to generate investment proposal. Please try again.",
				variant: "destructive",
			});
		},
	});
	...
	const rebalancingRecommendations: AIRecommendation[] =
		(rebalancingData as any)?.data || [];
			...
			<Card className="bg-gradient-to-r from-purple-50 to-blue-50 dark:from-purple-900/20 dark:to-blue-900/20 border-purple-200 dark:border-purple-800">
				<CardHeader>
					...
					<CardDescription className="text-purple-600 dark:text-purple-400">
						Get personalized investment recommendations and portfolio
						optimization powered by advanced AI analysis
					</CardDescription>
				</CardHeader>
			</Card>
				...
				<Card>
					...
					<CardContent>
						<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
							...
							<div className="text-center">
								<div
									className={`text-2xl font-bold ${portfolioSummary.performance?.totalGainLossPercent >= 0 ? "text-green-600" : "text-red-600"}`}
								>
									{portfolioSummary.performance?.totalGainLossPercent > 0
										? "+"
										: ""}
									{portfolioSummary.performance?.totalGainLossPercent?.toFixed(
										2,
									) || "0.00"}
									%
								</div>
			...
			<Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
				<ScrollableTabsList className="grid w-full grid-cols-2">
					...
					<TabsTrigger
						...
						data-testid="tab-investment-proposal"
					>
				...
				<TabsContent value="investment-proposal" className="space-y-6">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Get a comprehensive AI-generated investment proposal based on
							...
							<div className="space-y-4">
								<div>
									...
									<Input
										...
										onChange={(e) =>
											setAdditionalCapital(Number(e.target.value) || 0)
										}
										data-testid="input-additional-capital"
									/>
									<p className="text-xs text-muted-foreground mt-1">
										Optional: Specify additional capital you want to invest for
					...
					<Alert>
						...
						<AlertDescription>
							<strong>Disclaimer:</strong> AI-generated recommendations are for
							informational purposes only and should not be considered as
							financial advice. Please consult with a qualified financial
