...
const DATA_SOURCES: DataSource[] = [
	{
		...
		description:
			"Life, health, and general insurance policies from Turtlefin and other providers",
		defaultEnabled: true,
	},
	{
		id: "loans",
		...
		description:
			"Home loans, personal loans, credit card debts via CIBIL credit report",
		defaultEnabled: true,
	},
	{
		id: "mutual_funds",
...
export function AutoPopulationConsentDialog({
	...
	const consentMutation = useMutation({
		mutationFn: async () => {
			...
			return await apiRequest("/api/consents/batch-grant", "POST", {
				consents,
			});
		},
		onSuccess: () => {
	...
	return (
		<Dialog open={isOpen} onOpenChange={onOpenChange}>
			<DialogContent
				...
				data-testid="dialog-auto-population-consent"
			>
				<DialogHeader>
					<DialogTitle
						className="flex items-center gap-2"
						data-testid="text-auto-pop-consent-title"
					>
					...
					<DialogDescription data-testid="text-auto-pop-consent-description">
						Grant FintekPro permission to securely fetch and consolidate your
						financial data from multiple sources. This will give you a complete
						view of your wealth in one place.
					</DialogDescription>
				</DialogHeader>
				...
				<div className="space-y-6">
					...
					<div className="border rounded-lg p-4 bg-green-50 dark:bg-green-950/20">
						...
						<ul className="text-green-800 dark:text-green-200 text-sm space-y-1">
							<li>
								• <strong>Complete Picture:</strong> See all your investments,
								insurance, and loans in one dashboard
							</li>
							<li>
								• <strong>Accurate Analytics:</strong> Get precise IRR, XIRR,
								and asset allocation calculations
							</li>
							<li>
								• <strong>Time Saving:</strong> No manual data entry required
							</li>
							<li>
								• <strong>Always Updated:</strong> Automatic syncing keeps your
								portfolio current
							</li>
						</ul>
					</div>
					...
					<div className="border rounded-lg p-4">
						...
						<div className="space-y-3">
							{DATA_SOURCES.map((source) => (
								<div
									key={source.id}
									className="flex items-start space-x-3 p-3 border rounded hover:bg-accent/50 transition-colors"
								>
									<Checkbox
										id={`source-${source.id}`}
					...
					<div className="border rounded-lg p-4 bg-amber-50 dark:bg-amber-950/20">
						...
						<ul className="text-amber-800 dark:text-amber-200 text-sm space-y-1">
							<li>
								• All data is fetched through secure, encrypted government and
								financial institution APIs
							</li>
							<li>
								• Your credentials are never stored; we use token-based
								authentication
							</li>
							<li>
							...
							<li>
								• Access is limited to read-only operations for portfolio
								display
							</li>
							<li>
								• You can revoke consent for any data source at any time from
								Settings
							</li>
						</ul>
					</div>
					...
					<div className="border rounded-lg p-4 bg-blue-50 dark:bg-blue-950/20">
						...
						<p className="text-blue-800 dark:text-blue-200 text-sm">
							These consents will be valid for <strong>1 year</strong> from
							today. You will be prompted to renew consent when it expires. You
							can review or revoke consents anytime from your Profile → Privacy
							Settings.
						</p>
					</div>
					...
					<div className="flex items-start space-x-3 p-4 border rounded-lg bg-muted/50">
						...
						<div className="grid gap-1.5 leading-none">
							...
							<p className="text-xs text-muted-foreground">
								...
								selected financial data sources for portfolio management and
								analytics. This consent complies with RBI Account Aggregator
								Framework and data protection regulations.
							</p>
						</div>
					</div>
					...
					<div className="text-center text-sm text-muted-foreground">
						{selectedSources.size} of {DATA_SOURCES.length} data sources
						selected
					</div>
				</div>
				...
				<DialogFooter className="gap-2 flex-col sm:flex-row">
					...
					>
						{consentMutation.isPending
							? "Granting Consents..."
							: `Grant Consent (${selectedSources.size} sources)`}
					</Button>
				</DialogFooter>
			</DialogContent>
		</Dialog>
