    ...
    return (
      <TooltipProvider>
        <Tooltip>
          ...
          <TooltipContent>
            <p>Complete your SEBI risk profiling to access all investment products</p>
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
    );
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          ...
          >
            ...
            {isExpired && <AlertTriangle className="h-3 w-3 ml-1 text-amber-500" />}
          </Badge>
        </TooltipTrigger>
        <TooltipContent>
          <div className="space-y-1">
            <p className="font-semibold">{profile.riskTier} - {RISK_TIER_LABELS[profile.riskTier]}</p>
            <p className="text-xs">Score: {profile.riskScore}/100</p>
            {isExpired ? (
              <p className="text-xs text-amber-500">Expired - Please reassess</p>
            ) : (
...
export function ProductSuitabilityIndicator({ 
  ...
  if (isLoading) {
    return <Badge variant="outline" className={cn("animate-pulse", className)}>Checking...</Badge>;
  }
      ...
      <TooltipProvider>
        <Tooltip>
          <TooltipTrigger asChild>
            <Badge 
              variant="outline" 
              className={cn("text-green-600 border-green-200 bg-green-50 dark:bg-green-900/20", className)}
    ...
    <TooltipProvider>
      <Tooltip>
        <TooltipTrigger asChild>
          <Badge 
            variant="outline" 
            className={cn("text-red-600 border-red-200 bg-red-50 dark:bg-red-900/20", className)}
