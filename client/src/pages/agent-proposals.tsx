...
export default function AgentProposalsPage() {
	...
	const filteredProposals =
		proposals?.filter((proposal) => {
			...
			if (activeTab === "draft")
				return matchesSearch && proposal.status === "draft";
			if (activeTab === "shared")
				return (
					matchesSearch && ["shared", "client_viewed"].includes(proposal.status)
				);
			if (activeTab === "action_required")
				return (
					matchesSearch && ["approved", "rejected"].includes(proposal.status)
				);
			if (activeTab === "executed")
				return matchesSearch && proposal.status === "executed";
	...
	return (
		<div
			...
			data-testid="agent-proposals-page"
		>
			<div className="container mx-auto p-6">
				<div className="flex items-center justify-between mb-6">
					...
					<Alert className="max-w-md">
						...
						<AlertDescription className="text-xs">
							Proposals are system-generated. You can add notes but cannot
							modify allocations.
						</AlertDescription>
					</Alert>
				</div>
					...
					<Card>
						...
						<CardContent>
							<div className="text-2xl font-bold text-yellow-600">
								{proposals?.filter((p) =>
									["shared", "client_viewed"].includes(p.status),
								).length || 0}
							</div>
						</CardContent>
					</Card>
							...
							<CardContent>
								<Tabs value={activeTab} onValueChange={setActiveTab}>
									<ScrollableTabsList>
										...
										<TabsTrigger
											value="draft"
											data-testid="tab-draft-proposals"
										>
											Draft
										</TabsTrigger>
										<TabsTrigger
											value="shared"
											data-testid="tab-shared-proposals"
										>
											Shared
										</TabsTrigger>
										<TabsTrigger
											value="action_required"
											data-testid="tab-action-proposals"
										>
											Action Required
										</TabsTrigger>
										<TabsTrigger
											value="executed"
											data-testid="tab-executed-proposals"
										>
											Executed
										</TabsTrigger>
									</ScrollableTabsList>
															...
															>
																...
																<TableCell>
																	<span className="capitalize text-sm">
																		{proposal.sessionPurpose?.replace(
																			/_/g,
																			" ",
																		) || "N/A"}
																	</span>
												...
												{WORKFLOW_STEPS.map((step, index) => {
													const isCompleted =
														WORKFLOW_STEPS.findIndex(
															(s) => s.key === selectedProposal.workflowState,
														) > index;
													const isCurrent =
														step.key === selectedProposal.workflowState;
										...
										<div className="grid grid-cols-2 gap-4">
											<div>
												...
												<p className="font-bold text-lg">
													{formatCurrency(selectedProposal.investmentAmount)}
												</p>
											</div>
											<div>
												<p className="text-xs text-muted-foreground">
			...
			<Dialog open={showAddNoteDialog} onOpenChange={setShowAddNoteDialog}>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							Add an explanatory note to help the client understand the
							recommendation. Notes are visible to the client but do not modify
							the allocation.
						</DialogDescription>
					</DialogHeader>
					...
					<div className="space-y-4">
						<div>
							...
							>
								...
								<option value="disclaimer_addition">
						...
						<Alert>
							...
							<AlertDescription>
								Notes are logged in the compliance audit trail with your agent
								credentials.
							</AlertDescription>
						</Alert>
					</div>
			...
			<Dialog open={showShareDialog} onOpenChange={setShowShareDialog}>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							Choose how to share this proposal. The client will receive a
							secure view-only link.
						</DialogDescription>
					</DialogHeader>
					...
					<div className="space-y-4">
						...
						<Alert>
							...
							<AlertDescription>
								All shares are logged and tracked. Client actions (view,
								approve, reject) are recorded for compliance.
							</AlertDescription>
						</Alert>
					</div>
			...
			<AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							Are you sure you want to delete this proposal for{" "}
							{proposalToDelete?.clientName}? This action cannot be undone.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
						<AlertDialogCancel
