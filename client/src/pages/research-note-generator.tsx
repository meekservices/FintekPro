...
function RatingBadge({ rating }: { rating: string }) {
	...
	return (
		<span
			className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md text-white font-bold text-sm ${color}`}
		>
...
function MetricCard({
	...
	return (
		<div
			className={`rounded-lg border p-3 ${highlight ? "border-blue-200 bg-blue-50 dark:bg-blue-950/20 dark:border-blue-800" : "bg-card"}`}
		>
			<p className="text-xs text-muted-foreground mb-1">{label}</p>
			<p
				className={`text-base font-bold ${highlight ? "text-blue-700 dark:text-blue-300" : "text-foreground"}`}
			>
				{value}
			</p>
			{subText && (
...
export default function ResearchNoteGenerator() {
	...
	>({
		...
		queryFn: async () => {
			...
			return apiRequest(
				`/api/research-note/search?q=${encodeURIComponent(debouncedSearch)}`,
				"GET",
			);
		},
	...
	const symbolToAnalyse = selectedCompany
		? selectedCompany.symbol
		: searchText.trim();
	const cinToAnalyse =
		selectedCompany?.cin || (isUnlistedSelected ? symbolToAnalyse : null);
	...
	const downloadMutation = useMutation({
		mutationFn: async ({ type }: { type: "ppt" | "pdf" | "onepager" }) => {
			...
			const filename = match
				? match[1]
				: `Research_${type}.${type === "ppt" ? "pptx" : "pdf"}`;
	...
	const handlePreview = () => {
		...
		previewMutation.mutate({
			symbol: symbolToAnalyse,
			cin: cinToAnalyse,
			isUnlisted: isUnlistedSelected,
		});
	};
		...
		<div className="max-w-5xl mx-auto p-4 md:p-6 space-y-6">
			<div className="flex items-start justify-between">
				<div>
					...
					<p className="text-muted-foreground mt-1">
						Generate institutional-grade equity research reports for listed and
						unlisted companies
					</p>
				</div>
			...
			<Card>
				<CardHeader className="pb-3">
					...
					<CardDescription>
						Search listed companies (NSE/BSE) or unlisted companies by name,
						symbol, ISIN, or CIN
					</CardDescription>
				</CardHeader>
				<CardContent>
					<div className="flex gap-2 items-start">
												...
												<div className="flex items-center justify-between gap-2">
													<div className="min-w-0">
														<div className="flex items-center gap-1.5">
															...
															{c.type === "unlisted" && (
																<span className="shrink-0 text-[9px] font-bold px-1.5 py-0.5 rounded bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-400 border border-amber-300 dark:border-amber-700 uppercase tracking-wide">
																	Unlisted
																</span>
															)}
															{c.type === "external" && (
																<span className="shrink-0 text-[9px] font-bold px-1.5 py-0.5 rounded bg-blue-50 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400 border border-blue-200 dark:border-blue-700 uppercase tracking-wide">
																	NSE Live
																</span>
															)}
														</div>
													...
													{c.type !== "unlisted" && c.isin && (
														<p className="text-[10px] font-mono text-muted-foreground bg-muted px-1.5 py-0.5 rounded shrink-0">
															{c.isin}
														</p>
													)}
												</div>
						...
						<Button
							...
							disabled={!symbolToAnalyse || previewMutation.isPending}
						>
					...
					{selectedCompany && (
						<div
							className={`mt-3 flex items-center gap-3 px-3 py-2 rounded-lg border ${isUnlistedSelected ? "bg-amber-50 dark:bg-amber-950/20 border-amber-200 dark:border-amber-800" : "bg-blue-50 dark:bg-blue-950/20 border-blue-200 dark:border-blue-800"}`}
						>
							<Building2
								className={`h-4 w-4 shrink-0 ${isUnlistedSelected ? "text-amber-600" : "text-blue-600"}`}
							/>
							<div className="flex-1 min-w-0">
								<p
									className={`text-sm font-medium truncate ${isUnlistedSelected ? "text-amber-800 dark:text-amber-200" : "text-blue-800 dark:text-blue-200"}`}
								>
				...
				<Card className="border-blue-200 bg-blue-50 dark:bg-blue-950/20">
					<CardContent className="py-8 flex items-center justify-center gap-3 text-blue-600">
						...
						<span className="font-medium">
							Fetching live data, running valuation models, generating AI
							commentary...
						</span>
					</CardContent>
				</Card>
				...
				<>
						...
						<CardContent className="pt-5">
							<div className="flex flex-wrap items-start gap-4 justify-between">
								<div>
									...
									<p className="text-sm text-muted-foreground">
										...
										{d.industry && d.industry !== d.sector && (
							...
							{d.dataQuality && (
								<div className="flex flex-wrap gap-2 mb-4">
									<span
										className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full border font-medium ${d.dataQuality.price.source === "NSE_LIVE" ? "bg-green-50 text-green-700 border-green-200 dark:bg-green-950/20 dark:text-green-400 dark:border-green-800" : "bg-amber-50 text-amber-700 border-amber-200"}`}
									>
									...
									>
										Shareholding ·{" "}
										{d.dataQuality.shareholding.source === "NSE_LIVE"
											? "Live"
											: "Unavailable"}
									</span>
							...
							<div className="space-y-3">
								...
								<ScoreBar
									label="Fundamentals"
									score={d.rating.breakdown.fundamentals}
									weight="40%"
								/>
								<ScoreBar
									label="Valuation"
									score={d.rating.breakdown.valuation}
									weight="30%"
								/>
								<ScoreBar
									label="Momentum"
									score={d.rating.breakdown.momentum}
									weight="30%"
								/>
							</div>
						</CardContent>
						...
						<Card className="border-blue-200">
							<CardHeader className="pb-2">
								<CardTitle className="text-sm flex items-center gap-2">
									<Target className="h-4 w-4 text-blue-600" /> Price Target &
									Valuation
								</CardTitle>
							</CardHeader>
							<CardContent>
								<div className="grid grid-cols-3 gap-3 mb-4">
									<div className="rounded-lg bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-800 p-3 text-center">
										...
										<p className="text-lg font-bold text-red-700 dark:text-red-300">
											{priceRs(d.priceTarget.bear)}
										</p>
									</div>
									<div className="rounded-lg bg-blue-50 dark:bg-blue-950/20 border-2 border-blue-400 p-3 text-center">
										<p className="text-xs text-blue-600 font-medium">
											Base Target (FintekPro Est.)
										</p>
										<p className="text-xl font-bold text-blue-700 dark:text-blue-300">
											{priceRs(d.priceTarget.blended)}
										</p>
										<p
											className={`text-xs font-medium mt-1 ${(d.priceTarget.upside ?? 0) >= 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}
										>
											{(d.priceTarget.upside ?? 0) >= 0 ? "▲" : "▼"}{" "}
											{Math.abs(d.priceTarget.upside ?? 0).toFixed(1)}% from CMP
										</p>
									</div>
								...
								<div className="grid grid-cols-2 gap-2 text-xs text-muted-foreground">
									...
									{d.priceTarget.peBased && (
										<span>
											PE-Based:{" "}
											<strong className="text-foreground">
												{priceRs(d.priceTarget.peBased)}
											</strong>
										</span>
									)}
									{d.priceTarget.pbBased && (
										<span>
											PB-Based:{" "}
											<strong className="text-foreground">
												{priceRs(d.priceTarget.pbBased)}
											</strong>
										</span>
									)}
								</div>
					...
					{d.isUnlisted && d.unlistedExtras && (
						<>
							...
							<Card className="border-amber-200 dark:border-amber-800">
								<CardHeader className="pb-2">
									<CardTitle className="text-sm flex items-center gap-2">
										<BarChart3 className="h-4 w-4 text-amber-600" /> Valuation
										Models
										<span className="ml-auto text-xs font-normal text-muted-foreground">
											{d.unlistedExtras.valuationRange.method}
										</span>
									</CardTitle>
								</CardHeader>
								<CardContent className="space-y-4">
									...
									<div className="grid grid-cols-3 gap-3">
										<div className="rounded-lg bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-800 p-3 text-center">
											...
											<p className="text-lg font-bold text-red-700 dark:text-red-300">
												{d.unlistedExtras.valuationRange.low
													? `₹${Math.round(d.unlistedExtras.valuationRange.low).toLocaleString("en-IN")}`
													: "N/A"}
											</p>
										</div>
										<div className="rounded-lg bg-amber-50 dark:bg-amber-950/20 border-2 border-amber-400 p-3 text-center">
											...
											<p className="text-xl font-bold text-amber-800 dark:text-amber-200">
												{d.unlistedExtras.valuationRange.mid
													? `₹${Math.round(d.unlistedExtras.valuationRange.mid).toLocaleString("en-IN")}`
													: "N/A"}
											</p>
											{d.unlistedExtras.valuationRange.upside !== null && (
												<p
													className={`text-xs font-medium mt-1 ${d.unlistedExtras.valuationRange.upside >= 0 ? "text-green-600" : "text-red-600"}`}
												>
													{d.unlistedExtras.valuationRange.upside >= 0
														? "▲"
														: "▼"}{" "}
													{(
														Math.abs(d.unlistedExtras.valuationRange.upside) *
														100
													).toFixed(0)}
													% vs transaction price
												</p>
											)}
										</div>
										<div className="rounded-lg bg-green-50 dark:bg-green-950/20 border border-green-200 dark:border-green-800 p-3 text-center">
											...
											<p className="text-lg font-bold text-green-700 dark:text-green-300">
												{d.unlistedExtras.valuationRange.high
													? `₹${Math.round(d.unlistedExtras.valuationRange.high).toLocaleString("en-IN")}`
													: "N/A"}
											</p>
										</div>
									</div>
											...
											{d.unlistedExtras.valuationRange.evEbitda ? (
												<>
													...
													<p className="text-base font-bold text-amber-700 dark:text-amber-300">
														{d.unlistedExtras.valuationRange.evEbitda
															.perShareValue
															? `₹${Math.round(d.unlistedExtras.valuationRange.evEbitda.perShareValue).toLocaleString("en-IN")}/sh`
															: "Insufficient data"}
													</p>
												</>
											) : (
												<p className="text-sm text-muted-foreground">
													Insufficient EBITDA data
												</p>
											)}
										</div>
										{/* DCF */}
											...
											{d.unlistedExtras.valuationRange.dcf ? (
												<>
													...
													<p className="text-sm">
														...
														<strong>
															{(
																d.unlistedExtras.valuationRange.dcf.wacc * 100
															).toFixed(0)}
															%
														</strong>
													</p>
													<p className="text-base font-bold text-amber-700 dark:text-amber-300">
														{d.unlistedExtras.valuationRange.dcf.perShareValue
															? `₹${Math.round(d.unlistedExtras.valuationRange.dcf.perShareValue).toLocaleString("en-IN")}/sh`
															: "Insufficient data"}
													</p>
												</>
											) : (
												<p className="text-sm text-muted-foreground">
													Insufficient FCF data
												</p>
											)}
										</div>
										{/* Revenue Multiple */}
											...
											{d.unlistedExtras.valuationRange.revenueMultiple ? (
												<>
													...
													<p className="text-base font-bold text-amber-700 dark:text-amber-300">
														{d.unlistedExtras.valuationRange.revenueMultiple
															.perShareValue
															? `₹${Math.round(d.unlistedExtras.valuationRange.revenueMultiple.perShareValue).toLocaleString("en-IN")}/sh`
															: "Insufficient data"}
													</p>
												</>
											) : (
									...
									{d.unlistedExtras.transactionPrice && (
										<div className="flex items-center gap-2 pt-2 border-t text-sm">
											...
											<span className="font-bold">
												₹
												{d.unlistedExtras.transactionPrice.toLocaleString(
													"en-IN",
												)}
												/share
											</span>
											{d.unlistedExtras.totalShares && (
												<span className="text-muted-foreground ml-2">
													· {(d.unlistedExtras.totalShares / 1e7).toFixed(2)} Cr
													shares
												</span>
											)}
										</div>
							...
							<div className="grid md:grid-cols-2 gap-4">
								<Card>
									<CardHeader className="pb-2">
										<CardTitle className="text-sm flex items-center gap-2">
											<Activity className="h-4 w-4 text-purple-500" /> Financial
											Health Score
										</CardTitle>
									</CardHeader>
									<CardContent>
										<div className="mb-4">
											...
											<p className="text-xs text-muted-foreground mt-1">
												Composite score: ROE (35%) · Revenue Growth (30%) ·
												Leverage (35%)
											</p>
										</div>
										<div className="grid grid-cols-2 gap-2">
											<MetricCard
												label="ROE"
												value={
													d.unlistedExtras.ratios.roe !== null
														? `${(d.unlistedExtras.ratios.roe * 100).toFixed(1)}%`
														: "N/A"
												}
											/>
											<MetricCard
												label="ROCE"
												value={
													d.unlistedExtras.ratios.roce !== null
														? `${(d.unlistedExtras.ratios.roce * 100).toFixed(1)}%`
														: "N/A"
												}
											/>
											<MetricCard
												label="D/E Ratio"
												value={
													d.unlistedExtras.ratios.debtToEquity !== null
														? `${d.unlistedExtras.ratios.debtToEquity.toFixed(2)}x`
														: "N/A"
												}
											/>
											<MetricCard
												label="EBITDA Margin"
												value={
													d.unlistedExtras.ratios.ebitdaMargin !== null
														? `${(d.unlistedExtras.ratios.ebitdaMargin * 100).toFixed(1)}%`
														: "N/A"
												}
											/>
											<MetricCard
												label="Net Margin"
												value={
													d.unlistedExtras.ratios.netMargin !== null
														? `${(d.unlistedExtras.ratios.netMargin * 100).toFixed(1)}%`
														: "N/A"
												}
											/>
											<MetricCard
												...
												value={
													d.unlistedExtras.ratios.revenueGrowth !== null
														? `${(d.unlistedExtras.ratios.revenueGrowth * 100).toFixed(1)}%`
														: "N/A"
												}
											/>
										</div>
								...
								{d.unlistedExtras.directors.length > 0 && (
									<Card>
										<CardHeader className="pb-2">
											<CardTitle className="text-sm flex items-center gap-2">
												<Users className="h-4 w-4 text-blue-500" /> Key
												Directors ({d.unlistedExtras.directors.length})
											</CardTitle>
										</CardHeader>
										<CardContent>
											<div className="space-y-2">
												{d.unlistedExtras.directors
													.slice(0, 6)
													.map((dir, i) => (
														<div
															key={i}
														...
														>
															...
															{dir.is_active && (
																<span className="text-[9px] font-medium px-1.5 py-0.5 rounded bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-400 shrink-0">
																	Active
																</span>
															)}
														</div>
							...
							{d.unlistedExtras.compliance && (
								<Card
									className={`border-${d.unlistedExtras.compliance.overall_risk === "high" || d.unlistedExtras.compliance.overall_risk === "critical" ? "red" : "slate"}-200 dark:border-${d.unlistedExtras.compliance.overall_risk === "high" || d.unlistedExtras.compliance.overall_risk === "critical" ? "red" : "slate"}-800`}
								>
									<CardHeader className="pb-2">
										<CardTitle className="text-sm flex items-center gap-2">
											...
											<span
												className={`ml-auto text-xs font-bold px-2 py-0.5 rounded capitalize ${
													d.unlistedExtras.compliance.overall_risk === "low"
														? "bg-green-100 text-green-700"
														: d.unlistedExtras.compliance.overall_risk ===
																"medium"
									...
									<CardContent>
										{d.unlistedExtras.compliance.signals.length === 0 ? (
											<p className="text-sm text-muted-foreground">
												No significant compliance signals detected.
											</p>
										) : (
											<div className="space-y-2">
												{d.unlistedExtras.compliance.signals
													.slice(0, 5)
													.map((sig, i) => (
														<div
															key={i}
												...
												{d.unlistedExtras.compliance.charges_count !==
													undefined &&
													d.unlistedExtras.compliance.charges_count > 0 && (
					...
					<div className="grid md:grid-cols-2 gap-4">
						<Card>
							<CardHeader className="pb-2">
								<CardTitle className="text-sm flex items-center gap-2">
									<DollarSign className="h-4 w-4 text-blue-500" /> Financial
									Snapshot
									{d.isUnlisted && (
										<span className="ml-auto text-xs font-normal text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 px-2 py-0.5 rounded-full">
											MCA-Derived
										</span>
									)}
								</CardTitle>
							...
							<CardContent>
								<div className="grid grid-cols-2 gap-2">
									...
									<MetricCard
										label={d.isUnlisted ? "Implied P/B" : "P/B Ratio"}
										value={f.pbRatio !== null ? fmt(f.pbRatio, "", "x") : "N/A"}
									/>
									<MetricCard
										label="Book Value/Share"
										value={fmt(f.bookValue, cp)}
									/>
									<MetricCard
										label="Debt / Equity"
									...
									{!d.isUnlisted && (
										<MetricCard
											...
											subText={
												f.vwap !== null &&
												f.previousClose !== null &&
												Math.abs(f.vwap - f.previousClose) < 0.01
													? "prev. close"
													: undefined
											}
										/>
									...
									<div className="mt-3 pt-3 border-t grid grid-cols-3 gap-2 text-center">
										{[
											...
											<div key={String(label)}>
												...
												<p
													className={`text-sm font-bold ${(val as number | null) === null ? "text-muted-foreground" : (val as number) >= 0 ? "text-green-600" : "text-red-600"}`}
												>
													{(val as number | null) !== null
														? signPct(val as number)
														: "N/A"}
												</p>
											</div>
										))}
						...
						{d.isUnlisted ? (
							<Card>
								<CardHeader className="pb-2">
									<CardTitle className="text-sm flex items-center gap-2">
										<Target className="h-4 w-4 text-purple-500" /> Intrinsic
										Value Summary
									</CardTitle>
								</CardHeader>
								<CardContent className="space-y-3">
									<div className="grid grid-cols-2 gap-2">
										<MetricCard
											...
											highlight={!!d.priceTarget?.blended}
										/>
										<MetricCard
											label="Upside"
											value={
												d.priceTarget?.upside !== null &&
												d.priceTarget?.upside !== undefined
													? `${d.priceTarget.upside > 0 ? "+" : ""}${d.priceTarget.upside.toFixed(1)}%`
													: "N/A"
											}
											highlight={!!d.priceTarget?.upside}
										/>
										<MetricCard
											label="Bear Case"
											value={
									...
									<div className="rounded-lg bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 p-3 text-xs text-amber-800 dark:text-amber-200">
										Beta and 52-week range are not available for unlisted
										securities. Price targets are intrinsic value estimates, not
							...
							<Card>
								<CardHeader className="pb-2">
									<CardTitle className="text-sm flex items-center gap-2">
										<Target className="h-4 w-4 text-purple-500" /> Technical
										Levels
									</CardTitle>
								</CardHeader>
								<CardContent className="space-y-4">
									<div className="grid grid-cols-2 gap-2">
										<MetricCard
											label="Support"
											value={fmt(d.levels.support, cp)}
										/>
										<MetricCard
											label="Resistance"
											value={fmt(d.levels.resistance, cp)}
										/>
										<MetricCard
											label="Stop Loss"
											value={fmt(d.levels.stopLoss, cp)}
										/>
										<MetricCard
											label="Target 1"
											value={fmt(d.levels.target1, cp)}
											highlight
										/>
										<MetricCard
											label="Target 2"
											value={fmt(d.levels.target2, cp)}
											highlight
										/>
										<MetricCard
											label="Price Target"
											value={
												d.priceTarget?.blended
													? `${priceRs(d.priceTarget.blended)} (Est.)`
													: fmt(f.targetMeanPrice, cp)
											}
											highlight={!!d.priceTarget?.blended}
										/>
									</div>
									<div className="rounded-lg bg-muted/50 p-3 text-xs space-y-1">
										<p>
											...
											{fmt(f.fiftyTwoWeekHigh, cp)} ·{" "}
											<span className="font-medium">Low:</span>{" "}
											{fmt(f.fiftyTwoWeekLow, cp)}
										</p>
										<p className="text-muted-foreground">
											{d.weekRange52Position}
										</p>
									</div>
								</CardContent>
							</Card>
					...
					{d.companyDescription && (
						<Card>
							<CardHeader className="pb-2">
								<CardTitle className="text-sm flex items-center gap-2">
									<Building2 className="h-4 w-4 text-slate-600" /> About the
									Company
								</CardTitle>
							</CardHeader>
							<CardContent>
					...
					{(d.salesCagr3Y !== null ||
						...
						<Card>
							<CardHeader className="pb-2">
								<CardTitle className="text-sm flex items-center gap-2">
									<Activity className="h-4 w-4 text-blue-600" /> Growth Rates
									(CAGR)
								</CardTitle>
							</CardHeader>
							<CardContent>
									...
									{[
										...
										.map(({ label, val }) => {
											...
											return (
												<div
													key={label}
													className={`rounded-lg border px-3 py-2 text-center min-w-[110px] ${positive ? "bg-green-50 dark:bg-green-950/20 border-green-200 dark:border-green-800" : "bg-red-50 dark:bg-red-950/20 border-red-200 dark:border-red-800"}`}
												>
													<p className="text-xs text-muted-foreground">
														{label}
													</p>
													<p
														className={`text-base font-bold ${positive ? "text-green-700 dark:text-green-400" : "text-red-700 dark:text-red-400"}`}
													>
														{positive ? "+" : ""}
														{pct.toFixed(1)}%
													</p>
					...
					{d.plHistory && d.plHistory.rows.length > 0 && (
						<Card>
							<CardHeader className="pb-2">
								<CardTitle className="text-sm flex items-center gap-2">
									<TableIcon className="h-4 w-4 text-blue-600" /> Multi-Year
									Profit & Loss (₹ Cr)
								</CardTitle>
							</CardHeader>
							<CardContent>
								{(() => {
									const annualIndices = d
										.plHistory!.headers.map((h, i) => ({ h, i }))
												...
												<tbody>
													{d.plHistory.rows.map((row, ri) => {
														...
														const computedCagr = (() => {
															...
															const endIdx =
																annualIndices[annualIndices.length - 1];
															const sv = row.values[startIdx];
															const ev = row.values[endIdx];
															if (sv === null || ev === null || sv <= 0)
																return null;
															const cagrExp = 1 / cagrYrs; // extracted to avoid esbuild regex false-positive
															const result = (ev / sv) ** cagrExp - 1;
															return Number.isFinite(result) ? result : null;
														})();
														return (
															<tr
																key={row.label}
																className={`border-b last:border-0 ${ri % 2 === 0 ? "" : "bg-muted/30"}`}
															>
																...
																{row.values.map((v, vi) => {
																	...
																	return (
																		<td
																			key={vi}
																			className={`text-right py-1.5 px-2 ${trend}`}
																		>
																			{display}
																		</td>
																	);
					...
					{d.quarterlyHistory && d.quarterlyHistory.rows.length > 0 && (
						<Card>
							<CardHeader className="pb-2">
								<CardTitle className="text-sm flex items-center gap-2">
									<BarChart3 className="h-4 w-4 text-purple-600" /> Quarterly
									Results (₹ Cr)
								</CardTitle>
							</CardHeader>
							<CardContent>
									...
									<table className="w-full text-xs">
										<thead>
											<tr className="border-b bg-purple-50 dark:bg-purple-950/20">
												...
												{d.quarterlyHistory.headers.map((h, i) => (
													<th
														key={h}
														className={`text-right py-2 px-2 font-semibold whitespace-nowrap ${i === d.quarterlyHistory!.headers.length - 1 ? "text-purple-700 dark:text-purple-400" : "text-muted-foreground"}`}
													>
														{h}
													</th>
												))}
										...
										<tbody>
											{d.quarterlyHistory.rows.map((row, ri) => {
												...
												const qoqDenom =
													lastTwo[0] !== null ? Math.abs(lastTwo[0]) : 1;
												const qoqNumer =
													lastTwo.length === 2 &&
													lastTwo[0] !== null &&
													lastTwo[1] !== null &&
												...
												const yoyNumer =
													...
													yoyStart !== 0
														...
														: null;
												const yoy =
													yoyNumer !== null ? yoyNumer / yoyDenom : null;
												return (
													<tr
														key={row.label}
														className={`border-b last:border-0 ${ri % 2 === 0 ? "" : "bg-muted/30"}`}
													>
														...
														{row.values.map((v, vi) => {
															...
															return (
																<td
																	key={vi}
																	className={`text-right py-1.5 px-2 ${isLatest ? "font-semibold text-purple-700 dark:text-purple-400" : ""}`}
																>
																	{display}
																</td>
															);
														...
														<td className="text-right py-1.5 px-2 font-semibold">
															{qoq !== null ? (
																<span
																	className={
																		qoq >= 0 ? "text-green-600" : "text-red-600"
																	}
																>
																	{qoq >= 0 ? "▲" : "▼"}{" "}
																	{Math.abs(qoq * 100).toFixed(1)}%
																</span>
															) : (
																"—"
															)}
					...
					{d.ratiosHistory && d.ratiosHistory.rows.length > 0 && (
						<Card>
							<CardHeader className="pb-2">
								<CardTitle className="text-sm flex items-center gap-2">
									<Activity className="h-4 w-4 text-teal-600" /> Key Efficiency
									Ratios (5-Year Trend)
								</CardTitle>
							</CardHeader>
							<CardContent>
					...
					{d.bsHistory && d.bsHistory.rows.length > 0 && (
						<Card>
							<CardHeader className="pb-2">
								<CardTitle className="text-sm flex items-center gap-2">
									<Layers className="h-4 w-4 text-amber-600" /> Balance Sheet
									Snapshot (₹ Cr)
								</CardTitle>
							</CardHeader>
							<CardContent>
					...
					{d.cfHistory && d.cfHistory.rows.length > 0 && (
						<Card>
							<CardHeader className="pb-2">
								<CardTitle className="text-sm flex items-center gap-2">
									<Activity className="h-4 w-4 text-cyan-600" /> Cash Flow
									Statement (₹ Cr)
								</CardTitle>
							</CardHeader>
							<CardContent>
									...
									<table className="w-full text-xs">
										...
										<tbody>
											{d.cfHistory.rows.map((row, ri) => {
												const isNetCash = row.label
													.toLowerCase()
													.includes("net cash");
												return (
													<tr
														key={row.label}
														className={`border-b last:border-0 ${ri % 2 === 0 ? "" : "bg-muted/30"} ${isNetCash ? "font-semibold" : ""}`}
													>
														<td className="py-1.5 px-2 font-medium text-foreground">
															{row.label}
														</td>
					...
					<Card>
						<CardHeader className="pb-2">
							<CardTitle className="text-sm flex items-center gap-2">
								<BarChart3 className="h-4 w-4 text-blue-600" /> Valuation
								Snapshot
							</CardTitle>
						</CardHeader>
						<CardContent>
							<div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
								...
								{d.financials.pe !== null &&
									(() => {
										...
										return (
											<div
												className={`border-l-4 ${border} rounded-r-lg p-3 space-y-1`}
											>
												...
												<span
													className={`inline-block text-[10px] font-semibold px-1.5 py-0.5 rounded ${text}`}
												>
													{label}
												</span>
											</div>
								...
								{/* ROCE / Revenue Growth */}
								{(d.financials.roce !== null ||
									d.financials.revenueGrowth !== null) &&
									(() => {
					...
					{d.keyPoints &&
						(d.keyPoints.pros.length > 0 || d.keyPoints.cons.length > 0) && (
							<Card>
								<CardHeader className="pb-2">
									<CardTitle className="text-sm flex items-center gap-2">
									...
									<p className="text-xs text-muted-foreground">
										Machine-generated insights from Screener.in
									</p>
								</CardHeader>
								<CardContent className="space-y-3">
					...
					{d.thesis && d.thesis.length > 0 && (
						<Card>
							<CardHeader className="pb-2">
								<CardTitle className="text-sm flex items-center gap-2">
									<TrendingUp className="h-4 w-4 text-green-600" /> Investment
									Thesis
								</CardTitle>
							</CardHeader>
							<CardContent className="space-y-2">
								{d.thesis.map((bullet, i) => (
									<div
										key={i}
										className="flex items-start gap-3 p-2.5 rounded-lg bg-green-50 dark:bg-green-950/20 border border-green-100 dark:border-green-900"
									>
										<div className="shrink-0 w-5 h-5 rounded-full bg-green-600 text-white text-xs flex items-center justify-center font-bold mt-0.5">
											{i + 1}
										</div>
					...
					{d.risks && d.risks.length > 0 && (
						<Card>
							<CardHeader className="pb-2">
								<CardTitle className="text-sm flex items-center gap-2">
									<AlertTriangle className="h-4 w-4 text-red-600" /> Key Risk
									Factors
								</CardTitle>
							</CardHeader>
							<CardContent className="space-y-2">
								{d.risks.map((risk, i) => (
									<div
										key={i}
										className="flex items-start gap-3 p-2.5 rounded-lg bg-red-50 dark:bg-red-950/20 border border-red-100 dark:border-red-900"
									>
										<div className="shrink-0 w-5 h-5 rounded-full bg-red-600 text-white text-xs flex items-center justify-center font-bold mt-0.5">
											{i + 1}
										</div>
										...
										<tbody>
											...
											<tr className="border-b bg-blue-50 dark:bg-blue-950/20">
												<td className="py-2 font-semibold text-blue-700 dark:text-blue-300">
													{d.companyName.length > 22
														? d.companyName.slice(0, 22) + "…"
														: d.companyName}
												...
												<td className="text-right py-2">
													{f.pbRatio !== null ? f.pbRatio.toFixed(1) : "N/A"}
												</td>
											...
											{d.peers.map((peer) => (
												...
												>
													...
													<td className="text-right py-2">
														{peer.price
															? `₹${Math.round(peer.price).toLocaleString("en-IN")}`
															: "N/A"}
													</td>
													<td className="text-right py-2">
													...
													<td className="text-right py-2">
														{peer.pb !== null ? peer.pb.toFixed(1) : "N/A"}
													</td>
													<td className="text-right py-2">
														{fmtPct(peer.roe)}
													</td>
											...
											{d.sectorAvg && (
												<tr className="border-t bg-muted/30">
													<td className="py-1.5 text-xs text-muted-foreground italic">
														Sector Average ({d.sectorAvg.stockCount} stocks)
													</td>
													<td className="text-right py-1.5 text-xs text-muted-foreground">
														—
													</td>
													<td className="text-right py-1.5 text-xs text-muted-foreground">
														{d.sectorAvg.avgPE?.toFixed(1) ?? "—"}
													</td>
													<td className="text-right py-1.5 text-xs text-muted-foreground">
														{d.sectorAvg.avgPB?.toFixed(1) ?? "—"}
													</td>
													<td className="text-right py-1.5 text-xs text-muted-foreground">
														{fmtPct(d.sectorAvg.avgROE)}
													</td>
													<td className="text-right py-1.5 text-xs text-muted-foreground">
														—
													</td>
					...
					{d.shareholding && (
						<Card>
							<CardHeader className="pb-2">
								<CardTitle className="text-sm flex items-center gap-2">
									<Percent className="h-4 w-4 text-violet-600" /> Shareholding
									Pattern
								</CardTitle>
								{d.shareholding.quarter && (
									<p className="text-xs text-muted-foreground">
										As of {d.shareholding.quarter}
									</p>
								)}
							</CardHeader>
							<CardContent>
								<div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
									{[
										{
											label: "Promoter",
											val: d.shareholding.promoterPct,
											change: d.shareholding.promoterChange,
											color: "blue",
										},
										{
											label: "FII / FPI",
											val: d.shareholding.fiiPct,
											change: null,
											color: "purple",
										},
										{
											label: "DII / MF",
											val: d.shareholding.diiPct,
											change: null,
											color: "green",
										},
										{
											label: "Public",
											val: d.shareholding.publicPct,
											change: null,
											color: "amber",
										},
										...
										<div
											key={label}
											className={`rounded-lg border p-3 bg-${color}-50 dark:bg-${color}-950/20 border-${color}-200 dark:border-${color}-800`}
										>
											<p
												className={`text-xs font-medium text-${color}-700 dark:text-${color}-300`}
											>
												{label}
											</p>
											<p
												className={`text-xl font-bold text-${color}-800 dark:text-${color}-200`}
											>
												{val !== null ? `${val.toFixed(1)}%` : "N/A"}
											</p>
											{change !== null && (
												<p
													className={`text-xs mt-0.5 ${change >= 0 ? "text-green-600" : "text-red-600"}`}
												>
													{change >= 0 ? "▲" : "▼"}{" "}
													{Math.abs(change).toFixed(2)}% QoQ
												</p>
											)}
										</div>
									))}
								</div>
								{d.shareholding.pledgedPct !== null &&
									d.shareholding.pledgedPct > 0 && (
										<div
											className={`flex items-center gap-2 text-xs p-2 rounded ${d.shareholding.pledgedPct > 10 ? "bg-red-50 dark:bg-red-950/20 text-red-700" : "bg-amber-50 dark:bg-amber-950/20 text-amber-700"}`}
										>
											<AlertTriangle className="h-3.5 w-3.5 shrink-0" />
											Pledged shares: {d.shareholding.pledgedPct.toFixed(1)}%
											{d.shareholding.pledgedPct > 10 &&
												" — elevated pledge is an overhang risk"}
										</div>
									)}
								{d.shareholding.promoterChange !== null &&
									d.shareholding.promoterChange < -1.5 && (
										<div className="flex items-center gap-2 text-xs p-2 rounded bg-red-50 dark:bg-red-950/20 text-red-700 mt-2">
											...
											{Math.abs(d.shareholding.promoterChange).toFixed(1)}% QoQ
											— watch for further movement
										</div>
									)}
								{d.managementNote && (
							...
							<Card>
								<CardHeader className="pb-2">
									...
									<p className="text-[10px] text-muted-foreground">
										Machine-generated highlights from Screener.in · Exercise
										caution and do your own analysis
									</p>
								</CardHeader>
								<CardContent>
					...
					{d.commentary && (
						<Card>
							<CardHeader className="pb-2">
								<CardTitle className="text-sm flex items-center gap-2">
									<Sparkles className="h-4 w-4 text-amber-500" /> Industry
									Trends & Sector Outlook
								</CardTitle>
								<p className="text-[10px] text-muted-foreground">
									AI-generated overview based on sector dynamics and company
									profile
								</p>
							</CardHeader>
							<CardContent className="space-y-3">
								{[
									{
										label: "Industry Trends & Tailwinds",
										text: d.commentary.industryTrends,
									},
									{
					...
					<Card>
						<CardHeader className="pb-3">
							<CardTitle className="text-base flex items-center gap-2">
								<Download className="h-5 w-5 text-blue-600" /> Download Reports
							</CardTitle>
							<CardDescription>
								All reports are branded with FintekPro Research
							</CardDescription>
						</CardHeader>
						<CardContent>
								...
								<div className="border rounded-lg p-4 space-y-3">
									<div className="flex items-center gap-2">
										...
										<div>
											...
											<p className="text-xs text-muted-foreground">
												10-slide institutional presentation
											</p>
										</div>
									</div>
									<Button
								...
								<div className="border rounded-lg p-4 space-y-3">
									<div className="flex items-center gap-2">
										...
										<div>
											...
											<p className="text-xs text-muted-foreground">
												Quick reference with target price
											</p>
										</div>
									</div>
									<Button
			...
			{!d && !previewMutation.isPending && (
				<Card className="border-dashed">
					<CardContent className="py-16 flex flex-col items-center text-center text-muted-foreground gap-3">
						...
						<div>
							<p className="font-medium text-foreground">
								Enter a company to get started
							</p>
							<p className="text-sm mt-1">
								Search by NSE symbol (e.g. RELIANCE), ISIN, or company name.
							</p>
							<p className="text-sm">
								Live data + valuation models + AI sector commentary.
							</p>
						</div>
					</CardContent>
				</Card>
