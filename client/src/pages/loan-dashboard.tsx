...
export default function LoanDashboard() {
	const [selectedApplication, setSelectedApplication] =
		useState<LoanApplicationStatus | null>(null);
	...
	const {
		...
		isLoading,
		refetch,
	} = useQuery<{ applications: LoanApplicationStatus[] }>({
	...
	const loanSummary: LoanSummary = {
		...
		monthlyEMI: applications
			.filter((a) => a.status !== "rejected")
			.reduce((sum, a) => sum + a.emi, 0),
	};
	...
	const checkStatusMutation = useMutation({
		mutationFn: async ({
			applicationId,
			lenderId,
		}: { applicationId: string; lenderId: string }) => {
			const response = await fetch(
				`/api/loans/${applicationId}/status?lenderId=${lenderId}`,
			);
						...
						<CardContent className="py-6">
							<div className="flex items-center">
								...
								<div>
									...
									<p className="text-2xl font-bold text-foreground">
										₹{(loanSummary.totalLoanAmount / 100000).toFixed(1)}L
									</p>
								</div>
							</div>
						</CardContent>
						...
						<CardContent className="py-6">
							<div className="flex items-center">
								...
								<div>
									...
									<p className="text-2xl font-bold text-foreground">
										₹{loanSummary.monthlyEMI.toLocaleString()}
									</p>
								</div>
							</div>
						</CardContent>
												...
												<div className="flex items-center justify-between mb-4">
													<div className="flex items-center space-x-3">
														<span className="text-2xl">
															{getLenderLogo(application.lenderId)}
														</span>
														<div>
												...
												{application.documentsRequired && (
													<div className="mb-4">
														...
														<div className="grid grid-cols-1 md:grid-cols-2 gap-2">
															{application.documentsRequired.map(
																(doc, index) => (
																	<div
																		key={index}
										...
										<CardHeader>
											<CardTitle className="flex items-center space-x-2">
												<span className="text-xl">
													{getLenderLogo(selectedApplication.lenderId)}
												</span>
												<span>
													{selectedApplication.lenderName} - Status Details
												</span>
											</CardTitle>
										</CardHeader>
								...
								<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
									{[
										...
										},
										{
											name: "Address Proof",
											status: "verified",
											description: "Utility Bills, Passport",
										},
										{
											name: "Income Proof",
											status: "pending",
										...
										},
										{
											...
											status: "verified",
											description: "Offer Letter, ID Card",
										},
										{
											name: "Photograph",
											status: "verified",
