...
function Field({
	...
	value,
	icon: Icon,
}: { label: string; value: string | null | undefined; icon?: any }) {
	return (
...
export default function PartnerMyProfile() {
	...
	const verifyMutation = useMutation({
		mutationFn: (membershipNumber: string) =>
			apiRequest("/api/partner/verify-ca-membership", {
				method: "POST",
			...
			if (data?.status === "verified") {
				toast({
					...
					description: data.memberName
						? `Verified: ${data.memberName} (${data.memberType || "CA"})`
						: "Your ICAI membership has been verified.",
				});
			} else if (data?.status === "pending_review") {
				toast({
					title: "Submitted for Review",
					description:
						"Your ICAI number format is valid and has been queued for admin verification.",
				});
			}
		},
		onError: (err: any) =>
			toast({
				...
				description:
					err?.message ||
					"Could not verify ICAI number. Please check and try again.",
				variant: "destructive",
			}),
	});
		...
		return (
			<div className="space-y-4">
				{[1, 2, 3].map((i) => (
					<div
						key={i}
						className="h-32 rounded-xl bg-indigo-800/30 animate-pulse"
					/>
				))}
			</div>
		);
	...
	const statusColor = (s: string) => {
		...
		if (["ACTIVE", "APPROVED", "VERIFIED"].includes(u))
			return "bg-green-100 text-green-800 dark:bg-green-900/40 dark:text-green-300";
		if (["PENDING", "UNDER_REVIEW"].includes(u))
		...
		<div className="space-y-6 max-w-4xl">
					...
					<div className="flex items-start gap-5">
						...
						<div className="flex-1 min-w-0">
							...
							<div className="flex flex-wrap gap-2 mt-3">
								...
								<Badge
									className={statusColor(profile.hierarchyStatus) + " border-0"}
								>
								...
								{profile.empanelmentStatus && (
									<Badge
										className={
											statusColor(profile.empanelmentStatus) + " border-0"
										}
									>
			...
			<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
						...
						<div className="flex items-center justify-between py-2 border-b">
							<div className="flex items-center gap-2">
								...
								<div>
									...
									<p className="text-xs text-muted-foreground">
										{profile.aadhaarVerified ? "Verified" : "Not submitted"}
									</p>
								</div>
							</div>
								...
								<div>
									...
									<p className="text-xs text-muted-foreground">
										{profile.bankAccountNumber
											? maskAccount(profile.bankAccountNumber) +
				...
				<Card>
					<CardHeader className="pb-3">
						<CardTitle className="text-sm flex items-center gap-2">
							<Award className="h-4 w-4 text-violet-600" /> Professional
							Credentials
						</CardTitle>
					</CardHeader>
					<CardContent className="space-y-4">
						...
						<Field
							label="EUIN Number"
							value={profile.euinNumber}
							icon={KeyRound}
						/>
						<Field
							label="NISM Certificate"
							value={profile.nismCertificateNumber}
							icon={Award}
						/>
						{profile.nismCertificateType && (
							<Field
								label="NISM Type"
								value={profile.nismCertificateType}
								icon={Briefcase}
							/>
						)}
						{profile.nismExpiryDate && (
							<Field
								label="NISM Expiry"
								value={profile.nismExpiryDate}
								icon={Calendar}
							/>
						)}
						{profile.riaNumber && (
					...
					<CardContent className="space-y-4">
						{profile.bankAccountNumber ? (
							<>
								<div className="flex items-center justify-between">
									<Field
										...
										value={maskAccount(profile.bankAccountNumber)}
										icon={Landmark}
									/>
									<StatusBadge
										value={profile.bankVerified}
										trueLabel="Penny Drop Verified"
								...
								<Separator />
								<p className="text-xs text-muted-foreground">
									To update bank details, please go through the Agent
									Empanelment KYC wizard or contact support.
								</p>
							</>
						) : (
							<div className="text-center py-6 space-y-3">
								...
								<p className="text-xs text-muted-foreground">
									Complete the Agent Empanelment KYC flow to add your bank
									account.
								</p>
			...
			<Card
				className={
					profile.isCaQualified
						? "border-emerald-400 dark:border-emerald-600 ring-1 ring-emerald-200 dark:ring-emerald-800"
						: ""
				}
			>
				<CardHeader className="pb-3">
					<div className="flex items-center justify-between">
						<CardTitle className="text-sm flex items-center gap-2">
							<GraduationCap
								className={`h-4 w-4 ${profile.isCaQualified ? "text-emerald-600" : "text-muted-foreground"}`}
							/>
							CA Qualification
							{profile.isCaQualified &&
							...
							{profile.isCaQualified &&
								profile.caVerificationStatus !== "verified" && (
									<Badge
										variant="outline"
										className="text-amber-600 border-amber-300 gap-1 ml-1 text-[11px]"
									>
					...
					{caEditing ? (
							...
							<div className="flex items-center gap-3 p-3 rounded-lg bg-muted/50">
								...
								<div>
									...
									<p className="text-xs text-muted-foreground mt-0.5">
										Enables ITR filing cases, CA assignments, and Form 15CA/15CB
										workflows
									</p>
								</div>
							</div>
							{caForm.isCaQualified && (
								<div className="space-y-1.5">
									...
									<div className="flex gap-2">
										<Input
											...
											onChange={(e) =>
												setCaForm((f) => ({
													...f,
													caMembershipNumber: e.target.value,
												}))
											}
											className="h-8 text-sm"
											maxLength={6}
										...
										<Button
											...
											disabled={
												!caForm.caMembershipNumber ||
												caForm.caMembershipNumber.length < 6 ||
												verifyMutation.isPending
											}
											onClick={() =>
												verifyMutation.mutate(caForm.caMembershipNumber)
											}
										>
											{verifyMutation.isPending ? (
									...
									<p className="text-[10px] text-muted-foreground">
										6-digit ICAI membership number (ACA/FCA). Verification
										checks the ICAI registry.
									</p>
								</div>
							)}
						...
						<div className="space-y-4">
							<div className="flex items-center gap-3 p-3 rounded-lg bg-muted/40">
								...
								>
									<GraduationCap
										className={`h-5 w-5 ${profile.isCaQualified ? "text-emerald-600" : "text-muted-foreground"}`}
									/>
								</div>
								<div className="flex-1 min-w-0">
									...
									<div className="space-y-1">
										...
										<div className="flex items-center gap-2 flex-wrap">
											...
											{profile.caVerificationStatus === "pending_review" && (
												<Badge
													variant="outline"
													className="text-amber-600 border-amber-300 gap-1 text-[11px]"
												>
													<Clock className="h-3 w-3" /> Pending Review
												</Badge>
											)}
											{(profile.caVerificationStatus === "unverified" ||
												!profile.caVerificationStatus) &&
												profile.caMembershipNumber && (
													<Badge
														variant="outline"
														className="text-slate-500 border-slate-300 gap-1 text-[11px]"
													>
											...
											{profile.caVerificationStatus === "failed" && (
												<Badge
													variant="outline"
													className="text-red-600 border-red-300 gap-1 text-[11px]"
												>
											...
											{profile.caMembershipNumber &&
												profile.caVerificationStatus !== "verified" && (
													<Button
														...
														onClick={() =>
															verifyMutation.mutate(profile.caMembershipNumber!)
														}
													>
														{verifyMutation.isPending ? (
															<>
										...
										{profile.caVerifiedAt &&
											profile.caVerificationStatus === "verified" && (
												<p className="text-[10px] text-muted-foreground mt-0.5">
													Verified on{" "}
													{new Date(profile.caVerifiedAt).toLocaleDateString(
														"en-IN",
			...
			<Card>
				<CardHeader className="pb-3">
					<CardTitle className="text-sm flex items-center gap-2">
						<Briefcase className="h-4 w-4 text-muted-foreground" /> Account
						Information
					</CardTitle>
				</CardHeader>
				<CardContent>
					<div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
						<div className="space-y-1 sm:col-span-2">
							...
							<div className="flex items-center gap-1.5">
								<code
									className="text-[11px] font-mono bg-muted px-2 py-1 rounded break-all flex-1 select-all"
									title={`Reference UUID: ${profile.id}`}
								>
						...
						<div className="space-y-1">
							...
							<Badge className={statusColor(profile.approvalStatus)}>
