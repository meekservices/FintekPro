		...
		return (
			<div className="py-16 text-center text-muted-foreground">
				...
				<p className="text-sm mt-1">
					Your agents' cases will appear here once commissions are recorded
				</p>
			</div>
		);
	}
					...
					{rows.map((r) => (
						<TableRow key={r.agentId}>
							...
							<TableCell className="text-right">
								{compact(r.agentCommission)}
							</TableCell>
							<TableCell className="text-right text-muted-foreground">
								{(r.uplineRate * 100).toFixed(1)}%
							</TableCell>
							<TableCell className="text-right font-semibold text-green-600">
								{compact(r.uplineEarned)}
							</TableCell>
							<TableCell>
								<Badge
									className={
...
function PartnerPlanner({
	...
	month: number;
	year: number;
	agentNetwork: AgentNetworkRow[];
}) {
	const [targetAgents, setTargetAgents] = useState("10");
	const [targetIncome, setTargetIncome] = useState("100000");

	const ta = Number.parseInt(targetAgents) || 0;
	const ti = Number.parseFloat(targetIncome) || 0;

	const agentsPct =
		ta > 0 ? Math.min(100, (summary.totalAgents / ta) * 100) : 0;
	const incomePct =
		ti > 0 ? Math.min(100, (summary.uplineIncome / ti) * 100) : 0;

	const topAgents = [...agentNetwork]
		...
		<div className="space-y-6">
			<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
				...
				<Card>
					<CardHeader>
						<CardTitle className="flex items-center gap-2 text-base">
							<BarChart3 className="w-4 h-4 text-green-600" /> Progress vs
							Target
						</CardTitle>
					</CardHeader>
					<CardContent className="space-y-5">
						...
						<div className="space-y-2">
							<div className="flex justify-between text-sm">
								...
								<span className="font-medium">
									{compact(summary.uplineIncome)} / {ti ? compact(ti) : "—"}
								</span>
							</div>
			...
			{topAgents.length > 0 && (
				<Card>
					<CardHeader>
						<CardTitle className="flex items-center gap-2 text-base">
							<ArrowUpRight className="w-4 h-4 text-amber-600" /> Top Performing
							Agents
						</CardTitle>
					</CardHeader>
					<CardContent>
									...
									<div key={a.agentId} className="space-y-1">
										<div className="flex justify-between text-sm">
											...
											<span className="text-muted-foreground">
												{a.casesCount} cases · {compact(a.agentCommission)}
											</span>
										</div>
										<Progress
											value={(a.agentCommission / maxCommission) * 100}
											className="h-2"
										/>
									</div>
								);
					...
					<div className="p-4 bg-muted rounded-lg">
						<div className="flex justify-between items-center mb-3">
							...
							<p className="text-xl font-bold text-green-600">
								{compact(summary.uplineIncome)}
							</p>
						</div>
						<div className="flex justify-between items-center">
							<p className="text-sm text-muted-foreground">
								Network Commission Generated
							</p>
							<p className="font-medium">
								{compact(summary.networkCommission)}
							</p>
						</div>
					</div>
				</CardContent>
		...
		<div className="container mx-auto p-6 max-w-7xl space-y-6">
				...
				<div className="flex items-center gap-3">
					...
					<div>
						...
						<p className="text-muted-foreground text-sm">
							Agent network commissions and your upline income
						</p>
					</div>
				</div>
				<div className="flex items-center gap-3">
			...
			<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
				...
				<SCard
					...
					sub="total across team"
					icon={Receipt}
					color="text-indigo-600"
				/>
				<SCard
					label="Network Commission"
					value={compact(summary.networkCommission)}
					sub="agent earnings"
					icon={TrendingUp}
					color="text-green-600"
				/>
				<SCard
					label="Your Upline Income"
					value={compact(summary.uplineIncome)}
					sub="from agent network"
					icon={Banknote}
