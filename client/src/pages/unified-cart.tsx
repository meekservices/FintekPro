...
const PRODUCT_TYPES = {
	...
	etf: {
		label: "ETF",
		color: "bg-lime-100 dark:bg-lime-900/30 text-lime-700 dark:text-lime-300",
		icon: TrendingUp,
...
export default function UnifiedCart() {
	...
	const checkoutMutation = useMutation({
		...
		onSuccess: () => {
			toast({
				title: "Orders Placed!",
				description:
					"Your investment orders have been submitted for processing",
			});
			queryClient.invalidateQueries({ queryKey: ["/api/cart"] });
	...
	const approvedProposals =
		proposalData?.filter((p) => p.status === "approved") || [];
	...
	const categoryCounts = {
		...
		aif: cartItems.filter(
			(i: any) => i.productCategory === "aif" || i.category === "aif",
		).length,
		pms: cartItems.filter(
			(i: any) => i.productCategory === "pms" || i.category === "pms",
		).length,
		mld: cartItems.filter(
			(i: any) => i.productCategory === "mld" || i.category === "mld",
		).length,
		mutual_fund: cartItems.filter(
			(i: any) =>
				i.productCategory === "mutual_fund" || i.category === "mutual_fund",
		).length,
	};
			...
			<main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
				<div className="mb-8">
					...
					<p className="text-muted-foreground text-lg">
						Review and checkout your approved investment proposals across all
						product categories.
					</p>
				</div>
								...
								<TabsContent key={tab} value={tab} className="space-y-4">
									{getItemsByCategory(tab).length === 0 ? (
										...
										getItemsByCategory(tab).map((item: any, index: number) => {
											const productType =
												item.productCategory || item.category || "mutual_fund";
											const config =
												PRODUCT_TYPES[
													productType as keyof typeof PRODUCT_TYPES
												] || PRODUCT_TYPES.mutual_fund;
											...
											return (
												<Card
													...
													data-testid={`cart-item-${productType}-${index}`}
												>
													<CardContent className="p-0">
														<div className="flex">
															<div
																className={`w-2 bg-gradient-to-b ${config.gradient}`}
															/>
									...
									{categoryCounts.aif > 0 && (
										<div className="flex justify-between text-sm">
											...
											<span className="font-medium">
												₹
												{cartItems
													.filter((i: any) => i.productCategory === "aif")
													.reduce(
														(s: number, i: any) =>
															s + Number.parseFloat(i.amount || "0"),
														0,
													)
													.toLocaleString("en-IN")}
											</span>
									...
									{categoryCounts.pms > 0 && (
										<div className="flex justify-between text-sm">
											...
											<span className="font-medium">
												₹
												{cartItems
													.filter((i: any) => i.productCategory === "pms")
													.reduce(
														(s: number, i: any) =>
															s + Number.parseFloat(i.amount || "0"),
														0,
									...
									{categoryCounts.mld > 0 && (
										<div className="flex justify-between text-sm">
											...
											<span className="font-medium">
												₹
												{cartItems
													.filter((i: any) => i.productCategory === "mld")
													.reduce(
														(s: number, i: any) =>
															s + Number.parseFloat(i.amount || "0"),
														0,
									...
									{categoryCounts.mutual_fund > 0 && (
										<div className="flex justify-between text-sm">
											<span className="text-muted-foreground flex items-center gap-2">
												...
												Mutual Funds ({categoryCounts.mutual_fund})
											</span>
											<span className="font-medium">
												₹
												{cartItems
													.filter(
														(i: any) => i.productCategory === "mutual_fund",
													)
													.reduce(
														(s: number, i: any) =>
															s + Number.parseFloat(i.amount || "0"),
									...
									<div className="flex items-start gap-2">
										...
										<div>
											...
											<p className="text-amber-700 dark:text-amber-400 text-xs">
												All transactions are logged for audit purposes per SEBI
												guidelines.
											</p>
										</div>
									</div>
									...
									<div className="flex items-start gap-2">
										...
										<div>
											...
											<p className="text-blue-700 dark:text-blue-400 text-xs">
												Orders processed within 1-3 business days after payment
												confirmation.
											</p>
										</div>
									</div>
