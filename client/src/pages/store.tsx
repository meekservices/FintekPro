...
const categoryLinks = [
	...
	},
	{
		name: "Loans & Credit",
		icon: CreditCard,
		path: "/store?category=Loans+%26+Credit",
...
export default function StorePage() {
	...
	const [selectedLockedProduct, setSelectedLockedProduct] =
	...
	const { data: storeProductsData, isLoading: isLoadingStoreProducts } =
		useQuery<{
			success: boolean;
			products: any[];
			count: number;
	...
	const { data: legacyProductsData, isLoading: isLoadingLegacyProducts } =
		useQuery<any[]>({
	...
	const normalizeStoreProduct = (storeProduct: any): Product => {
		return {
			...
			shortDescription:
				storeProduct.shortDescription || storeProduct.description || "",
			...
			productType:
				storeProduct.productType || storeProduct.planType || "product",
			...
			expectedReturns:
				storeProduct.expectedReturns || storeProduct.returns1y || 0,
	...
	const normalizeLegacyProduct = (apiProduct: any): Product => {
		...
		return {
			...
			minimumInvestment:
				apiProduct.minimumInvestment || apiProduct.minInvestment || 0,
	...
	const getRequiredTierForProduct = (productCode: string): string => {
		...
		if (tierProducts.accredited_investor?.includes(productCode))
	...
	const getKycRequirementsForTier = (tier: string) => {
		...
		> = {
			basic: {
				...
				reason:
					"Basic KYC with PAN verification is required for accessing mutual funds, limited equity trading, and IPO retail subscriptions as per SEBI regulations.",
			},
			enhanced: {
				regulator: "SEBI/RBI",
				requirements: [
				...
				reason:
					"Enhanced KYC is mandated by SEBI for trading in derivatives (F&O), unlisted securities, margin trading, and higher value transactions.",
			},
			accredited_investor: {
				regulator: "SEBI",
				requirements: [
				...
				reason:
					"SEBI Accredited Investor status is required under SEBI (AIF) Regulations 2012 for investing in AIFs, PMS, Pre-IPO, private equity, and other sophisticated investment products.",
			},
		};
	...
	const getProductTypeRegulation = (productCode: string): string => {
		const regulations: Record<string, string> = {
			...
			aif_cat1:
				"SEBI (Alternative Investment Funds) Regulations, 2012 - Category I",
			aif_cat2:
				"SEBI (Alternative Investment Funds) Regulations, 2012 - Category II",
			aif_cat3:
				"SEBI (Alternative Investment Funds) Regulations, 2012 - Category III",
	...
	const hotDealsProducts = products.filter(
		(p) => p.badge === "HOT" || p.isNew || p.badge === "PREMIUM",
	);
	...
	const getCurrentCategoryProviders = () => {
		...
		const providers = [
			...new Set(categoryProducts.map((p) => p.provider).filter(Boolean)),
		];
	...
	const getFilteredProducts = (products: Product[]) => {
		...
		if (selectedProviderFilter !== "all") {
			if (selectedProviderFilter === "DB") {
				...
				filtered = filtered.filter(
					(p) => p.providerSource === selectedProviderFilter,
				);
			}
		}
		...
		if (selectedRisk !== "All") {
			filtered = filtered.filter(
				(p) => (p.riskLevel || "").toLowerCase() === selectedRisk.toLowerCase(),
			);
		}
		...
		filtered.sort((a, b) => {
			...
			switch (sortField) {
				...
				case "risk": {
					...
					comparison =
						riskOrder[(a.riskLevel || "medium") as keyof typeof riskOrder] -
						riskOrder[(b.riskLevel || "medium") as keyof typeof riskOrder];
					break;
				}
			}
	...
	const getBadgeColor = (badge: string) => {
		switch (badge) {
			...
			case "PREMIUM":
				return "bg-gradient-to-r from-yellow-400 to-orange-500 text-foreground";
			default:
					...
					<div className="grid grid-cols-2 gap-4 text-sm">
						...
						<div>
							...
							<span className="font-semibold">
								₹{(product.minimumInvestment || 0).toLocaleString()}
							</span>
						</div>
					</div>
					<div className="flex items-center justify-between">
						<Badge className={getRiskColor(product.riskLevel || "medium")}>
							{(product.riskLevel || "medium").charAt(0).toUpperCase() +
								(product.riskLevel || "medium").slice(1)}{" "}
							Risk
						</Badge>
					</div>
				...
				<div className="flex items-center gap-4 mb-6">
					...
					<div>
						...
						<p className="text-lg text-muted-foreground">
							Curated financial products • Expert recommendations • Trusted
							providers
						</p>
					</div>
						...
						<div className="space-y-6">
							<div className="flex items-center justify-between">
								<div className="flex items-center gap-3">
									...
									<div>
										<h2 className="text-2xl font-bold text-foreground">
											Hot Deals & New Launches
										</h2>
										<p className="text-sm text-muted-foreground">
											Limited time offers and latest products
										</p>
									</div>
								</div>
								<Badge
					...
					>
						...
						<TabsContent value={selectedCategory} className="mt-6">
							...
							isCategoryDisabled(selectedCategory) ? (
								<ComingSoonCategory
									categoryId={
										getCategoryInfo(selectedCategory)?.id || selectedCategory
									}
									categoryName={selectedCategory}
									message={getCategoryInfo(selectedCategory)?.comingSoonMessage}
									expectedDate={
										getCategoryInfo(selectedCategory)?.comingSoonExpectedDate
									}
								/>
							) : (
					...
					<div className="space-y-4 mt-4">
							...
							<div className="flex items-start gap-3">
								...
								<div>
									<p className="text-sm font-medium text-blue-800 dark:text-blue-300">
										...
										{getKycRequirementsForTier(requiredTier).regulator})
									</p>
						...
						<div className="space-y-2">
							...
							<ul className="space-y-2">
								{getKycRequirementsForTier(requiredTier).requirements.map(
									(req, idx) => (
										<li
											key={idx}
