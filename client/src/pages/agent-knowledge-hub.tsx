...
export default function AgentKnowledgeHub() {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {quickLinks.map((link) => (
          <Link key={link.href} href={link.href}>
            <Card className="bg-slate-900 border-slate-700 hover:border-slate-600 transition-colors cursor-pointer h-full" data-testid={`card-${link.title.toLowerCase().replace(/\s+/g, '-')}`}>
        ...
        <Card className="bg-slate-900 border-slate-700">
          ...
          <CardContent>
            <div className="space-y-4">
              <div>
                <h4 className="text-sm font-medium text-slate-300 mb-2">Market Snapshot</h4>
                <p className="text-slate-400 text-sm line-clamp-3">
                  {stats.todaysBrief.marketSnapshot}
                </p>
              </div>
              <div>
                <h4 className="text-sm font-medium text-slate-300 mb-2">What Changed</h4>
              ...
              {stats.todaysBrief.keyRisks && (
                <div className="p-3 bg-red-500/10 rounded-lg border border-red-500/20">
                  ...
                  <p className="text-slate-400 text-sm">{stats.todaysBrief.keyRisks}</p>
                </div>
              )}
      ...
      {!stats?.hasTodaysBrief && (
        <Card className="bg-slate-900 border-slate-700">
          <CardContent className="p-6 text-center">
            ...
            <h3 className="text-lg font-medium text-white mb-2">Market Brief Not Available</h3>
            ...
            <div className="space-y-2">
              {assetClasses.map((asset) => (
                <Link key={asset.name} href={`/agent/knowledge-hub/products?assetClass=${encodeURIComponent(asset.name.toLowerCase().replace(/\s+/g, '_'))}`}>
                  <div className="flex items-center justify-between p-3 rounded-lg bg-slate-800/50 hover:bg-slate-800 cursor-pointer transition-colors" data-testid={`asset-${asset.name.toLowerCase().replace(/\s+/g, '-')}`}>
          ...
          <CardContent>
            <div className="grid grid-cols-2 gap-4">
              <div className="p-4 rounded-lg bg-slate-800/50 text-center">
                <p className="text-3xl font-bold text-white">{stats?.productCardsCount || 0}</p>
                <p className="text-sm text-slate-400">Product Cards</p>
              </div>
              <div className="p-4 rounded-lg bg-slate-800/50 text-center">
                <p className="text-3xl font-bold text-white">{stats?.explanationTemplatesCount || 0}</p>
                <p className="text-sm text-slate-400">Templates</p>
              </div>
              <div className="p-4 rounded-lg bg-slate-800/50 text-center">
                <p className="text-3xl font-bold text-white">{stats?.certificationsCount || 0}</p>
                <p className="text-sm text-slate-400">Certifications</p>
              </div>
              <div className="p-4 rounded-lg bg-slate-800/50 text-center">
                <p className="text-3xl font-bold text-white">{stats?.assetInsightsCount || 0}</p>
      ...
      {disclaimer && (
        <div className="text-xs text-slate-500 text-center p-4 border-t border-slate-800">
