...
const SERVICE_OPTIONS = [
	{
		...
		subtitle: "ARN + NISM V-A required",
		icon: "📈",
	},
	{
		id: "insurance",
		label: "Insurance Distribution",
		subtitle: "Referral bonus · POSP code assigned by FintekPro",
		icon: "🛡️",
	},
	{
		id: "loans",
		label: "Loan / DSA",
		subtitle: "Referral bonus · no additional credentials needed",
		icon: "🏦",
	},
	{
		id: "ria",
		label: "Investment Advisory (RIA)",
		subtitle: "SEBI RIA number required",
		icon: "💼",
	},
	{
		id: "stocks",
		label: "Stock Broking Support",
		subtitle: "NISM XII required",
		icon: "📊",
	},
];
...
const NISM_TYPES = [
	...
	{
		value: "XII",
		label: "Series XII — Securities Operations & Risk Management",
	},
				...
				return (
					<div key={step.number} className="flex items-center flex-1 min-w-0">
						<div className="flex flex-col items-center gap-1 flex-shrink-0">
							<div
								className={`w-9 h-9 rounded-full flex items-center justify-center border-2 transition-all ${
										...
										: active
											...
											: reachable
												? "bg-background border-blue-300 text-blue-600"
							...
							<span
								className={`text-[10px] font-medium hidden sm:block ${active ? "text-blue-600" : done ? "text-green-600" : "text-muted-foreground"}`}
							>
								{step.label}
							</span>
						</div>
						{idx < STEPS.length - 1 && (
							<div
								className={`h-0.5 flex-1 mx-1 transition-all ${done ? "bg-green-400" : "bg-muted"}`}
							/>
						)}
					</div>
				);
...
export default function AgentKycEmpanelment() {
	...
	const { data: empanelmentData, isLoading } = useQuery<{
		success: boolean;
	...
	useEffect(() => {
		...
		if (emp.pan_verified) {
			...
			setPanName(emp.pan_name || "");
		}
		if (emp.aadhaar_verified) {
			setAadhaarVerified(true);
			setAadhaarLast4(emp.aadhaar_last4 || "");
		}
		if (emp.services_offered?.length) setServicesOffered(emp.services_offered);
		setArnCode(emp.arn_code || "");
		setArnExpiry(emp.arn_expiry_date || "");
		setEuinNumber(emp.euin_number || "");
		...
		setDsaCode(emp.dsa_code || "");
		setBankAccountNumber((emp.bank_account_number || "").replace(/\D/g, ""));
		setBankIfsc(emp.bank_ifsc || "");
		setBankHolderName(emp.bank_account_holder_name || "");
		setBankName(emp.bank_name || "");
		setBankBranch(emp.bank_branch || "");
		setBankVerified(emp.bank_verified || false);
		if (emp.bank_verified && emp.bank_name)
			setBankMsg(
				`Verified ✓ (${emp.bank_name}${emp.bank_branch ? " – " + emp.bank_branch : ""})`,
			);
		setDocNism(emp.doc_nism_certificate || "");
		setDocGrad(emp.doc_graduation_certificate || "");
		setDocPan(emp.doc_pan_card || "");
		setDocCheque(emp.doc_cancelled_cheque || "");
		setDocPhoto(emp.doc_photo || "");
		setPmla(emp.pmla_declaration_signed || false);
		setCriminal(emp.criminal_record_declaration || false);
		setFatca(emp.fatca_declaration_signed || false);
		setCoc(emp.code_of_conduct_accepted || false);
	...
	const submitMutation = useMutation({
		mutationFn: () =>
			apiRequest("/api/agent/empanelment/submit", { method: "POST" }),
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: ["/api/agent/empanelment"] });
			toast({
				title: "Application Submitted!",
				description: "Admin will review within 2–3 business days.",
			});
		},
		onError: (err: any) =>
			toast({
	...
	async function verifyPAN() {
		...
		try {
			const res = await apiRequest("/api/kyc/verify-pan", {
				method: "POST",
			...
			if (res.verified) {
				...
				toast({
					...
					description: res.message || "PAN could not be verified",
					variant: "destructive",
				});
			}
		} catch {
	...
	async function sendAadhaarOtp() {
		if (aadhaarNumber.length !== 12) {
			toast({ title: "Enter 12-digit Aadhaar number", variant: "destructive" });
			return;
		}
	...
	async function verifyAadhaarOtp() {
		...
		try {
			const res = await apiRequest(
				"/api/agent/empanelment/aadhaar/verify-otp",
				{
					method: "POST",
					body: JSON.stringify({
			...
			if (res.verified) {
				setAadhaarVerified(true);
				setAadhaarLast4(res.last4 || aadhaarNumber.slice(-4));
				toast({
					title: "Aadhaar Verified ✓",
					description: `Identity confirmed (****${res.last4 || aadhaarNumber.slice(-4)})`,
				});
			} else {
				toast({
					title: "Invalid OTP",
	...
	async function verifyBank() {
		...
		setBankMsg("Initiating penny drop verification…");
		try {
			const res = await apiRequest(
				"/api/agent/empanelment/step/4/verify-bank",
				{
					method: "POST",
					body: JSON.stringify({
	...
	function saveStep1() {
		if (!panVerified) {
			toast({ title: "PAN verification required", variant: "destructive" });
			return;
		}
		saveMutation.mutate({
			endpoint: "/api/agent/empanelment/step/1",
			body: { panVerified, panNumber, panName, aadhaarVerified, aadhaarLast4 },
		});
		setStep(2);
	}
	...
	function saveStep2() {
		...
		saveMutation.mutate({
			endpoint: "/api/agent/empanelment/step/2",
	...
	function saveStep3() {
		if (servicesOffered.includes("mutual_fund") && !arnCode) {
			toast({
				title: "ARN code required for MF distribution",
				variant: "destructive",
			});
			return;
		}
		if (servicesOffered.includes("ria") && !riaNumber) {
			toast({ title: "SEBI RIA number required", variant: "destructive" });
			return;
		}
		saveMutation.mutate({
			endpoint: "/api/agent/empanelment/step/3",
			body: {
				arnCode,
				arnExpiryDate: arnExpiry,
				euinNumber,
				nismCertificateNumber: nismCertNum,
				nismCertificateType: nismCertType,
				nismExpiryDate: nismExpiry,
				riaNumber,
				pospNumber,
				pospInsurer,
				dsaCode,
	...
	function saveStep4() {
		if (!bankVerified) {
			toast({
				title: "Bank account must be verified via penny drop",
				variant: "destructive",
			});
			return;
		}
	...
	function saveStep5() {
		saveMutation.mutate({
			endpoint: "/api/agent/empanelment/step/5",
			body: {
				docNismCertificate: docNism,
				docGraduationCertificate: docGrad,
				docPanCard: docPan,
				docCancelledCheque: docCheque,
				docPhoto,
			},
		});
		setStep(6);
	...
	function saveStep6() {
		if (!pmla || !fatca || !coc || !antiMis) {
			toast({
				title: "All declarations are mandatory",
		...
		saveMutation.mutate({
			endpoint: "/api/agent/empanelment/step/6",
			body: {
				pmlaDeclarationSigned: pmla,
				criminalRecordDeclaration: criminal,
				fatcaDeclarationSigned: fatca,
				codeOfConductAccepted: coc,
				antiMisSellingAccepted: antiMis,
			},
		});
		setStep(7);
	}
	...
	const isSubmitted =
		emp?.status === "submitted" || emp?.status === "under_review";
			...
			<div className="max-w-3xl mx-auto p-4 md:p-6 space-y-4">
				...
				<div className="flex items-start justify-between">
					<div>
						...
						<p className="text-muted-foreground mt-1 text-sm">
							Your professional empanelment profile. KYC data is verified and
							stored securely.
						</p>
					</div>
					{isApproved && (
				...
				{isSubmitted && (
					<div className="p-3 bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-800 rounded-lg text-sm text-blue-700 dark:text-blue-300">
						Your application is under review. Our compliance team will verify
						within <strong>2–3 business days</strong>. Editing is paused until
						approval.
					</div>
				)}
					...
					<CardContent className="pt-0 px-4 pb-4">
						<div className="grid grid-cols-2 gap-3 text-sm">
							...
							<div>
								...
								<p className="font-medium">{emp?.pan_name || "—"}</p>
							</div>
							<div>
						...
						<div className="flex flex-wrap gap-2">
							{(emp?.services_offered || []).map((s) => (
								<Badge key={s} variant="secondary">
									{SERVICE_OPTIONS.find((x) => x.id === s)?.label || s}
								</Badge>
							))}
							{!emp?.services_offered?.length && (
						...
						<div className="grid grid-cols-2 gap-3 text-sm">
							{emp?.arn_code && (
								<div>
									...
									<p className="font-mono font-medium">{emp.arn_code}</p>
								</div>
							)}
							{emp?.euin_number && (
								<div>
									<p className="text-xs text-muted-foreground">EUIN</p>
									<p className="font-mono font-medium">{emp.euin_number}</p>
								</div>
							)}
							{emp?.nism_certificate_number && (
								<div>
							...
							{emp?.nism_certificate_type && (
								<div>
									<p className="text-xs text-muted-foreground">NISM Series</p>
									<p className="font-medium">{emp.nism_certificate_type}</p>
								</div>
							)}
							{emp?.ria_number && (
								<div>
									<p className="text-xs text-muted-foreground">SEBI RIA</p>
									<p className="font-mono font-medium">{emp.ria_number}</p>
								</div>
							)}
							{emp?.posp_number && (
								<div>
									<p className="text-xs text-muted-foreground">POSP</p>
									<p className="font-mono font-medium">{emp.posp_number}</p>
								</div>
							)}
							{emp?.dsa_code && (
								<div>
									<p className="text-xs text-muted-foreground">DSA Code</p>
									<p className="font-mono font-medium">{emp.dsa_code}</p>
								</div>
							)}
							{!emp?.arn_code &&
								!emp?.ria_number &&
						...
						<div className="grid grid-cols-2 gap-3 text-sm">
							...
							<div>
								...
								<p className="font-mono font-medium">
									{"*".repeat(
										Math.max(0, (emp?.bank_account_number?.length || 0) - 4),
									)}
							...
							<div>
								...
								<p className="font-mono font-medium">{emp?.bank_ifsc || "—"}</p>
							</div>
							<div>
						...
						<div className="grid grid-cols-2 gap-2 text-sm">
							{[
								...
								<div key={label as string} className="flex items-center gap-2">
									...
									<span
										className={`text-xs ${val ? "text-foreground" : "text-muted-foreground"}`}
									>
										{label as string}
									</span>
								</div>
		...
		<div className="max-w-3xl mx-auto p-4 md:p-6 space-y-6">
				...
				<Card>
					<CardHeader>
						<CardTitle className="flex items-center gap-2">
							<User className="h-5 w-5 text-blue-600" /> Step 1 — Personal
							Identity Verification
						</CardTitle>
						<CardDescription>
							Verify your PAN and Aadhaar. This is your identity as the agent,
							not your clients'.
						</CardDescription>
					</CardHeader>
					<CardContent className="space-y-6">
						...
						<div className="space-y-3">
							...
							<div className="flex gap-3">
								...
								<div className="flex items-end">
									{panVerified ? (
										<Badge className="bg-green-100 text-green-800 border-green-200 gap-1 h-10 px-3">
							...
							{panVerified && panName && (
								<div className="p-3 bg-green-50 dark:bg-green-950/30 rounded-lg">
									<p className="text-sm text-green-700 dark:text-green-300">
										✓ Registered Name: <strong>{panName}</strong>
									</p>
								</div>
							)}
						</div>
							...
							{!aadhaarVerified ? (
								<>
									<div className="space-y-1">
										...
										/>
										<p className="text-xs text-muted-foreground">
											OTP will be sent to the mobile number linked to your
											Aadhaar
										</p>
									</div>
									{!aadhaarOtpSent ? (
										<Button
											...
											disabled={
												aadhaarNumber.length !== 12 || aadhaarSendingOtp
											}
										>
											{aadhaarSendingOtp ? (
												<>
										...
										<div className="space-y-2">
											...
											<div className="flex gap-3">
												...
												<Button
													...
													disabled={
														aadhaarOtp.length !== 6 || aadhaarVerifyingOtp
													}
												>
													{aadhaarVerifyingOtp ? (
														<>
						...
						<div className="p-3 bg-blue-50 dark:bg-blue-950/30 rounded-lg flex items-start gap-2">
							...
							<p className="text-xs text-blue-700 dark:text-blue-300">
								Your PAN and Aadhaar are used only for agent identity
								verification per SEBI/PMLA requirements. This is separate from
								your clients' KYC.
							</p>
						</div>
					</CardContent>
			...
			{step === 2 && (
				<Card>
					<CardHeader>
						<CardTitle className="flex items-center gap-2">
							<Briefcase className="h-5 w-5 text-blue-600" /> Step 2 — Role &
							Services
						</CardTitle>
						<CardDescription>
							Select the financial services you will offer. This determines
							which professional credentials are required.
						</CardDescription>
					</CardHeader>
					<CardContent>
					...
					<CardFooter className="justify-between">
						...
						<Button
							onClick={saveStep2}
							disabled={!servicesOffered.length}
							className="gap-2"
						>
							Continue <ArrowRight className="h-4 w-4" />
						</Button>
			...
			{step === 3 && (
				<Card>
					<CardHeader>
						<CardTitle className="flex items-center gap-2">
							<Award className="h-5 w-5 text-blue-600" /> Step 3 — Professional
							Credentials
						</CardTitle>
						<CardDescription>
							Enter your AMFI, NISM, and regulatory registration details.
						</CardDescription>
					</CardHeader>
					<CardContent className="space-y-6">
						{/* ARN + EUIN — for MF */}
								...
								<div className="grid md:grid-cols-2 gap-4">
									...
									<div className="space-y-1">
										...
										<Input
											...
											value={euinNumber}
											onChange={(e) =>
												setEuinNumber(e.target.value.toUpperCase())
											}
											className="font-mono"
										/>
									</div>
							...
							<div className="grid md:grid-cols-2 gap-4">
								<div className="space-y-1 md:col-span-2">
									...
									<Select value={nismCertType} onValueChange={setNismCertType}>
										<SelectTrigger>
											<SelectValue placeholder="Select NISM Series" />
										</SelectTrigger>
										<SelectContent>
											{NISM_TYPES.map((t) => (
								...
								<div className="space-y-1">
									...
									<Input
										placeholder="NISM-SERIESVA-XXXXXXXX-XX"
										value={nismCertNum}
										onChange={(e) =>
											setNismCertNum(e.target.value.toUpperCase())
										}
								...
								<div className="space-y-1">
									...
									<Input
										type="date"
										value={nismExpiry}
										onChange={(e) => setNismExpiry(e.target.value)}
									/>
								</div>
							</div>
						</div>
						...
						{servicesOffered.includes("insurance") && (
							<div className="p-4 border border-green-200 dark:border-green-800 rounded-xl bg-green-50 dark:bg-green-950/30">
								<h3 className="font-semibold text-sm text-green-700 dark:text-green-300 flex items-center gap-2 mb-2">
									<span className="text-lg">🛡️</span> Insurance Distribution —
									Referral Model
								</h3>
								<p className="text-sm text-green-700 dark:text-green-400">
									Your <strong>POSP code will be assigned by FintekPro</strong>{" "}
									— no license or certificate is required from you. You will
									earn referral bonuses on every policy sold through your link.
									No additional documents needed for this service.
								</p>
							</div>
						)}
						...
						{servicesOffered.includes("loans") && (
							<div className="p-4 border border-cyan-200 dark:border-cyan-800 rounded-xl bg-cyan-50 dark:bg-cyan-950/30">
								<h3 className="font-semibold text-sm text-cyan-700 dark:text-cyan-300 flex items-center gap-2 mb-2">
									<span className="text-lg">🏦</span> Loan / DSA — Referral
									Model
								</h3>
								<p className="text-sm text-cyan-700 dark:text-cyan-400">
									You operate as a <strong>referral partner</strong> for loan
									products. No DSA code or empanelment document is required from
									you. You will earn referral bonuses on every loan disbursed
									through your leads. No additional credentials needed.
								</p>
							</div>
						)}
					</CardContent>
					<CardFooter className="justify-between">
						...
						<Button onClick={saveStep3} className="gap-2">
							Continue <ArrowRight className="h-4 w-4" />
						</Button>
					</CardFooter>
				</Card>
			...
			{step === 4 && (
				<Card>
					<CardHeader>
						<CardTitle className="flex items-center gap-2">
							<Building2 className="h-5 w-5 text-blue-600" /> Step 4 — Bank
							Account Verification
						</CardTitle>
						<CardDescription>
							Your agent payout account. Verified via penny drop — a small
							credit will be sent to confirm ownership.
						</CardDescription>
					</CardHeader>
					<CardContent className="space-y-5">
						<div className="grid md:grid-cols-2 gap-4">
							<div className="space-y-1 md:col-span-2">
								...
								<Input
									...
									onChange={(e) =>
										setBankHolderName(e.target.value.toUpperCase())
									}
							...
							<div className="space-y-1">
								...
								/>
								<p className="text-xs text-muted-foreground">
									11 characters: Bank code (4) + 0 + Branch (6)
								</p>
							</div>
						</div>
						...
						<div className="p-3 bg-blue-50 dark:bg-blue-950/20 rounded-lg flex items-start gap-2">
							<Info className="h-4 w-4 text-blue-600 mt-0.5 flex-shrink-0" />
							<p className="text-xs text-blue-700 dark:text-blue-300">
								A ₹1 credit will be sent to verify your account ownership. This
								is refunded or used as the first commission credit. The name on
								your account must match your KYC name (80%+ match required).
							</p>
						</div>
					</CardContent>
					<CardFooter className="justify-between">
			...
			{step === 5 && (
				<Card>
					<CardHeader>
						<CardTitle className="flex items-center gap-2">
							<Upload className="h-5 w-5 text-blue-600" /> Step 5 — Document
							Uploads
						</CardTitle>
						<CardDescription>
							Upload scanned copies of your certificates and identity documents.
					...
					<CardContent className="space-y-4">
						{[
							{
								...
								value: docNism,
								set: setDocNism,
								hint: "Your NISM pass certificate PDF",
							},
							{
								...
								value: docGrad,
								set: setDocGrad,
								hint: "Required for SEBI/AMFI eligibility",
							},
							{
								label: "PAN Card *",
								key: "pan",
							...
							},
							{
								...
								value: docCheque,
								set: setDocCheque,
								hint: "Bank account matching Step 4",
							},
							{
								...
								value: docPhoto,
								set: setDocPhoto,
								hint: "Recent colour photo, white background",
							},
						].map((doc) => (
							<div
								key={doc.key}
								className="flex items-center gap-4 p-3 border rounded-xl hover:border-blue-300 transition-colors"
							>
								<div
									className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${doc.value ? "bg-green-100 dark:bg-green-900" : "bg-muted"}`}
								>
									{doc.value ? (
						...
						<div className="p-3 bg-amber-50 dark:bg-amber-950/20 rounded-lg flex items-start gap-2">
							...
							<p className="text-xs text-amber-700 dark:text-amber-300">
								Documents are securely stored and used only for regulatory
								verification. Originals may be requested for in-person
								verification.
							</p>
						</div>
					</CardContent>
					<CardFooter className="justify-between">
						...
						<Button onClick={saveStep5} className="gap-2">
							Continue <ArrowRight className="h-4 w-4" />
						</Button>
					</CardFooter>
				</Card>
			...
			{step === 6 && (
				<Card>
					<CardHeader>
						<CardTitle className="flex items-center gap-2">
							<ScrollText className="h-5 w-5 text-blue-600" /> Step 6 —
							Compliance Declarations
						</CardTitle>
						<CardDescription>
							Mandatory declarations required under SEBI, PMLA, and FATCA
							regulations. Read carefully before signing.
						</CardDescription>
					</CardHeader>
					<CardContent className="space-y-4">
						{[
							...
							<div
								key={decl.id}
								className={`p-4 border-2 rounded-xl transition-colors ${decl.checked ? "border-green-200 bg-green-50 dark:border-green-800 dark:bg-green-950/20" : "border-border"}`}
							>
								<div className="flex items-start gap-3">
									<Checkbox
										id={decl.id}
										checked={decl.checked}
										onCheckedChange={(v) => decl.set(v === true)}
										className="mt-1"
									/>
									<div className="space-y-1">
										<label
					...
					<CardFooter className="justify-between">
						...
						<Button
							onClick={saveStep6}
							disabled={!pmla || !fatca || !coc || !antiMis}
							className="gap-2"
						>
							Continue <ArrowRight className="h-4 w-4" />
						</Button>
			...
			{step === 7 && (
				<Card>
					<CardHeader>
						<CardTitle className="flex items-center gap-2">
							<Eye className="h-5 w-5 text-blue-600" /> Step 7 — Review & Submit
						</CardTitle>
						<CardDescription>
							Review all details before submitting for admin approval. You
					...
					<CardContent className="space-y-5">
						...
						<div className="p-4 border rounded-xl space-y-2">
							...
							<div className="grid grid-cols-2 gap-3 text-sm">
								{arnCode && (
									<div>
										<p className="text-xs text-muted-foreground">ARN</p>
										<p className="font-mono font-medium">{arnCode}</p>
									</div>
								)}
								{euinNumber && (
									<div>
										<p className="text-xs text-muted-foreground">EUIN</p>
										<p className="font-mono font-medium">{euinNumber}</p>
									</div>
								)}
								{nismCertNum && (
									<div>
										<p className="text-xs text-muted-foreground">NISM Cert</p>
										<p className="font-mono font-medium text-xs">
											{nismCertNum}
										</p>
									</div>
								...
								{nismCertType && (
									<div>
										<p className="text-xs text-muted-foreground">NISM Series</p>
										<p className="font-medium">{nismCertType}</p>
									</div>
								)}
								{riaNumber && (
									<div>
										<p className="text-xs text-muted-foreground">SEBI RIA</p>
										<p className="font-mono font-medium">{riaNumber}</p>
									</div>
								)}
								{pospNumber && (
									<div>
										<p className="text-xs text-muted-foreground">POSP</p>
										<p className="font-mono font-medium">{pospNumber}</p>
									</div>
								)}
								{dsaCode && (
									<div>
										<p className="text-xs text-muted-foreground">DSA Code</p>
										<p className="font-mono font-medium">{dsaCode}</p>
									</div>
								)}
							</div>
						</div>
						...
						<div className="p-4 border rounded-xl space-y-2">
							...
							<div className="grid grid-cols-2 gap-3 text-sm">
								<div>
									...
									<p className="font-medium">{bankHolderName}</p>
								</div>
								<div>
									<p className="text-xs text-muted-foreground">
										Account Number
									...
									<p className="font-mono font-medium">
										{"*".repeat(Math.max(0, bankAccountNumber.length - 4))}
										{bankAccountNumber.slice(-4)}
									</p>
								</div>
								<div>
									<p className="text-xs text-muted-foreground">IFSC</p>
									<p className="font-mono font-medium">{bankIfsc}</p>
								</div>
								<div>
									<p className="text-xs text-muted-foreground">Bank</p>
									<p className="font-medium flex items-center gap-1">
						...
						<div className="p-4 border rounded-xl space-y-2">
							...
							<div className="grid grid-cols-1 gap-1 text-sm">
								{[
									["PMLA / AML", pmla],
									["Criminal Record (None)", criminal],
									["FATCA / CRS", fatca],
									["Code of Conduct", coc],
									["Anti-Mis-Selling", antiMis],
								].map(([label, val]) => (
									<div
										key={label as string}
									...
									>
										...
										<span className={val ? "text-foreground" : "text-red-600"}>
											{label as string}
										</span>
									</div>
								))}
						...
						<div className="p-4 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 rounded-xl flex items-start gap-3">
							...
							<div>
								...
								<p className="text-xs text-amber-700 dark:text-amber-300 mt-1">
									Ensure all details are accurate. False declarations are
									punishable under applicable laws. You will not be able to edit
									after submission.
								</p>
							</div>
						</div>
					...
					<CardFooter className="justify-between">
						...
						<Button
							...
							disabled={
								submitMutation.isPending || !bankVerified || !panVerified
							}
