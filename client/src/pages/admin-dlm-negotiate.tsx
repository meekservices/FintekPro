...
// Permission matrix
const ROLE_PERMISSIONS: Record<UserRole, { canAcceptReject: boolean; canComment: boolean; canSuggestChanges: boolean }> = {
  admin: { canAcceptReject: true, canComment: true, canSuggestChanges: true },
  reviewer: { canAcceptReject: false, canComment: true, canSuggestChanges: false },
  editor: { canAcceptReject: false, canComment: true, canSuggestChanges: true },
  viewer: { canAcceptReject: false, canComment: false, canSuggestChanges: false },
};
...
export default function AdminDLMNegotiatePage() {
  ...
  const { data: documentData, isLoading: documentLoading } = useQuery<any>({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/dlm/documents/${documentId}`, { credentials: "include" });
  ...
  const { data: changesData, isLoading: changesLoading, refetch: refetchChanges } = useQuery<any>({
    queryKey: ["/api/dlm/documents", documentId, "changes"],
    queryFn: async () => {
      const res = await fetch(`/api/dlm/documents/${documentId}/changes`, { credentials: "include" });
  ...
  const { data: commentsData, isLoading: commentsLoading, refetch: refetchComments } = useQuery<any>({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/dlm/documents/${documentId}/comments`, { credentials: "include" });
  ...
  const { data: aiReviewData } = useQuery<any>({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/dlm/documents/${documentId}/ai-review`, { credentials: "include" });
  ...
  const resolveChangeMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/dlm/documents", documentId, "changes"] });
      toast({ title: "Change resolved", description: "The tracked change has been processed." });
    },
    onError: () => {
      toast({ title: "Error", description: "Failed to resolve change", variant: "destructive" });
    },
  });
  ...
  const addCommentMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/dlm/documents", documentId, "comments"] });
      setNewComment("");
  ...
  const resolveCommentMutation = useMutation({
    mutationFn: async (commentId: string) => {
      return apiRequest(`/api/dlm/comments/${commentId}/resolve`, { method: "PATCH" });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/dlm/documents", documentId, "comments"] });
  ...
  const triggerAIReviewMutation = useMutation({
    mutationFn: async () => {
      return apiRequest(`/api/dlm/documents/${documentId}/ai-review`, { method: "POST" });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/dlm/documents", documentId, "ai-review"] });
      toast({ title: "AI Review Complete", description: "The compliance analysis has been updated." });
    },
    onError: () => {
  ...
  const resolvedChanges = useMemo(() => changes.filter(c => c.status !== "pending"), [changes]);
  const unresolvedComments = useMemo(() => comments.filter(c => !c.isResolved), [comments]);

  const currentVersion = useMemo(() => {
    if (selectedVersion) {
      return versions.find((v: DocumentVersion) => v.id === selectedVersion);
    }
    return versions.find((v: DocumentVersion) => v.id === document?.currentVersionId) || versions[0];
  ...
  if (!document) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen gap-4">
        ...
        <Button onClick={() => setLocation("/admin/dlm")} data-testid="button-back-to-dlm">
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-xl font-semibold" data-testid="text-document-title">{document.title}</h1>
            ...
            <div className="flex items-center gap-3">
              ...
              >
                ...
                {triggerAIReviewMutation.isPending ? "Analyzing..." : "AI Review"}
              </Button>
            </div>
          </div>
        </div>
            ...
            <Card>
              ...
              <CardContent>
                <ScrollArea className="h-[500px] border rounded-lg p-4">
                  ...
                  <div className="prose prose-sm max-w-none" data-testid="document-content">
                    {currentVersion?.content ? (
                      <div className="whitespace-pre-wrap font-mono text-sm leading-relaxed">
                        {renderDocumentWithDiffs(currentVersion.content, pendingChanges, true)}
                      </div>
                    ) : (
                      <p className="text-muted-foreground italic">No content available for this version</p>
                    )}
                  </div>
                ...
                <Tabs value={activeTab} onValueChange={setActiveTab}>
                  ...
                  <TabsContent value="changes" className="mt-0">
                    ...
                    <div className="flex items-center justify-between mb-3 pb-2 border-b">
                      <span className="text-xs text-muted-foreground">
                        {pendingChanges.length} pending, {resolvedChanges.length} resolved
                      </span>
                      <Button
                        variant="ghost"
                        size="sm"
                        className="text-xs h-6"
                        onClick={() => setShowResolvedChanges(!showResolvedChanges)}
                        data-testid="button-toggle-resolved"
                      >
                        {showResolvedChanges ? "Hide Resolved" : "Show Resolved"}
                      </Button>
                    </div>
                    <ScrollArea className="h-[370px]">
                      {changesLoading ? (
                        ...
                        <div className="space-y-2">
                          ...
                          {pendingChanges.length > 0 && (
                            <div className="mb-3">
                              <h5 className="text-xs font-medium text-muted-foreground mb-2">Pending</h5>
                              {pendingChanges.map((change) => (
                                <div 
                                  key={change.id}
                          ...
                          {showResolvedChanges && resolvedChanges.length > 0 && (
                            <div>
                              <h5 className="text-xs font-medium text-muted-foreground mb-2">Resolved</h5>
                              {resolvedChanges.map((change) => (
                                ...
                                >
                                  ...
                                  {change.newText && (
                                    <p className="text-xs truncate">{change.newText}</p>
                                  )}
                          ...
                          {pendingChanges.length === 0 && !showResolvedChanges && (
                            <div className="text-center py-4 text-muted-foreground text-sm">
                              No pending changes. Click "Show Resolved" to view history.
                            </div>
                          )}
                        </div>
                      )}
            ...
            <Card>
              ...
              <CardContent className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{document.entityName || document.entityType}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Agreement Type</span>
                  <span className="font-medium">{document.agreementType?.replace(/_/g, " ")}</span>
                </div>
                {document.entityPan && (
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">PAN</span>
                    <span className="font-medium font-mono">{document.entityPan}</span>
                  </div>
                )}
                {document.effectiveDate && (
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Effective Date</span>
                    <span className="font-medium">{new Date(document.effectiveDate).toLocaleDateString()}</span>
                  </div>
                )}
                {document.expiryDate && (
                  <div className="flex justify-between">
