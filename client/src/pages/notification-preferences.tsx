...
export default function NotificationPreferences() {
	...
	const [categories, setCategories] =
		useState<NotificationCategory[]>(defaultCategories);
	...
	const handleTestNotification = async () => {
		...
		try {
			if ("Notification" in window && Notification.permission === "granted") {
				...
				try {
					...
					toast({
						...
						description:
							"Backend test not available, but local notification triggered.",
					});
				}
			} else {
				toast({
			...
			if ("Notification" in window && Notification.permission === "granted") {
				...
				toast({
					title: "Test Sent",
					description: "Local notification triggered.",
				});
			}
		} finally {
			setIsTesting(false);
					...
					{!pushSupported ? (
						...
						<Alert variant="destructive">
							...
							<AlertDescription>
								...
								browser settings to receive real-time alerts.
							</AlertDescription>
						</Alert>
					) : (
								...
								{pushSubscribed ? (
									...
									<>
										<Badge
											...
											data-testid="push-status-disabled"
										>
											Disabled
										</Badge>
										<span className="text-sm text-muted-foreground">
							...
							>
								...
								<div className="flex justify-center">
									<Switch
										...
										onCheckedChange={() =>
											toggleChannel(category.id, "whatsapp")
										}
				...
				<CardContent className="space-y-4">
					<div className="flex items-center justify-between">
						<div>
							...
							<p className="text-sm text-muted-foreground">
								How often would you like to receive portfolio summaries?
							</p>
						</div>
						<Select value={digestFrequency} onValueChange={setDigestFrequency}>
							<SelectTrigger
								className="w-[180px]"
								data-testid="digest-frequency-select"
							>
								<SelectValue />
							</SelectTrigger>
							<SelectContent>
