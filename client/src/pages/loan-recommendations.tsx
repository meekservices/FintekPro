...
function LoanRecommendationCard({
	recommendation,
	onApplyClick,
}: {
		...
		<Card className="group hover:shadow-xl transition-all duration-300 border-0 bg-gradient-to-br from-white to-gray-50 overflow-hidden">
			...
			<CardContent className="relative space-y-6">
				...
				<div className="flex gap-4">
					<div
						className={`flex-1 p-3 rounded-lg border ${getUrgencyColor(recommendation.urgency)}`}
					>
					...
					{recommendation.specialOffers &&
						recommendation.specialOffers.length > 0 && (
...
export default function LoanRecommendationsPage() {
	...
	const {
		...
		error,
		refetch,
	} = useQuery<{ data: RecommendationData }>({
	...
	const handleApplyClick = (recommendation: LoanRecommendation) => {
		...
		setLocation(
			`/partner-application/${lenderAdapter}?recommendation=${recommendationId}&amount=${recommendation.recommendedAmount}&tenure=${recommendation.tenure}&type=${recommendation.loanType}`,
		);
	};
	...
	const handleRefresh = () => {
		...
		toast({
			...
			description:
				"Your loan recommendations have been refreshed based on latest data.",
		});
	};
