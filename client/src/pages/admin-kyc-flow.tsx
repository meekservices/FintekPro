...
export default function AdminKycFlow() {
	...
	const updatePriceMutation = useMutation({
		mutationFn: async ({
			...
			providerId,
			pricePerCall,
		}: { stepId: string; providerId: string; pricePerCall: number }) => {
			return apiRequest(
				`/api/admin/kyc/flow/${stepId}/provider/${providerId}/price`,
				{
					method: "PATCH",
					headers: { "Content-Type": "application/json" },
		...
		onSuccess: () => {
			toast({
				...
				description: "Provider pricing has been saved",
			});
			setEditPricing(null);
			queryClient.invalidateQueries({ queryKey: ["/api/admin/kyc/flow"] });
		},
	...
	return (
		<div className="space-y-6">
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-muted-foreground">
						Configure priority-based provider selection for each KYC
						verification step
					</p>
				</div>
			...
			<Alert>
				...
				<AlertDescription>
					Each KYC step tries providers in priority order. If the top-priority
					provider fails or is not configured, the system falls back to the next
					one automatically. Priority and pricing changes are persisted to the
					database and survive server restarts. Only providers marked{" "}
					<strong>Ready</strong> will be selected for live verification flows.
				</AlertDescription>
			</Alert>

			<Accordion
				type="multiple"
				defaultValue={steps.map((s) => s.stepId)}
				className="space-y-3"
			>
				{steps.map((step) => {
					const StepIcon = stepIcons[step.stepId] || LucideShield;
					const configuredCount = step.providers.filter(
						(p) => p.isConfigured,
					).length;
					const sortedProviders = [...step.providers].sort(
											...
											>
												<div className="flex flex-col gap-1">
													<Button
														...
														disabled={
															index === 0 || updatePriorityMutation.isPending
														}
													...
													<div className="text-right">
														<div className="flex items-center gap-1">
															...
															<span className="font-bold text-sm">
																{provider.pricePerCall === 0
																	? "Free"
																	: `₹${provider.pricePerCall.toFixed(2)}`}
															</span>
														</div>
																	...
																	onClick={() => {
																		if (editPricing) {
																			updatePriceMutation.mutate({
																				stepId: editPricing.stepId,
																				providerId: editPricing.providerId,
																				pricePerCall: editPricing.price,
																			});
																		}
																	}}
