...
interface FormState {
	...
	// Agreements
	agreedCustomer: boolean;
	agreedMargin: boolean;
	agreedAccount: boolean;
}
...
function StatusBanner({
	status,
	accountNumber,
}: { status: string; accountNumber?: string }) {
	const config: Record<
		string,
		{ color: string; icon: any; title: string; desc: string }
	> = {
		SUBMITTED: {
			color:
	...
	return (
		<Alert className={`${cfg.color} mb-5`}>
			<Icon
				className={`h-4 w-4 ${status === "ACTIVE" || status === "APPROVED" ? "text-green-600" : status === "REJECTED" ? "text-red-500" : status === "SUBMITTED" ? "text-blue-500" : "text-amber-500"}`}
			/>
			<AlertDescription>
...
function StepIndicator({
	current,
	completed,
}: { current: number; completed: Set<number> }) {
	return (
				...
				return (
					<div key={step.id} className="flex items-center flex-1">
						<div className="flex flex-col items-center">
							...
							<span
								className={`text-xs mt-1 font-medium hidden md:block ${active ? "text-primary" : done ? "text-green-600 dark:text-green-400" : "text-muted-foreground"}`}
							>
								{step.label}
							</span>
						</div>
						{i < STEPS.length - 1 && (
							<div
								className={`flex-1 h-0.5 mx-1 ${done ? "bg-green-500" : "bg-muted"}`}
							/>
						)}
					</div>
				);
...
function IdentityStep({
	...
	setForm,
	prefill,
}: { form: FormState; setForm: any; prefill: Prefill }) {
	const f = (field: keyof FormState, val: string) =>
		setForm((p: FormState) => ({ ...p, [field]: val }));
		...
		<div className="space-y-5">
			<div>
				...
				<p className="text-sm text-muted-foreground">
					Pre-filled from your verified KYC profile. Please review and complete
					any missing fields.
				</p>
			</div>
			...
			<div className="grid grid-cols-3 gap-3">
				...
				<div className="space-y-1.5">
					...
					<Input
						...
						onChange={(e) => f("middleName", e.target.value)}
			...
			<div className="grid grid-cols-2 gap-3">
				...
				<div className="space-y-1.5">
					...
					<Input
						...
						onChange={(e) => f("countryOfCitizenship", e.target.value)}
						placeholder="IND"
					/>
				</div>
			</div>
			...
			<div className="space-y-1.5">
				...
				<Input
					...
					onChange={(e) => f("streetAddress", e.target.value)}
...
function FinancialStep({ form, setForm }: { form: FormState; setForm: any }) {
	const f = (field: keyof FormState, val: any) =>
		setForm((p: FormState) => ({ ...p, [field]: val }));
	...
	return (
		<div className="space-y-5">
			<div>
				...
				<p className="text-sm text-muted-foreground">
					Required by US regulators. This information is kept strictly
					confidential.
				</p>
			</div>
			...
			<div className="space-y-1.5">
				...
				<Select
					...
					onValueChange={(v) => f("fundingSource", v)}
				>
					<SelectTrigger>
				...
				>
					...
					<SelectContent>
						{INCOME_OPTIONS.map((o) => (
							<SelectItem key={o.min} value={`${o.min}|${o.max}`}>
								{o.label}
							</SelectItem>
						))}
					</SelectContent>
			...
			<div className="space-y-1.5">
				...
				<Select
					...
					onValueChange={(v) => {
						...
						setForm((p: FormState) => ({
							...p,
							liquidNetWorthMin: min,
							liquidNetWorthMax: max,
						}));
					}}
				>
					<SelectTrigger>
				...
				<p className="text-xs text-muted-foreground">
					Cash, stocks, bonds — assets easily converted to cash within 5 years
				</p>
			</div>
			...
			<div className="space-y-1.5">
				...
				<Select
					...
					onValueChange={(v) => {
						...
						setForm((p: FormState) => ({
							...p,
							totalNetWorthMin: min,
							totalNetWorthMax: max,
						}));
					}}
				>
					<SelectTrigger>
						<SelectValue placeholder="Select total net worth range" />
					</SelectTrigger>
					<SelectContent>
						{NW_OPTIONS.map((o) => (
							<SelectItem key={o.min} value={`${o.min}|${o.max}`}>
			...
			<Alert className="border-blue-200 bg-blue-50 dark:bg-blue-950/20 py-3">
				...
				<AlertDescription className="text-xs text-blue-700 dark:text-blue-300">
					<strong>LRS Note:</strong> Indian residents can remit up to{" "}
					<strong>$250,000 USD per financial year</strong> under RBI's
					Liberalised Remittance Scheme. Your investments via this account will
					be tracked against this limit.
				</AlertDescription>
			</Alert>
		</div>
	);
...
function DisclosuresStep({ form, setForm }: { form: FormState; setForm: any }) {
	const f = (field: keyof FormState, val: boolean) =>
		setForm((p: FormState) => ({ ...p, [field]: val }));
	...
	const disclosures = [
		{
			...
			description:
				"Are you a director, officer, or 10%+ shareholder of any publicly traded company?",
		},
		{
			field: "isAffiliatedExchangeOrFinra" as const,
			title: "Exchange / FINRA / SEBI Affiliation",
			description:
				"Are you or an immediate family member employed by, or associated with, a stock exchange, FINRA, or SEBI?",
		},
		{
			field: "isPoliticallyExposed" as const,
			title: "Politically Exposed Person (PEP)",
			description:
				"Are you a current or former senior political official, government leader, or head of state?",
		},
		{
			field: "immediateFamilyExposed" as const,
			title: "Immediate Family Member is PEP",
			description:
				"Is an immediate family member (spouse, parent, child, sibling) a politically exposed person?",
		},
	];
	...
	return (
		<div className="space-y-5">
			<div>
				...
				<p className="text-sm text-muted-foreground">
					US FINRA and SEC regulations require us to ask these questions. Answer
					honestly — most clients answer No to all.
				</p>
			</div>
			...
			<div className="space-y-4">
				{disclosures.map((d) => (
					<Card
						key={d.field}
						className={`border ${form[d.field] ? "border-amber-300 bg-amber-50 dark:bg-amber-950/10" : ""}`}
					>
			...
			{(form.isControlPerson ||
				form.isAffiliatedExchangeOrFinra ||
				form.isPoliticallyExposed ||
				form.immediateFamilyExposed) && (
				<Alert className="border-amber-300 bg-amber-50 dark:bg-amber-950/20">
					<AlertTriangle className="h-4 w-4 text-amber-600" />
					<AlertDescription className="text-sm text-amber-800 dark:text-amber-200">
						You have indicated one or more disclosures. Your account application
						may require additional review. Alpaca will contact you if further
	...
	const agreements = [
		{
			...
			description:
				"The Alpaca Customer Agreement governs your brokerage account relationship with Alpaca Securities LLC, including account opening, trading, and settlement terms.",
		...
		},
		{
			...
			description:
				"The Margin Agreement authorizes Alpaca to extend credit for securities purchases and governs your obligations with respect to margin balances.",
			url: "https://files.alpaca.markets/disclosures/library/AcctAppMarginAndCustAgmt.pdf",
		},
		{
			...
			description:
				"The Account Agreement covers account configuration, trading permissions, data sharing, and your rights as an account holder.",
			url: "https://files.alpaca.markets/disclosures/library/AcctAppMarginAndCustAgmt.pdf",
		},
	];
	...
	return (
		<div className="space-y-5">
			<div>
				...
				<p className="text-sm text-muted-foreground">
					Please read and agree to the following agreements required to open
					your Alpaca brokerage account.
				</p>
			</div>
			...
			<Alert className="border-blue-200 bg-blue-50 dark:bg-blue-950/20 py-3">
				...
				<AlertDescription className="text-xs text-blue-700 dark:text-blue-300">
					Your electronic signature will be recorded with a timestamp and your
					IP address as required by ESIGN Act and US regulations.
				</AlertDescription>
			</Alert>
			...
			<div className="space-y-3">
				{agreements.map((a) => (
					<Card
						key={a.field}
						className={`border transition-colors ${form[a.field] ? "border-green-300 bg-green-50 dark:bg-green-950/10" : ""}`}
					>
						<CardContent className="p-4">
							<div className="flex items-start gap-3">
								<Checkbox
			...
			<p className="text-xs text-muted-foreground">
				By checking the boxes above and submitting, you confirm that you have
				read, understood, and agree to be legally bound by these agreements.
				Your digital signature will be captured with the current timestamp.
			</p>
		</div>
	);
}
...
function ReviewStep({ form }: { form: FormState }) {
	...
	const incomeLabel =
		INCOME_OPTIONS.find((o) => o.min === form.annualIncomeMin)?.label || "—";
	const lwLabel =
		NW_OPTIONS.find((o) => o.min === form.liquidNetWorthMin)?.label || "—";
	const nwLabel =
		NW_OPTIONS.find((o) => o.min === form.totalNetWorthMin)?.label || "—";
	const fundingLabel =
		FUNDING_SOURCES.find((f) => f.value === form.fundingSource)?.label || "—";
	...
	return (
		<div className="space-y-5">
			<div>
				...
				<p className="text-sm text-muted-foreground">
					Please verify all information before submitting. Submitting sends your
					application to Alpaca for account creation.
				</p>
			</div>
			...
			<Card>
				...
				<CardContent className="pt-0">
					...
					<Row
						label="City / State"
						value={`${form.city}, ${form.state} ${form.postalCode}`}
					/>
				</CardContent>
			</Card>
			...
			<Card>
				<CardHeader className="pb-2">
					<CardTitle className="text-sm">Disclosures & Agreements</CardTitle>
				</CardHeader>
				<CardContent className="pt-0">
					<Row
						label="Control Person"
						value={form.isControlPerson ? "Yes" : "No"}
					/>
					<Row
						label="Exchange/FINRA Affiliated"
						value={form.isAffiliatedExchangeOrFinra ? "Yes" : "No"}
					/>
					<Row
						label="Politically Exposed"
						value={form.isPoliticallyExposed ? "Yes" : "No"}
					/>
					<Row
						label="Family PEP"
						value={form.immediateFamilyExposed ? "Yes" : "No"}
					/>
					<Row
						label="Customer Agreement"
						value={form.agreedCustomer ? "✓ Agreed" : "Not agreed"}
					/>
					<Row
						label="Margin Agreement"
						value={form.agreedMargin ? "✓ Agreed" : "Not agreed"}
					/>
					<Row
						label="Account Agreement"
						value={form.agreedAccount ? "✓ Agreed" : "Not agreed"}
					/>
				</CardContent>
			</Card>
			...
			<Alert className="border-amber-200 bg-amber-50 dark:bg-amber-950/20 py-3">
				...
				<AlertDescription className="text-xs text-amber-800 dark:text-amber-200">
					By submitting, you certify that all information provided is true and
					complete. False statements may result in account rejection or closure.
				</AlertDescription>
			</Alert>
		</div>
	);
	...
	return (
		<div className="text-center py-8 space-y-5">
			<div className="flex justify-center">
				<div
					className={`w-20 h-20 rounded-full flex items-center justify-center ${alpacaStatus === "ACTIVE" ? "bg-green-100 dark:bg-green-900/30" : "bg-blue-100 dark:bg-blue-900/30"}`}
				>
					{alpacaStatus === "ACTIVE" ? (
...
export default function OpenAccountPage() {
	...
	const [form, setForm] = useState<FormState>({
		...
		annualIncomeMax: "50000",
		liquidNetWorthMin: "25000",
		liquidNetWorthMax: "50000",
		totalNetWorthMin: "50000",
		totalNetWorthMax: "100000",
		isControlPerson: false,
		isAffiliatedExchangeOrFinra: false,
		isPoliticallyExposed: false,
		immediateFamilyExposed: false,
		agreedCustomer: false,
		agreedMargin: false,
		agreedAccount: false,
	});
		...
		if (data?.prefill) {
			...
			setForm((prev) => ({
				...
				isPoliticallyExposed: !!(
					p.pepStatus?.includes("pep") && !p.pepStatus?.includes("not")
				),
			}));
		}
	...
	const applyMutation = useMutation({
		mutationFn: () =>
			apiRequest("/api/us-trading/account/apply", {
				method: "POST",
				body: JSON.stringify({
					identity: {
		...
		onSuccess: (res: any) => {
			setSubmitted(true);
			setResult({
				alpacaStatus: res.alpacaStatus || "SUBMITTED",
				accountNumber: res.alpacaAccountNumber,
			});
			toast({
				title: "Account application submitted!",
				description: `Status: ${res.alpacaStatus}`,
			});
		},
		onError: (e: any) => {
			toast({
	...
	const canAdvance = () => {
		switch (stepIdx) {
			...
			case 1:
				return (
					form.fundingSource &&
					form.annualIncomeMin &&
					form.liquidNetWorthMin &&
					form.totalNetWorthMin
				);
			case 2:
				return true;
			case 3:
				return form.agreedCustomer && form.agreedMargin && form.agreedAccount;
			case 4:
				return true;
			default:
				return false;
			...
			<div className="mb-6">
				<div className="flex items-center gap-3 mb-2">
					...
					<div>
						...
						<p className="text-sm text-muted-foreground">
							Alpaca Broker-Dealer · FINRA Member
						</p>
					</div>
				</div>
			</div>
				...
				<>
						...
						<Alert className="border-red-200 bg-red-50 dark:bg-red-950/20 mb-5">
							...
							<AlertDescription className="text-sm text-red-700 dark:text-red-300">
								...
								<ul className="mt-1 list-disc list-inside">
									{compliance.blockers?.map((b: string) => (
										<li key={b}>{b}</li>
									))}
								</ul>
							</AlertDescription>
					...
					<Card>
						<CardContent className="p-6">
							...
							{stepIdx === 2 && (
								<DisclosuresStep form={form} setForm={setForm} />
							)}
							{stepIdx === 3 && (
								<AgreementsStep form={form} setForm={setForm} />
							)}
