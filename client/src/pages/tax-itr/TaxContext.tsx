...
interface TaxContextType {
  ...
  cgManualSaved: Array<{ id: number; assetType: string; summary: any; entryCount: number }>;
  setCgManualSaved: React.Dispatch<React.SetStateAction<any[]>>;
  ...
  taxCalcMutation: { mutate: (data: any) => void; isPending: boolean };
  regimeCompareMutation: { mutate: (data: any) => void; isPending: boolean; data: RegimeComparison | undefined };
}
...
export const TaxProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [cgManualSaved, setCgManualSaved] = useState<any[]>([]);
  ...
  const handleFetchAIS = async () => {
    ...
    try {
      ...
      if (res.salaryIncome) setSalaryDetails(prev => ({ ...prev, grossSalary: res.salaryIncome }));
      if (res.interestIncome) setOtherIncomeDetails(prev => ({ ...prev, interestIncome: res.interestIncome }));
      if (res.dividendIncome) setOtherIncomeDetails(prev => ({ ...prev, dividendIncome: res.dividendIncome }));
      toast({ title: "AIS Data Fetched", description: "Your income details have been updated from AIS." });
    } catch (err: any) {
      toast({ title: "AIS Fetch Failed", description: err.message, variant: "destructive" });
    } finally {
      setAisLoading(false);
    }
  };
  ...
  const totals = useMemo(() => {
    ...
    return { salaryIncome, hpIncome, capitalGains, otherIncome, businessIncome, grossTotalIncome, totalDeductions, totalTaxPaid };
  ...
  const computeCYLA = useMemo(() => {
    ...
    return { 
      adjustments, 
      totalIncomeAfterCYLA: adjustments.reduce((s, a) => s + a.incomeAfterSetOff, 0), 
  ...
  const computeBFLA = useMemo(() => {
    ...
    let remHP = lossCarryForward.filter(l => l.lossType === "house_property").reduce((s, l) => s + (l.lossAmount - l.setOffAmount), 0);
    let remBiz = lossCarryForward.filter(l => l.lossType === "business").reduce((s, l) => s + (l.lossAmount - l.setOffAmount), 0);
    ...
    return { bflaRows, totalIncomeAfterBFLA: bflaRows.reduce((s, r) => s + r.incomeAfterBFLA, 0), remainingHP: remHP, remainingBiz: remBiz };
