...
interface TaxContextType {
	...
	setHousePropertyDetails: React.Dispatch<
		React.SetStateAction<HousePropertyDetails>
	>;
	capitalGainsDetails: CapitalGainsDetails;
	setCapitalGainsDetails: React.Dispatch<
		React.SetStateAction<CapitalGainsDetails>
	>;
	...
	setForeignIncomeDetails: React.Dispatch<
		React.SetStateAction<ForeignIncomeDetails>
	>;
	...
	setOtherIncomeDetails: React.Dispatch<
		React.SetStateAction<OtherIncomeDetails>
	>;
	...
	setForeignTaxCredit: React.Dispatch<
		React.SetStateAction<ForeignTaxCreditDetails>
	>;
	...
	setSchedule112AEntries: React.Dispatch<
		React.SetStateAction<Schedule112AEntry[]>
	>;
	...
	setDepreciationEntries: React.Dispatch<
		React.SetStateAction<DepreciationEntry[]>
	>;
...
export const TaxProvider: React.FC<{ children: React.ReactNode }> = ({
	children,
}) => {
	...
	const [cgMode, setCgMode] = useState<"upload" | "manual" | "summary">(
		"upload",
	);
	...
	const [foreignIncomeDetails, setForeignIncomeDetails] =
		useState<ForeignIncomeDetails>({
			foreignSTCG: 0,
			foreignLTCG: 0,
			foreignDividends: 0,
			foreignInterest: 0,
	...
	const [foreignTaxCredit, setForeignTaxCredit] =
		useState<ForeignTaxCreditDetails>({ isApplicable: false });
	const [foreignAssets, setForeignAssets] = useState<ForeignAssetsDetails>({
		isApplicable: false,
		assets: [],
	});
	...
	const [otherIncomeDetails, setOtherIncomeDetails] =
		useState<OtherIncomeDetails>({
			interestIncome: 0,
			dividendIncome: 0,
			otherSources: 0,
	...
	const [schedule112AEntries, setSchedule112AEntries] = useState<
		Schedule112AEntry[]
	>([]);
	const [scheduleSI, setScheduleSI] = useState<ScheduleSIDetails>({
		...
		onlineGaming: 0,
		dtaaSpecialRate: 0,
		dtaaSpecialRatePercent: 10,
		otherSpecialRate: 0,
		otherSpecialRatePercent: 20,
	});
	const [scheduleEI, setScheduleEI] = useState<ScheduleEIDetails>({
		agriculturalIncome: 0,
		ltcgExemptUpTo125000: 0,
		dividendFromCooperative: 0,
		ppfInterest: 0,
		epfInterest: 0,
		section10Exemptions: 0,
		otherExemptIncome: 0,
		exemptIncomeDescription: "",
	});
	...
	const [schedule5A, setSchedule5A] = useState<Schedule5ADetails>({
		...
		nameOfSpouse: "",
		panOfSpouse: "",
		totalIncomeOfAssessee: 0,
		totalIncomeOfSpouse: 0,
		apportionedIncomeAssessee: 0,
		apportionedIncomeSpouse: 0,
		headwiseBreakdown: {
			salary: { assessee: 0, spouse: 0 },
			houseProperty: { assessee: 0, spouse: 0 },
			business: { assessee: 0, spouse: 0 },
			capitalGains: { assessee: 0, spouse: 0 },
	...
	const [matDetails, setMatDetails] = useState<MATDetails>({
		...
		additionsToBookProfit: {
			...
			transferToReserve: 0,
			provisionForDiminution: 0,
			expenditureRelatingExemptIncome: 0,
			notionalGain: 0,
			otherAdditions: 0,
		},
		deductionsFromBookProfit: {
			withdrawalFromReserve: 0,
			incomeExemptUs10: 0,
			incomeExemptUs11_12: 0,
			depreciationExcludingRevaluation: 0,
			withdrawalFromProvision: 0,
			lowerOfUnabsorbedDepOrBroughtForwardLoss: 0,
			notionalLoss: 0,
			otherDeductions: 0,
		},
		adjustedBookProfit: 0,
		matRate: 15,
		matTaxAmount: 0,
		surchargeOnMAT: 0,
		cessOnMAT: 0,
		totalMATLiability: 0,
		normalTaxLiability: 0,
		isMATApplicable: false,
		taxPayableHigherOfMATOrNormal: 0,
	});
	const [matcDetails, setMatcDetails] = useState<MATCreditDetails>({
		isApplicable: false,
		creditEntries: [],
		totalCreditBroughtForward: 0,
		creditUtilizedCurrentYear: 0,
	...
	const [amtDetails, setAmtDetails] = useState<AMTDetails>({
		...
		additions: {
			deduction80H_80RRB: 0,
			deduction10AA: 0,
			deduction35AD: 0,
			deduction80IA_80IB: 0,
			deduction80JJA: 0,
			deduction80P: 0,
			otherChapter6ADeductions: 0,
		},
		totalAdjustedIncome: 0,
		amtRate: 18.5,
		amtAmount: 0,
		surchargeOnAMT: 0,
		cessOnAMT: 0,
		totalAMTLiability: 0,
		normalTaxLiability: 0,
		isAMTApplicable: false,
		taxPayableHigherOfAMTOrNormal: 0,
	});
	...
	const [section87A, setSection87A] = useState<Section87ADetails>({
		...
		normalTaxLiability: 0,
		rebateAmount: 0,
		maxRebateOldRegime: 12500,
		maxRebateNewRegime: 25000,
		incomeThresholdOld: 500000,
		incomeThresholdNew: 700000,
		taxAfterRebate: 0,
	});
	...
	const [scheduleAL, setScheduleAL] = useState<ScheduleALDetails>({
		...
		vehiclesYachtsBoats: 0,
		totalAssets: 0,
		totalLiabilities: 0,
		liabilitiesRelatedToImmovable: 0,
		liabilitiesRelatedToOther: 0,
	});
	...
	const [itrUDetails, setItrUDetails] = useState<ItrUDetails>({
		originalAckNumber: "",
		originalFilingDate: "",
		reasonForUpdate: "income_not_reported",
	...
	const [challanResult, setChallanResult] = useState<ChallanResult | null>(
		null,
	);
	const [showToolsDialog, setShowToolsDialog] = useState(false);
	const [hraResult, setHraResult] = useState<HraResult | null>(null);
	const [form10EResult, setForm10EResult] = useState<Form10EResult | null>(
		null,
	);
	const [optimizerResult, setOptimizerResult] =
		useState<OptimizerResult | null>(null);
	const [preFilingResult, setPreFilingResult] =
		useState<PreFilingResult | null>(null);
	...
	const [regimeComparison, setRegimeComparison] =
		useState<RegimeComparison | null>(null);

	const [foreignAssetsLocal, setForeignAssetsLocal] =
		useState<ForeignAssetsDetails>({ isApplicable: false, assets: [] });

	const handleFetch26AS = async (): Promise<void> => {
		if (!panContext?.pan) return;
		setForm26ASLoading(true);
		try {
			const res = await apiRequest(`/api/tax/26as?pan=${panContext.pan}`, {
				method: "GET",
			});
			setTaxPaymentDetails((prev: TaxPaymentDetails) => ({
				...
				tdsDeducted:
					(res.tdsSalary || 0) + (res.tdsOther || 0) + (res.tdsProperty || 0),
				advanceTaxPaid: res.advanceTax || 0,
				selfAssessmentTax: res.selfAssessment || 0,
			}));
			toast({
				title: "Form 26AS Fetched",
				description: "Tax payments have been auto-filled.",
			});
		} catch (err: unknown) {
	...
	const [sandboxTaxResult, setSandboxTaxResult] =
		useState<SandboxTaxResult | null>(null);
	...
	const handleFetchAIS = async (): Promise<void> => {
		...
		setAisLoading(true);
		try {
			const res = await apiRequest(`/api/tax/ais?pan=${panContext.pan}`, {
				method: "GET",
			});
			setAisData({ ...res, loaded: true });
			if (res.salaryIncome)
				setSalaryDetails((prev: SalaryDetails) => ({
					...prev,
					grossSalary: res.salaryIncome,
				}));
			if (res.interestIncome)
				setOtherIncomeDetails((prev: OtherIncomeDetails) => ({
					...prev,
					interestIncome: res.interestIncome,
				}));
			if (res.dividendIncome)
				setOtherIncomeDetails((prev: OtherIncomeDetails) => ({
					...prev,
					dividendIncome: res.dividendIncome,
				}));
			toast({
				title: "AIS Data Fetched",
				description: "Your income details have been updated from AIS.",
			});
		} catch (err: unknown) {
			const message = err instanceof Error ? err.message : "Fetch failed";
			toast({
				title: "AIS Fetch Failed",
				description: message,
				variant: "destructive",
			});
		} finally {
			...
			if (data.parsed) {
				...
				if (data.parsed.tdsDeducted) {
					setTaxPaymentDetails(
						(prev: TaxPaymentDetails): TaxPaymentDetails => ({
							...prev,
							tdsDeducted: data.parsed.tdsDeducted,
						}),
					);
				}
				toast({
					title: "Form 16 Parsed",
					description: "Salary and TDS details auto-filled.",
				});
			}
		} catch {
			toast({
				title: "Upload Failed",
				description: "Could not parse Form 16.",
				variant: "destructive",
			});
		} finally {
	...
	const handleCgFileUpload = async (
		file: File,
		brokerId: string,
	): Promise<void> => {
		setCgUploading(true);
		try {
			...
			if (result.success) {
				...
				const uploadSTCG =
					cgUploads.reduce(
						(s: number, u: BrokerUploadInfo): number => s + u.summary.netSTCG,
						0,
					) + (result.summary?.netSTCG || 0);
				const uploadLTCG =
					cgUploads.reduce(
						(s: number, u: BrokerUploadInfo): number => s + u.summary.netLTCG,
						0,
					) + (result.summary?.netLTCG || 0);

				setCapitalGainsDetails(
					(prev: CapitalGainsDetails): CapitalGainsDetails => ({
						...prev,
						shortTermGains:
							uploadSTCG +
							cgManualSaved.reduce(
								(s: number, e: CGManualSavedItem): number =>
									s + e.summary.totalSTCG,
								0,
							),
						longTermGains:
							uploadLTCG +
	...
	const handleCgManualSave = async (): Promise<void> => {
		if (cgManualEntries.length === 0) {
			toast({
				title: "No entries",
				description: "Please add at least one transaction",
				variant: "destructive",
			});
			return;
		}
		try {
			...
			const result = await resp.json();
			if (result.success) {
				setCgManualSaved((prev: CGManualSavedItem[]) => [
					...prev,
					{
						id: result.entryId,
						assetType: cgManualAssetType,
				...
				const manualSTCG =
					cgManualSaved.reduce(
						(s: number, e: CGManualSavedItem) => s + e.summary.totalSTCG,
						0,
					) + (result.summary?.totalSTCG || 0);
				const manualLTCG =
					cgManualSaved.reduce(
						(s: number, e: CGManualSavedItem) => s + e.summary.totalLTCG,
						0,
					) + (result.summary?.totalLTCG || 0);

				setCapitalGainsDetails((prev: CapitalGainsDetails) => ({
					...prev,
					shortTermGains:
						cgUploads.reduce(
							(s: number, u: BrokerUploadInfo) => s + u.summary.netSTCG,
							0,
						) + manualSTCG,
					longTermGains:
						cgUploads.reduce(
							(s: number, u: BrokerUploadInfo) => s + u.summary.netLTCG,
							0,
						) + manualLTCG,
					exemptionsApplied:
						cgManualSaved.reduce(
							(s: number, e: CGManualSavedItem) =>
								s + e.summary.totalExemptions,
							0,
	...
	const totals = useMemo((): TaxTotals => {
		const salaryIncome = incomeSources.hasSalary
			? salaryDetails.grossSalary +
				salaryDetails.allowances +
				salaryDetails.perquisites +
				salaryDetails.profitInLieu -
				salaryDetails.standardDeduction -
		...
		if (incomeSources.hasHouseProperty) {
			housePropertyDetails.properties.forEach((prop: any): void => {
				if (prop.propertyType === "self_occupied")
					hpIncome += -Math.min(prop.interestOnLoan, 200000);
				else {
					const nav: number =
						prop.rentalIncome - prop.municipalTaxes - prop.unrealizedRent;
					hpIncome += nav * 0.7 - prop.interestOnLoan;
				}
			});
		}
		const capitalGains =
			capitalGainsDetails.shortTermGains +
			capitalGainsDetails.longTermGains -
			capitalGainsDetails.exemptionsApplied;
		const otherIncome =
			otherIncomeDetails.interestIncome +
			otherIncomeDetails.dividendIncome +
			otherIncomeDetails.otherSources;
		const businessIncome = incomeSources.hasBusinessIncome
		...
		const grossTotalIncome =
			Math.max(0, salaryIncome) +
			hpIncome +
			capitalGains +
			otherIncome +
			businessIncome;
		const totalDeductions =
			deductionDetails.section80C +
			deductionDetails.section80CCC +
			deductionDetails.section80CCD1 +
			deductionDetails.section80CCD1B +
			deductionDetails.section80CCD2 +
			deductionDetails.section80D +
			deductionDetails.section80DD +
			deductionDetails.section80DDB +
			deductionDetails.section80E +
			deductionDetails.section80EEA +
			deductionDetails.section80EEB +
			deductionDetails.section80G +
			deductionDetails.section80GG +
			deductionDetails.section80TTA +
			deductionDetails.section80TTB +
			deductionDetails.section80U +
		...
		return {
			...
			businessIncome,
			foreignCapitalGains:
				foreignIncomeDetails.foreignSTCG + foreignIncomeDetails.foreignLTCG,
			foreignOtherIncome:
	...
	const activeSteps = useMemo((): Step[] => {
		...
		if (
			["ITR-3", "ITR-5", "ITR-6"].includes(recommendedForm) &&
			incomeSources.hasBusinessIncome
		)
			stepIds.push("financials");
		...
		if (recommendedForm !== "ITR-1" && totals.grossTotalIncome > 5000000)
			stepIds.push("schedule_al");
		if (incomeSources.hasCapitalGains || incomeSources.hasBusinessIncome)
			stepIds.push("loss_adjustment");
		if (["ITR-2", "ITR-3", "ITR-5", "ITR-6"].includes(recommendedForm))
			stepIds.push("schedule_si_ei");
		...
		(stepId: string): StepValidation => {
			...
			switch (stepId) {
				case "basic":
					if (!panContext?.pan)
						errors.push(
							"PAN details are required. Please ensure your PAN is linked.",
						);
					break;
				case "sources":
					if (
					...
					if (incomeSources.hasBusinessIncome) {
						warnings.push(
							"Business income requires ITR-3/4. Ensure you have your P&L and Balance Sheet ready.",
						);
					}
					if (incomeSources.hasForeignIncome) {
						warnings.push(
							"Foreign income requires ITR-2 or higher and may need Schedule FA (Foreign Assets).",
						);
					}
					break;
				case "salary":
					if (incomeSources.hasSalary && salaryDetails.grossSalary <= 0) {
						errors.push(
							"Please enter your gross salary. You can find this in your Form 16 Part B.",
						);
					}
					if (salaryDetails.professionalTax > 2500) {
						warnings.push(
							"Professional Tax is typically capped at ₹2,500/year in most states.",
						);
					}
					if (
						salaryDetails.grossSalary > 0 &&
						salaryDetails.allowances > salaryDetails.grossSalary
					) {
				...
				case "property":
					...
					if (
						housePropertyDetails.isSelfOccupied &&
						housePropertyDetails.interestOnLoan > 200000
					) {
						warnings.push(
							"For self-occupied property, home loan interest deduction is capped at ₹2,0,000.",
						);
					}
					break;
				case "business":
						...
						if (businessDetails.isPresumptive) {
							if (businessDetails.businessType === "business") {
								if (businessDetails.grossTurnover <= 0)
									errors.push(
										"Please enter gross turnover for presumptive income under Section 44AD.",
									);
								if (businessDetails.grossTurnover > 30000000)
									errors.push("Turnover exceeds ₹3 Cr limit for Section 44AD.");
								const minDeemed = Math.round(
									businessDetails.grossTurnover * 0.06,
								);
								if (
									businessDetails.presumptiveIncome44AD > 0 &&
									businessDetails.presumptiveIncome44AD < minDeemed
								) {
									errors.push(
										`Deemed profit cannot be less than 6% of turnover (₹${minDeemed.toLocaleString("en-IN")}).`,
									);
								}
							}
							if (businessDetails.businessType === "profession") {
								if (businessDetails.grossReceipts <= 0)
									errors.push("Please enter gross receipts for Section 44ADA.");
								if (businessDetails.grossReceipts > 7500000)
									errors.push("Gross receipts exceed ₹75 lakhs limit.");
								const minDeemed = Math.round(
									businessDetails.grossReceipts * 0.5,
								);
								if (
									businessDetails.presumptiveIncome44ADA > 0 &&
									businessDetails.presumptiveIncome44ADA < minDeemed
								) {
									errors.push(
										`Deemed profit cannot be less than 50% (₹${minDeemed.toLocaleString("en-IN")}).`,
									);
								}
							}
						}
				...
				case "deductions":
					if (taxRegime === "new") {
						warnings.push(
							"Under the New Tax Regime, most Chapter VI-A deductions are not available.",
						);
					}
					if (
						deductionDetails.section80TTA > 0 &&
						deductionDetails.section80TTB > 0
					) {
				...
				case "review":
					if (
						sandboxTaxResult?.data?.refundAmount &&
						sandboxTaxResult.data.refundAmount > 0 &&
	...
	const currentValidation = useMemo(
		...
		[currentStepId, validateStep],
	);

	// Mutations
	const saveDraftMutation = useMutation({
		mutationFn: (draft: Partial<ITRDraft>) =>
			apiRequest("/api/tax/itr/draft", {
				method: "POST",
		...
		mutationFn: async () => {
			...
			housePropertyDetails.properties.forEach((prop: any): void => {
				if (prop.propertyType === "self_occupied") {
					...
					const nav: number =
						prop.rentalIncome - prop.municipalTaxes - prop.unrealizedRent;
					housePropertyIncome += nav * 0.7 - prop.interestOnLoan;
				}
			});
	...
	const saveDraft = (): void => {
		saveDraftMutation.mutate({
			...
			profitLoss,
			grossTotalIncome:
				sandboxTaxResult?.data?.totalIncome ?? totals.grossTotalIncome,
	...
	const computeCYLA = useMemo((): CYLAData => {
		const salary = incomeSources.hasSalary
			? salaryDetails.grossSalary +
				salaryDetails.allowances +
				salaryDetails.perquisites +
				salaryDetails.profitInLieu -
		...
		if (incomeSources.hasHouseProperty) {
			housePropertyDetails.properties.forEach((prop: any): void => {
				if (prop.propertyType === "self_occupied")
					hp += -Math.min(prop.interestOnLoan, 200000);
				else
					hp +=
						(prop.rentalIncome - prop.municipalTaxes - prop.unrealizedRent) *
							0.7 -
		...
		const business = incomeSources.hasBusinessIncome
			? businessDetails.isPresumptive
				? businessDetails.presumptiveIncome44AD +
					businessDetails.presumptiveIncome44ADA +
					businessDetails.presumptiveIncome44AE
		...
		for (const h of heads) {
			...
			adjustments.push({
				head: h.head,
				incomeBeforeSetOff: h.income,
				hpLossSetOff: hpUsed,
				businessLossSetOff: bizUsed,
				otherSourceLossSetOff: 0,
				incomeAfterSetOff: available,
			});
		}
		return {
			adjustments,
			totalIncomeAfterCYLA: adjustments.reduce(
				(s: number, a: CYLAAdjustment): number => s + a.incomeAfterSetOff,
				0,
			),
	...
	const computeBFLA = useMemo((): BFLAData => {
		...
		const bflaRows: BFLAAdjustment[] = cyla.adjustments.map(
			(a: CYLAAdjustment) => ({
				head: a.head,
				incomeAfterCYLA: a.incomeAfterSetOff,
				bfHPLossSetOff: 0,
				bfSTCLSetOff: 0,
				bfLTCLSetOff: 0,
				bfBusinessLossSetOff: 0,
				bfSpeculationSetOff: 0,
				incomeAfterBFLA: a.incomeAfterSetOff,
			}),
		);
		let remHP = lossCarryForward
			.filter((l: LossCarryForward): boolean => l.lossType === "house_property")
			.reduce(
				(s: number, l: LossCarryForward): number =>
					s + (l.lossAmount - l.setOffAmount),
				0,
			);
		let remBiz = lossCarryForward
			.filter((l: LossCarryForward): boolean => l.lossType === "business")
			.reduce(
				(s: number, l: LossCarryForward): number =>
					s + (l.lossAmount - l.setOffAmount),
				0,
		...
		for (const row of bflaRows) {
			...
			if (remHP > 0 && avail > 0) {
				const u = Math.min(remHP, avail);
				row.bfHPLossSetOff = u;
				remHP -= u;
				avail -= u;
			}
			if (remBiz > 0 && avail > 0 && row.head !== "salary") {
				const u = Math.min(remBiz, avail);
				row.bfBusinessLossSetOff = u;
				remBiz -= u;
				avail -= u;
			}
		...
		return {
			bflaRows,
			totalIncomeAfterBFLA: bflaRows.reduce(
				(s: number, r: BFLAAdjustment): number => s + r.incomeAfterBFLA,
				0,
			),
			remainingHP: remHP,
			remainingBiz: remBiz,
	...
	const computeCFL = useMemo((): CFLEntry[] => {
		...
		if (cyla.unabsorbedHPLoss > 0 || cyla.unabsorbedBizLoss > 0) {
			entries.push({
				...
				housePropertyLoss: cyla.unabsorbedHPLoss,
				shortTermCapitalLoss: 0,
				longTermCapitalLoss: 0,
				businessLoss: cyla.unabsorbedBizLoss,
				speculativeBusinessLoss: 0,
				specifiedBusinessLoss: 0,
			});
		}
		return entries;
	...
	const compute234Interest = useCallback((): void => {
		...
		const assessedTax = Math.max(
			0,
			taxLiability - taxPaymentDetails.reliefUs89,
		);
	...
	const value: TaxContextType = {
		...
		itrUDetails,
		setItrUDetails,
		isEntityForm: ["ITR-5", "ITR-6", "ITR-7"].includes(recommendedForm),
		aisLoading,
		aisData,
		setAisData,
		form16Uploading,
		...
		safeCurrentStep: Math.max(
			0,
			activeSteps.findIndex((s) => s.id === currentStepId),
		),
		progress:
			((activeSteps.findIndex((s) => s.id === currentStepId) + 1) /
				activeSteps.length) *
			100,
		currentValidation,
		cyla: computeCYLA,
		...
		validateStep,
		saveDraft,
		calculateTax: () => taxCalcMutation.mutate(),
		nextStep,
		prevStep,
		goToStep,
		handleFetchAIS,
