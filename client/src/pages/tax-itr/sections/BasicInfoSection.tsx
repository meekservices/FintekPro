            ...
            <div>
              ...
              <Input type="file" accept=".json" className="text-xs" onChange={async (e: React.ChangeEvent<HTMLInputElement>) => {
                ...
                try {
                  ...
                  if (result.success) {
                    const d = result.data;
                    if (d.salary?.grossSalary) setSalaryDetails((p: SalaryDetails) => ({ ...p, grossSalary: d.salary.grossSalary, standardDeduction: d.salary.standardDeduction || 75000 }));
                    if (d.capitalGains?.stcg || d.capitalGains?.ltcg) setCapitalGainsDetails((p: CapitalGainsDetails) => ({ ...p, shortTermGains: d.capitalGains.stcg, longTermGains: d.capitalGains.ltcg }));
                    if (d.otherSources?.interestIncome) setOtherIncomeDetails((p: OtherIncomeDetails) => ({ ...p, interestIncome: d.otherSources.interestIncome, dividendIncome: d.otherSources.dividendIncome }));
                    if (d.deductions?.section80C) setDeductionDetails((p: DeductionDetails) => ({ ...p, section80C: d.deductions.section80C, section80D: d.deductions.section80D || 0 }));
            ...
            <div>
              ...
              <Input type="file" accept=".json" className="text-xs" onChange={async (e: React.ChangeEvent<HTMLInputElement>) => {
                ...
                try {
                  ...
                  if (result.success) {
                    const d = result.data;
                    if (d.interestIncome > 0) setOtherIncomeDetails((p: OtherIncomeDetails) => ({ ...p, interestIncome: p.interestIncome + d.interestIncome }));
                    if (d.dividendIncome > 0) setOtherIncomeDetails((p: OtherIncomeDetails) => ({ ...p, dividendIncome: p.dividendIncome + d.dividendIncome }));
                    if (d.salaryIncome > 0) setSalaryDetails((p: SalaryDetails) => ({ ...p, grossSalary: d.salaryIncome }));
            ...
            <div>
              ...
              <Input type="file" accept=".csv,.xlsx" className="text-xs" onChange={async (e: React.ChangeEvent<HTMLInputElement>) => {
                ...
                try {
                  ...
                  const result = await resp.json();
                  if (result.success) {
                    setCapitalGainsDetails((p: CapitalGainsDetails) => ({ ...p, shortTermGains: p.shortTermGains + result.data.totalSTCG, longTermGains: p.longTermGains + result.data.totalLTCG }));
