...
export const TDSSchedulesSection: React.FC = () => {
  const {
    tds1Entries,
    setTds1Entries,
    tds2Entries,
  ...
  const totalTDS1 = tds1Entries.reduce((s: number, e: TDS1Entry) => s + e.tdsDeducted, 0);
  const totalTDS2 = tds2Entries.reduce((s: number, e: TDS2Entry) => s + e.tdsDeducted, 0);
  const totalTCS = tcsEntries.reduce((s: number, e: TCSEntry) => s + e.tcsCollected, 0);
  ...
  const compute234F = () => {
    ...
    if (filingDate <= dueDate) {
      setSection234F((p: Section234FDetails) => ({ ...p, lateFee: 0, isApplicable: false }));
      return;
    }
    const totalIncome = section234F.totalIncome;
    const isSmall = totalIncome <= 500000;
    setSection234F((p: Section234FDetails) => ({ ...p, lateFee: totalIncome <= 0 ? 0 : isSmall ? 1000 : 5000, isApplicable: true, isSmallTaxpayer: isSmall }));
  };
  ...
  const compute87A = () => {
    ...
    setSection87A((p: Section87ADetails) => ({ ...p, isEligible, rebateAmount: rebate, taxAfterRebate: Math.max(0, p.normalTaxLiability - rebate) }));
  };
        ...
        <TabsContent value="tds1">
          <Card>
            ...
            <CardContent className="space-y-4">
                  ...
                  <Card key={idx} className="border-dashed">
                    <CardContent className="pt-4 space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        <Button variant="ghost" size="sm" onClick={() => setTds1Entries((prev: TDS1Entry[]) => prev.filter((_: TDS1Entry, i: number) => i !== idx))}>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                          ...
                          <Input value={entry.employerTAN} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setTds1Entries((p: TDS1Entry[]) => p.map((x: TDS1Entry, i: number) => i === idx ? { ...x, employerTAN: e.target.value.toUpperCase() } : x))} placeholder="AAAA00000A" maxLength={10} />
                        </div>
                        <div>
                          <Label className="text-xs">Employer Name</Label>
                          <Input value={entry.employerName} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setTds1Entries((p: TDS1Entry[]) => p.map((x: TDS1Entry, i: number) => i === idx ? { ...x, employerName: e.target.value } : x))} />
                        </div>
                        <div>
                          <Label className="text-xs">Income Credited (₹)</Label>
                          <CurrencyInput id={`tds1-income-${idx}`} value={entry.incomeCredited} onChange={(v: number) => setTds1Entries((p: TDS1Entry[]) => p.map((x: TDS1Entry, i: number) => i === idx ? { ...x, incomeCredited: v } : x))} />
                        </div>
                        <div>
                          <Label className="text-xs">TDS Deducted (₹)</Label>
                          <CurrencyInput id={`tds1-tds-${idx}`} value={entry.tdsDeducted} onChange={(v: number) => setTds1Entries((p: TDS1Entry[]) => p.map((x: TDS1Entry, i: number) => i === idx ? { ...x, tdsDeducted: v } : x))} />
                        </div>
                      </div>
                    </CardContent>
                  </Card>
              ...
              <Button variant="outline" size="sm" onClick={() => setTds1Entries((p: TDS1Entry[]) => [...p, { employerTAN: "", employerName: "", salaryUnderSection: "17(1)" as const, incomeCredited: 0, tdsDeducted: 0, tdsClaimedCurrentYear: 0 }])}>
        ...
        <TabsContent value="tds2">
          <Card>
            ...
            <CardContent className="space-y-4">
                  ...
                  <Card key={idx} className="border-dashed">
                    <CardContent className="pt-4 space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        <Button variant="ghost" size="sm" onClick={() => setTds2Entries((prev: TDS2Entry[]) => prev.filter((_: TDS2Entry, i: number) => i !== idx))}>
                          <XCircle className="h-4 w-4 text-red-500" />
                        </Button>
                      </div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                          ...
                          <Input value={entry.deductorTAN} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setTds2Entries((p: TDS2Entry[]) => p.map((x: TDS2Entry, i: number) => i === idx ? { ...x, deductorTAN: e.target.value.toUpperCase() } : x))} placeholder="AAAA00000A" maxLength={10} />
                        </div>
                        <div>
                          <Label className="text-xs">Deductor Name</Label>
                          <Input value={entry.deductorName} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setTds2Entries((p: TDS2Entry[]) => p.map((x: TDS2Entry, i: number) => i === idx ? { ...x, deductorName: e.target.value } : x))} />
                        </div>
                        <div>
                          <Label className="text-xs">Income Type</Label>
                          <Select value={entry.incomeType} onValueChange={(v: string) => setTds2Entries((p: TDS2Entry[]) => p.map((x: TDS2Entry, i: number) => i === idx ? { ...x, incomeType: v as any } : x))}>
                        ...
                        <div>
                          ...
                          <Input value={entry.section} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setTds2Entries((p: TDS2Entry[]) => p.map((x: TDS2Entry, i: number) => i === idx ? { ...x, section: e.target.value } : x))} placeholder="e.g. 194A" />
                        </div>
                        <div>
                          <Label className="text-xs">Income Credited (₹)</Label>
                          <CurrencyInput id={`tds2-income-${idx}`} value={entry.incomeCredited} onChange={(v: number) => setTds2Entries((p: TDS2Entry[]) => p.map((x: TDS2Entry, i: number) => i === idx ? { ...x, incomeCredited: v } : x))} />
                        </div>
                        <div>
                          <Label className="text-xs">TDS Deducted (₹)</Label>
                          <CurrencyInput id={`tds2-tds-${idx}`} value={entry.tdsDeducted} onChange={(v: number) => setTds2Entries((p: TDS2Entry[]) => p.map((x: TDS2Entry, i: number) => i === idx ? { ...x, tdsDeducted: v } : x))} />
                        </div>
                      </div>
                    </CardContent>
                  </Card>
              ...
              <Button variant="outline" size="sm" onClick={() => setTds2Entries((p: TDS2Entry[]) => [...p, { deductorTAN: "", deductorName: "", incomeType: "interest" as const, section: "194A", dateOfPayment: "", incomeCredited: 0, tdsDeducted: 0, tdsClaimedCurrentYear: 0 }])}>
        ...
        <TabsContent value="tcs">
          <Card>
            ...
            <CardContent className="space-y-4">
                  ...
                  <Card key={idx} className="border-dashed">
                    <CardContent className="pt-4 space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        <Button variant="ghost" size="sm" onClick={() => setTcsEntries((prev: TCSEntry[]) => prev.filter((_: TCSEntry, i: number) => i !== idx))}>
                          <XCircle className="h-4 w-4 text-red-500" />
                        </Button>
                      </div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                          ...
                          <Input value={entry.collectorTAN} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setTcsEntries((p: TCSEntry[]) => p.map((x: TCSEntry, i: number) => i === idx ? { ...x, collectorTAN: e.target.value.toUpperCase() } : x))} />
                        </div>
                        <div>
                          <Label className="text-xs">Collector Name</Label>
                          <Input value={entry.collectorName} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setTcsEntries((p: TCSEntry[]) => p.map((x: TCSEntry, i: number) => i === idx ? { ...x, collectorName: e.target.value } : x))} />
                        </div>
                        <div>
                          <Label className="text-xs">Amount Paid (₹)</Label>
                          <CurrencyInput id={`tcs-amt-${idx}`} value={entry.amountPaid} onChange={(v: number) => setTcsEntries((p: TCSEntry[]) => p.map((x: TCSEntry, i: number) => i === idx ? { ...x, amountPaid: v } : x))} />
                        </div>
                        <div>
                          <Label className="text-xs">TCS Collected (₹)</Label>
                          <CurrencyInput id={`tcs-tcs-${idx}`} value={entry.tcsCollected} onChange={(v: number) => setTcsEntries((p: TCSEntry[]) => p.map((x: TCSEntry, i: number) => i === idx ? { ...x, tcsCollected: v } : x))} />
                        </div>
                      </div>
                    </CardContent>
                  </Card>
              ...
              <Button variant="outline" size="sm" onClick={() => setTcsEntries((p: TCSEntry[]) => [...p, { collectorTAN: "", collectorName: "", amountPaid: 0, tcsCollected: 0, tcsClaimedCurrentYear: 0 }])}>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <Input type="date" value={section234F.filingDueDate} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSection234F((p: Section234FDetails) => ({ ...p, filingDueDate: e.target.value }))} />
                </div>
                <div>
                  <Label className="text-xs">Actual Filing Date</Label>
                  <Input type="date" value={section234F.actualFilingDate} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSection234F((p: Section234FDetails) => ({ ...p, actualFilingDate: e.target.value }))} />
                </div>
                <div>
                  <Label className="text-xs">Total Income (₹)</Label>
                  <CurrencyInput id="s234f-income" value={section234F.totalIncome} onChange={(v: number) => setSection234F((p: Section234FDetails) => ({ ...p, totalIncome: v }))} />
                </div>
              </div>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <CurrencyInput id="s87a-income" value={section87A.taxableIncome} onChange={(v: number) => setSection87A((p: Section87ADetails) => ({ ...p, taxableIncome: v }))} />
                </div>
                <div>
                  <Label className="text-xs">Normal Tax Liability (₹)</Label>
                  <CurrencyInput id="s87a-tax" value={section87A.normalTaxLiability} onChange={(v: number) => setSection87A((p: Section87ADetails) => ({ ...p, normalTaxLiability: v }))} />
                </div>
              </div>
