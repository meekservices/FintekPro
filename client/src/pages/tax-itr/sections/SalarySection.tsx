		...
		<div className="space-y-6">
			<div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
				<p className="text-muted-foreground text-sm">
					Enter details from your Form 16 Part B, or upload it for auto-fill.
				</p>
				<div className="flex gap-2 flex-wrap">
					<Button
						variant="outline"
						size="sm"
						onClick={handleFetchAIS}
						disabled={aisLoading || !panContext?.pan}
						data-testid="btn-fetch-ais"
					>
					...
					<label htmlFor="form16-upload" className="cursor-pointer">
						<div
							className={`inline-flex items-center gap-2 px-4 py-2 rounded-md border text-sm font-medium transition-colors ${form16Uploading ? "opacity-50 cursor-wait" : "hover:bg-accent"}`}
						>
			...
			{aisData?.loaded && (
				<Alert className="bg-green-50 dark:bg-green-950 border-green-200">
					...
					<AlertDescription className="text-xs">
						<strong>AIS data pre-filled:</strong>{" "}
						{(aisData.salaryIncome ?? 0) > 0
							? `Salary ₹${aisData.salaryIncome?.toLocaleString("en-IN")}`
							: ""}
						{(aisData.interestIncome ?? 0) > 0
							? ` | Interest ₹${aisData.interestIncome?.toLocaleString("en-IN")}`
							: ""}
						{(aisData.dividendIncome ?? 0) > 0
							? ` | Dividends ₹${aisData.dividendIncome?.toLocaleString("en-IN")}`
							: ""}
						{Number(aisData.tdsEntries ?? 0) > 0
							? ` | ${aisData.tdsEntries} TDS entries`
							: ""}
						{Number(aisData.saleTransactions ?? 0) > 0
							? ` | ${aisData.saleTransactions} sale transactions`
							: ""}
					</AlertDescription>
				</Alert>
			)}
			...
			<Card className="bg-blue-50/50 dark:bg-blue-950/30 border-blue-200 dark:border-blue-800">
				<CardContent className="p-4 space-y-3">
					<p className="text-xs font-medium text-blue-700 dark:text-blue-300">
						Employer Details (as per Form 16 Part A)
					</p>
					<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
						<div className="space-y-1.5">
							<Label htmlFor="employerName" className="text-xs">
								Employer Name
							</Label>
							<Input
								id="employerName"
								...
								onChange={(e: React.ChangeEvent<HTMLInputElement>): void =>
									setEmployerDetails((prev: EmployerDetails) => ({
										...prev,
						...
						<div className="space-y-1.5">
							<Label htmlFor="employerTAN" className="text-xs">
								Employer TAN{" "}
								<FieldHint text="Tax Deduction Account Number of employer. 10-character alphanumeric. Found on Form 16 Part A." />
							</Label>
							<Input
								id="employerTAN"
				...
				<CardContent className="p-4 space-y-2">
					<div className="flex justify-between items-center text-sm">
						<span className="text-muted-foreground">
							Standard Deduction (Auto-applied for AY 2025-26)
						</span>
						<span className="font-medium">
							{formatCurrency(salaryDetails.standardDeduction)}
						</span>
					</div>
					<Separator />
					<div className="flex justify-between items-center">
						...
						<span className="font-bold text-lg">
							{formatCurrency(Math.max(0, salaryIncomeTotal))}
						</span>
					</div>
				</CardContent>
			</Card>
