	...
	const sources: SourceItem[] = [
		{
			...
			icon: Briefcase,
			desc: "Income from employment, Form 16",
			color: "text-blue-600",
		},
		{
		...
		},
		{
			...
			label: "Capital Gains",
			icon: TrendingUp,
			desc: "Stocks, MFs, property sale, STT/non-STT split",
		...
		},
		{
			...
			icon: Building2,
			desc: "Self-employed, freelancer, F&O, business P&L",
			color: "text-orange-600",
		},
		{
			key: "hasForeignIncome",
			label: "Foreign Income / Global Stocks",
			icon: Globe,
			desc: "US/global stocks, DTAA relief, Schedule FA & FSI",
			color: "text-red-600",
		},
		{
			key: "hasOtherIncome",
			label: "Other Sources",
			icon: Wallet,
			desc: "FD/savings interest, dividends, lottery, gaming",
			color: "text-teal-600",
		},
	];
	...
	return (
		<div className="space-y-6">
			<p className="text-muted-foreground text-sm">
				Select all sources of income for FY{" "}
				{assessmentYear === "2026-27"
					...
					: assessmentYear === "2025-26"
						...
						: assessmentYear === "2024-25"
							? "2023-24"
							: "2022-23"}
				. The system will automatically select the correct ITR form — schedules
				adjust internally.
			</p>
				...
				{sources.map((source: SourceItem): React.ReactElement => {
					...
					return (
						<Card
							...
							onClick={(): void =>
								setIncomeSources(
									(prev: IncomeSource): IncomeSource => ({
										...prev,
					...
					<div className="flex items-center gap-3">
						...
						<div className="flex-1">
							...
							<p className="text-xs text-muted-foreground">
								Based on your PAN type and income sources. Steps and schedules
								adjust automatically.
							</p>
						</div>
					</div>
					<div className="flex flex-wrap gap-1.5">
						{((formScheduleMap[recommendedForm] as string[]) || []).map(
							(sch: string, i: number): React.ReactElement => (
					...
					{recommendedForm !== "ITR-1" && (
						<div className="text-xs text-muted-foreground border-t pt-2 mt-2">
							...
							{incomeSources.hasForeignIncome &&
								"Foreign income/assets require ITR-2+. "}
							{incomeSources.hasBusinessIncome &&
								"Business income requires ITR-3/4. "}
							{residentialStatus !== "resident" &&
								"NRI/RNOR status requires ITR-2+. "}
							{housePropertyDetails.propertyCount > 1 &&
								"Multiple properties require ITR-2+. "}
							{isEntityForm &&
								`Entity type (${panContext?.panType}) requires ${recommendedForm}. `}
						</div>
					)}
				</CardContent>
			</Card>
