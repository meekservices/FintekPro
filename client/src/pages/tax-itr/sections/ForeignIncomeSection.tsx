...
export const ForeignIncomeSection: React.FC = (): React.ReactElement => {
	...
	const foreignTotalInINR =
		...
		(foreignIncomeDetails.currencyCode === "INR"
			? 1
			: foreignIncomeDetails.exchangeRate);

	const addForeignAsset = (): void => {
		setForeignAssets(
			(prev: ForeignAssetsDetails): ForeignAssetsDetails => ({
				...prev,
				assets: [
					...prev.assets,
					{
		...
		setForeignAssets(
			(prev: ForeignAssetsDetails): ForeignAssetsDetails => ({
				...
				assets: prev.assets.filter(
					(_asset: ForeignAssetEntry, i: number): boolean => i !== idx,
				),
			}),
		);
	};

	const updateForeignAsset = <K extends keyof ForeignAssetEntry>(
		idx: number,
		field: K,
		value: ForeignAssetEntry[K],
	): void => {
		...
		<div className="space-y-6">
			<Alert className="bg-blue-50 dark:bg-blue-950 border-blue-200 dark:border-blue-800">
				...
				<AlertDescription className="text-blue-700 dark:text-blue-300">
					<strong>Global Stock Investments</strong> — Report all foreign income
					in INR (converted at RBI reference rate on the date of credit/sale).
					Schedule FA disclosure is mandatory under the Black Money Act, 2015.
					Non-disclosure attracts ₹10 lakh penalty.
				</AlertDescription>
			</Alert>
			...
			<Card>
				<CardHeader className="pb-3">
					...
					<CardDescription>
						Set your primary investment country and currency for
						auto-conversion.
					</CardDescription>
				</CardHeader>
				<CardContent>
					<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
						<div className="space-y-1.5">
							...
							<Select
								value={foreignIncomeDetails.dtaaCountry}
								onValueChange={(v: string): void => {
									const country = DTAA_COUNTRIES.find(
										(c: { code: string; name: string; article: string }) =>
											c.code === v,
									);
									setForeignIncomeDetails(
										(prev: ForeignIncomeDetails): ForeignIncomeDetails => ({
											...prev,
											dtaaCountry: v,
						...
						<div className="space-y-1.5">
							...
							<Select
								value={foreignIncomeDetails.currencyCode}
								onValueChange={(v: string): void => {
									const cur = CURRENCY_CODES.find(
										(c: {
											code: string;
											symbol: string;
											name: string;
						...
						<div className="space-y-1.5">
							...
							<Input
								...
								onChange={(e: React.ChangeEvent<HTMLInputElement>): void =>
									setForeignIncomeDetails(
										(prev: ForeignIncomeDetails): ForeignIncomeDetails => ({
											...prev,
											exchangeRate: Number.parseFloat(e.target.value) || 0,
										}),
									)
								}
							...
							<p className="text-xs text-muted-foreground">
								Pre-filled approximate rate. Verify from RBI/SBI for actual
								filing.
							</p>
						</div>
					</div>
			...
			<Card>
				<CardHeader className="pb-3">
					<CardTitle className="text-sm flex items-center gap-2">
						<TrendingUp className="h-4 w-4" /> Foreign Capital Gains (Schedule
						CG)
					</CardTitle>
					<CardDescription>
						Enter capital gains from global stocks, ETFs, or other foreign
						assets. Enter amounts already converted to INR.
					</CardDescription>
				</CardHeader>
				<CardContent>
					...
					<div className="mt-3 p-3 rounded-md bg-amber-50 dark:bg-amber-950 border border-amber-200 dark:border-amber-800">
						<p className="text-xs text-amber-700 dark:text-amber-300 flex items-center gap-1">
							...
							Foreign equity holding period for LTCG is 24 months (not 12 months
							like Indian listed equity). Also, STT-based concessional rates
							(15%/10%) do NOT apply to foreign stocks.
						</p>
					</div>
				</CardContent>
			</Card>
			...
			<Card>
				<CardHeader className="pb-3">
					<CardTitle className="text-sm flex items-center gap-2">
						<Receipt className="h-4 w-4" /> Foreign Income — Other Heads
						(Schedule FSI)
					</CardTitle>
					<CardDescription>
						Report dividends, interest, and other income earned from foreign
						sources. Report amounts in INR.
					</CardDescription>
				</CardHeader>
				<CardContent>
			...
			<Card className="border-green-200 dark:border-green-800">
				<CardHeader className="pb-3">
					<CardTitle className="text-sm flex items-center gap-2">
						<Scale className="h-4 w-4 text-green-600" /> Foreign Tax Credit —
						DTAA Relief (Schedule TR)
					</CardTitle>
					<CardDescription>
						Claim credit for taxes already paid in the foreign country to avoid
						double taxation. You must file Form 67 before filing ITR.
					</CardDescription>
				</CardHeader>
				<CardContent>
					<div className="flex items-center gap-3 mb-4">
						<Switch
							checked={foreignTaxCredit.isApplicable}
							onCheckedChange={(v: boolean): void =>
								setForeignTaxCredit(
									(p: ForeignTaxCreditDetails): ForeignTaxCreditDetails => ({
										...p,
										isApplicable: v,
									}),
								)
						...
						<Label className="text-sm">
							I have paid tax in a foreign country and want to claim credit (Sec
							90/90A/91)
						</Label>
					</div>
					{foreignTaxCredit.isApplicable && (
						<div className="grid grid-cols-1 md:grid-cols-2 gap-5">
							...
							<div className="space-y-1.5">
								...
								<Input
									value={
										foreignIncomeDetails.dtaaArticle ||
										DTAA_COUNTRIES.find(
											(c: { code: string; name: string; article: string }) =>
												c.code === foreignIncomeDetails.dtaaCountry,
										)?.article ||
										""
									}
									onChange={(e: React.ChangeEvent<HTMLInputElement>): void =>
					...
					{foreignIncomeDetails.foreignTaxPaid > 0 && (
						<Alert className="mt-3 bg-green-50 dark:bg-green-950 border-green-200 dark:border-green-800">
							...
							<AlertDescription className="text-green-700 dark:text-green-300 text-sm">
								FTC of {formatCurrency(foreignIncomeDetails.foreignTaxPaid)}{" "}
								will be claimed under Section 90/91. Remember to file{" "}
								<strong>Form 67</strong> before your ITR filing date — FTC is
								not allowed without it.
							</AlertDescription>
						</Alert>
					)}
				</CardContent>
			...
			<Card className="border-red-200 dark:border-red-800">
				<CardHeader className="pb-3">
					<CardTitle className="text-sm flex items-center gap-2">
						<LucideShield className="h-4 w-4 text-red-600" /> Schedule FA —
					...
					<CardDescription>
						Mandatory for Resident & Ordinarily Resident (ROR) Indians. Disclose
						ALL foreign assets — even zero-balance accounts, dormant accounts,
						or assets held for even 1 day during the calendar year (Jan 1 – Dec
						31).
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-4">
					<div className="flex items-center gap-3 mb-4">
						<Switch
							checked={foreignAssets.isApplicable}
							onCheckedChange={(v: boolean): void =>
								setForeignAssets(
									(p: ForeignAssetsDetails): ForeignAssetsDetails => ({
										...p,
										isApplicable: v,
									}),
								)
						...
						/>
						<Label className="text-sm">
							I have assets / financial interests outside India (Sch FA)
						</Label>
					</div>
					{foreignAssets.isApplicable && (
						<div className="space-y-4">
							{foreignAssets.assets.map(
								(asset: ForeignAssetEntry, idx: number): React.ReactElement => (
										...
										<CardContent className="p-4 space-y-3">
											<div className="flex justify-between items-center">
												...
												<Button
													variant="ghost"
													size="sm"
													onClick={(): void => removeForeignAsset(idx)}
													...
													<Select
														...
														onValueChange={(v: string): void =>
															updateForeignAsset(
																idx,
																"assetType",
																v as ForeignAssetEntry["assetType"],
															)
														}
													>
														<SelectTrigger className="h-8 text-xs">
											...
											<div className="grid grid-cols-1 md:grid-cols-4 gap-3">
												...
												<div className="space-y-1">
													...
													<CurrencyInput
														...
														onChange={(v: number): void =>
															updateForeignAsset(idx, "peakBalance", v)
														}
												...
												<div className="space-y-1">
													...
													<CurrencyInput
														...
														onChange={(v: number): void =>
															updateForeignAsset(idx, "closingBalance", v)
														}
							...
							<Button
								...
								data-testid="button-add-foreign-asset"
							>
							...
							{foreignAssets.assets.length === 0 && (
								<Alert className="bg-red-50 dark:bg-red-950 border-red-200 dark:border-red-800">
									...
									<AlertDescription className="text-red-700 dark:text-blue-300 text-sm">
										Schedule FA is mandatory for residents holding foreign
										assets. The Income Tax Department receives data from 100+
										countries via CRS (Common Reporting Standard).
										Non-disclosure can lead to ₹10 lakh penalty and prosecution
			...
			<Card className="bg-muted/50">
				<CardContent className="p-4 space-y-2">
					...
					<div className="flex justify-between items-center text-sm">
						...
						<span className="font-medium">
							{formatCurrency(foreignTotalInINR)}
						</span>
					</div>
					{foreignIncomeDetails.foreignTaxPaid > 0 && (
					...
					<div className="flex justify-between items-center font-semibold">
						...
						<span>
							{formatCurrency(
								foreignTotalInINR - foreignIncomeDetails.foreignTaxPaid,
							)}
						</span>
					</div>
				</CardContent>
