...
const normalizeStatus = (status: string): TransactionStatus => {
	...
	if (s.includes("complet") || s.includes("success") || s === "paid")
		return "completed";
	if (s.includes("pend") || s === "initiated") return "pending";
	if (s.includes("fail") || s.includes("cancel") || s === "rejected")
		return "failed";
				...
				<div className="flex-1">
					<div className="flex items-center gap-2 mb-2 flex-wrap">
						...
						<h3
							...
							data-testid={`text-transaction-name-${transaction.id}`}
						>
						...
						>
							...
							<span className="ml-1">
								{transaction.source === "fintekpro"
									? "FintekPro"
									: transaction.source === "cashfree"
										? "Cashfree"
										: "PhonePe"}
							</span>
						</Badge>
					...
					<div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
						...
						<div>
							...
							<p className="font-medium capitalize">
								{productLabels[transaction.productType as ProductFilter] ||
									transaction.productType?.replace(/_/g, " ")}
							</p>
						</div>
						<div>
...
export default function TransactionReports() {
	...
	} = useQuery<UnifiedOrder[]>({
		queryKey: ["/api/unified-orders"],
	});

	const {
		data: cashfreeTransactions = [],
	...
	const {
		data: phonePeTransactions = [],
		isLoading: phonePeLoading,
		refetch: refetchPhonePe,
	} = useQuery<any[]>({
	...
	const filteredTransactions = useMemo(() => {
		return normalizedTransactions.filter((tx) => {
			...
			if (
				filters.status !== "all" &&
				normalizeStatus(tx.status) !== filters.status
			)
				return false;
			if (filters.product !== "all" && tx.productType !== filters.product)
				return false;
	...
	const stats = useMemo(() => {
		...
		const pending = filteredTransactions.filter(
			(tx) =>
				...
				normalizeStatus(tx.status) === "processing",
		).length;
	...
	const sourceStats = {
		...
		fintekpro: normalizedTransactions.filter((tx) => tx.source === "fintekpro")
			.length,
		cashfree: normalizedTransactions.filter((tx) => tx.source === "cashfree")
			.length,
		phonepe: normalizedTransactions.filter((tx) => tx.source === "phonepe")
			.length,
	};
	...
	const handleExportPDF = () => {
		...
		doc.text(
			`Total Value: ₹${stats.completedAmount.toLocaleString("en-IN")}`,
			14,
			summaryY + 7,
		);
		...
		const tableData = filteredTransactions.map((tx) => [
			...
			tx.source === "fintekpro"
				...
				: tx.source === "cashfree"
					? "Cashfree"
					: "PhonePe",
			productLabels[tx.productType as ProductFilter] ||
				tx.productType?.replace(/_/g, " "),
			tx.productName.length > 25
				? tx.productName.substring(0, 25) + "..."
				: tx.productName,
	...
	return (
		<div
			...
			data-testid="transaction-reports-page"
		>
			<div className="mb-6">
				<Link href="/reports">
					<Button
				...
				<div className="flex items-center justify-between flex-wrap gap-4">
					<div>
						...
						<p className="text-muted-foreground">
							View all your FintekPro orders and payment transactions
						</p>
					</div>
					...
					<div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
						...
						<div className="flex items-end">
							<div className="text-sm text-muted-foreground">
								...
								of {normalizedTransactions.length}
							</div>
						</div>
					</div>
				</CardContent>
