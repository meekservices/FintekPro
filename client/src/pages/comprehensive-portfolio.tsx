...
export default function ComprehensivePortfolioPage() {
	...
	const {
		...
		isLoading,
		error,
	} = useQuery<ComprehensivePortfolio>({
		...
		queryFn: async () => {
			...
			try {
				...
				const response = await fetch(
					`/api/portfolios/${portfolioId}/comprehensive/${selectedDate}`,
				);
				if (!response.ok) {
		...
		<div className="p-6 space-y-6" data-testid="comprehensive-portfolio-page">
			...
			<div className="flex items-center justify-between">
				<div>
					<h1
						...
						data-testid="title-comprehensive-portfolio"
					>
						Comprehensive Portfolio
					</h1>
					<p className="text-muted-foreground">
						Unified view of all your holdings across mutual funds, equities,
				...
				>
					<RefreshCw
						className={`h-4 w-4 mr-2 ${populatePortfolioMutation.isPending ? "animate-spin" : ""}`}
					/>
					Refresh Portfolio
				</Button>
			</div>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Select date and data sources to include
					</CardDescription>
				</CardHeader>
								...
								<div className="flex items-center space-x-2">
									<Switch
										...
										onCheckedChange={(checked) =>
											setPopulateSettings((prev) => ({
												...prev,
												includeMutualFunds: checked,
											}))
										}
								...
								<div className="flex items-center space-x-2">
									<Switch
										...
										onCheckedChange={(checked) =>
											setPopulateSettings((prev) => ({
												...prev,
												includeGovernmentSchemes: checked,
											}))
										}
			...
			{portfolioData && (
				<>
					...
					<div className="grid grid-cols-1 md:grid-cols-4 gap-6">
						...
						<Card>
							...
							<CardContent>
								<div
									className="text-2xl font-bold"
									data-testid="text-mutual-funds-value"
								>
									{formatCurrency(portfolioData.totalMutualFundValue)}
								</div>
								<p className="text-xs text-muted-foreground">
									{(
										(portfolioData.totalMutualFundValue /
						...
						<Card>
							...
							<CardContent>
								<div
									className="text-2xl font-bold"
									data-testid="text-govt-schemes-value"
								>
									{formatCurrency(portfolioData.totalGovernmentSchemeValue)}
								</div>
								<p className="text-xs text-muted-foreground">
									EPF: {formatCurrency(portfolioData.epfValue)} | PPF:{" "}
									{formatCurrency(portfolioData.ppfValue)}
								</p>
							</CardContent>
						</Card>
					...
					<Tabs defaultValue="all" className="space-y-4">
						<ScrollableTabsList>
							<TabsTrigger value="all" data-testid="tab-all-holdings">
								All Holdings
							</TabsTrigger>
						...
						<TabsContent value="all">
							<Card>
								<CardHeader>
									...
									<CardDescription>
										Complete list of holdings across all data sources
									</CardDescription>
								</CardHeader>
								<CardContent>
									<div className="space-y-2">
											...
											>
												...
												<div className="text-right">
													...
													{holding.gainLossPercent && (
														<div
															className={`text-sm flex items-center ${holding.gainLossPercent >= 0 ? "text-green-600" : "text-red-600"}`}
														>
										...
										<div className="space-y-2">
											{holdings.map((holding, index) => (
												<div
													key={index}
													className="flex items-center justify-between p-3 border rounded-lg"
													data-testid={`${source}-holding-${index}`}
												>
													<div>
														<div className="font-medium">
															{holding.assetName}
														...
														<div className="text-sm text-muted-foreground">
															...
															{holding.sourceAccountNumber &&
																` • Account: ${holding.sourceAccountNumber}`}
														</div>
													</div>
													<div className="text-right">
														<div className="font-medium">
