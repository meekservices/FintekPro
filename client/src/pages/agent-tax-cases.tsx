...
import { 
  FileText, Search, Filter, Eye, Edit, Send, CheckCircle2, Clock,
  AlertTriangle, User, Calendar, IndianRupee, ArrowRight, Loader2,
  FileCheck, Globe, MessageSquare, Upload
} from "lucide-react";
...
const itrStatusConfig: Record<string, { label: string; color: string }> = {
  ...
  in_progress: { label: "In Progress", color: "bg-indigo-100 text-indigo-700 dark:bg-indigo-900 dark:text-indigo-300" },
...
const form15StatusConfig: Record<string, { label: string; color: string }> = {
  ...
  approved: { label: "Approved", color: "bg-indigo-100 text-indigo-700 dark:bg-indigo-900 dark:text-indigo-300" },
  "15cb_signed": { label: "15CB Signed", color: "bg-cyan-100 text-cyan-700 dark:bg-cyan-900 dark:text-cyan-300" },
  "15ca_filed": { label: "15CA Filed", color: "bg-teal-100 text-teal-700 dark:bg-teal-900 dark:text-teal-300" },
...
export default function AgentTaxCasesPage() {
  ...
  const { data: form15Cases = [], isLoading: form15Loading } = useQuery<Form15Case[]>({
  ...
  const { data: notices = [], isLoading: noticesLoading } = useQuery<TaxNotice[]>({
  ...
  const updateCaseMutation = useMutation({
    mutationFn: async ({ caseId, action, notes, caseType }: { caseId: number | string; action: string; notes?: string; caseType: "itr" | "form15" }) => {
      if (caseType === "form15") {
        return await apiRequest(`/api/tax-compliance/form15/cases/${caseId}/submit-for-review`, {
          method: "POST",
  ...
  const handleSubmitForReview = (caseId: number | string, caseType: "itr" | "form15") => {
    updateCaseMutation.mutate({ caseId, action: "submit_for_review", notes: caseNotes, caseType });
  };

  const stats = {
    totalItrCases: itrCases.length,
    pendingItrCases: itrCases.filter(c => ["draft", "in_progress"].includes(c.status)).length,
    totalForm15Cases: form15Cases.length,
    pendingForm15Cases: form15Cases.filter(c => ["draft", "pending_documents"].includes(c.status)).length,
    ...
    <div className="container mx-auto p-6 space-y-6" data-testid="page-agent-tax-cases">
        ...
        <TabsContent value="itr" className="mt-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Manage income tax return filings for your clients</CardDescription>
            </CardHeader>
            <CardContent>
              {itrLoading ? (
                  ...
                  <TableBody>
                    {filteredItrCases.map((itrCase) => (
                      <TableRow key={itrCase.id}>
                        <TableCell className="font-medium">{itrCase.clientName || "N/A"}</TableCell>
                        <TableCell className="font-mono text-sm">{itrCase.clientPan || "N/A"}</TableCell>
                        ...
                        <TableCell>
                          <div className="flex gap-2">
                            ...
                            <Button 
                              size="sm" 
                              variant="ghost"
                              onClick={() => navigate(`/tax/itr/preview/${itrCase.id}`)}
        ...
        <TabsContent value="form15" className="mt-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Manage international remittance tax compliance cases</CardDescription>
            </CardHeader>
            <CardContent>
              {form15Loading ? (
                <div className="flex items-center justify-center py-8">
                  ...
                  <TableBody>
                    {filteredForm15Cases.map((form15Case) => (
                      <TableRow key={form15Case.id}>
                        <TableCell className="font-mono text-sm">{form15Case.case_number}</TableCell>
                        <TableCell className="font-medium">{form15Case.client_name}</TableCell>
                        <TableCell>
                          {form15Case.remittance_currency} {form15Case.remittance_amount?.toLocaleString()}
                        </TableCell>
                        ...
                        <TableCell>
                          <div className="flex gap-2">
                            <Button 
                              size="sm" 
                              variant="ghost"
                              onClick={() => handleViewCase(form15Case, "form15")}
        ...
        <TabsContent value="notices" className="mt-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Handle tax notices assigned to you for response</CardDescription>
            </CardHeader>
            <CardContent>
              {noticesLoading ? (
                <div className="flex items-center justify-center py-8">
                  ...
                  <TableBody>
                    {notices.map((notice) => (
                      <TableRow key={notice.id}>
                        ...
                        <TableCell>{new Date(notice.responseDeadline).toLocaleDateString("en-IN")}</TableCell>
                        <TableCell>
                          <Badge className={notice.priority === "high" ? "bg-red-500" : "bg-yellow-500"}>
                            {notice.priority}
                          </Badge>
                        </TableCell>
                        <TableCell>
      ...
      <Dialog open={showCaseDialog} onOpenChange={setShowCaseDialog}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>
              {selectedCase?.type === "itr" ? "ITR Case Details" : 
               selectedCase?.type === "form15" ? "15CA/CB Case Details" : "Notice Details"}
            </DialogTitle>
            <DialogDescription>
          ...
          <DialogFooter className="gap-2">
            ...
            {selectedCase && ["draft", "in_progress", "pending_documents"].includes(selectedCase.status) && (
              <Button 
                onClick={() => handleSubmitForReview(selectedCase.id, selectedCase.type === "form15" ? "form15" : "itr")}
