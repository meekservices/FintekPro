    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">Optimize profits through smart supplier selection</p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Choose a product to analyze supplier performance and profit optimization</CardDescription>
        </CardHeader>
        <CardContent>
        ...
        <Tabs defaultValue="analysis" className="space-y-4">
                    ...
                    <CardContent className="p-6">
                      <div className="flex items-center space-x-2">
                        ...
                        <div>
                          ...
                          <p className="text-2xl font-bold" data-testid="text-total-suppliers">{analysis.totalSuppliers}</p>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
                    ...
                    <CardContent className="p-6">
                      <div className="flex items-center space-x-2">
                        ...
                        <div>
                          ...
                          <p className="text-2xl font-bold" data-testid="text-avg-margin">{analysis.avgProfitMargin.toFixed(1)}%</p>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
                    ...
                    <CardContent className="p-6">
                      <div className="flex items-center space-x-2">
                        ...
                        <div>
                          ...
                          <p className="text-2xl font-bold" data-testid="text-best-margin">{analysis.bestMargin.toFixed(1)}%</p>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
                ...
                {optimal && (
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>Best supplier based on profit optimization algorithm</CardDescription>
                    </CardHeader>
                    <CardContent>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                          <p className="text-sm font-medium text-muted-foreground">Supplier</p>
                          <p className="font-semibold" data-testid="text-optimal-supplier">{optimal.supplierName}</p>
                        </div>
                        <div>
                          <p className="text-sm font-medium text-muted-foreground">Profit Margin</p>
                          <p className="font-semibold text-green-600" data-testid="text-optimal-margin">{optimal.profitMargin.toFixed(1)}%</p>
                        </div>
                        <div>
                          <p className="text-sm font-medium text-muted-foreground">Profit Score</p>
                          <p className="font-semibold" data-testid="text-profit-score">{optimal.profitScore.toFixed(1)}</p>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
          ...
          <TabsContent value="comparison" className="space-y-4">
            {isComparisonLoading ? (
              <Card>
                <CardContent className="p-6">
                  <div className="text-center">Loading supplier comparison...</div>
                </CardContent>
              </Card>
            ) : (
                ...
                {comparison.map((supplier, index) => (
                  <Card key={supplier.supplierId} className={index === 0 ? "border-green-500 bg-green-50 dark:bg-green-950" : ""}>
                    <CardHeader>
                      <div className="flex justify-between items-center">
                        <CardTitle className="flex items-center space-x-2">
                          <span data-testid={`text-supplier-name-${supplier.supplierId}`}>{supplier.supplierName}</span>
                          {index === 0 && <Badge variant="secondary" className="bg-green-500 text-white">Best Choice</Badge>}
                        </CardTitle>
                    ...
                    <CardContent>
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div>
                          ...
                          <p className="font-semibold" data-testid={`text-cost-price-${supplier.supplierId}`}>₹{supplier.costPrice.toLocaleString()}</p>
                        </div>
                        <div>
                          <p className="text-sm font-medium text-muted-foreground">Selling Price</p>
                          <p className="font-semibold" data-testid={`text-selling-price-${supplier.supplierId}`}>₹{supplier.sellingPrice.toLocaleString()}</p>
                        </div>
                        <div>
                          <p className="text-sm font-medium text-muted-foreground">Profit Margin</p>
                          <p className={`font-semibold ${supplier.profitMargin > 0 ? 'text-green-600' : 'text-red-600'}`} 
                        ...
                        <div>
                          ...
                          <div className="flex items-center space-x-2">
                            <Progress value={supplier.performanceRating * 20} className="flex-1" />
                            <span className="text-sm font-medium" data-testid={`text-rating-${supplier.supplierId}`}>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-4">
                        <div>
                          ...
                          <p className="font-semibold" data-testid={`text-sales-volume-${supplier.supplierId}`}>{supplier.salesVolume.toLocaleString()}</p>
                        </div>
                        <div>
                          <p className="text-sm font-medium text-muted-foreground">Revenue</p>
                          <p className="font-semibold" data-testid={`text-revenue-${supplier.supplierId}`}>₹{supplier.revenue.toLocaleString()}</p>
                        </div>
                        <div>
                          <p className="text-sm font-medium text-muted-foreground">Commission Rate</p>
                          <p className="font-semibold" data-testid={`text-commission-${supplier.supplierId}`}>{supplier.commissionRate.toFixed(1)}%</p>
                        </div>
                      </div>
                      ...
                      {supplier.notes && (
                        <div className="mt-4">
                          ...
                          <p className="text-sm" data-testid={`text-notes-${supplier.supplierId}`}>{supplier.notes}</p>
                        </div>
                      )}
                    </CardContent>
                  </Card>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {suppliers.map(supplier => (
                <Card key={supplier.id}>
                  <CardHeader>
                    <div className="flex justify-between items-center">
                      <CardTitle data-testid={`text-supplier-${supplier.id}`}>{supplier.name}</CardTitle>
                      <Badge variant={supplier.isActive ? "default" : "secondary"}>
                        {supplier.isActive ? "Active" : "Inactive"}
                      </Badge>
                    </div>
                  ...
                  <CardContent>
                    <div className="space-y-2">
                      {supplier.contactEmail && (
                        <p className="text-sm" data-testid={`text-email-${supplier.id}`}>
                          <span className="font-medium">Email:</span> {supplier.contactEmail}
                        </p>
                      )}
                      {supplier.contactPhone && (
                        <p className="text-sm" data-testid={`text-phone-${supplier.id}`}>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center space-x-2">
                          <Progress value={parseFloat(supplier.performanceRating || "0") * 20} className="w-16" />
                          <span className="text-sm" data-testid={`text-supplier-rating-${supplier.id}`}>
                            {parseFloat(supplier.performanceRating || "0").toFixed(1)}
                          </span>
                        </div>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-sm font-medium">Commission Rate</span>
                        <span className="text-sm font-semibold" data-testid={`text-supplier-commission-${supplier.id}`}>
                          {parseFloat(supplier.commissionRate || "0").toFixed(1)}%
                        </span>
                      </div>
                    </div>
                    <div className="flex space-x-2 mt-4">
                      ...
                      <Button variant="outline" size="sm" data-testid={`button-view-products-${supplier.id}`}>
                        View Products
                      </Button>
                    </div>
                  </CardContent>
      ...
      {!selectedProductId && (
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <p className="text-muted-foreground">Choose a product from the dropdown above to view supplier analysis and profit optimization insights.</p>
          </CardContent>
        </Card>
      )}
    </div>
