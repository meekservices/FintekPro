...
async function insertToExcel(
	...
	if (!isOfficeAvailable()) {
		throw new Error(
			"Excel API not available — open this page inside Excel as a task pane",
		);
	}
...
export default function ExcelAddinPage() {
	...
	const {
		...
		queryFn: () =>
			fetch(
				`/api/excel/chain/${symbol}${expiry ? `?expiry=${expiry}` : ""}`,
			).then((r) => r.json()),
		enabled: !!symbol,
	...
	const displayRows = strikeFilter
		? nearStrikes
	...
	const handleInsertChain = async () => {
		try {
			...
			toast({
				title: "Inserted to Excel",
				description: `${displayRows.length} rows written to selected range`,
			});
		} catch (e: any) {
			toast({
				title: "Insert failed",
		...
		>
			...
			<div className="bg-blue-700 text-white px-3 py-2 flex items-center justify-between">
				<div className="flex items-center gap-2">
					...
					{inExcel && (
						<Badge
							variant="secondary"
							className="text-xs bg-blue-600 text-white border-0 px-1.5"
						>
							Excel
						</Badge>
					)}
			...
			<Tabs value={tab} onValueChange={setTab} className="flex-1">
				<ScrollableTabsList className="px-3 pt-2 h-8 bg-transparent border-b rounded-none gap-1">
					<TabsTrigger value="chain" className="text-xs h-7 px-3">
						Chain
					</TabsTrigger>
					<TabsTrigger value="formulas" className="text-xs h-7 px-3">
						Formulas
					</TabsTrigger>
					<TabsTrigger value="install" className="text-xs h-7 px-3">
						Install
					</TabsTrigger>
				</ScrollableTabsList>
				...
				<TabsContent value="chain" className="m-0 p-0">
							...
							<Table>
								<TableHeader>
									<TableRow className="text-[10px]">
										<TableHead className="text-right pr-1 w-16 text-blue-600">
											LTP
										</TableHead>
										<TableHead className="text-right pr-1 w-12 text-blue-600">
											OI
										</TableHead>
										<TableHead className="text-right pr-1 w-12 text-blue-600">
											IV%
										</TableHead>
										<TableHead className="text-center font-semibold w-20 bg-muted">
											STRIKE
										</TableHead>
										<TableHead className="text-left pl-1 w-12 text-orange-600">
											IV%
										</TableHead>
										<TableHead className="text-left pl-1 w-12 text-orange-600">
											OI
										</TableHead>
										<TableHead className="text-left pl-1 w-16 text-orange-600">
											LTP
										</TableHead>
									</TableRow>
								</TableHeader>
								<TableBody>
									{displayRows.map((row) => {
										const atm =
											spot != null && Math.abs(row.strike - spot) < 50;
										...
										return (
											...
											>
												{/* CALL side */}
												<TableCell
													className={`text-right pr-1 font-mono ${itcCall ? "bg-blue-50 dark:bg-blue-950/20" : ""}`}
												>
												...
												<TableCell
													className={`text-right pr-1 font-mono text-muted-foreground text-[10px] ${itcCall ? "bg-blue-50 dark:bg-blue-950/20" : ""}`}
												>
													{fmtOI(row.call_oi)}
												</TableCell>
												<TableCell
													className={`text-right pr-1 font-mono text-[10px] ${itcCall ? "bg-blue-50 dark:bg-blue-950/20" : ""}`}
												>
													{row.call_iv != null
												...
												{/* PUT side */}
												<TableCell
													className={`text-left pl-1 font-mono text-[10px] ${itcPut ? "bg-orange-50 dark:bg-orange-950/20" : ""}`}
												>
													{row.put_iv != null
												...
												<TableCell
													className={`text-left pl-1 font-mono text-muted-foreground text-[10px] ${itcPut ? "bg-orange-50 dark:bg-orange-950/20" : ""}`}
												>
													{fmtOI(row.put_oi)}
												</TableCell>
												<TableCell
													className={`text-left pl-1 font-mono ${itcPut ? "bg-orange-50 dark:bg-orange-950/20" : ""}`}
												>
													{row.put_change != null && (
														<span
															className={
					...
					<div className="px-3 py-1.5 text-[10px] text-muted-foreground border-t flex justify-between">
						<span>
							CALL <span className="text-blue-600">■</span> &nbsp; ATM{" "}
							<span className="text-yellow-500">■</span> &nbsp; PUT{" "}
				...
				<TabsContent value="formulas" className="m-0 p-3 space-y-2">
					<p className="text-xs text-muted-foreground mb-2">
						Copy these formulas into any Excel cell. Requires the add-in to be
						installed.
					</p>
				...
				<TabsContent value="install" className="m-0 p-3 space-y-3">
					<div className="border rounded-md p-3 space-y-2">
						...
						<p className="text-[11px] text-muted-foreground">
							Download the manifest and sideload it into Excel to use custom
							formulas.
						</p>
						<a
					...
					<div className="border rounded-md p-3 space-y-1.5 text-[11px]">
						...
						<ol className="list-decimal list-inside space-y-1 text-muted-foreground">
							...
							<li>
								Open Excel → File → Options → Trust Center → Trust Center
								Settings
							</li>
							<li>
								Go to <strong>Trusted Add-in Catalogs</strong>
							</li>
							<li>
								Add catalog URL:{" "}
							...
							<li>
								Restart Excel and go to Insert → My Add-ins → FintekPro Options
							</li>
						</ol>
					</div>
					...
					<div className="border rounded-md p-2 bg-muted/30 text-[10px] text-muted-foreground">
						...
						<code className="font-mono text-blue-600">
							{window.location.origin}/excel-addin
						</code>
					</div>
				</TabsContent>
			</Tabs>
