...
export default function AgentTracker() {
	...
	const handleInitiate = async () => {
		...
		try {
			...
			if (data.requestId || data.success) {
				...
				toast({
					title: "OTP Sent",
					description: data.message || "Check client's registered mobile",
				});
			} else {
				toast({
					title: "Failed",
					description: data.error || "Could not initiate",
					variant: "destructive",
				});
			}
		} catch (err: any) {
			toast({
				title: "Error",
				description: err.message || "Failed to contact IRIS KFintech service",
				variant: "destructive",
			});
		} finally {
	...
	const handleVerify = async () => {
		...
		try {
			...
			toast({
				title: "Error",
				description: err.message || "IRIS verification failed",
				variant: "destructive",
			});
		} finally {
			setIsSubmitting(false);
	...
	const handleIrisSync = async () => {
		...
		try {
			...
			// apiRequest already returns parsed JSON — do NOT call .json() on it
			const fetchData = await apiRequest(
	...
	const equityPct =
		totalAUM > 0 ? Math.round((t!.summary.totalEquity / totalAUM) * 100) : 0;
	const debtPct =
		totalAUM > 0 ? Math.round((t!.summary.totalDebt / totalAUM) * 100) : 0;
	const goldPct =
		totalAUM > 0 ? Math.round((t!.summary.totalGold / totalAUM) * 100) : 0;
	const altPct =
		totalAUM > 0
			? Math.round((t!.summary.totalAlternatives / totalAUM) * 100)
			: 0;
	...
	return (
		<div className="p-4 md:p-6 space-y-6 max-w-7xl">
			...
			<div className="flex items-start justify-between flex-wrap gap-3">
				<div>
					...
					<p className="text-sm text-muted-foreground mt-1">
						Your complete MF book — AUM, SIPs, trail income, powered by IRIS
						KFintech
					</p>
				</div>
						...
						<DialogContent className="max-w-md">
							...
							<Tabs defaultValue="iris_otp" className="w-full">
								<TabsList className="grid w-full grid-cols-2">
									...
									<TabsTrigger value="iris_registry">
										IRIS (Direct Sync)
									</TabsTrigger>
								</TabsList>
									...
									{!otpStep ? (
										<div className="space-y-4">
											<div className="bg-blue-50 p-3 rounded-lg border border-blue-100 mb-4">
												...
												<p className="text-xs text-blue-600 mt-1">
													Sends an official OTP from KFintech to the client's
													registered mobile number for CAS authorization.
												</p>
											</div>
											<div>
										...
										<div className="space-y-4">
											<p className="text-sm text-muted-foreground">
												OTP sent to client's registered mobile via IRIS
												KFintech.
											</p>
											<div>
												<Label>OTP</Label>
									...
									<div className="space-y-4">
										<div className="bg-indigo-50 p-3 rounded-lg border border-indigo-100 mb-4">
											...
											<p className="text-xs text-indigo-600 mt-1">
												Fetches structured KFintech + CAMS holdings directly via
												the IRIS API. Fast, reliable, and OTP-less for
												previously linked clients.
											</p>
										</div>
										<div>
											<Label>Client PAN</Label>
										...
										<Button
											onClick={handleIrisSync}
											disabled={isSubmitting}
											className="w-full bg-indigo-600 hover:bg-indigo-700"
										>
											{isSubmitting
												? "Communicating with IRIS..."
												: "Sync via IRIS Registry"}
										</Button>
									</div>
								</TabsContent>
							</Tabs>
					...
					<CardContent className="p-4">
						...
						<div className="flex gap-1 mt-2 flex-wrap">
							{(t?.sipBook.expiringSips ?? 0) > 0 && (
								<Badge
									variant="outline"
									className="text-amber-600 border-amber-200 bg-amber-50 text-xs px-1"
								>
							...
							{(t?.sipBook.netNewThisMonth ?? 0) > 0 && (
								<Badge
									variant="outline"
									className="text-green-600 border-green-200 bg-green-50 text-xs px-1"
								>
				...
				<Card className="bg-gradient-to-br from-teal-50 to-teal-100 dark:from-teal-950 dark:to-teal-900 border-teal-200">
					<CardContent className="p-4">
						...
						<div className="mt-2 space-y-1">
							...
							<Progress
								value={
									t?.clientConnectivity.total
										? (t.clientConnectivity.withHoldings /
												t.clientConnectivity.total) *
											100
										: 0
								}
						...
						{(t?.monthlyTrend ?? []).some((m) => m.aum > 0) ? (
							...
							<div className="h-[220px] flex flex-col items-center justify-center text-muted-foreground">
								...
								<p className="text-xs mt-1">
									Import portfolios via IRIS Registry to see trend
								</p>
							</div>
						)}
					</CardContent>
					...
					<CardContent>
						{(t?.topAmcs ?? []).length > 0 ? (
							...
							<div className="h-[220px] flex flex-col items-center justify-center text-muted-foreground">
								...
								<p className="text-xs mt-1">
									Connect client portfolios to see breakdown
								</p>
							</div>
						)}
					</CardContent>
			...
			<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
					...
					<CardContent className="p-0">
						{(t?.topAmcs ?? []).length > 0 ? (
											...
											>
												<td className="p-3 flex items-center gap-2">
													<span
														...
														style={{
															backgroundColor:
																AMC_COLORS[i % AMC_COLORS.length],
														}}
													/>
													{amc.name}
												</td>
							...
							<div className="p-8 text-center text-muted-foreground">
								...
								<p className="text-xs mt-1">
									Use the "Connect Client Portfolio" button above
								</p>
							</div>
						)}
					</CardContent>
				...
				<Card>
					...
					<CardContent className="space-y-3">
						...
						{(t?.clientConnectivity.total ?? 0) >
							(t?.clientConnectivity.withHoldings ?? 0) && (
			...
			<p className="text-xs text-muted-foreground text-center">
				Trail commission is an estimate based on AUM × standard trail rates
				(0.8% equity, 0.1% debt p.a.). Actual payouts depend on AMC commission
				structures. Data from IRIS (CAS Fetch) last updated:{" "}
