...
const PARTNER_TYPES = [
	{
		value: "ifa",
		label: "Individual Financial Advisor (IFA)",
		desc: "Independent advisor selling financial products",
	},
	{
		value: "mfd",
		label: "Mutual Fund Distributor (MFD)",
		desc: "AMFI-registered MF distributor with ARN",
	},
	{
		value: "sub_broker",
		label: "Sub-Broker",
		desc: "Equity and derivatives sub-broker",
	},
	{
		value: "ca_partner",
		label: "CA Partner",
		desc: "Chartered Accountant with distribution license",
	},
	{
		value: "corporate_distributor",
		label: "Corporate Distributor",
		desc: "Company/firm registered as distributor",
	},
	{
		value: "nri_partner",
		label: "NRI Partner",
		desc: "NRI-based financial advisor",
	},
];
...
export default function PartnerRegister() {
	...
	const form1 = useForm<Step1, any, Step1>({
		resolver: zodResolver(step1Schema),
		defaultValues: { firstName: "", lastName: "", email: "", mobile: "" },
	});
	const form2 = useForm<Step2, any, Step2>({
	...
	const form3 = useForm<Step3, any, Step3>({
	...
	const registerMutation = useMutation({
		mutationFn: (data: any) =>
			apiRequest("/api/partner/register", {
				method: "POST",
					...
					<CardContent className="pt-10 pb-8 space-y-5">
						...
						<div>
							...
							<p className="text-muted-foreground mt-2 text-sm">
								Your partner registration is under review. Our team will verify
								your details and reach out on your registered email &amp; mobile
								within <strong>48–72 business hours</strong>.
							</p>
						</div>
			...
			<div className="w-full max-w-2xl">
				...
				<div className="text-center mb-8">
					...
					<h1 className="text-3xl font-bold text-white">
						Become a FintekPro Partner
					</h1>
					<p className="text-indigo-300 mt-2 text-sm">
						Distribute India's best financial products with industry-leading
						commissions
					</p>
				</div>
					...
					{STEPS.map((s, i) => {
						...
						return (
							<div key={s.id} className="flex items-center">
								...
								{i < STEPS.length - 1 && (
									<div
										className={`h-0.5 w-6 mx-1 ${step > s.id ? "bg-emerald-500" : "bg-slate-700"}`}
									/>
								)}
							</div>
						);
					...
					<Card className="shadow-2xl border-slate-700">
						<CardHeader>
							<CardTitle className="flex items-center gap-2">
								<User className="h-5 w-5 text-indigo-500" /> Personal
								Information
							</CardTitle>
							<CardDescription>
								Tell us who you are — name, email, and mobile
							</CardDescription>
						</CardHeader>
						<CardContent>
							<Form {...form1}>
				...
				{step === 2 && (
					<Card className="shadow-2xl border-slate-700">
						<CardHeader>
							<CardTitle className="flex items-center gap-2">
								<Building2 className="h-5 w-5 text-indigo-500" /> Business
								Details
							</CardTitle>
							<CardDescription>
								Your firm / practice details and regulatory credentials
							</CardDescription>
						</CardHeader>
						<CardContent>
							<Form {...form2}>
								...
								>
									<FormField
										...
										render={({ field }) => (
											<FormItem>
												...
												<FormControl>
													<Input
														placeholder="Sharma Financial Services"
														{...field}
													/>
												</FormControl>
												<FormDescription className="text-xs">
													Use your registered firm name, or your full name if
													individual
												</FormDescription>
												<FormMessage />
											</FormItem>
									...
									<FormField
										...
										render={({ field }) => (
											<FormItem>
												<FormLabel className="flex items-center gap-1.5">
													<Award className="h-3.5 w-3.5" /> Partner Type
												</FormLabel>
												<Select
												...
												>
													<FormControl>
														<SelectTrigger>
															<SelectValue placeholder="Select your partner category" />
														</SelectTrigger>
													</FormControl>
													<SelectContent>
														{PARTNER_TYPES.map((p) => (
									...
									/>
									{(watchPartnerType === "mfd" ||
										watchPartnerType === "ifa") && (
										<FormField
											control={form2.control}
											name="arnCode"
											render={({ field }) => (
												<FormItem>
													<FormLabel className="flex items-center gap-1.5">
														<Hash className="h-3.5 w-3.5" /> ARN Code
													</FormLabel>
													<FormControl>
														<Input placeholder="ARN-123456" {...field} />
													</FormControl>
													<FormDescription className="text-xs">
														AMFI Registration Number — leave blank if applying
														without ARN
													</FormDescription>
													<FormMessage />
												</FormItem>
											...
											<FormItem className="flex items-start gap-3 p-3 rounded-lg bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800">
												...
												<div>
													...
													<p className="text-xs text-emerald-700 dark:text-emerald-400 mt-0.5">
														Enables ITR filing, Form 15CA/15CB, CA Management
														dashboard, and CA support tickets
													</p>
												</div>
											</FormItem>
										)}
				...
				{step === 3 && (
					<Card className="shadow-2xl border-slate-700">
						<CardHeader>
							<CardTitle className="flex items-center gap-2">
								<FileText className="h-5 w-5 text-indigo-500" /> Declaration
								&amp; Consent
							</CardTitle>
							<CardDescription>
								Review your application and accept the terms before submitting
							</CardDescription>
						</CardHeader>
									...
									<div className="grid grid-cols-2 gap-y-2 text-sm">
										...
										<div>
											...
											<Badge variant="secondary" className="text-xs">
												{PARTNER_TYPES.find(
													(p) => p.value === step2Data.partnerType,
												)?.label || step2Data.partnerType}
											</Badge>
										</div>
										<div>
										...
										<div>
											...
											<span className="font-medium">
												{step2Data.city}, {step2Data.state}
											</span>
										</div>
										{step2Data.arnCode && (
											<div>
												<span className="text-muted-foreground">ARN: </span>
												<span className="font-medium">{step2Data.arnCode}</span>
											</div>
										)}
										{step2Data.isCA && (
											<div>
								...
								>
									...
									<FormField
										...
										render={({ field }) => (
											<FormItem className="flex items-start gap-3">
												...
												<div>
													<FormLabel className="cursor-pointer text-sm">
														I confirm I hold (or will obtain before activation)
														all required SEBI/AMFI/IRDA regulatory registrations
														applicable to my partner type.
													</FormLabel>
													<FormMessage />
												</div>
											</FormItem>
									...
									<div className="rounded-lg bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 p-3 flex gap-2">
										...
										<p className="text-xs text-amber-700 dark:text-amber-400">
											Your account will be created in <strong>PENDING</strong>{" "}
											status. A FintekPro admin will review and activate your
											account within 48–72 hours after credential verification.
										</p>
									</div>
									<div className="flex justify-between pt-2">
										<Button
				...
				<p className="text-center text-indigo-400 text-xs mt-6">
					Already a partner?{" "}
					<a href="/auth" className="text-indigo-300 underline">
						Sign in here
					</a>
				</p>
			</div>
