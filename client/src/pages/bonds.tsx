...
} from "@/components/regulatory/InvestorClassificationCard";
...
const getCreditRatingColors = (rating: string | null | undefined): string => {
	...
	if (r.startsWith("BB") || r.startsWith("B"))
		return "bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300 border-orange-200 dark:border-orange-800";
	if (r.startsWith("C") || r.startsWith("D"))
...
const getBondTypeColors = (type: string | null | undefined): string => {
	...
	if (t.includes("gsec") || t.includes("government"))
		return "bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 border-blue-200 dark:border-blue-800";
	if (t.includes("corporate"))
		return "bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 border-purple-200 dark:border-purple-800";
	if (t.includes("ncd"))
		return "bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300 border-orange-200 dark:border-orange-800";
	if (t.includes("tax") && t.includes("free"))
		return "bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 border-green-200 dark:border-green-800";
	...
	if (t.includes("sgb") || t.includes("gold"))
		return "bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 border-amber-200 dark:border-amber-800";
	if (t.includes("sdl") || t.includes("state"))
...
const getRiskLevelColors = (level: string | null | undefined): string => {
	...
	if (l.includes("low") || l === "conservative")
		return "bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 border-green-200 dark:border-green-800";
	if (l.includes("medium") || l.includes("moderate") || l === "balanced")
		return "bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300 border-yellow-200 dark:border-yellow-800";
	if (l.includes("high") || l === "aggressive")
		return "bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300 border-red-200 dark:border-red-800";
	return "bg-muted text-muted-foreground border-border";
};
...
const STAMP_DUTY_RATES: Record<
	string,
	{ rate: number; isExempt: boolean; reason?: string; payerSide: string }
> = {
	g_sec: {
		rate: 0,
		isExempt: true,
		reason: "Government Securities exempt under Section 9",
		payerSide: "buyer",
	},
	t_bill: {
		rate: 0,
		isExempt: true,
		reason: "Treasury Bills exempt as Government Securities",
		payerSide: "buyer",
	},
	sdl: {
		rate: 0,
		isExempt: true,
		reason: "State Development Loans exempt as Government Securities",
		payerSide: "buyer",
	},
	sgb: {
		rate: 0,
		isExempt: true,
		reason: "Sovereign Gold Bonds exempt as RBI-issued securities",
		payerSide: "buyer",
	},
...
function FeeBreakdownDisplay({
	...
	const fees = useMemo(
		() => calculateFees(amount, config, bondType),
		[amount, config, bondType],
	);
		...
		<div className="bg-gradient-to-r from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20 p-4 rounded-lg space-y-2 text-sm border border-blue-200 dark:border-blue-700">
			...
			<div className="flex justify-between text-xs">
				<span className="text-muted-foreground">
					Brokerage ({config.brokerageBps} bps)
				</span>
			...
			<div className="flex justify-between text-xs items-center">
				<span className="text-muted-foreground flex items-center gap-1">
					...
					{!fees.stampDutyExempt && stampDutyInfo && (
						<span className="text-muted-foreground">
							({stampDutyInfo.rate} bps)
						</span>
					)}
				</span>
				<span
					className={
						fees.stampDutyExempt ? "text-green-600 dark:text-green-400" : ""
					}
				>
					{fees.stampDutyExempt
						? "₹0.00"
						: `₹${fees.stampDuty.toLocaleString()}`}
				</span>
			</div>
			...
			<div className="border-t border-blue-200 dark:border-blue-700 pt-2 flex justify-between font-semibold">
				...
				<span className="text-finance-blue">
								...
								<div className="space-y-2 text-xs">
									...
									<div className="flex justify-between">
										...
										<span className="font-semibold text-finance-blue">
											{category.count} bonds
										</span>
									</div>
									<Badge
										variant="outline"
										className={`w-full justify-center mt-2 ${getRiskLevelColors(category.riskLevel)}`}
									>
...
function GovernmentSecurities() {
	...
	const isGSecEligible =
		eligibilitySummary?.eligibleCategories?.some((c) =>
			govtSecurityIds.includes(c.id),
		) ?? false;

	const riskAttestMutation = useMutation({
		mutationFn: (attestData: any) =>
			apiRequest("/api/bonds/risk-attestation", {
				method: "POST",
				body: JSON.stringify(attestData),
			}),
		onSuccess: () => {
	...
	const placeOrderMutation = useMutation({
		mutationFn: (orderData: any) =>
			apiRequest("/api/bonds/trading/gsec/orders", {
				method: "POST",
	...
	const handleRiskAttest = (bond: any) => {
		riskAttestMutation.mutate({
			...
			transactionValue: Number.parseFloat(bidAmount) || bond.minimumBidAmount,
				...
				<Card className="border-dashed">
					<CardContent className="p-8 text-center">
						...
						<p className="text-muted-foreground">
							No government securities available for auction
						</p>
					</CardContent>
				</Card>
			) : (
							...
							<div className="flex items-start justify-between">
								<div className="flex-1">
									<div className="flex items-center gap-3 mb-2">
										<h4 className="font-semibold text-foreground group-hover:text-finance-blue transition-colors">
											{bond.securityName}
										</h4>
										<Badge
											variant="outline"
									...
									<div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
										...
										<div>
											...
											<p className="font-semibold">
												₹{bond.minimumBidAmount?.toLocaleString()}
											</p>
										</div>
									</div>
									...
									{bond.auctionDate && (
										<p className="text-xs text-muted-foreground mt-3">
											Auction Date:{" "}
											{new Date(bond.auctionDate).toLocaleDateString()}
										</p>
									)}
								</div>

								<Dialog
									open={selectedBond?.isin === bond.isin}
									onOpenChange={(open) => {
										if (!open) {
											setSelectedBond(null);
											setOrderError(null);
										...
										<div className="space-y-4 mt-4">
											<div>
												...
												<Input
													...
													onChange={(e) => {
														...
														if (riskAcknowledged) setRiskAcknowledged(false);
													}}
											...
											{Number.parseFloat(bidAmount) > 0 && (
												<FeeBreakdownDisplay
													...
													bondType={
														bond.securityType === "g_sec"
															? "g_sec"
															: bond.securityType === "t_bill"
																? "t_bill"
																: "sdl"
													}
											...
											{showRiskDisclosure ? (
												...
												<>
													...
													{riskAcknowledged && (
														<Alert className="bg-green-50 dark:bg-green-950/30 border-green-200 dark:border-green-800">
															...
															<AlertDescription className="text-green-700 dark:text-green-300 text-sm">
																Risk disclosures acknowledged. You can proceed
																with your order.
															</AlertDescription>
														</Alert>
													)}
													...
													<Button
														...
														onClick={() => {
															...
															if (!riskAcknowledged) {
																toast({
																	...
																	description:
																		"Please review and acknowledge the risk disclosures before proceeding.",
																});
														...
														disabled={
															placeOrderMutation.isPending || !!orderError
														}
...
function CorporateBonds() {
	...
	const isCorporateEligible =
		eligibilitySummary?.eligibleCategories?.some(
			(c) => c.id === "corporate_listed",
		) ?? false;
	...
	const handleRiskAttest = (bond: any) => {
		riskAttestMutation.mutate({
			...
			transactionValue:
				...
				(Number.parseFloat(limitPrice) || bond.lastPrice || 0),
			disclosuresAcknowledged: true,
			orderType: "buy",
		});
	};
	...
	const placeOrderMutation = useMutation({
		mutationFn: (orderData: any) =>
			apiRequest("/api/bonds/trading/corporate/orders", {
				method: "POST",
				body: JSON.stringify(orderData),
			}),
		onSuccess: () => {
			...
			{bonds.length === 0 ? (
				<Card className="border-dashed">
					<CardContent className="p-8 text-center">
						...
						<p className="text-muted-foreground">
							No corporate bonds available for trading
						</p>
					</CardContent>
				</Card>
			) : (
							...
							<div className="flex items-start justify-between">
								<div className="flex-1">
									<div className="flex items-center gap-3 mb-2 flex-wrap">
										...
										<Badge
											variant="outline"
											className={getCreditRatingColors(
										...
										<Badge
											variant="outline"
											className={getBondTypeColors(bond.bondType || bond.type)}
										>
									...
									<div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
										<div>
											<p className="text-muted-foreground">Yield</p>
											<p className="font-semibold text-finance-green">
												{bond.yieldToMaturity || bond.currentYield || "N/A"}%
											</p>
										</div>
										<div>
											<p className="text-muted-foreground">Coupon</p>
									...
									<div className="mt-3 flex gap-4 text-xs text-muted-foreground">
										...
										{(bond.accruedInterest || bond.accrued) && (
											<span>
												Accrued: ₹
												{(
									...
									>
										...
										<DialogContent>
											...
											<div className="space-y-4 mt-4">
												...
												{(Number.parseFloat(quantity) || 0) > 0 && (
													<FeeBreakdownDisplay
														amount={
															(Number.parseFloat(quantity) || 0) *
															(Number.parseFloat(limitPrice) || bond.lastPrice)
														}
														bondType="corporate"
														commissionData={commissionData}
													/>
...
function NCDBonds() {
	...
	const placeOrderMutation = useMutation({
		mutationFn: (orderData: any) =>
			apiRequest("/api/bonds/trading/ncd/orders", {
				method: "POST",
				body: JSON.stringify(orderData),
			}),
		onSuccess: () => {
			...
			{bonds.length === 0 ? (
				<Card className="border-dashed">
					<CardContent className="p-8 text-center">
						...
						<p className="text-muted-foreground">
							No NCDs available for trading currently
						</p>
						<p className="text-xs text-muted-foreground mt-2">
							Check back during NCD issue periods
						</p>
					</CardContent>
				</Card>
			) : (
							...
							<div className="flex items-start justify-between">
								<div className="flex-1">
									<div className="flex items-center gap-3 mb-2 flex-wrap">
										...
										<Badge
											variant="outline"
											className={getCreditRatingColors(bond.rating)}
										>
											{bond.rating || "NR"}
										</Badge>
										<Badge
											variant="outline"
											className={getBondTypeColors("ncd")}
										>
											NCD
										</Badge>
										<EligibilityBadge
								...
								>
									...
									<DialogContent>
										<DialogHeader>
											...
											<DialogDescription>
												Buy{" "}
												{bond.name || bond.issuerName || bond.issuer || "these"}{" "}
												NCDs
											</DialogDescription>
										</DialogHeader>
														...
														onClick={() => {
															...
															if (!riskAcknowledged) {
																toast({
																	...
																	title: "Risk Disclosure Required",
																	description:
																		"Please review and acknowledge the risk disclosures.",
																});
																setShowRiskDisclosure(true);
																return;
															}
...
function TaxFreeBonds() {
	...
	const isTaxFreeEligible =
		eligibilitySummary?.eligibleCategories?.some((c) => c.id === "tax_free") ??
		false;
	...
	const placeOrderMutation = useMutation({
		mutationFn: (orderData: any) =>
			apiRequest("/api/bonds/trading/tax-free/orders", {
				method: "POST",
				body: JSON.stringify(orderData),
			}),
		onSuccess: () => {
			...
			{bonds.length === 0 ? (
				<Card className="border-dashed">
					<CardContent className="p-8 text-center">
						...
						<p className="text-muted-foreground">
							No tax-free bonds available in secondary market
						</p>
						<p className="text-xs text-muted-foreground mt-2">
							Tax-free bonds are available when PSUs issue them
						</p>
					</CardContent>
				</Card>
			) : (
							...
							<div className="flex items-start justify-between">
								<div className="flex-1">
									<div className="flex items-center gap-3 mb-2 flex-wrap">
										...
										<Badge
											variant="outline"
											className={getBondTypeColors("tax_free")}
										>
											Tax Free
										</Badge>
										<EligibilityBadge
								...
								>
									<DialogTrigger asChild>
										<Button
											...
											size="sm"
											data-testid={`buy-tax-free-${bond.isin}`}
										>
											Buy
										</Button>
									</DialogTrigger>
										...
										<div className="space-y-4 mt-4">
											...
											<div className="bg-green-50 dark:bg-green-900/20 p-4 rounded-lg space-y-2 text-sm">
												...
												<div className="flex items-center gap-1 text-green-700 dark:text-green-300 text-xs">
													...
													<span>
														Interest income is tax-exempt under Section 10(15)
													</span>
												</div>
											</div>
											...
											{showRiskDisclosure ? (
												<RiskDisclosureModal
													bondId={bond.isin}
													bondName={
														bond.name || bond.issuerName || "Tax-Free Bond"
													}
													onAttest={() => handleRiskAttest(bond)}
													isAttesting={riskAttestMutation.isPending}
												/>
...
function BondMarketplace() {
	...
	const [riskAcknowledgments, setRiskAcknowledgments] = useState<
		Record<string, boolean>
	>({});
	const [sellRiskAcknowledgments, setSellRiskAcknowledgments] = useState<
		Record<string, boolean>
	>({});
	const [tierCheckStatus, setTierCheckStatus] = useState<
		"idle" | "checking" | "done"
	>("idle");
	...
	const sellListings = Array.isArray(sellListingsRaw)
		? sellListingsRaw
		: sellListingsRaw?.data || [];
	const myListings = Array.isArray(myListingsRaw)
		? myListingsRaw
		: myListingsRaw?.data || [];
	const myRequests = Array.isArray(myRequestsRaw)
		? myRequestsRaw
		: myRequestsRaw?.data || [];
	const eligibility = eligibilityRaw?.data || eligibilityRaw;

	const allSellRisksAcknowledged = requiredRiskCategories.every(
		(cat) => sellRiskAcknowledgments[cat],
	);
	...
	const createSellListingMutation = useMutation({
		...
		onSuccess: () => {
			toast({
				title: "Success",
				description: "Sell listing created successfully",
			});
		...
		onError: (error: any) => {
			toast({
				...
				description:
					error.message ||
					"Please ensure you meet KYC requirements and acknowledge all risks",
			});
		},
	});
	...
	const handleSubmitSellListing = () => {
		...
		const transactionValue =
			Number.parseFloat(sellForm.askPrice) * Number.parseInt(sellForm.quantity);
		...
		if (requiresRiskAck && !allSellRisksAcknowledged) {
			toast({
				...
				description:
					"Please acknowledge all mandatory risk disclosures before proceeding",
			});
			return;
		}
		...
		createSellListingMutation.mutate({
			...
			quantity: Number.parseInt(sellForm.quantity),
			askPrice: sellForm.askPrice,
			floorPrice: sellForm.floorPrice,
			couponRate: sellForm.couponRate
				? Number.parseFloat(sellForm.couponRate)
				: null,
	...
	const toggleSellRiskAck = (category: string) => {
		setSellRiskAcknowledgments((prev) => ({
			...prev,
	...
	const allRisksAcknowledged = requiredRiskCategories.every(
		(cat) => riskAcknowledgments[cat],
	);
	...
	const handleSubmitBuyRequest = () => {
		...
		const transactionValue =
			Number.parseFloat(selectedListing.askPrice) *
			Number.parseInt(buyQuantity);
		const requiresRiskAck =
			!selectedListing.isListed || transactionValue > 5000000;
		...
		createBuyRequestMutation.mutate({
			...
			quantity: Number.parseInt(buyQuantity),
			...
			<div className="flex justify-between items-center">
				<div>
					...
					<p className="text-muted-foreground text-sm">
						Buy and sell bonds in our secondary market
					</p>
				</div>
				<Dialog
					open={showCreateSellDialog}
					onOpenChange={setShowCreateSellDialog}
				>
					<DialogTrigger asChild>
					...
					<DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
						<DialogHeader>
							...
							<DialogDescription>
								List your bonds for sale in the marketplace. A buyer will be
						...
						<div className="space-y-4 py-4">
							...
							<div className="grid grid-cols-2 gap-4">
								<div>
									...
									<Input
										...
										value={sellForm.isin}
										onChange={(e) =>
											setSellForm((p) => ({ ...p, isin: e.target.value }))
										}
										placeholder="e.g., INE123A01234"
										data-testid="input-sell-isin"
									/>
								</div>
								<div>
									...
									<Input
										id="sell-bondName"
										value={sellForm.bondName}
										onChange={(e) =>
											setSellForm((p) => ({ ...p, bondName: e.target.value }))
										}
										placeholder="e.g., HDFC NCD 2028"
										data-testid="input-sell-bondName"
									/>
								</div>
							</div>
							...
							<div className="grid grid-cols-2 gap-4">
								<div>
									...
									>
										<SelectTrigger data-testid="select-sell-bondType">
											<SelectValue />
										</SelectTrigger>
										<SelectContent>
								...
								<div>
									...
									<Select
										value={sellForm.creditRating}
										onValueChange={(v) =>
											setSellForm((p) => ({ ...p, creditRating: v }))
										}
									>
										<SelectTrigger data-testid="select-sell-creditRating">
											<SelectValue />
										</SelectTrigger>
										<SelectContent>
							...
							<div className="grid grid-cols-3 gap-4">
								<div>
									...
									<Input
										id="sell-faceValue"
										type="number"
										value={sellForm.faceValue}
										onChange={(e) =>
											setSellForm((p) => ({ ...p, faceValue: e.target.value }))
										}
										placeholder="e.g., 1000"
										data-testid="input-sell-faceValue"
									/>
								</div>
								<div>
									...
									<Input
										...
										onChange={(e) =>
											setSellForm((p) => ({ ...p, quantity: e.target.value }))
										}
										placeholder="e.g., 100"
								...
								<div>
									...
									<Input
										...
										type="number"
										step="0.01"
										value={sellForm.couponRate}
										onChange={(e) =>
											setSellForm((p) => ({ ...p, couponRate: e.target.value }))
										}
										placeholder="e.g., 8.5"
										data-testid="input-sell-couponRate"
									/>
								</div>
							</div>
							...
							<div className="grid grid-cols-2 gap-4">
								<div>
									...
									<Input
										id="sell-askPrice"
										type="number"
										value={sellForm.askPrice}
										onChange={(e) =>
											setSellForm((p) => ({ ...p, askPrice: e.target.value }))
										}
										placeholder="e.g., 1050"
										data-testid="input-sell-askPrice"
									/>
								</div>
								<div>
									...
									<Input
										id="sell-floorPrice"
										type="number"
										value={sellForm.floorPrice}
										onChange={(e) =>
											setSellForm((p) => ({ ...p, floorPrice: e.target.value }))
										}
										placeholder="e.g., 1000"
										data-testid="input-sell-floorPrice"
									/>
								</div>
							</div>
							...
							<div className="grid grid-cols-2 gap-4">
								<div>
									...
									<Input
										id="sell-maturityDate"
										type="date"
										value={sellForm.maturityDate}
										onChange={(e) =>
											setSellForm((p) => ({
												...p,
										...
										data-testid="input-sell-maturityDate"
									/>
								</div>
								<div>
									<Label htmlFor="sell-dematAccount">
										Demat Account Number
									</Label>
									<Input
										id="sell-dematAccount"
										value={sellForm.dematAccountNumber}
										onChange={(e) =>
											setSellForm((p) => ({
												...p,
										...
										data-testid="input-sell-dematAccount"
									/>
								</div>
							</div>
							...
							<div className="flex items-center gap-2">
								<input
									type="checkbox"
									id="sell-isListed"
									checked={sellForm.isListed}
									onChange={(e) =>
										setSellForm((p) => ({ ...p, isListed: e.target.checked }))
									}
									data-testid="checkbox-sell-isListed"
								/>
								<Label htmlFor="sell-isListed">
									Bond is listed on exchange
								</Label>
							</div>
							...
							<div className="border rounded-lg p-4 space-y-3">
								...
								<p className="text-sm text-muted-foreground">
									You must acknowledge the following risks before listing bonds
									for sale:
								</p>
								<div className="space-y-2">
									{requiredRiskCategories.map((category) => (
										<div key={category} className="flex items-start gap-2">
											<input
												type="checkbox"
												id={`sell-risk-${category}`}
												checked={sellRiskAcknowledgments[category] || false}
												onChange={() => toggleSellRiskAck(category)}
												className="mt-1"
												data-testid={`checkbox-sell-risk-${category}`}
											/>
											<label
												htmlFor={`sell-risk-${category}`}
												className="text-sm"
											>
												...
												{category === "credit" &&
													"I understand pricing may reflect issuer credit changes."}
												{category === "liquidity" &&
													"I understand my listing may not attract buyers immediately."}
												{category === "interest_rate" &&
													"I understand bond values fluctuate with rates."}
												{category === "default" &&
													"I confirm the bond has no current defaults."}
												{category === "regulatory" &&
													"I confirm compliance with SEBI/RBI regulations."}
											</label>
										</div>
									))}
								</div>
							...
							<Button
								onClick={handleSubmitSellListing}
								disabled={
									createSellListingMutation.isPending ||
									!allSellRisksAcknowledged
								}
								className="w-full"
								data-testid="btn-submit-sell-listing"
							>
								{createSellListingMutation.isPending
									? "Submitting..."
									: "Create Sell Listing"}
					...
					{selectedListing && (
						<div className="space-y-4 py-4">
							...
							<div className="border rounded-lg p-4 space-y-3">
								...
								<p className="text-sm text-muted-foreground">
									You must acknowledge the following risks before proceeding
									...
									{requiredRiskCategories.map((category) => (
										<div key={category} className="flex items-start gap-2">
											...
											<label htmlFor={`risk-${category}`} className="text-sm">
												...
												{category === "liquidity" &&
													"Bonds may be difficult to sell before maturity."}
												{category === "interest_rate" &&
													"Bond values fluctuate with interest rate changes."}
												{category === "default" &&
													"There is a risk of complete loss of principal."}
												{category === "regulatory" &&
													"Regulatory changes may affect bond value or taxability."}
											</label>
										</div>
									))}
								</div>
							...
							<Button
								...
								disabled={
									createBuyRequestMutation.isPending ||
				...
				<CardContent>
					{loadingSell ? (
						...
						<div className="text-center py-8 text-muted-foreground">
							...
							<p className="text-sm">
								Check back later or create a sell listing
							</p>
						</div>
					) : (
						<div className="space-y-4">
...
function BondHoldings() {
	...
	const totalValue = bonds.reduce(
		(sum: number, bond: any) => sum + (bond.currentValue || 0),
		0,
	);
	const totalInvested = bonds.reduce(
		(sum: number, bond: any) => sum + (bond.purchaseValue || 0),
		0,
	);
						...
						<div className="flex items-start justify-between">
							<div className="flex-1">
								<div className="flex items-center gap-3 mb-2">
									...
									<Badge variant="outline">{holding.bondType}</Badge>
								</div>
								...
								<div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm mt-3">
									...
									<div>
										...
										<p className="font-semibold text-finance-green">
											{holding.yieldToMaturity || holding.currentYield || "N/A"}
											%
										</p>
									</div>
									<div>
							...
							<div className="text-right">
								...
								<p
									className={`text-lg font-semibold ${holding.currentValue - holding.purchaseValue >= 0 ? "text-finance-green" : "text-red-600"}`}
								>
...
function BondOrders() {
	...
	const cancelOrderMutation = useMutation({
		mutationFn: async (orderId: string) => {
			const res = await apiRequest(`/api/bonds/orders/${orderId}/cancel`, {
				method: "POST",
			});
			return res.json();
		},
		...
		return (
			<Card className="border-dashed mt-6">
				<CardContent className="flex flex-col items-center justify-center py-8">
					...
					<p className="text-xs text-muted-foreground mt-1">
						Your bond orders will appear here
					</p>
				</CardContent>
			</Card>
		);
...
export default function Bonds() {
	...
	const { data: catalogData, isLoading: loadingUnifiedBonds } =
		useQuery<EnhancedCatalogResponse>({
			queryKey: ["/api/bonds/enhanced-catalog", catalogQueryString],
			queryFn: async () => {
				const response = await fetch(
					`/api/bonds/enhanced-catalog${catalogQueryString}`,
					{
						credentials: "include",
					},
				);
	...
	const rawBonds =
		(catalogData as any)?.data?.bonds ?? catalogData?.bonds ?? [];
	...
	const selectedBondsData = useMemo(() => {
		return unifiedBonds.filter((bond: any) =>
	...
	const calculateReturns = () => {
		...
		const rate = Number.parseFloat(bondYield) / 100 || 0;
				...
				<Tabs defaultValue="explore" className="space-y-8">
					...
					>
						...
						<div className="text-center mb-6">
							...
							<p className="text-muted-foreground">
								Click on a category to view available bonds
							</p>
						</div>
									...
									<div className="space-y-6">
										...
										<div className="grid grid-cols-2 gap-4">
											<div className="text-center p-4 bg-green-50 dark:bg-green-950/30 rounded-lg">
												...
												<p
													className="text-lg font-bold text-finance-green"
										...
										<Dialog>
											...
											<DialogContent className="max-w-5xl max-h-[80vh] overflow-y-auto">
												...
												<BondComparisonTable
													...
													onRemove={(bondId) => {
														const bond = selectedBondsData.find(
															(b: any) => b.id === bondId,
														);
														if (bond) {
															setSelectedBonds((prev) =>
																prev.filter((isin) => isin !== bond.isin),
															);
														}
													}}
												/>
													...
													<div className="flex-1">
														<div className="flex items-start justify-between">
															...
															<div className="flex gap-2">
																...
																<Badge
																	className={getBondTypeColors(bond.bondType)}
																>
																	{bond.bondType}
																</Badge>
															</div>
														...
														<div className="flex items-center gap-2 mt-4">
															{bond.riskLevel && (
																<Badge
																	className={getRiskLevelColors(bond.riskLevel)}
																>
																	{bond.riskLevel} Risk
																</Badge>
															)}
															<Button
																variant="outline"
																size="sm"
																data-testid={`view-bond-${bond.isin}`}
															>
																View Details
															</Button>
														</div>
							...
							<Card>
								<CardContent className="p-6">
									...
									<p className="text-muted-foreground mb-4">
										Bonds are debt securities where you lend money to an issuer
										(government or corporation) for a defined period at a fixed
										interest rate.
									</p>
