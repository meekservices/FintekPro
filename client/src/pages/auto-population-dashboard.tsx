...
const DATA_SOURCE_CONFIG = {
	mutual_funds: {
		...
		label: "Mutual Funds",
		color: "text-blue-600",
		provider: "BSE STAR MFD API",
		description: "Track all your mutual fund investments across AMCs",
	},
	demat: {
		icon: Wallet,
		...
		provider: "NSDL/CDSL",
		description: "View your stocks, ETFs, and bonds from demat accounts",
	},
	bank: {
		icon: Building,
		label: "Bank Accounts",
		...
		description: "Aggregate savings, current, and FD accounts",
	},
	loans: {
		icon: CreditCard,
		...
		provider: "CIBIL",
		description: "Track home loans, personal loans, and credit cards",
	},
	insurance: {
		icon: LucideShield,
		label: "Insurance Policies",
		color: "text-indigo-600",
		provider: "Turtlefin API",
		description: "View life, health, and general insurance policies",
	},
	epf: {
		icon: Wallet,
		label: "EPF/VPF Account",
		color: "text-amber-600",
		provider: "EPFO",
		description: "Track your provident fund balance and contributions",
	},
	nps: {
		icon: TrendingUp,
		label: "NPS Account",
		color: "text-cyan-600",
		provider: "NPS CRA",
		description: "National Pension System investments and balance",
	},
	apy: {
		icon: LucideShield,
		label: "APY Benefits",
		color: "text-rose-600",
		provider: "NSDL/APY",
		description: "Atal Pension Yojana pension benefits",
	},
};
...
export default function AutoPopulationDashboard() {
	...
	const grantConsentMutation = useMutation({
		...
		onSuccess: () => {
			toast({
				...
				description:
					"Data source access approved. Your data will be fetched shortly.",
			});
	...
	const retrySingleSourceMutation = useMutation({
		...
		onSuccess: (_data, variables) => {
			...
			queryClient.invalidateQueries({
				queryKey: ["/api/auto-populate/status", userId],
			});
		},
		onError: (error: any) => {
			toast({
	...
	const currentStatus =
		statusData?.status || workflows.find((w) => w.id === selectedWorkflow);
		...
		<div className="container mx-auto p-6 space-y-6">
						...
						<AutoPopulationProgressIndicator
							...
							onComplete={(status) => {
								toast({
									...
									variant:
										status.status === "completed" ? "default" : "destructive",
								});
								queryClient.invalidateQueries({
									queryKey: ["/api/auto-populate/workflows", userId],
								});
												...
												<div className="flex items-center justify-between">
													...
													{consentStatus === "active" && (
														<Badge
															...
															data-testid={`badge-consent-active-${key}`}
														>
															Active
														</Badge>
													)}
													{consentStatus === "expired" && (
														<Badge
															variant="secondary"
															data-testid={`badge-consent-expired-${key}`}
														>
															Expired
														</Badge>
													)}
													{consentStatus === "revoked" && (
														<Badge
															variant="destructive"
															data-testid={`badge-consent-revoked-${key}`}
														>
															Revoked
														</Badge>
													)}
													{consentStatus === "not_granted" && (
														<Badge
															variant="outline"
															data-testid={`badge-consent-not-granted-${key}`}
														>
															Not Granted
														</Badge>
													)}
						...
						<CardContent>
							{consentsLoading ? (
								...
								<Alert>
									...
									<AlertDescription>
										No consents granted yet. Grant consent to data sources to
												...
												<div className="flex items-center gap-4">
													...
													<div>
														<p className="font-medium">
															{config?.label || consent.dataSource}
														</p>
												...
												<div className="flex items-center gap-2">
													...
													{consent.status === "expired" && (
														<Button
															...
															size="sm"
															onClick={() =>
																renewConsentMutation.mutate(consent.dataSource)
															}
															disabled={renewConsentMutation.isPending}
					...
					<Card data-testid="card-workflows-history">
						...
						<CardContent>
							{workflowsLoading ? (
								...
								<Alert>
									<AlertDescription>
										No workflows yet. Start your first auto-population to see
										results here.
									</AlertDescription>
								</Alert>
							) : (
									...
									{workflows.map((workflow) => (
										<Collapsible key={workflow.id}>
														...
														<div className="flex items-center gap-3">
															<div className="text-right">
																...
																<p className="text-xs text-muted-foreground">
																	{workflow.summary.successfulSources}/
																	{workflow.summary.totalSources} sources
																</p>
															</div>
															<Badge
																variant={
											...
											<CollapsibleContent>
												...
												{workflow.sourceStatuses &&
													Object.keys(workflow.sourceStatuses).length > 0 && (
														<div className="mt-2 ml-4 p-3 border-l-2 border-muted space-y-2">
															{Object.entries(workflow.sourceStatuses).map(
																([source, status]: [string, any]) => {
																	const config =
																		DATA_SOURCE_CONFIG[
																					...
																					<>
																						...
																						{status?.retryable !== false && (
																							<Button
																								...
																								onClick={(e) => {
																									e.stopPropagation();
																									retrySingleSourceMutation.mutate(
																										source,
																									);
																								}}
																								disabled={
																									retrySingleSourceMutation.isPending
																								}
																								data-testid={`button-retry-${source}`}
																							>
																								<RotateCcw
																									className={`h-3 w-3 mr-1 ${retrySingleSourceMutation.isPending ? "animate-spin" : ""}`}
																								/>
																								Retry
																							</Button>
																						)}
			...
			{selectedSource && (
				<ConsentPreviewDialog
					...
					sourceLabel={
						DATA_SOURCE_CONFIG[selectedSource]?.label || selectedSource
					}
			...
			{selectedConsent && (
				<RevokeConsentDialog
					...
					sourceLabel={
						DATA_SOURCE_CONFIG[selectedConsent.dataSource]?.label ||
						selectedConsent.dataSource
					}
					onConfirm={(reason) =>
			...
			{selectedConsent && (
				<ConsentSettingsDrawer
					...
					sourceDescription={
						DATA_SOURCE_CONFIG[selectedConsent.dataSource]?.description || ""
					}
					onRevoke={() => {
						setSettingsDrawerOpen(false);
