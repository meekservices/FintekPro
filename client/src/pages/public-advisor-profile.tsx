			...
			<div className="min-h-screen flex items-center justify-center">
				<div className="text-center space-y-3">
					...
					<p className="text-muted-foreground text-sm">
						This advisor profile is either private or doesn't exist.
					</p>
					<Button
						variant="outline"
			...
			<div className="max-w-2xl mx-auto px-4 py-8 space-y-5">
					...
					<Card>
						<CardContent className="py-4">
							<h3 className="text-sm font-semibold mb-3 flex items-center gap-2">
								<LucideShield className="h-4 w-4 text-primary" /> Regulatory
								Registrations
							</h3>
							<div className="flex flex-wrap gap-2">
								{profile.arnCode && (
									...
									>
										<CheckCircle className="h-3 w-3" /> AMFI ARN:{" "}
										{profile.arnCode}
									</Badge>
								)}
								{profile.sebiRegNumber && (
									<Badge
										variant="outline"
										className="flex items-center gap-1 text-blue-600 border-blue-500"
									>
								...
								{profile.irdaiRegNumber && (
									<Badge
										variant="outline"
										className="flex items-center gap-1 text-purple-600 border-purple-500"
									>
				...
				<div className="grid grid-cols-3 gap-3">
					{[
						...
						{
							label: "Clients",
							value: profile.activeClients ? `${profile.activeClients}+` : "—",
							icon: Users,
						},
						{
							label: "AUM",
							value: profile.aumManaged
								? `₹${Number(profile.aumManaged).toFixed(0)} Cr`
								: "—",
							icon: IndianRupee,
						},
					].map((s) => (
				...
				<div className="text-center text-xs text-muted-foreground pt-4 border-t">
					<p>
						Verified advisor profile powered by <strong>FintekPro</strong>
					</p>
