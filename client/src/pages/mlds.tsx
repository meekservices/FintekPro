...
function ProposalsTab({
	...
	const {
		...
		isLoading,
		refetch,
	} = useQuery<any[]>({ queryKey: ["/api/proposals", { productType }] });

	const approveMutation = useMutation({
		mutationFn: async (proposalId: string) =>
			apiRequest(`/api/proposals/${proposalId}/approve`, { method: "POST" }),
		onSuccess: () => {
			toast({
			...
			<Card className="border-dashed border-2 border-teal-200 dark:border-teal-800 bg-teal-50 dark:bg-teal-950/30/50">
				<CardContent className="flex flex-col items-center justify-center py-16">
					...
					<h3 className="text-xl font-semibold mb-2">
						No Pending MLD Proposals
					</h3>
					<p className="text-muted-foreground text-center max-w-md mb-4">
						AI and agent recommendations for Market Linked Debentures will
						appear here.
					</p>
					<Button
						variant="outline"
						onClick={() => refetch()}
						className="border-teal-300 dark:border-teal-700 text-teal-600"
					>
		...
		<div className="space-y-4">
			{pendingProposals.map((proposal) => (
				<Card
					...
					data-testid={`mld-proposal-${proposal.id}`}
				>
					<CardContent className="p-0">
						<div className="flex">
							<div
								className={`w-2 ${proposal.proposalSource === "ai" ? "bg-gradient-to-b from-teal-500 to-cyan-600" : "bg-gradient-to-b from-emerald-500 to-green-600"}`}
							/>
							<div className="flex-1 p-6">
								...
								<div className="grid grid-cols-4 gap-4 mb-4 p-4 bg-muted rounded-lg">
									<div className="text-center">
										...
										<p className="text-lg font-bold text-green-600">
											{proposal.capitalProtection || "100%"}
										</p>
									</div>
									<div className="text-center">
										<p className="text-sm text-muted-foreground">
										...
										<p className="text-lg font-bold">
											{proposal.participationRate || "80%"}
										</p>
									</div>
									<div className="text-center">
										<p className="text-sm text-muted-foreground">Tenure</p>
										<p className="text-lg font-bold">
											{proposal.tenure || "3 Years"}
										</p>
									</div>
									<div className="text-center">
										<p className="text-sm text-muted-foreground">Underlying</p>
										<p className="text-lg font-bold text-blue-600">
											{proposal.underlyingIndex || "NIFTY 50"}
										</p>
									</div>
								</div>
								...
								<div className="flex gap-3">
									<Button
										...
										data-testid={`approve-mld-${proposal.id}`}
									>
									...
									<Button
										...
										data-testid={`reject-mld-${proposal.id}`}
									>
...
function CartTab({
	...
	const checkoutMutation = useMutation({
		mutationFn: async () => {
			for (const item of cartItems) {
				if (item.proposalId) {
					await apiRequest(`/api/proposals/${item.proposalId}/complete-order`, {
						method: "POST",
						body: JSON.stringify({ orderType: "LUMPSUM", productType: "mld" }),
					});
		...
		onSuccess: () => {
			toast({
				title: "Order Placed!",
				description: "Your MLD investment order has been submitted",
			});
			queryClient.invalidateQueries({ queryKey: ["/api/cart"] });
	...
	const totalValue = cartItems.reduce(
		(sum: number, item: any) => sum + Number.parseFloat(item.amount || "0"),
		0,
	);
			...
			<Card className="border-dashed border-2 border-teal-200 dark:border-teal-800 bg-teal-50 dark:bg-teal-950/30/50">
				<CardContent className="flex flex-col items-center justify-center py-16">
					...
					<p className="text-muted-foreground text-center max-w-md">
						Approve proposals to add them to cart.
					</p>
				</CardContent>
			</Card>
		);
							...
							<div className="flex items-start justify-between">
								<div className="flex-1">
									<div className="flex items-center gap-2 mb-2">
										...
										<h4 className="font-semibold">
											{item.productName || "MLD Investment"}
										</h4>
										<Badge className="bg-teal-100 dark:bg-teal-900/30 text-teal-700 dark:text-teal-300">
											MLD
										</Badge>
								...
								<div className="text-right">
									<p className="text-xl font-bold flex items-center justify-end">
										...
										{Number.parseFloat(item.amount || "0").toLocaleString(
											"en-IN",
										)}
									</p>
									<Button
							...
							<div className="flex items-start gap-2">
								...
								<div>
									...
									<p className="text-amber-700 dark:text-amber-300 text-xs">
										Minimum ₹1 Lakh. Principal protection at maturity.
									</p>
								</div>
							</div>
						</div>
						<Button
							className="w-full bg-gradient-to-r from-teal-500 to-cyan-500 text-foreground font-semibold py-6"
							onClick={() => checkoutMutation.mutate()}
							disabled={checkoutMutation.isPending}
							data-testid="mld-checkout-btn"
						>
					...
					<CardContent className="p-6">
						<div className="flex items-start justify-between mb-4">
							<div>
								<h4 className="font-semibold text-lg">
									{order.productName || order.schemeName}
								</h4>
								<p className="text-sm text-muted-foreground">
						...
						<div className="grid grid-cols-4 gap-4 text-sm">
							...
							<div>
								...
								<p className="font-semibold">{order.tenure || "3 Years"}</p>
							</div>
							<div>
				...
				<Card key={holding.id}>
					<CardContent className="p-6">
						<div className="flex justify-between items-start">
							<div>
								<h4 className="font-semibold">{holding.productName}</h4>
								<p className="text-sm text-muted-foreground">
									{holding.issuer}
								</p>
							</div>
							<div className="text-right">
								...
								<p className="text-sm text-muted-foreground">
									Maturity: {holding.maturityDate || "N/A"}
								</p>
							</div>
						</div>
					</CardContent>
...
export default function MLDs() {
	...
	const { data: mldProducts, isLoading } = useQuery<any[]>({
		queryKey: [
			...
			{
				protection:
					selectedProtection !== "all" ? selectedProtection : undefined,
				tenure: selectedTenure !== "all" ? selectedTenure : undefined,
	...
	const pendingProposals =
		proposalData?.filter(
			(p) => p.status === "pending" && p.productType === "mld",
		)?.length || 0;
	const cartCount =
		cartData?.items?.filter((i: any) => i.productCategory === "mld")?.length ||
		0;
	...
	if (isLoading)
		return (
			<div
				className="min-h-screen bg-finance-light p-8"
				data-testid="mlds-page"
			>
			...
			<main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
				<div className="mb-8">
					...
					<p className="text-muted-foreground text-lg">
						Structured debt instruments linked to market indices with capital
						protection options.
					</p>
				</div>
							...
							<div className="flex items-center justify-between">
								<div>
									...
									<p className="text-3xl font-bold text-green-600">
										{statistics.avgProtection}
									</p>
								</div>
								<LucideShield className="w-10 h-10 text-green-600" />
							</div>
						</CardContent>
					</Card>
					<Card>
				...
				<Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
					<ScrollableTabsList className="grid w-full grid-cols-6 mb-6">
						...
						<TabsTrigger value="proposals" data-testid="tab-proposals">
							...
							Proposals
							{pendingProposals > 0 && (
								<Badge className="ml-1 bg-teal-500 text-white text-xs">
									{pendingProposals}
								</Badge>
							)}
						</TabsTrigger>
					...
					<TabsContent value="schemes" className="space-y-6">
						<div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
							<Card>
								...
								<CardContent>
									<p className="text-sm text-muted-foreground">
										Returns based on underlying index performance (NIFTY,
										SENSEX, etc.)
									</p>
								</CardContent>
							</Card>
							<Card>
								<CardHeader>
									<div className="flex items-center gap-2">
										<LucideShield className="h-5 w-5 text-finance-green" />
										<CardTitle className="text-lg">
											Capital Protection
										</CardTitle>
									</div>
								...
								<CardContent>
									<p className="text-sm text-muted-foreground">
										Options with 100%, 90%, or 80% principal protection at
										maturity
									</p>
								</CardContent>
							</Card>
						...
						<div className="flex flex-wrap gap-4 mb-6">
							<div className="flex-1 min-w-[200px]">
								<div className="relative">
									...
									<Input
										placeholder="Search MLDs..."
										value={searchQuery}
										onChange={(e) => setSearchQuery(e.target.value)}
										className="pl-10"
										data-testid="search-mld"
									/>
								</div>
							</div>
							<Select
								value={selectedProtection}
								onValueChange={setSelectedProtection}
							>
						...
						<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
							{displayData.length > 0 ? (
										...
										<CardContent className="space-y-4">
											<div className="space-y-2">
												<div className="flex justify-between text-sm">
													...
													<span className="font-semibold">
														{product.benchmarkIndex || "NIFTY 50"}
													</span>
												</div>
												<div className="flex justify-between text-sm">
													<span className="text-muted-foreground">
												...
												<div className="flex justify-between text-sm">
													...
													<span className="font-semibold">
														{product.participationRate || "80%"}
													</span>
												</div>
												<div className="flex justify-between text-sm">
													<span className="text-muted-foreground">Tenure:</span>
													<span className="font-semibold">
														{product.tenure || "3 years"}
													</span>
												</div>
												<div className="flex justify-between text-sm">
													<span className="text-muted-foreground">
											...
											<div className="flex gap-2">
												...
												<Button
													variant="outline"
													size="sm"
													onClick={() => navigate(`/mld/${product.id}`)}
								...
								<div className="col-span-full text-center py-12">
									<p className="text-muted-foreground">
										No MLDs available. Check back soon!
									</p>
								</div>
							)}
						</div>
						...
						<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
							...
							>
								<CardContent className="p-6 text-center">
									...
									<h3 className="font-semibold text-lg mb-2">MLD Calculator</h3>
									<p className="text-sm text-muted-foreground">
							...
							<Card className="hover:shadow-lg cursor-pointer">
								<CardContent className="p-6 text-center">
									...
									<h3 className="font-semibold text-lg mb-2">Risk Analyzer</h3>
									<p className="text-sm text-muted-foreground">
				...
				<Card className="mt-8">
					...
					<CardContent className="space-y-4">
						<div>
							...
							<p className="text-sm text-muted-foreground">
								Market Linked Debentures are structured debt securities where
								returns are linked to the performance of underlying market
								indices like NIFTY 50, SENSEX, or sectoral indices. They offer a
								blend of debt security with equity-like upside potential.
							</p>
						</div>
						<div>
							...
							<ul className="text-sm text-muted-foreground space-y-1 list-disc list-inside">
								<li>
									Capital protection options (100%, 90%, or 80% of principal)
								</li>
								<li>
									Participation in index upside with defined participation rate
								</li>
								<li>Fixed tenure (typically 1-5 years)</li>
								<li>Tax efficiency for long-term holdings</li>
							</ul>
						</div>
					</CardContent>
				</Card>
