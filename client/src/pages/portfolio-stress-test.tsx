...
export default function PortfolioStressTest() {
	...
	const stressedPortfolio = useMemo(() => {
		...
		return portfolio.map((holding) => {
			...
			switch (holding.type) {
				case "equity":
					changePercent = params.equityChange;
					break;
				case "debt":
					changePercent = params.debtChange;
					break;
				case "gold":
					changePercent = params.goldChange;
					break;
				case "real_estate":
					changePercent = params.realEstateChange;
					break;
				default:
					changePercent = 0;
			}
	...
	const stressedTotal =
		stressedPortfolio?.reduce((sum, h) => sum + h.stressedValue, 0) || 0;
				...
				<Card className="text-center">
					<CardContent className="pt-6">
						...
						<p className="text-muted-foreground mb-4">
							Please log in to access portfolio stress testing.
						</p>
						<Link href="/auth">
							<Button data-testid="stress-test-login-btn">
								Login to Continue
							</Button>
						</Link>
					</CardContent>
	...
	return (
		<div
			...
			data-testid="portfolio-stress-test-page"
		>
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-muted-foreground mt-1">
						Analyze how your portfolio would perform under various market
						conditions
					</p>
				</div>
								...
								<CardContent>
									<div className="grid grid-cols-2 gap-2 text-sm">
										<div className="flex items-center justify-between">
											...
											<span
												className={
													scenario.parameters.equityChange >= 0
														? "text-green-600"
														: "text-red-600"
												}
											>
												{scenario.parameters.equityChange > 0 ? "+" : ""}
												{scenario.parameters.equityChange}%
											</span>
										</div>
										<div className="flex items-center justify-between">
											...
											<span
												className={
													scenario.parameters.debtChange >= 0
														? "text-green-600"
														: "text-red-600"
												}
											>
												{scenario.parameters.debtChange > 0 ? "+" : ""}
												{scenario.parameters.debtChange}%
											</span>
										</div>
										<div className="flex items-center justify-between">
											...
											<span
												className={
													scenario.parameters.goldChange >= 0
														? "text-green-600"
														: "text-red-600"
												}
											>
												{scenario.parameters.goldChange > 0 ? "+" : ""}
												{scenario.parameters.goldChange}%
											</span>
										</div>
										<div className="flex items-center justify-between">
											...
											<span
												className={
													scenario.parameters.realEstateChange >= 0
														? "text-green-600"
														: "text-red-600"
												}
											>
												{scenario.parameters.realEstateChange > 0 ? "+" : ""}
												{scenario.parameters.realEstateChange}%
											</span>
										</div>
									</div>
								</CardContent>
				...
				<TabsContent value="custom" className="space-y-6">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Adjust asset class returns to create your own scenario
							</CardDescription>
						</CardHeader>
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
								<div className="space-y-3">
									<div className="flex items-center justify-between">
										...
										<span
											className={
												customParameters.equityChange >= 0
													? "text-green-600"
													: "text-red-600"
											}
										>
											{customParameters.equityChange > 0 ? "+" : ""}
											{customParameters.equityChange}%
										</span>
									</div>
									<Slider
										value={[customParameters.equityChange]}
										onValueChange={([v]) =>
											setCustomParameters((p) => ({ ...p, equityChange: v }))
										}
										min={-60}
										max={60}
										step={1}
								...
								<div className="space-y-3">
									<div className="flex items-center justify-between">
										...
										<span
											className={
												customParameters.debtChange >= 0
													? "text-green-600"
													: "text-red-600"
											}
										>
											{customParameters.debtChange > 0 ? "+" : ""}
											{customParameters.debtChange}%
										</span>
									</div>
									<Slider
										value={[customParameters.debtChange]}
										onValueChange={([v]) =>
											setCustomParameters((p) => ({ ...p, debtChange: v }))
										}
										min={-30}
										max={30}
										step={1}
								...
								<div className="space-y-3">
									<div className="flex items-center justify-between">
										...
										<span
											className={
												customParameters.goldChange >= 0
													? "text-green-600"
													: "text-red-600"
											}
										>
											{customParameters.goldChange > 0 ? "+" : ""}
											{customParameters.goldChange}%
										</span>
									</div>
									<Slider
										value={[customParameters.goldChange]}
										onValueChange={([v]) =>
											setCustomParameters((p) => ({ ...p, goldChange: v }))
										}
										min={-30}
										max={50}
										step={1}
								...
								<div className="space-y-3">
									<div className="flex items-center justify-between">
										...
										<span
											className={
												customParameters.realEstateChange >= 0
													? "text-green-600"
													: "text-red-600"
											}
										>
											{customParameters.realEstateChange > 0 ? "+" : ""}
											{customParameters.realEstateChange}%
										</span>
									</div>
									<Slider
			...
			{showResults && stressedPortfolio && (
				<div className="space-y-6" data-testid="stress-test-results">
					<Card
						className={`border-2 ${totalImpact >= 0 ? "border-green-500 bg-green-50/50 dark:bg-green-950/50" : "border-red-500 bg-red-50/50 dark:bg-red-950/50"}`}
					>
						<CardContent className="pt-6">
								...
								{stressedPortfolio.map((holding, idx) => (
									...
									>
										<div className="flex items-center gap-3">
											...
											<div>
												...
												<p className="text-sm text-muted-foreground">
													{(holding.type || "equity").toUpperCase()}
												</p>
											</div>
										</div>
										<div className="text-right">
											...
											<p
												className={`text-sm ${holding.change >= 0 ? "text-green-600" : "text-red-600"}`}
											>
												{holding.change >= 0 ? (
