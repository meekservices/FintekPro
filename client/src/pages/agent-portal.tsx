...
const clientFormSchema = z.object({
	...
	panNumber: z
		.string()
		.regex(/^[A-Z]{5}[0-9]{4}[A-Z]{1}$/, "Valid PAN number is required"),
												...
												<FormControl>
													...
													>
														...
														<SelectContent>
															...
															<SelectItem value="1_5_lakh">
																₹1-5 Lakh
															</SelectItem>
															<SelectItem value="5_10_lakh">
																₹5-10 Lakh
															</SelectItem>
															<SelectItem value="10_25_lakh">
																₹10-25 Lakh
															</SelectItem>
															<SelectItem value="25_50_lakh">
																₹25-50 Lakh
															</SelectItem>
															<SelectItem value="above_50_lakh">
																Above ₹50 Lakh
															</SelectItem>
														</SelectContent>
													</Select>
											...
											<FormItem>
												...
												<FormControl>
													<Input
														...
														data-testid="input-client-account"
														{...field}
													/>
												</FormControl>
												<FormMessage />
...
export default function AgentPortal() {
	...
	if (userError || (!userLoading && !currentUser)) {
		return (
			<div
				className="min-h-screen bg-gradient-to-br from-background via-blue-50/30 to-indigo-50/30 dark:from-background dark:via-blue-950/30 dark:to-indigo-950/30 flex items-center justify-center"
				data-testid="agent-portal-login"
			>
				<Card className="w-full max-w-md">
					...
					<CardContent className="space-y-4">
						<p className="text-sm text-muted-foreground">
							The Partner Portal provides tools for managing clients, partners,
							and commissions.
						</p>
						<Button
							className="w-full"
	...
	const handleAddClient = (data: ClientFormData) => {
		...
		const clientData = {
			...data,
			masterAgentEuin: !data.assignedAgent
	...
	const canManagePartners = userRoles.some((role: string) =>
		["agent", "partner", "admin", "superadmin"].includes(role),
	);
	const canManageClients = userRoles.some((role: string) =>
		["agent", "partner", "admin", "superadmin"].includes(role),
	);
					...
					<div className="flex items-start sm:items-center justify-between flex-wrap gap-3">
						<div>
							<h1
								...
								data-testid="title-partner-portal"
							>
								Partner Portal
							</h1>
						...
						<div className="text-right">
							...
							<p className="text-xs text-muted-foreground">
								Partner ID: {agentProfile?.employeeId}
							</p>
						</div>
					</div>
				</div>
								...
								<div className="grid grid-cols-2 gap-3 sm:gap-6 lg:grid-cols-4">
									<Card data-testid="card-total-partners">
										...
										<CardContent>
											<div
												...
												data-testid="text-total-partners"
											>
									...
									<Card data-testid="card-monthly-commissions">
										...
										<CardContent>
											<div
												className="text-2xl font-bold"
												data-testid="text-monthly-commissions"
											>
							...
							<CardHeader>
								<div className="flex justify-between items-center">
									<div>
										...
										<CardDescription>
											Add and manage partners with or without EUIN/ARN numbers
										</CardDescription>
									</div>
									<Button
								...
								<Table>
									...
									<TableBody>
										{partnersData.map((partner: any, index: number) => (
											<TableRow
												key={partner.id}
												data-testid={`partner-row-${index + 1}`}
											>
												<TableCell>
													<div>
							...
							<CardHeader>
								<div className="flex justify-between items-center">
									<div>
										...
										<CardDescription>
											Manage client relationships and EUIN assignments
										</CardDescription>
									</div>
									<Button
										className="bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-700 hover:to-blue-700 text-foreground shadow-md"
								...
								<Table>
									...
									<TableBody>
										{clientsData.map((client: any, index: number) => (
											<TableRow
												key={client.id}
												data-testid={`client-row-${index + 1}`}
											>
												<TableCell>
													<div>
														<div className="font-medium">
													...
													<div className="flex space-x-1">
														<Button
															...
															onClick={() => {
																...
																navigate("/agent-client-onboarding");
															}}
						...
						<Card data-testid="card-reports">
							...
							<CardContent>
								<div className="grid gap-4 md:grid-cols-2">
									<Button
										variant="outline"
										className="h-20 flex-col border-blue-200 hover:bg-blue-50 hover:border-blue-300 dark:border-blue-800 dark:hover:bg-blue-950"
										onClick={() => {
											toast({
												title: "Generating Report",
												description: "Preparing Partner Report...",
											});
											navigate("/reports");
										}}
										data-testid="button-partner-report"
									>
									...
									<Button
										variant="outline"
										className="h-20 flex-col border-green-200 hover:bg-green-50 hover:border-green-300 dark:border-green-800 dark:hover:bg-green-950"
										onClick={() => {
											toast({
												title: "Generating Report",
												description: "Preparing Client Report...",
											});
											navigate("/reports");
										}}
										data-testid="button-client-report"
									>
									...
									<Button
										variant="outline"
										className="h-20 flex-col border-purple-200 hover:bg-purple-50 hover:border-purple-300 dark:border-purple-800 dark:hover:bg-purple-950"
										onClick={() => {
											toast({
												title: "Generating Report",
												description: "Preparing Commission Report...",
											});
											navigate("/reports");
										}}
									...
									<Button
										variant="outline"
										className="h-20 flex-col border-amber-200 hover:bg-amber-50 hover:border-amber-300 dark:border-amber-800 dark:hover:bg-amber-950"
										onClick={() => {
											toast({
												title: "Generating Report",
												description: "Preparing Activity Report...",
											});
											navigate("/reports");
										}}
										data-testid="button-activity-report"
									>
				...
				>
					<DialogContent className="max-w-2xl">
						<DialogHeader>
							...
							<DialogDescription>
								Add a new partner with or without EUIN/ARN numbers
							</DialogDescription>
						</DialogHeader>
						<Form {...partnerForm}>
							<form
								onSubmit={partnerForm.handleSubmit(handleAddPartner)}
								className="space-y-4"
							>
									...
									<div className="p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
										<div className="flex items-center gap-2">
											...
											<p className="text-sm text-yellow-800 dark:text-yellow-200">
												This partner will be mapped under your master EUIN:{" "}
												<strong>{agentProfile?.euinNumber}</strong>
											</p>
										</div>
									</div>
								)}
				...
				>
					<DialogContent className="max-w-xl">
						<DialogHeader>
							...
							<DialogDescription>
								Add a new client and assign EUIN mapping
							</DialogDescription>
						</DialogHeader>
						<Form {...clientForm}>
							<form
								onSubmit={clientForm.handleSubmit(handleAddClient)}
								className="space-y-4"
							>
								<div className="grid grid-cols-2 gap-4">
									<FormField
										...
										<FormItem>
											...
											<FormControl>
												<Input
													...
													data-testid="input-assigned-agent"
													{...field}
												/>
											</FormControl>
											<FormMessage />
									...
									<div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
										<div className="flex items-center gap-2">
											...
											<p className="text-sm text-blue-800 dark:text-blue-200">
												This client will be assigned to your master EUIN:{" "}
												<strong>{agentProfile?.euinNumber}</strong>
											</p>
										</div>
									</div>
