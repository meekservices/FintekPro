					...
					<div className="p-3 rounded-lg bg-muted/50 space-y-2 border">
						{error ? (
							<p className="text-xs text-destructive">
								Could not verify PAN. Ensure IRIS is configured.
							</p>
						) : kyc ? (
							<>
								<div className="flex justify-between items-center">
									...
									<Badge
										variant={
											kyc.kycStatus === "KYC_VERIFIED" ? "default" : "secondary"
										}
										className="text-[10px]"
									>
...
function statusVariant(
	...
	if (
		["ACTIVE", "COMPLETED", "SUCCESS", "KYC_VERIFIED", "ALLOTTED"].includes(s)
	)
		return "default";
	if (["PENDING", "IN_PROGRESS", "KYC_PENDING", "PROCESSING"].includes(s))
		return "secondary";
...
function DashboardTab() {
	const { data: aum, isLoading: aumL } = useQuery<IrisApiResponse<AumData>>({
		queryKey: ["/api/iris/dashboard/aum-summary"],
		retry: false,
	});
	const { data: earn, isLoading: earnL } = useQuery<
		IrisApiResponse<EarningsData>
	>({ queryKey: ["/api/iris/dashboard/fund-earnings"], retry: false });
	const { data: sip, isLoading: sipL } = useQuery<
		IrisApiResponse<SipSummaryData>
	>({ queryKey: ["/api/iris/dashboard/sip-summary"], retry: false });
	const { data: inv, isLoading: invL } = useQuery<
		IrisApiResponse<InvestorCountData>
	>({ queryKey: ["/api/iris/dashboard/unique-investors"], retry: false });
	const { data: inflowData, isLoading: inflowL } = useQuery<
		IrisApiResponse<{
			months?: InflowOutflowMonth[];
			data?: InflowOutflowMonth[];
		}>
	>({
		queryKey: ["/api/iris/dashboard/inflow-outflow"],
		retry: false,
	});
	const { data: euinsData, isLoading: euinsL } = useQuery<
		IrisApiResponse<{ euins?: EuinRecord[]; data?: EuinRecord[] }>
	>({
	...
	const riskDonutData: RiskDistItem[] =
		riskDistributionRaw ?? euinsRiskDist ?? [];
	...
	return (
		<div className="space-y-6">
			<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
				<StatCard
					title="Total AUM"
					value={fmt(aum?.data?.totalAum ?? aum?.data?.aum)}
					icon={IndianRupee}
					loading={aumL}
				...
				<StatCard
					title="Fund Earnings"
					value={fmt(earn?.data?.totalEarnings ?? earn?.data?.earnings)}
					icon={TrendingUp}
					loading={earnL}
					subtitle="Trail commission"
				/>
				<StatCard
					title="Active SIPs"
					value={
						sip?.data?.activeSips != null
							? sip.data.activeSips.toLocaleString()
							: sip?.data?.sipCount?.toLocaleString()
					}
					icon={Activity}
					loading={sipL}
					subtitle="Running systematic plans"
				/>
				<StatCard
					title="Unique Investors"
					value={
						inv?.data?.count != null
							? inv.data.count.toLocaleString()
							: inv?.data?.uniqueInvestors?.toLocaleString()
					}
					icon={Users}
					loading={invL}
					subtitle="Total investor base"
				/>
			</div>
					...
					<Card className="h-full">
						<CardHeader>
							<CardTitle className="text-base">
								Inflow vs Outflow — Trailing 12 Months
							</CardTitle>
							<CardDescription>
								Monthly net flows across all clients
							</CardDescription>
						</CardHeader>
						<CardContent>
							{inflowL ? (
								...
								<div className="h-56 flex items-center justify-center text-sm text-muted-foreground">
									No inflow/outflow data — IRIS credentials may need
									authentication
								</div>
							)}
						</CardContent>
			...
			<div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
				...
				<Card>
					<CardHeader>
						...
						<CardDescription>
							Book breakdown by investor risk category
						</CardDescription>
					</CardHeader>
					<CardContent>
						{inflowL || euinsL ? (
							...
							<div className="h-52 flex flex-col items-center justify-center text-sm text-muted-foreground gap-2">
								...
								<p className="text-xs">
									Data will appear when IRIS returns risk profile aggregates
								</p>
							</div>
						)}
					</CardContent>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						MF, AIF, PMS, FD, NPS execution via IRIS distributor platform
					</CardDescription>
				</CardHeader>
				<CardContent>
					<IrisStatusBadge />
...
function EmpanelmentTab() {
	const { data: amcList, isLoading: amcL } = useQuery<
		IrisApiResponse<{ amcs?: AmcEmpanelment[] }>
	>({ queryKey: ["/api/iris/empanelment/amc-list"], retry: false });
	const { data: fd, isLoading: fdL } = useQuery<
		IrisApiResponse<{ products?: FdProduct[] }>
	>({ queryKey: ["/api/iris/empanelment/fd-status"], retry: false });
	const { data: nps, isLoading: npsL } = useQuery<IrisApiResponse<NpsData>>({
		queryKey: ["/api/iris/empanelment/nps-status"],
		retry: false,
	});
	...
	const resendEsign = useMutation({
		mutationFn: (empanelmentId: string) =>
			apiRequest("/api/iris/empanelment/resend-esign", "POST", {
				body: { empanelmentId },
			}),
		onSuccess: () => toast({ title: "eSign link sent successfully" }),
		onError: () =>
			toast({ title: "Failed to send eSign link", variant: "destructive" }),
	});
	...
	return (
		<div className="space-y-6">
			<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
				<Card>
					<CardHeader>
						<CardTitle className="text-sm">Fixed Deposit Empanelment</CardTitle>
					</CardHeader>
					<CardContent>
						{fdL ? (
							...
							<div className="space-y-2">
								...
								{!fd?.data?.products?.length && (
									<p className="text-sm text-muted-foreground">
										No FD empanelment data
									</p>
								)}
							</div>
						)}
					...
					<CardContent>
						{npsL ? (
							...
							<div className="flex items-center gap-2">
								<Badge
									variant={
										nps.data.status === "ACTIVE" ? "default" : "secondary"
									}
								>
									{nps.data.status ?? "Unknown"}
								</Badge>
								{nps.data.distributor && (
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						MF fund house empanelment — click resend to trigger eSign email
					</CardDescription>
				</CardHeader>
				<CardContent>
					{amcL ? (
								...
								<div key={i} className="flex items-center justify-between py-3">
									<div>
										<p className="text-sm font-medium">
											{amc.amcName ?? amc.name}
										</p>
										<p className="text-xs text-muted-foreground">
											{amc.amcCode ?? amc.code}
										</p>
									</div>
									<div className="flex items-center gap-2">
										<Badge
						...
						<p className="text-sm text-muted-foreground">
							No AMC empanelment data available
						</p>
					)}
				</CardContent>
			</Card>
...
// ─── New SIP Dialog ──────────────────────────────────────────────────────────
function NewSipDialog({
	open,
	onClose,
	prefillPan,
}: { open: boolean; onClose: () => void; prefillPan: string }) {
	const { toast } = useToast();
	const qc = useQueryClient();
	const [pan, setPan] = useState(prefillPan);
	const [schemeQuery, setSchemeQuery] = useState("");
	const [selectedScheme, setSelectedScheme] = useState<SchemeResult | null>(
		null,
	);
	...
	const { data: schemeSearchData, isLoading: schemesLoading } = useQuery<
		IrisApiResponse<{ schemes?: SchemeResult[] } | SchemeResult[]>
	>({
		queryKey: ["/api/iris/transactions/scheme-search", schemeQuery],
		queryFn: () =>
			irisGet<IrisApiResponse<{ schemes?: SchemeResult[] } | SchemeResult[]>>(
				`/api/iris/transactions/scheme-search?q=${encodeURIComponent(schemeQuery)}`,
			),
	...
	const { data: mandatesData } = useQuery<
		IrisApiResponse<{ mandates?: MandateRecord[] } | MandateRecord[]>
	>({
		queryKey: ["/api/iris/transactions/mandates", pan],
		queryFn: () =>
			irisGet<
				IrisApiResponse<{ mandates?: MandateRecord[] } | MandateRecord[]>
			>(`/api/iris/transactions/mandates?pan=${encodeURIComponent(pan)}`),
	...
	const registerSip = useMutation({
		mutationFn: (body: Record<string, unknown>) =>
			apiRequest("/api/iris/transactions/sip/register", "POST", { body }),
		onSuccess: () => {
			toast({ title: "SIP registered successfully" });
			qc.invalidateQueries({
				queryKey: ["/api/iris/investors", pan, "systematic-plans"],
			});
			onClose();
			setPan(prefillPan);
			setSchemeQuery("");
	...
	function handleSubmit() {
		...
		if (!selectedScheme) {
			toast({ title: "Please select a scheme", variant: "destructive" });
			return;
		}
		...
		>
			<DialogContent className="sm:max-w-lg">
				<DialogHeader>
					...
					<DialogDescription>
						Set up a systematic investment plan for the investor
					</DialogDescription>
				</DialogHeader>
				<div className="space-y-4 pt-2">
						...
						{selectedScheme && (
							<div className="mt-1 flex items-center gap-2">
								<Badge variant="outline" className="text-xs">
									{selectedScheme.schemeCode ?? selectedScheme.code}
								</Badge>
								<button
								...
								<SelectContent>
									...
									{mandates.map((m, i) => (
										<SelectItem key={i} value={m.mandateId ?? m.id ?? `m${i}`}>
											{m.bankName ?? m.bank ?? "Mandate"} {m.mandateId ?? m.id}{" "}
											{m.status ? `(${m.status})` : ""}
										</SelectItem>
									))}
								</SelectContent>
					...
					<Button
						...
						disabled={registerSip.isPending}
					>
						{registerSip.isPending ? (
							<>
...
function RiskProfileDialog({
	pan,
	open,
	onClose,
}: { pan: string; open: boolean; onClose: () => void }) {
	const { toast } = useToast();
	const qc = useQueryClient();
	const [step, setStep] = useState(0);
	const [answers, setAnswers] = useState<Record<string, string>>({});

	const { data: questData, isLoading: questL } = useQuery<
		IrisApiResponse<{ questions?: RiskQuestion[] }>
	>({
	...
	const submitProfile = useMutation({
		mutationFn: () =>
			apiRequest(`/api/iris/investors/${pan}/risk-profile`, "POST", {
				body: { answers },
			}),
		onSuccess: () => {
		...
		onError: (e: Error) =>
			toast({
				title: "Failed to save risk profile",
			...
			<DialogContent className="sm:max-w-lg">
				<DialogHeader>
					<DialogTitle className="flex items-center gap-2">
						<Brain className="h-4 w-4" /> Risk Profile Assessment
					</DialogTitle>
					<DialogDescription>
						Answer the questions to determine the investor's risk profile
					</DialogDescription>
				</DialogHeader>
				{questL ? (
					...
					<p className="text-sm text-muted-foreground">
						No questionnaire available from IRIS
					</p>
				) : (
					<div className="space-y-4 pt-2">
						<p className="text-xs text-muted-foreground">
						...
						<div className="space-y-2">
							{(currentQ?.options ?? []).map((opt, i) => {
								...
								const qId =
									currentQ?.questionId ?? currentQ?.id ?? String(step);
								return (
									<button
										key={i}
...
// ─── Modify SIP Dialog ────────────────────────────────────────────────────────
function ModifySipDialog({
	open,
	onClose,
	sip,
	pan,
}: { open: boolean; onClose: () => void; sip: SipRecord | null; pan: string }) {
	const { toast } = useToast();
	const qc = useQueryClient();
	const [amount, setAmount] = useState("");
	const [sipDate, setSipDate] = useState("");
	...
	const modifySip = useMutation({
		mutationFn: (body: Record<string, unknown>) =>
			apiRequest(
				`/api/iris/transactions/sip/${sip?.sipRegistrationNo}/modify`,
				"PATCH",
				{ body },
			),
		onSuccess: () => {
	...
	function handleSubmit() {
		if (!sip?.sipRegistrationNo) {
			toast({
				title: "SIP registration number missing",
				variant: "destructive",
			});
			return;
		}
		...
		>
			<DialogContent className="sm:max-w-sm">
				<DialogHeader>
					...
					<DialogDescription>
						{sip?.schemeName ?? sip?.scheme ?? "SIP"} — {sip?.sipRegistrationNo}
					</DialogDescription>
				</DialogHeader>
				<div className="space-y-4 pt-2">
					<div>
...
function OrderStatusDialog({
	...
	const { data, isLoading } = useQuery<IrisApiResponse<OrderRecord>>({
		...
		queryFn: () =>
			irisGet<IrisApiResponse<OrderRecord>>(
					...
					{isLoading ? (
						...
						<div className="space-y-2 text-sm">
							...
							<div className="flex justify-between">
								...
								<span>{order.amount != null ? fmt(order.amount) : "—"}</span>
							</div>
							<div className="flex justify-between">
...
function ManageInvestorPanel({ pan, onClose }: ManagePanelProps) {
	const [tab, setTab] = useState<
		"nominee" | "bank" | "contact" | "fatca" | "idcw" | "mandate"
	>("nominee");
	const { toast } = useToast();
	const qc = useQueryClient();
	...
	useEffect(() => {
		const d = fatcaQ.data?.data as
			| { taxResidency?: string; taxIdNumber?: string }
			| undefined;
		if (d) {
			setFatcaForm({
	...
	const updateNominee = useMutation({
		mutationFn: () =>
			apiRequest(`/api/iris/non-financial/${pan}/nominee`, "POST", {
				body: nomineeForm,
			}),
		onSuccess: () => {
	...
	const updateBank = useMutation({
		mutationFn: () =>
			apiRequest(`/api/iris/non-financial/${pan}/bank`, "POST", {
				body: bankForm,
			}),
		onSuccess: () => {
	...
	const updateEmail = useMutation({
		mutationFn: () =>
			apiRequest(`/api/iris/non-financial/${pan}/email`, "POST", {
	...
	const updateMobile = useMutation({
		mutationFn: () =>
			apiRequest(`/api/iris/non-financial/${pan}/mobile`, "POST", {
				body: { mobile: contactForm.mobile },
			}),
	...
	const updateFatca = useMutation({
		mutationFn: () =>
			apiRequest(`/api/iris/non-financial/${pan}/fatca`, "POST", {
				body: fatcaForm,
			}),
		onSuccess: () => {
			toast({ title: "FATCA updated" });
			qc.invalidateQueries({
				queryKey: ["/api/iris/non-financial", pan, "fatca"],
			});
		},
	...
	const updateIdcw = useMutation({
		mutationFn: () =>
			apiRequest(`/api/iris/non-financial/${pan}/idcw`, "POST", {
				body: { idcwOption },
			}),
	...
	const manageMandate = useMutation({
		mutationFn: () =>
			apiRequest(`/api/iris/non-financial/${pan}/bank-mandate`, "POST", {
				body: mandateForm,
			}),
	...
	return (
		...
		>
			<DialogContent className="sm:max-w-2xl max-h-[85vh] overflow-y-auto">
				<DialogHeader>
					<DialogTitle className="flex items-center gap-2">
						<Settings className="h-4 w-4" /> Manage Investor — {pan}
					</DialogTitle>
					<DialogDescription>
						View and update non-financial details. Current values are pre-loaded
						where available.
					</DialogDescription>
				</DialogHeader>
				...
				<div className="space-y-4 pt-2">
					{tab === "nominee" && (
						<>
								...
								{nomineeQ.isLoading ? (
									...
									<>
										<DataRow
											label="Name"
											value={
												(nomineeQ.data.data as { nomineeName?: string })
													.nomineeName
											}
										/>
										<DataRow
											label="Relation"
											value={
												(nomineeQ.data.data as { nomineeRelation?: string })
													.nomineeRelation
											}
										/>
										<DataRow
											label="Allocation %"
											value={
												(nomineeQ.data.data as { nomineePercentage?: string })
													.nomineePercentage
											}
										/>
									</>
							...
							<div className="grid grid-cols-2 gap-3">
								...
								<div>
									...
									<Select
										...
										onValueChange={(v) =>
											setNomineeForm((p) => ({ ...p, nomineeRelation: v }))
										}
									>
										<SelectTrigger>
								...
								<div>
									...
									<Input
										type="number"
										value={nomineeForm.nomineePercentage}
										min={1}
										max={100}
										onChange={(e) =>
											setNomineeForm((p) => ({
							...
							<Button
								onClick={() => updateNominee.mutate()}
								disabled={updateNominee.isPending}
								className="w-full"
							>
								...
								{bankQ.isLoading ? (
									...
									<>
										<DataRow
											label="Account No"
											value={
												(bankQ.data.data as { accountNumber?: string })
													.accountNumber
											}
										/>
										<DataRow
											label="IFSC"
											value={
												(bankQ.data.data as { ifscCode?: string }).ifscCode
											}
										/>
										<DataRow
											label="Bank"
											value={
												(bankQ.data.data as { bankName?: string }).bankName
											}
										/>
										<DataRow
											label="Type"
											value={
												(bankQ.data.data as { accountType?: string })
													.accountType
											}
										/>
									</>
						...
						<>
							<div className="grid grid-cols-1 gap-3">
								...
								<Button
									...
									disabled={updateEmail.isPending || !contactForm.email}
								>
								...
								<Button
									onClick={() => updateMobile.mutate()}
									disabled={updateMobile.isPending || !contactForm.mobile}
								>
					...
					{tab === "fatca" && (
						<>
								...
								{fatcaQ.isLoading ? (
									...
									<>
										<DataRow
											label="Tax Residency"
											value={
												(fatcaQ.data.data as { taxResidency?: string })
													.taxResidency
											}
										/>
										<DataRow
											label="Tax ID"
											value={
												(fatcaQ.data.data as { taxIdNumber?: string })
													.taxIdNumber
											}
										/>
									</>
							...
							<div className="grid grid-cols-2 gap-3">
								<div>
									<Label className="text-xs">Country of Tax Residency</Label>
									<Input
										value={fatcaForm.taxResidency}
										onChange={(e) =>
											setFatcaForm((p) => ({
												...p,
												taxResidency: e.target.value,
											}))
										}
								...
								<div>
									<Label className="text-xs">Tax Identification Number</Label>
									<Input
										value={fatcaForm.taxIdNumber}
										onChange={(e) =>
											setFatcaForm((p) => ({
												...p,
							...
							<Button
								onClick={() => updateFatca.mutate()}
								disabled={updateFatca.isPending}
								className="w-full"
							>
					...
					{tab === "idcw" && (
						<>
							<p className="text-sm text-muted-foreground">
								Set dividend / IDCW payout preference for this investor.
							</p>
							<div>
							...
							<Button
								onClick={() => updateIdcw.mutate()}
								disabled={updateIdcw.isPending}
								className="w-full"
							>
					...
					{tab === "mandate" && (
						<>
							<p className="text-sm text-muted-foreground">
								Create or cancel a bank mandate (eNACH) for auto-debit.
							</p>
							<div className="grid grid-cols-2 gap-3">
								<div>
									<Label className="text-xs">Bank Account No</Label>
									<Input
										value={mandateForm.bankAccountNo}
										onChange={(e) =>
											setMandateForm((p) => ({
												...p,
								...
								<div>
									<Label className="text-xs">Max Mandate Amount (₹)</Label>
									<Input
										type="number"
										value={mandateForm.amount}
										onChange={(e) =>
											setMandateForm((p) => ({ ...p, amount: e.target.value }))
										}
									/>
								</div>
								<div>
									...
									<Select
										value={mandateForm.action}
										onValueChange={(v) =>
											setMandateForm((p) => ({ ...p, action: v }))
										}
									>
										<SelectTrigger>
											<SelectValue />
							...
							<Button
								onClick={() => manageMandate.mutate()}
								disabled={manageMandate.isPending}
								className="w-full"
							>
								{manageMandate.isPending
									? "Submitting…"
...
function OrderTrackingDialog({
	...
	const { data, isLoading } = useQuery<
		IrisApiResponse<{ events?: TrackingEvent[]; tracking?: TrackingEvent[] }>
	>({
	...
	const events: TrackingEvent[] =
		data?.data?.events ?? data?.data?.tracking ?? [];
						...
						{events.map((ev, i) => (
							<div key={i} className="flex gap-3 items-start">
								<div className="flex flex-col items-center">
									<div
										className={`h-3 w-3 rounded-full mt-0.5 ${i === 0 ? "bg-primary" : "bg-muted-foreground/40"}`}
									/>
									{i < events.length - 1 && (
...
function FolioBrowser({ pan }: { pan: string }) {
	...
	const txnsQ = useQuery<
		IrisApiResponse<{ transactions?: FolioTxn[] } | FolioTxn[]>
	>({
		queryKey: [
			"/api/iris/investors",
			pan,
		...
		queryFn: () =>
			irisGet(
				`/api/iris/investors/${pan}/folios/${selectedFolio}/transactions`,
			),
	...
	function resolveTxns(): FolioTxn[] {
		...
		return (
			(txnsQ.data.data as { transactions?: FolioTxn[] }).transactions ?? []
		);
	}
	...
	return (
		<div className="space-y-3">
			<Card>
				<CardHeader>
					<CardTitle className="text-sm">Folio Browser</CardTitle>
					<CardDescription>
						Click a folio to view transaction history
					</CardDescription>
				</CardHeader>
			...
			{selectedFolio && (
				<Card>
					<CardHeader>
						<CardTitle className="text-sm">
							Transactions — Folio {selectedFolio}
						</CardTitle>
					</CardHeader>
					<CardContent className="p-0">
						{txnsQ.isLoading ? (
										...
										{txns.map((t, i) => (
											<tr key={i} className="border-b hover:bg-muted/50">
												...
												<td className="p-2 text-right">
													<Badge
														variant={
															t.status === "SUCCESS" ? "default" : "secondary"
														}
														className="text-[10px]"
													>
														{t.status ?? "—"}
							...
							<p className="p-4 text-sm text-muted-foreground">
								No transactions for this folio
							</p>
						)}
					</CardContent>
				</Card>
...
function DividendHistory({ pan }: { pan: string }) {
	const q = useQuery<
		IrisApiResponse<{ dividends?: DividendRecord[] } | DividendRecord[]>
	>({
								...
								{records.map((r, i) => (
									<tr key={i} className="border-b hover:bg-muted/50">
										<td className="p-2 truncate max-w-[140px]">
											{r.schemeName ?? r.scheme ?? "—"}
										</td>
										<td className="p-2 text-xs">
											{r.dividendDate ?? r.date ?? "—"}
										</td>
										<td className="p-2 text-right text-xs">
											{r.dividendPerUnit != null
												? "₹" + r.dividendPerUnit
												: "—"}
										</td>
										<td className="p-2 text-right">
											{fmt(r.totalAmount ?? r.amount)}
										</td>
									</tr>
								))}
							</tbody>
...
function GoalsPanel({ pan }: { pan: string }) {
	...
	const createGoal = useMutation({
		mutationFn: () =>
			apiRequest(`/api/iris/investors/${pan}/goals`, "POST", {
				body: {
	...
	const updateGoal = useMutation({
		mutationFn: () => {
			...
			return apiRequest(`/api/iris/investors/${pan}/goals/${id}`, "PUT", {
				body: {
					goalName: form.goalName,
					targetAmount: Number(form.targetAmount),
					targetDate: form.targetDate,
	...
	const deleteGoal = useMutation({
		mutationFn: (id: string) =>
			apiRequest(`/api/iris/investors/${pan}/goals/${id}`, "DELETE"),
		onSuccess: () => {
		...
		<div className="space-y-3">
			<div className="flex items-center justify-between">
				<h3 className="text-sm font-medium flex items-center gap-2">
					<Target className="h-4 w-4" /> Financial Goals
				</h3>
				<Button
					size="sm"
					variant="outline"
					...
					<CardContent className="pt-4 space-y-3">
						<div className="grid grid-cols-2 gap-3">
							<div>
								...
								<Input
									value={form.goalName}
									onChange={(e) =>
										setForm((p) => ({ ...p, goalName: e.target.value }))
									}
									placeholder="Retirement, Education…"
								/>
							</div>
							<div>
							...
							<div>
								...
								<Input
									...
									onChange={(e) =>
										setForm((p) => ({ ...p, currentSavings: e.target.value }))
									}
								/>
							</div>
						</div>
						<div className="flex gap-2">
							...
							<Button
								className="flex-1"
								disabled={
									editGoal ? updateGoal.isPending : createGoal.isPending
								}
								onClick={() =>
									editGoal ? updateGoal.mutate() : createGoal.mutate()
								}
							>
								{(editGoal ? updateGoal.isPending : createGoal.isPending)
									? "Saving…"
			...
			{goalsQ.isLoading ? (
				...
				<div className="space-y-2">
					{goals.map((g, i) => {
						...
						const progress =
							g.targetAmount && g.currentSavings
								? Math.min(
										100,
										Math.round((g.currentSavings / g.targetAmount) * 100),
									)
								: 0;
						return (
							<Card key={i}>
									...
									<div className="flex items-start justify-between">
										<div className="min-w-0 flex-1">
											...
											<p className="text-xs text-muted-foreground mt-0.5">
												Target: {fmt(g.targetAmount)} by {g.targetDate ?? "—"} ·
												Saved: {fmt(g.currentSavings)}
											</p>
											<div className="mt-2 h-1.5 rounded-full bg-muted overflow-hidden">
												<div
													className="h-full bg-primary rounded-full iris-bar-fill"
													style={{ ["--bar-w" as string]: `${progress}%` }}
												/>
											</div>
				...
				<p className="text-sm text-muted-foreground">
					No goals set for this investor
				</p>
			)}
		</div>
	);
...
function DematDocsPanel({ pan }: { pan: string }) {
	...
	const dematQ = useQuery<
		IrisApiResponse<{ accounts?: DematAccount[] } | DematAccount[]>
	>({
	...
	const docsQ = useQuery<
		IrisApiResponse<{ documents?: InvestorDoc[] } | InvestorDoc[]>
	>({
	...
	const linkDemat = useMutation({
		mutationFn: () =>
			apiRequest(`/api/iris/investors/${pan}/demat-accounts`, "POST", {
				body: linkForm,
			}),
		onSuccess: () => {
	...
	const uploadDoc = useMutation({
		mutationFn: () =>
			apiRequest(`/api/iris/investors/${pan}/documents`, "POST", {
				body: { documentType: docType, url: docUrl },
			}),
		onSuccess: () => {
	...
	return (
		<div className="space-y-4">
			<div className="space-y-2">
				<div className="flex items-center justify-between">
					<h3 className="text-sm font-medium flex items-center gap-2">
						<CreditCard className="h-4 w-4" /> Demat Accounts
					</h3>
					<Button
						size="sm"
						variant="outline"
					...
					<Card className="border-dashed">
						<CardContent className="pt-4 space-y-3">
							<div className="grid grid-cols-3 gap-2">
								<div>
									<Label className="text-xs">DP ID</Label>
									<Input
										value={linkForm.dpId}
										onChange={(e) =>
											setLinkForm((p) => ({ ...p, dpId: e.target.value }))
										}
									/>
								</div>
								<div>
									...
									<Input
										...
										onChange={(e) =>
											setLinkForm((p) => ({ ...p, clientId: e.target.value }))
										}
									/>
								</div>
								<div>
									<Label className="text-xs">DP Name</Label>
									<Input
										value={linkForm.dpName}
										onChange={(e) =>
											setLinkForm((p) => ({ ...p, dpName: e.target.value }))
										}
									/>
								</div>
							</div>
							<div className="flex gap-2">
								...
								<Button
									className="flex-1"
									onClick={() => linkDemat.mutate()}
									disabled={linkDemat.isPending}
								>
			...
			<div className="space-y-2">
				<div className="flex items-center justify-between">
					<h3 className="text-sm font-medium flex items-center gap-2">
						<Upload className="h-4 w-4" /> Document Vault
					</h3>
					<Button
						size="sm"
						variant="outline"
					...
					<Card className="border-dashed">
						<CardContent className="pt-4 space-y-3">
							<div className="grid grid-cols-2 gap-3">
								...
								<div>
									<Label className="text-xs">Document URL / Path</Label>
									<Input
										value={docUrl}
							...
							<div className="flex gap-2">
								...
								<Button
									...
									disabled={uploadDoc.isPending || !docUrl.trim()}
								>
							...
							>
								...
								{d.url && (
									<a
										...
										aria-label={`Open document: ${d.fileName ?? d.url ?? "document"}`}
									>
...
function PortalLinkButton({ pan }: { pan: string }) {
	...
	const getLink = useQuery<
		IrisApiResponse<{ url?: string; link?: string; portalUrl?: string }>
	>({
	...
	const sendLink = useMutation({
		mutationFn: (via: "email" | "sms") =>
			apiRequest(`/api/iris/investors/${pan}/portal-link/send`, "POST", {
				body: { via },
			}),
		...
		<Card>
			<CardHeader className="pb-2">
				<CardTitle className="text-sm flex items-center gap-2">
					<ExternalLink className="h-4 w-4" /> Investor Portal
				</CardTitle>
			</CardHeader>
			<CardContent>
				...
				{link && (
					<a
						...
						target="_blank"
						rel="noopener noreferrer"
						className="mt-2 flex items-center gap-1 text-xs text-blue-500 hover:underline break-all"
					>
...
function InvestorsTab() {
	...
	const [modifySipTarget, setModifySipTarget] = useState<SipRecord | null>(
		null,
	);
	...
	const { data: investorsData, isLoading: invL } = useQuery<
		IrisApiResponse<{ investors?: IrisInvestor[] } | IrisInvestor[]>
	>({
		queryKey: ["/api/iris/investors", search],
		queryFn: () =>
			irisGet<IrisApiResponse<{ investors?: IrisInvestor[] } | IrisInvestor[]>>(
				`/api/iris/investors${search ? `?search=${encodeURIComponent(search)}` : ""}`,
			),
		retry: false,
	});
	...
	const { data: kycData, isLoading: kycL } = useQuery<IrisApiResponse<KycData>>(
		{
			...
			queryFn: () =>
				irisGet<IrisApiResponse<KycData>>(
					`/api/iris/investors/${selectedPan}/kyc`,
				),
			enabled: !!selectedPan,
			retry: false,
		},
	...
	const { data: familyData } = useQuery<IrisApiResponse<any>>({
		...
		queryFn: () =>
			irisGet(`/api/iris/investors/${selectedPan}/family-portfolio`),
		enabled: !!selectedPan && detailTab === "kyc-details",
		retry: false,
	});
	...
	const { data: insightData } = useQuery<IrisApiResponse<any>>({
		...
		queryFn: () =>
			irisGet(`/api/iris/investors/${selectedPan}/portfolio-insights`),
		enabled: !!selectedPan && detailTab === "kyc-details",
		retry: false,
	});

	const { data: portfolioData, isLoading: portfolioL } = useQuery<
		IrisApiResponse<PortfolioSummary>
	>({
		queryKey: ["/api/iris/investors", selectedPan, "portfolio-summary"],
		queryFn: () =>
			irisGet<IrisApiResponse<PortfolioSummary>>(
				`/api/iris/investors/${selectedPan}/portfolio-summary`,
			),
		enabled: !!selectedPan,
		retry: false,
	});

	const { data: holdingsData, isLoading: holdingsL } = useQuery<
		IrisApiResponse<{ holdings?: Holding[]; investments?: Holding[] }>
	>({
		queryKey: ["/api/iris/investors", selectedPan, "investments"],
		queryFn: () =>
			irisGet<
				IrisApiResponse<{ holdings?: Holding[]; investments?: Holding[] }>
			>(`/api/iris/investors/${selectedPan}/investments`),
		enabled: !!selectedPan && detailTab === "holdings",
		retry: false,
	});

	const { data: txnsData, isLoading: txnsL } = useQuery<
		IrisApiResponse<{ transactions?: Transaction[] } | Transaction[]>
	>({
		queryKey: ["/api/iris/investors", selectedPan, "transactions"],
		queryFn: () =>
			irisGet<
				IrisApiResponse<{ transactions?: Transaction[] } | Transaction[]>
			>(`/api/iris/investors/${selectedPan}/transactions`),
		enabled: !!selectedPan && detailTab === "transactions",
		retry: false,
	});

	const { data: sipsData, isLoading: sipsL } = useQuery<
		IrisApiResponse<{ sips?: SipRecord[] }>
	>({
		queryKey: ["/api/iris/investors", selectedPan, "systematic-plans"],
		queryFn: () =>
			irisGet<IrisApiResponse<{ sips?: SipRecord[] }>>(
				`/api/iris/investors/${selectedPan}/systematic-plans`,
			),
		enabled:
			!!selectedPan && (detailTab === "sips" || detailTab === "portfolio"),
		retry: false,
	});

	const { data: ordersData, isLoading: ordersL } = useQuery<
		IrisApiResponse<{ orders?: OrderRecord[] } | OrderRecord[]>
	>({
		queryKey: ["/api/iris/transactions/orders", selectedPan, failedOnly],
		queryFn: () =>
			irisGet<IrisApiResponse<{ orders?: OrderRecord[] } | OrderRecord[]>>(
				failedOnly
	...
	const { data: appsData, isLoading: appsL } = useQuery<
		IrisApiResponse<{ applications?: OrderApplication[] } | OrderApplication[]>
	>({
		queryKey: ["/api/iris/applications", selectedPan],
		queryFn: () =>
			irisGet<
				IrisApiResponse<
					{ applications?: OrderApplication[] } | OrderApplication[]
				>
			>(`/api/iris/applications?pan=${selectedPan}`),
		enabled: !!selectedPan && detailTab === "applications",
		retry: false,
	});

	const { data: riskData, isLoading: riskL } = useQuery<
		IrisApiResponse<RiskProfile>
	>({
		queryKey: ["/api/iris/investors", selectedPan, "risk-profile"],
		queryFn: () =>
			irisGet<IrisApiResponse<RiskProfile>>(
				`/api/iris/investors/${selectedPan}/risk-profile`,
			),
	...
	const sendEkyc = useMutation({
		mutationFn: (pan: string) =>
			apiRequest(`/api/iris/investors/${pan}/send-ekyc-mail`, "POST"),
		onSuccess: () => toast({ title: "eKYC mail sent" }),
		onError: () =>
			toast({ title: "Failed to send eKYC mail", variant: "destructive" }),
	});
	...
	const sipCancel = useMutation({
		mutationFn: (payload: SipActionPayload) =>
			apiRequest("/api/iris/transactions/sip/cancel", "POST", {
				body: payload,
			}),
		onSuccess: () => {
	...
	function resolveInvestors(): IrisInvestor[] {
		...
		if ("investors" in investorsData.data)
			return investorsData.data.investors ?? [];
		return [];
	}
	...
	function resolveTxns(): Transaction[] {
		...
		if ("transactions" in txnsData.data)
			return txnsData.data.transactions ?? [];
		return [];
	}
	...
	function resolveApps(): OrderApplication[] {
		...
		if ("applications" in appsData.data)
			return appsData.data.applications ?? [];
		return [];
	}
	...
	const ekycStatus =
		ekycData?.data?.status ??
		ekycData?.data?.ekycStatus ??
		ekycData?.data?.completionStatus;
	...
	return (
		<div className="grid grid-cols-1 md:grid-cols-5 gap-4">
											...
											>
												<div className="min-w-0">
													<p className="text-sm font-medium truncate">
														{inv.name ?? inv.investorName}
													</p>
													<p className="text-xs text-muted-foreground">
														{pan} · {inv.mobile ?? inv.mobileNo ?? ""}
													</p>
												</div>
				...
				{selectedPan ? (
					<div className="space-y-3">
						<div className="flex items-center justify-between flex-wrap gap-2">
							<div>
								...
								<div className="flex items-center gap-2 mt-1 flex-wrap">
									{kycL ? (
										...
										<Badge
											variant={
												kycData?.data?.kycStatus === "KYC_VERIFIED"
													? "default"
													: "secondary"
											}
										>
									...
									{ekycStatus && (
										<Badge
											variant={
												ekycStatus === "COMPLETED" ? "default" : "secondary"
											}
							...
							<div className="flex gap-2 flex-wrap">
								...
								<Button
									size="sm"
									variant="outline"
									onClick={() => sendEkyc.mutate(selectedPan)}
									disabled={sendEkyc.isPending}
								>
									Send eKYC Mail
								</Button>
							</div>
										...
										<div className="grid grid-cols-2 gap-3">
											<div>
												...
												<p className="font-semibold">
													{fmt(portfolioData?.data?.currentValue)}
												</p>
											</div>
											<div>
												<p className="text-xs text-muted-foreground">
												...
												<p className="font-semibold">
													{fmt(portfolioData?.data?.investedValue)}
												</p>
											</div>
											<div>
												<p className="text-xs text-muted-foreground">
												...
												<p
													className={`font-semibold ${(portfolioData?.data?.gainLoss ?? 0) >= 0 ? "text-green-600" : "text-red-500"}`}
												>
						...
						{detailTab === "holdings" && (
							<Card>
								<CardHeader>
									<CardTitle className="text-sm">Fund-wise Holdings</CardTitle>
								</CardHeader>
								<CardContent className="p-0">
									{holdingsL ? (
										<div className="p-4 space-y-2">
											...
											<div className="divide-y">
												{holdings.map((h, i) => (
													<div key={i} className="p-3">
														<p className="text-sm font-medium">
															{h.schemeName ?? h.scheme}
														</p>
														<div className="flex justify-between text-xs text-muted-foreground mt-1">
															<span>
																{h.units != null
																	? Number(h.units).toFixed(3) + " units"
																	: ""}
															</span>
														...
														{h.gainLossPercentage != null && (
															<p
																className={`text-xs mt-0.5 ${h.gainLossPercentage >= 0 ? "text-green-600" : "text-red-500"}`}
															>
												...
												{sips.map((s, i) => {
													...
													return (
														<div key={i} className="p-3">
															...
															<div className="flex justify-between text-xs text-muted-foreground mt-1">
																<span>
																	{fmt(s.amount)}/mo · {s.frequency}
																</span>
																<span>
																	Next:{" "}
																	{s.nextInstallmentDate ?? s.nextDate ?? "—"}
																</span>
															</div>
															{s.status === "ACTIVE" && (
																<div className="flex gap-1.5 mt-2">
																	<Button
																		...
																		onClick={() => {
																			setModifySipTarget(s);
																			setModifySipOpen(true);
																		}}
																	>
																		Modify
																	</Button>
							...
							<div className="space-y-4">
								<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
									...
									<Card>
										<CardHeader className="pb-2">
											<CardTitle className="text-sm flex items-center gap-2">
												<LucideShield className="h-4 w-4 text-primary" /> KRA &
												Identity
											</CardTitle>
										</CardHeader>
									...
									<Card>
										<CardHeader className="pb-2">
											<CardTitle className="text-sm flex items-center gap-2">
												<Building2 className="h-4 w-4 text-primary" /> Bank
												Details
											</CardTitle>
										</CardHeader>
										<CardContent className="space-y-3">
											<div className="flex justify-between items-center border-b pb-2">
												...
												<Badge
													variant={
														bankData?.data?.status === "VALIDATED"
															? "default"
															: "secondary"
													}
													className={
											...
											<div className="flex justify-between items-center border-b pb-2">
												...
												<span
													className={`text-xs font-medium ${bankData?.data?.pennyDropStatus === "SUCCESS" ? "text-green-600" : "text-muted-foreground"}`}
												>
													{bankData?.data?.pennyDropStatus === "SUCCESS"
														? "Success (₹1.00)"
														: "Not Initiated"}
												</span>
											</div>
											<div className="flex justify-between items-center border-b pb-2">
												...
												<span className="text-xs font-medium truncate max-w-[120px]">
													{bankData?.data?.bankName ?? "HDFC BANK LTD"}
												</span>
											</div>
											<div className="flex justify-between items-center">
												<span className="text-xs text-muted-foreground">
												...
												<span className="text-xs font-mono">
													{bankData?.data?.ifsc ?? "HDFC0001234"}
												</span>
											</div>
										</CardContent>
									</Card>
									...
									<Card>
										...
										<CardContent className="space-y-3">
											<div className="flex justify-between items-center border-b pb-2">
												...
												<Badge
													variant={
														fatcaData?.data?.status === "REGISTERED"
															? "default"
															: "secondary"
													}
												>
											...
											<div className="flex justify-between items-center border-b pb-2">
												...
												<Badge
													variant={
														nomineeData?.data?.nominees?.length > 0
															? "default"
															: "secondary"
													}
												>
											...
											<div className="flex justify-between items-center">
												...
												<span className="text-xs font-medium">
													{fatcaData?.data?.uboStatus ?? "NOT APPLICABLE"}
												</span>
											</div>
										</CardContent>
									</Card>
									...
									<Card>
										...
										<CardContent className="space-y-3">
											...
											<div className="flex justify-between items-center">
												...
												<span className="text-xs font-medium">
													{insightData?.data?.riskScore ?? "Moderate"}
												</span>
											</div>
										</CardContent>
									</Card>
								...
								<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
									...
									<Card>
										<CardHeader className="pb-2">
											<CardTitle className="text-sm flex items-center gap-2">
												<Activity className="h-4 w-4 text-primary" /> Onboarding
												Journey
											</CardTitle>
										</CardHeader>
										<CardContent className="space-y-3">
										...
										<ScrollArea className="h-[320px]">
											<div className="divide-y">
												{orders.map((o, i) => (
													<div key={i} className="p-3">
														<div className="flex justify-between items-start">
															<p className="text-sm font-medium truncate max-w-[55%]">
																{o.schemeName ?? o.scheme ?? "—"}
															</p>
															<div className="flex items-center gap-1.5">
																...
																<Button
																	...
																	className="h-6 text-[11px] px-2"
																	onClick={() => {
																		setOrderStatusId(o.orderId ?? o.id ?? "");
						...
						{detailTab === "alerts" && (
							<InvestorAlertsPanel pan={selectedPan!} />
						)}
						...
						{detailTab === "applications" && (
							<Card>
								<CardHeader>
									...
									<CardDescription>
										All order applications for {selectedPan}
									</CardDescription>
								</CardHeader>
								<CardContent className="p-0">
									{appsL ? (
													...
													return (
														<div key={i} className="p-3">
															<div className="flex justify-between items-start">
																<div className="min-w-0">
																	<p className="text-sm font-medium truncate">
																		{app.schemeName ?? app.scheme ?? "—"}
																	</p>
																	<p className="text-xs text-muted-foreground">
																		{app.type ?? app.transactionType} ·{" "}
																		{fmt(app.amount)} · {appId}
																	</p>
																</div>
																<Badge
							...
							<Card>
								<CardHeader className="flex flex-row items-center justify-between">
									<div>
										...
										<CardDescription>
											Investor's assessed risk category
										</CardDescription>
									</div>
									<Button
										size="sm"
								...
								<CardContent>
									{riskL ? (
										<Skeleton className="h-24 w-full" />
									) : riskProfile?.riskProfile || riskProfile?.riskCategory ? (
										<div className="space-y-4">
											<div className="flex items-center gap-3">
												...
												<div>
													...
													{riskProfile.assessedAt && (
														<p className="text-xs text-muted-foreground">
															Assessed: {riskProfile.assessedAt}
														</p>
													)}
												</div>
											</div>
											{riskProfile.recommendedCategories &&
												riskProfile.recommendedCategories.length > 0 && (
													<div>
														<p className="text-xs font-medium text-muted-foreground mb-2">
															Recommended scheme categories
														</p>
														<div className="flex flex-wrap gap-1.5">
															{riskProfile.recommendedCategories.map(
																(cat, i) => (
																	<Badge
																		key={i}
																		variant="outline"
			...
			<NewSipDialog
				...
				prefillPan={selectedPan ?? ""}
			/>
			<ModifySipDialog
				open={modifySipOpen}
				onClose={() => setModifySipOpen(false)}
				sip={modifySipTarget}
				pan={selectedPan ?? ""}
			/>
			<OrderStatusDialog
...
function OnboardingTab() {
	...
	const { data: appsData, isLoading: appsL } = useQuery<
		IrisApiResponse<
			{ applications?: OnboardingApplication[] } | OnboardingApplication[]
		>
	>({
		queryKey: ["/api/iris/onboarding/applications", statusFilter],
		queryFn: () =>
			irisGet(
				`/api/iris/onboarding/applications${statusFilter !== "ALL" ? `?status=${statusFilter}` : ""}`,
			),
		retry: false,
	});
	...
	const initiateMutation = useMutation({
		mutationFn: () =>
			apiRequest("/api/iris/onboarding/initiate", "POST", {
				body: {
					name: form.name,
					pan: form.pan,
					dob: form.dob,
		...
		onSuccess: () => {
			toast({
				...
				description: "Application link sent to the investor",
			});
		...
		onError: (e: Error) =>
			toast({
				title: "Failed to initiate onboarding",
				description: e.message,
				variant: "destructive",
			}),
	});
	...
	const resendMutation = useMutation({
		mutationFn: (applicationId: string) =>
			apiRequest(`/api/iris/onboarding/${applicationId}/resend-link`, "POST"),
		...
		<div className="space-y-4">
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Digital investor onboarding — track status and resend links
					</CardDescription>
				</CardHeader>
				<CardContent className="p-0">
					{appsL ? (
											...
											<div className="flex items-start justify-between">
												<div className="min-w-0">
													<p className="font-medium text-sm">
														{app.name ?? app.investorName ?? "—"}
													</p>
													<p className="text-xs text-muted-foreground">
														{app.pan} · {app.mobile} · {app.email}
													</p>
													<p className="text-xs text-muted-foreground mt-0.5">
														ID: {appId}
													</p>
...
function NfoTab() {
	const { toast } = useToast();
	const qc = useQueryClient();
	const [subscribeScheme, setSubscribeScheme] = useState<NfoScheme | null>(
		null,
	);
	...
	const { data: nfoData, isLoading: nfoL } = useQuery<
		IrisApiResponse<{ schemes?: NfoScheme[] } | NfoScheme[]>
	>({
		queryKey: ["/api/iris/nfo/active"],
		retry: false,
	});

	const { data: nfoAppsData, isLoading: nfoAppsL } = useQuery<
		IrisApiResponse<{ applications?: NfoApplication[] } | NfoApplication[]>
	>({
		queryKey: ["/api/iris/nfo/applications", appsSubmittedPan],
		queryFn: () =>
			irisGet(
				`/api/iris/nfo/applications${appsSubmittedPan ? `?pan=${appsSubmittedPan}` : ""}`,
			),
	...
	const applyMutation = useMutation({
		mutationFn: () =>
			apiRequest("/api/iris/nfo/apply", "POST", {
				body: {
					pan: subPan,
		...
		onSuccess: () => {
			...
			setSubPan("");
			setSubAmount("");
			setSubPaymentMode("NETBANKING");
		},
		onError: (e: Error) =>
			toast({
				title: "NFO subscription failed",
				description: e.message,
				variant: "destructive",
			}),
	});
	...
	const cancelNfoMutation = useMutation({
		mutationFn: (applicationId: string) =>
			apiRequest(`/api/iris/nfo/applications/${applicationId}/cancel`, "POST"),
		onSuccess: () => {
	...
	function resolveApps(): NfoApplication[] {
		...
		if ("applications" in nfoAppsData.data)
			return nfoAppsData.data.applications ?? [];
		return [];
	}
		...
		<div className="space-y-6">
			<Card>
				<CardHeader>
					...
					<CardDescription>
						New Fund Offers currently open for subscription
					</CardDescription>
				</CardHeader>
				<CardContent>
					{nfoL ? (
								...
								<Card key={i} className="border bg-card">
									<CardContent className="p-4">
										...
										<div className="text-xs text-muted-foreground space-y-0.5 mb-3">
											...
											{(s.minAmount ?? s.minimumAmount) && (
												<p>Min: {fmt(s.minAmount ?? s.minimumAmount)}</p>
											)}
										</div>
										<Button
											size="sm"
											className="w-full"
											onClick={() => setSubscribeScheme(s)}
										>
											Subscribe
										</Button>
									</CardContent>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						View and manage NFO subscriptions by investor PAN
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-4">
					<div className="flex gap-2">
						<Input
							...
							value={appsPan}
							onChange={(e) => setAppsPan(e.target.value.toUpperCase())}
							className="max-w-xs"
						/>
						<Button
							onClick={() => setAppsSubmittedPan(appsPan)}
							disabled={appsPan.length < 10}
						>
					...
					{nfoAppsL ? (
									...
									<div key={i} className="p-3">
										<div className="flex items-start justify-between">
											<div>
												...
												<p className="text-xs text-muted-foreground">
													{app.paymentMode} · {fmt(app.amount)} · {appId}
												</p>
											</div>
											<Badge
												variant={statusVariant(app.status)}
						...
						<p className="text-sm text-muted-foreground">
							No NFO applications found for {appsSubmittedPan}
						</p>
					) : null}
				</CardContent>
			</Card>
			...
			{/* Subscribe Dialog */}
			<Dialog
				open={!!subscribeScheme}
				onOpenChange={(v) => {
					if (!v) setSubscribeScheme(null);
				}}
			>
				<DialogContent className="sm:max-w-md">
					<DialogHeader>
						...
						<DialogDescription>
							{subscribeScheme?.schemeName ?? subscribeScheme?.name}
						</DialogDescription>
					</DialogHeader>
					<div className="space-y-4 pt-2">
						<div>
							...
							<Input
								value={subPan}
								onChange={(e) => setSubPan(e.target.value.toUpperCase())}
								placeholder="ABCDE1234F"
								maxLength={10}
							/>
						</div>
						<div>
							...
							<Input
								...
								placeholder={String(
									subscribeScheme?.minAmount ??
										subscribeScheme?.minimumAmount ??
										5000,
								)}
								min={1}
							/>
...
function SchemeSearchField({
	...
	const { data: schemeSearchData, isLoading } = useQuery<
		IrisApiResponse<{ schemes?: SchemeResult[] } | SchemeResult[]>
	>({
		queryKey: ["/api/iris/transactions/scheme-search", query],
		queryFn: () =>
			irisGet<IrisApiResponse<{ schemes?: SchemeResult[] } | SchemeResult[]>>(
				`/api/iris/transactions/scheme-search?q=${encodeURIComponent(query)}`,
			),
	...
	return (
		<div>
			...
			{selected && (
				<div className="mt-1 flex items-center gap-2">
					<Badge variant="outline" className="text-xs">
						{selected.schemeCode ?? selected.isinCode ?? selected.code}
					</Badge>
					<button
						className="text-xs text-muted-foreground hover:text-destructive flex items-center gap-0.5"
						onClick={onClear}
...
function OrdersPanel() {
	...
	const { data: fundsData } = useQuery<
		IrisApiResponse<{ funds?: FundHouse[] } | FundHouse[]>
	>({
	...
	return (
		<div className="space-y-4">
			<div className="flex gap-3 flex-wrap">
				<Button onClick={() => openModal("lumpsum")}>
					<ArrowUpRight className="h-4 w-4 mr-1" /> Lumpsum Purchase
				</Button>
				<Button variant="outline" onClick={() => openModal("sip")}>
					<Activity className="h-4 w-4 mr-1" /> Start SIP
				</Button>
				<Button variant="outline" onClick={() => openModal("redemption")}>
					<IndianRupee className="h-4 w-4 mr-1" /> Redemption
				</Button>
			</div>

			<Card>
				<CardHeader>
					<CardTitle className="text-sm">Available Fund Houses</CardTitle>
				</CardHeader>
				<CardContent>
					{funds.length > 0 ? (
						...
						<p className="text-sm text-muted-foreground">
							Fund data unavailable — IRIS credentials may need
							re-authentication
						</p>
					)}
				</CardContent>
			...
			<Dialog open={modalOpen} onOpenChange={setModalOpen}>
				<DialogContent className="sm:max-w-lg">
					<DialogHeader>
						...
						<DialogDescription>
							Enter investor and scheme details to place the order
						</DialogDescription>
					</DialogHeader>
					<div className="space-y-4 pt-2">
						<div>
...
function SwitchPanel() {
	...
	const { data: historyData, isLoading: historyL } = useQuery<
		IrisApiResponse<{ transactions?: SwitchOrder[] } | SwitchOrder[]>
	>({
		queryKey: [
			"/api/iris/investors",
			submittedHistoryPan,
	...
	function handleSubmit() {
		...
		if (!srcScheme) {
			toast({
				title: "Please select the source scheme",
				variant: "destructive",
			});
			return;
		}
		if (!tgtScheme) {
			toast({
				title: "Please select the target scheme",
				variant: "destructive",
			});
			return;
		}
		if (!switchAll && (!amount || Number(amount) <= 0)) {
			toast({
				title: "Enter a valid amount or select Switch All",
				variant: "destructive",
			});
			return;
		}
		...
		switchMutation.mutate({
			pan,
			fromSchemeCode:
				srcScheme.schemeCode ?? srcScheme.isinCode ?? srcScheme.code,
			toSchemeCode:
				tgtScheme.schemeCode ?? tgtScheme.isinCode ?? tgtScheme.code,
		...
		<div className="space-y-6">
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Same-AMC scheme-to-scheme switch (same-day at applicable NAV)
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-4">
					<div>
					...
					<div className="space-y-2">
						<div className="flex items-center gap-2">
							<Checkbox
								id="switch-all"
								checked={switchAll}
								onCheckedChange={(v) => setSwitchAll(!!v)}
							/>
							<label
								htmlFor="switch-all"
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						View past switch orders for an investor
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-3">
					<div className="flex gap-2">
						...
						<Button
							variant="outline"
							onClick={() => setSubmittedHistoryPan(historyPan.trim())}
							disabled={historyPan.length < 10}
						>
								...
								<tbody>
									{orders.map((o, i) => (
										<tr key={i} className="border-b hover:bg-muted/50">
											...
											<td className="p-2 text-right">
												{o.allUnits ? "All Units" : fmt(o.amount)}
											</td>
					...
					{submittedHistoryPan && !historyL && orders.length === 0 && (
						<p className="text-sm text-muted-foreground">
							No switch orders found for {submittedHistoryPan}.
						</p>
					)}
				</CardContent>
			</Card>
...
function StpPanel() {
	...
	const { data: stpData, isLoading: stpL } = useQuery<
		IrisApiResponse<
			{ stps?: StpRecord[]; systematicPlans?: StpRecord[] } | StpRecord[]
		>
	>({
		queryKey: [
			"/api/iris/investors",
	...
	function handleRegister() {
		...
		if (!srcScheme) {
			toast({ title: "Select the source scheme", variant: "destructive" });
			return;
		}
		if (!tgtScheme) {
			toast({ title: "Select the target scheme", variant: "destructive" });
			return;
		}
		if (!amount || Number(amount) <= 0) {
			toast({
				title: "Enter a valid installment amount",
				variant: "destructive",
			});
			return;
		}
		if (!installments || Number(installments) < 1) {
			toast({
				title: "Enter the number of installments",
				variant: "destructive",
			});
			return;
		}
		...
		<div className="space-y-6">
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Systematic Transfer Plan — periodically transfer from one scheme to
						another
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-4">
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						View and manage existing STP registrations
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-3">
					<div className="flex gap-2">
						...
						<Button
							variant="outline"
							onClick={() => setSubmittedListPan(listPan.trim())}
							disabled={listPan.length < 10}
						>
							...
							{stps.map((s, i) => (
								<div key={i} className="p-3">
									<div className="flex items-start justify-between gap-2">
										<div className="min-w-0">
											<p className="text-sm font-medium truncate">
												{s.sourceSchemeName ?? "Source Scheme"}
											</p>
											<p className="text-xs text-muted-foreground">
												→ {s.targetSchemeName ?? "Target Scheme"}
											</p>
											<div className="flex flex-wrap gap-x-3 text-xs text-muted-foreground mt-1">
												...
												<span>
													Next: {s.nextInstallmentDate ?? s.nextDate ?? "—"}
												</span>
												<span>
													Remaining:{" "}
													{s.remainingInstallments ?? s.remainingCount ?? "—"}
												</span>
											</div>
										</div>
														...
														<Button
															...
															onClick={() =>
																pauseMutation.mutate({
																	pan: submittedListPan,
																	stpId: s.stpId ?? s.id,
																	sourceSchemeCode: s.sourceSchemeCode,
																})
															}
														>
															Pause
					...
					{submittedListPan && !stpL && stps.length === 0 && (
						<p className="text-sm text-muted-foreground">
							No active STPs found for {submittedListPan}.
						</p>
					)}
				</CardContent>
			</Card>
...
function SwpPanel() {
	...
	const { data: swpData, isLoading: swpL } = useQuery<
		IrisApiResponse<
			{ swps?: SwpRecord[]; systematicPlans?: SwpRecord[] } | SwpRecord[]
		>
	>({
		queryKey: [
			"/api/iris/investors",
	...
	function handleRegister() {
		...
		if (!selectedScheme) {
			toast({ title: "Select a scheme", variant: "destructive" });
			return;
		}
		if (!amount || Number(amount) <= 0) {
			toast({
		...
		<div className="space-y-6">
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Systematic Withdrawal Plan — regular redemptions from a scheme
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-4">
					<div>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						View and manage existing SWP registrations
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-3">
					<div className="flex gap-2">
									...
									<div className="flex items-start justify-between gap-2">
										<div className="min-w-0">
											...
											<div className="flex flex-wrap gap-x-3 text-xs text-muted-foreground mt-1">
												...
												<span>
													Next: {s.nextWithdrawalDate ?? s.nextDate ?? "—"}
												</span>
												<span>
													Remaining:{" "}
													{s.remainingInstallments ?? s.remainingCount ?? "—"}
					...
					{submittedListPan && !swpL && swps.length === 0 && (
						<p className="text-sm text-muted-foreground">
							No active SWPs found for {submittedListPan}.
						</p>
					)}
				</CardContent>
			</Card>
...
function ProductsTab() {
	const { data: aif, isLoading: aifL } = useQuery<
		IrisApiResponse<{ links?: ProductLink[] } | ProductLink[]>
	>({ queryKey: ["/api/iris/products/aif-links"], retry: false });
	const { data: pms, isLoading: pmsL } = useQuery<
		IrisApiResponse<{ links?: ProductLink[] } | ProductLink[]>
	>({ queryKey: ["/api/iris/products/pms-links"], retry: false });
	const { data: fd, isLoading: fdL } = useQuery<
		IrisApiResponse<{ products?: ProductLink[] } | ProductLink[]>
	>({ queryKey: ["/api/iris/products/fixed-deposits"], retry: false });
	const { data: nps, isLoading: npsL } = useQuery<
		IrisApiResponse<{ links?: ProductLink[] } | ProductLink[]>
	>({ queryKey: ["/api/iris/products/nps-links"], retry: false });
	...
	const [prematureData, setPrematureData] = useState<FdPrematureClose | null>(
		null,
	);
	...
	const { data: fdOrdersData, isLoading: fdOrdersL } = useQuery<
		IrisApiResponse<{ orders?: FdOrder[] } | FdOrder[]>
	>({
		queryKey: ["/api/iris/products/fixed-deposits/orders", fdSubmittedPan],
		queryFn: () =>
			irisGet<IrisApiResponse<{ orders?: FdOrder[] } | FdOrder[]>>(
				`/api/iris/products/fixed-deposits/orders?pan=${encodeURIComponent(fdSubmittedPan)}`,
			),
		enabled: !!fdSubmittedPan,
		retry: false,
	});

	const { data: maturityData, isLoading: maturityL } = useQuery<
		IrisApiResponse<{ maturities?: FdMaturity[] } | FdMaturity[]>
	>({
		queryKey: ["/api/iris/products/fixed-deposits/maturity", maturityDays],
		queryFn: () =>
			irisGet<IrisApiResponse<{ maturities?: FdMaturity[] } | FdMaturity[]>>(
				`/api/iris/products/fixed-deposits/maturity?days=${maturityDays}`,
			),
		retry: false,
	});
	...
	const prematurePreviewMutation = useMutation({
		...
		onError: (err: Error) =>
			toast({
				title: "Could not fetch premature closure details",
				description: err.message,
				variant: "destructive",
			}),
	});
	...
	const prematureCloseMutation = useMutation({
		...
		onError: (err: Error) =>
			toast({
				title: "Premature closure failed",
				description: err.message,
				variant: "destructive",
			}),
	});
	...
	async function runInterestCalc() {
		if (!calcAmount || !calcTenure || !calcRate) {
			toast({
				title: "Please fill all calculator fields",
				variant: "destructive",
			});
			return;
		}
		...
		try {
			const qs = new URLSearchParams({
				...
				rate: calcRate,
				compounding: calcCompounding,
			});
			const data = await irisGet<IrisApiResponse<FdInterestResult>>(
				`/api/iris/products/fixed-deposits/interest-calculator?${qs}`,
			);
			if (data?.data) {
				...
				const n =
					calcCompounding === "MONTHLY"
						? 12
						: calcCompounding === "QUARTERLY"
							? 4
							: calcCompounding === "HALF_YEARLY"
								? 2
								: 1;
				...
				setCalcResult({
					...
					effectiveYield: ((maturity - p) / p) * 100,
				});
			}
		} catch {
			const p = Number(calcAmount);
	...
	function resolveMaturities(): FdMaturity[] {
		...
		if ("maturities" in maturityData.data)
			return maturityData.data.maturities ?? [];
		return [];
	}
	...
	function LinkList({
		items,
		isLoading,
	}: { items: ProductLink[]; isLoading: boolean }) {
	...
	return (
		<div className="space-y-6">
			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
				<Card>
					<CardHeader>
						<CardTitle className="text-sm">AIF Links</CardTitle>
						<CardDescription>Alternative Investment Funds</CardDescription>
					</CardHeader>
					<CardContent>
						<LinkList items={resolveItems(aif)} isLoading={aifL} />
					</CardContent>
				</Card>
				<Card>
					<CardHeader>
						<CardTitle className="text-sm">PMS Links</CardTitle>
						<CardDescription>Portfolio Management Services</CardDescription>
					</CardHeader>
					<CardContent>
						<LinkList items={resolveItems(pms)} isLoading={pmsL} />
					</CardContent>
				</Card>
				<Card>
					<CardHeader>
						<CardTitle className="text-sm">Fixed Deposits</CardTitle>
						<CardDescription>FD product brochures</CardDescription>
					</CardHeader>
					<CardContent>
						<LinkList items={resolveItems(fd)} isLoading={fdL} />
					</CardContent>
				</Card>
				<Card>
					<CardHeader>
						<CardTitle className="text-sm">NPS Links</CardTitle>
						<CardDescription>
							National Pension System onboarding
						</CardDescription>
					</CardHeader>
					<CardContent>
						<LinkList items={resolveItems(nps)} isLoading={npsL} />
					</CardContent>
				</Card>
			</div>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						View placed FD orders by investor PAN. Initiate premature closure if
						needed.
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-4">
					<div className="flex gap-2">
						...
						/>
						<Button
							onClick={() => setFdSubmittedPan(fdPan.trim())}
							disabled={fdPan.length < 10 || fdOrdersL}
						>
							{fdOrdersL ? (
					...
					{fdSubmittedPan &&
						(fdOrdersL ? (
							...
							<p className="text-sm text-muted-foreground">
								No FD orders found for {fdSubmittedPan}.
							</p>
						))}
				</CardContent>
			</Card>
			...
			>
				<DialogContent className="sm:max-w-sm">
					<DialogHeader>
						<DialogTitle className="flex items-center gap-2">
							<AlertTriangle className="h-4 w-4 text-amber-500" /> Premature FD
							Closure
						</DialogTitle>
						<DialogDescription>
							Order:{" "}
							<span className="font-mono">
								{prematureOrder?.orderId ?? prematureOrder?.id ?? "—"}
							</span>{" "}
							— {prematureOrder?.productName ?? prematureOrder?.name}
						</DialogDescription>
					</DialogHeader>
					<div className="space-y-3 pt-2">
						{prematurePreviewMutation.isPending ? (
							...
							<div className="space-y-2">
								<div className="rounded-lg bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 p-3 space-y-1.5">
									<div className="flex justify-between text-sm">
										...
										<span className="text-destructive font-medium">
											{fmt(
												prematureData.penalty ?? prematureData.penaltyAmount,
											)}
										</span>
									</div>
									<div className="flex justify-between text-sm">
										...
										<span className="font-semibold">
											{fmt(
												prematureData.finalPayout ?? prematureData.netPayout,
											)}
										</span>
									</div>
								...
								<p className="text-xs text-muted-foreground">
									Penalty deducted from principal due to early withdrawal. This
									action is irreversible.
								</p>
							</div>
						) : (
							<p className="text-sm text-muted-foreground">
								Penalty details not available from IRIS. Proceeding will
						...
						<div className="flex gap-2 pt-1">
							...
							<Button
								...
								onClick={() => {
									const id =
										prematureOrder?.orderId ?? prematureOrder?.id ?? "";
									if (id) prematureCloseMutation.mutate(id);
								}}
								disabled={prematureCloseMutation.isPending}
							>
								{prematureCloseMutation.isPending
									? "Closing…"
									: "Confirm Closure"}
							</Button>
						</div>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Upcoming FD maturities across the distributor's book
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-4">
					<div className="flex gap-2 items-center">
						<span className="text-sm text-muted-foreground">
							Show maturities in next
						</span>
					...
					{maturityL ? (
						...
						<p className="text-sm text-muted-foreground">
							No FD maturities in the next {maturityDays} days.
						</p>
					)}
				</CardContent>
			</Card>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Estimate FD returns based on amount, tenure, rate, and compounding
						frequency
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-4">
					<div className="grid grid-cols-2 md:grid-cols-4 gap-3">
						...
						<div>
							...
							<Select
								...
								onValueChange={setCalcCompounding}
							>
								<SelectTrigger>
									<SelectValue />
								</SelectTrigger>
						...
						<div className="grid grid-cols-3 gap-3 mt-2">
							<div className="bg-muted rounded-lg p-3">
								...
								<p className="font-bold text-base">
									{fmt(calcResult.maturityAmount)}
								</p>
							</div>
							<div className="bg-muted rounded-lg p-3">
								<p className="text-xs text-muted-foreground">Interest Earned</p>
								<p className="font-bold text-base text-green-600">
									{fmt(calcResult.interestEarned)}
								</p>
							</div>
							<div className="bg-muted rounded-lg p-3">
								<p className="text-xs text-muted-foreground">Effective Yield</p>
								<p className="font-bold text-base">
									{calcResult.effectiveYield != null
										? calcResult.effectiveYield.toFixed(2) + "%"
										: "—"}
								</p>
							</div>
						</div>
...
function NpsTab() {
	...
	const { data: subscriberData, isLoading: subL } = useQuery<
		IrisApiResponse<NpsSubscriber>
	>({
		queryKey: ["/api/iris/nps/subscriber", pran],
		queryFn: () =>
			irisGet<IrisApiResponse<NpsSubscriber>>(
	...
	const { data: portfolioData, isLoading: portfolioL } = useQuery<
		IrisApiResponse<{
			allocations?: NpsAllocation[];
			allocation?: NpsAllocation[];
		}>
	>({
	...
	const { data: fundValuesData, isLoading: fundL } = useQuery<
		IrisApiResponse<
			{ funds?: NpsFundValue[]; fundValues?: NpsFundValue[] } | NpsFundValue[]
		>
	>({
		...
		queryFn: () =>
			irisGet<
				IrisApiResponse<
					| { funds?: NpsFundValue[]; fundValues?: NpsFundValue[] }
					| NpsFundValue[]
				>
	...
	const { data: txnsData, isLoading: txnL } = useQuery<
		IrisApiResponse<{ transactions?: NpsTransaction[] } | NpsTransaction[]>
	>({
	...
	const schemeChangeMutation = useMutation({
		mutationFn: () =>
			apiRequest(`/api/iris/nps/subscriber/${pran}/scheme-change`, "POST", {
			...
			<Card>
				<CardHeader>
					<CardTitle className="flex items-center gap-2">
						...
						NPS Subscriber Lookup
					</CardTitle>
					<CardDescription>
						Enter PRAN to view subscriber details, portfolio allocation, and
						fund values
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-4">
					<div className="flex gap-2">
						<Input
							...
							value={pranInput}
							onChange={(e) =>
								setPranInput(e.target.value.replace(/\D/g, "").slice(0, 12))
							}
							className="max-w-xs font-mono"
						/>
						<Button
							onClick={() => setPran(pranInput.trim())}
							disabled={pranInput.length !== 12 || subL}
						>
							{subL ? (
								<RefreshCw className="h-4 w-4 animate-spin mr-1" />
							) : (
					...
					{pran && subscriber && (
						<div className="space-y-4">
								...
								<div className="flex items-start justify-between">
									<div>
										...
										<div className="flex gap-2 mt-1">
											...
											{(subscriber.tier ?? subscriber.tierType) && (
									...
									<div className="flex gap-2">
										<Button
											size="sm"
											variant="outline"
											onClick={() => setSchemeChangeOpen(true)}
										>
										...
										>
											<IndianRupee className="h-3.5 w-3.5 mr-1" /> Partial
											Withdrawal
										</Button>
									</div>
								</div>
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
												...
												{allocations.map((a, i) => (
													<div key={i} className="space-y-1">
														...
														<div className="w-full bg-muted rounded-full h-1.5">
															<div
																className="bg-primary rounded-full h-1.5 iris-bar-fill"
																style={{
																	["--bar-w" as string]: `${a.percentage ?? 0}%`,
																}}
															/>
														</div>
														{a.currentValue != null && (
								...
								<Card>
									<CardHeader className="pb-2">
										<CardTitle className="text-sm">Fund Values</CardTitle>
									</CardHeader>
									<CardContent>
										{fundL ? (
											<Skeleton className="h-24 w-full" />
													...
													<div key={i} className="py-2 text-sm">
														...
														<div className="flex justify-between text-xs text-muted-foreground mt-0.5">
															<span>
																...
																Units:{" "}
																{f.units != null ? f.units.toFixed(3) : "—"}
															</span>
															<span className="font-medium text-foreground">
																{fmt(f.currentValue)}
															</span>
					...
					{pran && !subL && !subscriber && (
						<p className="text-sm text-muted-foreground">
							No subscriber found for PRAN {pran}.
						</p>
					)}
					{subL && (
						<div className="space-y-2">
			...
			<Dialog open={schemeChangeOpen} onOpenChange={setSchemeChangeOpen}>
				<DialogContent className="sm:max-w-sm">
					<DialogHeader>
						...
						<DialogDescription>
							Change the fund manager or asset class allocation for PRAN {pran}
						</DialogDescription>
					</DialogHeader>
					<div className="space-y-4 pt-2">
						<div>
						...
						<div className="flex gap-2">
							<Button
								variant="outline"
								className="flex-1"
								onClick={() => setSchemeChangeOpen(false)}
							>
								Cancel
							</Button>
							<Button
							...
							>
								{schemeChangeMutation.isPending
									? "Submitting…"
									: "Submit Request"}
							</Button>
						</div>
			...
			<Dialog open={withdrawalOpen} onOpenChange={setWithdrawalOpen}>
				<DialogContent className="sm:max-w-sm">
					<DialogHeader>
						...
						<DialogDescription>
							Raise a partial withdrawal request for PRAN {pran}. Up to 25% of
							own contributions allowed after 3 years.
						</DialogDescription>
					</DialogHeader>
					<div className="space-y-4 pt-2">
						<div>
							...
							>
								...
								<SelectContent>
									...
									<SelectItem value="HOUSE_PURCHASE">
						...
						<p className="text-xs text-muted-foreground">
							Supporting documents (e.g., medical certificate, admission letter)
							must be uploaded via the PFRDA portal separately.
						</p>
						<div className="flex gap-2">
							<Button
								variant="outline"
...
function AdminOtpDialog({
	...
	const sendOtp = useMutation({
		mutationFn: () =>
			apiRequest("/api/iris/auth/send-otp", "POST", { body: { mobile } }),
		onSuccess: () => {
	...
	const submitOtp = useMutation({
		mutationFn: () =>
			apiRequest("/api/iris/auth/submit-otp", "POST", { body: { otp } }),
		onSuccess: () => {
	...
	return (
		...
		>
			<DialogContent className="sm:max-w-sm">
				<DialogHeader>
					<DialogTitle className="flex items-center gap-2">
						<KeyRound className="h-4 w-4" /> IRIS Re-Authentication
					</DialogTitle>
					<DialogDescription>
						Re-authenticate IRIS token via OTP when the session expires. Admin
						only.
					</DialogDescription>
				</DialogHeader>
				<div className="space-y-4 pt-2">
						...
						<>
							...
							<div className="flex gap-2">
								...
								<Button
									...
									disabled={submitOtp.isPending || otp.length < 4}
								>
...
function AnalyticsTab() {
	...
	const xirrQuery = useQuery<{
		...
		queryFn: () =>
			irisGet(
				`/api/iris/analytics/xirr/${submittedPan}${fromDate || toDate ? "?" + new URLSearchParams(Object.fromEntries(Object.entries({ fromDate, toDate }).filter(([, v]) => v))).toString() : ""}`,
			),
	...
	const portfolioXirrQuery = useQuery<{
		success: boolean;
		data: { xirr?: number; portfolioXirr?: number };
	}>({
		queryKey: ["/api/iris/analytics/portfolio-xirr", submittedPan],
		queryFn: () =>
			irisGet(`/api/iris/analytics/portfolio-xirr/${submittedPan}`),
		enabled: !!submittedPan,
		retry: false,
	});
	...
	const sipXirrQuery = useQuery<{
		success: boolean;
		data: {
			sipXirr?: number;
			xirr?: number;
			sips?: Array<{ schemeName?: string; xirr?: number; amount?: number }>;
		};
	}>({
	...
	const taxHarvestQuery = useQuery<{
		success: boolean;
		data: {
			opportunities?: Array<{
				schemeName?: string;
		...
		<div className="space-y-6">
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Enter a PAN to load XIRR, returns breakdown, SIP XIRR and tax
				...
				<>
					...
					<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
						<StatCard
							title="Overall XIRR"
							value={
								xirr?.xirr != null ? `${xirr.xirr.toFixed(2)}%` : undefined
							}
						...
						<StatCard
							...
							value={
								portfolioXirr?.xirr != null
									? `${portfolioXirr.xirr.toFixed(2)}%`
									: portfolioXirr?.portfolioXirr != null
						...
						<StatCard
							title="SIP XIRR"
							value={
								sipXirr?.sipXirr != null
									? `${sipXirr.sipXirr.toFixed(2)}%`
									: sipXirr?.xirr != null
										? `${sipXirr.xirr.toFixed(2)}%`
										: undefined
							}
						...
						<StatCard
							...
							value={
								xirr?.absoluteReturn != null
									? `${xirr.absoluteReturn.toFixed(2)}%`
									: undefined
							}
							subtitle={
								xirr?.currentValue != null ? fmt(xirr.currentValue) : undefined
							}
							icon={IndianRupee}
							loading={xirrQuery.isLoading}
						/>
					...
					{schemes.length > 0 && (
						<Card>
							<CardHeader>
								<CardTitle className="text-sm">
									Portfolio XIRR Trend (by Scheme)
								</CardTitle>
							</CardHeader>
							<CardContent>
								{returnsQuery.isLoading ? (
									...
									<div className="space-y-2">
										{[...schemes]
											...
											.map((s, i) => {
												const max = Math.max(
													...schemes.map((x) => Math.abs(x.xirr ?? 0)),
													1,
												);
												const pct = Math.min(
													Math.abs((s.xirr ?? 0) / max) * 100,
													100,
												);
												...
												return (
													...
													>
														...
														<div className="flex-1 bg-muted rounded h-5 relative overflow-hidden">
															...
															/>
															<span
																className={`absolute right-2 top-0 h-full flex items-center font-medium ${isPos ? "text-green-700" : "text-red-600"}`}
															>
										...
										<tbody>
											{schemes.map((s, i) => (
												<tr key={i} className="border-b hover:bg-muted/50">
													...
													<td
														className={`p-3 text-right ${(s.absoluteReturn ?? 0) >= 0 ? "text-green-600" : "text-red-500"}`}
													>
														{s.absoluteReturn != null
															? `${s.absoluteReturn.toFixed(2)}%`
															: "—"}
													</td>
													<td
														className={`p-3 text-right ${(s.xirr ?? 0) >= 0 ? "text-green-600" : "text-red-500"}`}
													>
														{s.xirr != null ? `${s.xirr.toFixed(2)}%` : "—"}
													</td>
													<td
														className={`p-3 text-right ${(s.cagr ?? 0) >= 0 ? "text-green-600" : "text-red-500"}`}
													>
					...
					{(sipXirr?.sips?.length ?? 0) > 0 && (
						<Card>
							<CardHeader>
								<CardTitle className="text-sm">SIP-specific XIRR</CardTitle>
							</CardHeader>
							<CardContent className="p-0">
								<div className="divide-y">
									{sipXirr!.sips!.map((s, i) => (
										...
										>
											...
											<div className="flex items-center gap-3 text-sm">
												...
												<span
													className={`font-medium ${(s.xirr ?? 0) >= 0 ? "text-green-600" : "text-red-500"}`}
												>
													{s.xirr != null ? `${s.xirr.toFixed(2)}%` : "—"}
												</span>
											</div>
					...
					<Card>
						<CardHeader>
							<CardTitle className="flex items-center gap-2 text-sm">
								<IndianRupee className="h-4 w-4 text-amber-500" /> Tax Harvest
								Opportunities
							</CardTitle>
							<CardDescription>
								Holdings where selling now qualifies for LTCG exemption or
								avoids exit load, ranked by potential tax saving.
							</CardDescription>
						</CardHeader>
						<CardContent>
							{taxHarvestQuery.isLoading ? (
										...
										<tbody>
											{taxOpps.map((op, i) => (
												<tr key={i} className="border-b hover:bg-muted/50">
													...
													<td
														className={`p-3 text-right ${(op.gainLoss ?? 0) >= 0 ? "text-green-600" : "text-red-500"}`}
													>
														{fmt(op.gainLoss)}
													</td>
													<td className="p-3 text-right">
														{op.ltcgExemption != null
															? fmt(op.ltcgExemption)
															: "—"}
													</td>
													<td className="p-3 text-right font-medium text-green-600">
														{op.taxSaving != null ? fmt(op.taxSaving) : "—"}
													</td>
													<td className="p-3 text-center">
														<Badge
															variant={
																op.exitLoadApplicable
																	? "destructive"
																	: "default"
															}
															className="text-[10px]"
													...
													<td className="p-3 text-center">
														<Button
															...
															onClick={() => {
																...
																if (!url) {
																	alert(
																		"Open Transact tab to initiate redemption for: " +
																			(op.schemeName ?? "this scheme"),
																	);
																	return;
																}
															}}
								...
								<p className="text-sm text-muted-foreground">
									Tax harvest data unavailable for this PAN.
								</p>
							) : (
								<p className="text-sm text-muted-foreground">
									No tax harvest opportunities found.
								</p>
							)}
						</CardContent>
					</Card>
...
function ResearchTab() {
	...
	const schemeSearchData = useQuery<{
		success: boolean;
		data: { schemes?: SchemeResult[] } | SchemeResult[];
	}>({
		queryKey: ["/api/iris/transactions/scheme-search", schemeQuery],
		queryFn: () =>
			irisGet(
	...
	const compareSearchData = useQuery<{
		success: boolean;
		data: { schemes?: SchemeResult[] } | SchemeResult[];
	}>({
		...
		queryFn: () =>
			irisGet(
				`/api/iris/transactions/scheme-search?q=${encodeURIComponent(compareQuery)}`,
			),
	...
	const navHistoryQuery = useQuery<{
		success: boolean;
		data: { history?: Array<{ date: string; nav: number }> };
	}>({
		queryKey: ["/api/iris/schemes", schemeCode, "nav-history", navPeriod],
		queryFn: () =>
			irisGet(
				`/api/iris/schemes/${schemeCode}/nav-history?period=${navPeriod}`,
			),
		enabled: !!schemeCode,
		retry: false,
	});
	...
	const fundManagerQuery = useQuery<{
		success: boolean;
		data: {
			name?: string;
			experience?: string;
			schemesManaged?: number | string;
	...
	const factsheetQuery = useQuery<{
		success: boolean;
		data: { downloadUrl?: string; url?: string; factsheetUrl?: string };
	}>({
	...
	const subcategoriesQuery = useQuery<{
		success: boolean;
		data: { subcategories?: string[] } | string[];
	}>({
		queryKey: ["/api/iris/subcategories", browseCategory],
		queryFn: () =>
			irisGet(
				`/api/iris/subcategories${browseCategory ? "?category=" + encodeURIComponent(browseCategory) : ""}`,
			),
	...
	const byCategoryQuery = useQuery<{
		...
		queryFn: () => {
			...
			if (browseSubcategory && browseSubcategory !== "ALL")
				params.subcategory = browseSubcategory;
			if (riskProfile && riskProfile !== "ANY")
				params.riskProfile = riskProfile;
			const qs =
				"?" +
				new URLSearchParams({ category: browseCategory, ...params }).toString();
	...
	const compareQuery2 = useMutation({
		mutationFn: (schemeCodes: string[]) =>
			apiRequest("/api/iris/schemes/compare", "POST", {
				body: { schemeCodes },
			}),
	});
	...
	const topPerformersQuery = useQuery<{
		...
		queryFn: () =>
			irisGet(
				`/api/iris/schemes/top-performers${browseCategory ? "?category=" + encodeURIComponent(browseCategory) : ""}`,
			),
		enabled: section === "browse" || section === "search",
		retry: false,
	});

	const riskRecommendationsQuery = useQuery<{
		success: boolean;
		data: { schemes?: SchemeResult[] } | SchemeResult[];
	}>({
	...
	function resolveCategories(): string[] {
		...
		if (Array.isArray(categoriesQuery.data.data))
			return categoriesQuery.data.data;
		return (
			(categoriesQuery.data.data as { categories?: string[] }).categories ?? []
		);
	}

	function resolveSubcategories(): string[] {
		if (!subcategoriesQuery.data?.data) return [];
		if (Array.isArray(subcategoriesQuery.data.data))
			return subcategoriesQuery.data.data;
		return (
			(subcategoriesQuery.data.data as { subcategories?: string[] })
				.subcategories ?? []
		);
	}
	...
	const factsheetUrl =
		factsheetQuery.data?.data?.downloadUrl ??
		factsheetQuery.data?.data?.url ??
		factsheetQuery.data?.data?.factsheetUrl;
	const compareResult = compareQuery2.data as
		| {
				data?: {
					schemes?: Array<{
						schemeName?: string;
		...
		<div className="space-y-4">
			...
			{section === "search" && (
				<div className="space-y-4">
					<Card>
						<CardHeader>
							<CardTitle className="text-sm">Search Scheme</CardTitle>
						</CardHeader>
						<CardContent>
						...
						<>
							<div className="flex items-center gap-2 flex-wrap">
								...
								{factsheetUrl && (
									...
									>
										<Button size="sm" variant="outline">
											<Download className="h-3.5 w-3.5 mr-1" /> Factsheet PDF
										</Button>
									</a>
								)}
							</div>
									...
									<CardContent>
										{performanceQuery.isLoading ? (
											...
											<div className="space-y-2 text-sm">
												<div className="grid grid-cols-3 gap-2">
													{[
														...
														>
															...
															<p
																className={`font-semibold ${(val ?? 0) >= 0 ? "text-green-600" : "text-red-500"}`}
															>
												...
												{perf?.expenseRatio != null && (
													<p className="text-xs text-muted-foreground">
														Expense Ratio:{" "}
														<span className="font-medium text-foreground">
															{perf.expenseRatio.toFixed(2)}%
														</span>
													</p>
												)}
												{perf?.stdDeviation != null && (
													<p className="text-xs text-muted-foreground">
														Std Dev:{" "}
														<span className="font-medium text-foreground">
															{perf.stdDeviation.toFixed(2)}
														</span>
													</p>
												)}
												{perf?.sharpe != null && (
								...
								<Card>
									<CardHeader>
										<CardTitle className="text-sm flex items-center gap-1">
											<User className="h-4 w-4" /> Fund Manager
										</CardTitle>
									</CardHeader>
									<CardContent>
										{fundManagerQuery.isLoading ? (
											<Skeleton className="h-16 w-full" />
										) : fundManager?.name ? (
											<div className="space-y-1 text-sm">
												...
												{fundManager.schemesManaged != null && (
													<p className="text-xs text-muted-foreground">
														Schemes managed: {fundManager.schemesManaged}
													</p>
												)}
											</div>
										) : (
											<p className="text-xs text-muted-foreground">
												Fund manager info unavailable
											</p>
										)}
									</CardContent>
								</Card>
							...
							{(benchmarkQuery.data?.data?.benchmark ||
								benchmarkQuery.data?.data?.benchmarkReturns) && (
								<Card>
									<CardHeader>
										<CardTitle className="text-sm">
											Benchmark Comparison
										...
										{benchmark?.benchmark && (
											<CardDescription>{benchmark.benchmark}</CardDescription>
										)}
									</CardHeader>
									<CardContent>
										{benchmarkQuery.isLoading ? (
											...
											<div className="grid grid-cols-3 gap-3">
												{[
													{
														label: "1Y",
														scheme: perf?.returns?.oneYear,
														bench: benchmark?.benchmarkReturns?.oneYear,
													},
													{
														label: "3Y",
														scheme: perf?.returns?.threeYear,
														bench: benchmark?.benchmarkReturns?.threeYear,
													},
													{
														label: "5Y",
														scheme: perf?.returns?.fiveYear,
														bench: benchmark?.benchmarkReturns?.fiveYear,
													},
													...
													<div key={label} className="bg-muted rounded p-3">
														...
														<div className="flex justify-between text-xs">
															<span>
																Fund:{" "}
																<span
																	className={`font-medium ${(scheme ?? 0) >= 0 ? "text-green-600" : "text-red-500"}`}
																>
																	{scheme != null
																		? `${scheme.toFixed(2)}%`
																		: "—"}
							...
							<Card>
										...
										<div className="flex gap-1">
											{NAV_PERIODS.map((p) => (
												<Button
													key={p}
													size="sm"
													variant={navPeriod === p ? "default" : "outline"}
								...
								<CardContent>
									{navHistoryQuery.isLoading ? (
											...
											<div className="relative h-36 bg-muted/30 rounded">
												...
												{(() => {
													...
													const points = navs
														.map(
															(n, i) =>
																`${(i / (navs.length - 1)) * w},${h - ((n - minNav) / range) * h}`,
														)
														.join(" ");
										...
										<p className="text-sm text-muted-foreground">
											NAV history unavailable for this period.
										</p>
									)}
								</CardContent>
							</Card>
					...
					{(() => {
						...
						return topPerformers.length > 0 ? (
							<Card>
								<CardHeader>
									<CardTitle className="text-sm flex items-center gap-1">
										<TrendingUp className="h-4 w-4 text-primary" /> Top
										Performing Schemes
									</CardTitle>
									<CardDescription>
										Ranked by 1-year return across all categories
									</CardDescription>
								</CardHeader>
								<CardContent className="p-0">
									<table className="w-full text-sm">
										...
										<tbody>
											{topPerformers.slice(0, 10).map((s, i) => (
												<tr key={i} className="border-b hover:bg-muted/50">
													...
													<td
														className={`p-3 text-right font-medium ${(s.return1y ?? s.oneYearReturn ?? 0) >= 0 ? "text-green-600" : "text-red-500"}`}
													>
														{(s.return1y ?? s.oneYearReturn) != null
															? `${(s.return1y ?? s.oneYearReturn)!.toFixed(2)}%`
															: "—"}
													</td>
													<td className="p-3 text-right">
														<Button
				...
				<div className="space-y-4">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Add schemes using the search box below. Side-by-side returns,
								risk, ratings and expense ratio.
							</CardDescription>
						</CardHeader>
						<CardContent className="space-y-3">
							<div className="flex gap-2 flex-wrap items-center">
								{compareList.map((s, i) => (
									...
									>
										...
										<button
											...
											onClick={() =>
												setCompareList((l) => l.filter((_, j) => j !== i))
											}
										>
								...
								<Button
									...
									onClick={() =>
										compareQuery2.mutate(
											compareList.map(
												(s) => s.schemeCode ?? s.isinCode ?? s.code ?? "",
											),
										)
									}
								>
									{compareQuery2.isPending ? (
										<RefreshCw className="h-4 w-4 animate-spin mr-1" />
									) : (
					...
					{compareResult?.data?.schemes &&
						compareResult.data.schemes.length > 0 && (
							<Card>
								<CardHeader>
										...
										<table className="w-full text-sm min-w-[600px]">
											<thead className="border-b bg-muted/50">
												<tr>
													...
													{compareResult.data.schemes.map((s, i) => (
														<th
															key={i}
															className="text-right p-3 font-medium max-w-[150px]"
														>
															{s.schemeName ?? `Scheme ${i + 1}`}
														</th>
													))}
												</tr>
											</thead>
											<tbody>
												{[
													{
														label: "1Y Return",
														fn: (s: (typeof compareResult.data.schemes)[0]) =>
															s.returns?.oneYear != null
																? `${s.returns.oneYear.toFixed(2)}%`
																: "—",
													},
													{
														label: "3Y Return",
														fn: (s: (typeof compareResult.data.schemes)[0]) =>
															s.returns?.threeYear != null
																? `${s.returns.threeYear.toFixed(2)}%`
																: "—",
													},
													{
														label: "5Y Return",
														fn: (s: (typeof compareResult.data.schemes)[0]) =>
															s.returns?.fiveYear != null
																? `${s.returns.fiveYear.toFixed(2)}%`
																: "—",
													},
													{
														label: "Std Deviation",
				...
				<div className="space-y-4">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Filter schemes by category and sub-category. Set a risk profile
								for recommendations.
							</CardDescription>
						</CardHeader>
						<CardContent className="space-y-3">
							<div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
								...
								<div>
									...
									<Select
										...
										onValueChange={setBrowseSubcategory}
								...
								<div>
									...
									<Select value={riskProfile} onValueChange={setRiskProfile}>
										<SelectTrigger>
											<SelectValue placeholder="Any risk profile" />
										</SelectTrigger>
										<SelectContent>
											...
											{categorySchemes.map((s, i) => (
												...
												>
													<div>
														...
														<p className="text-xs text-muted-foreground">
															{s.schemeCode ?? s.isinCode ?? s.code}
														</p>
													</div>
													<Button
														size="sm"
					...
					{riskProfile !== "ANY" && (
						<Card>
							<CardHeader>
								<CardTitle className="text-sm flex items-center gap-1">
									<Star className="h-4 w-4 text-amber-500" /> Recommended for{" "}
									{riskProfile} Profile
								</CardTitle>
								<CardDescription>
									Schemes curated for a {riskProfile.toLowerCase()} risk
									investor
								</CardDescription>
							</CardHeader>
							<CardContent>
								{riskRecommendationsQuery.isLoading ? (
									...
									(() => {
										...
										return recs.length > 0 ? (
											...
											<p className="text-sm text-muted-foreground">
												No recommendations available for this risk profile.
											</p>
										);
									})()
								)}
...
function CasImportTab() {
	...
	const casQuery = useQuery<{
		success: boolean;
		data: { holdings?: CasHolding[]; summary?: CasSummary };
	}>({
	...
	const importMutation = useMutation({
		mutationFn: () =>
			apiRequest("/api/iris/portfolio/import", "POST", {
				body: {
					pan: casSubmittedPan,
		...
		onSuccess: () => {
			toast({
				...
				description: `Holdings for ${casSubmittedPan} saved to IRIS.`,
			});
			qc.invalidateQueries({
				queryKey: ["/api/iris/portfolio/external", casSubmittedPan],
			});
		},
		onError: (e: Error) =>
			toast({
	...
	const generateCasMutation = useMutation({
		mutationFn: () =>
			apiRequest("/api/iris/reports/cas/generate", "POST", {
				body: {
					pan: casSubmittedPan,
		...
		onSuccess: () => {
			toast({
				title: "CAS statement generated",
				description: genEmail
					? `Sent to ${genEmail}`
	...
	const refreshMutation = useMutation({
		mutationFn: (pan: string) =>
			apiRequest(`/api/iris/portfolio/external/${pan}/refresh`, "POST", {
				body: {},
			}),
		onSuccess: (_d, pan) => {
			toast({
				title: "Refreshed",
				description: `External portfolio refreshed for ${pan}.`,
			});
	...
	const unlinkMutation = useMutation({
		mutationFn: (folioNo: string) =>
			apiRequest(`/api/iris/portfolio/external/${folioNo}`, "DELETE"),
		onSuccess: () => {
	...
	const linkMutation = useMutation({
		mutationFn: () =>
			apiRequest("/api/iris/portfolio/external/link", "POST", {
				body: {
					pan: extSubmittedPan,
					folioNo: linkFolioNo,
		...
		<div className="space-y-6">
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Pull a client's complete MF portfolio directly from KFintech by PAN
				...
				<CardContent className="space-y-4">
					...
					{casQuery.isError && (
						<div className="flex items-center gap-2 text-destructive text-sm">
							<AlertCircle className="h-4 w-4" /> Could not fetch CAS data —
							check PAN or IRIS credentials.
						</div>
					)}
					...
					{casHoldings.length > 0 && (
						<>
									...
									<tbody>
										{casHoldings.map((h, i) => (
											<tr key={i} className="border-b hover:bg-muted/50">
												...
												<td
													className={`p-2 text-right ${(h.gainLossPercentage ?? 0) >= 0 ? "text-green-600" : "text-red-500"}`}
												>
													{h.gainLossPercentage !== undefined
														? `${h.gainLossPercentage.toFixed(2)}%`
														: "—"}
												</td>
											</tr>
							...
							<div className="flex gap-2 flex-wrap">
								...
								<Button
									variant="outline"
									onClick={() => setGenEmailDialogOpen(true)}
								>
					...
					{casSubmittedPan &&
						...
						!casQuery.isError && (
							<p className="text-sm text-muted-foreground">
								No holdings found for PAN {casSubmittedPan}.
							</p>
						)}
				</CardContent>
			</Card>
					...
					<div className="space-y-3">
						...
						<div className="flex gap-2">
							<Button
								variant="outline"
								className="flex-1"
								onClick={() => setGenEmailDialogOpen(false)}
							>
								Cancel
							</Button>
							<Button
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						View and manage externally linked folios for an investor — covers
						both CAMS and KFintech registrars.
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-4">
					<div className="flex gap-2 flex-wrap">
					...
					{extQuery.isError && (
						<div className="flex items-center gap-2 text-destructive text-sm">
							<AlertCircle className="h-4 w-4" /> Could not load external
							portfolio.
						</div>
					)}
										...
										<tr key={i} className="border-b hover:bg-muted/50">
											...
											<td className="p-2 text-right">
												{f.currentValue !== undefined
													? `₹${Number(f.currentValue).toLocaleString("en-IN")}`
													: "—"}
											</td>
											<td className="p-2 text-right">
												<Button
													...
													onClick={() =>
														f.folioNo && unlinkMutation.mutate(f.folioNo)
													}
					...
					{extSubmittedPan &&
						...
						!extQuery.isError && (
							<p className="text-sm text-muted-foreground">
								No external folios linked for {extSubmittedPan}. Use "Link
								Folio" to add one, or import via CAS fetch above.
							</p>
						)}
				</CardContent>
			</Card>
			...
			<Dialog open={linkDialogOpen} onOpenChange={setLinkDialogOpen}>
				<DialogContent className="max-w-sm">
					<DialogHeader>
						...
						<DialogDescription>
							Manually link a folio from CAMS or KFintech to investor{" "}
							{extSubmittedPan}.
						</DialogDescription>
					</DialogHeader>
					<div className="space-y-3">
...
function MandatesTab() {
	...
	const enachQuery = useQuery<{
		success: boolean;
		data: { mandates?: EnachMandate[] } | EnachMandate[];
	}>({
		queryKey: ["/api/iris/enach", submittedPan],
		queryFn: () =>
			irisGet(`/api/iris/enach?pan=${encodeURIComponent(submittedPan)}`),
		enabled: !!submittedPan,
		retry: false,
	});
	...
	const createEnach = useMutation({
		mutationFn: (body: Record<string, unknown>) =>
			apiRequest("/api/iris/enach/create", "POST", { body }),
		onSuccess: () => {
	...
	const cancelEnach = useMutation({
		mutationFn: (mandateId: string) =>
			apiRequest(`/api/iris/enach/${mandateId}/cancel`, "POST"),
		onSuccess: () => {
	...
	const regenEnachLink = useMutation({
		mutationFn: (mandateId: string) =>
			apiRequest(`/api/iris/enach/${mandateId}/regenerate-link`, "POST"),
	...
	const upiQuery = useQuery<{
		success: boolean;
		data: { mandates?: UpiMandate[] } | UpiMandate[];
	}>({
		queryKey: ["/api/iris/mandates/upi", submittedPan],
		queryFn: () =>
			irisGet(`/api/iris/mandates/upi?pan=${encodeURIComponent(submittedPan)}`),
		enabled: !!submittedPan,
		retry: false,
	});
	...
	const createUpi = useMutation({
		mutationFn: (body: Record<string, unknown>) =>
			apiRequest("/api/iris/mandates/upi", "POST", { body }),
		onSuccess: () => {
		...
		onError: (e: Error) =>
			toast({
				title: "Failed to create UPI mandate",
				description: e.message,
				variant: "destructive",
			}),
	});
	...
	const cancelUpi = useMutation({
		mutationFn: (umrn: string) =>
			apiRequest(`/api/iris/mandates/upi/${umrn}/cancel`, "POST"),
		onSuccess: () => {
		...
		onError: (e: Error) =>
			toast({
				title: "Failed to cancel UPI mandate",
				description: e.message,
				variant: "destructive",
			}),
	});
	...
	const physQuery = useQuery<{
		success: boolean;
		data: { mandates?: PhysicalMandate[] } | PhysicalMandate[];
	}>({
		queryKey: ["/api/iris/mandates/physical", submittedPan],
		queryFn: () =>
			irisGet(
				`/api/iris/mandates/physical?pan=${encodeURIComponent(submittedPan)}`,
			),
		enabled: !!submittedPan,
		retry: false,
	});
		...
		<div className="space-y-6">
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Enter investor PAN to view and manage all mandates (eNACH, UPI
						Autopay, Physical NACH)
					</CardDescription>
				</CardHeader>
				<CardContent>
					<div className="flex gap-2">
			...
			<Card>
				<CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3">
					<div>
						...
						<CardDescription>
							Electronic NACH bank mandates for SIP auto-debit
						</CardDescription>
					</div>
					{submittedPan && (
				...
				<CardContent>
					{!submittedPan ? (
						<p className="text-sm text-muted-foreground">
							Enter PAN above to load mandates
						</p>
									...
									>
										<div className="min-w-0 flex-1">
											<div className="flex items-center gap-2 flex-wrap">
												...
												<Badge
													variant={mandateStatusVariant(m.status)}
													className="text-[10px] h-4"
												>
													{m.status ?? "Unknown"}
												</Badge>
						...
						<p className="text-sm text-muted-foreground">
							{enachQuery.isError
								? "Failed to load eNACH mandates"
								: `No eNACH mandates found for ${submittedPan}`}
						</p>
					)}
				</CardContent>
			</Card>
			...
			<Card>
				<CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3">
					<div>
						...
						<CardDescription>
							UPI-based recurring payment mandates (UMRN)
						</CardDescription>
					</div>
					{submittedPan && (
				...
				<CardContent>
					{!submittedPan ? (
										...
										{(m.status?.toUpperCase() === "ACTIVE" ||
											...
											umrn && (
												<Button
													size="sm"
													variant="outline"
													className="h-7 text-xs px-2 text-destructive hover:text-destructive flex-shrink-0"
						...
						<p className="text-sm text-muted-foreground">
							{upiQuery.isError
								? "Failed to load UPI mandates"
								: `No UPI mandates found for ${submittedPan}`}
						</p>
					)}
				</CardContent>
			</Card>
			...
			<Card>
				<CardHeader>
					<CardTitle className="text-sm font-semibold">
						Physical NACH Upload
					</CardTitle>
					<CardDescription>
						Upload a scanned NACH mandate form (PDF or image)
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-4">
					{!submittedPan ? (
						<p className="text-sm text-muted-foreground">
							Enter PAN above to upload physical NACH
						</p>
					) : (
						<>
							<div
								...
								onClick={() =>
									document.getElementById("phys-file-input")?.click()
								}
							>
								<input
									id="phys-file-input"
								...
								{physFile ? (
									<div className="flex items-center justify-center gap-2">
										...
										<span className="text-xs text-muted-foreground">
											({(physFile.size / 1024).toFixed(1)} KB)
										</span>
									</div>
								) : (
									<div>
										...
										<p className="text-sm text-muted-foreground">
											Drag & drop or click to select scanned NACH form
										</p>
										<p className="text-xs text-muted-foreground mt-1">
							...
							<div className="grid grid-cols-1 md:grid-cols-3 gap-3">
								...
								<div>
									...
									<Input
										value={physAccount}
										onChange={(e) => setPhysAccount(e.target.value)}
								...
								<div>
									...
									<Input
										value={physIfsc}
										onChange={(e) => setPhysIfsc(e.target.value.toUpperCase())}
			...
			<Dialog open={createEnachOpen} onOpenChange={setCreateEnachOpen}>
				<DialogContent className="sm:max-w-md">
					<DialogHeader>
						...
						<DialogDescription>
							Register a new electronic NACH mandate for {submittedPan}
						</DialogDescription>
					</DialogHeader>
					<div className="space-y-3 pt-1">
						...
						<div className="grid grid-cols-2 gap-3">
							<div className="col-span-2">
								...
								<Input
									value={enachBank}
									onChange={(e) => setEnachBank(e.target.value)}
									placeholder="HDFC Bank"
								/>
							</div>
							<div>
								...
								<Input
									value={enachAccount}
									onChange={(e) => setEnachAccount(e.target.value)}
							...
							<div>
								...
								<Input
									value={enachIfsc}
									onChange={(e) => setEnachIfsc(e.target.value.toUpperCase())}
									placeholder="HDFC0001234"
									maxLength={11}
								/>
							</div>
							<div>
								...
								<Input
									type="number"
									value={enachAmount}
									onChange={(e) => setEnachAmount(e.target.value)}
									placeholder="25000"
									min={1}
								/>
							</div>
							<div>
								...
								<Select value={enachFreq} onValueChange={setEnachFreq}>
									...
									<SelectContent>
										...
										<SelectItem value="HALF_YEARLY">Half-Yearly</SelectItem>
										<SelectItem value="YEARLY">Yearly</SelectItem>
										<SelectItem value="AS_AND_WHEN_PRESENTED">
											As Presented
										</SelectItem>
									</SelectContent>
								</Select>
						...
						<Button
							...
							onClick={() =>
								createEnach.mutate({
									pan: submittedPan,
									bankName: enachBank,
									accountNumber: enachAccount,
									ifscCode: enachIfsc,
									maxAmount: Number(enachAmount),
									frequency: enachFreq,
								})
							}
							disabled={
								createEnach.isPending ||
								!enachBank ||
								!enachAccount ||
								!enachIfsc ||
								!enachAmount
				...
				<DialogContent className="sm:max-w-sm">
					<DialogHeader>
						...
						<DialogDescription>
							...
							<span className="font-mono font-medium">{cancelEnachId}</span>?
			...
			<Dialog open={createUpiOpen} onOpenChange={setCreateUpiOpen}>
				<DialogContent className="sm:max-w-md">
					<DialogHeader>
						...
						<DialogDescription>
							Register a new UPI autopay mandate for {submittedPan}
						</DialogDescription>
					</DialogHeader>
					<div className="space-y-3 pt-1">
						<div className="flex items-center gap-2 p-2 rounded bg-muted/50 text-sm">
			...
			>
				<DialogContent className="sm:max-w-sm">
					<DialogHeader>
						...
						<DialogDescription>
							Are you sure you want to cancel UPI mandate{" "}
							<span className="font-mono font-medium">{cancelUpiUmrn}</span>?
							This action cannot be undone.
						</DialogDescription>
					</DialogHeader>
					<div className="flex gap-2 pt-2">
...
function CommissionsTab() {
	...
	const { data: statement, isLoading: stmtL } = useQuery<
		IrisApiResponse<{ commissions?: CommissionRow[] } | CommissionRow[]>
	>({
	...
	const { data: trail, isLoading: trailL } = useQuery<
		IrisApiResponse<{ trail?: TrailRow[] } | TrailRow[]>
	>({
	...
	const { data: amcWise, isLoading: amcL } = useQuery<
		IrisApiResponse<{ breakdown?: AmcCommission[] } | AmcCommission[]>
	>({
		queryKey: [
	...
	function resolveArr<T>(
		d:
			| IrisApiResponse<
					{ commissions?: T[]; trail?: T[]; breakdown?: T[] } | T[]
			  >
			| undefined,
		key: "commissions" | "trail" | "breakdown",
	): T[] {
		if (!d?.data) return [];
	...
	const commissions = resolveArr<CommissionRow>(
		statement as IrisApiResponse<
			{ commissions?: CommissionRow[] } | CommissionRow[]
		>,
		"commissions",
	);
	const trailRows = resolveArr<TrailRow>(
		trail as IrisApiResponse<{ trail?: TrailRow[] } | TrailRow[]>,
		"trail",
	);
	const amcRows = resolveArr<AmcCommission>(
		amcWise as IrisApiResponse<
			{ breakdown?: AmcCommission[] } | AmcCommission[]
		>,
		"breakdown",
	);
	...
	return (
		<div className="space-y-6">
			<Card>
				...
				<CardContent>
					<div className="grid grid-cols-1 md:grid-cols-3 gap-3">
						...
						<div>
							<Label>AMC Code (optional)</Label>
							<Input
								placeholder="e.g. HDFC"
								value={amcCode}
								onChange={(e) => setAmcCode(e.target.value.toUpperCase())}
							/>
						</div>
					</div>
				</CardContent>
			...
			<div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
				<Card>
					<CardHeader>
						<CardTitle className="text-sm">Commission Statement</CardTitle>
					</CardHeader>
					<CardContent className="p-0">
						{stmtL ? (
							<div className="p-4 space-y-2">
								...
								<table className="w-full text-sm">
									...
									<tbody>
										{commissions.map((c, i) => (
											<tr key={i} className="border-b hover:bg-muted/50">
												<td className="p-2">
													{c.schemeName ?? c.scheme ?? "—"}
												</td>
												<td className="p-2 text-right">
													{fmt(c.commissionAmount ?? c.amount)}
												</td>
												<td className="p-2 text-right">
													<Badge variant="outline" className="text-xs">
														{c.commissionType ?? c.type ?? "—"}
													</Badge>
												</td>
											</tr>
										))}
				...
				<Card>
					<CardHeader>
						<CardTitle className="text-sm">Trail Commission</CardTitle>
					</CardHeader>
					<CardContent className="p-0">
						{trailL ? (
							<div className="p-4 space-y-2">
									...
									<tbody>
										{trailRows.map((t, i) => (
											<tr key={i} className="border-b hover:bg-muted/50">
												...
												<td className="p-2 text-right">
													{t.trailRate != null ? t.trailRate + "%" : "—"}
												</td>
												<td className="p-2 text-right">
													{fmt(t.trailAmount ?? t.amount)}
												</td>
											</tr>
										))}
										{!trailRows.length && (
				...
				<CardHeader>
					<div className="flex items-center justify-between">
						...
						<Button
							...
							variant="outline"
							onClick={() => {
								const csv = ["AMC,AUM,Commission,Trail"]
									.concat(
										amcRows.map(
											(r) =>
												`${r.amcName ?? r.amc ?? ""},${r.aum ?? ""},${r.commission ?? ""},${r.trail ?? ""}`,
										),
									)
									.join("\n");
								...
								a.download = "amc-commissions.csv";
								a.click();
							}}
						>
...
function ReportsTab() {
	...
	async function runReport() {
		...
		try {
			...
			const json = await irisGet<typeof result>(
				`/api/iris/reports/${reportType}/${pan}${qs.toString() ? "?" + qs.toString() : ""}`,
			);
			setResult(json);
		} catch {
	...
	const bulkCgParams = new URLSearchParams({ fy: fySelector });
	if (amcFilter) bulkCgParams.set("amcCode", amcFilter);
	const {
		data: bulkCg,
		isLoading: bcgL,
		refetch: refetchBcg,
	} = useQuery<IrisApiResponse<{ downloadUrl?: string; records?: unknown[] }>>({
		queryKey: ["/api/iris/reports/bulk/capital-gains", fySelector, amcFilter],
		queryFn: () =>
			irisGet(
				`/api/iris/reports/bulk/capital-gains?${bulkCgParams.toString()}`,
			),
	...
	if (fromDate) divParams.set("fromDate", fromDate);
	if (toDate) divParams.set("toDate", toDate);
	if (amcFilter) divParams.set("amcCode", amcFilter);
	const {
		data: dividends,
		isLoading: divL,
		refetch: refetchDiv,
	} = useQuery<IrisApiResponse<{ dividends?: DividendRow[] } | DividendRow[]>>({
		queryKey: [
		...
		queryFn: () =>
			irisGet(
				`/api/iris/reports/dividend-tracker${divParams.toString() ? "?" + divParams.toString() : ""}`,
			),
	...
	const {
		data: sipCal,
		isLoading: sipCalL,
		refetch: refetchSipCal,
	} = useQuery<IrisApiResponse<{ sips?: SipCalRow[] } | SipCalRow[]>>({
		queryKey: ["/api/iris/reports/sip-maturity-calendar", sipWindow],
		queryFn: () =>
			irisGet(
				`/api/iris/reports/sip-maturity-calendar?${sipParams.toString()}`,
			),
	...
	if (toDate) bpParams.set("asOfDate", toDate);
	const {
		data: bulkPortfolio,
		isLoading: bpL,
		refetch: refetchBp,
	} = useQuery<IrisApiResponse<{ downloadUrl?: string }>>({
		queryKey: ["/api/iris/reports/bulk/portfolio", toDate],
		queryFn: () =>
			irisGet(
				`/api/iris/reports/bulk/portfolio${bpParams.toString() ? "?" + bpParams.toString() : ""}`,
			),
		...
		<div className="space-y-4">
			...
			{section === "per-investor" && (
				<>
					<Card>
						<CardHeader>
							<CardTitle className="text-sm">Per-Investor Report</CardTitle>
						</CardHeader>
						<CardContent className="space-y-4">
					...
					{result && (
						<Card>
							<CardHeader>
								<CardTitle className="text-sm">Report Result</CardTitle>
							</CardHeader>
							<CardContent>
								{result.success ? (
			...
			{section === "bulk-cg" && (
				<Card>
					<CardHeader>
						<CardTitle className="text-sm flex items-center gap-2">
							<TrendingDown className="h-4 w-4" /> Bulk Capital Gains Report
						</CardTitle>
						<CardDescription>
							All investors — capital gains for the selected financial year
						</CardDescription>
					</CardHeader>
					<CardContent className="space-y-4">
						<div className="flex gap-3 flex-wrap items-end">
							...
							<div>
								<Label>AMC Filter (optional)</Label>
								<Input
									placeholder="AMC code"
									value={amcFilter}
									onChange={(e) => setAmcFilter(e.target.value.toUpperCase())}
									className="w-32"
								/>
							</div>
							<Button onClick={() => refetchBcg()} disabled={bcgL}>
								{bcgL ? (
									<RefreshCw className="h-4 w-4 animate-spin mr-1" />
								) : (
						...
						{bulkCg && !bulkCg.data?.downloadUrl && (
							<pre className="text-xs bg-muted/30 rounded p-3 overflow-auto max-h-48">
								{JSON.stringify(bulkCg.data, null, 2)}
							</pre>
						)}
					</CardContent>
				</Card>
			...
			{section === "dividend-tracker" && (
				<Card>
					<CardHeader>
						<CardTitle className="text-sm flex items-center gap-2">
							<IndianRupee className="h-4 w-4" /> Dividend Tracker
						</CardTitle>
						<CardDescription>
							Upcoming dividends across your book
						</CardDescription>
					</CardHeader>
					<CardContent className="space-y-4">
						<div className="flex gap-3 flex-wrap items-end">
							...
							<Button onClick={() => refetchDiv()} disabled={divL}>
								{divL ? (
									<RefreshCw className="h-4 w-4 animate-spin mr-1" />
								) : (
							...
							{resolveDivArr().length > 0 && (
								<Button
									variant="outline"
									onClick={() => {
										const csv = ["Scheme,Dividend,Record Date,AMC"]
											.concat(
												resolveDivArr().map(
													(d) =>
														`"${d.schemeName ?? d.scheme ?? ""}",${d.dividendAmount ?? ""},${d.recordDate ?? ""},"${d.amcName ?? d.amc ?? ""}"`,
												),
											)
											.join("\n");
										const blob = new Blob([csv], { type: "text/csv" });
										const a = document.createElement("a");
										a.href = URL.createObjectURL(blob);
										a.download = "dividend-tracker.csv";
										a.click();
									}}
								>
								...
								<table className="w-full text-sm">
									<thead className="sticky top-0 bg-background border-b">
										<tr>
											...
											<th className="text-right p-2">Record Date</th>
											<th className="text-right p-2">AMC</th>
										</tr>
									</thead>
									<tbody>
										{resolveDivArr().map((d, i) => (
											<tr key={i} className="border-b hover:bg-muted/50">
												<td className="p-2">
													{d.schemeName ?? d.scheme ?? "—"}
												</td>
												<td className="p-2 text-right">
													{d.dividendAmount != null
														? `₹${d.dividendAmount}`
														: "—"}
												</td>
												<td className="p-2 text-right">
			...
			{section === "sip-calendar" && (
				<Card>
					<CardHeader>
						<CardTitle className="text-sm flex items-center gap-2">
							<Calendar className="h-4 w-4" /> SIP Maturity Calendar
						</CardTitle>
						<CardDescription>
							SIPs maturing within the selected window
						</CardDescription>
					</CardHeader>
					<CardContent className="space-y-4">
						<div className="flex gap-3 flex-wrap items-end">
							...
							<Button onClick={() => refetchSipCal()} disabled={sipCalL}>
								{sipCalL ? (
									<RefreshCw className="h-4 w-4 animate-spin mr-1" />
								) : (
									<RefreshCw className="h-4 w-4 mr-1" />
							...
							{resolveSipCalArr().length > 0 && (
								<Button
									variant="outline"
									onClick={() => {
										const csv = ["Investor/PAN,Scheme,Amount,Maturity Date"]
											.concat(
												resolveSipCalArr().map(
													(s) =>
														`"${s.investorName ?? s.pan ?? ""}","${s.schemeName ?? s.scheme ?? ""}",${s.amount ?? ""},${s.maturityDate ?? s.endDate ?? ""}`,
												),
											)
											.join("\n");
										const blob = new Blob([csv], { type: "text/csv" });
										const a = document.createElement("a");
										a.href = URL.createObjectURL(blob);
										a.download = "sip-maturity-calendar.csv";
										a.click();
									}}
								>
									<Download className="h-4 w-4 mr-1" /> Export CSV
								...
								<table className="w-full text-sm">
									<thead className="sticky top-0 bg-background border-b">
										<tr>
											...
											<th className="text-right p-2">Maturity Date</th>
										</tr>
									</thead>
									<tbody>
										{resolveSipCalArr().map((s, i) => (
											<tr key={i} className="border-b hover:bg-muted/50">
												...
												<td className="p-2 text-right">
													{s.maturityDate ?? s.endDate ?? "—"}
												</td>
											</tr>
										))}
										{!resolveSipCalArr().length && (
											<tr>
												<td
													colSpan={4}
													className="p-4 text-center text-muted-foreground text-sm"
												>
													No SIP maturities in this window
												</td>
											</tr>
										)}
									</tbody>
			...
			{section === "bulk-portfolio" && (
				<Card>
					<CardHeader>
						<CardTitle className="text-sm flex items-center gap-2">
							<PieChart className="h-4 w-4" /> Bulk Portfolio Report
						</CardTitle>
						<CardDescription>
							Portfolio report for all investors — downloadable
						</CardDescription>
					</CardHeader>
					<CardContent className="space-y-4">
						<div className="flex gap-3 flex-wrap items-end">
							...
							<Button onClick={() => refetchBp()} disabled={bpL}>
								{bpL ? (
									<RefreshCw className="h-4 w-4 animate-spin mr-1" />
								) : (
									<Download className="h-4 w-4 mr-1" />
						...
						{bulkPortfolio?.data?.downloadUrl && (
							<a
								href={bulkPortfolio.data.downloadUrl}
								target="_blank"
								rel="noopener noreferrer"
							>
								<Button variant="outline">
						...
						{bulkPortfolio && !bulkPortfolio.data?.downloadUrl && (
							<pre className="text-xs bg-muted/30 rounded p-3 overflow-auto max-h-48">
								{JSON.stringify(bulkPortfolio.data, null, 2)}
							</pre>
						)}
					</CardContent>
				</Card>
...
function ComplianceTab() {
	...
	const { data: pmla, isLoading: pmlaL } = useQuery<
		IrisApiResponse<{ records?: ComplianceRecord[] } | ComplianceRecord[]>
	>({
		queryKey: ["/api/iris/reports/pmla", fromDate, toDate],
		queryFn: () => irisGet(`/api/iris/reports/pmla${qsStr}`),
		retry: false,
	});
	const { data: aml, isLoading: amlL } = useQuery<
		IrisApiResponse<{ records?: AmlRecord[] } | AmlRecord[]>
	>({
		queryKey: ["/api/iris/reports/aml", fromDate, toDate],
		queryFn: () => irisGet(`/api/iris/reports/aml${qsStr}`),
		retry: false,
	});
	const { data: compliance, isLoading: compL } = useQuery<
		IrisApiResponse<{ records?: ComplianceRecord[] } | ComplianceRecord[]>
	>({
	...
	function resolveRecords<T>(
		d: IrisApiResponse<{ records?: T[] } | T[]> | undefined,
	): T[] {
		if (!d?.data) return [];
		if (Array.isArray(d.data)) return d.data as T[];
	...
	return (
		<div className="space-y-6">
			<Card>
				<CardHeader>
					<CardTitle className="text-sm flex items-center gap-2">
						<LucideShield className="h-4 w-4 text-amber-500" /> Compliance & AML
						Reports (Admin Only)
					</CardTitle>
				</CardHeader>
				<CardContent>
			...
			<Card>
				<CardHeader>
					<CardTitle className="text-sm">PMLA Report</CardTitle>
					<CardDescription>
						Prevention of Money Laundering Act — flagged transactions
					</CardDescription>
				</CardHeader>
				<CardContent>
					<ReportTable
						rows={pmlaRecs as unknown as Record<string, unknown>[]}
						cols={[
							{ key: "pan", label: "PAN" },
			...
			<Card>
				<CardHeader>
					<CardTitle className="text-sm">AML Flag Report</CardTitle>
					<CardDescription>
						Anti-money laundering — flagged investor accounts
					</CardDescription>
				</CardHeader>
				<CardContent>
					<ReportTable
						rows={amlRecs as unknown as Record<string, unknown>[]}
						cols={[
							{ key: "pan", label: "PAN" },
							{ key: "investorName", label: "Investor" },
							{ key: "flagReason", label: "Reason" },
							{ key: "flagDate", label: "Flagged On" },
			...
			<Card>
				<CardHeader>
					<CardTitle className="text-sm">General Compliance Summary</CardTitle>
					<CardDescription>Overall compliance records</CardDescription>
				</CardHeader>
				<CardContent>
					<ReportTable
						rows={compRecs as unknown as Record<string, unknown>[]}
						cols={[
							{ key: "pan", label: "PAN" },
							{ key: "investorName", label: "Investor" },
...
function InvestorAlertsPanel({ pan }: { pan: string }) {
	...
	const { data: alerts, isLoading } = useQuery<
		IrisApiResponse<{ alerts?: AlertRecord[] } | AlertRecord[]>
	>({
	...
	const createMut = useMutation({
		mutationFn: (body: Record<string, unknown>) =>
			apiRequest("/api/iris/alerts", "POST", { body }),
		onSuccess: () => {
			toast({ title: "Alert created" });
			setCreateOpen(false);
			qc.invalidateQueries({ queryKey: ["/api/iris/alerts", pan] });
		},
		onError: (e: Error) =>
			toast({
	...
	const updateMut = useMutation({
		mutationFn: ({ id, body }: { id: string; body: Record<string, unknown> }) =>
			apiRequest(`/api/iris/alerts/${id}`, "PUT", { body }),
		onSuccess: () => {
	...
	const AlertForm = ({
		...
		<div className="space-y-3">
			<div className="grid grid-cols-2 gap-2">
				<div>
					...
					<Select
						...
						onValueChange={(v) => setAlertForm((f) => ({ ...f, alertType: v }))}
					>
						<SelectTrigger>
							<SelectValue />
						</SelectTrigger>
				...
				<div>
					...
					<Select
						...
						onValueChange={(v) => setAlertForm((f) => ({ ...f, condition: v }))}
					>
						<SelectTrigger>
							<SelectValue />
						</SelectTrigger>
			...
			<div>
				...
				<Input
					value={alertForm.schemeCode}
					onChange={(e) =>
						setAlertForm((f) => ({ ...f, schemeCode: e.target.value }))
					}
					placeholder="e.g. INF204K01036"
				/>
			</div>
			<div>
	...
	return (
		<Card>
			<CardHeader>
				<div className="flex items-center justify-between">
					<CardTitle className="text-sm flex items-center gap-2">
						<Bell className="h-4 w-4" /> Price / NAV Alerts
					</CardTitle>
			...
			<CardContent className="space-y-3">
						...
						<AlertForm
							onSave={() =>
								createMut.mutate({
									...
									schemeCode: alertForm.schemeCode,
				...
				{isLoading ? (
						...
						{rows.map((a, i) => (
							...
							>
								<div>
									...
									<p className="text-xs text-muted-foreground">
										{a.alertType} · {a.condition} {a.threshold}
									</p>
								</div>
								<div className="flex items-center gap-1">
									...
									<Button
										...
										onClick={() => a.alertId && deleteMut.mutate(a.alertId)}
					...
					<p className="text-sm text-muted-foreground">
						No alerts set for this investor
					</p>
				)}
			</CardContent>
		</Card>
...
function WhatsAppPanel({ pan }: { pan: string }) {
	...
	const { data: templates, isLoading: tplL } = useQuery<
		IrisApiResponse<{ templates?: WaTemplate[] } | WaTemplate[]>
	>({
		queryKey: ["/api/iris/notifications/templates"],
		retry: false,
	});

	const { data: history, isLoading: histL } = useQuery<
		IrisApiResponse<{ messages?: WaMessage[] } | WaMessage[]>
	>({
		queryKey: ["/api/iris/notifications/history", pan],
		queryFn: () =>
			irisGet(`/api/iris/notifications/history?pan=${encodeURIComponent(pan)}`),
		retry: false,
	});

	const sendMut = useMutation({
		mutationFn: (body: Record<string, unknown>) =>
			apiRequest("/api/iris/notifications/whatsapp", "POST", { body }),
		onSuccess: () => {
				...
				<div className="space-y-2">
					...
					{tplL ? (
						...
						>
							...
							<SelectContent>
								{tpls.map((t, i) => (
									<SelectItem key={i} value={t.templateId ?? t.id ?? String(i)}>
										{t.templateName ?? t.name ?? "Template " + (i + 1)}
									</SelectItem>
								))}
								{!tpls.length && (
					...
					<div>
						...
						<Input
							...
							placeholder="Custom message or variable…"
						/>
					</div>
					<Button
						onClick={() =>
							...
							{msgs.map((m, i) => (
								...
								>
									<span className="truncate max-w-[60%]">
										{m.templateName ?? m.template ?? "—"}
									</span>
									<span className="text-muted-foreground">
										{m.sentAt ?? m.date ?? "—"}
									</span>
									<Badge
										variant={m.status === "DELIVERED" ? "default" : "secondary"}
										className="text-[10px]"
									>
										{m.status ?? "—"}
									</Badge>
...
export default function AgentIrisHub() {
	...
	const userRoles: string[] =
		(user as { roles?: string[]; role?: string } | null)?.roles ?? [];
	const singleRole =
		(user as { roles?: string[]; role?: string } | null)?.role ?? "";
	const isAdmin =
		userRoles.includes("admin") ||
		userRoles.includes("superadmin") ||
		singleRole === "admin" ||
		singleRole === "superadmin";
		...
		<div className="p-4 md:p-6 space-y-6">
			...
			<Tabs defaultValue="dashboard">
				<TabsList className="flex-wrap h-auto gap-1">
					...
					<TabsTrigger value="nfo">
						<Layers className="h-4 w-4 mr-1 inline" />
						NFO
					</TabsTrigger>
					<TabsTrigger value="empanelment">
						<LucideShield className="h-4 w-4 mr-1 inline" />
						Empanelment
					</TabsTrigger>
				...
				<TabsContent value="empanelment" className="mt-4">
					<EmpanelmentTab />
				</TabsContent>
				<TabsContent value="investors" className="mt-4">
					<InvestorsTab />
				</TabsContent>
				<TabsContent value="transact" className="mt-4">
					<TransactTab />
				</TabsContent>
				...
				<TabsContent value="cas-import" className="mt-4">
					<CasImportTab />
				</TabsContent>
			</Tabs>
