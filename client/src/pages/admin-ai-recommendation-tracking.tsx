...
export default function AdminAiRecommendationTracking() {
	...
	>({
		queryKey: [
			"/api/ai-recommendations-tracking",
			statusFilter,
			assetTypeFilter,
		],
		queryFn: async () => {
	...
	const checkExpiredMutation = useMutation({
		mutationFn: async () => {
			const res = await apiRequest(
				"POST",
				"/api/ai-recommendations-tracking/check-expired",
			);
			return res.json();
		},
		onSuccess: (data) => {
			...
			queryClient.invalidateQueries({
				queryKey: ["/api/ai-recommendations-tracking"],
			});
			queryClient.invalidateQueries({
				queryKey: ["/api/ai-recommendations-tracking/metrics"],
			});
	...
	return (
		<div
			...
			data-testid="admin-ai-recommendation-tracking-page"
		>
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-muted-foreground">
						Monitor and analyze AI recommendation success rates
					</p>
				</div>
				<div className="flex gap-2">
					...
					>
						<RefreshCw
							className={`h-4 w-4 mr-2 ${checkExpiredMutation.isPending ? "animate-spin" : ""}`}
						/>
						Check Expired
					</Button>
				</div>
						...
						<CardContent className="p-6">
							<div className="flex items-center justify-between">
								<div>
									...
									<p
										className={`text-3xl font-bold ${(metrics?.averageReturn ?? 0) >= 0 ? "text-green-600" : "text-red-600"}`}
									>
					...
					<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
						<Card data-testid="card-success-trend">
							<CardHeader>
								...
								<CardDescription>
									Daily success rate over the last 30 days
								</CardDescription>
							</CardHeader>
							<CardContent>
						...
						<Card data-testid="card-outcome-distribution">
							<CardHeader>
								...
								<CardDescription>
									Breakdown of recommendation outcomes
								</CardDescription>
							</CardHeader>
							<CardContent>
								<ResponsiveContainer width="100%" height={300}>
								...
								<div className="flex gap-2">
									<Select value={statusFilter} onValueChange={setStatusFilter}>
										...
										<SelectContent>
											...
											<SelectItem value="missed_target">
												Missed Target
											</SelectItem>
											...
											recommendations?.map((rec) => (
												...
												>
													...
													<TableCell>
														<Badge
															variant={
																rec.recommendationType.includes("buy")
																	? "default"
																	: "destructive"
															}
														>
													...
													<TableCell
														className={
															Number.parseFloat(rec.actualReturn || "0") >= 0
																? "text-green-600"
																: "text-red-600"
														}
													>
														{rec.actualReturn
															? `${Number.parseFloat(rec.actualReturn) >= 0 ? "+" : ""}${rec.actualReturn}%`
															: "-"}
													</TableCell>
													<TableCell>{rec.confidenceScore}%</TableCell>
													<TableCell>
														{getStatusBadge(rec.status || "pending")}
													</TableCell>
					...
					<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
								...
								<div className="text-center">
									<div className="flex items-center gap-4 text-4xl font-bold">
										...
										<span className="text-red-600">
											{(metrics?.missedTarget || 0) +
												(metrics?.stoppedOut || 0)}
										</span>
									</div>
								...
								<div className="text-center">
									<div className="text-4xl font-bold">
										{metrics && metrics.totalRecommendations > 0
											? Math.round(
													(metrics.resolvedRecommendations /
									...
									<p className="text-sm text-muted-foreground mt-2">
										{metrics?.resolvedRecommendations || 0} of{" "}
										{metrics?.totalRecommendations || 0} resolved
									</p>
								</div>
							</CardContent>
						</Card>
						...
						<Card data-testid="card-profit-factor">
							...
							<CardContent className="flex items-center justify-center">
								<div className="text-center">
									<div
										className={`text-4xl font-bold ${(metrics?.averageReturn || 0) >= 0 ? "text-green-600" : "text-red-600"}`}
									>
										{metrics?.averageReturn || 0}%
									</div>
									<p className="text-sm text-muted-foreground mt-2">
