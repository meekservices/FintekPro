...
export default function AdminAadhaarConfig() {
	...
	const updatePricingMutation = useMutation({
		...
		}: { provider: string; pricePerVerification: number }) => {
	...
	const cheapestConfigured =
		configuredProviders.length > 0
			? configuredProviders.reduce(
					(min, p) =>
						p.pricePerVerification < min.pricePerVerification ? p : min,
		...
		<div className="space-y-6">
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-muted-foreground">
						Manage Aadhaar verification providers for KYC and identity
						verification
					</p>
				</div>
				<Button
					variant="outline"
					onClick={() => refetch()}
					data-testid="button-refresh-aadhaar-providers"
				>
			...
			{cheapestConfigured && cheapestConfigured.provider !== activeProvider && (
				<Alert>
					...
					<AlertDescription>
						Switch to {cheapestConfigured.name} to save on Aadhaar verification
						costs. Current rate: ₹{cheapestConfigured.pricePerVerification}
						/verification
					</AlertDescription>
				</Alert>
			)}
							...
							>
								...
								<CardContent className="space-y-4">
									<div className="flex items-center justify-between p-3 bg-muted rounded-lg">
										...
										<div className="flex items-center gap-2">
											<span className="text-lg font-bold">
												₹{provider.pricePerVerification.toFixed(2)}
											</span>
											<Dialog>
													...
													<Button
														...
														onClick={() =>
															setEditPricing({
																...
																price: provider.pricePerVerification,
															})
														}
												...
												<DialogContent>
													<DialogHeader>
														...
														<DialogDescription>
															Set the cost per Aadhaar verification for{" "}
															{provider.name}
														</DialogDescription>
													</DialogHeader>
									...
									<Button
										...
										onClick={() =>
											setProviderMutation.mutate(provider.provider)
										}
				...
				<TabsContent value="comparison" className="space-y-4">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Compare provider costs based on your expected monthly
								verification volume
							</CardDescription>
						</CardHeader>
						<CardContent className="space-y-6">
							<div className="space-y-2">
								...
								<Input
									...
									value={monthlyVolume}
									onChange={(e) =>
										setMonthlyVolume(Number.parseInt(e.target.value) || 1)
									}
							...
							<div className="grid gap-4 md:grid-cols-2">
								{providers.map((provider) => {
									const monthlyCost =
										provider.pricePerVerification * monthlyVolume;
									const isCheapest =
										cheapestConfigured?.provider === provider.provider;
							...
							{configuredProviders.length >= 2 && (
								<Alert>
									...
									<AlertDescription>
										Based on your volume of {monthlyVolume.toLocaleString()}{" "}
										verifications/month,
										{cheapestConfigured && (
											<span className="font-medium">
												{" "}
												{cheapestConfigured.name}
											</span>
										)}{" "}
										is the most cost-effective option, saving you ₹
										{((providers.find(
											(p) => p.provider !== cheapestConfigured?.provider,
										)?.pricePerVerification || 0) -
											(cheapestConfigured?.pricePerVerification || 0)) *
											monthlyVolume}
										/month.
									</AlertDescription>
								</Alert>
						...
						<>
								...
								<Card data-testid="card-verifications-this-month">
									<CardHeader className="pb-2">
										...
										<CardTitle className="text-3xl flex items-center gap-2">
											...
											{usageStats?.thisMonth?.verifications?.toLocaleString() ||
												0}
										</CardTitle>
									</CardHeader>
								</Card>
								...
								<Card data-testid="card-total-cost">
									<CardHeader className="pb-2">
										...
										<CardTitle className="text-3xl flex items-center gap-2">
											...
											{usageStats?.thisMonth?.cost?.toLocaleString("en-IN", {
												maximumFractionDigits: 2,
											}) || 0}
										</CardTitle>
									</CardHeader>
							...
							<Card>
								<CardHeader>
									...
									<CardDescription>
										Breakdown of verifications by provider
									</CardDescription>
								</CardHeader>
								<CardContent>
									<div className="space-y-4">
										{providers.map((provider) => {
											const providerStats =
												usageStats?.byProvider?.[provider.provider];
											return (
												<div
													key={provider.provider}
												...
												>
													<div className="flex items-center gap-3">
														<div
															className={`w-3 h-3 rounded-full ${provider.isActive ? "bg-green-500" : "bg-muted-foreground"}`}
														/>
														<div>
															<p className="font-medium">{provider.name}</p>
															<p className="text-sm text-muted-foreground">
																{providerStats?.verifications?.toLocaleString() ||
																	0}{" "}
																verifications
															</p>
														</div>
													...
													<div className="text-right">
														<p className="font-medium">
															₹
															{providerStats?.cost?.toLocaleString("en-IN", {
																maximumFractionDigits: 2,
															}) || 0}
														</p>
														<p className="text-sm text-muted-foreground">
															{(
																(providerStats?.successRate || 0) * 100
															).toFixed(1)}
															% success rate
														</p>
													</div>
				...
				<TabsContent value="setup" className="space-y-4">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Required environment variables for each Aadhaar verification
								provider
							</CardDescription>
						</CardHeader>
						<CardContent className="space-y-6">
							{providers.map((provider) => (
								<div
									key={provider.provider}
									className="space-y-3"
									data-testid={`setup-provider-${provider.provider}`}
								>
									<div className="flex items-center justify-between">
					...
					<Card>
						...
						<CardContent className="space-y-4">
							...
							<div className="space-y-2">
								...
								<p className="text-sm text-muted-foreground">
									The active provider is used for all Aadhaar verifications. You
									can switch providers at any time without affecting existing
									verification data. Both Cashfree and Truthscreen are
									UIDAI-authorized ASAs (Authentication Service Agencies).
								</p>
							</div>
						</CardContent>
					</Card>
