...
export default function InvestmentDashboard() {
	...
	const { data: investmentIdeas = [], isLoading: loadingIdeas } = useQuery<
		InvestmentIdea[]
	>({
		queryKey: ["/api/investment-ideas"],
	});

	const { data: yieldTrackers = [], isLoading: loadingTrackers } = useQuery<
		YieldTracker[]
	>({
	...
	const { data: popularRecommendations } = useQuery<{
		recommendations: InvestmentIdea[];
	}>({
		queryKey: ["/api/investment-ideas/recommendations/popular"],
	});

	// Portfolio Analytics
	const { data: portfolioAnalytics, isLoading: loadingAnalytics } =
		useQuery<any>({
	...
	const portfolioOverview = {
		...
		activeIdeas: investmentIdeas.filter(
			(idea: InvestmentIdea) => idea.status === "active",
		).length,
		completedIdeas: investmentIdeas.filter(
			(idea: InvestmentIdea) => idea.status === "completed",
		).length,
			...
			<div className="container mx-auto p-6">
					...
					<div className="flex items-center justify-between">
						<div>
							<h1
								...
								data-testid="title-investment-dashboard"
							>
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
							...
							<div className="flex items-center justify-between">
								<div>
									...
									<p className="text-sm text-muted-foreground">
										Total Return:{" "}
										{formatPercentage(portfolioOverview.totalReturn)}
									</p>
								</div>
					...
					<Card
						...
						data-testid="card-tracked-investments"
					>
						<CardContent className="p-6">
							<div className="flex items-center justify-between">
								<div>
				...
				>
					<ScrollableTabsList
						...
						data-testid="tabs-investment-dashboard"
					>
								...
								<CardContent>
									<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
										{optimizationSuggestions.map(
											(suggestion: OptimizationSuggestion, index: number) => (
												<Alert
													key={index}
													className={
					...
					<TabsContent value="ideas" className="space-y-6">
						<div className="flex justify-between items-center">
							<h2
								...
								data-testid="title-investment-ideas"
							>
								Investment Ideas
							</h2>
							...
							<Dialog>
								...
								<DialogContent>
									...
									<Form {...newIdeaForm}>
										<form
											onSubmit={newIdeaForm.handleSubmit(handleGenerateIdeas)}
											className="space-y-4"
										>
											<FormField
												...
												render={({ field }) => (
													<FormItem>
														...
														<FormControl>
															<Input
																placeholder="RELIANCE, TCS, HDFC (comma separated)"
																{...field}
							...
							{loadingIdeas ? (
								<div className="col-span-full text-center py-8">
									...
									<p className="mt-2 text-muted-foreground">
										Loading investment ideas...
									</p>
								</div>
							) : investmentIdeas.length === 0 ? (
								<div
									className="col-span-full text-center py-8"
									data-testid="empty-investment-ideas"
								>
					...
					<TabsContent value="yield" className="space-y-6">
						<div className="flex justify-between items-center">
							<h2
								className="text-2xl font-bold"
								data-testid="title-yield-tracking"
							>
								Yield Tracking
							</h2>
							...
							<Card data-testid="card-diversification">
								...
								<CardContent>
									<div className="text-3xl font-bold text-blue-600">
										{((portfolioYield?.diversificationRatio || 0) * 10).toFixed(
											1,
										)}
										/10
									</div>
							...
							<CardContent>
								{loadingTrackers ? (
									<div className="text-center py-8">
										<div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600 mx-auto" />
										<p className="mt-2 text-muted-foreground">
											Loading yield trackers...
										</p>
									</div>
								) : yieldTrackers.length === 0 ? (
									<div
										className="text-center py-8"
										data-testid="empty-yield-trackers"
									>
										<BarChart3 className="h-12 w-12 text-muted-foreground mx-auto mb-4" />
										<p className="text-muted-foreground">
											No yield trackers yet. Add one to start monitoring
											performance!
										</p>
									</div>
								) : (
											...
											>
												<div className="flex-1">
													...
													<div className="flex items-center gap-6 mt-2 text-sm text-muted-foreground">
														<span>
															Investment:{" "}
															{formatCurrency(tracker.initialInvestment)}
														</span>
														<span>
															Current: {formatCurrency(tracker.currentValue)}
														</span>
														<span>
															Target: {formatPercentage(tracker.targetYield)}
														</span>
													</div>
												</div>
												<div className="text-right">
					...
					<TabsContent value="analytics" className="space-y-6">
						<h2 className="text-2xl font-bold" data-testid="title-analytics">
							Portfolio Analytics
						</h2>
						...
						{loadingAnalytics ? (
							...
							<>
								...
								<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
									...
									<Card data-testid="card-current-value">
										<CardHeader className="pb-2">
											...
											<CardTitle className="text-2xl">
												{formatCurrency(portfolioAnalytics.currentValue)}
											</CardTitle>
										</CardHeader>
									</Card>
									<Card data-testid="card-absolute-returns">
										<CardHeader className="pb-2">
											...
											<CardTitle
												className={`text-2xl ${portfolioAnalytics.absoluteReturns >= 0 ? "text-green-600" : "text-red-600"}`}
											>
												{portfolioAnalytics.absoluteReturns >= 0 ? "+" : ""}
												{formatCurrency(portfolioAnalytics.absoluteReturns)}
												<span className="text-sm ml-2">
													({portfolioAnalytics.absoluteReturnsPercentage}%)
												</span>
											</CardTitle>
										</CardHeader>
									</Card>
									<Card data-testid="card-xirr">
										<CardHeader className="pb-2">
											...
											<CardTitle
												className={`text-2xl ${portfolioAnalytics.xirr?.success ? (portfolioAnalytics.xirr.xirrPercentage >= 0 ? "text-green-600" : "text-red-600") : ""}`}
											>
												{portfolioAnalytics.xirr?.success
													? `${portfolioAnalytics.xirr.xirrPercentage}%`
													: "N/A"}
											</CardTitle>
										</CardHeader>
									</Card>
								...
								<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
									...
									<Card data-testid="card-asset-allocation">
										...
										<CardContent>
											<div className="space-y-3">
												{Object.entries(
													portfolioAnalytics.assetAllocation || {},
												)
											...
											<div className="space-y-4">
												<div className="text-center py-4">
													<div className="text-4xl font-bold text-blue-600">
														{portfolioAnalytics.riskProfile?.score || 0}
													</div>
													<Badge className="mt-2">
														{portfolioAnalytics.riskProfile?.classification ||
															"N/A"}
													</Badge>
												</div>
												<div className="space-y-2">
													<div className="flex justify-between text-sm">
														...
														<span className="font-medium">
															{portfolioAnalytics.riskProfile?.equityExposure?.toFixed(
																2,
															) || 0}
															%
														</span>
													...
													<div className="flex justify-between text-sm">
														...
														<span className="font-medium">
															{portfolioAnalytics.riskProfile?.debtExposure?.toFixed(
																2,
															) || 0}
															%
														</span>
												...
												<Separator />
												<p className="text-sm text-muted-foreground">
													{portfolioAnalytics.riskProfile?.recommendation}
												</p>
											</div>
										</CardContent>
									</Card>
								...
								<Card data-testid="card-category-performance">
									...
									<CardContent>
										<div className="space-y-4">
											{portfolioAnalytics.categoryPerformance?.map(
												(cat: any) => (
													<div
														key={cat.category}
													...
													>
														<div className="flex justify-between items-start mb-2">
															...
															<div className="text-right">
																...
																<p
																	className={`text-sm ${cat.returnsPercentage >= 0 ? "text-green-600" : "text-red-600"}`}
																>
																	{cat.returnsPercentage >= 0 ? "+" : ""}
																	{cat.returnsPercentage}%
																</p>
															</div>
														</div>
													</div>
					...
					<TabsContent value="alerts" className="space-y-6">
						<h2 className="text-2xl font-bold" data-testid="title-alerts">
							Investment Alerts
						</h2>
				...
				{selectedIdea && (
					...
					>
						<DialogContent
							className="max-w-2xl"
							data-testid="dialog-idea-detail"
						>
							<DialogHeader>
								...
								<div className="grid grid-cols-3 gap-4">
									<div className="space-y-2">
										...
										<p className="text-lg font-medium">
											{formatCurrency(selectedIdea.entryPrice)}
										</p>
									</div>
									<div className="space-y-2">
										<Label>Target Price</Label>
										<p className="text-lg font-medium text-green-600">
											{formatCurrency(selectedIdea.targetPrice)}
										</p>
									</div>
									<div className="space-y-2">
										<Label>Stop Loss</Label>
										<p className="text-lg font-medium text-red-600">
											{formatCurrency(selectedIdea.stopLoss)}
										</p>
									</div>
								</div>
										...
										<div className="grid grid-cols-2 gap-4 text-sm">
											<div>
												...
												<span className="ml-2 font-medium">
													{selectedIdea.technicalAnalysis.rsi}
												</span>
											</div>
											<div>
												<span className="text-muted-foreground">MACD:</span>
												<span className="ml-2 font-medium">
													{selectedIdea.technicalAnalysis.macd}
												</span>
											</div>
											<div>
												<span className="text-muted-foreground">SMA 20:</span>
												<span className="ml-2 font-medium">
													{selectedIdea.technicalAnalysis.sma20}
												</span>
											</div>
											<div>
												<span className="text-muted-foreground">SMA 50:</span>
												<span className="ml-2 font-medium">
													{selectedIdea.technicalAnalysis.sma50}
												</span>
											</div>
										</div>
									</div>
