        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <Card className="border-l-4 border-l-green-500" data-testid="card-active-ideas">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-gray-600 dark:text-gray-300">Active Ideas</p>
                  <p className="text-2xl font-bold text-gray-900 dark:text-white">
          ...
          <Card className="border-l-4 border-l-purple-500" data-testid="card-tracked-investments">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-gray-600 dark:text-gray-300">Tracked Investments</p>
                  <p className="text-2xl font-bold text-gray-900 dark:text-white">
          ...
          <Card className="border-l-4 border-l-orange-500" data-testid="card-alerts">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-gray-600 dark:text-gray-300">Alert Status</p>
                  <p className="text-2xl font-bold text-gray-900 dark:text-white">
                      ...
                      {popularRecommendations?.recommendations?.slice(0, 5).map((rec: any, index: number) => (
                        <div key={index} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                          <div>
                            ...
                            <p className="text-sm text-gray-600 dark:text-gray-300">{rec.instrumentType}</p>
                          </div>
                      ...
                      <Alert key={index} className={suggestion.priority === 'high' ? 'border-red-200' : suggestion.priority === 'medium' ? 'border-orange-200' : 'border-blue-200'}>
                        ...
                        <AlertDescription>
                          <div className="space-y-2">
                            ...
                            <p className="text-xs text-gray-600 dark:text-gray-300">{suggestion.expectedImpact}</p>
                          </div>
                        </AlertDescription>
                      </Alert>
                    ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {loadingIdeas ? (
                <div className="col-span-full text-center py-8">
                  ...
                  <p className="mt-2 text-gray-600 dark:text-gray-300">Loading investment ideas...</p>
                </div>
              ) : investmentIdeas.length === 0 ? (
                <div className="col-span-full text-center py-8" data-testid="empty-investment-ideas">
                  <Lightbulb className="h-12 w-12 text-gray-400 mx-auto mb-4" />
                  <p className="text-gray-600 dark:text-gray-300">No investment ideas yet. Generate some to get started!</p>
                </div>
              ) : (
              ...
              <CardContent>
                {loadingTrackers ? (
                  <div className="text-center py-8">
                    <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600 mx-auto"></div>
                    <p className="mt-2 text-gray-600 dark:text-gray-300">Loading yield trackers...</p>
                  </div>
                ) : yieldTrackers.length === 0 ? (
                  <div className="text-center py-8" data-testid="empty-yield-trackers">
                    <BarChart3 className="h-12 w-12 text-gray-400 mx-auto mb-4" />
                    <p className="text-gray-600 dark:text-gray-300">No yield trackers yet. Add one to start monitoring performance!</p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {yieldTrackers.map((tracker: YieldTracker) => (
                      <div key={tracker.id} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-800 rounded-lg" data-testid={`tracker-${tracker.id}`}>
              ...
              <CardContent>
                {loadingAlerts ? (
                  ...
                  <div className="text-center py-8" data-testid="empty-alerts">
                    ...
                    <p className="text-gray-600 dark:text-gray-300">All caught up! No new alerts.</p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {unreadAlerts.map((alert: any) => (
                      <Alert key={alert.id} className="cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800" onClick={() => markAlertReadMutation.mutate(alert.id)} data-testid={`alert-${alert.id}`}>
                        <AlertCircle className="h-4 w-4" />
                        <AlertDescription>
                          <div className="flex items-center justify-between">
                            <div>
                ...
                {selectedIdea.technicalAnalysis && (
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        <span className="text-gray-600 dark:text-gray-300">RSI:</span>
                        <span className="ml-2 font-medium">{selectedIdea.technicalAnalysis.rsi}</span>
                      </div>
                      <div>
                        <span className="text-gray-600 dark:text-gray-300">MACD:</span>
                        <span className="ml-2 font-medium">{selectedIdea.technicalAnalysis.macd}</span>
                      </div>
                      <div>
                        <span className="text-gray-600 dark:text-gray-300">SMA 20:</span>
                        <span className="ml-2 font-medium">{selectedIdea.technicalAnalysis.sma20}</span>
                      </div>
                      <div>
                        <span className="text-gray-600 dark:text-gray-300">SMA 50:</span>
