...
const GainLossIndicator = ({
	value,
	percent,
}: { value?: string | number; percent?: string | number }) => {
	const numValue = typeof value === "string" ? Number.parseFloat(value) : value;
	const numPercent =
		typeof percent === "string" ? Number.parseFloat(percent) : percent;
	...
	return (
		...
		>
			...
			{numPercent !== undefined && !Number.isNaN(numPercent) && (
...
export default function AlternativeInvestmentsPage() {
	...
	const addAifMutation = useMutation({
		...
		onSuccess: () => {
			toast({
				title: "AIF Investment Added",
				description: "Your AIF investment has been submitted for approval.",
			});
		...
		onError: (error: any) => {
			toast({
				title: "Error",
				description: error.message || "Failed to add AIF investment",
				variant: "destructive",
			});
		},
	});
	...
	const addPmsMutation = useMutation({
		...
		onSuccess: () => {
			toast({
				title: "PMS Investment Added",
				description: "Your PMS investment has been submitted for approval.",
			});
		...
		onError: (error: any) => {
			toast({
				title: "Error",
				description: error.message || "Failed to add PMS investment",
				variant: "destructive",
			});
		},
	});
	...
	const handleAddAif = () => {
		if (
			!aifForm.aifName ||
			!aifForm.commitmentAmount ||
			!aifForm.capitalCalled ||
			!aifForm.investedDate
		) {
			toast({
	...
	const aifSummary: PortfolioSummary = aifData?.summary || {
		totalCurrentValue: 0,
		totalInvested: 0,
		totalCommitment: 0,
		totalGainLoss: 0,
	...
	const pmsSummary: PortfolioSummary = pmsData?.summary || {
		totalCurrentValue: 0,
		totalInvested: 0,
		totalGainLoss: 0,
		totalGainLossPercent: 0,
			...
			<div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
				<div>
					...
					<p className="text-muted-foreground">
						Track your AIF and PMS investments in one place
					</p>
				</div>
				<div className="flex gap-2">
					<Button
						onClick={() => setShowAddAifDialog(true)}
						data-testid="button-add-aif"
					>
						<Plus className="w-4 h-4 mr-2" /> Add AIF
					</Button>
					<Button
						onClick={() => setShowAddPmsDialog(true)}
						variant="outline"
						data-testid="button-add-pms"
					>
			...
			<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
				<Card>
					<CardContent className="pt-4">
						...
						<p className="text-2xl font-bold">
							{aifSummary.holdings + pmsSummary.holdings}
						</p>
					</CardContent>
				</Card>
				<Card>
					<CardContent className="pt-4">
						...
						<p className="text-2xl font-bold">
							{formatCurrency(
								aifSummary.totalInvested + pmsSummary.totalInvested,
							)}
						</p>
					</CardContent>
				</Card>
				<Card>
					<CardContent className="pt-4">
						...
						<p className="text-2xl font-bold">
							{formatCurrency(
								aifSummary.totalCurrentValue + pmsSummary.totalCurrentValue,
							)}
						</p>
					</CardContent>
				</Card>
				<Card>
					<CardContent className="pt-4">
						<div className="flex items-center gap-2 text-muted-foreground mb-1">
							{aifSummary.totalGainLoss + pmsSummary.totalGainLoss >= 0 ? (
						...
						<GainLossIndicator
							...
							percent={
								aifSummary.totalInvested + pmsSummary.totalInvested > 0
									? ((aifSummary.totalGainLoss + pmsSummary.totalGainLoss) /
											(aifSummary.totalInvested + pmsSummary.totalInvested)) *
										100
									: 0
							}
						...
						<Card>
							<CardContent className="py-12 text-center">
								...
								<h3 className="text-lg font-semibold mb-2">
									No AIF Investments Yet
								</h3>
								<p className="text-muted-foreground mb-4">
									Add your existing AIF investments to track them here
								</p>
								<Button
									onClick={() => setShowAddAifDialog(true)}
									data-testid="button-add-first-aif"
								>
							...
							<div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
								<Card className="bg-blue-50 dark:bg-blue-950">
									<CardContent className="pt-4">
										...
										<p className="text-xl font-bold">
											{formatCurrency(aifSummary.totalCommitment)}
										</p>
									</CardContent>
								</Card>
								<Card className="bg-green-50 dark:bg-green-950">
									<CardContent className="pt-4">
										...
										<p className="text-xl font-bold">
											{formatCurrency(aifSummary.totalInvested)}
										</p>
									</CardContent>
								</Card>
								<Card className="bg-purple-50 dark:bg-purple-950">
									<CardContent className="pt-4">
										...
										<p className="text-xl font-bold">
											{formatCurrency(
												(aifSummary.totalCommitment || 0) -
													aifSummary.totalInvested,
											)}
										</p>
									</CardContent>
								</Card>
									...
									<Table>
										...
										<TableBody>
											{aifHoldings.map((holding) => (
												<TableRow
													key={holding.id}
													data-testid={`row-aif-${holding.id}`}
												>
													<TableCell>
														<div>
															<p className="font-medium">{holding.aifName}</p>
															{holding.registrationNo && (
													...
													<TableCell className="text-right">
														{formatCurrency(holding.commitmentAmount)}
													</TableCell>
													<TableCell className="text-right">
														{formatCurrency(holding.capitalCalled)}
													</TableCell>
													<TableCell className="text-right">
													...
													<TableCell className="text-right">
														<GainLossIndicator
															value={holding.unrealizedGainLoss}
															percent={holding.unrealizedGainLossPercent}
														/>
													</TableCell>
													<TableCell>
														<StatusBadge status={holding.entryStatus} />
													</TableCell>
													<TableCell className="text-right">
														<Button
															variant="ghost"
															size="icon"
															onClick={() =>
																deleteAifMutation.mutate(holding.id)
															}
					...
					{pmsLoading ? (
						...
						<Card>
							<CardContent className="py-12 text-center">
								...
								<h3 className="text-lg font-semibold mb-2">
									No PMS Investments Yet
								</h3>
								<p className="text-muted-foreground mb-4">
									Add your existing PMS investments to track them here
								</p>
								<Button
									onClick={() => setShowAddPmsDialog(true)}
									data-testid="button-add-first-pms"
								>
							...
							<div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
								<Card className="bg-indigo-50 dark:bg-indigo-950">
									<CardContent className="pt-4">
										...
										<p className="text-xl font-bold">
											{formatCurrency(pmsSummary.totalInvested)}
										</p>
									</CardContent>
								</Card>
								<Card className="bg-emerald-50 dark:bg-emerald-950">
									<CardContent className="pt-4">
										...
										<p className="text-xl font-bold">
											{formatCurrency(pmsSummary.totalCurrentValue)}
										</p>
									</CardContent>
								</Card>
								<Card
									className={
										pmsSummary.totalGainLoss >= 0
								...
								>
									<CardContent className="pt-4">
										...
										<GainLossIndicator
											value={pmsSummary.totalGainLoss}
											percent={pmsSummary.totalGainLossPercent}
										/>
									</CardContent>
								</Card>
							</div>
									...
									<Table>
										...
										<TableBody>
											{pmsHoldings.map((holding) => (
												<TableRow
													key={holding.id}
													data-testid={`row-pms-${holding.id}`}
												>
													<TableCell>
														<div>
													...
													<TableCell className="text-right">
														{formatCurrency(
															holding.totalInvested || holding.investedAmount,
														)}
													</TableCell>
													<TableCell className="text-right">
														{formatCurrency(
															holding.corpusValue || holding.currentValue,
														)}
													</TableCell>
													<TableCell className="text-right">
														<GainLossIndicator
													...
													<TableCell className="text-right">
														{holding.cagr ? (
															<span
																className={
																	Number.parseFloat(holding.cagr) >= 0
																		? "text-green-600"
																		: "text-red-600"
																}
															>
																{Number.parseFloat(holding.cagr).toFixed(2)}%
															</span>
														) : (
															"—"
														)}
													...
													<TableCell className="text-right">
														<Button
															...
															size="icon"
															onClick={() =>
																deletePmsMutation.mutate(holding.id)
															}
				...
				<TabsContent value="mld" className="mt-4">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Explore and invest in structured products with market-linked
								returns
							</CardDescription>
						</CardHeader>
						<CardContent className="space-y-4">
							<p className="text-muted-foreground">
								...
								structured payoff profiles. Browse available MLDs, analyze
								payoff scenarios, and add your existing holdings to track them.
							</p>
			...
			<Dialog open={showAddAifDialog} onOpenChange={setShowAddAifDialog}>
				<DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
					<DialogHeader>
						...
						<DialogDescription>
							Enter details of your Alternative Investment Fund holding
						</DialogDescription>
					</DialogHeader>
					...
					<div className="space-y-4">
						<Alert>
							...
							<AlertDescription>
								Your investment will be submitted for verification. Once
								approved, it will be included in your portfolio analysis.
							</AlertDescription>
						</Alert>
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
							<div className="md:col-span-2">
								...
								<Input
									...
									onChange={(e) =>
										setAifForm({ ...aifForm, aifName: e.target.value })
									}
							...
							<div>
								...
								<Input
									...
									onChange={(e) =>
										setAifForm({ ...aifForm, registrationNo: e.target.value })
									}
							...
							<div>
								...
								<Select
									value={aifForm.category}
									onValueChange={(v) => setAifForm({ ...aifForm, category: v })}
								>
							...
							<div>
								...
								<Input
									...
									onChange={(e) =>
										setAifForm({ ...aifForm, subcategory: e.target.value })
									}
							...
							<div>
								...
								<Input
									...
									onChange={(e) =>
										setAifForm({ ...aifForm, commitmentAmount: e.target.value })
									}
									data-testid="input-aif-commitment"
								/>
								<p className="text-xs text-muted-foreground mt-1">
									Total amount you committed to invest
								</p>
							</div>
							...
							<div>
								...
								<Input
									...
									onChange={(e) =>
										setAifForm({ ...aifForm, capitalCalled: e.target.value })
									}
									data-testid="input-aif-called"
								/>
								<p className="text-xs text-muted-foreground mt-1">
									Amount called by the fund till date
								</p>
							</div>
							...
							<div>
								...
								<Input
									...
									onChange={(e) =>
										setAifForm({ ...aifForm, investedDate: e.target.value })
									}
							...
							<div>
								...
								<Input
									...
									onChange={(e) =>
										setAifForm({ ...aifForm, lockinEndDate: e.target.value })
									}
							...
							<div>
								...
								<Input
									...
									onChange={(e) =>
										setAifForm({ ...aifForm, currentUnits: e.target.value })
									}
							...
							<div>
								...
								<Input
									...
									onChange={(e) =>
										setAifForm({ ...aifForm, entryNav: e.target.value })
									}
							...
							<div>
								...
								<Input
									...
									onChange={(e) =>
										setAifForm({ ...aifForm, latestNav: e.target.value })
									}
							...
							<div>
								...
								<Input
									...
									onChange={(e) =>
										setAifForm({ ...aifForm, currentValue: e.target.value })
									}
									data-testid="input-aif-current-value"
								/>
								<p className="text-xs text-muted-foreground mt-1">
									Leave blank if NAV × Units should be used
								</p>
							</div>
							...
							<div className="md:col-span-2">
								...
								<Textarea
									...
									value={aifForm.notes}
									onChange={(e) =>
										setAifForm({ ...aifForm, notes: e.target.value })
									}
					...
					<DialogFooter>
						<Button
							variant="outline"
							onClick={() => setShowAddAifDialog(false)}
						>
							Cancel
						</Button>
						<Button
							onClick={handleAddAif}
							disabled={addAifMutation.isPending}
							data-testid="button-submit-aif"
						>
							{addAifMutation.isPending
								? "Submitting..."
								: "Submit for Approval"}
						</Button>
					</DialogFooter>
			...
			<Dialog open={showAddPmsDialog} onOpenChange={setShowAddPmsDialog}>
				<DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
					<DialogHeader>
						...
						<DialogDescription>
							Enter details of your Portfolio Management Service holding
						</DialogDescription>
					</DialogHeader>
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
							<div className="md:col-span-2">
								...
								<Input
									...
									onChange={(e) =>
										setPmsForm({ ...pmsForm, pmsName: e.target.value })
									}
							...
							<div>
								...
								<Input
									...
									onChange={(e) =>
										setPmsForm({ ...pmsForm, registrationNo: e.target.value })
									}
							...
							<div>
								...
								<Select
									value={pmsForm.strategy}
									onValueChange={(v) => setPmsForm({ ...pmsForm, strategy: v })}
								>
							...
							<div>
								...
								<Input
									...
									onChange={(e) =>
										setPmsForm({ ...pmsForm, investedAmount: e.target.value })
									}
							...
							<div>
								...
								<Input
									...
									onChange={(e) =>
										setPmsForm({
											...pmsForm,
											additionalInfusions: e.target.value,
										})
									}
									data-testid="input-pms-additional"
								/>
								<p className="text-xs text-muted-foreground mt-1">
									Total additional investments after initial
								</p>
							</div>
							...
							<div>
								...
								<Input
									...
									onChange={(e) =>
										setPmsForm({ ...pmsForm, startDate: e.target.value })
									}
							...
							<div>
								...
								<Input
									...
									onChange={(e) =>
										setPmsForm({ ...pmsForm, corpusValue: e.target.value })
									}
									data-testid="input-pms-corpus"
								/>
								<p className="text-xs text-muted-foreground mt-1">
									Current portfolio value as per PMS statement
								</p>
							</div>
							...
							<div>
								...
								<Input
									...
									onChange={(e) =>
										setPmsForm({ ...pmsForm, latestNav: e.target.value })
									}
							...
							<div className="md:col-span-2">
								...
								<Textarea
									...
									value={pmsForm.notes}
									onChange={(e) =>
										setPmsForm({ ...pmsForm, notes: e.target.value })
									}
					...
					<DialogFooter>
						<Button
							variant="outline"
							onClick={() => setShowAddPmsDialog(false)}
						>
							Cancel
						</Button>
						<Button
							onClick={handleAddPms}
							disabled={addPmsMutation.isPending}
							data-testid="button-submit-pms"
						>
							{addPmsMutation.isPending
								? "Submitting..."
								: "Submit for Approval"}
						</Button>
					</DialogFooter>
