...
export default function TrackerPortfolioReport() {
	...
	const totalGainLoss =
		holdings?.reduce(
			(sum: number, h: any) => sum + Number.parseFloat(h.gainLoss || "0"),
			0,
		) || 0;
		...
		<div className="container mx-auto px-4 py-6 max-w-7xl space-y-6">
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-muted-foreground">
						PAN-level consolidated holdings from all sources
					</p>
				</div>
			...
			{showSync && (
				<Card>
					<CardHeader>
						...
						<CardDescription>
							Import holdings from CDSL, NSDL, or upload statements
						</CardDescription>
					</CardHeader>
					<CardContent>
			...
			<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
				...
				<Card>
					<CardContent className="p-6">
						...
						<div
							className={`text-2xl font-bold flex items-center gap-2 ${totalGainLoss >= 0 ? "text-green-600" : "text-red-600"}`}
						>
							{totalGainLoss >= 0 ? (
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Complete list of tracked investments
					</CardDescription>
				</CardHeader>
				<CardContent>
					{holdings && holdings.length > 0 ? (
							...
							<TableBody>
								{holdings.map((holding: any) => (
									<TableRow key={holding.id}>
										...
										<TableCell
											className={`text-right ${Number.parseFloat(holding.gainLoss || "0") >= 0 ? "text-green-600" : "text-red-600"}`}
										>
											{formatCurrency(
						...
						<div className="text-center py-8 text-muted-foreground">
							...
							<p className="text-sm">
								Sync your external holdings to see consolidated view
							</p>
						</div>
					)}
				</CardContent>
