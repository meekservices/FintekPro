...
export default function CASupportDetail() {
	...
	const updateStepMutation = useMutation({
		mutationFn: async ({
			...
			status,
			notes,
		}: { stepId: number; status: string; notes?: string }) => {
	...
	const addCommentMutation = useMutation({
		mutationFn: async ({
			...
			comment,
			isInternal,
		}: { stepId: number; comment: string; isInternal: boolean }) => {
			...
			<div className="container mx-auto p-6">
				<Card>
					<CardContent className="flex flex-col items-center justify-center py-12">
						...
						<Button
							onClick={() => setLocation("/partner/ca-support")}
					...
					<DropdownMenu>
						...
						<DropdownMenuContent align="end">
							...
							{ticket.status === "open" && (
								<DropdownMenuItem
									onClick={() =>
										updateTicketMutation.mutate({ status: "in_progress" })
									}
								>
							...
							{ticket.status === "in_progress" && (
								<DropdownMenuItem
									onClick={() =>
										updateTicketMutation.mutate({ status: "on_hold" })
									}
								>
									...
									<Dialog open={isAddStepOpen} onOpenChange={setIsAddStepOpen}>
										...
										<DialogContent>
											<DialogHeader>
												...
												<DialogDescription>
													Add a custom step to this workflow
												</DialogDescription>
											</DialogHeader>
									...
									<TabsContent value="workflow" className="mt-0">
										{stepsLoading ? (
											...
											<div className="text-center py-8 text-muted-foreground">
												...
												<p className="text-sm">
													Add steps to create a structured workflow.
												</p>
											</div>
										) : (
											...
											{comments.length === 0 ? (
												...
												<div className="space-y-3">
													{comments.map((comment) => (
														<div
															key={comment.id}
															className={`p-3 rounded-lg ${comment.isInternal ? "bg-yellow-50 dark:bg-yellow-900/20" : "bg-muted"}`}
														>
