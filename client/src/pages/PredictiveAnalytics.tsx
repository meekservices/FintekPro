...
export default function PredictiveAnalytics() {
	...
	const generateRiskAnalysis = useMutation({
		mutationFn: async (portfolioId: string) =>
			apiRequest("/api/analytics/generate-risk-analysis", "POST", {
				portfolioId,
			}),
		onSuccess: () => {
	...
	const latestRisk =
		riskAnalysis && riskAnalysis.length > 0 ? riskAnalysis[0] : null;
		...
		<div className="container mx-auto p-6 space-y-6">
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Choose a portfolio to analyze and generate predictions
					</CardDescription>
				</CardHeader>
								...
								<div className="grid grid-cols-1 md:grid-cols-4 gap-4">
									<Card>
										...
										<CardContent>
											<div className="text-2xl font-bold flex items-center gap-2">
												{Number.parseFloat(latestPrediction.expectedReturn) >=
												0 ? (
												...
												{Number.parseFloat(
													latestPrediction.expectedReturn,
												).toFixed(2)}
												%
											</div>
									...
									<Card>
										...
										<CardContent>
											<div className="text-2xl font-bold">
												₹
												{Number.parseFloat(
													latestPrediction.expectedValue,
												).toLocaleString("en-IN")}
											</div>
											<p className="text-xs text-muted-foreground mt-1">
												Range: ₹
												{Number.parseFloat(
													latestPrediction.lowerBound,
												).toLocaleString("en-IN")}{" "}
												- ₹
												{Number.parseFloat(
									...
									<Card>
										...
										<CardContent>
											<div className="text-2xl font-bold">
												{(
													Number.parseFloat(latestPrediction.volatility) * 100
												).toFixed(2)}
												%
											</div>
											<p className="text-xs text-muted-foreground mt-1">
									...
									<Card>
										...
										<CardContent>
											...
											<p className="text-xs text-muted-foreground mt-1">
												Strength:{" "}
												{Number.parseFloat(
													latestPrediction.trendStrength,
												).toFixed(0)}
												/100
											</p>
										</CardContent>
										...
										<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
											<div>
												...
												<p className="text-lg font-semibold">
													{Number.parseFloat(latestPrediction.cagr).toFixed(2)}%
												</p>
											</div>
											<div>
												<p className="text-sm text-muted-foreground">RSI</p>
												<p className="text-lg font-semibold">
													{Number.parseFloat(latestPrediction.rsi).toFixed(0)}
													<span className="text-xs ml-1">
														{Number.parseFloat(latestPrediction.rsi) > 70
															? "(Overbought)"
															: Number.parseFloat(latestPrediction.rsi) < 30
																? "(Oversold)"
																: ""}
													</span>
												</p>
											...
											<div>
												...
												<p className="text-lg font-semibold">
													{Number.parseFloat(latestPrediction.beta).toFixed(2)}
												</p>
											</div>
											<div>
							...
							<Card>
								<CardContent className="text-center py-12">
									...
									<p className="text-muted-foreground">
										No predictions available. Generate predictions to see
										portfolio forecasts.
									</p>
								</CardContent>
							</Card>
						...
						{riskLoading ? (
							...
							<>
								...
								<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
									<Card>
										...
										<CardContent>
											<div className="text-2xl font-bold text-green-600">
												{Number.parseFloat(
													latestRisk.diversificationScore,
												).toFixed(0)}
												/100
											</div>
										</CardContent>
									...
									<Card>
										...
										<CardContent>
											<div className="text-2xl font-bold">
												{Number.parseFloat(latestRisk.correlationRisk).toFixed(
													0,
												)}
												%
											</div>
								...
								<Card>
									<CardHeader>
										...
										<CardDescription>
											Impact on your portfolio under various market conditions
										</CardDescription>
									</CardHeader>
									<CardContent>
										<div className="space-y-4">
											{latestRisk.marketCrashScenario && (
												<div className="border-l-4 border-red-500 pl-4">
													<h4 className="font-semibold">
														{latestRisk.marketCrashScenario.scenario}
													</h4>
													<p className="text-sm text-muted-foreground">
														Impact:{" "}
													...
													<p className="text-sm text-muted-foreground">
														New Value: ₹
														{Number.parseFloat(
															latestRisk.marketCrashScenario.newValue,
														).toLocaleString("en-IN")}
													</p>
													<p className="text-xs text-muted-foreground">
														Recovery Time:{" "}
														{latestRisk.marketCrashScenario.recoveryTime}
													</p>
												</div>
											)}
											...
											{latestRisk.recessionScenario && (
												<div className="border-l-4 border-orange-500 pl-4">
													<h4 className="font-semibold">
														{latestRisk.recessionScenario.scenario}
													</h4>
													<p className="text-sm text-muted-foreground">
														Impact:{" "}
														<span className="text-orange-600 font-semibold">
															₹
															{Number.parseFloat(
																latestRisk.recessionScenario.impactOnPortfolio,
															).toLocaleString("en-IN")}
														</span>
													</p>
													<p className="text-sm text-muted-foreground">
														New Value: ₹
														{Number.parseFloat(
															latestRisk.recessionScenario.newValue,
														).toLocaleString("en-IN")}
													</p>
												</div>
											)}
											...
											{latestRisk.interestRateRise && (
												<div className="border-l-4 border-yellow-500 pl-4">
													<h4 className="font-semibold">
														{latestRisk.interestRateRise.scenario}
													</h4>
													<p className="text-sm text-muted-foreground">
														Impact:{" "}
														<span className="text-yellow-600 font-semibold">
															₹
															{Number.parseFloat(
																latestRisk.interestRateRise.impactOnPortfolio,
															).toLocaleString("en-IN")}
														</span>
													</p>
												</div>
									...
									<CardContent>
										<div className="grid grid-cols-3 gap-4">
											<div>
												...
												<p className="text-lg font-semibold">
													₹
													{Number.parseFloat(latestRisk.var1Day).toLocaleString(
														"en-IN",
													)}
												</p>
											</div>
								...
								{/* Recommendations */}
								{latestRisk.riskMitigationSuggestions &&
									latestRisk.riskMitigationSuggestions.length > 0 && (
										<Card>
											<CardHeader>
											...
											<CardContent>
												<div className="space-y-2">
													{latestRisk.riskMitigationSuggestions.map(
							...
							<Card>
								<CardContent className="text-center py-12">
									...
									<p className="text-muted-foreground">
										No risk analysis available. Generate risk analysis to see
										...
										<CardContent className="space-y-3">
											<div className="flex justify-between">
												...
												<span className="font-semibold">
													₹{Number.parseFloat(forecast.currentPrice).toFixed(2)}
												</span>
											</div>
											<div className="flex justify-between">
												<span className="text-sm text-muted-foreground">
												...
												<span className="font-semibold">
													₹
													{Number.parseFloat(forecast.predictedPrice).toFixed(
														2,
													)}
												</span>
											</div>
											<div className="flex justify-between">
												...
												<span
													className={`font-semibold ${Number.parseFloat(forecast.priceChange) >= 0 ? "text-green-600" : "text-red-600"}`}
												>
													{Number.parseFloat(forecast.priceChange).toFixed(2)}%
												</span>
											</div>
											<div className="flex items-center justify-between pt-2 border-t">
												<Badge
													variant={
														forecast.recommendation === "strong_buy" ||
														forecast.recommendation === "buy"
															? "default"
															: forecast.recommendation === "strong_sell" ||
												...
												<span className="text-xs text-muted-foreground">
													{Number.parseFloat(forecast.confidenceLevel).toFixed(
														0,
													)}
													% confidence
												</span>
