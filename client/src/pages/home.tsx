...
export default function Home() {
	...
	const totalValue = Array.isArray(holdings)
		...
		: 2850000; // Updated realistic fallback
	...
	const platformFeatures = [
		{
			...
			description:
				"Advanced portfolio analytics with real-time rebalancing suggestions and risk optimization using machine learning algorithms.",
			icon: Cpu,
			color: "blue",
			stats: statsLoading
				? "Loading..."
				: platformStatsData?.avgPortfolioValue
					? `${platformStatsData.avgPortfolioValue} Avg Portfolio`
					: "₹0 Avg Portfolio",
		},
		{
			title: "Real-Time Market Intelligence",
			description:
				"Live data from NSE, BSE, MCX, NCDEX with advanced charting and technical analysis tools.",
			icon: Activity,
			color: "green",
		...
		},
		{
			...
			description:
				"Access to Mutual Funds, IPOs, Pre-IPO, Bonds, AIFs, PMS, and alternative investments from one platform.",
			icon: Target,
			color: "purple",
			stats: statsLoading
				? "Loading..."
				: (platformStatsData?.investmentOptions || "0+") +
					" Investment Options",
		},
		{
			title: "Smart Financial Tools",
			description:
				"Advanced calculators for SIP, EMI, tax planning, retirement corpus, and goal-based investing.",
			icon: Calculator,
			color: "orange",
	...
	const investmentProducts = [
		{
			...
			volume: statsLoading
				? "Loading..."
				: `${platformStatsData?.mutualFundsCount || "0"} Schemes Available`,
		},
		{
			...
			volume: statsLoading
				? "Loading..."
				: `${platformStatsData?.portfoliosCount || "0"} Active Portfolios`,
		},
		{
			name: "IPO Center",
			...
			volume: statsLoading
				? "Loading..."
				: `${platformStatsData?.activeIpos || "0"} Active IPOs`,
		},
		{
			...
			volume: statsLoading
				? "Loading..."
				: `${platformStatsData?.stocksCount || "0"} Stocks Tracked`,
		},
		{
			name: "Bonds & NCDs",
			...
			volume: statsLoading
				? "Loading..."
				: `${platformStatsData?.bondsCount || "0"} Bonds Available`,
		},
		{
			name: "Loan Services",
	...
	const financialServices = [
		{
			...
			services: [
				{
					name: "NSDL Services",
					desc: "Depository services integration",
					icon: Database,
					route: "/nsdl-services",
				},
				{
					name: "CDSL Services",
					desc: "Depository account access",
					icon: LucideShield,
					route: "/cdsl-services",
				},
				{
					name: "Pre-IPO Access",
					desc: "Unicorn equity stakes",
					icon: Sparkles,
					route: "/pre-ipo",
				},
				{
					name: "Unlisted Securities",
					desc: "Pre-IPO opportunities",
					icon: Building2,
					route: "/unlisted",
				},
			],
			...
			services: [
				{
					...
					icon: CreditCard,
					route: "/loans?type=personal",
				},
				{
					name: "Home Loan",
					desc: "Competitive home loan rates",
					icon: Building2,
					route: "/loans?type=home",
				},
				{
					name: "Business Loan",
					desc: "SME & corporate financing",
					icon: Briefcase,
					route: "/loans?type=business",
				},
				{
					name: "Loan Against Securities",
					desc: "Flexible LAS facility",
					icon: Receipt,
					route: "/loans?type=las",
				},
			],
		},
		{
			...
			services: [
				{
					name: "Calculators",
					desc: "Financial planning tools",
					icon: Calculator,
					route: "/calculators",
				},
				{
					name: "CIBIL Services",
					desc: "Credit score check",
					icon: Activity,
					route: "/cibil",
				},
				{
	...
	const platformStats = [
		{
			label: "Active Users",
			value: statsLoading ? "..." : platformStatsData?.activeUsers || "0",
			icon: Users,
			color: "blue",
		},
		{
			label: "Portfolio Value",
			value: statsLoading ? "..." : platformStatsData?.portfolioValue || "₹0",
			icon: PieChart,
			color: "green",
		},
		{
			label: "Daily Trades",
			value: statsLoading ? "..." : platformStatsData?.dailyTrades || "0",
			icon: TrendingUp,
			color: "purple",
		},
		{
			label: "Monthly Trades",
			value: statsLoading ? "..." : platformStatsData?.monthlyTrades || "0",
			icon: Zap,
			color: "orange",
		},
	];
		...
		<div className="space-y-8" data-testid="home-page">
			...
			<section className="relative bg-gradient-to-r from-blue-900 via-blue-800 to-purple-900 text-foreground py-20 overflow-hidden">
				...
				<div className="absolute inset-0 opacity-10">
					<div className="absolute top-20 left-10 w-72 h-72 rounded-full bg-card/20 animate-pulse" />
					<div className="absolute bottom-20 right-10 w-96 h-96 rounded-full bg-purple-300/30 animate-bounce slow" />
				</div>
					...
					<div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
						...
						<div className="space-y-8">
							...
							{isAuthenticated && currentUser && (
								<div
									className="bg-card/10 backdrop-blur-sm rounded-xl p-4 border border-white/20"
							...
							<div className="space-y-4">
								...
								<p className="text-xl text-blue-100 leading-relaxed">
									Complete financial platform with AI-powered portfolio
									management, real-time market data, and access to diverse
									investment options. Start your wealth journey today.
								</p>
							</div>
							...
							<div className="flex flex-col sm:flex-row gap-4 w-full">
								<Link href="/wealth-management" className="w-full sm:w-auto">
									<Button
										size="lg"
										className="w-full sm:w-auto bg-gradient-to-r from-yellow-500 to-orange-600 hover:from-yellow-600 hover:to-orange-700 text-foreground font-semibold px-6 sm:px-8 py-3 sm:py-4 rounded-xl shadow-2xl transform hover:scale-105 transition-all"
									>
								...
								<Button
									size="lg"
									variant="outline"
									className="w-full sm:w-auto border-2 border-white/30 text-foreground hover:bg-card/10 font-semibold px-6 sm:px-8 py-3 sm:py-4 rounded-xl backdrop-blur-sm"
								>
				...
				<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
					<div className="text-center mb-16">
						...
						<p className="text-xl text-muted-foreground max-w-3xl mx-auto">
							Experience next-generation financial technology with AI-powered
							insights, real-time data, and comprehensive investment solutions.
						</p>
					</div>
									...
									<button
										...
										onKeyDown={(e) => {
											if (e.key === "ArrowDown") {
												e.preventDefault();
												setActiveFeature(
													(prev) => (prev + 1) % platformFeatures.length,
												);
							...
							>
								<div className="flex items-center mb-6">
									{(() => {
										...
										return (
											<ShowcaseIcon className="w-8 h-8 mr-3 text-yellow-400" />
										);
									})()}
			...
			<section
				...
				data-testid="investment-products-section"
			>
				<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
					<div className="text-center mb-16">
						<h2 className="text-4xl font-bold text-foreground mb-4">
						...
						<p className="text-xl text-muted-foreground">
							Complete suite of investment solutions for wealth creation
						</p>
					</div>
									...
									<Card className="group hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 border-0 shadow-lg">
										<CardHeader className="pb-4">
											<div className="flex items-center justify-between">
												<div
													className={`p-3 rounded-lg ${colorClasses[product.color as keyof typeof colorClasses]}`}
												>
						...
						{financialServices.map((category, index) => (
							<div key={index} className="space-y-4">
								<h3 className="text-2xl font-bold text-foreground mb-6 flex items-center">
									<div className="w-3 h-8 bg-gradient-to-b from-blue-500 to-purple-600 rounded-full mr-3" />
									{category.category}
								</h3>
								<div className="space-y-4">
				...
				<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
					<div className="text-center mb-10">
						...
						<p className="text-lg text-muted-foreground">
							Track government securities yields and compare historical trends
						</p>
					</div>
					<YieldCurveChart />
				...
				<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
					...
					<div className="mt-12 text-center">
						<Link href="/markets">
							<Button
								size="lg"
								className="bg-gradient-to-r from-yellow-500 to-orange-600 hover:from-yellow-600 hover:to-orange-700 text-foreground font-semibold px-8 py-4 rounded-xl"
							>
