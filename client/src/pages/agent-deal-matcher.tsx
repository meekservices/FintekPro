			...
			<Card className="border-0 bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-950/40 dark:to-teal-900/20">
				<CardContent className="pt-4 pb-3">
					...
					<div className="text-2xl font-bold text-emerald-900 dark:text-emerald-100">
						{p.totalInvestableRupees
							? formatCrore(p.totalInvestableRupees)
							: "—"}
					</div>
			...
			<Card className="border-0 bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-950/40 dark:to-indigo-900/20">
				<CardContent className="pt-4 pb-3">
					...
					<div className="text-xs text-blue-600 dark:text-blue-400 mt-1">
						{d.aifCount || 0} AIF · {d.pmsCount || 0} PMS
					</div>
				</CardContent>
			</Card>
		</div>
				...
				<div key={c.city} className="flex items-center gap-3 group">
					...
					<div className="w-20 text-right shrink-0">
						<span className="text-xs text-muted-foreground">
							{formatCrore(c.totalInvestable)}
						</span>
					</div>
					{c.hotCount > 0 && (
						<Badge
							variant="outline"
							className="text-[10px] px-1.5 py-0 bg-red-50 text-red-600 border-red-200 dark:bg-red-950/30 dark:text-red-300"
						>
							🔥 {c.hotCount}
						</Badge>
					)}
...
export default function AgentDealMatcher() {
		...
		mutationFn: () =>
			apiRequest("/api/deals/match-prospects", {
				...
				body: JSON.stringify({
					...
					limit: Math.min(Number.parseInt(limitStr) || 50, 100),
				}),
			}),
		onError: (e: any) => {
			toast({
	...
	const filteredMatches = (result?.matches || []).filter((m) => {
		...
		if (filterQuality !== "all" && m.leadQuality !== filterQuality)
			return false;
		if (searchText) {
			...
			<div className="flex items-start justify-between gap-4">
				<div>
					...
					<p className="text-sm text-muted-foreground mt-1">
						Match AIF and PMS deals to the best-fit HNI prospects using wealth,
						engagement, and geography signals.
					</p>
				</div>
					...
					<Card>
						<CardHeader className="pb-3">
							<CardTitle className="text-base">
								Select a Deal to Match
							</CardTitle>
							<CardDescription>
								Choose an AIF or PMS product and run the matching engine.
							</CardDescription>
						</CardHeader>
								...
								<div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3 max-h-72 overflow-y-auto pr-1">
									{deals.length === 0 ? (
										<div className="col-span-3 text-sm text-muted-foreground text-center py-8">
											No AIF or PMS products found. Add deals from the product
											catalogue.
										</div>
									) : (
										deals.map((deal) => (
											...
											>
												...
												<div className="flex items-center gap-2 mt-1.5">
													<span className="text-xs text-muted-foreground">
														Min:{" "}
														{formatMinTicket(
															Number.parseFloat(deal.minInvestment || "0"),
														)}
													</span>
													{deal.return1Y && (
														<span className="text-xs text-emerald-600 font-medium">
															+{Number.parseFloat(deal.return1Y).toFixed(1)}% 1Y
														</span>
													)}
												</div>
												{deal.category && (
						...
						<div className="space-y-4">
							<div className="flex items-center gap-4 flex-wrap">
								...
								<div className="text-sm text-muted-foreground">
									...
									<span className="font-semibold text-foreground">
										{formatCrore(result.totalInvestable)}
									</span>
								</div>
										...
										<TableBody>
											{filteredMatches.length === 0 ? (
												...
												filteredMatches.map((m, i) => {
													...
													const qual =
														qualityConfig[
															m.leadQuality as keyof typeof qualityConfig
														] || qualityConfig.cold;
													return (
														<TableRow
															key={m.prospectId}
														...
														>
															...
															<TableCell>
																<Badge
																	className={`text-[10px] gap-1 ${qual.color} border-0`}
																>
																	{qual.icon}
																	{(m.leadQuality || "cold")
																		.charAt(0)
																		.toUpperCase() +
																		(m.leadQuality || "cold").slice(1)}
																</Badge>
															</TableCell>
															...
															<TableCell className="text-right">
																<span
																	className={`text-xs font-semibold ${m.surplus_cover >= 1.5 ? "text-emerald-600" : m.surplus_cover >= 1 ? "text-amber-600" : "text-gray-500"}`}
																>
															...
															<TableCell>
																<Badge
																	className={`text-[10px] gap-1 border-0 ${tier.color}`}
																>
																	<span
				...
				<TabsContent value="geo">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Prospect concentration, average composite score, and investable
								capital by city.
							</CardDescription>
						</CardHeader>
						<CardContent>
