		...
		<div className="space-y-6">
			<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
				<Card>
					...
					<CardContent>
						<p className="text-2xl font-bold">
							{diagnostics?.uptime?.formatted || "N/A"}
						</p>
						<p className="text-xs text-muted-foreground">
							v{diagnostics?.serverVersion}
						</p>
					</CardContent>
				</Card>
				<Card>
					...
					<CardContent>
						<p className="text-2xl font-bold">
							{diagnostics?.memory?.heapUsedMB || 0} MB
						</p>
						<p className="text-xs text-muted-foreground">
							of{" "}
							{diagnostics?.memory
				...
				<Card>
					...
					<CardContent>
						<p className="text-2xl font-bold">
							{diagnostics?.system?.cpuCount || 0} CPUs
						</p>
						<p className="text-xs text-muted-foreground">
							{diagnostics?.system?.freeMemoryMB || 0} MB free /{" "}
							{diagnostics?.system?.totalMemoryMB || 0} MB total
						</p>
					</CardContent>
				</Card>
			</div>
					...
					<div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
						...
						<div>
							...
							<p className="font-medium text-xs truncate">
								{diagnostics?.environment?.deploymentUrl || "N/A"}
							</p>
						</div>
						<div>
							<p className="text-muted-foreground">DB Connections</p>
							<p className="font-medium">
								{diagnostics?.dbPool?.activeConnections ?? "N/A"}
							</p>
						</div>
					</div>
				</CardContent>
				...
				<CardContent>
					{healthCheck?.checks ? (
						<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
							{Object.entries(healthCheck.checks as Record<string, any>).map(
...
function ErrorLogTab() {
	...
	const allErrors: any[] = [];
	if (data?.byModule) {
		Object.values(data.byModule as Record<string, any>).forEach(
			(group: any) => {
									...
									<>
										...
										>
											...
											<TableCell>
												<Badge
													variant={
														SEVERITY_BADGE_VARIANT[error.severity] || "outline"
													}
												>
													{error.severity}
												</Badge>
...
function ExportDataTab() {
	...
	allErrors.forEach((e) => {
		severityCounts[e.severity] = (severityCounts[e.severity] || 0) + 1;
		moduleCounts[e.module || "unknown"] =
			(moduleCounts[e.module || "unknown"] || 0) + 1;
		const d = e.createdAt || "";
	...
	async function handleExport() {
		setIsExporting(true);
		try {
			const res = await fetch("/api/tester/errors/export", {
				credentials: "include",
			});
			...
			toast({
				...
				description: "Error data exported successfully.",
			});
		} catch {
			toast({
				title: "Export failed",
				description: "Could not download error data.",
				variant: "destructive",
			});
		} finally {
		...
		<div className="space-y-6">
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Overview of current error data available for export
					</CardDescription>
				</CardHeader>
					...
					<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
						...
						<div>
							...
							<p className="text-sm font-medium">
								{minDate ? format(new Date(minDate), "yyyy-MM-dd") : "N/A"} —{" "}
								{maxDate ? format(new Date(maxDate), "yyyy-MM-dd") : "N/A"}
							</p>
						</div>
						<div>
			...
			<Card>
				<CardContent className="py-8 flex flex-col items-center gap-4">
					...
					<p className="text-sm text-muted-foreground text-center max-w-md">
						Downloads all errors as a JSON file including metadata, timestamps,
						stack traces, and environment info.
					</p>
...
function ReportBugTab() {
	...
	const reportMutation = useMutation({
		mutationFn: async (data: any) => {
			return apiRequest("/api/tester/report", {
				method: "POST",
		...
		onError: () => {
			toast({
				...
				description: "Could not submit bug report.",
				variant: "destructive",
			});
		},
	});
	...
	return (
		<Card>
			<CardHeader>
				...
				<CardDescription>
					Report issues for the development team to investigate
				</CardDescription>
			</CardHeader>
			<CardContent>
					...
					<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
						...
						<div>
							...
							<Input
								...
								placeholder="e.g., auth, portfolio"
							/>
						</div>
						<div>
			...
			<div className="container mx-auto p-6">
				<Card>
					<CardContent className="py-16 text-center">
						...
						<p className="text-muted-foreground">
							Please sign in with a tester account to access diagnostics.
						</p>
					</CardContent>
				</Card>
			</div>
			...
			<div className="container mx-auto p-6">
				<Card>
					<CardContent className="py-16 text-center">
						...
						<p className="text-muted-foreground">
							Your account does not have the tester role. Contact an
		...
		<div className="container mx-auto p-6 space-y-6">
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-muted-foreground">
						System diagnostics, error tracking, and bug reporting
					</p>
				</div>
				<Button
					variant="outline"
