...
export default function MldDetailPage() {
	...
	const addToPortfolioMutation = useMutation({
		...
		onError: (err: any) => {
			toast({
				title: "Error",
				description: err.message || "Failed to add to portfolio",
				variant: "destructive",
			});
		},
	});
		...
		return (
			<div className="container mx-auto py-6">
				<Alert variant="destructive">
					...
					<AlertDescription>
						Failed to load MLD details. Please try again.
					</AlertDescription>
				</Alert>
	...
	const daysToMaturity = mld.maturityDate
		? differenceInDays(parseISO(mld.maturityDate), new Date())
		: 0;
	const yearsToMaturity = mld.maturityDate
		? differenceInYears(parseISO(mld.maturityDate), new Date())
		: 0;

	const simulatedPayoff =
		analytics?.payoffGraph?.find(
						...
						<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
							...
							<div>
								...
								<p className="font-medium">
									{format(parseISO(mld.issueDate), "dd MMM yyyy")}
								</p>
							</div>
							<div>
							...
							<div>
								...
								<p className="font-medium flex items-center gap-1">
									...
									{daysToMaturity > 0
										? `${yearsToMaturity} years (${daysToMaturity} days)`
										: "Matured"}
								</p>
							</div>
							<div>
						...
						<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
							...
							{mld.couponRate && (
								<div>
									...
									<p className="font-medium text-green-600">
										{mld.couponRate}% {mld.couponFrequency}
									</p>
								</div>
							)}
							{mld.creditRating && (
								<div>
									...
									<p className="font-medium">
										{mld.creditRating} ({mld.ratingAgency})
									</p>
								</div>
							)}
						</div>
					...
					<CardContent className="space-y-4">
						<div className="text-center">
							<div className="text-5xl font-bold">
								{mld.riskScore || analytics?.riskScore || "—"}
							</div>
								...
								{analytics.expectedReturn && (
									<div className="flex justify-between">
										...
										<span className="font-semibold">
											{analytics.expectedReturn.toFixed(2)}%
										</span>
									</div>
								)}
								{analytics.maxLoss && (
				...
				<TabsContent value="overview" className="mt-6 space-y-6">
					...
					{mld.riskFactors && (
						<Card>
							<CardHeader>
								<CardTitle className="flex items-center gap-2">
									<AlertTriangle className="w-5 h-5 text-orange-500" /> Risk
									Factors
								</CardTitle>
							</CardHeader>
							<CardContent>
				...
				<TabsContent value="scenarios" className="mt-6">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Estimated returns under different market conditions
							</CardDescription>
						</CardHeader>
						<CardContent>
                    ...
                    `}
									>
										<CardHeader className="pb-2">
											...
											<CardDescription>
												Index {scenario.indexChange >= 0 ? "+" : ""}
												{scenario.indexChange}%
											</CardDescription>
										</CardHeader>
										<CardContent>
											<div className="text-3xl font-bold">
												{scenario.returnPercent >= 0 ? "+" : ""}
												{scenario.returnPercent.toFixed(2)}%
											</div>
				...
				<TabsContent value="payoff" className="mt-6">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Expected payoff at different underlying levels
							</CardDescription>
						</CardHeader>
						<CardContent>
								...
								<div className="h-80">
									<ResponsiveContainer width="100%" height="100%">
										<RechartsLineChart data={analytics.payoffGraph}>
											...
											{mld.barrierLevel && (
												<ReferenceLine
													x={-Number.parseFloat(mld.barrierLevel)}
													stroke="#ef4444"
													strokeDasharray="5 5"
													label={{
														value: "Barrier",
					...
					<Card>
						...
						<CardContent className="space-y-6">
							...
							<div className="grid gap-4 md:grid-cols-3">
								...
								>
									<CardContent className="pt-6">
										...
										<p
											className={`text-2xl font-bold ${simulatedPayoff >= 0 ? "text-green-600" : "text-red-600"}`}
										>
											{simulatedPayoff >= 0 ? "+" : ""}
											{simulatedPayoff.toFixed(2)}%
										</p>
									</CardContent>
								</Card>
								<Card className="bg-muted/50">
									<CardContent className="pt-6">
										...
										<p className="text-2xl font-bold">
											{formatCurrency(
												Number.parseFloat(mld.faceValue) *
													(1 + simulatedPayoff / 100),
											)}
										</p>
									</CardContent>
								</Card>
							...
							<Alert>
								...
								<AlertDescription>
									This is a simplified simulation. Actual returns may vary based
									on exact payoff terms, observation dates, and market
									conditions. Please read the offer document carefully.
								</AlertDescription>
							</Alert>
						</CardContent>
					</Card>
					...
					<div className="space-y-4 py-4">
						<div>
							...
							<Input
								...
								onChange={(e) =>
									setPortfolioForm({ ...portfolioForm, units: e.target.value })
								}
						...
						<div>
							...
							<Input
								...
								onChange={(e) =>
									setPortfolioForm({ ...portfolioForm, notes: e.target.value })
								}
