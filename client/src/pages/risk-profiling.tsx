...
const RISK_TIER_COLORS: Record<string, string> = {
  RP1: "bg-green-100 text-green-800 border-green-200",
  RP2: "bg-blue-100 text-blue-800 border-blue-200",
  RP3: "bg-yellow-100 text-yellow-800 border-yellow-200",
  RP4: "bg-orange-100 text-orange-800 border-orange-200",
  RP5: "bg-red-100 text-red-800 border-red-200",
};
...
export default function RiskProfilingPage() {
  ...
  const [currentStep, setCurrentStep] = useState<"intro" | "questionnaire" | "review" | "result">("intro");
  const [currentQuestionIndex, setCurrentQuestionIndex] = useState(0);
  const [responses, setResponses] = useState<Record<number, { selectedOption: string; score: number }>>({});
  const [isSubmitting, setIsSubmitting] = useState(false);

  const { data: questionnaire, isLoading: loadingQuestions } = useQuery<{ questions: Question[]; version: string }>({
    queryKey: ["/api/sebi-risk-profiling/questionnaire"],
    enabled: currentStep === "questionnaire" || currentStep === "review",
  });

  const { data: existingProfile, isLoading: loadingProfile } = useQuery<RiskProfile>({
  ...
  const submitAssessmentMutation = useMutation({
    mutationFn: async (data: { questionnaireVersion: string; responses: Record<number, { selectedOption: string; score: number }> }) => {
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/sebi-risk-profiling/my-profile"] });
      queryClient.invalidateQueries({ queryKey: ["/api/sebi-risk-profiling/product-eligibility"] });
  ...
  const handleOptionSelect = (questionId: number, optionValue: string, score: number) => {
    setResponses((prev) => ({
      ...prev,
  ...
  const isCurrentAnswered = currentQuestion ? !!responses[currentQuestion.id] : false;
        ...
        <Card>
          <CardHeader className="text-center">
            ...
            <CardDescription className="text-base mt-2">
              As per SEBI regulations, all investors must complete a risk profiling assessment before investing in securities.
              This assessment helps us understand your investment preferences and recommend suitable products.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
                ...
                <AlertDescription>
                  <div className="mt-2 space-y-2">
                    <div className="flex items-center gap-2">
                      <Badge className={RISK_TIER_COLORS[existingProfile.riskTier]}>
                    ...
                    <p className="text-sm">
                      Valid until: {new Date(existingProfile.validUntil).toLocaleDateString()}
                    </p>
                    {existingProfile.sebiOverrideApplied && (
                      <p className="text-sm text-orange-600">
                        SEBI Override Applied: {existingProfile.sebiOverrideReason}
                      </p>
                    )}
                    <p className="text-sm text-muted-foreground mt-2">
                      You can retake the assessment if your financial situation has changed.
                    </p>
                  </div>
                </AlertDescription>
              </Alert>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <Card className="border-2 border-primary/20">
                ...
                <CardContent>
                  <p className="text-sm text-muted-foreground">
                    Your risk profile determines which investment products are suitable for you.
                  </p>
                </CardContent>
              </Card>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid gap-2 md:grid-cols-2">
                {[
                  { name: "Age & Demographics", desc: "Life stage and family responsibilities" },
                  { name: "Income Stability", desc: "Source and consistency of income" },
                  { name: "Net Worth", desc: "Assets, liabilities, and liquidity" },
                  { name: "Investment Horizon", desc: "Time frame for investments" },
                  { name: "Risk Tolerance", desc: "Comfort with volatility and losses" },
                  { name: "Investment Experience", desc: "Prior knowledge and experience" },
                ].map((cat, idx) => (
            ...
            <Alert variant="default" className="border-amber-200 bg-amber-50 dark:bg-amber-900/20">
              ...
              <AlertDescription className="text-amber-700 dark:text-amber-300">
                Please answer all questions honestly and accurately. Your responses will be used to determine 
                your investment suitability. Providing inaccurate information may result in unsuitable investment recommendations.
              </AlertDescription>
            </Alert>
          </CardContent>
          <CardFooter className="flex justify-between">
            <Button variant="outline" onClick={() => setLocation("/portfolio")} data-testid="button-back-portfolio">
              Back to Portfolio
            </Button>
            <Button onClick={() => setCurrentStep("questionnaire")} data-testid="button-start-assessment">
      ...
      return (
        <div className="container mx-auto py-8 max-w-4xl">
          <Alert variant="destructive">
            ...
            <AlertDescription>No questionnaire available. Please contact support.</AlertDescription>
          </Alert>
        </div>
      );
    }
        ...
        <Card>
          <CardHeader>
            ...
            <div className="flex items-center gap-3 mt-6">
              ...
              <CardTitle className="text-xl leading-tight">{currentQuestion.questionText}</CardTitle>
            </div>
            {currentQuestion.helpText && (
              <CardDescription className="mt-3">{currentQuestion.helpText}</CardDescription>
            )}
              ...
              {currentQuestion.options.map((option, idx) => (
                <div
                  ...
                  className={`flex items-center space-x-3 p-4 border rounded-lg cursor-pointer transition-colors ${
                    responses[currentQuestion.id]?.selectedOption === option.value
                ...
                >
                  <RadioGroupItem value={option.value} id={`option-${currentQuestion.id}-${idx}`} />
                  <Label htmlFor={`option-${currentQuestion.id}-${idx}`} className="cursor-pointer flex-1 text-base">
                    {option.label}
                  </Label>
                </div>
              ))}
            ...
            {Object.entries(groupedQuestions).map(([category, categoryQuestions]) => (
              <div key={category} className="space-y-3">
                ...
                {categoryQuestions.map((q, idx) => {
                  ...
                  const selectedOption = q.options.find((o) => o.value === response?.selectedOption);
                  return (
                    <div key={q.id} className="p-4 bg-muted/30 rounded-lg">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <Button
                          ...
                          size="sm"
                          onClick={() => {
                            setCurrentQuestionIndex(questions.findIndex((qu) => qu.id === q.id));
      ...
      <div className="container mx-auto py-8 max-w-4xl space-y-6">
        <Card className="border-2 border-primary/20">
          ...
          <CardContent className="text-center space-y-6">
            <div className="inline-block">
              <Badge className={`text-2xl px-6 py-3 ${RISK_TIER_COLORS[existingProfile.riskTier]}`}>
                {existingProfile.riskTier} - {existingProfile.tierLabel}
              </Badge>
            </div>
            ...
            <div className="max-w-md mx-auto">
              <div className="flex items-center justify-between text-sm mb-2">
                ...
                <span className="font-bold">{existingProfile.riskScore}/100</span>
              </div>
            ...
            <div className="text-sm text-muted-foreground">
              <p>Assessment Date: {new Date(existingProfile.assessmentDate).toLocaleDateString()}</p>
              <p>Valid Until: {new Date(existingProfile.validUntil).toLocaleDateString()}</p>
            </div>
          </CardContent>
        </Card>
                ...
                return (
                  <div key={category} className="p-4 bg-muted/30 rounded-lg">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium w-12 text-right">{score as number}%</span>
                    </div>
                  </div>
                );
              })}
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Based on your risk profile, you are eligible for the following investment products
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid gap-3 md:grid-cols-2">
              {productEligibility?.map((product, idx) => (
                ...
                <p className="text-muted-foreground col-span-2">Loading eligibility...</p>
              )}
            </div>
          </CardContent>
        </Card>
        ...
        <div className="flex justify-center gap-4">
          <Button variant="outline" onClick={() => setLocation("/portfolio")} data-testid="button-go-portfolio">
            Go to Portfolio
          </Button>
          <Button onClick={() => setLocation("/store")} data-testid="button-explore-products">
            Explore Products
          </Button>
        </div>
      </div>
