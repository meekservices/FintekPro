...
interface UsageResponse {
	...
	stats: {
		...
		byProvider: Record<
			string,
			{ verifications: number; successRate: number; cost: number }
		>;
	};
	mockData: boolean;
	note: string;
	...
	const setProviderMutation = useMutation({
		...
		onSuccess: (_, provider) => {
			toast({
				...
				description: `PAN verification provider switched to ${provider}`,
			});
		...
		<div className="space-y-6">
			<div className="flex items-center justify-between">
				<div>
					<h2 className="text-2xl font-bold tracking-tight">
						PAN Verification Provider Configuration
					</h2>
			...
			{cheapestConfigured && cheapestConfigured.provider !== activeProvider && (
				<Alert>
					...
					<AlertDescription>
						Switch to {cheapestConfigured.name} to save on PAN verification
											...
											<Dialog>
												...
												<DialogContent>
													<DialogHeader>
														...
														<DialogDescription>
															Set the cost per PAN verification for{" "}
															{provider.name}
														</DialogDescription>
													</DialogHeader>
				...
				<TabsContent value="comparison" className="space-y-4">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Compare provider costs based on your estimated monthly volume
							</CardDescription>
						</CardHeader>
						<CardContent className="space-y-4">
							<div className="space-y-2">
								...
								<Input
									...
									value={monthlyVolume}
									onChange={(e) =>
										setMonthlyVolume(Number.parseInt(e.target.value) || 0)
									}
								/>
							</div>
							<div className="grid gap-3 md:grid-cols-3">
								{providers.map((provider) => {
									...
									const isLowest =
										configuredProviders.length > 0 &&
										provider.pricePerVerification ===
											Math.min(
												...configuredProviders.map(
													(p) => p.pricePerVerification,
												),
											);
									return (
										<Card
								...
								<CardContent className="pt-6">
									<div className="text-center">
										...
										<p className="text-3xl font-bold">
											{usageStats?.totalVerifications || 0}
										</p>
									</div>
								</CardContent>
							</Card>
							...
							<Card>
								<CardContent className="pt-6">
									<div className="text-center">
										...
										<p className="text-3xl font-bold">
											{usageStats?.thisMonth?.verifications || 0}
										</p>
									</div>
								</CardContent>
							</Card>
							<Card>
								<CardContent className="pt-6">
									<div className="text-center">
										...
										<p className="text-3xl font-bold">
											₹{(usageStats?.totalCost || 0).toLocaleString("en-IN")}
										</p>
									</div>
								</CardContent>
							</Card>
