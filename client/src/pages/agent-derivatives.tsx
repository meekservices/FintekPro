...
export default function AgentDerivatives() {
  ...
  const { data: symbolsData } = useQuery<{ symbols: string[]; lotSizes: Record<string, number> }>({
    queryKey: ['/api/derivatives/symbols'],
  });

  const { data: optionsChain, isLoading: isLoadingChain, refetch: refetchChain } = useQuery<OptionsChain>({
    queryKey: ['/api/derivatives/options-chain', selectedSymbol, selectedExpiry],
    enabled: !!selectedSymbol,
  });

  const { data: futuresData, isLoading: isLoadingFutures } = useQuery<{ symbol: string; futures: FuturesData[] }>({
  ...
  const { data: expiryCalendar } = useQuery<{ calendar: { date: string; type: 'weekly' | 'monthly'; symbols: string[] }[] }>({
    queryKey: ['/api/derivatives/expiry-calendar'],
  });

  const greeksMutation = useMutation({
    mutationFn: async (params: { spotPrice: number; strikePrice: number; daysToExpiry: number; volatility: number; optionType: 'call' | 'put' }) => {
  ...
  const filteredCalls = optionsChain?.options.calls.filter(c => {
    if (!showITM) return c.strikePrice >= getATMStrike();
    return true;
  }) || [];

  const filteredPuts = optionsChain?.options.puts.filter(p => {
  ...
  return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold" data-testid="text-page-title">F&O Analytics</h1>
            <p className="text-muted-foreground">Futures & Options analysis with Greeks and strategy builder</p>
          </div>
          <div className="flex items-center gap-4">
            ...
            <Button variant="outline" onClick={() => refetchChain()} data-testid="button-refresh">
          ...
          <div className="grid grid-cols-4 gap-4">
            <Card>
              <CardContent className="p-4">
                ...
                <p className="text-2xl font-bold" data-testid="text-spot-price">₹{optionsChain.underlyingValue.toLocaleString('en-IN', { maximumFractionDigits: 2 })}</p>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-4">
                ...
                <p className="text-2xl font-bold" data-testid="text-atm-strike">{getATMStrike()}</p>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-4">
                ...
                <p className="text-2xl font-bold" data-testid="text-lot-size">{symbolsData?.lotSizes[selectedSymbol] || '-'}</p>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-4">
                ...
                <Select value={selectedExpiry || optionsChain.expiryDates[0]} onValueChange={setSelectedExpiry}>
                  <SelectTrigger className="mt-1" data-testid="select-expiry">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {optionsChain.expiryDates.map(exp => (
                      <SelectItem key={exp} value={exp}>{new Date(exp).toLocaleDateString('en-IN', { day: '2-digit', month: 'short' })}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </CardContent>
              ...
              <CardHeader className="pb-2">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Switch checked={showITM} onCheckedChange={setShowITM} data-testid="switch-itm" />
                  </div>
                </div>
                <CardDescription>Click on premium to add to strategy</CardDescription>
              </CardHeader>
              <CardContent>
                {isLoadingChain ? (
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow className="bg-green-50 dark:bg-green-950">
                          ...
                          <TableHead className="text-center text-green-600">CALLS</TableHead>
                          <TableHead className="text-center font-bold bg-muted">STRIKE</TableHead>
                          <TableHead className="text-center text-red-600">PUTS</TableHead>
                          <TableHead className="text-center">IV%</TableHead>
                          <TableHead className="text-center">Volume</TableHead>
                          <TableHead className="text-center">Chg OI</TableHead>
                          <TableHead className="text-center">OI</TableHead>
                      ...
                      <TableBody>
                        {optionsChain?.strikePrices.slice(5, 26).map(strike => {
                          ...
                          const isITMCall = strike < (optionsChain?.underlyingValue || 0);
                          const isITMPut = strike > (optionsChain?.underlyingValue || 0);
                          ...
                          return (
                            ...
                            >
                              <TableCell className={`text-center text-xs ${isITMCall ? 'bg-green-50 dark:bg-green-950/50' : ''}`}>
                                {call ? formatNumber(call.openInterest) : '-'}
                              </TableCell>
                              <TableCell className={`text-center text-xs ${isITMCall ? 'bg-green-50 dark:bg-green-950/50' : ''} ${call?.changeinOpenInterest && call.changeinOpenInterest > 0 ? 'text-green-600' : 'text-red-600'}`}>
                                {call ? formatNumber(call.changeinOpenInterest) : '-'}
                              </TableCell>
                              <TableCell className={`text-center text-xs ${isITMCall ? 'bg-green-50 dark:bg-green-950/50' : ''}`}>
                                {call ? formatNumber(call.totalTradedVolume) : '-'}
                              </TableCell>
                              <TableCell className={`text-center text-xs ${isITMCall ? 'bg-green-50 dark:bg-green-950/50' : ''}`}>
                                {call ? call.impliedVolatility.toFixed(1) : '-'}
                              </TableCell>
                              <TableCell className={`text-center ${isITMCall ? 'bg-green-50 dark:bg-green-950/50' : ''}`}>
                                {call && (
                                  <div className="flex items-center justify-center gap-1">
                                    ...
                                    <span className={`font-medium ${call.pChange >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                              ...
                              <TableCell className={`text-center ${isITMPut ? 'bg-red-50 dark:bg-red-950/50' : ''}`}>
                                {put && (
                                  <div className="flex items-center justify-center gap-1">
                                    <Button 
                                      variant="ghost" 
                                    ...
                                    <span className={`font-medium ${put.pChange >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                              ...
                              <TableCell className={`text-center text-xs ${isITMPut ? 'bg-red-50 dark:bg-red-950/50' : ''}`}>
                                {put ? put.impliedVolatility.toFixed(1) : '-'}
                              </TableCell>
                              <TableCell className={`text-center text-xs ${isITMPut ? 'bg-red-50 dark:bg-red-950/50' : ''}`}>
                                {put ? formatNumber(put.totalTradedVolume) : '-'}
                              </TableCell>
                              <TableCell className={`text-center text-xs ${isITMPut ? 'bg-red-50 dark:bg-red-950/50' : ''} ${put?.changeinOpenInterest && put.changeinOpenInterest > 0 ? 'text-green-600' : 'text-red-600'}`}>
                                {put ? formatNumber(put.changeinOpenInterest) : '-'}
                              </TableCell>
                              <TableCell className={`text-center text-xs ${isITMPut ? 'bg-red-50 dark:bg-red-950/50' : ''}`}>
          ...
          <TabsContent value="futures" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Current, Next, and Far month contracts</CardDescription>
              </CardHeader>
              <CardContent>
                {isLoadingFutures ? (
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    {futuresData?.futures.map((fut, idx) => (
                      <Card key={fut.expiryDate} className={idx === 0 ? 'border-primary' : ''}>
                        <CardHeader className="pb-2">
                          <div className="flex items-center justify-between">
                            <CardTitle className="text-lg">{['Current', 'Next', 'Far'][idx]} Month</CardTitle>
                            {idx === 0 && <Badge>Active</Badge>}
                          </div>
                          <CardDescription>{new Date(fut.expiryDate).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}</CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-4">
                          <div className="flex items-center justify-between">
                            <span className="text-2xl font-bold">₹{fut.lastPrice.toLocaleString('en-IN', { maximumFractionDigits: 2 })}</span>
                            <Badge variant={fut.pChange >= 0 ? "default" : "destructive"} className="flex items-center gap-1">
                              {fut.pChange >= 0 ? <TrendingUp className="h-3 w-3" /> : <TrendingDown className="h-3 w-3" />}
                          ...
                          <div className="grid grid-cols-2 gap-2 text-sm">
                            <div>
                              ...
                              <p className={`font-medium ${fut.basis >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                ₹{fut.basis.toFixed(2)} ({fut.basisPct.toFixed(2)}%)
                              </p>
                            </div>
                            <div>
                              <span className="text-muted-foreground">OI</span>
                              <p className="font-medium">{formatNumber(fut.openInterest)}</p>
                            </div>
                            <div>
                              <span className="text-muted-foreground">Volume</span>
                              <p className="font-medium">{formatNumber(fut.totalTradedVolume)}</p>
                            </div>
                            <div>
                              <span className="text-muted-foreground">OI Change</span>
                              <p className={`font-medium ${fut.changeinOpenInterest >= 0 ? 'text-green-600' : 'text-red-600'}`}>
            ...
            <div className="grid grid-cols-2 gap-4">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Build multi-leg options strategies</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  {strategyLegs.length === 0 ? (
                    <div className="text-center py-8 text-muted-foreground">
                      ...
                      <p className="text-sm">Click on options in the chain to add legs</p>
                    </div>
                  ) : (
                    <>
                      <Table>
                        ...
                        <TableBody>
                          {strategyLegs.map(leg => (
                            <TableRow key={leg.id}>
                              ...
                              <TableCell>
                                <Button variant="ghost" size="sm" onClick={() => removeLeg(leg.id)} data-testid={`button-remove-${leg.id}`}>
                      ...
                      <Button onClick={calculatePayoff} className="w-full" data-testid="button-analyze">
                ...
                <CardContent>
                  <div className="space-y-2">
                    {strategiesData?.strategies.slice(0, 6).map(strategy => (
                      <div key={strategy.name} className="flex items-center justify-between p-3 rounded-lg border hover:bg-muted/50 cursor-pointer">
                        <div>
                        ...
                        <div className="flex items-center gap-2">
                          <Badge variant={
                            strategy.outlook === 'bullish' ? 'default' :
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {marginMutation.data && (
                      <div className="text-right">
                        <p className="text-sm text-muted-foreground">Total Margin Required</p>
                        <p className="text-xl font-bold">₹{marginMutation.data.totalMargin.toLocaleString('en-IN')}</p>
                      </div>
                    )}
                  </div>
                </CardHeader>
                <CardContent>
                  <div className="grid grid-cols-4 gap-4 mb-6">
                    <div className="p-4 rounded-lg bg-green-50 dark:bg-green-950">
                      ...
                      <p className="text-xl font-bold text-green-600">
                        {payoffMutation.data.maxProfit === 'unlimited' ? 'Unlimited' : `₹${payoffMutation.data.maxProfit.toLocaleString('en-IN')}`}
                      </p>
                    </div>
                    <div className="p-4 rounded-lg bg-red-50 dark:bg-red-950">
                      <p className="text-sm text-muted-foreground">Max Loss</p>
                      <p className="text-xl font-bold text-red-600">
                        {payoffMutation.data.maxLoss === 'unlimited' ? 'Unlimited' : `₹${Math.abs(payoffMutation.data.maxLoss).toLocaleString('en-IN')}`}
                      </p>
                    </div>
                    <div className="p-4 rounded-lg bg-muted">
                      <p className="text-sm text-muted-foreground">Breakeven</p>
                      <p className="text-xl font-bold">
                        {payoffMutation.data.breakeven.length > 0 ? payoffMutation.data.breakeven.map((b: number) => b.toFixed(0)).join(', ') : '-'}
                      </p>
                    </div>
                    {marginMutation.data && (
                      <div className="p-4 rounded-lg bg-muted">
                        <p className="text-sm text-muted-foreground">Premium</p>
                        <p className="text-xl font-bold">₹{marginMutation.data.premium.toLocaleString('en-IN')}</p>
                      </div>
                    )}
                  </div>
                  <ResponsiveContainer width="100%" height={300}>
                    <AreaChart data={payoffMutation.data.payoffData}>
                      ...
                      <Area type="monotone" dataKey="profit" stroke="#2563eb" fill="url(#profitGradient)" />
                    </AreaChart>
                  </ResponsiveContainer>
                </CardContent>
              </Card>
          ...
          <TabsContent value="greeks" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Calculate option Greeks using Black-Scholes model</CardDescription>
              </CardHeader>
              <CardContent>
                <GreeksCalculator 
              ...
              <CardContent>
                <div className="space-y-2">
                  {expiryCalendar?.calendar.map(exp => (
                    <div key={exp.date} className="flex items-center justify-between p-4 rounded-lg border">
                      <div className="flex items-center gap-4">
                        <div className={`w-12 h-12 rounded-lg flex items-center justify-center ${exp.type === 'monthly' ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}>
                          <span className="text-lg font-bold">{new Date(exp.date).getDate()}</span>
                        </div>
                        <div>
                          <p className="font-medium">{new Date(exp.date).toLocaleDateString('en-IN', { weekday: 'long', day: '2-digit', month: 'short', year: 'numeric' })}</p>
                          <div className="flex items-center gap-2 mt-1">
                            <Badge variant={exp.type === 'monthly' ? 'default' : 'outline'}>{exp.type}</Badge>
                            <span className="text-xs text-muted-foreground">{exp.symbols.slice(0, 5).join(', ')}{exp.symbols.length > 5 ? ` +${exp.symbols.length - 5} more` : ''}</span>
                          </div>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="text-sm text-muted-foreground">Days to expiry</p>
                        <p className="text-xl font-bold">{Math.ceil((new Date(exp.date).getTime() - Date.now()) / (1000 * 60 * 60 * 24))}</p>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <Input value={spotPrice.toFixed(2)} disabled data-testid="input-spot-price" />
          </div>
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            ...
            <Input 
              type="number" 
              value={iv} 
              onChange={(e) => setIV(parseFloat(e.target.value) || 15)}
        ...
        <Button onClick={handleCalculate} disabled={isLoading} className="w-full" data-testid="button-calculate-greeks">
      ...
      {result && (
        <div className="grid grid-cols-2 gap-4">
          <div className="p-4 rounded-lg bg-blue-50 dark:bg-blue-950">
            ...
            <p className="text-xs text-muted-foreground mt-1">Price change per ₹1 move</p>
          </div>
          <div className="p-4 rounded-lg bg-purple-50 dark:bg-purple-950">
            ...
            <p className="text-xs text-muted-foreground mt-1">Delta change per ₹1 move</p>
          </div>
          <div className="p-4 rounded-lg bg-red-50 dark:bg-red-950">
            <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-xs text-muted-foreground mt-1">Daily time decay (₹)</p>
          </div>
          <div className="p-4 rounded-lg bg-green-50 dark:bg-green-950">
            <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-xs text-muted-foreground mt-1">Price change per 1% IV</p>
          </div>
          <div className="p-4 rounded-lg bg-amber-50 dark:bg-amber-950 col-span-2">
            ...
            <p className="text-xs text-muted-foreground mt-1">Price change per 1% interest rate</p>
          </div>
        </div>
      )}
    </div>
