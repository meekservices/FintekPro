...
export default function Loans() {
	...
	const { data: mpalProducts, isLoading: productsLoading } =
		useCreditProducts();
	...
	const createLoanRequestMutation = useMutation({
		...
		onSuccess: (response) => {
			toast({
				...
				description:
					"Your loan request has been saved. Now generating offers...",
			});
	...
	const generateOffers = async (requestId: string) => {
		...
		try {
			const response = await fetch(
				`/api/marketplace/loan-requests/${requestId}/generate-offers`,
				{
					method: "POST",
					credentials: "include",
				},
							...
							<CardContent className="p-6 flex items-center justify-between">
								<div>
									...
									<p className="text-white/80 mt-1 text-sm">
										Because of your existing investments, you qualify for
										instant liquidity up to
												...
												<CardContent className="p-6">
													...
													<div className="space-y-2 text-xs text-muted-foreground mb-4">
														...
														<div className="flex justify-between">
															<span>Tenure:</span>
															<span className="font-semibold">
																{Math.round(product.minTenure / 12)} -{" "}
																{Math.round(product.maxTenure / 12)} years
															</span>
														</div>
													</div>
						...
						<Card>
							<CardHeader>
								...
								<p className="text-muted-foreground">
									Fill in your requirements and we'll find the best loan offers
									for you
								</p>
							</CardHeader>
							<CardContent>
															...
															<SelectContent>
																{loanProducts?.data?.map((product: any) => (
																	<SelectItem
																		...
																		value={product.productKey || product.key}
																	>
							...
							<Card className="border-dashed border-2 border-border">
								<CardContent className="flex flex-col items-center justify-center py-12">
									...
									<p className="text-muted-foreground text-center mb-4">
										Submit a loan request to see personalized offers from
										multiple lenders
									</p>
									<Button
										variant="outline"
							...
							<div className="space-y-6">
								<div className="flex justify-between items-center">
									<div>
										...
										<p className="text-muted-foreground">
											Found {offers.length} offers matching your requirements
										</p>
									</div>
											...
											<CardHeader className="pb-3">
												<div className="flex justify-between items-start">
													...
													<div className="text-right">
														<div
															className={`inline-flex items-center px-2 py-1 rounded text-xs font-medium ${getRiskColor(offer.approvalProbability)}`}
														>
													...
													<div className="bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 rounded-lg p-3">
														...
														<div className="space-y-1 text-xs text-blue-700 dark:text-blue-300">
															<div className="flex justify-between">
																...
																<span className="font-semibold">
																	{offer.apr ||
																		(offer.interestRate + 0.5).toFixed(2)}
																	%
																</span>
															</div>
														...
														<Button
															...
															onClick={() => {
																const kfsUrl =
																	offer.kfsUrl ||
																	`/api/loans/kfs/${offer.id}?amount=${offer.amount}&tenure=${offer.tenure}&rate=${offer.interestRate}&provider=${encodeURIComponent(offer.providerName || "Partner Lender")}&fee=${offer.processingFee}`;
										...
										<CardHeader>
											<div className="flex justify-between items-center">
												<div>
													...
													<p className="text-muted-foreground">
														Compare selected offers in detail
													</p>
												</div>
												<div className="flex gap-2">
													<Button
													...
													<Button
														size="sm"
														onClick={() => {
															const bestOffer = selectedOffers
										...
										<CardContent>
															...
															{selectedOffers.map((offerId) => {
																...
																return offer ? (
																	<th
																		...
																		data-testid={`comparison-header-${offerId}`}
																	>
																		<div className="space-y-1">
													...
													<tbody>
														...
														<tr className="border-b hover:bg-muted">
															...
															{selectedOffers.map((offerId) => {
																...
																return (
																	<td
																		key={offerId}
																		className={`py-4 px-4 text-center ${isBest ? "bg-green-50 dark:bg-green-950/30" : ""}`}
																		data-testid={`rate-${offerId}`}
																	>
																		<div
																			className={`text-xl font-bold ${isBest ? "text-green-600" : "text-foreground"}`}
																		>
																			{offer.interestRate}%
																		</div>
																		{isBest && (
															...
															{selectedOffers.map((offerId) => {
																...
																return (
																	<td
																		key={offerId}
																		className={`py-4 px-4 text-center ${isBest ? "bg-green-50 dark:bg-green-950/30" : ""}`}
																		data-testid={`emi-${offerId}`}
																	>
																		<div
																			className={`text-lg font-semibold ${isBest ? "text-green-600" : "text-foreground"}`}
																		>
																			{formatCurrency(offer.monthlyEmi)}
																		</div>
																		{isBest && (
														...
														<tr className="border-b hover:bg-muted">
															...
															{selectedOffers.map((offerId) => {
																...
																const isBest =
																	offer.processingFee === lowestFee;
																return (
																	<td
																		key={offerId}
																		className={`py-4 px-4 text-center ${isBest ? "bg-green-50 dark:bg-green-950/30" : ""}`}
																		data-testid={`fee-${offerId}`}
																	>
																		<div
																			className={`text-lg font-semibold ${isBest ? "text-green-600" : "text-foreground"}`}
																		>
														...
														<tr className="border-b hover:bg-muted">
															...
															{selectedOffers.map((offerId) => {
																...
																const isBest =
																	offer.totalInterest === lowestInterest;
																return (
																	<td
																		key={offerId}
																		className={`py-4 px-4 text-center ${isBest ? "bg-green-50 dark:bg-green-950/30" : ""}`}
																		data-testid={`total-interest-${offerId}`}
																	>
																		<div
																			className={`text-lg font-semibold ${isBest ? "text-green-600" : "text-red-600"}`}
																		>
																			{formatCurrency(offer.totalInterest)}
																		</div>
																		{isBest && (
															...
															{selectedOffers.map((offerId) => {
																...
																return (
																	<td
																		...
																		data-testid={`response-time-${offerId}`}
																	>
											...
											<div className="mt-6 p-4 bg-blue-50 dark:bg-blue-950/30 rounded-lg">
												...
												<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
													{(() => {
														const selectedOfferData = selectedOffers
															.map((id) => offers.find((o) => o.id === id))
															.filter(Boolean);
														const bestRate = Math.min(
															...selectedOfferData.map((o) => o!.interestRate),
														);
														const worstRate = Math.max(
															...selectedOfferData.map((o) => o!.interestRate),
														);
														const bestEmi = Math.min(
															...selectedOfferData.map((o) => o!.monthlyEmi),
														);
														const worstEmi = Math.max(
															...selectedOfferData.map((o) => o!.monthlyEmi),
														);
														const rateDifference = worstRate - bestRate;
														const emiDifference = worstEmi - bestEmi;
														const bestRateOffer = selectedOfferData.find(
															(o) => o!.interestRate === bestRate,
														);
															...
															<>
																<div className="space-y-1">
																	...
																	<p className="text-blue-600">
																		{rateDifference.toFixed(2)}% spread
																	</p>
																	<p className="text-xs text-blue-600">
																		Best: {bestRate}% | Worst: {worstRate}%
																	</p>
																</div>
																<div className="space-y-1">
																	<p className="text-blue-800 dark:text-blue-200 font-medium">
																	...
																	<p className="text-blue-600">
																		{formatCurrency(emiDifference)} monthly
																	</p>
																	<p className="text-xs text-blue-600">
																		{formatCurrency(emiDifference * 12)}{" "}
																		annually
																	</p>
																</div>
																<div className="space-y-1">
																	...
																	<p className="text-blue-600 font-semibold">
																		{bestRateOffer?.providerName}
																	</p>
																	<p className="text-xs text-blue-600">
																		Lowest rate & competitive terms
																	</p>
																</div>
															</>
														);
											...
											<CardContent className="p-6">
												...
												<div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
													...
													<div>
														...
														<p className="font-semibold">
															{formatCurrency(application.monthlyEmi)}
														</p>
													</div>
													<div>
														<p className="text-muted-foreground">
					...
					>
						<Card>
							<CardHeader>
								...
								<p className="text-muted-foreground">
									Calculate your monthly EMI for any loan amount with full
									...
									<div className="mt-8 border rounded-lg overflow-hidden">
										<div className="bg-muted px-4 py-3 border-b">
											<h4 className="font-semibold">
												Amortization Schedule ({emiResult.schedule.length}{" "}
												months)
											</h4>
										</div>
					...
					<CardContent className="p-6">
						<div className="flex items-start gap-4">
							...
							<div className="space-y-4">
								<div>
									<h3 className="text-lg font-semibold text-foreground mb-2">
										RBI Digital Lending Disclosure
									</h3>
									<p className="text-sm text-muted-foreground">
										As per RBI Digital Lending Directions 2025, we are committed
										to transparent and unbiased loan offer presentation.
									</p>
								</div>
								...
								<div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
									<div className="space-y-2">
										...
										<p className="text-muted-foreground">
											Offers are ranked by: (1) Lowest APR (All-in Cost), (2)
											Eligibility match score based on your profile, (3)
											Processing time. No lender pays for preferential
											placement.
										</p>
									</div>
									<div className="space-y-2">
										...
										<p className="text-muted-foreground">
											Each offer includes a standardized KFS document with APR,
											fees, charges, and terms as mandated by RBI for borrower
											protection.
										</p>
									</div>
								</div>
								...
								<div className="flex items-center gap-2 pt-2">
									...
									<p className="text-xs text-muted-foreground">
										<strong>Grievance Redressal:</strong> For complaints,
										contact our Nodal Officer at grievance@fintekpro.com or
										escalate to RBI Ombudsman at rbiombudsman@rbi.org.in
									</p>
								</div>
							</div>
						</div>
