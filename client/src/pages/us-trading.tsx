...
export default function USTradingPage() {
	...
	const { canUseAi, canViewRecommendations, feeMode, requiresModeSelection } =
	...
	const placeOrderMutation = useMutation({
		mutationFn: async (orderData: {
			...
			const response = await apiRequest(
				"POST",
				"/api/mpal/broker/US_EQUITY/orders",
				mpalPayload,
			);
			if (!response.ok) {
				...
				<div className="flex items-center justify-between">
					...
					<div className="flex items-center gap-3">
						<Badge
							...
							data-testid="exchange-rate-badge"
						>
							<DollarSign className="h-3 w-3 mr-1" />1 USD = ₹
							{exchangeRate.toFixed(2)}
						</Badge>
						<Badge
							variant={
								marketData?.marketStatus === "open" ? "default" : "secondary"
							}
							className="text-sm py-1 px-3"
							...
							<div className="flex items-center justify-between">
								<div className="flex items-center gap-3">
									...
									<div>
										...
										<p className="text-sm text-green-600 dark:text-green-400">
											Risk Profile: {eligibilityData.riskProfile} | PAN
											Verified: ✓
										</p>
									</div>
								</div>
								<div className="text-right">
									...
									<p className="font-semibold text-lg">
										${eligibilityData.lrsRemaining.toLocaleString()} / $
										{eligibilityData.lrsLimit.toLocaleString()}
									</p>
								</div>
							</div>
						</CardContent>
								...
								<Card>
									...
									<CardContent className="space-y-2">
										{isLoadingMarket
											? Array(6)
													.fill(0)
													.map((_, i) => <Skeleton key={i} className="h-16" />)
											: POPULAR_STOCKS.slice(0, 6).map((stock) => {
													const quote = marketData?.stocks?.find(
													...
													return (
														...
														>
															...
															<div className="flex gap-1">
																...
																>
																	<Star
																		className={`h-4 w-4 ${isInWatchlist(stock.symbol) ? "fill-yellow-400 text-yellow-400" : ""}`}
																	/>
																</Button>
															</div>
														</div>
									...
									<CardContent className="space-y-2">
										{isLoadingMarket
											...
											: POPULAR_ETFS.map((etf) => {
													const quote = marketData?.etfs?.find(
													...
													return (
														...
														>
															...
															>
																<Star
																	className={`h-4 w-4 ${isInWatchlist(etf.symbol) ? "fill-yellow-400 text-yellow-400" : ""}`}
																/>
															</Button>
														</div>
													);
						...
						{isLoadingHoldings ? (
								...
								<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
									...
									<Card className="bg-gradient-to-br from-green-500 to-green-700 text-foreground">
										<CardContent className="p-6">
											...
											<p className="text-3xl font-bold">
												₹{holdings.totalValueINR.toLocaleString()}
											</p>
										</CardContent>
									</Card>
									<Card
										className={`bg-gradient-to-br ${holdings.totalProfitLoss >= 0 ? "from-emerald-500 to-emerald-700" : "from-red-500 to-red-700"} text-foreground`}
									>
										<CardContent className="p-6">
											<p className="text-foreground/80">Total P&L</p>
											<p className="text-3xl font-bold">
												{holdings.totalProfitLoss >= 0 ? "+" : ""}
												{holdings.totalProfitLossPercent.toFixed(2)}%
											</p>
										</CardContent>
									</Card>
								</div>
										...
										<div className="flex items-center justify-between">
											...
											{canUseAi && (
												<Button
													...
													data-testid="refresh-rebalancing"
												>
									...
									<CardContent>
										{!canUseAi ? (
											...
											<div className="space-y-4">
												<div className="p-4 bg-blue-50 dark:bg-blue-950/30 rounded-lg">
													...
													<p className="text-sm text-blue-700 dark:text-blue-300">
														{rebalancing.analysis.rationale}
													</p>
												</div>
												...
												{rebalancing.analysis.suggestedTrades.length > 0 ? (
													<>
														<div className="p-3 bg-green-50 dark:bg-green-950/30 rounded-lg">
															...
															<p className="text-sm text-green-700 dark:text-green-300">
																{rebalancing.analysis.expectedImpact}
															</p>
														</div>
														...
														<div className="space-y-2">
															...
															{rebalancing.analysis.suggestedTrades.map(
																(trade, idx) => (
																	<div
																		key={idx}
																				...
																				className={
																					trade.priority === "high"
																						...
																						: trade.priority === "medium"
																							? "border-yellow-300 dark:border-yellow-700 text-yellow-600"
																							: "border-border"
																				}
																			>
																				{trade.priority}
													...
													<div className="text-center py-4 text-muted-foreground">
														...
														<p>
															Your portfolio is well-balanced. No rebalancing
															needed.
														</p>
													</div>
												)}
											...
											<div className="text-center py-6 text-muted-foreground">
												...
												<p>
													Complete your risk profile to get personalized
							...
							<Card>
								<CardContent className="p-12 text-center">
									...
									<p className="text-muted-foreground mb-4">
										Start investing in US equities to build your global
										portfolio.
									</p>
								...
								<>
									<div className="grid grid-cols-2 gap-4 p-4 bg-muted/50 rounded-lg">
										...
										<div>
											...
											<p className="text-xl font-bold">
												₹
												{((selectedStock?.price || 0) * exchangeRate).toFixed(
													2,
												)}
											</p>
										</div>
										...
										<div className="flex items-center gap-2">
											<Button
												...
												onClick={() =>
													setQuantity(
														String(
															Math.max(0, (Number.parseInt(quantity) || 0) - 1),
														),
													)
												}
											...
											<Button
												...
												onClick={() =>
													setQuantity(
														String((Number.parseInt(quantity) || 0) + 1),
													)
												}
										...
										<div className="p-4 bg-blue-50 dark:bg-blue-950/30 rounded-lg">
											<div className="flex justify-between mb-2">
												...
												<span className="font-bold">
													$
													{(
														Number.parseInt(quantity) * selectedStock.price
													).toFixed(2)}
												</span>
											</div>
									...
									<Alert className="border-orange-200 bg-orange-50 dark:bg-orange-950/30">
										...
										<AlertDescription className="text-orange-700 dark:text-orange-300 text-sm">
											US equity trading is subject to FEMA regulations. This
											investment counts towards your LRS annual limit of
											$250,000.
										</AlertDescription>
									</Alert>
									...
									<div className="space-y-3">
										<div className="flex items-start gap-2">
											...
											>
												I understand and accept the risks associated with US
												equity trading, including currency fluctuation and
												regulatory risks.
											</label>
										</div>
										<div className="flex items-start gap-2">
											<Checkbox
												...
												onCheckedChange={(checked) =>
													setLrsDeclaration(checked as boolean)
												}
											...
											<label
												...
												className="text-sm leading-tight cursor-pointer"
											>
												I declare this transaction is within my LRS limit and
												complies with FEMA regulations for Indian residents.
											</label>
										</div>
									</div>
								</>
									...
									<div className="p-4 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900 rounded-lg">
										<div className="flex gap-3">
											...
											<div className="text-xs text-amber-800 dark:text-amber-200 space-y-1">
												...
												<p>
													FintekPro acts as an introducing agent. Securities are
													held by Alpaca Securities LLC, a member of FINRA/SIPC.
												</p>
												<p>
													By clicking "Confirm", you authorize this trade and
													acknowledge that execution is subject to market
													availability.
												</p>
											</div>
										</div>
