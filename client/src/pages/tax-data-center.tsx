...
const DATA_SOURCES = {
	...
	ais: {
		...
		description:
			"Income details from all sources including salary, interest, dividends",
		icon: Receipt,
		status: "available",
	},
	cams: {
...
export default function TaxDataCenter() {
	...
	const ocrParseMutation = useMutation({
		mutationFn: async ({
			file,
			docType,
		}: { file: File; docType: "form16" | "form26as" | "auto" }) => {
	...
	const completionPercentage =
		dataSources && Array.isArray(dataSources)
			? (dataSources.filter((s: DataSource) => s.status === "connected")
					.length /
					dataSources.length) *
				100
			: 0;
				...
				<>
						...
						<CardContent>
							<div className="space-y-4">
								...
								<div className="flex gap-2">
									...
									>
										<RefreshCw
											className={`h-4 w-4 mr-2 ${syncDataMutation.isPending ? "animate-spin" : ""}`}
										/>
										Sync All Sources
									</Button>
									<Button
					...
					{taxSummary && (
						<Card>
							<CardHeader>
								...
								<CardDescription>
									Consolidated view of all your income sources
								</CardDescription>
							</CardHeader>
							<CardContent>
								<div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
									<div className="text-center">
										<div className="text-2xl font-bold text-green-600">
											₹
											{((taxSummary as any)?.totalIncome || 0).toLocaleString()}
										</div>
									...
									<div className="text-center">
										<div className="text-2xl font-bold text-blue-600">
											₹{((taxSummary as any)?.totalTDS || 0).toLocaleString()}
										</div>
										<div className="text-sm text-muted-foreground">
											Total TDS
										</div>
									...
									<div className="text-center">
										<div className="text-2xl font-bold text-purple-600">
											₹
											{(
												(taxSummary as any)?.salaryIncome || 0
											).toLocaleString()}
										</div>
										<div className="text-sm text-muted-foreground">
											Salary Income
									...
									<div className="text-center">
										<div className="text-2xl font-bold text-orange-600">
											₹
											{(
												(taxSummary as any)?.capitalGains || 0
											).toLocaleString()}
										</div>
										<div className="text-sm text-muted-foreground">
											Capital Gains
									...
									<div className="text-center">
										<div className="text-2xl font-bold text-indigo-600">
											₹
											{(
												(taxSummary as any)?.dividendIncome || 0
											).toLocaleString()}
										</div>
										<div className="text-sm text-muted-foreground">
											Dividend Income
									...
									<div className="text-center">
										<div className="text-2xl font-bold text-teal-600">
											₹
											{(
												(taxSummary as any)?.interestIncome || 0
											).toLocaleString()}
										</div>
										<div className="text-sm text-muted-foreground">
											Interest Income
					...
					<Tabs defaultValue="sources" className="space-y-4">
						...
						<TabsContent value="sources" className="space-y-4">
							<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
								{Object.entries(DATA_SOURCES).map(([id, source]) => {
									const sourceData = Array.isArray(dataSources)
										? dataSources.find((s: DataSource) => s.id === id)
										: undefined;
						...
						<TabsContent value="ocr" className="space-y-4">
							...
							<Alert
								className={
									ocrStatus?.available ? "border-green-500" : "border-amber-500"
								}
							>
							...
							<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
								...
								<Card>
									<CardHeader>
										...
										<CardDescription>
											Upload Form 16 or Form 26AS PDF to automatically extract
											tax data
										</CardDescription>
									</CardHeader>
									<CardContent className="space-y-4">
										...
										<div className="space-y-2">
											...
											<div className="flex gap-2">
												<Button
													variant={
														documentType === "auto" ? "default" : "outline"
													}
													size="sm"
												...
												<Button
													variant={
														documentType === "form16" ? "default" : "outline"
													}
													size="sm"
												...
												<Button
													variant={
														documentType === "form26as" ? "default" : "outline"
													}
													size="sm"
									...
									<CardContent>
										{!ocrResult && !ocrParseMutation.isPending && (
											<div className="text-center py-8 text-muted-foreground">
												...
												<p>
													Upload and parse a document to see extracted data here
												</p>
											</div>
										)}
										...
										{ocrResult?.success && (
											<div className="space-y-4">
														...
														<div className="space-y-3">
															<div className="grid grid-cols-2 gap-2 text-sm">
																...
																<div className="font-medium">
																	{ocrResult.data.pan || "N/A"}
																</div>
																<div className="text-muted-foreground">
																	Employer:
																</div>
																<div className="font-medium">
																	{ocrResult.data.employerName || "N/A"}
																</div>
																<div className="text-muted-foreground">
																	Assessment Year:
																</div>
																<div className="font-medium">
																	{ocrResult.data.assessmentYear || "N/A"}
																</div>
															</div>
															<div className="border-t pt-3 space-y-2">
																...
																<div className="flex justify-between">
																	...
																	<span className="font-medium text-orange-600">
																		₹
																		{(
																			ocrResult.data.totalDeductions || 0
																		).toLocaleString()}
																	</span>
																</div>
																<div className="flex justify-between">
																	...
																	<span className="font-medium">
																		₹
																		{(
																			ocrResult.data.taxableIncome || 0
																		).toLocaleString()}
																	</span>
																</div>
												...
												{ocrResult.documentType === "form26as" &&
													ocrResult.data && (
														<div className="space-y-3">
															<div className="grid grid-cols-2 gap-2 text-sm">
																<div className="text-muted-foreground">
															...
															<div className="border-t pt-3">
																<div className="flex justify-between mb-3">
																	...
																	<span className="font-bold text-blue-600">
																		₹
																		{(
																			ocrResult.data.totalTDSDeducted || 0
																		).toLocaleString()}
																	</span>
																</div>
																{ocrResult.data.tdsEntries &&
																	ocrResult.data.tdsEntries.length > 0 && (
																		<div className="space-y-2">
																			<p className="text-sm font-medium">
																				TDS Entries (
																				{ocrResult.data.tdsEntries.length}):
																			</p>
										...
										{ocrResult && !ocrResult.success && (
											<Alert variant="destructive">
												...
												<AlertDescription>
													{ocrResult.message ||
														"Could not extract data from the document. Please ensure the PDF is a valid tax document."}
												</AlertDescription>
											</Alert>
										)}
									</CardContent>
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
								<Card>
									<CardHeader>
										...
										<CardDescription>
											Complete income summary from all sources
										</CardDescription>
									</CardHeader>
									<CardContent>
										<div className="space-y-3">
								...
								<Card>
									<CardHeader>
										...
										<CardDescription>
											Detailed reports by data source
										</CardDescription>
									</CardHeader>
									<CardContent>
										<div className="space-y-2">
											<Button
												...
												variant="outline"
												className="w-full"
												data-testid="button-form26as-report"
											>
												Form 26AS Report
											</Button>
											<Button
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
								<Card>
									<CardHeader>
										...
										<CardDescription>
											Download data in formats compatible with popular ITR
											platforms
										</CardDescription>
									</CardHeader>
									<CardContent>
										<div className="space-y-3">
											<Button
												className="w-full"
												data-testid="button-export-cleartax"
											>
												Export for ClearTax
											</Button>
											<Button
												variant="outline"
												className="w-full"
												data-testid="button-export-incometax"
											>
								...
								<Card>
									<CardHeader>
										...
										<CardDescription>
											Download complete datasets for custom processing
										</CardDescription>
									</CardHeader>
									<CardContent>
										<div className="space-y-3">
