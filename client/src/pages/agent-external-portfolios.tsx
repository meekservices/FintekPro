...
export default function AgentExternalPortfolios() {
	...
	const [selectedClient, setSelectedClient] = useState<ExternalHolding | null>(
		null,
	);
	...
	const isAgent =
		user?.roles?.includes("agent") || user?.roles?.includes("partner");
	...
	const initiateCobMutation = useMutation({
		...
		onSuccess: () => {
			toast({
				...
				description:
					"Change of Broker request has been submitted successfully.",
			});
		...
		<div className="container mx-auto px-4 py-6 max-w-7xl space-y-6">
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-muted-foreground">
						Manage client holdings from external brokers and initiate COB
						requests
					</p>
				</div>
			</div>
				...
				<CardHeader>
					<div className="flex items-center justify-between">
						<div>
							...
							<CardDescription>
								Holdings held with other brokers eligible for COB
							</CardDescription>
						</div>
						...
						<TableBody>
							{filteredHoldings.map((holding) => (
								<TableRow key={holding.id}>
									...
									<TableCell className="text-right">
										<Dialog
											open={cobDialogOpen && selectedClient?.id === holding.id}
											onOpenChange={(open) => {
