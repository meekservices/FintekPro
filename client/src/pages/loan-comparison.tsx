...
export default function LoanComparison() {
	...
	const [comparisonCriteria, setComparisonCriteria] =
		useState<ComparisonCriteria>({
			interestRate: 30,
			processingFee: 20,
			totalCost: 25,
	...
	const [sortBy, setSortBy] = useState<
		"bestOffer" | "lowestEMI" | "lowestRate" | "highestApproval"
	>("bestOffer");
	...
	const {
		...
		queryFn: async () => {
			const params = new URLSearchParams({
				...
				...(comparisonParams.creditScore && {
					creditScore: comparisonParams.creditScore.toString(),
				}),
			});
	...
	const processedOffers = useMemo(() => {
		...
		const processedData = loanOffers.map((offer) => {
			...
			const emi =
				(principal * monthlyRate * (1 + monthlyRate) ** offer.tenure) /
				((1 + monthlyRate) ** offer.tenure - 1);

			const totalRepayment = emi * offer.tenure;
			const totalInterest = totalRepayment - principal;
			const totalFees =
				offer.processingFee + offer.legalCharges + offer.otherCharges;
		...
		return processedData.map((offer) => {
			...
			const normalizedRate =
				maxRate > minRate
					? (1 - (offer.interestRate - minRate) / (maxRate - minRate)) * 100
					: 100;
			const normalizedFee =
				maxFee > minFee
					? (1 - (offer.processingFee - minFee) / (maxFee - minFee)) * 100
					: 100;
			const normalizedCost =
				maxCost > minCost
					? (1 - (offer.totalCost - minCost) / (maxCost - minCost)) * 100
					: 100;
			const normalizedRating =
				maxRating > minRating
					? ((offer.providerRating - minRating) / (maxRating - minRating)) * 100
					: 100;
	...
	const filteredOffers = useMemo(() => {
		const filtered = processedOffers.filter((offer) => {
			if (filterBy.provider && offer.providerName !== filterBy.provider)
				return false;
			if (offer.interestRate > filterBy.maxRate) return false;
			if (offer.approvedAmount < filterBy.minAmount) return false;
			if (filterBy.rateType !== "all" && offer.rateType !== filterBy.rateType)
				return false;
			return true;
		});
		...
		switch (sortBy) {
			case "bestOffer":
				filtered.sort(
					(a, b) => (b.comparisonScore || 0) - (a.comparisonScore || 0),
				);
				break;
			case "lowestEMI":
	...
	const generateOffersMutation = useMutation({
		...
		onSuccess: () => {
			...
			toast({
				...
				description:
					"Fresh loan offers have been generated based on your criteria.",
			});
		},
	});
	...
	const pieChartData = useMemo(() => {
		...
		return [
			...
			{
				name: "Total Interest",
				value: bestOffer.totalInterest,
				color: "#ef4444",
			},
			{
				name: "Processing Fee",
				value: bestOffer.processingFee,
				color: "#f59e0b",
			},
			{
				name: "Other Charges",
				value: bestOffer.legalCharges + bestOffer.otherCharges,
				color: "#10b981",
			},
		];
	}, [selectedOffersData]);
				...
				<CardContent>
						...
						<div className="space-y-2">
							...
							<Select
								...
								onValueChange={(value) =>
									setComparisonParams((prev) => ({
										...prev,
										tenure: Number.parseInt(value) * 12,
									}))
								}
							>
						...
						<div className="space-y-2">
							...
							<Input
								...
								onChange={(e) =>
									setComparisonParams((prev) => ({
										...prev,
										monthlyIncome: Number.parseInt(e.target.value) || 0,
									}))
								}
					...
					<div className="flex gap-4 mt-4">
						...
						<Button
							...
							disabled={
								saveComparisonMutation.isPending || selectedOffers.length < 2
							}
			...
			<Card>
				...
				<CardContent>
					...
					<div className="mt-4 text-sm text-muted-foreground">
						Total Weight:{" "}
						{Object.values(comparisonCriteria).reduce((a, b) => a + b, 0)}%
					</div>
				</CardContent>
			</Card>
				...
				<div className="flex items-center gap-4">
					...
					>
						...
						<SelectContent>
							...
							<SelectItem value="highestApproval">
								Highest Approval Chance
							</SelectItem>
						</SelectContent>
					</Select>

					<Select
						value={filterBy.rateType}
						onValueChange={(value) =>
									...
									<CardContent className="space-y-4">
										<div className="grid grid-cols-2 gap-4">
											<div>
												...
												<Badge
													variant={
														offer.rateType === "fixed" ? "default" : "secondary"
													}
												>
													{offer.rateType}
												</Badge>
										...
										<div className="space-y-2">
											...
											<Progress
												value={offer.approvalProbability}
												className="h-2"
											/>
										</div>
					...
					{selectedOffersData.length < 2 ? (
						<Card>
							<CardContent className="text-center py-12">
								...
								<p className="text-muted-foreground">
									Choose at least 2 offers from the available offers tab to see
									detailed comparison
								</p>
							</CardContent>
						</Card>
										...
										<table className="w-full">
											<thead>
												<tr className="border-b">
													...
													{selectedOffersData.map((offer) => (
														<th
															key={offer.id}
															className="text-center py-2 px-4 min-w-32"
														>
															{offer.providerName}
														</th>
													))}
											...
											<tbody>
												{[
													...
													const values = selectedOffersData.map(
														(offer) =>
															offer[param.key as keyof LoanOffer] as
																| number
																| string,
													);
													...
													return (
														<tr key={param.key} className="border-b">
															...
															{selectedOffersData.map((offer) => {
																const value =
																	offer[param.key as keyof LoanOffer];
																...
																return (
																	<td
																		key={offer.id}
																		className={`text-center py-3 px-4 ${isBest ? "bg-green-100 dark:bg-green-900/30 font-bold text-green-800 dark:text-green-200" : ""}`}
																	>
																		{param.prefix}
																		{typeof value === "number"
					...
					{selectedOffersData.length < 2 ? (
						<Card>
							<CardContent className="text-center py-12">
								...
								<h3 className="text-lg font-medium mb-2">
									Select Offers for Analytics
								</h3>
								<p className="text-muted-foreground">
									Choose at least 2 offers to see visual comparisons and
									analytics
								</p>
							</CardContent>
						</Card>
