...
export default function AgentHniLeaderboard() {
	...
	const [sortBy, setSortBy] = useState<
		"compositeScore" | "wealthScore" | "investableSurplus" | "estimatedNetworth"
	>("compositeScore");
	...
	const { data, isLoading, refetch } = useQuery<{
		...
		queryFn: () =>
			apiRequest(`/api/agent-wizard/prospects/top-ranked?limit=${limit}`),
	});

	const scoreMutation = useMutation({
		mutationFn: () =>
			apiRequest("/api/agent-wizard/prospects/bulk-score", {
				method: "POST",
				body: JSON.stringify({}),
			}),
		onSuccess: (res: any) => {
			toast({
				...
				description: `${res.scored ?? 0} prospects rescored`,
			});
	...
	const prospects = (data?.prospects ?? [])
		.filter((p) => {
			...
			if (qualityFilter !== "all" && p.leadQuality !== qualityFilter)
				return false;
			return true;
		})
	...
	const stats = {
		...
		platinum:
			data?.prospects.filter((p) => p.scoreTier === "platinum").length ?? 0,
		...
		avgComposite: data?.prospects.length
			? Math.round(
					data.prospects.reduce((s, p) => s + p.compositeScore, 0) /
		...
		<div className="p-4 md:p-6 space-y-6">
					...
					<div className="flex flex-col sm:flex-row gap-3">
						...
						>
							...
							<SelectContent>
								...
								<SelectItem value="investableSurplus">
									Investable Surplus
								</SelectItem>
								<SelectItem value="estimatedNetworth">
									Est. Net Worth
								</SelectItem>
							</SelectContent>
						</Select>
					...
					{isLoading ? (
						...
						<div className="text-center py-16 text-muted-foreground">
							...
							<p className="text-sm mt-1">
								Click "Re-score All" to compute wealth scores
							</p>
						</div>
					) : (
						<div className="divide-y">
							{prospects.map((p, idx) => {
								...
								const quality = p.leadQuality
									? QUALITY_CONFIG[p.leadQuality]
									: null;
								...
								return (
									...
									>
										<div className="flex items-center gap-3 flex-1 min-w-0">
											<div
												className={`flex-shrink-0 w-9 h-9 rounded-full ring-2 ${tier.ring} flex items-center justify-center font-bold text-sm ${idx < 3 ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
											>
												{idx < 3 ? ["🥇", "🥈", "🥉"][idx] : idx + 1}
											</div>
											<div className="min-w-0">
												<div className="flex items-center gap-2 flex-wrap">
													...
													<Badge
														className={`text-xs py-0 px-1.5 ${tier.color}`}
													>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Composite = 30% Wealth + 20% Activity + 30% Relationship + 20%
						Financial Health
					</CardDescription>
				</CardHeader>
				<CardContent>
					<div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
						{Object.entries(TIER_CONFIG).map(([key, cfg]) => {
							...
							return (
								<div
									key={key}
									className={`rounded-lg p-3 ${cfg.color} flex items-center gap-3`}
								>
