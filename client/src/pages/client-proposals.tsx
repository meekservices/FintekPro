...
export default function ClientProposalsPage() {
	...
	const [selectedOrderType, setSelectedOrderType] = useState<"LUMPSUM" | "SIP">(
		"LUMPSUM",
	);
	...
	const handleCompleteOrder = (proposal: Proposal) => {
		if (proposal.status !== "accepted") {
			toast({
				...
				description:
					"Please accept the proposal first before completing the order.",
			});
			return;
		}
	...
	const expiredProposals = proposals.filter(
		(p) => isExpired(p.validUntil) || p.status === "expired",
	);
						...
						<div className="flex items-center justify-between">
							<div>
								...
								<p className="text-2xl font-bold">
									{formatCurrency(
										proposals.reduce((sum, p) => sum + p.totalAmount, 0),
									)}
								</p>
							</div>
										...
										<div className="space-y-4">
											...
											<div className="flex items-center justify-between">
												...
												<span className="text-sm">
													{proposal.items.length} products
												</span>
											</div>
											<div className="flex items-center justify-between">
												<span className="text-sm text-muted-foreground">
												...
												<span
													className={`text-sm ${expired ? "text-red-500" : ""}`}
												>
						...
						<div className="space-y-6">
								...
								<div>
									...
									<div className="mt-1">
										<Badge
											className={
												statusColors[
													isExpired(selectedProposal.validUntil)
														? "expired"
							...
							<div>
								<Label className="text-sm font-medium mb-3 block">
									Investment Breakdown ({selectedProposal.items.length} items)
								</Label>
												...
												<TableRow key={index}>
													...
													<TableCell>
														<div className="text-xs space-y-1">
															...
															{item.rate && (
																<div>
																	<strong>Rate:</strong> {item.rate}% p.a.
																</div>
															)}
															{item.duration && (
																<div>
						...
						<div className="space-y-4">
							...
							<div className="space-y-2">
								...
								<select
									...
									onChange={(e) =>
										setSelectedOrderType(e.target.value as "LUMPSUM" | "SIP")
									}
								...
								<div className="flex items-start gap-2">
									...
									<div className="text-sm">
										...
										<p className="text-blue-700 dark:text-blue-300 mt-1">
											Your order will be processed through BSE Star MF platform
											for secure and regulated mutual fund transactions.
										</p>
									</div>
								</div>
							</div>
