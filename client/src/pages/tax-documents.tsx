...
export default function TaxDocuments() {
  ...
  const { data: documents = [], isLoading: documentsLoading } = useQuery({
    ...
    queryFn: () => apiRequest(`/api/tax/documents?financialYear=${selectedYear}`)
  });
  ...
  const { data: calculations = [], isLoading: calculationsLoading } = useQuery({
    ...
    queryFn: () => apiRequest(`/api/tax/calculations?financialYear=${selectedYear}`)
  });

  // Upload document mutation
  const uploadMutation = useMutation({
    mutationFn: (data: any) => apiRequest('/api/tax/documents', {
      method: 'POST',
  ...
  const processMutation = useMutation({
    mutationFn: (documentId: string) => apiRequest(`/api/tax/documents/${documentId}/process`, {
      method: 'POST'
    }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/tax/documents'] });
  ...
  const generateITRMutation = useMutation({
    mutationFn: (financialYear: string) => apiRequest('/api/tax/generate-itr', {
      method: 'POST',
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      default: return 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-200';
    }
  };
  ...
  return (
    <div className="container mx-auto p-6 space-y-6" data-testid="tax-documents-page">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold" data-testid="page-title">Tax Documents & ITR Filing</h1>
          <p className="text-muted-foreground mt-2">
            Upload Form 26AS, AIS, and other tax documents to automatically generate your ITR
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card data-testid="summary-tds">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted-foreground">TDS Deducted</p>
                <p className="text-2xl font-bold">{formatCurrency(totalTDSDeducted)}</p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted-foreground">
                  {latestCalculation?.refundDue > 0 ? 'Refund Due' : 'Tax Payable'}
                </p>
                <p className="text-2xl font-bold">
                  {latestCalculation ? formatCurrency(
                    latestCalculation.refundDue > 0 ? latestCalculation.refundDue : latestCalculation.taxPayable
                  ) : '₹0'}
                </p>
              </div>
        ...
        <TabsContent value="upload" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Upload your Form 26AS, AIS, or other tax-related documents to get started
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              >
                ...
                <p className="text-lg font-medium mb-2">
                  {uploadData.fileName || 'Drop your document here or click to browse'}
                </p>
              ...
              <Button 
                ...
                disabled={uploadMutation.isPending || !uploadData.documentType || !uploadData.fileName}
                className="w-full"
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        <Badge className={getStatusColor(doc.processingStatus)}>
                          ...
                          <span className="ml-1 capitalize">{doc.processingStatus}</span>
                        </Badge>
            ...
            <CardContent>
              {taxDataLoading ? (
                ...
                <Alert>
                  ...
                  <AlertDescription>
                    No processed tax data available. Upload and process documents to see analysis.
                  </AlertDescription>
                </Alert>
              ) : (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {Object.entries(
                        taxData.reduce((acc: any, item: StructuredTaxData) => {
                          ...
                          acc[key] = (acc[key] || 0) + (item.taxableAmount || 0);
                          return acc;
                        }, {})
                      ).map(([source, amount]) => (
                  ...
                  <div>
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full border-collapse border border-border">
                        ...
                        <tbody>
                          {taxData
                            .filter((item: StructuredTaxData) => item.dataType === 'TDS')
                            .slice(0, 10)
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Calculate your tax for {selectedYear} under different tax regimes
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                >
                  ...
                  {generateITRMutation.isPending ? 'Generating...' : 'Generate ITR JSON'}
                </Button>
              </CardContent>
            </Card>
              ...
              <CardContent>
                {calculationsLoading ? (
                  ...
                  <Alert>
                    <AlertCircle className="h-4 w-4" />
                    <AlertDescription>
                      No tax calculations available. Calculate your tax liability to see results.
                    </AlertDescription>
                  </Alert>
                ) : (
                      ...
                      <div key={calc.id} className="border rounded-lg p-4">
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm">
                          <div>
                            ...
                            <p className="font-semibold">{formatCurrency(calc.totalIncome)}</p>
                          </div>
                          <div>
                            <p className="text-muted-foreground">Taxable Income</p>
                            <p className="font-semibold">{formatCurrency(calc.taxableIncome)}</p>
                          </div>
                          <div>
                            <p className="text-muted-foreground">Tax Liability</p>
                            <p className="font-semibold">{formatCurrency(calc.totalTaxPayable)}</p>
                          </div>
                          <div>
                            ...
                            <p className={`font-semibold ${
                              calc.refundDue > 0 ? 'text-green-600' : 'text-red-600'
                            }`}>
                              {formatCurrency(calc.refundDue > 0 ? calc.refundDue : calc.taxPayable)}
                            </p>
                          </div>
                        </div>
                      </div>
