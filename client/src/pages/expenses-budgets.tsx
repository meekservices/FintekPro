...
export default function ExpensesBudgetsPage() {
	...
	const generateInsightsMutation = useMutation({
		mutationFn: async () =>
			apiRequest("/api/insights/generate", { method: "POST" }),
		onSuccess: () => {
	...
	const handleBudgetSubmit = (data: any) => {
		createBudgetMutation.mutate({
			...data,
			budgetAmount: Number.parseFloat(data.budgetAmount),
			alertThreshold: data.alertThreshold
				? Number.parseFloat(data.alertThreshold)
				: undefined,
		});
	};
	...
	const totalSpent = (expenses as any[]).reduce(
		(sum: number, e: any) => sum + Number.parseFloat(e.amount || 0),
		0,
	);
	const totalBudget = (budgets as any[]).reduce(
		(sum: number, b: any) => sum + Number.parseFloat(b.budgetAmount || 0),
		0,
	);
			...
			<div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
				<div>
					...
					<p className="text-muted-foreground">
						Track your spending with AI-powered insights
					</p>
				</div>
				<div className="flex gap-2">
								...
								>
									...
									<FormField
										...
										render={({ field }) => (
											<FormItem>
												<FormLabel>
													Category (AI will suggest if empty)
												</FormLabel>
												<Select
													value={field.value}
					...
					<Dialog open={budgetDialogOpen} onOpenChange={setBudgetDialogOpen}>
						...
						<DialogContent className="max-w-md">
							...
							<Form {...budgetForm}>
								<form
									onSubmit={budgetForm.handleSubmit(handleBudgetSubmit)}
									className="space-y-4"
								>
									<FormField
				...
				<Card>
					...
					<CardContent>
						...
						<p className="text-xs text-muted-foreground">
							{(budgets as any[]).length} active budgets
						</p>
					</CardContent>
				</Card>
				<Card>
					...
					<CardContent>
						<div
							className={`text-2xl font-bold ${totalBudget - totalSpent < 0 ? "text-red-500" : ""}`}
						...
						<CardHeader>
							<div className="flex justify-between items-center">
								<div>
									...
									<CardDescription>
										{(filteredExpenses as any[]).length} transactions
									</CardDescription>
								</div>
								<Select
											...
											<div className="flex-1">
												<div className="flex items-center gap-2">
													<h4
														...
														data-testid={`text-expense-description-${expense.id}`}
													>
														{expense.description}
													</h4>
					...
					{budgetsLoading ? (
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
							{(budgets as any[]).map((budget: any) => {
								const spent = Number.parseFloat(budget.currentSpend || 0);
								const total = Number.parseFloat(budget.budgetAmount);
								const percentage = Math.min((spent / total) * 100, 100);
								const alertThreshold = budget.alertThreshold
									? Number.parseFloat(budget.alertThreshold)
									: 80;
										...
										<CardHeader>
											<div className="flex justify-between items-start">
												<div>
													...
													<CardDescription
														data-testid={`text-budget-category-${budget.id}`}
													>
														{(budget.category || "general")
															.replace("_", " ")
															.toUpperCase()}{" "}
														• {(budget.period || "monthly").toUpperCase()}
													</CardDescription>
												</div>
												{isOverBudget && (
										...
										<CardContent className="space-y-2">
											<div className="flex justify-between text-sm">
												...
												<span
													...
													data-testid={`text-budget-spent-${budget.id}`}
												>
													₹
											...
											<div className="flex justify-between text-sm text-muted-foreground">
												...
												<span data-testid={`text-budget-total-${budget.id}`}>
													of ₹
										...
										<Card
											...
											className={`${
												insight.priority === "high"
													? "border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-950/30/50"
													: insight.priority === "medium"
														? "border-orange-200 dark:border-orange-800 bg-orange-50 dark:bg-orange-950/30/50"
											...
											<CardHeader>
												<div className="flex items-start justify-between">
													<div className="flex-1">
														...
														<CardDescription
															className="mt-1"
															data-testid={`text-insight-description-${insight.id}`}
														>
															{insight.description}
														</CardDescription>
													</div>
