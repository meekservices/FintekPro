...
export default function InvestableSurplusPage() {
  ...
  const addIncomeMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/surplus/summary', userId] });
      setIncomeDialogOpen(false);
      toast({ title: "Income stream added", description: "Your income has been recorded." });
    },
    onError: (error: any) => {
  ...
  const addObligationMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "Obligation added", description: "Your obligation has been recorded." });
    },
    onError: (error: any) => {
      toast({ title: "Error", description: error.message, variant: "destructive" });
    },
  ...
  const updateEmergencyFundMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "Emergency fund updated", description: "Your emergency fund details have been saved." });
    },
    onError: (error: any) => {
      toast({ title: "Error", description: error.message, variant: "destructive" });
    },
  ...
  const recalculateMutation = useMutation({
    mutationFn: async () => {
      return apiRequest(`/api/surplus/surplus/assess/${userId}`, { method: 'POST' });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/surplus/summary', userId] });
      toast({ title: "Assessment complete", description: "Your investable surplus has been recalculated." });
    },
    onError: (error: any) => {
      toast({ title: "Error", description: error.message, variant: "destructive" });
    },
  ...
  const surplusFlowData = [
    ...
    { name: 'Tax Deductions', amount: (liveCalc?.totalGrossIncome || 0) - (liveCalc?.totalNetIncome || 0) },
    { name: 'Obligations', amount: liveCalc?.totalObligations || 0 },
    { name: 'Emergency Buffer', amount: liveCalc?.emergencyBufferAmount || 0 },
    { name: 'Investable Surplus', amount: liveCalc?.annualInvestableSurplus || 0 },
  ];
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold" data-testid="page-title">Investable Surplus Engine</h1>
          <p className="text-muted-foreground">Calculate your real investable capacity based on income, obligations, and emergency reserves.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card data-testid="card-net-income">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">{formatCurrency(liveCalc?.totalNetIncome || 0)}</div>
        ...
        <Card data-testid="card-obligations">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-medium">Annual Obligations</CardTitle>
            <CreditCard className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-red-600">{formatCurrency(liveCalc?.totalObligations || 0)}</div>
            <p className="text-xs text-muted-foreground">EMIs, rent, insurance, etc.</p>
          </CardContent>
        </Card>

        <Card data-testid="card-surplus">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-medium">Monthly Investable Surplus</CardTitle>
            <Wallet className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-blue-600">{formatCurrency(liveCalc?.monthlyInvestableSurplus || 0)}</div>
        ...
        <Card data-testid="card-segment">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-medium">Client Segment</CardTitle>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {SEGMENT_DESCRIPTIONS[segment?.segment as keyof typeof SEGMENT_DESCRIPTIONS] || 'Retail Investor'}
            </p>
          </CardContent>
        </Card>
      </div>
        ...
        <Alert variant={emergencyFund.status === 'adequate' ? 'default' : 'destructive'} data-testid="alert-emergency-fund">
          ...
          <AlertDescription className="flex items-center justify-between">
            <span>
              ...
              Current: {formatCurrency(emergencyFund.current)} | 
              Required: {formatCurrency(emergencyFund.required)}
            </span>
            <Dialog open={emergencyDialogOpen} onOpenChange={setEmergencyDialogOpen}>
              <DialogTrigger asChild>
                <Button variant="outline" size="sm" data-testid="button-update-emergency">
              ...
              <DialogContent>
                ...
                <Form {...emergencyForm}>
                  <form onSubmit={emergencyForm.handleSubmit((data) => updateEmergencyFundMutation.mutate(data))} className="space-y-4">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input type="number" placeholder="50000" {...field} data-testid="input-monthly-expenses" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input type="number" placeholder="300000" {...field} data-testid="input-emergency-fund" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input placeholder="HDFC Savings Account" {...field} data-testid="input-fund-location" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    />
                    <DialogFooter>
                      <Button type="submit" disabled={updateEmergencyFundMutation.isPending} data-testid="button-save-emergency">
                        Save Changes
                      </Button>
                    </DialogFooter>
                  </form>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="grid w-full grid-cols-5">
          ...
          <TabsTrigger value="eligibility" data-testid="tab-eligibility">Product Eligibility</TabsTrigger>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>How reliable is your investable surplus?</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="flex items-center justify-between">
                  <span>Stability Rating</span>
                  <Badge variant={
                    liveCalc?.surplusStability === 'stable' ? 'default' :
                    liveCalc?.surplusStability === 'moderate' ? 'secondary' : 'destructive'
                  }>
                ...
                <div className="grid grid-cols-2 gap-4 pt-4">
                  <div className="text-center p-3 bg-muted rounded-lg">
                    ...
                    <div className="text-xs text-muted-foreground">Monthly SIP Capacity</div>
                  </div>
                  <div className="text-center p-3 bg-muted rounded-lg">
                    <div className="text-2xl font-bold text-blue-600">
                      {formatCurrency(liveCalc?.annualInvestableSurplus || 0)}
                    </div>
                    <div className="text-xs text-muted-foreground">Annual Lumpsum</div>
                  </div>
                </div>
              </CardContent>
            </Card>
          ...
          <div className="flex justify-between items-center">
            ...
            <Dialog open={incomeDialogOpen} onOpenChange={setIncomeDialogOpen}>
              ...
              <DialogContent>
                ...
                <Form {...incomeForm}>
                  <form onSubmit={incomeForm.handleSubmit((data) => addIncomeMutation.mutate(data))} className="space-y-4">
                    <FormField
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input placeholder="e.g., TCS Ltd" {...field} data-testid="input-source-name" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl>
                              <Input type="number" placeholder="100000" {...field} data-testid="input-gross-amount" />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl>
                              <Input type="number" placeholder="80000" {...field} data-testid="input-net-amount" />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                    ...
                    />
                    <DialogFooter>
                      <Button type="submit" disabled={addIncomeMutation.isPending} data-testid="button-save-income">
                        Add Income
                      </Button>
                    </DialogFooter>
                  </form>
            ...
            {summary?.incomeStreams?.length > 0 ? (
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {INCOME_TYPES.find(t => t.value === stream.incomeType)?.label} • {stream.frequency}
                        </p>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="font-bold text-green-600">{formatCurrency(parseFloat(stream.netAmount || "0"))}</div>
                      <div className="text-xs text-muted-foreground">Net ({stream.frequency})</div>
                    </div>
                  </CardContent>
                </Card>
              ))
              ...
              <Card>
                <CardContent className="py-12 text-center">
                  ...
                  <p className="text-sm text-muted-foreground mb-4">Add your income sources to calculate investable surplus</p>
                  <Button onClick={() => setIncomeDialogOpen(true)} data-testid="button-add-first-income">
          ...
          <div className="flex justify-between items-center">
            ...
            <Dialog open={obligationDialogOpen} onOpenChange={setObligationDialogOpen}>
              ...
              <DialogContent>
                ...
                <Form {...obligationForm}>
                  <form onSubmit={obligationForm.handleSubmit((data) => addObligationMutation.mutate(data))} className="space-y-4">
                    <FormField
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input placeholder="e.g., HDFC Bank" {...field} data-testid="input-creditor-name" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input type="number" placeholder="25000" {...field} data-testid="input-monthly-amount" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>Total Outstanding (₹) - Optional</FormLabel>
                          <FormControl>
                            <Input type="number" placeholder="2500000" {...field} data-testid="input-outstanding" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    />
                    <DialogFooter>
                      <Button type="submit" disabled={addObligationMutation.isPending} data-testid="button-save-obligation">
                        Add Obligation
                      </Button>
                    </DialogFooter>
                  </form>
            ...
            {summary?.obligations?.length > 0 ? (
                ...
                <Card key={obligation.id} data-testid={`card-obligation-${obligation.id}`}>
                  <CardContent className="flex items-center justify-between py-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h4 className="font-medium">{obligation.creditorName}</h4>
                        <p className="text-sm text-muted-foreground">
                          {OBLIGATION_TYPES.find(t => t.value === obligation.obligationType)?.label}
                        </p>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="font-bold text-red-600">{formatCurrency(parseFloat(obligation.monthlyAmount || "0"))}</div>
              ...
              <Card>
                <CardContent className="py-12 text-center">
                  ...
                  <p className="text-sm text-muted-foreground mb-4">Add your EMIs, rent, and other financial commitments</p>
                  <Button onClick={() => setObligationDialogOpen(true)} data-testid="button-add-first-obligation">
        ...
        <TabsContent value="eligibility" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Your segment: {SEGMENT_DESCRIPTIONS[segment?.segment as keyof typeof SEGMENT_DESCRIPTIONS] || 'Retail Investor'}
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-6">
                <div>
                  <h4 className="font-medium mb-3 flex items-center gap-2">
                    <CheckCircle className="h-4 w-4 text-green-500" /> Eligible Products
                  </h4>
                  <div className="flex flex-wrap gap-2">
                    {segment?.eligibleProducts?.map((product: string) => (
                      ...
                      <span className="text-muted-foreground">All retail products</span>
                    )}
                  </div>
                </div>
                ...
                <div>
                  <h4 className="font-medium mb-3 flex items-center gap-2">
                    <AlertTriangle className="h-4 w-4 text-amber-500" /> Restricted Products
                  </h4>
                  <div className="flex flex-wrap gap-2">
                ...
                {segment?.investmentCaps && Object.values(segment.investmentCaps).some((v: any) => v !== null) && (
                  <div>
                    <h4 className="font-medium mb-3 flex items-center gap-2">
                      <Landmark className="h-4 w-4 text-blue-500" /> Investment Caps
                    </h4>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {Object.entries(segment.investmentCaps as Record<string, number | null>)
                        .filter(([_, value]) => value !== null)
                        .map(([key, value]) => (
                          <div key={key} className="p-3 bg-muted rounded-lg text-center">
                            <div className="text-lg font-bold">{formatCurrency(value as number)}</div>
                            <div className="text-xs text-muted-foreground">{key.toUpperCase()} min</div>
                          </div>
                        ))}
                    </div>
                  </div>
          ...
          <div className="grid gap-4">
              ...
              <Card>
                ...
                <CardContent>
                  <ul className="space-y-2">
                    {liveCalc.recommendations.immediate.map((rec: string, i: number) => (
            ...
            {liveCalc?.recommendations?.shortTerm?.length > 0 && (
              <Card>
                ...
                <CardContent>
                  <ul className="space-y-2">
                    {liveCalc.recommendations.shortTerm.map((rec: string, i: number) => (
                      <li key={i} className="flex items-start gap-2">
            ...
            {liveCalc?.recommendations?.longTerm?.length > 0 && (
              <Card>
                ...
                <CardContent>
                  <ul className="space-y-2">
                    {liveCalc.recommendations.longTerm.map((rec: string, i: number) => (
                      <li key={i} className="flex items-start gap-2">
            ...
            {(!liveCalc?.recommendations?.immediate?.length && 
              ...
              <Card>
                <CardContent className="py-12 text-center">
                  ...
                  <h3 className="font-medium mb-2">Add your financial data</h3>
                  <p className="text-sm text-muted-foreground">
                    Add income streams and obligations to get personalized recommendations
                  </p>
                </CardContent>
              </Card>
            )}
