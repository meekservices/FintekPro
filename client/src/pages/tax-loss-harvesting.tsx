...
const mockSwapSuggestions: SwapSuggestion[] = [
	{
		sellSymbol: "TATAMOTORS",
		sellName: "Tata Motors",
		buySymbol: "MARUTI",
		buyName: "Maruti Suzuki",
		taxSavings: 3150,
		correlation: 0.85,
		reason: "Similar sector exposure with stronger fundamentals",
	},
	{
		sellSymbol: "TATASTEEL",
		sellName: "Tata Steel",
		buySymbol: "JSWSTEEL",
		buyName: "JSW Steel",
		taxSavings: 400,
		correlation: 0.92,
		reason: "High correlation, maintains metals exposure",
	},
	{
		sellSymbol: "BHARTIARTL",
		sellName: "Bharti Airtel",
		buySymbol: "RELIANCE",
		buyName: "Reliance (Jio)",
		taxSavings: 1050,
		correlation: 0.78,
		reason: "Telecom exposure via diversified conglomerate",
	},
];
...
export default function TaxLossHarvesting() {
	...
	const totalUnrealizedLoss = mockLossPositions.reduce(
		(sum, p) => sum + p.unrealizedLoss,
		0,
	);
	const shortTermLosses = mockLossPositions
		.filter((p) => p.holdingPeriod === "short_term")
		.reduce((sum, p) => sum + p.unrealizedLoss, 0);
	const longTermLosses = mockLossPositions
		.filter((p) => p.holdingPeriod === "long_term")
		.reduce((sum, p) => sum + p.unrealizedLoss, 0);

	const potentialTaxSavings = useMemo(() => {
		const shortTermRate = 0.3;
		const longTermRate = 0.1;
		return (
			Math.abs(shortTermLosses * shortTermRate) +
			Math.abs(longTermLosses * longTermRate)
		);
				...
				<Card className="text-center">
					<CardContent className="pt-6">
						...
						<p className="text-muted-foreground mb-4">
							Please log in to access tax loss harvesting tools.
						</p>
						<Link href="/auth">
	...
	return (
		<div
			...
			data-testid="tax-loss-harvesting-page"
		>
			...
			<Alert className="bg-blue-50 dark:bg-blue-950 border-blue-200 dark:border-blue-800">
				...
				<AlertDescription className="text-blue-700 dark:text-blue-300">
					...
					capital gains taxes. Our AI suggests similar investments to maintain
					your portfolio allocation while capturing the tax benefit.
				</AlertDescription>
			</Alert>
				...
				<TabsContent value="positions" className="space-y-4">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Select positions to analyze for tax-loss harvesting
								opportunities
							</CardDescription>
						</CardHeader>
						<CardContent>
										...
										<div className="flex items-center gap-6">
											...
											<div className="text-center">
												...
												<p className="font-bold text-red-600">
													{formatCurrency(position.unrealizedLoss)}
												</p>
											...
											<Badge
												variant={
													position.holdingPeriod === "short_term"
														? "destructive"
														: "secondary"
												}
											>
					...
					{showResults ? (
						<div className="space-y-4">
							<Alert className="bg-green-50 dark:bg-green-950 border-green-200 dark:border-green-800">
								...
								<AlertDescription className="text-green-700 dark:text-green-300">
									Found {mockSwapSuggestions.length} tax-efficient swap
									opportunities that maintain your portfolio exposure.
								</AlertDescription>
							</Alert>
											...
											<div className="flex items-center gap-6">
												<div className="text-center">
													...
													<p className="font-bold">
														{(suggestion.correlation * 100).toFixed(0)}%
													</p>
												</div>
												<div className="text-center">
													<p className="text-sm text-muted-foreground">
													...
													<p className="font-bold text-green-600">
														{formatCurrency(suggestion.taxSavings)}
													</p>
												</div>
							...
							<div className="text-center text-sm text-muted-foreground">
								<p>
									Note: Wash sale rules apply. Wait 30 days before repurchasing
									the same security.
								</p>
							</div>
						</div>
						...
						<Card>
							<CardContent className="pt-12 pb-12 text-center">
								...
								<p className="text-muted-foreground mb-4">
									Select positions from the Loss Positions tab and run the AI
									analysis
								</p>
							</CardContent>
						</Card>
