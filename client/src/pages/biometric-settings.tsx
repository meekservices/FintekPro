...
export default function BiometricSettings() {
	...
	const renameMutation = useMutation({
		mutationFn: ({ id, name }: { id: string; name: string }) =>
			apiRequest(`/api/webauthn/credentials/${id}`, {
				method: "PATCH",
		...
		try {
			...
			if (!platformAvailable) {
				toast({
					title: "Not Supported",
					description:
						"Your device does not have a platform authenticator (Face ID / Fingerprint) available.",
					variant: "destructive",
				});
				return;
			}

			const optionsRes = await apiRequest("/api/webauthn/register/options", {
				method: "POST",
			});
			...
			const createOptions: CredentialCreationOptions = {
				publicKey: {
					...
					excludeCredentials: (optionsRes.excludeCredentials ?? []).map(
						(c: any) => ({
							...c,
			...
			toast({
				...
				description: "Your device biometric has been registered successfully.",
			});
			setEnrollOpen(false);
		} catch (err: any) {
			if (err?.name === "NotAllowedError") {
				...
				toast({
					...
					description: err.message || "Could not enroll biometric.",
					variant: "destructive",
				});
			}
		} finally {
		...
		try {
			...
			const getOptions: CredentialRequestOptions = {
				publicKey: {
					...optionsRes,
					challenge: base64urlToUint8Array(optionsRes.challenge),
					allowCredentials: (optionsRes.allowCredentials ?? []).map(
						(c: any) => ({
							...c,
							id: base64urlToUint8Array(c.id),
						}),
			...
			if (err?.name === "NotAllowedError") {
				toast({
					title: "Cancelled",
					description: "Biometric test was cancelled.",
					variant: "destructive",
				});
			} else {
				toast({
					title: "Test Failed",
					description: err.message || "Biometric verification failed.",
					variant: "destructive",
				});
			}
		} finally {
		...
		<div className="container max-w-3xl mx-auto py-8 px-4 space-y-6">
			...
			{!isWebAuthnSupported() && (
				<Alert variant="destructive">
					...
					<AlertDescription>
						Your browser does not support biometric authentication. Please use a
						modern browser on a compatible device.
					</AlertDescription>
				</Alert>
			)}
			...
			<Tabs defaultValue="devices">
				<ScrollableTabsList className="grid w-full grid-cols-2">
					<TabsTrigger value="devices">
						Enrolled Devices ({credentials.length})
					</TabsTrigger>
						...
						<Card>
							<CardContent className="py-12 text-center text-muted-foreground">
								...
								<p className="text-sm mt-1">
									Add your device to enable Face ID or Fingerprint login
								</p>
							</CardContent>
						</Card>
					) : (
								...
								<CardContent className="py-4 flex items-center gap-4">
									...
									<div className="flex-1 min-w-0">
										...
										<div className="text-xs text-muted-foreground flex flex-wrap gap-2 mt-1">
											<span>
												Added{" "}
												{formatDistanceToNow(new Date(cred.createdAt), {
													addSuffix: true,
												})}
											</span>
											{cred.lastUsedAt && (
												<span>
													· Last used{" "}
													{formatDistanceToNow(new Date(cred.lastUsedAt), {
														addSuffix: true,
													})}
												</span>
											)}
			...
			<Dialog open={enrollOpen} onOpenChange={setEnrollOpen}>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							Your device will prompt you to use Face ID, Fingerprint, or PIN.
							Your biometric data never leaves your device — only a
							cryptographic key is stored securely.
						</DialogDescription>
					</DialogHeader>
					<div className="bg-blue-50 dark:bg-blue-950 rounded-lg p-4 text-sm space-y-2">
						<div className="font-medium text-blue-700 dark:text-blue-300">
							SEBI-compliant security
						</div>
			...
			<Dialog open={renameOpen} onOpenChange={setRenameOpen}>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							Give this device a recognisable name.
						</DialogDescription>
					</DialogHeader>
					<Input
						value={newName}
						...
						<AlertDialogDescription>
							...
							<strong>
								{deleteTarget?.deviceName ||
									deleteTarget?.deviceType ||
									"this device"}
							</strong>
							. You will no longer be able to sign in using this device's
							biometric.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
