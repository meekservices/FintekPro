...
export default function BBPSPage() {
			...
			const verifyPaymentStatus = async () => {
				try {
					const response = await fetch(
						`/api/bbps/transactions/${transactionId}/status`,
					);
	...
	const payBillMutation = useMutation({
		mutationFn: async (data: { billId: string; paymentAmount: string }) => {
			...
			if (!response.ok) {
				...
				throw new Error(
					errorData.details || errorData.error || "Failed to process payment",
				);
			}
		...
		onSuccess: (data) => {
			if (data.success && data.paymentUrl) {
				...
				toast({
					...
					description:
						"Please complete your payment on the secure payment page.",
				});
	...
	const formatCurrency = (amount: string) => {
		const numAmount = Number.parseFloat(amount) / 100; // Convert paise to rupees
		...
		<div className="container mx-auto p-6" data-testid="bbps-page">
			<div className="mb-6">
				...
				<p className="text-muted-foreground" data-testid="page-description">
					Pay your utility bills, mobile recharges, and more through India's
			...
			<Tabs defaultValue="pay-bills" className="w-full" data-testid="bbps-tabs">
				...
				>
					<TabsTrigger value="pay-bills" data-testid="tab-pay-bills">
						Pay Bills
					</TabsTrigger>
				...
				<TabsContent
					value="pay-bills"
					className="space-y-6"
					data-testid="tab-content-pay-bills"
				>
						...
						<Card data-testid="card-categories">
							<CardHeader>
								<CardTitle
									...
									data-testid="card-title-categories"
								>
							...
							<CardContent className="space-y-4">
								<div className="grid gap-3" data-testid="category-grid">
									{categories.map((category) => {
										const IconComponent =
											categoryIcons[category.categoryCode] || IndianRupee;
										return (
											<Button
												key={category.id}
												variant={
								...
								{selectedCategory && (
									<>
										<div className="space-y-2">
											<Label htmlFor="biller-select" data-testid="label-biller">
												Service Provider
											</Label>
											<Select
												...
												<SelectContent>
													{billers.map((biller) => (
														<SelectItem
															key={biller.id}
															value={biller.id}
															data-testid={`biller-option-${biller.billerCode}`}
														>
															<div>
																<div className="font-medium">
																	{biller.billerName}
										...
										{selectedBiller && (
											<div className="space-y-2">
												<Label
													htmlFor="customer-param"
													data-testid="label-customer-param"
												>
												...
												<Button
													...
													disabled={
														fetchBillMutation.isPending || !customerParam
													}
													className="w-full"
					...
					{fetchedBill && (
						<Card data-testid="card-bill-details">
							<CardHeader>
								<CardTitle
									className="flex items-center gap-2"
									data-testid="card-title-bill-details"
								>
								...
								{fetchedBill.billFetchStatus === "SUCCESS" ? (
									<>
										<div
											...
											data-testid="bill-info-grid"
										>
											<div className="space-y-2">
												<Label data-testid="label-customer-info">
												...
												>
													<div className="text-sm">
														...
														{fetchedBill.customerParam}
													</div>
													{fetchedBill.billData && (
														<>
															{JSON.parse(fetchedBill.billData)
																.customerName && (
																<div className="text-sm">
																	<strong>Name:</strong>{" "}
																	{
											...
											<div className="space-y-2">
												<Label data-testid="label-bill-info">
													Bill Information
												</Label>
												<div
													className="p-3 bg-muted rounded-lg"
													data-testid="bill-info"
												>
													<div className="text-sm">
														<strong>Amount:</strong>{" "}
														{formatCurrency(fetchedBill.billAmount || "0")}
													</div>
													<div className="text-sm">
										...
										<div className="space-y-4">
											<Alert className="bg-blue-50 border-blue-200 dark:bg-blue-950 dark:border-blue-800">
												<AlertDescription className="text-sm">
													You'll be redirected to Cashfree payment gateway to
													complete your payment securely using UPI, Net Banking,
											...
											>
												...
												{payBillMutation.isPending
													? "Initiating Payment..."
													: `Pay ${formatCurrency(fetchedBill.billAmount || "0")}`}
											</Button>
										</div>
									</>
								) : (
									<Alert
										variant="destructive"
										data-testid="alert-bill-fetch-failed"
									>
										<XCircle className="h-4 w-4" />
										<AlertDescription>
											Failed to fetch bill details. Please check your
				...
				>
					<Card data-testid="card-bill-history">
						<CardHeader>
							<CardTitle
								className="flex items-center gap-2"
								data-testid="card-title-history"
							>
										...
										>
											<div>
												<div
													...
													data-testid={`bill-customer-param-${bill.id}`}
												>
													{bill.customerParam}
												</div>
												<div
													className="text-sm text-muted-foreground"
													data-testid={`bill-period-${bill.id}`}
												>
													{bill.billPeriod}
												</div>
											</div>
				...
				<TabsContent
					value="transactions"
					className="space-y-4"
					data-testid="tab-content-transactions"
				>
					<Card data-testid="card-transaction-history">
						<CardHeader>
							<CardTitle
								className="flex items-center gap-2"
								data-testid="card-title-transactions"
							>
						...
						<CardContent>
							{userTransactions.length > 0 ? (
									...
									{userTransactions.map((transaction) => (
										...
										>
											<div>
												...
												<div
													className="text-sm text-muted-foreground"
													data-testid={`transaction-customer-param-${transaction.id}`}
												>
													{transaction.customerParam}
												</div>
												<div
											...
											<div className="text-right">
												...
												<div className="flex items-center gap-2">
													<div
														className={`w-2 h-2 rounded-full ${getStatusColor(transaction.paymentStatus)}`}
														data-testid={`transaction-status-indicator-${transaction.id}`}
													/>
													<Badge
														variant={
								...
								>
									No transactions found. Complete a bill payment to see your
									transactions here.
								</div>
							)}
						</CardContent>
