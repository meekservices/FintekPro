...
export default function AgentSampleReport() {
	...
	const generatePDF = async (report: any) => {
		...
		try {
			...
			doc.text("Portfolio Analysis Report", pageWidth / 2, yPos, {
				align: "center",
			});
			yPos += 10;
			...
			if (selectedSections.portfolioSnapshot) {
				...
				doc.setTextColor(0);
				doc.text(
					`Total Value: ${formatCurrency(report.portfolio.totalValue)}`,
					14,
					yPos,
				);
				yPos += 6;
				doc.text(
					`Overall Gain: ${formatCurrency(report.portfolio.overallGain)} (${report.portfolio.overallGainPercent}%)`,
					14,
					yPos,
				);
				yPos += 6;
				...
				yPos += 6;
				doc.text(
					`Risk Score: ${report.riskMetrics.riskScore}/100 (${report.riskMetrics.riskCategory})`,
					14,
					yPos,
				);
				yPos += 12;
			...
			if (selectedSections.assetAllocation) {
				...
				report.assetAllocation.current.forEach((item: any) => {
					doc.text(
						`${item.asset}: ${item.percentage}% (${formatCurrency(item.value)})`,
						14,
						yPos,
					);
					yPos += 6;
			...
			if (selectedSections.topHoldings) {
				...
				report.topHoldings.slice(0, 5).forEach((holding: any, idx: number) => {
					doc.text(
						`${idx + 1}. ${holding.name} - ${formatCurrency(holding.value)} (${holding.percentage}%)`,
						14,
						yPos,
					);
					yPos += 6;
			...
			if (selectedSections.goals) {
				...
				report.goals.forEach((goal: any) => {
					doc.text(
						`${goal.name}: ${goal.progress}% complete (${goal.onTrack ? "On Track" : "Off Track"})`,
						14,
						yPos,
					);
					yPos += 6;
			...
			if (selectedSections.freshInvestments) {
				...
				doc.setTextColor(0);
				doc.text(
					`Investable Surplus: ${formatCurrency(report.freshInvestments.investableSurplus)}`,
					14,
					yPos,
				);
				yPos += 8;
				...
				report.freshInvestments.sipRecommendations.forEach((fund: any) => {
					doc.text(
						`  - ${fund.name} @ ${formatCurrency(fund.suggestedSIP)}/mo`,
						14,
						yPos,
					);
					yPos += 5;
			...
			if (showAgentDetails) {
				...
				doc.setTextColor(0);
				doc.text(
					`Prepared by: ${report.agent.name} (${report.agent.code})`,
					14,
					yPos,
				);
			}
			...
			doc.save(
				`Portfolio_Report_${report.client.name.replace(/\s+/g, "_")}_${new Date().toISOString().split("T")[0]}.pdf`,
			);
			toast({
			...
			<div className="max-w-7xl mx-auto p-6 space-y-6">
						...
						<Sheet>
							...
							<SheetContent>
								<SheetHeader>
									...
									<SheetDescription>
										Configure which sections to include and display options
									</SheetDescription>
								</SheetHeader>
						...
						<div className="flex items-center gap-4">
							...
							<div>
								...
								<p
									...
									data-testid="text-client-pan"
								>
									PAN: {report.client.pan} | Risk Profile:{" "}
									{report.client.riskProfile}
								</p>
							</div>
						</div>
				...
				{selectedSections.portfolioSnapshot && (
					<div
						...
						data-testid="section-portfolio-snapshot"
					>
						<Card data-testid="card-total-value">
							<CardContent className="p-4">
								...
								>
									{formatCurrency(report.portfolio.totalValue)}
								</p>
								<div
									className={`flex items-center text-sm mt-1 ${report.portfolio.dayChangePercent >= 0 ? "text-green-600" : "text-red-600"}`}
									data-testid="text-day-change"
								>
									{report.portfolio.dayChangePercent >= 0 ? (
										<ArrowUpRight className="h-4 w-4" />
									) : (
										<ArrowDownRight className="h-4 w-4" />
									)}
									{formatCurrency(Math.abs(report.portfolio.dayChange))} (
									{report.portfolio.dayChangePercent}%)
								</div>
							</CardContent>
						</Card>
						<Card data-testid="card-overall-gain">
							<CardContent className="p-4">
								...
								<p
									...
									data-testid="text-overall-gain"
								>
									+{formatCurrency(report.portfolio.overallGain)}
								</p>
								<p className="text-sm text-green-600">
									+{report.portfolio.overallGainPercent}%
								</p>
							</CardContent>
						</Card>
						<Card data-testid="card-xirr">
							<CardContent className="p-4">
								...
								<p className="text-2xl font-bold mt-2" data-testid="text-xirr">
									{report.portfolio.xirr}%
								</p>
						...
						<Card data-testid="card-risk-score">
							<CardContent className="p-4">
								...
								>
									{report.riskMetrics.riskScore}/100
								</p>
								<Badge variant="secondary" data-testid="badge-risk-category">
									{report.riskMetrics.riskCategory}
								</Badge>
							</CardContent>
						</Card>
					</div>
				...
				<Tabs defaultValue="allocation" className="w-full">
					...
					<TabsContent
						value="allocation"
						className="space-y-6"
						data-testid="tabcontent-allocation"
					>
								...
								<Card data-testid="card-asset-allocation">
									...
									<CardContent>
										<div className="h-64">
											<ResponsiveContainer width="100%" height="100%">
												<PieChart>
													...
													>
														{report.assetAllocation.current.map(
										...
										<div
											...
											data-testid="list-asset-allocation"
										>
											{report.assetAllocation.current.map(
												(item: any, index: number) => (
													<div
														key={item.asset}
														className="flex items-center justify-between"
														data-testid={`row-allocation-${item.asset.toLowerCase().replace(/\s+/g, "-")}`}
													>
														<div className="flex items-center gap-2">
															...
															/>
															<span
																data-testid={`text-allocation-asset-${index}`}
															>
																{item.asset}
															</span>
														</div>
														<div className="text-right">
															<span
																...
																data-testid={`text-allocation-percentage-${index}`}
															>
																{item.percentage}%
															</span>
															<span
																className="text-muted-foreground ml-2"
																data-testid={`text-allocation-value-${index}`}
															>
								...
								<Card>
										...
										<CardDescription>
											{report.assetAllocation.rebalancingNeeded && (
												<Badge variant="destructive" className="mt-1">
													Rebalancing Needed (
													{report.assetAllocation.driftPercentage}% drift)
												</Badge>
											)}
										</CardDescription>
									</CardHeader>
									<CardContent>
										<div className="h-64">
											<ResponsiveContainer width="100%" height="100%">
												<BarChart
													data={report.assetAllocation.current.map(
														(item: any) => ({
															asset: item.asset,
															current: item.percentage,
															target:
																report.assetAllocation.target.find(
																	(t: any) => t.asset === item.asset,
																)?.percentage || 0,
														}),
													)}
												>
						...
						{selectedSections.topHoldings && (
							<Card data-testid="card-top-holdings">
								<CardHeader>
									...
									<CardDescription>
										Your major investments by value
									</CardDescription>
								</CardHeader>
								<CardContent>
											...
											>
												<div className="flex items-center gap-4">
													...
													<div>
														<p
															className="font-medium"
															data-testid={`text-holding-name-${index}`}
														>
															{holding.name}
														</p>
														<div className="flex items-center gap-2">
															<Badge
																variant="outline"
																data-testid={`badge-holding-type-${index}`}
															>
																{holding.type}
															</Badge>
															{holding.rating && (
																<div
																	...
																	data-testid={`stars-holding-rating-${index}`}
																>
												...
												<div className="text-right">
													<p
														className="font-bold"
														data-testid={`text-holding-value-${index}`}
													>
														{formatCurrency(holding.value)}
													</p>
													<p
														className="text-sm text-muted-foreground"
														data-testid={`text-holding-percentage-${index}`}
													>
														{holding.percentage}% of portfolio
													</p>
													<p
														className={`text-sm ${holding.returns1Y >= 0 ? "text-green-600" : "text-red-600"}`}
														data-testid={`text-holding-returns-${index}`}
													>
														1Y: {holding.returns1Y >= 0 ? "+" : ""}
														{holding.returns1Y}%
													</p>
												</div>
											</div>
							...
							<CardHeader>
								...
								<CardDescription>
									...
									<Badge
										className="ml-2"
										variant={
											report.comparisonWithBenchmark.tracking ===
											"Outperforming"
												? "default"
												: "destructive"
										}
							...
							<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
								...
								<Card>
									<CardContent className="p-4 text-center">
										...
										<p className="text-2xl font-bold">
											{report.riskMetrics.sharpeRatio}
										</p>
									</CardContent>
								</Card>
								<Card>
									<CardContent className="p-4 text-center">
										...
										<p className="text-2xl font-bold">
											{report.riskMetrics.standardDeviation}%
										</p>
									</CardContent>
								</Card>
								<Card>
									<CardContent className="p-4 text-center">
										...
										<p className="text-2xl font-bold text-red-600">
											{report.riskMetrics.maxDrawdown}%
										</p>
									</CardContent>
								</Card>
							</div>
										...
										{report.goals.map((goal: any, index: number) => (
											...
											>
												<div className="flex items-center justify-between mb-2">
													<div>
														<h4
															...
															data-testid={`text-goal-name-${index}`}
														>
															{goal.name}
														</h4>
														<p
															className="text-sm text-muted-foreground"
															data-testid={`text-goal-date-${index}`}
														>
															Target:{" "}
													...
													<Badge
														...
														data-testid={`badge-goal-status-${index}`}
													>
														{goal.onTrack ? (
												...
												<div className="flex justify-between text-sm">
													<span data-testid={`text-goal-current-${index}`}>
														Current: {formatCurrency(goal.currentValue)}
													</span>
													<span
														className="font-medium"
														data-testid={`text-goal-progress-${index}`}
													>
														{goal.progress}% complete
													</span>
													<span data-testid={`text-goal-target-${index}`}>
												...
												{goal.shortfall && (
													<p
														...
														data-testid={`text-goal-shortfall-${index}`}
													>
														Shortfall: {formatCurrency(goal.shortfall)} -
														Increase SIP recommended
													</p>
												)}
											</div>
										))}
					...
					<TabsContent
						value="investments"
						className="space-y-6"
						data-testid="tabcontent-investments"
					>
						{selectedSections.freshInvestments && (
							<>
								<Card
									className="border-l-4 border-l-green-500"
									data-testid="card-investable-surplus"
								>
									<CardContent className="p-4">
										<div className="flex items-center justify-between">
											<div>
												<h3 className="font-semibold">
													Investable Surplus Detected
												</h3>
												<p className="text-muted-foreground">
													Based on income and expense analysis
												</p>
											</div>
											<div className="text-right">
												<p
													className="text-2xl font-bold text-green-600"
													data-testid="text-investable-surplus"
												>
													{formatCurrency(
														report.freshInvestments.investableSurplus,
													)}
												</p>
												<p
													className="text-sm text-muted-foreground"
													data-testid="text-suggested-allocation"
												>
													Suggested:{" "}
													{formatCurrency(
														report.freshInvestments.suggestedAllocation.lumpsum,
													)}{" "}
													Lumpsum +{" "}
													{formatCurrency(
														report.freshInvestments.suggestedAllocation
															.sipMonthly,
													)}
													/mo SIP
												</p>
								...
								<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
									<Card data-testid="card-lumpsum-recommendations">
										...
										<CardContent>
											<div
												className="space-y-4"
												data-testid="list-lumpsum-recommendations"
											>
												{report.freshInvestments.lumpsumRecommendations.map(
													(fund: any, index: number) => (
														<div
															key={index}
															className="border rounded-lg p-4"
															data-testid={`row-lumpsum-${index}`}
														>
															<div className="flex items-center justify-between mb-2">
																<div>
																	<p
																		className="font-medium"
																		data-testid={`text-lumpsum-name-${index}`}
																	>
																		{fund.name}
																	</p>
																	<Badge
																		variant="outline"
																		data-testid={`badge-lumpsum-category-${index}`}
																	>
																		{fund.category}
																	</Badge>
																</div>
																<Badge
																	...
																	data-testid={`badge-lumpsum-match-${index}`}
																>
															...
															<div className="grid grid-cols-3 gap-2 text-sm mt-2">
																<div>
																	...
																	<p
																		...
																		data-testid={`text-lumpsum-return-${index}`}
																	>
																		{fund.expectedReturn}%
																	</p>
																</div>
																<div>
																	...
																	<p
																		className="font-medium"
																		data-testid={`text-lumpsum-risk-${index}`}
																	>
																		{fund.riskLevel}
																	</p>
																</div>
																<div>
																	...
																	<div
																		className="flex"
																		data-testid={`stars-lumpsum-rating-${index}`}
																	>
															...
															<p
																...
																data-testid={`text-lumpsum-reason-${index}`}
															>
																"{fund.reason}"
															</p>
															<Button
																...
																data-testid={`button-invest-lumpsum-${index}`}
															>
																Invest Now
															</Button>
														</div>
									...
									<Card data-testid="card-sip-recommendations">
										...
										<CardContent>
											<div
												className="space-y-4"
												data-testid="list-sip-recommendations"
											>
												{report.freshInvestments.sipRecommendations.map(
													(fund: any, index: number) => (
														<div
															key={index}
															className="border rounded-lg p-4"
															data-testid={`row-sip-${index}`}
														>
															<div className="flex items-center justify-between mb-2">
																<div>
																	<p
																		className="font-medium"
																		data-testid={`text-sip-name-${index}`}
																	>
																		{fund.name}
																	</p>
																	<Badge
																		variant="outline"
																		data-testid={`badge-sip-category-${index}`}
																	>
																		{fund.category}
																	</Badge>
																</div>
																<Badge
																	...
																	data-testid={`badge-sip-match-${index}`}
																>
																	{fund.matchScore}% Match
																</Badge>
															</div>
															<div className="grid grid-cols-3 gap-2 text-sm mt-2">
																<div>
																	...
																	<p
																		className="font-medium"
																		data-testid={`text-sip-suggested-${index}`}
																	>
																		{formatCurrency(fund.suggestedSIP)}/mo
																	</p>
																</div>
																<div>
																	<p className="text-muted-foreground">
																	...
																	<p
																		className="font-medium text-green-600"
																		data-testid={`text-sip-return-${index}`}
																	>
																		{fund.expectedReturn}%
																	</p>
																</div>
																<div>
																	...
																	<p
																		className="font-medium"
																		data-testid={`text-sip-min-${index}`}
																	>
																		{formatCurrency(fund.minSIP)}
																	</p>
																</div>
															</div>
															<p
																className="text-sm text-muted-foreground mt-2 italic"
																data-testid={`text-sip-reason-${index}`}
															>
																"{fund.reason}"
															</p>
															<Button
																...
																variant="outline"
																className="mt-2 w-full"
																data-testid={`button-start-sip-${index}`}
															>
																Start SIP
															</Button>
														</div>
				...
				{selectedSections.aiInsights && (
					<Card data-testid="card-ai-insights">
						<CardHeader>
							<CardTitle className="flex items-center gap-2">
								<Lightbulb className="h-5 w-5 text-yellow-500" /> AI-Powered
								Insights
							</CardTitle>
						</CardHeader>
						<CardContent>
										...
										<div className="flex items-start justify-between">
											<div>
												<div className="flex items-center gap-2">
													...
													<h4
														className="font-semibold"
														data-testid={`text-insight-title-${index}`}
													>
														{insight.title}
													</h4>
												</div>
												<p
													...
													data-testid={`text-insight-description-${index}`}
												>
													{insight.description}
												</p>
											</div>
				...
				<Card className="bg-muted" data-testid="card-disclaimers">
					<CardContent className="p-4">
						...
						<ul
							...
							data-testid="list-disclaimers"
						>
							{report.disclaimers.map((disclaimer: string, index: number) => (
								<li key={index} data-testid={`text-disclaimer-${index}`}>
									• {disclaimer}
								</li>
							))}
						</ul>
