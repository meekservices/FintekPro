...
export default function AdminApiUsage() {
	...
	const [editingProvider, setEditingProvider] =
		useState<ProviderPricing | null>(null);
	...
	} = useQuery<{ success: boolean; providers: ProviderPricing[] }>({
	...
	} = useQuery<MonthlyEstimate & { success: boolean }>({
	...
	const handleSavePricing = () => {
		...
		updatePricingMutation.mutate({
			...
			costPerCall: Number.parseFloat(newCost),
		});
	};
			...
			<div className="space-y-6">
				...
				<div className="grid gap-4 md:grid-cols-4">
					<Card>
						...
						<CardContent>
							<div className="text-2xl font-bold" data-testid="text-mtd-spend">
								₹{totalMonthlySpend.toFixed(2)}
							</div>
							<p className="text-xs text-muted-foreground">
								{monthlyEstimate?.month || "Current Month"}
							</p>
						</CardContent>
					</Card>
					...
					<Card>
						...
						<CardContent>
							<div
								...
								data-testid="text-projected-spend"
							>
				...
				<div className="grid gap-6 md:grid-cols-2">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								API calls and costs over the last 30 days
							</CardDescription>
						</CardHeader>
						<CardContent>
							{dailyUsage.length > 0 ? (
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Monthly spend breakdown by API provider
							</CardDescription>
						</CardHeader>
						<CardContent>
							{monthlyEstimate?.byProvider &&
							monthlyEstimate.byProvider.length > 0 ? (
								<ResponsiveContainer width="100%" height={300}>
				...
				<Card>
					<CardHeader>
						...
						<CardDescription>
							Configure the cost per API call for each provider. These rates are
							used to calculate estimated monthly expenditure.
						</CardDescription>
					</CardHeader>
					<CardContent>
						{loadingProviders ? (
									...
									{providers.map((provider) => {
										...
										return (
											<TableRow
												...
												data-testid={`row-provider-${provider.providerName}`}
											>
				...
				<Card className="border-amber-200 bg-amber-50 dark:bg-amber-950/20 dark:border-amber-900">
					...
					<CardContent className="text-sm text-amber-700 dark:text-amber-400 space-y-2">
						<p>
							• API usage is tracked automatically when external services are
							called.
						</p>
						<p>
							• Costs are estimated based on the per-call rates you configure
							here.
						</p>
						<p>
							• Actual billing may vary based on your agreements with each
							provider.
						</p>
						<p>
							• Update pricing regularly to maintain accurate cost projections.
						</p>
					</CardContent>
				</Card>
			</div>
