...
export default function ProspectAnalytics() {
	...
	const { data: surplusCompanies } = useQuery<{
		companies: InvestableSurplus[];
	}>({
			...
			<div className="flex justify-between items-center">
				<div>
					<h1
						...
						data-testid="title-prospect-analytics"
					>
						Prospect Analytics
					</h1>
				...
				>
					<RefreshCw
						className={`w-4 h-4 mr-2 ${checkAlertsMutation.isPending ? "animate-spin" : ""}`}
					/>
			...
			>
					...
					<Card>
						...
						<CardContent>
							<div className="space-y-4">
								{surplusCompanies?.companies?.map((company) => (
									...
									>
										<div className="flex justify-between items-start mb-3">
											...
											<Badge
												className={
													company.surplusCategory === "high"
														? "bg-green-500"
														: company.surplusCategory === "medium"
															? "bg-yellow-500"
															: "bg-muted"
												}
											>
										...
										<div className="grid grid-cols-2 gap-4 mb-3">
											...
											<div>
												...
												<div className="flex items-center gap-2">
													<Progress
														value={company.investmentReadiness}
														className="h-2"
													/>
													<span className="text-sm">
														{company.investmentReadiness}%
													</span>
												</div>
											</div>
										</div>
								...
								{(!surplusCompanies?.companies ||
									surplusCompanies.companies.length === 0) && (
									<p className="text-sm text-muted-foreground text-center py-8">
										No companies with significant investable surplus found. Add
										more leads with financial data to analyze.
									</p>
								)}
							</div>
						</CardContent>
					...
					<Card>
						...
						<CardContent className="space-y-4">
							<div className="flex gap-2">
								...
								<Button
									onClick={() =>
										searchCIN && scoreLeadMutation.mutate(searchCIN)
									}
						...
						<CardContent>
							<div className="space-y-4">
								...
								{(!sectors || sectors.length === 0) && (
									<p className="text-sm text-muted-foreground text-center py-8">
										No sector data available. Import leads with sector
										information.
									</p>
								)}
							</div>
				...
				<TabsContent value="geography" className="space-y-4">
					<Card>
						...
						<CardContent>
							<div className="space-y-4">
								{geoData?.underservedRegions &&
									geoData.underservedRegions.length > 0 && (
										<div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-4">
											...
											<div className="flex flex-wrap gap-2">
												{geoData.underservedRegions.map((region) => (
													<Badge key={region} className="bg-green-500">
														{region}
													</Badge>
												))}
											</div>
									...
									<div key={region.state} className="border rounded-lg p-4">
										<div className="flex justify-between items-start mb-3">
											<div>
												...
												<p className="text-xs text-muted-foreground">
													{region.prospectCount} prospects |{" "}
													{region.hotLeadsCount} hot leads
												</p>
											</div>
											<Badge
												className={
													region.growthPotential === "high"
														? "bg-green-500"
														: region.growthPotential === "medium"
															? "bg-yellow-500"
															: "bg-muted"
												}
											>
								...
								{(!geoData?.regions || geoData.regions.length === 0) && (
									<p className="text-sm text-muted-foreground text-center py-8">
										No geographic data available. Import leads with location
										information.
									</p>
								)}
							</div>
											...
											<div className="flex items-center gap-3">
												...
												<div>
													<p className="font-medium">
														{alert.companyName || "Unknown Company"}
													</p>
													<p className="text-sm text-muted-foreground">
														{alert.message}
													</p>
