...
export default function AdminDashboard() {
  const { data: dashboardData, isLoading: dashboardLoading, refetch: refetchDashboard } = useQuery<DashboardStats>({
    queryKey: ["/api/admin/dashboard"],
  });

  const { data: kycResponse, isLoading: kycLoading } = useQuery<{ success: boolean; data: KycStats }>({
    queryKey: ["/api/admin/kyc/dashboard"],
  });
  const kycStats = kycResponse?.data;

  const { data: stakeholderResponse, isLoading: stakeholderLoading } = useQuery<{ stats: StakeholderStats }>({
    queryKey: ["/api/admin/stakeholders/stats"],
  });
  const stakeholderStats = stakeholderResponse?.stats;

  const { data: pendingOrdersData, isLoading: ordersLoading } = useQuery<PendingOrdersCount>({
  ...
  const kycDistribution = kycStats ? [
    { name: 'Tier 1 (Basic)', value: kycStats.tier1Count || 0, color: '#3b82f6' },
    { name: 'Tier 2 (Enhanced)', value: kycStats.tier2Count || 0, color: '#10b981' },
    { name: 'Tier 3 (Accredited)', value: kycStats.tier3Count || 0, color: '#f59e0b' },
  ] : [];
    ...
    <div className="space-y-6">
      ...
      <Card className="bg-card border-border">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { name: 'API Server', status: 'operational', uptime: '99.99%', icon: Activity },
              { name: 'Database', status: 'operational', uptime: '100%', icon: ShieldCheck },
