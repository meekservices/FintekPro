...
export default function RiskComplianceExport() {
  ...
  const { data: report, isLoading, refetch } = useQuery<{ success: boolean; data: ComplianceReport }>({
    queryKey: ["/api/sebi-risk-profiling/compliance/report", fromDate, toDate, reportType],
    enabled: false,
  });
  ...
  const generateReport = async () => {
    try {
      ...
      refetch();
      toast({ title: "Report Generated", description: "Compliance report generated successfully" });
    } catch (error: any) {
    ...
    <div className="container mx-auto py-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Generate and export risk profiling compliance reports for SEBI inspections
          </p>
        </div>
      </div>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="flex items-end gap-2">
              <Button onClick={generateReport} disabled={isLoading} data-testid="button-generate-report">
        ...
        <>
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Generated: {format(new Date(reportData.summary.reportGeneratedAt), "PPpp")}
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="p-4 bg-muted/50 rounded-lg text-center">
                  <p className="text-3xl font-bold">{reportData.summary.totalAssessments}</p>
                  <p className="text-sm text-muted-foreground">Total Assessments</p>
                </div>
                <div className="p-4 bg-muted/50 rounded-lg text-center">
                  <p className="text-3xl font-bold">{reportData.summary.overridesApplied}</p>
                  <p className="text-sm text-muted-foreground">Overrides Applied</p>
                </div>
                <div className="p-4 bg-muted/50 rounded-lg text-center">
                  <p className="text-3xl font-bold">{reportData.summary.auditLogCount}</p>
                  <p className="text-sm text-muted-foreground">Audit Log Entries</p>
                </div>
                <div className="p-4 bg-muted/50 rounded-lg text-center">
                  <p className="text-3xl font-bold">
                    {reportData.summary.totalAssessments > 0
                      ? Math.round((reportData.summary.overridesApplied / reportData.summary.totalAssessments) * 100)
                      : 0}%
                  </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <div className="space-y-2">
                    {Object.entries(reportData.summary.assessmentsByProfile).map(([profile, count]) => (
                      <div key={profile} className="flex items-center justify-between p-2 bg-muted/30 rounded">
                        <Badge className={PROFILE_COLORS[profile]}>{profile}</Badge>
                        <span className="font-medium">{count}</span>
                      </div>
                    ))}
                    {Object.keys(reportData.summary.assessmentsByProfile).length === 0 && (
                      <p className="text-sm text-muted-foreground">No assessments in period</p>
                    )}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {Object.entries(reportData.summary.overridesByType).map(([type, count]) => (
                      <div key={type} className="flex items-center justify-between p-2 bg-muted/30 rounded">
                        <span className="capitalize">{type.replace(/_/g, " ")}</span>
                        <Badge variant="secondary">{count}</Badge>
                      </div>
                    ))}
                    {Object.keys(reportData.summary.overridesByType).length === 0 && (
                      <p className="text-sm text-muted-foreground">No overrides in period</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                  {Object.entries(reportData.summary.auditLogsByCategory).map(([category, count]) => (
                    <div key={category} className="p-3 bg-muted/30 rounded text-center">
              ...
              <CardContent>
                  ...
                  <TableBody>
                    {reportData.assessments.slice(0, 50).map((a) => (
                      <TableRow key={a.id}>
                        ...
                        <TableCell>
                          <Badge className={PROFILE_COLORS[a.profileCode]}>{a.profileCode}</Badge>
                        </TableCell>
                        <TableCell>{Math.round(a.rawScore)}</TableCell>
                        <TableCell>
                          {a.hasOverride ? (
                            <Badge variant="secondary">{a.overrideType || "Yes"}</Badge>
                          ) : (
                ...
                {reportData.assessments.length > 50 && (
                  <p className="text-sm text-muted-foreground mt-4 text-center">
                    Showing 50 of {reportData.assessments.length} assessments. Export CSV for full data.
                  </p>
                )}
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                ...
                {reportData.auditLogs.length > 50 && (
                  <p className="text-sm text-muted-foreground mt-4 text-center">
                    Showing 50 of {reportData.auditLogs.length} logs. Export CSV for full data.
                  </p>
                )}
              </CardContent>
            </Card>
      ...
      <Alert>
        ...
        <AlertDescription>
          ...
          and include full traceability for regulatory inspections. Export reports periodically for backup.
        </AlertDescription>
      </Alert>
    </div>
  );
