...
const sectors = [
  ...
  "Industrials", "Energy", "Materials", "Real Estate", "Utilities", "Communication Services", "Index"
];
...
export default function GlobalSeedAdmin() {
  ...
  const { data: instrumentsData, isLoading } = useQuery<{
    ...
    queryKey: ["/api/admin/global-instruments", activeTab, marketFilter, search],
    queryFn: async () => {
      ...
      const response = await fetch(`/api/admin/global-instruments?${params}`, { credentials: "include" });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      resetForm();
      queryClient.invalidateQueries({ queryKey: ["/api/admin/global-instruments"] });
    },
    onError: (error: any) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      return apiRequest(`/api/admin/global-instruments/${id}`, { method: "DELETE" });
    },
    onSuccess: () => {
  ...
  const seedSampleMutation = useMutation({
    mutationFn: async () => {
      return apiRequest("/api/admin/global-instruments/seed-sample", { method: "POST" });
    },
    onSuccess: (data: any) => {
  ...
  const handleBulkImport = () => {
    try {
      ...
      const instruments = lines.map(line => {
        const [symbol, name, assetClass, exchange, market, currency, isin, sector] = line.split(",").map(s => s.trim());
        return { symbol, name, assetClass, exchange, market, currency, isin, sector };
      });
      bulkImportMutation.mutate(instruments);
    } catch (error) {
      toast({ title: "Invalid Format", description: "Please use CSV format", variant: "destructive" });
    }
  };
      ...
      <div className="p-6">
        ...
        {stats && (
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            ...
            {Object.entries(stats.byAssetClass).slice(0, 4).map(([cls, count]) => (
              <Card key={cls}>
                <CardContent className="pt-4">
                  <div className="text-2xl font-bold">{count}</div>
                  <div className="text-sm text-muted-foreground capitalize">{cls.replace("_", " ")}s</div>
                </CardContent>
              </Card>
            ))}
          </div>
                ...
                {assetClasses.map(ac => (
                  <TabsTrigger key={ac.value} value={ac.value}>
                    ...
                    {stats?.byAssetClass[ac.value] && (
                      <Badge variant="secondary" className="ml-2">{stats.byAssetClass[ac.value]}</Badge>
                    )}
                  </TabsTrigger>
                ))}
              </TabsList>
                ...
                <TabsContent key={ac.value} value={ac.value}>
                  {isLoading ? (
                    ...
                    <div className="text-center p-8 text-muted-foreground">
                      No {ac.label.toLowerCase()} found. Click "Seed Sample Data" or "Add Instrument" to get started.
                    </div>
                  ) : (
                        ...
                        <TableBody>
                          {instruments.map(inst => (
                            <TableRow key={inst.id}>
                              <TableCell className="font-mono font-bold">{inst.symbol}</TableCell>
                              <TableCell className="max-w-[200px] truncate">{inst.name}</TableCell>
                              <TableCell>
                                <Badge variant="outline">{inst.exchange}</Badge>
                              </TableCell>
                              <TableCell>
                                <Badge variant="secondary">{inst.market}</Badge>
                              </TableCell>
                              <TableCell className="text-sm text-muted-foreground">{inst.sector || "-"}</TableCell>
                              <TableCell className="font-mono text-xs">{inst.isin || "-"}</TableCell>
                              <TableCell>
                                <Switch
                                  checked={inst.isActive}
                                  onCheckedChange={(checked) => toggleMutation.mutate({ id: inst.id, isActive: checked })}
                                />
                              </TableCell>
                              <TableCell>
        ...
        <Dialog open={addDialogOpen || !!editingInstrument} onOpenChange={(open) => {
          if (!open) {
            setAddDialogOpen(false);
            setEditingInstrument(null);
            resetForm();
          ...
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              ...
              <DialogDescription>
                {editingInstrument ? "Update instrument details" : "Add a new global instrument to the catalog"}
              </DialogDescription>
            </DialogHeader>
            <div className="grid grid-cols-2 gap-4 py-4">
              ...
              <div className="space-y-2">
                ...
                <Select value={formData.assetClass} onValueChange={(v) => setFormData({ ...formData, assetClass: v })}>
              ...
              <div className="space-y-2">
                ...
                <Select value={formData.marketCapCategory} onValueChange={(v) => setFormData({ ...formData, marketCapCategory: v })}>
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => { setAddDialogOpen(false); setEditingInstrument(null); resetForm(); }}>
                Cancel
              </Button>
        ...
        <Dialog open={bulkDialogOpen} onOpenChange={setBulkDialogOpen}>
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              ...
              <DialogDescription>
                Paste CSV data with format: symbol, name, assetClass, exchange, market, currency, isin, sector
              </DialogDescription>
            </DialogHeader>
...
SPY,SPDR S&P 500 ETF Trust,etf,NYSE,US,USD,,Index"
              ...
              <p className="text-sm text-muted-foreground">
                Each line represents one instrument. Fields: symbol, name, assetClass (stock/etf/mutual_fund/bond), exchange, market, currency, isin (optional), sector (optional)
              </p>
            </div>
            <DialogFooter>
