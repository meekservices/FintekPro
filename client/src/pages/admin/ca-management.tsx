...
export default function CAManagement() {
  ...
  const verifyMutation = useMutation({
    mutationFn: async ({ partnerId, action, rejectionReason }: { 
      partnerId: string; 
      ...
      return await apiRequest(`/api/ca/admin/verify/${partnerId}`, {
        method: 'POST',
        body: JSON.stringify({ action, rejectionReason, adminId: 'current-admin' }),
      });
    },
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['/api/ca/admin/pending-verifications'] });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage Chartered Accountant partners and their performance</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{summary?.availableCAs || 0}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{summary?.avgUtilization || 0}%</div>
        ...
        <TabsContent value="pending" className="mt-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Review and verify CA registrations</CardDescription>
            </CardHeader>
            <CardContent>
              {pendingLoading ? (
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>Manage all registered CA partners</CardDescription>
                </div>
                      ...
                      <TableRow key={ca.id}>
                        ...
                        <TableCell>
                          <div>
                            ...
                            <Badge variant="outline" className="text-xs">{ca.membershipType}</Badge>
                          </div>
                        </TableCell>
                        <TableCell>
                          {ca.city}, {ca.state}
                        ...
                        <TableCell>
                          <div className="flex items-center gap-1">
                            <Star className="h-4 w-4 fill-yellow-400 text-yellow-400" />
                            {parseFloat(ca.rating || '0').toFixed(1)}
                          </div>
                        </TableCell>
                        <TableCell>
                          <Button variant="ghost" size="icon" data-testid={`button-view-ca-${ca.id}`}>
        ...
        <TabsContent value="performance" className="mt-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Monitor CA workload and performance metrics</CardDescription>
            </CardHeader>
            <CardContent>
              {performanceLoading ? (
                <div className="flex justify-center py-8">
                      ...
                      <TableRow key={ca.id}>
                        ...
                        <TableCell>
                          <div className="flex items-center gap-2 min-w-[120px]">
                            <Progress value={ca.utilizationRate} className="h-2 flex-1" />
                            <span className="text-sm">{ca.utilizationRate}%</span>
                          </div>
                        </TableCell>
      ...
      <Dialog open={showVerifyDialog} onOpenChange={setShowVerifyDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {verifyAction === 'approve' ? 'Approve CA Application' : 'Reject CA Application'}
            </DialogTitle>
            <DialogDescription>
