...
function PortfolioApprovalsTab() {
	...
	const [approvalType, setApprovalType] = useState<"aif" | "pms" | "mld">(
		"aif",
	);
	...
	const { data: portfolioData, isLoading } = useQuery<{
		...
		mld: PortfolioMLD[];
		summary: {
			totalAifHoldings: number;
			totalPmsHoldings: number;
			totalMldHoldings: number;
	...
	const approveMutation = useMutation({
		...
		onSuccess: (_, variables) => {
			...
			toast({
				title:
					variables.action === "approve" ? "Entry Approved" : "Entry Rejected",
	...
	const pendingAIFCount = aifHoldings.filter(
		(h) => h.entryStatus === "pending",
	).length;
	const pendingPMSCount = pmsHoldings.filter(
		(h) => h.entryStatus === "pending",
	).length;
	const pendingMLDCount = mldHoldings.filter(
		(h) => h.entryStatus === "pending",
	).length;
		...
		<Card className="bg-card border-border">
			<CardHeader>
				...
				<CardDescription className="text-muted-foreground">
					Review and approve client-submitted AIF, PMS, and MLD holdings for
					portfolio analysis
				</CardDescription>
			</CardHeader>
				...
				{approvalType === "aif" ? (
					isLoading ? (
						...
						<div className="text-center py-12 text-muted-foreground">
							...
							<p>
								No {statusFilter === "all" ? "" : statusFilter} AIF entries
								found
							</p>
						</div>
					) : (
									...
									{filteredAIF.map((holding) => (
										...
										>
											<TableCell className="text-foreground">
												<div>
													<p className="font-medium">
														{holding.client?.name || "Unknown"}
													</p>
													<p className="text-xs text-muted-foreground">
														{holding.client?.email}
													</p>
												</div>
											</TableCell>
											<TableCell className="text-foreground">
											...
											<TableCell>
												<Badge
													className={
														holding.entryStatus === "approved"
															? "bg-green-500/20 text-green-400"
															: holding.entryStatus === "rejected"
					...
					isLoading ? (
						...
						<div className="text-center py-12 text-muted-foreground">
							<LucideShield className="w-12 h-12 mx-auto mb-4 opacity-50" />
							<p>
								No {statusFilter === "all" ? "" : statusFilter} PMS entries
								found
							</p>
						</div>
					) : (
								...
								<TableBody>
									{filteredPMS.map((holding) => (
										...
										>
											...
											<TableCell className="text-right text-foreground">
												{formatCurrency(holding.corpusValue)}
											</TableCell>
											<TableCell className="text-right">
												<span
													className={
														Number.parseFloat(holding.cagr || "0") >= 0
															? "text-green-400"
															: "text-red-400"
													}
												>
					...
					isLoading ? (
						...
						<div className="text-center py-12 text-muted-foreground">
							<LucideShield className="w-12 h-12 mx-auto mb-4 opacity-50" />
							<p>
								No {statusFilter === "all" ? "" : statusFilter} MLD entries
								found
							</p>
						</div>
					) : (
								...
								<TableBody>
									{filteredMLD.map((holding) => (
										<TableRow
											key={holding.id}
											className="border-border hover:bg-muted/20"
											data-testid={`row-mld-${holding.id}`}
										>
											<TableCell className="text-foreground">
												<div>
													<p className="font-medium">
...
export default function StoreManagement() {
	...
	console.log(
		"[StoreManagement] Categories data:",
		categoriesData,
		"isLoading:",
		isLoadingCategories,
		"isError:",
	...
	const toggleCategoryMutation = useMutation({
		...
		onSuccess: (data: any) => {
			...
			toast({
				...
				description:
					data?.message || "Category and all children updated successfully",
			});
	...
	const toggleSubcategoryMutation = useMutation({
		...
		onSuccess: (data: any) => {
			...
			toast({
				...
				description:
					data?.message || "Subcategory and products updated successfully",
			});
			setConfirmDialog(null);
		},
		onError: (error: any) => {
	...
	const getProductsForCategoryDirectly = (categoryId: string) => {
		return products.filter(
			(p) => p.categoryId === categoryId && !p.subcategoryId,
		);
	};
	...
	const getLicenseBadge = (categoryId: string, categoryName: string) => {
		if (
			categoryId === "cat-mutual-funds" ||
			categoryName.includes("Regular Schemes")
		) {
			return {
				type: "ARN",
				label: "ARN Licensed",
		...
		if (
			categoryId === "cat-mf-direct" ||
			categoryName.includes("Direct Schemes")
		) {
			return {
				type: "RIA",
				label: "RIA Required",
		...
		<div className="space-y-6">
			<div className="flex items-center justify-between">
				<div>
					<h1
						...
						data-testid="heading-store-management"
					>
						Store Management
					</h1>
					<p className="text-muted-foreground mt-1">
						Control product and category visibility across all portals
					</p>
				</div>
				<Button
					variant="outline"
			...
			>
				<TabsList className="grid w-full grid-cols-4 max-w-2xl bg-card border-border">
					...
					<TabsTrigger
						value="portfolio-approvals"
						data-testid="tab-portfolio-approvals"
					>
				...
				<TabsContent value="hierarchy">
					<Card className="bg-card border-border">
						<CardHeader>
							...
							<CardDescription className="text-muted-foreground">
								Manage your store hierarchy. Disabling a category will cascade
								to all subcategories and products.
							</CardDescription>
						...
						<CardContent>
							{isLoadingCategories || isLoadingProducts ? (
								...
								<div className="text-center py-12 text-muted-foreground">
									...
									<p className="text-sm mt-2">
										Create categories to organize your store products
									</p>
								</div>
							) : (
												...
												>
													<div className="flex items-center gap-3">
														...
														<div className="flex items-center gap-2">
															...
															<div>
																...
																{(category.id === "cat-mf-direct" ||
																	category.name.includes("Direct Schemes")) && (
																	<p className="text-xs text-amber-400 mt-1 flex items-center gap-1">
																		<ShieldAlert className="w-3 h-3" />
																		Enable when RIA authorization is obtained.
																		Requires client advisory subscription.
																	</p>
																)}
															</div>
														</div>
													...
													<div className="flex items-center gap-4 flex-shrink-0">
														<div className="hidden md:flex items-center gap-2 text-sm">
															<Badge
																variant="outline"
																className="text-xs whitespace-nowrap text-muted-foreground border-border"
															>
															...
															<Badge
																...
																className="text-xs whitespace-nowrap text-muted-foreground border-border"
															>
																{getProductsForCategoryDirectly(category.id)
																	.length +
																	(category.subcategories || []).reduce(
																		(acc, sub) =>
																			acc +
																			getProductsForSubcategory(sub.id).length,
																		0,
																	)}{" "}
																items
															</Badge>
														...
														{(() => {
															const getSeedRoute = (cat: Category) => {
																...
																if (
																	...
																	cat.name === "Unlisted Shares"
																) {
																	return "/admin/unlisted/seed";
																}
																if (
																	slug === "fixed-income" ||
																	cat.name === "Fixed Income"
																) {
																...
																if (
																	slug === "mf-regular" ||
																	slug === "mf-direct" ||
																	cat.name.includes("Mutual Funds")
																) {
																...
																if (
																	...
																	cat.name.includes("MLD") ||
																	cat.name.includes("Market Linked")
																) {
																...
																if (
																	...
																	slug === "listed-stocks" ||
																	slug === "equities" ||
																	cat.name.includes("Listed Stocks") ||
																	cat.name.includes("Equities")
																) {
																...
																if (
																	...
																	slug === "global-stocks" ||
																	slug === "international" ||
																	cat.name.includes("Global Markets") ||
																...
																if (
																	...
																	slug === "reits-invits" ||
																	slug === "real-estate" ||
																	cat.name.includes("REIT") ||
																	cat.name.includes("InvIT") ||
																	cat.name.includes("Real Estate")
																) {
																	return "/admin/reits-invits-seed";
																}
																if (
																	slug === "tax" ||
																	cat.name.includes("Tax Services")
																) {
															...
															const getSubcategorySeedRoute = (
																sub: Subcategory,
															) => {
																const slug = sub.slug || sub.id;
																if (
																	slug === "us-market" ||
																	sub.name.includes("US Market")
																) {
																	return "/admin/global-seed?market=US";
																}
																if (
																	slug === "uk-europe-market" ||
																	sub.name.includes("UK") ||
																	sub.name.includes("Europe")
																) {
																	return "/admin/global-seed?market=UK";
																}
																if (
																	slug === "japan-market" ||
																	sub.name.includes("Japan")
																) {
																	return "/admin/global-seed?market=JP";
																}
																if (
																	slug === "china-hk-market" ||
																	sub.name.includes("China") ||
																	sub.name.includes("Hong Kong")
																) {
																	return "/admin/global-seed?market=HK";
																}
																if (
																	slug === "other-markets" ||
																	sub.name.includes("Other")
																) {
														...
														<Switch
															...
															onCheckedChange={(checked) =>
																handleToggle("category", category, checked)
															}
													...
													<div className="border-t border-border bg-card/30">
																...
																<div key={subcategory.id}>
																	...
																	>
																		...
																		<div className="flex items-center gap-4">
																				...
																				(() => {
																					const getMarketCode = (
																						...
																						if (
																							...
																							return "CN";
																						if (slug === "other-markets")
																							return "SG";
																						return null;
																					};
																					const marketCode =
																						getMarketCode(subcategory);
																					if (marketCode) {
																						return (
																							<Link
																								href={`/admin/global-seed?market=${marketCode}`}
																							>
																								<Button
																									size="sm"
																									variant="outline"
																			...
																			<Switch
																				...
																				disabled={
																					!category.isActive ||
																					toggleSubcategoryMutation.isPending
																				}
																	...
																	{expandedSubcategories.has(
																		...
																		<div className="pl-20 py-2 space-y-1">
																					...
																					<div className="flex items-center gap-2">
																						...
																						{product.trailCommission && (
																							<span
																								...
																								title="SEBI Disclosure: This Regular plan includes distributor commission"
																							>
																								Trail:{" "}
																								{(
																									Number.parseFloat(
																						...
																						{product.planType === "regular" && (
																							<Badge
																								...
																								title="SEBI Regulation: Commission is paid by AMC to distributor"
																							>
																								SEBI Disclosed
																							</Badge>
																						)}
																			...
																			{getProductsForSubcategory(subcategory.id)
																				.length === 0 && (
														...
														{getProductsForCategoryDirectly(category.id)
															...
															<div className="border-t border-border/50 pl-12 py-2">
																...
																{getProductsForCategoryDirectly(
																	category.id,
																).map((product) => (
																	<div
																		key={product.id}
												...
												return (
													...
													>
														<TableCell className="font-medium text-foreground">
															<div className="flex items-center gap-2">
																<Package
																	className={`w-4 h-4 ${product.isActive ? "text-green-400" : "text-muted-foreground"}`}
																/>
																{product.name}
															</div>
														</TableCell>
															...
															{product.planType ? (
																<div className="flex flex-col gap-1">
																	...
																	{product.trailCommission && (
																		<span
																			className="text-xs text-yellow-400 font-medium"
																			title="SEBI Disclosure: Distributor commission included"
																		>
																			Trail:{" "}
																			{(
																				Number.parseFloat(
																	...
																	{product.planType === "regular" && (
																		<Badge
																			variant="outline"
																			className="text-xs text-amber-300 border-amber-500/40 font-medium"
																			title="Commission paid by AMC to distributor as per SEBI regulations"
																		>
																			SEBI Disclosed
																		</Badge>
																	)}
														...
														<TableCell>
															<div className="flex items-center gap-2">
																...
																{isParentDisabled && (
																	<Badge
																		variant="outline"
																		className="text-xs text-yellow-300 border-yellow-500/40 font-medium"
																	>
																		Parent Off
																	</Badge>
																)}
														...
														<TableCell className="text-right">
															<Switch
																checked={product.isActive}
																onCheckedChange={(checked) =>
																	handleToggle("product", product, checked)
																}
																disabled={
																	isParentDisabled ||
																	toggleProductMutation.isPending
				...
				<TabsContent value="audit">
					<Card className="bg-card border-border">
						<CardHeader>
							...
							<CardDescription className="text-muted-foreground">
								Track all store management changes for compliance (7-year
								retention)
							</CardDescription>
						</CardHeader>
						<CardContent>
							{isLoadingAuditLogs ? (
								...
								<div className="text-center py-12 text-muted-foreground">
									...
									<p className="text-sm mt-2">
										Changes to store items will appear here
									</p>
								</div>
							) : (
								<ScrollArea className="h-[500px]">
								...
								{confirmDialog?.action === "disable" ? (
									<>
										...
										<ul className="list-disc list-inside mt-2 space-y-1">
											{confirmDialog?.type === "category" && (
												<>
													...
													<li>
														All products under this category and its
														subcategories
													</li>
												</>
											)}
										...
										<p className="mt-3 text-yellow-400">
											Customers will see an inquiry form instead of these items.
										</p>
									</>
								) : (
									<>
										<p>
											...
											will make it visible to customers.
										</p>
										{confirmDialog?.type === "category" && (
											<p className="mt-2">
												Note: Subcategories and products will need to be enabled
												individually.
											</p>
										)}
									</>
