...
function transformApiAgentToAgentData(agent: ApiAgent): AgentData {
	const totalRevenue = Number.parseFloat(agent.totalRevenue || "0");
	const activeClients = agent.activeClients || 0;

	const conversionRate =
		activeClients > 0 ? Math.min(100, 50 + activeClients * 5) : 50;
	...
	else if (agent.status === "on_leave" || agent.status === "warning")
		status = "warning";
...
export default function AgentPerformanceDashboard() {
	...
	const {
		...
		isLoading,
		refetch,
	} = useQuery<{ success: boolean; data: { data: ApiAgent[]; total: number } }>(
		{
	...
	const agents: AgentData[] = (agentsResponse?.data?.data || []).map(
		transformApiAgentToAgentData,
	);
	...
	const avgConversionRate =
		totalAgents > 0
			? agents.reduce((sum, agent) => sum + agent.conversionRate, 0) /
				totalAgents
			: 0;
			...
			<div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
				<div>
					...
					<p className="text-muted-foreground mt-1">
						Monitor and analyze all agents' performance metrics
					</p>
				</div>
				...
				<Card className="bg-card border-border">
					...
					<CardContent>
						{isLoading ? (
							...
							<div
								className="text-2xl font-bold text-foreground"
								data-testid="text-avg-conversion"
							>
								...
								{filteredAgents.map((agent) => (
									...
									>
										...
										<TableCell className="text-right text-muted-foreground">
											{formatCurrency(agent.aum)}
										</TableCell>
										<TableCell className="text-right text-muted-foreground">
											{formatCurrency(agent.revenueMTD)}
										</TableCell>
										<TableCell className="text-right text-muted-foreground">
											{agent.clients}
										</TableCell>
