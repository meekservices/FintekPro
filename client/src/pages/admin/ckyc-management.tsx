...
export default function CkycManagement() {
	...
	// Fetch CKYC records
	const { data: ckycRecords, isLoading: recordsLoading } = useQuery<
		CkycRecord[]
	>({
	...
	const createProgressStepMutation = useMutation({
		...
		onSuccess: () => {
			toast({
				title: "Success",
				description: "Progress step created successfully",
			});
	...
	const updateStatusMutation = useMutation({
		...
		onSuccess: () => {
			toast({
				title: "Success",
				description: "CKYC status updated successfully",
			});
			queryClient.invalidateQueries({ queryKey: ["/api/admin/ckyc"] });
		},
		onError: (error: Error) => {
	...
	const processNotificationsMutation = useMutation({
		mutationFn: async () => {
			return await apiRequest("/api/admin/ckyc/process-notifications", {
				method: "POST",
			});
		},
		onSuccess: () => {
			...
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-muted-foreground">
						Manage CKYC records, monitor progress, and trigger notifications
					</p>
				</div>
						...
						<CardHeader>
							<div className="flex items-center justify-between">
								<div>
									...
									<CardDescription>
										Manage and monitor CKYC verification records
									</CardDescription>
								</div>
								<div className="flex items-center gap-2">
											...
											<div className="flex items-center justify-between">
												<div className="space-y-1">
													...
													<div className="text-sm text-muted-foreground space-y-1">
														...
														{record.ckycNumber && (
															<div>CKYC: {record.ckycNumber}</div>
														)}
													</div>
												</div>
												<div className="flex items-center gap-2">
													<Dialog>
														...
														<DialogContent className="max-w-2xl">
															<DialogHeader>
																...
																<DialogDescription>
																	Complete information for {record.firstName}{" "}
																	{record.lastName}
																</DialogDescription>
															</DialogHeader>
						...
						<CardHeader>
							<div className="flex items-center justify-between">
								<div>
									...
									<CardDescription>
										Monitor and manage notification triggers
									</CardDescription>
								</div>
							</div>
						</CardHeader>
											...
											<div className="flex items-center justify-between">
												<div className="space-y-1">
													...
													<div className="text-sm text-muted-foreground">
														<div>
															Type: {trigger.triggerType.replace("_", " ")}
														</div>
														...
														{trigger.scheduledAt && (
															<div>
																Scheduled:{" "}
																{new Date(trigger.scheduledAt).toLocaleString()}
															</div>
														)}
														{trigger.sentAt && (
															<div>
						...
						<CardHeader>
							<div className="flex items-center justify-between">
								<div>
									...
									<CardDescription>
										Monitor CKYC verification progress steps
									</CardDescription>
								</div>
								<Button
				...
				<TabsContent value="logs" className="space-y-4">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Track all CKYC-related administrative actions
							</CardDescription>
						</CardHeader>
						<CardContent>
							{actionLogs?.length === 0 ? (
										...
										>
											<div className="flex items-center justify-between">
												<div className="space-y-1">
													...
													<div className="text-sm text-muted-foreground">
														<div>
															Action: {log.actionType.replace("_", " ")}
														</div>
														<div>
															By: {log.actionBy} ({log.actionByType})
														</div>
														<div>
															Time: {new Date(log.actionAt).toLocaleString()}
														</div>
													</div>
												</div>
											</div>
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="max-w-md">
				<DialogHeader>
					...
					<DialogDescription>
						Create a notification trigger for {selectedRecord?.firstName}{" "}
					...
					<div>
						...
						>
							...
							<SelectContent>
								...
								<SelectItem value="document_required">
									Document Required
								</SelectItem>
...
function ProgressStepDialog({
	open,
	onOpenChange,
	selectedRecord,
	onSubmit,
	isLoading,
}: ProgressStepDialogProps) {
	const [formData, setFormData] = useState({
		stepName: "",
		stepDescription: "",
		stepOrder: 1,
				...
				<form onSubmit={handleSubmit} className="space-y-4">
					<div>
						...
						<Select
							...
							onValueChange={(value) =>
								setFormData((prev) => ({ ...prev, stepName: value }))
							}
						>
							<SelectTrigger>
							...
							<SelectContent>
								...
								<SelectItem value="documents_uploaded">
									Documents Uploaded
								</SelectItem>
					...
					<div>
						...
						<Input
							...
							onChange={(e) =>
								setFormData((prev) => ({
									...prev,
									stepOrder: Number.parseInt(e.target.value),
								}))
							}
							min={1}
							required
					...
					<div>
						...
						<Input
							...
							onChange={(e) =>
								setFormData((prev) => ({
									...prev,
									estimatedCompletionTime: Number.parseInt(e.target.value),
								}))
							}
							min={1}
							required
...
function CkycProviderSettings() {
	...
	const { data: providersResponse, isLoading } =
		useQuery<CkycProvidersResponse>({
	...
	const healthCheckMutation = useMutation({
		mutationFn: async () => {
			return await apiRequest("/api/admin/ckyc/health-check-all", {
				method: "POST",
			});
		},
		onSuccess: () => {
	...
	const testVerifyMutation = useMutation({
		...
		onSuccess: (data) => {
			...
			toast({
				title: "Test Complete",
				description: "CKYC verification test completed",
			});
		},
		onError: (error: Error) => {
			toast({
	...
	const getProviderIcon = (code: string) => {
		switch (code) {
			case "truthscreen":
				return <LucideShield className="h-5 w-5 text-blue-600" />;
			case "sandbox":
			...
			case "nsdl":
				return <LucideShield className="h-5 w-5 text-purple-600" />;
			case "kra":
	...
	const verificationCategories = [
		{
			...
			description:
				"Identity verification via PAN card with government data sources",
			icon: <FileText className="h-5 w-5 text-amber-600" />,
			providers: [
				{
					code: "sandbox",
					name: "Sandbox.co.in",
					description: "PAN verification via Sandbox.co.in API",
					data: sandbox,
					role: "Primary",
					configured: meta?.sandboxConfigured,
				},
				{
					code: "cashfree",
					name: "Cashfree",
					description: "PAN verification via Cashfree Verification Suite",
					data: null,
					role: "Available",
					configured: meta?.cashfreeConfigured,
				},
				{
					code: "bse_star",
					name: "BSE Star MFD",
					description: "PAN verification via BSE Star MFD API",
					data: null,
					role: "Available",
					configured: meta?.bseStarConfigured,
				},
			],
		},
		{
			...
			description:
				"Aadhaar-based identity verification with OTP or offline XML",
			icon: <LucideShield className="h-5 w-5 text-green-600" />,
			providers: [
				{
					code: "sandbox",
					name: "Sandbox.co.in",
					description: "Aadhaar OTP verification via Sandbox API",
					data: sandbox,
					role: "Primary",
					configured: meta?.sandboxConfigured,
				},
				{
					code: "truthscreen",
					name: "TruthScreen",
					description: "Aadhaar eKYC & PAN-Aadhaar linkage via TruthScreen",
					data: truthscreen,
					role: "Fallback",
					configured: meta?.truthscreenConfigured,
				},
				{
					code: "cashfree",
					name: "Cashfree",
					description: "Aadhaar OKYC (OTP-based) via Cashfree API",
					data: null,
					role: "Available",
					configured: meta?.cashfreeConfigured,
				},
				{
					code: "offline_aadhaar",
					name: "Offline Aadhaar XML",
					description: "UIDAI offline XML verification (no API needed)",
					data:
						providers.find((p) => p.providerCode === "offline_aadhaar") || null,
					role: "Fallback",
					configured: true,
				},
			],
		},
		{
			title: "CKYC Lookup",
			description:
				"Central KYC (CKYC) record lookup via KIN/PAN from CERSAI registry",
			icon: <LucideShield className="h-5 w-5 text-blue-600" />,
			providers: [
				{
					code: "truthscreen",
					name: "TruthScreen",
					description: "CKYC/KRA record lookup via TruthScreen API",
					data: truthscreen,
					role: "Primary",
					configured: meta?.truthscreenConfigured,
				},
				{
					code: "nsdl",
					name: "NSDL CKYC",
					description: "Direct NSDL CKYC registry lookup",
					data: null,
					role: "Non-functional",
					configured: meta?.nsdlCkycConfigured,
				},
				{
					code: "kra",
					name: "KRA (NSDL/CVL)",
					description: "KYC Registration Agency status check via NSDL/CVL KRA",
					data: null,
					role: "Available",
					configured: meta?.kraConfigured,
				},
			],
		},
		{
			...
			providers: [
				{
					...
					name: "DigiLocker",
					description:
						"Government document pull (Aadhaar, PAN, DL) via DigiLocker API",
					data: null,
					role: "Available",
					configured: meta?.digilockerConfigured,
				},
			],
		},
	];
	...
	const otherProviders = providers.filter(
		(p) =>
			!["truthscreen", "sandbox", "offline_aadhaar"].includes(p.providerCode),
	);
				...
				<CardHeader>
					<div className="flex items-center justify-between">
						<div>
							...
							<CardDescription>
								All KYC service providers available in FintekPro, organized by
								verification type. Active providers are highlighted; configure
								credentials to enable additional providers.
							</CardDescription>
						</div>
						<Button
							variant="outline"
						...
						>
							<RefreshCw
								size={16}
								className={healthCheckMutation.isPending ? "animate-spin" : ""}
							/>
							...
							<div key={category.title} className="space-y-3">
								...
								<div className="grid gap-3 md:grid-cols-2 pl-7">
									{category.providers.map((prov) => {
										...
										return (
											<Card
												key={`${category.title}-${prov.code}`}
												className={`border transition-colors ${isNonFunctional ? "border-dashed border-muted opacity-60" : isAvailableOnly && !hasCredentials ? "border-dashed border-muted opacity-70" : provData?.isEnabled ? "border-primary/50 bg-primary/5" : "border-muted"}`}
											>
														...
														{isNonFunctional ? (
															...
															<>
																...
																{hasCredentials ? (
																	...
																	<Badge
																		variant="outline"
																		className="text-xs border-orange-300 text-orange-700 dark:text-orange-400"
																	>
																		<AlertCircle size={12} className="mr-1" />
																		Not Configured
																	</Badge>
			...
			<Card>
				...
				<CardContent className="space-y-4">
					...
					<Button
						...
						disabled={
							testVerifyMutation.isPending || !testPan || !testName || !testDob
						}
						className="flex items-center gap-2"
					>
						<Play size={16} />
						{testVerifyMutation.isPending
							? "Verifying..."
							: "Test Verification"}
					</Button>
