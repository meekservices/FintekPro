...
export default function FinancialOperations() {
	...
	if (productTypeFilter !== "all")
		ordersParams.set("productType", productTypeFilter);
	if (paymentStatusFilter !== "all")
		ordersParams.set("paymentStatus", paymentStatusFilter);
	...
	const cashfreeQueryStr =
		statusFilter !== "all" ? `?status=${statusFilter}` : "";
	const { data: cashfreeResponse } = useQuery<{
		success: boolean;
		data: Transaction[];
	}>({
		...
		queryFn: async () => {
			const res = await fetch(
				`/api/admin/financial/cashfree-transactions${cashfreeQueryStr}`,
				{ credentials: "include" },
			);
			if (!res.ok)
				throw new Error(`Failed to fetch cashfree transactions: ${res.status}`);
			return res.json();
		},
	...
	const phonePeQueryStr =
		statusFilter !== "all" ? `?state=${statusFilter}` : "";
	const { data: phonePeResponse } = useQuery<{
		success: boolean;
		data: Transaction[];
	}>({
		...
		queryFn: async () => {
			const res = await fetch(
				`/api/admin/financial/phonepe-transactions${phonePeQueryStr}`,
				{ credentials: "include" },
			);
			if (!res.ok)
				throw new Error(`Failed to fetch PhonePe transactions: ${res.status}`);
			return res.json();
		},
		enabled: activeTab === "transactions",
	});
	...
	const { data: reconciliationResponse } = useQuery<{
		success: boolean;
		data: any;
	}>({
	...
	const { data: refundsResponse } = useQuery<{
		...
		queryFn: async () => {
			const res = await fetch(
				`/api/admin/financial/refunds${refundsQueryStr}`,
				{ credentials: "include" },
			);
	...
	const formatCurrency = (amount: string | number | undefined | null) => {
		...
		return `₹${Number.parseFloat(amount.toString()).toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
	};
		...
		<div className="space-y-6">
			...
			<div>
				<h1
					...
					data-testid="heading-financial-operations"
				>
					Financial Operations
				</h1>
				<p className="text-muted-foreground">
					Manage orders, payments, revenue, and refunds
				</p>
			</div>
												...
												<TableCell>
													<div className="flex gap-2">
														<Button
															...
															variant="outline"
															onClick={() =>
																viewOrderDetailsMutation.mutate(order.id)
															}
					...
					<div className="grid gap-4 md:grid-cols-2">
						...
						<Card>
							<CardHeader>
								...
								<CardDescription>
									{cashfreeTransactions.length} transactions
								</CardDescription>
							</CardHeader>
							<CardContent>
						...
						<Card>
							<CardHeader>
								...
								<CardDescription>
									{phonePeTransactions.length} transactions
								</CardDescription>
							</CardHeader>
							<CardContent>
								<div className="space-y-4 max-h-[500px] overflow-y-auto">
						...
						<div className="grid gap-4 md:grid-cols-3">
							<Card>
								...
								<CardContent>
									<div
										className="text-2xl font-bold"
										data-testid="text-cashfree-total"
									>
										{formatCurrency(reconciliation.cashfreeTotal)}
									</div>
								</CardContent>
							</Card>
							...
							<Card>
								...
								<CardContent>
									<div
										className="text-2xl font-bold"
										data-testid="text-phonepe-total"
									>
										{formatCurrency(reconciliation.phonePeTotal)}
									</div>
								</CardContent>
							</Card>
							...
							<Card>
								...
								<CardContent>
									...
									>
										{formatCurrency(reconciliation.totalCollected)}
									</div>
								</CardContent>
							</Card>
							...
							<Card>
								...
								<CardContent>
									<div
										...
										data-testid="text-successful-payments"
									>
										{reconciliation.successfulPayments}
									</div>
								</CardContent>
							</Card>
							...
							<Card>
								...
								<CardContent>
									<div
										...
										data-testid="text-failed-payments"
									>
										{reconciliation.failedPayments}
									</div>
								</CardContent>
							</Card>
