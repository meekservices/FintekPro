...
export default function EmailIntelligencePage() {
	...
	const { data, isLoading } = useQuery({
		...
		queryFn: async () => {
			const r = await fetch(
				`/api/admin/copilot/mail/inbox?limit=50${filter ? `&urgency=${filter}` : ""}`,
			);
			return r.json();
		},
	...
	const s: any = {
		...
		chip: (u: string) => ({
			background: URGENCY_COLORS[u] + "22",
			...
			<div style={s.topbar}>
				...
				<div>
					...
					<div style={{ fontSize: "0.75rem", color: "#64748b" }}>
						Classify · Draft · Approve — never auto-send
					</div>
				</div>
				<div style={{ marginLeft: "auto", display: "flex", gap: "0.75rem" }}>
					{["", "critical", "high", "medium", "low"].map((f) => (
						<button
							...
							onClick={() => setFilter(f)}
							style={{
								...s.btn(filter === f ? "#6366f1" : "rgba(255,255,255,0.08)"),
								fontSize: "0.78rem",
				...
				<div style={s.detail}>
					{!selected ? (
						...
						<>
							...
							>
								...
								<div style={{ display: "flex", gap: "0.75rem" }}>
									...
									<button
										...
										style={s.btn("rgba(255,255,255,0.08)")}
									>
										Close
									</button>
								</div>
							...
							<InfoRow
								label="Category"
								value={CAT_LABELS[selected.category] || selected.category}
							/>
							<InfoRow
								label="Urgency"
								value={selected.urgency?.toUpperCase()}
							/>
							<InfoRow label="Intent" value={selected.intent} />
							<InfoRow
								label="Action Required"
								value={selected.actionRequired}
							/>
							...
							<InfoRow
								...
								value={`${((selected.confidenceScore || 0) * 100).toFixed(0)}%`}
							/>
							<div style={{ marginTop: "1.5rem" }}>
								...
								>
									<span>⚠️</span>
									<span>
										This draft requires Admin approval before sending. Do not
										send without review.
									</span>
								</div>
							</div>
