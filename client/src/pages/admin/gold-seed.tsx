...
import { 
  RefreshCw, Search, Loader2, ArrowLeft, 
  TrendingUp, AlertTriangle, Plus, Edit,
  Coins, Calendar, Percent, IndianRupee
} from "lucide-react";
...
export default function GoldSeedPage() {
  ...
  const { data: sgbIssuesData, isLoading: isLoadingSgb, refetch: refetchSgb } = useQuery<{ issues: SgbIssue[] }>({
    queryKey: ["/api/admin/gold/sgb-issues"],
  });

  const { data: goldProductsData, isLoading: isLoadingProducts, refetch: refetchProducts } = useQuery<{ products: GoldProduct[] }>({
  ...
  const togglePublishMutation = useMutation({
    mutationFn: async ({ id, isPublished }: { id: number; isPublished: boolean }) => {
      const response = await apiRequest(`/api/admin/gold/sgb-issues/${id}/publish`, {
        method: "PATCH",
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/gold/sgb-issues"] });
  ...
  const refreshMutation = useMutation({
    mutationFn: async () => {
      const response = await apiRequest("/api/admin/gold/refresh", { method: "POST" });
      return response;
    },
    onSuccess: () => {
      refetchSgb();
      refetchProducts();
      toast({ title: "Success", description: "Gold data refreshed from RBI/NSE" });
    },
    onError: () => {
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">Manage Sovereign Gold Bonds and gold investment products</p>
          </div>
        </div>
        <Button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Total SGB Issues</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Gold Products</CardTitle>
          </CardHeader>
          <CardContent>
            ...
            <TabsContent value="sgb-issues" className="mt-4">
              {isLoadingSgb ? (
                ...
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p className="text-sm">Click "Refresh from RBI/NSE" to fetch latest data</p>
                </div>
              ) : (
                    ...
                    <TableBody>
                      {filteredSgbIssues.map((issue) => (
                        <TableRow key={issue.id}>
                          <TableCell>
                            <div className="font-medium">{issue.seriesNumber}</div>
                            <div className="text-sm text-muted-foreground">{issue.seriesName}</div>
                          </TableCell>
                          ...
                          <TableCell>
                            <div className="font-medium">{formatCurrency(issue.issuePrice)}</div>
                            {issue.discountOnlinePayment && (
                              <div className="text-xs text-green-600">
                                Online: {formatCurrency((parseFloat(issue.issuePrice) - parseFloat(issue.discountOnlinePayment)).toString())}
                              </div>
                            )}
                          </TableCell>
                          <TableCell>
                          ...
                          <TableCell>{getStatusBadge(issue.issueStatus)}</TableCell>
                          <TableCell>
                            <Switch
                              checked={issue.isPublished}
                              onCheckedChange={(checked) => 
                                togglePublishMutation.mutate({ id: issue.id, isPublished: checked })
                              }
            ...
            <TabsContent value="gold-products" className="mt-4">
              {isLoadingProducts ? (
                ...
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p className="text-sm">Gold products from bond catalog will appear here</p>
                </div>
              ) : (
                <ScrollArea className="h-[500px]">
                  <Table>
                    ...
                    <TableBody>
                      {filteredGoldProducts.map((product) => (
                        <TableRow key={product.id}>
                          <TableCell className="font-mono text-sm">{product.isin}</TableCell>
                          <TableCell className="font-medium">{product.securityName}</TableCell>
                          <TableCell>
                            <Badge variant="outline">{product.securityType}</Badge>
                          </TableCell>
                          <TableCell>{formatDate(product.issueDate)}</TableCell>
                          <TableCell>{formatDate(product.maturityDate)}</TableCell>
                          <TableCell>{product.couponRate ? `${product.couponRate}%` : "—"}</TableCell>
                          <TableCell>{formatCurrency(product.currentPrice)}</TableCell>
                          <TableCell>{formatCurrency(product.sgbGoldPrice)}</TableCell>
                          <TableCell>
                            <Badge variant={product.tradingStatus === "active" ? "default" : "secondary"}>
