...
export default function BondCommissionSettings() {
	...
	const [editingConfig, setEditingConfig] =
		useState<BondCommissionConfig | null>(null);
	...
	const updateMutation = useMutation({
		mutationFn: (data: {
			id: string;
			updates: Partial<BondCommissionConfig>;
		}) =>
	...
	const calculateSampleFees = (
		config: BondCommissionConfig,
		amount: number = 100000,
	) => {
		const brokerage = Math.min(
			Math.max(
				(amount * Number.parseFloat(config.brokerageBps || "0")) / 10000,
				Number.parseFloat(config.brokerageMinAmount || "0"),
			),
			Number.parseFloat(config.brokerageMaxAmount || "0"),
		);

		const platformFee =
			config.platformFeeType === "fixed"
				? Number.parseFloat(config.platformFeeFixed || "0")
				: (amount * Number.parseFloat(config.platformFeePercent || "0")) / 100;

		const transactionCharge =
			(amount * Number.parseFloat(config.transactionChargeBps || "0")) / 10000;
		const stampDuty =
			(amount * Number.parseFloat(config.stampDutyBps || "0")) / 10000;
		const sebiFee =
			(amount * Number.parseFloat(config.sebiTurnoverFeeBps || "0")) / 10000;

		const subtotal = brokerage + platformFee + transactionCharge;
		const gst = (subtotal * Number.parseFloat(config.gstRate || "0")) / 100;
		...
		<div className="space-y-6 p-6" data-testid="bond-commission-settings">
			<div className="flex items-center gap-4">
				...
				<div>
					<h1 className="text-2xl font-bold text-foreground">
						Bond Commission Settings
					</h1>
					<p className="text-muted-foreground">
						Configure brokerage and platform fees for each bond type
					</p>
				</div>
			</div>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Set brokerage rates (in basis points), platform fees, and regulatory
						charges for each bond category. 1 basis point = 0.01%
					</CardDescription>
				</CardHeader>
				<CardContent>
					<Table>
						...
						<TableBody>
							{(configs || []).map((config) => {
								const IconComponent =
									bondTypeIcons[config.bondType] || IndianRupee;
								const colorClass =
									bondTypeColors[config.bondType] || "bg-muted text-foreground";
								const sampleFees = calculateSampleFees(config);

								return (
									<TableRow
										key={config.id}
										data-testid={`config-row-${config.bondType}`}
									>
										<TableCell>
										...
										<TableCell className="text-right font-mono">
											{Number.parseFloat(config.brokerageBps).toFixed(2)} bps
										</TableCell>
										<TableCell className="text-right text-sm">
											₹{Number.parseFloat(config.brokerageMinAmount).toFixed(0)}{" "}
											- ₹
											{Number.parseFloat(config.brokerageMaxAmount).toFixed(0)}
										</TableCell>
										<TableCell className="text-right">
											{config.platformFeeType === "fixed" ? (
												<span>
													₹
													{Number.parseFloat(config.platformFeeFixed).toFixed(
														0,
													)}
												</span>
											) : (
												<span>
													{Number.parseFloat(config.platformFeePercent).toFixed(
														2,
													)}
													%
												</span>
										...
										<TableCell className="text-right font-mono">
											{Number.parseFloat(config.gstRate).toFixed(0)}%
										</TableCell>
											...
											<Dialog>
												...
												<DialogContent className="max-w-2xl">
													<DialogHeader>
														...
														<DialogDescription>
															Configure brokerage and fee settings for{" "}
															{config.bondTypeLabel}
														</DialogDescription>
													</DialogHeader>
															...
															<div className="space-y-2">
																...
																<Input
																	...
																	onChange={(e) =>
																		setFormData({
																			...formData,
																			brokerageMinAmount: e.target.value,
																		})
																	}
															...
															<div className="space-y-2">
																...
																<Input
																	...
																	onChange={(e) =>
																		setFormData({
																			...formData,
																			brokerageMaxAmount: e.target.value,
																		})
																	}
														...
														<div className="grid grid-cols-3 gap-4">
															...
															<div className="space-y-2">
																...
																<Input
																	...
																	onChange={(e) =>
																		setFormData({
																			...formData,
																			platformFeeFixed: e.target.value,
																		})
																	}
																	disabled={
															...
															<div className="space-y-2">
																...
																<Input
																	...
																	disabled={
																		formData.platformFeeType === "fixed"
																	}
															...
															<div className="space-y-2">
																...
																<Input
																	...
																	onChange={(e) =>
																		setFormData({
																			...formData,
																			transactionChargeBps: e.target.value,
																		})
																	}
															...
															<div className="space-y-2">
																...
																<Input
																	...
																	onChange={(e) =>
																		setFormData({
																			...formData,
																			sebiTurnoverFeeBps: e.target.value,
																		})
																	}
																		...
																		<span className="font-mono">
																			₹
																			{
																				calculateSampleFees({
																					...config,
																					...formData,
																				} as BondCommissionConfig).brokerage
																			}
																		</span>
																	</div>
																	<div className="flex justify-between">
																		...
																		<span className="font-mono">
																			₹
																			{
																				calculateSampleFees({
																					...config,
																					...formData,
																				} as BondCommissionConfig).platformFee
																			}
																		</span>
																	</div>
																	<div className="flex justify-between">
																		...
																		<span className="font-mono">
																			₹
																			{
																				calculateSampleFees({
																					...config,
																					...formData,
																				} as BondCommissionConfig).stampDuty
																			}
																		</span>
																	</div>
																	<div className="flex justify-between">
										...
										<CardContent className="py-2 space-y-1 text-xs">
											...
											<div className="flex justify-between">
												...
												<span>
													₹
													{(
														Number.parseFloat(fees.transactionCharge) +
														Number.parseFloat(fees.gst)
													).toFixed(2)}
												</span>
											</div>
