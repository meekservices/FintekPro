		...
		case "published":
			return (
				<Badge
					...
					data-testid="badge-pricing-published"
				>
		...
		case "pending_review":
			return (
				<Badge
					...
					data-testid="badge-pricing-pending"
				>
		...
		default:
			return (
				<Badge
					...
					data-testid="badge-pricing-draft"
				>
...
const getComplianceStatusBadge = (
	...
	if (status === "blocked" || score > 70) {
		return (
			<Badge
				...
				data-testid="badge-compliance-blocked"
			>
	...
	if (status === "warning" || score > 40) {
		return (
			<Badge
				className="bg-yellow-600 text-white"
				data-testid="badge-compliance-warning"
			>
...
export default function UnlistedPricingPreviewPage() {
	...
	const {
		...
		isLoading: isValidatingPrices,
		refetch: refetchValidation,
	} = useQuery<PriceValidation>({
		queryKey: [
		...
		queryFn: async () => {
			if (!effectiveBuyPrice || !effectiveSellPrice) {
				return {
					valid: false,
					warnings: [],
					errors: ["Buy and sell prices are required"],
				};
			}
			...
			if (!response.ok) {
				return {
					valid: false,
					warnings: [],
					errors: ["Failed to validate prices"],
				};
			}
			const result = await response.json();
	...
	>({
		...
		queryFn: async () => {
			const response = await fetch(
				`/api/unlisted/companies/${companyId}/audit-log?limit=10`,
				{
					credentials: "include",
				},
			);
	...
	const refreshPricesMutation = useMutation({
		mutationFn: async () => {
			const response = await apiRequest(
				`/api/unlisted/admin/refresh-company-data/${companyId}`,
				{
					method: "POST",
				},
			);
		...
		onSuccess: () => {
			...
			toast({
				...
				description:
					"Latest price data has been fetched from external sources.",
			});
		},
		onError: (error: any) => {
			toast({
	...
	const checkComplianceMutation = useMutation({
		mutationFn: async () => {
			const response = await apiRequest(
				`/api/unlisted/companies/${companyId}/check-compliance`,
				{
					method: "POST",
				},
			);
	...
	const publishMutation = useMutation({
		mutationFn: async () => {
			const response = await apiRequest(
				`/api/unlisted/companies/${companyId}/publish-prices`,
				{
					method: "POST",
					body: JSON.stringify({
	...
	const spreadPercent =
		priceValidation?.spreadPercent ??
		(effectiveBuyPrice && effectiveSellPrice
			? ((Number.parseFloat(effectiveSellPrice) -
					Number.parseFloat(effectiveBuyPrice)) /
					Number.parseFloat(effectiveBuyPrice)) *
				100
			: undefined);

	const priceChangePercent =
		priceValidation?.priceChangePercent ??
		(company?.lastPublishedBuyPrice && effectiveBuyPrice
			? ((Number.parseFloat(effectiveBuyPrice) -
					Number.parseFloat(company.lastPublishedBuyPrice)) /
					Number.parseFloat(company.lastPublishedBuyPrice)) *
				100
			: undefined);

	const isBlocked =
		company?.complianceStatus === "blocked" || (company?.riskScore ?? 0) > 70;
	const canPublish =
		!isBlocked &&
	...
	return (
		<div className="space-y-6" data-testid="pricing-preview-page">
					...
					<div className="flex items-center gap-3">
						...
						{company.tradingSuspended && (
							<Badge
								...
								data-testid="badge-trading-suspended"
							>
								<AlertTriangle className="w-3 h-3 mr-1" />
								Trading Suspended
							</Badge>
			...
			{company.tradingSuspended && (
				<Alert
					className="border-red-600 bg-red-950/20"
					data-testid="alert-trading-suspended"
				>
					<AlertTriangle className="h-4 w-4 text-red-500" />
					<AlertTitle className="text-red-400">Trading Suspended</AlertTitle>
					<AlertDescription className="text-red-300">
						Trading for this company has been suspended. Publishing is not
						allowed until the suspension is lifted.
					</AlertDescription>
				</Alert>
			)}
			...
			<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
				...
				<Card
					...
					data-testid="card-price-preview"
				>
					<CardHeader>
					...
					<CardContent className="space-y-6">
						<div className="grid grid-cols-2 gap-4">
							<div className="bg-muted rounded-lg p-4 text-center">
								...
								>
									{effectiveBuyPrice
										? `₹${Number.parseFloat(effectiveBuyPrice).toLocaleString("en-IN")}`
										: "—"}
								</p>
							</div>
							<div className="bg-muted rounded-lg p-4 text-center">
								...
								<p
									...
									data-testid="text-sell-price"
								>
									{effectiveSellPrice
										? `₹${Number.parseFloat(effectiveSellPrice).toLocaleString("en-IN")}`
										: "—"}
								</p>
							</div>
						</div>
						...
						<div className="bg-muted/50 rounded-lg p-4">
							<div className="flex justify-between items-center">
								...
								<span
									className={`text-lg font-semibold ${getSpreadColor(spreadPercent)}`}
									data-testid="text-spread-percent"
								>
							...
							{spreadPercent !== undefined && (
								<p className="text-xs text-muted-foreground mt-1">
									{spreadPercent > 20
										? "⚠️ High spread - may reduce client interest"
										: spreadPercent >= 15
											? "⚠️ Moderate spread"
						...
						{company.lastPublishedBuyPrice &&
							priceChangePercent !== undefined && (
								<div className="bg-muted/50 rounded-lg p-4">
									<div className="flex justify-between items-center">
										<span className="text-muted-foreground">
											Change from Last Published
										</span>
										<span
											className={`flex items-center gap-1 text-lg font-semibold ${priceChangePercent >= 0 ? "text-green-400" : "text-red-400"}`}
						...
						{(priceValidation?.errors?.length ?? 0) > 0 && (
							<Alert
								className="border-red-600 bg-red-950/20"
								data-testid="alert-price-errors"
							>
						...
						{(priceValidation?.warnings?.length ?? 0) > 0 && (
							<Alert
								...
								data-testid="alert-price-warnings"
							>
								...
								<div className="grid grid-cols-3 gap-4 text-center text-sm">
									<div>
										...
										<p className="text-foreground font-medium">
											{company.faceValue ? `₹${company.faceValue}` : "—"}
										</p>
									</div>
									<div>
									...
									<div>
										...
										<p
											className={`font-medium ${getSpreadColor(spreadPercent)}`}
										>
											{spreadPercent !== undefined
										...
										<ul className="text-xs text-yellow-300/80 mt-1 space-y-0.5">
											...
											{(company.riskDisclosures?.length ?? 0) > 2 && (
												<li className="text-yellow-500">
													+{(company.riskDisclosures?.length ?? 0) - 2} more
												</li>
											)}
										</ul>
									</div>
					...
					<CardContent className="space-y-6">
						...
						{company.complianceLastChecked && (
							<div className="flex items-center gap-2 text-sm text-muted-foreground">
								...
								<span>
									Last checked:{" "}
									{format(new Date(company.complianceLastChecked), "PPp")}
								</span>
							</div>
						)}
								...
								{auditLog.map((entry) => (
									...
									>
										...
										<div className="flex-1 min-w-0">
											...
											{entry.details &&
												(entry.details.oldBuyPrice ||
													entry.details.newBuyPrice) && (
													<p className="text-xs text-muted-foreground mt-1">
														Price: {entry.details.oldBuyPrice || "—"} →{" "}
														{entry.details.newBuyPrice || "—"}
													</p>
												)}
				...
				<CardContent className="pt-6">
					<div className="flex items-center justify-between">
						...
						<div className="flex items-center gap-3">
							<Button
								...
								onClick={() =>
									navigate(
										`/admin/unlisted/preview/${companyId}?buyPrice=${effectiveBuyPrice}&sellPrice=${effectiveSellPrice}`,
									)
								}
					...
					{!canPublish && !company.tradingSuspended && (
						<p className="text-xs text-amber-400 mt-3 text-right">
							{isBlocked
								? "⚠️ Cannot publish: Compliance issues detected"
								: !effectiveBuyPrice || !effectiveSellPrice
									? "⚠️ Cannot publish: Prices required"
			...
			<Dialog open={showPublishDialog} onOpenChange={setShowPublishDialog}>
				<DialogContent
					className="bg-card border-border"
					data-testid="dialog-publish-confirm"
				>
					<DialogHeader>
						...
						<DialogDescription className="text-muted-foreground">
							You are about to publish {company.name} to the marketplace with
					...
					<div className="py-4 space-y-4">
						<div className="grid grid-cols-2 gap-4">
							<div className="bg-muted rounded-lg p-4 text-center">
								...
								<p className="text-xl font-bold text-green-400">
									₹
									{Number.parseFloat(effectiveBuyPrice || "0").toLocaleString(
										"en-IN",
									)}
								</p>
							</div>
							<div className="bg-muted rounded-lg p-4 text-center">
								...
								<p className="text-xl font-bold text-red-400">
									₹
									{Number.parseFloat(effectiveSellPrice || "0").toLocaleString(
										"en-IN",
									)}
								</p>
							</div>
						...
						<Alert className="border-amber-600 bg-amber-950/20">
							...
							<AlertDescription className="text-amber-300 text-sm">
								This action will make the company visible to all clients on the
								marketplace. Make sure all compliance checks have passed.
							</AlertDescription>
						</Alert>
					</div>
