...
function KpiRow() {
	...
	const pending = accs.filter((a: any) =>
		["SUBMITTED", "PENDING", "APPROVAL_PENDING", "ACTION_REQUIRED"].includes(
			a.status,
		),
	).length;
	const newToday = accs.filter((a: any) => {
		try {
			return new Date(a.created_at) > new Date(Date.now() - 86_400_000);
		} catch {
			return false;
		}
	}).length;
...
function AccountsTab() {
	...
	const { data: cipData, isLoading: cipLoading } = useQuery<{ cip: any }>({
		...
		queryFn: () =>
			fetch(`${BASE}/broker/accounts/${selectedAcc.id}/cip`).then((r) =>
				r.json(),
			),
	});
	...
	const closeMutation = useMutation({
		mutationFn: (id: string) =>
			apiRequest("DELETE", `${BASE}/broker/accounts/${id}`),
		onSuccess: () => {
										...
										<TableRow key={acc.id} className="hover:bg-muted/30">
											...
											<TableCell className="text-right">
												<div className="flex items-center justify-end gap-1">
													{/* CIP Dialog */}
													<Dialog
														open={cipOpen && selectedAcc?.id === acc.id}
														onOpenChange={(open) => {
															setCipOpen(open);
															if (open) setSelectedAcc(acc);
														}}
													>
													...
													<Button
														size="sm"
														variant="ghost"
														className="h-7 text-xs px-2 text-red-500 hover:text-red-700"
														onClick={() => {
															if (
...
function OrdersTab() {
	...
	const { data, isLoading, refetch } = useQuery<{
		...
		queryFn: () =>
			fetch(
				`${BASE}/broker/orders?${new URLSearchParams({
		...
		<div className="space-y-4">
			<div className="flex flex-col sm:flex-row gap-3">
				<div className="relative flex-1">
					...
					<Input
						className="pl-9"
						placeholder="Filter by Account ID (UUID)…"
						value={acctId}
								...
								{isLoading ? (
									...
									orders.map((o: any) => (
										<TableRow key={o.id} className="hover:bg-muted/30">
											...
											<TableCell>
												<span
													className={`inline-flex items-center gap-1 text-xs font-semibold ${o.side === "buy" ? "text-emerald-600" : "text-red-500"}`}
												>
													{o.side === "buy" ? (
											...
											<TableCell>
												{o.filled_avg_price ? usd(o.filled_avg_price) : "—"}
											</TableCell>
											<TableCell>
											...
											<TableCell className="text-xs text-muted-foreground">
												{fmtDateTime(o.created_at || o.submitted_at)}
											</TableCell>
										</TableRow>
									))
								)}
								...
								{isLoading ? (
									...
									positions.map((p: any) => (
										<TableRow key={p.symbol} className="hover:bg-muted/30">
											...
											<TableCell>
												<span
													className={`text-xs font-medium ${p.side === "long" ? "text-emerald-600" : "text-red-500"}`}
												>
													{p.side?.toUpperCase()}
												</span>
											</TableCell>
					...
					<DialogContent>
						...
						<div className="grid gap-4 py-2">
							...
							<div className="space-y-1.5">
								...
								<Input
									placeholder="Source account UUID"
...
function ComplianceTab() {
	...
	const accs = accounts?.accounts || [];
	const actionRequired = accs.filter(
		(a: any) => a.status === "ACTION_REQUIRED",
	);
	...
	const complianceScore =
		accs.length > 0
			? Math.round(
					(accs.filter(
						(a: any) => a.status === "ACTIVE" && a.kyc_results?.approved,
					).length /
						accs.length) *
						100,
				)
		...
		<div className="space-y-6">
			...
			<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
				<Card className="md:col-span-1">
					...
					<CardContent>
						...
						<p className="text-xs text-muted-foreground">
							...
							of {accs.length} accounts fully compliant
						</p>
					</CardContent>
				</Card>
				...
				<Card>
					<CardHeader className="pb-2">
						<CardTitle className="text-sm flex items-center gap-2">
							<AlertTriangle className="h-4 w-4 text-orange-500" /> Action
							Required
						</CardTitle>
					</CardHeader>
					<CardContent>
						...
						{actionRequired.slice(0, 3).map((a: any) => (
							<div
								key={a.id}
								className="mt-2 text-xs p-1.5 bg-orange-50 dark:bg-orange-950/20 rounded border border-orange-200 dark:border-orange-800"
							>
								{a.identity?.given_name} {a.identity?.family_name} ·{" "}
								{a.account_number?.slice(0, 8)}
							</div>
						))}
					</CardContent>
				</Card>
			...
			{actionRequired.length > 0 && (
				<Card className="border-orange-200 dark:border-orange-800">
					<CardHeader className="pb-3">
						...
						<CardDescription>
							These accounts need additional documentation or verification
						</CardDescription>
					</CardHeader>
					<CardContent className="p-0">
						<Table>
			...
			<Card>
				<CardHeader className="pb-3">
					<CardTitle className="text-sm flex items-center gap-2">
						<Globe className="h-4 w-4 text-primary" /> India LRS / FEMA
						Compliance
					</CardTitle>
					<CardDescription>
						RBI Liberalised Remittance Scheme compliance overview
					</CardDescription>
				</CardHeader>
				<CardContent>
					<div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
						{[
							{
								label: "LRS Annual Limit",
								value: "$250,000 per PAN",
								color: "text-blue-600",
							},
							{
								label: "TCS Threshold (FY)",
								value: "₹7,00,000",
								color: "text-amber-600",
							},
							{
								label: "TCS Rate Above Threshold",
								value: "20%",
								color: "text-red-600",
							},
							{
								label: "Purpose Code",
								value: "S0001 (Portfolio Investment)",
								color: "text-purple-600",
							},
							{
								label: "Form Required",
								value: "Form A2 (at AD-I bank)",
								color: "text-emerald-600",
							},
							{
								label: "FEMA Declaration",
								value: "Required per transaction",
								color: "text-sky-600",
							},
						].map(({ label, value, color }) => (
...
function ReportsTab() {
	...
	const downloadMutation = useMutation({
		mutationFn: (id: string) =>
			fetch(`${BASE}/broker/reports/${id}/download`).then((r) => r.json()),
		onSuccess: (d: any) => {
			...
			<div className="flex items-start justify-between">
				<div>
					...
					<p className="text-muted-foreground text-sm mt-0.5">
						Fully-disclosed broker-dealer dashboard — accounts, trading, funding
						& compliance
					</p>
				</div>
				<div className="flex items-center gap-2 flex-wrap justify-end">
					...
					<Badge
						variant={configData?.isBrokerApi ? "default" : "outline"}
						className="gap-1"
					>
					...
					<Button variant="outline" size="sm" asChild>
						<a
							href="https://broker-app.alpaca.markets"
							target="_blank"
							rel="noopener noreferrer"
						>
			...
			{configured && !authOk && (
				<Alert className="border-red-200 bg-red-50 dark:bg-red-950/20">
					...
					<AlertDescription className="text-sm text-red-700 dark:text-red-300">
						<strong>Alpaca auth failed (401).</strong> Update{" "}
						<code>ALPACA_SECRET_KEY</code> in environment secrets, then go to{" "}
						<a
