...
export default function GlobalAdvisoryManagement() {
	...
	const { data: marketsData, isLoading: marketsLoading } = useQuery<{
		...
		queryFn: async () => {
			const response = await fetch("/api/global-advisory/markets?all=true", {
				credentials: "include",
			});
	...
	const { data: productsData } = useQuery<{
		...
		queryFn: async () => {
			const response = await fetch("/api/global-advisory/market-products", {
				credentials: "include",
			});
			return response.json();
		},
	...
	const toggleMarketMutation = useMutation({
		mutationFn: async ({
			marketCode,
			isEnabled,
		}: { marketCode: string; isEnabled: boolean }) => {
		...
		onSuccess: () => {
			...
			toast({
				...
				description: "Market status has been updated",
			});
		},
		onError: (error: any) => {
			toast({
	...
	const updateFlagMutation = useMutation({
		mutationFn: async ({
			flagKey,
			updates,
		}: { flagKey: string; updates: Partial<FeatureFlag> }) => {
	...
	const killSwitch = flags.find(
		(f) => f.flagKey === "GLOBAL_ADVISORY_KILL_SWITCH",
	);
				...
				<div className="flex items-center gap-3">
					...
					<div>
						...
						<p className="text-muted-foreground">
							Manage markets, products, and feature flags
						</p>
					</div>
				</div>
			...
			{killSwitch?.killSwitchActivatedAt && (
				<Card className="border-destructive dark:border-red-700 bg-destructive/10 dark:bg-red-950/30">
					<CardContent className="flex items-center gap-3 p-4">
						...
						<div>
							<div className="font-semibold text-destructive dark:text-red-300">
								Global Advisory is DISABLED
							</div>
						...
						<Card key={phase}>
							<CardHeader>
								<CardTitle className="flex items-center gap-2">
									<Badge
										variant={phase === "Phase 1" ? "default" : "secondary"}
									>
										{phase}
									</Badge>
								...
								<CardDescription>
									{phase === "Phase 1"
										? "Initial rollout markets"
										: "Upcoming expansion markets"}
								</CardDescription>
							</CardHeader>
							<CardContent>
								<div className="space-y-2">
									{phaseMarkets.map((market) => {
										const marketProducts = getProductsForMarket(
											market.marketCode,
										);
										...
										return (
											<Collapsible
												...
												open={isExpanded}
												onOpenChange={() =>
													toggleMarketExpanded(market.marketCode)
												}
											>
												<div className="flex items-center justify-between p-4 border rounded-lg bg-muted/30/20">
													<div className="flex items-center gap-4">
														<CollapsibleTrigger asChild>
															<Button
																variant="ghost"
																size="sm"
																data-testid={`expand-${market.marketCode}`}
															>
																{isExpanded ? (
														...
														<div>
															...
															<div className="text-sm text-muted-foreground">
																{market.marketCode} | {market.baseCurrency} |{" "}
																{market.regulatoryBody}
															</div>
														</div>
													</div>
													...
													<div className="flex items-center gap-4">
														...
														<Badge
															variant={
																market.executionAllowed ? "default" : "outline"
															}
														>
															{market.executionAllowed
																? "Execution"
																: "No Execution"}
														</Badge>
																	...
																	<TableBody>
																		{marketProducts.map((product) => (
																			<TableRow key={product.id}>
																				...
																				<TableCell className="capitalize">
																					{product.riskCategory || "-"}
																				</TableCell>
																				<TableCell>
																					{product.etfOnlyRestriction && (
																						<Badge
																							variant="outline"
																							className="text-xs"
																						>
																					...
																					{product.requiresAccreditedInvestor && (
																						<Badge
																							variant="outline"
				...
				<TabsContent value="flags" className="space-y-4">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Control Global Advisory features
							</CardDescription>
						</CardHeader>
						<CardContent>
							<Table>
											...
											<TableCell className="text-right">
												<Switch
													...
													disabled={
														...
														(flag.isKillSwitch &&
															flag.killSwitchActivatedAt !== null)
													}
			...
			<Dialog open={killSwitchOpen} onOpenChange={setKillSwitchOpen}>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							This will immediately disable all Global Advisory features. This
							action is logged and requires justification.
						</DialogDescription>
					</DialogHeader>
					...
					<DialogFooter>
						...
						<Button
							...
							disabled={
								!killSwitchReason.trim() || activateKillSwitchMutation.isPending
							}
	...
	const { data: logsData, isLoading } = useQuery({
		...
		queryFn: async () => {
			...
			const response = await fetch(
				`/api/global-advisory/audit-logs?${params}`,
				{ credentials: "include" },
			);
			return response.json();
		},
	...
	return (
		<Card>
			<CardHeader>
				...
				<CardDescription>
					View Global Advisory activity for SEBI compliance
				</CardDescription>
			</CardHeader>
			<CardContent className="space-y-4">
				<div className="flex gap-4">
					...
					<Input
						...
						onChange={(e) =>
							setFilters((f) => ({ ...f, marketCode: e.target.value }))
						}
						className="max-w-xs"
