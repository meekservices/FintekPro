...
const TASK_TYPE_CONFIG: Record<
	string,
	{ label: string; icon: typeof LucideShield; color: string }
> = {
	kyc_renewal: {
		label: "KYC Renewal",
		icon: LucideShield,
...
export default function AdminTaskOversight() {
	...
	} = useQuery<{ success: boolean; agents: AgentTaskOverview[] }>({
	...
	const filteredAgents = agentOverview.filter((agent) => {
		...
		const matchesStatus =
			...
			(statusFilter === "non_compliant" &&
				agent.complianceStatus === "non_compliant");
		...
		<div className="space-y-6">
			<div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
				<div>
					...
					<p className="text-muted-foreground mt-1">
						Monitor all agents' tasks and compliance status
					</p>
				</div>
				...
				<Card className="bg-card border-border">
					...
					<CardContent>
						...
						<p className="text-xs text-green-400 mt-1">
							Across {stats?.agentCount || 0} agents
						</p>
					</CardContent>
				</Card>
			...
			<div className="flex flex-wrap gap-3 items-center">
				...
				<Popover>
					...
					<PopoverContent
						className="w-auto p-0 bg-muted border-border"
						align="start"
					>
						<Calendar
							mode="range"
							...
							{alertsLoading ? (
								...
								<div className="text-center py-8">
									...
									<p className="text-muted-foreground">
										No compliance alerts - all tasks on track!
									</p>
								</div>
							) : (
								complianceAlerts.map((alert) => {
									const typeConfig =
										TASK_TYPE_CONFIG[alert.taskType] || TASK_TYPE_CONFIG.custom;
