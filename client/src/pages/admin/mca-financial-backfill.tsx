...
import { 
  Database, RefreshCw, Upload, Search, Play, Pause, 
  CheckCircle, XCircle, AlertTriangle, TrendingUp, Building2
} from "lucide-react";
...
export default function McaFinancialBackfill() {
  ...
  const { data: coverageStats, isLoading: loadingStats } = useQuery<{ success: boolean; stats: CoverageStats }>({
    queryKey: ["/api/admin/mca-backfill/coverage/stats"],
  });

  const { data: needsBackfill, isLoading: loadingNeeds } = useQuery<{ success: boolean; companies: CompanyBackfillInfo[] }>({
    queryKey: ["/api/admin/mca-backfill/coverage/needs-backfill"],
  });

  const { data: schedulerStatus } = useQuery<{ success: boolean } & SchedulerStatus>({
    queryKey: ["/api/admin/mca-backfill/scheduler/status"],
    refetchInterval: 10000,
  });

  const { data: fieldCoverageData } = useQuery<{ success: boolean; totalSnapshots: number; fields: FieldCoverage[] }>({
  ...
  const bulkBackfillMutation = useMutation({
    mutationFn: async (cins: string[]) => {
      return apiRequest("POST", "/api/admin/mca-backfill/backfill-bulk", { cins });
    },
    onSuccess: (data: any) => {
      toast({
  ...
  const importMutation = useMutation({
    mutationFn: async (data: any[]) => {
      return apiRequest("POST", "/api/admin/mca-backfill/import-bulk", { data });
    },
    onSuccess: (data: any) => {
    ...
    onError: (error: any) => {
      try {
        const errorData = typeof error === 'object' ? error : JSON.parse(error.message || '{}');
  ...
  const schedulerMutation = useMutation({
    ...
    onSuccess: (data: any) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["/api/admin/mca-backfill/scheduler/status"] });
    },
    onError: (error: any) => {
      toast({
        title: "Error",
  ...
  const handleBulkBackfill = () => {
    const cins = bulkCins.split(/[\n,]/).map(c => c.trim()).filter(c => c.length === 21);
    if (cins.length === 0) {
      toast({
        title: "Invalid Input",
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            variant="outline"
            size="sm"
            onClick={() => schedulerMutation.mutate(schedulerStatus?.isRunning ? "stop" : "start")}
            disabled={schedulerMutation.isPending}
          >
            {schedulerStatus?.isRunning ? <Pause className="w-4 h-4 mr-1" /> : <Play className="w-4 h-4 mr-1" />}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium">Financial Snapshots</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <Progress value={stats?.averageCompleteness || 0} className="mt-2" />
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{schedulerStatus?.pendingJobs || 0}</div>
            <p className="text-xs text-muted-foreground">
                  ...
                  <TableBody>
                    {companies.slice(0, 20).map((company) => (
                      <TableRow key={company.cin}>
                        ...
                        <TableCell>
                          <div className="flex items-center gap-2">
                            <Progress value={company.avgCompleteness} className="w-20" />
                            <span className="text-xs">{company.avgCompleteness}%</span>
                          </div>
                        </TableCell>
                        <TableCell>
                          <Button
            ...
            <Card>
              <CardContent className="pt-6">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <div className="text-lg font-bold">{stats?.snapshotsWithRevenue || 0}</div>
            ...
            <Card>
              <CardContent className="pt-6">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <div className="text-lg font-bold">{stats?.snapshotsWithPAT || 0}</div>
                    <div className="text-xs text-muted-foreground">With PAT</div>
                  </div>
                </div>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="pt-6">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <div className="text-lg font-bold">{stats?.companiesWithoutData || 0}</div>
                    <div className="text-xs text-muted-foreground">Without Data</div>
                  </div>
                </div>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="pt-6">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <div className="text-lg font-bold">{stats?.snapshotsWithFullData || 0}</div>
                    <div className="text-xs text-muted-foreground">Complete (80%+)</div>
                  </div>
                </div>
              </CardContent>
            </Card>
                  ...
                  <TableBody>
                    {fieldCoverageData.fields.map((field) => (
                      <TableRow key={field.fieldName}>
                        ...
                        <TableCell className="text-green-600">{field.filledCount}</TableCell>
                        <TableCell className="text-red-600">{field.missingCount}</TableCell>
                        <TableCell>
                          <div className="flex items-center gap-2">
                            <Progress value={field.coverage} className="w-24" />
                            <span className="text-xs font-medium">{field.coverage}%</span>
                          </div>
                        </TableCell>
                      </TableRow>
                    ))}
            ...
            <CardContent className="space-y-4">
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  disabled={searchCin.length !== 21 || backfillMutation.isPending}
                >
        ...
        <TabsContent value="bulk">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Enter multiple CINs to backfill (one per line or comma-separated)
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <Textarea
              ...
              >
                ...
                {bulkBackfillMutation.isPending ? "Processing..." : "Start Bulk Backfill"}
              </Button>
            </CardContent>
          </Card>
        </TabsContent>
                ...
                <div className="mt-4 space-y-4">
                  <div className="grid grid-cols-4 gap-4">
                    ...
                    <div className="bg-yellow-50 dark:bg-yellow-900/20 p-3 rounded">
                      <div className="text-lg font-bold text-yellow-600">{importResult.validationErrors?.length || 0}</div>
                    ...
                    <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded p-3">
                      ...
                      <div className="max-h-40 overflow-y-auto text-sm">
                        {importResult.validationErrors?.map((err, i) => (
                          <div key={`v-${i}`} className="text-red-600 dark:text-red-400">
