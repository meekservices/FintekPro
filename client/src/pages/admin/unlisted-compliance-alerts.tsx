...
export default function UnlistedComplianceAlerts() {
	...
	const { data: statsData, isLoading: isLoadingStats } = useQuery<{
		...
		queryFn: async () => {
			const response = await fetch("/api/unlisted/admin/compliance/stats", {
				credentials: "include",
			});
	...
	const {
		...
		refetch,
	} = useQuery<{ success: boolean; data: ComplianceAlert[] }>({
		queryKey: [
			"/api/unlisted/admin/compliance/alerts",
			activeTab,
			severityFilter,
			refreshKey,
		],
		...
		<div className="space-y-6">
			<div className="flex justify-between items-center">
				<div>
					...
					<p className="text-muted-foreground mt-1">
						Monitor blocked trades, KYC failures, and high-risk activities
					</p>
				</div>
				<Button
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-4">
				...
				<Card
					...
					data-testid="card-blocked-trades"
				>
					<CardHeader className="pb-2">
						<CardDescription className="text-muted-foreground">
							Blocked Trades
