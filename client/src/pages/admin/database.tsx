...
export default function AdminDatabase() {
	...
	} = useQuery<{ success: boolean; stats: DbStats }>({
	...
	} = useQuery<{ success: boolean; tables: TableInfo[] }>({
	...
	const { data: columnsData } = useQuery<{
		...
		queryFn: async () => {
			...
			const res = await fetch(
				`/api/admin/database/tables/${selectedTable}/columns`,
			);
			return res.json();
		},
	...
	const {
		...
		queryFn: async () => {
			...
			const res = await fetch(
				`/api/admin/database/tables/${selectedTable}/data?${params}`,
			);
			return res.json();
		},
		enabled: !!selectedTable,
												...
												rows.map((row, i) => (
													<TableRow key={i}>
														{columns.map((col) => (
															<TableCell
																key={col.name}
																className="font-mono text-xs whitespace-nowrap max-w-[250px] truncate"
															>
