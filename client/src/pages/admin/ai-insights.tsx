...
export default function AdminAIInsights() {
	...
	const { data: platformInsightsData, isLoading: insightsLoading } = useQuery<
		PlatformInsight[]
	>({
		queryKey: [`/api/admin/ai-insights/platform?timeRange=${timeRange}`],
	});

	const { data: agentRecommendationsData, isLoading: recommendationsLoading } =
		useQuery<AgentRecommendation[]>({
			queryKey: ["/api/admin/ai-insights/recommendations"],
		});

	const { data: trendChartData, isLoading: trendsLoading } = useQuery<
		TrendChartData[]
	>({
	...
	const activeAlerts = platformInsights.filter(
		(i) => i.category === "risk_alerts",
	).length;
	const riskScore =
		platformInsights.length > 0
			? Math.round(
					(platformInsights.filter(
			...
			: 0;
	const trendSignals = platformInsights.filter(
		(i) => i.category === "market_trends",
	).length;
	const anomaliesDetected = platformInsights.filter(
	...
	const filteredInsights = platformInsights.filter((insight) => {
		...
		const matchesPriority =
			priorityFilter === "all" || insight.severity === priorityFilter;
		const matchesSearch =
	...
	const filteredRecommendations = agentRecommendations.filter((rec) => {
		...
		const matchesSearch =
			rec.agentName.toLowerCase().includes(searchTerm.toLowerCase()) ||
			...
			<div className="space-y-6">
				<div className="flex flex-col items-center justify-center min-h-[400px] text-center">
					...
					<p className="text-muted-foreground max-w-md">
						AI platform insights, risk alerts, and recommendations will appear
						here once the system has analyzed platform data.
					</p>
				</div>
			</div>
		);
		...
		<div className="space-y-6">
			<div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
				<div>
					...
					<p className="text-muted-foreground mt-1">
						AI-powered platform trends, risk alerts, and actionable
						recommendations
					</p>
				</div>
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
				<Card className="bg-card border-border border-l-4 border-l-red-500">
					...
					<CardContent>
						<div
							...
							data-testid="text-active-alerts"
						>
							{activeAlerts}
						</div>
				...
				<Card className="bg-card border-border border-l-4 border-l-blue-500">
					...
					<CardContent>
						<div
							className="text-2xl font-bold text-foreground"
							data-testid="text-trend-signals"
						>
							{trendSignals}
						</div>
						<p className="text-xs text-blue-400 mt-1 flex items-center gap-1">
							<Sparkles className="h-3 w-3" />2 actionable today
						</p>
					</CardContent>
				</Card>
				...
				<Card className="bg-card border-border border-l-4 border-l-purple-500">
					...
					<CardContent>
						...
						<p className="text-xs text-purple-400 mt-1 flex items-center gap-1">
							<AlertCircle className="h-3 w-3" />1 requires review
						</p>
					</CardContent>
				</Card>
			</div>
			...
			<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
				{Object.entries(CATEGORY_CONFIG).map(([category, config]) => {
					const categoryInsights = filteredInsights.filter(
				...
				<CardHeader>
					<div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
						<div>
							...
							<CardDescription className="text-muted-foreground">
								Personalized next-best-actions for each agent based on their
								client portfolio
							</CardDescription>
						</div>
					</div>
