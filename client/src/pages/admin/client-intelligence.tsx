...
export default function ClientIntelligence() {
	...
	const filteredData = intelligenceData?.filter((intel) => {
		...
		return (
			...
			intel.companyName?.toLowerCase().includes(query)
		);
	});
	...
	const filterByTab = (data: ClientIntelligence[] | undefined) => {
		...
		if (selectedTab === "high")
			return data.filter((i) => i.investmentPotential === "high");
		if (selectedTab === "medium")
			return data.filter((i) => i.investmentPotential === "medium");
		if (selectedTab === "low")
			return data.filter((i) => i.investmentPotential === "low");
		return data;
	};
	...
	const stats = {
		...
		high:
			intelligenceData?.filter((i) => i.investmentPotential === "high")
				.length || 0,
		medium:
			intelligenceData?.filter((i) => i.investmentPotential === "medium")
				.length || 0,
		low:
			intelligenceData?.filter((i) => i.investmentPotential === "low").length ||
			0,
		synced: intelligenceData?.filter((i) => i.synced).length || 0,
		avgScore:
			intelligenceData && intelligenceData.length > 0
				? (
						intelligenceData.reduce(
							(sum, i) => sum + (i.credhiveScore || i.probe42Score || 0),
							0,
		...
		<div className="space-y-6">
			...
			<div className="flex justify-between items-center">
				...
				>
					...
					{syncAllIntelligenceMutation.isPending
						? "Syncing..."
						: "Sync All Clients"}
				</Button>
			</div>
			...
			<div className="grid gap-4 md:grid-cols-4">
				...
				<Card>
					...
					<CardContent>
						<div
							...
							data-testid="text-high-potential"
						>
							{stats.high}
						</div>
						<p className="text-xs text-muted-foreground">
				...
				<Card>
					...
					<CardContent>
						<div
							...
							data-testid="text-medium-potential"
						>
							{stats.medium}
						</div>
						<p className="text-xs text-muted-foreground">
				...
				<Card>
					...
					<CardContent>
						<div
							className="text-2xl font-bold"
							data-testid="text-avg-credhive-score"
						>
							{stats.avgScore}/5
						</div>
						<p className="text-xs text-muted-foreground">
								...
								<div className="text-center py-12">
									...
									<p className="text-muted-foreground">
										{searchQuery
											...
											: "No client intelligence data available"}
									</p>
								</div>
							) : (
										...
										>
											<div className="flex items-start justify-between">
													...
													<div className="flex items-center gap-3 mb-3">
														<div
															className={`p-2 rounded-full ${
																intel.investmentPotential === "high"
																	? "bg-green-100 dark:bg-green-900/30"
																	: intel.investmentPotential === "medium"
														...
														>
															{(intel.credhiveScore ||
																...
																<TrendingUp className="h-5 w-5 text-green-600" />
																	...
																	intel.probe42Score ||
																	0) <= 2 &&
																(intel.credhiveScore || intel.probe42Score) ? (
														...
														<div>
															<h3
																...
																data-testid={`text-client-name-${intel.id}`}
															>
																{intel.userName}
															</h3>
													...
													<div className="grid grid-cols-2 md:grid-cols-5 gap-4">
														<div>
															...
															<p className="font-semibold">
																{intel.credhiveScore || intel.probe42Score
																	? `${intel.credhiveScore || intel.probe42Score}/5`
																	: "N/A"}
															</p>
														</div>
														<div>
														...
														<div>
															...
															<p className="font-semibold">
																{intel.annualRevenue
																	? `₹${(Number.parseFloat(intel.annualRevenue) / 10000000).toFixed(2)}Cr`
																	: "N/A"}
															</p>
														</div>
														<div>
															...
															<p className="font-semibold text-green-600">
																{intel.investableSurplus
																	? `₹${(Number.parseFloat(intel.investableSurplus) / 100000).toFixed(2)}L`
																	: "N/A"}
															</p>
														</div>
													</div>
												...
												<Button
													...
													variant="outline"
													onClick={() =>
														syncIntelligenceMutation.mutate(intel.userId)
													}
											...
											{intel.lastUpdated && (
												<p className="text-xs text-muted-foreground mt-3">
													Last updated:{" "}
													{new Date(intel.lastUpdated).toLocaleString()}
												</p>
											)}
										</div>
									))}
