...
export default function SmsInbox() {
	...
	const { data: combinedUnreadCount, refetch: refetchUnread } =
		useQuery<CombinedUnreadCount>({
	...
	const {
		...
		queryFn: async () => {
			...
			const response = await fetch(
				`/api/twilio/admin/messages?${params.toString()}`,
			);
	...
	const {
		...
		queryFn: async () => {
			...
			const response = await fetch(
				`/api/twilio/admin/calls?${params.toString()}`,
			);
	...
	const updateCallbackMutation = useMutation({
		mutationFn: async ({
			...
			status,
			notes,
		}: { callId: string; status: string; notes?: string }) => {
		...
		switch (status) {
			case "pending":
				return (
					<Badge
						variant="outline"
						className="bg-yellow-50 dark:bg-yellow-950/30 text-yellow-700 dark:text-yellow-300 border-yellow-200 dark:border-yellow-800"
					>
						Pending
					</Badge>
				);
		...
		<div className="container mx-auto p-6 space-y-6">
				...
				<div className="flex items-center gap-3">
					...
					<Button
						...
						disabled={
							...
							(combinedUnreadCount?.messageCount ?? 0) === 0
						}
					>
			...
			<div className="grid grid-cols-1 md:grid-cols-5 gap-4">
				...
				<Card>
					...
					<CardContent>
						<div className="text-2xl font-bold text-orange-600">
							{combinedUnreadCount?.messageCount || 0}
						</div>
					</CardContent>
				</Card>
				<Card>
					...
					<CardContent>
						<div className="text-2xl font-bold">
							{messagesData?.messages.filter((m) => m.channel === "sms")
								.length || 0}
						</div>
					</CardContent>
				</Card>
				<Card>
					...
					<CardContent>
						<div className="text-2xl font-bold">
							{messagesData?.messages.filter((m) => m.channel === "whatsapp")
								.length || 0}
						</div>
					</CardContent>
				</Card>
						...
						<div className="text-2xl font-bold text-purple-600">
							...
							{(combinedUnreadCount?.callCount || 0) > 0 && (
								<span className="text-sm text-orange-600 ml-2">
									({combinedUnreadCount?.callCount} new)
								</span>
							)}
						</div>
					</CardContent>
				...
				<CardHeader>
					<div className="flex items-center justify-between">
						<div>
							...
							<CardDescription>
								All SMS and WhatsApp replies received on your Twilio number
							</CardDescription>
						</div>
						<div className="flex items-center gap-3">
									...
									{isLoading ? (
										...
										<div className="text-center py-12 text-muted-foreground">
											...
											<p className="text-sm mt-2">
												When users reply to your SMS or WhatsApp messages, they
												will appear here.
											</p>
										</div>
									) : (
																...
																>
																	...
																	<DialogContent>
																		<DialogHeader>
																			...
																			<DialogDescription>
																				Send a reply to{" "}
																				{selectedMessage.fromNumber} via{" "}
																...
																>
																	...
																	<DialogContent>
																		<DialogHeader>
																			...
																			<DialogDescription>
																				Add a note to this message for future
																				reference.
																			</DialogDescription>
																		</DialogHeader>
																		<Textarea
															...
															<div className="border-t pt-4">
																...
																<div className="flex items-center gap-2 mt-1">
																	...
																	<div className="flex flex-col">
																		<span className="font-medium">
																			{selectedMessage.userFirstName ||
																			selectedMessage.userLastName
								...
								<TabsContent value="calls" className="mt-4">
									{isLoadingCalls ? (
										...
										<div className="text-center py-12 text-muted-foreground">
											...
											<p>No calls found</p>
											<p className="text-sm mt-2">
												When users call your Twilio number, they will appear
												here.
											</p>
										</div>
									) : (
																...
																>
																	...
																	<DialogContent>
																		<DialogHeader>
																			...
																			<DialogDescription>
																				Update the callback status for this
																				call.
																			</DialogDescription>
																		</DialogHeader>
																		<div className="space-y-4">
																...
																>
																	...
																	<DialogContent>
																		<DialogHeader>
																			<DialogTitle>Add Admin Note</DialogTitle>
																			<DialogDescription>
																				Add a note to this call for future
																				reference.
																			</DialogDescription>
																		</DialogHeader>
																		<Textarea
														...
														<div className="space-y-3">
															...
															<div className="grid grid-cols-2 gap-4">
																<div>
																	...
																	<p>
																		{format(
																			new Date(selectedCall.callStartedAt),
																			"PPpp",
																		)}
																	</p>
																</div>
														...
														{selectedCall.userId && (
															<div className="border-t pt-4">
																...
																<div className="flex items-center gap-2 mt-1">
																	...
																	<div className="flex flex-col">
																		<span className="font-medium">
																			{selectedCall.userFirstName ||
																			selectedCall.userLastName
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Configure these webhook URLs in your Twilio Console to receive
						incoming messages
					</CardDescription>
				</CardHeader>
					...
					<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
						<div className="p-4 border rounded-lg">
							...
							<p className="text-xs text-muted-foreground mt-2">
								Set this URL in Twilio Console → Phone Numbers → +17623851291 →
								Messaging
							</p>
						</div>
						<div className="p-4 border rounded-lg">
							...
							<div className="mt-2 p-2 bg-muted rounded font-mono text-sm break-all">
								{typeof window !== "undefined"
									? `${window.location.origin}/api/twilio/whatsapp/webhook`
						...
						<div className="p-4 border rounded-lg">
							...
							<div className="mt-2 p-2 bg-muted rounded font-mono text-sm break-all">
								{typeof window !== "undefined"
									? `${window.location.origin}/api/twilio/voice/webhook`
