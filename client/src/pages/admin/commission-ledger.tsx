...
export default function CommissionLedgerPage() {
  ...
  const { data: ledger, isLoading: ledgerLoading, refetch: refetchLedger } = useQuery<{ data: CommissionEntry[] }>({
    ...
    queryFn: async () => {
      ...
      const res = await fetch(`/api/loan-commission/ledger?${params.toString()}`);
      return res.json();
    },
  });

  const { data: summary, isLoading: summaryLoading } = useQuery<{ data: CommissionSummary }>({
  ...
  const { data: calculatedCommission, refetch: calculateCommission } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch("/api/loan-commission/calculate", {
        ...
        body: JSON.stringify({ loanAmount: calcAmount, productType: calcProduct }),
      });
      return res.json();
    },
    enabled: false,
  ...
  const updateStatusMutation = useMutation({
    ...
    onSuccess: () => {
      toast({ title: "Status Updated", description: "Commission status has been updated" });
      queryClient.invalidateQueries({ queryKey: ["/api/loan-commission/ledger"] });
    },
    onError: () => {
    ...
    <div className="container mx-auto py-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">Track and manage DSA loan commissions</p>
        </div>
        <Button onClick={() => refetchLedger()} variant="outline" className="gap-2">
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-muted-foreground">Total Gross Commission</p>
                    ...
                    {ledger?.data?.length === 0 ? (
                      ...
                      ledger?.data?.map((entry) => (
                        <TableRow key={entry.id}>
                          <TableCell className="font-mono text-sm">{entry.applicationId}</TableCell>
                          <TableCell>
                            <Badge variant="outline">{entry.providerId}</Badge>
                          </TableCell>
                          <TableCell className="capitalize">{entry.productId}</TableCell>
                                ...
                                <div className="space-y-4">
                                  <div className="grid grid-cols-2 gap-4 text-sm">
                                    <div>
                                      ...
                                      <p className="font-medium">{formatCurrency(entry.loanAmount)}</p>
                                    </div>
                                    <div>
                                    ...
                                    <div>
                                      <p className="text-muted-foreground">Gross Commission</p>
                                      <p className="font-medium text-green-600">{formatCurrency(entry.grossCommission)}</p>
                                    </div>
                                    <div>
                                      <p className="text-muted-foreground">TDS ({entry.tdsRate}%)</p>
                                      <p className="font-medium text-red-600">-{formatCurrency(entry.tdsAmount)}</p>
                                    </div>
                                    <div>
                                      <p className="text-muted-foreground">GST ({entry.gstRate}%)</p>
                                      <p className="font-medium">{formatCurrency(entry.gstAmount)}</p>
                                    </div>
                                    <div>
                                      <p className="text-muted-foreground">Net Commission</p>
                                      <p className="font-medium text-blue-600">{formatCurrency(entry.netCommission)}</p>
                                    </div>
                                  </div>
                                  <hr />
                                  <div className="space-y-2">
                                  ...
                                  {entry.status === "pending" && (
                                    <div className="flex gap-2">
                                      ...
                                      <Button
                                        size="sm"
                                        variant="outline"
                                        onClick={() => updateStatusMutation.mutate({ id: entry.id, status: "invoiced" })}
                                      >
                                        Mark Invoiced
                                      </Button>
                                    </div>
                ...
                <TableBody>
                  {rates?.data?.map((rate) => (
                    <TableRow key={rate.productType}>
                      <TableCell className="font-medium capitalize">{rate.productType}</TableCell>
                      <TableCell className="text-right">{rate.minRate}%</TableCell>
                      <TableCell className="text-right">{rate.maxRate}%</TableCell>
                      <TableCell className="text-right font-medium text-green-600">{rate.defaultRate}%</TableCell>
                      <TableCell className="text-right">{rate.tdsRate}%</TableCell>
                      <TableCell className="text-right">{rate.fintekProShare}%</TableCell>
                      <TableCell className="text-right">{rate.partnerShare}%</TableCell>
                      <TableCell className="text-right">{rate.agentShare}%</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
                  ...
                  <div>
                    ...
                    <Select value={calcProduct} onValueChange={setCalcProduct}>
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="securities">Loan Against Securities</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                ...
                {calculatedCommission?.data && (
                  <div className="bg-muted p-4 rounded-lg space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      <div>
                        <p className="text-muted-foreground">Loan Amount</p>
                        <p className="font-medium">{formatCurrency(calculatedCommission.data.breakdown.loanAmount)}</p>
                      </div>
                      <div>
                        <p className="text-muted-foreground">Rate Applied</p>
                        <p className="font-medium">{calculatedCommission.data.commissionRate}%</p>
                      </div>
                      <div>
                        <p className="text-muted-foreground">Gross Commission</p>
                        <p className="font-medium text-green-600">{formatCurrency(calculatedCommission.data.grossCommission)}</p>
                      </div>
                      <div>
                        <p className="text-muted-foreground">TDS Deduction</p>
                        <p className="font-medium text-red-600">-{formatCurrency(calculatedCommission.data.tdsAmount)}</p>
                      </div>
                      <div>
                        <p className="text-muted-foreground">Net Commission</p>
                        <p className="font-medium text-blue-600">{formatCurrency(calculatedCommission.data.netCommission)}</p>
                      </div>
                    </div>
                    <hr />
        ...
        <TabsContent value="analytics" className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Card>
              ...
              <CardContent>
                {summaryData?.byProvider && Object.keys(summaryData.byProvider).length > 0 ? (
                  <div className="space-y-3">
                    {Object.entries(summaryData.byProvider).map(([provider, data]) => (
                      <div key={provider} className="flex justify-between items-center p-3 bg-muted rounded">
                        <div>
                          <p className="font-medium">{provider}</p>
                          <p className="text-sm text-muted-foreground">{data.count} loans</p>
                        </div>
            ...
            <Card>
              ...
              <CardContent>
                {summaryData?.byProduct && Object.keys(summaryData.byProduct).length > 0 ? (
                  <div className="space-y-3">
                    {Object.entries(summaryData.byProduct).map(([product, data]) => (
                      <div key={product} className="flex justify-between items-center p-3 bg-muted rounded">
                        <div>
