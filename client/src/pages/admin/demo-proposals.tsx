...
export default function DemoProposalsTracking() {
  ...
  const [convertDialog, setConvertDialog] = useState<{ open: boolean; proposal: DemoProposal | null }>({ open: false, proposal: null });

  // Fetch demo proposals
  const { data: demoProposals = [], isLoading, refetch } = useQuery<DemoProposal[]>({
  ...
  const convertMutation = useMutation({
    mutationFn: (proposalId: string) => apiRequest(`/api/admin/demo-proposals/${proposalId}/convert`, { method: 'POST' }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/admin/demo-proposals'] });
      queryClient.invalidateQueries({ queryKey: ['/api/admin/demo-proposals/stats'] });
      setConvertDialog({ open: false, proposal: null });
      toast({
        title: "Demo Converted",
        description: "The demo proposal has been converted to a real investment proposal",
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const sampleDemos: DemoProposal[] = [
    ...
    },
    {
      ...
      isDemo: true,
      demoViewCount: 2,
      demoLastViewedAt: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString(),
      demoConvertedAt: null,
      demoConvertedBy: null,
    ...
    },
    {
      ...
      agentId: "a3",
      agentName: "Amit Desai",
      title: "Conservative Debt Portfolio",
      description: "Low-risk debt-focused portfolio",
      proposalSource: "hybrid",
    ...
    },
    {
      ...
      clientEmail: "kiran@example.com",
      agentId: "a2",
      agentName: "Vikram Singh",
      title: "SIP Growth Plan",
      description: "Monthly SIP in diversified funds",
      proposalSource: "agent",
      totalInvestmentAmount: "300000",
      status: "converted",
      isDemo: true,
      demoViewCount: 6,
      demoLastViewedAt: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000).toISOString(),
      demoConvertedAt: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000).toISOString(),
  ...
  const calculatedStats: DemoStats = stats || {
    ...
    converted: demos.filter(d => d.status === 'converted' || d.demoConvertedAt).length,
    pending: demos.filter(d => d.status === 'pending' && !d.demoConvertedAt).length,
    expired: demos.filter(d => d.status === 'expired').length,
    conversionRate: (demos.filter(d => d.status === 'converted' || d.demoConvertedAt).length / demos.length) * 100,
    avgTimeToConvert: 5.2,
    totalDemoValue: demos.reduce((sum, d) => sum + parseFloat(d.totalInvestmentAmount || '0'), 0),
    convertedValue: demos.filter(d => d.status === 'converted' || d.demoConvertedAt).reduce((sum, d) => sum + parseFloat(d.totalInvestmentAmount || '0'), 0),
  };

  // Filter demos
  const filteredDemos = demos.filter(demo => {
    const matchesSearch = demo.clientName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    const matchesSource = sourceFilter === "all" || demo.proposalSource === sourceFilter;
    return matchesSearch && matchesStatus && matchesSource;
  });

  // Funnel data
  const funnelData = [
    { name: "Demos Created", value: calculatedStats.totalDemos, fill: "#3b82f6" },
    { name: "Viewed (3+ times)", value: demos.filter(d => d.demoViewCount >= 3).length, fill: "#8b5cf6" },
    { name: "Engaged", value: demos.filter(d => d.demoViewCount >= 5).length, fill: "#f59e0b" },
    { name: "Converted", value: calculatedStats.converted, fill: "#10b981" },
  ];

  // Source distribution
  const sourceData = [
    { name: "AI Generated", value: demos.filter(d => d.proposalSource === 'ai').length, fill: "#3b82f6" },
    { name: "Agent Created", value: demos.filter(d => d.proposalSource === 'agent').length, fill: "#10b981" },
    { name: "Hybrid", value: demos.filter(d => d.proposalSource === 'hybrid').length, fill: "#8b5cf6" },
  ];
  ...
  const getSourceBadge = (source: string) => {
    if (source === 'ai') {
      return <Badge variant="outline" className="text-blue-400 border-blue-500/30"><Sparkles className="h-3 w-3 mr-1" />AI</Badge>;
    }
    if (source === 'agent') {
      return <Badge variant="outline" className="text-green-400 border-green-500/30">Agent</Badge>;
    }
    return <Badge variant="outline" className="text-purple-400 border-purple-500/30">Hybrid</Badge>;
  };
    ...
    <div className="space-y-6">
        ...
        <Card className="bg-slate-900 border-slate-800">
          ...
          <CardContent>
            ...
            <div className="flex items-center mt-2">
              ...
              <span className="text-xs text-green-400">+5.2% vs last month</span>
            </div>
          </CardContent>
        </Card>
                    ...
                    <TableRow key={demo.id} className="border-slate-700 hover:bg-slate-800/50">
                      ...
                      <TableCell className="text-right">
                        {!demo.demoConvertedAt && demo.status !== 'converted' && demo.status !== 'expired' && (
                          <Button
                            size="sm"
                            onClick={() => setConvertDialog({ open: true, proposal: demo })}
      ...
      <Dialog open={convertDialog.open} onOpenChange={(open) => setConvertDialog({ open, proposal: open ? convertDialog.proposal : null })}>
        <DialogContent className="bg-slate-900 border-slate-700">
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="bg-green-500/10 border border-green-500/30 rounded-lg p-4">
                <p className="text-sm text-green-400">
                  Converting this demo will mark it as a real investment proposal and enable payment processing.
                </p>
              </div>
            </div>
          )}
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setConvertDialog({ open: false, proposal: null })}>
              Cancel
            </Button>
            <Button
              onClick={() => convertDialog.proposal && convertMutation.mutate(convertDialog.proposal.id)}
