...
export default function AdminAgentPayoutsPage() {
	...
	} = useQuery<{ success: boolean; data: PayoutClaim[] }>({
		queryKey: ["/api/admin/agent-payouts/payout-claims", statusFilter],
		queryFn: async () => {
			const response = await fetch(
				`/api/admin/agent-payouts/payout-claims?status=${statusFilter}`,
			);
	...
	const reviewMutation = useMutation({
		mutationFn: async ({
			...
			action,
			remarks,
		}: { claimId: string; action: "approve" | "reject"; remarks?: string }) => {
			return apiRequest(
				`/api/admin/agent-payouts/payout-claims/${claimId}/review`,
				{
					method: "POST",
					body: JSON.stringify({
						action,
		...
		onSuccess: () => {
			...
			queryClient.invalidateQueries({
				queryKey: ["/api/admin/agent-payouts/payout-claims"],
			});
			queryClient.invalidateQueries({
				queryKey: ["/api/admin/agent-payouts/stats"],
			});
	...
	const markPaidMutation = useMutation({
		mutationFn: async ({
			claimId,
			paymentReference,
		}: { claimId: string; paymentReference?: string }) => {
			return apiRequest(
				`/api/admin/agent-payouts/payout-claims/${claimId}/mark-paid`,
				{
					method: "POST",
					body: JSON.stringify({
		...
		<div className="container mx-auto p-6 space-y-6">
			<div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
				<div>
					...
					<p className="text-muted-foreground">
						Review and process agent commission payouts
					</p>
				</div>
					...
					<Card>
						...
						<CardContent>
							...
							<p className="text-xs text-muted-foreground">
								{stats.totalPending} claims pending
							</p>
						</CardContent>
					</Card>
					<Card>
						...
						<CardContent>
							...
							<p className="text-xs text-muted-foreground">
								{stats.totalApproved} claims approved
							</p>
						</CardContent>
					</Card>
					<Card>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Review and process agent commission claims
					</CardDescription>
				</CardHeader>
				<CardContent>
					{loadingClaims ? (
						...
						<div className="text-center py-12 text-muted-foreground">
							...
							<p>No claims found</p>
							<p className="text-sm">
								No payout claims match the selected filter
							</p>
						</div>
					) : (
									...
									<div className="flex items-start justify-between">
											...
											<div className="flex items-center gap-4">
												<div>
													...
													<span className="font-medium ml-1">
														{formatCurrency(claim.disbursedAmount || "0")}
													</span>
												</div>
												<div>
													<span className="text-sm text-muted-foreground">
										...
										<div className="flex items-center gap-2">
											...
											{claim.status === "approved" && (
												<Button size="sm" onClick={() => handleMarkPaid(claim)}>
							...
							<div className="grid grid-cols-2 gap-4 text-sm">
								...
								<div>
									...
									<p className="font-medium">
										{selectedClaim.agentName || selectedClaim.agentEmail}
									</p>
								</div>
								<div>
									<Label className="text-muted-foreground">Application</Label>
									<p className="font-medium">
										{selectedClaim.applicationNumber}
									</p>
								</div>
								<div>
								...
								<div>
									...
									<p className="font-medium">
										{loanTypeLabels[selectedClaim.loanType] ||
											selectedClaim.loanType}
									</p>
								</div>
								<div>
									...
									<p className="font-medium">
										{formatCurrency(selectedClaim.disbursedAmount || "0")}
									</p>
								</div>
								<div>
							...
							{selectedClaim.paymentDate && (
								<div>
									...
									<p className="font-medium text-emerald-600">
										Paid on{" "}
										{format(new Date(selectedClaim.paymentDate), "dd MMM yyyy")}
					...
					<DialogFooter>
						...
						<Button
							...
							disabled={
								...
								(actionType === "reject" && !adminRemarks.trim())
							}
						>
							{reviewMutation.isPending ? (
								<>
			...
			<Dialog open={payDialogOpen} onOpenChange={setPayDialogOpen}>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							Record the payment for this payout claim
						</DialogDescription>
					</DialogHeader>
					{selectedClaim && (
						<div className="space-y-4">
