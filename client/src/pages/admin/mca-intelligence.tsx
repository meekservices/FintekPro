...
export default function McaIntelligence() {
		...
		if (payment) {
			if (payment === "success") {
				toast({
					...
					description: amount
						? `API credits added: ₹${amount}`
						: "API credits have been added",
				});
				...
				toast({
					...
					description:
						"Your payment is being processed. It will be credited shortly.",
				});
	...
	const ingestMutation = useMutation({
		...
		onSuccess: (data) => {
			if (data.success) {
				toast({
					title: "XBRL ingested successfully",
					description: data.message,
				});
	...
	const companyIngestMutation = useMutation({
		...
		onSuccess: (data) => {
			if (data.success) {
				toast({
					title: "Company ingested successfully",
					description: `${data.data?.companyName} added to database`,
				});
				setCompanyForm({
					cin: "",
					companyName: "",
				...
				queryClient.invalidateQueries({
					queryKey: ["/api/mca/profitable-companies"],
				});
			} else {
				toast({
					title: "Company ingest failed",
	...
	const financialIngestMutation = useMutation({
		...
		onSuccess: (data) => {
			if (data.success) {
				toast({
					title: "Financial data ingested",
					description: `FY ${data.data?.financialYear} added`,
				});
				setFinancialForm({
					cin: "",
					financialYear: "",
		...
		if (value.trim()) {
			try {
				...
				if (parsed.companies || parsed.financials) {
					...
					setBulkError(
						'JSON must contain "companies" and/or "financials" arrays',
					);
				}
			} catch (e) {
			...
			<div className="flex items-center justify-between">
				...
				<div className="flex items-center gap-2">
					<Badge variant="secondary" className="text-sm py-1 px-3">
						...
						API Requests:{" "}
						{apiUsage?.requestsThisMonth || stats?.totalQueries || 0} this month
					</Badge>
							...
							<Card>
								...
								<CardContent>
									<div className="grid grid-cols-2 md:grid-cols-3 gap-4">
										...
										<div>
											...
											<p className="text-xl font-bold">
												{apiUsage?.totalRequests || stats?.totalQueries || 0}
											</p>
										</div>
										<div>
											<p className="text-sm text-muted-foreground">
									...
									<div className="mt-4 p-3 bg-muted rounded-lg">
										<p className="text-sm text-muted-foreground">
											<strong>Direct Billing:</strong> Each MCA API request is
											billed directly to your Sandbox.co.in account. Add credits
											at{" "}
											<a
												href="https://dashboard.sandbox.co.in/billing"
												target="_blank"
				...
				<TabsContent value="query" className="space-y-6">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Execute queries against MCA data. All queries are logged for
								audit.
							</CardDescription>
						</CardHeader>
							...
							<div className="grid grid-cols-2 gap-4">
								<div>
									...
									<Select value={queryType} onValueChange={setQueryType}>
										...
										<SelectContent>
											<SelectItem value="company_lookup">
												Company Lookup
											</SelectItem>
											<SelectItem value="financial_availability">
												Financial Availability
											</SelectItem>
											<SelectItem value="last_filed_aoc4">
												Last Filed AOC-4
											</SelectItem>
											<SelectItem value="profit_check">
												Profit Check (&gt;₹1 Cr)
											</SelectItem>
											<SelectItem value="filing_status">
												Filing Status
											</SelectItem>
											<SelectItem value="wallet_check">
												API Usage Stats
											</SelectItem>
											<SelectItem value="charges_analysis">
												Charges Analysis
											</SelectItem>
										</SelectContent>
									</Select>
						...
						<CardContent className="space-y-4">
							...
							<div className="grid grid-cols-4 gap-4">
								...
								<div className="flex items-end">
									<Button
										onClick={() => refetchProfitable()}
										disabled={loadingProfitable}
									>
										{loadingProfitable ? (
											<Loader2 className="h-4 w-4 mr-2 animate-spin" />
										) : (
							...
							{loadingProfitable ? (
								...
								<div className="border rounded-lg">
									<Table>
										...
										<TableBody>
											{profitableData?.result &&
											profitableData.result.length > 0 ? (
												profitableData.result.map(
													(company: ProfitableCompany, index: number) => (
														<TableRow
															key={`${company.cin}-${company.financialYear}-${index}`}
														>
												...
												<TableRow>
													...
													>
														No profitable companies found. Try adjusting filters
														or ingest more data.
													</TableCell>
												</TableRow>
											)}
				...
				<TabsContent value="ingest" className="space-y-6">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Ingest company and financial data directly via JSON. No XBRL
								parsing required.
							</CardDescription>
						</CardHeader>
						<CardContent>
							<Tabs value={ingestSubTab} onValueChange={setIngestSubTab}>
									...
									<div className="grid grid-cols-2 gap-4">
										...
										<div>
											...
											<Select
												...
												onValueChange={(v) =>
													setCompanyForm({ ...companyForm, companyStatus: v })
												}
											>
												<SelectTrigger>
													<SelectValue />
												...
												<SelectContent>
													...
													<SelectItem value="Under Liquidation">
														Under Liquidation
													</SelectItem>
								...
								<TabsContent value="financial" className="space-y-4">
										...
										<div>
											...
											<Input
												...
												onChange={(e) =>
													setFinancialForm({
														...financialForm,
														longTermBorrowing: e.target.value,
													})
												}
										...
										<div>
											...
											<Input
												...
												onChange={(e) =>
													setFinancialForm({
														...financialForm,
														shortTermBorrowing: e.target.value,
													})
												}
												placeholder="50000000"
											/>
									...
									<div className="flex items-center gap-2 p-3 bg-muted rounded-lg text-sm">
										...
										<span>
											Company must exist in database before adding financial
											data.
										</span>
									</div>
									<Button
										onClick={() =>
											financialIngestMutation.mutate(financialForm)
										}
										disabled={
											financialIngestMutation.isPending ||
											!financialForm.cin ||
											!financialForm.financialYear ||
											financialForm.cin.length !== 21 ||
											!/^\d{4}-\d{2}$/.test(financialForm.financialYear)
										}
										className="mt-4"
									>
										{financialIngestMutation.isPending ? (
  ...
  "financials": [
    {
      ...
      "profitAfterTax": "75000000"
									...
									<div className="flex items-center gap-2 p-3 bg-muted rounded-lg text-sm">
										...
										<span>
											Bulk ingest processes companies first, then financials.
									...
									<Button
										onClick={() =>
											bulkPreview?.data &&
											bulkIngestMutation.mutate(bulkPreview.data)
										}
									...
									<div className="space-y-2">
										...
										{auditData.data.filter(
											...
											<p className="text-sm text-muted-foreground text-center py-4">
												No recent ingest activity. Use the forms above to add
												data.
											</p>
										)}
									</div>
					...
					<Card>
						...
						<CardContent>
							<div className="text-center py-12 text-muted-foreground">
								...
								<p className="text-sm">
									Use "Ingest XBRL" to add financial data from AOC-4 filings.
								</p>
							</div>
						</CardContent>
					</Card>
