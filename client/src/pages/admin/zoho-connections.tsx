...
export default function ZohoConnectionsPage() {
	...
	const handleSetupConnection = async () => {
		if (!connectionName || selectedServices.length === 0) {
			toast({
				...
				description:
					"Please provide a connection name and select at least one service",
				variant: "destructive",
			});
			return;
		}
	...
	const handleDeleteConnection = async (connectionId: string) => {
		if (
			!confirm(
				"Are you sure you want to delete this connection? This will revoke access and remove all sync mappings.",
			)
		) {
			return;
		}
						...
						<div className="space-y-4 py-4">
							...
							<div className="space-y-2">
								...
								<Select value={dataCenter} onValueChange={setDataCenter}>
									<SelectTrigger
										id="data-center"
										data-testid="select-data-center"
									>
										<SelectValue />
									</SelectTrigger>
									<SelectContent>
				...
				<Card>
					<CardContent className="flex flex-col items-center justify-center py-12">
						...
						<p className="text-muted-foreground text-center mb-4">
							Connect FintekPro to Zoho to enable CRM, Books, Desk, and
							WorkDrive integrations
						</p>
						<Button
							onClick={() => setIsSetupOpen(true)}
							data-testid="button-setup-first-connection"
						>
									...
									<div className="flex items-start justify-between">
										<div className="flex-1">
											...
											<CardDescription className="mt-1">
												Data Center: {connection.zohoDataCenter} • Created:{" "}
										...
										<div className="flex gap-2">
											...
											>
												<RefreshCw
													className={`w-4 h-4 ${isRefreshing === connection.id ? "animate-spin" : ""}`}
												/>
											</Button>
											<Button
												size="sm"
												...
												<div className="space-y-2">
													<div className="flex justify-between text-sm">
														...
														<span className="font-medium">
															{rateLimit.percentUsed.toFixed(1)}%
														</span>
													</div>
													<div className="w-full bg-secondary rounded-full h-2">
														<div
															className={`h-2 rounded-full transition-all ${
																rateLimit.percentUsed > 90
																	? "bg-destructive"
																	: rateLimit.percentUsed > 70
																		? "bg-yellow-500"
																		: "bg-primary"
															}`}
															style={{
																width: `${Math.min(rateLimit.percentUsed, 100)}%`,
															}}
														/>
													</div>
													<div className="flex justify-between text-xs text-muted-foreground">
														...
														<span>
															{rateLimit.maxTokens.toLocaleString()} total
														</span>
													</div>
												</div>
											</div>
