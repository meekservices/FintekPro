...
export default function GlobalFeeModelAdmin() {
	...
	const [overrideMode, setOverrideMode] = useState<
		"ADVISORY_PLATFORM" | "PLATFORM_ONLY"
	>("ADVISORY_PLATFORM");
	...
	} = useQuery<{ success: boolean; settings: AdminSettings }>({
		queryKey: ["/api/fee-mode/admin/settings"],
	});

	const { data: statsData, isLoading: statsLoading } = useQuery<{
		success: boolean;
		statistics: FeeModeStats;
	}>({
	...
	const updateSettingsMutation = useMutation({
		...
		onSuccess: () => {
			toast({
				...
				description: "Fee model policy has been saved successfully.",
			});
	...
	const overrideModeMutation = useMutation({
		...
		onSuccess: () => {
			toast({
				...
				description: "Client fee mode has been changed.",
			});
			queryClient.invalidateQueries({ queryKey: ["/api/fee-mode/admin"] });
					...
					<Card>
						...
						<CardContent>
							<div className="text-2xl font-bold text-amber-600">
								{stats.advisoryPlatformCount}
							</div>
							<p className="text-xs text-muted-foreground">
								{stats.totalClients > 0
									? (
											(stats.advisoryPlatformCount / stats.totalClients) *
											100
										).toFixed(1)
									: 0}
								% of clients
					...
					<Card>
						...
						<CardContent>
							<div className="text-2xl font-bold text-blue-600">
								{stats.platformOnlyCount}
							</div>
							<p className="text-xs text-muted-foreground">
								{stats.totalClients > 0
									? (
											(stats.platformOnlyCount / stats.totalClients) *
											100
										).toFixed(1)
									: 0}
								% of clients
						...
						<CardContent className="space-y-6">
								...
								<div className="space-y-4">
									<div className="flex items-center justify-between p-4 border rounded-lg">
										<div>
											<Label className="text-base font-medium">
												Enable Platform-Only Mode
											</Label>
											<p className="text-sm text-muted-foreground">
												Allow clients to choose execution-only access without
												advisory
											</p>
										</div>
										<Switch
											checked={
												isEditing
													? editedSettings.enablePlatformOnlyMode ??
														settings?.enablePlatformOnlyMode
													: settings?.enablePlatformOnlyMode
											}
											disabled={!isEditing}
											onCheckedChange={(checked) =>
												setEditedSettings((prev) => ({
													...prev,
													enablePlatformOnlyMode: checked,
												}))
											}
									...
									<div className="flex items-center justify-between p-4 border rounded-lg">
										...
										<Switch
											checked={
												isEditing
													? editedSettings.allowClientSelfSelection ??
														settings?.allowClientSelfSelection
													: settings?.allowClientSelfSelection
											}
											disabled={!isEditing}
											onCheckedChange={(checked) =>
												setEditedSettings((prev) => ({
													...prev,
													allowClientSelfSelection: checked,
												}))
											}
									...
									<div className="p-4 border rounded-lg">
										...
										<div className="flex gap-2">
											<Button
												variant={
													(isEditing
														? editedSettings.defaultFeeMode
														: settings?.defaultFeeMode) === "ADVISORY_PLATFORM"
														? "default"
														: "outline"
												}
												className="flex-1"
												...
												onClick={() =>
													setEditedSettings((prev) => ({
														...prev,
														defaultFeeMode: "ADVISORY_PLATFORM",
													}))
												}
											>
											...
											<Button
												variant={
													(isEditing
														? editedSettings.defaultFeeMode
														: settings?.defaultFeeMode) === "PLATFORM_ONLY"
														? "default"
														: "outline"
												}
												className="flex-1"
							...
							{settings?.lastUpdatedBy && (
								<Alert>
									...
									<AlertDescription>
										Policy v{settings.policyVersion} was last modified on{" "}
				...
				<TabsContent value="fees" className="mt-6">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Set advisory and platform fees in basis points (bps). 100 bps =
								1%
							</CardDescription>
						</CardHeader>
						<CardContent>
									...
									<div className="space-y-3">
										<div>
											...
											<div className="flex items-center gap-2 mt-1">
												<Input
													...
													value={
														isEditing
															? editedSettings.advisoryFeeBps ??
																settings?.advisoryFeeBps
															: settings?.advisoryFeeBps
													}
													onChange={(e) =>
														setEditedSettings((prev) => ({
										...
										<div>
											...
											<Input
												...
												value={
													(isEditing
														? editedSettings.advisoryFeeCapInr
														: settings?.advisoryFeeCapInr) ?? ""
												}
												onChange={(e) =>
													setEditedSettings((prev) => ({
														...prev,
														advisoryFeeCapInr: e.target.value
									...
									<div className="space-y-3">
										...
										<div>
											...
											<Input
												...
												value={
													(isEditing
														? editedSettings.platformFeeCapInr
														: settings?.platformFeeCapInr) ?? ""
												}
												onChange={(e) =>
													setEditedSettings((prev) => ({
														...prev,
														platformFeeCapInr: e.target.value
															? Number.parseInt(e.target.value)
															: null,
													}))
												}
								...
								<div className="grid grid-cols-3 gap-4 text-sm">
									...
									<div>
										...
										<span className="font-mono ml-2 text-amber-600">
											...
											{(
												(100000 *
													((settings?.advisoryFeeBps || 25) +
														(settings?.platformFeeBps || 10))) /
												10000
											).toFixed(2)}
										</span>
									</div>
									<div>
										...
										<span className="font-mono ml-2 text-blue-600">
											₹
											{(
												(100000 * (settings?.platformFeeBps || 10)) /
												10000
											).toFixed(2)}
										</span>
									</div>
			...
			<Dialog open={overrideDialogOpen} onOpenChange={setOverrideDialogOpen}>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							Override a client's fee mode selection. This action is logged for
							SEBI compliance.
						</DialogDescription>
					</DialogHeader>
					<div className="space-y-4 py-4">
						...
						<div>
							...
							<div className="flex gap-2 mt-2">
								<Button
									variant={
										overrideMode === "ADVISORY_PLATFORM" ? "default" : "outline"
									}
									className="flex-1"
								...
								<Button
									variant={
										overrideMode === "PLATFORM_ONLY" ? "default" : "outline"
									}
									className="flex-1"
					...
					<DialogFooter>
						...
						<Button
							...
							disabled={overrideModeMutation.isPending}
						>
							{overrideModeMutation.isPending
								? "Applying..."
								: "Apply Override"}
						</Button>
					</DialogFooter>
