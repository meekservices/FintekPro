	...
	useEffect(() => {
		if (data?.configs) {
			...
			data.configs.forEach((cfg: any) => {
				if (cfg.configKey === "PLATFORM_FEE_PERCENT")
					newFees.PLATFORM_FEE_PERCENT = Number(cfg.configValue);
				if (cfg.configKey === "REFERRAL_BONUS_PERCENT")
					newFees.REFERRAL_BONUS_PERCENT = Number(cfg.configValue);
			});
			setFees(newFees);
		}
	}, [data]);
				...
				<CardContent className="space-y-8">
					<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
						...
						<div className="space-y-4 p-4 rounded-lg border bg-card">
							...
							<p className="text-sm text-muted-foreground">
								The percentage of the total transaction value kept by FintekPro
								for CA marketplace consultations.
							</p>
						...
						<div className="space-y-4 p-4 rounded-lg border bg-card">
							...
							<p className="text-sm text-muted-foreground">
								The percentage of the total transaction value paid to the
								referring partner (CA/Sub-broker).
							</p>
							<div className="flex items-end gap-3">
								<div className="flex-1 space-y-1.5">
									<Label>Current Percentage (%)</Label>
									<Input
										...
										onChange={(e) =>
											setFees({
												...fees,
												REFERRAL_BONUS_PERCENT: Number(e.target.value),
											})
										}
										className="font-mono"
									/>
					...
					<div className="p-4 rounded-lg bg-emerald-50 border border-emerald-100 dark:bg-emerald-950 dark:border-emerald-900 flex items-start gap-3">
						...
						<div className="text-sm text-emerald-800 dark:text-emerald-200">
							...
							<p>
								Updates to these settings are automatically captured in{" "}
								<strong>Regulatory Audit Packs</strong> for all subsequent
								transactions, ensuring full fee transparency for SEBI/AMFI
								audits.
							</p>
						</div>
					</div>
