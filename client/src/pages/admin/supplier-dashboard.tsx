...
export default function SupplierDashboard() {
	...
	const { data: productPerformanceData, isLoading: performanceLoading } =
		useQuery<any>({
			queryKey: ["/api/admin/product-performance", selectedSupplier],
		});
	const productPerformance = Array.isArray(productPerformanceData)
		? productPerformanceData
		: productPerformanceData?.performance || [];

	// Fetch optimization suggestions
	const { data: optimizationSuggestionsData, isLoading: suggestionsLoading } =
		useQuery<any>({
			queryKey: ["/api/admin/product-optimization"],
		});
	const optimizationSuggestions = Array.isArray(optimizationSuggestionsData)
		? optimizationSuggestionsData
		: optimizationSuggestionsData?.suggestions || [];
	...
	const promoteProductMutation = useMutation({
		...
		onSuccess: () => {
			toast({
				...
				description:
					"Product has been successfully promoted with high visibility.",
			});
	...
	const totalRevenue =
		productPerformance?.reduce(
			(sum: number, product: ProductPerformance) => sum + product.revenue,
			0,
		) || 0;
	const averageMargin =
		productPerformance?.reduce(
			(sum: number, product: ProductPerformance) => sum + product.profitMargin,
			0,
		) / (productPerformance?.length || 1) || 0;
	const highMarginProducts =
		productPerformance?.filter(
			(product: ProductPerformance) => product.profitMargin > 15,
		).length || 0;
	const promotedProducts =
		productPerformance?.filter(
			(product: ProductPerformance) => product.isPromoted,
		).length || 0;
		...
		<div className="space-y-6">
			...
			<div className="flex justify-between items-center">
				<div>
					...
					<p className="text-muted-foreground">
						Optimize product performance and profit margins
					</p>
				</div>
					...
					<Dialog>
						...
						<DialogContent>
							<DialogHeader>
								...
								<DialogDescription>
									Register a new supplier to track product performance and
									commissions.
								</DialogDescription>
							</DialogHeader>
			...
			<Tabs defaultValue="performance" className="space-y-6">
				<ScrollableTabsList>
					<TabsTrigger
						value="performance"
						data-testid="tab-product-performance"
					>
						...
						<CardHeader>
							<div className="flex justify-between items-center">
								<div>
									...
									<CardDescription>
										Track sales, profit margins, and revenue by product
									</CardDescription>
								</div>
								<div className="flex gap-2">
									<Select
									...
									<TableBody>
										{productPerformance?.map((product: ProductPerformance) => (
											<TableRow key={product.id}>
												...
												<TableCell>
													<span
														className={`font-semibold ${getProfitColor(product.profitMargin)}`}
													>
				...
				<TabsContent value="optimization" className="space-y-6">
					<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
						...
						<Card>
							<CardHeader>
								...
								<CardDescription>
									AI-powered recommendations to maximize profits
								</CardDescription>
							</CardHeader>
							<CardContent>
								{suggestionsLoading ? (
											...
											(
												...
												<div key={index} className="border rounded-lg p-4">
													<div className="flex justify-between items-start mb-2">
														...
														<Badge
															className={getPriorityBadge(suggestion.priority)}
														>
													...
													<div className="flex justify-between items-center">
														<div>
															<p className="text-sm text-muted-foreground">
																Current Margin: {suggestion.currentMargin}%
															</p>
							...
							<CardContent className="space-y-4">
								<div className="space-y-3">
									...
									<div className="flex justify-between items-center">
										...
										<div className="flex items-center gap-2">
											<Progress
												value={Math.min(averageMargin * 5, 100)}
												className="w-24"
											/>
											<span className="text-sm font-medium">
												{averageMargin.toFixed(1)}%
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Promote high-margin products and optimize performance
							</CardDescription>
						</CardHeader>
						<CardContent>
				...
				<TabsContent value="suppliers" className="space-y-6">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Manage supplier relationships and commission rates
							</CardDescription>
						</CardHeader>
						<CardContent>
							{suppliersLoading ? (
