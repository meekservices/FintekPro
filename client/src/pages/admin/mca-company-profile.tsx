...
function getStatusBadge(status: string): {
	...
	if (statusLower.includes("strike") || statusLower.includes("dormant")) {
...
function getRatioBadge(
	value: number | null,
	type: "margin" | "roe" | "roa" | "de" | "cagr",
	...
	switch (type) {
		...
		case "cagr":
			if (value >= 20)
				return { color: "text-green-600", label: "Strong Growth" };
...
function getRiskGradeStyle(grade: string): {
	bg: string;
	text: string;
	label: string;
} {
	...
	return (
		<Card>
			<CardHeader className="pb-3">
				...
				<CardDescription>
					Composite risk score based on financial and compliance metrics
				</CardDescription>
			</CardHeader>
			<CardContent>
				<div className="flex items-center gap-6 mb-6">
					<div
						className={`w-24 h-24 rounded-full flex items-center justify-center ${gradeStyle.bg}`}
					>
						<div className="text-center">
							<div className={`text-3xl font-bold ${gradeStyle.text}`}>
								{riskGrade}
							</div>
					...
					<div>
						<div className={`text-lg font-semibold ${gradeStyle.text}`}>
							{gradeStyle.label} Risk
						</div>
	...
	return (
		<Card>
			<CardHeader className="pb-3">
				...
				<CardDescription>
					Key performance metrics and growth indicators
				</CardDescription>
			</CardHeader>
			<CardContent>
				<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
					<div className="p-4 bg-muted rounded-lg text-center">
						...
						<div
							className={`text-2xl font-bold ${getRatioBadge(ratios.patMargin, "margin").color}`}
						>
							{ratios.patMargin !== null
								? `${ratios.patMargin.toFixed(1)}%`
								: "—"}
						</div>
						<Badge
							variant="outline"
							className={`text-xs mt-1 ${getRatioBadge(ratios.patMargin, "margin").color}`}
						>
					...
					<div className="p-4 bg-muted rounded-lg text-center">
						...
						<div
							className={`text-2xl font-bold ${getRatioBadge(ratios.returnOnEquity, "roe").color}`}
						>
						...
						<Badge
							variant="outline"
							className={`text-xs mt-1 ${getRatioBadge(ratios.returnOnEquity, "roe").color}`}
						>
					...
					<div className="p-4 bg-muted rounded-lg text-center">
						...
						<div
							className={`text-2xl font-bold ${getRatioBadge(ratios.returnOnAssets, "roa").color}`}
						>
							{ratios.returnOnAssets !== null
								? `${ratios.returnOnAssets.toFixed(1)}%`
								: "—"}
						</div>
						<Badge
							variant="outline"
							className={`text-xs mt-1 ${getRatioBadge(ratios.returnOnAssets, "roa").color}`}
						>
					...
					<div className="p-4 bg-muted rounded-lg text-center">
						...
						<div
							className={`text-2xl font-bold ${getRatioBadge(ratios.debtToEquity, "de").color}`}
						>
						...
						<Badge
							variant="outline"
							className={`text-xs mt-1 ${getRatioBadge(ratios.debtToEquity, "de").color}`}
						>
						...
						<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
							{growth.revenueCAGR !== null && (
								<div className="flex items-center gap-2">
									<div
										className={`text-lg font-semibold ${getRatioBadge(growth.revenueCAGR, "cagr").color}`}
									>
										{growth.revenueCAGR >= 0 ? "▲" : "▼"}{" "}
										{Math.abs(growth.revenueCAGR).toFixed(1)}%
									</div>
							...
							{growth.patCAGR !== null && (
								<div className="flex items-center gap-2">
									<div
										className={`text-lg font-semibold ${getRatioBadge(growth.patCAGR, "cagr").color}`}
									>
										{growth.patCAGR >= 0 ? "▲" : "▼"}{" "}
										{Math.abs(growth.patCAGR).toFixed(1)}%
									</div>
									<span className="text-sm text-muted-foreground">
										PAT CAGR
									</span>
							...
							{growth.revenueYoY !== null && (
								<div className="flex items-center gap-2">
									<div
										className={`text-lg font-semibold ${growth.revenueYoY >= 0 ? "text-green-600" : "text-red-600"}`}
									>
										{growth.revenueYoY >= 0 ? "▲" : "▼"}{" "}
										{Math.abs(growth.revenueYoY).toFixed(1)}%
									</div>
									<span className="text-sm text-muted-foreground">
										Revenue YoY
									</span>
							...
							{growth.patYoY !== null && (
								<div className="flex items-center gap-2">
									<div
										className={`text-lg font-semibold ${growth.patYoY >= 0 ? "text-green-600" : "text-red-600"}`}
									>
										{growth.patYoY >= 0 ? "▲" : "▼"}{" "}
										{Math.abs(growth.patYoY).toFixed(1)}%
									</div>
		...
		return (
			<div className="min-h-screen bg-background p-6">
				<div className="max-w-4xl mx-auto space-y-6">
					<Button
						variant="ghost"
						onClick={() => setLocation("/admin/mca-intelligence")}
					>
					...
					<Card className="border-destructive">
						...
						<CardContent className="text-center space-y-4">
							<p className="text-muted-foreground">
								The company may not exist in the MCA database, or there was an
				...
				<Card>
					<CardHeader>
						...
						<div className="flex flex-wrap gap-2 mt-3">
							...
							{company.class && (
								<Badge variant="outline">{company.class}</Badge>
							)}
							{company.industry && (
				...
				<Tabs value={activeTab} onValueChange={setActiveTab}>
					<TabsList className="grid w-full grid-cols-4">
						...
						<TabsTrigger value="directors">
							Directors ({summary.totalDirectors})
						</TabsTrigger>
						<TabsTrigger value="charges">
							Charges ({summary.activeCharges})
						</TabsTrigger>
						<TabsTrigger value="financials">
							Financials ({summary.financialYears})
						</TabsTrigger>
					</TabsList>
					...
					<TabsContent value="overview" className="space-y-4">
						<div className="grid md:grid-cols-2 gap-4">
							<Card>
								...
								<CardContent className="space-y-3">
									...
									<div className="flex justify-between">
										...
										<span className="font-medium">
											{formatDate(company.lastBalanceSheet)}
										</span>
									</div>
									<Separator />
									<div className="flex justify-between">
										...
										<span className="font-medium">
											{formatDate(company.lastAnnualReturn)}
										</span>
									</div>
								</CardContent>
							</Card>
									...
									<div className="flex justify-between">
										...
										<div className="flex items-center gap-2">
											<Progress
												value={
													company.authorizedCapital && company.paidUpCapital
														? (Number.parseFloat(company.paidUpCapital) /
																Number.parseFloat(company.authorizedCapital)) *
															100
														: 0
												}
												className="w-20 h-2"
											...
											<span className="text-sm">
												{company.authorizedCapital && company.paidUpCapital
													? `${((Number.parseFloat(company.paidUpCapital) / Number.parseFloat(company.authorizedCapital)) * 100).toFixed(1)}%`
													: "—"}
											</span>
										</div>
									</div>
							...
							<Card className="md:col-span-2">
								...
								<CardContent>
									<p className="text-sm">
										{company.registeredAddress || "Not available"}
									</p>
									<div className="flex gap-4 mt-3 text-sm text-muted-foreground">
										...
										{company.registeredState && (
											<span>{company.registeredState}</span>
										)}
										{company.email && (
					...
					<TabsContent value="directors">
						<Card>
							<CardHeader>
								...
								<CardDescription>
									Current and past directors/signatories
								</CardDescription>
							</CardHeader>
							<CardContent>
								{directors.length === 0 ? (
											...
											{directors.map((director, idx) => (
												<TableRow key={director.din || idx}>
													...
													<TableCell className="text-right">
														{director.activeAppointments} /{" "}
														{director.totalAppointments}
													</TableCell>
												</TableRow>
											))}
										</TableBody>
					...
					<TabsContent value="charges">
						<Card>
							<CardHeader>
								...
								<CardDescription>
									Securities registered with the Registrar of Companies
								</CardDescription>
							</CardHeader>
							<CardContent>
								{charges.length === 0 ? (
										...
										<TableBody>
											{charges.map((charge, idx) => (
												<TableRow key={charge.chargeId || idx}>
													...
													<TableCell>
														{formatDate(charge.creationDate)}
													</TableCell>
													<TableCell>
														<Badge
															variant={
					...
					<TabsContent value="financials" className="space-y-4">
						{financials.length > 0 && <FinancialRatiosCard cin={cin} />}
						<Card>
							<CardHeader>
								<CardTitle>Financial History</CardTitle>
								<CardDescription>
									Year-wise financial performance data with YoY growth
								</CardDescription>
							</CardHeader>
							<CardContent>
								{financials.length === 0 ? (
									<div className="text-center py-8 text-muted-foreground">
										...
										<p className="text-sm mt-2">
											Financial details will be populated when AOC-4 filings are
											ingested
										</p>
									</div>
								) : (
										...
										<Table>
											...
											<TableBody>
												{financials.map((f, idx) => {
													...
													const revenueGrowth =
														prevF && Number.parseFloat(prevF.revenue || "0") > 0
															? ((Number.parseFloat(f.revenue || "0") -
																	Number.parseFloat(prevF.revenue || "0")) /
																	Number.parseFloat(prevF.revenue || "1")) *
																100
															: null;
													const patGrowth =
														prevF &&
														Number.parseFloat(prevF.profitAfterTax || "0") > 0
															? ((Number.parseFloat(f.profitAfterTax || "0") -
																	Number.parseFloat(
																		prevF.profitAfterTax || "0",
																	)) /
																	Number.parseFloat(
																		prevF.profitAfterTax || "1",
																	)) *
																100
															: null;
														...
														<TableRow key={f.financialYear || idx}>
															...
															<TableCell className="text-right">
																<div className="flex flex-col items-end">
																	...
																	{patGrowth !== null && (
																		<span
																			className={`text-xs ${patGrowth >= 0 ? "text-green-600" : "text-red-600"}`}
																		>
																			{patGrowth >= 0 ? "▲" : "▼"}{" "}
																			{Math.abs(patGrowth).toFixed(1)}%
																		</span>
																	)}
																</div>
															</TableCell>
				...
				<Card className="bg-muted/50">
					<CardContent className="py-4">
						<div className="flex items-center justify-between text-xs text-muted-foreground">
							...
							<span>
								Last updated: {formatDate(meta.lastUpdated)} | Source:{" "}
								{meta.dataSource}
							</span>
						</div>
					</CardContent>
