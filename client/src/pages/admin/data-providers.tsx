...
function StatusBadge({
	isHealthy,
	isAvailable,
}: { isHealthy: boolean; isAvailable: boolean }) {
	if (!isAvailable)
		return (
			<Badge
				variant="outline"
...
export default function AdminDataProviders() {
	...
	const resetMutation = useMutation({
		mutationFn: async (providerName?: string) => {
			return apiRequest("POST", "/api/admin/data-providers/reset-metrics", {
				providerName,
			});
		},
		onSuccess: () => {
			...
			toast({
				title: "Metrics Reset",
				description: "Provider metrics have been reset.",
			});
		},
	});
	...
	const testMutation = useMutation({
		mutationFn: async (symbol: string) => {
			const res = await fetch(
				`/api/admin/data-providers/test/registry?symbol=${encodeURIComponent(symbol)}`,
			);
		...
		onSuccess: (data) => {
			if (data.test?.hasData) {
				toast({
					title: "Test Passed",
					description: `${data.test.provider}: ${data.test.data?.companyName} (${data.test.latencyMs}ms)`,
				});
			} else {
				toast({
					title: "No Data",
					description: "No data returned from any provider",
					variant: "destructive",
				});
			}
			refetchHealth();
	...
	if (healthLoading)
		return <LoadingState message="Loading provider health..." />;
		...
		<div className="space-y-6 p-6">
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
						Monitor financial data provider status, fallback behavior, and API
						usage
					</p>
				</div>
					...
					<CardContent className="pt-6">
						<div className="flex items-center justify-between">
							<div>
								...
								<p className="text-2xl font-bold">
									{healthData?.fallbacksTriggered || 0}
								</p>
							</div>
			...
			<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
				{providers.map((provider) => (
					<Card
						key={provider.name}
						className={`${!provider.isHealthy ? "border-red-300 dark:border-red-700" : "border-green-200 dark:border-green-800"}`}
					>
						...
						<CardContent className="space-y-4">
							...
							<div className="space-y-1 text-xs">
								...
								<div className="flex justify-between">
									...
									<span className={provider.lastFailure ? "text-red-500" : ""}>
										{formatTime(provider.lastFailure)}
									</span>
								</div>
								{provider.lastError && (
									<div
										className="mt-1 p-2 bg-red-50 dark:bg-red-950 rounded text-red-600 dark:text-red-400 text-xs truncate"
							...
							<div className="flex gap-2 pt-1">
								<Button
									...
									size="sm"
									className="text-xs"
									onClick={() => resetMutation.mutate(provider.name)}
									disabled={resetMutation.isPending}
								>
									Reset Metrics
								</Button>
			...
			<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
				<Card>
					<CardHeader>
						...
						<CardDescription>
							Daily API call consumption per provider
						</CardDescription>
					</CardHeader>
					<CardContent className="space-y-4">
						{usageLoading ? (
								...
								{fmpUsage && (
									<div className="space-y-2">
										<div className="flex justify-between text-sm">
											...
											<span>
												{fmpUsage.dailyCalls} / {fmpUsage.maxDaily}
											</span>
										</div>
										<Progress
											value={
												fmpUsage.maxDaily > 0
													? (fmpUsage.dailyCalls / fmpUsage.maxDaily) * 100
													: 0
											}
											className="h-2"
										/>
										<p className="text-xs text-gray-500 dark:text-gray-400">
											{fmpUsage.remaining} calls remaining today
										</p>
									</div>
								)}
								{avUsage && (
									<div className="space-y-2 mt-4">
										<div className="flex justify-between text-sm">
											...
											<span>
												{avUsage.dailyCalls} / {avUsage.maxDaily}
											</span>
										</div>
										<Progress
											value={
												avUsage.maxDaily > 0
													? (avUsage.dailyCalls / avUsage.maxDaily) * 100
													: 0
											}
											className="h-2"
										/>
										<p className="text-xs text-gray-500 dark:text-gray-400">
											{avUsage.remaining} calls remaining today |{" "}
											{avUsage.minuteCalls}/{avUsage.maxPerMinute} this minute
										</p>
									</div>
								)}
							</>
				...
				<Card>
					<CardHeader>
						...
						<CardDescription>
							Test data retrieval with fallback chain
						</CardDescription>
					</CardHeader>
					<CardContent className="space-y-4">
						<div className="flex gap-2">
							...
							/>
							<Button
								onClick={() => testMutation.mutate(testSymbol)}
							...
							<div className="p-3 bg-gray-50 dark:bg-gray-900 rounded-lg text-sm space-y-1">
								...
								{testMutation.data.test.data && (
									<>
										<div className="flex justify-between">
											...
											<span className="font-medium">
												{testMutation.data.test.data.companyName}
											</span>
										</div>
										<div className="flex justify-between">
										...
										<div className="flex justify-between">
											...
											<span>
												{testMutation.data.test.data.marketCap
													? `$${(testMutation.data.test.data.marketCap / 1e9).toFixed(1)}B`
													: "N/A"}
											</span>
										</div>
									</>
								...
								{!testMutation.data.test.hasData && (
									<p className="text-red-500 text-xs">
										No data returned from any provider
									</p>
								)}
							</div>
						)}
