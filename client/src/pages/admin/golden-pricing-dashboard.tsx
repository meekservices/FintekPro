...
export default function GoldenPricingDashboard() {
	...
	const runDailyMutation = useMutation({
		...
		onSuccess: () => {
			toast({
				...
				description: "Daily golden pricing run has been initiated.",
			});
		},
		onError: (e: any) =>
			toast({
	...
	const overrideMutation = useMutation({
		mutationFn: async (data: any) =>
			apiRequest("POST", "/api/pricing/override", data),
		onSuccess: () => {
			toast({
				title: "Price Overridden",
				description: "The price has been manually updated and audited.",
			});
	...
	const batchFetchMutation = useMutation({
		mutationFn: async (isins: string[]) =>
			apiRequest("POST", "/api/pricing/batch", { isins }),
		onSuccess: () => {
			toast({
				title: "Batch Fetch Complete",
				description: "Price sync for selected ISINs is complete.",
			});
		},
		onError: (e: any) =>
			toast({
		...
		<div className="container mx-auto p-6 space-y-6">
			<div className="flex justify-between items-center">
				<div>
					<h1 className="text-3xl font-bold tracking-tight">
						Golden Pricing Engine
					</h1>
					<p className="text-muted-foreground">
						Bloomberg-grade institutional pricing dashboard
					</p>
				</div>
				<div className="flex gap-2">
					<Button
						variant="outline"
						onClick={() => batchFetchMutation.mutate(["INE001A01036"])}
					>
				...
				<TabsContent value="live" className="space-y-4">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Search for the latest golden price of any instrument
							</CardDescription>
						</CardHeader>
						<CardContent>
							<div className="flex gap-4">
								...
								<Button
									...
									disabled={!searchIsin || isSearching}
								>
									{isSearching ? (
						...
						<Card>
							<CardHeader className="pb-2">
								...
								<CardTitle className="text-2xl font-bold">
									{stats?.byAssetClass.reduce(
										(acc, curr) => acc + Number.parseInt(curr.total),
										0,
									) || 0}
								</CardTitle>
							</CardHeader>
						...
						<Card>
							<CardHeader className="pb-2">
								...
								<CardTitle className="text-2xl font-bold text-green-600">
									{stats?.byAssetClass.reduce(
										(acc, curr) => acc + Number.parseInt(curr.validated),
										0,
									) || 0}
								</CardTitle>
							</CardHeader>
						...
						<Card>
							<CardHeader className="pb-2">
								...
								<CardTitle className="text-2xl font-bold text-red-600">
									{stats?.byAssetClass.reduce(
										(acc, curr) => acc + Number.parseInt(curr.flagged),
										0,
									) || 0}
								</CardTitle>
							</CardHeader>
									...
									{isLoadingStats ? (
										...
										stats?.byAssetClass.map((row: any) => (
											<TableRow key={row.asset_class}>
												...
												<TableCell>
													{Number.parseInt(row.from_model) +
														Number.parseInt(row.from_broker_quote)}
												</TableCell>
				...
				<TabsContent value="flagged" className="space-y-4">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Instruments with price deviations {">"} 20% or unusual data
								patterns
							</CardDescription>
						</CardHeader>
						<CardContent>
				...
				<TabsContent value="audit" className="space-y-4">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Full price provenance history for SEBI/RBI compliance
							</CardDescription>
						</CardHeader>
						<CardContent>
							<div className="flex gap-4 mb-6">
								...
								<Button
									...
									disabled={!auditIsin || isLoadingAudit}
								>
									View Audit History
								</Button>
							</div>
									...
									<TableBody>
										{auditData.audit.map((entry) => (
											<TableRow key={entry.id}>
												...
												<TableCell
													className="text-xs italic max-w-[150px] truncate"
			...
			<Dialog open={!!overrideData} onOpenChange={() => setOverrideData(null)}>
				<DialogContent>
						...
						<div className="space-y-4 py-4">
							...
							<div className="space-y-2">
								...
								<Input
									...
									onChange={(e) =>
										setOverrideData({ ...overrideData, price: e.target.value })
									}
								/>
							</div>
							<div className="space-y-2">
					...
					<DialogFooter>
						<Button variant="outline" onClick={() => setOverrideData(null)}>
							Cancel
						</Button>
						<Button
