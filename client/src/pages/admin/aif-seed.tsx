...
export default function AifSeedPage() {
	...
	const handleImportSelected = () => {
		if (!importPreview) return;
		const selectedListings = importPreview.listings.filter((l) =>
			selectedForImport.has(l.registrationNo),
		);
				...
				<CardContent className="pt-6">
					<div className="flex flex-wrap gap-1 mb-4">
						{[
							...
							{
								value: "published",
								label: `Published (${aifs.filter((a) => a.isPublished).length})`,
							},
							{
								value: "unpublished",
								label: `Unpublished (${aifs.filter((a) => !a.isPublished).length})`,
							},
							{
								value: "category1",
								label: `Category I (${aifs.filter((a) => a.category === "Category I").length})`,
							},
							{
								value: "category2",
								label: `Category II (${aifs.filter((a) => a.category === "Category II").length})`,
							},
							{
								value: "category3",
								label: `Category III (${aifs.filter((a) => a.category === "Category III").length})`,
							},
						].map((tab) => (
							<Button
								key={tab.value}
									...
									{filteredAifs.map((aif) => (
										<TableRow key={aif.id}>
											...
											<TableCell>
												{aif.category ? (
													<Badge
														className={getCategoryBadgeColor(aif.category)}
													>
														{aif.category}
													</Badge>
												) : (
			...
			<Dialog open={showImportDialog} onOpenChange={setShowImportDialog}>
				<DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
					<DialogHeader>
						...
						<DialogDescription>
							Import registered Alternative Investment Funds from SEBI's
							database.
						</DialogDescription>
					</DialogHeader>
										...
										{importPreview.listings.map((listing) => (
											...
											>
												<TableCell>
													<input
														type="checkbox"
														checked={selectedForImport.has(
															listing.registrationNo,
														)}
														onChange={() =>
															toggleImportSelection(listing.registrationNo)
														}
												...
												<TableCell>
													<Badge
														className={getCategoryBadgeColor(listing.category)}
													>
														{listing.category}
													</Badge>
												</TableCell>
			...
			<Dialog open={showSeedDialog} onOpenChange={setShowSeedDialog}>
				<DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
					<DialogHeader>
						...
						<DialogDescription>
							Generate and import 100+ comprehensive AIFs with performance data
						...
						<div className="flex flex-col items-center justify-center py-12 space-y-6">
							...
							<div className="text-center space-y-2 max-w-md">
								<p className="text-lg font-medium">
									Comprehensive AIF Seed Data
								</p>
								<p className="text-muted-foreground text-sm">
									This will generate 100+ Alternative Investment Funds from 35+
									fund houses with complete performance metrics, risk scores,
									AUM, and NAV data.
								</p>
							</div>
							<div className="flex gap-4 text-sm text-muted-foreground">
								...
								<Badge
									variant="outline"
									className="text-purple-600 border-purple-300 dark:border-purple-700"
								>
									Category II: ~40
								</Badge>
								<Badge
						...
						<div className="space-y-4 flex-1 overflow-hidden flex flex-col">
							...
							{seedPreview.summary.new === 0 ? (
								<Alert>
									...
									<AlertDescription>
										All AIFs already exist in the database. No new AIFs to
										import.
									</AlertDescription>
								</Alert>
							) : (
										...
										<TableBody>
											{seedPreview.listings.slice(0, 50).map((listing) => (
												...
												>
													...
													<TableCell>
														{formatCurrency(listing.minInvestment)}
													</TableCell>
													<TableCell>{formatCurrency(listing.aum)}</TableCell>
													<TableCell
														className={
															Number.parseFloat(listing.return1Y) >= 0
																? "text-green-600"
																: "text-red-600"
														}
													>
