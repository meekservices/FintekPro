      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{formatCurrency(stats?.overview.totalAmount || 0)}</div>
              <p className="text-xs text-muted-foreground">Loan value requested</p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{stats?.overview.approvalRate || 0}%</div>
              <p className="text-xs text-muted-foreground">Of completed applications</p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{stats?.overview.activeBanks || 0}</div>
              <p className="text-xs text-muted-foreground">Partner financiers</p>
            </CardContent>
          </Card>
        </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Submission and approval metrics per financier</CardDescription>
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {stats?.bankWiseStats.map((bank) => (
                  <TableRow key={bank.bankCode}>
                    ...
                    <TableCell>
                      <Badge variant="outline" className="capitalize">{bank.connectorType}</Badge>
                    </TableCell>
                    <TableCell>{bank.priority}</TableCell>
                    <TableCell>{bank.interestRange}</TableCell>
                    <TableCell className="text-center">{bank.submitted}</TableCell>
                    <TableCell className="text-center text-green-600">{bank.approved}</TableCell>
                    <TableCell className="text-center text-red-600">{bank.rejected}</TableCell>
                    <TableCell className="text-center text-orange-600">{bank.pending}</TableCell>
                    <TableCell className="text-center font-medium">{bank.approvalRate}%</TableCell>
                  </TableRow>
                ))}
                ...
                <TableBody>
                  {applications.map((app) => (
                    <TableRow key={app.id}>
                      ...
                      <TableCell>{formatCurrency(parseFloat(app.requestedAmount))}</TableCell>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    ...
                    <TableRow key={bank.bankCode}>
                      ...
                      <TableCell>
                        <div className="flex flex-wrap gap-1">
                          {(bank.supportedLoanTypes || []).slice(0, 3).map((type: string) => (
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Bank-specific qualification criteria</CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                ...
                <TableBody>
                  {rules.slice(0, 8).map((rule) => (
                    <TableRow key={rule.id}>
                      <TableCell className="font-medium">{rule.bankCode}</TableCell>
                      <TableCell>{rule.loanType}</TableCell>
                      <TableCell>{rule.minCreditScore || '-'}</TableCell>
                      <TableCell>
                        {rule.minMonthlyIncome ? formatCurrency(parseFloat(rule.minMonthlyIncome)) : '-'}
                      </TableCell>
                      <TableCell>
                        {rule.isActive ? (
                          <Badge className="bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-200">Active</Badge>
