...
export default function AdminApprovalQueue() {
  ...
  const processMutation = useMutation({
    mutationFn: (data: { id: number; status: 'approved' | 'rejected'; comments: string }) =>
  ...
  if (isLoading) return <div className="p-8 text-center">Loading pending requests...</div>;

  return (
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Governance Queue</h1>
          <p className="text-muted-foreground">Maker-Checker workflow for high-risk administrative changes</p>
        </div>
      </div>
                      ...
                      <Dialog open={selectedRequest?.id === request.id} onOpenChange={(open) => !open && setSelectedRequest(null)}>
                        ...
                        <DialogContent className="max-w-xl">
                          <DialogHeader>
                            ...
                            <DialogDescription>
                              ID: #{request.id} | Priority: {request.priority.toUpperCase()}
                            </DialogDescription>
                          </DialogHeader>
                          ...
                          <div className="space-y-4 py-4">
                            <div className="grid grid-cols-2 gap-4 text-sm">
                              ...
                              <div className="space-y-1">
                                ...
                                <span className="font-medium">{request.entityId}</span>
                              </div>
                            </div>
                            
                            <div className="space-y-1">
                              <span className="text-muted-foreground text-sm block">Justification</span>
                            ...
                            <div className="space-y-1">
                              <span className="text-muted-foreground text-sm block">Request Data</span>
                          ...
                          <DialogFooter className="gap-2">
                            <Button 
                              variant="destructive" 
                              onClick={() => processMutation.mutate({ id: request.id, status: 'rejected', comments: reviewComments })}
                            ...
                            <Button 
                              ...
                              onClick={() => processMutation.mutate({ id: request.id, status: 'approved', comments: reviewComments })}
                              disabled={processMutation.isPending}
                            >
