...
export default function UnlistedOrders() {
  ...
  const updateBuyRequestMutation = useMutation({
    ...
    onError: () => {
      toast({ title: 'Error', description: 'Failed to update buy request', variant: 'destructive' });
    },
  });
  ...
  const handleAction = (type: 'approve' | 'reject' | 'match', item: SellListing | BuyRequest, isListing: boolean) => {
    if (isListing) {
  ...
  const confirmAction = () => {
    if (actionType === 'approve') {
      if (selectedListing) {
        updateListingMutation.mutate({ id: selectedListing.id, status: 'active' });
      } else if (selectedBuyRequest) {
        updateBuyRequestMutation.mutate({ id: selectedBuyRequest.id, status: 'active' });
      }
    } else if (actionType === 'reject') {
      if (selectedListing) {
        updateListingMutation.mutate({ id: selectedListing.id, status: 'cancelled' });
      } else if (selectedBuyRequest) {
        updateBuyRequestMutation.mutate({ id: selectedBuyRequest.id, status: 'cancelled' });
      }
    }
  };
                        ...
                        <TableRow key={listing.id} className="border-border" data-testid={`row-listing-${listing.id}`}>
                          ...
                          <TableCell className="text-right">
                            <div className="flex justify-end gap-1">
                              <Link href={`/admin/unlisted/preview/${listing.companyId}`}>
                                <Button variant="ghost" size="sm" data-testid={`button-view-listing-${listing.id}`}>
                              ...
                              {listing.status === 'pending' && (
                                <>
                                  <Button 
                                    ...
                                    onClick={() => handleAction('approve', listing, true)}
                                  ...
                                  <Button 
                                    ...
                                    onClick={() => handleAction('reject', listing, true)}
                    ...
                    <TableBody>
                      {filteredBuyRequests.map(request => (
                        <TableRow key={request.id} className="border-border" data-testid={`row-request-${request.id}`}>
                          ...
                          <TableCell className="text-right">
                            <div className="flex justify-end gap-1">
                              <Link href={`/admin/unlisted/preview/${request.companyId}`}>
                              ...
                              {request.status === 'pending' && (
                                <>
                                  <Button 
                                    ...
                                    size="sm" 
                                    className="text-green-400 hover:text-green-300"
                                    onClick={() => handleAction('approve', request, false)}
                                  ...
                                  <Button 
                                    ...
                                    size="sm" 
                                    className="text-red-400 hover:text-red-300"
                                    onClick={() => handleAction('reject', request, false)}
      ...
      <Dialog open={actionDialogOpen} onOpenChange={setActionDialogOpen}>
        <DialogContent className="bg-card border-border">
          <DialogHeader>
            ...
            <DialogDescription>
              {actionType === 'approve' && 'This will activate the order for trading.'}
              {actionType === 'reject' && 'This will cancel the order permanently.'}
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
          ...
          <DialogFooter>
            ...
            <Button 
              ...
              disabled={updateListingMutation.isPending || updateBuyRequestMutation.isPending}
              data-testid="button-confirm-action"
            >
              {(updateListingMutation.isPending || updateBuyRequestMutation.isPending) && (
                <Loader2 className="h-4 w-4 mr-2 animate-spin" />
              )}
              Confirm {actionType === 'approve' ? 'Approval' : actionType === 'reject' ? 'Rejection' : 'Match'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
