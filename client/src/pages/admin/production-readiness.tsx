	...
	},
	{
		...
		items: [
			...
			},
			{
				...
				status: "ready",
				description:
					"Dual-gateway system: Cashfree (primary) + PhonePe (secondary)",
			},
			{
				name: "KYC System",
				status: "ready",
...
export default function ProductionReadiness() {
	const totalChecks = readinessChecks.reduce(
		(sum, cat) => sum + cat.items.length,
		0,
	);
	const readyCount = readinessChecks.reduce(
		(sum, cat) =>
			sum + cat.items.filter((item) => item.status === "ready").length,
		0,
	);
	...
	const criticalCount = readinessChecks.reduce(
		(sum, cat) =>
			sum + cat.items.filter((item) => item.status === "critical").length,
		0,
	);
		...
		<div className="space-y-6">
			<div>
				...
				<p className="text-muted-foreground mt-1">
					System deployment checklist and status
				</p>
			</div>
				...
				<Card className="bg-red-50 dark:bg-red-950/30 border-red-200 dark:border-red-800">
					<CardHeader>
						...
						<CardDescription className="text-red-700 dark:text-red-300">
							...
							{criticalCount === 1 ? "issue" : "issues"} must be resolved before
