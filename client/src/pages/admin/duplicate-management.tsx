...
export default function DuplicateManagementPage() {
	...
	const mergeMutation = useMutation({
		...
		onSuccess: () => {
			toast({
				...
				description:
					"The duplicate account has been deactivated. Note: Data transfer must be done manually.",
			});
	...
	const mediumRiskDuplicates = duplicates.filter(
	...
	if (error) {
		return (
			<div className="p-6">
				<Alert
					variant="destructive"
					className="bg-red-50 dark:bg-red-950/50 border-red-300 dark:border-red-800"
				>
				...
				<div className="flex items-start justify-between flex-wrap gap-3">
					<div className="space-y-3">
						<div className="flex items-center gap-3 flex-wrap">
							<Badge
								className={`${getRiskBadgeColor(match.riskLevel)} text-sm px-3 py-1`}
							>
								{(match.riskLevel || "medium").toUpperCase()} RISK
							</Badge>
							<span
								className="text-foreground text-sm font-medium"
								data-testid={`risk-score-${index}`}
							>
								Risk Score:{" "}
								<span className="font-bold">{match.riskScore}/100</span>
							</span>
						...
						<div className="flex flex-wrap gap-2">
							{match.reasons.map((reason, idx) => (
								<span
									key={idx}
									className="text-xs bg-muted text-foreground px-3 py-1.5 rounded-full font-medium"
								>
									{reason}
								</span>
							))}
					...
					<div className="space-y-4 p-5 bg-blue-50 dark:bg-blue-950/30 rounded-xl border-2 border-blue-200 dark:border-blue-800">
						...
						<div className="space-y-3 text-sm">
							<div className="flex items-center gap-2">
								...
								<span
									className="text-foreground font-mono bg-card px-2 py-1 rounded"
									data-testid={`user1-id-${index}`}
								>
									{match.user1.userId}
								</span>
							</div>
	...
	return (
		<div
			...
			data-testid="page-duplicate-management"
		>
			{/* Header */}
				...
				<div className="flex items-center gap-4">
					...
					<div>
						...
						<p className="text-muted-foreground mt-1 text-base">
							Intelligent duplicate detection with risk scoring and fuzzy name
							matching
						</p>
					</div>
				</div>
				...
				<Tabs defaultValue="high" className="w-full">
					...
					<TabsContent value="high" className="space-y-4 mt-6">
						{highRiskDuplicates.length === 0 ? (
							...
							highRiskDuplicates.map((match, idx) =>
								renderDuplicateCard(match, idx),
							)
						)}
					</TabsContent>
					...
					<TabsContent value="medium" className="space-y-4 mt-6">
						{mediumRiskDuplicates.length === 0 ? (
							...
							mediumRiskDuplicates.map((match, idx) =>
								renderDuplicateCard(match, idx),
							)
						)}
					</TabsContent>
					...
					<TabsContent value="low" className="space-y-4 mt-6">
						{lowRiskDuplicates.length === 0 ? (
							...
							lowRiskDuplicates.map((match, idx) =>
								renderDuplicateCard(match, idx),
							)
						)}
					</TabsContent>
			...
			<Alert className="bg-blue-50 dark:bg-blue-950/50 border-2 border-blue-200 dark:border-blue-800">
				...
				<AlertDescription className="text-blue-800 dark:text-blue-200 ml-2 text-base">
					<span className="font-bold">How it works:</span> This system uses
					intelligent algorithms to detect duplicates based on PAN numbers,
					email addresses, mobile numbers, and fuzzy name matching. High-risk
					duplicates (same PAN + similar name) are likely the same person
					registering twice. Low-risk duplicates (same contact but different
					names) might be legitimate family members.
				</AlertDescription>
			</Alert>
		</div>
	);
