			...
			<div className="flex justify-between items-center">
				<div>
					<h1 className="text-3xl font-bold text-foreground">
						Bond Marketplace Dashboard
					</h1>
					<p className="text-muted-foreground mt-1">
						Manage bond sell listings, buy requests, and deals
					</p>
				</div>
				<Button
					variant="outline"
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
					...
					<CardContent>
						<div className="flex items-center justify-between mb-4">
							<Badge variant="outline">
								{stats?.totalSellListings || 0} Total
							</Badge>
							<Badge className="bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300">
								{stats?.activeSellListings || 0} Active
							</Badge>
						</div>
				...
				<Card className="hover:shadow-lg transition-shadow">
					...
					<CardContent>
						<div className="flex items-center justify-between mb-4">
							<Badge variant="outline">
								{stats?.totalBuyRequests || 0} Total
							</Badge>
							<Badge className="bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300">
								{stats?.activeBuyRequests || 0} Active
							</Badge>
						</div>
						<Link href="/admin/bonds/buy-requests">
							<Button
								className="w-full"
								variant="outline"
								data-testid="btn-go-buy-requests"
							>
								Manage Buy Requests
								<ArrowRight className="h-4 w-4 ml-2" />
							</Button>
				...
				<CardHeader>
					<div className="flex items-center gap-3">
						...
						<div>
							...
							<CardDescription>
								7-year SEBI/RBI compliant audit log retention
							</CardDescription>
						</div>
					</div>
				</CardHeader>
