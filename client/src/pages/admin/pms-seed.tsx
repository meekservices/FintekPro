											...
											{filteredPms.map((pms) => (
												<TableRow key={pms.id}>
													...
													<TableCell>
														{pms.strategy ? (
															<Badge
																className={getStrategyBadgeColor(pms.strategy)}
															>
																{pms.strategy}
															</Badge>
														) : (
													...
													<TableCell>
														{formatCurrency(pms.minInvestment)}
													</TableCell>
													<TableCell>
														<Badge
															variant={
			...
			<Dialog open={showImportDialog} onOpenChange={setShowImportDialog}>
				<DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
					<DialogHeader>
						...
						<DialogDescription>
							Import registered Portfolio Management Services from SEBI's
							database.
						</DialogDescription>
					</DialogHeader>
									...
									<TableBody>
										{importPreview.listings.map((listing) => (
											...
											>
												...
												<TableCell>
													<Badge
														className={getStrategyBadgeColor(listing.strategy)}
													>
														{listing.strategy || "—"}
													</Badge>
												</TableCell>
			...
			<Dialog open={showSeedDialog} onOpenChange={setShowSeedDialog}>
				<DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
					<DialogHeader>
						...
						<DialogDescription>
							Generate and import 80+ comprehensive PMS with performance data
						...
						<div className="flex flex-col items-center justify-center py-12 space-y-6">
							...
							<div className="text-center space-y-2 max-w-md">
								<p className="text-lg font-medium">
									Comprehensive PMS Seed Data
								</p>
								<p className="text-muted-foreground text-sm">
									This will generate 80+ Portfolio Management Services from 30+
									fund houses with complete performance metrics, AUM, NAV, and
									risk data.
								</p>
							</div>
							<div className="flex gap-2 flex-wrap justify-center text-sm text-muted-foreground">
								...
								<Badge
									variant="outline"
									className="text-indigo-600 border-indigo-300 dark:border-indigo-700"
								>
									Mid-cap
								</Badge>
								<Badge
									...
									{Object.entries(seedPreview.summary.byStrategy || {})
										...
										.map(([strategy, count]) => (
											<Badge
												...
												variant="outline"
												className={
													getStrategyBadgeColor(strategy)
														.replace("text-foreground", "")
														.replace("bg-", "text-")
														.replace("-500", "-600") + " border-current"
												}
											>
												{strategy}: {count}
											</Badge>
										...
										<TableBody>
											{seedPreview.listings.slice(0, 50).map((listing) => (
												...
												>
													...
													<TableCell>
														<Badge
															className={getStrategyBadgeColor(
																listing.strategy,
															)}
														>
															{listing.strategy}
