...
export default function WhatsAppSetupPage() {
	...
	const { data: otpPriority, isLoading: priorityLoading } =
		useQuery<OtpPriorityData>({
	...
	useEffect(() => {
		if (status?.isReady && polling) {
			...
			toast({
				...
				description: "Your device is now linked and ready to send messages.",
			});
		}
	...
	const initMutation = useMutation({
		mutationFn: () =>
			apiRequest("/api/admin/whatsapp/init", { method: "POST" }),
		onSuccess: () => {
			toast({
				title: "WhatsApp initializing",
				description: "Starting up — QR code will appear in ~10 seconds.",
			});
			setPolling(true);
	...
	const priorityMutation = useMutation({
		mutationFn: (channels: string[]) =>
			apiRequest("/api/admin/settings/otp-priority", {
				method: "PUT",
				body: JSON.stringify({ channels }),
			}),
		onSuccess: () => {
			toast({
				title: "OTP priority saved",
				description: `Order set to: ${priority.join(" → ")}`,
			});
	...
	const isDirty =
		...
		JSON.stringify(otpPriority?.channels ?? ["email", "whatsapp", "sms"]);
		...
		<div className="p-6 max-w-2xl mx-auto space-y-6">
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-muted-foreground mt-1">
						Configure WhatsApp device linking and the global OTP delivery
						channel priority.
					</p>
				</div>
				<Button
			...
			<Card>
				<CardHeader className="pb-3">
					...
					<CardDescription>
						Set the global default delivery order for login and registration
						OTPs. Users can override this in their own communication settings.
					</CardDescription>
				</CardHeader>
								...
								{[0, 1, 2].map((slot) => {
									...
									return (
										<div key={slot} className="space-y-1">
											...
											<Select
												value={ch}
												onValueChange={(v) => handlePriorityChange(slot, v)}
											>
			...
			{!isConnected && !hasQr && (
				<Card>
					<CardHeader className="pb-3">
						...
						<CardDescription>
							Initializes the WhatsApp Web client and generates a QR code. Make
							sure{" "}
							<code className="text-xs bg-muted px-1 rounded">
								ENABLE_WHATSAPP=true
							</code>{" "}
							is set in Railway environment variables before deploying.
						</CardDescription>
					</CardHeader>
					<CardContent>
						<Button
			...
			{hasQr && !isConnected && (
				<Card className="border-[#25d366]">
					<CardHeader className="pb-3">
						...
						<CardDescription>
							Open WhatsApp on your phone → tap the three-dot menu →{" "}
					...
					<CardContent className="flex flex-col items-center gap-4">
						{qrImage ? (
							<>
								...
								/>
								<p className="text-xs text-muted-foreground">
									QR codes expire after ~60 seconds. Click Refresh if it stops
									working.
								</p>
							...
							<div className="text-muted-foreground text-sm">
								QR image loading… click Refresh.
							</div>
						)}
					</CardContent>
				</Card>
				...
				<AlertDescription className="text-sm space-y-1">
					...
					<ul className="list-disc list-inside space-y-1 mt-1">
						...
						<li>
							...
							— this keeps you logged in across deploys
						</li>
						<li>
							Scan the QR once; subsequent deploys will resume the saved session
							automatically
						</li>
					</ul>
				</AlertDescription>
