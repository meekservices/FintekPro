...
function TransactionTimeline({
	errors,
	onViewError,
}: { errors: ErrorEntry[]; onViewError: (error: ErrorEntry) => void }) {
	...
	const sortedTransactions = Object.entries(transactionGroups)
		.map(([txId, errs]) => ({
			...
			firstError: errs.reduce(
				(min, e) => (new Date(e.createdAt) < new Date(min.createdAt) ? e : min),
				errs[0],
			),
			lastError: errs.reduce(
				(max, e) => (new Date(e.createdAt) > new Date(max.createdAt) ? e : max),
				errs[0],
			),
		...
		.sort(
			(a, b) =>
				new Date(b.lastError.createdAt).getTime() -
				new Date(a.lastError.createdAt).getTime(),
		);
		...
		return (
			<Card>
				<CardContent className="py-12 text-center text-muted-foreground">
					...
					<p className="text-sm">
						Errors without transaction IDs are not shown here
					</p>
				</CardContent>
			</Card>
		);
				...
				<ScrollArea className="h-[500px]">
					<div className="space-y-4">
						{sortedTransactions.map(
							({ transactionId, errors, hasCritical }) => (
								<div
									key={transactionId}
								...
								>
									<div className="flex items-center justify-between mb-3">
										...
										<span className="text-xs text-muted-foreground">
											{formatDistanceToNow(new Date(errors[0].createdAt), {
												addSuffix: true,
											})}
										</span>
									</div>
									...
									<div className="relative pl-4 border-l-2 border-border space-y-3">
										{errors.map((error, idx) => (
											<div key={error.id} className="relative">
												<div
													className={`absolute -left-[17px] w-3 h-3 rounded-full ${SEVERITY_COLORS[error.severity] || "bg-muted-foreground"}`}
												/>
									...
									>
										<div>
											...
											<p className="text-xs text-muted-foreground mt-1">
												{webhook.triggerOnCritical && "Critical"}{" "}
												{webhook.triggerOnSpike && "• Spikes"} • Cooldown:{" "}
												{webhook.cooldownMinutes}min
											</p>
										</div>
						...
						{(thresholds?.length ?? 0) === 0 ? (
							<div className="text-center py-8 text-muted-foreground">
								...
								<p className="text-xs mt-1">
									10 errors in 5 minutes triggers alert
								</p>
							</div>
						) : (
								...
								{thresholds?.map((threshold: any) => (
									...
									>
										<div>
											...
											<p className="text-xs text-muted-foreground">
												{threshold.occurrenceThreshold} errors in{" "}
												{threshold.windowMinutes} min
											</p>
										</div>
										<Badge
											variant={threshold.isEnabled ? "default" : "secondary"}
										>
			...
			<Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							Configure a new alerting webhook
						</DialogDescription>
					</DialogHeader>
					<div className="space-y-4">
						<div>
						...
						<div>
							...
							<Select
								...
								onValueChange={(v) =>
									setNewWebhook({ ...newWebhook, provider: v })
								}
							>
						...
						<div>
							...
							<Select
								...
								onValueChange={(v) =>
									setNewWebhook({ ...newWebhook, environment: v })
								}
							>
						...
						<div>
							...
							<Input
								...
								onChange={(e) =>
									setNewWebhook({
										...newWebhook,
										cooldownMinutes: Number.parseInt(e.target.value) || 5,
									})
								}
...
function AIInsightsPanel() {
	...
	const refreshMutation = useMutation({
		mutationFn: () =>
			apiRequest("/api/activity-centre/insights/refresh", { method: "POST" }),
		onSuccess: () => {
		...
		onError: () => {
			toast({
				...
				description: "Could not generate new insights.",
				variant: "destructive",
			});
		},
	});
		...
		<div className="space-y-6">
				...
				<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
					<Card>
						...
						<CardContent>
							...
							<p className="text-xs text-muted-foreground">
								{metrics.users.newThisWeek} new this week
							</p>
						</CardContent>
					</Card>
					<Card
					...
					>
						...
						<CardContent>
							<div className="text-2xl font-bold text-orange-500">
								{metrics.users.incompleteKyc}
							</div>
							<p className="text-xs text-muted-foreground">
								Potential revenue blocked (Click to view)
							</p>
						</CardContent>
					</Card>
					<Card>
					...
					<Card
						className={`cursor-pointer group relative overflow-hidden transition-all duration-300 hover:shadow-lg border-l-4 ${
							metrics.regulatory.highDeviationDeals > 0 ||
					...
					>
						<CardHeader className="pb-2">
							<div className="flex items-center justify-between">
								...
								<LucideShield
									className={`h-4 w-4 ${metrics.regulatory.highDeviationDeals > 0 || metrics.regulatory.investorLimitAlerts > 0 ? "text-red-500 animate-pulse" : "text-indigo-500"}`}
								/>
							</div>
						</CardHeader>
						<CardContent>
							<div className="flex items-baseline gap-2">
								<div
									className={`text-3xl font-black ${metrics.regulatory.highDeviationDeals > 0 || metrics.regulatory.investorLimitAlerts > 0 ? "text-red-600" : "text-indigo-600"}`}
								>
									{metrics.regulatory.highDeviationDeals +
										metrics.regulatory.pendingStrFlags +
										metrics.regulatory.investorLimitAlerts}
								</div>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						AI-generated suggestions to improve performance, revenue, and
						engagement
					</CardDescription>
				</CardHeader>
				<CardContent>
									...
									>
										<div className="flex items-start justify-between gap-4">
											<div className="flex items-start gap-3">
												<div
													className={`p-2 rounded-lg bg-muted ${getCategoryColor(insight.category)}`}
												>
...
function ErrorSupportReport({
	error,
	onClose,
}: { error: ErrorEntry; onClose: () => void }) {
	const { toast } = useToast();
							...
							<div>
								<div className="flex items-center justify-between mb-2">
									...
									<Button
										variant="ghost"
										size="sm"
										onClick={() => copyToClipboard(report.textReport)}
									>
								...
								<div>
									<div className="flex items-center justify-between mb-2">
										...
										<Button
											...
											onClick={() =>
												copyToClipboard(
													JSON.stringify(report.jsonReport, null, 2),
												)
											}
										>
									...
									<div className="grid grid-cols-2 md:grid-cols-3 gap-3">
										<div className="p-3 border rounded-md bg-muted/30">
											...
											<p className="text-sm font-medium flex items-center gap-1">
												<AlertCircle
													className={`h-3 w-3 ${SEVERITY_COLORS[report.jsonReport.severity]}`}
												/>
										...
										<div className="p-3 border rounded-md bg-muted/30">
											...
											<p className="text-xs truncate">
												{report.jsonReport.replitContext.userEmail || "Guest"}
											</p>
										</div>
									</div>
								</div>
...
function GlobalPlatformHealth() {
	const { data: health } = useQuery<{
		...
		services?: Record<
			string,
			{ status: string; latency: number; uptime: number }
		>;
		...
		<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 mb-6">
			<Card className="bg-muted/30">
				<CardContent className="p-4 flex items-center gap-4">
					<div
						className={`p-2 rounded-full ${health.status === "ok" ? "bg-green-100 text-green-600" : "bg-red-100 text-red-600"}`}
					>
			...
			<Card className="bg-muted/30">
				<CardContent className="p-4 flex items-center gap-4">
					...
					<div>
						...
						<h4 className="text-lg font-bold">
							{health.services?.database?.latency ?? 0}ms
						</h4>
					</div>
				</CardContent>
			</Card>
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="max-w-5xl max-h-[85vh] flex flex-col">
				<DialogHeader>
					...
					<DialogDescription>
						Users who haven't completed KYC and AI-recommended actions to
						convert them
					</DialogDescription>
				</DialogHeader>
										...
										<TableCell>
											<div className="flex flex-col">
												<div className="flex items-center gap-1">
													<Badge
														variant={
															user.recommendation.priority === "high"
																? "destructive"
																: "secondary"
														}
...
export default function ActivityCentre() {
	...
	const {
		...
		isLoading,
		refetch,
	} = useQuery<{ errors: ErrorEntry[]; total: number }>({
		queryKey: ["/api/errors"],
	});
	...
	return (
		<div className="container mx-auto py-8 px-4 max-w-7xl">
						...
						<Card>
							...
							<CardContent>
								<div className="text-2xl font-bold truncate">
									{Object.entries(metrics?.byModule || {}).sort(
										(a, b) => b[1] - a[1],
									)[0]?.[0] || "N/A"}
								</div>
								<p className="text-xs text-muted-foreground mt-1">
									Most frequent error source
								</p>
							</CardContent>
						</Card>
						<Card>
							...
							<CardContent>
								<div className="text-2xl font-bold text-amber-600">
									{metrics?.clientImpactScore || 0}/100
								</div>
								<p className="text-xs text-muted-foreground mt-1">
									Weighted user frustration index
								</p>
							</CardContent>
						</Card>
					</div>
						...
						<CardHeader>
							<div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
								<div>
									...
									<CardDescription>
										Search and filter system errors
									</CardDescription>
								</div>
											...
											filteredErrors?.map((error) => (
												...
												>
													<TableCell>
														<Badge
															variant={
																(SEVERITY_BADGES[error.severity] as any) ||
																"outline"
															}
														>
															{error.severity}
					...
					<DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col p-0">
						<DialogHeader className="p-6 pb-2">
							<div className="flex items-center gap-2 mb-1">
								<Badge
									variant={
										(SEVERITY_BADGES[selectedError.severity] as any) ||
										"outline"
									}
								>
						...
						<div className="flex-1 overflow-auto p-6 pt-2 space-y-6">
							<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
								...
								<div className="space-y-1">
									...
									<Badge
										variant={
											(STATUS_BADGES[selectedError.status] as any) || "outline"
										}
									>
								...
								<div className="bg-slate-950 text-slate-300 p-4 rounded-lg font-mono text-[11px] overflow-auto max-h-[300px]">
									<pre>
										{selectedError.stackTrace ||
											"No stack trace provided for this error entry."}
									</pre>
								</div>
							</div>
									...
									<div className="p-3 border rounded-md bg-muted/30 text-sm space-y-2">
										<div className="flex justify-between">
											...
											<span className="text-xs">
												{format(new Date(selectedError.firstOccurrence), "PPp")}
											</span>
										</div>
										<div className="flex justify-between">
											<span className="text-muted-foreground text-xs">
											...
											<span className="text-xs">
												{format(new Date(selectedError.lastOccurrence), "PPp")}
											</span>
										</div>
										<div className="flex justify-between">
											<span className="text-muted-foreground text-xs">
											...
											<span className="text-xs font-medium">
												~{(selectedError.occurrenceCount / 24).toFixed(1)} /
												hour
											</span>
										</div>
									</div>
			...
			{selectedError && (
				<ErrorSupportReport error={selectedError} onClose={() => {}} />
			)}
