...
export default function UnlistedNegotiations() {
	...
	const { data, isLoading, error } = useQuery<NegotiationsResponse>({
		queryKey: [
			"/api/unlisted/admin/negotiations",
			{
				page,
		...
		queryFn: async () => {
			...
			if (minMatchScore > 0)
				params.append("minMatchScore", minMatchScore.toString());
		...
		return (
			<div className="space-y-6">
				<div>
					...
					<p className="text-muted-foreground mt-1">
						Monitor and manage active marketplace negotiations
					</p>
				</div>
				<Card className="bg-card border-border">
					<CardContent className="pt-6">
						<p className="text-red-400">
							Error loading negotiations: {(error as Error).message}
						</p>
					</CardContent>
				</Card>
			</div>
		...
		<div className="space-y-6">
			...
			<Card className="bg-card border-border">
				...
				<CardContent className="space-y-4">
					{/* Company Search */}
					<div>
						<Label htmlFor="companySearch" className="text-muted-foreground">
							Search Company
						</Label>
			...
			<Card className="bg-card border-border">
				<CardHeader>
					...
					<CardDescription className="text-muted-foreground">
						Showing {negotiations.length} of {pagination?.total || 0}{" "}
						negotiations
					</CardDescription>
				</CardHeader>
				<CardContent>
									...
									{negotiations.map((negotiation) => (
										...
										>
											...
											<TableCell className="text-right">
												...
												<div className="text-xs text-muted-foreground">
													{negotiation.sellListing.quantity.toLocaleString()}{" "}
													shares
												</div>
											</TableCell>
											...
											<TableCell className="text-right">
												...
												<div className="text-xs text-muted-foreground">
													{negotiation.matchingBuyRequestsCount} buyer
													{negotiation.matchingBuyRequestsCount !== 1
														? "s"
														: ""}
												</div>
											</TableCell>
											...
											<TableCell className="text-right">
												...
												<Badge
													variant="outline"
													className={`mt-1 text-xs ${getConfidenceBadge(negotiation.confidence)}`}
												>
												...
												{negotiation.ratios ? (
													<div className="space-y-1 text-xs">
														...
														<div className="text-muted-foreground">
															...
															<span className="text-foreground">
																{Number.parseFloat(
																	negotiation.ratios.debtToEquity,
																).toFixed(2)}
															</span>
														</div>
													</div>
												...
												{negotiation.lastDealPrice ? (
													<div>
														...
														{negotiation.lastDealDate && (
															<div className="text-xs text-muted-foreground">
																{format(
																	new Date(negotiation.lastDealDate),
																	"MMM d, yyyy",
																)}
															</div>
														)}
											...
											<TableCell className="text-right">
												<div className="flex gap-2 justify-end">
													<Button
														...
														size="sm"
														onClick={() =>
															handleViewDetails(negotiation.company.id)
														}
