...
export default function LeadProspecting() {
	...
	const searchCompaniesMutation = useMutation({
			...
			if (data.available === false) {
				if (data.usingFallback && data.companies?.length > 0) {
					...
					toast({
						...
						description:
							"Credhive is unavailable. Showing leads from your database instead.",
					});
				} else {
					...
					toast({
						...
						description:
							...
							"External company search is currently unavailable.",
						variant: "destructive",
					});
				}
			} else {
		...
		onError: () => {
			...
			toast({
				title: "Search failed",
				description:
					"Unable to connect to company search. Try again later or create leads manually.",
				variant: "destructive",
			});
		},
	});
	...
	const enrichPreviewMutation = useMutation({
		...
		onError: (error: any) => {
			toast({
				...
				description:
					error.message ||
					"Unable to enrich company data. Try importing directly.",
				variant: "destructive",
			});
		},
	});
	...
	const handleViewDetails = (
		company: CompanySearchResult | DirectorCompany,
	) => {
		enrichPreviewMutation.mutate({
			cin: company.cin,
			companyName:
				"legalName" in company ? company.legalName : company.companyName,
		});
	};
		...
		onSuccess: (data: any) => {
			...
			if (data.available === false) {
				...
				toast({
					...
					description:
						data.error || "External director search is currently unavailable.",
					variant: "destructive",
				});
			} else {
				setDirectorApiError(null);
				const totalCompanies =
					data.directors?.reduce(
						(sum: number, d: DirectorSearchResult) => sum + d.companies.length,
						0,
					) || 0;
				toast({
	...
	const handleSearch = (e: React.FormEvent<HTMLFormElement>) => {
		...
		if (formData.get("companyName"))
			filters.nameStartsWith = formData.get("companyName");
		if (formData.get("city")) filters.city = formData.get("city");
		if (formData.get("state")) filters.state = formData.get("state");
		if (formData.get("minRevenue"))
			filters.minRevenue = Number.parseInt(
				formData.get("minRevenue") as string,
			);
		if (formData.get("minProfit"))
			filters.minProfit = Number.parseInt(formData.get("minProfit") as string);
		if (formData.get("credhiveScore")) {
			filters.credhiveScore = Number.parseInt(
				formData.get("credhiveScore") as string,
			);
		}
								...
								<div className="grid gap-4 md:grid-cols-2">
									...
									<div className="space-y-2">
										...
										<p className="text-xs text-muted-foreground">
											Required: Enter at least 4 characters (e.g., "Tata",
											"Reliance")
										</p>
									</div>
									...
									<div className="space-y-2">
										...
										/>
										<p className="text-xs text-muted-foreground">
											Financial health: 5 = Excellent, 1 = Poor
										</p>
									</div>
								</div>
									...
									<div className="flex items-start gap-3">
										...
										<div className="flex-1">
											<h4 className="font-medium text-amber-800 dark:text-amber-200">
												Credhive API Unavailable
											</h4>
											<p className="text-sm text-amber-700 dark:text-amber-300 mt-1">
												{apiError}
											</p>
											<p className="text-sm text-amber-600 dark:text-amber-400 mt-2">
												You can still create B2B leads manually in the{" "}
												<a
													href="/admin/prospect-dashboard"
													className="underline font-medium hover:text-amber-800 dark:text-amber-200"
												>
													Prospect Dashboard
												</a>{" "}
												or import from Zoho CRM.
											</p>
										</div>
									</div>
								</div>
										...
										{searchResults.map((company) => (
											...
											>
												...
												{company.registeredAddress &&
													company.registeredAddress.length > 30 && (
						...
						<TabsContent value="director-lookup">
							<form onSubmit={handleDirectorSearch} className="space-y-4">
								<div className="space-y-2">
									...
									<p className="text-xs text-muted-foreground">
										Search for directors to discover their company associations
										with financial data
									</p>
								</div>
							</form>
							...
							{searchDirectorsMutation.isPending && (
								<div className="mt-6 flex flex-col items-center justify-center py-12">
									...
									<p className="text-muted-foreground">
										Searching directors and enriching company data...
									</p>
									...
									<div className="mt-6 space-y-4">
										<h3 className="font-semibold">
											...
											<span className="text-muted-foreground font-normal ml-2">
												(
												{directorResults.reduce(
													(sum, d) => sum + d.companies.length,
													0,
												)}{" "}
										...
										<Accordion type="multiple" className="w-full space-y-2">
											{directorResults.map((director, idx) => (
												<AccordionItem
													key={director.din || idx}
													value={director.din || `director-${idx}`}
																		...
																		{company.paidUpCapital > 0 && (
																			<div className="flex items-center gap-2 text-muted-foreground">
																				...
																				<span>
																					...
																					{company.paidUpCapital.toLocaleString(
																						"en-IN",
																					)}
																				</span>
																			</div>
																		...
																		{company.sumOfCharges > 0 && (
																			<div className="flex items-center gap-2 text-amber-600">
																				...
																				<span>
																					...
																					{company.sumOfCharges.toLocaleString(
																						"en-IN",
																					)}
																				</span>
																			</div>
				...
				<Card>
					...
					<CardContent>
						<div
							...
							data-testid="text-converted-leads"
						>
					...
					<CardContent>
						<div className="text-2xl font-bold" data-testid="text-avg-score">
							{leads && leads.length > 0
								? (
										leads.reduce((sum, l) => sum + l.leadScore, 0) /
										leads.length
									).toFixed(0)
								: 0}
						</div>
									...
									<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 mb-4">
										...
										<div className="space-y-2">
											...
											<div className="text-sm space-y-1">
												<p>
													...
													{lead.annualRevenue
														? `₹${(Number.parseFloat(lead.annualRevenue) / 10000000).toFixed(2)}Cr`
														: "N/A"}
												</p>
												<p>
													<strong>Paid-up:</strong>{" "}
													{lead.paidUpCapital
														? `₹${(Number.parseFloat(lead.paidUpCapital) / 10000000).toFixed(2)}Cr`
														: "N/A"}
												</p>
												<p>
													<strong>Net Profit:</strong>{" "}
													{lead.netProfit
														? `₹${(Number.parseFloat(lead.netProfit) / 10000000).toFixed(2)}Cr`
														: "N/A"}
												</p>
												{lead.sumOfCharges && (
													<p>
														...
														<span className="text-orange-600">
															₹
															{(
																Number.parseFloat(lead.sumOfCharges) / 10000000
															).toFixed(2)}
															Cr
														</span>
													</p>
											...
											<div className="text-sm space-y-1">
												...
												{lead.entityType && (
													<p
														...
														title={lead.entityType}
													>
														{lead.entityType.length > 30
															? lead.entityType.substring(0, 30) + "..."
															: lead.entityType}
													</p>
												)}
												<div className="flex items-center gap-1">
													...
													{lead.gstStatus ? (
														<Badge
															variant={
																lead.gstStatus === "Active"
																	? "default"
																	: lead.gstStatus === "Not Registered"
																		? "secondary"
																		: "destructive"
															}
															className="text-xs ml-1"
													...
													{lead.creditRating ? (
														<>
															...
															{lead.creditRatingOutlook && (
																<span
																	className={`text-xs ${lead.creditRatingOutlook === "Positive" ? "text-green-600" : lead.creditRatingOutlook === "Negative" ? "text-red-600" : ""}`}
																>
																	({lead.creditRatingOutlook})
																</span>
															)}
												...
												<div className="flex items-center gap-1">
													...
													{(lead.openChargesCount || 0) === 0 ? (
														...
														<Badge
															...
															className="text-xs ml-1"
														>
															{lead.openChargesCount} open
														</Badge>
													)}
												</div>
												{lead.totalChargesAmount &&
													Number.parseFloat(lead.totalChargesAmount) > 0 && (
														<p className="text-xs">
															₹
															{(
																Number.parseFloat(lead.totalChargesAmount) /
																10000000
															).toFixed(2)}
															Cr
														</p>
										...
										<div className="space-y-2">
											...
											<div className="text-sm space-y-1">
												...
												{lead.suitFiledCasesCount !== undefined &&
													lead.suitFiledCasesCount > 0 && (
														<p className="text-xs text-muted-foreground">
															{lead.suitFiledCasesCount} suits filed
														</p>
													)}
												<p>
									...
									{lead.directors && lead.directors.length > 0 && (
										<div className="mb-4 p-3 bg-blue-50 dark:bg-blue-950/20 rounded-lg">
											<h4 className="text-xs font-medium text-muted-foreground flex items-center gap-1 mb-2">
												<Users className="h-3 w-3" /> Board of Directors (
												{lead.directors.length})
											</h4>
											<div className="grid gap-2 md:grid-cols-2 lg:grid-cols-3">
												{lead.directors.slice(0, 6).map((director, idx) => (
													<div
														key={idx}
														className="text-xs p-2 bg-card rounded border"
													>
														...
														{director.otherCompaniesCount &&
															director.otherCompaniesCount > 0 && (
																<p className="mt-1 text-orange-600 dark:text-orange-400">
																	+{director.otherCompaniesCount} other
																	companies
																</p>
															)}
													</div>
										...
										<div className="pt-3 border-t flex items-center justify-between text-xs text-muted-foreground">
											<div className="flex items-center gap-4">
												...
												{lead.enrichmentSources && (
													<span>
														{lead.enrichmentSources.length}/10 sources
													</span>
												)}
											</div>
											{lead.enrichedAt && (
												<span>
													Enriched:{" "}
													{new Date(lead.enrichedAt).toLocaleDateString(
														"en-IN",
													)}
												</span>
											)}
							...
							<div className="space-y-6">
								...
								<div className="p-4 bg-gradient-to-r from-background to-muted rounded-lg border">
									...
									<div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
										...
										<div className="text-center p-2 bg-card rounded border">
											<div
												className={`text-xl font-bold ${
													(selectedCompany.enrichmentScore || 0) >= 70
														? "text-green-600"
														: (selectedCompany.enrichmentScore || 0) >= 40
															? "text-yellow-600"
															: "text-red-600"
												}`}
											>
										...
										{/* Total Debt/Charges */}
										{selectedCompany.totalChargesAmount &&
										selectedCompany.totalChargesAmount > 0 ? (
											<div className="text-center p-2 bg-card rounded border">
												<div className="text-xl font-bold text-amber-600">
													₹
													{(
														selectedCompany.totalChargesAmount / 10000000
													).toFixed(0)}{" "}
													Cr
												</div>
												<div className="text-xs text-muted-foreground">
										...
										<div className="text-center p-2 bg-card rounded border">
											<div
												className={`text-xl font-bold ${(selectedCompany.openChargesCount || 0) > 0 ? "text-amber-600" : "text-green-600"}`}
											>
										...
										<div className="text-center p-2 bg-card rounded border">
											<div
												className={`text-xl font-bold ${(selectedCompany.suitFiledCasesCount || 0) > 0 ? "text-red-600" : "text-green-600"}`}
											>
									...
									{/* Risk Indicator */}
									{((selectedCompany.totalChargesAmount &&
										selectedCompany.totalChargesAmount > 1000000000) ||
										(selectedCompany.suitFiledCasesCount &&
											selectedCompany.suitFiledCasesCount > 0)) && (
										<div className="flex items-center gap-2 p-2 bg-red-50 dark:bg-red-950 border border-red-200 dark:border-red-800 rounded text-sm">
											...
											<span className="text-red-700 dark:text-red-300 font-medium">
												...
												{selectedCompany.suitFiledCasesCount &&
												selectedCompany.suitFiledCasesCount > 0
													? ` • ${selectedCompany.suitFiledCasesCount} legal case(s)`
													: ""}
											</span>
										</div>
									)}
								...
								{selectedCompany.isEnriched === false && (
									<div className="p-3 bg-amber-50 dark:bg-amber-950 border border-amber-200 dark:border-amber-800 rounded-lg">
										<div className="flex items-center gap-2 text-amber-700 dark:text-amber-300 text-sm">
											<AlertTriangle className="h-4 w-4" />
											<span>
												Limited data available. Full financial details will be
												fetched upon import.
											</span>
										</div>
									</div>
								...
								<div>
									...
									<div className="grid gap-2 text-sm">
										{selectedCompany.authorizedCapital &&
											selectedCompany.authorizedCapital > 0 && (
												<div className="flex items-center gap-2">
													...
													<span className="text-muted-foreground">
														₹
														{selectedCompany.authorizedCapital.toLocaleString(
															"en-IN",
														)}
													</span>
												</div>
										...
										{selectedCompany.paidUpCapital &&
											selectedCompany.paidUpCapital > 0 && (
												<div className="flex items-center gap-2">
													<span className="font-medium">Paid-up Capital:</span>
													<span className="text-muted-foreground">
														₹
														{selectedCompany.paidUpCapital.toLocaleString(
															"en-IN",
														)}
													</span>
												</div>
											...
											selectedCompany.totalChargesAmount > 0 && (
												<div className="flex items-center gap-2">
													...
													<span className="text-amber-600 font-semibold">
														₹
														{selectedCompany.totalChargesAmount.toLocaleString(
															"en-IN",
														)}
													</span>
												</div>
											)}
										{selectedCompany.openChargesCount !== undefined &&
											selectedCompany.openChargesCount > 0 && (
												<div className="flex items-center gap-2">
													...
													>
														{selectedCompany.openChargesCount} active
													</Badge>
												</div>
											)}
										...
										{(!selectedCompany.authorizedCapital ||
											...
											(!selectedCompany.totalChargesAmount ||
												...
												<p className="text-muted-foreground italic">
													Financial data will be fetched upon import
												</p>
											)}
									</div>
								</div>
								...
								{/* Risk & Legal Section */}
								{selectedCompany.suitFiledCasesCount !== undefined &&
									selectedCompany.suitFiledCasesCount > 0 && (
										<>
											<Separator />
											<div>
												...
												<div className="grid gap-2 text-sm">
													<div className="flex items-center gap-2">
														...
														<Badge variant="destructive">
															{selectedCompany.suitFiledCasesCount} cases
														</Badge>
													</div>
												</div>
											</div>
											...
											<div className="grid gap-2 text-sm">
												{selectedCompany.listingStatus && (
													<div className="flex items-center gap-2">
														...
														<Badge
															variant={
																selectedCompany.listingStatus === "Listed"
																	? "default"
																	: "secondary"
															}
														>
												...
												{selectedCompany.activeCompliance && (
													<div className="flex items-center gap-2">
														...
														<Badge
															variant={(() => {
																const compliance =
																	selectedCompany.activeCompliance?.toLowerCase() ||
																	"";
																return compliance.includes("yes") ||
								...
								{(selectedCompany.enrichmentScore !== undefined ||
									(selectedCompany.apiAccessIssues &&
										selectedCompany.apiAccessIssues.length > 0)) && (
									<>
										<Separator />
										<div>
											<h4 className="font-semibold mb-3 flex items-center gap-2">
															...
															<div className="w-24 h-2 bg-muted rounded-full overflow-hidden">
																<div
																	className={`h-full rounded-full ${
																		selectedCompany.enrichmentScore >= 70
																			? "bg-green-500"
																			: selectedCompany.enrichmentScore >= 40
																				? "bg-yellow-500"
																				: "bg-red-500"
																	}`}
																	style={{
																		width: `${selectedCompany.enrichmentScore}%`,
																	}}
																/>
															</div>
															<span className="text-sm font-medium">
																{selectedCompany.enrichmentScore}%
															</span>
														</div>
													</div>
												)}
												{selectedCompany.enrichmentSources &&
													selectedCompany.enrichmentSources.length > 0 && (
														<div className="flex flex-wrap gap-1">
															{selectedCompany.enrichmentSources.map(
																(source, idx) => (
																	<Badge
																		key={idx}
																		variant="outline"
