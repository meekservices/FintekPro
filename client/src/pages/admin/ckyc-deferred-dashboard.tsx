...
function getStatusBadge(status: string) {
	...
	> = {
		...
		manual_kyc_initiated: {
			variant: "outline",
			className:
				"bg-purple-50 dark:bg-purple-950/30 text-purple-700 dark:text-purple-300 border-purple-300 dark:border-purple-700",
		},
		vkyc_scheduled: {
			variant: "outline",
			className:
...
export default function CkycDeferredDashboard() {
	...
	const {
	...
	} = useQuery<{ cases: DeferredCase[]; total: number }>({
		...
		queryFn: async () => {
			...
			const result = await apiRequest(
				`/api/admin/ckyc-deferred/cases?${params}`,
			);
	...
	const actionMutation = useMutation({
		mutationFn: async ({
			...
			action,
			reason,
		}: { caseId: string; action: string; reason: string }) => {
			const result = await apiRequest(
				`/api/admin/ckyc-deferred/cases/${caseId}/action`,
				{
					method: "POST",
		...
		() => [
			{
				...
				cell: (item) => (
					<div>
						<p className="font-medium" data-testid={`text-userName-${item.id}`}>
							{item.userName || "N/A"}
						</p>
						<p
							className="text-sm text-muted-foreground"
							data-testid={`text-userEmail-${item.id}`}
						>
							{item.userEmail}
						</p>
					</div>
			...
			},
			{
				...
				cell: (item) => (
					<span
						...
						title={item.reason}
						data-testid={`text-reason-${item.id}`}
					>
			...
			},
			{
				...
				cell: (item) => (
					<div className="flex items-center gap-2">
						<Timer
							className={`h-4 w-4 ${getSlaColor(item.hoursRemaining, item.slaBreached)}`}
						/>
						<span
							className={`text-sm font-medium ${getSlaColor(item.hoursRemaining, item.slaBreached)}`}
						...
						{item.slaBreached && (
							<Badge
								variant="destructive"
								className="text-xs"
								data-testid={`badge-breached-${item.id}`}
							>
								BREACHED
							</Badge>
						)}
			...
			},
			{
				...
				header: "Attempts",
				cell: (item) => (
					<span className="text-sm" data-testid={`text-attempts-${item.id}`}>
						{item.attempts}
					</span>
				),
			},
		...
		() => [
			{
				...
				confirmDescription:
					"Send escalation notification to compliance team for selected cases?",
			...
			},
			{
				...
				confirmDescription:
					"Schedule video KYC appointments for selected cases?",
				onExecute: async (items) => {
					let successCount = 0;
					for (const item of items) {
						try {
	...
	const totalBreached =
		dashboardStats?.agingBuckets
			?.filter((b) => b.breached)
			.reduce((sum, b) => sum + b.count, 0) || 0;
	const totalPending =
		dashboardStats?.agingBuckets?.reduce((sum, b) => sum + b.count, 0) || 0;
		...
		<div className="space-y-6 p-6" data-testid="ckyc-deferred-dashboard">
			...
			{totalBreached > 0 && (
				<Alert variant="destructive">
					...
					<AlertDescription>
						{totalBreached} case(s) have breached SLA. Immediate action required
						to maintain SEBI compliance.
					</AlertDescription>
				</Alert>
			)}
				...
				{dashboardStats?.agingBuckets?.map((bucket, index) => (
					...
					>
						<CardHeader className="pb-2">
							...
							<CardTitle
								className={`text-3xl ${bucket.breached ? "text-red-600" : ""}`}
							>
								{bucket.count}
							</CardTitle>
						</CardHeader>
						<CardContent>
							<Progress
								value={
									totalPending > 0 ? (bucket.count / totalPending) * 100 : 0
								}
				...
				<DialogContent className="max-w-lg">
					<DialogHeader>
						...
						<DialogDescription>
							{selectedCase && (
								<span>
									User: {selectedCase.userName} ({selectedCase.panNumber})
								</span>
							)}
						</DialogDescription>
					</DialogHeader>
