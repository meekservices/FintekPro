							...
							<FormItem>
								...
								<Select onValueChange={field.onChange} value={field.value}>
									...
									<SelectContent>
										...
										<SelectItem value="corporate_bond">
											Corporate Bond
										</SelectItem>
						...
						render={({ field }) => (
							<FormItem>
								...
								<FormControl>
									<Input
										{...field}
										placeholder="NCD-2025-001"
										data-testid="input-issue-code"
									/>
								</FormControl>
								<FormMessage />
							</FormItem>
							...
							<FormItem>
								...
								<FormControl>
									<Input
										...
										type="number"
										placeholder="1000"
										data-testid="input-price-per-ncd"
									/>
								</FormControl>
								<FormMessage />
							</FormItem>
							...
							<FormItem>
								...
								<FormControl>
									<Input
										{...field}
										type="date"
										data-testid="input-issue-open-date"
									/>
								</FormControl>
								<FormMessage />
							</FormItem>
							...
							<FormItem>
								...
								<FormControl>
									<Input
										{...field}
										type="date"
										data-testid="input-issue-close-date"
									/>
								</FormControl>
								<FormMessage />
							</FormItem>
...
function BondsManagement() {
	...
	const saveBondMutation = useMutation({
		...
		onSuccess: () => {
			toast({
				...
				description: selectedBond
					? "Bond has been updated successfully"
					: "New bond has been added to the marketplace",
			});
	...
	const filteredBonds = (bondsData?.bonds || []).filter(
		(bond) =>
			!searchTerm ||
	...
	return (
		<div className="space-y-4">
								...
								<TableRow key={bond.id} data-testid={`bond-row-${bond.isin}`}>
									...
									<TableCell>
										<Badge
											variant="outline"
											className={
												bond.creditRating?.startsWith("AAA")
													? "bg-green-50 dark:bg-green-950/30 text-green-700 dark:text-green-300"
													: bond.creditRating?.startsWith("AA")
									...
									<TableCell className="text-right">
										<Button
											...
											data-testid={`btn-edit-${bond.isin}`}
										>
			...
			<Dialog
				open={showBondDialog}
				onOpenChange={(open) => !open && setShowBondDialog(false)}
			>
				<DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
					<DialogHeader>
						<DialogTitle>
							{selectedBond ? "Edit Bond" : "Add New Bond"}
						</DialogTitle>
						<DialogDescription>
							{selectedBond
								? "Update bond details"
								: "Add a new bond to the marketplace catalog"}
						</DialogDescription>
					</DialogHeader>
					<BondForm
						bond={selectedBond}
...
function NcdManagement() {
	...
	const saveNcdMutation = useMutation({
		mutationFn: (data: any) => {
			if (selectedIssue) {
				return apiRequest(
					`/api/fixed-income/admin/ncd-issues/${selectedIssue.id}`,
					{
						method: "PUT",
						body: JSON.stringify(data),
					},
		...
		onSuccess: () => {
			toast({
				...
				description: selectedIssue
					? "NCD issue has been updated"
					: "New NCD issue has been created",
			});
	...
	return (
		<div className="space-y-4">
					...
					<Table>
						...
						<TableBody>
							{issues.map((issue) => (
								<TableRow
									key={issue.id}
									data-testid={`ncd-row-${issue.issueCode}`}
								>
									...
									<TableCell className="text-sm">
										{new Date(issue.issueOpenDate).toLocaleDateString()} -{" "}
										{new Date(issue.issueCloseDate).toLocaleDateString()}
									</TableCell>
									<TableCell>
										<Badge
											variant={
									...
									<TableCell className="text-right">
										<Button
											...
											data-testid={`btn-edit-ncd-${issue.issueCode}`}
										>
											<Edit className="h-4 w-4" />
										</Button>
									</TableCell>
			...
			<Dialog
				open={showNcdDialog}
				onOpenChange={(open) => !open && setShowNcdDialog(false)}
			>
				<DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
					<DialogHeader>
						<DialogTitle>
							{selectedIssue ? "Edit NCD Issue" : "Add NCD Issue"}
						</DialogTitle>
						<DialogDescription>
							{selectedIssue
								? "Update NCD issue details"
								: "Create a new public NCD issue"}
						</DialogDescription>
					</DialogHeader>
					<NcdIssueForm
						issue={selectedIssue}
		...
		<div className="space-y-4">
			...
			<Alert className="bg-amber-50 dark:bg-amber-950/30 border-amber-200 dark:border-amber-800">
				...
				<AlertDescription className="text-amber-700 dark:text-amber-300">
					SGB issues are announced by RBI. Use sync to fetch latest issue
	...
	return (
		<div className="space-y-4">
				...
				<div className="flex gap-2">
					<Select defaultValue="all">
						...
						<SelectContent>
							...
							<SelectItem value="suitability_check">
								Suitability Check
							</SelectItem>
			...
			<Alert>
				...
				<AlertDescription>
					All audit logs are retained for 7 years as per PMLA regulations. Logs
...
function BondMarketplaceManagement() {
	...
	const { data: sellListingsResponse, isLoading: loadingSell } = useQuery<{
		success: boolean;
		data: BondSellListing[];
	}>({
		queryKey: ["/api/bonds/admin/listings"],
	});
	const sellListings = sellListingsResponse?.data || [];

	const { data: buyRequestsResponse, isLoading: loadingBuy } = useQuery<{
		success: boolean;
		data: BondBuyRequest[];
	}>({
			...
			{activeTab === "deals" && (
				<div>
					<Alert className="mb-4">
						...
						<AlertDescription>
							Match sell listings with buy requests to create deals. Ensure
							compliance checks are completed before approving.
						</AlertDescription>
					</Alert>
					<EmptyState
						icon={Handshake}
			...
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-muted-foreground">
						Manage bonds, NCDs, SGBs, and monitor marketplace activity
					</p>
				</div>
			</div>
