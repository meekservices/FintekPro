...
export default function SMSCampaigns() {
	...
	const { data: consentedUsers, isLoading: usersLoading } = useQuery<
		Array<{
			userId: string;
			mobile: string;
			name: string;
	...
	>({
		...
		queryFn: async () => {
			const response = await fetch(
				`/api/admin/marketing/audience?filter=${audienceFilter}&consentOnly=true`,
			);
					...
					<Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
						...
						<DialogContent className="max-w-2xl">
							<DialogHeader>
								...
								<DialogDescription>
									Send bulk SMS to users who have consented to marketing
									communications
								</DialogDescription>
							</DialogHeader>
							<div className="space-y-4">
								<div className="space-y-2">
									...
									<p className="text-xs text-muted-foreground">
										{usersLoading
											? "Loading..."
											: `${consentedUsers?.length || 0} recipients selected`}
									</p>
								</div>
								...
								<div className="space-y-2">
									...
									<div className="flex justify-between text-xs text-muted-foreground">
										...
										<span
											className={
												customMessage.length > 160 ? "text-orange-500" : ""
											}
										>
									...
									<div className="flex items-start gap-2">
										...
										<div className="text-sm">
											...
											<p className="text-amber-700 dark:text-amber-300">
												Only users who have opted in to marketing communications
												will receive this SMS. All messages include opt-out
												instructions.
											</p>
										</div>
									</div>
								...
								<div className="flex justify-end gap-2">
									...
									<Button
										...
										disabled={
											sendBulkSMSMutation.isPending || !consentedUsers?.length
										}
				...
				<TabsContent value="quick-send">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Send a promotional SMS to a single user
							</CardDescription>
						</CardHeader>
						<CardContent>
							<QuickSendForm
								onSend={sendPromotionalSMSMutation.mutate}
								isPending={sendPromotionalSMSMutation.isPending}
							/>
						</CardContent>
					</Card>
				</TabsContent>
				...
				<TabsContent value="templates">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Pre-approved message templates for different product types
							</CardDescription>
						</CardHeader>
						<CardContent>
								...
								{[
									{
										...
										title: "IPO Alert",
										icon: "🎯",
										preview: "IPO opens [Date]. Price: ₹[Min]-₹[Max]",
									},
									{
										type: "mutual_fund",
										title: "Mutual Fund Update",
										icon: "📈",
										preview: "[Fund] delivered [X]% returns",
									},
									{
										type: "stock_tip",
										title: "Stock Alert",
										icon: "💹",
										preview: "[Symbol]: [Action] @ ₹[Price]",
									},
									{
										type: "loan",
										title: "Loan Offer",
										icon: "🏦",
										preview: "Pre-approved loan up to ₹[Amount]",
									},
									{
										type: "kyc_reminder",
										title: "KYC Reminder",
										icon: "🔐",
										preview: "Complete your KYC to unlock features",
									},
									{
										type: "portfolio_update",
										title: "Portfolio Update",
										icon: "📊",
										preview: "Your portfolio: ₹[Value], Today: [Change]%",
									},
								].map((template) => (
									<div
										key={template.type}
				...
				<TabsContent value="history">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								View past SMS campaigns and their performance
							</CardDescription>
						</CardHeader>
						<CardContent>
...
function QuickSendForm({
	onSend,
	isPending,
}: { onSend: (data: any) => void; isPending: boolean }) {
	...
	return (
		<form onSubmit={handleSubmit} className="space-y-4">
			...
			<Button
				type="submit"
				disabled={isPending || !phoneNumber || !productType}
