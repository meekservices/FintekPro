	...
	useEffect(() => {
		if (data?.config) {
			setConfidenceThreshold(
				Math.round(data.config.minConfidenceThreshold * 100),
			);
		}
	}, [data?.config]);
			...
			toast({
				...
				description:
					type === "all"
						...
						: "Profile cache has been cleared",
			});
		},
	});
					...
					<CardContent className="pt-6">
						<div className="text-center">
							...
							<p className="text-2xl font-bold text-green-600">
								{metrics?.totalParses
									? (
											(metrics.successfulParses / metrics.totalParses) *
											100
										).toFixed(0)
									: 0}
								%
					...
					<CardContent className="space-y-6">
						<div className="flex items-center justify-between">
							<div>
								...
								<p className="text-xs text-muted-foreground">
									Store successful parsing patterns for improved accuracy
								</p>
							</div>
							<Switch
								id="learning"
						...
						<div className="space-y-3">
							...
							<Slider
								...
								onValueCommit={([value]) =>
									updateConfig.mutate({ minConfidenceThreshold: value / 100 })
								}
								min={0}
								max={100}
								step={5}
							...
							/>
							<p className="text-xs text-muted-foreground">
								Holdings with confidence below this threshold will be flagged
								for manual review
							</p>
						</div>
				...
				<Card>
					...
					<CardContent className="space-y-4">
						<div className="grid grid-cols-3 gap-4 text-center">
							...
							<div>
								<p className="text-2xl font-bold text-green-600">
									{((cache?.hitRate ?? 0) * 100).toFixed(0)}%
								</p>
								<p className="text-xs text-muted-foreground">Hit Rate</p>
							</div>
							<div>
								<p className="text-2xl font-bold text-orange-600">
									{((cache?.missRate ?? 0) * 100).toFixed(0)}%
								</p>
				...
				Object.keys(metrics.providerBreakdown).length > 0 && (
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Document sources detected by provider type
							</CardDescription>
						</CardHeader>
						<CardContent>
			...
			{errorsData?.errors && errorsData.errors.length > 0 && (
				<Card>
					<CardHeader>
						...
						<CardDescription>
							Last {errorsData.count} parsing errors
						</CardDescription>
					</CardHeader>
					<CardContent>
