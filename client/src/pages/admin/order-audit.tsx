...
import type { UnifiedCartItem, ProductCategory, CartItemStatus, CartItemSource } from "@shared/schema";
...
const getSourceColor = (source: string) => {
  switch (source) {
    case 'ai': return 'bg-purple-900/30 text-purple-400 border-purple-700';
    case 'agent': return 'bg-blue-900/30 text-blue-400 border-blue-700';
    default: return 'bg-green-900/30 text-green-400 border-green-700';
  }
};
...
const getCategoryColor = (cat: ProductCategory) => {
  switch (cat) {
    case 'mutual_fund': return 'bg-blue-900/30 text-blue-400 border-blue-700';
    case 'bond': return 'bg-green-900/30 text-green-400 border-green-700';
...
export default function OrderAuditDashboard() {
  ...
  const { data: ordersResponse, isLoading, refetch } = useQuery<{
    ...
    queryKey: ['/api/unified-cart/admin/all', filters.source, filters.status, currentPage],
    queryFn: async () => {
      ...
      const res = await fetch(`/api/unified-cart/admin/all?${params.toString()}`, {
        credentials: 'include'
      });
  ...
  const stats = {
    ...
    totalValue: orders.reduce((sum, o) => sum + (parseFloat(o.amount || '0') || 0), 0)
  };
  
  const categories: ('all' | ProductCategory)[] = ['all', 'store', 'unlisted', 'mutual_fund', 'bond', 'ncd', 'ipo'];
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="client" data-testid="option-source-client">Client</SelectItem>
                  <SelectItem value="agent" data-testid="option-source-agent">Agent</SelectItem>
                  <SelectItem value="ai" data-testid="option-source-ai">AI</SelectItem>
                </SelectContent>
              </Select>
              <Select 
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="pending_approval" data-testid="option-status-pending">Pending Approval</SelectItem>
              ...
              {categories.map(cat => (
                ...
                >
                  <span className="flex items-center gap-2">
                    {cat === 'all' ? <ShoppingCart className="w-4 h-4" /> : getCategoryIcon(cat as ProductCategory)}
                    {categoryLabels[cat]}
                    <Badge variant="secondary" className="ml-1 text-xs" data-testid={`badge-count-${cat}`}>
                      {cat === 'all' 
                        ? orders.length 
                        : orders.filter(o => o.productCategory === cat).length}
                    </Badge>
                  </span>
                </TabsTrigger>
              ))}
                        ...
                        {filteredOrders.map(order => (
                          <TableRow key={order.id} className="border-border" data-testid={`row-order-${order.id}`}>
                            ...
                            <TableCell>
                              <Badge 
                                variant="outline" 
                                className={getCategoryColor(order.productCategory as ProductCategory)}
                            ...
                            <TableCell>
                              ...
                              >
                                ...
                                {sourceLabels[(order.source || 'client') as CartItemSource]}
                              </Badge>
                            </TableCell>
                            ...
                            <TableCell>
                              <Badge 
                                variant="outline" 
                                className={getStatusColor(order.status || 'active')}
      ...
      <Card className="bg-card border-border" data-testid="card-source-legend">
        ...
        <CardContent>
          <div className="flex flex-wrap gap-4">
            <div className="flex items-center gap-2" data-testid="legend-client">
              <Badge variant="outline" className="flex items-center gap-1 bg-green-900/30 text-green-400 border-green-700">
