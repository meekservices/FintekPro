...
const statusConfig: Record<string, { label: string; color: string }> = {
  ...
  needs_review: { label: "Needs Review", color: "bg-orange-500/20 text-orange-400" },
};
...
export default function ExchangeFilingsAdmin() {
  ...
  const { data: statsData, isLoading: statsLoading } = useQuery<{ success: boolean; data: FilingStats }>({
    queryKey: ["/api/admin/exchange-filings/stats"],
  });

  const { data: filingsData, isLoading: filingsLoading, refetch } = useQuery<{ success: boolean; data: FilingsResponse }>({
    queryKey: [
  ...
  const { data: filingDetail, isLoading: detailLoading } = useQuery<{ success: boolean; data: FilingDetail }>({
    queryKey: ["/api/admin/exchange-filings", selectedFiling],
    enabled: !!selectedFiling,
  });

  const fetchMutation = useMutation({
    mutationFn: async (params: { exchange?: string; fromDate?: string; toDate?: string }) => {
    ...
    onSuccess: (data: any) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["/api/admin/exchange-filings"] });
      refetch();
    },
    onError: (error: any) => {
      toast({
  ...
  const approveMutation = useMutation({
    mutationFn: async ({ filingId, metricId, justification }: { filingId: string; metricId: string; justification: string }) => {
      return apiRequest(`/api/admin/exchange-filings/${filingId}/metrics/${metricId}/approve`, {
        method: "POST",
        body: JSON.stringify({
    ...
    onSuccess: () => {
      toast({ title: "Metric Approved", description: "The metric value has been approved for use." });
      queryClient.invalidateQueries({ queryKey: ["/api/admin/exchange-filings", selectedFiling] });
      setApprovalJustification("");
    },
  });

  const overrideMutation = useMutation({
    mutationFn: async ({ filingId, metricId, newValue, reason }: { filingId: string; metricId: string; newValue: string; reason: string }) => {
      return apiRequest(`/api/admin/exchange-filings/${filingId}/metrics/${metricId}/override`, {
        method: "POST",
        body: JSON.stringify({
          newValue,
    ...
    onSuccess: () => {
      toast({ title: "Metric Overridden", description: "The metric value has been overridden with audit trail." });
      queryClient.invalidateQueries({ queryKey: ["/api/admin/exchange-filings", selectedFiling] });
      setOverrideValue("");
      setOverrideReason("");
    },
  ...
  return (
    <div className="min-h-screen bg-background p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex items-center justify-between" data-testid="header-section">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground">
                NSE/BSE filings ingestion and data extraction with SEBI compliance
              </p>
            </div>
          </div>
          <div className="flex gap-2">
            ...
            <Button variant="ghost" onClick={() => refetch()} data-testid="button-refresh">
              <RefreshCw className="h-4 w-4" />
            </Button>
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-5 gap-4" data-testid="stats-section">
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-blue-400">{stats?.byExchange?.nse || 0}</p>
                </div>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-purple-400">{stats?.byExchange?.bse || 0}</p>
                </div>
                  ...
                  <TableBody>
                    {filings.map((filing) => (
                      <TableRow key={filing.id} data-testid={`row-filing-${filing.id}`}>
                        <TableCell>
                          <Badge variant="outline" className={filing.exchange === "NSE" ? "border-blue-500 text-blue-400" : "border-purple-500 text-purple-400"}>
                            {filing.exchange}
                          </Badge>
                        </TableCell>
                        <TableCell className="font-mono">{filing.symbol}</TableCell>
                        <TableCell className="max-w-48 truncate">{filing.company_name}</TableCell>
                        <TableCell>
                        ...
                        <TableCell>
                          {filing.extraction_confidence ? (
                            <span className={filing.extraction_confidence >= 0.9 ? "text-green-400" : filing.extraction_confidence >= 0.8 ? "text-yellow-400" : "text-red-400"}>
                        ...
                        <TableCell>
                          <div className="flex gap-1">
                            ...
                            {filing.processing_status === "pending" && filing.document_type === "XBRL" && (
                              <Button
                                variant="ghost"
                                size="sm"
                            ...
                            <a href={filing.document_url} target="_blank" rel="noopener noreferrer">
                              <Button variant="ghost" size="sm" data-testid={`button-original-${filing.id}`}>
            ...
            {detailLoading ? (
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <Label className="text-muted-foreground">Exchange / Symbol</Label>
                    <p className="font-medium">{detail.filing.exchange} - {detail.filing.symbol}</p>
                  </div>
                  <div>
                    <Label className="text-muted-foreground">Filing Type</Label>
                    <p className="font-medium">{detail.filing.filing_type} ({detail.filing.financial_type})</p>
                  </div>
                  <div>
                    <Label className="text-muted-foreground">Document Hash</Label>
                    <p className="font-mono text-sm truncate">{detail.documentHash}</p>
                  </div>
                </div>
                      ...
                      {detail.extractedMetrics.map((metric) => (
                        <TableRow key={metric.id} data-testid={`row-metric-${metric.id}`}>
                          ...
                          <TableCell className="font-mono">
                            {parseFloat(metric.metric_value || "0").toLocaleString("en-IN")}
                          </TableCell>
                          <TableCell>
                            <span className={metric.extraction_confidence >= 0.9 ? "text-green-400" : "text-yellow-400"}>
                              {(metric.extraction_confidence * 100).toFixed(0)}%
                            </span>
                          </TableCell>
                          <TableCell>
                            <Badge variant="outline">{metric.extraction_method}</Badge>
                          </TableCell>
                          <TableCell>
                            {metric.is_approved ? (
                          ...
                          <TableCell>
                            <div className="flex gap-1">
                                ...
                                <Dialog>
                                  ...
                                  <DialogContent>
                                    <DialogHeader>
                                      ...
                                      <DialogDescription>
                                        Provide justification for SEBI compliance audit trail
                                      </DialogDescription>
                                    </DialogHeader>
                                    <div className="space-y-4">
                                      <div>
                                        <Label>Metric: {metric.metric}</Label>
                                        <p className="text-lg font-mono">{metric.metric_value}</p>
                                      </div>
                                      <div>
                                    ...
                                    <DialogFooter>
                                      <Button
                                        ...
                                        disabled={approvalJustification.length < 10 || approveMutation.isPending}
                              ...
                              <Dialog>
                                ...
                                <DialogContent>
                                  <DialogHeader>
                                    <DialogTitle>Override Metric Value</DialogTitle>
                                    <DialogDescription>
                                      Provide new value and detailed reason for SEBI compliance
                                    </DialogDescription>
                                  </DialogHeader>
                                  <div className="space-y-4">
                                    <div>
                                      <Label>Current Value</Label>
                                      <p className="font-mono text-lg">{metric.metric_value}</p>
                                    </div>
                                    <div>
                                  ...
                                  <DialogFooter>
                                    <Button
                                      variant="destructive"
                                      onClick={() => {
                                        if (overrideReason.length >= 20 && overrideValue) {
                                      ...
                                      disabled={overrideReason.length < 20 || !overrideValue || overrideMutation.isPending}
                                      data-testid="button-confirm-override"
                                    >
                                      {overrideMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : "Override Value"}
                                    </Button>
                                  </DialogFooter>
                                </DialogContent>
                              </Dialog>
