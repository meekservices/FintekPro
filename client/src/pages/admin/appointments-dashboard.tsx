...
export default function AdminAppointmentsDashboard() {
  ...
  const [selectedUser, setSelectedUser] = useState<PendingAppointment | null>(null);
  ...
  const approveMutation = useMutation({
    mutationFn: (userId: string) =>
      apiRequest(`/api/admin/appointments/${userId}/approve`, { method: "POST" }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/appointments/pending"] });
      queryClient.invalidateQueries({ queryKey: ["/api/admin/appointments/stats"] });
      toast({
  ...
  const filteredAppointments = pendingData?.appointments?.filter(apt => {
    const matchesSearch = 
    ...
    const matchesRole = roleFilter === "all" || apt.requestedRole === roleFilter;
          ...
          <TabsContent value="pending" className="mt-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Review and take action on pending role appointments</CardDescription>
              </CardHeader>
              <CardContent>
                {isLoading ? (
                        ...
                        <TableRow key={apt.id} data-testid={`appointment-row-${apt.id}`}>
                          <TableCell>
                            <div>
                              ...
                              <div className="text-sm text-muted-foreground">{apt.email}</div>
                              {apt.panNumber && (
                                <div className="text-xs text-muted-foreground">PAN: {apt.panNumber}</div>
                              )}
                            </div>
                          </TableCell>
                          <TableCell>{getRoleBadge(apt.requestedRole)}</TableCell>
                          <TableCell>
                            <div>
                              <div className="text-sm">{apt.initiatorName}</div>
                              <div className="text-xs text-muted-foreground">
                                {apt.appointmentInitiatorRole?.replace(/_/g, " ")}
                              </div>
                            </div>
                          </TableCell>
                          <TableCell>
                            {apt.appointmentCostCentreId || <span className="text-muted-foreground">-</span>}
                          </TableCell>
                          <TableCell>{getStatusBadge(apt.appointmentStatus)}</TableCell>
                          <TableCell>
          ...
          <TabsContent value="audit" className="mt-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Complete history of appointment actions</CardDescription>
              </CardHeader>
              <CardContent>
        ...
        <Dialog open={showApproveDialog} onOpenChange={setShowApproveDialog}>
          <DialogContent>
            ...
            <DialogFooter>
              ...
              >
                {approveMutation.isPending ? "Approving..." : "Approve Appointment"}
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
