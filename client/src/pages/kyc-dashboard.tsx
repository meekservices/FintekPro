...
export default function KYCDashboard() {
	...
	const upgradeMutation = useMutation({
		mutationFn: async (targetTier: string) => {
			return await apiRequest("POST", "/api/kyc/request-upgrade", {
				body: { targetTier },
			});
		},
		onSuccess: (data) => {
			toast({
				...
				description:
					data.message || "Your KYC tier upgrade request has been submitted.",
			});
		...
		<div className="container mx-auto p-6 space-y-6">
			...
			<div className="flex justify-between items-center">
				<div>
					...
					<p className="text-muted-foreground">
						Manage your verification and access
					</p>
				</div>
				<Badge
					className={`${getTierColor(profile?.kycTier || "basic")} text-foreground px-4 py-2 text-lg`}
					data-testid="badge-kyc-tier"
				>
					{getTierIcon(profile?.kycTier || "basic")}
					<span className="ml-2">
						{formatTierName(profile?.kycTier || "basic")}
					</span>
				</Badge>
			</div>
			...
			<Card data-testid="card-kyc-profile">
				<CardHeader>
					...
					<CardDescription>
						Your unique identification and verification status
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-4">
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
						...
						<div>
							...
							<p className="font-semibold" data-testid="text-user-id">
								{profile?.userId}
							</p>
						</div>
						...
						<div>
							...
							<p className="font-semibold" data-testid="text-full-name">
						...
						<div>
							...
							<p className="font-semibold" data-testid="text-mobile">
								{profile?.mobile}
							</p>
						</div>
						...
						<div>
							...
							<p className="font-semibold" data-testid="text-pan">
								{profile?.panNumber || "Not verified"}
							</p>
						</div>
						...
						<div>
							...
							<Badge
								variant={
									profile?.kycStatus === "approved" ? "default" : "secondary"
								}
					...
					<div>
						...
						<div className="grid grid-cols-2 md:grid-cols-4 gap-3">
							...
							<ComplianceItem
								label="FATCA Status"
								value={
									profile?.fatcaStatus === "Y" ? "Declared" : "Not Applicable"
								}
			...
			<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
				...
				<Card data-testid="card-current-tier">
					<CardHeader>
						...
						<CardDescription>
							{profile?.kycTierMetadata?.description}
						</CardDescription>
					</CardHeader>
					<CardContent>
						<div className="space-y-3">
							<div className="flex items-center gap-2">
								...
								<span className="text-sm">
									{profile?.kycTierMetadata?.productsUnlocked?.length || 0}{" "}
									Products Unlocked
								</span>
							</div>
							{profile?.kycTierMetadata?.maxAnnualInvestment && (
								<div className="flex items-center gap-2">
									...
									<span className="text-sm">
										Max Investment: ₹
										{(
											profile.kycTierMetadata.maxAnnualInvestment / 1000
										).toFixed(0)}
										K/year
									</span>
								</div>
				...
				{eligibility?.nextTier && (
					...
					>
						<CardHeader>
							...
							<CardDescription>
								Unlock more investment products and higher limits
							</CardDescription>
						</CardHeader>
						<CardContent>
							<Dialog>
								...
								<DialogContent>
									<DialogHeader>
										...
										<DialogDescription>
											...
											{formatTierName(eligibility.nextTier)}. Our compliance
											team will review your request within 24-48 hours.
										</DialogDescription>
									</DialogHeader>
									<Button
			...
			<Card data-testid="card-product-eligibility">
				<CardHeader>
					...
					<CardDescription>
						Products you can access based on your KYC tier
					</CardDescription>
				</CardHeader>
				<CardContent>
					<div className="space-y-6">
						...
						<div>
							...
							<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
								{eligibility?.accessibleProducts
									?.slice(0, 6)
									.map((product: any) => (
										<ProductCard
			...
			{profile?.riskNextReview && (
				<Alert data-testid="alert-rekyc">
					...
					<AlertDescription>
						Next KYC Review Due:{" "}
						{new Date(profile.riskNextReview).toLocaleDateString("en-IN")}
					</AlertDescription>
				</Alert>
			)}
		</div>
			...
			<div className="flex items-center gap-3 p-3 bg-red-50 dark:bg-red-950 border border-red-200 dark:border-red-800 rounded-lg">
				...
				<div>
					...
					<p className="text-xs text-red-600 dark:text-red-400">
						Expired on {format(expiry, "dd MMM yyyy")} — please renew your Video
						KYC
					</p>
				</div>
		...
		return (
			<div className="flex items-center gap-3 p-3 bg-red-50 dark:bg-red-950 border border-red-200 dark:border-red-800 rounded-lg">
				...
				<div>
					...
					<p className="text-xs text-red-600 dark:text-red-400">
						Expiry date: {format(expiry, "dd MMM yyyy")}
					</p>
				</div>
				<Badge variant="destructive" className="ml-auto shrink-0">
	...
	if (monthsLeft <= 6) {
		return (
			<div className="flex items-center gap-3 p-3 bg-amber-50 dark:bg-amber-950 border border-amber-200 dark:border-amber-800 rounded-lg">
				...
				<Badge className="ml-auto shrink-0 bg-amber-100 text-amber-800 dark:bg-amber-900 dark:text-amber-200">
					Expires in {monthsLeft}mo
				</Badge>
			</div>
		);
	}
