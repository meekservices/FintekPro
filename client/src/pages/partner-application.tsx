...
export default function PartnerApplicationPage() {
	...
	const currentLender = lendersData?.data?.find(
		(l: LenderInfo) => l.name === lender,
	);
	...
	const submitApplicationMutation = useMutation({
		...
		onSuccess: () => {
			...
			toast({
				...
				description:
					"Your application has been submitted to the lender successfully.",
			});
		...
		return (
			<div className="container mx-auto px-4 py-8">
				<Alert variant="destructive">
					<AlertCircle className="h-4 w-4" />
					<AlertDescription>
						Lender not found. Please select a valid lender from loan
						recommendations.
					</AlertDescription>
				</Alert>
			</div>
	...
	return (
		...
		>
			...
			<div className="mb-8">
				...
				>
					Complete your loan application with pre-filled information from your
					profile.
				</p>
											...
											<FormItem>
												...
												<FormControl>
													<Input
														...
														data-testid="input-aadharNumber"
													/>
												</FormControl>
												<FormMessage />
											</FormItem>
										...
										<FormItem>
											...
											<FormControl>
												<Textarea
													...
													data-testid="textarea-currentAddress"
												/>
											</FormControl>
											<FormMessage />
										</FormItem>
											...
											<FormItem>
												...
												<FormControl>
													<Input
														{...field}
														placeholder="110001"
														data-testid="input-currentPincode"
													/>
												</FormControl>
												<FormMessage />
											</FormItem>
										...
										render={({ field }) => (
											<FormItem>
												...
												<FormControl>
													<Input
														{...field}
														placeholder="Optional - for faster processing"
														data-testid="input-accountNumber"
													/>
												</FormControl>
												<FormDescription>
													Account number is optional but helps in faster
													processing
												</FormDescription>
												<FormMessage />
											</FormItem>
										...
										<FormItem>
											...
											<FormControl>
												<Textarea
													...
													data-testid="textarea-loanPurpose"
												/>
											</FormControl>
											<FormMessage />
										</FormItem>
						...
						<Card data-testid="card-documents">
							...
							<CardContent className="space-y-6">
								<div className="grid grid-cols-1 gap-6">
									<DocumentUploadField
										...
										onUploadComplete={(doc) => {
											...
											const currentIds =
												form.getValues("documentIds.panCard") || [];
											form.setValue("documentIds.panCard", [
												...currentIds,
												doc.id,
											]);
										}}
									...
									<DocumentUploadField
										...
										onUploadComplete={(doc) => {
											...
											const currentIds =
												form.getValues("documentIds.aadharCard") || [];
											form.setValue("documentIds.aadharCard", [
												...currentIds,
												doc.id,
											]);
										}}
										onRemove={(docId) => {
											setUploadedDocuments((prev) => ({
												...prev,
												aadharCard: prev.aadharCard.filter(
													(doc) => doc.id !== docId,
												),
											}));
											const currentIds =
									...
									<DocumentUploadField
										...
										onUploadComplete={(doc) => {
											...
											const currentIds =
												form.getValues("documentIds.salarySlips") || [];
											form.setValue("documentIds.salarySlips", [
												...currentIds,
												doc.id,
											]);
										}}
										onRemove={(docId) => {
											setUploadedDocuments((prev) => ({
												...prev,
												salarySlips: prev.salarySlips.filter(
													(doc) => doc.id !== docId,
												),
											}));
											const currentIds =
									...
									<DocumentUploadField
										...
										onUploadComplete={(doc) => {
											...
											const currentIds =
												form.getValues("documentIds.bankStatements") || [];
											form.setValue("documentIds.bankStatements", [
												...currentIds,
												doc.id,
											]);
										}}
										onRemove={(docId) => {
											setUploadedDocuments((prev) => ({
												...prev,
												bankStatements: prev.bankStatements.filter(
													(doc) => doc.id !== docId,
												),
											}));
											const currentIds =
								...
								<Alert>
									...
									<AlertDescription>
										Document upload is optional at this stage. You can submit
										your application and upload documents later when requested
										by the lender.
									</AlertDescription>
								</Alert>
							</CardContent>
								...
								<div className="grid grid-cols-1 md:grid-cols-2 gap-8">
									<div>
										...
										<div className="space-y-2 text-sm">
											...
											<div>
												<strong>PAN:</strong> {form.watch("panNumber")}
											</div>
											<div>
												<strong>Email:</strong> {form.watch("email")}
											</div>
											<div>
												<strong>Mobile:</strong> {form.watch("mobile")}
											</div>
										</div>
									</div>
									...
									<div>
										...
										<div className="space-y-2 text-sm">
											<div>
												<strong>Type:</strong> {form.watch("employmentType")}
											</div>
											<div>
												<strong>Company:</strong> {form.watch("employerName")}
											</div>
											<div>
												<strong>Income:</strong> ₹
												{form.watch("monthlyIncome")?.toLocaleString()}/month
											</div>
										</div>
									</div>
									...
									<div>
										...
										<div className="space-y-2 text-sm">
											...
											<div>
												<strong>Tenure:</strong> {form.watch("tenure")} months
											</div>
											<div>
												<strong>Purpose:</strong> {form.watch("loanPurpose")}
											</div>
										</div>
									</div>
									...
									<div>
										...
										<div className="space-y-2 text-sm">
											<div>
												<strong>Bank:</strong> {form.watch("bankName")}
											</div>
											<div>
												<strong>Type:</strong> {form.watch("accountType")}
											</div>
										</div>
									</div>
								</div>
