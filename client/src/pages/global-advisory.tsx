			...
			<CardContent>
				<div className="grid grid-cols-2 gap-4 mb-4">
					<div>
						...
						<p className="font-semibold">
							{formatCurrency(rec.currentPrice, rec.currency)}
						</p>
						<p className="text-xs text-muted-foreground">
							₹{rec.currentPriceInr.toFixed(2)}
						</p>
					</div>
					<div>
				...
				<div className="flex justify-between items-center mb-4">
					<div>
						...
						<p
							className={`font-semibold ${rec.expectedReturn >= 0 ? "text-green-600" : "text-red-600"}`}
						>
				...
				{expanded && (
					<div className="mt-4 pt-4 border-t space-y-3">
						...
						<div className="bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg">
							...
							<p className="text-xs text-muted-foreground">
								STCG: {rec.taxImplications.stcgRate}% | LTCG:{" "}
								{rec.taxImplications.ltcgRate}%
								{rec.taxImplications.dtaaRate &&
									` | DTAA: ${rec.taxImplications.dtaaRate}%`}
							</p>
							<p className="text-xs text-blue-600 mt-1">
								{rec.taxImplications.taxTip}
							</p>
						</div>
...
function GlobalAdvisoryPage() {
	...
	const { data: lrsData, isLoading: lrsLoading } = useQuery<{
		success: boolean;
		data: LrsStatus;
	}>({
	...
	} = useQuery<{ success: boolean; data: GlobalRecommendation[] }>({
		queryKey: [
			"/api/global-advisory/advisory/stocks",
			{ riskLevel, maxResults: 10 },
		],
	...
	const {
		...
		queryKey: [
			"/api/global-advisory/advisory/etfs",
			{ riskLevel, maxResults: 10 },
		],
	...
	const {
		...
		queryKey: [
			"/api/global-advisory/advisory/bonds",
			{ riskLevel, maxResults: 10 },
		],
			...
			<div className="flex justify-between items-center mb-6">
				<div>
					...
					<p className="text-muted-foreground">
						AI-powered recommendations for global stocks, ETFs, bonds, and
						mutual funds
					</p>
				</div>
						...
						{lrsLoading ? (
							...
							<div>
								<p className="text-2xl font-bold">
									${(lrsStatus.remainingLimitUsd / 1000).toFixed(0)}K
								</p>
								<p className="text-xs text-muted-foreground">
									Remaining of $250K
								</p>
					...
					<CardContent>
						{lrsLoading ? (
							...
							<div>
								<Badge
									variant={
										lrsStatus?.fatcaStatus === "compliant"
											? "default"
											: "secondary"
									}
								>
								...
								<p className="text-xs text-muted-foreground mt-2">
									W-8BEN: {lrsStatus?.w8benStatus || "Not filed"}
								</p>
							</div>
						)}
					</CardContent>
				...
				<Card>
					...
					<CardContent>
						...
						<p className="text-xs text-muted-foreground">
							Above ₹
							{(
								(lrsStatus?.taxImplications?.tcsThreshold || 700000) / 100000
							).toFixed(0)}{" "}
							lakhs/FY
						</p>
					</CardContent>
				...
				<TabsContent value="stocks">
					...
					{stocksLoading ? (
						...
						<Card className="p-8 text-center">
							...
							<p className="text-muted-foreground">
								No stock recommendations available. Try refreshing.
							</p>
						</Card>
					)}
				</TabsContent>
				...
				<TabsContent value="etfs">
					...
					{etfsLoading ? (
						...
						<Card className="p-8 text-center">
							<Info className="h-12 w-12 mx-auto text-muted-foreground mb-4" />
							<p className="text-muted-foreground">
								No ETF recommendations available. Try refreshing.
							</p>
						</Card>
					)}
				</TabsContent>
				...
				<TabsContent value="bonds">
					...
					{bondsLoading ? (
						...
						<Card className="p-8 text-center">
							<Info className="h-12 w-12 mx-auto text-muted-foreground mb-4" />
							<p className="text-muted-foreground">
								No bond recommendations available. Try refreshing.
							</p>
						</Card>
					)}
				</TabsContent>
					...
					<Card className="p-8">
						<div className="text-center">
							...
							<p className="text-muted-foreground mb-6">
								Add your global holdings to get AI-powered rebalancing
								recommendations with buy/hold/sell signals
							</p>
					...
					<div className="flex items-start gap-3">
						...
						<div>
							...
							<p className="text-xs text-blue-600 dark:text-blue-300 mt-1">
								These recommendations are for informational purposes only and do
								not constitute investment advice. Past performance is not
								indicative of future results. Investments in global markets
								involve currency risk, regulatory risk, and market volatility.
								Please consult with a SEBI-registered investment advisor before
								making investment decisions. LRS remittances are subject to RBI
								guidelines and TCS provisions.
							</p>
						</div>
					</div>
				</CardContent>
