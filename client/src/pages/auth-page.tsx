...
const PORTAL_DESCRIPTIONS: Record<
	string,
	{ hero: string; features: { icon: string; title: string; desc: string }[] }
> = {
	main: {
		...
		features: [
			{
				...
				desc: "Multiple sign-in options for your convenience",
			},
			{
				icon: "trending",
				title: "ITR & Tax Services",
				desc: "AI-powered tax filing with expert assistance",
			},
			{
				icon: "chart",
				title: "Portfolio Management",
				desc: "Track and manage your investments",
			},
			{
				icon: "message",
				title: "Real-time Insights",
				desc: "Live market data and AI recommendations",
			},
		],
	},
	partner: {
		...
		features: [
			{
				...
				desc: "Track commissions, payouts, and performance",
			},
			{
				icon: "trending",
				title: "Client Management",
				desc: "Manage your client portfolio efficiently",
			},
			{
				icon: "chart",
				title: "Commission Tracking",
				desc: "Real-time earnings and payout statements",
			},
			{
				icon: "message",
				title: "Growth Tools",
				desc: "Marketing and lead management resources",
			},
		],
	},
	agent: {
		...
		features: [
			{
				...
				desc: "Comprehensive tools for client management",
			},
			{
				icon: "trending",
				title: "KYC & Onboarding",
				desc: "Streamlined client verification workflows",
			},
			{
				icon: "chart",
				title: "Portfolio Analysis",
				desc: "Deep insights into client portfolios",
			},
			{
				icon: "message",
				title: "AI Recommendations",
				desc: "Smart suggestions for client investments",
			},
		],
	},
	admin: {
		...
		features: [
			{
				...
				desc: "Control access and manage platform users",
			},
			{
				icon: "trending",
				title: "Compliance Monitor",
				desc: "SEBI/RBI regulatory compliance tracking",
			},
			{
				icon: "chart",
...
export default function AuthPage() {
	...
	const portalDesc =
		PORTAL_DESCRIPTIONS[portalType] || PORTAL_DESCRIPTIONS.main;
	...
	const [registrationOtpDialogOpen, setRegistrationOtpDialogOpen] =
		useState(false);
	const [registrationIdentifier, setRegistrationIdentifier] = useState("");
	const [registrationOtpChannel, setRegistrationOtpChannel] =
		useState<string>("");
	const [registrationOtpTimer, setRegistrationOtpTimer] = useState(300);
	const [canResendRegistrationOtp, setCanResendRegistrationOtp] =
		useState(false);
	...
	const [loginStep, setLoginStep] = useState<
		"credentials" | "otp" | "pin-entry" | "pin-setup" | "complete"
	>("credentials");
	...
	const pinDigitRefs = useRef<Array<HTMLInputElement | null>>([
		null,
		null,
		null,
		null,
	...
	const pinSetupRefs = useRef<Array<HTMLInputElement | null>>([
		null,
		null,
		null,
		null,
	...
	const mainPinSetupRefs = useRef<Array<HTMLInputElement | null>>([
		null,
		null,
		null,
		null,
	]);
	const mainPinConfirmRefs = useRef<Array<HTMLInputElement | null>>([
		null,
		null,
		null,
		null,
	...
	const [loginMethod, setLoginMethod] = useState<"password" | "otp" | "pin">(
		"password",
	);
	...
	const pinDirectRefs = useRef<Array<HTMLInputElement | null>>([
		null,
		null,
		null,
		null,
	...
	useEffect(() => {
		...
		return () => {
			document.title = "FintekPro - Smart Financial Services Platform";
		};
		...
		onSuccess: (response) => {
			...
			if (data.requiresPin) {
				...
				toast({
					...
					description:
						"Trusted device recognised — enter your 4-digit PIN to continue",
				});
				return;
			}
			...
			if (!data.requiresOtp) {
				console.error(
					"Security Error: Login response missing requiresOtp/requiresPin flag",
				);
				toast({
					title: "Login Error",
			...
			otpForm.setValue(
				"identifier",
				data.identifier || loginForm.getValues("identifier"),
			);
	...
	const pinSetupMutation = useMutation({
		...
		onSuccess: (response: any) => {
			...
			toast({
				title: "PIN created!",
				description: "Next time you log in on this device, just use your PIN.",
			});
			navigate(portalHomeRoute);
		},
		onError: (error: Error) => {
	...
	const registerMutation = useMutation({
		...
		onSuccess: (
			...
			if (
				data.warnings?.hasDuplicates &&
				(data.warnings.duplicates?.length ?? 0) > 0
			) {
	...
	const requestOtpMutation = useMutation({
		...
		onSuccess: (data: any) => {
			...
			toast({
				title: "OTP Sent",
				description: `Verification code sent to ${sentTo}`,
			});
		},
		onError: (error: Error) => {
			toast({
	...
	if (portalType === "agent") {
		return (
			<div className="min-h-screen flex flex-col lg:flex-row">
				...
				<div className="lg:w-5/12 flex flex-col justify-between p-6 sm:p-10 lg:p-16 relative overflow-hidden min-h-[200px] sm:min-h-[260px] lg:min-h-screen bg-[#0d1b2e]">
					...
					>
						<g stroke="#3b82f6" strokeOpacity="0.12" strokeWidth="1">
							<line x1="60" y1="90" x2="190" y2="170" />
							<line x1="190" y1="170" x2="330" y2="75" />
							<line x1="330" y1="75" x2="490" y2="130" />
							<line x1="490" y1="130" x2="610" y2="55" />
							<line x1="610" y1="55" x2="720" y2="145" />
							<line x1="60" y1="90" x2="110" y2="260" />
							<line x1="110" y1="260" x2="210" y2="305" />
							<line x1="210" y1="305" x2="310" y2="255" />
							<line x1="310" y1="255" x2="330" y2="75" />
							<line x1="410" y1="195" x2="460" y2="355" />
							<line x1="460" y1="355" x2="560" y2="285" />
							<line x1="560" y1="285" x2="660" y2="225" />
							<line x1="660" y1="225" x2="720" y2="145" />
							<line x1="110" y1="260" x2="185" y2="455" />
							<line x1="185" y1="455" x2="285" y2="385" />
							<line x1="285" y1="385" x2="385" y2="455" />
							<line x1="490" y1="505" x2="585" y2="425" />
							<line x1="585" y1="425" x2="685" y2="385" />
							<line x1="685" y1="385" x2="760" y2="305" />
							<line x1="125" y1="555" x2="225" y2="525" />
							<line x1="225" y1="525" x2="325" y2="560" />
							<line x1="325" y1="560" x2="490" y2="505" />
						</g>
						<g fill="#60a5fa" fillOpacity="0.35">
							<circle cx="60" cy="90" r="3" />
							<circle cx="190" cy="170" r="2.5" />
							<circle cx="330" cy="75" r="3" />
							<circle cx="490" cy="130" r="2" />
							<circle cx="610" cy="55" r="3" />
							<circle cx="720" cy="145" r="2.5" />
							<circle cx="110" cy="260" r="2" />
							<circle cx="210" cy="305" r="3" />
							<circle cx="310" cy="255" r="2" />
							<circle cx="410" cy="195" r="2.5" />
							<circle cx="460" cy="355" r="3" />
							<circle cx="560" cy="285" r="2" />
							<circle cx="660" cy="225" r="2.5" />
							<circle cx="185" cy="455" r="2" />
							<circle cx="285" cy="385" r="3" />
							<circle cx="385" cy="455" r="2" />
							<circle cx="490" cy="505" r="2.5" />
							<circle cx="585" cy="425" r="2" />
							<circle cx="685" cy="385" r="3" />
							<circle cx="760" cy="305" r="2" />
							<circle cx="125" cy="555" r="2.5" />
							<circle cx="225" cy="525" r="2" />
							<circle cx="325" cy="560" r="3" />
						</g>
					</svg>
					...
					<div className="relative z-10 mt-auto pt-4 sm:pt-8 lg:pt-12">
						...
						<p className="hidden sm:block text-blue-200/65 text-sm lg:text-base mb-6 sm:mb-10 leading-relaxed max-w-sm">
							The complete toolkit for SEBI-registered distributors and
							advisors. Manage clients, execute trades, and grow your AUM
							efficiently.
						</p>
						{/* Stats */}
						<div className="flex gap-6 sm:gap-10">
							<div>
								...
								<div className="text-blue-300/55 text-[10px] sm:text-xs uppercase tracking-widest mt-0.5">
									AUM Managed
								</div>
							</div>
							<div>
						...
						{authMode === "login" ? (
							<>
									...
									<form
										onSubmit={(e) => {
											...
											if (otpLoginEmail.trim())
												requestOtpMutation.mutate(otpLoginEmail.trim());
										}}
										className="space-y-4"
									>
										<div>
											<Label
												htmlFor="agent-otp-id"
												className="text-gray-700 text-sm font-medium"
											>
												Mobile or Email
											</Label>
										...
										<Button
											...
											className="w-full bg-blue-600 hover:bg-blue-700 text-white font-medium"
											disabled={
												requestOtpMutation.isPending || !otpLoginEmail.trim()
											}
										>
								...
								{loginMethod === "pin" && (
									<div className="space-y-4">
										...
										<div>
											<Label
												htmlFor="agent-pin-direct-group"
												className="text-gray-700 text-sm font-medium"
											>
												4-Digit PIN
											</Label>
											...
											<p className="text-xs text-gray-400 mt-2 text-center">
												Available on trusted devices only
											</p>
										</div>
							...
							<>
								...
								<div className="bg-blue-50 border border-blue-100 rounded-lg p-3 mb-5 text-sm text-blue-700 flex items-start gap-2">
									...
									<span>
										You'll receive a unique Agent ID on registration. Your
										account will be active immediately — use your email or ID to
										log in.
									</span>
								</div>

								{registrationStep === "otp" &&
									!registrationOtpDialogOpen &&
									registrationIdentifier && (
										<div className="mb-4 p-3 rounded-lg border-2 border-amber-400 bg-amber-50 text-sm text-amber-700">
											<strong>Verification pending!</strong> A 6-digit code was
											sent to <strong>{registrationOtpChannel}</strong>.{" "}
											<button
												type="button"
				...
				>
					<DialogContent className="sm:max-w-[380px] text-center">
						<DialogHeader>
							...
							<DialogDescription>
								Trusted device recognised. Enter your 4-digit PIN to log in
								instantly.
							</DialogDescription>
						</DialogHeader>
				...
				<Dialog
					open={registrationOtpDialogOpen}
					onOpenChange={setRegistrationOtpDialogOpen}
				>
					<DialogContent
					...
					>
						<DialogHeader>
							...
							<DialogDescription>
								...
								<strong>{registrationOtpChannel}</strong>
							</DialogDescription>
						</DialogHeader>
						<form
							onSubmit={(e) => {
								...
								const otp = (formData.get("agent-registration-otp") ??
									"") as string;
								if (otp && otp.length === 6) {
						...
						>
								...
								<div className="flex items-center justify-between">
									<div className="flex items-center gap-2 text-blue-700">
										...
										<span className="text-sm font-medium">
											{registrationOtpTimer > 0
												? `Code expires in ${formatTime(registrationOtpTimer)}`
												: "Code expired"}
										</span>
									</div>
							...
							<Alert>
								...
								<AlertDescription className="text-xs">
									...
									Resend after the timer expires.
								</AlertDescription>
							</Alert>
							<div className="flex gap-2">
								<Button
								...
								>
									{registrationOtpVerificationMutation.isPending && (
										<Loader2 className="mr-2 h-4 w-4 animate-spin" />
									)}
					...
					<DialogContent className="sm:max-w-md">
						<DialogHeader>
							...
							<DialogDescription className="text-center">
								Welcome to FintekPro Agent Portal! Your account has been
								created.
							</DialogDescription>
						</DialogHeader>
						<div className="space-y-4">
							<div className="p-4 rounded-lg border-2 border-blue-200 bg-blue-50">
								...
								<p className="text-xs text-muted-foreground mt-2 text-center">
									Save this ID — you can use it to login along with email or
									mobile
								</p>
							</div>
							<Alert>
								...
								<AlertDescription className="text-sm">
									You can now sign in using your email, mobile number, or this
									Agent ID with your password.
								</AlertDescription>
							</Alert>
							<Button
				...
				>
					<DialogContent className="sm:max-w-lg">
						<DialogHeader>
							...
							<DialogDescription className="text-center">
								We found {duplicateWarnings.length} existing{" "}
								{duplicateWarnings.length === 1 ? "account" : "accounts"} with
						...
						<div className="space-y-4">
							...
							<Alert>
								<Info className="h-4 w-4" />
								<AlertDescription className="text-sm">
									If this is your existing account, please login instead. If
									you're a different agent, you can still proceed.
								</AlertDescription>
							</Alert>
									...
									onClick={() => {
										...
										if (pendingRegistrationData?.requiresOtp) {
											...
											setRegistrationIdentifier(
												pendingRegistrationData.identifier ||
													pendingRegistrationData.user?.email ||
													"",
											);
											setRegistrationOtpChannel(
												pendingRegistrationData.otpSentTo ||
													"your email and mobile",
											);
											setRegistrationToken(
												pendingRegistrationData.registrationToken || "",
											);
											setRegistrationOtpTimer(300);
											setCanResendRegistrationOtp(false);
											setRegistrationOtpDialogOpen(true);
      ...
      `}</style>
			<div className="min-h-screen portal-gradient-bg">
						...
						<div className="lg:pr-8">
							<div className="text-center lg:text-left">
								<h1 className="text-4xl font-bold text-foreground mb-2">
									Welcome to{" "}
									<span className="[color:var(--portal-color)]">
										{portalLabel}
									</span>
								</h1>
								<p className="text-sm font-medium mb-4 [color:var(--portal-color)]">
									{portalTagline}
								</p>
							...
							<Card className="w-full max-w-md shadow-lg">
								...
								<CardContent>
									<Tabs
										value={authMode}
										onValueChange={(v) => {
											if (v === "login" || v === "register") setAuthMode(v);
										}}
										className="space-y-4"
									>
										<ScrollableTabsList
											className={`grid w-full ${isAdminPortal ? "grid-cols-1" : "grid-cols-2"}`}
										>
											...
											{loginStep !== "credentials" && (
												<div className="space-y-2 p-3 rounded-lg border [border-color:color-mix(in_srgb,var(--portal-color)_30%,transparent)] [background-color:color-mix(in_srgb,var(--portal-color)_5%,transparent)]">
													<div className="flex items-center justify-between text-sm">
														...
														<span
															className={`flex items-center gap-1 font-medium ${loginStep === "complete" ? "text-green-600" : "[color:var(--portal-color)]"}`}
														>
															{loginStep === "complete" ? (
																<CheckCircle2 className="h-4 w-4" />
															) : (
														...
														<span
															className={`flex items-center gap-1 ${loginStep === "complete" ? "text-green-600 font-medium" : "text-muted-foreground"}`}
														>
															<CheckCircle2 className="h-4 w-4" />
															Success
														</span>
											...
											>
												...
												<Alert className="border [border-color:color-mix(in_srgb,var(--portal-color)_30%,transparent)] [background-color:color-mix(in_srgb,var(--portal-color)_5%,transparent)]">
													...
													<AlertDescription className="text-sm [color:var(--portal-color)]">
														After entering credentials, you'll receive a 6-digit
														OTP via email/SMS for verification.
													</AlertDescription>
												</Alert>
										...
										<TabsContent value="register" className="space-y-4">
												...
												registrationIdentifier && (
													...
													>
														...
														<AlertDescription className="text-sm text-amber-700 dark:text-amber-400">
															<strong>Verification pending!</strong> A 6-digit
															code was sent to{" "}
															<strong>{registrationOtpChannel}</strong>. You
															must enter it to complete registration.
															<Button
																type="button"
																size="sm"
											...
											<Alert
												className="border"
												style={{
													backgroundColor: `${portalColor}08`,
											...
											>
												...
												>
													{portalType === "agent" ? (
														<>
															You'll receive a unique Agent ID on registration.
															Your account will be active immediately — use your
															email or ID to log in.
														</>
													) : portalType === "partner" ? (
														<>
															Partner accounts are created with{" "}
															<strong>Pending</strong> approval status. Admin
															will verify and activate your account.
														</>
													) : (
														<>
															Upon registration, you'll receive a unique User ID
															in the format <strong>FTP001234</strong>. Save it
															for future logins!
														</>
													)}
												</AlertDescription>
				...
				<Dialog open={otpDialogOpen} onOpenChange={setOtpDialogOpen}>
					<DialogContent
						className="sm:max-w-md"
						data-testid="dialog-otp-verification"
					>
						<DialogHeader>
							<DialogTitle className="flex items-center gap-2">
				...
				<Dialog open={showUserIdDialog} onOpenChange={setShowUserIdDialog}>
					<DialogContent
						className="sm:max-w-md"
						data-testid="dialog-user-id-success"
					>
						<DialogHeader>
							<div className="flex justify-center mb-4">
							...
							<div className="p-4 rounded-lg border-2 [border-color:color-mix(in_srgb,var(--portal-color)_40%,transparent)] [background-color:color-mix(in_srgb,var(--portal-color)_5%,transparent)]">
								...
								<div className="flex items-center justify-center gap-2">
									...
									<Badge
										...
										style={{ backgroundColor: "var(--portal-color)" }}
									>
										{registeredUserId}
									</Badge>
								</div>
				...
				>
					<DialogContent
						...
						data-testid="dialog-duplicate-warning"
					>
						<DialogHeader>
							<div className="flex justify-center mb-4">
						...
						<div className="space-y-4">
							...
							<Alert>
								...
								<AlertDescription className="text-sm">
									If this is your existing account, please login instead. If
									you're a family member, you can link your account to the
									existing one.
								</AlertDescription>
							</Alert>
								...
								<Button
									...
									onClick={() => {
										...
										toast({
											...
											description:
												"This feature will be available after account creation",
											variant: "default",
										});
										...
										{[0, 1, 2, 3].map((i) => (
											<input
												...
												onChange={(e) => {
													...
													if (i === 3 && digit)
														mainPinConfirmRefs.current[0]?.focus();
												}}
												onKeyDown={(e) => {
													if (
														e.key === "Backspace" &&
									...
									<div className="flex justify-center gap-3">
										{[0, 1, 2, 3].map((i) => (
											<input
												key={i}
												ref={(el) => {
													mainPinConfirmRefs.current[i] = el;
												}}
												type="password"
												inputMode="numeric"
												aria-label={`Confirm PIN digit ${i + 1} of 4`}
