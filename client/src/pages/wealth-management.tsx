	...
	const { data: returnData, isLoading } = useQuery({
		...
		queryFn: async () => {
			const assets = holdings.map((h) => ({
				assetId: h.id,
				assetType: (h.assetType.toLowerCase().includes("fund")
					? "mutual_fund"
					: h.assetType.toLowerCase().includes("bond")
						? "bond"
						: h.assetType.toLowerCase().includes("gold")
							? "gold"
							: "equity") as any,
				assetName: h.symbol,
				currentValue:
					(Number.parseFloat(h.quantity) || 0) *
					(Number.parseFloat(h.avgPrice) || 0), // Use avgPrice as current for mock
				investedAmount:
					(Number.parseFloat(h.quantity) || 0) *
	...
	if (!returnData || !returnData.success)
		return (
			<div className="p-8 text-center text-muted-foreground border rounded-lg bg-card">
				No return data available. Add holdings to see forecasting.
			</div>
		);
		...
		<div className="space-y-6">
				...
				<Card className="bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-950/20 dark:to-teal-950/20 border-emerald-200">
					...
					<CardContent>
						<div className="text-3xl font-bold text-emerald-700 dark:text-emerald-400">
							₹
							{((portfolio.currentValue * portfolio.cagr) / 100).toLocaleString(
								"en-IN",
								{ maximumFractionDigits: 0 },
							)}
						</div>
						<p className="text-xs text-emerald-600 mt-1">
							Projected absolute gain
						</p>
					</CardContent>
				</Card>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Detailed forecasting and risk analysis per asset
					</CardDescription>
				</CardHeader>
				<CardContent>
								...
								<div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-4 border-t">
									<div>
										...
										<p className="font-bold text-green-600">
											{(asset.metrics?.annualizedReturn || 0).toFixed(2)}%
										</p>
									</div>
									<div>
										<p className="text-[10px] text-muted-foreground uppercase">
										...
										<p className="font-bold">
											{(asset.metrics?.absoluteReturn || 0).toFixed(2)}%
										</p>
									</div>
									<div>
										<p className="text-[10px] text-muted-foreground uppercase">
										...
										<p className="font-bold">
											{(asset.metrics?.holdingPeriodYears || 0).toFixed(2)} Yrs
										</p>
									</div>
									<div>
										<p className="text-[10px] text-muted-foreground uppercase">
				...
				<div className="max-w-7xl mx-auto space-y-8">
					<div className="text-center space-y-2">
						<h1 className="text-4xl font-bold text-foreground">
							FintekPro Premium Investments
						</h1>
						<p className="text-xl text-muted-foreground">
							Elite investment opportunities for sophisticated investors
						</p>
					</div>
					<Alert className="border-yellow-500 bg-yellow-50 dark:bg-yellow-950/30">
						<AlertDescription className="text-center">
							<p className="font-medium mb-2">
								Complete Your Profile to Access Wealth Management
							</p>
							<p className="text-sm text-muted-foreground">
								Please complete your financial profile and KYC verification to
								unlock personalized investment recommendations and wealth
								management features.
							</p>
						</AlertDescription>
					</Alert>
		...
		<div className="min-h-screen bg-muted p-6">
			<div className="max-w-7xl mx-auto space-y-8">
				...
				<div className="text-center space-y-2">
					...
					<p className="text-xl text-muted-foreground">
						Access REITs, InvITs, PMS & AIF with your{" "}
						{formatCurrency(financialAnalysis.additionalCapacity)} monthly
						surplus - Elite investment opportunities for sophisticated investors
					</p>
				</div>
				...
				>
					...
					<TabsContent value="dashboard" className="space-y-6">
						...
						<Card
							data-testid="card-investment-capacity"
							className="bg-gradient-to-r from-green-50 dark:from-green-950/30 to-emerald-50 dark:to-emerald-950/30 border-green-200 dark:border-green-800"
						>
							<CardHeader>
								<CardTitle className="flex items-center gap-2 text-green-800 dark:text-green-200">
									<TrendingUp className="w-6 h-6 text-green-600" />🚀 Your
									Investment Opportunity
								</CardTitle>
								<CardDescription className="text-green-700 dark:text-green-300">
									Based on CIBIL analysis, you have significant untapped
								...
								<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
									...
									<div className="text-center p-4 bg-card rounded-lg border border-purple-200 dark:border-purple-800">
										<div className="text-3xl font-bold text-purple-600 mb-2">
											{financialAnalysis.creditScore}
										</div>
										<p className="text-sm font-medium text-purple-800 dark:text-purple-200">
											Credit Score
										</p>
										<p className="text-xs text-purple-600 mt-1">
											Excellent for investment loans
										</p>
									</div>
								</div>
						...
						<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
									...
									<div className="flex items-center justify-between">
										<div>
											...
											<p className="text-2xl font-bold">
												{formatCurrency(financialAnalysis.totalPortfolioValue)}
											</p>
											<p className="text-xs text-green-600">
												+{financialAnalysis.returnPercentage}% returns
											</p>
										</div>
									...
									<div className="flex items-center justify-between">
										<div>
											...
											<p className="text-2xl font-bold text-blue-600">
												{formatCurrency(financialAnalysis.monthlyIncome)}
											</p>
							...
							<Card
								data-testid="card-monthly-obligations"
									...
									<div className="flex items-center justify-between">
										<div>
											...
											<p className="text-2xl font-bold text-red-600">
												{formatCurrency(financialAnalysis.monthlyObligations)}
											</p>
							...
							<Card
								data-testid="card-investment-surplus"
									...
									<div className="flex items-center justify-between">
										<div>
											...
											<p className="text-2xl font-bold text-green-600">
												{formatCurrency(
													financialAnalysis.availableForInvestment,
												)}
											</p>
											<p className="text-xs text-green-600">
												Available monthly
														...
														<div>
															...
															<div className="flex items-center gap-2">
																...
																<Badge
																	variant="secondary"
																	className={`text-xs ${
																		holding.sector === "Mutual Fund" ||
																		(holding as any).dataSource === "AA" ||
																		(holding as any).dataSource === "CAS"
																			? "bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300"
																			: (holding as any).dataSource ===
																						"NSDL" ||
																					(holding as any).dataSource === "CDSL"
						...
						<Card data-testid="card-credit-obligations">
							<CardHeader>
								...
								<CardDescription>
									Real-time CIBIL data integration for comprehensive obligation
									tracking
								</CardDescription>
							</CardHeader>
							<CardContent>
						...
						<Card data-testid="card-wealth-actions">
							<CardHeader>
								...
								<CardDescription>
									AI-powered recommendations based on your CIBIL obligations
								</CardDescription>
							</CardHeader>
							<CardContent>
						...
						<Card data-testid="card-cibil-financial-health">
							<CardHeader>
								...
								<CardDescription>
									Real-time credit health analysis from your CIBIL report
								</CardDescription>
							</CardHeader>
							<CardContent>
								<div className="space-y-6">
										...
										<div className="space-y-4">
											...
											<div className="space-y-2">
												<div className="flex items-center gap-2 p-2 bg-blue-50 dark:bg-blue-950/30 rounded">
													...
													<span className="text-sm">
														₹2.4L total monthly obligations
													</span>
												</div>
												<div className="flex items-center gap-2 p-2 bg-blue-50 dark:bg-blue-950/30 rounded">
						...
						<Card data-testid="card-investment-opportunities">
							<CardHeader>
								<CardTitle className="flex items-center gap-2">
									<Target className="w-5 h-5 text-purple-600" />🏆 Accelerate
									Your Goals with Smart Investments
								</CardTitle>
								<CardDescription>
									With ₹72,000 monthly surplus, you can significantly boost your
									...
									<div className="p-4 bg-gradient-to-br from-blue-50 dark:from-blue-950/30 to-blue-100 dark:to-blue-900/30 rounded-lg border border-blue-200 dark:border-blue-800">
										...
										<div className="space-y-3">
											...
											<div className="bg-green-100 dark:bg-green-900/30 p-3 rounded border border-green-200 dark:border-green-800">
												<p className="text-sm font-medium text-green-800 dark:text-green-200">
													With additional ₹30K SIP:
												</p>
												<p className="text-lg font-bold text-green-900 dark:text-green-100">
													Achieve goal 2 years earlier!
												</p>
												<p className="text-xs text-green-600">
									...
									<div className="p-4 bg-gradient-to-br from-green-50 dark:from-green-950/30 to-green-100 dark:to-green-900/30 rounded-lg border border-green-200 dark:border-green-800">
										...
										<div className="space-y-3">
											...
											<div className="bg-blue-100 dark:bg-blue-900/30 p-3 rounded border border-blue-200 dark:border-blue-800">
												<p className="text-sm font-medium text-blue-800 dark:text-blue-200">
													With additional ₹20K SIP:
												</p>
												<p className="text-lg font-bold text-blue-900 dark:text-blue-100">
													Ensure full coverage!
												</p>
												<p className="text-xs text-blue-600">
													Zero education loan needed
												</p>
											</div>
										</div>
									</div>
								...
								<div className="mt-6 p-6 bg-gradient-to-r from-emerald-600 to-green-600 rounded-lg text-foreground">
									<div className="text-center space-y-4">
										<h3 className="text-xl font-bold">
											💰 Start Your Wealth Acceleration Today!
										</h3>
										<p className="text-emerald-100">
											Your excellent credit profile and surplus income make this
						...
						<Card
							data-testid="card-premium-investments"
						...
						>
							<CardHeader>
								<CardTitle className="flex items-center gap-2 text-amber-800 dark:text-amber-200">
									<Star className="w-6 h-6 text-amber-600" />🏆 Premium
									Investment Opportunities
								</CardTitle>
								<CardDescription className="text-amber-700 dark:text-amber-300">
									Sophisticated investment products tailored for your ₹72,000
							...
							<CardContent>
								<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
									{/* REITs & InvITs */}
									<Card
										data-testid="card-reits-invits"
										className="border-2 border-blue-200 dark:border-blue-800 hover:border-blue-300 dark:border-blue-700 transition-colors"
									>
										<CardContent className="p-4">
											...
											>
												Invest ₹
												{(
													financialAnalysis.additionalCapacity * 0.15
									...
									<Card
										data-testid="card-pms"
										className="border-2 border-purple-200 dark:border-purple-800 hover:border-purple-300 dark:border-purple-700 transition-colors"
									>
										<CardContent className="p-4">
											<div className="flex items-center gap-2 mb-3">
											...
											<div className="space-y-2 mb-4">
												<Badge
													variant="secondary"
													className="bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-200 text-xs"
												>
													Min: ₹50L
												</Badge>
												<Badge
											...
											<div className="text-xs text-green-600 mb-2">
												With ₹72K/month: Reach ₹50L in{" "}
												{Math.ceil(
													5000000 / financialAnalysis.additionalCapacity,
												)}{" "}
												months
											</div>
											<Button
												size="sm"
												variant="outline"
												className="w-full border-purple-300 dark:border-purple-700 text-purple-700 dark:text-purple-300 hover:bg-purple-50 dark:bg-purple-950/30"
											>
									...
									<Card
										data-testid="card-aif"
										className="border-2 border-indigo-200 dark:border-indigo-800 hover:border-indigo-300 dark:border-indigo-700 transition-colors"
									>
										<CardContent className="p-4">
											<div className="flex items-center gap-2 mb-3">
											...
											<div className="space-y-2 mb-4">
												<Badge
													variant="secondary"
													className="bg-indigo-100 dark:bg-indigo-900/30 text-indigo-800 dark:text-indigo-200 text-xs"
												>
													Min: ₹1Cr
												</Badge>
												<Badge
													variant="secondary"
													className="bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-200 text-xs"
												>
											...
											<div className="text-xs text-green-600 mb-2">
												With ₹72K/month: Reach ₹1Cr in{" "}
												{Math.ceil(
													10000000 / financialAnalysis.additionalCapacity,
												)}{" "}
												months
											</div>
											<Button
												size="sm"
												variant="outline"
												className="w-full border-indigo-300 dark:border-indigo-700 text-indigo-700 dark:text-indigo-300 hover:bg-indigo-50 dark:bg-indigo-950/30"
											>
									...
									<Card
										data-testid="card-bonds"
										className="border-2 border-green-200 dark:border-green-800 hover:border-green-300 dark:border-green-700 transition-colors"
									>
										<CardContent className="p-4">
											<div className="flex items-center gap-2 mb-3">
											...
											>
												Invest ₹
												{(
													financialAnalysis.additionalCapacity * 0.25
												).toLocaleString("en-IN")}
												/month
											</Button>
										</CardContent>
									...
									<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
										<div className="space-y-3">
											<h5 className="font-medium text-foreground">
												Immediate Allocation (Available Now)
											</h5>
											<div className="space-y-2">
												...
												<div className="flex justify-between items-center p-2 bg-muted rounded">
													...
													<span className="font-medium text-muted-foreground">
														₹
														{(
															financialAnalysis.additionalCapacity * 0.2
														).toLocaleString("en-IN")}
													</span>
												</div>
											</div>
										...
										<div className="space-y-3">
											<h5 className="font-medium text-foreground">
												Future Goals (Accumulation Strategy)
											</h5>
											<div className="space-y-2">
												...
												<div className="flex justify-between items-center p-2 bg-amber-50 dark:bg-amber-950/30 rounded">
													...
													<span className="font-medium text-amber-600">
														₹
														{(
															financialAnalysis.additionalCapacity * 12
														).toLocaleString("en-IN")}
													</span>
												</div>
											</div>
						...
						<Card
							data-testid="card-investment-urgency"
							className="bg-gradient-to-r from-red-50 dark:from-red-950/30 to-orange-50 dark:to-orange-950/30 border-red-200 dark:border-red-800"
						>
							<CardContent className="p-6">
								<div className="text-center space-y-4">
									...
									<div className="mt-6 p-4 bg-gradient-to-r from-emerald-600 to-green-600 text-foreground rounded-lg">
										<p className="text-lg font-bold mb-2">
											🎯 Your CIBIL Profile is Perfect for Wealth Building!
										</p>
										<p className="text-emerald-100 mb-4">
											With 785 credit score and only 35% obligation ratio,
											you're in the top 5% of investors. Start your ₹72,000
											monthly SIP today and secure your financial future.
										</p>
										<Button
											size="lg"
											variant="secondary"
