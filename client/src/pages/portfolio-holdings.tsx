...
export default function PortfolioHoldings() {
	...
	const totalDomesticValue =
		holdings?.reduce(
			(sum: number, h: any) => sum + Number.parseFloat(h.currentValue || "0"),
			0,
		) || 0;
	const totalGlobalValue =
		(globalHoldings as any)?.positions?.reduce(
			(sum: number, p: any) => sum + (p.marketValue || 0),
			0,
		) || 0;
		...
		<div className="container mx-auto px-4 py-6 max-w-7xl space-y-6">
			...
			<Tabs value={activeTab} onValueChange={setActiveTab}>
					...
					{holdings && holdings.length > 0 && (
						<Card>
							<CardHeader>
								...
								<CardDescription>
									{holdings.length} investments across asset classes
								</CardDescription>
							</CardHeader>
							<CardContent>
										...
										>
											...
											<div className="text-right">
												<p className="font-semibold">
													{formatCurrency(
														Number.parseFloat(holding.currentValue || "0"),
													)}
												</p>
												<p
													className={`text-sm flex items-center justify-end ${Number.parseFloat(holding.gainLossPercent || "0") >= 0 ? "text-green-600" : "text-red-600"}`}
												>
													{Number.parseFloat(holding.gainLossPercent || "0") >=
													0 ? (
				...
				<TabsContent value="global" className="mt-6">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								US stocks and international holdings
							</CardDescription>
						</CardHeader>
						<CardContent>
							{(globalHoldings as any)?.positions?.length > 0 ? (
								<div className="space-y-3">
									{(globalHoldings as any).positions.map(
										(position: any, idx: number) => (
											...
											>
												...
												<div className="text-right">
													...
													<p
														className={`text-sm ${position.unrealizedPL >= 0 ? "text-green-600" : "text-red-600"}`}
													>
								...
								<div className="text-center py-8 text-muted-foreground">
									...
									<p className="text-sm">
										Start investing in US stocks to diversify globally
									</p>
								</div>
							)}
						</CardContent>
				...
				<TabsContent value="insurance" className="mt-6">
					...
					>
						<div className="text-sm text-muted-foreground">
							{insuranceHoldings
								? `${insuranceHoldings.length} policies`
								: "Loading..."}
						</div>
					</ConsentAwareSchemeTab>
				</TabsContent>
				...
				<TabsContent value="epf" className="mt-6">
					<ConsentAwareSchemeTab schemeType="epf" onRequestConsent={() => {}}>
						<div className="text-sm text-muted-foreground">
							{epfHoldings ? `${epfHoldings.length} records` : "Loading..."}
						</div>
					</ConsentAwareSchemeTab>
				</TabsContent>

				<TabsContent value="ppf" className="mt-6">
					<ConsentAwareSchemeTab schemeType="ppf" onRequestConsent={() => {}}>
						<div className="text-sm text-muted-foreground">
							{ppfHoldings ? `${ppfHoldings.length} records` : "Loading..."}
						</div>
					</ConsentAwareSchemeTab>
				</TabsContent>

				<TabsContent value="nps" className="mt-6">
					<ConsentAwareSchemeTab schemeType="nps" onRequestConsent={() => {}}>
						<div className="text-sm text-muted-foreground">
							{npsAccounts ? `${npsAccounts.length} accounts` : "Loading..."}
						</div>
					</ConsentAwareSchemeTab>
				</TabsContent>
