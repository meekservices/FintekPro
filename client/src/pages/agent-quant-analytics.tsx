		...
		<div className="container mx-auto p-6 space-y-6">
			<div className="flex flex-col gap-2">
				...
				<p className="text-muted-foreground">
					Advanced portfolio analytics and forecasting powered by the Python
					Quant Engine.
				</p>
			</div>
	...
	return (
		<Card>
			<CardHeader>
				...
				<CardDescription>
					Calculate Internal Rate of Return for a specific client portfolio.
				</CardDescription>
			</CardHeader>
...
function RollingReturns() {
	...
	const { data, isLoading, refetch } = useQuery<any>({
		queryKey: [
			...
			{ scheme_code: schemeCode, periods: periods.join(",") },
		],
		enabled: false,
	});
	...
	return (
		<Card>
			<CardHeader>
				...
				<CardDescription>
					Analyze consistency of performance across different holding periods.
				</CardDescription>
			</CardHeader>
			<CardContent className="space-y-6">
				<div className="space-y-4">
					...
					<Button
						onClick={() => refetch()}
						disabled={isLoading || !schemeCode || periods.length === 0}
					>
						{isLoading ? (
							<Loader2 className="h-4 w-4 animate-spin mr-2" />
						) : null}
	...
	const { data, isLoading, refetch } = useQuery<any>({
		...
		queryFn: async () => {
			const res = await apiRequest(
				...
				{
					sipAmount: Number.parseFloat(params.sipAmount),
					horizonMonths: Number.parseInt(params.horizonMonths),
					expectedReturn: Number.parseFloat(params.expectedReturn),
					stepUpRate: Number.parseFloat(params.stepUpRate),
				},
			);
			return res.json();
		},
		...
		<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
			<Card className="lg:col-span-1">
				<CardHeader>
					...
					<CardDescription>
						Simulate wealth creation with step-up SIPs.
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-4">
					<div className="space-y-2">
						...
						<Input
							type="number"
							value={params.sipAmount}
							onChange={(e) =>
								setParams({ ...params, sipAmount: e.target.value })
							}
						/>
					</div>
					<div className="space-y-2">
						...
						<Input
							...
							onChange={(e) =>
								setParams({ ...params, horizonMonths: e.target.value })
							}
						/>
					</div>
					<div className="space-y-2">
						...
						<Input
							...
							onChange={(e) =>
								setParams({ ...params, expectedReturn: e.target.value })
							}
						/>
					</div>
					<div className="space-y-2">
						...
						<Input
							type="number"
							value={params.stepUpRate}
							onChange={(e) =>
								setParams({ ...params, stepUpRate: e.target.value })
							}
						/>
					</div>
					<Button
						...
						<div className="space-y-6">
							<div className="grid grid-cols-3 gap-4">
								...
								<div className="p-4 bg-blue-500/10 text-blue-600 rounded-lg">
									...
									<p className="text-xl font-bold">
										₹{data.inflationAdjustedValue?.toLocaleString()}
									</p>
								</div>
							</div>
		...
		queryFn: async () => {
			const res = await apiRequest(
				...
				"/api/python/forecasting/return-forecast",
				{
					...
					currentValue: Number.parseFloat(params.currentValue),
					annualReturn: Number.parseFloat(params.annualReturn),
					years: Number.parseInt(params.years),
				},
			);
			return res.json();
		},
		...
		<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
			<Card className="lg:col-span-1">
				<CardHeader>
					...
					<CardDescription>
						Probabilistic return forecasting with confidence bands.
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-4">
					<div className="space-y-2">
						...
						<Select
							...
							onValueChange={(v) => setParams({ ...params, assetType: v })}
						>
							<SelectTrigger>
								<SelectValue />
							</SelectTrigger>
					...
					<div className="space-y-2">
						...
						<Input
							...
							onChange={(e) =>
								setParams({ ...params, currentValue: e.target.value })
							}
						/>
					</div>
					<div className="space-y-2">
						...
						<Input
							...
							onChange={(e) =>
								setParams({ ...params, annualReturn: e.target.value })
							}
						/>
					</div>
					<div className="space-y-2">
		...
		<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Detect common holdings across different mutual funds.
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-4">
					{funds.map((fund, idx) => (
						<div key={idx} className="flex gap-4 items-end">
							...
							<div className="w-24 space-y-2">
								...
								<Input
									...
									onChange={(e) => {
										const next = [...funds];
										next[idx].weight = Number.parseFloat(e.target.value);
										setFunds(next);
									}}
								/>
							</div>
					...
					<Button variant="outline" className="w-full" onClick={addFund}>
						Add Fund
					</Button>
					<Button
						className="w-full"
						onClick={() => refetch()}
						disabled={isLoading || funds.every((f) => !f.isin)}
					>
						{isLoading ? (
							<Loader2 className="h-4 w-4 animate-spin mr-2" />
						) : null}
		...
		<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Find the optimal asset allocation on the Efficient Frontier.
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-4">
					<div className="space-y-4">
							...
							<div key={idx} className="grid grid-cols-4 gap-2">
								...
								<div>
									...
									<Input
										...
										onChange={(e) => {
											const next = [...assets];
											next[idx].expectedReturn = Number.parseFloat(
												e.target.value,
											);
											setAssets(next);
										}}
								...
								<div>
									...
									<Input
										...
										onChange={(e) => {
											const next = [...assets];
											next[idx].risk = Number.parseFloat(e.target.value);
											setAssets(next);
										}}
									/>
								</div>
							...
							<div className="space-y-2">
								...
								<div className="grid grid-cols-2 gap-4">
									{data.optimalWeights?.map((w: any, i: number) => (
										<div
											key={i}
											className="flex justify-between p-2 bg-muted/50 rounded text-xs"
										>
											<span>{w.name}</span>
