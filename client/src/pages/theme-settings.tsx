...
export function ThemeSettingsContent() {
	...
	useEffect(() => {
		if (prefs.autoSwitchEnabled) {
			const checkTime = () => {
				...
				if (
					currentTime >= prefs.lightModeStart &&
					currentTime < prefs.darkModeStart
				) {
	...
	const handleResetAccessibility = () => {
		...
		toast({
			title: "Reset",
			description: "Accessibility settings restored to defaults.",
		});
	};
							...
							<div className="grid grid-cols-2 gap-4 pt-4 border-t">
								<div className="space-y-2">
									...
									<Input
										...
										onChange={(e) =>
											updatePrefs({ lightModeStart: e.target.value })
										}
								...
								<div className="space-y-2">
									...
									<Input
										...
										onChange={(e) =>
											updatePrefs({ darkModeStart: e.target.value })
										}
					...
					<CardContent className="space-y-6">
							...
							<div>
								...
								<div className="flex items-center gap-4">
									...
									<Slider
										...
										onValueChange={(v) =>
											setAccessibility({ textBrightness: v[0] })
										}
										min={0}
										max={100}
										step={5}
								...
								<p className="text-xs text-muted-foreground mt-2">
									Adjust the brightness of text across the interface
								</p>
							</div>
						</div>
						...
						<div className="pt-4 border-t">
							...
							<RadioGroup
								...
								onValueChange={(v) =>
									setAccessibility({
										textSize: v as "small" | "medium" | "large",
									})
								}
						...
						<div className="pt-4 border-t space-y-3">
							...
							<div className="flex items-center gap-4">
								...
								<Slider
									...
									onValueChange={(v) =>
										setAccessibility({ transparency: v[0] })
									}
									min={0}
									max={100}
									step={5}
							...
							<p className="text-xs text-muted-foreground">
								Adjust background transparency (solid helps reduce visual noise)
							</p>
						</div>
						...
						<div className="pt-4 border-t space-y-3">
							...
							<p className="text-xs text-muted-foreground">
								Control animation and transition speed
							</p>
						</div>
					</CardContent>
				</Card>
