...
export default function AdminProposalsPage() {
  ...
  const createProposalMutation = useMutation({
    mutationFn: async (proposalData: any) => {
      const response = await apiRequest('POST', '/api/admin/proposals', { body: proposalData });
      return await response.json();
    },
    onSuccess: () => {
      toast({
        title: "Proposal created successfully",
        description: "The proposal has been created and can now have items added to it."
      });
  ...
  const loadToCartMutation = useMutation({
    mutationFn: async (proposalId: string) => {
      const response = await apiRequest('POST', `/api/proposals/${proposalId}/load-to-cart`);
      return await response.json();
    },
  ...
  const updateStatusMutation = useMutation({
    mutationFn: async ({ proposalId, status }: { proposalId: string; status: string }) => {
      const response = await apiRequest('PUT', `/api/admin/proposals/${proposalId}/status`, { body: { status } });
      return await response.json();
    },
    onSuccess: () => {
      toast({
  ...
  const deleteProposalMutation = useMutation({
    mutationFn: async (proposalId: string) => {
      const response = await apiRequest('DELETE', `/api/admin/proposals/${proposalId}`);
      return await response.json();
    },
    onSuccess: () => {
      toast({
  ...
  const handleCreateProposal = () => {
    ...
    createProposalMutation.mutate({
      ...newProposal,
      validUntil: newProposal.validUntil || new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]
    });
  };
  ...
  return (
    <div className="container mx-auto py-8 px-4">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  onChange={(e) => setNewProposal(prev => ({ ...prev, advisorNotes: e.target.value }))}
                  ...
                  {filteredProposals.map((proposal) => {
                    ...
                    return (
                      <TableRow key={proposal.id} data-testid={`row-proposal-${proposal.id}`}>
                        ...
                        <TableCell>
                          <span className="text-sm">{proposal.items.length} items</span>
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell className="text-right">
                          <div className="flex justify-end gap-2">
                            ...
                            <Select
                              ...
                              onValueChange={(status) => updateStatusMutation.mutate({ proposalId: proposal.id, status })}
                            >
                              <SelectTrigger className="h-8 w-20" data-testid={`select-status-${proposal.id}`}>
                                <SelectValue />
                              </SelectTrigger>
                              <SelectContent>
                                <SelectItem value="draft">Draft</SelectItem>
      ...
      <Dialog open={isViewDialogOpen} onOpenChange={setIsViewDialogOpen}>
        <DialogContent className="sm:max-w-2xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800">
          <DialogHeader>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{selectedProposal.clientEmail}</p>
                </div>
                <div>
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  ...
                  <p className="text-lg font-semibold">{formatCurrency(selectedProposal.totalAmount)}</p>
                </div>
                <div>
              ...
              <div>
                <Label className="text-sm font-medium mb-2 block">Proposal Items ({selectedProposal.items.length})</Label>
                <div className="rounded-md border">
                  <Table>
                    <TableHeader>
                      <TableRow>
              ...
              {selectedProposal.advisorNotes && (
                <div>
                  ...
                  <p className="text-sm mt-1 p-3 bg-muted rounded-md">{selectedProposal.advisorNotes}</p>
                </div>
              )}
            </div>
          )}
