...
export default function ReitInvitPage() {
	...
	const [invitFilters, setInvitFilters] = useState({
		sector: "all",
		riskLevel: "all",
		aiSignal: "all",
	});
		...
		useQuery<any>({
			...
			queryFn: async () => {
				const response = await fetch(
					"/api/reit-invit/ai-recommendations?riskProfile=moderate",
				);
	...
	const { data: unlistedReitsData, isLoading: unlistedReitsLoading } =
		useQuery<any>({
			queryKey: ["/api/reit-invit/unlisted-reits", unlistedReitSearch],
			queryFn: async () => {
				const qs = unlistedReitSearch
					? `?search=${encodeURIComponent(unlistedReitSearch)}`
					: "";
	...
	const { data: unlistedInvitsData, isLoading: unlistedInvitsLoading } =
		useQuery<any>({
			queryKey: [
				"/api/reit-invit/unlisted-invits",
				unlistedInvitSearch,
				unlistedInvitIndustry,
			],
			queryFn: async () => {
				const params = new URLSearchParams();
				if (unlistedInvitSearch) params.set("search", unlistedInvitSearch);
				if (unlistedInvitIndustry !== "all")
					params.set("industry", unlistedInvitIndustry);
		...
		>
			...
			>
				<TabsList className="grid w-full grid-cols-3 md:grid-cols-6 lg:w-auto lg:inline-flex">
					...
					<TabsTrigger
						value="unlisted-invits"
						data-testid="tab-unlisted-invits"
					>
						<Lock className="h-4 w-4 mr-2" />
						Unlisted InvITs
					</TabsTrigger>
									...
									<CardContent className="space-y-4">
										<div className="grid grid-cols-2 gap-4">
											...
											<div>
												...
												<p className="text-2xl font-bold">
													{formatMarketCap(overview.reits?.totalMarketCap || 0)}
												</p>
											</div>
										</div>
											...
											<div className="pt-4 border-t">
												...
												<div className="flex items-center justify-between">
													<span className="font-medium">
														{overview.reits.topPerformer.symbol}
													</span>
													<Badge
														variant="outline"
													...
													>
														{formatPercent(
															overview.reits.topPerformer.returns1Y,
														)}
													</Badge>
												</div>
											</div>
									...
									<CardContent className="space-y-4">
										<div className="grid grid-cols-2 gap-4">
											...
											<div>
												...
												<p className="text-2xl font-bold">
													{overview.invits?.avgConcessionLife} yrs
												</p>
											</div>
											<div>
												<p className="text-sm text-muted-foreground">
												...
												<p className="text-2xl font-bold">
													{formatMarketCap(
														overview.invits?.totalMarketCap || 0,
													)}
												</p>
											</div>
										</div>
										{overview.invits?.topPerformer && (
											<div className="pt-4 border-t">
												...
												<div className="flex items-center justify-between">
													<span className="font-medium">
														{overview.invits.topPerformer.symbol}
													</span>
													<Badge
														variant="outline"
														className="bg-green-500/10 text-green-600"
													>
														{formatPercent(
															overview.invits.topPerformer.returns1Y,
														)}
													</Badge>
												</div>
											</div>
							...
							<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
								...
								<div>
									...
									<Select
										...
										onValueChange={(v) =>
											setReitFilters((f) => ({ ...f, riskLevel: v }))
										}
									>
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
							{reits.map((reit: ReitData) => (
								<Card
									...
									data-testid={`reit-card-${reit.symbol}`}
								>
										...
										<div className="grid grid-cols-2 gap-4">
											<div>
												...
												<p className="text-lg font-semibold">
													{formatCurrency(reit.currentPrice)}
												</p>
											</div>
											<div>
												<p className="text-xs text-muted-foreground">
											...
											<div>
												...
												<p
													className={`text-sm font-medium ${Number.parseFloat(reit.premiumToNav) < 0 ? "text-green-600" : "text-red-600"}`}
												>
											...
											<div>
												...
												<p
													className={`text-sm font-medium ${Number.parseFloat(reit.returns1Y || "0") >= 0 ? "text-green-600" : "text-red-600"}`}
												>
											...
											{reit.aiTargetPrice && (
												<p className="text-sm mt-1">
													...
													<span className="font-medium text-primary">
														{formatCurrency(reit.aiTargetPrice)}
													</span>
												</p>
											)}
										</div>
							...
							<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
								...
								<div>
									...
									<Select
										...
										onValueChange={(v) =>
											setInvitFilters((f) => ({ ...f, riskLevel: v }))
										}
									>
								...
								<div>
									...
									<Select
										...
										onValueChange={(v) =>
											setInvitFilters((f) => ({ ...f, aiSignal: v }))
										}
									>
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
							{invits.map((invit: InvitData) => (
								<Card
									key={invit.id}
									className="hover:shadow-lg transition-shadow"
									data-testid={`invit-card-${invit.symbol}`}
								>
									<CardHeader className="pb-2">
										<div className="flex items-start justify-between">
											<div>
									...
									<CardContent className="space-y-4">
										...
										<div className="grid grid-cols-2 gap-4">
											<div>
												...
												<p className="text-lg font-semibold">
													{formatCurrency(invit.currentPrice)}
												</p>
											</div>
											<div>
												<p className="text-xs text-muted-foreground">
											...
											<div>
												...
												<p
													className={`text-sm font-medium ${Number.parseFloat(invit.premiumToNav) < 0 ? "text-green-600" : "text-red-600"}`}
												>
											...
											<div>
												...
												<p
													className={`text-sm font-medium ${Number.parseFloat(invit.returns1Y || "0") >= 0 ? "text-green-600" : "text-red-600"}`}
												>
										...
										<div className="flex items-center gap-2 flex-wrap">
											<Badge
												variant="outline"
												className={getRiskColor(invit.riskLevel)}
											>
												<LucideShield className="h-3 w-3 mr-1" />
												{invit.riskLevel}
											</Badge>
										...
										<div className="pt-2 border-t">
											...
											{invit.aiTargetPrice && (
												<p className="text-sm mt-1">
													<span className="text-muted-foreground">Target:</span>{" "}
													<span className="font-medium text-primary">
														{formatCurrency(invit.aiTargetPrice)}
													</span>
												</p>
											)}
										</div>
				...
				<TabsContent value="unlisted-reits" className="space-y-6">
					<Card className="border-amber-200 dark:border-amber-800 bg-amber-50/40 dark:bg-amber-950/20">
						<CardHeader className="pb-2">
							...
							<CardDescription>
								SEBI-registered REITs not yet listed on exchanges. Register your
								interest to be notified when they open for investment.
							</CardDescription>
						</CardHeader>
					</Card>
										...
										<Button
											...
											onClick={() =>
												toast({
													...
													description: `We'll notify you when ${r.name} opens for investment.`,
												})
											}
										>
											Register Interest
				...
				<TabsContent value="unlisted-invits" className="space-y-6">
					<Card className="border-purple-200 dark:border-purple-800 bg-purple-50/40 dark:bg-purple-950/20">
						<CardHeader className="pb-2">
							...
							<CardDescription>
								SEBI-registered infrastructure investment trusts not yet listed
								on exchanges. Covering roads, energy, telecom, logistics and
								more.
							</CardDescription>
						</CardHeader>
					</Card>
					...
					<div className="flex flex-wrap items-center gap-3">
						...
						<Select
							value={unlistedInvitIndustry}
							onValueChange={setUnlistedInvitIndustry}
						>
				...
				<TabsContent value="ai" className="space-y-6">
					<Card className="border-primary/20 bg-primary/5">
						<CardHeader>
							...
							<CardDescription>
								Personalized picks based on your risk profile and investment
								goals
							</CardDescription>
						</CardHeader>
						<CardContent>
								...
								<div className="grid grid-cols-3 gap-4 mb-6">
									<div className="text-center p-4 bg-background rounded-lg">
										<p className="text-3xl font-bold text-primary">
											{aiRecommendations.summary.totalRecommendations}
										</p>
										<p className="text-sm text-muted-foreground">
											Recommendations
										</p>
									...
									<div className="text-center p-4 bg-background rounded-lg">
										<p className="text-3xl font-bold text-green-600">
											{aiRecommendations.summary.avgYield}%
										</p>
										<p className="text-sm text-muted-foreground">Avg. Yield</p>
									</div>
									<div className="text-center p-4 bg-background rounded-lg">
										<p className="text-3xl font-bold">
											{aiRecommendations.summary.avgConfidence}%
										</p>
										<p className="text-sm text-muted-foreground">
											Avg. Confidence
										</p>
						...
						<div className="space-y-4">
							{recommendations.map((rec: Recommendation, index: number) => (
								<Card
									key={rec.symbol}
									className="hover:shadow-lg transition-shadow"
									data-testid={`ai-pick-${rec.symbol}`}
								>
											...
											<div className="flex-1 space-y-3">
												...
												<div className="flex items-center gap-4">
													...
													<div className="flex items-center gap-2">
														...
														<Progress
															value={Number.parseFloat(rec.aiConfidence)}
															className="w-24 h-2"
														/>
												...
												<div className="flex items-center justify-between pt-2">
													<div className="flex items-center gap-2">
														...
														<span className="text-sm">
															Target: {formatCurrency(rec.aiTargetPrice)}
														</span>
														<span className="text-xs text-green-600">
															(
															{formatPercent(
																((Number.parseFloat(rec.aiTargetPrice) -
