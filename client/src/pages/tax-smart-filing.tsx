...
export default function TaxSmartFiling() {
	...
	const steps = [
		{
			id: 1,
			title: "Consent & Setup",
			description: "PAN verification and consent",
		},
		{
			id: 2,
		...
		},
		{
			...
			description: "Review data and fix issues",
		},
		{
			id: 4,
			title: "AI Optimization",
			description: "Smart tax-saving suggestions",
		},
	...
	const fileMutation = useMutation({
		mutationFn: async ({
			sessionId,
			itrJson,
		}: { sessionId: string; itrJson: string }) => {
	...
	const renderConsentStep = () => (
		<Card data-testid="card-consent-step">
			<CardHeader>
				...
				<CardDescription>
					Provide your PAN and consent to start the intelligent tax filing
					process
				</CardDescription>
			</CardHeader>
			<CardContent>
				<Form {...sessionForm}>
					...
					>
								...
								<FormItem>
									...
									<Select onValueChange={field.onChange} value={field.value}>
										...
										<SelectContent>
											...
											<SelectItem value="2024-25">
												2024-25 (FY 2023-24)
											</SelectItem>
											<SelectItem value="2023-24">
												2023-24 (FY 2022-23)
											</SelectItem>
										</SelectContent>
									</Select>
						...
						<Alert>
							...
							<AlertDescription>
								Your data is encrypted and secure. We only access tax-related
							...
							render={({ field }) => (
								<FormItem className="flex flex-row items-start space-x-3 space-y-0">
									...
									<div className="space-y-1 leading-none">
										<FormLabel>
											I consent to data aggregation and AI-powered tax
											optimization
										</FormLabel>
										<FormMessage />
									</div>
			...
			<Card data-testid="card-aggregate-step">
				...
				<CardContent className="space-y-4">
					{!initializeDataSourcesMutation.isSuccess && (
						<Button
							onClick={() =>
								sessionId && initializeDataSourcesMutation.mutate(sessionId)
							}
							disabled={initializeDataSourcesMutation.isPending}
							className="w-full"
							data-testid="button-initialize-sources"
						>
							{initializeDataSourcesMutation.isPending
								? "Initializing..."
						...
						<>
							...
							<Collapsible open={showAdvanced} onOpenChange={setShowAdvanced}>
								...
								<CollapsibleContent className="space-y-2">
									<Alert>
										<AlertDescription>
											Advanced users can manually select specific data sources
											or configure sync parameters.
										</AlertDescription>
									</Alert>
								</CollapsibleContent>
							</Collapsible>
		...
		return (
			<Card data-testid="card-review-step">
				<CardHeader>
					...
					<CardDescription>
						AI-powered validation found {validationData.summary.totalIssues}{" "}
						items to review
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-4">
									...
									{validationData.issues
										?.filter(
											(issue: ValidationIssue) =>
												issue.severity === severity.slice(0, -1),
										)
											...
											<div className="flex items-center gap-4">
												<div className="flex items-center gap-2">
													...
													<Switch
														...
														checked={
															selectedSuggestions[suggestion.id] === "accepted"
														}
					...
					{filingRecord && (
						<>
							<Alert>
								<CheckCircle className="h-4 w-4" />
								<AlertDescription>
									ITR filed successfully! Your acknowledgment number is{" "}
									{filingRecord.acknowledgmentNumber}
								</AlertDescription>
							</Alert>
							...
							<div className="grid gap-3">
								...
								<div className="flex justify-between">
									...
									<span>
										{new Date(filingRecord.filingDate).toLocaleDateString()}
									</span>
								</div>
								<div className="flex justify-between">
									<span>ITR Form:</span>
							...
							<Button
								className="w-full"
								data-testid="button-download-acknowledgment"
							>
								<Download className="h-4 w-4 mr-2" />
