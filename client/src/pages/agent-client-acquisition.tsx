...
export default function AgentClientAcquisitionPage() {
	...
	const [selectedProspect, setSelectedProspect] =
		useState<ProspectClient | null>(null);
	...
	const stats = prospectsData?.stats || {
		total: 0,
		prospects: 0,
		onboarded: 0,
		activeClients: 0,
	...
	const hasPortfolioData = (prospect: ProspectClient) => {
		return (
			prospect.unifiedPortfolio ||
			prospect.fetchedPortfolio ||
			prospect.uploadedPortfolio
		);
	};
	...
	const getPortfolioSummary = (prospect: ProspectClient) => {
		...
		return legacyPortfolio
			? {
					...
					holdingsCount:
						legacyPortfolio.holdings?.length ||
						legacyPortfolio.parsedHoldings?.length ||
						0,
	...
	const updateStateMutation = useMutation({
		...
		onSuccess: () => {
			toast({
				title: "State Updated",
				description: "Prospect state has been updated",
			});
			queryClient.invalidateQueries({
				queryKey: ["/api/agent/prospect-clients"],
			});
	...
	const analyzePortfolioMutation = useMutation({
		mutationFn: async (id: string) => {
			return await apiRequest(
				`/api/agent/prospect-clients/${id}/analyze-portfolio`,
				{
					method: "POST",
					headers: { "Content-Type": "application/json" },
				},
	...
	const fetchPortfolioMutation = useMutation({
		...
		onSuccess: () => {
			toast({
				title: "Portfolio Fetched",
				description: "Portfolio data retrieved from demat accounts",
			});
			queryClient.invalidateQueries({
				queryKey: ["/api/agent/prospect-clients"],
			});
	...
	return (
		<div className="container mx-auto py-6 space-y-6">
			<div className="flex justify-between items-center">
				<div>
					...
					<p className="text-muted-foreground">
						Manage prospect pipeline and convert to active clients
					</p>
				</div>
					...
					<DropdownMenu>
						<DropdownMenuTrigger asChild>
							<Button
								...
								data-testid="btn-smart-proposal"
							>
						...
						<DropdownMenuContent align="end" className="w-72">
							...
							>
								...
								<div>
									...
									<p className="text-xs text-muted-foreground">
										Import holdings & get AI rebalancing advice
									</p>
								</div>
							</DropdownMenuItem>
							<DropdownMenuItem
							...
							>
								...
								<div>
									...
									<p className="text-xs text-muted-foreground">
										AI recommendations for new investments
									</p>
								</div>
							</DropdownMenuItem>
							<DropdownMenuItem
								...
								onSelect={() =>
									navigate("/agent-prospect-wizard?mode=combined")
								}
							>
								...
								<div>
									...
									<p className="text-xs text-muted-foreground">
										Analyze existing + suggest fresh investments
									</p>
								</div>
							</DropdownMenuItem>
						</DropdownMenuContent>
			...
			<Alert className="bg-amber-50 dark:bg-amber-950 border-amber-200 dark:border-amber-800">
				...
				<AlertDescription>
					The analysis and proposals shared are advisory in nature. Final
					investment decisions are made by the client with your assistance as a
					licensed agent.
				</AlertDescription>
			</Alert>
				...
				<CardContent className="py-4">
					<div className="flex items-center gap-2 mb-3">
						...
						<h3 className="font-semibold text-sm">
							How to Create AI-Powered Proposals
						</h3>
					</div>
					<div className="grid grid-cols-1 md:grid-cols-4 gap-4">
						<div className="flex items-start gap-3">
							<div className="h-8 w-8 rounded-full bg-indigo-600 text-white flex items-center justify-center text-sm font-bold flex-shrink-0">
								1
							</div>
							<div>
								<p className="font-medium text-sm">Add Prospect</p>
								<p className="text-xs text-muted-foreground">
									Click "Add Prospect" with name & PAN
								</p>
							</div>
						</div>
						<div className="flex items-start gap-3">
							...
							<div>
								...
								<p className="text-xs text-muted-foreground">
									Fetch via PAN or manually add holdings
								</p>
							</div>
						</div>
						<div className="flex items-start gap-3">
							...
							<div>
								...
								<p className="text-xs text-muted-foreground">
									Get asset allocation, risk & gap analysis
								</p>
							</div>
						</div>
						<div className="flex items-start gap-3">
							<div className="h-8 w-8 rounded-full bg-green-600 text-white flex items-center justify-center text-sm font-bold flex-shrink-0">
								4
							</div>
							<div>
								<p className="font-medium text-sm">Generate Proposal</p>
								<p className="text-xs text-muted-foreground">
									Share PDF with rebalancing + fresh ideas
								</p>
							</div>
						</div>
					</div>
							...
							<div className="flex flex-col md:flex-row gap-4 justify-between">
								...
								<Select value={filterState} onValueChange={setFilterState}>
									...
									<SelectContent>
										...
										<SelectItem value="active_client">
											Active Clients
										</SelectItem>
									</SelectContent>
								</Select>
												...
												<TableCell>
													<Badge variant="outline">
														{CLIENT_TYPE_OPTIONS.find(
															(c) => c.value === prospect.clientType,
														)?.label || prospect.clientType}
													</Badge>
												</TableCell>
												<TableCell>
													<Badge
														className={
															STATE_BADGES[prospect.state]?.color || ""
														}
													>
														{STATE_BADGES[prospect.state]?.label ||
															prospect.state}
													</Badge>
												</TableCell>
												<TableCell>
													{hasPortfolioData(prospect) ? (
														<Badge
															variant="outline"
															className={
																prospect.unifiedPortfolio?.isVerified
																	? "text-blue-600"
																	: "text-green-600"
															}
														>
													...
													<div className="flex items-center gap-2">
														...
														{prospect.pan && !hasPortfolioData(prospect) && (
															<Button
																...
																size="sm"
																onClick={() =>
																	fetchPortfolioMutation.mutate(prospect.id)
																}
																disabled={fetchPortfolioMutation.isPending}
																data-testid={`btn-fetch-${prospect.id}`}
															>
																<RefreshCw
																	className={`h-4 w-4 ${fetchPortfolioMutation.isPending ? "animate-spin" : ""}`}
																/>
															</Button>
														)}
														{hasPortfolioData(prospect) && (
															...
															>
																<Sparkles
																	className={`h-4 w-4 ${analyzePortfolioMutation.isPending ? "animate-spin" : ""}`}
																/>
															</Button>
														)}
														{getNextState(prospect.state) && (
															<Button
																...
																onClick={() =>
																	updateStateMutation.mutate({
																		id: prospect.id,
																		newState: getNextState(prospect.state)!,
																	})
																}
															...
															>
																...
																{getNextState(prospect.state) === "onboarded"
																	? "Onboard"
																	: "Activate"}
															</Button>
														)}
														<Link
															href={`/agent/proposals?prospect=${prospect.id}`}
														>
															<Button
																variant="outline"
																size="sm"
																data-testid={`btn-proposal-${prospect.id}`}
															>
				...
				<TabsContent value="analysis" className="space-y-4">
					<Card className="bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-950 dark:to-purple-950 border-indigo-200 dark:border-indigo-800">
						<CardHeader>
							...
							<CardDescription>
								Choose your workflow to create personalized investment proposals
								for prospects
							</CardDescription>
						</CardHeader>
						<CardContent>
							<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
								...
								>
									...
									<CardContent>
										<p className="text-sm text-muted-foreground mb-4">
											Import client's current holdings and get AI-powered
										...
										<ul className="text-xs text-muted-foreground space-y-1">
											...
											<li className="flex items-center gap-1">
												<CheckCircle2 className="h-3 w-3 text-green-500" /> Risk
												assessment & gaps
											</li>
										</ul>
									</CardContent>
								...
								>
									<CardHeader className="pb-2">
										...
										<CardTitle className="text-base">
											Fresh Investment Proposal
										</CardTitle>
									</CardHeader>
									<CardContent>
										<p className="text-sm text-muted-foreground mb-4">
											Create AI-powered investment recommendations for new
											capital deployment
										</p>
										<ul className="text-xs text-muted-foreground space-y-1">
											<li className="flex items-center gap-1">
									...
									<CardFooter>
										<Button
											variant="ghost"
											className="w-full group-hover:bg-green-50 dark:group-hover:bg-green-900"
										>
								...
								>
									...
									<CardContent>
										<p className="text-sm text-muted-foreground mb-4">
											Full analysis: rebalance existing + recommend new
					...
					<Card>
						...
						<CardContent>
							...
							{filteredProspects.filter((p) => hasPortfolioData(p)).length ===
								0 && (
								<div className="text-center py-8 text-muted-foreground">
									<FileUp className="h-12 w-12 mx-auto mb-4 opacity-50" />
									<p>No prospects with portfolio data yet</p>
									<p className="text-sm">
										Fetch or upload portfolio for a prospect to enable analysis
									</p>
								</div>
							)}
						</CardContent>
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
						<Card>
							<CardHeader>
								...
								<CardDescription>
									Track prospects through your pipeline
								</CardDescription>
							</CardHeader>
							<CardContent className="space-y-4">
								...
								<div className="space-y-2">
									...
									<Progress
										value={
											metrics.total > 0
												? (metrics.onboarded / metrics.total) * 100
												: 0
										}
								...
								<div className="space-y-2">
									...
									<Progress
										value={
											metrics.total > 0
												? (metrics.activeClients / metrics.total) * 100
												: 0
										}
										className="h-3 bg-muted"
									/>
						...
						<Card>
							...
							<CardContent className="space-y-4">
								<div className="grid grid-cols-2 gap-4">
									...
									<div className="text-center p-4 bg-muted rounded-lg">
										<p className="text-2xl font-bold">
											{metrics.proposalStats.converted}
										</p>
										<p className="text-sm text-muted-foreground">Converted</p>
									</div>
									<div className="text-center p-4 bg-muted rounded-lg">
										<p className="text-2xl font-bold">
											{metrics.proposalStats.viewed}
										</p>
										<p className="text-sm text-muted-foreground">Viewed</p>
									</div>
									<div className="text-center p-4 bg-muted rounded-lg">
										<p className="text-2xl font-bold">
											{metrics.proposalStats.acceptanceRate.toFixed(1)}%
										</p>
										<p className="text-sm text-muted-foreground">
											Acceptance Rate
										</p>
					...
					{addDialogSuccess ? (
						<>
							<DialogHeader>
								...
								<DialogDescription>
									{newProspect.name} has been added to your pipeline. What would
									you like to do next?
								</DialogDescription>
							</DialogHeader>
							<div className="py-4">
							...
							<DialogFooter className="flex-col sm:flex-row gap-2">
								...
								<Button
									...
									data-testid="btn-continue-wizard"
								>
									Continue to Wizard
						...
						<>
							...
							<div className="space-y-4">
								...
								<div>
									...
									<Select
										...
										onValueChange={(value) =>
											setNewProspect((prev) => ({ ...prev, clientType: value }))
										}
									>
							...
							<DialogFooter>
								...
								<Button
									...
									disabled={
										!newProspect.name || createProspectMutation.isPending
									}
						...
						<div className="space-y-6">
							<div className="grid grid-cols-2 gap-4">
								<div>
									...
									<div className="mt-1">
										<Badge
											className={STATE_BADGES[selectedProspect.state]?.color}
										>
								...
								<div>
									...
									<p className="mt-1 font-medium">
										{
											CLIENT_TYPE_OPTIONS.find(
												(c) => c.value === selectedProspect.clientType,
											)?.label
										}
									</p>
								</div>
								...
								<div>
									...
									<p className="mt-1 capitalize">
										{selectedProspect.indicativeRiskProfile || "-"}
									</p>
								</div>
							</div>
									...
									<div className="p-4 bg-muted rounded-lg">
										<div className="flex justify-between items-center mb-2">
											...
											<span className="font-bold">
												₹
												{(
													getPortfolioSummary(selectedProspect)?.totalValue || 0
										...
										<div className="flex justify-between items-center mb-2">
											<span>Holdings</span>
											<span>
												{getPortfolioSummary(selectedProspect)?.holdingsCount ||
													0}{" "}
												items
											</span>
										</div>
										{selectedProspect.unifiedPortfolio && (
											<div className="flex justify-between items-center">
												...
												<Badge
													variant="outline"
													className={
														selectedProspect.unifiedPortfolio.isVerified
															? "text-blue-600"
															: "text-amber-600"
													}
												>
									...
									<div className="p-4 border border-dashed rounded-lg text-center text-muted-foreground">
										...
										{selectedProspect.pan && (
											<Button
												...
												className="mt-2"
												onClick={() =>
													fetchPortfolioMutation.mutate(selectedProspect.id)
												}
												disabled={fetchPortfolioMutation.isPending}
											>
												<RefreshCw
													className={`h-4 w-4 mr-2 ${fetchPortfolioMutation.isPending ? "animate-spin" : ""}`}
												/>
												Fetch Portfolio
											</Button>
										)}
							...
							<div className="flex gap-2 justify-end">
								{getNextState(selectedProspect.state) && (
									<Button
										onClick={() => {
											updateStateMutation.mutate({
												...
												newState: getNextState(selectedProspect.state)!,
											});
									...
									>
										<ChevronRight className="h-4 w-4 mr-1" />
										Advance to{" "}
										{getNextState(selectedProspect.state) === "onboarded"
											? "Onboarded"
											: "Active Client"}
									</Button>
								)}
			...
			<Dialog open={showAnalysisDialog} onOpenChange={setShowAnalysisDialog}>
				<DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
					<DialogHeader>
						<DialogTitle>
							Portfolio Analysis - {selectedProspect?.name}
						</DialogTitle>
						<DialogDescription>
					...
					{selectedProspect?.portfolioAnalysis ? (
						<div className="space-y-6">
							<Alert className="bg-amber-50 dark:bg-amber-950 border-amber-200">
								<Info className="h-4 w-4" />
								<AlertDescription>
									This analysis is for informational purposes only. No buy/sell
							...
							<div className="grid grid-cols-2 gap-4">
								<Card>
									...
									<CardContent>
										<div className="flex items-center gap-4">
											<Progress
												value={selectedProspect.portfolioAnalysis.overallScore}
												className="flex-1"
											/>
											<span className="text-2xl font-bold">
												{selectedProspect.portfolioAnalysis.overallScore}/100
											</span>
										</div>
									</CardContent>
								</Card>
								<Card>
									...
									<CardContent>
										<div className="flex items-center gap-4">
											<Progress
												value={selectedProspect.portfolioAnalysis.riskScore}
												className="flex-1"
											/>
											<span className="text-2xl font-bold">
												{selectedProspect.portfolioAnalysis.riskScore}/100
											</span>
										</div>
									</CardContent>
								</Card>
							...
							{selectedProspect.portfolioAnalysis.concentrationRisk?.alerts
								?.length > 0 && (
								<Card className="border-amber-200 dark:border-amber-800">
									...
									<CardContent>
										<ul className="space-y-2">
											{selectedProspect.portfolioAnalysis.concentrationRisk.alerts.map(
							...
							{selectedProspect.portfolioAnalysis.missingAssetClasses?.length >
								0 && (
								<Card>
									<CardHeader className="pb-2">
										<CardTitle className="text-base">
									...
									<CardContent>
										<div className="flex flex-wrap gap-2">
											{selectedProspect.portfolioAnalysis.missingAssetClasses.map(
												(asset: string) => (
													<Badge
														key={asset}
														variant="outline"
							...
							{selectedProspect.portfolioAnalysis.gapAnalysis?.length > 0 && (
								<Card>
									...
									<CardContent>
										<div className="space-y-4">
											{selectedProspect.portfolioAnalysis.gapAnalysis.map(
												(gap: any, i: number) => (
