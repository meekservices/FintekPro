    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold flex items-center gap-2" data-testid="text-client-name">
        ...
        <div className="flex items-center gap-2">
          ...
          <Dialog open={interactionDialogOpen} onOpenChange={setInteractionDialogOpen}>
            <DialogTrigger asChild>
              <Button className="bg-emerald-600 hover:bg-emerald-700" data-testid="button-log-interaction">
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold">₹{((overview?.stats?.totalOpportunityValue || 0) / 100000).toFixed(1)}L</p>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold">{overview?.stats?.activeOpportunities || 0}</p>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold">{overview?.stats?.totalInteractions || 0}</p>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold">{overview?.stats?.pendingTasksCount || 0}</p>
        ...
        <TabsContent value="timeline">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Recent activities with this client</CardDescription>
            </CardHeader>
            <CardContent>
                    ...
                    {overview?.activityTimeline?.map((activity, i) => (
                      <div key={activity.id || i} className="flex gap-4">
                        <div className="relative">
                          <div className={`p-2 rounded-full ${getInteractionColor(activity.activityType)}`}>
                            {(() => {
                              const Icon = getInteractionIcon(activity.activityType);
                              return <Icon className="h-4 w-4" />;
                            })()}
                          </div>
                          {i < (overview?.activityTimeline?.length || 0) - 1 && (
                      ...
                      return (
                        <div key={interaction.id} className="flex items-start gap-3 p-3 rounded-lg border">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between">
                              <p className="font-medium">{interaction.subject || `${interaction.type} interaction`}</p>
                              <Badge variant="outline" className="capitalize">{interaction.type}</Badge>
                            </div>
                            {interaction.description && (
                              <p className="text-sm text-muted-foreground mt-1 line-clamp-2">{interaction.description}</p>
                            )}
                            <div className="flex items-center gap-2 mt-2 text-xs text-muted-foreground">
                              <Clock className="h-3 w-3" />
                              {interaction.createdAt && format(new Date(interaction.createdAt), "MMM d, yyyy h:mm a")}
                              {interaction.outcome && (
                                <>
                                  <span>•</span>
                                  <span className="capitalize">{interaction.outcome.replace(/_/g, ' ')}</span>
                                </>
                              )}
                            </div>
                          </div>
                    ...
                    <div key={opp.id} className="flex items-center justify-between p-3 rounded-lg border">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground capitalize">{opp.stage} • {opp.productType || 'General'}</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="font-bold">₹{Number(opp.expectedAmount || 0).toLocaleString('en-IN')}</p>
                        <Badge variant="outline" className="capitalize">{opp.priority}</Badge>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="space-y-3">
                  {overview?.pendingTasks?.map((task) => (
                    <div key={task.id} className="flex items-center justify-between p-3 rounded-lg border">
                      ...
                      <div className="text-right">
                        {task.dueDate && (
                          <p className="text-sm">{format(new Date(task.dueDate), "MMM d")}</p>
                        )}
