...
interface BrandProfile {
	...
	bio: string | null;
	arnCode: string | null;
	arnExpiryDate: string | null;
	euinNumber: string | null;
	sebiRegNumber: string | null;
	irdaiRegNumber: string | null;
	nismCertNumber: string | null;
	nismCertExpiry: string | null;
	cfpNumber: string | null;
	cfpExpiry: string | null;
...
export default function AdvisorBrandProfile() {
	...
	const saveMutation = useMutation({
		...
		onSuccess: () => {
			...
			toast({
				title: "Profile saved",
				description: "Your advisor profile has been updated.",
			});
		},
	...
	function copyReferralCode() {
		if (form.referralCode) {
			navigator.clipboard.writeText(
				`${window.location.origin}/advisor/${form.referralCode}`,
			);
			setCodeCopied(true);
	...
	const publicUrl = form.referralCode
		? `${window.location.origin}/advisor/${form.referralCode}`
		: null;
			...
			<div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
				<div>
					...
					<p className="text-sm text-muted-foreground mt-1">
						Build your professional identity — photo, credentials,
						specialisations, and a shareable public microsite.
					</p>
				</div>
				...
				<TabsContent value="branding" className="space-y-4 mt-4">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Shown on your public profile and marketing cards.
							</CardDescription>
						</CardHeader>
							...
							<div className="flex-shrink-0">
								{form.photoUrl ? (
									...
									<div className="w-24 h-24 rounded-full bg-primary/10 flex items-center justify-center text-3xl font-bold text-primary">
										{(form.fullName || form.marketingName || "A")
											.charAt(0)
											.toUpperCase()}
									</div>
								)}
							...
							<div className="flex-1 space-y-3">
								<div>
									...
									<p className="text-xs text-muted-foreground mt-1">
										Paste a direct image URL (HTTPS). Upload to an image host if
										needed.
									</p>
								</div>
							</div>
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Your practice name, logo, and tagline appear on your public
								microsite.
							</CardDescription>
						</CardHeader>
						<CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-4">
							<div>
								...
								<Input
									placeholder="e.g. Mohanty Wealth Advisory"
									className="mt-1"
									{...field("firmName")}
								/>
							</div>
							...
							<div className="sm:col-span-2">
								<Label>Tagline</Label>
								<Input
									placeholder="e.g. Empowering families with smarter financial decisions"
									className="mt-1"
									{...field("tagline")}
								/>
							</div>
				...
				<TabsContent value="credentials" className="space-y-4 mt-4">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								These are shown as verified badges on your public profile.
							</CardDescription>
						</CardHeader>
						<CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-4">
							<div>
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>
								NISM, CFP and other professional certifications.
							</CardDescription>
						</CardHeader>
						<CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-4">
							<div>
					...
					<Card>
						<CardHeader>
							<CardTitle>Credential Badges Preview</CardTitle>
						</CardHeader>
						<CardContent className="flex flex-wrap gap-2">
							...
							{form.sebiRegNumber && (
								...
								>
									<LucideShield className="h-3 w-3" /> SEBI:{" "}
									{form.sebiRegNumber}
								</Badge>
							)}
							{form.irdaiRegNumber && (
								...
								>
									<LucideShield className="h-3 w-3" /> IRDAI:{" "}
									{form.irdaiRegNumber}
								</Badge>
							)}
							{form.nismCertNumber && (
							...
							{form.cfpNumber && (
								<Badge
									variant="outline"
									className="flex items-center gap-1 text-indigo-600 border-indigo-600"
								>
							...
							{form.euinNumber && (
								<Badge
									variant="outline"
									className="flex items-center gap-1 text-gray-600 border-gray-500"
								>
							...
							{!form.arnCode && !form.sebiRegNumber && !form.irdaiRegNumber && (
								<p className="text-sm text-muted-foreground">
									Add credentials above to see badges here.
								</p>
							)}
						</CardContent>
					</Card>
				...
				<TabsContent value="business" className="space-y-4 mt-4">
					<div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
						{[
							...
							{
								label: "AUM Managed (₹Cr)",
								value: form.aumManaged
									? `${Number(form.aumManaged).toFixed(1)} Cr`
									: "—",
								icon: IndianRupee,
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>
								These numbers are displayed on your public profile to build
								credibility.
							</CardDescription>
						</CardHeader>
						<CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-4">
							<div>
								...
								<Input
									...
									onChange={(e) =>
										setForm((f) => ({
											...f,
											aumManaged: Number(e.target.value),
										}))
									}
								/>
							</div>
							...
							<div>
								...
								<Input
									...
									onChange={(e) =>
										setForm((f) => ({
											...f,
											activeClients: Number(e.target.value),
										}))
									}
								/>
							</div>
				...
				<TabsContent value="specializations" className="space-y-4 mt-4">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Select all that apply — shown as tags on your public profile and
								proposals.
							</CardDescription>
						</CardHeader>
						<CardContent>
							...
							{form.specializations.length > 0 && (
								<div className="mt-4">
									<p className="text-sm font-medium mb-2">
										Selected ({form.specializations.length}):
									</p>
									<div className="flex flex-wrap gap-2">
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Help clients find you based on their preferred language.
							</CardDescription>
						</CardHeader>
						<CardContent>
							<div className="flex flex-wrap gap-2">
				...
				<TabsContent value="social" className="space-y-4 mt-4">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Shown as clickable icons on your public microsite.
							</CardDescription>
						</CardHeader>
						<CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-4">
							<div>
					...
					{(form.linkedinUrl ||
						...
						<Card>
							<CardHeader>
								<CardTitle>Links Preview</CardTitle>
							</CardHeader>
							<CardContent className="flex flex-wrap gap-3">
								...
								{form.whatsappBusiness && (
									<a
										href={`https://wa.me/${form.whatsappBusiness.replace(/\D/g, "")}`}
										target="_blank"
										rel="noopener noreferrer"
										className="flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-sm hover:bg-accent"
									>
					...
					<Card>
						<CardHeader>
							<CardTitle className="flex items-center gap-2">
								...
								Your Referral Code & Public Microsite
							</CardTitle>
							<CardDescription>
								Share this link with clients — they land on your personalised
								profile page. Toggle <strong>Public</strong> at the top to make
								it live.
							</CardDescription>
						</CardHeader>
								...
								<>
									...
									{!form.profilePublic && (
										<div className="flex items-center gap-2 p-3 bg-yellow-50 dark:bg-yellow-900/20 rounded-md text-yellow-700 dark:text-yellow-400 text-sm">
											...
											Profile is currently private. Toggle{" "}
											<strong>Public</strong> at the top and save to publish
											your microsite.
										</div>
									)}
									...
									<div className="flex items-center gap-2 text-sm text-muted-foreground">
										...
										<span>
											Total referrals: <strong>{form.referralCount}</strong>
										</span>
									</div>
								</>
							) : (
					...
					<Card>
						<CardHeader>
							<CardTitle className="flex items-center gap-2">
								...
								Festival Greeting Card Details
							</CardTitle>
							<CardDescription>
								These override your default name/designation on festival
								greeting cards. Used by the Marketing Tools section.
							</CardDescription>
						</CardHeader>
						<CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-4">
							<div>
								...
								<Input
									placeholder="e.g. Sangram Mohanty"
									className="mt-1"
									{...field("marketingName")}
								/>
							</div>
						...
						<CardContent>
							<div className="rounded-xl bg-gray-900 p-4 flex items-center gap-3 max-w-sm">
								<div className="w-12 h-12 rounded-full bg-amber-600 flex items-center justify-center text-white text-xl font-bold flex-shrink-0">
									{(form.marketingName || form.fullName || "A")
										.charAt(0)
										.toUpperCase()}
								</div>
								<div>
									<p className="text-white font-bold text-sm">
										{form.marketingName || form.fullName || "Your Name"}
									</p>
									<p className="text-yellow-400 text-xs">
										{form.marketingDesignation ||
											form.firmName ||
											"Financial Advisor"}
									</p>
									{form.marketingEmail && (
