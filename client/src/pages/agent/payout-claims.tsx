...
export default function AgentPayoutClaimsPage() {
	...
	const {
		...
		refetch: refetchDisbursed,
	} = useQuery<{ success: boolean; data: DisbursedLoan[] }>({
	...
	const approvedClaims = myClaims.filter(
	...
	const totalPending = pendingClaims.reduce(
		(sum, c) => sum + Number.parseFloat(c.claimAmount),
		0,
	);
	...
	const totalPaid = myClaims
		...
		.reduce((sum, c) => sum + Number.parseFloat(c.claimAmount), 0);
	...
	const claimMutation = useMutation({
		mutationFn: async ({
			...
			invoiceNumber,
			remarks,
		}: { applicationId: string; invoiceNumber?: string; remarks?: string }) => {
			return apiRequest(
				`/api/agent/loans/applications/${applicationId}/claim-payout`,
				{
					method: "POST",
			...
			<div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
				<div>
					...
					<p className="text-muted-foreground">
						Track your commission payouts for disbursed loans
					</p>
				</div>
				<Button
					variant="outline"
			...
			<div className="grid grid-cols-1 md:grid-cols-4 gap-4">
				...
				<Card>
					...
					<CardContent>
						...
						<p className="text-xs text-muted-foreground">
							{approvedClaims.length} claims approved
						</p>
					</CardContent>
				</Card>
				<Card>
				...
				<Card>
					...
					<CardContent>
						<div className="text-2xl font-bold text-blue-600">
							{disbursedLoans.filter((l) => !l.hasClaim).length}
						</div>
						<p className="text-xs text-muted-foreground">
							Disbursed loans pending claim
						</p>
					</CardContent>
				</Card>
			</div>
				...
				<TabsContent value="disbursed">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Loans that have been disbursed and are eligible for commission
								claims
							</CardDescription>
						</CardHeader>
						<CardContent>
											...
											<div className="flex items-start justify-between">
												...
												<div className="flex items-center gap-2">
													{loan.hasClaim ? (
														<Badge
															className={
																statusColors[loan.claimStatus || "pending"]
															}
														>
														...
														<Button
															size="sm"
															onClick={() => handleClaimPayout(loan)}
														>
				...
				<TabsContent value="claims">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Track the status of your commission claims
							</CardDescription>
						</CardHeader>
						<CardContent>
							{loadingClaims ? (
			...
			<Dialog open={claimDialogOpen} onOpenChange={setClaimDialogOpen}>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							Submit a payout claim for the disbursed loan
						</DialogDescription>
					</DialogHeader>
					{selectedLoan && (
						<div className="space-y-4">
							<div className="bg-muted p-4 rounded-lg space-y-2">
								...
								<div className="flex justify-between">
									...
									<span className="font-medium">
										{loanTypeLabels[selectedLoan.loanType] ||
