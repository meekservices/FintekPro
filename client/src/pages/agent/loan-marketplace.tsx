...
> = {
	...
	car: {
		...
		description: "Drive your new vehicle",
	},
	business: {
		label: "Business Loan",
		icon: Building2,
	...
	gold: {
		...
		description: "Leverage your gold assets",
	},
	lap: {
	...
	las: {
		...
		description: "Leverage your stock portfolio",
	},
};
...
export default function AgentLoanMarketplace() {
	...
	const filteredBanks = banks.filter((bank) => {
		const matchesSearch = bank.bankName
			?.toLowerCase()
			.includes(searchQuery.toLowerCase());
		const bankLoanTypes = bank.supportedLoanTypes || [];
		const matchesType =
			selectedType === "all" || bankLoanTypes.includes(selectedType);
			...
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-muted-foreground">
						Browse and recommend loan products to your clients
					</p>
				</div>
			...
			<div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
				{loanTypes.map((type) => {
					...
					const count = banks.filter(
						(b) => (b.supportedLoanTypes || []).includes(type) && b.isActive,
					).length;
					return (
						<Card
							key={type}
						...
						>
							<CardContent className="p-4 text-center">
								<div
									className={`w-10 h-10 mx-auto rounded-full flex items-center justify-center ${config.color}`}
								>
								...
								<div className="flex items-center gap-2">
									...
									<div>
										...
										<p className="font-medium">
											₹{(bank.minAmount / 100000).toFixed(0)}L - ₹
											{(bank.maxAmount / 10000000).toFixed(0)}Cr
										</p>
									</div>
								</div>
								<div className="flex items-center gap-2">
						...
						<div className="flex items-start gap-3">
							...
							<div>
								...
								<p className="text-sm text-muted-foreground">
									Submit to multiple banks simultaneously
								</p>
							</div>
						</div>
						<div className="flex items-start gap-3">
							...
							<div>
								...
								<p className="text-sm text-muted-foreground">
									Earn up to 2% on disbursed loans
								</p>
							</div>
						</div>
						<div className="flex items-start gap-3">
