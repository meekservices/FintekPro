...
function fmtDateTime(s?: string | null) {
  if (!s) return "—";
  try { return format(parseISO(s), "dd MMM HH:mm"); } catch { return s; }
}
...
function AgentOverviewCards() {
  ...
  const pending = accs.filter((a: any) => ["SUBMITTED", "PENDING", "APPROVAL_PENDING"].includes(a.status)).length;

  const cards = [
    {
      icon: Users, label: "Client US Accounts", value: accs.length,
      sub: `${active} active · ${pending} pending`,
      color: "text-blue-600", bg: "from-blue-50 to-blue-100 dark:from-blue-950 dark:to-blue-900",
    },
    {
      icon: TrendingUp, label: "Portfolio Value", value: positions?.configured ? usd(positions.totalValueUSD) : "—",
      sub: positions?.configured ? `₹${(positions.totalValueINR || 0).toLocaleString("en-IN", { maximumFractionDigits: 0 })}` : "Configure Alpaca API",
      color: "text-emerald-600", bg: "from-emerald-50 to-emerald-100 dark:from-emerald-950 dark:to-emerald-900",
    },
    {
      icon: BadgeIndianRupee, label: "LRS Used (FY)", value: lrs ? `$${(lrs.usedUsd || 0).toLocaleString()}` : "—",
      sub: lrs ? `${(lrs.usedPercent || 0).toFixed(1)}% of $2,50,000` : "Annual LRS quota",
...
function ClientAccountsTab() {
  ...
  const { data, isLoading, refetch } = useQuery<{ accounts: any[]; configured: boolean }>({
    ...
    <div className="space-y-4">
      {!data?.configured && (
        <Alert className="border-amber-200 bg-amber-50 dark:bg-amber-950/20">
          ...
          <AlertDescription className="text-amber-700 dark:text-amber-300 text-sm">
            Alpaca Broker API not configured. Contact your administrator to activate US Trading.
          </AlertDescription>
        </Alert>
      )}
      ...
      <div className="flex gap-3 flex-col sm:flex-row">
        <div className="relative flex-1">
          ...
          <Input className="pl-9" placeholder="Search client accounts…" value={search}
  ...
  return (
    <div className="space-y-4">
      {data?.configured && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            { label: "Portfolio Value (USD)", value: usd(data.totalValueUSD), sub: data.isPaper ? "Paper account" : "Live account" },
            { label: "Portfolio Value (INR)", value: `₹${(data.totalValueINR || 0).toLocaleString("en-IN", { maximumFractionDigits: 0 })}`, sub: "At current FX rate" },
            { label: "Total Unrealised P&L", value: usd(data.totalGainLossUSD), sub: `${(data.totalGainLossPercent || 0).toFixed(2)}%` },
            { label: "# Holdings", value: positions.length, sub: "Open positions" },
          ].map(({ label, value, sub }) => (
            <Card key={label}>
              <CardContent className="pt-4 pb-3 px-4">
      ...
      <div className="flex justify-end">
        <Button variant="outline" size="sm" onClick={() => refetch()}>
          <RefreshCw className={`h-4 w-4 mr-1.5 ${isLoading ? "animate-spin" : ""}`} /> Live Refresh
        </Button>
      </div>
...
function AgentActivityTab() {
  ...
  const iconFor = (type: string) => {
    const m: Record<string, any> = {
      ...
      ACATC: ArrowRightLeft, ACATS: ArrowRightLeft,
    };
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        <Select value={actType} onValueChange={setActType}>
          <SelectTrigger className="w-[220px]"><SelectValue placeholder="All activity types" /></SelectTrigger>
          <SelectContent>
            {["", "FILL", "JNLC", "JNLS", "DIV", "CSD", "CSW", "ACATC"].map(v => (
      ...
      <div className="space-y-2">
        {isLoading ? (
          ...
          activities.map((a: any) => {
            ...
            return (
              <div key={a.id} className="flex items-center gap-4 p-3 rounded-lg border bg-card hover:bg-muted/30 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {a.symbol && <span className="font-medium text-sm">{a.symbol}</span>}
                    {a.qty && <span className="text-xs text-muted-foreground">× {a.qty}</span>}
                  </div>
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Acct: {a.account_id?.slice(0, 8)}… · {fmtDateTime(a.date)}
                  </p>
                </div>
                <div className={`font-semibold text-sm ${net >= 0 ? "text-emerald-600" : "text-red-500"}`}>
                  {a.net_amount ? `${net >= 0 ? "+" : ""}${usd(net)}` : a.price ? usd(a.price) : "—"}
                </div>
              </div>
            );
          })
...
function LrsTab() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base flex items-center gap-2">
            <Globe className="h-4 w-4 text-primary" /> LRS Utilization — FY {lrs.financialYear || "—"}
          </CardTitle>
          <CardDescription>RBI Liberalised Remittance Scheme quota tracking</CardDescription>
        </CardHeader>
          ...
          {isLoading ? (
            ...
            <>
              <div className="flex justify-between text-sm mb-1">
                <span className="font-medium">${(lrs.usedUsd || 0).toLocaleString()} used</span>
                <span className="text-muted-foreground">Limit: $250,000</span>
              </div>
              <Progress value={usedPct} className={`h-3 ${usedPct >= 100 ? "bg-red-200" : usedPct >= 80 ? "bg-amber-200" : ""}`} />
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-2">
                {[
                  { label: "Used (USD)", value: `$${(lrs.usedUsd || 0).toLocaleString()}`, color: "text-blue-600" },
                  { label: "Remaining (USD)", value: `$${(lrs.remainingUsd || 250000).toLocaleString()}`, color: "text-emerald-600" },
                  { label: "Used (INR)", value: `₹${(lrs.usedInr || 0).toLocaleString("en-IN")}`, color: "text-purple-600" },
                  { label: "FX Rate", value: `₹${lrs.usdInrRate || 84}/$`, color: "text-amber-600" },
              ...
              {lrs.warning && (
                <Alert className={`border-amber-200 bg-amber-50 dark:bg-amber-950/20 py-2`}>
                  ...
                  <AlertDescription className="text-xs text-amber-700 dark:text-amber-300">{lrs.warning}</AlertDescription>
                </Alert>
              )}
            </>
          )}
      ...
      {/* TCS Status */}
      <Card className={tcsApplies ? "border-orange-200 dark:border-orange-800" : ""}>
        <CardHeader className="pb-3">
          <CardTitle className="text-base flex items-center gap-2">
            <BadgeIndianRupee className={`h-4 w-4 ${tcsApplies ? "text-orange-500" : "text-muted-foreground"}`} />
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            <div className="p-3 rounded-lg border bg-muted/20">
              <p className="text-xs text-muted-foreground">TCS Threshold (FY)</p>
              <p className="font-bold">₹7,00,000</p>
            </div>
            <div className="p-3 rounded-lg border bg-muted/20">
              <p className="text-xs text-muted-foreground">TCS Rate (Above Threshold)</p>
              <p className="font-bold">20%</p>
            </div>
            <div className={`p-3 rounded-lg border ${tcsApplies ? "bg-orange-50 border-orange-200 dark:bg-orange-950/20 dark:border-orange-800" : "bg-muted/20"}`}>
              <p className="text-xs text-muted-foreground">TCS Applicable?</p>
              <p className={`font-bold ${tcsApplies ? "text-orange-600" : "text-emerald-600"}`}>
                {tcsApplies ? `Yes — ₹${(lrs.tcsAmountInr || 0).toLocaleString("en-IN")}` : "Not yet"}
              </p>
            </div>
          </div>
          <p className="text-xs text-muted-foreground mt-3">
            Your AD-I bank deducts 20% TCS on LRS amounts exceeding ₹7L/FY. Claim TCS credit when filing your ITR under §206C(1G).
          </p>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base flex items-center gap-2">
            <Wallet className="h-4 w-4 text-primary" /> How to Fund Your Alpaca Account
          </CardTitle>
          <CardDescription>SWIFT wire transfer via LRS from your India bank</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-3">
            {[
              { step: 1, title: "Get your USD account details", desc: "Check Funding → Wallet for your dedicated Alpaca beneficiary account number." },
              { step: 2, title: "Visit your AD-I bank (LRS desk)", desc: "SBI, HDFC, ICICI, Axis, Kotak, etc. Online: HDFC Netbanking → Forex, or Wise." },
              { step: 3, title: "Fill Form A2", desc: "Purpose: 'Overseas portfolio investment in US equities under LRS'. Purpose code: S0001." },
              { step: 4, title: "Provide PAN + KYC", desc: "PAN is mandatory. Ensure PAN is linked to Aadhaar at your bank." },
              { step: 5, title: "Initiate SWIFT wire", desc: "Send USD to your Alpaca beneficiary account. Settlement: 2–5 business days." },
            ].map(({ step, title, desc }) => (
...
export default function AlpacaHubAgent() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Tabs */}
      <Tabs value={activeTab} onValueChange={v => setActiveTab(v as AgentTabValue)}>
        ...
        <div className="mt-4">
          <TabsContent value="accounts"  className="mt-0"><ClientAccountsTab /></TabsContent>
          <TabsContent value="positions" className="mt-0"><AgentPositionsTab /></TabsContent>
          <TabsContent value="activity"  className="mt-0"><AgentActivityTab /></TabsContent>
          <TabsContent value="lrs"       className="mt-0"><LrsTab /></TabsContent>
        </div>
      </Tabs>
      ...
      <Alert className="border-blue-200 bg-blue-50/50 dark:bg-blue-950/10">
        ...
        <AlertDescription className="text-xs text-blue-700 dark:text-blue-300">
          <strong>Note:</strong> FintekPro operates as a Fully-Disclosed Broker-Dealer via Alpaca Markets.
          All US equity accounts are held in custody by Velox Clearing LLC (FINRA member, SIPC protected).
          Data refreshes every 30 seconds for live accounts.
        </AlertDescription>
      </Alert>
    </div>
  );
