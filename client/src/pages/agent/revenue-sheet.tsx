...
function MonthlyPlanner({
	...
	month,
	year,
}: { cases: Case[]; summary: Summary; month: number; year: number }) {
	const [targetCases, setTargetCases] = useState("20");
	const [targetCommission, setTargetCommission] = useState("50000");

	const tc = Number.parseInt(targetCases) || 0;
	const tcom = Number.parseFloat(targetCommission) || 0;
	const casesPct = tc > 0 ? Math.min(100, (summary.totalCases / tc) * 100) : 0;
	const commissionPct =
		tcom > 0 ? Math.min(100, (summary.totalCommission / tcom) * 100) : 0;
			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
				<Card>
					<CardHeader>
						...
						<CardDescription>
							Track your progress against your goals for {MONTHS[month - 1]}{" "}
							{year}
						</CardDescription>
					</CardHeader>
				...
				<Card>
					<CardHeader>
						...
						<CardDescription>
							Actual vs your set targets this month
						</CardDescription>
					</CardHeader>
					<CardContent className="space-y-5">
						<div className="space-y-2">
							<div className="flex justify-between text-sm">
								...
								<span className="font-medium">
									{summary.totalCases} / {tc || "—"}
								</span>
							</div>
							<Progress value={casesPct} className="h-3" />
							<p className="text-xs text-right text-muted-foreground">
								{casesPct.toFixed(0)}% achieved
							</p>
						</div>
						...
						<div className="space-y-2">
							<div className="flex justify-between text-sm">
								...
								<span className="font-medium">
									{formatCompact(summary.totalCommission)} /{" "}
									{tcom ? formatCompact(tcom) : "—"}
								</span>
							</div>
							<Progress value={commissionPct} className="h-3" />
							<p className="text-xs text-right text-muted-foreground">
								{commissionPct.toFixed(0)}% achieved
							</p>
						</div>
						...
						{commissionPct >= 100 && (
							<div className="p-3 bg-green-50 dark:bg-green-900/20 rounded-lg text-center">
								<p className="text-sm font-medium text-green-700 dark:text-green-300">
									🎉 Target achieved! Excellent work this month.
								</p>
							</div>
						)}
						{commissionPct >= 80 && commissionPct < 100 && (
							<div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg text-center">
								<p className="text-sm font-medium text-blue-700 dark:text-blue-300">
									Almost there! Just{" "}
									{formatCompact(tcom - summary.totalCommission)} more to go.
								</p>
							</div>
						)}
					</CardContent>
							...
							{productBreakdown.map((p) => {
								const pct =
									summary.totalCommission > 0
										? (p.amount / summary.totalCommission) * 100
										: 0;
								return (
		...
		<div className="space-y-6">
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Status of your commissions for this month
					</CardDescription>
				</CardHeader>
				<CardContent>
...
export default function AgentRevenueSheet() {
	...
	const handleDownload = () => {
		toast({
			...
			description: "Your revenue sheet is being prepared for download.",
		});
	};
				...
				<div className="flex items-center gap-3">
					...
					<div>
						...
						<p className="text-muted-foreground text-sm">
							Case-wise payout tracker with monthly planner
						</p>
					</div>
				</div>
				<div className="flex items-center gap-3">
						...
						<CardContent>
							{isLoading ? (
								...
								<div className="py-16 text-center text-muted-foreground">
									...
									<p className="font-medium">
										No cases found for {MONTHS[month - 1]} {year}
									</p>
									<p className="text-sm mt-1">
										Cases will appear here once commissions are recorded
									</p>
								</div>
							) : (
												...
												return (
													<TableRow key={c.id}>
														...
														<TableCell>
															<Badge
																className={
																	categoryColor[c.productCategory] ||
																	categoryColor.loans
																}
															>
																{c.productType}
