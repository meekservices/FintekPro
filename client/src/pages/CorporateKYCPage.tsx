...
const entityTypeSchema = z.object({
	entityType: z.enum(
		["company", "partnership", "trust", "llp", "huf", "society", "cooperative"],
...
const beneficialOwnerSchema = z.object({
	...
	ownershipPercentage: z
		.string()
		.regex(/^[0-9]{1,3}(\.[0-9]{1,2})?$/, "Invalid percentage"),
...
export default function CorporateKYCPage() {
	...
	const [currentStep, setCurrentStep] = useState<
		"entity" | "verification" | "directors" | "beneficial-owners" | "review"
	>("entity");
		...
		onSuccess: (data: any) => {
			...
			toast({
				...
				description: data.verified
					? "Entity verified successfully"
					: "Verification completed with some issues",
			});

			// Auto-populate directors from MCA data if available
			if (data.details?.mca?.directors) {
				setDirectors(
					data.details.mca.directors.map((d: any) => ({
						din: d.din,
						name: d.name,
	...
	const submitKYCMutation = useMutation({
		...
		onSuccess: () => {
			toast({
				...
				description:
					"Your corporate KYC application has been submitted for review",
			});
		},
	});
	...
	const onCompanyVerificationSubmit = (
		data: z.infer<typeof companyVerificationSchema>,
	) => {
			...
			<div className="max-w-5xl mx-auto">
				<div className="mb-8">
					...
					<p className="text-muted-foreground mt-2">
						Complete KYC verification for non-individual entities using
				...
				{currentStep === "entity" && (
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Choose the type of organization for KYC verification
							</CardDescription>
						</CardHeader>
						<CardContent>
							<Form {...entityForm}>
								<form
									onSubmit={entityForm.handleSubmit(onEntityTypeSubmit)}
									className="space-y-6"
								>
									<FormField
									...
									<Button
										type="submit"
										className="w-full"
										data-testid="button-continue-entity"
									>
				...
				{currentStep === "verification" && (
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Verify {selectedEntityType} details using government databases
								(MCA, GSTIN, PAN)
							</CardDescription>
						</CardHeader>
						<CardContent>
							...
							{verificationResult && (
								<div className="mt-6 space-y-4">
									...
									<Alert
										variant={
											verificationResult.verified ? "default" : "destructive"
										}
									>
										...
										<Card>
											...
											<CardContent className="space-y-2 text-sm">
												<div>
													...
													{verificationResult.details.mca.companyName}
												</div>
												<div>
													<strong>Status:</strong>{" "}
													<Badge>
														{verificationResult.details.mca.companyStatus}
													</Badge>
												</div>
												<div>
													<strong>Incorporation:</strong>{" "}
													{verificationResult.details.mca.dateOfIncorporation}
												</div>
												<div>
													<strong>Directors:</strong>{" "}
													{verificationResult.details.mca.directors?.length ||
														0}
												</div>
											</CardContent>
										</Card>
				...
				{currentStep === "directors" && (
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Add KYC details for all authorized persons
							</CardDescription>
						</CardHeader>
						<CardContent className="space-y-6">
							...
							{directors.length > 0 && (
								<div className="space-y-2">
									<h3 className="font-semibold">
										Added Directors ({directors.length})
									</h3>
							...
							<Form {...directorForm}>
								<form
									onSubmit={directorForm.handleSubmit(addDirector)}
									className="space-y-4"
								>
									<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
										<FormField
												...
												<FormItem>
													...
													<FormControl>
														<Input
															...
															data-testid="input-director-din"
														/>
													</FormControl>
													<FormMessage />
												</FormItem>
												...
												<FormItem>
													...
													<FormControl>
														<Input
															...
															data-testid="input-director-pan"
														/>
													</FormControl>
													<FormMessage />
												</FormItem>
												...
												<FormItem>
													...
													<FormControl>
														<Input
															...
															data-testid="input-director-designation"
														/>
													</FormControl>
													<FormMessage />
												</FormItem>
				...
				{currentStep === "beneficial-owners" && (
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Declare ultimate beneficial owners (25% or more ownership)
							</CardDescription>
						</CardHeader>
						<CardContent className="space-y-6">
							{beneficialOwners.length > 0 && (
								<div className="space-y-2">
									<h3 className="font-semibold">
										Declared Beneficial Owners ({beneficialOwners.length})
									</h3>
							...
							<Form {...beneficialOwnerForm}>
								<form
									onSubmit={beneficialOwnerForm.handleSubmit(
										addBeneficialOwner,
									)}
									className="space-y-4"
								>
												...
												<FormItem>
													...
													<FormControl>
														<Input
															{...field}
															placeholder="25.00"
															data-testid="input-ubo-percentage"
														/>
													</FormControl>
													<FormMessage />
												</FormItem>
							...
							<div className="flex gap-4 pt-4">
								<Button
									variant="outline"
									onClick={() => setCurrentStep("directors")}
								>
									Back
								</Button>
								<Button
				...
				{currentStep === "review" && (
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Review all information before submitting KYC application
							</CardDescription>
						</CardHeader>
						<CardContent className="space-y-6">
							<div className="space-y-4">
								<div>
									...
									<div className="text-sm space-y-1">
										<div>
											Type: <Badge>{selectedEntityType}</Badge>
										</div>
										<div>
											Name:{" "}
											{verificationResult?.details?.mca?.companyName ||
								...
								<div>
									<h3 className="font-semibold mb-2">
										Directors/Authorized Persons ({directors.length})
									</h3>
									{directors.map((dir, idx) => (
