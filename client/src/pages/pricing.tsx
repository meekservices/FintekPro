...
function FeatureCheck({ enabled, value }: { enabled: boolean; value?: string }) {
  if (value) return <span className="text-sm font-medium text-foreground">{value}</span>;
  if (enabled) return <Check className="h-4 w-4 text-green-500 mx-auto" />;
  return <span className="h-4 w-4 block text-center text-muted-foreground/40 mx-auto">—</span>;
}
...
function PlanCard({ plan, annual, currentTier, onSelect, loading }: PlanCardProps) {
  const isCurrent = currentTier === plan.tier;
  const isUpgrade = !isCurrent && plan.tier !== 'free' &&
    (['free'].includes(currentTier || 'free') || (currentTier === 'pro' && plan.tier === 'elite'));
  ...
  const savingsPercent = !isFree && !isElite && plan.monthlyPriceInr && plan.annualPriceInr
    ? Math.round(100 - (plan.annualPriceInr / (plan.monthlyPriceInr * 12)) * 100)
    : 0;
  ...
  return (
    ...
    >
      ...
      {isCurrent && (
        <div className="absolute -top-3 left-0 right-0 flex justify-center">
          <Badge className="bg-green-600 text-white px-4 py-0.5 text-xs font-semibold">Current Plan</Badge>
        </div>
      )}
      ...
      <CardHeader className={`pb-4 ${plan.highlight ? 'pt-7' : 'pt-5'}`}>
        <div className="flex items-center gap-2 mb-2">
          ...
          {plan.tier === 'elite' && <Crown className="h-5 w-5 text-yellow-500" />}
            ...
            <>
              <div className="flex items-baseline gap-1">
                ...
                <span className="text-sm text-muted-foreground">
                  {isElite ? '/year' : isFree ? '' : annual ? '/year' : '/month'}
                </span>
              </div>
              {annual && savingsPercent > 0 && (
                <Badge variant="outline" className="mt-1 text-xs text-green-600 border-green-300 bg-green-50">
                  Save {savingsPercent}% annually
                </Badge>
              )}
              {isElite && (
                <p className="text-xs text-muted-foreground mt-1">Annual billing only. HNI & advisor pricing on request.</p>
              )}
            </>
          ) : (
      ...
      <CardContent className="flex-1 flex flex-col gap-4">
        ...
        <div className="rounded-lg bg-muted/40 p-3 space-y-1.5 text-xs">
          ...
          <div className="flex justify-between">
            ...
            <span className="font-semibold">
              {plan.tradeFeeInr === 0 ? '₹0 (bundled)' : plan.tradeFeeInr === null ? 'N/A' : `₹${plan.tradeFeeInr}`}
            </span>
          </div>
        </div>
        ...
        {/* CTA */}
        {isFree ? (
          <Button variant="outline" className="w-full mt-2" disabled={isCurrent}>
      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          ...
          <tr className="hover:bg-muted/20 transition-colors">
            ...
            {plans.map((p) => (
              <td key={p.tier} className="text-center px-4 py-3 font-semibold">
                {p.tradeFeeInr === 0 ? 'Bundled' : p.tradeFeeInr === null ? '—' : `₹${p.tradeFeeInr}`}
              </td>
            ))}
          </tr>
        </tbody>
...
function PaymentModal({ tier, cycle, onClose }: PaymentModalProps) {
  ...
  const checkoutMutation = useMutation({
    mutationFn: async () => {
      const res = await apiRequest('POST', '/api/subscriptions/checkout', { tier, cycle });
      return res.json();
    },
    onSuccess: (data: any) => {
      if (data.paymentUrl) {
  ...
  const price = tier === 'pro'
    ? cycle === 'annual' ? '₹9,999/year' : '₹999/month'
    : '₹25,000/year';
      ...
      <div className="bg-background rounded-2xl border shadow-xl max-w-md w-full p-6 space-y-5">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">{price} billed {cycle}</p>
          </div>
        ...
        <div className="space-y-3 text-sm">
          ...
          <div className="flex items-center gap-3 rounded-lg bg-muted/40 p-3">
            ...
            <div>
              ...
              <div className="text-xs text-muted-foreground">Your plan upgrades immediately after payment</div>
            </div>
          </div>
          <div className="flex items-center gap-3 rounded-lg bg-muted/40 p-3">
            ...
            <div>
              ...
              <div className="text-xs text-muted-foreground">18% GST included, invoice sent to your email</div>
            </div>
          </div>
        </div>
...
export default function PricingPage() {
  ...
  const { toast } = useToast();
  const [selectedPlan, setSelectedPlan] = useState<{ tier: string; cycle: string } | null>(null);
      ...
      <div className="max-w-7xl mx-auto px-4 py-12 space-y-16">
        ...
        <div className="text-center space-y-4">
          ...
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
            Access US markets, AI-powered research, and institutional-grade analytics.
            No hidden fees — transparent pricing built for serious Indian investors.
          </p>
          ...
          <div className="flex items-center justify-center gap-3 mt-6">
            ...
            <Label htmlFor="billing-toggle" className={annual ? 'font-semibold' : 'text-muted-foreground'}>
              Annual
              <Badge variant="outline" className="ml-2 text-xs text-green-600 border-green-300 bg-green-50">
                Save up to 17%
              </Badge>
            </Label>
          </div>
        ...
        <div className="rounded-2xl bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-950/20 dark:to-indigo-950/20 border border-blue-100 dark:border-blue-900 p-8">
          <div className="text-center mb-8">
            <h2 className="text-2xl font-bold mb-2">How FintekPro makes money</h2>
            <p className="text-muted-foreground text-sm">
              Transparent revenue model — you know exactly what you pay and what we earn
            </p>
          </div>
          <RevenueInfo />
        </div>
        ...
        {plans.length > 0 && (
          <div>
            <h2 className="text-2xl font-bold text-center mb-6">Full Feature Comparison</h2>
        ...
        <div className="rounded-2xl border bg-card p-8">
          <h2 className="text-2xl font-bold mb-2 text-center">Expected returns by tier</h2>
          <p className="text-sm text-muted-foreground text-center mb-6">
            What investors typically earn annually across each plan
          </p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              { tier: 'Free', arpu: '₹1,500–₹3,000/yr', color: 'bg-slate-100 dark:bg-slate-800', desc: 'Via FX spread, securities lending & cash yield share' },
              { tier: 'Pro', arpu: '₹12,000–₹18,000/yr', color: 'bg-blue-50 dark:bg-blue-950/30', desc: 'Platform value + lower FX + AI alpha generation' },
              { tier: 'Elite', arpu: '₹50,000–₹2,00,000/yr', color: 'bg-amber-50 dark:bg-amber-950/30', desc: 'Advisory alpha, alt data, institutional-grade execution' },
            ].map(({ tier, arpu, color, desc }) => (
              <div key={tier} className={`rounded-xl ${color} p-5 text-center space-y-2`}>
                <div className="text-lg font-bold">{tier}</div>
                <div className="text-2xl font-extrabold text-primary">{arpu}</div>
        ...
        <div className="max-w-2xl mx-auto space-y-4">
          ...
          {[
            { q: 'How does FX spread work?', a: 'When you remit funds to your Alpaca account (via LRS), FintekPro charges a small spread on the USD/INR conversion. Free users pay 1%, Pro 0.5%, Elite 0.25–0.4%. On a ₹10L investment, this is ₹10,000 vs ₹5,000 vs ₹2,500.' },
            { q: 'Can I cancel my subscription?', a: 'Yes, you can cancel anytime. Your plan remains active until the billing period ends. No automatic renewal without consent.' },
            { q: 'Is GST included in the price?', a: 'All displayed prices are inclusive of 18% GST. A proper GST invoice will be sent to your registered email.' },
            { q: 'What payment methods are supported?', a: 'UPI (GPay, PhonePe, Paytm), Net Banking, Credit/Debit Cards (Visa, Mastercard, RuPay), and popular wallets — all via Cashfree.' },
            { q: 'Elite plan custom pricing?', a: 'For family offices, RIAs managing ₹10Cr+ AUM, or firms needing white-labeling, contact us at enterprise@fintekpro.com for bespoke pricing.' },
          ].map(({ q, a }) => (
            <details key={q} className="group rounded-xl border bg-card px-5 py-4 cursor-pointer">
