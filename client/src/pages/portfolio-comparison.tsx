...
function PortfolioSearchInput({
	...
	const filteredResults = availablePortfolios
		.filter(
			(portfolio) =>
				...
				<Card>
					...
					<CardContent>
						<p className="font-semibold">
							{comparison.analysis.bestDiversified}
						</p>
					</CardContent>
				</Card>
			</div>
									...
									<tbody>
										{comparison.portfolios.map((portfolio) => (
											<tr key={portfolio.id} className="border-b">
												...
												<td
													className={`text-center p-2 ${getReturnColor(portfolio.returns["1M"])}`}
												>
													{portfolio.returns["1M"].toFixed(2)}%
												</td>
												<td
													className={`text-center p-2 ${getReturnColor(portfolio.returns["6M"])}`}
												>
													{portfolio.returns["6M"].toFixed(2)}%
												</td>
												<td
													className={`text-center p-2 ${getReturnColor(portfolio.returns["1Y"])}`}
												>
													{portfolio.returns["1Y"].toFixed(2)}%
												</td>
												<td
													className={`text-center p-2 ${getReturnColor(portfolio.returns["3Y"])}`}
												>
													{portfolio.returns["3Y"].toFixed(2)}%
												</td>
												<td
													className={`text-center p-2 ${getReturnColor(portfolio.returns["5Y"])}`}
												>
				...
				<TabsContent value="risk" className="space-y-4">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Compare risk-adjusted performance metrics across portfolios.
								Higher values are better for ratios (except Beta &gt; 1 means
									...
									<tbody>
										{comparison.portfolios.map((portfolio) => (
											...
											>
												<td className="p-2 font-medium">{portfolio.name}</td>
												<td
													className={`text-center p-2 font-medium ${(portfolio.metrics.alpha || 0) > 0 ? "text-green-600" : (portfolio.metrics.alpha || 0) < 0 ? "text-red-600" : ""}`}
												>
													{(portfolio.metrics.alpha || 0).toFixed(2)}%
												</td>
												<td
													className={`text-center p-2 ${(portfolio.metrics.beta || 1) > 1.2 ? "text-orange-600" : (portfolio.metrics.beta || 1) < 0.8 ? "text-blue-600" : ""}`}
												>
													{(portfolio.metrics.beta || 1).toFixed(2)}
												</td>
												<td
													className={`text-center p-2 font-medium ${(portfolio.metrics.sharpeRatio || 0) > 1 ? "text-green-600" : (portfolio.metrics.sharpeRatio || 0) < 0.5 ? "text-red-600" : ""}`}
												>
													{(portfolio.metrics.sharpeRatio || 0).toFixed(2)}
												</td>
												<td
													className={`text-center p-2 ${(portfolio.metrics.treynorRatio || 0) > 5 ? "text-green-600" : ""}`}
												>
													{(portfolio.metrics.treynorRatio || 0).toFixed(2)}
												</td>
												<td
													className={`text-center p-2 ${portfolio.metrics.sortinoRatio != null && portfolio.metrics.sortinoRatio > 1.5 ? "text-green-600" : ""}`}
												>
													{portfolio.metrics.sortinoRatio != null
														? portfolio.metrics.sortinoRatio.toFixed(2)
														: "N/A"}
												</td>
												<td
													className={`text-center p-2 ${(portfolio.metrics.informationRatio || 0) > 0.5 ? "text-green-600" : ""}`}
												>
								...
								<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 text-sm">
									<div>
										...
										<p className="text-muted-foreground">
											Excess returns over benchmark. Positive = outperforming
									...
									<div>
										...
										<p className="text-muted-foreground">
											Market sensitivity. Beta=1 moves with market, &gt;1 more
											volatile, &lt;1 less volatile.
										</p>
									</div>
									<div>
										<span className="font-medium text-primary">
										...
										<p className="text-muted-foreground">
											Return per unit of total risk. &gt;1 is good, &gt;2 is
											excellent.
										</p>
									</div>
									<div>
										...
										<p className="text-muted-foreground">
											Return per unit of systematic risk (beta). Higher is
									...
									<div>
										...
										<p className="text-muted-foreground">
											Like Sharpe but only penalizes downside volatility. Better
									...
									<div>
										...
										<p className="text-muted-foreground">
											Active return per tracking error. Measures manager skill
											vs benchmark.
										</p>
									</div>
									<div>
										...
										<p className="text-muted-foreground">
											Largest peak-to-trough decline. Worst-case historical
											loss.
										</p>
									</div>
									<div>
										...
										<p className="text-muted-foreground">
											Standard deviation of returns. Higher = more price
											fluctuation.
										</p>
									</div>
								</div>
									...
									<div className="space-y-3">
										<div className="flex justify-between items-center">
											...
											<span className="font-semibold">
												{portfolio.assetAllocation.equity.toFixed(1)}%
											</span>
										</div>
										<div className="w-full bg-muted rounded-full h-2">
											<div
												...
												style={{
													width: `${portfolio.assetAllocation.equity}%`,
												}}
											/>
										</div>
										...
										<div className="flex justify-between items-center">
											<span>Debt</span>
											<span className="font-semibold">
												{portfolio.assetAllocation.debt.toFixed(1)}%
											</span>
										</div>
										<div className="w-full bg-muted rounded-full h-2">
											<div
										...
										<div className="flex justify-between items-center">
											<span>Cash</span>
											<span className="font-semibold">
												{portfolio.assetAllocation.cash.toFixed(1)}%
											</span>
										</div>
										<div className="w-full bg-muted rounded-full h-2">
											<div
										...
										<div className="flex justify-between items-center">
											<span>Others</span>
											<span className="font-semibold">
												{portfolio.assetAllocation.others.toFixed(1)}%
											</span>
										</div>
										<div className="w-full bg-muted rounded-full h-2">
											<div
												...
												style={{
													width: `${portfolio.assetAllocation.others}%`,
												}}
											/>
										</div>
									</div>
			...
			<Card>
				<CardHeader>
					...
					<div className="flex items-center gap-2">
						...
						<Badge
							variant={
								comparison.recommendationScore >= 8
									? "default"
									: comparison.recommendationScore >= 6
										? "secondary"
										: "destructive"
							}
						>
...
export default function PortfolioComparison() {
	...
	const handlePortfolioRemove = (portfolioId: string) => {
		setSelectedPortfolios(
			selectedPortfolios.filter((p) => p.id !== portfolioId),
		);
	};
	...
	return (
		<div className="container mx-auto px-4 py-8 max-w-7xl">
			<div className="mb-8">
				...
				<p className="text-muted-foreground">
					Compare multiple portfolios side by side to analyze performance, risk
					metrics, and asset allocation.
				</p>
			</div>
								...
								<div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
									...
									<div>
										...
										<Select
											...
											onValueChange={setBenchmarkIndex}
										>
											...
											<SelectContent>
												...
												<SelectItem value="NIFTY_SMALLCAP">
													NIFTY SMALLCAP
												</SelectItem>
											</SelectContent>
										</Select>
