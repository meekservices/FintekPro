...
function TierUpgradeProgress({
	...
	const unlockedPercent =
		total > 0 ? (unlockedProducts.length / total) * 100 : 0;
	...
	const currentTier =
		currentTierIndex >= 0 ? TIER_ORDER[currentTierIndex] : "basic";
	const nextTier =
		currentTierIndex < highestRequiredIndex
		...
		<Card>
			...
			<CardContent className="space-y-4">
				<div className="flex items-center gap-3">
					<Badge
						variant="outline"
						className={`${getTierColor(currentTier)} text-white border-0`}
					>
					...
					{nextTier && (
						<>
							...
							<Badge
								variant="outline"
								className={`${getTierColor(nextTier)} text-white border-0`}
							>
					...
					<Alert>
						...
						<AlertDescription>
							<span className="block mb-2">
								Upgrade to <strong>{getTierLabel(nextTier)}</strong> tier by
								completing:
							</span>
...
function ProductCard({ product }: { product: ProductEligibility }) {
	const cardContent = (
		<Card
			className={`transition-all ${product.locked ? "border-muted" : "border-green-200 dark:border-green-800"}`}
		>
			...
			<CardContent className="space-y-3">
				<div className="flex items-center gap-2">
					<Badge
						variant={product.eligible ? "default" : "destructive"}
						className="text-xs"
					>
		...
		toast({
			...
			description:
				error instanceof Error
					? error.message
					: "Failed to load product eligibility data",
			variant: "destructive",
		});
	}
			...
			<div className="space-y-6 p-4 md:p-6">
				<div>
					...
					<p className="text-muted-foreground">
						View which products you can access based on your KYC tier and
						verification status.
					</p>
				</div>
				...
				{eligibility.length === 0 && !isLoading && (
					<Alert>
						...
						<AlertDescription>
							Complete your KYC verification to see which products you can
							access.
						</AlertDescription>
					</Alert>
				)}
