...
export default function OneClickTaxFiling() {
	...
	const oneClickPopulateMutation = useMutation({
		...
		onSuccess: () => {
			toast({
				...
				description:
					"Your tax return has been automatically filled with data from all sources.",
			});
	...
	const autoValidateMutation = useMutation({
		mutationFn: async () => {
			const response = await apiRequest(
				`/api/itr/auto-validate/${itrData?.id}`,
				{
					method: "POST",
					body: JSON.stringify({}),
				},
		...
		onSuccess: (data) => {
			const errorCount =
				data.validationErrors?.filter((e: any) => e.severity === "error")
					.length || 0;
	...
	useEffect(() => {
		if (
			...
			!oneClickPopulateMutation.isPending &&
			!itrData
		) {
	...
	const connectedSourcesCount = dataSourcesList.filter(
		...
		<div className="max-w-6xl mx-auto p-6 space-y-6">
			...
			<div className="text-center space-y-3">
				...
				<p className="text-lg text-muted-foreground">
					File your Income Tax Return in minutes with intelligent
					auto-population
				</p>
			</div>
			...
			{currentStep === "connect" && (
				<Card>
					<CardHeader>
						...
						<CardDescription>
							We'll automatically detect and connect all available tax data
							sources
						</CardDescription>
					</CardHeader>
						...
						<div className="flex items-center justify-between">
							<div>
								<p className="font-medium">
									Ready to connect {totalSourcesCount} data sources
								</p>
								<p className="text-sm text-muted-foreground">
			...
			{currentStep === "populate" && (
				<Card>
					<CardHeader>
						...
						<CardDescription>
							Intelligently gathering data from {connectedSourcesCount}{" "}
							connected sources
						</CardDescription>
					</CardHeader>
					<CardContent>
						...
						<CardContent className="space-y-4">
							<div className="flex items-center justify-between">
								...
								<div className="flex items-center gap-3">
									<Progress
										value={itrData.completionPercentage}
										className="w-32"
									/>
									<span className="font-semibold">
										{itrData.completionPercentage}%
									</span>
								</div>
							</div>
							...
							<div className="flex items-center justify-between">
								...
								<div className="flex gap-3">
									...
									<Button
										...
										disabled={
											fileITRMutation.isPending || !itrData.readyForFiling
										}
					...
					<CardContent className="p-8">
						<div className="flex flex-col items-center justify-center text-center space-y-4">
							...
							<p className="text-lg text-green-700 dark:text-green-300">
								Your Income Tax Return for AY {selectedYear} has been
								successfully filed.
							</p>
