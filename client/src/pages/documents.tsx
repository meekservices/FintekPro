...
function DocumentCard({ document, onSign }: { document: Document; onSign: (doc: Document) => void }) {
            ...
            <div className="flex gap-2">
              ...
              {canDownload && (
                <Button 
                  size="sm" 
                  variant="outline"
                  onClick={() => window.open(document.signedDocumentUrl || document.documentUrl || '', '_blank')}
                >
...
export default function DocumentsPage() {
  ...
  const pendingDocs = data?.documents.filter(d => d.clientStatus === 'pending') || [];
  const completedDocs = data?.documents.filter(d => d.clientStatus === 'completed') || [];
  ...
  return (
    <div className="container max-w-4xl mx-auto py-6 space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">View and sign documents requiring your attention</p>
      </div>

      <div className="grid grid-cols-3 gap-4">
        <Card>
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-amber-600">{data?.pendingCount || 0}</div>
            <div className="text-sm text-muted-foreground">Pending Signature</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-green-600">{data?.completedCount || 0}</div>
          ...
          {isLoading ? (
            ...
            <Card>
              <CardContent className="p-8 text-center">
                ...
                <p className="text-sm text-muted-foreground">No documents pending your signature</p>
              </CardContent>
            </Card>
          ) : (
            pendingDocs.map(doc => (
          ...
          {isLoading ? (
            ...
            <Card>
              <CardContent className="p-8 text-center">
                ...
                <p className="text-sm text-muted-foreground">Documents you sign will appear here</p>
              </CardContent>
            </Card>
          ) : (
          ...
          {isLoading ? (
            ...
            <Card>
              <CardContent className="p-8 text-center">
                ...
                <p className="text-sm text-muted-foreground">Documents sent to you will appear here</p>
              </CardContent>
            </Card>
          ) : (
            allDocs.map(doc => (
