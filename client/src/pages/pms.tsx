			...
			<Card className="border-dashed border-2 border-purple-200 dark:border-purple-800 bg-purple-50 dark:bg-purple-950/30/50">
				<CardContent className="flex flex-col items-center justify-center py-16">
					...
					<h3 className="text-xl font-semibold mb-2">
						No Pending PMS Proposals
					</h3>
					<p className="text-muted-foreground text-center max-w-md mb-4">
						AI and agent recommendations for Portfolio Management Services will
						appear here.
					</p>
					<Button
						variant="outline"
		...
		<div className="space-y-4">
			{pendingProposals.map((proposal) => (
				<Card
					...
					data-testid={`pms-proposal-${proposal.id}`}
				>
					<CardContent className="p-0">
						<div className="flex">
							<div
								className={`w-2 ${proposal.proposalSource === "ai" ? "bg-gradient-to-b from-purple-500 to-pink-600" : "bg-gradient-to-b from-teal-500 to-cyan-600"}`}
							/>
							<div className="flex-1 p-6">
								...
								<div className="grid grid-cols-4 gap-4 mb-4 p-4 bg-muted rounded-lg">
									<div className="text-center">
										...
										<p className="text-lg font-bold text-emerald-600">
											{proposal.expectedReturns || "N/A"}%
										</p>
									</div>
									<div className="text-center">
										<p className="text-sm text-muted-foreground">Strategy</p>
										<p className="text-lg font-bold">
											{proposal.strategy || "Multicap"}
										</p>
									</div>
									<div className="text-center">
										<p className="text-sm text-muted-foreground">Lock-in</p>
										<p className="text-lg font-bold">
											{proposal.lockIn || "None"}
										</p>
									</div>
									<div className="text-center">
								...
								<div className="flex gap-3">
									<Button
										...
										data-testid={`approve-pms-${proposal.id}`}
									>
									...
									<Button
										...
										data-testid={`reject-pms-${proposal.id}`}
									>
...
function CartTab({
	...
	const checkoutMutation = useMutation({
			...
			for (const item of cartItems) {
				if (item.proposalId) {
					await apiRequest(`/api/proposals/${item.proposalId}/complete-order`, {
						method: "POST",
						body: JSON.stringify({ orderType: "LUMPSUM", productType: "pms" }),
					});
		...
		onSuccess: () => {
			toast({
				title: "Order Placed!",
				description: "Your PMS investment order has been submitted",
			});
			queryClient.invalidateQueries({ queryKey: ["/api/cart"] });
								...
								<div className="flex-1">
									<div className="flex items-center gap-2 mb-2">
										...
										<h4 className="font-semibold">
											{item.productName || "PMS Investment"}
										</h4>
										<Badge className="bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300">
											PMS
										</Badge>
							...
							<div className="flex items-start gap-2">
								...
								<div>
									...
									<p className="text-amber-700 dark:text-amber-300 text-xs">
										Minimum ₹50 Lakhs. Advisory fees apply.
									</p>
								</div>
							</div>
						</div>
						<Button
							className="w-full bg-gradient-to-r from-purple-500 to-pink-500 text-foreground font-semibold py-6"
							onClick={() => checkoutMutation.mutate()}
							disabled={checkoutMutation.isPending}
							data-testid="pms-checkout-btn"
						>
...
export default function PMS() {
	...
	const { data: pmsResponse, isLoading } = useQuery<{
		schemes: any[];
		pagination: any;
	}>({
		queryKey: [
	...
	const pendingProposals =
		proposalData?.filter(
			(p) => p.status === "pending" && p.productType === "pms",
		)?.length || 0;
	const cartCount =
		cartData?.items?.filter((i: any) => i.productCategory === "pms")?.length ||
		0;
	...
	const statistics = {
		...
		totalAUM: displayData.reduce(
			(sum: number, fund: any) =>
				sum + (Number.parseFloat(fund.aum || "0") || 0),
			0,
		),
		averageReturns:
			displayData.length > 0
				? displayData.reduce(
						(sum: number, f: any) =>
							sum + (Number.parseFloat(f.return1Y || "0") || 0),
						0,
					) / displayData.length
				: 0,
	...
	if (isLoading)
		return (
			<div className="min-h-screen bg-finance-light p-8" data-testid="pms-page">
			...
			<main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
				<div className="mb-8">
					...
					<p className="text-muted-foreground text-lg max-w-3xl">
						Professional portfolio management with customized strategies from
							...
							<div className="flex items-center justify-between">
								<div>
									...
									<p className="text-3xl font-bold text-green-600">
										₹{(statistics.totalAUM / 10000000).toFixed(0)} Cr
									</p>
								</div>
						...
						<div className="flex flex-wrap gap-4 mb-6">
							...
							<Select value={selectedStyle} onValueChange={setSelectedStyle}>
								...
								<SelectContent>
									...
									<SelectItem value="non_discretionary">
										Non-Discretionary
									</SelectItem>
						...
						<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
							{displayData.map((scheme: any) => (
								<Card
									...
									data-testid={`pms-scheme-${scheme.id}`}
								>
									<CardHeader>
										<div className="flex justify-between items-start">
											<CardTitle className="text-lg">{scheme.name}</CardTitle>
											<Badge variant="outline">
												{scheme.strategy || "Multicap"}
											</Badge>
										</div>
					...
					<TabsContent value="sip">
						<Card className="border-dashed border-2 border-purple-200 dark:border-purple-800 bg-purple-50 dark:bg-purple-950/30/50">
							<CardContent className="flex flex-col items-center justify-center py-16">
								...
								<p className="text-muted-foreground text-center max-w-md">
									Systematic Investment Plans for PMS will be available soon.
