...
function TrustedDevicesCard() {
	...
	const revokeMutation = useMutation({
		mutationFn: async (deviceId: string) => {
			return await apiRequest(`/api/user/trusted-devices/${deviceId}`, {
				method: "DELETE",
			});
		},
		...
		onSuccess: () => {
			toast({
				...
				description: "The device can no longer skip OTP.",
			});
	...
	function parseDevice(ua: string | null): {
		...
		if (!ua)
			return { icon: <Laptop className="h-5 w-5" />, label: "Unknown Device" };
			...
			<CardHeader>
				<div className="flex items-center gap-2">
					...
					<div>
						...
						<CardDescription>
							Devices where you can log in with your PIN instead of OTP
						</CardDescription>
					</div>
				</div>
			</CardHeader>
			<CardContent>
				...
				{!isLoading && !isError && devices.length === 0 && (
					<div className="text-sm text-muted-foreground py-4 flex items-center gap-2">
						...
						No trusted devices yet. Log in and set a PIN to register your first
						device.
					</div>
				)}
				...
				{!isLoading && devices.length > 0 && (
					<div className="divide-y">
						{devices.map((device) => {
							...
							const isThisDevice =
								localStorage.getItem("pinDeviceToken") !== null &&
										...
										<div className="space-y-0.5">
											<div className="flex items-center gap-2">
												...
												{isThisDevice && (
													<Badge
														variant="secondary"
														className="text-xs h-5 bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-950 dark:text-emerald-300"
													>
														This device
													</Badge>
												)}
											...
											<div className="flex flex-wrap gap-x-3 gap-y-0.5 text-xs text-muted-foreground">
												...
												<span>
													Trusted{" "}
													{formatDistanceToNow(new Date(device.trustedAt), {
														addSuffix: true,
													})}
												</span>
												<span>
...
function PinManagementCard() {
	...
	const pinMutation = useMutation({
		mutationFn: async (pin: string) => {
			return await apiRequest("/api/login/set-pin", {
				method: "POST",
		...
		onSuccess: () => {
			toast({
				...
				description: isPinSet
					...
					: "4-digit login PIN set successfully. It will be used on trusted devices.",
			});
		...
		onError: (err: any) => {
			setPinError(
				err?.message || "Failed to update PIN. Ensure your mobile is verified.",
			);
		},
	});
	...
	const handleSubmit = () => {
		...
		if (pin.length !== 4) {
			setPinError("Enter all 4 digits for your new PIN.");
			return;
		}
		if (pin !== confirm) {
			setPinError("PINs don't match. Please re-enter.");
			setConfirmPin(["", "", "", ""]);
			confirmRefs[0].current?.focus();
			return;
		}
		if (mode === "change" && currentPin.join("").length !== 4) {
			setPinError("Enter your current 4-digit PIN first.");
			return;
		}
				...
				<div className="flex items-center justify-between">
					<div className="flex items-center gap-2">
						...
						<div>
							...
							<CardDescription>
								4-digit PIN for fast sign-in on trusted devices
							</CardDescription>
						</div>
					</div>
					{isPinSet && mode === "idle" && (
						<Badge
							variant="secondary"
							className="bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-950 dark:text-emerald-300 gap-1"
						>
						...
						<div className="flex flex-wrap gap-3">
							{!isPinSet && (
								<Button
									onClick={() => {
										setMode("set");
										setTimeout(() => newRefs[0].current?.focus(), 100);
									}}
							...
							{isPinSet && (
								<Button
									...
									onClick={() => {
										setMode("change");
										setTimeout(() => currentRefs[0].current?.focus(), 100);
									}}
		...
		return (
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Admin users have a fixed left sidebar navigation for optimal
						workflow management.
					</CardDescription>
				</CardHeader>
				<CardContent>
	...
	const handlePositionChange = (position: NavPosition) => {
		if (isMobile && position === "left") {
			toast({
				...
				description:
					"Left sidebar is not available on mobile. Switched to bottom navigation.",
			});
					...
					{positions.map((pos) => {
						...
						const button = (
							...
							>
								...
								<div
									className={`p-4 rounded-full ${isActive ? "bg-primary/10" : "bg-muted"}`}
								>
									<pos.icon
				...
				<div className="pt-4 border-t">
					<div className="flex items-center justify-between">
						<div>
							...
							<p className="text-sm text-muted-foreground">
								Your navigation is displayed on the{" "}
								{navPosition === "left"
									? "left side"
									: navPosition === "top"
										? "top"
...
function DisplayPreferencesCard() {
	...
	const handleToggle = (
		key: "reduceMotion" | "highContrast" | "compactMode",
		label: string,
	) => {
	...
	return (
		<Card>
			<CardHeader>
				...
				<CardDescription>
					Additional display settings for your experience
				</CardDescription>
			</CardHeader>
			<CardContent className="space-y-4">
				<div className="flex items-center justify-between">
					...
					<Switch
						...
						onCheckedChange={() =>
							handleToggle("reduceMotion", "Reduce Motion")
						}
				...
				<div className="flex items-center justify-between">
					...
					<Switch
						...
						onCheckedChange={() =>
							handleToggle("highContrast", "High Contrast")
						}
		...
		<Card className="border-dashed">
			<CardContent className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pt-6">
				<div>
					...
					<p className="text-sm text-muted-foreground">
						Restore theme, navigation position, and display preferences to their
						original defaults
					</p>
				</div>
				<Button
		...
		<div className="container mx-auto py-6 px-4 max-w-7xl">
			<div className="mb-6">
				...
				<p className="text-muted-foreground">
					Manage your account settings, notifications, security, and connected
					services
				</p>
			</div>
							...
							<div className="space-y-4 pt-6 border-t">
								...
								<div className="flex items-center justify-between">
									<div>
										...
										<p className="text-sm text-muted-foreground">
											Holdings and performance changes
										</p>
									</div>
									<Switch
										checked={portfolioUpdates}
										onCheckedChange={setPortfolioUpdates}
										data-testid="switch-portfolio"
									/>
								</div>
									...
									<FormField
										...
										render={({ field }) => (
											<FormItem>
												...
												<FormDescription>
													At least 8 characters, must differ from current
												</FormDescription>
												<FormMessage />
											</FormItem>
										)}
						...
						<CardContent>
							...
							{twoFactorEnabled && (
								<div className="mt-4 p-4 bg-muted rounded-lg">
									<p className="text-sm">
										Setup instructions will be sent to your registered email
									</p>
								</div>
							)}
						</CardContent>
				...
				<TabsContent value="appearance" className="space-y-6">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Choose your preferred appearance theme. Select Light for a
								bright interface, Dark for reduced eye strain, or System to
						...
						<CardContent className="space-y-6">
							<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
								...
								>
									...
									<div
										className={`p-4 rounded-full ${theme === "light" ? "bg-primary/10" : "bg-muted"}`}
									>
										<Sun
								...
								>
									...
									<div
										className={`p-4 rounded-full ${theme === "dark" ? "bg-primary/10" : "bg-muted"}`}
									>
										<Moon
								...
								>
									...
									<div
										className={`p-4 rounded-full ${theme === "system" ? "bg-primary/10" : "bg-muted"}`}
									>
										<Monitor
							...
							<div className="pt-4 border-t">
								<div className="flex items-center justify-between">
									<div>
										...
										<p className="text-sm text-muted-foreground">
											Your interface is currently displaying in {resolvedTheme}{" "}
											mode
