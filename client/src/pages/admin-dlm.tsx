...
export default function AdminDLMPage() {
	...
	const {
		...
		queryFn: async () => {
			...
			const res = await fetch(`/api/dlm/documents?${params.toString()}`, {
				credentials: "include",
			});
	...
	const transitionMutation = useMutation({
		mutationFn: async ({
			...
			action,
			reason,
		}: { documentId: string; action: string; reason?: string }) => {
	...
	return (
		<div className="container mx-auto p-6 space-y-6">
			<div className="flex justify-between items-center">
				<div>
					<h1 className="text-3xl font-bold" data-testid="heading-dlm-title">
						Document Lifecycle Management
					</h1>
					<p className="text-muted-foreground">
						SEBI-compliant document governance and compliance system
					</p>
				</div>
				<Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
					...
					<DialogContent className="max-w-2xl">
						<DialogHeader>
							...
							<DialogDescription>
								Add a new agreement or document to the system
							</DialogDescription>
						</DialogHeader>
						<Form {...form}>
							<form
									...
									render={({ field }) => (
										<FormItem>
											...
											<FormControl>
												<Input
													{...field}
													placeholder="e.g., Partnership Agreement - ABC Ltd"
				...
				<Card>
					<CardContent className="p-4">
						<div className="flex items-center gap-2">
							...
							<div>
								<p className="text-2xl font-bold">
									{stats?.statusCounts?.negotiation || 0}
								</p>
											...
											return (
												...
												>
													<div className="flex items-center gap-4">
														<div
															className={`p-2 rounded-lg ${STATUS_COLORS[doc.status]}`}
														>
							...
							{stats?.highRiskDocuments?.length > 0 ? (
								<div className="space-y-3">
									{stats.highRiskDocuments.map((doc: any) => (
										<div
											key={doc.id}
											className="flex items-center justify-between p-4 border rounded-lg border-red-200 dark:border-red-800"
										>
											<div>
												<h4 className="font-medium">{doc.title}</h4>
												<p className="text-sm text-muted-foreground">
						...
						<div className="space-y-4">
							<div className="grid grid-cols-2 gap-4">
								<div>
									...
									<p className="font-medium">
										{selectedDocument.entityName || selectedDocument.entityType}
									</p>
								</div>
								<div>
									<p className="text-sm text-muted-foreground">
									...
									<p className="font-medium">
										{selectedDocument.agreementType?.replace(/_/g, " ")}
									</p>
								</div>
								<div>
								...
								<div>
									...
									<p className="font-medium">
										{selectedDocument.riskScore || 0}/100
									</p>
								</div>
								<div>
									<p className="text-sm text-muted-foreground">
