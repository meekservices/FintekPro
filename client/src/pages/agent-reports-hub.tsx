...
const REPORT_TEMPLATES: ReportTemplate[] = [
	{
		...
		description:
			"Complete overview of client portfolio with holdings, allocation, and performance",
		category: "portfolio",
		icon: Briefcase,
	...
	},
	{
		...
		description:
			"Detailed returns analysis with XIRR, CAGR, and benchmark comparison",
	...
	},
	{
		...
		description:
			"Compliance status, document verification, and regulatory summary",
		category: "compliance",
		icon: CheckCircle,
		...
		<Card className="bg-card border-border">
			<CardHeader className="flex flex-row items-center justify-between pb-3">
				<div>
					...
					<CardDescription>
						PDFs saved to cloud storage from the Research Note Generator
					</CardDescription>
				</div>
...
export default function AgentReportsHub() {
	...
	const { data: generatedReportsData, isLoading: reportsLoading } = useQuery<
		GeneratedReport[]
	>({
				...
				<div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
					<div>
						...
						<p className="text-muted-foreground mt-1">
							Generate and share client reports
						</p>
					</div>
						...
						<CardContent className="p-4">
							<div className="flex justify-between items-start">
								<div>
									...
									<p className="text-2xl font-bold text-foreground">
										{generatedReports.filter((r) => r.status === "sent").length}
									</p>
								</div>
						...
						<CardContent className="p-4">
							<div className="flex justify-between items-start">
								<div>
									...
									<p className="text-2xl font-bold text-foreground">
										{
											generatedReports.filter((r) => r.status === "ready")
												.length
										}
									</p>
								</div>
						...
						<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
							{filteredTemplates.map((template) => {
								...
								return (
									<Card
										key={template.id}
										className="bg-card/50 border-border hover:border-emerald-500/50 transition-colors cursor-pointer"
											...
											<div className="flex items-start justify-between">
												...
												<Badge className={getCategoryColor(template.category)}>
													{(template.category || "report")
														.charAt(0)
														.toUpperCase() +
														(template.category || "report").slice(1)}
												</Badge>
											</div>
						...
						<div className="space-y-4 mt-4">
							<div>
								...
								<Select
									...
									onValueChange={(value) =>
										setReportConfig({ ...reportConfig, clientId: value })
									}
								>
									<SelectTrigger
										className="mt-1 bg-card border-border"
										data-testid="select-report-client"
									>
							...
							<div>
								...
								>
									...
									<SelectContent className="bg-card border-border">
										...
										<SelectItem value="fy_2024">
											FY 2024 (Apr 24 - Mar 25)
										</SelectItem>
										<SelectItem value="fy_2023">
											FY 2023 (Apr 23 - Mar 24)
										</SelectItem>
