...
export default function AgentBulkCommunication() {
  ...
  const filteredClients = useMemo(() => {
    ...
    return clients.filter(client => {
      if (recipientFilter.kycStatus && client.kycStatus !== recipientFilter.kycStatus) {
        return false;
      }
      if (recipientFilter.minInvestmentValue && client.investmentValue < recipientFilter.minInvestmentValue) {
        return false;
      }
      if (recipientFilter.maxInvestmentValue && client.investmentValue > recipientFilter.maxInvestmentValue) {
        return false;
      }
      if (recipientFilter.lastInteractionDays && client.lastInteraction) {
        const daysSinceInteraction = Math.floor((Date.now() - new Date(client.lastInteraction).getTime()) / (1000 * 60 * 60 * 24));
        if (daysSinceInteraction > recipientFilter.lastInteractionDays) {
          return false;
        }
      }
      if (recipientFilter.tags && recipientFilter.tags.length > 0) {
        if (!client.tags || !recipientFilter.tags.some(tag => client.tags?.includes(tag))) {
          return false;
        }
      }
      return true;
  ...
  const createCampaignMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ 
        ...
        description: scheduleType === 'now' ? 'Your campaign is being sent.' : 'Your campaign has been scheduled.'
      });
    },
    onError: () => {
      toast({ 
  ...
  const syncAnalyticsMutation = useMutation({
    mutationFn: async (campaignId: string) => {
      return apiRequest(`/api/agent/campaigns/${campaignId}/sync-analytics`, 'POST');
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/agent/campaigns'] });
  ...
  const handleTemplateSelect = (templateName: string) => {
    const template = whatsappTemplates?.find(t => t.name === templateName) || null;
  ...
  const handleSubmitCampaign = () => {
    ...
    const payload: any = {
      name: campaignName,
      recipients,
      scheduledAt: scheduleType === 'schedule' ? `${scheduledDate}T${scheduledTime}` : undefined
    };
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex justify-between items-center">
        ...
        <Dialog open={isWizardOpen} onOpenChange={(open) => { if (!open) resetWizard(); else setIsWizardOpen(true); }}>
          <DialogTrigger asChild>
            <Button className="bg-emerald-600 hover:bg-emerald-700" data-testid="button-create-campaign">
            ...
            <DialogHeader>
              ...
              <DialogDescription className="text-muted-foreground">
                Step {wizardStep} of 5 - {
                  ...
                  wizardStep === 3 ? 'Message Composer' :
                  wizardStep === 4 ? 'Schedule' :
                  'Preview & Confirm'
                }
              </DialogDescription>
            </DialogHeader>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      {[
                        ...
                        { id: 'email', label: 'Email', icon: Mail, description: 'Rich email campaigns' },
                        { id: 'whatsapp', label: 'WhatsApp', icon: MessageSquare, description: 'Template-based messages' }
                      ].map(channel => (
                        <div
                          key={channel.id}
                          onClick={() => setSelectedChannel(channel.id as 'sms' | 'email' | 'whatsapp')}
              ...
              {wizardStep < 5 ? (
                <Button
                  ...
                  disabled={
                    ...
                    (wizardStep === 3 && selectedChannel === 'whatsapp' && !selectedTemplate) ||
                    (wizardStep === 3 && selectedChannel !== 'whatsapp' && !messageContent) ||
                    (wizardStep === 4 && scheduleType === 'schedule' && (!scheduledDate || !scheduledTime))
                  }
      ...
      <Tabs defaultValue="dashboard" className="space-y-6">
        <TabsList className="bg-card border-border">
          ...
          <TabsTrigger value="campaigns" className="data-[state=active]:bg-emerald-600" data-testid="tab-campaigns">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-2">
                            <span className={`p-1.5 rounded ${
                              campaign.channel === 'sms' ? 'bg-emerald-500/20 text-emerald-400' :
                              campaign.channel === 'email' ? 'bg-blue-500/20 text-blue-400' :
