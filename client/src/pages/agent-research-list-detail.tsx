...
export default function AgentResearchListDetail() {
	...
	const { data: searchResults } = useQuery<{
		...
		queryKey: [
			...
			{ universe: data?.list?.universeType, query: instrumentSearch },
		],
	...
	const removeItemMutation = useMutation({
		...
		onError: () => {
			toast({
				title: "Error",
				description: "Failed to remove instrument",
				variant: "destructive",
			});
		},
	});
											...
											>
												...
												{activeTab === "basics" && (
													<>
														<TableCell className="text-right text-muted-foreground">
															₹
															{(metrics.nav || metrics.currentPrice)?.toFixed(
																2,
															) || "—"}
														</TableCell>
														<TableCell>
												...
												{activeTab === "risk" && (
													<>
														...
														<TableCell className="text-right text-muted-foreground">
															{metrics.aum
																? (metrics.aum / 10000000).toFixed(0)
																: "—"}
														</TableCell>
													</>
												)}
