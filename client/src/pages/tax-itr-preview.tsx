...
export default function TaxITRPreviewPage() {
	...
	const exportMutation = useMutation({
		...
		onSuccess: (blob) => {
			...
			toast({
				title: "Success",
				description: "Tax computation PDF exported successfully.",
			});
		},
		onError: () => {
			toast({
				title: "Error",
				description: "Failed to export computation PDF.",
				variant: "destructive",
			});
		},
	});
	...
	const validationErrors: ValidationError[] = [
		...(draft?.tdsCredits &&
		draft.tdsCredits > 0 &&
		!draft.salaryDetails?.grossSalary
			? [
					{
						field: "TDS",
						message: "TDS credits claimed but no income declared",
						severity: "warning" as const,
					},
				]
			: []),
		...(draft?.deductionDetails?.section80C &&
		draft.deductionDetails.section80C > 150000
			? [
					{
						field: "80C",
						message: "Section 80C deduction exceeds limit of ₹1,50,000",
						severity: "error" as const,
					},
				]
			: []),
		...(draft?.deductionDetails?.section80TTA &&
		draft.deductionDetails.section80TTA > 10000
			? [
					{
						field: "80TTA",
						message: "Section 80TTA deduction exceeds limit of ₹10,000",
						severity: "error" as const,
					},
				]
			: []),
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
				<Card>
					<CardContent className="p-4 text-center">
						...
						<p className="text-2xl font-bold text-primary">
							{formatCurrency(draft?.grossTotalIncome)}
						</p>
					</CardContent>
				</Card>
				<Card>
					<CardContent className="p-4 text-center">
						...
						<p className="text-2xl font-bold text-green-600">
							{formatCurrency(draft?.totalDeductions)}
						</p>
					</CardContent>
				</Card>
				<Card>
					<CardContent className="p-4 text-center">
						...
						<p className="text-2xl font-bold">
							{formatCurrency(draft?.taxableIncome)}
						</p>
					</CardContent>
				</Card>
				<Card
					className={
						draft?.refundDue && draft.refundDue > 0
				...
				>
					<CardContent className="p-4 text-center">
						...
						<p
							className={`text-2xl font-bold ${draft?.refundDue && draft.refundDue > 0 ? "text-green-600" : "text-red-600"}`}
						>
							{formatCurrency(
								draft?.refundDue && draft.refundDue > 0
									? draft.refundDue
					...
					<div className="space-y-3">
						<div className="flex justify-between py-2 border-b">
							...
							<span className="font-medium">
								{formatCurrency(
									(draft?.salaryDetails?.grossSalary || 0) -
										(draft?.salaryDetails?.standardDeduction || 0) -
										(draft?.salaryDetails?.professionalTax || 0),
								)}
							</span>
						</div>
						{draft?.housePropertyDetails && (
							<div className="flex justify-between py-2 border-b">
								...
								<span className="font-medium">
									{formatCurrency(
										(draft.housePropertyDetails.rentalIncome || 0) -
											(draft.housePropertyDetails.municipalTaxes || 0) -
											(draft.housePropertyDetails.interestOnLoan || 0),
									)}
								</span>
							</div>
						)}
						{draft?.capitalGainsDetails && (
							<div className="flex justify-between py-2 border-b">
								...
								<span className="font-medium">
									{formatCurrency(
										(draft.capitalGainsDetails.shortTermGains || 0) +
											(draft.capitalGainsDetails.longTermGains || 0) -
											(draft.capitalGainsDetails.exemptionsApplied || 0),
									)}
								</span>
							</div>
						)}
						{draft?.otherIncomeDetails && (
							<div className="flex justify-between py-2 border-b">
								...
								<span className="font-medium">
									{formatCurrency(
										(draft.otherIncomeDetails.interestIncome || 0) +
											(draft.otherIncomeDetails.dividendIncome || 0) +
											(draft.otherIncomeDetails.otherSources || 0),
									)}
								</span>
							</div>
						)}
						...
						<div className="flex justify-between py-2 border-b">
							...
							<span className="font-medium">
								{formatCurrency(draft?.taxPayable)}
							</span>
						</div>
						<div className="flex justify-between py-2 border-b">
							<span className="text-muted-foreground">Less: TDS Credits</span>
							<span className="font-medium text-green-600">
								-{formatCurrency(draft?.tdsCredits)}
							</span>
						</div>
						<div className="flex justify-between py-3 bg-muted px-3 rounded-lg font-bold">
							...
							>
								{formatCurrency(
									draft?.refundDue && draft.refundDue > 0
										? draft.refundDue
										: (draft?.taxPayable || 0) - (draft?.tdsCredits || 0),
								)}
							</span>
						</div>
					</div>
					...
					<CardContent>
						<div className="grid grid-cols-2 gap-4 text-sm">
							<div className="flex justify-between">
								...
								<span className="dark:text-foreground">
									{formatCurrency(draft.salaryDetails.grossSalary)}
								</span>
							</div>
							<div className="flex justify-between">
								<span className="text-muted-foreground">Allowances</span>
								<span className="dark:text-foreground">
									{formatCurrency(draft.salaryDetails.allowances)}
								</span>
							</div>
							<div className="flex justify-between">
								<span className="text-muted-foreground">
								...
								<span className="text-green-600">
									-{formatCurrency(draft.salaryDetails.standardDeduction)}
								</span>
							</div>
							<div className="flex justify-between">
								<span className="text-muted-foreground">Professional Tax</span>
								<span className="text-green-600">
									-{formatCurrency(draft.salaryDetails.professionalTax)}
								</span>
							</div>
						</div>
					</CardContent>
					...
					<CardContent>
						<div className="grid grid-cols-2 gap-4 text-sm">
							<div className="flex justify-between">
								...
								<span className="dark:text-foreground">
									{formatCurrency(draft.otherIncomeDetails.interestIncome)}
								</span>
							</div>
							<div className="flex justify-between">
								<span className="text-muted-foreground">Dividend Income</span>
								<span className="dark:text-foreground">
									{formatCurrency(draft.otherIncomeDetails.dividendIncome)}
								</span>
							</div>
						</div>
					</CardContent>
		...
		<Card>
			...
			<CardContent>
				<div className="space-y-3 text-sm">
					{draft?.deductionDetails &&
						Object.entries(draft.deductionDetails).map(([key, value]) => {
			...
			{hasErrors && (
				<Alert className="bg-red-50 dark:bg-red-950 border-red-200 dark:border-red-800">
					...
					<AlertDescription>
						There are{" "}
						{validationErrors.filter((e) => e.severity === "error").length}{" "}
						error(s) that must be fixed before filing.
					</AlertDescription>
				</Alert>
			)}
			...
			<Card className="border-primary">
				<CardContent className="p-4">
					<div className="flex items-start gap-3">
						...
						>
							I have reviewed all the details in this return and confirm that
							the information provided is true, correct, and complete to the
							best of my knowledge. I understand that filing incorrect
							information may attract penalties under the Income Tax Act.
						</Label>
					</div>
				</CardContent>
			</Card>
