...
function getStatusBadge(status: string | null) {
	switch (status) {
		...
		case "existing_only":
			return (
				<Badge className="bg-blue-500 text-white">Existing Investors</Badge>
			);
		default:
...
function OtherFundsByManager({
	...
	const { data, isLoading } = useQuery<{ otherFunds: OtherFund[] }>({
		queryKey: [
			"/api/store/fund-managers",
			managerId,
			"other-funds",
			{ excludeFundId: currentFundId, fundType },
		],
		enabled: !!managerId,
	});
	...
	return (
		<Card>
			<CardHeader>
				...
				<CardDescription>
					Explore other investment strategies managed by the same fund manager
				</CardDescription>
			</CardHeader>
			<CardContent>
					...
					{otherFunds.map((fund) => (
						...
						>
							<div className="flex items-start justify-between mb-2">
								...
								<span
									className={`text-xs px-2 py-0.5 rounded ${fund.fundType === "pms" ? "bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300" : "bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300"}`}
								>
									{fund.fundType.toUpperCase()}
								</span>
							</div>
							<p className="text-xs text-muted-foreground mb-3">
								{fund.strategy || fund.category || "N/A"}
							</p>
							<div className="flex justify-between text-xs">
								<div>
									<span className="text-muted-foreground">1Y Return</span>
									<p
										className={`font-semibold ${fund.return1Y && Number.parseFloat(fund.return1Y) >= 0 ? "text-green-600" : "text-red-600"}`}
									>
...
export default function AIFDetail() {
	const [, navigate] = useLocation();
	const [match, params] = useRoute("/aif/:id");
	const id = params?.id;

	const { data: scheme, isLoading: schemeLoading } = useQuery<AifScheme>({
		queryKey: ["/api/store/aif", id],
		enabled: !!id,
	});
	...
	const chartData = monthlyPerformance
		.map((p) => ({
			period: `${monthNames[p.month - 1]} ${p.year}`,
			return: Number.parseFloat(p.returnPercent) || 0,
			benchmark: p.benchmarkReturn
				? Number.parseFloat(p.benchmarkReturn)
				: null,
		}))
		.reverse();
		...
		return (
			<div className="container mx-auto px-4 py-8">
				<Card className="p-8 text-center">
					...
					<h3 className="text-lg font-semibold">AIF Scheme Not Found</h3>
					<p className="text-muted-foreground mb-4">
						The requested scheme could not be found.
					</p>
					<Button onClick={() => navigate("/aif")}>Back to AIF List</Button>
				</Card>
			</div>
		);
	}
	...
	return (
		<div className="container mx-auto px-4 py-8">
			...
			>
				<ArrowLeft className="w-4 h-4 mr-2" /> Back to AIF List
			</Button>

			<div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-6">
				<div>
					<h1 className="text-3xl font-bold mb-2" data-testid="scheme-name">
						{scheme.name}
					</h1>
					<div className="flex items-center gap-3 text-muted-foreground">
						<Building2 className="w-5 h-5" />
						<span>{scheme.fundHouseName || "Unknown Fund House"}</span>
						{scheme.registrationNo && (
							<span className="text-sm text-muted-foreground">
								| Reg: {scheme.registrationNo}
							</span>
						)}
					</div>
				</div>
				<div className="flex flex-wrap gap-2">
					...
					{scheme.subcategory && (
						<Badge variant="outline">{scheme.subcategory}</Badge>
					)}
			...
			<div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
				<Card>
					<CardContent className="p-4">
						<p className="text-sm text-muted-foreground flex items-center gap-1">
							<TrendingUp className="w-4 h-4" /> 1Y Return
						</p>
						<p
							className={`text-2xl font-bold ${getReturnColor(scheme.return1Y)}`}
							data-testid="return-1y"
						>
				...
				<Card>
					<CardContent className="p-4">
						<p className="text-sm text-muted-foreground flex items-center gap-1">
							<TrendingUp className="w-4 h-4" /> 3Y Return
						</p>
						<p
							className={`text-2xl font-bold ${getReturnColor(scheme.return3Y)}`}
							data-testid="return-3y"
						>
				...
				<Card>
					<CardContent className="p-4">
						<p className="text-sm text-muted-foreground flex items-center gap-1">
							<PieChart className="w-4 h-4" /> AUM
						</p>
						<p className="text-2xl font-bold text-indigo-600" data-testid="aum">
							{formatCurrency(scheme.aum)}
						</p>
					</CardContent>
				</Card>
				<Card>
					<CardContent className="p-4">
						<p className="text-sm text-muted-foreground flex items-center gap-1">
							<LucideShield className="w-4 h-4" /> Risk
						</p>
						...
						<Card>
							...
							<CardContent className="space-y-3">
								...
								<div className="flex justify-between py-2 border-b">
									...
									<span className="font-medium">
										{scheme.liquidityFrequency || "N/A"}
									</span>
								</div>
								<div className="flex justify-between py-2 border-b">
									<span className="text-muted-foreground">Benchmark</span>
									<span className="font-medium">
										{scheme.benchmark || "N/A"}
									</span>
								</div>
								<div className="flex justify-between py-2">
									<span className="text-muted-foreground">Inception Date</span>
									<span className="font-medium">
										{formatDate(scheme.inceptionDate)}
									</span>
								</div>
							</CardContent>
						</Card>
						...
						<Card>
							...
							<CardContent className="space-y-3">
								<div className="flex justify-between py-2 border-b">
									...
									<span
										className={`font-medium ${getReturnColor(scheme.return1M)}`}
									>
										{formatPercent(scheme.return1M)}
									</span>
								</div>
								<div className="flex justify-between py-2 border-b">
									<span className="text-muted-foreground">3 Months</span>
									<span
										className={`font-medium ${getReturnColor(scheme.return3M)}`}
									>
										{formatPercent(scheme.return3M)}
									</span>
								</div>
								<div className="flex justify-between py-2 border-b">
									<span className="text-muted-foreground">6 Months</span>
									<span
										className={`font-medium ${getReturnColor(scheme.return6M)}`}
									>
										{formatPercent(scheme.return6M)}
									</span>
								</div>
								<div className="flex justify-between py-2 border-b">
									<span className="text-muted-foreground">1 Year</span>
									<span
										className={`font-medium ${getReturnColor(scheme.return1Y)}`}
									>
										{formatPercent(scheme.return1Y)}
									</span>
								</div>
								<div className="flex justify-between py-2 border-b">
									...
									<span
										className={`font-medium ${getReturnColor(scheme.return3Y)}`}
									>
										{formatPercent(scheme.return3Y)}
									</span>
								</div>
								<div className="flex justify-between py-2">
									...
									<span
										className={`font-medium ${getReturnColor(scheme.returnSI)}`}
									>
										{formatPercent(scheme.returnSI)}
									</span>
								</div>
							</CardContent>
						</Card>
				...
				<TabsContent value="performance" className="space-y-6">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Monthly returns vs benchmark over the past 3 years
							</CardDescription>
						</CardHeader>
						<CardContent>
							{chartData.length > 0 ? (
					...
					{rolling && (
						<Card>
							<CardHeader>
								...
								<CardDescription>
									As of {formatDate(rolling.asOfDate)}
								</CardDescription>
							</CardHeader>
							<CardContent>
								<div className="grid grid-cols-2 md:grid-cols-5 gap-4">
									<div className="text-center p-4 bg-muted rounded-lg">
										...
										<p
											className={`text-xl font-bold ${getReturnColor(rolling.roll1Y)}`}
										>
											{formatPercent(rolling.roll1Y)}
										</p>
									</div>
									<div className="text-center p-4 bg-muted rounded-lg">
										<p className="text-sm text-muted-foreground">3Y Rolling</p>
										<p
											className={`text-xl font-bold ${getReturnColor(rolling.roll3Y)}`}
										>
											{formatPercent(rolling.roll3Y)}
										</p>
									</div>
									<div className="text-center p-4 bg-muted rounded-lg">
										<p className="text-sm text-muted-foreground">5Y Rolling</p>
										<p
											className={`text-xl font-bold ${getReturnColor(rolling.roll5Y)}`}
										>
											{formatPercent(rolling.roll5Y)}
										</p>
									</div>
									<div className="text-center p-4 bg-green-50 dark:bg-green-950/30 rounded-lg">
										<p className="text-sm text-muted-foreground">Best 1Y</p>
										<p className="text-xl font-bold text-green-600">
											{formatPercent(rolling.bestRoll1Y)}
										</p>
									</div>
									<div className="text-center p-4 bg-red-50 dark:bg-red-950/30 rounded-lg">
										<p className="text-sm text-muted-foreground">Worst 1Y</p>
										<p className="text-xl font-bold text-red-600">
											{formatPercent(rolling.worstRoll1Y)}
										</p>
									</div>
								</div>
							</CardContent>
						...
						<Card>
							...
							<CardContent className="space-y-4">
								<div className="flex justify-between items-center py-2 border-b">
									<div>
										<span className="font-medium">Volatility (Std Dev)</span>
									...
									<span className="font-bold text-lg">
										{scheme.volatility
											? `${Number.parseFloat(scheme.volatility).toFixed(2)}%`
											: "N/A"}
									</span>
								</div>
								<div className="flex justify-between items-center py-2 border-b">
									...
									<span className="font-bold text-lg text-red-600">
										{scheme.maxDrawdown
											? `${Number.parseFloat(scheme.maxDrawdown).toFixed(2)}%`
											: "N/A"}
									</span>
								</div>
								<div className="flex justify-between items-center py-2 border-b">
									...
									<span className="font-bold text-lg">
										{scheme.sharpeRatio
											? Number.parseFloat(scheme.sharpeRatio).toFixed(2)
											: "N/A"}
									</span>
								</div>
								<div className="flex justify-between items-center py-2 border-b">
									...
									<span className="font-bold text-lg">
										{scheme.sortinoRatio
											? Number.parseFloat(scheme.sortinoRatio).toFixed(2)
											: "N/A"}
									</span>
								</div>
						...
						<Card>
							...
							<CardContent className="space-y-4">
								<div className="flex justify-between items-center py-2 border-b">
									...
									<span
										className={`font-bold text-lg ${scheme.alpha && Number.parseFloat(scheme.alpha) >= 0 ? "text-green-600" : "text-red-600"}`}
									>
								...
								<div className="flex justify-between items-center py-2 border-b">
									...
									<span className="font-bold text-lg">
										{scheme.beta
											? Number.parseFloat(scheme.beta).toFixed(2)
											: "N/A"}
									</span>
								</div>
								<div className="flex justify-between items-center py-2">
									...
									<span className="font-bold text-lg">
										{scheme.informationRatio
											? Number.parseFloat(scheme.informationRatio).toFixed(2)
											: "N/A"}
									</span>
								</div>
							</CardContent>
					...
					{scheme.manager ? (
								...
								<CardContent>
									<div className="flex flex-col md:flex-row gap-6">
										<div className="flex-1">
											...
											<p className="text-muted-foreground mb-4">
												{scheme.manager.designation || "Fund Manager"}
											</p>
											{scheme.manager.bio && (
												...
												<div className="p-3 bg-muted rounded-lg">
													...
													<p className="font-semibold">
														{scheme.manager.experienceYears
															? `${scheme.manager.experienceYears}+ years`
															: "N/A"}
													</p>
												</div>
												<div className="p-3 bg-muted rounded-lg">
													...
													<p className="font-semibold">
														{scheme.manager.fundsManaged || "N/A"}
													</p>
												</div>
												<div className="p-3 bg-muted rounded-lg">
													<p className="text-sm text-muted-foreground">
													...
													<p className="font-semibold">
														{formatCurrency(scheme.manager.totalAumManaged)}
													</p>
												</div>
												<div className="p-3 bg-muted rounded-lg">
													<p className="text-sm text-muted-foreground">
													...
													<p
														className={`font-semibold ${scheme.manager.avgAlpha && Number.parseFloat(scheme.manager.avgAlpha) >= 0 ? "text-green-600" : "text-red-600"}`}
													>
						...
						<Card className="p-8 text-center">
							...
							<h3 className="text-lg font-semibold text-foreground">
								Fund Manager Information Not Available
							</h3>
							<p className="text-muted-foreground">
								Contact the fund house for manager details.
							</p>
						</Card>
					)}
				</TabsContent>
			...
			<div className="fixed bottom-6 right-6">
				<Button size="lg" className="shadow-lg" data-testid="invest-now">
