...
type AlertType = 'market_price' | 'market_change' | 'spending_budget' | 'portfolio_value';

export default function AlertsPage() {
  const { toast } = useToast();
  const [createAlertOpen, setCreateAlertOpen] = useState(false);
  const [selectedAlertType, setSelectedAlertType] = useState<AlertType>('market_price');
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">Manage your market and portfolio alerts</p>
        </div>
        <Dialog open={createAlertOpen} onOpenChange={setCreateAlertOpen}>
          ...
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              ...
              <DialogDescription>Set up a custom alert to monitor your investments</DialogDescription>
            </DialogHeader>
            <CreateAlertDialog 
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold" data-testid="stat-active-alerts">{activeAlerts}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold" data-testid="stat-recent-triggers">{recentTriggers}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold" data-testid="stat-total-alerts">{alerts.length}</div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
        <ScrollableTabsList>
          <TabsTrigger value="my-alerts" data-testid="tab-my-alerts">My Alerts</TabsTrigger>
          ...
          {alertsLoading ? (
            ...
            <Card>
              <CardContent className="flex flex-col items-center justify-center py-12">
                ...
                <p className="text-sm text-muted-foreground mb-4">Create your first alert to get started</p>
                <Button onClick={() => setCreateAlertOpen(true)} data-testid="button-create-first-alert">
                  <Plus className="mr-2 h-4 w-4" />
                  Create Alert
                </Button>
              </CardContent>
          ...
          <Card>
            ...
            <CardContent>
              {historyLoading ? (
                ...
                <p className="text-center py-8 text-muted-foreground">No alert history yet</p>
              ) : (
                <Table>
                  <TableHeader>
                    <TableRow>
                  ...
                  <TableBody>
                    {alertHistory.map((history: any) => (
                      <TableRow key={history.id} data-testid={`row-history-${history.id}`}>
                        <TableCell>{history.alertSnapshot?.alertName || 'Unknown'}</TableCell>
                        <TableCell>{format(new Date(history.triggeredAt), 'PPp')}</TableCell>
                        <TableCell>
                          <pre className="text-xs">{JSON.stringify(history.triggerValue, null, 2)}</pre>
                        </TableCell>
                        <TableCell>
                          <div className="flex gap-2">
                            {history.isRead && <Badge variant="outline">Read</Badge>}
                            {history.isDismissed && <Badge variant="secondary">Dismissed</Badge>}
                          </div>
                        </TableCell>
                        <TableCell>
                          <div className="flex gap-2">
                            {!history.isRead && (
                              <Button 
                                size="sm" 
                                variant="ghost"
                                onClick={() => markViewedMutation.mutate(history.id)}
                            ...
                            {!history.isDismissed && (
                              <Button 
                                size="sm" 
                                variant="ghost"
                                onClick={() => dismissAlertMutation.mutate(history.id)}
        ...
        <TabsContent value="templates">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Quick setup with popular alert templates</CardDescription>
            </CardHeader>
            <CardContent>
              {templatesLoading ? (
                <div>Loading templates...</div>
              ) : templates.length === 0 ? (
                <p className="text-center py-8 text-muted-foreground">No templates available</p>
              ) : (
                <div className="grid gap-4 md:grid-cols-2">
...
function AlertCard({ alert, onToggle, onDelete }: { alert: any; onToggle: () => void; onDelete: () => void }) {
        ...
        <div className="space-y-3">
          <div className="text-sm">
            ...
            <p className="font-medium">
              ...
              {alert.operator === 'increase' && `${alert.threshold}% increase in ${alert.timeframe}`}
              {alert.operator === 'decrease' && `${alert.threshold}% decrease in ${alert.timeframe}`}
          ...
          {alert.lastTriggeredAt && (
            <div className="text-sm">
              ...
              <p className="font-medium">{format(new Date(alert.lastTriggeredAt), 'PPp')}</p>
            </div>
          )}

          <div className="text-sm">
            <p className="text-muted-foreground">Trigger count: {alert.triggerCount || 0}</p>
          </div>
          ...
          <div className="flex gap-2 pt-2">
            ...
            >
              {alert.isActive ? <BellOff className="h-4 w-4 mr-1" /> : <Bell className="h-4 w-4 mr-1" />}
...
function CreateAlertDialog({ selectedType, onTypeChange, onClose }: { 
  ...
  const renderAlertForm = () => {
    switch (selectedType) {
      case 'market_price':
        return <MarketPriceAlertForm onSubmit={(data) => createAlertMutation.mutate({ ...data, alertType: 'market_price', category: 'market' })} />;
      case 'market_change':
        return <MarketChangeAlertForm onSubmit={(data) => createAlertMutation.mutate({ ...data, alertType: 'market_change', category: 'market' })} />;
      case 'spending_budget':
        return <SpendingBudgetAlertForm onSubmit={(data) => createAlertMutation.mutate({ ...data, alertType: 'spending_budget', category: 'spending' })} />;
      case 'portfolio_value':
        return <PortfolioValueAlertForm onSubmit={(data) => createAlertMutation.mutate({ ...data, alertType: 'portfolio_value', category: 'portfolio' })} />;
    }
  };
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <Select value={selectedType} onValueChange={(value) => onTypeChange(value as AlertType)}>
          ...
          <SelectContent>
            ...
            <SelectItem value="spending_budget">Spending Budget Alert</SelectItem>
            <SelectItem value="portfolio_value">Portfolio Value Alert</SelectItem>
          </SelectContent>
        </Select>
      </div>
      {renderAlertForm()}
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input placeholder="e.g., RELIANCE price alert" {...field} data-testid="input-alert-name" />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input placeholder="e.g., RELIANCE" {...field} data-testid="input-symbol" />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input type="number" placeholder="e.g., 2500" {...field} data-testid="input-target-value" />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <DialogFooter>
          <Button type="submit" data-testid="button-submit-alert">Create Alert</Button>
        </DialogFooter>
      </form>
    </Form>
  );
...
function MarketChangeAlertForm({ onSubmit }: { onSubmit: (data: any) => void }) {
  const form = useForm({
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>Alert Name</FormLabel>
              <FormControl>
                <Input placeholder="e.g., TCS 5% move alert" {...field} data-testid="input-alert-name" />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>Symbol</FormLabel>
              <FormControl>
                <Input placeholder="e.g., TCS" {...field} data-testid="input-symbol" />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input type="number" placeholder="e.g., 5" {...field} data-testid="input-threshold" />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
...
function SpendingBudgetAlertForm({ onSubmit }: { onSubmit: (data: any) => void }) {
  const form = useForm({
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>Alert Name</FormLabel>
              <FormControl>
                <Input placeholder="e.g., Grocery budget alert" {...field} data-testid="input-alert-name" />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input type="number" placeholder="e.g., 10000" {...field} data-testid="input-target-value" />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input type="number" placeholder="e.g., 80" {...field} data-testid="input-threshold" />
              </FormControl>
              <FormDescription>Alert when spending reaches this percentage of budget</FormDescription>
              <FormMessage />
            </FormItem>
          )}
        />
...
function PortfolioValueAlertForm({ onSubmit }: { onSubmit: (data: any) => void }) {
  const form = useForm({
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>Alert Name</FormLabel>
              <FormControl>
                <Input placeholder="e.g., Portfolio 10% gain alert" {...field} data-testid="input-alert-name" />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
