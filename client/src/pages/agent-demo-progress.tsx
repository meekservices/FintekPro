...
export default function AgentDemoProgress() {
  ...
  const convertMutation = useMutation({
    mutationFn: (proposalId: string) => apiRequest(`/api/agent/demo-proposals/${proposalId}/convert`, { method: 'POST' }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/agent/demo-proposals'] });
      queryClient.invalidateQueries({ queryKey: ['/api/agent/demo-proposals/stats'] });
  ...
  const calculatedStats: DemoStats = stats || {
    ...
    conversionRate: (demos.filter(d => d.status === 'converted' || d.demoConvertedAt).length / Math.max(demos.length, 1)) * 100,
  ...
  const getSourceBadge = (source: string) => {
    ...
    return <Badge variant="outline" className="text-green-400 border-green-500/30">Manual</Badge>;
  };
                    ...
                    filteredDemos.map((demo) => (
                      <TableRow key={demo.id} className="border-border hover:bg-card/50">
                        ...
                        <TableCell className="text-right">
                          {!demo.demoConvertedAt && demo.status !== 'converted' && (
                            <Button
                              size="sm"
