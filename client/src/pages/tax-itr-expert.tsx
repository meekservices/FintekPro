...
const REQUIRED_DOCUMENTS = [
	{
		id: "form16",
		name: "Form 16",
		desc: "TDS certificate from employer",
		forSources: ["hasSalary"],
	},
	{
		id: "form16a",
		name: "Form 16A",
		desc: "TDS on other income",
		forSources: ["hasOtherIncome"],
	},
	{
		id: "form26as",
		name: "Form 26AS",
	...
	},
	{
		id: "ais",
		name: "AIS",
		desc: "Annual Information Statement",
		forSources: ["all"],
	},
	{
		id: "bankStatements",
	...
	},
	{
		...
		desc: "From broker/demat",
		forSources: ["hasCapitalGains"],
	},
	{
		id: "rentalAgreement",
		name: "Rental Agreement",
		desc: "Property rental docs",
		forSources: ["hasHouseProperty"],
	},
	{
		id: "businessBooks",
		name: "Books of Accounts",
		desc: "P&L, Balance Sheet",
		forSources: ["hasBusinessIncome"],
	},
	{
		id: "foreignIncome",
		name: "Foreign Income Proof",
		desc: "DTAA certificates",
		forSources: ["hasForeignIncome"],
	},
	{
		id: "investments",
...
const EXPERT_PRICING: Record<
	string,
	{ base: number; priority: number; urgent: number }
> = {
...
export default function TaxITRExpertPage() {
	...
	const getRecommendedForm = (): string => {
		...
		if (incomeSources.hasCapitalGains || incomeSources.hasForeignIncome)
			return "ITR-2";
	...
	const getRequiredDocs = () => {
		return REQUIRED_DOCUMENTS.filter((doc) => {
			...
			return doc.forSources.some(
				(source) => incomeSources[source as keyof IncomeSource],
			);
		});
	};
	...
	const calculatePrice = () => {
		...
		switch (expertPrefs.urgency) {
			case "priority":
				return pricing.priority;
			case "urgent":
				return pricing.urgent;
			default:
				return pricing.base;
		}
	};
		...
		<div className="space-y-6">
			<div>
				...
				<p className="text-muted-foreground text-sm">
					This helps us determine the right ITR form and assign the appropriate
					expert.
				</p>
			</div>
				...
				{[
					{
						...
						label: "Salary Income",
						icon: Briefcase,
						desc: "Income from employment",
					},
					{
						key: "hasHouseProperty",
						label: "House Property",
						icon: Home,
						desc: "Rental income or home loan interest",
					},
					{
						key: "hasCapitalGains",
						label: "Capital Gains",
						icon: TrendingUp,
						desc: "Stocks, mutual funds, property sale",
					},
					{
						key: "hasBusinessIncome",
						label: "Business/Profession",
						icon: Building2,
						desc: "Self-employed or business income",
					},
					{
						key: "hasForeignIncome",
						label: "Foreign Income",
						icon: Receipt,
						desc: "Income from outside India",
					},
					{
						key: "hasOtherIncome",
						label: "Other Sources",
						icon: Wallet,
						desc: "Interest, dividends, etc.",
					},
				].map((source) => {
					...
					return (
						<Card
							...
							onClick={() =>
								setIncomeSources((prev) => ({
									...prev,
									[source.key]: !prev[source.key as keyof IncomeSource],
								}))
							}
						...
						>
							<CardContent className="p-4 flex items-start gap-3">
								<div
									className={`p-2 rounded-lg ${isChecked ? "bg-primary/10" : "bg-muted"}`}
								>
									<Icon
										className={`h-5 w-5 ${isChecked ? "text-primary" : "text-muted-foreground"}`}
									/>
								</div>
				...
				<CardContent className="p-4 flex items-center gap-3">
					...
					<div>
						<p className="font-medium">
							Recommended Form: {getRecommendedForm()}
						</p>
	...
	const renderCaseDetailsStep = () => (
		<div className="space-y-6">
			<div>
				...
				<p className="text-muted-foreground text-sm">
					Provide additional details to help our experts prepare your return
					accurately.
				</p>
			</div>
			...
			<div className="space-y-4">
					...
					>
						{ASSESSMENT_YEARS.map((ay) => (
							<div key={ay} className="flex items-center space-x-2">
								...
								<Label htmlFor={`ay-${ay}`} className="cursor-pointer">
									{ay}
								</Label>
							</div>
						))}
				...
				<div>
					<Label htmlFor="estimatedIncome">
						Estimated Annual Income (Optional)
					</Label>
					<div className="relative mt-1">
						...
						<Input
							...
							onChange={(e) =>
								setCaseDetails((prev) => ({
									...prev,
									estimatedIncome: e.target.value,
								}))
							}
				...
				<div>
					...
					<RadioGroup
						...
						onValueChange={(v) =>
							setCaseDetails((prev) => ({ ...prev, preferredTime: v }))
						}
		...
		return (
			<div className="space-y-6">
				<div>
					...
					<p className="text-muted-foreground text-sm">
						Upload the required documents for your return. You can also upload
						later.
					</p>
				</div>
					...
					{requiredDocs.map((doc) => {
						...
						return (
							...
							>
								<CardContent className="p-4 flex items-center gap-3">
									<div
										className={`p-2 rounded-lg ${isUploaded ? "bg-green-100 dark:bg-green-900" : "bg-muted"}`}
									>
										{isUploaded ? (
				...
				<Alert className="bg-amber-50 dark:bg-amber-950 border-amber-200 dark:border-amber-800">
					...
					<AlertDescription>
						You can upload documents now or later. Our expert will request any
						missing documents.
					</AlertDescription>
				</Alert>
			</div>
			...
			<div className="space-y-4">
					...
					>
						{[
							...
							},
							{
								value: "tax_expert",
								label: "Tax Expert",
								desc: "Certified tax professional",
								icon: FileCheck,
								badge: "Standard",
							},
							{
							...
							<Card
								...
								onClick={() =>
									setExpertPrefs((prev) => ({
										...prev,
										preferredExpertType: opt.value as any,
									}))
								}
							>
								<CardContent className="p-4 text-center">
									<div className="flex justify-center mb-2">
										<opt.icon
											className={`h-8 w-8 ${expertPrefs.preferredExpertType === opt.value ? "text-primary" : "text-muted-foreground"}`}
										/>
									</div>
				...
				<div>
					...
					<RadioGroup
						...
						onValueChange={(v) =>
							setExpertPrefs((prev) => ({ ...prev, urgency: v as any }))
						}
						className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-3"
					>
						{[
							{
								value: "normal",
								label: "Standard",
								desc: "5-7 working days",
								price: EXPERT_PRICING[getRecommendedForm()]?.base || 999,
							},
							{
								value: "priority",
								label: "Priority",
								desc: "2-3 working days",
								price: EXPERT_PRICING[getRecommendedForm()]?.priority || 1499,
							},
							{
								value: "urgent",
								label: "Urgent",
								desc: "24-48 hours",
								price: EXPERT_PRICING[getRecommendedForm()]?.urgent || 1999,
							},
						].map((opt) => (
							<Card
								key={opt.value}
								...
								onClick={() =>
									setExpertPrefs((prev) => ({
										...prev,
										urgency: opt.value as any,
									}))
								}
							>
								<CardContent className="p-4 text-center">
									<Clock
										className={`h-6 w-6 mx-auto mb-2 ${expertPrefs.urgency === opt.value ? "text-primary" : "text-muted-foreground"}`}
									/>
									<p className="font-medium">{opt.label}</p>
									<p className="text-sm text-muted-foreground">{opt.desc}</p>
		...
		return (
			<div className="space-y-6">
						...
						<CardContent className="space-y-2 text-sm">
							<div className="flex justify-between">
								...
								<span className="font-medium capitalize">
									{expertPrefs.preferredExpertType.replace("_", " ")}
								</span>
							</div>
							<div className="flex justify-between">
				...
				<Alert className="bg-green-50 dark:bg-green-950 border-green-200 dark:border-green-800">
					...
					<AlertDescription>
						Your data is secure and will only be shared with your assigned
						expert. Payment will be collected after draft review.
					</AlertDescription>
				</Alert>
				...
				<div className="flex items-start gap-2">
					...
					<Label htmlFor="terms" className="text-sm text-muted-foreground">
						I agree to the Terms of Service and authorize FintekPro to share my
						documents with the assigned expert for ITR preparation.
					</Label>
				</div>
			</div>
		);
	...
	const renderCurrentStep = () => {
		switch (currentStep) {
			case 0:
				return renderIncomeSourcesStep();
			case 1:
				return renderCaseDetailsStep();
			case 2:
				return renderDocumentsStep();
			case 3:
				return renderExpertSelectionStep();
			case 4:
	...
	const canProceed = () => {
		switch (currentStep) {
			case 0:
				return Object.values(incomeSources).some((v) => v);
			case 1:
				return true;
			case 2:
				return true;
			...
			<div className="flex items-center gap-4">
				...
				<div>
					...
					<p className="text-muted-foreground">
						Let our experts prepare and file your ITR
					</p>
				</div>
			</div>
