...
import { 
  Bell, Plus, Trash2, AlertTriangle, TrendingUp, TrendingDown, 
  Activity, DollarSign, Percent, Clock, Mail, MessageSquare, Smartphone
} from "lucide-react";
...
export default function CompoundAlerts() {
  ...
  const { data, isLoading } = useQuery<{ success: boolean; alerts: CompoundAlert[] }>({
  ...
  const addCondition = () => {
    setFormData({
      ...formData,
      conditions: [...formData.conditions, { type: "price_above", value: 0, operator: ">" }]
    });
  };
  ...
  const handleSubmit = () => {
    if (!formData.name || !formData.symbol || formData.conditions.length === 0) {
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Create smart alerts with multiple conditions for precise notifications
          </p>
        </div>
        ...
        <Card className="text-center py-12">
          <CardContent>
            ...
            <p className="text-muted-foreground mb-4">
              Create compound alerts to get notified when multiple conditions are met.
            </p>
              ...
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription className="font-mono">{alert.symbol}</CardDescription>
                  </div>
              ...
              <CardContent>
                <div className="space-y-3">
                  <div className="flex flex-wrap gap-2">
                    {alert.conditions.map((cond, i) => (
                      <Badge key={i} variant="outline" className="text-xs">
                        {CONDITION_TYPES.find(t => t.value === cond.type)?.label}: {cond.value}
                      </Badge>
                    ))}
                    <Badge variant="secondary" className="text-xs">{alert.conditionLogic}</Badge>
                  </div>
                  ...
                  <div className="flex items-center gap-3 text-sm text-muted-foreground">
                    ...
                    <span className="ml-auto">Triggered: {alert.triggeredCount}x</span>
                  </div>
                </div>
