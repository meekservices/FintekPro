...
import { 
  Settings, LucideShield as LucideShield, AlertCircle, RefreshCw, BarChart3,
...
export default function AdminCkycConfig() {
  ...
  const [editPriority, setEditPriority] = useState<{ code: string; priority: number } | null>(null);
  ...
  const toggleMutation = useMutation({
    ...
    onSuccess: (_, { code, enabled }) => {
      toast({ title: "Provider Updated", description: `${code} has been ${enabled ? 'enabled' : 'disabled'}` });
      queryClient.invalidateQueries({ queryKey: ["/api/admin/ckyc/providers"] });
    },
    onError: (error: any) => {
  ...
  const priorityMutation = useMutation({
    ...
    onSuccess: () => {
      toast({ title: "Priority Updated", description: "Provider priority has been updated" });
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <p className="text-3xl font-bold text-red-600">{providers.length - enabledCount}</p>
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <p className="text-xl font-bold">{meta?.environment || 'Unknown'}</p>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Tabs defaultValue="providers" className="space-y-4">
        ...
        <TabsContent value="providers" className="space-y-4">
          <Alert>
            ...
            <AlertDescription>
              Providers are tried in priority order (1 = highest). If the primary provider fails,
              the system automatically falls back to the next enabled provider.
            </AlertDescription>
          </Alert>

          <div className="space-y-3">
            {providers.map((provider) => {
              const isApiConfigured = configStatusMap[provider.providerCode] ?? false;
              const eligibility = provider.eligibilityRules || {};
              const features: string[] = [];
              if (eligibility.requiresApiCredentials) features.push('API-Based');
              if (eligibility.requiresAadhaarConsent) features.push('Aadhaar Consent');
              if (eligibility.requiresCkycReference) features.push('CKYC Reference');
              if (eligibility.requiresXmlUpload) features.push('XML Upload');
              if (eligibility.requiresLiveSession) features.push('Live Session');
              const riskCategories = (eligibility.allowedRiskCategories as string[]) || [];

              return (
                <Card key={provider.providerCode} className={provider.isEnabled ? 'ring-1 ring-green-200' : 'opacity-75'}>
                  <CardContent className="pt-6">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4 flex-1">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            {Boolean(eligibility.requiresApiCredentials) && (
                              isApiConfigured ? (
                                <Badge variant="outline" className="text-green-600 border-green-600">
                                  <CheckCircle className="h-3 w-3 mr-1" />API Ready
                                </Badge>
                              ) : (
                          ...
                          <p className="text-sm text-muted-foreground mt-1">{provider.providerDescription}</p>
                          <div className="flex flex-wrap gap-1.5 mt-2">
                            ...
                            {riskCategories.map((r, i) => (
                              <Badge key={`risk-${i}`} variant="outline" className="text-xs capitalize">
                                {r} risk
                              </Badge>
                            ))}
                          </div>
                      ...
                      <div className="flex items-center gap-4 ml-4">
                        <div className="flex items-center gap-2">
                          <Label htmlFor={`priority-${provider.providerCode}`} className="text-xs text-muted-foreground">Priority</Label>
                          <Input
                            ...
                            onBlur={(e) => {
                              ...
                              if (newPriority && newPriority !== provider.priority) {
                                priorityMutation.mutate({ code: provider.providerCode, priority: newPriority });
                              }
                            }}
                          />
                        </div>
                        <div className="flex items-center gap-2">
                          <Label htmlFor={`toggle-${provider.providerCode}`} className="text-xs text-muted-foreground">
        ...
        <TabsContent value="config" className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            {[
              { label: 'TruthScreen', key: 'truthscreenConfigured', vars: ['TRUTHSCREEN_USERNAME', 'TRUTHSCREEN_PASSWORD'] },
              { label: 'Sandbox.co.in', key: 'sandboxConfigured', vars: ['SANDBOX_API_KEY', 'SANDBOX_API_SECRET'] },
              { label: 'Cashfree', key: 'cashfreeConfigured', vars: ['CASHFREE_APP_ID', 'CASHFREE_SECRET_KEY'] },
              { label: 'BSE Star MFD', key: 'bseStarConfigured', vars: ['BSE_STAR_API_KEY', 'BSE_STAR_USER_ID'] },
              { label: 'KRA (KYC Registration Agency)', key: 'kraConfigured', vars: ['KRA_API_KEY'] },
              { label: 'NSDL CKYC', key: 'nsdlCkycConfigured', vars: ['CKYC_API_KEY', 'CKYC_API_SECRET'] },
              { label: 'DigiLocker', key: 'digilockerConfigured', vars: ['DIGILOCKER_CLIENT_ID'] },
            ].map((config) => {
              const isConfigured = meta?.[config.key as keyof typeof meta] as boolean || false;
              return (
                <Card key={config.key}>
