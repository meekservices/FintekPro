...
interface TaxDocument {
  id: string;
  name: string;
  type: "itr" | "computation" | "form26as" | "ais" | "tis" | "form15" | "acknowledgement" | "other";
...
export default function TaxDocumentVaultPage() {
  ...
  const filteredDocuments = documents.filter(doc => {
    ...
    const matchesYear = selectedYear === "all" || doc.assessmentYear === selectedYear;
    const matchesType = selectedType === "all" || doc.type === selectedType;
    return matchesSearch && matchesYear && matchesType;
  });

  const years = Array.from(new Set(documents.map(d => d.assessmentYear))).sort().reverse();
  const documentsByYear: Record<string, TaxDocument[]> = years.reduce((acc, year) => {
    ...
    <div className="container mx-auto p-6 space-y-6">
        ...
        <CardContent className="py-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-green-700 dark:text-green-300">All documents are encrypted and stored securely. 8-year retention as per compliance requirements.</p>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {DOCUMENT_CATEGORIES.slice(0, 4).map((category) => {
          const count = documents.filter(d => d.type === category.type).length;
          const Icon = category.icon;
          return (
              ...
              <CardContent className="pt-6">
                <div className="flex items-center justify-between">
                  ...
                  <div className={`p-3 rounded-lg ${category.color.split(" ")[1]}`}>
                    <Icon className={`h-6 w-6 ${category.color.split(" ")[0]}`} />
                  </div>
                </div>
              </CardContent>
            </Card>
      ...
      <div className="space-y-6">
        {Object.entries(documentsByYear).filter(([_, docs]) => docs.length > 0).map(([year, docs]) => (
