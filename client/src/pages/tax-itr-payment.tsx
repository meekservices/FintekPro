...
const COUPON_CODES: Record<
	string,
	{ discount: number; type: "percent" | "flat"; description: string }
> = {
	FIRST50: {
		discount: 50,
		type: "percent",
		description: "50% off on first filing",
	},
...
export default function TaxITRPaymentPage() {
	...
	return (
		<div
			...
			data-testid="page-itr-payment"
		>
			<div className="flex items-center gap-4">
				<Button
					variant="ghost"
					size="icon"
					onClick={() => navigate(`/tax/itr/preview/${draftId}`)}
				...
				<div>
					...
					<p className="text-muted-foreground">
						Complete payment to proceed with filing
					</p>
				</div>
			</div>
								...
								{PAYMENT_METHODS.map((method) => (
									...
									>
										<CardContent className="p-4 flex items-center gap-3">
											...
											<div
												className={`p-2 rounded-lg ${selectedPaymentMethod === method.id ? "bg-primary/10" : "bg-muted"}`}
											>
												<method.icon
													className={`h-5 w-5 ${selectedPaymentMethod === method.id ? "text-primary" : "text-muted-foreground"}`}
												/>
											</div>
											<div>
												<Label
							...
							<div className="flex items-start gap-2">
								...
								<div className="text-sm">
									...
									<p className="text-amber-700 dark:text-amber-300 mt-1">
										Filing fee is non-refundable once ITR is submitted to the
