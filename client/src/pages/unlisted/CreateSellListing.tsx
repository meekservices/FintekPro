...
const sellListingFormSchema = z
	...
	.refine((data) => Number(data.landingPrice) <= Number(data.askPrice), {
		message: "Landing price must be less than or equal to ask price",
		path: ["landingPrice"],
	})
	.refine((data) => Number(data.floorPrice) <= Number(data.landingPrice), {
...
export default function CreateSellListing() {
	...
	const isKycEligible =
		kycTier === "enhanced" || kycTier === "accredited_investor";
	...
	const onSubmit = (data: SellListingFormData) => {
		if (!isKycEligible) {
			toast({
				...
				description:
					"Please upgrade your KYC to Enhanced or Accredited Investor tier",
				variant: "destructive",
			});
			return;
		}
	...
	return (
		<div
			...
			data-testid="create-sell-listing"
		>
					...
					<CardContent>
						...
						{!isKycEligible && (
							<Alert className="mb-6 border-yellow-500 dark:border-yellow-700">
								...
								<AlertDescription>
									Unlisted share trading requires Enhanced or Accredited
									Investor KYC. Please upgrade your KYC to create sell listings.
								</AlertDescription>
							</Alert>
						)}
								...
								<FormField
									...
									render={({ field }) => (
										<FormItem>
											...
											<FormDescription>Must be ≤ ask price</FormDescription>
											<FormMessage />
										</FormItem>
									)}
								/>
								...
								<FormField
									...
									render={({ field }) => (
										<FormItem>
											...
											<FormDescription>Must be ≤ landing price</FormDescription>
											<FormMessage />
										</FormItem>
									)}
								/>
								...
								<FormField
									...
									render={({ field }) => (
										<FormItem>
											...
											<FormDescription>Listing expiry date</FormDescription>
											<FormMessage />
										</FormItem>
									)}
								/>
