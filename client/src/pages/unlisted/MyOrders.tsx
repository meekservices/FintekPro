...
export default function MyOrders() {
  ...
  const { data: buyRequestsData, isLoading: isLoadingBuys, refetch: refetchBuys } = useQuery<{ data: BuyRequest[] }>({
    queryKey: ['/api/unlisted/my-buy-requests'],
    enabled: !!user,
  });

  const { data: sellListingsData, isLoading: isLoadingSells, refetch: refetchSells } = useQuery<{ data: SellListing[] }>({
  ...
  const filteredBuyRequests = statusFilter === 'all' 
    ? buyRequests 
    : buyRequests.filter(r => r.status === statusFilter);

  const filteredSellListings = statusFilter === 'all'
    ? sellListings
  ...
  const getStatusBadge = (status: string) => {
    const statusConfig: Record<string, { variant: "default" | "secondary" | "destructive" | "outline"; icon: any; label: string }> = {
      ...
      deal_matched: { variant: "default", icon: CheckCircle2, label: "Deal Matched" },
      payment_pending: { variant: "secondary", icon: AlertTriangle, label: "Payment Pending" },
      paid: { variant: "default", icon: CheckCircle2, label: "Paid" },
      transfer_pending: { variant: "secondary", icon: Clock, label: "Transfer Pending" },
    ...
    <div className="min-h-screen bg-background p-4 md:p-6" data-testid="my-orders">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <Button 
            variant="outline" 
            onClick={() => { refetchBuys(); refetchSells(); }}
          ...
          <TabsContent value="buy" className="space-y-4">
            {filteredBuyRequests.length === 0 ? (
              ...
              filteredBuyRequests.map((request) => (
                <Card key={request.id} className="overflow-hidden" data-testid={`buy-request-${request.id}`}>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    {['matched', 'deal_matched', 'payment_pending', 'paid', 'transfer_pending', 'settled', 'completed'].includes(request.status) && (
                    ...
                    <div className="flex gap-2">
                      <Button 
                        variant="outline" 
                        size="sm"
                        onClick={() => navigate(`/unlisted/company/${request.companyId}`)}
                      ...
                      {request.status === 'payment_pending' && (
                        <Button 
                          size="sm"
                          onClick={() => navigate(`/unlisted/pay/${request.id}`)}
          ...
          <TabsContent value="sell" className="space-y-4">
            {filteredSellListings.length === 0 ? (
              ...
              filteredSellListings.map((listing) => (
                <Card key={listing.id} className="overflow-hidden" data-testid={`sell-listing-${listing.id}`}>
                  <CardHeader className="pb-2">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                  ...
                  <CardContent className="space-y-4">
                    ...
                    {['matched', 'deal_matched', 'payment_pending', 'transfer_pending', 'settled', 'completed'].includes(listing.status) && (
                      <UnlistedOrderTracker 
                    ...
                    <div className="flex gap-2">
                      <Button 
                        variant="outline" 
                        size="sm"
                        onClick={() => navigate(`/unlisted/company/${listing.companyId}`)}
