...
export default function CompanyDetails() {
	...
	const { data: financials, error: financialsError } = useQuery<
		CompanyFinancials[]
	>({
		...
		queryFn: async () => {
			...
			if (!response.ok) {
				if (response.status === 401)
					throw {
						status: 401,
						message: "Please sign in to view financial data",
					};
	...
	const { data: ratios, error: ratiosError } = useQuery<CompanyRatios[]>({
		...
		queryFn: async () => {
			...
			if (!response.ok) {
				if (response.status === 401)
					throw { status: 401, message: "Please sign in to view ratio data" };
	...
	>({
		...
		queryFn: async () => {
			const response = await fetch(
				`/api/unlisted/companies/${id}/price-history?limit=50`,
			);
			if (!response.ok) {
				if (response.status === 401)
					throw {
						status: 401,
						message: "Please sign in to view price history",
					};
	...
	const { data: eligibilityData } = useQuery<{
		success: boolean;
		data: { eligible: boolean; maxTradeValue: number; reasons?: string[] };
	}>({
	...
	const latestRatios = ratios?.find(
		(r) => r.financialYear === latestFinancials?.financialYear,
	);

	// Prepare chart data
	const financialChartData = sortedFinancials
		.slice(0, 5)
		.reverse()
		.map((f) => ({
		...
		<div className="container mx-auto p-4 md:p-6 space-y-6">
			...
			{isNotLoggedIn && (
				<Alert
					...
					data-testid="alert-login-required"
				>
					...
					<AlertDescription className="text-blue-700 dark:text-blue-300">
						<p className="mb-3">
							Please sign in to view detailed company financials, ratios, and
							price history for your research.
						</p>
						<Button
							size="sm"
			...
			{!isNotLoggedIn && !canTrade && (
				<Alert
					...
					data-testid="alert-trading-eligibility"
				>
					<LucideShield className="h-5 w-5 text-amber-600" />
					<AlertTitle className="text-amber-800 dark:text-amber-400">
						Trading Eligibility Required
					</AlertTitle>
					<AlertDescription className="text-amber-700 dark:text-amber-300">
						<p className="mb-3">
							You can view all company data for research purposes. To place
							buy/sell orders, SEBI regulations require:
						</p>
					...
					<Card data-testid="card-metric-revenue">
						...
						<CardContent>
							...
							<p className="text-xs text-muted-foreground">
								FY {latestFinancials.financialYear}
							</p>
						</CardContent>
					</Card>
						...
						<Card className="border-amber-200 dark:border-amber-800">
							<CardContent className="flex flex-col items-center justify-center py-12">
								...
								<p className="text-muted-foreground text-center max-w-md mb-4">
									Complete Level 2 KYC verification to access detailed financial
									statements and analysis.
								</p>
								<Button
									onClick={() => setLocation("/kyc-dashboard")}
									data-testid="button-complete-kyc-financials"
								>
						...
						<>
							<Card>
								<CardHeader>
									...
									<CardDescription>
										Revenue, Net Profit, and EBITDA over time (in Crores)
									</CardDescription>
								</CardHeader>
								<CardContent>
									...
									<div className="space-y-4">
										{sortedFinancials.map((f) => (
											<div
												...
												data-testid={`financial-${f.financialYear}`}
											>
												...
												<div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-sm">
													<div>
														...
														<div className="font-medium">
															₹{((f.revenue || 0) / 10000000).toFixed(2)} Cr
														</div>
													</div>
													<div>
														<span className="text-muted-foreground">
														...
														<div className="font-medium">
															₹{((f.netProfit || 0) / 10000000).toFixed(2)} Cr
														</div>
													</div>
													<div>
														<span className="text-muted-foreground">
														...
														<div className="font-medium">
															₹{((f.ebitda || 0) / 10000000).toFixed(2)} Cr
														</div>
													</div>
													<div>
														<span className="text-muted-foreground">
														...
														<div className="font-medium">
															₹{((f.totalAssets || 0) / 10000000).toFixed(2)} Cr
														</div>
													</div>
													<div>
														<span className="text-muted-foreground">
														...
														<div className="font-medium">
															₹
															{((f.totalLiabilities || 0) / 10000000).toFixed(
																2,
															)}{" "}
															Cr
														</div>
													...
													<div>
														...
														<div className="font-medium">
															₹{((f.equity || 0) / 10000000).toFixed(2)} Cr
														</div>
													</div>
												</div>
											</div>
					...
					{isKycBlocked ? (
						<Card className="border-amber-200 dark:border-amber-800">
							<CardContent className="flex flex-col items-center justify-center py-12">
								...
								<h3 className="text-xl font-semibold mb-2">KYC Required</h3>
								<p className="text-muted-foreground text-center max-w-md mb-4">
									Complete Level 2 KYC verification to access valuation ratios
									and financial analysis.
								</p>
								<Button
									onClick={() => setLocation("/kyc-dashboard")}
									data-testid="button-complete-kyc-ratios"
								>
									Complete KYC Verification
								</Button>
							</CardContent>
						...
						<>
							...
							<Card>
								<CardHeader>
									...
									<CardDescription>
										ROE and Debt-to-Equity over time
									</CardDescription>
								</CardHeader>
								<CardContent>
									<ResponsiveContainer width="100%" height={300}>
										...
										{[...(ratios || [])]
											...
											.map((r) => (
												<div
													key={r.id}
													className="border-b pb-4 last:border-0"
													data-testid={`ratio-${r.financialYear}`}
												>
													<div className="flex items-center gap-2 mb-3">
														<Calendar className="w-4 h-4 text-muted-foreground" />
														<h3 className="font-semibold">
													...
													<div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
														...
														{r.debtToEquity != null && (
															<div>
																...
																<div className="font-medium">
																	{Number(r.debtToEquity).toFixed(2)}
																</div>
															</div>
														)}
														{r.currentRatio != null && (
															<div>
																...
																<div className="font-medium">
																	{Number(r.currentRatio).toFixed(2)}
																</div>
															</div>
														)}
														{r.evToEbitda != null && (
															<div>
																...
																<div className="font-medium">
																	{Number(r.evToEbitda).toFixed(2)}x
																</div>
															</div>
														)}
													</div>
					...
					{isKycBlocked ? (
						<Card className="border-amber-200 dark:border-amber-800">
							<CardContent className="flex flex-col items-center justify-center py-12">
								...
								<h3 className="text-xl font-semibold mb-2">KYC Required</h3>
								<p className="text-muted-foreground text-center max-w-md mb-4">
									Complete Level 2 KYC verification to access price history and
									market data.
								</p>
								<Button
									onClick={() => setLocation("/kyc-dashboard")}
									data-testid="button-complete-kyc-price"
								>
									Complete KYC Verification
								</Button>
							</CardContent>
						...
						<>
							<Card>
								<CardHeader>
									...
									<CardDescription>
										Historical pricing from deals and seller feeds
									</CardDescription>
								</CardHeader>
								<CardContent>
									<ResponsiveContainer width="100%" height={350}>
