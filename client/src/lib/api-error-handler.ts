...
export async function handleApiError(
	...
	if (!silent) {
		const errorCode =
			apiError.errorCode ||
			`${apiError.type.toUpperCase()}_${operation.toUpperCase().replace(/\s+/g, "_")}`;

		let trackPromise: Promise<string | null>;

		if (apiError.type === "network") {
			trackPromise = trackNetworkError(errorCode, 0, apiError.message, {
				transactionId,
				clientId,
			});
		} else if (
			module === "payment" ||
			operation.toLowerCase().includes("payment")
		) {
			...
			trackPromise = trackKycError(errorCode, apiError.message, "error", {
				clientId,
			});
		} else {
