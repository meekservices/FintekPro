...
function decryptData(encryptedData: string, userId: string): Record<string, any> {
  const key = getEncryptionKey(userId);
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (event) => {
      ...
      if (!db.objectStoreNames.contains(DRAFTS_STORE)) {
        ...
        draftsStore.createIndex('userId_module', ['userId', 'module'], { unique: false });
      }
      ...
      if (!db.objectStoreNames.contains(ACTIONS_STORE)) {
        ...
        actionsStore.createIndex('idempotencyKey', 'idempotencyKey', { unique: true });
      }
    };
  });
}
...
export class DraftStorage {
  ...
  private pendingChanges: Map<string, { data: Record<string, any>; metadata: Partial<DraftMetadata> }> = new Map();

  async saveDraft(
    id: string,
    data: Record<string, any>,
    metadata: Omit<DraftMetadata, 'id' | 'timestamp' | 'lastModified' | 'syncStatus' | 'version' | 'checksum'>
  ): Promise<Draft> {
  ...
  async queueAction(action: Omit<QueuedAction, 'id' | 'timestamp' | 'retryCount' | 'status'>): Promise<QueuedAction> {
    const db = await openDB();
  ...
  async getPendingActions(userId: string): Promise<QueuedAction[]> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        const actions = request.result.filter((a: QueuedAction) => a.userId === userId);
        resolve(actions);
      };
      request.onerror = () => reject(request.error);
    });
  ...
  async updateActionStatus(id: string, status: QueuedAction['status'], errorMessage?: string): Promise<void> {
    const db = await openDB();
  ...
  startAutoSave(
    ...
    getDraftData: () => { id: string; data: Record<string, any>; metadata: Omit<DraftMetadata, 'id' | 'timestamp' | 'lastModified' | 'syncStatus' | 'version' | 'checksum'> } | null
  ): void {
    this.stopAutoSave();
    ...
    this.autoSaveInterval = window.setInterval(async () => {
      ...
      if (draftInfo) {
        try {
          await this.saveDraft(draftInfo.id, draftInfo.data, draftInfo.metadata);
