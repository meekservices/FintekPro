...
export async function initSentry() {
	...
	if (!sentryDsn) {
		console.log(
			"[ErrorTracking] Sentry DSN not configured - using API-only tracking",
		);
	...
	console.log(
		"[ErrorTracking] Sentry package not installed - using API-only tracking",
	);
	console.log(
		"[ErrorTracking] To enable Sentry, install @sentry/react package",
	);
	sentryInitialized = true;
}
...
function detectModule(): ErrorModule {
	...
	if (path.includes("/mutual-fund") || path.includes("/mf"))
		return "mutual_fund";
