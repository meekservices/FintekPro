...
export function useSubmitCreditApplication() {
	...
	return useMutation({
		...
		onSuccess: () => {
			...
			queryClient.invalidateQueries({
				queryKey: ["/api/mpal/financial-profile"],
			}); // Invalidate profile to update liabilities
		},
	});
}
...
export function useBrokerMarketData(
	assetClass: "US_EQUITY" | "INDIAN_EQUITY" | "MUTUAL_FUND" | "UNLISTED",
) {
	return useQuery({
