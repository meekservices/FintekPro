...
function hashHoldings(holdings: any[]): string {
  return JSON.stringify(holdings.map(h => ({ name: h.name || h.productName, value: h.currentValue })));
}
...
export function useSectionAnalytics({
  ...
  const [loadedSections, setLoadedSections] = useState<Set<AnalyticsSection>>(new Set());
  ...
  const loadSection = useCallback(async (section: AnalyticsSection) => {
  ...
  const loadMultipleSections = useCallback(async (sections: AnalyticsSection[]) => {
    if (!enabled || !holdings.length || !riskProfile || !analysis) {
      return {};
    }
    ...
    sectionsToLoad.forEach(section => {
      ...
      setSectionData(prev => ({
        ...prev,
        [section]: { ...prev[section], isLoading: true, error: null } as SectionData
      }));
    });
    ...
    try {
      ...
      sectionsToLoad.forEach(section => {
        setSectionData(prev => ({
          ...prev,
          [section]: { ...prev[section], isLoading: false, error: error.message } as SectionData
        }));
      });
      return {};
    } finally {
  ...
  const getSectionState = useCallback((section: AnalyticsSection): SectionData | null => {
    return sectionData[section] || null;
  }, [sectionData]);

  const isSectionLoaded = useCallback((section: AnalyticsSection): boolean => {
    return loadedSections.has(section);
  }, [loadedSections]);

  const isSectionLoading = useCallback((section: AnalyticsSection): boolean => {
