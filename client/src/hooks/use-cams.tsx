...
export function useCamsTransactions(pan: string, fromDate?: string, toDate?: string) {
  ...
  return useQuery<CamsTransaction[]>({
    ...
    queryFn: () => apiRequest(`/api/cams/transactions/${pan}?${params.toString()}`),
    enabled: !!pan && pan.length === 10,
...
export function useCamsPurchase() {
  ...
  return useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['/api/cams/portfolio', variables.pan] });
      queryClient.invalidateQueries({ queryKey: ['/api/cams/transactions', variables.pan] });
    },
  });
}
...
export function useCamsSipSetup() {
  ...
  return useMutation({
    ...
    onSuccess: (data, variables) => {
      queryClient.invalidateQueries({ queryKey: ['/api/cams/sip', variables.pan] });
      queryClient.invalidateQueries({ queryKey: ['/api/cams/portfolio', variables.pan] });
    },
  });
}
