...
export function useAddHolding() {
	...
	return useMutation({
		mutationFn: async ({
			portfolioId,
			holdingData,
		}: { portfolioId: string; holdingData: any }) => {
...
export function useRebalancePortfolio() {
	...
	return useMutation({
		mutationFn: async ({
			portfolioId,
			targetAllocations,
		}: { portfolioId: string; targetAllocations: any[] }) => {
		...
		mutationFn: async ({ file, options }) => {
			...
			if (!res.ok) {
				const error = await res
					.json()
					.catch(() => ({ message: "Failed to parse PDF" }));
...
export function useParseCASStatement() {
	return useMutation<
		ImportResult,
		Error,
		{ file: File; type: "cas" | "demat"; options?: ParseOptions }
	>({
		mutationFn: async ({ file, type, options }) => {
			...
			if (!res.ok) {
				const error = await res
					.json()
					.catch(() => ({ message: "Failed to parse CAS statement" }));
...
export function useSmartImport() {
	return useMutation<
		ImportResult,
		Error,
		{ file?: File; url?: string; prospectId?: string }
	>({
		mutationFn: async ({ file, url, prospectId }) => {
			...
			if (!res.ok) {
				const error = await res
					.json()
					.catch(() => ({ message: "Failed to import portfolio" }));
				throw new Error(
					error.message || error.error || "Failed to import portfolio",
				);
			}
			return res.json();
		},
...
export function useImportWealthyURL() {
	...
	return useMutation<
		ImportResult,
		Error,
		{ url: string; replaceExisting?: boolean }
	>({
		mutationFn: async ({ url, replaceExisting }) => {
			const res = await apiRequest("POST", "/api/portfolio/import-wealthy", {
				url,
				replaceExisting,
			});
			return res.json();
...
export function useUpdateHolding() {
	...
	return useMutation<
		...
		Error,
		{
			holdingId: string;
			portfolioId: string;
			updates: Partial<ImportedHolding>;
		}
	>({
...
export function useRecordImportHistory() {
	...
	return useMutation<
		{ success: boolean; entry: ImportHistoryEntry },
		Error,
		{ clientId: string; data: Omit<ImportHistoryEntry, "id" | "timestamp"> }
	>({
		mutationFn: async ({ clientId, data }) => {
			const res = await apiRequest(
				`/api/ai-investment/portfolio/${clientId}/import-history`,
				{
					method: "POST",
