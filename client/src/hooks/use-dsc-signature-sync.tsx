...
export function useDSCSignatureSync() {
  ...
  const submitSignature = useCallback(async (sig) => {
    try {
      ...
      if (submitResponse.success) {
        console.log('[DSC Sync] Successfully submitted queued signature:', sig.id, 'for transaction:', sig.transactionId);
        return true;
      }
  ...
  const syncQueuedSignatures = useCallback(async () => {
    ...
    console.log(`[DSC Sync] Attempting to sync ${queue.length} queued signature(s)`);
