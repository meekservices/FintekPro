...
export function useOfflineRBAC() {
	...
	const getActionMessage = useCallback(
		(action: ActionCategory): string | undefined => {
			const result = checkOfflinePermission(userRole, action, networkState);
	...
	const tryAction = useCallback(
		(action: ActionCategory, onAllowed: () => void): boolean => {
			const result = checkOfflinePermission(userRole, action, networkState);
			...
			toast({
				...
				description:
					result.message ||
					"This action is not available in your current network state.",
				variant: "destructive",
			});
...
export function withOfflineGuard<P extends object>(
	...
	return function OfflineGuardedComponent(props: P) {
		...
		if (!canPerformAction(requiredAction)) {
			return (
				<div
					className="flex flex-col items-center justify-center p-8 text-center bg-muted/50 rounded-lg border border-dashed"
					data-testid="offline-guard-blocked"
				>
