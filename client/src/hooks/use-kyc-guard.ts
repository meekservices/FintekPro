...
export function useKycGuard() {
  ...
  const guardAction = useCallback(
    async (transactionType: TransactionType, action: () => void) => {
      setIsChecking(true);
      try {
        const res = await fetch(`/api/kyc/transaction-check?type=${transactionType}`, {
          credentials: "include",
        });
  ...
  const proceedToKyc = useCallback(
    (returnPath?: string) => {
      ...
      const returnTo = encodeURIComponent(returnPath || window.location.pathname + window.location.search);
      closeModal();
