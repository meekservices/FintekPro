...
export function useFeeBreakdown(options: UseFeeBreakdownOptions) {
	...
	const query = useQuery<{ success: boolean; data: FeeBreakdown }>({
		queryKey: [
			"/api/admin/platform-fees/calculate",
			transactionAmount,
			productType,
			investorTier,
			includeGst,
...
export function useAggregatedFeeBreakdown(
	options: UseAggregatedFeeBreakdownOptions,
) {
	const {
		items,
		investorTier = "retail",
	...
	const itemsKey = JSON.stringify(
		items.map((i) => `${i.productType}:${i.amount}`).sort(),
	);

	const query = useQuery<{ success: boolean; data: any }>({
		queryKey: [
			"/api/admin/platform-fees/calculate-aggregated",
			itemsKey,
			investorTier,
			includeGst,
			applyWaivers,
