...
interface UnifiedCartContextType {
	...
	addItem: (
		item: Omit<InsertUnifiedCartItem, "userId">,
	) => Promise<UnifiedCartItem>;
	updateItem: (
		id: string,
		updates: Partial<InsertUnifiedCartItem>,
	) => Promise<UnifiedCartItem>;
...
export function UnifiedCartProvider({ children }: { children: ReactNode }) {
	...
	const {
		...
		error,
		refetch,
	} = useQuery<{ items: UnifiedCartItem[]; count: number }>({
	...
	const updateItemMutation = useMutation({
		mutationFn: async ({
			id,
			updates,
		}: { id: string; updates: Partial<InsertUnifiedCartItem> }) => {
