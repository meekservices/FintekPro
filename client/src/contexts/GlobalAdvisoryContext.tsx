...
} from "@/components/global-advisory/GlobalAdvisoryDisclaimer";
...
const GlobalAdvisoryContext = createContext<GlobalAdvisoryContextType | null>(
	null,
);
...
export function GlobalAdvisoryProvider({
	children,
}: GlobalAdvisoryProviderProps) {
	...
	const selectedMarket =
		markets.find((m) => m.marketCode === selectedMarketCode) ||
		markets.find((m) => m.marketCode === "IN") ||
		null;
	...
	const isAnalyticsMode =
		eligibilityData?.eligibility?.isAnalyticsMode ?? !canExecute;
	const displayCurrency =
		preferences?.displayCurrency || selectedMarket?.baseCurrency || "INR";

	const eligibleMarkets = eligibilityData?.eligibility?.markets || [];
	const currentMarketEligibility =
		eligibleMarkets.find((m) => m.marketCode === selectedMarketCode) || null;
	...
	const jurisdictionFlags: JurisdictionFeatureFlags = useMemo(
		() => ({
			...
			hasEtfOnlyRestriction:
				currentMarketEligibility?.restrictions?.some((r) =>
					r.includes("ETF"),
				) ?? false,
			requiresAccreditedStatus:
				currentMarketEligibility?.restrictions?.some((r) =>
					r.includes("accredited"),
				) ?? false,
			requiredAcknowledgments: isGlobalMode
	...
	const checkDisclaimerAndProceed = useCallback(
		(marketCode: string, onProceed: () => void) => {
			if (marketCode === "IN") {
				onProceed();
				return;
			}
...
export function useGlobalAdvisory() {
	...
	if (!context) {
		throw new Error(
			"useGlobalAdvisory must be used within a GlobalAdvisoryProvider",
		);
	}
	return context;
}
