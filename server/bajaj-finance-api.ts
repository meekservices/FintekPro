...
export class BajajFinanceAPI {
	...
	calculateFD(
		amount: number,
		tenure: number,
		fdType: "regular" | "senior-citizen" = "regular",
	): {
		...
		let interestRate = 8.1; // Base rate for regular FD

		if (fdType === "senior-citizen") {
			interestRate = 8.5; // Additional 0.4% for senior citizens
		}

		// Tenure-based rate adjustments
		if (tenure >= 36) interestRate = 8.35;
		else if (tenure >= 24) interestRate = 8.25;
		else if (tenure >= 12) interestRate = 8.1;
		else interestRate = 7.9;

		if (fdType === "senior-citizen") interestRate += 0.4;

		const maturityAmount = financial.fv(
			interestRate / 100 / 12,
			tenure,
			0,
			-amount,
	...
	calculateSIP(
		...
		const futureValue = financial.fv(
			monthlyReturn,
			totalMonths,
			-monthlyAmount,
			0,
	...
	getCurrentRates(): {
		...
		return {
			personalLoan: { min: 10.99, max: 21.0 },
			businessLoan: { min: 14.0, max: 20.0 },
			homeLoan: { min: 8.5, max: 11.5 },
			fixedDeposit: { regular: 8.1, seniorCitizen: 8.5 },
			autoLoan: { min: 9.5, max: 13.99 },
		};
	}
