 ...
 */
export async function getUpcomingCoupons(
	userId: string,
	daysAhead: number = 30,
	...
	const holdings = await db
		...
		.from(bondHoldings)
		.where(
			and(eq(bondHoldings.userId, userId), isNotNull(bondHoldings.couponRate)),
		);

	const upcomingCoupons: CouponPayment[] = [];

	for (const holding of holdings) {
		const couponRate = Number.parseFloat(holding.couponRate || "0");
		...
		if (holding.nextCouponDate) {
			...
			nextCouponDateValue = calculateNextCouponDate(
				null,
				purchaseDateValue,
				"semi-annual",
			);
		...
		if (nextCouponDateValue >= today && nextCouponDateValue <= futureDate) {
			const faceValue = Number.parseFloat(holding.faceValue || "1000");
...
export async function getBondAlerts(userId: string): Promise<{
	...
	const totalUpcomingCouponAmount = coupons.reduce(
		(sum, c) => sum + c.estimatedAmount,
		0,
	);
	const totalMaturityAmount = maturities.reduce(
		(sum, m) => sum + m.maturityAmount,
		0,
	);
...
export async function createAlertPreferences(
	...
	if (existing.length > 0) {
		await db
			.update(fixedIncomeNotificationPrefs)
			.set({
...
async function sendPendingAlerts(): Promise<void> {
	...
	const alertPrefs = (await db
		.select()
		.from(fixedIncomeNotificationPrefs)) as any[];
	...
	for (const pref of alertPrefs) {
		...
		try {
			...
			if (pref.couponPaymentReminder) {
				const coupons = await getUpcomingCoupons(
					pref.userId,
					pref.reminderDaysBefore || 7,
				);
				for (const coupon of coupons) {
					console.log(
						`[Alert] Coupon due for ${pref.userId}: ${coupon.bondName} - ₹${coupon.estimatedAmount.toLocaleString()}`,
					);
			...
			if (pref.maturityReminder) {
				const maturities = await getMaturityAlerts(
					pref.userId,
					pref.reminderDaysBefore || 30,
				);
				for (const maturity of maturities) {
					if (maturity.daysToMaturity <= (pref.reminderDaysBefore || 30)) {
						console.log(
							`[Alert] Maturity in ${maturity.daysToMaturity} days for ${pref.userId}: ${maturity.bondName}`,
						);
						// In production: Send email/SMS/push notification
					}
				}
			...
			console.error(
				`[Alert Service] Error processing alerts for user ${pref.userId}:`,
				error,
			);
		}
	}
