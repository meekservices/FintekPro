...
class KFintechAPI {
	...
	async validateInvestor(
		...
		KFintechResponse<{ isValid: boolean; investorName?: string; details?: any }>
	> {
	...
	async getInvestorPortfolio(
		pan: string,
	): Promise<KFintechResponse<InvestorPortfolio>> {
			...
			const portfolio: InvestorPortfolio = {
				...
				folios: Array.isArray(portfolioData.Folios?.Folio)
					? portfolioData.Folios.Folio.map((folio: any) => ({
							...
							nav: Number.parseFloat(folio.NAV || "0"),
							currentValue: Number.parseFloat(folio.CurrentValue || "0"),
							investmentValue: Number.parseFloat(folio.InvestmentValue || "0"),
							gainLoss: Number.parseFloat(folio.GainLoss || "0"),
							gainLossPercentage: Number.parseFloat(
								folio.GainLossPercentage || "0",
							),
						}))
					: [],
			};
			...
			const history: TransactionHistory = {
				transactions: Array.isArray(transactionsData.Transaction)
					? transactionsData.Transaction.map((txn: any) => ({
							...
							units: Number.parseFloat(txn.Units || "0"),
							nav: Number.parseFloat(txn.NAV || "0"),
	...
	async getSIPDetails(
		pan: string,
		folioNumber?: string,
	): Promise<KFintechResponse<SIPDetails>> {
			...
			const sipDetails: SIPDetails = {
				sips: Array.isArray(sipData.SIP)
					? sipData.SIP.map((sip: any) => ({
							...
							amount: Number.parseFloat(sip.Amount || "0"),
							frequency: sip.Frequency,
							startDate: sip.StartDate,
							endDate: sip.EndDate,
							nextInstallmentDate: sip.NextInstallmentDate,
							status: sip.Status,
							totalInstallments: Number.parseInt(sip.TotalInstallments || "0"),
							executedInstallments: Number.parseInt(
	...
	): Promise<KFintechResponse<{ transactionId: string; status: string }>> {
	...
	async setupSIP(
		request: SIPRequest,
	): Promise<KFintechResponse<{ sipId: string; status: string }>> {
		const xmlRequest = this.buildXMLRequest("SETUP_SIP", {
			...
			SIPDetails: {
				...
				...(request.installments && {
					TotalInstallments: request.installments.toString(),
				}),
			},
			BankDetails: {
				AccountNumber: request.bankAccount,
	...
	async cancelSIP(
		pan: string,
		sipId: string,
	): Promise<KFintechResponse<{ status: string }>> {
