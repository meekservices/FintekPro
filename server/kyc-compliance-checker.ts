...
const KYC_LEVELS = {
	...
	ENHANCED: {
		name: "enhanced",
		maxAmount: Number.POSITIVE_INFINITY, // Enhanced KYC for transactions >₹10L
		requirements: [
			"pan",
			"name",
			"email",
...
async function checkKYCLevel(
	...
	const requirements =
		KYC_LEVELS[level.toUpperCase() as keyof typeof KYC_LEVELS].requirements;
		...
		switch (req) {
			...
			case "pan_verified":
				...
				if (
					...
					missing.push(
						"PAN verification (complete PAN verification via the KYC wizard)",
					);
				}
				break;
			case "video_kyc":
				if (
					!(profile as any).videoKycCompleted &&
					!(profile as any).videoKycCompletedAt
				)
					missing.push("Video KYC (In-Person Verification)");
				break;
			case "income_proof":
...
export async function checkKYCCompliance(
	...
	try {
		...
		const { met, missing, profile } = await checkKYCLevel(
			userId,
			requiredLevel,
		);
		...
		if (profile.amlStatus === "rejected") {
			result.blockers.push(
				"Account compliance verification failed. Please contact support",
			);
		...
		if (!met) {
			result.blockers.push(
				`${requiredLevel.toUpperCase()} KYC required for this transaction`,
			);
		...
		if (profile.riskCategory === "high" && context.amount > 100000) {
			result.warnings.push(
				"High-value transaction on high-risk profile may require manual review",
			);
		}
		...
		if (profile.complianceScore !== null && profile.complianceScore < 50) {
			result.warnings.push(
				"Low compliance score - additional verification may be required",
			);
		}
...
async function determineCurrentLevel(
	userId: string,
	profile: any,
): Promise<"none" | "basic" | "full" | "enhanced"> {
	...
	const hasFull =
		hasBasic &&
		(profile.bankAccountNumber || hasDigiLockerVerification) &&
	...
	const hasEnhanced =
		hasFull &&
		((profile as any).videoKycCompleted ||
			(hasDigiLockerVerification && profile.complianceScore >= 90));
...
export async function getKYCStatus(userId: string) {
	...
	if (currentLevel === "none" || currentLevel === "basic") {
		...
		if (!(profile as any).isProfileCompleted)
			pendingActions.push("Complete profile verification");
	}

	if (currentLevel !== "enhanced") {
		if (!(profile as any).videoKycCompleted)
			pendingActions.push("Complete Video KYC");
	}
