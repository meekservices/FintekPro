...
export class ClientAutoPopulateService {
	...
	async autoPopulateClientData(
		userId: string,
		minimalData: MinimalClientData,
	): Promise<AutoPopulatedResult> {
		console.log(
			`Starting auto-populate for user ${userId} with data:`,
			minimalData,
		);
		...
		try {
			...
			if (minimalData.panNumber) {
				result.personalInfo = await this.fetchPersonalInfoByPAN(
					minimalData.panNumber,
				);
			}

			// Step 3: Generate investment profile based on banking patterns
			const investmentProfile = await this.generateInvestmentProfile(
				minimalData,
				result.bankingData,
			);
			...
			result.productRecommendations =
				await this.fetchProductRecommendations(investmentProfile);

			// Step 5: Create initial portfolio setup
			result.portfolioData = await this.createInitialPortfolio(
				userId,
				investmentProfile,
			);
			...
			console.log(
				`Auto-populate completed for user ${userId}. Total data points: ${result.totalDataPoints}`,
			);
			return result;
		} catch (error) {
	...
	private async fetchBankingData(data: MinimalClientData) {
		...
		try {
			if (data.bankName === "ICICI" && data.accountNumber) {
				...
				const balanceResult = await this.iciciAPI.getAccountBalance(
					data.accountNumber,
				);
				...
				if (transactionResult.success && transactionResult.data) {
					bankingData.transactionPatterns = this.analyzeTransactionPatterns(
						transactionResult.data,
					);
					bankingData.monthlyAverage = this.calculateMonthlyAverage(
						transactionResult.data,
					);
				}
			}
			...
			if (data.bankName === "HDFC" && data.accountNumber) {
				...
				const balanceResult = await this.hdfcAPI.getAccountBalance(
					data.accountNumber,
				);
				if (balanceResult.success && balanceResult.data) {
					bankingData.accounts.push({
			...
			if (bankingData.transactionPatterns) {
				bankingData.inferredRiskProfile =
					this.inferRiskProfileFromBanking(bankingData);
			}
		} catch (error) {
	...
	private async generateInvestmentProfile(
		minimalData: MinimalClientData,
		bankingData: any,
	) {
		const profile = {
	...
	private async fetchProductRecommendations(investmentProfile: any) {
		...
		try {
			...
			for (const product of allProducts.slice(0, 20)) {
				const matchScore = this.calculateProductMatch(
					product,
					investmentProfile,
				);
				if (matchScore > 60) {
	...
	private async createInitialPortfolio(userId: string, investmentProfile: any) {
		const portfolioData = {
			...
			allocation: {
				equity:
					investmentProfile.riskTolerance === "aggressive"
						? 70
						: investmentProfile.riskTolerance === "balanced"
							? 50
							: 30,
				debt:
					investmentProfile.riskTolerance === "aggressive"
	...
	private analyzeTransactionPatterns(transactions: any[]) {
		...
		if (transactions.length > 0) {
			...
			patterns.averageTransactionSize =
	...
	private inferRiskProfileFromBanking(bankingData: any) {
		...
		if (totalBalance > 500000 && transactionPatterns.volatility > 0.3) {
			return "balanced";
		}
		return "conservative";
	}
	...
	private calculateProductMatch(product: any, profile: any): number {
		...
		if (productRisk === profile.riskTolerance) score += 30;
			...
			Math.abs(
				this.riskToNumber(productRisk) -
					this.riskToNumber(profile.riskTolerance),
			) === 1
		)
			score += 15;
		...
		if (
			product.minimumInvestment <= profile.maxInvestment &&
	...
	private mapRiskLevel(product: any): string {
		if (
			product.category?.includes("III") ||
			product.riskMetrics?.volatility > 25
		)
			return "aggressive";
		if (
			product.category?.includes("II") ||
			product.riskMetrics?.volatility > 15
		)
			return "balanced";
		return "conservative";
	}
	...
	private calculateVolatility(amounts: number[]): number {
		...
		const squaredDiffs = amounts.map((amount) => (amount - mean) ** 2);
...
export function setupClientAutoPopulateRoutes(app: express.Application) {
	...
	app.post("/api/client/auto-populate", async (req, res) => {
		try {
			...
			const result = await autoPopulateService.autoPopulateClientData(
				req.user.id,
				minimalData,
			);
