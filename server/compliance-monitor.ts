...
class ComplianceMonitor {
	...
	private loginAttempts: Map<string, { count: number; lastAttempt: Date }> =
		new Map();
	private ipTracking: Map<
		string,
		{ requests: number; firstSeen: Date; flagged: boolean }
	> = new Map();
	...
	async logEvent(
		...
		if (!this.lastChecksum) {
			const lastEntry = await db
				.select({ checksum: immutableAuditLogs.checksum })
		...
		if (
			complianceEvent.riskLevel !== "low" ||
			process.env.NODE_ENV !== "production"
		) {
			console.log(
				`[COMPLIANCE] ${complianceEvent.eventType}: ${complianceEvent.action}`,
				{
					userId: complianceEvent.userId,
	...
	private createAlert(alert: Omit<SecurityAlert, "id" | "timestamp">): string {
		...
		console.warn(
			`[SECURITY ALERT] ${securityAlert.alertType}: ${securityAlert.description}`,
			{
	...
	getEvents(filters?: {
		...
		if (filters) {
			...
			if (filters.riskLevel) {
				filteredEvents = filteredEvents.filter(
					(e) => e.riskLevel === filters.riskLevel,
				);
			}
		}
		...
		const report = {
			...
			summary: {
				...
				successfulLogins: events.filter(
					(e) => e.eventType === "login" && e.outcome === "success",
				).length,
			},
	...
	private getTopRiskIPs(events: ComplianceEvent[]) {
		const ipRisks: Record<
			string,
			{ events: number; highRisk: number; critical: number }
		> = {};
		...
		return Object.entries(ipRisks)
			.sort(
				([, a], [, b]) =>
					b.critical * 10 + b.highRisk - (a.critical * 10 + a.highRisk),
			)
			.slice(0, 10)
...
export const complianceMiddleware = (
	...
	res.on("finish", () => {
		...
		complianceMonitor
			...
			.catch((err) =>
				console.error("[COMPLIANCE] Background logging failed:", err),
			);
	});
