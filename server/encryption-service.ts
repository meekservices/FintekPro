...
class EncryptionService {
	...
	constructor() {
		this.masterKey =
			process.env.ENCRYPTION_MASTER_KEY || "2zkwzv8sbuy30ck75p63dszd42xtcicb";

		if (!process.env.ENCRYPTION_MASTER_KEY) {
			console.warn(
				"⚠️  ENCRYPTION_MASTER_KEY not found in environment, using production stabilization fallback.",
			);
		}
	}
	...
	decrypt(encryptedData: string | null | undefined): string | null {
		...
		try {
			...
			throw new DecryptionError(
				"Failed to decrypt data - potentially invalid key or corrupted data",
				error,
			);
		}
	}
	...
	encryptPII(data: {
		...
		return {
			...
			drivingLicense: data.drivingLicense
				? this.encrypt(data.drivingLicense)
				: null,
	...
	decryptPII(data: {
		...
		return {
			...
			drivingLicense: data.drivingLicense
				? this.decrypt(data.drivingLicense)
				: null,
