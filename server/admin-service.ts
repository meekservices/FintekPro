...
class AdminService {
	...
	async logActivity(activity: Omit<InsertUserActivity, "id" | "createdAt">) {
		...
		console.log(
			`📊 User Activity: ${activity.userId} - ${activity.action} ${activity.resource || ""}`,
		);
	...
	async getUserStats(): Promise<UserStats> {
		...
		const totalLogins = allUsers.reduce(
			(sum, user) => sum + (user.loginCount || 0),
			0,
		);
	...
	async getActivityMetrics(): Promise<ActivityMetrics> {
		...
		const recentActivities = activities.filter(
			(a) => a.createdAt && new Date(a.createdAt).getTime() > last24h,
		);

		const actionCounts = recentActivities.reduce(
			(acc, activity) => {
	...
	async getPlatformInsights(): Promise<PlatformInsights> {
		...
		for (let i = 29; i >= 0; i--) {
			...
			const count = allUsers.filter(
				(user) =>
					user.createdAt &&
					new Date(user.createdAt) >= dayStart &&
					new Date(user.createdAt) < dayEnd,
			).length;
		...
		const featureCounts = activities.reduce(
			(acc, activity) => {
		...
		const monthlyActiveUsers = new Set(
			activities
				.filter(
					(a) => a.createdAt && new Date(a.createdAt).getTime() > monthAgo,
				)
				.map((a) => a.userId)
				.filter(Boolean),
		).size;
	...
	async updateUserLoginStats(
		...
		if (user) {
			...
			console.log(
				`🔑 User ${userId} logged in (${user.loginCount} total logins)`,
			);
		}
	}
