...
export function initializeUnlistedCrons(): void {
	...
	(async () => {
		try {
			...
			if (results.length > 0) {
				console.log(
					`[ListingTracker] Startup sweep: ${results.length} company(ies) transitioned to listed:`,
					results.map((r) => r.companyName).join(", "),
				);
			} else {
				console.log(
					"[ListingTracker] Startup sweep: no new listing transitions found",
				);
			}
		} catch (err) {
	...
	if (!isProductionEnvironment()) {
		...
		console.log(
			"⏭️ [OrderCleanup] Skipped (development mode - production only)",
		);
		console.log(
			"⏭️ [ExpiryWarning] Skipped (development mode - production only)",
		);
		console.log(
			"⏭️ [Price/MoneyControl/BondCalendar] Skipped (development mode - production only)",
		);
		console.log(
			"⏭️ [MF Cron Jobs] Monthly Returns, NAV Pre-warm SKIPPED (development mode)",
		);
		console.log(
			"⏭️ [Credhive Alerts/Lead Scoring] Skipped (development mode - production only)",
		);
		console.log(
			"⏭️ [ProspectAutoScoring] Nightly scoring skipped (development mode - production only)",
		);
		console.log(
			"⏭️ [CompanyRefresh] Skipped (development mode - production only)",
		);
		console.log(
			"⏭️ [CacheWarming] Skipped (development mode - production only)",
		);
		return;
	}
	...
	cron.schedule(
		...
		async () => {
			console.log(
				"[CRON][ListingTracker] Running daily listing transition sweep...",
			);
			try {
				const results = await unlistedListingTracker.runTransitionSweep();
				console.log(
					`[CRON][ListingTracker] Sweep complete: ${results.length} transitioned`,
				);
				results.forEach((r) =>
					console.log(
						`  ✅ ${r.companyName} → ${r.exchange}:${r.nseSymbol} (${r.listedOn}), ${r.picksExpired} pick(s) expired`,
					),
				);
			} catch (err) {
	...
	cron.schedule("0 */12 * * *", async () => {
		...
		try {
			...
			if (expiredListings > 0 || expiredRequests > 0) {
				console.log(
					`[CRON] Cleanup: ${expiredListings} expired listings, ${expiredRequests} expired requests`,
				);
				console.log(
					`[CRON][AUDIT] Unlisted marketplace cleanup: ${JSON.stringify({
						timestamp: new Date().toISOString(),
						expiredListings,
						expiredRequests,
						action: "auto_expire",
				...
				console.log(
					"[CRON] Order cleanup: No expired listings or requests found",
				);
			}
		} catch (error: any) {
	...
	cron.schedule("30 3 * * *", async () => {
		...
		try {
			...
			if (expiringListings.length > 0 || expiringRequests.length > 0) {
				console.log(
					`[CRON] ${expiringListings.length} listings + ${expiringRequests.length} requests expiring in 24 h`,
				);
					...
					try {
						...
						if (user[0]?.email) {
							const company = await db
								...
								.where(eq(unlistedCompanies.id, listing.companyId))
								.limit(1);
							const companyName = company[0]?.name || "Unlisted Stock";
							await emailService.sendEmail({
								to: user[0].email,
								subject: `⏰ Your sell listing expires in 24 hours - ${companyName}`,
								html: `<h2>Listing Expiry Reminder</h2><p>Your sell listing for <strong>${companyName}</strong> expires soon. Quantity: ${listing.quantity || 0} shares @ ₹${listing.pricePerShare ? Number.parseFloat(String(listing.pricePerShare)).toLocaleString("en-IN") : "N/A"}/share.</p><p>Log in to FintekPro to extend or modify your listing.</p>`,
								text: `Your sell listing for ${companyName} expires in 24 hours.`,
							});
						}
					} catch (err) {
						console.error(
							`[CRON] Failed to send notification for listing ${listing.id}:`,
							err,
						);
					}
				}
					...
					try {
						...
						if (user[0]?.email) {
							...
							await es.sendEmail({
								...
								html: `<h2>Buy Request Expiry Reminder</h2><p>Your buy request for <strong>${reqCompanyName}</strong> expires soon. Quantity: ${request.quantity || 0} @ max ₹${request.maxPricePerShare ? Number.parseFloat(String(request.maxPricePerShare)).toLocaleString("en-IN") : "N/A"}/share.</p><p>Log in to FintekPro to extend or modify your request.</p>`,
								text: `Your buy request for ${reqCompanyName} expires in 24 hours.`,
							});
						}
					} catch (err) {
						console.error(
							`[CRON] Failed to send notification for request ${request.id}:`,
							err,
						);
					}
				}
	...
	cron.schedule("0 */12 * * *", async () => {
		...
		try {
			...
			console.log(
				`[CRON] Price suggestions refreshed for ${refreshed} companies`,
			);
		} catch (error: any) {
	...
	cron.schedule("30 15 * * *", async () => {
		...
		try {
			const { moneyControlScraper } = await import(
				"./services/moneycontrol-scraper"
			);
			const result = await moneyControlScraper.executeImport();
			console.log(
				`[CRON] MoneyControl sync: ${result.imported} imported, ${result.matched} matched, ${result.unmatchedCompanies.length} unmatched`,
			);
		} catch (error: any) {
	...
	cron.schedule("30 0 * * *", async () => {
		...
		try {
			const { financialCalendarService } = await import(
				"./services/financial-calendar-service"
			);
	...
	cron.schedule("35 3 * * *", async () => {
		...
		try {
			const { liveMFDataService } = await import(
				"./services/live-mf-data-service"
			);
	...
	cron.schedule("30 0 1 * *", async () => {
		...
		if (!isEnrichmentWindow()) {
			console.log("⏭️ [Monthly Returns] Outside 8PM-8AM IST window, skipping");
			return;
		}
		...
		try {
			...
			for (const fund of funds) {
				const count = await mfDataSyncService.calculateMonthlyReturnsForScheme(
					fund.schemeCode,
				);
			...
			console.error(
				"[CRON] Monthly returns calculation failed:",
				error.message,
			);
		}
	});
	...
	cron.schedule("30 2 * * *", async () => {
		...
		try {
			...
			console.log(
				`[CRON] Prospecting alerts: ${alerts.length} new high-value prospects`,
			);
			const hotAlerts = alerts.filter((a) => a.priority === "high").length;
			if (hotAlerts > 0)
				console.log(
					`[CRON] 🔥 ${hotAlerts} HOT leads require immediate attention`,
				);
		} catch (error: any) {
	...
	cron.schedule("30 0 * * 0", async () => {
		...
		try {
			...
			for (const lead of leads) {
				if (lead.cin) {
					const score = await analyticsService.calculateSmartLeadScore(
						lead.cin,
					);
					if (score) {
						await db
	...
	cron.schedule(
		...
		async () => {
			console.log(
				"[CRON][ProspectAutoScoring] Starting nightly prospect scoring...",
			);
			try {
				const { bulkScoreProspects } = await import(
					"./services/prospect-scoring-engine"
				);
				...
				console.log(
					`[CRON][ProspectAutoScoring] Done: ${result.succeeded} scored, ${result.failed} failed (of ${result.processed} checked)`,
				);
				if (result.errors.length > 0) {
					console.warn(
						"[CRON][ProspectAutoScoring] Errors:",
						result.errors.slice(0, 5).join("; "),
					);
				}
			} catch (error: any) {
				console.error(
					"[CRON][ProspectAutoScoring] Nightly scoring failed:",
					error.message,
				);
			}
		},
	...
	console.log(
		"⚡ [ProspectAutoScoring] Nightly scoring cron scheduled (3:00 AM IST)",
	);
	...
	try {
		...
		console.error(
			"[CRON] Failed to start Proactive Cache Warming Service:",
			error.message,
		);
	}
}
...
if (isProductionEnvironment()) {
	cron.schedule(
		...
		async () => {
			console.log(
				"[CRON][ValuationGovernance] Starting quarterly staleness sweep...",
			);
			try {
				const { unlistedValuationGovernanceService } = await import(
					"./services/unlisted-valuation-governance-service"
				);
				const report =
					await unlistedValuationGovernanceService.runStalenessSweep();
				console.log(
					`[CRON][ValuationGovernance] Sweep complete: ${report.markedStale} newly stale, ${report.alreadyStale} already stale, ${report.neverValued} never valued (of ${report.totalChecked} checked)`,
				);
			} catch (error: any) {
				console.error(
					"[CRON][ValuationGovernance] Quarterly sweep failed:",
					error.message,
				);
			}
		},
	...
	console.log(
		"📊 [ValuationGovernance] Quarterly staleness cron scheduled (3:00 AM IST, 1st of Jan/Apr/Jul/Oct)",
	);
} else {
	console.log(
		"⏭️ [ValuationGovernance] Quarterly staleness cron skipped (development mode - production only)",
	);
}
