...
const FORM15_ALLOWED_ROLES: RoleId[] = [
	"superadmin",
	"master_agent",
	"admin",
	"compliance_officer",
...
function getUserRole(req: Request): string {
	...
	if (roleInfo?.highestRole) {
		...
		if (
			["superadmin", "master_agent", "admin", "compliance_officer"].includes(
				role,
			)
		)
			return "admin";
...
function getSessionUser(
	req: Request,
): { id: string; email: string; role?: string; roles?: string[] } | undefined {
	const roleInfo = getRoleInfo(req);
...
async function isUserAuthorizedForCase(
	userId: string,
	userRole: string,
	caseId: string,
): Promise<{ authorized: boolean; caseData: any }> {
	const result = await db.execute(
		sql`SELECT * FROM form_15_cases WHERE id = ${caseId}`,
	);
...
function canEditCase(role: string, caseStatus: string): boolean {
	...
	if (
		role === "ca_subordinate_agent" &&
		["draft", "pending_documents", "sent_back_to_agent"].includes(caseStatus)
	)
		return true;
...
function determineRule37BB(
	...
	if (exemptPayments.includes(natureOfPayment) && remittanceAmount <= 700000) {
		...
		justification =
			"Payment exempt under Section 195 - no tax deductible at source";
	} else if (remittanceAmount <= 500000 && !dtaaApplicable) {
		form15caRequired = true;
		form15caPart = "B";
		form15cbRequired = false;
		justification =
			"Remittance amount <= 5 lakh without DTAA benefit - Part B applicable";
	} else if (dtaaApplicable && trcAvailable) {
		form15caRequired = true;
		form15caPart = "C";
		form15cbRequired = true;
		justification =
			"DTAA benefit claimed with TRC - Form 15CB required, Part C applicable";
	} else {
		form15caRequired = true;
		form15caPart = "D";
		form15cbRequired = true;
		justification =
			"Tax deductible at source - Form 15CB required, Part D applicable";
	}
...
export function registerForm15Routes(app: Express) {
	...
	router.post("/cases", async (req: Request, res: Response) => {
		try {
			...
			if (!canCreateCase(userRole)) {
				return res
					.status(403)
					.json({ error: "Not authorized to create cases" });
			}
			...
			const rule37bb = determineRule37BB(
				Number.parseFloat(remittanceAmount),
				natureOfPayment,
				dtaaApplicable === true,
				trcAvailable === true,
			);
			...
			const actualClientId =
				userRole === "client" ? userId : clientId || userId;
	...
	router.get("/cases/:caseId", async (req: Request, res: Response) => {
      ...
      `);
			...
			if (!isAuthorized) {
				return res
					.status(403)
					.json({ error: "Not authorized to view this case" });
			}
	...
	router.patch("/cases/:caseId", async (req: Request, res: Response) => {
		try {
			...
			if (!isAuthorized) {
				return res
					.status(403)
					.json({ error: "Not authorized to access this case" });
			}
			...
			if (!canEditCase(userRole, currentCase.status as string)) {
				return res
					.status(403)
					.json({
						error: "Not authorized to edit this case in current status",
					});
			}
			...
			if (
				...
				dtaaApplicable !== undefined ||
				trcAvailable !== undefined ||
				natureOfPayment !== undefined
			) {
				rule37bb = determineRule37BB(
					Number.parseFloat(remittanceAmount || currentCase.remittance_amount),
					natureOfPayment || currentCase.nature_of_payment,
					(dtaaApplicable !== undefined
						? dtaaApplicable
						: currentCase.dtaa_applicable) === true,
					(trcAvailable !== undefined
						? trcAvailable
						: currentCase.trc_available) === true,
				);
			}
	...
	router.post(
		...
		async (req: Request, res: Response) => {
			try {
				...
				// Authorization check
				const { authorized, caseData: currentCase } =
					await isUserAuthorizedForCase(userId, userRole, caseId);
				...
				if (userRole !== "ca_subordinate_agent" && userRole !== "client") {
					return res
						.status(403)
						.json({
							error: "Only subordinate agents or clients can submit for review",
						});
				}

				if (
					!["draft", "pending_documents", "sent_back_to_agent"].includes(
						currentCase.status as string,
					)
				) {
					return res
						.status(400)
						.json({ error: "Case cannot be submitted in current status" });
				}
		...
		async (req: Request, res: Response) => {
			try {
				...
				if (caseData.status !== "pending_ca_review") {
					return res
						.status(400)
						.json({ error: "Case is not pending CA review" });
				}
				...
				if (
					...
					return res
						.status(403)
						.json({ error: "Case already assigned to another CA" });
				}
				...
				await logAudit(
					...
					userRole,
					userEmail,
					"review_pickup_attempt",
					"CA attempting to pick up case for review",
					{
						ipAddress: req.ip,
	...
	router.post(
		...
		async (req: Request, res: Response) => {
			try {
				...
				if (!userId || (userRole !== "ca" && userRole !== "admin")) {
					return res
						.status(403)
						.json({ error: "Only CA can send back to agent" });
				}

				// Authorization check - only assigned CA or admin can send back
				const { authorized, caseData } = await isUserAuthorizedForCase(
					userId,
					userRole,
					caseId,
				);
      ...
      `);
				...
				await logAudit(
					...
					userEmail,
					"ca_sent_back",
					`Case sent back to agent: ${reason}`,
					{
						ipAddress: req.ip,
						userAgent: req.get("user-agent"),
					},
	...
	router.post("/cases/:caseId/approve", async (req: Request, res: Response) => {
		try {
			...
			const {
				documentsReviewed,
				dtaaVerified,
				taxComputationConfirmed,
				legalResponsibilityAccepted,
				remarks,
			} = req.body;
			const userId = getSessionUser(req)?.id;
			const userEmail = getSessionUser(req)?.email || "";
      ...
      `);
			...
			await logAudit(
				...
				userEmail,
				"ca_approved",
				"CA approved case with full checklist confirmation",
				{
					ipAddress: req.ip,
					userAgent: req.get("user-agent"),
					metadata: {
	...
	router.post(
		...
		async (req: Request, res: Response) => {
			try {
				...
				if (!userId || !canSignForm15CB(userRole)) {
					return res
						.status(403)
						.json({ error: "Only verified CA can sign Form 15CB" });
				}
				...
				if (!dscSerialNumber || !icaiMembershipNumber) {
					return res
						...
						.json({
							error:
								"DSC serial number and ICAI membership number are required",
						});
				}
      ...
      `);
				...
				if (!caVerification.rows || caVerification.rows.length === 0) {
					return res
						.status(403)
						.json({ error: "CA is not verified for Form 15CB signing" });
				}
				if ((currentCase.status as string) !== "approved") {
					return res
						.status(400)
						.json({ error: "Case must be approved before signing Form 15CB" });
				}

				if (!currentCase.form_15cb_required) {
					return res
						.status(400)
						.json({ error: "Form 15CB is not required for this case" });
				}
      ...
      `);
				...
				await logAudit(
					...
					userEmail,
					"15cb_signed",
					`Form 15CB signed with certificate ${form15cbNumber}`,
					{
						dscSerialNumber,
	...
	router.post(
		...
		async (req: Request, res: Response) => {
			try {
				...
				if (
					currentCase.form_15cb_required &&
					(currentCase.status as string) !== "15cb_signed"
				) {
					return res
						.status(400)
						.json({
							error: "Form 15CB must be signed before filing Form 15CA",
						});
				}
				if (
					!currentCase.form_15cb_required &&
					(currentCase.status as string) !== "approved"
				) {
					return res
						.status(400)
						.json({ error: "Case must be approved before filing Form 15CA" });
				}
      ...
      `);
				...
				await logAudit(
					...
					userEmail,
					"15ca_filed",
					`Form 15CA filed with acknowledgement ${acknowledgementNumber}`,
					{
						ipAddress: req.ip,
						userAgent: req.get("user-agent"),
					},
	...
	router.post("/cases/:caseId/everify", async (req: Request, res: Response) => {
		try {
			...
			if (caseData.client_id !== userId && userRole !== "admin") {
				return res
					.status(403)
					.json({ error: "Only case owner or admin can e-verify" });
			}
      ...
      `);
			...
			await logAudit(
				...
				userEmail,
				"everified",
				"Form 15CA e-verified by client",
				{
					ipAddress: req.ip,
					userAgent: req.get("user-agent"),
				},
	...
	router.post(
		"/cases/:caseId/generate-compliance-pack",
		async (req: Request, res: Response) => {
			try {
				const { caseId } = req.params;
				const userId = getSessionUser(req)?.id;
				...
				if (
					!["15ca_filed", "everified", "completed"].includes(
						currentCase.status as string,
					)
				) {
					return res
						...
						.json({
							error:
								"Form 15CA must be filed before generating compliance pack",
						});
				}
      ...
      `);
				...
				await logAudit(
					...
					"Bank compliance pack generated",
					{
						ipAddress: req.ip,
						userAgent: req.get("user-agent"),
					},
			...
			try {
				...
				if (caseData.form_15cb_locked) {
					return res
						.status(400)
						.json({
							error: "Cannot upload documents after Form 15CB is signed",
						});
				}
	...
	router.get(
		...
		async (req: Request, res: Response) => {
			try {
				...
				// Authorization check
				const { authorized } = await isUserAuthorizedForCase(
					userId,
					userRole,
					caseId,
				);
	...
	router.post("/ca-verification", async (req: Request, res: Response) => {
		try {
			...
			if (!icaiMembershipNumber || !panNumber) {
				return res
					.status(400)
					.json({ error: "ICAI membership number and PAN are required" });
			}
