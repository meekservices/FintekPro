...
const SANDBOX_ITR_TEST_PANS: Record<
	string,
	{ pan: string; description: string; itrForm: string }
> = {
	"ITR-1": {
		pan: "QUIPT2025E",
		description: "Salaried individual (Sandbox test)",
		itrForm: "1",
	},
	"ITR-2": {
		pan: "ADIPV7548K",
		description: "Individual with capital gains (Sandbox test)",
		itrForm: "2",
	},
	"ITR-3": {
		pan: "AVPPJ6869G",
		description: "Individual with business income (Sandbox test)",
		itrForm: "3",
	},
	"ITR-4": {
		pan: "AWGPS6028Q",
		description: "Presumptive income (Sandbox test)",
		itrForm: "4",
	},
	"ITR-5": {
		pan: "AOFPL2415R",
		description: "LLP / Partnership firm (Sandbox test)",
		itrForm: "5",
	},
	"ITR-6": {
		pan: "AAACA1234A",
		description: "Company (Sandbox test)",
		itrForm: "6",
	},
	"ITR-7": {
		pan: "AAETP3993P",
		description: "Trust / Charitable (Sandbox test)",
		itrForm: "7",
	},
};
...
class SandboxITRService {
	...
	constructor() {
		...
		if (!this.apiKey || !this.apiSecret) {
			console.warn(
				"⚠️ [Sandbox ITR] API credentials missing. SANDBOX_API_KEY and SANDBOX_API_SECRET are not set. Service will operate in MOCK mode.",
			);
		} else {
			const env = this.apiKey.startsWith("key_test")
				? "TEST"
				: this.apiKey.startsWith("key_live")
					? "PRODUCTION"
					: "UNKNOWN";
			console.log(
				`✅ Sandbox.co.in ITR Service initialized (${env} environment → ${SANDBOX_BASE_URL})`,
			);
		}
	}
	...
	async eriLogin(
		...
		try {
			...
			return {
				...
				authToken: "",
				message:
					response.message || "ERI login failed — no auth_token returned",
			};
		} catch (error) {
			...
			return {
				...
				userId: "",
				authToken: "",
				message: error instanceof Error ? error.message : "ERI login failed",
			};
		}
	}
	...
	private async makeAPICall(
		...
		if (!this.apiKey || !this.apiSecret) {
			console.warn(
				`[Sandbox API] MOCKING ${method} ${endpoint} (Missing Credentials)`,
			);
			return {
				success: true,
				message: "Mock response (Missing Credentials)",
				data: {},
			};
		}
		...
		try {
			...
			if (response.status === 401) {
				console.log(
					"[Sandbox API] Token expired or invalid, re-authenticating...",
				);
				clearSandboxToken();
				...
				if (!retryResponse.ok) {
					console.error(
						`[Sandbox API] Retry Error ${retryResponse.status}: ${retryText}`,
					);
					throw new Error(
						`Sandbox API returned ${retryResponse.status}: ${retryText}`,
					);
				}
				try {
					...
					throw new Error(
						`Sandbox API returned non-JSON response: ${retryText.substring(0, 200)}`,
					);
				}
			}
			...
			if (!response.ok) {
				console.error(
					`[Sandbox API] Error ${response.status}: ${responseText}`,
				);
				throw new Error(
					`Sandbox API returned ${response.status}: ${responseText}`,
				);
			}
			...
			try {
				...
				throw new Error(
					`Sandbox API returned non-JSON response: ${responseText.substring(0, 200)}`,
				);
			}
		} catch (error) {
			...
			if (
				...
				error.message.startsWith("Sandbox authentication")
			) {
				throw error;
			}
			console.error("[Sandbox API] Network/fetch error:", error);
			throw new Error(
				`Sandbox API unreachable: ${error instanceof Error ? error.message : "Unknown error"}`,
			);
		}
	}
	...
	async calculateTax(formData: ITRFormData): Promise<ITRCalculationResponse> {
		try {
			...
			console.log(
				"[Sandbox ITR] Using native Indian Tax Calculator (Sandbox IT API has no income tax calculator endpoint)",
			);
	...
	async calculateTaxFromWizard(wizardData: {
		...
		try {
			...
			console.log(
				"[Sandbox ITR] Using native Indian Tax Calculator for wizard (Sandbox IT API has no income tax calculator endpoint)",
			);
	...
	async prepareITR(formData: ITRFormData): Promise<ITRFilingResponse> {
		try {
			...
			console.log(
				"[Sandbox ITR] Using native ITR preparation (Sandbox IT API has no e-filing endpoint)",
			);
			...
			return {
				success: true,
				message:
					"ITR prepared via native Indian Tax Calculator. For actual e-Filing, integrate with the Income Tax Portal via ERI.",
				data: {
	...
	async getITRStatus(
		...
		try {
			...
			if (pan) {
				...
				if (!response.ok) {
					throw new Error(
						`ITR status API returned ${response.status}: ${parsed.message || responseText.substring(0, 200)}`,
					);
				}
				return parsed;
			}
	...
	async getForm26AS(pan: string, assessmentYear: string): Promise<any> {
		...
		console.log(
			"[Sandbox ITR] Form 26AS fetch endpoint does not exist — use parseForm26AS() OCR for PDF upload",
		);
		return {
			success: true,
			message:
				"Form 26AS data is not available via API fetch. Please upload your Form 26AS PDF for OCR extraction.",
			data: {
				pan,
				assessmentYear,
	...
	async getAIS(pan: string, assessmentYear: string): Promise<any> {
		...
		console.log(
			"[Sandbox ITR] AIS fetch endpoint does not exist in Sandbox.co.in IT API",
		);
		return {
			success: true,
			message:
				"AIS (Annual Information Statement) is not available via Sandbox.co.in API. Please download AIS directly from the Income Tax Portal (incometax.gov.in).",
			data: {
				pan,
				assessmentYear,
				source: "not_available",
	...
	async downloadITRV(
		...
		try {
			...
			if (pan) {
				...
				if (!fetchRes.ok) {
					throw new Error(
						`ITR-V API returned ${fetchRes.status}: ${response.message || responseText.substring(0, 200)}`,
					);
				}
			} else {
			...
			if (response.success) {
				return {
					...
					data: {
						downloadUrl:
							response.data?.itrVUrl ||
							`/api/itr/download/itr-v/${acknowledgmentNumber}.pdf`,
	...
	getSuitableITRForm(
		incomeDetails: ITRFormData["incomeDetails"],
		...
		if (entityType === "company") {
			return {
				form: "ITR-6",
				reason:
					"ITR-6 is mandatory for all companies except those claiming exemption under Section 11 (Charitable/Religious purposes)",
		...
		if (
			...
			return {
				form: "ITR-4",
				reason:
					"ITR-4 (Sugam) is applicable for partnership firms opting for presumptive taxation under Section 44AD (business income up to Rs. 2-3 crores, no capital gains)",
		...
		if (entityType === "huf") {
			if (businessIncome > 0) {
				return {
					form: "ITR-3",
					reason:
						"ITR-3 is required for HUF with business or professional income",
			...
			return {
				form: "ITR-2",
				reason:
					"ITR-2 is applicable for HUF with income from salary, house property, capital gains, or other sources",
				applicableForms: ["ITR-2", "ITR-3"],
			};
		}
		...
		if (
			...
			businessIncome === 0 &&
			capitalGains === 0 &&
			(rentalIncome === 0 || rentalIncome > 0)
		) {
			// Only one house property allowed
			return {
				form: "ITR-1",
				reason:
					"ITR-1 (Sahaj) is the simplest form for salaried individuals with total income up to Rs. 50 lakhs, no capital gains, and no business income",
		...
		if (
			...
			return {
				form: "ITR-4",
				reason:
					"ITR-4 (Sugam) is applicable for presumptive taxation under Section 44AD/44ADA/44AE with business income up to Rs. 2-3 crores",
		...
		if (businessIncome > 0) {
			return {
				form: "ITR-3",
				reason:
					"ITR-3 is required for individuals with business or professional income not opting for presumptive taxation, or income exceeding presumptive limits",
		...
		if (capitalGains > 0 || totalIncome > 5000000) {
			return {
				form: "ITR-2",
				reason:
					"ITR-2 is required for individuals with capital gains, income exceeding Rs. 50 lakhs, foreign assets/income, or multiple house properties",
				applicableForms: ["ITR-2", "ITR-3"],
			};
		}
		...
		return {
			form: "ITR-2",
			reason:
				"ITR-2 is the appropriate form based on your income sources and entity type",
			applicableForms: ["ITR-2", "ITR-3"],
		};
	}

	// Helper method for backwards compatibility
	getSuitableITRFormSimple(
		incomeDetails: ITRFormData["incomeDetails"],
	): string {
	...
	> {
		return {
			...
			"ITR-2": {
				...
				notApplicableIf: ["Has business or professional income"],
			},
			"ITR-3": {
				fullName: "ITR-3",
				applicableTo: [
				...
				notApplicableIf: ["Firms, companies, or other entities"],
			},
			"ITR-4": {
	...
	async parseForm16(
		fileBuffer: Buffer,
		fileName: string,
	): Promise<Form16OCRResponse> {
		try {
			if (!this.apiKey || !this.apiSecret) {
				throw new Error(
					"Sandbox ITR API not configured. Set SANDBOX_API_KEY and SANDBOX_API_SECRET for ITR services.",
				);
			}
			...
			if (!response.ok) {
				...
				throw new Error(
					`Form 16 OCR failed: ${response.status} ${response.statusText}`,
				);
			}
	...
	async parseForm26AS(
		...
		try {
			...
			if (!response.ok) {
				...
				throw new Error(
					`Form 26AS OCR failed: ${response.status} ${response.statusText}`,
				);
			}
			...
			return {
				success: false,
				message:
					error instanceof Error ? error.message : "Form 26AS OCR failed",
				errors: [error instanceof Error ? error.message : "Unknown error"],
			};
		}
	}
	...
	private transformForm26ASResponse(
		apiResponse: any,
	): Form26ASOCRResponse["data"] {
		...
		// Part III: TCS
		const partIII: any[] =
			data["Part III"] || data.part_iii || data.part_a2 || [];
		...
		const partA_TDS = partI.map((entry: any) => ({
			...
			amountPaid:
				Number.parseFloat(
					entry.total_amount_paid_credited || entry.amount_paid || "0",
				) || 0,
			tdsDeducted:
				Number.parseFloat(
					entry.total_tax_deducted || entry.tds_deducted || "0",
				) || 0,
			tdsDeposited:
				Number.parseFloat(
					entry.total_tds_deposited || entry.tds_deposited || "0",
				) || 0,
		...
		const partA2_TCS = partIII.map((entry: any) => ({
			...
			amountReceived:
				Number.parseFloat(
					entry.total_amount_paid_credited || entry.amount_received || "0",
				) || 0,
			tcsCollected:
				Number.parseFloat(
					entry.total_tax_deducted || entry.tcs_collected || "0",
				) || 0,
			tcsDeposited:
				Number.parseFloat(
					entry.total_tds_deposited || entry.tcs_deposited || "0",
				) || 0,
		}));

		// Compute totals from Part I for summary
		const totalTDSDeducted = partA_TDS.reduce(
			(sum: number, e: any) => sum + (e.tdsDeducted || 0),
			0,
		);
		const totalTCSCollected = partA2_TCS.reduce(
			(sum: number, e: any) => sum + (e.tcsCollected || 0),
			0,
		);
		...
		return {
			...
			partB_AdvanceTax: partIV.map((entry: any) => ({
				...
				amount:
					Number.parseFloat(
						entry.amount || entry.total_amount_paid_credited || "0",
					) || 0,
			})),
			...
			partE_AIRTransactions: partV.map((entry: any) => ({
				transactionType:
					entry.transaction_type || entry.nature_of_transaction || "",
	...
	async eVerifyITR(
		...
		try {
			...
			if (!response.ok) {
				throw new Error(
					`E-verify API returned ${response.status}: ${parsed.message || responseText.substring(0, 200)}`,
				);
			}
			...
			return {
				...
				message:
					error instanceof Error
						? error.message
						: "E-verification failed — Sandbox API error",
			};
		}
	}

	async calculateIndexedCost(
		items: Array<{
		...
		try {
			...
			const results = items.map((item) => {
				...
				const ciiSale =
					CII_VALUES[item.saleYear] || CII_VALUES["2024-25"] || 363;
				const indexedCost = Math.round(
					item.acquisitionCost * (ciiSale / ciiAcquisition),
				);
				return {
			...
			return {
				...
				message:
					"Indexed cost calculated using official Cost Inflation Index values",
			};
		} catch (error) {
			...
			return {
				...
				message:
					error instanceof Error
						? error.message
						: "Indexed cost calculation failed",
			};
		}
	}
	 ...
	 */
	async calculateTaxPnL(
		assetClass: "domestic" | "foreign" | "crypto" | "real_estate" | "other",
		transactions: Array<{
			symbol?: string;
			buyDate: string;
			sellDate: string;
		...
		try {
			...
			if (!jobId || !s3UploadUrl) {
				throw new Error(
					"Tax P&L job creation failed: no job_id or S3 URL in response",
				);
			}
			...
			console.error(
				`[Sandbox ITR] Tax P&L (${assetClass}) job creation failed:`,
				error,
			);
			return {
				success: false,
		...
		try {
			...
			return {
				...
				message:
					error instanceof Error
						? error.message
						: "Tax P&L job status check failed",
			};
		}
	}
	...
	async getCapitalGainsReport(
		...
		try {
			...
			const financialYear = ayMatch
				? `FY ${Number.parseInt(ayMatch[1]) - 1}-${ayMatch[2]}`
				: `FY 2024-25`;
			...
			if (!jobId) {
				throw new Error(
					"Capital gains report job creation failed: no job_id in response",
				);
			}
			return {
				success: true,
			...
			return {
				...
				message:
					error instanceof Error
						? error.message
						: "Capital gains report job creation failed",
			};
		}
	}
