...
const API_CONFIG = IS_PRODUCTION ? BSE_DIRECT_CONFIG.production : BSE_DIRECT_CONFIG.demo;
...
export class BSEDirectApiService {
  ...
  private validateCredentials(): boolean {
    if (IS_PRODUCTION) {
      return !!(BSE_DIRECT_CREDENTIALS.userId && 
   ...
   */
  async placeDirectOrder(request: DirectOrderRequest): Promise<DirectOrderResponse> {
    try {
      if (!this.validateCredentials()) {
        return {
          success: false,
          message: 'BSE Direct API credentials not configured. Contact administrator for ETI access.'
        };
      }
   ...
   */
  private simulateDirectOrder(request: DirectOrderRequest): DirectOrderResponse {
    ...
    return {
      ...
      executionDetails: {
        ...
        orderStatus: request.orderCategory === 'market' ? 'executed' : 'pending'
      }
    };
  }
   ...
   */
  async getMarketQuote(symbol: string, segment: string = 'equity'): Promise<MarketQuote | null> {
    try {
      if (!IS_PRODUCTION) {
        ...
        return [
          {
            ...
            segment: 'equity',
            quantity: 100,
            averagePrice: 3450.50,
            currentPrice: 3500.00,
            unrealizedPL: 4950.00,
            plPercent: 1.43
          },
          {
            symbol: 'RELIANCE',
