...
export async function runStartupSchemaRepairs() {
	...
	try {
		...
		// ── Fast-path for fund_financial_ratios columns (Scheme Level) ───────
		const _fundRatioCols: Array<[string, string]> = [
			["pe_ratio", "NUMERIC(10,2)"],
			["pb_ratio", "NUMERIC(10,2)"],
			["portfolio_turnover", "NUMERIC(10,2)"],
			["treynor_ratio", "NUMERIC(10,2)"],
		...
		// ── Fast-path for portfolio_metrics_daily (Client Portfolio Level) ───
		const _userPortfolioCols: Array<[string, string]> = [
			["aggregate_pe", "NUMERIC(10,4)"],
			["aggregate_pb", "NUMERIC(10,4)"],
			["weighted_expense_ratio", "NUMERIC(8,4)"],
		];
		for (const [col, colType] of _userPortfolioCols) {
			try {
				await migPool.query(`ALTER TABLE portfolio_metrics_daily ADD COLUMN IF NOT EXISTS "${col}" ${colType}`);
			} catch { /* table or col exists */ }
		}
...
crypto_status VARCHAR,
    ...
    `);
      ...
      VALUES
        ...
        ('psu-defence-atmanirbhar', 'PSU & Defence Atmanirbhar', 'India''s self-reliance mission — government capex + defence indigenisation', 'aggressive', 'thematic',
          '["capital_appreciation","thematic","government_capex"]', 15000, '5-7 years', 'Nifty India Defence Index', TO_CHAR(NOW() - INTERVAL '2 days', 'YYYY-MM-DD'), 'quarterly', 8, 'HAL, BEL, GRSE, Cochin Shipyard — India defence capex supercycle', '🪖', TRUE,
