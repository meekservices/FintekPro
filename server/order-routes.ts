...
export function registerOrderRoutes(app: Express) {
	...
	app.get(
		...
		async (req: Request, res: Response) => {
			try {
				...
				const documents =
					await orderManagementService.getOrderDocuments(orderId);
			...
			try {
				...
				if (!isAuthorized) {
					return res.status(403).json({
						success: false,
						error:
							"Forbidden: This endpoint is restricted to system administrators only",
						message:
							"Order status updates can only be performed by internal services or administrators",
					});
				}
	 ...
	 */
	app.post(
		"/api/orders/:orderId/balance-payment",
		async (req: Request, res: Response) => {
			try {
				const userId = (req as any).user?.id;
				if (!userId) {
				...
				const { orderId } = req.params;
				const { paymentAmount, paymentMethod, paymentGateway, transactionId } =
					req.body;
