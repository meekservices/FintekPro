...
class IBApiService extends EventEmitter {
	...
	private setupEventHandlers(): void {
		...
		this.ib.on(
			"position",
			(account: string, contract: any, position: number, avgCost: number) => {
				if (position !== 0) {
		...
		this.ib.on("accountSummaryEnd", (reqId: number) => {
			this.emit(
				...
				Array.from(this.accountSummary.values()),
			);
		});
...
const defaultIBConfig: IBConfig = {
	...
	port: Number.parseInt(process.env.IB_PORT || "7497"), // 7497 for paper trading, 7496 for live
	clientId: Number.parseInt(process.env.IB_CLIENT_ID || "1"),
