...
class AmfiValidationService {
			...
			return {
				...
				errorMessage:
					error instanceof Error
						? error.message
						: "Unknown error occurred during ARN validation",
			};
		}
	}
		...
		try {
			...
			if (!euinPattern.test(euinNumber.toUpperCase())) {
				return {
					...
					status: "failed",
					errorMessage:
						"Invalid EUIN format. Expected format: EXXXXXX (E followed by 6 digits)",
				};
			}
			...
			return {
				...
				errorMessage:
					error instanceof Error
						? error.message
						: "Unknown error occurred during EUIN validation",
			};
		}
	}
	...
	async getDistributorDetails(
		arnCode: string,
	): Promise<AmfiDistributorDetails | null> {
	...
	createVerificationLog(
		...
		const log: InsertAmfiVerificationLog = {
			...
			distributorName:
				result && "distributorDetails" in result
					? result.distributorDetails?.distributorName
					: undefined,
			distributorStatus:
				result && "distributorDetails" in result
					? result.distributorDetails?.distributorStatus
					: undefined,
			arnExpiryDate:
				result && "distributorDetails" in result
					? result.distributorDetails?.arnExpiryDate
					: undefined,
		};
