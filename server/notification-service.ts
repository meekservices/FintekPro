...
class NotificationService {
  ...
  async processNotifications(): Promise<void> {
    try {
      ...
      const pendingOnly = pendingNotifications.filter(n => n.status === 'pending');
  ...
  private async processNotification(notification: CkycNotificationTrigger): Promise<void> {
    try {
      console.log(`📤 Processing notification ${notification.id}: ${notification.subject}`);
      ...
      if (success) {
        console.log(`✅ Notification sent successfully: ${notification.subject}`);
      } else {
  ...
  private async sendEmail(notification: CkycNotificationTrigger): Promise<boolean> {
    try {
  ...
  private async sendSMS(notification: CkycNotificationTrigger): Promise<boolean> {
    try {
      ...
      if (!this.smsConfig.apiKey) {
        console.log(`📱 [SIMULATED] SMS sent to ${notification.recipientMobile}: ${notification.message}`);
        return true;
      }
      ...
      console.log(`📱 SMS sent to ${notification.recipientMobile}: ${notification.message}`);
      return true;
    } catch (error) {
  ...
  private async sendWhatsApp(notification: CkycNotificationTrigger): Promise<boolean> {
    try {
      if (!notification.recipientMobile) {
        throw new Error('No recipient mobile number provided');
      }
      ...
      if (whatsappService.isClientReady()) {
        await whatsappService.sendMessage(notification.recipientMobile, message);
        console.log(`📲 WhatsApp sent to ${notification.recipientMobile}: ${notification.subject}`);
        return true;
      } else {
        console.log(`📲 [SIMULATED] WhatsApp sent to ${notification.recipientMobile}: ${notification.subject}`);
        return true;
      }
    } catch (error) {
  ...
  async scheduleNotification(notification: Partial<CkycNotificationTrigger>, scheduledAt: Date): Promise<void> {
    try {
      ...
      console.log(`📅 Notification scheduled for ${scheduledAt.toLocaleString()}: ${notification.subject}`);
    } catch (error) {
  ...
  async processScheduledNotifications(): Promise<void> {
    try {
      ...
      const scheduledNotifications = allNotifications.filter(n => n.status === 'scheduled');
      ...
      for (const notification of scheduledNotifications) {
        if (notification.scheduledAt && new Date(notification.scheduledAt) <= now) {
          ...
          await storage.updateCkycNotificationStatus(notification.id, 'pending');
        }
      }
    } catch (error) {
  ...
  async getNotificationStats(): Promise<{
    ...
    try {
      ...
      return {
        ...
        sent: allNotifications.filter((n: CkycNotificationTrigger) => n.status === 'sent').length,
        pending: allNotifications.filter((n: CkycNotificationTrigger) => n.status === 'pending').length,
        failed: allNotifications.filter((n: CkycNotificationTrigger) => n.status === 'failed').length,
        scheduled: allNotifications.filter((n: CkycNotificationTrigger) => n.status === 'scheduled').length
      };
    } catch (error) {
    ...
    try {
      ...
      for (const notification of failedNotifications) {
        ...
        if (retryCount < maxRetries) {
          ...
          console.log(`🔄 Retrying notification ${notification.id} (attempt ${retryCount + 1}/${maxRetries})`);
        }
      }
    } catch (error) {
