...
export class ComprehensiveAIFPMSAPI {
	...
	async getComprehensiveAIFData(
		...
		try {
			...
			if (dbData.length > 0) {
				console.log(
					`[AIF] Returning ${dbData.length} funds from database (real data)`,
				);
				return dbData;
			}
			...
			const combinedData = this.combineAIFData(
				sebiData,
				pmsBazaarData,
				pmsWorldData,
			);
	...
	async getComprehensivePMSData(
		...
		try {
			...
			if (dbData.length > 0) {
				console.log(
					`[PMS] Returning ${dbData.length} portfolios from database (real data)`,
				);
				return dbData;
			}
			...
			const combinedData = this.combinePMSData(
				sebiData,
				pmsBazaarData,
				apmiData,
			);
	...
	private async fetchAIFFromDatabase(
		aifId?: string,
		category?: string,
	): Promise<ComprehensiveAIFData[]> {
		try {
			...
			const query = db.select().from(aifMaster);
	...
	private async fetchPMSFromDatabase(
		pmsId?: string,
		category?: string,
	): Promise<ComprehensivePMSData[]> {
		try {
			...
			const query = db.select().from(pmsMaster);
	...
	private mapDatabaseAIFToComprehensive(fund: any): ComprehensiveAIFData {
		return {
			...
			pastPerformance: {
				"1M": Number.parseFloat(fund.return1M) || 0,
				"3M": Number.parseFloat(fund.return3M) || 0,
				"6M": Number.parseFloat(fund.return6M) || 0,
				"1Y": Number.parseFloat(fund.return1Y) || 0,
				"3Y": Number.parseFloat(fund.return3Y) || 0,
				"5Y": Number.parseFloat(fund.return5Y) || 0,
				sinceInception: Number.parseFloat(fund.returnSinceInception) || 0,
				annualizedReturns: [],
			},
			startDate: fund.inceptionDate || "",
			fundTenure: "",
			lockInPeriod: fund.lockIn || "",
			minimumInvestment: Number.parseFloat(fund.minInvestment) || 10000000,
			targetCorpus: 0,
			currentAUM: Number.parseFloat(fund.aum) || 0,
			managementFee: Number.parseFloat(fund.managementFee) || 2,
			performanceFee: Number.parseFloat(fund.performanceFee) || 20,
			hurdle_rate: Number.parseFloat(fund.hurdleRate) || 0,
			highWaterMark: true,
			topHoldings: fund.metadata?.topHoldings || [],
			riskMetrics: {
				volatility: Number.parseFloat(fund.volatility) || 0,
				sharpeRatio: Number.parseFloat(fund.sharpeRatio) || 0,
				maxDrawdown: Number.parseFloat(fund.maxDrawdown) || 0,
				beta: Number.parseFloat(fund.beta) || 1,
				alpha: Number.parseFloat(fund.alpha) || 0,
				informationRatio: 0,
			},
			sebiCompliance: {
				lastInspectionDate: "",
	...
	private mapDatabasePMSToComprehensive(pms: any): ComprehensivePMSData {
		return {
			...
			pastPerformance: {
				...
				"3M": 0,
				"6M": 0,
				"1Y": Number.parseFloat(pms.returns1Y) || 0,
				"3Y": Number.parseFloat(pms.returns3Y) || 0,
				"5Y": Number.parseFloat(pms.returns5Y) || 0,
				sinceInception: Number.parseFloat(pms.returnsSinceInception) || 0,
				calendarYearReturns: [],
			},
			minimumInvestment: Number.parseFloat(pms.minimumInvestment) || 5000000,
			currentAUM: Number.parseFloat(pms.aum) || 0,
			managementFee: Number.parseFloat(pms.managementFee) || 2,
			performanceFee: Number.parseFloat(pms.performanceFee) || 0,
			exitLoad: Number.parseFloat(pms.exitLoad) || 0,
			topHoldings: pms.topHoldings || [],
			// sectorAllocation removed
			...(pms.sectorAllocation
				? ({ sectorAllocation: pms.sectorAllocation } as any)
				: {}),
			riskMetrics: {
				volatility: Number.parseFloat(pms.volatility) || 0,
				sharpeRatio: Number.parseFloat(pms.sharpeRatio) || 0,
				maxDrawdown: Number.parseFloat(pms.maxDrawdown) || 0,
				beta: Number.parseFloat(pms.beta) || 1,
				alpha: Number.parseFloat(pms.alpha) || 0,
				standardDeviation: 0,
				informationRatio: 0,
				sortinoRatio: 0,
			} as any,
	...
	async getAIFByFilters(filters: {
		...
		return allAIFs.filter((aif) => {
			...
			if (filters.subCategory && aif.subCategory !== filters.subCategory)
				return false;
			if (
				filters.fundManager &&
				!aif.fundManager.name
					.toLowerCase()
					.includes(filters.fundManager.toLowerCase())
			)
				return false;
			if (filters.minAUM && aif.currentAUM < filters.minAUM) return false;
			if (filters.maxAUM && aif.currentAUM > filters.maxAUM) return false;
			if (
				filters.minReturns1Y &&
				aif.pastPerformance["1Y"] < filters.minReturns1Y
			)
				return false;
			return true;
		});
	...
	async getPMSByFilters(filters: {
		...
		return allPMS.filter((pms) => {
			...
			if (filters.subCategory && pms.subCategory !== filters.subCategory)
				return false;
			if (
				filters.fundManager &&
				!pms.fundManager.name
					.toLowerCase()
					.includes(filters.fundManager.toLowerCase())
			)
				return false;
			...
			if (
				filters.minReturns1Y &&
				pms.pastPerformance["1Y"] < filters.minReturns1Y
			)
				return false;
			if (
				filters.investmentStyle &&
				pms.investmentStyle !== filters.investmentStyle
			)
				return false;
			return true;
		});
	...
	private async fetchSEBIAIFData(
		aifId?: string,
		category?: string,
	): Promise<any[]> {
		try {
			...
			if (line.trim() && !line.startsWith(";")) {
				...
				if (parts.length >= 6) {
					funds.push({
						...
						isin: parts[1],
						schemeName: parts[3],
						nav: Number.parseFloat(parts[4]) || 0,
						date: parts[5],
				...
				if (cells.length >= 3) {
					...
					if (regNumber && fundName) {
						aifData.push({
							...
							schemaName: fundName,
							category: fundType.includes("Category I")
								? "Category I"
								: fundType.includes("Category II")
									? "Category II"
									: "Category III",
		...
		try {
			...
			$(".performance-table tr, .fund-table tr").each((index, element) => {
				...
				if (cells.length >= 4) {
					...
					const returns1Y =
						Number.parseFloat($(cells[2]).text().replace("%", "")) || 0;
				...
				if (cells.length >= 5) {
					...
					if (pmName && regNumber) {
						pmsData.push({
							...
							sebiRegistrationNumber: regNumber,
							currentAUM: this.parseAUMString(aum),
							clientCount: Number.parseInt(clients) || 0,
			...
			$(".performance-report tr, table tr").each((index, element) => {
				...
				if (cells.length >= 6) {
					...
					const returns3M =
						Number.parseFloat($(cells[2]).text().replace("%", "")) || 0;
					const returns1Y =
						Number.parseFloat($(cells[3]).text().replace("%", "")) || 0;
					const returns3Y =
						Number.parseFloat($(cells[4]).text().replace("%", "")) || 0;
	...
	private parseAUMString(aumStr: string): number {
		...
		const multiplier = cleanStr.includes("crore")
			? 10000000
		...
		return Number.parseFloat(numStr) * multiplier || 0;
	}

	private combineAIFData(
		sebiData: any[],
		pmsBazaarData: any[],
		pmsWorldData: any[],
	): ComprehensiveAIFData[] {
		...
		sebiData.forEach((sebi) => {
			const existing = combinedData.find(
				(item) => item.sebiRegistrationNumber === sebi.sebiRegistrationNumber,
			);
	...
	private combinePMSData(
		sebiData: any[],
		pmsBazaarData: any[],
		apmiData: any[],
	): ComprehensivePMSData[] {
		...
		apmiData.forEach((apmi) => {
			...
			if (existing) {
				...
				existing.investmentStyle =
					apmi.investmentStyle || existing.investmentStyle;
			}
		});
	...
	private convertSEBIToPMS(sebiData: any): ComprehensivePMSData {
		return {
			...
			portfolioComposition: {
				...
				cashAllocation: 5,
				numberOfStocks: 20,
				portfolioTurnover: 15,
				averageMarketCap: 100000000000,
			},
			topHoldings: [],
			riskMetrics: {
				volatility: 0,
