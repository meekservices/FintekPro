import type { PartnerApplication, InsertPartnerApplication } from "@shared/schema";
...
export interface IProviderAdapter {
  ...
  validateData(data: UnifiedApplicationData): { isValid: boolean; errors: string[] };
...
export class BajajFinanceAdapter implements IProviderAdapter {
  ...
  getRequiredFields(): string[] {
    return [
      'firstName', 'lastName', 'panNumber', 'dateOfBirth', 'gender', 
      'email', 'mobile', 'currentAddress', 'currentCity', 'currentState', 
      'currentPincode', 'employmentType', 'monthlyIncome', 'loanAmount', 'tenure'
    ];
  }
  ...
  validateData(data: UnifiedApplicationData): { isValid: boolean; errors: string[] } {
    ...
    if (data.loanAmount < 25000 || data.loanAmount > 3000000) {
      errors.push('Loan amount must be between ₹25,000 and ₹30,00,000 for Bajaj Finance');
    }
    
    if (data.tenure < 12 || data.tenure > 84) {
      errors.push('Loan tenure must be between 12 and 84 months for Bajaj Finance');
    }
...
export class TataCapitalAdapter implements IProviderAdapter {
  ...
  validateData(data: UnifiedApplicationData): { isValid: boolean; errors: string[] } {
    ...
    if (data.loanAmount < 75000 || data.loanAmount > 2500000) {
      errors.push('Loan amount must be between ₹75,000 and ₹25,00,000 for Tata Capital');
    }
    
    if (data.tenure < 12 || data.tenure > 72) {
      errors.push('Loan tenure must be between 12 and 72 months for Tata Capital');
    }
...
export class HdfcBankAdapter implements IProviderAdapter {
  ...
  validateData(data: UnifiedApplicationData): { isValid: boolean; errors: string[] } {
    ...
    if (data.loanAmount < 100000 || data.loanAmount > 4000000) {
      errors.push('Loan amount must be between ₹1,00,000 and ₹40,00,000 for HDFC Bank');
    }
...
export class IcicieBankAdapter implements IProviderAdapter {
  ...
  validateData(data: UnifiedApplicationData): { isValid: boolean; errors: string[] } {
    ...
    if (data.loanAmount < 50000 || data.loanAmount > 5000000) {
      errors.push('Loan amount must be between ₹50,000 and ₹50,00,000 for ICICI Bank');
    }
    
    if (data.tenure < 12 || data.tenure > 72) {
      errors.push('Loan tenure must be between 12 and 72 months for ICICI Bank');
    }
...
export class ProviderAdapterRegistry {
  ...
  validateApplicationData(lenderName: string, data: UnifiedApplicationData): { isValid: boolean; errors: string[] } {
    const adapter = this.getAdapter(lenderName);
    return adapter.validateData(data);
  }

  transformToProviderFormat(lenderName: string, data: UnifiedApplicationData): ProviderApiRequest {
    const adapter = this.getAdapter(lenderName);
    return adapter.transformToProvider(data);
  }

  transformFromProviderFormat(lenderName: string, providerResponse: any): Partial<PartnerApplication> {
    const adapter = this.getAdapter(lenderName);
