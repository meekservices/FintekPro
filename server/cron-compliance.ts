...
export function initializeComplianceCrons(): void {
	...
	try {
		const auditCheckIntervalMinutes = Number.parseInt(
			process.env.AUDIT_INTEGRITY_CHECK_INTERVAL_MINUTES || "60",
			10,
		);
	...
	cron.schedule("30 0 1 1,4,7,10 *", async () => {
		...
		try {
			const { regulatoryReportingService } = await import(
				"./services/regulatory-reporting-service"
			);
	...
	cron.schedule("30 20 1 * *", async () => {
		...
		try {
			...
			for (const row of rows) {
				const liveTup = Number.parseInt(row.n_live_tup ?? "0", 10);
				const seqScan = Number.parseInt(row.seq_scan ?? "0", 10);
				const idxScan = Number.parseInt(row.idx_scan ?? "0", 10);
				const totalScans = seqScan + idxScan;
				const lastVacuumDate = row.last_autovacuum
					? new Date(row.last_autovacuum)
					: null;
				const ninetyDaysAgo = new Date(Date.now() - 90 * 24 * 60 * 60 * 1000);
				const vacuumedRecently =
					lastVacuumDate && lastVacuumDate > ninetyDaysAgo;
			...
			logger.error("[CRON][DbAudit] Monthly table audit failed", {
				error: error.message,
			});
		}
	});
	...
	cron.schedule("30 3 1 * *", async () => {
      ...
      `);
			...
			for (const u of users) {
				const expiryFormatted = new Date(u.expiryDate).toLocaleDateString(
					"en-IN",
					{
						day: "2-digit",
						month: "short",
	...
	cron.schedule("0 22 * * *", async () => {
		...
		try {
			...
			const expiredEntries =
				await caRegistryService.getExpiredRegistryEntries(50);
			logger.info(
				`[CRON][CARegistry] Found ${expiredEntries.length} entries due for revalidation`,
			);
				...
				try {
					...
					if (
						...
						(result.membershipStatus === "ACTIVE" ||
							result.membershipStatus === "FELLOW")
					) {
						successCount++;
					} else {
						failureCount++;
						await caRegistryService.markRevalidationFailed(
							entry.icaiMembershipNumber,
						);
					}
			...
			logger.info(
				`[CRON][CARegistry] Revalidation complete: ${successCount} re-verified, ${failureCount} failed/stale`,
			);
		} catch (error: any) {
	...
	cron.schedule("30 22 * * *", async () => {
		...
		try {
			...
			logger.info(
				`[CRON][RegulatoryIntegrity] Check complete. Status: ${result.status}, Verified: ${result.verifiedRecords}/${result.totalRecords}`,
			);

			if (result.status === "failed") {
				logger.error(
					`[CRON][RegulatoryIntegrity] CRITICAL: Tampering detected in regulatory audit packs! Check IDs: ${result.checksumMismatches.join(", ")}`,
				);
			}
		} catch (error: any) {
			logger.error("[CRON][RegulatoryIntegrity] Job failed:", {
				error: error.message,
			});
		}
	});
	...
	cron.schedule("0 23 * * *", async () => {
      ...
      `);
			...
			logger.info(
				`[CRON][RevenueDrift] Scanning ${rows.length} recent transactions for configuration drifts...`,
			);
			...
			for (const row of rows) {
				if (
					!row.platform_config_snapshot ||
					Object.keys(row.platform_config_snapshot).length === 0
				) {
					driftCount++;
					logger.warn(
						`[CRON][RevenueDrift] Potential drift or missing config in pack ${row.id}`,
					);
				}
			}
			...
			if (driftCount > 0) {
				logger.warn(
					`[CRON][RevenueDrift] Detected ${driftCount} packs with configuration anomalies.`,
				);
			} else {
				logger.info(
					"[CRON][RevenueDrift] No configuration drifts detected in the last 24 hours.",
				);
			}
		} catch (error: any) {
			logger.error("[CRON][RevenueDrift] Job failed:", {
				error: error.message,
			});
		}
	});
	...
	cron.schedule("0 * * * *", async () => {
      ...
      `);
			...
			logger.info(
				`[CRON][eSignSweep] Checking ${rows.length} stuck eSign transactions...`,
			);

			const { truthScreenESignService } = await import(
				"./services/truthscreen-esign-service"
			);

			for (const row of rows) {
				try {
					const status = await truthScreenESignService.checkStatus(
						row.transaction_id as string,
					);
					logger.info(
						`[CRON][eSignSweep] ${row.transaction_id}: ${status.status}`,
					);
				} catch (e: any) {
					logger.warn(
						`[CRON][eSignSweep] Could not check ${row.transaction_id}: ${e?.message}`,
					);
				}
			}
		} catch (err: any) {
	...
	cron.schedule("30 3 * * *", async () => {
      ...
      `);
			...
			logger.info(
				`[CRON][eSignExpiry] Found ${workflows.length} workflows needing reminders`,
			);
			let sent = 0;
			let skipped = 0;
			...
			for (const w of workflows) {
				...
				const deadlineFormatted = new Date(w.deadline).toLocaleDateString(
					"en-IN",
					{
						day: "2-digit",
						month: "short",
				...
				try {
					...
					if (result.success) sent++;
						...
						skipped++;
						logger.warn(
							`[CRON][eSignExpiry] WhatsApp failed for workflow ${w.workflowId}: ${result.error}`,
						);
					}
				} catch (e: any) {
					skipped++;
					logger.error(
						`[CRON][eSignExpiry] Send error for workflow ${w.workflowId}: ${e?.message}`,
					);
				}
			}

			logger.info(
				`[CRON][eSignExpiry] Reminders complete — sent: ${sent}, skipped/failed: ${skipped}`,
			);
		} catch (err: any) {
