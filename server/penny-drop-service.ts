...
async function verifyBankAccountViaCashfree(
	...
	console.log(
		`🏦 [BankVerify] Using Cashfree VRS (Sandbox.co.in credentials not configured)`,
	);
	...
	if (!result.success) {
		return {
			success: false,
			errorMessage:
				result.error || "Bank verification failed via Cashfree Secure ID",
	...
	const nameAtBank: string =
		data.name_at_bank || data.account_holder_name || "";
	...
	const nameMatchScore = nameAtBank
		? calculateNameMatchScore(accountHolderName, nameAtBank)
		: undefined;
	...
	if (!nameAtBank) {
		return {
			success: false,
			errorMessage:
				data.message || "Could not retrieve account holder name from bank",
			providerResponse: result.data,
		};
	}

	console.log(
		`✅ [BankVerify] Cashfree VRS succeeded. Name at bank: "${nameAtBank}", match: ${nameMatchScore ?? "N/A"}%`,
	);
	return {
		success: true,
...
export async function verifyBankAccountPennyDrop(
	...
	if (!sandboxApiKey || !sandboxApiSecret) {
		if (hasCashfreeSecureIDCredentials()) {
			return verifyBankAccountViaCashfree(
				accountNumber,
				ifscCode,
				accountHolderName,
			);
		...
		return {
			success: false,
			errorMessage:
				"Bank verification service not configured. Contact support.",
			providerResponse: {
				error:
					"No credentials: SANDBOX_API_KEY or CASHFREE_VERIFICATION_APP_ID required",
			},
		};
	}
	...
	try {
		...
		console.log(
			`🏦 Initiating penny drop for account: ${accountNumber.slice(-4)} (${SANDBOX_BASE_URL})`,
		);
		...
		if (apiResponse.code === 200 && apiResponse.data) {
			const { account_exists, name_at_bank, account_status, amount_deposited } =
				apiResponse.data;
			...
			console.log(
				`✅ Penny drop verification successful. Name match: ${nameMatchScore}%`,
			);
			...
			return {
				...
				accountStatus:
					(account_status as "active" | "inactive" | "dormant") || "active",
				verifiedName: name_at_bank,
				nameMatchScore,
				amount: Number(amount_deposited) || 1.0,
				providerResponse: apiResponse,
			};
		}
		...
		return {
			...
			errorMessage:
				apiResponse.message ||
				(apiResponse.error as any)?.message ||
				"Unexpected response from bank verification service",
			providerResponse: apiResponse,
		};
	} catch (error: any) {
		if (
			...
			clearSandboxToken();
			console.error(
				"❌ Penny drop auth failed — token cleared for retry:",
				error.response?.data?.message || error.message,
			);
			return {
				success: false,
				errorMessage:
					"Bank verification authentication failed. Please try again.",
...
export function isNameMatchAcceptable(
	score: number,
	threshold: number = 80,
): boolean {
