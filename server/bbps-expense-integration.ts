...
export class BbpsExpenseIntegration {
  ...
  private mapBbpsCategoryToExpenseCategory(bbpsCategoryCode: string): {
    ...
    const categoryMap: Record<string, { category: string; subcategory?: string }> = {
      ...
      'TELECOM_POSTPAID': { category: 'utilities', subcategory: 'Mobile Postpaid' },
      ...
      'HEALTH_INSURANCE': { category: 'insurance', subcategory: 'Health Insurance' },
      'VEHICLE_INSURANCE': { category: 'insurance', subcategory: 'Vehicle Insurance' },
      'GENERAL_INSURANCE': { category: 'insurance', subcategory: 'General Insurance' },
      ...
      // Education
      'EDUCATION_FEES': { category: 'education', subcategory: 'School/College Fees' },
    ...
    return categoryMap[bbpsCategoryCode] || { category: 'other', subcategory: 'Bill Payment' };
  }

  // Auto-create expense when BBPS payment is successful
  async createExpenseFromBbpsPayment(paymentData: BbpsPaymentCompleteData): Promise<void> {
    try {
      // Get category mapping
      const { category, subcategory } = this.mapBbpsCategoryToExpenseCategory(paymentData.categoryCode);
      ...
      console.log(`✅ Created expense from BBPS payment: ${paymentData.transactionId}`);
    } catch (error) {
  ...
  async getBillSpendingInsights(userId: string, months: number = 6): Promise<any> {
    try {
      ...
      return {
        totalBillsSpent: recentBbpsExpenses.reduce((sum, e) => sum + parseFloat(e.amount), 0),
