...
async function seedAgentCommissions() {
	...
	for (const clientId of CLIENT_IDS) {
		try {
			await db
				.insert(clientAgentRelationships)
				.values({
					id: nanoid(),
					clientId,
					agentId: AGENT_ID,
	...
	for (let i = 0; i < recordCount; i++) {
		...
		const settlementStatus =
			daysSinceTransaction > 30 && Math.random() > 0.3 ? "settled" : "pending";
	...
	for (let i = 0; i < commissionRecords.length; i += batchSize) {
		...
		console.log(
			`✅ Inserted batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(commissionRecords.length / batchSize)}`,
		);
	}
	...
	const pendingTotal = commissionRecords
		...
		.reduce((sum, r) => sum + Number.parseFloat(r.agentNetCommission), 0);
	...
	console.log(`📊 Summary:`);
	console.log(
		`   Pending Commissions: ₹${Math.round(pendingTotal).toLocaleString()}`,
	);
	console.log(
		`   Settled Commissions: ₹${Math.round(settledTotal).toLocaleString()}`,
	);
	console.log(
		`   Total Revenue: ₹${Math.round(pendingTotal + settledTotal).toLocaleString()}`,
	);
}
