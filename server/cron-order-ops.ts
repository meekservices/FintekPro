  ...
  cron.schedule('0 */6 * * *', async () => {
    ...
    try {
      ...
      for (const order of staleOrders) {
        try {
          await db.update(unifiedOrders).set({
            status: 'expired',
      ...
      console.log(`[CRON] Stale order cleanup: ${cancelled} expired, ${failed} failed`);
    } catch (error: any) {
  ...
  cron.schedule('15 * * * *', async () => {
    ...
    try {
      ...
      const stuckOrders = await db.select({
        ...
        and(eq(unifiedOrders.status, 'processing'), lt(unifiedOrders.updatedAt, fourHoursAgo))
      ).limit(50);
      ...
      console.log(`[CRON] Stuck order check: ${flagged} orders flagged for review`);
    } catch (error: any) {
