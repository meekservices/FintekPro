...
function recordFailure(status?: number): void {
	...
	errorTrackingService
		.ingestError({
			...
			message:
				`Python service call failed` + (status ? ` with status ${status}` : ""),
			context: {
				module: "PythonClient",
...
function recordSuccess(): void {
	if (circuitFailures > 0) {
		console.info(
			"[PythonClient] ✅ Circuit CLOSED — Python service healthy again",
		);
	}
	circuitFailures = 0;
	circuitOpenUntil = 0;
...
function issuePythonToken(user: any): string {
	const secret =
		process.env.PYTHON_SERVICE_SECRET || process.env.SESSION_SECRET;
...
export async function probePythonHealth(): Promise<boolean> {
	...
	if (!baseUrl) {
		console.info(
			"[PythonClient] ℹ️  PYTHON_SERVICE_URL not set — Python features are disabled",
		);
		return false;
	}
	...
	try {
		...
		if (res.ok) {
			...
			console.info(
				`[PythonClient] ✅ Python service healthy at ${baseUrl}` +
					(caps.length
						? ` — capabilities: ${caps.slice(0, 5).join(", ")}${caps.length > 5 ? "…" : ""}`
						: ""),
			);
			return true;
		}
		console.warn(
			`[PythonClient] ⚠️  Python health probe → HTTP ${res.status} from ${url}`,
		);
		...
		console.warn(
			`[PythonClient] ❌ Python health probe failed: ${err.message}. ${tip}`,
		);
		recordFailure();
		return false;
	}
...
export function startPythonKeepAlive(): void {
	...
	const ping = async () => {
		try {
			...
			if (res.ok) {
				if (!lastWasHealthy) {
					console.info(
						"[PythonClient] ✅ Keep-alive: Python service recovered",
					);
				}
				...
				if (lastWasHealthy) {
					console.warn(
						`[PythonClient] ⚠️  Keep-alive ping → HTTP ${res.status} from ${baseUrl}/health`,
					);
				}
			...
			if (lastWasHealthy) {
				console.warn(
					`[PythonClient] ⚠️  Keep-alive ping failed: ${err.message}`,
				);
			}
			lastWasHealthy = false;
			recordFailure();
	...
	console.info(
		`[PythonClient] 🔄 Keep-alive active — pinging ${baseUrl}/health every 5 min`,
	);
...
export async function callPython<T = any>(
	...
	try {
		...
		if (!res.ok) {
			...
			if (circuitFailures < CIRCUIT_OPEN_THRESHOLD) {
				console.warn(
					`[PythonClient] ${method} ${path} → HTTP ${res.status}: ${summary}`,
				);
			}
			recordFailure(res.status);
			return null;
		...
		if (circuitFailures < CIRCUIT_OPEN_THRESHOLD) {
			console.warn(
				`[PythonClient] ${method} ${path} unreachable: ${err.message}`,
			);
		}
		recordFailure();
		return null;
