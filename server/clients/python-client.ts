 ...
 * (python.fintekpro.com / localhost:8000 in dev).
 *
...
const BASE_URL = process.env.PYTHON_SERVICE_URL?.replace(/\/$/, '') || '';
...
export async function proxyToPython(req: Request, res: Response, path: string): Promise<void> {
  if (!BASE_URL) {
    res.status(503).json({
      ...
      hint: 'Deploy services/python/ as a separate Replit and set PYTHON_SERVICE_URL to its URL.',
    });
    return;
  }
