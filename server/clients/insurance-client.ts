...
export async function proxyToInsurance(
	req: Request,
	res: Response,
	path: string,
): Promise<void> {
	if (!BASE_URL) {
		res.status(503).json({
			...
			detail:
				"Set INSURANCE_SERVICE_URL environment variable to enable the insurance micro-service.",
		});
		return;
	}
	...
	try {
		...
		res
			.status(502)
			.json({ error: "Insurance Service unreachable", detail: err.message });
	}
}
