	...
	return async (req: KYCRequest, res: Response, next: NextFunction) => {
		try {
			...
			if (!amount || amount <= 0) {
				if (!options.defaultAmount) {
					...
					return res.status(400).json({
						...
						message:
							"Transaction amount is required and must be greater than zero",
					});
				}
			}
			...
			if (options.skipForDemo && process.env.NODE_ENV === "development") {
				console.log(
					`[KYC] Demo mode: Skipping KYC check for ${transactionType} (₹${amount})`,
				);
				await logKYCAttempt(
					userId,
					transactionType,
			...
			await logKYCAttempt(
				...
				amount,
				"passed",
				`${kycResult.level.toUpperCase()} KYC verified`,
			);
			console.log(
				`[KYC] ✓ User ${userId} passed ${kycResult.level} KYC for ${transactionType} (₹${amount})`,
			);
...
export function validateProductAccess(
	...
	return async (req: KYCRequest, res: Response, next: NextFunction) => {
		try {
			...
			const product = PRODUCT_ELIGIBILITY_MATRIX.find(
				(p) => p.productCode === productCategory,
			);

			if (!product) {
				console.warn(
					`[Product Access] Unknown product category: ${productCategory}`,
				);
			...
			if (
				userKycTier === "basic" &&
				product.maxInvestmentWithoutEnhanced &&
				transactionAmount > product.maxInvestmentWithoutEnhanced
			) {
			...
			if (
				...
				(!req.user.annualIncomeAmount ||
					req.user.annualIncomeAmount < product.minAnnualIncome)
			) {
				missingVerifications.push(
					`Minimum Annual Income ₹${(product.minAnnualIncome / 100000).toFixed(0)} Lakh`,
				);
			}

			if (
				product.minNetWorth &&
				(!req.user.netWorthAmount ||
					req.user.netWorthAmount < product.minNetWorth)
			) {
				missingVerifications.push(
					`Minimum Net Worth ₹${(product.minNetWorth / 10000000).toFixed(1)} Crore`,
				);
			}
			...
			console.log(
				`[Product Access] ✓ User ${userId} authorized for ${product.productName} (${userKycTier} tier)`,
			);
			next();
		} catch (error) {
...
export function blockSubAgentTransactions() {
	return async (req: Request, res: Response, next: NextFunction) => {
		try {
			...
			console.log(
				`[Sub-Agent Control] Blocked transaction attempt by sub-agent ${agent.id} (${userId})`,
			);
			...
			return res.status(403).json({
				...
				message:
					"Sub-agents cannot execute transactions. You can refer clients to master agents for transaction execution.",
				data: {
					...
					contactSupport:
						"Contact your master agent to execute transactions on behalf of referred clients",
				},
			});
		} catch (error) {
...
export function allowSubAgentProductViewing() {
	return async (req: KYCRequest, res: Response, next: NextFunction) => {
		try {
			...
			if (agent && agent.agentLevel === "sub_agent") {
				console.log(
					`[Sub-Agent Access] Allowing product view for sub-agent ${agent.id}`,
				);
...
export async function logKYCAttempt(
	...
	try {
		...
		console.error(
			"[KYC Audit] Failed to persist audit log to DB:",
			dbErr?.message,
		);
	}
}
