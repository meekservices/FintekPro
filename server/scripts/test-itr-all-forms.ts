...
async function testITRForm(testCase: typeof TEST_CASES[0]): Promise<TestResult> {
  ...
  try {
    const { status, data } = await apiCall("POST", "/api/sandbox-itr/suggest-form", {
  ...
  try {
    ...
    if (status === 200 && data.success) {
      ...
      if (status !== 200) result.overallStatus = result.overallStatus === "PASS" ? "PARTIAL" : result.overallStatus;
    }
  } catch (err) {
    result.steps.push({
      name: "Wizard Tax Calculation",
...
async function runAllTests() {
  ...
  try {
    ...
    console.log(`📡 Sandbox API Status: ${data.configured ? "✅ CONFIGURED" : "❌ NOT CONFIGURED"}`);
    console.log(`   Message: ${data.message}\n`);
    if (!data.configured) {
      console.error("FATAL: Sandbox API not configured. Set SANDBOX_API_KEY and SANDBOX_API_SECRET.");
      process.exit(1);
    }
  } catch (err) {
  ...
  for (const testCase of TEST_CASES) {
    ...
    console.log(`   Email: ${testCase.email} | Entity: ${testCase.entityType} | PAN: ${testCase.pan}`);
  ...
  const partial = allResults.filter((r) => r.overallStatus === "PARTIAL").length;
  const failed = allResults.filter((r) => r.overallStatus === "FAIL").length;

  for (const r of allResults) {
    const icon =
      r.overallStatus === "PASS" ? "✅" : r.overallStatus === "FAIL" ? "❌" : "⚠️";
  ...
  if (failures.length > 0) {
    ...
    for (const f of failures) {
      console.log(`  - ${f.itrForm} (${f.entityType}) → ${f.step}: ${f.detail}`);
    }
    console.log("");
  }
