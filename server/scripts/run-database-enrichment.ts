...
async function runEnrichmentReport() {
  ...
  `);
	...
	console.log(
		`With Returns: ${mf.has_returns} (${((mf.has_returns / mf.total) * 100).toFixed(1)}%)`,
	);
	console.log(
		`Missing 1Y Returns: ${mf.missing_1y} (${((mf.missing_1y / mf.total) * 100).toFixed(1)}%)`,
	);
	console.log(
		`Missing Sharpe Ratio: ${mf.missing_sharpe} (${((mf.missing_sharpe / mf.total) * 100).toFixed(1)}%)`,
	);
	console.log(
		`Missing Expense Ratio: ${mf.missing_expense} (${((mf.missing_expense / mf.total) * 100).toFixed(1)}%)`,
	);
	console.log(
		`Missing Category: ${mf.missing_category} (${((mf.missing_category / mf.total) * 100).toFixed(1)}%)`,
	);
	console.log(
		`Missing ISIN: ${mf.missing_isin} (${((mf.missing_isin / mf.total) * 100).toFixed(1)}%)`,
	);
  ...
  `);
	...
	console.log(
		`Missing PE Ratio: ${stocks.missing_pe} (${((stocks.missing_pe / stocks.total) * 100).toFixed(1)}%)`,
	);
	console.log(
		`Missing Dividend Yield: ${stocks.missing_dividend} (${((stocks.missing_dividend / stocks.total) * 100).toFixed(1)}%)`,
	);
	console.log(
		`Missing Market Cap: ${stocks.missing_market_cap} (${((stocks.missing_market_cap / stocks.total) * 100).toFixed(1)}%)`,
	);
	console.log(
		`Missing Sector: ${stocks.missing_sector} (${((stocks.missing_sector / stocks.total) * 100).toFixed(1)}%)`,
	);
  ...
  `);
	...
	console.log(
		`Missing CIN: ${unlisted.missing_cin} (${((unlisted.missing_cin / unlisted.total) * 100).toFixed(1)}%)`,
	);
	console.log(
		`Missing Sector: ${unlisted.missing_sector} (${((unlisted.missing_sector / unlisted.total) * 100).toFixed(1)}%)`,
	);
	console.log(
		`Missing Buy Price: ${unlisted.missing_buy_price} (${((unlisted.missing_buy_price / unlisted.total) * 100).toFixed(1)}%)`,
	);
	console.log(
		`Missing Sell Price: ${unlisted.missing_sell_price} (${((unlisted.missing_sell_price / unlisted.total) * 100).toFixed(1)}%)`,
	);
  ...
  `);
	...
	if (Number(mf.missing_1y) > 100) {
		recommendations.push(
			`1. MF Returns: ${mf.missing_1y} funds need returns calculation from MFAPI`,
		);
	}
	if (Number(stocks.missing_pe) > 100) {
		recommendations.push(
			`2. Stock Metrics: ${stocks.missing_pe} stocks need PE/dividend from Yahoo Finance`,
		);
	}
	if (Number(unlisted.missing_buy_price) > 0) {
		recommendations.push(
			`3. Unlisted Pricing: ${unlisted.missing_buy_price} companies need price suggestions`,
		);
	}
	if (Number(alt.reits) < 10) {
		recommendations.push(
			`4. REITs: Only ${alt.reits} REITs - need to expand from NSE/BSE feeds`,
		);
	}
	if (Number(op.transactions) === 0) {
		recommendations.push(
			`5. Demo Data: Operational tables empty - need seed data for development`,
		);
	}
...
async function runMutualFundEnrichment(maxFunds: number = 100) {
	...
	console.log(
		`✅ MF Enrichment Complete: ${result.successful}/${result.processed} successful, ${result.failed} failed`,
	);
...
async function runMFExtendedEnrichment(forceRefresh: boolean = false) {
	...
	const result = await mfExtendedEnrichmentService.enrichAllFunds({
		forceRefresh,
	});
	console.log(
		`✅ MF Extended Enrichment Complete: ${result.terUpdated} TER, ${result.aumUpdated} AUM updated`,
	);
...
async function runStockEnrichment() {
	...
	const result = await stockFinancialEnrichmentService.enrichAllStocks({
		useFmp: true,
		maxFmpStocks: 40,
		includeReturns: true,
	});
...
async function runUnlistedPricingEnrichment() {
  ...
  `);
	...
	for (const company of results.rows as any[]) {
		...
		if (company.networth && Number.parseFloat(company.networth) > 0) {
			buyPrice = Math.round(Number.parseFloat(company.networth) / 10000000); // 1 share ~ net worth / 10M
		} else if (company.pe_ratio && company.revenue) {
			const earnings = Number.parseFloat(company.revenue) * 0.1; // Assume 10% margin
			buyPrice = Math.round(
				(earnings / 10000000) * Number.parseFloat(company.pe_ratio),
			);
		} else {
      ...
      `);
			enriched++;
			console.log(
				`  Updated ${company.name}: Buy ₹${buyPrice}, Sell ₹${sellPrice}`,
			);
		}
	}
...
const maxFunds = Number.parseInt(args[1]) || 100;
...
switch (command) {
	...
	case "mf":
		...
		break;
	case "mf-extended":
		runMFExtendedEnrichment(args.includes("--force")).then(() =>
			process.exit(0),
		);
		break;
	case "stocks":
