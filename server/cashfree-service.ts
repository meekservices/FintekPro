...
export class CashfreeService {
	...
	constructor() {
		...
		if (!this.appId || !this.secretKey) {
			console.warn(
				"⚠️ Cashfree PG credentials (CASHFREE_PG_APP_ID, CASHFREE_PG_SECRET_KEY) not configured",
			);
			console.warn(
				"⚠️ Cashfree Payment Gateway APIs will not function. Set credentials to enable payments.",
			);
		}
	...
	async createOrder(
		...
		try {
			...
			const returnUrl =
				...
				`${getAppBaseUrl()}/api/payments/cashfree/callback`;
			...
			const requestBody = {
				...
				customer_details: {
					...
					customer_email:
						paymentRequest.email || `${paymentRequest.userId}@example.com`,
			...
			console.log("Creating Cashfree v5 order:", {
				orderId,
			...
			if (statusCode === 401 || statusCode === 403) {
				userMessage =
					"Payment gateway authentication failed. Please verify API credentials.";
	...
	async getSettlements(orderId?: string): Promise<any[]> {
		try {
			const path = orderId
				? `/settlements?order_id=${orderId}`
				: "/settlements";
			const response = await this.apiClient.get(path);
			return response.data || [];
		} catch (error: any) {
			console.error(
				"Cashfree settlements error:",
				error.response?.data || error.message,
			);
			return [];
		}
	...
	async verifyBankAccount(
		...
		try {
			...
			console.error(
				"Cashfree bank verification error:",
				error.response?.data || error.message,
			);
			return {
				success: false,
	...
	private calculateNameMatchScore(name1: string, name2: string): number {
		...
		const matrix = Array(n1.length + 1)
			.fill(null)
			.map(() => Array(n2.length + 1).fill(null));
	...
	async createEMandate(
		...
		try {
			...
			return {
				success: false,
				status: "FAILED",
				message: response.data.message || "eMandate creation failed",
			};
		} catch (error: any) {
			console.error(
				"Cashfree eMandate error:",
				error.response?.data || error.message,
			);
