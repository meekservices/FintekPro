...
router.get("/api/admin/users", requireAdmin, async (req, res) => {
	try {
		...
		const limitNum = Number.parseInt(limit as string) || 50;
		...
		const agentIds = [
			...new Set(
				rawResults.filter((u) => u.agentId).map((u) => u.agentId as string),
			),
		];
...
router.patch("/api/admin/users/:id", requireAdmin, async (req, res) => {
	try {
		...
		if (validatedData.panNumber !== undefined)
			updateData.panNumber = validatedData.panNumber;
...
router.post(
	"/api/admin/users/batch-activate",
	requireAdmin,
	async (req, res) => {
		try {
			const { ids } = req.body;
...
router.get("/api/admin/users-stats", requireAdmin, async (req, res) => {
	try {
		...
		const [adminResult] = await db
			...
			.from(users)
			.where(
				sql`'admin' = ANY(${users.roles}) OR 'superadmin' = ANY(${users.roles})`,
			);
