...
export abstract class LoanProviderAdapter {
	...
	protected validateLoanRequest(request: LoanOfferRequest): string[] {
		...
		if (request.preferredTenure <= 0)
			errors.push("Preferred tenure must be greater than 0");
...
export class ProviderAdapterFactory {
	...
	static register(
		...
		ProviderAdapterFactory.adapters.set(providerKey, adapterClass);
	}

	static create(
		provider: LoanProvider,
		integration: any,
	): LoanProviderAdapter | null {
		const AdapterClass = ProviderAdapterFactory.adapters.get(
			provider.providerKey,
		);
		if (!AdapterClass) {
			console.warn(
				`No adapter registered for provider: ${provider.providerKey}`,
			);
			return null;
		}
	...
	static getRegisteredProviders(): string[] {
		return Array.from(ProviderAdapterFactory.adapters.keys());
	}
}
...
export class ProviderRegistry {
	private providers: Map<
		string,
		{ provider: LoanProvider; adapter: LoanProviderAdapter; integration: any }
	> = new Map();
