...
export class LoanOrchestrator {
		...
		try {
			...
			if (!creditProfile) {
				return {
					...
					errors: [
						"Credit profile not found. Please complete your profile first.",
					],
				};
			}
			...
			const suitableProviders = await this.getSuitableProviders(
				productKey,
				requestedAmount,
				creditProfile,
			);
			...
			const providerOffers = await this.generateOffersFromProviders(
				suitableProviders,
				offerRequest,
			);

			// 6. Process and rank offers
			const rankedOffers = await this.rankOffers(
				providerOffers,
				loanRequest.id,
			);
			...
			return {
				...
				metadata: {
					...
					providersResponded: providerOffers.filter(
						(o) => o.success && o.offers.length > 0,
					).length,
					avgResponseTime,
					timestamp: new Date(),
				},
	...
	private async getCreditProfile(
		userId: string,
	...
	private async createLoanRequest(data: {
		...
		const requestData = {
			...
			estimatedCollateralValue:
				data.collateralDetails?.estimatedValue?.toString(),
	...
	> {
		...
		for (const { provider, adapter, integration } of activeProviders) {
			try {
				...
				const providerProducts = await storage.getProviderProductsByProvider(
					provider.id,
					productKey,
				);
				...
				const isEligible = this.checkBasicEligibility(
					providerProducts,
					requestedAmount,
					creditProfile,
				);
				...
				console.error(
					`Error evaluating provider ${provider.providerName}:`,
					error,
				);
	...
	private checkBasicEligibility(
		...
		return products.some((product) => {
			...
			const minAmount = product.minAmount
				? Number.parseFloat(product.minAmount.toString())
				: 0;
			const maxAmount = product.maxAmount
				? Number.parseFloat(product.maxAmount.toString())
	...
	private async generateOffersFromProviders(
		...
		const promises = providers.map(({ provider, adapter, products }) =>
			this.getOfferFromProvider(adapter, request).catch((error) => {
	...
	private async rankOffers(
		...
		const sortedOffers = scoredOffers.sort(
			(a, b) => b.recommendationScore - a.recommendationScore,
		);

		// Convert to ranked offers
		const rankedOffers: RankedLoanOffer[] = sortedOffers.map(
			(offer, index) => ({
				...offer,
				id: "", // Will be set when stored in DB
				rank: index + 1,
				reasonsForRanking: this.generateRankingReasons(
					offer,
					index === 0,
					allOffers,
				),
	...
	private calculateRecommendationScore(
		offer: ProviderLoanOffer,
		allOffers: ProviderLoanOffer[],
	): number {
		let score = 0;
		...
		const rateScore =
			maxRate === minRate
				? 100
				: ((maxRate - offer.interestRate) / (maxRate - minRate)) * 100;
		score += rateScore * weights.interestRate;

		// Processing fee score (lower is better)
		const processingFees = allOffers.map(
			(o) => (o.processingFee / o.approvedAmount) * 100,
		);
		const minFee = Math.min(...processingFees);
		const maxFee = Math.max(...processingFees);
		const currentFeePercent =
			(offer.processingFee / offer.approvedAmount) * 100;
		const feeScore =
			maxFee === minFee
				? 100
				: ((maxFee - currentFeePercent) / (maxFee - minFee)) * 100;
		...
		const disbursalScore = offer.expectedDisbursalTime.includes("24 hours")
			? 100
			: offer.expectedDisbursalTime.includes("2-3 days")
				? 80
				: offer.expectedDisbursalTime.includes("week")
					? 60
					: 40;
	...
	private generateRankingReasons(
		...
		const sortedByRate = allOffers.sort(
			(a, b) => a.interestRate - b.interestRate,
		);
		const rateRank =
			sortedByRate.findIndex((o) => o.providerId === offer.providerId) + 1;
	...
	private identifyAdvantages(
		...
		const avgRate =
			allOffers.reduce((sum, o) => sum + o.interestRate, 0) / allOffers.length;
		if (offer.interestRate < avgRate) {
			advantages.push(
				`Interest rate ${(avgRate - offer.interestRate).toFixed(2)}% below average`,
			);
		}

		const avgFee =
			allOffers.reduce(
				(sum, o) => sum + (o.processingFee / o.approvedAmount) * 100,
				0,
			) / allOffers.length;
		const offerFeePercent = (offer.processingFee / offer.approvedAmount) * 100;
		if (offerFeePercent < avgFee) {
			advantages.push(
				`Processing fee ${(avgFee - offerFeePercent).toFixed(2)}% below average`,
			);
		}
	...
	private identifyDisadvantages(
		...
		if (offer.interestRate > bestRate) {
			...
			if (diff > 1) {
				disadvantages.push(
					`Interest rate ${diff.toFixed(2)}% higher than best available`,
				);
			}
		}

		const minFee = Math.min(
			...allOffers.map((o) => (o.processingFee / o.approvedAmount) * 100),
		);
		const offerFeePercent = (offer.processingFee / offer.approvedAmount) * 100;
	...
	private async storeOffers(
		rankedOffers: RankedLoanOffer[],
		requestId: string,
	): Promise<void> {
		for (const offer of rankedOffers) {
			try {
				const offerData: InsertLoanOffer = {
					...
					validUntil: new Date(
						Date.now() + offer.validityDays * 24 * 60 * 60 * 1000,
					),
				};
	...
	async getOffersForRequest(requestId: string): Promise<RankedLoanOffer[]> {
		try {
			...
			return offers.map((offer, index) => ({
				...
				productKey: "",
				productName: "",
				approvedAmount: Number.parseFloat(offer.approvedAmount),
				interestRate: Number.parseFloat(offer.interestRate),
				tenure: offer.tenure,
				emi: Number.parseFloat(offer.emi),
				processingFee: Number.parseFloat(offer.processingFee),
				legalCharges: Number.parseFloat(offer.legalCharges || "0"),
				otherCharges: Number.parseFloat(offer.otherCharges || "0"),
				totalCost: Number.parseFloat(offer.totalCost),
				eligibilityScore: Number.parseFloat(offer.eligibilityScore ?? "0"),
				approvalProbability: Number.parseFloat(
					offer.approvalProbability ?? "0",
				),
				qualityScore: Number.parseFloat(offer.qualityScore ?? "0"),
				offerSource: offer.offerSource as "api" | "rules_engine",
				rateType: offer.rateType as "fixed" | "floating" | "hybrid",
				ltvRatio: offer.ltvRatio
					? Number.parseFloat(offer.ltvRatio)
					: undefined,
				...
				recommendationScore: Number.parseFloat(offer.qualityScore),
			}));
		} catch (error) {
	...
	getProviderProducts(
		providerKey: string,
	): ProviderProductOffering[] | undefined {
	...
	getProductProviders(productKey: string): LoanProviderData[] {
		return LOAN_PROVIDERS.filter((p) =>
			p.products.some(
				(prod) => prod.productKey === productKey && prod.isActive,
			),
		);
	}
	...
	async softPrequalify(
		...
		const maxEligibleAmount = this.calculateMaxLoanFromEMI(
			maxEMI,
			avgRate,
			avgTenure,
		);
		...
		if (requestedAmount < product.minAmount) {
			reasons.push(
				`Minimum loan amount is ₹${product.minAmount.toLocaleString("en-IN")}`,
			);
		}
		if (requestedAmount > product.maxAmount) {
			reasons.push(
				`Maximum loan amount is ₹${product.maxAmount.toLocaleString("en-IN")}`,
			);
		}
		...
		if (monthlyIncome < (product.minIncome || 0)) {
			reasons.push(
				`Minimum monthly income requirement is ₹${(product.minIncome || 0).toLocaleString("en-IN")}`,
			);
		}
		...
		if (creditScore && creditScore < (product.minCibilScore || 600)) {
			reasons.push(
				`Minimum credit score requirement is ${product.minCibilScore || 600}`,
			);
		...
		if (requestedAmount > maxEligibleAmount) {
			...
			recommendations.push(
				`Based on your income, you may be eligible for up to ₹${Math.floor(maxEligibleAmount).toLocaleString("en-IN")}`,
			);
		}
		...
		if (eligible) {
			recommendations.push(
				"You appear to be eligible for this loan. Complete your application to get personalized offers.",
			);
		}
		...
		return {
			...
			reasons,
			recommendations,
			suggestedProviders: eligible
				? this.getProductProviders(productKey)
						.slice(0, 3)
	...
	private calculateMaxLoanFromEMI(
		...
		const principal =
			(maxEMI * ((1 + monthlyRate) ** tenureMonths - 1)) /
...
const LOAN_PRODUCTS: LoanProductData[] = [
	{
		...
		category: "unsecured",
		description:
			"Multipurpose loan for personal needs like medical emergencies, travel, wedding, or home renovation.",
		icon: "User",
		minAmount: 50000,
		maxAmount: 4000000,
		minTenure: 12,
	...
	},
	{
		...
		description:
			"Finance your dream home with competitive interest rates and long tenure options.",
		icon: "Home",
		minAmount: 500000,
		maxAmount: 100000000,
		minTenure: 60,
	...
	},
	{
		...
		description:
			"Drive your dream car home with easy financing for new and pre-owned vehicles.",
		icon: "Car",
		minAmount: 100000,
		maxAmount: 10000000,
		minTenure: 12,
	...
	},
	{
		...
		category: "unsecured",
		description:
			"Fuel your business growth with working capital and expansion finance.",
		icon: "Building2",
		minAmount: 100000,
		maxAmount: 50000000,
		minTenure: 12,
	...
	},
	{
		...
		category: "unsecured",
		description:
			"Invest in your future with education loans for higher studies in India and abroad.",
		icon: "GraduationCap",
		minAmount: 100000,
		maxAmount: 15000000,
		minTenure: 36,
	...
	},
	{
		...
		collateralType: "gold",
		description:
			"Get instant funds against your gold jewelry with minimal documentation.",
		icon: "Star",
		minAmount: 10000,
		maxAmount: 10000000,
		minTenure: 3,
	...
	},
	{
		...
		description:
			"Get funds against your shares, mutual funds, bonds, and other securities without selling them.",
		icon: "TrendingUp",
		minAmount: 100000,
		maxAmount: 200000000,
		minTenure: 12,
		...
		eligibilityCriteria: [
			...
			"Minimum portfolio value ₹5 Lakhs",
			"CIBIL score 700+",
		],
		isActive: true,
	},
...
const LOAN_PROVIDERS: LoanProviderData[] = [
	{
		...
		providerType: "bank",
		description:
			"India's leading private sector bank with extensive loan offerings.",
		rating: 4.5,
		...
		products: [
			{
				...
				processingFee: 2.25,
				maxProcessingFee: 5000,
				commissionRate: 0.75,
				features: ["Instant approval", "Zero prepayment charges"],
				isActive: true,
			},
			{
				productKey: "home",
				...
				maxLTV: 80,
				commissionRate: 0.4,
				features: ["Balance transfer benefit", "Property search assistance"],
				isActive: true,
			},
			{
				productKey: "car",
				...
				processingFee: 0.5,
				maxProcessingFee: 6000,
				maxLTV: 100,
				commissionRate: 0.5,
				features: ["100% on-road funding", "Used car loans available"],
				isActive: true,
			},
			{
				productKey: "business",
				...
				processingFee: 2.0,
				commissionRate: 1.0,
				features: ["Overdraft facility", "Quick turnaround"],
				isActive: true,
			},
			{
				productKey: "lap",
				...
				maxLTV: 65,
				commissionRate: 0.5,
				features: ["High loan amount", "Balance transfer option"],
				isActive: true,
			},
			{
				productKey: "securities",
	...
	},
	{
		...
		providerType: "bank",
		description:
			"India's largest private sector bank by assets with diverse loan portfolio.",
		rating: 4.6,
		...
		products: [
			{
				...
				processingFee: 2.5,
				maxProcessingFee: 4500,
				commissionRate: 0.8,
				features: ["Pre-approved offers", "Instant disbursal"],
				isActive: true,
			},
			{
				productKey: "home",
				...
				maxLTV: 85,
				commissionRate: 0.35,
				features: ["Top-up facility", "Part-prepayment allowed"],
				isActive: true,
			},
			{
				productKey: "car",
			...
			},
			{
				...
				interestRateMax: 19.5,
				processingFee: 1.75,
				commissionRate: 0.9,
				features: ["Working capital finance", "GST-linked loans"],
				isActive: true,
			},
			{
				productKey: "education",
				...
				processingFee: 1.0,
				commissionRate: 0.4,
				features: ["Moratorium benefit", "Top foreign universities covered"],
				isActive: true,
			},
			{
				productKey: "securities",
	...
	},
	{
		...
		providerType: "nbfc",
		description:
			"Leading NBFC with quick digital loan processing and wide product range.",
		rating: 4.3,
		...
		products: [
			{
				...
				processingFee: 3.0,
				maxProcessingFee: 4000,
				commissionRate: 1.0,
				features: ["Flexi loan option", "Part-prepayment allowed"],
				isActive: true,
			},
			{
				productKey: "business",
				...
				processingFee: 2.5,
				commissionRate: 1.25,
				features: ["Flexi business loan", "Instant approval"],
				isActive: true,
			},
			{
				productKey: "gold",
				...
				maxLTV: 75,
				commissionRate: 0.6,
				features: ["Instant disbursement", "No income proof needed"],
				isActive: true,
			},
			{
				productKey: "lap",
				...
				maxLTV: 70,
				commissionRate: 0.65,
				features: ["High loan-to-value", "Flexible tenure"],
				isActive: true,
			},
		],
		isActive: true,
	},
	{
		...
		providerType: "nbfc",
		description:
			"Trusted Tata Group company offering comprehensive lending solutions.",
		rating: 4.2,
		avgProcessingTime: "2-3 days",
		hasApi: false,
		...
		products: [
			{
				...
				processingFee: 2.0,
				commissionRate: 0.85,
				features: ["Trusted brand", "Transparent pricing"],
				isActive: true,
			},
			{
				productKey: "home",
				...
				maxLTV: 80,
				commissionRate: 0.45,
				features: ["Home improvement loans", "Plot + construction"],
				isActive: true,
			},
			{
				productKey: "car",
				...
				maxLTV: 90,
				commissionRate: 0.5,
				features: ["Used car financing", "Refinance available"],
				isActive: true,
			},
			{
				productKey: "business",
				...
				processingFee: 2.0,
				commissionRate: 1.0,
				features: ["MSME focus", "Equipment finance"],
				isActive: true,
			},
			{
				productKey: "education",
				...
				processingFee: 1.0,
				commissionRate: 0.45,
				features: ["Study abroad loans", "Domestic courses covered"],
				isActive: true,
			},
		],
		isActive: true,
		...
		products: [
			{
				...
				maxProcessingFee: 5000,
				commissionRate: 0.7,
				features: ["Instant approval", "No hidden charges"],
				isActive: true,
			},
			{
				productKey: "home",
				...
				maxLTV: 80,
				commissionRate: 0.38,
				features: ["Doorstep service", "Quick sanction"],
				isActive: true,
			},
			{
				productKey: "car",
				...
				maxLTV: 100,
				commissionRate: 0.52,
				features: ["Pre-approved for salary accounts", "Zero documentation"],
				isActive: true,
			},
			{
				productKey: "business",
				...
				processingFee: 1.5,
				commissionRate: 0.95,
				features: ["Trade finance", "Bill discounting"],
				isActive: true,
			},
			{
				productKey: "gold",
				...
				maxLTV: 75,
				commissionRate: 0.55,
				features: ["Online gold loan", "Flexible repayment"],
				isActive: true,
			},
			{
				productKey: "lap",
				...
				maxLTV: 65,
				commissionRate: 0.55,
				features: ["Competitive rates", "Fast processing"],
				isActive: true,
			},
			{
				productKey: "securities",
