...
export class InsuranceMarketplaceAPI {
	...
	static async getInsuranceQuotes(req: Request, res: Response) {
		try {
			const { insuranceType, age, income, familyMembers, city, coverage } =
				req.body;
			...
			const basePremium = InsuranceMarketplaceAPI.calculateBasePremium(
				insuranceType,
				age,
				coverage,
			);

			const quotes = InsuranceMarketplaceAPI.generateProviderQuotes(
				insuranceType,
				basePremium,
				coverage,
			);
			...
			res.json({
				...
				data: {
					...
					recommendations: InsuranceMarketplaceAPI.getRecommendations(quotes),
	...
	static async calculateHealthInsurance(req: Request, res: Response) {
		try {
			const { age, city, familyMembers, preExistingDiseases, coverage } =
				req.body;
	...
	static async calculateLifeInsurance(req: Request, res: Response) {
		try {
			const { age, income, dependents, existingCoverage, smokingStatus } =
				req.body;
	...
	static async calculateMotorInsurance(req: Request, res: Response) {
		try {
			const { vehicleType, vehicleAge, city, idv, previousClaims, ncb } =
				req.body;
	...
	private static generateProviderQuotes(
		insuranceType: string,
		basePremium: number,
		coverage?: number,
	) {
		const typeKey = insuranceType
			.toLowerCase()
			.replace(" insurance", "") as keyof typeof this.insuranceProviders;
		const providers =
			InsuranceMarketplaceAPI.insuranceProviders[typeKey] ||
			InsuranceMarketplaceAPI.insuranceProviders.health;
		...
		return providers.map((provider) => ({
			...
			logo: provider.logo,
			specialOffers: InsuranceMarketplaceAPI.getSpecialOffers(provider.id),
			benefits: InsuranceMarketplaceAPI.getBenefits(insuranceType),
			exclusions: InsuranceMarketplaceAPI.getExclusions(insuranceType),
		}));
	}
	...
	private static getPlanFeatures(insuranceType: string): string[] {
		...
		const typeFeatures =
			features[insuranceType.toLowerCase() as keyof typeof features] ||
	...
	private static getBenefits(insuranceType: string): string[] {
		...
		const typeBenefits =
			benefits[insuranceType.toLowerCase() as keyof typeof benefits] ||
	...
	private static getExclusions(insuranceType: string): string[] {
		...
		const typeExclusions =
			exclusions[insuranceType.toLowerCase() as keyof typeof exclusions] ||
	...
	static async compareInsurancePlans(req: Request, res: Response) {
		try {
			...
			let quotes = InsuranceMarketplaceAPI.generateProviderQuotes(
				insuranceType,
				basePremium,
				criteria?.coverage,
			);
			...
			res.json({
				...
				data: {
					comparisons: quotes,
					bestValue: quotes.reduce((min, quote) =>
						quote.premium < min.premium ? quote : min,
					),
					topRated: quotes.reduce((max, quote) =>
						quote.rating > max.rating ? quote : max,
					),
					bestClaims: quotes.reduce((max, quote) =>
						quote.claimSettlementRatio > max.claimSettlementRatio ? quote : max,
					),
					comparisonMatrix:
						InsuranceMarketplaceAPI.generateComparisonMatrix(quotes),
					lastUpdated: new Date().toISOString(),
				},
			});
		} catch (error) {
	...
	private static generateComparisonMatrix(quotes: any[]) {
		...
		return criteria.map((criterion) => ({
			...
			values: quotes.map((quote) => ({
				...
				value:
					criterion === "features" || criterion === "benefits"
	...
	static getProvidersByType(insuranceType: string) {
		...
		return providers.map((provider) => ({
			...
			logo: provider.logo,
			specialization: InsuranceMarketplaceAPI.getProviderSpecialization(
				provider.id,
				insuranceType,
			),
		}));
	...
	private static getProviderSpecialization(
		providerId: string,
		insuranceType: string,
	): string[] {
		...
		return (
			specializations[providerId as keyof typeof specializations] || [
	...
	private static getRecommendations(quotes: any[]): string[] {
		...
		recommendations.push(
			`Best Value: ${cheapest.insurerName} - ${cheapest.planName}`,
		);

		// Find highest rating
		const topRated = quotes.reduce((max, quote) =>
			quote.rating > max.rating ? quote : max,
		);
		recommendations.push(
			`Highest Rated: ${topRated.insurerName} - ${topRated.rating}/5 rating`,
		);

		// Find best claim settlement
		const bestClaims = quotes.reduce((max, quote) =>
			quote.claimSettlementRatio > max.claimSettlementRatio ? quote : max,
		);
		recommendations.push(
			`Best Claims: ${bestClaims.insurerName} - ${bestClaims.claimSettlementRatio}% settlement ratio`,
		);
