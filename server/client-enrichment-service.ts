...
class ClientEnrichmentEngine {
	...
	async enrichWithPANData(userId: string, panNumber: string) {
		...
		try {
			throw new Error(
				"PAN verification API not configured. Karza API integration required for PAN enrichment.",
			);
		} catch (error) {
	...
	async enrichWithBusinessData(userId: string, gstNumber: string) {
		...
		try {
			throw new Error(
				"GST verification API not configured. Karza API integration required for business enrichment.",
			);
		} catch (error) {
	...
	async enrichWithBankStatementData(userId: string, bankStatementFile: string) {
		...
		try {
			throw new Error(
				"Bank statement analysis API not configured. Signzy API integration required for bank statement enrichment.",
			);
		} catch (error) {
	...
	async enrichWithCreditData(userId: string, panNumber: string) {
		...
		try {
			throw new Error(
				"Credit bureau API not configured. Bureau API integration required for credit enrichment.",
			);
		} catch (error) {
	...
	async performComprehensiveEnrichment(
		userId: string,
		enrichmentParams: {
			panNumber?: string;
			gstNumber?: string;
		...
		try {
			...
			if (enrichmentParams.panNumber) {
				const panResult: any = await this.enrichWithPANData(
					userId,
					enrichmentParams.panNumber,
				);
			...
			if (enrichmentParams.gstNumber) {
				const gstResult: any = await this.enrichWithBusinessData(
					userId,
					enrichmentParams.gstNumber,
				);
			...
			if (enrichmentParams.bankStatement) {
				const bankResult: any = await this.enrichWithBankStatementData(
					userId,
					enrichmentParams.bankStatement,
				);
			...
			if (enrichmentParams.panNumber) {
				const creditResult: any = await this.enrichWithCreditData(
					userId,
					enrichmentParams.panNumber,
				);
			...
			const avgScore =
				results.length > 0 ? Math.round(overallScore / results.length) : 0;
	...
	private generateEnrichmentSummary(results: any[]) {
		return {
			...
			averageScore:
				results.reduce(
					(sum: number, r: any) => sum + (r.enrichmentScore || 0),
					0,
				) / (results.length || 1),
	...
	async enrichClient(req: Request, res: Response) {
		try {
			...
			if (enrichmentType === "comprehensive") {
				const result = await enrichmentEngine.performComprehensiveEnrichment(
					userId,
					{
						panNumber,
						gstNumber,
			...
			switch (enrichmentType) {
				case "pan_verification":
					if (!panNumber) {
						return res
							.status(400)
							.json({ error: "PAN number required for PAN verification" });
					}
				...
				case "business_verification":
					if (!gstNumber) {
						return res
							.status(400)
							.json({ error: "GST number required for business verification" });
					}
					result = await enrichmentEngine.enrichWithBusinessData(
						userId,
						gstNumber,
					);
					break;
				...
				case "bank_analysis":
					if (!bankStatement) {
						return res
							.status(400)
							.json({
								error: "Bank statement required for financial analysis",
							});
					}
					result = await enrichmentEngine.enrichWithBankStatementData(
						userId,
						bankStatement,
					);
					break;
				...
				case "credit_analysis":
					if (!panNumber) {
						return res
							.status(400)
							.json({ error: "PAN number required for credit analysis" });
					}
					result = await enrichmentEngine.enrichWithCreditData(
						userId,
						panNumber,
					);
					break;
	...
	async getEnrichmentHistory(req: Request, res: Response) {
		try {
			...
			let query = db
				...
				.leftJoin(
					externalDataSources,
					eq((clientEnrichmentData as any).sourceId, externalDataSources.id),
				)
				.leftJoin(
					apiIntegrationLogs,
					eq(
						(clientEnrichmentData as any).sourceId,
						(apiIntegrationLogs as any).sourceId,
					),
				)
			...
			if (enrichmentType) {
				query = (query as any).where(
					and(
						...
						eq(
							(clientEnrichmentData as any).enrichmentType,
