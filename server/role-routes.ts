...
const requireAnyRole = (allowedRoles: RoleId[]) => {
	return (req: any, res: Response, next: NextFunction) => {
		...
		const userRoles = roleService.normalizeRoles(
...
export function registerRoleRoutes(app: Express) {
	...
	app.get(
		"/api/roles/can-access-portal/:portal",
		requireAuth,
		async (req: any, res: Response) => {
			try {
	...
	app.get(
		...
		async (req: Request, res: Response) => {
			try {
				const rolesWithInfo = Object.entries(ROLE_DEFINITIONS).map(
					([roleId, def]) => ({
						roleId,
						name: def.name,
						portal: def.portal,
	...
	app.get(
		"/api/agent/dashboard/overview",
		requireAnyRole(AGENT_PORTAL_ROLES),
		async (req: any, res: Response) => {
			try {
				...
				const { eq, and, gte, sql, count, sum, desc, inArray } = await import(
					"drizzle-orm"
				);
				...
				if (clientIds.length > 0) {
					...
					totalAUM = portfolioValues.reduce(
						(sum, p) => sum + (Number.parseFloat(p.totalValue || "0") || 0),
						0,
					);
				}
				...
				const monthlyEarned =
					Number.parseFloat(monthlyCommissions[0]?.total || "0") || 0;
				...
				const pendingEarned =
					Number.parseFloat(pendingCommissions[0]?.total || "0") || 0;
	...
	app.get(
		"/api/agent/dashboard/recent-activity",
		async (req: any, res: Response) => {
			try {
				const { db } = await import("./db");
				...
				recentProspects.forEach((prospect, idx) => {
					...
					activities.push({
						...
						message: prospect.uploadedPortfolio
							? "Portfolio uploaded"
							: "New prospect added",
						time: timeStr,
					});
	...
	app.get("/api/agent/itr-cases", async (req: any, res: Response) => {
		try {
			...
			res
				...
				.json({
					success: false,
					message: "Failed to fetch ITR cases",
					error: error.message,
				});
		}
	});
			...
			res.json([
				{
					...
					specialization: "Individual ITR, Capital Gains",
					activeCases: 12,
				},
				{
					id: "ca-2",
					name: "CA Meera Joshi",
					specialization: "Business ITR, GST",
					activeCases: 8,
				},
				{
					id: "ca-3",
	...
	app.get(
		"/api/agent/training/quiz-questions",
		async (req: any, res: Response) => {
			try {
				...
				res.json([
					{
						id: "1",
						question:
							"What is the maximum investment limit in ELSS per year for tax benefit?",
					...
					},
					{
						id: "3",
						question:
							"Which section covers tax deduction for home loan principal repayment?",
						options: [
							"Section 80C",
							"Section 80D",
							"Section 24",
	...
	app.get("/api/agent/itr/stats", async (req: any, res: Response) => {
		try {
			...
			res
				...
				.json({
					success: false,
					message: "Failed to fetch ITR stats",
					error: error.message,
				});
		}
	});
	...
	app.get("/api/agent/itr/available-cas", async (req: any, res: Response) => {
		try {
			...
			res
				...
				.json({
					success: false,
					message: "Failed to fetch available CAs",
					error: error.message,
				});
		}
	});
	...
	app.get("/api/agent/stats", async (req: any, res: Response) => {
		try {
				...
				if (clientIds.length > 0) {
					...
					if (ids.length > 0) {
						const aumResult = await db
							.select({
								total: sql<number>`COALESCE(SUM(CAST(total_value AS NUMERIC)), 0)`,
							})
							.from(portfolios)
			...
			const targetProgress =
				monthlyTarget > 0
					? Math.round((thisMonthBusiness / monthlyTarget) * 100)
					: 0;
	...
	app.get("/api/agent/clients/list", async (req: any, res: Response) => {
		try {
			const { db } = await import("./db");
			const { clientAgentRelationships, users, prospectClients } = await import(
				"@shared/schema"
			);
