...
async function runCompleteFlowTest(): Promise<TestResult[]> {
	...
	try {
		const consentResult =
			await governmentSchemeConsentOrchestrator.initiateConsent({
				userId: TEST_USER_ID,
				schemeType: "epf",
				channel: "email",
				ipAddress: "127.0.0.1",
	...
	if (challengeId) {
		try {
			...
			const verifyResult =
				await governmentSchemeConsentOrchestrator.verifyOTPAndGrantConsent({
					userId: TEST_USER_ID,
					schemeType: "epf",
					challengeId,
					otp: "123456",
			...
			if (verifyResult.success) {
				...
				if (verifyResult.dataFetched) {
					console.log(
						`   📊 Data fetched: ${verifyResult.dataFetched.recordsCreated} created, ${verifyResult.dataFetched.recordsUpdated} updated`,
					);
				}
			} else {
	...
	try {
		...
		if (epfRecords.length > 0) {
			...
			const nameMatches =
				record.memberName?.toLowerCase().includes("sangram") &&
				record.memberName?.toLowerCase().includes("mohanty");
			const employerMatches = record.employerName
				?.toLowerCase()
				.includes("jm financial");
			...
			if (verificationPassed) {
				...
				console.log(
					`      Member: ${record.memberName} (expected: ${TEST_EXPECTED_NAME})`,
				);
				console.log(
					`      Employer: ${record.employerName} (expected: ${TEST_EXPECTED_EMPLOYER})`,
				);
			} else {
				results.push({
					step: "EPF Database Verification",
					...
					data: {
						...
						expected: {
							memberName: TEST_EXPECTED_NAME,
							employerName: TEST_EXPECTED_EMPLOYER,
						},
					},
				});
				console.log(`   ❌ EPF data verification failed`);
				console.log(
					`      Expected: ${TEST_EXPECTED_NAME} @ ${TEST_EXPECTED_EMPLOYER}`,
				);
				console.log(
					`      Actual: ${record.memberName} @ ${record.employerName}`,
				);
			}
		} else {
			results.push({
			...
			console.log(
				`   ⚠️ No EPF records found (external API may return mock data in dev mode)`,
			);
		}
	} catch (error: any) {
		results.push({
	...
	try {
		...
		if (auditLogs.length > 0) {
			...
			auditLogs.forEach((log) => {
				console.log(
					`      - ${log.eventType} (${log.schemeType}) at ${log.timestamp}`,
				);
			});
		} else {
			results.push({
	...
	for (const schemeType of schemeTypes) {
		try {
			...
			console.log(
				`   ${result.success ? "✅" : "❌"} ${schemeType.toUpperCase()}: ${result.success ? "OK" : result.message}`,
			);
		} catch (error: any) {
