...
import {
	casStatementService,
	parseCASDate,
} from "../services/cas-statement-service";
...
const GOLDEN_FIXTURES: GoldenFixture[] = [
	{
		...
		filePath:
			"attached_assets/AJXXXXXX9R_01012021-24012026_CP203204091_24012026071436194_-_A_1769782761584.pdf",
...
export async function runGoldenFixtureTest(fixture: GoldenFixture): Promise<{
	...
	try {
		...
		const costDiff = Math.abs(
			result.summary.totalInvestedValue - fixture.expectedTotalCost,
		);
		...
		const marketDiff = Math.abs(
			result.summary.totalCurrentValue - fixture.expectedTotalMarket,
		);
		...
		details.isinSamplesPassed = fixture.isinSamples.every((isin) =>
			parsedIsins.has(isin),
		);
...
export async function runFormatVarianceTest(test: FormatVarianceTest): Promise<{
	...
	try {
		...
		if (
			test.expectedNav !== undefined &&
			Math.abs(holding.nav - test.expectedNav) > 0.01
		) {
			errors.push(
		...
		if (
			test.expectedMarket !== undefined &&
			Math.abs(holding.marketValue - test.expectedMarket) > 1
		) {
			errors.push(
...
export async function runAllCASRegressionTests(): Promise<{
	...
	const allPassed =
		goldenPassed === goldenResults.length &&
		formatPassed === formatResults.length;
...
`;
	...
	try {
		const result = await casStatementService.parseStatement(
			sampleTransactionText,
		);
		...
		if (!holding) {
			...
			const allLotsHaveDates =
				holding.lots?.every((lot) => lot.transactionDate) || false;
...
`;
		...
		const sipResult =
			await casStatementService.parseStatement(sipTransactionText);
		const sipHolding = sipResult.holdings.find(
			(h) => h.isin === "INFTEST08SIP",
		);
...
export function testDateParsing(): { passed: boolean; results: any[] } {
	...
	console.log(
		`Date parsing tests: ${results.filter((r) => r.passed).length}/${results.length} passed`,
	);
