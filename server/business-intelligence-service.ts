...
export class BusinessIntelligenceService {
	async getBusinessMetrics(): Promise<BusinessMetrics> {
		try {
			...
			const totalRevenue = products.reduce(
				(sum, p) => sum + ((p as any).revenueGenerated || 0),
				0,
			);
	...
	async generateProfitabilityInsights(
...
Format your response as a JSON object.`;
	...
	async generateServiceQualityInsights(
		metrics: BusinessMetrics,
	): Promise<AIInsight> {
...
Format your response as a JSON object with actionable operational improvements.`;
	...
	async generateAllInsights(): Promise<AIInsight[]> {
		try {
			...
			const [profitability, serviceQuality, marketing, operations] =
				await Promise.all([
