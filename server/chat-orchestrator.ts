...
How can I assist you today?`,
	...
	async sendMessage(
		...
		// Build context
		const portfolio = session.portfolioId
			? await storage.getPortfolio(session.portfolioId)
			: null;
		const userProfile = await storage.getUserProfile(userId);
		...
		const functionSchemas = functions
			.map((f) => functionRegistry.getFunctionSchema(f.name))
			.filter(Boolean);
