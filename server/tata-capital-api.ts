...
export class TataCapitalAPI {
  ...
  calculatePersonalLoan(principal: number, tenure: number, employmentType: 'salaried' | 'self-employed'): {
    emi: number;
    interestRate: number;
    ...
    const eligibility = principal >= 25000 && principal <= 3500000 && tenure >= 12 && tenure <= 84;
  ...
  calculateHomeLoan(principal: number, tenure: number, propertyType: 'ready' | 'under-construction'): {
    emi: number;
    interestRate: number;
    processingFee: number;
    totalAmount: number;
    ...
    if (propertyType === 'under-construction') {
      interestRate = 9.00; // Higher rate for under-construction
    }
  ...
  calculateBusinessLoan(principal: number, tenure: number, businessVintage: number, turnover: number): {
    emi: number;
    interestRate: number;
    processingFee: number;
    ...
    let interestRate = 12.50; // Base rate
    ...
    const eligibility = businessVintage >= 2 && turnover >= 2000000 && principal <= 75000000;
  ...
  calculateUsedCarLoan(vehiclePrice: number, vehicleAge: number, downPayment: number, tenure: number): {
    loanAmount: number;
    emi: number;
    interestRate: number;
    processingFee: number;
  ...
  calculateLoanAgainstProperty(propertyValue: number, loanAmount: number, tenure: number, propertyType: 'residential' | 'commercial'): {
    emi: number;
    interestRate: number;
    maxLoanAmount: number;
    loanToValue: number;
    ...
    // Interest rates
    let interestRate = 9.50; // Base rate for residential
  ...
  calculateLoanAgainstSecurities(portfolioValue: number, loanAmount: number, securityType: 'equity' | 'mutual-fund' | 'bonds'): {
    emi: number;
    interestRate: number;
    maxLoanAmount: number;
    loanToValue: number;
  ...
  async checkCreditEligibility(pan: string, income: number, loanType: string): Promise<{
    creditScore: number;
    eligible: boolean;
    ...
    let recommendedProducts = ['Personal Loan'];
    if (income >= 50000) {
  ...
  async verifyGST(gstin: string): Promise<{
    ...
    const valid = gstin.length === 15 && /^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[1-9A-Z]{1}[Z]{1}[0-9A-Z]{1}$/.test(gstin);
  ...
  analyzeBankStatement(statements: any[]): {
    ...
    else if (averageBalance < 50000 || bounceCharges > 3000) riskProfile = 'high';
  ...
  async uploadDocument(documentType: string, file: Buffer, customerId: string): Promise<{
    success: boolean;
    documentId: string;
  ...
  async instantDisbursement(loanAccountNumber: string, amount: number, beneficiaryAccount: string): Promise<{
    success: boolean;
  ...
  async getOutstandingBalance(loanAccountNumber: string): Promise<{
    ...
    const totalOutstanding = principalOutstanding + interestDue + overduePrincipal + overdueInterest + bounceCharges;
  ...
  async getForeclosureDetails(loanAccountNumber: string): Promise<{
    ...
    const totalForeclosureAmount = principalOutstanding + interestTillDate + foreclosureCharges;
  ...
  getCurrentRates(): {
    ...
    return {
      personalLoan: { min: 10.99, max: 20.00 },
      homeLoan: { min: 8.75, max: 11.50 },
      businessLoan: { min: 12.50, max: 18.00 },
      usedCarLoan: { min: 9.75, max: 14.00 },
      loanAgainstProperty: { min: 9.50, max: 12.00 },
      loanAgainstSecurities: { min: 9.25, max: 11.00 }
    };
  }
