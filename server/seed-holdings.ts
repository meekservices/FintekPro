...
async function seedHoldings() {
  ...
  for (const portfolio of allPortfolios) {
    ...
    for (const asset of sampleAssets) {
      try {
        const investedValue = parseFloat(asset.quantity || asset.units || "0") * parseFloat(asset.avgPrice);
        const marketValue = parseFloat(asset.quantity || asset.units || "0") * parseFloat(asset.currentPrice);
