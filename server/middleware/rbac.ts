 ...
 */
export const alpacaAccountGuard = async (req: Request, res: Response, next: NextFunction) => {
  const accountId = req.params.accountId || (req.query.accountId as string) || (req.query.account_id as string);
...
async function checkAccess(user: any, clientId: string, res: Response, next: NextFunction) {
  ...
  return apiResponse.forbidden(res, "You do not have permission to access this Alpaca account");
}
