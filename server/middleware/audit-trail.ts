...
export const auditTrailMiddleware = (
	...
	const isSensitiveGet =
		req.method === "GET" &&
		SENSITIVE_GET_PATTERNS.some((p) => req.path.startsWith(p));
	...
	res.on("finish", () => {
		...
		const userId =
			(req as any).user?.id || (req.session as any)?.passport?.user;
		...
		if (contentLength)
			details.requestBodySize = Number.parseInt(contentLength, 10);

		const user = (req as any).user;
		const actorType = user?.roles?.includes("agent")
			? "agent"
			: user
				? "client"
				: null;
