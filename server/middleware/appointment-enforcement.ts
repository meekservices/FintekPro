...
export function appointmentEnforcementMiddleware(
  ...
  const isProtected = PROTECTED_ROUTE_PATTERNS.some(pattern => req.path.includes(pattern));
  if (!isProtected) {
    return next();
  }
  ...
  if (appointmentStatus !== APPOINTMENT_STATUSES.ACTIVE) {
    ...
    console.log(`[Appointment Enforcement] Blocked access for user ${user.id}: status=${appointmentStatus}, path=${req.path}`);
    
    let message: string;
    switch (appointmentStatus) {
      case APPOINTMENT_STATUSES.PENDING_ADMIN_APPROVAL:
        message = "Your account is pending administrator approval. Please wait for activation.";
        break;
      ...
      case APPOINTMENT_STATUSES.DRAFT:
        message = "Your account setup is incomplete. Please complete registration.";
        break;
      default:
...
export function roleRestrictionMiddleware(
  ...
  if (userRoles.includes('ca')) {
    ...
    const isRestricted = restrictedForCA.some(pattern => req.path.includes(pattern));
    if (isRestricted) {
  ...
  if (userRoles.includes('support_staff')) {
    ...
    const isRestricted = restrictedForSupport.some(pattern => req.path.includes(pattern));
    if (isRestricted) {
...
export async function attachAppointmentStatus(
  ...
  try {
    ...
    if (dbUser) {
      ...
      (req as any).user.appointmentRejectionReason = dbUser.appointmentRejectionReason;
    }
  } catch (error) {
