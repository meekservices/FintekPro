...
import { db } from "../db";
import { b2bApiKeys, b2bClients, b2bApiUsageLogs } from "../../shared/schema/b2b";
 ...
 */
export async function b2bAuthMiddleware(req: B2BRequest, res: Response, next: NextFunction) {
  ...
  try {
    ...
    const [apiKeyRecord] = await db.select()
      .from(b2bApiKeys)
      .where(and(eq(b2bApiKeys.keyHash, hashedKey), eq(b2bApiKeys.isActive, true)))
      .limit(1);
    ...
    const [clientRecord] = await db.select()
      .from(b2bClients)
      .where(and(eq(b2bClients.id, apiKeyRecord.clientId), eq(b2bClients.isActive, true)))
      .limit(1);
    ...
    if (clientRecord.allowedIps) {
      ...
      if (allowedIpsArray.length > 0 && requestIp && !allowedIpsArray.includes(requestIp)) {
         return res.status(403).json({ error: "IP Address not allowlisted for this API Key." });
      }
    }
    ...
    db.update(b2bApiKeys).set({ lastUsedAt: new Date() }).where(eq(b2bApiKeys.id, apiKeyRecord.id)).execute();
 ...
 */
const rateLimitMap = new Map<string, { tokens: number, lastRefreshed: number }>();

export function b2bRateLimiter(req: B2BRequest, res: Response, next: NextFunction) {
  ...
  if (!rateLimitMap.has(clientId)) {
     rateLimitMap.set(clientId, { tokens: MAX_CAPACITY - 1, lastRefreshed: now });
     return next();
  }
  ...
  if (bucket.tokens >= 1) {
     ...
     db.insert(b2bApiUsageLogs).values({
        id: trackingId, clientId: clientId, endpoint: req.originalUrl, status: 429
     }).execute();
