...
async function isClientAssignedToAgent(
	agentId: string,
	clientId: string,
): Promise<boolean> {
	...
	try {
		...
		const assigned =
			(result as any).rows?.length > 0 || (result as any).length > 0;
		...
		logger.warn(
			"[OwnershipGuard] DB check failed — failing closed to protect access control",
			{
				agentId,
				clientId,
				error: String(err),
...
export function requireOwnership(
	...
	const { from = "params", adminBypass = true, exemptRoles = [] } = opts;
	...
	return async (
		...
		if (!allowed) {
			logger.warn(
				"[OwnershipGuard] Access denied — client not assigned to agent",
				{
					agentId: user.id,
 ...
 */
export const requireSelfOrAgent = requireOwnership("userId", {
	from: "params",
});
