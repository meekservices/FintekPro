...
function rateLimitHandler(
	...
	res.status(429).json({
		...
		retry_after_seconds: info
			? Math.ceil((Number(info) * 1000 - Date.now()) / 1000)
			: 60,
		meta: {
...
export const otpLimiter = rateLimit({
	...
	max: 5,
	message:
		"Too many OTP requests. Please wait 15 minutes. If this is urgent, contact support.",
	keyGenerator: (req) => ipKeyGenerator(req.ip ?? ""),
});
...
export const aiLimiter = rateLimit({
	...
	max: 30,
	message:
		"AI request limit reached. Please slow down — maximum 30 requests per minute.",
	keyGenerator: (req) => {
...
export const adminCopilotLimiter = rateLimit({
	...
	max: 60,
	message:
		"Admin Copilot rate limit exceeded. Maximum 60 requests per minute to protect Zoho API quotas.",
	keyGenerator: (req) => {
		const userId = (req as any).user?.id || (req as any).session?.userId;
