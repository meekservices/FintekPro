  ...
  try {
    ...
    if (!profile || !profile.riskCategory) {
      return {
        suitable: false,
        userRiskLevel: 'low',
        reason: 'Risk profile assessment not found. Please complete the SEBI risk assessment before investing.',
      };
    }

    // Check if assessment is stale (>1 year old per SEBI IA Reg)
    const assessmentAge = Date.now() - (profile.riskLastAssessed?.getTime() ?? 0);
    const ONE_YEAR_MS = 365 * 24 * 60 * 60 * 1000;
    if (assessmentAge > ONE_YEAR_MS) {
      return {
        suitable: false,
        userRiskLevel: profile.riskCategory as RiskLevel,
        reason: 'Your risk profile assessment has expired (>1 year). Please update your risk profile before investing. Required under SEBI Investment Adviser Regulations 2020.',
      };
    }
    ...
    logger.error('[SuitabilityGate] Check failed, failing open', { userId, productRiskLevel, err });
...
export function requireSuitability(
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    const productRiskLevel = typeof riskLevelOrResolver === 'function'
    ...
    if (!result.suitable) {
      ...
      return res.status(403).json({
        ...
        productRiskLevel,
        userRiskLevel: result.userRiskLevel,
        regulatoryBasis: 'SEBI Investment Adviser Regulations 2013 (amended 2020), Regulation 17',
