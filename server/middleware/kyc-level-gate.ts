...
export async function getUserKYCLevel(userId: string): Promise<{
	...
	const hasLevel1Requirements =
		panVerified && addressOvdVerified && photographCaptured;
	...
	const hasLevel2Requirements =
		hasLevel1Requirements &&
		hasCentralKycVerification &&
		hasIdentityVerification &&
		bankPennyDropVerified;

	// Compute V-CIP expiry state for compliance details
	const videoKycExpired =
		vcipRawCompleted && !!profile.videoKycExpiryDate && !videoKycCompleted;
	const videoKycExpiryDate = profile.videoKycExpiryDate
		? new Date(profile.videoKycExpiryDate)
		: null;

	// Build missing requirements list for user guidance
	if (!panVerified) missingRequirements.push("PAN verification");
	if (!addressOvdVerified)
		missingRequirements.push("Address proof (Aadhaar/OVD)");
	if (!photographCaptured)
		missingRequirements.push("Complete profile with photograph");

	if (hasLevel1Requirements) {
		if (!hasCentralKycVerification)
			missingRequirements.push("CKYC/KRA registration");
		if (videoKycExpired) {
			missingRequirements.push(
				"Video KYC (V-CIP) has expired — renewal required",
			);
		} else if (!videoKycCompleted && !ipvCompleted) {
			missingRequirements.push("Video KYC or In-Person Verification");
		}
		if (!bankPennyDropVerified)
			missingRequirements.push("Bank account verification");
	}
...
export const requireLevel1 = async (
	...
	try {
		...
		const { level, profile, complianceDetails } = await getUserKYCLevel(
			req.user.id,
		);
		req.kycLevel = level;
		...
		if (!hasRequiredKYCLevel(level, "1")) {
			return res.status(403).json({
				...
				message:
					"Please complete Standard KYC to access loans and insurance products",
				kycLevel: level,
				requiredLevel: "1",
				complianceDetails,
				...
				nextStep: {
					...
					url: "/onboarding",
					description:
						"Complete PAN verification, address proof, and photograph to unlock loans and insurance",
	...
	try {
		...
		if (!hasRequiredKYCLevel(level, "2")) {
			return res.status(403).json({
				...
				message:
					"Please complete Full KYC verification to access investment products",
				kycLevel: level,
				requiredLevel: "2",
				complianceDetails,
				missingRequirements: complianceDetails.missingRequirements,
...
export const injectKYCLevel = async (
	...
	try {
		...
		logger.error("[KycLevelGate] Error fetching KYC level", {
...
export async function checkProductAccess(
	...
	const hasAccess = hasRequiredKYCLevel(
		level,
		requiredLevel as "0" | "1" | "2",
	);
	...
	let message = "";
	const nextStepUrl = "/onboarding";

	if (level === "0" && requiredLevel === "1") {
		message =
			"Please complete Standard KYC (PAN + Address Proof + Photograph) to access this product";
	} else if (level === "0" && requiredLevel === "2") {
		message =
			"Please complete Full KYC verification to access this product. Start with PAN verification.";
	} else if (level === "1" && requiredLevel === "2") {
		message =
			"Please complete CKYC/KRA registration, Video KYC, and bank verification to access investment products";
	}
...
export async function getAccessibleProducts(userId: string): Promise<{
	...
	for (const [product, requiredLevel] of Object.entries(
