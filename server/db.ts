...
if (dbUrl) {
	...
	if (cloudSqlMatch?.[4]?.startsWith("/")) {
		...
		console.log(
			`[DB] 🔧 Cloud SQL socket mode: user=${POOL_CONFIG.user} db=${POOL_CONFIG.database} socket=${POOL_CONFIG.host}`,
		);
	} else {
...
if (isProduction) {
	try {
		...
		if (fs.existsSync(rootDir)) {
			...
			console.warn(
				`[DB] ⚠️ ${rootDir} directory does NOT exist. This usually means the Cloud SQL instance is not attached to the Cloud Run service.`,
			);
		}

		if (POOL_CONFIG.host?.startsWith("/")) {
			console.log(
			...
			POOL_CONFIG.port = undefined;
			POOL_CONFIG.connectionString = undefined; // Prefer explicit host if socket is found
		} else {
			console.warn(
				`[DB] ⚠️ No socket found at ${socketPath}. Attempting to proceed with current configuration...`,
			);

			// Fallback: If no socket, and we have a connection string, check if it's usable
			if (!dbUrl) {
				console.error(
					`[DB] ❌ CRITICAL: No DATABASE_URL and no Unix Socket found. Connection will likely fail.`,
				);
			}
		}
	} catch (err) {
		console.error(
			`[DB] ❌ Error in socket diagnostic: ${err instanceof Error ? err.message : String(err)}`,
		);
	}
}
...
export function getProductionDb() {
	...
	prodPool.on("error", (err) => {
		console.warn(
			`[ProductionDB] Pool error (auto-recovering): ${err?.message || err}`,
		);
	});

	prodDb = drizzle(prodPool, { schema });
	console.log(
		"[ProductionDB] Production database connection initialized for enrichment",
	);
	return prodDb;
}
...
export function getEnrichmentWriteDb() {
	if (!hasProductionDb()) {
		throw new Error(
			"[ProductionDB] PRODUCTION_DATABASE_URL not set. Enrichment writes require production DB.",
		);
	}
...
export function requireProductionDb(serviceName: string): boolean {
	if (!hasProductionDb()) {
		console.error(
			`[${serviceName}] ❌ PRODUCTION_DATABASE_URL not set. Enrichment runs on production only. Aborting.`,
		);
		return false;
	}
