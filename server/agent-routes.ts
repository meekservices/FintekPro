...
const isSecuritiesAgent = (agent: any): boolean => {
	...
	return agent.productTypes.some((pt: string) =>
		securitiesProducts.includes(pt),
	);
};
...
const getRequiredCredentials = (
	productType: string,
): { requiresARN: boolean; requiresEUIN: boolean; requiresIRDAI: boolean } => {
...
const agentRegistrationSchema = z
	.object({
		...
		productTypes: z
			.array(
				z.enum(["loans", "mutual_funds", "aif", "pms", "insurance", "equity"]),
			)
		...
		arnCode: z
			.string()
			.regex(/^ARN-\d{5,6}$/i, "Invalid ARN format")
			.optional(),
		euinNumber: z
			.string()
			.regex(/^E\d{6}$/i, "Invalid EUIN format")
			.optional(),
	...
	.superRefine((data, ctx) => {
		...
		if (!isSubAgent) {
			...
			const hasSecurities = data.productTypes.some((pt) =>
				securitiesProducts.includes(pt),
			);
			...
			if (hasSecurities) {
				if (!data.arnCode) {
					ctx.addIssue({
						...
						message:
							"ARN code is required for distributing mutual funds, PMS, AIF, or equity products (SEBI-regulated)",
						path: ["arnCode"],
					});
				}
				if (!data.euinNumber) {
					ctx.addIssue({
						code: z.ZodIssueCode.custom,
						message:
							"EUIN is required for distributing mutual funds, PMS, AIF, or equity products (SEBI-regulated)",
						path: ["euinNumber"],
					});
				}
			}
...
router.post("/register", async (req, res) => {
	try {
		...
		const securitiesProducts = ["mutual_funds", "aif", "pms", "equity"]; // SEBI-regulated only
		const hasSecurities = productTypes.some((pt) =>
			securitiesProducts.includes(pt),
		);
		...
		let regulatoryCategory:
			| "loan_dsa"
			| "securities_distributor"
			| "insurance_agent"
			| "hybrid" = "loan_dsa";

		const productCount =
			(hasSecurities ? 1 : 0) + (hasLoans ? 1 : 0) + (hasInsurance ? 1 : 0);
		...
		if (hasSecurities && !isSubAgent) {
			...
			if (euinNumber) {
				euinValidation = await amfiValidationService.validateEuin(
					euinNumber,
					arnCode,
				);
		...
		const agentData = {
			...
			// Verification status
			arnVerificationStatus:
				hasSecurities && !isSubAgent
					? arnValidation?.isValid
						? "verified"
						: "pending"
					: "not_required",
			euinVerificationStatus:
				hasSecurities && !isSubAgent
					? euinValidation?.isValid
						? "verified"
						: "pending"
					: "not_required",
			...
			amfiVerifiedAt:
				arnValidation?.isValid || euinValidation?.isValid ? new Date() : null,
		};
...
router.post("/validate-euin", async (req, res) => {
	try {
		...
		const euinValidation = await amfiValidationService.validateEuin(
			euinNumber,
			arnCode,
		);
...
router.post("/:agentId/aadhaar/verify-otp", requireAuth, async (req, res) => {
	try {
		...
		if (!otp || !refId) {
			return res
				.status(400)
				.json({ error: "OTP and reference ID are required" });
		}

		// Verify OTP using Cashfree OKYC
		const verificationResponse = await CashfreeAadhaarService.verifyOTP(
			otp,
			refId,
		);
		...
		if (
			!verificationResponse.data?.aadhaarNumber ||
		...
		await storage.updateCustomerCareAgent(agentId, {
			...
			aadharVerified: true,
			onboardingStatus:
				agent.panVerified && agent.bankAccountVerified ? "approved" : "pending",
		});
...
router.post("/:agentId/documents", requireAuth, async (req, res) => {
	try {
		...
		const { documentType, documentName, documentUrl, documentNumber } =
			validation.data;
		...
		if (documentType === "amfi_certificate")
			updates.amfiCertificateVerified = false;
...
router.post(
	"/api/admin/agents/documents/:documentId/verify",
	requireAdmin,
	async (req, res) => {
		try {
			...
			// Update agent's document verification flags
			const updates: any = {};
			if (document.documentType === "pan_card")
				updates.panVerified = status === "verified";
			if (document.documentType === "aadhar_card")
				updates.aadharVerified = status === "verified";
			if (document.documentType === "amfi_certificate")
				updates.amfiCertificateVerified = status === "verified";
			if (document.documentType === "euin_card")
				updates.euinCardVerified = status === "verified";
...
router.post(
	"/api/admin/agents/:agentId/approve",
	requireAdmin,
	async (req, res) => {
		try {
			const { agentId } = req.params;
			...
			const agent = await storage.updateAgentVerificationStatus(
				agentId,
				updates,
			);
...
router.get(
	"/api/agents/:masterAgentId/sub-agents",
	requireAuth,
	async (req, res) => {
		try {
...
router.post(
	"/api/admin/agents/commission-splits",
	requireAdmin,
	async (req, res) => {
		try {
			...
			const totalShare =
				Number.parseFloat(subAgentShare) + Number.parseFloat(masterAgentShare);
			if (Math.abs(totalShare - 100) > 0.01) {
				return res
					.status(400)
					.json({ error: "Commission shares must add up to 100%" });
			}
	...
	try {
		...
		if (!accessGranted) {
			return res
				.status(403)
				.json({
					error: "Forbidden: You can only access your own commission splits",
				});
		}
...
router.get(
	"/api/agents/:agentId/commission-summary",
	requireAuth,
	async (req, res) => {
		try {
			const { agentId } = req.params;
...
router.get(
	"/api/agents/:agentId/referral-stats",
	requireAuth,
	async (req, res) => {
		try {
			const { agentId } = req.params;
...
router.get(
	"/api/agents/:agentId/referred-clients",
	requireAuth,
	async (req, res) => {
		try {
			const { agentId } = req.params;
...
router.post(
	"/api/agents/:agentId/refer-client",
	requireAuth,
	async (req, res) => {
		try {
			const { agentId } = req.params;
			...
			const { firstName, lastName, email, mobile, interestedProducts, notes } =
				req.body;
...
const appointmentsStore: any[] = [
	...
	{
		id: "2",
		title: "SIP Discussion",
		description: "New SIP recommendations",
		type: "call",
		clientId: "2",
...
router.patch("/appointments/:id", requireAuth, async (req, res) => {
	try {
		const { id } = req.params;
		const appointmentIndex = appointmentsStore.findIndex(
			(apt) => apt.id === id,
		);
		...
		if (locationDetails !== undefined)
			allowedUpdates.locationDetails = locationDetails;
...
router.post("/clients/onboard/draft", requireAuth, async (req, res) => {
	try {
		...
		const draftId =
			draftData.draftId ||
			`DRAFT-${Date.now()}-${Math.random().toString(36).substr(2, 6).toUpperCase()}`;
		...
		const draft = {
			...
			updatedAt: new Date().toISOString(),
			createdAt:
				onboardingDraftsStore.get(draftId)?.createdAt ||
	...
	try {
		...
		if (!connection) {
			return res.status(200).json({
				...
				total: 0,
				message:
					"Zoho CRM not connected - please configure integration in admin settings",
			});
		}
...
router.post("/zoho/leads/:leadId/proposal", requireAuth, async (req, res) => {
	try {
		...
		const statusUpdated = await zohoCRM.updateLeadStatus(
			leadId,
			"Proposal Sent",
		);
