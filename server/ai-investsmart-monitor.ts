...
class AIInvestSmartMonitor {
	...
	private parseIncomeValue(incomeValue: string | number): number {
		...
		if (monthlyIncome > MAX_MONTHLY) {
			console.warn(
				`Unusually high monthly income calculated: ₹${monthlyIncome}, capping to ₹${MAX_MONTHLY}`,
			);
	...
	async analyzePageStructure(
		userId: string,
	): Promise<InvestSmartPageStructure> {
		try {
			...
			const processedData = enrichmentRecords?.processedData as Record<
				string,
				any
			> | null;
			if (monthlyIncome === 0 && processedData?.estimatedIncome) {
				...
				if (processedData.incomeFrequency === "annual") {
					...
					const annualValue =
						typeof processedData.estimatedIncome === "number"
							? processedData.estimatedIncome
							: Number.parseFloat(
									String(processedData.estimatedIncome).replace(/[₹,]/g, ""),
								);
			...
			if (cibilData) {
				...
				const cibilProcessed = cibilData.processedData as Record<
					string,
					any
				> | null;
				if (cibilProcessed?.totalEMI) {
					monthlyObligations =
						typeof cibilProcessed.totalEMI === "string"
							? Number.parseFloat(cibilProcessed.totalEMI)
							: cibilProcessed.totalEMI;
				}
				if (cibilProcessed?.creditScore) {
					creditScore =
						typeof cibilProcessed.creditScore === "string"
							? Number.parseInt(cibilProcessed.creditScore)
			...
			if (monthlyObligations === 0 && processedData?.monthlyObligations) {
				monthlyObligations =
					typeof processedData.monthlyObligations === "string"
						? Number.parseFloat(processedData.monthlyObligations)
			...
			if (creditScore === 0) {
				...
				if (processedData?.creditScore) {
					creditScore =
						typeof processedData.creditScore === "string"
							? Number.parseInt(processedData.creditScore)
			...
			const totalPortfolioValue =
				this.calculateTotalPortfolioValue(userPortfolios);

			// Calculate real investment surplus
			const availableForInvestment = Math.max(
				0,
				monthlyIncome - monthlyObligations,
			);
			...
			const riskProfile = this.analyzeRiskProfile(
				userPortfolios,
				transactionAnalysis,
			);

			// Credit obligations from real enrichment data
			const creditObligations = this.analyzeCreditObligations(
				enrichmentRecords,
				cibilData,
			);
	...
	async generateAIInsights(
		pageStructure: InvestSmartPageStructure,
	): Promise<AIInsightsGeneration> {
		try {
...
Provide insights in this JSON format:
{
  ...
  "nextSteps": ["step1", "step2"]
			...
			console.error(
				"[AI InvestSmart] generateAIInsights failed:",
	...
	private async getClientEnrichmentData(userId: string) {
		try {
			const [latestEnrichment] = await db
				.select()
				.from(clientEnrichmentData)
	...
	private calculateTotalPortfolioValue(portfolios: any[]): number {
		return portfolios.reduce((total, portfolio) => {
			const portfolioValue =
				portfolio.holdings?.reduce((sum: number, holding: any) => {
					return (
						sum +
						Number.parseFloat(holding.quantity || "0") *
							Number.parseFloat(holding.currentPrice || "0")
					);
				}, 0) || 0;
			...
			return [
				{
					name: "No Goals Set",
					message:
						"Complete your profile and set financial goals to track progress",
					target: 0,
					current: 0,
					progress: 0,
				},
		...
		if (monthlySurplus > 0) {
			...
			goals.push({
				...
				timelineAcceleration:
					monthlySurplus > dashboardMetrics.monthlyIncome * 0.3
						? "Ahead of schedule"
						: "On track",
			});
			...
			const emergencyTarget =
				dashboardMetrics.monthlyObligations * 6 ||
				dashboardMetrics.monthlyIncome * 3;
			goals.push({
				name: "Emergency Fund",
				...
				timelineAcceleration:
					portfolioValue * 0.2 >= emergencyTarget ? "Completed" : "In progress",
			});
			...
			goals.push({
				...
				timelineAcceleration:
					monthlySurplus > dashboardMetrics.monthlyIncome * 0.25
						? "Building steadily"
						: "Needs attention",
			});
		}
	...
	private analyzeRiskProfile(portfolios: any[], transactionAnalysis: any) {
		...
		const equityRatio =
			totalHoldings > 0 ? (equityPercentage / totalHoldings) * 100 : 50;
		...
		if (equityRatio > 80) {
			recommendedAdjustments.push(
				"Consider reducing equity exposure for better risk management",
			);
		...
		if (totalHoldings < 5) {
			recommendedAdjustments.push(
				"Add more holdings to reduce concentration risk",
			);
		}
	...
	private analyzeCreditObligations(enrichmentData: any, cibilData: any) {
		...
		if (cibilData?.processedData) {
			...
			if (
				cibilProcessed.creditCards &&
				Array.isArray(cibilProcessed.creditCards)
			) {
	...
	private calculateAssetAllocation(holdings: any[]) {
		...
		holdings.forEach((holding) => {
			...
			if (
				...
				assetType.includes("fixed") ||
				sector.includes("debt")
			) {
				allocation.debt += percentage;
			} else if (
				assetType.includes("gold") ||
				assetType.includes("commodity") ||
				sector.includes("gold") ||
				sector.includes("commodity")
			) {
				...
				assetType.includes("mutual fund") ||
				sector === "mutual fund"
			) {
	...
	private calculatePerformance(holdings: any[]) {
		...
		holdings.forEach((holding) => {
			...
			const currentPrice = Number.parseFloat(holding.currentPrice || avgPrice);
		...
		const estimatedVolatility =
			holdingsCount >= 10 ? 12 : holdingsCount >= 5 ? 15 : 20;
	...
	private validateAndEnhanceInsights(
		insights: any,
		pageStructure: InvestSmartPageStructure,
		...
		return {
			...
			keyOpportunities: Array.isArray(insights.keyOpportunities)
			...
			urgentActions: Array.isArray(insights.urgentActions)
				? insights.urgentActions
				: ["Optimize tax-saving investments before year-end"],
			goalAcceleration: Array.isArray(insights.goalAcceleration)
				? insights.goalAcceleration
				: ["Increase SIP allocation to achieve goals 2-3 years earlier"],
			riskOptimization: Array.isArray(insights.riskOptimization)
				? insights.riskOptimization
				: ["Rebalance portfolio for better risk-adjusted returns"],
			investmentRecommendations: Array.isArray(
				insights.investmentRecommendations,
			)
				? insights.investmentRecommendations
				: ["Consider hybrid mutual funds for balanced growth"],
			complianceAlerts: Array.isArray(insights.complianceAlerts)
				? insights.complianceAlerts
				: ["No compliance issues detected"],
			nextSteps: Array.isArray(insights.nextSteps)
				? insights.nextSteps
				: [
	...
	private generateFallbackInsights(
		...
		if (creditScore >= 750) {
			keyOpportunities.push(
				`Excellent credit score (${creditScore}) enables favorable loan terms for investments`,
			);
		} else if (creditScore >= 650) {
			keyOpportunities.push(
				`Good credit score (${creditScore}) provides access to most investment products`,
			);
		}
		if (obligationRatio < 40) {
			keyOpportunities.push(
				"Low obligation ratio provides flexibility for aggressive wealth building",
			);
		}
		...
		if (portfolioValue > 0) {
			urgentActions.push(
				"Review and rebalance portfolio allocation for optimal returns",
			);
		}
		if (surplus > 0) {
		...
		if (homePurchaseAlloc > 0) {
			goalAcceleration.push(
				`Allocate ₹${homePurchaseAlloc.toLocaleString()}/month towards home purchase goal`,
			);
		}
		if (educationAlloc > 0) {
			goalAcceleration.push(
				`Set aside ₹${educationAlloc.toLocaleString()}/month for education fund`,
			);
		}
		if (retirementAlloc > 0) {
			goalAcceleration.push(
				`Direct ₹${retirementAlloc.toLocaleString()}/month to retirement planning`,
			);
		}
		if (goalAcceleration.length === 0) {
			goalAcceleration.push(
				"Define your financial goals to get personalized guidance",
			);
		}

		// Risk optimization based on portfolio
		if (pageStructure.riskProfile?.diversificationScore < 60) {
			riskOptimization.push(
		...
		if (riskOptimization.length === 0) {
			riskOptimization.push(
				"Your portfolio is well-optimized for your risk profile",
			);
		}
		...
		if (obligationRatio < 30) {
			complianceAlerts.push(
				`Healthy debt-to-income ratio at ${obligationRatio}%`,
			);
		} else if (obligationRatio < 50) {
			complianceAlerts.push(
				`Obligation ratio at ${obligationRatio}% - monitor carefully`,
			);
		} else {
			complianceAlerts.push(
				`High obligation ratio at ${obligationRatio}% - consider debt reduction`,
			);
		}
		if (creditScore >= 700) {
		...
		return {
			...
			investmentRecommendations: [
				...
				surplus > 50000
					...
					: "ELSS funds for tax-efficient growth",
			],
			complianceAlerts,
			nextSteps: [
				surplus > 0
					? `Implement SIP of ₹${surplus.toLocaleString()} monthly`
...
export const aiInvestSmartMonitorService = {
	...
	async getAIInsights(req: Request, res: Response) {
		try {
			...
			const pageStructure =
				await aiInvestSmartMonitor.analyzePageStructure(userId);

			// Generate AI-powered insights
			const aiInsights =
				await aiInvestSmartMonitor.generateAIInsights(pageStructure);
		...
		try {
			...
			const healthMetrics: {
				...
				categories: {
					...
					obligations:
						pageStructure.dashboardMetrics.obligationRatio < 40 ? 95 : 70,
					liquidity:
						pageStructure.dashboardMetrics.availableForInvestment > 50000
							? 100
							: 60,
