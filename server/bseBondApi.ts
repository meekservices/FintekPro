...
const API_CONFIG = IS_PRODUCTION ? BSE_BOND_CONFIG.production : BSE_BOND_CONFIG.demo;
...
export class BSEBondApiService {
  ...
  private validateCredentials(): boolean {
    if (IS_PRODUCTION) {
      return !!(BSE_BOND_CREDENTIALS.userId && 
  ...
  async searchBonds(query: {
    ...
    try {
      ...
      return allBonds.filter(bond => {
        if (query.issuer && !bond.issuer.toLowerCase().includes(query.issuer.toLowerCase())) {
          return false;
        }
