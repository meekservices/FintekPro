...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/learning/modules", async (req, res) => {
    try {
      ...
      // Add computed progress data for modules (mock data for now)
      const modulesWithProgress = modules.map(module => ({
        ...module,
        lessonsCount: 3, // Mock - should be calculated from actual lessons
        completedLessons: module.id === 'module-1' ? 2 : 0, // Mock progress
        progress: module.id === 'module-1' ? 67 : 0,
        isLocked: module.orderIndex > 1 && module.id !== 'module-2' // First two modules unlocked
      }));
  ...
  app.get("/api/learning/stats", async (req, res) => {
    try {
      // Mock user ID - should be from authenticated session
      ...
      if (!stats) {
        ...
        const newStats = await storage.upsertUserStats(defaultStats);
  ...
  app.get("/api/learning/lessons/:lessonId", async (req, res) => {
    try {
      ...
      // Get associated quiz
      const quiz = await storage.getLearningQuiz(lessonId);
  ...
  // Submit quiz answer
  app.post("/api/learning/quiz/:quizId/submit", async (req, res) => {
    try {
      ...
      // Find the quiz
      const quiz = Array.from((storage as any).learningQuizzes.values())
      ...
      const pointsEarned = isCorrect ? quiz.pointsReward : 0;
      ...
      if (currentStats) {
        ...
        await storage.upsertUserStats(updatedStats);
      }
  ...
  // Complete a lesson
  app.post("/api/learning/lesson/:lessonId/complete", async (req, res) => {
    try {
      const { lessonId } = req.params;
      const userId = "demo-user-1"; // Mock user ID
  ...
  app.get("/api/learning/progress/:moduleId", async (req, res) => {
    try {
      const { moduleId } = req.params;
      const userId = "demo-user-1"; // Mock user ID
      
      const progress = await storage.getUserProgressForModule(userId, moduleId);
      res.json(progress);
    } catch (error) {
  ...
  app.get("/api/learning/achievements", async (req, res) => {
    try {
      const userId = "demo-user-1"; // Mock user ID
