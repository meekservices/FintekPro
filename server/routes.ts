...
export async function registerRoutes(app: Express, existingServer?: Server): Promise<Server> {
  ...
  // Public yield curve endpoint (no KYC required - for homepage display)
  ...
  app.get("/api/bonds/search/advanced", requireLevel2, async (req, res) => {
    try {
      const filters = {
  ...
  app.get("/api/bonds/filter/rating", requireLevel2, async (req, res) => {
    try {
  ...
  app.get("/api/bonds/filter/yield", requireLevel2, async (req, res) => {
    try {
  ...
  app.get("/api/bonds/filter/maturity", requireLevel2, async (req, res) => {
    try {
      const params = {
  ...
  // Get tax-free bonds
  app.get("/api/bonds/tax-free-bonds", requireLevel2, async (req, res) => {
    try {
  ...
  app.get("/api/bonds/infrastructure-bonds", requireLevel2, async (req, res) => {
    try {
  ...
  app.get("/api/bonds/yield-curve", requireLevel2, async (req, res) => {
    try {
  ...
  app.get("/api/bonds/auctions/historical", requireLevel2, async (req, res) => {
    try {
      const params = {
  ...
  app.post("/api/bonds/market-prices", requireLevel2, async (req, res) => {
    try {
  ...
  app.get("/api/bonds/mcx-listed", requireLevel2, async (req, res) => {
    try {
          ...
          if (!process.env.CREDHIVE_API_KEY) {
            ...
            try {
              ...
              result = {
                ...
                message: searchResp.success ? 'Credhive API connection successful' : (searchResp.error || 'API connection failed'),
                details: searchResp.success ? { resultsReturned: searchResp.data?.length ?? 0 } : undefined,
                latency: Date.now() - startTime
              };
            } catch (e: any) {
              result = { success: false, message: e.message };
  ...
  app.post("/api/insurance/compare", requireLevel1, InsuranceMarketplaceAPI.compareInsurancePlans);
  ...
  app.post("/api/tax/documents", async (req, res) => {
    try {
      ...
      const { documentType, financialYear, fileName, fileFormat, documentUrl, metadata } = req.body;
      
      if (!documentType || !financialYear || !fileName) {
        return res.status(400).json({ 
          error: "Missing required fields: documentType, financialYear, fileName" 
        });
      }
  ...
  app.get("/api/tax/documents", async (req, res) => {
    try {
      ...
      const documents = await storage.getTaxDocuments(req.user.id, financialYear as string);
  ...
  app.get("/api/tax/data", async (req, res) => {
    try {
      ...
      const { financialYear } = req.query;
      const structuredData = await storage.getStructuredTaxDataByUser(req.user.id, financialYear as string);
  ...
  app.post("/api/tax/calculate", async (req, res) => {
    try {
      ...
      const calculation = await storage.calculateTaxLiability(req.user.id, financialYear, taxRegime);
  ...
  app.get("/api/tax/calculations", async (req, res) => {
    try {
      ...
      const { financialYear } = req.query;
      const calculations = await storage.getTaxCalculations(req.user.id, financialYear as string);
  ...
  app.post("/api/tax/generate-itr", async (req, res) => {
    try {
      ...
      const itrResult = await storage.generateITRJson(req.user.id, financialYear);
      
      // Set appropriate headers for file download
      res.setHeader('Content-Type', 'application/json');
      res.setHeader('Content-Disposition', `attachment; filename="ITR_${financialYear}_${req.user.id}.json"`);
  ...
  app.post('/api/investment-ideas', async (req, res) => {
    try {
      ...
      complianceMonitor.logEvent({ 
        eventType: 'data_access', action: 'create_investment_idea', 
        resource: idea.id,
      ...
      complianceMonitor.logEvent({ 
        eventType: 'data_access', action: 'create_investment_idea', 
        outcome: 'failure', riskLevel: 'medium',
        error: error instanceof Error ? error.message : 'Unknown error'
      });
  ...
  app.get('/api/investment-ideas/:id', async (req, res) => {
    try {
      ...
      complianceMonitor.logEvent({ 
        eventType: 'data_access', action: 'read_investment_idea', 
  ...
  app.put('/api/investment-ideas/:id', async (req, res) => {
    try {
      ...
      complianceMonitor.logEvent({ 
        eventType: 'data_access', action: 'update_investment_idea', 
        resource: req.params.id,
        outcome: 'success', riskLevel: 'low'
      });
  ...
  app.get('/api/investment-ideas/:id/tracking', async (req, res) => {
    try {
      ...
      complianceMonitor.logEvent({ 
        eventType: 'data_access', action: 'read_idea_tracking', 
        resource: req.params.id,
        outcome: 'success', riskLevel: 'low'
      });
  ...
  app.put('/api/yield-tracker/:id', async (req, res) => {
    try {
      ...
      complianceMonitor.logEvent({ 
        eventType: 'data_access', action: 'update_yield_tracker', 
        resource: req.params.id,
        outcome: 'success', riskLevel: 'low'
      });
  ...
  app.put('/api/yield-tracker/:id/price', async (req, res) => {
    try {
      ...
      complianceMonitor.logEvent({ 
        eventType: 'data_access', action: 'update_tracker_price', 
        resource: req.params.id,
        outcome: 'success', riskLevel: 'low'
      });
  ...
  app.get('/api/yield-tracker/:id/metrics', async (req, res) => {
    try {
      ...
      complianceMonitor.logEvent({ 
        eventType: 'data_access', action: 'read_yield_metrics', 
        resource: req.params.id,
        outcome: 'success', riskLevel: 'low'
      });
  ...
  app.post('/api/yield-tracker/bulk-update', async (req, res) => {
    try {
      ...
      complianceMonitor.logEvent({ 
        eventType: 'data_access', action: 'bulk_update_trackers', 
  ...
  app.put('/api/admin/store/categories/:id', requireAdmin, async (req: any, res) => {
    try {
      const { name, description, icon, displayOrder } = req.body;
      const existingCategory = await storage.getStoreCategoryById(req.params.id);
  ...
  app.post('/api/admin/store/categories/:id/toggle', requireAdmin, async (req: any, res) => {
    try {
      ...
      res.json({
        success: true,
        message: "Category " + (isActive ? "enabled" : "disabled") + " successfully with " + result.subcategories.length + " subcategories and " + result.products.length + " products",
        ...result,
      });
    } catch (error) {
      console.error('Error toggling category:', error);
      res.status(500).json({ message: error instanceof Error ? error.message : 'Failed to toggle category' });
    }
  });

  // Delete category
  app.delete('/api/admin/store/categories/:id', requireAdmin, async (req: any, res) => {
    try {
      const category = await storage.getStoreCategoryById(req.params.id);
      if (!category) {
        return res.status(404).json({ message: 'Category not found' });
      ...
      const subcategories = await storage.getStoreSubcategoriesByCategory(req.params.id);
      if (subcategories.length > 0) {
        return res.status(400).json({ message: 'Cannot delete category with existing subcategories. Delete subcategories first.' });
      }

      const products = await storage.getStoreProductsByCategory(req.params.id);
      if (products.length > 0) {
        return res.status(400).json({ message: 'Cannot delete category with existing products. Delete products first.' });
      }
  ...
  app.get('/api/admin/store/subcategories', requireAdmin, async (req, res) => {
    try {
      ...
      let subcategories;
      if (categoryId) {
        subcategories = await storage.getStoreSubcategoriesByCategory(categoryId as string);
      } else {
  ...
  app.get('/api/admin/store/subcategories/:id', requireAdmin, async (req, res) => {
    try {
      const subcategory = await storage.getStoreSubcategoryById(req.params.id);
      if (!subcategory) {
  ...
  // Create subcategory
  app.post('/api/admin/store/subcategories', requireAdmin, async (req: any, res) => {
    try {
  ...
  // Toggle subcategory
  app.post('/api/admin/store/subcategories/:id/toggle', requireAdmin, async (req: any, res) => {
    try {
      const { isActive } = req.body;
      ...
      const category = await storage.getStoreCategoryById(subcategory.categoryId);
      if (!category?.isActive && isActive) {
        return res.status(400).json({ message: 'Cannot enable subcategory when parent category is disabled' });
      }
      ...
      res.json({
        success: true,
        message: "Subcategory " + (isActive ? "enabled" : "disabled") + " successfully with " + result.products.length + " products",
        ...result,
      });
    } catch (error) {
      console.error('Error toggling subcategory:', error);
      res.status(500).json({ message: error instanceof Error ? error.message : 'Failed to toggle subcategory' });
    }
  });

  // Delete subcategory
  app.delete('/api/admin/store/subcategories/:id', requireAdmin, async (req: any, res) => {
    try {
      const subcategory = await storage.getStoreSubcategoryById(req.params.id);
      if (!subcategory) {
        return res.status(404).json({ message: 'Subcategory not found' });
      ...
      const products = await storage.getStoreProductsBySubcategory(req.params.id);
      if (products.length > 0) {
  ...
  app.get('/api/admin/store/products', requireAdmin, async (req, res) => {
    try {
      ...
      let products;
      if (subcategoryId) {
        products = await storage.getStoreProductsBySubcategory(subcategoryId as string);
      } else if (categoryId) {
        products = await storage.getStoreProductsByCategory(categoryId as string);
      } else {
  ...
  app.post('/api/admin/store/products/:id/toggle', requireAdmin, async (req: any, res) => {
    try {
      const { isActive } = req.body;
      ...
      if (!category?.isActive && isActive) {
        return res.status(400).json({ message: 'Cannot enable product when parent category is disabled' });
      }

      if (product.subcategoryId) {
        const subcategory = await storage.getStoreSubcategoryById(product.subcategoryId);
        if (!subcategory?.isActive && isActive) {
          return res.status(400).json({ message: 'Cannot enable product when parent subcategory is disabled' });
        }
      }
      ...
      const updated = await storage.updateStoreProductStatus(req.params.id, isActive);
  ...
  app.post('/api/store/inquiries', async (req: any, res) => {
    try {
      const { productId, categoryId, subcategoryId, userId, name, email, mobile, message } = req.body;
  ...
  app.get('/api/store/catalog', async (req, res) => {
    try {
      ...
      const activeSubcategories = subcategories.filter(s => s.isActive && activeCategories.some(c => c.id === s.categoryId));
      ...
      const catalog = activeCategories.map(cat => ({
        ...
        subcategories: activeSubcategories
          ...
          .map(sub => ({
            ...sub,
            products: activeProducts.filter(prod => prod.subcategoryId === sub.id),
          })),
        products: activeProducts.filter(prod => prod.categoryId === cat.id && !prod.subcategoryId),
      }));
  ...
  // GET /api/agent/prospect-clients - List all prospect clients for the agent with stats
  app.get("/api/agent/prospect-clients", requireAgent, async (req: any, res) => {
    try {
      ...
      const [prospectCount] = await db
        .select({ count: sql<number>`count(*)` })
        .from(prospectClients)
        .where(and(eq(prospectClients.agentId, agentId), eq(prospectClients.state, 'prospect')));
      
      const [onboardedCount] = await db
        .select({ count: sql<number>`count(*)` })
        .from(prospectClients)
        .where(and(eq(prospectClients.agentId, agentId), eq(prospectClients.state, 'onboarded')));
  ...
  app.post("/api/agent/prospect-clients", requireAgent, async (req: any, res) => {
    try {
      const agentId = req.user.id;
      const { name, email, mobile, pan, clientType, indicativeRiskProfile } = req.body;
      ...
      if (pan) {
        const existingPan = await db
          .select()
          .from(prospectClients)
          .where(and(eq(prospectClients.agentId, agentId), eq(prospectClients.pan, pan.toUpperCase())))
          .limit(1);
        
        if (existingPan.length > 0) {
          return res.status(400).json({ message: 'A prospect with this PAN already exists' });
        }
      }
  ...
  app.get("/api/agent/prospect-clients/:id", requireAgent, async (req: any, res) => {
    try {
      const agentId = req.user.id;
      const { id } = req.params;
  ...
  app.patch("/api/agent/prospect-clients/:id", requireAgent, async (req: any, res) => {
    try {
      const agentId = req.user.id;
      const { id } = req.params;
      const updates = req.body;
  ...
  app.patch("/api/agent/prospect-clients/:id/state", requireAgent, async (req: any, res) => {
    try {
      const agentId = req.user.id;
      const { id } = req.params;
      ...
      res.status(500).json({ message: 'Failed to transition prospect state' });
    }
  });

  // POST /api/agent/prospect-clients/:id/fetch-portfolio - Trigger PAN-based portfolio fetch (simulated)
  app.post("/api/agent/prospect-clients/:id/fetch-portfolio", requireAgent, async (req: any, res) => {
    try {
      const agentId = req.user.id;
      const { id } = req.params;
      ...
      if (!client.pan) {
        return res.status(400).json({ message: 'PAN is required to fetch portfolio' });
      }
      
      if (!consentGranted) {
        return res.status(400).json({ message: 'Portfolio fetch consent is required' });
      }
      ...
      const simulatedPortfolio = {
        ...
        source: 'pan_fetch',
        holdings: [
          { name: 'Reliance Industries', productType: 'equity', symbol: 'RELIANCE', quantity: 50, currentValue: 145000, purchaseValue: 120000 },
          { name: 'HDFC Balanced Advantage Fund', productType: 'mutual_fund', isin: 'INF179K01UT8', quantity: 1000, currentValue: 85000, purchaseValue: 75000 },
          { name: 'Tata Motors', productType: 'equity', symbol: 'TATAMOTORS', quantity: 100, currentValue: 95000, purchaseValue: 80000 },
          { name: 'ICICI Prudential Bluechip Fund', productType: 'mutual_fund', isin: 'INF109K01Z48', quantity: 500, currentValue: 42000, purchaseValue: 38000 },
          { name: 'SBI Corporate Bond Fund', productType: 'debt_fund', isin: 'INF200K01RZ5', quantity: 2000, currentValue: 68000, purchaseValue: 65000 }
        ],
        totalValue: 435000,
      ...
      res.json({ success: true, portfolio: simulatedPortfolio, client: updated });
    } catch (error) {
...
8. riskScore - risk level score 0-100`;
  ...
  await registerBBPSRoutes(app);
  registerCapitalGainsRoutes(app);
