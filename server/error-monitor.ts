...
class ErrorMonitor {
  ...
  private updatePerformanceMetrics() {
    ...
    this.systemHealth.performance.uptime = Math.min(100, (uptimeMs / (24 * 60 * 60 * 1000)) * 100);
  }
  ...
  // Check API health
  async checkApiHealth(apiName: string, testUrl: string): Promise<'up' | 'down' | 'slow'> {
    try {
      ...
      if (duration > 3000) { // Slow if > 3 seconds
  ...
  async generateErrorAnalysis(): Promise<any> {
    ...
    try {
      ...
      return {
        ...
        recommendations: ['Check system logs manually', 'Review recent changes'],
        priority: 'medium',
        category: 'monitoring'
      };
    }
  ...
  async generateReplitAgentInstructions(): Promise<any> {
    ...
    const appState = JSON.stringify({
      ...
      apis: ['Alpha Vantage', 'Yahoo Finance', 'Gemini AI', 'ICICI Bank', 'JM Financial', 'Probe42'],
