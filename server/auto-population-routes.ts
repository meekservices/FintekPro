...
router.post("/admin/setup-from-pan", async (req: Request, res: Response) => {
    ...
    `);
		...
		const panRecord = {
			...rawPanRecord,
			verified_at:
				rawPanRecord.verified_at instanceof Date
		...
		if (existingVault.length > 0) {
			console.log(
				`📋 KYC Vault already exists for user ${userId}, skipping vault creation`,
			);
		} else {
			// Step 4: Create KYC vault entry with encrypted data
			console.log(`🔐 Creating KYC Vault for user ${userId}...`);

			const encryptedFullName = encryptionService.encrypt(
				panRecord.full_name || "",
			);
			const encryptedDob = encryptionService.encrypt(
				panRecord.date_of_birth || "",
			);
			const encryptedMobile = user.mobile
				? encryptionService.encrypt(user.mobile)
				: null;
		...
		res.json({
			...
			panDetails: {
				pan:
					panRecord.pan_number.substring(0, 4) +
					"****" +
					panRecord.pan_number.slice(-1),
...
router.get(
	"/consent/check/:userId/:dataSource",
	requireOwnership("userId"),
	async (req: Request, res: Response) => {
		try {
...
router.post("/consent/revoke", async (req: Request, res: Response) => {
	try {
		...
		const consents =
			await consentManagementService.getUserConsents(sessionUserId);
	...
	async (req: Request, res: Response) => {
		try {
			...
			const expiringConsents =
				await consentManagementService.getExpiringConsents(userId);
...
router.get("/status/:workflowId", async (req: Request, res: Response) => {
	try {
		...
		const status =
			await autoPopulationOrchestrator.getWorkflowStatus(workflowId);
...
router.get(
	...
	async (req: Request, res: Response) => {
		try {
			...
			const workflows =
				await autoPopulationOrchestrator.getUserWorkflows(userId);
...
router.post("/retry-source", async (req: Request, res: Response) => {
	try {
		...
		const hasConsent = await (consentManagementService as any).hasValidConsent(
			userId,
			dataSource,
		);
		if (!hasConsent) {
		...
		const result = await autoPopulationOrchestrator.retryDataSource(
			userId,
			dataSource,
		);
	...
	try {
		...
		if (!userRecord.panNumber) {
			return {
				valid: false,
				error:
					"No verified PAN on file for this user. Please complete KYC first.",
			};
		}
		...
		try {
			storedPan =
				encryptionService.decrypt(userRecord.panNumber) || userRecord.panNumber;
		} catch {}
		...
		if (normalizedProvidedPan !== normalizedStoredPan) {
			console.warn(
				`[SECURITY] PAN mismatch for user ${sessionUserId}: provided PAN does not match verified PAN on file`,
			);
			return {
				valid: false,
				error:
					"The provided PAN does not match the verified PAN on your account. You can only fetch data for your own PAN.",
			};
		}
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (!ownershipCheck.valid) {
				console.warn(
					`[SECURITY] PAN ownership verification failed for user ${sessionUserId}`,
				);
				return res.status(403).json({
					success: false,
			...
			console.info(
				`✅ PAN ownership verified for user ${sessionUserId}, proceeding with CIBIL fetch`,
			);
