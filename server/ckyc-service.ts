...
import {
	...
	CkycComplianceError,
} from "./services/ckyc-environment-service";
...
export class CKYCService {
	...
	constructor() {
		this.baseUrl =
			process.env.CKYC_API_BASE_URL || "https://api.nsdl.com/ckyc/v1";
		...
		if (!hasCredentials && !CKYCService.credentialWarningLogged) {
			...
			console.log(
				"CKYC API credentials (CKYC_API_KEY, CKYC_API_SECRET) not configured",
			);
			console.log("CKYC registration and KIN polling will use mock responses");
			console.log(
				"ℹ️ Set CKYC_API_KEY and CKYC_API_SECRET to enable real CKYC integration",
			);
		}
	}
	 ...
	 */
	async uploadCKYCDocuments(
		request: CKYCUploadRequest,
		context?: { userId?: string },
	): Promise<CKYCUploadResponse> {
		const hasCredentials = this.hasValidCredentials();

		// Use mock mode when credentials are not configured
		if (!hasCredentials) {
			throw new Error(
				"CKYC service not configured. AuthBridge CKYC API credentials required for KYC upload and verification.",
			);
		}

		console.log(
			"📤 Uploading CKYC documents to NSDL for PAN:",
			request.personalInfo.panNumber.slice(0, 4) + "***",
		);
		...
		if (!response.ok) {
			throw new Error(
				`CKYC upload failed: ${response.status} ${response.statusText}`,
			);
		}
	...
	async pollKINStatus(
		...
		const response = await fetch(
			`${this.baseUrl}/status/${applicationNumber}`,
			{
				method: "GET",
				headers: {
					"X-API-Key": this.apiKey,
		...
		if (!response.ok) {
			throw new Error(
				`KIN status poll failed: ${response.status} ${response.statusText}`,
			);
		}
	 ...
	 */
	private assertNotProductionMock(context?: {
		userId?: string;
		panNumber?: string;
	}): void {
		if (ckycEnvironmentService.isProductionMode()) {
			// Fire and forget - log in background, throw immediately
			this.logMockBlockedAttemptSync("mock_assertion", context).catch(
				console.error,
			);
	 ...
	 */
	private async logMockBlockedAttemptSync(
		attemptType: string,
		context?: { userId?: string; panNumber?: string },
	): Promise<void> {
		try {
			...
			console.error(
				`[CKYC SECURITY] 🚫 Mock ${attemptType} blocked in PROD - logged to audit table`,
			);
		} catch (error) {
			console.error(
				`[CKYC SECURITY] Failed to log mock blocked attempt:`,
				error,
			);
		}
	}
	...
	async registerCKYC(
		request: CKYCRegistrationRequest,
	): Promise<CKYCRegistrationResponse> {
		const hasCredentials = this.hasValidCredentials();

		// Use mock mode when credentials are not configured
		if (!hasCredentials) {
			console.warn(
				"⚠️ Using mock CKYC registration response (credentials not configured)",
			);
			return {
				success: true,
		...
		if (!response.ok) {
			throw new Error(
				`CKYC registration failed: ${response.status} ${response.statusText}`,
			);
		}
	...
	async searchCKYC(request: CKYCSearchRequest): Promise<CKYCSearchResponse> {
		try {
			...
			if (request.panNumber)
				queryParams.append("pan_number", request.panNumber);
			if (request.ckycNumber)
				queryParams.append("ckyc_number", request.ckycNumber);
			if (request.aadharNumber)
				queryParams.append("aadhar_number", request.aadharNumber);
			if (request.passportNumber)
				queryParams.append("passport_number", request.passportNumber);
	 ...
	 */
	async searchCKYCWithCaching(
		userId: string,
		request: CKYCSearchRequest,
	): Promise<CKYCSearchResponse> {
		try {
			// Step 1: Check local cache first
			const cachedRecord = await this.getCachedCKYCRecord(
				request.panNumber || "",
			);

			if (cachedRecord && !this.isCacheExpired(cachedRecord)) {
				console.log(
					"✅ CKYC cache hit for PAN:",
					request.panNumber?.slice(0, 4) + "***",
				);
			...
			console.log(
				"🔍 CKYC cache miss, querying registry for PAN:",
				request.panNumber?.slice(0, 4) + "***",
			);
	 ...
	 */
	private mapDbStatusToApiStatus(
	...
	private convertCacheToResponse(record: CkycRecord): CKYCSearchResponse {
		return {
			...
			verificationLevel:
				(record.verificationLevel as "basic" | "enhanced") || "basic",
	 ...
	 */
	async cacheCKYCRecord(
		userId: string,
		request: CKYCSearchRequest,
		response: CKYCSearchResponse,
	): Promise<void> {
		...
		try {
			...
			const existingRecord = await this.getCachedCKYCRecord(
				request.panNumber || "",
			);
	...
	async registerKRA(
		request: KRARegistrationRequest,
	...
	async registerCVL(
		request: CVLRegistrationRequest,
	): Promise<CVLRegistrationResponse> {
		try {
	...
	async performComprehensiveKYC(user: User): Promise<{
		...
		if (results.kra?.success && user.cdslBoId) {
