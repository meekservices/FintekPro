...
(async () => {
  ...
  try {
    ...
    try {
      ...
      console.warn('⚠️ [Boot] API Gateway status check failed:', err instanceof Error ? err.message : String(err));
    }
    
    logBootProgress("Step 3b: Initializing Credit Ratings Service...");
    // Pre-cache credit ratings to avoid latency on first bond view
    try {
      await creditRatingsService.initialize();
      console.log('✅ Credit ratings service initialized');
    } catch (err) {
      console.warn('⚠️ [Boot] Credit ratings pre-caching failed:', err instanceof Error ? err.message : String(err));
    }
    
    logBootProgress("Step 3c: Initializing Symbol Mapping...");
    // Initialize symbol mapping for unified instrument tracking
    try {
      await symbolMappingService.initialize();
      console.log('✅ Symbol mapping service initialized');
    } catch (err) {
      console.warn('⚠️ [Boot] Symbol mapping initialization failed:', err instanceof Error ? err.message : String(err));
    }

    logBootProgress("Step 4: Registering Application Routes...");
    // Register business routes (CRM, KYC, Trades, etc.)
    const { setupStaticResouces } = await import("./routes");
    await setupStaticResouces(app);
    await registerRoutes(app);
    
    // Register role-specific dashboard routes
    await registerRoleRoutes(app);
    
    // Auth event consumers for analytics/compliance
    registerAuthEventConsumers();

    logBootProgress("Step 5: Initializing Background Tasks...");
    // Start hourly market data sync and notification workers
    initializeCronJobs();

    logBootProgress("Step 6: Registering SPA Catch-all...");
    // Register the final catch-all route for the SPA (this is for deep links)
    // registered at the very end of the routing chain.
    ...
    console.error('❌ [FATAL] Critical error during route registration:', error);
    logger.error('Critical error during route registration', { error: error.message, stack: error.stack });
    
    // Even if registration fails, we stay alive for health checks and diagnostics
    bootState.routesReady = true; // Set to true so 503 gate opens, though routes may be partial
  }
} catch (err) {
  const error = err instanceof Error ? err : new Error(String(err));
  bootState.error = error.message;
  console.error('❌ [FATAL] Handled boot failure:', error);
  logger.error('Handled boot failure', { error: error.message, stack: error.stack });
  // We don't process.exit(1) because we want the health probe to stay 200
  // so the user can see the error in the Diagnostic UI instead of a 502/CrashLoop.
}
})();
