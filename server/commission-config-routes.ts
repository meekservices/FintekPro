...
async function sendCommissionNotification(
	...
	try {
		...
		const admins = await db
			...
			.from(users)
			.where(inArray(users.roles as any, adminRoles));
    ...
    `;
		...
		console.log(
			`[Commission Notification] Sent ${notification.eventType} notification to ${admins.length} admins for ${notification.productType}`,
		);
	} catch (error) {
		console.error(
			"[Commission Notification] Failed to send notification:",
			error,
		);
	}
}
...
const COMMISSION_ROLE_PERMISSIONS: Record<
	CommissionRole,
...
function getUserCommissionRole(req: Request): CommissionRole {
	...
	if (normalizedRoles.includes("compliance_officer"))
...
const createCommissionPlanSchema = z.object({
	product_type: z.enum(
		CommissionProductTypes as unknown as [string, ...string[]],
	),
	roles: z.array(
		z.object({
			role_id: z.string(),
...
router.get(
	"/commission-plans",
	requireCommissionPermission("canView"),
...
router.post(
	"/commission-plan",
	requireCommissionPermission("canCreate"),
	async (req: Request, res: Response) => {
		try {
			...
			const totalPercentage = data.roles.reduce(
			...
			const maxRolePercentage = Math.max(
				...data.roles.map((r) => r.percentage),
			);
			...
			if (effectiveDate < today && !isSuperadmin) {
				return res.status(400).json({
					error: "Validation failed",
					message:
						"Cannot backdate commission plans without Superadmin override",
				});
			}
...
router.put(
	"/commission-plan/:id",
	requireCommissionPermission("canEdit"),
	async (req: Request, res: Response) => {
		try {
			const planId = Number.parseInt(req.params.id);
			const userId = (req as any).session?.userId;
			...
			if (existingPlan.status === "active") {
				return res
					.status(400)
					.json({
						error: "Cannot edit active plan. Create a new version instead.",
					});
			}
			...
			if (totalPercentage > 100) {
				return res
					.status(400)
					.json({ error: "Total payout percentage cannot exceed 100%" });
			}

			const oldRoleMaps = await db
				.select()
				.from(commissionRoleMaps)
				.where(eq(commissionRoleMaps.commissionPlanId, planId));
			...
			await db
				.delete(commissionHierarchySplits)
				.where(eq(commissionHierarchySplits.commissionPlanId, planId));
			...
			for (const newRole of data.roles) {
				...
				const oldPct = oldRole
					? Number.parseFloat(oldRole.payoutPercentage.toString())
					: undefined;
			...
			for (const oldRole of oldRoleMaps) {
				if (!data.roles.find((r) => r.role_id === oldRole.roleId)) {
					percentageChanges.push({
						...
						oldPercentage: Number.parseFloat(
							oldRole.payoutPercentage.toString(),
						),
...
router.post(
	"/commission-plan/:id/activate",
	requireCommissionPermission("canActivate"),
	async (req: Request, res: Response) => {
		try {
			const planId = Number.parseInt(req.params.id);
			const userId = (req as any).session?.userId;
...
router.post(
	...
	async (req, res) => {
		try {
			...
			const updatedRequest = await AdminApprovalService.updateStatus(
				requestId,
				{
					status,
					reviewedBy: userId,
			...
			if (
				status === "approved" &&
				updatedRequest.entityType === "commission_plan" &&
				updatedRequest.action === "activate"
			) {
...
router.post(
	"/commission-plan/:id/freeze",
	requireCommissionPermission("canFreeze"),
	async (req: Request, res: Response) => {
		try {
			const planId = Number.parseInt(req.params.id);
			const userId = (req as any).session?.userId;
...
router.get(
	"/active-commission-plan/:productType",
	async (req: Request, res: Response) => {
		try {
			...
			if (!plan) {
				return res
					.status(404)
					.json({
						error: "No active commission plan found for this product type",
					});
			}
