...
export class TruthscreenAadhaarService {
  ...
  private static encrypt(payload: object): string {
    ...
    const key = CryptoJS.enc.Utf8.parse(password.padEnd(32, '0').substring(0, 32));
   ...
   */
  static async validateAadhaar(aadhaarNumber: string): Promise<TruthscreenValidationResponse> {
    try {
      ...
      if (!this.isConfigured()) {
        ...
        return this.mockValidateAadhaar(aadhaarNumber);
      }
      ...
      const response = await axios.post(
        `${this.BASE_URL}${this.NID_ENDPOINT}`,
   ...
   */
  static async checkPanAadhaarLinkage(pan: string, aadhaar: string): Promise<TruthscreenPanAadhaarResponse> {
    try {
      ...
      if (!this.isConfigured()) {
        ...
        return this.mockCheckPanAadhaarLinkage(pan, aadhaar);
      }
      ...
      const response = await axios.post(
        `${this.BASE_URL}${this.PAN_AADHAAR_ENDPOINT}`,
        { requestData: encryptedPayload },
        { headers: this.getHeaders(), timeout: 30000 }
      );
      ...
      const linkStatus = decryptedResponse.linkStatus || decryptedResponse.status;
      const isLinked = linkStatus === 'LINKED' || decryptedResponse.linked === true;
      
      return {
        success: true,
        message: isLinked ? 'PAN and Aadhaar are linked' : 'PAN and Aadhaar are not linked',
        linked: isLinked,
      ...
      console.error('[Truthscreen] PAN-Aadhaar linkage check error:', error.message);
      ...
      if (error.response?.data) {
        try {
          ...
          return {
            success: false,
            message: decryptedError.message || 'Failed to check PAN-Aadhaar linkage',
            linked: false
          };
        } catch {
   ...
   */
  static async generateOTP(aadhaarNumber: string): Promise<TruthscreenOTPResponse> {
    try {
      if (!/^\d{12}$/.test(aadhaarNumber)) {
        return {
          success: false,
      ...
      if (!this.isConfigured()) {
        ...
        return this.mockGenerateOTP(aadhaarNumber);
      }
   ...
   */
  static async verifyOTP(refId: string, otp: string): Promise<TruthscreenVerificationResponse> {
    try {
      if (!refId || !otp) {
        return {
          success: false,
      ...
      if (!this.isConfigured()) {
        ...
        return this.mockVerifyOTP(refId, otp);
      }
  ...
  private static mockValidateAadhaar(aadhaarNumber: string): TruthscreenValidationResponse {
    const maskedAadhaar = `XXXX XXXX ${aadhaarNumber.slice(-4)}`;
  ...
  private static mockCheckPanAadhaarLinkage(pan: string, aadhaar: string): TruthscreenPanAadhaarResponse {
  ...
  private static mockGenerateOTP(aadhaarNumber: string): TruthscreenOTPResponse {
    const maskedAadhaar = `XXXX XXXX ${aadhaarNumber.slice(-4)}`;
  ...
  private static mockVerifyOTP(refId: string, otp: string): TruthscreenVerificationResponse {
    if (otp !== '123456') {
      return {
        success: false,
