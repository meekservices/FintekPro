...
interface EnrichmentProgress {
	...
	phase2Stats: {
		...
		minSipUpdated: number;
		minLumpsumUpdated: number;
		launchDateUpdated: number;
	};
	phase3Stats: {
		subCategoryUpdated: number;
		launchDateFromNav: number;
		metaFetched: number;
	};
	phase4Stats: {
	...
	phase5Stats: {
		exitLoadDefaults: number;
		minSipDefaults: number;
		minLumpsumDefaults: number;
		subCategoryDefaults: number;
	};
	phase6Stats: {
		ratingsComputed: number;
		ratingsPersisted: number;
...
function createEmptyProgress(): EnrichmentProgress {
	return {
		...
		phase5Stats: {
			exitLoadDefaults: 0,
			minSipDefaults: 0,
			minLumpsumDefaults: 0,
			subCategoryDefaults: 0,
		},
...
const CATEGORY_EXIT_LOAD_DEFAULTS: Record<
	string,
...
class MFComprehensiveEnrichmentService {
	...
	static getInstance(): MFComprehensiveEnrichmentService {
		if (!MFComprehensiveEnrichmentService.instance) {
			MFComprehensiveEnrichmentService.instance =
				new MFComprehensiveEnrichmentService();
		}
		return MFComprehensiveEnrichmentService.instance;
	}
	...
	private async logAuditChange(
		...
		try {
			...
			console.warn(
				`[AuditLog] Failed to log change for ${schemeCode}.${fieldName}: ${error.message}`,
			);
		}
	}
	...
	private async recordAumHistory(
		...
		try {
			...
			const previousRecords = await getEnrichmentReadDb(db)
				.select({
			...
			if (previousRecords.length > 0 && previousRecords[0].aum) {
				const prevAum = Number.parseFloat(previousRecords[0].aum);
				if (prevAum > 0) {
			...
			console.warn(
				`[AumHistory] Failed to record AUM for ${schemeCode}: ${error.message}`,
			);
		}
	}
	...
	private parseAUM(aumStr: string): number | null {
		...
		if (croreMatch) return Number.parseFloat(croreMatch[1]);
		const lakhMatch = cleaned.match(/^([\d.]+)\s*(L|Lakh|lakh)?$/i);
		if (lakhMatch) return Number.parseFloat(lakhMatch[1]) / 100;
	...
	private mapSchemeCategory(category: string): string | null {
		...
		if (lower.includes("medium duration") || lower.includes("medium to long"))
		...
		if (lower.includes("large cap") && lower.includes("mid cap"))
		...
		if (lower.includes("value") || lower.includes("contra"))
			return "Value/Contra";
		...
		if (lower.includes("balanced advantage") || lower.includes("dynamic asset"))
		...
		if (
			...
			return "Index Funds";
		if (lower.includes("fof") || lower.includes("fund of fund"))
	...
	private async fetchAmfiData(): Promise<Map<string, AmfiNavRecord>> {
		...
		try {
			console.log(
				"[ComprehensiveEnrichment] Fetching official AMFI NAV data from amfiindia.com...",
			);
			...
			console.log(
				`[ComprehensiveEnrichment] Loaded ${data.size} schemes from official AMFI feed`,
			);
			return data;
		} catch (error: any) {
			console.warn(
				"[ComprehensiveEnrichment] AMFI fetch failed:",
				error.message,
			);
			return this.amfiCache;
		}
	...
	private async fetchMFAPIWithRetry(
		schemeCode: string,
		retryAttempt = 0,
	): Promise<MFAPIResponse | null> {
		try {
			...
			if (isRateLimit && retryAttempt < 3) {
				...
				this.currentDelay = Math.min(
					800 * 2 ** this.consecutiveRateLimits,
					60000,
				);
	...
	private calculateCAGR(
		...
		return ((currentNav / oldNav) ** (1 / years) - 1) * 100;
	}

	private findClosestNav(
		navData: MFAPINavPoint[],
		targetDate: Date,
		maxDaysOffset = 10,
		...
		for (const point of navData) {
			...
			if (diff < closestDiff && diff <= maxDaysOffset * 24 * 60 * 60 * 1000) {
				closestDiff = diff;
				closest = { nav: Number.parseFloat(point.nav), date: pointDate };
			}
		}
		return closest;
	}

	private calculateReturns(navData: MFAPINavPoint[]): {
		...
		const currentNav = Number.parseFloat(sorted[0].nav);
		const currentDate = this.parseAmfiDate(sorted[0].date);
		if (!currentDate || Number.isNaN(currentNav))
			return { returns1y: null, returns3y: null, returns5y: null };
	...
	private calculateFinancialRatios(navData: MFAPINavPoint[]): {
		...
		for (let i = 1; i < sorted.length; i++) {
			const prev = Number.parseFloat(sorted[i - 1].nav);
			const curr = Number.parseFloat(sorted[i].nav);
		...
		const variance =
			dailyReturns.reduce((sum, ret) => sum + (ret - meanReturn) ** 2, 0) /
		...
		const downsideVariance =
			negativeReturns.length > 0
				? negativeReturns.reduce((sum, ret) => sum + ret ** 2, 0) /
		...
		let maxDrawdown = 0;
		let peak = Number.parseFloat(sorted[0].nav);
		for (const point of sorted) {
			const nav = Number.parseFloat(point.nav);
		...
		return {
			standardDeviation: Number.parseFloat(annualizedStdDev.toFixed(4)),
			sharpeRatio:
				sharpeRatio !== null ? Number.parseFloat(sharpeRatio.toFixed(4)) : null,
			sortinoRatio:
				sortinoRatio !== null
					? Number.parseFloat(sortinoRatio.toFixed(4))
					: null,
			maxDrawdown: Number.parseFloat(maxDrawdown.toFixed(4)),
		};
	}

	async runComprehensiveEnrichment(
		options: {
		...
		try {
			const [countResult] = await getEnrichmentReadDb(db)
			...
			enrichmentProgress.currentStep =
				"All enrichment phases completed (including FintekPro Smart Ratings)";
			enrichmentProgress.duration =
				Date.now() - enrichmentProgress.startedAt!.getTime();
			console.log(
				`[ComprehensiveEnrichment] All phases completed in ${enrichmentProgress.duration}ms`,
			);
		} catch (error: any) {
			...
			enrichmentProgress.duration =
				Date.now() - (enrichmentProgress.startedAt?.getTime() || Date.now());
		}

		return enrichmentProgress;
	}

	private async phase1_AmfiEnrichment(
		batchSize: number,
		enrichmentRunId: string,
	): Promise<void> {
		enrichmentProgress.status = "phase1_amfi";
		enrichmentProgress.currentStep =
			"Phase 1: Fetching official AMFI NAV feed (amfiindia.com)...";
		console.log(
			"[ComprehensiveEnrichment] Phase 1: Official AMFI feed enrichment",
		);
		...
		const funds = await getEnrichmentReadDb(db)
			...
			.where(
				or(
					isNull(mutualFunds.category),
					isNull(mutualFunds.isin),
					isNull(mutualFunds.fundHouse),
					isNull(mutualFunds.nav),
				),
			);
			...
			const updatePromises = batch.map(async (fund) => {
				try {
					...
					if (
						...
						amfiRow.isinDividendReinvest &&
						amfiRow.isinDividendReinvest !== "-" &&
						amfiRow.isinDividendReinvest.length > 3
					) {
					...
					enrichmentProgress.errors.push(
						`P1 ${fund.schemeCode}: ${error.message}`,
					);
				}
			});
		...
		console.log(
			`[ComprehensiveEnrichment] Phase 1 done: Category=${enrichmentProgress.phase1Stats.categoryUpdated}, ISIN=${enrichmentProgress.phase1Stats.isinUpdated} (source: AMFI official feed)`,
		);
	}

	private async phase2_ExtendedDataExtraction(
		batchSize: number,
		enrichmentRunId: string,
	): Promise<void> {
		enrichmentProgress.status = "phase2_extdata";
		enrichmentProgress.currentStep =
			"Phase 2: Extracting from extendedData JSONB...";
			...
			for (const fund of batch) {
				try {
					...
					if (fund.minSipAmount === null && ext.minSipAmount) {
						...
						if (val) {
							updates.minSipAmount = val.toString();
							enrichmentProgress.phase2Stats.minSipUpdated++;
						}
					}
					...
					if (fund.minLumpsumAmount === null && ext.minInvestment) {
						...
						if (val) {
							updates.minLumpsumAmount = val.toString();
							enrichmentProgress.phase2Stats.minLumpsumUpdated++;
						}
					}
					...
					if (fund.launchDate === null && ext.launchDate) {
						...
						if (parsed) {
							updates.launchDate = parsed;
							enrichmentProgress.phase2Stats.launchDateUpdated++;
						}
					}
					...
					enrichmentProgress.errors.push(
						`P2 ${fund.schemeCode}: ${error.message}`,
					);
				}
			}
		}
		...
		console.log(
			`[ComprehensiveEnrichment] Phase 2 done: ExitLoad=${enrichmentProgress.phase2Stats.exitLoadUpdated}, MinSIP=${enrichmentProgress.phase2Stats.minSipUpdated}, MinLumpsum=${enrichmentProgress.phase2Stats.minLumpsumUpdated}, LaunchDate=${enrichmentProgress.phase2Stats.launchDateUpdated}`,
		);
	}

	private async phase3_MFAPIMetadata(
		maxFunds: number,
		enrichmentRunId: string,
	): Promise<void> {
		enrichmentProgress.status = "phase3_mfapi_meta";
		enrichmentProgress.currentStep =
			"Phase 3: Fetching MFapi.in metadata (sub-category, launch date)...";
		console.log(
			"[ComprehensiveEnrichment] Phase 3: MFapi.in metadata enrichment",
		);
		...
		for (let i = 0; i < funds.length; i++) {
			...
			try {
				...
				enrichmentProgress.errors.push(
					`P3 ${fund.schemeCode}: ${error.message}`,
				);
			}
		}

		console.log(
			`[ComprehensiveEnrichment] Phase 3 done: SubCategory=${enrichmentProgress.phase3Stats.subCategoryUpdated}, LaunchDate=${enrichmentProgress.phase3Stats.launchDateFromNav}, MetaFetched=${enrichmentProgress.phase3Stats.metaFetched}`,
		);
	}

	private async phase4_MFAPIReturns(
		maxFunds: number,
		enrichmentRunId: string,
	): Promise<void> {
		enrichmentProgress.status = "phase4_mfapi_returns";
		enrichmentProgress.currentStep =
			"Phase 4: Calculating returns & ratios from MFapi.in historical NAV...";
		...
		const funds = await getEnrichmentReadDb(db)
			...
			.from(mutualFunds)
			.where(isNull(mutualFunds.returns1y))
			.limit(maxFunds);
		...
		for (let i = 0; i < funds.length; i++) {
			...
			try {
				...
				if (returns.returns1y !== null)
					updates.returns1y = returns.returns1y.toFixed(4);
				if (returns.returns3y !== null)
					updates.returns3y = returns.returns3y.toFixed(4);
				if (returns.returns5y !== null)
					updates.returns5y = returns.returns5y.toFixed(4);
				...
				if (Object.keys(updates).length > 1) {
					...
					if (returns.returns1y !== null)
						enrichmentProgress.phase4Stats.returnsUpdated++;
					if (ratios.sharpeRatio !== null)
						enrichmentProgress.phase4Stats.ratiosUpdated++;
            ...
            `);
						...
						console.error(
							`[ComprehensiveEnrichment] P4 metrics upsert failed for ${fund.schemeCode}:`,
							metricsErr,
						);
					}
				...
				if (i % 20 === 0) {
					console.log(
						`[ComprehensiveEnrichment] Phase 4 progress: ${i + 1}/${funds.length} (returns=${enrichmentProgress.phase4Stats.returnsUpdated}, ratios=${enrichmentProgress.phase4Stats.ratiosUpdated})`,
					);
				}
				...
				enrichmentProgress.errors.push(
					`P4 ${fund.schemeCode}: ${error.message}`,
				);
			}
		}

		console.log(
			`[ComprehensiveEnrichment] Phase 4 done: Returns=${enrichmentProgress.phase4Stats.returnsUpdated}, Ratios=${enrichmentProgress.phase4Stats.ratiosUpdated}, Synced=${enrichmentProgress.phase4Stats.fundsSynced}`,
		);
	}

	private async phase5_CategoryDefaults(
		batchSize: number,
		enrichmentRunId: string,
	): Promise<void> {
		enrichmentProgress.status = "phase5_defaults";
		enrichmentProgress.currentStep =
			"Phase 5: Applying category-based defaults for remaining nulls...";
		...
		for (let i = 0; i < funds.length; i += batchSize) {
			...
			for (const fund of batch) {
				try {
					const updates: Record<string, any> = {};
					const cat =
						fund.category || this.inferCategoryFromName(fund.schemeName || "");

					if (fund.exitLoadPercent === null) {
						const exitLoadDefault = cat
							? CATEGORY_EXIT_LOAD_DEFAULTS[cat]
							: null;
						if (exitLoadDefault) {
					...
					if (fund.minLumpsumAmount === null) {
						const amounts = cat ? CATEGORY_MIN_AMOUNTS[cat] : null;
						updates.minLumpsumAmount = (
							amounts?.lumpsum || DEFAULT_MIN_LUMPSUM
						).toString();
					...
					enrichmentProgress.errors.push(
						`P5 ${fund.schemeCode}: ${error.message}`,
					);
				}
			}
		}
		...
		console.log(
			`[ComprehensiveEnrichment] Phase 5 done: ExitLoad=${enrichmentProgress.phase5Stats.exitLoadDefaults}, MinSIP=${enrichmentProgress.phase5Stats.minSipDefaults}, MinLumpsum=${enrichmentProgress.phase5Stats.minLumpsumDefaults}, SubCategory=${enrichmentProgress.phase5Stats.subCategoryDefaults}`,
		);
	}
	...
	private inferCategoryFromName(name: string): string | null {
		...
		if (
			...
			return "Liquid";
		if (lower.includes("index") && !lower.includes("debt"))
			return "Index Funds";
		...
		if (lower.includes("large & mid") || lower.includes("large and mid"))
			return "Large & Mid Cap";
		if (lower.includes("large cap") || lower.includes("largecap"))
			return "Large Cap";
		...
		if (lower.includes("gilt")) return "Gilt";
		if (lower.includes("corporate bond")) return "Corporate Bond";
		if (lower.includes("banking") && lower.includes("psu"))
			return "Banking & PSU";
		if (lower.includes("credit risk")) return "Credit Risk";
		if (lower.includes("dynamic bond")) return "Dynamic Bond";
		if (lower.includes("floater") || lower.includes("floating"))
			return "Floater";
		if (lower.includes("ultra short")) return "Ultra Short Duration";
		if (lower.includes("short duration") || lower.includes("short term"))
			return "Short Duration";
		if (lower.includes("long duration") || lower.includes("long term"))
			return "Long Duration";
		if (lower.includes("medium duration") || lower.includes("medium term"))
			return "Medium Duration";
	...
	private parseExitLoadText(text: string | null | undefined): {
		percent: number | null;
		days: number | null;
	} {
		...
		if (
			...
			normalized === "0" ||
			normalized === "0%" ||
			normalized.includes("no exit load") ||
			normalized.includes("nil exit")
		) {
		...
		if (match) {
			...
			const timeValue = Number.parseInt(match[2]);
		...
		if (simpleDays)
			return {
				percent: Number.parseFloat(simpleDays[1]),
				days: Number.parseInt(simpleDays[2]),
			};
	...
	private parseDateString(dateStr: string | null | undefined): Date | null {
		...
		if (dash) {
			const d = new Date(
				`${dash[3]}-${dash[2].padStart(2, "0")}-${dash[1].padStart(2, "0")}`,
			);
			if (!Number.isNaN(d.getTime())) return d;
		}
	...
	private async phase6_FintekProRatings(
		enrichmentRunId: string,
	): Promise<void> {
		enrichmentProgress.status = "phase6_ratings";
		enrichmentProgress.currentStep =
			"Phase 6: Computing FintekPro Smart Ratings for unrated funds...";
		console.log(
			"[ComprehensiveEnrichment] Phase 6: FintekPro Smart Rating computation",
		);

		try {
			const { default: fintekProRatingService } = await import(
				"./fintekpro-rating-service"
			);
			const result = await fintekProRatingService.batchComputeAndPersist({
				...
				onProgress: (processed, total, schemeCode) => {
					...
					if (processed % 500 === 0) {
						console.log(
							`[Phase6] Rated ${processed}/${total} funds (latest: ${schemeCode})`,
						);
					}
				},
			});
			...
			console.log(
				`[Phase6] Complete: ${result.persisted} ratings persisted, ${result.failed} failed`,
			);
		} catch (error: any) {
	...
	async getNullColumnStats(): Promise<
		Record<string, { nullCount: number; filledCount: number; total: number }>
	> {
		...
		const result: Record<
			string,
			{ nullCount: number; filledCount: number; total: number }
		> = {};
...
export const mfComprehensiveEnrichmentService =
	MFComprehensiveEnrichmentService.getInstance();
