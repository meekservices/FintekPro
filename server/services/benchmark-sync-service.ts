...
class BenchmarkSyncService {
	...
	async syncAllBenchmarks(): Promise<{ synced: number; failed: string[] }> {
		...
		for (const mapping of INDEX_SYMBOL_MAPPINGS) {
			...
			for (let attempt = 0; attempt < 3 && !synced; attempt++) {
				try {
					if (attempt > 0) {
						const backoff = 15000 * 2 ** (attempt - 1);
						console.log(
							`[BenchmarkSync] Retry ${attempt}/2 for ${mapping.indexCode} after ${backoff / 1000}s`,
						);
					...
					if (attempt === 2) {
						...
						console.warn(
							`[BenchmarkSync] Failed ${mapping.indexCode} after 3 attempts: ${error.message?.substring(0, 80)}`,
						);
					}
				}
			}
		...
		console.log(
			`[BenchmarkSync] Completed: ${results.synced} synced, ${results.failed.length} failed`,
		);
		return results;
	}

	async syncBenchmarkIndex(
		indexCode: string,
		yahooSymbol: string,
	): Promise<void> {
		...
		if (!marketIndex) {
			console.warn(
				`[BenchmarkSync] Index ${indexCode} not found in market_indices table`,
			);
			return;
		}
		...
		console.log(
			`[BenchmarkSync] Fetching ${indexCode} from ${startDate.toISOString()} to ${endDate.toISOString()}`,
		);
		...
		try {
			...
			const quotes = historicalData.quotes.filter(
				(q) => q.close !== null && q.close !== undefined,
			);

			let previousClose: number | null = null;
			if (lastNav.length > 0 && lastNav[0].closeValue) {
				previousClose = Number.parseFloat(lastNav[0].closeValue);
			}
			...
			for (const quote of quotes) {
				...
				try {
					...
					console.error(
						`[BenchmarkSync] Insert error for ${indexCode} on ${navDateStr}:`,
						err,
					);
				}
			...
			console.log(
				`[BenchmarkSync] Inserted ${insertCount} records for ${indexCode}`,
			);
		} catch (error: any) {
			if (
				...
				console.warn(
					`[BenchmarkSync] Rate limited for ${yahooSymbol}, will retry later`,
				);
				throw error;
			}
			throw error;
	...
	async getIndexReturns(
		...
		startDate: Date,
		endDate: Date,
	): Promise<{ date: Date; dailyReturn: number }[]> {
		const [marketIndex] = await db
			.select()
			.from(marketIndices)
			.where(eq(marketIndices.indexCode, indexCode))
		...
		return navData
			...
			.map((n) => ({
				...
				dailyReturn: Number.parseFloat(n.dailyReturn!),
			}));
	}

	async getIndexCloseValues(
		indexCode: string,
		startDate?: Date,
		endDate?: Date,
	): Promise<{ date: Date; closeValue: number }[]> {
		const [marketIndex] = await db
			.select()
			.from(marketIndices)
			.where(eq(marketIndices.indexCode, indexCode))
		...
		return navData
			...
			.map((n) => ({
				date: new Date(n.navDate!),
				closeValue: Number.parseFloat(n.closeValue),
			}));
	}
	...
	> {
		...
		for (const index of indices) {
			...
			coverage.push({
				...
				latestDate:
					navData.length > 0
						? String(navData[navData.length - 1].navDate)
						: null,
			});
		}
