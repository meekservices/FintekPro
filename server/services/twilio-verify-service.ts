...
class TwilioVerifyService {
  ...
  constructor() {
    ...
    if (accountSid && authToken && verifySid) {
      ...
      console.log('⚠️ Twilio Verify service partially configured - missing TWILIO_VERIFY_SERVICE_SID');
      console.log('   Create a Verify Service in Twilio Console and set TWILIO_VERIFY_SERVICE_SID');
    } else {
      this.isConfigured = false;
      console.log('⚠️ Twilio Verify service not configured - missing credentials');
    }
  }
  ...
  async sendVerification(
    ...
    if (!this.isConfigured) {
      console.log(`📱 Verify OTP to ${to.substring(0, 6)}**** via ${channel} (not configured)`);
      return { success: false, error: 'Verify service not configured - missing TWILIO_VERIFY_SERVICE_SID' };
    }
    ...
    try {
      ...
      console.log(`✅ Verification sent to ${formattedTo.substring(0, 8)}*** via ${channel} - SID: ${verification.sid}`);
      ...
      console.error(`❌ Failed to send verification via ${channel}:`, error.message);
      return { success: false, error: error.message };
    }
  }

  async checkVerification(to: string, code: string): Promise<VerificationCheckResult> {
    if (!this.isConfigured) {
      console.log(`🔍 Verify check for ${to.substring(0, 6)}**** with code ${code} (not configured)`);
    ...
    try {
      ...
      console.log(`🔍 Verification check for ${formattedTo.substring(0, 8)}***: ${verificationCheck.status}`);
  ...
  async sendOTPWithFallback(to: string, preferredChannel: VerifyChannel = 'sms'): Promise<VerificationResult> {
    const channels: VerifyChannel[] = ['sms', 'whatsapp', 'call'];
    const orderedChannels = [preferredChannel, ...channels.filter(c => c !== preferredChannel)];
