...
class UnifiedCompanyDataService {
	...
	private logTelemetry(telemetry: DataSourceTelemetry): void {
		...
		console.log(
			`${logLevel} [DataSource Telemetry] ${telemetry.companyIdOrCin} | ${sourcesInfo} | Quality: ${telemetry.dataQualityScore}% | Duration: ${telemetry.duration}ms`,
		);

		if (telemetry.fallbackTriggered) {
			console.log(
				`📊 [Fallback Analysis] Reason: ${telemetry.fallbackReason} | Attempted: [${telemetry.sourcesAttempted.join(", ")}] | Succeeded: [${telemetry.sourcesSucceeded.join(", ")}]`,
			);
		}
	...
	async getCompanyData(
		...
		console.log(
			`[UnifiedData] Fetching data for: ${companyIdOrCin} (Request: ${requestId})`,
		);
		...
		const cin =
			company?.cin ||
			(companyIdOrCin.length === 21 ? companyIdOrCin : undefined);
			...
			try {
				if (mcaService.isConfigured()) {
					...
					results.push({
						...
						source: "mca",
						error:
							"MCA service not configured (missing SANDBOX_API_KEY/SANDBOX_API_SECRET)",
					});
				}
			} catch (error: any) {
				results.push({
			...
			try {
				...
				if (dirResp.success && dirResp.data && dirResp.data.length > 0) {
					...
					results.push({
						success: true,
						source: "credhive",
						data: credhiveDirectorData,
					});
					console.log(
						`[UnifiedData] Got Credhive directors: ${credhiveDirectorData.directors?.length || 0}`,
					);
				}
			} catch (error: any) {
		...
		if (company?.cin && credhiveService.isAvailable())
			sourcesAttempted.push("credhive");
		...
		const fallbackTriggered =
			!fintekproHasUsableData && sourcesAttempted.includes("mca");
		let fallbackReason: string | undefined;

		if (fallbackTriggered) {
			fallbackReason =
				"No financial data in FintekPro database - using MCA fallback";
			if (company) {
				warnings.push(
					"Primary data source (FintekPro) has no financial records - using MCA data",
				);
			}
		}
		...
		const financialsSource = this.determineFinancialsSource(
			ownFinancials,
			mcaData,
		);
	...
	private aggregateData(
		...
		if (financialsSource === "fintekpro" && ownFinancials.length > 0) {
			...
			financialsSource === "mca" &&
			mcaData?.balanceSheets &&
			mcaData.balanceSheets.length > 0
		) {
		...
		const directorsData = this.buildDirectorsData(
			company,
			mcaData,
			credhiveDirectorData,
		);
	...
	private determineRatiosSource(ownRatios: CompanyRatios[]): DataSource {
	...
	private convertOwnFinancials(f: CompanyFinancials): FinancialData {
		return {
			...
			totalAssets: f.totalAssets ? Number.parseFloat(f.totalAssets) : undefined,
			totalLiabilities: f.totalLiabilities
				? Number.parseFloat(f.totalLiabilities)
				: undefined,
			totalDebt: f.totalDebt ? Number.parseFloat(f.totalDebt) : undefined,
			ebitda: f.ebitda ? Number.parseFloat(f.ebitda) : undefined,
			shareCapital: f.shareCapital
				? Number.parseFloat(f.shareCapital)
				: undefined,
	...
	private convertOwnRatios(r: CompanyRatios): RatiosData {
		return {
			...
			peRatio: r.peRatio ? Number.parseFloat(r.peRatio) : undefined,
			pbRatio: r.pbRatio ? Number.parseFloat(r.pbRatio) : undefined,
			roe: r.roe ? Number.parseFloat(r.roe) : undefined,
			roce: r.roce ? Number.parseFloat(r.roce) : undefined,
			debtEquity: r.debtEquity ? Number.parseFloat(r.debtEquity) : undefined,
			currentRatio: r.currentRatio
				? Number.parseFloat(r.currentRatio)
				: undefined,
			marginEbitda: r.marginEbitda
				? Number.parseFloat(r.marginEbitda)
				: undefined,
			marginPat: r.marginPat ? Number.parseFloat(r.marginPat) : undefined,
			marginOperating: r.marginOperating
				? Number.parseFloat(r.marginOperating)
				: undefined,
		};
	}
	...
	private buildCapitalData(
		...
		if (mcaData) {
			...
			return {
				...
				faceValue,
				totalShares:
					mcaData.paidUpCapital > 0
						? Math.floor(mcaData.paidUpCapital / faceValue)
						: undefined,
				source: "mca",
			};
		}
		...
		if (company) {
			return {
				...
				faceValue: company.faceValue
					? Number.parseFloat(company.faceValue)
					: undefined,
				totalShares: company.totalShares
					? Number(company.totalShares)
					: undefined,
				source: "fintekpro",
			};
		}
	...
	private buildDirectorsData(
		...
		if (
			credhiveDirectorData?.directors &&
			credhiveDirectorData.directors.length > 0
		) {
			return {
				...
				for (const yearData of data.financials.years) {
					...
					if (!existing) {
						const financialsToInsert: InsertCompanyFinancials = {
							...
							totalLiabilities:
								yearData.data.totalLiabilities?.toString() || null,
