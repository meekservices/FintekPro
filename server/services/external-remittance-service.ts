...
class ExternalRemittanceService {
	...
	async createRemittanceRequest(
		request: RemittanceUploadRequest,
	): Promise<RemittanceProofDocument> {
		...
		try {
			const [inserted] = await db
				.insert(externalRemittanceProofs)
				.values({
			...
			console.log(
				`[RemittanceTracking] Created remittance request ${inserted.id} for ${request.productType} order ${request.orderId}`,
			);

			return this.mapDbToInterface(inserted);
		} catch (error) {
			console.error(
				"[RemittanceTracking] Failed to create remittance request:",
				error,
			);
			throw error;
		}
	...
	private mapDbToInterface(record: any): RemittanceProofDocument {
		return {
			...
			expectedAmount: Number.parseFloat(record.expectedAmount),
	...
	async uploadRemittanceProof(
		upload: RemittanceDocumentUpload,
		...
		try {
			...
			if (remittance.userId !== upload.userId) {
				return {
					success: false,
					message: "Unauthorized - user does not own this remittance request",
				};
			}
			...
			if (remittance.status !== "pending_upload") {
				return {
					success: false,
					message: `Cannot upload proof for remittance in ${remittance.status} status`,
				};
			}

			if (remittance.expiresAt && new Date() > remittance.expiresAt) {
				await db
					.update(externalRemittanceProofs)
					.set({ status: "expired", updatedAt: new Date() })
					.where(eq(externalRemittanceProofs.id, upload.remittanceId));
				return {
					success: false,
					message: "Remittance upload window has expired",
				};
			}
			...
			console.log(
				`[RemittanceTracking] Proof uploaded for remittance ${upload.remittanceId}`,
			);

			return {
				success: true,
				message:
					"Remittance proof uploaded successfully. Awaiting verification.",
			};
		} catch (error) {
	...
	async verifyRemittance(
		request: RemittanceVerificationRequest,
	): Promise<{ success: boolean; message: string }> {
		try {
			const [remittance] = await db
				.select()
			...
			if (
				remittance.status !== "uploaded" &&
				remittance.status !== "under_review"
			) {
				return {
					success: false,
					message: `Cannot verify remittance in ${remittance.status} status`,
				};
			}
			...
			if (request.action === "verify") {
				...
				console.log(
					`[RemittanceTracking] Remittance ${request.remittanceId} VERIFIED by ${request.verifierId}`,
				);
			...
			console.log(
				`[RemittanceTracking] Remittance ${request.remittanceId} REJECTED: ${rejectionReason}`,
			);
	...
	async getRemittance(
		remittanceId: string,
	): Promise<RemittanceProofDocument | undefined> {
		try {
	...
	async getRemittancesByOrder(
		...
		try {
			...
			console.error(
				"[RemittanceTracking] Failed to get remittances by order:",
				error,
			);
			return [];
		}
	...
	async getRemittancesByUser(
		...
		try {
			...
			console.error(
				"[RemittanceTracking] Failed to get remittances by user:",
				error,
			);
			return [];
		}
	...
	async getPendingVerifications(): Promise<RemittanceProofDocument[]> {
		try {
			const records = await db
				.select()
				.from(externalRemittanceProofs)
				.where(and(eq(externalRemittanceProofs.status, "uploaded")))
				.orderBy(externalRemittanceProofs.uploadedAt);
			return records.map((r) => this.mapDbToInterface(r));
		} catch (error) {
			console.error(
				"[RemittanceTracking] Failed to get pending verifications:",
				error,
			);
			return [];
		}
		...
		try {
			...
			return {
				...
				totalVerifiedAmount: all
					...
					.reduce(
						(sum, r) => sum + Number.parseFloat(r.expectedAmount || "0"),
						0,
					),
			...
			console.error(
				"[RemittanceTracking] Failed to get compliance report:",
				error,
			);
			return {
				totalRemittances: 0,
	...
	async checkRemittanceProofRequired(
		productType: "aif" | "pms",
		orderId: string,
	): Promise<{
		required: boolean;
