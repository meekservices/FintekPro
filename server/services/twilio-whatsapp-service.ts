...
class TwilioWhatsAppService {
	...
	constructor() {
		...
		console.log(
			"📱 WhatsApp service initialized (using Replit Twilio connector)",
		);
		if (this.messagingServiceSid) {
			console.log(
				`   Messaging Service SID: ${this.messagingServiceSid.substring(0, 10)}...`,
			);
		}
	...
	async sendMessage(
		...
		if (!isConfigured) {
			console.log(
				`📱 WhatsApp message to ${to.substring(0, 6)}****: ${body.substring(0, 50)}... (not configured)`,
			);
		...
		try {
			...
			const templateSid = templateType
			...
			console.log(
				`⚠️ No template configured for ${templateType || "notification"}, falling back to freeform message...`,
			);
			return this.sendFreeformMessage(toNumber, body, mediaUrl);
		} catch (error: any) {
	...
	private async sendFreeformMessage(
		...
		try {
			...
			const fromNumber =
				this.whatsappNumber || (await getTwilioFromPhoneNumber());
			...
			if (fromNumber) {
				messageOptions.from = fromNumber.startsWith("whatsapp:")
					? fromNumber
					: `whatsapp:${fromNumber}`;
			}
			...
			console.log(
				`📱 Sending freeform WhatsApp message to ${toNumber.substring(0, 15)}***`,
			);
	...
	async sendTemplateMessage(
		...
		try {
			...
			console.log(
				`📱 Sending template WhatsApp message to ${toNumber.substring(0, 15)}*** (template: ${contentSid})`,
			);
			...
			console.error(
				"❌ Failed to send template WhatsApp message:",
				error.message,
			);
			return { success: false, error: error.message };
		}
