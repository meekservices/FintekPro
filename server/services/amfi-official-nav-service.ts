...
class AmfiOfficialNavService {
	...
	static getInstance(): AmfiOfficialNavService {
		if (!AmfiOfficialNavService.instance) {
			AmfiOfficialNavService.instance = new AmfiOfficialNavService();
		}
		return AmfiOfficialNavService.instance;
	}
	...
	async syncNavToDatabase(
		...
		try {
				...
				const batchPromises = batch.map(async (record) => {
					try {
						...
						const isValidIsin = (isin: string): boolean => {
							if (!isin || isin === "-" || isin === "N/A" || isin.trim() === "")
								return false;
							return isin.startsWith("INF") && isin.length >= 12;
						};

						const validGrowthIsin = isValidIsin(record.isinGrowth)
							? record.isinGrowth
							: undefined;
						const validDividendIsin = isValidIsin(record.isinDividendReinvest)
						...
						if (updateResult.length > 0) {
							...
							if (parsedNavDate) {
								try {
									...
									console.warn(
										`[AMFI Official] History insert failed for ${record.schemeCode}:`,
						...
						if (result.errorDetails.length < 10) {
							result.errorDetails.push(
								`${record.schemeCode}: ${error.message}`,
							);
						}
			...
			console.log(
				`[AMFI Official] Sync completed: ${result.updatedFunds} funds updated in ${result.syncDuration}ms`,
			);
		} catch (error: any) {
