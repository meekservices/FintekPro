...
class ClientFeeModeService {
	...
	async updateAdminSettings(
		updates: Partial<GlobalInvestmentAdminSettings>,
		updatedBy: string,
	): Promise<boolean> {
		try {
			...
			if (!existing) {
				...
				await db
					.update(globalInvestmentAdminSettings)
					.set({
						...updates,
						policyUpdatedBy: updatedBy,
						policyUpdatedAt: new Date(),
	...
	async getClientFeeMode(
		clientId: string,
	): Promise<GlobalInvestmentClientFeeMode | null> {
		try {
	...
	async resolveClientCapabilities(
		clientId: string,
		...
		const requiresModeSelection = !feeModeSelected;
	...
	async selectFeeMode(
		request: FeeModeSelectionRequest,
	): Promise<{ success: boolean; error?: string }> {
		const { clientId, feeMode, disclaimerAcknowledged, ipAddress, userAgent } =
			request;
		...
		if (!adminSettings?.allowClientSelfSelection) {
			return {
				success: false,
				error: "Self-selection is not enabled. Please contact support.",
			};
		}
		...
		try {
			...
			if (existingMode) {
				await db
					.update(globalInvestmentClientFeeMode)
					.set({
						feeMode,
	...
	async adminOverrideMode(
		...
		if (!reason || reason.trim().length < 10) {
			return {
				success: false,
				error: "A detailed reason is required for admin overrides",
			};
		}
	...
	async calculateFees(
		...
		if (adminSettings.platformFeeCapInr) {
			platformFeeAmount = Math.min(
				platformFeeAmount,
				Number.parseFloat(adminSettings.platformFeeCapInr),
			);
		}
		...
		if (feeMode === "ADVISORY_PLATFORM") {
			...
			if (adminSettings.advisoryFeeCapInr) {
				advisoryFeeAmount = Math.min(
					advisoryFeeAmount,
					Number.parseFloat(adminSettings.advisoryFeeCapInr),
				);
			}
		}
		...
		return {
			...
			totalFeeAmount:
				Math.round((advisoryFeeAmount + platformFeeAmount) * 100) / 100,
			feeMode,
		};
	}
	...
	async getFeeModeStatistics(): Promise<{
		...
		try {
			...
			const advisoryPlatform = allModes.filter(
				(m) => m.feeMode === "ADVISORY_PLATFORM",
			).length;
			const platformOnly = allModes.filter(
				(m) => m.feeMode === "PLATFORM_ONLY",
			).length;
	...
	async generateZohoInvoiceLineItems(
		...
		if (isAdvisory && feeBreakdown.advisoryFeeAmount > 0) {
			lineItems.push({
				...
				description:
					`Investment advisory service fee for ${orderDetails.symbol} (${orderDetails.assetClass}). ` +
		...
		lineItems.push({
			...
			description:
				`Execution platform service fee for ${orderDetails.symbol} (${orderDetails.assetClass}). ` +
				`Order ID: ${orderDetails.orderId}, Qty: ${orderDetails.quantity}, ` +
				`Order Value: ₹${orderValueInr.toLocaleString("en-IN")}. ` +
