...
class OTPThrottleService {
	...
	canRequestOTP(
		...
		const timeSinceLastAttempt =
			(Date.now() - attempt.lastAttemptAt.getTime()) / 1000;
		if (timeSinceLastAttempt < this.COOLDOWN_SECONDS) {
			const remainingCooldown = Math.ceil(
				this.COOLDOWN_SECONDS - timeSinceLastAttempt,
			);
			return {
				allowed: false,
	...
	recordAttempt(sessionId: string, userId: string, aadhaarHash: string): void {
		...
		console.log(
			`📊 [OTP Throttle] Recorded attempt for user ${userId.substring(0, 8)}...: ${this.attempts.get(key)?.attempts}/${this.MAX_ATTEMPTS}`,
		);
	}
	...
	resetAttempts(userId: string, aadhaarHash: string): void {
		...
		this.attempts.delete(key);
		console.log(
			`✅ [OTP Throttle] Reset attempts for user ${userId.substring(0, 8)}...`,
		);
	}
	...
	cleanup(): void {
		...
		for (const [key, attempt] of this.attempts.entries()) {
			...
			const age =
				(now.getTime() - attempt.createdAt.getTime()) / 1000 / 60 / 60;
