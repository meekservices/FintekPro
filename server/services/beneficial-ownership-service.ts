...
interface CreateDeclarationInput {
  ...
  registeredAddress: string;
  declarationType: 'initial' | 'annual_update' | 'change_notification';
  significantBeneficialOwners: Omit<BeneficialOwner, 'id' | 'declarationDate'>[];
...
class BeneficialOwnershipService {
  private readonly REGULATORY_REFERENCE = 'Companies (Significant Beneficial Owners) Rules 2018';
  ...
  private declarationsStore: Map<string, BeneficialOwnershipDeclaration> = new Map();

  async createDeclaration(input: CreateDeclarationInput): Promise<BeneficialOwnershipDeclaration> {
    const declarationId = `BOD_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    const now = new Date();
    const validUntil = new Date(now.getTime() + this.DECLARATION_VALIDITY_DAYS * 24 * 60 * 60 * 1000);
    
    const beneficialOwners: BeneficialOwner[] = input.significantBeneficialOwners.map((sbo, index) => ({
      ...sbo,
      id: `SBO_${declarationId}_${index + 1}`,
      declarationDate: now,
      isUltimateOwner: sbo.ownershipPercentage >= this.UBO_THRESHOLD || sbo.votingRightsPercentage >= this.UBO_THRESHOLD,
    }));
  ...
  private validateSBOThresholds(owners: BeneficialOwner[]): void {
    for (const owner of owners) {
      if (owner.ownershipPercentage < this.SBO_THRESHOLD_SHARES && owner.votingRightsPercentage < this.SBO_THRESHOLD_VOTING) {
        console.warn(`Owner ${owner.name} may not qualify as SBO: ownership ${owner.ownershipPercentage}%, voting ${owner.votingRightsPercentage}%`);
      }
    }
  ...
  async verifyDeclaration(declarationId: string, verifierId: string): Promise<{ success: boolean; message: string }> {
  ...
  async updateDeclaration(declarationId: string, updates: Partial<CreateDeclarationInput>, updatedBy: string): Promise<BeneficialOwnershipDeclaration | null> {
    ...
    if (updates.significantBeneficialOwners) {
      existing.significantBeneficialOwners = updates.significantBeneficialOwners.map((sbo, index) => ({
        ...sbo,
        id: `SBO_${declarationId}_${index + 1}`,
        declarationDate: now,
        isUltimateOwner: sbo.ownershipPercentage >= this.UBO_THRESHOLD || sbo.votingRightsPercentage >= this.UBO_THRESHOLD,
  ...
  getEntityDeclarations(entityClientId: string): BeneficialOwnershipDeclaration[] {
    ...
    return declarations.sort((a, b) => b.declarationDate.getTime() - a.declarationDate.getTime());
  }
  ...
  async checkComplianceStatus(entityClientId: string): Promise<{
    ...
    if (declarations.length === 0) {
      return {
        ...
        nextActionRequired: 'Submit initial beneficial ownership declaration (Form BEN-1)',
      };
    }
    ...
    if (latestDeclaration.status === 'expired' || latestDeclaration.validUntil < now) {
    ...
    if (latestDeclaration.significantBeneficialOwners.length > 0 && !latestDeclaration.complianceDetails.formBen2Filed) {
      issues.push('Form BEN-2 not filed with MCA');
    }
    
    const isCompliant = issues.length === 0 && latestDeclaration.status === 'verified';
    
    return {
      isCompliant,
      hasValidDeclaration: latestDeclaration.validUntil >= now && latestDeclaration.status !== 'expired',
      lastDeclaration: latestDeclaration,
      issues,
      nextActionRequired: issues.length > 0 
        ? issues[0] 
        : 'No action required. Ensure annual review before expiry.',
    };
  }
  
  async markFormsFiled(declarationId: string, formType: 'BEN-1' | 'BEN-2', filedBy: string): Promise<{ success: boolean; message: string }> {
    const declaration = this.declarationsStore.get(declarationId);
    ...
    return { success: true, message: `${formType} filing recorded successfully` };
  }
  ...
  getSBOThresholds(): { shareThreshold: number; votingThreshold: number; uboThreshold: number } {
    return {
