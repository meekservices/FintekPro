...
export class KycExpiryMonitor {
  ...
  static getInstance(): KycExpiryMonitor {
    if (!this.instance) {
      this.instance = new KycExpiryMonitor();
    }
  ...
  start(): void {
    ...
    this.runCheck().catch(e => console.error("[KycExpiryMonitor] Initial check error:", e));
    this.intervalHandle = setInterval(() => {
      this.runCheck().catch(e => console.error("[KycExpiryMonitor] Check error:", e));
    }, CHECK_INTERVAL_MS);
  }
  ...
  async runCheck(): Promise<ExpiryAlert[]> {
    const now = new Date();
    const warnDate = new Date(now.getTime() + WARNING_DAYS * 24 * 60 * 60 * 1000);
    ...
    try {
      const expiredOrSoonAgents = await db.execute(sql`
        ...
        WHERE arn_expiry_date IS NOT NULL
          ...
          AND is_active = true
      `);
      const agentRows = (expiredOrSoonAgents as any).rows || expiredOrSoonAgents;
      for (const row of agentRows) {
        ...
        const alert: ExpiryAlert = {
          ...
          severity: daysUntil <= 0 ? 'EXPIRED' : daysUntil <= 7 ? 'CRITICAL' : 'WARNING',
        };
        alerts.push(alert);
    ...
    try {
      const expiredOrSoonPartners = await db.execute(sql`
        ...
        WHERE arn_expiry_date IS NOT NULL
          ...
          AND is_active = true
      `);
      const partnerRows = (expiredOrSoonPartners as any).rows || expiredOrSoonPartners;
    ...
    if (alerts.length > 0) {
      console.log(`[KycExpiryMonitor] Found ${alerts.length} expiry alert(s): ${alerts.filter(a => a.severity === 'EXPIRED').length} expired, ${alerts.filter(a => a.severity === 'CRITICAL').length} critical, ${alerts.filter(a => a.severity === 'WARNING').length} warning`);
    }
  ...
  private async handleAlert(alert: ExpiryAlert): Promise<void> {
    if (alert.severity === 'EXPIRED') {
      ...
      try {
        ...
        console.log(`[KycExpiryMonitor] SUSPENDED ${alert.entityType} ${alert.name} (${alert.entityId}) — ARN expired ${Math.abs(alert.daysUntilExpiry)} days ago`);
      } catch (e) {
        console.error(`[KycExpiryMonitor] Failed to suspend ${alert.entityType} ${alert.entityId}:`, e);
      }
    } else {
      console.warn(`[KycExpiryMonitor] ${alert.severity}: ${alert.entityType} "${alert.name}" ARN expires in ${alert.daysUntilExpiry} days (${alert.arnExpiryDate.toISOString().split('T')[0]})`);
    }
  }
