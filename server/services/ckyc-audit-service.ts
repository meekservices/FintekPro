...
class CkycAuditService {
	...
	async logEvent(entry: AuditLogEntry): Promise<CkycAuditLog> {
		...
		console.log(
			`[CKYC Audit] Logged ${entry.eventType} for case ${entry.caseId}`,
		);
		return created;
	}
	...
	async reconstructJourney(
		caseId: string,
	): Promise<CkycJourneyReconstruction | null> {
		// Get the case
		...
		const providerAttempts = ((caseRecord.fallbackAttempts as any[]) || []).map(
			(attempt: any) => ({
