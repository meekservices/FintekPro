...
export interface ReportSectionDef {
  ...
  order: number;
  dependencies: string[];
  checkFn: (proposalId: string) => Promise<{ met: boolean; reason?: string; missing?: string[] }>;
}
...
const SECTION_DEFINITIONS: ReportSectionDef[] = [
  ...
  },
  {
    ...
    checkFn: async (proposalId) => {
      ...
      return {
        met: hasHoldings,
        reason: hasHoldings ? undefined : 'No current portfolio holdings available',
  ...
  },
  {
    ...
    checkFn: async (proposalId) => {
      ...
      return {
        ...
        reason: !hasSells ? 'No SELL verdicts' : !hasGainsType ? 'Holding dates not available for tax calculation' : undefined,
        missing: [...(!hasSells ? ['sell_verdicts'] : []), ...(!hasGainsType ? ['holding_dates'] : [])]
      };
    }
  },
  {
    ...
    checkFn: async (proposalId) => {
      ...
      return {
        met: hasTaxData,
        reason: hasTaxData ? undefined : 'No exit load or capital gains data available',
...
export class ReportDependencyResolver {
  static async resolveAllSections(proposalId: string): Promise<ReportSectionStatus[]> {
    ...
    for (const section of SECTION_DEFINITIONS) {
      ...
      const status: ReportSectionStatus = {
        ...
        isEnabled: existing.length > 0 ? (existing[0].isEnabled ?? false) : checkResult.met,
        enabledByAgent: existing.length > 0 ? (existing[0].enabledByAgent ?? false) : false,
        enabledByAi: existing.length > 0 ? (existing[0].enabledByAi ?? false) : checkResult.met
      };
  ...
  static async autoSelectSections(proposalId: string): Promise<string[]> {
    const allSections = await this.resolveAllSections(proposalId);
    const enabledSections: string[] = [];

    for (const section of allSections) {
      if (section.dependencyMet) {
        await this.toggleSection(proposalId, section.code, true, false);
