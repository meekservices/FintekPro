...
export class AlpacaOptionsService {
   ...
   */
  generateOsiSymbol(underlying: string, expiry: string, type: 'C' | 'P', strike: number) {
  ...
  async getOptionChain(underlying: string) {
    try {
      const response = await alpacaClient.call(`/v1beta1/options/chain/${underlying.toUpperCase()}`, 'GET');
      return response;
    } catch (error: any) {
      logger.error(`[AlpacaOptionsService] Failed to fetch option chain for ${underlying}`, error.response?.data || error.message);
      throw error;
    }
  }
  ...
  async getOptionSnapshots(symbols: string[]) {
    try {
      const response = await alpacaClient.call(`/v1beta1/options/snapshots?symbols=${symbols.join(',')}`, 'GET');
      return response;
    } catch (error: any) {
      logger.error(`[AlpacaOptionsService] Failed to fetch option snapshots`, error.response?.data || error.message);
      throw error;
    }
  }
}
