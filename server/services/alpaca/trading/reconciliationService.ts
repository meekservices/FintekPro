...
export class AlpacaReconciliationService {
	...
	async reconcileAllUsers() {
		...
		logger.info(
			`[AlpacaReconciliation] Found ${activeUsers.length} active Alpaca accounts to reconcile`,
		);

		let totalSynced = 0;
		for (const user of activeUsers) {
			try {
				const syncedCount = await this.reconcileUser(
					user.id,
				...
				logger.error(
					`[AlpacaReconciliation] Failed to reconcile user ${user.id}:`,
					err,
				);
			}
		}
		...
		logger.info(
			`[AlpacaReconciliation] Completed. Total orders updated: ${totalSynced}`,
		);
		return { totalSynced };
	}
	...
	async reconcileUser(userId: string, alpacaAccountId: string) {
		...
		const remoteOrders =
			await alpacaTradingEngine.getOpenOrders(alpacaAccountId);
		...
		for (const local of localOrders) {
			...
			if (remote) {
				...
				if (remote.status !== local.status) {
					await db
						...
						.set({
							...
							filledQty: remote.filled_qty
								? remote.filled_qty.toString()
								: local.filledQty,
							filledAvgPrice: remote.filled_avg_price
								? remote.filled_avg_price.toString()
								: local.filledAvgPrice,
							updatedAt: new Date(),
						})
				...
				try {
					...
					const finishedOrder = await alpacaClient.call(
						`/trading/accounts/${alpacaAccountId}/orders/${local.providerOrderId}`,
					);
					...
					await db
						...
						.set({
							...
							filledQty: finishedOrder.filled_qty
								? finishedOrder.filled_qty.toString()
								: local.filledQty,
							filledAvgPrice: finishedOrder.filled_avg_price
								? finishedOrder.filled_avg_price.toString()
								: local.filledAvgPrice,
							updatedAt: new Date(),
						})
						.where(eq(alpacaOrders.id, local.id));
					...
					logger.warn(
						`[AlpacaReconciliation] Could not find order ${local.providerOrderId} on remote. Possible archival.`,
					);
				}
			}
		}
