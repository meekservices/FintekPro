...
export class ExecutionValidator {
   ...
   */
  async validateOrder(alpacaAccountId: string, symbol: string, qty: number, side: 'buy' | 'sell') {
    logger.info(`[ExecutionValidator] Validating order for ${alpacaAccountId}: ${side} ${qty} ${symbol}`);

    const account = await alpacaAccountService.checkAccountStatus(alpacaAccountId);

    if (account.status !== 'ACTIVE') {
      throw new Error(`Account is not active (Status: ${account.status}). Trading disabled.`);
    }
    ...
    if (!this.isUSMarketOpen()) {
      logger.warn('[ExecutionValidator] Market is closed. Order will be queued for next open if time_in_force allows, or rejected.');
    }
  ...
  isUSMarketOpen(): boolean {
    ...
    const options: Intl.DateTimeFormatOptions = { timeZone: 'America/New_York', hour: 'numeric', minute: 'numeric', hour12: false, weekday: 'long' };
