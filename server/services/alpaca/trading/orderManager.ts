...
export class OrderManager {
	...
	async placeOrder(
		userId: string,
		payload: Omit<OrderPayload, "client_order_id">,
	) {
		logger.info(
			`[OrderManager] User ${userId} requesting order: ${payload.side} ${payload.symbol}`,
		);
		...
		await executionValidator.validateOrder(
			alpacaAccountId,
			payload.symbol,
			qty,
			payload.side,
		...
		const estimatedValue = payload.notional
			? payload.notional
			: payload.qty
				? payload.qty * 100
				: 0; // Price should be fetched for real estimate
		const commission = await alpacaCommissionService.calculateCommission(
			accountType,
			assetClass,
			estimatedValue,
		);
		...
		try {
			...
			logger.info(
				`[OrderManager] Order successfully submitted for ${userId}. Alpaca Order ID: ${orderResult.id}`,
			);
			return orderResult;
		} catch (error: any) {
