...
export class AlpacaCryptoService {
   ...
   */
  async placeCryptoOrder(userId: string, symbol: string, qty: number | null, notional: number | null, side: 'buy' | 'sell') {
    logger.info(`[AlpacaCryptoService] User ${userId} requesting crypto order: ${side} ${symbol}`);
    ...
    try {
      const orderResult = await alpacaTradingEngine.dispatchOrder(alpacaAccountId, payload);
      ...
      logger.error(`[AlpacaCryptoService] Crypto order failed for user ${userId}`, error.response?.data || error.message);
      
      await db.update(alpacaOrders)
        .set({ status: 'rejected' })
        .where(eq(alpacaOrders.clientOrderId, clientOrderId));

      throw new Error(`Crypto Order Failed: ${error.response?.data?.message || error.message}`);
    }
  }
}
