...
export class AlpacaPortfolioSync {
	...
	async syncUserPortfolio(userId: string) {
		logger.info(
			`[AlpacaPortfolioSync] Initiating portfolio sync for user ${userId}`,
		);
		...
		try {
			...
			if (!user.pan) {
				logger.warn(
					`[AlpacaPortfolioSync] User ${userId} has no PAN, skipping IRIS sync part`,
				);
			}
			...
			logger.error(`[AlpacaPortfolioSync] Sync failed for user ${userId}`, {
				error: error.message,
			});
			throw error;
		}
