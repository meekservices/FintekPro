...
export class CandleService {
	...
	async getCandles(
		...
		logger.info(
			`[CandleService] Fetching ${timeframe} candles for ${symbol} from ${start}`,
		);

		try {
			const bars = await alpacaMarketDataService.getHistoricalBars(
				symbol,
				timeframe,
				start,
				end,
			...
			logger.error(
				`[CandleService] Failed to fetch candles for ${symbol}`,
				error,
			);
