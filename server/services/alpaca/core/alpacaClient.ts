...
export class AlpacaClient {
	...
	async call<T = any>(
		...
		if (this.isCircuitOpen()) {
			throw new Error(
				"Alpaca API is currently unavailable (Circuit Breaker Open)",
			);
		}
		...
		try {
			...
			if (err.response?.status === 429 && retryCount < 3) {
				...
				const backoff = resetTime
					? Number.parseInt(resetTime) * 1000 - Date.now()
					: 2 ** retryCount * 1000;
				logger.warn(
					`[AlpacaClient] Rate limited (429), retrying in ${backoff}ms...`,
				);
			...
			if (err.response?.status >= 500 || err.code === "ECONNABORTED") {
				...
				if (retryCount < 2) {
					...
					logger.warn(
						`[AlpacaClient] API error 5xx, retrying in ${backoff}ms...`,
					);
	...
	async createAchRelationship(accountId: string, achData: any) {
		return this.call(
			`/accounts/${accountId}/ach_relationships`,
			"POST",
			achData,
		);
	}
