...
export class AlpacaAuthManager {
	...
	constructor() {
		this.keyId =
			...
			"";
		this.secretKey =
			process.env.ALPACA_BROKER_SECRET_KEY ||
			process.env.ALPACA_KEY_SECRET ||
	...
	getCredentials() {
		if (!this.isConfigured) {
			logger.warn(
				"[AlpacaAuthManager] Credentials missing. Check GCP Secret Manager or environment variables.",
			);
		}
		return {
			keyId: this.keyId,
	...
	async getAccessToken(): Promise<string> {
		...
		try {
			logger.info(
				"[AlpacaAuthManager] Requesting new access token (Client Credentials flow)",
			);
			...
			logger.info(
				`[AlpacaAuthManager] Token acquired. Expires in ${expires_in}s`,
			);
			return this.accessToken!;
		} catch (error: any) {
			logger.error(
				"[AlpacaAuthManager] Failed to fetch access token",
				error.response?.data || error.message,
			);
			throw new Error(
				`Alpaca Auth Failed: ${error.response?.data?.message || error.message}`,
			);
		}
	}
