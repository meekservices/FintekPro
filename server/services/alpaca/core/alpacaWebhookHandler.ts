 ...
 */
export function verifyAlpacaWebhookSignature(
	rawBody: Buffer,
	signature: string,
): boolean {
	...
	if (!secret) {
		logger.warn(
			"[AlpacaWebhook] ALPACA_WEBHOOK_SECRET not set — skipping HMAC verification (INSECURE)",
		);
	...
	try {
		...
		logger.error("[AlpacaWebhook] HMAC verification error", {
			error: err.message,
		});
		return false;
	}
...
export class AlpacaWebhookHandler {
	...
	async handleEvent(
		...
		switch (eventType) {
			...
			case "account_updates":
				...
				break;
			case "journal_status":
				await this.handleJournalUpdate(
					eventData.data as Record<string, unknown>,
				);
				break;
			default:
			...
			if (existingOrders.length > 0) {
				await db
					...
					.set({
						...
						filledQty:
							(order?.filled_qty as string) ?? existingOrders[0].filledQty,
						filledAvgPrice:
							(order?.filled_avg_price as string) ??
							existingOrders[0].filledAvgPrice,
						updatedAt: new Date(),
					})
					.where(eq(alpacaOrders.providerOrderId, orderId));
	...
	private async handleAccountUpdate(
		...
		try {
			...
			if (!user) {
				logger.warn(
					"[AlpacaWebhook] No FintekPro user found for Alpaca account",
					{
			...
			try {
				...
				logger.warn("[AlpacaWebhook] Referral code generation failed", {
              ...
              <div style="font-family:Inter,Arial,sans-serif;max-width:600px;margin:0 auto;background:#0f172a;color:#e2e8f0;border-radius:12px;overflow:hidden;">
                ...
                <div style="padding:32px;">
                  ...
                  <p style="color:#475569;font-size:12px;margin-top:24px;padding-top:16px;border-top:1px solid #1e293b;">
					...
					logger.info("[AlpacaWebhook] Welcome email sent", {
						event: "ALPACA_WELCOME_EMAIL_SENT",
						user_id: user.id,
						status: "success",
						latency_ms: 0,
					});
				} catch (emailErr: any) {
					logger.error("[AlpacaWebhook] Welcome email failed", {
			...
			if (user.mobile) {
				try {
					...
					if (result.success) {
						logger.info("[AlpacaWebhook] Welcome WhatsApp sent", {
							event: "ALPACA_WELCOME_WHATSAPP_SENT",
							user_id: user.id,
					...
					logger.error("[AlpacaWebhook] Welcome WhatsApp failed", {
						error: waErr.message,
						user_id: user.id,
					});
				}
	 ...
	 */
	private async handleJournalUpdate(
		data: Record<string, unknown>,
	): Promise<void> {
		...
		const isCompleted = ["executed", "EXECUTED", "correct", "CORRECT"].includes(
			journalStatus ?? "",
		);
		...
		try {
			...
			if (!user) {
				logger.warn(
					"[AlpacaWebhook] No FintekPro user found for funded Alpaca account",
					{
              ...
              <div style="font-family:Inter,Arial,sans-serif;max-width:600px;margin:0 auto;background:#0f172a;color:#e2e8f0;border-radius:12px;overflow:hidden;">
                ...
                <div style="padding:32px;">
                  ...
                  <p style="color:#475569;font-size:12px;margin-top:24px;padding-top:16px;border-top:1px solid #1e293b;">
					...
					logger.info("[AlpacaWebhook] First-deposit email sent", {
						event: "ALPACA_DEPOSIT_EMAIL_SENT",
						user_id: user.id,
						status: "success",
						latency_ms: 0,
					});
				} catch (emailErr: any) {
					logger.error("[AlpacaWebhook] First-deposit email failed", {
						error: emailErr.message,
						user_id: user.id,
					});
				}
			...
			if (user.mobile) {
				try {
					...
					const result = await whatsappDispatcher.send({
						mobile: user.mobile,
						message: waMsg,
						category: "FIRST_DEPOSIT",
					});
					if (result.success) {
						logger.info("[AlpacaWebhook] First-deposit WhatsApp sent", {
							event: "ALPACA_DEPOSIT_WHATSAPP_SENT",
							user_id: user.id,
							provider: result.provider,
							status: "success",
							latency_ms: 0,
					...
					logger.error("[AlpacaWebhook] First-deposit WhatsApp failed", {
						error: waErr.message,
						user_id: user.id,
					});
				}
