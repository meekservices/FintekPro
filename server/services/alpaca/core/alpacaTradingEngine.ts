...
export class AlpacaTradingEngine {
  ...
  async dispatchOrder(alpacaAccountId: string, payload: OrderPayload) {
    try {
      ...
      logger.info(`[AlpacaTradingEngine] Dispatching order for ${alpacaAccountId}: ${payload.side} ${payload.symbol}`);
      
      const orderResult = await alpacaClient.placeOrder(alpacaAccountId, payload);
      return orderResult;
    } catch (error: any) {
      logger.error(`[AlpacaTradingEngine] Order failed for ${alpacaAccountId}`, error.response?.data || error.message);
  ...
  async cancelOrder(alpacaAccountId: string, orderId: string) {
    return alpacaClient.call(`/trading/accounts/${alpacaAccountId}/orders/${orderId}`, 'DELETE');
  }
}
