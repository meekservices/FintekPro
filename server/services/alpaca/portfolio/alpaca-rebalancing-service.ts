...
export class AlpacaRebalancingService {
	...
	async createPortfolio(
		name: string,
		assets: { symbol: string; percent: number }[],
	) {
		const payload = {
			name,
			assets: assets.map((a) => ({
		...
		try {
			...
			logger.error(
				`[AlpacaRebalancingService] Failed to create portfolio`,
	...
	async subscribeAccount(alpacaAccountId: string, portfolioId: string) {
		...
		try {
			const response = await alpacaClient.call(
				`/rebalancing/accounts/${alpacaAccountId}/subscriptions`,
				"POST",
				payload,
			);
			logger.info(
				`[AlpacaRebalancingService] Account ${alpacaAccountId} subscribed to portfolio ${portfolioId}`,
			);
			return response;
		} catch (error: any) {
			logger.error(
				`[AlpacaRebalancingService] Subscription failed`,
				error.response?.data || error.message,
			);
			throw error;
		}
	...
	async triggerRebalance(alpacaAccountId: string) {
		try {
			const response = await alpacaClient.call(
				`/rebalancing/accounts/${alpacaAccountId}/runs`,
				"POST",
				{ type: "full_rebalance" },
			);
			logger.info(
				`[AlpacaRebalancingService] Rebalance triggered for ${alpacaAccountId}`,
			);
			return response;
		} catch (error: any) {
			logger.error(
				`[AlpacaRebalancingService] Rebalance trigger failed`,
				error.response?.data || error.message,
			);
			throw error;
		}
