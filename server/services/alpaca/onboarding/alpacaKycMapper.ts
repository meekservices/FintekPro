...
export class AlpacaKycMapper {
	...
	mapToAlpacaSchema(user: any, profile: any, ipAddress: string = "127.0.0.1") {
		...
		const taxId = cleanAlphanumeric(
			profile.panNumber || profile.passportNumber || "",
		);
		if (!taxId) {
		...
		const payload: any = {
			...
			identity: {
				...
				tax_id: taxId,
				tax_id_type:
					profile.countryOfResidence === "India" ? "IND_PAN" : "USA_SSN",
				country_of_citizenship:
					profile.countryOfCitizenship === "India" ? "IND" : "USA",
				country_of_birth:
					profile.countryOfBirth === "India"
						? "IND"
						: profile.countryOfBirth || "IND",
				country_of_tax_residence:
					profile.countryOfResidence === "India" ? "IND" : "USA",
				...
				liquid_net_worth: profile.liquidNetWorth
					? profile.liquidNetWorth.toString()
					: undefined,
			},
			disclosures: {
				...
				investment_objective:
					profile.investmentObjective || "capital_appreciation",
		...
		if (profile.countryOfResidence !== "USA") {
			logger.info(
				`[AlpacaKycMapper] Including W-8BEN declaration for non-US user: ${user.id}`,
			);
