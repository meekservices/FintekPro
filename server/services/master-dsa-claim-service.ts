import { db } from "../db";
import {
  masterDsaClaims, masterDsaAttachments, masterDsaPayments,
  payoutClaims, leadRegistry, bankerConfirmationEmails, leadAuditLogs,
...
const DSA_CLAIM_STATUS_ORDER = ['DRAFT', 'SUBMITTED', 'ACKNOWLEDGED', 'PAID'] as const;

export class MasterDsaClaimService {
  private static instance: MasterDsaClaimService;

  static getInstance(): MasterDsaClaimService {
    if (!this.instance) this.instance = new MasterDsaClaimService();
  ...
  async createClaimFromPayout(
    ...
    const [payoutClaim] = await db.select().from(payoutClaims)
    ...
    if (payoutClaim.status !== 'CONFIRMED_BY_FINANCIER' && payoutClaim.status !== 'APPROVED' && payoutClaim.status !== 'ON_HOLD_PDD') {
      return { success: false, error: "Payout claim must be confirmed/approved before creating Master DSA claim" };
    }

    const [existingDsaClaim] = await db.select().from(masterDsaClaims)
      .where(eq(masterDsaClaims.payoutClaimId, payoutClaimId));

    if (existingDsaClaim) {
      return { success: false, error: "Master DSA claim already exists for this payout claim" };
    }

    const [lead] = await db.select().from(leadRegistry)
    ...
    const [claim] = await db.insert(masterDsaClaims).values({
      payoutClaimId,
    ...
    await db.insert(leadAuditLogs).values({
      ...
      details: { dsaClaimId: claim.dsaClaimId, claimedAmount: data.claimedAmount, masterDsaName: data.masterDsaName },
    });
...
Disbursement Amount : ₹${parseFloat(data.disbursementAmount).toLocaleString('en-IN')}
Disbursement Date   : ${data.disbursementDate}

COMMISSION CLAIM
────────────────────────────────────
Commission Amount   : ₹${parseFloat(data.claimedAmount).toLocaleString('en-IN')}
...
support@fintekpro.com`;
  ...
  async addAttachment(dsaClaimId: string, data: {
    fileName: string;
    fileType: string;
    fileSize: number;
    fileHash: string;
    ...
    adminId: string;
  }): Promise<{ success: boolean; attachment?: MasterDsaAttachment; error?: string }> {
    const [claim] = await db.select().from(masterDsaClaims)
    ...
    if (claim.status !== 'DRAFT') {
      return { success: false, error: "Attachments can only be added in DRAFT status" };
    }

    if (!ALLOWED_ATTACHMENT_TYPES.includes(data.fileType.toLowerCase())) {
      return { success: false, error: `Invalid file type. Allowed: ${ALLOWED_ATTACHMENT_TYPES.join(', ')}` };
    }
    ...
    const [attachment] = await db.insert(masterDsaAttachments).values({
      dsaClaimId,
    ...
    await db.insert(leadAuditLogs).values({
      ...
      details: { dsaClaimId, fileName: data.fileName, attachmentType: data.attachmentType },
    });

    return { success: true, attachment };
  }

  async submitClaim(dsaClaimId: string, adminId: string): Promise<{ success: boolean; claim?: MasterDsaClaim; error?: string }> {
    const [claim] = await db.select().from(masterDsaClaims)
      .where(eq(masterDsaClaims.dsaClaimId, dsaClaimId));
    ...
    if (claim.status !== 'DRAFT') {
      return { success: false, error: "Claim must be in DRAFT status to submit" };
    }

    const attachments = await db.select().from(masterDsaAttachments)
      .where(eq(masterDsaAttachments.dsaClaimId, dsaClaimId));

    const hasConfirmationEmail = attachments.some(a => a.attachmentType === 'CONFIRMATION_EMAIL');
    if (!hasConfirmationEmail) {
      return { success: false, error: "Banker/financier confirmation email attachment is mandatory before submission" };
    }
    ...
    const [updated] = await db.update(masterDsaClaims).set({
      ...
      updatedAt: new Date(),
    }).where(eq(masterDsaClaims.dsaClaimId, dsaClaimId)).returning();
  ...
  async sendClaimEmail(dsaClaimId: string, adminId: string): Promise<{ success: boolean; claim?: MasterDsaClaim; error?: string }> {
    const [claim] = await db.select().from(masterDsaClaims)
      .where(eq(masterDsaClaims.dsaClaimId, dsaClaimId));
    ...
    if (claim.status !== 'SUBMITTED') {
      return { success: false, error: "Claim must be SUBMITTED before sending email" };
    }
  ...
  async updateStatus(
    ...
    switch (newStatus) {
      case 'ACKNOWLEDGED': updateData.acknowledgedAt = new Date(); break;
      case 'PAID': updateData.paidAt = new Date(); break;
      case 'DISPUTED': updateData.disputedAt = new Date(); break;
      case 'REJECTED':
    ...
    const [updated] = await db.update(masterDsaClaims).set(updateData)
      .where(eq(masterDsaClaims.dsaClaimId, dsaClaimId)).returning();
  ...
  async recordPayment(dsaClaimId: string, data: {
    amount: string;
    paymentDate: string;
    ...
    notes?: string;
    adminId: string;
  }): Promise<{ success: boolean; payment?: MasterDsaPayment; claim?: MasterDsaClaim; discrepancy?: boolean; error?: string }> {
    const [claim] = await db.select().from(masterDsaClaims)
      .where(eq(masterDsaClaims.dsaClaimId, dsaClaimId));
    ...
    if (!['ACKNOWLEDGED', 'PARTIALLY_PAID'].includes(claim.status as string)) {
      return { success: false, error: "Payments can only be recorded for ACKNOWLEDGED or PARTIALLY_PAID claims" };
    }

    const [payment] = await db.insert(masterDsaPayments).values({
      dsaClaimId,
      amount: data.amount,
    ...
    const allPayments = await db.select().from(masterDsaPayments)
    ...
    const [updatedClaim] = await db.update(masterDsaClaims).set(claimUpdate)
      .where(eq(masterDsaClaims.dsaClaimId, dsaClaimId)).returning();
    ...
    await db.insert(leadAuditLogs).values({
      ...
      details: {
        dsaClaimId, paymentAmount: data.amount, totalPaid: totalPaid.toFixed(2),
        outstanding: outstanding.toFixed(2), discrepancy, referenceNumber: data.referenceNumber,
      },
    });
  ...
  async getClaimById(dsaClaimId: string): Promise<{
    ...
    const payments = await db.select().from(masterDsaPayments)
      .where(eq(masterDsaPayments.dsaClaimId, dsaClaimId))
  ...
  async getClaimByPayoutId(payoutClaimId: string): Promise<MasterDsaClaim | null> {
    const [claim] = await db.select().from(masterDsaClaims)
      .where(eq(masterDsaClaims.payoutClaimId, payoutClaimId));
    return claim || null;
  }
  ...
  async getAllClaims(status?: string): Promise<MasterDsaClaim[]> {
    if (status) {
      return db.select().from(masterDsaClaims)
