...
export async function cacheMarketData(
	...
	const [result] = await db
		...
		.onConflictDoUpdate({
			target: [
				marketDataSnapshots.assetType,
				marketDataSnapshots.assetId,
				marketDataSnapshots.snapshotDate,
			],
			set: {
...
export async function cacheFundamentals(
	...
	const [result] = await db
		...
		.onConflictDoUpdate({
			target: [
				productFundamentalsCache.productType,
				productFundamentalsCache.productId,
			],
			set: {
...
export async function invalidateRationale(
	...
	if (rationaleType)
		conditions.push(eq(aiRationaleCache.rationaleType, rationaleType));
	if (productType)
		conditions.push(eq(aiRationaleCache.productType, productType));
...
export async function cachePortfolioMetrics(
	...
	const [result] = await db
		...
		.onConflictDoUpdate({
			target: [
				portfolioMetricsDaily.userId,
				portfolioMetricsDaily.portfolioId,
				portfolioMetricsDaily.metricsDate,
			],
			set: {
...
export async function cacheProposal(
	data: Omit<
		InsertProposalMaterialization,
		"expiresAt" | "priceValidUntil" | "inputHash"
	>,
	inputParams: Record<string, unknown>,
...
export async function getCachedFinancialMetrics(
	...
	return {
		...
		altmanZScore: cached.altmanZScore
			? Number.parseFloat(cached.altmanZScore)
			: undefined,
		pegRatio: cached.pegRatio ? Number.parseFloat(cached.pegRatio) : undefined,
		evToEbitda: cached.evToEbitda
			? Number.parseFloat(cached.evToEbitda)
			: undefined,
		roic: cached.roic ? Number.parseFloat(cached.roic) : undefined,
		earningsQuality: cached.earningsQuality
			? Number.parseFloat(cached.earningsQuality)
			: undefined,
		debtToEquity: cached.debtToEquity
			? Number.parseFloat(cached.debtToEquity)
			: undefined,
		currentRatio: cached.currentRatio
			? Number.parseFloat(cached.currentRatio)
			: undefined,
		roe: cached.roe ? Number.parseFloat(cached.roe) : undefined,
		roce: cached.roce ? Number.parseFloat(cached.roce) : undefined,
...
export async function getCachedFinancialMetricsBatch(
	...
	const conditions = sanitizedSymbols.map((s) =>
		eq(stockFinancialMetrics.symbol, s),
	);
		...
		if (!map.has(r.symbol)) {
			map.set(r.symbol, {
				...
				altmanZScore: r.altmanZScore
					? Number.parseFloat(r.altmanZScore)
					: undefined,
				pegRatio: r.pegRatio ? Number.parseFloat(r.pegRatio) : undefined,
				evToEbitda: r.evToEbitda ? Number.parseFloat(r.evToEbitda) : undefined,
				roic: r.roic ? Number.parseFloat(r.roic) : undefined,
				earningsQuality: r.earningsQuality
					? Number.parseFloat(r.earningsQuality)
					: undefined,
				debtToEquity: r.debtToEquity
					? Number.parseFloat(r.debtToEquity)
					: undefined,
...
export function calculateEnhancedQualityScore(
	metrics: CachedFinancialMetrics,
): number {
	...
	if (metrics.altmanZScore !== undefined) {
		if (metrics.altmanZScore > 2.99)
			score += 15; // Safe zone
		else if (metrics.altmanZScore >= 1.81)
			score += 5; // Grey zone
...
export async function cleanupExpiredCache(): Promise<{
	...
	const [md] = await db
		.delete(marketDataSnapshots)
		.where(lte(marketDataSnapshots.expiresAt, cutoffDate))
		.returning();
	const [pf] = await db
		.delete(productFundamentalsCache)
		.where(lte(productFundamentalsCache.expiresAt, cutoffDate))
		.returning();
	const [ar] = await db
		.delete(aiRationaleCache)
		.where(lte(aiRationaleCache.expiresAt, cutoffDate))
		.returning();
	const [pm] = await db
		.delete(portfolioMetricsDaily)
		.where(
			lte(
				portfolioMetricsDaily.metricsDate,
		...
		.returning();
	const [pp] = await db
		.delete(proposalMaterializations)
		.where(lte(proposalMaterializations.expiresAt, cutoffDate))
		.returning();
