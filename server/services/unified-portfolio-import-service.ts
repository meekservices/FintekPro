...
import {
	holdingNormalizationService,
	type IsinEnrichmentResult,
} from "./holding-normalization-service";
...
import {
	WealthyImportService,
	type WealthyHolding,
} from "./wealthy-import-service";
import { db } from "../db";
import {
...
class UnifiedPortfolioImportService {
	...
	async importFromPDF(
		...
		try {
			...
			const isCAS =
				options?.detectCAS !== false && this.detectCASStatement(text);
			...
			const parserResult = await unifiedPDFParser.parseDocument(buffer, {
				fileName,
			});

			if (!parserResult.success || parserResult.confidenceScore < 0.5) {
				console.warn(
					`[Parser] Unified parser result insufficient (success=${parserResult.success}, confidence=${parserResult.confidenceScore})`,
				);
				// Fall through to legacy parsing
			} else {
				// Convert holdings to unified format with lot information
				result.holdings = parserResult.holdings.map((h) => {
					const matchingLots = (parserResult.holdingLots || []).filter(
						(lot) =>
				...
				result.holdings = await this.enrichHoldingsWithIsin(
					result.holdings,
					parserResult.holdings,
				);

				(result as any).holdingLots = parserResult.holdingLots;

				result.summary = holdingNormalizationService.computeSummary(
					result.holdings,
				);
				result.confidenceScore = parserResult.confidenceScore;
				result.success = parserResult.success;
				result.parsingStatus = parserResult.success
					? "completed"
					: "needs_review";
			...
			result.parsingStatus = portfolioResult.success
				? "completed"
				: "needs_review";
	...
	async importFromCASText(
		...
		try {
			...
			const holdingsWithLots = result.holdings.filter(
				(h) => h.lots && h.lots.length > 0,
			);
			result.lotCounts = {
				withLots: holdingsWithLots.length,
				withMultipleLots: result.holdings.filter(
					(h) => h.lots && h.lots.length > 1,
				).length,
	...
	async importFromWealthyURL(url: string): Promise<UnifiedImportResult> {
		...
		try {
			const wealthyResult =
				await this.wealthyService.fetchAndParsePortfolio(url);
	...
	async importManualHoldings(
		holdings: Partial<UnifiedHolding>[],
	): Promise<UnifiedImportResult> {
	...
	private convertImportedHoldings(
	...
	private convertCASHoldings(holdings: CASHolding[]): UnifiedHolding[] {
		return holdings.map((h) => {
			const unified: UnifiedHolding = {
				...
				lotSummary:
					h.lotSummary ||
					`${h.lots?.length || 0} lot${(h.lots?.length || 0) !== 1 ? "s" : ""}`,
				lots:
					h.lots?.map((lot) => ({
						transactionDate:
	...
	private convertWealthyHoldings(holdings: WealthyHolding[]): UnifiedHolding[] {
		return holdings.map((h) => {
			const unified: UnifiedHolding = {
				name: h.fundName,
				folioNumber: h.folio,
				assetType: holdingNormalizationService.normalizeAssetTypeFromMFCategory(
					h.category,
				),
				quantity: 0,
				...
				unrealizedGainPercent:
					h.invested > 0 ? (h.returns / h.invested) * 100 : 0,
				broker: "WEALTHY_IN",
	...
	private async enrichHoldingsWithIsin(
		...
		const enrichmentMap =
			await holdingNormalizationService.enrichByIsins(isins);
		...
		const enrichedHoldings = holdings.map((holding, index) => {
			...
			if (enrichment?.found) {
				...
				const holdingPeriodDays = this.calculateHoldingPeriodDays(
				...
				return {
					...
					assetType: enrichment.category
						? holdingNormalizationService.normalizeAssetTypeFromMFCategory(
								enrichment.category,
							)
						: holding.assetType,
					// Fund metadata
		...
		console.log(
			`[Import] Enriched ${enrichedCount}/${holdings.length} holdings with ISIN metadata`,
		);
	 ...
	 */
	private async enrichCASHoldingsWithIsin(
		...
		console.log(
			`[CAS Import] Enriched ${enrichedCount}/${holdings.length} holdings with ISIN metadata from database`,
		);
		return enrichedHoldings;
	}
	...
	async importFromCSV(
		...
		try {
			...
			if (!columnMap.hasMinimumColumns) {
				result.errors.push(
					"CSV must have at least name/symbol and quantity/units columns",
				);
				result.parsingStatus = "failed";
				return result;
			}
			...
			result.parsingStatus =
				parseErrors.length > 0 ? "needs_review" : "completed";
			...
			console.log(
				`[CSV Import] Parsed ${result.holdings.length} holdings from ${fileName || "CSV"} in ${Date.now() - startTime}ms`,
			);
			return result;
		} catch (error: any) {
	...
	private detectCSVColumns(headers: string[]): CSVColumnMap {
		const findColumn = (patterns: string[]): number => {
			for (const pattern of patterns) {
				const idx = headers.findIndex(
					(h) => h.includes(pattern) || h === pattern,
				);
				if (idx !== -1) return idx;
			}
			return -1;
		...
		const currentValueIdx = findColumn([
			"current value",
			"value",
			"market value",
			"amount",
		]);
		const investedValueIdx = findColumn([
			"invested",
			"investment",
			"cost value",
			"purchase value",
		]);
		const typeIdx = findColumn([
			"type",
			"asset",
			"category",
			"asset type",
		...
		return {
			...
			dateIdx,
			hasMinimumColumns:
				(nameIdx !== -1 || symbolIdx !== -1 || isinIdx !== -1) &&
	...
	private parseCSVRowToHolding(
		...
		const name =
			getValue(columnMap.nameIdx) || getValue(columnMap.symbolIdx) || "";
		...
		const investedValue =
			getNumber(columnMap.investedValueIdx) || quantity * avgPrice;
	 ...
	 */
	private async enrichCSVHoldings(
		holdings: UnifiedHolding[],
	): Promise<UnifiedHolding[]> {
		...
		for (const holding of holdings) {
			...
			if (!holding.isin && (holding.symbol || holding.name)) {
				try {
					...
					const enrichment =
						await holdingNormalizationService.enrichWithISIN(searchTerm);
					if (enrichment.isin) {
			...
			if (enriched.investedValue && enriched.currentValue) {
				enriched.unrealizedGain =
					enriched.currentValue - enriched.investedValue;
				enriched.unrealizedGainPercent =
					enriched.investedValue > 0
	...
	async importFromExcel(
		...
		try {
			...
			if (!columnMap.hasMinimumColumns) {
				result.errors.push(
					"Excel must have at least name/symbol and quantity/units columns",
				);
				result.parsingStatus = "failed";
				return result;
			}
			...
			console.log(
				`[Excel Import] Parsed ${result.holdings.length} holdings from ${fileName || "Excel"} in ${Date.now() - startTime}ms`,
			);
			return result;
		} catch (error: any) {
	...
	async notifyLinkedAgents(
		...
		try {
			...
			console.log(
				`[Portfolio Notification] Notified ${activeRelationships.length} agent(s) about ${clientName}'s portfolio import (${holdingsCount} holdings)`,
			);
		} catch (error) {
...
export const unifiedPortfolioImportService =
	new UnifiedPortfolioImportService();
