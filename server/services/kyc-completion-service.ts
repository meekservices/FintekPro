...
export async function transferVerifiedKYCData(
	...
	if (session.panDob && !currentUser.dateOfBirth) {
		const dobDate =
			typeof session.panDob === "string"
				? new Date(session.panDob)
				: session.panDob;
	...
	if (userProfile) {
		...
		if (
			!(userProfile as any).kycTier ||
			(userProfile as any).kycTier === "basic"
		) {
	...
	try {
		...
		console.error(
			"[KYC Completion] Failed to transfer prospect portfolio:",
			error,
		);
...
async function transferProspectPortfolioToUser(userId: string): Promise<void> {
	...
	if (existingUserPortfolio.length > 0) {
		...
		console.log(
			"[KYC Completion] Marked existing user portfolio as verified:",
			userId,
		);
	...
	if (!matchingProspect) {
		console.log(
			"[KYC Completion] No matching prospect found for user:",
			userId,
	...
	if (prospectPortfolio.length === 0) {
		console.log(
			"[KYC Completion] No prospect portfolio found to transfer for prospect:",
			matchingProspect.id,
		);
		return;
	}
	...
	console.log(
		"[KYC Completion] Successfully transferred and verified portfolio for user:",
		userId,
	);
 ...
 */
function triggerPortfolioRefresh(
	userId: string,
	...
	setImmediate(async () => {
		try {
			...
			if (!panNumber) {
				console.log(
					"[Portfolio Refresh] No PAN available for user, skipping auto-fetch:",
					userId,
				);
				await db
			...
			console.log(
				"[Portfolio Refresh] Starting background portfolio fetch for user:",
				userId,
			);
			...
			if (!userName || !user.dateOfBirth) {
				console.log(
					"[Portfolio Refresh] Missing required user data (name or DOB), skipping CAS fetch:",
					userId,
				);
			...
			console.log(
				"[Portfolio Refresh] Fetching MF + Demat data in parallel...",
			);
			...
			console.log(
				`[Portfolio Refresh] Fetched: MF=${mfHoldingsCount} holdings/${mfTxCount} transactions, Demat=${dematHoldingsCount} holdings/${dematTxCount} transactions`,
			);
			...
			await db.transaction(async (tx) => {
				...
				if (!userPortfolio) {
					...
					console.log(
						"[Portfolio Refresh] Created new portfolio for user:",
						userId,
					);
				} else {
				...
				for (const holding of casResult.holdings) {
					...
					if (insertedKeys.has(dedupKey)) {
						console.log(
							"[Portfolio Refresh] Skipping duplicate holding:",
							dedupKey,
						);
						continue;
					}
				...
				console.log(
					"[Portfolio Refresh] MF holdings sync complete - refreshed",
					insertedKeys.size,
					"MF holdings",
				);

				// Sync Demat holdings if available
				const hasDematHoldings =
					dematSuccess && dematResult?.holdings?.length > 0;
				...
				if (hasDematHoldings) {
					...
					console.log(
						"[Portfolio Refresh] Demat holdings sync complete - refreshed",
						dematInsertedCount,
						"demat holdings",
					);
				}
				...
				const hasMfTransactions =
					mfTransactionResult?.success &&
					mfTransactionResult?.transactions &&
					mfTransactionResult.transactions.length > 0;

				if (hasMfTransactions) {
					console.log(
						"[Portfolio Refresh] Syncing",
						mfTransactionResult.transactions.length,
						"MF transactions",
					);
					...
					for (const tx_record of mfTransactionResult.transactions) {
						...
						if (
							tx_record.transactionType === "purchase" ||
							tx_record.transactionType === "sip"
						) {
							purchaseTotal += amount;
						} else if (tx_record.transactionType === "redemption") {
							redemptionTotal += amount;
						} else if (
							tx_record.transactionType === "switch_in" ||
							tx_record.transactionType === "switch_out"
						) {
							switchTotal += amount;
						...
						await tx.insert(schema.transactionRecords).values({
							reportId: mfReportId,
							userId,
					...
					console.log(
						"[Portfolio Refresh] MF transaction sync complete - synced",
						mfTransactionResult.transactions.length,
						"transactions",
					);
				}
				...
				const hasDematTransactions =
					dematTransactionResult?.success &&
				...
				if (hasDematTransactions) {
					console.log(
						"[Portfolio Refresh] Syncing",
						dematTransactionResult.transactions.length,
						"demat transactions",
					);
					...
					const existingDematReport =
						await tx.query.transactionReports.findFirst({
							where: and(
								eq(schema.transactionReports.userId, userId),
								eq(schema.transactionReports.financialYear, financialYear),
								eq(
									schema.transactionReports.source,
									dematTransactionResult.source || "nsdl",
								),
					...
					for (const tx_record of dematTransactionResult.transactions) {
						await tx.insert(schema.transactionRecords).values({
							...
							netAmount:
								tx_record.netAmount?.toString() || tx_record.amount.toString(),
							createdAt: new Date(),
						} as any);
					}

					console.log(
						"[Portfolio Refresh] Demat transaction sync complete - synced",
						dematTransactionResult.transactions.length,
						"transactions",
					);
				}
				...
				const totalTransactions =
					(hasMfTransactions ? mfTransactionResult.transactions.length : 0) +
					(hasDematTransactions
						? dematTransactionResult.transactions.length
						: 0);
				console.log(
					"[Portfolio Refresh] Unified sync complete - refreshed",
					totalHoldings,
					"holdings and",
					totalTransactions,
			...
			console.error(
				"[Portfolio Refresh] Background fetch failed for user:",
				userId,
				error,
			);
			...
			try {
				...
				console.error(
					"[Portfolio Refresh] Failed to update error status:",
					statusError,
				);
			}
		}
