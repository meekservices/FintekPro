...
async function fetchDeal(
	...
	if (dealType === "aif") {
		...
		return {
			...
			name: row.name,
			dealType: "aif",
			minInvestment: Number.parseFloat(String(row.minInvestment || "10000000")),
			category: row.category,
			riskScore: row.riskScore,
			return1Y: Number.parseFloat(String(row.return1Y || "0")) || null,
	...
	return {
		...
		name: row.name,
		dealType: "pms",
		minInvestment: Number.parseFloat(String(row.minInvestment || "5000000")),
...
function computeMatchScore(
	...
	const surplus =
		prospect.investableSurplus || prospect.estimatedNetworth * 0.15;
	...
	if (surplusCover >= 3) {
		wealthPts = 40;
		reasons.push("3× surplus cover — highly liquid");
	} else if (surplusCover >= 1.5) {
		wealthPts = 32;
		reasons.push("1.5× surplus cover — comfortable");
	} else if (surplusCover >= 1.0) {
		wealthPts = 24;
		reasons.push("Meets minimum ticket");
	} else if (surplusCover >= 0.7) {
		wealthPts = 14;
		reasons.push("Near minimum — possible stretch");
	}
	score += wealthPts;
	...
	score += qPts;
	if (prospect.leadQuality === "hot")
		reasons.push("Hot lead — high engagement");
	else if (prospect.leadQuality === "warm")
		reasons.push("Warm lead — moderate engagement");

	// Composite score (25 pts normalised from 0–100)
	const compPts = Math.round((prospect.compositeScore / 100) * 25);
	score += compPts;
	if (prospect.compositeScore >= 70)
		reasons.push(
			`High composite score (${prospect.compositeScore.toFixed(0)})`,
		);
...
export async function matchDealToProspects(
	...
	if (!deal)
		throw new Error(`${dealType.toUpperCase()} deal not found: ${dealId}`);
	...
	const scored: ProspectMatchResult[] = rows
		.map((r) => {
			const surplus = Number.parseFloat(String(r.investableSurplus || "0"));
			const networth = Number.parseFloat(String(r.estimatedNetworth || "0"));
			const cs = Number.parseFloat(String(r.compositeScore || "0"));
			const ws = Number.parseFloat(String(r.wealthScore || "0"));
			...
			return {
				...
				surplus_cover: Number.parseFloat(surplusCover.toFixed(2)),
...
export async function getGeoIntelligence(): Promise<{
	...
	for (const r of rows) {
		...
		const cs = Number.parseFloat(String(r.compositeScore || "0"));
		const nw = Number.parseFloat(String(r.estimatedNetworth || "0"));
		const inv = Number.parseFloat(String(r.investableSurplus || "0"));
	...
	const cities = Object.entries(cityMap)
		.map(([city, d]) => ({
			...
			avgComposite: d.scores.length
				? Math.round(d.scores.reduce((a, b) => a + b, 0) / d.scores.length)
				: 0,
			avgNetworth: d.scores.length
				? Math.round(d.networthSum / d.scores.length)
				: 0,
			hotCount: d.hotCount,
