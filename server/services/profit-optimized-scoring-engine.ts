...
class ProfitOptimizedScoringEngine {
	...
	calculateSuitabilityScore(
		...
		const riskMatch = this.calculateRiskMatch(
			product.risk_level,
			clientProfile.risk_category,
		);
		const timeHorizonMatch = this.calculateHorizonMatch(
			product.investment_horizon,
			clientProfile.investment_horizon,
		);
		const liquidityMatch = this.calculateLiquidityMatch(
			product.liquidity,
			clientProfile.liquidity_needs,
		);
		const regulatoryEligibility = this.calculateRegulatoryEligibility(
			product,
			clientProfile,
		);
		...
		const isEligible =
			total >= SUITABILITY_THRESHOLD && regulatoryEligibility >= 50;
	...
	private calculateHorizonMatch(
		productHorizon: InvestmentHorizon,
		clientHorizon: InvestmentHorizon,
	): number {
	...
	private calculateLiquidityMatch(
		productLiquidity: LiquidityLevel,
		clientNeeds: LiquidityLevel,
	): number {
	...
	private calculateRegulatoryEligibility(
		product: InvestmentProduct,
		clientProfile: ClientProfile,
	): number {
		const allowedProducts =
			CLIENT_CATEGORY_PRODUCTS[clientProfile.client_category] || [];
	...
	private calculateStockUpside(product: InvestmentProduct): UpsideScore {
		...
		const returnPotential = Math.min(
			100,
			Math.max(0, ((product.expected_return_band.max - 8) / 20) * 100),
		);
		...
		return {
			...
			methodology:
				"STOCK: Return potential (30%) + Momentum (25%) + Valuation (25%) + Sector (20%)",
			inputs: {
	...
	private calculateSectorScore(sector: string): number {
		...
		if (moderateGrowthSectors.some((s) => normalizedSector.includes(s)))
			return 65;
	...
	private calculateMFUpside(product: InvestmentProduct): UpsideScore {
		...
		const returnPotential = Math.min(
			100,
			Math.max(0, ((product.expected_return_band.max - 6) / 15) * 100),
		);
		...
		return {
			...
			methodology:
				"MF: Return potential (35%) + Alpha (25%) + Consistency (25%) + Rating (15%)",
			inputs: {
				expectedReturnMax: product.expected_return_band.max,
	...
	private calculateConsistencyScore(raw: any): number {
		...
		const volatility =
			Math.abs(returns1y - avgReturn) +
			Math.abs(returns3y - avgReturn) +
			Math.abs(returns5y - avgReturn);
	...
	private calculateRatingScore(rating?: string): number {
		...
		if (
			normalizedRating.includes("5") ||
			normalizedRating.includes("aaa") ||
			normalizedRating === "platinum"
		)
			return 100;
		if (
			normalizedRating.includes("4") ||
			normalizedRating.includes("aa") ||
			normalizedRating === "gold"
		)
			return 80;
		if (
			normalizedRating.includes("3") ||
			normalizedRating.includes("a") ||
			normalizedRating === "silver"
		)
			return 60;
		if (normalizedRating.includes("2") || normalizedRating.includes("bb"))
			return 40;
		return 30;
	}
	...
	private calculateBondUpside(product: InvestmentProduct): UpsideScore {
		...
		const yieldScore = Math.min(
			...
			Math.max(
				0,
				(((product.yield_or_return || product.expected_return_band.max) - 5) /
					8) *
					100,
			),
		);
		...
		return {
			...
			methodology:
				"BOND: Yield (40%) + Credit Quality (25%) + Duration (20%) + Spread (15%)",
			inputs: {
	...
	private calculateCreditScore(rating?: string): number {
		...
		if (normalizedRating.includes("AAA") || normalizedRating.includes("SOV"))
			return 100;
	...
	private calculateREITInvITUpside(product: InvestmentProduct): UpsideScore {
		...
		return {
			...
			methodology:
				"REIT/InvIT: Yield (35%) + NAV Discount (25%) + Occupancy (20%) + Growth (20%)",
			inputs: {
				dividendYield,
				navDiscount,
	...
	private calculateIPOUpside(product: InvestmentProduct): UpsideScore {
		...
		return {
			...
			methodology:
				"IPO: Subscription (30%) + Valuation (30%) + Sector (20%) + Issuer Quality (20%)",
			inputs: {
	...
	private calculateUnlistedUpside(product: InvestmentProduct): UpsideScore {
		...
		return {
			...
			methodology:
				"UNLISTED: IPO Proximity (30%) + Growth (30%) + Valuation (25%) + Liquidity Premium (15%)",
			inputs: {
	...
	private calculateAIFPMSUpside(product: InvestmentProduct): UpsideScore {
		...
		return {
			...
			methodology:
				"AIF/PMS: Track Record (30%) + Strategy (25%) + Alpha (25%) + Risk-Adjusted (20%)",
			inputs: {
				returns: raw.returns,
	...
	private calculateStrategyScore(raw: any): number {
		...
		if (strategy.includes("multi") || strategy.includes("diversified"))
			return 75;
		...
		if (strategy.includes("long_short") || strategy.includes("hedge"))
			return 65;
		return 60;
	}
	...
	private calculateDefaultUpside(product: InvestmentProduct): UpsideScore {
		const returnPotential = Math.min(
			100,
			Math.max(0, ((product.expected_return_band.max - 6) / 20) * 100),
		);
	...
	calculateFinalScore(
		...
		const effectiveMode = this.killSwitchActive
	...
	scoreProducts(
		...
		for (const product of products) {
			...
			if (!suitability.isEligible) {
				console.log(
					`[SCORING] Excluded: ${product.name} - ${suitability.exclusionReason}`,
				);
				continue;
			}
	...
	registerOverride(
		override: Omit<AgentOverride, "overrideId" | "timestamp">,
	): AgentOverride {
		...
		console.log(
			`[AUDIT] Override registered: ${fullOverride.overrideType} by agent ${fullOverride.agentId}`,
		);
	...
	createAuditLog(
		params: Omit<RecommendationAuditLog, "logId" | "timestamp">,
	): RecommendationAuditLog {
		...
		console.log(
			`[AUDIT] Recommendation logged: ${log.mode} mode, ${log.productsRecommended} products for client ${log.clientId}`,
		);
	...
	compareWithBalanced(
		...
		const growthRankings = new Map(
			growthOptimizedProducts.map((p) => [p.product_id, p.ranking]),
		);
		const balancedRankings = new Map(
			balancedProducts.map((p) => [p.product_id, p.ranking]),
		);
		...
		for (const p of growthOptimizedProducts.slice(0, 10)) {
			growthAllocation[p.product_type] =
				(growthAllocation[p.product_type] || 0) + 1;
		}
		for (const p of balancedProducts.slice(0, 10)) {
			balancedAllocation[p.product_type] =
				(balancedAllocation[p.product_type] || 0) + 1;
		}

		const allProductTypes = new Set([
			...Object.keys(growthAllocation),
			...Object.keys(balancedAllocation),
		]);
