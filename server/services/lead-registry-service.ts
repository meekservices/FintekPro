...
const STATUS_ORDER = ['REGISTERED', 'LOGGED_IN', 'APPROVED', 'DISBURSED'] as const;

export class LeadRegistryService {
  private static instance: LeadRegistryService;

  static getInstance(): LeadRegistryService {
    if (!this.instance) this.instance = new LeadRegistryService();
  ...
  async registerLead(data: {
    ...
    const existing = await db.select().from(leadRegistry)
      .where(and(
    ...
    const [lead] = await db.insert(leadRegistry).values({
      pan: data.pan,
      mobile: data.mobile,
      ...
      status: 'REGISTERED',
      statusHistory: [{ status: 'REGISTERED', timestamp: new Date(), actorId: data.agentId }],
    }).returning();
  ...
  async setProcessingMode(
    ...
    if (lead.processingMode) {
      return { success: false, error: "Processing mode already set and cannot be changed" };
    }

    const [updated] = await db.update(leadRegistry).set({
      processingMode: mode as any,
      processingModeSetAt: new Date(),
    }).where(eq(leadRegistry.leadId, leadId)).returning();
  ...
  async setFinancierDetails(
    ...
    if (lead.processingMode !== 'EXTERNAL_FINANCIER') {
      return { success: false, error: "Financier details only applicable for external processing" };
    }
    ...
    await db.insert(leadAuditLogs).values({
      ...
      details: { financierName: data.financierName, bankerName: data.bankerName },
      ipAddress,
    });
  ...
  async updateStatus(
    ...
    if (!lead.processingMode) {
      return { success: false, error: "Processing mode must be set before updating status" };
    }
    ...
    if (newIndex <= currentIndex) {
      return { success: false, error: `Cannot transition from ${lead.status} to ${newStatus}. Only forward transitions are allowed.` };
    }

    const existingHistory = Array.isArray(lead.statusHistory) ? lead.statusHistory : [];
    const updatedHistory = [...existingHistory, { status: newStatus, timestamp: new Date(), actorId, actorRole }];
  ...
  async getLeadsByAgent(agentId: string): Promise<LeadRegistry[]> {
    return db.select().from(leadRegistry)
