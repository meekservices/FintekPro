...
class AIBondRecommendationService {
	constructor() {
		...
		console.log(
			`✅ AI Bond Recommendation Service initialized via Unified Engine (primary: ${status.primary})`,
		);
	}

	async generateRecommendations(
		params: BondRecommendationParams,
	): Promise<BondPortfolioSummary> {
		const govBonds = await this.fetchGovernmentSecurities(params);
		const corpBonds = await this.fetchCorporateBonds(params);

		const scoredGovBonds = govBonds.map((bond) =>
			this.scoreBond(bond, "government", params),
		);
		const scoredCorpBonds = corpBonds.map((bond) =>
			this.scoreBond(bond, "corporate", params),
		);

		const allBonds = [...scoredGovBonds, ...scoredCorpBonds].sort(
			(a, b) => b.suitabilityScore - a.suitabilityScore,
		);

		const selectedBonds = this.selectOptimalPortfolio(allBonds, params);

		const enrichedBonds = await this.enrichWithAIRationale(
			selectedBonds,
			params,
		);
	...
	private async fetchGovernmentSecurities(
		params: BondRecommendationParams,
	): Promise<GovernmentSecurity[]> {
		...
		if (params.preferredBondTypes.includes("t_bill"))
			allowedTypes.push("t_bill");
		...
		if (params.preferredBondTypes.includes("tax_free_bond"))
			allowedTypes.push("tax_free_bond");
		if (params.preferredBondTypes.includes("infrastructure_bond"))
			allowedTypes.push("infrastructure_bond");
		...
		try {
			...
			throw new Error(
				"Bond data API not configured. Live bond market data service required.",
			);
		}
	}

	private async fetchCorporateBonds(
		params: BondRecommendationParams,
	): Promise<CorporateBond[]> {
		const allowedTypes: string[] = [];
		if (params.preferredBondTypes.includes("corporate_bond"))
			allowedTypes.push("corporate_bond");
		...
		const minRatingIndex = RATING_ORDER.indexOf(
			params.minimumRating === "any" ? "D" : params.minimumRating,
		);
	...
	private scoreBond(
		...
		const creditRating = isGov
			? govBond?.creditRating || "AAA"
			: corpBond?.creditRating || "AA";
		const ratingScore = RATING_SCORES[creditRating] || 50;

		const ytm =
			Number.parseFloat(
				(isGov ? govBond?.yieldToMaturity : corpBond?.yieldToMaturity) || "7",
			) || 7;
		const couponRate =
			Number.parseFloat(
				(isGov ? govBond?.couponRate : corpBond?.couponRate) || "6",
			) || 6;
		const currentPrice =
			Number.parseFloat(
				(isGov ? govBond?.currentPrice : corpBond?.currentPrice) || "100",
			) || 100;
		const duration =
			Number.parseFloat(
				(isGov ? govBond?.duration : corpBond?.duration) || "3",
			) || 3;
		const modDuration =
			Number.parseFloat(
				(isGov ? govBond?.modifiedDuration : corpBond?.modifiedDuration) ||
					"2.8",
			) || 2.8;
		...
		const riskToleranceWeights: Record<
			string,
			{ safety: number; yield: number }
		> = {
		...
		if (
			...
			isGov &&
			govBond?.securityType === "sgb"
		) {
			suitabilityScore += 12;
		}

		if (params.monthlyIncomeNeeded) {
			const couponFreq = isGov
				? "semi_annual"
				: corpBond?.couponFrequency || "annual";
		...
		const horizonMonths =
			params.investmentHorizon === "short"
				? 18
				: params.investmentHorizon === "medium"
					? 60
					: 120;
		const daysToTarget = horizonMonths * 30;
		const maturityMatch =
			1 - Math.min(1, Math.abs(daysToMaturity - daysToTarget) / daysToTarget);
		suitabilityScore += maturityMatch * 10;

		const riskScore = Math.round(100 - ratingScore + modDuration * 3);

		const taxBracketNum = Number.parseInt(params.taxBracket);
		const taxStatus = isGov ? govBond?.taxStatus : corpBond?.taxStatus;
		...
		return {
			...
			suitabilityScore: Math.min(
				100,
				Math.max(0, Math.round(suitabilityScore)),
			),
			aiRationale: "",
			pros: [],
			cons: [],
	...
	private selectOptimalPortfolio(
		...
		const targetCount = Math.min(
			8,
			Math.max(3, Math.floor(params.investmentAmount / 100000)),
		);
		...
		const govAllocation =
			params.riskTolerance === "conservative"
				? 0.7
				: params.riskTolerance === "moderately_conservative"
					? 0.6
					: params.riskTolerance === "moderate"
						? 0.5
						: params.riskTolerance === "moderately_aggressive"
							? 0.4
							: 0.3;
		...
		const remainingAmount = params.investmentAmount;
		...
		selected.forEach((bond, index) => {
			...
			bond.expectedAnnualIncome =
				units * bond.currentPrice * (bond.couponRate / 100);
		});

		const totalAllocation = selected.reduce(
			(sum, b) => sum + b.suggestedAllocation,
			0,
		);
		if (totalAllocation !== 100 && selected.length > 0) {
			const adjustment = (100 - totalAllocation) / selected.length;
			selected.forEach((bond) => {
				bond.suggestedAllocation = Math.round(
					bond.suggestedAllocation + adjustment,
				);
				bond.suggestedAmount = Math.round(
					params.investmentAmount * (bond.suggestedAllocation / 100),
				);
			});
		}
...
[{"bondIndex": 0, "rationale": "...", "pros": ["..."], "cons": ["..."], "taxImplications": "..."}]`;
	...
	private addRuleBasedRationale(
		...
		const taxBracket = Number.parseInt(params.taxBracket);
		let taxImplications = "";
		if (bond.taxEfficiency === "high") {
			taxImplications =
				"Interest income is tax-free, maximizing post-tax returns.";
		} else {
	...
	private buildPortfolioSummary(
		...
		const totalInvestment = bonds.reduce(
			(sum, b) => sum + b.suggestedAmount,
			0,
		);

		const weightedYield = bonds.reduce(
			(sum, b) => sum + (b.yieldToMaturity * b.suggestedAllocation) / 100,
			0,
		);

		const weightedDuration = bonds.reduce(
			(sum, b) => sum + (b.duration * b.suggestedAllocation) / 100,
			0,
		);

		const avgRatingScore = bonds.reduce(
			(sum, b) =>
				sum +
				((RATING_SCORES[b.creditRating] || 50) * b.suggestedAllocation) / 100,
			0,
		);
		...
		const expectedAnnualIncome = bonds.reduce(
			(sum, b) => sum + b.expectedAnnualIncome,
			0,
		);
		...
		if (bonds.length >= 3) {
			...
			const mediumTerm = bonds.filter(
				(b) => b.daysToMaturity >= 1095 && b.daysToMaturity < 2555,
			);
	...
	private generatePortfolioRationale(
		...
		parts.push(
			`This portfolio of ${bonds.length} bonds is designed for a ${params.riskTolerance.replace("_", " ")} investor with a ${params.investmentHorizon}-term horizon.`,
		);

		parts.push(
			`The portfolio offers a weighted average yield of ${metrics.weightedYield.toFixed(2)}% with ${metrics.averageRating} average credit quality.`,
		);

		if (params.taxOptimization) {
			const taxFreeCount = bonds.filter(
				(b) => b.taxEfficiency === "high",
			).length;
			if (taxFreeCount > 0) {
				parts.push(
					`${taxFreeCount} tax-efficient bonds are included to optimize after-tax returns for the ${params.taxBracket}% tax bracket.`,
				);
			}
		}

		const govAlloc = bonds
			.filter((b) => b.bondType === "government")
			.reduce((sum, b) => sum + b.suggestedAllocation, 0);
		if (govAlloc > 40) {
			parts.push(
				`${govAlloc}% allocation to government securities provides a stable foundation with sovereign backing.`,
			);
		}

		parts.push(
			`Duration of ${metrics.weightedDuration.toFixed(1)} years balances income generation with interest rate risk management.`,
		);
