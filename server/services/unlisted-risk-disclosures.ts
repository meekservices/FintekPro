...
export const SEBI_RISK_DISCLOSURES: RiskDisclosure[] = [
	{
		...
		description:
			"Unlisted securities are not traded on recognized stock exchanges and carry significantly higher risk than listed securities. You may lose part or all of your investment.",
		mandatory: true,
	},
	{
		id: "liquidity-1",
		...
		description:
			"Unlike listed securities, there is no established market for unlisted shares. You may not be able to sell your shares when you want to, or at the price you expect. Finding a buyer may take considerable time.",
		mandatory: true,
	},
	{
		id: "liquidity-2",
		...
		description:
			"There is no transparent price discovery mechanism for unlisted securities. The prices quoted may not reflect the true value of the shares and can vary significantly between transactions.",
		mandatory: true,
	},
	{
		id: "regulatory-1",
		...
		description:
			"Unlisted securities are not subject to the same level of regulatory scrutiny and investor protection as listed securities. SEBI regulations for listed companies may not apply.",
		mandatory: true,
	},
	{
		id: "regulatory-2",
		...
		description:
			"Unlisted companies are not required to make the same disclosures as listed companies. Financial information may be limited, outdated, or not independently verified.",
		mandatory: true,
	},
	{
		id: "information-1",
		...
		description:
			"There may be significant information asymmetry between buyers and sellers. Company insiders may possess material non-public information that affects the value of shares.",
		mandatory: true,
	},
	{
		id: "information-2",
		...
		description:
			"Financial data provided for unlisted companies may not be audited or verified. The accuracy and completeness of financial information cannot be guaranteed.",
		mandatory: true,
	},
	{
		id: "market-1",
		...
		description:
			"The value of unlisted shares can be highly volatile and may fluctuate significantly based on company performance, market conditions, or speculative trading.",
		mandatory: true,
	},
	{
		id: "market-2",
		...
		description:
			"Companies may or may not pursue an IPO. Even if announced, IPO timelines are uncertain and may be delayed or cancelled. Pre-IPO share prices may not correlate with eventual listing prices.",
		mandatory: true,
	},
	{
		id: "operational-1",
		...
		description:
			"Transfer of unlisted shares involves procedural risks including delayed transfers, documentation issues, and potential disputes over ownership.",
		mandatory: true,
	},
	{
		id: "operational-2",
		...
		description:
			"In over-the-counter transactions, there is counterparty risk where the other party may fail to fulfill their obligations.",
		mandatory: true,
	},
	{
		id: "operational-3",
		...
		description:
			"Transactions in unlisted securities may have different tax implications than listed securities. Consult your tax advisor for specific guidance.",
		mandatory: false,
	},
	{
		id: "valuation-1",
		...
		description:
			"Unlisted securities lack standard market-based valuation. Prices are based on internal valuations, comparable transactions, or negotiations. Actual value may differ significantly from quoted prices.",
		mandatory: true,
	},
	{
		id: "valuation-2",
		...
		description:
			"Unlike listed securities, there is no independent third-party valuation mechanism. Valuations may be influenced by interested parties and may not reflect fair market value.",
		mandatory: true,
	},
	{
		id: "fraud-1",
		...
		description:
			"The unlisted securities market is susceptible to fraudulent schemes, including pump-and-dump schemes, fake company documents, and misrepresentation of financial data. Exercise extreme caution.",
		mandatory: true,
	},
	{
		id: "fraud-2",
		...
		description:
			"Verification of company information, share certificates, and seller identity may be limited. FintekPro performs due diligence but cannot guarantee the authenticity of all information.",
		mandatory: true,
	},
];
...
I confirm that I am making this trade voluntarily and understand all associated risks.`;
	...
	getCompanySpecificRisks(companyData: {
		...
		if (companyData.netWorth !== undefined && companyData.netWorth < 0) {
			risks.push(
				"⚠️ This company has negative net worth, indicating financial distress.",
			);
		}

		if (
			companyData.debtEquityRatio !== undefined &&
			companyData.debtEquityRatio > 2
		) {
			risks.push(
				"⚠️ This company has high leverage (Debt/Equity > 2x), increasing financial risk.",
			);
		}

		if (
			companyData.profitMargin !== undefined &&
			companyData.profitMargin < 0
		) {
		...
		if (companyData.riskScore !== undefined && companyData.riskScore > 70) {
			risks.push(
				"⚠️ This company has been flagged as high-risk based on our compliance assessment.",
			);
		}
	...
	formatDisclosuresForDisplay(): {
		...
		const categories = Object.keys(categoryTitles)
			.map((cat) => ({
				...
				disclosures: this.getDisclosuresByCategory(
					cat as RiskDisclosure["category"],
				),
			}))
...
export const unlistedRiskDisclosureService =
	new UnlistedRiskDisclosureService();
...
export async function hasValidRiskAcknowledgment(
	...
	const [record] = await db
		...
		.where(
			and(
				...
				eq(
					unlistedRiskDisclosureAcknowledgments.disclosureVersion,
					currentVersion,
				),
				eq(
					unlistedRiskDisclosureAcknowledgments.allMandatoryAcknowledged,
					true,
				),
				gte(
					unlistedRiskDisclosureAcknowledgments.acknowledgedAt,
					twentyFourHoursAgo,
				),
			),
		)
	...
	if (!record) {
		return {
			valid: false,
			reason: "No valid risk disclosure acknowledgment found for this company",
		};
	}
	...
	if (record.expiresAt && new Date(record.expiresAt) < new Date()) {
		return {
			valid: false,
			reason: "Risk disclosure acknowledgment has expired",
			record,
		};
	}
...
export async function saveRiskAcknowledgment(data: {
	...
	const mandatoryIds = unlistedRiskDisclosureService
		.getMandatoryDisclosures()
		.map((d) => d.id);
	const allMandatoryAcknowledged = mandatoryIds.every((id) =>
		data.acknowledgedDisclosureIds.includes(id),
	);

	if (!allMandatoryAcknowledged) {
		const missing = mandatoryIds.filter(
			(id) => !data.acknowledgedDisclosureIds.includes(id),
		);
		return {
			success: false,
	...
	const [record] = await db
		...
		.values({
			...
			companySpecificRisksAcknowledged:
				data.companySpecificRisksAcknowledged || [],
			acknowledgmentStatement:
				data.acknowledgmentStatement ||
				unlistedRiskDisclosureService.getAcknowledgmentText(),
 ...
 */
async function resolveCompanyIdFromEntity(
	req: Request,
): Promise<string | null> {
...
export const requireRiskDisclosure = (tradeType: "buy" | "sell") => {
	return async (
		...
		try {
			...
			if (!companyId) {
				console.warn(
					`[RiskDisclosure] Could not resolve companyId for ${req.method} ${req.path}`,
				);
				return res.status(400).json({
					success: false,
					error: "COMPANY_RESOLUTION_FAILED",
					message:
						"Could not determine company for risk disclosure verification",
				});
			}

			const { valid, record, reason } = await hasValidRiskAcknowledgment(
				userId,
				companyId,
				tradeType,
			);
			...
			if (!valid) {
				return res.status(403).json({
					...
					message:
						"You must acknowledge risk disclosures before placing this order",
					reason,
					disclosures:
