...
export class SIPSimulatorEngine {
	...
	async simulateSIP(input: SIPSimulationInput): Promise<SIPSimulationResult> {
		const { sipAmount, candidateFunds, existingPortfolio, horizonMonths } =
			input;
		...
		for (let month = 1; month <= horizonMonths; month++) {
			...
			const totalPortfolioValue =
				projectedPortfolio.reduce((sum, f) => sum + (f.currentValue || 0), 0) +
				runningTotal;

			if (sipRouting.sipRouting.length > 0) {
				// Update portfolio with SIP allocations
				for (const routing of sipRouting.sipRouting) {
					const existingFund = projectedPortfolio.find(
						(f) => f.mfIsin === routing.fundIsin,
					);
					if (existingFund) {
						existingFund.currentValue =
							(existingFund.currentValue || 0) + routing.amount;
					} else {
				...
				// Recalculate weights
				const newTotalValue = projectedPortfolio.reduce(
					(sum, f) => sum + (f.currentValue || 0),
					0,
				);
			...
			const improvementRate = this.calculateImprovementRate(
				horizonMonths,
				sipRouting.sipRouting.length,
			);
			const monthlyImprovement =
				improvementRate * (1 - Math.exp(-month / (horizonMonths / 3)));
			currentScore = Math.min(
				100,
				startingScore.score + monthlyImprovement * (month / horizonMonths) * 20,
			);
		...
		const endScore =
			monthlySnapshots[monthlySnapshots.length - 1].diversificationScore;
		const scoreImprovement = endScore - startingScore.score;

		// Generate overlap reduction summary based on routing
		const overlapReductionSummary = this.generateOverlapSummary(
		...
		return {
			...
			riskDisclosure:
				"Past performance is not indicative of future results. Investment in mutual funds is subject to market risks. This simulation is for educational purposes only and does not guarantee any specific outcome.",
		};
	}
