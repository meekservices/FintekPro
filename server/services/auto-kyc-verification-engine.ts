...
const PARTNER_ROLES: AutoKycRole[] = [
	"partner",
	"agent",
	"sub_agent",
	"associate",
...
class AutoKycVerificationEngine {
	async verify(payload: AutoKycPayload): Promise<AutoKycResult> {
		...
		const productType = PARTNER_ROLES.includes(role)
			? "partner_kyc"
			: "client_kyc";
		...
		const panVerified =
			panResult.status === "fulfilled" && panResult.value?.success === true;
		const bankVerified =
			bankResult.status === "fulfilled" && bankResult.value?.success === true;
		const ckycFound =
			ckycResult.status === "fulfilled" && ckycResult.value?.success === true;
		...
		if (hasLevel1) {
			await this.writeVerificationFlags({
				...
				kycLevel,
				panVerificationData:
					panResult.status === "fulfilled" ? panResult.value?.data : undefined,
				ckycData:
					ckycResult.status === "fulfilled"
						? ckycResult.value?.data
						: undefined,
			});
	...
	private async writeVerificationFlags(params: {
		...
		try {
			...
			try {
				...
				logger.warn("[AutoKycEngine] Failed to invalidate compliance cache", {
					userId,
				});
			}

			logger.info("[AutoKycEngine] Verification flags written to DB", {
				userId,
				kycLevel,
			});
		} catch (err) {
