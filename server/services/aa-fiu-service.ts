...
export type AAAssetType = 'MF' | 'DEMAT' | 'PPF' | 'NPS' | 'EPF' | 'LOANS' | 'BANK';
...
export class AAFIUService {
  ...
  private initializeCircuitBreakers() {
    const fiuNames = ['CAMS', 'KFinTech', 'NSDL', 'CDSL', 'NPS_CRA', 'EPFO', 'BANK'];
  ...
  async createConsentRequest(request: ConsentRequest): Promise<ConsentResponse> {
    ...
    console.log(`🔐 Creating AA consent request for user ${request.userId} with PAN ${request.panNumber.substring(0, 5)}XXXXX`);
        ...
        const response = await axios.post(
          ...
          consentPayload,
          {
            headers: {
              ...
              'x-jws-signature': await this.generateJWSSignature(consentPayload),
  ...
  async handleConsentCallback(
    ...
    console.log(`📥 Processing consent callback for ${consentHandleId}: ${status}`);
   ...
   */
  async fetchAggregatedData(consentSessionId: string): Promise<AggregatedPortfolio> {
    const [session] = await db
      .select()
      .from(aaConsentSessions)
    ...
    if (session.status !== 'approved') {
      throw new Error(`Consent not approved. Current status: ${session.status}`);
    }

    console.log(`🔄 Fetching aggregated data for consent session: ${consentSessionId}`);

    const fetchSessionId = `FETCH_${nanoid(12)}`;
    const assetTypes = (session.assetTypes as AAAssetType[]) || ['MF', 'DEMAT', 'NPS', 'EPF', 'PPF'];

    const results: AADataFetchResult[] = [];

    // Fetch data from each FIU in parallel
    const fetchPromises = assetTypes.map(async (assetType) => {
      return this.fetchDataForAssetType(
        session,
        fetchSessionId,
        assetType
      );
    ...
    console.log(`✅ Aggregated portfolio fetched: ${results.filter(r => r.success).length}/${results.length} sources successful`);
  ...
  private async fetchDataForAssetType(
    ...
    const [logEntry] = await db.insert(aaDataFetchLogs).values(fetchLog).returning();

    try {
      // Check circuit breaker
      if (this.isCircuitOpen(fiuName)) {
        console.log(`⚡ Circuit breaker open for ${fiuName}, using fallback`);
        return this.useFallback(session, fetchSessionId, assetType, logEntry.id, 'Circuit breaker open');
      }
      ...
      // Store raw payload
      await this.storeRawPayload(session.id, session.userId, fetchSessionId, fiuName, assetType, data);
      ...
      return this.useFallback(session, fetchSessionId, assetType, logEntry.id, error.message);
    }
  }
  ...
  private async useFallback(
    ...
    try {
      ...
      console.error(`❌ Fallback also failed for ${assetType}:`, fallbackError.message);
   ...
   */
  async getActiveConsentSession(userId: string): Promise<AAConsentSession | null> {
    const [session] = await db
      .select()
      .from(aaConsentSessions)
      .where(
   ...
   */
  async checkConsentStatus(consentHandleId: string): Promise<{ status: string; session: AAConsentSession | null }> {
    const [session] = await db
      .select()
      .from(aaConsentSessions)
      .where(eq(aaConsentSessions.consentHandleId, consentHandleId))
  ...
  private calculateTotalValue(data: any): number {
    ...
    if (Array.isArray(data)) {
      return data.reduce((sum, item) => {
        return sum + (item.marketValue || item.balance || item.totalBalance || item.value || 0);
      }, 0);
    }
    return data.marketValue || data.balance || data.totalBalance || data.value || 0;
  }
