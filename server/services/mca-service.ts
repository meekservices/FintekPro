...
class MCAService {
	...
	async getCompanyByCIN(cin: string): Promise<MCACompanyMasterData | null> {
		...
		try {
			...
			if (
				...
				response.data?.data?.company_master_data
			) {
		...
		if (!cin || cin.length !== 21) {
			return {
				...
				error: {
					...
					troubleshooting:
						"CIN must be exactly 21 characters in the format: U12345AB1234ABC123456",
					isRetryable: false,
				},
			};
		}
		...
		if (!SANDBOX_API_KEY || !SANDBOX_API_SECRET) {
			return {
				...
				error: {
					...
					troubleshooting:
						"Sandbox.co.in API credentials (SANDBOX_API_KEY, SANDBOX_API_SECRET) are not set. Contact system administrator.",
					isRetryable: false,
				},
			};
		}
		...
		try {
			...
			return {
				...
				error: {
					...
					troubleshooting:
						"The CIN may not exist in MCA records, or the company data is not yet available. Try verifying the CIN on the MCA portal.",
					isRetryable: false,
				},
			};
		} catch (error: any) {
			...
			switch (errorCode) {
				case 401:
					troubleshooting =
						"Authentication failed. API credentials may be expired or invalid. Contact system administrator.";
					isRetryable = false;
					break;
				case 403:
					troubleshooting =
						"Access denied by Sandbox.co.in API. Possible causes: (1) API subscription does not include this CIN, (2) API quota exhausted, (3) CIN not indexed in test environment. Try in production or contact Sandbox support.";
					isRetryable = false;
					break;
				case 404:
					troubleshooting =
						"Company not found. The CIN may be incorrect or the company is not registered with MCA.";
					isRetryable = false;
					break;
				case 429:
					troubleshooting =
						"Rate limit exceeded. Too many requests to Sandbox.co.in API. Please wait a few minutes and try again.";
					isRetryable = true;
					break;
				case 500:
				case 502:
				...
				case 504:
					troubleshooting =
						"Sandbox.co.in API is temporarily unavailable. Please try again later.";
					isRetryable = true;
					break;
				default:
	 ...
	 */
	private transformMCAResponse(
		masterData: any,
		fullData: any,
	): MCACompanyMasterData {
		const parseNumber = (value: string | number | undefined): number => {
			if (typeof value === "number") return value;
			if (!value) return 0;
			return Number.parseFloat(value.toString().replace(/[,\s]/g, "")) || 0;
		};
		...
		const directors: MCADirector[] = (
			fullData["directors/signatory_details"] || []
		).map((d: any) => ({
			din: d["din/pan"] || "",
			name: d.name || "",
		...
		const charges: MCACharge[] = (fullData.charges || []).map((c: any) => ({
			...
			dateOfModification:
				c.date_of_modification !== "-"
					? parseDate(c.date_of_modification)
					: undefined,
		...
		const shareholdingData =
			fullData.shareholding_pattern || fullData.shareholding || [];
		for (const sh of shareholdingData) {
			shareholding.push({
				...
				institutionalHolding: parseNumber(
					sh.institutional_holding || sh.institutionalHolding,
				),
		...
		if (
			...
			masterData.whether_listed_or_not?.toLowerCase() === "unlisted"
		) {
			...
			shareholding.push({
				financialYear: latestFY
					? `${Number.parseInt(latestFY) - 1}-${latestFY.slice(-2)}`
					: "",
				promoterHolding: 100,
				publicHolding: 0,
	...
	async searchCompanyByName(companyName: string): Promise<MCASearchResult[]> {
		...
		try {
			...
			if (response.data?.code === 200 && response.data?.data?.companies) {
				...
				console.log(
					`[MCA] Found ${companies.length} companies matching "${searchName}"`,
				);
				...
				return companies.map((c: any) => ({
					...
					authorizedCapital: Number.parseFloat(c.authorized_capital) || 0,
					paidUpCapital: Number.parseFloat(c.paid_up_capital) || 0,
	...
	async getCINByCompanyName(
		companyName: string,
	): Promise<{ cin: string; officialName: string } | null> {
		...
		for (const result of results) {
			...
			// Word overlap
				...
				score =
					(matchingWords / Math.max(inputWords.length, resultWords.size)) * 70;
			}
		...
		if (bestMatch && bestScore >= 40) {
			console.log(
				`[MCA] Best match for "${companyName}": "${bestMatch.name}" (CIN: ${bestMatch.cin}, score: ${bestScore})`,
			);
			return {
				cin: bestMatch.cin,
		...
		console.log(
			`[MCA] No confident match found for "${companyName}" (best score: ${bestScore})`,
		);
		return null;
	}
	...
	toFintekProCompanyData(mcaData: MCACompanyMasterData): {
		...
		return {
			...
			cin: mcaData.cin,
			status: mcaData.companyStatus.toLowerCase().includes("active")
				? "active"
				: "inactive",
	...
	estimateBasicMetrics(mcaData: MCACompanyMasterData): {
		...
		const totalActiveChargeAmount = activeCharges.reduce(
