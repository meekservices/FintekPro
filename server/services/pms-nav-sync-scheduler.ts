...
class PmsNavSyncScheduler {
  ...
  private scheduleNextSync(): void {
    ...
    if (nowIST.getHours() >= 7 || (nowIST.getHours() === 7 && nowIST.getMinutes() >= 30)) {
      next730AM.setDate(next730AM.getDate() + 1);
    }
    
    const msUntilNext = next730AM.getTime() - nowIST.getTime();
    
    console.log(`[PMS Sync] Next NAV sync scheduled in ${Math.round(msUntilNext / 1000 / 60)} minutes`);
  ...
  async runNAVRefresh(): Promise<{ updated: number; errors: number }> {
    ...
    try {
      ...
      const staleFunds = await db.select({
        ...
        .from(pmsMaster)
        .where(sql`${pmsMaster.updatedAt} IS NULL OR ${pmsMaster.updatedAt} < ${staleThreshold}`)
        .orderBy(sql`${pmsMaster.updatedAt} ASC NULLS FIRST`)
        .limit(100);
      
      console.log(`[PMS Sync] Found ${staleFunds.length} stale PMS funds to refresh`);
      ...
      console.log(`[PMS Sync] NAV refresh complete: ${updated} updated, ${errors} errors`);
    } catch (error) {
      console.error('[PMS Sync] NAV refresh failed:', error);
    }
  ...
  async runStartupCatchUp(): Promise<{ updated: number; errors: number }> {
    ...
    console.log(`[PMS Sync] Startup catch-up complete: ${totalUpdated} updated, ${totalErrors} errors`);
  ...
  async getStatus(): Promise<{
    ...
    const [total] = await db.select({ count: sql<number>`count(*)` }).from(pmsMaster);
