...
export class FinancialMetricsCalculator {
  ...
  calculatePriceToBook(price: number, bookValuePerShare: number): number | null {
  ...
  calculateEnterpriseValue(marketCap: number, totalDebt: number, cash: number): number {
  ...
  calculateCAGR(startValue: number, endValue: number, years: number): number | null {
  ...
  calculateMultiYearCAGR(values: { year: string; value: number }[], targetYears: number): number | null {
    if (values.length < targetYears + 1) return null;
    const sortedValues = [...values].sort((a, b) => a.year.localeCompare(b.year));
  ...
  calculateInterestCoverage(ebit: number, interestExpense: number): number | null {
  ...
  calculateQuickRatio(currentAssets: number, inventory: number, currentLiabilities: number): number | null {
    if (!currentLiabilities || currentLiabilities <= 0) return null;
  ...
  calculateInventoryTurnover(cogs: number, avgInventory: number): number | null {
    if (!avgInventory || avgInventory <= 0) return null;
    return cogs / avgInventory;
  }

  calculateReceivablesTurnover(revenue: number, avgReceivables: number): number | null {
  ...
  calculateCashConversionCycle(inventoryDays: number, receivableDays: number, payableDays: number): number {
    return (inventoryDays || 0) + (receivableDays || 0) - (payableDays || 0);
  }

  calculateWorkingCapitalTurnover(revenue: number, workingCapital: number): number | null {
  ...
  calculatePiotroskiFScore(data: FinancialData, prevData?: FinancialData): number {
    let score = 0;
    ...
    if (prevData && data.currentAssets && data.currentLiabilities && prevData.currentAssets && prevData.currentLiabilities) {
      const currentRatioCurrent = data.currentAssets / data.currentLiabilities;
      const currentRatioPrev = prevData.currentAssets / prevData.currentLiabilities;
    ...
    // Efficiency (2 points)
    if (prevData?.grossProfit && data.grossProfit && prevData.revenue && data.revenue) {
    ...
    if (prevData && data.revenue && data.totalAssets && prevData.revenue && prevData.totalAssets) {
  ...
  calculateAccrualRatio(netIncome: number, ocf: number, totalAssets: number): number | null {
    if (!totalAssets || totalAssets <= 0) return null;
  ...
  calculateAllMetrics(
    ...
    const capitalEmployed = (current.totalAssets || 0) - (current.currentLiabilities || 0);
    
    // Get historical values for CAGR calculations
    const revenueHistory = historical.map(h => ({ year: h.fiscalYear, value: h.data.revenue || 0 }));
    const epsHistory = historical.map(h => ({ year: h.fiscalYear, value: h.data.eps || 0 }));
    const patHistory = historical.map(h => ({ year: h.fiscalYear, value: h.data.netIncome || 0 }));
    
    if (current.revenue) revenueHistory.push({ year: 'current', value: current.revenue });
    if (current.eps) epsHistory.push({ year: 'current', value: current.eps });
    if (current.netIncome) patHistory.push({ year: 'current', value: current.netIncome });
    
    const inventoryTurnover = this.calculateInventoryTurnover(
      (current.revenue || 0) * 0.65, // Estimate COGS as 65% of revenue
      current.inventory || 0
    );
    const receivablesTurnover = this.calculateReceivablesTurnover(current.revenue || 0, current.receivables || 0);
    const payablesTurnover = current.payables ? (current.revenue || 0) * 0.65 / current.payables : null;
    
    const inventoryDays = this.calculateDaysOutstanding(inventoryTurnover || 0);
    const receivableDays = this.calculateDaysOutstanding(receivablesTurnover || 0);
    ...
    return {
      ...
      forwardPe: epsEstimate ? this.calculateForwardPE(price, epsEstimate)?.toString() : undefined,
      ...
      priceToBook: this.calculatePriceToBook(price, current.bookValuePerShare || 0)?.toString(),
      priceToSales: this.calculatePriceToSales(current.marketCap || 0, current.revenue || 0)?.toString(),
      priceToFreeCashFlow: this.calculatePriceToFCF(current.marketCap || 0, current.freeCashFlow || 0)?.toString(),
      ...
      earningsYield: this.calculateEarningsYield(current.eps || 0, price)?.toString(),
      
      // Profitability
      grossMargin: this.calculateGrossMargin(current.grossProfit || 0, current.revenue || 0)?.toString(),
      operatingMargin: this.calculateOperatingMargin(current.ebit || 0, current.revenue || 0)?.toString(),
      netMargin: this.calculateNetMargin(current.netIncome || 0, current.revenue || 0)?.toString(),
      ebitdaMargin: this.calculateEBITDAMargin(current.ebitda || 0, current.revenue || 0)?.toString(),
      fcfMargin: this.calculateFCFMargin(current.freeCashFlow || 0, current.revenue || 0)?.toString(),
      roe: this.calculateROE(current.netIncome || 0, current.totalEquity || 0)?.toString(),
      roa: this.calculateROA(current.netIncome || 0, current.totalAssets || 0)?.toString(),
      roce: this.calculateROCE(current.ebit || 0, capitalEmployed)?.toString(),
      roic: this.calculateROIC((current.ebit || 0) * 0.75, capitalEmployed)?.toString(),
      
      // Growth YoY
      revenueGrowthYoy: this.calculateYoYGrowth(current.revenue || 0, prevYear?.revenue || 0)?.toString(),
      epsGrowthYoy: this.calculateYoYGrowth(current.eps || 0, prevYear?.eps || 0)?.toString(),
      netIncomeGrowthYoy: this.calculateYoYGrowth(current.netIncome || 0, prevYear?.netIncome || 0)?.toString(),
      ebitdaGrowthYoy: this.calculateYoYGrowth(current.ebitda || 0, prevYear?.ebitda || 0)?.toString(),
      bookValueGrowthYoy: this.calculateYoYGrowth(current.bookValuePerShare || 0, prevYear?.bookValuePerShare || 0)?.toString(),
      ocfGrowthYoy: this.calculateYoYGrowth(current.operatingCashFlow || 0, prevYear?.operatingCashFlow || 0)?.toString(),
      fcfGrowthYoy: this.calculateYoYGrowth(current.freeCashFlow || 0, prevYear?.freeCashFlow || 0)?.toString(),
      ...
      // Leverage
      debtToEquity: this.calculateDebtToEquity(current.totalDebt || 0, current.totalEquity || 0)?.toString(),
      debtToAssets: this.calculateDebtToAssets(current.totalDebt || 0, current.totalAssets || 0)?.toString(),
      interestCoverage: this.calculateInterestCoverage(current.ebit || 0, current.interestExpense || 0)?.toString(),
      currentRatio: this.calculateCurrentRatio(current.currentAssets || 0, current.currentLiabilities || 0)?.toString(),
      quickRatio: this.calculateQuickRatio(current.currentAssets || 0, current.inventory || 0, current.currentLiabilities || 0)?.toString(),
      cashRatio: this.calculateCashRatio(current.cash || 0, current.currentLiabilities || 0)?.toString(),
      netDebt: netDebt.toString(),
      netDebtToEbitda: this.calculateNetDebtToEBITDA(netDebt, current.ebitda || 0)?.toString(),
      
      // Efficiency
      assetTurnover: this.calculateAssetTurnover(current.revenue || 0, current.totalAssets || 0)?.toString(),
      ...
      cashConversionCycle: this.calculateCashConversionCycle(inventoryDays || 0, receivableDays || 0, payableDays || 0).toString(),
      workingCapitalTurnover: this.calculateWorkingCapitalTurnover(current.revenue || 0, current.workingCapital || 0)?.toString(),
      
      // Quality Scores
      piotroskiFScore: this.calculatePiotroskiFScore(current, prevYear),
      altmanZScore: this.calculateAltmanZScore(current)?.toString(),
      accrualRatio: this.calculateAccrualRatio(current.netIncome || 0, current.operatingCashFlow || 0, current.totalAssets || 0)?.toString(),
      earningsQuality: this.calculateEarningsQuality(current.operatingCashFlow || 0, current.netIncome || 0)?.toString(),
      
      // Dividend
      dividendYield: this.calculateDividendYield(current.dividendPerShare || 0, price)?.toString(),
      dividendPayoutRatio: this.calculateDividendPayoutRatio(current.dividendPerShare || 0, current.eps || 0)?.toString(),
      dividendCoverRatio: this.calculateDividendCoverRatio(current.eps || 0, current.dividendPerShare || 0)?.toString(),
  ...
  calculateSharpeRatio(returns: number, riskFreeRate: number, stdDev: number): number | null {
    if (!stdDev || stdDev <= 0) return null;
    return (returns - riskFreeRate) / stdDev;
  }

  calculateSortinoRatio(returns: number, riskFreeRate: number, downsideDeviation: number): number | null {
    if (!downsideDeviation || downsideDeviation <= 0) return null;
    return (returns - riskFreeRate) / downsideDeviation;
  }

  calculateTreynorRatio(returns: number, riskFreeRate: number, beta: number): number | null {
    if (!beta || beta === 0) return null;
    return (returns - riskFreeRate) / beta;
  }

  calculateInformationRatio(portfolioReturn: number, benchmarkReturn: number, trackingError: number): number | null {
  ...
  calculateUpsideCaptureRatio(fundReturns: number[], benchmarkReturns: number[]): number | null {
    const upPeriods = benchmarkReturns.map((br, i) => br > 0 ? { fund: fundReturns[i], bench: br } : null).filter(Boolean);
    if (upPeriods.length === 0) return null;
    
    const fundUp = upPeriods.reduce((sum, p) => sum + (p?.fund || 0), 0) / upPeriods.length;
    const benchUp = upPeriods.reduce((sum, p) => sum + (p?.bench || 0), 0) / upPeriods.length;
    
    if (benchUp === 0) return null;
    return (fundUp / benchUp) * 100;
  }

  calculateDownsideCaptureRatio(fundReturns: number[], benchmarkReturns: number[]): number | null {
    const downPeriods = benchmarkReturns.map((br, i) => br < 0 ? { fund: fundReturns[i], bench: br } : null).filter(Boolean);
    if (downPeriods.length === 0) return null;
    
    const fundDown = downPeriods.reduce((sum, p) => sum + (p?.fund || 0), 0) / downPeriods.length;
    const benchDown = downPeriods.reduce((sum, p) => sum + (p?.bench || 0), 0) / downPeriods.length;
  ...
  calculateYieldToMaturity(
    ...
    const ytm = (couponPayment + (faceValue - currentPrice) / n) / ((faceValue + currentPrice) / 2);
  ...
  calculateMacaulayDuration(
    ...
    for (let t = 1; t <= n; t++) {
      const pv = couponPayment / Math.pow(1 + y, t);
      weightedSum += t * pv;
      priceSum += pv;
    }
  ...
  calculateModifiedDuration(macaulayDuration: number, ytm: number, couponFrequency: number = 2): number {
  ...
  calculateConvexity(
    ...
    return convexitySum / (priceSum * Math.pow(1 + y, 2) * couponFrequency * couponFrequency);
  }
  ...
  calculateFFO(netIncome: number, depreciation: number, gainOnSale: number = 0): number {
