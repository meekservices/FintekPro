...
class InsuranceSuitabilityService {
	...
	private assessmentCache: Map<string, InsuranceSuitabilityAssessment> =
		new Map();

	async conductSuitabilityAssessment(
		input: SuitabilityInput,
	): Promise<InsuranceSuitabilityAssessment> {
		const assessmentId = `ISA_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
		const now = new Date();
		const validUntil = new Date(
			now.getTime() + this.ASSESSMENT_VALIDITY_DAYS * 24 * 60 * 60 * 1000,
		);

		const suitabilityScore = this.calculateSuitabilityScore(input);
		const recommendedProducts = this.generateProductRecommendations(
			input,
			suitabilityScore,
		);
		const riskDisclosures = this.generateRiskDisclosures(
			input,
			recommendedProducts,
		);
	...
	private calculateSuitabilityScore(input: SuitabilityInput): number {
		...
		const incomeToExpenseRatio =
			input.financialProfile.annualIncome /
			12 /
			Math.max(input.financialProfile.monthlyExpenses, 1);
		if (incomeToExpenseRatio >= 2) score += 15;
		else if (incomeToExpenseRatio >= 1.5) score += 10;
		else if (incomeToExpenseRatio >= 1.2) score += 5;

		const existingCoverage = input.financialProfile.existingInsurance.reduce(
			(sum, ins) => sum + ins.sumAssured,
			0,
		);
		const coverageGap = input.insuranceNeeds.coverageAmount - existingCoverage;
		if (coverageGap > 0) score += 10;

		const premiumAffordabilityRatio =
			input.insuranceNeeds.premiumAffordability /
			(input.financialProfile.annualIncome / 12);
		if (premiumAffordabilityRatio >= 0.1 && premiumAffordabilityRatio <= 0.2)
			score += 10;
	...
	private generateProductRecommendations(
		input: SuitabilityInput,
		suitabilityScore: number,
	): InsuranceSuitabilityAssessment["recommendedProducts"] {
		...
		if ((goal === "life_protection" || hasDependents) && age < 60) {
			const coverageNeeded =
				input.insuranceNeeds.coverageAmount > 0
			...
			recommendations.push({
				...
				premiumEstimate: Math.round(
					coverageNeeded * 0.001 * (age < 35 ? 0.5 : age < 45 ? 0.8 : 1.2),
				),
			});
		}

		if (
			goal === "health_coverage" ||
			input.healthProfile.existingConditions.length === 0
		) {
			const healthCoverage = Math.max(
				500000,
				input.financialProfile.annualIncome * 0.5,
			);
			...
			recommendations.push({
				...
				reasoning:
					"Health insurance is essential protection against medical expenses. Family floater recommended for dependents.",
		...
		if (
			(goal === "wealth_creation" || goal === "retirement") &&
			input.insuranceNeeds.riskAppetite !== "conservative"
		) {
			...
			recommendations.push({
				...
				suitabilityRating:
					input.insuranceNeeds.riskAppetite === "aggressive"
						? "suitable"
						: "moderately_suitable",
				reasoning:
					"ULIPs combine insurance with market-linked returns. Suitable for long-term wealth creation with some risk appetite.",
		...
		if (goal === "child_education" && input.personalInfo.dependents > 0) {
			recommendations.push({
				...
				suitabilityRating: "highly_suitable",
				reasoning:
					"Child plans ensure education funding continues even in unfortunate circumstances.",
				coverageRecommendation: 2500000,
				premiumEstimate: Math.round(
					(2500000 * 0.05) / input.insuranceNeeds.preferredTenure,
				),
			});
		}
		...
		if (goal === "retirement" && age >= 40) {
			...
			recommendations.push({
				...
				reasoning:
					"Pension plans provide guaranteed income post-retirement for financial security.",
		...
		if (goal === "tax_saving") {
			recommendations.push({
				...
				reasoning:
					"Endowment plans offer tax benefits under Section 80C with guaranteed maturity benefits.",
	...
	private generateRiskDisclosures(
		input: SuitabilityInput,
		recommendations: InsuranceSuitabilityAssessment["recommendedProducts"],
	): string[] {
		...
		if (input.healthProfile.existingConditions.length > 0) {
			disclosures.push(
				"Pre-existing conditions may affect premium loading or policy terms.",
			);
		}
		...
		if (recommendations.some((r) => r.productType.includes("ULIP"))) {
			disclosures.push(
				"ULIPs are subject to market risks. Read offer document carefully before investing.",
			);
		...
		if (input.personalInfo.age >= 55) {
			disclosures.push(
				"Coverage options may be limited due to age. Medical underwriting may be stricter.",
			);
		}

		disclosures.push(
			`This assessment is valid for ${this.ASSESSMENT_VALIDITY_DAYS} days from the assessment date.`,
		);
		disclosures.push(
			"IRDAI does not involve in sale of any insurance product. Verify agent registration on IRDAI website.",
		);

		return disclosures;
	}

	async acknowledgeAssessment(
		assessmentId: string,
		clientId: string,
