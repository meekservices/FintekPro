...
export class DLMWorkflowService {
	...
	static async createDocument(data: {
		...
		if (data.content) {
			const contentHash = DLMWorkflowService.generateHash(data.content);
	...
	static async transitionStatus(data: {
		...
		if (!actionConfig.from.includes(currentStatus)) {
			throw new Error(
				`Action '${data.action}' not allowed from status '${currentStatus}'`,
			);
		}

		const newStatus = actionConfig.to;

		if (!DLMWorkflowService.isValidTransition(currentStatus, newStatus)) {
			throw new Error(
		...
		const [transition] = await db
			.insert(schema.documentWorkflowTransitions)
			.values({
				documentId: data.documentId,
	...
	static async createAuditEvent(data: {
		...
		const eventHash = DLMWorkflowService.generateHash(eventPayload);

		const [auditEvent] = await db
			.insert(schema.documentAuditEvents)
			.values({
				documentId: data.documentId,
	...
	static async getDocumentWithVersions(documentId: string) {
		...
		return {
			...
			versions,
			transitions,
			allowedActions: DLMWorkflowService.getAllowedActions(document.status),
		};
	}
	...
	static async listDocuments(filters: {
		...
		if (filters.entityType) {
			conditions.push(
				eq(schema.documents.entityType, filters.entityType as any),
			);
		}
		if (filters.status) {
		...
		if (filters.agreementType) {
			conditions.push(
				eq(schema.documents.agreementType, filters.agreementType as any),
			);
		}
	...
	static async createTrackedChange(data: {
		...
		const [change] = await db
			.insert(schema.documentTrackedChanges)
			.values({
				documentId: data.documentId,
				versionId: data.versionId,
	...
	static async resolveTrackedChange(data: {
		...
		const [change] = await db
			.update(schema.documentTrackedChanges)
			.set({
				status: data.status,
