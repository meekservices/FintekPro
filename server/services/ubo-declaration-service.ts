...
class UBODeclarationService {
	...
	createDeclaration(
		...
		console.log(
			`📋 [UBO] Created declaration ${declarationId} for entity ${entityName}`,
		);
	...
	addBeneficialOwner(
		...
		if (declaration.status !== "draft") {
			return {
				isValid: false,
				errors: ["Cannot modify submitted declaration"],
				warnings: [],
			};
		}

		// Validate ownership percentage
		const validation = this.validateOwnership(
			declaration,
		...
		if (
			declaration.beneficialOwners.some(
				(bo) => bo.panNumber === owner.panNumber,
			)
		) {
			return {
				isValid: false,
				errors: ["Beneficial owner with this PAN already exists"],
				warnings: [],
			};
		}
		...
		declaration.totalOwnershipDeclared =
			this.calculateTotalOwnership(declaration);
		...
		this.declarations.set(declarationId, declaration);
		console.log(
			`➕ [UBO] Added beneficial owner (${owner.ownershipPercentage}%) to declaration ${declarationId}`,
		);
	...
	removeBeneficialOwner(
		...
		const ownerIndex = declaration.beneficialOwners.findIndex(
			(bo) => bo.id === ownerId,
		);
	...
	declareNoUBO(
		...
		if (declaration.beneficialOwners.length > 0) {
			return {
				isValid: false,
				errors: [
					"Cannot declare no UBO when beneficial owners are already added. Remove all owners first.",
				],
				warnings: [],
			};
		}
	...
	reviewDeclaration(
		...
		this.declarations.set(declarationId, declaration);
		console.log(
			`📋 [UBO] Declaration ${declarationId} ${approved ? "approved" : "rejected"} by ${reviewedBy}`,
		);
	...
	private validateOwnership(
		...
		if (newPercentage < this.MIN_OWNERSHIP_THRESHOLD) {
			errors.push(
				`Beneficial owner must have at least ${this.MIN_OWNERSHIP_THRESHOLD}% ownership`,
			);
		}
		...
		if (newTotal > this.MAX_TOTAL_OWNERSHIP) {
			errors.push(
				`Total ownership would exceed 100% (current: ${declaration.totalOwnershipDeclared}%, adding: ${newPercentage}%)`,
			);
		}

		if (newTotal > 100 - this.MIN_OWNERSHIP_THRESHOLD && newTotal < 100) {
			warnings.push(
				"Remaining ownership is less than 25%. Consider adding all remaining owners.",
			);
		}
	...
	private validateDeclaration(
		...
		if (
			!declaration.noUBODeclaration &&
			declaration.beneficialOwners.length === 0
		) {
			errors.push(
				'At least one beneficial owner must be declared, or select "No UBO" option',
			);
		}
		...
		const unverifiedOwners = declaration.beneficialOwners.filter(
			(bo) => !bo.panVerified,
		);
		if (unverifiedOwners.length > 0) {
			warnings.push(
				`${unverifiedOwners.length} beneficial owner(s) have unverified PAN`,
			);
		}

		// Check for authorized signatory
		const hasSignatory = declaration.beneficialOwners.some(
			(bo) => bo.isAuthorizedSignatory,
		);
		if (!hasSignatory && declaration.beneficialOwners.length > 0) {
			warnings.push(
				"No authorized signatory designated among beneficial owners",
			);
		}
	...
	private calculateTotalOwnership(declaration: UBODeclaration): number {
		return declaration.beneficialOwners.reduce(
