...
export class BankerConfirmationService {
	...
	static getInstance(): BankerConfirmationService {
		if (!BankerConfirmationService.instance)
			BankerConfirmationService.instance = new BankerConfirmationService();
		return BankerConfirmationService.instance;
	}

	async triggerBankerConfirmation(
		claimId: string,
		...
		const [lead] = await db
			.select()
			.from(leadRegistry)
			.where(eq(leadRegistry.leadId, claim.leadId));
		if (!lead) return { success: false, error: "Lead not found" };

		if (!lead.bankerEmail) {
			return {
				success: false,
				error: "Banker email not set on lead. Set financier details first.",
			};
		}
    ...
    `;
		...
		console.log(
			`📧 Banker confirmation email ${sent ? "sent" : "simulated"} for claim ${claimId} to ${lead.bankerEmail}`,
		);
...
export const bankerConfirmationService =
	BankerConfirmationService.getInstance();
