...
class AmfiSubscriptionSyncService {
	...
	async sync(): Promise<SubscriptionSyncResult> {
		...
		try {
			...
			const overseas = allFunds.filter((f) =>
				regulatoryInvestabilityService.isOverseasFund({
			...
			console.log(
				`[SubscriptionSync] ${toSync.length} funds to sync (${overseas.length} overseas + ${legacyFunds.length} legacy restricted)`,
			);
			...
			for (let i = 0; i < toSync.length; i += BATCH_SIZE) {
				...
				for (const fund of batch) {
					try {
						...
						else if (status.subscriptionStatus === "DISCONTINUED")
							result.discontinued++;
						else if (status.subscriptionStatus === "RESTRICTED")
							result.restricted++;
						else result.open++;
		...
		try {
			...
			if (!resp.ok || resp.status === 404) {
				...
				return {
					...
					restrictionReason:
						"Fund not found on mfapi.in — likely wound up or merged",
				};
			}

			const json = (await resp.json()) as {
				meta?: { scheme_type?: string };
				data?: any[];
				status?: string;
			};
			...
			if (
				schemeType.includes("close ended") ||
				schemeType.includes("interval")
			) {
				return {
					lumpsumAllowed: false,
					sipAllowed: false,
			...
			if (!json.data || json.data.length === 0) {
				return {
					...
					subscriptionStatus: "DISCONTINUED",
					restrictionReason:
						"No NAV data returned from mfapi.in — fund may be discontinued",
				};
			}
			...
			if (!extLumpsumOk || !extSipOk) {
				return {
					...
					sipAllowed: extSipOk,
					subscriptionStatus:
						!extLumpsumOk && !extSipOk ? "CLOSED" : "RESTRICTED",
	...
	getLastRunInfo(): {
		lastRun: Date | null;
		lastResult: SubscriptionSyncResult | null;
		isRunning: boolean;
	} {
		return {
