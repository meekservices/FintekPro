...
const recommendationCache: Map<string, any[]> = new Map();
...
export async function getRecommendationsByCategory(
	...
	if (
		...
		recommendationCache.has(cacheKey)
	) {
	...
	try {
		const products = await db
			...
			.orderBy(
				desc(recommendationProducts.priority),
		...
		const transformed = products.map((p) => ({
			...
			ticker: p.symbol,
			symbol: p.symbol,
			minInvestment: p.minimumInvestment
				? Number.parseFloat(p.minimumInvestment)
				: undefined,
			requiresEnhancedKYC: p.requiresEnhancedKYC,
			dividendYield: p.dividendYield,
			currentPrice: p.currentPrice
				? Number.parseFloat(p.currentPrice)
				: undefined,
			peRatio: p.peRatio ? Number.parseFloat(p.peRatio) : undefined,
		...
		console.error(
			`[RecommendationProductsService] Error fetching ${productType}/${riskProfile}:`,
			error,
		);
		return [];
	}
...
export async function getAllActiveRecommendations(): Promise<
	Record<string, Record<string, RecommendationProductData[]>>
> {
	...
	try {
		...
		const grouped: Record<
			string,
			Record<string, RecommendationProductData[]>
		> = {};
		...
		console.error(
			"[RecommendationProductsService] Error fetching all active:",
			error,
		);
		return {};
	}
...
export async function hasRecommendationsInDatabase(
	...
	try {
		const products = await getRecommendationsByCategory(
			productType,
			riskProfile,
		);
...
export function generateRecommendationRationale(
	...
	const productTypeDesc =
		PRODUCT_TYPE_DESCRIPTIONS[product.productType] || product.productType;
	...
	switch (product.productType) {
		case "listed_stock":
			riskConsideration =
				"Subject to market volatility and company-specific risks. Diversification recommended.";
			break;
		case "unlisted_stock":
			riskConsideration =
				"Pre-IPO investments have limited liquidity and longer lock-in periods. Suitable for patient capital.";
			break;
		case "reit":
			riskConsideration =
				"Real estate market exposure with potential interest rate sensitivity. Dividend distributions may vary.";
			break;
		case "invit":
			riskConsideration =
				"Infrastructure assets with regulatory oversight. Returns linked to underlying project performance.";
			break;
		default:
			riskConsideration =
				"Standard market risks apply. Please review the product documentation.";
	}
