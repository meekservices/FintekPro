...
export interface ProposalPdfConfig {
  ...
  // Risk heat map data
  riskHeatMap?: {
    concentrationRisk: Array<{ sector: string; weight: number; risk: 'low' | 'medium' | 'high' }>;
...
export class RegulatorGradePdfRenderer {
  ...
  async generateProposal(config: ProposalPdfConfig): Promise<PdfGenerationResult> {
  ...
  private addFooter(): void {
    ...
    this.pdf.text(`Page ${this.pageNumber}`, this.pageWidth - this.margin - 15, this.pageHeight - 8);
    this.pdf.text('FintekPro Financial Services', this.margin, this.pageHeight - 8);
  }
  ...
  private formatGoalName(goal: string): string {
    ...
    return names[goal] || goal.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
  }
  ...
  private renderCoverPage(config: ProposalPdfConfig): void {
    ...
    const details = [
      ...
      ['Investment Goal:', this.formatGoalName(config.investmentGoals.primaryGoal)],
  ...
  private renderTableOfContents(): void {
    ...
    this.tocEntries.forEach((entry, index) => {
      ...
      this.pdf.text(`${entry.page}`, this.pageWidth - this.margin - 10, this.currentY);
      this.currentY += 8;
    });
  }
  ...
  private renderExecutiveSummary(config: ProposalPdfConfig): void {
    ...
    const snapshotData = [
      ...
      ['Equity Allocation', `${config.existingAllocation?.equity || 0}%`, `${config.proposedAllocation.equity}%`],
      ['Debt Allocation', `${config.existingAllocation?.debt || 0}%`, `${config.proposedAllocation.debt}%`],
      ['Gold Allocation', `${config.existingAllocation?.gold || 0}%`, `${config.proposedAllocation.gold}%`],
      ['Total Value', this.formatCurrency(config.existingAllocation?.totalValue || 0), this.formatCurrency(config.proposedAllocation.totalValue)],
      ['Risk Profile', this.formatRiskCategory(config.riskProfile.category), this.formatRiskCategory(config.riskProfile.category)],
    ];
  ...
  private renderPortfolioOverview(config: ProposalPdfConfig): void {
    ...
    this.pdf.text(`Data Source: CAS Statement / Manual Entry | Last NAV Date: ${new Date().toLocaleDateString('en-IN')}`, this.margin, this.currentY);
  }
  ...
  private renderProductRecommendations(config: ProposalPdfConfig): void {
    ...
    autoTable(this.pdf, {
      startY: this.currentY,
      head: [['Instrument', 'Category', 'Current Value', 'Verdict', 'Rationale']],
      body: verdictRows,
      theme: 'striped',
      headStyles: { fillColor: COLORS.primary },
      margin: { left: this.margin, right: this.margin },
      ...
      didParseCell: function(data: any) {
        if (data.column.index === 3 && data.section === 'body') {
          ...
          else if (verdict === 'SELL') data.cell.styles.textColor = COLORS.danger;
          else if (verdict === 'HOLD') data.cell.styles.textColor = COLORS.warning;
        }
      },
    });
    ...
    this.pdf.setFont('helvetica', 'italic');
    this.pdf.setFontSize(8);
    this.pdf.text('Note: All instruments must have BUY/HOLD/SELL verdict assigned. No blanks allowed.', this.margin, this.currentY);
  }
  ...
  private renderCapitalGainsSummary(config: ProposalPdfConfig): void {
    ...
    const gainsData = [
      ['Short-Term Capital Gains (STCG)', this.formatCurrency(config.capitalGains.stcgAmount)],
      ['Long-Term Capital Gains (LTCG)', this.formatCurrency(config.capitalGains.ltcgAmount)],
      ['Total Capital Gains', this.formatCurrency(config.capitalGains.totalGains)],
    ];
    ...
    // Tax disclosure
    this.pdf.setFillColor(...COLORS.lightGray);
    this.pdf.rect(this.margin, this.currentY - 3, this.pageWidth - 2 * this.margin, 15, 'F');
    this.pdf.setFont('helvetica', 'italic');
    this.pdf.setFontSize(9);
    this.pdf.text(
  ...
  private renderTaxImpactSummary(config: ProposalPdfConfig): void {
    ...
    const taxData = [
      ['Total Tax on Capital Gains', this.formatCurrency(config.taxImpact.totalTax)],
      ['Total Exit Load', this.formatCurrency(config.taxImpact.totalExitLoad)],
      ['Tax Offsets (Losses)', `- ${this.formatCurrency(config.taxImpact.taxOffsets)}`],
      ['Net Rebalancing Cost', this.formatCurrency(config.taxImpact.netRebalancingCost)],
    ];
    ...
    this.pdf.setFont('helvetica', 'italic');
    this.pdf.setFontSize(8);
    this.pdf.text('Actual tax liability may vary based on individual circumstances.', this.margin, this.currentY);
  }
  ...
  private renderRebalancingSipRecommendations(config: ProposalPdfConfig): void {
    ...
    this.pdf.setFont('helvetica', 'italic');
    this.pdf.setFontSize(8);
    this.pdf.text('Source: Rebalancing = from existing holdings | Fresh = new investment | Hybrid = combination', this.margin, this.currentY);
  }
  ...
  private renderPortfolioHealthScore(config: ProposalPdfConfig): void {
    ...
    if (!config.portfolioHealth) {
      this.pdf.text('No portfolio health data available.', this.margin, this.currentY);
      return;
    }

    const healthData = [
      ['Dimension', 'Existing', 'Proposed'],
      ['Risk Alignment', `${config.portfolioHealth.existing.riskAlignment}/100`, `${config.portfolioHealth.proposed.riskAlignment}/100`],
      ['Diversification', `${config.portfolioHealth.existing.diversification}/100`, `${config.portfolioHealth.proposed.diversification}/100`],
      ['Cost Efficiency', `${config.portfolioHealth.existing.costEfficiency}/100`, `${config.portfolioHealth.proposed.costEfficiency}/100`],
      ['Overall Score', `${config.portfolioHealth.existing.overallScore}/100`, `${config.portfolioHealth.proposed.overallScore}/100`],
    ];
  ...
  private renderExpenseRatioAnalysis(config: ProposalPdfConfig): void {
    ...
    if (!config.expenseAnalysis) {
      this.pdf.text('No expense analysis data available.', this.margin, this.currentY);
      return;
    }

    const expenseData = [
      ['Weighted TER (Existing)', `${config.expenseAnalysis.existingWeightedTER.toFixed(2)}%`],
      ['Weighted TER (Proposed)', `${config.expenseAnalysis.proposedWeightedTER.toFixed(2)}%`],
      ['Cost Drag Impact over Horizon', this.formatCurrency(config.expenseAnalysis.costDragImpact)],
    ];
  ...
  private renderRiskHeatMap(config: ProposalPdfConfig): void {
    ...
    if (!config.riskHeatMap) {
      this.pdf.text('No risk heat map data available.', this.margin, this.currentY);
      return;
    }
    ...
    autoTable(this.pdf, {
      ...
      didParseCell: function(data: any) {
        if (data.column.index === 2 && data.section === 'body') {
          ...
          else if (risk === 'MEDIUM') data.cell.styles.textColor = COLORS.warning;
  ...
  private renderBenchmarkComparison(config: ProposalPdfConfig): void {
    ...
    if (!config.benchmarkComparison) {
      this.pdf.text('No benchmark comparison data available.', this.margin, this.currentY);
      return;
    }

    this.pdf.setFont('helvetica', 'normal');
    this.pdf.setFontSize(10);
    this.pdf.text(`Benchmark: ${config.benchmarkComparison.benchmarkName} (${config.benchmarkComparison.benchmarkCode})`, this.margin, this.currentY);
    this.currentY += 10;

    const comparisonData = [
      ['Portfolio', 'vs Benchmark'],
      ['Existing Portfolio', `${config.benchmarkComparison.existingVsBenchmark >= 0 ? '+' : ''}${config.benchmarkComparison.existingVsBenchmark.toFixed(2)}%`],
      ['Proposed Portfolio', `${config.benchmarkComparison.proposedVsBenchmark >= 0 ? '+' : ''}${config.benchmarkComparison.proposedVsBenchmark.toFixed(2)}%`],
    ];
    ...
    if (config.benchmarkComparison.rationale) {
      this.pdf.setFont('helvetica', 'italic');
      this.pdf.setFontSize(9);
      const lines = this.pdf.splitTextToSize(`Selection Rationale: ${config.benchmarkComparison.rationale}`, this.pageWidth - 2 * this.margin);
  ...
  private renderWhatIfScenarios(config: ProposalPdfConfig): void {
    ...
    if (!config.whatIfScenarios?.length) {
      this.pdf.text('No what-if scenarios available.', this.margin, this.currentY);
      return;
    }
    ...
    // Disclosure
    this.pdf.setFillColor(...COLORS.lightGray);
    this.pdf.rect(this.margin, this.currentY - 3, this.pageWidth - 2 * this.margin, 12, 'F');
    this.pdf.setFont('helvetica', 'italic');
    this.pdf.setFontSize(8);
    this.pdf.text('Scenario analysis is hypothetical and for illustration only. Actual results may vary.', this.margin + 5, this.currentY + 3);
  }
  ...
  private renderDividendProjection(config: ProposalPdfConfig): void {
    ...
    if (!config.dividendProjection) {
      this.pdf.text('No dividend projection data available.', this.margin, this.currentY);
      return;
    }

    const dividendData = [
      ['Metric', 'Existing', 'Proposed'],
      ['Annual Dividend Income', this.formatCurrency(config.dividendProjection.existingAnnual), this.formatCurrency(config.dividendProjection.proposedAnnual)],
      ['Yield Assumption', `${config.dividendProjection.yieldAssumption}%`, `${config.dividendProjection.yieldAssumption}%`],
    ];
    ...
    autoTable(this.pdf, {
      ...
      didParseCell: function(data: any) {
        if (data.column.index === 3 && data.section === 'body') {
          ...
          else if (urgency === 'MEDIUM') data.cell.styles.textColor = COLORS.warning;
          else data.cell.styles.textColor = COLORS.success;
        }
      },
    });
  ...
  private renderPortfolioGrowthProjection(config: ProposalPdfConfig): void {
    ...
    if (!config.growthProjection) {
      this.pdf.text('No growth projection data available.', this.margin, this.currentY);
      return;
    }

    const projectionData = [
      ['Expected CAGR', `${config.growthProjection.cagr.toFixed(1)}%`],
      ['Goal Probability', `${config.growthProjection.goalProbability.toFixed(0)}%`],
      ['Time to Goal', config.growthProjection.timeToGoal],
      ['Projected Value at Goal', this.formatCurrency(config.growthProjection.projectedValue)],
    ];
  ...
  private renderMandatoryDisclaimers(): void {
    ...
    const disclaimers = [
      {
        ...
        content: 'Investments in securities market are subject to market risks. Read all the related documents carefully before investing. The NAV of mutual fund schemes may go up or down depending upon the factors and forces affecting the securities market.',
      },
      {
        title: 'No Assurance of Returns',
        content: 'Past performance is not indicative of future results. There is no assurance or guarantee that the objectives of the scheme will be achieved. The value of investments may fluctuate based on market conditions.',
      },
      {
        title: 'Advisory Responsibility',
        content: 'This proposal is for informational purposes only and does not constitute investment advice. Investment decisions should be made based on individual circumstances, risk tolerance, and financial goals after consulting with a qualified financial advisor.',
      },
      {
        title: 'SEBI/AMFI Compliance',
        content: 'FintekPro is a SEBI registered investment advisor. All recommendations are made in compliance with SEBI (Investment Advisers) Regulations, 2013 and AMFI guidelines. Mutual fund investments are subject to market risks.',
      },
    ];
    ...
    disclaimers.forEach(d => {
      ...
      this.pdf.setFont('helvetica', 'normal');
      this.pdf.setFontSize(9);
      const lines = this.pdf.splitTextToSize(d.content, this.pageWidth - 2 * this.margin);
      this.pdf.text(lines, this.margin, this.currentY);
  ...
  private renderAdvisorDeclaration(config: ProposalPdfConfig): void {
    ...
    this.pdf.setFont('helvetica', 'normal');
    this.pdf.setFontSize(9);
    const advisorText = 'The recommendations contained in this proposal are suitable based on the information provided by the client. I have conducted proper due diligence and this advice is in the best interest of the client.';
    const advisorLines = this.pdf.splitTextToSize(advisorText, this.pageWidth - 2 * this.margin);
    ...
    this.currentY += 5;
    this.pdf.line(this.margin, this.currentY + 15, this.margin + 60, this.currentY + 15);
    this.pdf.text(`Name: ${config.advisor.name}`, this.margin, this.currentY + 22);
    this.pdf.text(`ARN/RIA: ${config.advisor.arnId || config.advisor.riaId || 'N/A'}`, this.margin, this.currentY + 28);
    ...
    this.pdf.line(clientX, this.currentY + 15, clientX + 60, this.currentY + 15);
    this.pdf.text(`Name: ${config.client.name}`, clientX, this.currentY + 22);
    this.pdf.text(`PAN: ${this.maskPan(config.client.pan)}`, clientX, this.currentY + 28);
...
export async function generateRegulatorGradePdf(config: ProposalPdfConfig): Promise<PdfGenerationResult> {
  const renderer = new RegulatorGradePdfRenderer(config.settings?.orientation || 'portrait');
