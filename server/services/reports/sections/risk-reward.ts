...
export function computeRiskReward(
  ...
  const cumulativeReturn = monthlyReturns.reduce((acc, r) => acc * (1 + r / 100), 1) - 1;
  const annualizedReturn = (Math.pow(1 + cumulativeReturn, 12 / monthlyReturns.length) - 1) * 100;

  const avgMonthlyReturn = monthlyReturns.reduce((a, b) => a + b, 0) / monthlyReturns.length;
  const variance = monthlyReturns.reduce((sum, r) => sum + Math.pow(r - avgMonthlyReturn, 2), 0) / monthlyReturns.length;
  ...
  const yearlyReturns = Array.from(yearlyMap.entries())
    .map(([year, returns]) => ({
      year,
      return: returns.reduce((a, b) => (1 + a / 100) * (1 + b / 100) - 1, 0) * 100,
    }))
    .sort((a, b) => a.year - b.year);

  const yearReturnsOnly = yearlyReturns.map(y => y.return);
  const bestYear = yearReturnsOnly.length > 0 ? Math.max(...yearReturnsOnly) : 0;
  const worstYear = yearReturnsOnly.length > 0 ? Math.min(...yearReturnsOnly) : 0;
  const averageAnnual = yearReturnsOnly.length > 0 
...
function getDefaultRiskReward(years: number, startDate: Date, endDate: Date): RiskRewardData {
  return {
    period: {
      years,
      startDate: startDate.toISOString().split('T')[0],
