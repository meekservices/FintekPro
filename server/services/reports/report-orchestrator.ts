...
class ReportOrchestratorService {
	...
	async getPortfolioData(portfolioId: string): Promise<PortfolioData | null> {
		try {
			...
			console.error(
				"[ReportOrchestrator] Error fetching portfolio data:",
				error,
			);
			return null;
		}
	...
	async runPreFlightValidation(
		...
		if (portfolioData.holdings.length === 0) {
			result.warnings.push(
				"Portfolio has no holdings. Some sections may be empty.",
			);
		}
		...
		if (sections.riskReward?.enabled) {
			...
			result.sections.push({
				...
				reason: !valid
					? `Insufficient history for ${sections.riskReward.years}-year analysis`
					: undefined,
			});
			if (!valid) {
				result.warnings.push(
					`Risk/Reward section disabled: insufficient history (${Math.floor(historyMonths / 12)} years available, ${sections.riskReward.years} years required)`,
				);
			}
		}
		...
		if (sections.scatterPlot?.enabled) {
			...
			result.sections.push({
				...
				actualDataAvailable: `${Math.floor(historyMonths / 12)} years ${historyMonths % 12} months`,
				reason: !valid
					? `Insufficient history for ${sections.scatterPlot.years}-year scatterplot`
					: undefined,
			});
		}
		...
		if (sections.rollingReturns?.enabled) {
			...
			result.sections.push({
				...
				reason: !valid
					? `Insufficient history for ${sections.rollingReturns.months}-month rolling returns`
					: undefined,
			});
		}

		if (sections.stockIntersection) {
			const equityHoldings = portfolioData.holdings.filter(
				(h) => h.assetType === "equity",
			);
			result.sections.push({
				...
				reason:
					equityHoldings.length < 2
						? "Need at least 2 equity holdings for intersection analysis"
						: undefined,
			});
		}
		...
		if (sections.correlationMatrix) {
			result.sections.push({
				...
				dataAvailable: historyMonths >= 12,
				reason:
					portfolioData.holdings.length < 2
						? "Need at least 2 holdings for correlation"
						: historyMonths < 12
		...
		if (sections.investmentGrowth) {
			result.sections.push({
				...
				reason:
					portfolioData.snapshots.length < 2
						? "Need at least 2 snapshots for growth chart"
						: undefined,
			});
		}
		...
		if (sections.targetAssetAllocation) {
			...
			result.sections.push({
				...
				reason: !hasModel
					? "No allocation model selected - section disabled"
					: undefined,
			});
			if (!hasModel) {
				result.warnings.push(
					"Target Asset Allocation disabled: no allocation model selected",
				);
			}
		}
		...
		if (sections.priceDistribution) {
			result.sections.push({
				...
				reason:
					historyMonths < 12
						? "Need at least 12 months history for price distribution"
						: undefined,
			});
		}
	...
	async saveTemplate(
		...
		const [template] = await db
			.insert(portfolioReportTemplates)
			.values({
				id: nanoid(),
				name,
	...
	async getTemplate(
		templateId: string,
	): Promise<PortfolioReportTemplate | null> {
		const [template] = await db
			.select()
			.from(portfolioReportTemplates)
	...
	async deleteTemplate(templateId: string, userId: string): Promise<boolean> {
		const result = await db
			.delete(portfolioReportTemplates)
			.where(
				and(
	...
	async createGeneratedReport(
		...
		const [report] = await db
			.insert(portfolioGeneratedReports)
			.values({
				id: nanoid(),
				clientId,
	...
	async updateReportStatus(
		...
		await db
			.update(portfolioGeneratedReports)
			.set({
				status,
	...
	async getGeneratedReport(
		reportId: string,
	): Promise<PortfolioGeneratedReport | null> {
		const [report] = await db
			.select()
			.from(portfolioGeneratedReports)
			.where(eq(portfolioGeneratedReports.id, reportId))
	...
	async attachToProposal(
		...
		await this.logAudit(reportId, "attached", userId, {
			metadata: { proposalId },
		});
	}
