...
export class PortfolioReportPDFRenderer {
	...
	private async tryRenderFactorAttribution(
		portfolioData: PortfolioData,
	): Promise<void> {
		try {
			const mfHoldings = portfolioData.holdings
				...
				.sort(
					(a, b) =>
						Number(b.quantity ?? 0) * Number(b.avgPrice ?? 0) -
						Number(a.quantity ?? 0) * Number(a.avgPrice ?? 0),
				)
				.slice(0, 5);

			if (mfHoldings.length === 0) return;

			const factorResult = await callPython<any>(
				"/api/factor/batch-fund-factors",
				"POST",
				{
					funds: mfHoldings.map((h) => ({
			...
			if (!factorResult || factorResult.error || !factorResult.results?.length)
				return;
	...
	private addFooter(): void {
		...
		this.pdf.setFontSize(7);
		this.pdf.setTextColor(...COLORS.secondary);
		this.pdf.text(footer, this.margin, this.pageHeight - 10, {
	...
	private renderCoverPage(
		...
		this.pdf.text(
			config.coverPage?.title || "Portfolio Report",
			this.margin,
			40,
		);
		...
		const details = [
			[
				"Client Name:",
				config.coverPage?.clientName || portfolioData.client?.fullName || "N/A",
			],
			["Prepared By:", config.coverPage?.preparedBy || "FintekPro Advisor"],
			[
				"Report Date:",
				config.coverPage?.date || reportDate.toLocaleDateString("en-IN"),
			],
	...
	private renderPortfolioXray(data: any): void {
		...
		this.pdf.setTextColor(...COLORS.text);
		this.pdf.text(
			`Total Portfolio Value: ${this.formatCurrency(data.totalValue)}`,
			this.margin,
			this.currentY,
		);
		this.currentY += 8;
	...
	private renderPortfolioSnapshot(portfolioData: PortfolioData): void {
		...
		const data = [
			[
				"Total Value",
				this.formatCurrency(
					Number(snapshot?.totalValue || portfolio.totalValue || 0),
				),
			],
			[
				"Equity Value",
				this.formatCurrency(Number(snapshot?.totalEquityValue || 0)),
			],
			[
				"Debt Value",
				this.formatCurrency(Number(snapshot?.totalDebtValue || 0)),
			],
			[
				"Mutual Fund Value",
				this.formatCurrency(Number(snapshot?.totalMutualFundValue || 0)),
			],
	...
	private renderRiskReward(data: any): void {
		this.renderSectionHeader(
			`Risk/Reward Analysis (${data.period.years} Years)`,
		);
	...
	private renderRollingReturns(data: any): void {
		...
		const statsData = [
			[
				...
				`${data.statistics.averageRolling.toFixed(2)}%`,
			],
	...
	private renderCorrelationMatrix(data: any): void {
		...
		if (data.symbols.length < 2) {
			this.pdf.setFontSize(this.fontSize);
			this.pdf.text(
				"Insufficient holdings for correlation analysis (minimum 2 required)",
				this.margin,
				this.currentY,
			);
			return;
		...
		this.pdf.setFont("helvetica", "bold");
		this.pdf.text(
			`Diversification Score: ${data.insights.diversificationScore}/100`,
			this.margin,
			this.currentY,
		);
	}
