...
class ErrorSpikeDetectionService {
  ...
  async checkForSpike(errorCode: string, module: string): Promise<SpikeCheckResult> {
    const thresholdConfig = await errorWebhookService.getThresholdConfig(module, errorCode);
  ...
  async handleErrorIngested(errorEntry: {
    ...
    if (spikeResult.isSpike) {
      ...
      if (lastAlertTime && Date.now() - lastAlertTime < this.ALERT_COOLDOWN_MS) {
        console.log(`[SpikeDetection] Spike alert for ${alertKey} still in cooldown`);
        return;
      }
  ...
  private async sendSpikeAlert(
    ...
    try {
      ...
      console.log(`[SpikeDetection] Spike alert sent for ${errorEntry.errorCode}`);
    } catch (error) {
  ...
  private async escalateErrorSeverity(errorId: string): Promise<void> {
    try {
      ...
      console.error('[SpikeDetection] Failed to escalate error severity:', error);
    }
  }

  private async getRecentErrorIds(errorCode: string, windowMinutes: number): Promise<string[]> {
    try {
      const windowStart = new Date(Date.now() - windowMinutes * 60 * 1000);
      
      const errors = await db.select({ id: errorLedger.id })
        .from(errorLedger)
        .where(and(
          eq(errorLedger.errorCode, errorCode),
          gte(errorLedger.lastOccurrence, windowStart)
  ...
  async getModuleSpikeSummary(): Promise<Array<{
    module: string;
    totalErrors: number;
    spikeCount: number;
    ...
    try {
      ...
      console.error('[SpikeDetection] Failed to get module spike summary:', error);
      return [];
    }
  }
