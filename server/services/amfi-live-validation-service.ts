...
class AmfiLiveValidationService {
	...
	async validateArn(arnCode: string): Promise<LiveArnResult> {
		...
		try {
			...
			logger.warn(
				"[AmfiLive] Local DB ARN lookup failed, trying API fallback",
				{ arnCode, err },
			);
		}
		...
		if (process.env.KFINTECH_API_KEY) {
			try {
				...
				logger.warn("[AmfiLive] KFintech ARN API fallback failed", {
					arnCode,
					err,
				});
			}
		...
		logger.warn(
			"[AmfiLive] ARN not found in DB or API — format-only validation used",
		...
		return {
			...
			source: "format_only",
			warning:
				"ARN could not be confirmed against AMFI registry. Manual verification required before agent activation.",
		};
	}
	...
	async validateEuin(
		...
		try {
			...
			logger.warn("[AmfiLive] Local DB EUIN lookup failed", {
				euinNumber,
				err,
			});
		}
	...
	async syncAmfiDistributors(): Promise<{ synced: number; errors: number }> {
		const AMFI_API =
			process.env.AMFI_DISTRIBUTOR_API_URL ||
		...
		logger.info("[AmfiSync] Starting AMFI distributor registry sync via API", {
			endpoint: AMFI_API,
		});
			...
			try {
				const resp = await axios.get(AMFI_API, {
					...
					headers: {
						...
						"User-Agent":
							"FintekPro-Compliance-Sync/1.0 (regulatory-arn-validation)",
					},
				});
				...
				totalPages =
					meta?.pageCount ??
					Math.ceil((meta?.total ?? records.length) / PAGE_SIZE);
				...
				for (const rec of records) {
					...
					const expiryDate = rec.ARNValidTill
						? new Date(rec.ARNValidTill)
						: null;
					...
					try {
						await db
							.insert(schema.amfiDistributors)
							.values({
								arnCode,
								...
								registrationDate: rec.ARNValidFrom
									? new Date(rec.ARNValidFrom)
									: null,
				...
				logger.info("[AmfiSync] Page synced", {
					page,
					totalPages,
					synced,
					errors,
				...
				logger.error("[AmfiSync] Failed to fetch page from AMFI API", {
					page,
					error: fetchErr,
				});
				errors++;
		...
		logger.info("[AmfiSync] AMFI distributor sync complete", {
			synced,
			errors,
		});
	...
	private async lookupArnInDb(arnCode: string): Promise<LiveArnResult | null> {
		...
		const isExpired = record.arnExpiryDate
			? new Date() > record.arnExpiryDate
			: false;
	 ...
	 */
	private async lookupArnViaAmfiApi(
		arnCode: string,
	): Promise<LiveArnResult | null> {
		const AMFI_API =
			process.env.AMFI_DISTRIBUTOR_API_URL ||
			"https://www.amfiindia.com/api/distributor-agent";
		...
		try {
			...
			const expiryDate = match.ARNValidTill
				? new Date(match.ARNValidTill)
				: null;
			const isExpired = expiryDate ? new Date() > expiryDate : false;
			const effectiveStatus: LiveArnResult["status"] = isExpired
				? "lapsed"
				: "active";
			...
			logger.warn("[AmfiLive] AMFI API lookup failed (slow server?)", {
				arnCode,
				err,
			});
			return null;
	 ...
	 */
	private async lookupArnViaKfintech(
		arnCode: string,
	): Promise<LiveArnResult | null> {
		const baseUrl =
			process.env.KFINTECH_BASE_URL || "https://mfapi.kfintech.com";
