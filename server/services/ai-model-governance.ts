...
class AIModelGovernance {
	async runGovernanceCheck(
		...
		for (const model of activeModels) {
			try {
				const report = await this.checkModelHealth(
					model.modelName,
					model.modelVersion,
					cfg,
				);
		...
		const criticalModels = reports.filter(
			(r) => r.status === "critical",
		).length;
	...
	async checkModelHealth(
		...
		const performanceDecay = await this.getPerformanceDecay(
			modelName,
			modelVersion,
		);

		const trainedAt =
			params?.trainingMetrics?.trainedAt ||
			model.createdAt?.toISOString() ||
			new Date().toISOString();
		const daysSinceTraining = Math.floor(
			(Date.now() - new Date(trainedAt).getTime()) / (1000 * 60 * 60 * 24),
		);

		const trainingSharpe =
			perfMetrics?.sharpeRatio ?? perfMetrics?.cv?.avgDirectionalAccuracy ?? 0;
		const trainingDA =
			perfMetrics?.directionalAccuracy ??
			params?.trainingMetrics?.directionalAccuracy ??
			0;
		const trainingRmse =
			perfMetrics?.rmse ?? params?.trainingMetrics?.rmse ?? 0;
		...
		const sharpeDecay =
			trainingSharpe !== 0
				? Math.max(
						0,
						(trainingSharpe - recentSharpe) / Math.abs(trainingSharpe),
					)
				: 0;

		const accuracyDecay =
			trainingDA !== 0 ? Math.max(0, (trainingDA - recentDA) / trainingDA) : 0;
		...
		if (daysSinceTraining > cfg.staleModelDays! && predictionCount === 0) {
			...
			overallDriftScore > cfg.psiThreshold! ||
			sharpeDecay > cfg.sharpeDecayThreshold!
		) {
			status = "critical";
		} else if (
	...
	async detectFeatureDrift(
		...
		const featureNames: string[] =
			params.featureNames || Object.keys(params.featureMeans);
		...
		for (const fname of featureNames) {
			...
			const liveStd =
				liveValues.length > 1 ? ss.standardDeviation(liveValues) : 0;

			const syntheticTraining: number[] = [];
			for (let i = 0; i < Math.max(liveValues.length, 100); i++) {
				syntheticTraining.push(
					trainingMean + (Math.random() - 0.5) * 2 * trainingStd,
				);
			}

			const psiScore = this.computePSI(syntheticTraining, liveValues);
			const ksStatistic = this.computeKSStatistic(
				syntheticTraining,
				liveValues,
			);
			...
			const isDrifted =
				psiScore > cfg.psiThreshold! || ksStatistic > cfg.ksThreshold!;
	...
	async getPerformanceDecay(
		...
		try {
			...
			const predictions = recentLogs.map((l) =>
				Number.parseFloat(l.predictedReturn || "0"),
			);
			const actuals = recentLogs.map((l) =>
				Number.parseFloat(l.actualReturn || "0"),
			);
			...
			const stdReturn =
				dailyReturns.length > 1 ? ss.standardDeviation(dailyReturns) : 1;
			const recentSharpe =
			...
			return {
				...
				recentDirectionalAccuracy:
					Math.round(recentDirectionalAccuracy * 10000) / 10000,
	...
	async rollbackModel(
		...
		try {
			...
			if (currentModels.length === 0) {
				return {
					success: false,
					error: `No active model found for ${assetClass}`,
				};
			}
			...
			if (targetVersion) {
				...
				if (targetRows.length === 0) {
					return {
						success: false,
						error: `Target version ${targetVersion} not found for ${assetClass}`,
					};
				}
				...
				if (previousModels.length === 0) {
					return {
						success: false,
						error: `No previous model version found for ${assetClass}`,
					};
				}
	...
	async updatePredictionOutcomes(): Promise<{ updated: number }> {
		try {
			...
			for (const log of pendingLogs) {
				...
				const pick = picks[0];
				const actualReturn = pick.returnPct
					? Number.parseFloat(pick.returnPct) / 100
					: null;

				if (actualReturn === null) continue;

				const predictedReturn = Number.parseFloat(log.predictedReturn || "0");
	...
	private generateRecommendations(
		report: Partial<ModelHealthReport>,
	): string[] {
		...
		if (status === "critical") {
			recommendations.push(
				"CRITICAL: Immediate retraining recommended to restore model performance.",
			);
		} else if (status === "stale") {
			recommendations.push(
				"Model is stale. Consider retraining or deactivating if no longer needed.",
			);
		}

		if (recommendations.length === 0) {
			recommendations.push(
				"Model is performing within expected parameters. No action needed.",
			);
		}
	...
	private resolveConfig(config?: GovernanceConfig): Required<GovernanceConfig> {
		return {
			...
			sharpeDecayThreshold: config?.sharpeDecayThreshold ?? 0.3,
