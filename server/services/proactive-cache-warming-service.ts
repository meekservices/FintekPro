...
class ProactiveCacheWarmingService {
	...
	start(): void {
		...
		console.log(
			`✅ Proactive Cache Warming Service started (interval: ${this.config.checkIntervalMs / 1000 / 60} min)`,
		);
	...
	getPopularItems(
		...
		const items = Array.from(this.accessPatterns.values())
			.filter(
				(p) => p.type === type && p.accessCount >= this.config.minAccessCount,
			)
	...
	async runWarmingCycle(): Promise<{ stocks: number }> {
		...
		try {
			const popularStocks = [
				...this.config.popularStocks,
				...this.getPopularItems("stock", 10),
			];
			const uniqueStocks = [...new Set(popularStocks)];

			if (uniqueStocks.length > 0) {
				console.log(
					`[CacheWarming] Warming ${uniqueStocks.length} popular stocks...`,
				);
