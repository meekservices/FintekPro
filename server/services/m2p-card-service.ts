...
export const CREDIT_CARD_CATALOG: M2PCardProduct[] = [
	...
	},
	{
		...
		features: [
			"8 complimentary lounge visits",
			"Milestone benefits",
			"Fuel surcharge waiver",
			"Movie ticket offers",
		],
		benefits: [
			"Annual vouchers worth ₹5,000",
			"Dining privileges",
			"Golf privileges",
		],
	...
	},
	{
		...
		joiningFee: 0,
		rewardRate:
			"5% on Amazon for Prime, 3% on Amazon for non-Prime, 2% on partner merchants",
		features: [
			"Lifetime free",
			"Instant cashback",
			"No reward point redemption needed",
		],
		benefits: [
			"5% on Amazon.in (Prime)",
			"Amazon Pay balance credited directly",
		],
	...
	},
	{
		...
		rewardRate:
			"5% cashback on Google Pay, 4% on utilities/bills, 2% on all spends",
		features: [
			"Fuel surcharge waiver",
	...
	},
	{
		...
		features: [
			...
			"Welcome benefits ₹2,500",
			"Golf privileges",
			"Dining rewards",
		],
		benefits: [
			"SmartBuy 20X accelerated rewards",
			"Golf round per quarter",
	...
	},
	{
		...
		features: [
			"Unlimited airport lounge",
			"4 international lounge visits",
			"Concierge",
			"Golf benefits",
		],
		benefits: [
			"Travel insurance ₹3Cr",
			"Yes PayNow",
			"Milestone rewards ₹15,000/year",
		],
...
export class M2PCardService {
	...
	private async request<T>(
		...
		if (!this.isConfigured()) {
			throw new Error(
				"M2P credentials not configured. Set M2P_BASE_URL, M2P_API_KEY, M2P_SECRET_KEY.",
			);
		}
		...
		if (!resp.ok) {
			const msg =
				data?.message || data?.error || data?.errorMessage || resp.statusText;
	...
	async checkEligibility(
		req: M2PEligibilityRequest,
	): Promise<M2PEligibilityResult> {
		if (!this.isConfigured()) {
			...
			const recommended = CREDIT_CARD_CATALOG.filter(
				(c) =>
					req.annualIncome >= c.eligibility.minIncome &&
					score >= c.eligibility.minCreditScore,
			);
			return {
				eligible: recommended.length > 0,
				preApprovedLimit:
					recommended.length > 0 ? Math.min(req.annualIncome / 6, 500000) : 0,
	 ...
	 */
	async applyForCard(
		req: M2PCardApplicationRequest,
	): Promise<M2PCardApplication> {
	...
	async issueCard(
		...
		return this.request("POST", "/card/issue", {
	...
	async replaceCard(
		cardId: string,
		reason: "lost" | "damaged" | "expired",
	): Promise<{ newCardId: string; estimatedDelivery: string }> {
	...
	async listCards(customerId: string): Promise<M2PCard[]> {
		return this.request<{ cards: M2PCard[] }>(
			"GET",
	...
	async updateCreditLimit(
		...
		return this.request("PUT", `/card/${cardId}/limit`, {
			limit: newLimit,
			reason,
		});
	}
	...
	async listTransactions(
		...
		return this.request(
			"GET",
			`/card/${cardId}/transactions${qs ? "?" + qs : ""}`,
		);
	}
	...
	async disputeTransaction(
		...
		return this.request("POST", "/transaction/dispute", {
			card_id: cardId,
	...
	async generateStatement(
		...
		return this.request("POST", `/card/${cardId}/statement/generate`, {
	...
	async listStatements(cardId: string): Promise<M2PStatement[]> {
		return this.request<{ statements: M2PStatement[] }>(
			"GET",
			`/card/${cardId}/statements`,
		).then((r) => r.statements);
	}
