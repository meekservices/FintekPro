...
class GiftCityMaintenanceService {
  ...
  async runMaintenance(): Promise<MaintenanceResult> {
    ...
    try {
      ...
      for (const product of allProducts) {
        ...
        if (!product.minimumInvestment || parseFloat(product.minimumInvestment) <= 0) {
        ...
        if (!product.regulatoryBenefits || product.regulatoryBenefits.length === 0) {
          productIssues.push(`No regulatory benefits defined for ${product.name}`);
        }

        if (product.flowDirection === "outbound" && !product.lrsCategory && product.lrsApplicable) {
          productIssues.push(`LRS category missing for LRS-applicable product ${product.name}`);
        }
      ...
      console.log(`[GIFT City Maintenance] Completed: ${validatedCount}/${allProducts.length} products healthy, ${updatedCount} refreshed`);
  ...
  async refreshProductPricing(): Promise<{ updated: number; skipped: number }> {
    ...
    for (const product of products) {
      try {
        ...
        console.error(`[GIFT City Maintenance] Failed to refresh ${product.name}:`, error);
        skipped++;
      }
    }

    console.log(`[GIFT City Maintenance] Pricing refresh complete: ${updated} updated, ${skipped} skipped`);
  ...
  async cleanupDuplicates(): Promise<{ removed: number }> {
    ...
    if (duplicateIds.length > 0) {
      ...
      console.log(`[GIFT City Maintenance] Removed ${duplicateIds.length} duplicate products`);
    }
  ...
  async getMaintenanceStats(): Promise<{
    ...
    const nextScheduled = this.lastMaintenanceAt 
      ? new Date(this.lastMaintenanceAt.getTime() + this.refreshIntervalHours * 60 * 60 * 1000)
      : null;
  ...
  setRefreshInterval(hours: number): void {
    ...
    console.log(`[GIFT City Maintenance] Refresh interval set to ${hours} hours`);
  }
