...
class PlatformStatsCache {
  ...
  private async fetchStats(): Promise<PlatformStats> {
    ...
    return {
      ...
      portfolioValue: this.formatCurrency(Number(stats.total_portfolio_value) || 0),
      portfolioValueRaw: Number(stats.total_portfolio_value) || 0,
      avgPortfolioValue: this.formatCurrency(Number(stats.avg_portfolio_value) || 0),
      ...
      mutualFundsCount: this.formatNumber(Number(stats.mutual_funds_count) || 0),
  ...
  private async refreshCache(): Promise<void> {
    if (this.refreshLock) {
      console.log('⏳ [PlatformStatsCache] Refresh already in progress, skipping');
      return;
    }
    ...
    try {
      ...
      console.log(`✅ [PlatformStatsCache] Cache refreshed in ${this.metrics.lastRefreshDuration}ms`);
    } catch (error) {
      this.metrics.errors++;
  ...
  async getStats(): Promise<{ data: PlatformStats; cached: boolean; cacheAge: number }> {
    const now = Date.now();
    ...
    if (this.cache) {
      ...
      if (cacheAge < STALE_TTL_MS) {
        this.metrics.hits++;
        console.log(`📦 [PlatformStatsCache] Serving stale cache (age: ${Math.round(cacheAge / 1000)}s), refreshing in background`);
