...
class PaymentWebhookRetryService {
	...
	private async loadPendingRetriesFromDb(): Promise<void> {
		try {
			...
			console.log(
				`[WebhookRetry] Found ${ordersWithPendingRetries.length} orders with pending webhook retries`,
			);

			// These will be retried in the next retry loop cycle
		} catch (error) {
			console.error(
				"[WebhookRetry] Failed to load pending retries from database:",
				error,
			);
		}
	}
	...
	async recordFailedWebhook(
		...
		console.log(
			`[WebhookRetry] Recorded failed webhook for order ${orderId}, first retry at ${failedWebhook.nextRetryAt.toISOString()}`,
		);
	...
	private async persistFailedWebhook(webhook: FailedWebhook): Promise<void> {
		try {
			...
			console.error(
				`[WebhookRetry] Failed to persist webhook for order ${webhook.orderId}:`,
				error,
			);
		}
	}
	...
	async processRetryQueue(): Promise<void> {
		try {
			...
			console.log(
				`[WebhookRetry] Processing ${ordersToRetry.length} webhooks due for retry`,
			);
			...
			for (const order of ordersToRetry) {
				...
				if (!claimed.rowCount || claimed.rowCount === 0) {
					console.log(
						`[WebhookRetry] Order ${order.id} already claimed by another instance, skipping`,
					);
					continue;
				}
	...
	async retryWebhook(webhook: FailedWebhook): Promise<boolean> {
		...
		console.log(
			`[WebhookRetry] Retrying webhook for order ${webhook.orderId}, attempt ${webhook.retryCount + 1}/${webhook.maxRetries}`,
		);

		try {
			const result = await paymentExecutionBridge.processPaymentCallback(
			...
			if (result.success) {
				...
				console.log(
					`[WebhookRetry] Successfully processed webhook for order ${webhook.orderId} on retry ${webhook.retryCount + 1}`,
				);
			...
			if (webhook.retryCount >= webhook.maxRetries) {
				...
				console.error(
					`[WebhookRetry] Webhook exhausted for order ${webhook.orderId} after ${webhook.maxRetries} attempts`,
				);
			...
			const nextDelay =
				RETRY_DELAYS_MS[webhook.retryCount] ||
				RETRY_DELAYS_MS[RETRY_DELAYS_MS.length - 1];
			webhook.nextRetryAt = new Date(Date.now() + nextDelay);
			webhook.status = "pending_retry";

			console.log(
				`[WebhookRetry] Webhook retry ${webhook.retryCount} failed for order ${webhook.orderId}, next retry at ${webhook.nextRetryAt.toISOString()}`,
			);
	...
	private async updateWebhookStatus(
		...
		try {
			...
			const existingIndex = webhookRetries.findIndex(
				(r: any) => r.webhookId === webhook.id,
			);
			...
			await db
				...
				.set({
					metadata: {
						...
						nextRetryAt:
							status === "pending_retry"
								? webhook.nextRetryAt?.toISOString()
								: null,
					},
					updatedAt: new Date(),
				})
			...
			console.error(
				`[WebhookRetry] Failed to update webhook status for order ${webhook.orderId}:`,
				error,
			);
		}
	}
	 ...
	 */
	private async markOrderForManualReview(
		webhook: FailedWebhook,
	): Promise<void> {
		try {
			await db
			...
			console.log(
				`[WebhookRetry] Order ${webhook.orderId} marked for manual review`,
			);
		} catch (error) {
			console.error(
				`[WebhookRetry] Failed to mark order ${webhook.orderId} for manual review:`,
				error,
			);
		}
	}
	...
	async manualRetry(
		...
		try {
			...
			if (!latestRetry || !latestRetry.callbackData) {
				return {
					success: false,
					message: "No pending webhook found for this order",
				};
			}
	...
	async getStatistics(): Promise<{
		...
		try {
			...
			const [exhaustedResult] = await db
				...
				.from(unifiedOrders)
				.where(
					sql`${unifiedOrders.metadata}->>'lastWebhookStatus' = 'exhausted'`,
				);
