...
export class RecommendationEngine {
	...
	async getBestProducts(
		...
		try {
			...
			const dbProducts = await getRecommendationsByCategory(
				category,
				riskProfile,
			);
			...
			logger.error(
				`[RecommendationEngine] Error fetching products for ${category}/${riskProfile}`,
				error,
			);
			return [];
		}
	...
	findFundInCatalog(name: string): any | null {
		...
		for (const assetClass of Object.keys(FUND_RECOMMENDATIONS_BY_CATEGORY)) {
			for (const risk of Object.keys(
				FUND_RECOMMENDATIONS_BY_CATEGORY[assetClass],
			)) {
