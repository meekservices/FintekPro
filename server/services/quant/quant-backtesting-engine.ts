...
class QuantBacktestingEngine {
	runWeightBacktest(
		...
		const annualizedVolatility =
			this.annualizeMonthlyVolatility(portfolioReturns);
		const sharpeRatio =
		...
		const excessReturns = portfolioReturns.map(
			(r) => r - fullConfig.benchmarkReturn / 12,
		);
		...
		const downsideVariance =
			squaredBelowMAR.reduce((s, v) => s + v, 0) /
			Math.max(portfolioReturns.length, 1);
		const downside = Math.sqrt(downsideVariance) * Math.sqrt(12) || 0.001;
		const sortinoRatio =
			(annualizedReturn - fullConfig.riskFreeRate) / downside;
		...
		const winRate =
			portfolioReturns.filter((r) => r > 0).length /
			Math.max(portfolioReturns.length, 1);
		const volatilityReduction = 1 - annualizedVolatility / 0.15;

		if (sharpeRatio < PROMOTION_THRESHOLDS.minSharpe)
			failReasons.push(
				`Sharpe ${sharpeRatio.toFixed(3)} < ${PROMOTION_THRESHOLDS.minSharpe}`,
			);
		if (maxDrawdown < PROMOTION_THRESHOLDS.maxDrawdown)
			failReasons.push(
				`MaxDD ${(maxDrawdown * 100).toFixed(1)}% < ${PROMOTION_THRESHOLDS.maxDrawdown * 100}%`,
			);
		if (winRate < PROMOTION_THRESHOLDS.minWinRate)
			failReasons.push(
				`WinRate ${(winRate * 100).toFixed(1)}% < ${PROMOTION_THRESHOLDS.minWinRate * 100}%`,
			);
	...
	runDriftModelBacktest(
		...
		if (rocAuc < PROMOTION_THRESHOLDS.minROCAUC)
			failReasons.push(
				`ROC-AUC ${rocAuc.toFixed(3)} < ${PROMOTION_THRESHOLDS.minROCAUC}`,
			);
		if (precision < PROMOTION_THRESHOLDS.minPrecision)
			failReasons.push(
				`Precision ${precision.toFixed(3)} < ${PROMOTION_THRESHOLDS.minPrecision}`,
			);
		if (falsePositiveRate > PROMOTION_THRESHOLDS.maxFalsePositiveRate)
			failReasons.push(
				`FPR ${falsePositiveRate.toFixed(3)} > ${PROMOTION_THRESHOLDS.maxFalsePositiveRate}`,
			);
	...
	validateWeightStability(
		...
		const categories = [
			...new Set([...Object.keys(oldWeights), ...Object.keys(newWeights)]),
		];
		let turnover = 0;
		for (const cat of categories) {
		...
		return {
			...
			failReason: passed
				? null
				: `Turnover increase ${(turnoverIncrease * 100).toFixed(1)}% > ${PROMOTION_THRESHOLDS.maxTurnoverIncrease * 100}%`,
		};
	}
