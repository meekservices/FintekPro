...
class DriftPredictionEngine {
	async extractFeatures(
		...
		const historicalData = await this.getHistoricalDriftData(
			config.lookbackDays,
		);

		return driftMetrics.map((dm) => {
			const catHistory = historicalData.filter(
				(h) => h.assetCategory === dm.category,
			);
			const driftHistory = catHistory.map((h) => h.driftPct || 0);

			const historicalDriftMean =
				driftHistory.length > 0
					? driftHistory.reduce((s, d) => s + d, 0) / driftHistory.length
					: 0;

			const historicalDriftStd =
				driftHistory.length > 2
					? Math.sqrt(
							driftHistory.reduce(
								(s, d) => s + (d - historicalDriftMean) ** 2,
								0,
							) /
			...
			const driftVelocity =
				recentDrifts.length >= 2
					? (recentDrifts[recentDrifts.length - 1] - recentDrifts[0]) /
						recentDrifts.length
					: dm.drift * 0.01;

			const driftAcceleration =
				recentDrifts.length >= 3 ? this.computeAcceleration(recentDrifts) : 0;

			const catVol = CATEGORY_VOLATILITY[dm.category] || 0.15;

			const lastRebalance = catHistory.find(
				(h) =>
					h.finalAction === "REDUCE" ||
					h.finalAction === "INCREASE" ||
					h.finalAction === "SELL" ||
					h.finalAction === "BUY",
			);
			const daysSinceLastRebalance = lastRebalance?.createdAt
				? Math.floor(
						(Date.now() - new Date(lastRebalance.createdAt).getTime()) /
							(1000 * 60 * 60 * 24),
					)
				: 90;
	...
	predict(
		...
		return features.map((f) => {
			...
			const breachProbability = this.computeBreachProbability(
				f,
				predictedDrift,
				toleranceBandPct,
				config,
			...
			const predictedDirection: "OVERWEIGHT" | "UNDERWEIGHT" | "STABLE" =
				Math.abs(predictedDrift) < toleranceBandPct * 0.3
					? "STABLE"
					: predictedDrift > 0
						? "OVERWEIGHT"
						: "UNDERWEIGHT";

			const timeToBreachDays = this.estimateTimeToBreachDays(
				f,
				toleranceBandPct,
			);
	...
	async run(
		...
		try {
			...
			const portfolioBreachProbability =
				predictions.length > 0
					? 1 -
						predictions.reduce((prod, p) => prod * (1 - p.breachProbability), 1)
					: 0;

			const recommendPreemptiveRebalance =
	...
	private predictDriftEMA(
		...
		const volAdjustment =
			features.categoryVolatility * config.volatilityMultiplier;

		const regimeMultiplier =
			features.marketRegime === "CRISIS"
				? 2.0
				: features.marketRegime === "HIGH_VOL"
					? 1.5
					: features.marketRegime === "NORMAL"
						? 1.0
						: 0.7;
		...
		const regimeFactor =
			features.marketRegime === "CRISIS"
				...
				: features.marketRegime === "HIGH_VOL"
					...
					: 1.0;
		const timeFactor =
			1 + Math.min(features.daysSinceLastRebalance / 90, 1) * 0.3;
	...
	private estimateTimeToBreachDays(
		features: DriftFeatures,
		toleranceBandPct: number,
	): number | null {
	...
	private detectMarketRegime(
		...
		if (combinedVol < 0.1) return "LOW_VOL";
		return "NORMAL";
	}
