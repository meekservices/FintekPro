...
class MVOEngine {
	computeCovarianceMatrix(
		...
		const returnSeries = assetsData.map((a) => {
			...
			return r.length >= 20
				? r
				: this.generateSyntheticReturns(a.category, lookback);
		});

		const sampleCov = this.computeSampleCovariance(returnSeries);
		const shrunkCov = this.ledoitWolfShrinkage(
			sampleCov,
			config.shrinkageIntensity,
		);
	...
	private computeSampleCovariance(returnSeries: number[][]): number[][] {
		...
		for (let i = 0; i < n; i++) {
			for (let j = i; j < n; j++) {
				...
				for (let t = 0; t < T; t++) {
					sum +=
						(returnSeries[i][t] - means[i]) * (returnSeries[j][t] - means[j]);
				}
				const value = sum / (T - 1);
				cov[i][j] = value;
				cov[j][i] = value;
	...
	private ledoitWolfShrinkage(
		...
		const shrunk: number[][] = Array.from({ length: n }, (_, i) =>
			Array.from(
				{ length: n },
				(_, j) => (1 - intensity) * sampleCov[i][j] + intensity * target[i][j],
			),
		);
	...
	optimize(
		...
		if (tc.categoryBindings && Object.keys(tc.categoryBindings).length > 0)
			constraintsApplied.push("category_binding");
		if (tc.sectorCaps && Object.keys(tc.sectorCaps).length > 0)
			constraintsApplied.push("sector_caps");
		if (tc.maxPosition < 1.0) constraintsApplied.push("max_position");
		if (escalationRounds > 0)
			constraintsApplied.push(`gamma_escalation_x${escalationRounds}`);
		if (filteredCount > 0)
			constraintsApplied.push(`min_weight_filter_removed_${filteredCount}`);
	...
	private solveConstrainedQP(
		...
		for (let iter = 0; iter < maxIter; iter++) {
			...
			for (let i = 0; i < n; i++) {
				...
				grad[i] =
					mu[i] - 2 * lambda * sigmaW - 2 * gamma * (weights[i] - w0[i]);
			}

			const learningRate = 0.005 / (1 + iter * 0.0005);

			const newWeights = weights.map((w, i) => {
				const updated = w + learningRate * grad[i];
				return Math.max(0, Math.min(maxPos, updated));
			});

			this.projectAllConstraints(
				newWeights,
				w0,
				tc,
				categoryMap,
			...
			const maxDiff = Math.max(
				...newWeights.map((w, i) => Math.abs(w - weights[i])),
			);
			weights = newWeights;
	...
	private logProjectionSummary(
		...
		if (this._projectionNonConvergenceCount > 0) {
			const violations = this.getConstraintViolations(
				weights,
				w0,
				tc,
				categoryMap,
	...
	private getConstraintViolations(
		...
		if (maxW > maxPos + tol)
			violations.push(`maxWeight=${maxW.toFixed(4)}>${maxPos}`);

		const turnover = this.computeTurnover(weights, w0);
		if (turnover > tc.turnoverCap + tol)
			violations.push(
				`turnover=${(turnover * 100).toFixed(1)}%>${(tc.turnoverCap * 100).toFixed(0)}%`,
			);
		...
		if (tc.categoryBindings) {
			for (const [cat, target] of Object.entries(tc.categoryBindings)) {
				...
				if (Math.abs(catSum - target) > tol)
					violations.push(`cat:${cat}=${catSum.toFixed(4)}!=${target}`);
			}
		}
		...
		if (tc.sectorCaps) {
			for (const [sector, cap] of Object.entries(tc.sectorCaps)) {
				...
				if (sectorSum > cap + tol)
					violations.push(
						`sector:${sector}=${(sectorSum * 100).toFixed(1)}%>${(cap * 100).toFixed(0)}%`,
					);
			}
		}
	...
	private buildGroupMap(
		assetsData: AssetData[] | undefined,
		field: "sector" | "instrumentType",
	...
	async run(
		...
		try {
			...
			const expectedReturns =
				...
				this.computeExpectedReturns(assetsData, fullConfig);
			...
			for (let i = 0; i < optimalWeights.length; i++) {
				...
				for (let j = 0; j < optimalWeights.length; j++) {
					portfolioVariance +=
						optimalWeights[i] * optimalWeights[j] * covarianceMatrix[i][j];
				}
			}
			...
			const portfolioVolatility = Math.sqrt(
				Math.max(0, portfolioVariance * 252),
			);
			const sharpeRatio =
				portfolioVolatility > 0
					? (portfolioReturn - 0.0715) / portfolioVolatility
					: 0;
			...
			const annualizedCovarianceMatrix = covarianceMatrix.map((row) =>
				row.map((v) => v * 252),
			);
	...
	async persistWeights(portfolioId: string, result: MVOResult): Promise<void> {
		const entries = result.categories.map((cat, i) => ({
			...
			volatility: result.covarianceMatrix[i]?.[i]
				? Math.sqrt(result.covarianceMatrix[i][i])
				: null,
			sharpeContribution: result.weights[cat]
				? (result.weights[cat] * (result.expectedReturns[i] || 0)) /
	...
	private getCategoryReturnParams(category: string): {
		dailyMean: number;
		dailyVol: number;
	} {
		const params: Record<string, { dailyMean: number; dailyVol: number }> = {
			...
			reit: { dailyMean: 0.0003, dailyVol: 0.0088 },
			invit: { dailyMean: 0.00028, dailyVol: 0.0082 },
			bonds: { dailyMean: 0.00024, dailyVol: 0.0044 },
			mld: { dailyMean: 0.0003, dailyVol: 0.005 },
			pms: { dailyMean: 0.0005, dailyVol: 0.0126 },
	...
	private hashInput(categories: string[], config: MVOConfig): string {
		const str = JSON.stringify({
			...
			ra: config.riskAversion,
			si: config.shrinkageIntensity,
		});
		let hash = 0;
		for (let i = 0; i < str.length; i++) {
