...
import {
	quantBacktestingEngine,
	type BacktestResult,
} from "./quant-backtesting-engine";
...
class QuantRetrainingPipeline {
	async retrainMVOExpectedReturns(): Promise<RetrainingResult> {
		...
		try {
			...
			if (marketRegimeDetector.detectBlackSwanEvent()) {
				throw new Error(
					"RETRAINING_SUSPENDED: Systemic Black Swan regime detected. Safety lock active to prevent model poisoning.",
				);
			}
			...
			for (const config of configs) {
				try {
					...
					if (
						!bestBacktest ||
						backtest.sharpeRatio > bestBacktest.sharpeRatio
					) {
			...
			if (activeModel?.artifactData) {
				...
				stabilityCheck = quantBacktestingEngine.validateWeightStability(
					oldWeights,
					bestResult.weights,
				);
			}
			...
			const shouldPromote =
				...
				(!activeModel ||
					bestBacktest.sharpeRatio >= (activeModel.backtestSharpe || 0));

			const candidateEntry = await db
				.insert(quantModelRegistry)
				.values({
					modelName,
					version: newVersion,
					modelType: "MVO",
	...
	async retrainCovarianceMatrix(): Promise<RetrainingResult> {
		...
		try {
			...
			let maxEig = 0,
				minEig = Number.POSITIVE_INFINITY;
			for (let i = 0; i < n; i++) {
				maxEig = Math.max(maxEig, covMatrix[i][i]);
				minEig = Math.min(minEig, covMatrix[i][i]);
			}
			const conditionNumber =
				minEig > 0 ? maxEig / minEig : Number.POSITIVE_INFINITY;
		...
		try {
			...
			if (recentRuns.length >= 10) {
				...
				if (tacticalTilts.length > 0) {
					...
					const avgTilt =
						allTiltValues.reduce((s, v) => s + v, 0) / allTiltValues.length;
					const variance =
						allTiltValues.reduce((s, v) => s + (v - avgTilt) ** 2, 0) /
						allTiltValues.length;
	...
	async retrainDriftPredictionModel(): Promise<RetrainingResult> {
		...
		try {
			...
			const historicalDecisions = await db
				.select()
				.from(rebalanceDecisionLog)
				.where(
					gte(
						rebalanceDecisionLog.createdAt,
			...
			const backtest =
				quantBacktestingEngine.runDriftModelBacktest(predictions);
	...
	async getModelHealth(): Promise<Record<string, any>> {
		const models = [
			"MVO_EXPECTED_RETURNS",
			"COVARIANCE_MATRIX",
			"BL_CONFIDENCE_CALIBRATION",
			"DRIFT_PREDICTION",
		];
		...
		for (const modelName of models) {
			...
			const daysSinceRetrain = lastRetrain
				? Math.floor(
						(Date.now() - new Date(lastRetrain.createdAt!).getTime()) /
							(1000 * 60 * 60 * 24),
					)
				: null;
	...
	private trainDriftHyperparameters(
		...
		const volatilityMultiplier = Math.max(
			1.0,
			Math.min(3.0, 1 / Math.max(breachRate, 0.1)),
		);
		const driftProbabilityTrigger = Math.max(
			0.5,
			Math.min(0.9, 1 - breachRate),
		);
	...
	private generateSyntheticDriftTrainingData(
		...
		for (let i = 0; i < count; i++) {
			...
			const probability = Math.min(
				1,
				Math.max(0, Math.abs(predictedDrift) / (tolerance * 2)),
			);
			const predictedBreach = probability > 0.5;
	...
	private generateHistoricalReturns(category: string, days: number): number[] {
		const params: Record<string, { mean: number; vol: number }> = {
			...
			reit: { mean: 0.0003, vol: 0.0088 },
			invit: { mean: 0.00028, vol: 0.0082 },
			bonds: { mean: 0.00024, vol: 0.0044 },
			mld: { mean: 0.0003, vol: 0.005 },
			pms: { mean: 0.0005, vol: 0.0126 },
