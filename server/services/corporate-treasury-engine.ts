...
const PRODUCT_YIELDS: Record<
	string,
	{ yield: number; volatility: number; rating: string; maturityDays: number }
> = {
...
class CorporateTreasuryEngine {
	analyzeTreasury(input: TreasuryInput): TreasuryAnalysis {
		...
		const yieldAnalysis = this.analyzeYield(
			bucketAllocations,
			productRecommendations,
		);
	...
	private allocateToBuckets(input: TreasuryInput): BucketAllocation[] {
		...
		operationalPercent = Math.max(
			operationalPercent,
			minLiquidityPercent * 0.5,
		);
		...
		TREASURY_BUCKETS.forEach((bucket, index) => {
			...
			const expectedReturn = products.reduce(
				(sum, p) => sum + (p.expectedYield * p.allocationPercent) / 100,
				0,
			);
			const expectedVolatility = Math.sqrt(
				products.reduce(
					(sum, p) =>
						sum +
						(PRODUCT_YIELDS[p.productType]?.volatility || 0) ** 2 *
							(p.allocationPercent / 100) ** 2,
					0,
				),
			);
	...
	private allocateProductsInBucket(
		bucket: TreasuryBucket,
		amount: number,
		input: TreasuryInput,
	): ProductAllocation[] {
		...
		if (preferredProducts && preferredProducts.length > 0) {
			const preferredEligible = eligibleProducts.filter((p) =>
				preferredProducts.includes(p),
			);
	...
	private getProductRationale(productType: string, bucketType: string): string {
		...
		return (
			rationales[bucketType]?.[productType] ||
			"Diversified allocation for risk management"
		);
	}
	...
	private calculateRiskMetrics(
		...
		recommendations.forEach((rec) => {
			...
			const issuerType =
				rec.productType.includes("g_sec") ||
				rec.productType.includes("treasury")
					? "Government"
					: rec.productType.includes("bank")
						? "Banks"
						: "Corporates";
			counterpartyMap.set(
				issuerType,
				(counterpartyMap.get(issuerType) || 0) + rec.amount,
			);
		});
		...
		concentrationRisk =
			maxExposure > 40 ? 8 : maxExposure > 30 ? 5 : maxExposure > 20 ? 3 : 1;

		const avgDuration =
			recommendations.reduce((sum, r) => sum + r.maturityDays * r.amount, 0) /
			totalValue;
		const durationGap = avgDuration / 365;

		const overallRiskScore =
			(creditRisk + interestRateRisk + liquidityRisk + concentrationRisk) / 4;
	...
	private analyzeYield(
		...
		const weightedAverageYield =
			totalValue > 0 ? weightedYield / totalValue : 0;
	...
	private checkCompliance(
		...
		const operationalAlloc = allocations.find(
			(a) => a.bucket.type === "operational",
		);
		const shortTermAlloc = allocations.find(
			(a) => a.bucket.type === "short_term",
		);
		const liquidAmount =
			(operationalAlloc?.allocatedAmount || 0) +
			(shortTermAlloc?.allocatedAmount || 0);
		...
		if (liquidAmount >= input.minimumLiquidity) {
			...
			checks.push({
				...
				recommendation:
					"Increase allocation to operational and short-term buckets",
			});
		}
		...
		allocations.forEach((alloc) => {
			alloc.products.forEach((product) => {
				...
				if (
					productRatingIndex > minRatingIndex &&
					product.rating !== "SOV" &&
					product.rating !== "A1+" &&
					product.rating !== "A1"
				) {
	...
	private generateRecommendations(
		...
		if (riskMetrics.overallRiskScore < 3) {
			recommendations.push(
				"Risk profile is conservative. Consider slightly higher-yielding instruments to improve returns.",
			);
		} else if (riskMetrics.overallRiskScore > 6) {
			recommendations.push(
				"Risk profile is elevated. Consider de-risking by moving to higher-rated instruments.",
			);
		}

		if (riskMetrics.concentrationRisk > 5) {
			recommendations.push(
				"High concentration detected. Diversify across more issuers and instrument types.",
			);
		}

		if (riskMetrics.durationGap > 1.5) {
			recommendations.push(
				"Portfolio duration is extended. Consider shorter-maturity instruments for better liquidity.",
			);
		}
		...
		const strategicAlloc = allocations.find(
			(a) => a.bucket.type === "strategic",
		);
		if (
			strategicAlloc &&
			strategicAlloc.allocationPercent < 20 &&
			input.objectives.yieldOptimization
		) {
			recommendations.push(
				"Consider increasing strategic reserve allocation for better yield optimization.",
			);
		}

		if (input.objectives.taxEfficiency) {
			recommendations.push(
				"Review tax-efficient options like tax-free bonds and debt funds with indexation benefits.",
			);
		}

		if (recommendations.length === 0) {
			recommendations.push(
				"Portfolio is well-balanced and aligned with treasury objectives.",
			);
		}
