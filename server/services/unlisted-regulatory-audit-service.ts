...
class UnlistedRegulatoryAuditService {
	...
	async logAuditEvent(entry: AuditLogEntry): Promise<string> {
		try {
			...
			retentionExpiresAt.setFullYear(
				retentionExpiresAt.getFullYear() + RETENTION_YEARS,
			);
			...
			const riskLevel =
				entry.complianceContext?.riskLevel || this.calculateRiskLevel(entry);
			...
			console.log(
				`[UnlistedAudit] Logged ${entry.action} for ${entry.entityType}:${entry.entityId}`,
			);
			return auditId;
		} catch (error) {
	...
	async queryAuditLogs(filters: AuditLogFilters = {}) {
		try {
			...
			if (filters.companyId) {
				conditions.push(
					eq(unlistedRegulatoryAuditLog.companyId, filters.companyId),
				);
			}
			if (filters.dealId) {
			...
			if (filters.actionCategory) {
				conditions.push(
					eq(unlistedRegulatoryAuditLog.actionCategory, filters.actionCategory),
				);
			}
			if (filters.entityType) {
				conditions.push(
					eq(unlistedRegulatoryAuditLog.entityType, filters.entityType),
				);
			}
			if (filters.complianceRelated !== undefined) {
				conditions.push(
					eq(
						unlistedRegulatoryAuditLog.complianceRelated,
						filters.complianceRelated,
					),
				);
			}
			if (filters.sebiReportable !== undefined) {
				conditions.push(
					eq(unlistedRegulatoryAuditLog.sebiReportable, filters.sebiReportable),
				);
			}
			if (filters.riskLevel) {
				conditions.push(
					eq(unlistedRegulatoryAuditLog.riskLevel, filters.riskLevel),
				);
			}
			if (filters.startDate) {
				conditions.push(
					gte(unlistedRegulatoryAuditLog.timestamp, filters.startDate),
				);
			}
			if (filters.endDate) {
				conditions.push(
					lte(unlistedRegulatoryAuditLog.timestamp, filters.endDate),
				);
			}
	...
	async getLogs(options: {
		...
		try {
			...
			if (options.riskLevel && options.riskLevel !== "all") {
				conditions.push(
					eq(unlistedRegulatoryAuditLog.riskLevel, options.riskLevel),
				);
			}
			if (options.actionType && options.actionType !== "all") {
				conditions.push(
					eq(unlistedRegulatoryAuditLog.action, options.actionType),
				);
			}
			if (options.userId) {
	...
	async getHeartbeat() {
		try {
			...
			const diffMinutes =
				(new Date().getTime() - new Date(lastPulse).getTime()) / (1000 * 60);
	...
	async getSebiReportableEvents(startDate?: Date, endDate?: Date) {
		try {
			...
			console.error(
				"[UnlistedAudit] Failed to get SEBI reportable events:",
				error,
			);
			throw error;
		}
	...
	async getRetentionStats(): Promise<RetentionStats> {
		try {
			...
			const [totalResult, nearingExpiryResult, archivedResult, expiredResult] =
				await Promise.all([
					db
						.select({ count: sql<number>`count(*)` })
						.from(unlistedRegulatoryAuditLog),
					db
						...
						.where(
							and(
								lte(
									unlistedRegulatoryAuditLog.retentionExpiresAt,
									nearExpiryDate,
								),
	...
	async archiveOldRecords(cutoffDate: Date) {
		try {
			...
			console.log(
				`[UnlistedAudit] Archived records older than ${cutoffDate.toISOString()}`,
			);
			return result;
		} catch (error) {
	...
	async cleanupExpiredRecords(dryRun: boolean = true) {
		try {
			...
			console.log(
				`[UnlistedAudit] Found ${expiredRecords.length} expired records`,
			);
			...
			console.log(
				`[UnlistedAudit] Archiving ${expiredRecords.length} records to cold storage...`,
			);
			...
			if (failedArchivals.length > 0) {
				throw new Error(
					`Failed to archive ${failedArchivals.length} records. Aborting deletion to prevent data loss.`,
				);
			}
			...
			console.error(
				"[UnlistedAudit] Failed to cleanup expired records:",
				error,
			);
			throw error;
		}
	...
	private isSebiReportable(entry: AuditLogEntry): boolean {
		...
		if (
			entry.complianceContext?.complianceRelated &&
			entry.complianceContext?.riskLevel === "critical"
		) {
			return true;
		}
	...
	private calculateRiskLevel(entry: AuditLogEntry): RiskLevel {
		...
		// High risk
		if (
			totalValue >= SEBI_REPORTING_THRESHOLD_INR ||
			complianceFlags.length >= 2
		) {
			return "high";
		}
		...
		if (
			...
			["compliance_override", "dispute_raised"].includes(entry.action)
		) {
			return "medium";
		}
...
export const unlistedRegulatoryAuditService =
	new UnlistedRegulatoryAuditService();
