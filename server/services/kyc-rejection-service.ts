...
class KycRejectionService {
	constructor() {
		...
		console.log(
			`   Reason codes: ${Object.keys(KYC_REJECTION_REASON_CODES).join(", ")}`,
		);
	}
	...
	async reject(params: RejectKycParams): Promise<{
		...
		try {
			...
			if (!validReasons.includes(params.reasonCode as any)) {
				return {
					success: false,
					error: `Invalid reason code. Must be one of: ${validReasons.join(", ")}`,
				};
			}

			const [rejection] = await db
				.insert(kycRejectionEvents)
				.values({
		...
		try {
			...
			const [newSession] = await db
				...
				.values({
					...
					initiatedBy:
						params.initiatedByRole === "agent" ? "agent" : "customer",
	...
	async fileDispute(params: FileDisputeParams): Promise<{
		...
		try {
			...
			if (rejection.disputeStatus) {
				return {
					success: false,
					error: `Dispute already ${rejection.disputeStatus}`,
				};
			}
	...
	async getDisputes(status?: string): Promise<any[]> {
		try {
			...
			if (status) {
				query = query.where(
					eq(kycRejectionEvents.disputeStatus, status),
				) as any;
			}
