...
class FixedIncomeMarketplaceService {
	async getMarketplaceBonds(
		...
		if (
			...
			if (filters.securityType) {
				conditions.push(
					eq(governmentSecurities.securityType, filters.securityType),
				);
			}
			if (filters.minYield) {
				conditions.push(
					gte(
						governmentSecurities.yieldToMaturity,
						filters.minYield.toString(),
					),
				);
			}
			if (filters.maxYield) {
		...
		const allBonds = [
			...governmentBonds.map((b) => ({
				...b,
				category: "government" as const,
			})),
			...corpBonds.map((b) => ({ ...b, category: "corporate" as const })),
		].sort((a, b) => {
			const yieldA = Number.parseFloat(a.yieldToMaturity?.toString() || "0");
			const yieldB = Number.parseFloat(b.yieldToMaturity?.toString() || "0");
	...
	async placeOrder(
		...
		if (request.orderType === "sell") {
			const holdingValidation = await this.validateSellOrder(
				request.userId,
				request.isin,
				request.quantity,
			);
			if (!holdingValidation.valid) {
				throw new Error(
					holdingValidation.error || "Insufficient holdings for sell order",
				);
			}
		}
		...
		const [order] = await db.insert(bondOrders).values(orderData).returning();
	...
	async validateSellOrder(
		...
		isin: string,
		quantity: number,
	): Promise<{ valid: boolean; error?: string; holding?: any }> {
		const holdings = await db
			.select()
			.from(bondHoldings)
			.where(and(eq(bondHoldings.userId, userId), eq(bondHoldings.isin, isin)));

		if (!holdings || holdings.length === 0) {
			return {
				valid: false,
				error: "You do not hold any bonds with this ISIN",
			};
		}

		const totalHoldingQuantity = holdings.reduce(
		...
		const reservedQuantity = pendingSellOrders.reduce(
	...
	async getHoldingByIsin(userId: string, isin: string) {
		...
		const totalValue = holdings.reduce(
			(sum, h) =>
				sum + Number.parseFloat(h.currentValue || h.totalInvestedAmount),
			0,
		);
	...
	async processSellSettlement(
		...
		if (order.orderStatus !== "awaiting_settlement")
			throw new Error("Order not ready for settlement");
		...
		for (const holding of holdings) {
			...
			if (newQuantity === 0) {
				...
				const newTotalFaceValue =
					Number.parseFloat(holding.faceValue) * newQuantity;
				const pricePerUnit =
					Number.parseFloat(holding.totalInvestedAmount) / holding.quantity;
				...
				await db
					...
					.set({
						...
						currentValue: (
							Number.parseFloat(holding.currentPrice || holding.purchasePrice) *
							newQuantity
						).toString(),
					})
		...
		return {
			success: true,
			orderId,
			message: "Sell order settled successfully",
		};
	}
	...
	async getUserWatchlist(userId: string) {
		return db
			...
			.from(bondWatchlist)
			.where(
				and(eq(bondWatchlist.userId, userId), eq(bondWatchlist.isActive, true)),
			)
	...
	async getPortfolioSummary(userId: string) {
		...
		for (const holding of holdings) {
			const invested = Number.parseFloat(
				holding.totalInvestedAmount?.toString() || "0",
			);
			const faceValue = Number.parseFloat(
				holding.totalFaceValue?.toString() || "0",
			);
		...
		for (const payment of upcomingPayments) {
			upcomingCoupons += Number.parseFloat(
				payment.netAmount?.toString() || "0",
			);
		}
	...
	async calculateYieldToMaturity(
		...
		for (let i = 0; i < 100; i++) {
			...
			for (let t = 1; t <= yearsToMaturity; t++) {
				presentValue += annualCoupon / (1 + ytm) ** t;
			}
			presentValue += faceValue / (1 + ytm) ** yearsToMaturity;
	...
	async getAuditLogs(
		...
		if (filters?.startDate) {
			conditions.push(
				gte(fixedIncomeAuditLog.eventTimestamp, filters.startDate),
			);
		}
		if (filters?.endDate) {
	...
	async createNcdIssue(data: any) {
		const [issue] = await db
			.insert(ncdPublicIssues)
			.values({
				issueId: data.issueId || data.issueCode,
				issuerName: data.issuerName || data.issuer,
				issueName:
					data.issueName || `${data.issuerName || data.issuer} NCD Issue`,
				...
				couponRate: data.couponRate?.toString(),
				couponFrequency:
					data.couponFrequency || data.interestPaymentFrequency || "annual",
				...
				maturityDate: data.maturityDate,
				minimumApplication:
					data.minimumApplication?.toString() ||
					data.minApplicationAmount?.toString() ||
					"10000",
