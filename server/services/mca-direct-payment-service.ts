...
class McaDirectPaymentService {
	...
	async initiatePayment(params: InitiatePaymentParams): Promise<{
		...
		try {
			...
			console.log(
				`[MCA Direct Payment] Initiated payment ${payment.id} for ${params.cin} - ${params.feeType} - ₹${params.amount}`,
			);
			...
			console.error(
				"[MCA Direct Payment] Error initiating payment:",
				error.message,
			);
			return {
				success: false,
	...
	async confirmPayment(params: ConfirmPaymentParams): Promise<{
		...
		try {
			...
			console.log(
				`[MCA Direct Payment] Confirmed payment ${params.paymentId} - Challan: ${params.mcaChallanNumber}`,
			);
			...
			try {
				...
				console.error(
					"[MCA Direct Payment] Zoho sync failed:",
					zohoError.message,
				);
			}
			...
			console.error(
				"[MCA Direct Payment] Error confirming payment:",
				error.message,
			);
	...
	async syncToZohoBooks(payment: McaDirectPayment): Promise<{
		...
		try {
			...
			const zohoOrganizationId =
				process.env.ZOHO_ORGANIZATION_ID || process.env.ZOHO_ZSOID;
			...
			const booksService = getZohoBooksService(
				zohoConnectionId,
				zohoOrganizationId,
			);
			...
			const feeDescription =
				MCA_FEE_DESCRIPTIONS[payment.feeType as McaFeeType] || payment.feeType;
			...
			console.log(
				`[MCA Direct Payment] Synced to Zoho Books: ${expense.expense_id}`,
			);
	...
	async retryZohoSync(paymentId: string): Promise<{
		...
		if (payment.status !== "confirmed") {
			return {
				success: false,
				error: "Payment must be confirmed before syncing to Zoho",
			};
		}
	...
	async getPaymentHistory(params?: {
		...
		if (params?.fromDate) {
			conditions.push(
				gte(mcaDirectPayments.createdAt, new Date(params.fromDate)),
			);
		}
		if (params?.toDate) {
			conditions.push(
				lte(mcaDirectPayments.createdAt, new Date(params.toDate)),
			);
		}
	...
	async getPaymentSummary(): Promise<PaymentSummary> {
		...
		return {
			...
			pendingConfirmation: payments.filter((p) => p.status === "initiated")
				.length,
			confirmed: payments.filter((p) => p.status === "confirmed").length,
			syncedToZoho: payments.filter((p) => p.zohoSyncStatus === "synced")
				.length,
	...
	async getReconciliationReport(
		...
		return {
			...
			summary: {
				...
				syncedToZoho: payments.filter((p) => p.zohoSyncStatus === "synced")
					.length,
				failedSync: payments.filter((p) => p.zohoSyncStatus === "failed")
					.length,
			},
			payments,
			zohoMismatches,
