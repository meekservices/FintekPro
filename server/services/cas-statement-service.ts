...
class CASStatementService {
	...
	async parseStatement(text: string): Promise<CASStatementResult> {
		...
		try {
			...
			if (result.portfolioSummary) {
				console.log(
					`[CAS Service v4] Portfolio Summary: ${result.portfolioSummary.entries.length} AMCs`,
				);
				console.log(
					`[CAS Service v4] Summary Total Cost: ₹${(result.portfolioSummary.totalCostValue / 100000).toFixed(2)} L`,
				);
				console.log(
					`[CAS Service v4] Summary Total Market: ₹${(result.portfolioSummary.totalMarketValue / 100000).toFixed(2)} L`,
				);
			}

			// Tiered Parsing: Extract scheme blocks with fallback recovery
			const { blocks: schemeBlocks, droppedForTier2 } =
				this.splitBySchemeBlocks(text);
			console.log(
				"[CAS Service v4] Found",
				schemeBlocks.length,
				"Tier 1 scheme blocks",
			);
			...
			for (let i = 0; i < schemeBlocks.length; i++) {
				try {
					...
					console.warn(
						"[CAS Service v4] Failed to parse scheme block",
						i,
						":",
						error.message,
					);
			...
			for (let i = 0; i < droppedForTier2.length; i++) {
				...
				try {
					const tier2Holding = this.parseTier2ValuationOnlyBlock(
						blockText,
						isin,
					...
					console.warn(
						"[CAS Service v4] Tier 2 recovery failed for",
						isin,
						":",
						error.message,
					);
			...
			if (tier2RecoveryCount > 0) {
				console.log(
					`[CAS Service v4] Tier 2 recovered ${tier2RecoveryCount} valuation-only holdings`,
				);
				result.warnings.push(
					`${tier2RecoveryCount} holdings imported without transaction history. Capital gains not computed for these.`,
				);
			}
			...
			if (result.portfolioSummary) {
				const tier3Holdings = this.applyTier3SummaryDiffInjection(
					result.holdings,
				...
				if (tier3Holdings.length > 0) {
					...
					console.log(
						`[CAS Service v4] Tier 3 injected ${tier3Holdings.length} placeholder holdings for missing AMC value`,
					);
					result.warnings.push(
						`${tier3Holdings.length} holdings require manual review - imported from CAS summary only.`,
					);
				}
			}
			...
			if (result.portfolioSummary) {
				const preEnrichmentResult = {
					...result,
					summary: preEnrichmentSummary,
				};
				result.reconciliation = this.performStrictReconciliation(
					preEnrichmentResult,
					result.portfolioSummary,
				);
				...
				if (!result.reconciliation.passed) {
					...
					console.error(
						`[CAS Service v4] RECONCILIATION FAILED: ${result.reconciliation.message}`,
					);
					result.warnings.push(
						`Reconciliation warning: ${result.reconciliation.message}`,
					);
					result.warnings.push(
						"Holdings parsed but may be incomplete. Please review before using for tax calculations.",
					);
					...
					result.holdings.forEach((h) => {
						...
						h.tierWarnings.push(
							"Reconciliation failed - verify holdings before tax calculations",
						);
					});
					...
					if (result.reconciliation.deltaPercent > 0.1) {
						result.warnings.push(
							`Minor reconciliation delta: ${result.reconciliation.deltaPercent.toFixed(2)}%`,
						);
					}
				}
			} else {
				...
				result.success = result.holdings.length > 0;
				result.warnings.push(
					"No Portfolio Summary found for validation - reconciliation skipped",
				);
			}

			// Database enrichment happens AFTER reconciliation to update with latest NAVs
			if (result.holdings.length > 0) {
				result.holdings = await this.enrichHoldingsWithDatabase(
					result.holdings,
				);
			}
			...
			if (result.holdings.length > 0) {
				result.lotLedger = fifoLotLedgerService.processAllHoldings(
					result.holdings,
				);
				console.log(
					`[CAS Service v4] Lot Ledger: ${result.lotLedger.summary.totalLots} lots across ${result.lotLedger.summary.successfulLedgers} holdings`,
				);
				...
				for (const lotResult of result.lotLedger.results) {
					if (lotResult.reconciliation.warning) {
						result.warnings.push(
							`${lotResult.isin}: ${lotResult.reconciliation.warning}`,
						);
					}
				}
			}
			...
			console.log("[CAS Service v4] Parse complete:", {
				...
				expectedTotal:
					result.portfolioSummary?.totalMarketValue.toFixed(2) || "N/A",
	...
	private normalizeTextForParsing(text: string): string {
		...
		console.log(
			`[CAS Service v4] Text normalization: ${originalLines} -> ${normalizedLines} lines (added ${normalizedLines - originalLines})`,
		);

		// Log sample of date patterns found after normalization (both formats)
		const dateLines = normalized
			.split("\n")
			.filter((line) => DATE_PATTERN.test(line.trim().substring(0, 11)));
		console.log(
			`[CAS Service v4] Found ${dateLines.length} lines starting with dates after normalization`,
		);
	...
	private extractPortfolioSummary(
		text: string,
	): CASPortfolioSummary | undefined {
		try {
			const summaryMatch = text.match(
				/PORTFOLIO\s+SUMMARY[\s\S]*?(?=Date\s+Transaction|Folio No:|$)/i,
			);
			if (!summaryMatch) {
			...
			for (let i = 0; i < lines.length; i++) {
				...
				if (
					...
					!line.toLowerCase().includes("mutual fund")
				) {
					...
					if (totalMatch) {
						totalCostValue = Number.parseFloat(totalMatch[1].replace(/,/g, ""));
						totalMarketValue = Number.parseFloat(
							totalMatch[2].replace(/,/g, ""),
						);
						console.log(
							`[CAS Service v4] Portfolio Summary Total - Cost: ${totalCostValue}, Market: ${totalMarketValue}`,
						);
					} else {
						...
						if (numbers && numbers.length >= 2) {
							...
							if (vals.length >= 2) {
								...
								console.log(
									`[CAS Service v4] Portfolio Summary Total (alt) - Cost: ${totalCostValue}, Market: ${totalMarketValue}`,
								);
							}
						}
					}
				...
				for (const amcName of amcPatterns) {
					if (
						line.toLowerCase().includes(amcName.toLowerCase().substring(0, 15))
					) {
						...
						if (numbers && numbers.length >= 2) {
							...
							if (vals.length >= 2) {
								...
								console.log(
									`[CAS Service v4] Portfolio Summary: ${amcName} - Cost: ${vals[vals.length - 2]}, Market: ${vals[vals.length - 1]}`,
								);
							}
						}
						break;
			...
			if (entries.length === 0) {
				for (const line of lines) {
					...
					if (
						!trimmed ||
						/^\s*(Total|Cost Value|Market Value|Mutual Fund|INR|\(INR\)|PORTFOLIO)/i.test(
							trimmed,
						)
					)
						continue;
					// Must look like text (AMC name portion) followed by numbers
					const amcTextMatch = trimmed.match(
						/^([A-Za-z0-9&\s\-\.]{5,60?}?)\s+([\d,]+\.?\d+)\s+([\d,]+\.?\d+)\s*$/,
					);
					if (amcTextMatch) {
						const candidateName = amcTextMatch[1].trim();
						const cost = Number.parseFloat(amcTextMatch[2].replace(/,/g, ""));
						const market = Number.parseFloat(amcTextMatch[3].replace(/,/g, ""));
						...
						if (
							...
							!entries.some((e) =>
								e.amcName
									.toLowerCase()
									.includes(candidateName.toLowerCase().substring(0, 10)),
							)
						) {
							entries.push({
								amcName: candidateName,
								costValue: cost,
								marketValue: market,
							});
							console.log(
								`[CAS Service v4] Portfolio Summary (regex fallback): ${candidateName} - Cost: ${cost}, Market: ${market}`,
							);
						}
					}
				}
			...
			if (entries.length === 0 && totalCostValue === 0) {
				console.log(
					"[CAS Service v4] Could not parse Portfolio Summary entries",
				);
				return undefined;
			}
			...
			console.log(
				`[CAS Service v4] Portfolio Summary: ${entries.length} AMCs, Total Cost: ${totalCostValue}, Total Market: ${totalMarketValue}`,
			);
			...
			console.error(
				"[CAS Service v4] Error extracting Portfolio Summary:",
				error.message,
			);
			return undefined;
		}
	 ...
	 */
	private parseTier2ValuationOnlyBlock(
		blockText: string,
		isin: string,
		index: number,
	): CASHolding | null {
		...
		const folioNumber = folioMatch
			? folioMatch[1].replace(/\s/g, "").trim()
			: "unknown";
		const isDemat =
			/\(Demat\)/i.test(blockText) && !/\(Non-?Demat\)/i.test(blockText);
		...
		const schemeMatch = blockText.match(
			/([A-Z][A-Za-z0-9\s&-]+(?:Fund|Growth|IDCW|Dividend|Plan))/i,
		);
		if (schemeMatch) {
			schemeName = schemeMatch[1].trim().substring(0, 100);
		}

		const registrar: "CAMS" | "KFINTECH" | "UNKNOWN" = /KFINTECH|KARVY/i.test(
			blockText,
		)
			? "KFINTECH"
		...
		console.log(
			`[CAS Service Tier2] Recovered valuation-only: ${isin} | Folio ${folioNumber} | Units: ${unitBalance.toFixed(3)} | Market: ${marketValue.toFixed(2)}`,
		);
		...
		return {
			...
			tierWarnings: [
				"Recovered from valuation-only block - no transaction history available",
			],
	 ...
	 */
	private createTier3PlaceholderHolding(
		amcName: string,
		costValue: number,
		marketValue: number,
		index: number,
		...
		console.log(
			`[CAS Service Tier3] Created placeholder for ${amcName}: Cost ${costValue.toFixed(2)}, Market ${marketValue.toFixed(2)}`,
		);
		...
		return {
			...
			eligibleForTax: false,
			tierWarnings: [
				"Manual review required - holdings not found in parsed data",
			],
	...
	private applyTier3SummaryDiffInjection(
		...
		const parsedByAMC = new Map<
			string,
			{ costValue: number; marketValue: number }
		>();
		...
		for (const holding of holdings) {
			...
			const current = parsedByAMC.get(normalized) || {
				costValue: 0,
				marketValue: 0,
			};
		...
		for (const entry of summary.entries) {
			...
			if (!parsed) {
				...
				console.log(
					`[CAS Service Tier3] AMC "${entry.amcName}" not found in parsed holdings: Expected ₹${(entry.marketValue / 100000).toFixed(2)} L`,
				);
				console.log(
					`[CAS Service Tier3] Available AMCs: ${parsedAMCNames.join(", ")}`,
				);
				injectedHoldings.push(
					this.createTier3PlaceholderHolding(
						entry.amcName,
						entry.costValue,
						entry.marketValue,
						placeholderIndex++,
				...
				console.log(
					`[CAS Service Tier3] AMC "${entry.amcName}" matched to "${matchedKey}": Parsed ₹${(parsed.marketValue / 100000).toFixed(2)} L vs Expected ₹${(entry.marketValue / 100000).toFixed(2)} L`,
				);
			}
	...
	private amcNamesMatch(name1: string, name2: string): boolean {
		...
		if (words1.length > 0 && words2.length > 0) {
			...
			if (numericPrefix1 && numericPrefix2 && numericPrefix1 === numericPrefix2)
				return true;
		}
	 ...
	 */
	private splitBySchemeBlocks(text: string): {
		blocks: string[];
		droppedForTier2: { isin: string; blockText: string }[];
	} {
		const blocks: string[] = [];
		...
		console.log(
			`[CAS Service v4] Found ${isinMatches.length} ISIN occurrences`,
		);
		...
		for (let i = 0; i < isinMatches.length; i++) {
			...
			const hasClosingBalance = /Closing Unit Balance:\s*[\d,]+\.?\d*/i.test(
				blockText,
			);
			const hasCostValue = /Total Cost(?: Value)?:\s*[\d,]+\.?\d*/i.test(
				blockText,
			);
			...
			if (!hasClosingBalance) {
				...
				const hasMarket = /Market Value|Valuation|Current Value/i.test(
					blockText,
				);
				if (hasNAV || hasMarket) {
		...
		// Log diagnostics
		console.log(
			`[CAS Service v4] Extracted ${blocks.length} unique scheme blocks from ${isinMatches.length} ISIN occurrences`,
		);
		...
		if (droppedBlocks.length > 0) {
			...
			for (const dropped of droppedBlocks.slice(0, 5)) {
				console.log(
					`  - ${dropped.reason}: ${dropped.preview.substring(0, 100)}`,
				);
			}
		...
		for (const [isin, folios] of isinToFolios) {
			if (folios.length > 1) {
				console.log(
					`[CAS Service v4] ISIN ${isin} appears in ${folios.length} folios: ${folios.join(", ")}`,
				);
			}
		}
		...
		if (droppedForTier2.length > 0) {
			console.log(
				`[CAS Service v4] ${droppedForTier2.length} blocks available for Tier 2 valuation-only recovery`,
			);
		}
	...
	private parseSchemeBlock(
		...
		if (singleMatch && singleMatch.length >= 6) {
			...
			console.log(
				`[CAS Service v4] Single-line match: Units=${unitBalance}, Cost=${costValue}, Market=${marketValue}`,
			);
		} else {
			// Pattern 2: Multi-line format from pdf-parse (common case)
			// Extract Units + Cost from "Closing Unit Balance: X Total Cost Value: Y"
			const closingCostPattern =
				/Closing Unit Balance:\s*([\d,]+\.?\d*)\s+Total Cost(?: Value)?:\s*([\d,]+\.?\d*)/i;
			...
			if (closingCostMatch) {
				...
				console.log(
					`[CAS Service v4] Multi-line: Units=${unitBalance}, Cost=${costValue}`,
				);
			} else {
			...
			const navMarketPattern =
				/NAV on\s*(\d{1,2}[-\/][A-Za-z]{3}[-\/]\d{4}):\s*(?:INR|Rs\.?)\s*([\d,]+\.?\d*)\s+Market Value on[^:]+:\s*(?:INR|Rs\.?)\s*([\d,]+\.?\d*)/i;
			...
			if (navMarketMatch) {
				...
				console.log(
					`[CAS Service v4] NAV+Market: NAV=${nav}, Market=${marketValue}, Date=${navDate}`,
				);
			}
		}
		...
		if (!navDate) {
			const navDateMatch = blockText.match(
				/NAV on\s*(\d{1,2}[-\/][A-Za-z]{3}[-\/]\d{4})/i,
			);
			if (navDateMatch) {
		...
		if (missingFields.length > 0) {
			console.warn(
				`[CAS Service v4] WARN: Block ${index} (ISIN: ${isin}) missing fields: [${missingFields.join(", ")}]`,
			);
		...
		if (unitBalance > 0 && marketValue <= 0 && nav > 0) {
			...
			console.log(
				`[CAS Service v4] Computed marketValue from units*NAV: ${marketValue.toFixed(2)}`,
			);
		}
		...
		if (/Direct\s*Plan/i.test(schemeName) || advisorArn === "DIRECT") {
			...
			/Regular\s*Plan/i.test(schemeName) ||
			/ARN-\d+/i.test(advisorArn)
		) {
			planType = "Regular";
		}
		...
		const nominee1Match = blockText.match(
			/Nominee\s*1:\s*([A-Za-z\s]+?)(?=\s*Nominee\s*2:|$)/i,
		);
		if (nominee1Match?.[1].trim())
			nomineeDetails.nominee1 = nominee1Match[1].trim();
		const nominee2Match = blockText.match(
			/Nominee\s*2:\s*([A-Za-z\s]+?)(?=\s*Nominee\s*3:|$)/i,
		);
		if (nominee2Match?.[1].trim())
			nomineeDetails.nominee2 = nominee2Match[1].trim();
		const nominee3Match = blockText.match(/Nominee\s*3:\s*([A-Za-z\s]+)/i);
		if (nominee3Match?.[1].trim())
			nomineeDetails.nominee3 = nominee3Match[1].trim();

		let openingUnitBalance = 0;
		const openingMatch = blockText.match(
			/Opening Unit Balance:\s*([\d,]+\.?\d*)/i,
		);
		if (openingMatch) {
		...
		const transactions = this.parseTransactionsFromBlock(
			blockText,
			isin,
			folioNumber,
			schemeName,
		...
		for (let lotIdx = 0; lotIdx < purchaseTransactions.length; lotIdx++) {
			...
			if (!parsedDate) {
				console.warn(
					`[CAS Lot Builder] Skipping lot with unparseable date: ${txn.transactionDate}`,
				);
				continue;
			}
		...
		const purchaseLots = lots.filter(
			(l) => l.transactionType === "PURCHASE",
		).length;
		const switchLots = lots.filter(
			(l) => l.transactionType === "SWITCH_IN",
		).length;
		const bonusLots = lots.filter(
			(l) =>
				l.transactionType === "BONUS" || l.transactionType === "REINVESTMENT",
		).length;
		...
		console.log(
			`[CAS Lot Builder] ${schemeName.substring(0, 30)}... | ${lots.length} lots | ${lotSummary}`,
		);
		...
		const derivedAvgCostPerUnit =
			lotTotalUnits > 0
				? lotTotalCost / lotTotalUnits
				: unitBalance > 0
					? costValue / unitBalance
					: 0;

		const unrealizedGain = marketValue - derivedCostValue;
		const unrealizedGainPercent =
			derivedCostValue > 0 ? (unrealizedGain / derivedCostValue) * 100 : 0;

		const hasPurchaseTransactions = lots.length > 0;
		const holdingTier: HoldingTier = hasPurchaseTransactions
			? "FULL"
			: "VALUATION_ONLY";
		...
		if (holdingTier === "VALUATION_ONLY") {
			tierWarnings.push(
				"No purchase transactions found - capital gains cannot be computed",
			);
		}
		...
		const totalRedemptionUnits = redemptionTxns.reduce(
			(sum, t) => sum + Math.abs(t.units),
			0,
		);
		const calculatedBalance =
			openingUnitBalance + lotTotalUnits - totalRedemptionUnits;
		...
		if (
			...
			reconciliationDelta > reconciliationTolerance
		) {
			const deltaPct =
				unitBalance > 0 ? (reconciliationDelta / unitBalance) * 100 : 0;
			if (deltaPct > 5) {
				tierWarnings.push(
					`Lot reconciliation mismatch: ${lots.length} lots = ${calculatedBalance.toFixed(3)} units vs closing ${unitBalance.toFixed(3)} (delta: ${reconciliationDelta.toFixed(3)})`,
				);
				console.warn(
					`[CAS Lot Reconciliation] MISMATCH for ${isin}: Lots=${calculatedBalance.toFixed(3)}, Closing=${unitBalance.toFixed(3)}, Delta=${reconciliationDelta.toFixed(3)} (${deltaPct.toFixed(2)}%)`,
				);
			...
			console.log(
				`[CAS Lot Reconciliation] ✓ PASS for ${isin}: ${lots.length} lots = ${lotTotalUnits.toFixed(3)} units, Closing = ${unitBalance.toFixed(3)}`,
			);
		}

		console.log(
			`[CAS Service v4] Parsed: Folio ${folioNumber} | ${schemeName.substring(0, 35)}... | Units: ${unitBalance.toFixed(3)} | Lots: ${lots.length} (${lotSummary}) | Cost: ${derivedCostValue.toFixed(2)} | Avg: ${derivedAvgCostPerUnit.toFixed(2)} | Market: ${marketValue.toFixed(2)} | Tier: ${holdingTier}`,
		);
	 ...
	 */
	private parseTransactionsFromBlock(
		blockText: string,
		isin: string,
		folioNumber: string,
		schemeName: string,
		...
		const keywordPattern = new RegExp(
			`(${transactionKeywords.join("|")})`,
			"i",
		);

		console.log(
			`[CAS Txn Parser] Processing ${lines.length} lines for ISIN ${isin}, Folio ${folioNumber}`,
		);
		...
		for (let i = 0; i < lines.length; i++) {
			...
			if (numbers.length < 2) {
				console.log(
					`[CAS Txn Parser] Skipped line (insufficient numbers): ${line.substring(0, 60)}...`,
				);
				continue;
			}
			...
			if (
				units <= 0 &&
				!restOfLine.toLowerCase().includes("redemption") &&
				!restOfLine.toLowerCase().includes("switch out")
			) {
				continue;
			}
			...
			const { transactionType, isCredit } =
				this.classifyTransaction(description);
			...
			console.log(
				`[CAS Txn Parser] ✓ Txn ${txnIndex}: ${dateStr} ${transactionType} | Units: ${units.toFixed(3)} | NAV: ${nav.toFixed(3)} | Amount: ${amount.toFixed(2)}`,
			);
		}

		console.log(
			`[CAS Txn Parser] Extracted ${transactions.length} transactions for ISIN ${isin}`,
		);
		...
		if (purchaseTxns.length > 0) {
			...
			console.log(
				`[CAS Txn Parser] Purchase lots: ${purchaseTxns.length} | Total units: ${totalUnits.toFixed(3)}`,
			);
		}
	...
	private classifyTransaction(description: string): {
		transactionType: CASTransaction["transactionType"];
		isCredit: boolean;
	} {
	...
	private async enrichHoldingsWithDatabase(
		holdings: CASHolding[],
	): Promise<CASHolding[]> {
		const isins = holdings
			.map((h) => h.isin)
			.filter((isin) => isin.startsWith("INF"));

		if (isins.length === 0) return holdings;

		console.log(
			"[CAS Service v3] Enriching",
			isins.length,
		...
		try {
			...
			return holdings.map((holding) => {
				if (fundLookup.has(holding.isin)) {
					...
					const unrealizedGainPercent =
						holding.costValue > 0
							? (unrealizedGain / holding.costValue) * 100
							: 0;
			...
			console.warn(
				"[CAS Service v3] Database enrichment failed:",
				error.message,
			);
			return holdings;
		}
	...
	private calculateSummary(
		holdings: CASHolding[],
	): CASStatementResult["summary"] {
		const totalInvestedValue = holdings.reduce(
			(sum, h) => sum + h.costValue,
			0,
		);
	 ...
	 * Epic 4.1: Build per-holding confidence scores
	 */
	private buildHoldingConfidence(
		holdings: CASHolding[],
	): Map<string, HoldingConfidence> {
		...
		for (const holding of holdings) {
			...
			if (holding.unitBalance > 0 && holding.costValue > 0) {
				...
				if (
					...
					Math.abs(avgCost - holding.avgCostPerUnit) > 1
				) {
					warnings.push(
						`Avg cost inconsistency: calculated ${avgCost.toFixed(2)} vs stored ${holding.avgCostPerUnit?.toFixed(2)}`,
					);
				}
			}
	...
	private performStrictReconciliation(
		...
		console.log(
			`  Parsed Total:  ₹${parsedTotal.toLocaleString("en-IN", { minimumFractionDigits: 2 })}`,
		);
		console.log(
			`  Expected Total: ₹${expectedTotal.toLocaleString("en-IN", { minimumFractionDigits: 2 })}`,
		);
		console.log(
			`  Delta: ₹${delta.toLocaleString("en-IN", { minimumFractionDigits: 2 })} (${deltaPercent.toFixed(3)}%)`,
		);
		...
		for (const holding of result.holdings) {
			...
			holdingsByAmc.set(
				amcName,
				(holdingsByAmc.get(amcName) || 0) + holding.marketValue,
			);
		}
		...
		for (const entry of summary.entries) {
			...
			const amcDeltaPercent =
				entry.marketValue > 0 ? (amcDelta / entry.marketValue) * 100 : 0;
			if (amcDeltaPercent > 0.5) {
				console.log(
					`  AMC ${entry.amcName}: Parsed ₹${parsedAmcValue.toLocaleString("en-IN")} vs Expected ₹${entry.marketValue.toLocaleString("en-IN")} (${amcDeltaPercent.toFixed(2)}% delta)`,
				);
			}
		}
	...
	private validateAgainstPortfolioSummary(
		...
		const discrepancyPercent =
			expectedTotal > 0 ? (discrepancy / expectedTotal) * 100 : 0;

		console.log(
			`[CAS Service v4] Validation: Parsed ₹${(parsedTotal / 100000).toFixed(2)} L vs Expected ₹${(expectedTotal / 100000).toFixed(2)} L`,
		);
		console.log(
			`[CAS Service v4] Discrepancy: ₹${(discrepancy / 100000).toFixed(2)} L (${discrepancyPercent.toFixed(1)}%)`,
		);
		...
		for (const entry of summary.entries) {
			...
			if (amcDiscrepancy > 10000) {
				console.log(
					`[CAS Service v4] AMC Discrepancy - ${entry.amcName}: Parsed ₹${(parsedAmcValue / 100000).toFixed(2)} L vs Expected ₹${(entry.marketValue / 100000).toFixed(2)} L`,
				);
			}
		}
	...
	private calculateConfidenceScore(result: CASStatementResult): number {
		...
		const holdingsWithIsin = result.holdings.filter(
		...
		const holdingsWithTransactions = result.holdings.filter(
			(h) => h.transactions.length > 0,
		);
	...
	async getHoldingByISIN(
		holdings: CASHolding[],
		isin: string,
	): Promise<CASHolding | undefined> {
		return holdings.find((h) => h.isin === isin);
	}

	async getHoldingsByRegistrar(
		holdings: CASHolding[],
	...
	convertToPortfolioHoldingsWithDates(
		...
		return holdings.map((holding) => {
			...
			return {
				...
				averageCost:
					holding.avgCostPerUnit ||
					(holding.unitBalance > 0
						? holding.costValue / holding.unitBalance
						: 0),
	...
	async persistLotsToDatabase(
		...
		for (const holding of result.holdings) {
			const purchaseTransactions = holding.transactions.filter(
				(t) =>
					t.isCredit &&
					["Purchase", "SIP", "Switch In", "Bonus", "Reinvestment"].includes(
						t.transactionType,
					),
			);
