...
export class ListingTransitionService {
	...
	validateTransitionRequest(
		...
		if (!this.isValidTransition(currentStage, request.targetStage)) {
			errors.push(
				`Invalid transition from '${currentStage}' to '${request.targetStage}'. Valid transitions: ${this.getValidTransitions(currentStage).join(", ")}`,
			);
		}

		const requiredFields = this.getRequiredFieldsForTransition(
			request.targetStage,
		);
		for (const field of requiredFields) {
			if (!(request as any)[field]) {
				errors.push(
					`Field '${field}' is required for transition to '${request.targetStage}'`,
				);
			}
		}
	...
	async executeTransition(
		...
		try {
			...
			if (request.targetStage === "listed") {
				...
				const stockResult = await this.createStockFromUnlisted(
					company,
					request,
				);
				if (stockResult.success && stockResult.stockId) {
					...
					const holdingsResult = await this.migratePortfolioHoldings(
						request.companyId,
						stockId,
						company.name,
					);
					...
					// Send notifications
					const notifResult = await this.notifyStakeholders(
						request.companyId,
						company.name,
						request,
					);
	...
	private async createStockFromUnlisted(
		...
		try {
			...
			await db.insert(listedStocks).values({
				...
				nseCode:
					request.exchange === "NSE" || request.exchange === "NSE_BSE"
						? "EQ"
						: null,
				bseCode:
					request.exchange === "BSE" || request.exchange === "NSE_BSE"
						? request.stockSymbol
						: null,
			...
			return {
				success: false,
				errors: [`Failed to create stock: ${error.message}`],
			};
		}
	}
		...
		try {
			...
			for (const holding of holdings) {
				try {
					...
					errors.push(
						`Failed to migrate holding ${holding.id}: ${holdingError.message}`,
					);
				}
			}
			...
			return {
				updated: 0,
				errors: [`Failed to query holdings: ${error.message}`],
			};
		}
	}
	...
	private async notifyStakeholders(
		...
		try {
			...
			const uniqueUserIds = [
				...new Set(holdings.map((h) => h.userId).filter(Boolean)),
			];
			...
			for (const userId of uniqueUserIds) {
				...
				console.log(
					`[ListingTransition] Notification for user ${userId}: ${companyName} listed on ${request.exchange} as ${request.stockSymbol}`,
				);
				sent++;
			}
			...
			return {
				sent: 0,
				errors: [`Failed to send notifications: ${error.message}`],
			};
		}
	}
