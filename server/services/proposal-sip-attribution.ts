...
import {
	...
	proposalVerdicts,
	InsertProposalSipRecommendation,
} from "@shared/schema";
...
export class ProposalSipAttribution {
	...
	static async createSipRecommendation(
		...
		try {
			...
			const values: InsertProposalSipRecommendation = {
				...
				sipStartDate: recommendation.sipStartDate
					? recommendation.sipStartDate.toISOString().split("T")[0]
					: null,
			...
			const [result] = await db
				.insert(proposalSipRecommendations)
				.values(values)
				.returning({ id: proposalSipRecommendations.id });
	...
	static async convertLumpsumToSip(
		...
		if (existing.length > 0) {
			...
			await ProposalSipAttribution.createSipRecommendation(proposalId, {
	...
	static async getSipSummary(proposalId: string): Promise<SipSummary> {
		...
		const sumAmount = (items: any[]) =>
			items.reduce(
				(sum, s) => sum + Number.parseFloat(s.sipAmount?.toString() || "0"),
				0,
			);
		...
		return {
			...
			recommendations: sips.map((s) => ({
				...
				sipAmount: Number.parseFloat(s.sipAmount?.toString() || "0"),
				sipFrequency: (s.sipFrequency || "monthly") as
					| "monthly"
					| "quarterly"
					| "weekly",
				...
				originalLumpsumAmount: s.originalLumpsumAmount
					? Number.parseFloat(s.originalLumpsumAmount.toString())
					: undefined,
			})),
		};
	}
