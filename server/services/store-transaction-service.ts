...
class StoreTransactionService {
	...
	async logTransaction(params: TransactionLogParams): Promise<string> {
		try {
			...
			await db.insert(storeTransactionLogs).values({
				...recordData,
				checksum,
				consentChecksum: params.consentTimestamp
			...
			console.log(
				`[StoreTransaction] Logged: ${transactionId} - ${params.transactionType} - ${params.productCategory}`,
			);
	...
	async getTransactionsByUser(
		...
		if (options?.category) {
			conditions.push(
				eq(storeTransactionLogs.productCategory, options.category),
			);
		}
		...
		return {
			...
			hasMore:
				(options?.offset || 0) + transactions.length <
