...
export class DematHoldingsService {
	...
	async fetchHoldings(request: DematFetchRequest): Promise<DematFetchResponse> {
		try {
			...
			if (!this.hasValidCredentials()) {
				throw new Error(
					"Demat data service not configured. Set AA_API_KEY for Account Aggregator demat holdings.",
				);
			}
			...
			const totalInvestedAmount = enrichedHoldings.reduce(
			...
			const nsdlHoldings = enrichedHoldings.filter(
				(h) => h.depository === "NSDL",
			).length;
			const cdslHoldings = enrichedHoldings.filter(
				(h) => h.depository === "CDSL",
			).length;

			console.log(
				`✅ Fetched ${enrichedHoldings.length} demat holdings across ${accounts.length} accounts (Total Value: ₹${totalValue.toFixed(2)})`,
			);
	 ...
	 */
	private async callAccountAggregatorAPI(
		...
		try {
			...
			throw new Error(
				`AA API failed: ${error.response?.data?.message || error.message}`,
			);
		}
	}
		...
		for (const accountData of accountsData) {
			...
			for (const holding of holdingsData) {
				...
				if (quantity <= 0 || !holding.isin) {
					console.warn(
						`Skipping invalid holding: ${holding.isin || "unknown"} - quantity: ${quantity}`,
					);
					continue;
				}

				// Calculate values if not provided
				const investedAmount = safeParseFloat(
				...
				holdings.push({
					...
					returns,
					returnsPercentage,
					assetType: this.determineAssetType(
						holding.isin,
	...
	private determineAssetType(isin: string, name: string): DematAssetType {
		...
		if (upperIsin.startsWith("INE")) {
			...
			if (
				...
				lowerName.includes("real estate investment trust")
			)
				return "reit";
			if (
				lowerName.includes("invit") ||
				lowerName.includes("infrastructure investment trust")
			)
				return "invit";
			if (lowerName.includes("etf") || lowerName.includes("exchange traded"))
				return "etf";
			if (
				lowerName.includes("sovereign gold bond") ||
				lowerName.includes("sgb")
			)
				return "sgb";
			if (
				lowerName.includes("market linked debenture") ||
				lowerName.includes("mld")
			)
				return "mld";
			if (
				lowerName.includes("ncd") ||
				lowerName.includes("non convertible debenture")
			)
				return "ncd";
			if (lowerName.includes("bond") || lowerName.includes("debenture"))
				return "bond";
			if (lowerName.includes("preference share") || lowerName.includes("pref"))
				return "preference_share";
			if (lowerName.includes("convertible")) return "convertible";
			if (
				lowerName.includes("pms") ||
				lowerName.includes("portfolio management")
			)
				return "pms";
		...
		// G-Sec patterns
		if (
			lowerName.includes("treasury bill") ||
			lowerName.includes("t-bill") ||
		...
		if (
			...
			lowerName.includes("alternative investment")
		)
			return "aif";
	...
	async fetchTransactions(
		...
		try {
			...
			if (!this.hasValidCredentials()) {
				throw new Error(
					"Demat data service not configured. Set AA_API_KEY for Account Aggregator demat transactions.",
				);
			}

			// Production: Call Account Aggregator Transaction API
			const txResponse = await this.callAATransactionAPI(
				request,
				startDate,
				endDate,
			);
	...
	private async callAATransactionAPI(
		...
		try {
			...
			throw new Error(
				`AA Transaction API failed: ${error.response?.data?.message || error.message}`,
			);
		}
	}
	...
	private parseTransactionResponse(aaResponse: any): DematTransaction[] {
		...
		for (const tx of txData) {
			...
			const price = Number.parseFloat(tx.price || 0);
			const amount = Number.parseFloat(tx.amount || quantity * price);
			...
			transactions.push({
				...
				price,
				amount,
				brokerage: Number.parseFloat(tx.brokerage || 0),
				stt: Number.parseFloat(tx.stt || 0),
				stampDuty: Number.parseFloat(tx.stamp_duty || 0),
				gst: Number.parseFloat(tx.gst || 0),
				otherCharges: Number.parseFloat(tx.other_charges || 0),
				netAmount: Number.parseFloat(tx.net_amount || amount),
	...
	private normalizeTransactionType(type: string): DematTransactionType {
		...
		if (t.includes("CORP") || t.includes("CORPORATE"))
	...
	async fetchDematWithTransactions(
		...
		console.log(
			`📊 Fetching unified Demat (Holdings + Transactions) for PAN: ${request.panNumber.slice(0, 4)}****`,
		);
		...
		console.log(
			`✅ Unified Demat fetch complete: ${holdingsResult.holdings.length} holdings, ${transactionsResult.transactions.length} transactions`,
		);
