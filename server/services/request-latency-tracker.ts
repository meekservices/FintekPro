  ...
  recordLatency(method: string, path: string, latencyMs: number): void {
    ...
    if (!entry) {
      if (this.endpoints.size >= MAX_TRACKED_ENDPOINTS) {
        const oldestKey = Array.from(this.endpoints.entries())
          .sort(([, a], [, b]) => a.lastUpdated.getTime() - b.lastUpdated.getTime())[0]?.[0];
...
export function latencyTrackingMiddleware(req: Request, res: Response, next: NextFunction): void {
