...
export class RiskEngine {
   ...
   */
  async evaluateTrade(userId: string, symbol: string, notionalValue: number, currentPositionValue: number, dailyTradeCount: number) {
    ...
    if ((currentPositionValue + notionalValue) > this.MAX_EXPOSURE_PER_STOCK) {
      throw new Error(`Risk Violation: Max exposure of $${this.MAX_EXPOSURE_PER_STOCK} per stock exceeded for ${symbol}.`);
    }

    // Rule 2: Trade Velocity (Pattern Day Trading proxy/throttle)
    if (dailyTradeCount >= this.MAX_DAILY_TRADES) {
      throw new Error(`Risk Violation: Max daily trade count of ${this.MAX_DAILY_TRADES} exceeded.`);
    }
  ...
  async scanForSuspiciousActivity(userId: string) {
    ...
    logger.info(`[RiskEngine] Scanning for suspicious activity for user ${userId}`);
