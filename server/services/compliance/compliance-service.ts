 ...
 */
const SEBI_BLOCKED_STATUSES = new Set([
	"BLOCKED",
	"OVERLAP_BREACH",
	"GLIDE_PATH_INVALID",
]);
...
export class ComplianceService {
	...
	async checkFundsCompliance(
		...
		try {
			logger.info(
				`[ComplianceService] Starting bulk compliance check for ${fundNames.length} funds`,
			);
        ...
        WHERE ${mutualFundsTable.schemeName} ILIKE '%' || n || '%'
			...
			for (const originalName of fundNames) {
				...
				if (fund && SEBI_BLOCKED_STATUSES.has(fund.complianceStatus || "")) {
					logger.warn(
						`[ComplianceService] Fund ${originalName} is NON-COMPLIANT (${fund.complianceStatus})`,
					);
					...
					if (fund.category) {
						const catSearchNames =
							blockedFundsByCategory.get(fund.category) || [];
			...
			if (categoriesToFind.length > 0) {
				logger.info(
					`[ComplianceService] Fetching substitutes for ${categoriesToFind.length} categories`,
				);
				...
				for (const [category, searchNames] of blockedFundsByCategory) {
					...
					const bestSub = substitutes.find(
						(s) =>
							s.category === category &&
							!searchNames.some((n) =>
								s.schemeName.toLowerCase().includes(n.toLowerCase()),
							),
					);
