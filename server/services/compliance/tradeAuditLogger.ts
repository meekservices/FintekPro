...
export class TradeAuditLogger {
  ...
  async logTrade(payload: {
    ...
    logger.info(`[TradeAuditLogger] Logging trade event for ${payload.userId}: ${payload.status}`);
    ...
    try {
      ...
      //   timestamp: new Date()
      // });
      logger.info(`[TradeAuditLogger] (Mock) Inserted log into alpaca_trade_logs`);
    } catch (error) {
      logger.error(`[TradeAuditLogger] CRITICAL: Failed to write to audit log!`, error);
      // Depending on strictness, we might throw here to halt execution if auditing fails.
    }
  }

  async logAdminOverride(adminId: string, targetUserId: string, action: string, reason: string) {
    logger.warn(`[TradeAuditLogger] ADMIN OVERRIDE by ${adminId} on ${targetUserId}. Action: ${action}. Reason: ${reason}`);
