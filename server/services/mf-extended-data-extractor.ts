...
class MFExtendedDataExtractor {
  ...
  parseExitLoadText(text: string | null | undefined): ParsedExitLoad {
    ...
    if (normalizedText === 'nil' || normalizedText === 'none' || normalizedText === '0' || 
        normalizedText === '0%' || normalizedText === 'no exit load' || normalizedText === 'n.a.' ||
        normalizedText.includes('no exit load') || normalizedText.includes('nil exit')) {
    ...
    if (rangeMatch) {
      result.percent = parseFloat(rangeMatch[1]);
      result.days = parseInt(rangeMatch[3]); // Use end of range
      return result;
    }
    ...
    if (withinMatch) {
      result.percent = parseFloat(withinMatch[1]);
      const timeValue = parseInt(withinMatch[2]);
    ...
    const beforeDaysPattern = /(\d+\.?\d*)\s*%.*?(?:before|up\s*to|upto|within)\s*(\d+)\s*days?/i;
    const beforeDaysMatch = normalizedText.match(beforeDaysPattern);
    if (beforeDaysMatch) {
      result.percent = parseFloat(beforeDaysMatch[1]);
      result.days = parseInt(beforeDaysMatch[2]);
      return result;
    }
    ...
    if (simpleDaysMatch) {
      result.percent = parseFloat(simpleDaysMatch[1]);
      result.days = parseInt(simpleDaysMatch[2]);
      return result;
    }
  ...
  parseDate(dateStr: string | null | undefined): Date | null {
    ...
    const ddMmmYyyy = trimmed.match(/^(\d{1,2})[\s\-]*(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*[\s\-,]*(\d{4})$/i);
    if (ddMmmYyyy) {
    ...
    const mmmDdYyyy = trimmed.match(/^(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*[\s\-]*(\d{1,2})[\s,]*(\d{4})$/i);
    if (mmmDdYyyy) {
  ...
  async extractAllFunds(options: {
    ...
    try {
      ...
      console.log(`[MFExtendedDataExtractor] Processing ${funds.length} funds with extendedData`);
      ...
      for (let i = 0; i < funds.length; i += batchSize) {
        ...
        for (const fund of batch) {
          try {
            const extendedData = fund.extendedData as ExtendedDataContent | null;
            ...
            if ((fund.exitLoadPercent === null || forceRefresh) && extracted.exitLoadPercent !== null) {
            ...
            if ((fund.minLumpsumAmount === null || forceRefresh) && extracted.minLumpsumAmount !== null) {
              updates.minLumpsumAmount = extracted.minLumpsumAmount.toString();
              stats.minAmountsUpdated++;
              hasUpdate = true;
            }
            
            if ((fund.minSipAmount === null || forceRefresh) && extracted.minSipAmount !== null) {
              updates.minSipAmount = extracted.minSipAmount.toString();
              hasUpdate = true;
            }
            
            if ((fund.launchDate === null || forceRefresh) && extracted.launchDate !== null) {
            ...
            extractionProgress.errors.push(`Fund ${fund.schemeCode}: ${error.message}`);
          }
        }
      }
      ...
      console.log(`[MFExtendedDataExtractor] Completed: ${stats.exitLoadUpdated} exit loads, ${stats.minAmountsUpdated} min amounts, ${stats.launchDateUpdated} launch dates`);
