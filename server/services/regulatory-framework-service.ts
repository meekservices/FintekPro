...
const DEFAULT_BROKERAGE_STRUCTURES: Record<
	string,
	Partial<InvestorBrokerageStructure>
> = {
	retail: {
...
export class RegulatoryFrameworkService {
	...
	async classifyInvestor(
		...
		const effectiveNetWorth =
			...
			(userProfile?.netWorthAmount
				? Number.parseFloat(String(userProfile.netWorthAmount))
				: 0);
		const effectiveKycTier = kycTier || userProfile?.kycTier || "basic";
		const effectiveEntityType =
			entityType || userProfile?.clientType || "individual";
		...
		if (investmentAmount) {
			...
			if (investmentAmount <= SEBI_THRESHOLDS.SHNI_MAX) {
				return {
		...
		if (effectiveNetWorth >= SEBI_THRESHOLDS.SHNI_MIN) {
			return {
				classificationType: "sHNI",
				classificationBasis: "net_worth",
				rule: await this.getClassificationRule("sHNI"),
	 ...
	 */
	async getClassificationRule(
		classificationType: string,
	): Promise<InvestorClassificationRule | null> {
		const [rule] = await db
			.select()
			...
			.where(
				and(
					eq(
						investorClassificationRules.classificationType,
						classificationType,
					),
	...
	async saveUserClassification(
		...
		if (existing) {
			await db
				.update(userInvestorClassifications)
				.set({
					classificationStatus:
						existing.classificationType === classification.classificationType
							? "active"
							: "upgraded",
					updatedAt: new Date(),
				})
		...
		const rule = await this.getClassificationRule(
			classification.classificationType,
		);

		// Calculate expiry (1 year for QIB/accredited, null for retail/HNI)
		const expiresAt = ["qib", "anchor"].includes(
			classification.classificationType,
		)
		...
		const [newClassification] = await db
			.insert(userInvestorClassifications)
			.values({
				userId,
				classificationType: classification.classificationType,
				classificationRuleId: rule?.id,
				classificationBasis: classification.classificationBasis,
				investmentAmountAtClassification:
					classification.investmentAmount?.toString(),
	...
	async getUserClassification(
		userId: string,
	): Promise<UserInvestorClassification | null> {
	...
	async getBrokerageStructure(
		...
		return (
			DEFAULT_BROKERAGE_STRUCTURES[investorType] ||
			DEFAULT_BROKERAGE_STRUCTURES.retail
		);
	}
	...
	calculateTransactionCosts(
		...
		const brokerage =
			investmentAmount *
			(Number.parseFloat(String(brokerageStructure.brokerageFeePercent || 0)) /
				100);
		const platformFee =
			investmentAmount *
				(Number.parseFloat(String(brokerageStructure.platformFeePercent || 0)) /
					100) +
			Number.parseFloat(String(brokerageStructure.flatPlatformFee || 0));
		const exchangeCharges =
			investmentAmount *
			(Number.parseFloat(
				String(brokerageStructure.exchangeChargePercent || 0),
			) /
				100);
		const clearingCharges =
			investmentAmount *
			(Number.parseFloat(
				String(brokerageStructure.clearingChargePercent || 0),
			) /
				100);
		const sebiFee =
			investmentAmount *
			(Number.parseFloat(String(brokerageStructure.sebiFeePercent || 0)) / 100);
		const stampDuty =
			investmentAmount *
			(Number.parseFloat(String(brokerageStructure.stampDutyPercent || 0)) /
				100);

		const chargesBeforeGst = brokerage + platformFee;
		const gst =
			chargesBeforeGst *
			(Number.parseFloat(String(brokerageStructure.gstPercent || 0)) / 100);
		...
		return {
			...
			netInvestmentAmount:
				Math.round((investmentAmount - totalCharges) * 100) / 100,
	...
	async checkProductEligibility(
		...
		if (!classification) {
			const autoClassification = await this.classifyInvestor(
				userId,
				investmentAmount,
			);
		...
		for (const rule of rules) {
			...
			const userKycTierIndex = kycTierOrder.indexOf(
				userProfile?.kycTier || "basic",
			);
			...
			const hasOverride = activeOverrides.some(
				(o) => o.overrideType === "investment_limit",
			);

			if (!hasOverride) {
				if (
					rule.minInvestment &&
					investmentAmount < Number.parseFloat(String(rule.minInvestment))
				) {
					violations.push({
						code: "MIN_INVESTMENT_NOT_MET",
						message: `Minimum investment amount is ₹${Number.parseFloat(String(rule.minInvestment)).toLocaleString("en-IN")}`,
					});
				}

				if (
					rule.maxInvestment &&
					investmentAmount > Number.parseFloat(String(rule.maxInvestment))
				) {
					violations.push({
						code: "MAX_INVESTMENT_EXCEEDED",
						message: `Maximum investment amount is ₹${Number.parseFloat(String(rule.maxInvestment)).toLocaleString("en-IN")}`,
					});
				}
			}
			...
			if (
				rule.requiresAccreditedInvestor &&
				userProfile?.accreditedInvestorStatus !== "verified"
			) {
				violations.push({
		...
		const brokerageStructure = await this.getBrokerageStructure(
			investorType,
			productCategory,
		);
		...
		if (violations.length > 0) {
			for (const violation of violations) {
				await db.insert(regulatoryViolationLogs).values({
					userId,
					violationType: violation.code.includes("KYC")
						? "kyc_insufficient"
						: violation.code.includes("INVESTMENT")
	 ...
	 */
	async createOverrideProposal(proposal: {
		userId: string;
		productCategory: string;
		...
		const [created] = await db
			.insert(investmentLimitOverrideProposals)
			.values({
	...
	async reviewOverrideProposal(
		...
		const [updated] = await db
			.update(investmentLimitOverrideProposals)
			.set(updateData)
			.where(eq(investmentLimitOverrideProposals.id, proposalId))
			.returning();
	 ...
	 */
	private async activateOverride(
		proposal: InvestmentLimitOverrideProposal,
	): Promise<void> {
	...
	async getPendingProposals(filters?: {
		...
		if (filters?.status) {
			query = query.where(
				eq(investmentLimitOverrideProposals.status, filters.status),
			) as typeof query;
		}

		if (filters?.proposerRole) {
			query = query.where(
				eq(investmentLimitOverrideProposals.proposerRole, filters.proposerRole),
			) as typeof query;
		}
	...
	async getRiskDisclosureTemplates(
		...
		if (disclosureType) {
			conditions.push(
				eq(riskDisclosureTemplates.disclosureType, disclosureType),
			);
		}
		...
		const defaultRules = [
			{
				...
				description:
					"Individual investors with investment up to ₹2 lakhs per PAN",
			...
			},
			{
				...
				description:
					"Non-institutional investors with investment between ₹2 lakhs to ₹10 lakhs",
			...
			},
			{
				...
				description:
					"Non-institutional investors with investment above ₹10 lakhs",
			...
			},
			{
				...
				description:
					"SEBI-registered institutional investors with assets exceeding ₹100 crore",
			...
			},
			{
				...
				description:
					"QIBs applying before public opening with minimum ₹10 crore investment",
