    ...
    try {
      ...
      if (!profile) {
        return {
          ...
          nomineeCount: 0,
          hasOptedOut: false,
          blockingReason: 'User profile not found. Please complete your profile before investing.',
        };
      }
      ...
      return {
        ...
        nomineeCount: 0,
        hasOptedOut: false,
        blockingReason: 'Please add a nominee or explicitly opt out of nomination before investing. Required under SEBI Circular SEBI/HO/IMD/IMD-II-DOF3/P/CIR/2022/0093.',
      };
    } catch (err) {
      logger.error('[NomineeEnforcement] Failed to check nominee status', { userId, err });
...
export interface GrievanceSubmission {
  userId: string;
  category: 'kyc' | 'transaction' | 'payment' | 'account' | 'commission' | 'other';
  subject: string;
  description: string;
...
class GrievanceService {
  ...
  async submitGrievance(submission: GrievanceSubmission): Promise<GrievanceResult> {
    ...
    expectedResolutionDate.setDate(expectedResolutionDate.getDate() + this.RESOLUTION_DAYS);
    ...
    try {
      ...
      logger.error('[Grievance] Failed to create support ticket', { submission, err });
      throw err;
    }

    // Notify compliance team via admin parallel notifier
    try {
      const { notifyGrievanceSubmitted } = await import('./admin-parallel-notifier');
  ...
  getRegulatorLinks() {
    return {
      scores: {
        url: this.SCORES_URL,
        name: 'SEBI SCORES',
        description: 'SEBI Complaints Redress System — file complaints directly with SEBI',
      },
      smartOdr: {
        url: this.SMART_ODR_URL,
        name: 'SmartODR',
        description: 'SEBI Online Dispute Resolution platform for investment disputes',
      },
      amfi: {
