...
const IDEAL_ALLOCATIONS: Record<
	string,
...
export class AIProposalEngine {
	async getOrCreateRiskProfile(
		userId: string,
	): Promise<ClientRiskProfile | null> {
		const existing = await db
			.select()
			.from(clientRiskProfiles)
			.where(eq(clientRiskProfiles.userId, userId))
			.limit(1);
		...
		const riskCategory = this.mapRiskTolerance(
	...
	async getPortfolioHoldings(userId: string): Promise<PortfolioHolding[]> {
		...
		for (const h of mfData) {
			const value = Number.parseFloat(
				h.mf_holdings.currentValue?.toString() || "0",
			);
			totalValue += value;
		}
		...
		for (const mld of mldData) {
			const value = Number.parseFloat(
				mld.currentValue || mld.totalInvested || "0",
			);
			totalValue += value;
		}
		...
		for (const h of mfData) {
			...
			const assetSubType = this.classifyMFScheme(
				h.mf_holdings.schemeName || "",
			);
			...
			holdings.push({
				...
				lockIn: h.mf_holdings.lockInEndDate
					? new Date(h.mf_holdings.lockInEndDate) > new Date()
					: false,
		...
		for (const mld of mldData) {
			...
			const riskScore =
				RISK_RATINGS[riskKey] || RISK_RATINGS.mld || mld.riskScore || 7;
			...
			holdings.push({
				...
				weightPercent: totalValue > 0 ? (value / totalValue) * 100 : 0,
				riskScore: riskScore,
				lockIn: mld.maturityDate
					? new Date(mld.maturityDate) > new Date()
					: true,
	...
	calculateAssetAllocation(
		...
		for (const h of holdings) {
			...
			if (
				assetType === "cash" ||
				subType.includes("liquid") ||
				subType.includes("overnight")
			) {
				...
				subType.includes("debt") ||
				subType.includes("gilt") ||
				subType.includes("corporate") ||
				...
				const equityRatio = subType.includes("aggressive")
					? 0.65
					: subType.includes("conservative")
						? 0.25
						: 0.5;
		...
		for (const h of holdings) {
			if (h.weightPercent > maxSingleStock) {
				issues.push({
					...
					severity:
						h.weightPercent > maxSingleStock * 1.5 ? "critical" : "warning",
				});
			}
		}
	...
	async analyzeMFOverlap(
		holdings: PortfolioHolding[],
	): Promise<{ overlapPercent: number; details: OverlapDetail[] }> {
				...
				mfHolds.map(async (h) => {
					...
					return {
						...
						holdings: stocks.map((s) => ({
							stock: s.stockSymbol,
							weight: Number(s.holdingPercentage ?? 1),
						})),
					};
				}),
			);

			const fundsWithData = fundsWithHoldings.filter(
				(f) => f.holdings.length > 0,
			);

			if (fundsWithData.length >= 2) {
				const pyResult = await callPython<any>(
					"/api/portfolio/overlap-analysis",
					"POST",
					{
						funds: fundsWithData,
						topN: 10,
		...
		for (let i = 0; i < mfHolds.length; i++) {
			for (let j = i + 1; j < mfHolds.length; j++) {
				...
				if (est > 20) {
					details.push({
						scheme1: mfHolds[i].schemeName,
						scheme2: mfHolds[j].schemeName,
						overlapPercent: est,
						commonStocks: [],
					});
					totalOverlap += est;
	...
	private mapPythonOverlapToResult(
		...
		for (const row of matrix) {
			for (const cell of row) {
				...
				if (cell.fundA !== cell.fundB && cell.overlap > 20 && !seen.has(key)) {
					seen.add(key);
					const nameA =
						mfHolds.find((h) => h.isin === cell.fundA)?.schemeName ??
						cell.fundA;
					const nameB =
						mfHolds.find((h) => h.isin === cell.fundB)?.schemeName ??
						cell.fundB;
					details.push({
						scheme1: nameA,
						scheme2: nameB,
						overlapPercent: Math.round(cell.overlap),
						commonStocks: [],
					});
		...
		const topStocks = (pyResult.topStocks ?? [])
			.map((s: any) => s.stock)
			.slice(0, 5);
	...
	analyzeAllocationDeviation(
		...
		const normalisedCategory =
			riskCategory === "very_aggressive" ? "aggressive" : riskCategory;
		const ideal = (IDEAL_ALLOCATIONS[normalisedCategory] ||
			IDEAL_ALLOCATIONS.moderate) as Record<
			string,
			{ min: number; max: number; target: number }
		>;
		const deviation: Record<
			string,
	...
	async runPortfolioDiagnostics(userId: string): Promise<PortfolioDiagnostics> {
		...
		const issueCount = {
			critical: concentrationIssues.filter((i) => i.severity === "critical")
				.length,
			warning:
				concentrationIssues.filter((i) => i.severity === "warning").length +
				(overlapPercent > 30 ? 1 : 0),
			info: 0,
		};
		...
		const diagnosticsData = {
			...
			idealAllocation: IDEAL_ALLOCATIONS[
				riskProfile.riskCategory as keyof typeof IDEAL_ALLOCATIONS
			] as any,
		...
		const [created] = await db
			.insert(portfolioDiagnostics)
			.values(diagnosticsData as any)
			.returning();
	...
	private generateHealthSummary(
		...
		if (concentrationIssues.length > 0) {
			const criticalCount = concentrationIssues.filter(
				(i) => i.severity === "critical",
			).length;
			if (criticalCount > 0) {
				parts.push(
					`There are ${criticalCount} critical concentration issues that need immediate attention.`,
				);
			} else {
				parts.push(
					`There are ${concentrationIssues.length} concentration warnings to review.`,
				);
			}
		}
		...
		if (overlapPercent > 30) {
			parts.push(
				"High overlap detected between your mutual fund holdings. Consider consolidating similar schemes.",
			);
		...
		if (Math.abs(riskMismatch) > 2) {
			if (riskMismatch > 0) {
				parts.push(
					"Your portfolio carries more risk than your stated risk tolerance.",
				);
			} else {
				parts.push(
					"Your portfolio is more conservative than your risk tolerance suggests.",
				);
			}
		}
	...
	async generateRecommendations(
		...
		const diagnostics = await db
			...
			.where(eq(portfolioDiagnostics.id, diagnosticsId))
			.limit(1);
		...
		const concentrationIssues =
			(diag.concentrationIssues as ConcentrationIssue[]) || [];
		const allocationDeviation = diag.allocationDeviation as Record<
			string,
			{ current: number; target: number; deviation: number }
		>;
		const mfOverlapPercent = Number.parseFloat(
			diag.mfOverlapPercent?.toString() || "0",
		);
		const riskMismatch = Number.parseFloat(
			diag.riskMismatchPercent?.toString() || "0",
		);

		for (const issue of concentrationIssues) {
			if (issue.type === "single_stock" && issue.severity === "critical") {
				const holding = (snapshot.holdings as any[]).find(
					(h: any) => h.schemeName === issue.name,
				);
				if (holding) {
					const sellAmount =
						holding.currentValue *
						(1 - issue.limitPercent / issue.currentPercent);

					// T008: FIFO-based tax estimate
					const taxInfo = await this.computeTaxEstimate(
						userId,
						holding.isin,
						sellAmount,
					);
					...
					recommendations.push({
						...
						riskImpactPercent:
							"-" +
							Math.round((issue.currentPercent - issue.limitPercent) / 2) +
							"%",
						priority: 1,
					});
				}
		...
		if (mfOverlapPercent > 30) {
			const mfSnapshotHoldings = (snapshot.holdings as any[]).filter(
				(h: any) => h.assetType === "mutual_fund",
			);
			if (mfSnapshotHoldings.length >= 2) {
				const smallerHolding = mfSnapshotHoldings.sort(
					(a: any, b: any) => a.currentValue - b.currentValue,
				)[0];
				...
				try {
					...
					console.warn(
						"[ProposalEngine] SWITCH target fund DB lookup failed:",
						e,
					);
				}
				...
				recommendations.push({
					...
					riskInvolved:
						"Switching may incur exit loads and tax implications. Verify exit load period before executing.",
		...
		try {
			const [clientProf] = await db
				.select()
				.from(clientRiskProfiles)
				.where(eq(clientRiskProfiles.userId, userId))
				.limit(1);
			const riskScorePct = Math.min(
				100,
				Math.max(1, Math.round((diag.portfolioRiskScore ?? 5) * 10)),
			);
			const horizonYears = clientProf?.timeHorizonYears ?? 5;
			const objectives =
				(clientProf?.investmentObjectives as string[] | null) ?? [];
			const goalType = objectives.includes("income")
				? "income"
				: objectives.includes("preservation")
					? "preservation"
					: objectives.includes("growth")
						? "growth"
						: "balanced";
			...
			if (pyAlloc?.allocations?.length && allocationDeviation) {
				const ASSET_TYPE_MAP: Record<string, string> = {
					...
					mid_cap_equity: "equity",
					small_cap_equity: "equity",
					multi_cap_equity: "equity",
					international_equity: "equity",
					flexi_cap_equity: "equity",
					government_bonds: "debt",
					corporate_bonds: "debt",
					liquid_funds: "debt",
					short_duration_debt: "debt",
					gold: "gold",
				...
				for (const [assetClass, pyTarget] of Object.entries(pyTargets)) {
					if (!allocationDeviation[assetClass]) {
						const currentPct =
							(snapshot.assetAllocation as any)?.[assetClass]?.percentage || 0;
						allocationDeviation[assetClass] = {
							current: currentPct,
							target: pyTarget,
							deviation: currentPct - pyTarget,
						};
					}
				}
			}
			...
			for (const [assetClass, dev] of Object.entries(allocationDeviation)) {
				if (dev.deviation > 15) {
					recommendations.push({
						...
						riskImpactPercent:
							"-" + Math.round(Math.abs(dev.deviation) / 3) + "%",
						priority: 3,
					});
					...
					recommendations.push({
						...
						riskImpactPercent:
							"+" + Math.round(Math.abs(dev.deviation) / 4) + "%",
						priority: 3,
					});
				}
			}
		...
		if (riskMismatch > 2) {
			recommendations.push({
				...
				rationale:
					"Reduce overall portfolio risk by decreasing equity exposure.",
				...
				portfolioImpactSummary:
					"Aligns portfolio risk with your stated risk tolerance.",
		...
		if (cashPercent > 10) {
			recommendations.push({
				...
				rationale:
					"Deploy excess cash into a balanced fund for better returns while maintaining flexibility.",
				...
				portfolioImpactSummary:
					"Improves capital deployment and potential returns.",
				riskImpactPercent: "+2%",
				priority: 4,
			});
		}
	...
	async createProposal(
		...
		const newTotalValue = Math.max(
			1,
			totalValue + totalInvestment - totalRedemption,
		);
		...
		for (const [key, absValue] of Object.entries(buckets)) {
			afterAllocation[key] =
				Math.round((absValue / newTotalValue) * 10000) / 100; // 2dp %
		}

		// T007: aiModelUsed reflects actual engine
		const aiModelUsed = process.env.PYTHON_SERVICE_URL
			? "python-sidecar-v3.0.0"
			: "rule-based-v2.0.0";
		...
		const proposalData: InsertAiProposal = {
			...
			proposalNumber,
			title:
				title || `Investment Proposal - ${new Date().toLocaleDateString()}`,
	...
	async getProposal(
		proposalId: string,
	): Promise<{ proposal: AiProposal; items: AiProposalItem[] } | null> {
		...
		const items = await db
			.select()
			.from(aiProposalItems)
			.where(eq(aiProposalItems.proposalId, proposalId))
			.orderBy(aiProposalItems.priority);
	...
	async getClientProposals(clientId: string): Promise<AiProposal[]> {
		return db
			.select()
			.from(aiProposals)
			.where(eq(aiProposals.clientId, clientId))
	...
	async getAgentProposals(agentId: string): Promise<AiProposal[]> {
		return db
			.select()
			.from(aiProposals)
			.where(eq(aiProposals.agentId, agentId))
			.orderBy(desc(aiProposals.createdAt));
	}
	...
	async updateProposalItem(
		...
		updates: {
			amount?: number;
			status?: string;
			agentModificationReason?: string;
		},
		actorId: string,
		actorRole: string,
	...
	async submitProposalToClient(
		proposalId: string,
		agentId: string,
	): Promise<AiProposal> {
		return this.updateProposalStatus(
			proposalId,
			"pending_review",
			agentId,
			"agent",
	...
	async clientApproveItem(
		itemId: string,
		clientId: string,
	): Promise<AiProposalItem> {
		return this.updateProposalItem(
			itemId,
			{ status: "approved" },
			clientId,
			"client",
	...
	private async rollbackCartItem(cartItemId: string): Promise<void> {
		try {
			await db
				.delete(unifiedCartItems)
				.where(eq(unifiedCartItems.id, cartItemId));
		} catch (error) {
	...
	private async addSwitchSellToCart(
		...
		try {
			...
			console.error(
				`Failed to add switch sell leg for item ${item.id}:`,
				error,
			);
			return { success: false, error: error.message };
		}
	...
	async finalizeProposalApproval(
		proposalId: string,
		clientId: string,
	): Promise<AiProposal> {
		const items = await db
		...
		for (const item of approvedItems) {
			...
			// Handle SWITCH items - add both sell (old fund) and buy (new fund) atomically
				...
				const buyResult = await this.addItemToCart(
					item,
					clientId,
					proposalId,
					true,
				...
				if (item.switchFromIsin || item.switchFromSchemeName) {
					const sellResult = await this.addSwitchSellToCart(
						item,
						clientId,
						proposalId,
					);
					...
					await this.markItemExecuted(item.id, [
						buyResult.cartItemId,
						sellResult.cartItemId,
					]);
		...
		try {
			...
			try {
				...
				if (fundInfo?.category) {
					isEquity =
						!fundInfo.category.toLowerCase().includes("debt") &&
				...
				/* keep default isEquity = true */
			}

			const invested = Number.parseFloat(
				holding.investedValue?.toString() || "0",
			);
			const current = Number.parseFloat(
				holding.currentValue?.toString() || "0",
			);
			...
			const estimatedTax = Math.max(
				0,
				Math.round((estimatedGain * taxRate) / 100),
			);
	...
	private async logAuditEntry(
		entry: Omit<InsertAiAuditLog, "timestamp">,
	): Promise<void> {
		try {
	...
	async getAuditLogs(proposalId: string): Promise<any[]> {
		return db
			.select()
			.from(aiAuditLogs)
			.where(eq(aiAuditLogs.proposalId, proposalId))
	...
	async getLatestDiagnostics(
		...
		const [result] = await db
			...
			.orderBy(desc(portfolioDiagnostics.analysisDate))
			.limit(1);
		return result || null;
	}
	...
	async getSmartMFRecommendationsForProposal(
		...
		const riskCategory = riskProfile.riskCategory as
			| "conservative"
			| "moderate"
			| "aggressive";
		...
		const allocations = {
			conservative: { equity: 0.2, debt: 0.6, hybrid: 0.15, commodity: 0.05 },
			moderate: { equity: 0.5, debt: 0.3, hybrid: 0.15, commodity: 0.05 },
			aggressive: { equity: 0.7, debt: 0.15, hybrid: 0.1, commodity: 0.05 },
		};
		...
		for (const fund of aiRecommendations.equityFunds.slice(0, 3)) {
			const amount = Math.round(
				(investmentAmount * allocation.equity) /
					aiRecommendations.equityFunds.length,
			);
			recommendations.push({
				type: "BUY",
				assetClass: "mutual_fund",
		...
		for (const fund of aiRecommendations.debtFunds.slice(0, 2)) {
			const amount = Math.round(
				(investmentAmount * allocation.debt) /
					aiRecommendations.debtFunds.length,
			);
			recommendations.push({
				type: "BUY",
				assetClass: "mutual_fund",
		...
		if (aiRecommendations.commodityFunds.length > 0) {
			...
			recommendations.push({
				...
				rationale:
					fund.rationale +
					" Provides downside protection during market volatility.",
	...
	async analyzeUserMFPortfolio(userId: string) {
		...
		const holdingsData = userHoldings.map((h) => ({
			...
			units: Number.parseFloat(h.units?.toString() || "0"),
			category: undefined,
			fundHouse: undefined,
		}));
		...
		// Run AI analysis
		const analysis =
			await aiMFRecommendationService.analyzePortfolioHoldings(holdingsData);
