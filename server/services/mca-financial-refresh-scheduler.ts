...
class McaFinancialRefreshScheduler {
	...
	start(): void {
		...
		console.log(
			"[MCA Refresh] Starting daily financial data refresh scheduler...",
		);
	...
	async runRefreshCycle(): Promise<RefreshResult> {
		if (this.isRunning) {
			console.log(
				"[MCA Refresh] Refresh cycle already in progress, skipping...",
			);
			return {
				jobsProcessed: 0,
				jobsSuccessful: 0,
		...
		try {
			...
			try {
				...
				console.warn(
					`[MCA Refresh] Error identifying refresh jobs: ${jobError?.message || "Unknown"}`,
				);
				jobs = [];
			}
			console.log(
				`[MCA Refresh] Found ${jobs.length} companies needing refresh`,
			);
			...
			for (const job of jobs) {
				...
				try {
					const backfillResult =
						await mcaFinancialBackfillService.backfillFromCompanyFinancials(
							job.cin,
			...
			console.log(
				`[MCA Refresh] Cycle complete: ${result.jobsSuccessful}/${result.jobsProcessed} successful`,
			);
			...
			try {
				...
				console.warn(
					`[MCA Refresh] Stale companies refresh error: ${staleError?.message || "Unknown"}`,
				);
			}
		} catch (error: any) {
			console.error(
				"[MCA Refresh] Error during refresh cycle:",
	...
	private async refreshStaleCompaniesFromApi(): Promise<void> {
		try {
			console.log(
				"[MCA Refresh] Checking for stale companies needing API enrichment...",
			);
			...
			if (companiesRefreshed > 0) {
				console.log(
					`[MCA Refresh] Enriched ${companiesRefreshed}/${companiesChecked} stale companies from API`,
				);
			} else if (companiesChecked === 0) {
				console.log("[MCA Refresh] No stale companies found");
			} else {
				console.log(
					`[MCA Refresh] Failed to enrich stale companies: ${errors.slice(0, 3).join(", ")}`,
				);
			}
		} catch (error: any) {
			console.error(
				"[MCA Refresh] Error refreshing stale companies:",
				error.message,
			);
		}
	}
