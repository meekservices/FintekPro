...
interface TrainingPrompt {
  id: string;
  category: "OVERLAP" | "DIVERSIFICATION" | "SIP" | "REPLACEMENT" | "GOAL" | "GENERAL";
...
export class AdvisorTrainingService {
  ...
  generateTrainingPrompts(context: PortfolioContext): TrainingPrompt[] {
    ...
    const overlappingStocks = context.diversificationScore.stockExposures.filter(
      s => s.fundCount >= 2
    );
    ...
    if (overlappingStocks.length > 0) {
      prompts.push({
        ...
        suggestedApproach: "Use the analogy of putting eggs in different baskets. Explain that while the funds may have different names, they own similar stocks, which means the client's actual diversification is lower than it appears.",
        doNotSay: [
    ...
    if (context.diversificationScore.score < 70) {
      prompts.push({
        ...
        suggestedApproach: "Describe the score as a 'spread indicator' - how spread out the investments are. A higher score means investments are distributed across more different companies and sectors.",
        doNotSay: [
          "Your score is bad",
      ...
      prompts.push({
        ...
        suggestedApproach: "Explain that penalties are like deductions for concentration. When too much money is in similar stocks or sectors, the score is reduced to reflect this hidden concentration.",
        doNotSay: [
    ...
    if (context.sipRoutingApplied) {
      prompts.push({
        ...
        suggestedApproach: "Focus on the process benefit: 'The SIP is structured to gradually improve how spread out your investments are. Each month, more money goes to funds that add diversity rather than duplicate what you already own.'",
        doNotSay: [
      ...
      prompts.push({
        ...
        context: "Different funds receive different SIP amounts based on overlap analysis.",
        suggestedApproach: "Explain that funds holding unique stocks get more allocation because they add more diversity. Funds with stocks you already own heavily get less to avoid further concentration.",
        doNotSay: [
    ...
    if (context.replaceFundSuggestions && context.replaceFundSuggestions.length > 0) {
      ...
      prompts.push({
        ...
        suggestedApproach: "Focus on redundancy: 'These two funds own many of the same stocks. It's like having two copies of the same book. We can replace one with a fund that owns different stocks, giving you a broader library.'",
        doNotSay: [
          "This fund is bad",
    ...
    prompts.push({
      ...
      suggestedApproach: "Avoid timing discussions. Focus on systematic investing: 'Rather than trying to predict market movements, we focus on building a well-structured portfolio that can weather different market conditions.'",
      doNotSay: [
    ...
    prompts.push({
      ...
      suggestedApproach: "Acknowledge their concern: 'Change can feel risky. These suggestions are about improving structure, not criticizing past choices. You can make gradual adjustments over time.'",
      doNotSay: [
