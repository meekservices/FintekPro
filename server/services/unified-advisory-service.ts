...
export class UnifiedAdvisoryService {
	...
	constructor() {
		console.log(
			"✅ Unified Advisory Engine initialized with multi-provider AI support (Preferred: Groq)",
		);
	}

	async validateTriggerConditions(
		clientId: string,
		...
		const conditions: AdvisoryTriggerConditions = {
			onboardingComplete:
				user.isEmailVerified === true || user.isMobileVerified === true,
			panVerified: !!user.panNumber && user.panVerifiedViaSmartKyc === true,
			kycComplete:
				user.panVerifiedViaSmartKyc === true &&
				user.aadhaarVerifiedViaSmartKyc === true,
			...
			clientCategoryValidated:
				!!profile?.riskTolerance && !!profile?.annualIncomeAmount,
		};
	...
	async getClientProfile(clientId: string): Promise<ClientProfile | null> {
		...
		else if (riskTolerance === "aggressive" || riskTolerance === "high")
			riskCategory = "aggressive";
		else if (riskTolerance === "very_aggressive")
			riskCategory = "very_aggressive";

		const netWorth = Number.parseFloat(String(profile?.netWorthAmount)) || 0;
		...
		if (experience.includes("short") || experience.includes("less"))
			investmentHorizon = "short";
		else if (experience.includes("long") || experience.includes("5+"))
			investmentHorizon = "long";
		else if (experience.includes("very") || experience.includes("10+"))
		...
		return {
			...
			investmentHorizon,
			netWorth,
			annualIncome: Number.parseFloat(String(profile?.annualIncomeAmount)) || 0,
			onboardingComplete:
				user.isEmailVerified === true || user.isMobileVerified === true,
			panVerified: !!user.panNumber && user.panVerifiedViaSmartKyc === true,
	...
	async getEligibleProducts(clientId: string): Promise<{
		...
		for (const rule of PRODUCT_ELIGIBILITY_MATRIX) {
			...
			if (!rule.allowedRiskCategories.includes(profile.riskCategory)) {
				reasons.push(
					`Risk profile ${profile.riskCategory} not suitable for ${rule.productType}`,
				);
			}

			if (!rule.requiredClientCategories.includes(profile.clientCategory)) {
				reasons.push(
					`Client category ${profile.clientCategory} not eligible. Required: ${rule.requiredClientCategories.join("/")}`,
				);
			}

			if (!isHorizonSufficient(profile.investmentHorizon, rule.minHorizon)) {
				reasons.push(
					`Investment horizon too short. Required: ${rule.minHorizon}+`,
				);
			}
	...
	async getPortfolioSummary(
		...
		if (portfolio) {
			...
			for (const holding of holdings) {
				...
				const avgPrice = Number.parseFloat((holding as any).avgPrice) || 0;
				...
				const currentPrice = market?.price
					? Number.parseFloat(market.price)
					: avgPrice;
		...
		const mfFolioRows = await db
			.select()
			.from(mfFolios)
			.where(eq(mfFolios.userId, clientId));
		for (const folio of mfFolioRows) {
			const mfRows = await db
				.select()
				.from(mfHoldings)
				.where(eq(mfHoldings.folioId, folio.id));
			for (const h of mfRows) {
				const currentValue = Number.parseFloat(
					h.currentValue?.toString() || "0",
				);
				const investedValue =
					Number.parseFloat(h.units?.toString() || "0") *
					Number.parseFloat(h.avgNav?.toString() || "0");
				totalValue += currentValue;
				totalInvested += investedValue;
				assetAllocation.MF = (assetAllocation.MF || 0) + currentValue;
				sectorAllocation["Mutual Fund"] =
					(sectorAllocation["Mutual Fund"] || 0) + currentValue;
				holdingDetails.push({
					symbol: h.schemeCode,
		...
		for (const sector of Object.keys(sectorAllocation)) {
			sectorAllocation[sector] =
				totalValue > 0 ? (sectorAllocation[sector] / totalValue) * 100 : 0;
		}
		for (const asset of Object.keys(assetAllocation)) {
			assetAllocation[asset] =
				totalValue > 0 ? (assetAllocation[asset] / totalValue) * 100 : 0;
		}

		const maxConcentration = Math.max(...Object.values(sectorAllocation), 0);
		const topHoldingWeight =
			holdingDetails.length > 0
				? Math.max(...holdingDetails.map((h: any) => h.weight))
				: 0;
		const equityWeight =
			(assetAllocation.EQUITY || assetAllocation.equity || 0) +
			(assetAllocation.MF || 0) * 0.7;
		const riskScore = Math.min(
			100,
			Math.round(
				maxConcentration * 0.3 + topHoldingWeight * 0.3 + equityWeight * 0.4,
			),
		);
	...
	async generateRecommendations(
		...
		if (!validation.canProceed) {
			throw new Error(
				`Advisory blocked: ${validation.blockerReasons.join("; ")}`,
			);
		}
		...
		try {
			const regimeResult = await callPython<any>("/api/regime/detect", "POST", {
				lookback_days: 90,
			});
			if (regimeResult?.regime) {
				...
				console.log(
					`[UnifiedAdvisory] Market regime: ${marketRegime.regime} (score=${marketRegime.signal_score}, confidence=${marketRegime.confidence})`,
				);
			}
		} catch {
			console.warn(
				"[UnifiedAdvisory] Regime detection unavailable — proceeding without regime context",
			);
		}
	...
	private async generateProductRecommendations(
		...
		const logic = PRODUCT_ADVISORY_LOGIC.find(
			(l: any) => l.productType === productType,
		);
		...
		try {
			...
			console.error(
				`[UnifiedAdvisory] Gemini supplement failed for ${productType}:`,
				error,
			);
		}
...
- Top Holdings: ${portfolio.holdings
				.slice(0, 5)
				.map((h: any) => `${h.name} (${h.weight.toFixed(1)}%)`)
				.join(", ")}`
...
- If regime is bear or high_vol, prefer defensive products`;
		...
		try {
			...
			console.error(
				"[UnifiedAdvisory] Gemini returned invalid JSON:",
				text.slice(0, 300),
			);
			return [];
		}
		...
		for (const rec of parsed.recommendations || []) {
			recommendations.push({
				...
				portfolioImpact: {
					...
					riskBefore: (rec.riskBefore ||
						this.mapRiskScore(portfolio?.riskScore || 50)) as any,
	...
	private async generateRuleBasedRecommendations(
		...
		const isBearRegime =
			marketRegime &&
			(marketRegime.regime === "bear" || marketRegime.regime === "high_vol");
		const effectiveCount = isBearRegime ? Math.max(1, count - 1) : count;

		const productDatabase = await this.fetchDBFundCatalogue(
			productType,
		...
		for (let i = 0; i < Math.min(effectiveCount, productDatabase.length); i++) {
			...
			const estimatedImpact =
				action === "BUY" ? 1.5 : action === "SELL" ? -0.5 : 0;
			...
			recommendations.push({
				...
				portfolioImpact: {
					...
					riskAfter: this.mapRiskScore(
						(portfolio?.riskScore || 50) + (action === "BUY" ? 5 : -5),
					),
				},
				primaryReason,
				supportingFactors: [
					...
					...(marketRegime
						? [
								`Market regime: ${marketRegime.regime} (score: ${marketRegime.signal_score.toFixed(2)})`,
							]
						: []),
					...(product.schemeCode
						? [
								`Scheme: ${product.schemeCode}`,
								`AMC: ${product.fundHouse || ""}`,
							]
						: []),
				].filter(Boolean),
	...
	> {
		if (productType === "MF") {
			...
			try {
				...
				if (rows.length > 0) {
					return rows.map((r: any) => ({
						...
						riskFactors: [
							...
							...(r.sharpe
								? [
										`Sharpe ratio: ${Number.parseFloat(r.sharpe.toString()).toFixed(2)}`,
									]
								: []),
						],
					}));
				...
				console.warn(
					"[UnifiedAdvisory] DB fund catalogue query failed, using fallback:",
					err,
				);
			}
			...
			return [
				{
					...
					riskFactors: ["Market risk", "Tracking error"],
				},
				{
					symbol: "PPFASFLEX",
					name: "Parag Parikh Flexi Cap Fund",
					riskFactors: ["Currency risk", "Market risk"],
				},
				{
					symbol: "SBILARGE",
					name: "SBI Bluechip Fund",
					riskFactors: ["Market risk", "Fund manager risk"],
				},
			];
		}

		// Non-MF product types use a static catalogue (no separate DB table in scope)
		const staticCatalogue: Record<
			string,
			Array<{
				symbol: string;
				name: string;
		...
		> = {
			STOCK: [
				{
					...
					name: "HDFC Bank Ltd",
					isin: "INE040A01034",
					riskFactors: ["Interest rate risk", "Credit cycle exposure"],
				},
				{
					symbol: "RELIANCE",
					name: "Reliance Industries",
					isin: "INE002A01018",
					riskFactors: ["Oil price volatility", "High capex"],
				},
				{
					symbol: "TCS",
					name: "Tata Consultancy Services",
					isin: "INE467B01029",
					riskFactors: ["Currency risk", "IT spending slowdown"],
				},
				{
					symbol: "INFY",
					name: "Infosys Ltd",
				...
				},
				{
					...
					name: "ICICI Bank Ltd",
					isin: "INE090A01021",
					riskFactors: ["Credit risk", "NPA exposure"],
				},
			],
			BOND: [
				{
					...
					name: "GOI 7.26% 2030",
					isin: "IN0020220017",
					riskFactors: ["Interest rate risk", "Duration risk"],
				},
				{
					symbol: "ICICIBOND",
					name: "ICICI Bank 8.5% NCD",
					riskFactors: ["Credit risk", "Liquidity risk"],
				},
				{
					symbol: "HDFCNCD",
					name: "HDFC Ltd 8.75% NCD",
					riskFactors: ["Credit risk", "Interest rate risk"],
				},
				{
					symbol: "TBILL91",
					name: "Treasury Bill 91 Days",
					riskFactors: ["Low yield", "Reinvestment risk"],
				},
			],
			UNLISTED: [
				{
				...
				},
				{
					...
					riskFactors: ["Regulatory risk", "Pre-IPO pricing risk"],
				},
			],
			MLD: [
				{
					symbol: "HDFCMLD",
					name: "HDFC Nifty Linked MLD",
					riskFactors: [
						"Capital at risk",
						"Complex payoff",
						"Issuer credit risk",
				...
				{
					symbol: "ABORAMLD",
					name: "Aditya Birla Equity Linked MLD",
					riskFactors: ["Market-linked returns", "Early exit penalty"],
				},
			],
			PMS: [
				{
					...
					riskFactors: ["Manager discretion", "Capital risk", "High fees"],
				},
				{
					symbol: "ASWPMS",
					name: "ASK India Select PMS",
					riskFactors: ["Concentrated portfolio", "Liquidity constraints"],
				},
			],
			AIF: [
				{
					...
					riskFactors: ["Lock-in 5 years", "High minimum", "Capital calls"],
				},
				{
					symbol: "ICICICREDIT",
					name: "ICICI Prudential Credit AIF",
					riskFactors: ["Credit risk", "Illiquidity", "Long lock-in"],
				},
			],
			CFD: [
				{
					symbol: "SPYCFD",
					name: "S&P 500 CFD",
					riskFactors: ["Leverage risk", "Currency risk", "Not SEBI regulated"],
				},
				{
					symbol: "GOLDCFD",
					name: "Gold CFD",
			...
			TREASURY: [
				...
				},
				{
					symbol: "LIQUIDFUND",
					name: "Liquid Fund",
					riskFactors: ["Credit risk", "Interest rate sensitivity"],
				},
			],
		};
	...
	private calculateSuggestedAmount(
		...
		const eligibilityRule = PRODUCT_ELIGIBILITY_MATRIX.find(
			(r: any) => r.productType === productType,
		);
	...
	async createProposalFromRecommendations(
		...
		const totalAmount = approvedDecisions.reduce(
		...
		for (const decision of approvedDecisions) {
			const allocationPct =
				totalAmount > 0 ? (decision.amount / totalAmount) * 100 : 0;
			await db.insert(investmentProposalItems).values({
				proposalId: proposal.id,
				productType: decision.productType.toLowerCase(),
				productCode:
					decision.productSymbol ||
					decision.productName.replace(/\s/g, "_").toUpperCase(),
		...
		await this.logAuditEvent({
			...
			metadata: {
				proposalId: proposal.id,
				decisions: approvedDecisions.map((d: any) => d.decisionId),
			},
			retentionYears: 8,
		});
	...
	getExecutionChannel(productType: ProductType): ExecutionChannel {
		const rule = PRODUCT_ELIGIBILITY_MATRIX.find(
			(r: any) => r.productType === productType,
		);
	...
	async getAuditTrail(
		...
		try {
			...
			return rows.map((row: any) => {
				...
				return {
					...
					eventType: (row.action ||
						"trigger_check") as AdvisoryAuditLog["eventType"],
			...
			console.error(
				"[UnifiedAdvisory] getAuditTrail DB query failed, falling back to in-memory:",
				err,
			);
