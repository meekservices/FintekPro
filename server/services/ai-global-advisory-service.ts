...
class AIGlobalAdvisoryService {
	private recommendationCache = new Map<
		string,
		{ data: GlobalRecommendation[]; timestamp: Date }
	>();
	...
	constructor() {
		...
		console.log(
			`✅ AI Global Advisory Service initialized via Unified Engine (primary: ${status.primary})`,
		);
	}
		...
		for (let attempt = 0; attempt <= maxRetries; attempt++) {
			try {
				...
				if (isRateLimited && attempt < maxRetries) {
					...
					console.log(
						`[GlobalAdvisory] Rate limited, retrying in ${Math.round(delay)}ms (attempt ${attempt + 1}/${maxRetries})`,
					);
				...
				if (attempt === maxRetries) {
					console.error(
						`[GlobalAdvisory] Failed after ${maxRetries} retries: ${error.message}`,
					);
				}
				throw error;
			}
	...
	async fetchGlobalInstrumentData(
		symbol: string,
	): Promise<GlobalInstrumentData | null> {
		try {
			const { alpacaMarketDataService } = await import(
			...
			const exchangeRate = await currencyExchangeService.fetchExchangeRates(
				currency,
				"INR",
			);
			...
			if (
				symbol.endsWith("ETF") ||
				["SPY", "QQQ", "VOO", "DIA"].includes(symbol)
			)
				assetClass = "etf";
			...
			return {
				...
				lastEnrichedAt: enriched?.lastEnrichedAt
					? new Date(enriched.lastEnrichedAt)
					: undefined,
			} as any;
		} catch (error: any) {
			console.error(
				`[GlobalAdvisory] Failed to fetch data for ${symbol}: ${error.message}`,
			);
			return null;
		}
	}
	...
	async getGlobalBondRecommendations(
		...
		for (const symbol of symbols) {
			...
			const recommendation = await this.generateRecommendation(
				data,
				filters.riskLevel || "conservative",
			);
			if (recommendation) {
				recommendations.push(recommendation);
			}
	...
	async getFilteredGlobalRecommendations(
		...
		if (cached && Date.now() - cached.timestamp.getTime() < this.CACHE_TTL_MS) {
			...
			return this.organizeRecommendations(
				recommendations,
		...
		for (const [market, instrumentTypes] of Object.entries(
			globalAdvisorySelections,
		)) {
			...
			for (const instrumentType of instrumentTypes) {
				if (!validInstruments.includes(instrumentType)) {
					validationErrors.push(
						`Unknown instrument type: ${instrumentType} for market ${market}`,
					);
					continue;
				}
				...
				if (symbols.length === 0) {
					validationErrors.push(
						`No symbols available for ${instrumentType} in ${market}`,
					);
					continue;
				}
		...
		for (let i = 0; i < symbolsToFetch.length; i += batchSize) {
			...
			const batchPromises = batch.map(
				async ({ symbol, market, instrumentType }) => {
					const data = await this.fetchGlobalInstrumentData(symbol);
					if (!data) return null;
			...
			allRecommendations.push(
				...batchResults.filter((r): r is GlobalRecommendation => r !== null),
			);
	...
	private async organizeRecommendations(
		...
		const instrumentTypesIncluded = [
			...new Set(Object.values(globalAdvisorySelections).flat()),
		];

		const usdToInrRate = await currencyExchangeService.fetchExchangeRates(
			"USD",
			"INR",
		);
		...
		const normalizedScores = buyRecommendations.map(
			(r: any) => r.suitabilityScore ?? DEFAULT_SUITABILITY,
		);
		const totalSuitabilityScore = normalizedScores.reduce(
			(sum: any, score: any) => sum + score,
			0,
		);
		const hasBuyRecommendations =
			buyRecommendations.length > 0 && totalSuitabilityScore > 0;
		...
		if (hasBuyRecommendations) {
			for (let i = 0; i < buyRecommendations.length; i++) {
				...
				(r as any).suggestedAllocationPct =
					Math.round(suitabilityWeight * 10000) / 100;
			}
		}
		...
		const lrsStatus = {
			...
			warningMessage:
				totalUtilization > LRS_ANNUAL_LIMIT * 0.8
	...
	private async generateRecommendation(
		...
		const stopLoss =
			data.currentPrice * (recommendation.includes("buy") ? 0.92 : 0.88);
			...
			timeHorizon:
				riskLevel === "aggressive"
					...
					: riskLevel === "moderate"
						...
						: "long_term",
			timeHorizonDays:
				riskLevel === "aggressive" ? 90 : riskLevel === "moderate" ? 180 : 365,
	...
	private calculateScore(
		...
		if (data.peRatio) {
			if (data.peRatio < 15) {
				total += 15;
				keyFactors.push("Attractive valuation (low P/E)");
			} else if (data.peRatio < 25) {
				total += 8;
				keyFactors.push("Fair valuation");
			} else if (data.peRatio > 40) {
				total -= 10;
				riskFactors.push("High valuation concern");
			}
		}
		...
		if (data.week52High && data.week52Low && data.currentPrice) {
			...
			if (position < 0.3) {
				total += 10;
				keyFactors.push("Near 52-week low - potential upside");
		...
		if (data.sector) {
			...
			if (
				...
				defensiveSectors.includes(data.sector)
			) {
				total += 5;
		...
		if (advancedMetrics.priceToBook !== undefined) {
			if (
				advancedMetrics.priceToBook > 0 &&
				advancedMetrics.priceToBook < 1.5
			) {
				total += 5;
	...
	private calculateAdvancedGlobalMetrics(data: GlobalInstrumentData): {
		...
		try {
			...
			if (data.peRatio && data.peRatio > 0) {
				...
				const epsGrowthEstimate =
					data.priceChangePercent > 0
						? Math.min(30, data.priceChangePercent * 2)
						: 10;
			...
			console.error(
				"[GlobalAdvisory] Error calculating advanced metrics:",
				error,
			);
		}
	...
	private getRecommendationFromScore(
		score: number,
	...
	private generateRationale(
		...
		const direction = recommendation.includes("buy")
			? "positive"
			: recommendation === "hold"
				? "neutral"
				: "negative";
		return (
			`${data.name} (${data.symbol}) shows ${direction} indicators. ` +
	...
	private getTaxImplications(
		market: string,
	): GlobalRecommendation["taxImplications"] {
	...
	async generatePortfolioRebalancing(
		...
		for (const position of positions) {
			...
			const positionValueInr =
				positionValueNative *
				(await currencyExchangeService.fetchExchangeRates(
					data.currency,
					"INR",
				));
			...
			assetAllocation[assetType] =
				(assetAllocation[assetType] || 0) + positionValueInr;

			geographicAllocation[data.market] =
				(geographicAllocation[data.market] || 0) + positionValueInr;
			if (data.sector) {
				sectorAllocation[data.sector] =
					(sectorAllocation[data.sector] || 0) + positionValueInr;
			}
		}
		...
		for (const key of Object.keys(assetAllocation)) {
			assetAllocation[key] =
				totalValueInr > 0 ? (assetAllocation[key] / totalValueInr) * 100 : 0;
		}
		for (const key of Object.keys(geographicAllocation)) {
			geographicAllocation[key] =
				totalValueInr > 0
					? (geographicAllocation[key] / totalValueInr) * 100
					: 0;
		}
		for (const key of Object.keys(sectorAllocation)) {
			sectorAllocation[key] =
				totalValueInr > 0 ? (sectorAllocation[key] / totalValueInr) * 100 : 0;
		}
		...
		for (const position of positions) {
			...
			const currentAllocation =
				totalValueInr > 0 ? (positionValueInr / totalValueInr) * 100 : 0;
			...
			const tradeValueUsd =
				tradeValueInr /
				(await currencyExchangeService.fetchExchangeRates("USD", "INR"));
			...
			actions.push({
				...
				riskFactors:
					action === "buy" ? ["Currency risk on new investment"] : [],
				lrsImpact: action === "buy" ? tradeValueUsd : 0,
				complianceFlags: {
					lrsCheck:
						action === "buy" && tradeValueUsd > lrsRemaining
							? "block"
							: action === "buy" && tradeValueUsd > lrsRemaining * 0.8
								? "warning"
								: "pass",
					fatcaCheck: "pass",
				},
		...
		const totalBuyValueInr = buyActions.reduce(
			(sum: any, a: any) => sum + a.tradeValueInr,
			0,
		);
		const totalSellValueInr = sellActions.reduce(
			(sum: any, a: any) => sum + a.tradeValueInr,
			0,
		);
		const totalBuyValueUsd =
			totalBuyValueInr /
			(await currencyExchangeService.fetchExchangeRates("USD", "INR"));
		const lrsRemaining = LRS_ANNUAL_LIMIT_USD - lrsUtilizedYtdUsd;
...
Focus on actionable advice for an Indian investor investing globally.`;
			...
			const { result } = await unifiedAIRecommendationEngine.runPrompt<string>({
				...
				responseParser: (text: string) =>
					text ||
					"Portfolio analysis complete. Review the recommended actions.",
