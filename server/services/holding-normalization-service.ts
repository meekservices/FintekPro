...
const amfiLookupCache = new Map<
	string,
	{ result: AmfiLookupResult | null; timestamp: number }
>();
...
class HoldingNormalizationService {
	normalizeAssetType(type: string | undefined | null): AssetType {
		...
		if (
			...
			normalized.includes("ncd")
		)
			return "debt";
		if (
			normalized.includes("gold") ||
			normalized.includes("sgb") ||
		...
		if (
			...
			return "hybrid";
		if (normalized.includes("fd") || normalized.includes("deposit"))
			return "fd";
		if (normalized.includes("pms")) return "pms";
		if (normalized.includes("aif") || normalized.includes("alternative"))
			return "aif";
		if (normalized.includes("reit")) return "reit";
		if (normalized.includes("invit") || normalized.includes("infrastructure"))
			return "invit";
		if (normalized.includes("unlisted") || normalized.includes("pre-ipo"))
			return "unlisted";
	...
	normalizeAssetTypeFromMFCategory(category: string | undefined): AssetType {
		...
		if (
			...
			cat.includes("dynamic bond") ||
			cat.includes("floater") ||
			cat.includes("short duration") ||
			cat.includes("medium duration") ||
			cat.includes("long duration") ||
		...
		if (cat.includes("fund of funds") || cat.includes("fof"))
			return "mutual_fund";
	...
	deriveAllocationFromHoldings(
		...
		Object.keys(allocation).forEach((key) => {
			allocation[key as keyof AllocationBreakdown] =
				Math.round(allocation[key as keyof AllocationBreakdown] * 100) / 100;
		});
	...
	computeSummary(holdings: UnifiedHolding[]): UnifiedPortfolioSummary {
		...
		holdings.forEach((h) => {
			totalInvestedValue +=
				h.investedValue || (h.avgCostPerUnit || 0) * h.quantity;
		...
		return {
			...
			totalUnrealizedGainPercent:
				Math.round(totalUnrealizedGainPercent * 100) / 100,
	...
	private isValidIsin(isin: string): boolean {
		...
		for (let i = digits.length - 1; i >= 0; i--) {
			let d = Number.parseInt(digits[i]);
			if (isSecond) {
				d *= 2;
				if (d > 9) d -= 9;
			}
	...
	normalizeAndExtract(name: string): {
		...
		const { cleanName: afterIsin, isin } =
			this.extractIsinFromName(workingName);
		workingName = afterIsin;

		const { cleanName: afterFolio, folio } =
			this.extractFolioFromName(workingName);
	...
	detectPlanType(name: string): "Regular" | "Direct" | undefined {
		...
		if (
			...
			upperName.includes("-DIR") ||
			upperName.includes(" DIR ")
		) {
			return "Direct";
		}
		if (
			upperName.includes("REGULAR") ||
			upperName.includes("-REG") ||
			upperName.includes(" REG ")
		) {
			return "Regular";
		}
	...
	detectOptionType(name: string): "Growth" | "IDCW" | "Dividend" | undefined {
		const upperName = name.toUpperCase();
		if (
			upperName.includes("GROWTH") ||
			upperName.includes("-G") ||
			upperName.includes(" G ")
		) {
			return "Growth";
		}
		if (
			upperName.includes("IDCW") ||
			upperName.includes("DIV") ||
			upperName.includes("DIVIDEND")
		) {
	...
	calculateGains(holding: UnifiedHolding): UnifiedHolding {
		const investedValue =
			holding.investedValue || (holding.avgCostPerUnit || 0) * holding.quantity;
		const currentValue = holding.currentValue || 0;

		const unrealizedGain = currentValue - investedValue;
		const unrealizedGainPercent =
			investedValue > 0 ? (unrealizedGain / investedValue) * 100 : 0;
	...
	async lookupAmfiScheme(query: {
		isin?: string;
		name?: string;
	}): Promise<AmfiLookupResult | null> {
	...
	async enrichWithAmfi(
		holding: Partial<UnifiedHolding>,
			...
			if (amfiResult && amfiResult.confidence >= 80) {
				return {
					...
					confidenceScore: Math.max(
						enriched.confidenceScore || 0,
						amfiResult.confidence,
					),
				};
			}
		}
	...
	async enrichByIsin(isin: string): Promise<IsinEnrichmentResult> {
		...
		try {
			...
			let exitLoadPercent = fund.exitLoadPercent
				? Number.parseFloat(fund.exitLoadPercent)
				: undefined;
			let exitLoadDays = fund.exitLoadDays ?? undefined;

			if (!exitLoadPercent && extData?.exitLoad) {
				const exitLoadStr =
					typeof extData.exitLoad === "string" ? extData.exitLoad : "";
				...
				if (percentMatch) {
					exitLoadPercent = Number.parseFloat(percentMatch[1]);
				}
				if (daysMatch) {
					const value = Number.parseInt(daysMatch[1], 10);
			...
			return {
				...
				returns3y: fund.returns3y
					? Number.parseFloat(fund.returns3y)
					: undefined,
				returns5y: fund.returns5y
					? Number.parseFloat(fund.returns5y)
					: undefined,
	...
	private deriveCategoryFromName(name: string): string | undefined {
		...
		if (
			...
			lower.includes("flexicap") ||
			lower.includes("multi cap")
		)
			return "Equity - Flexi Cap";
		if (
			lower.includes("focused") ||
			lower.includes("thematic") ||
			lower.includes("sectoral")
		)
	...
	async enrichByIsins(
		isins: string[],
	): Promise<Map<string, IsinEnrichmentResult>> {
		...
		try {
			...
			for (const fund of funds) {
				const matchedIsin = validIsins.find(
					(isin) =>
						fund.isin === isin ||
				...
				if (matchedIsin && !results.has(matchedIsin)) {
					...
					results.set(matchedIsin, {
						...
						category:
							fund.category || this.deriveCategoryFromName(fund.schemeName),
						fundHouse: fund.fundHouse || undefined,
						currentNav: fund.nav ? Number.parseFloat(fund.nav) : undefined,
						navDate: fund.lastUpdated?.toISOString().split("T")[0],
						exitLoadPercent,
			...
			console.warn(
				"[HoldingNormalization] Batch ISIN enrichment failed:",
				error,
			);
		}
