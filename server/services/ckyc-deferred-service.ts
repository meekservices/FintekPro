...
class CkycDeferredService {
  ...
  async createDeferredCase(data: Omit<InsertCkycDeferredCase, 'slaDeadline'> & { slaHours?: number }): Promise<CkycDeferredCase> {
    const slaHours = data.slaHours || 72;
    const slaDeadline = ckycEnvironmentService.calculateSlaDeadline(slaHours);
    
    const [created] = await db.insert(ckycDeferredCases).values({
      ...data,
      slaDeadline,
    }).returning();
    
    console.log(`[CKYC Deferred] Created case for user ${data.userId}, SLA deadline: ${slaDeadline.toISOString()}`);
    
    return created;
  }

  async getDeferredCaseByUserId(userId: string): Promise<CkycDeferredCase | null> {
  ...
  async getAllDeferredCases(filters?: {
    ...
    if (filters?.assignedTo) {
      conditions.push(eq(ckycDeferredCases.assignedToAdmin, filters.assignedTo));
    }
  ...
  async getDeferredCaseById(caseId: string): Promise<DeferredCaseDetail | null> {
    const [result] = await db.select()
      .from(ckycDeferredCases)
  ...
  async takeAdminAction(caseId: string, action: AdminAction): Promise<CkycDeferredCase> {
    ...
    if (action.action === 'manual_kyc_initiated' || action.action === 'vkyc_scheduled') {
    ...
    const [updated] = await db.update(ckycDeferredCases)
      .set({
        status: newStatus,
        ...
        resolvedAt,
        resolutionMethod: action.action === 'resolved' ? 'admin_override' : null,
        updatedAt: now,
      })
      .where(eq(ckycDeferredCases.id, caseId))
      .returning();
  ...
  async checkAndEscalateSLABreaches(): Promise<number> {
    ...
    for (const c of breachedCases) {
      ...
      console.warn(`[CKYC Deferred] SLA breach for case ${c.id}, escalating to compliance head`);
    }
  ...
  async blockTransactionForDeferredUser(userId: string): Promise<{ blocked: boolean; message?: string }> {
    const deferredCase = await this.getDeferredCaseByUserId(userId);
    
    if (deferredCase) {
      return {
        blocked: true,
        message: 'Your CKYC verification is pending. Some transactions are restricted until verification is complete. Our team is working on your case.',
      };
    }
  ...
  private toCaseSummary(c: CkycDeferredCase): DeferredCaseSummary {
    const agingBucket = ckycEnvironmentService.getSlaAgingBucket(new Date(c.slaStartedAt));
