...
import {
	nseIndiaProviderInstance,
	bseIndiaProviderInstance,
} from "./market-movers-cache";
...
class DataEnrichmentService {
	...
	getSourceConfidence(source: DataSource): number {
		...
		const confidenceMap: Record<DataSource, number> = {
			...
			bse: 0.9, // BSE exchange filings (good for SME and unlisted tracking)
			nse_bse: 0.9, // Combined exchange filings (legacy)
			finnhub: 0.75, // Limited Indian coverage
			google_finance: 0.7, // Google Finance HTML parsing (price + key ratios)
			yahoo: 0.65, // Some Indian stocks via .NS/.BO suffix
			manual: 0.5, // Manual overrides
		};
	...
	async fetchFromNSEBSE(
      ...
      `);
			...
			for (const filing of pendingFilings.rows as any[]) {
				console.log(
					`[DataEnrichment] Processing XBRL filing ${filing.id} for ${companyId}`,
				);
				try {
					...
					console.error(
						`[DataEnrichment] XBRL parsing error for ${filing.id}: ${parseError.message}`,
					);
					await exchangeFilingsService.updateFilingStatus(
						filing.id,
						"failed",
      ...
      `);
			...
			for (const row of rows) {
				if (!metrics[row.metric]) {
					metrics[row.metric] = {
						value: Number.parseFloat(row.metric_value) || null,
						source: "nse_bse",
						retrievedAt: new Date(row.created_at),
						confidenceScore:
							Number.parseFloat(row.extraction_confidence) || 0.95,
					};
				}
			}
			...
			const avgConfidence =
				rows.length > 0
					? rows.reduce(
							(sum: any, r: any) =>
								sum + (Number.parseFloat(r.extraction_confidence) || 0.95),
							0,
						) / rows.length
					: 0;
	...
	async fetchFromMCA(
		...
		try {
			...
			const { mcaIntelligenceService } = await import(
				"./mca-intelligence-service"
			);
			const financialHistory = await mcaIntelligenceService.getFinancialHistory(
				cin,
				3,
			);

			if (
				financialHistory.hasData &&
				financialHistory.financialYears.length > 0
			) {
				...
				if (
					latestYear.totalLiabilities !== null &&
					latestYear.totalLiabilities > 0
				) {
				...
				if (
					latestYear.totalBorrowing !== null &&
					latestYear.totalBorrowing > 0
				) {
					metrics.totalDebt = {
				...
				console.log(
					`[DataEnrichment] Fetched ${metricsCount} metrics from MCA Intelligence Service for ${cin}`,
				);
			...
			console.log(
				`[DataEnrichment] No MCA Intelligence data, falling back to basic MCA service for ${cin}`,
			);
			...
			if (mcaData.charges && mcaData.charges.length > 0) {
				const activeCharges = mcaData.charges.filter(
					(c: any) =>
						c.status &&
						c.status.toLowerCase() !== "closed" &&
						c.status.toLowerCase() !== "satisfied",
				);
				const totalDebt = activeCharges.reduce(
					(sum: any, c: any) => sum + toNumber(c.chargeAmount),
					0,
				);
			...
			if (metricsCount > 0) {
				console.log(
					`[DataEnrichment] Fetched ${metricsCount} metrics from basic MCA for ${cin}`,
				);
			} else {
				console.log(
					`[DataEnrichment] MCA data found but no usable metrics for ${cin} (capital values may be zero or invalid)`,
				);
			}
	...
	async fetchFromBSE(
		...
		if (!scripcode && !symbol) {
			console.log(
				`[DataEnrichment] No scripcode or symbol for BSE fetch, skipping`,
			);
			return { metrics: {}, confidence: 0 };
		}
		...
		try {
			...
			if (scripcode) {
				try {
					const response = await fetch(
						`${baseUrl}/StockReachGraph/w?scripcode=${scripcode}&flag=0&fromdate=&todate=&seression=`,
						{ headers },
					);
					if (response.ok) {
			...
			if (!stockData && symbol) {
				try {
					...
					const searchResponse = await fetch(
						`${baseUrl}/Suggest/w?flag=1&query=${encodeURIComponent(normalizedSymbol)}`,
						{ headers },
					);
					if (searchResponse.ok) {
						...
						if (Array.isArray(searchResults) && searchResults.length > 0) {
							...
							if (match) {
								...
								console.log(
									`[DataEnrichment] BSE symbol ${symbol} resolved to scripcode ${foundScripcode} (${match.scrip_nm})`,
								);
								const response = await fetch(
									`${baseUrl}/StockReachGraph/w?scripcode=${foundScripcode}&flag=0&fromdate=&todate=&seression=`,
									{ headers },
								);
								if (response.ok) {
									stockData = await response.json();
			...
			if (!stockData) {
				console.log(
					`[DataEnrichment] No BSE data found for ${scripcode || symbol}`,
				);
				return { metrics: {}, confidence: 0 };
			}
			...
			if (stockData.CurrVal) {
				metrics.currentPrice = {
					value: Number.parseFloat(stockData.CurrVal) || 0,
					source: "bse",
					retrievedAt: now,
					confidenceScore,
				};
			...
			if (stockData.Chng) {
				metrics.priceChange = {
					value: Number.parseFloat(stockData.Chng) || 0,
					source: "bse",
					retrievedAt: now,
					confidenceScore,
				};
			...
			if (stockData.ChngPer) {
				metrics.priceChangePercent = {
					value: Number.parseFloat(stockData.ChngPer) || 0,
					source: "bse",
					retrievedAt: now,
					confidenceScore,
				};
			...
			if (stockData.MktCap) {
				metrics.marketCap = {
					value: Number.parseFloat(stockData.MktCap?.replace(/,/g, "")) || 0,
					source: "bse",
					retrievedAt: now,
					confidenceScore,
				};
			...
			if (stockData.PE) {
				metrics.peRatio = {
					value: Number.parseFloat(stockData.PE) || 0,
					source: "bse",
					retrievedAt: now,
					confidenceScore,
				};
			...
			if (stockData.PB) {
				metrics.pbRatio = {
					value: Number.parseFloat(stockData.PB) || 0,
					source: "bse",
					retrievedAt: now,
					confidenceScore,
				};
			...
			if (stockData.EPS) {
				metrics.eps = {
					value: Number.parseFloat(stockData.EPS) || 0,
					source: "bse",
					retrievedAt: now,
					confidenceScore,
				};
			...
			if (stockData.Div) {
				metrics.dividend = {
					value: Number.parseFloat(stockData.Div) || 0,
					source: "bse",
					retrievedAt: now,
					confidenceScore,
				};
			...
			const resolvedScripcode =
				scripcode || stockData._resolvedScripcode || stockData.scripcode;
			if (resolvedScripcode) {
				try {
					...
					if (financialsResponse.ok) {
						...
						if (
							financialsData?.Table &&
							Array.isArray(financialsData.Table) &&
							financialsData.Table.length > 0
						) {
							...
							if (latestFinancials.Revenue) {
								metrics.revenue = {
									value:
										Number.parseFloat(
											latestFinancials.Revenue?.replace(/,/g, ""),
										) || 0,
									source: "bse",
									retrievedAt: now,
									confidenceScore,
							...
							if (latestFinancials.PAT) {
								metrics.pat = {
									value:
										Number.parseFloat(
											latestFinancials.PAT?.replace(/,/g, ""),
										) || 0,
									source: "bse",
									retrievedAt: now,
									confidenceScore,
							...
							if (latestFinancials.NetProfit) {
								metrics.netProfit = {
									value:
										Number.parseFloat(
											latestFinancials.NetProfit?.replace(/,/g, ""),
										) || 0,
									source: "bse",
									retrievedAt: now,
									confidenceScore,
			...
			const metricsCount = Object.keys(metrics).length;
			console.log(
				`[DataEnrichment] Fetched ${metricsCount} metrics from BSE for ${scripcode || symbol}`,
			);
	...
	async fetchFromBSESME(
		...
		try {
			...
			const smeResponse = await fetch(
				`${baseUrl}/SMEGetTopGainersLosers/w?GLession=G&IndxID=S`,
				{ headers },
			);
			...
			if (smeData?.Table && Array.isArray(smeData.Table)) {
				...
				if (stock) {
					if (stock.LTP) {
						metrics.currentPrice = {
							value: Number.parseFloat(stock.LTP) || 0,
							source: "bse",
							retrievedAt: now,
							confidenceScore,
						};
					...
					if (stock.Chg) {
						metrics.priceChange = {
							value: Number.parseFloat(stock.Chg) || 0,
							source: "bse",
							retrievedAt: now,
							confidenceScore,
						};
					...
					if (stock.ChgPer) {
						metrics.priceChangePercent = {
							value: Number.parseFloat(stock.ChgPer) || 0,
							source: "bse",
							retrievedAt: now,
							confidenceScore,
						};
	...
	async fetchFromYahoo(
		...
		if (!symbol && !isin) {
			console.log(
				`[DataEnrichment] No symbol or ISIN for Yahoo fetch, skipping`,
			);
			return { metrics: {}, confidence: 0 };
		}
		...
		try {
			...
			for (const sym of symbolsToTry) {
				try {
					console.log(
						`[DataEnrichment] Trying Yahoo Finance with symbol: ${sym}`,
					);
					...
					console.log(
						`[DataEnrichment] Yahoo symbol ${sym} not found, trying next...`,
					);
				}
			}
			...
			try {
				...
				console.log(
					`[DataEnrichment] Yahoo quoteSummary failed: ${summaryError.message}`,
				);
			}

			console.log(
				`[DataEnrichment] Fetched ${Object.keys(metrics).length} metrics from Yahoo for ${usedSymbol}`,
			);
			return {
				metrics,
				confidence: Object.keys(metrics).length > 0 ? confidenceScore : 0,
	...
	async enrichCompanyFinancials(
		...
		if (!company.cin && company.isin) {
			try {
				console.log(
					`[DataEnrichment] Attempting ISIN→CIN discovery for ${company.isin}`,
				);
				auditTrail.push({
					id: crypto.randomUUID(),
					timestamp: new Date(),
				...
				if (cinLookup.cin) {
					...
					console.log(
						`[DataEnrichment] Discovered CIN ${discoveredCIN} from ISIN ${company.isin} (via ${cinLookup.source}, confidence: ${cinLookup.confidence})`,
					);
					...
					try {
						...
						console.log(
							`[DataEnrichment] Could not save discovered CIN (may be duplicate): ${saveError.message}`,
						);
						auditTrail.push({
							id: crypto.randomUUID(),
							timestamp: new Date(),
					...
					console.log(
						`[DataEnrichment] NSDL lookup failed, trying MCA name search for: ${(company as any).companyName || company.name}`,
					);

					try {
						const companyName =
							(company as any).companyName || company.name || "";
						if (companyName && mcaService.isConfigured()) {
							const mcaSearchResults =
								await mcaService.searchCompanyByName(companyName);

							if (mcaSearchResults.length > 0) {
								// Take the best match (first result, highest confidence)
								discoveredCIN = mcaSearchResults[0].cin;
								console.log(
									`[DataEnrichment] Discovered CIN ${discoveredCIN} via MCA name search for "${companyName}"`,
								);
						...
						console.log(
							`[DataEnrichment] MCA name search fallback failed: ${mcaSearchError.message}`,
						);
					}
				}
			} catch (isinError: any) {
				console.error(
					`[DataEnrichment] ISIN→CIN discovery error: ${isinError.message}`,
				);
				auditTrail.push({
					id: crypto.randomUUID(),
					timestamp: new Date(),
		...
		try {
			...
			const nseBseResult = await this.fetchFromNSEBSE(
				companyId,
				(company as any).symbol || undefined,
				financialYear,
			);
		...
		if ((company as any).symbol || options.externalSymbols?.bse) {
			try {
				...
				const bseSymbol =
					options.externalSymbols?.bse || (company as any).symbol;
				const bseResult = await this.fetchFromBSE(
					companyId,
					undefined,
					bseSymbol,
				);
				...
				if (
					Object.keys(bseSmeResult.metrics).length > 0 &&
					!enriched.sources.includes("bse")
				) {
					enriched.sources.push("bse");
				}
		...
		if (options.externalSymbols?.finnhub && finnhubService.isReady()) {
			try {
				...
				const result = await finnhubService.getBasicFinancials(
					options.externalSymbols.finnhub,
				);
		...
		if ((company as any).symbol || options.externalSymbols?.yahoo) {
			try {
				auditTrail.push({
					...
					source: "yahoo",
					reason:
						"Fetching fallback data from Yahoo Finance (Indian market .NS/.BO)",
				});

				const yahooSymbol =
					options.externalSymbols?.yahoo || (company as any).symbol;
				const yahooResult = await this.fetchFromYahoo(
					companyId,
					yahooSymbol,
					company.isin,
				);
		...
		enriched.overallConfidence =
			metricConfidences.length > 0
				? Number(
						(
							metricConfidences.reduce((a: any, b: any) => a + b, 0) /
							metricConfidences.length
						).toFixed(2),
					)
				: 0;
		...
		console.log(
			`[DataEnrichment] Enriched ${companyId} with ${Object.keys(enriched.metrics).length} metrics from ${enriched.sources.join(", ")}`,
		);
	...
	private async persistAuditLog(
        ...
        `);
			...
			console.log(
				`[DataEnrichment] Persisted ${entries.length} audit entries for ${companyId}`,
			);
		} catch (error: any) {
			console.error(
				`[DataEnrichment] Failed to persist audit log: ${error.message}`,
			);
		}
	}
	...
	async getMetricProvenance(
      ...
      `);
			...
			const sourceMap = new Map<
				DataSource,
				{ confidence: number; lastFetched?: Date; fetchCount: number }
			>();
			for (const row of rows) {
				...
				existing.confidence = Math.max(
					existing.confidence,
					Number.parseFloat(row.confidence_score) || 0,
				);
				if (
					!existing.lastFetched ||
					new Date(row.created_at) > existing.lastFetched
				) {
			...
			return {
				...
				currentValue: history[0]
					...
					: undefined,
				history,
				sourceBreakdown: Array.from(sourceMap.entries()).map(
					([source, data]) => ({
						source,
						...data,
					}),
			...
			console.error(
				`[DataEnrichment] Provenance query failed: ${error.message}`,
			);
			return {
				metric,
				history: [],
	...
	async getDataUsageFlags(
		...
		const financials = await storage.getCompanyFinancialsByYear(
			companyId,
			financialYear,
		);
	...
	async fetchNSEFinancialReports(symbol: string): Promise<{
		...
		console.log(
			`[DataEnrichment] Fetching NSE financial reports for ${symbol}`,
		);
		const providerResult =
			await nseIndiaProviderInstance.fetchCorporateReports(symbol);
	 ...
	 */
	async fetchBSEFinancialReports(
		symbol?: string,
		scripcode?: string,
	): Promise<{
		...
		console.log(
			`[DataEnrichment] Fetching BSE financial reports for ${symbol || scripcode}`,
		);
		const providerResult = await bseIndiaProviderInstance.fetchCorporateReports(
			symbol,
			scripcode,
		);
	 ...
	 */
	async fetchUnifiedFinancialReports(
		symbol: string,
		scripcode?: string,
	): Promise<{
		quarterlyResults: any[];
		...
		console.log(
			`[DataEnrichment] Fetching unified financial reports for ${symbol}`,
		);
		...
		if (nseData.status === "fulfilled") {
			...
			if (
				nseResults.quarterlyResults.length > 0 ||
				nseResults.announcements.length > 0
			) {
				sources.push("nse");
			}
		}
		...
		if (bseData.status === "fulfilled") {
			...
			if (
				bseResults.financialResults.length > 0 ||
				bseResults.announcements.length > 0
			) {
				sources.push("bse");
			}
		}
		...
		const mergedAnnouncements = [
			...
			(a: any, b: any) =>
				new Date(b.broadcastDate || b.newsDate || 0).getTime() -
				new Date(a.broadcastDate || a.newsDate || 0).getTime(),
		);
		...
		const confidence =
			sources.length === 2 ? 0.95 : sources.length === 1 ? 0.85 : 0;

		console.log(
			`[DataEnrichment] Unified reports: ${mergedQuarterlyResults.length} results, ${mergedAnnouncements.length} announcements from ${sources.join(", ") || "none"}`,
		);
