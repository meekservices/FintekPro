...
class ProspectPortfolioSyncService {
	 ...
	 */
	async migrateToCurrentPortfolio(
		prospectId: string,
	): Promise<{ migrated: number; holdings: NormalizedHolding[] }> {
		...
		const existingHoldings =
			(prospect?.currentPortfolio as NormalizedHolding[]) || [];

		// Normalize and sanitize holdings from table
		const normalizedHoldings: NormalizedHolding[] = holdings.map((h) =>
			this.sanitizeHolding({
				id: h.id || nanoid(),
				...
				averageCost: h.avgPrice ? Number.parseFloat(h.avgPrice) : undefined,
				currentValue: h.currentValue ? Number.parseFloat(h.currentValue) : 0,
				investedValue: h.investedValue
					? Number.parseFloat(h.investedValue)
					: undefined,
				folioNumber: h.folioNumber || undefined,
				broker: h.broker || undefined,
				confidenceScore: h.confidenceScore || undefined,
				purchaseDate: h.purchaseDate
					? h.purchaseDate.toISOString().split("T")[0]
					: undefined,
			}),
		);

		// Merge with existing holdings (deduplicate by ISIN or name+assetType)
		const mergedHoldings = this.mergeHoldings(
			existingHoldings,
			normalizedHoldings,
		);
	...
	async addHolding(
		prospectId: string,
		holding: Partial<NormalizedHolding>,
	...
	async addHoldings(
		prospectId: string,
		holdings: Partial<NormalizedHolding>[],
	): Promise<NormalizedHolding[]> {
		const [prospect] = await db
			.select({ currentPortfolio: prospectClients.currentPortfolio })
			.from(prospectClients)
	...
	async updateHolding(
		prospectId: string,
		holdingId: string,
		updates: Partial<NormalizedHolding>,
	): Promise<NormalizedHolding[]> {
	 ...
	 */
	async migrateAllProspects(): Promise<{
		total: number;
		migrated: number;
		errors: string[];
	}> {
		...
		const uniqueProspectIds = [
			...new Set(
				prospectsWithPortfolios.map((p) => p.prospectId).filter(Boolean),
			),
		];
		...
		for (const prospectId of uniqueProspectIds) {
			...
			try {
				...
				if (result.migrated > 0) {
					migrated++;
					console.log(
						`[PortfolioSync] Migrated ${result.migrated} holdings for prospect ${prospectId}`,
					);
				}
			} catch (error: any) {
	...
	private sanitizeHolding(
		...
		const sanitized: NormalizedHolding = {
			...
			averageCost: holding.averageCost
				? Math.max(0, Number(holding.averageCost))
				: undefined,
			currentValue: Math.max(0, Number(holding.currentValue) || 0),
			currentNav: holding.currentNav
				? Math.max(0, Number(holding.currentNav))
				: undefined,
			investedValue: holding.investedValue
				? Math.max(0, Number(holding.investedValue))
				: undefined,
			unrealizedGain:
				holding.unrealizedGain !== undefined
					? Number(holding.unrealizedGain)
					: undefined,
			unrealizedGainPercent:
				holding.unrealizedGainPercent !== undefined
					? Number(holding.unrealizedGainPercent)
					: undefined,
			folioNumber: holding.folioNumber?.trim() || undefined,
			broker: holding.broker?.trim() || undefined,
			confidenceScore: holding.confidenceScore
				? Math.min(100, Math.max(0, Number(holding.confidenceScore)))
				: undefined,
			...
			returns1y:
				holding.returns1y !== undefined ? Number(holding.returns1y) : undefined,
	...
	private recalculateHoldingFromLots(holding: NormalizedHolding): void {
		...
		holding.averageCost =
			totalQuantity > 0 ? totalInvestedValue / totalQuantity : undefined;
		...
		if (holding.currentNav && holding.currentNav > 0) {
			...
			holding.unrealizedGainPercent =
				totalInvestedValue > 0
					? ((holding.currentValue - totalInvestedValue) / totalInvestedValue) *
						100
					: 0;
	...
	async addHoldingWithLots(
		...
		const existingIndex = existingHoldings.findIndex((h) => {
			...
			return (
				h.name.toLowerCase() === newHolding.name.toLowerCase() &&
			...
			const existingIndex = merged.findIndex((h) => {
				...
				if (h.isin && newHolding.isin) {
					...
					if (h.folioNumber && newHolding.folioNumber) {
						return (
							h.isin === newHolding.isin &&
							h.folioNumber === newHolding.folioNumber
						);
					}
