...
class AccreditedInvestorService {
	...
	grantAccreditation(
		...
		console.log(
			`✅ [Accredited Investor] Granted to user ${userId.substring(0, 8)}..., expires: ${expiresAt.toISOString()}`,
		);
	...
	checkProductAccess(userId: string, productType: string): ProductAccessResult {
		const isAdvancedProduct = this.ADVANCED_PRODUCTS.includes(
		...
		if (!status.isAccredited) {
			if (status.isExpired) {
				return {
					hasAccess: false,
					reason:
						"Your accredited investor status has expired. Please renew to access this product.",
			...
			return {
				hasAccess: false,
				reason:
					"This product requires accredited investor status. Please complete accreditation to proceed.",
	...
	expireAccreditation(
		userId: string,
		reason: string = "Accreditation period expired",
	): void {
		const investor = this.investors.get(userId);
		if (!investor) return;
		...
		this.investors.set(userId, investor);
		console.log(
			`⚠️ [Accredited Investor] Expired for user ${userId.substring(0, 8)}...: ${reason}`,
		);
	}
	...
	renewAccreditation(
		...
		this.investors.set(userId, investor);
		console.log(
			`🔄 [Accredited Investor] Renewed for user ${userId.substring(0, 8)}..., new expiry: ${newExpiresAt.toISOString()}`,
		);
	...
	processExpiredAccreditations(): number {
		...
		if (expired.length > 0) {
			console.log(
				`⏰ [Accredited Investor] Processed ${expired.length} expired accreditations`,
			);
		}
	...
	exportComplianceReport(): {
		...
		for (const investor of this.investors.values()) {
			if (investor.isActive && now < investor.expiresAt) {
				active++;
				byType[investor.accreditationType] =
					(byType[investor.accreditationType] || 0) + 1;
			} else {
				expired++;
			}
		}
