...
export class SandboxKYCService {
	...
	async verifyCompanyMCA(cin: string): Promise<MCACompanyDetails> {
		...
		const endpoint = isLLP
			? "/mca/llp/master-data/search"
		...
		try {
			...
			return {
				...
				companyStatus:
					cmd["company_status(for_efiling)"] ||
					cmd.company_status ||
					cmd.llp_status ||
					"",
				companyClass:
					cmd.class_of_company || cmd.company_class || (isLLP ? "LLP" : ""),
				...
				paidUpCapital:
					cmd["paid_up_capital(rs)"] ||
					cmd.paid_up_capital ||
					cmd.total_obligation_of_contribution ||
					"",
				authorizedCapital:
					cmd["authorised_capital(rs)"] || cmd.authorized_capital || "",
				directors: (
					raw.directors ||
					raw.signatory ||
					raw.partners_directors ||
					[]
				).map((d: any) => ({
					din:
						d.din ||
						d.director_identification_number ||
						d.dpins ||
						d.dpin ||
						"",
					...
					appointmentDate:
						d.begin_date || d.appointment_date || d.date_of_appointment || "",
				})),
				...
				lastBalanceSheetDate:
					cmd.date_of_balance_sheet || cmd.last_balance_sheet_date || "",
			};
		} catch (error: any) {
			console.error(
				"MCA verification error:",
				error.response?.data || error.message,
			);
			throw new Error(
				`MCA verification failed: ${error.response?.data?.message || error.message}`,
			);
		}
	}
	...
	async verifyDirectorMCA(din: string): Promise<DirectorMCADetails> {
		...
		try {
			...
			if (response.data.code !== 200) {
				throw new Error(
					response.data.message || "Director MCA verification failed",
				);
			}
			...
			console.error(
				"Director MCA verification error:",
				error.response?.data || error.message,
			);
			throw new Error(
				`Director MCA verification failed: ${error.response?.data?.message || error.message}`,
			);
		}
	}
	...
	async verifyGSTIN(gstin: string): Promise<GSTINDetails> {
		...
		try {
			...
			throw new Error(
				`GSTIN verification failed: ${error.response?.data?.message || error.message}`,
			);
		}
	}
	 ...
	 */
	async verifyCorporatePAN(
		pan: string,
		name: string,
	): Promise<CorporatePANDetails> {
		const token = await getSandboxAccessToken();
		...
		try {
			...
			console.error(
				`Sandbox Corporate PAN API Error (${statusCode}):`,
				errorMsg,
			);

			if (statusCode === 400) {
				throw new Error(
					`Invalid request: ${errorMsg}. Check API credentials or input format.`,
				);
			}
			if (statusCode === 401) {
				throw new Error(
					"Authentication failed. Verify SANDBOX_API_KEY and SANDBOX_API_SECRET.",
				);
			}
	...
	async verifyIndividualPAN(
		...
		try {
			...
			if (response.data.code !== 200) {
				throw new Error(
					response.data.message || "Individual PAN verification failed",
				);
			}
	...
	async verifyTAN(tan: string): Promise<TANDetails> {
		...
		try {
			...
			console.error(
				"TAN verification error:",
				error.response?.data || error.message,
			);
			throw new Error(
				`TAN verification failed: ${error.response?.data?.message || error.message}`,
			);
		}
	}
	...
	async verifyCorporateEntity(params: {
		...
		try {
			results.details.pan = await this.verifyCorporatePAN(
				params.pan,
		...
		if (
			params.cin &&
			(params.entityType === "company" || params.entityType === "llp")
		) {
			try {
	...
	async verifyPAN(
		...
		if (
			!/^(0[1-9]|[12][0-9]|3[01])\/(0[1-9]|1[0-2])\/[0-9]{4}$/.test(dateOfBirth)
		) {
		...
		try {
			...
			throw new Error(
				`PAN verification failed: ${error.response?.data?.message || error.message}`,
			);
		}
	}
	...
	async generateAadhaarOTP(
		...
		try {
			...
			if (
				statusCode === 404 &&
				isTestEnvironment &&
				errorMessage?.includes("does not match any saved example")
			) {
				console.warn(
					"⚠️ [Sandbox KYC] Aadhaar OTP API not subscribed on test account, falling back to mock",
				);
	...
	async verifyAadhaarOTP(
		...
		try {
			...
			if (response.data.code !== 200) {
				throw new Error(
					response.data.message || "Aadhaar OTP verification failed",
				);
			}
			...
			if (
				...
				console.warn(
					"⚠️ [Sandbox KYC] Aadhaar verify API not subscribed on test account, falling back to mock",
				);
				return {
			...
			if (referenceId.startsWith("mock_ref_")) {
				console.log(
					"🔧 [Sandbox KYC] Using mock Aadhaar verification for mock reference",
				);
				return {
					aadhaarNumber: "XXXX-XXXX-9012",
					fullName: SANDBOX_AADHAAR_TEST_DATA.testName,
	 ...
	 */
	async verifyBankAccountPennyDrop(
		...
		try {
			...
			if (response.data.code !== 200) {
				throw new Error(
					response.data.message || "Penny drop verification failed",
				);
			}
	...
	async verifyIFSC(ifsc: string): Promise<{
		...
		try {
			const response = await axios.get(`${SANDBOX_BASE_URL}/bank/${ifsc}`, {
				headers: {
					"x-api-key": SANDBOX_API_KEY,
					Authorization: token,
					"x-api-version": "1.0.0",
			...
			throw new Error(
				`IFSC verification failed: ${error.response?.data?.message || error.message}`,
			);
		}
	}
	 ...
	 */
	getPennylessSupportedBanks(): Array<{
		ifscPrefix: string;
		bankName: string;
	}> {
		return Object.entries(SandboxKYCService.PENNYLESS_SUPPORTED_BANKS).map(
			([ifscPrefix, bankName]) => ({
				ifscPrefix,
				bankName,
			}),
	 ...
	 */
	async verifyBankAccountPennyless(
		accountNumber: string,
		ifsc: string,
	): Promise<{
		accountNumber: string;
		...
		if (!supportedBank) {
			throw new Error(
				`Bank with IFSC prefix ${ifsc.substring(0, 4)} does not support Pennyless verification. Use Penny Drop instead.`,
			);
		}
		...
		try {
			...
			if (response.data.code !== 200) {
				throw new Error(
					response.data.message || "Pennyless verification failed",
				);
			}
	...
	async initiateDigiLockerSession(
		...
		if (!redirectUrl || !/^https?:\/\/.+/.test(redirectUrl)) {
			throw new Error(
				"Valid redirect URL is required (must start with http:// or https://)",
			);
		}
		...
		try {
			...
			if (response.data.code !== 200) {
				throw new Error(
					response.data.message || "DigiLocker session initiation failed",
				);
			}
			...
			console.error(
				"DigiLocker initiation error:",
				error.response?.data || error.message,
			);
			throw new Error(
				`DigiLocker initiation failed: ${error.response?.data?.message || error.message}`,
			);
		}
	}
	...
	async getDigiLockerSessionStatus(sessionId: string): Promise<{
		...
		try {
			...
			if (response.data.code !== 200) {
				throw new Error(
					response.data.message || "Failed to get DigiLocker session status",
				);
			}
			...
			throw new Error(
				`Failed to get session status: ${error.response?.data?.message || error.message}`,
			);
		}
	}
	...
	async createDigiLockerSDKSession(
		...
		try {
			...
			if (response.data.code !== 200) {
				throw new Error(
					response.data.message || "DigiLocker SDK session creation failed",
				);
			}
			...
			console.error(
				"DigiLocker SDK session error:",
				error.response?.data || error.message,
			);
			throw new Error(
				`DigiLocker SDK session failed: ${error.response?.data?.message || error.message}`,
			);
		}
	}
	...
	async getDigiLockerSDKDocument(
		...
		try {
			...
			if (response.data.code !== 200) {
				throw new Error(
					response.data.message || "Failed to get DigiLocker document",
				);
			}
			...
			throw new Error(
				`Failed to get document: ${error.response?.data?.message || error.message}`,
			);
		}
	}
	...
	async fetchDigiLockerDocument(
		...
		try {
			...
			if (response.data.code !== 200) {
				throw new Error(
					response.data.message || "DigiLocker document fetch failed",
				);
			}
			...
			throw new Error(
				`DigiLocker fetch failed: ${error.response?.data?.message || error.message}`,
			);
		}
	}
	...
	async initiateEntityLockerSession(
		...
		try {
			...
			if (response.data.code !== 200) {
				throw new Error(
					response.data.message || "EntityLocker session initiation failed",
				);
			}
			...
			console.error(
				"EntityLocker initiation error:",
				error.response?.data || error.message,
			);
			throw new Error(
				`EntityLocker initiation failed: ${error.response?.data?.message || error.message}`,
			);
		}
	}
	 ...
	 */
	async fetchEntityLockerDocument(
		sessionId: string,
		documentType: string,
	): Promise<{
		documentType: string;
		...
		try {
			...
			if (response.data.code !== 200) {
				throw new Error(
					response.data.message || "EntityLocker document fetch failed",
				);
			}
			...
			console.error(
				"EntityLocker fetch error:",
				error.response?.data || error.message,
			);
			throw new Error(
				`EntityLocker fetch failed: ${error.response?.data?.message || error.message}`,
			);
		}
	}
