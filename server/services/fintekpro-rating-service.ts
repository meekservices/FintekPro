...
export class FintekProRatingService {
	...
	private constructor() {
		...
		if (apiKey) {
			...
			console.log(
				"🏆 FintekPro Smart Rating Service initialized (rule-based mode)",
			);
		}
	}

	async getRating(
		paramsOrSchemeCode: RatingParams | string,
	): Promise<FintekProAnalysis | null> {
		// Backward compatibility: if string is passed, assume it's a mutual fund scheme code
		const params: RatingParams =
			typeof paramsOrSchemeCode === "string"
		...
		try {
			...
			switch (params.assetClass) {
				...
				case "government_security":
					analysis = await this.rateGovernmentSecurity(
						params.id,
						params.includeAI,
					);
					break;
			...
			console.error(
				`❌ Error calculating FintekPro rating for ${params.id}:`,
				error,
			);
			return null;
		}
	...
	async persistRating(
		...
		try {
			...
			console.warn(
				`[FintekProRating] Failed to persist rating for ${schemeCode}: ${error.message}`,
			);
			return false;
		}
	}

	async computeAndPersistRating(
		schemeCode: string,
	): Promise<FintekProAnalysis | null> {
	...
	async batchComputeAndPersist(
		...
		console.log(
			`[FintekProRating] Batch rating: ${funds.length} funds to process`,
		);
		...
		console.log(
			`[FintekProRating] Batch complete: ${result.persisted} persisted, ${result.failed} failed out of ${result.processed}`,
		);
		return result;
	}
	...
	private async rateMutualFund(
		...
		const returns1y = Number.parseFloat(fund.returns1Y?.toString() || "0");
		const returns3y = Number.parseFloat(fund.returns3y?.toString() || "0");
		const returns5y = Number.parseFloat(fund.returns5y?.toString() || "0");
		const expenseRatio = Number.parseFloat(
			fund.expenseRatio?.toString() || "1.5",
		);
		const aum = Number.parseFloat(fund.aum?.toString() || "1000");
		const nav = Number.parseFloat(fund.nav?.toString() || "100");

		const riskAdjustedScore = this.calculateMFRiskAdjustedScore(
			returns1y,
			returns3y,
			returns5y,
			expenseRatio,
		);
		const qualityScore = this.calculateMFQualityScore(
			fund.fundHouse || "",
			aum,
			fund.crisilRating,
		);
		const liquidityScore = this.calculateMFLiquidityScore(
			aum,
			fund.category || "",
		);
		const momentumScore = this.calculateMomentumScore(returns1y, returns3y);
		const valuationScore = this.calculateMFValuationScore(
			expenseRatio,
			fund.category || "",
		);
		...
		if (mlResult) {
			...
			console.log(
				`[FintekPro ML] MF ${schemeCode}: base=${baseOverallScore.toFixed(1)} ml=${mlScore.toFixed(1)} final=${overallScore.toFixed(1)} predictedReturn=${mlResult.predictedReturn.toFixed(2)}%`,
			);
		}
		// --- end ML scoring ---
		...
		const rating: FintekProRating = {
			...
			dataSource: mlEnhanced
				? "ml_enhanced"
				: includeAI && this.genAI
					? "ai_enhanced"
					: "database",
			mlEnhanced,
			mlPredictedReturn,
		...
		const keyMetrics = {
			...
			nav: `₹${nav.toFixed(2)}`,
			fintekproRating: fund.crisilRating
				? `${fund.crisilRating}-Star`
				: "Not Rated",
			...(mlPredictedReturn !== undefined
				? { mlPredictedReturn: `${mlPredictedReturn.toFixed(2)}%` }
				: {}),
			...(mlConfidence !== undefined
				? { mlConfidence: `${mlConfidence.toFixed(0)}%` }
				: {}),
		};

		const strengths = this.generateMFStrengths(
			fund,
			overallScore,
			returns1y,
			returns3y,
		...
		const concerns = this.generateMFConcerns(
			fund,
			overallScore,
			returns1y,
			aum,
	...
	private async rateStock(
		...
		const peRatio =
			enriched?.fundamentals?.peRatio ??
			Number.parseFloat(stock.peRatio?.toString() || "25");
		const pbRatio =
			enriched?.fundamentals?.pbRatio ??
			Number.parseFloat(stock.pbRatio?.toString() || "3");
		const roe =
			enriched?.fundamentals?.roe ??
			Number.parseFloat(stock.roe?.toString() || "15");
		const marketCap =
			enriched?.fundamentals?.marketCap ??
			Number.parseFloat(stock.marketCap?.toString() || "10000");
		const returns1y = Number.parseFloat(stock.returns1Y?.toString() || "0");
		const returns3y = Number.parseFloat(stock.returns3y?.toString() || "0");
		const beta = Number.parseFloat(stock.beta?.toString() || "1");
		const debtToEquity =
			enriched?.fundamentals?.debtToEquity ??
			Number.parseFloat(stock.debtToEquity?.toString() || "1");
		const enrichedRoic = enriched?.fundamentals?.roic ?? null;
		const enrichedEvToEbitda = enriched?.fundamentals?.evToEbitda ?? null;
		const enrichedInterestCoverage =
			enriched?.fundamentals?.interestCoverage ?? null;
		const enrichedDividendYield = enriched?.fundamentals?.dividendYield ?? null;

		const hasEnrichedData = !!enriched;

		const riskAdjustedScore = this.calculateStockRiskAdjustedScore(
			returns1y,
			returns3y,
			beta,
		);
		let qualityScore = this.calculateStockQualityScore(
			roe,
			debtToEquity.toString(),
			stock.sector || "",
		);
		const liquidityScore = this.calculateStockLiquidityScore(
			marketCap,
			stock.avgVolume?.toString(),
		);
		let momentumScore = this.calculateMomentumScore(returns1y, returns3y);
		let valuationScore = this.calculateStockValuationScore(
			peRatio,
			pbRatio,
			stock.sector || "",
		);
		...
		if (
			enriched?.growth?.epsGrowth != null &&
			enriched.growth.epsGrowth > 0 &&
			peRatio > 0
		) {
			const epsGrowthPct =
				enriched.growth.epsGrowth > 1
					? enriched.growth.epsGrowth / 100
					: enriched.growth.epsGrowth;
			if (epsGrowthPct > 0) {
		...
		if (
			advancedMetrics.pegRatio !== undefined &&
			advancedMetrics.pegRatio < 1.5
		) {
			valuationScore += 10;
		}
		...
		if (enriched?.growth) {
			...
			if (g.freeCashFlowGrowth != null && g.freeCashFlowGrowth > 10)
				growthBoost += 5;
			if (g.revenueGrowth != null && g.revenueGrowth < -10) growthBoost -= 5;
			if (g.epsGrowth != null && g.epsGrowth < -15) growthBoost -= 5;
		...
		if (mlResultStock) {
			...
			console.log(
				`[FintekPro ML] Stock ${stockId}: base=${baseOverallScore.toFixed(1)} ml=${mlScore.toFixed(1)} final=${overallScore.toFixed(1)} predictedReturn=${mlResultStock.predictedReturn.toFixed(2)}%`,
			);
		}
		// --- end ML scoring ---
		...
		const dataSource = stockMlEnhanced
			? ("ml_enhanced" as const)
			: includeAI && this.genAI
				? ("ai_enhanced" as const)
				: hasEnrichedData
					? ("database" as const)
					: ("calculated" as const);
		...
		const rating: FintekProRating = {
			...
			overallScore,
			confidenceLevel:
				marketCap > 50000 ? "high" : marketCap > 10000 ? "medium" : "low",
			dataSource,
		...
		if (enriched?.growth?.epsGrowth != null)
			keyMetrics.epsGrowth = `${enriched.growth.epsGrowth.toFixed(2)}%`;
		if (enriched?.dcf?.upsidePercent != null)
			keyMetrics.dcfUpside = `${enriched.dcf.upsidePercent.toFixed(1)}%`;
		if (enriched?.companyRating?.ratingScore != null)
			keyMetrics.fmpRating = enriched.companyRating.ratingScore;
		if (enriched?.fundamentals?.grahamNumber != null)
			keyMetrics.grahamNumber = `₹${enriched.fundamentals.grahamNumber.toFixed(2)}`;
		if (enrichedEvToEbitda != null)
			keyMetrics.evToEbitda = enrichedEvToEbitda.toFixed(2);
		if (enriched?.technicals?.rsi != null)
			keyMetrics.rsi = enriched.technicals.rsi.toFixed(1);
		if (stockMlPredictedReturn !== undefined)
			keyMetrics.mlPredictedReturn = `${stockMlPredictedReturn.toFixed(2)}%`;
		if (stockMlConfidence !== undefined)
			keyMetrics.mlConfidence = `${stockMlConfidence.toFixed(0)}%`;

		const strengths = this.generateStockStrengths(
			stock,
			overallScore,
			returns1y,
			roe,
		...
		const concerns = this.generateStockConcerns(
			stock,
			overallScore,
			returns1y,
			peRatio,
		...
		if (
			enriched?.analystTargets &&
			enriched.analystTargets.count > 0 &&
			enriched.analystTargets.avgPriceTarget != null
		) {
	...
	private async rateCorporateBond(
		bondId: string,
		includeAI?: boolean,
	): Promise<FintekProAnalysis | null> {
		...
		const yieldToMaturity = Number.parseFloat(
			bond.yieldToMaturity?.toString() || "8",
		);
		const couponRate = Number.parseFloat(bond.couponRate?.toString() || "7");
		const faceValue = Number.parseFloat(bond.faceValue?.toString() || "1000");
		const currentPrice = Number.parseFloat(
			bond.currentPrice?.toString() || "1000",
		);
		...
		const durationScore = this.getDurationScore(
		...
		const strengths = this.generateBondStrengths(
			bond,
			overallScore,
			yieldToMaturity,
			creditScore,
		);
		const concerns = this.generateBondConcerns(
			bond,
			overallScore,
			yieldToMaturity,
		);
	...
	private async rateGovernmentSecurity(
		securityId: string,
		includeAI?: boolean,
	): Promise<FintekProAnalysis | null> {
		const securities = await db
			...
			.where(
				or(
					eq(governmentSecurities.id, securityId),
					eq(governmentSecurities.isin, securityId),
				),
			)
			.limit(1);
		...
		const yieldToMaturity = Number.parseFloat(
			security.yieldToMaturity?.toString() || "7",
		);
		const couponRate = Number.parseFloat(
			security.couponRate?.toString() || "6",
		);
		...
		const durationScore = this.getDurationScore(
			security.maturityDate?.toString() || "",
		);
		...
		return {
			...
			strengths: [
				...
				`Attractive yield of ${yieldToMaturity.toFixed(2)}%`,
			],
			concerns: [
	...
	private async getTopRatedMutualFunds(
		limit: number,
	): Promise<FintekProAnalysis[]> {
		const funds = await db
			.select()
...
Focus on: investment suitability, key risks, and potential. Be factual and balanced.`;
	...
	private calculateMFRiskAdjustedScore(
		returns1y: number,
		returns3y: number,
		returns5y: number,
		expenseRatio: number,
	...
	private calculateMFQualityScore(
		fundHouse: string,
		aum: number,
	...
	private calculateMFValuationScore(
		expenseRatio: number,
		category: string,
	): number {
		let score = 80;
	...
	private calculateStockRiskAdjustedScore(
		returns1y: number,
		returns3y: number,
		beta: number,
	): number {
	...
	private calculateAdvancedMetricsForStock(stock: any): {
		...
		try {
			const netIncome = Number.parseFloat(stock.netIncome || 0);
			const totalAssets = Number.parseFloat(stock.totalAssets || 1);
			const operatingCashFlow = Number.parseFloat(stock.operatingCashFlow || 0);
			const longTermDebt = Number.parseFloat(stock.longTermDebt || 0);
			const currentRatio = Number.parseFloat(stock.currentRatio || 1.5);
			const sharesOutstanding = Number.parseFloat(stock.sharesOutstanding || 1);
			const grossMargin = Number.parseFloat(stock.grossMargin || 0);
			const revenue = Number.parseFloat(stock.revenue || 0);
			const assetTurnover =
				revenue > 0 && totalAssets > 0 ? revenue / totalAssets : 0;

			if (netIncome && totalAssets && operatingCashFlow) {
				metrics.piotroskiFScore =
					financialMetricsCalculator.calculatePiotroskiFScore(
						netIncome,
						totalAssets,
						operatingCashFlow,
						longTermDebt,
			...
			const workingCapital = Number.parseFloat(
				stock.workingCapital || totalAssets * 0.2,
			);
			const retainedEarnings = Number.parseFloat(
				stock.retainedEarnings || netIncome * 3,
			);
			const ebit = Number.parseFloat(stock.ebit || netIncome * 1.3);
			const marketCap = Number.parseFloat(stock.marketCap || 0);
			const totalLiabilities = Number.parseFloat(
				stock.totalLiabilities || totalAssets * 0.4,
			);
			...
			if (operatingCashFlow && netIncome) {
				metrics.earningsQualityRatio =
					financialMetricsCalculator.calculateEarningsQualityRatio(
						operatingCashFlow,
						netIncome,
					);
			}

			const peRatio = Number.parseFloat(stock.peRatio || 0);
			const epsGrowth = Number.parseFloat(stock.epsGrowth || 15);
			...
			const ebitda = Number.parseFloat(stock.ebitda || ebit * 1.15);
			const enterpriseValue = Number.parseFloat(
				stock.enterpriseValue || marketCap * 1.1,
			);
			if (enterpriseValue && ebitda && ebitda > 0) {
				metrics.evToEbitda = financialMetricsCalculator.calculateEVtoEBITDA(
					enterpriseValue,
					ebitda,
				);
			}

			const investedCapital = Number.parseFloat(
				stock.investedCapital || totalAssets * 0.7,
			);
			const taxRate = 0.25;
			if (ebit && investedCapital && investedCapital > 0) {
				const nopat = ebit * (1 - taxRate);
				metrics.roic = financialMetricsCalculator.calculateROIC(
					nopat,
					investedCapital,
				);
			}
			...
			console.warn(
				"[FintekProRating] Error calculating advanced metrics:",
				error,
			);
		}
	...
	private calculateStockQualityScore(
		roe: number,
		debtToEquity?: string,
		sector?: string,
	): number {
		...
		const de = Number.parseFloat(debtToEquity || "1");
		if (de < 0.5) score += 10;
		else if (de > 2) score -= 10;
		return Math.min(100, Math.max(30, score));
	}

	private calculateStockLiquidityScore(
		marketCap: number,
		avgVolume?: string,
	): number {
		let score = 60;
	...
	private calculateStockValuationScore(
		peRatio: number,
		pbRatio: number,
		sector: string,
	): number {
	...
	private determineMFCategory(
		category: string,
	): "equity" | "debt" | "hybrid" | "commodity" | "alternative" {
		...
		if (
			...
			return "commodity";
		if (cat.includes("reit") || cat.includes("invit") || cat.includes("fof"))
			return "alternative";
		return "equity";
	}

	private generateMFStrengths(
		fund: any,
		score: number,
		returns1y: number,
		returns3y: number,
		...
		if (returns3y > 12)
			strengths.push(`Strong 3-year returns of ${returns3y.toFixed(1)}%`);
		if (returns1y > 15)
			strengths.push(
				`Excellent 1-year performance of ${returns1y.toFixed(1)}%`,
			);
		if (aum > 20000) strengths.push("Large AUM indicates investor confidence");
		if (expenseRatio < 1.0)
			strengths.push(`Low expense ratio of ${expenseRatio.toFixed(2)}%`);
		if (fund.crisilRating && fund.crisilRating <= 2)
			strengths.push("Top-tier FintekPro Smart Rating");
		if (score > 80) strengths.push("Strong risk-adjusted performance");
		return strengths.length > 0
			? strengths
			: ["Established fund with consistent track record"];
	}

	private generateMFConcerns(
		fund: any,
		score: number,
		returns1y: number,
		aum: number,
		...
		if (expenseRatio > 2.0)
			concerns.push(`High expense ratio of ${expenseRatio.toFixed(2)}%`);
	...
	private generateStockStrengths(
		stock: any,
		score: number,
		returns1y: number,
		roe: number,
		...
		const strengths: string[] = [];
		if (returns1y > 20)
			strengths.push(`Strong 1-year returns of ${returns1y.toFixed(1)}%`);
		if (roe > 18)
			strengths.push(`High ROE of ${roe.toFixed(1)}% indicates efficiency`);
		if (peRatio < 20)
			strengths.push("Attractive valuation relative to earnings");
		if (score > 80) strengths.push("Strong fundamentals and growth potential");
		return strengths.length > 0
			? strengths
			: ["Established company with market presence"];
	}

	private generateStockConcerns(
		stock: any,
		score: number,
		returns1y: number,
		peRatio: number,
		...
		return concerns.length > 0
			? concerns
			: ["Sector-specific and market risks apply"];
	}

	private generateBondStrengths(
		bond: any,
		score: number,
		yield_: number,
		creditScore: number,
		...
		const strengths: string[] = [];
		if (creditScore >= 90)
			strengths.push("High credit quality (investment grade)");
		if (yield_ > 9) strengths.push(`Attractive yield of ${yield_.toFixed(2)}%`);
		if (bond.tradingStatus === "active")
			strengths.push("Active secondary market trading");
		return strengths.length > 0 ? strengths : ["Regular coupon payments"];
	}

	private generateBondConcerns(
		bond: any,
		score: number,
		yield_: number,
	): string[] {
		...
		return concerns.length > 0
			? concerns
			: ["Interest rate and credit risk apply"];
	}
	...
	private generateMFRationale(
		fund: any,
		rating: FintekProRating,
		metrics: Record<string, any>,
	): string {
		const performance =
			rating.overallScore > 80
				? "excellent"
				: rating.overallScore > 60
					? "good"
					: "moderate";
	...
	private generateStockRationale(
		...
		const quality =
			rating.qualityScore > 80
				? "strong"
				: rating.qualityScore > 60
					? "good"
					: "moderate";
