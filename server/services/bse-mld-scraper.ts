...
const BSE_DEBT_URL =
	"https://www.bseindia.com/markets/Debt/debt_corporatebond.html";
const BSE_CORP_DEBT_API =
	"https://api.bseindia.com/BseIndiaAPI/api/DebsSecuritiesData/w";
...
export async function scrapeBseMldListings(): Promise<BseMldImportResult> {
	...
	try {
		...
		if (!Array.isArray(data)) {
			console.log(
				"[BSE MLD Scraper] API response is not an array, trying HTML scrape...",
			);
			return await scrapeBseDebtHtml();
		}

		console.log(
			`[BSE MLD Scraper] Found ${data.length} debt securities from BSE API`,
		);

		for (const item of data) {
			try {
				const securityName =
					item.SecName || item.SecurityName || item.Security_Name || "";
				const issuerName =
					item.IssuerName ||
					item.Issuer ||
					item.Company ||
				...
				listings.push({
					...
					maturityDate: parseDate(
						item.MaturityDate || item.Maturity_Date || item.Redemption_Date,
					),
		...
		console.error(
			"[BSE MLD Scraper] Error fetching from BSE API:",
			error.message,
		);
...
async function scrapeBseDebtHtml(): Promise<BseMldImportResult> {
	...
	try {
		console.log(
			"[BSE MLD Scraper] Attempting HTML scrape from BSE debt page...",
		);
		...
		console.log(
			`[BSE MLD Scraper] HTML scrape found ${listings.length} MLDs from ${totalRows} rows`,
		);
