...
export const GRANDFATHERING_FMV_APPRECIATION_ESTIMATE = 0.40;
...
export const PRE_BUDGET_2024_RATES: Record<string, TaxRateRule> = {
  equity:       { stcg: 0.15, ltcg: 0.10, ltcgExemption: 100000, ltcgThresholdDays: 365,  isProvisional: false },
  hybrid_equity:{ stcg: 0.15, ltcg: 0.10, ltcgExemption: 100000, ltcgThresholdDays: 365,  isProvisional: false },
  elss:         { stcg: 0.15, ltcg: 0.10, ltcgExemption: 100000, ltcgThresholdDays: 1095, isProvisional: false },
  index:        { stcg: 0.15, ltcg: 0.10, ltcgExemption: 100000, ltcgThresholdDays: 365,  isProvisional: false },
  sectoral:     { stcg: 0.15, ltcg: 0.10, ltcgExemption: 100000, ltcgThresholdDays: 365,  isProvisional: false },
  debt:         { stcg: 0.30, ltcg: 0.30, ltcgExemption: 0,      ltcgThresholdDays: Infinity, isProvisional: false },
  hybrid_debt:  { stcg: 0.30, ltcg: 0.30, ltcgExemption: 0,      ltcgThresholdDays: Infinity, isProvisional: false },
  liquid:       { stcg: 0.30, ltcg: 0.30, ltcgExemption: 0,      ltcgThresholdDays: Infinity, isProvisional: false },
  overnight:    { stcg: 0.30, ltcg: 0.30, ltcgExemption: 0,      ltcgThresholdDays: Infinity, isProvisional: false },
  gold:         { stcg: 0.30, ltcg: 0.20, ltcgExemption: 0,      ltcgThresholdDays: 1095, isProvisional: false },
  international:{ stcg: 0.30, ltcg: 0.30, ltcgExemption: 0,      ltcgThresholdDays: Infinity, isProvisional: false },
  unknown:      { stcg: 0.15, ltcg: 0.10, ltcgExemption: 100000, ltcgThresholdDays: 365,  isProvisional: false },
};
...
export function getTaxRegime(transactionDate: Date = new Date()): 'PRE_BUDGET_2024' | 'POST_BUDGET_2024' {
  return transactionDate >= BUDGET_2024_EFFECTIVE_DATE ? 'POST_BUDGET_2024' : 'PRE_BUDGET_2024';
}
...
export function getTaxRatesForAsset(
  ...
  const rates = regime === 'POST_BUDGET_2024' ? POST_BUDGET_2024_RATES : PRE_BUDGET_2024_RATES;
  return rates[assetClass] || rates['unknown'];
}

export function isSlabBased(assetClass: TaxAssetClass, transactionDate: Date = new Date()): boolean {
  const regime = getTaxRegime(transactionDate);
  if (regime === 'PRE_BUDGET_2024') {
    return ['debt', 'hybrid_debt', 'liquid', 'overnight', 'international'].includes(assetClass);
  }
  return false;
}

export function calculateSurcharge(taxAmount: number, totalGains: number): number {
...
export function getProvisionalDisclaimer(assetClasses: TaxAssetClass[]): string | null {
