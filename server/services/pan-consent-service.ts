...
export class PANConsentService {
	private static readonly ENCRYPTION_KEY =
		process.env.PAN_ENCRYPTION_KEY || crypto.randomBytes(32).toString("hex");
	...
	static async encryptPAN(panNumber: string): Promise<string> {
		return PANConsentService.encryptPANInternal(panNumber);
	}
	...
	static async decryptPAN(encryptedPAN: string): Promise<string> {
		return PANConsentService.decryptPANInternal(encryptedPAN);
	}
	...
	private static encryptPANInternal(panNumber: string): string {
		try {
			const key = Buffer.from(PANConsentService.ENCRYPTION_KEY, "hex");
			const iv = crypto.randomBytes(16);
			const cipher = crypto.createCipheriv(
				PANConsentService.ALGORITHM,
				key,
				iv,
			);
	...
	static async storePANConsent(request: ConsentRequest): Promise<PanConsent> {
		...
		// Validate PAN format
		if (!PANConsentService.validatePANFormat(panNumber)) {
			throw new Error(
				"Invalid PAN format. PAN should be in format: ABCDE1234F",
			);
		}

		// Check if user already has active consent
		const existingConsent = await PANConsentService.getActivePANConsent(userId);
		if (existingConsent) {
			throw new Error(
				"User already has active PAN consent. Use update method to modify.",
			);
		}

		try {
			const encryptedPan = PANConsentService.encryptPANInternal(panNumber);
			const panHash = PANConsentService.hashPAN(panNumber);
			...
			await PANConsentService.logPANAccess({
	...
	static async getDecryptedPAN(
		...
		try {
			...
			return PANConsentService.decryptPANInternal(consent.encryptedPan);
		} catch (error) {
	 ...
	 */
	static async updatePANVerification(
		userId: string,
		verified: boolean,
	 ...
	 */
	static async getPANAuditLog(
		userId: string,
		limit = 50,
	): Promise<PanConsentAuditLog[]> {
		try {
	 ...
	 */
	static async getMaskedPANForUser(
		userId: string,
		requestId?: string,
	): Promise<{
		maskedPan: string;
		...
		try {
			...
			const actualPan = PANConsentService.decryptPANInternal(
				consent.encryptedPan,
			);

			// Create masked version (show first 3 and last 2 characters)
			const maskedPan =
				actualPan.substring(0, 3) + "X".repeat(5) + actualPan.substring(8, 10);
	...
	static async getPANStatusForUser(userId: string): Promise<{
		...
		try {
			...
			const maskedInfo = await PANConsentService.getMaskedPANForUser(userId);
			const compliance = await PANConsentService.checkConsentCompliance(userId);
			...
			return {
				...
				compliance: {
					isCompliant:
						compliance.isValid && compliance.complianceStatus === "compliant",
	 ...
	 */
	static async verifyUserDataAccess(
	...
	static async checkConsentCompliance(userId: string): Promise<{
		...
		try {
			...
			const isValid =
				consent.isActive && consent.consentGiven && !consent.revokedAt;
			const complianceStatus =
				isValid && issues.length === 0 ? "compliant" : "needs_attention";
