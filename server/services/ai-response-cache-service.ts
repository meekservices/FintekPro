...
class AIResponseCacheService {
	...
	get<T>(type: string, input: any): T | null {
		...
		this.metrics.estimatedCostSavingsUSD +=
			(GEMINI_COST_PER_1K_TOKENS * AVG_TOKENS_PER_REQUEST) / 1000;

		console.log(
			`[AICache] Cache HIT for ${type} (hash: ${hash}, hits: ${entry.hitCount})`,
		);
	...
	getMetrics(): CacheMetrics & {
		...
		return {
			...
			estimatedCostSavingsUSD:
				Math.round(this.metrics.estimatedCostSavingsUSD * 1000) / 1000,
		};
	}
