	...
	async verify(
		...
		try {
			...
			if (response.data?.code === 200 && data) {
				...
				const nameMatch =
					data.name_as_per_pan_match === true || data.name_match === true;
				const dobMatch =
					data.date_of_birth_match === true || data.dob_match === true;
			...
			return {
				...
				responseTimeMs,
				message:
					error.response?.data?.message || error.message || "Sandbox API error",
