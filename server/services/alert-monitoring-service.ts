...
export class AlertMonitoringService {
	...
	private async checkAllAlerts(retries = 2): Promise<void> {
		...
		try {
			...
			if (Date.now() - this.startTime > 120000) {
				console.log(
					"[AlertMonitoring] Cycle skipped due to transient DB issue",
				);
			}
		}
	}
	...
	private async checkMarketAlert(alert: typeof userAlerts.$inferSelect) {
		...
		try {
			...
			if (alert.alertType === "market_price") {
				if (
					triggerCondition.type === "price_above" &&
					marketData.currentPrice > triggerCondition.value
				) {
					shouldTrigger = true;
					triggerReason = `${alert.symbol} price reached ₹${marketData.currentPrice.toFixed(2)} (above target ₹${triggerCondition.value})`;
				} else if (
					triggerCondition.type === "price_below" &&
					marketData.currentPrice < triggerCondition.value
				) {
					shouldTrigger = true;
				...
				if (
					triggerCondition.type === "percent_gain" &&
					marketData.changePercent >= triggerCondition.value
				) {
					shouldTrigger = true;
					triggerReason = `${alert.symbol} gained ${marketData.changePercent.toFixed(2)}% (target: ${triggerCondition.value}%)`;
				} else if (
					triggerCondition.type === "percent_loss" &&
					marketData.changePercent <= -triggerCondition.value
				) {
					shouldTrigger = true;
	...
	private async sendAlertNotifications(
		...
		try {
			...
			console.warn(
				"Could not fetch notification preferences, using defaults:",
				prefsError,
			);
		}
		...
		if (channels.includes("whatsapp") && whatsappEnabled && user.mobile) {
			try {
				...
				if (result.success) {
					console.log(
						`✅ Alert WhatsApp sent to ${user.mobile.slice(0, 6)}**** via ${result.provider}`,
					);
				}
			} catch (error) {
		...
		if (channels.includes("sms") && smsEnabled && user.mobile) {
			try {
				...
				console.warn(
					`[AlertMonitor] SMS channel requested for alert ${alert.id} but generic send is not available; delivering via WhatsApp fallback.`,
				);
				const result = await whatsappDispatcher.send({
					mobile: user.mobile,
					message,
				...
				if (result.success) {
					console.log(
						`✅ Alert SMS-fallback (WhatsApp) sent to ${user.mobile}`,
					);
				}
			} catch (error) {
	...
	private async fetchMarketData(symbol: string): Promise<MarketQuote | null> {
		try {
			...
			console.warn(
				`[AlertMonitor] Failed to fetch market data for ${symbol}:`,
				error.message,
			);
	...
	private async calculateSpending(
		...
		try {
			...
			const totalSpending = Number.parseFloat(result[0]?.totalSpending || "0");
			console.log(
				`[AlertMonitor] Calculated spending for user ${userId}, category: ${category}, period: ${period}: ₹${totalSpending.toLocaleString()}`,
			);

			return totalSpending;
		} catch (error: any) {
			console.error(
				`[AlertMonitor] Failed to calculate spending for user ${userId}:`,
				error.message,
			);
			return 0;
		}
	...
	private async calculatePortfolioValue(userId: string): Promise<number> {
		try {
			...
			console.log(
				`[AlertMonitor] Calculated portfolio value for user ${userId}: ₹${totalValue.toLocaleString()}`,
			);
			return totalValue;
		} catch (error: any) {
			console.error(
				`[AlertMonitor] Failed to calculate portfolio value for user ${userId}:`,
				error.message,
			);
			return 0;
		}
