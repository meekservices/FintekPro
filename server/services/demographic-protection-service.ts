...
export class DemographicProtectionService {
  ...
  static isProtectedField(fieldName: string): boolean {
    return this.PROTECTED_DEMOGRAPHIC_FIELDS.includes(fieldName);
  }
  ...
  static getProtectedFields(): string[] {
    return [...this.PROTECTED_DEMOGRAPHIC_FIELDS];
  }

  /**
   * Validate demographic change request (blocks direct changes)
   */
  static validateDemographicChange(fieldName: string, userId: string): { 
    allowed: boolean; 
    reason: string;
    ...
    return {
      allowed: false,
      reason: "Demographic data changes require re-CKYC verification for compliance",
      requiresReCKYC: true
    };
  }
   ...
   */
  static async createDemographicChangeRequest(request: DemographicChangeRequest): Promise<{
    success: boolean;
    ...
    try {
      ...
      const validation = this.validateDemographicChange(request.fieldName, request.userId);
      ...
      await this.logDemographicActivity({
        userId: request.userId,
      ...
      return {
        success: true,
        reCkycRequestId,
        message: "Demographic change request created. Re-CKYC verification required."
      };
  ...
  static async processReCkycUpdate(request: CkycUpdateRequest): Promise<{
    ...
    try {
      ...
      await db.insert(ckycRecords).values({
        ...
        dateOfBirth: changes.dateOfBirth || new Date().toISOString().split('T')[0],
  ...
  private static async logDemographicActivity(activity: {
    ...
    try {
      ...
      console.log(`[DEMOGRAPHIC_AUDIT] User: ${activity.userId}, Action: ${activity.action}`, {
   ...
   */
  static getDemographicFieldRestrictions(): Record<string, {
    isProtected: boolean;
    ...
    this.PROTECTED_DEMOGRAPHIC_FIELDS.forEach(field => {
      restrictions[field] = {
        isProtected: true,
        restrictionMessage: "This field can only be updated through re-CKYC verification for regulatory compliance",
        reCkycRequired: true
      };
    });
