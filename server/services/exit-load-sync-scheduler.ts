...
class ExitLoadSyncScheduler {
	...
	start(): void {
		...
		this.cronJob = cron.schedule(
			...
			async () => {
				console.log(
					"[ExitLoadSync] Monthly cron triggered at",
		...
		setTimeout(
			async () => {
				...
				if (shouldRunInitial) {
					console.log(
						"[ExitLoadSync] Running initial enrichment (no recent run detected)...",
					);
					try {
						await this.runFullEnrichment();
					} catch (err) {
					...
					console.log(
						"[ExitLoadSync] Skipping initial enrichment (recent run exists)",
					);
				}
			},
			2 * 60 * 1000,
	...
	private async shouldRunInitialEnrichment(): Promise<boolean> {
		try {
			...
			if (stats.seededFunds === 0) {
				console.log(
					"[ExitLoadSync] No exit load data found, initial run needed",
				);
				return true;
			}
			...
			console.log(
				"[ExitLoadSync] Exit load data exists (" +
					stats.seededFunds +
					" funds), skipping initial run",
			);
			return false;
		} catch (error) {
			console.error(
				"[ExitLoadSync] Error checking initial enrichment status:",
				error,
			);
			return false;
		}
	...
	private getNextScheduledRun(): Date {
		...
		if (
			nowIst.getDate() > 1 ||
			(nowIst.getDate() === 1 && nowIst.getHours() >= 3)
		) {
			// Move to next month
			if (currentMonth === 11) {
	...
	async runSync(): Promise<{
		...
		try {
			...
			console.log(
				`[ExitLoadSync] Sync complete: ${result.seeded} seeded, ${result.skipped} skipped, ${result.errors} errors`,
			);
			return result;
		} catch (error) {
	...
	async runFullEnrichment(): Promise<{
		...
		try {
			...
			console.log(
				`   ISIN enrichment: ${result.isinEnrichment.enriched} enriched`,
			);
			console.log(
				`   Total coverage: ${result.finalStats.withIsin}/${result.finalStats.seededFunds} with ISIN`,
			);
