...
export class PortfolioComparisonService {
	...
	async comparePortfolios(
		...
		try {
			...
			const correlationMatrix =
				this.calculateCorrelationMatrix(portfolioMetrics);
			const performanceRanking =
				this.rankPortfoliosByPerformance(portfolioMetrics);
			const riskAnalysis = this.analyzeRisk(portfolioMetrics);
			const assetAllocationAnalysis =
				this.analyzeAssetAllocation(portfolioMetrics);
			const rebalancingSuggestions =
				await this.generateRebalancingSuggestions(portfolioMetrics);
			...
			const result: PortfolioComparisonResult = {
				...
				worstPortfolio:
					performanceRanking[performanceRanking.length - 1]?.portfolioId || "",
	...
	private async calculatePortfolioMetrics(
		...
		const hasHistoricalData = Object.values(returns).some(
			(v: any) => v !== null,
		);
		...
		return {
			...
			dataAvailability: {
				historicalReturns: hasHistoricalData
					? "available"
					: "insufficient_data",
	...
	private calculateVolatility(holdings: PortfolioHolding[]): number | null {
		...
		const betas = holdings
			.map((h: any) => Number.parseFloat(h.beta || "1"))
			.filter((b: any) => !Number.isNaN(b));
		if (betas.length === 0) return null;

		const avgBeta =
			betas.reduce((sum: any, beta: any) => sum + beta, 0) / betas.length;
	...
	private calculateDownsideDeviation(
		...
		const returns = holdings
			.map((h: any) => {
				const currentValue =
					Number.parseFloat(h.quantity) *
					Number.parseFloat(h.currentPrice || h.avgPrice);
				const investedValue =
					Number.parseFloat(h.quantity) * Number.parseFloat(h.avgPrice);
				return ((currentValue - investedValue) / investedValue) * 100;
			})
			.filter((r: any) => !Number.isNaN(r));
	...
	private calculateDiversificationScore(
		...
		const concentrationPenalty = holdings.reduce(
			(penalty: any, holding: any) => {
				const weight =
					(Number.parseFloat(holding.quantity) *
						Number.parseFloat(holding.avgPrice)) /
					totalValue;
	...
	private calculateConcentrationRisk(
		...
		const hhi = weights.reduce(
			(sum: any, weight: any) => sum + weight * weight,
			0,
		);
		...
		for (let i = 0; i < portfolios.length; i++) {
			...
			for (let j = 0; j < portfolios.length; j++) {
				if (i === j) {
					...
					const correlation = this.calculateAssetAllocationCorrelation(
						p1.assetAllocation,
						p2.assetAllocation,
					);
	...
	private calculateAssetAllocationCorrelation(
		allocation1: any,
		allocation2: any,
	): number {
	...
	private analyzeRisk(portfolios: PortfolioMetrics[]) {
		const sortedByRiskAdjusted = [...portfolios].sort(
			(a: any, b: any) => (b.sharpeRatio || 0) - (a.sharpeRatio || 0),
		);
		const sortedByDiversification = [...portfolios].sort(
			(a: any, b: any) =>
				(b.diversificationScore || 0) - (a.diversificationScore || 0),
		);
		const sortedByVolatility = [...portfolios].sort(
			(a: any, b: any) => (a.volatility || 100) - (b.volatility || 100),
		);
		const sortedBySharpe = [...portfolios].sort(
			(a: any, b: any) => (b.sharpeRatio || 0) - (a.sharpeRatio || 0),
		);
	...
	private analyzeAssetAllocation(portfolios: PortfolioMetrics[]) {
		...
		portfolios.forEach((portfolio: any) => {
			const deviation = Object.keys(idealAllocation).reduce(
				(sum: any, asset: any) => {
					return (
						sum +
						Math.abs(
							portfolio.assetAllocation[asset as keyof typeof idealAllocation] -
								idealAllocation[asset as keyof typeof idealAllocation],
						)
					);
				},
				0,
		...
		const highestEquity = portfolios.reduce((max: any, portfolio: any) =>
			portfolio.assetAllocation.equity > max.assetAllocation.equity
				? portfolio
				: max,
		);
		...
		const mostConservative = portfolios.reduce((max: any, portfolio: any) =>
			...
			max.assetAllocation.debt + max.assetAllocation.cash
				? portfolio
				: max,
		);
		...
		for (let i = 0; i < portfolios.length; i++) {
			for (let j = i + 1; j < portfolios.length; j++) {
				const overlap = this.calculatePortfolioOverlap(
					portfolios[i],
					portfolios[j],
				);
	...
	private calculatePortfolioOverlap(
		portfolio1: PortfolioMetrics,
		portfolio2: PortfolioMetrics,
	) {
		...
		const commonHoldings = [...holdings1].filter((symbol: any) =>
			holdings2.has(symbol),
		).length;
		const totalUniqueHoldings = new Set([...holdings1, ...holdings2]).size;

		const overlapPercent =
			totalUniqueHoldings > 0
				? (commonHoldings / totalUniqueHoldings) * 100
				: 0;
	...
	private generatePortfolioSuggestions(portfolio: PortfolioMetrics) {
		...
		if (allocation.equity > 80) {
			suggestions.push({
				...
				reason:
					"High equity concentration detected. Consider reducing equity exposure to 70%.",
			});
		}
		...
		if (allocation.debt < 15 && allocation.equity > 50) {
			suggestions.push({
				...
				reason:
					"Low debt allocation. Consider increasing to 20% for better risk management.",
			});
		}
		...
		if (portfolio.diversificationScore && portfolio.diversificationScore < 60) {
			suggestions.push({
				...
				reason:
					"Low diversification score. Consider adding more diversified equity positions.",
			});
		}
		...
		if (portfolio.concentrationRisk > 25) {
			suggestions.push({
				...
				reason:
					"High concentration risk detected. Consider reducing large positions.",
			});
		}
	...
	private async generateAIInsights(
		...
		const bestPerformer = portfolios.reduce((best: any, portfolio: any) =>
			portfolio.totalGainLossPercent > best.totalGainLossPercent
				? portfolio
				: best,
		);
	...
	private calculateOverallRiskScore(portfolios: PortfolioMetrics[]): number {
		const avgRiskScore =
			portfolios.reduce((sum: any, p: any) => sum + p.riskScore, 0) /
			portfolios.length;
