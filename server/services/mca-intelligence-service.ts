...
class McaIntelligenceService {
	private readonly SOURCE_ATTRIBUTION =
	...
	hasAccess(
		...
		return (
			...
			permissions[role]?.includes("full") ||
			false
		);
	}
	...
	async handleQuery(
		...
		try {
			...
			switch (queryType) {
				...
				case "profit_check":
					result = await this.checkProfitability(
						params.cin,
				...
				case "director_search":
					result = await this.searchByDirector(
						params.din || params.directorName,
					);
					break;
	...
	async checkFinancialAvailability(cin: string): Promise<{
		...
		return {
			...
			latestPat: snapshots[0]?.profitAfterTax
				? Number.parseFloat(snapshots[0].profitAfterTax)
				: undefined,
		};
	}
	...
	async checkProfitability(
		...
		const pat = latestSnapshot?.profitAfterTax
			? Number.parseFloat(latestSnapshot.profitAfterTax)
			: undefined;
	...
	async getCompanyFinancialRatios(cin: string): Promise<{
		...
		const revenue = latest.revenue ? Number.parseFloat(latest.revenue) : null;
		const pat = latest.profitAfterTax
			? Number.parseFloat(latest.profitAfterTax)
			: null;
		const netWorth = latest.netWorth
			? Number.parseFloat(latest.netWorth)
			: null;
		const totalAssets = latest.totalAssets
			? Number.parseFloat(latest.totalAssets)
			: null;
		const totalLiabilities = latest.totalLiabilities
			? Number.parseFloat(latest.totalLiabilities)
			: null;
		const longTermBorrowing = latest.longTermBorrowing
			? Number.parseFloat(latest.longTermBorrowing)
			: 0;
		const shortTermBorrowing = latest.shortTermBorrowing
			? Number.parseFloat(latest.shortTermBorrowing)
			: 0;
		const totalBorrowing = longTermBorrowing + shortTermBorrowing;

		// Calculate ratios
		const patMargin =
			revenue && pat && revenue > 0 ? (pat / revenue) * 100 : null;
		const returnOnEquity =
			netWorth && pat && netWorth > 0 ? (pat / netWorth) * 100 : null;
		const returnOnAssets =
			totalAssets && pat && totalAssets > 0 ? (pat / totalAssets) * 100 : null;
		const debtToEquity =
			netWorth && netWorth > 0 ? totalBorrowing / netWorth : null;
		const assetTurnover =
			totalAssets && revenue && totalAssets > 0 ? revenue / totalAssets : null;
		...
		if (snapshots.length >= 2) {
			...
			// Revenue CAGR
			const oldestRevenue = oldest.revenue
				? Number.parseFloat(oldest.revenue)
				: null;
			if (oldestRevenue && oldestRevenue > 0 && revenue && revenue > 0) {
				revenueCAGR = ((revenue / oldestRevenue) ** (1 / years) - 1) * 100;
			}

			// PAT CAGR (only if both are positive)
			const oldestPat = oldest.profitAfterTax
				? Number.parseFloat(oldest.profitAfterTax)
				: null;
			if (oldestPat && oldestPat > 0 && pat && pat > 0) {
				patCAGR = ((pat / oldestPat) ** (1 / years) - 1) * 100;
			}

			// YoY Growth (year-over-year from previous year)
			const prevRevenue = previousYear.revenue
				? Number.parseFloat(previousYear.revenue)
				: null;
			if (prevRevenue && prevRevenue > 0 && revenue) {
				revenueYoY = ((revenue - prevRevenue) / prevRevenue) * 100;
			}

			const prevPat = previousYear.profitAfterTax
				? Number.parseFloat(previousYear.profitAfterTax)
				: null;
		...
		return {
			...
			ratios: {
				patMargin:
					patMargin !== null ? Math.round(patMargin * 100) / 100 : null,
				returnOnEquity:
					returnOnEquity !== null
						? Math.round(returnOnEquity * 100) / 100
						: null,
				returnOnAssets:
					returnOnAssets !== null
						? Math.round(returnOnAssets * 100) / 100
						: null,
				debtToEquity:
					debtToEquity !== null ? Math.round(debtToEquity * 100) / 100 : null,
				assetTurnover:
					assetTurnover !== null ? Math.round(assetTurnover * 100) / 100 : null,
			},
			growth: {
				revenueCAGR:
					revenueCAGR !== null ? Math.round(revenueCAGR * 100) / 100 : null,
				patCAGR: patCAGR !== null ? Math.round(patCAGR * 100) / 100 : null,
				revenueYoY:
					revenueYoY !== null ? Math.round(revenueYoY * 100) / 100 : null,
	...
	async getFinancialHistory(
		...
		if (snapshots.length === 0 && autoEnrich) {
			console.log(
				`[MCA Intelligence] No cached data for ${cin}, triggering API enrichment...`,
			);

			try {
				const { mcaDataCacheService } = await import(
					"./mca-data-cache-service"
				);
				const cacheResult = await mcaDataCacheService.getCompanyWithCache(cin, {
					forceRefresh: false,
				});
				...
				if (cacheResult.success && cacheResult.apiCallMade) {
					...
					console.log(
						`[MCA Intelligence] API enrichment successful for ${cin}`,
					);
					...
					console.log(
						`[MCA Intelligence] API enrichment failed for ${cin}: ${enrichmentError}`,
					);
				}
			} catch (error: any) {
				enrichmentError = error.message || "Enrichment error";
				console.error(
					`[MCA Intelligence] Enrichment error for ${cin}:`,
					error.message,
				);
			}
		}
		...
		const financialYears = snapshots.map((snapshot, index) => {
			const revenue = snapshot.revenue
				...
				: null;
			const pat = snapshot.profitAfterTax
				? Number.parseFloat(snapshot.profitAfterTax)
				: null;
			const netWorth = snapshot.netWorth
				? Number.parseFloat(snapshot.netWorth)
				: null;
			...
			const totalLiabilities = snapshot.totalLiabilities
				? Number.parseFloat(snapshot.totalLiabilities)
				: null;
			const longTermBorrowing = snapshot.longTermBorrowing
				? Number.parseFloat(snapshot.longTermBorrowing)
				: 0;
			const shortTermBorrowing = snapshot.shortTermBorrowing
				? Number.parseFloat(snapshot.shortTermBorrowing)
				: 0;
			const totalBorrowing = longTermBorrowing + shortTermBorrowing;

			// Calculate ratios
			const patMargin =
				revenue && pat && revenue > 0
					? Math.round((pat / revenue) * 10000) / 100
					: null;
			const returnOnEquity =
				netWorth && pat && netWorth > 0
					? Math.round((pat / netWorth) * 10000) / 100
					: null;
			const returnOnAssets =
				totalAssets && pat && totalAssets > 0
					? Math.round((pat / totalAssets) * 10000) / 100
					: null;
			const debtToEquity =
				netWorth && netWorth > 0
					? Math.round((totalBorrowing / netWorth) * 100) / 100
					: null;
			const assetTurnover =
				totalAssets && revenue && totalAssets > 0
			...
			if (index < snapshots.length - 1) {
				...
				const prevRevenue = prevSnapshot.revenue
					? Number.parseFloat(prevSnapshot.revenue)
					: null;
				const prevPat = prevSnapshot.profitAfterTax
					? Number.parseFloat(prevSnapshot.profitAfterTax)
					: null;
				const prevNetWorth = prevSnapshot.netWorth
					? Number.parseFloat(prevSnapshot.netWorth)
					: null;
				...
				if (prevNetWorth && prevNetWorth > 0 && netWorth) {
					netWorthGrowth =
						Math.round(((netWorth - prevNetWorth) / prevNetWorth) * 10000) /
						100;
				}
			}
	...
	async calculateRiskScore(cin: string): Promise<{
		...
		if (!company) {
			return {
				...
				riskGrade: "F",
				components: {
					profitConsistency: { score: 100, weight: 0.3, details: "No data" },
					leverage: { score: 100, weight: 0.3, details: "No data" },
					complianceFreshness: { score: 100, weight: 0.2, details: "No data" },
					companyStatus: { score: 100, weight: 0.1, details: "No data" },
					operatingMargins: { score: 100, weight: 0.1, details: "No data" },
				},
		...
		if (snapshots.length >= 2) {
			const patValues = snapshots
				.map((s) => Number.parseFloat(s.profitAfterTax || "0"))
			...
			if (patValues.length >= 2) {
				...
				const variance =
					patValues.reduce((a, b) => a + (b - mean) ** 2, 0) / patValues.length;
				...
				profitScore = Math.min(
					100,
					Math.max(0, Math.round((1 - profitRatio) * 50 + volatility * 50)),
				);
		...
		if (snapshots.length > 0) {
			const latest = snapshots[0];
			const netWorth = Number.parseFloat(latest.netWorth || "0");
			const longTerm = Number.parseFloat(latest.longTermBorrowing || "0");
			const shortTerm = Number.parseFloat(latest.shortTermBorrowing || "0");
			...
			if (netWorth > 0) {
				...
				if (debtToEquity <= 0.5) {
					...
					leverageScore = Math.min(
						100,
						Math.round(80 + (debtToEquity - 3) * 10),
					);
				}
		...
		if (company.lastBalanceSheet || company.lastFilingYear) {
			...
			if (company.lastBalanceSheet) {
				...
				if (fyMatch) {
					yearsAgo = currentYear - Number.parseInt(fyMatch[1]) - 1;
				}
			}
		...
		if (snapshots.length > 0) {
			const latest = snapshots[0];
			const revenue = Number.parseFloat(latest.revenue || "0");
			const pat = Number.parseFloat(latest.profitAfterTax || "0");
			...
			if (revenue > 0) {
				...
				if (patMargin >= 20) {
					...
					marginScore = Math.min(
						100,
						Math.round(70 + Math.abs(patMargin) * 0.5),
					);
				}
	...
	async getFilingStatus(cin: string): Promise<FilingStatusResult> {
		...
		const isCurrent =
			lastFilingYear === expectedFilingYear ||
			lastFilingYear ===
	...
	async getWalletStatus(): Promise<WalletInfo> {
		...
		const balance = Number.parseFloat(wallet.currentBalance || "0");
		const threshold = Number.parseFloat(wallet.alertThreshold || "1000");

		return {
			currentBalance: balance,
			monthlySpend: Number.parseFloat(wallet.totalSpentThisMonth || "0"),
			totalSpentAllTime: Number.parseFloat(wallet.totalSpentAllTime || "0"),
			monthlyBudget: Number.parseFloat(wallet.monthlyBudget || "50000"),
	...
	async getProfitableCompanies(params: {
		...
		const query = db
			...
			.leftJoin(
				mcaCompanyMaster,
				eq(mcaFinancialSnapshot.cin, mcaCompanyMaster.cin),
			)
			.where(
				and(
					gte(mcaFinancialSnapshot.profitAfterTax, patMin.toString()),
					params.patMax
						? lte(mcaFinancialSnapshot.profitAfterTax, params.patMax.toString())
						: undefined,
					params.state
						? eq(mcaCompanyMaster.registeredState, params.state)
						: undefined,
					params.industry
						? like(mcaCompanyMaster.industry, `%${params.industry}%`)
						: undefined,
				),
			)
		...
		return results.map((r) => {
			const pat = Number.parseFloat(r.profitAfterTax || "0");
			const revenue = r.revenue ? Number.parseFloat(r.revenue) : undefined;
			const netWorth = r.netWorth ? Number.parseFloat(r.netWorth) : undefined;
			const longTermBorrowing = r.longTermBorrowing
				? Number.parseFloat(r.longTermBorrowing)
				: 0;
			const shortTermBorrowing = r.shortTermBorrowing
				? Number.parseFloat(r.shortTermBorrowing)
				: 0;
			const totalBorrowing = longTermBorrowing + shortTermBorrowing;
	...
	async analyzeCharges(cin: string): Promise<{
		...
		const closedCharges = charges.filter(
			(c) => c.status.toLowerCase() === "closed",
		);
	...
	private parseXBRL(xbrlContent: string): XBRLFinancials {
		...
		const extractValue = (pattern: RegExp): number | undefined => {
			...
			if (match?.[1]) {
				return Number.parseFloat(match[1].replace(/[,\s]/g, ""));
			}
			return undefined;
		};

		financials.profitAfterTax =
			extractValue(/ProfitLossForPeriod[^>]*>([0-9.,]+)</i) ||
			extractValue(/ProfitAfterTax[^>]*>([0-9.,]+)</i);

		financials.profitBeforeTax = extractValue(
			/ProfitBeforeTax[^>]*>([0-9.,]+)</i,
		);

		financials.revenue =
			extractValue(/RevenueFromOperations[^>]*>([0-9.,]+)</i) ||
			extractValue(/TotalRevenue[^>]*>([0-9.,]+)</i);

		financials.netWorth =
			extractValue(/NetWorth[^>]*>([0-9.,]+)</i) ||
			extractValue(/ShareholdersEquity[^>]*>([0-9.,]+)</i);

		financials.totalAssets = extractValue(/TotalAssets[^>]*>([0-9.,]+)</i);

		financials.totalLiabilities = extractValue(
			/TotalLiabilities[^>]*>([0-9.,]+)</i,
		);

		financials.shareCapital =
			extractValue(/ShareCapital[^>]*>([0-9.,]+)</i) ||
			extractValue(/PaidUpCapital[^>]*>([0-9.,]+)</i);

		financials.reserves = extractValue(/ReservesAndSurplus[^>]*>([0-9.,]+)</i);

		financials.longTermBorrowing = extractValue(
			/LongTermBorrowings[^>]*>([0-9.,]+)</i,
		);

		financials.shortTermBorrowing = extractValue(
			/ShortTermBorrowings[^>]*>([0-9.,]+)</i,
		);
	 ...
	 */
	async getWalletPaymentByOrderId(
		orderId: string,
	 ...
	 */
	async updateWalletPaymentStatus(
		orderId: string,
		update: {
	 ...
	 */
	async markPaymentSuccessIfPending(
		orderId: string,
		update: {
			transactionId?: string;
			paymentMethod?: string;
	...
	async companyExists(cin: string): Promise<boolean> {
		const [existing] = await db
			.select({ cin: mcaCompanyMaster.cin })
			.from(mcaCompanyMaster)
			.where(eq(mcaCompanyMaster.cin, cin))
			.limit(1);
		return !!existing;
	...
	async upsertCompanyMaster(data: {
		...
		if (existing) {
			...
			const [updated] = await db
				...
				.set({
					...
					sourceAttribution:
						data.sourceAttribution || existing.sourceAttribution,
					updatedAt: new Date(),
				})
				.where(eq(mcaCompanyMaster.cin, data.cin))
				.returning();
			console.log(
				`[MCA Intelligence] Updated company master: ${data.cin} - ${data.companyName}`,
			);
			return updated;
		}
		// Insert new record
		const [created] = await db
			...
			.returning();
		console.log(
			`[MCA Intelligence] Created company master: ${data.cin} - ${data.companyName}`,
		);
		return created;
	}
	...
	async upsertFinancialSnapshot(data: {
		...
		if (existing) {
			...
			const [updated] = await db
				...
				.set({
					...
					longTermBorrowing:
						data.longTermBorrowing ?? existing.longTermBorrowing,
					shortTermBorrowing:
						data.shortTermBorrowing ?? existing.shortTermBorrowing,
				...
				.returning();
			console.log(
				`[MCA Intelligence] Updated financial snapshot: ${data.cin} FY ${data.financialYear}`,
			);
			return updated;
		}
		// Insert new record
		const [created] = await db
			...
			.returning();
		console.log(
			`[MCA Intelligence] Created financial snapshot: ${data.cin} FY ${data.financialYear}`,
		);
		return created;
	}
	 ...
	 */
	async getCompaniesNeedingRefresh(
		options: {
			maxAgeDays?: number;
			limit?: number;
	...
	> {
		...
		for (const company of companies) {
			...
			if (company.companyStatus?.toLowerCase().includes("strike")) {
				continue;
			}
	...
	async enrichCompanyData(
		...
		console.log(
			`[MCA Intelligence] Enriching company data for ${cin} (forceRefresh: ${forceRefresh})`,
		);
		...
		try {
			...
			return {
				success: true,
				cin,
				message: result.apiCallMade
					? "Data enriched from API"
	...
	async bulkEnrichCompanies(
		...
		console.log(
			`[MCA Intelligence] Bulk enriching ${cins.length} companies (batch: ${batchSize}, delay: ${delayMs}ms)`,
		);
		...
		console.log(
			`[MCA Intelligence] Bulk enrichment complete: ${successful}/${cins.length} successful, ${apiCallsMade} API calls`,
		);
	...
	async refreshStaleCompanies(
		...
		console.log(
			`[MCA Intelligence] Starting stale company refresh (maxAge: ${maxAgeDays} days, limit: ${limit})`,
		);

		const staleCompanies = await this.getCompaniesNeedingRefresh({
			maxAgeDays,
			limit,
			onlyUnlisted,
		});
		...
		const bulkResult = await this.bulkEnrichCompanies(cins, {
			forceRefresh: true,
		});
	...
	async getEnrichmentStats(): Promise<{
		...
		const [withFinancialsResult] = await db
			.select({
				count: sql<number>`count(distinct ${mcaFinancialSnapshot.cin})`,
			})
			.from(mcaFinancialSnapshot);
