...
class DSCTokenESignService {
	...
	constructor() {
		...
		this.tsaUrl =
			process.env.DSC_TSA_URL ||
			DSCTokenESignService.TSA_URLS[this.environment];

		console.log(
			`✅ [DSC Token eSign] Service initialized in ${this.environment.toUpperCase()} mode`,
		);
	...
	private validateIssuer(issuerCN: string): {
		valid: boolean;
		recognized: boolean;
	} {
		const isRecognized = DSCTokenESignService.KNOWN_ISSUERS.some((issuer) =>
			issuerCN.toLowerCase().includes(issuer.toLowerCase()),
		);
		return { valid: true, recognized: isRecognized };
	}

	private async checkOCSPStatus(
		certificate: DSCCertificateInfo,
	...
	async validateCertificate(certInfo: DSCCertificateInfo): Promise<{
		...
		if (!issuerCheck.recognized) {
			warnings.push(
				`Certificate issuer not in known list: ${certInfo.issuer.commonName}`,
			);
		}
		...
		if (
			certInfo.certificateClass === "Class1" &&
			[
				"itr_verification",
				"form_15ca",
				"form_15cb",
				"investment_agreement",
			].includes("itr_verification")
		) {
			errors.push(
				"Class 1 certificates cannot be used for financial documents. Class 2 or 3 required.",
			);
		}
	...
	async initiateSigningSession(
		request: DSCSigningRequest,
	): Promise<DSCSigningInitResponse> {
		...
		try {
			...
			const dataToSign = this.prepareDataToSign(
			...
			return {
				...
				requestId,
				message:
					"DSC signing session initiated. Please sign the data using your token.",
				provider: "dsc_token",
				...
				validationDetails: {
					...
					issuer: request.certificateInfo.issuer.commonName,
					validUntil: request.certificateInfo.validTo,
					ocspStatus: validation.details.ocspStatus as
						| "good"
						| "revoked"
						| "unknown",
				},
	...
	async submitSignature(
		submission: DSCSignatureSubmission,
	): Promise<DSCSigningCompleteResponse> {
		try {
			...
			if (esignRequest.status !== "pending_signature") {
				throw new AppError(
					`Invalid request status: ${esignRequest.status}`,
					400,
					"INVALID_STATUS",
				);
			}
			...
			await db.insert(esignCertificates).values({
				...
				validTo: new Date(
					tokenInfo?.validTo ||
						new Date(submission.signedAt.getTime() + 365 * 24 * 60 * 60 * 1000),
				),
			...
			return {
				...
				signatureData: {
					...
					validTo: new Date(
						tokenInfo?.validTo ||
							new Date(
								submission.signedAt.getTime() + 365 * 24 * 60 * 60 * 1000,
							),
					),
			...
			throw new AppError(
				"Failed to complete DSC signing",
				500,
				"DSC_ESIGN_COMPLETE_FAILED",
			);
		}
	}
	...
	private async getTimestamp(
		signature: string,
	): Promise<{ timestamp: Date; authority: string } | null> {
		if (this.isInMockMode()) {
			return {
				timestamp: new Date(),
	...
	async cancelSession(
		...
		try {
			...
			if (esignRequest.status === "completed") {
				throw new AppError(
					"Cannot cancel completed signing",
					400,
					"ALREADY_COMPLETED",
				);
			}
			...
			await this.logAuditEvent(transactionId, userId, "dsc_cancel", "success", {
				reason,
			});
			...
			if (error instanceof AppError) throw error;
			throw new AppError(
				"Failed to cancel DSC signing session",
				500,
				"DSC_CANCEL_FAILED",
			);
		}
