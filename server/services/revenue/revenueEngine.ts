...
export class RevenueEngine {
	...
	async recordRevenueEvent(event: {
		...
		logger.info(
			`[RevenueEngine] Processing revenue event for provider ${event.providerId}`,
		);

		let estimatedRevenue = 0;

		if (event.domain === "INVESTMENT") {
			estimatedRevenue = this.calculateInvestmentRevenue(
				event.providerId,
				event.transactionValue,
			);
		} else if (event.domain === "CREDIT") {
			estimatedRevenue = this.calculateCreditRevenue(
				event.providerId,
				event.productType,
				event.transactionValue,
			);
	...
	private calculateInvestmentRevenue(
		providerId: string,
		value: number,
	): number {
		switch (providerId) {
	...
	private calculateCreditRevenue(
		providerId: string,
