...
export class ChatGPTService {
	...
	async streamMessage(
		...
		const { provider = "openai", model = "gpt-4o" } = options;
		const result = await aiService.streamChat(messages, onChunk, {
			provider,
			model,
			...options,
		});
...
Recommend the top 5 ${productType} for this user. Respond in JSON format:
{
  ...
  "reasoning": "Overall reasoning for these recommendations"
	...
	async generateReport(
		...
		const systemPrompts = {
			portfolio_summary:
				"You are a financial report writer. Create a comprehensive portfolio summary report with clear sections and professional language.",
			tax_report:
				"You are a tax reporting expert. Generate a detailed tax report covering capital gains, dividends, and tax-saving recommendations for India.",
			performance_review:
				"You are an investment performance analyst. Create a detailed performance review with metrics, comparisons, and insights.",
			compliance:
				"You are a regulatory compliance expert. Generate a compliance report covering KYC, SEBI regulations, and investor protection guidelines.",
		};
...
Respond in JSON format:
{
  ...
  "extractedData": { "key": "value" } (data extracted from user response)
	...
	private buildMessagesFromHistory(
		...
		const systemPrompt =
			customSystemPrompt || this.getDefaultSystemPrompt(session.sessionType);
