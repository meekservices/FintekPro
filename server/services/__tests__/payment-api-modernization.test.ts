...
async function runTests() {
	...
	await runTest("Cashfree v5: createOrder payload check", async () => {
		...
		if (req.headers["x-api-version"] !== "2025-01-01")
			throw new Error("Incorrect API version header");
		if (req.body.customer_details.customer_id !== "user_123")
			throw new Error("Missing customer_id in v5 payload");
		if (!req.body.order_meta.return_url.includes("order_id={order_id}"))
			throw new Error("Incorrect return_url format for v5");
	});
	...
	await runTest("Cashfree v5: createRefund endpoint check", async () => {
		...
		const req = mockAxios.lastRequest;
		if (req.url !== "/orders/order_123/refunds")
			throw new Error("Incorrect refund endpoint for v5");
		if (req.body.refund_amount !== 50)
			throw new Error("Incorrect refund amount");
	});
	...
	await runTest("Alpaca: createRtpTransfer payload check", async () => {
		...
		const req = mockAxios.lastRequest;
		if (req.url !== "/v1/accounts/acc_123/transfers/rtp")
			throw new Error("Incorrect RTP endpoint");
		if (req.body.amount !== "500.00")
			throw new Error("Incorrect amount structure");
	});
	...
	await runTest("Alpaca: listBankAccounts endpoint check", async () => {
		...
		const req = mockAxios.lastRequest;
		if (req.url !== "/v1/accounts/acc_123/bank_accounts")
			throw new Error("Incorrect bank accounts endpoint");
	});
	...
	await runTest("Alpaca: getPortfolioHistoryWithResolution logic", async () => {
		...
		await service.getPortfolioHistoryWithResolution("acc_123", {
			timeframe: "5Min",
		});

		const req = mockAxios.lastRequest;
		if (!req.url.includes("/account/portfolio/history"))
			throw new Error("Incorrect history endpoint");
	...
	await runTest("Iris: getDirectPayStatus logic", async () => {
		...
		const req = mockAxios.lastRequest;
		if (!req.url.includes("/sif/transactions/direct-pay-status"))
			throw new Error("Incorrect Direct Pay endpoint");
		if (
			!req.url.includes("pan=PAN123") ||
			!req.url.includes("accountNo=BANK123")
		)
	...
	await runTest("Iris: listActiveMandatesByBank logic", async () => {
		...
		const req = mockAxios.lastRequest;
		if (!req.url.includes("/sif/mandates/active"))
			throw new Error("Incorrect Mandates endpoint");
	});
