...
class ErrorWebhookService {
	async getActiveWebhooks(
		...
		try {
			...
			console.error(
				"[ErrorWebhookService] Failed to get active webhooks:",
				error,
			);
			return [];
		}
	...
	async sendCriticalAlert(payload: AlertPayload): Promise<void> {
		...
		console.log(
			`[ErrorWebhookService] Sending critical alert to ${criticalWebhooks.length} webhooks`,
		);
	...
	async sendSpikeAlert(payload: AlertPayload): Promise<void> {
		...
		console.log(
			`[ErrorWebhookService] Sending spike alert to ${spikeWebhooks.length} webhooks`,
		);
	...
	private async sendToWebhook(
		...
		if (webhook.lastTriggeredAt) {
			const timeSinceLastTrigger =
				now.getTime() - new Date(webhook.lastTriggeredAt).getTime();
			if (timeSinceLastTrigger < cooldownMs) {
				console.log(
					`[ErrorWebhookService] Webhook ${webhook.name} is in cooldown, skipping`,
				);
				return;
			}
		}
		...
		if (webhook.triggerModules && webhook.triggerModules.length > 0) {
			if (!webhook.triggerModules.includes(payload.module)) {
				console.log(
					`[ErrorWebhookService] Module ${payload.module} not in webhook trigger list, skipping`,
				);
				return;
			}
		}
		...
		try {
			...
			alertHistory.deliveryResponse = JSON.stringify({
				status: response.status,
			});

			await db
				.update(errorWebhookConfig)
				.set({ lastTriggeredAt: now, updatedAt: now })
				.where(eq(errorWebhookConfig.id, webhook.id));

			console.log(
				`[ErrorWebhookService] Successfully sent alert to ${webhook.name}`,
			);
		} catch (error: any) {
			console.error(
				`[ErrorWebhookService] Failed to send to ${webhook.name}:`,
				error.message,
			);
			...
			try {
				...
				console.error(
					"[ErrorWebhookService] Failed to record alert history:",
					dbError,
				);
			}
		}
	...
	private formatSlackPayload(payload: AlertPayload): SlackMessage {
		...
		const fields = [
			...
			{
				title: "Occurrences",
				value: String(payload.occurrenceCount || 1),
				short: true,
			},
			...(payload.windowMinutes
				? [
						{
							title: "Window",
							value: `${payload.windowMinutes} min`,
							short: true,
						},
					]
				: []),
			{
				title: "Error IDs",
				value:
					payload.errorIds.slice(0, 3).join(", ") +
					(payload.errorIds.length > 3 ? "..." : ""),
				short: false,
			},
		];
		...
		try {
			...
			return (
				...
				({
					...
					autoEscalateToCritical: true,
				} as ErrorAlertThreshold)
			);
		} catch (error) {
			console.error(
				"[ErrorWebhookService] Failed to get threshold config:",
				error,
			);
			return {
				windowMinutes: 5,
