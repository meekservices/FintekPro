import { ckycProviderResolutionService, ProviderResolutionResult } from './ckyc-provider-resolution-service';
import { ckycDeferredService } from './ckyc-deferred-service';
import { ckycEnvironmentService, CkycComplianceError } from './ckyc-environment-service';
import { db } from '../db';
...
export interface CkycProviderResult {
  ...
  slaDeadline?: Date;
  fallbackAttempts?: Array<{ provider: string; failedAt: string; reason: string }>;
}
...
class CkycOrchestratorService {
   ...
   */
  async initiateVerification(context: CkycOrchestratorContext): Promise<CkycProviderResult> {
    console.log(`[CKYC Orchestrator] Initiating verification for PAN: ${context.panNumber.slice(0, 4)}***`);
    
    // Check if user already has an active deferred case
    const existingDeferred = await this.checkExistingDeferredCase(context.userId);
    if (existingDeferred) {
      console.log(`[CKYC Orchestrator] User has existing deferred case: ${existingDeferred.id}`);
      return {
        status: 'deferred',
        deferralCode: existingDeferred.deferralCode,
        deferralMessage: existingDeferred.deferralMessage || 'CKYC verification is pending admin review',
    ...
    if (resolution.status === 'deferred' || resolution.selectedProvider === 'none') {
  ...
  private async handleProviderExhaustion(
    ...
    console.log(`[CKYC Orchestrator] All providers exhausted for user ${context.userId}`);
    ...
    const deferralMessage = resolution.deferralReason || 
    ...
    try {
      ...
      const [deferredCase] = await db.insert(ckycDeferredCases).values({
        userId: context.userId,
        panNumber: context.panNumber,
        status: 'ckyc_deferred',
        deferralCode,
        deferralMessage,
        lastProviderAttempted: resolution.fallbackAttempts?.slice(-1)[0]?.provider || 'unknown',
        fallbackAttempts: resolution.fallbackAttempts || [],
      ...
      console.log(`[CKYC Orchestrator] Created deferred case: ${deferredCase.id}`);
      
      // Log compliance event
      console.log(`[CKYC COMPLIANCE] DEFERRED: User ${context.userId}, Case ${deferredCase.id}, Code: ${deferralCode}`);
      ...
      console.error(`[CKYC Orchestrator] Failed to create deferred case:`, error);
  ...
  private async checkExistingDeferredCase(userId: string): Promise<any | null> {
    try {
      ...
      if (existingCase && ['ckyc_deferred', 'manual_review_in_progress'].includes(existingCase.status)) {
        return existingCase;
      }
      return null;
    } catch (error) {
      console.error(`[CKYC Orchestrator] Error checking existing deferred case:`, error);
      return null;
    }
  }
   ...
   */
  async recordVerificationSuccess(userId: string, ckycNumber: string, providerId: string): Promise<void> {
    console.log(`[CKYC Orchestrator] Recording success for user ${userId}, CKYC: ${ckycNumber}`);
  ...
  async recordProviderFailure(
    ...
    console.log(`[CKYC Orchestrator] Provider ${providerId} failed for user ${userId}: ${reason}`);
