...
class KycOrchestratorService {
	...
	async createSession(params: {
		...
		if (
			...
			profile?.ckycAuthBridgeStatus === "found"
		) {
		...
		const kycLevel = Number.parseInt(profile?.kycLevel || "0", 10);
		...
		try {
			...
			console.error(
				"[KYC_ORCHESTRATOR] Failed to persist session to DB:",
				dbErr?.message,
			);
		}
	...
	canOverrideEntity(requestorRole: string): {
		allowed: boolean;
	...
	async buildPanVerificationResult(
		...
		return {
			...
			source: "sandbox",
			full_name:
				verificationData?.registered_name || verificationData?.fullName,
	...
	computeCkycConfidence(ckycData: any): CkycDecisionResult {
		...
		return {
			ckyc_found: true,
			confidence_score: Number.parseFloat(confidence.toFixed(2)),
	...
	resolveNextStep(
		...
		for (let i = currentIndex + 1; i < STEP_ORDER.length; i++) {
			...
			if (
				candidateStep === "aadhaar_otp_verify" &&
				(!stepStatus.aadhaar_required || !stepStatus.aadhaar_otp_sent)
			) {
				continue;
			}
			...
			if (
				initiatedBy === "agent" &&
				AGENT_BLOCKED_STEPS.includes(candidateStep)
			) {
				continue;
			}
	...
	computeAmlResult(screeningData: any): AmlCheckResult {
		const score =
			screeningData?.riskProfile?.riskScore || screeningData?.aml_score || 10;
		const pep =
			screeningData?.pepMatch?.length > 0 || screeningData?.pep === true;
		const sanctions =
			screeningData?.sanctionsMatch?.length > 0 ||
			screeningData?.sanctions === true;
	...
	computeTierResult(params: {
		...
		if (
			(params.ckycFetched || params.aadhaarVerified) &&
			params.kycLevel >= 2
		) {
			level = 2;
			tier = "enhanced";
			productsUnlocked.push(
				"stocks",
				"bonds",
				"mutual_funds_full",
				"unlisted_shares",
		...
		if (params.complianceSigned && params.amlScreened) {
			if (
				params.amlRiskLevel === "HIGH" ||
				params.amlRiskLevel === "CRITICAL"
			) {
				tierStatus = "provisional";
			...
			tierStatus = "provisional";
			if (!params.riskProfilingDone)
				upgradeActions.push("Complete risk profiling");
			if (!params.complianceSigned)
				upgradeActions.push("Complete FATCA & compliance sign-off");
	...
	async logAuditEvent(params: {
		...
		try {
			...
			console.error(
				"[KYC_AUDIT] Failed to persist audit event to DB:",
				dbErr?.message,
			);
		}
