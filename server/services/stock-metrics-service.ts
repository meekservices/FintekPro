...
export class StockMetricsService {
	...
	static getInstance(): StockMetricsService {
		if (!StockMetricsService.instance) {
			StockMetricsService.instance = new StockMetricsService();
		}
		return StockMetricsService.instance;
	}
	...
	async fetchFromFinnhub(symbol: string): Promise<StockMetrics | null> {
		...
		try {
			...
			if (!response.ok) {
				console.warn(
					`[StockMetrics] Finnhub returned ${response.status} for ${symbol}`,
				);
				return null;
			}
			...
			return {
				...
				marketCapValue: m.marketCapitalization
					? m.marketCapitalization * 10000000
					: null, // Convert to INR
				dataSource: "Finnhub",
			...
			console.error(
				`[StockMetrics] Finnhub error for ${symbol}:`,
				error.message,
			);
			return null;
		}
	...
	async fetchFromGoogleFinance(symbol: string): Promise<StockMetrics | null> {
		try {
			...
			console.error(
				`[StockMetrics] Google Finance error for ${symbol}:`,
				error.message,
			);
			return null;
		}
	...
	async fetchFromYahooFinance(symbol: string): Promise<StockMetrics | null> {
		try {
			...
			return {
				peRatio:
					keyStats.trailingPE?.raw || summaryDetail.trailingPE?.raw || null,
			...
			console.error(
				`[StockMetrics] Yahoo Finance error for ${symbol}:`,
				error.message,
			);
			return null;
		}
	...
	calculateROCE(
		...
		const taxRate = 0.25;
		const roce =
			(roe * (1 - taxRate) * (1 + debtToEquity)) /
			(1 + debtToEquity * (1 - taxRate));
	...
	async batchUpdateMetrics(
		...
		console.log(
			`[StockMetrics] Starting batch update for ${symbols.length} stocks...`,
		);
		...
		for (let i = 0; i < symbols.length; i += batchSize) {
			...
			console.log(
				`[StockMetrics] Progress: ${processed}/${symbols.length} (${successfulUpdates} updated, ${skippedNoData} skipped, ${failedUpdates} failed)`,
			);
	...
	async refreshAllMetrics(
		...
		if (forceRefresh) {
			...
			query = db
				.select({ symbol: listedStocks.symbol })
				.from(listedStocks)
				.where(or(isNull(listedStocks.peRatio), isNull(listedStocks.roe)))
				.limit(limit);
		}
