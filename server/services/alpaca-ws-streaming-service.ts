...
class AlpacaWsStreamingService extends EventEmitter {
	...
	private get isSandbox(): boolean {
		...
		return (
			base.includes("sandbox") || !base.includes("broker-api.alpaca.markets")
		);
	}
	...
	connect(_feed?: string): void {
		if (!this.isConfigured()) {
			logger.warn(
				"[AlpacaWS] Not configured — set ALPACA_API_KEY / ALPACA_SECRET_KEY",
			);
			return;
		}
		if (
	...
	private scheduleReconnect(): void {
		...
		logger.info(
			`[AlpacaWS] Reconnecting in ${Math.round(delay / 1000)}s (attempt ${this.reconnectAttempts})`,
		);
	...
	private handleMessage(msg: any): void {
		...
		if (T === "subscription") {
			logger.info(
				`[AlpacaWS] Subscription confirmed: trades=${msg.trades?.length || 0} quotes=${msg.quotes?.length || 0} bars=${msg.bars?.length || 0}`,
			);
			return;
		}
