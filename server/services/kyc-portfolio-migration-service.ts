...
class KycPortfolioMigrationService {
	...
	async onKycComplete(params: {
		...
		console.log(
			`[KycMigration] Starting migration for prospect ${prospectId} -> user ${userId}`,
		);
		...
		try {
			...
			const prospectHoldings =
				await unifiedHoldingsReaderService.getHoldings(prospectId);
			console.log(
				`[KycMigration] Found ${prospectHoldings.length} holdings to migrate`,
			);
			...
			const hasAAConsent = consentRecords.some(
				(c) =>
					c.consentGiven &&
			...
			console.log(
				`[KycMigration] Migration complete: ${migratedHoldings} holdings migrated`,
			);
	...
	private async recordConsents(
		...
		for (const consent of consents) {
			await db.insert(dataSourceConsents).values({
				...
				provider:
					consent.dataSource === "mutual_funds"
						? "BSE_STAR"
						: consent.dataSource === "demat"
							? "NSDL_CDSL"
							: "ALL",
	...
	private async migrateHoldingsToComprehensive(
		...
		for (const holding of holdings) {
			...
			if (existingKey) {
				...
				if (duplicate) {
					console.log(
						`[KycMigration] Skipping duplicate: ${holding.isin} / ${holding.folioNumber}`,
					);
					continue;
				}
			}
			...
			const record: InsertComprehensiveHolding = {
				...
				avgCostPerUnit: holding.averageCost
				...
				costBasis: holding.investedValue
					? String(holding.investedValue)
					: undefined,
				unrealizedGainLoss: holding.unrealizedGain
					? String(holding.unrealizedGain)
					: undefined,
				unrealizedGainLossPercent: holding.unrealizedGainPercent
					? String(holding.unrealizedGainPercent)
					: undefined,
	 ...
	 */
	async onAutoSyncRefresh(
		userId: string,
		dataSource: "mutual_funds" | "demat" | "all",
	): Promise<{
		clearedCount: number;
		readyForFetch: boolean;
	}> {
		console.log(
			`[KycMigration] Auto-sync refresh triggered for ${userId}, source: ${dataSource}`,
		);
		...
		console.log(
			`[KycMigration] Cleared ${clearedCount} holdings before refresh`,
		);
	...
	private getConsentText(dataSource: string): string {
		const texts: Record<string, string> = {
			mutual_funds:
				"I authorize FintekPro to fetch my mutual fund holdings from CAMS/KFintech via BSE StarMF for portfolio analysis and investment advisory.",
			demat:
				"I authorize FintekPro to fetch my demat holdings from NSDL/CDSL via Account Aggregator for portfolio analysis and investment advisory.",
