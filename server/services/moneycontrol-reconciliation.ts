...
class MoneyControlReconciliationService {
	...
	private normalizeForMatch(name: string): string {
		return name
			.toLowerCase()
			.replace(
				/\b(ltd|limited|pvt|private|inc|incorporated|llp|llc|india|co|company)\b/gi,
				"",
			)
			.replace(/[^\w\s]/g, "")
			.replace(/\s+/g, " ")
	...
	async fetchAndCacheMoneyControlCompanies(
		forceRefresh: boolean = false,
	): Promise<MoneyControlExternalCompany[]> {
		const now = new Date();

		if (!forceRefresh && this.cache && this.cache.expiresAt > now) {
			console.log(
				`[MC Reconciliation] Using cached data (${this.cache.companies.length} companies, expires ${this.cache.expiresAt.toISOString()})`,
			);
		...
		try {
			...
			const externalCompanies: MoneyControlExternalCompany[] = mcCompanies.map(
				(mc) => ({
					name: mc.name,
					isin: mc.isin,
					price: mc.price,
			...
			console.log(
				`[MC Reconciliation] Cached ${externalCompanies.length} companies from MoneyControl`,
			);
			return externalCompanies;
		} catch (error: any) {
			console.error(
				"[MC Reconciliation] Failed to fetch MoneyControl data:",
				error.message,
			);
			throw error;
		}
	...
	async getReconciliationSuggestions(forceRefresh: boolean = false): Promise<{
		...
		console.log(
			`[MC Reconciliation] Found ${suggestions.length} unmatched companies (${matchedCount} already synced)`,
		);
	...
	async syncCompanyFromMoneyControl(
		mcCompany: MoneyControlExternalCompany,
		adminUserId?: string,
	): Promise<UnlistedCompany> {
		console.log(
			`[MC Reconciliation] Syncing company: ${mcCompany.name} (ISIN: ${mcCompany.isin})`,
		);
		...
		if (existingByIsin) {
			throw new Error(
				`Company with ISIN ${mcCompany.isin} already exists: ${existingByIsin.name}`,
			);
		}
		...
		console.log(
			`[MC Reconciliation] Successfully synced: ${newCompany.name} (ID: ${newCompany.id})`,
		);
		return newCompany;
	}

	async syncMultipleCompanies(
		mcCompanies: MoneyControlExternalCompany[],
		adminUserId?: string,
	): Promise<{
		...
		for (const mcCompany of mcCompanies) {
			try {
				const synced = await this.syncCompanyFromMoneyControl(
					mcCompany,
					adminUserId,
				);
				success.push(synced);
	...
	async lookupMarketPrice(
		...
		try {
			...
			if (isin) {
				const byIsin = mcCompanies.find(
					(c) => c.isin?.toUpperCase() === isin.toUpperCase(),
				);
			...
			if (bestScore >= 70 && bestPrice !== null) {
				console.log(
					`[MC Reconciliation] Price lookup: "${name}" → ₹${bestPrice} (score: ${bestScore})`,
				);
				return bestPrice;
			}
			...
			console.warn(
				`[MC Reconciliation] lookupMarketPrice failed for "${name}": ${err.message}`,
			);
			return null;
		}
	}
...
export const moneyControlReconciliation =
	new MoneyControlReconciliationService();
