...
import {
	dscTokenESignService,
	DSCSigningRequest,
	DSCSignatureSubmission,
} from "./dsc-token-esign-service";
...
class UnifiedESignService {
	...
	private defaultProviders: Map<ESignProvider, ESignProviderConfig> = new Map([
		[
			"truthscreen",
			{
				...
				description:
					"TruthScreen (AuthBridge) Aadhaar-based eSign with OTP verification (docType 373)",
				pricingPerSign: 12.0,
				pricingCurrency: "INR",
				isActive: true,
				isConfigured: false,
		...
		[
			"dsc_token",
			{
				...
				description:
					"Digital Signature Certificate via USB Token or Smart Card - Class 2/3 certificates",
				pricingPerSign: 0.0,
				pricingCurrency: "INR",
				isActive: true,
				isConfigured: true,
		...
		[
			"user_signature",
			{
				...
				description:
					"Use your uploaded, drawn, or typed signature to sign documents instantly",
				pricingPerSign: 0.0,
				pricingCurrency: "INR",
				isActive: true,
				isConfigured: true,
	...
	private initializeProviderStatus(): void {
		...
		truthscreenConfig.environment = truthScreenESignService.getEnvironment() as
			| "sandbox"
			| "production";

		const proteanConfig = this.defaultProviders.get("protean")!;
		proteanConfig.isConfigured = !proteanESignService.isInMockMode();
		proteanConfig.environment = proteanESignService.getEnvironment() as
			| "sandbox"
			| "production";

		const dscConfig = this.defaultProviders.get("dsc_token")!;
		dscConfig.isConfigured = true;
		dscConfig.environment = dscTokenESignService.getEnvironment() as
			| "sandbox"
			| "production";

		console.log("✅ Unified eSign Service initialized");
		console.log(
			`   TruthScreen: ${truthscreenConfig.isConfigured ? "Configured" : "Mock Mode"}`,
		);
		console.log(
			`   Protean: ${proteanConfig.isConfigured ? "Configured" : "Mock Mode"}`,
		);
		console.log(
			`   DSC Token: ${dscConfig.isConfigured ? "Available" : "Mock Mode"}`,
		);
	}
		...
		try {
			...
			for (const config of configs) {
				if (config.key.startsWith(UnifiedESignService.PRICING_PREFIX)) {
					const provider = config.key.replace(
						UnifiedESignService.PRICING_PREFIX,
						"",
					) as ESignProvider;
	...
	async setActiveProvider(
		...
		try {
			...
			console.log(
				`[UnifiedESign] Active provider changed to: ${provider} by admin: ${adminUserId}`,
			);
	...
	async getProviderConfigs(): Promise<ESignProviderConfig[]> {
		...
		return Array.from(this.defaultProviders.values()).map((config) => ({
			...config,
			pricingPerSign:
				persistedPricing.get(config.provider) ?? config.pricingPerSign,
	...
	async initiateESign(
		...
		const activeProvider = await this.getActiveProvider();
		console.log(
			`[UnifiedESign] Initiating eSign with provider: ${activeProvider}`,
		);
	...
	async verifyESign(request: ESignVerifyRequest): Promise<ESignVerifyResponse> {
		...
		console.log(
			`[UnifiedESign] Verifying eSign with provider: ${provider} for transaction: ${transactionId}`,
		);
		...
		switch (provider) {
			...
			default: {
				const defaultVerifyResult =
					await truthScreenESignService.verifyESign(request);
	...
	async resendOTP(
		transactionId: string,
	): Promise<{ success: boolean; message: string; provider: ESignProvider }> {
		const provider = this.detectProviderFromTransactionId(transactionId);
		...
		switch (provider) {
			...
			case "protean": {
				const proteanResult =
					await proteanESignService.resendOTP(transactionId);
				return { ...proteanResult, provider: "protean-esign" };
			}

			default: {
				const defaultResendResult =
					await truthScreenESignService.resendOTP(transactionId);
	...
	async getStatus(transactionId: string): Promise<{
		...
		switch (provider) {
			...
			case "protean": {
				const proteanStatus =
					await proteanESignService.getStatus(transactionId);
			...
			case "user_signature": {
				const { userSignatureESignService } = await import(
					"./user-signature-esign-service"
				);
				const userSigStatus =
					await userSignatureESignService.getSigningStatus(transactionId);
				return { ...userSigStatus, provider: "user_signature" };
			}

			default: {
				const defaultStatus =
					await truthScreenESignService.getStatus(transactionId);
	...
	async submitDSCSignature(submission: DSCSignatureSubmission) {
		console.log(
			`[UnifiedESign] Submitting DSC signature for transaction: ${submission.transactionId}`,
		);
		return dscTokenESignService.submitSignature(submission);
	}

	async cancelDSCSession(
		transactionId: string,
		userId: string,
		reason?: string,
	) {
	...
	> {
		try {
			...
			return requests.map((r) => ({
				...
				provider:
					(r.provider === "authbridge" ? "truthscreen" : r.provider) ||
					"truthscreen",
				cost: 12,
			}));
		} catch (error) {
