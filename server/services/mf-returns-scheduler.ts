...
class MFReturnsScheduler {
	...
	static getInstance(): MFReturnsScheduler {
		if (!MFReturnsScheduler.instance) {
			MFReturnsScheduler.instance = new MFReturnsScheduler();
		}
		return MFReturnsScheduler.instance;
	}
	...
	async runInitialSync(): Promise<void> {
		console.log(
			"[MFReturnsScheduler] Running initial sync for popular funds...",
		);
		...
		for (const schemeCode of POPULAR_SCHEME_CODES) {
			try {
				...
				if (returns && returns.dataQuality !== "insufficient") {
					synced++;
					console.log(
						`[MFReturnsScheduler] Synced ${schemeCode}: 1Y=${returns.returns1y?.toFixed(1)}%, 3Y=${returns.returns3y?.toFixed(1)}%`,
					);
				...
				console.log(
					`[MFReturnsScheduler] Sync failed for ${schemeCode}: ${error.message}`,
				);
			}
		}

		console.log(
			`[MFReturnsScheduler] Initial sync complete: ${synced} synced, ${failed} failed`,
		);
	 ...
	 */
	private async updateCacheFromSchemeCode(
		schemeCode: string,
		returns: {
	...
	private async warmupCacheFromDatabase(): Promise<void> {
		try {
			console.log(
				"[MFReturnsScheduler] Warming up live returns cache from database...",
			);

			const fundsWithReturns = await db
				.select({
				...
				.from(mutualFunds)
				.where(
					sql`${mutualFunds.returns1y} IS NOT NULL OR ${mutualFunds.returns3y} IS NOT NULL`,
				)
				.limit(500);
				...
				if (fund.schemeName) {
					updateLiveReturnsCache(fund.schemeName, {
						returns1Y: fund.returns1y
							? Number.parseFloat(fund.returns1y as string)
							: null,
						returns3Y: fund.returns3y
							? Number.parseFloat(fund.returns3y as string)
							: null,
						returns5Y: fund.returns5y
							? Number.parseFloat(fund.returns5y as string)
							: null,
						dataSource: "live",
					});
				}
			...
			console.log(
				`[MFReturnsScheduler] Cache warmed up with ${fundsWithReturns.length} funds`,
			);
		} catch (error: any) {
	...
	scheduleDailySync(): void {
		...
		console.log(
			`[MFReturnsScheduler] Hourly sync scheduled (every 60 min, batch size: ${BATCH_SIZE})`,
		);
	...
	async runHourlySync(batchSize: number): Promise<void> {
		console.log(
			`[MFReturnsScheduler] Running hourly sync (batch: ${batchSize})...`,
		);
		console.log(
			`[MFReturnsScheduler] Step 1: Calculate returns from stored historical NAV data`,
		);
		...
		try {
			...
			const fundsNeedingReturns = await db
				.select({
				...
				.where(
					and(
						...
						or(
							isNull(mutualFunds.lastUpdated),
							lt(mutualFunds.lastUpdated, twentyHoursAgo),
						),
					),
				)
				.limit(batchSize);
			...
			for (const fund of fundsNeedingReturns) {
				...
				if (isPoolClosed()) {
					console.log("[MFReturnsScheduler] Pool closing — aborting sync loop");
					break;
				}

				// Try to calculate from stored historical data first
				const storedReturns =
					await historicalNavService.calculateReturnsFromStoredData(
						fund.schemeCode,
					);

				if (
					storedReturns.dataQuality !== "insufficient" &&
					storedReturns.returns1y !== null
				) {
					...
					try {
						await historicalNavService.fetchAndStoreMutualFundHistory(
							fund.schemeCode,
						);
						const freshReturns =
							await historicalNavService.calculateReturnsFromStoredData(
						...
						try {
							...
							if (!msg.includes("end on the pool")) {
								console.warn(
									"[MFReturnsScheduler] Failed to update timestamp:",
									msg,
								);
							}
						}
			...
			console.log(
				`[MFReturnsScheduler] Hourly sync complete: ${dbCalculated} from DB, ${apiSynced} from AMFI API`,
			);
	...
	async runDailySync(): Promise<void> {
		...
		try {
			...
			console.log(
				`[MFReturnsScheduler] Daily sync complete: ${result.successful}/${result.processed} successful`,
			);
	...
	async syncBenchmarkData(): Promise<void> {
		...
		try {
			...
			console.log(
				`[MFReturnsScheduler] Benchmark sync: ${result.synced} indices synced`,
			);
		} catch (error: any) {
			console.error(
				"[MFReturnsScheduler] Benchmark sync error:",
				error.message,
			);
		}
	}
	...
	async computeRelativeMetrics(): Promise<void> {
		...
		try {
			...
			console.log(
				`[MFReturnsScheduler] Relative metrics: ${result.success}/${result.processed} computed`,
			);
		} catch (error: any) {
			console.error(
				"[MFReturnsScheduler] Relative metrics error:",
				error.message,
			);
		}
	}
	...
	async syncAmfiBenchmarks(): Promise<void> {
		...
		try {
			const { amfiBenchmarkIngestionService } = await import(
				"./amfi-benchmark-ingestion-service"
			);
			...
			console.log(
				`[MFReturnsScheduler] AMFI sync: ${syncResult.normalized}/${syncResult.total} normalized`,
			);
			...
			console.log(
				`[MFReturnsScheduler] AMFI mapping: ${mapResult.mapped} new, ${mapResult.updated} updated`,
			);
		} catch (error: any) {
	...
	async getSyncedFundsCount(): Promise<{ total: number; withReturns: number }> {
		...
		const withReturnsResult = await db
			.select({ count: sql<number>`count(*)` })
			.from(mutualFunds)
