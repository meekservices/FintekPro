...
export class FinancialCalculations {
  ...
  static async calculateXIRR(
    ...
    while (iterations < maxIterations) {
      ...
      if (Math.abs(npv) < tolerance) {
        return {
          xirr: rate,
          xirrPercentage: parseFloat((rate * 100).toFixed(2)),
          success: true,
          iterations
        };
      }
  ...
  static calculateIRR(
    ...
    while (iterations < maxIterations) {
      ...
      if (Math.abs(npv) < tolerance) {
        return {
          irr: rate,
          irrPercentage: parseFloat((rate * 100).toFixed(2)),
          success: true
        };
      }
  ...
  static calculatePortfolioXIRR(allCashFlows: CashFlow[]): XIRRResult {
    return this.calculateXIRR(allCashFlows);
  }
  ...
  static calculateSimpleReturns(
    ...
    const years = this.getYearFraction(startDate, endDate);
