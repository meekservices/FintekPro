...
class UnifiedOCRService {
	...
	private constructor() {
		...
		if (this.geminiAvailable) {
			...
			console.warn(
				"[UnifiedOCR] Initialized — Gemini API key not set; OCR fallback unavailable",
			);
		}
	}
	...
	async extractTextFromScannedPDF(
		...
		const computeConfidence = (text: string): OCRResult["confidence"] => {
			...
			const hasFinancialNumbers =
				(text.match(/[\d,]+\.\d{2,4}/g) || []).length >= 3;
			const hasDatePattern = /\d{2}[-\/][A-Za-z]{3}[-\/]\d{4}/.test(text);
			const signalBoost =
				(hasISIN ? 1 : 0) +
				(hasFinancialNumbers ? 1 : 0) +
				(hasDatePattern ? 1 : 0);
		...
		const isRetryableError = (err: any): boolean => {
			...
			return /5\d{2}|rate.?limit|overloaded|unavailable|timeout|econnreset|socket hang/i.test(
				msg,
			);
		};
		...
		try {
			...
			try {
				...
				if (isRetryableError(firstErr)) {
					console.warn(
						"[UnifiedOCR] Gemini transient error — retrying once after 1 s:",
						firstErr.message,
					);
			...
			if (!text || text.trim().length === 0) {
				return {
					...
					processingTimeMs: Date.now() - start,
					error:
						"Gemini returned empty text — document may be blank or unreadable",
					usedFallback: true,
				};
			}
	...
	async extractTextFromImage(
		...
		try {
			...
			const confidence: OCRResult["confidence"] =
				wordCount > 200 || signalBoost >= 2
					? "high"
					: wordCount > 50 || signalBoost >= 1
						? "medium"
						: "low";
