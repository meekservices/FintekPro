...
class UnifiedPaymentGateway {
	...
	async createOrder(
		...
		const failureReason =
			cashfreeUnexpectedException?.message ||
			cashfreeResult?.message ||
			"Cashfree service unavailable";
		console.warn(
			"[UnifiedPaymentGateway] Cashfree unreachable or returned 5xx, falling back to PhonePe. Reason:",
			failureReason,
		);
	...
	private async logFallbackEvent(
		...
		try {
			...
			console.error(
				"[UnifiedPaymentGateway] Failed to log fallback event:",
				err,
			);
		}
	}
	...
	async acquireIdempotencyLock(
		...
		const pendingPayload: UnifiedOrderResponse = {
			success: false,
			gateway: "cashfree",
			message: "pending",
		};
		...
		const inserted = await db
			...
			.onConflictDoUpdate({
				target: [
					...
					paymentIdempotencyKeys.idempotencyKey,
				],
				set: {
					orderId: "pending",
					gateway: "cashfree",
	...
	async getGatewayHealth(): Promise<PaymentGatewayHealth> {
		...
		const cashfreeBaseUrl =
			cashfreeEnv.toUpperCase() === "PRODUCTION"
		...
		const phonePeBaseUrl =
			phonePeEnv.toUpperCase() === "PRODUCTION"
