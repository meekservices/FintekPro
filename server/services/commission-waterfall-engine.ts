...
export class CommissionWaterfallEngine {
	...
	static getInstance(): CommissionWaterfallEngine {
		if (!CommissionWaterfallEngine.instance) {
			CommissionWaterfallEngine.instance = new CommissionWaterfallEngine();
		}
		return CommissionWaterfallEngine.instance;
	}
	...
	async processCommission(data: {
		...
		if (!rule) {
			return {
				success: false,
				error: `No commission rule found for product type: ${data.productType}`,
			};
		}
		...
		const agentPct = Number.parseFloat(rule.agentPct?.toString() || "0");
		const subPartnerPct = Number.parseFloat(
			rule.subPartnerPct?.toString() || "0",
		);
		const masterPartnerPct = Number.parseFloat(
			rule.masterPartnerPct?.toString() || "0",
		);
		const platformPct = Number.parseFloat(rule.platformPct?.toString() || "0");
		...
		const intermediatePctEach =
			intermediateCount > 0 ? subPartnerPct / intermediateCount : 0;
		...
		for (let i = 0; i < chainLength; i++) {
			...
			const kycGated =
				partner.kyc_status !== "VERIFIED" && partner.kycStatus !== "VERIFIED";

			const [entry] = await db
				.insert(partnerCommissionLedger)
				.values({
		...
		const platformAmount =
			(totalCommission * (platformPct + Math.max(unallocated, 0))) / 100;
		...
		if (platformAmount > 0) {
			const [platformEntry] = await db
				...
				.values({
					...
					kycGated: false,
					metadata: {
						pctApplied: platformPct + Math.max(unallocated, 0),
						rollupFromMissingLevels: unallocated > 0,
					},
				})
				.returning();
	...
	async debitWallet(
		...
		const currentBalance = Number.parseFloat(wallet.balance?.toString() || "0");
	...
	async createCommissionRule(data: {
		...
		const total =
			data.agentPct +
			data.subPartnerPct +
			data.masterPartnerPct +
			data.platformPct;
		if (Math.abs(total - 100) > 0.01) {
			return {
				success: false,
				error: `Commission percentages must sum to 100%. Current sum: ${total}%`,
			};
		}

		const [rule] = await db
			.insert(partnerCommissionRules)
			.values({
				productType: data.productType,
	...
	async updateCommissionRule(
		...
		const agentPct =
			data.agentPct ?? Number.parseFloat(existing.agentPct?.toString() || "0");
		const subPartnerPct =
			data.subPartnerPct ??
			Number.parseFloat(existing.subPartnerPct?.toString() || "0");
		const masterPartnerPct =
			data.masterPartnerPct ??
			Number.parseFloat(existing.masterPartnerPct?.toString() || "0");
		const platformPct =
			data.platformPct ??
			Number.parseFloat(existing.platformPct?.toString() || "0");
	...
	validateAntiMLM(transactionType: string): { valid: boolean; error?: string } {
		...
		if (forbiddenTypes.includes(transactionType.toLowerCase())) {
			return {
				valid: false,
				error:
					"Commission on partner onboarding/recruitment is not allowed (Anti-MLM)",
			};
		}
		return { valid: true };
	}
	...
	private async processCaMarketplaceCommission(data: {
		...
		try {
			...
			const platformPct = Number.parseFloat(
				config!.caPlatformFeePct?.toString() || "10.00",
			);
			const referralPct = Number.parseFloat(
				config!.caReferralBonusPct?.toString() || "5.00",
			);

			// 2. Resolve the CA and their referring partner
			const [caRegistry] = await db
				.select()
			...
			if (referrerCode) {
				const [referrer] = await db
					.select({ id: fintekproCaRegistry.partnersTableId })
					.from(fintekproCaRegistry)
			...
			const caPayoutPct =
				100 - platformPct - (referrerPartnerId ? referralPct : 0);
			const caPayoutAmount = (totalAmount * caPayoutPct) / 100;

			const [caEntry] = await db
				.insert(partnerCommissionLedger)
				.values({
			...
			logger.error("[Commission] CA Marketplace processing failed", {
				error: err.message,
			});
...
export const commissionWaterfallEngine =
	CommissionWaterfallEngine.getInstance();
