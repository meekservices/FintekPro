...
export class UnlistedEligibilityService {
	async checkUserEligibility(userId: string): Promise<EligibilityResult> {
		...
		const accreditedStatus =
			profile.accreditedInvestorStatus || "not_applicable";
		...
		const riskProfileComplete = Boolean(
			profile.riskCategory || profile.amlRiskScore,
		);
		...
		if (isAccreditedExpired && accreditedStatus === "verified") {
			warnings.push(
				"Accredited investor status has expired. Renewal required for trades above ₹50 lakhs.",
			);
		}

		if (!riskProfileComplete) {
			warnings.push(
				"Risk profile assessment is incomplete. Complete your risk assessment for personalized recommendations.",
			);
		}

		const maxTradeValue =
			accreditedStatusMet && !isAccreditedExpired
	...
	async checkTradeEligibility(
		input: TradeEligibilityInput,
	): Promise<EligibilityResult & { tradeAllowed: boolean }> {
		...
		if (
			input.tradeValue > ACCREDITED_INVESTOR_THRESHOLD &&
			!eligibility.requirements.accreditedStatusMet
		) {
			return {
				...eligibility,
				tradeAllowed: false,
		...
		if (!eligibility.requirements.riskProfileComplete) {
			eligibility.warnings.push(
				`Risk profile incomplete: We recommend completing your risk assessment before trading unlisted securities.`,
			);
		}
			...
			requirements: [
				{
					...
					description:
						"Complete full KYC verification including PAN, Aadhaar, address proof, and video verification",
					mandatory: true,
				},
				{
					name: "Risk Profile Assessment",
					description:
						"Complete the risk profiling questionnaire to understand your investment suitability",
					mandatory: false,
				},
				{
				...
				},
				{
					...
					description:
						"Read and accept the SEBI-mandated risk disclosures for unlisted securities",
					mandatory: true,
				},
			],
			thresholds: {
