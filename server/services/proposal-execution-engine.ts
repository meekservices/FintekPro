...
export class ProposalExecutionEngine {
	...
	createProposal(input: z.infer<typeof createProposalInputSchema>): Proposal {
		...
		const weightedReturn = input.items.reduce(
			(sum, item) => sum + (item.expectedReturn * item.allocationPercent) / 100,
			0,
		);
		...
		const proposal: Proposal = {
			...
			goalId: input.goalId,
			validUntil: new Date(
				now.getTime() + input.validDays * 24 * 60 * 60 * 1000,
			),
			createdAt: now,
			updatedAt: now,
		};
	...
	getClientProposals(clientId: string, status?: ProposalStatus): Proposal[] {
		return Array.from(this.proposals.values())
			.filter(
				(p) => p.clientId === clientId && (!status || p.status === status),
			)
	...
	checkout(input: z.infer<typeof checkoutInputSchema>): ExecutionOrder {
		...
		const approvalConfig = input.approvalConfig || {
			requireChecker: true,
		...
		const requiresMakerChecker =
			input.makerCheckerEnabled &&
			cart.totalAmount >= approvalConfig.thresholdAmount;
	 ...
	 */
	processApproval(
		input: z.infer<typeof approvalActionInputSchema>,
	): ExecutionOrder {
		...
		if (order.approvalStatus !== "pending") {
			throw new Error(
				`Order is not pending approval. Current status: ${order.approvalStatus}`,
			);
		}
		...
		const pendingSteps = order.approvalSteps.filter(
			(s) => s.status === "pending",
		);
	...
	getClientOrders(
		...
		return Array.from(this.orders.values())
			.filter(
				(o) =>
					o.clientId === clientId && (!status || o.executionStatus === status),
			)
			.sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime());
	}
	...
	getWorkflowSummary(clientId?: string): {
		...
		const carts = Array.from(this.carts.values()).filter(
			(c) => !clientId || c.userId === clientId,
		);
		...
		for (const o of orders) {
			ordersByStatus[o.executionStatus] =
				(ordersByStatus[o.executionStatus] || 0) + 1;
		}
	...
	getMakerCheckerStatus(orderId: string):
		...
		const completedSteps = order.approvalSteps.filter(
			(s) => s.status === "approved",
		);
		const pendingSteps = order.approvalSteps.filter(
			(s) => s.status === "pending",
