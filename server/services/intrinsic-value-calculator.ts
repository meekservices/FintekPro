...
const DEFAULT_RISK_FREE_RATE = 7.1;
...
class IntrinsicValueCalculatorService {
	 ...
	 */
	async calculateListedStockValue(
		symbol: string,
		...
		if (enriched) {
			this.addAuditEntry(
				"EnrichedData",
				"FMP enriched snapshot loaded",
				{
					symbol,
					hasFundamentals: enriched.fundamentals ? "yes" : "no",
					hasGrowth: enriched.growth ? "yes" : "no",
					hasDCF: enriched.dcf ? "yes" : "no",
				},
				null,
				"FMP enriched",
			);
		...
		const relativeResult = await this.calculateRelativeValuation(
			stockData,
			enriched,
		);
	 ...
	 */
	async calculateUnlistedStockValue(
		companyId: string,
	): Promise<IntrinsicValueResult> {
		this.auditLog = [];
	...
	private async calculateDCF(
		...
		if (enriched?.dcf?.dcfValue && enriched.dcf.dcfValue > 0) {
			...
			this.addAuditEntry(
				"DCF",
				"FMP DCF valuation used directly",
				{
					dcfValue,
					stockPrice,
					upsidePercent:
						enriched.dcf.upsidePercent != null
							? enriched.dcf.upsidePercent
							: "N/A",
				},
				dcfValue,
				"FMP DCF",
		...
		if (
			...
			if (
				enriched?.fundamentals?.freeCashFlowPerShare &&
				enriched.fundamentals.freeCashFlowPerShare > 0 &&
				sharesOutstanding &&
				sharesOutstanding > 0
			) {
				...
				for (let i = 0; i < projectedFCFs.length; i++) {
					pvOfFCF += projectedFCFs[i] / (1 + wacc / 100) ** (i + 1);
				}

				const terminalFCF = projectedFCFs[projectedFCFs.length - 1];
				const terminalValue =
					(terminalFCF * (1 + terminalGrowthRate / 100)) /
					((wacc - terminalGrowthRate) / 100);
				const pvOfTerminalValue =
					terminalValue / (1 + wacc / 100) ** projectionYears;
				...
				this.addAuditEntry(
					"DCF",
					"DCF using FMP enriched FCF per share",
			...
			this.addAuditEntry(
				...
				null,
				"N/A",
				"Requires minimum 2 years of FCF data",
			);
			return null;
		}
		...
		if (currentFCF <= 0) {
			this.addAuditEntry(
				...
				null,
				"financial_data",
				"DCF not applicable for negative cash flows",
			);
			return null;
		}
		...
		if (
			enriched?.growth?.freeCashFlowGrowth != null &&
			enriched.growth.freeCashFlowGrowth > -50
		) {
		...
		const confidence = this.assessDCFConfidence(
			freeCashFlows.length,
			fcfGrowthRate,
			wacc,
		);
	...
	private calculateGrahamValue(
		...
		if (
			enriched?.fundamentals?.earningsYield &&
			enriched.fundamentals.earningsYield > 0 &&
			currentPrice &&
			currentPrice > 0
		) {
			const enrichedEps =
				(enriched.fundamentals.earningsYield * currentPrice) / 100;
		...
		if (!eps || eps <= 0) {
			if (
				enriched?.fundamentals?.grahamNumber &&
				enriched.fundamentals.grahamNumber > 0
			) {
				...
				this.addAuditEntry(
					"Graham",
					"FMP Graham Number used directly",
					{ grahamNumber, currentPrice: currentPrice || "N/A" },
					grahamNumber,
					"FMP Graham Number",
				);
				return {
			...
			this.addAuditEntry(
				...
				null,
				"N/A",
				"Graham formula requires positive EPS",
			);
			return null;
		}
		...
		const intrinsicValue =
			eps *
			(GRAHAM_NO_GROWTH_PE + GRAHAM_GROWTH_MULTIPLIER * cappedGrowthRate) *
			(4.4 / aaaBondYield);
		...
		const confidence = usedEnrichedGrowth
			? ("high" as const)
			: this.assessGrahamConfidence(eps, cappedGrowthRate, epsGrowthRate);
		const dataSource = usedEnrichedGrowth
			? "FMP enriched + financial_statements"
			: "financial_statements";
	 ...
	 */
	private async calculateRelativeValuation(
		stockData: StockFinancialData,
		enriched?: EnrichedStockSnapshot | null,
	): Promise<RelativeValuationResult | null> {
		...
		if (enriched?.fundamentals) {
			if (
				...
				enriched.fundamentals.peRatio > 0 &&
				stockData.currentPrice &&
				stockData.currentPrice > 0
			) {
				const enrichedEps =
					stockData.currentPrice / enriched.fundamentals.peRatio;
				if (enrichedEps > 0) eps = enrichedEps;
			}
			if (
				enriched.fundamentals.bookValuePerShare &&
				enriched.fundamentals.bookValuePerShare > 0
			) {
				bookValuePerShare = enriched.fundamentals.bookValuePerShare;
			}
			if (
				enriched.fundamentals.evToEbitda &&
				enriched.fundamentals.evToEbitda > 0 &&
				enriched.fundamentals.enterpriseValue
			) {
				const enrichedEbitda =
					enriched.fundamentals.enterpriseValue /
					enriched.fundamentals.evToEbitda;
		...
		if (!sector) {
			this.addAuditEntry(
				...
				null,
				"N/A",
				"Sector classification required for relative valuation",
			);
			return null;
		}
		...
		if (
			!sectorAverages.avgPE &&
			!sectorAverages.avgPB &&
			!sectorAverages.avgEvEbitda
		) {
			this.addAuditEntry(
				"Relative",
				"No sector comparables",
				{ sector },
				null,
				"database",
				"Insufficient peer data for sector averages",
			);
			return null;
		}
		...
		if (eps && eps > 0 && sectorAverages.avgPE) {
			...
			this.addAuditEntry(
				"Relative-PE",
				`Fair Value = EPS × Sector Avg P/E = ${eps.toFixed(2)} × ${sectorAverages.avgPE.toFixed(2)}`,
				{ eps, sectorAvgPE: sectorAverages.avgPE },
				peBasedValue,
				"sector_database",
			);
		}
		...
		if (bookValuePerShare && bookValuePerShare > 0 && sectorAverages.avgPB) {
			...
			this.addAuditEntry(
				"Relative-PB",
				`Fair Value = BVPS × Sector Avg P/B = ${bookValuePerShare.toFixed(2)} × ${sectorAverages.avgPB.toFixed(2)}`,
				{ bookValuePerShare, sectorAvgPB: sectorAverages.avgPB },
				pbBasedValue,
				"sector_database",
			);
		}
		...
		if (
			...
			if (evEbitdaBasedValue > 0) {
				...
				this.addAuditEntry(
					...
					{
						ebitda,
						sectorAvgEvEbitda: sectorAverages.avgEvEbitda,
						netDebt,
						sharesOutstanding,
					},
					evEbitdaBasedValue,
		...
		const compositeValue =
			valueMethods.reduce((a: any, b: any) => a + b, 0) / valueMethods.length;
		const usedEnrichedRelative = !!enriched?.fundamentals;
		const confidence = usedEnrichedRelative
			? ("high" as const)
			: this.assessRelativeConfidence(
					valueMethods.length,
					sectorAverages.peerCount,
				);
		const relativeDataSource = usedEnrichedRelative
			? "FMP enriched + sector_database"
			: "sector_database";
		...
		return {
			...
			methods: {
				peBasedValue: peBasedValue
					? Math.round(peBasedValue * 100) / 100
					: null,
				pbBasedValue: pbBasedValue
					? Math.round(pbBasedValue * 100) / 100
					: null,
				evEbitdaBasedValue: evEbitdaBasedValue
					? Math.round(evEbitdaBasedValue * 100) / 100
					: null,
			},
			sectorAverages: {
				avgPE: sectorAverages.avgPE
					? Math.round(sectorAverages.avgPE * 100) / 100
					: null,
				avgPB: sectorAverages.avgPB
					? Math.round(sectorAverages.avgPB * 100) / 100
					: null,
				avgEvEbitda: sectorAverages.avgEvEbitda
					? Math.round(sectorAverages.avgEvEbitda * 100) / 100
					: null,
				sectorName: sector,
	 ...
	 */
	private calculateBookValue(
		stockData: StockFinancialData,
		enriched?: EnrichedStockSnapshot | null,
	): BookValueResult | null {
		const {
			totalAssets,
			totalLiabilities,
			intangibleAssets,
		...
		if (
			...
			this.addAuditEntry(
				"BookValue",
				"FMP enriched bookValuePerShare used",
		...
		if (!totalAssets || !sharesOutstanding || sharesOutstanding <= 0) {
			this.addAuditEntry(
				"BookValue",
				"Insufficient balance sheet data",
				{},
				null,
				"N/A",
				"Requires total assets and shares outstanding",
			);
			return null;
		}

		const netAssetValue =
			(totalAssets - (totalLiabilities || 0)) / sharesOutstanding;
		const tangibleBookValue =
			(totalAssets - (intangibleAssets || 0) - (totalLiabilities || 0)) /
			sharesOutstanding;
		...
		if (intrinsicValue <= 0) {
			this.addAuditEntry(
				"BookValue",
				"Negative book value",
				{ netAssetValue, tangibleBookValue },
				null,
				"balance_sheet",
		...
		const confidence = this.assessBookValueConfidence(
			totalAssets,
	...
	private async fetchListedStockData(
		symbol: string,
		...
		try {
			...
			console.error(
				"[IntrinsicValue] Error fetching listed stock data:",
				error,
			);
			return null;
		}
	...
	private async fetchUnlistedStockData(
		companyId: string,
	): Promise<StockFinancialData | null> {
		try {
			...
			const sharesOutstanding = (company as any).sharesOutstanding
				? Number((company as any).sharesOutstanding)
				: null;
			const currentPrice = (company as any).lastPrice
				? Number((company as any).lastPrice)
				: null;
			...
			const eps =
				sharesOutstanding && sharesOutstanding > 0
					? pat / sharesOutstanding
					: null;
			const bookValuePerShare =
				sharesOutstanding && sharesOutstanding > 0 && networth > 0
					? networth / sharesOutstanding
					: null;

			let epsGrowthRate = null;
			if (financials.length >= 2) {
				const prevPat = Number(
					financials[1].pat || financials[1].netProfit || 0,
				);
				if (prevPat > 0 && pat > 0) {
			...
			console.error(
				"[IntrinsicValue] Error fetching unlisted stock data:",
				error,
			);
			return null;
		}
	 ...
	 */
	private async getHistoricalFCFs(
		stockId: string,
		stockType: "listed" | "unlisted",
	): Promise<number[]> {
		try {
			if (stockType === "listed") {
				this.addAuditEntry(
					"FCF-Data",
					"Listed stock FCF data not available",
					{ reason: "company_financials table is for unlisted companies only" },
					0,
					"listed_stocks",
					"No FCF source for listed stocks",
				);
				return [];
			}
			const financials = await db
			...
			if (fcfs.length > 0) {
				this.addAuditEntry(
					"FCF-Data",
					"Historical FCF from company_financials",
					{ years: fcfs.length, latestFCF: fcfs[0] },
					fcfs[0],
					"company_financials",
				);
			}
	...
	private compileResults(
		...
		if (valuations.dcf) {
			...
			weights.push(
				valuations.dcf.confidence === "high"
					? 0.4
					: valuations.dcf.confidence === "medium"
						? 0.3
						: 0.2,
			);
		}
		if (valuations.graham) {
			...
			weights.push(
				valuations.graham.confidence === "high"
					? 0.3
					: valuations.graham.confidence === "medium"
						? 0.25
						: 0.15,
			);
		}
		if (valuations.relative) {
			...
			weights.push(
				valuations.relative.confidence === "high"
					? 0.2
					: valuations.relative.confidence === "medium"
						? 0.15
						: 0.1,
			);
		}
		...
		if (validValues.length > 0) {
			...
			compositeValue =
				validValues.reduce((sum, val, i) => sum + val * weights[i], 0) /
				totalWeight;
		...
		if (
			...
			upside =
				((compositeValue - stockData.currentPrice) / stockData.currentPrice) *
				100;
		...
		return {
			...
			dataAvailability: {
				...
				hasBookValue:
					!!stockData.bookValuePerShare && stockData.bookValuePerShare > 0,
	...
	private determineCompositeConfidence(valuations: {
		...
		const highCount = availableMethods.filter(
			(v: any) => v?.confidence === "high",
		).length;
		const mediumCount = availableMethods.filter(
	...
	private assessDCFConfidence(
		...
		if (fcfYears >= 3 && growthRate > -10 && growthRate < 40) {
			return "medium";
		}
		return "low";
	}

	private assessGrahamConfidence(
		eps: number,
		cappedGrowthRate: number,
		originalGrowthRate: number | null,
	): "high" | "medium" | "low" {
		if (
			eps > 0 &&
			originalGrowthRate !== null &&
			Math.abs(cappedGrowthRate - (originalGrowthRate || 0)) < 5
		) {
			return "high";
		}
		if (eps > 0) {
	...
	private assessRelativeConfidence(
		methodCount: number,
		peerCount: number,
	): "high" | "medium" | "low" {
		...
		if (methodCount >= 1 && peerCount >= 5) {
			return "medium";
		}
		return "low";
	}

	private assessBookValueConfidence(
		totalAssets: number,
		...
		if (
			hasCompleteLiabilities &&
			hasIntangibles &&
			totalAssets > (totalLiabilities || 0)
		) {
			return "high";
		}
		if (hasCompleteLiabilities) {
			return "medium";
		}
		return "low";
	}
