...
import {
	amfiValidationService,
	AmfiValidationResult,
	EuinValidationResult,
} from "../amfi-validation-service";
...
class MfBatchCredentialValidator {
	...
	async validateBatchCredentials(
		batch: BatchCredentialValidation,
		...
		if (cached && this.isCacheValid(cached.timestamp)) {
			console.log(
				`[MFBatchValidator] Using cached validation for ARN ${batch.arnCode}`,
			);
	...
	private async performFullValidation(
		...
		console.log(
			`[MFBatchValidator] Validating ARN: ${batch.arnCode}, EUIN: ${batch.euinCode || "N/A"}`,
		);

		const arnValidation = await amfiValidationService.validateArn(
			batch.arnCode,
		);
		...
		if (arnValidation.distributorDetails) {
			if (arnValidation.distributorDetails.distributorStatus === "suspended") {
				...
				arnValidation.distributorDetails.distributorStatus === "expired"
			) {
				errors.push("ARN has expired - renewal required");
			} else if (
				arnValidation.distributorDetails.distributorStatus === "inactive"
			) {
				errors.push(
					"ARN holder is currently inactive - cannot process transactions",
				);
			} else if (
				arnValidation.distributorDetails.distributorStatus !== "active"
			) {
				errors.push(
					`ARN status is ${arnValidation.distributorDetails.distributorStatus} - must be active to proceed`,
				);
			}

			if (arnValidation.distributorDetails.arnExpiryDate) {
				const expiryDate = new Date(
					arnValidation.distributorDetails.arnExpiryDate,
				);
				...
				if (daysUntilExpiry <= 0) {
					...
					warnings.push(
						`ARN expires in ${daysUntilExpiry} days - renewal recommended`,
					);
		...
		if (batch.euinCode) {
			...
			if (batch.totalAmount > 50000) {
				warnings.push(
					"EUIN not provided for transaction > ₹50,000 - may require declaration",
				);
		...
		if (batch.totalAmount > 10000000) {
			warnings.push(
				"High-value batch (>₹1 Cr) - additional compliance checks recommended",
			);
			requiresManualReview = true;
		}
		...
		const result: CredentialValidationResult = {
			isValid:
				arnValidation.isValid && (!euinValidation || euinValidation.isValid),
			arnValidation,
			euinValidation,
			warnings,
			errors,
		...
		complianceMonitor.logEvent({
			...
			riskLevel:
				errors.length > 0 ? "high" : warnings.length > 0 ? "medium" : "low",
			userId: batch.agentId,
			metadata: {
				arnCode: batch.arnCode,
				euinCode: batch.euinCode,
		...
		console.log(
			`[MFBatchValidator] Validation complete: canProceed=${canProceed}, errors=${errors.length}, warnings=${warnings.length}`,
		);
	...
	private async logValidation(
		...
		try {
			...
			const registryResponseHash = crypto
				.createHash("sha256")
				.update(JSON.stringify(registryResponseSnapshot))
				.digest("hex");

			const [inserted] = await db
				.insert(mfBatchValidationLogs)
				.values({
					...
					arnStatus:
						result.arnValidation.distributorDetails?.distributorStatus ||
						"unknown",
					arnExpiryDate:
						result.arnValidation.distributorDetails?.arnExpiryDate?.toString() ||
						null,
			...
			console.error(
				"[MFBatchValidator] Failed to persist validation log:",
				error,
			);
		}
	}
		...
		try {
			...
			if (!arnCode) {
				return {
					ready: false,
					message:
						"Agent does not have an ARN configured. Please update credentials.",
				};
			}
