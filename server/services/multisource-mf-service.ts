...
import FintekProRatingService, {
	FintekProAnalysis,
} from "./fintekpro-rating-service";
...
interface CacheConfig {
	schemes: {
		ttl: number;
		data: Map<string, { fund: FundExtended; timestamp: number }>;
	};
	nav: { ttl: number; data: Map<string, { nav: string; timestamp: number }> };
	historical: {
		ttl: number;
		data: Map<string, { records: NAVRecord[]; timestamp: number }>;
	};
	popular: {
		ttl: number;
		data: { funds: FundExtended[]; timestamp: number } | null;
	};
}
...
export class MultiSourceMFService {
	...
	constructor(storage: IStorage) {
		...
		this.sourceHealth = new Map([
			...
			[
				"FINTEKPRO_RATING",
				{ isHealthy: true, errorRate: 0, consecutiveFailures: 0 },
			],
		]);
		...
		this.amfiCircuitBreaker = new CircuitBreaker("AMFI", {
			...
			onFailure: (error, name) => {
				console.warn(
					`[CircuitBreaker] ${name} failure recorded:`,
					error.message,
				);
			},
		});
	...
	async getFund(schemeCode: string): Promise<FundExtended | null> {
		...
		for (const source of sources) {
			try {
				...
				if (fund) {
					...
					try {
						fundWithProvenance =
							await this.enrichWithFintekProRating(fundWithProvenance);
					} catch (error) {
						console.warn(
							`Failed to enrich fund ${fundWithProvenance.schemeCode} with FintekPro rating:`,
							error,
						);
					}
					...
					try {
						...
						console.warn(
							`Failed to save fund ${schemeCode} to database:`,
							error,
						);
					}
		...
		throw new Error(
			`All sources failed for fund ${schemeCode}. Last error: ${lastError?.message}`,
		);
	}
	...
	async searchFunds(query: string): Promise<FundExtended[]> {
			...
			return Promise.race([
				...
				new Promise<T>((_, reject) =>
					setTimeout(
						() =>
							reject(
								new Error(`${source} search timeout after ${timeoutMs}ms`),
							),
						timeoutMs,
					),
				),
		...
		const [amfiResult, mfapiResult] = await Promise.allSettled([
			this.amfiCircuitBreaker.execute(() =>
				withTimeout(
					this.fetchFromSource("AMFI", "search", query),
					SEARCH_TIMEOUT,
					"AMFI",
				),
			),
			this.mfapiCircuitBreaker.execute(() =>
				withTimeout(
					this.fetchFromSource("MFAPI", "search", query),
					SEARCH_TIMEOUT,
					"MFAPI",
				),
			),
		...
		if (dedupedFunds.length > 0) {
			console.log(
				`✅ Search found ${dedupedFunds.length} funds from ${usedSources.join(", ")} in ${Date.now() - startTime}ms`,
			);
		...
		console.log(
			`⚠️ All APIs failed/empty for query "${query}", falling back to database...`,
		);
		try {
			const dbFunds = await this.storage.searchMutualFunds(query);
			if (dbFunds.length > 0) {
				console.log(
					`✅ Database fallback found ${dbFunds.length} cached funds`,
				);
				return dbFunds.map((dbFund) => {
					...
					return {
						...
						provenance: {
							...
							lastRefreshed:
								dbFund.lastUpdated?.toISOString() || new Date().toISOString(),
	...
	private async cacheSearchResultsToDb(funds: FundExtended[]): Promise<void> {
		const now = new Date();
		for (const fund of funds.slice(0, 50)) {
		...
		for (const source of sources) {
			try {
				...
				if (funds && funds.length > 0) {
					console.log(
						`✅ Successfully fetched ${funds.length} popular funds from ${source}`,
					);
					const enrichedFunds = await Promise.all(
						funds.map(async (fund: any) => {
							try {
								const performance = await this.enrichWithPerformance(
									fund,
									source,
								);
								return {
								...
								console.warn(
									`Failed to enrich fund ${fund.schemeCode}:`,
									error,
								);
								return {
									...fund,
					...
					try {
						ratedFunds =
							await this.enrichFundsWithFintekProRating(enrichedFunds);
					} catch (error) {
						console.warn(
							"Failed to enrich funds with FintekPro ratings:",
							error,
						);
				...
				this.updateSourceHealth(source, false);
				console.warn(
					`Best performing funds failed for source ${source}:`,
					error,
				);
			}
		}
	...
	async listFunds(params: FundSearchParams = {}): Promise<FundListResponse> {
		...
		try {
			...
			if (fundHouse) {
				funds = funds.filter((f) =>
					f.fundHouse?.toLowerCase().includes(fundHouse.toLowerCase()),
				);
			}
			...
			funds.sort((a, b) => {
				...
				switch (sortBy) {
					case "nav":
						comparison =
							Number.parseFloat(a.currentNav || "0") -
							Number.parseFloat(b.currentNav || "0");
						break;
					case "returns1Y":
	...
	getSourcesStatus(): MultiSourceStatus {
		const sources: SourceStatus[] = Array.from(this.sourceHealth.entries()).map(
			([source, health]) => ({
				source,
	...
	private async fetchFromSource(
		...
		console.log(
			`🔍 Attempting to fetch ${type} from source: ${source}${param ? ` (param: ${param})` : ""}`,
		);
	...
	private async fetchFromAMFI(type: string, param?: string): Promise<any> {
		...
		try {
			...
			for (const line of lines) {
				...
				if (Number.isNaN(Number.parseFloat(trimmedNav))) continue; // Skip if NAV is not a valid number
			...
			if (type === "popular") {
				...
				console.log(
					`✅ Selected ${selectedFunds.length} diverse funds from AMFI (Equity: ${categorizedFunds.equity.length > 30 ? 30 : categorizedFunds.equity.length}, Debt: ${categorizedFunds.debt.length > 25 ? 25 : categorizedFunds.debt.length}, Hybrid: ${categorizedFunds.hybrid.length > 15 ? 15 : categorizedFunds.hybrid.length})`,
				);
		...
		try {
			...
			switch (type) {
				...
				case "popular":
					...
					console.warn(
						"MFAPI does not support popular funds endpoint - falling back to AMFI",
					);
					return [];
				default:
			...
			if (type === "search") {
				return Array.isArray(response.data)
					? response.data.map((fund: any) => this.normalizeMFAPIFund(fund))
					: [];
			}
	...
	private normalizeMFAPIFund(data: any): FundExtended {
		...
		const schemeCode =
			data.meta?.scheme_code || data.schemeCode || data.id?.toString() || "";
		const schemeName =
			data.meta?.scheme_name || data.schemeName || data.name || "";
	 ...
	 */
	private async enrichWithPerformance(
		fund: any,
		source: string,
	): Promise<FundPerformance> {
		try {
			// Get historical NAV data for CAGR calculations
			const historicalData = await this.getHistoricalNAV(
				fund.schemeCode,
				source,
			);
			const currentNav = Number.parseFloat(fund.currentNav || fund.nav || "0");
			...
			console.warn(
				`Failed to enrich performance for ${fund.schemeCode}:`,
				error,
			);
			return {
				currentNav: fund.currentNav || fund.nav || "0",
	 ...
	 */
	private async getHistoricalNAV(
		schemeCode: string,
		source: string,
	): Promise<NAVRecord[]> {
		...
		try {
			...
			console.warn(
				`Failed to fetch historical NAV for ${schemeCode} from ${source}:`,
				error,
			);
			return [];
		}
	...
	private calculateCAGRReturns(
		...
		for (const [period, days] of Object.entries(periods)) {
			...
			if (historicalNAV) {
				const pastNav = Number.parseFloat(historicalNAV.nav);
				if (pastNav > 0) {
					if (period === "1M" || period === "6M") {
						...
						returns[period] = ((currentNav / pastNav) ** (1 / years) - 1) * 100;
					}
				}
			}
		}
	 ...
	 */
	private async enrichWithFintekProRating(
		fund: FundExtended,
	): Promise<FundExtended> {
		try {
			const fintekProAnalysis = await FintekProRatingService.getRating(
				fund.schemeCode,
			);
			...
			if (fintekProAnalysis) {
				...
				fund.crisilRiskAdjustedScore =
					fintekProAnalysis.rating.riskAdjustedScore;
				fund.crisilAssetQualityScore =
					fintekProAnalysis.rating.assetQualityScore;
				fund.crisilLiquidityScore = fintekProAnalysis.rating.liquidityScore;
				fund.crisilConcentrationScore =
					fintekProAnalysis.rating.concentrationScore;
				...
				console.log(
					`✅ Enhanced ${fund.schemeName} with FintekPro ${fintekProAnalysis.rating.rating}-star rating`,
				);
			}
		} catch (error) {
			console.warn(
				`⚠️ Failed to enrich ${fund.schemeCode} with FintekPro rating:`,
				error,
			);
	 ...
	 */
	private async enrichFundsWithFintekProRating(
		funds: FundExtended[],
	): Promise<FundExtended[]> {
		...
		for (let i = 0; i < funds.length; i += batchSize) {
			...
			const batchPromises = batch.map((fund) =>
				this.enrichWithFintekProRating(fund),
			);
	...
	private convertDbToExtended(dbFund: any): FundExtended {
		...
		return {
			...
			nav: dbFund.nav ? Number.parseFloat(dbFund.nav) : undefined,
			expenseRatio: dbFund.expenseRatio || undefined,
			aum: dbFund.aum || undefined,
			riskLevel: dbFund.riskLevel || undefined,
			returns1y: dbFund.returns1y
				? Number.parseFloat(dbFund.returns1y)
				: undefined,
			returns3y: dbFund.returns3y
				? Number.parseFloat(dbFund.returns3y)
				: undefined,
			returns5y: dbFund.returns5y
				? Number.parseFloat(dbFund.returns5y)
				: undefined,

			// Restore extended data from jsonb column
			currentNav:
				extData.currentNav ||
				(dbFund.nav ? Number.parseFloat(dbFund.nav).toString() : undefined),
			...
			crisilPercentile: dbFund.crisilPercentile
				? Number.parseFloat(dbFund.crisilPercentile)
				: undefined,
			crisilEvaluationDate: dbFund.crisilEvaluationDate,
			crisilRiskAdjustedScore: dbFund.crisilRiskAdjustedScore
				? Number.parseFloat(dbFund.crisilRiskAdjustedScore)
				: undefined,
			crisilAssetQualityScore: dbFund.crisilAssetQualityScore
				? Number.parseFloat(dbFund.crisilAssetQualityScore)
				: undefined,
			crisilLiquidityScore: dbFund.crisilLiquidityScore
				? Number.parseFloat(dbFund.crisilLiquidityScore)
				: undefined,
			crisilConcentrationScore: dbFund.crisilConcentrationScore
				? Number.parseFloat(dbFund.crisilConcentrationScore)
				: undefined,
			crisilOverallScore: dbFund.crisilOverallScore
				? Number.parseFloat(dbFund.crisilOverallScore)
				: undefined,
	...
	private async saveFundToDatabase(fund: FundExtended): Promise<void> {
		try {
			...
			if (
				!fund.schemeCode ||
				!fund.schemeName ||
				fund.schemeCode.trim() === "" ||
				fund.schemeName.trim() === ""
			) {
				console.warn(
					`⚠️  Rejecting fund with missing data: schemeCode="${fund.schemeCode}", schemeName="${fund.schemeName}"`,
				);
				throw new Error(
					"Invalid fund data: schemeCode and schemeName are required",
				);
			}
			...
			if (!/^\d+$/.test(fund.schemeCode)) {
				console.warn(
					`⚠️  Rejecting fund with invalid scheme code: ${fund.schemeCode}`,
				);
			...
			const basicData = {
				...
				crisilRiskAdjustedScore:
					fund.crisilRiskAdjustedScore?.toString() || null,
				crisilAssetQualityScore:
					fund.crisilAssetQualityScore?.toString() || null,
				crisilLiquidityScore: fund.crisilLiquidityScore?.toString() || null,
				crisilConcentrationScore:
					fund.crisilConcentrationScore?.toString() || null,
				...
				isin:
					(fund as any).isin ||
					(fund as any).isinGrowth ||
					(fund as any).isinDiv ||
					null, // ISIN code for search
				...
				minSipAmount:
					(fund as any).minSipAmount?.toString() ||
					fund.minInvestment?.sip?.toString() ||
					null,
				minLumpsumAmount:
					(fund as any).minLumpsumAmount?.toString() ||
					fund.minInvestment?.lumpsum?.toString() ||
					null,
	...
	getCacheStats(): {
		...
		const staleSchemes = Array.from(this.cache.schemes.data.values()).filter(
			(entry) => now - entry.timestamp > this.cache.schemes.ttl,
		).length;
