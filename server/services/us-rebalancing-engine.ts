...
const US_EQUITY_TARGET_WITHIN_EQUITY = 0.20;

class UsRebalancingEngine {
  async analyzePortfolio(userId: string): Promise<RebalancingAnalysis | null> {
    const [riskProfile] = await db.select().from(riskProfiles).where(eq(riskProfiles.userId, userId));
    const usHoldingsData = await db.select().from(usHoldings).where(eq(usHoldings.clientId, userId));
    
    if (!riskProfile) {
      return null;
    }

    const riskTolerance = (riskProfile.riskTolerance as keyof AllocationTarget) || 'moderate';
    const targets = TARGET_ALLOCATIONS[riskTolerance] || TARGET_ALLOCATIONS.moderate;

    let usEquityValue = 0;
    const holdingsBySymbol = new Map<string, { quantity: number; value: number; avgPrice: number }>();
    
    for (const holding of usHoldingsData) {
      const value = parseFloat(holding.marketValueInr || '0');
      usEquityValue += value;
      holdingsBySymbol.set(holding.symbol, {
        quantity: parseFloat(holding.quantity || '0'),
        value,
        avgPrice: parseFloat(holding.avgPriceUsd || '0'),
      });
    }

    const estimatedTotalPortfolio = usEquityValue / (US_EQUITY_TARGET_WITHIN_EQUITY * (targets.equity / 100)) || usEquityValue * 5;
    
    const currentAllocation = {
      usEquity: estimatedTotalPortfolio > 0 ? (usEquityValue / estimatedTotalPortfolio) * 100 : 0,
      domesticEquity: estimatedTotalPortfolio > 0 ? ((targets.equity - (usEquityValue / estimatedTotalPortfolio) * 100)) : targets.equity * 0.8,
      debt: targets.debt,
      gold: targets.gold,
      cash: targets.cash,
      other: 0,
    ...
    const targetDomesticEquity = targets.equity * (1 - US_EQUITY_TARGET_WITHIN_EQUITY);
    ...
    const rationale = this.generateRationale(deviations, riskTolerance, suggestedTrades.length);
    const expectedImpact = this.generateExpectedImpact(deviations, suggestedTrades);
  ...
  private async generateTradesuggestions(
    ...
    holdings: Map<string, { quantity: number; value: number; avgPrice: number }>,
    ...
    try {
      ...
      if (deviations.usEquity < -3) {
        const targetIncrease = Math.abs(deviations.usEquity) / 100 * totalPortfolio;
        const stocksToConsider = riskTolerance === 'conservative' ? coreStocks : [...coreStocks, ...growthStocks];
        ...
        for (const stock of stocksToConsider.slice(0, 3)) {
          ...
          if (quote && quote.price > 0) {
            ...
            if (quantity > 0 && (!currentHolding || currentHolding.quantity < 10)) {
              trades.push({
                symbol: stock.symbol,
                name: stock.name,
                side: 'buy',
      ...
      if (deviations.usEquity > 5) {
        ...
        for (const [symbol, holding] of sortedHoldings) {
          ...
          const quote = (await polygonMarketService.getMultipleQuotes([symbol])).get(symbol);
          ...
          if (quote && quote.price > 0) {
            ...
            if (quantity > 0) {
              trades.push({
                ...
                quantity,
                estimatedValue: quantity * priceInr,
                reason: 'Reduce overweight position to achieve target allocation.',
      ...
      if (uniqueHoldings.size < 4 && holdings.size > 0) {
        const missingStocks = coreStocks.filter(s => !uniqueHoldings.has(s.symbol));
        
        for (const stock of missingStocks.slice(0, 2)) {
          const quote = (await polygonMarketService.getMultipleQuotes([stock.symbol])).get(stock.symbol);
          if (quote && quote.price > 0) {
            const priceInr = quote.price * fxRate;
  ...
  private generateRationale(
    ...
    if (Math.abs(deviations.usEquity) > 5) {
      issues.push(deviations.usEquity > 0 
  ...
  private generateExpectedImpact(
    ...
    if (sellTrades.length > 0) {
      const totalSell = sellTrades.reduce((sum, t) => sum + t.estimatedValue, 0);
      impacts.push(`Free up ₹${(totalSell / 100000).toFixed(2)}L for reallocation`);
    }
  ...
  async saveSuggestion(userId: string, analysis: RebalancingAnalysis): Promise<string | null> {
    try {
      const [existing] = await db.select().from(rebalancingSuggestions)
      ...
      const suggestionData = {
        ...
        priority: analysis.riskScore > 50 ? 'high' : analysis.riskScore > 25 ? 'medium' : 'low',
        title: 'Portfolio Rebalancing Recommendation',
        description: analysis.rationale,
        actions: analysis.suggestedTrades,
        expectedImpact: { summary: analysis.expectedImpact, riskScore: analysis.riskScore },
        confidenceScore: String(Math.min(95, 60 + analysis.suggestedTrades.length * 5)),
        implementationSteps: analysis.suggestedTrades.map(t => 
          `${t.side.toUpperCase()} ${t.quantity} shares of ${t.symbol}`
        ),
      };

      if (existing) {
        await db.update(rebalancingSuggestions)
          .set(suggestionData)
          .where(eq(rebalancingSuggestions.id, existing.id));
        return existing.id;
      } else {
        const [inserted] = await db.insert(rebalancingSuggestions)
