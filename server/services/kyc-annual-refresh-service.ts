...
class KYCAnnualRefreshService {
	...
	registerForReminders(
		...
		console.log(
			`📅 [Annual Refresh] Registered ${refreshType} reminder for user ${userId.substring(0, 8)}..., expires: ${expiryDate.toISOString()}`,
		);
	...
	markRefreshed(
		...
		if (reminder) {
			...
			this.reminders.set(key, reminder);
			console.log(
				`✅ [Annual Refresh] ${refreshType} refreshed for user ${userId.substring(0, 8)}..., new expiry: ${newExpiryDate.toISOString()}`,
			);
		}
	}
	...
	async processDueReminders(): Promise<{
		...
		for (const [key, reminder] of this.reminders.entries()) {
			...
			reminder.daysUntilExpiry = this.calculateDaysUntilExpiry(
				reminder.currentExpiryDate,
			);
			...
			const alreadySent = reminder.remindersSent.some(
				(r) => r.type === config.reminderType,
			);
			...
			try {
				...
				errors.push(
					`Failed to send reminder to ${reminder.userId}: ${(error as Error).message}`,
				);
			}
		}

		console.log(
			`📧 [Annual Refresh] Cron run complete: ${processed} processed, ${emailsSent} emails, ${smsSent} SMS`,
		);
	 ...
	 */
	getUsersNeedingRefresh(daysThreshold: number = 30): Array<{
		userId: string;
		refreshType: string;
		daysUntilExpiry: number;
		email: string;
	...
	generateEmailContent(
		...
		const label =
			refreshTypeLabels[reminder.refreshType] || reminder.refreshType;
...
FintekPro Team`;
	...
	private async sendReminders(
		...
		if (config.channels.includes("email")) {
			...
			console.log(
				`📧 [Annual Refresh] Would send email to ${reminder.email}: ${subject}`,
			);
			emailSent = true;
		}
		...
		if (config.channels.includes("sms") && reminder.mobile) {
			...
			console.log(
				`📱 [Annual Refresh] Would send SMS to ${reminder.mobile}: ${smsContent}`,
			);
			smsSent = true;
		}
	...
	private async blockUserProducts(
		userId: string,
		refreshType: string,
	): Promise<void> {
		// In production, integrate with product access control
		console.log(
			`🔒 [Annual Refresh] Blocking products for user ${userId} due to expired ${refreshType}`,
		);
	}
}
