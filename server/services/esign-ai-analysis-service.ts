...
import { db } from "../db";
import {
	esignDocumentAnnotations,
	esignAiAnalysisSessions,
	esignAnnotationReplies,
	esignAnnotationAuditLog,
	InsertEsignDocumentAnnotation,
	InsertEsignAiAnalysisSession,
} from "@shared/schema";
...
class ESignAIAnalysisService {
	...
	async analyzeDocument(request: AnalysisRequest): Promise<AnalysisResult> {
		...
		try {
			const [session] = await db
				.insert(esignAiAnalysisSessions)
				.values({
					documentId,
			...
			if (this.genAI) {
				...
				throw new Error(
					"AI analysis service not configured. GEMINI_API_KEY required for document analysis.",
				);
			}

			const savedAnnotations = await this.saveAnnotations(
				documentId,
				request.workflowId,
				annotations,
			);
			...
			await db
				.update(esignAiAnalysisSessions)
				.set({
					status: "completed",
					completedAt: new Date(),
					processingTimeMs: processingTime,
					totalAnnotations: annotations.length,
					summaryCount: annotations.filter((a) => a.category === "summary")
						.length,
					correctionCount: annotations.filter(
						(a) => a.category === "correction",
					).length,
					missingClauseCount: annotations.filter(
						(a) => a.category === "missing_clause",
					).length,
					complianceCount: annotations.filter(
	...
	private async performAIAnalysis(
		...
		const documentTypeContext =
			DOCUMENT_TYPE_PROMPTS[request.documentType] ||
...
Return ONLY a valid JSON array, no other text.`;
		...
		try {
			...
			if (!jsonMatch) {
				...
				throw new Error(
					"AI analysis service not configured. OpenAI API key required for document analysis.",
				);
			}
	...
	private validateCategory(category: string): AnnotationCategory {
		const valid: AnnotationCategory[] = [
			"summary",
			"correction",
			"missing_clause",
			"compliance",
		...
		return valid.includes(category as AnnotationCategory)
			? (category as AnnotationCategory)
			: "general";
	}

	private validateSeverity(severity: string): AnnotationSeverity {
		const valid: AnnotationSeverity[] = [
			"info",
			"warning",
			"error",
			"critical",
		];
		return valid.includes(severity as AnnotationSeverity)
			? (severity as AnnotationSeverity)
			: "info";
	}
		...
		for (const annotation of annotations) {
			try {
				const [saved] = await db
					.insert(esignDocumentAnnotations)
					.values({
						documentId,
						workflowId,
	...
	async getAnnotationsForDocument(
		documentId: string,
		options?: {
			category?: AnnotationCategory;
		...
		return db
			...
			.orderBy(
				desc(
					sql`CASE WHEN severity = 'critical' THEN 1 WHEN severity = 'error' THEN 2 WHEN severity = 'warning' THEN 3 ELSE 4 END`,
				),
	...
	async updateAnnotationStatus(
		...
		const [current] = await db
			.select({ status: esignDocumentAnnotations.status })
			.from(esignDocumentAnnotations)
			.where(eq(esignDocumentAnnotations.id, annotationId));
		...
		await db
			.update(esignDocumentAnnotations)
			.set(updateData)
			.where(eq(esignDocumentAnnotations.id, annotationId));
