...
class FinancialCalendarService {
	...
	async initialize() {
		...
		try {
			...
			console.error(
				"❌ Failed to initialize Financial Calendar Service:",
				error,
			);
		}
	}
	...
	async getUpcomingEvents(
		...
		if (instrumentTypes && instrumentTypes.length > 0) {
			conditions.push(
				inArray(bondCalendarEvents.instrumentType, instrumentTypes),
			);
		}
	...
	async updateEvent(
		id: string,
		updates: Partial<InsertBondCalendarEvent>,
	): Promise<BondCalendarEvent | null> {
			...
			for (const gsec of gsecs) {
				if (gsec.maturityDate) {
					...
					for (let i = 0; i < 2; i++) {
						...
						if (
							isAfter(couponDate, today) &&
							isBefore(couponDate, sixMonthsLater)
						) {
							const existingEvent = await db
								.select()
								.from(bondCalendarEvents)
								.where(
									and(
										...
										eq(
											bondCalendarEvents.eventDate,
											format(couponDate, "yyyy-MM-dd"),
										),
									),
								)
								.limit(1);
		...
		const sampleAuctions: InsertBondCalendarEvent[] = [
			{
				...
				eventDescription:
					"RBI auction for 10-year Government Security bearing 7.18% coupon maturing in 2033",
			...
			},
			{
				...
				eventDescription:
					"Auction for State Development Loans from multiple states",
			...
			},
			{
				...
				eventDescription:
					"Sovereign Gold Bond issuance - Series IV of FY 2024-25. Invest in gold-backed government securities.",
		...
		const sampleNCDs: InsertBondCalendarEvent[] = [
			{
				...
				eventDescription:
					"Public issue of Secured Redeemable Non-Convertible Debentures by Bajaj Finance Ltd. AAA rated with attractive yields.",
			...
			},
			{
				...
				eventDescription:
					"Public issue of Secured NCDs by Muthoot Finance. Gold loan leader offering competitive rates.",
			...
			},
			{
				...
				eventDescription:
					"Indian Railway Finance Corporation Tax-Free Bond issue. Government-backed with tax-free interest income.",
	...
	async syncExternalRBICalendar(): Promise<number> {
		...
		try {
			...
			for (const auction of rbiAuctions) {
				...
				if (existingEvent.length === 0) {
					await db.insert(bondCalendarEvents).values({
						...
						instrumentType: auction.securityType
							.toLowerCase()
							.replace(/[- ]/g, "_"),
						issuerName: "Reserve Bank of India",
						issuerType: "government",
			...
			console.log(
				`[Financial Calendar] Synced ${syncedCount} RBI auction events`,
			);
		} catch (error) {
	...
	private generateWeeklyTBillAuctions(
		startDate: Date,
		weeks: number,
	): RBIAuctionEvent[] {
	...
	private generateSGBIssuances(
		...
		for (let i = 0; i < count; i++) {
			...
			const fy =
				new Date().getFullYear() + (new Date().getMonth() >= 3 ? 0 : -1);
			auctions.push({
				date: format(currentDate, "yyyy-MM-dd"),
				securityType: "SGB",
	...
	async syncExternalSEBICalendar(): Promise<number> {
		...
		try {
			...
			for (const issue of sebiIssues) {
				...
				if (existingEvent.length === 0) {
					const instrumentType = issue.issueType
						.toLowerCase()
						.includes("tax-free")
						? "tax_free_bond"
					...
					await db.insert(bondCalendarEvents).values({
						...
						issuerType:
							issue.issuerName.includes("Railway") ||
							issue.issuerName.includes("Government")
								? "psu"
								: "nbfc",
						...
						tags: [
							...
							"public_issue",
							"external",
							issue.creditRating?.split(" ")[0]?.toLowerCase() || "",
						],
					});
					syncedCount++;
				}
			...
			console.log(
				`[Financial Calendar] Synced ${syncedCount} SEBI public issue events`,
			);
		} catch (error) {
	...
	async syncExternalNSECalendar(): Promise<number> {
		...
		try {
			...
			for (const bond of nseBonds) {
				...
				if (existingEvent.length === 0) {
					await db.insert(bondCalendarEvents).values({
						...
						isHighlighted: true,
						tags: [
							bond.issueType.toLowerCase().replace(/ /g, "_"),
							"nse",
							"external",
						],
					});
			...
			console.log(
				`[Financial Calendar] Synced ${syncedCount} NSE bond announcements`,
			);
		} catch (error) {
	...
	async syncExternalBSECalendar(): Promise<number> {
		...
		console.log(
			"[Financial Calendar] Syncing BSE bond platform announcements...",
		);
		...
		try {
			...
			for (const bond of bseBonds) {
				const existingEvent = await db
					...
					.where(
						and(
							...
							eq(
								bondCalendarEvents.instrumentName,
								`${bond.issuerName} ${bond.issueType}`,
							),
						),
					)
					.limit(1);
				...
				if (existingEvent.length === 0) {
					...
					await db.insert(bondCalendarEvents).values({
						...
						creditRating: bond.creditRating,
						minInvestment: String(bond.minInvestment),
						maxInvestment: bond.maxInvestment
							? String(bond.maxInvestment)
							: undefined,
						source: "bse_external",
						sourceUrl:
							"https://www.bseindia.com/markets/debt/debt_corporatebonds.aspx",
						status: "upcoming",
						isHighlighted: true,
						tags: [
							instrumentType,
							"bse",
							"external",
							bond.issueType.includes("54EC") ? "capital_gains" : "tax_benefit",
						],
					});
					syncedCount++;
				}
			...
			console.log(
				`[Financial Calendar] Synced ${syncedCount} BSE bond announcements`,
			);
		} catch (error) {
	...
	generateICalEvent(event: BondCalendarEvent): string {
		...
		if (event.issuerName) {
			lines.push(
				`ORGANIZER;CN=${this.escapeICalText(event.issuerName)}:MAILTO:info@${event.issuerName.toLowerCase().replace(/[^a-z]/g, "")}.com`,
			);
		}
	...
	async generateICalFeed(
		...
		if (options.sources && options.sources.length > 0) {
			filteredEvents = events.filter((e) =>
				options.sources!.some((s) => e.source.includes(s)),
			);
		}

		const icalEvents = filteredEvents
			.map((e) => this.generateICalEvent(e))
			.join("\r\n");
	...
	async getCalendarStats(): Promise<{
		...
		const [auctions, issuances, maturities, coupons, highlighted] =
			await Promise.all([
				db
					.select({ count: sql<number>`count(*)` })
					.from(bondCalendarEvents)
					.where(
						and(...baseConditions, eq(bondCalendarEvents.eventType, "auction")),
					),
				db
					.select({ count: sql<number>`count(*)` })
					.from(bondCalendarEvents)
					.where(
						and(
							...baseConditions,
							or(
								eq(bondCalendarEvents.eventType, "issuance"),
								eq(bondCalendarEvents.eventType, "ipo_open"),
							),
						),
					),
				...
				db
					...
					.from(bondCalendarEvents)
					.where(
						and(...baseConditions, eq(bondCalendarEvents.isHighlighted, true)),
					),
			]);
