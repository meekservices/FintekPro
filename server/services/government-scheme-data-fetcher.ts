...
class GovernmentSchemeDataFetcher {
	...
	async fetchSchemeData(request: FetchRequest): Promise<FetchResult> {
		console.log(
			`🔄 [DATA_FETCHER] Starting ${request.schemeType.toUpperCase()} data fetch for user ${request.userId}`,
		);
		...
		try {
			...
			console.error(
				`❌ [DATA_FETCHER] Error fetching ${request.schemeType} data:`,
				error,
			);
			return {
				success: false,
	 ...
	 */
	private async fetchAndStoreEPFData(
		...
		console.log(
			`✅ [DATA_FETCHER] EPF: ${created} created, ${updated} updated`,
		);
	 ...
	 */
	private async fetchAndStoreEPSData(
		request: FetchRequest,
	): Promise<FetchResult> {
		const epfResponse = await this.epfoService.fetchEPFAccounts({
			panNumber: request.panNumber,
		...
		for (const account of epfResponse.accounts) {
			...
			const pensionableService = this.calculatePensionableService(
				account.dateOfJoining,
				account.dateOfExit,
			);
			const expectedPension = this.calculateExpectedPension(
				account.pensionContribution,
				pensionableService,
			);

			if (existing.length > 0) {
				await db
					.update(schema.epsHoldings)
					.set({
		...
		console.log(
			`✅ [DATA_FETCHER] EPS: ${created} created, ${updated} updated`,
		);
	 ...
	 */
	private async fetchAndStoreNPSData(
		request: FetchRequest,
	): Promise<FetchResult> {
		...
		console.log(
			`✅ [DATA_FETCHER] NPS: ${created} created, ${updated} updated`,
		);
	 ...
	 */
	private async fetchAndStorePPFData(
		request: FetchRequest,
	): Promise<FetchResult> {
		throw new Error(
			"PPF data service not configured. Bank API integration required for PPF data.",
		);
	}
	 ...
	 */
	private async fetchAndStoreAPYData(
		request: FetchRequest,
	): Promise<FetchResult> {
		...
		console.log(
			`✅ [DATA_FETCHER] APY: ${created} created, ${updated} updated`,
		);
	 ...
	 */
	private async fetchAndStoreInsuranceData(
		request: FetchRequest,
	): Promise<FetchResult> {
		throw new Error(
			"Insurance data service not configured. Turtlefin/CAMS API integration required for insurance data.",
		);
	}

	// Helper methods
	private calculatePensionableService(
		...
		const years =
			(exitDate.getTime() - joinDate.getTime()) /
			(1000 * 60 * 60 * 24 * 365.25);
		return Math.floor(years);
	}

	private calculateExpectedPension(
