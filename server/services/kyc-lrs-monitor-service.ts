...
class KycLrsMonitorService {
	...
	start(cronExpression: string = "0 0 * * *") {
		console.log(
			`[LRS Monitor] Starting scheduler with expression: ${cronExpression}`,
		);

		cron.schedule(cronExpression, async () => {
			if (this.isRunning) {
				console.warn(
					"[LRS Monitor] Previous run still in progress, skipping...",
				);
				return;
			}
	...
	async runMonitoringCycle() {
		...
		console.log(
			`[LRS Monitor] Running monitoring cycle for FY ${currentFY}...`,
		);
		...
		console.log(
			`[LRS Monitor] Found ${activeTracking.length} active LRS records to check`,
		);
		...
		for (const record of activeTracking) {
			try {
				...
				console.error(
					`[LRS Monitor] Failed to process user ${record.userId}:`,
					err,
				);
			}
		}
	...
	async processUserLrsLimits(record: any) {
		...
		const totalUsd = Number.parseFloat(stats.totalUsd || "0");
		const totalInr = Number.parseFloat(stats.totalInr || "0");
		const txCount = Number.parseInt((stats.count as any) || "0");
		...
		if (totalUsd >= LRS_LIMIT_USD * 0.9) {
			console.log(
				`[LRS Monitor] ALERT: User ${userId} has utilized >90% of LRS limit ($${totalUsd})`,
			);
		}

		if (totalInr >= TCS_THRESHOLD_INR) {
			console.log(
				`[LRS Monitor] INFO: User ${userId} has crossed ₹7 Lakh TCS threshold (Total: ₹${totalInr})`,
			);
		}
	}
