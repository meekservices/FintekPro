...
import {
	ckycProviderConfig,
	ckycProviderAuditLog,
	ckycVerificationRequests,
	users,
} from "@shared/schema";
import { eq, and, asc, isNull } from "drizzle-orm";
import {
	ckycEnvironmentService,
	type CkycVerificationStatus,
	type CkycDeferralReason,
...
const DEFAULT_PROVIDERS: Array<{
...
}> = [
	{
		...
		providerDescription:
			"Primary CKYC verification via TruthScreen API with real-time KRA lookup",
		priority: 1,
		eligibilityRules: {
	...
	},
	{
		...
		providerDescription:
			"CKYC verification using existing CKYC reference number from CERSAI registry",
		priority: 2,
		eligibilityRules: {
			requiresAadhaarConsent: false,
	...
	},
	{
		...
		providerDescription:
			"Live video verification with geo-tagging and PAN verification",
		priority: 3,
		eligibilityRules: {
			requiresAadhaarConsent: false,
			allowedRiskCategories: ["low", "medium", "high"],
	...
	},
	{
		...
		providerDescription:
			"Admin-assisted manual CKYC verification with mandatory justification",
		priority: 4,
		eligibilityRules: {
			requiresAadhaarConsent: false,
			allowedRiskCategories: ["low", "medium", "high"],
...
export class CkycProviderResolutionService {
	...
	static getInstance(): CkycProviderResolutionService {
		if (!CkycProviderResolutionService.instance) {
			CkycProviderResolutionService.instance =
				new CkycProviderResolutionService();
		}
	...
	async seedDefaultProviders(): Promise<void> {
		try {
			...
			if (existingProviders.length === 0) {
					...
					await db.insert(ckycProviderConfig).values({
						...
						isEnabled:
							provider.providerCode === "manual"
								? true
								: this.checkProviderCredentials(provider.providerCode),
				...
				console.log(
					`[CKYC Provider] Seeded ${DEFAULT_PROVIDERS.length} default providers`,
				);
			} else {
				console.log(
					`[CKYC Provider] ${existingProviders.length} providers already configured`,
				);
			}
		} catch (error) {
	...
	async getEnabledProviders(
		environment?: string,
	): Promise<(typeof ckycProviderConfig.$inferSelect)[]> {
		const currentEnv =
			environment ||
		...
		return allEnabled.filter(
			(p) => p.environment === "all" || p.environment === currentEnv,
		);
	}
	...
	async resolveCkycProvider(
		...
		if (isProductionMode) {
			enabledProviders = enabledProviders.filter(
				(p) => p.providerCode !== "mock",
			);
			console.log(
				"[CKYC Provider] PROD mode: Mock provider excluded from resolution chain",
			);
		}
		...
		const manualProvider = enabledProviders.find(
			(p) => p.providerCode === "manual",
		);
		if (manualProvider) {
			const manualEligibility = await this.checkClientEligibility(
				manualProvider,
				context,
			);
			if (manualEligibility.eligible) {
				return {
					...
					providerName: "Manual CKYC",
					selectionReason:
						"All automated providers exhausted, manual verification required",
					fallbackChain,
					fallbackAttempts,
					isFallback: true,
					...
					deferralReason: {
						...
						message:
							"All automated CKYC providers exhausted. Case routed to manual review queue.",
						lastProviderAttempted,
						failureReasons: fallbackAttempts,
						slaStartedAt: new Date().toISOString(),
						slaDeadline: ckycEnvironmentService
							.calculateSlaDeadline(72)
							.toISOString(),
					},
				};
			}
		...
		console.warn(
			`[CKYC Provider] All providers exhausted for user ${context.userId}, status: CKYC_DEFERRED`,
		);
		...
		return {
			...
			selectionReason:
				"All CKYC providers exhausted. Case requires immediate admin attention.",
			fallbackChain,
			fallbackAttempts,
			isFallback: true,
			isDeferred: true,
			...
			deferralReason: {
				...
				message:
					"All CKYC providers failed or unavailable. This case has been escalated to the admin queue.",
				lastProviderAttempted,
				failureReasons: fallbackAttempts,
				slaStartedAt: new Date().toISOString(),
				slaDeadline: ckycEnvironmentService
	...
	private isRateLimited(
		...
		if (
			...
			new Date(provider.rateLimitResetAt) < now
		) {
	...
	private async checkClientEligibility(
		...
		if (
			rules.requiresApiCredentials &&
			!this.checkProviderCredentials(provider.providerCode as CkycProviderCode)
		) {
		...
		const allowedRiskCategories =
			(rules.allowedRiskCategories as string[]) || [];
		if (context.riskCategory && allowedRiskCategories.length > 0) {
			if (!allowedRiskCategories.includes(context.riskCategory)) {
				return {
					eligible: false,
					reason: `Risk category ${context.riskCategory} not allowed for this provider`,
				};
			}
		}
	...
	async updateProviderHealth(
		...
		const updates: Partial<typeof ckycProviderConfig.$inferInsert> = {
			healthStatus: status,
		...
		if (status === "unhealthy") {
			updates.consecutiveFailures =
				(currentProvider.consecutiveFailures || 0) + 1;
	...
	async getProviderByCode(
		providerCode: CkycProviderCode,
	): Promise<typeof ckycProviderConfig.$inferSelect | null> {
	...
	async getProviderAuditLog(
		providerCode?: CkycProviderCode,
		limit: number = 50,
	): Promise<(typeof ckycProviderAuditLog.$inferSelect)[]> {
		if (providerCode) {
			return db
				.select()
	...
	async recordVerificationRequest(
		...
		const result = await db
			.insert(ckycVerificationRequests)
			.values({
				userId,
				panNumber,
				selectedProvider,
	...
	async updateVerificationRequestResponse(
		...
		await db
			.update(ckycVerificationRequests)
			.set({
				responseStatus,
				responseCode,
				responseMessage,
	...
	async verifyCkyc(
		...
		const providerCodes = enabledProviders.map(
			(p) => p.providerCode as CkycProviderCode,
		);
		...
		for (const providerCode of providerCodes) {
			...
			if (providerConfig) {
				...
				const eligibility = await this.checkClientEligibility(
					providerConfig,
					context,
				);
				if (!eligibility.eligible) {
			...
			try {
				...
				console.error(
					`[CKYC Provider] Error with ${providerCode}:`,
					error.message,
				);
...
export const ckycProviderResolutionService =
	CkycProviderResolutionService.getInstance();
