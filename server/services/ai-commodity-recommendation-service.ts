...
class AICommodityRecommendationService {
	constructor() {
		...
		console.log(
			`✅ AI Commodity Recommendation Service initialized via Unified Engine (primary: ${status.primary})`,
		);
	}

	async generateRecommendations(
		params: CommodityRecommendationParams,
	): Promise<CommodityPortfolioSummary> {
		const availableCommodities = await this.fetchCommodities(params);
		const scoredCommodities = availableCommodities.map((c) =>
			this.scoreCommodity(c, params),
		);
		const sortedCommodities = scoredCommodities.sort(
			(a, b) => b.suitabilityScore - a.suitabilityScore,
		);
		const selectedCommodities = this.selectOptimalPortfolio(
			sortedCommodities,
			params,
		);
		const enrichedCommodities = await this.enrichWithAIRationale(
			selectedCommodities,
			params,
		);
	...
	private async fetchCommodities(
		...
		const allowedTypes =
			params.preferredCommodityTypes.length > 0
		...
		try {
			...
			if (result.length === 0) {
				throw new Error(
					"Commodity data API not configured. Live commodity market data service required.",
				);
			}

			// Enrich commodities missing a live price from the commodity_prices sync table
			const needsPrice = result.filter(
				(c) =>
					!c.currentPrice || Number.parseFloat(c.currentPrice.toString()) <= 0,
			);
			if (needsPrice.length > 0) {
				try {
					...
					for (const commodity of needsPrice) {
						const livePrice = priceBySymbol.get(
							(commodity.symbol || "").toUpperCase(),
						);
						if (livePrice && livePrice > 0) {
							(commodity as any).currentPrice = livePrice.toString();
							console.log(
								`[CommodityAI] Price enrichment: ${commodity.symbol} → ₹${livePrice} from sync table`,
							);
						}
					}
				} catch (enrichErr) {
					console.warn(
						"[CommodityAI] commodity_prices enrichment failed:",
	...
	private scoreCommodity(
		commodity: Commodity,
		params: CommodityRecommendationParams,
	): CommodityRecommendation & { rawScore: number } {
		let suitabilityScore = 50;
		let riskScore = 50;

		const typeScore =
			COMMODITY_TYPE_SCORES[commodity.commodityType || "precious_metal"] || 50;
		suitabilityScore += (typeScore - 50) * 0.2;

		const volatility = Number.parseFloat(
			commodity.volatility?.toString() || "20",
		);
		...
		const returns1y = Number.parseFloat(
			commodity.returns1y?.toString() || "10",
		);
		...
		return {
			...
			currentPrice: Number.parseFloat(
				commodity.currentPrice?.toString() || "0",
			),
			...
			suggestedAmount: 0,
			investmentVehicle,
			returns1y: Number.parseFloat(commodity.returns1y?.toString() || "0"),
			returns3y: Number.parseFloat(commodity.returns3y?.toString() || "0"),
			volatility: Number.parseFloat(commodity.volatility?.toString() || "0"),
			suitabilityScore,
			riskScore,
			aiRationale: "",
	...
	private selectInvestmentVehicle(
		commodity: Commodity,
		params: CommodityRecommendationParams,
	): string {
		if (params.investmentVehicle !== "any") {
		...
		if (commodity.hasFutures && params.riskTolerance === "aggressive")
			return "futures";
	...
	private generatePros(commodity: Commodity): string[] {
		...
		if (commodity.safeHaven)
			pros.push("Safe haven asset during market turmoil");
		if (commodity.globalDemand === "high") pros.push("Strong global demand");
		if (commodity.supplyOutlook === "bullish")
			pros.push("Favorable supply outlook");

		const returns1y = Number.parseFloat(commodity.returns1y?.toString() || "0");
		if (returns1y > 15)
			pros.push(`Strong 1-year returns of ${returns1y.toFixed(1)}%`);

		if (commodity.hasEtf) pros.push("Easy access via ETFs");
		if (commodity.hasSgb) pros.push("Sovereign Gold Bond option available");

		const volatility = Number.parseFloat(
			commodity.volatility?.toString() || "0",
		);
	...
	private generateCons(commodity: Commodity): string[] {
		...
		if (volatility > 25)
			cons.push(`High volatility of ${volatility.toFixed(1)}%`);
		...
		if (commodity.supplyOutlook === "bearish")
			cons.push("Unfavorable supply outlook");

		const returns1y = Number.parseFloat(commodity.returns1y?.toString() || "0");
		if (returns1y < 0)
			cons.push(`Negative 1-year returns of ${returns1y.toFixed(1)}%`);

		if (!commodity.hasEtf && !commodity.hasSgb)
			cons.push("Limited investment vehicle options");
		if (commodity.hasPhysical && !commodity.hasEtf)
			cons.push("Storage and security considerations");
			...
			if (Array.isArray(result) && result.length > 0) {
				return recommendations.map((rec) => {
					...
					return {
						...rec,
						aiRationale:
							aiData?.rationale || this.generateRuleBasedRationale(rec, params),
					};
				});
			}
		} catch (error) {
	...
	private generateRuleBasedRationale(
		...
		if (rec.inflationHedge && params.inflationProtection) {
			parts.push(
				`${rec.name} serves as an effective inflation hedge, protecting your purchasing power.`,
			);
		}

		if (rec.safeHaven && params.safeHavenAllocation) {
			parts.push(
				`As a safe haven asset, ${rec.name} provides stability during market uncertainty.`,
			);
		}

		if (rec.returns1y > 15) {
			parts.push(
				`With ${rec.returns1y.toFixed(1)}% returns over the past year, ${rec.name} shows strong momentum.`,
			);
		}

		if (rec.volatility < 15) {
			parts.push(
				`Low volatility of ${rec.volatility.toFixed(1)}% aligns with your ${params.riskTolerance} risk profile.`,
			);
		}

		if (parts.length === 0) {
			parts.push(
				`${rec.name} offers diversification benefits and exposure to the ${rec.commodityType.replace("_", " ")} sector.`,
			);
		}
	...
	private buildPortfolioSummary(
		...
		const weightedReturn = recommendations.reduce(
			(sum, r) => sum + (r.returns1y * r.suggestedAllocation) / 100,
			0,
		);

		const weightedVolatility = recommendations.reduce(
			(sum, r) => sum + (r.volatility * r.suggestedAllocation) / 100,
			0,
		);

		const uniqueTypes = new Set(recommendations.map((r) => r.commodityType))
			.size;
		...
		const riskAnalysis = {
			priceVolatilityRisk:
				weightedVolatility > 25
					? ("high" as const)
					: weightedVolatility > 15
						? ("medium" as const)
						: ("low" as const),
			currencyRisk:
				allocationBreakdown.energy > 30
					? ("high" as const)
					: ("medium" as const),
			storageRisk: recommendations.some(
				(r) => r.investmentVehicle === "physical",
			)
				? ("medium" as const)
				: ("low" as const),
			liquidityRisk: recommendations.some(
				(r) => r.investmentVehicle === "futures",
			)
				? ("medium" as const)
				: ("low" as const),
		};

		const portfolioRationale = this.generatePortfolioRationale(
			recommendations,
			params,
			allocationBreakdown,
		);
	...
	private generatePortfolioRationale(
		...
		if (breakdown.preciousMetals > 50) {
			parts.push(
				`Portfolio emphasizes precious metals (${breakdown.preciousMetals.toFixed(0)}%) for stability and inflation protection.`,
			);
		}
		...
		if (params.inflationProtection) {
			...
			parts.push(
				`${hedgeCount} inflation-hedging commodities included to protect purchasing power.`,
			);
		}
		...
		if (params.safeHavenAllocation) {
			...
			parts.push(
				`${safeHavenCount} safe haven assets provide downside protection.`,
			);
		}

		parts.push(
			`Diversified across ${recommendations.length} commodities to manage concentration risk.`,
		);

		return parts.join(" ");
	}

	async getClientCommodityProfile(
		clientId: string,
	): Promise<CommodityRecommendationParams | null> {
		try {
	...
	private mapRiskTolerance(
		risk: string | null,
	): CommodityRecommendationParams["riskTolerance"] {
...
export const aiCommodityRecommendationService =
	new AICommodityRecommendationService();
