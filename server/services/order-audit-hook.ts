...
class OrderAuditHook {
	...
	async logOrderEvent(options: OrderAuditOptions): Promise<void> {
		...
		const generatedReferenceId =
			referenceId ||
			`${productType.toUpperCase()}-${orderId.substring(0, 8)}-${Date.now()}`;
		const generatedComplianceHash =
			complianceHash ||
			this.generateComplianceHash({
				orderId,
				productType,
				action,
		...
		try {
			await auditLogService.log(`ORDER_${productType.toUpperCase()}`, action, {
				userId,
				userRole,
			...
			console.log(
				`[OrderAudit] ${productType}:${action} - Order ${orderId.substring(0, 8)}... [ref:${generatedReferenceId.substring(0, 12)}]`,
			);
		} catch (error) {
			console.error(
				`[OrderAudit] Failed to log ${productType}:${action}:`,
				error,
			);
		}
	}
	...
	private detectChannel(
		req?: Request,
	): "web" | "mobile" | "api" | "agent_assisted" | "auto_rebalance" {
		if (!req) return "api";
