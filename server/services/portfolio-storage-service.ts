...
class PortfolioStorageService {
	async upsertProspectPortfolio(
		...
		await this.updateProspectUploadedPortfolio(
			prospectId,
			holdings,
			summary,
			options,
	...
	async syncExternalHoldings(
		...
		for (const h of holdings) {
			try {
				...
				console.error(
					`[PortfolioStorage] Failed to insert holding: ${h.name}`,
					error,
				);
				skipped++;
			}
	...
	async getProspectPortfolio(
		prospectId: string,
	): Promise<{ portfolio: any; holdings: any[] } | null> {
