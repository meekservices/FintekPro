...
class UnifiedPDFParser {
	...
	async extractText(
		...
		if (buffer.length > maxFileSize) {
			throw new Error(
				`PDF file size (${(buffer.length / 1024 / 1024).toFixed(2)}MB) exceeds maximum allowed (${(maxFileSize / 1024 / 1024).toFixed(2)}MB)`,
			);
		}
		...
		try {
			...
			if (!parseResult.text || parseResult.text.trim().length === 0) {
				console.warn(
					"[UnifiedPDFParser] Warning: PDF parsed but contains no extractable text",
				);
			}
			...
			if (parser) {
				try {
					...
					console.warn(
						"[UnifiedPDFParser] Warning: Failed to destroy parser instance",
						destroyError,
					);
				}
			}
	...
	async extractTextSafe(
		input: Buffer | string,
		options: TextExtractOptions = {},
	): Promise<SafeExtractResult> {
		const { enableOCRFallback = true, ocrHint = "financial_statement" } =
			options;
		...
		try {
			...
			if (!result.text || result.text.trim().length === 0) {
				...
				if (rawBuffer) {
					const pdfHead = rawBuffer
						.slice(0, Math.min(rawBuffer.length, 65536))
						.toString("latin1");
					if (pdfHead.includes("/Encrypt")) {
						console.warn(
							"[UnifiedPDFParser] PDF appears to be password-protected (/Encrypt found)",
						);
						return {
							success: false,
							result,
				...
				if (enableOCRFallback && rawBuffer) {
					console.log(
						"[UnifiedPDFParser] Empty text from pdf-parse — attempting Gemini OCR fallback",
					);
					const ocrResult = await unifiedOCRService.extractTextFromScannedPDF(
						rawBuffer,
						ocrHint,
					);
					...
					console.warn(
						"[UnifiedPDFParser] OCR fallback also returned empty — giving up",
					);
					return {
						success: false,
						result,
	...
	async profileDocument(buffer: Buffer): Promise<PDFProfile> {
		...
		if (cached) {
			console.log(
				"[UnifiedPDFParser] Profile cache hit:",
		...
		console.log(
			`[UnifiedPDFParser] Document profiled in ${Date.now() - startTime}ms:`,
			{
	...
	async parseDocument(
		...
		try {
			...
			const semanticResult = this.extractSemanticData(
				text,
				layoutAnalysis,
				profile,
			);
			...
			const unitBalanceValidation = this.validateUnitBalance(
				semanticResult.transactions,
			);
			...
			if (!unitBalanceValidation.valid) {
				warnings.push(
					`Unit balance discrepancies: ${unitBalanceValidation.discrepancies.length}`,
				);
			}
			...
			const result: ParseResult = {
				...
				summary: {
					...
					totalInvestedValue: holdingsWithDates.reduce(
						(sum, h) => sum + (h.investedValue || 0),
						0,
					),
					totalCurrentValue: holdingsWithDates.reduce(
						(sum, h) => sum + (h.currentValue || 0),
						0,
					),
					totalUnrealizedGain: holdingsWithDates.reduce(
						(sum, h) => sum + (h.unrealizedGain || 0),
						0,
					),
				},
				...
				parsingMetrics: {
					...
					validationsPassed: confidenceBreakdown.flags.filter(
						(f) => !f.startsWith("LOW_"),
					).length,
					validationsFailed: confidenceBreakdown.flags.filter(
						(f) => f.startsWith("LOW_") || f === "BELOW_THRESHOLD",
					).length,
				},
				errors,
				warnings,
				requiresEnrichment:
					summaryAnalysis.requiresEnrichment ||
					semanticResult.requiresEnrichment,
	...
	private detectPDFType(text: string): PDFType {
		...
		if (/groww\.in|groww\s+portfolio|groww\s+investments/i.test(text))
			return "broker_groww";
		...
		if (/angel\s*one|angelbroking|angelone\.in/i.test(text))
		...
		if (/religare\s*broking|religare\s*securities/i.test(text))
			return "broker_religare";
		...
		if (/vested\.co\.in|vested\s*finance/i.test(text))
			return "broker_vested_us";
		if (/funds\s*india|fundsindia/i.test(text)) return "aggregator_fundsindia";

		if (/wealthy\.in|wealthy\s+portfolio/i.test(text))
			return "aggregator_wealthy";
		if (/mf\s*central|mfcentral\.com/i.test(text))
		...
		const hasTransactions =
			/transaction\s*date|purchase|redemption|switch\s*(in|out)/i.test(text);
		const hasHoldings = /unit\s*balance|market\s*value|current\s*value/i.test(
			text,
		);
	...
	private detectLayoutType(text: string): LayoutType {
		...
		for (const line of lines) {
			...
			const hasMultipleNumbers =
				(trimmed.match(/[\d,]+\.\d{2}/g) || []).length >= 2;
	...
	private detectHeaderPatterns(text: string): string[] {
		...
		for (const line of lines) {
			...
			const matchCount = headerKeywords.filter((k) =>
	...
	analyzeLayout(text: string): LayoutAnalysis {
		...
		for (let pageIdx = 0; pageIdx < pages.length; pageIdx++) {
			...
			const isContinuation =
				previousPageHadContinuation || this.detectContinuation(pageLines);
		...
		analysisResult.layoutConfidence =
			this.calculateLayoutConfidence(analysisResult);
	...
	private identifyPageZones(lines: string[], pageIndex: number): PageSection[] {
		...
		const footerStartIdx = Math.max(
			lineCount - Math.floor(lineCount * 0.1),
			lineCount - 10,
		);
	...
	private endsWithContinuation(lines: string[]): boolean {
		...
		return /continued\s+on\s+next|to\s+be\s+continued|\.{3,}$/.test(
			lastFewLines,
		);
	}
	...
	private extractSemanticData(
		...
		for (let i = 0; i < lines.length; i++) {
			...
			const schemeMatch = line.match(
				/^([A-Z][A-Za-z\s\-]+(?:Fund|Plan|Growth|Dividend|Direct|Regular))/i,
			);
			...
			const unitsMatch = line.match(
				/(\d+(?:,\d{3})*\.?\d*)\s*(?:units?|balance)/i,
			);
			const valueMatch = line.match(
				/(?:current|market|value)[:\s]*₹?\s*(\d+(?:,\d{3})*\.?\d*)/i,
			);
			...
			const txnMatch = line.match(
				/(\d{2}[-\/]\d{2}[-\/]\d{4})\s+(purchase|sip|redemption|switch\s*(?:in|out)|dividend|bonus)/i,
			);
			if (txnMatch) {
				const amountMatch = line.match(/(\d+(?:,\d{3})*\.?\d*)\s*$/);
				const unitsMatch2 = line.match(
					/(\d+\.?\d*)\s+(?:units?)?.*(\d+(?:,\d{3})*\.?\d*)$/i,
				);
				...
				transactions.push({
					...
					units: unitsMatch2 ? Number.parseFloat(unitsMatch2[1]) : 0,
					nav: 0,
					amount: amountMatch
		...
		const holdingsWithExitLoad = this.extractExitLoadForHoldings(
			text,
			holdings,
		);

		const requiresEnrichment =
			holdingsWithExitLoad.some((h) => h.requiresEnrichment) ||
			unresolvedItems.length > 0 ||
			profile.pdfType === "summary_only";

		return {
			holdings: holdingsWithExitLoad,
			transactions,
			investor,
			unresolvedItems,
			requiresEnrichment,
	...
	private extractInvestorInfo(text: string): InvestorInfo {
		...
		const nameMatch = text.match(
			/(?:investor|account\s*holder|name)\s*[:\-]?\s*([A-Z][A-Za-z\s]+?)(?:\n|pan|email)/i,
		);
		if (nameMatch) {
		...
		const mobileMatch = text.match(
			/(?:mobile|phone|contact)\s*[:\-]?\s*(\+?91?\s*)?(\d{10})/i,
		);
		if (mobileMatch) {
	...
	private mapTransactionType(type: string): TransactionRow["type"] {
		...
		if (lowerType.includes("switch") && lowerType.includes("in"))
			return "switch_in";
		if (lowerType.includes("switch") && lowerType.includes("out"))
			return "switch_out";
		...
		for (const pattern of exitLoadPatterns) {
			...
			if (match) {
				...
				for (const pPattern of percentPatterns) {
					...
					if (pMatch) {
						exitLoadPercent = Number.parseFloat(pMatch[1]);
						break;
					}
				}
				...
				for (const dPattern of daysPatterns) {
					...
					if (dMatch) {
						const value = Number.parseInt(dMatch[1], 10);
				...
				if (!exitLoadDays) {
					if (
						exitLoadText.toLowerCase().includes("1 year") ||
						exitLoadText.toLowerCase().includes("12 month")
					) {
						exitLoadDays = 365;
	 ...
	 */
	private extractExitLoadForHoldings(
		text: string,
		holdings: SemanticHolding[],
	): SemanticHolding[] {
		const lines = text.split("\n");
		...
		return holdings.map((holding) => {
			// Find the ISIN line
			const isinLine = lines.findIndex(
				(line) => line.includes(holding.isin || "") && holding.isin,
			);
			...
			const matchingTxns = transactions
				.filter((t) => {
					...
					if (holding.schemeName && t.description) {
						const normalizedHoldingName = holding.schemeName
							.toLowerCase()
							.replace(/\s+/g, "");
						const normalizedTxnDesc = t.description
							.toLowerCase()
							.replace(/\s+/g, "");
						if (
							normalizedHoldingName.includes(normalizedTxnDesc) ||
	...
	private buildHoldingLots(transactions: TransactionRow[]): HoldingLot[] {
		...
		const purchaseTxns = transactions.filter(
			(t) =>
				t.type === "purchase" ||
				t.type === "sip" ||
				t.type === "switch_in" ||
		...
		const redemptionTxns = transactions
			.filter((t) => t.type === "redemption" || t.type === "switch_out")
	...
	private calculateConfidenceScore(
		...
		breakdown.components.isinMatch =
			holdingsWithISIN / Math.max(holdings.length, 1);
		if (breakdown.components.isinMatch < 0.5) {
			breakdown.flags.push("LOW_ISIN_MATCH");
			breakdown.recommendations.push(
				"Consider manual ISIN lookup for unmatched holdings",
			);
		}

		const holdingsWithDate = holdings.filter(
			(h) => h.purchaseDate && h.purchaseDateSource !== "unresolved",
		).length;
		breakdown.components.dateResolution =
			holdingsWithDate / Math.max(holdings.length, 1);
		...
		const holdingsWithValue = holdings.filter(
			(h) => h.currentValue && h.currentValue > 0,
		).length;
		breakdown.components.valueAccuracy =
			holdingsWithValue / Math.max(holdings.length, 1);
	...
	private detectSummaryPDF(
		...
		const hasTransactions =
			/transaction\s*date|purchase.*\d{2}[-\/]\d{2}/i.test(text);
		const hasSummaryMarkers =
			/portfolio\s*summary|investment\s*summary|valuation\s*summary/i.test(
				text,
			);
		const hasDetailedHoldings = /isin.*nav|folio.*units.*value/i.test(text);

		const isSummaryOnly =
			hasSummaryMarkers && !hasTransactions && !hasDetailedHoldings;
		...
		if (
			profile.pdfType === "aggregator_wealthy" ||
			profile.pdfType === "aggregator_indmoney"
		) {
	...
	private async storeLearnedPattern(
		profile: PDFProfile,
		result: {
			success: boolean;
			holdingsCount: number;
		...
		if (existing) {
			...
			existing.avgHoldingsCount =
				(existing.avgHoldingsCount + result.holdingsCount) / 2;
			existing.avgConfidenceScore =
				(existing.avgConfidenceScore + result.confidenceScore) / 2;
		} else {
	...
	private createParsingLog(
		eventType: string,
		data: Partial<ParsingLogEntry>,
	): void {
	...
	getParsingMetrics(timeWindowHours: number = 24): ParsingMetrics {
		...
		const enrichmentNeeded = recentLogs.filter(
			(e) => e.eventType === "enrichment_needed",
		).length;
		...
		return {
			...
			patternMatchRate:
				(recentLogs.filter((e) => e.eventType === "pattern_matched").length /
					Math.max(total, 1)) *
				100,
		};
	}
	...
	getErrorSummary(limit: number = 20): Array<{
		...
		return parsingHistory
			.filter(
				(e) => e.eventType === "parse_error" && e.errors && e.errors.length > 0,
			)
			.slice(-limit)
			.map((e) => ({
	...
	setLearningEnabled(enabled: boolean): void {
		...
		console.log(
			`[UnifiedPDFParser] Learning ${enabled ? "enabled" : "disabled"}`,
		);
	}
	...
	setMinConfidenceThreshold(threshold: number): void {
		...
		console.log(
			`[UnifiedPDFParser] Min confidence threshold set to ${this.minConfidenceThreshold}`,
		);
	}
