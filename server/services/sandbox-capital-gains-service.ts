...
const taxResultCache = new Map<
	string,
	{ result: TaxPnLResult; timestamp: number }
>();
...
class SandboxCapitalGainsService {
	async submitTaxPnLJob(request: TaxPnLJobRequest): Promise<JobSubmitResponse> {
		...
		try {
			...
			if (!data?.job_id || !data?.upload_url) {
				throw new Error(
					response.data.message ||
						"Invalid response: missing job_id or upload_url",
				);
			}
			...
			console.error(
				"[SandboxCapitalGains] Job submit failed:",
	...
	async uploadTransactionPayload(
		uploadUrl: string,
		transactions: SecurityTransaction[],
	): Promise<boolean> {
		try {
			...
			console.error(
				"[SandboxCapitalGains] Payload upload failed:",
				error?.response?.data || error.message,
			);
			throw error;
		}
	...
	async getJobStatus(jobId: string): Promise<JobStatusResponse> {
		...
		try {
			...
			console.error(
				"[SandboxCapitalGains] Job status check failed:",
				error?.response?.data || error.message,
			);
			throw error;
		}
	...
	async calculateTaxPnL(request: TaxPnLJobRequest): Promise<TaxPnLResult> {
		...
		await this.uploadTransactionPayload(
			jobResponse.uploadUrl,
	...
	private findMatchingScripResult(
		...
		if (productName) {
			const byName = scripWise.find(
				(s) => s.scripName.toLowerCase() === productName.toLowerCase(),
			);
			if (byName) return byName;

			const byPartialName = scripWise.find(
				(s) =>
	...
	async calculateSingleHoldingTax(
		holding: CapitalGainsTaxInput,
	): Promise<CapitalGainsTaxOutput> {
		...
		const isLongTerm = this.isLongTermHolding(
			holding.assetType,
			holdingPeriodDays,
		);
		...
		try {
			...
			const scripResult = this.findMatchingScripResult(
				result.scripWise,
				holding.isin,
				holding.productName,
			);
			...
			return {
				...
				applicableTaxRate:
					scripResult?.applicableTaxRate ??
					(isLongTerm ? taxRates.ltcgEquity : taxRates.stcgEquity),
				estimatedTax:
					scripResult?.taxLiability ??
					this.calculateLocalTax(unrealizedGain, isLongTerm, holding.assetType),
				effectiveTaxRate:
					unrealizedGain > 0
						? (scripResult?.taxLiability ?? 0) / unrealizedGain
						: 0,
			...
			console.warn(
				"[SandboxCapitalGains] API failed, using local fallback:",
				error,
			);
			return this.calculateLocalFallback(
				holding,
				holdingPeriodDays,
				isLongTerm,
				unrealizedGain,
	...
	async getCurrentTaxRates(): Promise<{
		...
		try {
			const dummyTransaction: SecurityTransaction = {
				...
				transactionType: "BUY",
				transactionDate: new Date(Date.now() - 400 * 24 * 60 * 60 * 1000)
					.toISOString()
					.split("T")[0],
				quantity: 1,
				pricePerUnit: 100,
			...
			console.warn(
				"[SandboxCapitalGains] Could not fetch tax rates from API, using local fallback",
			);
			return {
				stcgEquity: 0.2,
				ltcgEquity: 0.125,
	...
	private parseResult(rawResult: any): TaxPnLResult {
		return {
			...
			taxRates: {
				stcgEquity: rawResult.tax_rates?.stcg_equity || 0.2,
				ltcgEquity: rawResult.tax_rates?.ltcg_equity || 0.125,
				stcgDebt: rawResult.tax_rates?.stcg_debt || 0.2,
	...
	private calculateLocalFallback(
		...
		const estimatedTax = this.calculateLocalTax(
			unrealizedGain,
			isLongTerm,
			holding.assetType,
		);
		...
		return {
			...
			purchaseValue: holding.purchaseValue,
			currentValue: holding.currentValue,
			applicableTaxRate: isLongTerm ? 0.125 : 0.2,
			estimatedTax,
	...
	private generateCacheKey(request: TaxPnLJobRequest): string {
		const transactionHash = request.transactions
			.map(
				(t) =>
					`${t.isin || t.scripName}:${t.transactionType}:${t.transactionDate}:${t.quantity}`,
			)
			.sort()
			.join("|");
