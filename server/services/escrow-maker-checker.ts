...
export class EscrowMakerCheckerService {
		...
		try {
			...
			if (request.disSlipVerified) {
				complianceChecks.push({
					check: "dis_slip_verified",
					verified: true,
					verifiedBy: request.makerUserId,
					verifiedAt: new Date().toISOString(),
				});
			}
			if (request.shareTransferVerified) {
				complianceChecks.push({
					check: "share_transfer_verified",
					verified: true,
					verifiedBy: request.makerUserId,
					verifiedAt: new Date().toISOString(),
				});
			...
			const [approval] = await db
				.insert(unlistedEscrowApprovals)
				.values({
					...
					requestedAmount:
						request.requestType === "release"
							? deal.totalValue
							: deal.buyerCharge || deal.totalValue,
			...
			console.log(
				`[MakerChecker] Escrow ${request.requestType} initiated by maker ${request.makerUserId} for deal ${request.dealId}`,
			);
			...
			return {
				success: false,
				error: error.message || "Failed to initiate approval",
			};
		}
	}
	 ...
	 */
	async processCheckerAction(
		request: CheckerApprovalRequest,
	): Promise<ApprovalResult> {
		try {
			...
			if (approval.expiresAt && new Date(approval.expiresAt) < new Date()) {
				await db
					.update(unlistedEscrowApprovals)
			...
			if (approval.makerUserId === request.checkerUserId) {
				return {
					success: false,
					error:
						"Checker must be a different admin than the maker (dual-control requirement)",
				};
			}
			...
			if (request.action === "rejected") {
				...
				console.log(
					`[MakerChecker] Escrow ${approval.requestType} rejected by checker ${request.checkerUserId} for deal ${approval.dealId}`,
				);
			...
			if (approval.requestType === "release") {
				...
				if (deal) {
					const disPrerequisites =
						await disVerificationService.getEscrowReleasePrerequisites(
							approval.dealId,
							Number.parseFloat(deal.quantity || "0"),
							deal.isin || "",
					...
					if (!disPrerequisites.canRelease) {
						...
						console.warn(
							`[MakerChecker] DIS verification FAILED for deal ${approval.dealId}: ${failureDetails}`,
						);
					...
					console.log(
						`[MakerChecker] DIS verification PASSED for deal ${approval.dealId}`,
					);
				}
			}
			...
			try {
				...
				if (deal) {
					...
					await auditLogArchivalService.archiveUnlistedMarketplaceEvent({
						...
						amount: Number.parseFloat(approval.requestedAmount || "0"),
						metadata: {
							approvalId: approval.id,
					...
					const amount = Number.parseFloat(approval.requestedAmount || "0");
					if (amount >= 1000000) {
				...
				console.error(
					"[MakerChecker] Pre-execution compliance logging failed:",
					preComplianceError,
				);
			...
			await db
				...
				.set({
					...
					executionResult,
					rejectionReason: executionResult.success
						? null
			...
			console.log(
				`[MakerChecker] Escrow ${approval.requestType} ${executionResult.success ? "executed" : "failed"} after checker approval for deal ${approval.dealId}`,
			);
			...
			try {
				...
				if (deal) {
					const outcomeEventType = executionResult.success
						? approval.requestType === "release"
							? "escrow_released_dual_approved"
							: "escrow_refunded_dual_approved"
						: approval.requestType === "release"
							? "escrow_release_failed_dual_approved"
							: "escrow_refund_failed_dual_approved";
					...
					if (amount >= 1000000) {
						const reportEventType = executionResult.success
							? approval.requestType === "release"
								? "high_value_escrow_release_completed"
								: "high_value_escrow_refund_completed"
							: approval.requestType === "release"
								? "high_value_escrow_release_failed"
								: "high_value_escrow_refund_failed";
				...
				console.error(
					"[MakerChecker] Post-execution compliance logging failed:",
					postComplianceError,
				);
			...
			return {
				success: false,
				error: error.message || "Failed to process checker action",
			};
		}
	}
	...
	async expireOldApprovals(): Promise<number> {
		try {
			...
			const expiredCount = result.filter(
				(r) => r.expiresAt && new Date(r.expiresAt) < new Date(),
			).length;

			if (expiredCount > 0) {
				console.log(
					`[MakerChecker] Expired ${expiredCount} old approval requests`,
				);
			}
