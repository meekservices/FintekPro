...
export class KycOrchestrationEngine {
	async executeVerification(
		...
		console.log(
			`[KYC-ENGINE] Starting verification for step=${request.kycStep}, product=${request.productType}, user=${request.userId}`,
		);
		...
		try {
			...
			if (filteredPriorities.length === 0) {
				console.log(
					`[KYC-ENGINE] No providers configured for step=${request.kycStep}, product=${request.productType}`,
				);
				return {
					success: false,
					providerId: 0,
			...
			for (const priorityEntry of filteredPriorities) {
				...
				if (!provider) {
					console.log(
						`[KYC-ENGINE] Provider id=${priorityEntry.providerId} not found, skipping`,
					);
					continue;
				}
				...
				if (!provider.isEnabled || !provider.isConfigured) {
					console.log(
						`[KYC-ENGINE] Provider ${provider.providerCode} is not enabled/configured, skipping`,
					);
					continue;
				}
				...
				if (result.success) {
					console.log(
						`[KYC-ENGINE] Verification succeeded via provider=${provider.providerCode}, latency=${result.latencyMs}ms`,
					);
				...
				const fallbackErrorCodes = priorityEntry.fallbackErrorCodes as
					| string[]
					| null;
				const shouldFallback =
					fallbackErrorCodes === null
						? true
						: result.errorCode
							? fallbackErrorCodes.includes(result.errorCode)
							: false;

				if (shouldFallback) {
					console.log(
						`[KYC-ENGINE] Fallback triggered: provider=${provider.providerCode}, errorCode=${result.errorCode}, trying next provider`,
					);
					await this.logAuditEvent(
						"kyc_fallback",
						"provider",
				...
				console.log(
					`[KYC-ENGINE] Non-recoverable error from provider=${provider.providerCode}, errorCode=${result.errorCode} (fallback suppressed by config)`,
				);
				return result;
			}

			console.log(
				`[KYC-ENGINE] All providers exhausted for step=${request.kycStep}, fallbackChain=${fallbackChain.join(" -> ")}`,
			);
			return {
				success: false,
				providerId: 0,
			...
			console.error(
				`[KYC-ENGINE] Unexpected error during verification:`,
				error?.message || error,
			);
			return {
				success: false,
	...
	async executeProviderCall(
		...
		for (let attempt = 0; attempt <= maxRetries; attempt++) {
			if (attempt > 0) {
				...
				console.log(
					`[KYC-ENGINE] Retry attempt ${attempt}/${maxRetries} for provider=${context.provider.providerCode}, backoff=${backoffMs}ms`,
				);
			...
			try {
				...
				const latencyMs = Date.now() - callStart;
				console.log(
					`[KYC-ENGINE] Provider ${context.provider.providerCode} call failed (attempt ${attempt + 1}/${maxRetries + 1}): ${error?.message}`,
				);
				...
				if (attempt >= maxRetries) {
					return {
						...
						errorMessage:
							error?.message || "Provider call failed after retries",
						latencyMs,
						retryCount: attempt,
						fallbackChain: [],
					};
	...
	private async callProvider(
		...
		switch (providerCode) {
			...
			case "sandbox_pan": {
				...
				if (!pan || !name) {
					return {
						...
						errorMessage: "pan and name are required for sandbox_pan",
					};
				}
				try {
					const result = await sandboxKYCService.verifyPAN(
						pan,
						name,
						dob,
						p.reason || "KYC verification",
					);
					...
					return {
						...
						errorMessage:
							result.remarks || "PAN is invalid or not found in NSDL records",
					};
				} catch (err: any) {
					return {
						success: false,
			...
			case "cashfree_pan": {
				console.log(
					`[KYC-ENGINE] Calling Cashfree Secure ID PAN Lite verification`,
				);
				const pan = (p.pan || "").toUpperCase();
				const name = p.name || "";
				if (!pan || !name) {
					return {
						success: false,
						errorCode: "MISSING_PAYLOAD",
						errorMessage: "pan and name are required for cashfree_pan",
					};
				}
				if (!CashfreePANService.isConfigured()) {
					return {
						...
						errorMessage:
							"Cashfree Secure ID credentials not set — add CASHFREE_SECUREID_APP_ID / CASHFREE_SECUREID_SECRET_KEY",
					};
				}
				try {
			...
			case "truthscreen_pan": {
				console.log(
					`[KYC-ENGINE] Calling TruthScreen CKYC search for PAN verification`,
				);
				const pan = (p.pan || "").toUpperCase();
				const name = p.name || "";
				if (!pan) {
					return {
						success: false,
						errorCode: "MISSING_PAYLOAD",
						errorMessage: "pan is required for truthscreen_pan",
					};
				}
				...
				if (!tsAdapter.isConfigured()) {
					return {
						...
						errorCode: "PROVIDER_NOT_CONFIGURED",
						errorMessage:
							"TruthScreen credentials not set — add TRUTHSCREEN_USERNAME / TRUTHSCREEN_PASSWORD",
					};
				}
				try {
					...
					if (result.success && !result.found) {
						return {
							success: false,
							errorCode: "PAN_NOT_IN_CKYC",
							errorMessage:
								result.message ||
								"PAN not found in CKYC registry — try sandbox or Cashfree provider",
						};
					}
					return {
						success: false,
						errorCode: result.errorCode || "CKYC_SEARCH_FAILED",
						errorMessage: result.message || "TruthScreen CKYC search failed",
					};
				} catch (err: any) {
					return {
						success: false,
			...
			case "sandbox_aadhaar": {
				const subStep =
					p.subStep || (p.aadhaarNumber ? "generate_otp" : "verify_otp");
				if (subStep === "generate_otp") {
					...
					if (!p.aadhaarNumber) {
						return {
							...
							errorCode: "MISSING_PAYLOAD",
							errorMessage:
								"aadhaarNumber is required for aadhaar generate_otp",
						};
					}
					try {
					...
					if (!p.refId || !p.otp) {
						return {
							success: false,
							errorCode: "MISSING_PAYLOAD",
							errorMessage: "refId and otp are required for aadhaar verify_otp",
						};
					}
					try {
						const result = await sandboxKYCService.verifyAadhaarOTP(
							p.refId,
							p.otp,
						);
						return {
			...
			case "cashfree_aadhaar": {
				...
				if (subStep === "generate_otp") {
					console.log(
						`[KYC-ENGINE] Calling Cashfree Aadhaar OKYC OTP generation`,
					);
					if (!p.aadhaarNumber) {
						return {
							success: false,
					...
					try {
						const result = await CashfreeAadhaarService.generateOTP(
							p.aadhaarNumber,
						);
						if (result.success) {
							return {
					...
					console.log(
						`[KYC-ENGINE] Calling Cashfree Aadhaar OKYC OTP verification`,
					);
					if (!p.refId || !p.otp) {
						return {
							success: false,
					...
					try {
						const result = await CashfreeAadhaarService.verifyOTP(
							p.otp,
							p.refId,
						);
						if (result.verified) {
			...
			case "offline_aadhaar_xml": {
				console.log(
					`[KYC-ENGINE] offline_aadhaar_xml — requires file upload flow, not inline`,
				);
				return {
					success: false,
					errorCode: "MANUAL_UPLOAD_REQUIRED",
					errorMessage:
						"Aadhaar XML upload must be handled through the document upload flow, not the engine verify endpoint",
				};
			}
			...
			case "truthscreen_aadhaar": {
				...
				if (subStep === "generate_otp") {
					console.log(
						`[KYC-ENGINE] Calling TruthScreen Aadhaar OTP generation`,
					);
					if (!p.aadhaarNumber) {
						return {
							success: false,
					...
					try {
						const result = await TruthscreenAadhaarService.generateOTP(
							p.aadhaarNumber,
						);
						if (result.success) {
							return {
					...
					console.log(
						`[KYC-ENGINE] Calling TruthScreen Aadhaar OTP verification`,
					);
					if (!p.refId || !p.otp) {
						return {
							success: false,
					...
					try {
						const result = await TruthscreenAadhaarService.verifyOTP(
							p.refId,
							p.otp,
						);
						if (result.verified) {
			...
			case "sandbox_bank": {
				console.log(
					`[KYC-ENGINE] Calling Sandbox.co.in Bank penny-drop verification`,
				);
				...
				if (!accountNo || !ifsc) {
					return {
						success: false,
						errorCode: "MISSING_PAYLOAD",
						errorMessage: "accountNo and ifsc are required for sandbox_bank",
					};
				}
				try {
					const result = await sandboxKYCService.verifyBankAccountPennyDrop(
						accountNo,
						ifsc,
					);
					if (result.verified) {
					...
					return {
						...
						errorMessage: "Bank account could not be verified",
					};
				} catch (err: any) {
					return {
						success: false,
			...
			case "cashfree_bank": {
				console.log(
					`[KYC-ENGINE] Calling Cashfree VRS Bank Account V2 (sync) verification`,
				);
				...
				if (!bankAccount || !ifsc) {
					return {
						success: false,
						errorCode: "MISSING_PAYLOAD",
						errorMessage: "accountNo and ifsc are required for cashfree_bank",
					};
				}
				try {
			...
			case "truthscreen_ckyc": {
				...
				if (!pan) {
					return {
						success: false,
						errorCode: "MISSING_PAYLOAD",
						errorMessage: "pan is required for truthscreen_ckyc",
					};
				}
				const tsAdapter = new TruthScreenCkycAdapter();
				if (!tsAdapter.isConfigured()) {
				...
				try {
					...
					if (result.success && !result.found) {
						return {
							success: false,
							errorCode: "CKYC_NOT_FOUND",
							errorMessage:
								result.message || "No CKYC record found for this PAN",
						};
					}
					return {
						success: false,
			...
			case "authbridge_ckyc": {
				...
				return {
					...
					errorMessage: "AuthBridge CKYC integration not yet configured",
				};
			}
			...
			case "cersai_ckyc": {
				...
				return {
					success: false,
					errorCode: "PROVIDER_NOT_IMPLEMENTED",
					errorMessage: "CERSAI CKYC integration not yet configured",
				};
			}
			...
			case "vkyc_ckyc": {
				...
				return {
					success: false,
					errorCode: "PROVIDER_NOT_IMPLEMENTED",
					errorMessage: "Video KYC integration not yet configured",
				};
			}

			case "manual_ckyc": {
				console.log(
					`[KYC-ENGINE] manual_ckyc — marking as PENDING_MANUAL_REVIEW`,
				);
				return {
					success: true,
					data: {
			...
			case "sandbox_gstin": {
				...
				if (!gstin) {
					return {
						success: false,
						errorCode: "MISSING_PAYLOAD",
						errorMessage: "gstin is required for sandbox_gstin",
					};
				}
				try {
					...
					return {
						...
						errorMessage:
							result.message || "GSTIN is not valid or not registered",
					};
				} catch (err: any) {
					return {
						success: false,
			...
			case "sandbox_mca": {
				console.log(
					`[KYC-ENGINE] Calling Sandbox.co.in MCA company verification`,
				);
				...
				if (!cin && !companyName) {
					return {
						success: false,
						errorCode: "MISSING_PAYLOAD",
						errorMessage: "cin or companyName is required for sandbox_mca",
					};
				}
				try {
					const result = cin
			...
			default:
				console.error(
					`[KYC-ENGINE] Unknown provider code: ${providerCode} — no implementation registered`,
				);
				return {
					success: false,
					errorCode: "UNKNOWN_PROVIDER",
					errorMessage: `No implementation for provider code: ${providerCode}`,
				};
		}
	}
		...
		try {
			...
			if (existing.length > 0) {
				...
				const existingErrors =
					(metric.errorCodes as Record<string, number>) || {};
				if (errorCode) {
					existingErrors[errorCode] = (existingErrors[errorCode] || 0) + 1;
				}
				const newFallbacks =
					(metric.fallbacksTriggered ?? 0) + (errorCode ? 1 : 0);
			...
			if (provider) {
				...
				const newSuccessfulCalls =
					(provider.successfulCalls ?? 0) + (success ? 1 : 0);
				const newFailedCalls = (provider.failedCalls ?? 0) + (success ? 0 : 1);
				const newErrorRate =
					newTotalCalls > 0 ? newFailedCalls / newTotalCalls : 0;
				...
				await db
					...
					.set({
						...
						avgLatencyMs: newAvg,
						errorRate: Number.parseFloat(newErrorRate.toFixed(4)),
						updatedAt: new Date(),
					} as any)
					.where(eq(kycProviders.id, providerId));
			}
			...
			console.error(
				`[KYC-ENGINE] Failed to record provider metric:`,
				error?.message || error,
			);
		}
	}
	...
	async getProviderChainForStep(
		...
		Array<{
			provider: typeof kycProviders.$inferSelect;
			priority: typeof kycProviderPriority.$inferSelect;
		}>
	> {
		try {
			const priorities = await db
			...
			console.error(
				`[KYC-ENGINE] Failed to get provider chain:`,
				error?.message || error,
			);
			return [];
		}
	...
	async getFlowForProduct(
		productType: string,
	): Promise<Array<{ step: string; sequence: number }>> {
		try {
			...
			if (activeFlow?.steps) {
				const steps = activeFlow.steps as Array<{
					step: string;
					sequence: number;
				}>;
			...
			if (productConfig?.requiredKycSteps) {
			...
			console.log(
				`[KYC-ENGINE] No flow or product config found for productType=${productType}`,
			);
			return [];
		} catch (error: any) {
			console.error(
				`[KYC-ENGINE] Failed to get flow for product:`,
				error?.message || error,
			);
			return [];
		}
	...
	async updateProviderPriority(
		...
		try {
			...
			if (!existing) {
				console.log(
					`[KYC-ENGINE] No priority entry found for step=${kycStep}, providerId=${providerId}`,
				);
				return;
			}
			...
			console.log(
				`[KYC-ENGINE] Updated priority for step=${kycStep}, providerId=${providerId}: ${previousPriority} -> ${newPriority}`,
			);
			...
			console.error(
				`[KYC-ENGINE] Failed to update provider priority:`,
				error?.message || error,
			);
		}
	}
	...
	async logAuditEvent(
		...
		try {
			...
			console.error(
				`[KYC-ENGINE] Failed to log audit event:`,
				error?.message || error,
			);
		}
	}
	...
	async trackFunnelStep(
		...
		try {
			...
			console.log(
				`[KYC-ENGINE] Tracked funnel step: user=${userId}, funnel=${funnelType}, step=${currentStep}, seq=${stepSequence}`,
			);
		} catch (error: any) {
			console.error(
				`[KYC-ENGINE] Failed to track funnel step:`,
				error?.message || error,
			);
		}
	}
