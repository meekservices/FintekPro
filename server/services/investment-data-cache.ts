...
import type {
	InvestmentProduct,
	UnifiedProductType as ProductType,
	RiskLevel,
	LiquidityLevel as Liquidity,
...
class InvestmentDataCache {
	private productCache: Map<ProductType, CacheEntry<InvestmentProduct[]>> =
		new Map();
	...
	async initialize(): Promise<void> {
		...
		await Promise.allSettled(
			priorityTypes.map((type) => this.refreshProductType(type)),
		);
			...
			for (const [type, config] of Object.entries(PRODUCT_CACHE_CONFIG)) {
				...
				if (cacheAge >= config.ttlMs && !this.refreshLocks.get(productType)) {
					this.refreshProductType(productType).catch((err) => {
						console.warn(
							`[InvestmentDataCache] Background refresh failed for ${productType}: ${err?.message || err}`,
						);
					});
				}
			}
	...
	private async refreshProductType(type: ProductType): Promise<void> {
		...
		try {
			...
			this.metrics.errors++;
			console.warn(
				`[InvestmentDataCache] Refresh failed for ${type}: ${(error as any)?.message || error}`,
			);
			if (existingEntry) {
				existingEntry.isRefreshing = false;
			}
	...
	private async fetchREITs(): Promise<InvestmentProduct[]> {
		try {
			...
			return reitList.map((reit: any) => ({
				...
				lock_in_period: null,
				min_investment:
					Number.parseFloat((reit as any).currentPrice?.toString() || "0") ||
					300,
				regulatory_tags: ["SEBI_REGULATED", "EXCHANGE_TRADED"],
				source: "exchange",
				current_price: Number.parseFloat(
					(reit as any).currentPrice?.toString() || "0",
				),
				yield_or_return: Number.parseFloat(
					(reit as any).dividendYield?.toString() || "0",
				),
				sector: (reit as any).assetType || "Real Estate",
				raw_data: reit,
				last_updated:
					reit.lastUpdated?.toISOString() || new Date().toISOString(),
			})) as any[];
		} catch (error) {
			console.warn(
				`[InvestmentDataCache] Failed to fetch REITs: ${(error as any)?.message || error}`,
			);
			return [];
		}
	}
	...
	private async fetchInvITs(): Promise<InvestmentProduct[]> {
		try {
			...
			return invitList.map((invit: any) => ({
				...
				lock_in_period: null,
				min_investment:
					Number.parseFloat((invit as any).currentPrice?.toString() || "0") ||
					200,
				regulatory_tags: ["SEBI_REGULATED", "EXCHANGE_TRADED"],
				source: "exchange",
				current_price: Number.parseFloat(
					(invit as any).currentPrice?.toString() || "0",
				),
				yield_or_return: Number.parseFloat(
					(invit as any).distributionYield?.toString() || "0",
				),
				sector: (invit as any).assetType || "Infrastructure",
				raw_data: invit,
				last_updated:
					invit.lastUpdated?.toISOString() || new Date().toISOString(),
			})) as any[];
		} catch (error) {
			console.warn(
				`[InvestmentDataCache] Failed to fetch InvITs: ${(error as any)?.message || error}`,
			);
			return [];
		}
	}
	...
	private async fetchIPOs(): Promise<InvestmentProduct[]> {
		try {
			...
			return ipoList.map((ipo: any) => ({
				...
				lock_in_period: null,
				min_investment:
					Number.parseFloat((ipo as any).minLotPrice?.toString() || "0") ||
					15000,
				regulatory_tags: ["SEBI_REGULATED"],
				source: "exchange",
				current_price: Number.parseFloat(
					(ipo as any).priceRangeHigh?.toString() ||
						(ipo as any).issuePrice?.toString() ||
						"0",
				),
				yield_or_return: 0,
				...
				raw_data: ipo,
				last_updated:
					(ipo as any).updatedAt?.toISOString() || new Date().toISOString(),
			})) as any[];
		} catch (error) {
			console.warn(
				`[InvestmentDataCache] Failed to fetch IPOs: ${(error as any)?.message || error}`,
			);
			return [];
		}
	}
	...
	private async fetchUnlistedEquities(): Promise<InvestmentProduct[]> {
		try {
			...
			return unlistedList.map((company: any) => ({
				...
				min_investment:
					Number.parseFloat(
						(company as any).minimumInvestment?.toString() || "0",
					) || 100000,
				regulatory_tags: ["SEBI_ACCREDITED_INVESTOR"],
				source: "unlisted_marketplace",
				current_price: Number.parseFloat(
					(company as any).currentPrice?.toString() || "0",
				),
				yield_or_return: 0,
				sector: company.sector || "Various",
				raw_data: company,
				last_updated:
					(company as any).lastPriceUpdate?.toISOString() ||
					new Date().toISOString(),
			})) as any[];
		} catch (error) {
			console.warn(
				`[InvestmentDataCache] Failed to fetch Unlisted Equities: ${(error as any)?.message || error}`,
			);
			return [];
		}
	}
	...
	private async fetchAIFs(): Promise<InvestmentProduct[]> {
		try {
			...
			return aifList.map((aif: any) => ({
				...
				risk_level: (aif.riskScore && aif.riskScore >= 7
					? "aggressive"
					: aif.riskScore && aif.riskScore >= 4
						? "moderate"
						: "conservative") as RiskLevel,
				liquidity: "very_low" as Liquidity,
				investment_horizon: "very_long",
				expected_return_band: { min: 12, max: 25 },
				volatility_proxy: 35,
				tax_treatment: "aif",
				lock_in_period: aif.lockIn
					? Number.parseInt(aif.lockIn.toString()) * 365
					: 1095,
				min_investment:
					Number.parseFloat(aif.minInvestment?.toString() || "0") || 10000000,
				regulatory_tags: ["SEBI_REGULATED", "ACCREDITED_INVESTOR_ONLY"],
				source: "sebi_aif",
				current_price: Number.parseFloat(aif.latestNav?.toString() || "0"),
				yield_or_return: Number.parseFloat(aif.return1Y?.toString() || "0"),
			...
			console.warn(
				`[InvestmentDataCache] Failed to fetch AIFs: ${(error as any)?.message || error}`,
			);
			return [];
		}
	}
	...
	private normalizeStock(stock: any): InvestmentProduct {
		const price = Number.parseFloat(stock.currentPrice) || 0;
	...
	private normalizeMutualFund(fund: any): InvestmentProduct {
		...
		let riskLevel: RiskLevel = "moderate";
		if (category.includes("equity") || category.includes("aggressive"))
		...
		if (category.includes("liquid") || category.includes("overnight"))
			horizon = "short";
		else if (category.includes("elss") || category.includes("equity"))
			horizon = "long";
		...
		return {
			...
			volatility_proxy:
				riskLevel === "aggressive" ? 25 : riskLevel === "moderate" ? 15 : 8,
			...
			source: "amfi",
			current_price: nav,
			yield_or_return: Number.parseFloat(fund.returns1Y) || 0,
			sector: category,
			raw_data: fund,
	...
	private async fetchBonds(type: ProductType): Promise<InvestmentProduct[]> {
		...
		return bonds
			.slice(0, 50)
			.map((bond: any) => this.normalizeBond(bond, type));
	}

	private normalizeBond(bond: any, type: ProductType): InvestmentProduct {
		const price = Number.parseFloat(bond.faceValue) || 1000;
		const yieldValue = Number.parseFloat(bond.couponRate) || 7;
	...
	async getProducts(
		type: ProductType,
	): Promise<{ data: InvestmentProduct[]; cached: boolean; cacheAge: number }> {
		const now = Date.now();
		...
		if (entry) {
			...
			if (cacheAge < config.staleTtlMs) {
				...
				if (!this.refreshLocks.get(type)) {
					this.refreshProductType(type).catch((err) =>
						console.warn(
							`[InvestmentDataCache] Stale refresh failed for ${type}: ${err?.message || err}`,
						),
					);
				}
				return { data: entry.data, cached: true, cacheAge };
	...
	async getAllProducts(
		...
		stats: Record<ProductType, number>;
	}> {
		const targetTypes = types || (["STOCK", "MF", "BOND"] as ProductType[]);
		const results = await Promise.allSettled(
			targetTypes.map((type) => this.getProducts(type)),
		);

		const products: InvestmentProduct[] = [];
		const stats: Record<ProductType, number> = {} as Record<
			ProductType,
			number
		>;
	...
	getMetrics(): CacheMetrics & { hitRate: string; totalProducts: number } {
		...
		const totalProducts = Object.values(this.metrics.productCounts).reduce(
			(sum, count) => sum + count,
			0,
		);
