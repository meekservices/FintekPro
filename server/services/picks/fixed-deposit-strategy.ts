...
const SYNTHETIC_FD_POOL: Array<{
  ...
  { name: "SBI Fixed Deposit - 3 Year", issuer: "State Bank of India", interestRate: 6.80, tenureMonths: 36, riskLevel: 'low' },
  { name: "HDFC Bank FD - 2 Year", issuer: "HDFC Bank", interestRate: 7.00, tenureMonths: 24, riskLevel: 'low' },
  { name: "Shriram Finance FD - 1 Year", issuer: "Shriram Finance", interestRate: 8.85, tenureMonths: 12, riskLevel: 'medium' },
  { name: "Bajaj Finance FD - 18 Month", issuer: "Bajaj Finance", interestRate: 8.60, tenureMonths: 18, riskLevel: 'medium' },
  { name: "ICICI Bank FD - 1 Year", issuer: "ICICI Bank", interestRate: 6.90, tenureMonths: 12, riskLevel: 'low' },
];
...
export class FixedDepositStrategy extends BaseStrategy {
  ...
  async generate(context: StrategyContext): Promise<DailyPickData | null> {
    try {
      ...
      console.log(`[FixedDepositStrategy] No DB instruments found. Using synthetic FD: ${synth.name}`);

      const unitInvestment = 100_000; // ₹1 lakh as unit for price representation
      const yearFraction = synth.tenureMonths / 12;
      const maturityAmount = unitInvestment * (1 + (synth.interestRate / 100) * yearFraction);
      ...
      return {
        ...
        timeHorizon: synth.tenureMonths <= 12 ? 'short_term' : synth.tenureMonths <= 24 ? 'medium_term' : 'long_term',
        confidenceScore: 90,
        ...
        keyMetrics: {
          ...
          taxBenefit: synth.tenureMonths >= 60 ? '80C eligible' : 'Regular income',
  ...
  private async buildPick(context: StrategyContext, fd: FdCandidate): Promise<DailyPickData> {
    const interestRate = fd.interestRate ? parseFloat(fd.interestRate) : 7.5;
    const tenureStr = fd.tenure || '12';
    const tenureMonths = parseInt(tenureStr) || 12;

    // Use ₹1 lakh as the unit investment for price normalization
    const unitInvestment = 100_000;
    const yearFraction = tenureMonths / 12;
    const maturityAmount = unitInvestment * (1 + (interestRate / 100) * yearFraction);
    const targetPrice = Math.round(maturityAmount * 100) / 100;
    ...
    return {
      ...
      suitableFor: ['Conservative', 'Moderate'],
      timeHorizon: tenureMonths <= 12 ? 'short_term' : tenureMonths <= 24 ? 'medium_term' : 'long_term',
      confidenceScore: 90,
      sectorCategory: 'Fixed Income',
      keyMetrics: {
        interestRate,
  ...
  score(fd: FdCandidate): number {
    const rate = fd.interestRate ? parseFloat(fd.interestRate) : 0;
    if (rate >= 9) return 95;
    if (rate >= 8) return 85;
    if (rate >= 7) return 75;
    return 65;
  ...
  async getLivePrice(instrumentId: string): Promise<number | null> {
    try {
      ...
      const row = await db
        ...
        .limit(1);
      return row[0]?.lastPrice ? parseFloat(row[0].lastPrice) : 100_000;
    } catch {
      return 100_000;
    }
  }
