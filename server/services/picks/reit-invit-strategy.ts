...
function scoreREIT(reit: any): number {
  ...
  const yield_ = parseFloat(reit.distributionYield ?? reit.dividendYield ?? reit.yield ?? '0');
  if (yield_ > 9) score += 25;
  else if (yield_ > 7) score += 18;
  else if (yield_ > 5) score += 10;

  // Debt coverage ratio
  const dcr = parseFloat(reit.debtCoverageRatio ?? reit.dcr ?? '0');
  if (dcr > 2.0) score += 15;
  else if (dcr > 1.5) score += 10;
  else if (dcr > 1.2) score += 5;

  // Occupancy rate (REITs only — may be null for InvITs)
  const occ = parseFloat(reit.occupancyRate ?? '0');
  if (occ > 95) score += 15;
  ...
  if (sector.includes('commercial') || sector.includes('office') || sector.includes('industrial')) score += 5;
  else if (sector.includes('power') || sector.includes('road') || sector.includes('infra')) score += 4;
...
export class REITInvITStrategy extends BaseStrategy {
  ...
  async generate(context: StrategyContext): Promise<DailyPickData | null> {
    try {
      ...
      const distributionYield = parseFloat(top.distributionYield ?? '0');
      const debtCoverageRatio = parseFloat(top.debtCoverageRatio ?? '0');
      const occupancyRate = parseFloat(top.occupancyRate ?? '0');
      ...
      const rationale = await context.service.generateRationale({
        ...
        metrics: {
          type: top.type,
          sector: top.sector,
          distributionYield: distributionYield > 0 ? distributionYield : undefined,
          debtCoverageRatio: debtCoverageRatio > 0 ? debtCoverageRatio : undefined,
      ...
      const riskLevel = debtCoverageRatio > 1.8 ? 'low'
        : debtCoverageRatio > 1.3 ? 'medium'
        : 'high';
