...
function ratingToRiskLevel(rating: string | null | undefined): string {
  ...
  if (r.includes('AAA') || r.includes('SOV') || r.includes('G-SEC') || r.includes('GSEC')) return 'low';
...
function durationYears(maturityDate: string | null | undefined): number | null {
  ...
  try {
    ...
    const now = new Date();
    return (maturity.getTime() - now.getTime()) / (1000 * 60 * 60 * 24 * 365.25);
  } catch {
    return null;
  }
}
...
export class BondStrategy extends BaseStrategy {
  ...
  async generate(context: StrategyContext): Promise<DailyPickData | null> {
    try {
      ...
      const ytm = topBond.yieldToMaturity ? parseFloat(topBond.yieldToMaturity) : null;
      const coupon = topBond.couponRate ? parseFloat(topBond.couponRate) : null;
      ...
      const rationale = await context.service.generateRationale({
        ...
        metrics: {
          ...
          coupon,
          rating,
          maturityYears: maturityYears ? Math.round(maturityYears * 10) / 10 : undefined,
      ...
      return {
        ...
        confidenceScore: this.getConfidenceScore('bonds', scoredBonds[0].score, 60),
        ...
        keyMetrics: {
          ...
          maturityYears: maturityYears ? Math.round(maturityYears * 10) / 10 : null,
          issuerType: this.detectIssuerType(topBond.issuerName),
        },
      };
    } catch (error) {
  ...
  private detectIssuerType(name: string | null | undefined): 'sovereign' | 'psu' | 'nbfc' | 'corporate' {
    if (!name) return 'corporate';
    const lower = name.toLowerCase();
    if (lower.includes('government') || lower.includes('g-sec') || lower.includes('rbi') || lower.includes('india bond')) return 'sovereign';
    if (lower.includes('nhai') || lower.includes('ntpc') || lower.includes('power finance') || lower.includes('rec ') ||
        lower.includes('nabard') || lower.includes('irfc') || lower.includes('hudco') || lower.includes('pfc') ||
        lower.includes('bharat') || lower.includes('nhpc') || lower.includes('iifcl')) return 'psu';
    if (lower.includes('nbfc') || lower.includes('finance') || lower.includes('capital') || lower.includes('housing')) return 'nbfc';
    return 'corporate';
  }
