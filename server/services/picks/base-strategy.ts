...
export abstract class BaseStrategy implements IPickStrategy {
  ...
  protected getConfidenceScore(category: PickCategory, score: number, maxScore: number): number {
  ...
  protected deriveSuitableFor(riskLevel: string | null | undefined, category: PickCategory): string[] {
  ...
  protected filterRecentPicks<T extends { id?: string | number }>(
    ...
    if (filtered.length === 0) {
      console.log(`[PickStrategy:${this.category}] All candidates recently picked, allowing repeats`);
      return candidates;
    }
    return filtered;
  }
