...
export const COMMISSION_RATE_CONFIG: Record<
	LoanProductType,
...
class LoanCommissionService {
	...
	calculateUnifiedCommission(params: {
		...
		if (
			params.originationMode === OriginationMode.AGENT_ASSISTED &&
			params.agentRole
		) {
			...
			if (params.agentRole === "master_agent") {
				const bonus = Math.round(baseResult.agentAmount * 0.2 * 100) / 100;
				adjustedAgentAmount =
					Math.round((baseResult.agentAmount + bonus) * 100) / 100;
				adjustedFintekProAmount =
					Math.round((baseResult.fintekProAmount - bonus) * 100) / 100;
			}
		}
		...
		return {
			...
			commissionPolicyVersion:
				params.commissionPolicyVersion || CURRENT_COMMISSION_POLICY_VERSION,
		};
	}
	...
	calculateCommission(
		...
		const config =
			COMMISSION_RATE_CONFIG[productType] || COMMISSION_RATE_CONFIG.personal;
	...
	async createCommissionEntry(
		...
		try {
			...
			if (existing.length > 0) {
				console.warn(
					`[CommissionService] Commission entry already exists for application ${applicationId} (id: ${existing[0].id}) — skipping duplicate creation`,
				);
			...
			console.log(
				`[CommissionService] Created commission entry for application ${applicationId}`,
				{
			...
			console.error(
				"[CommissionService] Failed to create commission entry:",
				error,
			);
			return null;
		}
	...
	async onLoanSanctioned(
		...
		console.log(
			`[CommissionService] Processing sanction for application ${applicationId}`,
			{
				sanctionedAmount,
				bankCode,
				productType,
	...
	async onLoanDisbursed(
		...
		try {
			...
			if (existing.length === 0) {
				console.warn(
					`[CommissionService] No ledger entry found for application ${applicationId} — cannot update on disbursement`,
				);
				return;
			}
			...
			const customRate = entry.commissionRate
				? Number.parseFloat(entry.commissionRate)
				: undefined;
			...
			await db
				...
				.set({
					...
					disbursementDate,
					commissionableBase:
						calculation.breakdown.commissionableBase.toString(),
					grossCommission: calculation.grossCommission.toString(),
					tdsAmount: calculation.tdsAmount.toString(),
					gstAmount: calculation.gstAmount.toString(),
					netCommission: calculation.netCommission.toString(),
			...
			console.log(
				`[CommissionService] Recalculated commission on actual disbursement of ₹${disbursedAmount} for application ${applicationId}`,
				{
					grossCommission: calculation.grossCommission,
					netCommission: calculation.netCommission,
				},
			...
			console.error(
				"[CommissionService] Failed to update on disbursement:",
				error,
			);
		}
	}
