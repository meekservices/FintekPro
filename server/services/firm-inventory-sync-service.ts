...
export interface FirmTxInput {
  transactionType: 'buy' | 'sell' | 'transfer_in' | 'transfer_out' | 'dividend' | 'fee' | 'adjustment';
  securityName: string;
  isin?: string;
  companyId?: string;
  quantity: number;
...
class FirmInventorySyncService {
  ...
  static getInstance(): FirmInventorySyncService {
    if (!this.instance) this.instance = new FirmInventorySyncService();
    return this.instance;
  }

  // ── Holding CRUD ──────────────────────────────────────────────────────────

  async getOrCreateHolding(securityName: string, isin?: string, companyId?: string, securityType = 'unlisted_equity'): Promise<typeof firmDpHoldings.$inferSelect> {
    const where = isin
      ? and(eq(firmDpHoldings.partnerId, FIRM_PARTNER_ID), eq(firmDpHoldings.isin, isin))
      : and(eq(firmDpHoldings.partnerId, FIRM_PARTNER_ID), eq(firmDpHoldings.securityName, securityName));

    const [existing] = await db.select().from(firmDpHoldings).where(where).limit(1);
    if (existing) return existing;

    const [created] = await db.insert(firmDpHoldings).values({
  ...
  async updateHoldingPosition(holdingId: number, delta: number, pricePerShare?: number): Promise<void> {
    const [holding] = await db.select().from(firmDpHoldings).where(eq(firmDpHoldings.id, holdingId)).limit(1);
    if (!holding) return;

    const currentQty = parseFloat(holding.quantity ?? '0');
    const newQty = Math.max(0, currentQty + delta);

    let newAvgCost = holding.avgCostPrice ? parseFloat(holding.avgCostPrice) : 0;
    ...
    await db.update(firmDpHoldings).set({
  ...
  async getAllHoldings(): Promise<typeof firmDpHoldings.$inferSelect[]> {
    return db.select().from(firmDpHoldings)
  ...
  async recordTransaction(input: FirmTxInput): Promise<{ txId: number; syncResult: SyncResult }> {
    const holding = await this.getOrCreateHolding(input.securityName, input.isin, input.companyId);

    const netValue = (input.totalValue + (input.charges ?? 0)).toFixed(4);

    const [tx] = await db.insert(firmTransactions).values({
      partnerId: FIRM_PARTNER_ID,
    ...
    const qtyDelta = ['buy', 'transfer_in', 'dividend'].includes(input.transactionType)
      ? input.quantity
      : -input.quantity;
  ...
  async syncTransactionToZohoBooks(txId: number): Promise<SyncResult> {
    const [tx] = await db.select().from(firmTransactions).where(eq(firmTransactions.id, txId)).limit(1);
    if (!tx) return { success: false, transactionId: txId, error: 'Transaction not found' };
    if (tx.zohoStatus === 'synced') return { success: true, transactionId: txId, zohoRecordType: 'skipped' };

    const zoho = await getZohoBooksService();
    if (!zoho) {
      await this._markSyncStatus(txId, 'skipped', undefined, undefined, 'Zoho Books not configured');
      return { success: true, transactionId: txId, zohoRecordType: 'skipped', error: 'Zoho not configured' };
    }

    try {
      const date = tx.transactionDate;
      const qty = parseFloat(tx.quantity ?? '1');
      const rate = parseFloat(tx.pricePerShare ?? tx.netValue ?? '0');
      ...
      if (tx.transactionType === 'buy' || tx.transactionType === 'transfer_in') {
        ...
        const [holding] = tx.holdingId
          ? await db.select().from(firmDpHoldings).where(eq(firmDpHoldings.id, tx.holdingId)).limit(1)
          : [];
        ...
        await this._markSyncStatus(txId, 'synced', undefined, bill?.bill_id, null);
        return { success: true, transactionId: txId, zohoRecordId: bill?.bill_id, zohoRecordType: 'bill' };
        ...
        await this._markSyncStatus(txId, 'synced', invoice?.invoice_id, undefined, null);
        return { success: true, transactionId: txId, zohoRecordId: invoice?.invoice_id, zohoRecordType: 'invoice' };
      ...
      console.error('[FirmInventorySync] Zoho sync failed for tx', txId, errMsg);
  ...
  async processZohoBooksBillPaid(zohoEventId: string, bill: {
    bill_id: string;
    vendor_name?: string;
    ...
    const exists = await db.select({ id: firmTransactions.id })
    ...
    console.log(`[FirmInventorySync] Created FintekPro tx from Zoho Bill ${bill.bill_id}`);
  }

  async processZohoBooksInvoicePaid(zohoEventId: string, invoice: {
    invoice_id: string;
    ...
    console.log(`[FirmInventorySync] Created FintekPro tx from Zoho Invoice ${invoice.invoice_id}`);
  }
  ...
  async getZohoBooksBalance(): Promise<{
    ...
    const zoho = await getZohoBooksService();
    if (!zoho) {
      return { totalReceivables: 0, totalPayables: 0, netPosition: 0, lastRefreshed: new Date().toISOString() };
    }
  ...
  async retryPendingSync(limit = 20): Promise<{ retried: number; succeeded: number; failed: number }> {
  ...
  async getTransactions(page = 1, limit = 50): Promise<typeof firmTransactions.$inferSelect[]> {
    const offset = (page - 1) * limit;
    return db.select().from(firmTransactions)
  ...
  private async _ensureZohoItemLinked(
    ...
    try {
      const item = await zoho.createItem({
        ...
        item_type: 'inventory',
        product_type: 'goods',
        initial_stock: parseFloat(holding.quantity ?? '0'),
      ...
      console.warn('[FirmInventorySync] Could not create Zoho item for holding', holdingId, e);
    }
  }
}
