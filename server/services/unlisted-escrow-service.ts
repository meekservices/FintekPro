...
export class UnlistedEscrowService {
		...
		try {
			...
			try {
				const { unlistedValuationGovernanceService } = await import(
					"./unlisted-valuation-governance-service"
				);
				const latestVal =
					await unlistedValuationGovernanceService.getLatestValuation(
						deal.companyId,
					);

				if (latestVal) {
					fmvPrice = Number.parseFloat(latestVal.price.toString());
					const agreedPrice = Number.parseFloat(deal.agreedPrice);
					valuationDeviation =
						Math.abs((agreedPrice - fmvPrice) / fmvPrice) * 100;
					...
					if (valuationDeviation > 20) {
						...
						console.warn(
							`[Escrow] Valuation deviation alert for deal ${deal.id}: ${valuationDeviation.toFixed(2)}%`,
						);
					}
				}
			} catch (e) {
			...
			try {
				const { regulatoryComplianceService } = await import(
					"./unlisted-regulatory-compliance-service"
				);
				const limitCheck = await regulatoryComplianceService.checkInvestorLimit(
					deal.companyId,
					request.buyerUserId,
				);
				...
				if (!limitCheck.allowed) {
					return {
						success: false,
						message:
							limitCheck.reason || "Investor limit reached for this company.",
			...
			try {
				...
				if (
					regulatoryComplianceService.requiresSourceOfFundsVerification(
						totalValue,
					)
				) {
					// This should have been verified during KYC, but we double check the flag
					const investorRecord =
						await db.query.unlistedInvestorTracking.findFirst({
							where: and(
			...
			const returnUrl =
				request.returnUrl || `${getAppBaseUrl()}/api/unlisted/payment/callback`;
			...
			console.log(
				`💰 Escrow payment initiated for deal ${deal.id}, amount: ₹${fees.buyerTotal}`,
			);
		...
		try {
			...
			if (orderStatus.orderStatus === "PAID") {
				...
				return {
					...
					status: "escrowed",
					message:
						"Payment received and held in escrow. Awaiting share transfer.",
				};
			}
			if (
	...
	async releaseEscrow(
		...
		try {
			...
			const sellerPayout = Number.parseFloat(deal.sellerPayout || "0");
			const platformFee = Number.parseFloat(deal.platformFee || "0");
			...
			console.log(
				`✅ Escrow released for deal ${dealId}, seller payout: ₹${sellerPayout}`,
			);

			return {
				success: true,
				transactionId: `payout_${dealId}_${Date.now()}`,
				releasedAmount: Number.parseFloat(deal.totalValue),
				sellerPayout,
				platformFee,
				message:
					"Escrow released successfully. Funds will be transferred to seller.",
			};
		} catch (error: any) {
	...
	async refundEscrow(
		...
		try {
			...
			const refundAmount = Number.parseFloat(
			...
			console.log(
				`💸 Escrow refunded for deal ${dealId}, amount: ₹${refundAmount}`,
			);
	...
	async getEscrowStatus(dealId: string): Promise<EscrowStatus | null> {
		try {
			...
			switch (deal.status) {
				...
				case "failed":
				case "cancelled":
					escrowStatus = deal.cancellationReason?.includes("Refund")
						? "refunded"
						: "failed";
					break;
			}
			...
			return {
				...
				dealId: deal.id,
				status: escrowStatus,
				amount: Number.parseFloat(deal.totalValue),
				buyerPaymentStatus,
				sellerPayoutStatus,
	...
	async markTransferPending(
		...
		try {
			...
			if (deal.sellerUserId !== sellerUserId) {
				return {
					success: false,
					message: "Only the seller can mark transfer as pending",
				};
			}
			...
			if (deal.status !== "escrowed") {
				return {
					success: false,
					message: `Cannot update deal in ${deal.status} status`,
				};
			}
			...
			return {
				success: true,
				message:
					"Transfer marked as pending. Admin will verify and release escrow.",
			};
		} catch (error: any) {
