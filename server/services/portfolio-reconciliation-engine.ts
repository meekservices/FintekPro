...
class PortfolioReconciliationEngine {
  ...
  async reconcileClient(clientId: string): Promise<ReconciliationResult> {
    ...
    try {
      ...
      const livePositions = unifiedHoldings.status === 'fulfilled' ? unifiedHoldings.value : [];
      staleBrokers = brokerResult.status === 'fulfilled'
      ...
      for (const row of ledgerRows) {
        ...
        if (row.symbol) {
          if (!ledgerBySymbol.has(row.symbol)) ledgerBySymbol.set(row.symbol, []);
      ...
      for (const [isin, rows] of ledgerByIsin.entries()) {
        if (rows.length > 1) {
          discrepancies.push({
            ...
            ledgerValue: rows.reduce((s, r) => s + parseFloat(r.marketValue ?? '0'), 0),
            brokerValue: null,
            diffAbsolute: null,
            diffPercent: null,
      ...
      for (const row of ledgerRows) {
        const lastEnriched = row.lastEnrichedAt ? new Date(row.lastEnrichedAt) : null;
        if (!lastEnriched || lastEnriched < staleCutoff) {
          discrepancies.push({
            ...
            ledgerValue: parseFloat(row.currentPrice ?? '0'),
            brokerValue: null,
            diffAbsolute: null,
            diffPercent: null,
      ...
      for (const [isin, rows] of ledgerByIsin.entries()) {
        ...
        const livePos = liveByIsin.get(isin) ?? (ledgerRow.symbol ? liveBySymbol.get(ledgerRow.symbol) : undefined);

        const ledgerQty = parseFloat(ledgerRow.quantity ?? ledgerRow.units ?? '0');
        const ledgerValue = parseFloat(ledgerRow.marketValue ?? '0');
        ...
        if (!livePos) {
          ...
          const manualSources = ['manual', 'epf', 'ppf', 'nps', 'government_portal'];
          if (!manualSources.includes(ledgerRow.dataSource?.toLowerCase() ?? '')) {
            discrepancies.push({
              clientId,
        ...
        if (ledgerQty > 0 && Math.abs(ledgerQty - liveQty) / ledgerQty > 0.001) {
          ...
          discrepancies.push({
            ...
            isin,
            assetType: ledgerRow.assetType,
            source: (livePos as any).broker ?? (livePos as any).dataSource ?? 'LIVE',
        ...
        if (ledgerValue > 100 && liveValue > 0) {
          const valueDiffPct = Math.abs((liveValue - ledgerValue) / ledgerValue) * 100;
      ...
      const result: ReconciliationResult = {
        ...
        status: staleBrokers.length === livePositions.length + 1 ? 'partial' : 'success',
        discrepancies,
        summary,
        staleBrokers,
        checksum,
  ...
  async reconcileAllClients(): Promise<{
    ...
    `);
    const clientIds: string[] = ((clientRows as any).rows ?? clientRows).map((r: any) => r.user_id).filter(Boolean);

    let succeeded = 0, failed = 0, totalDiscrepancies = 0, criticalDiscrepancies = 0;
    ...
    for (const clientId of clientIds) {
      try {
        ...
        logger.warn(`[ReconEngine] Client ${clientId} failed`, { message: e?.message });
        failed++;
      }
  ...
  private generateChecksum(clientId: string, discrepancies: HoldingDiscrepancy[], runAt: string): string {
    const data = JSON.stringify({ clientId, runAt, count: discrepancies.length });
   ...
   */
  private async persistReconciliationLog(result: ReconciliationResult): Promise<void> {
    try {
...
export const portfolioReconciliationEngine = new PortfolioReconciliationEngine();
