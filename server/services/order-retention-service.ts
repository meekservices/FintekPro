...
class OrderRetentionService {
	...
	async archiveOrder(orderId: string): Promise<ArchivedOrder> {
		const [order] = await db
			.select()
			.from(bondOrders)
			.where(eq(bondOrders.orderNumber, orderId));
		...
		const orderData: OrderData = {
			...
			price: Number.parseFloat(order.orderPrice || "0"),
			netAmount: Number.parseFloat(order.netAmount || "0"),
	...
	async archiveOldOrders(olderThanDays: number = 90): Promise<ArchiveReport> {
		...
		console.log(
			`[Order Retention] Archive report: ${ordersArchived}/${ordersProcessed} orders archived`,
		);
		return report;
	}
	...
	async retrieveArchivedOrder(orderId: string): Promise<ArchivedOrder | null> {
		...
		const currentChecksum = await this.generateChecksum(
			JSON.stringify(archived.orderData),
		);
	 ...
	 */
	async processRegulatoryRequest(
		request: Omit<
			RegulatoryRequest,
			"status" | "retrievedRecords" | "responseDate"
		>,
	): Promise<RegulatoryRequest> {
		const requestId =
			request.requestId ||
			`REQ-${request.requestType.toUpperCase()}-${nanoid(8)}`;
		...
		try {
			if (request.orderIds && request.orderIds.length > 0) {
				...
				const archivedRecords = await db
					.select()
					.from(complianceAuditTrail)
					.where(
						and(
			...
			console.error(
				`[Order Retention] Regulatory request failed: ${error.message}`,
			);
			return {
				...request,
				requestId,
	...
	async cleanupExpiredArchives(): Promise<{
		...
		try {
			...
			console.log(
				`[Order Retention] Found ${expiredRecords.length} expired archives for cleanup`,
			);
			...
			for (const record of expiredRecords) {
				try {
					...
					errors.push(
						`Failed to mark archive ${record.id} as expired: ${error.message}`,
					);
				}
			}
		} catch (error: any) {
	...
	async getRetentionStatistics(): Promise<{
		...
		for (const archive of archives) {
			...
			if (
				...
				new Date(data.retentionExpiry) <= thirtyDaysFromNow
			) {
		...
		return {
			...
			totalStorageBytes,
			oldestArchive:
				archives.length > 0 ? archives[archives.length - 1].createdAt : null,
	...
	async verifyArchiveIntegrity(archiveId: string): Promise<{
		...
		const encryptionValid = true; // Assume valid unless proven otherwise
		const storageAccessible = true; // Assume accessible
		...
		try {
			...
			const archived = results[0].newValue
				? JSON.parse(results[0].newValue)
				: null;
