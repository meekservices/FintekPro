...
function getEncryptionKey(): Buffer {
	...
	if (!key) {
		const isProduction =
			...
			process.env.KYC_ENVIRONMENT === "production";
		if (isProduction) {
			throw new Error(
				"FATAL: KYC_ENCRYPTION_KEY or SESSION_SECRET must be set in production. Aadhaar/PAN encryption requires a secure key.",
			);
		}
		console.warn(
			"⚠️ KYC Encryption: Using dev-only fallback key. Set KYC_ENCRYPTION_KEY for production.",
		);
		return crypto.scryptSync(
			"fintekpro-dev-only-key-not-for-prod",
			"fintekpro-kyc-salt-v2",
			32,
		);
	}
...
class KycEncryptionService {
	...
	decryptAadhaar(encryptedData: string): string {
		try {
			...
			throw new Error(
				"Failed to decrypt Aadhaar data - key mismatch or corrupted data",
			);
		}
	}
