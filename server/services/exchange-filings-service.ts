...
class ExchangeFilingsService {
	...
	private async checkRateLimit(): Promise<void> {
		...
		if (this.requestCount >= RATE_LIMIT_PER_MINUTE) {
			...
			console.log(
				`[ExchangeFilings] Rate limit reached, waiting ${waitTime}ms`,
			);
		...
		for (let attempt = 0; attempt <= retries; attempt++) {
			try {
				...
				if (error.response?.status === 429 || error.response?.status >= 500) {
					...
					console.log(
						`[ExchangeFilings] Error ${error.response?.status}, retrying in ${delay}ms`,
					);
	...
	async fetchNSEFilings(
		...
		try {
			...
			console.log(
				`[NSE] Fetching filings from ${params.from_date} to ${params.to_date}`,
			);
			...
			if (error.response?.status === 403) {
				result.errors.push(
					"NSE may require browser session cookies. Using fallback method.",
				);
			}
		}
	...
	async fetchBSEFilings(
		...
		try {
			...
			console.log(
				`[BSE] Fetching filings from ${params.strPrevDate} to ${params.strToDate}`,
			);
	...
	private parseNSEFiling(item: any): ExchangeFilingMetadata | null {
		...
		return {
			...
			financialType:
				item.consolidated?.toLowerCase() === "yes" || item.cons === "Y"
					? "CONSOLIDATED"
					: "STANDALONE",
			documentUrl: documentUrl.startsWith("http")
				? documentUrl
				: `${NSE_BASE_URL}${documentUrl}`,
	...
	async persistFilings(filings: ExchangeFilingMetadata[]): Promise<{
        ...
        `);
				...
				result.errors.push(
					`Failed to insert filing ${filing.symbol}: ${error.message}`,
				);
			}
		}

		console.log(
			`[ExchangeFilings] Persisted: ${result.inserted} inserted, ${result.duplicates} duplicates`,
		);
		return result;
	}

	async linkFilingToCompany(
		filingId: string,
		companyId: string,
	): Promise<boolean> {
		const { db } = await import("../db");
      ...
      `);
			...
			console.error(
				`[ExchangeFilings] Failed to link filing ${filingId}: ${error.message}`,
			);
			return false;
		}
	}
    ...
    `);
		...
		return {
			...
			completed: Number.parseInt(row.completed) || 0,
			failed: Number.parseInt(row.failed) || 0,
			byExchange: {
				nse: Number.parseInt(row.nse) || 0,
				bse: Number.parseInt(row.bse) || 0,
			},
		};
	}
