...
class UserSignatureESignService {
	...
	private async embedSignatureInPdf(
		...
		const targetPage =
			position?.page && position.page <= pages.length
	...
	async signDocument(
		request: UserSignatureSignRequest,
	): Promise<UserSignatureSignResponse> {
		const transactionId = `USIG-${nanoid(12)}`;
		const signedAt = new Date();

		try {
			let signatureDataUrl = request.signatureDataUrl;
			const signatureId = request.signatureId || "";
			let signatureName = "Direct Signature";

			if (!signatureDataUrl) {
				const userSig = await this.getUserSignature(
					request.userId,
					request.signatureId,
				);
				if (!userSig) {
			...
			const userIdNum =
				typeof request.userId === "string"
					? Number.parseInt(request.userId, 10)
					: request.userId;
			...
			console.log(
				`[UserSignatureESign] Document signed successfully - Transaction: ${transactionId}`,
			);
	...
	async validateSignature(signatureDataUrl: string): Promise<{
		...
		try {
			if (!signatureDataUrl || !signatureDataUrl.startsWith("data:image/")) {
				return {
					valid: false,
					message: "Invalid signature format. Must be a base64 data URL.",
				};
			}
			...
			if (buffer.length < 100) {
				return {
					valid: false,
					message: "Signature image is too small or empty.",
				};
			}
			...
			if (buffer.length > 500000) {
				return {
					valid: false,
					message: "Signature image is too large. Maximum 500KB allowed.",
				};
			}
