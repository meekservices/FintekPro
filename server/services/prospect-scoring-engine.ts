			...
			if (dirResp.success && Array.isArray(dirResp.data)) {
				for (const dir of dirResp.data.filter((d) => d.is_active)) {
					...
					for (const assocCin of associatedCins.slice(0, 5)) {
						try {
							const enriched =
								await credhiveService.enrichDirectorCompanyData(assocCin);
							if (enriched?.revenue) {
								companies.push({
...
export function scoreProspect(data: ProspectScoreInput): ProspectScoreResult {
	...
	if (data.openChargesCount && data.openChargesCount > 5)
		financialHealthScore -= 10;
	if (data.activeLegalCases && data.activeLegalCases > 0)
	...
	const investableSurplus = Number.parseFloat(
		(data.networth * 0.15).toFixed(2),
	); // Upgrade 3

	return {
		wealthScore: Number.parseFloat(wealthScore.toFixed(2)),
		activityScore: Number.parseFloat(activityScore.toFixed(2)),
		relationshipScore: Number.parseFloat(relationshipScore.toFixed(2)),
		financialHealthScore: Number.parseFloat(financialHealthScore.toFixed(2)),
		compositeScore,
		estimatedNetworth: Number.parseFloat(data.networth.toFixed(2)),
		investableSurplus,
...
export async function enrichAndScoreProspect(
	...
	const fallbackRevenue = lead.annualRevenue
		...
		try {
			...
			if (profileResult.success && profileResult.data) {
				...
				if (rawAddr && typeof rawAddr === "object") {
					...
					if (!lead.pincode && rawAddr.pincode)
						addrUpdates.pincode = rawAddr.pincode;
					if (!lead.address) {
						const parts = [
	...
	const estimatedNetworth = calculateWealthFromDirectorships(
		directorCompanies,
		fallbackRevenue,
	);
...
export async function bulkScoreProspects(
	...
	const whereClause = staleThreshold
		? or(
				isNull(prospectLeads.scoredAt),
				ltDrizzle(prospectLeads.scoredAt, staleThreshold),
			)
	...
	for (const lead of leads) {
		try {
			await enrichAndScoreProspect(lead.id, {
				relationshipStrength,
				triggeredBy,
			});
			succeeded++;
...
let _benchmarkCache: { data: SectorBenchmark[]; expiresAt: number } | null =
	null;
...
export async function getSectorBenchmarks(): Promise<SectorBenchmark[]> {
	...
	const data = rows
		...
		.map((r) => ({
			...
			avgCompositeScore: Number.parseFloat(String(r.avgComposite || "0")),
			avgWealthScore: Number.parseFloat(String(r.avgWealth || "0")),
			avgActivityScore: Number.parseFloat(String(r.avgActivity || "0")),
			avgRelationshipScore: Number.parseFloat(String(r.avgRelationship || "0")),
...
export async function getBenchmarkForSegment(
	segment: string,
