...
export const ASSET_CLASSES: AssetClass[] = [
	...
	{
		type: "money_market",
		name: "Money Market/Liquid Funds",
		expectedReturn: 5.5,
		volatility: 1,
		minAllocation: 0,
		maxAllocation: 30,
...
const CORRELATION_MATRIX: CorrelationMatrix = {
	...
	mid_cap_equity: {
		large_cap_equity: 0.85,
		mid_cap_equity: 1.0,
		small_cap_equity: 0.9,
		international_equity: 0.65,
	...
	international_equity: {
		large_cap_equity: 0.7,
		mid_cap_equity: 0.65,
		small_cap_equity: 0.6,
	...
	government_bonds: {
		large_cap_equity: -0.2,
		mid_cap_equity: -0.15,
		small_cap_equity: -0.1,
	...
	corporate_bonds: {
		large_cap_equity: 0.1,
		mid_cap_equity: 0.15,
		small_cap_equity: 0.2,
	...
	real_estate: {
		...
		mid_cap_equity: 0.55,
		small_cap_equity: 0.45,
		international_equity: 0.4,
		government_bonds: 0.1,
...
class AssetAllocationOptimizer {
	...
	getConstraints(riskScore: number, segment: string): AllocationConstraints {
		...
		if (segment === "bhni" || segment === "shni") {
			constraints.maxAlternatives = Math.min(
				constraints.maxAlternatives + 10,
				35,
			);
			...
			constraints.maxEquity = Math.max(
				constraints.maxEquity - 15,
				constraints.minEquity,
			);
		}
	...
	private calculatePortfolioVariance(weights: number[]): number {
		...
		for (let i = 0; i < n; i++) {
			...
			for (let j = 0; j < n; j++) {
				...
				const correlation =
					CORRELATION_MATRIX[type1]?.[type2] ?? (type1 === type2 ? 1 : 0);
	...
	private projectToConstraints(
		...
		for (let iter = 0; iter < 50; iter++) {
			let changed = false;
			const equitySum = this.getGroupSum(projected, EQUITY_TYPES);
		...
		for (let iter = 0; iter < maxIter; iter++) {
			...
			for (let i = 0; i < n; i++) {
				...
				for (let j = 0; j < n; j++) {
					const asset2 = ASSET_CLASSES[j];
					const correlation =
						CORRELATION_MATRIX[asset.type]?.[asset2.type] ?? 0;
					varianceGrad +=
						(weights[j] / 100) *
						(asset.volatility / 100) *
						(asset2.volatility / 100) *
						correlation;
				}
				varianceGrad *= 2;
		...
		for (let trial = 0; trial < 20; trial++) {
			...
			for (let iter = 0; iter < 200; iter++) {
				...
				for (let i = 0; i < n; i++) {
					...
					const dSharpeVol =
						-(portfolioReturn - this.riskFreeRate) /
						(volatility * volatility + 1e-8);
					const volGrad =
						(varianceGrad / (2 * Math.sqrt(variance) + 1e-8)) * 100;
	...
	generateEfficientFrontier(
		...
		const minVolWeights = this.optimizeForTargetVolatility(
			2,
			bounds,
			constraints,
		);
		...
		const minVol =
			Math.sqrt(this.calculatePortfolioVariance(minVolWeights)) * 100;
		const maxVol =
			Math.sqrt(this.calculatePortfolioVariance(maxSharpeWeights)) * 100;
		...
		for (let i = 0; i < points; i++) {
			...
			const weights = this.optimizeForTargetVolatility(
				targetVol,
				bounds,
				constraints,
			);
	...
	optimize(input: OptimizationInput): OptimizationResult {
		...
		const maxDrawdownEstimate = this.estimateMaxDrawdown(
			volatility,
			investmentHorizon,
		);
		...
		for (let i = 0; i < weights.length; i++) {
			if (weights[i] > 0.01) {
				...
				allocationResults.push({
					...
					contributionToRisk:
						Math.round((contribution / volatility) * 100 * 100) / 100,
					amount: investableAmount
						? Math.round((investableAmount * weights[i]) / 100)
						: undefined,
				});
			}
		}
	...
	private generateRationale(
		...
		rationale.push(
			`Based on your risk score of ${input.riskScore}/100, you fall into the "${profile.replace(/_/g, " ")}" risk category.`,
		);
		rationale.push(
			`Mean-variance optimization produced a portfolio with ${Math.round(equityAllocation)}% equity and ${Math.round(debtAllocation)}% debt.`,
		);
		...
		if (input.goalType) {
			const goalDescriptions: { [key: string]: string } = {
				...
				preservation:
					"capital preservation with emphasis on low-volatility assets",
				balanced: "a balance between growth and income",
			};
			rationale.push(
				`The allocation is optimized for ${goalDescriptions[input.goalType]}.`,
			);
		}

		rationale.push(
			`Expected portfolio return: ${metrics.expectedReturn.toFixed(1)}% with volatility of ${metrics.volatility.toFixed(1)}%.`,
		);
		rationale.push(
			`Risk-adjusted return (Sharpe ratio): ${metrics.sharpeRatio.toFixed(2)}.`,
		);

		if (input.segment === "retail") {
			rationale.push(
				`As a retail investor, complex alternatives are excluded to ensure liquidity and accessibility.`,
			);
		} else if (input.segment === "bhni" || input.segment === "shni") {
			rationale.push(
				`Your HNI status enables access to alternative investments for enhanced diversification.`,
			);
		}
	...
	getAvailableAssetClasses(segment: string): AssetClass[] {
		return ASSET_CLASSES.map((asset) => {
			...
			if (segment === "retail") {
				...
				if (asset.type === "real_estate") {
					adjustedAsset.maxAllocation = Math.min(
						adjustedAsset.maxAllocation,
						5,
					);
				}
			}
