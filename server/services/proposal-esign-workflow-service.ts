...
import {
	...
	prospectProposals,
	users,
	InsertProposalEsignWorkflow,
	InsertProposalEsignVersion,
	InsertProposalEsignParticipant,
	InsertProposalEsignComment,
	InsertProposalEsignFieldEdit,
	InsertProposalEsignAuditLog,
	ProposalEsignWorkflow,
	ProposalEsignParticipant,
	ProposalEsignVersion,
} from "@shared/schema";
...
class ProposalEsignWorkflowService {
	...
	): Promise<ProposalEsignParticipant> {
		const [participant] = await db
			.insert(proposalEsignParticipants)
			.values({
	...
	async getWorkflowByProposal(
		proposalId: string,
	): Promise<ProposalEsignWorkflow | null> {
		const [workflow] = await db
			.select()
			.from(proposalEsignWorkflows)
	...
	async createVersion(
		...
		const [version] = await db
			.insert(proposalEsignVersions)
			.values({
				workflowId: options.workflowId,
				versionNumber: newVersionNumber,
				negotiationRound: workflow.negotiationRound,
				versionLabel:
					options.versionLabel || `Revision ${newVersionNumber - 1}`,
				documentUrl: options.documentUrl,
				documentHash: options.documentHash,
	...
	async recordFieldEdit(
		...
		const workflow = await this.getWorkflow(workflowId);
		if (!workflow) throw new Error("Workflow not found");
		if (!workflow.allowEditing)
			throw new Error("Editing is not allowed for this document");
	...
	async approveFieldEdit(editId: string, approvedBy: string): Promise<void> {
		...
		if (edit.approvalStatus !== "pending")
			throw new Error("Edit is not pending approval");

		await db
			.update(proposalEsignFieldEdits)
			.set({
	...
	async initiateSignature(
		...
		if (!participant.canSign)
			throw new Error("Participant is not authorized to sign");

		if (!workflow.editingLockedAt) {
			await this.lockDocumentForSigning(
				workflowId,
		...
		if (method === "zoho_sign") {
			try {
				...
				result = {
					message: `Zoho Sign error: ${error instanceof Error ? error.message : "Unknown error"}`,
				};
			}
			...
			result = {
				transactionId,
				message:
					"Aadhaar eSign initiated - OTP will be sent to registered mobile",
			};
		} else if (method === "dsc_token") {
			result = {
				message:
					"DSC Token signing initiated - please connect your hardware token",
			};
		} else {
			result = {
	...
	async getWorkflowsForUser(userId: string, role?: string) {
		...
		const participations = await db
			...
			.innerJoin(
				proposalEsignWorkflows,
				eq(proposalEsignParticipants.workflowId, proposalEsignWorkflows.id),
			)
