...
export class KycEngine {
	...
	async processKyc(userId: string, pan: string, dob: string) {
		...
		try {
			...
			const ckycResponse = await ckycProviderAdapter.searchCkycStatus({
				pan,
				dob,
			});
			...
			if (ckycResponse.success && ckycResponse.data) {
				...
				logger.info(
					`[KycEngine] CKYC failed, initiating Digilocker fallback for user ${userId}`,
				);
			...
			logger.error(`[KycEngine] KYC orchestration failed for user ${userId}`, {
				error: error.message,
			});
			throw error;
		}
