// @ts-nocheck
import { db } from '../db';
import { kycApprovals, kycAuditLogs, MAKER_CHECKER_ENTITY_TYPES } from '@shared/schema';
...
class KycMakerCheckerService {
  ...
  requiresMakerChecker(entityType: string | null): boolean {
    ...
    return (MAKER_CHECKER_ENTITY_TYPES as readonly string[]).includes(entityType.toUpperCase());
  }
  ...
  async submit(params: SubmitForApprovalParams): Promise<{
    ...
    try {
      if (!this.requiresMakerChecker(params.entityType)) {
        return { success: false, error: `Entity type ${params.entityType} does not require maker-checker approval` };
      }
      ...
      const [approval] = await db.insert(kycApprovals).values({
  ...
  async approve(params: ApproveRejectParams): Promise<{
    ...
    try {
      ...
      if (approval.makerId === params.checkerId) {
        return { success: false, error: 'Maker and Checker cannot be the same person (segregation of duties)' };
      }

      await db.update(kycApprovals)
        .set({
  ...
  async reject(params: ApproveRejectParams): Promise<{
    ...
    try {
      ...
      if (approval.makerId === params.checkerId) {
        return { success: false, error: 'Maker and Checker cannot be the same person' };
      }
