...
export class ProspectReadinessService {
	...
	static getInstance(): ProspectReadinessService {
		if (!ProspectReadinessService.instance) {
			ProspectReadinessService.instance = new ProspectReadinessService();
		}
		return ProspectReadinessService.instance;
	}
	...
	async checkReadiness(prospectId: string): Promise<ReadinessCheckResult> {
		...
		const hasHoldings =
			...
			(prospect.currentPortfolio as any[]).length > 0;
		...
		if (currentOrder >= 1 || hasHoldings)
			completedSteps.push("Holdings Imported");
	...
	async advanceOnHoldingsImport(prospectId: string): Promise<ReadinessStatus> {
		...
		if (currentStatus === "INITIAL") {
			return this.transitionStatus(
				...
				"INITIAL",
				"HOLDINGS_IMPORTED",
				"Holdings imported from CAS/manual entry",
			);
		}
	...
	async advanceOnRiskProfileComplete(
		prospectId: string,
		...
		if (currentStatus === "INITIAL") {
			return this.transitionStatus(
				...
				"INITIAL",
				"RISK_PROFILE_COMPLETED",
				"Risk questionnaire completed (holdings pending)",
			);
		}
	...
	async advanceOnTaxProfileComplete(
		prospectId: string,
	): Promise<ReadinessStatus> {
		const currentStatus = await this.getCurrentStatus(prospectId);
	...
	async evaluateAndAdvanceToReady(
		prospectId: string,
	...
	private async transitionStatus(
		...
		console.log(
			`[ProspectReadiness] ${prospectId}: ${fromStatus} -> ${toStatus} | ${reason}`,
		);
