import { Configuration, PlaidApi, PlaidEnvironments, LinkTokenCreateRequest, ProcessorTokenCreateRequest } from 'plaid';
...
export class PlaidService {
  ...
  async createLinkToken(userId: string) {
    ...
    try {
      ...
      logger.error('[PlaidService] Error creating link token', error.response?.data || error.message);
  ...
  async createProcessorToken(publicToken: string, accountId: string) {
    logger.info(`[PlaidService] Exchanging public token for processor token (Account: ${accountId})`);
    ...
    try {
      ...
      logger.error('[PlaidService] Error creating processor token', error.response?.data || error.message);
