...
export class CommissionDisputeService {
	...
	static getInstance(): CommissionDisputeService {
		if (!CommissionDisputeService.instance) {
			CommissionDisputeService.instance = new CommissionDisputeService();
		}
		return CommissionDisputeService.instance;
	}
	...
	async createDispute(data: {
		...
		const openDisputes = existing.filter(
		...
		if (ledgerEntries.length === 0) {
			return {
				success: false,
				error:
					"No commission entries found for this partner on this transaction",
			};
		}
	...
	async processReversal(data: {
		...
		if (ledgerEntries.length === 0) {
			return {
				success: false,
				error: "No commission entries found for reversal",
			};
		}
		...
		const reversedLedgerIds = new Set(
			existingReversals.map((r) => r.originalLedgerId),
		);

		const toReverse = ledgerEntries.filter(
			(e) => !reversedLedgerIds.has(e.ledgerId),
		);
		if (toReverse.length === 0) {
			return {
				success: false,
				error: "All entries for this transaction are already reversed",
			};
		}
		...
		try {
			const reversals = await db.transaction(async (tx) => {
				...
				for (const entry of toReverse) {
					const amount = Number.parseFloat(entry.amount?.toString() || "0");
				...
				await tx.insert(partnerAuditLogs).values({
					...
					metadata: {
						...
						totalReversed: toReverse.reduce(
							(sum, e) => sum + Number.parseFloat(e.amount?.toString() || "0"),
							0,
						),
					},
				});
	...
	async getDisputes(
		...
		if (filters.partnerId)
			conditions.push(eq(disputeCases.raisedByPartnerId, filters.partnerId));
		if (filters.status)
			conditions.push(eq(disputeCases.status, filters.status));
		if (filters.transactionId)
			conditions.push(eq(disputeCases.transactionId, filters.transactionId));
