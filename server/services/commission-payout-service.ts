...
export class CommissionPayoutService {
	...
	static getInstance(): CommissionPayoutService {
		if (!CommissionPayoutService.instance) {
			CommissionPayoutService.instance = new CommissionPayoutService();
		}
		return CommissionPayoutService.instance;
	}
	...
	async processTransaction(data: {
		...
		if (!config) {
			return {
				success: false,
				error: `No active commission config for product type: ${data.productType}`,
			};
		}

		const agentPct = Number.parseFloat(config.agentPct?.toString() || "0");
		const platformPct = Number.parseFloat(
			config.platformPct?.toString() || "0",
		);

		if (agentPct + platformPct >= 100) {
			return {
				success: false,
				error: "agent_pct + platform_pct must be < 100",
			};
		}

		const total = data.grossCommission;
		const uplineIncentivePct = Number.parseFloat(
			config.uplineIncentivePct?.toString() || "0",
		);
		const minResidualThreshold = Number.parseFloat(
			config.minResidualThreshold?.toString() || "0",
		);
        ...
        `);
				const allPartners =
					(partnerChainResult as any).rows || partnerChainResult || [];
				...
				const [agentEntry] = await tx
					.insert(progressiveCommissionLedger)
					.values({
						transactionId: data.transactionId,
				...
				for (const upline of partnerChain) {
					if (remaining < minResidualThreshold || level > MAX_UPLINE_LEVELS)
						break;
				...
				if (
					sellerPartner &&
					(sellerPartner.kyc_status === "VERIFIED" ||
						sellerPartner.kycStatus === "VERIFIED") &&
					(sellerPartner.hierarchy_status === "ACTIVE" ||
						sellerPartner.hierarchyStatus === "ACTIVE" ||
						sellerPartner.is_active === true ||
						sellerPartner.isActive === true)
				) {
			...
			if (e.message === "IDEMPOTENCY_DUPLICATE") {
				return {
					success: false,
					error: "Transaction already processed (idempotency guard)",
				};
			}
			throw e;
		}
	...
	private checkEligibility(partner: any): boolean {
		...
		const isActive =
			partner.hierarchy_status === "ACTIVE" ||
			partner.hierarchyStatus === "ACTIVE" ||
	...
	private validateAntiMLM(transactionType: string): {
		valid: boolean;
		error?: string;
	} {
		const forbidden = [
	...
	async createConfig(data: {
		...
		if (data.agentPct + data.platformPct >= 100) {
			return {
				success: false,
				error: "agent_pct + platform_pct must be less than 100",
			};
		}
