...
export class NPSService {
  ...
  async fetchNPSAccounts(request: NPSFetchRequest): Promise<NPSFetchResponse> {
    try {
      ...
      return {
        ...
        totalReturns: 0,
        returnsPercentage: 0,
        message: error instanceof Error ? error.message : 'Failed to fetch NPS data',
        fetchedAt: new Date().toISOString()
      };
    }
  }
   ...
   */
  private async fetchFromNPSCRA(request: NPSFetchRequest): Promise<NPSFetchResponse> {
    try {
      ...
      throw new Error(`NPS CRA API error: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private parseNPSResponse(data: any): NPSFetchResponse {
    ...
    const tierIBalance = parseFloat(data.tier_i?.current_balance || '0');
    const tierIIBalance = parseFloat(data.tier_ii?.current_balance || '0');
    ...
    const tierIContributions = parseFloat(data.tier_i?.total_contributions || '0');
    const tierIIContributions = parseFloat(data.tier_ii?.total_contributions || '0');
    const totalContributions = tierIContributions + tierIIContributions;
    const totalReturns = totalBalance - totalContributions;
    const returnsPercentage = totalContributions > 0 ? (totalReturns / totalContributions) * 100 : 0;
    ...
    holdings.push({
      ...
      tierIAssetAllocation: data.tier_i?.asset_allocation || { equityPercent: 0, corporateBondPercent: 0, governmentBondPercent: 0, alternativePercent: 0 },
      tierIIBalance,
      tierIIContributions,
      tierIIReturns: tierIIBalance - tierIIContributions,
      tierIIAssetAllocation: hasTierII ? (data.tier_ii?.asset_allocation || null) : null,
      totalBalance,
      totalContributions,
      totalReturns,
      returnsPercentage,
  ...
  /**
   * Generate mock NPS data for development
   */
  private getMockNPSData(panNumber: string): NPSFetchResponse {
    const mockAccounts: NPSAccount[] = [
      {
        pran: '123456789012', // 12-digit PRAN
        accountHolderName: 'Test User',
        tier: 'Both',
        tierIBalance: 485000.00,
        tierIIBalance: 125000.00,
        totalBalance: 610000.00,
        fundManager: 'HDFC Pension Management Company Ltd',
        scheme: 'Active Choice (50% E, 30% C, 20% G)',
        status: 'active'
      }
    ];

    const mockHoldings: NPSHolding[] = [
      {
        pran: '123456789012',
        accountHolderName: 'Test User',
        dateOfBirth: '1990-05-15',
        tier: 'Both',
        // Tier I (Mandatory retirement account - Cannot withdraw before 60)
        tierIBalance: 485000.00,
        tierIContributions: 390000.00,
        tierIReturns: 95000.00,
        tierIAssetAllocation: {
          equityPercent: 50, // E - Equity
          corporateBondPercent: 30, // C - Corporate Bonds
          governmentBondPercent: 20, // G - Government Securities
          alternativePercent: 0 // A - Alternative Assets
        },
        // Tier II (Voluntary savings - Can withdraw anytime)
        tierIIBalance: 125000.00,
        tierIIContributions: 100000.00,
        tierIIReturns: 25000.00,
        tierIIAssetAllocation: {
          equityPercent: 75,
          corporateBondPercent: 15,
          governmentBondPercent: 10,
          alternativePercent: 0
        },
        // Total
        totalBalance: 610000.00,
        totalContributions: 490000.00,
        totalReturns: 120000.00,
        returnsPercentage: 24.49, // (120000 / 490000) * 100
        // Account Details
        fundManager: 'HDFC Pension Management Company Ltd',
        scheme: 'Active Choice (50% E, 30% C, 20% G)',
        nominee: 'Spouse Name',
