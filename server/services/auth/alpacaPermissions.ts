...
export class AlpacaPermissions {
	...
	enforce(role: Role, action: AlpacaAction) {
		if (!this.hasPermission(role, action)) {
			logger.error(
				`[AlpacaPermissions] Access Denied for role ${role} attempting action ${action}`,
			);
			throw new Error(
				"Unauthorized Access: You do not have permission to perform this US trading action.",
			);
		}
	}
}
