...
class WhatsAppMarketingService {
  ...
  async sendTemplateMessage(
    ...
    if (!template || !template.contentSid) {
      console.log(`⚠️ Template ${templateType} not configured, falling back to freeform`);
      return { 
        success: false, 
    ...
    try {
      ...
      console.log(`📱 Sending WhatsApp template (${templateType}) to ${toNumber.substring(0, 15)}***`);
  ...
  async sendMarketingMessage(
    ...
    if (!hasConsent) {
      console.log(`🚫 WhatsApp marketing blocked - no consent: ${to.substring(0, 6)}****`);
      return { 
        success: false, 
    ...
    const templateResult = await this.sendTemplateMessage(to, templateType, variables);
    
    if (!templateResult.success && !templateResult.usedTemplate) {
      console.log(`⚠️ WhatsApp marketing requires approved template - freeform fallback disabled for compliance`);
    }
  ...
  async sendBulkTemplateMessages(
    ...
    templateType: keyof typeof MARKETING_TEMPLATES,
    variablesGenerator: (recipient: { mobile: string; name?: string }) => Record<string, string>,
    campaignId?: string
    ...
    for (const recipient of recipients) {
      ...
      if (!hasConsent) {
        console.log(`🚫 WhatsApp bulk skipped - no consent: ${recipient.mobile.substring(0, 6)}****`);
      ...
      const result = await this.sendTemplateMessage(recipient.mobile, templateType, variables);
    ...
    console.log(`📊 Bulk WhatsApp complete: ${results.sent}/${results.totalRecipients} sent, ${results.failed} failed`);
  ...
  async sendIPOAlert(
    to: string, 
    details: { companyName: string; openDate: string; priceMin: number; priceMax: number }
  ): Promise<WhatsAppMarketingResult> {
    return this.sendMarketingMessage(to, 'ipo_alert', {
  ...
  async sendPortfolioUpdate(
    ...
    return this.sendMarketingMessage(to, 'portfolio_update', {
  ...
  async sendKYCReminder(
    ...
    return this.sendMarketingMessage(to, 'kyc_reminder', {
  ...
  async sendPromotion(
    ...
    return this.sendMarketingMessage(to, 'promotion', {
  ...
  async sendMutualFundPromo(
    ...
    return this.sendMarketingMessage(to, 'mutual_fund', {
  ...
  async sendDividendAlert(
    ...
    return this.sendMarketingMessage(to, 'dividend_alert', {
      company_name: details.companyName,
  ...
  getAvailableTemplates(): Array<{ type: string; name: string; configured: boolean; category: string }> {
