...
class ValuationService {
	 ...
	 */
	async getValuationData(companyIdentifier: {
		cin?: string;
		name?: string;
		isin?: string;
		companyId?: string;
		...
		if (companyIdentifier.companyId) {
			company =
				(await storage.getUnlistedCompanyById(companyIdentifier.companyId)) ||
				undefined;
		} else if (companyIdentifier.cin) {
			company =
				(await storage.getUnlistedCompanyByCIN(companyIdentifier.cin)) ||
				undefined;
		}
		...
		if (this.isCrunchbaseAvailable() && companyName !== "Unknown") {
			try {
				...
				errors.push(
					`Crunchbase: ${error instanceof Error ? error.message : "Unknown error"}`,
				);
			}
		}
		...
		if (company?.id) {
			try {
				...
				errors.push(
					`FintekPro: ${error instanceof Error ? error.message : "Unknown error"}`,
				);
			}
		}
		...
		if (company?.id) {
			try {
				...
				errors.push(
					`Calculation: ${error instanceof Error ? error.message : "Unknown error"}`,
				);
			}
		}
	 ...
	 */
	private async fetchFromCrunchbase(
		companyName: string,
		...
		try {
			...
			if (!searchResponse.ok) {
				console.error(
					"[Valuation] Crunchbase search failed:",
			...
			if (!orgResponse.ok) {
				console.error(
					"[Valuation] Crunchbase org fetch failed:",
					orgResponse.status,
				);
				return null;
			}
	 ...
	 */
	private async getFromFintekProData(
		companyId: string,
	): Promise<ValuationData | null> {
		try {
			...
			if (company?.totalShares && mostRecentDeal.agreedPrice) {
				...
				const pricePerShare = Number.parseFloat(mostRecentDeal.agreedPrice);
	 ...
	 */
	private async calculateFromFinancials(
		companyId: string,
	): Promise<ValuationData | null> {
		try {
			...
			const sortedFinancials = [...financials].sort((a, b) => {
				const yearA = Number.parseInt(a.financialYear.split("-")[0]) || 0;
				const yearB = Number.parseInt(b.financialYear.split("-")[0]) || 0;
				return yearB - yearA;
			});
			...
			const networth = latest.networth
				...
				: null;
			const pat = latest.pat ? Number.parseFloat(latest.pat) : null;
