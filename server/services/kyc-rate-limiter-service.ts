...
class KycRateLimiterService {
  constructor() {
    ...
    console.log(`   Limits: Aadhaar OTP ${RATE_LIMIT_CONFIGS.aadhaar_otp.maxAllowed}/day/user, PAN ${RATE_LIMIT_CONFIGS.pan_verify.maxAllowed}/day/agent, CKYC ${RATE_LIMIT_CONFIGS.ckyc_lookup.maxAllowed}/day/IP`);
  }

  private getWindowBounds(windowMinutes: number): { windowStart: Date; windowEnd: Date } {
    const now = new Date();
  ...
  async checkLimit(action: string, identifier: string): Promise<{
    allowed: boolean;
    remaining: number;
    resetAt: Date;
    isLocked: boolean;
    ...
    if (!config) {
      return { allowed: true, remaining: 999, resetAt: new Date(), isLocked: false };
    }

    const limitKey = `${config.limitType}:${identifier}`;
    const { windowStart, windowEnd } = this.getWindowBounds(config.windowMinutes);
    ...
    try {
      ...
      if (existing.length > 0) {
        ...
        if (counter.isLocked) {
          return {
            ...
            isLocked: true,
            lockReason: counter.lockedReason || 'Rate limit exceeded - account locked',
          };
        }
        ...
        if (currentCount >= config.maxAllowed) {
          await db.update(kycRateLimitCounters)
            .set({
              isLocked: true,
              lockedAt: new Date(),
      ...
      console.error(`[RateLimiter] Error checking limit for ${limitKey}:`, error);
      return { allowed: true, remaining: config.maxAllowed, resetAt: new Date(), isLocked: false };
    }
  }
  ...
  async incrementCounter(action: string, identifier: string): Promise<void> {
    ...
    try {
      ...
      console.error(`[RateLimiter] Error incrementing counter for ${limitKey}:`, error);
    }
  }

  async adminUnlock(action: string, identifier: string, adminId: string): Promise<boolean> {
    const config = RATE_LIMIT_CONFIGS[action];
    ...
    try {
      const result = await db.update(kycRateLimitCounters)
        .set({
          isLocked: false,
          lockedAt: null,
  ...
  async getCounterStatus(action: string, identifier: string): Promise<{
    count: number;
    maxAllowed: number;
    isLocked: boolean;
    windowEnd: Date | null;
    ...
    const config = RATE_LIMIT_CONFIGS[action];
    if (!config) return { count: 0, maxAllowed: 0, isLocked: false, windowEnd: null };
    ...
    try {
      ...
      return { count: 0, maxAllowed: config.maxAllowed, isLocked: false, windowEnd: null };
    } catch (error) {
      return { count: 0, maxAllowed: config.maxAllowed, isLocked: false, windowEnd: null };
    }
  }
