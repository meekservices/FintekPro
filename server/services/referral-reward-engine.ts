...
export class ReferralRewardEngine {
	...
	static getInstance(): ReferralRewardEngine {
		if (!ReferralRewardEngine.instance) {
			ReferralRewardEngine.instance = new ReferralRewardEngine();
		}
		return ReferralRewardEngine.instance;
	}
	 ...
	 */
	async processKycComplete(
		refereeId: string,
	): Promise<{ credited: boolean; amount?: number; error?: string }> {
		try {
			...
			const rewardAmount =
				Number.parseFloat(referral.referrerRewardAmount?.toString() || "0") ||
				DEFAULT_KYC_REWARD;
			...
			await this.creditReferrerWallet(
				referral.referrerId,
				rewardAmount,
				"kyc_complete",
				referral.id,
			...
			console.log(
				`[ReferralReward] KYC reward ₹${rewardAmount} credited to referrer ${referral.referrerId} for referee ${refereeId}`,
			);
	...
	async processFirstInvestment(
		...
		try {
			...
			const rewardAmount =
				Number.parseFloat(referral.refereeRewardAmount?.toString() || "0") ||
				DEFAULT_FIRST_INVESTMENT_REWARD;
			...
			console.log(
				`[ReferralReward] First-investment reward ₹${rewardAmount} credited to referrer ${referral.referrerId}`,
			);
			return { credited: true, amount: rewardAmount };
		} catch (error) {
	 ...
	 */
	private async creditReferrerWallet(
		referrerId: string,
		amount: number,
		event: string,
		referralId: string,
		...
		try {
			...
			console.error(
				`[ReferralReward] Failed to credit wallet for referrer ${referrerId}:`,
				error,
			);
		}
	}
	...
	async getReferralSummary(referrerId: string): Promise<{
		...
		const totalRewardCredited = referrals
			...
			.reduce(
				(sum, r) =>
					sum + Number.parseFloat(r.referrerRewardAmount?.toString() || "0"),
				0,
			);
