...
export class APYService {
	...
	constructor() {
		this.apiBaseUrl =
			process.env.APY_API_URL || "https://api.accountaggregator.org.in/v1";
	...
	private getComingSoonResponse(): APYFetchResponse {
		return {
			...
			message:
				"Coming Soon - APY integration will be available once API credentials are configured. Please contact support to enable this feature.",
	 ...
	 */
	private async fetchFromAccountAggregator(
		request: APYFetchRequest,
	): Promise<APYFetchResponse> {
		try {
			...
			if (
				!apyResponse.data.accounts ||
				apyResponse.data.accounts.length === 0
			) {
				return {
					success: true,
					accounts: [],
			...
			throw new Error(
				`Account Aggregator API error: ${error instanceof Error ? error.message : "Unknown error"}`,
			);
		}
	}
	...
	private parseAPYResponse(data: any): APYFetchResponse {
		...
		data.accounts.forEach((account: any) => {
			const balance = Number.parseFloat(account.total_balance || "0");
			const userContribution = Number.parseFloat(
				account.total_contribution || "0",
			);
			const govtContribution = Number.parseFloat(
				account.government_contribution || "0",
			);
			...
			accounts.push({
				...
				pensionAmount: Number.parseFloat(account.pension_amount),
				monthlyContribution: Number.parseFloat(account.monthly_contribution),
				totalBalance: balance,
			...
			holdings.push({
				...
				enrollmentAge: Number.parseInt(account.enrollment_age),
				pensionAmount: Number.parseFloat(account.pension_amount),
				monthlyContribution: Number.parseFloat(account.monthly_contribution),
				totalContribution: userContribution,
