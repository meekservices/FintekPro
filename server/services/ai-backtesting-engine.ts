import { db } from "../db";
import { dailyPicks, aiPriceHistory, aiFeatureSnapshots, aiModelRegistry } from "@shared/schema";
...
export class AIBacktestingEngine {
  ...
  async runBacktest(config: BacktestConfig): Promise<BacktestResult> {
    ...
    const trades: BacktestTrade[] = completedPicks.map(pick => {
      const entryPrice = parseFloat(pick.recoPrice || '0');
      let exitPrice = parseFloat(pick.currentPrice || '0');

      if (pick.status === 'target_hit') {
        exitPrice = parseFloat(pick.targetPrice || '0') || exitPrice;
      } else if (pick.status === 'stoploss_hit') {
        exitPrice = parseFloat(pick.stoplossPrice || '0') || exitPrice;
      }

      if (exitPrice === 0) exitPrice = entryPrice;

      const rawReturnPct = pick.returnPct
        ? parseFloat(pick.returnPct)
      ...
      const daysHeld = pick.daysHeld
        ? pick.daysHeld
        : Math.max(1, Math.round((new Date(statusDate).getTime() - new Date(recoDate).getTime()) / (1000 * 60 * 60 * 24)));

      let transactionCosts = 0;
      if (includeTransactionCosts && entryPrice > 0) {
        const tradeValue = entryPrice;
        const costs = aiAnalyticsEngine.computeTransactionCosts(tradeValue, { slippagePct });
    ...
    const avgWinReturn = winningTrades.length > 0
      ? winningTrades.reduce((sum, t) => sum + t.netReturnPct, 0) / winningTrades.length
      : 0;

    const avgLossReturn = losingTrades.length > 0
      ? losingTrades.reduce((sum, t) => sum + t.netReturnPct, 0) / losingTrades.length
      : 0;
    ...
    const finalValue = portfolioValues.length > 0
    ...
    if (equityCurve.length >= 2) {
      ...
      years = Math.max((lastDate.getTime() - firstDate.getTime()) / (1000 * 60 * 60 * 24 * 365.25), 0.01);
    }

    const cagr = aiAnalyticsEngine.computeCAGR(initialCapital, finalValue, years);
    ...
    const totalTransactionCosts = trades.reduce((sum, t) => sum + t.transactionCosts, 0);
    ...
    if (trades.length > 0) {
      bestTrade = trades.reduce((best, t) => t.netReturnPct > best.netReturnPct ? t : best, trades[0]);
      worstTrade = trades.reduce((worst, t) => t.netReturnPct < worst.netReturnPct ? t : worst, trades[0]);
    }

    const byAssetClass: Record<string, { totalTrades: number; winRate: number; avgReturn: number; sharpeRatio: number }> = {};
    ...
    for (const [cls, clsTrades] of assetClassGroups) {
      ...
      const clsAvg = clsTrades.reduce((s, t) => s + t.netReturnPct, 0) / clsTrades.length;
    ...
    const byRegime: Record<string, { totalTrades: number; winRate: number; avgReturn: number }> = {};
    ...
    for (const [regime, regimeTrades] of regimeGroups) {
      ...
      const regimeAvg = regimeTrades.reduce((s, t) => s + t.netReturnPct, 0) / regimeTrades.length;
      byRegime[regime] = {
  ...
  async runWalkForwardBacktest(config: BacktestConfig): Promise<BacktestResult[]> {
    ...
    if (config.startDate) {
      ...
      startDate = minDateStr ? new Date(minDateStr) : new Date(endDate.getTime() - 365 * 24 * 60 * 60 * 1000);
    }
  ...
  async getBacktestHistory(assetClass?: string, limit: number = 20): Promise<any[]> {
    const conditions: any[] = [
      eq(aiModelRegistry.modelType, 'backtest'),
    ];
  ...
  private computeEquityCurve(trades: BacktestTrade[], initialCapital: number): EquityCurvePoint[] {
    if (trades.length === 0) {
      return [{
    ...
    for (const date of dateList) {
      ...
      const cumulativeReturn = initialCapital > 0 ? (portfolioValue - initialCapital) / initialCapital : 0;
  ...
  private computeMonthlyReturns(equityCurve: EquityCurvePoint[]): { month: string; return: number }[] {
    ...
    for (const point of equityCurve) {
      ...
      if (!monthlyMap.has(month)) {
        monthlyMap.set(month, { first: point.portfolioValue, last: point.portfolioValue });
      } else {
    ...
    for (let i = 0; i < months.length; i++) {
      ...
      const prevValue = i > 0 ? monthlyMap.get(months[i - 1])!.last : data.first;
      const monthReturn = prevValue > 0 ? ((data.last - prevValue) / prevValue) * 100 : 0;
      results.push({
        month,
