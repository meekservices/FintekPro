...
class TruthScreenEAadhaarDigiLockerService {
	...
	constructor() {
		...
		if (!this.username) {
			console.warn(
				"⚠️ [TruthScreen E-Aadhaar DGL] No TRUTHSCREEN_USERNAME configured — mock mode",
			);
		} else {
			console.log(
				`✅ [TruthScreen E-Aadhaar DGL] Initialized (${this.baseUrl})`,
			);
		}
	}
	...
	private async encrypt(payload: object): Promise<string> {
		...
		if (!encrypted || typeof encrypted !== "string") {
			console.error(
				"[TruthScreen E-Aadhaar DGL] Encrypt response:",
				JSON.stringify(response.data).substring(0, 300),
			);
			throw new Error(
				"TruthScreen encryption did not return an encrypted string",
			);
		}

		console.log(
			`[TruthScreen E-Aadhaar DGL] Step 1 complete (${encrypted.length} chars)`,
		);
		return encrypted;
	}
	...
	private async submitRequest(encryptedData: string): Promise<string> {
		console.log(
			`[TruthScreen E-Aadhaar DGL] Step 2: Submitting to /api/v1.0/eaadhaardigilocker/`,
		);
		...
		if (!encryptedResponse) {
			console.error(
				"[TruthScreen E-Aadhaar DGL] Submit response (raw):",
				JSON.stringify(response.data).substring(0, 300),
			);
			throw new Error(
				"TruthScreen E-Aadhaar DGL endpoint did not return response data",
			);
		}

		if (typeof encryptedResponse === "object") {
			console.log(
				`[TruthScreen E-Aadhaar DGL] Step 2 complete: Got unencrypted response object`,
			);
			return JSON.stringify(encryptedResponse);
		}

		console.log(
			`[TruthScreen E-Aadhaar DGL] Step 2 complete (${encryptedResponse.length} chars)`,
		);
	...
	private async decrypt(encryptedData: string): Promise<any> {
		try {
			...
			console.log(
				`[TruthScreen E-Aadhaar DGL] Response was already unencrypted JSON`,
			);
			return parsed;
		} catch {}
	...
	async generateDigiLockerLink(
		transId?: string,
	): Promise<EAadhaarLinkResponse> {
		...
		try {
			...
			console.log(
				`[TruthScreen E-Aadhaar DGL] Initiating DigiLocker link (trans_id: ${txnId})`,
			);
			...
			console.log(
				`[TruthScreen E-Aadhaar DGL] Link response keys: ${Object.keys(decrypted || {}).join(", ")}`,
			);
			console.log(
				`[TruthScreen E-Aadhaar DGL] Link response snippet: ${JSON.stringify(decrypted).substring(0, 500)}`,
			);

			const digilockerUrl =
				decrypted?.url ||
				decrypted?.digilockerUrl ||
				decrypted?.redirect_url ||
				decrypted?.link;
			const tsTransId =
				decrypted?.ts_transid ||
				decrypted?.tsTransId ||
				decrypted?.ts_trans_id ||
				decrypted?.transId;
			...
			return {
				success: true,
				message:
					"DigiLocker link generated successfully. Redirect user to complete authentication.",
			...
			console.error(
				`[TruthScreen E-Aadhaar DGL] Link generation error:`,
				error.message,
			);
			if (error.response?.data) {
				console.error(
					"[TruthScreen E-Aadhaar DGL] Error response:",
	...
	async checkStatus(tsTransId: string): Promise<EAadhaarStatusResponse> {
		...
		try {
			...
			console.log(
				`[TruthScreen E-Aadhaar DGL] Checking status (ts_trans_id: ${tsTransId})`,
			);
			...
			console.log(
				`[TruthScreen E-Aadhaar DGL] Status response keys: ${Object.keys(decrypted || {}).join(", ")}`,
			);
			console.log(
				`[TruthScreen E-Aadhaar DGL] Status response snippet: ${JSON.stringify(decrypted).substring(0, 500)}`,
			);
			...
			if (
				...
				return {
					success: true,
					message:
						msg || "User has not completed DigiLocker authentication yet",
					status: "pending",
					rawResponse: decrypted,
				};
			}
			...
			console.error(
				`[TruthScreen E-Aadhaar DGL] Status check error:`,
				error.message,
			);
			if (error.response?.data) {
				console.error(
	...
	private mockGenerateLink(transId?: string): EAadhaarLinkResponse {
		...
		console.log(
			`🔧 [TruthScreen E-Aadhaar DGL] Mock: Generating DigiLocker link (${txnId})`,
		);

		return {
			success: true,
			message:
				"[MOCK] DigiLocker link generated. In production, redirect user to the URL below.",
	...
	private mockCheckStatus(tsTransId: string): EAadhaarStatusResponse {
		console.log(
			`🔧 [TruthScreen E-Aadhaar DGL] Mock: Checking status (${tsTransId})`,
		);
		...
		return {
			...
			data: {
				...
				country: "India",
				address:
					"123, Mock Street, Near Mock Plaza, Mock Colony, Mumbai, Maharashtra, 400001",
				mobile: "9876543210",
	...
	getServiceInfo() {
		return {
			...
			phases: [
				"LINK (generate DigiLocker URL)",
				"STATUS (fetch E-Aadhaar after user auth)",
			],
		};
	}
}

export const truthScreenEAadhaarDGLService =
	new TruthScreenEAadhaarDigiLockerService();
