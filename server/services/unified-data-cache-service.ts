...
export const API_COST_ESTIMATES = {
	credhive_company_search: 0.5,
	credhive_company_details: 1.0,
	credhive_financials: 2.0,
	sandbox_pan: 0.75,
	sandbox_gstin: 0.5,
	sandbox_mca: 1.5,
	cashfree_pan: 0.5,
	cashfree_aadhaar: 1.0,
	finnhub_quote: 0.01,
	nse_quote: 0.005,
};
...
export class UnifiedDataCacheService {
	...
	async saveMarketData(params: {
		...
		try {
			...
			switch (params.dataType) {
				case "quote":
					ttl = CACHE_TTL.MARKET_QUOTE;
					break;
				case "index":
					ttl = CACHE_TTL.MARKET_INDEX;
					break;
				case "nav":
					ttl = CACHE_TTL.MARKET_NAV;
					break;
				case "eod_price":
					ttl = CACHE_TTL.MARKET_EOD;
					break;
				default:
					ttl = CACHE_TTL.MARKET_QUOTE;
			}
	...
	async trackApiUsage(
		...
		try {
			...
			const estimatedCost = cacheHit
				? 0
				: API_COST_ESTIMATES[costKey as keyof typeof API_COST_ESTIMATES] || 0;
	...
	async getCacheStats(): Promise<{
        ...
        `),
			...
			const totalCalls = Number.parseInt(
				apiStats.rows?.[0]?.total_calls || "0",
			);
			const cacheHits = Number.parseInt(apiStats.rows?.[0]?.cache_hits || "0");

			return {
				companyMaster: {
					count: Number.parseInt(companyCount.rows?.[0]?.count || "0"),
				},
				verifications: {
					count: Number.parseInt(verificationStats.rows?.[0]?.total || "0"),
					expired: Number.parseInt(verificationStats.rows?.[0]?.expired || "0"),
				},
				financials: {
					count: Number.parseInt(financialsStats.rows?.[0]?.total || "0"),
					expired: Number.parseInt(financialsStats.rows?.[0]?.expired || "0"),
				},
				marketData: {
					count: Number.parseInt(marketDataStats.rows?.[0]?.total || "0"),
					expired: Number.parseInt(marketDataStats.rows?.[0]?.expired || "0"),
				},
				apiUsage: {
					totalCalls,
					cacheHits,
					hitRate: totalCalls > 0 ? (cacheHits / totalCalls) * 100 : 0,
					estimatedSavings: Number.parseFloat(
						apiStats.rows?.[0]?.estimated_savings || "0",
					),
				},
			};
		} catch (error) {
	...
	async cleanupExpiredCache(): Promise<{ deleted: number }> {
		try {
			...
			const [v, f, m] = await Promise.all([
				db.execute(
					sql`DELETE FROM verification_cache WHERE expires_at <= ${now}`,
				),
				db.execute(
					sql`DELETE FROM company_financials_cache WHERE expires_at <= ${now}`,
				),
				db.execute(
					sql`DELETE FROM market_data_cache WHERE expires_at <= ${now}`,
				),
			]);
