...
export class AIPortfolioOptimizer {
  ...
  private async optimizeBasketInternal(config?: OptimizationConfig): Promise<OptimizedBasket> {
    const cfg: Required<OptimizationConfig> = {
      ...
      maxPerAssetClass: config?.maxPerAssetClass ?? 0.40,
    ...
    const portfolioMetrics = this.computePortfolioMetrics(optimizedWeights, selected, covMatrix, cfg.riskFreeRate);

    const alternativePortfolios: OptimizedBasket['alternativePortfolios'] = [];

    if (cfg.includeAlternatives) {
      const minVarWeights = this.computeMinVarianceWeights(selected, covMatrix, cfg);
      const minVarMetrics = this.computePortfolioMetrics(minVarWeights, selected, covMatrix, cfg.riskFreeRate);
      ...
      const eqMetrics = this.computePortfolioMetrics(eqWeights, selected, covMatrix, cfg.riskFreeRate);
      alternativePortfolios.push({
        name: 'Equal Weight',
        weights: eqWeights,
        metrics: eqMetrics,
  ...
  async computeAssetMetrics(picks: any[]): Promise<AssetCandidate[]> {
    ...
    for (const pick of picks) {
      try {
        const currentPrice = parseFloat(pick.currentPrice || pick.recoPrice || '0');
        const targetPrice = parseFloat(pick.targetPrice || '0');
        const stoplossPrice = parseFloat(pick.stoplossPrice || '0');

        if (currentPrice <= 0) continue;

        const expectedReturn = targetPrice > 0 ? (targetPrice - currentPrice) / currentPrice : 0;
  ...
  optimizeWeights(
    ...
    let prevSharpe = this.portfolioSharpe(weights, candidates, covMatrix, riskFreeRate);
    ...
    for (let iter = 0; iter < maxIterations; iter++) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        const sharpePlus = this.portfolioSharpe(normalizedPlus, candidates, covMatrix, riskFreeRate);
      ...
      const currentSharpe = this.portfolioSharpe(weights, candidates, covMatrix, riskFreeRate);
    ...
    return candidates.map((c, i) => {
      const portVar = aiAnalyticsEngine.computePortfolioVariance(weights, covMatrix);
      ...
      const portReturn = weights.reduce((sum, w, idx) => sum + w * candidates[idx].expectedReturn, 0);
      const marginalSharpe = portVol > 0
        ? (c.expectedReturn - riskFreeRate) / portVol
        : 0;
  ...
  computeMinVarianceWeights(
    ...
    let weights = new Array(n).fill(1 / n);
    let prevVar = aiAnalyticsEngine.computePortfolioVariance(weights, covMatrix);
    ...
    for (let iter = 0; iter < maxIterations; iter++) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        const varPlus = aiAnalyticsEngine.computePortfolioVariance(normalizedPlus, covMatrix);
      ...
      const currentVar = aiAnalyticsEngine.computePortfolioVariance(weights, covMatrix);
  ...
  computePortfolioMetrics(
    ...
    const expectedReturn = w.reduce((sum, wi, i) => sum + wi * candidates[i].expectedReturn, 0);
    ...
    const diversificationRatio = this.computeDiversificationRatio(w, volatilities, expectedVolatility);
    ...
    const correlationAvg = correlationCount > 0 ? correlationSum / correlationCount : 0;
  ...
  computeDiversificationRatio(weights: number[], volatilities: number[], portfolioVol: number): number {
  ...
  async getDiversificationScore(pickIds?: number[]): Promise<{
    ...
    if (picks.length === 0) {
      return { score: 0, breakdown: {}, recommendations: ['No active picks found. Generate daily picks first.'] };
    }
    ...
    const concentrations = Object.values(assetClassCounts).map(c => c / totalPicks);
    ...
    const score = Math.round(classScore * 0.4 + concentrationScore * 0.4 + countScore * 0.2);
    ...
    if (herfindahl > 0.3) {
      recommendations.push('Portfolio is concentrated in few asset classes. Spread allocation more evenly.');
    }
    if (totalPicks < 5) {
      recommendations.push('Increase the number of positions for better risk distribution.');
    }

    const missingClasses = ['listed_stocks', 'mutual_funds', 'bonds', 'global_stocks', 'etfs']
      .filter(cls => !assetClassCounts[cls]);
    if (missingClasses.length > 0) {
      recommendations.push(`Consider adding picks from: ${missingClasses.join(', ')}`);
    }

    if (recommendations.length === 0) {
      recommendations.push('Portfolio is well-diversified across asset classes.');
    }

    return { score: Math.min(score, 100), breakdown, recommendations };
  }

  private async fetchReturnSeries(assetId: string, assetClass: string, days: number = 90): Promise<number[]> {
    try {
      const cutoffDate = new Date();
      cutoffDate.setDate(cutoffDate.getDate() - days);
      const cutoff = cutoffDate.toISOString().split('T')[0];
      ...
      const priceData = await db
        .select({ close: aiPriceHistory.close, priceDate: aiPriceHistory.priceDate })
        .from(aiPriceHistory)
        .where(
          and(
      ...
      console.warn(`[PortfolioOptimizer] Error fetching return series for ${assetId}:`, err);
      return this.generateSyntheticReturns(days);
    }
  }
  ...
  private applyConstraints(
    ...
    const maxWeight = config.maxWeightPerAsset ?? 0.25;
    const minWeight = config.minWeightPerAsset ?? 0.02;
    const maxPerAssetClass = config.maxPerAssetClass ?? 0.40;
  ...
  private portfolioSharpe(
    ...
    const portReturn = weights.reduce((sum, w, i) => sum + w * candidates[i].expectedReturn, 0);
    const portVar = aiAnalyticsEngine.computePortfolioVariance(weights, covMatrix);
    const portVol = Math.sqrt(Math.max(portVar, 0));
  ...
  private stdDev(arr: number[]): number {
    ...
    const variance = arr.reduce((sum, v) => sum + (v - mean) * (v - mean), 0) / (arr.length - 1);
