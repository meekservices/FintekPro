...
const SEGMENT_INVESTMENT_CAPS: Record<
	string,
	{
		pms: number | null;
...
export class InvestableSurplusEngine {
	...
	async calculateTotalIncome(userId: string): Promise<{
		...
		for (const stream of streams) {
			const grossAmount = Number.parseFloat(stream.grossAmount || "0");
			const netAmount = Number.parseFloat(stream.netAmount || "0");
			...
			switch (frequency) {
				...
				case "one_time":
					annualMultiplier = 1;
					break; // One-time treated as annual
			}
	...
	async calculateTotalObligations(userId: string): Promise<{
		...
		for (const obligation of obligations) {
			const monthlyAmount = Number.parseFloat(obligation.monthlyAmount || "0");
	...
	async getEmergencyFundStatus(userId: string): Promise<{
		...
		const current = Number.parseFloat(
			emergencyFund.currentEmergencyFund || "0",
		);
	 ...
	 */
	async calculateInvestableSurplus(
		userId: string,
	): Promise<SurplusCalculationResult> {
		// Get income
	...
	private generateRecommendations(
		...
		if (emergency.status === "inadequate") {
			...
			shortTerm.push(
				`Grow emergency fund to ₹${(emergency.shortfall / 100000).toFixed(1)}L (6 months coverage)`,
			);
		} else if (emergency.status === "partial") {
			shortTerm.push(
				`Increase emergency fund by ₹${(emergency.shortfall / 100000).toFixed(1)}L to reach 6 months coverage`,
			);
		}
		...
		if (debtToIncome > 50) {
			immediate.push(
				"High debt-to-income ratio (>50%). Consider debt consolidation",
			);
		} else if (debtToIncome > 40) {
			shortTerm.push(
				"Consider reducing debt obligations to below 40% of income",
			);
		}
		...
		if (surplus > 0) {
			...
			if (monthlySurplus >= 50000) {
				shortTerm.push(
					"Start SIPs in diversified equity funds for long-term wealth creation",
				);
				...
				immediate.push(
					"Start small SIPs of ₹500-1000 to build investment habit",
				);
				shortTerm.push("Focus on increasing income or reducing expenses");
			}
		} else {
			immediate.push(
				"No investable surplus currently. Focus on increasing income or reducing expenses",
			);
		}
	...
	async determineClientSegment(
		...
		const eligibleProducts =
			SEGMENT_PRODUCT_ELIGIBILITY[segment] ||
			SEGMENT_PRODUCT_ELIGIBILITY.retail;
		const allProducts = Object.values(SEGMENT_PRODUCT_ELIGIBILITY).flat();
		const uniqueProducts = Array.from(new Set(allProducts));
		const restrictedProducts = uniqueProducts.filter(
			(p) => !eligibleProducts.includes(p),
		);
		const thresholds =
			SEGMENT_THRESHOLDS[segment as keyof typeof SEGMENT_THRESHOLDS];
		...
		return {
			...
			restrictedProducts,
			investmentCaps:
				SEGMENT_INVESTMENT_CAPS[segment] || SEGMENT_INVESTMENT_CAPS.retail,
			segmentThreshold: {
				min: thresholds.min,
				max: thresholds.max,
				currency: "INR",
	 ...
	 */
	async saveInvestableSurplus(
		userId: string,
		result: SurplusCalculationResult,
	): Promise<InvestableSurplus> {
		...
		if (existing.length > 0) {
			...
			const [updated] = await db
				...
				.set({
					...
					previousSegment:
						existing[0].segment !== result.segment
							? existing[0].segment
							: undefined,
					segmentChangedAt:
						existing[0].segment !== result.segment ? new Date() : undefined,
					assessedAt: new Date(),
					updatedAt: new Date(),
				})
				.where(eq(clientSegments.userId, userId))
	...
	async performFullAssessment(userId: string): Promise<{
		...
		const segment = await this.saveClientSegment(
			userId,
			segmentResult,
