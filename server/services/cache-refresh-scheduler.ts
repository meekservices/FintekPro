...
class CacheRefreshScheduler {
		...
		try {
			...
			if (assetType === "stock") {
				...
				const schemes = await db
					.select({ id: aifSchemes.id, name: aifSchemes.name })
					.from(aifSchemes)
					.limit(200);
				assets = schemes.map((s) => ({ assetId: s.id, assetName: s.name }));
			} else if (assetType === "pms") {
				const schemes = await db
					.select({ id: pmsSchemes.id, name: pmsSchemes.name })
					.from(pmsSchemes)
					.limit(200);
				assets = schemes.map((s) => ({ assetId: s.id, assetName: s.name }));
			}
			...
			for (let i = 0; i < assets.length; i += batchSize) {
				...
				const promises = batch.map(async (asset) => {
					try {
						...
						const marketData = await this.fetchMarketDataForAsset(
							assetType,
							asset.assetId,
						);
			...
			logger.info(
				`[CacheRefreshScheduler] Market data refresh completed for ${assetType}: ${itemsProcessed} processed, ${itemsFailed} failed`,
			);
		} catch (err: any) {
			logger.error(
				`[CacheRefreshScheduler] Market data refresh failed for ${assetType}: ${err.message}`,
			);
	...
	private async fetchMarketDataForAsset(
		assetType: string,
		assetId: string,
	): Promise<any> {
	...
	async refreshAllMarketData(): Promise<void> {
		logger.info(
			"[CacheRefreshScheduler] Starting comprehensive market data refresh",
		);
		...
		logger.info(
			"[CacheRefreshScheduler] Comprehensive market data refresh completed",
		);
	}
		...
		try {
			...
			for (const product of staleProducts) {
				try {
					...
					const fundamentals = await this.fetchFundamentalsForProduct(
						productType,
						product.productId,
					);
			...
			logger.info(
				`[CacheRefreshScheduler] Fundamentals refresh completed for ${productType}: ${itemsProcessed} processed, ${itemsFailed} failed`,
			);
		} catch (err: any) {
			logger.error(
				`[CacheRefreshScheduler] Fundamentals refresh failed for ${productType}: ${err.message}`,
			);
			errors.push(err.message);
		}
	...
	private async fetchFundamentalsForProduct(
		productType: string,
		productId: string,
	): Promise<any> {
	...
	async precomputePortfolioMetrics(): Promise<RefreshJobResult> {
		...
		try {
				...
				try {
					...
					for (const portfolio of userPortfolios) {
						try {
							...
							const metrics = await this.computePortfolioMetrics(
								userId,
								portfolio.id,
							);
			...
			logger.info(
				`[CacheRefreshScheduler] Portfolio metrics precomputation completed: ${itemsProcessed} processed, ${itemsFailed} failed`,
			);
		} catch (err: any) {
			logger.error(
				`[CacheRefreshScheduler] Portfolio metrics precomputation failed: ${err.message}`,
			);
			errors.push(err.message);
		}
	...
	private async computePortfolioMetrics(
		userId: string,
		portfolioId: string,
	): Promise<any> {
		...
		for (const holding of holdings) {
			...
			const avgCost = Number.parseFloat(holding.avgCost || "0");
			const currentPrice = Number.parseFloat(
				holding.currentPrice || holding.avgCost || "0",
			);
		...
		return {
			...
			allocationEquity:
				totalValue > 0 ? (equityValue / totalValue).toString() : "0",
			allocationDebt:
				totalValue > 0 ? (debtValue / totalValue).toString() : "0",
			allocationGold:
				totalValue > 0 ? (goldValue / totalValue).toString() : "0",
			allocationCash:
				totalValue > 0 ? (cashValue / totalValue).toString() : "0",
			allocationAlternatives:
				totalValue > 0 ? (alternativesValue / totalValue).toString() : "0",
			totalHoldings: holdings.length,
			equityHoldings: holdings.filter((h) =>
				["stock", "equity_mf"].includes(h.assetType || ""),
			).length,
			debtHoldings: holdings.filter((h) =>
				["bond", "debt_mf", "ncd"].includes(h.assetType || ""),
			).length,
			mfHoldings: holdings.filter((h) => (h.assetType || "").includes("_mf"))
				.length,
		};
	}
	...
	async performCacheCleanup(): Promise<void> {
        ...
        Proposals: ${stats.proposalsDeleted} deleted`);
			...
			logger.error(
				`[CacheRefreshScheduler] Cache cleanup failed: ${err.message}`,
			);
		}
	}
	...
	async triggerMarketDataRefresh(
		assetType?: string,
	...
	async triggerFundamentalsRefresh(
		productType?: string,
	): Promise<RefreshJobResult[]> {
		if (productType) {
