...
export class APSESimulator {
  ...
  public async runFullSimulation(context: SimulationRequestContext): Promise<SimulationOutputContract> {
    const startTime = Date.now();

    // 1. Process Input Context & Assumptions
    const baseReturn = context.market_assumptions.base_expected_return ?? 0.10; // Default 10%
    ...
    const [mcResult, scenarioResult] = await Promise.all([
      Promise.resolve(monteCarloEngine.simulatePortfolio({
        portfolioVolatility: baseVolatility,
        expectedAnnualReturn: baseReturn,
        timeHorizonYears
      })),
      Promise.resolve(scenarioEngine.generateScenarioTraces({
  ...
  private persistSimulationLog(context: SimulationRequestContext, output: SimulationOutputContract): void {
    db.insert(apseSimulationLogs).values({
      id: uuidv4(),
    ...
    }).execute().catch(e => console.error("[APSE Failsafe]: Failed to persist simulation tracking map.", e));
  }
}
