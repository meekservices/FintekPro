...
export class KYCWorkflowOrchestrator {
	...
	async createCKYC(
		...
		try {
			...
			const registrationResult = await this.ckycService.registerCKYC({
				...
				addressLine1:
					`${okycData.address.house} ${okycData.address.street}`.trim(),
	...
	async storeInVault(
		...
		try {
			...
			const encryptedFatherName = okycData.fatherName
				? encryptionService.encrypt(okycData.fatherName)
				: null;
			...
			const encryptedPincode = encryptionService.encrypt(
				okycData.address.pincode,
			);

			// Encrypt contact info
			const encryptedMobile = okycData.mobile
				? encryptionService.encrypt(okycData.mobile)
				: null;
			const encryptedEmail = okycData.email
				? encryptionService.encrypt(okycData.email)
				: null;
			...
			if (okycData.photoUrl) {
				const hashResult = await faceHashingService.hashFaceImageFromUrl(
					okycData.photoUrl,
				);
				faceImageHash =
					hashResult.success && hashResult.hash ? hashResult.hash : null;
			}
	...
	async validateVaultStorage(
		...
		try {
			...
			console.log(
				`✅ Vault validation successful for user ${userId} (Audit: ${decryptionResult.auditLogId})`,
			);
	...
	async recordConsent(
		...
		try {
			...
			if (!hmacKey)
				throw new Error(
					"ENCRYPTION_MASTER_KEY is not configured — cannot sign KYC consent",
				);
	...
	async storePreVerifiedKYCData(
		...
		try {
			console.log(
				"🚀 Starting pre-verified KYC storage workflow for user:",
				userId,
			);

			// Step 1: Check CKYC Registry
			const ckycCheckResult = await this.checkCKYC(
				panNumber,
				okycData.aadhaarNumber,
			);
			...
			if (ckycCheckResult.data?.found && ckycCheckResult.ckycKinNumber) {
				// CKYC exists
				console.log(
					"✅ Existing CKYC record found:",
					ckycCheckResult.ckycKinNumber,
				);
			...
			const validationResult = await this.validateVaultStorage(
				userId,
				panNumber,
				okycData.name,
			);
		...
		try {
			...
			if (vaultRecord && vaultRecord.kycStatus !== "verified") {
				...
				return {
					...
					message:
						"KYC was interrupted after vault storage. Can resume from consent step.",
				};
			}
			...
			if (consentLog && !vaultRecord) {
				return {
					...
					resumeData: null,
					message:
						"Consent recorded but vault storage failed. Requires re-verification.",
				};
			}
			...
			if (userMeta.okycRefId && !vaultRecord) {
				...
				const hoursSinceInit = initiatedAt
					? (Date.now() - new Date(initiatedAt).getTime()) / (1000 * 60 * 60)
					: 0;
				...
				if (hoursSinceInit > 24) {
					...
					return {
						...
						resumeData: null,
						message:
							"OTP expired (>24 hours). Please restart Aadhaar verification.",
					};
				}
				...
				return {
					...
					message:
						"OTP was sent but not verified. Can resume from OTP verification.",
				};
			}
	...
	async resumeWorkflow(
		...
		try {
			...
			console.log(
				`[KYC Recovery] Resuming workflow for user ${userId} from step: ${recoveryState.currentStep}`,
			);
			...
			switch (recoveryState.currentStep) {
				case "aadhaar_otp_sent": {
					...
					// Verify OTP first
					const okycResult = await this.verifyOKYC(
						resumeData.otp,
					...
					const panNumber =
						resumeData.panNumber || recoveryState.resumeData?.panNumber;
