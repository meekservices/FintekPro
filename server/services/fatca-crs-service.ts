...
class FATCACRSService {
	...
	async getFATCAStatus(userId: string): Promise<FATCAStatus> {
		...
		const lastDoc = [...w8benDocs, ...w9Docs].sort(
			(a, b) =>
				(b.verificationDate?.getTime() || 0) -
				(a.verificationDate?.getTime() || 0),
		)[0];
		...
		if (lastCertificationDate) {
			...
			nextCertificationDue.setFullYear(
				nextCertificationDue.getFullYear() + FORM_VALIDITY_YEARS,
			);
		}
	...
	async submitSelfCertification(params: {
		...
		if (params.taxResidencies.length > 0) {
			const primaryResidence =
				params.taxResidencies.find((tr) => tr.isPrimaryResidence) ||
				params.taxResidencies[0];
			await db
				.update(users)
				.set({
	...
	async checkUSIndicia(userId: string): Promise<{
		...
		if (
			...
			user.countryOfResidence?.toUpperCase() === "US"
		) {
	...
	async generateCRSReport(
		...
		const usersInJurisdiction = await db
			.select()
			.from(users)
		...
		for (const user of usersInJurisdiction) {
			...
			if (
				user.residentStatus === "resident" &&
				reportingJurisdiction !== "IN"
			) {
				continue;
			}
			...
			const account: ReportableAccount = {
				...
				accountNumber:
					user.nsdlClientId || user.cdslBoId || user.bankAccountNumber || "",
	...
	async getComplianceSummary(userId: string): Promise<{
		...
		if (
			fatca.fatcaStatus === "non_compliant" ||
			crs.crsStatus === "non_compliant"
		) {
	 ...
	 * Log FATCA/CRS event
	 */
	private async logFATCACRSEvent(
		userId: string,
		action: string,
		details: any,
	): Promise<void> {
