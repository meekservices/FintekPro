...
import {
	unifiedHoldingsReaderService,
	type UnifiedHolding,
} from "./unified-holdings-reader-service";
import type {
	AiProfitPick,
	InsertAiProfitPick,
	PortfolioAlert,
...
class AIInvestmentService {
	constructor() {
		...
		console.log(
			`✅ AI Investment Service initialized via Unified Engine (primary: ${status.primary})`,
		);
	}
	...
	private addDefaultStockFields(
		...
		const timeHorizonDays =
			base.timeHorizon === "long"
				? 365
				: base.timeHorizon === "medium"
					? 180
					: base.timeHorizon === "short"
						? 90
						: 30;
		const riskScore =
			base.riskLevel === "low"
		...
		return {
			...
			confidenceLevel: confidenceLevel as
				| "low"
				| "medium"
				| "high"
				| "very_high",
	 ...
	 */
	async getStoreEligibleMutualFunds(
		options: {
			category?: string;
			riskLevel?: string;
			limit?: number;
		...
		try {
			...
			return eligibleFunds.map((fund) => {
				const nav = Number.parseFloat(fund.nav || "0");
	...
	async getStoreEligibleAIF(
		...
		try {
			...
			return eligibleAIFs.map((aif) => {
				const nav = Number.parseFloat(aif.latestNav || "100");
				const returns1y = Number.parseFloat(aif.return1Y || "0");
				const minInvestment = Number.parseFloat(
					aif.minInvestment || "10000000",
				);
				const risk =
					(aif.riskScore || 5) > 7
						? "very_high"
				...
				return this.addDefaultStockFields({
					...
					keyFactors: [
						...
						`AUM: ₹${(Number.parseFloat(aif.aum || "0") / 10000000).toFixed(0)}Cr`,
					],
					riskFactors: [
						"Illiquid investment",
						"Long lock-in period",
	...
	async getStoreEligiblePMS(
		...
		try {
			...
			return eligiblePMS.map((pms) => {
				const nav = Number.parseFloat(pms.latestNav || "100");
				const returns1y = Number.parseFloat(pms.return1Y || "0");
				const minInvestment = Number.parseFloat(pms.minInvestment || "5000000");
				const risk =
					(pms.riskScore || 5) > 7
						? "high"
						: (pms.riskScore || 5) > 4
		...
		try {
			...
			return eligibleMLDs.map((mld) => {
				const price = Number.parseFloat(
					mld.latestPrice || mld.faceValue || "1000000",
				);
				const ytm = Number.parseFloat(mld.ytm || "8");
				const risk =
					(mld.riskScore || 5) > 6
						? "high"
						: (mld.riskScore || 5) > 3
	 ...
	 */
	async getStoreEligibleUnlistedStocks(
		options: {
			sector?: string;
			limit?: number;
		} = {},
		...
		try {
			...
			return eligibleUnlisted.map((company) => {
				const buyPrice = Number.parseFloat(company.publishedBuyPrice || "0");
				const sellPrice = Number.parseFloat(company.publishedSellPrice || "0");
	...
	async getStoreEligibleBonds(
		...
		try {
			...
			const conditions = [eq(corporateBonds.tradingStatus, "active")];
			...
			if (creditRating) {
				conditions.push(
					ilike(corporateBonds.creditRating, `%${creditRating}%`),
				);
			}
			...
			return eligibleBonds.map((bond) => {
				const price = Number.parseFloat(
					bond.currentPrice || bond.faceValue || "1000",
				);
				const ytm = Number.parseFloat(bond.yieldToMaturity || "8");
				const coupon = Number.parseFloat(bond.couponRate || "7");
				const ratingScore = bond.creditRating?.includes("AAA")
					? 1
					: bond.creditRating?.includes("AA")
						? 2
						: 3;
				const risk =
					ratingScore === 1 ? "low" : ratingScore === 2 ? "moderate" : "high";
	...
	async getStoreEligibleNCDs(
		...
		try {
			...
			return eligibleNCDs.map((ncd) => {
				const faceValue = Number.parseFloat(ncd.faceValue || "1000");
				const coupon = Number.parseFloat(ncd.couponRate || "8");
				const yield_ = Number.parseFloat(
					ncd.effectiveYield || ncd.couponRate || "8",
				);
				const ratingScore = ncd.creditRating?.includes("AAA")
					? 1
					: ncd.creditRating?.includes("AA")
						? 2
						: 3;
				const risk =
					ratingScore === 1 ? "low" : ratingScore === 2 ? "moderate" : "high";
	 ...
	 */
	async getStoreEligibleListedStocks(
		options: {
			riskLevel?: string;
			sectors?: string[];
			limit?: number;
		...
		try {
			...
			const allowedMarketCaps = normalizedRisk
				? riskToMarketCap[normalizedRisk] || ["Large Cap"]
				: ["Large Cap", "Mid Cap"];
				...
				.filter((stock) => {
					...
					if (sectors && sectors.length > 0) {
						if (
							!stock.sector ||
							!sectors.some((s) =>
								stock.sector?.toLowerCase().includes(s.toLowerCase()),
							)
						) {
							return false;
						}
			...
			console.log(
				`[AI Service] Risk: ${riskLevel} -> ${normalizedRisk}, Allowed caps: ${allowedMarketCaps.join(", ")}, Found: ${filteredStocks.length} stocks`,
			);

			return filteredStocks.map((stock) => {
				const currentPrice = Number.parseFloat(stock.currentPrice || "0");
				const previousClose = Number.parseFloat(stock.previousClose || "0");
				const dayChangePercent = Number.parseFloat(
					stock.dayChangePercent || "0",
				);
				const returns1Y = Number.parseFloat(stock.returns1Y || "0");
				const beta = Number.parseFloat(stock.beta || "1");
				const low52Week = Number.parseFloat(stock.weekLow52 || "0");

				// Calculate upside based on returns momentum and market cap
				const estimatedUpside = Math.max(
					5,
					Math.min(
						30,
						10 + returns1Y * 0.3 + (stock.marketCap === "Large Cap" ? 2 : 5),
					),
				);
	...
	async getClientPortfolio(
		clientId: string,
	): Promise<{ portfolio: any; holdings: PortfolioHolding[] } | null> {
		// Use unified holdings reader - single source of truth for both prospects and registered clients
		const clientType =
			await unifiedHoldingsReaderService.getClientType(clientId);
		const unifiedHoldings =
			await unifiedHoldingsReaderService.getHoldings(clientId);
	...
	async analyzePortfolio(
		...
		const portfolioRiskAlignment = this.assessRiskAlignment(
			metrics.riskScore,
			clientRiskProfile,
		);
		...
		const [analysisResult] = await db
			...
			.values({
				...
				top5HoldingsWeight: String(
					metrics.topHoldings.slice(0, 5).reduce((sum, h) => sum + h.weight, 0),
				),
				clientRiskProfile,
				portfolioRiskAlignment,
				overallHealthScore,
	...
	private async calculatePortfolioMetrics(
		...
		for (const holding of holdings) {
			...
			const currentPrice = marketDataEntry?.price
				? Number.parseFloat(marketDataEntry.price)
				: avgPrice;
	...
	private assessRiskAlignment(
		...
		const threshold =
			riskThresholds[clientRiskProfile] || riskThresholds.moderate;

		if (
			portfolioRiskScore >= threshold.min &&
			portfolioRiskScore <= threshold.max
		) {
			return "aligned";
		}
		if (
			Math.abs(portfolioRiskScore - (threshold.min + threshold.max) / 2) <= 20
		) {
	...
	private calculateHealthScore(
		...
		if (metrics.gainLossPercent > 0)
			score += Math.min(10, metrics.gainLossPercent);
		else score += Math.max(-10, metrics.gainLossPercent / 2);

		const maxSectorConcentration = Math.max(
			...Object.values(metrics.sectorConcentration),
			0,
		);
...
${metrics.topHoldings
	.slice(0, 5)
	.map((h) => `- ${h.symbol}: ${h.weight.toFixed(1)}%`)
	.join("\n")}
...
Provide a JSON response with:
{
  ...
  "recommendations": ["recommendation1", "recommendation2", "recommendation3"]
		...
		const fallbackResult = this.generateRuleBasedInsights(
			holdings,
			metrics,
			clientRiskProfile,
		);
	...
	private generateRuleBasedInsights(
		...
		if (metrics.gainLossPercent > 5) {
			strengths.push(
				`Portfolio showing positive returns of ${metrics.gainLossPercent.toFixed(1)}%`,
			);
		}
		...
		if (maxConcentration > 40) {
			weaknesses.push(
				`High concentration in single sector (${maxConcentration.toFixed(1)}%)`,
			);
			recommendations.push(
				"Consider reducing sector concentration to below 30%",
			);
		}

		if (metrics.topHoldings[0]?.weight > 25) {
			weaknesses.push(
				`Top holding represents ${metrics.topHoldings[0].weight.toFixed(1)}% of portfolio`,
			);
			recommendations.push("Reduce single stock concentration to below 20%");
		}

		const riskAlignment = this.assessRiskAlignment(
			metrics.riskScore,
			clientRiskProfile,
		);
		if (riskAlignment === "misaligned") {
			weaknesses.push(
				`Portfolio risk (${metrics.riskScore}) doesn't match ${clientRiskProfile} profile`,
			);
			if (clientRiskProfile === "conservative" && metrics.riskScore > 50) {
				recommendations.push(
					"Increase debt allocation to reduce portfolio risk",
				);
		...
		if (metrics.debtAllocation < 10 && clientRiskProfile !== "aggressive") {
			recommendations.push(
				"Add fixed income instruments for portfolio stability",
			);
		}
	...
	async generateProfitPicks(
		...
		const stockPicks = await this.generateStockRecommendations(
			clientRiskProfile,
			{
				count,
				timeHorizon,
	...
	private async generateStockRecommendations(
		...
		const mappedHorizon = options.timeHorizon
			? this.mapTimeHorizon(options.timeHorizon)
			: null;
		...
		const [
			...
			this.getStoreEligibleListedStocks({
				riskLevel: clientRiskProfile,
				limit: 30,
			}),
			this.getStoreEligibleMutualFunds({
				category: mfCategory,
		...
		console.log(
			`[AI Service] Fetched ${stockUniverse.length} listed stocks and ${storeEligibleFunds.length + storeEligibleAIFs.length + storeEligiblePMS.length + storeEligibleMLDs.length + storeEligibleUnlisted.length + storeEligibleBonds.length + storeEligibleNCDs.length} other products from database`,
		);
		...
		let filteredStocks = combinedUniverse.filter(
			(s) => !options.existingHoldings.includes(s.symbol),
		);

		if (mappedHorizon) {
			filteredStocks = filteredStocks.filter(
				(s) => s.timeHorizon === mappedHorizon,
			);
		}

		const targetRiskLevel =
			options.riskLevel || this.mapRiskProfile(clientRiskProfile);
		if (targetRiskLevel) {
	...
	async generateAlerts(clientId: string): Promise<PortfolioAlert[]> {
		...
		if (maxSectorConcentration > 40) {
			const topSector = Object.entries(metrics.sectorConcentration).sort(
				(a, b) => b[1] - a[1],
			)[0];
		...
		if (metrics.topHoldings[0]?.weight > 20) {
			alerts.push({
				...
				actionDescription:
					"Reduce position size to below 15% for better diversification",
				status: "active",
			});
		}
		...
		if (metrics.gainLossPercent < -10) {
			alerts.push({
				...
				actionDescription:
					"Review holdings and consider tax-loss harvesting opportunities",
		...
		if (metrics.gainLossPercent > 25) {
			alerts.push({
				...
				recommendedAction: "sell",
				actionDescription:
					"Consider booking partial profits in outperforming positions",
				status: "active",
			});
		}

		// EXIT CALL MONITORING: Individual holding target achievement and market dynamics
		const exitAlerts = await this.generateExitAlerts(
			clientId,
			holdings,
			portfolio.id,
		);
	...
	private async generateExitAlerts(
		...
		const stockDataList =
			symbols.length > 0
				? await db
						.select()
						.from(listedStocks)
						.where(inArray(listedStocks.symbol, symbols))
				: [];

		const stockDataMap = new Map(stockDataList.map((s) => [s.symbol, s]));

		for (const holding of holdings) {
			if (!holding.symbol || holding.assetType?.toLowerCase() === "cash")
				continue;
			...
			try {
				...
				const currentPrice = Number.parseFloat(stockData.currentPrice || "0");
				const returns1Y = Number.parseFloat(stockData.returns1Y || "0");
				const returns3Y = Number.parseFloat(stockData.returns3Y || "0");
				const dayChange = Number.parseFloat(stockData.dayChangePercent || "0");
				const high52Week = Number.parseFloat(stockData.weekHigh52 || "0");
				const low52Week = Number.parseFloat(stockData.weekLow52 || "0");
				const peRatio = Number.parseFloat(stockData.peRatio || "0");
				const pbRatio = Number.parseFloat(stockData.pbRatio || "0");
				const dividendYield = Number.parseFloat(stockData.dividendYield || "0");
				const volume = Number.parseFloat(stockData.averageVolume || "0");
				const avgVolume = Number.parseFloat(stockData.averageVolume || "1");
				const beta = Number.parseFloat(stockData.beta || "1");
				...
				const targetPrice = avgPrice * 1.2;
				...
				if (holdingGainPercent >= 15) {
					...
					exitAlerts.push({
						...
						recommendedAction: "sell",
						actionDescription:
							holdingGainPercent >= 25
								? `Strong gains! Sell 50-75% to lock profits. Trail stop-loss at ₹${(currentPrice * 0.92).toFixed(0)}`
								: `Book 25-50% profits. Set trailing stop-loss at ₹${(currentPrice * 0.9).toFixed(0)}`,
				...
				if (
					sector &&
					weakSectors.some((ws) =>
						sector.toLowerCase().includes(ws.toLowerCase()),
					) &&
					returns1Y < 0
				) {
					exitAlerts.push({
				...
				console.error(
					`[AI Profit Engine] Error analyzing ${holding.symbol}:`,
					error,
				);
			}
		}
		...
		console.log(
			`[AI Profit Engine] Generated ${exitAlerts.length} profit optimization alerts for ${holdings.length} holdings`,
		);
		return exitAlerts;
	}
	...
	private async findBetterAlternatives(
		...
		Array<{ symbol: string; profitScore: number; upsidePercent: number }>
	> {
		try {
			...
			return alternatives
				...
				.map((alt) => ({
					...
					profitScore: Math.min(
						95,
						70 + Number.parseFloat(alt.returns1Y || "0") * 0.5,
					),
					upsidePercent: Math.max(
						5,
						Number.parseFloat(alt.returns1Y || "0") * 0.3 + 10,
					),
				}))
	...
	calculateUnifiedProfitScore(stock: {
		...
		if (stock.peRatio && stock.peRatio > 0) {
			...
			if (peDeviation < -0.2) {
				fundamentalScore += 15;
				insights.push(
					`Undervalued: P/E ${stock.peRatio.toFixed(1)} vs sector avg ${sectorPE.toFixed(1)}`,
				);
			} else if (peDeviation > 0.5) {
				fundamentalScore -= 15;
				insights.push(
					`Overvalued: P/E ${stock.peRatio.toFixed(1)} is ${(peDeviation * 100).toFixed(0)}% above sector`,
				);
		...
		if (stock.pbRatio && stock.pbRatio > 0) {
			if (stock.pbRatio < 1) {
				fundamentalScore += 10;
				insights.push(
					`Asset discount: Trading below book value (P/B: ${stock.pbRatio.toFixed(2)})`,
				);
		...
		if (stock.dividendYield && stock.dividendYield > 0) {
			if (
				...
				stock.dividendYield < 8 &&
				stock.returns1Y > -10
			) {
				fundamentalScore += 10;
				insights.push(
					`Healthy dividend yield: ${stock.dividendYield.toFixed(1)}%`,
				);
			} else if (stock.dividendYield > 8 && stock.returns1Y < -15) {
				fundamentalScore -= 10;
				urgentActions.push(
					`Dividend trap warning: ${stock.dividendYield.toFixed(1)}% yield with poor price action`,
				);
			}
		}
		...
		if (stock.high52Week && stock.low52Week && stock.currentPrice > 0) {
			...
			if (position > 0.95) {
				...
				technicalScore -= 10;
				urgentActions.push(
					`Near 52-week low: Review fundamentals before averaging`,
				);
		...
		if (stock.volume && stock.avgVolume && stock.avgVolume > 0) {
			...
			if (volumeRatio > 3 && stock.dayChangePercent < -2) {
				technicalScore -= 10;
				urgentActions.push(
					`Heavy selling: Volume ${volumeRatio.toFixed(1)}x normal with price decline`,
				);
		...
		if (stock.dayChangePercent <= -5) {
			technicalScore -= 10;
			urgentActions.push(
				`Sharp decline: ${Math.abs(stock.dayChangePercent).toFixed(1)}% drop today`,
			);
		...
		if (stock.returns1Y > 30) {
			momentumScore += 20;
			insights.push(
				`Strong momentum: ${stock.returns1Y.toFixed(1)}% 1Y returns`,
			);
		...
		if (stock.avgPrice && stock.avgPrice > 0) {
			...
			if (holdingReturn >= 25) {
				urgentActions.push(
					`Target achieved: ${holdingReturn.toFixed(1)}% gain - consider partial profit booking`,
				);
			} else if (holdingReturn <= -15) {
				urgentActions.push(
					`Stop loss zone: ${holdingReturn.toFixed(1)}% loss - review thesis`,
				);
			}
		}
		...
		if (beta > 1.5) {
			...
			riskScore += 10;
			insights.push(
				`Low volatility: Defensive stock (Beta ${beta.toFixed(2)})`,
			);
		}
		...
		if (stock.holdingPeriodDays) {
			if (stock.holdingPeriodDays >= 365) {
				...
				stock.holdingPeriodDays >= 330 &&
				stock.holdingPeriodDays < 365
			) {
				taxEfficiencyScore += 15;
				insights.push(
					`Near LTCG: ${365 - stock.holdingPeriodDays} days to qualify`,
				);
			}
		}
		...
		if (
			...
			if (loss > 5) {
				...
				urgentActions.push(
					`Tax-loss harvest: Book ${loss.toFixed(1)}% loss before March 31`,
				);
			}
		}
	...
	async generateTalkingPoints(
		...
		talkingPoints.push({
			...
			agentScript: `Your portfolio is currently valued at ₹${Number.parseFloat(analysis.totalValue || "0").toLocaleString("en-IN")} with an overall health score of ${analysis.overallHealthScore}/100. ${analysis.aiSummary || ""}`,
			clientFacingVersion: `Portfolio value: ₹${Number.parseFloat(analysis.totalValue || "0").toLocaleString("en-IN")}. Health Score: ${analysis.overallHealthScore}/100.`,
			supportingData: {
	...
	async createProposalFromPicks(
		...
		const totalAmount = picks.reduce((sum, pick) => {
			...
			const price = Number.parseFloat(
				pick.modifiedTargetPrice || pick.currentPrice,
			);
		...
		const proposalItemsData = picks.map((pick) => {
			const quantity = pick.modifiedQuantity || pick.proposedQuantity || 10;
			const price = Number.parseFloat(pick.currentPrice);
			const itemAmount = quantity * price;
			const allocationPct =
				totalAmount > 0 ? (itemAmount / totalAmount) * 100 : 0;
