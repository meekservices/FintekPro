...
class OptimisticExplanationTemplates {
  ...
  generateExplanation(
    ...
    } else if (mode === RECOMMENDATION_MODE.CONSERVATIVE) {
  ...
  private generateOptimisticExplanation(params: ExplanationParams): GeneratedExplanation {
    const { productName, productType, upsideScore, upsideBreakdown, sector } = params;
  ...
  private generateOptimisticHeadline(productType: UnifiedProductType, upsideScore: number): string {
    const strength = upsideScore >= 80 ? "Strong" : upsideScore >= 60 ? "Favorable" : "Moderate";
  ...
  private generateOptimisticKeyPoints(params: ExplanationParams): string[] {
    const { productType, upsideBreakdown, expectedReturnMax, rating, yieldOrReturn, sector } = params;
    ...
    if (expectedReturnMax && expectedReturnMax > 12) {
      keyPoints.push(`Return potential of up to ${expectedReturnMax.toFixed(1)}% aligns with growth objectives`);
    }

    if (yieldOrReturn && yieldOrReturn > 7) {
      keyPoints.push(`Yield of ${yieldOrReturn.toFixed(2)}% provides attractive income opportunity`);
    }

    if (rating && (rating.includes('AAA') || rating.includes('5') || rating.toLowerCase().includes('gold'))) {
      keyPoints.push(`Top-tier rating reflects quality characteristics`);
    }

    if (sector) {
      const highGrowthSectors = ['technology', 'fintech', 'healthcare', 'renewable', 'ev', 'ai'];
  ...
  private getFactorDescription(factor: string, score: number, productType: UnifiedProductType): string {
    ...
    return factorDescriptions[factor] || `Strong ${factor.replace(/([A-Z])/g, ' $1').toLowerCase()} of ${score}`;
  }

  private generateOptimisticBody(params: ExplanationParams): string {
    const { productName, productType, suitabilityScore, upsideScore, finalScore, sector } = params;

    const suitabilityDesc = suitabilityScore >= 80 
      ? "strongly aligns with" 
      : suitabilityScore >= 60 
        ? "appropriately matches" 
        : "is compatible with";

    const upsideDesc = upsideScore >= 80
  ...
  private generateBalancedExplanation(params: ExplanationParams): GeneratedExplanation {
    const { productName, productType, suitabilityScore, upsideScore, finalScore } = params;

    const headline = `${productName} - Balanced Selection for Your Portfolio`;

    const body = `This ${this.getProductTypeLabel(productType)} has been selected based on a balanced evaluation of ` +
  ...
  private generateConservativeExplanation(params: ExplanationParams): GeneratedExplanation {
    ...
    const body = `This ${this.getProductTypeLabel(productType)} has been selected with emphasis on ` +
  ...
  generateBatchExplanations(
    ...
    for (const product of products) {
      explanations.set(product.product_id, this.generateExplanation(product, mode));
    }
...
export const optimisticExplanationTemplates = new OptimisticExplanationTemplates();
