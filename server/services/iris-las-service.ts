...
async function withRetry<T>(fn: () => Promise<T>, maxRetries = 3): Promise<T> {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      lastErr = err;
      const isTransient = !err?.response?.status || err?.response?.status >= 500;
      if (!isTransient || attempt === maxRetries) throw err;
      const delay = Math.pow(2, attempt) * 300;
      await new Promise(r => setTimeout(r, delay));
      logger.warn('[IrisLAS] Retrying after transient error', { attempt, error: err?.message });
    }
  }
  throw lastErr;
}
...
class IrisLasService {
   ...
   */
  async checkMfEligibility(pan: string, folioNos?: string[]): Promise<LasEligibilityResult> {
    ...
    if (!irisKfintechService.isConfigured) {
      return {
        ...
        warning: 'IRIS KFintech not configured. Set IRIS_USERNAME and IRIS_PASSWORD.',
      };
    }
    ...
    try {
      ...
      const folios: LasEligibilityResult['eligibleFolios'] = (data?.eligibleFolios ?? data?.folios ?? []).map((f: any) => ({
      ...
      const totalPledgeableValue = folios.reduce((s, f) => s + f.pledgeableValue, 0);
      const totalMaxLoanAmount = folios.reduce((s, f) => s + f.maxLoanAmount, 0);
      ...
      return {
        ...
        engine_version: ENGINE_VERSION,
        calculation_timestamp: ts,
        warning: err?.response?.data?.message ?? err?.message ?? 'IRIS eligibility check failed',
      };
    }
  }
   ...
   */
  async checkSecuritiesEligibility(pan: string, dpId?: string): Promise<LasEligibilityResult> {
    const ts = new Date().toISOString();
    ...
    if (!irisKfintechService.isConfigured) {
      return {
        success: false, pan,
        totalPledgeableValue: 0, totalMaxLoanAmount: 0,
        engine_version: ENGINE_VERSION, calculation_timestamp: ts,
    ...
    try {
      ...
      const securities: LasEligibilityResult['eligibleSecurities'] = (data?.eligibleSecurities ?? data?.securities ?? []).map((s: any) => ({
        isin: s.isin,
      ...
      const totalPledgeableValue = securities.reduce((s, x) => s + x.pledgeableValue, 0);
      const totalMaxLoanAmount = securities.reduce((s, x) => s + x.maxLoanAmount, 0);

      return {
        success: true, pan, eligibleSecurities: securities,
        totalPledgeableValue, totalMaxLoanAmount,
        engine_version: ENGINE_VERSION, calculation_timestamp: ts,
      };
    } catch (err: any) {
      ...
      return {
        ...
        totalPledgeableValue: 0, totalMaxLoanAmount: 0,
        engine_version: ENGINE_VERSION, calculation_timestamp: ts,
        warning: err?.response?.data?.message ?? err?.message ?? 'Securities eligibility check failed',
      };
    }
  }
  ...
  async initiateMfPledge(opts: {
    ...
    const [pledge] = await db.insert(irisLasPledges).values({
      userId: opts.userId,
      pan: opts.pan,
    ...
    try {
      ...
      const irisPledgeId = data?.pledgeId ?? data?.pledge_id ?? data?.referenceId;
      const irisStatus = data?.status ?? 'initiated';

      await db.update(irisLasPledges)
        .set({
          irisPledgeId,
          pledgeStatus: irisStatus,
          totalPledgedValue: String(data?.totalPledgedValue ?? data?.total_pledged_value ?? opts.loanAmount),
          maxLoanEligible: String(data?.maxLoanAmount ?? data?.max_loan_amount ?? opts.loanAmount),
      ...
      await db.update(irisLasPledges)
        .set({ pledgeStatus: 'failed', irisResponse: { error: err?.message } as any, updatedAt: new Date() })
        .where(eq(irisLasPledges.id, pledge.id));
      ...
      return {
        ...
        localPledgeId: pledge.id,
        pledgeStatus: 'failed',
        message: err?.response?.data?.message ?? err?.message ?? 'Pledge initiation failed',
        engine_version: ENGINE_VERSION,
        calculation_timestamp: ts,
      };
    }
  ...
  async initiateSecuritiesPledge(opts: {
    ...
    try {
      ...
      return {
        success: true, localPledgeId: pledge.id, irisPledgeId,
        pledgeStatus: irisStatus,
        engine_version: ENGINE_VERSION, calculation_timestamp: ts,
      };
    } catch (err: any) {
      await db.update(irisLasPledges)
        ...
        .where(eq(irisLasPledges.id, pledge.id));
      return {
        success: false, localPledgeId: pledge.id, pledgeStatus: 'failed',
  ...
  async applyForLoan(opts: {
    ...
    if (!pledge || pledge.userId !== opts.userId) {
      return {
        success: false, localLoanId: '', loanStatus: 'failed',
    ...
    const loanType = pledge.pledgeType === 'mutual_fund' ? 'against_mutual_funds' : 'against_securities';

    const [loan] = await db.insert(irisLasLoans).values({
      userId: opts.userId,
      pan: opts.pan,
    ...
    try {
      ...
      const sanctionedAmount = data?.sanctionedAmount ?? data?.sanctioned_amount;
      const interestRate = data?.interestRate ?? data?.interest_rate;

      await db.update(irisLasLoans)
        .set({
          irisLoanId,
          loanStatus: irisStatus,
          sanctionedAmount: sanctionedAmount ? String(sanctionedAmount) : undefined,
      ...
      return {
        success: true, localLoanId: loan.id, irisLoanId,
        loanStatus: irisStatus,
      ...
      await db.update(irisLasLoans)
        .set({ loanStatus: 'failed', irisResponse: { error: err?.message } as any, updatedAt: new Date() })
        .where(eq(irisLasLoans.id, loan.id));

      logger.error('[IrisLAS] Loan application failed', {
        event: 'IRIS_LAS_LOAN_FAILED',
        user_id: opts.userId, localLoanId: loan.id,
        error: err?.response?.data ?? err?.message,
      });
      return {
        success: false, localLoanId: loan.id, loanStatus: 'failed',
        message: err?.response?.data?.message ?? err?.message ?? 'Loan application failed',
        disclaimer: LOAN_DISCLAIMER,
        engine_version: ENGINE_VERSION, calculation_timestamp: ts,
      };
    }
  ...
  async getPledgeStatus(localPledgeId: string, userId: string): Promise<any> {
    const [pledge] = await db
      .select()
      .from(irisLasPledges)
      .where(and(eq(irisLasPledges.id, localPledgeId), eq(irisLasPledges.userId, userId)))
      .limit(1);

    if (!pledge) throw new Error('Pledge not found');

    if (pledge.irisPledgeId) {
      const data = await withRetry(() => irisKfintechService.getPledgeStatus(pledge.irisPledgeId!));
  ...
  async getLoanStatus(localLoanId: string, userId: string): Promise<any> {
    const [loan] = await db
      .select()
      .from(irisLasLoans)
      .where(and(eq(irisLasLoans.id, localLoanId), eq(irisLasLoans.userId, userId)))
      .limit(1);

    if (!loan) throw new Error('Loan not found');

    if (loan.irisLoanId) {
      const data = await withRetry(() => irisKfintechService.getLoanStatus(loan.irisLoanId!));
      const irisData = data as any;
      await db.update(irisLasLoans)
        .set({
          loanStatus: irisData?.status ?? loan.loanStatus,
          disbursedAmount: irisData?.disbursedAmount ? String(irisData.disbursedAmount) : loan.disbursedAmount,
          outstandingAmount: irisData?.outstandingAmount ? String(irisData.outstandingAmount) : loan.outstandingAmount,
          disbursementDate: irisData?.disbursementDate ? new Date(irisData.disbursementDate) : loan.disbursementDate,
          updatedAt: new Date(),
        })
   ...
   */
  async getLoanStatement(userId: string, pan: string, loanId?: string): Promise<any> {
    const data = await withRetry(() => irisKfintechService.getLoanStatement(pan, loanId));
    return {
      data,
      engine_version: ENGINE_VERSION,
  ...
  async repayLoan(opts: {
    ...
    const [loan] = await db
      .select()
      .from(irisLasLoans)
      .where(and(eq(irisLasLoans.id, opts.localLoanId), eq(irisLasLoans.userId, opts.userId)))
      .limit(1);

    if (!loan?.irisLoanId) throw new Error('Loan not found or not yet active in IRIS');
    ...
    return { success: true, data, engine_version: ENGINE_VERSION, calculation_timestamp: new Date().toISOString() };
  }
  ...
  async releasePledge(opts: {
    ...
    const [pledge] = await db
      .select()
      .from(irisLasPledges)
      .where(and(eq(irisLasPledges.id, opts.localPledgeId), eq(irisLasPledges.userId, opts.userId)))
      .limit(1);

    if (!pledge?.irisPledgeId) throw new Error('Pledge not found or not yet active in IRIS');
  ...
  async listUserPledges(userId: string, pan?: string): Promise<any[]> {
    ...
    return db.select().from(irisLasPledges).where(conditions).orderBy(desc(irisLasPledges.createdAt));
  }
  ...
  async listUserLoans(userId: string, pan?: string): Promise<any[]> {
    ...
    return db.select().from(irisLasLoans).where(conditions).orderBy(desc(irisLasLoans.createdAt));
  }
}
