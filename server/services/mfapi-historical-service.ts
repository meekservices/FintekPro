...
class MFApiHistoricalService {
	...
	private pendingRequests: Map<string, Promise<CalculatedReturns | null>> =
		new Map();
	...
	private findClosestNav(
		...
		for (const point of navData) {
			...
			if (
				pointDate.getTime() <
				targetTime - maxDaysOffset * 24 * 60 * 60 * 1000
			) {
				break;
			}
		}
	...
	async fetchHistoricalNav(schemeCode: string): Promise<MFApiResponse | null> {
		try {
			...
			console.error(
				`[MFApi] Error fetching historical NAV for ${schemeCode}:`,
				error.message,
			);
			return null;
		}
	...
	async calculateReturns(
		schemeCode: string,
	): Promise<CalculatedReturns | null> {
	...
	private sanitizeReturns(returns: number, category: string = ""): number {
		...
		if (Math.abs(returns) > maxReasonableReturn) {
			console.warn(
				`[MFApi] Unrealistic return ${returns}% detected, likely data error - returning 0`,
			);
			return 0;
		}
		return returns;
	...
	private async doCalculateReturns(
		schemeCode: string,
	): Promise<CalculatedReturns | null> {
		...
		if (
			...
			console.warn(
				`[MFApi] No historical data available for scheme ${schemeCode}`,
			);
			return null;
		}
		...
		const currentNav = Number.parseFloat(latestNav.nav);
		...
		const nav1yAgo = nav1yAgoPoint
			? Number.parseFloat(nav1yAgoPoint.nav)
			: null;
		const nav3yAgo = nav3yAgoPoint
			? Number.parseFloat(nav3yAgoPoint.nav)
			: null;
		const nav5yAgo = nav5yAgoPoint
			? Number.parseFloat(nav5yAgoPoint.nav)
			: null;
		...
		if (hasNav1yError || hasNav3yError || hasNav5yError) {
			console.warn(
				`[MFApi] NAV data anomaly detected for ${schemeCode} - possible decimal point error in source data`,
			);
		}

		let returns1y =
			nav1yAgo && !hasNav1yError
				? this.calculateCAGR(currentNav, nav1yAgo, 1)
				: 0;
		let returns3y =
			nav3yAgo && !hasNav3yError
				? this.calculateCAGR(currentNav, nav3yAgo, 3)
				: 0;
		let returns5y =
			nav5yAgo && !hasNav5yError
				? this.calculateCAGR(currentNav, nav5yAgo, 5)
				: 0;
		...
		const result: CalculatedReturns = {
			...
			...(nav1yAgoPoint &&
				!hasNav1yError && {
					nav1yAgo: { nav: nav1yAgo!, date: nav1yAgoPoint.date },
				}),
			...(nav3yAgoPoint &&
				!hasNav3yError && {
					nav3yAgo: { nav: nav3yAgo!, date: nav3yAgoPoint.date },
				}),
			...(nav5yAgoPoint &&
				!hasNav5yError && {
					nav5yAgo: { nav: nav5yAgo!, date: nav5yAgoPoint.date },
				}),
		};

		console.log(
			`[MFApi] Calculated returns for ${schemeCode}: 1Y=${result.returns1y}%, 3Y=${result.returns3y}%, 5Y=${result.returns5y}%`,
		);

		return result;
	}

	async calculateReturnsBatch(
		schemeCodes: string[],
	): Promise<Map<string, CalculatedReturns>> {
