	...
	async fetchCAS(request: CASFetchRequest): Promise<CASFetchResponse> {
		try {
			...
			if (!this.hasValidCredentials()) {
				throw new Error(
					"BSE STAR CAS service not configured. Set BSE_USER_ID, BSE_MEMBER_ID, BSE_PASSWORD, and BSE_PASS_KEY for mutual fund data.",
				);
			}
			...
			const rtaSummary = {
				...
				franklinHoldings: holdings.filter((h) => h.rtaCode === "FRANKLIN")
					.length,
			};

			console.log(
				`✅ Fetched ${holdings.length} mutual fund holdings (Total Value: ₹${totalValue.toFixed(2)})`,
			);
  ...
  <soap:Body>
	...
	private async parseXMLResponse(xmlData: string): Promise<any> {
		try {
			...
			const casResult =
				body?.GetCASSummaryResponse?.GetCASSummaryResult || body;
			...
			if (status !== "Success" && status !== "100") {
				throw new Error(
					casResult.Message || casResult.message || "CAS fetch failed",
				);
			}

			// Extract folios from XML structure
			const foliosData = casResult.Folios?.Folio || casResult.Data?.Folio || [];
			const folios = Array.isArray(foliosData)
				? foliosData
				: [foliosData].filter(Boolean);
			...
			return {
				...
				Folios: folios.map((f: any) => ({
					...
					InvestedAmount:
						f.InvestedAmount || f.investedAmount || f.InvestedValue || "0",
	 ...
	 */
	private async parseCASResponse(
		apiResponse: any,
		...
		try {
			...
			for (const folio of folios) {
				const holding: MutualFundHolding = {
					...
					units: Number.parseFloat(folio.Units || 0),
					nav: Number.parseFloat(folio.NAV || folio.CurrentNAV || 0),
					currentValue: Number.parseFloat(
						folio.MarketValue || folio.CurrentValue || 0,
					),
					investedAmount: Number.parseFloat(
						folio.InvestedAmount || folio.InvestedValue || 0,
					),
					returns: 0, // Calculated below
					returnsPercentage: 0, // Calculated below
					averageNav: Number.parseFloat(folio.AverageNAV || folio.AvgCost || 0),
	...
	private identifyRTA(registrarName: string): string {
		...
		if (name.includes("KARVY") || name.includes("KFINTECH")) {
			return "KARVY";
		}
	 ...
	 */
	async fetchHoldingsByRTA(
		panNumber: string,
		rtaCode: "CAMS" | "KARVY" | "FRANKLIN",
	): Promise<MutualFundHolding[]> {
		const casResponse = await this.fetchCAS({ panNumber, name: "", dob: "" });
	...
	async fetchTransactionStatement(
		...
		try {
			...
			if (!this.hasValidCredentials()) {
				throw new Error(
					"BSE STAR CAS service not configured. Set BSE_USER_ID, BSE_MEMBER_ID, BSE_PASSWORD, and BSE_PASS_KEY for mutual fund transactions.",
				);
			}

			// Production: Call BSE STAR Transaction Statement API
			const transactionResponse = await this.callBSETransactionAPI(
				request,
				startDate,
				endDate,
			);
			...
			// Parse and normalize the response
			const transactions =
				await this.parseTransactionResponse(transactionResponse);
  ...
  <soap:Body>
	...
	private async parseTransactionXMLResponse(xmlData: string): Promise<any> {
		try {
			...
			const body = parsed?.Envelope?.Body || parsed?.Body || parsed;
			const txResult =
				body?.GetTransactionStatementResponse?.GetTransactionStatementResult ||
				body;
			...
			if (status !== "Success" && status !== "100") {
				throw new Error(
					txResult.Message || txResult.message || "Transaction fetch failed",
				);
			}

			const txData =
				txResult.Transactions?.Transaction || txResult.Data?.Transaction || [];
			const transactions = Array.isArray(txData)
				? txData
				: [txData].filter(Boolean);
	 ...
	 */
	private async parseTransactionResponse(
		apiResponse: any,
		...
		try {
			...
			for (const tx of txData) {
				const transaction: MutualFundTransaction = {
					...
					units: Number.parseFloat(tx.Units || 0),
					nav: Number.parseFloat(tx.NAV || 0),
					amount: Number.parseFloat(tx.Amount || tx.GrossAmount || 0),
					stampDuty: Number.parseFloat(tx.StampDuty || 0),
					stt: Number.parseFloat(tx.STT || 0),
					tds: Number.parseFloat(tx.TDS || 0),
					netAmount: Number.parseFloat(tx.NetAmount || tx.Amount || 0),
	...
	private normalizeTransactionType(
		type: string,
	): MutualFundTransaction["transactionType"] {
		...
		if (
			...
			return "redemption";
		if (t.includes("SWITCH") && (t.includes("IN") || t.includes("TO")))
			return "switch_in";
		if (t.includes("SWITCH") && (t.includes("OUT") || t.includes("FROM")))
			return "switch_out";
	...
	async fetchCASWithTransactions(
		...
		console.log(
			`📊 Fetching unified CAS (Holdings + Transactions) for PAN: ${request.panNumber.slice(0, 4)}****`,
		);
		...
		console.log(
			`✅ Unified CAS fetch complete: ${holdingsResult.holdings.length} holdings, ${transactionsResult.transactions.length} transactions`,
		);
