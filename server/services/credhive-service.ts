...
class CredhiveService {
	...
	async searchCompanies(query: string): Promise<CredhiveSearchResponse> {
		if (!this.available) {
			return {
				...
				error: "CREDHIVE_API_KEY not configured",
				data: [],
			};
		}
		try {
			const isCin = /^[A-Z]{1}[0-9]{5}[A-Z]{2}[0-9]{4}[A-Z]{3}[0-9]{6}$/.test(
			...
			if (status === 401 || status === 403) {
				return {
					success: false,
					error: "Invalid CREDHIVE_API_KEY — check your credentials",
					isApiKeyMissing: true,
				};
			}
			return {
				success: false,
				error: err?.message || "Credhive search failed",
			};
		}
	}
	...
	async getCompanyProfile(
		cin: string,
		forceRefresh = false,
	): Promise<CredhiveProfileResponse> {
		if (!this.available) {
			return {
				success: false,
				isApiKeyMissing: true,
		...
		if (!forceRefresh) {
			const cached =
				await distributedCache.getJson<CredhiveCompanyProfile>(cacheKey);
			if (cached) {
		...
		return guardedExecution(
			async () => {
				...
				const profile: CredhiveCompanyProfile = {
					...
					date_of_incorporation:
						d.date_of_incorporation || d.incorporation_date,
					registered_address: d.registered_address
						? typeof d.registered_address === "string"
							? d.registered_address
							: [
									d.registered_address.address_line,
									d.registered_address.city,
									d.registered_address.state,
									d.registered_address.pincode,
								]
									.filter(Boolean)
									.join(", ")
						: undefined,
			...
			},
			{
				...
				fallback: {
					success: false,
					error: "Credhive profile fetch failed — using fallback",
				} as CredhiveProfileResponse,
	...
	async getFinancials(
		cin: string,
		forceRefresh = false,
	): Promise<CredhiveFinancialsResponse> {
		if (!this.available) {
			return {
				success: false,
				isApiKeyMissing: true,
		...
		try {
			const response = await this.client.get(
				`/company/${encodeURIComponent(cin)}/financials`,
			);
			const raw: any[] =
				response.data?.data || response.data?.financials || response.data || [];
			const mapped: CredhiveFinancialStatement[] = raw.map((r: any) => ({
				...
				short_term_debt: this._num(
					r.short_term_debt || r.short_term_borrowings,
				),
				cash_and_equivalents: this._num(
					r.cash_and_equivalents || r.cash_and_bank_balances,
				),
				operating_cash_flow: this._num(
					r.operating_cash_flow || r.cash_from_operations,
				),
				investing_cash_flow: this._num(
					r.investing_cash_flow || r.cash_from_investing,
				),
				financing_cash_flow: this._num(
					r.financing_cash_flow || r.cash_from_financing,
				),
			...
			return {
				success: false,
				error: err?.message || "Credhive financials fetch failed",
			};
		}
	}
	...
	async getDirectors(
		cin: string,
		forceRefresh = false,
	): Promise<CredhiveDirectorsResponse> {
		if (!this.available) {
			return {
				success: false,
				isApiKeyMissing: true,
		...
		if (!forceRefresh) {
			const cached =
				await distributedCache.getJson<CredhiveDirector[]>(cacheKey);
			if (cached) {
		...
		try {
			const response = await this.client.get(
				`/company/${encodeURIComponent(cin)}/directors`,
			);
			const raw: any[] =
				response.data?.data || response.data?.directors || response.data || [];
			...
			return {
				success: false,
				error: err?.message || "Credhive directors fetch failed",
			};
		}
	}
	...
	async getCompliance(
		cin: string,
		forceRefresh = false,
	): Promise<CredhiveComplianceResponse> {
		if (!this.available) {
			return {
				success: false,
				isApiKeyMissing: true,
		...
		if (!forceRefresh) {
			const cached =
				await distributedCache.getJson<CredhiveComplianceData>(cacheKey);
			if (cached) {
		...
		try {
			const response = await this.client.get(
				`/company/${encodeURIComponent(cin)}/compliance`,
			);
			const d: any = response.data?.data || response.data;
			const signals: CredhiveComplianceSignal[] = (
				d?.signals ||
				d?.risk_signals ||
				[]
			).map((s: any) => ({
			...
			return {
				success: false,
				error: err?.message || "Credhive compliance fetch failed",
			};
		}
	}
	 ...
	 */
	async searchCompanyByNameOrCIN(
		query: string,
	): Promise<CredhiveSearchResult[]> {
	...
	async getCompanyFinancials(
		...
		return {
			...
			data: {
				...
				roe:
					latest.networth && latest.net_profit
						? (latest.net_profit / latest.networth) * 100
						: undefined,
				roce: undefined,
				debtToEquityRatio:
	...
	async getFullEnrichment(cin: string): Promise<{
		...
		const [profileRes, financialsRes, directorsRes, complianceRes] =
		...
		const profile =
			profileRes.status === "fulfilled" && profileRes.value.success
				...
				: null;
		const financials =
			financialsRes.status === "fulfilled" && financialsRes.value.success
				? financialsRes.value.data || []
				: [];
		const directors =
			directorsRes.status === "fulfilled" && directorsRes.value.success
				? directorsRes.value.data || []
				: [];
		const compliance =
			complianceRes.status === "fulfilled" && complianceRes.value.success
				? complianceRes.value.data || null
				: null;
		...
		const baseDetails = profile
			? {
					...
					financials: financials.map((f) => ({
						...
						networth: f.networth,
						roe:
							f.networth && f.net_profit
								? (f.net_profit / f.networth) * 100
								: undefined,
						debtToEquityRatio:
							f.total_debt && f.networth
								? f.total_debt / f.networth
								: undefined,
					})),
					directors,
	...
	extractEnrichmentData(
		enrichment: Awaited<ReturnType<CredhiveService["getFullEnrichment"]>>,
	): {
		directors: CredhiveDirector[];
	...
	async searchAndEnrich(filters: {
				...
				batch.map(async (company) => {
					...
					const profile =
						profileRes.status === "fulfilled" && profileRes.value.success
							...
							: null;
					const fins =
						finRes.status === "fulfilled" && finRes.value.success
							? finRes.value.data || []
							: [];
		...
		if (filters.minRevenue)
			filtered = filtered.filter(
				(c) => (c.latestRevenue || 0) >= filters.minRevenue!,
			);
		if (filters.minProfit)
			filtered = filtered.filter(
				(c) => (c.latestNetProfit || 0) >= filters.minProfit!,
			);
		if (filters.minEbitda)
			filtered = filtered.filter(
				(c) => (c.latestEbitda || 0) >= filters.minEbitda!,
			);
		...
		return {
			...
			companyDetails: profile
				? {
						...profile,
						probe42Score: null,
						riskLevel: compliance?.overall_risk || "medium",
					}
				: null,
			riskFlags,
		};
	...
	async searchDirectorsByName(
		...
		return {
			directors: [],
			available: false,
			error: "Director search by name is not supported by Credhive",
		};
	}
	...
	calculateLeadScore(company: any): number {
		let score = 20;
		const fin = Array.isArray(company?.financials)
	...
	calculateInvestableSurplus(financial: any): number {
		...
		const netProfit =
			financial.netProfit ?? financial.net_profit ?? financial.pat ?? 0;
		const operatingCF =
			financial.operatingCashFlow ?? financial.operating_cash_flow ?? 0;
...
export function computeIdentityConfidence(company: {
	...
	const breakdown = {
		...
		legalName: {
			present: false,
			quality: "missing" as "complete" | "partial" | "missing",
			score: 0,
		},
	...
	if (
		company.cin &&
		/^[A-Z]\d{5}[A-Z]{2}\d{4}[A-Z]{3}\d{6}$/.test(company.cin)
	) {
	...
	if (nameToCheck) {
		...
		const hasLegalSuffix =
			/\b(pvt|private|ltd|limited|llp|inc|corp|plc)\b/i.test(nameToCheck);
	...
	if (totalScore >= 0.8) {
		...
		enrichmentBlockReason =
			"Insufficient identity data — provide CIN, ISIN, or valid legal name";
	}
	...
	return {
		...
		breakdown: {
			...
			legalName: {
				present: breakdown.legalName.present,
				quality: breakdown.legalName.quality,
				score: breakdown.legalName.score,
			},
 ...
 */
export function normalizeCompanyResult(
	profile: CredhiveCompanyProfile | null,
): {
	cin: string;
	companyName: string;
...
export function getCredhiveService() {
	return credhiveService;
}
