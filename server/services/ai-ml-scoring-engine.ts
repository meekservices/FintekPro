...
const FEATURE_KEYS = ['pe', 'returns1y', 'returns3y', 'volatility', 'sharpeRatio', 'yield', 'confidenceScore'];

const ASSET_CLASSES: AssetClass[] = [
  'listed_stocks', 'mutual_funds', 'bonds', 'unlisted',
  'global_stocks', 'etfs', 'reits_invits', 'fixed_deposits', 'sgb', 'derivatives'
];

const MODEL_CACHE_TTL_MS = 60 * 60 * 1000;

class AIMLScoringEngine {
  private modelCache: Map<string, { model: ScoringModel; cachedAt: number }> = new Map();

  async buildTrainingDataset(config: TrainingConfig): Promise<TrainingExample[]> {
    ...
    for (const pick of picks) {
      ...
      const returnPct = pick.returnPct ? parseFloat(pick.returnPct) : null;
      if (returnPct === null || isNaN(returnPct)) continue;
  ...
  async trainModel(config: TrainingConfig): Promise<ScoringModel> {
    ...
    if (examples.length < minSamples) {
      throw new Error(`Insufficient training data for ${assetClass}: ${examples.length} samples (need ${minSamples})`);
    }
    ...
    for (let i = 0; i < n; i++) {
      ...
      for (const fname of featureNames) {
        ...
        row.push(this.standardizeFeature(raw, featureMeans[fname], featureStdDevs[fname]));
      }
    ...
    for (let round = 0; round < maxStumps; round++) {
      ...
      const stump = this.findBestStump(featureMatrix, featureNames, residuals, sampleIndices);
      ...
      for (let i = 0; i < n; i++) {
        ...
        const prediction = featureVal <= stump.threshold ? stump.leftValue : stump.rightValue;
    ...
    const predictions = examples.map((_, i) =>
      this.predictWithStumps(stumps, this.featureRowToRecord(featureMatrix[i], featureNames), intercept, featureMeans, featureStdDevs, true)
    );
    ...
    console.log(`✅ [AIMLScoringEngine] Trained model for ${assetClass}: RMSE=${rmse.toFixed(4)}, R²=${model.trainingMetrics.r2.toFixed(4)}, DA=${(directionalAccuracy * 100).toFixed(1)}%, samples=${n}`);
  ...
  async score(
    ...
    for (const stump of model.stumps) {
      ...
      const prediction = stdVal <= stump.threshold ? stump.leftValue : stump.rightValue;
      const contribution = stump.weight * prediction;
      featureContributions[stump.featureName] = (featureContributions[stump.featureName] || 0) + contribution;
    }
    ...
    let confidence = Math.min(95, Math.max(10, Math.round(
      30 + r2Factor * 40 + Math.min(predMagnitude * 200, 25)
    )));
    ...
    for (const assetClass of ASSET_CLASSES) {
      try {
        ...
        if (err.message?.includes('Insufficient training data')) {
          console.log(`[AIMLScoringEngine] Skipping ${assetClass}: ${err.message}`);
        } else {
          console.error(`[AIMLScoringEngine] Error training ${assetClass}:`, err);
        }
      }
    }

    console.log(`✅ [AIMLScoringEngine] Trained ${models.length}/${ASSET_CLASSES.length} models`);
    return models;
  }
  ...
  async getActiveModel(assetClass: string): Promise<ScoringModel | null> {
    ...
    try {
      ...
      console.error(`[AIMLScoringEngine] Failed to load model for ${assetClass}:`, err);
      return null;
    }
  }
  ...
  async evaluateModel(modelName: string, modelVersion: string): Promise<any> {
    try {
      ...
      const predictions = logs.map(l => parseFloat(l.predictedReturn || '0'));
      const actuals = logs.map(l => parseFloat(l.actualReturn || '0'));
  ...
  ): { folds: number; avgRmse: number; avgR2: number; avgDirectionalAccuracy: number } {
    const nFolds = config.nFolds ?? 5;
    const maxStumps = config.maxStumps ?? 50;
    const learningRate = config.learningRate ?? 0.1;
    ...
    for (let fold = 0; fold < nFolds; fold++) {
      ...
      const trainMatrix: number[][] = trainSet.map(ex =>
        featureNames.map(fname =>
          this.standardizeFeature(ex.features[fname] ?? 0, fMeans[fname], fStds[fname])
        )
      );
      ...
      for (let round = 0; round < maxStumps; round++) {
        const stump = this.findBestStump(trainMatrix, featureNames, fResiduals, trainIndices);
        stump.weight = learningRate;
      ...
      const testVariance = testTargets.length > 1 ? ss.variance(testTargets) : 1;
      foldR2s.push(testVariance > 0 ? Math.max(0, 1 - testMSE / testVariance) : 0);

      let correctDir = 0;
      for (let i = 0; i < testPredictions.length; i++) {
        if ((testPredictions[i] >= 0 && testTargets[i] >= 0) || (testPredictions[i] < 0 && testTargets[i] < 0)) {
          correctDir++;
        }
      }
  ...
  private standardizeFeature(value: number, mean: number, stddev: number): number {
  ...
  private predictWithStumps(
    ...
    for (const stump of stumps) {
      ...
      const stumpPrediction = featureVal <= stump.threshold ? stump.leftValue : stump.rightValue;
  ...
  private featureRowToRecord(row: number[], featureNames: string[]): Record<string, number> {
