...
const BENCHMARK_NORMALIZATION_RULES: Array<{
	pattern: RegExp;
	indexCode: string;
	priority: number;
}> = [
	{
		pattern:
			/\bNIFTY\s*50\b(?!\s*(?:Next|Midcap|Smallcap|Equal|Value|Growth|100|200|500))/i,
		indexCode: "NIFTY50",
		priority: 100,
	},
	{ pattern: /\bNIFTY\s*100\b/i, indexCode: "NIFTY100", priority: 95 },
	{
		pattern: /\bNIFTY\s*Next\s*50\b/i,
	...
	{
		pattern: /\bNIFTY\s*Midcap\s*150\b/i,
		indexCode: "NIFTY_MIDCAP_150",
		priority: 95,
	},
	{
		pattern: /\bNIFTY\s*Midcap\s*100\b/i,
		indexCode: "NIFTY_MIDCAP_100",
		priority: 90,
	},
	{
		pattern: /\bNIFTY\s*Midcap\s*50\b/i,
		indexCode: "NIFTY_MIDCAP_50",
		priority: 85,
	},
	{
		pattern: /\bNIFTY\s*Smallcap\s*250\b/i,
		indexCode: "NIFTY_SMALLCAP_250",
		priority: 95,
	},
	{
		pattern: /\bNIFTY\s*Smallcap\s*100\b/i,
		indexCode: "NIFTY_SMALLCAP_100",
		priority: 90,
	},
	{
		pattern: /\bNIFTY\s*Smallcap\s*50\b/i,
		indexCode: "NIFTY_SMALLCAP_50",
		priority: 85,
	},
	{ pattern: /\bNIFTY\s*Bank\b/i, indexCode: "NIFTY_BANK", priority: 95 },
	{
		pattern: /\bNIFTY\s*Financial\s*Services?\b/i,
		indexCode: "NIFTY_FINANCIAL",
		priority: 90,
	},
	{
		pattern: /\bNIFTY\s*IT\b|\bNIFTY\s*Information\s*Technology\b/i,
		indexCode: "NIFTY_IT",
		priority: 95,
	},
	{
		pattern: /\bNIFTY\s*Pharma\b|\bNIFTY\s*Healthcare\b/i,
		indexCode: "NIFTY_PHARMA",
		priority: 95,
	},
	{
		pattern: /\bNIFTY\s*Auto\b|\bNIFTY\s*Automobile\b/i,
	...
	{
		pattern: /\bNIFTY\s*Infra\b|\bNIFTY\s*Infrastructure\b/i,
		indexCode: "NIFTY_INFRA",
		priority: 90,
	},
	{ pattern: /\bNIFTY\s*Realty\b/i, indexCode: "NIFTY_REALTY", priority: 90 },
	{
		pattern: /\bNIFTY\s*PSE\b|\bNIFTY\s*PSU\s*Bank\b/i,
		indexCode: "NIFTY_PSE",
		priority: 90,
	},
	{
		pattern: /\bNIFTY\s*Private\s*Bank\b/i,
		indexCode: "NIFTY_PVT_BANK",
		priority: 90,
	},
	{
		pattern: /\bNIFTY\s*Consumption\b/i,
		indexCode: "NIFTY_CONSUMPTION",
		priority: 85,
	},
	{
		pattern: /\bNIFTY\s*Commodities?\b/i,
		indexCode: "NIFTY_COMMODITIES",
		priority: 85,
	},
	{ pattern: /\bNIFTY\s*MNC\b/i, indexCode: "NIFTY_MNC", priority: 85 },
	{
		pattern: /\bNIFTY\s*Services\s*Sector\b/i,
		indexCode: "NIFTY_SERVICES",
		priority: 85,
	},
	{
		pattern: /\bNIFTY\s*Large\s*Midcap\s*250\b/i,
		indexCode: "NIFTY_LARGEMIDCAP_250",
		priority: 90,
	},
	{
		pattern: /\bNIFTY\s*Midcap\s*Select\b/i,
		indexCode: "NIFTY_MIDCAP_SELECT",
		priority: 85,
	},
	{
		pattern: /\bBSE\s*Sensex\b|\bS&P\s*BSE\s*Sensex\b|\bSENSEX\b/i,
		indexCode: "SENSEX",
		priority: 100,
	},
	{
		pattern: /\bS&P\s*BSE\s*100\b|\bBSE\s*100\b/i,
		indexCode: "BSE100",
		priority: 90,
	},
	{
		pattern: /\bS&P\s*BSE\s*200\b|\bBSE\s*200\b/i,
		indexCode: "BSE200",
		priority: 85,
	},
	{
		pattern: /\bS&P\s*BSE\s*500\b|\bBSE\s*500\b/i,
		indexCode: "BSE500",
		priority: 80,
	},
	{
		pattern:
			/\bBSE\s*LargeMidcap\b|\bS&P\s*BSE\s*LargeMidcap\b|\bBSE\s*Large\s*Mid\s*Cap\b/i,
		indexCode: "BSE_LARGEMID",
		priority: 85,
	},
	{
		pattern: /\bBSE\s*Midcap\b|\bS&P\s*BSE\s*Midcap\b/i,
		indexCode: "BSE_MIDCAP",
		priority: 85,
	},
	{
		pattern: /\bBSE\s*Smallcap\b|\bS&P\s*BSE\s*Smallcap\b/i,
...
class AmfiBenchmarkIngestionService {
	...
	static getInstance(): AmfiBenchmarkIngestionService {
		if (!AmfiBenchmarkIngestionService.instance) {
			AmfiBenchmarkIngestionService.instance =
				new AmfiBenchmarkIngestionService();
		}
	...
	async syncAmfiSchemeBenchmarks(): Promise<{
		total: number;
		parsed: number;
		normalized: number;
		failed: number;
		...
		console.log(
			"[AmfiBenchmarkIngestion] Starting AMFI scheme benchmark sync...",
		);
		...
		console.log(
			`[AmfiBenchmarkIngestion] Found ${fundsWithBenchmark.length} funds with benchmark data`,
		);
		...
		for (const fund of fundsWithBenchmark) {
			...
			try {
				...
				console.error(
					`[AmfiBenchmarkIngestion] Error processing ${fund.isin}:`,
					error,
				);
				failed++;
			}
		...
		console.log(
			`[AmfiBenchmarkIngestion] Sync complete: ${parsed} parsed, ${normalized} normalized, ${failed} failed`,
		);
		return { total: fundsWithBenchmark.length, parsed, normalized, failed };
	}

	async autoMapFromAmfi(): Promise<{
		mapped: number;
		updated: number;
		skipped: number;
		recompute: string[];
		...
		console.log(
			"[AmfiBenchmarkIngestion] Auto-mapping funds from AMFI data...",
		);

		const amfiData = await db
			.select()
		...
		for (const amfi of amfiData) {
			...
			if (existingMap.length > 0) {
				...
				if (
					...
					existing.indexCode === amfi.normalizedBenchmark
				) {
					skipped++;
					continue;
				}
		...
		console.log(
			`[AmfiBenchmarkIngestion] Auto-map complete: ${mapped} new, ${updated} updated, ${skipped} skipped`,
		);
	...
	> {
		const conflicts = await db
			...
			.innerJoin(
				mfBenchmarkMap,
				eq(amfiSchemeBenchmarks.mfIsin, mfBenchmarkMap.mfIsin),
			)
			.where(
				and(
					isNotNull(amfiSchemeBenchmarks.normalizedBenchmark),
	...
	async resolveConflict(
		isin: string,
		resolution: "accept_amfi" | "keep_current" | "manual",
...
export const amfiBenchmarkIngestionService =
	AmfiBenchmarkIngestionService.getInstance();
