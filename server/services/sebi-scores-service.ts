...
class SebiScoresService {
  ...
  private calculatePriority(category: ComplaintCategory, amount?: number): ComplaintPriority {
    if (category === 'unauthorized_transaction' || category === 'data_privacy') {
      return 'critical';
    }
  ...
  async submitComplaint(params: SubmitComplaintParams): Promise<GrievanceComplaint> {
    const id = nanoid();
    ...
    const priority = this.calculatePriority(params.category, params.details.transactionAmount);
  ...
  async acknowledgeComplaint(complaintId: string, acknowledgedBy: string): Promise<GrievanceComplaint | null> {
    ...
    console.log(`[SEBI SCORES] Complaint ${complaint.scoresReferenceNumber} acknowledged`);

    return complaint;
  }

  async updateComplaint(complaintId: string, params: UpdateComplaintParams): Promise<GrievanceComplaint | null> {
    const complaint = this.complaints.get(complaintId);
    if (!complaint) return null;
    ...
    this.complaints.set(complaintId, complaint);
    console.log(`[SEBI SCORES] Complaint ${complaint.scoresReferenceNumber} updated`);

    return complaint;
  }

  async resolveComplaint(complaintId: string, params: ResolveComplaintParams): Promise<GrievanceComplaint | null> {
    const complaint = this.complaints.get(complaintId);
    if (!complaint) return null;
    ...
    complaint.auditLog.push({
      ...
      details: {
        ...
        resolutionDays: Math.ceil((now.getTime() - complaint.submittedAt.getTime()) / (1000 * 60 * 60 * 24))
      }
    });

    this.complaints.set(complaintId, complaint);
    console.log(`[SEBI SCORES] Complaint ${complaint.scoresReferenceNumber} resolved: ${params.resolutionType}`);

    return complaint;
  }

  async closeComplaint(complaintId: string, closedBy: string, reason?: string): Promise<GrievanceComplaint | null> {
    const complaint = this.complaints.get(complaintId);
    if (!complaint) return null;
    ...
    this.complaints.set(complaintId, complaint);
    console.log(`[SEBI SCORES] Complaint ${complaint.scoresReferenceNumber} closed`);

    return complaint;
  }

  async escalateComplaint(complaintId: string, escalatedBy: string, reason: string): Promise<GrievanceComplaint | null> {
    const complaint = this.complaints.get(complaintId);
    if (!complaint) return null;
    ...
    if (complaint.priority !== 'critical') {
      complaint.priority = complaint.priority === 'low' ? 'medium' : 
    ...
    this.complaints.set(complaintId, complaint);
    console.log(`[SEBI SCORES] Complaint ${complaint.scoresReferenceNumber} escalated to level ${complaint.escalationLevel}`);
  ...
  getComplaintByReference(scoresReference: string): GrievanceComplaint | undefined {
  ...
  getAllComplaints(filters?: {
    ...
    if (filters) {
      ...
      if (filters.isEscalated !== undefined) {
        complaints = complaints.filter(c => c.isEscalated === filters.isEscalated);
      }
      if (filters.assignedTo) {
        complaints = complaints.filter(c => c.assignedTo === filters.assignedTo);
      }
      if (filters.fromDate) {
        complaints = complaints.filter(c => c.submittedAt >= filters.fromDate!);
      }
      if (filters.toDate) {
  ...
  getPendingEscalations(): GrievanceComplaint[] {
    ...
    return Array.from(this.complaints.values())
      .filter(c => {
        if (['resolved', 'closed', 'rejected', 'escalated'].includes(c.status)) {
          return false;
        }
    ...
    for (const complaint of complaints) {
      ...
      if (complaint.resolvedAt) {
        ...
        const resolutionDays = Math.ceil(
          (complaint.resolvedAt.getTime() - complaint.submittedAt.getTime()) / (1000 * 60 * 60 * 24)
        );
  ...
  async checkSlaBreaches(): Promise<{
    ...
    if (breached.length > 0) {
      console.log(`[SEBI SCORES SLA] ${breached.length} complaints have breached SLA deadline`);
    }
    if (nearingDeadline.length > 0) {
      console.log(`[SEBI SCORES SLA] ${nearingDeadline.length} complaints are nearing SLA deadline`);
    }

    return { breached, nearingDeadline };
  }

  getCategoryOptions(): Array<{ value: ComplaintCategory; label: string; description: string }> {
    return [
      { value: 'investment_advice', label: 'Investment Advice', description: 'Issues with investment recommendations or advice provided' },
      { value: 'unauthorized_transaction', label: 'Unauthorized Transaction', description: 'Transactions executed without proper authorization' },
      { value: 'service_delay', label: 'Service Delay', description: 'Delays in service delivery or response' },
      { value: 'fee_dispute', label: 'Fee/Charges Dispute', description: 'Disputes regarding fees, charges, or commissions' },
      { value: 'kyc_issue', label: 'KYC/Onboarding Issue', description: 'Problems with KYC verification or onboarding process' },
      { value: 'portfolio_management', label: 'Portfolio Management', description: 'Issues with portfolio management services' },
      { value: 'mutual_fund', label: 'Mutual Fund', description: 'Issues related to mutual fund transactions or services' },
      { value: 'bonds_securities', label: 'Bonds & Securities', description: 'Issues with bond or securities transactions' },
      { value: 'insurance_related', label: 'Insurance Related', description: 'Issues with insurance products or services' },
      { value: 'loan_related', label: 'Loan Related', description: 'Issues with loan applications or servicing' },
      { value: 'data_privacy', label: 'Data Privacy', description: 'Concerns about data handling or privacy breaches' },
      { value: 'platform_issue', label: 'Platform/Technical Issue', description: 'Technical problems with the platform' },
