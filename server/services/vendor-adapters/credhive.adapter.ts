...
class CredhiveAdapter implements CorporateDataProvider {
  ...
  async fetchCompanyProfile(cin: string): Promise<CompanyProfile | null> {
    ...
    try {
      ...
      await this.logApiCall(endpoint, cin, start, resp.success ? 200 : 500, resp.success, resp.error);
      ...
      try {
        ...
        if (dirResp.success && dirResp.data) {
          directors = dirResp.data
            ...
            .map(dir => ({ name: dir.name, din: dir.din, designation: dir.designation }));
        }
      } catch {
      ...
      console.warn(`[CredhiveAdapter] fetchCompanyProfile failed for ${cin}:`, err.message);
      return null;
    }
  }
  ...
  async fetchFinancials(cin: string, years: number = 5): Promise<FinancialStatement[]> {
    ...
    try {
      ...
      await this.logApiCall(endpoint, cin, start, 500, false, err.message);
      console.warn(`[CredhiveAdapter] fetchFinancials failed for ${cin}:`, err.message);
      return [];
    }
  }
  ...
  async fetchRatios(cin: string, years: number = 5): Promise<FinancialRatios[]> {
    ...
    const ratios: FinancialRatios[] = financials.map((f, i) => {
      ...
      const roe = (f.pat != null && f.networth && f.networth !== 0)
        ? (f.pat / f.networth) * 100 : null;
      const debtEquity = (f.totalDebt != null && f.networth && f.networth !== 0)
        ? f.totalDebt / f.networth : null;
      const marginEbitda = (f.ebitda != null && f.revenue && f.revenue !== 0)
        ? (f.ebitda / f.revenue) * 100 : null;
      const marginPat = (f.pat != null && f.revenue && f.revenue !== 0)
        ? (f.pat / f.revenue) * 100 : null;
      const revenueGrowth = (prev?.revenue && f.revenue != null && prev.revenue !== 0)
        ? ((f.revenue - prev.revenue) / prev.revenue) * 100 : null;
      const profitGrowth = (prev?.pat && f.pat != null && prev.pat !== 0)
        ? ((f.pat - prev.pat) / prev.pat) * 100 : null;
  ...
  async fetchDirectors(cin: string): Promise<Director[]> {
    ...
    try {
      ...
      return resp.data
        ...
        .map(d => ({ name: d.name, din: d.din || null, designation: d.designation || null }));
    } catch (err: any) {
      await this.logApiCall(endpoint, cin, start, 500, false, err.message);
      console.warn(`[CredhiveAdapter] fetchDirectors failed for ${cin}:`, err.message);
      return [];
    }
  }
  ...
  async fetchCharges(cin: string): Promise<Charge[]> {
    ...
    try {
      ...
      await this.logApiCall(endpoint, cin, start, 500, false, err.message);
      console.warn(`[CredhiveAdapter] fetchCharges failed for ${cin}:`, err.message);
      return [];
    }
  }
}
