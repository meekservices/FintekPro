...
// ─────────────────────────────────────────────────────────────
async function gstHeaders(
	opts: { source?: string; contentType?: boolean } = {},
	...
	try {
		...
		throw new Error(
			"Sandbox credentials not configured (SANDBOX_API_KEY / SANDBOX_API_SECRET)",
		);
	}
...
async function get(
	...
	const headers = {
		...(await gstHeaders({ contentType: false })),
		...extraHeaders,
	};
 ...
 */
export async function gstVerifyOtp(
	username: string,
	gstin: string,
	otp: string,
) {
...
export async function gstRefreshSession() {
	...
	return sandboxFetch("/gst/compliance/tax-payer/session/refresh", {
		method: "POST",
		headers,
	});
}
...
export async function gstLogout() {
	...
	return sandboxFetch("/gst/compliance/tax-payer/logout", {
		method: "POST",
		headers,
	});
}
 ...
 */
export async function gstr2aSection(
	section: Gstr2aSection,
	year: string,
	month: string,
) {
	return get(
		`/gst/compliance/tax-payer/gstrs/gstr-2a/${section}/${year}/${month}`,
	);
}
 ...
 */
export async function eInvoiceAuthenticate(
	username: string,
	password: string,
	gstin: string,
) {
...
export async function eInvoiceGet(irn: string) {
	return get(
		`/gst/compliance/e-invoice/tax-payer/invoice/${encodeURIComponent(irn)}`,
		{ "x-source": "primary" },
	);
}
 ...
 */
export async function eInvoiceCancel(
	irn: string,
	cancelReason: string,
	cancelRemark: string,
) {
...
export async function eInvoiceGetByDocData(params: {
	...
	return get(
		`/gst/compliance/e-invoice/tax-payer/invoice${qs ? "?" + qs : ""}`,
		{ "x-source": "primary" },
	);
}
 ...
 */
export async function eInvoiceGenerateEwb(
	irn: string,
	ewbData: Record<string, any>,
) {
	return post(
 ...
 * Body: { username, password, gstin }
 */
export async function ewbAuthenticate(
	username: string,
	password: string,
	gstin: string,
) {
...
export async function ewbGet(ewbNo: string) {
	return get(
		`/gst/compliance/e-way-bill/tax-payer/bill/${encodeURIComponent(ewbNo)}`,
	);
}
 ...
 */
export async function ewbConsignorGetByDate(
	params: {
		date?: string;
		type?: string;
		fromDate?: string;
 ...
 */
export async function ewbConsignorCancel(
	ewbNo: number | string,
	cancelRsnCode: number,
	cancelRmrk: string,
) {
	return post(
		`/gst/compliance/e-way-bill/consignor/bill/${encodeURIComponent(ewbNo)}/cancel`,
		{
			ewbNo,
			cancelRsnCode,
			cancelRmrk,
 ...
 */
export async function ewbConsignorExtendValidity(
	ewbNo: number | string,
	extendData: Record<string, any>,
) {
	return post(
		`/gst/compliance/e-way-bill/consignor/bill/${encodeURIComponent(ewbNo)}/extend`,
		{
			ewbNo,
			...extendData,
		},
 ...
 */
export async function ewbConsignorUpdateVehicle(
	ewbNo: number | string,
	vehicleData: Record<string, any>,
) {
	return put(
		`/gst/compliance/e-way-bill/consignor/bill/${encodeURIComponent(ewbNo)}/vehicle`,
		{
			ewbNo,
			...vehicleData,
		},
 ...
 */
export async function ewbConsignorUpdateTransporter(
	ewbNo: number | string,
	transporterId: string,
) {
	return put(
		`/gst/compliance/e-way-bill/consignor/bill/${encodeURIComponent(ewbNo)}/transporter`,
		{
			ewbNo,
			transporterId,
		},
 ...
 */
export async function ewbConsignorConsolidate(
	consolidateData: Record<string, any>,
) {
	return post(
 ...
 */
export async function ewbConsigneeGetByDate(
...
export async function ewbConsigneeReject(ewbNo: number | string) {
	return post(
		`/gst/compliance/e-way-bill/consignee/bill/${encodeURIComponent(ewbNo)}/reject`,
		{
			ewbNo: String(ewbNo),
		},
	);
 ...
 */
export async function ewbTransporterGetByDateAndState(
	params: {
		fromDate?: string;
		toDate?: string;
		state?: string;
	...
	const qs = new URLSearchParams(params as Record<string, string>).toString();
	return get(
		`/gst/compliance/e-way-bill/transporter/bills${qs ? "?" + qs : ""}`,
	);
}
