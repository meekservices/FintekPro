 ...
 */
export function scheduleIrisPortfolioRefresh(
	pan: string,
	userId?: string,
): void {
	if (!pan) return;
	setTimeout(async () => {
		try {
			...
			logger.error("[IRISSync] Deferred refresh failed", {
				pan,
				error: err?.message,
			});
		}
 ...
 */
export async function syncIrisHoldingsForPan(
	pan: string,
	userId?: string,
): Promise<{
	synced: number;
	...
	console.log(
		`[IRISSync] Starting sync for PAN ${pan} (User: ${resolvedUserId})`,
	);
	...
	try {
		...
		logger.error("[IRISSync] IRIS portfolio fetch failed", {
			pan,
			error: err?.message,
		});
	...
	const holdings: any[] =
		portfolioData?.holdings ?? portfolioData?.data?.holdings ?? [];
	if (!holdings.length) {
	...
	try {
		...
		logger.error("[IRISSync] Portfolio lookup failed", {
	...
	for (const h of holdings) {
		try {
			...
			const schemeName =
				h.schemeName ?? h.scheme_name ?? h.name ?? h.fundName ?? "Mutual Fund";
			const symbol =
				h.schemeCode ?? h.scheme_code ?? isin ?? schemeName.slice(0, 20);

			const units = Number.parseFloat(h.units ?? h.qty ?? h.balance ?? "0");
			const nav = Number.parseFloat(h.nav ?? h.currentNav ?? h.navValue ?? "0");
			const mktValue = Number.parseFloat(
				h.marketValue ?? h.currentValue ?? (units * nav).toString(),
			);
			const invested = Number.parseFloat(
				h.investedValue ?? h.costBasis ?? h.purchaseValue ?? "0",
			);
			...
			await db
				...
				.onConflictDoUpdate({
					target: [
						comprehensiveHoldings.userId,
						comprehensiveHoldings.isin,
						comprehensiveHoldings.folio,
					],
					set: {
						units: String(units),
						marketValue: String(mktValue),
			...
			logger.error("[IRISSync] Holding record failed", {
				isin: h.isin,
				error: e.message,
			});
	...
	logger.info("[IRISSync] Sync complete", {
		pan,
		synced,
...
export async function runNightlyIrisCasSync(): Promise<void> {
	...
	logger.info("[IRISSync] Nightly CAS sync complete", {
		total,
		success,
		failed,
	});
...
export async function recordLrsRemittance(params: {
	...
	const { userId, alpacaAccountId, transferId, amountUsd, financialYear } =
		params;
	try {
		let usdInrRate = 84;
		try {
		...
		logger.info("[LRS] Recorded remittance", {
			userId,
			amountUsd,
			financialYear,
		});
