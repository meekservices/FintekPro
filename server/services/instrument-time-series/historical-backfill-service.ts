import { db } from '../../db';
import { listedStocks, instrumentPrices, enrichmentJobLog, enrichmentRetryQueue } from '@shared/schema';
...
async function pushToRetryQueue(
  ...
  if (existing.length > 0) {
    ...
    if (entry.retryCount >= entry.maxRetries) {
      await logJob('HISTORICAL_BACKFILL', instrumentId, symbol, 'MAX_RETRIES_EXCEEDED', `Retry cap (${entry.maxRetries}) reached: ${error}`);
      return;
    }
    const backoffMinutes = Math.pow(2, entry.retryCount) * 30;
    await db
...
export async function runHistoricalBackfill(batchSize: number = 5): Promise<BackfillResult> {
  const result: BackfillResult = { processed: 0, skipped: 0, errors: 0, retried: 0, apiCalls: 0, totalRecords: 0 };
  ...
  console.log(`[HistoricalBackfill] Found ${instruments.length} instruments needing backfill`);
  ...
  for (const instrument of instruments) {
    try {
      ...
      const { result: prices } = await registry.getHistoricalPrices(fmpSymbol, fromDate, toDate);
      result.apiCalls++;

      if (!prices || prices.length === 0) {
        result.skipped++;
        await logJob('HISTORICAL_BACKFILL', instrument.id, instrument.symbol, 'NO_DATA', `No historical data found for ${fromDate} to ${toDate}`);
        continue;
      }
        ...
        for (const p of batch) {
          try {
            ...
            if (!insertErr.message?.includes('duplicate')) {
              console.error(`[HistoricalBackfill] Insert error for ${instrument.symbol} on ${p.date}: ${insertErr.message}`);
            }
          }
        }
      }
      ...
      await logJob('HISTORICAL_BACKFILL', instrument.id, instrument.symbol, 'SUCCESS', 
        `Backfilled ${insertedCount} records from ${earliestDate} to ${latestDate}`, insertedCount);
      ...
      console.log(`[HistoricalBackfill] ${instrument.symbol}: ${insertedCount} records (${earliestDate} to ${latestDate})`);
    } catch (err: any) {
      result.errors++;
      console.error(`[HistoricalBackfill] Error for ${instrument.symbol}: ${err.message}`);
      await logJob('HISTORICAL_BACKFILL', instrument.id, instrument.symbol, 'FAILED', err.message);
  ...
  console.log(`[HistoricalBackfill] Complete: ${result.processed} instruments, ${result.totalRecords} records, ${result.errors} errors, ${result.retried} retries`);
  await logJob('HISTORICAL_BACKFILL_BATCH', null, null, 'COMPLETE',
...
async function processBackfillRetryQueue(
  ...
  for (const item of retryItems) {
    try {
      ...
      const { result: prices } = await registry.getHistoricalPrices(fmpSymbol, fiveYearsAgo, today);
      result.apiCalls++;
      ...
      if (prices && prices.length > 0) {
        ...
        result.retried++;
        result.totalRecords += insertedCount;
        await logJob('HISTORICAL_BACKFILL', item.instrumentId, item.symbol, 'RETRY_SUCCESS', 
          `Resolved on retry ${item.retryCount}: ${insertedCount} records`, insertedCount);
      }
    } catch (err: any) {
