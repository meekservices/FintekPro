...
export class KycUpgradeNotificationService {
  ...
  static getInstance(): KycUpgradeNotificationService {
    if (!KycUpgradeNotificationService.instance) {
      KycUpgradeNotificationService.instance = new KycUpgradeNotificationService();
    }
  ...
  async scheduleReminders(userId: string): Promise<void> {
    try {
      ...
      if (kycStatus.missingSteps.length === 0) {
        console.log(`[KYC Notification] User ${userId} has complete KYC, no reminders needed`);
        return;
      }
      ...
      if (existingReminders.length > 0) {
        console.log(`[KYC Notification] User ${userId} already has ${existingReminders.length} pending reminders`);
        return;
      }
      ...
      for (let i = 0; i < REMINDER_SCHEDULE_DAYS.length; i++) {
        ...
        const scheduledFor = new Date(now.getTime() + daysFromNow * 24 * 60 * 60 * 1000);
        ...
        await db.insert(kycUpgradeReminders).values({
          ...
          targetKycTier: kycStatus.currentTier === 'basic' ? 'enhanced' : 'accredited',
          missingSteps: kycStatus.missingSteps,
          scheduledFor,
          status: 'pending',
          metadata: {
            blockedProducts: BLOCKED_PRODUCTS_BY_TIER[kycStatus.currentTier as keyof typeof BLOCKED_PRODUCTS_BY_TIER] || [],
            percentComplete: kycStatus.percentComplete,
          },
        });
      }

      console.log(`[KYC Notification] Scheduled ${REMINDER_SCHEDULE_DAYS.length} reminders for user ${userId}`);
    } catch (error) {
   ...
   */
  async sendReminder(userId: string, sequence: number): Promise<ReminderResult> {
    ...
    try {
      ...
      const [reminder] = await db.insert(kycUpgradeReminders).values({
        userId,
      ...
      console.log(`[KYC Notification] Sent reminder #${sequence} to user ${userId} (email: ${result.emailSent}, in-app: ${result.inAppCreated}, sms: ${result.smsSent})`);
   ...
   */
  async processScheduledReminders(): Promise<{ processed: number; sent: number }> {
    ...
    try {
      ...
      console.log(`[KYC Notification] Processing ${pendingReminders.length} pending reminders`);
      ...
      for (const reminder of pendingReminders) {
        ...
        if (kycStatus.missingSteps.length === 0) {
          ...
          console.log(`[KYC Notification] User ${reminder.userId} completed KYC, expiring reminders`);
          continue;
        }

        // Send the reminder
        const result = await this.sendReminder(reminder.userId, reminder.reminderSequence);
      ...
      console.log(`[KYC Notification] Processed ${stats.processed} reminders, sent ${stats.sent}`);
      return stats;
    } catch (error) {
      console.error('[KYC Notification] Error processing scheduled reminders:', error);
      return stats;
    }
  }
  ...
  async acknowledgeReminder(userId: string, reminderId?: string): Promise<void> {
    try {
      ...
      console.log(`[KYC Notification] Reminder acknowledged for user ${userId}`);
    } catch (error) {
  ...
  async getPendingNotifications(userId: string): Promise<{
    ...
    return {
      ...
      percentComplete: kycStatus.percentComplete,
      missingSteps: kycStatus.missingSteps.map(step => 
  ...
  private getNotificationMessage(sequence: number, kycStatus: KycStatus): string {
    const blockedProducts = BLOCKED_PRODUCTS_BY_TIER[kycStatus.currentTier as keyof typeof BLOCKED_PRODUCTS_BY_TIER] || [];
    const nextStep = kycStatus.missingSteps[0];
    const stepName = KYC_STEPS[nextStep as keyof typeof KYC_STEPS]?.name || nextStep;
        ...
        <div class="container">
          ...
          <div class="content">
            ...
            <div class="step-list">
              ...
              ${kycStatus.missingSteps.map(step => `
...
export const kycUpgradeNotificationService = KycUpgradeNotificationService.getInstance();
