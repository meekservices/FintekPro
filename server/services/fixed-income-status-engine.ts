...
export class FixedIncomeStatusEngine {
	...
	private calculateTradingDaysGap(lastTradeDate: Date | null): number {
		...
		const diffTime = Math.abs(
			now.getTime() - new Date(lastTradeDate).getTime(),
		);
	...
	async evaluateInstrumentStatus(
		isin: string,
	): Promise<StatusEvaluation | null> {
		...
		const gate2: GateResult = {
			...
			reason:
				regulatoryEligibility === "retail"
					? "Retail allowed"
					: `Institutional only (${regulatoryEligibility})`,
		...
		const bidAskSpread = Number.parseFloat(
			bond.bidAskSpread?.toString() || "999",
		);
		...
		const gate3: GateResult = {
			gate: "liquidity",
			passed:
				liquidityScore >= LIQUIDITY_THRESHOLD &&
		...
		if (!gate3.passed) {
			const reasons = [];
			if (liquidityScore < LIQUIDITY_THRESHOLD)
				reasons.push(
					`Liquidity score ${liquidityScore} < ${LIQUIDITY_THRESHOLD}`,
				);
			if (bidAskSpread > BID_ASK_THRESHOLD)
				reasons.push(`Bid-ask spread ${bidAskSpread}% > ${BID_ASK_THRESHOLD}%`);
			if (tradingDaysGap > MAX_TRADING_DAYS_GAP)
				reasons.push(
					`Last trade ${tradingDaysGap} days ago > ${MAX_TRADING_DAYS_GAP} days`,
				);
	...
	async updateInstrumentStatus(
		...
		await db
			...
			.set({
				...
				structureComplexity:
					evaluation.gateResults.find((g) => g.gate === "structural_simplicity")
						?.details?.complexity || null,
			})
			.where(eq(corporateBonds.isin, isin));
	...
	async refreshAllInstruments(): Promise<{
		...
		const allBonds = await db
			.select({ isin: corporateBonds.isin })
			.from(corporateBonds)
			.where(
				and(
		...
		for (const bond of allBonds) {
			try {
				...
				errors.push(
					`Error processing ${bond.isin}: ${error instanceof Error ? error.message : "Unknown error"}`,
				);
			}
		}

		console.log(
			`[FixedIncomeStatusEngine] Daily refresh complete: ${processed} processed, ${changed} changed`,
		);
		console.log(
			`[FixedIncomeStatusEngine] Status distribution: ${sellable} SELLABLE, ${visible} VISIBLE, ${hidden} HIDDEN`,
		);
	...
	async getVisibleInstruments(limit: number = 100, offset: number = 0) {
		return db
			.select()
			.from(corporateBonds)
			.where(sql`${corporateBonds.instrumentStatus} IN ('SELLABLE', 'VISIBLE')`)
			.limit(limit)
			.offset(offset);
	}
