...
export class DuplicateDetectionService {
	...
	async checkForDuplicates(params: {
		...
		const { email, mobile, panNumber, firstName, lastName, excludeUserId } =
			params;
	...
	private analyzeDuplicate(
		...
		if (
			user1.panNumber &&
			user2.panNumber &&
			user1.panNumber === user2.panNumber
		) {
	...
	async getDuplicateStats(): Promise<{
		...
		return {
			...
			autoMergeRecommended: duplicates.filter((d) => d.autoMergeRecommended)
				.length,
		};
	}
}
