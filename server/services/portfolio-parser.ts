...
function setCachedResult(hash: string, result: ParseResult): void {
	if (parseCache.size > 100) {
		const oldest = Array.from(parseCache.entries()).sort(
...
function detectBroker(text: string): {
	broker: string | null;
	confidence: number;
} {
...
function parseZerodhaFormat(text: string): ImportedHolding[] {
	...
	const holdingPattern =
		/([A-Z][A-Z0-9&\-\s]{2,30})\s+([0-9,]+(?:\.\d+)?)\s+₹?\s*([0-9,]+(?:\.\d+)?)\s+₹?\s*([0-9,]+(?:\.\d+)?)/i;

	for (const line of lines) {
		const match = line.match(holdingPattern);
		if (match) {
			const [, rawName, qty, avgCost, currentVal] = match;
			const quantity = Number.parseFloat(qty.replace(/,/g, ""));
			const averageCost = Number.parseFloat(avgCost.replace(/,/g, ""));
			const currentValue = Number.parseFloat(currentVal.replace(/,/g, ""));
...
function parseGrowwFormat(text: string): ImportedHolding[] {
	...
	const mfPattern =
		/([A-Za-z0-9\s\-&]+(?:Fund|Scheme|Plan|Growth|IDCW)[A-Za-z0-9\s\-&]*)\s+([0-9,]+(?:\.\d+)?)\s+units?\s+(?:₹|INR)?\s*([0-9,]+(?:\.\d+)?)/i;
	// Pattern: "TATA MOTORS 50 shares ₹ 45,000.00"
	const stockPattern =
		/([A-Z0-9&\-\s]{2,40})\s+([0-9,]+)\s+(?:shares?|qty)\s+(?:₹|INR)?\s*([0-9,]+(?:\.\d+)?)/i;
	...
	for (const line of lines) {
		...
		if (match) {
			...
			holdings.push({
				...
				quantity: Number.parseFloat(units.replace(/,/g, "")),
				currentValue: Number.parseFloat(value.replace(/,/g, "")),
				broker: "Groww",
				confidenceScore: 80,
			});
			continue;
		...
		if (match) {
			...
			holdings.push({
				...
				name,
				assetType: "equity",
				quantity: Number.parseInt(qty.replace(/,/g, "")),
				currentValue: Number.parseFloat(value.replace(/,/g, "")),
				broker: "Groww",
				confidenceScore: 80,
			});
...
function parseMFCentralFormat(text: string): ImportedHolding[] {
	...
	const mfPattern =
		/([A-Za-z0-9\s\-&]+(?:Fund|Scheme|Plan|Growth|IDCW)[A-Za-z0-9\s\-&]*)\s+(?:Folio[:\s]*)?(\d+)\s+(\d+(?:\.\d+)?)\s+₹?\s*(\d+(?:,\d+)*(?:\.\d+)?)/i;
	...
	for (const line of lines) {
		...
		if (match) {
			...
			holdings.push({
				...
				name: name.trim(),
				assetType: "mutual_fund",
				quantity: Number.parseFloat(units),
				currentValue: Number.parseFloat(value.replace(/,/g, "")),
				folioNumber: folio,
				broker: "MF Central",
				confidenceScore: 85,
...
function parseCASFormat(text: string): ImportedHolding[] {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (isinMatch) {
			...
			if (!currentSchemeName || currentSchemeName.length < 10) {
				for (let j = 1; j <= 3; j++) {
					...
					if (
						prevLine &&
						!prevLine.includes("CAS") &&
						!prevLine.includes("Statement") &&
						prevLine.length > 10 &&
						!prevLine.match(/^\d+$/)
					) {
						currentSchemeName =
							prevLine + (currentSchemeName ? " " + currentSchemeName : "");
						if (
							prevLine.includes("Closing Unit") ||
							prevLine.includes("Valuation")
						)
							break;
					}
				}
		...
		const mfCentralPattern =
			/Closing\s*Unit\s*Balance\s*[:\s]+\s*([0-9,]+(?:\.\d+)?)\s+.*Nav.*[:\s]+\s*(?:INR\s*)?([0-9,]+(?:\.\d+)?)\s+.*Valuation.*[:\s]+\s*(?:INR\s*)?([0-9,]+(?:\.\d+)?)/i;

		const match = line.match(mfCentralPattern);
		if (match) {
			const units = Number.parseFloat(match[1].replace(/,/g, ""));
			const nav = Number.parseFloat(match[2].replace(/,/g, ""));
			const valuation = Number.parseFloat(match[3].replace(/,/g, ""));

			if (units > 0 && valuation > 0) {
				const normalizedName =
					holdingNormalizationService.normalizeHoldingName(currentSchemeName);
...
function detectColumnHeaders(text: string): ColumnMapping {
	...
	for (const line of lines) {
		...
		if (columnKeywords.cost.some((k) => lowerLine.includes(k)))
			keywordMatches++;
		if (columnKeywords.units.some((k) => lowerLine.includes(k)))
			keywordMatches++;
		if (columnKeywords.nav.some((k) => lowerLine.includes(k))) keywordMatches++;
		if (columnKeywords.market.some((k) => lowerLine.includes(k)))
			keywordMatches++;
		...
		if (keywordMatches >= 3) {
			...
			for (const keyword of columnKeywords.cost) {
				...
				if (idx >= 0) {
					positions.push({ type: "cost", index: idx });
					break;
				}
			}
			for (const keyword of columnKeywords.units) {
				const idx = lowerLine.indexOf(keyword);
				if (idx >= 0) {
					positions.push({ type: "units", index: idx });
					break;
				}
			}
			for (const keyword of columnKeywords.nav) {
				const idx = lowerLine.indexOf(keyword);
				if (idx >= 0) {
					positions.push({ type: "nav", index: idx });
					break;
				}
			}
			for (const keyword of columnKeywords.market) {
				const idx = lowerLine.indexOf(keyword);
				if (idx >= 0) {
					positions.push({ type: "market", index: idx });
					break;
				}
			}
...
function extractTransactionLots(holdingBlockText: string): TransactionLot[] {
	...
	for (const line of lines) {
		...
		const numbers = numberMatches
			...
			.filter((n) => !Number.isNaN(n)); // Allow 0, but filter out NaN
		...
		const isRedemption =
			...
			(lowerLine.includes("switch") &&
				(lowerLine.includes("out") || lowerLine.includes("-out"))) ||
		...
		if (numbers.length >= 4) {
			...
			if (balance < units * 0.1) {
				// Balance shouldn't typically be 10x smaller than transaction units unless mostly redeemed
				// Likely [Amount, Units]
				amount = numbers[0];
				units = numbers[1];
		...
		if (lowerLine.includes("sip") || lowerLine.includes("systematic")) {
			...
			lowerLine.includes("switch") &&
			(lowerLine.includes("in") || lowerLine.includes("-in"))
		) {
			transactionType = "switch_in";
		} else if (isRedemption) {
			transactionType = lowerLine.includes("switch")
				? "switch_out"
				: "redemption";
			...
			lowerLine.includes("reinvestment")
		) {
...
function parseCAMSHoldingStatementFormat(text: string): ImportedHolding[] {
	...
	const spacedIsinPattern =
		/IN[EF0]\s*[A-Z0-9]{2}\s*[A-Z0-9]{2}\s*[A-Z0-9]{2}\s*[A-Z0-9]{3}/gi;
	const spacedMatches = (text.match(spacedIsinPattern) || []).map((m) =>
		m.replace(/\s/g, ""),
	);
	...
	console.log(
		"[CAMS Holding Parser] Found",
		allIsinMatches.length,
		"unique ISINs",
	);
	console.log(
		"[CAMS Holding Parser] ISIN breakdown - INF (MF):",
		allIsinMatches.filter((i) => i.startsWith("INF")).length,
		"INE (Stock):",
		allIsinMatches.filter((i) => i.startsWith("INE")).length,
		"IN0 (Debt):",
		allIsinMatches.filter((i) => i.startsWith("IN0")).length,
	);

	if (allIsinMatches.length === 0) {
		// Check for UNCLAIMDISIN or other special markers
		const hasUnclaimedFunds =
			text.includes("UNCLAIMDISIN") || text.includes("Unclaimed");
		if (hasUnclaimedFunds) {
			console.log(
				"[CAMS Holding Parser] Found unclaimed funds - skipping those",
			);
		}
		return holdings;
	}
	...
	for (const isin of allIsinMatches) {
		try {
			...
			const beforeIsin = text.substring(
				Math.max(0, isinIndex - 100),
				isinIndex,
			);
			const afterIsin = text.substring(
				isinIndex + isin.length,
				Math.min(text.length, isinIndex + isin.length + 2000),
			);

			console.log("[CAMS Holding Parser] Processing ISIN:", isin);
			console.log("[CAMS Holding Parser] Before:", beforeIsin.slice(-50));
			console.log(
				"[CAMS Holding Parser] After (first 100):",
				afterIsin.substring(0, 100),
			);
			...
			const schemeMatch = afterIsin.match(
				/^\s*([A-Z0-9]+)\s*-\s*([^0-9]+?)(?=\d{1,3}(?:,\d{3})*\.)/i,
			);
			let schemeName = "";
			if (schemeMatch) {
			...
			const afterWithoutCommas = afterWithoutDecimals.replace(
				indianCommaPattern,
				" ",
			);
			...
			console.log(
				"[CAMS Holding Parser] Extracted numbers:",
				numbers.slice(0, 8),
			);
			...
			const navPattern =
				/NAV\s+on\s+[\d\w-]+:\s*(?:INR|Rs\.?|₹)?\s*([\d,]+\.?\d*)/i;
			const marketPattern =
				/Market\s+Value\s+on\s+[\d\w-]+:\s*(?:INR|Rs\.?|₹)?\s*([\d,]+\.?\d*)/i;
			...
			if (navMatch && marketMatch && closingUnitsMatch) {
				...
				marketValue = Number.parseFloat(marketMatch[1].replace(/,/g, ""));
				unitBalance = Number.parseFloat(closingUnitsMatch[1].replace(/,/g, ""));
				...
				while ((purchaseMatch = purchasePattern.exec(afterIsin)) !== null) {
					const amount = Number.parseFloat(purchaseMatch[1].replace(/,/g, ""));
					if (amount > 1000) {
				...
				console.log(
					"[CAMS Holding Parser] PRIORITY 0 MATCH - Explicit patterns found:",
				);
				console.log(
					`[CAMS Holding Parser]   Units: ${unitBalance}, NAV: ₹${nav}, Market: ₹${marketValue.toLocaleString()}, Cost: ₹${costValue.toLocaleString()}`,
				);
			}
			...
			if (
				unitBalance === 0 &&
				marketValue === 0 &&
				columnMapping.headerDetected &&
				numbers.length >= 4
			) {
				// Map numbers using detected column order
				const maxIdx = Math.max(
					columnMapping.costIndex,
					columnMapping.unitsIndex,
					columnMapping.navIndex,
					columnMapping.marketIndex,
				);
				...
				if (maxIdx < numbers.length) {
					...
					const tolerance =
						Math.abs(calculated - marketValue) / Math.max(marketValue, 1);

					if (tolerance < 0.05) {
						console.log(
							"[CAMS Holding Parser] Header-based mapping validated! Units:",
							unitBalance,
							"Cost:",
							costValue,
							"NAV:",
						...
						console.log(
							"[CAMS Holding Parser] Header-based mapping failed validation, falling back to combination search",
						);
			...
			if (unitBalance === 0 && marketValue === 0) {
						...
						for (let nIdx = 0; nIdx < numLimit; nIdx++) {
							...
							for (let mIdx = 0; mIdx < numLimit; mIdx++) {
								...
								const marketTolerance =
									Math.abs(calculated - testMarket) / Math.max(testMarket, 1);
								...
								if (marketTolerance < 0.05) {
									...
									const costScore =
										Math.abs(Math.log(Math.max(costRatio, 0.1))) / 5;
									const score =
										marketTolerance +
										(costReasonable ? 0 : 2) +
										costScore * 0.1 +
				...
				if (candidates.length > 0) {
					...
					console.log(
						"[CAMS Holding Parser] Best match found - Units:",
						unitBalance,
						"Cost:",
						costValue,
						"NAV:",
					...
					const calculated = unitBalance * nav;
					if (
						Math.abs(calculated - marketValue) / marketValue > 0.1 &&
						unitBalance > 0
					) {
						nav = marketValue / unitBalance;
					}
					console.log(
						"[CAMS Holding Parser] Using fallback - Units:",
						unitBalance,
						"Cost:",
						costValue,
						"NAV:",
			...
			const registrar = registrarMatch
				? registrarMatch[1].toUpperCase()
				: "CAMS";

			// Skip if we don't have meaningful data
			if (unitBalance === 0 && marketValue === 0) {
				console.log(
					"[CAMS Holding Parser] Skipping ISIN - no valid data:",
					isin,
				);
				continue;
			}
			...
			if (lots.length > 0) {
				...
				console.log(
					`[CAMS Holding Parser] Found ${lots.length} transaction lots. Purchases: ₹${purchaseTotal.toLocaleString()}, Redemptions: ₹${redemptionTotal.toLocaleString()}`,
				);
			...
			const normalized = holdingNormalizationService.normalizeAndExtract(
				schemeName || isin,
			);
			...
			const holding: ImportedHolding = {
				...
				unrealizedGainPercent:
					finalCostValue > 0
						? ((marketValue - finalCostValue) / finalCostValue) * 100
						: 0,
			...
			console.error(
				"[CAMS Holding Parser] Error processing ISIN",
				isin,
				":",
				error.message,
			);
	...
	console.log(
		"[CAMS Holding Parser] Total holdings extracted:",
		holdings.length,
	);
	console.log(
		"[CAMS Holding Parser] Total market value:",
...
function parseCAMSKfintechTableFormat(text: string): ImportedHolding[] {
	...
	lines.slice(0, 30).forEach((line, idx) => {
		console.log(
			`[CAMS Table Parser] Line ${idx}: "${line.substring(0, 120)}${line.length > 120 ? "..." : ""}"`,
		);
	});
	...
	console.log(
		"[CAMS Table Parser] Found ISINs in text:",
		isinMatches.length,
		isinMatches.slice(0, 5),
	);
	...
	const mergedLinePattern =
		/^(\d{5,}(?:\/\d+)?)\s+([\d,]+\.?\d*)([\dA-Z]+)\s*-\s*(.+)$/i;
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (
			line.includes("Folio No") ||
			line.includes("Scheme Name") ||
			line.includes("NAV Date") ||
			line.includes("Registrar") ||
			line.includes("ISIN") ||
			line.match(/^\s*\(INR\)\s*$/) ||
			line.includes("Entry Load") ||
			line.includes("Exit Load") ||
			line.includes("w.e.f") ||
			line.includes("Total") ||
			line.includes("Loads and Fees") ||
			line.includes("Page ") ||
			line.includes("CAMSCASWS") ||
			line.includes("Version:") ||
			line.includes("Email Id") ||
			line.includes("Consolidated Account") ||
			line.includes("brought to you") ||
			line.includes("IDCW -") ||
			line.match(/^#?\s*IDCW/i)
		) {
			continue;
		...
		if (mergedMatch) {
			...
			const cleanMarketValue = Number.parseFloat(marketValue.replace(/,/g, ""));
			...
			for (const isin of uniqueISINs) {
				...
				if (
					isinIndex >= 0 &&
					schemeCodeIndex >= 0 &&
					Math.abs(isinIndex - schemeCodeIndex) < 200
				) {
					matchedISIN = isin;
					break;
				}
			...
			if (cleanMarketValue > 0 && fullSchemeName.length > 5) {
				...
				console.log(
					"[CAMS Table Parser] Found holding (merged):",
					fullSchemeName,
					"Value:",
					cleanMarketValue,
				);
	...
	if (holdings.length === 0 && mfISINs.length > 0) {
		console.log(
			"[CAMS Table Parser] Trying ISIN-based extraction for",
			mfISINs.length,
			"mutual fund ISINs...",
		);
		...
		for (const isin of mfISINs) {
			...
			if (isinIndex >= 0) {
				...
				const beforeIsin = text.substring(
					Math.max(0, isinIndex - 300),
					isinIndex,
				);
				// Look for data AFTER the ISIN - contains Cost, Folio, MarketValue, NextScheme
				const afterIsin = text.substring(
					isinIndex + isin.length,
					Math.min(text.length, isinIndex + isin.length + 400),
				);

				console.log("[CAMS Table Parser] ISIN:", isin);
				console.log(
					"[CAMS Table Parser] Before ISIN (last 100):",
					beforeIsin.slice(-100),
				);
				console.log(
					"[CAMS Table Parser] After ISIN (first 100):",
					afterIsin.substring(0, 100),
				);
				...
				const unitsMatch = beforeIsin.match(
					/(\d{1,3}(?:,\d{3})*\.\d{2,4})\s+\d{1,2}-[A-Za-z]{3}-\d{4}/,
				);
				let units = 0;
				if (unitsMatch) {
					units = Number.parseFloat(unitsMatch[1].replace(/,/g, ""));
				...
				let investedValue = 0;
				if (costMatch) {
					investedValue = Number.parseFloat(costMatch[1].replace(/,/g, ""));
					console.log(
						"[CAMS Table Parser] Extracted cost/invested:",
						investedValue,
					);
				}
				...
				if (folioMatch) {
					const afterFolio = afterIsin.substring(
						afterIsin.indexOf(folioMatch[0]) + folioMatch[0].length,
					);
					const mvMatch = afterFolio.match(/^\s*([\d,]+\.\d{2})/);
					if (mvMatch) {
						statementMarketValue = Number.parseFloat(
							mvMatch[1].replace(/,/g, ""),
						);
						console.log(
							"[CAMS Table Parser] Extracted statement market value:",
				...
				if (units === 0 && investedValue === 0 && statementMarketValue === 0) {
					console.log(
						"[CAMS Table Parser] Skipping ISIN - no valid data extracted:",
						isin,
					);
					continue;
				}
				...
				const currentValue =
					statementMarketValue > 0 ? statementMarketValue : investedValue;
				...
				if (currentValue > 100) {
					holdings.push({
						...
						quantity: quantity,
						averageCost:
							quantity > 0 && investedValue > 0 ? investedValue / quantity : 0,
					...
					console.log(
						"[CAMS Table Parser] Found holding:",
						isin,
						"Units:",
						quantity,
						"Cost:",
...
async function enrichWithISINIntelligence(
	holdings: ImportedHolding[],
): Promise<ImportedHolding[]> {
	const holdingsWithISIN = holdings.filter(
		(h) => h.isin && h.isin.length === 12,
	);
	...
	console.log(
		`[ISIN Intelligence] Classifying ${holdingsWithISIN.length} instruments by ISIN prefix`,
	);
	...
	for (const holding of holdingsWithISIN) {
		try {
			...
			const metadata = {
				...
				isETF:
					holding.name?.toUpperCase().includes("ETF") ||
					holding.name?.toUpperCase().includes("BEES") ||
					holding.name?.toUpperCase().includes("EXCHANGE TRADED"),
			};

			const detection = await isinIntelligenceService.detectInstrument(
				holding.isin!,
				metadata,
			);
			...
			switch (detection.assetClass) {
				...
				case "Mutual Fund":
					assetType =
						detection.instrumentType === "ETF" ? "etf" : "mutual_fund";
					break;
				case "Fixed Income":
				case "Debt":
					assetType = "bond";
				...
				case "Alternatives":
					assetType =
						detection.instrumentType === "REIT" ||
						detection.instrumentType === "InvIT"
			...
			if (detection.isEdgeCase) {
				console.log(
					`[ISIN Intelligence] Edge case detected: ${holding.isin} → ${detection.instrumentType} (${detection.edgeCaseType})`,
				);
			}
		} catch (error: any) {
			console.warn(
				`[ISIN Intelligence] Failed to classify ${holding.isin}: ${error.message}`,
			);
		}
	}

	console.log(
		`[ISIN Intelligence] Classified ${classificationMap.size} instruments`,
	);
	...
	return holdings.map((holding) => {
		if (holding.isin && classificationMap.has(holding.isin)) {
			...
			return {
				...holding,
				assetType:
					classification.assetType !== "other"
...
export async function enrichHoldingsWithDatabaseLookup(
	holdings: ImportedHolding[],
): Promise<ImportedHolding[]> {
	...
	console.log(
		`[CAS Parser] Looking up ${isins.length} ISINs in database for fund name and NAV enrichment`,
	);
	...
	return holdings.map((holding) => {
		if (holding.isin && fundLookup.has(holding.isin)) {
			...
			if (originalMarketValue === 0) {
				shouldUseDbNav = true;
				console.log(
					`[CAS Parser] No market value found, will calculate from DB NAV`,
				);
				...
				shouldUseDbNav = true;
				console.log(
					`[CAS Parser] Market value equals invested value (fallback), will recalculate`,
				);
			}
			...
			if (shouldUseDbNav && holding.quantity > 0 && currentNav > 0) {
				...
				console.log(
					`[CAS Parser] Calculated current value: ${holding.quantity} units × ₹${currentNav} NAV = ₹${finalCurrentValue.toFixed(2)}`,
				);
				...
				console.log(
					`[CAS Parser] PRESERVING original CAS market value: ₹${originalMarketValue.toFixed(2)} (DB NAV would give ₹${(holding.quantity * currentNav).toFixed(2)})`,
				);
			}
			...
			console.log(
				`[CAS Parser] Enriched: ${dbFund.schemeName} | Folio: ${holding.folioNumber} | Units: ${holding.quantity} | NAV: ₹${currentNav} | Value: ₹${finalCurrentValue.toFixed(2)}`,
			);
...
function parseWealthyPDFFormat(text: string): ImportedHolding[] {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (
			line.includes("Sync Info") ||
			line === "More" ||
			line.includes("https://") ||
			line.includes("Portfolio Overview") ||
			line.match(/^MUTUAL FUNDS \(\d+\)/) ||
			line.includes("Holding Analysis") ||
			line.match(/^\d+\/\d+\/\d+/) ||
			line.length < 3 ||
			line.match(/^-- \d+ of \d+ --$/) ||
			line === "Hello" ||
			line.includes("Wealth Manager") ||
			line.includes("Here is an analysis") ||
			line.includes("fund portfolio") ||
			line.includes("manager will walk")
		) {
			continue;
		}
		...
		if (expectingInvestedValue && currentFund) {
			...
			if (valueMatch) {
				currentFund.investedValue = Number.parseFloat(
					valueMatch[1].replace(/,/g, ""),
				);
				console.log(
					`[Wealthy Parser] Invested value for ${currentFund.name}: ₹${currentFund.investedValue}`,
				);
			}
		...
		if (expectingCurrentValue && currentFund) {
			const valueMatch = line.match(/₹\s*([\d,]+)/);
			if (valueMatch) {
				currentFund.currentValue = Number.parseFloat(
					valueMatch[1].replace(/,/g, ""),
				);
				console.log(
					`[Wealthy Parser] Current value for ${currentFund.name}: ₹${currentFund.currentValue}`,
				);
			}
		...
		if (expectingReturns && currentFund) {
			...
			if (percentMatch) {
				currentFund.unrealizedGainPercent = Number.parseFloat(percentMatch[1]);
				currentFund.unrealizedGain =
					currentFund.currentValue - currentFund.investedValue;
				console.log(
					`[Wealthy Parser] Returns for ${currentFund.name}: ${currentFund.unrealizedGainPercent}%`,
				);
			}
		...
		const matchingPrefix = amcPrefixes.find((prefix) =>
			line.startsWith(prefix),
		);
		const isAmcPrefix =
			matchingPrefix &&
			nextLine &&
			nextLine.includes("Fund") &&
		...
		if (isFundName) {
			...
			if (currentFund?.name && currentFund.currentValue > 0) {
				holdings.push(currentFund);
				console.log(
					`[Wealthy Parser] ✓ Added fund: ${currentFund.name} = ₹${currentFund.currentValue}`,
				);
			}
			...
			const nextLineIsContinuation =
				...
				(nextLine.includes("(G)") ||
					...
					nextLine.match(
						/^(Direct|Regular|Active|Growth|Dividend|Hybrid|Services)/i,
					));
			...
			console.log(
				`[Wealthy Parser] Found fund #${fundIndex}: ${currentFund.name}`,
			);
		}
	}
	...
	if (currentFund?.name && currentFund.currentValue > 0) {
		holdings.push(currentFund);
		console.log(
			`[Wealthy Parser] ✓ Added last fund: ${currentFund.name} = ₹${currentFund.currentValue}`,
		);
	}
...
function parseGenericFormat(text: string): ImportedHolding[] {
	...
	const valuePattern =
		/([A-Za-z][A-Za-z0-9\s\-&\.]{3,40})\s+(\d+(?:,\d+)*(?:\.\d+)?)\s+(?:units?|shares?|qty)?\s*₹?\s*(\d+(?:,\d+)*(?:\.\d+)?)/i;
	...
	for (const line of lines) {
		...
		if (match) {
			...
			const quantity = Number.parseFloat(qty.replace(/,/g, ""));
			const currentValue = Number.parseFloat(value.replace(/,/g, ""));
...
function calculateAllocation(holdings: ImportedHolding[]): ImportedAllocation {
	...
	for (const holding of holdings) {
		...
		if (holding.assetType === "gold" || /gold|sovereign/i.test(name)) {
			...
			/debt|bond|gilt|fixed|liquid|overnight/i.test(name)
		) {
			debt += value;
		} else if (
			holding.assetType === "equity" ||
			holding.assetType === "mutual_fund" ||
			/equity|growth|index|flexi|multi.*cap/i.test(name)
		) {
			equity += value;
		} else {
			others += value;
...
export async function parsePDFPortfolio(
	buffer: Buffer,
	fileName: string,
): Promise<ParseResult> {
	...
	try {
		...
		const portfolioSummaryMatch = text.match(
			/PORTFOLIO\s+SUMMARY[\s\S]*?Total\s+([\d,]+\.?\d*)\s+([\d,]+\.?\d*)/i,
		);
		if (portfolioSummaryMatch) {
			expectedTotalCost = Number.parseFloat(
				portfolioSummaryMatch[1].replace(/,/g, ""),
			);
			expectedTotalValue = Number.parseFloat(
				portfolioSummaryMatch[2].replace(/,/g, ""),
			);
			console.log("[PDF Parser] PORTFOLIO SUMMARY found:");
			console.log(
				`[PDF Parser]   Total Cost: ₹${expectedTotalCost.toLocaleString("en-IN")}`,
			);
			console.log(
				`[PDF Parser]   Total Market: ₹${expectedTotalValue.toLocaleString("en-IN")}`,
			);

			// Extract individual AMC lines
			// Pattern: "AMC Name  cost_value  market_value" (AMC names often have "Mutual Fund" suffix)
			const amcPattern =
				/([A-Za-z\s]+(?:Mutual\s+Fund|MF))\s+([\d,]+\.?\d*)\s+([\d,]+\.?\d*)/gi;
			let amcMatch;
			while ((amcMatch = amcPattern.exec(text)) !== null) {
				const amcName = amcMatch[1].trim();
				const cost = Number.parseFloat(amcMatch[2].replace(/,/g, ""));
				const market = Number.parseFloat(amcMatch[3].replace(/,/g, ""));

				// Only add if values are reasonable (> 1000)
				if (
					cost > 1000 &&
					market > 1000 &&
					!amcName.toLowerCase().includes("total")
				) {
					amcSummaries.push({ amcName, costValue: cost, marketValue: market });
					console.log(
						`[PDF Parser]   ${amcName}: Cost ₹${cost.toLocaleString("en-IN")}, Market ₹${market.toLocaleString("en-IN")}`,
					);
				}
			}
		}
		...
		if (!expectedTotalValue) {
			...
			for (const pattern of totalValuePatterns) {
				...
				if (match) {
					expectedTotalValue = Number.parseFloat(match[1].replace(/,/g, ""));
					console.log(
						"[PDF Parser] Expected total value from PDF:",
						expectedTotalValue.toLocaleString("en-IN"),
					);
					break;
				}
			}
		...
		if (expectedCount) {
			console.log(
				"[PDF Parser] Expected fund count from PDF header:",
				expectedCount,
			);
		}
		...
		if (broker === "CAMS/KFintech CAS") {
			...
			console.log(
				"[PDF Parser] CAMS Holding Statement parser found:",
				holdings.length,
				"holdings",
			);
			...
			if (holdings.length === 0) {
				...
				console.log(
					"[PDF Parser] CAMS Table parser found:",
					holdings.length,
					"holdings",
				);
			}
			...
			if (holdings.length === 0) {
				...
				console.log(
					"[PDF Parser] CAS format parser found:",
					holdings.length,
					"holdings",
				);
			}
		...
		if (holdings.length === 0) {
			...
			if (holdings.length > 0) {
				console.log(
					"[PDF Parser] Fallback: CAMS Holding Statement parser found:",
					holdings.length,
					"holdings",
				);
			}
		...
		const unimportedCount = expectedCount
			? Math.max(0, expectedCount - importedCount)
			: 0;
		const needsManualReview = unimportedCount > 0;

		// Calculate parsed total value
		let parsedTotalValue = holdings.reduce(
			(sum, h) => sum + (h.currentValue || 0),
			0,
		);
		const parsedTotalCost = holdings.reduce(
			(sum, h) => sum + (h.investedValue || 0),
			0,
		);
		console.log(
			"[PDF Parser] Parsed total value:",
			parsedTotalValue.toLocaleString("en-IN"),
		);
		console.log(
			"[PDF Parser] Parsed total cost:",
			parsedTotalCost.toLocaleString("en-IN"),
		);
		...
		if (holdings.length === 0) {
			errors.push(
				"Could not extract holdings from PDF. Please verify the format.",
			);
		}
		...
		if (expectedTotalValue && amcSummaries.length > 0 && parsedTotalValue > 0) {
			const valueDiffPercent =
				(Math.abs(parsedTotalValue - expectedTotalValue) / expectedTotalValue) *
				100;

			if (valueDiffPercent > 20) {
				console.log(
					"[PDF Parser] Significant value mismatch detected. Attempting AMC-based correction...",
				);
				...
				for (const amc of amcSummaries) {
					...
					const normalizedName = amc.amcName
						.toLowerCase()
						.replace(/\s+/g, " ")
						.trim();
				...
				for (const holding of holdings) {
					...
					for (const [key, amc] of amcLookup.entries()) {
						if (
							fundName.includes(key) ||
							(key.split(" ")[0] && fundName.includes(key.split(" ")[0]))
						) {
							matchedAMC = amc;
							break;
						}
				...
				for (const [amcName, amcHoldings] of holdingsByAMC.entries()) {
					...
					const parsedAMCTotal = amcHoldings.reduce(
						(sum, h) => sum + (h.currentValue || 0),
						0,
					);
					...
					if (
						parsedAMCTotal > 0 &&
						Math.abs(parsedAMCTotal - expectedAMCTotal) / expectedAMCTotal > 0.1
					) {
						const scaleFactor = expectedAMCTotal / parsedAMCTotal;
						console.log(
							`[PDF Parser] AMC ${amcName}: Scaling by ${scaleFactor.toFixed(2)} (parsed ₹${parsedAMCTotal.toLocaleString("en-IN")} → expected ₹${expectedAMCTotal.toLocaleString("en-IN")})`,
						);
				...
				if (correctedCount > 0) {
					...
					parsedTotalValue = holdings.reduce(
						(sum, h) => sum + (h.currentValue || 0),
						0,
					);
					console.log(
						`[PDF Parser] Applied AMC-based corrections to ${correctedCount} holdings. New total: ₹${parsedTotalValue.toLocaleString("en-IN")}`,
					);
				}
			}
		}
		...
		if (expectedTotalValue && parsedTotalValue > 0) {
			...
			if (valueDiffPercent > 10) {
				...
				console.warn(
					`[Portfolio Parser] VALUE MISMATCH: Expected ₹${expectedTotalValue.toLocaleString("en-IN")}, Parsed ₹${parsedTotalValue.toLocaleString("en-IN")}`,
				);
				console.warn(
					`[Portfolio Parser] Missing value: ₹${missingValue.toLocaleString("en-IN")} (${valueDiffPercent.toFixed(1)}% difference)`,
				);

				if (missingValue > 0) {
					errors.push(
						`Parsing may be incomplete. Expected ₹${(expectedTotalValue / 100000).toFixed(2)}L but found ₹${(parsedTotalValue / 100000).toFixed(2)}L. Some holdings may need manual review.`,
					);
				}
			}
		}
		...
		if (needsManualReview && expectedCount) {
			...
			console.warn(
				`[Portfolio Parser] ALERT: ${errorMsg} (File: ${fileName}, Broker: ${broker})`,
			);
			console.warn(
				`[Portfolio Parser] Imported funds: ${holdings.map((h) => h.name).join(", ")}`,
			);
		}

		let avgConfidence =
			holdings.length > 0
				? holdings.reduce((sum, h) => sum + (h.confidenceScore || 50), 0) /
					holdings.length
				: 30;
		...
		if (holdings.length === 0 || avgConfidence < 60) {
			console.log(
				"[PDF Parser] Low confidence or no holdings, trying AI-assisted parsing...",
			);
			try {
				const aiHoldings = await parsePortfolioWithAI(text);
				if (aiHoldings.length > 0) {
					console.log(
						`[PDF Parser] AI parser found ${aiHoldings.length} holdings`,
					);
					holdings = aiHoldings;
					avgConfidence = 75;
					errors.length = 0;
					errors.push(
						"Parsed using AI assistance - please verify extracted data",
					);
				}
			} catch (aiError) {
...
export async function parseURLPortfolio(
	html: string,
	url: string,
): Promise<ParseResult> {
	try {
		...
		const isExpiredLink = expiredIndicators.some((indicator) =>
		...
		if (isExpiredLink) {
			return {
				...
				confidenceScore: 0,
				errors: [
					"The report link has expired or is no longer valid. Please generate a fresh link from your broker platform and use it immediately.",
				],
			};
		}
		...
		let errorMessage =
			"Could not extract holdings from URL. Please verify the link.";
		if (url.includes("wealthy.in") && holdings.length === 0) {
			errorMessage =
				"Could not extract holdings. Wealthy.in links expire quickly. Please generate a fresh report link and use it within 5 minutes, or save the page as HTML and upload it.";
		}
...
function parseWealthyHTML($: cheerio.CheerioAPI): ImportedHolding[] {
	...
	const fundNamePattern =
		/^[\d\s]*[A-Za-z0-9][\w\s\-\(\)&'\.]+(?:Fund|Scheme|\(G\)|\(IDCW\)|\(Growth\)|\(Dividend\))[\s\-\(\)A-Za-z0-9]*$/i;
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (currentFund) {
			...
			if (returnsMatch) {
				currentFund.unrealizedGainPercent = Number.parseFloat(returnsMatch[1]);
			}
...
function extractHoldingsFromJSON(data: any, broker: string): ImportedHolding[] {
	...
	for (const arr of possibleArrays) {
		...
		for (let i = 0; i < arr.length; i++) {
			const item = arr[i];
			const name =
				item.schemeName || item.fundName || item.name || item.scheme_name || "";
			const value =
				item.currentValue ||
				item.current_value ||
				item.marketValue ||
			...
			const invested =
				item.investedValue || item.invested_value || item.cost || value;
			...
			if (name && value > 0) {
				holdings.push({
					...
					assetType: "mutual_fund",
					quantity:
						typeof units === "number" ? units : Number.parseFloat(units) || 1,
					currentValue:
						typeof value === "number" ? value : Number.parseFloat(value) || 0,
					investedValue:
						typeof invested === "number"
							? invested
							: Number.parseFloat(invested) || undefined,
...
function parseGrowwHTML($: cheerio.CheerioAPI): ImportedHolding[] {
	...
	$('[class*="stock"], [class*="fund"], [class*="holding"]').each((i, elem) => {
		...
		const valueText = $(elem)
			.find('[class*="value"], [class*="current"]')
			.first()
			.text()
			.trim();

		if (name && valueText) {
			const value = Number.parseFloat(valueText.replace(/[₹,\s]/g, ""));
			if (value > 0) {
				holdings.push({
					id: `holding-${Date.now()}-${i}`,
					name,
...
function parseGenericHTML($: cheerio.CheerioAPI): ImportedHolding[] {
	...
	$("table tr").each((i, row) => {
		...
		if (cells.length >= 3) {
			...
			const qty = Number.parseFloat($(cells[1]).text().replace(/[,\s]/g, ""));
			...
			if (name && !Number.isNaN(value) && value > 0) {
				holdings.push({
					id: `holding-${Date.now()}-${i}`,
					name,
					assetType: /fund|scheme|growth/i.test(name)
						? "mutual_fund"
						: "equity",
...
export function createPortfolioSnapshot(
	...
	const totalInvestedValue = holdings.reduce(
		(sum, h) => sum + (h.investedValue || h.currentValue),
		0,
	);
	const allocation = calculateAllocation(holdings);

	const hasLowConfidence = holdings.some(
		(h) => (h.confidenceScore || 100) < 60,
	);
