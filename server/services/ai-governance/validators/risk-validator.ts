...
export class RiskValidator implements GovernanceValidator {
	async validate(input: GovernanceInput): Promise<ValidationResult> {
		...
		if (
			recommendationStr.includes("equity") ||
			recommendationStr.includes("stock") ||
			recommendationStr.includes("crypto")
		) {
			risk_level = "high";
			downside_probability = 0.6;
			volatility_score = 0.8;
			liquidity_profile = recommendationStr.includes("unlisted")
				? "low"
				: "high";
		} else if (
			recommendationStr.includes("bond") ||
			recommendationStr.includes("fd") ||
			recommendationStr.includes("gsec")
		) {
			risk_level = "low";
