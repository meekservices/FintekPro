...
export class FinancialValidator implements GovernanceValidator {
	async validate(input: GovernanceInput): Promise<ValidationResult> {
		...
		if (
			recommendationStr.includes("undervalued") &&
			input.ai_output?.confidence_score &&
			input.ai_output.confidence_score < 0.7
		) {
			violations.push({
				module: "FinancialValidator",
				severity: "MINOR",
				message:
					"AI claims 'undervalued' but confidence score is too low to deterministically support it without core engine alignment.",
				code: "FIN_001",
			});
		}
				...
				? {
						suggested_modifications: {
							enforce_language: [
								{
									search: "undervalued",
									replace: "potentially undervalued based on historicals",
								},
							],
						},
					}
