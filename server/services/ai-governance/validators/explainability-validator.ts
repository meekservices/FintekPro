  ...
  async validate(input: GovernanceInput): Promise<ValidationResult> {
    ...
    if (!ai_output) {
      return {
        ...
        violations: [{
          ...
          severity: "CRITICAL",
          message: "No structured AI output present / Black-box output detected.",
          code: "EXP_001"
        }]
      };
    }

    if (!ai_output.factors_considered || ai_output.factors_considered.length === 0) {
      violations.push({
        module: "ExplainabilityValidator",
        severity: "CRITICAL",
        message: "No explainable factors_considered provided in advisory recommendation.",
        code: "EXP_002"
      });
    }
    ...
    if (!ai_output.confidence_score) {
      ...
      violations.push({
        module: "ExplainabilityValidator",
        severity: "CRITICAL",
        message: "AI confidence score below threshold. Must suggest human advisor.",
        code: "EXP_004"
      });
    }
