...
export class FinancialMetricsRefreshScheduler {
  ...
  static getInstance(): FinancialMetricsRefreshScheduler {
    if (!this.instance) {
      this.instance = new FinancialMetricsRefreshScheduler();
    }
  ...
  start(): void {
    ...
    console.log('[MetricsScheduler] Starting financial metrics refresh scheduler...');
    this.isRunning = true;
    ...
    this.mutualFundCronJob = cron.schedule('30 17 * * *', async () => {
      console.log('[MetricsScheduler] Triggering scheduled mutual fund refresh (11 PM IST)');
    ...
    this.stockCronJob = cron.schedule('0 18 * * 1-5', async () => {
      console.log('[MetricsScheduler] Triggering scheduled stock metrics refresh (6 PM IST)');
    ...
    console.log('[MetricsScheduler] - Mutual fund refresh: Daily at 11:00 PM IST');
    console.log('[MetricsScheduler] - Stock metrics refresh: Weekdays at 6:00 PM IST');
  }
  ...
  async refreshMutualFundMetrics(): Promise<RefreshJobResult> {
    ...
    console.log(`[MetricsScheduler] Starting mutual fund metrics refresh at ${startedAt.toISOString()}`);
    ...
    try {
      ...
      const result = await mutualFundMetricsService.refreshAllReturns({ limit: 500 });
      ...
      const jobResult: RefreshJobResult = {
        ...
        status: result.failedUpdates === 0 ? 'success' : 
                result.successfulUpdates > 0 ? 'partial' : 'failed',
      ...
      console.log(`[MetricsScheduler] Mutual fund refresh completed: ${result.successfulUpdates} updated, ${result.failedUpdates} failed`);
  ...
  async refreshStockMetrics(): Promise<RefreshJobResult> {
    const startedAt = new Date();
    console.log(`[MetricsScheduler] Starting stock metrics refresh at ${startedAt.toISOString()}`);

    try {
      // Refresh metrics for stocks that need updating
      const result = await stockMetricsService.refreshAllMetrics({ limit: 200 });
      ...
      console.log(`[MetricsScheduler] Stock refresh completed: ${result.successfulUpdates} updated, ${result.failedUpdates} failed`);
  ...
  private async logJobExecution(result: RefreshJobResult): Promise<void> {
    try {
      ...
      console.log('[MetricsScheduler] Could not log to database (table may not exist)');
    }
  }
  ...
  getStatus(): SchedulerStatus {
    ...
    return {
      ...
      nextMutualFundRefresh: this.lastMutualFundRefresh 
        ? new Date(this.lastMutualFundRefresh.completedAt.getTime() + 24 * 60 * 60 * 1000)
        : new Date(now.getTime() + 60 * 60 * 1000), // 1 hour from now if never run
      nextStockRefresh: this.lastStockRefresh
        ? new Date(this.lastStockRefresh.completedAt.getTime() + 24 * 60 * 60 * 1000)
...
export const financialMetricsRefreshScheduler = FinancialMetricsRefreshScheduler.getInstance();
