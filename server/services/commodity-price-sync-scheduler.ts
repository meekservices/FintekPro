...
class CommodityPriceSyncScheduler {
	...
	constructor() {
		console.log(
			"✅ Commodity Price Sync Scheduler initialized (4-tier: AV GLOBAL_QUOTE → AV Physical LME → Python/yfinance → Yahoo)",
		);
	}
	...
	start(): void {
		...
		this.isRunning = true;
		console.log(
			"[CommoditySync] Starting — refreshes every 6 hours, first run at T+9 min",
		);
		...
		setTimeout(
			async () => {
				try {
					...
					console.error("[CommoditySync] Startup refresh failed:", err);
				}
			},
			9 * 60 * 1000,
		);
	...
	private scheduleNextSync(): void {
		this.syncTimer = setTimeout(async () => {
			try {
				...
				console.error("[CommoditySync] Scheduled refresh failed:", err);
			}
			if (this.isRunning) this.scheduleNextSync();
		}, this.syncIntervalMs);
		console.log(
			`[CommoditySync] Next sync in ${this.syncIntervalMs / 60000} minutes`,
		);
	}
	...
	private async fetchAlphaVantagePrice(
		...
		try {
			...
			const prevClose = Number.parseFloat(q["08. previous close"]) || undefined;
			const change = Number.parseFloat(q["09. change"]) || undefined;
			const changePct =
			...
			return {
				...
				high: Number.parseFloat(q["03. high"]) || undefined,
				low: Number.parseFloat(q["04. low"]) || undefined,
	...
	// ── Tier 2: Python / yfinance batch (/market/quotes) ─────────────────────
	private async fetchPythonBatchPrices(
		internalSymbols: string[],
	): Promise<Map<string, CommodityPriceData>> {
		...
		const futuresSymbols = internalSymbols
			.map((s) => ({
				internal: s,
				futures: this.FUTURES_SYMBOLS[s.toUpperCase()],
			}))
			.filter((x) => x.futures);
		...
		try {
			...
			for (const { internal, futures } of futuresSymbols) {
				...
				results.set(internal, {
					symbol: internal,
					price: Number.parseFloat(q.price),
					previousClose:
						q.previousClose != null
							? Number.parseFloat(q.previousClose)
							: undefined,
					change: q.change != null ? Number.parseFloat(q.change) : undefined,
					changePercent:
						q.changePercent != null
							? Number.parseFloat(q.changePercent)
							: undefined,
					high: q.dayHigh != null ? Number.parseFloat(q.dayHigh) : undefined,
					low: q.dayLow != null ? Number.parseFloat(q.dayLow) : undefined,
	...
	private async fetchAlphaVantagePhysicalPrice(
		internalSymbol: string,
	): Promise<CommodityPriceData | null> {
		const apiKey = process.env.ALPHA_VANTAGE_API_KEY;
		if (!apiKey) return null;
		...
		const avFunction =
			this.AV_PHYSICAL_COMMODITY_FUNCTIONS[internalSymbol.toUpperCase()];
		...
		try {
			...
			const price = Number.parseFloat(data[0]?.value);
			if (!price || price <= 0) return null;

			const prev =
				data.length > 1 ? Number.parseFloat(data[1].value) : undefined;
	...
	async runPriceRefresh(): Promise<{ updated: number; errors: number }> {
		...
		const staleCommodities = await db
			.select({
				id: commodities.id,
			...
			.from(commodities)
			.where(
				sql`${commodities.lastUpdated} IS NULL OR ${commodities.lastUpdated} < ${staleThreshold}`,
			)
		...
		console.log(
			`[CommoditySync] ${staleCommodities.length} stale commodities → starting tiered refresh`,
		);
		...
		for (const c of staleCommodities) {
			if (c.symbol)
				byId.set(c.symbol.toUpperCase(), {
					id: c.id,
					currentPrice: c.currentPrice?.toString() || "0",
				});
		}

		const allSymbols = staleCommodities
		...
		if (avSymbols.length && process.env.ALPHA_VANTAGE_API_KEY) {
			console.log(
				`[CommoditySync] Tier 1 — Alpha Vantage for ${avSymbols.length} metals/energy`,
			);
			const settled = await Promise.allSettled(
				avSymbols.map((s) => this.fetchAlphaVantagePrice(s)),
			);
			let avHit = 0;
		...
		const avPhysNeeded = allSymbols.filter(
			(s) => !priced.has(s) && this.AV_PHYSICAL_COMMODITY_FUNCTIONS[s],
		);
		if (avPhysNeeded.length && process.env.ALPHA_VANTAGE_API_KEY) {
			console.log(
				`[CommoditySync] Tier 1.5 — AV Physical for ${avPhysNeeded.length} LME metals`,
			);
			const avPhysSettled = await Promise.allSettled(
				avPhysNeeded.map((s) => this.fetchAlphaVantagePhysicalPrice(s)),
			);
			let avPhysHit = 0;
			...
			if (avPhysHit > 0)
				console.log(
					`[CommoditySync] AV Physical filled ${avPhysHit}/${avPhysNeeded.length}`,
				);
		}
		...
		const pythonNeeded = allSymbols.filter(
			(s) => !priced.has(s) && !this.MCX_INDIA_ONLY.has(s),
		);
		if (pythonNeeded.length) {
			console.log(
				`[CommoditySync] Tier 2 — Python/yfinance batch for ${pythonNeeded.length} symbols`,
			);
			...
			console.log(
				`[CommoditySync] Python filled ${pythonResults.size}/${pythonNeeded.length}`,
			);
		}
		...
		const yahooNeeded = allSymbols.filter(
			(s) => !priced.has(s) && !this.MCX_INDIA_ONLY.has(s),
		);
		if (yahooNeeded.length) {
			console.log(
				`[CommoditySync] Tier 3 — Yahoo last-resort for ${yahooNeeded.length} symbols`,
			);
		...
		for (const [sym, data] of priced) {
			...
			try {
				...
				errors++;
				console.error(
					`[CommoditySync] DB save failed for ${sym}: ${err.message}`,
				);
			}
		}
		...
		if (apiMissed.length) {
			...
			console.warn(
				`[CommoditySync] ${apiMissed.length} symbols had no price from any tier: ${apiMissed.join(", ")}`,
			);
		}

		console.log(
			`[CommoditySync] Refresh complete — updated: ${updated}, errors: ${errors}`,
		);
		return { updated, errors };
	}
	...
	async runStartupCatchUp(): Promise<{ updated: number; errors: number }> {
		...
		console.log(
			`[CommoditySync] Startup catch-up done: ${result.updated} updated, ${result.errors} errors`,
		);
		return result;
	}
