...
export class IrisClient {
  ...
  constructor() {
    ...
    logger.info('[IRIS] Resilient Client initialized', { baseUrl: IRIS_BASE_URL });
  }
  ...
  async authenticate(): Promise<{ success: boolean; requiresOtp?: boolean; message?: string }> {
    ...
    try {
      ...
      logger.error('[IRIS] Login failed', { error: err?.response?.data || err.message });
  ...
  async call<T = any>(
    ...
    if (this.isCircuitOpen()) {
      throw new Error('IRIS API is currently unavailable (Circuit Breaker Open)');
    }
  ...
  async fetchProducts(type: 'mf' | 'pms' | 'aif' | 'fd') {
    switch (type) {
      case 'mf': return this.call('/sif/schemes');
      case 'pms': return this.call('/pms/links');
      case 'aif': return this.call('/aif/links');
      case 'fd': return this.call('/user/fixed-deposit/products');
      default: throw new Error('Unsupported product type');
    }
  }

  async placeOrder(orderPayload: any) {
    return this.call('/sif/transactions/purchase', 'POST', { ...orderPayload, partnerCode: "FINTEKPRO" });
  }
