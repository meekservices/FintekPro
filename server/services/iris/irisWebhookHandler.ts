...
export class IrisWebhookHandler {
	...
	async handleWebhook(payload: any, signature?: string) {
		logger.info("[IrisWebhookHandler] Received webhook payload", {
		...
		try {
			switch (payload.eventType) {
				...
				default:
					logger.warn(
						`[IrisWebhookHandler] Unhandled event type: ${payload.eventType}`,
					);
			}
			return { success: true };
		} catch (error: any) {
			logger.error("[IrisWebhookHandler] Failed to process webhook", {
				error: error.message,
			});
			throw error;
		}
	...
	private async processOrderStatusUpdate(payload: any) {
		...
		logger.info(
			`[IrisWebhookHandler] Updating order ${orderId} to status ${status}`,
		);
	...
	private async processSipUpdate(payload: any) {
		...
		logger.info(
			`[IrisWebhookHandler] Updating SIP ${sipId} to status ${status}`,
		);
		// Database updates go here...
	}
	...
	private async processRedemptionUpdate(payload: any) {
		...
		logger.info(
			`[IrisWebhookHandler] Processing redemption ${orderId} for amount ${amount}`,
		);
		// Database updates go here...
	}
}
