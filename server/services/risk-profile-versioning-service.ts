...
class RiskProfileVersioningService {
	...
	createVersion(
		...
		const newVersion: RiskProfileVersion = {
			...
			createdAt: new Date(),
			expiresAt: new Date(
				Date.now() + this.PROFILE_VALIDITY_DAYS * 24 * 60 * 60 * 1000,
			),
			isActive: true,
		};
		...
		console.log(
			`📊 [Risk Profile] Created version ${versionNumber} for user ${userId.substring(0, 8)}...: ${computedProfile.riskCategory} (score: ${computedProfile.riskScore})`,
		);
	...
	computeRiskProfile(
		answers: RiskProfileVersion["answers"],
	): RiskProfileVersion["computedProfile"] {
		const objectiveWeight = 0.25;
		const horizonWeight = 0.2;
		const toleranceWeight = 0.3;
		const incomeWeight = 0.15;
		const experienceWeight = 0.1;

		const objectiveScore =
			this.weights.investmentObjective[answers.investmentObjective] || 50;
		const horizonScore =
			this.weights.investmentHorizon[answers.investmentHorizon] || 50;
		const toleranceScore =
			this.weights.riskTolerance[answers.riskTolerance] || 50;
		const incomeScore = this.weights.incomeLevel[answers.incomeLevel] || 50;
		const experienceScore =
			this.weights.tradingExperience[answers.tradingExperience] || 50;
	...
	compareVersions(
		...
		if (v1.answers.investmentObjective !== v2.answers.investmentObjective) {
			changes.push(
				`Investment objective: ${v1.answers.investmentObjective} → ${v2.answers.investmentObjective}`,
			);
		}
		if (v1.answers.investmentHorizon !== v2.answers.investmentHorizon) {
			changes.push(
				`Investment horizon: ${v1.answers.investmentHorizon} → ${v2.answers.investmentHorizon}`,
			);
		}
		if (v1.answers.riskTolerance !== v2.answers.riskTolerance) {
			changes.push(
				`Risk tolerance: ${v1.answers.riskTolerance} → ${v2.answers.riskTolerance}`,
			);
		}
		if (v1.answers.incomeLevel !== v2.answers.incomeLevel) {
			changes.push(
				`Income level: ${v1.answers.incomeLevel} → ${v2.answers.incomeLevel}`,
			);
		}
		if (v1.answers.tradingExperience !== v2.answers.tradingExperience) {
			changes.push(
				`Trading experience: ${v1.answers.tradingExperience} → ${v2.answers.tradingExperience}`,
			);
		}
		...
		return {
			...
			changes,
			riskCategoryChanged:
				v1.computedProfile.riskCategory !== v2.computedProfile.riskCategory,
			scoreDifference:
				v2.computedProfile.riskScore - v1.computedProfile.riskScore,
		};
	}
