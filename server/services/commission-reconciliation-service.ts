...
class CommissionReconciliationService {
	...
	async processPaymentStatement(
		...
		for (let i = 0; i < rows.length; i++) {
			...
			try {
				const paymentResult = await this.processPaymentRow(
					row,
					paidBy,
					sourceType,
					batch.id,
	...
	private async processPaymentRow(
      ...
      `);
			...
			if (ledgerEntries.length > 0) {
				...
				expectedAmount = Number.parseFloat(ledgerEntry.net_commission) || 0;
				...
				if (Math.abs(matchVariance) <= this.defaultTolerance) {
					...
					row.commissionAmount < expectedAmount
				) {
					matchStatus = "partial";
				} else {
    ...
    `);
		...
		for (const payment of payments) {
			totalReceived += Number.parseFloat(payment.paid_amount) || 0;
			...
			if (payment.matched_at && payment.payment_date) {
				const days = Math.floor(
					(new Date(payment.matched_at).getTime() -
		...
		const totalExpected = ledgerEntries.reduce(
			(sum, e) => sum + (Number.parseFloat(e.net_commission) || 0),
			0,
		);
		...
		return {
			...
			overduePayments,
			avgDaysToPayment:
				paymentCount > 0 ? Math.round(totalDaysToPayment / paymentCount) : 0,
			disputeRate:
				payments.length > 0
    ...
    `);
		...
		const expectedAmount = Number.parseFloat(ledgerEntry.net_commission) || 0;
		const paidAmount = Number.parseFloat(payment.paid_amount) || 0;
		const variance = paidAmount - expectedAmount;
		const matchStatus =
			Math.abs(variance) <= this.defaultTolerance ? "matched" : "partial";
		const now = new Date().toISOString();
    ...
    `);
		...
		for (const entry of ledgerEntries) {
			if (entry.created_at) {
				...
				monthlyData[month].amount +=
					Number.parseFloat(entry.net_commission) || 0;
		...
		const avgCommission =
			totalLoans > 0 ? summary.totalExpected / totalLoans : 0;

		return {
			approvalToDisbursementRate:
				totalLoans > 0
...
export const commissionReconciliationService =
	new CommissionReconciliationService();
