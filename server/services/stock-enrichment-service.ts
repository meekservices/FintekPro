...
class StockEnrichmentService {
	...
	async enrichAllMissingData(): Promise<EnrichmentResult[]> {
		...
		try {
			...
			console.log(
				`[StockEnrichment] Starting enrichment for ${this.progress.total} stocks`,
			);
			...
			for (const stock of stocksNeedingEnrichment) {
				...
				if (this.progress.processed % 50 === 0) {
					console.log(
						`[StockEnrichment] Progress: ${this.progress.processed}/${this.progress.total}`,
					);
				}
			...
			console.log(
				`[StockEnrichment] Complete: ${this.progress.enriched} enriched, ${this.progress.failed} failed`,
			);
		} catch (error: any) {
	 ...
	 */
	async enrichSingleStock(
		stock: typeof listedStocks.$inferSelect,
		...
		try {
			...
			if (!stock.cin || !stock.companyPan) {
				try {
					const searchResult = await credhiveService.searchCompany(
						stock.companyName,
					);
					...
					console.log(
						`[StockEnrichment] Credhive failed for ${stock.symbol}:`,
						error,
					);
				}
			}
			...
			if (!stock.roe || !stock.roce) {
				try {
					...
					console.log(
						`[StockEnrichment] Credhive financials failed for ${stock.symbol}:`,
						error,
					);
				}
			}
			...
			if (!stock.peRatio || !stock.bseCode || !stock.currentPrice) {
				try {
					const exchangeData = await exchangeStockService.getStockData(
						stock.symbol,
					);
					...
					if (exchangeData) {
						...
						if (
							fieldsEnriched.some((f) =>
								["peRatio", "bseCode", "currentPrice"].includes(f),
							)
						) {
					...
					console.log(
						`[StockEnrichment] NSE/BSE failed for ${stock.symbol}:`,
						error,
					);
				}
			}
			...
			if (!updateData.peRatio && !stock.peRatio) {
				try {
					...
					let finnhubSymbol = stock.nseCode
						? `${stock.symbol}.NS`
						: stock.bseCode
							? `${stock.symbol}.BO`
							: stock.symbol;
					let finnhubResult =
						await finnhubService.getBasicFinancials(finnhubSymbol);
					...
					if (
						...
						finnhubResult.data?.metric?.peBasicExclExtraTTM
					) {
						updateData.peRatio = String(
							finnhubResult.data.metric.peBasicExclExtraTTM,
						);
						fieldsEnriched.push("peRatio");
						primarySource =
							primarySource === "local" ? "finnhub" : primarySource;
					}
				} catch (error) {
					console.log(
						`[StockEnrichment] Finnhub failed for ${stock.symbol}:`,
						error,
					);
				}
			}
			...
			const hasBroadSector = Boolean(
				stock.broadSector || updateData.broadSector,
			);
			...
			updateData.enrichmentStatus = allKeyFieldsFilled
				? "complete"
				: someKeyFieldsFilled
					? "partial"
					: "pending";
	...
	async populateBroadSectors(): Promise<{ updated: number; errors: number }> {
		...
		console.log(
			`[StockEnrichment] Broad sectors populated: ${updated} updated, ${errors} errors`,
		);
	...
	async getEnrichmentStats(): Promise<{
      ...
      `);
			...
			return {
				...
				withCin: Number.parseInt(row.with_cin) || 0,
				withPan: Number.parseInt(row.with_pan) || 0,
				withBroadSector: Number.parseInt(row.with_broad_sector) || 0,
				withPe: Number.parseInt(row.with_pe) || 0,
				pending: Number.parseInt(row.pending) || 0,
				complete: Number.parseInt(row.complete) || 0,
				failed: Number.parseInt(row.failed) || 0,
			};
		} catch (error) {
