...
const JOB_LOCKS: Map<string, { lockId: string; acquiredAt: Date; expiresAt: Date }> = new Map();
...
const LARGE_CAP_SYMBOLS = [
  ...
  'BHARTIARTL', 'SBIN', 'LICI', 'BAJFINANCE', 'ITC',
  'LT', 'AXISBANK', 'KOTAKBANK', 'TATAMOTORS', 'MARUTI',
  'SUNPHARMA', 'TITAN', 'ASIANPAINT', 'HINDUNILVR', 'WIPRO',
];
...
class FilingSchedulerService {
  ...
  private initializeJobs() {
    this.jobs.set('daily_large_cap', {
      ...
      isEnabled: true,
      priority: 'high',
      description: 'Fetches filings for Nifty 50 and Sensex companies daily at 6 AM',
    });
    ...
    this.jobs.set('weekly_all_companies', {
      ...
      isEnabled: true,
      priority: 'medium',
      description: 'Comprehensive weekly fetch for all listed companies on Sunday 2 AM',
    });
  ...
  private async logSebiAudit(action: string, details: any): Promise<void> {
    ...
    try {
      ...
      console.error(`[FilingScheduler] SEBI audit log failed: ${error.message}`);
    }
  }
  ...
  async runJob(jobId: string): Promise<JobRunResult> {
    ...
    try {
      ...
      console.log(`[FilingScheduler] Job ${job.name} completed: ${result.newFilings} new filings`);
    } catch (error: any) {
      result.errors.push(error.message);
      console.error(`[FilingScheduler] Job ${job.name} failed: ${error.message}`);
    } finally {
  ...
  private async fetchLargeCapFilings() {
    ...
    const persistResult = await exchangeFilingsService.persistFilings(allFilings);
  ...
  private async fetchAllCompanyFilings() {
    ...
    return {
      ...
      newFilings: persistResult.inserted,
      errors: [...fetchResult.nse.errors, ...fetchResult.bse.errors, ...persistResult.errors],
    };
  }
  ...
  private async logJobRun(jobId: string, result: JobRunResult) {
    ...
    try {
      ...
      console.error(`[FilingScheduler] Failed to log job run: ${error.message}`);
    }
  }

  async triggerManualFetch(options: {
    ...
    try {
      ...
      console.log(`[FilingScheduler] Manual fetch completed: ${result.newFilings} new filings`);
    } catch (error: any) {
      result.errors.push(error.message);
