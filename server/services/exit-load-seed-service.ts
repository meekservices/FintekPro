		...
		tiers: [
			{
				...
				maxDays: 365,
				exitLoadPercent: 2.0,
				description: "2% if redeemed within 1 year",
			},
			{
				tier: 2,
				minDays: 366,
				maxDays: 730,
				exitLoadPercent: 1.0,
				description: "1% if redeemed between 1-2 years",
			},
			{
				tier: 3,
				minDays: 731,
		...
		tiers: [
			{
				...
				maxDays: 15,
				exitLoadPercent: 0.5,
				description: "0.5% if redeemed within 15 days",
			},
			{
				tier: 2,
				minDays: 16,
		...
		tiers: [
			{
				...
				maxDays: 1095,
				exitLoadPercent: 0,
				description: "Lock-in period: Cannot redeem for 3 years",
			},
			{
				tier: 2,
				minDays: 1096,
		...
		tiers: [
			{
				...
				minDays: 0,
				maxDays: 1,
				exitLoadPercent: 0.007,
				description: "0.0070% for Day 1",
			},
			{
				tier: 2,
				minDays: 2,
				maxDays: 2,
				exitLoadPercent: 0.0065,
				description: "0.0065% for Day 2",
			},
			{
				tier: 3,
				minDays: 3,
				maxDays: 3,
				exitLoadPercent: 0.006,
				description: "0.0060% for Day 3",
			},
			{
				tier: 4,
				minDays: 4,
				maxDays: 4,
				exitLoadPercent: 0.0055,
				description: "0.0055% for Day 4",
			},
			{
				tier: 5,
				minDays: 5,
				maxDays: 5,
				exitLoadPercent: 0.005,
				description: "0.0050% for Day 5",
			},
			{
				tier: 6,
				minDays: 6,
				maxDays: 6,
				exitLoadPercent: 0.0045,
				description: "0.0045% for Day 6",
			},
			{
				tier: 7,
				minDays: 7,
		...
		tiers: [
			{
				...
				minDays: 0,
				maxDays: 7,
				exitLoadPercent: 0.25,
				description: "0.25% if redeemed within 7 days",
			},
			{
				tier: 2,
				minDays: 8,
		...
		tiers: [
			{
				...
				maxDays: 30,
				exitLoadPercent: 0.25,
				description: "0.25% if redeemed within 30 days",
			},
			{
				tier: 2,
				minDays: 31,
...
class ExitLoadSeedService {
	 ...
	 */
	async seedExitLoadData(): Promise<{
		seeded: number;
		skipped: number;
		errors: number;
	}> {
		...
		for (const fund of POPULAR_FUND_EXIT_LOADS) {
			try {
				...
				console.error(
					`[ExitLoadSeed] Error seeding fund ${fund.schemeCode}:`,
					error,
				);
				errors++;
			}
		...
		console.log(
			`[ExitLoadSeed] Completed: ${seeded} seeded, ${skipped} skipped, ${errors} errors`,
		);
	 ...
	 */
	async updateFundExitLoad(
		schemeCode: string,
		tiers: ExitLoadSeedData["tiers"],
	): Promise<boolean> {
		try {
			// Delete existing tiers
			await db
				.delete(mfSchemeExitLoads)
				...
				const tiers = [
					{
						tier: 1,
						minDays: 0,
						maxDays: update.exitLoadDays,
						exitLoadPercent: update.exitLoadPercent,
						description:
							update.description ||
							`${update.exitLoadPercent}% if redeemed within ${update.exitLoadDays} days`,
					},
					{
						tier: 2,
						minDays: update.exitLoadDays + 1,
						maxDays: null,
						exitLoadPercent: 0,
						description: `Nil after ${update.exitLoadDays} days`,
					},
				];
	 ...
	 */
	async enrichWithIsins(): Promise<{
		enriched: number;
		alreadyHasIsin: number;
		noMatchFound: number;
	}> {
		let enriched = 0;
		...
		try {
			...
			console.log(
				`[ExitLoadSeed] ISIN enrichment complete: ${enriched} enriched, ${alreadyHasIsin} already had ISIN, ${noMatchFound} no match in MF table`,
			);
	 ...
	 */
	async seedCategoryBasedExitLoads(): Promise<{
		seeded: number;
		skipped: number;
		errors: number;
	}> {
		...
		try {
			...
			console.log(
				`[ExitLoadSeed] Found ${fundsWithoutExitLoad.length} funds without exit load data`,
			);

			for (const fund of fundsWithoutExitLoad) {
				try {
					const tiers = this.getCategoryExitLoadTiers(
						fund.category || "",
						fund.schemeName || "",
					);
			...
			console.log(
				`[ExitLoadSeed] Category-based seeding complete: ${seeded} seeded, ${skipped} skipped, ${errors} errors`,
			);
			return { seeded, skipped, errors };
		} catch (error) {
	 ...
	 */
	private getCategoryExitLoadTiers(
		category: string,
		schemeName: string,
	): Array<{
		tier: number;
			...
			return [
				{
					...
					maxDays: 30,
					exitLoadPercent: 0.5,
					description: "0.5% if redeemed within 30 days",
				},
				{
					tier: 2,
					minDays: 31,
		...
		if (
			catLower.includes("corporate bond") ||
			catLower.includes("short duration") ||
			catLower.includes("low duration") ||
			catLower.includes("floating rate")
		) {
			return [
				{
					tier: 1,
					...
					maxDays: 90,
					exitLoadPercent: 0.25,
					description: "0.25% if redeemed within 90 days",
				},
				{
					tier: 2,
					minDays: 91,
		...
		if (
			catLower.includes("medium duration") ||
			catLower.includes("long duration") ||
			catLower.includes("gilt") ||
			catLower.includes("dynamic bond")
		) {
			return [
				{
					tier: 1,
					...
					maxDays: 365,
					exitLoadPercent: 0.5,
					description: "0.5% if redeemed within 1 year",
				},
				{
					tier: 2,
					minDays: 366,
			...
			return [
				{
					...
					maxDays: null,
					exitLoadPercent: 0,
					description: "Nil exit load (3-year lock-in applies)",
				},
			];
		}
			...
			return [
				{
					...
					maxDays: null,
					exitLoadPercent: 0,
					description: "Nil exit load (lock-in applies)",
				},
			];
		}
		...
		if (
			catLower.includes("aggressive hybrid") ||
			catLower.includes("balanced advantage") ||
			catLower.includes("multi asset") ||
			catLower.includes("dynamic asset")
		) {
			return [
				{
					tier: 1,
		...
		if (
			catLower.includes("conservative hybrid") ||
			catLower.includes("equity savings")
		) {
			return [
				{
					tier: 1,
		...
		if (
			catLower.includes("sectoral") ||
			catLower.includes("thematic") ||
			catLower.includes("pharma") ||
			catLower.includes("banking") ||
		...
		if (
			...
			catLower.includes("fof") ||
			catLower.includes("fund of fund") ||
			nameLower.includes("global") ||
			nameLower.includes("us ") ||
			nameLower.includes("nasdaq")
		) {
			return [
				{
					tier: 1,
		...
		if (
			...
			catLower.includes("large cap") ||
			catLower.includes("mid cap") ||
			catLower.includes("small cap") ||
			catLower.includes("flexi cap") ||
		...
		if (
			...
			return [
				...
				},
				{
					...
					maxDays: 365,
					exitLoadPercent: 0.1,
					description: "0.1% if redeemed within 1 year",
				},
				{
					tier: 3,
					minDays: 366,
	...
	async runFullEnrichment(): Promise<{
		...
		console.log(
			`  Final coverage: ${finalStats.withIsin}/${finalStats.seededFunds} funds with ISIN`,
		);
