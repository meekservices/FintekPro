...
export class URCAEPlanner {
	 ...
	 */
	public async generateTargetAllocation(
		context: URCAEInputContext,
	): Promise<URCAEOutputContext> {
		const allocationId = uuidv4();
		const selectedModel = context.optimization_model || "mean_variance";

		try {
			// 24.1 Systemic Resilience: Intercept for extreme volatility events
			if (marketRegimeDetector.detectBlackSwanEvent()) {
				const safetyWeights: AllocationOutput[] = [
				...
				return {
					...
					system_trace: {
						model_used: "BLACK_SWAN_AUTO_FALLBACK",
						active_constraints: ["10_SIGMA_VOLATILITY_INTERCEPT"],
					},
				};
			}
			...
			const regimeShift = marketRegimeDetector.getMarketRegimeConstraints(
				context.market_state.macro_regime,
			);
			availableAssets = availableAssets.map((a) => {
				if (a.id.includes("equity"))
					a.volatility = a.volatility * regimeShift.targetEquityShift;
				if (a.id.includes("bonds"))
					a.volatility = a.volatility * regimeShift.targetDebtShift;
				return a;
			});

			// 2. Execute Quantitative Math Array
			let rawAllocation: AllocationOutput[] = [];
			const targetReturnRequest =
				context.user_profile.risk_profile === "high"
					? 0.15
					: context.user_profile.risk_profile === "medium"
						? 0.1
						: 0.07;
			...
			if (selectedModel === "risk_parity") {
				rawAllocation =
					optimizerEngine.generateRiskParityAllocation(availableAssets);
			} else if (selectedModel === "black_litterman") {
				rawAllocation = optimizerEngine.attemptBlackLittermanOptimization(
					availableAssets,
					targetReturnRequest,
				);
				fallbackTriggered = true; // Flag for audit DB
			} else {
				rawAllocation = optimizerEngine.generateMeanVarianceAllocation(
					availableAssets,
					targetReturnRequest,
				);
			}
			...
			const globalMaxClass =
				context.user_profile.risk_profile === "high" ? 0.4 : 0.2; // Generic ceiling for massive capital sinks
			const constraints = {
				max_single_asset: globalMaxClass,
				min_cash: context.user_profile.liquidity_needs === "high" ? 0.15 : 0.05,
			};

			const enforcedAllocation = constraintEngine.applyConstraints(
				rawAllocation,
				constraints,
			);
			...
			const aageCheck = await aiGovernanceEngine.validateAndResolve({
				...
				user_profile: {
					...
					investment_horizon: context.user_profile.investment_horizon,
					kyc_status: "verified",
					user_segment: "retail",
				},
			...
			if (aageCheck.decision === "BLOCK") {
				throw new Error(
					`AAGE Compliance Intercept: ${aageCheck.risk_flags.join(", ")}`,
				);
			}
			...
			const riskBudgetMap: { [key: string]: number } = {};

			enforcedAllocation.forEach((alloc) => {
				const assetTrace = availableAssets.find(
					(a) => a.id === alloc.asset_class,
				);
				if (assetTrace) {
					expectedReturnFinal += alloc.weight * assetTrace.expectedReturn;
					approxVolatilityFinal += alloc.weight * assetTrace.volatility;
					riskBudgetMap[alloc.asset_class] =
						alloc.weight * assetTrace.volatility; // Simplified Risk Contribution (RCi) = wi * σi
				}
			});
			...
			const outputPayload: URCAEOutputContext = {
				...
				strategy_overlay: [
					context.user_profile.risk_profile === "low" ? "income" : "growth",
				],
				risk_budget: riskBudgetMap,
				expected_metrics: {
					return: expectedReturnFinal,
					volatility: approxVolatilityFinal,
				},
				system_trace: {
					model_used: selectedModel,
					active_constraints: [
						`max_asset_${globalMaxClass}`,
						`min_cash_${constraints.min_cash}`,
					],
				},
			};

			// 6. DB Verification Insert
			db.insert(urcaeAllocationLogs)
				.values({
					id: allocationId,
				...
				.execute()
				.catch((e) => console.error("URCAE DB Audit Fault:", e));

			return outputPayload;
		} catch (e: any) {
			console.error("[URCAE FATAL FAULT]", e);
			// Failsafe Layer: Default to generalized defensive rule-based states
			const defensiveWeights: AllocationOutput[] = [
				{ asset_class: "cash", weight: 1.0 },
			]; // Entire portfolio shifted to safety buffer
			return {
				allocation_id: allocationId,
				target_allocation: defensiveWeights,
				strategy_overlay: ["capital_preservation"],
				risk_budget: { cash: 0.01 },
				expected_metrics: { return: 0.03, volatility: 0.01 },
				system_trace: {
					model_used: "SEBI_FAILSAFE_DEFAULT",
					active_constraints: ["ALL_CASH_FALLBACK"],
				},
			};
		}
	}
