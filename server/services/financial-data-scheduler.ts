...
const DEFAULT_GLOBAL_STOCKS = [
  ...
  'UNH', 'MA', 'JNJ', 'XOM', 'HD', 'PG', 'CVX', 'MRK', 'ABBV', 'PFE',
];

const DEFAULT_ETFS = [
  'SPY', 'QQQ', 'IWM', 'VTI', 'VOO', 'IVV', 'VEA', 'VWO', 'EFA', 'AGG',
  'BND', 'LQD', 'TLT', 'GLD', 'SLV', 'XLF', 'XLK', 'XLE', 'XLV', 'XLI',
];

const DEFAULT_MUTUAL_FUNDS = [
  '119551', '120503', '120505', '119597', '120847',
  '118551', '118632', '118633', '118634', '118636',
  '102885', '100356', '106235', '105758', '100474',
];

const DEFAULT_DEBT_INSTRUMENTS = [
  { symbol: 'GSEC-2030', type: 'govt_security', name: 'Government Securities 2030', yieldPercent: 7.25, couponRate: 7.26 },
  { symbol: 'GSEC-2033', type: 'govt_security', name: 'Government Securities 2033', yieldPercent: 7.32, couponRate: 7.18 },
  { symbol: 'SGB-2024', type: 'bond', name: 'Sovereign Gold Bond 2024', yieldPercent: 2.50 },
  { symbol: 'REC-NCD-24', type: 'ncd', name: 'REC Limited NCD 2024', yieldPercent: 7.95, couponRate: 7.95 },
  { symbol: 'PFC-NCD-24', type: 'ncd', name: 'PFC Limited NCD 2024', yieldPercent: 7.85, couponRate: 7.85 },
];
...
class FinancialDataScheduler {
  ...
  async start(): Promise<void> {
    ...
    console.log('🚀 [FinancialDataScheduler] Starting periodic data refresh...');
    this.isRunning = true;
  ...
  private async runInitialRefresh(): Promise<void> {
    ...
    try {
      ...
      console.error('❌ [FinancialDataScheduler] Initial refresh failed:', error);
    }
  }
  ...
  private async refreshGlobalStocks(): Promise<void> {
    ...
    try {
      ...
      console.error('❌ [FinancialDataScheduler] Global stocks refresh error:', error);
    }
  }
  ...
  private async refreshETFs(): Promise<void> {
    ...
    try {
      ...
      console.log(`✅ ETFs: ${result.success} updated, ${result.failed} failed`);
    } catch (error) {
  ...
  private async refreshMutualFunds(): Promise<void> {
    ...
    try {
      const result = await financialDataRepository.refreshMutualFunds(DEFAULT_MUTUAL_FUNDS);
      this.lastRefreshTimes['mutualFunds'] = new Date();
      console.log(`✅ Mutual funds: ${result.success} updated, ${result.failed} failed`);
    } catch (error) {
      console.error('❌ [FinancialDataScheduler] Mutual funds refresh error:', error);
    }
  }
  ...
  private async refreshDebtInstruments(): Promise<void> {
    ...
    try {
      const result = await financialDataRepository.refreshDebtInstruments(DEFAULT_DEBT_INSTRUMENTS);
      this.lastRefreshTimes['debt'] = new Date();
      console.log(`✅ Debt instruments: ${result.success} updated, ${result.failed} failed`);
    } catch (error) {
  ...
  async forceRefresh(type?: 'globalStocks' | 'etfs' | 'mutualFunds' | 'debt' | 'all'): Promise<void> {
    if (!type || type === 'all') {
      await Promise.all([
        this.refreshGlobalStocks(),
        this.refreshETFs(),
