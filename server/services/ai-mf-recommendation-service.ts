...
class AIMFRecommendationService {
	...
	async getSmartRecommendations(
		...
		try {
			...
			const diversifiedFunds = this.applyAMCDiversification(
				scoredFunds,
				maxFundsPerAMC,
				minAMCs,
			);

			const recommendations = diversifiedFunds.map((fund) =>
				this.buildRecommendation(fund),
			);
	...
	private sanitizeReturns(returns: number, category: string): number {
		...
		if (
			lowerCategory.includes("overnight") ||
			lowerCategory.includes("liquid") ||
			lowerCategory.includes("money market") ||
			lowerCategory.includes("ultra short")
		) {
			maxReturn = 15;
		} else if (
		...
		if (Math.abs(returns) > maxReturn) {
			console.warn(
				`[AI-MF] Capping unrealistic return ${returns}% to 0 for ${category} fund`,
			);
			return 0;
		}
		return returns;
	...
	private scoreFund(fund: any): any & { totalScore: number; metrics: any } {
		...
		const rawReturns1y = Number.parseFloat(fund.returns1y || "0");
		const rawReturns3y = Number.parseFloat(fund.returns3y || "0");
		const rawReturns5y = Number.parseFloat(fund.returns5y || "0");
		...
		const expenseRatio = Number.parseFloat(fund.expenseRatio || "1.5");
		const aum = Number.parseFloat(fund.aum || "0");
		const smartRating = fund.crisilRating || 3;
		const smartPercentile = Number.parseFloat(fund.crisilPercentile || "50");
		const riskAdjustedScore = Number.parseFloat(
			fund.crisilRiskAdjustedScore || "0",
		);
		...
		const returnsScore = this.calculateReturnsScore(
			returns1y,
			returns3y,
			returns5y,
			category,
		);
		const sharpeEstimate = this.calculateSharpeEstimate(
			returns1y,
			returns3y,
			smartPercentile,
			category,
		...
		const exitLoadScore =
			exitLoad.percent === 0 ? 100 : exitLoad.percent < 1 ? 80 : 60;
		...
		const advancedMetrics = this.calculateAdvancedMFMetrics(
			fund,
			returns1y,
			returns3y,
		);
		const advancedMetricsBonus = this.getAdvancedMetricsBonus(
			advancedMetrics,
			category,
		);
		...
		const totalScore = Math.min(
			100,
			baseScore + categoryBonus + advancedMetricsBonus,
		);
	...
	private calculateAdvancedMFMetrics(
		fund: any,
		returns1y: number,
		returns3y: number,
	): {
		...
		try {
			...
			const portfolioPE = Number.parseFloat(extendedData.portfolioPE || 0);
			const portfolioROE = Number.parseFloat(extendedData.portfolioROE || 0);
			...
			const volatility = Number.parseFloat(
				extendedData.volatility || fund.volatility || 15,
			);
			...
			const downsideCaptureRatio = Number.parseFloat(
				extendedData.downsideCaptureRatio || 100,
			);
			...
			if (extendedData.navHistory && Array.isArray(extendedData.navHistory)) {
				const navValues = extendedData.navHistory.map((n: any) =>
					Number.parseFloat(n.nav || n),
				);
				if (navValues.length > 10) {
					metrics.maxDrawdown =
						financialMetricsCalculator.calculateMaxDrawdown(navValues) * 100;
				}
			}

			// Risk-adjusted alpha estimation
			const benchmarkReturns = this.getCategoryBenchmarks(
				fund.category || "",
			).expected1Y;
			if (returns1y && benchmarkReturns) {
				const beta = Number.parseFloat(extendedData.beta || 1);
	...
	private calculateReturnsScore(
		...
		const r1yScore =
			r1y >= benchmarks.expected1Y
				? Math.min(30, 20 + (r1y - benchmarks.expected1Y))
				: Math.max(0, 20 - (benchmarks.expected1Y - r1y) * 2);
		const r3yScore =
			r3y >= benchmarks.expected3Y
				? Math.min(25, 15 + (r3y - benchmarks.expected3Y))
				: Math.max(0, 15 - (benchmarks.expected3Y - r3y) * 2);
		const r5yScore =
			r5y >= benchmarks.expected5Y
				? Math.min(20, 10 + (r5y - benchmarks.expected5Y))
				: Math.max(0, 10 - (benchmarks.expected5Y - r5y) * 2);
	...
	private getCategoryBenchmarks(categoryOrName: string): {
		expected1Y: number;
		expected3Y: number;
		expected5Y: number;
		volatility: number;
		...
		if (
			...
			cat.includes("precious") ||
			(cat.includes("fof") && (cat.includes("gold") || cat.includes("silver")))
		) {
	...
	private detectFundCategoryEnhanced(fund: any): string {
		...
		if (
			schemeName.includes("gold") ||
			schemeName.includes("silver") ||
			schemeName.includes("commodity") ||
			schemeName.includes("precious")
		) {
	...
	private async isFundInvestable(
		fund: any,
	): Promise<{ investable: boolean; reason: string | null }> {
	...
	static getOverseasInvestmentStatus(): {
		investmentFrozen: boolean;
		etfFrozen: boolean;
	} {
	...
	private calculateSharpeEstimate(
		r1y: number,
		r3y: number,
		percentile: number,
		category: string,
		...
		const estimatedVolatility =
			benchmarks.volatility * (1 + (100 - percentile) / 100);
	...
	private calculateConsistencyScore(
		...
		const stability =
			Math.abs(r1y - r3y) < 5 && Math.abs(r3y - r5y) < 5
				? 30
				: Math.abs(r1y - r3y) < 10
					? 20
					: 10;
		const positivity =
			(r1y > 0 ? 15 : 0) + (r3y > 0 ? 15 : 0) + (r5y > 0 ? 10 : 0);

		return Math.min(100, trajectory + stability + positivity);
	}

	// Expense ratio scoring with category awareness
	private calculateExpenseScore(
		expenseRatio: number,
		category: string,
	): number {
		const categoryAvgExpense =
			category.includes("debt") || category.includes("liquid")
				? 0.5
				: category.includes("index") || category.includes("etf")
					? 0.3
					: category.includes("small") || category.includes("mid")
						? 2.0
						: 1.5;
	...
	private parseExitLoad(exitLoadStr: string | undefined): {
		percent: number;
		days: number;
	} {
		...
		let percent = percentMatch ? Number.parseFloat(percentMatch[1]) : 1;
		let days = 365;
	...
	private buildRecommendation(fund: any): MFRecommendation {
		...
		const confidence = this.calculateCalibratedConfidence(
			fund.totalScore,
			metrics,
			signal,
		);
	...
	private determineSignal(metrics: any): "buy" | "hold" | "exit" {
		...
		if (
			returns1y < benchmarks.expected1Y * -0.5 &&
			returns3y < benchmarks.expected3Y * 0.3
		)
			return "exit";
		if (smartRating >= 5 && smartPercentile < 15 && returns1y < 0)
			return "exit";

		// Strong buy signals - outperformance with quality
		if (
			fintekproRating >= 4 &&
			returns1y >= benchmarks.expected1Y &&
			purchaseAllowed
		)
			return "buy";
		if (
			fintekproRating >= 3 &&
			returns3y >= benchmarks.expected3Y * 1.2 &&
			consistencyScore >= 60
		)
			return "buy";
		if (
			smartRating <= 2 &&
			smartPercentile >= 75 &&
			returns1y >= benchmarks.expected1Y * 0.8
		)
			return "buy";
	...
	private calculateCalibratedConfidence(
		...
		if (!metrics.smartPercentile || metrics.smartPercentile === 0)
			baseConfidence -= 5;
	...
	private generateRichRationale(
		...
		if (metrics.smartPercentile >= 70) {
			parts.push(
				`Top ${100 - Math.round(metrics.smartPercentile)}% in ${fund.category || "category"}`,
			);
		} else if (metrics.smartPercentile >= 50) {
			parts.push(`Above average in ${fund.category || "category"}`);
		} else {
			parts.push(
				`Bottom ${Math.round(metrics.smartPercentile)}% in ${fund.category || "category"}`,
			);
		}
		...
		if (metrics.returns1y > 15) {
			...
			parts.push(
				`Negative 1Y returns of ${metrics.returns1y.toFixed(1)}% - review needed`,
			);
		}

		if (metrics.returns3y > 12) {
			parts.push(
				`3Y CAGR of ${metrics.returns3y.toFixed(1)}% shows consistency`,
			);
		}

		if (metrics.sharpeEstimate > 1) {
			parts.push(
				`Strong risk-adjusted returns (Sharpe ~${metrics.sharpeEstimate.toFixed(2)})`,
			);
		}
		...
		if (metrics.expenseRatio <= 1) {
			...
			parts.push(
				`High expense ratio of ${metrics.expenseRatio.toFixed(2)}% impacts returns`,
			);
		} else {
		...
		if (metrics.exitLoadPercent === 0) {
			...
			parts.push(
				`Exit Load: ${metrics.exitLoadPercent}% within ${metrics.exitLoadDays} days`,
			);
		}
	...
	async getRecommendationsWithLiveNAV(
		filters: RecommendationFilters = {},
	): Promise<MFRecommendation[]> {
	 ...
	 */
	async analyzePortfolioHoldings(
		holdings: {
			schemeCode?: string;
			schemeName: string;
			currentValue: number;
		...
		try {
			...
			for (const holding of holdings) {
				...
				if (fundData) {
					...
					const signal = this.determineSignalFromScore(
						scored.totalScore,
						scored.cagr1Y,
						scored.categoryRank,
					);
					...
					if (signal === "exit" || signal === "switch") {
						...
						if (
							...
							alternatives[0].schemeCode !== fundData.schemeCode
						) {
					...
					holdingsAnalysis.push({
						...
						confidence: 30,
						rationale:
							"Fund data not available in our database. Manual review recommended.",
						metrics: {},
					});
				}
			}
	...
	private determineSignalFromScore(
		totalScore: number,
		cagr1Y: number,
		categoryRank: number,
	): string {
	...
	private generateHoldingRationale(
		scored: any,
		fundData: any,
		signal: string,
	): string {
		...
		const stars =
			"★".repeat(scored.fintekproRating || 3) +
			"☆".repeat(5 - (scored.fintekproRating || 3));
		...
		if (scored.cagr1Y !== undefined) {
			if (scored.cagr1Y > 15) {
				...
				parts.push(
					`Negative 1Y returns of ${scored.cagr1Y.toFixed(1)}% - review needed.`,
				);
			}
		}
	...
	private generatePortfolioSummary(
		...
		if (exitCount > 0) {
			parts.push(
				`${exitCount} fund${exitCount > 1 ? "s" : ""} should be reviewed for potential switching.`,
			);
		}

		const strongHoldings = holdings.filter(
			(h) => h.signal === "buy_more" || h.signal === "hold",
		).length;
		if (strongHoldings > 0) {
			parts.push(
				`${strongHoldings} holding${strongHoldings > 1 ? "s are" : " is"} performing well.`,
			);
		}

		if (!hasCommodity) {
			parts.push(
				"Consider adding 5-10% allocation to Gold/Silver FOF for downside protection.",
			);
		}
	...
	async getProposalRecommendations(params: {
		...
		const [equityFunds, debtFunds, hybridFunds, commodityFunds] =
			await Promise.all([
