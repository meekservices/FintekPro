...
class MarketHolidayService {
	...
	private initializeHolidays() {
		const allHolidays: MarketHoliday[] = [
			...
			{
				date: "2026-05-01",
				name: "Maharashtra Day / Buddha Purnima",
				exchanges: ["NSE", "BSE"],
				type: "full",
			},
			{
	...
	private initializeBankHolidays() {
		...
		console.log(
			"🏦 Bank Holiday Service initialized with",
			allBankHolidays.length,
			"holidays",
		);
	}
	...
	private initializeInternationalHolidays() {
		...
		console.log(
			"🌍 International Holiday Service initialized with",
			allInternationalHolidays.length,
			"holidays",
		);
	}
	...
	getHoliday(
		...
		return (
			yearHolidays.find(
				(h) => h.date === dateStr && h.exchanges.includes(exchange),
			) || null
		);
	}
	...
	getMarketStatus(
		...
		return {
			...
			nextTradingDayName: isWithinTradingHours
				? dayNames[getDay(istTime)]
				: nextTradingDayName,
			tradingHours,
		};
	}

	private getTradingHours(exchange: "NSE" | "BSE" | "MCX" | "NCDEX"): {
		open: string;
		close: string;
	} {
		switch (exchange) {
	...
	getInternationalHolidaysForYear(
		year: number,
		region?: Region,
	): MarketHoliday[] {
	...
	getUpcomingInternationalHolidays(
		count: number = 10,
		region?: Region,
	): MarketHoliday[] {
		const now = new Date();
