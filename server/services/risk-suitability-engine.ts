...
class RiskSuitabilityEngine {
	...
	private readonly productRiskMatrix: Record<
		string,
		{
			minScore: number;
			maxScore: number;
	...
	private getRiskCategory(
		...
		if (score <= this.riskCategoryThresholds.conservative.max)
			return "conservative";
		if (score <= this.riskCategoryThresholds.moderate.max) return "moderate";
		if (score <= this.riskCategoryThresholds.aggressive.max)
			return "aggressive";
	...
	private getInvestmentHorizonFromAnswers(
		answers: RiskQuestionAnswer[],
	): string {
	...
	private getIncomeStabilityFromAnswers(answers: RiskQuestionAnswer[]): string {
		const stabilityAnswer = answers.find(
			(a) =>
				a.questionId.includes("stability") || a.questionId.includes("income"),
		);
	...
	private getLiquidityNeedsFromAnswers(answers: RiskQuestionAnswer[]): string {
		const liquidityAnswer = answers.find((a) =>
			a.questionId.includes("liquidity"),
		);
	...
	async getProductEligibility(userId: string): Promise<ProductEligibility[]> {
		...
		for (const [productType, config] of Object.entries(
			this.productRiskMatrix,
		)) {
			const riskEligible =
				riskScore >= config.minScore && riskScore <= config.maxScore;
			const segmentEligible =
				!config.segments || config.segments.includes(clientSegmentType);
	...
	async checkProductSuitability(
		...
		const riskProfileMatch =
			riskScore >= productConfig.minScore &&
			riskScore <= productConfig.maxScore;
		if (!riskProfileMatch) {
			if (riskScore < productConfig.minScore) {
				warnings.push(
					`This product requires a minimum risk score of ${productConfig.minScore}. Your score: ${riskScore}`,
				);
				recommendations.push(
					"Consider less risky alternatives or update your risk profile",
				);
			} else {
				recommendations.push(
					"This product may be too conservative for your risk profile",
				);
			}
		}
		...
		const clientSegmentType = segment[0]?.segment || "retail";
		const clientSegmentMatch =
			!productConfig.segments ||
			productConfig.segments.includes(clientSegmentType);

		if (!clientSegmentMatch) {
			warnings.push(
				`This product requires ${productConfig.segments?.join(" or ")} client segment`,
			);
			recommendations.push(
				"Increase your investable surplus to qualify for higher segments",
			);
		}
		...
		const monthlyInvestable = Number.parseFloat(
			surplus[0]?.monthlyInvestableSurplus || "0",
		);
		...
		if (!investmentCapacityMatch) {
			if (investmentAmount > annualInvestable) {
				warnings.push(
					"Investment amount exceeds your annual investable surplus",
				);
				recommendations.push(
					"Consider a smaller investment or wait until you have more surplus",
				);
			} else if (investmentAmount > annualInvestable * 0.5) {
				warnings.push(
					"Investment exceeds 50% of annual surplus - high concentration risk",
				);
		...
		const suitable =
			riskProfileMatch && clientSegmentMatch && suitabilityScore >= 60;
	...
	async getFullSuitabilityReport(userId: string): Promise<{
		...
		if (!riskProfile) {
			recommendations.push(
				"Complete risk assessment to unlock investment recommendations",
			);
		} else {
			const eligibleProducts = productEligibility.filter((p) => p.eligible);
			const eligibleHighRisk = eligibleProducts.filter(
				(p) => p.riskLevel === "high" || p.riskLevel === "very_high",
			);

			if (eligibleHighRisk.length === 0 && riskProfile.riskScore > 50) {
				recommendations.push(
					"Consider increasing your surplus to access higher-return products",
				);
			}

			if (riskProfile.riskScore < 30) {
				recommendations.push(
					"Focus on debt instruments and fixed deposits for capital preservation",
				);
			} else if (riskProfile.riskScore < 50) {
				recommendations.push(
					"Consider balanced funds for moderate growth with stability",
				);
			} else if (riskProfile.riskScore < 70) {
				recommendations.push(
					"Equity mutual funds and direct stocks align with your risk profile",
				);
			} else {
				recommendations.push(
					"Your profile supports alternative investments and derivatives",
				);
			}
		}
	...
	async getDefaultRiskQuestions(): Promise<any[]> {
		return [
			...
			},
			{
				...
				question:
					"How would you react if your investment dropped 20% in value?",
				options: [
			...
			},
			{
				...
				options: [
					...
					{
						id: "e5",
						text: "Expert - derivatives and alternatives",
						score: 95,
					},
				],
			},
