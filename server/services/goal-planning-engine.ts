...
export class GoalPlanningEngine {
	calculateInflationAdjustedAmount(
		...
		return currentAmount * (1 + inflationRate / 100) ** years;
	}
	...
	calculateSIPForGoal(
		...
		const remainingTarget = Math.max(
			0,
			inflationAdjustedTarget - futureValueOfCurrentSavings,
		);
		...
		if (remainingTarget > 0 && monthsRemaining > 0) {
			if (monthlyRate === 0) {
				...
				requiredSipAmount =
					...
					((1 + monthlyRate) ** monthsRemaining - 1);
			}
		}

		const totalInvestment =
			currentSavings + requiredSipAmount * monthsRemaining;
		const sipFV =
			monthlyRate === 0
				? requiredSipAmount * monthsRemaining
				: (requiredSipAmount * ((1 + monthlyRate) ** monthsRemaining - 1)) /
					monthlyRate;
	...
	calculateLumpsumForGoal(
		...
		const requiredLumpsum =
			...
			(1 + expectedReturnRate / 100) ** yearsRemaining;
	...
	suggestAssetAllocation(
		...
		if (yearsRemaining <= 1) {
			...
			cash = 50;
			reasoning =
				"Very short-term goal: Focus on capital preservation with liquid instruments.";
			...
			reasoning =
				"Short-term goal (1-3 years): Conservative allocation with limited equity exposure.";
			...
			reasoning =
				"Medium-term goal (3-5 years): Balanced allocation with moderate equity.";
			...
			reasoning =
				"Long-term goal (5-10 years): Growth-focused with significant equity allocation.";
		} else {
			if (riskProfile === "conservative") {
				equity = 55;
				debt = 35;
			...
			reasoning =
				"Very long-term goal (10+ years): Maximum growth with high equity for wealth creation.";
		}
	...
	calculateGoalProjection(
		...
		const futureValueOfCurrent =
			currentAmount * (1 + monthlyRate) ** monthsRemaining;

		let sipFutureValue = 0;
		if (monthlyRate > 0 && monthsRemaining > 0) {
			sipFutureValue =
				(monthlyContribution * ((1 + monthlyRate) ** monthsRemaining - 1)) /
				monthlyRate;
		} else {
		...
		const projectedPercentage =
			(projectedValue / inflationAdjustedTarget) * 100;
	...
	generateDefaultMilestones(
		...
		percentages.forEach((pct, index) => {
			const milestoneDate = new Date(
				startDate.getTime() + (totalDuration * pct) / 100,
			);
			milestones.push({
				goalId,
	...
	async createGoal(data: InsertFinancialGoal): Promise<FinancialGoal> {
		...
		const targetAmount = Number.parseFloat(
			data.targetAmount?.toString() || "0",
		);
		const currentAmount = Number.parseFloat(
			data.currentAmount?.toString() || "0",
		);
		const inflationRate = Number.parseFloat(
			data.inflationRate?.toString() ||
				categoryDefaults.defaultInflation.toString(),
		);
		const expectedReturnRate = Number.parseFloat(
			data.expectedReturnRate?.toString() ||
				categoryDefaults.defaultReturn.toString(),
		);
		...
		const riskProfile =
			(data.riskProfile as "conservative" | "moderate" | "aggressive") ||
			"moderate";
		...
		const projection = this.calculateGoalProjection(
			...
			Number.parseFloat(
				data.monthlyContribution?.toString() ||
					sipCalc.requiredSipAmount.toString(),
			),
			monthsRemaining,
			expectedReturnRate,
			inflationRate,
	...
	async getGoalWithDetails(goalId: string): Promise<{
		...
		const investments = await db
			.select()
			.from(goalInvestmentLinks)
			.where(
				and(
					eq(goalInvestmentLinks.goalId, goalId),
					eq(goalInvestmentLinks.isActive, true),
				),
			);
		...
		const projection = this.calculateGoalProjection(
			Number.parseFloat(goal.currentAmount?.toString() || "0"),
			Number.parseFloat(goal.targetAmount?.toString() || "0"),
			Number.parseFloat(goal.monthlyContribution?.toString() || "0"),
			monthsRemaining,
			Number.parseFloat(goal.expectedReturnRate?.toString() || "12"),
			Number.parseFloat(goal.inflationRate?.toString() || "6"),
		);
	...
	async updateGoalProgress(goalId: string): Promise<FinancialGoal> {
		...
		let totalCurrentValue = Number.parseFloat(
			goal.currentAmount?.toString() || "0",
		);
		for (const inv of investments) {
			totalCurrentValue += Number.parseFloat(
				inv.currentValue?.toString() || "0",
			);
		}
	...
	async checkAndUpdateMilestones(
		...
		for (const milestone of milestones) {
			const targetPct = Number.parseFloat(
				milestone.targetPercentage?.toString() || "0",
			);
	...
	async linkInvestmentToGoal(
		data: InsertGoalInvestmentLink,
	): Promise<GoalInvestmentLink> {
		const [link] = await db
			.insert(goalInvestmentLinks)
			.values(data as any)
			.returning();
	...
	async takeProgressSnapshot(goalId: string): Promise<void> {
		...
		const investmentsValue = investments.map((inv) => ({
			investmentId: inv.id,
			value: Number.parseFloat(inv.currentValue?.toString() || "0"),
		}));
	...
	calculateSipWithStepUp(
		...
		const remainingTarget = Math.max(
			0,
			inflationAdjustedTarget - futureValueOfCurrent,
		);

		let low = 0,
			high = (remainingTarget / monthsRemaining) * 2;
