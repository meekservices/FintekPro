...
class FinnhubService {
  ...
  constructor() {
    ...
    if (!this.isConfigured) {
      console.warn('⚠️ FINNHUB_API_KEY not configured. Finnhub service will not be available.');
    } else {
  ...
  getStatus(): { configured: boolean; baseUrl: string; rateLimitRemaining: number } {
    return {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        if (error.response?.status === 429) {
          ...
          console.log(`[Finnhub] Rate limited, retrying in ${delay}ms (attempt ${attempt + 1}/${retries + 1})`);
          await new Promise(resolve => setTimeout(resolve, delay));
        } else if (error.response?.status >= 500) {
          const delay = BASE_DELAY_MS * Math.pow(2, attempt);
          console.log(`[Finnhub] Server error, retrying in ${delay}ms (attempt ${attempt + 1}/${retries + 1})`);
          await new Promise(resolve => setTimeout(resolve, delay));
        } else {
          throw error;
        }
  ...
  async searchSymbol(query: string): Promise<FinnhubFetchResult<FinnhubSearchResult>> {
    if (!this.isConfigured) {
      return {
        success: false,
        source: 'finnhub',
  ...
  async getCompanyProfile(symbol: string): Promise<FinnhubFetchResult<FinnhubCompanyProfile>> {
    if (!this.isConfigured) {
      return {
        success: false,
        source: 'finnhub',
  ...
  async getFinancialStatements(
    ...
    try {
      ...
      console.error(`[Finnhub] Statements error for ${symbol}: ${error.message}`);
      return {
        success: false,
        source: 'finnhub',
        retrievedAt: new Date(),
