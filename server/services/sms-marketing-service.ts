...
class SMSMarketingService {
	...
	private async initialize(): Promise<void> {
		try {
			...
			console.log(
				"✅ SMS Marketing service initialized via shared Twilio client",
			);
		} catch (error) {
			this.isConfigured = false;
			console.log(
				"⚠️ SMS Marketing service not configured - Twilio credentials missing",
			);
		}
	}
	...
	async sendMarketingSMS(
	...
	): Promise<SMSMarketingResult> {
		await this.ensureInitialized();

		if (!this.isConfigured) {
			console.log(
				`📱 Marketing SMS to ${to.substring(0, 6)}****: ${message.substring(0, 50)}... (not configured)`,
			);
		...
		if (!skipConsentCheck) {
			...
			if (!hasConsent) {
				console.log(
					`🚫 Marketing SMS blocked - no consent: ${to.substring(0, 6)}**** (TRAI compliance)`,
				);
				return {
					success: false,
					error:
						"User has not consented to marketing communications (TRAI compliance)",
				};
			}
		}
		...
		try {
			...
			console.log(
				`📱 Sending marketing SMS to ${formattedTo.substring(0, 8)}***`,
			);

			const result = await this.client.messages.create(messageOptions);

			console.log(
				`✅ Marketing SMS sent - SID: ${result.sid}, Status: ${result.status}`,
			);
	...
	async sendBulkSMS(
		...
		for (const recipient of recipients) {
			...
			const result = await this.sendMarketingSMS(
				recipient.mobile,
				personalizedMessage,
				campaignId,
			);
		...
		console.log(
			`📊 Bulk SMS complete: ${results.sent}/${results.totalRecipients} sent, ${results.failed} failed`,
		);
	...
	async sendCampaignSMS(config: SMSCampaignConfig): Promise<BulkSMSResult> {
		...
		const query = db
			...
			.from(users)
			.where(
				and(eq(users.marketingConsent, true), sql`${users.mobile} IS NOT NULL`),
			);
		...
		console.log(
			`📊 Campaign ${campaignId}: Found ${recipients.length} eligible recipients`,
		);
	...
	async getStatus(): Promise<{
		...
		return {
			...
			messagingServiceSid: this.messagingServiceSid
				? `${this.messagingServiceSid.substring(0, 10)}...`
				: "Not configured",
