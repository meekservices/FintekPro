...
import {
	sandboxCapitalGainsService,
	type AssetType as SandboxAssetType,
} from "./sandbox-capital-gains-service";
...
const SANDBOX_TAX_API_AVAILABLE =
	sandboxCapitalGainsService.isServiceAvailable();

const TAX_REGIMES = {
	PRE_BUDGET_2024: Object.fromEntries(
		Object.entries(PRE_BUDGET_2024_RATES).map(([k, v]) => [
			k,
			{
				stcg: {
					rate: v.stcg,
					thresholdDays: v.ltcgThresholdDays,
					slabBased: isSlabBasedFromConfig(
						k as TaxAssetClass,
						new Date("2024-01-01"),
					),
				},
	...
	POST_BUDGET_2024: Object.fromEntries(
		Object.entries(POST_BUDGET_2024_RATES).map(([k, v]) => [
			k,
			{
				stcg: {
					rate: v.stcg,
					thresholdDays: v.ltcgThresholdDays,
					slabBased: isSlabBasedFromConfig(k as TaxAssetClass),
				},
				ltcg: {
					rate: v.ltcg,
					thresholdDays: v.ltcgThresholdDays,
...
class ProposalCapitalGainsService {
	...
	getAssetCategory(
		productType: string,
		category?: string,
	): keyof typeof TAX_REGIMES.POST_BUDGET_2024 {
		...
		if (lowerType.includes("hybrid") || lowerCategory.includes("hybrid")) {
			...
			if (
				lowerCategory.includes("aggressive") ||
				lowerCategory.includes("balanced advantage") ||
				lowerCategory.includes("equity savings") ||
				lowerCategory.includes("multi asset")
			) {
	...
	calculateGrandfatheringBenefit(
		...
		adjustedCost: number;
		benefit: number;
		usedActualNav: boolean;
	} {
		if (!purchaseDate) {
			return {
				applies: false,
				adjustedCost: investedAmount,
				benefit: 0,
				usedActualNav: false,
			};
		}
		...
		const estimatedFMV =
			investedAmount * (1 + GRANDFATHERING_FMV_APPRECIATION_ESTIMATE);
	...
	async calculateGrandfatheringBenefitAsync(params: {
		...
		if (schemeCode && units && units > 0) {
			try {
				const grandfatheringNav =
					await historicalNavService.getGrandfatheringNav(schemeCode);
				if (grandfatheringNav) {
					fmv = grandfatheringNav.nav * units;
					usedActualNav = true;
					navDate = grandfatheringNav.date;
					console.log(
						`[Grandfathering] Used actual NAV ${grandfatheringNav.nav} from ${navDate} for scheme ${schemeCode}`,
					);
				} else {
					// Fallback to estimate
					fmv = investedAmount * (1 + GRANDFATHERING_FMV_APPRECIATION_ESTIMATE);
					console.log(
						`[Grandfathering] No historical NAV for scheme ${schemeCode}, using estimate`,
					);
				}
			} catch (error) {
				...
				fmv = investedAmount * (1 + GRANDFATHERING_FMV_APPRECIATION_ESTIMATE);
				console.log(
					`[Grandfathering] Error fetching NAV for scheme ${schemeCode}, using estimate`,
				);
			}
		} else {
	...
	async calculateExitLoadAsync(params: {
		...
		if (isin || schemeCode) {
			try {
				...
				console.error(
					"[ProposalCapitalGains] Exit load lookup failed, using generic:",
					error,
				);
			}
		}
		...
		const exitLoad = this.calculateExitLoad(
			amount,
			holdingPeriodDays,
			productType,
			category,
		...
		if (isSlabBased) {
			...
			if (unrealizedGain > 0) {
				alerts.push({
					...
					severity: "info",
					message:
						"Debt fund gains are taxed at slab rate (no LTCG benefit). Consider STP to spread gains across FYs.",
				});
			}
		} else {
			...
			if (
				holdingPeriodDays >= stcgThreshold &&
				unrealizedGain > 0 &&
				...
				alerts.push({
					...
					severity: "info",
					message:
						"₹1.25L LTCG exemption applies at portfolio level (shared across all equity sales in FY)",
				});
			}

			// Holding Period Alert - close to LTCG threshold (only for non-slab-based)
			if (
				holdingPeriodDays >= stcgThreshold - 90 &&
				holdingPeriodDays < stcgThreshold
			) {
			...
			if (unrealizedGain > 500000 && rules.ltcg?.exemption > 0) {
				alerts.push({
					...
					severity: "info",
					message:
						"Consider STP over 2-3 FYs to spread tax liability and utilize LTCG exemption each year",
				});
			}
		}
		...
		if (exitLoadDays && holdingPeriodDays > 0) {
			...
			if (daysToExitLoadFree > 0 && daysToExitLoadFree <= 30) {
				...
				alerts.push({
					...
					severity: "info",
					message:
						"Exit load period has passed. No exit load applicable on redemption.",
				});
			}
		}
	...
	async generateAlertsAsync(
		...
		if (holding.isin && holding.currentValue > 0) {
			try {
				...
				if (
					exitLoadInfo.daysToZeroExitLoad !== null &&
					exitLoadInfo.daysToZeroExitLoad > 0
				) {
					const daysRemaining = exitLoadInfo.daysToZeroExitLoad;

					// Check if we already have an exit load alert
					const hasExitLoadAlert = alerts.some(
						(a) =>
							a.type === "HOLDING_PERIOD_ALERT" &&
							a.message.includes("Exit load"),
					);
	...
	calculateHoldingTax(
		...
		const regime = TAX_REGIMES[taxRegime];
		const assetCategory = this.getAssetCategory(
			holding.productType,
			holding.category,
		);
		const rules = regime[assetCategory as keyof typeof regime] as any;
		...
		if (isSlabBased) {
			...
			taxType = "SLAB";
			applicableTaxRate = rules?.stcg?.rate || 0.3; // Use the slab rate (30%)
			...
			// Get applicable rate
			applicableTaxRate = isSTCG
				? rules?.stcg?.rate || 0.2
				: rules?.ltcg?.rate || 0.125;
		...
		// Calculate base tax
		const estimatedTax = Math.max(0, taxableGain * applicableTaxRate);
	...
	async calculateHoldingTaxAsync(
		...
		if (USE_SANDBOX_API_FOR_TAX && SANDBOX_TAX_API_AVAILABLE) {
			try {
				const result = await this.calculateHoldingTaxWithSandboxAPI(
					holding,
					transactionDate,
				);
				...
				console.warn(
					`[CapitalGains] Sandbox API failed for ${holding.name}, using local calculation:`,
					error,
				);
			}
		}
		...
		if (isSlabBased) {
			taxType = "SLAB";
			applicableTaxRate = rules?.stcg?.rate || 0.3;
			taxableGain = unrealizedGain;
		} else {
			const isSTCG = holdingPeriodDays < (rules?.stcg?.thresholdDays || 365);
			taxType = isSTCG ? "STCG" : "LTCG";
		...
		const alerts = await this.generateAlertsAsync(
			holding,
			holdingPeriodDays,
			unrealizedGain,
			assetCategory,
		...
		const provisionalNote = getProvisionalDisclaimer([
			assetCategory as TaxAssetClass,
		]);

		this.logTaxAuditEvent(
			"HOLDING_TAX_CALCULATED",
			{
			...
			},
			{
				...
				grandfathering: grandfathering.usedActualNav
					? "historical_nav_db"
					: "estimated_fmv",
			},
		);
	...
	async calculateHoldingTaxWithSandboxAPI(
		...
		HoldingWithTax & { taxRateSource: "SANDBOX_API" | "LOCAL_FALLBACK" }
	> {
		const currentValue = holding.sellAmount || holding.currentValue;
		const investedAmount = holding.investedAmount || currentValue * 0.85;
		const holdingPeriodDays = this.calculateHoldingPeriod(holding.purchaseDate);
		...
		if (sandboxCapitalGainsService.isServiceAvailable() && purchaseDateStr) {
			try {
				const sandboxAssetType = this.mapToSandboxAssetType(
					holding.productType,
					holding.category,
				);

				const sandboxResult =
					await sandboxCapitalGainsService.calculateSingleHoldingTax({
				...
				const estimatedTaxWithCess =
					sandboxResult.estimatedTax * (1 + CESS_RATE);

				console.log(
					`[CapitalGains] Using Sandbox API rates for ${holding.name}: ${sandboxResult.taxType} @ ${(sandboxResult.applicableTaxRate * 100).toFixed(1)}%`,
				);

				this.logTaxAuditEvent(
					"HOLDING_TAX_SANDBOX_API",
					{
						holdingName: holding.name,
						isin: holding.isin,
						productType: holding.productType,
				...
				return {
					...
					taxType:
						sandboxResult.taxType === "SLAB" ? "SLAB" : sandboxResult.taxType,
				...
				console.warn(
					`[CapitalGains] Sandbox API failed for ${holding.name}, using local fallback:`,
					error,
				);
			}
		}

		const localResult = await this.calculateHoldingTaxAsync(
			holding,
			transactionDate,
		);
		return {
	...
	calculateRebalancingTaxSummary(
		...
		const EXEMPTIBLE_EQUITY_CATS = new Set([
			"equity",
			"hybrid_equity",
			"elss",
			"index",
		...
		for (const holding of holdings) {
			...
			if (taxInfo.unrealizedGain > 0) {
				if (taxInfo.taxType === "SLAB" || taxInfo.isSlabBased) {
					...
					const cat = this.getAssetCategory(
						holding.productType,
						holding.category,
					);
		...
		if (exemptibleLTCGGains > 0) {
			const revisedExemptibleTax =
				Math.max(0, exemptibleLTCGGains - LTCG_ANNUAL_EXEMPTION) *
				EQUITY_LTCG_RATE;
		...
		const fyBreakdown = this.generateFYBreakdown(
			holdingsWithTax,
			transactionDate,
		);
		...
		const assetClasses = holdings
			...
			.map(
				(h) =>
					this.getAssetCategory(h.productType, h.category) as TaxAssetClass,
			);
		const provisionalNote = getProvisionalDisclaimer(assetClasses);

		this.logTaxAuditEvent(
			"REBALANCING_TAX_SUMMARY",
			{
	...
	generateFYBreakdown(
		...
		const currentFYTotal = holdings.reduce(
			(acc, h) => {
		...
		return [
			{
				...
				totalTax:
					currentFYTotal.stcgTax +
					currentFYTotal.ltcgTax +
					currentFYTotal.slabTax,
			},
			{
	...
	getNextFY(currentFY: string): string {
		...
		const startYear = Number.parseInt(match[1]) + 1;
...
_Tax regime version: ${TAX_REGIME_VERSION}_
	...
	enrichRecommendationsWithTax(
		...
		return recommendations.map((rec) => {
			...
			const holding = portfolioHoldings.find(
				(h) => h.name === rec.productName || h.productName === rec.productName,
			);
			...
			if (waitAlert?.daysToWait && waitAlert.daysToWait <= 90) {
				...
				enrichedRec.taxEfficientAlternative = {
					type: "STP",
					description:
						"Use Systematic Transfer Plan to spread gains across financial years",
					potentialSavings: taxInfo.estimatedTax * 0.3, // Rough estimate of savings
					suggestedAction:
						"Set up monthly STP over 12-24 months to utilize LTCG exemption each FY",
				};
			}
	...
	calculateLotWiseTax(
		...
		for (const lot of lots) {
			...
			const daysToLTCG =
				holdingPeriodDays < stcgThreshold
					...
					: null;
			const daysToExitLoadFree =
				holdingPeriodDays < effectiveExitLoadDays
		...
		// Generate alerts
		const lotsNearLTCG = lotTaxInfos.filter(
			(l) => l.daysToLTCG !== null && l.daysToLTCG <= 30,
		);
		if (lotsNearLTCG.length > 0) {
			const totalValueNearLTCG = lotsNearLTCG.reduce(
				(sum, l) => sum + l.currentValue,
				0,
			);
			const potentialSavings = lotsNearLTCG.reduce(
				(sum, l) => sum + l.estimatedTax * 0.5,
				0,
			);
			alerts.push({
				type: "WAIT_FOR_LTCG",
		...
		const lotsNearExitLoadFree = lotTaxInfos.filter(
			(l) =>
				l.daysToExitLoadFree !== null &&
				l.daysToExitLoadFree <= 30 &&
				l.exitLoadApplicable,
		);
		if (lotsNearExitLoadFree.length > 0) {
			const potentialSavings = lotsNearExitLoadFree.reduce(
				(sum, l) => sum + l.exitLoadAmount,
				0,
			);
			alerts.push({
				type: "HOLDING_PERIOD_ALERT",
				...
				potentialSavings,
				daysToWait: Math.max(
					...lotsNearExitLoadFree.map((l) => l.daysToExitLoadFree || 0),
				),
			});
		}

		this.logTaxAuditEvent(
			"LOT_WISE_TAX_CALCULATED",
			{
				holdingName,
				isin,
				schemeCode,
			...
			},
			{
				...
				totalLTCGTax,
				totalExitLoad,
				totalCurrentValue: lotTaxInfos.reduce(
					(sum, l) => sum + l.currentValue,
					0,
				),
			},
	...
	getOptimizedRedemptionPlan(
		...
		switch (strategy) {
			...
			case "fifo":
				...
				sortedLots = [...lotWiseSummary.lots].sort(
					(a, b) => b.holdingPeriodDays - a.holdingPeriodDays,
				);
				break;
		}

		const lotsToRedeem: Array<{
			lotId: string;
			unitsToRedeem: number;
			value: number;
			taxType: string;
		...
		for (const lot of sortedLots) {
			...
			const exitLoad = lot.exitLoadApplicable
				? lot.exitLoadAmount * redeemRatio
				: 0;
	 ...
	 */
	async generateTaxEfficientSellAdvice(
		holdings: Array<{
			name: string;
			productType: string;
			category?: string;
		...
		for (const holding of holdings) {
			...
			const ltcgThreshold = this.isEquityOriented(
				holding.productType,
				holding.category,
			)
				? 365
				: 1095;
			const daysToLTCG =
				holdingPeriodDays < ltcgThreshold
					? ltcgThreshold - holdingPeriodDays
					: null;
			...
			const shouldWaitForLTCG =
				daysToLTCG !== null &&
				daysToLTCG <= 90 &&
				taxDetails.unrealizedGain > 0;

			// Check if exit load applies and close to expiry (within 60 days)
			const shouldWaitForExitLoad =
				daysToExitLoadFree !== null &&
				daysToExitLoadFree > 0 &&
				daysToExitLoadFree <= 60;
			...
			if (shouldWaitForLTCG && shouldWaitForExitLoad) {
				...
				const stcgTax = taxDetails.unrealizedGain * 0.2;
				...
				sellAdvice = "SELL_NOW";
				advisoryMessage =
					"Consider tax loss harvesting - this loss can offset gains";
			} else {
				sellAdvice = "OPTIMAL";
	...
	calculateIndexationBenefit(input: {
		...
		if (purchaseDate >= INDEXATION_CUTOFF_DATE) {
			eligibilityReason =
				"Purchased after April 1, 2023 - indexation benefit not available for debt funds";
			...
			const isDebtOrGold = this.isDebtOrGoldFund(
				input.productType,
				input.category,
			);
			if (!isDebtOrGold) {
				eligibilityReason =
					"Indexation benefit only applies to debt and gold funds";
			} else {
				eligible = true;
				eligibilityReason =
					"Eligible for indexation benefit (debt/gold fund purchased before April 2023, held for 3+ years)";
			}
		}

		// Get CII values
		const purchaseCII =
			COST_INFLATION_INDEX[purchaseFY] || COST_INFLATION_INDEX["2024-25"];
		const saleCII =
			COST_INFLATION_INDEX[saleFY] || COST_INFLATION_INDEX["2025-26"];

		// Calculate indexation factor
		const indexationFactor = saleCII / purchaseCII;

		// Calculate indexed cost of acquisition
		const indexedCost = eligible
			? input.purchaseCost * indexationFactor
			: input.purchaseCost;
		...
		const taxRateWithoutIndexation = 0.3;
		const taxRateWithIndexation = 0.2;

		const taxWithoutIndexation = Math.max(
			0,
			gainWithoutIndexation * taxRateWithoutIndexation,
		);
		const taxWithIndexation = eligible
			? Math.max(0, gainWithIndexation * taxRateWithIndexation)
			: taxWithoutIndexation;

		const taxSavingsFromIndexation = eligible
			? taxWithoutIndexation - taxWithIndexation
			: 0;
		...
		return {
			...
			taxSavingsFromIndexation:
				Math.round(taxSavingsFromIndexation * 100) / 100,
			purchaseFY,
			saleFY,
			purchaseCII,
			saleCII,
	...
	private isDebtOrGoldFund(productType: string, category?: string): boolean {
		...
		if (
			category &&
			debtGoldCategories.some((c) => category.toLowerCase().includes(c))
		) {
			return true;
		}
	...
	private isEquityOriented(productType: string, category?: string): boolean {
		...
		if (
			category &&
			equityCategories.some((c) => category.toLowerCase().includes(c))
		) {
			return true;
		}
	...
	async calculateLotWiseTaxFromDatabase(
		...
		try {
			const { lots } = await holdingLotsStorageService.getLotsByIsin(
				userId,
				isin,
			);
			...
			const formattedLots = lots
				...
				.map((lot) => {
					const purchaseNav = Number.parseFloat(
						lot.purchaseNav ?? lot.costPerUnit ?? "0",
					);
					const purchaseValue = Number.parseFloat(lot.totalCost ?? "0");
					const units = Number.parseFloat(lot.units ?? "0");
					const remainingUnits = Number.parseFloat(
						lot.remainingUnits ?? lot.units ?? "0",
					);

					return {
						id: lot.id,
						purchaseDate: lot.purchaseDate,
						purchaseNav: Number.isNaN(purchaseNav) ? 0 : purchaseNav,
						purchaseValue: Number.isNaN(purchaseValue) ? 0 : purchaseValue,
						units: Number.isNaN(units) ? 0 : units,
						remainingUnits: Number.isNaN(remainingUnits) ? 0 : remainingUnits,
						source: (lot.transactionType === "sip"
							? "sip"
							: lot.transactionType === "switch_in"
								? "switch_in"
								: lot.transactionType === "bonus"
									? "bonus"
									: "purchase") as "purchase" | "sip" | "switch_in" | "bonus",
					};
				})
			...
			console.log(
				`[CapitalGains] Calculating tax for ${formattedLots.length} lots from database for ${isin}`,
			);
