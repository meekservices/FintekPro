...
export class TreasuryService {
  ...
  async getEntityById(id: string) {
    const [entity] = await db.select().from(treasuryEntities).where(eq(treasuryEntities.id, id));
    return entity;
  }
  ...
  async linkBankAccount(entityId: string, data: {
    ...
    bankName: string;
    branchName?: string;
    accountType: 'current' | 'savings' | 'escrow' | 'virtual' | 'investment' | 'debt' | 'collection' | 'disbursement';
    currency: string;
    provider: string;
