...
class KYCVaultDecryptionService {
	...
	async decryptVaultData(
		...
		try {
			console.log(
				`🔓 Decrypting KYC vault for user ${userId}, purpose: ${options.purpose}`,
			);
			...
			if (!vault.tokenizedPan) {
				await this.logVaultAccess(
					...
					options,
					"failed",
					"Tokenized PAN not found in vault",
				);
				return {
					success: false,
			...
			if (vault.tokenizedAadhaar) {
				...
				if (
					aadhaarDetokenResult.success &&
					aadhaarDetokenResult.originalValue
				) {
			...
			const decryptedDOB = encryptionService.decrypt(
				vault.encryptedDateOfBirth,
			);
			...
			console.log(
				`✅ KYC vault decrypted successfully for user ${userId} (${durationMs}ms)`,
			);
	...
	async decryptSpecificFields(
		userId: string,
		fields: Array<
			"pan" | "aadhaar" | "fullName" | "dateOfBirth" | "mobile" | "email"
		>,
		options: VaultAccessOptions,
	): Promise<VaultDecryptionResult> {
	...
	private async logVaultAccess(
		...
		try {
			...
			console.error(
				"🚨 CRITICAL: Failed to log KYC vault access:",
				error.message,
			);
			console.error(
				"   User:",
