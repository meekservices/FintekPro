...
export class ReminderSchedulerService {
	...
	start(): void {
		...
		console.log(
			"✅ Capital gains reminder scheduler started (runs daily at 9:00 AM IST)",
		);
		...
		try {
			...
			const upcomingReminders = await db
				...
				.leftJoin(
					taxReminderSubscriptions,
					eq(
						capitalGainsTaxReminders.subscriptionId,
						taxReminderSubscriptions.id,
					),
				)
				.leftJoin(users, eq(capitalGainsTaxReminders.userId, users.id))
			...
			const activeReminders = upcomingReminders.filter(
				(item) =>
			...
			console.log(
				`✅ ${activeReminders.length} reminders with active subscriptions`,
			);
			...
			for (const item of activeReminders) {
				try {
					...
					const notification: ReminderNotification = {
						...
						estimatedSTCG: Number.parseFloat(
							item.reminder.estimatedSTCG || "0",
						),
						estimatedLTCG: Number.parseFloat(
							item.reminder.estimatedLTCG || "0",
						),
						totalTaxLiability: Number.parseFloat(
							item.reminder.totalTaxLiability || "0",
						),
						reminderChannels: (item.subscription
							?.reminderChannels as string[]) || ["email"],
					};

					await this.sendNotification(notification);

					// Update reminder status to 'sent'
					await capitalGainsCalculator.updateReminderStatus(
						item.reminder.id,
						"sent",
					);

					console.log(
						`✉️ Sent reminder for ${notification.quarter} to ${notification.email}`,
					);
				} catch (error) {
					console.error(
						`❌ Error sending reminder for user ${item.user?.id}:`,
						error,
					);
	...
	private async sendEmailNotification(data: {
		...
		console.log(
			`     This is a reminder that your advance tax payment for ${data.quarter} is due on ${data.dueDate}.`,
		);
		console.log(`     `);
		...
		console.log(
			`     Payment Link: https://www.incometax.gov.in/iec/foportal/`,
		);
	...
	private async sendSMSNotification(data: {
		...
		console.log(
			`   Dear ${data.firstName}, your advance tax for ${data.quarter} (${data.totalTaxLiability}) is due on ${data.dueDate}. Visit our portal for details.`,
		);
	...
	private async logNotification(
		...
		try {
			...
			console.log(
				`📝 Audit Log: ${channel} notification sent to ${recipient} for ${quarter} at ${new Date().toISOString()}`,
			);
	...
	async recalculateAllUsers(): Promise<{
		...
		console.log(
			"🔄 Recalculating advance tax reminders for all users with realized gains...",
		);
		...
		try {
			...
			console.log(
				`✅ Recalculation complete: ${recalculated} users updated, ${errors} errors`,
			);
		} catch (error) {
	...
	async getAdvanceTaxStatus(userId: string, fiscalYear?: string): Promise<any> {
		return realizedGainsAggregationService.getAdvanceTaxStatus(
			userId,
			fiscalYear,
		);
	}
