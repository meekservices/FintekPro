...
class AlpacaBrokerService {
	...
	constructor() {
		...
		this.isPaper =
			this.baseUrl.includes("sandbox") || this.baseUrl.includes("paper");
	...
	private _buildClient(): AxiosInstance {
		...
		if (this._isBrokerApi()) {
			const encoded = Buffer.from(`${this.apiKey}:${this.secretKey}`).toString(
				"base64",
			);
	...
	async testConnection(): Promise<{
		...
		try {
			...
			const response = await this.client.get(path, {
				params: this._isBrokerApi() ? { max: 1 } : {},
			});
	...
	async createBrokerAccount(data: {
		...
		try {
			...
			throw new Error(
				alpacaMsg || error.message || "Alpaca account creation failed",
			);
		}
	}
	...
	async getTradingAccount(accountId: string): Promise<AlpacaAccount | null> {
		...
		try {
			const response = await this.client.get(
				`${this._tradingBase(accountId)}/account`,
			);
			return response.data;
		} catch (error: any) {
	...
	async deleteAchRelationship(
		...
		await this.client.delete(
			`/v1/accounts/${accountId}/ach_relationships/${achRelationshipId}`,
		);
	}
	...
	async listBankAccounts(accountId: string): Promise<any[]> {
		try {
			const response = await this.client.get(
				`/v1/accounts/${accountId}/bank_accounts`,
			);
			return Array.isArray(response.data) ? response.data : [];
		} catch (error: any) {
	...
	async getAllActivities(params?: {
		...
		try {
			const response = await this.client.get("/v1/accounts/activities", {
				params,
			});
			return Array.isArray(response.data) ? response.data : [];
		} catch (error: any) {
	...
	async getAsset(symbolOrId: string): Promise<AlpacaAsset | null> {
		try {
			const path = this._isBrokerApi()
				? `/v1/assets/${symbolOrId}`
				: `/v2/assets/${symbolOrId}`;
			const response = await this.client.get(path);
			return response.data;
		} catch (error: any) {
	...
	async listWatchlists(accountId: string): Promise<AlpacaWatchlist[]> {
		try {
			const response = await this.client.get(
				`${this._tradingBase(accountId)}/watchlists`,
			);
			return Array.isArray(response.data) ? response.data : [];
		} catch (error: any) {
	...
	async getWatchlist(
		accountId: string,
		watchlistId: string,
	): Promise<AlpacaWatchlist | null> {
		try {
			const response = await this.client.get(
				`${this._tradingBase(accountId)}/watchlists/${watchlistId}`,
			);
			return response.data;
		} catch (error: any) {
			if (error.response?.status !== 404)
	...
	async removeFromWatchlist(
		...
		await this.client.delete(
			`${this._tradingBase(accountId)}/watchlists/${watchlistId}/${symbol}`,
		);
	}
	...
	async placeOrder(
		...
		if (request.type === "trailing_stop") {
			if (request.trail_price)
				orderPayload.trail_price = request.trail_price.toString();
			else if (request.trail_percent)
				orderPayload.trail_percent = request.trail_percent.toString();
		}
		...
		if (request.order_class && request.order_class !== "simple") {
			...
			if (request.take_profit)
				orderPayload.take_profit = {
					limit_price: request.take_profit.limit_price.toString(),
				};
			if (request.stop_loss)
				orderPayload.stop_loss = {
					stop_price: request.stop_loss.stop_price.toString(),
					...(request.stop_loss.limit_price && {
						limit_price: request.stop_loss.limit_price.toString(),
					}),
				};
		}
		const path =
			this._isBrokerApi() && request.account_id
	...
	async getPortfolioHistoryWithResolution(
		...
		try {
			const response = await this.client.get(
				`${this._tradingBase(accountId)}/account/portfolio/history`,
				{ params },
			);
			return response.data;
		} catch (error: any) {
			console.error(
				"Error fetching high-res portfolio history:",
				error.message,
			);
			return null;
		}
	...
	async getMarketCalendar(params?: { start?: string; end?: string }): Promise<
		any[]
	> {
		try {
	...
	async createFundingWallet(accountId: string): Promise<AlpacaFundingWallet> {
		const response = await this.client.post(
			`/v1beta/accounts/${accountId}/funding_wallet`,
		);
		return response.data;
	}
	...
	async getFundingWallet(
		accountId: string,
	): Promise<AlpacaFundingWallet | null> {
		try {
			const response = await this.client.get(
				`/v1beta/accounts/${accountId}/funding_wallet`,
			);
	...
	async getFundingWalletDetails(
		accountId: string,
		walletId: string,
	): Promise<AlpacaFundingDetail[]> {
		try {
			const response = await this.client.get(
				`/v1beta/accounts/${accountId}/funding_wallet/${walletId}/details`,
			);
			return Array.isArray(response.data) ? response.data : [];
		} catch (error: any) {
	...
	async getFundingWalletTransfers(
		accountId: string,
		walletId: string,
	): Promise<any[]> {
		try {
			const response = await this.client.get(
				`/v1beta/accounts/${accountId}/funding_wallet/${walletId}/transfers`,
			);
			return Array.isArray(response.data) ? response.data : [];
		} catch (error: any) {
	...
	async listRecipientBanks(accountId: string): Promise<AlpacaRecipientBank[]> {
		try {
			const response = await this.client.get(
				`/v1beta/accounts/${accountId}/recipient_banks`,
			);
			return Array.isArray(response.data) ? response.data : [];
		} catch (error: any) {
	...
	async deleteRecipientBank(accountId: string, bankId: string): Promise<void> {
		await this.client.delete(
			`/v1beta/accounts/${accountId}/recipient_banks/${bankId}`,
		);
	}
	...
	async createWireWithdrawal(
		...
		const response = await this.client.post(
			`/v1beta/accounts/${accountId}/funding_wallet/withdrawals`,
			{
	...
	async getOptionContract(
		...
		try {
			...
			if (error.response?.status !== 404)
				console.error("Error fetching option contract:", error.message);
			return null;
		}
	}
	...
	async getAccountConfig(
		accountId?: string,
	): Promise<AlpacaAccountConfig | null> {
		try {
			const path =
				this._isBrokerApi() && accountId
	...
	async updateAccountConfig(
		updates: Partial<AlpacaAccountConfig>,
		accountId?: string,
	): Promise<AlpacaAccountConfig> {
		const path =
			this._isBrokerApi() && accountId
				? `/v1/trading/accounts/${accountId}/account/configurations`
				: "/v2/account/configurations";
	...
	async createBatchJournals(
		...
		try {
			const response = await this.client.post("/v1/journals:batch", {
				journals,
			});
	...
	async getRebalancingPortfolio(
		portfolioId: string,
	): Promise<AlpacaRebalancingPortfolio | null> {
		try {
			const response = await this.client.get(
	...
	async subscribeAccountToPortfolio(
		portfolioId: string,
		accountId: string,
	): Promise<any> {
		const response = await this.client.post(
	...
	async unsubscribeAccountFromPortfolio(
		portfolioId: string,
	...
	async createRebalancingRun(
		portfolioId: string,
		type:
			| "full_rebalance"
			| "partial_rebalance"
			| "liquidation" = "full_rebalance",
	): Promise<AlpacaRebalancingRun> {
		const response = await this.client.post(
			`/v1/rebalancing/portfolios/${portfolioId}/runs`,
			{ type },
		);
		return response.data;
	}

	async listRebalancingRuns(
		portfolioId: string,
	): Promise<AlpacaRebalancingRun[]> {
		try {
			const response = await this.client.get(
	 ...
	 */
	async getCorporateActionAnnouncement(
		announcementId: string,
	): Promise<any | null> {
		try {
			const response = await this.client.get(
				`/v1/corporate_actions/announcements/${announcementId}`,
			);
			return response.data;
		} catch (error: any) {
			if (error.response?.status !== 404)
				console.error("[CorpAction] getAnnouncement error:", error.message);
			return null;
		}
	}
	 ...
	 */
	async getAccountCorporateActions(
		accountId: string,
		params?: {
			symbol?: string;
			types?: string;
		...
		try {
			...
			if (error.response?.status !== 404) {
				console.error(
					"[CorpAction] getAccountCorporateActions error:",
					error.message,
				);
			}
			return [];
	 ...
	 */
	async getCorporateActionElections(
		accountId: string,
		announcementId: string,
	): Promise<any[]> {
		try {
			...
			if (error.response?.status !== 404)
				console.error("[CorpAction] getElections error:", error.message);
			return [];
		}
	}
	...
	async getPositionTaxLots(accountId: string, symbol: string): Promise<any[]> {
		try {
			...
			if (error.response?.status !== 404)
				console.error("[TaxLots] getPositionTaxLots error:", error.message);
			return [];
		}
	}
	...
	async getAllPositionTaxLots(accountId: string): Promise<any[]> {
		try {
			...
			if (error.response?.status !== 404)
				console.error("[TaxLots] getAllPositionTaxLots error:", error.message);
			return [];
		}
	}
	...
	async exerciseOptions(
		accountId: string,
		contracts: Array<{ symbol: string; qty: number }>,
		type: "e" | "a" = "e",
	): Promise<any> {
		const response = await this.client.post(
	...
	async listOptionsExercises(
		...
		try {
			...
			if (error.response?.status !== 404)
				console.error("[Options] listExercises error:", error.message);
			return [];
		}
	}
	...
	async getAccountRestrictions(accountId: string): Promise<any | null> {
		try {
			...
			if (error.response?.status !== 404)
				console.error(
					"[Restrictions] getAccountRestrictions error:",
					error.message,
				);
			return null;
		}
	 ...
	 */
	async updateAccountRestrictions(
		accountId: string,
		restrictions: {
	...
	async getTrustedContact(accountId: string): Promise<any | null> {
		try {
			const response = await this.client.get(
				`/v1/accounts/${accountId}/trusted_contact`,
			);
			return response.data;
		} catch (e: any) {
			if (e.response?.status === 404) return null;
	...
	async reverseJournal(journalId: string): Promise<AlpacaJournal> {
		const response = await this.client.post(
			`/v1/journals/${journalId}/reversals`,
		);
		return response.data;
	}
	...
	async getCashInterestStatus(accountId: string): Promise<any | null> {
		try {
			const response = await this.client.get(
				`/v1/accounts/${accountId}/interest`,
			);
			return response.data;
		} catch (e: any) {
			if (e.response?.status === 404) return null;
	...
	async getIpAllowlist(): Promise<string[]> {
		...
		try {
			...
			if (error.response?.status !== 403)
				console.error("[Security] getIpAllowlist error:", error.message);
			return [];
		}
	}
	...
	async getAssets(params?: {
		...
		try {
			...
			console.warn(
				"[AlpacaBrokerService] getAssets failed (non-fatal):",
				error?.response?.data ?? error?.message,
			);
			return [];
		}
