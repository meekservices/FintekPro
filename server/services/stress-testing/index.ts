...
export class StressTestingGovernanceInterceptor {
	 ...
	 */
	public evaluateSimulationViolations(
		simContext: SimulationOutputContract,
		thresholds: StressTestThresholds,
	): { status: "PASS" | "BLOCK" | "MODIFY"; reason?: string } {
		...
		if (
			simContext.scenario_results.bear.drawdown > thresholds.max_drawdown_limit
		) {
			return {
				status: "MODIFY",
				reason:
					"WARNING: Base metrics passed, but simulated Bear Scenario breached severe drawdown conditions. Appending manual Risk Override UI warnings.",
			};
		}
...
export const stressTestingInterceptor =
	new StressTestingGovernanceInterceptor();
