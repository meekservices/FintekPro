...
class TransactionAutomationEngine {
  ...
  async submitOrder(request: OrderRequest): Promise<OrderResult> {
    const { userId, agentId, partnerId, instrumentType, amountInr, gatewayPayload, requiredKycLevel = '1' } = request;

    // ── 1. KYC Gate ───────────────────────────────────────────────────────────
    try {
      const { level } = await getUserKYCLevel(userId);
      const levelNum = parseInt(level);
      const requiredNum = parseInt(requiredKycLevel);
      ...
      logger.warn('[TransactionEngine] KYC check failed, proceeding with caution', { userId, instrumentType });
    }
    ...
    try {
      result = await this.dispatchToGateway(instrumentType, gateway.provider, gatewayPayload, userId);
    } catch (err) {
      ...
      return {
        ...
        clientMessage: 'Your order could not be submitted. Please try again or contact support.',
        provider: gateway.provider,
      };
    }
    ...
    if (result.status === 'SUBMITTED' || result.status === 'QUEUED') {
      ...
      if (partnerId) {
        this.checkCommissionConfig(partnerId, instrumentType, result.orderId ?? 'unknown').catch(() => {});
      }
    }
    ...
    switch (provider) {
      ...
      case 'iris_kfintech': {
        ...
        return {
          ...
          message: response.message ?? (response.success ? 'Order submitted to KFintech (Iris).' : 'KFintech order failed.'),
      ...
      case 'alpaca': {
        ...
        return {
          ...
          message: response.id ? 'Order submitted to Alpaca.' : 'Alpaca order failed.',
      ...
      case 'bse_star': {
        ...
        return {
          ...
          message: response.message ?? (response.success ? 'Order submitted to BSE STAR.' : 'BSE STAR order failed.'),
          clientMessage: response.success
      ...
      case 'iifl': {
        ...
        return {
          ...
          clientMessage: 'Indian stock trading is coming soon. We are finalising our partnership with IIFL Securities.',
          comingSoon: true,
          provider,
        };
      }
      ...
      default: {
        ...
        logger.error('[TransactionEngine] No dispatch handler for provider', { provider, instrumentType });
        return {
          status: 'GATEWAY_UNAVAILABLE',
          message: `No handler for provider: ${provider}`,
          clientMessage: 'This service is temporarily unavailable. Please try again later.',
          provider,
        };
      }
    }
  ...
  private async checkCommissionConfig(partnerId: string, instrumentType: string, orderId: string): Promise<void> {
    try {
      ...
      if (!commission || commission.commissionRate === null) {
        ...
        const [partner] = await db.select({ name: schema.partners.businessName })
      ...
      logger.warn('[TransactionEngine] Commission check failed', { partnerId, instrumentType });
    }
  }
}
