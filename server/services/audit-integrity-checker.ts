...
class AuditIntegrityChecker {
	...
	async initialize(intervalMinutes: number = 60): Promise<void> {
		...
		console.log(
			`[AuditIntegrity] Initializing with ${intervalMinutes} minute interval`,
		);

		const result = await this.runIntegrityCheck();
		if (result.status === "failed") {
			console.error(
				"[AuditIntegrity] CRITICAL: Initial integrity check failed!",
			);
		}
	...
	startScheduledChecks(): void {
		...
		this.isRunning = true;
		console.log(
			`[AuditIntegrity] Scheduled checks started (every ${this.scheduleIntervalMinutes} minutes)`,
		);
	}
	...
	private async handleIntegrityFailure(
		...
		console.error(
			"[CRITICAL] [AuditIntegrity] AUDIT TRAIL INTEGRITY FAILURE DETECTED!",
		);
		console.error(
			"[CRITICAL] [AuditIntegrity] Broken links:",
			result.brokenLinks,
		);
		console.error(
			"[CRITICAL] [AuditIntegrity] Checksum mismatches:",
			result.checksumMismatches,
		);
		console.error(
			"[CRITICAL] [AuditIntegrity] This may indicate tampering or data corruption!",
		);
	...
	private async storeFailedVerification(
		result: IntegrityCheckResult,
	): Promise<void> {
		try {
      ...
      `);
			console.log(
				"[AuditIntegrity] Failed verification result stored for admin review",
			);
		} catch (error: any) {
			if (error.message?.includes("does not exist")) {
				console.warn(
					"[AuditIntegrity] audit_integrity_failures table does not exist - creating...",
				);
				await this.createFailuresTable();
				await this.storeFailedVerification(result);
			} else {
				console.error(
					"[AuditIntegrity] Failed to store verification result:",
					error,
				);
			}
		}
	...
	private async sendAdminAlert(result: IntegrityCheckResult): Promise<void> {
      ...
      `);
			...
			if (adminEmails.length === 0) {
				console.warn(
					"[AuditIntegrity] No admin emails found for alert notification",
				);
				return;
			}
      ...
      `;
			...
			for (const email of adminEmails) {
				try {
					...
					console.error(
						`[AuditIntegrity] Failed to send alert to ${email}:`,
						emailError.message,
					);
				}
			}
	...
	async getFailedVerifications(limit: number = 50): Promise<any[]> {
      ...
      `);
			...
			console.error(
				"[AuditIntegrity] Failed to get failed verifications:",
				error,
			);
			return [];
		}
	...
	async markVerificationReviewed(
		failureId: string,
		reviewedBy: string,
	): Promise<boolean> {
		try {
      ...
      `);
			...
			console.error(
				"[AuditIntegrity] Failed to mark verification as reviewed:",
				error,
			);
			return false;
		}
