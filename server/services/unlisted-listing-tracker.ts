...
async function probeBSEByISIN(isin: string): Promise<{
  ...
  try {
    ...
    if (data.COM_EQ_DT_LISTING) {
      ...
      if (parts.length === 3) listingDate = `${parts[2]}-${parts[1]}-${parts[0]}`;
    }
    return {
...
async function probeNSEByName(companyName: string, knownIsin?: string | null): Promise<{
  symbol: string;
  isin?: string;
  listingDate?: string;
} | null> {
  try {
    ...
    const hits: any[] = (data?.symbols || []).filter((h: any) => h.series === 'EQ');
    if (hits.length === 0) return null;

    // Pick best match: prefer ISIN match, then name overlap ≥ 0.6
    let best: any = null;
    for (const hit of hits) {
      if (knownIsin && hit.isin === knownIsin) { best = hit; break; }
    ...
    const rawDate = details?.securityInfo?.listingDate || details?.info?.listingDate || null;

    return { symbol: meta.symbol, isin: meta.isin, listingDate: rawDate ?? undefined };
  } catch {
    return null;
  }
}
...
async function detectListing(company: {
  ...
  if (!isin && company.cin) {
    ...
    if (isin) {
      ...
      console.log(`[ListingTracker] ISIN ${isin} discovered for ${company.name} via Credhive`);
    }
  }
  ...
  if (isin || company.cin) {  // only try if we have some identifier
    await sleep(600);
...
export class UnlistedListingTracker {
  ...
  async runTransitionSweep(): Promise<ListingTransition[]> {
    ...
    try {
      ...
      console.log(`[ListingTracker] Sweep started — ${candidates.length} candidates to check`);
      ...
      for (const company of candidates) {
        try {
          ...
          // ── Perform transition ─────────────────────────────────────────
          await db.update(unlistedCompanies)
            .set({ status: 'inactive', listingStage: 'listed', updatedAt: new Date() })
            .where(eq(unlistedCompanies.id, company.id));
   ...
   */
  async checkSingleCompany(companyId: string): Promise<ListingTransition | null> {
    const rows = await db.select().from(unlistedCompanies)
      .where(eq(unlistedCompanies.id, companyId)).limit(1);
    if (!rows[0]) return null;

    const company = rows[0];
    const detected = await detectListing({
      id: company.id, name: company.name, isin: company.isin, cin: company.cin
    });
