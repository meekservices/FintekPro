...
export async function syncDeskTickets(
	...
	for (const ticket of result.data) {
		const [existing] = await db
			...
			.where(eq(aiDeskTicketActions.zohoTicketId, ticket.id))
			.limit(1);
		...
		try {
			...
			console.error(
				`[Desk Agent] Classification failed for ticket ${ticket.id}:`,
				err,
			);
		}
		...
		const baseData = {
			...
			createdInZoho: ticket.createdTime
...
Overdue: ${ticket.isOverDue} | Due in: ${Number.isFinite(dueHrs) ? `${dueHrs}h` : "No due date"}
...
Rules: Professional tone, acknowledge issue, state next steps (24h contact), no profit promises, add SEBI disclaimer if investment-related.`;
	...
	await auditLog({
		...
		userRole: "admin",
		agentType: "desk",
		agentAction: "draft_response",
		entityId: deskTicketActionId,
		entityType: "ai_desk_ticket_actions",
...
export async function flagSlaBreachRisk(
	...
	for (const ticket of overdue) {
		await db.insert(aiComplianceAlerts).values({
			...
			severity:
				ticket.priority === "Urgent" || ticket.isEscalated
					? "critical"
					: "high",
...
export async function sendDraftResponse(
	...
	if (record.approvalStatus !== "approved")
		throw new Error("Draft response not approved");
