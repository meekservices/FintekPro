...
function createConfirmationToken(action: ApprovalAction, entityId: string, adminId: string): string {
  const token = `confirm_${randomUUID()}`;
  pendingConfirmations.set(token, {
    action, entityId, adminId, expiresAt: Date.now() + 5 * 60 * 1000, // 5 min TTL
  });
  return token;
}
...
async function markEntityApproved(
  ...
  switch (entityType) {
    case 'ai_email_classifications':
      await db.update(aiEmailClassifications).set({
        approvalStatus: 'approved', approvedBy: adminId, approvedAt: now, updatedAt: now,
      }).where(eq(aiEmailClassifications.id, entityId));
      break;
    case 'ai_proposal_drafts':
      await db.update(aiProposalDrafts).set({
        approvalStatus: 'approved', approvedBy: adminId, approvedAt: now, updatedAt: now,
      }).where(eq(aiProposalDrafts.id, entityId));
      break;
    case 'ai_admin_tasks':
      await db.update(aiAdminTasks).set({
        approvalStatus: 'approved', approvedBy: adminId, approvedAt: now,
        status: 'approved', updatedAt: now,
      }).where(eq(aiAdminTasks.id, entityId));
      break;
    case 'ai_invoice_drafts':
      await db.update(aiInvoiceDrafts).set({
        approvalStatus: 'approved', approvedBy: adminId, approvedAt: now, updatedAt: now,
      }).where(eq(aiInvoiceDrafts.id, entityId));
      break;
    case 'ai_meeting_actions':
      await db.update(aiMeetingActions).set({
        approvalStatus: 'approved', approvedBy: adminId, approvedAt: now, updatedAt: now,
      }).where(eq(aiMeetingActions.id, entityId));
      break;
    case 'ai_meeting_notes':
      await db.update(aiMeetingNotes).set({
        approvalStatus: 'approved', approvedBy: adminId, approvedAt: now, updatedAt: now,
...
async function markEntityRejected(
  ...
  switch (entityType) {
    case 'ai_email_classifications':
      await db.update(aiEmailClassifications).set({
        approvalStatus: 'rejected', draftReplyStatus: 'rejected', updatedAt: now,
      }).where(eq(aiEmailClassifications.id, entityId));
      break;
    case 'ai_proposal_drafts':
      await db.update(aiProposalDrafts).set({
        approvalStatus: 'rejected', rejectionReason: reason, updatedAt: now,
      }).where(eq(aiProposalDrafts.id, entityId));
      break;
    case 'ai_admin_tasks':
      await db.update(aiAdminTasks).set({
        approvalStatus: 'rejected', status: 'closed', updatedAt: now,
      }).where(eq(aiAdminTasks.id, entityId));
      break;
    case 'ai_invoice_drafts':
      await db.update(aiInvoiceDrafts).set({
    ...
    case 'ai_meeting_actions':
      await db.update(aiMeetingActions).set({
        approvalStatus: 'rejected', meetingStatus: 'cancelled', updatedAt: now,
      }).where(eq(aiMeetingActions.id, entityId));
      break;
  }
}
...
export async function processApproval(req: ApprovalRequest): Promise<ApprovalResult> {
  const {
    agentType, entityId, entityType, action,
    adminId, adminRole, editedContent, notes, confirmationToken,
  } = req;
  ...
  if (twoStepActions.includes(action)) {
    ...
    if (!validateConfirmationToken(confirmationToken, adminId)) {
      return {
        success:    false,
        approvalId: randomUUID(),
        message:    'Invalid or expired confirmation token. Please restart the approval.',
      };
    }
  ...
  // Audit log
  await auditLog({
    userId: adminId, userRole: adminRole,
    agentType, agentAction: `approval_${action}`,
    entityId, entityType,
    approvalStatus: action === 'approve' ? 'approved' : action === 'reject' ? 'rejected' : action,
