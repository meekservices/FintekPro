...
Format: numbered list with time estimates. Include welcome, 2-4 main items, action items, Q&A. Max 150 words.`;
	...
	const { data: agendaData, meta } = await callGemini<string>(
		systemPrompt,
		userPrompt,
		{ parseJson: false },
	);
	const agenda =
		typeof agendaData === "string" ? agendaData : JSON.stringify(agendaData);

	const [meeting] = await db
		.insert(aiMeetingActions)
		.values({
			connectionId,
...
Return:
{
  ...
  "isComplianceSensitive": boolean
	...
	const { data: parsed, meta } = await callGemini<PostMeetingAI>(
		systemPrompt,
		userPrompt,
		{ parseJson: true },
	);
	...
	await auditLog({
		...
		userRole: "admin",
		agentType: "meeting",
		agentAction: "post_meeting_summary",
		entityId: notes.id,
		entityType: "ai_meeting_notes",
...
export async function extractFollowupTasks(
	...
	const [notes] = await db
		.select()
		.from(aiMeetingNotes)
		.where(eq(aiMeetingNotes.id, meetingNotesId))
		.limit(1);
	if (!notes) throw new Error("Meeting notes not found");

	const actionItems =
		(notes.actionItems as {
			task: string;
			owner: string;
			dueDate?: string;
		}[]) || [];
	...
	for (const item of actionItems) {
		...
		const [followup] = await db
			.insert(aiMeetingFollowups)
			.values({
				meetingNotesId,
				meetingActionId: notes.meetingActionId,
				taskTitle: item.task,
				taskDescription: `From meeting notes: ${notes.summary?.substring(0, 100)}`,
				assignedToRole: item.owner?.toLowerCase().includes("admin")
					? "admin"
					: "agent",
				dueDate,
				priority:
					dueDate && dueDate < new Date(Date.now() + 3 * 24 * 3600000)
						? "high"
						: "medium",
	...
	await auditLog({
		...
		userRole: "admin",
		agentType: "meeting",
		agentAction: "extract_followups",
		entityId: meetingNotesId,
		entityType: "ai_meeting_notes",
