...
function getBooksOrgId(): string {
  ...
  if (!id) throw new Error('ZOHO_BOOKS_ORG_ID is not set. Please add it to GCP Secrets.');
  return id;
}
...
export async function syncBooksData(
  ...
  const [invoices, payments, overdue] = await Promise.all([
    books.getInvoices({ per_page: 200, sort_column: 'created_time', sort_order: 'descending' }),
  ...
  const [action] = await db.insert(aiBooksFinanceActions).values({
    connectionId,
    ...
    dataScope:       { dateRange: 'all', entityType: 'invoices+payments', count: invoices.totalRecords },
    result:          summary,
    confidenceScore: 1.0,
    modelVersion:    'system',
  ...
  return { actionId: action.id, invoiceCount: invoices.totalRecords, paymentCount: payments.totalRecords };
}
...
export async function draftInvoiceFromCrmDeal(
  ...
  interface InvoiceDraftAI {
    lineItems:    { description: string; hsnSac: string; quantity: number; rate: number; taxPct: number; amount: number }[];
    subtotal:     number;
...
Return JSON:
{
  ...
  const { data: parsed, meta } = await callGemini<InvoiceDraftAI>(systemPrompt, userPrompt, { parseJson: true });

  const [draft] = await db.insert(aiInvoiceDrafts).values({
    connectionId,
    zohoBooksOrgId:    orgId,
...
export async function calculatePayoutSuggestion(
  ...
  const start   = Date.now();
  const orgId   = getBooksOrgId();
  const tdsRate = params.tdsRate ?? 0.10;

  const gross   = params.brokerageAmount + params.trailAmount + params.incentiveAmount;
  const tds     = Number((gross * tdsRate).toFixed(2));
  const net     = Number((gross - tds).toFixed(2));

  const [payout] = await db.insert(aiPayoutSuggestions).values({
    connectionId,
    zohoBooksOrgId:  orgId,
...
export async function runRevenueReconciliation(
  ...
  const [invoices, payments] = await Promise.all([
    books.getInvoices({ per_page: 200, date_start: startStr, date_end: endStr }),
    books.getPaymentsReceived({ per_page: 200, date_start: startStr, date_end: endStr }),
  ]);
  ...
  const [reconciliation] = await db.insert(aiRevenueReconciliation).values({
    connectionId,
    zohoBooksOrgId:        orgId,
    ...
    deltaPercent,
    hasMismatch,
    mismatchDetails:       hasMismatch ? { expectedRev, receivedRev, delta: outstanding, deltaPercent } : null,
    flaggedItems:          [],
...
export async function getGstSummary(
  ...
  const invoices = await books.getInvoices({ per_page: 200, date_start: startStr, date_end: endStr });

  const gstData = {
    period:       { from: startStr, to: endStr },
    invoiceCount: invoices.totalRecords,
    totalTaxable: invoices.items.reduce((s, i) => s + (i.total || 0), 0),
    paidAmount:   invoices.items.filter(i => i.status === 'paid').reduce((s, i) => s + (i.total || 0), 0),
  };
  ...
  const { data: summaryText, meta } = await callGemini<string>(systemPrompt, userPrompt, { parseJson: false });

  const summary = typeof summaryText === 'string' ? summaryText : JSON.stringify(summaryText);
  ...
  await auditLog({
    ...
    agentType: 'books', agentAction: 'gst_summary',
    entityId: action.id, entityType: 'ai_books_finance_actions',
    outputSummary: summary.substring(0, 200),
    confidenceScore: meta.confidence_score,
    modelVersion: meta.model_version,
    latencyMs: Date.now() - start,
    status: 'success', approvalStatus: 'draft',
  });
...
export async function flagOverdueInvoices(
  connectionId: string,
  adminUserId:  string,
  overdueInvoices?: { invoice_number: string; customer_name: string; balance: number; due_date: string }[],
): Promise<number> {
  const books  = buildBooksService(connectionId);
  const items  = overdueInvoices
...
export async function issueInvoiceToZohoBooks(
  ...
  const [draft] = await db.select().from(aiInvoiceDrafts)
    .where(eq(aiInvoiceDrafts.id, invoiceDraftId)).limit(1);

  if (!draft)               throw new Error('Invoice draft not found');
  if (draft.issuedToZohoBooks) throw new Error('Invoice already issued to Zoho Books');
  if (draft.approvalStatus !== 'approved') throw new Error('Invoice draft not yet approved');

  const lineItems = draft.lineItems as Array<{ description: string; rate: number; quantity: number }>;

  const zohoInvoice = await books.createInvoice({
    customer_name: draft.customerName,
    date:          draft.invoiceDate?.toISOString().split('T')[0] || new Date().toISOString().split('T')[0],
  ...
  await db.update(aiInvoiceDrafts)
    .set({ issuedToZohoBooks: true, zohoBooksInvoiceId: zohoInvoice.invoice_id, updatedAt: new Date() })
    .where(eq(aiInvoiceDrafts.id, invoiceDraftId));

  await auditLog({
    userId:        adminUserId, userRole: 'admin',
    agentType:     'books', agentAction: 'issue_invoice',
    entityId:      invoiceDraftId, entityType: 'ai_invoice_drafts',
    outputSummary: `Invoice issued to Zoho Books: ${zohoInvoice.invoice_id}`,
    latencyMs:     Date.now() - start, status: 'success',
    approvalStatus: 'approved',
    externalApiCalled: true, externalService: 'zoho_books',
