...
export async function syncCrmLeads(
  ...
  for (const lead of leads) {
    const [existing] = await db.select().from(aiCrmLeadActions)
      .where(eq(aiCrmLeadActions.zohoLeadId, lead.id || '')).limit(1);
    ...
    let actionId: string;
    if (existing) {
      await db.update(aiCrmLeadActions)
        .set({ ...baseData, updatedAt: new Date() })
        .where(eq(aiCrmLeadActions.id, existing.id));
      actionId = existing.id;
    } else {
      const [inserted] = await db.insert(aiCrmLeadActions)
        .values({ ...baseData, approvalStatus: 'draft', modelVersion: 'gemini-2.0-flash' })
        .returning();
      ...
      try {
        ...
        console.error(`[CRM Agent] Intelligence generation failed for lead ${actionId}:`, err);
      }
    }
  }
  ...
  await auditLog({
    ...
    agentType: 'crm', agentAction: 'sync_crm_leads',
    ...
    externalApiCalled: true, externalService: 'zoho_crm',
...
export async function generateLeadIntelligence(
  ...
  adminUserId: string,
): Promise<{ intentScore: number; nextBestAction: string; confidenceScore: number }> {
  const start = Date.now();
...
Return:
{
  ...
  const { data: parsed, meta } = await callGemini<LeadIntelligenceAI>(systemPrompt, userPrompt, { parseJson: true });
  ...
  await auditLog({
    userId: adminUserId, userRole: 'admin',
    agentType: 'crm', agentAction: 'lead_intelligence',
    entityId: crmLeadActionId, entityType: 'ai_crm_lead_actions',
  ...
  return { intentScore: parsed.intentScore, nextBestAction: parsed.nextBestAction, confidenceScore: meta.confidence_score };
}
...
export async function updateLeadStage(
  ...
  const [record] = await db.select().from(aiCrmLeadActions)
    .where(eq(aiCrmLeadActions.id, crmLeadActionId)).limit(1);

  if (!record)            throw new Error('CRM lead action not found');
  if (!record.zohoLeadId) throw new Error('Lead has no Zoho Lead ID — cannot update stage');
  ...
  await auditLog({
    userId: adminUserId, userRole: 'admin',
    agentType: 'crm', agentAction: 'update_lead_stage',
    entityId: crmLeadActionId, entityType: 'ai_crm_lead_actions',
...
export async function syncDeals(
  ...
  await auditLog({
    userId: adminUserId, userRole: 'admin',
    agentType: 'crm', agentAction: 'sync_deals',
