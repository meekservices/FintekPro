...
type EmailCategory =
  ...
  | 'loan_enquiry' | 'mf_enquiry' | 'pms_aif_enquiry' | 'reit_invit'
  | 'compliance' | 'support' | 'other';
...
export async function syncAndClassifyEmails(
  ...
  adminUserId:  string,
  limit = 50,
): Promise<{ classified: number; errors: number; results: Array<{ id: string }> }> {
  ...
  try {
    ...
    await auditLog({
      userId: adminUserId, agentType: 'mail', agentAction: 'fetch_emails',
      status: 'failure', errorMessage: err.message, retryable: true,
      externalApiCalled: true, externalService: 'zoho_mail',
    });
    throw err;
  }
  ...
  for (const msg of messages) {
    try {
      const content = await fetchEmailContent(accessToken, accountId, msg.messageId);
...
`.trim();
      ...
      const { data, meta } = await callGemini<EmailClassificationResult>(
        CLASSIFY_SYSTEM, userPrompt,
      );

      const auditId = randomUUID();

      const [record] = await db.insert(aiEmailClassifications).values({
        ...
        subject:         msg.subject,
        receivedAt:      msg.receivedTime ? new Date(msg.receivedTime) : new Date(),
      ...
      if (data.category === 'complaint' && (data.urgency === 'critical' || data.urgency === 'high')) {
        await auditLog({
          userId: adminUserId, agentType: 'mail', agentAction: 'complaint_flagged',
          entityId: record.id, entityType: 'ai_email_classifications',
          outputSummary: `Complaint flagged: ${data.intent}`,
          confidenceScore: meta.confidence_score, approvalStatus: 'draft',
          externalApiCalled: true, externalService: 'zoho_mail',
      ...
      await auditLog({
        userId: adminUserId, agentType: 'mail', agentAction: 'email_classified',
        entityId: record.id, entityType: 'ai_email_classifications',
        inputContext:  { subject: msg.subject, from: msg.fromAddress },
        outputSummary: `${data.category} | ${data.urgency} | ${data.intent}`,
        confidenceScore: meta.confidence_score,
        approvalStatus: 'draft', approvingAdmin: undefined,
        externalApiCalled: true, externalService: 'zoho_mail',
        externalCallStatus: 'success',
      ...
      errors++;
      console.error('[MailAgent] Failed to classify email', msg.messageId, err.message);
    }
  }

  logCopilotEvent('MAIL_AGENT_SYNC', adminUserId, Date.now() - startMs, 'success', {
    classified, errors, total: messages.length,
  });
...
`.trim();
  ...
  const { data } = await callGemini<{ draftReply: string }>(CLASSIFY_SYSTEM, userPrompt);

  await db.update(aiEmailClassifications)
    .set({ draftReply: data.draftReply, draftReplyStatus: 'draft', updatedAt: new Date() })
    .where((t: any) => t.id.eq(classificationId));

  await auditLog({
    userId: adminUserId, agentType: 'mail', agentAction: 'draft_reply_regenerated',
    entityId: classificationId, entityType: 'ai_email_classifications',
    approvalStatus: 'draft',
  });
