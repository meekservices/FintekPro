...
type TaskStatus = 'draft' | 'approved' | 'assigned' | 'in_progress' | 'completed' | 'escalated' | 'closed';
...
async function buildContextFromSource(
  ...
  if (source === 'email') {
    const [rec] = await db.select().from(aiEmailClassifications)
      .where(eq(aiEmailClassifications.id, sourceId)).limit(1);
...
Action required: ${rec.actionRequired}`;
  ...
  if (source === 'desk') {
    const [rec] = await db.select().from(aiDeskTicketActions)
      .where(eq(aiDeskTicketActions.id, sourceId)).limit(1);
...
Escalation: ${rec.escalationReason ?? 'none'}`;
  ...
  if (source === 'crm') {
    const [rec] = await db.select().from(aiCrmLeadActions)
      .where(eq(aiCrmLeadActions.id, sourceId)).limit(1);
...
export async function createTaskFromSource(params: {
  ...
  const {
    source, sourceId, adminUserId, adminPrompt,
    linkedEmailId, linkedCrmLeadId, linkedTicketId,
    linkedProposalId, linkedMeetingId,
  } = params;
  ...
  const [task] = await db.insert(aiAdminTasks).values({
    title:            data.title,
    ...
    dueDate,
    linkedEmailId:    linkedEmailId   ?? (source === 'email' ? sourceId : undefined),
    linkedCrmLeadId:  linkedCrmLeadId ?? (source === 'crm'   ? sourceId : undefined),
    linkedTicketId:   linkedTicketId  ?? (source === 'desk'   ? sourceId : undefined),
    linkedProposalId,
    linkedMeetingId,
  ...
  await auditLog({
    userId: adminUserId, agentType: 'task', agentAction: 'task_created',
    entityId: task.id, entityType: 'ai_admin_tasks',
  ...
  logCopilotEvent('TASK_AGENT_CREATE', adminUserId, Date.now() - startMs, 'success', {
    taskId: task.id, source, priority: data.priority,
  });
...
export async function updateTaskStatus(
  ...
  const updates: Partial<typeof aiAdminTasks.$inferInsert> = {
    ...
    updatedAt: new Date(),
    ...(notes ? { notes } : {}),
    ...(status === 'completed' ? { completedAt: new Date(), completedBy: adminUserId } : {}),
    ...(status === 'approved'  ? { approvalStatus: 'approved', approvedBy: adminUserId, approvedAt: new Date() } : {}),
  };

  await db.update(aiAdminTasks)
    .set(updates)
    .where(eq(aiAdminTasks.id, taskId));

  await auditLog({
    userId: adminUserId, agentType: 'task', agentAction: `task_status_${status}`,
    entityId: taskId, entityType: 'ai_admin_tasks',
    approvalStatus: status, approvingAdmin: adminUserId, source: 'admin',
  });
}
