	...
	elss: [
		{
			...
			maxDays: 1095,
			exitLoadPercent: 0,
			description: "Lock-in period - 3 years (no exit load but cannot redeem)",
		},
		{
			minDays: 1096,
			maxDays: null,
	...
	default: [
		{
			...
			maxDays: 365,
			exitLoadPercent: 1.0,
			description: "1% if redeemed within 1 year (default)",
		},
		{
			minDays: 366,
			maxDays: null,
...
class ExitLoadService {
	...
	async getExitLoad(params: ExitLoadLookupParams): Promise<ExitLoadResult> {
		...
		if (cached) {
			...
			if (dbResult.length > 0) {
				...
				const fundCategory = await this.detectFundCategory(
					isin,
					schemeCode,
					category,
					schemeName,
		...
		const daysToZero = this.calculateDaysToZeroExitLoad(
			exitLoadRules,
			holdingDays,
		);
	 ...
	 */
	async getExitLoadTimeline(
		isin?: string,
		schemeCode?: string,
	): Promise<ExitLoadRule[]> {
		const dbResult = await this.lookupFromDatabase(isin, schemeCode);
		if (dbResult.length > 0) {
			return dbResult;
		}
	...
	async calculateExitLoadAmount(
		...
		return {
			...
			netRedemption:
				Math.round((params.redemptionAmount - result.exitLoadAmount) * 100) /
				100,
	...
	async getDataCoverageStats(): Promise<{
		...
		return {
			...
			coveragePercent:
				totalFunds > 0
					? Math.round((totalWithExitLoad / totalFunds) * 100 * 100) / 100
					: 0,
	...
	private async lookupFromDatabase(
		...
		try {
			...
			return rows.map((r) => ({
				minDays: r.minDays,
				maxDays: r.maxDays,
				exitLoadPercent: Number.parseFloat(r.exitLoadPercent || "0"),
	...
	private async detectFundCategory(
		...
		if (schemeCode || isin) {
			try {
				...
				if (fund?.category) {
					return this.normalizeFundCategory(
						fund.category,
						fund.schemeName || schemeName,
					);
				}
			} catch (error) {
	...
	private normalizeFundCategory(category: string, schemeName?: string): string {
		...
		if (
			lowerCategory.includes("elss") ||
			lowerName.includes("elss") ||
			lowerName.includes("tax saver")
		) {
			return "elss";
		}
		if (
			lowerCategory.includes("index") ||
		...
		if (
			lowerCategory.includes("sectoral") ||
			lowerCategory.includes("thematic") ||
			lowerName.includes("sectoral")
		) {
			return "sectoral";
		}
		...
		if (
			...
			lowerCategory.includes("flexi cap") ||
	...
	private calculateDaysToZeroExitLoad(
		rules: ExitLoadRule[],
	...
	async calculateLotWiseExitLoad(
		...
		for (const lot of activeLots) {
			const units = Number.parseFloat(lot.remainingUnits ?? lot.units ?? "0");

			if (Number.isNaN(units) || units <= 0) continue;
	...
	async getExitLoadCalendar(
		...
		Array<{
			date: string;
			unitsFreeFromExitLoad: number;
			cumulativeFreedUnits: number;
		}>
	> {
		...
		for (const lot of result.lots) {
			if (lot.daysToZeroExitLoad && lot.daysToZeroExitLoad > 0) {
				...
				lotsByExitFreeDate.set(
					dateStr,
					(lotsByExitFreeDate.get(dateStr) || 0) + lot.units,
				);
			}
		}
		...
		for (const date of sortedDates) {
			...
			calendar.push({
				date,
				unitsFreeFromExitLoad: unitsFree,
				cumulativeFreedUnits: cumulative,
			});
		}
