...
const RP_ID = process.env.NODE_ENV === "production" ? "fintekpro.com" : "localhost";
...
export function calculateRiskScore(ctx: RiskContext): RiskResult {
  ...
  if (ctx.newDevice) { factors.new_device = 30; score += 30; }
  if (ctx.geoMismatch) { factors.geo_mismatch = 25; score += 25; }
  if ((ctx.txnAmount ?? 0) > 500000) { factors.high_txn_amount = 40; score += 40; }
  if ((ctx.inactiveDays ?? 0) > 30) { factors.inactive_days = 20; score += 20; }
  if ((ctx.failedAttempts ?? 0) > 3) { factors.failed_attempts = 50; score += 50; }
...
export const webauthnService = {
  async getRegistrationOptions(userId: string, userEmail: string, userName: string) {
  ...
  async verifyRegistration(
    ...
    if (!challengeRow) {
      await this.auditLog(userId, "registration_failure", null, ipAddress, userAgent, false, "No valid challenge found");
    ...
    try {
      ...
      await this.auditLog(userId, "registration_failure", null, ipAddress, userAgent, false, String(err));
      throw err;
    } finally {
    ...
    if (!verification.verified || !verification.registrationInfo) {
      await this.auditLog(userId, "registration_failure", null, ipAddress, userAgent, false, "Verification failed");
    ...
    await this.auditLog(userId, "registration_success", credentialId, ipAddress, userAgent, true, null, credentialDeviceType ?? undefined);
    return saved;
  },
  ...
  async verifyAuthentication(
    ...
    if (!challengeRow) {
      await this.auditLog(userId, "auth_failure", null, ipAddress, userAgent, false, "Challenge expired or not found");
      throw new Error("Challenge expired or not found");
    }
    ...
    if (!credential) {
      await this.auditLog(userId, "auth_failure", credentialIdB64, ipAddress, userAgent, false, "Credential not found");
    ...
    try {
      ...
      await this.auditLog(userId, "auth_failure", credentialIdB64, ipAddress, userAgent, false, String(err));
      throw err;
    } finally {
      await db.delete(webauthnChallenges).where(eq(webauthnChallenges.id, challengeRow.id));
    }
    ...
    if (!verification.verified) {
      await this.auditLog(userId, "auth_failure", credentialIdB64, ipAddress, userAgent, false, "Signature verification failed");
    ...
    if (newCounter <= credential.signCount && credential.signCount > 0) {
      await this.auditLog(userId, "replay_blocked", credentialIdB64, ipAddress, userAgent, false, "Replay attack: counter not incremented");
    ...
    await this.auditLog(
      userId, "auth_success", credentialIdB64, ipAddress, userAgent, true, null,
      credential.deviceType ?? undefined, risk.score, risk.factors, risk.stepUpRequired
    );
  ...
  async renameCredential(userId: string, credentialDbId: string, deviceName: string) {
  ...
  async deleteCredential(userId: string, credentialDbId: string, ipAddress?: string) {
    ...
    if (deleted) {
      await this.auditLog(userId, "credential_deleted", deleted.credentialId, ipAddress, undefined, true, null);
    }
    return deleted;
  },
...
export function requireBiometricStepUp(minRiskLevel: "medium" | "high" | "very_high" = "medium") {
  return async (req: any, res: any, next: any) => {
    ...
    const isRecentlyVerified = sessionFlag && Date.now() - new Date(sessionFlag).getTime() < 15 * 60 * 1000;
