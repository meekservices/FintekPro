...
class ExchangeStockService {
	private nseProgress: SyncProgress = {
		exchange: "NSE",
		status: "idle",
		total: 0,
		processed: 0,
	...
	private bseProgress: SyncProgress = {
		exchange: "BSE",
		status: "idle",
		total: 0,
		processed: 0,
	...
	async getAllBSESymbols(): Promise<BseScripData[]> {
		...
		try {
			...
			const scrips: BseScripData[] = response.data
				...
				.map((item: any) => ({
					symbol: (item.scrip_id || item.SCRIP_NAME || "")
						.replace(/\s+/g, "")
						.toUpperCase(),
	...
	private getExtendedBSEStocks(): BseScripData[] {
		return [
			...
			{ symbol: "UCOBANK", bseCode: "532505", companyName: "UCO Bank" },
			{
				symbol: "J&KBANK",
				bseCode: "532209",
				companyName: "Jammu & Kashmir Bank Ltd.",
			},
			{
				symbol: "SOUTHBANK",
				bseCode: "532218",
			...
			{ symbol: "CUB", bseCode: "532210", companyName: "City Union Bank Ltd." },
			{
				symbol: "TMB",
				bseCode: "543483",
				companyName: "Tamilnad Mercantile Bank Ltd.",
			},
			{
				symbol: "EQUITASBNK",
				bseCode: "543243",
			...
			},
			{
				symbol: "ESAFSFB",
				bseCode: "543561",
				companyName: "ESAF Small Finance Bank Ltd.",
			},
			{
				symbol: "ADANIPOWER",
				bseCode: "533096",
			...
			},
			{
				symbol: "LALPATHLAB",
				bseCode: "539524",
				companyName: "Dr. Lal PathLabs Ltd.",
			},
			{
				symbol: "MAXHEALTH",
				bseCode: "543220",
			...
			},
			{
				symbol: "PHOENIXLTD",
				bseCode: "503100",
				companyName: "The Phoenix Mills Ltd.",
			},
			{
				symbol: "UNITDSPR",
				bseCode: "543340",
			...
			},
			{
				symbol: "DEVYANI",
				bseCode: "543330",
				companyName: "Devyani International Ltd.",
			},
			{
				symbol: "SAPPHIRE",
				bseCode: "543331",
				companyName: "Sapphire Foods India Ltd.",
			},
			{
				symbol: "RAJESHEXPO",
				bseCode: "531500",
			...
			},
			{
				symbol: "METROBRAND",
				bseCode: "543426",
				companyName: "Metro Brands Ltd.",
			},
			{
				symbol: "CAMPUS",
				bseCode: "543523",
				companyName: "Campus Activewear Ltd.",
			},
			{
				symbol: "AIAENG",
				bseCode: "532683",
			...
			},
			{
				symbol: "GREENPANEL",
				bseCode: "542827",
				companyName: "Greenpanel Industries Ltd.",
			},
			{
				symbol: "DEEPAKFERT",
				bseCode: "500645",
				companyName: "Deepak Fertilizers and Petrochemicals Corporation Ltd.",
			},
			{
				symbol: "GNFC",
				bseCode: "500670",
			...
			},
			{
				symbol: "CHAMBAL",
				bseCode: "500085",
				companyName: "Chambal Fertilizers and Chemicals Ltd.",
			},
			...
			},
			{
				symbol: "RENUKA",
				bseCode: "532670",
				companyName: "Shree Renuka Sugars Ltd.",
			},
			{
				symbol: "EIDPARRY",
				bseCode: "500125",
			...
			},
			{
				symbol: "NATIONALUM",
				bseCode: "532234",
				companyName: "National Aluminium Company Ltd.",
			},
			{ symbol: "MOIL", bseCode: "533286", companyName: "MOIL Ltd." },
			{
				symbol: "GMRINFRA",
				bseCode: "532754",
				companyName: "GMR Airports Infrastructure Ltd.",
			},
			{
				symbol: "IRCON",
				bseCode: "541956",
	...
	async getBSEStockDetails(
		bseCode: string,
		symbol?: string,
	): Promise<ExchangeStockData | null> {
		try {
	...
	async syncNSEStocks(
		options: { limit?: number; topOnly?: boolean } = {},
	): Promise<SyncProgress> {
		if (
			this.nseProgress.status !== "idle" &&
	...
	async syncBSEStocks(
		...
		try {
			...
			for (let i = 0; i < targetSymbols.length; i += batchSize) {
				...
				await Promise.all(
					batch.map(async (scrip) => {
						try {
							const stockData = await this.getBSEStockDetails(
								scrip.bseCode,
								scrip.symbol,
							);
							if (stockData) {
	...
	async lookupAndAddStock(
		query: string,
	): Promise<{
		found: boolean;
		action?: "added" | "updated";
		stock?: ExchangeStockData;
	}> {
		const symbol = query
			.trim()
			.toUpperCase()
		...
		try {
			...
			if (action === "added") {
				await db.execute(
					sql`UPDATE listed_stocks SET is_published = true WHERE symbol = ${stockData.symbol}`,
				);
			}
