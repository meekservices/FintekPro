...
export class CashfreePANService {
	...
	static async verifyPAN(
		...
		try {
			...
			const response = await axios.post(
				`${CashfreePANService.getBaseUrl()}/pan-lite`,
				{
					verification_id: verificationId,
					pan: pan.toUpperCase(),
					name: name.trim(),
				},
				{ headers: CashfreePANService.getHeaders() },
			);
			...
			if (isValid) {
				...
				return {
					...
					data: {
						...
						registeredName:
							d.registered_name || d.name_pan_card || d.name || "",
			...
			return {
				...
				message:
					d?.message ||
					"PAN verification failed. Please check the PAN and name.",
				verified: false,
			};
		} catch (error: any) {
			...
			const isCredentialMismatch =
				statusCode === 401 ||
				/prod environment|sandbox environment|invalid client|client secret/i.test(
					apiMessage,
				);
			...
			if (isCredentialMismatch) {
				console.error(
					...
					`URL: ${CashfreePANService.getBaseUrl()}/pan-lite`,
				...
				return {
					success: false,
					message:
						"PAN verification is temporarily unavailable. Please try again later.",
					verified: false,
				};
			}
	...
	static async verifyCompanyPAN(
		...
		const result = await CashfreePANService.verifyPAN(pan, companyName);
	...
	static async getVerificationStatus(
		...
		try {
			const response = await axios.get(
				`${CashfreePANService.getBaseUrl()}/pan/${referenceId}`,
				{ headers: CashfreePANService.getHeaders() },
			);
			...
			console.error(
				"Cashfree PAN status retrieval error:",
