...
export class MutualFundMetricsService {
	...
	static getInstance(): MutualFundMetricsService {
		if (!MutualFundMetricsService.instance) {
			MutualFundMetricsService.instance = new MutualFundMetricsService();
		}
		return MutualFundMetricsService.instance;
	}
	...
	calculateCAGR(startNav: number, endNav: number, years: number): number {
		...
		const cagr = (endNav / startNav) ** (1 / years) - 1;
	...
	private async getNavForDate(
		...
		if (localResult.length > 0) {
			return {
				nav: Number.parseFloat(localResult[0].nav as string),
	...
	async fetchAndStoreNavHistory(schemeCode: string): Promise<boolean> {
		try {
			const result =
				await this.historicalNavService.fetchAndStoreMutualFundHistory(
					schemeCode,
				);
			return result.success && result.recordsStored > 0;
		} catch (error) {
			console.error(
				`[MFMetrics] Error fetching NAV history for ${schemeCode}:`,
				error,
			);
			return false;
		}
	 ...
	 */
	async calculateReturnsForScheme(
		schemeCode: string,
	): Promise<CalculatedReturns> {
		const now = new Date();
		...
		if (currentNavResult && nav1yResult) {
			result.returns1y = this.calculateCAGR(
				nav1yResult.nav,
				currentNavResult.nav,
				1,
			);
			await this.logAudit(
				schemeCode,
				...
				currentNavResult.nav,
				nav1yResult.date,
				currentNavResult.date,
			);
		}

		if (currentNavResult && nav3yResult) {
			result.returns3y = this.calculateCAGR(
				nav3yResult.nav,
				currentNavResult.nav,
				3,
			);
		...
		if (currentNavResult && nav5yResult) {
			result.returns5y = this.calculateCAGR(
				nav5yResult.nav,
				currentNavResult.nav,
				5,
			);
	...
	private async logAudit(
		...
		const years =
			calculationType === "1Y" ? 1 : calculationType === "3Y" ? 3 : 5;
	...
	async updateSchemeReturns(schemeCode: string): Promise<boolean> {
      ...
      `);
			...
			console.error(
				`[MFMetrics] Error updating returns for ${schemeCode}:`,
				error,
			);
			return false;
		}
	...
	async batchUpdateReturns(
		...
		console.log(
			`[MFMetrics] Starting batch update for ${schemeCodes.length} schemes...`,
		);
		...
		for (let i = 0; i < schemeCodes.length; i += batchSize) {
			...
			console.log(
				`[MFMetrics] Progress: ${processed}/${schemeCodes.length} (${successfulUpdates} updated, ${skippedNoData} skipped, ${failedUpdates} failed)`,
			);
	...
	async fetchExpenseRatioAndAum(schemeCode: string): Promise<{
		...
		try {
			...
			console.error(
				`[MFMetrics] Error fetching expense/AUM for ${schemeCode}:`,
				error,
			);
			return { expenseRatio: null, aum: null };
		}
