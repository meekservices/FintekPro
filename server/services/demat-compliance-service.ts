...
class DematComplianceService {
	...
	async validateDP(dpId: string): Promise<DPValidationResult> {
		...
		if (!isNSDL && !isCDSL) {
			...
			return {
				...
				status: "cancelled",
				errors: [
					"Invalid DP ID format. NSDL format: IN + 6 digits, CDSL format: 8 digits",
				],
			};
		}
		...
		try {
			...
			if (dp.length === 0) {
				...
				console.error(
					`[Demat Compliance] DP ${dpId} not found in SEBI registry database. Validation FAILED.`,
				);
			...
			if (dpRecord.status !== "active") {
				...
				return {
					...
					errors: [
						...
						dpRecord.statusReason ||
							"Please contact your DP for more information.",
					],
				};
			}
			...
			console.error(
				`[Demat Compliance] Database error during DP validation:`,
				error,
			);
	...
	async verifyBeneficialOwner(
		...
		const kycVerified =
			user.panVerifiedViaSmartKyc && user.aadhaarVerifiedViaSmartKyc;
	...
	async authorizeTransaction(
		...
		if (totalHolding < quantity) {
			restrictions.push(
				`Insufficient holdings: Available ${totalHolding}, Required ${quantity}`,
			);
		}
	...
	async verifyHoldings(
		...
		const freeBalance =
			totalQuantity - pledgedQuantity - lockedQuantity - pendingTransfer;
	...
	async validateNomineeCompliance(userId: string): Promise<{
		...
		if (hasNominee && user.nomineeDetails) {
			try {
				const parsed =
					typeof user.nomineeDetails === "string"
				...
				nomineeDetails = [
					{
						...
						relationship:
							user.nomineeRelation || parsed.relationship || "Not specified",
	...
	async generateComplianceReport(userId: string): Promise<{
		...
		const accountLinked =
			!!(user?.nsdlDpId && user?.nsdlClientId) ||
			!!(user?.cdslBoId && user?.cdslDpId);
		if (!accountLinked) {
		...
		const totalValue = holdings.reduce(
			(sum, h) =>
				sum + Number.parseFloat(h.currentValue || h.totalInvestedAmount || "0"),
			0,
		);
