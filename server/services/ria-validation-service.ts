...
class RIAValidationService {
	private readonly SEBI_INTERMEDIARY_URL =
		"https://www.sebi.gov.in/sebiweb/other/OtherAction.do?doRecognisedFpi=yes&intmId=13";
	private readonly REGULATORY_REFERENCE =
		"SEBI (Investment Advisers) Regulations 2013";
	private registrationCache: Map<
		string,
		{ registration: RIARegistration; fetchedAt: Date }
	> = new Map();
	...
	private mockRIARegistry: Map<string, RIARegistration> = new Map([
		...
		[
			"INA200011111",
			{
				...
				sebiOrdersAgainst: 2,
				warnings: [
					"SEBI suspension order vide Order No. WTM/XX/2023",
	...
	private async fetchRegistrationFromSEBI(
		registrationNumber: string,
	): Promise<RIARegistration | null> {
		await new Promise((resolve) => setTimeout(resolve, 100));
		return this.mockRIARegistry.get(registrationNumber) || null;
	}

	private buildValidationResult(
		registration: RIARegistration | null,
		registrationNumber: string,
	): RIAValidationResult {
		const now = new Date();
		...
		if (!registration) {
			return {
				...
				adviceScopes: [],
				warnings: [
					"RIA registration not found in SEBI database. Verify registration number.",
				],
		...
		if (registration.scopeOfAdvice.mutualFunds)
			adviceScopes.push("Mutual Funds");
		if (registration.scopeOfAdvice.insurance) adviceScopes.push("Insurance");
		if (registration.scopeOfAdvice.derivatives)
			adviceScopes.push("Derivatives");
		if (registration.scopeOfAdvice.commodities)
			adviceScopes.push("Commodities");
		...
		if (registration.scopeOfAdvice.globalSecurities)
			adviceScopes.push("Global Securities");

		const warnings: string[] = [...registration.warnings];

		if (registration.registrationStatus === "pending_renewal") {
			warnings.push(
				"Registration renewal is pending. Advise client accordingly.",
			);
		}

		if (registration.sebiOrdersAgainst > 0) {
			warnings.push(
				`${registration.sebiOrdersAgainst} SEBI order(s) on record. Review before proceeding.`,
			);
		}

		const daysUntilExpiry = Math.ceil(
			(registration.validUntil.getTime() - now.getTime()) /
				(1000 * 60 * 60 * 24),
		);
		if (daysUntilExpiry > 0 && daysUntilExpiry <= 90) {
			warnings.push(
				`Registration expires in ${daysUntilExpiry} days. Initiate renewal process.`,
			);
		}
	...
	async checkAdviceEligibility(
		...
		if (!validation.isValid) {
			return {
				eligible: false,
				reason: `RIA registration is ${validation.status}. Cannot provide investment advice.`,
			};
		}
		...
		return {
			eligible: true,
			reason: "RIA is authorized to provide this type of advice.",
		};
	}
	...
	async getRIADetails(
		...
		const registration =
			await this.fetchRegistrationFromSEBI(registrationNumber);
		if (registration) {
			this.registrationCache.set(registrationNumber, {
				registration,
				fetchedAt: new Date(),
