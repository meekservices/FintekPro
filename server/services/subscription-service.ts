...
export function isActivePlan(user: { planTier?: string | null; planExpiresAt?: Date | null }): boolean {
  if (!user.planTier || user.planTier === 'free') return true; // free never expires
  if (!user.planExpiresAt) return false;
  return new Date(user.planExpiresAt) > new Date();
}

export function getEffectiveTier(user: { planTier?: string | null; planExpiresAt?: Date | null }): PlanTier {
...
export function calcFxRevenue(amountInr: number, tier: PlanTier): number {
  ...
  return parseFloat(((amountInr * spread) / 100).toFixed(2));
}
...
export async function getUserSubscription(userId: string) {
  const [sub] = await db
    ...
    .where(and(eq(platformSubscriptions.userId, userId), eq(platformSubscriptions.status, 'active')))
...
export async function activateSubscription(
  ...
  if (!pending) {
    throw new Error(`No pending subscription found for order ${cashfreeOrderId}`);
  }
  ...
  logger.info(`[Subscription] Activated ${tier}/${cycle} for user ${userId}, expires ${expiresAt.toISOString()}`);
}
