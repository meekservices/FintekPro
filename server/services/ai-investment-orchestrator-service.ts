...
class AIInvestmentOrchestratorService {
	...
	private productCache = new Map<
		UnifiedProductType,
		{ products: InvestmentProduct[]; timestamp: Date }
	>();
	...
	constructor() {
		...
		console.log(
			`✅ AI Investment Orchestrator initialized via Unified Engine (primary: ${status.primary})`,
		);
	}
	...
	async generateRecommendationBasket(
		...
		const eligibleTypes =
			productTypes || this.getEligibleProductTypes(clientProfile);

		const allProducts = await this.fetchAllProducts(eligibleTypes);

		const filteredProducts = this.applyEligibilityFilters(
			allProducts,
			clientProfile,
		);

		const evaluatedProducts = this.evaluateProducts(
			filteredProducts,
			clientProfile,
			context,
		);
		...
		urcaeTarget.target_allocation.forEach((a: any) => {
			...
			if (a.asset_class === "equity_largecap")
				structuralTarget.STOCK = (structuralTarget.STOCK || 0) + a.weight * 100;
			if (a.asset_class === "equity_midcap")
				structuralTarget.MF = (structuralTarget.MF || 0) + a.weight * 100;
			if (a.asset_class === "bonds")
				structuralTarget.BOND = (structuralTarget.BOND || 0) + a.weight * 100;
			if (a.asset_class === "cash")
				structuralTarget.MLD = (structuralTarget.MLD || 0) + a.weight * 100;
		});

		const basket = await this.optimizeBasket(
			evaluatedProducts,
			clientProfile,
			investmentAmount,
			structuralTarget,
		...
		if (aageCheck.decision === "BLOCK") {
			throw new Error(
				`Governance Intercept: ${aageCheck.violations.map((v) => v.message).join(", ")}`,
			);
		}
	...
	private getEligibleProductTypes(
		clientProfile: ClientProfile,
	): UnifiedProductType[] {
		const eligible: UnifiedProductType[] = [];

		for (const [type, allowedCategories] of Object.entries(
			PRODUCT_CLIENT_REQUIREMENTS,
		)) {
	...
	private async fetchAllProducts(
		...
		const fetchPromises = productTypes.map(async (type) => {
			if (this.config.cacheEnabled) {
				...
				if (
					cached &&
					Date.now() - cached.timestamp.getTime() < this.PRODUCT_CACHE_TTL
				) {
	...
	private async fetchProductsByType(
		...
		try {
			switch (type) {
				...
				case "MF": {
					const fundsWithMetrics = await db
						.select({
							id: mutualFunds.id,
	...
	private applyEligibilityFilters(
		...
		return products.filter((product) => {
			if (
				!PRODUCT_CLIENT_REQUIREMENTS[product.product_type]?.includes(
			...
			if (
				RISK_HIERARCHY[product.risk_level] >
				RISK_HIERARCHY[clientProfile.risk_category] + 1
			) {
				return false;
			}

			if (
				HORIZON_HIERARCHY[product.investment_horizon] >
				HORIZON_HIERARCHY[clientProfile.investment_horizon] + 1
			) {
				return false;
			}

			if (
				clientProfile.liquidity_needs === "high" &&
				LIQUIDITY_HIERARCHY[product.liquidity] < 3
			) {
				return false;
			}
	...
	private evaluateProducts(
		...
		return products.map((product) => {
			const { score, breakdown } = this.calculateProductScore(
				product,
				clientProfile,
				marketContext,
			);
			...
			return {
				...
				rationale_inputs: {
					...
					opportunity_factors: this.getOpportunityFactors(
						product,
						marketContext,
					),
				},
	...
	private calculateProductScore(
		...
		const riskDiff = Math.abs(
			RISK_HIERARCHY[product.risk_level] -
				RISK_HIERARCHY[clientProfile.risk_category],
		);
		breakdown.risk_alignment = Math.max(0, 25 - riskDiff * 10);

		const horizonDiff = Math.abs(
			HORIZON_HIERARCHY[product.investment_horizon] -
				HORIZON_HIERARCHY[clientProfile.investment_horizon],
		);
		breakdown.horizon_alignment = Math.max(0, 20 - horizonDiff * 5);

		const expectedReturn =
			(product.expected_return_band.min + product.expected_return_band.max) / 2;
		...
		if (product.sector && marketContext.sector_momentum[product.sector]) {
			breakdown.sector_momentum = Math.min(
				10,
				marketContext.sector_momentum[product.sector] * 2,
			);
		} else {
	...
	private getRatingScore(rating: string): number {
		...
		if (r.includes("5 STAR") || r === "AAA" || r.includes("STRONG BUY"))
			return 15;
	...
	private getOpportunityFactors(
		...
		if (
			marketContext.market_regime === "bull" &&
		...
		if (
			marketContext.interest_rate_outlook === "falling" &&
	...
	private async optimizeBasket(
		...
		const targetAllocation =
			targetAllocationOverride || this.getTargetAllocation(clientProfile);
		...
		for (const product of evaluatedProducts) {
			...
			basketItems.push({
				...
				action: "buy",
				priority:
					product.suitability_score > 70
						? "must_have"
						: product.suitability_score > 50
							? "recommended"
							: "optional",
			});
		...
		const totalAllocation = basketItems.reduce(
		...
		if (totalAllocation < 100 && basketItems.length > 0) {
			...
			basketItems.forEach((item) => {
				...
				item.suggested_amount =
					(investmentAmount * item.allocation_percent) / 100;
			});
		}
		...
		basketItems.forEach((item) => {
			...
			assetAllocation[type] =
				(assetAllocation[type] || 0) + item.allocation_percent;
		});
		...
		return {
			...
			portfolio_rationale: this.generatePortfolioRationale(
				basketItems,
				clientProfile,
			),
	...
	private getTargetAllocation(
		clientProfile: ClientProfile,
	): Record<UnifiedProductType, number> {
	...
	private generateRuleBasedRationale(
		product: InvestmentProduct,
		clientProfile: ClientProfile,
	): string {
		const parts: string[] = [];
		...
		parts.push(
			`${product.name} is a ${product.risk_level} risk ${product.product_type.toLowerCase()} investment.`,
		);
		...
		parts.push(
			`Suitable for ${product.investment_horizon.replace("_", " ")} investment horizon.`,
		);
	...
	private calculateWeightedReturn(items: BasketItem[]): number {
		...
		items.forEach((item) => {
			const avgReturn =
				(item.product.expected_return_band.min +
					item.product.expected_return_band.max) /
				2;
	...
	private calculateWeightedRisk(items: BasketItem[]): number {
		...
		items.forEach((item) => {
			weightedRisk +=
				RISK_HIERARCHY[item.product.risk_level] * item.allocation_percent;
			totalWeight += item.allocation_percent;
		});
	...
	private calculateDiversificationScore(items: BasketItem[]): number {
		...
		const sectors = new Set(
			items.filter((i) => i.product.sector).map((i) => i.product.sector),
		);
	...
	private generatePortfolioRationale(
		...
		return (
			`This diversified portfolio includes ${types.length} asset classes (${types.join(", ")}) ` +
	...
	private analyzePortfolioRisk(
		items: BasketItem[],
	): RecommendationBasket["risk_analysis"] {
		const avgLiquidity =
			items.reduce(
				(sum, i) => sum + LIQUIDITY_HIERARCHY[i.product.liquidity],
				0,
			) / items.length;
		const avgRisk =
			items.reduce((sum, i) => sum + RISK_HIERARCHY[i.product.risk_level], 0) /
			items.length;
		const maxAllocation = Math.max(...items.map((i) => i.allocation_percent));

		return {
			concentration_risk:
				maxAllocation > 30 ? "high" : maxAllocation > 20 ? "medium" : "low",
			liquidity_risk:
				avgLiquidity < 2 ? "high" : avgLiquidity < 3 ? "medium" : "low",
			market_risk: avgRisk > 3 ? "high" : avgRisk > 2 ? "medium" : "low",
			credit_risk: items.some(
				(i) =>
					i.product.product_type === "BOND" &&
					!i.product.rating?.includes("AA"),
			)
				? "medium"
				: "low",
		};
	...
	private checkCompliance(
		...
		items.forEach((item) => {
			if (
				item.product.product_type === "UNLISTED" &&
				clientProfile.client_category === "retail"
			) {
				flags.push(`${item.product.name}: Not suitable for retail investors`);
			}

			if (
				item.product.product_type === "AIF" &&
				!["sHNI", "bHNI"].includes(clientProfile.client_category)
			) {
	...
	private async enrichWithAIRationales(
		...
		for (const item of basket.products) {
			...
			try {
				const cached = await getCachedRationale(
			...
			const memCacheKey = this.getRationaleCacheKey(
				item.product,
				clientProfile,
			);
			...
			if (memCached) {
				const ttl =
					AI_RATIONALE_TTL[item.product.product_type] || 4 * 60 * 60 * 1000;
		...
		console.log(
			`🔄 Generating ${itemsNeedingRationale.length} AI rationales (cache miss)`,
		);
		...
		for (let i = 0; i < itemsNeedingRationale.length; i += batchSize) {
			...
			await Promise.all(
				batch.map((item) => this.generateAIRationale(item, clientProfile)),
			);
		}
	}

	private getRationaleCacheKey(
		product: InvestmentProduct,
		clientProfile: ClientProfile,
	): string {
	...
	private async generateAIRationale(
		item: BasketItem,
		clientProfile: ClientProfile,
	): Promise<void> {
...
Format response as JSON: {"rationale": "...", "pros": ["...", "...", "..."], "cons": ["...", "...", "..."]}`;
			...
			let parsed: { rationale: string; pros: string[]; cons: string[] } | null =
				null;
			let modelUsed = "";
			...
			if (useGpt52) {
				...
				const fallbackResult = {
					rationale: this.generateRuleBasedRationale(
						item.product,
						clientProfile,
					),
					pros: item.product.rationale_inputs?.key_factors || [],
					cons: item.product.rationale_inputs?.risk_factors || [],
				};
				const { result, modelUsed: usedModel } =
					await unifiedAIRecommendationEngine.runPrompt<{
						rationale: string;
						pros: string[];
						cons: string[];
					}>({
			...
			if (parsed) {
				...
				await cacheRationale(
					...
					console.warn("Failed to persist rationale cache:", err),
				);
			}
		} catch (error) {
			console.warn(
				`AI rationale generation failed for ${item.product.name}:`,
				error,
			);
		}
	}
	...
	async getQuickRecommendations(
		...
		const evaluated = this.evaluateProducts(
			filtered,
			mockProfile,
			this.getDefaultMarketContext(),
		);
