...
class SchemeGovernanceService {
		...
		try {
			...
			if (identifierType === "isin") {
				...
				const [r] = await db
					...
					.from(schemeTransactionRules)
					.where(
						ilike(schemeTransactionRules.schemeName, `%${schemeIdentifier}%`),
					)
					.limit(1);
				rule = r;
			}
			...
			return {
				...
				minLumpsumAmount: rule.minLumpsumAmount
					? Number(rule.minLumpsumAmount)
					: undefined,
				maxLumpsumAmount: rule.maxLumpsumAmount
					? Number(rule.maxLumpsumAmount)
					: undefined,
	...
	async validateProposalEligibility(
		...
		for (const scheme of schemes) {
			...
			const identifierType = scheme.isin
				? ("isin" as const)
				: ("name" as const);
			...
			if (scheme.investmentType === "lumpsum") {
				if (
					eligibility.minLumpsumAmount &&
					scheme.amount < eligibility.minLumpsumAmount
				) {
					amountValid = false;
				}
				if (
					eligibility.maxLumpsumAmount &&
					scheme.amount > eligibility.maxLumpsumAmount
				) {
					amountValid = false;
				}
			} else if (scheme.investmentType === "sip") {
				if (
					eligibility.minSipAmount &&
					scheme.amount < eligibility.minSipAmount
				) {
					amountValid = false;
				}
			}
			...
			const reason = !isTypeAllowed
				? eligibility.restrictionReason ||
					`${scheme.investmentType} not allowed for this scheme`
				: !amountValid
			...
			results.push({
				...
				eligible,
				reason,
				alternativeName: eligible
					? undefined
					: eligibility.alternativeSchemeName || undefined,
			});
			...
			try {
				...
				for (const fund of funds) {
					...
					const lumpsumAllowed = !(
						entry.restrictionType === "lumpsum" ||
						entry.restrictionType === "both"
					);
					const sipAllowed = !(
					...
					if (existing.length > 0) {
						await db
							...
							.set({
								...
								sipAllowed,
								subscriptionStatus:
									lumpsumAllowed && sipAllowed ? "OPEN" : "RESTRICTED",
	...
	async createProposalVersion(
		...
		try {
			const [latest] = await db
				.select({
					maxVersion: sql<number>`COALESCE(MAX(${proposalVersions.versionNumber}), 0)`,
				})
	...
	async getRecentRenames(limit: number = 50): Promise<
		Array<{
			schemeCode: string;
			oldName: string;
			newName: string;
	...
	async getTransactionRules(filters?: {
		...
		const query = db.select().from(schemeTransactionRules);
		...
		if (filters?.subscriptionStatus) {
			return db
				...
				.where(
					eq(
						schemeTransactionRules.subscriptionStatus,
