...
class TruthScreenESignService {
	...
	constructor() {
		...
		if (!this.isConfigured()) {
			...
			console.log(
				`✅ [TruthScreen eSign] Initialized (${this.environment} → ${this.baseUrl})`,
			);
		}
	}
	...
	async initiateESign(request: TruthScreenESignInitRequest): Promise<{
		...
		try {
			...
			if (this.isInMockMode()) {
				console.log(
					`[TruthScreen eSign] Mock mode - simulating eSign initiation: ${transactionId}`,
				);
				...
				return {
					...
					message:
						"eSign initiated successfully (mock mode). OTP will be sent to Aadhaar-linked mobile.",
					otpSent: true,
			...
			form.append(
				"reason",
				request.reason ||
					"I agree to sign the documents as its legally binded and approved.",
			);
			...
			if (request.documentBuffer) {
				...
				try {
					const fileResponse = await axios.get(request.documentUrl, {
					...
					console.error(
						`[TruthScreen eSign] Failed to fetch document from URL: ${fetchErr.message}`,
					);
					throw new AppError(
						"Failed to fetch document for signing",
						400,
				...
				const placeholderPdf = Buffer.from(
					"%PDF-1.4 placeholder for hash-based signing",
				);
			...
			console.log(
				`[TruthScreen eSign] Submitting eSign request: ${transactionId}`,
			);
			...
			console.log(
				`[TruthScreen eSign] Response:`,
				JSON.stringify(data).substring(0, 500),
			);

			const esignUrl =
				data?.url || data?.esignUrl || data?.redirect_url || data?.redirectUrl;
			...
			return {
				...
				message: isSuccess
					? data?.msg ||
						"eSign initiated. Complete signing via the provided URL."
					: data?.msg || data?.message || "eSign initiation failed",
				otpSent: !!isSuccess,
			...
			if (error.response?.data) {
				console.error(
					"[TruthScreen eSign] Response body:",
			...
			throw new AppError(
				error.response?.data?.msg ||
					...
					"Failed to initiate eSign via TruthScreen",
	...
	async checkStatus(
		transactionId: string,
	): Promise<TruthScreenESignStatusResponse> {
		if (this.isInMockMode()) {
		...
		try {
			...
			const data = response.data;
			console.log(
				`[TruthScreen eSign] Status response:`,
				JSON.stringify(data).substring(0, 500),
			);

			const statusStr = (data?.status || data?.esignStatus || "")
				.toString()
				.toLowerCase();
			const isSigned =
				statusStr === "signed" ||
					...
					if (esignRequest) {
						try {
							await db.insert(esignCertificates).values({
								...
								signedDocumentUrl:
									data.signedPdfUrl || data.signed_pdf_url || "",
							...
							console.warn(
								`[TruthScreen eSign] Certificate insert failed (may already exist):`,
								certErr.message,
							);
						}
					}
		...
		if (esignRequest.status !== "completed" && !this.isInMockMode()) {
			try {
				...
				return {
					...
					completedAt:
						liveStatus.status === "completed" ? new Date() : undefined,
	...
	async verifyESign(request: { transactionId: string; otp: string }): Promise<{
			...
			return {
				...
				signatureData: statusResult.signedAt
					? {
							...
							validTo: new Date(
								new Date(statusResult.signedAt).getTime() +
									365 * 24 * 60 * 60 * 1000,
							),
						}
					: undefined,
		...
		return {
			...
			message:
				...
				"eSign not yet completed. User must complete signing via the eSign URL.",
		};
	}
	...
	async resendOTP(
		...
		return {
			success: false,
			message:
				"TruthScreen Aadhaar eSign uses redirect-based OTP. Please use the eSign URL to complete signing.",
		};
	}
