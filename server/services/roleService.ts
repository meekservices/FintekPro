...
class RoleService {
	...
	checkRoleAccess(
		...
		if (userRoles.includes("tester")) {
			return {
				allowed: true,
				userRoles,
				reason: "Tester role grants universal access",
			};
		}
			...
			for (const requiredRole of requiredRoles) {
				if (hasAuthorityOver(userRole, requiredRole)) {
					return {
						allowed: true,
						userRoles,
						reason: `${getRoleDisplayName(userRole)} has authority over ${getRoleDisplayName(requiredRole)}`,
					};
				}
			}
		}
	...
	checkPermissionAccess(
		...
		if (userRoles.includes("tester")) {
			return {
				allowed: true,
				userRoles,
				reason: "Tester role grants universal permissions",
			};
		}
	...
	isDistributionNetwork(userRoles: RoleId[]): boolean {
		const distributionRoles: RoleId[] = [
			"partner",
			"partner_ops",
			"agent",
			"sub_agent",
	...
	getHighestRole(userRoles: RoleId[]): RoleId | null {
		...
		let lowestLevel =
			ROLE_DEFINITIONS[userRoles[0]]?.level ?? Number.POSITIVE_INFINITY;

		for (const role of userRoles) {
			const level = ROLE_DEFINITIONS[role]?.level ?? Number.POSITIVE_INFINITY;
	 ...
	 */
	getPublicRoleData() {
		const publicData: Record<
			string,
			{ name: string; portal: string; level: number }
		> = {};
