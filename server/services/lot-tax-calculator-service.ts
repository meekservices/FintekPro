...
class LotTaxCalculatorService {
	...
	private classifyFromFundData(fund: any): AssetClass {
		...
		if (
			...
			subCategory.includes("elss") ||
			schemeName.includes("elss")
		) {
			return "elss";
		}
		...
		if (category.includes("equity") && !category.includes("hybrid")) {
			if (
				...
				schemeName.includes("index") ||
				schemeName.includes("nifty") ||
				schemeName.includes("sensex")
			) {
				return "index";
			}
			if (
				category.includes("sector") ||
				subCategory.includes("sector") ||
				subCategory.includes("thematic")
			) {
				return "sectoral";
			}
			return "equity";
		...
		if (category.includes("hybrid")) {
			...
			const equityAllocation = Number.parseFloat(fund.equityAllocation || "0");
		...
		if (
			...
			schemeName.includes("international") ||
			schemeName.includes("global") ||
			schemeName.includes("us ") ||
			schemeName.includes("nasdaq")
		) {
	...
	private classifyFromSchemeName(schemeName: string): AssetClass {
		...
		if (name.includes("hybrid") || name.includes("balanced")) {
			if (name.includes("aggressive") || name.includes("equity"))
				return "hybrid_equity";
			return "hybrid_debt";
		}
		if (
	...
	async calculateLotTax(
		...
		const holdingPeriodDays = Math.floor(
			(referenceDate.getTime() - lot.purchaseDate.getTime()) /
				(1000 * 60 * 60 * 24),
		);
		...
		const {
			thresholds,
			taxRates: regimeTaxRates,
			equityExemption,
		} = getThresholdsAndRatesForDate(referenceDate);
		const ltcgThreshold = thresholds[assetClass];
		const capitalGainsType: "stcg" | "ltcg" =
			holdingPeriodDays >= ltcgThreshold ? "ltcg" : "stcg";
		...
		if (
			assetClass === "equity" &&
			lot.purchaseDate < GRANDFATHERING_DATE &&
			capitalGainsType === "ltcg"
		) {
		...
		const taxRate =
			capitalGainsType === "stcg"
				? regimeTaxRates[assetClass].stcg
				: regimeTaxRates[assetClass].ltcg;
		...
		const netProceedsAfterTaxAndExit =
			currentValue - estimatedTax - exitLoadAmount;

		// Calculate days to LTCG
		const daysToLTCG =
			capitalGainsType === "stcg" ? ltcgThreshold - holdingPeriodDays : null;
	...
	private generateRecommendation(
		...
		if (
			...
			daysToZeroExitLoad > 0 &&
			daysToZeroExitLoad <= 15
		) {
		...
		if (
			capitalGainsType === "ltcg" &&
			(daysToZeroExitLoad === null || daysToZeroExitLoad === 0)
		) {
			return "SELL_NOW";
		}
	...
	async calculateHoldingTaxSummary(
		...
		const assetClass =
			lotResults.length > 0 ? lotResults[0].assetClass : "unknown";
		const totalUnits = lotResults.reduce((sum, l) => sum + l.units, 0);
		const totalCostBasis = lotResults.reduce((sum, l) => sum + l.costBasis, 0);
		const totalCurrentValue = lotResults.reduce(
			(sum, l) => sum + l.currentValue,
			0,
		);
		const totalUnrealizedGain = lotResults.reduce(
			(sum, l) => sum + l.unrealizedGain,
			0,
		);
		...
		const ltcgAmount = lotResults
			...
			.reduce((sum, l) => sum + Math.max(0, l.unrealizedGain), 0);
		const totalEstimatedTax = lotResults.reduce(
			(sum, l) => sum + l.estimatedTax,
			0,
		);
		const totalExitLoad = lotResults.reduce(
			(sum, l) => sum + (l.exitLoad?.exitLoadAmount || 0),
			0,
		);
		const netProceedsAfterTaxAndExit =
			totalCurrentValue - totalEstimatedTax - totalExitLoad;

		// Overall recommendation
		const waitingLots = lotResults.filter(
			(l) =>
				l.recommendation === "WAIT_FOR_LTCG" ||
				l.recommendation === "WAIT_FOR_EXIT_LOAD",
		);
	 ...
	 */
	validateLotsForTax(
		lots: Array<{ purchaseDate: string | Date | null; units: number }>,
	): {
		isValid: boolean;
		...
		if (!lots || lots.length === 0) {
			return {
				...
				disabledReason:
					"No transaction-level data from CAS statement. Tax and exit load calculations disabled.",
			};
		}
		...
		for (const lot of lots) {
			if (!lot.purchaseDate) {
				...
				if (Number.isNaN(date.getTime())) {
					...
					errors.push(
						`Lot with ${lot.units} units has invalid date: ${lot.purchaseDate}`,
					);
				}
			}
		}
	...
	async simulateFIFORedemption(
		...
		for (const lot of sortedLots) {
			...
			try {
				...
				console.warn(
					`[LotTax FIFO] Exit load lookup failed for lot ${lot.purchaseDate}, using zero:`,
					error,
				);
			}
