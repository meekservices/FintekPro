...
class TwilioVoiceService {
	...
	constructor() {
		...
		if (voiceNumber) {
			this.fromNumber = voiceNumber.startsWith("+")
				? voiceNumber
				: `+${voiceNumber}`;
			console.log(
				"📞 Voice service initialized (using Replit Twilio connector)",
			);
...
<Response>
  <Say voice="alice" language="en-IN">
	...
	async sendOTPCall(to: string, otp: string): Promise<VoiceOTPResult> {
		...
		if (!isAvailable) {
			console.log(
				`📞 Voice OTP to ${to.substring(0, 6)}****: ${otp} (not configured)`,
			);
		...
		try {
			...
			console.log(
				`✅ Voice OTP call initiated to ${toNumber.substring(0, 8)}*** - SID: ${call.sid}`,
			);
	...
	async makeCall(to: string, twiml: string): Promise<VoiceCallResult> {
		...
		if (!isAvailable) {
			console.log(
				`📞 Voice call to ${to.substring(0, 6)}**** (not configured)`,
			);
			return { success: false, error: "Voice service not configured" };
		}
		...
		try {
			...
			console.log(
				`✅ Voice call initiated to ${toNumber.substring(0, 8)}*** - SID: ${call.sid}`,
			);
	...
	async sendPasswordResetOTP(to: string, otp: string): Promise<VoiceOTPResult> {
		...
		if (!isAvailable) {
			console.log(
				`📞 Password reset voice OTP to ${to.substring(0, 6)}****: ${otp} (not configured)`,
			);
			return { success: false, error: "Voice service not configured" };
		}
...
<Response>
  <Say voice="alice" language="en-IN">
    ...
    If you did not request this, please contact support immediately.
			...
			console.log(
				`✅ Password reset voice call to ${toNumber.substring(0, 8)}*** - SID: ${call.sid}`,
			);

			return { success: true, callSid: call.sid };
		} catch (error: any) {
			console.error(
				"❌ Failed to send password reset voice OTP:",
				error.message,
			);
			return { success: false, error: error.message };
		}
...
<Response>
  <Say voice="alice" language="en-IN">
    ...
    If you did not authorize this, please contact support immediately.
			...
			console.log(
				`✅ Transaction alert call to ${toNumber.substring(0, 8)}*** - SID: ${call.sid}`,
			);
	...
	async getCallStatus(
		callSid: string,
	): Promise<{ status?: string; duration?: number; error?: string }> {
		...
		try {
			...
			return {
				status: call.status,
				duration: call.duration ? Number.parseInt(call.duration) : undefined,
			};
		} catch (error: any) {
