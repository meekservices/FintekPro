import { db } from "../db";
import { aiRecommendationTracking, type AiRecommendationTracking, type InsertAiRecommendationTracking } from "../../shared/schema";
...
export class AiRecommendationTrackingService {
  ...
  async recordRecommendation(data: InsertAiRecommendationTracking): Promise<AiRecommendationTracking> {
    const [recommendation] = await db.insert(aiRecommendationTracking)
      .values(data)
      .returning();
    return recommendation;
  }

  async getRecommendation(id: string): Promise<AiRecommendationTracking | undefined> {
  ...
  async getAllRecommendations(filters?: {
    ...
    if (filters?.assetType) {
      conditions.push(eq(aiRecommendationTracking.assetType, filters.assetType));
    }
    if (filters?.sector) {
  ...
  async updateRecommendationPrice(
    ...
    if (highestPrice !== undefined) {
      const existingHigh = recommendation.highestPrice ? parseFloat(recommendation.highestPrice) : 0;
      updates.highestPrice = Math.max(existingHigh, highestPrice).toString();
    }

    if (lowestPrice !== undefined) {
      const existingLow = recommendation.lowestPrice ? parseFloat(recommendation.lowestPrice) : Infinity;
      updates.lowestPrice = Math.min(existingLow, lowestPrice).toString();
    }

    await db.update(aiRecommendationTracking)
      .set(updates)
      .where(eq(aiRecommendationTracking.id, id));
  }
  ...
  async resolveRecommendation(
    ...
    const [updated] = await db.update(aiRecommendationTracking)
      .set({
        status,
        currentPrice: currentPrice.toString(),
  ...
  async getSuccessMetrics(dateFrom?: Date, dateTo?: Date): Promise<SuccessMetrics> {
    const conditions = [];
    if (dateFrom) {
    ...
    const resolvedRecommendations = recommendations.filter(r => r.status !== 'pending').length;
    const hitTarget = recommendations.filter(r => r.status === 'hit_target').length;
    const missedTarget = recommendations.filter(r => r.status === 'missed_target').length;
    const stoppedOut = recommendations.filter(r => r.status === 'stopped_out').length;
    const expired = recommendations.filter(r => r.status === 'expired').length;
    
    const successRate = resolvedRecommendations > 0 
      ? (hitTarget / resolvedRecommendations) * 100 
      : 0;

    const resolvedWithReturns = recommendations.filter(r => r.actualReturn !== null);
    const averageReturn = resolvedWithReturns.length > 0
      ? resolvedWithReturns.reduce((sum, r) => sum + parseFloat(r.actualReturn || '0'), 0) / resolvedWithReturns.length
      : 0;

    const averageConfidence = totalRecommendations > 0
      ? recommendations.reduce((sum, r) => sum + parseFloat(r.confidenceScore), 0) / totalRecommendations
      : 0;
  ...
  async getMetricsBySector(): Promise<SectorMetrics[]> {
    ...
    const sectorMap = new Map<string, { total: number; hits: number; returns: number[] }>();
    ...
    for (const rec of recommendations) {
      ...
      if (rec.actualReturn) data.returns.push(parseFloat(rec.actualReturn));
    }

    return Array.from(sectorMap.entries()).map(([sector, data]) => ({
      sector,
      total: data.total,
      successRate: data.total > 0 ? Math.round((data.hits / data.total) * 10000) / 100 : 0,
  ...
  async getMetricsByTimeframe(): Promise<TimeframeMetrics[]> {
    ...
    const timeframeMap = new Map<string, { total: number; hits: number; returns: number[] }>();
    ...
    for (const rec of recommendations) {
      const label = timeframeLabels[rec.timeframeInDays] || `${rec.timeframeInDays} Days`;
  ...
  async getMetricsByAssetType(): Promise<AssetTypeMetrics[]> {
    ...
    const assetMap = new Map<string, { total: number; hits: number; returns: number[] }>();
  ...
  async getTopPerformingRecommendations(limit: number = 10): Promise<AiRecommendationTracking[]> {
    return db.select()
      .from(aiRecommendationTracking)
      .where(eq(aiRecommendationTracking.status, 'hit_target'))
      .orderBy(desc(sql`CAST(${aiRecommendationTracking.actualReturn} AS DECIMAL)`))
      .limit(limit);
  }

  async getWorstPerformingRecommendations(limit: number = 10): Promise<AiRecommendationTracking[]> {
    return db.select()
      .from(aiRecommendationTracking)
      .where(sql`${aiRecommendationTracking.status} IN ('missed_target', 'stopped_out')`)
  ...
  async checkAndUpdateExpiredRecommendations(): Promise<number> {
    ...
    for (const rec of expired) {
      const currentPrice = rec.currentPrice ? parseFloat(rec.currentPrice) : parseFloat(rec.entryPrice);
      await this.resolveRecommendation(rec.id, 'expired', currentPrice, 'Expired without hitting target');
      count++;
    }
  ...
  async deleteRecommendation(id: string): Promise<boolean> {
    const result = await db.delete(aiRecommendationTracking)
      .where(eq(aiRecommendationTracking.id, id));
    return true;
  }
}

export const aiRecommendationTrackingService = new AiRecommendationTrackingService();
