...
export class TaxEstimationEngine {
  ...
  private readonly LTCG_RATE = 0.10; // 10% Long term (often with exemption, ignored here for conservative buffer)
   ...
   */
  public estimateRebalanceImpact(actions: TaxEstimationContext[]): TaxImpactReport {
    let ltcg = 0;
    let stcg = 0;
    let losses = 0;
    ...
    let remainingLosses = Math.max(0, losses - stcg);
    
    const effectiveLtcg = Math.max(0, ltcg - remainingLosses);

    const projectedTax = (effectiveStcg * this.STCG_RATE) + (effectiveLtcg * this.LTCG_RATE);
    ...
    const severityScore = projectedTax > 10000 ? 80 : projectedTax > 1000 ? 40 : 10;
