...
function generateBond(index: number): any {
	...
	const couponType =
		COUPON_TYPES[Math.floor(Math.random() * COUPON_TYPES.length)];
	const couponFrequency =
		COUPON_FREQUENCIES[Math.floor(Math.random() * COUPON_FREQUENCIES.length)];
	const ratingAgency =
		RATING_AGENCIES[Math.floor(Math.random() * RATING_AGENCIES.length)];
	...
	const liquidityScore = isListed
		? Math.floor(40 + Math.random() * 60)
		: Math.floor(10 + Math.random() * 40);
	const bidAskSpread = isListed
		? 0.2 + Math.random() * 1.5
		: 1.5 + Math.random() * 3;

	const regulatoryEligibility =
		issuer.rating === "AAA" || issuer.rating === "AA+"
			? "retail"
			: issuer.rating === "AA" || issuer.rating === "AA-"
				? Math.random() > 0.5
					? "retail"
					: "hni_only"
				: issuer.rating === "A+" || issuer.rating === "A"
					? Math.random() > 0.7
						? "hni_only"
						: "qib_only"
					: "qib_only";
	...
	const securityType = ["senior_secured", "subordinated", "unsecured"][
	...
	const structureComplexity =
		...
		(isCallable ? 1 : 0) +
		(isPuttable ? 1 : 0) +
		(securityType === "subordinated" ? 1 : 0) +
		(couponType === "floating" ? 1 : 0);
	...
	return {
		...
		ratingAgency,
		ratingDate: randomDate(new Date(now.getFullYear() - 1, 0, 1), now)
			.toISOString()
			.split("T")[0],
		outlookStatus: ["stable", "positive", "negative"][
			Math.floor(Math.random() * 3)
		],
...
export async function seedBondUniverse(count: number = 8000): Promise<{
	...
	for (let batch = 0; batch < batches; batch++) {
		...
		try {
			...
			if ((batch + 1) % 10 === 0) {
				console.log(
					`[BondUniverseSeeder] Progress: ${inserted}/${count} bonds inserted`,
				);
			}
		} catch (error) {
			errors.push(
				`Batch ${batch} failed: ${error instanceof Error ? error.message : "Unknown error"}`,
			);
		}
	}
	...
	console.log(
		`[BondUniverseSeeder] Status distribution: ${sellable} SELLABLE, ${visible} VISIBLE, ${hidden} HIDDEN`,
	);
...
export async function getBondUniverseStats(): Promise<{
  ...
  `);
	...
	return {
		total: Number.parseInt((totalResult.rows[0]?.count as string) || "0"),
		byStatus: Object.fromEntries(
			statusResult.rows.map((r) => [
				r.instrument_status,
				Number.parseInt(r.count as string),
			]),
		),
		byType: Object.fromEntries(
			typeResult.rows.map((r) => [
				r.bond_type,
				Number.parseInt(r.count as string),
			]),
		),
		bySector: Object.fromEntries(
			sectorResult.rows.map((r) => [
				r.issuer_sector || "Unknown",
				Number.parseInt(r.count as string),
			]),
		),
		byRating: Object.fromEntries(
			ratingResult.rows.map((r) => [
				r.credit_rating || "Unrated",
				Number.parseInt(r.count as string),
			]),
		),
	};
