import { db } from "../db";
import { mfMonthwisePerformance, historicalNavData, mutualFunds } from "@shared/schema";
...
export class MfMonthwisePerformanceService {
  ...
  static getInstance(): MfMonthwisePerformanceService {
    if (!this.instance) {
      this.instance = new MfMonthwisePerformanceService();
    }
    return this.instance;
  }

  async calculateAndStoreMonthlyReturns(schemeCode: string): Promise<CalculationResult> {
    try {
      console.log(`[MFMonthwise] Calculating monthly returns for scheme ${schemeCode}...`);
      ...
      for (const monthReturn of monthlyReturns) {
        try {
          ...
          console.error(`[MFMonthwise] Error inserting month ${monthReturn.monthYear}:`, err);
        }
      }

      const months = monthlyReturns.map((r) => r.monthYear).sort();
      
      console.log(`[MFMonthwise] Stored ${insertedCount} monthly records for scheme ${schemeCode}`);
  ...
  private calculateMonthlyReturns(
    ...
    for (const monthKey of sortedMonths) {
      ...
      const returnPercent = navStart > 0 ? ((navEnd - navStart) / navStart) * 100 : null;
      
      const benchmarkReturn = NIFTY_MONTHLY_RETURNS[monthKey] || null;
      
      const excessReturn = returnPercent !== null && benchmarkReturn !== null
      ...
      returns.push({
        ...
        navStart,
        navEnd,
        returnPercent: returnPercent !== null ? Math.round(returnPercent * 100) / 100 : null,
        benchmarkReturn,
        excessReturn: excessReturn !== null ? Math.round(excessReturn * 100) / 100 : null,
        isPartial,
      });
    }
  ...
  async getMonthwisePerformance(
    ...
    try {
      ...
      return results.map((r) => ({
        ...
        navStart: r.navStart ? parseFloat(r.navStart) : null,
        navEnd: r.navEnd ? parseFloat(r.navEnd) : null,
        returnPercent: r.returnPercent ? parseFloat(r.returnPercent) : null,
        benchmarkReturn: r.benchmarkReturn ? parseFloat(r.benchmarkReturn) : null,
        excessReturn: r.excessReturn ? parseFloat(r.excessReturn) : null,
  ...
  async refreshAllFundsMonthlyPerformance(limit: number = 100): Promise<{
    ...
    try {
      ...
      for (const fund of funds) {
        const result = await this.calculateAndStoreMonthlyReturns(fund.schemeCode);
...
export const mfMonthwisePerformanceService = MfMonthwisePerformanceService.getInstance();
