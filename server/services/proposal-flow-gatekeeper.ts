...
export class ProposalFlowGatekeeper {
	...
	static async getFlowStatus(proposalId: string): Promise<ProposalFlowStatus> {
		...
		if (!state) {
			await ProposalFlowGatekeeper.initializeFlowState(proposalId);
			[state] = await db
				.select()
				.from(proposalFlowState)
				.where(eq(proposalFlowState.proposalId, proposalId))
		...
		const phases: PhaseValidation[] = PHASE_ORDER.map((phase) => {
			...
			const missingPrerequisites = prerequisites.filter((prereq) => {
				return !ProposalFlowGatekeeper.isPhaseCompleted(state, prereq);
			});
		...
		const currentPhaseIndex = PHASE_ORDER.indexOf(
			state.currentPhase as ProposalPhase,
		);
	...
	static async validatePhaseTransition(
		...
		const targetPhaseValidation = status.phases.find(
			(p) => p.phase === targetPhase,
		);
	...
	static async completePhase(
		...
		const validation = await ProposalFlowGatekeeper.validatePhaseTransition(
			proposalId,
			phase,
		);
		if (!validation.valid) {
	...
	static async validatePortfolioAnalysis(
		proposalId: string,
	): Promise<{ valid: boolean; errors: string[] }> {
		const errors: string[] = [];
		...
		const hasFreshAmount =
			Number.parseFloat(proposal.totalInvestmentAmount?.toString() || "0") > 0;

		if (!hasHoldings && !hasFreshAmount) {
			errors.push(
				"Cannot analyze portfolio without holdings or fresh investment amount",
			);
		}
	...
	static async validateRecommendations(
		...
		if (!proposal.targetAllocation) {
			errors.push(
				"Cannot generate recommendations without target asset allocation",
			);
		}
	...
	static async validateReportGeneration(
		proposalId: string,
	): Promise<{ valid: boolean; errors: string[] }> {
		const errors: string[] = [];
	...
	static async validateStrategyIntegrity(
		...
		const { proposalVersions, investmentProposals } = await import(
			"@shared/schema"
		);
		...
		if (!lockedVersion) {
			integrityChecks.push({
				check: "strategy_lock_exists",
				passed: false,
				detail: "No locked strategy found",
			});
			errors.push(
				"Strategy must be locked before proceeding to report generation",
			);
			return {
				valid: false,
				errors,
		...
		integrityChecks.push({
			check: "strategy_lock_exists",
			passed: true,
			detail: `Version ${lockedVersion.versionNumber} locked`,
		});

		const snapshot = lockedVersion.strategySnapshot as any;
		if (
			!snapshot?.assetAllocation ||
			!Array.isArray(snapshot.assetAllocation)
		) {
			integrityChecks.push({
				check: "snapshot_structure",
				passed: false,
				detail: "Strategy snapshot missing or malformed",
			});
			...
			integrityChecks.push({
				check: "snapshot_structure",
				passed: true,
				detail: `${snapshot.assetAllocation.length} asset classes defined`,
			});

			const totalWeight = snapshot.assetAllocation.reduce(
				(sum: number, a: any) => sum + (a.weight || 0),
				0,
			);
			if (Math.abs(totalWeight - 100) > 0.01) {
				integrityChecks.push({
					check: "weight_sum_100",
					passed: false,
					detail: `Total weight: ${totalWeight.toFixed(2)}%`,
				});
				errors.push(
					`Allocation weights total ${totalWeight.toFixed(2)}%, must be 100%`,
				);
			} else {
				integrityChecks.push({
					check: "weight_sum_100",
			...
			const negativeWeights = snapshot.assetAllocation.filter(
				(a: any) => a.weight < 0,
			);
			if (negativeWeights.length > 0) {
				integrityChecks.push({
					...
					passed: false,
					detail: `${negativeWeights.length} negative weights`,
				});
			...
			const emptyAssets = snapshot.assetAllocation.filter(
				(a: any) => !a.assetClass || a.assetClass.trim() === "",
			);
			if (emptyAssets.length > 0) {
				integrityChecks.push({
					check: "no_empty_asset_classes",
					passed: false,
					detail: `${emptyAssets.length} unnamed asset classes`,
				});
		...
		if (
			!lockedVersion.allocationMode ||
			!["AI_DRIVEN", "MANUAL"].includes(lockedVersion.allocationMode)
		) {
			integrityChecks.push({
				check: "valid_allocation_mode",
				passed: false,
				detail: `Mode: ${lockedVersion.allocationMode || "undefined"}`,
			});
			...
			integrityChecks.push({
				check: "valid_allocation_mode",
				passed: true,
				detail: `Mode: ${lockedVersion.allocationMode}`,
			});
		}
		...
		if (agentId) {
			...
			await db.insert(agentComplianceAuditLogs).values({
				...
				actionType:
					errors.length > 0
						? "strategy_integrity_failed"
						: "strategy_integrity_validated",
