...
class KycEnvironmentService {
	...
	constructor() {
		...
		console.log(
			`✅ KYC Environment Service initialized (${this.flags.environment})`,
		);
		if (this.flags.fixedOtpEnabled) {
			console.log("   ⚠️ Fixed OTP enabled (sandbox mode)");
		}
		console.log(
			`   Active providers → Pan: ${this.flags.prodProviders.pan}, Aadhaar: ${this.flags.prodProviders.aadhaar}, CKYC: ${this.flags.prodProviders.ckyc}, AML: ${this.flags.prodProviders.aml}`,
		);
	}
	...
	validatePanForEnvironment(panNumber: string): {
		allowed: boolean;
		reason?: string;
	} {
		if (
			this.isProduction() &&
			this.flags.testPanBlockedInProd &&
			this.isTestPan(panNumber)
		) {
			return {
				allowed: false,
				reason: "Test PAN numbers are not allowed in production environment",
			};
		}
		return { allowed: true };
	}
	...
	getActiveProvider(service: "pan" | "aadhaar" | "ckyc" | "aml"): string {
		const providers = this.isSandbox()
			? this.flags.sandboxProviders
			: this.flags.prodProviders;
	...
	> {
		...
		return {
			pan: {
				provider: providers.pan,
				displayName: getDisplayName(providers.pan),
				status: getStatus(providers.pan),
				environment: this.flags.environment,
			},
			aadhaar: {
				provider: providers.aadhaar,
				displayName: getDisplayName(providers.aadhaar),
				status: getStatus(providers.aadhaar),
				environment: this.flags.environment,
			},
			ckyc: {
				provider: providers.ckyc,
				displayName: getDisplayName(providers.ckyc),
				status: getStatus(providers.ckyc),
				environment: this.flags.environment,
			},
			aml: {
				provider: providers.aml,
				displayName: getDisplayName(providers.aml),
				status: getStatus(providers.aml),
				environment: this.flags.environment,
			},
		};
	}
