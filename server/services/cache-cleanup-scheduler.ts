...
class CacheCleanupScheduler {
	...
	async runDailyCleanup(): Promise<{ deleted: number }> {
		try {
			...
			console.log(
				`[CacheScheduler] Daily cleanup complete. Deleted ${result.deleted} expired entries.`,
			);
	...
	async logQuarterlyRefreshReminder() {
		...
		console.log(
			"[CacheScheduler] Company financials will be refreshed on next access (120-day TTL expired).",
		);
	...
	async updateCacheStatistics() {
		try {
			...
			console.log(
				`[CacheScheduler] Cache stats - Companies: ${stats.companyMaster.count}, Verifications: ${stats.verifications.count}, Hit rate: ${stats.apiUsage.hitRate.toFixed(1)}%`,
			);
		} catch (error) {
			console.error(
				"[CacheScheduler] Failed to update cache statistics:",
				error,
			);
		}
	}
	...
	async initializeScheduleRecords() {
        ...
        `);
			...
			console.error(
				"[CacheScheduler] Failed to initialize schedule records:",
				error,
			);
		}
	}
