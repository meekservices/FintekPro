...
} from "../bank-api-rate-limiter";
import {
	bankAPIAuditService,
	type BankAPIAuditEntry,
} from "../bank-api-audit-service";
...
export abstract class BaseBankConnector {
	...
	protected async getAuthHeaders(): Promise<Record<string, string>> {
		...
		const token = await bankTokenManagementService.getValidToken(
			this.config.bankCode,
			environment,
		);
		if (!token) {
			console.log(
				`[${this.config.bankCode}] No OAuth token available, using API key auth`,
			);
			const credentials = await bankCredentialsVaultService.getCredentials(
				this.config.bankCode,
				environment,
			);
	...
	protected async checkRateLimit(
		...
		const result = await bankAPIRateLimiter.checkRateLimit(
			this.config.bankCode,
			operation,
		);
		if (!result.allowed) {
			console.warn(
				`[${this.config.bankCode}] Rate limited for ${operation}. Retry after ${result.retryAfterMs}ms`,
			);
		}
	...
	protected async auditApiCall(
		...
		await bankAPIAuditService.logAPICall({
			...
			metadata: {
				responseStatus: redactedResponse.status,
				hasData: !!redactedResponse,
			},
		});
	}

	protected redactSensitiveFields(
