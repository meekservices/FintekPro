...
export function mapEmploymentTypeToCanonical(
...
export function applicationToCanonical(
	...
	return {
		...
		applicant: {
			...
			gender: application.gender?.toUpperCase() as
				| "MALE"
				| "FEMALE"
				| "OTHER"
				| undefined,
		...
		income: {
			monthly_income: Number.parseFloat(application.monthlyIncome) || 0,
			annual_income: application.annualIncome
				? Number.parseFloat(application.annualIncome)
				: undefined,
		...
		loan: {
			...
			amount: Number.parseFloat(application.requestedAmount) || 0,
		...
		existing_obligations: application.existingLoans
			? {
					total_emi: Number.parseFloat(application.existingEmiAmount || "0"),
		...
		collateral: application.collateralType
			? {
					...
					value: Number.parseFloat(application.collateralValue || "0"),
...
export function validateCanonicalPayload(payload: CanonicalLoanPayload): {
	valid: boolean;
	errors: string[];
} {
	const errors: string[] = [];
	...
	if (
		!payload.applicant.pan ||
		!/^[A-Z]{5}[0-9]{4}[A-Z]$/.test(payload.applicant.pan)
	) {
		errors.push("Valid PAN is required");
	}
	if (
		!payload.applicant.mobile ||
		!/^\d{10}$/.test(
			payload.applicant.mobile.replace(/^\+91/, "").replace(/\D/g, ""),
		)
	) {
		errors.push("Valid 10-digit mobile number is required");
	}
	if (!payload.loan.amount || payload.loan.amount <= 0)
		errors.push("Loan amount must be positive");
	if (!payload.loan.tenure_months || payload.loan.tenure_months <= 0)
		errors.push("Loan tenure must be positive");
	if (!payload.income.monthly_income || payload.income.monthly_income <= 0)
