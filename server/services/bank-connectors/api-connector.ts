...
export class APIBankConnector extends BaseBankConnector {
	...
	async submitApplication(
		...
		try {
			...
			console.log(
				`[${this.config.bankCode}] Submitting loan application via API`,
				{
			...
			console.error(
				`[${this.config.bankCode}] API submission failed:`,
				error.message,
			);
	...
	async checkStatus(bankReference: string): Promise<BankStatusResponse> {
		...
		console.log(
			`[${this.config.bankCode}] Checking status for reference:`,
			bankReference,
		);
		...
		return {
			...
			applicant: {
				...
				dateOfBirth: application.dateOfBirth
					? this.formatDate(application.dateOfBirth)
					: null,
			...
			employment: {
				...
				annualIncome: application.annualIncome
					? this.formatAmount(application.annualIncome)
					: null,
			},
			loan: {
			...
			credit: {
				...
				existingEmi: application.existingEmiAmount
					? this.formatAmount(application.existingEmiAmount)
					: 0,
			},
		};
	}
