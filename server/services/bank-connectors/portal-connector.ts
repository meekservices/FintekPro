...
export class PortalBankConnector extends BaseBankConnector {
	...
	async submitApplication(
		...
		try {
			...
			return {
				...
				message:
					"Portal submission task created (manual intervention required)",
				expectedResponseTime: this.config.expectedResponseTime || 96,
			};
		} catch (error: any) {
			console.error(
				`[${this.config.bankCode}] Portal submission task creation failed:`,
				error.message,
			);
			return {
				success: false,
	...
	async checkStatus(bankReference: string): Promise<BankStatusResponse> {
		console.log(
			`[${this.config.bankCode}] Checking portal status for reference:`,
			bankReference,
		);
