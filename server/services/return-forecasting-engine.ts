...
const ASSET_CLASS_PARAMS: Record<string, { expectedReturn: number; volatility: number; drawdownFactor: number }> = {
  equity: { expectedReturn: 12.0, volatility: 18.0, drawdownFactor: 0.55 },
  mutual_fund: { expectedReturn: 10.0, volatility: 14.0, drawdownFactor: 0.40 },
  bond: { expectedReturn: 7.0, volatility: 4.0, drawdownFactor: 0.10 },
  fd: { expectedReturn: 6.5, volatility: 0.5, drawdownFactor: 0.0 },
  etf: { expectedReturn: 11.0, volatility: 16.0, drawdownFactor: 0.45 },
  real_estate: { expectedReturn: 8.0, volatility: 10.0, drawdownFactor: 0.30 },
  gold: { expectedReturn: 6.0, volatility: 12.0, drawdownFactor: 0.25 },
  alternative: { expectedReturn: 15.0, volatility: 25.0, drawdownFactor: 0.60 },
};

// Stress test scenarios
const STRESS_SCENARIOS: Array<{ name: string; description: string; probability: number; shockFactor: Record<string, number> }> = [
  {
    name: 'market_crash',
    description: 'Severe market crash similar to 2008 financial crisis',
    probability: 0.05,
    shockFactor: {
      equity: -0.50, mutual_fund: -0.40, bond: -0.05, fd: 0.0,
      etf: -0.45, real_estate: -0.30, gold: 0.10, alternative: -0.55
    }
  },
  {
    ...
    probability: 0.15,
    shockFactor: {
      equity: -0.20, mutual_fund: -0.15, bond: -0.02, fd: 0.0,
      etf: -0.18, real_estate: -0.10, gold: 0.05, alternative: -0.25
    }
  },
  {
    name: 'stagflation',
    ...
    probability: 0.10,
    shockFactor: {
      equity: -0.15, mutual_fund: -0.10, bond: -0.10, fd: -0.03,
      etf: -0.12, real_estate: 0.05, gold: 0.15, alternative: -0.20
    }
  },
  {
    name: 'deflation',
    ...
    probability: 0.05,
    shockFactor: {
      equity: -0.25, mutual_fund: -0.20, bond: 0.10, fd: 0.02,
      etf: -0.22, real_estate: -0.15, gold: 0.05, alternative: -0.30
    }
  },
  {
    name: 'bull_market',
    ...
    probability: 0.20,
    shockFactor: {
      equity: 0.25, mutual_fund: 0.20, bond: 0.05, fd: 0.0,
      etf: 0.22, real_estate: 0.15, gold: -0.05, alternative: 0.30
    }
  }
];
...
export class ReturnForecastingEngine {
  ...
  calculateReturnMetrics(asset: AssetReturns): ReturnMetrics {
    const holdingPeriodYears = this.getYearsDifference(asset.inceptionDate, new Date());
    const totalGain = asset.currentValue - asset.investedAmount;
    const absoluteReturn = (totalGain / asset.investedAmount) * 100;
    const cagr = this.calculateCAGR(asset.investedAmount, asset.currentValue, holdingPeriodYears);
  ...
  runStressTests(asset: AssetReturns): StressTestResult[] {
    return STRESS_SCENARIOS.map(scenario => {
      ...
      const recoveryPeriodMonths = drawdown > 0 
        ? Math.ceil((drawdown / params.expectedReturn) * 12)
        : 0;
  ...
  calculateDrawdownMetrics(historicalReturns: number[]): DrawdownMetrics {
    ...
    const averageDrawdown = nonZeroDrawdowns.length > 0
      ? nonZeroDrawdowns.reduce((a, b) => a + b, 0) / nonZeroDrawdowns.length
      : 0;

    // Ulcer Index (RMS of drawdowns)
    const squaredDrawdowns = drawdowns.map(d => d * d);
    const ulcerIndex = Math.sqrt(squaredDrawdowns.reduce((a, b) => a + b, 0) / drawdowns.length);
    ...
    return {
      ...
      maxDrawdownPeriod: maxDrawdown > 0 ? {
        start: new Date(Date.now() - (prices.length - maxDrawdownStart) * 30 * 24 * 60 * 60 * 1000),
        end: new Date(Date.now() - (prices.length - maxDrawdownEnd) * 30 * 24 * 60 * 60 * 1000)
      } : null,
  ...
  calculateRiskAdjustedReturns(asset: AssetReturns): {
    ...
    const calmarRatio = maxDrawdown > 0 ? returnMetrics.annualizedReturn / maxDrawdown : 0;
    ...
    const informationRatio = (returnMetrics.annualizedReturn - benchmarkReturn) / trackingError;
   ...
   */
  getComprehensiveReturns(asset: AssetReturns, cashFlows?: CashFlow[]): ComprehensiveReturns {
    const returnMetrics = this.calculateReturnMetrics(asset);
    ...
    const drawdownMetrics = this.calculateDrawdownMetrics(asset.historicalReturns || []);
  ...
  calculatePortfolioReturns(assets: AssetReturns[]): {
    ...
    const portfolioVolatility = weightedVolatility * Math.sqrt(
      (1 - avgCorrelation) / assets.length + avgCorrelation
    );
    const diversificationBenefit = ((weightedVolatility - portfolioVolatility) / weightedVolatility) * 100;
