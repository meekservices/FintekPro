...
class HistoricalNavRefreshJob {
	...
	static getInstance(): HistoricalNavRefreshJob {
		if (!HistoricalNavRefreshJob.instance) {
			HistoricalNavRefreshJob.instance = new HistoricalNavRefreshJob();
		}
		return HistoricalNavRefreshJob.instance;
	}
	...
	async runInitialWarmup(): Promise<void> {
		...
		for (const schemeCode of popularSchemes) {
			if (warmupConsecutiveFailures >= WARMUP_CIRCUIT_BREAKER) {
				console.warn(
					`[HistoricalNavRefresh] Warmup circuit breaker tripped — MFAPI appears down. Skipping remaining warmup schemes.`,
				);
				break;
			}
			...
			try {
				...
				if (warmupConsecutiveFailures === 0) {
					console.error(
						`[HistoricalNavRefresh] Warmup failed for ${schemeCode}: ${msg}`,
					);
				}
				warmupConsecutiveFailures++;
			}
		...
		console.log(
			`[HistoricalNavRefresh] Initial warmup complete: ${successCount}/${popularSchemes.length} schemes, ${newRecords} records`,
		);
	}
	...
	async runRefresh(): Promise<RefreshStats> {
		if (this.isRunning) {
			console.log(
				"[HistoricalNavRefresh] Refresh already in progress, skipping...",
			);
		...
		try {
			...
			console.log(
				`[HistoricalNavRefresh] Refreshing ${stats.totalSchemes} schemes...`,
			);
			...
			for (const scheme of existingSchemes) {
				if (isPoolClosed()) {
					console.log(
						"[HistoricalNavRefresh] Pool closing — aborting refresh loop",
					);
					break;
				}
				...
				if (Date.now() - batchStart > MAX_RUN_MS) {
					console.warn(
						`[HistoricalNavRefresh] 45m time cap reached — stopping after ${stats.successfulRefreshes} schemes`,
					);
					break;
				}
				...
				if (consecutiveFailures >= CIRCUIT_BREAKER_THRESHOLD) {
					console.warn(
						`[HistoricalNavRefresh] Circuit breaker tripped — ${consecutiveFailures} consecutive failures. MFAPI appears down. Aborting run.`,
					);
					break;
				}
				...
				try {
					...
					if (consecutiveFailures === 0) {
						console.error(
							`[HistoricalNav] Error fetching ${scheme.identifier}: ${msg}`,
						);
					}
					stats.failedRefreshes++;
					consecutiveFailures++;
