...
export class ComplianceSnapshotService {
	...
	static getInstance(): ComplianceSnapshotService {
		if (!ComplianceSnapshotService.instance) {
			ComplianceSnapshotService.instance = new ComplianceSnapshotService();
		}
		return ComplianceSnapshotService.instance;
	}
	...
	private checkRiskProfileMatch(
		...
		const isMatch =
			equityWeight >= threshold.min && equityWeight <= threshold.max;
		const deviation = isMatch
			? 0
			: equityWeight < threshold.min
				? threshold.min - equityWeight
				: equityWeight - threshold.max;
	...
	private checkSuitabilityFactors(
		input: SuitabilityInput,
	): ComplianceSnapshot["details"]["suitabilityFactors"] {
	...
	isProposalSuitable(
		...
		return (
			snapshot.suitabilityScore >= minScore &&
...
export const complianceSnapshotService =
	ComplianceSnapshotService.getInstance();
