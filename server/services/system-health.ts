...
async function checkPaymentGateway(): Promise<ServiceHealth> {
	...
	return {
		...
		lastCheck: new Date().toISOString(),
		message: cashfreeConfigured
			? "Cashfree PG configured"
			: "Cashfree PG credentials missing (CASHFREE_PG_APP_ID / CASHFREE_PG_SECRET_KEY)",
	};
}
...
async function checkVerificationAPIs(): Promise<ServiceHealth> {
	...
	return {
		...
		message: sandboxConfigured
			? "Sandbox.co.in configured"
			: cashfreeVerify
				? "Cashfree verification ready"
				: "No verification API configured",
...
export async function getEndpointHealth(): Promise<
	{ endpoint: string; status: string; latency: number }[]
> {
	const endpoints = [
		"/api/auth/me",
