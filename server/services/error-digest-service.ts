		...
		try {
			...
			errors.forEach((err) => {
				...
				errorCodeCount[err.errorCode] =
					(errorCodeCount[err.errorCode] || 0) + 1;
			});
	...
	async sendCriticalAlert(error: any): Promise<void> {
		try {
			...
			const adminUsers = await db
				...
				.from(users)
				.where(
					sql`${users.roles} @> ARRAY['admin']::varchar[] AND ${users.isActive} = true`,
				)
				.limit(5);
			...
			try {
				...
				console.error(
					"[ErrorDigest] AI analysis for critical alert failed:",
					e,
				);
			}
    ...
    <div style="margin-top: 15px; text-align: center;">
      <a href="${process.env.REPLIT_DOMAINS ? `https://${process.env.REPLIT_DOMAINS.split(",")[0]}` : ""}/admin/error-command-center" 
        ...
        View in Error Command Center
			...
			console.log(
				`[ErrorDigest] Sent critical alert for ${error.errorCode} to ${adminUsers.length} admin(s)`,
			);
		} catch (err) {
