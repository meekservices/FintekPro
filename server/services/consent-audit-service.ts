...
class ConsentAuditService {
	async recordConsent(params: RecordConsentParams): Promise<ConsentAuditLog> {
		...
		console.log(
			`[ConsentAudit] Recorded ${params.action} for ${params.consentType} - User: ${params.userId || "anonymous"}`,
		);
		return record[0];
	}
	...
	async recordBulkConsent(
		...
		console.log(
			`[ConsentAudit] Recorded ${records.length} consent actions - User: ${commonParams.userId || "anonymous"}`,
		);
		return records;
	}
	...
	async getSessionConsentHistory(
		sessionId: string,
	): Promise<ConsentAuditLog[]> {
		return db
			.select()
	...
	async getConsentStats(): Promise<{
		...
		const [totalResult, byTypeResult, last24hResult, last7dResult] =
			await Promise.all([
      ...
      `),
		...
		for (const row of byTypeResult.rows as any[]) {
			...
			byType[row.consent_type][row.action as "granted" | "withdrawn"] =
				Number.parseInt(row.count);
		}

		return {
			totalRecords: Number.parseInt((totalResult.rows[0] as any)?.count || "0"),
			byType,
			last24Hours: Number.parseInt(
				(last24hResult.rows[0] as any)?.count || "0",
			),
			last7Days: Number.parseInt((last7dResult.rows[0] as any)?.count || "0"),
		};
	}
