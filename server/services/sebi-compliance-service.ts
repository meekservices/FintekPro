...
import { db } from '../db';
import { 
  users, complianceAuditTrail, complianceDocuments, bondOrders, 
  bondHoldings, fixedIncomeSettlements
} from '@shared/schema';
...
export interface PMLAAuditEntry {
  id: string;
  userId: string;
  eventType: 'transaction' | 'kyc_update' | 'account_activity' | 'document_upload' | 'suspicious_activity';
...
const MARGIN_REQUIREMENTS: Record<string, { initial: number; maintenance: number; variation: number }> = {
...
class SEBIComplianceService {
  ...
  async checkComplianceStatus(userId: string): Promise<SEBIComplianceStatus> {
    ...
    let overallStatus: 'compliant' | 'non_compliant' | 'pending_review' | 'suspended';
   ...
   */
  private async checkKYCCompliance(userId: string): Promise<SEBIComplianceCheck> {
    try {
      ...
      if (!user.isProfileCompleted || (user.profileCompleteness && user.profileCompleteness < 80)) {
      ...
      return {
        ...
        details: issues.length === 0 ? 'All KYC requirements met' : issues.join('; '),
        lastChecked: new Date(),
        requiredAction: issues.length > 0 ? 'Complete pending KYC steps' : undefined
      };
    } catch (error: any) {
   ...
   */
  private async checkDematCompliance(userId: string): Promise<SEBIComplianceCheck> {
    try {
      const [user] = await db.select().from(users).where(eq(users.id, userId));
      ...
      return {
        ...
        regulationCode: SEBI_REGULATIONS.DEPOSITORIES.code,
        status: issues.length === 0 ? 'passed' : 'failed',
        details: issues.length === 0 ? 'Demat account properly configured' : issues.join('; '),
        lastChecked: new Date(),
        requiredAction: issues.length > 0 ? 'Link valid demat account' : undefined
      };
    } catch (error: any) {
   ...
   */
  private async checkPMLACompliance(userId: string): Promise<SEBIComplianceCheck> {
    try {
      const [user] = await db.select().from(users).where(eq(users.id, userId));
      ...
      if (user.pepStatus === 'yes' || user.pepStatus === 'related') {
        issues.push(`PEP status requires enhanced due diligence: ${user.pepStatus}`);
      }
      ...
      return {
        regulation: SEBI_REGULATIONS.PMLA.name,
        regulationCode: SEBI_REGULATIONS.PMLA.code,
        status: issues.length === 0 ? 'passed' : (issues.some(i => i.includes('PEP')) ? 'pending' : 'failed'),
        details: issues.length === 0 ? 'PMLA compliance verified' : issues.join('; '),
        lastChecked: new Date(),
        requiredAction: issues.length > 0 ? 'Complete declarations or enhanced due diligence' : undefined
      };
    } catch (error: any) {
   ...
   */
  private async checkFATCACompliance(userId: string): Promise<SEBIComplianceCheck> {
    try {
      const [user] = await db.select().from(users).where(eq(users.id, userId));
      ...
      return {
        regulation: SEBI_REGULATIONS.FATCA_CRS.name,
        regulationCode: SEBI_REGULATIONS.FATCA_CRS.code,
        status: issues.length === 0 ? 'passed' : (user.isUSPerson && issues.length > 0 ? 'failed' : 'passed'),
        details: issues.length === 0 ? 'FATCA/CRS declarations complete' : issues.join('; '),
        lastChecked: new Date(),
        requiredAction: issues.length > 0 ? 'Complete tax declarations' : undefined
      };
    } catch (error: any) {
   ...
   */
  private async checkSuitabilityCompliance(userId: string): Promise<SEBIComplianceCheck> {
    try {
      const [user] = await db.select().from(users).where(eq(users.id, userId));
      ...
      return {
        ...
        regulationCode: SEBI_REGULATIONS.STOCK_BROKERS.code,
        status: issues.length === 0 ? 'passed' : 'failed',
        details: issues.length === 0 ? 'Client suitability requirements met' : issues.join('; '),
        lastChecked: new Date(),
        requiredAction: issues.length > 0 ? 'Complete suitability assessment' : undefined
      };
    } catch (error: any) {
  ...
  async assessProductSuitability(
    ...
    const riskMapping: Record<string, string[]> = {
      ...
      'moderate': ['g_sec', 't_bill', 'sdl', 'tax_free', 'sgb', 'corporate', 'infrastructure'],
      'aggressive': ['g_sec', 't_bill', 'sdl', 'tax_free', 'sgb', 'corporate', 'infrastructure', 'ncd']
    };

    const suitableProducts = riskMapping[userRiskProfile] || riskMapping['conservative'];
    ...
    if (user.investmentExperience === 'none' && ['ncd', 'corporate'].includes(productCategory)) {
      suitabilityScore -= 20;
    ...
    if (user.dateOfBirth) {
      ...
      if (age > 60 && productCategory === 'ncd') {
        suitabilityScore -= 10;
        reasons.push('Long-term products may not be suitable for senior investors');
      }
    }
  ...
  async calculateMarginRequirement(
    ...
    const margins = MARGIN_REQUIREMENTS[productType] || MARGIN_REQUIREMENTS['corporate'];
   ...
   */
  async logPMLAAuditEntry(entry: Omit<PMLAAuditEntry, 'id' | 'retentionExpiry'>): Promise<string> {
    ...
    await db.insert(complianceAuditTrail).values({
      ...
      riskImpact: entry.riskScore > 50 ? 'high' : entry.riskScore > 25 ? 'medium' : 'low',
      complianceImpact: entry.flagged ? 'major' : 'none',
      metadata: entry.metadata
    });

    console.log(`[PMLA Audit] Logged entry ${auditId} for user ${entry.userId}`);
    return auditId;
  }
   ...
   */
  private async getRecentTransactions(userId: string, hoursBack: number): Promise<any[]> {
   ...
   */
  async archiveOrderLog(orderId: string, orderData: any): Promise<OrderLogArchive> {
   ...
   */
  async validateDebtListingCompliance(isin: string, bondData: any): Promise<{
    compliant: boolean;
    issues: string[];
    ...
    if (!bondData.creditRating || !['AAA', 'AA+', 'AA', 'AA-', 'A+', 'A', 'A-'].includes(bondData.creditRating)) {
    ...
    if (!bondData.issuerNetWorth || bondData.issuerNetWorth < 10000000000) { // 100 Crores
  ...
  async generateComplianceReport(
    ...
    const complianceEvents = await db.select()
    ...
    const flaggedCount = complianceEvents.filter(e => 
      e.complianceImpact === 'major' || e.complianceImpact === 'critical'
    ).length;

    const sarCount = complianceEvents.filter(e => 
      e.action === 'transaction' && e.newValue?.includes('"reportedToFIU":true')
    ).length;
    ...
    const report = {
      ...
      statistics: {
        ...
        kycCompletions: complianceEvents.filter(e => e.action === 'kyc_completion').length,
        complianceViolations: complianceEvents.filter(e => e.complianceImpact === 'critical').length
      },
