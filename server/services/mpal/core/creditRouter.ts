...
export class CreditRouter {
	...
	async routeCreditApplication(app: CreditApplication): Promise<any> {
		...
		const scoredOptions = await creditScoringEngine.scoreProviders(
			providers,
			app,
		);
		...
		if (scoredOptions.eligibleProviders.length === 0) {
			throw new Error(
				`Credit Routing Failed: User is not eligible for this product with any provider.`,
			);
		}
		...
		logger.info(
			`[CreditRouter] Routing application for user ${app.userId} to ${bestProviderId}`,
		);
