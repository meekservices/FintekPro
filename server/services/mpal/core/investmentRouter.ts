...
export class InvestmentRouter {
   ...
   */
  private resolveBroker(capability: BrokerCapability, preferredBrokerId?: string) {
    ...
    if (preferredBrokerId) {
      try {
        ...
        if (!(e instanceof BrokerNotFoundError || e instanceof BrokerNotConfiguredError)) throw e;
        logger.warn(`[InvestmentRouter] Preferred broker '${preferredBrokerId}' not available for '${capability}', falling back.`);
      }
    }
  ...
  async executeOrder(
    ...
    logger.info(`[InvestmentRouter] Routing ${capability} order to ${broker.brokerId}`, {
      event: 'ORDER_ROUTED',
      user_id: user.id,
      capability,
      brokerId: broker.brokerId,
    });
    return broker.placeOrder({ ...orderPayload, userId: user.id, capability } as BrokerOrder);
  }
  ...
  async getQuote(assetClass: string, symbol: string) {
    ...
    return { symbol, price: null, message: 'Quote service not yet implemented for this asset class' };
  }
  ...
  private normalizeCapability(assetClass: string): BrokerCapability {
    ...
    if (!cap) {
      throw new Error(`Unknown assetClass '${assetClass}'. Must be one of: ${Object.keys(map).join(', ')}`);
    }
    return cap;
  }
}
