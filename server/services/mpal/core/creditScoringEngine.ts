...
export class CreditScoringEngine {
	...
	async scoreProviders(providers: ICreditProvider[], app: CreditApplication) {
		logger.info(
			`[CreditScoringEngine] Scoring ${providers.length} providers for application ${app.userId}`,
		);

		// 1. Fetch user's unified financial profile (Investments + Liabilities)
		const financialProfile = await financialProfileEngine.buildProfile(
			app.userId,
		);
		...
		for (const provider of providers) {
			try {
				...
				logger.warn(
					`[CreditScoringEngine] Provider ${provider.providerId} failed eligibility check`,
				);
			}
		}
	...
	private calculateRiskScore(profile: any): number {
		...
		const portfolioBonus = Math.min(
			(profile.investmentAllocation.totalValue / 10000) * 10,
			100,
		);
	...
	async scoreUser(userId: number) {
		...
		return {
			...
			approvedAmount:
				score > 600
					? Math.floor(financialProfile.investmentAllocation.totalValue * 0.5)
					: 0,
			reasons:
				score > 700
					? ["Strong Portfolio", "Good Asset-to-Liability Ratio"]
					: ["Enhance portfolio diversity to increase score"],
			breakdown: {
