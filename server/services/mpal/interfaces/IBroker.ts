...
export class BrokerNotFoundError extends BrokerError {
	constructor(brokerId: string) {
		super(
			brokerId,
			"BROKER_NOT_FOUND",
			`Broker '${brokerId}' is not registered in ProviderRegistry.`,
			false,
		);
	}
}
...
export class BrokerNotConfiguredError extends BrokerError {
	constructor(brokerId: string) {
		super(
			...
			`Broker '${brokerId}' is registered but not configured (missing env vars). Set the required credentials to enable it.`,
			false,
		);
	}
}
...
export class BrokerUnavailableError extends BrokerError {
	constructor(capability: BrokerCapability) {
		super(
			"NONE",
			"BROKER_UNAVAILABLE",
			`No configured broker supports capability '${capability}'.`,
			false,
		);
	}
}
...
export class BrokerCapabilityError extends BrokerError {
	constructor(brokerId: string, capability: BrokerCapability) {
		super(
			brokerId,
			"BROKER_CAPABILITY_NOT_SUPPORTED",
			`Broker '${brokerId}' does not support capability '${capability}'.`,
			false,
		);
	}
}
