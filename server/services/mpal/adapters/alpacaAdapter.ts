...
export class AlpacaAdapter extends BaseBroker {
	...
	async healthCheck(timeoutMs = 3000): Promise<BrokerHealthStatus> {
		...
		if (!this.isConfigured()) {
			return {
				...
				configured: false,
				healthy: false,
				message: "ALPACA_API_KEY not set",
				checkedAt,
			};
		}
	...
	async createAccount(user: {
		...
		return alpacaAccountCreator.createAccountForUser(
			user.id,
			"127.0.0.1",
			user.referredByCode,
		);
	...
	async getPositions(accountId: string): Promise<NormalizedPosition[]> {
		...
		return (raw ?? []).map((p: any) => ({
			...
			quantity: Number.parseFloat(p.qty ?? p.quantity ?? "0"),
			averageCost: Number.parseFloat(p.avg_entry_price ?? p.averageCost ?? "0"),
			currentPrice: Number.parseFloat(p.current_price ?? "0") || undefined,
			unrealizedPnl: Number.parseFloat(p.unrealized_pl ?? "0") || undefined,
	...
	async placeOrder(order: BrokerOrder): Promise<BrokerOrderResult> {
		...
		return {
			...
			filledQty: Number.parseFloat(result.filled_qty ?? "0") || undefined,
			filledPrice:
				Number.parseFloat(result.filled_avg_price ?? "0") || undefined,
			_raw: result,
		};
	}
