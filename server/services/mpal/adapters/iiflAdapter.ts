...
export class IIFLAdapter extends BaseBroker {
	...
	async healthCheck(timeoutMs = 3000): Promise<BrokerHealthStatus> {
		...
		if (!this.isConfigured()) {
			return {
				...
				configured: false,
				healthy: false,
				message: "IIFL_API_KEY or IIFL_CLIENT_ID not set",
				checkedAt,
			};
		}
	...
	async createAccount(user: {
		...
		logger.info(`[IIFLAdapter] createAccount requested`, {
			event: "IIFL_CREATE_ACCOUNT",
			user_id: user.id,
			status: "stub",
		});
		if (!this.isConfigured()) {
			return {
				status: "IIFL_NOT_LIVE",
				providerId: null,
				message: "IIFL not configured",
			};
		}
		...
		throw new BrokerError(
			this.brokerId,
			"IIFL_NOT_LIVE",
			"IIFL account creation not yet implemented. Set IIFL credentials and complete TODO.",
			false,
		);
	}
	...
	async getPositions(clientCode: string): Promise<NormalizedPosition[]> {
		...
		return this.callWithRetry(async () => {
			...
			logger.info(`[IIFLAdapter] getPositions stub`, {
				event: "IIFL_GET_POSITIONS",
				user_id: clientCode,
				status: "stub",
			});
			throw new BrokerError(
				this.brokerId,
				"IIFL_NOT_LIVE",
				"IIFL positions not yet implemented.",
				false,
			);
		}, "getPositions");
	}
	...
	async placeOrder(order: BrokerOrder): Promise<BrokerOrderResult> {
		if (!this.isConfigured()) {
			throw new BrokerError(
				this.brokerId,
				"IIFL_NOT_CONFIGURED",
				"IIFL_API_KEY and IIFL_CLIENT_ID must be set to place orders.",
				false,
			);
		}
		return this.callWithRetry(async () => {
			...
			throw new BrokerError(
				this.brokerId,
				"IIFL_NOT_LIVE",
				"IIFL order placement not yet implemented. Complete TODO in iiflAdapter.ts.",
				false,
			);
		}, "placeOrder");
	}
	...
	async cancelOrder(orderId: string): Promise<void> {
		...
		return this.callWithRetry(async () => {
			...
			throw new BrokerError(
				this.brokerId,
				"IIFL_NOT_LIVE",
				"IIFL cancel order not yet implemented.",
				false,
			);
		}, "cancelOrder");
	}
	...
	async getOrderStatus(orderId: string): Promise<BrokerOrderResult> {
		...
		return this.callWithRetry(async () => {
			...
			throw new BrokerError(
				this.brokerId,
				"IIFL_NOT_LIVE",
				"IIFL order status not yet implemented.",
				false,
			);
		}, "getOrderStatus");
	}
