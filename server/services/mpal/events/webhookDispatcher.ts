...
export class WebhookDispatcher {
	...
	async dispatchEvent(
		...
		try {
			switch (providerId.toUpperCase()) {
				case "IRIS":
					await irisWebhookHandler.handleWebhook(
						payload,
				...
				case "IIFL":
					...
					logger.info(
						`[WebhookDispatcher] IIFL webhook received — updating broker_orders`,
						{
							event: "IIFL_WEBHOOK",
				...
				case "M2P":
					logger.info(`[WebhookDispatcher] M2P webhook received`, {
						event: "M2P_WEBHOOK",
						payload,
					});
				...
				case "SETU":
					logger.info(`[WebhookDispatcher] Setu webhook received`, {
						event: "SETU_WEBHOOK",
						payload,
					});
				...
				default:
					logger.warn(
						...
						{ event: "WEBHOOK_UNKNOWN_PROVIDER", providerId },
					);
			...
			logger.error(
				`[WebhookDispatcher] Failed to dispatch event for ${providerId}`,
				{
					event: "WEBHOOK_ERROR",
					providerId,
					latency_ms: Date.now() - start,
	...
	private async updateBrokerOrderFromWebhook(
		...
		try {
			...
			logger.warn(
				`[WebhookDispatcher] Failed to update broker_orders for ${brokerId}/${brokerOrderId}`,
				err,
			);
		}
	}
