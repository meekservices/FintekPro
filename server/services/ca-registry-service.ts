...
function computeTier(entry: {
  ...
  const rating = parseFloat(entry.averageRating ?? '0');
...
export const caRegistryService = {
   ...
   */
  async lookupFromRegistry(icaiNumber: string): Promise<schema.FintekproCaRegistry | null> {
    ...
    try {
      ...
      if (entry.nextRevalidationDue && new Date() > entry.nextRevalidationDue) {
        logger.info('[CARegistry] Entry found but revalidation due — treating as stale', { icaiNumber: normalized });
        return null;
      }
      ...
      logger.warn('[CARegistry] Lookup failed (non-fatal)', { icaiNumber: normalized, err });
      return null;
    }
  },
  ...
  async upsertToRegistry(entry: CaRegistryEntry): Promise<void> {
    ...
    const now = new Date();
    const nextRevalidation = new Date(now.getTime() + 365 * 24 * 60 * 60 * 1000); // +12 months
    ...
    try {
      await db.insert(schema.fintekproCaRegistry).values({
        ...
        verifiedAt: now,
        verifiedBy,
        confidenceScore: confidenceScore !== undefined ? String(confidenceScore) : null,
        ...
        set: {
          ...
          verifiedAt: now,
          verifiedBy,
          confidenceScore: confidenceScore !== undefined ? String(confidenceScore) : sql`confidence_score`,
          verificationSource: verifiedBy,
          rawVerificationResponse: safeResponse ?? sql`raw_verification_response`,
          ...
          ...(partnersTableId && { partnersTableId, isFintekProPartner: true }),
          ...(city && { city }),
          ...(state && { state }),
      ...
      logger.warn('[CARegistry] Upsert failed (non-fatal)', { icaiNumber: normalized, err });
    }
  },
   ...
   */
  async getExpiredRegistryEntries(limit = 50): Promise<schema.FintekproCaRegistry[]> {
    try {
      return await db
        .select()
        .from(schema.fintekproCaRegistry)
  ...
  async markRevalidationFailed(icaiNumber: string): Promise<void> {
    ...
    try {
      ...
      logger.warn('[CARegistry] markRevalidationFailed error', { icaiNumber: normalized, err });
    }
  },
   ...
   */
  async linkPartner(icaiNumber: string, params: {
    userId: string;
    partnersTableId: string;
    city?: string;
    state?: string;
    ...
    try {
      await db.update(schema.fintekproCaRegistry)
        .set({
      ...
      logger.info('[CARegistry] Linked to partner', { icaiNumber: normalized, userId: params.userId });
    } catch (err) {
      logger.warn('[CARegistry] linkPartner failed', { icaiNumber: normalized, err });
    }
  },
   ...
   */
  async redeemReferralCode(code: string): Promise<{ referrerId: string } | null> {
    try {
      const [referrer] = await db
        .select({ id: schema.fintekproCaRegistry.id, userId: schema.fintekproCaRegistry.userId })
        .from(schema.fintekproCaRegistry)
        .where(eq(schema.fintekproCaRegistry.referralCode, code.trim().toUpperCase()))
        .limit(1);
  ...
  async findAvailableCAs(params: {
    ...
    try {
      ...
      return results.filter(ca => {
        if (params.city && ca.city?.toLowerCase() !== params.city.toLowerCase()) return false;
        if (params.state && ca.state?.toLowerCase() !== params.state.toLowerCase()) return false;
        if (params.tier && ca.tier !== params.tier) return false;
        if (params.specialization) {
          const specs = (ca.specializations as string[]) ?? [];
          if (!specs.some(s => s.toLowerCase().includes(params.specialization!.toLowerCase()))) return false;
        }
        return true;
      });
    } catch (err) {
