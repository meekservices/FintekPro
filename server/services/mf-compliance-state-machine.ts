...
import mfLifecycleGlidePathService, {
	LifecycleMetadata,
} from "./mf-lifecycle-glide-path-service";
...
const ADMIN_ONLY_FROM: ComplianceStatus[] = [
	"BLOCKED",
	"OVERLAP_BREACH",
	"GLIDE_PATH_INVALID",
];
...
class MfComplianceStateMachine {
	...
	static getInstance(): MfComplianceStateMachine {
		if (!MfComplianceStateMachine.instance) {
			MfComplianceStateMachine.instance = new MfComplianceStateMachine();
		}
		return MfComplianceStateMachine.instance;
	}
	...
	async transition(
		...
		const fromStatus =
			(fund?.complianceStatus as ComplianceStatus | null) ?? null;
	...
	async runFullComplianceCheck(
		...
		if (!fund) {
			return {
				...
				finalStatus: "PENDING",
				checks: {
					naming: { status: "SKIPPED", reason: "Scheme not found" },
					lifecycle: null,
					overlap: null,
				},
			};
		...
		if (namingResult.status === "FAILED") {
			await this.transition(
				schemeCode,
				"BLOCKED",
				`Naming validation failed: ${namingResult.reason}`,
				triggeredBy,
				true,
			);
		...
		if (fund.lifecycleMetadata && typeof fund.lifecycleMetadata === "object") {
			...
			const lifecycleResult =
				await mfLifecycleGlidePathService.validateAndPersist(schemeCode, meta);
			checks.lifecycle = {
				valid: lifecycleResult.valid,
				violations: lifecycleResult.violations,
			};
      ...
      `);
			...
			if (rows.length > 0) {
				checks.overlap = {
					breached: true,
					overlapPct: Number.parseFloat(rows[0].overlap_percent),
				};
				await this.transition(
					schemeCode,
					"OVERLAP_BREACH",
