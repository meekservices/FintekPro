import { db } from "../db";
import { progressiveCommissionLedger, partnerWallets, reversalLedger } from "@shared/schema";
...
export class PartnerStatementService {
  ...
  static getInstance(): PartnerStatementService {
    if (!this.instance) {
      this.instance = new PartnerStatementService();
    }
    return this.instance;
  }

  async getPayoutStatement(partnerId: string, options: {
    fromDate?: string;
    toDate?: string;
    ...
    const conditions: any[] = [eq(progressiveCommissionLedger.partnerId, partnerId)];
    if (fromDate) {
      conditions.push(gte(progressiveCommissionLedger.createdAt, new Date(fromDate)));
    }
    if (toDate) {
    ...
    const entries = await db.select().from(progressiveCommissionLedger)
      .where(and(...conditions))
      .orderBy(desc(progressiveCommissionLedger.createdAt));

    const reversals = await db.select().from(reversalLedger)
    ...
    for (const r of reversals) {
      ...
      reversalMap.set(r.originalLedgerId, existing + parseFloat(r.reversalAmount?.toString() || '0'));
    }
    ...
    const formattedEntries = entries.map(e => {
      ...
      return {
        ...
        payout_status: reversed >= amount ? 'REVERSED' : reversed > 0 ? 'PARTIALLY_REVERSED' : 'CREDITED',
        created_at: e.createdAt,
      };
    });

    const walletBalance = wallet.length > 0 ? parseFloat(wallet[0].balance?.toString() || '0') : 0;
    const totalDebited = wallet.length > 0 ? parseFloat(wallet[0].totalDebited?.toString() || '0') : 0;
  ...
  private groupByDate(entries: any[], mode: 'day' | 'month') {
    const groups: Record<string, { date: string; entries: any[]; total: number }> = {};
    for (const entry of entries) {
      ...
      groups[key].total += parseFloat(entry.net_amount);
    }
