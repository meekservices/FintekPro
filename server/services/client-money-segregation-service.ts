...
class ClientMoneySegregationService {
  ...
  private initializePaymentFlowDocumentation(): void {
    this.paymentFlowRegistry.set('mutual_fund', {
      ...
      regulatoryBasis: 'SEBI (Mutual Funds) Regulations, 1996 read with AMFI Guidelines',
      segregationCompliant: true,
      paymentChannel: 'BSE Star MF API generates payment URL → Payment to BSE Clearing Corporation → AMC',
      settlementProcess: 'T+1 settlement via ICCL (Indian Clearing Corporation). Units allotted directly to investor folio.',
      auditEvidence: [
    ...
    this.paymentFlowRegistry.set('bonds', {
      ...
      segregationCompliant: true,
      paymentChannel: 'Exchange order placement → Margin blocked in trading account → Exchange settlement',
      settlementProcess: 'T+2 settlement via clearing corporation. Securities credited to demat account.',
      auditEvidence: [
    ...
    this.paymentFlowRegistry.set('unlisted_equity', {
      ...
      regulatoryBasis: 'Companies Act, 2013 Section 56 read with SEBI (Unlisted Securities) Guidelines',
      segregationCompliant: true,
      paymentChannel: 'Cashfree Escrow → SEBI-Registered Trustee Bank → Release after DIS verification',
      settlementProcess: 'Funds held in escrow until share transfer verified via DIS (Delivery Instruction Slip) confirmation from depository.',
      auditEvidence: [
    ...
    this.paymentFlowRegistry.set('aif', {
      ...
      segregationCompliant: true,
      paymentChannel: 'Direct wire transfer to AIF fund account (bank details provided by fund manager)',
      settlementProcess: 'Capital call process. Units allocated post KYC and subscription agreement execution.',
      auditEvidence: [
    ...
    this.paymentFlowRegistry.set('pms', {
      ...
      segregationCompliant: true,
      paymentChannel: 'Direct transfer to PMS custody account maintained with SEBI-registered custodian',
      settlementProcess: 'Funds deposited to designated custody account. Portfolio manager manages investments.',
      auditEvidence: [
    ...
    this.paymentFlowRegistry.set('ipo', {
      ...
      regulatoryBasis: 'SEBI (Issue of Capital and Disclosure Requirements) Regulations, 2018',
      segregationCompliant: true,
      paymentChannel: 'ASBA (Application Supported by Blocked Amount) - Funds blocked in investor bank account',
      settlementProcess: 'ASBA mechanism ensures funds remain in investor account until allotment. Only debited on successful allotment.',
      auditEvidence: [
    ...
    this.paymentFlowRegistry.set('sgb', {
      ...
      segregationCompliant: true,
      paymentChannel: 'Direct subscription via authorized banks or stock exchanges',
      settlementProcess: 'Payment to RBI via authorized channel. SGB units credited to demat account.',
      auditEvidence: [
    ...
    this.paymentFlowRegistry.set('reit_invit', {
      ...
      regulatoryBasis: 'SEBI (Real Estate Investment Trusts) Regulations, 2014 / SEBI (InvITs) Regulations, 2014',
      segregationCompliant: true,
      paymentChannel: 'Exchange-traded: Regular buy order on NSE/BSE',
      settlementProcess: 'T+1 settlement like equity. Units credited to demat account.',
      auditEvidence: [
      ...
      riskFactors: [
        'Liquidity risk for thinly traded units'
      ],
      mitigationControls: [
    ...
    this.paymentFlowRegistry.set('etf', {
      ...
      segregationCompliant: true,
      paymentChannel: 'Exchange order → Broker settlement → Clearing corporation',
    ...
    this.paymentFlowRegistry.set('structured_products', {
      ...
      segregationCompliant: true,
      paymentChannel: 'Direct subscription to issuer. Payment to issuer bank account.',
      settlementProcess: 'Issuance of structured product certificate/demat credit.',
      auditEvidence: [
  ...
  getPaymentFlowDocumentation(productType: InvestmentProduct): PaymentFlowDocumentation | undefined {
  ...
  async logClientMoneyAudit(record: Omit<ClientMoneyAuditRecord, 'id' | 'timestamp'>): Promise<ClientMoneyAuditRecord> {
  ...
  validatePaymentFlow(
    ...
    if (documentation.flowType === 'platform_facilitated') {
      issues.push('CRITICAL: Platform-facilitated payment flow violates SEBI IA Regulations');
      recommendations.push('Route payments directly to regulated counterparty');
      recommendations.push('Remove any FintekPro intermediary accounts from payment chain');
    }

    if (documentation.flowType === 'external_remittance' && !paymentMethod.includes('proof')) {
    ...
    return {
      ...
      issues,
      recommendations: recommendations.length > 0 ? recommendations : documentation.mitigationControls
    };
  }
...
`;
  ...
  generateComplianceArtifacts(): {
    ...
    controlsChecklist: Array<{ control: string; implemented: boolean; evidence: string }>;
  } {
    return {
      ...
      controlsChecklist: [
        {
          ...
          implemented: true,
          evidence: 'Architecture documentation - all payments to external counterparties'
        },
        {
        ...
        },
        {
          ...
          implemented: true,
          evidence: 'Cashfree/PhonePe webhook signature verification in payments/index.ts'
        },
        {
...
export const clientMoneySegregationService = new ClientMoneySegregationService();
