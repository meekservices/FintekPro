...
export class DecentroService {
	...
	async validateAccount(accountNumber: string, ifsc: string, name: string) {
		try {
			...
			if (cached) {
				logger.info(
					`[DecentroService] Cache HIT for account validation: ${accountNumber}`,
				);
				return {
					success: true,
			...
			const response = await axios.post<
				DecentroResponse<Record<string, unknown>>
			>(
			...
			if (success) {
				...
				await db
					...
					.onConflictDoUpdate({
						target: [
							verificationCache.verificationType,
							verificationCache.identifierHash,
						],
						set: {
							verified: true,
							verifiedAt: new Date(),
			...
			logger.error(
				"[DecentroService] Account validation failed:",
	...
	async getBalance(accountNumber: string) {
		try {
			...
			const errorMessage =
				error instanceof Error ? error.message : String(error);
			logger.error(
				"[DecentroService] Balance check failed:",
				errorData || errorMessage,
			);
			return {
				success: false,
	...
	async getTransactions(
		...
		try {
			...
			const errorMessage =
				error instanceof Error ? error.message : String(error);
			logger.error(
				"[DecentroService] Statement fetch failed:",
				errorData || errorMessage,
			);
			return {
				success: false,
