 ...
 */
export function deriveHoldingsFromLots(
	lots: PortfolioLot[],
): TransformedHolding[] {
	...
	return Array.from(map.values()).map((h) => {
		...
		const purchaseLots = h.lots.filter(
		...
		if (h.lots.length > 0) {
			h.firstPurchaseDate = h.lots[0].transactionDate
				.toISOString()
				.split("T")[0];
		}
 ...
 */
export function assertLotsNotDropped(
	holdings: { lots?: any[]; holdingTier?: string }[],
): void {
	...
	if (fullTierWithNoLots.length > 0) {
		console.error(
			`[CRITICAL] CAS_LOTS_DROPPED: ${fullTierWithNoLots.length} FULL tier holdings have no lots`,
		);
		throw new Error(
			`CAS_LOTS_DROPPED: Transaction rows found in CAS but lost during processing (${fullTierWithNoLots.length} holdings affected)`,
		);
	}
}
