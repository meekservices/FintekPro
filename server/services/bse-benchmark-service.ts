...
const BSE_INDICES = [
	{
		...
		provider: "BSE",
		description: "S&P BSE Sensex Total Return Index - Top 30 companies",
	},
	{
		indexCode: "BSE100",
		indexName: "S&P BSE 100",
		provider: "BSE",
		description: "S&P BSE 100 Index - Top 100 companies by market cap",
	},
	{
		indexCode: "BSE200",
		indexName: "S&P BSE 200",
		provider: "BSE",
		description: "S&P BSE 200 Index - Top 200 companies by market cap",
	},
	{
		indexCode: "BSE500",
		indexName: "S&P BSE 500",
		provider: "BSE",
		description: "S&P BSE 500 Index - Top 500 companies by market cap",
	},
	{
		indexCode: "BSE_LARGEMID",
		indexName: "S&P BSE LargeMidCap",
		provider: "BSE",
		description: "S&P BSE Large & Mid Cap Index",
	},
	{
		indexCode: "BSE_MIDCAP",
		indexName: "S&P BSE MidCap",
		provider: "BSE",
		description: "S&P BSE Mid Cap Index",
	},
	{
		indexCode: "BSE_SMALLCAP",
		indexName: "S&P BSE SmallCap",
		provider: "BSE",
		description: "S&P BSE Small Cap Index",
	},
];
...
class BseBenchmarkService {
	...
	async seedBseIndices(): Promise<{ seeded: number; existing: number }> {
		console.log(
			"[BseBenchmark] Seeding BSE indices to market_indices table...",
		);
		...
		console.log(
			`[BseBenchmark] BSE indices seeded: ${seeded} new, ${existing} existing`,
		);
	...
	async resolveBenchmark(mfIsin: string): Promise<BenchmarkResolution> {
		...
		if (fund.length > 0 && fund[0].benchmarkIndex) {
			const bseFromFund = this.checkBseExplicitBenchmark(
				fund[0].benchmarkIndex,
			);
			if (bseFromFund) {
				return {
	...
	private checkBseExplicitBenchmark(
		rawBenchmark: string | null | undefined,
	): string | null {
	...
	private getCategoryDefault(category: string): string | null {
		...
		if (
			normalizedCategory.includes("large") &&
	...
	async autoMapWithBsePrecedence(): Promise<{
		mapped: number;
		updated: number;
		skipped: number;
		...
		console.log(
			"[BseBenchmark] Running BSE-aware auto-mapping with precedence rules...",
		);
		...
		for (const fund of funds) {
			...
			if (existingMapping.length > 0) {
				...
				if (
					existing.indexCode === resolution.indexCode &&
		...
		console.log(
			`[BseBenchmark] Auto-map complete: ${mapped} new, ${updated} updated, ${skipped} skipped, ${lineageRecords} lineage records`,
		);
		return { mapped, updated, skipped, lineageRecords };
	}

	async getBenchmarkLineage(
		mfIsin?: string,
		limit: number = 50,
	): Promise<
		Array<{
	...
	async resolveConflict(
		...
		if (resolution === "accept_amfi") {
			...
			const bseIndex = this.checkBseExplicitBenchmark(
				amfiData[0]?.rawBenchmark,
			);
	...
	async getAmfiBseConflicts(): Promise<
		Array<{
			isin: string;
	...
	> {
		const schemesWithBenchmarks = await db
			.select({
