...
class KycProductEligibilityService {
  ...
  private tierRank(tier: string): number {
    switch (tier) {
      ...
      case 'accredited_investor': return 3;
      default: return 0;
    }
  }
  ...
  private checkConditions(conditions: string[], userState: UserKycState): string[] {
    const missing: string[] = [];
    for (const cond of conditions) {
      switch (cond) {
        case 'AML_OK':
          if (!userState.amlRiskLevel || ['HIGH', 'CRITICAL'].includes(userState.amlRiskLevel)) {
            missing.push('AML_OK');
          }
          break;
        case 'FATCA_SIGNED':
        ...
        case 'MAKER_CHECKER_APPROVED':
          if (!userState.makerCheckerApproved) missing.push('MAKER_CHECKER_APPROVED');
          break;
        case 'FEMA_COMPLIANT':
  ...
  async checkEligibility(userState: UserKycState): Promise<EligibilityResult[]> {
    try {
      ...
      return rules.map(rule => {
        ...
        const tierMet = this.tierRank(userState.kycTier) >= this.tierRank(rule.requiredTier);
        const statusMet = this.tierStatusRank(userState.kycTierStatus) >= this.tierStatusRank(rule.requiredTierStatus || 'final');

        const amlOk = !rule.amlMaxRisk || !userState.amlRiskLevel ||
          this.amlRiskRank(userState.amlRiskLevel) <= this.amlRiskRank(rule.amlMaxRisk);

        const eligible = tierMet && statusMet && missingConditions.length === 0 && amlOk;
  ...
  async checkSingleProduct(productCode: string, userState: UserKycState): Promise<EligibilityResult | null> {
