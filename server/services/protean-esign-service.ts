...
class ProteanESignService {
  ...
  private static readonly SANDBOX_URL = 'https://esign.proteantech.in/esign2.1/2.1';
  private static readonly PRODUCTION_URL = 'https://esign.proteantech.in/esign2.1/2.1';
  ...
  constructor() {
    ...
    this.baseUrl = process.env.PROTEAN_ESIGN_BASE_URL || 
      (this.environment === 'production' ? ProteanESignService.PRODUCTION_URL : ProteanESignService.SANDBOX_URL);
    ...
    try {
      ...
      if (this.isInMockMode()) {
        ...
        return {
          ...
          message: 'OTP sent successfully to Aadhaar-linked mobile (Protean Mock)',
          otpSent: true,
      ...
      if (error instanceof AxiosError) {
        throw new AppError(
          error.response?.data?.message || 'Failed to initiate eSign via Protean',
  ...
  async verifyESign(request: ESignVerifyRequest): Promise<ESignVerifyResponse> {
    try {
      ...
      throw new AppError('Real Protean API not yet implemented', 501, 'NOT_IMPLEMENTED');

    } catch (error) {
      console.error('[Protean eSign] Verify error:', error);
      if (error instanceof AppError) throw error;
      throw new AppError('Failed to verify eSign OTP', 500, 'PROTEAN_ESIGN_VERIFY_FAILED');
    }
  }
  ...
  async resendOTP(transactionId: string): Promise<{ success: boolean; message: string }> {
    try {
      ...
      throw new AppError('Real Protean resend OTP not yet implemented', 501, 'NOT_IMPLEMENTED');

    } catch (error) {
      console.error('[Protean eSign] Resend OTP error:', error);
      if (error instanceof AppError) throw error;
      throw new AppError('Failed to resend OTP', 500, 'PROTEAN_ESIGN_RESEND_FAILED');
    }
  }
  ...
  private buildESignXmlRequest(request: ESignInitiateRequest, transactionId: string): string {
