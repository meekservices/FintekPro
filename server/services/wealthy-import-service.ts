...
export class WealthyImportService {
	...
	private validateUrl(urlString: string): URL {
		...
		if (!this.ALLOWED_HOSTS.includes(parsedUrl.hostname)) {
			throw new Error(
				`Invalid host. Only ${this.ALLOWED_HOSTS.join(", ")} is allowed`,
			);
		}
	...
	async fetchAndParsePortfolio(url: string): Promise<WealthyPortfolio> {
		...
		if (!response.ok) {
			throw new Error(
				`Failed to fetch Wealthy.in report: ${response.status} ${response.statusText}`,
			);
		}
	...
	private parseSummary($: cheerio.CheerioAPI): WealthyPortfolio["summary"] {
		...
		const investedMatch = bodyText.match(
			/Total Invested[^\d₹]*₹?\s*([\d,]+(?:\.\d+)?)/i,
		);
		if (investedMatch) {
		...
		if (growthMatch) {
			growth = Number.parseFloat(growthMatch[1]);
		}

		const equityMatch = bodyText.match(/([\d.]+)%\s*Equity/i);
		if (equityMatch) {
			equityPercent = Number.parseFloat(equityMatch[1]);
		}

		const debtMatch = bodyText.match(/([\d.]+)%\s*Debt/i);
		if (debtMatch) {
			debtPercent = Number.parseFloat(debtMatch[1]);
		}
	...
	private parseHoldingSection(section: string): WealthyHolding | null {
		...
		if (/Debt|Bond|Liquid|Money Market|Overnight|Ultra Short/i.test(section)) {
			...
			/Hybrid|Balanced|Multi.?Asset|Dynamic Asset|Conservative/i.test(section)
		) {
			assetType = "Hybrid";
		}
		...
		const investedMatch = section.match(
			/Invested[^\d₹]*₹?\s*([\d,]+(?:\.\d+)?)/i,
		);
		const invested = investedMatch
			? this.parseIndianNumber(investedMatch[1])
			: 0;

		const currentMatch = section.match(
			/Current[^\d₹]*₹?\s*([\d,]+(?:\.\d+)?)/i,
		);
		const currentValue = currentMatch
			? this.parseIndianNumber(currentMatch[1])
			: 0;

		const returnsMatch = section.match(/Returns[^\d%-]*(-?[\d.]+)\s*%/i);
		const returns = returnsMatch ? Number.parseFloat(returnsMatch[1]) : 0;

		const folioMatch = section.match(/Folio[:\s]*([^\s*]+)/i);
		const folio = folioMatch ? folioMatch[1].replace(/\*+$/, "") : "";

		const arnMatch = section.match(
			/(?:ARN|Wealth Partner)[:\s]*([A-Z]{3}[0-9\-]+)/i,
		);
		...
		const longTermUnits = ltUnitsMatch ? Number.parseFloat(ltUnitsMatch[1]) : 0;

		const stUnitsMatch = section.match(
			/Short Term Units[^\d]*(\d+(?:\.\d+)?)/i,
		);
		const shortTermUnits = stUnitsMatch
			? Number.parseFloat(stUnitsMatch[1])
			: 0;

		const isDemat = /Demat[:\s]*Yes/i.test(section);

		const transactions: WealthyTransaction[] = [];
		const txMatches = section.matchAll(
			/Purchase\s*\|\s*([\d.]+)\s*\|\s*₹?\s*([\d,.]+)\s*\|\s*([\d.]+)\s*\|\s*([\d-]+)/gi,
		);
		for (const match of txMatches) {
			transactions.push({
				category: "Purchase",
				units: Number.parseFloat(match[1]),
				amount: this.parseIndianNumber(match[2]),
				nav: Number.parseFloat(match[3]),
				navDate: match[4],
			});
		}
	...
	private parseAlternateFormat(
		...
		for (const pattern of fundPatterns) {
			...
			while ((match = pattern.exec(bodyText)) !== null) {
				...
				const investedMatch = context.match(
					/Invested[^\d₹]*₹?\s*([\d,]+(?:\.\d+)?)/i,
				);
				const invested = investedMatch
					? this.parseIndianNumber(investedMatch[1])
	...
	async importToExternalHoldings(
		userId: string,
		portfolio: WealthyPortfolio,
	): Promise<{
		imported: number;
		skipped: number;
		holdings: Array<{
