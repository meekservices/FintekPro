...
class FeeCalculatorService {
  ...
  async getApplicableFees(productType: string): Promise<PlatformFeeConfig[]> {
    ...
    return this.feeCache.filter(fee => 
      fee.applicableTo === 'all' || fee.applicableTo === productType
    );
  }
  ...
  calculateSingleFee(
    ...
    const tierRates = fee.investorTierRates as Record<string, number> | null | undefined;
    let rate = parseFloat(fee.rateValue || '0');
    if (isNaN(rate)) rate = 0;

    if (tierRates && typeof tierRates === 'object' && tierRates[investorTier] !== undefined) {
      const tierRate = tierRates[investorTier];
      if (typeof tierRate === 'number' && !isNaN(tierRate)) {
        rate = tierRate;
      }
    }
    ...
    switch (fee.chargeType) {
      ...
      case 'tiered':
        const slabs = fee.tierSlabs as Array<{ from: number; to: number; rate: number }> | null | undefined;
        if (slabs && Array.isArray(slabs) && slabs.length > 0) {
          const applicableSlab = slabs.find(s => 
            s && typeof s.from === 'number' && typeof s.to === 'number' && typeof s.rate === 'number' &&
            amount >= s.from && amount <= s.to
          );
          if (applicableSlab) {
      ...
      case 'hybrid':
        const flatPart = parseFloat(fee.minAmount || '0') || 0;
        baseAmount = flatPart + (amount * (rate / 100));
        rateApplied = `₹${flatPart} + ${rate}%`;
        break;
    }

    const minAmount = parseFloat(fee.minAmount || '0') || 0;
    ...
    if (maxAmount !== null && !isNaN(maxAmount) && baseAmount > maxAmount) baseAmount = maxAmount;

    let waiverAmount = 0;
    let isWaived = false;
    if (applyWaiver && fee.isWaivable && waiverPercent > 0) {
      const maxWaiver = parseFloat(fee.maxWaiverPercent || '0') || 0;
      const effectiveWaiver = Math.min(waiverPercent, maxWaiver > 0 ? maxWaiver : 100);
    ...
    if (fee.isGstApplicable) {
      const gstRate = parseFloat(fee.gstRate || '18') || 18;
  ...
  async getSTT(productType: string, amount: number, side: 'buy' | 'sell' = 'both'): Promise<number> {
    const fees = await this.getApplicableFees(productType);
  ...
  async getBrokerage(productType: string, amount: number, investorTier: string = 'retail'): Promise<number> {
    const fees = await this.getApplicableFees(productType);
    const brokerageFee = fees.find(f => f.feeCode.startsWith('BROKERAGE_'));
    
    if (!brokerageFee) return 0;

    const calculated = this.calculateSingleFee(brokerageFee, amount, investorTier);
    return calculated.baseAmount;
  }
