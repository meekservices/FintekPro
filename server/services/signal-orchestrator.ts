...
const DEFAULT_GOVERNANCE_RULES: GovernanceRule[] = [
	{
		...
		priority: "high",
		description:
			"POTD BUY conflicts with Rebalance SELL — partial sell instead of full",
		enabled: true,
	},
	{
	...
	},
	{
		...
		priority: "medium",
		description:
			"POTD BUY takes precedence over Rebalance HOLD for new positions",
		enabled: true,
	},
	{
	...
	},
	{
		...
		priority: "medium",
		description:
			"POTD BUY with no Rebalance signal — fresh investment suggestion",
		enabled: true,
	},
	{
		...
		priority: "high",
		description:
			"POTD BUY conflicts with Rebalance REDUCE — partial reduction preserving POTD upside",
		enabled: true,
	},
	{
		...
		priority: "high",
		description:
			"POTD BUY reinforced by Rebalance INCREASE — stronger buy signal",
		enabled: true,
	},
	{
...
class SignalOrchestrator {
	...
	constructor(rules?: GovernanceRule[], toleranceBands?: ToleranceBandConfig) {
	...
	private findRule(
		...
		const mappedSignal =
			rebalanceSignal === "HOLD_COST_FILTER"
				? "HOLD"
				: rebalanceSignal === "HOLD_RISK_LIMIT"
					? "HOLD"
					: rebalanceSignal;
		return (
			this.governanceRules.find(
				(r) =>
					...
					(r.rebalanceSignal === rebalanceSignal ||
						r.rebalanceSignal === mappedSignal),
			) || null
		);
	}
	...
	private buildOrchestratedRecommendation(
		...
		const resolvedAction =
			rule?.resolvedAction || (rec.action as OrchestratedAction);
		...
		if (
			resolvedAction === "REDUCE" &&
			(rec.action === "REDUCE" || rec.action === "SELL")
		) {
