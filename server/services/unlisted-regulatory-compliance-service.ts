...
class UnlistedRegulatoryComplianceService {
   ...
   */
  async checkInvestorLimit(companyId: string, userId: string): Promise<{
    allowed: boolean;
    currentCount: number;
    maxAllowed: number;
  ...
  async recordInvestorTransaction(data: {
    ...
    try {
      ...
      // Check limit first
      const limitCheck = await this.checkInvestorLimit(data.companyId, data.userId);
      ...
      if (limitCheck.isExistingInvestor) {
        ...
        await db.update(unlistedInvestorTracking)
          .set({
      ...
      const [record] = await db.insert(unlistedInvestorTracking)
        .values(insertData)
   ...
   */
  async checkLockInStatus(companyId: string, userId: string, sharesToSell: number): Promise<{
    canSell: boolean;
    ...
    const lockInRecords = await db.select()
  ...
  async recordShareAcquisition(data: {
    ...
    try {
      ...
      console.log(`[RegCompliance] Recorded lock-in for ${data.sharesAcquired} shares until ${lockInEndDate.toISOString()}`);
  ...
  async checkCompanyListingStatus(companyId: string): Promise<{
    ...
    try {
      ...
      const isListed = statusLower.includes('listed') || 
      ...
      return {
        ...
        source: 'mca',
        listingDate: mcaData.listingDate ? new Date(mcaData.listingDate) : undefined,
      };
    } catch (error: any) {
   ...
   */
  async handleListingStatusChange(companyId: string, data: {
    newStatus: string;
    listingDate?: Date;
    exchangeSymbol?: string;
    exchangeName?: string;
    ...
    try {
      ...
      const logData: InsertUnlistedCompanyStatusLog = {
        ...
        suspensionReason: isNowListed ? 'Company is now listed. P2P trading disabled. Use exchange for transactions.' : undefined,
  ...
  async detectRedFlags(transaction: {
    ...
    const flags: { type: string; severity: 'low' | 'medium' | 'high' | 'critical'; reason: string }[] = [];
    ...
    if (transaction.amount >= HIGH_VALUE_THRESHOLD * 0.8 && transaction.amount < HIGH_VALUE_THRESHOLD) {
      flags.push({
  ...
  }): Promise<{ success: boolean; flagId?: string; dueDate?: Date; error?: string }> {
    try {
      const now = new Date();
      ...
      const [record] = await db.insert(unlistedSTRFlags)
        .values(insertData)
        .returning({ id: unlistedSTRFlags.id });
      
      console.log(`[RegCompliance] STR flag recorded: ${data.flagType} (${data.severity}) - Due: ${dueDate.toISOString()}`);
   ...
   */
  async markSourceOfFundsVerified(companyId: string, userId: string, verifiedBy: string): Promise<{ success: boolean }> {
    try {
      const fy = getCurrentFinancialYear();
      ...
      console.log(`[RegCompliance] Source of funds verified for user ${userId} by ${verifiedBy}`);
  ...
  async getComplianceOverview(): Promise<{
    ...
    // Investor limits
    const investorCounts = await db.select({
    ...
    const unlockingThisMonth = await db.select({ count: count() })
      .from(unlistedShareLockIn)
      .where(and(
        eq(unlistedShareLockIn.isActive, true),
    ...
    const filedThisMonth = await db.select({ count: count() })
      .from(unlistedSTRFlags)
      .where(and(
    ...
    // Status changes
    const listedThisMonth = await db.select({ count: count() })
...
export const regulatoryComplianceService = new UnlistedRegulatoryComplianceService();
