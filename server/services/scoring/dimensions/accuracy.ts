...
export function calculateAccuracyScore(rec: TrackedRecommendation, outcome: OutcomeData): ScoreDimensionResult {
  let score = 100;
  const factors = [];
  ...
  if (predicted !== undefined && actual !== undefined) {
    ...
    factors.push(`Predicted Return: ${predicted}%, Actual: ${actual}%. Penalty applied: ${penalty.toFixed(2)}`);
  } else {
    // If no predicted vs actual returns are provided, we default to neutral 50 but log factor
    score = 50;
    factors.push("Missing predicted or actual return metrics to perform absolute accuracy score.");
  }
