...
import { ScoringEngine, TrackedRecommendation, OutcomeData, ScoreResult } from "./types";
...
export class AIRecommendationScoringEngine implements ScoringEngine {
  ...
  async calculateScore(recommendation: TrackedRecommendation, outcome: OutcomeData): Promise<ScoreResult> {
    ...
    const outcomeQuality = calculateOutcomeQualityScore(recommendation, outcome);
    ...
    }).execute().catch(e => console.error("[ARSE Core]: Failed to log score to database", e));

    // 4. Trigger Feedback Loop Asynchronously
    aiFeedbackService.processScoreFeedback(recommendation.model_version, totalScore, finalResult)
