...
class ErrorTrackingService {
	async ingestError(
		payload: ErrorIngestionPayload,
		ipAddress?: string,
	): Promise<ErrorLedgerEntry> {
	...
	): Promise<{ errors: ErrorLedgerEntry[]; total: number }> {
		const conditions = [];
	...
	async getMetrics(dateFrom?: Date, dateTo?: Date): Promise<ErrorMetrics> {
		...
		const topErrorCodesResult = await db
			.select({
		...
		const [impactResult] = await db
			...
			.from(errorLedger)
			.where(and(whereClause, eq(errorLedger.severity, "critical")));
		...
		byModuleResult.forEach((r) => {
			byModule[r.module] = Number(r.count);
		});
...
Respond ONLY in valid JSON format:
{
  ...
  "confidence": 85
			...
			if (jsonMatch) {
				...
				console.log(
					`🤖 AI error analysis completed for ${errorId} using Gemini`,
				);
				return analysis;
			}
	...
	async analyzeErrorPatterns(errorIds: string[]): Promise<{
		...
		const errors = await Promise.all(
			errorIds.map((id) => this.getErrorById(id)),
		);
    ...
    `;
		...
		for (const email of emails) {
			try {
				...
				console.error(
					`[CRITICAL ALERT] Failed to send email to ${email}:`,
					err,
				);
			}
		}
	...
	private async sendWebhookAlert(error: ErrorLedgerEntry, webhookUrl: string) {
		try {
			...
			if (response.ok) {
				...
				console.error(
					`[CRITICAL ALERT] Webhook failed with status ${response.status}`,
				);
			}
		} catch (err) {
...
`;
			...
			return {
				success: false,
				errorCount: 0,
				errors: ["Failed to generate batch report"],
			};
		}
	}
}
...
export async function logErrorWithTraceId(
	...
	const severity =
		error.status >= 500 ? "error" : error.status >= 400 ? "warning" : "info";
	...
	try {
		...
		console.error(
			"[ERROR_TRACKING] Failed to log error with trace ID:",
			traceId,
			trackingError,
		);
	}
