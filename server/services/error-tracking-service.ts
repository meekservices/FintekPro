...
Respond ONLY in valid JSON format:
{
  ...
  "confidence": 85
      ...
      const response = await aiService.chat(
        ...
        { provider: 'gemini', model: 'gemini-3-flash-preview', temperature: 0.3, maxTokens: 2048 }
      );
...
Respond in JSON:
{
  ...
  "prioritizedFixes": [
    ...
    {"errorId": "...", "priority": 2, "fix": "..."}
      ...
      const response = await aiService.chat(
        [{ role: 'user', content: prompt }],
        { provider: 'gemini', model: 'gemini-3-flash-preview', temperature: 0.2, maxTokens: 2048 }
      );
