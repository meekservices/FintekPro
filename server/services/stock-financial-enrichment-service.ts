...
class StockFinancialEnrichmentService {
	...
	private async fmpFetch<T>(
		...
		try {
			...
			if (
				...
				typeof d === "object" &&
				!Array.isArray(d) &&
				d["Error Message"]?.includes("Legacy Endpoint")
			) {
				return null;
			}
			return d as T;
	...
	async fetchFmpComprehensive(
		symbol: string,
	): Promise<Partial<StockFinancials> | null> {
		...
		if (profileData?.[0]) {
			...
			if (p.lastDiv != null && p.price) {
				...
				if (divYield > 0 && divYield < 50)
					result.dividendYield = Math.round(divYield * 10000) / 10000;
			}
			gotData = true;
		}
		...
		if (ratiosData?.[0]) {
			...
			if (r.returnOnEquity != null)
				result.roe = Math.round(r.returnOnEquity * 10000) / 100;
			if (r.returnOnCapitalEmployed != null)
				result.roce = Math.round(r.returnOnCapitalEmployed * 10000) / 100;
			...
			if (r.netProfitMargin != null)
				result.profitMargin = Math.round(r.netProfitMargin * 10000) / 100;
			if (r.revenueGrowth != null)
				result.revenueGrowth = Math.round(r.revenueGrowth * 10000) / 100;
			gotData = true;
		}

		if (quoteData?.[0]) {
			const q = quoteData[0];
			if (q.avgVolume && !result.averageVolume)
				result.averageVolume = q.avgVolume;
	...
	async fetchFmpReturnsAndAnalyst(
		symbol: string,
	): Promise<Partial<StockFinancials>> {
		const fmpSymbol = this.toFmpSymbol(symbol);
		const result: Partial<StockFinancials> = {};
		...
		const [histData, ratingData, priceTargetData] = await Promise.all([
			this.fmpFetch<any>(`/historical-price-full/${fmpSymbol}`, {
				from: from5y,
				to: toDate,
			}),
		...
		if (histData?.historical && histData.historical.length > 0) {
			const prices = histData.historical.sort(
				(a: any, b: any) =>
	...
	private findReturn(
		...
		return (
			Math.round(((currentPrice - closest.close) / closest.close) * 10000) / 100
		);
	}

	private mapAnalystRating(recommendation: string): string {
		const lower = recommendation.toLowerCase();
		if (lower.includes("strong buy") || lower === "a+" || lower === "a")
			return "Strong Buy";
		...
		if (lower.includes("hold") || lower.includes("neutral") || lower === "c")
			return "Hold";
	...
	async fetchYahooFinancials(
		...
		try {
			...
			const response = await axios.get(
				this.YAHOO_FINANCE_URL + "/" + yahooSymbol,
				{
					params: {
			...
			return {
				symbol,
				peRatio:
					this.extractValue(summary.trailingPE) ||
					this.extractValue(price.regularMarketPE),
				...
				dividendYield: this.extractValue(summary.dividendYield)
					? this.extractValue(summary.dividendYield)! * 100
					: null,
				...
				roe: this.extractValue(financials.returnOnEquity)
					? this.extractValue(financials.returnOnEquity)! * 100
					: null,
				revenueGrowth: this.extractValue(financials.revenueGrowth)
					? this.extractValue(financials.revenueGrowth)! * 100
					: null,
				profitMargin: this.extractValue(financials.profitMargins)
					? this.extractValue(financials.profitMargins)! * 100
					: null,
	...
	private async fetchFinnhubFinancials(
		symbol: string,
	): Promise<Partial<StockFinancials> | null> {
		...
		try {
			...
			const metricsResp = await axios.get(
			...
			if (m.bookValuePerShareQuarterly)
				result.bookValue = m.bookValuePerShareQuarterly;
	...
	private inferFromSector(
		...
		const inferredBookValue =
			averages.pb > 0 ? currentPrice / averages.pb : null;

		return {
			peRatio: averages.pe,
			eps: inferredEps != null ? Math.round(inferredEps * 100) / 100 : null,
			bookValue:
				inferredBookValue != null
					? Math.round(inferredBookValue * 100) / 100
					: null,
	...
	private mergeFinancials(
		base: Partial<StockFinancials>,
		override: Partial<StockFinancials> | null | undefined,
	): Partial<StockFinancials> {
	...
	async enrichAllStocks(
		...
		if (!hasProductionDb()) {
			console.warn(
				"[Stock Enrichment] PRODUCTION_DATABASE_URL not set. Enrichment runs on production only. Aborting.",
			);
			return {
				totalStocks: 0,
				stocksWithNullPe: 0,
		...
		console.log(
			"[Stock Enrichment] ✅ Connected to PRODUCTION database for enrichment",
		);
		...
		enrichmentProgress.currentStep =
			"Querying stocks needing enrichment (production DB)...";
		...
		try {
			...
			stats.stocksWithNullPe = stocksToProcess.filter(
				(s) => s.peRatio === null,
			).length;
			stats.stocksWithNullEps = stocksToProcess.filter(
				(s) => s.eps === null,
			).length;
				...
				for (const stock of batch) {
					try {
						...
						if (useFmp && process.env.FMP_API_KEY) {
							...
							if (
								includeReturns &&
								(stock.returns1Y === null || stock.returns1M === null)
							) {
								const returnsData = await this.fetchFmpReturnsAndAnalyst(
									stock.symbol,
								);
						...
						if (
							useYahoo &&
							yahooRequestCount < maxYahooRequests &&
							this.rateLimitRemaining > 0
						) {
						...
						if (!financials.peRatio && !financials.eps) {
							try {
								const finnhubData = await this.fetchFinnhubFinancials(
									stock.symbol,
								);
								if (finnhubData) {
								...
								console.warn(
									"[StockEnrichment] Finnhub enrichment failed:",
									enrichErr?.message,
								);
							}
						}
						...
						if (!financials.peRatio) {
							const currentPrice = stock.currentPrice
								...
								: null;
							const sectorData = this.inferFromSector(
								stock.sector,
								currentPrice,
							);
						...
						if (financials && Object.keys(financials).length > 0) {
							...
							if (
								stock.analystRating === null &&
								financials.analystRating != null
							) {
							...
							if (
								stock.numberOfAnalysts === null &&
								financials.numberOfAnalysts != null &&
								Number.isFinite(financials.numberOfAnalysts)
							) {
								updates.numberOfAnalysts = Math.round(
									financials.numberOfAnalysts,
								);
							}
							if (
								stock.averageVolume === null &&
								financials.averageVolume != null &&
								Number.isFinite(financials.averageVolume)
							) {
								updates.averageVolume = Math.round(
									financials.averageVolume,
								).toString();
							}
							...
							if (
								stock.broadSector === null &&
								financials.broadSector != null
							) {
							...
							if (Object.keys(updates).length > 0) {
								...
								console.log(
									`[Stock Enrichment] Enriched ${stock.symbol}: ${Object.keys(
										updates,
									)
										.filter(
											(k) =>
		...
		console.log(
			`[Stock Enrichment] Completed in ${stats.duration}ms: ${stats.stocksEnriched} stocks enriched, ${this.fmpCallCount} FMP API calls used`,
		);
	...
	async getEnrichmentStats(): Promise<{
		...
		return {
			...
			percentEnriched:
				total > 0 ? Math.round((fullyEnriched / total) * 100) : 0,
		};
	}
}

export const stockFinancialEnrichmentService =
	new StockFinancialEnrichmentService();
