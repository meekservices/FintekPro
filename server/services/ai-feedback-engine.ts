...
export type InteractionType = 'view' | 'click' | 'invest' | 'exit' | 'watchlist_add' | 'share' | 'dismiss';
...
class AIFeedbackEngine {
  ...
  async logInteraction(event: InteractionEvent): Promise<void> {
    try {
      if (!event.userId || !event.pickId || !event.interactionType) {
        console.warn('[AIFeedbackEngine] Invalid interaction event, missing required fields');
        return;
      }
      ...
      if (!pick) {
        console.warn(`[AIFeedbackEngine] Pick ${event.pickId} not found, skipping interaction`);
        return;
      }
      ...
      if (event.interactionType === 'invest' || event.interactionType === 'exit') {
        setImmediate(() => {
          this.updateUserProfile(event.userId).catch(err =>
            console.error(`[AIFeedbackEngine] Async profile update failed for ${event.userId}:`, err)
          );
        });
      }
    } catch (error) {
  ...
  async logBatchInteractions(events: InteractionEvent[]): Promise<void> {
    try {
      ...
      const validEvents = events.filter(e => e.userId && e.pickId && e.interactionType);
      ...
      const usersToUpdate = new Set(
        validEvents
          .filter(e => e.interactionType === 'invest' || e.interactionType === 'exit')
          .map(e => e.userId)
      );
      ...
      for (const userId of usersToUpdate) {
        setImmediate(() => {
          this.updateUserProfile(userId).catch(err =>
            console.error(`[AIFeedbackEngine] Async batch profile update failed for ${userId}:`, err)
          );
        });
      }
    } catch (error) {
      console.error('[AIFeedbackEngine] Error in batch interaction logging:', error);
    }
  }
  ...
  async updateUserProfile(userId: string): Promise<UserProfile> {
    ...
    const pickDetailsMap = new Map<number, { category: string; riskLevel: string | null }>();
    ...
    if (pickIds.length > 0) {
      const picks = await db
        ...
        .from(dailyPicks)
        .where(sql`${dailyPicks.id} IN (${sql.join(pickIds.map(id => sql`${id}`), sql`, `)})`);

      for (const p of picks) {
        pickDetailsMap.set(p.id, { category: p.category, riskLevel: p.riskLevel });
      }
    }

    const investInteractions = interactions.filter(i => i.interactionType === 'invest');
    const exitInteractions = interactions.filter(i => i.interactionType === 'exit');
    ...
    const interactionsLast30d = interactions.filter(i => i.createdAt && new Date(i.createdAt) >= thirtyDaysAgo).length;
    const interactionsLast7d = interactions.filter(i => i.createdAt && new Date(i.createdAt) >= sevenDaysAgo).length;
    const engagementScore = Math.min(100, interactionsLast30d * 3 + interactionsLast7d * 10);
    ...
    if (holdingDaysValues.length > 0) {
      avgHoldingDays = Math.round(holdingDaysValues.reduce((a, b) => a + b, 0) / holdingDaysValues.length * 100) / 100;
    }
    ...
    if (investAmounts.length > 0) {
      avgInvestmentAmount = Math.round(investAmounts.reduce((a, b) => a + b, 0) / investAmounts.length * 100) / 100;
    }
    ...
    if (exitInteractions.length > 0) {
      ...
      profitableTradesRatio = Math.round((profitableTrades / exitInteractions.length) * 10000) / 10000;
    }

    const preferredRiskLevel = this.mapRiskScore(riskToleranceScore);

    const lastActiveAt = interactions.length > 0 && interactions[0].createdAt
  ...
  async getUserProfile(userId: string): Promise<UserProfile | null> {
    ...
    if (existing) {
      return {
        ...
        riskToleranceScore: parseFloat(existing.riskToleranceScore || '50'),
        engagementScore: parseFloat(existing.engagementScore || '0'),
        preferredCategories: (existing.preferredCategories as string[]) || [],
        avgHoldingDays: parseFloat(existing.avgHoldingDays || '0'),
        avgInvestmentAmount: parseFloat(existing.avgInvestmentAmount || '0'),
        totalInteractions: existing.totalInteractions || 0,
        investmentCount: existing.investmentCount || 0,
        profitableTradesRatio: parseFloat(existing.profitableTradesRatio || '0'),
        preferredRiskLevel: existing.preferredRiskLevel || 'medium',
        lastActiveAt: existing.lastActiveAt ? new Date(existing.lastActiveAt).toISOString() : new Date().toISOString(),
      };
    }
  ...
  async getPersonalizedPicks(userId: string, limit: number = 10): Promise<PersonalizedPick[]> {
      ...
      const recentViewInteractions = await db
        ...
        .where(
          and(
            ...
            gte(aiUserInteractions.createdAt, new Date(Date.now() - 7 * 24 * 60 * 60 * 1000))
          )
        );
    ...
    const personalizedPicks: PersonalizedPick[] = livePicks.map(pick => {
      const factors = this.computePersonalizationFactors(pick, profile, recentViews);

      const baseScore = (pick.confidenceScore || 70) / 100;
      const boost = factors.categoryPreference + factors.riskAlignment + factors.engagementBoost - factors.recencyPenalty;
      ...
      if (categoryMatch) matchReasons.push(`Matches your preferred category: ${pick.category}`);
      if (riskMatch) matchReasons.push(`Risk level aligns with your profile: ${pick.riskLevel}`);
      if (factors.engagementBoost > 0.01) matchReasons.push('Boosted for active engagement');
  ...
  async getInteractionHistory(userId: string, limit: number = 50): Promise<any[]> {
  ...
  async getEngagementStats(userId: string): Promise<any> {
    ...
    for (const i of interactions) {
      interactionsByType[i.interactionType] = (interactionsByType[i.interactionType] || 0) + 1;
    }
    ...
    const investEvents = interactions.filter(i => i.interactionType === 'invest');
    ...
    const winRate = exitEvents.length > 0 ? Math.round((wins / exitEvents.length) * 10000) / 10000 : 0;
  ...
  private computePersonalizationFactors(
    ...
    const top3Categories = profile.preferredCategories.slice(0, 3);
    const categoryPreference = top3Categories.includes(pick.category) ? 0.15 : 0;

    const riskAlignment = pick.riskLevel === profile.preferredRiskLevel ? 0.10 : 0;
