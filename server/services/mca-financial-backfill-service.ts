...
class McaFinancialBackfillService {
	...
	private calculateCompleteness(
		snapshot: Partial<McaFinancialSnapshot>,
	): number {
		let filled = 0;
	...
	async backfillFromCompanyFinancials(
		cin: string,
		userId?: string,
					...
					const updateResult = await this.updateFinancialSnapshot(
						{
							...
							revenue: fin.revenue ? Number.parseFloat(fin.revenue) : undefined,
							profitBeforeTax: fin.pbt ? Number.parseFloat(fin.pbt) : undefined,
							profitAfterTax: fin.pat ? Number.parseFloat(fin.pat) : undefined,
							netWorth: fin.networth
								? Number.parseFloat(fin.networth)
								: undefined,
							totalAssets: fin.totalAssets
								? Number.parseFloat(fin.totalAssets)
								: undefined,
							totalLiabilities: fin.totalLiabilities
								? Number.parseFloat(fin.totalLiabilities)
								: undefined,
							shareCapital: fin.shareCapital
								? Number.parseFloat(fin.shareCapital)
								: undefined,
							reserves: fin.reserves
								? Number.parseFloat(fin.reserves)
								: undefined,
							longTermBorrowing: fin.longTermDebt
								? Number.parseFloat(fin.longTermDebt)
								: undefined,
							shortTermBorrowing: fin.shortTermDebt
								? Number.parseFloat(fin.shortTermDebt)
								: undefined,
							ebitda: fin.ebitda ? Number.parseFloat(fin.ebitda) : undefined,
							operatingCashFlow: fin.operatingCashFlow
								? Number.parseFloat(fin.operatingCashFlow)
								: undefined,
						},
						userId,
					);
	...
	async getFieldCoverageStats(): Promise<{
		...
		const fields = FINANCIAL_FIELDS.map((field) => {
			...
			return {
				...
				coverage:
					totalSnapshots > 0
						? Math.round((filledCount / totalSnapshots) * 100)
						: 0,
			};
		});
	...
	async getCoverageStats(): Promise<CoverageStats> {
		...
		for (const snapshot of snapshots) {
			...
			if (snapshot.revenue && Number.parseFloat(snapshot.revenue) > 0)
		...
		return {
			...
			averageCompleteness:
				snapshots.length > 0
					? Math.round(totalCompleteness / snapshots.length)
					: 0,
		};
	}

	async getCompaniesNeedingBackfill(): Promise<
		Array<{
			cin: string;
			companyName: string;
	...
	async getFinancialSummaryByCIN(cin: string): Promise<{
		...
		const years = snapshots.map((s) => ({
			...
			revenue: s.revenue ? Number.parseFloat(s.revenue) : null,
			pat: s.profitAfterTax ? Number.parseFloat(s.profitAfterTax) : null,
			netWorth: s.netWorth ? Number.parseFloat(s.netWorth) : null,
			totalAssets: s.totalAssets ? Number.parseFloat(s.totalAssets) : null,
