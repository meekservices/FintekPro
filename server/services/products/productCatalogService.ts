...
export class ProductCatalogService {
  ...
  async getMutualFunds(forceRefresh = false) {
    return this.getCachedProducts('MUTUAL_FUND', async () => {
      const rawProducts = await irisClient.fetchProducts('mf');
      // Assume rawProducts is an array of schemes
      return (rawProducts?.schemes || []).map((p: any) => IrisProductMapper.normalizeMutualFund(p));
    }, forceRefresh);
  }

  async getFixedDeposits(forceRefresh = false) {
    return this.getCachedProducts('FIXED_DEPOSIT', async () => {
      const rawProducts = await irisClient.fetchProducts('fd');
      // Assume rawProducts is an array of FDs
      return (rawProducts?.products || []).map((p: any) => IrisProductMapper.normalizeFixedDeposit(p));
    }, forceRefresh);
  }

  async getPmsProducts(forceRefresh = false) {
    return this.getCachedProducts('PMS', async () => {
  ...
  async getAifProducts(forceRefresh = false) {
    return this.getCachedProducts('AIF', async () => {
  ...
  private async getCachedProducts(key: string, fetchFn: () => Promise<any>, forceRefresh: boolean) {
    ...
    try {
      ...
      logger.error(`[ProductCatalog] Failed to fetch products for ${key}`, { error: error.message });
      // If we have stale cache, return it as fallback
      if (cache.has(key)) {
        logger.warn(`[ProductCatalog] Returning stale cache for ${key} due to fetch error`);
