...
export class ProductEligibilityEngine {
   ...
   */
  evaluateEligibility(user: UserProfile, product: Product): { eligible: boolean; reasons: string[] } {
    ...
    switch (product.assetClass) {
      ...
      case 'AIF':
        ...
        if (product.minInvestment < 5000000) { // 50 Lakh minimum for PMS generally
          logger.warn(`[EligibilityEngine] Suspicious PMS product with low minimum: ${product.minInvestment}`);
        }
        break;
    ...
    if (!this.isRiskCompatible(user.riskProfile, product.riskLevel)) {
      reasons.push(`Product risk (${product.riskLevel}) exceeds user risk tolerance (${user.riskProfile})`);
    }
