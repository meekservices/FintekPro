...
export class HistoricalNavService {
	...
	static getInstance(): HistoricalNavService {
		if (!HistoricalNavService.instance) {
			HistoricalNavService.instance = new HistoricalNavService();
		}
		return HistoricalNavService.instance;
	}
	 ...
	 */
	async fetchAndStoreMutualFundHistory(
		schemeCode: string,
		...
		try {
			console.log(
				`[HistoricalNav] Fetching full history for scheme ${schemeCode}...`,
			);

			// Check what data we already have
			const existingRange = await this.getExistingDateRange(
				identifier,
				identifierType,
			);
			...
			const newRange = await this.getExistingDateRange(
				identifier,
				identifierType,
			);

			console.log(
				`[HistoricalNav] Stored ${insertedCount} records for ${schemeCode} (${newRange.start} to ${newRange.end})`,
			);
			...
			console.error(
				`[HistoricalNav] Error fetching ${schemeCode}:`,
				error.message,
			);
			return {
				success: false,
	 ...
	 */
	private async batchUpsertNavData(
		records: Array<{
			identifier: string;
			identifierType: string;
			date: string;
	...
	async getNavHistory(
		...
		return results.map((r) => ({
			...
			nav: Number.parseFloat(r.nav as string),
		}));
	}
	...
	async getNavForDate(
		...
		if (exactResult.length > 0) {
			return {
				nav: Number.parseFloat(exactResult[0].nav as string),
		...
		if (nearbyResults.length > 0) {
			return {
				nav: Number.parseFloat(nearbyResults[0].nav as string),
	 ...
	 */
	async batchGetGrandfatheringNavs(
		schemeCodes: string[],
	): Promise<Map<string, { nav: number; date: string; isExact: boolean }>> {
		const results = new Map<
			string,
			{ nav: number; date: string; isExact: boolean }
		>();
		...
		for (const [code, dataPoints] of schemeData) {
			...
			if (exact) {
				results.set(code, {
					nav: Number.parseFloat(exact.nav),
					date: exact.date,
					isExact: true,
				});
				...
				const closest = dataPoints.reduce((prev, curr) => {
					const prevDelta = Math.abs(
						new Date(prev.date).getTime() - targetDateMs,
					);
					const currDelta = Math.abs(
						new Date(curr.date).getTime() - targetDateMs,
					);
					return currDelta < prevDelta ? curr : prev;
				});
				results.set(code, {
					nav: Number.parseFloat(closest.nav),
					date: closest.date,
					isExact: false,
				});
			}
	...
	private calculateReturns(
		...
		for (let i = 1; i < navData.length; i++) {
			const dailyReturn =
				(navData[i].nav - navData[i - 1].nav) / navData[i - 1].nav;
	...
	private computeMetrics(
		...
		CachedMetrics,
		"dataPoints" | "dateRange" | "calculatedFromRealData"
	> {
		// CAGR
		...
		const cagr = years > 0 ? (endNav / startNav) ** (1 / years) - 1 : null;
		...
		const sortinoRatio =
			downsideDeviation > 0 ? excessReturn / downsideDeviation : null;
		...
		return {
			...
			sharpeRatio:
				sharpeRatio !== null ? Math.round(sharpeRatio * 100) / 100 : null,
			sortinoRatio:
				sortinoRatio !== null ? Math.round(sortinoRatio * 100) / 100 : null,
	...
	private async getCachedMetrics(
		...
		const c = cached[0];
		return {
			cagr: c.cagr ? Number.parseFloat(c.cagr) : null,
			volatility: c.volatility ? Number.parseFloat(c.volatility) : null,
			maxDrawdown: c.maxDrawdown ? Number.parseFloat(c.maxDrawdown) : null,
			sharpeRatio: c.sharpeRatio ? Number.parseFloat(c.sharpeRatio) : null,
			sortinoRatio: c.sortinoRatio ? Number.parseFloat(c.sortinoRatio) : null,
			beta: c.beta ? Number.parseFloat(c.beta) : null,
			alpha: c.alpha ? Number.parseFloat(c.alpha) : null,
	...
	async calculateReturnsFromStoredData(schemeCode: string): Promise<{
		...
		try {
			...
			const currentNav = Number.parseFloat(String(firstRecord.nav));
			const currentDate = new Date(String(firstRecord.date));

			if (Number.isNaN(currentNav) || Number.isNaN(currentDate.getTime())) {
				return {
					returns1y: null,
					returns3y: null,
					returns5y: null,
			...
			const findClosestNav = (targetDate: Date): number | null => {
				...
				for (const point of navData) {
					...
					if (diff < minDiff && diff <= 7 * 24 * 60 * 60 * 1000) {
						minDiff = diff;
						const navValue = Number.parseFloat(String(point.nav));
			...
			if (
				...
				console.error(
					`[HistoricalNav] Error calculating returns for ${schemeCode}:`,
					error.message,
				);
			}
			return {
	 ...
	 */
	async bulkCalculateReturnsFromStored(schemeCodes: string[]): Promise<
		Map<
			string,
			{
				returns1y: number | null;
