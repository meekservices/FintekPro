...
export class MFAdapter implements ProductAdapter<any> {
	...
	normalize(fund: any): InvestmentProduct {
		...
		return {
			...
			rating: fund.crisilRating
				? `${fund.crisilRating}-Star FintekPro Rating`
				: undefined,
	...
	private getTaxTreatment(category: string): TaxTreatment {
		if (category.includes("equity") || category.includes("elss"))
			return "equity";
		if (
	...
	getRiskLevel(fund: any): RiskLevel {
		...
		if (category.includes("liquid") || category.includes("overnight"))
			return "conservative";
		if (category.includes("debt") || category.includes("money market"))
			return "conservative";
		if (category.includes("hybrid") || category.includes("balanced"))
			return "moderate";
		if (category.includes("small cap") || category.includes("sectoral"))
			return "very_aggressive";
		return "aggressive";
	}
	...
	getHorizon(fund: any): InvestmentHorizon {
		...
		if (category.includes("liquid") || category.includes("overnight"))
			return "ultra_short";
		if (category.includes("short") || category.includes("money market"))
			return "short";
		if (category.includes("debt") || category.includes("hybrid"))
			return "medium";
		if (category.includes("elss") || category.includes("small cap"))
			return "long";
		return "medium";
	}
...
export class BondAdapter implements ProductAdapter<any> {
	...
	normalize(bond: any): InvestmentProduct {
		...
		return {
			...
			regulatory_tags: isGovSec
				? ["RBI_REGULATED", "SOVEREIGN"]
				: ["SEBI_REGULATED"],
			source: "store",
			current_price:
				parseDecimal(bond.lastTradedPrice) || parseDecimal(bond.faceValue),
			yield_or_return:
				parseDecimal(bond.yieldToMaturity) || parseDecimal(bond.couponRate),
	...
	private getTaxTreatment(bond: any): TaxTreatment {
		...
		if (type.includes("tax_free") || type.includes("tax-free"))
			return "tax_free";
	...
	getHorizon(bond: any): InvestmentHorizon {
		...
		const yearsToMaturity =
			(maturityDate.getTime() - Date.now()) / (365.25 * 24 * 60 * 60 * 1000);
	...
	getExpectedReturn(bond: any): { min: number; max: number } {
		const ytm =
			parseDecimal(bond.yieldToMaturity) || parseDecimal(bond.couponRate) || 7;
		return {
			min: ytm - 0.5,
			max: ytm + 0.5,
		};
...
export class REITAdapter implements ProductAdapter<any> {
	...
	normalize(reit: any): InvestmentProduct {
		return {
			...
			lock_in_period: null,
			min_investment:
				parseDecimal(reit.lotSize) * parseDecimal(reit.currentPrice) || 50000,
			regulatory_tags: ["SEBI_REGULATED", "EXCHANGE_TRADED"],
			source: "exchange",
...
export class InvITAdapter implements ProductAdapter<any> {
	...
	normalize(invit: any): InvestmentProduct {
		return {
			...
			lock_in_period: null,
			min_investment:
				parseDecimal(invit.lotSize) * parseDecimal(invit.currentPrice) ||
				100000,
			regulatory_tags: ["SEBI_REGULATED", "EXCHANGE_TRADED"],
			source: "exchange",
...
export class IPOAdapter implements ProductAdapter<any> {
	...
	normalize(ipo: any): InvestmentProduct {
		return {
			...
			lock_in_period: null,
			min_investment:
				parseDecimal(ipo.priceBandMax) * (ipo.lotSize || 1) || 15000,
...
export const productAdapters: Record<
	UnifiedProductType,
	ProductAdapter<any>
> = {
