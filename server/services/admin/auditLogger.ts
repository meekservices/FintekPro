...
export class AuditLogger {
   ...
   */
  async logEvent(eventType: AuditEventType, userId: string, metadata: Record<string, any>, ipAddress?: string) {
    try {
      const auditPayload = {
      ...
      logger.error(`[AUDIT_FAILURE] Critical failure writing to audit log`, { error: error.message, eventType, userId });
    }
  }
}
