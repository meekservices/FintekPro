...
function checkRateLimit(): void {
  ...
  if (rateBucket.count >= 10) {
    throw new Error('ICAI scraper rate limit reached (10 req/min). Please retry after a minute.');
  }
  rateBucket.count++;
}
...
function parseICAIHtml(html: string): {
  ...
  $('table tr, .result-row, .member-info tr, .search-result tr').each((_, el) => {
  ...
  if (!name) {
    $('td, li, p, span, div').each((_, el) => {
      ...
      if (/member\s*name\s*[:\-]/i.test(text)) {
        name = text.replace(/.*member\s*name\s*[:\-]/i, '').trim().split(/\n/)[0].trim();
      }
    });
  }
  if (!status) {
  ...
  if (!membershipType) {
    ...
    else if (/\bASSOCIATE\b|\bACA\b/.test(allText)) membershipType = 'ASSOCIATE';
  }
  if (!copStatus) {
    if (/COP\s*(STATUS)?\s*[:\-]?\s*ACTIVE/i.test(html)) copStatus = 'ACTIVE';
    else if (/COP\s*(STATUS)?\s*[:\-]?\s*INACTIVE/i.test(html)) copStatus = 'INACTIVE';
  }
...
async function fetchViaPuppeteer(membershipNumber: string): Promise<{
  ...
  try {
    ...
    const inputSelectors = [
      '#membership_number', '#member_no', '#txtMembNo', 'input[name="member_id"]',
      'input[name="member_no"]', 'input[name="txtMembNo"]', 'input[type="text"]',
    ];
    let filled = false;
    ...
    const submitSelectors = ['#search_button', 'button[type="submit"]', 'input[type="submit"]'];
...
async function getCachedResult(membershipNumber: string): Promise<CachedResult | null> {
  try {
...
export async function verifyICAIMembership(
  ...
  if (!forceRefresh) {
    ...
    if (cached) {
      console.log(`[ICAI] Cache hit for ${cleaned} (scraped: ${cached.scrapedAt?.toISOString()})`);
      const matchScore = expectedName && cached.nameAtIcai
      ...
      return {
        ...
        confidenceScore: parseFloat(cached.confidenceScore ?? '0.75'),
        source: 'CACHE',
  ...
  if (!html) {
    const failResult: ICAIVerificationResult = {
      ...
      error: 'Could not reach ICAI member search. Manual verification required.',
    };
  ...
  const membershipStatus: ICAIVerificationResult['membershipStatus'] =
    parsed.status?.includes('ACTIVE') ? 'ACTIVE' :
    parsed.status?.includes('INACTIVE') ? 'INACTIVE' :
    parsed.status?.includes('FELLOW') ? 'FELLOW' :
    parsed.status?.includes('ASSOCIATE') ? 'ASSOCIATE' :
    'UNKNOWN';

  const matchScore = expectedName && parsed.name
    ? nameMatchScore(expectedName, parsed.name)
    : undefined;

  let confidenceScore = 0.75;
  if (membershipStatus === 'ACTIVE' || membershipStatus === 'FELLOW' || membershipStatus === 'ASSOCIATE') {
    confidenceScore = 0.80;
  }
  if (matchScore !== undefined) {
    if (matchScore >= 85) confidenceScore = 0.90;
