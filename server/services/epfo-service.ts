...
export class EPFOService {
	...
	private getComingSoonResponse(): EPFFetchResponse {
		return {
			...
			accounts: [],
			message:
				"Coming Soon - EPFO integration will be available once API credentials are configured. Please contact support to enable this feature.",
		};
	}
	...
	async fetchEPFAccounts(request: EPFFetchRequest): Promise<EPFFetchResponse> {
		try {
			...
			const totalEmployeeContribution = accounts.reduce(
				(sum, a) => sum + a.employeeContribution,
				0,
			);
			const totalEmployerContribution = accounts.reduce(
				(sum, a) => sum + a.employerContribution,
				0,
			);
			const totalPensionContribution = accounts.reduce(
				(sum, a) => sum + a.pensionContribution,
				0,
			);
			const totalInterestEarned = accounts.reduce(
				(sum, a) => sum + a.interestEarned,
				0,
			);

			console.log(
				`✅ Fetched ${accounts.length} EPF accounts (Total Balance: ₹${totalBalance.toFixed(2)})`,
			);
	...
	private async callEPFOAPI(request: EPFFetchRequest): Promise<any> {
		try {
			...
			if (!uan) {
				...
				const uanResponse = await axios.post(
					uanLookupEndpoint,
					{
			...
			throw new Error(
				`EPFO API failed: ${error.response?.data?.message || error.message}`,
			);
		}
	}
	...
	private async parseEPFOResponse(epfoResponse: any): Promise<EPFAccount[]> {
		...
		for (const establishment of accountsData) {
			accounts.push({
				epfAccountNumber:
					establishment.account_number || establishment.establishment_id,
				...
				employeeContribution: Number.parseFloat(
					establishment.employee_share || "0",
				),
				employerContribution: Number.parseFloat(
					establishment.employer_share || "0",
				),
				pensionContribution: Number.parseFloat(establishment.eps_share || "0"),
				totalBalance: Number.parseFloat(establishment.total_balance || "0"),
				interestEarned: Number.parseFloat(establishment.interest_earned || "0"),
				interestRate: Number.parseFloat(establishment.interest_rate || "8.25"), // Current EPF interest rate
