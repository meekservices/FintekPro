...
import { db } from '../db';
import { 
  cashfreeTransactions, phonePeTransactions, 
  unlistedDeals, complianceAuditTrail 
} from '@shared/schema';
...
export interface Discrepancy {
  id: string;
  type: 'payment_mismatch' | 'order_orphan' | 'duplicate_payment' | 'amount_difference' | 'status_inconsistency';
...
class DailyReconciliationService {
  ...
  async runDailyReconciliation(
    ...
    console.log(`[Reconciliation] Starting daily reconciliation for ${runDate.toISOString().split('T')[0]}`);

    try {
      const cashfreeRecon = await this.reconcileCashfree(periodStart, periodEnd);
      ...
      const discrepancies = await this.detectDiscrepancies(periodStart, periodEnd);

      const totalTransactions = cashfreeRecon.transactionCount + phonePeRecon.transactionCount;
      const totalAmount = cashfreeRecon.totalAmount + phonePeRecon.totalAmount;
      const successfulTransactions = cashfreeRecon.successCount + phonePeRecon.successCount;
      const failedTransactions = cashfreeRecon.failedCount + phonePeRecon.failedCount;
      const pendingTransactions = cashfreeRecon.pendingCount + phonePeRecon.pendingCount;
      ...
      const summary: ReconciliationSummary = {
        ...
        failedTransactions,
        pendingTransactions,
        matchedOrders: orderRecon.mutualFunds.confirmedPayments + 
                       orderRecon.bonds.confirmedPayments + 
                       orderRecon.unlistedEquity.confirmedPayments,
        unmatchedOrders: (orderRecon.mutualFunds.orderCount - orderRecon.mutualFunds.confirmedPayments) +
                        (orderRecon.bonds.orderCount - orderRecon.bonds.confirmedPayments) +
                        (orderRecon.unlistedEquity.orderCount - orderRecon.unlistedEquity.confirmedPayments),
      ...
      const report: ReconciliationReport = {
        ...
        periodStart,
        periodEnd,
        status: discrepancies.some(d => d.severity === 'critical') ? 'partial' : 'completed',
        summary,
      ...
      console.log(`[Reconciliation] Daily reconciliation completed: ${reportId}`);
      console.log(`[Reconciliation] Summary: ${totalTransactions} transactions, ${discrepancies.length} discrepancies, ${summary.reconciliationRate}% match rate`);
  ...
  private async reconcileCashfree(start: Date, end: Date): Promise<GatewayReconciliation> {
    try {
      ...
      for (const tx of transactions) {
        ...
        if (tx.status === 'PAID' || tx.status === 'success') {
          successCount++;
        } else if (tx.status === 'FAILED' || tx.status === 'failed' || tx.status === 'EXPIRED') {
          failedCount++;
        } else {
          pendingCount++;
        }
  ...
  private async reconcilePhonePe(start: Date, end: Date): Promise<GatewayReconciliation> {
    try {
      const transactions = await db
        .select()
  ...
  private async reconcileOrders(start: Date, end: Date): Promise<OrderReconciliation> {
    ...
    try {
      ...
      for (const deal of unlistedResult) {
        totalValue += parseFloat(deal.totalValue || '0');
        if (['completed', 'escrowed', 'released'].includes(deal.status || '')) {
          confirmed++;
        } else if (['payment_pending', 'pending', 'negotiating'].includes(deal.status || '')) {
          pending++;
        } else if (['failed', 'cancelled', 'rejected'].includes(deal.status || '')) {
          failed++;
        }
      }
      ...
      unlistedRecon = {
        ...
        totalValue,
        matchRate: unlistedResult.length > 0 ? Math.round((confirmed / unlistedResult.length) * 100) : 100
      };
  ...
  private async detectDiscrepancies(start: Date, end: Date): Promise<Discrepancy[]> {
      ...
      for (const tx of cashfreeTx) {
        if (tx.status === 'PAID' && !tx.completedAt) {
          discrepancies.push({
            ...
            expectedAmount: parseFloat(tx.amount || '0'),
            detectedAt: new Date(),
            resolved: false
          });
        }

        if (tx.status !== 'PAID' && tx.status !== 'ACTIVE' && tx.status !== 'PENDING') {
          const createdAt = new Date(tx.createdAt as Date);
          const hoursSinceCreation = (Date.now() - createdAt.getTime()) / (1000 * 60 * 60);
          
          if (hoursSinceCreation > 24 && tx.status !== 'EXPIRED' && tx.status !== 'FAILED') {
            discrepancies.push({
              id: nanoid(),
              type: 'status_inconsistency',
              severity: 'low',
  ...
  private generateChecksum(summary: ReconciliationSummary, discrepancies: Discrepancy[]): string {
    const data = JSON.stringify({ summary, discrepancyCount: discrepancies.length });
