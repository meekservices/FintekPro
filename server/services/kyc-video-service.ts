...
class KycVideoService {
  ...
  async initiate(params: InitiateVideoKycParams): Promise<{
    ...
    try {
      ...
      const [videoSession] = await db.insert(kycVideoSessions).values({
  ...
  async complete(params: CompleteVideoKycParams): Promise<{
    ...
    try {
      ...
      await db.insert(kycAuditLogs).values({
        ...
        action: params.status === 'COMPLETED' ? 'VIDEO_KYC_COMPLETED' : 'VIDEO_KYC_FAILED',
        step: 'video_kyc',
        performedBy: params.officerId,
        performedByRole: 'officer',
        previousValue: { status: session.status },
        newValue: { status: params.status, recordingHash: params.recordingHash },
      });
  ...
  shouldRequireVideoKyc(amlRiskLevel: string | null, isReKyc: boolean = false): boolean {
    if (!amlRiskLevel) return false;
    const level = amlRiskLevel.toUpperCase();
    return level === 'HIGH' || level === 'CRITICAL' || isReKyc;
  }

  canFinalizeTier(userId: string, videoRequired: boolean, videoStatus: string | null): boolean {
