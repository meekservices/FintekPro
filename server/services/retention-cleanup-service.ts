...
class RetentionCleanupService {
	 ...
	 */
	private async archiveExpiredConsents(
		cutoffDate: Date,
		...
		try {
			...
			for (const consent of expiredConsents) {
				try {
					...
					errors.push(
						`Failed to archive consent ${consent.id}: ${error.message}`,
					);
				}
			}
		} catch (error: any) {
	 ...
	 */
	private async deleteArchivedConsents(
		cutoffDate: Date,
	 ...
	 */
	private async deleteOldAuditLogs(): Promise<{
		deleted: number;
		errors: string[];
	}> {
		const errors: string[] = [];
		...
		try {
			...
			const result = await db.delete(schema.governmentSchemeAudit).where(
				and(
	...
	async runCleanup(): Promise<CleanupResult> {
		if (this.isRunning) {
			console.log(
				"[RETENTION_CLEANUP] Cleanup already in progress, skipping...",
			);
			return {
				consentsArchived: 0,
				consentsDeleted: 0,
		...
		console.log(
			`   Cutoff date: ${cutoffDate.toISOString()} (${RETENTION_PERIOD_YEARS} years ago)`,
		);
		...
		try {
			...
			console.log(
				`   Consents: ${result.consentsArchived} archived, ${result.consentsDeleted} deleted`,
			);
			console.log(
				`   Audit logs: ${result.auditLogsArchived} archived, ${result.auditLogsDeleted} deleted`,
			);
	...
	scheduleCleanup(): void {
		...
		console.log(
			"📅 [RETENTION_CLEANUP] Scheduled daily cleanup at 2:00 AM IST",
		);
	}
