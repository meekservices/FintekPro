...
export async function getAllFeatureFlags(
	...
	if (category) {
		return db
			.select()
			.from(platformFeatureFlags)
			.where(eq(platformFeatureFlags.category, category));
	}
...
export async function updateFeatureFlag(
	flagKey: string,
	updates: Partial<InsertPlatformFeatureFlag>,
	userId?: string,
): Promise<PlatformFeatureFlag | null> {
...
export async function updateMarket(
	marketCode: string,
	updates: Partial<InsertMarketMaster>,
	userId?: string,
): Promise<MarketMaster | null> {
	const [updated] = await db
		.update(marketsMaster)
		.set({ ...updates, updatedAt: new Date(), updatedBy: userId })
		.where(eq(marketsMaster.marketCode, marketCode))
		.returning();
	return updated || null;
...
export async function getProductsForMarket(
	marketCode: string,
): Promise<MarketProductMatrix[]> {
	return db
		.select()
...
export async function updateMarketProduct(
	id: string,
	updates: Partial<InsertMarketProductMatrix>,
	userId?: string,
): Promise<MarketProductMatrix | null> {
	const [updated] = await db
		.update(marketProductMatrix)
		.set({ ...updates, updatedAt: new Date(), updatedBy: userId })
...
export async function isProductAllowedInMarket(
	marketCode: string,
	productCategory: string,
): Promise<{ allowed: boolean; advisoryLevel: string; restrictions?: string }> {
	...
	if (!product) {
		return {
			allowed: false,
			advisoryLevel: "NONE",
			restrictions: "Product not available in this market",
		};
	}
...
export async function getUserMarketPreferences(
	userId: string,
): Promise<UserMarketPreferences | null> {
	const [prefs] = await db
		.select()
		.from(userMarketPreferences)
		.where(eq(userMarketPreferences.userId, userId));
	return prefs || null;
}

export async function upsertUserMarketPreferences(
	userId: string,
	updates: Partial<InsertUserMarketPreferences>,
): Promise<UserMarketPreferences> {
	const existing = await getUserMarketPreferences(userId);

	if (existing) {
		const [updated] = await db
			.update(userMarketPreferences)
	...
	const [created] = await db
		.insert(userMarketPreferences)
		.values({ userId, ...updates })
		.returning();
	return created;
}

export async function setSelectedMarket(
	userId: string,
	marketCode: string,
): Promise<UserMarketPreferences> {
...
export async function hasUserAcknowledged(
	...
	const [ack] = await db
		...
		.where(
			and(
				...
				eq(
					globalAdvisoryAcknowledgments.acknowledgmentType,
					acknowledgmentType,
				),
	...
	if (!ack) return false;
	if ((ack as any).expiresAt && new Date((ack as any).expiresAt) < new Date())
		return false;
...
export async function recordAcknowledgment(
	...
	const [ack] = await db
		.insert(globalAdvisoryAcknowledgments)
		.values({
			userId,
			marketCode,
			acknowledgmentType,
...
export async function logAuditEvent(
	...
	const checksumHash = generateChecksum({
		...logEntry,
		timestamp: new Date().toISOString(),
	});

	const [log] = await db
		.insert(globalAdvisoryAuditLog)
		.values({
			...logEntry,
			checksumHash,
		})
...
export async function getAuditLogs(filters: {
	...
	const conditions = [];
	if (filters.userId)
		conditions.push(eq(globalAdvisoryAuditLog.userId, filters.userId));
	if (filters.marketCode)
		conditions.push(eq(globalAdvisoryAuditLog.marketCode, filters.marketCode));
	if (filters.eventType)
		conditions.push(eq(globalAdvisoryAuditLog.eventType, filters.eventType));
	if (filters.startDate)
		conditions.push(
			sql`${globalAdvisoryAuditLog.eventTimestamp} >= ${filters.startDate}`,
		);
	if (filters.endDate)
		conditions.push(
			sql`${globalAdvisoryAuditLog.eventTimestamp} <= ${filters.endDate}`,
		);
	...
	return query
		.orderBy(desc(globalAdvisoryAuditLog.eventTimestamp))
		.limit(filters.limit || 100);
}

// Execution Guard - CRITICAL: Prevents execution outside India
export async function canExecuteInMarket(
	marketCode: string,
...
export function getAdvisoryBadge(advisoryLevel: string): {
	label: string;
...
export async function convertCurrency(
	...
	const convertedAmount = await currencyExchangeService.convertAmount(
		amount,
		fromCurrency,
		toCurrency,
	);
...
export async function getUserMarketEligibility(userId: string): Promise<{
	...
	const selectedMarket = enabledMarkets.find(
		(m: any) => m.marketCode === primaryMarket,
	);
...
export async function getMarketEligibilityForUser(
	userId: string,
	marketCode: string,
): Promise<MarketEligibility | null> {
	const market = await getMarketByCode(marketCode);
...
export async function getJurisdictionFeatureFlags(
	marketCode: string,
): Promise<JurisdictionFeatureFlags> {
	const market = await getMarketByCode(marketCode);
	const products = await getProductsForMarket(marketCode);
	...
	return {
		...
		requiresAccreditedStatus: products.some(
			(p: any) => p.requiresAccreditedInvestor,
		),
...
export async function generateSEBIExport(
	userId: string,
	startDate: Date,
	endDate: Date,
): Promise<{
	...
	for (const view of advisoryViews) {
		if (view.marketCode) {
			marketBreakdown[view.marketCode] =
				(marketBreakdown[view.marketCode] || 0) + 1;
		}
		eventTypeBreakdown[view.eventType] =
			(eventTypeBreakdown[view.eventType] || 0) + 1;
	}
