...
class AuditBufferService {
	...
	private ensureFallbackDir(): void {
		try {
			...
			logger.warn("[AuditBuffer] Could not create fallback log directory", {
	...
	push(entry: Omit<AuditEntry, "createdAt">): void {
		...
		if (this.buffer.length >= BUFFER_MAX) {
			this.flush().catch((err) =>
				logger.error("[AuditBuffer] Threshold flush error", {
					error: String(err),
				}),
			);
		}
	...
	async flush(): Promise<void> {
		...
		try {
			...
			if (!this.dbAvailable) {
				logger.info(
					"[AuditBuffer] DB connection restored — resuming normal writes",
				);
	...
	private writeBatchToFile(batch: AuditEntry[]): void {
		try {
			...
			if (stats.size > 50 * 1024 * 1024) {
				const rotated = FALLBACK_LOG_FILE.replace(
					".jsonl",
					`-${Date.now()}.jsonl`,
				);
			...
			logger.error(
				"[AuditBuffer] Cannot write to fallback file — entries lost!",
				{
					count: batch.length,
	...
	private flushSync(): void {
		...
		this.writeBatchToFile(batch);
		logger.info(
			"[AuditBuffer] SIGTERM flush — wrote remaining entries to fallback file",
			{
				count: batch.length,
			},
		);
