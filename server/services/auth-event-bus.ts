...
class AuthEventBus extends EventEmitter {
	...
	on<K extends keyof AuthEventMap>(
		event: K,
		listener: (payload: AuthEventMap[K][0]) => void,
	): this {
...
console.log(
	"✅ Auth Event Bus initialized (AUTH_SUCCESS | AUTH_STEPUP_REQUIRED | HIGH_RISK_TXN | CREDENTIAL_ENROLLED | CREDENTIAL_DELETED)",
);
