...
class TwilioWebhookService {
	...
	constructor() {
		...
		if (this.isConfigured) {
			...
			console.log(
				"⚠️ Twilio Webhook Service not configured - missing credentials",
			);
		}
	}
	...
	async handleIncomingSMS(message: IncomingMessage): Promise<string> {
		...
		await this.saveInboundMessage({
			...
			toNumber: message.To,
			body: message.Body,
			numMedia: Number.parseInt(message.NumMedia || "0"),
		...
		console.log(
			`📥 Incoming SMS from ${phoneNumber}: ${message.Body.substring(0, 50)}...`,
		);
	...
	async handleIncomingWhatsApp(message: IncomingMessage): Promise<string> {
		...
		// Generate auto-reply response
		const autoReply = await this.generateResponse(
			command,
			args,
			user,
			"whatsapp",
		...
		console.log(
			`📥 Incoming WhatsApp from ${phoneNumber}: ${message.Body.substring(0, 50)}...`,
		);
...
Reply HELP for available commands.`;
	...
	private async saveInboundMessage(data: {
		...
		try {
			...
			console.log(
				`💾 Saved inbound ${data.channel} message from ${data.fromNumber}`,
			);
		} catch (error) {
...
export function createTwilioWebhookRouter(): Router {
	...
	const CALL_FORWARD_NUMBER =
		process.env.TWILIO_CALL_FORWARD_NUMBER || "+919686854321";

	router.post("/voice/webhook", async (req: Request, res: Response) => {
		try {
			const callData: IncomingCall = req.body;
			console.log(
				`📞 Incoming voice call from ${callData.From} to ${callData.To}`,
			);
			...
			try {
				...
				if (userResult.length > 0) {
					...
					userName =
						`${userResult[0].firstName || ""} ${userResult[0].lastName || ""}`.trim() ||
						null;
			...
			const greetingMessage =
				"Thank you for calling FintekPro. Your call back request is successfully registered. Our client support executive will call you back soon.";
...
<Response>
  <Say>We're experiencing technical difficulties. Please try again later.</Say>
	...
	router.post("/voice/status", async (req: Request, res: Response) => {
		console.log(
			`📞 Call status update: ${req.body.CallStatus} for ${req.body.CallSid}`,
		);
		...
		try {
			...
			const duration = Number.parseInt(req.body.CallDuration || "0", 10);
			...
			if (callSid) {
				...
				console.log(
					`📞 Call log updated: ${callSid} - Status: ${callStatus}, Duration: ${duration}s`,
				);
			}
		} catch (err) {
	...
	router.post("/voice/recording", async (req: Request, res: Response) => {
		...
		console.log(
			`   Duration: ${req.body.RecordingDuration}s, From: ${req.body.From}`,
		);
	...
	router.post("/sms/webhook", async (req: Request, res: Response) => {
		try {
			if (
				...
				!twilioWebhookService.validateRequest(req)
			) {
  ...
  <Message>${escapeXml(response)}</Message>
	...
	router.post("/whatsapp/webhook", async (req: Request, res: Response) => {
		try {
			...
			const message: IncomingMessage = req.body;
			const response =
				await twilioWebhookService.handleIncomingWhatsApp(message);
...
<Response>
  <Message>${escapeXml(response)}</Message>
	...
	router.post(
		"/webhook/messaging-service",
		async (req: Request, res: Response) => {
			try {
				console.log(
					"📨 Messaging Service webhook received:",
				...
				const isWhatsApp =
					...
					message.To?.startsWith("whatsapp:");
  ...
  <Message>${escapeXml(response)}</Message>
	...
	router.post("/webhook/status", async (req: Request, res: Response) => {
		try {
			...
			console.log(
				`📬 Message status update - SID: ${MessageSid}, Status: ${MessageStatus}, To: ${To}`,
			);

			if (ErrorCode || ErrorMessage) {
				console.error(
					`❌ Message delivery error - SID: ${MessageSid}, Error: ${ErrorCode} - ${ErrorMessage}`,
				);
			}
			...
			if (MessageStatus === "delivered") {
				...
				console.error(
					`❌ Message failed to ${To}: ${ErrorMessage || "Unknown error"}`,
				);
			}
	...
	router.post(
		...
		async (req: Request, res: Response) => {
			try {
				...
				const success = await twilioWebhookService.markMessageAsRead(
					messageId,
					readBy,
				);
	...
	router.post(
		...
		async (req: Request, res: Response) => {
			try {
				...
				const success = await twilioWebhookService.addAdminNote(
					messageId,
					note,
				);
			...
			try {
				...
				if (channel === "whatsapp") {
					...
					const {
						...
						isTwilioConfigured,
					} = await import("./twilio-client");
					const isConfigured = await isTwilioConfigured();

					if (!isConfigured) {
						return res
							.status(500)
							.json({ error: "SMS service not configured" });
					}
				...
				if (result.success) {
					console.log(
						`📤 Admin reply sent to ${recipientNumber} via ${channel} - SID: ${result.messageSid}`,
					);
					res.json({
						success: true,
					...
					res
						.status(500)
						.json({ error: result.error || "Failed to send reply" });
				}
			} catch (error) {
	...
	router.post(
		...
		async (req: Request, res: Response) => {
			try {
				...
				if (result.success) {
					console.log(
						`📤 Admin message sent to ${to} via ${channel || "sms"} - SID: ${result.messageSid}`,
					);
					res.json({
						success: true,
						messageSid: result.messageSid,
	...
	router.get(
		...
		async (req: Request, res: Response) => {
			try {
				...
				if (callbackStatus) {
					conditions.push(
						eq(callLogs.callbackStatus, callbackStatus as string),
					);
				}
				if (isRead === "true") {
				...
				if (startDate) {
					conditions.push(
						gte(callLogs.callStartedAt, new Date(startDate as string)),
					);
				}
				if (endDate) {
					conditions.push(
						lte(callLogs.callStartedAt, new Date(endDate as string)),
					);
				}
				if (agentId) {
	...
	router.get(
		...
		async (req: Request, res: Response) => {
			try {
				...
				res.json({
					unreadCount:
						(messageCount[0]?.count || 0) + (callCount[0]?.count || 0),
