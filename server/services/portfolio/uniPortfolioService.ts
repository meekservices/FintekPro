...
import { portfolioAggregator, UnifiedPortfolioResult, NormalizedHolding } from '../portfolio/portfolioAggregator';
...
export class UniPortfolioService {
  ...
  async getSnapshot(
    ...
    if (cached && cached.expiresAt > Date.now()) {
      logger.info(`[UniPortfolioService] Cache hit`, { event: 'PORTFOLIO_CACHE_HIT', user_id: userId });
      return cached.snapshot;
    }
    const snapshot = await this.buildSnapshot(userId, riskScore, horizon, segment, targetModel);
    snapshotCache.set(cacheKey, { snapshot, expiresAt: Date.now() + CACHE_TTL_MS });
    return snapshot;
  }
   ...
   */
  async refresh(userId: string, riskScore = 50, horizon = 5, segment = 'retail'): Promise<UniPortfolioSnapshot> {
  ...
  private async buildSnapshot(
    ...
    const currentAllocation = this.weightsToAssetWeights(aggregated.summary.assetClassWeights);
    ...
    try {
      ...
      logger.warn(`[UniPortfolioService] Rebalancing engine failed, using empty result`, err?.message);
      rebalancing = this.emptyRebalance();
    }

    // 5. Concentration risk — top 5 positions
    const concentration = this.computeConcentration(aggregated.holdings, aggregated.summary.totalValueInr);
    ...
    for (const br of aggregated.brokerResults) {
      freshness[br.brokerId] = br.success ? Math.round((Date.now() - fetchedAt) / 1000) : -1;
    }
    ...
    const snapshot: UniPortfolioSnapshot = {
      ...
      meta: {
        ...
        brokers_configured: aggregated.brokerResults.filter(b => b.configured).map(b => b.brokerId),
  ...
  private weightsToAssetWeights(weights: Record<string, number>) {
    return Object.entries(weights).map(([asset, weight]) => ({ asset, weight }));
  }

  private computeAssetValues(holdings: NormalizedHolding[]): Record<string, number> {
    const values: Record<string, number> = {};
    for (const h of holdings) {
      values[h.allocationGroup] = (values[h.allocationGroup] ?? 0) + h.currentValueInr;
    }
    return values;
  }

  private computeConcentration(holdings: NormalizedHolding[], totalValueInr: number): ConcentrationRisk[] {
    ...
    return [...holdings]
      ...
      .map(h => ({
        ...
        pct: parseFloat(((h.currentValueInr / totalValueInr) * 100).toFixed(2)),
      }));
  }
  ...
  private emptyRebalance(): RebalanceAnalysis {
    return {
      ...
      urgency: 'none',
      driftAnalysis: { maxDrift: 0, averageDrift: 0, assetDrifts: [], equityDrift: 0, debtDrift: 0, portfolioRiskDrift: 0 },
      trades: [],
