...
import { NormalizedPosition, BrokerCapability } from '../mpal/interfaces/IBroker';
...
export interface UnifiedPortfolioResult {
  ...
  summary: {
    ...
    brokerBreakdown: { brokerId: string; count: number; valueInr: number; configured: boolean }[];
  };
  fetchedAt: string;
}
...
export class PortfolioAggregator {
  ...
  async getUnifiedPortfolio(
    ...
    logger.info(`[PortfolioAggregator] Building multi-broker unified portfolio`, {
      event: 'PORTFOLIO_AGGREGATE_START',
      user_id: userId,
    });

    // 1. Collect brokers to poll
    const allBrokers = providerRegistry.getAllBrokers().filter(b => b.isConfigured());
    const brokersToQuery = preferCapabilities
      ? allBrokers.filter(b => preferCapabilities.some(cap => b.capabilities.includes(cap)))
      : allBrokers;

    if (brokersToQuery.length === 0) {
      logger.warn(`[PortfolioAggregator] No brokers configured — returning empty portfolio`);
      return this.emptyResult();
    }

    // 2. Fetch all brokers in parallel (allSettled = never crashes)
    const fetchResults = await Promise.allSettled(
      brokersToQuery.map(broker => this.fetchFromBroker(broker.brokerId, userId))
    );
    ...
    const staleBrokers = brokerResults.filter(r => !r.success).map(r => r.brokerId);

    // 3. Get FX rate once for all USD conversions
    const fxRateUsdInr = await valuationEngine.getExchangeRate('USD', 'INR').catch(() => 84); // fallback

    // 4. Normalize + enrich all positions
    const holdings: NormalizedHolding[] = [];
    for (const result of brokerResults) {
      for (const pos of result.positions) {
        const valueInr = pos.currency === 'USD'
        ...
        const costInr = pos.averageCost
          ? (pos.currency === 'USD' ? pos.averageCost * pos.quantity * fxRateUsdInr : pos.averageCost * pos.quantity)
          : undefined;

        const unrealizedPnlInr = (costInr !== undefined && valueInr) ? valueInr - costInr : undefined;
        const unrealizedPnlPct = (costInr && costInr > 0 && unrealizedPnlInr !== undefined)
          ? (unrealizedPnlInr / costInr) * 100 : undefined;
    ...
    const totalValueUsd = holdings.filter(h => h.currency === 'USD').reduce(
      (s, h) => s + (h.currentPrice ?? 0) * h.quantity, 0
    );
    const unrealizedPnlInr = totalValueInr - totalCostInr;
    const unrealizedPnlPct = totalCostInr > 0 ? (unrealizedPnlInr / totalCostInr) * 100 : 0;

    // Asset class weights
    const assetClassWeights: Record<string, number> = {};
    for (const h of holdings) {
      assetClassWeights[h.allocationGroup] = (assetClassWeights[h.allocationGroup] ?? 0) + h.currentValueInr;
    }
    if (totalValueInr > 0) {
      for (const k of Object.keys(assetClassWeights)) {
        assetClassWeights[k] = parseFloat(((assetClassWeights[k] / totalValueInr) * 100).toFixed(2));
      }
    }
    ...
    const countryWeights = {
      IN: totalValueInr > 0 ? parseFloat(((countryTotals.IN / totalValueInr) * 100).toFixed(2)) : 0,
      US: totalValueInr > 0 ? parseFloat(((countryTotals.US / totalValueInr) * 100).toFixed(2)) : 0,
      OTHER: totalValueInr > 0 ? parseFloat(((countryTotals.OTHER / totalValueInr) * 100).toFixed(2)) : 0,
    };
  ...
  private async fetchFromBroker(brokerId: string, userId: string): Promise<BrokerFetchResult> {
    const start = Date.now();
    try {
      const broker = providerRegistry.getBroker(brokerId);
      const positions = await broker.getPositions(userId);
      return { brokerId, configured: true, success: true, positions, latencyMs: Date.now() - start };
    } catch (err: any) {
      ...
      return { brokerId, configured: true, success: false, positions: [], latencyMs: Date.now() - start, errorMessage: err?.message };
    }
  }
