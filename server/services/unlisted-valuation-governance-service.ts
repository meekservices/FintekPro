...
class UnlistedValuationGovernanceService {
	...
	async addValuation(
		...
		const daysDiff = Math.floor(
			(today.getTime() - valDate.getTime()) / (1000 * 60 * 60 * 24),
		);
	...
	async runStalenessSweep(): Promise<StalenessReport> {
		...
		for (const company of all) {
			const lastDate = company.lastValuationDate
				? new Date(company.lastValuationDate)
				: null;
		...
		console.log(
			`[UnlistedValuationGovernance] Staleness sweep: ${markedStale} newly stale, ${alreadyStale} already stale, ${neverValued} never valued`,
		);
	...
	async getHealthReport(): Promise<UnlistedHealthReport> {
		...
		return {
			staleValuations: staleRows.map((r) => {
				const lastDate = r.lastValuationDate
					? new Date(r.lastValuationDate)
					: null;
				const daysSince = lastDate
					? Math.floor(
...
export const unlistedValuationGovernanceService =
	new UnlistedValuationGovernanceService();
