...
class ActivityInsightsService {
		...
		try {
			...
			const [
				...
				db
					...
					.from(users)
					.where(
						and(lt(users.lastLoginAt, thirtyDaysAgo), eq(users.isActive, true)),
					)
					.catch(() => [{ count: 0 }]),
				db
					.select({ count: count() })
				...
				db
					.select({ count: count() })
					.from(bondOrders)
					.where(eq(bondOrders.orderStatus, "pending"))
					.catch(() => [{ count: 0 }]),
				db
					.select({ count: count() })
					.from(unlistedDeals)
				...
				db
					...
					.from(mfOrders)
					.where(inArray(mfOrders.status, ["created", "pending_payment"]))
					.catch(() => [{ count: 0, totalValue: 0 }]),
				db
					.select({
						count: count(),
			...
			const auditFailedLogins = (authSecurityEvents as any[]).filter(
				(e: any) =>
					(e.eventType === "authentication" || e.eventType === "login") &&
					(e.action?.toLowerCase().includes("fail") ||
						e.action?.toLowerCase().includes("denied")),
			).length;
			...
			const auditRateLimitEvents = (authSecurityEvents as any[]).filter(
				(e: any) =>
					e.action?.toLowerCase().includes("rate") ||
					e.action?.toLowerCase().includes("throttl") ||
					e.action?.toLowerCase().includes("blocked"),
			).length;
			const complianceRateLimits = complianceAlerts.filter(
				(a) =>
					a.alertType === "suspicious_ip" ||
					a.description?.toLowerCase().includes("rate limit"),
			).length;
			...
			const auditSuspicious = (authSecurityEvents as any[]).filter(
				(e: any) =>
					e.eventType === "security" &&
					(e.action?.toLowerCase().includes("suspicious") ||
						e.action?.toLowerCase().includes("breach") ||
						e.action?.toLowerCase().includes("unauthorized")),
			).length;
			...
			const abandonedCarts =
				(cancelledBondOrders[0]?.count || 0) +
				(cancelledMfOrders[0]?.count || 0) +
				(cancelledUnlistedDeals[0]?.count || 0);
			const completedDeals =
				(completedBondOrders[0]?.count || 0) +
				(completedUnlistedDeals[0]?.count || 0) +
				(completedMfOrders[0]?.count || 0);
			const pendingMfValue = Number(pendingMfOrders[0]?.totalValue || 0);
			const pendingBondVal = Number(pendingBondValue[0]?.totalValue || 0);
			const pendingUnlistedVal = Number(
				pendingUnlistedValue[0]?.totalValue || 0,
			);
			...
			const cancelledUnlistedVal = Number(
				cancelledUnlistedDeals[0]?.totalValue || 0,
			);
			...
			const companiesNearLimit = (investorCounts as any[]).filter(
				(c: any) =>
					c.investorCount >= nearLimitThreshold &&
					c.investorCount < atLimitThreshold,
			).length;
			...
			return {
				...
				revenue: {
					pendingOrders:
						(pendingBondOrders[0]?.count || 0) +
						(pendingUnlistedDeals[0]?.count || 0) +
						(pendingMfOrders[0]?.count || 0),
					abandonedCarts,
					completedDeals,
			...
			console.error(
				"[ActivityInsights] Error fetching metrics:",
				error?.message || error,
			);
			return {
				errors: {
				...
				regulatory: {
					highDeviationDeals: 0,
					pendingStrFlags: 0,
					investorLimitAlerts: 0,
	...
	async generateAIInsights(metrics: ActivityMetrics): Promise<AIInsight[]> {
		...
		const providerStatus = (aiService as any).providerStatus as Record<
			string,
			{ healthy: boolean; lastErrorTime: number }
		>;
		const COOL_DOWN_MS =
			((aiService as any).COOL_DOWN_MS as number) ?? 5 * 60 * 1000;
		const allUnhealthy =
			providerStatus &&
		...
		if (allUnhealthy) {
			console.warn(
				"[ActivityInsights] All AI providers are in cool-down — skipping scan to preserve quota. Serving cached/default insights.",
			);
			return this.cachedInsights.length > 0
				? this.cachedInsights
				: this.getDefaultInsights(metrics);
		}
...
- Prioritize revenue-critical and security-critical items.`;
			...
			try {
				...
				if (!jsonMatch) {
					console.error(
						"[ActivityInsights] Could not parse AI response as JSON",
					);
					return this.getDefaultInsights(metrics);
				}
				...
				console.log(
					`[ActivityInsights] Generated ${this.cachedInsights.length} AI insights`,
				);

				return this.cachedInsights;
			} catch (parseError) {
				console.error(
					"[ActivityInsights] Failed to parse AI response:",
					parseError,
				);
				return this.getDefaultInsights(metrics);
			}
	...
	private getDefaultInsights(metrics: ActivityMetrics): AIInsight[] {
		...
		if (metrics.errors.critical > 0) {
			insights.push({
				...
				suggestedAction:
					"Review and resolve critical errors in the Error tab immediately.",
		...
		if (metrics.users.dormant30Days > 0) {
			insights.push({
				...
				suggestedAction:
					"Launch re-engagement campaign with personalized investment opportunities.",
		...
		if (metrics.performance.highErrorRateModules.length > 0) {
			insights.push({
				...
				suggestedAction:
					"Review error logs and deploy fixes for these modules.",
		...
		if (metrics.regulatory.highDeviationDeals > 0) {
			insights.push({
				...
				suggestedAction:
					"Review compliance notes and audit the valuation source for these deals.",
		...
		if (metrics.regulatory.investorLimitAlerts > 0) {
			insights.push({
				...
				suggestedAction:
					"Halt new buy orders for these companies to avoid forced public listing compliance.",
	...
	async getStuckKycUsers(): Promise<any[]> {
      ...
      `);
			...
			console.error(
				"[ActivityInsights] Error fetching stuck KYC users:",
				error,
			);
			return [];
		}
	...
	private getKycActionRecommendation(user: any): {
		action: string;
		priority: string;
		helperText: string;
	} {
		...
		if (
			user.smartKycStep === "completed" &&
			user.smartKycOutcome === "failed"
		) {
			return {
	...
	startAutomatedMonitoring() {
		console.log(
			"[ActivityInsights] Starting automated regulatory monitoring...",
		);
		...
		const runScan = async () => {
			try {
				...
				for (const insight of criticalInsights) {
					const { adminParallelNotifier } = await import(
						"./admin-parallel-notifier"
					);

					let taskType: any = "UNLISTED_REGULATORY_BREACH";
					if (
						insight.title.toLowerCase().includes("valuation") ||
						insight.title.toLowerCase().includes("fmv")
					) {
