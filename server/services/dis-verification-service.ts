...
class DISVerificationService {
	...
	private initializeTrusteeValidation(): TrusteeValidation {
		return {
			...
			validationNotes:
				"Escrow account operated by ICICI Bank Ltd as trustee under SEBI regulations. Account ring-fenced for unlisted securities transactions only.",
		};
	}
	...
	async uploadDISDocuments(
		request: DISVerificationRequest,
	): Promise<DISVerificationResult> {
		const documentIds: string[] = [];
		const verificationChecks: VerificationCheck[] = [];
		const requiredDocTypes: DISDocument["documentType"][] = [
			"dis_slip",
			"depository_confirmation",
		];
		...
		if (missingDocuments.includes("dis_slip")) {
			recommendations.push(
				"Upload Delivery Instruction Slip (DIS) signed by seller",
			);
		}
		if (missingDocuments.includes("depository_confirmation")) {
			recommendations.push(
				"Upload depository (CDSL/NSDL) transaction confirmation",
			);
		}
	...
	async verifyDocument(
		...
		if (document.verificationStatus !== "pending") {
			return {
				success: false,
				message: `Document already ${document.verificationStatus}`,
			};
		}
	...
	async validateTransferDetails(
		...
		const dealDocuments = Array.from(this.documents.values()).filter(
			(doc) => doc.dealId === dealId,
		);

		const disSlip = dealDocuments.find((d) => d.documentType === "dis_slip");
		const depositoryConfirmation = dealDocuments.find(
			(d) => d.documentType === "depository_confirmation",
		);
		...
		checks.push({
			...
			severity: "critical",
			regulatoryReference:
				"SEBI (Depositories and Participants) Regulations, 2018 - Regulation 45",
		});
		...
		checks.push({
			...
			severity: "critical",
			regulatoryReference:
				"SEBI (Depositories and Participants) Regulations, 2018 - Regulation 38",
		});
		...
		const exerciseLetter = dealDocuments.find(
			(d) => d.documentType === "exercise_letter",
		);
		...
		if (
			depositoryConfirmation &&
			depositoryConfirmation.verificationStatus !== "verified"
		) {
			checks.push({
		...
		if (
			metadata?.quantityTransferred &&
			metadata.quantityTransferred !== expectedQuantity
		) {
			checks.push({
		...
		if (
			metadata?.buyerDematAccount &&
			metadata.buyerDematAccount !== buyerDematAccount
		) {
			checks.push({
	...
	async getEscrowReleasePrerequisites(
		...
		const trusteeCheck: VerificationCheck = {
			...
			status:
				this.trusteeInfo.validationStatus === "validated"
					? "passed"
					: "pending",
		...
		const mandatoryChecks = allChecks.filter((c) =>
			mandatoryCheckNames.includes(c.checkName),
		);
		const mandatoryChecksPassed = mandatoryChecks.every(
			(c) => c.status === "passed",
		);
		...
		if (!canRelease) {
			if (!mandatoryChecksPassed) {
				complianceNotes.push(
					"BLOCKED: Mandatory pre-release checks not passed",
				);
			}
			criticalFailures.forEach((check) => {
				complianceNotes.push(
					`CRITICAL FAILURE: ${check.checkName} - ${check.details}`,
				);
			});
		} else {
			complianceNotes.push(
				"All mandatory checks passed. Escrow release approved.",
			);
			complianceNotes.push(
				`Trustee: ${this.trusteeInfo.trusteeName} (${this.trusteeInfo.registrationNumber})`,
			);
		}
