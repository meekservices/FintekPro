...
class ImprovementFeaturesService {
	...
	async updateDashboardWidgets(
		...
		try {
			...
			if (existing) {
				await db
					.update(dashboardWidgetPreferences)
		...
		try {
			...
			const stats = {
				...
				totalEarnings: referrals
					...
					.reduce(
						(sum, r) => sum + Number.parseFloat(r.referrerRewardAmount || "0"),
						0,
					),
	...
	async createScheduledReport(
		...
		try {
			const nextScheduled = this.calculateNextSchedule(
				data.frequency,
				data.dayOfWeek,
				data.dayOfMonth,
			);
	...
	async globalSearch(
		...
		try {
			...
			const stocksResults = shouldSearchStocks
				? await db
						.select({
			...
			// Search mutual funds
			const mfResults = shouldSearchMutualFunds
				? await db
						.select({
							id: mutualFunds.id,
			...
			// Search bonds
			const bondResults = shouldSearchBonds
				? await db
						.select({
			...
			const sortedMutualFunds = mfResults
				...
				.sort((a, b) => {
					...
					const aFundHouseMatch = (a.fundHouse || "")
						.toLowerCase()
						.includes(queryLower)
						? 0
						: 1;
					const bFundHouseMatch = (b.fundHouse || "")
						.toLowerCase()
						.includes(queryLower)
						? 0
						: 1;
					if (aFundHouseMatch !== bFundHouseMatch)
						return aFundHouseMatch - bFundHouseMatch;
