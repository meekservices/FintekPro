...
class BankAPIAuditService {
	...
	createAxiosInterceptor(bankCode: string, environment: string) {
		return {
			request: (config: any) => {
				const requestId = this.startOperation(
					bankCode,
					this.inferOperation(config.url, config.method),
				);
	...
	async getBankMetrics(
		...
		for (const log of logs) {
			// Count by operation
			callsByOperation[log.operation] =
				(callsByOperation[log.operation] || 0) + 1;
