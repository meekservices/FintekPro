...
export class CapitalGainsCalculatorService {
	...
	async getTaxRates(
		assetClass: TaxAssetClass = "equity",
		transactionDate: Date = new Date(),
	): Promise<{ stcgRate: number; ltcgRate: number; thresholdDays: number }> {
		const rates = getTaxRatesForAsset(assetClass, transactionDate);
		return {
			stcgRate: rates.stcg,
			ltcgRate: rates.ltcg,
			thresholdDays: rates.ltcgThresholdDays,
		};
	}

	async calculatePortfolioGains(
		userId: string,
	): Promise<CapitalGainsBreakdown> {
		try {
			...
			for (const holding of flattenedHoldings) {
				...
				const unrealizedGain = hasMarketPrice
				...
				const unrealizedTax =
					hasMarketPrice && unrealizedGain > 0 ? unrealizedGain * taxRate : 0;
	...
	async calculateRealizedGains(
		userId: string,
		fiscalYear?: string,
	): Promise<RealizedGainsBreakdown> {
		try {
			...
			for (const order of settledSellOrders) {
				...
				const taxableGain = Number.parseFloat(
					order.taxableGain?.toString() || "0",
				);
				const estimatedTax = Number.parseFloat(
			...
			const ltcgExemption = Math.min(
				equityRates.ltcgExemption,
				Math.max(0, totalLTCG),
			);
	...
	async getCombinedGains(
		userId: string,
		fiscalYear?: string,
	): Promise<CombinedGainsBreakdown> {
		const fy = fiscalYear || this.getCurrentFiscalYear();
		...
		return {
			...
			combined: {
				...
				totalTaxLiability:
					unrealized.totalTaxLiability + realized.totalTaxLiability,
			},
			fiscalYear: fy,
		};
	}
	...
	async generateQuarterlyReminders(
		...
		try {
			...
			if (annualTaxLiability <= 0) {
				console.log(
					`No tax liability for user ${userId}, skipping reminder generation`,
				);
				return [];
			}
			...
			const fyStartYear = Number.parseInt(startYear);
			...
			for (const schedule of this.QUARTERLY_SCHEDULE) {
				...
				const dueMonth = Number.parseInt(month);
				...
				reminderRecords.push({
					...
					financialYear,
					dueDate,
					estimatedSTCG: Math.round(
						reminderData.estimatedSTCG * 100,
					).toString(),
					estimatedLTCG: Math.round(
						reminderData.estimatedLTCG * 100,
					).toString(),
					totalTaxLiability: Math.round(
						cumulativeTaxLiability * 100,
					).toString(),
					status: "pending",
				});
			}
			...
			if (reminderRecords.length > 0) {
				...
				console.log(
					`Created ${reminderRecords.length} quarterly reminders for user ${userId}`,
				);
			}
	...
	async simulateFIFORedemption(
		...
		for (const lot of sortedLots) {
			...
			try {
				...
				console.warn(
					`[CapitalGains FIFO] Exit load lookup failed for lot ${lot.purchaseDate}, using zero:`,
					error,
				);
			}
