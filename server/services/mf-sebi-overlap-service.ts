...
class MfSebiOverlapService {
	...
	static getInstance(): MfSebiOverlapService {
		if (!MfSebiOverlapService.instance) {
			MfSebiOverlapService.instance = new MfSebiOverlapService();
		}
		return MfSebiOverlapService.instance;
	}

	// ── Import holdings from mf_scheme_stock_holdings → mf_portfolio_holdings ─
	async importHoldingsFromSchemeStockHoldings(
		limit = 10000,
	): Promise<{ imported: number; schemes: number }> {
		console.log(
			"[SEBIOverlap] Importing scheme holdings from mf_scheme_stock_holdings...",
		);
      ...
      `);
          ...
          `);
					...
					const totalWeight = holdings.reduce(
						(sum: number, h: any) => sum + Number.parseFloat(h.weight || 0),
						0,
					);
					...
					for (const h of holdings) {
						const normalizedWeight =
							(Number.parseFloat(h.weight) / totalWeight) * 100;
            ...
            `);
					...
					console.error(
						`[SEBIOverlap] Import error for scheme ${scheme_code}:`,
						e.message,
					);
				}
			}
			...
			console.log(
				`[SEBIOverlap] Import complete: ${totalImported} holdings across ${schemes.length} schemes`,
			);
	...
	async computeOverlap(
      ...
      `);
			...
			return Number.parseFloat(rows[0]?.overlap || "0");
		} catch (e: any) {
			console.error(
				`[SEBIOverlap] computeOverlap error (${schemeCodeA}, ${schemeCodeB}):`,
				e.message,
			);
			return 0;
		}
    ...
    `;
		...
		if (filterByCategory) {
			...
			console.log(
				`[SEBIOverlap] Filtered to ${filtered.length} ${filterByCategory} schemes`,
			);
		}
		...
		for (let i = 0; i < activeSchemes.length; i++) {
			for (let j = i + 1; j < activeSchemes.length; j++) {
				...
				const overlap = await this.computeOverlap(
					schemeA.scheme_code,
					schemeB.scheme_code,
				);
				if (overlap === 0) continue; // Skip non-overlapping pairs

				const thresholdA = isThematicSectoral(schemeA.category)
					? THEMATIC_SECTORAL_THRESHOLD
					: DEFAULT_OVERLAP_THRESHOLD;
				const thresholdB = isThematicSectoral(schemeB.category)
					? THEMATIC_SECTORAL_THRESHOLD
					: DEFAULT_OVERLAP_THRESHOLD;
          ...
          `);
				...
				if (pairsComputed % 100 === 0) {
					console.log(
						`[SEBIOverlap] Progress: ${pairsComputed} pairs computed, ${breachesFound} breaches...`,
					);
				}
			}
		}
		...
		console.log(
			`[SEBIOverlap] Complete: ${pairsComputed} pairs, ${breachesFound} breaches, ${activeSchemes.length} schemes`,
		);
		return {
			pairsComputed,
			breachesFound,
			schemesCovered: activeSchemes.length,
		};
	}
    ...
    `);
		...
		const breachedSchemes = breachedRows.map(
			(r: any) => r.scheme_code as string,
		);
        ...
        `);
				...
				console.error(
					`[SEBIOverlap] applyBreachRules error for ${schemeCode}:`,
					e.message,
				);
			}
		}
		...
		console.log(
			`[SEBIOverlap] Applied breach rules to ${breachedSchemes.length} schemes`,
		);
    ...
    `);
		...
		return rows.map((r: any) => ({
			...
			overlapPercent: Number.parseFloat(r.overlap_percent),
