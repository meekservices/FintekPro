...
class DsaLoanService {
	...
	): Promise<DsaLoanApplication | null> {
		const [application] = await db
			.select()
			.from(dsaLoanApplications)
	...
	async deleteApplication(id: string): Promise<void> {
		await db.delete(dsaLoanApplications).where(eq(dsaLoanApplications.id, id));
	}
	...
	async checkEligibility(applicationId: string): Promise<EligibilityResult[]> {
		...
		for (const bank of activeBanks) {
			...
			if (
				bank.minAmount &&
				Number(application.requestedAmount) < Number(bank.minAmount)
			) {
				results.push({
					bankCode: bank.bankCode,
					bankName: bank.bankName,
			...
			if (
				bank.maxAmount &&
				Number(application.requestedAmount) > Number(bank.maxAmount)
			) {
				results.push({
					bankCode: bank.bankCode,
					bankName: bank.bankName,
			...
			const bankRules = rules.filter(
				(r: any) =>
					r.bankCode === bank.bankCode && r.loanType === application.loanType,
			);
			let eligible = true;
			...
			for (const rule of bankRules) {
				if (
					...
					application.creditScore < rule.minCreditScore
				) {
					eligible = false;
				...
				if (
					...
					eligible = false;
					reasons.push(
						`Monthly income below minimum (₹${rule.minMonthlyIncome})`,
					);
					matchScore -= 25;
				}
				...
				if (
					...
					if (
						!rule.allowedEmploymentTypes.includes(application.employmentType)
					) {
						eligible = false;
	...
	private async updateApplicationStatusFromResponses(
		applicationId: string,
	): Promise<void> {
		...
		const pending = histories.filter(
			(h: any) => h.bankStatus === "pending" || h.bankStatus === "in_review",
		);
	...
	async uploadDocument(
		data: Partial<InsertDsaLoanDocument> & {
			applicationId: string;
			documentType: string;
			documentName: string;
			fileName: string;
	...
	async createAuditLog(
		data: Partial<InsertDsaLoanAuditLog> & {
			action: string;
	...
	async checkEligibilityByCriteria(criteria: {
		...
		const income = Number.parseInt(criteria.monthlyIncome) || 0;
		const score = criteria.creditScore || 700;
		const eligibleBanks: string[] = [];

		for (const rule of rules) {
			const minIncome = Number.parseInt((rule as any).minIncome || "0");
	...
	async generateKFS(offerId: string): Promise<any | null> {
		...
		const amount = Number.parseInt(application.requestedAmount || "0");
		const tenure = application.requestedTenure || 12;
		const baseRate = Number.parseFloat(
			(bank?.config?.minRate as string) || "10",
		);
		const processingFee = Number.parseFloat(
			(bank?.config?.processingFee as string) || "1",
		);

		const monthlyRate = baseRate / 12 / 100;
		const emi =
			(amount * monthlyRate * (1 + monthlyRate) ** tenure) /
		...
		return {
			...
			fees: {
				...
				prepaymentCharges:
					bank?.config?.prepaymentCharges ||
					"Nil for floating rate, 2% for fixed rate",
				latePaymentFee:
					bank?.config?.latePaymentFee || "2% per month on overdue amount",
			...
			cooling_off: {
				period: "3 days",
				description:
					"Borrower can exit the loan within 3 days of disbursement by repaying principal plus pro-rata interest. No prepayment charges apply during this period.",
			},
			...
			rbiDisclosure: {
				statement:
					"This Key Facts Statement (KFS) is provided as per RBI Digital Lending Directions 2025.",
				...
				coolingOff:
					"You have a 3-day cooling-off period after disbursement to exit the loan.",
			},
		};
	}
	...
	async triggerBackgroundRouting(
		...
		const eligibilityResults = await (this as any).runEligibilityCheck(
			applicationId,
		);
		...
		if (borderlineResults.length === 0) {
			return {
				...
				existingOffers: eligibilityResults.filter((r: any) => r.eligible)
					.length,
			};
		}
