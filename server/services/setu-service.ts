...
export class SetuService {
  ...
  async createPaymentLink(amount: number, description: string) {
    try {
      ...
      logger.error('[SetuService] Payment link creation failed:', error.response?.data || error.message);
      return {
        success: false,
        message: error.response?.data?.message || 'Payment link creation failed'
      };
    }
  }
  ...
  async initiateConsent(phoneNumber: string) {
    try {
      ...
      logger.error('[SetuService] Consent initiation failed:', error.response?.data || error.message);
      return {
        success: false,
