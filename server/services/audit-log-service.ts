...
class AuditLogService {
	...
	async initialize(): Promise<void> {
      ...
      `);
			...
			console.warn(
				"[AuditLog] Failed to load last checksum, starting fresh:",
				error,
			);
			this.initialized = true;
		}
	...
	async verifyChainOnStartup(): Promise<boolean> {
		try {
			const { valid, brokenLinks, totalVerified } =
			...
			console.log(
				`[AuditLog] Chain integrity verified: ${totalVerified} entries`,
			);
			return true;
		} catch (error) {
	...
	private generateChecksum(
		entry: Omit<AuditLogEntry, "checksum" | "previousChecksum">,
		previousChecksum: string,
	): string {
		...
		const secret =
			process.env.COMPLIANCE_SECRET || "fintekpro_audit_fallback_key_2024";
	...
	async verifyChainIntegrity(
      ...
      `);
			...
			for (let i = 0; i < rows.length; i++) {
				...
				const recalculated = this.generateChecksum(
					entry,
				...
				if (i > 0) {
					...
					if (row.previous_checksum !== previous.checksum) {
						valid = false;
						brokenLinks.push(
							`${row.id} (CHAIN_BREAK: previous_checksum mismatch)`,
						);
					}
				}
			}
