...
class MFReturnsSyncService {
	...
	static getInstance(): MFReturnsSyncService {
		if (!MFReturnsSyncService.instance) {
			MFReturnsSyncService.instance = new MFReturnsSyncService();
		}
		return MFReturnsSyncService.instance;
	}
	...
	calculateFinancialRatios(navData: NavDataPoint[]): FinancialRatios {
		...
		for (let i = 1; i < sortedNav.length; i++) {
			const prevNav = Number.parseFloat(sortedNav[i - 1].nav);
			const currNav = Number.parseFloat(sortedNav[i].nav);
			if (prevNav > 0) {
		...
		let maxDrawdown = 0;
		let peak = Number.parseFloat(sortedNav[0].nav);
	...
	private findClosestNav(
		...
		for (const point of navData) {
			...
			if (diff < closestDiff && diff <= maxDaysOffset * 24 * 60 * 60 * 1000) {
				closestDiff = diff;
				closestNav = { nav: Number.parseFloat(point.nav), date: pointDate };
			}
		}
	...
	calculateReturnsFromHistory(navData: NavDataPoint[]): CalculatedReturns {
		...
		if (
			!sortedNavData[0]?.nav ||
			Number.isNaN(Number.parseFloat(sortedNavData[0].nav))
		) {
		...
		const currentNav = Number.parseFloat(sortedNavData[0].nav);
		...
		// Calculate returns
		const returns1y = nav1y
			? this.calculateCAGR(currentNav, nav1y.nav, 1)
			: null;
		const returns3y = nav3y
			? this.calculateCAGR(currentNav, nav3y.nav, 3)
			: null;
		const returns5y = nav5y
			? this.calculateCAGR(currentNav, nav5y.nav, 5)
			: null;
	...
	private onRateLimit(): void {
		...
		this.currentDelay = Math.min(
			BASE_DELAY * BACKOFF_MULTIPLIER ** this.consecutiveRateLimits,
			60000, // Max 60 second delay
		);
		console.log(
			`[MFReturnsSync] Rate limit detected, increasing delay to ${this.currentDelay}ms`,
		);
	}
		...
		try {
			...
			if (isRateLimit) {
				...
				if (retryAttempt < MAX_RETRY_ATTEMPTS) {
					...
					console.log(
						`[MFReturnsSync] Rate limited for ${schemeCode}, retrying in ${backoffDelay}ms (attempt ${retryAttempt + 1}/${MAX_RETRY_ATTEMPTS})`,
					);
					await new Promise((resolve) => setTimeout(resolve, backoffDelay));
					return this.fetchHistoricalNAV(schemeCode, retryAttempt + 1);
				}
				console.log(
					`[MFReturnsSync] Max retries exceeded for scheme ${schemeCode}`,
				);
			}
			return null;
		}
	...
	async getFundsNeedingSync(limit: number = 200): Promise<string[]> {
		...
		const fundsNeedingSync = await readDb
			.select({
	...
	async runBatchSync(maxFunds: number = 100): Promise<{
		...
		this.isRunning = true;
		console.log(
			`[MFReturnsSync] Starting batch sync for up to ${maxFunds} funds...`,
		);
		...
		try {
			...
			console.log(
				`[MFReturnsSync] Found ${fundsToSync.length} funds needing sync`,
			);
			...
			for (const schemeCode of fundsToSync) {
				try {
					...
					if (returns && returns.dataQuality !== "insufficient") {
						...
						if (successful % 10 === 0) {
							console.log(
								`[MFReturnsSync] Progress: ${successful}/${processed} successful`,
							);
						}
					} else {
						failed++;
					...
					console.log(
						`[MFReturnsSync] Batch sync error for ${schemeCode}: ${error.message}`,
					);
				}
			}
			...
			console.log(
				`[MFReturnsSync] Batch sync complete: ${successful}/${processed} successful, ${failed} failed`,
			);
		} finally {
	 ...
	 */
	async getReturnsForFunds(
		schemeCodes: string[],
	): Promise<Record<string, CalculatedReturns>> {
		...
		for (const fund of funds) {
			...
			if (fund.returns1y && isFresh) {
				results[fund.schemeCode] = {
					returns1y: Number.parseFloat(fund.returns1y as string) || null,
					returns3y: Number.parseFloat(fund.returns3y as string) || null,
					returns5y: Number.parseFloat(fund.returns5y as string) || null,
					currentNav: Number.parseFloat(fund.nav as string) || 0,
					dataQuality: "full",
				};
