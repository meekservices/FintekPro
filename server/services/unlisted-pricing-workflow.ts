...
export class UnlistedPricingWorkflowService {
  ...
  async saveDraftPrices(
    ...
    if (sellPrice <= buyPrice) {
      return { success: false, message: 'Sell price must be greater than buy price' };
    }
    ...
    const previousSellPrice = company.draftSellPrice || company.publishedSellPrice;
  ...
  async validatePrices(
    ...
    if (spread > 20) {
      warnings.push(`Spread of ${spread.toFixed(2)}% exceeds recommended 20% threshold`);
    }
    ...
    if (company?.publishedBuyPrice) {
      ...
      const buyPriceChange = Math.abs((buyPrice - publishedBuy) / publishedBuy) * 100;
      
      if (buyPriceChange > 15) {
        warnings.push(`Buy price change of ${buyPriceChange.toFixed(2)}% exceeds 15% from published price (₹${publishedBuy})`);
      }
    }
    
    if (company?.publishedSellPrice) {
      const publishedSell = Number(company.publishedSellPrice);
      const sellPriceChange = Math.abs((sellPrice - publishedSell) / publishedSell) * 100;
      
      if (sellPriceChange > 15) {
        warnings.push(`Sell price change of ${sellPriceChange.toFixed(2)}% exceeds 15% from published price (₹${publishedSell})`);
      }
    }
  ...
  async publishPrices(
    ...
    if (company.complianceStatus === 'blocked') {
      return { 
        success: false, 
        message: 'Cannot publish prices: Company has blocking compliance issues',
      };
    }
    ...
    if (previousBuyPrice) {
      priceChangePercent = ((Number(company.draftBuyPrice) - Number(previousBuyPrice)) / Number(previousBuyPrice)) * 100;
    }
   ...
   */
  async checkComplianceAndUpdate(companyId: string): Promise<ComplianceCheckResult> {
    ...
    const complianceStatus = hasBlockingFlags ? 'blocked' : (flags.length > 0 ? 'flagged' : 'cleared');
   ...
   */
  async getAuditLog(companyId: string, limit: number = 50): Promise<UnlistedAuditLog[]> {
    const logs = await db
      .select()
...
export const unlistedPricingWorkflowService = new UnlistedPricingWorkflowService();
