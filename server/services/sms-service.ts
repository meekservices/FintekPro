...
class SMSService {
	...
	constructor() {
		this.primaryPhone =
			process.env.TWILIO_PRIMARY_PHONE || process.env.TWILIO_PHONE_NUMBER || "";
		...
		if (this.messagingServiceSid) {
			console.log(
				`   Backup Messaging Service SID: ${this.messagingServiceSid.substring(0, 10)}...`,
			);
		}
	}
	...
	private async sendSMSWithFallback(
		...
		if (this.primaryPhone) {
			try {
				console.log(
					`📱 Trying primary phone ${this.primaryPhone} for ${to}...`,
				);
				...
				console.log(
					`⚠️ Primary phone failed: ${primaryError.message}, trying backup...`,
				);
			}
		}
		...
		if (this.messagingServiceSid) {
			try {
				...
				console.log(
					`⚠️ Messaging Service failed: ${backupError.message}, trying connector phone...`,
				);
			}
		}
	...
	async sendOTP(mobile: string, otp: string): Promise<boolean> {
		try {
			...
			if (result.success) {
				console.log(
					`✅ SMS OTP sent to ${formattedMobile} - SID: ${result.sid}`,
				);
			}
	...
	async sendPasswordResetOTP(mobile: string, otp: string): Promise<boolean> {
		try {
			...
			if (!isConfigured) {
				console.log(
					`📱 Password Reset OTP for ${mobile}: ${otp} (Twilio not configured)`,
				);
				return false;
			}
			...
			if (result.success) {
				console.log(
					`✅ Password reset SMS sent to ${formattedMobile} - SID: ${result.sid}`,
				);
			}
			return result.success;
		} catch (error) {
