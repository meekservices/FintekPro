...
import {
	partners,
	partnerHierarchyAgreements,
	partnerClientOwnership,
	users,
} from "@shared/schema";
...
export class PartnerHierarchyService {
	...
	static getInstance(): PartnerHierarchyService {
		if (!PartnerHierarchyService.instance) {
			PartnerHierarchyService.instance = new PartnerHierarchyService();
		}
		return PartnerHierarchyService.instance;
	}
	...
	async createPartner(data: {
		...
		if (data.parentPartnerId) {
			const parent = await db
				.select()
				.from(partners)
				.where(eq(partners.id, data.parentPartnerId))
				.limit(1);
			if (parent.length === 0)
				return { success: false, error: "Parent partner not found" };
			const parentPartner = parent[0];

			if (parentPartner.hierarchyStatus !== "ACTIVE")
				return { success: false, error: "Parent partner is not active" };
			if (parentPartner.kycStatus !== "VERIFIED")
				return { success: false, error: "Parent partner KYC not verified" };
			...
			if (!childLevel) {
				return {
					success: false,
					error: `${parentLevel} partners cannot create sub-partners (maximum hierarchy depth reached)`,
				};
			}
			...
			if (depth >= maxDepth)
				return {
					success: false,
					error: `Maximum hierarchy depth (${maxDepth}) reached`,
				};

			const existing = await db
				.select()
				.from(partners)
				.where(eq(partners.contactEmail, data.contactEmail))
				.limit(1);
			...
			if (integrityWarnings > 0) {
				console.warn(
					`[HierarchyIntegrity] ${integrityWarnings} issue(s) detected after creating partner ${newPartner.id}:`,
	...
	async approvePartner(
		...
		if (partner.approvalStatus === "APPROVED")
			return { success: false, error: "Partner already approved" };

		// Create agreement
		const [agreement] = await db
			.insert(partnerHierarchyAgreements)
			.values({
				partnerId,
    ...
    `);
		...
		return rows.map((row: any) => {
			...
			if (!maskPII) {
				...
				partner.contactEmail = localPart
					? `${localPart.substring(0, 2)}***@${domain || ""}`
					: "***";
				partner.contactPhone = row.contact_phone
					? `${row.contact_phone.substring(0, 4)}****`
					: null;
			}
	...
	async assignClientOwnership(
		...
		if (existing.length > 0) {
			if (existing[0].isLocked) {
				return {
					success: false,
					error: "Client ownership is locked and cannot be changed",
				};
			}
			// Update if not locked
			await db
				.update(partnerClientOwnership)
				.set({
					ownerPartnerId,
					updatedAt: new Date(),
				})
	...
	async getAllPartners(limit = 100, offset = 0): Promise<any[]> {
		...
		const hierarchyPartners = await db
			.select({
				id: partners.id,
		...
		const registeredEmails = new Set(
			hierarchyPartners.map((p) => p.contactEmail?.toLowerCase()),
		);
    ...
    `);
		...
		const merged = [
			...hierarchyPartners.map((p) => ({
				...p,
				sourceType: "hierarchy" as const,
			})),
			...userRows,
		];
