...
function yfInterval(tf: BarTimeframe): {
	interval: string;
	fallbackRange: string;
} {
	switch (tf) {
		case "1Min":
			return { interval: "1m", fallbackRange: "1d" };
		case "5Min":
			return { interval: "5m", fallbackRange: "1d" };
		case "15Min":
			return { interval: "15m", fallbackRange: "5d" };
		case "30Min":
			return { interval: "30m", fallbackRange: "5d" };
		case "1Hour":
			return { interval: "60m", fallbackRange: "5d" };
		case "4Hour":
			return { interval: "60m", fallbackRange: "1mo" };
		case "1Day":
			return { interval: "1d", fallbackRange: "1y" };
		case "1Week":
			return { interval: "1wk", fallbackRange: "5y" };
		case "1Month":
			return { interval: "1mo", fallbackRange: "max" };
		default:
			return { interval: "1d", fallbackRange: "1y" };
	}
}
...
class AlpacaMarketDataService {
	...
	testConnection(): { success: boolean; message: string; feed: string } {
		...
		return {
			success: true,
			message: "Alpaca Data API configured",
			feed: this.feed,
		};
	}
	...
	private async fetchYahooSnapshot(symbol: string): Promise<StockSnapshot> {
		...
		const dailyTs =
			lastIdx >= 0 ? new Date(timestamps[lastIdx] * 1000).toISOString() : now;
		const prevTs =
			prevIdx >= 0 ? new Date(timestamps[prevIdx] * 1000).toISOString() : now;
		...
		return {
			...
			dailyBar: {
				open: opens[lastIdx] ?? open,
				high: highs[lastIdx] ?? high,
				low: lows[lastIdx] ?? low,
				close: closes[lastIdx] ?? price,
				volume: vols[lastIdx] ?? volume,
				vwap: price,
				timestamp: dailyTs,
			},
			prevDailyBar: {
				open: opens[prevIdx] ?? prevClose,
				high: highs[prevIdx] ?? prevClose,
				low: lows[prevIdx] ?? prevClose,
				close: closes[prevIdx] ?? prevClose,
				volume: vols[prevIdx] ?? 0,
				vwap: prevClose,
				timestamp: prevTs,
			},
		};
	...
	private async fetchYahooFallback(
		symbols: string[],
		result: Map<string, StockSnapshot>,
	): Promise<void> {
		await pMap(
			symbols,
			async (sym) => {
	...
	private toQuote(snap: StockSnapshot): StockQuote {
		...
		return {
			...
			timestamp: new Date(
				snap.dailyBar.timestamp || snap.latestTrade.timestamp || Date.now(),
			).getTime(),
	...
	async getQuote(symbol: string): Promise<StockQuote | null> {
		...
		if (cached && this.valid(cached.cachedAt, QUOTE_CACHE_TTL))
			return cached.data;
	...
	async getBars(
		...
		const symList = (Array.isArray(symbols) ? symbols : [symbols]).map((s) =>
			s.toUpperCase(),
		);
		...
		if (this.isConfigured()) {
			try {
				const data = await this.alpacaBars(
					symList,
					timeframe,
					start,
					end,
				...
				logger.warn(
					`MarketData: Alpaca bars failed (${err.message}), falling back to Yahoo Finance`,
				);
			}
		}
		...
		await pMap(
			...
			async (sym) => {
				try {
					...
					logger.warn(
						`MarketData: getBars(YF) failed for ${sym}: ${err.message}`,
					);
					result.set(sym, []);
				}
			},
	...
	async getLatestQuotes(symbols: string[]): Promise<
		Map<
			string,
			{
				bidPrice: number;
	...
	> {
		...
		if (this.isConfigured()) {
			try {
				...
				logger.warn(
					`MarketData: getLatestTrades failed (${err.message}), falling back to snapshots`,
				);
			}
		}
	...
	async getStockDetails(symbol: string): Promise<StockDetails | null> {
		...
		if (cached && this.valid(cached.cachedAt, DETAILS_CACHE_TTL))
			return cached.data;
		...
		if (this.isConfigured()) {
			try {
				...
				const res = await axios.get(
					`${this.brokerBaseUrl}/v1/assets/${upper}`,
					{
						headers,
						timeout: 8_000,
					},
	...
	async searchSymbols(query: string, limit = 10): Promise<StockDetails[]> {
		if (this.isConfigured()) {
			try {
				...
				logger.warn(
					`MarketData: Alpaca asset search failed (${err.message}), falling back to Yahoo`,
				);
			}
		}
	...
	> {
		const meta: Record<
			string,
			{ name: string; category: string; expenseRatio: number }
		> = {
			SPY: {
		...
		return Object.keys(meta).map((sym) => {
			...
			return {
				...
				type: "ETF",
				currency: "USD",
				category: meta[sym].category,
				expenseRatio: meta[sym].expenseRatio,
				price: quote?.price,
				change: quote?.change,
				changePercent: quote?.changePercent,
			};
	...
	async getAvailableDatasets(): Promise<string[]> {
		logger.info(
			"MarketData: getAvailableDatasets — Polygon flat files not available via Alpaca Data API",
		);
		return [];
	}
	...
	async listFlatFiles(_prefix: string, _maxKeys = 50): Promise<any[]> {
		logger.info(
			"MarketData: listFlatFiles — Polygon flat files not available via Alpaca Data API",
		);
		return [];
	}
	...
	async getHistoricalDayAggs(_date: string): Promise<any[]> {
		logger.info(
			"MarketData: getHistoricalDayAggs — use getBars() with 1Day timeframe instead",
		);
		return [];
	}
	...
	async getEnrichedMarketData(
		symbol: string,
	): Promise<Partial<StockDetails> | null> {
		const upper = symbol.toUpperCase();
		try {
			...
			logger.warn(
				`MarketData: Failed to fetch enriched data for ${upper}: ${err.message}`,
			);
			return null;
		}
	}
	...
	async getBestBuys(
		...
		const ETFS: Array<{
			...
			{
				symbol: "SOXL",
				name: "Direxion Semicon Bull 3x ETF",
				riskLevel: "high",
			},
		];
		...
		for (const item of universe) {
			...
			if (momentum > 3) {
				score += 25;
				factors.push(`Strong momentum +${momentum.toFixed(1)}%`);
			} else if (momentum > 1) {
				score += 15;
				factors.push(`Positive momentum +${momentum.toFixed(1)}%`);
			} else if (momentum > 0) {
				score += 7;
				factors.push(`Mild uptrend +${momentum.toFixed(1)}%`);
			} else if (momentum < -3) {
				score -= 25;
				factors.push(`Weak momentum ${momentum.toFixed(1)}%`);
			} else if (momentum < -1) {
				score -= 15;
				factors.push(`Negative momentum ${momentum.toFixed(1)}%`);
			} else {
				score -= 5;
				factors.push(`Flat/slight decline ${momentum.toFixed(1)}%`);
			}
			...
			if (high > low) {
				...
				if (rangePct >= 70) {
					score += 15;
					factors.push(`Near intraday high (${rangePct.toFixed(0)}% of range)`);
				} else if (rangePct >= 40) {
					score += 5;
					factors.push(`Mid range (${rangePct.toFixed(0)}% of range)`);
				} else {
					score -= 10;
					factors.push(`Near intraday low (${rangePct.toFixed(0)}% of range)`);
				}
			}

			// Factor 3: Volume analysis (above-average volume = conviction)
			const vol = q.volume;
			const vwapImpliedAvg =
				snap.dailyBar.vwap > 0
					? snap.dailyBar.volume / (snap.dailyBar.vwap / q.price)
					: vol;
			const volRatio = vwapImpliedAvg > 0 ? vol / vwapImpliedAvg : 1;
			if (volRatio > 1.5) {
				score += 10;
				factors.push(`High volume (${volRatio.toFixed(1)}x avg)`);
			} else if (volRatio > 1.1) {
				score += 5;
				factors.push(`Above avg volume`);
			...
			if (spreadPct < 0.05) {
				score += 5;
				factors.push("Tight bid-ask spread");
			} else if (spreadPct > 0.5) {
				score -= 5;
				factors.push("Wide bid-ask spread");
			}
			...
			const type: "stock" | "etf" = ETFS.some((e) => e.symbol === item.symbol)
				? "etf"
				: "stock";
			...
			recommendations.push({
				...
				type,
				price: q.price,
				priceInr: Number.parseFloat((q.price * fxRate).toFixed(2)),
				change: Number.parseFloat(q.change.toFixed(2)),
				changePercent: Number.parseFloat(q.changePercent.toFixed(2)),
				volume: q.volume,
				bid: snap.latestQuote.bidPrice,
				ask: snap.latestQuote.askPrice,
				signal,
		...
		return {
			...
			disclaimer:
				"AI-generated recommendations are for informational purposes only. Past performance does not guarantee future results. FintekPro is not a SEBI-registered investment advisor. All US investments involve currency risk and geopolitical risk. Consult a qualified financial advisor before investing.",
		};
	}
}
