	...
	for (let attempt = 1; attempt <= maxRetries; attempt++) {
		try {
			...
			if (!isRetryable || attempt === maxRetries) {
				console.error(
					`[Zoho Meeting] ${operationName} failed after ${attempt} attempt(s):`,
					error.message,
				);
				throw error;
			}
			...
			console.warn(
				`[Zoho Meeting] ${operationName} attempt ${attempt} failed, retrying in ${delayMs}ms...`,
			);
			await delay(delayMs);
		}
	}
...
class ZohoMeetingService {
	...
	private async refreshAccessToken(): Promise<string> {
		...
		if (!this.refreshToken) {
			throw new Error(
				"Zoho refresh token not available. Please complete OAuth flow.",
			);
		}
	...
	async createMeeting(
		params: ZohoMeetingCreateParams,
	): Promise<ZohoMeetingResponse> {
		if (!this.isConfigured()) {
		...
		try {
			return await withRetry(async () => {
				...
				const response = await axios.post(
					`https://meeting.zoho.in/api/v2/${zsoid}/sessions.json`,
					{
						session: {
							...
							participants:
								params.participantEmails?.map((email) => ({ email })) || [],
						},
					},
					{
						headers: {
				...
				console.log(
					`[Zoho Meeting] Successfully created meeting: ${session.sessionKey || session.meetingKey}`,
				);
	...
	async testConnection(): Promise<{
		...
		if (!this.isConfigured()) {
			return {
				success: false,
				message:
					"Zoho Meeting credentials not configured (missing ZOHO_CLIENT_ID or ZOHO_CLIENT_SECRET)",
			};
		}
		...
		if (!this.refreshToken) {
			return {
				success: false,
				message:
					"Zoho refresh token not available (missing ZOHO_REFRESH_TOKEN)",
			};
		}
		...
		try {
			...
			console.error(
				`[Zoho Meeting] Test failed (HTTP ${statusCode}):`,
			...
			return {
				...
				details: {
					error:
						typeof errorDetails === "string"
							? "HTML error page returned - likely invalid ZSOID or API endpoint"
							: errorDetails,
					statusCode,
