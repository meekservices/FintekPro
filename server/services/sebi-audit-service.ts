...
export class SEBIAuditService {
	...
	private async flushPendingLogs(): Promise<void> {
		...
		try {
			...
			console.log(
				`[SEBIAuditService] Flushed ${logsToInsert.length} audit logs`,
			);
		} catch (error) {
	...
	async logImmediate(
		...
		try {
			...
			const [result] = await db
				.insert(sebiAuditLogs)
				.values(logEntry)
				.returning({ id: sebiAuditLogs.id });
	...
	async generateAuditSummary(proposalId: string): Promise<{
		...
		for (const log of logs) {
			actionBreakdown[log.actionType] =
				(actionBreakdown[log.actionType] || 0) + 1;
		...
		return {
			...
			actionBreakdown,
			complianceStatus: hasFlaggedItems
				? "FLAGGED"
				: hasRiskDisclosures
					? "COMPLIANT"
					: "PENDING_REVIEW",
