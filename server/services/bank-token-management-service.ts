...
import { db } from '../db';
import { bankOAuthTokens, type BankOAuthToken, type InsertBankOAuthToken } from '@shared/dsa-loan-schema';
import { encryptionService } from '../encryption-service';
import { bankCredentialsVaultService, type Environment, type CredentialType } from './bank-credentials-vault-service';
...
class BankTokenManagementService {
   ...
   */
  async getAccessToken(bankCode: string, environment: Environment): Promise<string | null> {
    ...
    if (storedToken?.refreshToken) {
      const refreshed = await this.refreshToken(bankCode, environment, storedToken.refreshToken);
      if (refreshed) {
   ...
   */
  async obtainNewToken(bankCode: string, environment: Environment): Promise<Token | null> {
    try {
      ...
      if (!clientId || !clientSecret) {
        console.error(`[TokenMgmt] Missing credentials for ${bankCode}/${environment}`);
        return null;
      }
      ...
      if (!tokenEndpoint) {
        console.error(`[TokenMgmt] No token endpoint configured for ${bankCode}`);
        return null;
      }
      ...
      if (token) {
        ...
        console.log(`[TokenMgmt] Obtained new token for ${bankCode}/${environment}`);
        return token;
      }

      return null;
    } catch (error: any) {
      console.error(`[TokenMgmt] Error obtaining token for ${bankCode}:`, error.message);
   ...
   */
  async refreshToken(bankCode: string, environment: Environment, refreshToken: string): Promise<Token | null> {
    try {
      const credentials = await bankCredentialsVaultService.getBankCredentials(
        bankCode,
        ['client_id', 'client_secret'],
      ...
      if (token) {
        ...
        console.log(`[TokenMgmt] Refreshed token for ${bankCode}/${environment}`);
        return token;
      }

      return null;
    } catch (error: any) {
      console.error(`[TokenMgmt] Error refreshing token for ${bankCode}:`, error.message);
      this.recordFailure(`${bankCode}:${environment}`);
      return null;
    }
  }
  ...
  private async storeToken(
    ...
    try {
      ...
      await db.update(bankOAuthTokens)
      ...
      const [existing] = await db.select({ refreshCount: bankOAuthTokens.refreshCount })
   ...
   */
  private async getStoredToken(bankCode: string, environment: Environment): Promise<Token | null> {
    try {
  ...
  private isTokenValid(token: Token): boolean {
    const now = new Date();
    const bufferTime = new Date(token.expiresAt.getTime() - this.REFRESH_BUFFER_MS);
   ...
   */
  private scheduleRefresh(bankCode: string, environment: Environment, token: Token): void {
    ...
    const refreshTime = token.expiresAt.getTime() - this.REFRESH_BUFFER_MS - Date.now();
   ...
   * Revoke a token
   */
  async revokeToken(bankCode: string, environment: Environment): Promise<boolean> {
    try {
      await db.update(bankOAuthTokens)
   ...
   */
  async getTokenStatus(bankCode: string, environment: Environment): Promise<{
  ...
  async cleanupExpiredTokens(): Promise<number> {
    const result = await db.update(bankOAuthTokens)
      .set({ status: 'expired', updatedAt: new Date() })
      .where(
        and(
          eq(bankOAuthTokens.status, 'active'),
