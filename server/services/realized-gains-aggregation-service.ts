...
class RealizedGainsAggregationService {
  private readonly STCG_EQUITY_RATE = 0.20; // 20% (Budget 2024)
  private readonly LTCG_EQUITY_RATE = 0.125; // 12.5% (Budget 2024)
  private readonly STCG_DEBT_RATE = 0.30; // Slab rate (assumed 30%)
  private readonly LTCG_DEBT_RATE = 0.20; // 20% with indexation (pre-Apr 2023)
  ...
  async aggregateRealizedGains(userId: string, fiscalYear?: string): Promise<FiscalYearGains> {
    ...
    const result: FiscalYearGains = {
      fiscalYear: fy,
      userId,
      stcg: { equity: 0, debtPreApr2023: 0, debtPostApr2023: 0, others: 0, total: 0 },
      ltcg: { equity: 0, equityWithGrandfathering: 0, debtWithIndexation: 0, debtWithoutIndexation: 0, others: 0, total: 0 },
  ...
  private async calculateQuarterlyBreakdown(
    ...
    for (const schedule of this.QUARTERLY_SCHEDULE) {
      ...
      const previousPercentage = this.QUARTERLY_SCHEDULE[this.QUARTERLY_SCHEDULE.indexOf(schedule) - 1]?.percentage || 0;
      const quarterTaxDue = gains.totalTaxLiability * (schedule.percentage - previousPercentage);
      
      const reminder = reminderMap.get(schedule.quarter);
      const taxPaid = reminder?.status === 'paid' 
        ? parseFloat(reminder.totalTaxLiability?.toString() || '0') 
        : 0;
      ...
      breakdown.push({
        ...
        totalGains: gains.totalRealizedGains * (schedule.percentage - previousPercentage),
  ...
  async getAdvanceTaxStatus(userId: string, fiscalYear?: string): Promise<AdvanceTaxStatus> {
    const fy = fiscalYear || this.getCurrentFiscalYear();
    ...
    for (let i = 0; i < this.QUARTERLY_SCHEDULE.length; i++) {
      ...
      const previousPercentage = i > 0 ? this.QUARTERLY_SCHEDULE[i - 1].percentage : 0;
      ...
      const quarterTax = gains.totalTaxLiability * (schedule.percentage - previousPercentage);
      const quarterData = gains.quarterlyBreakdown.find(q => q.quarter === schedule.quarter);
  ...
  async calculateAndStoreTradeGains(orderId: string): Promise<void> {
    ...
    const purchaseDate = order.purchaseDate ? new Date(order.purchaseDate) : null;
    const saleDate = order.settledAt || new Date();
    
    if (!purchaseDate) {
      console.warn(`[RealizedGains] No purchase date for order ${orderId}, using today`);
    }

    const actualPurchaseDate = purchaseDate || new Date();
    const holdingPeriodDays = Math.floor(
      (saleDate.getTime() - actualPurchaseDate.getTime()) / (1000 * 60 * 60 * 24)
    );

    const purchaseValue = parseFloat(order.purchaseValue?.toString() || '0');
    const saleValue = parseFloat(order.payoutAmount?.toString() || order.amount?.toString() || '0');
    ...
    if (isLTCG && actualPurchaseDate < new Date('2018-02-01')) {
      const jan312018Nav = await this.getJan312018Nav(order.isin || order.schemeCode);
      if (jan312018Nav) {
        const units = parseFloat(order.units?.toString() || '0');
    ...
    await db
      ...
      .set({
        ...
        gainType,
        holdingPeriodDays,
        grandfatheredValue: grandfatheredValue > 0 ? grandfatheredValue.toString() : null,
    ...
    console.log(`[RealizedGains] Calculated gains for order ${orderId}: ${gainType} gain of ₹${realizedGain.toFixed(2)}, tax ₹${estimatedTax.toFixed(2)}`);
  }

  private async getJan312018Nav(isinOrSchemeCode: string): Promise<number | null> {
    try {
      ...
      console.warn(`[RealizedGains] Could not fetch Jan 31, 2018 NAV for ${isinOrSchemeCode}`);
      return null;
    }
  }
  ...
  async recalculateUserReminders(userId: string): Promise<void> {
    ...
    for (let i = 0; i < this.QUARTERLY_SCHEDULE.length; i++) {
      ...
      if (existing.length > 0) {
        ...
        await db
          .insert(capitalGainsTaxReminders)
          .values({
            userId,
            financialYear: fy,
            quarter: schedule.quarter,
    ...
    console.log(`[RealizedGains] Updated advance tax reminders for user ${userId}, FY ${fy}`);
  }
}

export const realizedGainsAggregationService = new RealizedGainsAggregationService();
