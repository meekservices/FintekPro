...
class McaDataCacheService {
	...
	async getCompanyWithCache(
		...
		try {
			...
			if (!forceRefresh) {
				...
				if (cachedData) {
					...
					if (cacheAge < CACHE_TTL_HOURS) {
						console.log(
							`[MCA Cache] Serving cached data for ${cin} (age: ${cacheAge.toFixed(1)}h)`,
						);
						return {
							success: true,
							data: {
					...
					console.log(
						`[MCA Cache] Cache stale for ${cin} (age: ${cacheAge.toFixed(1)}h), refreshing...`,
					);
				}
			}
			...
			// Fetch from API
			console.log(
				`[MCA Cache] Fetching fresh data for ${cin} from Sandbox API...`,
			);
			...
			if (!apiResult.success || !apiResult.data) {
				console.log(
					`[MCA Cache] Sandbox API failed for ${cin}: ${apiResult.error?.message}`,
				);
				...
				try {
					...
					if (credhiveResult.success && credhiveResult.data) {
						...
						const credhiveMcaData: Partial<MCACompanyMasterData> = {
							...
							companyCategory:
								credhiveData.category || "Company limited by Shares",
							...
							incorporationDate: "",
							authorizedCapital:
								credhiveData.authorizedCapital?.toString() || "0",
							paidUpCapital: credhiveData.paidUpCapital?.toString() || "0",
						};

						await this.persistCompanyDataFromCredhive(
							credhiveMcaData,
							userId,
							runId,
						);
						await this.logIngestionComplete(
							runId,
							"completed",
							1,
							"Credhive fallback",
					...
					console.log(
						`[MCA Cache] Credhive fallback also failed: ${credhiveError.message}`,
					);
				}
				...
				if (staleCache) {
					console.log(
						`[MCA Cache] APIs failed, returning stale cache for ${cin}`,
					);
					return {
						success: true,
						data: {
				...
				return {
					...
					error:
						apiResult.error?.message ||
						"Failed to fetch company data from both Sandbox and CredHive",
					apiCallMade: true,
				};
			}
	 ...
	 */
	async getCachedCompany(
		cin: string,
	): Promise<Omit<CachedCompanyData, "fromCache" | "cacheAge"> | null> {
		try {
			// Get company master
			...
			console.error(
				`[MCA Cache] Error fetching cached data for ${cin}:`,
				error.message,
			);
			return null;
		}
	...
	async persistCompanyData(
		...
		try {
			...
			for (const bs of data.balanceSheets || []) {
				await this.persistFinancialSnapshot(
					cin,
					bs.financialYear,
					userId,
					runId,
			...
			console.log(
				`[MCA Cache] Persisted data for ${cin}: ${data.directors?.length || 0} directors, ${data.charges?.length || 0} charges, ${data.shareholding?.length || 0} shareholding records`,
			);
		} catch (error: any) {
			console.error(
				`[MCA Cache] Error persisting data for ${cin}:`,
				error.message,
			);
			throw error;
		}
	...
	async persistCompanyDataFromCredhive(
		...
		try {
			...
			console.error(
				`[MCA Cache] Error persisting CredHive data for ${cin}:`,
				error.message,
			);
			throw error;
		}
	...
	private async persistDirector(
		...
		try {
			...
			console.error(
				`[MCA Cache] Error persisting director ${director.din}:`,
				error.message,
			);
		}
	}
	...
	private async persistCharge(
		...
		try {
			...
			const chargeData = {
				...
				creationDate:
					charge.dateOfCreation || new Date().toISOString().split("T")[0],
				modificationDate: charge.dateOfModification || null,
				status: charge.status?.toLowerCase().includes("satisfied")
					? "satisfied"
					: "active",
				updatedAt: new Date(),
			};
			...
			console.error(
				`[MCA Cache] Error persisting charge for ${cin}:`,
				error.message,
			);
		}
	}
	...
	private async persistFinancialSnapshot(
		...
		try {
			...
			console.error(
				`[MCA Cache] Error persisting financial snapshot:`,
				error.message,
			);
		}
	}
		...
		try {
			const existing = await db
				...
				.where(
					and(
						...
						eq(
							mcaShareholdingPattern.financialYear,
							shareholding.financialYear,
						),
					),
				)
				.limit(1);
			...
			const shareholdingData = {
				...
				institutionalHolding:
					shareholding.institutionalHolding?.toString() || null,
			...
			console.error(
				`[MCA Cache] Error persisting shareholding:`,
				error.message,
			);
		}
	}
	 ...
	 * Log ingestion start
	 */
	private async logIngestionStart(
		runId: string,
		cin: string,
		userId?: string,
	): Promise<void> {
		try {
			...
			console.error(
				`[MCA Cache] Error logging ingestion start:`,
				error.message,
			);
		}
	}
	...
	private async logIngestionComplete(
		...
		try {
			...
			console.error(
				`[MCA Cache] Error logging ingestion complete:`,
				error.message,
			);
		}
	}
	...
	async getDataFreshnessStats(): Promise<{
		...
		try {
			...
			const [missing] = await db
				...
				.from(mcaCompanyMaster)
				.where(
					sql`${mcaCompanyMaster.lastFilingYear} IS NULL OR ${mcaCompanyMaster.lastFilingYear} NOT IN (${expectedFY}, ${previousFY})`,
				);
			...
			console.error(
				"[MCA Cache] Error getting data freshness stats:",
				error.message,
			);
			return {
	...
	async searchCachedCompanies(
		...
		return db
			...
			.from(mcaCompanyMaster)
			.where(
				sql`${mcaCompanyMaster.companyName} ILIKE ${searchPattern} OR ${mcaCompanyMaster.cin} ILIKE ${searchPattern}`,
			)
			.limit(limit);
	}
}
