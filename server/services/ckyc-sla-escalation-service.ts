...
const ESCALATION_CHAIN: EscalationConfig[] = [
	{
		...
		recipientEmail:
			process.env.COMPLIANCE_HEAD_EMAIL || "compliance@fintekpro.com",
	...
	},
	{
		...
		recipientEmail:
			...
			"compliance-manager@fintekpro.com",
...
class CkycSlaEscalationService {
	...
	async checkAndEscalateBreaches(): Promise<void> {
		if (this.isRunning) {
			console.log(
				"[CKYC SLA] Escalation check already in progress, skipping...",
			);
			return;
		}
		...
		console.log(
			`[CKYC SLA] Checking for SLA breaches at ${now.toISOString()}...`,
		);
		...
		try {
			...
			for (const caseRecord of activeCases) {
				...
				if (isBreached) {
					...
					const hoursOverdue = Math.floor(
						(now.getTime() - new Date(caseRecord.slaDeadline).getTime()) /
							(1000 * 60 * 60),
					);
			...
			console.log(
				`[CKYC SLA] Check complete: ${breachedCount} breached, ${escalatedCount} escalated`,
			);
		} catch (error) {
	 ...
	 */
	private async markCaseAsBreached(
		...
		await this.logAuditEvent({
			...
			eventData: {
				...
				hoursOverdue: Math.floor(
					(Date.now() - new Date(caseRecord.slaDeadline).getTime()) /
						(1000 * 60 * 60),
				),
			},
			actorRole: "system",
	...
	private async escalateCase(
		...
		const escalationConfig = ESCALATION_CHAIN.find(
		...
		const emailSent = await this.sendEscalationEmail(
			caseRecord,
			escalationConfig,
			hoursOverdue,
		);
		...
		console.log(
			`[CKYC SLA] Case ${caseRecord.id} escalated to level ${targetLevel} (${escalationConfig.recipientRole})`,
		);
	}
	...
	private async sendEscalationEmail(
		...
		try {
			...
			const slaDeadline = new Date(caseRecord.slaDeadline).toLocaleString(
				"en-IN",
				{
					timeZone: "Asia/Kolkata",
				},
      ...
      `;
			...
			console.log(
				`[CKYC SLA] Escalation email sent to ${escalationConfig.recipientEmail}`,
			);
			return true;
		} catch (error) {
	...
	private async logAuditEvent(
		data: Omit<InsertCkycAuditLog, "checksum" | "eventTimestamp">,
	): Promise<void> {
