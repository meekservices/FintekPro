...
class KYCReuseTokenService {
	...
	constructor() {
		...
		if (!this.JWT_SECRET) {
			console.warn(
				"⚠️ JWT_SECRET or ENCRYPTION_MASTER_KEY not set! KYC Reuse Token service will be crippled.",
			);
			console.warn(
				"   This is UNSAFE for production but allowed for diagnostic boot.",
			);
	...
	async generateToken(
		...
		try {
			...
			if (
				vault.isExpired ||
				(vault.kycExpiryDate && new Date() > new Date(vault.kycExpiryDate))
			) {
				return {
					success: false,
			...
			const fullName = vault.encryptedFullName
				? encryptionService.decrypt(vault.encryptedFullName)
				: null;
			const dob = vault.encryptedDateOfBirth
				? encryptionService.decrypt(vault.encryptedDateOfBirth)
				: null;
			const ckycKin = vault.encryptedCkycKin
				? encryptionService.decrypt(vault.encryptedCkycKin)
				: null;
			...
			if (vault.tokenizedPan) {
				const panResult = await tokenizationService.detokenize(
					vault.tokenizedPan,
					userId,
				);
			...
			const claims: KYCClaims = {
				...
				verifiedAt:
					vault.kycVerifiedAt?.toISOString() || new Date().toISOString(),
	...
	async validateToken(tokenId: string): Promise<ValidateTokenResult> {
		try {
			...
			// Check usage limit
			if (
				tokenRecord.maxUsageLimit &&
				(tokenRecord.usageCount || 0) >= tokenRecord.maxUsageLimit
			) {
				return {
					success: true,
					valid: false,
			...
			const decryptedPayload = encryptionService.decrypt(
				tokenRecord.encryptedJwtPayload,
			);
