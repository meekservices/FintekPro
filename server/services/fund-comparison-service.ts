...
export class FundComparisonService {
	...
	async compareFunds(
		...
		try {
			console.log(
				`Starting fund comparison for codes: ${fundCodes.join(", ")}`,
			);
	 ...
	 */
	private async getFundMetrics(
		fundCodes: string[],
		timePeriod: string,
	): Promise<FundMetrics[]> {
		const metricsPromises = fundCodes.map((code) =>
			this.calculateFundMetrics(code, timePeriod),
		);
	 ...
	 */
	private async calculateFundMetrics(
		schemeCode: string,
		timePeriod: string,
	): Promise<FundMetrics> {
		try {
			...
			return {
				...
				currentNAV: Number.parseFloat(fundData.currentNav || "0"),
				...
				expenseRatio: fundData.expenseRatio
					? Number.parseFloat(fundData.expenseRatio.toString())
					: null,
				aum: fundData.aum ? Number.parseFloat(fundData.aum.toString()) : null,
	 ...
	 */
	private async fetchHistoricalNAV(
		schemeCode: string,
	...
	private calculateReturns(
		...
		const currentNAV = Number.parseFloat(sortedData[0].nav);
	...
	private getReturnForPeriod(
		...
		const pastNAV = Number.parseFloat(closestData.nav);
	...
	private getAnnualizedReturn(
		...
		return ((1 + simpleReturn / 100) ** (1 / years) - 1) * 100;
	}
	...
	private calculateRiskMetrics(
		...
		for (let i = 1; i < sortedData.length; i++) {
			const prevNAV = Number.parseFloat(sortedData[i - 1].nav);
			const currentNAV = Number.parseFloat(sortedData[i].nav);
		...
		const navValues = sortedData.map((d) => Number.parseFloat(d.nav));
		let maxDrawdown = 0;
	...
	private determineBestPerformer(
		funds: FundMetrics[],
	): ComparisonResult["bestPerformer"] {
		...
		const bestExpense = funds.reduce((best, fund) =>
			(fund.expenseRatio || Number.POSITIVE_INFINITY) <
			(best.expenseRatio || Number.POSITIVE_INFINITY)
				? fund
				: best,
		);
	...
	private calculateSummaryMetrics(
		...
		const validReturns1Y = funds
			.map((f) => f.returns["1Y"])
			.filter((r) => r !== null) as number[];
		const validVolatility = funds
			.map((f) => f.volatility)
			.filter((v) => v !== null) as number[];
		const validExpense = funds
			.map((f) => f.expenseRatio)
			.filter((e) => e !== null) as number[];

		const averageReturns = {
			"1Y":
				validReturns1Y.length > 0
					? validReturns1Y.reduce((sum, r) => sum + r, 0) /
						validReturns1Y.length
					: 0,
		};

		const riskRange =
			validVolatility.length > 0
				? {
	...
	private generateRecommendation(
		funds: FundMetrics[],
		bestPerformer: ComparisonResult["bestPerformer"],
	): string {
		const fundCount = funds.length;
		const avgReturn =
			funds.reduce((sum, f) => sum + (f.returns["1Y"] || 0), 0) / fundCount;
		const avgExpense =
			funds.reduce((sum, f) => sum + (f.expenseRatio || 0), 0) / fundCount;
		...
		if (avgReturn > 15) {
			...
			recommendation +=
				"Below-average performance requires careful consideration. ";
		}

		if (avgExpense < 1.0) {
			recommendation +=
				"Low expense ratios make these cost-efficient choices. ";
		} else if (avgExpense < 2.0) {
			recommendation +=
				"Moderate expense ratios are acceptable for the category. ";
		} else {
	...
	private assessOverallRisk(funds: FundMetrics[]): "low" | "medium" | "high" {
		const avgVolatility =
			funds.reduce((sum, f) => sum + (f.volatility || 0), 0) / funds.length;
