...
class UnifiedAIRecommendationEngine {
	private _regimeCache: {
		data: { regime: string; signal_score: number; confidence: number };
		fetchedAt: number;
	} | null = null;
	...
	async analyzeProduct(
		...
		// Check cache first
		const cached = aiResponseCacheService.get(
			cacheKey,
			`${product.category}_recommendation`,
		);
		if (cached) {
							...
							enrichedFMP: {
								...
								companyRating: snapshot.companyRating
									? {
											...
											recommendation:
												snapshot.companyRating.ratingRecommendation,
										}
									: null,
		...
		const analysis = await this.analyzeWithPython(
			enrichedProduct,
			clientProfile,
		);
	...
	async generateRecommendation(
		...
		const avgScore =
			recommended.length > 0
				? recommended.reduce((sum, r) => sum + r.analysis.overallScore, 0) /
					recommended.length
				: 0;
	...
	private async fetchRegimeFromPython(): Promise<{
		regime: string;
		signal_score: number;
		confidence: number;
	} | null> {
		...
		if (
			this._regimeCache &&
			Date.now() - this._regimeCache.fetchedAt < TTL_MS
		) {
			return this._regimeCache.data;
		}
		try {
			const result = await callPython<any>("/api/regime/detect", "POST", {
				lookback_days: 90,
			});
			if (result?.regime) {
				this._regimeCache = {
					data: {
						...
						signal_score: result.signal_score ?? 0,
	...
	private async fetchMFPercentileRank(
		schemeCode: string,
	): Promise<{
		...
		try {
			...
			return {
				percentileRank:
					metrics.percentileRank != null
						? Number(metrics.percentileRank)
						: null,
				categoryRank:
					metrics.categoryRank != null ? Number(metrics.categoryRank) : null,
				categorySize:
					metrics.categorySize != null ? Number(metrics.categorySize) : null,
			};
		} catch {
			return { percentileRank: null, categoryRank: null, categorySize: null };
		}
	...
	private async analyzeWithPython(
		...
		let adjReturn = Math.min(
			100,
			Math.max(0, base.returnPotentialScore + delta.ret),
		);
		...
		if (product.category === "stocks" && product.rawData?.enrichedFMP) {
			...
			if (fmp.dcf?.upsidePercent != null) {
				adjReturn = Math.min(
					...
					Math.max(
						0,
						adjReturn + Math.min(15, Math.max(-15, fmp.dcf.upsidePercent / 10)),
					),
				);
			}
		...
		if (product.category === "mutual_funds" && product.id) {
			const { percentileRank, categoryRank, categorySize } =
				await this.fetchMFPercentileRank(product.id);
		...
		try {
			const mlAssets = [
				{
					...
					returns1y:
						product.rawData?.returns?.["1y"] ??
						product.rawData?.return1y ??
						null,
					returns3y:
						product.rawData?.returns?.["3y"] ??
						product.rawData?.return3y ??
						null,
					volatility: product.rawData?.volatility ?? product.volatility ?? null,
					sharpeRatio:
						product.rawData?.sharpeRatio ?? product.sharpeRatio ?? null,
					yield:
						product.dividendYield ?? product.rawData?.dividendYield ?? null,
					confidenceScore: base.confidenceScore,
				},
			];
			const mlResult = await callPython<any>("/api/ml/score", "POST", {
				assetClass:
					product.category === "mutual_funds"
						? "mutual_fund"
						: product.category,
				assets: mlAssets,
			...
			if (mlResult?.results?.length && !mlResult.error) {
				...
				if (mlScore?.predictedReturn != null) {
					const mlReturnDelta = Math.min(
						10,
						Math.max(-10, mlScore.predictedReturn * 100),
					);
					adjReturn = Math.min(
						100,
						Math.max(0, adjReturn + mlReturnDelta * 0.3),
					);
					adjConf = Math.min(
						95,
						Math.max(20, adjConf + (mlScore.confidence - 50) * 0.1),
					);
		...
		const adjOverall = Math.round(
			adjReturn * 0.35 +
				(100 - adjRisk) * 0.25 +
				base.qualityScore * 0.25 +
				base.valuationScore * 0.15,
		);
		...
		return {
			...
			keyStrengths: [
				...
				...(regime.regime === "bull"
					? ["Favourable market conditions (regime: bull)"]
					: []),
				...(mfRankNote?.startsWith("Top") ? [mfRankNote] : []),
			].slice(0, 4),
			keyRisks: [
				...base.keyRisks.filter((r) => r !== "Market risk").slice(0, 2),
				...(regime.regime === "bear"
					? [`Bear market regime (score: ${regime.signal_score.toFixed(2)})`]
					: []),
				...(regime.regime === "high_vol"
					? [`Elevated volatility regime detected`]
					: []),
				...(mfRankNote?.startsWith("Below") ? [mfRankNote] : []),
				"Market risk",
			].slice(0, 4),
	...
	private analyzeWithRules(
		...
		return {
			...
			selectionRationale: this.generateRuleBasedRationale(
				product,
				overallScore,
			),
			...
			regulatoryWarnings: this.generateRegulatoryWarnings(
				product,
				clientProfile,
			),
...
Provide analysis as JSON with these fields:
{
  ...
  "regulatoryWarnings": ["warning if any"]
		...
		try {
			...
			return {
				...
				selectionRationale:
					...
					"Analysis based on historical performance and risk metrics.",
				investmentThesis:
					parsed.investmentThesis ||
					"Potential for steady returns with manageable risk.",
	...
	private calculateMatchScore(
		...
		// Risk alignment
		const riskMatch = this.getRiskMatchScore(
	...
	private getRiskMatchScore(
		productRisk: RiskProfile,
		clientRisk: RiskProfile,
	): number {
	...
	private isKycCompliant(product: ProductData, client: ClientProfile): boolean {
		...
		const required =
			product.kycRequirement || CATEGORY_CONFIG[product.category].defaultKyc;
	...
	private calculateDiversification(products: RankedProduct[]): number {
		...
		const sectors = new Set(
			products.map((p) => p.product.sector).filter(Boolean),
		);
	...
	private generateRegulatoryWarnings(
		product: ProductData,
		client?: ClientProfile,
	): string[] {
	...
	async runPrompt<T = any>(options: {
		...
		result: T;
		modelUsed: AIProvider | "fallback";
		cacheHit: boolean;
	}> {
		const { prompt, category, systemPrompt, responseParser, fallback } =
			options;

		const cacheKey =
			options.cacheKey || `runPrompt:${category}:${this.hashPrompt(prompt)}`;
				...
				[
					{
						...
						content:
							systemPrompt ||
							"You are a SEBI-registered investment advisor. Respond with valid JSON only.",
					},
...
export const unifiedAIRecommendationEngine =
	new UnifiedAIRecommendationEngine();
