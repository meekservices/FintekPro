...
class BackgroundJobQueue extends EventEmitter {
  ...
  private handlers: Map<JobType, (payload: JobPayload) => Promise<any>> = new Map();
   ...
   */
  registerHandler(type: JobType, handler: (payload: JobPayload) => Promise<any>): void {
  ...
  async addJob(
    ...
    console.log(`📥 [JOB_QUEUE] Added job ${jobId} (${type}) for user ${payload.userId}`);
    ...
    try {
      ...
      if (job.callbackFn) {
        try {
          ...
          console.error(`[JOB_QUEUE] Callback error for job ${job.id}:`, callbackError);
        }
      }
      ...
      if (job.retryCount < job.maxRetries) {
        ...
        const delay = Math.min(
          this.config.retryDelayMs * Math.pow(2, job.retryCount - 1),
          this.config.maxRetryDelayMs
        );
        job.status = 'pending';
        job.nextRetryAt = new Date(Date.now() + delay);
        
        console.log(`🔄 [JOB_QUEUE] Job ${job.id} failed, retry ${job.retryCount}/${job.maxRetries} in ${delay}ms`);
        ...
        job.status = 'failed';
        job.completedAt = new Date();
        console.error(`❌ [JOB_QUEUE] Job ${job.id} failed after ${job.maxRetries} retries: ${error.message}`);
  ...
  private async sendWebhook(job: Job): Promise<void> {
    ...
    try {
      ...
      console.error(`[JOB_QUEUE] Webhook failed for job ${job.id}: ${error.message}`);
    }
  }
