...
const INSTRUMENT_GATEWAYS: Record<InstrumentType, GatewayConfig> = {
  ...
  us_stock: {
    ...
    adminNote:   'Missing Alpaca credentials. Add ALPACA_API_KEY, ALPACA_SECRET_KEY, ALPACA_BASE_URL.',
  },
  indian_stock: {
    provider:    'iifl',
    ...
    adminNote:   'IIFL API keys are still being procured. Contact IIFL onboarding to expedite.',
  },
  bond: {
    provider:    'bse_star',
    displayName: 'BSE STAR MF — Bonds',
    envKeys:     ['BSE_STAR_API_KEY', 'BSE_STAR_MEMBER_ID'],
    clientMessage: 'Bond purchases are being set up. Please check back shortly.',
  ...
  gold: {
    ...
    adminNote:   'Missing AUGMONT_API_KEY. Contact Augmont to obtain credentials.',
  },
  insurance: {
  ...
  sgb: {
    ...
    clientMessage: 'SGB subscription windows open periodically. No active window currently.',
...
function computeReadiness(instrumentType: InstrumentType): GatewayReadiness {
  ...
  return {
    ...
    displayName: config.displayName,
    missingKeys,
    clientMessage: missingKeys.length > 0
      ? (config.clientMessage ?? 'This service is temporarily unavailable.')
      : '',
    adminNote: missingKeys.length > 0
 ...
 */
export function getAllGatewayStatuses(): Record<InstrumentType, GatewayReadiness> {
...
export function logGatewayReadinessSummary(): void {
  ...
  const ready = Object.entries(all).filter(([, v]) => v.ready).map(([k]) => k);
  const comingSoon = Object.entries(all).filter(([, v]) => v.comingSoon).map(([k]) => k);
  const unavailable = Object.entries(all).filter(([, v]) => !v.ready && !v.comingSoon).map(([k]) => k);
  ...
  if (unavailable.length > 0) {
    logger.warn('[GatewayReadiness] Some gateways are unavailable due to missing credentials', {
