...
const RBI_PURPOSE_CODES: Map<string, PurposeCode> = new Map([
  ...
  ['S0001', {
    code: 'S0001',
    description: 'Investment in equity shares of overseas company (under ODI)',
    ...
    a2FormRequired: true,
    documentsRequired: ['Board Resolution', 'ODI Form', 'Valuation Report', 'CA Certificate'],
    tcsApplicable: false,
    tcsRate: 0
  }],
  ['S0002', {
  ...
  ['S0503', {
    ...
    form15cbRequired: true,
    a2FormRequired: true,
    documentsRequired: ['Royalty agreement', 'RBI approval if needed', 'Invoice'],
    tcsApplicable: false,
    tcsRate: 0
  }],
...
export interface LRSTransaction {
  ...
  tcsRate: number;
  tcsPan?: string;
  status: 'draft' | 'pending_ad_approval' | 'approved' | 'remitted' | 'rejected';
  remarks?: string;
  createdAt: Date;
  updatedAt: Date;
}
...
export interface LRSAlert {
  type: 'threshold_50' | 'threshold_75' | 'threshold_90' | 'limit_exceeded' | 'tcs_threshold';
  message: string;
  triggeredAt: Date;
...
class FEMAComplianceService {
  ...
  constructor() {
    ...
    console.log(`   TCS Threshold: ₹${this.TCS_THRESHOLD_INR.toLocaleString()}`);
  }
  ...
  getPurposeCodesByCategory(category: 'capital_account' | 'current_account'): PurposeCode[] {
    return this.getAllPurposeCodes().filter(pc => pc.category === category);
  }

  validatePurposeCode(code: string, amountUSD: number): { valid: boolean; errors: string[]; warnings: string[] } {
    ...
    if (purposeCode.lrsApplicable && purposeCode.maxLimitUSD && amountUSD > purposeCode.maxLimitUSD) {
      errors.push(`Amount exceeds LRS limit of $${purposeCode.maxLimitUSD.toLocaleString()} for purpose code ${code}`);
    }

    if (purposeCode.form15cbRequired) {
      warnings.push('Form 15CB (CA Certificate) is required for this remittance');
    }
    ...
    if (purposeCode.documentsRequired.length > 3) {
      warnings.push(`Multiple documents required: ${purposeCode.documentsRequired.join(', ')}`);
    }
  ...
  async getLRSStatus(userId: string, financialYear?: string): Promise<LRSLimitStatus> {
    const fy = financialYear || this.getCurrentFinancialYear();
    const userTransactions = this.transactions.get(userId) || [];
    const fyTransactions = userTransactions.filter(t => t.financialYear === fy && t.status === 'remitted');

    const totalRemittedUSD = fyTransactions.reduce((sum, t) => sum + t.amountUSD, 0);
    const remainingLimitUSD = Math.max(0, this.LRS_LIMIT_USD - totalRemittedUSD);
  ...
  async checkLRSEligibility(userId: string, proposedAmountUSD: number): Promise<{
    eligible: boolean;
    ...
    const shortfall = eligible ? 0 : proposedAmountUSD - status.remainingLimitUSD;
  ...
  async generateA2Form(transactionId: string, data: Omit<A2FormData, 'formNumber' | 'generatedAt' | 'status' | 'documentHash'>): Promise<A2FormData> {
    ...
    await this.logAudit('a2_form', formNumber, 'generate', data.applicantDetails.pan, {
      transactionId,
  ...
  async submitA2Form(formNumber: string): Promise<{ success: boolean; acknowledgementNumber?: string; error?: string }> {
    ...
    if (form.status !== 'generated') {
      return { success: false, error: `Cannot submit form in ${form.status} status` };
    }

    form.status = 'submitted';
    const acknowledgementNumber = `ACK-${Date.now()}-${nanoid(6)}`;

    await this.logAudit('a2_form', formNumber, 'submit', form.applicantDetails.pan, {
  ...
  async generateADCertificate(
    ...
    await this.logAudit('ad_certificate', certificateNumber, 'issue', data.applicantPan, {
      transactionId,
      purposeCode: data.purposeCode,
  ...
  async getADCertificate(certificateNumber: string): Promise<ADCertificate | undefined> {
  ...
  async recordTransaction(transaction: Omit<LRSTransaction, 'id' | 'createdAt' | 'updatedAt'>): Promise<LRSTransaction> {
    ...
    await this.logAudit('lrs_transaction', newTransaction.id, 'create', transaction.userId, {
  ...
  async updateTransactionStatus(
    ...
    await this.logAudit('lrs_transaction', transactionId, 'status_change', userId, {
      previousStatus,
      newStatus: status,
      remarks
    });
  ...
  async generateComplianceReport(userId: string, financialYear?: string): Promise<{
    userId: string;
    financialYear: string;
