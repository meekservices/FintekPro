...
class AIRecommendationSyncService {
	...
	private initialize() {
		...
		if (apiKey) {
			...
			console.log(
				"⚠️ AI Recommendation Sync Service: No API key, will use rule-based fallback",
			);
		}
	}
	...
	private getRuleBasedAnalysis(product: AifPmsData): AIAnalysisResult {
		const return1Y = Number.parseFloat(product.return1Y || "0");
		const riskScore = product.riskScore || 5;
		const volatility = Number.parseFloat(product.volatility || "15");
		...
		const suitabilityScore = Math.min(
			...
			Math.max(
				...
				(return1Y > 15 ? 3 : return1Y > 10 ? 2 : 1) +
					...
					(Number.parseFloat(product.sharpeRatio || "0") > 1 ? 2 : 1) +
					(Number.parseFloat(product.aum || "0") > 100000000 ? 2 : 1),
			),
		);
	...
	private inferSector(product: AifPmsData): string {
		...
		if (name.includes("bank") || name.includes("financ"))
	...
	async analyzeWithAI(
		...
		const productDataList: ProductData[] = products.map((p) => ({
			...
			returns1Y: Number.parseFloat(p.return1Y || "0"),
			returns3Y: Number.parseFloat(p.return3Y || "0"),
			returns5Y: Number.parseFloat(p.return5Y || "0"),
			volatility: Number.parseFloat(p.volatility || "15"),
			sharpeRatio: Number.parseFloat(p.sharpeRatio || "0"),
			maxDrawdown: Number.parseFloat(p.maxDrawdown || "0"),
			aum: Number.parseFloat(p.aum || "0"),
			minInvestment: Number.parseFloat(p.minInvestment || "0"),
		...
		for (let i = 0; i < productDataList.length; i += batchSize) {
			...
			for (const productData of batch) {
				try {
					...
					console.log(
						`[AI Sync] Analyzed ${productData.name} via unified engine (cache: ${analysis.cacheHit}, model: ${analysis.modelUsed})`,
					);
				} catch (error) {
					console.error(
						`[AI Sync] Unified engine analysis failed for ${productData.name}:`,
						error,
					);
					...
					if (originalProduct) {
						results.set(
							productData.id,
							this.getRuleBasedAnalysis(originalProduct),
						);
					}
				}
			}
...
${idx + 1}. ID: ${p.id}
   ...
   Min Investment: ₹${Number.parseFloat(p.minInvestment || "0").toLocaleString("en-IN")}
   ...
   AUM: ₹${Number.parseFloat(p.aum || "0").toLocaleString("en-IN")}
`,
			)
			.join("\n");
...
Respond ONLY with the JSON array, no other text.`;
			...
			for (const analysis of analyses) {
				if (analysis.id && analysis.riskProfile) {
					results.set(analysis.id, {
						riskProfile:
							analysis.riskProfile as AIAnalysisResult["riskProfile"],
						suitabilityScore: Math.min(
							10,
							Math.max(1, analysis.suitabilityScore || 5),
						),
						selectionRationale: analysis.selectionRationale || "",
						investmentThesis: analysis.investmentThesis || "",
	...
	async previewSync(
		...
		console.log(
			`[AI Sync] Starting preview: ${aifLimit} AIFs, ${pmsLimit} PMS`,
		);
		...
		const existing = await db
			.select({
				productId: recommendationProducts.productId,
		...
		const existingIds = new Set(
			existing.map((e) => `${e.productType}:${e.productId}`),
		);
		...
		console.log(
			`[AI Sync] Preview complete: ${newAifs} new AIFs, ${newPms} new PMS`,
		);
	...
	async executeSync(
			...
			try {
				await db.insert(recommendationProducts).values({
					...
					riskLevel: item.product.riskScore
						? item.product.riskScore <= 3
							? "Low"
							: item.product.riskScore <= 6
								? "Moderate"
								: "High"
						: "Moderate",
				...
				console.error(
					`[AI Sync] Failed to sync ${item.product.name}:`,
					error.message,
				);
		...
		console.log(
			`[AI Sync] Completed: ${imported} imported, ${preview.summary.existingAifs + preview.summary.existingPms} skipped, ${errors.length} errors`,
		);
