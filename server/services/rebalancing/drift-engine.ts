...
export class DriftEngine {
	...
	async calculateDrift(
		...
		for (const category of categories) {
			...
			const driftStatus: "DRIFT_BREACH" | "WITHIN_BAND" =
				Math.abs(drift) > (policy.toleranceBandPct ?? 5)
					? "DRIFT_BREACH"
					: "WITHIN_BAND";
		...
		logger.info(
			`[DriftEngine] Initial drift analysis complete for ${prospectId}`,
		);

		// 2. Quant Optimization (MVO)
		try {
			const quantInput = {
				assetsData: driftMetrics
					.filter((dm) => dm.targetPercent > 0)
					.map((dm) => ({
			...
			if (
				quantResult.usedMvo &&
				Object.keys(quantResult.optimizedWeights).length > 0
			) {
				const quantAllocations = quantOrchestrator.convertWeightsToAllocations(
					quantResult.optimizedWeights,
					categories,
				);
				for (const dm of driftMetrics) {
					if (quantAllocations[dm.category] !== undefined) {
						...
						if (newTarget !== dm.targetPercent) {
							logger.info(
								`[DriftEngine][Quant] ${dm.category}: target ${dm.targetPercent}% → ${newTarget}% (quant-optimized)`,
							);
							...
							dm.driftStatus =
								Math.abs(dm.drift) > (policy.toleranceBandPct ?? 5)
									? "DRIFT_BREACH"
									: "WITHIN_BAND";
						}
					}
			...
			if (quantResult.preemptiveRebalanceRecommended) {
				logger.warn(
					`[DriftEngine][Quant] Preemptive rebalance triggered for: ${quantResult.highRiskCategories.join(", ")}`,
				);
				for (const dm of driftMetrics) {
					if (
						quantResult.highRiskCategories.includes(dm.category) &&
						dm.driftStatus === "WITHIN_BAND"
					) {
			...
			logger.warn(
				`[DriftEngine] Quant orchestrator error: ${quantError.message}`,
			);
		}

		// 3. Risk Engine (Volatility Limits)
		const currentPortfolioVolatility =
			totalValue > 0
				? driftMetrics.reduce(
						(sum, dm) =>
							sum +
							(dm.currentPercent / 100) *
								(CATEGORY_RISK_MAP[dm.category] || 15),
						0,
					)
				: 0;
		...
		for (const dm of driftMetrics) {
			...
			if (dm.drift < 0 && dm.driftStatus === "DRIFT_BREACH") {
				...
				const projectedVol =
					currentPortfolioVolatility + additionalWeight * catVol;
				dm.riskFlag =
					projectedVol >
					(policy.targetVolatilityPct ?? 15) +
						(policy.riskToleranceBandPct ?? 3)
						? "VOL_BREACH"
						: "OK";
			} else {
				dm.riskFlag = "OK";
		...
		for (const dm of driftMetrics) {
			...
			const estimatedCost =
				changeAmount * ((policy.brokerageRatePct ?? 0.03) / 100);
			dm.costEstimate = estimatedCost;
			dm.costFlag =
				changeAmount < (policy.minTradeValueInr ?? 5000) ||
				estimatedCost > changeAmount * 0.02
					? "TOO_EXPENSIVE"
					: "ACCEPTABLE";
		}
		...
		for (const dm of driftMetrics) {
			dm.rawAction = this.determineAction(
				dm.drift,
				dm.driftStatus,
				dm.riskFlag || "OK",
				dm.costFlag || "ACCEPTABLE",
			);
			...
			if (
				dm.rawAction === "HOLD" ||
				dm.rawAction === "HOLD_COST_FILTER" ||
				dm.rawAction === "HOLD_RISK_LIMIT"
			) {
				dm.rationaleCode =
					dm.rawAction === "HOLD_COST_FILTER"
						? "COST_FILTER"
	...
	private determineAction(
		...
		if (riskFlag === "VOL_BREACH")
			return drift > 0 ? "REDUCE" : "HOLD_RISK_LIMIT";
		if (driftStatus === "DRIFT_BREACH")
			return drift > 0 ? "REDUCE" : "INCREASE";
		return "HOLD";
	}
}
