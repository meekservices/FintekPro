...
class AuditLogArchivalService {
	...
	private getObjectPath(
		entry: Omit<AuditLogEntry, "entryId" | "checksum" | "previousChecksum">,
	): string {
	 ...
	 */
	async archiveAuditEntry(
		entry: Omit<AuditLogEntry, "entryId" | "checksum" | "previousChecksum">,
	): Promise<ArchivalResult> {
		if (!this.bucketName || !this.privateDir) {
			console.warn(
				"[Audit Archival] Object storage not configured, skipping archival",
			);
			return {
				success: false,
				entryId: "",
		...
		try {
			...
			expirationDate.setFullYear(
				expirationDate.getFullYear() + RETENTION_YEARS,
			);
			...
			console.log(
				`[Audit Archival] Archived entry ${entryId} to ${objectPath}`,
			);
	...
	async verifyEntryIntegrity(
		objectPath: string,
	): Promise<{ valid: boolean; entry?: AuditLogEntry; error?: string }> {
		if (!this.bucketName) {
		...
		try {
			...
			if (storedChecksum !== calculatedChecksum) {
				console.error(
					`[Audit Archival] Integrity violation detected for ${objectPath}`,
				);
				return {
					valid: false,
					entry,
