...
class BankAPIRateLimiter {
	...
	private waitQueues: Map<
		string,
		Array<{ resolve: () => void; timestamp: number }>
	> = new Map();
	...
	tryAcquire(bankCode: string, operation: string = "DEFAULT"): boolean {
		...
		const tokensNeeded =
			OPERATION_WEIGHTS[operation] || OPERATION_WEIGHTS.DEFAULT;
		...
		// Track peak usage
		const currentUsage =
			(bucket.config.maxTokens - bucket.tokens) / bucket.config.maxTokens;
	...
	async acquire(
		...
		if (waitTimeMs > timeoutMs) {
			console.warn(
				`[RateLimiter] ${bankCode}: Would exceed timeout (${waitTimeMs}ms > ${timeoutMs}ms)`,
			);
			return false;
		}
		...
		metrics.averageWaitTime =
			(metrics.averageWaitTime * (metrics.totalRequests - 1) + actualWaitTime) /
			metrics.totalRequests;
