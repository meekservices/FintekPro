...
class InstrumentSearchService {
  ...
  async search(query: string, limit: number = 20): Promise<InstrumentSearchResult[]> {
  ...
  private async searchMutualFunds(searchTerm: string, isIsinSearch: boolean, limit: number): Promise<InstrumentSearchResult[]> {
    try {
      ...
      return funds.map(fund => {
        ...
        const isinMatch = [fund.isin, fund.isinGrowth, fund.isinDividendPayout, fund.isinDividendReinvest]
          .filter(Boolean)
        ...
        if (isinMatch) {
          ...
          matchScore = fund.schemeName?.toUpperCase().startsWith(searchTerm) ? 85 : 70;
        }
  ...
  private async searchStocks(searchTerm: string, isIsinSearch: boolean, limit: number): Promise<InstrumentSearchResult[]> {
    try {
      const searchPattern = `%${searchTerm}%`;
      ...
      return stocks.map(stock => {
        ...
        if (stock.isin?.toUpperCase().includes(searchTerm)) {
          ...
          matchScore = stock.companyName?.toUpperCase().startsWith(searchTerm) ? 80 : 65;
        }
        ...
        return {
          ...
          currentPrice: stock.currentPrice ? parseFloat(stock.currentPrice) : undefined,
          matchScore,
          matchField,
        };
      });
  ...
  private async searchBonds(searchTerm: string, isIsinSearch: boolean, limit: number): Promise<InstrumentSearchResult[]> {
    try {
      const searchPattern = `%${searchTerm}%`;
      ...
      return bonds.map(bond => {
        ...
        if (bond.isin?.toUpperCase().includes(searchTerm)) {
          ...
          matchScore = bond.issuerName?.toUpperCase().startsWith(searchTerm) ? 80 : 65;
        }
        ...
        return {
          ...
          subCategory: bond.maturityDate ? `Maturity: ${bond.maturityDate}` : undefined,
          currentPrice: bond.faceValue ? parseFloat(bond.faceValue) : undefined,
          matchScore,
          matchField,
        };
      });
  ...
  async getInstrumentDetails(isin: string): Promise<InstrumentSearchResult | null> {
  ...
  async enrichHoldingWithInstrumentData(holding: {
    ...
    if (holding.symbol) {
      ...
      const exactMatch = results.find(r => 
        r.symbol?.toUpperCase() === holding.symbol?.toUpperCase()
      );
