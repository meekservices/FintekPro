...
const MIN_BENCHMARK_CONFIDENCE = 0.70;
...
class MfRelativeMetricsEngine {
  ...
  async alignTimeSeries(schemeCode: string, indexCode: string, years: number = 3): Promise<AlignedTimeSeries | null> {
    ...
    const fundNavData = await db.select({
    ...
    if (fundNavData.length < MIN_DATA_POINTS) {
      console.log(`[RelativeMetrics] Insufficient fund data for ${schemeCode}: ${fundNavData.length} points`);
      return null;
    }
    ...
    const benchmarkNavData = await db.select({
    ...
    if (benchmarkNavData.length < MIN_DATA_POINTS) {
      console.log(`[RelativeMetrics] Insufficient benchmark data for ${indexCode}: ${benchmarkNavData.length} points`);
      return null;
    }
    ...
    for (const b of benchmarkNavData) {
      if (b.navDate && b.closeValue) {
        benchmarkByDate.set(String(b.navDate), parseFloat(b.closeValue));
      }
    }
    ...
    for (const f of fundNavData) {
      ...
      if (benchmarkValue !== undefined) {
        alignedFundNavs.push(parseFloat(f.nav));
    ...
    if (alignedFundNavs.length < MIN_DATA_POINTS) {
      console.log(`[RelativeMetrics] Insufficient aligned data: ${alignedFundNavs.length} points`);
      return null;
    }
    ...
    for (let i = 1; i < alignedFundNavs.length; i++) {
      const fundReturn = (alignedFundNavs[i] - alignedFundNavs[i - 1]) / alignedFundNavs[i - 1];
      const benchmarkReturn = (alignedBenchmarkNavs[i] - alignedBenchmarkNavs[i - 1]) / alignedBenchmarkNavs[i - 1];
      
      if (isFinite(fundReturn) && isFinite(benchmarkReturn)) {
    ...
    if (fundReturns.length < MIN_DATA_POINTS - 1) {
      console.log(`[RelativeMetrics] Insufficient return data: ${fundReturns.length} points`);
      return null;
    }
  ...
  calculateRelativeMetrics(alignedData: AlignedTimeSeries): RelativeMetrics {
    ...
    if (beta !== null) {
      const expectedReturn = dailyRiskFreeRate * TRADING_DAYS_PER_YEAR + 
    ...
    const trackingError = Math.sqrt(trackingErrorSum / (n - 1)) * Math.sqrt(TRADING_DAYS_PER_YEAR);
    ...
    return {
      ...
      treynorRatio: treynorRatio !== null ? Math.round(treynorRatio * 10000) / 10000 : null,
      informationRatio: informationRatio !== null ? Math.round(informationRatio * 10000) / 10000 : null,
      trackingError: trackingError > 0 ? Math.round(trackingError * 10000) / 10000 : null,
    };
  }

  async computeAndStoreMetrics(schemeCode: string, isin: string): Promise<boolean> {
    const benchmarkMapping = await mfBenchmarkMappingService.getBenchmarkMapping(isin);
    ...
    if (benchmarkMapping.confidenceScore < MIN_BENCHMARK_CONFIDENCE) {
      console.log(`[RelativeMetrics] Low confidence mapping for ${isin}: ${benchmarkMapping.confidenceScore}`);
      return false;
    }
    ...
    console.log(`[RelativeMetrics] Computed metrics for ${schemeCode}: alpha=${metrics.alpha}, beta=${metrics.beta}`);
    return true;
  }

  async recomputeAllMetrics(batchSize: number = 100): Promise<{ processed: number; success: number; failed: number }> {
    ...
    .where(sql`${mfBenchmarkMap.confidenceScore} >= ${MIN_BENCHMARK_CONFIDENCE}`)
    .limit(batchSize);
    ...
    for (const fund of fundsWithBenchmark) {
      ...
      try {
        const result = await this.computeAndStoreMetrics(fund.schemeCode, fund.isin);
        processed++;
        if (result) success++;
        else failed++;
      } catch (error) {
        console.error(`[RelativeMetrics] Error processing ${fund.schemeCode}:`, error);
        failed++;
        processed++;
      }
    ...
    console.log(`[RelativeMetrics] Batch complete: processed=${processed}, success=${success}, failed=${failed}`);
