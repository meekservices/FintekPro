...
class PMLAAuditService {
	 ...
	 */
	async logAuditEvent(
		event: Omit<PMLAAuditEvent, "eventId" | "checksum" | "retentionExpiry">,
	): Promise<string> {
		const eventId = `PMLA-${Date.now()}-${nanoid(8)}`;
		const retentionExpiry = new Date();
		retentionExpiry.setFullYear(
			retentionExpiry.getFullYear() + PMLA_THRESHOLDS.RETENTION_YEARS,
		);
		...
		try {
			...
			console.log(
				`[PMLA Audit] Event ${eventId} logged for user ${event.userId}`,
			);
			return eventId;
		} catch (error: any) {
	...
	async monitorTransaction(params: {
		...
		if (
			...
			const recentSimilar = await this.checkStructuringPattern(
				params.userId,
				params.amount,
			);
			if (recentSimilar) {
		...
		const countries = [params.sourceCountry, params.destinationCountry].filter(
			Boolean,
		) as string[];
	 ...
	 */
	private async checkStructuringPattern(
		userId: string,
		...
		const nearThresholdTxns = recentOrders.filter((o) => {
			const amount = Number.parseFloat(o.netAmount || "0");
			return (
				amount >= PMLA_THRESHOLDS.STRUCTURING_AMOUNT &&
				amount < PMLA_THRESHOLDS.CTR_THRESHOLD_INR
			);
		});

		// Total of near-threshold transactions
		const totalNearThreshold =
			nearThresholdTxns.reduce(
				(sum, o) => sum + Number.parseFloat(o.netAmount || "0"),
				0,
			) + currentAmount;

		// If combined amount exceeds threshold and there are multiple transactions
		return (
			nearThresholdTxns.length >= 2 &&
			totalNearThreshold >= PMLA_THRESHOLDS.CTR_THRESHOLD_INR
		);
	}
	...
	private async checkVelocity(
		userId: string,
	): Promise<{ exceeded: boolean; details: string }> {
		const cutoff = new Date();
		...
		if (txnCount >= PMLA_THRESHOLDS.VELOCITY_LIMIT_24H) {
			return {
				exceeded: true,
				details: `${txnCount} transactions in 24h (limit: ${PMLA_THRESHOLDS.VELOCITY_LIMIT_24H})`,
			};
		}
		...
		if (totalAmount >= PMLA_THRESHOLDS.VELOCITY_AMOUNT_24H) {
			return {
				exceeded: true,
				details: `INR ${totalAmount.toLocaleString()} in 24h (limit: ${PMLA_THRESHOLDS.VELOCITY_AMOUNT_24H.toLocaleString()})`,
			};
		}
	...
	async generateFIUReport(params: {
		...
		const totalAmount = transactions.reduce(
			(sum, t) => sum + Number.parseFloat(t.netAmount || "0"),
			0,
		);

		const riskScore =
			params.suspicionIndicators.length * 15 +
		...
		console.log(
			`[PMLA Audit] FIU Report ${reportId} generated for user ${params.userId}`,
		);
		return report;
	}
	...
	async getAuditHistory(
		...
		return results.map((r) => {
			...
			return {
				...
				riskLevel: (r.riskImpact || "low") as
					| "low"
					| "medium"
					| "high"
					| "critical",
				flagged:
					r.complianceImpact === "major" || r.complianceImpact === "critical",
