...
class MfLifecycleGlidePathService {
	...
	static getInstance(): MfLifecycleGlidePathService {
		if (!MfLifecycleGlidePathService.instance) {
			MfLifecycleGlidePathService.instance = new MfLifecycleGlidePathService();
		}
		return MfLifecycleGlidePathService.instance;
	}

	validateGlidePath(
		schemeCode: string,
		lifecycleMetadata: LifecycleMetadata,
	): GlidePathValidationResult {
		...
		if (!glidePathSteps || glidePathSteps.length < 2) {
			violations.push(
				"Life Cycle Fund must have at least 2 glide path steps defining the equity/debt allocation over time.",
			);
		...
		for (let i = 1; i < steps.length; i++) {
			if (steps[i].year - steps[i - 1].year > 1) {
				violations.push(
					`Missing year in glide path: gap between year ${steps[i - 1].year} and ${steps[i].year}. Steps must be consecutive.`,
				);
			}
		}
	...
	async validateAndPersist(
        ...
        `);
				console.log(
					`[LifecycleValidator] ${schemeCode}: INVALID — ${result.violations.length} violations`,
				);
			} else {
        ...
        `);
			...
			console.error(
				`[LifecycleValidator] DB update error for ${schemeCode}:`,
				e.message,
			);
		}
	...
	async validateAllLifecycleSchemes(): Promise<BulkLifecycleValidationSummary> {
		console.log(
			"[LifecycleValidator] Starting bulk lifecycle glide path validation...",
		);

		const lifecycleFunds = await db
			.select({
		...
		console.log(
			`[LifecycleValidator] Complete: ${lifecycleFunds.length} lifecycle funds, ${valid} valid, ${invalid} invalid`,
		);
	...
	isPublishingAllowed(complianceStatus: string | null): boolean {
		return (
			complianceStatus !== "GLIDE_PATH_INVALID" &&
...
export const mfLifecycleGlidePathService =
	MfLifecycleGlidePathService.getInstance();
