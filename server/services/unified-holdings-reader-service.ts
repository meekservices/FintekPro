...
class UnifiedHoldingsReaderService {
	...
	async getClientType(clientId: string): Promise<ClientType> {
		const [prospect] = await db
			.select({
				...
				kycStatus: (prospectClients as any).kycStatus,
			})
	...
	async getAlpacaHoldings(userId: string): Promise<UnifiedHolding[]> {
		try {
			...
			return positions.value.map((pos): UnifiedHolding => {
				const qty = Number.parseFloat(pos.qty ?? "0");
				const mktValueUsd = Number.parseFloat(pos.market_value ?? "0");
				const avgPriceUsd = Number.parseFloat(pos.avg_entry_price ?? "0");
				const plUsd = Number.parseFloat((pos as any).unrealized_pl ?? "0");
				const plPct = Number.parseFloat((pos as any).unrealized_plpc ?? "0");
	 ...
	 */
	private async getProspectHoldings(
		prospectId: string,
	 ...
	 */
	private async getRegisteredClientHoldings(
		userId: string,
	): Promise<UnifiedHolding[]> {
	 ...
	 */
	private async getLegacyPortfolioHoldings(
		userId: string,
	): Promise<UnifiedHolding[]> {
		...
		return holdings.map((h) => ({
			...
			averageCost: (h as any).avgCostPerUnit
				? Number((h as any).avgCostPerUnit)
				: undefined,
			currentPrice: (h as any).currentPrice
				? Number((h as any).currentPrice)
				: undefined,
			currentValue: Number((h as any).currentValue) || 0,
			investedValue:
				Number((h as any).investedAmount || (h as any).investedValue) ||
				undefined,
			unrealizedGain: (h as any).returns
				? Number((h as any).returns)
				: undefined,
			unrealizedGainPercent: (h as any).returnsPercent
				? Number((h as any).returnsPercent)
				: undefined,
	...
	private mapComprehensiveToUnified(h: ComprehensiveHolding): UnifiedHolding {
		return {
			...
			unrealizedGain: h.unrealizedGainLoss
				? Number(h.unrealizedGainLoss)
				: undefined,
			unrealizedGainPercent: h.unrealizedGainLossPercent
				? Number(h.unrealizedGainLossPercent)
				: undefined,
	 ...
	 */
	private normalizeToUnifiedFormat(
		h: any,
		source: "prospect" | "registered",
	): UnifiedHolding {
		return {
			id: h.id || nanoid(),
	...
	async getPortfolioSummary(clientId: string): Promise<PortfolioSummary> {
		...
		holdings.forEach((h) => {
			...
			if (["equity", "stock", "etf", "reit", "invit"].includes(type)) {
				...
				["debt", "bond", "ncd", "gsec", "fixed_deposit"].includes(type)
			) {
	...
	async clearAllHoldings(
		...
		if (clientType.isProspect && !clientType.isRegistered) {
			...
			console.log(
				`[UnifiedHoldingsReader] Cleared prospect holdings for ${clientId}: ${reason}`,
			);
			return { cleared: 1 };
		}
		...
		if (clientType.isRegistered) {
			...
			console.log(
				`[UnifiedHoldingsReader] Cleared ${deleted.length} holdings for registered client ${clientId}: ${reason}`,
			);
	...
	private normalizeAssetType(type: string | undefined): string {
		...
		if (["mutual_fund", "mf", "mutualfund"].includes(lower))
			return "mutual_fund";
		if (["equity", "stock", "stocks", "us_equity", "us equity"].includes(lower))
			return "equity";
