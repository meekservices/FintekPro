...
class ISINIntelligenceService {
	 ...
	 */
	verifyISINChecksum(isin: string): {
		valid: boolean;
		...
		const providedCheckDigit = Number.parseInt(upperISIN.charAt(11), 10);

		if (Number.isNaN(providedCheckDigit)) {
			return { valid: false, computedCheckDigit: -1, providedCheckDigit: -1 };
		}
		...
		let sum = 0;
		const digits = numericString.split("").map((d) => Number.parseInt(d, 10));
	...
	private getRegionFromISIN(isin: string): {
		region: "APAC" | "EMEA" | "Americas" | "Global" | null;
		country: string | null;
		exchange: string | null;
		marketType: "domestic" | "international" | "gift_city" | null;
	...
	async detectInstrument(
		...
		const prefix = isin.substring(0, 3).toUpperCase() as
			| "INF"
			| "INE"
			| "INS"
			| "INV"
		...
		switch (prefix) {
			case "INF":
				...
				break;
			case "INS":
				result = this.detectGovernmentSecurity(
					isin,
					metadata,
					validationErrors,
				);
				break;
			case "INV":
				result = this.detectSecuritisedInstrument(
					isin,
					metadata,
					validationErrors,
				);
				break;
			case "INX":
				result = this.detectEntitlementInstrument(
					isin,
					metadata,
					validationErrors,
				);
		...
		result.validationStatus =
			validationErrors.length === 0
				? "validated"
				: validationErrors.some((e) => e.includes("REJECT"))
					? "conflict"
					: "validated";
	...
	private detectMutualFundOrETF(
		isin: string,
		metadata: ISINMetadata | undefined,
		errors: string[],
	): ISINDetectionResult {
		if (metadata?.coupon) {
	...
	private detectGovernmentSecurity(
		isin: string,
		metadata: ISINMetadata | undefined,
		errors: string[],
	): ISINDetectionResult {
	...
	private detectSecuritisedInstrument(
		isin: string,
		metadata: ISINMetadata | undefined,
		errors: string[],
	): ISINDetectionResult {
		if (!metadata?.issuerName) {
			errors.push(
				"WARNING: INV prefix should have issuer name for securitised instruments",
			);
		}
	...
	private detectEntitlementInstrument(
		isin: string,
		metadata: ISINMetadata | undefined,
		errors: string[],
	): ISINDetectionResult {
		if (metadata?.maturityDate) {
			errors.push(
				"REJECT: INX prefix (entitlement) should have expiry_date, not maturity_date",
			);
		}
	...
	private deepResolveINE(
		...
		const hasCoupon =
			metadata?.coupon !== undefined && metadata.coupon !== null;
		...
		if (hasCoupon && hasEquityFlag && !isConvertible) {
			errors.push(
				"FLAG: INE has both coupon and equity_flag - may be convertible instrument",
			);
		}
		...
		if (
			(detectedIssuerType === "nbfc" || detectedIssuerType === "corporate") &&
			hasCoupon &&
			hasMaturity
		) {
			return this.createNCDResult(
				isin,
				detectedIssuerType,
				metadata?.isSecured,
				errors,
	...
	private createConvertibleResult(
		isin: string,
		errors: string[],
	): ISINDetectionResult {
		return {
	...
	async validateISIN(
		isin: string,
		metadata: ISINMetadata,
	): Promise<{ valid: boolean; errors: string[]; checksumValid?: boolean }> {
		const errors: string[] = [];
		...
		if (!checksumResult.valid) {
			errors.push(
				`Invalid ISIN checksum: expected ${checksumResult.computedCheckDigit}, got ${checksumResult.providedCheckDigit}`,
			);
		}
		...
		if (countryCode === "IN") {
			switch (prefix) {
				case "INF":
					...
					if (metadata.maturityDate)
						errors.push("INF prefix should not have maturity date");
					break;
				case "INS":
					if (!metadata.maturityDate && !metadata.isGoldLinked)
						errors.push("INS prefix requires maturity date");
					if (metadata.issuerType && metadata.issuerType !== "government")
						errors.push("INS must have government issuer");
					break;
				case "INE":
					if (
						metadata.coupon &&
						metadata.hasEquityFlag &&
						!metadata.isConvertible
					) {
						errors.push(
							"INE has both coupon and equity flag without convertible marker",
						);
					}
					break;
				case "INV":
					if (!metadata.issuerName)
						errors.push("INV requires issuer identification");
					break;
				case "INX":
					if (metadata.maturityDate)
						errors.push("INX should have expiry date, not maturity date");
					break;
				default:
	...
	async upsertInstrument(
		isin: string,
		detection: ISINDetectionResult,
		metadata?: ISINMetadata,
	): Promise<InstrumentMaster | null> {
		try {
			...
			const instrumentData = {
				...
				validationNotes:
					detection.validationErrors.length > 0
						? detection.validationErrors.join("; ")
						: null,
	...
	private determineComplianceRegime(detection: ISINDetectionResult): string {
		...
		if (
			detection.primaryRegulator === "RBI" &&
			detection.secondaryRegulator === "SEBI"
		)
			return "dual";
	...
	getRegulatorInfo(detection: ISINDetectionResult): {
		primary: string;
