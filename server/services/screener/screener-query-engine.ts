import { db } from '../../db';
import { screenerStocks, screenerFinancials, screenerDerivedMetrics } from '@shared/schema';
import { eq, and, gte, lte, sql, desc, asc, like, or, ilike, inArray, isNotNull } from 'drizzle-orm';
...
export async function queryScreener(filters: ScreenerFilters): Promise<ScreenerResponse> {
  ...
  if (filters.sector) conditions.push(eq(screenerStocks.sector, filters.sector));
  if (filters.industry) conditions.push(eq(screenerStocks.industry, filters.industry));
  if (filters.marketCapCategory) conditions.push(eq(screenerStocks.marketCapCategory, filters.marketCapCategory));
  if (filters.exchange) conditions.push(eq(screenerStocks.exchange, filters.exchange));
  ...
  if (filters.minPE != null) financialConditions.push(gte(screenerFinancials.peRatio, filters.minPE.toString()));
  if (filters.maxPE != null) financialConditions.push(lte(screenerFinancials.peRatio, filters.maxPE.toString()));
  if (filters.minPB != null) financialConditions.push(gte(screenerFinancials.pbRatio, filters.minPB.toString()));
  if (filters.maxPB != null) financialConditions.push(lte(screenerFinancials.pbRatio, filters.maxPB.toString()));
  if (filters.minROE != null) financialConditions.push(gte(screenerFinancials.roe, filters.minROE.toString()));
  if (filters.maxROE != null) financialConditions.push(lte(screenerFinancials.roe, filters.maxROE.toString()));
  if (filters.minDebtToEquity != null) financialConditions.push(gte(screenerFinancials.debtToEquity, filters.minDebtToEquity.toString()));
  if (filters.maxDebtToEquity != null) financialConditions.push(lte(screenerFinancials.debtToEquity, filters.maxDebtToEquity.toString()));
  if (filters.minDividendYield != null) financialConditions.push(gte(screenerFinancials.dividendYield, filters.minDividendYield.toString()));
  if (filters.maxDividendYield != null) financialConditions.push(lte(screenerFinancials.dividendYield, filters.maxDividendYield.toString()));

  const derivedConditions: any[] = [];
  if (filters.minCompositeScore != null) derivedConditions.push(gte(screenerDerivedMetrics.compositeScore, filters.minCompositeScore.toString()));
  if (filters.maxCompositeScore != null) derivedConditions.push(lte(screenerDerivedMetrics.compositeScore, filters.maxCompositeScore.toString()));
  if (filters.minFintekRating != null) derivedConditions.push(gte(screenerDerivedMetrics.fintekRating, filters.minFintekRating));
  ...
  const [sectors, industries, marketCaps] = await Promise.all([
    db.selectDistinct({ value: screenerStocks.sector })
      .from(screenerStocks)
      .where(and(eq(screenerStocks.isActive, true), isNotNull(screenerStocks.sector))),
    db.selectDistinct({ value: screenerStocks.industry })
      .from(screenerStocks)
      .where(and(eq(screenerStocks.isActive, true), isNotNull(screenerStocks.industry))),
    db.selectDistinct({ value: screenerStocks.marketCapCategory })
      .from(screenerStocks)
      .where(and(eq(screenerStocks.isActive, true), isNotNull(screenerStocks.marketCapCategory))),
  ]);
...
export async function getStockDetail(symbol: string) {
  ...
  const [financials, derived] = await Promise.all([
    db.select().from(screenerFinancials).where(eq(screenerFinancials.symbol, symbol)).orderBy(desc(screenerFinancials.fiscalYear)).limit(5),
    db.select().from(screenerDerivedMetrics).where(eq(screenerDerivedMetrics.symbol, symbol)).limit(1),
  ]);
...
export async function getScreenerStats() {
  const [stockCount] = await db.select({ count: sql<number>`count(*)` }).from(screenerStocks).where(eq(screenerStocks.isActive, true));
  const [financialCount] = await db.select({ count: sql<number>`count(*)` }).from(screenerFinancials);
  const [derivedCount] = await db.select({ count: sql<number>`count(*)` }).from(screenerDerivedMetrics);
