...
class AlphaVantageProvider implements IDataProvider {
  ...
  private async fetchAV<T>(params: Record<string, string>): Promise<T | null> {
    ...
    try {
      ...
      if (!response.ok) {
        avThrottledWarn(`http_${response.status}`, `[AlphaVantage] HTTP ${response.status}`);
        return null;
      }
  ...
  async getRatios(symbol: string): Promise<FinancialRatios | null> {
    ...
    return {
      ...
      netProfitMargin: parseFloat(data.ProfitMargin) || undefined,
      ...
      dividendPayout: parseFloat(data.PayoutRatio) || undefined,
      revenueGrowth: parseFloat(data.QuarterlyRevenueGrowthYOY) || undefined,
      earningsGrowth: parseFloat(data.QuarterlyEarningsGrowthYOY) || undefined,
    };
  }
  ...
  async getIncomeStatement(symbol: string, period = 'annual'): Promise<FinancialStatement[]> {
    ...
    const reports = period === 'quarter' ? data.quarterlyReports : data.annualReports;
    ...
    return reports.slice(0, 5).map((r: any) => ({
      ...
      revenue: parseFloat(r.totalRevenue) || undefined,
      netIncome: parseFloat(r.netIncome) || undefined,
      grossProfit: parseFloat(r.grossProfit) || undefined,
      operatingIncome: parseFloat(r.operatingIncome) || undefined,
    }));
  }
  ...
  async getBalanceSheet(symbol: string, period = 'annual'): Promise<FinancialStatement[]> {
    ...
    return reports.slice(0, 5).map((r: any) => ({
      ...
      date: r.fiscalDateEnding,
      period: period === 'quarter' ? 'Q' : 'FY',
      totalDebt: (parseFloat(r.longTermDebt) || 0) + (parseFloat(r.shortTermDebt) || 0),
      totalEquity: parseFloat(r.totalShareholderEquity) || undefined,
      totalAssets: parseFloat(r.totalAssets) || undefined,
    }));
  }
  ...
  async getCashFlow(symbol: string, period = 'annual'): Promise<FinancialStatement[]> {
    ...
    return reports.slice(0, 5).map((r: any) => ({
      ...
      date: r.fiscalDateEnding,
      period: period === 'quarter' ? 'Q' : 'FY',
      operatingCashFlow: parseFloat(r.operatingCashflow) || undefined,
      freeCashFlow: (parseFloat(r.operatingCashflow) || 0) - Math.abs(parseFloat(r.capitalExpenditures) || 0) || undefined,
      capitalExpenditure: parseFloat(r.capitalExpenditures) || undefined,
    }));
  }
  ...
  async getStockScreener(_marketCapMin?: number, _exchange?: string, _limit?: number): Promise<StockScreenerResult[]> {
    return [];
  }
