...
function makeResult(task: string, processed = 0, errors = 0, skipped = 0, apiCallsUsed = 0, remaining = 0): EnrichmentResult {
...
export async function enrichTier1(budget: number): Promise<TierResult> {
  ...
  const ratiosBudget = Math.floor(budget * 0.30);
  const growthBudget = Math.floor(budget * 0.20);
  const keyMetricsBudget = Math.floor(budget * 0.20);
  const dcfBudget = Math.floor(budget * 0.15);
  const ratingBudget = budget - ratiosBudget - growthBudget - keyMetricsBudget - dcfBudget;
  ...
  if (await canContinue()) {
    ...
    for (const stock of rows) {
      ...
      } catch (err: any) { errors++; console.error(`[Tier1:Growth] ${stock.symbol}: ${err.message}`); }
    }
  }
  tasks.growth = makeResult('financial_growth', processed, errors, skipped, apiCalls);
  totalApiCalls += apiCalls;

  processed = 0; errors = 0; skipped = 0; apiCalls = 0;
  if (await canContinue()) {
    const stocks = await db.execute(sql`
      SELECT ss.symbol, ss.fmp_symbol FROM screener_stocks ss
    ...
    for (const stock of rows) {
      ...
      } catch (err: any) { errors++; console.error(`[Tier1:KeyMetrics] ${stock.symbol}: ${err.message}`); }
    }
  }
  tasks.keyMetrics = makeResult('key_metrics', processed, errors, skipped, apiCalls);
  totalApiCalls += apiCalls;
  ...
  if (await canContinue()) {
    ...
    for (const stock of rows) {
      ...
      } catch (err: any) { errors++; console.error(`[Tier1:DCF] ${stock.symbol}: ${err.message}`); }
    }
  }
  tasks.dcf = makeResult('dcf_valuations', processed, errors, skipped, apiCalls);
  totalApiCalls += apiCalls;
  ...
  if (await canContinue()) {
    ...
    for (const stock of rows) {
      ...
      } catch (err: any) { errors++; console.error(`[Tier1:Rating] ${stock.symbol}: ${err.message}`); }
    }
  }
  tasks.ratings = makeResult('company_ratings', processed, errors, skipped, apiCalls);
  totalApiCalls += apiCalls;
...
export async function enrichTier2(budget: number): Promise<TierResult> {
  ...
  const gradesBudget = Math.floor(budget * 0.20);
  const earningsBudget = Math.floor(budget * 0.15);
  const dividendBudget = Math.floor(budget * 0.10);
  const splitBudget = Math.floor(budget * 0.05);
  const ipoBudget = Math.floor(budget * 0.10);
  const econBudget = budget - targetsBudget - gradesBudget - earningsBudget - dividendBudget - splitBudget - ipoBudget;
  ...
  tasks.analystTargets = makeResult('analyst_targets', processed, errors, skipped, apiCalls);
  totalApiCalls += apiCalls;
  ...
  tasks.analystGrades = makeResult('analyst_grades', processed, errors, skipped, apiCalls);
  totalApiCalls += apiCalls;
  ...
  tasks.earningsCalendar = makeResult('earnings_calendar', processed, errors, skipped, apiCalls);
  totalApiCalls += apiCalls;
  ...
  tasks.dividendCalendar = makeResult('dividend_calendar', processed, errors, skipped, apiCalls);
  totalApiCalls += apiCalls;
  ...
  tasks.splitCalendar = makeResult('split_calendar', processed, errors, skipped, apiCalls);
  totalApiCalls += apiCalls;
  ...
  tasks.ipoCalendar = makeResult('ipo_calendar', processed, errors, skipped, apiCalls);
  totalApiCalls += apiCalls;
  ...
  tasks.economicCalendar = makeResult('economic_calendar', processed, errors, skipped, apiCalls);
  totalApiCalls += apiCalls;
...
export async function enrichTier3(budget: number): Promise<TierResult> {
  ...
  const insiderBudget = Math.floor(budget * 0.20);
  const newsBudget = Math.floor(budget * 0.20);
  const sectorBudget = Math.max(1, Math.floor(budget * 0.05));
  const technicalBudget = budget - institutionalBudget - insiderBudget - newsBudget - sectorBudget;
  ...
  tasks.institutionalHolders = makeResult('institutional_holders', processed, errors, skipped, apiCalls);
  totalApiCalls += apiCalls;
  ...
  tasks.insiderTrades = makeResult('insider_trades', processed, errors, skipped, apiCalls);
  totalApiCalls += apiCalls;
  ...
  tasks.stockNews = makeResult('stock_news', processed, errors, skipped, apiCalls);
  totalApiCalls += apiCalls;
  ...
  if (await canContinue()) {
    try {
      ...
      apiCalls++;
      const today = new Date().toISOString().split('T')[0];
      await db.execute(sql`DELETE FROM screener_sector_performance WHERE date = ${today}`);
      for (const s of data) {
        await db.execute(sql`
  ...
  tasks.sectorPerformance = makeResult('sector_performance', processed, errors, skipped, apiCalls);
  totalApiCalls += apiCalls;
  ...
  if (await canContinue()) {
    ...
    for (const stock of rows) {
      ...
      try {
        const fmpSymbol = stock.fmp_symbol || `${stock.symbol}.NS`;
        const rsiData = await provider.getTechnicalIndicator(fmpSymbol, 'daily', 'rsi', 14);
        apiCalls++;
        if (rsiData.length > 0) {
  ...
  tasks.technicalIndicators = makeResult('technical_indicators', processed, errors, skipped, apiCalls);
  totalApiCalls += apiCalls;
...
export async function enrichTier4(budget: number): Promise<TierResult> {
  ...
  const pricesBudget = Math.floor(budget * 0.50);
  ...
  tasks.marketData = makeResult('market_data', processed, errors, skipped, apiCalls);
  totalApiCalls += apiCalls;
...
export async function runPriorityEnrichmentBatch(
  ...
  const allocation: TierBudget = {
    ...
    tier4: availableCalls - Math.floor(availableCalls * budgetSplit.tier1)
      - Math.floor(availableCalls * budgetSplit.tier2) - Math.floor(availableCalls * budgetSplit.tier3),
  };

  console.log(`[PriorityScheduler] Starting with ${availableCalls} calls. T1=${allocation.tier1} T2=${allocation.tier2} T3=${allocation.tier3} T4=${allocation.tier4}`);
  ...
  if (await canContinue()) {
    ...
    tiers.push(t2);
    console.log(`[PriorityScheduler] Tier2 done: ${t2.totalApiCalls} calls used`);
  }

  if (await canContinue()) {
    const t3 = await enrichTier3(allocation.tier3);
    tiers.push(t3);
    console.log(`[PriorityScheduler] Tier3 done: ${t3.totalApiCalls} calls used`);
  }

  if (await canContinue()) {
    const t4 = await enrichTier4(allocation.tier4);
    tiers.push(t4);
    console.log(`[PriorityScheduler] Tier4 done: ${t4.totalApiCalls} calls used`);
  }

  const totalApiCalls = tiers.reduce((sum, t) => sum + t.totalApiCalls, 0);
  const finalStats = await fmpUsageMonitor.getDailyStats();

  console.log(`[PriorityScheduler] Complete: ${totalApiCalls} total API calls. Remaining: ${finalStats.remaining}`);
...
export async function getExtendedEnrichmentProgress(): Promise<{
  ...
  const tier1Metrics = [withRatios, withReturns, withGrowth, withKeyMetrics, withDCF, withRatings];
  const tier1Percent = Math.round((tier1Metrics.reduce((a, b) => a + b, 0) / (total * tier1Metrics.length)) * 100);

  const tier2Percent = Math.round(((withAnalystTargets + withAnalystGrades) / (total * 2)) * 100);
  const tier3Percent = Math.round(((withInstitutional + withInsider + withNews + withTechnicals) / (total * 4)) * 100);
  const tier4Percent = Math.round((withReturns / total) * 100);

  const overallPercent = Math.round((tier1Percent * 0.4 + tier2Percent * 0.3 + tier3Percent * 0.2 + tier4Percent * 0.1));
  const totalMissing = (total * 6 - tier1Metrics.reduce((a, b) => a + b, 0)) +
...
export async function enrichSingleTier(tierNumber: 1 | 2 | 3 | 4, budget = 50): Promise<TierResult> {
  switch (tierNumber) {
    case 1: return enrichTier1(budget);
    case 2: return enrichTier2(budget);
    case 3: return enrichTier3(budget);
    case 4: return enrichTier4(budget);
    default: throw new Error(`Invalid tier: ${tierNumber}`);
  }
}
