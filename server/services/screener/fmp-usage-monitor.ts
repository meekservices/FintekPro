...
class FmpUsageMonitor {
	...
	async getUsageCount(): Promise<number> {
		...
		try {
			const [record] = await db
				...
				.from(fmpUsageLog)
				.where(
					and(eq(fmpUsageLog.date, today), eq(fmpUsageLog.provider, "fmp")),
				)
				.limit(1);
	...
	async incrementUsage(
		endpoint?: string,
	): Promise<{ count: number; remaining: number; alertLevel: string | null }> {
		const today = this.getToday();
		this.memoryCount++;
		this.currentDate = today;
			...
			if (existing) {
				...
				await db
					...
					.set({
						...
						callDetails: [
							...currentDetails.slice(-50),
							{ endpoint, time: new Date().toISOString() },
						],
					})
		...
		if (alertLevel === "WARNING_80PCT") {
			console.warn(
				`[FmpUsage] ⚠️ 80% daily limit reached (${this.memoryCount}/${DAILY_LIMIT})`,
			);
		} else if (alertLevel === "LIMIT_REACHED") {
			console.warn(
				`[FmpUsage] 🛑 Auto-stop threshold reached (${this.memoryCount}/${AUTO_STOP_THRESHOLD}). No more API calls today.`,
			);
		}
	...
	async getDailyStats(): Promise<{
		...
		return {
			...
			alertLevel:
				count >= AUTO_STOP_THRESHOLD
					? "LIMIT_REACHED"
					: count >= DAILY_LIMIT * ALERT_THRESHOLD
						? "WARNING_80PCT"
						: null,
		};
	}
