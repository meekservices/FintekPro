import type { IDataProvider, CompanyProfile, FinancialRatios, FinancialStatement, HistoricalPrice, StockScreenerResult } from './data-provider';
...
type ProviderMethod = 'getCompanyProfile' | 'getRatios' | 'getIncomeStatement' | 'getBalanceSheet' | 'getCashFlow' | 'getHistoricalPrices' | 'getStockScreener' | 'getQuote';
...
class DataProviderRegistry {
  ...
  register(provider: IDataProvider, priority: number = 100): void {
    ...
    console.log(`[ProviderRegistry] Registered: ${provider.name} (priority: ${priority})`);
  }
  ...
  async getCompanyProfile(symbol: string): Promise<{ result: CompanyProfile | null; provider: string }> {
    return this.executeWithFallback<CompanyProfile | null>('getCompanyProfile', [symbol], null);
  }

  async getRatios(symbol: string): Promise<{ result: FinancialRatios | null; provider: string }> {
    return this.executeWithFallback<FinancialRatios | null>('getRatios', [symbol], null);
  }

  async getIncomeStatement(symbol: string, period?: string): Promise<{ result: FinancialStatement[]; provider: string }> {
    return this.executeWithFallback<FinancialStatement[]>('getIncomeStatement', [symbol, period], []);
  }

  async getBalanceSheet(symbol: string, period?: string): Promise<{ result: FinancialStatement[]; provider: string }> {
    return this.executeWithFallback<FinancialStatement[]>('getBalanceSheet', [symbol, period], []);
  }

  async getCashFlow(symbol: string, period?: string): Promise<{ result: FinancialStatement[]; provider: string }> {
    return this.executeWithFallback<FinancialStatement[]>('getCashFlow', [symbol, period], []);
  }

  async getHistoricalPrices(symbol: string, from?: string, to?: string): Promise<{ result: HistoricalPrice[]; provider: string }> {
    return this.executeWithFallback<HistoricalPrice[]>('getHistoricalPrices', [symbol, from, to], []);
  }

  async getStockScreener(marketCapMin?: number, exchange?: string, limit?: number): Promise<{ result: StockScreenerResult[]; provider: string }> {
    return this.executeWithFallback<StockScreenerResult[]>('getStockScreener', [marketCapMin, exchange, limit], []);
  }

  async getQuote(symbol: string): Promise<{ result: { price: number; change: number; changePercent: number; volume: number } | null; provider: string }> {
    return this.executeWithFallback<{ price: number; change: number; changePercent: number; volume: number } | null>('getQuote', [symbol], null);
  }
  ...
  getStats(): RegistryStats {
    ...
    for (const entry of ordered) {
      ...
      providers.push({
        ...
        totalCalls: total,
        successRate: total > 0 ? Math.round((entry.metrics.successCount / total) * 100) : 100,
        avgLatencyMs: total > 0 ? Math.round(entry.metrics.latencySum / entry.metrics.successCount) || 0 : 0,
...
export function getProviderRegistry(): DataProviderRegistry {
  if (!registryInstance) {
    ...
    console.log('✅ [ProviderRegistry] Initialized with FMP (primary) + AlphaVantage (fallback)');
  }
