...
class FMPFreeProvider implements IDataProvider {
	...
	private async fetchWithRateLimit<T>(
		...
		try {
			...
			if (
				...
				typeof data === "object" &&
				!Array.isArray(data) &&
				data["Error Message"]?.includes("Legacy Endpoint")
			) {
				return null;
			}
			return data as T;
	...
	async getRatios(symbol: string): Promise<FinancialRatios | null> {
		const data = await this.fetchWithRateLimit<any[]>(`/ratios/${symbol}`, {
			limit: "1",
		});
		if (!data || !data[0]) return null;
	...
	async getInsiderTrading(symbol: string, limit = 50): Promise<any[]> {
		const data = await this.fetchWithRateLimit<any[]>("/insider-trading", {
			symbol,
			limit: String(limit),
		});
		return data || [];
	...
	async getMutualFundHolders(symbol: string): Promise<any[]> {
		const data = await this.fetchWithRateLimit<any[]>(
			`/mutual-fund-holder/${symbol}`,
		);
		return data || [];
	}

	async getStockNews(symbol: string, limit = 20): Promise<any[]> {
		const data = await this.fetchWithRateLimit<any[]>("/stock_news", {
			tickers: symbol,
			limit: String(limit),
		});
		return data || [];
	...
	async getGeneralNews(limit = 20): Promise<any[]> {
		const data = await this.fetchWithRateLimit<any[]>("/general_news", {
			limit: String(limit),
		});
		return data || [];
	}
	...
	async searchSymbol(query: string, limit = 10): Promise<any[]> {
		const data = await this.fetchWithRateLimit<any[]>("/search", {
			query,
			limit: String(limit),
		});
		return data || [];
	...
	async getCompanyOutlook(symbol: string): Promise<any | null> {
		const data = await this.fetchWithRateLimit<any>(`/company-outlook`, {
			symbol,
		});
		return data || null;
	}
	...
	async getSharesFloat(symbol: string): Promise<any[]> {
		const data = await this.fetchWithRateLimit<any[]>(`/shares_float`, {
			symbol,
		});
		return data || [];
	}
