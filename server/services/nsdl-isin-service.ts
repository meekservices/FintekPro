...
class NSDLISINService {
	...
	private getSecurityType(
		description: string,
		cfi: string,
	): "equity" | "debt" | "preference" | "warrant" | "other" {
		const descUpper = description.toUpperCase();
		const cfiUpper = cfi.toUpperCase();

		if (
			descUpper.includes(" EQ") ||
			descUpper.endsWith(" EQ") ||
			cfiUpper.startsWith("ES")
		) {
			return "equity";
		}
		if (
			descUpper.includes("PREF") ||
			cfiUpper.startsWith("EP") ||
			cfiUpper.startsWith("EF")
		) {
			return "preference";
		}
		...
		if (
			descUpper.includes("NCD") ||
			descUpper.includes("BD") ||
			descUpper.includes("LOA") ||
			descUpper.includes("DEB") ||
			cfiUpper.startsWith("DB") ||
			cfiUpper.startsWith("DC") ||
			cfiUpper.startsWith("DY")
		) {
			return "debt";
		}
	...
	private calculateMatchScore(searchTerm: string, issuerName: string): number {
		...
		for (const searchWord of searchWords) {
			if (
				nameWords.some(
					(nameWord) =>
						nameWord === searchWord || nameWord.startsWith(searchWord),
				)
			) {
				matchedWords++;
			}
	...
	async searchByCompanyName(
		...
		for (const record of data) {
			...
			if (matchScore > 40) {
				const recordSecurityType = this.getSecurityType(
	...
	async getISINByExactName(
		companyName: string,
		securityType: "equity" | "all" = "equity",
	): Promise<string | null> {
	...
	determineInstrumentType(
		...
		if (
			desc.includes("GOVT") ||
			desc.includes("GOI") ||
			issuer.includes("GOVERNMENT OF INDIA")
		) {
			if (
		...
		if (
			issuer.includes("STATE") &&
			(desc.includes("SDL") || desc.includes("STATE DEV"))
		) {
			return "sdl";
		}
	...
	async lookupCINByISIN(isin: string): Promise<{
		...
		try {
			...
			console.log(
				`[NSDL ISIN] NSDL lookup failed, continuing with other sources`,
			);
		}
		...
		if (issuerName) {
			...
			try {
				const credhiveResults =
					await credhiveService.searchCompanyByNameOrCIN(issuerName);
	 ...
	 */
	async batchLookupCINByISIN(isins: string[]): Promise<
		Map<
			string,
			{
				cin: string;
	...
	> {
		...
		console.log(
			`[NSDL ISIN] Batch complete: ${found}/${isins.length} CINs found`,
		);
