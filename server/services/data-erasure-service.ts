...
class DataErasureService {
	...
	async eraseUserData(
		...
		logger.info("[DataErasure] Starting erasure for user", {
			userId,
			requestedBy,
		});
		...
		await db
			.insert(schema.platformAuditLogs)
			.values({
				userId,
		...
		logger.info("[DataErasure] Erasure completed", {
			userId,
			anonymisedTables,
			deletedTables,
		});
	...
	async exportUserData(userId: string): Promise<Record<string, any>> {
		...
		const sanitisedProfile = profile
			? {
					...
					panNumber: profile.panNumber
						? `${profile.panNumber.slice(0, 4)}*****${profile.panNumber.slice(-1)}`
						: null,
		...
		return {
			...
			bankAccounts: bankAccounts.map((a) => ({
				...a,
				bankAccountNumber: `****${a.bankAccountNumber?.slice(-4)}`,
			})),
			portfolios,
	...
	private async checkOpenPositions(
		userId: string,
	): Promise<{ hasOpenPositions: boolean; description: string }> {
		try {
			...
			if (count > 0) {
				return {
					...
					description: `${count} active portfolio holding(s)`,
				};
			}
		} catch {
			/* non-fatal */
	...
	private async anonymiseKycRecords(
		userId: string,
		tables: string[],
		notes: string[],
	): Promise<void> {
		try {
			...
			notes.push(
				`Warning: could not fully anonymise user_profiles: ${err instanceof Error ? err.message : err}`,
			);
		}
	}

	private async anonymiseAuditTrails(
		userId: string,
		retained: string[],
		notes: string[],
	): Promise<void> {
		...
		notes.push(
			"Audit trails retained for 5 years per PMLA §12 (PII anonymised in-place).",
		);
	}

	private async deletePortfolioData(
		userId: string,
		tables: string[],
	): Promise<void> {
		try {
			...
			if (portfolioIds.length > 0) {
				await db
					...
					.where(inArray(schema.portfolioHoldings.portfolioId, portfolioIds));
	...
	private async deletePreferencesAndNotifications(
		userId: string,
		tables: string[],
	): Promise<void> {
		try {
	...
	private async deleteConsentAndDocuments(
		userId: string,
		tables: string[],
	): Promise<void> {
		try {
			await db
				.delete(schema.aadhaarConsentArtifacts)
				.where(eq(schema.aadhaarConsentArtifacts.userId, userId));
	...
	private async deleteFinancialProfiles(
		userId: string,
		tables: string[],
	): Promise<void> {
		try {
	...
	private async anonymiseUserRecord(
		userId: string,
		tables: string[],
	): Promise<void> {
		try {
