...
export class OverlapIntelligenceEngine {
	...
	private calculateStockExposures(
		...
		const fundWeightMap = new Map(
			funds.map((f) => [f.mfIsin, f.portfolioWeight]),
		);
		...
		for (const holding of holdings) {
			...
			const holdingPct = Number.parseFloat(holding.holdingPercentage) || 0;
	...
	private calculateSectorExposures(
		stockExposures: StockExposure[],
	): SectorExposure[] {
	...
	async calculateOverlapRiskScore(
		...
		score: number;
		action: "EXCLUDE" | "RANK_LOWER" | "ALLOWED";
		details: string;
	}> {
		if (!existingPortfolio.length) {
			return {
				score: 0,
				action: "ALLOWED",
				details: "Empty portfolio - no overlap risk",
			};
		}
		...
		if (!candidateHoldings.length) {
			return {
				score: 0,
				action: "ALLOWED",
				details: "No holdings data for candidate fund",
			};
		}

		// Get existing portfolio holdings
		const existingIsins = existingPortfolio
			.map((f) => f.mfIsin)
			.filter(Boolean);
		...
		for (const holding of existingHoldings) {
			...
			if (!fund) continue;
			const contribution =
				(fund.portfolioWeight * Number.parseFloat(holding.holdingPercentage)) /
				100;
		...
		for (const candidateHolding of candidateHoldings) {
			const candidateWeight =
				Number.parseFloat(candidateHolding.holdingPercentage) || 0;
			const existingExposure =
				existingExposures.get(candidateHolding.stockSymbol) || 0;
	 ...
	 */
	async detectReplacementCandidates(
		funds: PortfolioFund[],
	): Promise<ReplaceFundSuggestion[]> {
		...
		for (let i = 0; i < funds.length; i++) {
			for (let j = i + 1; j < funds.length; j++) {
				...
				const intersection = new Set(
					[...holdingsA].filter((x) => holdingsB.has(x)),
				);
	...
	private determineFundToReplace(
		fundA: PortfolioFund,
		fundB: PortfolioFund,
	): PortfolioFund {
	...
	private generateMetricsComparison(
		...
		if (replace.sharpeRatio && keep.sharpeRatio) {
			parts.push(
				`Sharpe: ${replace.sharpeRatio.toFixed(2)} vs ${keep.sharpeRatio.toFixed(2)}`,
			);
		}
	...
	async findAlternatives(
		...
		for (const candidate of candidates) {
			...
			if (overlapCheck.action === "ALLOWED") {
				...
				const currentScore =
					await this.calculateDiversificationScore(existingPortfolio);
				const newPortfolio = [
					...portfolioWithoutTarget,
					{
						...fundToReplace,
						mfIsin: candidate.isin,
						name: candidate.schemeName || "",
					},
				];
				const projectedScore =
					await this.calculateDiversificationScore(newPortfolio);
				...
				alternatives.push({
					...
					expenseRatio: candidate.expenseRatio
						? Number.parseFloat(candidate.expenseRatio)
						: undefined,
				});
			}
		...
		return alternatives.sort(
			(a, b) => b.diversificationGain - a.diversificationGain,
		);
	}
	...
	async simulateDiversificationImpact(
		...
		const currentScore =
			await this.calculateDiversificationScore(currentPortfolio);
		...
		for (const change of changes) {
			switch (change.action) {
				...
				case "REMOVE": {
					const removedFund = newPortfolio.find(
						(f) => f.mfIsin === change.fundIsin,
					);
				...
				case "REPLACE":
					if (change.replacementIsin) {
						const idx = newPortfolio.findIndex(
							(f) => f.mfIsin === change.fundIsin,
						);
						if (idx >= 0) {
							...
							if (replacementFund.length) {
								...
								changesApplied.push(
									`Replaced ${oldName} with ${replacementFund[0].schemeName}`,
								);
							}
						}
					}
		...
		// Normalize weights
		const totalWeight = newPortfolio.reduce(
		...
		if (diversificationScore.score < 60) {
			talkingPoints.push({
				...
				data: {
					...
					grade: diversificationScore.grade,
				},
			});
		}
	...
	async analyzePortfolioIntelligence(
		...
		const cacheKey = funds
			.map((f) => `${f.mfIsin}:${f.portfolioWeight}`)
			.sort()
			.join("|");
		...
		const diversificationScore =
			await this.calculateDiversificationScore(funds);
		...
		for (const suggestion of replaceSuggestions) {
			...
			if (fundToReplace) {
				suggestion.alternatives = await this.findAlternatives(
					fundToReplace,
					funds,
					3,
				);
	...
	async optimizeSIPAllocation(
		...
		if (!candidateFunds.length || sipAmount <= 0) {
			return {
				sipRouting: [],
				explanation: "No valid candidates for SIP allocation",
				totalAllocated: 0,
			};
		}
		...
		for (const fundIsin of candidateFunds) {
			...
			const overlapRisk = await this.calculateOverlapRiskScore(
				fundIsin,
				existingPortfolio,
			);
		...
		if (!fundScores.length) {
			return {
				sipRouting: [],
				explanation: "No fund details found",
				totalAllocated: 0,
			};
		}
		...
		const totalInverseOverlap = fundScores.reduce(
			(sum, f) => sum + (100 - f.overlapScore),
			0,
		);
	...
	async calculateGoalBasedDiversificationScore(
		...
		const goalWeights = {
			WEALTH_CREATION: {
				stockOverlapMultiplier: 1.0,
				sectorPenaltyMultiplier: 0.7,
			},
			RETIREMENT: { stockOverlapMultiplier: 1.5, sectorPenaltyMultiplier: 1.5 },
			CHILD_EDUCATION: {
				stockOverlapMultiplier: 1.3,
				sectorPenaltyMultiplier: 1.2,
			},
		...
		for (const penalty of baseResult.penalties) {
			...
			if (penalty.type === "STOCK_OVERLAP") {
				adjustedImpact = Math.round(
					penalty.impact * weights.stockOverlapMultiplier,
				);
			} else if (penalty.type === "SECTOR_CONCENTRATION") {
				adjustedImpact = Math.round(
					penalty.impact * weights.sectorPenaltyMultiplier,
				);
			}
	 ...
	 */
	generateSEBICompliantNarratives(context: {
		type:
			| "OVERLAP_RISK"
			| "SIP_ROUTING"
			| "REPLACE_FUND"
		...
		const templates: Record<
			string,
			{ text: string; disclaimer: string; id: string }
		> = {
			OVERLAP_RISK: {
		...
		if (context.type === "OVERLAP_RISK" && context.data.stockCount) {
			...
			context.type === "DIVERSIFICATION_SCORE" &&
			context.data.score !== undefined
		) {
	...
	generateAllSEBINarratives(
		...
		const overlappingStocks = diversificationScore.stockExposures.filter(
			(s) => s.fundCount >= 2,
		);
		narratives.push({
			type: "OVERLAP_RISK",
...
export const overlapIntelligenceEngine =
	OverlapIntelligenceEngine.getInstance();
