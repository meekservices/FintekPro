...
class AIUnlistedRecommendationService {
	constructor() {
		...
		console.log(
			`✅ AI Unlisted Stock Recommendation Service initialized via Unified Engine (primary: ${status.primary})`,
		);
	}
...
Return JSON array sorted by suitabilityScore (highest first).`;
	...
	private getDefaultRecommendations(
		...
		const scored = assets
			...
			.map((asset) => {
				...
				const roe = Number.parseFloat(asset.roe || "0");
				if (roe > 20) score += 15;
				else if (roe > 10) score += 10;
				else if (roe > 0) score += 5;

				const debtToEquity = Number.parseFloat(asset.debtToEquity || "0");
				if (debtToEquity < 0.5) score += 10;
				else if (debtToEquity < 1) score += 5;
				else if (debtToEquity > 2) score -= 10;

				const revenueGrowth = Number.parseFloat(asset.revenueGrowth || "0");
				...
				const riskLevel =
					debtToEquity > 1.5 || asset.complianceRiskScore! > 50
						? "high"
						: debtToEquity > 0.8 || !asset.pat
							? "moderate"
							: "low";

				const priceStr =
					asset.publishedBuyPrice || asset.publishedSellPrice || "0";
				const price = Number.parseFloat(priceStr) || 0;
				const safeRevenueGrowth = Number.isNaN(revenueGrowth)
					? 0
					: revenueGrowth;
				const targetPrice =
					price > 0 ? price * (1 + (safeRevenueGrowth / 100) * 0.5) : 0;
				...
				return {
					...
					aiSignal: (score >= 70 ? "buy" : score >= 40 ? "hold" : "avoid") as
						| "buy"
						| "hold"
						| "avoid",
	...
	private getKeyRisks(asset: UnlistedStockAsset): string[] {
		...
		if (!asset.pat || Number.parseFloat(asset.pat) <= 0) {
...
export const aiUnlistedRecommendationService =
	new AIUnlistedRecommendationService();
