...
function detectOption(schemeName: string): string {
	...
	if (
		nameLower.includes("idcw payout") ||
		nameLower.includes("dividend payout") ||
		nameLower.includes("payout")
	)
		return "idcw-payout";
	if (
		nameLower.includes("idcw reinvestment") ||
		nameLower.includes("dividend reinvestment") ||
		nameLower.includes("reinvestment")
	)
...
function categorizeScheme(
	...
	if (
		...
		nameLower.includes("government securities")
	) {
		return "Gilt";
	}
	...
	if (
		nameLower.includes("aggressive hybrid") ||
		nameLower.includes("equity hybrid")
	) {
		return "Aggressive Hybrid";
	}
	if (
		nameLower.includes("conservative hybrid") ||
		nameLower.includes("debt hybrid")
	) {
	...
	if (
		...
		nameLower.includes("us equity") ||
		nameLower.includes("emerging market")
	) {
	...
	for (const line of lines) {
		...
		if (!trimmedLine.includes(";")) {
			if (
				...
				!trimmedLine.startsWith("Scheme") &&
				!trimmedLine.startsWith("Open Ended")
			) {
		...
		if (!schemeCode || !schemeName || !nav || nav === "N.A." || nav === "N/A")
			continue;

		const navNum = Number.parseFloat(nav);
		if (Number.isNaN(navNum)) continue;
...
function determineRiskLevel(category: string | null): string {
	...
	if (
		["liquid", "overnight", "ultra short duration", "money market"].some((c) =>
	...
	if (
		[
			...
			"corporate bond",
			"floater",
		].some((c) => categoryLower.includes(c))
	) {
...
export async function importAmfiData(): Promise<ImportResult> {
	...
	try {
		...
		if (!response.ok) {
			throw new Error(
				`Failed to fetch AMFI data: ${response.status} ${response.statusText}`,
			);
		}
		...
		for (let i = 0; i < schemes.length; i += batchSize) {
			...
			for (const scheme of batch) {
				try {
					...
					if (!existingAmcs.has(scheme.fundHouse.toLowerCase())) {
						try {
							...
							if (!amcError.message?.includes("duplicate")) {
								console.error(
									`[AMFI Import] Error creating AMC ${scheme.fundHouse}:`,
									amcError,
								);
							}
						}
					...
					if (schemeError.message?.includes("duplicate")) {
						...
						if (errors.length <= 10) {
							console.error(
								`[AMFI Import] Error importing scheme ${scheme.schemeCode}:`,
								schemeError,
							);
						}
					}
			...
			if (i % 500 === 0) {
				console.log(
					`[AMFI Import] Progress: ${i + batch.length}/${schemes.length} schemes processed`,
				);
			}
		}
		...
		console.log(
			`[AMFI Import] Completed in ${duration}ms: ${importedSchemes} new, ${updatedSchemes} updated, ${skippedSchemes} skipped, ${newAmcs} new AMCs`,
		);
...
async function syncAmcSchemeCounts(): Promise<void> {
	try {
		...
		const counts = new Map<
			string,
			{ total: number; regular: number; direct: number }
		>();
