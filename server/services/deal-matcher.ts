...
export class DealMatcherService {
	 ...
	 */
	async findMatchesForSellListing(
		sellListingId: string,
	): Promise<MatchResult[]> {
		...
		const buyRequests = await this.storage.getBuyRequestsByCompany(
			listing.companyId,
		);
	...
	async findMatchesForBuyRequest(buyRequestId: string): Promise<MatchResult[]> {
		...
		const sellListings = await this.storage.getSellListingsByCompany(
			buyRequest.companyId,
		);
	...
	async evaluateMatch(
		...
		if (!this.isKycEligible(seller) || !this.isKycEligible(buyer)) {
			return {
				matched: false,
				reason:
					"Both parties must have Enhanced or Accredited Investor KYC tier",
				sellListing: listing,
				buyRequest,
				matchScore: 0,
			};
		...
		if (buyMaxPrice >= sellLandingPrice) {
			...
			if (buyMaxPrice >= sellAskPrice) {
				...
				const priceProximity =
					(buyMaxPrice - sellLandingPrice) / (sellAskPrice - sellLandingPrice);
			...
			const priceProximity =
				(buyMaxPrice - sellFloorPrice) / (sellLandingPrice - sellFloorPrice);
			matchScore += Math.floor(priceProximity * 15);
		}

		// Quantity compatibility
		const minQuantity = Math.max(
			listing.minimumLotSize || 1,
			buyRequest.preferredLotSize || 1,
		);
		...
		return {
			...
			reason: matched
				? reasons.join("; ")
				: "Match score too low for automatic execution",
			sellListing: listing,
			buyRequest,
			matchScore,
		};
	...
	async createDealFromMatch(
		...
		const hasBlockingFlags = await complianceService.hasBlockingFlags(
			sellListing.companyId,
		);
		if (hasBlockingFlags) {
			throw new Error(
				"Deal creation blocked: Company has high-risk compliance flags. Contact admin for review.",
			);
		}
		...
		return await this.storage.withTransaction(async (tx) => {
			...
			await tx.insert(schema.unlistedPriceHistory).values({
				companyId: sellListing.companyId,
				date: new Date(),
			...
			if (!buyerPmlaResult.allowed) {
				console.error(
					`[PMLA] Buyer transaction blocked for deal ${deal.id}: ${buyerPmlaResult.flags.join(", ")}`,
				);
				throw new Error(
					`Transaction blocked by AML compliance: Buyer failed PMLA checks. Risk score: ${buyerPmlaResult.riskScore}`,
				);
			}

			if (!sellerPmlaResult.allowed) {
				console.error(
					`[PMLA] Seller transaction blocked for deal ${deal.id}: ${sellerPmlaResult.flags.join(", ")}`,
				);
				throw new Error(
					`Transaction blocked by AML compliance: Seller failed PMLA checks. Risk score: ${sellerPmlaResult.riskScore}`,
				);
			}

			// Log if transaction requires FIU reporting
			if (
				buyerPmlaResult.requiresFIUReport ||
				sellerPmlaResult.requiresFIUReport
			) {
				console.log(
					`[PMLA] Deal ${deal.id} flagged for FIU reporting. Buyer report: ${buyerPmlaResult.reportType || "none"}, Seller report: ${sellerPmlaResult.reportType || "none"}`,
				);
			}
			...
			await auditLogArchivalService.archiveUnlistedMarketplaceEvent({
				...
				riskLevel:
					Math.max(buyerPmlaResult.riskScore, sellerPmlaResult.riskScore) >= 60
						? "high"
						: "low",
			});
			...
			if (match.matched && match.matchScore && match.matchScore >= 80) {
				...
				try {
					...
					const updatedListing =
						await this.storage.getSellListingById(sellListingId);
			...
			if (match.matched && match.matchScore && match.matchScore >= 80) {
				...
				try {
					...
					const updatedRequest =
						await this.storage.getBuyRequestById(buyRequestId);
		...
		for (const company of allCompanies) {
			...
			for (const listing of listings) {
				if (
					...
					listing.validUntil &&
					new Date(listing.validUntil) < now
				) {
					await this.storage.updateSellListing(listing.id, {
						status: "expired",
					});
					expiredListings++;
				}
			...
			for (const request of requests) {
				if (
					...
					await this.storage.updateBuyRequest(request.id, {
						status: "expired",
					});
					expiredRequests++;
				}
