...
export class PriceSuggestionService {
	...
	async calculateSuggestedPrice(companyId: string): Promise<PriceSuggestion> {
		...
		const [financials, ratios, dealHistory, sellListings, priceHistory] =
			await Promise.all([
		...
		const valuationFactors = this.calculateValuationFactors(
			latestRatios,
			dealHistory,
			sellListings,
		);
		...
		if (latestFinancials && latestRatios) {
			...
			const dataSource = (latestFinancials.dataSource || "")
				.toLowerCase()
				.trim();
			const sourceLabel = dataSource.includes("credhive")
				? " (MCA via CredHive)"
				: dataSource.includes("mca")
					? " (MCA Registry)"
					: dataSource
						? ` (${latestFinancials.dataSource})`
						: "";
			const confidenceScore = Number.parseFloat(
				String(latestFinancials.confidenceScore || "0"),
			);
			const confidenceNote = confidenceScore >= 0.9 ? " [High Confidence]" : "";

			rationale.push(
				`Fundamental valuation: ₹${fundamentalValue.toLocaleString("en-IN")} based on P/E ${peRatio?.toFixed(2)}, P/B ${pbRatio?.toFixed(2)}, ROE ${roe?.toFixed(2)}%${sourceLabel}${confidenceNote}`,
			);
		}
		...
		if (dealHistory.length > 0) {
			...
			rationale.push(
				`Recent deals average: ₹${dealHistoryValue.toLocaleString("en-IN")} from ${dealHistory.length} transactions`,
			);
		}
		...
		if (sellListings.length > 0) {
			...
			rationale.push(
				`Market listings average: ₹${marketValue.toLocaleString("en-IN")} from ${sellListings.length} active sellers`,
			);
		}
		...
		if (priceHistory.length > 0) {
			...
			rationale.push(
				`Price history indicates: ₹${sellerFeedValue.toLocaleString("en-IN")} from recent data points`,
			);
		}

		// Weighted average calculation
		let { suggestedPrice, minPrice, maxPrice, confidence, methodology } =
			this.calculateWeightedPrice(factors, valuationFactors);

		// Apply risk adjustment discount if CredHive data shows high risk
		const riskDiscount = this.calculateRiskDiscount(
			latestFinancials,
			latestRatios,
		);
		if (riskDiscount > 0) {
			...
			rationale.push(
				`Risk discount applied: ${(riskDiscount * 100).toFixed(0)}% due to high Debt/Equity or negative Networth`,
			);
		}
	...
	private calculateRiskDiscount(
		...
		const RISK_DISCOUNT_RATE = 0.1; // 10% discount
	...
	private calculateFundamentalValue(
		...
		return Math.round(
			(peBasedValue * 0.6 + pbBasedValue * 0.4) * roeMultiplier,
		);
	}
	...
	private calculateDealHistoryValue(deals: UnlistedDeal[]): number {
		...
		const weightedSum = deals.reduce(
			(sum: number, deal: UnlistedDeal, index: number) => {
		...
		const totalWeight = deals.reduce(
			(sum: number, deal: UnlistedDeal, index: number) => {
				const dealTime = new Date(deal.createdAt || new Date()).getTime();
				const daysSinceDeal = (now - dealTime) / (1000 * 60 * 60 * 24);
	...
	private calculateMarketValue(listings: SellListing[]): number {
		...
		if (landingPrices.length === 0) {
			...
			const askPrices = listings
				...
				.filter((p: number) => p > 0);
			return askPrices.length > 0
				? Math.round(
						askPrices.reduce((a: number, b: number) => a + b) /
							askPrices.length,
					)
				: 0;
		}
	 ...
	 */
	private calculateSellerFeedValue(
		...
		const weightedSum = recentPrices.reduce(
			(sum: number, record: UnlistedPriceHistory, index: number) => {
		...
		const totalWeight = recentPrices.reduce(
			(sum: number, _: UnlistedPriceHistory, index: number) => {
	...
	private calculateValuationFactors(
		...
		const recentDeals = deals.filter((d: UnlistedDeal) => {
			const daysSince =
				(Date.now() - new Date(d.createdAt || new Date()).getTime()) /
				(1000 * 60 * 60 * 24);
			return daysSince <= 90;
		});
		...
		const activeListings = listings.filter(
			(l: SellListing) => new Date(l.validUntil || new Date()) > new Date(),
		);
		marketSentiment = Math.min(1, activeListings.length / 3); // Max at 3 active listings

		const confidence =
			(fundamentalScore + liquidityScore + marketSentiment) / 3;
	...
	private calculateWeightedPrice(
		...
		const RECENT_DEAL_WEIGHT = 0.3; // Deal history
		const MARKET_FEED_WEIGHT = 0.2; // Price history (sellerFeedValue)
		...
		const coefficientOfVariation =
			priceValues.length > 0 ? Math.sqrt(priceVariance) / suggestedPrice : 1;
		...
		if (
			...
			valuationFactors.confidence > 0.6 &&
			...
			valuationFactors.confidence > 0.4 &&
	...
	private async getRecentDeals(companyId: string): Promise<UnlistedDeal[]> {
		...
		return allDeals
			.filter(
				(d: UnlistedDeal) =>
					new Date(d.createdAt || new Date()) >= sixMonthsAgo,
			)
			.sort(
	 ...
	 */
	private async getActiveSellListings(
		companyId: string,
	): Promise<SellListing[]> {
		...
		return allListings.filter((listing: SellListing) => {
			return (
				...
				new Date(listing.validUntil || new Date()) > now
			);
		});
	}
	 ...
	 */
	async calculateBatchSuggestions(
		companyIds: string[],
		...
		const suggestions = await Promise.all(
			companyIds.map(async (companyId: string) => {
				try {
					...
					console.error(
						`Failed to calculate price for company ${companyId}:`,
						error,
					);
					return null;
				}
		...
		return suggestions.filter(
			(s: PriceSuggestion | null): s is PriceSuggestion => s !== null,
		);
	}
}
