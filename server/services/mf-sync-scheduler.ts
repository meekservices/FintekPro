...
class MFSyncScheduler {
	...
	start(): void {
		...
		setTimeout(
			...
			3 * 60 * 1000,
		); // Wait 3 minutes after server starts
	...
	private scheduleNextAMFISync(): void {
		...
		console.log(
			`[MF Sync] Next AMFI sync scheduled in ${Math.round(msUntilNext / 1000 / 60)} minutes`,
		);
	...
	async runAMFIMasterSync(): Promise<{
		updated: number;
		added: number;
		errors: number;
	}> {
			...
			for (let i = 0; i < funds.length; i += batchSize) {
				...
				try {
					...
					if (errors <= batchSize * 2) {
						console.warn(
							`[MF Sync] Batch upsert error (batch ${Math.floor(i / batchSize) + 1}):`,
							err.message,
						);
					}
				}
	...
	async runNAVRefresh(
		...
		try {
			...
			const nullVerifiedFunds = await db
			...
			const staleFunds =
				remainingSlots > 0
					? await db
							...
							.from(mutualFunds)
							.where(
								sql`${mutualFunds.lastVerifiedAt} < ${staleThreshold} AND ${mutualFunds.lastVerifiedAt} IS NOT NULL`,
							)
			...
			console.log(
				`[MF Sync] Processing ${nullVerifiedFunds.length} unverified + ${staleFunds.length} stale funds`,
			);
			...
			console.log(
				`[MF Sync] NAV refresh complete: ${updated} updated, ${errors} errors`,
			);
		} catch (error) {
	...
	async runStartupCatchUp(): Promise<{ updated: number; errors: number }> {
		...
		const [countResult] = await db
			...
			.from(mutualFunds)
			.where(
				sql`${mutualFunds.lastVerifiedAt} IS NULL OR ${mutualFunds.lastVerifiedAt} < ${staleThreshold24h}`,
			);

		const staleFundCount = Number(countResult?.count || 0);
		console.log(
			`[MF Sync] Found ${staleFundCount} funds needing refresh (>24h stale or unverified)`,
		);
		...
		for (let i = 0; i < maxIterations; i++) {
			...
			const result = await this.runNAVRefreshBatch(
				batchSize,
				staleThreshold24h,
			);
			...
			console.log(
				`[MF Sync] Batch ${i + 1}: ${result.updated} updated, ${remainingCount - result.updated} remaining`,
			);
			...
			if (result.updated === 0 && result.errors > 0) {
				console.log(
					`[MF Sync] Stopping catch-up: no progress made in last batch`,
				);
				break;
			}
		}
		...
		console.log(
			`[MF Sync] Startup catch-up complete: ${totalUpdated} updated, ${totalErrors} errors`,
		);
		return { updated: totalUpdated, errors: totalErrors };
	}

	private async runNAVRefreshBatch(
		batchSize: number,
		staleThreshold: Date,
	): Promise<{ updated: number; errors: number }> {
		let updated = 0;
	...
	private async markStaleSchemes(): Promise<void> {
		...
		try {
			const result = await db
				...
				.where(
					sql`${mutualFunds.lastVerifiedAt} < ${staleThreshold} AND ${mutualFunds.schemeStatus} = 'active'`,
				);
	...
	private parseAMFIData(data: string): AMFIFund[] {
		...
		for (const line of lines) {
			...
			if (Number.isNaN(Number.parseFloat(nav?.trim()))) continue;
	...
	private async fetchNAVFromMFAPI(
		schemeCode: string,
	): Promise<{ nav: string; navDate: string } | null> {
		try {
			const response = await axios.get(
	...
	async getStatus(): Promise<{
		...
		const [staleResult] = await db
			...
			.from(mutualFunds)
			.where(
				sql`${mutualFunds.lastVerifiedAt} < ${staleThreshold} OR ${mutualFunds.lastVerifiedAt} IS NULL`,
			);

		const [lastSyncResult] = await db
			.select({ lastSync: sql<Date>`MAX(${mutualFunds.lastVerifiedAt})` })
			.from(mutualFunds);
	...
	async refreshFundBySchemeCode(
		...
		try {
			...
			console.log(
				`[MF Sync] Fund ${schemeCode} refreshed: NAV=${navData.nav}, Date=${navData.navDate}`,
			);
