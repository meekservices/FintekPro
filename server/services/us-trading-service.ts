...
class UsTradingService {
  ...
  async getFeatureFlags(): Promise<FeatureFlagStatus> {
    try {
      ...
      flags.forEach(f => { flagMap[f.flagName] = f.isEnabled; });
  ...
  async initializeFeatureFlags(): Promise<void> {
    ...
    for (const flag of defaultFlags) {
      try {
        await db.insert(usFeatureFlags)
          .values(flag)
          .onConflictDoNothing();
      } catch (error) {
  ...
  async setFeatureFlag(flagName: string, isEnabled: boolean, updatedBy?: string): Promise<boolean> {
    try {
      await db.update(usFeatureFlags)
  ...
  async createBrokerAccount(data: InsertUsBrokerAccount): Promise<UsBrokerAccount> {
    const fy = this.getCurrentFinancialYear();
    const [account] = await db.insert(usBrokerAccounts)
      .values({ ...data, lrsFinancialYear: fy })
      .returning();
    return account;
  }

  async updateBrokerAccount(clientId: string, updates: Partial<UsBrokerAccount>): Promise<UsBrokerAccount | null> {
    const [updated] = await db.update(usBrokerAccounts)
  ...
  async createOrder(data: InsertUsOrder): Promise<UsOrder> {
    const [order] = await db.insert(usOrders)
      .values(data)
      .returning();
    return order;
  }

  async updateOrderStatus(orderId: string, status: string, updates?: Partial<UsOrder>): Promise<UsOrder | null> {
    const [updated] = await db.update(usOrders)
  ...
  async upsertHolding(clientId: string, symbol: string, data: Partial<InsertUsHolding>): Promise<UsHolding> {
    const existing = await db.select()
      .from(usHoldings)
      .where(and(
        eq(usHoldings.clientId, clientId),
        eq(usHoldings.symbol, symbol)
      ))
      .limit(1);

    if (existing[0]) {
      const [updated] = await db.update(usHoldings)
        .set({ ...data, updatedAt: new Date() })
        .where(eq(usHoldings.id, existing[0].id))
        .returning();
      return updated;
    } else {
      const [created] = await db.insert(usHoldings)
  ...
  async recordConsent(data: InsertUsConsent): Promise<UsConsent> {
    const [consent] = await db.insert(usConsents)
      .values(data)
      .returning();
    return consent;
  }
  ...
  async getConsentsForOrder(orderId: string): Promise<UsConsent[]> {
    return db.select()
      .from(usConsents)
      .where(eq(usConsents.orderId, orderId));
  }

  async getLrsUsage(clientId: string, financialYear?: string): Promise<{ used: number; remaining: number; declarations: UsLrsDeclaration[] }> {
    ...
    const used = declarations.reduce((sum, d) => sum + parseFloat(d.amountUsd), 0);
  ...
  async recordLrsDeclaration(data: InsertUsLrsDeclaration): Promise<UsLrsDeclaration> {
    ...
    const [declaration] = await db.insert(usLrsDeclarations)
      .values({ ...data, declarationHash })
      .returning();

    await db.update(usBrokerAccounts)
      .set({ 
  ...
  async addToWatchlist(data: InsertUsWatchlist): Promise<UsWatchlist> {
    const [item] = await db.insert(usWatchlist)
      .values(data)
      .returning();
    return item;
  }

  async removeFromWatchlist(clientId: string, symbol: string): Promise<boolean> {
    const result = await db.delete(usWatchlist)
      .where(and(
        eq(usWatchlist.clientId, clientId),
        eq(usWatchlist.symbol, symbol)
      ));
    return true;
  }
}
