...
class CompanyDataRefreshScheduler {
	...
	start(): void {
		...
		console.log(
			`✅ Company Data Refresh Scheduler started (refresh every ${this.config.refreshIntervalDays} days, checking daily)`,
		);
	...
	private async checkAndRefreshIfDue(): Promise<void> {
		const refreshIntervalMs =
			this.config.refreshIntervalDays * 24 * 60 * 60 * 1000;
		const now = Date.now();

		if (
			this.metrics.lastRunAt &&
			now - this.metrics.lastRunAt < refreshIntervalMs
		) {
			const daysRemaining = Math.ceil(
				(refreshIntervalMs - (now - this.metrics.lastRunAt)) /
					(24 * 60 * 60 * 1000),
			);
			console.log(
				`[CompanyRefresh] Next refresh in ${daysRemaining} days, skipping`,
			);
			return;
		}
	...
	async runRefreshCycle(): Promise<{ refreshed: number; errors: number }> {
		...
		try {
			...
			console.log(
				`[CompanyRefresh] Found ${staleCompanies.length} stale companies`,
			);
			...
			for (let i = 0; i < staleCompanies.length; i += this.config.batchSize) {
				...
				try {
					const detailsResult =
						await credhiveService.batchGetCompanyDetails(cins);
					const successfulDetails = [...detailsResult.values()].filter(
						(v) => v !== null,
					).length;
					...
					const financialsResult =
						await credhiveService.batchGetCompanyFinancials(cins);
					const successfulFinancials = [...financialsResult.values()].filter(
						(v) => v !== null,
					).length;
			...
			console.log(
				`[CompanyRefresh] Cycle complete: ${refreshed} refreshed, ${errors} errors`,
			);
		} finally {
	 ...
	 */
	private async getStaleCompanies(): Promise<
		Array<{
			id: number;
			cin: string;
	...
	> {
		try {
			...
			if (!columnsExist) {
				console.log(
					"[CompanyRefresh] Required columns not found, using fallback query",
				);
				return this.getStaleCompaniesFallback();
			}

			const financialsAgeThreshold = new Date();
			financialsAgeThreshold.setDate(
				financialsAgeThreshold.getDate() - this.config.financialsMaxAgeDays,
			);

			const detailsAgeThreshold = new Date();
			detailsAgeThreshold.setDate(
				detailsAgeThreshold.getDate() - this.config.detailsMaxAgeDays,
			);
      ...
      `,
				[
					financialsAgeThreshold.toISOString(),
					detailsAgeThreshold.toISOString(),
				],
			);
			...
			console.error(
				"[CompanyRefresh] Error fetching stale companies:",
				error.message,
			);
			return [];
		}
	 ...
	 */
	private async getStaleCompaniesFallback(): Promise<
		Array<{
			id: number;
			cin: string;
			needsFinancials: boolean;
	...
	async refreshCompany(cin: string): Promise<boolean> {
		try {
			...
			console.error(
				`[CompanyRefresh] Manual refresh failed for ${cin}: ${error.message}`,
			);
			this.metrics.errors++;
			return false;
		}
