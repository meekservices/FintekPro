import { db } from '../db';
import { proposalVerdicts, investmentProposals, InsertProposalVerdict } from '@shared/schema';
...
export class ProposalVerdictNormalizer {
  ...
  static async assignVerdict(
    ...
    if (!this.validateVerdict(assignment.verdict)) {
      return { 
        success: false, 
    ...
    const changeAmount = assignment.targetValue && assignment.currentValue
      ? assignment.targetValue - assignment.currentValue
      : null;
    const changePercent = changeAmount && assignment.currentValue
    ...
    const values: Partial<InsertProposalVerdict> = {
      ...
      currentValue: assignment.currentValue ? String(assignment.currentValue) : null,
      targetValue: assignment.targetValue ? String(assignment.targetValue) : null,
      ...
      exitLoadPercent: assignment.exitLoadPercent ? String(assignment.exitLoadPercent) : null,
      capitalGainsType: assignment.capitalGainsType,
      estimatedTax: assignment.estimatedTax ? String(assignment.estimatedTax) : null,
      updatedAt: new Date()
    };
  ...
  static async bulkAssignVerdicts(
    ...
    for (const assignment of assignments) {
      const result = await this.assignVerdict(proposalId, assignment);
      if (result.success) {
        assigned++;
      } else {
  ...
  static async validateProposalVerdicts(proposalId: string): Promise<VerdictValidation> {
    ...
    const instrumentNames = recommendations.map((r: any) => r.instrumentName || r.productName);
    ...
    const invalidVerdicts = verdicts.filter(v => !this.validateVerdict(v.verdict!));
  ...
  static async getVerdictSummary(proposalId: string): Promise<VerdictSummary> {
    ...
    const sumTargetValue = (items: any[]) => 
      items.reduce((sum, v) => sum + parseFloat(v.targetValue || '0'), 0);

    const exitLoadImpact = sellVerdicts.reduce((sum, v) => {
      if (v.exitLoadApplicable) {
        const value = parseFloat(v.currentValue?.toString() || '0');
        const percent = parseFloat(v.exitLoadPercent?.toString() || '0');
    ...
    const taxImpact = sellVerdicts.reduce(
      (sum, v) => sum + parseFloat(v.estimatedTax?.toString() || '0'), 
      0
    );
    ...
    return {
      ...
      verdicts: verdicts.map(v => ({
        ...
        currentValue: parseFloat(v.currentValue?.toString() || '0'),
        targetValue: parseFloat(v.targetValue?.toString() || '0'),
        exitLoadApplicable: v.exitLoadApplicable || false,
        exitLoadPercent: parseFloat(v.exitLoadPercent?.toString() || '0'),
        capitalGainsType: v.capitalGainsType as 'STCG' | 'LTCG' | 'nil' | undefined,
        estimatedTax: parseFloat(v.estimatedTax?.toString() || '0')
      }))
    };
  }

  static async blockIfIncomplete(proposalId: string): Promise<{ blocked: boolean; reason?: string }> {
    const validation = await this.validateProposalVerdicts(proposalId);
