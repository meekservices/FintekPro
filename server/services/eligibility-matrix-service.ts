...
import { db } from "../db";
import {
	bankEligibilityRules,
	InsertBankEligibilityRules,
} from "@shared/schema";
...
class EligibilityMatrixService {
	...
	private evaluateRule(
		...
		if (rule.minCibilScore && applicant.cibilScore !== undefined) {
			if (applicant.cibilScore >= rule.minCibilScore) {
				matchedCriteria.push(
					`CIBIL Score ${applicant.cibilScore} >= ${rule.minCibilScore}`,
				);
				if (applicant.cibilScore >= 750) eligibilityScore += 20;
				else if (applicant.cibilScore >= 700) eligibilityScore += 10;
			} else {
				failedCriteria.push(
					`CIBIL Score ${applicant.cibilScore} < ${rule.minCibilScore} required`,
				);
				eligibilityScore -= 40;
			}
		}
		...
		if (rule.minMonthlyIncome && applicant.monthlyIncome !== undefined) {
			const minIncome = Number.parseFloat(rule.minMonthlyIncome);
			if (applicant.monthlyIncome >= minIncome) {
				matchedCriteria.push(
					`Monthly Income ₹${applicant.monthlyIncome.toLocaleString()} >= ₹${minIncome.toLocaleString()}`,
				);
				eligibilityScore += 10;
			} else {
				failedCriteria.push(
					`Monthly Income ₹${applicant.monthlyIncome.toLocaleString()} < ₹${minIncome.toLocaleString()} required`,
				);
				eligibilityScore -= 30;
			}
		}
		...
		if (
			rule.allowedEmploymentTypes &&
			rule.allowedEmploymentTypes.length > 0 &&
			applicant.employmentType
		) {
			if (rule.allowedEmploymentTypes.includes(applicant.employmentType)) {
				matchedCriteria.push(
					`Employment type '${applicant.employmentType}' is allowed`,
				);
			} else {
				failedCriteria.push(
					`Employment type '${applicant.employmentType}' not in allowed types: ${rule.allowedEmploymentTypes.join(", ")}`,
				);
				eligibilityScore -= 50;
			}
		}
		...
		if (
			rule.minBusinessVintageMonths &&
			applicant.businessVintageMonths !== undefined
		) {
			if (applicant.businessVintageMonths >= rule.minBusinessVintageMonths) {
				matchedCriteria.push(
					`Business vintage ${applicant.businessVintageMonths} months >= ${rule.minBusinessVintageMonths} required`,
				);
			} else {
				failedCriteria.push(
					`Business vintage ${applicant.businessVintageMonths} months < ${rule.minBusinessVintageMonths} required`,
		...
		if (rule.minAnnualTurnover && applicant.annualTurnover !== undefined) {
			const minTurnover = Number.parseFloat(rule.minAnnualTurnover);
			if (applicant.annualTurnover >= minTurnover) {
				matchedCriteria.push(
					`Annual Turnover ₹${(applicant.annualTurnover / 100000).toFixed(1)}L >= ₹${(minTurnover / 100000).toFixed(1)}L`,
				);
			} else {
				failedCriteria.push(
					`Annual Turnover ₹${(applicant.annualTurnover / 100000).toFixed(1)}L < ₹${(minTurnover / 100000).toFixed(1)}L required`,
				);
				eligibilityScore -= 30;
			}
		}
		...
		if (rule.minLoanAmount && applicant.requestedAmount !== undefined) {
			const minAmount = Number.parseFloat(rule.minLoanAmount);
			if (applicant.requestedAmount >= minAmount) {
				matchedCriteria.push(`Loan amount within range`);
			} else {
				failedCriteria.push(
					`Requested amount ₹${applicant.requestedAmount.toLocaleString()} < ₹${minAmount.toLocaleString()} minimum`,
				);
				eligibilityScore -= 20;
			}
		}
		...
		if (rule.maxLoanAmount && applicant.requestedAmount !== undefined) {
			const maxAmount = Number.parseFloat(rule.maxLoanAmount);
			if (applicant.requestedAmount <= maxAmount) {
				matchedCriteria.push(`Loan amount within maximum limit`);
			} else {
				failedCriteria.push(
					`Requested amount ₹${applicant.requestedAmount.toLocaleString()} > ₹${maxAmount.toLocaleString()} maximum`,
				);
				eligibilityScore -= 20;
			}
		}
		...
		if (
			rule.allowedPropertyTypes &&
			rule.allowedPropertyTypes.length > 0 &&
			applicant.propertyType
		) {
			if (rule.allowedPropertyTypes.includes(applicant.propertyType)) {
				matchedCriteria.push(
					`Property type '${applicant.propertyType}' is allowed`,
				);
			} else {
				failedCriteria.push(
					`Property type '${applicant.propertyType}' not in allowed types: ${rule.allowedPropertyTypes.join(", ")}`,
				);
				eligibilityScore -= 40;
			}
		}
		...
		return {
			...
			reasons: isEligible ? ["Meets all eligibility criteria"] : failedCriteria,
			matchedCriteria,
			failedCriteria,
		};
	}
	...
	async updateRule(
		id: string,
		updates: Partial<InsertBankEligibilityRules>,
	): Promise<any> {
	...
	async seedDefaultRules(): Promise<void> {
		...
		console.log(
			`[EligibilityMatrix] Seeded ${defaultRules.length} default eligibility rules`,
		);
	}
}
