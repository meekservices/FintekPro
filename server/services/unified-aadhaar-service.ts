...
class UnifiedAadhaarService {
  ...
  setActiveProvider(provider: AadhaarProvider, adminId: string): { success: boolean; message: string } {
    ...
    console.log(`[UnifiedAadhaar] Provider switched from ${previousProvider} to ${provider} by admin: ${adminId} at ${logEntry.timestamp.toISOString()}`);
  ...
  getProviderConfig(provider: AadhaarProvider): AadhaarProviderConfig | undefined {
    const config = this.providerConfigs.get(provider);
    if (config) {
      return {
        ...config,
  ...
  updateProviderPricing(provider: AadhaarProvider, pricePerVerification: number, adminId: string): { success: boolean; message: string } {
    const config = this.providerConfigs.get(provider);
    if (!config) {
      return { success: false, message: `Unknown provider: ${provider}` };
    }
    ...
    console.log(`[UnifiedAadhaar] Pricing updated for ${config.name}: ₹${pricePerVerification}/verification by admin: ${adminId}`);
  ...
  async generateOTP(aadhaarNumber: string): Promise<AadhaarOTPResponse> {
    ...
    try {
      ...
      console.error(`[UnifiedAadhaar] OTP generation error via ${provider}:`, error.message);
      return {
        success: false,
  ...
  async verifyOTP(refId: string, otp: string): Promise<AadhaarVerificationResponse> {
    ...
    try {
      ...
      console.error(`[UnifiedAadhaar] OTP verification error via ${provider}:`, error.message);
      return {
        success: false,
  ...
  async validateAadhaar(aadhaarNumber: string): Promise<AadhaarValidationResponse> {
    const provider = this.activeProvider;
    console.log(`[UnifiedAadhaar] Validating Aadhaar via provider: ${provider}`);
    ...
    try {
      ...
      console.error(`[UnifiedAadhaar] Aadhaar validation error via ${provider}:`, error.message);
      return {
        success: false,
  ...
  async checkPanAadhaarLinkage(pan: string, aadhaar: string): Promise<PanAadhaarLinkageResponse> {
    const provider = this.activeProvider;
    console.log(`[UnifiedAadhaar] Checking PAN-Aadhaar linkage via provider: ${provider}`);
    ...
    try {
      ...
      console.error(`[UnifiedAadhaar] PAN-Aadhaar linkage check error via ${provider}:`, error.message);
      return {
        success: false,
  ...
  getProviderUsageStats(): {
    ...
    const stats: Map<AadhaarProvider, { switchCount: number; lastActive: Date | null }> = new Map();
    ...
    return Array.from(this.providerConfigs.entries()).map(([provider, config]) => {
