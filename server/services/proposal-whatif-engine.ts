...
export class WhatIfSimulatorEngine {
	static async runSimulation(
		...
		const baseReturn = Number.parseFloat(
			proposal.expectedReturns?.toString() || "10",
		);
		...
		if (mode === "static") {
			scenarios = await WhatIfSimulatorEngine.generateStaticScenarios(
				proposalId,
				totalAmount,
				baseReturn,
				riskProfile,
			...
			const customScenario = await WhatIfSimulatorEngine.pythonProjection(
				"custom",
				totalAmount,
			...
			await WhatIfSimulatorEngine.saveScenario(
				proposalId,
				"interactive",
				"custom",
				assumptions,
		...
		const baseScenario =
			scenarios.find((s) => s.scenarioName === "base") || scenarios[0];
	...
	private static async generateStaticScenarios(
		...
		for (const scenario of STATIC_SCENARIOS) {
			...
			const projection = await WhatIfSimulatorEngine.pythonProjection(
				scenario.name,
				totalAmount,
				adjustedReturn,
			...
			await WhatIfSimulatorEngine.saveScenario(
				...
				scenario.name,
				{
					returnDelta: scenario.returnDelta,
					volatility: scenario.volatilityMult,
				},
				projection,
			);
		}
	...
	private static calculateProjection(
		...
		const projectedValue1Y = totalAmount * (1 + returnRate) ** 1;
		const projectedValue3Y = totalAmount * (1 + returnRate) ** 3;
		const projectedValue5Y = totalAmount * (1 + returnRate) ** 5;
		const projectedValue10Y = totalAmount * (1 + returnRate) ** 10;

		const maxDrawdown = Math.min(adjustedVolatility * 2.5, 0.6) * 100;
		const probabilityOfLoss =
			Math.max(0, 0.5 - (returnRate / adjustedVolatility) * 0.1) * 100;
	...
	private static async pythonProjection(
		...
		try {
			...
			if (pyResult?.summary && pyResult?.snapshots?.length) {
				...
				const finalCorpus = Math.round(
					pyResult.summary.finalCorpus ?? totalAmount,
				);
				...
				return {
					...
					projectedValue1Y:
						snapAt(12) ||
						Math.round(totalAmount * (1 + annualReturn / 100) ** 1),
					projectedValue3Y:
						snapAt(36) ||
						Math.round(totalAmount * (1 + annualReturn / 100) ** 3),
					projectedValue5Y:
						snapAt(60) ||
						Math.round(totalAmount * (1 + annualReturn / 100) ** 5),
					projectedValue10Y:
						finalCorpus ||
						Math.round(totalAmount * (1 + annualReturn / 100) ** 10),
					maxDrawdown: Math.round(Math.min(adjVol * 2.5, 0.6) * 10000) / 100,
					probabilityOfLoss: Math.min(
						100,
						Math.max(
							0,
							Math.round((0.5 - (xirr / adjVol) * 0.1) * 100 * 100) / 100,
						),
					),
		...
		return WhatIfSimulatorEngine.calculateProjection(
			scenarioName,
			totalAmount,
			annualReturn,
			volatilityMult,
	...
	static async getScenarios(proposalId: string): Promise<WhatIfProjection[]> {
		...
		return scenarios.map((s) => ({
			...
			projectedValue1Y: Number.parseFloat(
				s.projectedValue1Y?.toString() || "0",
			),
			projectedValue3Y: Number.parseFloat(
				s.projectedValue3Y?.toString() || "0",
			),
			projectedValue5Y: Number.parseFloat(
				s.projectedValue5Y?.toString() || "0",
			),
			projectedValue10Y: Number.parseFloat(
				s.projectedValue10Y?.toString() || "0",
			),
			maxDrawdown: Number.parseFloat(s.maxDrawdown?.toString() || "0"),
			probabilityOfLoss: Number.parseFloat(
				s.probabilityOfLoss?.toString() || "0",
			),
			valueAtRisk95: Number.parseFloat(s.valueAtRisk95?.toString() || "0"),
		}));
	}
	...
	static async getScenariosForReport(
		proposalId: string,
	): Promise<WhatIfProjection[]> {
		const scenarios = await db
			.select()
