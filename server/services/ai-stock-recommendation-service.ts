...
class AIStockRecommendationService {
	private recommendationCache = new Map<
		string,
		{ recommendations: StockRecommendation[]; timestamp: Date }
	>();
	...
	async getSmartRecommendations(
		...
		try {
			...
			eligibleStocks = eligibleStocks.filter(
				(s: any) => s.currentPrice && Number.parseFloat(s.currentPrice) > 0,
			);
			...
			if (eligibleStocks.length < 10) {
				...
				eligibleStocks = [
					...eligibleStocks,
					...defaultPool.slice(0, 10 - eligibleStocks.length),
				];
			}

			const stocksWithLiveData = await this.enhanceWithLiveData(eligibleStocks);
			const scoredStocks = stocksWithLiveData.map((stock) =>
				this.scoreStock(stock, riskLevel),
			);
			...
			const enhancedStocks = await Promise.all(
				stocks.map(async (stock) => {
					...
					const cachedFundamentals =
						this.FUNDAMENTALS_CACHE[stock.symbol] || {};

					const enrichedBase: any = {};
					if (ef) {
						enrichedBase.peRatio =
							ef.peRatio ?? stock.peRatio ?? cachedFundamentals.peRatio;
						enrichedBase.pbRatio =
							ef.pbRatio ?? stock.pbRatio ?? cachedFundamentals.pbRatio;
						enrichedBase.roe = ef.roe ?? stock.roe ?? cachedFundamentals.roe;
						enrichedBase.roic = ef.roic ?? cachedFundamentals.roic;
						enrichedBase.debtToEquity =
							ef.debtToEquity ??
							stock.debtToEquity ??
							cachedFundamentals.debtToEquity;
						enrichedBase.currentRatio =
							ef.currentRatio ??
							stock.currentRatio ??
							cachedFundamentals.currentRatio;
						enrichedBase.dividendYield =
							ef.dividendYield ??
							stock.dividendYield ??
							cachedFundamentals.dividendYield;
						enrichedBase.evToEbitda =
							ef.evToEbitda ?? cachedFundamentals.evToEbitda;
					} else {
	...
	private scoreStock(stockData: any, riskLevel: string): ScoredStock {
		...
		const enriched = stockData.enrichedSnapshot as
			| EnrichedStockSnapshot
			| undefined;

		const peRatio = Number.parseFloat(
			live.peRatio || stock.peRatio || cachedFundamentals.peRatio || 25,
		);
		const roe = Number.parseFloat(stock.roe || cachedFundamentals.roe || 15);
		const roce = Number.parseFloat(stock.roce || cachedFundamentals.roce || 12);
		const pbRatio = Number.parseFloat(
			live.pbRatio || stock.pbRatio || cachedFundamentals.pbRatio || 3,
		);
		const returns1Y = Number.parseFloat(
			stock.returns1Y || cachedFundamentals.returns1Y || 0,
		);
		const returns3M = Number.parseFloat(stock.returns3M || 0);
		...
		if (live.movingAvg50 && live.movingAvg200 && live.currentPrice) {
			if (
				live.currentPrice > live.movingAvg50 &&
				live.movingAvg50 > live.movingAvg200
			) {
				technicalScore = 95;
		...
		let fundamentalScore =
			(valuationScore + qualityScore + dividendYield * 8) / 2;
	...
	private calculateAdvancedMetrics(
		...
		try {
			const netIncome = Number.parseFloat(
				stock.netIncome || cachedFundamentals.netIncome || 0,
			);
			const totalAssets = Number.parseFloat(
				stock.totalAssets || cachedFundamentals.totalAssets || 1,
			);
			const operatingCashFlow = Number.parseFloat(
				stock.operatingCashFlow || cachedFundamentals.operatingCashFlow || 0,
			);
			const revenue = Number.parseFloat(
				stock.revenue || cachedFundamentals.revenue || 0,
			);
			const grossMargin = Number.parseFloat(
				stock.grossMargin || cachedFundamentals.grossMargin || 0,
			);
			const currentRatio = Number.parseFloat(
				stock.currentRatio || live.currentRatio || 1.5,
			);
			const debtToEquity = Number.parseFloat(
				stock.debtToEquity ||
					live.debtToEquity ||
					cachedFundamentals.debtToEquity ||
					0.5,
			...
			const longTermDebt = Number.parseFloat(
				stock.longTermDebt || cachedFundamentals.longTermDebt || 0,
			);
			const sharesOutstanding = Number.parseFloat(
				stock.sharesOutstanding || cachedFundamentals.sharesOutstanding || 1,
			);
			const assetTurnover = revenue / totalAssets;

			const prevNetIncome = Number.parseFloat(
				cachedFundamentals.prevNetIncome || netIncome * 0.9,
			);
			const prevTotalAssets = Number.parseFloat(
				cachedFundamentals.prevTotalAssets || totalAssets,
			);
			const prevLongTermDebt = Number.parseFloat(
				cachedFundamentals.prevLongTermDebt || longTermDebt,
			);
			const prevCurrentRatio = Number.parseFloat(
				cachedFundamentals.prevCurrentRatio || currentRatio,
			);
			const prevGrossMargin = Number.parseFloat(
				cachedFundamentals.prevGrossMargin || grossMargin,
			);
			const prevAssetTurnover = Number.parseFloat(
				cachedFundamentals.prevAssetTurnover || assetTurnover,
			);
			const prevSharesOutstanding = Number.parseFloat(
				cachedFundamentals.prevSharesOutstanding || sharesOutstanding,
			);
			...
			const workingCapital = Number.parseFloat(
				stock.workingCapital ||
					cachedFundamentals.workingCapital ||
					totalAssets * 0.2,
			);
			const retainedEarnings = Number.parseFloat(
				stock.retainedEarnings ||
					cachedFundamentals.retainedEarnings ||
					netIncome * 3,
			);
			const ebit = Number.parseFloat(
				stock.ebit || cachedFundamentals.ebit || netIncome * 1.3,
			);
			const marketCap = Number.parseFloat(
				live.marketCap || stock.marketCap || cachedFundamentals.marketCap || 0,
			);
			const totalLiabilities = Number.parseFloat(
				stock.totalLiabilities ||
					cachedFundamentals.totalLiabilities ||
					totalAssets * 0.4,
			);
			...
			const peRatio = Number.parseFloat(
				live.peRatio || stock.peRatio || cachedFundamentals.peRatio || 0,
			);
			const epsGrowth = Number.parseFloat(
				stock.epsGrowth || cachedFundamentals.epsGrowth || 15,
			);
			...
			const ebitda = Number.parseFloat(
				stock.ebitda || cachedFundamentals.ebitda || ebit * 1.15,
			);
			const enterpriseValue = Number.parseFloat(
				stock.enterpriseValue ||
					cachedFundamentals.enterpriseValue ||
					marketCap * 1.1,
			);
			...
			const enrichedSnap = stock.enrichedSnapshot as
				| EnrichedStockSnapshot
				| undefined;
			if (
				metrics.evToEbitda === undefined &&
				enrichedSnap?.fundamentals?.evToEbitda != null
			) {
				metrics.evToEbitda = enrichedSnap.fundamentals.evToEbitda;
			}

			const investedCapital = Number.parseFloat(
				stock.investedCapital ||
					cachedFundamentals.investedCapital ||
					totalAssets * 0.7,
			);
			const taxRate = Number.parseFloat(stock.taxRate || 0.25);
			...
			if (
				...
				enrichedSnap?.fundamentals?.roic != null
			) {
			...
			const batchRecommendations = await Promise.all(
				batch.map(async (scored) => {
					try {
						...
						return this.buildAIRecommendation(
							scored,
							aiAnalysis,
...
Provide analysis in JSON format:
{
  ...
  "fintekproRating": (1-5, where 5 is best)
	...
	private generateRuleBasedAnalysis(
		...
		const keyFactors = [];
		if (scored.fundamentalScore > 70)
			keyFactors.push("Strong fundamentals with healthy ROE/ROCE");
		if (scored.technicalScore > 70)
			keyFactors.push("Bullish technical setup above key moving averages");
		if (scored.momentumScore > 70)
			keyFactors.push("Strong price momentum with positive returns");
		if (scored.valuationScore > 70)
			keyFactors.push("Attractive valuations relative to peers");
		if (scored.qualityScore > 70)
			keyFactors.push("High-quality business with consistent earnings");
		...
		const riskFactors = [];
		if (scored.fundamentalScore < 50)
			riskFactors.push("Weak fundamentals require caution");
		if (scored.technicalScore < 40)
			riskFactors.push("Bearish technical indicators");
	...
	private buildAIRecommendation(
			...
			et?.macd != null
				? et.macd > 0
					...
					: et.macd < 0
						? "Bearish"
						: "Neutral"
				: scored.technicalScore > 60
					? "Bullish"
					: scored.technicalScore < 40
						? "Bearish"
						: "Neutral";
		const volumeTrend =
			live.volume && live.avgVolume
				? live.volume > live.avgVolume * 1.2
					? "High"
					: live.volume < live.avgVolume * 0.8
						? "Low"
						: "Normal"
				: "Normal";
		...
		return {
			...
			fintekproRating:
				aiAnalysis.fintekproRating || this.calculateFintekproRating(scored),
			...
			fundamentals: {
				peRatio: this.safeParseFloat(
					ef?.peRatio ??
						live.peRatio ??
						stock.peRatio ??
						cachedFundamentals.peRatio,
				),
				pbRatio: this.safeParseFloat(
					ef?.pbRatio ??
						live.pbRatio ??
						stock.pbRatio ??
						cachedFundamentals.pbRatio,
				),
				roe: this.safeParseFloat(
					ef?.roe ?? live.roe ?? stock.roe ?? cachedFundamentals.roe,
				),
				...
				debtToEquity: this.safeParseFloat(
					ef?.debtToEquity ??
						stock.debtToEquity ??
						cachedFundamentals.debtToEquity,
				),
				currentRatio: this.safeParseFloat(
					ef?.currentRatio ??
						stock.currentRatio ??
						cachedFundamentals.currentRatio,
				),
			...
			technicals: {
				...
				movingAvg50:
					et?.sma50 ??
					(Number.parseFloat(live.movingAvg50) || currentPrice * 0.95),
				movingAvg200:
					et?.sma200 ??
					(Number.parseFloat(live.movingAvg200) || currentPrice * 0.9),
				weekHigh52:
					Number.parseFloat(live.weekHigh52 || stock.weekHigh52) ||
					currentPrice * 1.2,
				weekLow52:
					Number.parseFloat(live.weekLow52 || stock.weekLow52) ||
					currentPrice * 0.7,
				volumeTrend,
			},
	...
	private buildRuleBasedRecommendation(
		...
		const analysis = this.generateRuleBasedAnalysis(scored, {
			timeHorizon,
			riskLevel,
		});
	...
	private calculateTaxImplications(
		timeHorizon: string,
	): StockRecommendation["taxImplications"] {
	...
	private getFallbackRecommendations(
		...
		return fallbackStocks.map((stock, idx) => ({
			...
			technicals: {
				rsi: 55,
				macd: "Neutral",
				movingAvg50: stock.currentPrice * 0.95,
				movingAvg200: stock.currentPrice * 0.9,
				weekHigh52: stock.currentPrice * 1.2,
			...
			taxImplications: this.calculateTaxImplications(
				filters.timeHorizon || "medium_term",
			),
			generatedAt: new Date(),
		}));
