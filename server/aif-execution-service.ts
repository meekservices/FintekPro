...
export const AIF_CATEGORIES = {
	CAT_I: {
		name: "Category I AIF",
		description:
			"Venture capital, SME funds, social venture funds, infrastructure funds",
...
class AIFExecutionService {
	...
	): Promise<AIFExecutionResult> {
		const errors: string[] = [];

		try {
			console.log(
				`[AIF Execution] Starting execution for order ${request.orderId}`,
			);

			// Step 1: Validate accredited investor status (MANDATORY for AIF)
			const investorValidation = await this.validateAccreditedInvestor(
				request.userId,
			);
			...
			console.log(
				`[AIF Execution] Accredited investor verified via ${investorValidation.qualificationRoute}`,
			);
			...
			console.log(
				`[AIF Execution] Payment validation: Total=₹${totalInvestment.toLocaleString()}, Paid=₹${paidAmount.toLocaleString()}, Balance=₹${balanceAmount.toLocaleString()}, Stage=${paymentStage}`,
			);

			// Step 3: Generate subscription agreement
			const agreement = await this.generateSubscriptionAgreement(request);
			console.log(
				`[AIF Execution] Subscription agreement generated: ${agreement.agreementId}`,
			);
			...
			await orderManagementService.updateOrderStatus({
				...
				metadata: {
					...
					balanceDueDate: isPartialPayment
						? new Date(Date.now() + 90 * 24 * 60 * 60 * 1000).toISOString()
						: null, // 90 days from now
				},
				actorId: "system",
				actorType: "system",
			});
			...
			const executionResult = await this.processAIFSubscription(
				request,
				agreement,
			);
			...
			if (executionResult.success) {
				...
				const agreementUrl = await this.storeSubscriptionAgreement(
					agreement,
					request,
				);
				...
				console.log(
					`[AIF Execution] Order ${request.orderId} executed successfully`,
				);
			...
			await this.handleExecutionFailure(
				request.orderId,
				executionResult.errors || ["Unknown execution error"],
			);
			return {
				success: false,
				orderId: request.orderId,
			...
			console.error(
				`[AIF Execution] Error executing order ${request.orderId}:`,
				error,
			);
			const errorMessage =
	 ...
	 */
	private async validateAccreditedInvestor(
		userId: string,
	): Promise<AccreditedInvestorValidation> {
		const errors: string[] = [];
		...
		try {
			...
			if ((profile as any).kycTier !== "accredited_investor") {
				errors.push(
					`KYC tier is ${(profile as any).kycTier}, must be accredited_investor for AIF investments`,
				);
			}
			...
			if (profile.accreditedInvestorStatus !== "verified") {
				errors.push(
					`Accredited investor status is ${profile.accreditedInvestorStatus}, must be verified`,
				);
			}

			// Check if accredited status has expired
			if (
				profile.accreditedInvestorExpiryDate &&
				new Date(profile.accreditedInvestorExpiryDate) < new Date()
			) {
				errors.push(
					"Accredited investor certification has expired. Annual renewal required.",
				);
			}

			// Verify qualification route
			const qualificationRoute =
				(profile as any).accreditedInvestorType || "unknown";

			// Income-based qualification
			if (qualificationRoute === "income_based") {
				if (
					!(profile as any).annualIncomeAmount ||
					Number((profile as any).annualIncomeAmount) <
						ACCREDITED_INVESTOR_CRITERIA.ANNUAL_INCOME_THRESHOLD
				) {
					errors.push(
						`Annual income must be ₹${(ACCREDITED_INVESTOR_CRITERIA.ANNUAL_INCOME_THRESHOLD / 10000000).toFixed(1)}Cr+`,
					);
				}
				if (
					!(profile as any).incomeProofDocuments ||
					(Array.isArray((profile as any).incomeProofDocuments) &&
						(profile as any).incomeProofDocuments.length === 0)
				) {
			...
			// Net worth-based qualification
				...
				if (
					!(profile as any).netWorthExcludingResidence ||
					Number((profile as any).netWorthExcludingResidence) <
						ACCREDITED_INVESTOR_CRITERIA.NET_WORTH_THRESHOLD
				) {
					errors.push(
						`Net worth (excluding residence) must be ₹${(ACCREDITED_INVESTOR_CRITERIA.NET_WORTH_THRESHOLD / 10000000).toFixed(1)}Cr+`,
					);
				}
			...
			// Portfolio-based qualification
				...
				if (
					...
					Number(profile.portfolioValueAmount) <
						ACCREDITED_INVESTOR_CRITERIA.PORTFOLIO_VALUE_THRESHOLD
				) {
					errors.push(
						`Portfolio value must be ₹${(ACCREDITED_INVESTOR_CRITERIA.PORTFOLIO_VALUE_THRESHOLD / 10000000).toFixed(1)}Cr+`,
					);
				}
			...
			// Professional qualification
				...
				if (
					...
					errors.push(
						"Valid professional qualification (CA/CFA/MBA Finance) required",
					);
				}
				...
				if (
					(profile.professionalExperienceYears || 0) <
					ACCREDITED_INVESTOR_CRITERIA.MIN_EXPERIENCE_YEARS
				) {
					errors.push(
						`Minimum ${ACCREDITED_INVESTOR_CRITERIA.MIN_EXPERIENCE_YEARS} years professional experience required`,
					);
				}
			} else {
			...
			if (profile.pepStatus === "Y") {
				errors.push(
					"PEP (Politically Exposed Person) status requires additional compliance approval",
				);
			}
			...
			console.error(
				"[AIF Execution] Accredited investor validation error:",
				error,
			);
			return {
				isValid: false,
	 ...
	 */
	private async generateSubscriptionAgreement(
		request: AIFExecutionRequest,
		...
		return {
			...
			riskDisclosures: [
				...
				...(isPartialPayment
					? [
							"Balance payment obligation must be met on AMC demand to avoid forfeiture",
						]
					: []),
			],
			termsAndConditions: [...baseTerms, ...paymentTerms, ...finalTerms],
	...
	private async processAIFSubscription(
		...
		try {
			console.log(
				`[AIF Execution] Processing subscription for ${request.fundName}`,
			);
			...
			console.log(
				`[AIF Execution] Subscription processed. Folio: ${folioNumber}`,
			);
	...
	private async storeSubscriptionAgreement(
		...
		try {
			...
			console.error(
				`[AIF Execution] Error creating agreement reference:`,
				error,
			);
	...
	private async handleExecutionFailure(
		...
		try {
			...
			console.error(
				`[AIF Execution] Failed to update order status for ${orderId}:`,
				error,
			);
		}
	}
