...
function resolveChromiumPath(): string | undefined {
	...
	for (const bin of whichCandidates) {
		try {
			const found = execSync(`which ${bin} 2>/dev/null`, { timeout: 2000 })
				.toString()
				.trim();
...
export class WhatsAppService {
	...
	constructor() {
		...
		if (chromiumPath) {
			...
			if (!this.isTwilioEnabled) {
				console.log(
					"[WhatsApp] No Chromium path found — Puppeteer will use its bundled browser",
				);
			}
		}
	...
	private setupEventHandlers() {
		this.client.on("qr", async (qr: string) => {
			...
			try {
				...
				console.log(
					"[WhatsApp] QR Code ready — visit /api/admin/whatsapp/qr to scan",
				);
			} catch (err) {
	...
	async initialize() {
		if (this.isTwilioEnabled) {
			console.log(
				"[WhatsApp] Twilio is configured. Local client initialization skipped.",
			);
			this.isReady = true;
			return;
		}
	...
	private async handleIncomingMessage(message: Message) {
		...
		console.log(
			`Message from ${contact.name || contact.number}: ${message.body}`,
		);
		...
		if (/^\d{6}$/.test(messageText)) {
			const session = Array.from(this.authSessions.values()).find(
				(s) =>
					...
					s.verificationCode === messageText &&
					!s.verified,
			);
		...
		if (
			messageText.toLowerCase().includes("login") ||
			messageText.toLowerCase().includes("signin")
		) {
		...
		if (
			message.body.toLowerCase().includes("portfolio") ||
	...
	async sendMessage(phoneNumber: string, message: string): Promise<boolean> {
		if (this.isTwilioEnabled) {
			try {
				const cleanPhone = phoneNumber.includes("@c.us")
					? phoneNumber.split("@")[0]
					: phoneNumber;
	...
	private async initiateClientLogin(
		phoneNumber: string,
		message: Message,
	): Promise<void> {
		try {
			...
			console.log(
				`Login verification sent to ${phoneNumber}: ${verificationCode}`,
			);
		} catch (error) {
	...
	async sendPasswordResetOTP(mobile: string, otp: string): Promise<boolean> {
		if (this.isTwilioEnabled) {
			try {
				...
				console.error(
					"Failed to send Twilio WhatsApp Password Reset OTP:",
					error,
				);
				return false;
			}
		...
		if (!this.isReady) {
			console.log(
				`📱 WhatsApp Password Reset OTP for ${mobile}: ${otp} (WhatsApp not ready)`,
			);
			return false;
		}
