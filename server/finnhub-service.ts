...
export class FinnhubService {
	...
	constructor() {
		...
		if (!this.apiKey) {
			console.warn(
				"⚠️ FINNHUB_API_KEY environment variable is required for real-time market data. Finnhub service will be disabled.",
			);
			return;
		}
		console.log(
			"Initializing Finnhub with API key length:",
			this.apiKey.length,
		);
	...
	async getQuote(symbol: string): Promise<FinnhubQuote> {
		return new Promise((resolve, reject) => {
			this.client.quote(
				symbol,
				(error: any, data: FinnhubQuote, response: any) => {
					if (error) {
						reject(error);
					} else {
						resolve(data);
	...
	async getCandles(
		...
		return new Promise((resolve, reject) => {
			this.client.stockCandles(
				...
				from,
				to,
				(error: any, data: FinnhubCandle, response: any) => {
					if (error) {
						reject(error);
					} else {
						resolve(data);
	...
	async getCompanyProfile(symbol: string): Promise<FinnhubCompanyProfile> {
		return new Promise((resolve, reject) => {
			this.client.companyProfile2(
				{ symbol },
				(error: any, data: FinnhubCompanyProfile, response: any) => {
					if (error) {
						reject(error);
					} else {
						resolve(data);
