...
IMPORTANT: Return ONLY valid JSON, no markdown code blocks or extra text.`;
			...
			return (
				aiResponse.recommendations?.map((rec: any, index: number) => ({
					...
					expectedImpact:
						rec.expectedImpact || "Impact assessment not available",
	...
	async generateInvestmentProposal(
		...
		try {
			const prompt = this.buildInvestmentProposalPrompt(
				portfolioData,
				userProfile,
				additionalCapital,
			);
...
IMPORTANT: Return ONLY valid JSON, no markdown code blocks or extra text.`;
			...
			const proposal: InvestmentProposal = {
				...
				summary:
					aiResponse.summary ||
					"Comprehensive investment proposal based on your profile and goals.",
				totalRecommendedInvestment:
					aiResponse.totalRecommendedInvestment ||
					additionalCapital ||
					portfolioData.totalValue * 0.1,
				recommendations:
					aiResponse.recommendations?.map((rec: any) => ({
						...
						suggestedInstruments:
							rec.suggestedInstruments?.map((inst: any) => ({
						...
						rebalancingActions:
							rec.rebalancingActions?.map((action: any, index: number) => ({
								id: `action-${Date.now()}-${index}`,
								title: action.title || "Rebalancing Action",
								priority: action.priority || "medium",
								recommendation:
									action.recommendation || "Rebalancing recommendation",
								reasoning: action.reasoning || "Based on portfolio analysis",
								expectedImpact:
									action.expectedImpact || "Positive impact expected",
								actionRequired:
									action.actionRequired || "Take recommended action",
				...
				riskAssessment: {
					...
					mitigationStrategies: aiResponse.riskAssessment
						?.mitigationStrategies || ["Diversification", "Regular monitoring"],
				},
				expectedOutcomes: {
					shortTerm:
						aiResponse.expectedOutcomes?.shortTerm ||
						"Portfolio stabilization and risk adjustment",
					mediumTerm:
						aiResponse.expectedOutcomes?.mediumTerm ||
						"Steady growth aligned with market performance",
					longTerm:
						aiResponse.expectedOutcomes?.longTerm ||
						"Long-term wealth creation and goal achievement",
				},
				implementationPlan:
					aiResponse.implementationPlan?.map((phase: any) => ({
						phase: phase.phase || 1,
			...
			if (clientId) {
				await this.saveAIProposalToDatabase(
					proposal,
					clientId,
					portfolioData.id,
				);
	...
	async generateMonthlySurplusProposal(
		...
		try {
			...
			const dbProposalId = await this.saveAIProposalToDatabase(
				aiProposal,
				clientId,
				portfolioId,
			);
	...
	private async saveAIProposalToDatabase(
		...
		try {
			...
			const savedProposal =
				await this.storage.createInvestmentProposal(proposalData);
				...
				for (const instrument of recommendation.suggestedInstruments) {
					const itemData = {
						...
						allocationPercentage: (
							(instrument.recommendedAmount /
								aiProposal.totalRecommendedInvestment) *
							100
						).toString(),
						...
						expectedOutcome:
							instrument.expectedReturn || "Market aligned returns",
	...
	private async convertToPortfolioData(portfolio: any): Promise<PortfolioData> {
		...
		return {
			id: portfolio.id,
			totalValue: Number.parseFloat(portfolio.totalValue || "100000"),
	...
	private buildRebalancingPrompt(
		portfolioData: PortfolioData,
		userProfile: UserProfile,
	): string {
		return `
...
${portfolioData.holdings
	...
	.map(
		(h) =>
			`- ${h.symbol} (${h.assetType}): ₹${h.currentValue.toLocaleString("en-IN")}`,
	)
	.join("\n")}
