...
export class PhonePeService {
	...
	async createOrder(
		...
		try {
			...
			const redirectUrl =
				paymentRequest.redirectUrl || `${getAppBaseUrl()}/payment-success`;
	...
	async checkOrderStatus(
		merchantTransactionId: string,
	): Promise<PhonePeOrderStatus | null> {
		try {
	...
	verifyCallback(base64Response: string, receivedSignature: string): boolean {
		try {
			if (!this.saltKey) {
				console.error(
					"[PhonePe] PHONEPE_SALT_KEY is not set — cannot verify callback signature",
				);
				return false;
			}
