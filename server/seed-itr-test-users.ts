...
const ITR_TEST_USERS = [
	...
	},
	{
		...
		prefix: "ITR",
		label:
			"ITR-2 — Individual with capital gains, multiple properties, foreign income",
		itrForm: "ITR-2",
	},
	{
	...
	},
	{
		...
		prefix: "ITR",
		label:
			"ITR-4 (Sugam) — Individual with presumptive income (44AD/44ADA/44AE)",
		itrForm: "ITR-4",
	},
	{
...
async function seedITRTestUsers(): Promise<void> {
	...
	try {
		for (const testUser of ITR_TEST_USERS) {
			try {
				...
				if (existing.length > 0) {
					...
					console.log(
						`✅ CREATED: ${testUser.email} (${userId}) — ${testUser.label}`,
					);
					created++;
				}
			} catch (err) {
				console.error(
					`❌ FAILED: ${testUser.email} — ${err instanceof Error ? err.message : err}`,
				);
				failed++;
			}
		}
		...
		console.log(
			"║            ITR TEST USERS — ALL FORMS (ITR-1 to ITR-7)          ║",
		);
		console.log(
			"╠═══════════════════════════════════════════════════════════════════╣",
		);
		console.log(
			"║ Password: Test@123456       OTP: 123456 (tester role)            ║",
		);
		console.log(
			"║ All users have ALL roles (superadmin → tester)                   ║",
		);
		console.log(
			"╠═══════════════════════════════════════════════════════════════════╣",
		);
		...
		console.log(
			"║ ITR-1 │ itr1-individual@fintekpro.com │ Individual (Salaried)    ║",
		);
		console.log(
			"║ ITR-2 │ itr2-individual@fintekpro.com │ Individual (CG/Foreign)  ║",
		);
		console.log(
			"║ ITR-3 │ itr3-business@fintekpro.com   │ Individual (Business)    ║",
		);
		console.log(
			"║ ITR-3 │ itr3-huf@fintekpro.com        │ HUF (Business)           ║",
		);
		console.log(
			"║ ITR-4 │ itr4-presumptive@fintekpro.com│ Individual (Presumptive) ║",
		);
		console.log(
			"║ ITR-4 │ itr4-firm@fintekpro.com       │ Firm (Presumptive)       ║",
		);
		console.log(
			"║ ITR-5 │ itr5-partnership@fintekpro.com│ LLP                      ║",
		);
		console.log(
			"║ ITR-5 │ itr5-aop@fintekpro.com        │ AOP                      ║",
		);
		console.log(
			"║ ITR-5 │ itr5-boi@fintekpro.com        │ BOI                      ║",
		);
		console.log(
			"║ ITR-6 │ itr6-company@fintekpro.com    │ Company (Pvt Ltd)        ║",
		);
		console.log(
			"║ ITR-7 │ itr7-trust@fintekpro.com      │ Trust (Charitable)       ║",
		);
		console.log(
			"║ ITR-7 │ itr7-society@fintekpro.com    │ Society                  ║",
		);
		console.log(
			"║                                                                  ║",
		);
		...
		console.log(
			`║ Results: ${created} created, ${updated} updated, ${failed} failed${" ".repeat(Math.max(0, 26 - String(created).length - String(updated).length - String(failed).length))}║`,
		);
		console.log(
			"╚═══════════════════════════════════════════════════════════════════╝\n",
		);
