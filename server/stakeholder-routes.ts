...
router.get("/api/admin/stakeholders/stats", requireAdmin, async (req, res) => {
	try {
		...
		const [{ count: clientsTotal }] = await db
			.select({ count: sql<number>`count(*)` })
			.from(users);
		const [{ count: partnersTotal }] = await db
			.select({ count: sql<number>`count(*)` })
			.from(partners);
		const [{ count: agentsTotal }] = await db
			.select({ count: sql<number>`count(*)` })
		...
		const [{ count: clientsLastMonth }] = await db
			.select({ count: sql<number>`count(*)` })
			.from(users)
			.where(lt(users.createdAt, oneMonthAgo));

		const [{ count: partnersLastMonth }] = await db
			.select({ count: sql<number>`count(*)` })
			.from(partners)
			.where(lt(partners.createdAt, oneMonthAgo));

		const [{ count: agentsLastMonth }] = await db
			.select({ count: sql<number>`count(*)` })
			.from(customerCareAgents)
			.where(lt(customerCareAgents.createdAt, oneMonthAgo));

		const [{ count: suppliersLastMonth }] = await db
			.select({ count: sql<number>`count(*)` })
			.from(suppliers)
		...
		const stats = {
			...
			suppliers: {
				...
				growth: calculateGrowth(
					Number(suppliersTotal),
					Number(suppliersLastMonth),
				),
			},
		};
		...
		return apiResponse.error(
			res,
			"Failed to fetch stakeholder stats",
			500,
			error.message,
		);
	}
...
router.post("/api/admin/partners", requireAdmin, async (req, res) => {
	try {
		...
		const [existing] = await db
			.select()
			.from(partners)
			.where(eq(partners.contactEmail, validatedData.contactEmail))
			.limit(1);
		if (existing) {
...
router.patch("/api/admin/partners/:id", requireAdmin, async (req, res) => {
	try {
		...
		if (
			...
			validatedData.contactEmail !== existing.contactEmail
		) {
	...
	try {
		...
		const results = whereClause
			? await query
					...
					.limit(limitNum)
					.offset(offset)
					.orderBy(sql`${customerCareAgents.createdAt} DESC`)
			: await query
					.limit(limitNum)
					.offset(offset)
					.orderBy(sql`${customerCareAgents.createdAt} DESC`);
...
router.patch("/api/admin/agents/:id", requireAdmin, async (req, res) => {
	try {
		...
		if (
			defaultCommissionShare !== undefined &&
			defaultCommissionShare !== null
		) {
			const shareValue = Number.parseFloat(defaultCommissionShare);
			if (Number.isNaN(shareValue) || shareValue < 0 || shareValue > 100) {
				return apiResponse.badRequest(
					res,
		...
		if (commissionSplitModel !== undefined)
			updateData.commissionSplitModel = commissionSplitModel;
		// Allow setting commission share to null (for standard model) or a specific value (for custom model)
		if (defaultCommissionShare !== undefined)
			updateData.defaultCommissionShare = defaultCommissionShare;
