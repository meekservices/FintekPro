import {
	...
	type InsertCapitalGainsReport,
	type TransactionReport,
	type InsertTransactionReport,
	type TransactionRecord,
	type InsertTransactionRecord,
	type CustomerCareAgent,
	type InsertCustomerCareAgent,
	type AgentPartnerMapping,
	type InsertAgentPartnerMapping,
	type CkycRecord,
	type InsertCkycRecord,
	type CkycDocument,
	type InsertCkycDocument,
	type CkycStatusHistory,
	type InsertCkycStatusHistory,
	type ClientAgentRelationship,
	type InsertClientAgentRelationship,
	type InvestmentProposal,
	type InsertInvestmentProposal,
	type InvestmentProposalItem,
	type InsertInvestmentProposalItem,
	type ProposalPayment,
	type InsertProposalPayment,
	type IBAccount,
	type InsertIBAccount,
	type IBOrder,
	type InsertIBOrder,
	type IBPosition,
	type InsertIBPosition,
	type IBAccountSummary,
	type InsertIBAccountSummary,
	type IBMarketDataSubscription,
	type InsertIBMarketDataSubscription,
	type IBTradingSession,
	type InsertIBTradingSession,
	type Partner,
	type InsertPartner,
	type Agent,
	type InsertAgent,
	...
	type PpfHolding,
	type EpsHolding,
	type GovernmentSchemeConsent,
	type InsertGovernmentSchemeConsent,
	type InsuranceHolding,
	type InsertInsuranceHolding,
	type UserBankAccount,
	type InsertUserBankAccount,
	type UserDematAccount,
	type InsertUserDematAccount,
	...
	type InsertExternalHolding,
	type StructuredTaxData,
	type InsertStructuredTaxData,
	type UserAlert,
	...
	type FamilyGoalContribution,
	type InsertFamilyGoalContribution,
	type FamilyActivityLog,
	type InsertFamilyActivityLog,
	type FamilyDiscussion,
	type InsertFamilyDiscussion,
	type FamilyBudget,
	type InsertFamilyBudget,
	type FamilyPortfolioPermission,
	type InsertFamilyPortfolioPermission,
	type TaxCalculation,
	type InsertTaxCalculation,
	type TaxDocumentAccessLog,
	type InsertTaxDocumentAccessLog,
	type TaxSession,
	type InsertTaxSession,
	type TaxDataSource,
	type InsertTaxDataSource,
	type ValidationIssue,
	type InsertValidationIssue,
	type FilingRecord,
	type InsertFilingRecord,
	type AiOptimizationSuggestion,
	type InsertAiOptimizationSuggestion,
	type FundExtended,
	type Provenance,
	...
	type LoanApplicationMarketplace,
	type InsertLoanApplicationMarketplace,
	type ProviderIntegration,
	type InsertProviderIntegration,
	type PartnerApplicationDocument,
	type InsertPartnerApplicationDocument,
	type InvestmentIdea,
	type InsertInvestmentIdea,
	type InvestmentIdeaTracking,
	type InsertInvestmentIdeaTracking,
	type InvestmentIdeaAlert,
	type InsertInvestmentIdeaAlert,
	type YieldTracker,
	type InsertYieldTracker,
	type PartnerApplication,
	type InsertPartnerApplication,
	type TaxRule,
	type InsertTaxRule,
	type TaxReminderSubscription,
	type InsertTaxReminderSubscription,
	type CapitalGainsTaxReminder,
	type InsertCapitalGainsTaxReminder,
	type UserExpense,
	type InsertUserExpense,
	type UserBudget,
	type InsertUserBudget,
	type ExpenseInsight,
	type InsertExpenseInsight,
	type FinancialObligation,
	type InsertFinancialObligation,
	type NpsAccount,
	type ApyAccount,
	type ClientTask,
...
import {
	...
	type InsertAgentCommissionSplit,
	type AgentCommission,
	type InsertAgentCommission,
	type AmfiVerificationLog,
	type InsertAmfiVerificationLog,
} from "@shared/schema";
import {
	type Product,
	type InsertProduct,
	...
	type ProductAccountPreference,
	type InsertProductAccountPreference,
	type ICICILoanApplication,
	type InsertICICILoanApplication,
	type ICICICreditScore,
	type InsertICICICreditScore,
	type PortfolioComparison,
	type InsertPortfolioComparison,
	type ChatSession,
	...
	type ChatFunction,
	type InsertChatFunction,
	type CurrencyRate,
	type InsertCurrencyRate,
	type CkycNotificationTrigger,
	type InsertCkycNotificationTrigger,
	type KycVerificationSession,
	type InsertKycVerificationSession,
	type ManualKycSubmission,
	type InsertManualKycSubmission,
	type ManualKycDocument,
	type InsertManualKycDocument,
	type UnlistedCompany,
	type InsertUnlistedCompany,
	type CompanyFinancials,
	type InsertCompanyFinancials,
	type CompanyRatios,
	type InsertCompanyRatios,
	type UnlistedPriceHistory,
	type InsertUnlistedPriceHistory,
	type SellListing,
	type InsertSellListing,
	type BuyRequest,
	type InsertBuyRequest,
	type UnlistedDeal,
	type InsertUnlistedDeal,
	type Probe42SyncLog,
	type InsertProbe42SyncLog,
	type SupportTemplate,
	type InsertSupportTemplate,
	type SupportStep,
	type InsertSupportStep,
	type SupportStepComment,
	type InsertSupportStepComment,
...
export class DatabaseStorage implements IStorage {
	...
	async getUserByUsername(username: string): Promise<User | undefined> {
		const [user] = await db
			...
			.from(schema.users)
			.where(
				or(eq(schema.users.userId, username), eq(schema.users.email, username)),
			);
		return user;
	}
	// User methods
	...
	async createUser(user: UpsertUser): Promise<User> {
		...
		if (!user.userId) {
			throw new Error(
				"userId is required and must be provided when creating a user",
			);
		}
	...
	async updateUser(
		...
		if (updates.mobile !== undefined) {
			...
			const currentMobile = currentUser?.mobile?.trim() || null;
			const nextMobile = updates.mobile?.trim() || null;

			if (currentMobile !== nextMobile) {
				mobileChanged = true;
				finalUpdates.isMobileVerified = false;
				finalUpdates.isPinSet = false;
				finalUpdates.loginPin = null;
			}
		}
		...
		const [updatedUser] = await db
			...
			.where(eq(schema.users.id, id))
			.returning();
		if (updatedUser && mobileChanged) {
			await db
				.update(schema.userTrustedDevices)
				.set({ revokedAt: new Date(), updatedAt: new Date() })
				.where(eq(schema.userTrustedDevices.userId, id));
		}
	...
	async getPortfoliosByUserPan(panNumber: string): Promise<Portfolio[]> {
		...
		const users = await db
			.select()
			.from(schema.users)
			.where(eq(schema.users.panNumber, panNumber));
	...
	async getExternalHoldings(userId: string): Promise<ExternalHolding[]> {
		return await db
			.select()
			.from(schema.externalHoldings)
			.where(eq(schema.externalHoldings.userId, userId));
	}

	async getExternalHoldingsBySource(
		userId: string,
		source: string,
	...
	async createExternalHolding(
		holding: InsertExternalHolding,
	): Promise<ExternalHolding> {
		const [newHolding] = await db
	...
	async deleteExternalHoldingsBySource(
		userId: string,
		source: string,
	): Promise<number> {
		const result = await db
			.delete(schema.externalHoldings)
			.where(
				and(
					eq(schema.externalHoldings.userId, userId),
					eq(schema.externalHoldings.source, source),
	...
	async getMultipleMarketData(symbols: string[]): Promise<MarketData[]> {
		...
		return await db
			.select()
			.from(schema.marketData)
			.where(sql`${schema.marketData.symbol} = ANY(${symbols})`);
	}
	...
	async getEpfHoldings(userId: string): Promise<EpfHolding[]> {
		return db
			...
			.where(eq(schema.epfHoldings.userId, userId));
	}
	...
	async getPpfHoldings(userId: string): Promise<PpfHolding[]> {
		return db
			...
			.where(eq(schema.ppfHoldings.userId, userId));
	}
	...
	async getEpsHoldings(userId: string): Promise<EpsHolding[]> {
		return db
			...
			.where(eq(schema.epsHoldings.userId, userId));
	}
	...
	async getNpsAccounts(userId: string): Promise<NpsAccount[]> {
		return db
			...
			.where(eq(schema.npsAccounts.userId, userId));
	}
	...
	async getApyAccounts(userId: string): Promise<ApyAccount[]> {
		return db
			...
			.where(eq(schema.apyAccounts.userId, userId));
	}

	// Financial Obligations methods
	async getFinancialObligations(
		userId: string,
	): Promise<FinancialObligation[]> {
		return db
			.select()
			.from(schema.financialObligations)
			.where(eq(schema.financialObligations.userId, userId))
			.orderBy((schema.financialObligations as any).dueDate);
	}

	async getFinancialObligationById(
		id: string,
	): Promise<FinancialObligation | undefined> {
	...
	async createFinancialObligation(
		data: InsertFinancialObligation,
	): Promise<FinancialObligation> {
		const results = await db
			.insert(schema.financialObligations)
			.values(data)
			.returning();
		return results[0];
	}
	...
	async updateFinancialObligation(
		id: string,
		updates: Partial<FinancialObligation>,
	): Promise<FinancialObligation | undefined> {
		const results = await db
			.update(schema.financialObligations)
	...
	async deleteFinancialObligation(id: string): Promise<void> {
		await db
			.delete(schema.financialObligations)
			.where(eq(schema.financialObligations.id, id));
	}
	...
	async deleteUserCibilObligations(userId: string): Promise<void> {
		await db
			...
			.where(
				and(
					eq(schema.financialObligations.userId, userId),
					eq((schema.financialObligations as any).fromCibil, true),
				),
			);
	}
	...
	async createInsuranceHolding(
		holding: InsertInsuranceHolding,
	): Promise<InsuranceHolding> {
		throw new Error("Method not implemented");
	}

	async updateInsuranceHolding(
		id: string,
		updates: Partial<InsuranceHolding>,
	): Promise<InsuranceHolding | undefined> {
		throw new Error("Method not implemented");
	}

	async checkGovernmentSchemeConsent(
		userId: string,
		panNumber: string,
		schemeType: string,
	): Promise<boolean> {
	...
	async createGovernmentSchemeConsent(
		...
		try {
			...
			const [result] = await db
				.insert(schema.dataSourceConsents)
				.values({
					id: consentId,
	...
	async getGovernmentSchemeConsents(
		userId: string,
		panNumber?: string,
	): Promise<GovernmentSchemeConsent[]> {
		try {
			...
			const filteredResults = results.filter((r) =>
				schemeTypes.includes(r.dataSource || ""),
			);
	...
	async revokeGovernmentSchemeConsent(
		userId: string,
		panNumber: string,
		schemeType: string,
	): Promise<boolean> {
		try {
			const result = await db
				.update(schema.dataSourceConsents)
				.set({
					isActive: false,
					revokedAt: new Date(),
	...
	async recordPanVerificationConsent(
		userId: string,
		ipAddress: string,
		userAgent: string,
	): Promise<void> {
	...
	async upsertAssetAllocation(
		allocation: InsertAssetAllocation,
	...
	async createCapitalGainsReport(
		report: InsertCapitalGainsReport,
	): Promise<CapitalGainsReport> {
		throw new Error("Method not implemented");
	}

	async getCapitalGainsReports(
		userId?: string,
	...
	async getCapitalGainsReport(
		id: string,
	): Promise<CapitalGainsReport | undefined> {
		return undefined;
	}

	async updateCapitalGainsReport(
		id: string,
		updates: Partial<CapitalGainsReport>,
	): Promise<CapitalGainsReport | undefined> {
		return undefined;
	}

	async createTransactionReport(
		report: InsertTransactionReport,
	...
	async getTransactionReport(
		id: string,
	): Promise<TransactionReport | undefined> {
		return undefined;
	}

	async updateTransactionReport(
		id: string,
		updates: Partial<TransactionReport>,
	): Promise<TransactionReport | undefined> {
		return undefined;
	}
	...
	async getTransactionRecordsByUser(
		userId: string,
		financialYear?: string,
	): Promise<TransactionRecord[]> {
		return [];
	...
	async createCustomerCareAgent(
		agent: InsertCustomerCareAgent,
	): Promise<CustomerCareAgent> {
	...
	async getCustomerCareAgent(
		id: string,
	): Promise<CustomerCareAgent | undefined> {
	...
	async updateCustomerCareAgent(
		id: string,
		updates: Partial<CustomerCareAgent>,
	): Promise<CustomerCareAgent | undefined> {
	...
	async createAgentPartnerMapping(
		mapping: InsertAgentPartnerMapping,
	): Promise<AgentPartnerMapping> {
		throw new Error("Method not implemented");
	}

	async getAgentPartnerMappings(
		agentId?: string,
		partnerId?: string,
	): Promise<AgentPartnerMapping[]> {
		try {
			let query = db
				...
				.leftJoin(
					schema.partners,
					eq(schema.agentPartnerMappings.partnerId, schema.partners.id),
				);
	...
	async updateAgentPartnerMapping(
		id: string,
		updates: Partial<AgentPartnerMapping>,
	): Promise<AgentPartnerMapping | undefined> {
		return undefined;
	}
	...
	async getAgentMappingCounts(
		agentId: string,
	): Promise<{ partnerCount: number; clientCount: number }> {
		try {
	...
	async createAchievementCategory(
		category: InsertAchievementCategory,
	...
	async markAchievementCompleted(
		id: string,
	): Promise<UserAchievement | undefined> {
		return undefined;
	}
	...
	async updateCkycRecord(
		userId: string,
		updates: Partial<CkycRecord>,
	): Promise<CkycRecord | undefined> {
		return undefined;
	}

	async getAllCkycRecords(options?: {
		status?: string;
		page?: number;
		limit?: number;
	}): Promise<CkycRecord[]> {
	...
	async addCkycStatusHistory(
		history: InsertCkycStatusHistory,
	): Promise<CkycStatusHistory> {
		throw new Error("Method not implemented");
	}

	// CKYC Notification Triggers method implementation
	async getCkycNotificationTriggers(
		ckycRecordId?: string,
		status?: string,
	): Promise<any[]> {
	...
	async getClientAgentRelationships(
		clientId?: string,
		agentId?: string,
	): Promise<ClientAgentRelationship[]> {
		try {
			console.log("getClientAgentRelationships called with:", {
				clientId,
				agentId,
			});
			...
			const basicResults = await db
				...
				.from(schema.clientAgentRelationships);
			console.log(
				"Basic relationships query returned:",
				basicResults.length,
				"records",
			);
			...
			const clientIds = Array.from(
				new Set(basicResults.map((r) => r.clientId)),
			);
	...
	async getClientAgentRelationship(
		clientId: string,
		agentId: string,
	): Promise<ClientAgentRelationship | undefined> {
		try {
			const result = await db
				.select()
				.from(schema.clientAgentRelationships)
	...
	async createClientAgentRelationship(
		relationship: InsertClientAgentRelationship,
	): Promise<ClientAgentRelationship> {
		try {
			const result = await db
	...
	async updateClientAgentRelationship(
		id: string,
		updates: Partial<ClientAgentRelationship>,
	): Promise<ClientAgentRelationship | undefined> {
		try {
			const result = await db
				.update(schema.clientAgentRelationships)
				.set({ ...updates, updatedAt: new Date() })
	...
	async deleteClientAgentRelationship(id: string): Promise<boolean> {
		try {
			const result = await db
				.delete(schema.clientAgentRelationships)
				.where(eq(schema.clientAgentRelationships.id, id))
				.returning();
	...
	async getAgentForClient(
		...
		if (relationshipType) {
			conditions.push(
				eq(schema.clientAgentRelationships.relationshipType, relationshipType),
			);
		}
		...
		const [result] = await db
			...
			.innerJoin(
				schema.users,
				eq(schema.clientAgentRelationships.agentId, schema.users.id),
			)
	...
	async autoAssignDefaultAgent(
		userId: string,
	): Promise<ClientAgentRelationship | null> {
		try {
			...
			console.log(
				`✅ Auto-assigned user ${userId} to default agent ${defaultAgent.fullName}`,
			);
			return relationship;
		} catch (error) {
	...
	async getInvestmentProposals(options?: {
		clientId?: string;
		agentId?: string;
		status?: string;
		...
		try {
			...
			if (options?.clientId) {
				conditions.push(
					eq(schema.investmentProposals.clientId, options.clientId),
				);
			}
			if (options?.agentId) {
				conditions.push(
					eq(schema.investmentProposals.agentId, options.agentId),
				);
			}
			if (options?.status) {
	...
	async getInvestmentProposal(
		id: string,
	): Promise<InvestmentProposal | undefined> {
		try {
	...
	async createInvestmentProposal(
		proposal: InsertInvestmentProposal,
	): Promise<InvestmentProposal> {
		try {
			const [created] = await db
	...
	async updateInvestmentProposal(
		id: string,
		updates: Partial<InvestmentProposal>,
	): Promise<InvestmentProposal | undefined> {
		try {
			const [updated] = await db
	...
	async getProposalItems(
		proposalId: string,
	): Promise<InvestmentProposalItem[]> {
		return [];
	}

	async createProposalItem(
		item: InsertInvestmentProposalItem,
	): Promise<InvestmentProposalItem> {
		throw new Error("Method not implemented");
	}
	...
	async updateProposalItem(
		id: string,
		updates: Partial<InvestmentProposalItem>,
	): Promise<InvestmentProposalItem | undefined> {
		return undefined;
	}
	...
	async getInvestmentProposalItems(
		proposalId: string,
	): Promise<InvestmentProposalItem[]> {
		return [];
	}
			...
			return {
				...
				message:
					error instanceof Error
						? error.message
						: "Failed to add proposal to cart",
			};
		}
	}

	async createProposalPayment(
		payment: InsertProposalPayment,
	): Promise<ProposalPayment> {
		throw new Error("Method not implemented");
	}

	async getProposalPayments(
		proposalId?: string,
		status?: string,
	): Promise<ProposalPayment[]> {
		return [];
	...
	async updateProposalPayment(
		id: string,
		updates: Partial<ProposalPayment>,
	): Promise<ProposalPayment | undefined> {
		return undefined;
	}

	async getAgentTransactionReports(
		agentId: string,
		filters?: { clientId?: string; status?: string; reportType?: string },
	): Promise<TransactionReport[]> {
		return [];
	}

	async getAgentCapitalGainsReports(
		agentId: string,
		filters?: { clientId?: string; financialYear?: string; status?: string },
	): Promise<CapitalGainsReport[]> {
		return [];
	}
	...
	async updateIBAccount(
		id: string,
		updates: Partial<IBAccount>,
	): Promise<IBAccount | undefined> {
		return undefined;
	}
	...
	async updateIBAccountConnectionStatus(
		id: string,
		status: string,
		lastConnected?: Date,
	): Promise<IBAccount | undefined> {
	...
	async updateIBOrder(
		id: string,
		updates: Partial<IBOrder>,
	): Promise<IBOrder | undefined> {
		return undefined;
	}
	...
	async updateIBPosition(
		id: string,
		updates: Partial<IBPosition>,
	): Promise<IBPosition | undefined> {
		return undefined;
	}
	...
	async getIBAccountSummary(
		userId: string,
		ibAccountId?: string,
	): Promise<IBAccountSummary[]> {
		return [];
	}

	async createIBAccountSummary(
		summary: InsertIBAccountSummary,
	): Promise<IBAccountSummary> {
		throw new Error("Method not implemented");
	}

	async updateIBAccountSummary(
		id: string,
		updates: Partial<IBAccountSummary>,
	): Promise<IBAccountSummary | undefined> {
		return undefined;
	}

	async upsertIBAccountSummary(
		summary: InsertIBAccountSummary,
	): Promise<IBAccountSummary> {
		throw new Error("Method not implemented");
	}

	async getIBMarketDataSubscriptions(
		userId: string,
		ibAccountId?: string,
	): Promise<IBMarketDataSubscription[]> {
		return [];
	}

	async createIBMarketDataSubscription(
		subscription: InsertIBMarketDataSubscription,
	): Promise<IBMarketDataSubscription> {
		throw new Error("Method not implemented");
	}

	async updateIBMarketDataSubscription(
		id: string,
		updates: Partial<IBMarketDataSubscription>,
	): Promise<IBMarketDataSubscription | undefined> {
		return undefined;
	}
	...
	async getIBMarketDataSubscriptionBySymbol(
		symbol: string,
		ibAccountId: string,
	): Promise<IBMarketDataSubscription | undefined> {
		return undefined;
	...
	async getIBTradingSessions(
		userId: string,
		ibAccountId?: string,
	): Promise<IBTradingSession[]> {
		return [];
	}
	...
	async createIBTradingSession(
		session: InsertIBTradingSession,
	): Promise<IBTradingSession> {
		throw new Error("Method not implemented");
	}

	async updateIBTradingSession(
		id: string,
		updates: Partial<IBTradingSession>,
	): Promise<IBTradingSession | undefined> {
		return undefined;
	}

	async getActiveIBTradingSession(
		ibAccountId: string,
	): Promise<IBTradingSession | undefined> {
		return undefined;
	}

	async endIBTradingSession(
		id: string,
	...
	async getSupplierPerformance(supplierId: string): Promise<any> {
		...
		const totalRevenue = performance.reduce(
			(sum, p) => sum + Number.parseFloat(p.revenue || "0"),
			0,
		);
		const totalSales = performance.reduce(
			(sum, p) => sum + (p.salesVolume || 0),
			0,
		);
		const avgProfitMargin =
			performance.length > 0
				? performance.reduce(
						(sum, p) => sum + Number.parseFloat(p.profitMargin || "0"),
						0,
					) / performance.length
				: 0;
	...
	async getProducts(filters?: {
		...
		if (filters?.minReturn1y !== undefined) {
			conditions.push(
				gte(schema.products.returns1y, filters.minReturn1y.toString()),
			);
		}
		...
		const results = await db
			...
			.where(and(...conditions))
			.orderBy(
				desc((schema.products as any).priority),
	...
	async getProductsByCategory(
		...
		const results = await db
			...
			.orderBy(
				desc((schema.products as any).priority),
				desc(schema.products.returns1y),
			)
			.limit(100);
	...
	async calculatePerformanceTag(product: Product): Promise<string | null> {
		const returns1y = Number.parseFloat(product.returns1y || "0");
		const returns3y = Number.parseFloat(product.returns3y || "0");
		const returns6m = Number.parseFloat(product.returns6m || "0");
		const returns3m = Number.parseFloat(product.returns3m || "0");
	...
	async updateCreditProfile(
		userId: string,
		updates: Partial<CreditProfile>,
	): Promise<CreditProfile | undefined> {
		const [updated] = await db
	...
	async getLoanProducts(): Promise<LoanProduct[]> {
		return db
			...
			.orderBy(schema.loanProducts.productName);
	}
	...
	async getLoanProductByKey(
		productKey: string,
	): Promise<LoanProduct | undefined> {
		const [product] = await db
			.select()
			.from(schema.loanProducts)
	...
	async getLoanProviders(): Promise<LoanProvider[]> {
		return db
			...
			.orderBy(schema.loanProviders.providerName);
	}
	...
	async getLoanProviderByKey(
		providerKey: string,
	): Promise<LoanProvider | undefined> {
		const [provider] = await db
			.select()
			.from(schema.loanProviders)
	...
	async createLoanProvider(
		provider: InsertLoanProvider,
	): Promise<LoanProvider> {
	...
	async getProviderProductsByProvider(
		providerId: string,
		productKey?: string,
	...
	async updateProviderProduct(
		id: string,
		updates: Partial<ProviderProduct>,
	): Promise<ProviderProduct | undefined> {
		const [updated] = await db
	...
	async getLoanApplicationsMarketplace(
		userId?: string,
	): Promise<LoanApplicationMarketplace[]> {
		if (userId) {
			return db
				.select()
	...
	async getLoanApplicationMarketplace(
		id: string,
	): Promise<LoanApplicationMarketplace | undefined> {
	...
	async createLoanApplicationMarketplace(
		application: InsertLoanApplicationMarketplace,
	): Promise<LoanApplicationMarketplace> {
	...
	async updateLoanApplicationMarketplace(
		id: string,
		updates: Partial<LoanApplicationMarketplace>,
	): Promise<LoanApplicationMarketplace | undefined> {
		const [updated] = await db
	...
	async getProviderIntegrationsByProvider(
		providerId: string,
	...
	async createProviderIntegration(
		integration: InsertProviderIntegration,
	): Promise<ProviderIntegration> {
	...
	async updateProviderIntegration(
		id: string,
		updates: Partial<ProviderIntegration>,
	): Promise<ProviderIntegration | undefined> {
		const [updated] = await db
	...
	async generatePortfolioRebalanceRecommendations(
		portfolioId: string,
		goals: FinancialGoal[],
	): Promise<any[]> {
		return [];
	...
	async createBankAccount(
		bankAccount: InsertUserBankAccount,
		...
		const dataToInsert = {
			...bankAccount,
			isPrimary:
				existingAccounts.length === 0 ? true : bankAccount.isPrimary ?? false,
		};
	...
	async getUserBankAccounts(userId: string): Promise<UserBankAccount[]> {
		return await db
			...
			.orderBy(
				desc(schema.userBankAccounts.isPrimary),
				desc(schema.userBankAccounts.createdAt),
			);
	}
	...
	async updateBankAccount(
		id: string,
		updates: Partial<UserBankAccount>,
	): Promise<UserBankAccount | undefined> {
		const [updated] = await db
	...
	async setDefaultBankAccount(
		accountId: string,
		defaultType: "mutualFunds" | "primary",
	): Promise<boolean> {
	...
	async createDematAccount(
		dematAccount: InsertUserDematAccount,
	): Promise<UserDematAccount> {
		throw new Error("Method not implemented");
	}
	...
	async updateDematAccount(
		id: string,
		updates: Partial<UserDematAccount>,
	): Promise<UserDematAccount | undefined> {
		return undefined;
	}
	...
	async setDefaultDematAccount(
		accountId: string,
		defaultType: "equity" | "mutualFunds",
	): Promise<boolean> {
		return false;
	}

	// Product Account Preference Methods
	async createProductAccountPreference(
		preference: InsertProductAccountPreference,
	): Promise<ProductAccountPreference> {
	...
	async getUserProductAccountPreferences(
		userId: string,
	): Promise<ProductAccountPreference[]> {
		return await db
			.select()
	...
	async getProductAccountPreference(
		userId: string,
		productType: string,
	): Promise<ProductAccountPreference | undefined> {
		const [preference] = await db
			...
			.where(
				and(
					...
					eq(
						(schema.productAccountPreferences as any).productType,
						productType,
					),
	...
	async updateProductAccountPreference(
		id: string,
		updates: Partial<ProductAccountPreference>,
	): Promise<ProductAccountPreference | undefined> {
		const [updated] = await db
	...
	async initializeUserPasswords(): Promise<void> {
		...
		console.log(
			"DatabaseStorage: Password initialization handled in user creation/update methods",
		);
	}

	// ICICI Bank Loan Application methods
	async createICICILoanApplication(
		application: InsertICICILoanApplication,
	): Promise<ICICILoanApplication> {
	...
	async getICICILoanApplicationsByUser(
		userId: string,
	): Promise<ICICILoanApplication[]> {
		return await db
			.select()
	...
	async getICICILoanApplication(
		id: string,
	): Promise<ICICILoanApplication | undefined> {
		const [application] = await db
			.select()
			.from(schema.iciciBankLoanApplications)
	...
	async getICICILoanApplicationByApplicationId(
	...
	async updateICICILoanApplicationStatus(
		applicationId: string,
		updates: Partial<ICICILoanApplication>,
	): Promise<ICICILoanApplication | undefined> {
	...
	async createICICICreditScore(
		creditScore: InsertICICICreditScore,
	): Promise<ICICICreditScore> {
	...
	async getICICICreditScoresByUser(
		userId: string,
	): Promise<ICICICreditScore[]> {
		return await db
			.select()
	...
	async getLatestICICICreditScore(
		userId: string,
	): Promise<ICICICreditScore | undefined> {
	...
	async createPortfolioComparison(
		comparison: InsertPortfolioComparison,
	): Promise<string> {
		try {
			const [result] = await db
				.insert(schema.portfolioComparisons)
				.values(comparison)
				.returning({ id: schema.portfolioComparisons.id });
			return result.id;
		} catch (error) {
	...
	async getPortfolioComparison(id: string): Promise<any> {
		try {
			const [comparison] = await db
				.select()
				.from(schema.portfolioComparisons)
				.where(eq(schema.portfolioComparisons.id, id));
			return comparison;
		} catch (error) {
	...
	async createStructuredTaxData(
		data: InsertStructuredTaxData,
	): Promise<StructuredTaxData> {
		const [result] = await this.db
	...
	async getStructuredTaxDataByUser(
		userId: string,
		financialYear?: string,
	): Promise<StructuredTaxData[]> {
		try {
			...
			const results = await this.db
				...
				.innerJoin(
					schema.taxDocuments,
					eq(schema.structuredTaxData.documentId, schema.taxDocuments.id),
				)
				.where(and(...conditions))
				.orderBy(desc(schema.structuredTaxData.createdAt));
	...
	async updateStructuredTaxData(
		id: string,
		updates: Partial<StructuredTaxData>,
	): Promise<StructuredTaxData | undefined> {
		const [result] = await this.db
	...
	async getTaxCalculations(
		...
		try {
			...
			if (financialYear) {
				conditions.push(
					eq(schema.taxCalculations.financialYear, financialYear),
				);
			}
	...
	async createTaxDocumentAccessLog(
		log: InsertTaxDocumentAccessLog,
	): Promise<TaxDocumentAccessLog> {
		const [result] = await this.db
	...
	async getTaxDocumentAccessLogs(
		documentId: string,
	): Promise<TaxDocumentAccessLog[]> {
		return await this.db
			.select()
	...
	async processTaxDocument(
		...
		try {
			...
			throw new Error(
				"Tax document processing service not configured. Document parsing API integration required.",
			);
		} catch (error) {
	...
	async validateTaxData(
		...
		for (const data of structuredData) {
			...
			if (data.taxableAmount && Number.parseFloat(data.taxableAmount) < 0) {
				errors.push(`Negative taxable amount for entry ${data.id}`);
			}

			if (data.taxDeducted && Number.parseFloat(data.taxDeducted) < 0) {
	...
	async generateITRJson(
		userId: string,
		financialYear: string,
	): Promise<{ itrJson: string; warnings: string[] }> {
		// Get all structured tax data for the user and financial year
		const taxData = await this.getStructuredTaxDataByUser(
			userId,
			financialYear,
		);
		const warnings: string[] = [];
		...
		const itrJson = {
			...
			income: {
				...
				capitalGains: this.calculateCapitalGains(
					groupedData.capital_gains || [],
				),
				otherSources: this.calculateOtherIncome(
					groupedData.interest || [],
					groupedData.dividend || [],
				),
			},
			deductions: {
			...
			taxComputation: {
				totalIncome: 0,
				taxPayable: 0,
				taxPaid: this.calculateTaxPaid(
					groupedData.TDS || [],
					groupedData.advance_tax || [],
				),
			},
		};

		// Calculate totals
		const totalIncome = Object.values(itrJson.income).reduce(
			(sum: number, val: number) => sum + val,
			0,
		);
		itrJson.taxComputation.totalIncome =
			totalIncome - itrJson.deductions.totalDeductions;
	...
	async calculateTaxLiability(
		...
		const otherIncome = this.calculateOtherIncome(
			groupedData.interest || [],
			groupedData.dividend || [],
		);
		...
		const grossTaxLiability = this.calculateTaxBySlabs(
			taxableIncome,
			taxRegime,
		);
		...
		const advanceTaxPaid = this.calculateTaxPaid(
			[],
			groupedData.advance_tax || [],
		);
	...
	private getAssessmentYear(financialYear: string): string {
		...
		const assessmentYear = Number.parseInt(startYear) + 1;
	...
	private calculateSalaryIncome(salaryData: any[]): number {
		return salaryData.reduce(
			(total, item) => total + (Number.parseFloat(item.taxableAmount) || 0),
			0,
		);
	}

	private calculateCapitalGains(capitalGainsData: any[]): number {
		return capitalGainsData.reduce(
			(total, item) => total + (Number.parseFloat(item.taxableAmount) || 0),
			0,
		);
	}

	private calculateOtherIncome(
		interestData: any[],
		dividendData: any[],
	): number {
		const interest = interestData.reduce(
			(total, item) => total + (Number.parseFloat(item.taxableAmount) || 0),
			0,
		);
		const dividend = dividendData.reduce(
			(total, item) => total + (Number.parseFloat(item.taxableAmount) || 0),
			0,
		);
	...
	private calculateTaxPaid(tdsData: any[], advanceTaxData: any[]): number {
		const tds = tdsData.reduce(
			(total, item) => total + (item.taxDeducted || 0),
			0,
		);
		const advanceTax = advanceTaxData.reduce(
			(total, item) => total + (item.taxDeducted || 0),
			0,
		);
	...
	async createInvestmentIdea(
		idea: InsertInvestmentIdea,
	): Promise<InvestmentIdea> {
		const [result] = await db
			.insert(schema.investmentIdeas)
			.values(idea)
			.returning();
		return result;
	}
	...
	async updateInvestmentIdea(
		id: string,
		updates: Partial<InvestmentIdea>,
	): Promise<InvestmentIdea | undefined> {
		const [result] = await db
	...
	async createInvestmentIdeaTracking(
		tracking: InsertInvestmentIdeaTracking,
	): Promise<InvestmentIdeaTracking> {
		const [result] = await db
			.insert(schema.investmentIdeaTracking)
			.values(tracking)
			.returning();
		return result;
	}

	async getInvestmentIdeaTracking(
		ideaId: string,
	): Promise<InvestmentIdeaTracking[]> {
		return await db
			.select()
	...
	async getLatestIdeaTracking(
		ideaId: string,
	): Promise<InvestmentIdeaTracking | undefined> {
		const [result] = await db
			.select()
			.from(schema.investmentIdeaTracking)
			.where(eq(schema.investmentIdeaTracking.ideaId, ideaId))
	...
	async createInvestmentIdeaAlert(
		alert: InsertInvestmentIdeaAlert,
	): Promise<InvestmentIdeaAlert> {
		const [result] = await db
			.insert(schema.investmentIdeaAlerts)
			.values(alert)
			.returning();
		return result;
	}

	async getInvestmentIdeaAlerts(
		userId: string,
	): Promise<InvestmentIdeaAlert[]> {
		return await db
			.select()
	...
	async markInvestmentIdeaAlertAsRead(
		id: string,
	): Promise<InvestmentIdeaAlert | undefined> {
		const [result] = await db
	...
	async updateYieldTracker(
		id: string,
		updates: Partial<YieldTracker>,
	...
	async createPartnerApplication(
		application: InsertPartnerApplication,
	...
	async getPartnerApplicationsByUserId(
		userId: string,
	): Promise<PartnerApplication[]> {
		return await db
			.select()
	...
	async updatePartnerApplication(
		id: string,
	...
	async getApplicationPrefillData(
		userId: string,
		lender: string,
		...
		const prefillData: any = {
			...
			monthlyIncome: userProfile.annualIncome
				? Math.round(Number.parseFloat(userProfile.annualIncome) / 12)
				: 0,
	...
	async createApplicationDocument(
		document: InsertPartnerApplicationDocument,
	): Promise<PartnerApplicationDocument> {
		const [result] = await db
			.insert(schema.partnerApplicationDocuments)
			.values(document)
			.returning();
		return result;
	}
	...
	async getApplicationDocument(
		id: string,
	): Promise<PartnerApplicationDocument | undefined> {
		const [result] = await db
			.select()
			.from(schema.partnerApplicationDocuments)
	...
	async getApplicationDocumentsByType(
		applicationId: string,
		documentType: string,
	): Promise<PartnerApplicationDocument[]> {
		return await db
	...
	async updateApplicationDocument(
		id: string,
		updates: Partial<PartnerApplicationDocument>,
	): Promise<PartnerApplicationDocument | undefined> {
		const [result] = await db
	...
	async createCashfreeTransaction(
		transaction: InsertCashfreeTransaction,
	): Promise<CashfreeTransaction> {
		const [result] = await db
			.insert(schema.cashfreeTransactions)
			.values(transaction)
			.returning();
		return result;
	}

	async getCashfreeTransaction(
		id: string,
	): Promise<CashfreeTransaction | undefined> {
		const [result] = await db
			.select()
	...
	async getCashfreeTransactionByOrderId(
		orderId: string,
	): Promise<CashfreeTransaction | undefined> {
		const [result] = await db
			.select()
	...
	async updateCashfreeTransaction(
		id: string,
		updates: Partial<CashfreeTransaction>,
	): Promise<CashfreeTransaction | undefined> {
		const [result] = await db
	...
	async getCashfreeTransactionsByUserId(
		userId: string,
	): Promise<CashfreeTransaction[]> {
		return await db
			.select()
	...
	async getCashfreeTransactionsByStatus(
		status: string,
	): Promise<CashfreeTransaction[]> {
		return await db
			.select()
	...
	async createPhonePeTransaction(
		transaction: InsertPhonePeTransaction,
	): Promise<PhonePeTransaction> {
		const [result] = await db
			.insert(schema.phonePeTransactions)
			.values(transaction)
			.returning();
		return result;
	}

	async getPhonePeTransaction(
		id: string,
	): Promise<PhonePeTransaction | undefined> {
		const [result] = await db
			.select()
	...
	async getPhonePeTransactionByOrderId(
		orderId: string,
	): Promise<PhonePeTransaction | undefined> {
		const [result] = await db
			.select()
	...
	async getPhonePeTransactionByMerchantId(
		...
		const [result] = await db
			...
			.where(
				eq(
					schema.phonePeTransactions.merchantTransactionId,
	...
	async updatePhonePeTransaction(
		id: string,
		updates: Partial<PhonePeTransaction>,
	): Promise<PhonePeTransaction | undefined> {
		const [result] = await db
	...
	async getPhonePeTransactionsByUserId(
		userId: string,
	): Promise<PhonePeTransaction[]> {
		return await db
			.select()
	...
	async getPhonePeTransactionsByStatus(
		status: string,
	): Promise<PhonePeTransaction[]> {
		return await db
			.select()
	...
	async upsertTaxRule(rule: InsertTaxRule): Promise<TaxRule> {
		const existingRule = await this.getTaxRule(
			rule.ruleType,
			rule.category,
	...
	async createTaxReminderSubscription(
		subscription: InsertTaxReminderSubscription,
	): Promise<TaxReminderSubscription> {
		const [result] = await db
	...
	async getUserTaxReminderSubscription(
		userId: string,
	): Promise<TaxReminderSubscription | undefined> {
		const [result] = await db
			.select()
	...
	async updateTaxReminderSubscription(
		id: string,
		updates: Partial<TaxReminderSubscription>,
	): Promise<TaxReminderSubscription | undefined> {
		const [result] = await db
	...
	async createCapitalGainsReminder(
		reminder: InsertCapitalGainsTaxReminder,
	): Promise<CapitalGainsTaxReminder> {
		const [result] = await db
	...
	async updateCapitalGainsReminder(
		id: string,
		updates: Partial<CapitalGainsTaxReminder>,
	): Promise<CapitalGainsTaxReminder | undefined> {
		const [result] = await db
	...
	async getUserFamilies(
		userId: string,
	): Promise<(FamilyGroup & { memberCount: number; role: string })[]> {
		const results = await db
			.select({
			...
			.groupBy(schema.familyGroups.id, schema.familyMembers.role);
	...
	async addGoalContribution(
		data: InsertFamilyGoalContribution,
	...
	> {
		const results = await db
			...
			.leftJoin(
				schema.userProfiles,
				eq(schema.familyGoalContributions.userId, schema.userProfiles.userId),
			)
	...
	async logFamilyActivity(
		data: InsertFamilyActivityLog,
	): Promise<FamilyActivityLog> {
		const [result] = await db
	...
	async createDiscussion(
		data: InsertFamilyDiscussion,
	): Promise<FamilyDiscussion> {
		const [result] = await db
	...
	> {
		const results = await db
			...
			.leftJoin(
				schema.userProfiles,
				eq(schema.familyDiscussions.authorId, schema.userProfiles.userId),
			)
			.leftJoin(
	...
	async grantPortfolioPermission(
		data: InsertFamilyPortfolioPermission,
	): Promise<FamilyPortfolioPermission> {
		const [result] = await db
	...
	async checkPortfolioPermission(
		portfolioId: string,
		userId: string,
	): Promise<FamilyPortfolioPermission | undefined> {
		const [result] = await db
			.select()
	...
	async getFamilyDashboardData(
		...
		const portfoliosResult = await db
			...
			.innerJoin(
				schema.portfolios,
				eq(schema.familyMembers.userId, schema.portfolios.userId),
			)
			.where(
				and(
					eq(schema.familyMembers.familyId, familyId),
		...
		const totalNetWorth = portfoliosResult.reduce((sum, p) => {
			const value = Number.parseFloat(p.totalValue || "0");
			return sum + value;
		}, 0);
	...
	async updateUserAlert(
		id: string,
		updates: Partial<InsertUserAlert>,
	): Promise<UserAlert | undefined> {
		const [result] = await db
	...
	async getAlertTemplates(category?: string): Promise<AlertTemplate[]> {
		...
		return await db
			...
			.where(and(...conditions))
			.orderBy(
				desc(schema.alertTemplates.isPopular),
				asc(schema.alertTemplates.templateName),
			);
	}
	...
	async getChatFunction(
		functionName: string,
	): Promise<ChatFunction | undefined> {
		const [result] = await db
			.select()
			.from(schema.chatFunctions)
	...
	async updateChatAction(
		id: string,
		updates: Partial<ChatAction>,
	...
	async updateCurrencyRates(
		...
		for (const [targetCurrency, rate] of Object.entries(rates)) {
			await db
				...
				.onConflictDoUpdate({
					target: [
						schema.currencyRates.baseCurrency,
						schema.currencyRates.targetCurrency,
					],
					set: {
						exchangeRate: rate.toString(),
	...
	async convertPortfolioValue(
		...
		if (baseCurrency === targetCurrency) {
			return Number.parseFloat(portfolio.totalValue || "0");
		}
		...
		return (
			Number.parseFloat(portfolio.totalValue || "0") *
			Number.parseFloat(rate.exchangeRate)
		);
	}
	...
	async getUserFinancialAnalysis(userId: string): Promise<{
		...
		try {
			...
			const hasCompletedKyc =
				ckycRecord?.status === "CKYC_VERIFIED" ||
			...
			for (const portfolio of portfolios) {
				const portfolioValue = Number.parseFloat(portfolio.totalValue || "0");
			...
			const currentInvestments =
				totalInvestedAmount > 0 ? totalInvestedAmount / 12 : 0; // Monthly average
			...
			const obligationRatio =
				monthlyIncome > 0 ? (monthlyObligations / monthlyIncome) * 100 : 0;
	...
	async createExpense(expense: InsertUserExpense): Promise<UserExpense> {
		const [created] = await db
			.insert(schema.userExpenses)
			.values(expense)
			.returning();

		// Update budget spend if expense has category
		if (expense.category && expense.amount) {
			await this.updateUserBudgetSpend(
				expense.userId ?? "",
				expense.category ?? "",
	...
	async getUserExpenses(
		...
		const query = db
			.select()
			.from(schema.userExpenses)
			.where(eq(schema.userExpenses.userId, userId));

		const conditions: any[] = [eq(schema.userExpenses.userId, userId)];

		if (filters.startDate) {
			conditions.push(
				gte(schema.userExpenses.transactionDate, filters.startDate),
			);
		}
		if (filters.endDate) {
			conditions.push(
				lte(schema.userExpenses.transactionDate, filters.endDate),
			);
		}
		if (filters.category) {
		...
		if (filters.minAmount !== undefined) {
			conditions.push(
				gte(schema.userExpenses.amount, filters.minAmount.toString()),
			);
		}
		if (filters.maxAmount !== undefined) {
			conditions.push(
				lte(schema.userExpenses.amount, filters.maxAmount.toString()),
			);
		}
	...
	async updateExpense(
		...
		if (original.category && original.amount) {
			const originalAmount = Number.parseFloat(original.amount.toString());
			const newCategory = updates.category || original.category;
			const newAmount = updates.amount
				? Number.parseFloat(updates.amount.toString())
				: originalAmount;
	...
	async updateBudget(
		id: string,
		updates: Partial<UserBudget>,
	): Promise<UserBudget | undefined> {
		const [updated] = await db
			.update(schema.userBudgets)
			.set({ ...updates, updatedAt: new Date() })
			.where(eq(schema.userBudgets.id, id))
			.returning();
		return updated || undefined;
	...
	async updateUserBudgetSpend(
		...
		for (const budget of budgets) {
			const currentSpend = Number.parseFloat(
				budget.currentSpend?.toString() || "0",
			);
			await db
				.update(schema.userBudgets)
				.set({
			...
			switch (budget.period) {
				...
				case "quarterly": {
					const quarterDiff = Math.floor(
						(now.getMonth() - lastReset.getMonth()) / 3,
					);
	...
	async updateInsight(
		id: string,
		updates: Partial<ExpenseInsight>,
	): Promise<ExpenseInsight | undefined> {
		const [updated] = await db
			.update(schema.expenseInsights)
			.set(updates)
	...
	async getBbpsTransactionByReference(
		orderId: string,
	): Promise<any | undefined> {
	...
	async createKycVerificationSession(
		...
		const [created] = await db
			.insert(schema.kycVerificationSessions)
			.values(session)
			.returning();
		return created;
	}

	async getKycVerificationSession(
		id: string,
	...
	async updateKycVerificationSession(
		id: string,
		updates: Partial<KycVerificationSession>,
	): Promise<KycVerificationSession | undefined> {
		const [updated] = await db
	...
	async createManualKycSubmission(
		submission: InsertManualKycSubmission,
	): Promise<ManualKycSubmission> {
		const [created] = await db
			.insert(schema.manualKycSubmissions)
			.values(submission)
			.returning();
		return created;
	}

	async getManualKycSubmission(
		id: string,
	): Promise<ManualKycSubmission | undefined> {
	...
	async getUserManualKycSubmissions(
		userId: string,
	): Promise<ManualKycSubmission[]> {
		return await db
			.select()
	...
	async getAllManualKycSubmissions(filters?: {
		status?: string;
		...
		if (filters?.applicantType) {
			conditions.push(
				eq(schema.manualKycSubmissions.applicantType, filters.applicantType),
			);
		}
	...
	async updateManualKycSubmission(
		id: string,
		updates: Partial<ManualKycSubmission>,
	): Promise<ManualKycSubmission | undefined> {
		const [updated] = await db
			.update(schema.manualKycSubmissions)
			.set({ ...updates, updatedAt: new Date() })
			.where(eq(schema.manualKycSubmissions.id, id))
			.returning();
		return updated || undefined;
	...
	async reviewManualKycSubmission(
		id: string,
		reviewerId: string,
		status: string,
		notes?: string,
	...
	async createManualKycDocument(
		document: InsertManualKycDocument,
	): Promise<ManualKycDocument> {
		const [created] = await db
			.insert(schema.manualKycDocuments)
			.values(document)
			.returning();
		return created;
	}

	async getManualKycDocuments(
		submissionId: string,
	): Promise<ManualKycDocument[]> {
		return await db
			.select()
	...
	async updateManualKycDocument(
		id: string,
		updates: Partial<ManualKycDocument>,
	): Promise<ManualKycDocument | undefined> {
		const [updated] = await db
			.update(schema.manualKycDocuments)
			.set(updates)
	...
	async getKycDashboardStats(): Promise<{
		...
		const pendingReviews = manualSubmissions.filter(
	...
	async getUnifiedKycSubmissions(filters?: {
		...
		let manualQuery = db
			.select({
				...
				fullName:
					sql<string>`CONCAT(${schema.manualKycSubmissions.firstName}, ' ', ${schema.manualKycSubmissions.lastName})`.as(
						"fullName",
					),
		...
		let ckycQuery = db
			.select({
				...
				fullName:
					sql<string>`CONCAT(${schema.ckycRecords.firstName}, ' ', ${schema.ckycRecords.lastName})`.as(
						"fullName",
					),
		...
		if (filters?.status) {
			manualQuery = manualQuery.where(
				eq(schema.manualKycSubmissions.status, filters.status),
			) as any;
			ckycQuery = ckycQuery.where(
				eq(schema.ckycRecords.status, filters.status),
			) as any;
		}
		...
		if (filters?.dateFrom) {
			...
			manualQuery = manualQuery.where(
				gte(schema.manualKycSubmissions.createdAt, fromDate),
			) as any;
			ckycQuery = ckycQuery.where(
				gte(schema.ckycRecords.createdAt, fromDate),
			) as any;
		}
		...
		if (filters?.dateTo) {
			...
			manualQuery = manualQuery.where(
				lte(schema.manualKycSubmissions.createdAt, toDate),
			) as any;
			ckycQuery = ckycQuery.where(
				lte(schema.ckycRecords.createdAt, toDate),
			) as any;
		}
	...
	async getKycSubmissionDetails(
		...
		const manualSubmission = await db
			.select()
			.from(schema.manualKycSubmissions)
	...
	async getAllKycDocuments(filters?: {
		...
		if (filters?.status) {
			manualDocsQuery = manualDocsQuery.where(
				eq(schema.manualKycDocuments.verificationStatus, filters.status),
			) as any;
			ckycDocsQuery = ckycDocsQuery.where(
				eq(schema.ckycDocuments.verificationStatus, filters.status),
			) as any;
		}

		if (filters?.documentType) {
			manualDocsQuery = manualDocsQuery.where(
				eq(schema.manualKycDocuments.documentType, filters.documentType),
			) as any;
			ckycDocsQuery = ckycDocsQuery.where(
				eq(schema.ckycDocuments.documentType, filters.documentType),
			) as any;
		}
		...
		allDocuments.sort(
			(a, b) =>
				new Date((b as any).uploadedAt || 0).getTime() -
				new Date((a as any).uploadedAt || 0).getTime(),
		);
	...
	async verifyKycDocument(
		...
		const ckycDoc = await db
			.update(schema.ckycDocuments)
			.set({
				verificationStatus: status,
				verifiedAt: new Date(),
			})
	...
	async bulkApproveKycSubmissions(
		...
		for (const subId of submissionIds) {
			try {
				const manualResult = await this.reviewManualKycSubmission(
					subId,
					approverId,
					"approved",
					notes,
				...
				const ckycResult = await db
					.update(schema.ckycRecords)
					.set({
						status: "verified",
						updatedAt: new Date(),
					})
	...
	async bulkRejectKycSubmissions(
		submissionIds: string[],
		rejectorId: string,
		reason: string,
	): Promise<{ success: number; failed: number }> {
	...
	async bulkAssignKycSubmissions(
		submissionIds: string[],
		reviewerId: string,
		assignedBy: string,
	): Promise<{ success: number; failed: number }> {
	...
	async getComplianceAlerts(filters?: {
		...
		if (filters?.dateFrom) {
			conditions.push(
				gte(schema.complianceAuditTrail.createdAt, new Date(filters.dateFrom)),
			);
		}

		if (filters?.dateTo) {
			conditions.push(
				lte(schema.complianceAuditTrail.createdAt, new Date(filters.dateTo)),
			);
		}
		...
		const alerts = (allRecords as any[])
			.filter(
				(r) =>
					r.riskImpact === "high" ||
					r.complianceImpact === "major" ||
					r.complianceImpact === "critical",
			)
			.map((record: any) => ({
				...record,
				severity:
					record.complianceImpact === "critical"
						? "critical"
						: record.riskImpact === "high"
							? "high"
							: "medium",
				status: "pending",
			}));
	...
	async getComplianceAuditTrail(filters?: {
		...
		if (filters?.performedBy) {
			conditions.push(
				eq(schema.complianceAuditTrail.performedBy, filters.performedBy),
			);
		}
	...
	async resolveComplianceAlert(
		...
		const [updated] = await db
			.update(schema.complianceAuditTrail)
			.set({
	...
	async getComplianceStats(): Promise<{
		...
		const alerts = (allRecords as any[]).filter(
			(r) =>
				r.riskImpact === "high" ||
				r.complianceImpact === "major" ||
				r.complianceImpact === "critical",
		...
		const criticalAlerts = alerts.filter(
			(a: any) => a.complianceImpact === "critical",
		).length;
	...
	async getUserKycStatus(userId: string): Promise<any> {
		...
		return {
			...
			kycStatus:
				ckycRecords[0]?.status || manualSubmissions[0]?.status || "not_started",
			ckycRecords,
			manualSubmissions,
	...
	async getFinancialOrdersDashboard() {
		...
		const ordersByProductType = await db
			.select({
	...
	async getUnifiedOrders(filters?: {
		...
		if (filters?.productType) {
			conditions.push(
				eq(schema.unifiedOrders.productType, filters.productType),
			);
		}
		if (filters?.paymentStatus) {
			conditions.push(
				eq(schema.unifiedOrders.paymentStatus, filters.paymentStatus),
			);
		}
		if (filters?.executionStatus) {
			conditions.push(
				eq(schema.unifiedOrders.executionStatus, filters.executionStatus),
			);
		}
		if (filters?.dateFrom) {
			conditions.push(
				gte(schema.unifiedOrders.createdAt, new Date(filters.dateFrom)),
			);
		}
		if (filters?.dateTo) {
			conditions.push(
				lte(schema.unifiedOrders.createdAt, new Date(filters.dateTo)),
			);
		}
		if (filters?.search) {
			conditions.push(sql`(
	...
	async getUnifiedOrderDetails(orderId: string) {
		const [order] = await db
			.select()
			.from(schema.unifiedOrders)
			.where(eq(schema.unifiedOrders.id, orderId));
	...
	async getCashfreeTransactions(filters?: {
		...
		if (filters?.dateFrom) {
			conditions.push(
				gte(schema.cashfreeTransactions.createdAt, new Date(filters.dateFrom)),
			);
		}
		if (filters?.dateTo) {
			conditions.push(
				lte(schema.cashfreeTransactions.createdAt, new Date(filters.dateTo)),
			);
		}
	...
	async getPhonePeTransactions(filters?: {
		...
		if (filters?.dateFrom) {
			conditions.push(
				gte(schema.phonePeTransactions.createdAt, new Date(filters.dateFrom)),
			);
		}
		if (filters?.dateTo) {
			conditions.push(
				lte(schema.phonePeTransactions.createdAt, new Date(filters.dateTo)),
			);
		}
	...
	async getPaymentReconciliation(dateFrom?: string, dateTo?: string) {
		...
		if (dateFrom) {
			cashfreeConditions.push(
				gte(schema.cashfreeTransactions.createdAt, new Date(dateFrom)),
			);
			phonePeConditions.push(
				gte(schema.phonePeTransactions.createdAt, new Date(dateFrom)),
			);
		}
		if (dateTo) {
			cashfreeConditions.push(
				lte(schema.cashfreeTransactions.createdAt, new Date(dateTo)),
			);
			phonePeConditions.push(
				lte(schema.phonePeTransactions.createdAt, new Date(dateTo)),
			);
		}

		const [cashfreeStats] = await db
			.select({
			...
			.from(schema.cashfreeTransactions)
			.where(
				cashfreeConditions.length > 0 ? and(...cashfreeConditions) : undefined,
			);

		const [phonePeStats] = await db
			.select({
			...
			.from(schema.phonePeTransactions)
			.where(
				phonePeConditions.length > 0 ? and(...phonePeConditions) : undefined,
			);

		const cashfreeTotal = Number.parseFloat(cashfreeStats?.total || "0");
		const phonePeTotal = Number.parseFloat(phonePeStats?.total || "0");
		...
		return {
			...
			successfulPayments:
				(cashfreeStats?.successCount || 0) + (phonePeStats?.successCount || 0),
			failedPayments:
				(cashfreeStats?.failedCount || 0) + (phonePeStats?.failedCount || 0),
			pendingPayments:
				(cashfreeStats?.pendingCount || 0) + (phonePeStats?.pendingCount || 0),
		};
	}
	...
	async getRevenueAnalytics(dateFrom?: string, dateTo?: string) {
		...
		const revenueByProductType = await db
			.select({
	...
	async getRefunds(filters?: {
		...
		if (filters?.dateFrom) {
			conditions.push(
				gte(schema.orderRefunds.createdAt, new Date(filters.dateFrom)),
			);
		}
		if (filters?.dateTo) {
			conditions.push(
				lte(schema.orderRefunds.createdAt, new Date(filters.dateTo)),
			);
		}
	...
	async getAgentByEmail(email: string) {
		const [agent] = await db
			.select()
			.from(schema.customerCareAgents)
			.where(eq(schema.customerCareAgents.email, email));
		return agent;
	}
	...
	async getAgentByArn(arnCode: string) {
		const [agent] = await db
			.select()
			.from(schema.customerCareAgents)
			.where(eq(schema.customerCareAgents.arnCode, arnCode));
		return agent;
	}
	...
	async getAgentByEuin(euinNumber: string) {
		const [agent] = await db
			.select()
			.from(schema.customerCareAgents)
			.where(eq(schema.customerCareAgents.euinNumber, euinNumber));
		return agent;
	}
	...
	async getAllAgentsByStatus(status?: string) {
		if (status) {
			return await db
				.select()
				.from(schema.customerCareAgents)
				.where(eq(schema.customerCareAgents.onboardingStatus, status));
		}
	...
	async getSubAgents(masterAgentId: string) {
		return await db
			.select()
			.from(schema.customerCareAgents)
			.where(eq(schema.customerCareAgents.masterAgentId, masterAgentId));
	}
	...
	async getAgentDocuments(agentId: string) {
		return await db
			.select()
			.from(schema.agentDocuments)
			.where(eq(schema.agentDocuments.agentId, agentId));
	}
	...
	async updateAgentDocumentVerification(
		documentId: string,
		status: string,
		verifiedBy: string,
		rejectionReason?: string,
	...
	async createAmfiVerificationLog(log: InsertAmfiVerificationLog) {
		const [created] = await db
			.insert(schema.amfiVerificationLog)
			.values(log)
			.returning();
		return created;
	}
	...
	async getAmfiVerificationLogs(agentId?: string, verificationType?: string) {
		...
		if (verificationType) {
			conditions.push(
				eq(schema.amfiVerificationLog.verificationType, verificationType),
			);
		}
		const whereClause = conditions.length > 0 ? and(...conditions) : undefined;
		return await db
			.select()
			.from(schema.amfiVerificationLog)
			.where(whereClause)
			.orderBy(desc(schema.amfiVerificationLog.createdAt));
	}

	// Agent Commission Splits
	async createCommissionSplit(split: InsertAgentCommissionSplit) {
		const [created] = await db
			.insert(schema.agentCommissionSplits)
			.values(split)
			.returning();
		return created;
	}
	...
	async getCommissionSplits(subAgentId?: string, masterAgentId?: string) {
		...
		if (masterAgentId) {
			conditions.push(
				eq(schema.agentCommissionSplits.masterAgentId, masterAgentId),
			);
		}
		const whereClause = conditions.length > 0 ? and(...conditions) : undefined;
		return await db
			.select()
			.from(schema.agentCommissionSplits)
			.where(whereClause);
	}
	...
	async getActiveCommissionSplit(subAgentId: string, productType?: string) {
		...
		if (productType) {
			conditions.push(
				eq(schema.agentCommissionSplits.productType, productType),
			);
		}
	...
	async updateCommissionSplit(
		splitId: string,
		updates: Partial<AgentCommissionSplit>,
	) {
		const [updated] = await db
			.update(schema.agentCommissionSplits)
			.set({ ...updates, updatedAt: new Date() })
	...
	async getAgentCommissions(
		...
		if (filters?.productType) {
			conditions.push(
				eq(schema.agentCommissions.productType, filters.productType),
			);
		}
		if (filters?.settlementStatus) {
			conditions.push(
				eq(
					schema.agentCommissions.agentSettlementStatus,
	...
	async getMasterAgentCommissions(
		masterAgentId: string,
		filters?: { month?: string; productType?: string },
	) {
		const conditions = [
			eq(schema.agentCommissions.masterAgentId, masterAgentId),
		];
		if (filters?.month) {
			conditions.push(eq(schema.agentCommissions.month, filters.month));
		}
	...
	async updateCommissionSettlementStatus(
		commissionId: string,
	...
	async getCommissionSummary(agentId: string, month?: string) {
		...
		const totalEarned = commissions.reduce(
			(sum, c) => sum + Number.parseFloat(c.agentNetCommission.toString()),
			0,
		);
	...
	async getAgentReferralStats(agentId: string) {
		...
		const activeClients = (relationships as any[]).filter(
			(r) => r.status === "active",
		).length;
		const totalReferrals = relationships.length;
		const conversionRate =
			totalReferrals > 0 ? (activeClients / totalReferrals) * 100 : 0;
	...
	async getReferredClients(agentId: string) {
		...
		const clientsWithDetails = await Promise.all(
			relationships.map(async (rel) => {
				...
				const clientCommissions = await db
					.select()
					.from(schema.agentCommissions)
					.where(
						and(
				...
				const totalEarnings = clientCommissions.reduce(
					(sum, c) => sum + Number.parseFloat(c.agentNetCommission.toString()),
					0,
				);
	...
	async getAgentEarnings(agentId: string) {
		...
		const earningsWithDetails = await Promise.all(
			commissions.map(async (comm) => {
				const [client] = comm.clientId
					? await db
							.select()
							.from(schema.users)
							.where(eq(schema.users.id, comm.clientId))
					: [null];

				const grossCommission = Number.parseFloat(
					(comm as any).grossCommission?.toString() ?? "0",
				);
				const agentNetCommission = Number.parseFloat(
					comm.agentNetCommission.toString(),
				);
				const tdsAmount = Number.parseFloat(
					(comm as any).tdsAmount?.toString() ?? "0",
				);
				...
				return {
					...
					transactionAmount: Number.parseFloat(
						comm.transactionAmount.toString(),
					),
					commissionRate: Number.parseFloat(
						(comm as any).commissionRate?.toString() ?? "0",
					),
	...
	async createClientReferral(data: {
		...
		if (existingRelationship.length > 0) {
			const [updated] = await db
				...
				.where(
					eq(schema.clientAgentRelationships.id, existingRelationship[0].id),
				)
				.returning();
			return updated;
		}
	...
	async getPreApprovedLoanOffers(userId: string): Promise<any[]> {
		const offers = await db
			...
			.orderBy(
				desc(schema.preApprovedLoanOffers.displayPriority),
				desc(schema.preApprovedLoanOffers.createdAt),
			);
	...
	async createPreApprovedLoanOffer(offer: any): Promise<any> {
		const [newOffer] = await db
			.insert(schema.preApprovedLoanOffers)
			.values({
				...offer,
				id: randomUUID(),
				createdAt: new Date(),
	...
	async updateLoanOfferApplicationStatus(
		offerId: string,
		status: string,
		applicationData?: any,
	): Promise<any | undefined> {
		...
		if (applicationData) {
			if (applicationData.applicationId)
				updateData.applicationId = applicationData.applicationId;
			if (status === "in_progress" && !applicationData.appliedAt)
				updateData.appliedAt = new Date();
			if (status === "approved") updateData.approvedAt = new Date();
			if (status === "disbursed") {
				updateData.disbursedAt = new Date();
				if (applicationData.disbursedAmount)
					updateData.disbursedAmount = applicationData.disbursedAmount;
			}
		}

		const [updated] = await db
			.update(schema.preApprovedLoanOffers)
			.set(updateData)
	...
	async getAllStoreCategories(): Promise<any[]> {
		const categories = await db
			...
			.orderBy(
				asc(schema.storeCategories.displayOrder),
				asc(schema.storeCategories.name),
			);
		return categories;
	}

	async updateStoreProductStatus(
		productId: string,
		isActive: boolean,
	): Promise<any | undefined> {
		const [updated] = await db
	...
	async updateStoreCategoryStatus(
		categoryId: string,
		isActive: boolean,
	): Promise<any | undefined> {
		const [updated] = await db
	...
	async getStoreProductBySourceCompanyId(
	...
	async seedDefaultStoreCategories(): Promise<void> {
		...
		if (existingCategories.length > 0) {
			console.log(
				`✅ Store categories already seeded (${existingCategories.length} categories exist)`,
			);
			return;
		}
		...
		const defaultCategories = [
			{
				id: "cat-mutual-funds",
				name: "Mutual Funds - Regular Schemes",
				slug: "mf-regular",
				icon: "TrendingUp",
				displayOrder: 1,
				isActive: true,
			...
			{
				id: "cat-mf-direct",
				name: "Mutual Funds - Direct Schemes",
				slug: "mf-direct",
				icon: "TrendingUp",
				displayOrder: 2,
				isActive: false,
		...
		console.log(
			`✅ Seeded ${defaultCategories.length} default store categories`,
		);
	}
	...
	async getAllStoreSubcategories(): Promise<any[]> {
		const subcategories = await db
			...
			.orderBy(
				asc(schema.storeSubcategories.displayOrder),
				asc(schema.storeSubcategories.name),
			);
	...
	async createStoreSubcategory(data: any): Promise<any> {
		const [subcategory] = await db
			.insert(schema.storeSubcategories)
			.values({
				...data,
				id: randomUUID(),
				createdAt: new Date(),
	...
	async updateStoreSubcategory(
		...
		const [updated] = await db
			.update(schema.storeSubcategories)
			.set({
				...data,
				updatedAt: new Date(),
			})
	...
	async updateStoreSubcategoryStatus(
		id: string,
		isActive: boolean,
	): Promise<any | undefined> {
		const [updated] = await db
	...
	async deleteStoreSubcategory(id: string): Promise<boolean> {
		await db
			.delete(schema.storeSubcategories)
			.where(eq(schema.storeSubcategories.id, id));
		return true;
	}
	...
	async getStoreProductsByCategory(categoryId: string): Promise<any[]> {
		const products = await db
			...
			.orderBy(
				asc(schema.storeProducts.displayOrder),
				asc(schema.storeProducts.name),
			);
		return products;
	}
	...
	async getStoreAuditLogs(filters?: {
		targetType?: string;
		targetId?: string;
		adminId?: string;
		limit?: number;
		...
		const logs = await query
			.orderBy(desc(schema.storeAuditLogs.timestamp))
			.limit(filters?.limit || 100);
		return logs;
	}
	...
	async createStoreProductInquiry(data: any): Promise<any> {
		const [inquiry] = await db
			.insert(schema.storeProductInquiries)
			.values({
				...data,
				id: randomUUID(),
				createdAt: new Date(),
	...
	async getStoreProductInquiries(filters?: {
		status?: string;
		productId?: string;
		categoryId?: string;
	}): Promise<any[]> {
		...
		if (filters?.productId) {
			conditions.push(
				eq(schema.storeProductInquiries.productId, filters.productId),
			);
		}
		if (filters?.categoryId) {
			conditions.push(
				eq(schema.storeProductInquiries.categoryId, filters.categoryId),
			);
		}
		...
		const inquiries = await query.orderBy(
			desc(schema.storeProductInquiries.createdAt),
		);
		return inquiries;
	}

	async updateStoreProductInquiry(
		id: string,
		data: any,
	): Promise<any | undefined> {
		const [updated] = await db
			.update(schema.storeProductInquiries)
			.set(data)
	...
	async toggleCategoryWithCascade(
		categoryId: string,
		isActive: boolean,
		adminId: string,
		adminEmail: string,
	): Promise<{ category: any; subcategories: any[]; products: any[] }> {
		...
		// Update category
		const updatedCategory = await this.updateStoreCategoryStatus(
			categoryId,
			isActive,
		);
		...
		const subcategories =
			await this.getStoreSubcategoriesByCategory(categoryId);
		...
		for (const subcat of subcategories) {
			...
			const updated = await this.updateStoreSubcategoryStatus(
				subcat.id,
				isActive,
			);
	...
	async toggleSubcategoryWithCascade(
		subcategoryId: string,
		isActive: boolean,
		adminId: string,
		adminEmail: string,
	): Promise<{ subcategory: any; products: any[] }> {
		...
		// Update subcategory
		const updatedSubcategory = await this.updateStoreSubcategoryStatus(
			subcategoryId,
			isActive,
		);
	...
	async getPortfolioPredictions(
		...
		const predictions = await query.orderBy(
			desc(schema.portfolioPredictions.predictionDate),
		);
		return predictions;
	}
	...
	async getAssetForecasts(userId: string, holdingId?: string): Promise<any[]> {
		...
		const forecasts = await query.orderBy(
			desc(schema.assetForecasts.forecastDate),
		);
		return forecasts;
	}
	...
	async getRiskAnalysis(userId: string, portfolioId?: string): Promise<any[]> {
		...
		const analysis = await query.orderBy(
			desc(schema.riskAnalysis.analysisDate),
		);
		return analysis;
	}
	...
	async createPortfolioPrediction(prediction: any): Promise<any> {
		const [newPrediction] = await db
			.insert(schema.portfolioPredictions)
			.values(prediction)
			.returning();
	...
	async createPredictionAccuracy(accuracy: any): Promise<any> {
		const [newAccuracy] = await db
			.insert(schema.predictionAccuracy)
			.values(accuracy)
			.returning();
		return newAccuracy;
	}
	...
	// Unlisted Companies
	async createUnlistedCompany(
		data: InsertUnlistedCompany,
	): Promise<UnlistedCompany> {
		const [company] = await db
			.insert(schema.unlistedCompanies)
			.values(data)
			.returning();
		return company;
	}
	...
	async getUnlistedCompanyByName(
		name: string,
	): Promise<UnlistedCompany | null> {
		const [company] = await db
			.select()
			.from(schema.unlistedCompanies)
	...
	async getUnlistedCompanyByISIN(
		isin: string,
	): Promise<UnlistedCompany | null> {
		const [company] = await db
			.select()
	...
	async getAllUnlistedCompanies(filters?: {
		status?: string;
		sector?: string;
		...
		if (filters?.storePublishedOnly) {
			...
			const storeLinkedCompanyIds = await db
				.select({ sourceCompanyId: schema.storeProducts.sourceCompanyId })
				.from(schema.storeProducts)
				.where(
					and(
			...
			const linkedIds = storeLinkedCompanyIds
		...
		const companies = await query.orderBy(
			desc(schema.unlistedCompanies.createdAt),
		);
		return companies;
	}

	async updateUnlistedCompany(
		id: string,
		data: Partial<InsertUnlistedCompany>,
	): Promise<UnlistedCompany> {
		const [updated] = await db
			.update(schema.unlistedCompanies)
			.set({ ...data, updatedAt: new Date() })
			.where(eq(schema.unlistedCompanies.id, id))
			.returning();
		return updated;
	...
	async deleteUnlistedCompany(id: string): Promise<boolean> {
		await db
			.delete(schema.companyFinancials)
			.where(eq(schema.companyFinancials.companyId, id));
		await db
			.delete(schema.companyRatios)
			.where(eq(schema.companyRatios.companyId, id));
		await db
			.delete(schema.unlistedPriceHistory)
			.where(eq(schema.unlistedPriceHistory.companyId, id));
		await db
			.delete(schema.sellListings)
			.where(eq(schema.sellListings.companyId, id));
		await db
			.delete(schema.buyRequests)
			.where(eq(schema.buyRequests.companyId, id));
		await db
			.delete(schema.probe42SyncLog)
			.where(eq(schema.probe42SyncLog.companyId, id));
		const result = await db
			.delete(schema.unlistedCompanies)
			.where(eq(schema.unlistedCompanies.id, id));
		return true;
	}

	// Company Financials
	async createCompanyFinancials(
		data: InsertCompanyFinancials,
	): Promise<CompanyFinancials> {
		const [financials] = await db
			.insert(schema.companyFinancials)
			.values(data)
			.returning();
		return financials;
	}
	...
	async getCompanyFinancialsByYear(
		companyId: string,
		financialYear: string,
	...
	async updateCompanyFinancials(
		id: string,
		data: Partial<InsertCompanyFinancials>,
	): Promise<CompanyFinancials> {
	...
	async updateCompanyRatios(
		id: string,
		data: Partial<InsertCompanyRatios>,
	): Promise<CompanyRatios> {
		const { companyId, financialYear, ...mutableData } = data;
		const [updated] = await db
			.update(schema.companyRatios)
			.set({ ...mutableData, updatedAt: new Date() })
	...
	async createPriceHistory(
		data: InsertUnlistedPriceHistory,
	): Promise<UnlistedPriceHistory> {
		const [priceHistory] = await db
			.insert(schema.unlistedPriceHistory)
			.values(data)
			.returning();
		return priceHistory;
	}

	async getPriceHistoryByDate(
		companyId: string,
		date: Date,
	): Promise<UnlistedPriceHistory | null> {
		const [priceHistory] = await db
			.select()
	...
	async upsertPriceHistory(
		...
		const existing = await this.getPriceHistoryByDate(
			data.companyId,
			data.date,
		);
		if (existing) {
			const [updated] = await db
				.update(schema.unlistedPriceHistory)
				.set({
					price: data.price,
					volume: data.volume,
	...
	async updateSellListing(
		id: string,
		data: Partial<InsertSellListing>,
	): Promise<SellListing> {
		const [updated] = await db
			.update(schema.sellListings)
			.set({ ...data, updatedAt: new Date() })
			.where(eq(schema.sellListings.id, id))
			.returning();
		return updated;
	...
	async updateUnlistedDeal(
		id: string,
		data: Partial<InsertUnlistedDeal>,
	): Promise<UnlistedDeal> {
		const [updated] = await db
	...
	async getUnlistedDealsPendingAcceptance(
		userId: string,
	): Promise<UnlistedDeal[]> {
		const deals = await db
			.select()
	...
	// Probe42 Sync Log
	async createProbe42SyncLog(
		data: InsertProbe42SyncLog,
	): Promise<Probe42SyncLog> {
		const [log] = await db
			.insert(schema.probe42SyncLog)
			.values(data)
			.returning();
		return log;
	}
	...
	async createSupportTemplate(
		data: InsertSupportTemplate,
	): Promise<SupportTemplate> {
		const id = randomUUID();
		const [template] = await db
			.insert(schema.supportTemplates)
	...
	async updateSupportTemplate(
		id: string,
		data: Partial<InsertSupportTemplate>,
	): Promise<SupportTemplate | null> {
		const [updated] = await db
			.update(schema.supportTemplates)
			.set({ ...data, updatedAt: new Date() })
			.where(eq(schema.supportTemplates.id, id))
			.returning();
		return updated || null;
	...
	async deleteSupportTemplate(id: string): Promise<boolean> {
		const [deleted] = await db
			.delete(schema.supportTemplates)
			.where(eq(schema.supportTemplates.id, id))
			.returning();
		return !!deleted;
	}
	...
	async createSupportStep(data: InsertSupportStep): Promise<SupportStep> {
		const id = randomUUID();
		const [step] = await db
			.insert(schema.supportSteps)
			.values({ ...data, id, createdAt: new Date(), updatedAt: new Date() })
			.returning();
		return step;
	}

	async getSupportStepsByTemplateId(
		templateId: string,
	): Promise<SupportStep[]> {
		return await db
			.select()
	...
	async updateSupportStep(
		id: string,
		data: Partial<InsertSupportStep>,
	): Promise<SupportStep | null> {
		const [updated] = await db
			.update(schema.supportSteps)
			.set({ ...data, updatedAt: new Date() })
			.where(eq(schema.supportSteps.id, id))
			.returning();
		return updated || null;
	...
	async createSupportStepComment(
		data: InsertSupportStepComment,
	): Promise<SupportStepComment> {
		const id = randomUUID();
		const [comment] = await db
			.insert(schema.supportStepComments)
	...
	async deleteSupportStepComment(id: string): Promise<boolean> {
		const [deleted] = await db
			.delete(schema.supportStepComments)
	...
	async createUnifiedCartItem(
		item: InsertUnifiedCartItem,
	): Promise<UnifiedCartItem> {
		const [newItem] = await db
			.insert(schema.unifiedCartItems)
			.values({
				...item,
				createdAt: new Date(),
				updatedAt: new Date(),
	...
	async updateUnifiedCartItem(
		id: string,
		updates: Partial<UnifiedCartItem>,
	): Promise<UnifiedCartItem | undefined> {
		const [updated] = await db
			.update(schema.unifiedCartItems)
			.set({ ...updates, updatedAt: new Date() })
			.where(eq(schema.unifiedCartItems.id, id))
			.returning();
		return updated || undefined;
	...
	async deleteUnifiedCartItem(id: string): Promise<boolean> {
		const [deleted] = await db
			.delete(schema.unifiedCartItems)
			.where(eq(schema.unifiedCartItems.id, id))
			.returning();
		return !!deleted;
	}

	async getUnifiedCartByCategory(
		userId: string,
		category: string,
	): Promise<UnifiedCartItem[]> {
		return await db
	...
	async getAllUnifiedCartItemsForAdmin(filters?: {
		userId?: string;
		category?: string;
		source?: string;
		status?: string;
		...
		if (filters?.category) {
			conditions.push(
				eq(schema.unifiedCartItems.productCategory, filters.category),
			);
		}
		if (filters?.source) {
	...
	async checkoutCartItems(
		...
		for (const cartItemId of cartItemIds) {
			...
			const [order] = await db
				.insert(schema.unifiedOrders)
				.values({
					orderNumber,
					userId: cartItem.userId,
					productType:
						productTypeMap[cartItem.productCategory] ||
						cartItem.productCategory,
					productId:
						cartItem.storeProductId ||
						cartItem.unlistedCompanyId ||
						cartItem.mutualFundSchemeCode ||
						cartItem.bondIsin ||
						cartItem.ncdIsin ||
						cartItem.ipoId ||
						undefined,
					productName:
						cartItem.displayName || `${cartItem.productCategory} Item`,
					orderType: "buy",
	...
	async createAgentNotification(data: {
		agentId: string;
		title: string;
		body: string;
		type?: string;
	...
	async getPortfolioSnapshotByDate(
		portfolioId: string,
		date: string,
	): Promise<any | undefined> {
	...
	async getComprehensiveHoldings(
		portfolioId: string,
		date?: string,
	): Promise<any[]> {
	...
	async updateComprehensiveHolding(
		id: string,
		updates: any,
	): Promise<any | undefined> {
	...
	async getComprehensiveHoldingsByUser(
		userId: string,
		date?: string,
	): Promise<any[]> {
	...
	async populatePortfolioFromCams(
		userId: string,
		panNumber: string,
		date: string,
	): Promise<any[]> {
	...
	async populatePortfolioFromKfintech(
		userId: string,
		panNumber: string,
		date: string,
	): Promise<any[]> {
	...
	async populatePortfolioFromNsdl(
		userId: string,
	...
	async populatePortfolioFromCdsl(
		userId: string,
		boId: string,
		date: string,
	): Promise<any[]> {
	...
	async populateGovernmentSchemeHoldings(
		userId: string,
		date: string,
	): Promise<any[]> {
	...
	async createCkycNotificationTrigger(
		trigger: InsertCkycNotificationTrigger,
	): Promise<CkycNotificationTrigger> {
		throw new Error("Method not implemented: createCkycNotificationTrigger");
	}

	async updateCkycNotificationStatus(
		id: string,
		status: string,
		sentAt?: Date,
		failureReason?: string,
	): Promise<CkycNotificationTrigger | undefined> {
	...
	async getFundsCacheStats(): Promise<{
		totalCount: number;
		staleCount: number;
		lastUpdated: Date;
	}> {
	...
	async getTaxSessionByPanAndYear(
		userId: string,
		panNumber: string,
		assessmentYear: string,
	): Promise<TaxSession | undefined> {
		throw new Error("Method not implemented: getTaxSessionByPanAndYear");
	}

	async updateTaxSession(
		id: string,
	...
	async createTaxDataSource(
		dataSource: InsertTaxDataSource,
	): Promise<TaxDataSource> {
		throw new Error("Method not implemented: createTaxDataSource");
	}

	async updateTaxDataSource(
		id: string,
		updates: Partial<TaxDataSource>,
	): Promise<TaxDataSource | undefined> {
	...
	async getValidationIssuesBySection(
		sessionId: string,
		section: string,
	): Promise<ValidationIssue[]> {
	...
	async getFilingRecordByAckNumber(
		acknowledgmentNumber: string,
	): Promise<FilingRecord | undefined> {
	...
	async getAiOptimizationSuggestion(
		id: string,
	): Promise<AiOptimizationSuggestion | undefined> {
		throw new Error("Method not implemented: getAiOptimizationSuggestion");
	}

	async createAiOptimizationSuggestion(
		suggestion: InsertAiOptimizationSuggestion,
	): Promise<AiOptimizationSuggestion> {
		throw new Error("Method not implemented: createAiOptimizationSuggestion");
	}

	async updateAiOptimizationSuggestion(
		id: string,
		updates: Partial<AiOptimizationSuggestion>,
	): Promise<AiOptimizationSuggestion | undefined> {
