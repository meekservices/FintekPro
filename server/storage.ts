...
export interface IStorage {
  ...
  updateRebalanceExecution(id: string, updates: Partial<RebalanceExecution>): Promise<RebalanceExecution | undefined>;
  
  getRebalanceExecution(id: string): Promise<RebalanceExecution | undefined>;
  
  getRebalanceExecutionsByPortfolio(portfolioId: string): Promise<RebalanceExecution[]>;
  
  createRebalanceTransaction(transaction: InsertRebalanceTransaction): Promise<RebalanceTransaction>;
  
  updateRebalanceTransaction(id: string, updates: Partial<RebalanceTransaction>): Promise<RebalanceTransaction | undefined>;
  
  getRebalanceTransactionsByExecution(executionId: string): Promise<RebalanceTransaction[]>;
  
  getRebalancingPreferences(userId: string): Promise<RebalancingPreferences | undefined>;
  
  upsertRebalancingPreferences(preferences: InsertRebalancingPreferences): Promise<RebalancingPreferences>;
  ...
  // Production KYC methods
  getUserPanDetails(userId: string, panNumber: string): Promise<any | undefined>;
  saveUserPanDetails(data: { userId: string; panNumber: string; fullName: string; dateOfBirth: string; panType: string; verified: boolean }): Promise<any>;
  getLatestProductionKycSession(userId: string): Promise<any | undefined>;
  cancelProductionKycSession(sessionId: string, userId: string): Promise<void>;
  createProductionKycSession(data: any): Promise<any>;
  getProductionKycSession(sessionId: string, userId: string): Promise<any | undefined>;
  updateProductionKycSession(sessionId: string, updates: any): Promise<any | undefined>;
...
export class DatabaseStorage implements IStorage, IKycStorage {
  ...
  async createRebalanceExecution(execution: InsertRebalanceExecution): Promise<RebalanceExecution> {
  
    const [result] = await db.insert(schema.rebalanceExecutions).values(execution).returning();
  ...
  async updateRebalanceExecution(id: string, updates: Partial<RebalanceExecution>): Promise<RebalanceExecution | undefined> {
  
    const [result] = await db
  
      .update(schema.rebalanceExecutions)
  
      .set({ ...updates, updatedAt: sql`now()` })
  
      .where(eq(schema.rebalanceExecutions.id, id))
  ...
  async getRebalanceExecution(id: string): Promise<RebalanceExecution | undefined> {
  
    const [result] = await db
  
      .select()
  
      .from(schema.rebalanceExecutions)
  ...
  async getRebalanceExecutionsByPortfolio(portfolioId: string): Promise<RebalanceExecution[]> {
    ...
    return await db
      ...
      .where(eq(schema.rebalanceExecutions.portfolioId, portfolioId))
  
      .orderBy(desc(schema.rebalanceExecutions.executionDate));
  
  }
  

  
  async createRebalanceTransaction(transaction: InsertRebalanceTransaction): Promise<RebalanceTransaction> {
  
    const [result] = await db.insert(schema.rebalanceTransactions).values(transaction).returning();
  ...
  async updateRebalanceTransaction(id: string, updates: Partial<RebalanceTransaction>): Promise<RebalanceTransaction | undefined> {
  
    const [result] = await db
  
      .update(schema.rebalanceTransactions)
  
      .set(updates)
  
      .where(eq(schema.rebalanceTransactions.id, id))
  ...
  async getRebalanceTransactionsByExecution(executionId: string): Promise<RebalanceTransaction[]> {
  
    return await db
  
      .select()
  
      .from(schema.rebalanceTransactions)
  
      .where(eq(schema.rebalanceTransactions.rebalanceExecutionId, executionId));
  
  }
  

  
  async getRebalancingPreferences(userId: string): Promise<RebalancingPreferences | undefined> {
  
    const [result] = await db
  
      .select()
  
      .from(schema.rebalancingPreferences)
  
      .where(eq(schema.rebalancingPreferences.userId, userId));
  ...
  async upsertRebalancingPreferences(preferences: InsertRebalancingPreferences): Promise<RebalancingPreferences> {
  
    const [result] = await db
  
      .insert(schema.rebalancingPreferences)
  
      .values(preferences)
  
      .onConflictDoUpdate({
  
        target: schema.rebalancingPreferences.userId,
  ...
  // Production KYC Storage delegation
  async getUserPanDetails(userId: string, panNumber: string) {
    return productionKycStorage.getUserPanDetails(userId, panNumber);
  }
  ...
  async saveUserPanDetails(data: {
    ...
    return productionKycStorage.saveUserPanDetails(data);
  }

  async getLatestProductionKycSession(userId: string) {
    return productionKycStorage.getLatestProductionKycSession(userId);
  }

  async cancelProductionKycSession(sessionId: string, userId: string) {
    return productionKycStorage.cancelProductionKycSession(sessionId, userId);
  }

  async createProductionKycSession(data: any) {
    return productionKycStorage.createProductionKycSession(data);
  }

  async getProductionKycSession(sessionId: string, userId: string) {
    return productionKycStorage.getProductionKycSession(sessionId, userId);
  }

  async updateProductionKycSession(sessionId: string, updates: any) {
    return productionKycStorage.updateProductionKycSession(sessionId, updates);
  }
}
