...
export class OrderManagementService {
	...
	async updateOrderStatus(request: UpdateOrderStatusRequest) {
		...
		if (request.paymentTransactionId)
			updates.paymentTransactionId = request.paymentTransactionId;
		if (request.paymentAmount)
			updates.paymentAmount = request.paymentAmount.toString();
		...
		if (request.executionStatus)
			updates.executionStatus = request.executionStatus;
		if (request.externalOrderId)
			updates.externalOrderId = request.externalOrderId;
		if (request.externalReference)
			updates.externalReference = request.externalReference;
		if (request.executionPrice)
			updates.executionPrice = request.executionPrice.toString();
		if (request.executedQuantity)
			updates.executedQuantity = request.executedQuantity.toString();
		if (request.settlementStatus)
			updates.settlementStatus = request.settlementStatus;
		if (request.settlementDate) updates.settlementDate = request.settlementDate;
		if (request.settlementReference)
			updates.settlementReference = request.settlementReference;
	...
	private getEventName(previousState: any, newState: any): string {
		...
		if (previousState.executionStatus !== newState.executionStatus) {
			if (newState.executionStatus === "in_progress")
		...
		if (previousState.settlementStatus !== newState.settlementStatus) {
			if (newState.settlementStatus === "completed")
	...
	async cancelOrder(orderId: string, reason: string, userId: string) {
		...
		if (
			["executed", "settled", "completed"].includes(order.executionStatus || "")
		) {
