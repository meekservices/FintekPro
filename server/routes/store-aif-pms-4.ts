...
router.get("/inquiries", requireAdmin, async (req, res) => {
	try {
		...
		if (productType && productType !== "all") {
			conditions.push(
				eq(investmentInquiries.productType, productType as string),
			);
		}
...
router.get("/fund-managers", requireAdmin, async (req, res) => {
	try {
		...
		const managers = await db
			...
			.orderBy(
				sortBy === "experience"
					? desc(fundManagers.experienceYears)
					: asc(fundManagers.name),
			)
			.limit(Number.parseInt(limit as string))
			.offset(offset);
...
router.get("/fund-managers/:id/other-funds", async (req, res) => {
	try {
		...
		pmsFunds.forEach((fund) => {
			if (excludeFundId && fundType === "pms" && fund.id === excludeFundId)
				return;
