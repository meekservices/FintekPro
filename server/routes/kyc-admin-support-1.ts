...
export function registerKYCAdminSupporPart1Routes(app: Express): void {
	...
	app.get("/api/admin/financial/dashboard", requireAdmin, async (req, res) => {
		try {
			...
			const [{ count: completedOrders }] = await db
				.select({ count: sql<number>`count(*)` })
				.from(schema.unifiedOrders)
			...
			const productTypeCounts = await db
				.select({
	...
	app.get("/api/admin/financial/orders", requireAdmin, async (req, res) => {
		try {
			...
			const filters: any[] = [];
			if (status)
				filters.push(eq(schema.unifiedOrders.status, status as string));
			if (productType)
				filters.push(
					eq(schema.unifiedOrders.productType, productType as string),
				);
			if (paymentStatus)
				filters.push(
					eq(schema.unifiedOrders.paymentStatus, paymentStatus as string),
				);
			if (search) {
				filters.push(
					or(
	...
	app.get("/api/admin/financial/orders/:id", requireAdmin, async (req, res) => {
		try {
			...
			const [order] = await db
				.select()
				.from(schema.unifiedOrders)
				.where(eq(schema.unifiedOrders.id, id));
			if (!order) {
				return res
					.status(404)
	...
	app.get(
		"/api/admin/financial/cashfree-transactions",
		requireAdmin,
		async (req, res) => {
			try {
				...
				if (status) {
					query = query.where(
						eq(schema.cashfreeTransactions.status, status as string),
					) as any;
				}
	...
	app.get(
		"/api/admin/financial/phonepe-transactions",
		requireAdmin,
		async (req, res) => {
			try {
				...
				if (state) {
					query = query.where(
						eq(schema.phonePeTransactions.state, state as string),
					) as any;
				}
				const transactions = await query
	...
	app.get(
		"/api/admin/financial/payment-reconciliation",
		requireAdmin,
		async (req, res) => {
			try {
				// Get counts by payment status
				const orderPaymentStats = await db
					.select({
					...
					.from(schema.unifiedOrders)
					.groupBy(schema.unifiedOrders.paymentStatus);

				// Get Cashfree totals
				const [cashfreeTotals] = await db
					.select({
						count: sql<number>`count(*)`,
						total: sql<string>`COALESCE(SUM(CAST(amount AS DECIMAL)), 0)`,
					})
					.from(schema.cashfreeTransactions)
					.where(eq(schema.cashfreeTransactions.status, "success"));

				// Get PhonePe totals
				const [phonePeTotals] = await db
					.select({
						count: sql<number>`count(*)`,
						total: sql<string>`COALESCE(SUM(CAST(amount AS DECIMAL)), 0)`,
					})
					.from(schema.phonePeTransactions)
					.where(eq(schema.phonePeTransactions.state, "COMPLETED"));
				...
				res.json({
					...
					data: {
						...
						cashfree: {
							count: Number(cashfreeTotals?.count) || 0,
							total: cashfreeTotals?.total || "0",
						},
						phonePe: {
							count: Number(phonePeTotals?.count) || 0,
							total: phonePeTotals?.total || "0",
						},
					},
				});
			} catch (error) {
		...
		async (req, res) => {
			try {
				...
				res.json({
					...
					data: {
						last30Days: {
							total: last30Days?.total || "0",
							count: Number(last30Days?.count) || 0,
						},
						last7Days: {
							total: last7Days?.total || "0",
							count: Number(last7Days?.count) || 0,
						},
	...
	app.post(
		"/api/admin/financial/refunds/initiate",
		requireAdmin,
		async (req: any, res) => {
			try {
    ...
    `);
			...
			const duplicateEmails = await Promise.all(
				emailDuplicates.rows.map(async (dup) => {
					const users = await db
						...
						.from(schema.users)
						.where(eq(schema.users.email, dup.email));
			...
			const duplicateMobiles = await Promise.all(
				mobileDuplicates.rows.map(async (dup) => {
					const users = await db
						...
						.from(schema.users)
						.where(eq(schema.users.mobile, dup.mobile));
			...
			duplicateEmails.forEach((de) =>
				de.users.forEach((u) => affectedUserIds.add(u.id)),
			);
			duplicateMobiles.forEach((dm) =>
				dm.users.forEach((u) => affectedUserIds.add(u.id)),
			);
	...
	app.get("/api/kyc/manual-submissions", async (req: any, res) => {
		try {
			...
			const submissions = await storage.getUserManualKycSubmissions(
				req.user.id,
			);
	...
	app.get(
		"/api/admin/kyc/manual-submissions",
		requireAdmin,
		async (req, res) => {
			try {
