...
router.post("/prospects/:id/holdings", async (req: Request, res: Response) => {
	try {
		...
		const updatedHoldings = [
			...currentHoldings,
			{ ...newHolding, addedAt: new Date().toISOString() },
		];
...
router.put(
	"/prospects/:id/holdings/:holdingIndex",
	async (req: Request, res: Response) => {
		try {
			...
			const holdingIndex = Number.parseInt(req.params.holdingIndex);
			const currentHoldings = (prospect.currentPortfolio as any[]) || [];
	...
	async (req: Request, res: Response) => {
		try {
			...
			const getHoldingKey = (h: any): string => {
				...
				const type = (h.productType || h.assetType || "mutual_fund")
					.trim()
					.toLowerCase();
