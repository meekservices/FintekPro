		...
		try {
			...
			if (
				...
				res.status(400).json({
					success: false,
					error:
						"Missing required fields: clientId, advisorId, recommendationType, action, rationale",
				});
				return;
			}
...
router.post(
	...
	async (req: Request, res: Response): Promise<void> => {
		try {
			...
			logger.error("[AdvisoryCompliance] Acknowledgment failed", {
 ...
 */
router.get(
	"/consent/check/:clientId",
	async (req: Request, res: Response): Promise<void> => {
		try {
			const { clientId } = req.params;
			const result =
				investmentAdvisoryCompliance.checkRequiredConsents(clientId);
...
router.post(
	...
	async (req: Request, res: Response): Promise<void> => {
		try {
			...
			const declaration =
				investmentAdvisoryCompliance.createSuitabilityDeclaration({
					clientId,
					advisorId,
					proposalId,
		...
		try {
			...
			if (!clientId || !proposalId || !products || !Array.isArray(products)) {
				res.status(400).json({
					success: false,
					error:
						"Missing required fields: clientId, proposalId, products (array)",
				});
				return;
			}
			...
			disclosures: [
				{
					...
					content:
						"Investments in securities market are subject to market risks. There is no assurance or guarantee that the objectives of any of the Schemes will be achieved. Past performance of the Sponsor/AMC/Mutual Fund is not necessarily indicative of future results.",
					regulatoryReference:
						"SEBI Circular CIR/OIAE/1/2015 dated 23-Mar-2015",
					mandatory: true,
				},
				{
					...
					content:
						"FintekPro is a SEBI Registered Investment Adviser under SEBI (Investment Advisers) Regulations, 2013. All investment advice is provided based on assessment of your risk profile and investment objectives.",
				...
				},
				{
					...
					content:
						"FintekPro may receive commissions, trail fees, or other compensation from product manufacturers. We endeavor to provide unbiased advice in your best interest despite potential conflicts.",
				...
				},
				{
					...
					content:
						"Investment recommendations are made based on your stated investment objectives, risk tolerance, time horizon, and financial situation. Please ensure these details are accurate and updated.",
					regulatoryReference:
				...
				},
				{
					...
					content:
						"For complaints, contact our compliance officer at compliance@fintekpro.com. You may also register complaints with SEBI at SCORES portal (https://scores.gov.in) or contact SEBI Helpline 1800-22-7575.",
			...
			keyProvisions: [
				{
					...
					summary:
						"Investment adviser shall act in fiduciary capacity towards clients, maintain integrity and fair dealing",
				},
				{
					regulation: "Regulation 16",
					title: "Know Your Client",
					summary:
						"Before providing investment advice, verify client identity, financial situation, and investment experience",
				},
				{
					regulation: "Regulation 17(1)",
					title: "Risk Profiling",
					summary:
						"Assess risk appetite of client based on financial situation, investment objectives, and risk tolerance",
				},
				{
					regulation: "Regulation 17(2)",
					title: "Suitability",
					summary:
						"Ensure investment advice is appropriate based on risk profile; document rationale for recommendations",
				},
				{
					regulation: "Regulation 17(3)",
					title: "Documentation",
					summary:
						"Maintain records of all investment advice, risk assessments, and client acknowledgments for minimum 5 years",
				},
				{
					regulation: "Regulation 17(4)",
					title: "Disclosure of Rationale",
					summary:
						"Disclose reasoning and rationale for investment recommendations to client",
				},
				{
					regulation: "Regulation 18",
					title: "Conflicts of Interest",
					summary:
						"Disclose all conflicts, avoid preferential treatment, segregate advisory from distribution activities",
				},
				{
					regulation: "Regulation 21",
					title: "Fee Structure",
					summary:
						"Investment advisor may charge fees as fixed, asset-based, or performance-based; no hidden charges",
				},
			],
