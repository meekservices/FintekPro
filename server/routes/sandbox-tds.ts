...
export function registerSandboxTDSRoutes(app: Express): void {
   ...
   */
  app.post('/api/tds/calculator/non-salary', async (req: Request, res: Response) => {
    try {
      const {
        deducteeType,
        isPanAvailable,
      ...
      if (!deducteeType || !residentialStatus || !natureOfPayment || creditAmount === undefined) {
        return res.status(400).json({ success: false, message: 'deducteeType, residentialStatus, natureOfPayment and creditAmount are required' });
      }
      ...
      return res.status(500).json({ success: false, message: error instanceof Error ? error.message : 'Calculation failed' });
    }
  });
   ...
   */
  app.post('/api/tds/analytics/potential-notices', async (req: Request, res: Response) => {
    try {
      const { tan, quarter, form, financialYear } = req.body;
      if (!tan || !quarter || !form || !financialYear) {
        return res.status(400).json({ success: false, message: 'tan, quarter, form and financialYear are required' });
      }
      const result = await sandboxTDSService.submitTDSAnalyticsJob({ tan, quarter, form, financialYear });
      return res.json(result);
    } catch (error) {
      console.error('[TDS Analytics] submit job error:', error);
      return res.status(500).json({ success: false, message: error instanceof Error ? error.message : 'Failed to submit analytics job' });
    }
  });
   ...
   */
  app.get('/api/tds/analytics/potential-notices', async (req: Request, res: Response) => {
    try {
      const { job_id } = req.query;
      if (!job_id) {
        return res.status(400).json({ success: false, message: 'job_id query param is required' });
      }
      const result = await sandboxTDSService.pollTDSAnalyticsJob(String(job_id));
      return res.json(result);
    } catch (error) {
      console.error('[TDS Analytics] poll job error:', error);
      return res.status(500).json({ success: false, message: error instanceof Error ? error.message : 'Failed to poll analytics job' });
    }
  });
   ...
   */
  app.post('/api/tcs/analytics/potential-notices', async (req: Request, res: Response) => {
    try {
      ...
      const result = await sandboxTDSService.submitTCSAnalyticsJob({ tan, quarter, financialYear });
      return res.json(result);
    } catch (error) {
      console.error('[TCS Analytics] submit job error:', error);
      return res.status(500).json({ success: false, message: error instanceof Error ? error.message : 'Failed to submit TCS analytics job' });
    }
  });
   ...
   */
  app.get('/api/tcs/analytics/potential-notices', async (req: Request, res: Response) => {
    try {
      const { job_id } = req.query;
      if (!job_id) {
        return res.status(400).json({ success: false, message: 'job_id query param is required' });
      }
      const result = await sandboxTDSService.pollTCSAnalyticsJob(String(job_id));
      return res.json(result);
    } catch (error) {
      console.error('[TCS Analytics] poll job error:', error);
      return res.status(500).json({ success: false, message: error instanceof Error ? error.message : 'Failed to poll TCS analytics job' });
    }
  });
  ...
  app.post('/api/tds/reports/txt', async (req: Request, res: Response) => {
    try {
      const { tan, quarter, form, financialYear, previousReceiptNumber } = req.body;
      if (!tan || !quarter || !form || !financialYear) {
        return res.status(400).json({ success: false, message: 'tan, quarter, form and financialYear are required' });
      }
      const result = await sandboxTDSService.submitTDSReportJob({ tan, quarter, form, financialYear, previousReceiptNumber });
      return res.json(result);
    } catch (error) {
      console.error('[TDS Reports] submit job error:', error);
      return res.status(500).json({ success: false, message: error instanceof Error ? error.message : 'Failed to submit TDS report job' });
    }
  });
  ...
  app.get('/api/tds/reports/txt', async (req: Request, res: Response) => {
    try {
      ...
      return res.status(500).json({ success: false, message: error instanceof Error ? error.message : 'Failed to poll TDS report job' });
    }
  });
   ...
   */
  app.post('/api/tds/reports/txt/search', async (req: Request, res: Response) => {
    try {
      const { tan, quarter, form, financialYear, fromDate, toDate, pageSize, lastEvaluatedKey } = req.body;
      if (!tan) {
        return res.status(400).json({ success: false, message: 'tan is required' });
      }
      const result = await sandboxTDSService.searchTDSReportJobs({
        tan, quarter, form, financialYear,
      ...
      return res.status(500).json({ success: false, message: error instanceof Error ? error.message : 'Failed to search TDS report jobs' });
    }
  });
  ...
  app.post('/api/tcs/reports/txt', async (req: Request, res: Response) => {
    try {
      ...
      const result = await sandboxTDSService.submitTCSReportJob({ tan, quarter, financialYear, previousReceiptNumber });
      return res.json(result);
    } catch (error) {
      console.error('[TCS Reports] submit job error:', error);
      return res.status(500).json({ success: false, message: error instanceof Error ? error.message : 'Failed to submit TCS report job' });
    }
  });
  ...
  app.get('/api/tcs/reports/txt', async (req: Request, res: Response) => {
    try {
      ...
      return res.status(500).json({ success: false, message: error instanceof Error ? error.message : 'Failed to poll TCS report job' });
    }
  });
   ...
   */
  app.post('/api/tcs/reports/txt/search', async (req: Request, res: Response) => {
    try {
      const { tan, quarter, financialYear, fromDate, toDate, pageSize, lastEvaluatedKey } = req.body;
      if (!tan) {
        return res.status(400).json({ success: false, message: 'tan is required' });
      }
      const result = await sandboxTDSService.searchTCSReportJobs({
        tan, quarter, financialYear,
        fromDate: fromDate ? Number(fromDate) : undefined,
        toDate: toDate ? Number(toDate) : undefined,
        pageSize: pageSize ? Number(pageSize) : undefined,
        lastEvaluatedKey,
      ...
      return res.status(500).json({ success: false, message: error instanceof Error ? error.message : 'Failed to search TCS report jobs' });
    }
  });
   ...
   */
  app.post('/api/tds/compliance/traces/deductors/forms/:certificateType', async (req: Request, res: Response) => {
    try {
      const { certificateType } = req.params;
      if (certificateType !== 'form-16' && certificateType !== 'form-16a') {
        return res.status(400).json({ success: false, message: 'certificateType must be "form-16" or "form-16a"' });
      }

      const { username, password, tan, securityCaptcha, rememberMe } = req.body;
      if (!username || !password || !tan || !securityCaptcha) {
        return res.status(400).json({ success: false, message: 'username, password, tan and securityCaptcha are required' });
      }
      ...
      return res.status(500).json({ success: false, message: error instanceof Error ? error.message : 'Failed to submit Form 16 job' });
    }
  });
   ...
   */
  app.post('/api/tds/compliance/traces/deductors/forms/:certificateType/status', async (req: Request, res: Response) => {
    try {
      const { certificateType } = req.params;
      if (certificateType !== 'form-16' && certificateType !== 'form-16a') {
        return res.status(400).json({ success: false, message: 'certificateType must be "form-16" or "form-16a"' });
      ...
      if (!username || !password || !tan) {
        return res.status(400).json({ success: false, message: 'username, password and tan are required' });
      }
      ...
      return res.status(500).json({ success: false, message: error instanceof Error ? error.message : 'Failed to poll Form 16 status' });
    }
  });
   ...
   */
  app.post('/api/tds/compliance/fvu/generate', async (req: Request, res: Response) => {
    try {
      const { tan, financialYear, quarter, form, filingType } = req.body;
      if (!tan || !financialYear || !quarter || !form || !filingType) {
        return res.status(400).json({ success: false, message: 'tan, financialYear, quarter, form and filingType are required' });
      }
      if (filingType !== 'regular' && filingType !== 'correction') {
        return res.status(400).json({ success: false, message: 'filingType must be "regular" or "correction"' });
      }
      const result = await sandboxTDSService.submitFVUGenerateJob({ tan, financialYear, quarter, form, filingType });
      return res.json(result);
    } catch (error) {
      console.error('[TDS Compliance] FVU generate error:', error);
      return res.status(500).json({ success: false, message: error instanceof Error ? error.message : 'Failed to submit FVU generate job' });
    }
  });
   ...
   */
  app.get('/api/tds/compliance/fvu/generate', async (req: Request, res: Response) => {
    try {
      const { job_id } = req.query;
      if (!job_id) {
        return res.status(400).json({ success: false, message: 'job_id query param is required' });
      }
      const result = await sandboxTDSService.pollFVUGenerateJob(String(job_id));
      return res.json(result);
    } catch (error) {
      console.error('[TDS Compliance] FVU poll error:', error);
      return res.status(500).json({ success: false, message: error instanceof Error ? error.message : 'Failed to poll FVU generate job' });
    }
  });
   ...
   * Returns: { job_id }
   */
  app.post('/api/tds/compliance/e-file', async (req: Request, res: Response) => {
    try {
      const { tan, financialYear, form, quarter } = req.body;
      if (!tan || !financialYear || !form || !quarter) {
        return res.status(400).json({ success: false, message: 'tan, financialYear, form and quarter are required' });
      }
      const result = await sandboxTDSService.submitEFileJob({ tan, financialYear, form, quarter });
      return res.json(result);
    } catch (error) {
      console.error('[TDS Compliance] e-file submit error:', error);
      return res.status(500).json({ success: false, message: error instanceof Error ? error.message : 'Failed to submit e-file job' });
    }
  });
  ...
  app.get('/api/tds/compliance/e-file', async (req: Request, res: Response) => {
    try {
      ...
      return res.status(500).json({ success: false, message: error instanceof Error ? error.message : 'Failed to poll e-file job' });
    }
  });
  ...
  console.log('✅ Sandbox TDS routes registered (Calculator / Analytics / Reports / Compliance)');
}
