...
export function registerAgentCapitalGainPart2Part1Routes(app: Express): void {
	app.get("/api/agent/clients", requireAgent, async (req, res) => {
		try {
			...
			const clientRelationships = await storage.getClientsForAgent(
				req.user!.id,
			);

			console.log(
				"[Agent Clients] Client relationships found:",
				clientRelationships.length,
			);
			// Get client IDs for batch fetching
			const clientIds = clientRelationships
				.map((r) => r.clientId)
				.filter(Boolean);
			...
			if (clientIds.length > 0) {
				const clientsData = await db
					...
					.leftJoin(
						schema.kycVault,
						eq(schema.users.id, schema.kycVault.userId),
					)
				...
				const portfolioMap = new Map(
					portfolioValues.map((p) => [p.userId, Number(p.totalValue) || 0]),
				);
				...
				enrichedClients = clientsData.map((client) => {
					...
					let clientCategory: string =
						client.clientType === "corporate" ? "corporate" : "retail";
					...
					return {
						...
						totalPortfolioValue,
						createdAt:
							client.createdAt?.toISOString() || new Date().toISOString(),
			...
			const prospectsMapped = prospects.map((prospect) => {
				...
				return {
					...
					createdAt:
						prospect.createdAt?.toISOString() || new Date().toISOString(),
					isActive: true,
					isProspect: true,
			...
			if (searchTerm) {
				...
				allClientsAndProspects = allClientsAndProspects.filter(
					(client) =>
			...
			console.log(
				"[Agent Clients] Returning:",
				allClientsAndProspects.length,
				"total items",
			);
	...
	app.post("/api/agent/clients", requireAgent, async (req, res) => {
		try {
			...
			if (error.name === "ZodError") {
				return res
					.status(400)
					.json({ error: error.issues[0]?.message || "Validation failed" });
			}
	...
	app.get("/api/agent/stats", requireAgent, async (req, res) => {
		try {
			...
			const activeClients = clientRelationships.filter(
				(c) => c.isActive,
			).length;
			...
			if (clientIds.length > 0) {
				...
				totalAUM = portfolioValues.reduce(
					(acc, p) => acc + (Number.parseFloat(p.totalValue || "0") || 0),
					0,
				);
			}
			...
			const thisMonthBusiness =
				Number.parseFloat(thisMonthCommissions[0]?.total || "0") || 0;
			...
			const lastMonthBusiness =
				Number.parseFloat(lastMonthCommissions[0]?.total || "0") || 0;
			...
			const totalCommissions =
				Number.parseFloat(totalCommissionsResult[0]?.total || "0") || 0;
			...
			const pendingCommissions =
				Number.parseFloat(pendingCommissionsResult[0]?.total || "0") || 0;

			// Monthly target (configurable, default 500000)
			const monthlyTarget = 500000;
			const targetProgress =
				monthlyTarget > 0 ? (thisMonthBusiness / monthlyTarget) * 100 : 0;
	...
	app.get("/api/agent/appointments", requireAgent, async (req, res) => {
		try {
			...
			const appointments = await db
				...
				.where(eq(schema.agentAppointments.agentId, agentId))
				.orderBy(
					schema.agentAppointments.date,
					schema.agentAppointments.startTime,
				);
	...
	app.patch("/api/agent/appointments/:id", requireAgent, async (req, res) => {
		try {
			...
			const [existing] = await db
				...
				.where(
					and(
						eq(schema.agentAppointments.id, id),
						eq(schema.agentAppointments.agentId, agentId),
					),
				);
			if (!existing) {
			...
			const [appointment] = await db
				...
				.set(updateData)
				.where(eq(schema.agentAppointments.id, id))
				.returning();
	...
	app.post(
		"/api/agent/appointments/:id/send-reminder",
		requireAgent,
		async (req, res) => {
			try {
				const agentId = req.user!.id;
		...
		try {
			...
			const commissionRates = {
				mutual_funds: {
					...
					minInvestment: 5000,
					description:
						"Earn trail commission on AUM plus upfront on new investments",
				},
				bonds: {
					trailPercent: 0.25,
					upfrontPercent: 0.5,
					minInvestment: 10000,
					description:
						"Fixed income products with competitive commission structure",
				},
				insurance: {
					trailPercent: 2.5,
					upfrontPercent: 15.0,
					minInvestment: 25000,
					description:
						"Life and health insurance with high upfront commissions",
				},
				unlisted_stocks: {
					trailPercent: 0,
					upfrontPercent: 2.0,
