...
function signMobileToken(
	payload: Omit<MobileJwtPayload, "iat" | "exp">,
): string {
 ...
 */
async function compareScryptPassword(
	supplied: string,
	stored: string,
): Promise<boolean> {
	try {
...
mobileAuthRouter.post("/token", async (req: Request, res: Response) => {
	...
	if (!email || !password || !portalType) {
		return res.status(400).json({
			...
			error: {
				...
				message: "email, password and portalType are required",
				retryable: false,
			},
		});
	}
	...
	try {
		...
		const normalised = email.includes("@")
			? email.toLowerCase()
			: email
					.replace(/\D/g, "")
					.replace(/^91(\d{10})$/, "$1")
		...
		const isAgent = roles.some((r) =>
			["agent", "master_agent", "sub_agent", "admin", "super_admin"].includes(
				r,
			),
		);
		...
		if (portalType === "agent" && !isAgent) {
			return res.status(403).json({
				...
				error: {
					...
					message: "You do not have agent portal access",
					retryable: false,
				},
			});
		}
