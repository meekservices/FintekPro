...
// ── Coverage stats ───────────────────────────────────────────────────────────
router.get('/api/admin/mf-analytics/coverage', requireAuth, async (req: Request, res: Response) => {
  try {
...
// ── Run a named job ──────────────────────────────────────────────────────────
router.post('/api/admin/mf-analytics/run/:job', requireAuth, async (req: Request, res: Response) => {
  try {
    const role = (req as any).user?.role;
    ...
    const result = await callPython<any>(`/api/mf/${job}`, 'POST', req.body || {}, (req as any).user);
