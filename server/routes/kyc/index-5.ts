...
export function registerKYCWizardPart5Routes(app: Express) {
			...
			try {
				...
				res.json({
					...
					verifiedFields: {
						...
						videoKycCompleted: !!(
							(userProfile as any)?.videoKycCompletedAt ||
							(userProfile as any)?.videoKycCompletedDate
						),
						videoKycCompletedAt:
							(userProfile as any)?.videoKycCompletedAt ||
							(userProfile as any)?.videoKycCompletedDate ||
						...
						videoKycExpired: !!(
							userProfile?.videoKycExpiryDate &&
							new Date() > new Date(userProfile.videoKycExpiryDate)
						),
				...
				res
					...
					.json({
						success: false,
						message: "Failed to fetch KYC vault status",
					});
			}
		},
	);
	...
	// ============================================================================
	app.get(
		"/api/kyc/sufficiency/:productCode",
		requireClientOrHigher,
		async (req: any, res) => {
			try {
				const userId = req.user!.id;
				const { productCode } = req.params;
				const { kycSufficiencyService, PRODUCT_PROFILES } = await import(
				...
				res
					.status(500)
					.json({ success: false, message: "Failed to check KYC sufficiency" });
			}
		},
	);
	...
	app.get(
		...
		async (req: any, res) => {
			try {
				const userId = req.user!.id;
				const { kycSufficiencyService } = await import(
					"../../services/kyc-sufficiency-service"
				);
				...
				res
					...
					.json({
						success: false,
						message: "Failed to check product sufficiency",
					});
			}
		},
	);
	...
	// ============================================================================
	app.get(
		"/api/kyc/incremental/:productCode",
		requireClientOrHigher,
		async (req: any, res) => {
			try {
				const userId = req.user!.id;
				...
				const result = await kycSufficiencyService.getIncrementalRequirements(
					userId,
					productCode as any,
				);
				...
				res
					...
					.json({
						success: false,
						message: "Failed to get incremental requirements",
					});
			}
		},
	);
	...
	app.get("/api/kyc/vault", requireClientOrHigher, async (req: any, res) => {
		try {
			...
			const vault = {
				...
				pan: {
					number: panVerified
						? maskPan(profile?.panNumber || userRow?.panNumber)
						: null,
					numberFull: panVerified
						? profile?.panNumber || userRow?.panNumber
						: null,
				...
				fatca: {
					...
					taxResidencyCountry:
						profile?.fatcaCountryOfTaxResidence ||
						(userRow as any)?.taxResidencyCountry ||
						"India",
					tinNumber:
						profile?.fatcaTinNumber || (userRow as any)?.tinNumber || null,
			...
			res
				.status(500)
				.json({ success: false, message: "Failed to load KYC vault" });
		}
	});
	...
	app.get(
		...
		async (req: any, res) => {
			try {
				...
				switch (product) {
					case "bse-mf": {
						...
						prefill = {
							...
							fatherOrSpouseName:
								profile?.fatherName || profile?.spouseName || "",
							...
							bankName: primaryBank?.bankName || "",
							bankAccountType: (primaryBank?.accountType || "SB")
								.toUpperCase()
								.slice(0, 2),
							accountHolderName: primaryBank?.accountHolderName || fullName,
							// FATCA
							taxResidencyCountry:
								profile?.fatcaCountryOfTaxResidence || "India",
					...
					case "alpaca": {
						...
						prefill = {
							...
							country_of_tax_residence: profile?.fatcaCountryOfTaxResidence
								? profile.fatcaCountryOfTaxResidence === "India"
									? "IND"
									: profile.fatcaCountryOfTaxResidence
								: "IND",
							...
							phone_number: userRow?.mobile
								? `+91${userRow.mobile.replace(/^\+91/, "")}`
								: "",
							// Address
				...
				res
					.status(500)
					.json({ success: false, message: "Failed to generate prefill data" });
			}
		},
	);

	console.log(
		"✅ KYC Wizard v2 routes registered (Orchestrator + Entity Lock + CKYC Scoring + Agent Blocks + AML + Tier Engine + Vault Status + Sufficiency)",
	);
}
