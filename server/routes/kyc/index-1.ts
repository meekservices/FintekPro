...
import {
	getComplianceStatus,
	ROLE_KYC_MINIMUM,
} from "../../middleware/universal-kyc-gate";
...
export function registerKYCWizardPart1Routes(app: Express) {
	...
	app.get(
		...
		async (req: any, res) => {
			try {
				...
				let guidanceMessage =
					"Complete your KYC verification to access FintekPro.";
				if (
					userRoles.some((r: any) => ["master_agent", "partner"].includes(r))
				) {
					guidanceMessage =
					...
					userRoles.some((r: any) =>
						["agent", "sub_agent", "associate"].includes(r),
					)
				) {
					guidanceMessage =
					...
					userRoles.some((r: any) =>
						["compliance_officer", "regulatory_auditor"].includes(r),
					)
				) {
					guidanceMessage =
					...
					userRoles.some((r: any) =>
						["client", "user", "business_client"].includes(r),
					)
				) {
					guidanceMessage =
				...
				res
					.status(500)
					.json({ error: "Failed to check KYC compliance status" });
			}
		},
	);
				...
				testPANs:
					env === "TEST"
						? [
								{
									pan: "XXXPX1234A",
									description: "Individual, Valid, Name & DOB match",
								},
								{
									pan: "XXXPX1234H",
									description: "Individual, Valid, Deceased",
								},
								{
									pan: "XXXPX1234O",
									description: "Individual, Valid, Name match, DOB mismatch",
								},
								{
									pan: "XXXPX1234L",
									description: "Individual, Valid, Name & DOB unmatched",
								},
	...
	app.get("/api/kyc/status", requireClientOrHigher, async (req: any, res) => {
		try {
			...
			const tierResult = kycOrchestratorService.computeTierResult({
				kycLevel: Number.parseInt(level, 10),
			...
			res.json({
				...
				data: {
					...
					canAccessInvestments:
						level >= "2" && tierResult.tier_status === "final",
					nextAction:
						level === "0"
	...
	app.get(
		"/api/kyc/transaction-check",
		requireClientOrHigher,
		async (req: any, res) => {
			const TRANSACTION_REQUIREMENTS: Record<
				string,
				{ level: number; label: string; steps: string[]; sebiRef?: string }
			> = {
				// Level 1 (PAN only)
				mutual_funds: {
					level: 1,
					label: "Mutual Funds",
					steps: ["PAN verification"],
					sebiRef: "AMFI Circular",
				},
				insurance: {
					level: 1,
					label: "Insurance",
				...
				ipo: {
					...
					steps: ["PAN verification", "Demat account"],
					sebiRef: "SEBI ICDR Reg.",
				},
				nps: {
					level: 1,
					label: "NPS",
			...
			try {
				...
				if (
					!(profile as any)?.panVerifiedViaSandbox &&
					!(profile as any)?.panVerifiedViaSmartKyc
				) {
					missingSteps.push("PAN verification");
				}
				if (requiredLevel >= 2) {
					if (!(profile as any)?.aadhaarVerifiedViaSmartKyc)
						missingSteps.push("Aadhaar verification");
					if (!(profile as any)?.ckycFetchedViaAuthBridge)
						missingSteps.push("CKYC registration");
					if (!(profile as any)?.isProfileCompleted)
						missingSteps.push("Risk profiling");
					// Add transaction-specific steps beyond base Level 2
					const txSpecific = requirement.steps.filter(
						(s: any) =>
							![
								"PAN verification",
								"Aadhaar verification",
				...
				if (
					...
					requiredLevel >= 2 &&
					!(profile as any)?.aadhaarVerifiedViaSmartKyc
				) {
					kycPath = "/onboarding?step=aadhaar";
				} else if (
					requiredLevel >= 2 &&
					!(profile as any)?.ckycFetchedViaAuthBridge
				) {
					kycPath = "/onboarding?step=ckyc";
				} else if (
					requiredLevel >= 2 &&
					!(profile as any)?.isProfileCompleted
				) {
				...
				res.json({
					...
					currentLevel,
					requiredLevel,
					missingSteps: canProceed
						? []
						: missingSteps.filter((s: any) => requirement.steps.includes(s)),
					kycPath,
				...
				res
					.status(500)
					.json({ success: false, error: "Failed to check KYC requirements" });
			}
		},
	);
			...
			try {
				...
				res.json({
					...
					data: {
						...
						panVerified: (profile as any).panVerifiedViaSandbox || false,
						aadhaarVerified:
							(profile as any).aadhaarVerifiedViaCashfree || false,
						...
						kycTierMetadata: {
							...
							maxAnnualInvestment:
								level === "0" ? 0 : level === "1" ? 50000 : 10000000,
						},
					},
				});
			} catch (error) {
	...
	app.post(
		...
		async (req: any, res) => {
			try {
				...
				if (
					(existingUserProfile as any)?.kycLevel === "2" &&
					(existingUserProfile as any)?.isProfileCompleted
				) {
					return res.json({
						success: true,
				...
				try {
					...
					console.warn(
						"[KYC Wizard] Error fetching active session, will attempt cleanup:",
						sessionErr,
					);
				}
				...
				if (existingSession && !forceNew) {
					...
					const reconciledStatus: any = {
						...(existingSession.stepStatus as any),
					};
					...
					if (profile) {
						if (
							((profile as any).panVerifiedViaSandbox ||
								(profile as any).panSandboxStatus === "VALID") &&
							!reconciledStatus.pan_verified
						) {
							reconciledStatus.pan_verified = true;
							if (reconciledStep === "pan_verification")
								reconciledStep = "ckyc_kra_check";
						}
						if (
							((profile as any).ckycFetchedViaAuthBridge ||
								(profile as any).ckycAuthBridgeStatus === "found") &&
							!reconciledStatus.ckyc_fetched
						) {
							...
							if (
								[
									...
									"aadhaar_otp",
									"aadhaar_otp_verify",
								].includes(reconciledStep)
							) {
								reconciledStep = "risk_profiling";
							}
						} else if (
							(profile as any).aadhaarVerifiedViaSmartKyc &&
							!reconciledStatus.aadhaar_verified
						) {
					...
					if (reconciledStep !== existingSession.currentStep) {
						console.log(
							`[KYC Wizard] Reconciled session ${existingSession.id}: ${existingSession.currentStep} → ${reconciledStep}`,
						);
						try {
							...
							console.warn(
								"[KYC Wizard] Failed to persist reconciled step:",
								reconcileErr,
							);
						}
					}
					...
					return res.json({
						...
						data: {
							...
							aadhaarVerified:
								existingSession.aadhaarVerified ||
								reconciledStatus.aadhaar_verified,
				...
				try {
					...
					console.warn(
						"[KYC Wizard] Error deactivating old sessions:",
						expireErr,
					);
				}
				...
				if (
					(userProfile as any)?.panVerifiedViaSandbox ||
					(userProfile as any)?.panSandboxStatus === "VALID"
				) {
					stepStatus.pan_verified = true;
					initialStep = "ckyc_kra_check"; // Check CKYC/KRA first per data minimization

					if (
						(userProfile as any)?.ckycFetchedViaAuthBridge ||
						(userProfile as any)?.ckycAuthBridgeStatus === "found"
					) {
					...
					if (
						(userProfile as any)?.isProfileCompleted &&
						stepStatus.ckyc_fetched
					) {
				...
				try {
					session = await storage.createKycVerificationSession({
						...
						panVerificationData:
							(userProfile as any)?.panSandboxResponse ||
							(userProfile as any)?.panVerificationData ||
							undefined,
						entityTypeSupported:
							entityType || (userProfile as any)?.entityType || "individual",
					} as any);
				} catch (createErr: any) {
	...
	app.post(
		"/api/kyc/wizard/verify-pan",
		requireClientOrHigher,
		async (req: any, res) => {
			try {
				const { sessionId, panNumber, dob, fullName, onboardingMode } =
					req.body;
				const userId = req.user!.id;
				...
				let verification;
				try {
					verification = await sandboxPANService.verifyPAN(
						panNumber,
						fullName,
						dob,
					);
					...
					const testHint = isTest
						? " In TEST environment, use test PANs like XXXPX1234A."
						: "";
					return res.json({
						success: false,
				...
				const panResult =
					await kycOrchestratorService.buildPanVerificationResult(
						panNumber,
						verification.data,
						req.user?.role || "user",
					);
				...
				try {
					...
					console.warn(
						"[KYC] Duplicate PAN check failed (non-fatal):",
						(dupErr as any)?.message,
					);
				}
				// ──────────────────────────────────────────────────────────────────────

				const existingProfileRes = await db
					.select()
					.from(schema.userProfiles)
					.where(eq(schema.userProfiles.userId, userId))
					.limit(1);
				...
				if (
					...
					panResult.entity_detected !== "INDIVIDUAL"
				) {
					try {
						...
						console.error(
							"[KYC] Auto-KYC Engine failed (non-fatal):",
							(autoKycErr as any)?.message,
						);
					}
				}
				...
				res.json({
					...
					message:
						"PAN verified. You can now access loans and insurance marketplace.",
				});
			} catch (error) {
				...
				if (error instanceof Error) {
					...
					if (
						...
						return res.status(500).json({
							success: false,
							message:
								"PAN verification service is not configured. Please contact support.",
						});
					}
