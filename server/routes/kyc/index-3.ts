...
export function registerKYCWizardPart3Routes(app: Express) {
			...
			try {
					...
					if (!created[0]) {
						return res
							...
							.json({
								success: false,
								message:
									"Profile could not be created. Please contact support.",
							});
					}
				...
				for (const [field, newValue] of Object.entries(updates)) {
					...
					if (KYC_FIELD_RULES.immutable.includes(field)) {
						const isLocked =
							field === "panNumber"
								? currentProfile.panVerifiedViaSandbox ||
									currentProfile.panSandboxStatus === "VALID"
								: field === "dateOfBirth"
									? currentProfile.panVerifiedViaSandbox
									: false;
						...
						if (isLocked) {
							errors.push(
								`${field} is verified and cannot be changed. Please contact support.`,
							);
							continue;
						}
					}
					...
					if (KYC_FIELD_RULES.documentRequired.includes(field)) {
						...
						if (documentIds.length === 0) {
							errors.push(
								`${field} change requires supporting documents. Please upload proof documents first.`,
							);
							continue;
						}
						...
						if (["firstName", "middleName", "lastName"].includes(field)) {
							if (!metadata.nameChangeReason) {
								warnings.push(
									`Name change requires a reason (e.g., marriage, legal name change).`,
								);
							}
						}
						...
						auditEntries.push({
							...
							reason:
								metadata.nameChangeReason ||
								metadata.addressChangeReason ||
					...
					// Check OTP-required fields
						...
						if (!metadata.otpVerified) {
							errors.push(
								`${field} change requires OTP verification. Please verify your ${field} first.`,
							);
							continue;
						}
						...
						const otpRecords = await db
							...
							.where(
								eq(
									...
									`profile_change_${userId}_${field}`,
								),
							)
							.limit(1);
						...
						if (!otpRecord || !otpRecord.verified) {
							errors.push(
								`${field} OTP verification is invalid or expired. Please request a new OTP.`,
							);
							continue;
						}
					...
					// Unknown field - reject
						...
						errors.push(
							`Field '${field}' is not editable through this interface.`,
						);
					}
				}
				...
				res.json({
					...
					updatedFields: Object.keys(allowedUpdates).filter(
						(k) => k !== "kycLastUpdatedDate" && k !== "kycUpdateMethod",
					),
					errors: errors.length > 0 ? errors : undefined,
					warnings: warnings.length > 0 ? warnings : undefined,
					requiresDocumentUpload: auditEntries.some(
						(e) => e.requiresDocumentProof,
					),
				});
			} catch (error) {
	...
	app.get(
		...
		async (req: any, res) => {
			try {
				...
				res
					.status(500)
					.json({ success: false, message: "Failed to fetch change history" });
			}
		},
	);
			...
			try {
				...
				if (!type || !newValue) {
					return res
						...
						.json({
							success: false,
							message: "Type and new value are required",
						});
				}
				...
				if (!["email", "mobile"].includes(type)) {
					return res
						.status(400)
						.json({ success: false, message: "Type must be email or mobile" });
				}
				...
				if (type === "email") {
					...
					if (!mobileRegex.test(newValue)) {
						return res
							...
							.json({
								success: false,
								message: "Invalid mobile format (10 digits starting with 6-9)",
							});
					}
				}
				...
				console.log(
					`[KYC Profile Change] Sending OTP to ${type}: ${newValue} for user ${userId}`,
				);
		...
		async (req: any, res) => {
			try {
				...
				if (!type || !otp) {
					return res
						.status(400)
						.json({ success: false, message: "Type and OTP are required" });
				}
			...
			try {
				...
				if (!documentType || !changeType) {
					return res
						...
						.json({
							success: false,
							message: "Document type and change type are required",
						});
				}
				...
				const validDocs =
					changeType === "name" ? validNameDocs : validAddressDocs;
	...
	app.get(
		"/api/kyc/profile-change/required-documents/:changeType",
		requireClientOrHigher,
		async (req: any, res) => {
			try {
						...
						acceptedTypes: [
							{
								...
								description: "Official gazette notification for name change",
							},
							{
								id: "marriage_certificate",
								name: "Marriage Certificate",
								description: "For name change due to marriage",
							},
							{
								id: "court_order",
								name: "Court Order",
								description: "Legal court order for name change",
							},
							{
								id: "passport",
								name: "Passport",
								description: "Passport with new name",
							},
						],
						message:
							"As per SEBI/RBI guidelines, name changes require legal documentation.",
					},
					address: {
						required: true,
						acceptedTypes: [
							{
								...
								name: "Utility Bill",
								description:
									"Recent electricity/water/gas bill (not older than 3 months)",
							},
							{
								id: "bank_statement",
								name: "Bank Statement",
								description: "Recent bank statement with new address",
							},
							{
								id: "aadhaar_card",
								name: "Aadhaar Card",
								description: "Updated Aadhaar card with new address",
							},
							{
								id: "rental_agreement",
							...
							},
							{
								id: "passport",
								name: "Passport",
								description: "Passport with new address",
							},
						],
						message:
							"Address proof document must be recent (within 3 months) as per KYC guidelines.",
					},
				};
				...
				if (!requirements) {
					return res.json({
						...
						data: {
							required: false,
							message: "No document required for this change type",
						},
					});
				}
	...
	async function callTruthScreenAML(params: {
		...
		if (tsUsername && tsPassword) {
			try {
				...
				const encryptedRequest =
					encResp.data?.encryptedString ||
					encResp.data?.encrypted_string ||
					encResp.data;
				...
				const encryptedResponse =
					searchResp.data?.responseData ||
					searchResp.data?.response_data ||
					searchResp.data;
				...
				const pepMatches: any[] =
					decrypted?.pepData || decrypted?.pep_data || [];
				const sanctionMatches: any[] =
					decrypted?.sanctionData || decrypted?.sanction_data || [];
				const matchCount = pepMatches.length + sanctionMatches.length;
				const riskScore =
					matchCount === 0
						? params.panVerified && params.aadhaarVerified
							? 8
							: 15
						: matchCount >= 3
							? 75
				...
				return {
					...
					factors:
						matchCount > 0
							? [{ type: "sanctions_pep", severity: riskLevel }]
							: [],
					source: "truthscreen_live",
				};
			} catch (tsErr: any) {
				console.warn(
					"[AML/TruthScreen] Live call failed, using heuristic fallback:",
					tsErr?.message,
				);
			}
		}
		...
		console.log(
			`[AML/Heuristic] TruthScreen creds not set — heuristic score=${riskScore}`,
		);
		return {
			riskScore,
			riskLevel,
	...
	app.post(
		...
		async (req: any, res) => {
			try {
				const userId = req.user!.id;
				const { sessionId, firstName, lastName, dateOfBirth, nationality } =
					req.body;
				...
				const [panVerifiedUser] = await db
					.select({
						panVerified: schema.users.panVerifiedViaSmartKyc,
						aadhaarVerified: schema.users.aadhaarVerifiedViaSmartKyc,
					})
					.from(schema.users)
				...
				const amlResult =
					kycOrchestratorService.computeAmlResult(screeningData);
				...
				res
					.status(500)
					.json({ success: false, message: "Failed to perform AML screening" });
			}
		},
	);
