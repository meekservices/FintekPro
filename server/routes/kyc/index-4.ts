		...
		async (req: any, res) => {
			try {
				...
				const entityDetected =
					kycOrchestratorService.detectEntityFromPAN(panNumber);
				const overrideCheck = kycOrchestratorService.canOverrideEntity(
					req.user?.role || "user",
				);
			...
			try {
				...
				try {
					...
					if (truthScreenAdapter.isConfigured()) {
						...
						console.log(
							`[CKYC] TruthScreen check: found=${ckycResult.found}, kin=${ckycResult.kin || "N/A"}`,
						);
					} else {
				...
				res
					.status(500)
					.json({ success: false, message: "Failed to check CKYC" });
			}
		},
	);
			...
			try {
				...
				if (["agent", "partner", "sub_partner"].includes(userRole)) {
					return res.status(403).json({
						success: false,
						message:
							"Aadhaar OTP can only be initiated by the customer directly.",
				...
				if (isDemoMode) {
					...
					console.log(
						`[KYC] Aadhaar OTP sent via Sandbox API for user ${req.user!.id}, ref: ${result.referenceId}`,
					);
		...
		async (req: any, res) => {
			try {
				...
				if (isDemoMode) {
					...
					try {
						const existingVaultDemo = await db
						...
						console.warn(
							"[KYC] Failed to update kycVault for Aadhaar (demo):",
							demoVaultErr,
						);
					}
					return res.json({
				...
				if (!refId) {
					return res
						...
						.json({
							success: false,
							message:
								"No pending Aadhaar OTP request found. Please request a new OTP.",
						});
				}

				const result = await sandboxKYCService.verifyAadhaarOTP(refId, otp);
				console.log(
					`[KYC] Aadhaar verified via Sandbox API for user ${userId}`,
				);
				...
				if (result.address) {
					...
					console.log(
						`[KYC] Address saved from Aadhaar eKYC for user ${userId}: ${standaloneProfileUpdate.city}, ${standaloneProfileUpdate.state} ${standaloneProfileUpdate.pincode}`,
					);
				}
				...
				try {
					const existingVaultStandalone = await db
						.select({ id: schema.kycVault.id })
						.from(schema.kycVault)
						.where(eq(schema.kycVault.userId, userId))
						.limit(1);
					...
					console.warn(
						"[KYC] Failed to update kycVault for Aadhaar (standalone):",
						vaultErrStandalone,
					);
				}
	...
	app.post(
		...
		async (req: any, res) => {
			try {
				...
				if (
					!["agent", "partner", "sub_partner", "admin", "superadmin"].includes(
						userRole,
					)
				) {
					return res
				...
				const { firstName, lastName, email, mobile, panNumber, entityType } =
					req.body;
				...
				if (panNumber) {
					detectedEntity = kycOrchestratorService
						.detectEntityFromPAN(panNumber)
						.toLowerCase();
				}
				...
				const customerKycLink = kycOrchestratorService.generateCustomerKycLink(
					session.id,
					prospectId,
				);
				...
				res.json({
					...
					data: {
						...
						message:
							"Prospect created. Customer must complete Aadhaar, FATCA, and sign-off steps.",
					},
				});
			} catch (error) {
				...
				res
					.status(500)
					.json({ success: false, message: "Failed to create prospect" });
			}
		},
	);
	...
	app.patch(
		...
		async (req: any, res) => {
			try {
				...
				if (
					!kycOrchestratorService.canResumeStep(
				...
				res
					.status(500)
					.json({ success: false, message: "Failed to transition step" });
			}
		},
	);
