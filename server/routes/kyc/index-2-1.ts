...
export function registerKYCWizardPart2Sub1Routes(app: Express) {
	 ...
	 */
	app.post(
		"/api/kyc/wizard/send-aadhaar-otp",
			...
			try {
				...
				if (kycOrchestratorService.isAgentBlocked("aadhaar_otp", initiatedBy)) {
					return res.status(403).json({
						success: false,
						message:
							"Aadhaar OTP can only be initiated by the customer directly. Agents cannot trigger OTP verification.",
				...
				if (isDemoMode) {
					...
					console.log(
						`[KYC] Aadhaar OTP sent via Sandbox API for user ${userId}, ref: ${referenceId}`,
					);
				}
	...
	app.post(
		"/api/kyc/wizard/verify-aadhaar-otp",
		requireClientOrHigher,
		async (req: any, res) => {
			try {
				...
				if (isDemoMode) {
					...
					console.log(
						`[KYC] Aadhaar OTP verified in DEMO mode for user ${userId}`,
					);
				} else {
					if (!referenceId) {
						return res.status(400).json({
					...
					if (referenceId.startsWith("mock_ref_")) {
						...
						console.log(
							`[KYC] Aadhaar OTP verified via MOCK for user ${userId} (test environment)`,
						);
					} else {
						...
						console.log(
							`[KYC] Aadhaar OTP verified via Sandbox API for user ${userId}, name: ${result.fullName}`,
						);
					}
				}
				...
				if (verificationData.address) {
					...
					profileUpdate.address =
						addressParts.join(", ") || addr.addressLine || "";
					profileUpdate.city = addr.district || addr.city || "";
					profileUpdate.state = addr.state || "";
					profileUpdate.pincode = addr.pincode || addr.zip || "";
					console.log(
						`[KYC] Address saved from Aadhaar eKYC for user ${userId}: ${profileUpdate.city}, ${profileUpdate.state} ${profileUpdate.pincode}`,
					);
				}
				...
				try {
					const existingVaultAadhaar = await db
					...
					console.warn(
						"[KYC] Failed to update kycVault for Aadhaar:",
						vaultErr,
					);
				}
		...
		async (req: any, res) => {
			try {
				...
				res.json({
					success: true,
					message:
						"KYC completed successfully. You now have full access to all investment products.",
				});
			} catch (error) {
