...
export function registerKYCWizardPart2Sub2Routes(app: Express) {
	 ...
	 */
	app.post(
		"/api/kyc/wizard/check-kra-status",
		...
		async (req: any, res) => {
			try {
				...
				if (
					...
					profile?.ckycAuthBridgeResponse
				) {
					...
					const nextStep = ckycDecision.aadhaar_required
						? kycOrchestratorService.isAgentBlocked("aadhaar_otp", initiatedBy)
							? "risk_profiling"
							: "aadhaar_otp"
						: "risk_profiling";
				...
				try {
					...
					if (truthScreenAdapter.isConfigured()) {
						const tsResult = await truthScreenAdapter.verify({
							...
							dateOfBirth: session.panDob
								? new Date(session.panDob).toISOString().split("T")[0]
								: "",
							userId,
						});
						ckycResult = tsResult;
						console.log(
							`[KYC Wizard] TruthScreen CKYC check: found=${tsResult.found}, kin=${tsResult.kin || "N/A"}`,
						);
					} else {
						ckycResult = await authBridgeCKYCService.fetchCKYC({
							pan: (rawPan || "").toUpperCase(),
							full_name: (session.panVerificationData as any)?.name || "",
							date_of_birth: session.panDob
								? new Date(session.panDob).toISOString().split("T")[0]
								: "",
						});
					}
					...
					console.warn(
						"[KYC Wizard] CKYC/KRA check failed, proceeding with manual KYC flow:",
						(ckycErr as any)?.message,
					);
				}
				...
				const ckycDecision = kycOrchestratorService.computeCkycConfidence(
					isTruthScreenResult
						? {
								found: ckycResult.found,
								data: ckycResult.data,
								kin: ckycResult.kin,
								provider: "truthscreen",
							}
				...
				if (ckycDecision.ckyc_found) {
					...
					if (!ckycDecision.aadhaar_required && ckycResult.data) {
						...
						try {
							const existingVaultCkyc = await db
							...
							if (existingVaultCkyc.length > 0) {
								...
								await db
									...
									.values({
										...
										aadhaarVerifiedAt: now,
										addressVerifiedAt: now,
										source: "ckyc_registry",
										kycStatus: "pending",
									} as any);
							}
						} catch (vaultCkycErr) {
							console.warn(
								"[KYC] Non-fatal: vault update after CKYC auto-populate failed:",
								(vaultCkycErr as any)?.message,
							);
						}
						console.log(
							`[KYC Wizard] CKYC-complete: aadhaarVerifiedViaSmartKyc set for user ${userId} (skipped OTP via CKYC registry)`,
						);
					}
					// ──────────────────────────────────────────────────────────────────────
					...
					await kycOrchestratorService.logAuditEvent({
						...
						step: "ckyc_kra_check",
						details: {
							confidence: ckycDecision.confidence_score,
							missing: ckycDecision.missing_fields,
							aadhaar_required: ckycDecision.aadhaar_required,
							auto_populated: !ckycDecision.aadhaar_required,
						},
						performedBy: userId,
					});
				...
				await storage.updateKycVerificationSession(sessionId, {
					currentStep: kycOrchestratorService.isAgentBlocked(
						"aadhaar_otp",
						initiatedBy,
					)
						? "risk_profiling"
				...
				res.json({
					...
					ckycFound: false,
					message:
						"No existing CKYC record found. Proceeding to Aadhaar verification.",
					data: {
		...
		async (req: any, res) => {
			try {
				...
				if (!sessionId || !riskData) {
					console.error("Risk profiling validation failed:", {
						sessionId: !!sessionId,
						riskData: !!riskData,
	...
	app.post(
		"/api/kyc/wizard/compliance-signoff",
		requireClientOrHigher,
		async (req: any, res) => {
			try {
				const {
				...
				const initiatedBy = (session as any).initiatedBy || "customer";
				if (
					kycOrchestratorService.isAgentBlocked(
						"compliance_signoff",
						initiatedBy,
					)
				) {
					return res.status(403).json({
						success: false,
						message:
							"Compliance sign-off must be completed by the customer directly.",
				...
				const tierResult = kycOrchestratorService.computeTierResult({
					...
					panVerified: true,
					ckycFetched: !!(
						stepStatus.ckyc_fetched || (session as any).ckycFetched
					),
					aadhaarVerified: !!(
						stepStatus.aadhaar_verified || session.aadhaarVerified
					),
				...
				await db
					...
					.set({
						...
						numberOfDependents: riskProfile.numberOfDependents
							? Number.parseInt(riskProfile.numberOfDependents)
							: 0,
					} as any)
					.where(eq(schema.userProfiles.userId, userId));
				...
				try {
					...
					if (session.panNumber) {
						try {
							decryptedPan = await PANConsentService.decryptPAN(
								session.panNumber,
							);
						} catch {
					...
					const aadhaarVerificationData =
						session.aadhaarVerificationData as any;
					...
					const usersUpdate: Record<string, any> = {
						...
						aadhaarVerificationDate: aadhaarVerifiedFlag
							? session.aadhaarVerifiedAt || new Date()
							: null,
					};
					if (decryptedPan) {
					...
					if (!existingProfile?.firstName && panVerificationData?.name) {
						const nameParts = (panVerificationData.name as string)
							.trim()
							.split(/\s+/);
					...
					if (
						...
						(panVerificationData?.dob || session.panDob)
					) {
					...
					console.error(
						"[KYC] Non-fatal: failed to save PAN/name to users table after KYC completion:",
						saveErr,
					);
				}
				...
				await kycOrchestratorService.logAuditEvent({
					...
					details: {
						tier: tierResult.kyc_tier,
						tier_status: tierResult.tier_status,
						level: 2,
					},
					performedBy: userId,
					ipAddress: req.ip,
				...
				try {
					...
					const existingVaultFinal = await db
						.select({ id: schema.kycVault.id })
						.from(schema.kycVault)
						.where(eq(schema.kycVault.userId, userId))
						.limit(1);
					...
					if (existingVaultFinal.length > 0) {
						...
						await db
							.insert(schema.kycVault)
							.values({ ...vaultPayload, createdAt: new Date() });
					}
					console.log(
						"[KYC] kycVault populated with verified status for user:",
						userId,
					);
					// Invalidate caches so next sufficiency and compliance checks reflect new vault state
					try {
						const { invalidateSufficiencyCache } = await import(
							"../../services/kyc-sufficiency-service"
						);
					...
					console.warn(
						"[KYC] Non-fatal: failed to populate kycVault after compliance signoff:",
						vaultErr,
					);
				}
		...
		async (req: any, res) => {
			try {
				...
				if (panIsVerified) {
					...
					lockReasons.panNumber =
						"PAN verified — locked per SEBI guidelines. Use the Re-KYC request form to request a correction.";
				}
				...
				if (panIsVerified && userProfile?.dateOfBirth) {
					...
					lockReasons.dateOfBirth =
						"Date of birth verified via PAN — locked per SEBI guidelines. Use the Re-KYC request form to request a correction.";
				}
					...
					data: {
						...
						currentValues: {
							panNumber: userProfile?.panNumber
								? `XXXX-XXXX-${userProfile.panNumber.slice(-4)}`
								: null,
							...
							investmentExperience:
								(userProfile as any)?.investmentExperience || null,
				...
				res
					.status(500)
					.json({ success: false, message: "Failed to fetch field rules" });
			}
		},
	);
