...
router.get('/api/portfolio-reports/sample', async (req: Request, res: Response) => {
  try {
    const sampleReport = {
      reportId: 'sample-001',
      ...
      assetAllocation: {
        current: [
          { asset: 'Equity', value: 1527000, percentage: 60, color: '#4F46E5', subCategories: [
          ...
          { asset: 'Debt', value: 763500, percentage: 30, color: '#10B981', subCategories: [
            { name: 'Corporate Bonds', value: 382000, percentage: 15 },
            { name: 'Government Securities', value: 254500, percentage: 10 },
            { name: 'Liquid Funds', value: 127000, percentage: 5 }
          ]},
          { asset: 'Gold', value: 178150, percentage: 7, color: '#F59E0B' },
          { asset: 'Cash & Equivalents', value: 76350, percentage: 3, color: '#6B7280' }
        ],
        target: [
      ...
      topHoldings: [
        { name: 'HDFC Flexi Cap Fund', type: 'Mutual Fund', value: 450000, percentage: 17.7, returns1Y: 18.5, rating: 5 },
        { name: 'Reliance Industries', type: 'Stock', value: 320000, percentage: 12.6, returns1Y: 22.3 },
        { name: 'SBI Magnum Gilt Fund', type: 'Mutual Fund', value: 280000, percentage: 11.0, returns1Y: 7.2, rating: 4 },
        { name: 'ICICI Prudential Bluechip', type: 'Mutual Fund', value: 250000, percentage: 9.8, returns1Y: 16.8, rating: 5 },
        { name: 'TCS', type: 'Stock', value: 220000, percentage: 8.6, returns1Y: 14.2 },
        { name: 'HDFC Bank', type: 'Stock', value: 180000, percentage: 7.1, returns1Y: 12.5 },
        { name: 'Infosys', type: 'Stock', value: 160000, percentage: 6.3, returns1Y: 8.9 },
        { name: 'Axis Long Term Equity', type: 'ELSS', value: 150000, percentage: 5.9, returns1Y: 19.2, rating: 4 }
      ],
      riskMetrics: {
        beta: 0.85,
        sharpeRatio: 1.24,
      ...
      goals: [
        { name: 'Retirement Corpus', targetAmount: 50000000, currentValue: 1200000, targetDate: '2045-01-01', progress: 24, onTrack: true },
        { name: 'Child Education', targetAmount: 5000000, currentValue: 800000, targetDate: '2032-06-01', progress: 45, onTrack: true },
        { name: 'House Down Payment', targetAmount: 3000000, currentValue: 545000, targetDate: '2028-01-01', progress: 18, onTrack: false, shortfall: 150000 }
      ],
      freshInvestments: {
        lumpsumRecommendations: [
          { name: 'Parag Parikh Flexi Cap Fund', category: 'Equity - Flexi Cap', minAmount: 5000, expectedReturn: 14.5, riskLevel: 'Moderate', rating: 5, matchScore: 95, reason: 'Excellent diversification with international exposure' },
          { name: 'HDFC Corporate Bond Fund', category: 'Debt - Corporate Bond', minAmount: 5000, expectedReturn: 7.8, riskLevel: 'Low', rating: 4, matchScore: 88, reason: 'High quality AAA rated bonds for stable returns' },
          { name: 'Mirae Asset Large Cap Fund', category: 'Equity - Large Cap', minAmount: 5000, expectedReturn: 13.2, riskLevel: 'Moderate', rating: 5, matchScore: 92, reason: 'Consistent performer in large cap space' }
        ],
        sipRecommendations: [
          { name: 'Axis Small Cap Fund', category: 'Equity - Small Cap', minSIP: 500, suggestedSIP: 5000, expectedReturn: 18.5, riskLevel: 'High', rating: 5, matchScore: 90, reason: 'High growth potential with SIP averaging benefit' },
          { name: 'SBI Equity Hybrid Fund', category: 'Hybrid - Balanced', minSIP: 500, suggestedSIP: 10000, expectedReturn: 11.5, riskLevel: 'Moderate', rating: 4, matchScore: 93, reason: 'Balanced approach with automatic rebalancing' },
          { name: 'Kotak Emerging Equity Fund', category: 'Equity - Mid Cap', minSIP: 1000, suggestedSIP: 7500, expectedReturn: 15.8, riskLevel: 'Moderate-High', rating: 5, matchScore: 88, reason: 'Strong mid-cap exposure for wealth creation' }
        ],
      ...
      aiInsights: [
        { type: 'opportunity', title: 'Tax Loss Harvesting', description: 'Switch from underperforming small cap fund to save ₹12,000 in taxes', priority: 'high', actionable: true },
        { type: 'rebalancing', title: 'Portfolio Rebalancing', description: 'Equity allocation is 5% above target. Consider moving ₹1.2L to debt', priority: 'medium', actionable: true },
        { type: 'risk', title: 'Sector Concentration', description: 'IT sector at 22% - consider diversifying into pharma or auto', priority: 'low', actionable: false },
        { type: 'goal', title: 'House Down Payment Gap', description: 'Increase SIP by ₹8,000/month to stay on track for 2028 goal', priority: 'high', actionable: true }
      ],
      comparisonWithBenchmark: {
        ...
        periods: [
          ...
          { period: '5Y', portfolio: 78.2, benchmark: 65.4, difference: 12.8 }
        ]
      },
      disclaimers: [
    ...
    console.error('[Portfolio Reports] Error generating sample report:', error);
    res.status(500).json({ error: 'Failed to generate sample report' });
  }
});

router.get('/api/portfolio-reports/customization-options', async (req: Request, res: Response) => {
  const options = {
    sections: [
      { id: 'portfolioSnapshot', name: 'Portfolio Snapshot', description: 'Overview of portfolio value and returns', default: true },
      { id: 'assetAllocation', name: 'Asset Allocation', description: 'Donut chart with current vs target allocation', default: true },
      { id: 'sectorExposure', name: 'Sector Exposure', description: 'Breakdown by industry sectors', default: true },
      { id: 'topHoldings', name: 'Top Holdings', description: 'List of major investments', default: true },
      { id: 'riskMetrics', name: 'Risk Metrics', description: 'Beta, Sharpe ratio, volatility analysis', default: true },
      { id: 'performanceHistory', name: 'Performance History', description: 'Returns comparison with benchmark', default: true },
      { id: 'goals', name: 'Goal Tracking', description: 'Progress towards financial goals', default: false },
      { id: 'freshInvestments', name: 'Fresh Investment Ideas', description: 'Lumpsum and SIP recommendations', default: false },
      { id: 'aiInsights', name: 'AI Insights', description: 'Smart recommendations and alerts', default: true },
      { id: 'disclaimers', name: 'Disclaimers', description: 'SEBI-compliant disclosures', default: true }
    ],
