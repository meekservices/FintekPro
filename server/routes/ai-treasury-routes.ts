...
router.post("/copilot/query", async (req, res) => {
	try {
		...
		const entityData = await treasuryService.getPositions(
			entityId || "default",
		);
		...
		const { getCachedRationale, cacheRationale } = await import(
			"../services/investment-cache-service"
		);

		// 2. Check Cache
		const cachedResult = await getCachedRationale(
			"treasury_copilot",
			inputParams,
		);
		...
		if (cachedResult) {
			console.log(
				`[TreasuryCopilot] Cache HIT for query: ${query.slice(0, 50)}...`,
			);
			return res.json({
		...
		console.log(
			`[TreasuryCopilot] Cache MISS for query: ${query.slice(0, 50)}...`,
		);
