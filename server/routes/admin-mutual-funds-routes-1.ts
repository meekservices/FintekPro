...
function enforceCommissionCompliance(body: any): string | null {
	const upfront = Number.parseFloat(
		body.upfrontCommission ?? body.upfrontCommissionRate ?? "0",
	);
	if (upfront > 0) {
	...
	const trail = Number.parseFloat(
		body.trailCommission ?? body.regularTrailCommission ?? "0",
	);
...
router.get("/mutual-funds", async (req: Request, res: Response) => {
	try {
		...
		const conditions: any[] = [eq(storeProducts.productType, "mutual_fund")];
...
router.post("/mutual-funds/seed", async (req: Request, res: Response) => {
	try {
		...
		const planSuffix =
			data.planType === "direct" ? " - Direct Plan" : " - Regular Plan";
		...
		const [newFund] = await db
			...
			.values({
				...
				shortDescription:
					...
					`${data.fundHouse} - ${data.planType.toUpperCase()} Plan`,
				...
				trailCommission:
					data.planType === "regular" ? data.trailCommission?.toString() : null,
...
router.post("/mutual-funds/seed-both", async (req: Request, res: Response) => {
	try {
		const baseData = seedMutualFundSchema
			.omit({ planType: true })
			.parse(req.body);
		const { directExpenseRatio, regularExpenseRatio, regularTrailCommission } =
			req.body;
		...
		const directData = {
			...
			expenseRatio:
				directExpenseRatio ||
				(baseData.expenseRatio ? baseData.expenseRatio * 0.6 : undefined),
		...
		if (existingRegular.length === 0) {
			const [regularFund] = await db
				...
				.values({
					...
					price: baseData.nav?.toString(),
					expenseRatio: (
						regularExpenseRatio || baseData.expenseRatio
					)?.toString(),
...
router.put("/categories/:id/toggle", async (req: Request, res: Response) => {
	try {
		...
		if (data.directFundsEnabled !== undefined)
			updateFields.directFundsEnabled = data.directFundsEnabled;
		if (data.comingSoonMessage !== undefined)
			updateFields.comingSoonMessage = data.comingSoonMessage;
		if (data.comingSoonExpectedDate !== undefined)
			updateFields.comingSoonExpectedDate = data.comingSoonExpectedDate;
...
router.get("/source-funds", async (req: Request, res: Response) => {
	try {
		...
		const funds = await query.limit(Number.parseInt(limit as string));
...
router.get("/fund-by-isin/:isin", async (req: Request, res: Response) => {
	try {
		...
		if (!isin || isin.length !== 12) {
			return res
				.status(400)
				.json({ success: false, error: "Invalid ISIN format" });
		}
...
router.post(
	"/advisory-subscriptions/:id/cancel",
	async (req: Request, res: Response) => {
		try {
			const { id } = req.params;
			...
			console.error(
				"[Admin MF] Error cancelling advisory subscription:",
				error,
			);
			res.status(500).json({ success: false, error: error.message });
		}
