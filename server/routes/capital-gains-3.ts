...
export function registerCapitalGainPart3Routes(app: Express): void {
	app.post("/api/capital-gains/itr-export-actual", async (req, res) => {
		try {
			...
			const fy =
				fiscalYear || realizedGainsAggregationService.getCurrentFiscalYear();
			...
			res.json({
				...
				sectionA1_EquitySTCG: {
					description:
						"Short Term Capital Gain on equity shares/units on which STT is paid",
				...
				sectionA2_EquityLTCG: {
					description:
						"Long Term Capital Gain on equity shares/units on which STT is paid",
			...
			res
				.status(500)
				.json({ error: "Failed to generate ITR export from realized gains" });
		}
	});

	app.post(
		"/api/tax/regime-comparison",
		requireAuth,
		...
		async (req: Request, res: Response) => {
			try {
				...
				const oldRegimeTotalDeductions =
					deductions80C + deductions80D + homeLoanInterest + 50000;
				const oldRegimeTaxableNonCG = Math.max(
					0,
					totalNonCGIncome - oldRegimeTotalDeductions,
				);
				...
				if (oldRegimeTaxableNonCG > 250000) {
					if (oldRegimeTaxableNonCG <= 500000) {
						...
						oldRegimeIncomeTax = 12500 + (oldRegimeTaxableNonCG - 500000) * 0.2;
					} else {
						oldRegimeIncomeTax =
							112500 + (oldRegimeTaxableNonCG - 1000000) * 0.3;
					}
				}

				const newRegimeStandardDeduction = 75000;
				const newRegimeTaxableNonCG = Math.max(
					0,
					totalNonCGIncome - newRegimeStandardDeduction,
				);
				...
				if (newRegimeTaxableNonCG > 300000) {
					if (newRegimeTaxableNonCG <= 700000) {
						...
						newRegimeIncomeTax = 20000 + (newRegimeTaxableNonCG - 700000) * 0.1;
					} else if (newRegimeTaxableNonCG <= 1200000) {
						newRegimeIncomeTax =
							50000 + (newRegimeTaxableNonCG - 1000000) * 0.15;
					} else if (newRegimeTaxableNonCG <= 1500000) {
						newRegimeIncomeTax =
							80000 + (newRegimeTaxableNonCG - 1200000) * 0.2;
					} else {
						newRegimeIncomeTax =
							140000 + (newRegimeTaxableNonCG - 1500000) * 0.3;
					}
				}

				const stcgTax = Math.max(0, stcg) * 0.2;
				...
				const recommendedRegime =
					newRegimeFinalTax <= oldRegimeFinalTax ? "new" : "old";
				...
				console.log(
					`[TaxRegimeComparison] User ${req.user!.id}: Old=${oldRegimeFinalTax.toFixed(2)}, New=${newRegimeFinalTax.toFixed(2)}, Recommended=${recommendedRegime}`,
				);
			...
			try {
				...
				const scheduleCGData = {
					...
					shortTermCapitalGains111A: {
						section: "111A",
						description:
							"STCG on sale of listed equity shares/units of equity oriented MF where STT paid",
						fullValueOfConsideration: gains.trades
							.filter((t) => t.gainType === "STCG")
							.reduce(
								(sum, t) =>
									sum + (t.taxableGain + (t.realizedGain - t.taxableGain)),
								0,
							),
					...
					longTermCapitalGains112A: {
						section: "112A",
						description:
							"LTCG on sale of listed equity shares/units of equity oriented MF where STT paid",
						fullValueOfConsideration: gains.trades
						...
						exemptionUnder112A: Math.min(
							125000,
							Math.max(0, ltcgEquity + ltcgGrandfathered),
						),
						netTaxableGain: Math.max(
							0,
							ltcgEquity + ltcgGrandfathered - 125000,
						),
						taxRate: 12.5,
					},
					...
					summary: {
						...
						ltcgExemption: Math.min(
							125000,
							Math.max(0, ltcgEquity + ltcgGrandfathered),
						),
						netTaxableLTCG: Math.max(
							0,
							ltcgEquity + ltcgGrandfathered - 125000,
						),
				...
				console.log(
					`[ITRAutoPopulate] Generated Schedule CG for PAN ${panNumber}, AY ${assessmentYear}, ${gains.trades.length} trades`,
				);
			...
			try {
				...
				const pnlReport = {
					...
					shortTermCapitalGains: {
						...
						taxRate: "20%",
						taxAmount: Math.max(0, gains.stcg.total) * 0.2,
					},
					...
					disclaimer:
						"This report is for informational purposes. Please verify all calculations with a qualified tax professional before filing.",
				};

				console.log(
					`[TaxPnLReport] Generated for user ${req.user!.id}, FY ${fy}, format: ${format}`,
				);
	...
	app.post(
		...
		async (req: Request, res: Response) => {
			try {
				...
				const advanceTaxStatus =
					await realizedGainsAggregationService.getAdvanceTaxStatus(
						req.user!.id,
						fy,
					);
				...
				const stcgTax = Math.max(0, gains.stcg.total) * 0.2;
				...
				const validationResult = {
					...
					advanceTaxSchedule: {
						q1: {
							dueDate: `${fy.split("-")[0]}-06-15`,
							percentage: 15,
							status: advanceTaxStatus.quarters[0]?.status,
						},
						q2: {
							dueDate: `${fy.split("-")[0]}-09-15`,
							percentage: 45,
							status: advanceTaxStatus.quarters[1]?.status,
						},
						q3: {
							dueDate: `${fy.split("-")[0]}-12-15`,
							percentage: 75,
							status: advanceTaxStatus.quarters[2]?.status,
						},
						q4: {
							dueDate: `${Number.parseInt(fy.split("-")[0]) + 1}-03-15`,
							percentage: 100,
							status: advanceTaxStatus.quarters[3]?.status,
						},
					},
				...
				if (!isConsistent) {
					console.warn(
						`[AdvanceTaxValidation] Discrepancy for user ${req.user!.id}: Expected ${expectedTotal.toFixed(2)}, Got ${gains.totalTaxLiability.toFixed(2)}`,
					);
				}
				...
				res
					.status(500)
					.json({ error: "Failed to validate advance tax calculations" });
			}
		},
	);
	 ...
	 */
	app.post(
		"/api/capital-gains/validate-lots",
		requireAuth,
		async (req: Request, res: Response) => {
			try {
				const { lots } = req.body;
				...
				if (!lots || !Array.isArray(lots)) {
					return res.json({
						...
						validation: {
							...
							disabledReason:
								"Transaction lots are required for tax calculations",
						},
					});
				}
				...
				res.json({
					...
					validation,
					complianceNote:
						"Capital gains and exit load calculations are based on transaction-level data from the client's CAS statement and FIFO methodology, as per Indian tax regulations.",
				});
			} catch (error) {
	 ...
	 */
	app.post(
		"/api/capital-gains/calculate-tax-breakdown",
		requireAuth,
		async (req: Request, res: Response) => {
			try {
				...
				const validation = capitalGainsCalculator.validateLotsForTax(
					lots || [],
				);
				...
				const taxBreakdown = capitalGainsCalculator.validateTaxCalculation(
					lots,
					saleDate,
				);
				...
				res.json({
					...
					summary: {
						...
						taxStatus:
							taxBreakdown.ltcgUnits > 0 && taxBreakdown.stcgUnits > 0
								? "Mixed"
								: taxBreakdown.ltcgUnits > 0
									? "All LTCG"
									: "All STCG",
						referenceDate: taxBreakdown.referenceDate,
					},
					complianceNote:
						"FIFO lot-wise calculation per Indian equity mutual fund tax regulations.",
				});
			} catch (error) {
	 ...
	 */
	app.post(
		"/api/capital-gains/simulate-redemption",
		requireAuth,
		async (req: Request, res: Response) => {
			try {
				...
				const simulation = capitalGainsCalculator.simulateFIFORedemption(
					lots,
					Number.parseFloat(redemptionAmount),
					Number.parseFloat(currentNav),
				);
				...
				res.json({
					...
					summary: {
						...
						lotsWithExitLoad: simulation.consumedLots.filter(
							(l) => l.hasExitLoad,
						).length,
					},
					complianceNote:
						"FIFO methodology applied. Exit load calculated per lot based on holding period.",
				});
			} catch (error) {
