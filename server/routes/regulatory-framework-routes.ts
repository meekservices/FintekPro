...
router.post("/classify-investor", async (req: Request, res: Response) => {
	try {
		...
		const { investmentAmount, netWorth, kycTier, entityType } =
	...
	try {
		...
		if (!classificationType || !classificationBasis) {
			return res
				.status(400)
				.json({
					error: "classificationType and classificationBasis are required",
				});
		}

		const classification =
			await regulatoryFrameworkService.saveUserClassification(userId, {
				classificationType,
				classificationBasis,
				investmentAmount,
				netWorth,
...
router.get("/my-classification", async (req: Request, res: Response) => {
	try {
		...
		const classification =
			await regulatoryFrameworkService.getUserClassification(userId);
		...
		if (!classification) {
			return res.json({
				...
				classification: null,
				message:
					"No classification found. Classification will be auto-determined on first investment.",
			});
		}
 ...
 */
router.get(
	"/brokerage-structure/:investorType/:productCategory",
	...
	try {
		...
		if (!investmentAmount || !investorType || !productCategory) {
			return res.status(400).json({
				error:
					"investmentAmount, investorType, and productCategory are required",
			});
		}

		const brokerageStructure =
			await regulatoryFrameworkService.getBrokerageStructure(
				investorType,
				productCategory,
			);
...
router.post("/check-eligibility", async (req: Request, res: Response) => {
	try {
		...
		const eligibility =
			await regulatoryFrameworkService.checkProductEligibility(
				userId,
				productCategory,
				investmentAmount,
				isin,
	...
	async (req: Request, res: Response) => {
		try {
			...
			const updatedProposal =
				await regulatoryFrameworkService.reviewOverrideProposal(
					id,
					reviewLevel,
					reviewedBy,
					decision,
			...
			res
				.status(500)
				.json({ error: error.message || "Failed to review proposal" });
		}
	},
);
 ...
 */
router.get(
	"/risk-disclosures/:productCategory",
	async (req: Request, res: Response) => {
		try {
			...
			const templates =
				await regulatoryFrameworkService.getRiskDisclosureTemplates(
					productCategory,
			...
			res
				.status(500)
				.json({ error: "Failed to fetch risk disclosure templates" });
		}
	},
);
