...
import { db } from "../db";
import { comprehensiveHoldings, portfolios, users } from "@shared/schema";
...
router.post("/create", async (req: Request, res: Response) => {
	try {
		...
		if (!userId || !holdings || !Array.isArray(holdings)) {
			return res
				.status(400)
				.json({ error: "userId and holdings array are required" });
		}
		...
		const stagedHoldings: StagedHolding[] = holdings.map((h: any) => {
			const holding: StagedHolding = {
				...
				units: Number.parseFloat(h.units) || undefined,
				averageCost:
					Number.parseFloat(h.averageCost) ||
					Number.parseFloat(h.avgPrice) ||
					undefined,
				currentPrice:
					Number.parseFloat(h.currentPrice) ||
					Number.parseFloat(h.currentNav) ||
					undefined,
				currentValue:
					Number.parseFloat(h.currentValue) ||
					Number.parseFloat(h.marketValue) ||
					undefined,
		...
		const totalValue = stagedHoldings.reduce(
		...
		console.log(
			`[Staging] Created session for user ${userId} with ${stagedHoldings.length} holdings`,
		);
...
router.post("/approve", async (req: Request, res: Response) => {
	try {
		...
		if (!userId || !holdingIds || !Array.isArray(holdingIds)) {
			return res
				.status(400)
				.json({ error: "userId and holdingIds array are required" });
		}
...
router.patch("/update", async (req: Request, res: Response) => {
	try {
		...
		if (!userId || !holdingId || !data) {
			return res
				.status(400)
				.json({ error: "userId, holdingId, and data are required" });
		}
		...
		if (updated.currentValue && updated.investedValue) {
			...
			updated.gainLossPercent =
				updated.investedValue > 0
					? ((updated.currentValue - updated.investedValue) /
							updated.investedValue) *
						100
					: 0;
		}
...
router.post("/sync", async (req: Request, res: Response) => {
	try {
		...
		const holdingsToSync = session.holdings.filter(
		...
		for (const holding of holdingsToSync) {
			try {
				const existingCheck = holding.isin
					? await db
							...
							.where(
								and(
									...
									holding.folioNumber
										? eq(
												comprehensiveHoldings.folio as any,
												holding.folioNumber,
											)
										: undefined,
								),
...
function updateSessionStatus(session: StagingSession) {
	const approved = session.holdings.filter(
		(h) => h.status === "approved" || h.status === "modified",
	).length;
