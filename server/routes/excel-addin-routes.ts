...
<OfficeApp
  ...
  <VersionOverrides xmlns="http://schemas.microsoft.com/office/taskpaneappversionoverrides" xsi:type="VersionOverridesV1_0">
	...
	res.setHeader(
		"Content-Disposition",
		'attachment; filename="fintekpro-addin-manifest.xml"',
	);
	res.send(xml);
});
...
router.get("/expiry/:symbol", async (req: Request, res: Response) => {
	try {
		...
		const n = req.query.n
			? Number.parseInt(req.query.n as string) - 1
			: undefined;
		if (n !== undefined) {
			...
			.map(([strike, { call, put }]) => ({
				...
				call_iv:
					call?.impliedVolatility != null
						? Number.parseFloat(call.impliedVolatility.toFixed(2))
						: null,
				call_change:
					call?.change != null
						? Number.parseFloat(call.change.toFixed(2))
						: null,
				...
				put_iv:
					put?.impliedVolatility != null
						? Number.parseFloat(put.impliedVolatility.toFixed(2))
						: null,
				put_change:
					put?.change != null ? Number.parseFloat(put.change.toFixed(2)) : null,
...
router.get(
	"/option/:symbol/:type/:strike/:expiry",
	async (req: Request, res: Response) => {
		try {
			...
			const sp = Number.parseFloat(strike);
			...
			const FIELD_MAP: Record<string, () => number | null> = {
				LTP: () => opt.lastPrice,
				OI: () => opt.openInterest,
				IV: () => Number.parseFloat(opt.impliedVolatility?.toFixed(4) ?? ""),
				CHANGE: () => Number.parseFloat(opt.change?.toFixed(2) ?? ""),
				CHANGE_PCT: () => Number.parseFloat(opt.pChange?.toFixed(4) ?? ""),
			...
			if (!fn)
				return res
					.status(400)
					.json({
						error: `Unknown field: ${field}. Valid: LTP OI IV CHANGE CHANGE_PCT VOLUME BID ASK BID_QTY ASK_QTY SPOT GREEKS`,
					});
