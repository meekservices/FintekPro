...
async function ensureTable() {
    ...
    `);
		...
		await db.execute(
			sql`ALTER TABLE agent_empanelments ADD COLUMN IF NOT EXISTS is_ca_qualified BOOLEAN DEFAULT FALSE`,
		);
		await db.execute(
			sql`ALTER TABLE agent_empanelments ADD COLUMN IF NOT EXISTS ca_membership_number TEXT`,
		);
		await db.execute(
			sql`ALTER TABLE agent_empanelments ADD COLUMN IF NOT EXISTS ca_verification_status TEXT DEFAULT 'unverified'`,
		);
		await db.execute(
			sql`ALTER TABLE agent_empanelments ADD COLUMN IF NOT EXISTS ca_verified_at TIMESTAMPTZ`,
		);
		await db.execute(
...
router.post("/step/1", requireAuth, async (req: Request, res: Response) => {
	try {
		...
		const { panVerified, panNumber, panName, aadhaarVerified, aadhaarLast4 } =
			req.body;
...
router.post("/step/2", requireAuth, async (req: Request, res: Response) => {
    ...
    `);
      ...
      `);
			...
			console.warn(
				"[AgentEmpanelment] Step 2 users mirror skipped:",
				mirrorErr?.message,
			);
		}
...
router.post("/step/3", requireAuth, async (req: Request, res: Response) => {
	try {
		...
		if (arnCode && !/^ARN[-\s]?\d{4,10}$/i.test(arnCode.trim())) {
			return res
				.status(400)
				.json({ error: "Invalid ARN format. Expected: ARN-XXXXX" });
		}
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (!validateAccountNumber(accountNumber)) {
				return res
					.status(400)
					.json({ error: "Invalid account number. Must be 9–18 digits." });
			}
			if (!validateIFSC(ifscCode)) {
				return res
					.status(400)
					.json({
						error: "Invalid IFSC code. Format: AAAA0NNNNNN (e.g. HDFC0001234)",
					});
			}
			...
			console.log(
				`[AgentEmpanelment] Bank verify: acct=****${accountNumber.slice(-4)} ifsc=${ifscCode.toUpperCase()} name="${accountHolderName}"`,
			);
			const result = await verifyBankAccountPennyDrop(
				accountNumber,
			...
			if (!result.success) {
				console.warn(
					`[AgentEmpanelment] Bank verify FAILED: ${result.errorMessage}`,
					result.providerResponse,
				);
			}

			const nameMatch = result.nameMatchScore
				? isNameMatchAcceptable(result.nameMatchScore)
				: false;
			...
			res.json({
				...
				message: verified
					...
					: result.success && !nameMatch
						...
						: result.errorMessage ||
							"Bank verification failed. Please check account details.",
			});
		} catch (err: any) {
			console.error("[AgentEmpanelment] Bank verify error:", err);
			res
				.status(500)
				.json({ success: false, verified: false, error: err.message });
		}
	},
);
...
router.post("/step/6", requireAuth, async (req: Request, res: Response) => {
	try {
		...
		const {
			pmlaDeclarationSigned,
			criminalRecordDeclaration,
			fatcaDeclarationSigned,
			codeOfConductAccepted,
			antiMisSellingAccepted,
		} = req.body;
		...
		if (
			...
			return res
				.status(400)
				.json({ error: "All compliance declarations are mandatory" });
		}
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			const { kycEnvironmentService } = await import(
				"../services/kyc-environment-service"
			);
			...
			const { sandboxKYCService } = await import(
				"../services/sandbox-kyc-service"
			);
			...
			console.log(
				`[AgentEmpanelment] Aadhaar OTP ${isMockFallback ? "MOCK (SANDBOX_BASE_URL not set)" : "sent"} for agent ${agentId}, ref: ${result.referenceId}`,
			);
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (!otp || !/^\d{6}$/.test(otp)) {
				return res
					.status(400)
					.json({ success: false, message: "Valid 6-digit OTP is required" });
			}
			...
			console.log(
				`[AgentEmpanelment] Aadhaar verified for agent ${agentId} (****${last4})`,
			);
			...
			console.error(
				"[AgentEmpanelment] Aadhaar OTP verify error:",
				err.message,
			);
			return res
				.status(400)
...
router.post("/submit", requireAuth, async (req: Request, res: Response) => {
	try {
		...
		const rows = await db.execute(
			sql`SELECT * FROM agent_empanelments WHERE agent_id = ${agentId}`,
		);
		if (rows.rows.length === 0)
			return res.status(404).json({ error: "Empanelment not found" });
		...
		if (!rec.pan_verified)
			return res
				.status(400)
				.json({ error: "PAN verification is required before submission" });
		if (!rec.bank_verified)
			return res
				.status(400)
				.json({
					error:
						"Bank account must be verified via penny drop before submission",
				});
		if (
			!rec.pmla_declaration_signed ||
			!rec.code_of_conduct_accepted ||
			!rec.fatca_declaration_signed
		) {
			return res
				.status(400)
				.json({ error: "All compliance declarations must be signed" });
		}
		if (rec.status === "submitted" || rec.status === "under_review") {
			return res
				.status(400)
				.json({ error: "Application already submitted and under review" });
		}
		if (rec.status === "approved") {
			return res
				.status(400)
				.json({ error: "Your empanelment is already approved" });
		}
    ...
    `);
		...
		res.json({
			success: true,
			message:
				"Empanelment application submitted. Admin will review within 2–3 business days.",
		});
	} catch (err: any) {
