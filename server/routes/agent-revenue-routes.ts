...
async function getAgentIdForUser(userId: string | undefined): Promise<string | null> {
  ...
  const agent = await db.select({ id: customerCareAgents.id })
...
async function calculateAgentAUM(clientIds: string[]): Promise<{ current: number; previous: number }> {
  ...
  if (portfolioIds.length === 0) {
    ...
    const totalFromPortfolios = clientPortfolios.reduce(
      (sum, p) => sum + parseFloat(p.totalValue || '0'), 0
    );
    return { current: totalFromPortfolios, previous: totalFromPortfolios * 0.9 };
  }
  
  // Sum current values from holdings
  const holdingsSum = await db
    .select({ total: sql<string>`COALESCE(SUM(CAST(current_value AS DECIMAL)), 0)` })
    .from(portfolioHoldings)
    .where(inArray(portfolioHoldings.portfolioId, portfolioIds));
  
  const currentAUM = parseFloat(holdingsSum[0]?.total || '0');
...
router.get("/api/agent/revenue/metrics/:period?", async (req, res) => {
  try {
    ...
    const pendingCommissions = commissions
      ...
      .reduce((sum, c) => sum + parseFloat(c.agentNetCommission || '0'), 0);
    ...
    const currentPeriodRevenue = commissions
      .filter(c => c.transactionDate && new Date(c.transactionDate) >= startDate)
      .reduce((sum, c) => sum + parseFloat(c.agentNetCommission || '0'), 0);
    
    const prevPeriodRevenue = commissions
      .filter(c => c.transactionDate && new Date(c.transactionDate) >= prevStartDate && new Date(c.transactionDate) < startDate)
      .reduce((sum, c) => sum + parseFloat(c.agentNetCommission || '0'), 0);
...
router.get("/api/agent/revenue/product-mix", async (req, res) => {
  try {
    ...
    const totalCommission = commissionsByProduct.reduce(
      (sum, p) => sum + parseFloat(p.totalCommission || '0'), 0
    );
    
    const productMix = commissionsByProduct.map(p => {
      const config = productConfig[p.productType] || { name: p.productType, color: '#6b7280' };
      const commission = parseFloat(p.totalCommission || '0');
      return {
        name: config.name,
        value: totalCommission > 0 ? Math.round((commission / totalCommission) * 100) : 0,
        color: config.color,
...
router.get("/api/agent/revenue/trends/:period?", async (req, res) => {
  try {
    ...
    // Build trends array
    const commissionMap = new Map(monthlyCommissions.map(c => [c.month, parseFloat(c.revenue || '0')]));
    const aumMap = new Map(portfolioData.map(p => [p.month, parseFloat(p.aum || '0')]));
    
    // Calculate cumulative client count per month
    let cumulativeClients = clientIds.length;
...
router.get("/api/agent/revenue/commissions", async (req, res) => {
  try {
    ...
    for (const row of commissionData) {
      const existing = productMap.get(row.productType) || { pending: 0, realized: 0 };
    ...
    const commissions = Array.from(productMap.entries()).map(([productType, data]) => ({
