...
router.get(
	"/unlisted-stocks/recommendations",
...
router.post(
	"/unlisted-stocks/populate-sectors",
	async (req: Request, res: Response): Promise<void> => {
		try {
...
router.post(
	...
	async (req: Request, res: Response): Promise<void> => {
		try {
			...
			console.log(
				"[Agent CAS Parse] Using unified service:",
			...
			try {
				...
				const msg =
					lotsError instanceof Error ? lotsError.message : String(lotsError);
			...
			const holdings = importResult.holdings.map((h, idx) => ({
				...
				lotSummary:
					h.lotSummary ||
					`${h.lotCount || 0} lot${(h.lotCount || 0) !== 1 ? "s" : ""}`,
			...
			const calculatedValue = holdings.reduce(
				(sum, h) => sum + h.currentValue,
				0,
			);
			const calculatedInvested = holdings.reduce(
				(sum, h) => sum + h.investedValue,
				0,
			);
			const totalLots = holdings.reduce(
				(sum, h) => sum + (h.lots?.length || 0),
				0,
			);
			const authoritativeMarketValue =
				importResult.portfolioSummary?.totalMarketValue || calculatedValue;
			const authoritativeCostValue =
				importResult.portfolioSummary?.totalCostValue || calculatedInvested;

			console.log(
				`[Agent CAS Parse] SUCCESS: ${holdings.length} holdings, ${totalLots} lots, Value: ₹${(authoritativeMarketValue / 100000).toFixed(2)} L`,
			);
			...
			res.json({
				...
				summary: {
					...
					hasReconciliationDelta:
						Math.abs(calculatedValue - authoritativeMarketValue) >
						authoritativeMarketValue * 0.005,
				},
		...
		try {
			...
			if (returns) {
				...
				res
					...
					.json({
						success: false,
						error: "Fund not found or no data available",
					});
			}
		} catch (error: unknown) {
			const msg = error instanceof Error ? error.message : String(error);
