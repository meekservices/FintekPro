...
export function registerBankAccountsDemaPart2Routes(app: Express): void {
	...
	app.put("/api/demat-accounts/:id/set-default", async (req, res) => {
		try {
			...
			const success = await storage.setDefaultDematAccount(
				req.params.id,
				defaultType,
			);
			if (success) {
				res.json({
					success: true,
					message: "Default demat account updated successfully",
				});
			} else {
	...
	app.get("/api/product-account-preferences", async (req, res) => {
		try {
			...
			const preferences = await storage.getUserProductAccountPreferences(
				req.user!.id,
			);
			...
			res
				.status(500)
				.json({ error: "Failed to fetch product account preferences" });
		}
	});
	...
	app.get("/api/product-account-preferences/:productType", async (req, res) => {
		try {
			...
			res
				.status(500)
				.json({ error: "Failed to fetch product account preference" });
		}
	});
	...
	app.post("/api/product-account-preferences", async (req, res) => {
		try {
			...
			const validatedData = insertProductAccountPreferenceSchema.parse(
				req.body,
			);
			...
			const preference =
				await storage.createProductAccountPreference(preferenceData);
			...
			res
				.status(500)
				.json({ error: "Failed to create product account preference" });
		}
	});
	...
	app.put("/api/product-account-preferences/:id", async (req, res) => {
		try {
			...
			if (!existingPref) {
				return res
					.status(404)
					.json({ error: "Preference not found or access denied" });
			}

			// Validate request body (partial schema - only allow specific fields)
			const updateSchema = insertProductAccountPreferenceSchema
				.pick({
					bankAccountId: true,
					dematAccountId: true,
				})
			...
			const updated = await storage.updateProductAccountPreference(
				req.params.id,
				validatedData,
			);
			...
			res
				.status(500)
				.json({ error: "Failed to update product account preference" });
		}
	});
	...
	app.delete("/api/product-account-preferences/:id", async (req, res) => {
		try {
			...
			const deleted = await storage.deleteProductAccountPreference(
				req.params.id,
			);
			...
			res
				.status(500)
				.json({ error: "Failed to delete product account preference" });
		}
	});
	...
	app.post("/api/product-accounts/:productType/validate", async (req, res) => {
		try {
			...
			const { bankAccountId, dematAccountId } = validationSchema.parse(
				req.body,
			);

			// Check if product type is valid
			const requirements = productAccountService.getProductRequirements(
				req.params.productType,
			);
			if (!requirements) {
		...
		try {
			...
			if (level !== "2") {
				return res.status(403).json({
					...
					message:
						"You must complete full KYC verification before creating a UCC for mutual fund trading",
					kycLevel: level,
					requiredLevel: "2",
					nextStep: {
						...
						url: "/onboarding",
						description:
							"Complete CKYC and KRA verification to unlock mutual fund trading",
					},
				});
			}
			...
			if (
				...
				return res.status(400).json({
					...
					message:
						"PAN number, first name, last name, date of birth, gender, mobile, and email are required",
				});
			}
			...
			if (!bankAccountNumber || !bankIfscCode || !bankAccountType) {
				return res.status(400).json({
					...
					message:
						"Bank account number, IFSC code, and account type are required",
				});
			}
