	...
	app.post(
		...
		async (
			req: Request<
				any,
				any,
				{ name?: string; panNumber?: string; email?: string; mobile?: string }
			>,
			res: Response,
		): Promise<void> => {
			try {
				...
				// Cast to any to bypass strict Zod/Drizzle insert types for this shortcut, but maintaining safety where possible
		...
		async (
			...
			try {
				...
				if (!Array.isArray(holdings)) {
					res
						.status(400)
						.json({ status: "error", error: "Holdings must be an array" });
					return;
				}
				...
				const enhancedHoldings = await Promise.all(
					holdings.map(async (holding) => {
						try {
							...
							return {
								id: `holding-${Date.now()}-${Math.random().toString(36).substring(2, 11)}`,
								...
								investedValue,
								gainLoss,
								gainLossPercent: Number.parseFloat(gainLossPercent.toFixed(2)),
							...
							const currentPrice =
								holding.avgPrice * (1 + (Math.random() - 0.5) * 0.1); // ±5% random variation
							const quantity = holding.quantity || 100;
							const currentValue = currentPrice * quantity;
							const investedValue = holding.avgPrice * quantity;
							const gainLoss = currentValue - investedValue;
							...
							return {
								...
								gainLoss: Number.parseFloat(gainLoss.toFixed(2)),
								gainLossPercent: Number.parseFloat(gainLossPercent.toFixed(2)),
								sector: holding.sector || "Technology",
								lastUpdated: new Date().toISOString(),
							};
				...
				const totalCurrentValue = enhancedHoldings.reduce(
					(sum, h) => sum + h.currentValue,
					0,
				);
				const totalInvestedValue = enhancedHoldings.reduce(
					(sum, h) => sum + h.investedValue,
					0,
				);
				const totalGainLoss = totalCurrentValue - totalInvestedValue;
				const totalGainLossPercent =
					(totalGainLoss / totalInvestedValue) * 100 || 0;

				const portfolio = {
					id: portfolioId,
					holdings: enhancedHoldings,
					summary: {
						totalCurrentValue: Number.parseFloat(totalCurrentValue.toFixed(2)),
						totalInvestedValue: Number.parseFloat(
							totalInvestedValue.toFixed(2),
						),
						totalGainLoss: Number.parseFloat(totalGainLoss.toFixed(2)),
						totalGainLossPercent: Number.parseFloat(
				...
				console.log(
					"Portfolio created with real-time data:",
					portfolio.summary,
				);
	...
	app.post(
		...
		async (_req: Request, res: Response): Promise<void> => {
			try {
				...
				const amcPartners = [
					{
						...
						phone: "1800-425-6969",
						address:
							"9th Floor, Nehru Centre, Dr. Annie Besant Road, Worli, Mumbai 400018",
					...
					},
					{
						...
						phone: "1800-425-4332",
						address:
							"2nd Floor, HDFC House, 165-166, Backbay Reclamation, H.T. Parekh Marg, Mumbai 400020",
					...
					},
					{
						...
						phone: "1800-222-999",
						address:
							"ICICI Prudential Asset Management Company Limited, One BKC, C 66, G Block, Bandra Kurla Complex, Mumbai 400051",
					...
					},
					{
						...
						phone: "1800-425-0060",
						address:
							"Ground Floor, Axis House, C-2, Wadia International Centre, Pandurang Budhkar Marg, Mumbai 400025",
					...
					},
					{
						...
						code: "ABSL",
						description:
							"Joint venture between Aditya Birla Group and Sun Life Financial Inc",
						website: "https://www.sunlifeindia.com",
						email: "customercare@sunlifeindia.com",
						phone: "1800-270-7000",
						address:
							"One World Center, Tower 1, 841, Senapati Bapat Marg, Elphinstone Road, Mumbai 400013",
					...
					},
					{
						...
						code: "NIPPON",
						description:
							"Formerly known as Reliance Nippon Life Asset Management",
						website: "https://mf.nipponlife.in",
						email: "customercare@nipponlife.in",
						phone: "1800-266-7777",
						address:
							"6th Floor, Tower A, Peninsula Business Park, Ganpatrao Kadam Marg, Lower Parel, Mumbai 400013",
					...
					},
					{
						...
						phone: "1800-222-626",
						address:
							"1st Floor, 27 BKC, Plot No. C-12, G-Block, Bandra Kurla Complex, Mumbai 400051",
					...
					},
					{
						...
						phone: "1800-425-4255",
						address:
							"7th Floor, Brigade Seshadri Iyer Memorial Building, 4/1, Cubbon Road, Bangalore 560001",
					...
					},
					{
						...
						code: "DSP",
						description:
							"Leading asset management company with BlackRock partnership",
					...
					},
					{
						...
						phone: "1800-425-0101",
						address:
							"24th Floor, Platinum Techno Park, Plot No. 17/18, Sector 30A, Vashi, Navi Mumbai 400705",
				...
				console.log(
					`Loaded ${amcPartners.length} AMC partners:`,
					amcPartners.map((amc) => amc.name),
				);
				...
				res.status(201).json({
					...
					summary: {
						...
						totalAUM: amcPartners.reduce(
							(sum, amc) => sum + Number.parseInt(amc.aum.replace(/,/g, "")),
							0,
						),
						totalSchemes: amcPartners.reduce(
							(sum, amc) => sum + amc.schemes,
							0,
						),
						activePartners: amcPartners.filter((amc) => amc.status === "active")
							.length,
					},
				});
			} catch (error: unknown) {
