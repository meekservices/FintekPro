...
const FEMA_ALLOWED_ROLES: RoleId[] = [
	"superadmin",
	"admin",
	"compliance_officer",
	"master_agent",
...
router.get(
	"/purpose-codes",
	requireAuth,
	requireRole(FEMA_ALLOWED_ROLES),
	async (req: Request, res: Response) => {
		try {
			...
			res
				.status(500)
				.json({ success: false, error: "Failed to fetch purpose codes" });
		}
	},
);
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (!purposeCode) {
				return res
					.status(404)
					.json({ success: false, error: "Purpose code not found" });
			}
			...
			res
				.status(500)
				.json({ success: false, error: "Failed to fetch purpose code" });
		}
	},
);

router.post(
	"/purpose-codes/validate",
	requireAuth,
	requireRole(FEMA_ALLOWED_ROLES),
	async (req: Request, res: Response) => {
		try {
			...
			if (!purposeCode) {
				return res
					.status(400)
					.json({ success: false, error: "Purpose code is required" });
			}

			const validation = femaComplianceService.validatePurposeCode(
				purposeCode,
				amountUSD || 0,
			);
			...
			res
				.status(500)
				.json({ success: false, error: "Failed to validate purpose code" });
		}
	},
);
...
router.get(
	...
	async (req: Request, res: Response) => {
		try {
			...
			const status = await femaComplianceService.getLRSStatus(
				userId,
			...
			res
				.status(500)
				.json({ success: false, error: "Failed to fetch LRS status" });
		}
	},
);
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (!amountUSD || amountUSD <= 0) {
				return res
					.status(400)
					.json({ success: false, error: "Valid amount in USD is required" });
			}

			const eligibility = await femaComplianceService.checkLRSEligibility(
				userId,
				amountUSD,
			);
			...
			res
				.status(500)
				.json({ success: false, error: "Failed to check LRS eligibility" });
		}
	},
);
		...
		try {
			...
			if (!remittanceAmountINR || remittanceAmountINR <= 0) {
				return res
					...
					.json({
						success: false,
						error: "Valid remittance amount in INR is required",
					});
			}
			...
			res
				.status(500)
				.json({ success: false, error: "Failed to calculate TCS" });
		}
	},
);
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			res
				.status(500)
				.json({ success: false, error: "Failed to generate A2 form" });
		}
	},
);
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (!form) {
				return res
					.status(404)
					.json({ success: false, error: "A2 form not found" });
			}
			...
			res
				.status(500)
				.json({ success: false, error: "Failed to fetch A2 form" });
		}
	},
);

router.post(
	"/a2-form/:formNumber/submit",
	requireAuth,
	requireRole(FEMA_ALLOWED_ROLES),
	async (req: Request, res: Response) => {
		try {
			...
			res
				.status(500)
				.json({ success: false, error: "Failed to submit A2 form" });
		}
	},
);
		...
		try {
			...
			if (!adBankName || !adCode || !applicantPan || !purposeCode) {
				return res.status(400).json({
					success: false,
					error:
						"AD bank details, applicant PAN, and purpose code are required",
				});
			}
			...
			res
				.status(500)
				.json({ success: false, error: "Failed to generate AD certificate" });
		}
	},
);

router.get(
	"/ad-certificate/:certificateNumber",
	requireAuth,
	requireRole(FEMA_ALLOWED_ROLES),
	async (req: Request, res: Response) => {
		try {
			...
			const certificate =
				await femaComplianceService.getADCertificate(certificateNumber);

			if (!certificate) {
				return res
					.status(404)
					.json({ success: false, error: "AD certificate not found" });
			}
			...
			res
				.status(500)
				.json({ success: false, error: "Failed to fetch AD certificate" });
		}
	},
);

router.post(
	"/ad-certificate/:certificateNumber/validate",
	requireAuth,
	requireRole(FEMA_ALLOWED_ROLES),
	async (req: Request, res: Response) => {
		try {
			...
			const validation =
				await femaComplianceService.validateADCertificate(certificateNumber);
			...
			res
				.status(500)
				.json({ success: false, error: "Failed to validate AD certificate" });
		}
	},
);
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			res
				.status(500)
				.json({ success: false, error: "Failed to record transaction" });
		}
	},
);
...
router.get(
	...
	async (req: Request, res: Response) => {
		try {
			...
			const report = await femaComplianceService.generateComplianceReport(
				userId,
				financialYear as string,
			);
