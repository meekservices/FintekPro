...
} from "../services/sebi-audit-service";
...
router.post("/simulate", async (req: Request, res: Response) => {
	try {
		const schema = z.object({
			...
			horizonMonths: z.enum(["6", "12", "24"]).transform(Number) as z.ZodType<
				6 | 12 | 24
			>,
		});
		...
		await sebiAuditService.log(
			{
				...
				inputData: {
					...
					fundCount: input.candidateFunds.length,
				},
				outputData: {
...
router.post("/training-prompts", async (req: Request, res: Response) => {
	try {
		...
		res.json({
			success: true,
			data: { prompts, totalPrompts: prompts.length },
		});
	} catch (error: any) {
