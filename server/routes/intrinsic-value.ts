 ...
 */
router.get('/stocks/:symbol/intrinsic-value', async (req: Request, res: Response) => {
  try {
    ...
    console.log(`[IntrinsicValue API] Calculating for listed stock: ${symbol}`);
    
    const result = await intrinsicValueCalculator.calculateListedStockValue(symbol.toUpperCase());
 ...
 * Returns all valuation methods with confidence scores and audit trail
 */
router.get('/unlisted/:companyId/intrinsic-value', async (req: Request, res: Response) => {
  try {
    ...
    console.log(`[IntrinsicValue API] Calculating for unlisted company: ${companyId}`);
    
    const result = await intrinsicValueCalculator.calculateUnlistedStockValue(companyId);
    ...
    methods: [
      {
        ...
        description: 'Projects future free cash flows and discounts them to present value using WACC',
        formula: 'EV = Σ(FCF_t / (1 + WACC)^t) + TV / (1 + WACC)^n',
        inputs: ['Free Cash Flow history', 'WACC components (beta, risk-free rate, equity risk premium)', 'Terminal growth rate'],
        bestFor: 'Companies with stable, positive cash flows',
        limitations: 'Sensitive to growth rate and WACC assumptions; not suitable for negative FCF companies',
      },
      {
        id: 'graham',
        name: "Graham's Intrinsic Value Formula",
        description: 'Benjamin Graham\'s formula relating earnings, growth, and bond yields',
        formula: 'V = EPS × (8.5 + 2g) × (4.4/Y)',
        inputs: ['EPS (Earnings Per Share)', 'Expected growth rate (g)', 'AAA bond yield (Y)'],
        bestFor: 'Value investing approach for stable companies',
        limitations: 'Assumes linear relationship between growth and value; may overvalue high-growth stocks',
      },
      {
        id: 'relative',
        ...
        limitations: 'Assumes sector is correctly valued; ignores company-specific factors',
      },
      {
        id: 'book_value',
        name: 'Book Value / NAV Approach',
        description: 'Values company based on net assets with margin of safety',
        formula: 'Intrinsic = (Assets - Liabilities - Intangibles) / Shares × (1 - MoS)',
        inputs: ['Total Assets', 'Total Liabilities', 'Intangible Assets', 'Shares Outstanding'],
        bestFor: 'Asset-heavy companies, financial institutions, liquidation value',
        limitations: 'Ignores earning power and growth potential; book values may not reflect market values',
      },
    ],
    constants: {
...
router.post('/intrinsic-value/batch', async (req: Request, res: Response) => {
  try {
    ...
    const results = await Promise.all(
      stocks.map(async (stock: { symbol?: string; companyId?: string; type: 'listed' | 'unlisted' }) => {
        try {
          if (stock.type === 'unlisted' && stock.companyId) {
            return await intrinsicValueCalculator.calculateUnlistedStockValue(stock.companyId);
          } else if (stock.symbol) {
            return await intrinsicValueCalculator.calculateListedStockValue(stock.symbol.toUpperCase());
          }
          return null;
        } catch (error) {
          console.error(`[IntrinsicValue Batch] Error for ${stock.symbol || stock.companyId}:`, error);
          return null;
        }
      })
    );
