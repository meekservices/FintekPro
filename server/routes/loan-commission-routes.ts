...
function requireAdminAuth(req: Request, res: Response, next: NextFunction) {
	...
	if (!userRole || !ADMIN_ROLES.includes(userRole)) {
		return res
			...
			.json({
				success: false,
				error: "Admin role required to access commission data",
			});
	}
	next();
}
...
router.get("/rates/:productType", async (req: Request, res: Response) => {
	try {
		...
		const config = loanCommissionService.getCommissionRateConfig(
...
router.post("/calculate", async (req: Request, res: Response) => {
	try {
		...
		const result = loanCommissionService.calculateCommission(
			...
			productType as LoanProductType,
			customRate ? Number.parseFloat(customRate) : undefined,
			customShares,
		);
...
router.get("/ledger", async (req: Request, res: Response) => {
	try {
		const { status, providerId, startDate, endDate, partnerId, agentId } =
			req.query;
	...
	try {
		...
		if (!applicationId || !providerId || !productType || !loanAmount) {
			return res.status(400).json({
				success: false,
				error:
					"applicationId, providerId, productType, and loanAmount are required",
			});
		}
	...
	try {
		...
		if (!applicationId || !sanctionedAmount || !bankCode || !productType) {
			return res.status(400).json({
				success: false,
				error:
					"applicationId, sanctionedAmount, bankCode, and productType are required",
			});
		}

		const entry = await loanCommissionService.onLoanSanctioned(
			applicationId,
			Number.parseFloat(sanctionedAmount),
			bankCode,
			productType as LoanProductType,
			disbursementDate ? new Date(disbursementDate) : undefined,
			partnerId,
	...
	try {
		...
		if (!applicationId || !disbursedAmount || !disbursementDate) {
			return res.status(400).json({
				success: false,
				error:
					"applicationId, disbursedAmount, and disbursementDate are required",
			});
		}

		await loanCommissionService.onLoanDisbursed(
			applicationId,
			Number.parseFloat(disbursedAmount),
...
router.post("/transformers/preview", async (req: Request, res: Response) => {
	try {
		...
		const transformed = payloadTransformerFactory.transform(
			bankCode,
			canonical,
		);
