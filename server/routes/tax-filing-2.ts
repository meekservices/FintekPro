...
export function registerTaxFilingPart2Routes(app: Express): void {
	...
	app.put("/api/tax/documents/:documentId", async (req, res) => {
		try {
			...
			const updatedDocument = await storage.updateTaxDocument(
				documentId,
				updates,
			);
	...
	app.get("/api/tax-rules/:ruleType/:category", async (req, res) => {
		try {
			...
			const rule = await storage.getTaxRule(
				ruleType,
				category,
				calculationDate,
			);
	...
	app.post("/api/tax/calculate-capital-gains", async (req, res) => {
		try {
			...
			const { stcgAmount, ltcgAmount, financialYear, calculationDate } =
				validation.data;
			const calcDate = calculationDate ? new Date(calculationDate) : new Date();

			// Fetch STCG rate from database
			const stcgRule = await storage.getTaxRule(
				"capital_gains",
				"stcg",
				calcDate,
			);
			if (!stcgRule) {
				return res.status(404).json({
					error:
						"STCG tax rate not found in database. Please contact administrator.",
				});
			}

			// Fetch LTCG rate and exemption from database
			const ltcgRateRule = await storage.getTaxRule(
				"capital_gains",
				"ltcg_rate",
				calcDate,
			);
			const ltcgExemptionRule = await storage.getTaxRule(
				"capital_gains",
				"ltcg_exemption",
				calcDate,
			);
			...
			if (!ltcgRateRule || !ltcgExemptionRule) {
				return res.status(404).json({
					error:
						"LTCG tax rate or exemption not found in database. Please contact administrator.",
				});
			}

			// Parse rates from database (stored as decimal)
			const stcgRate = Number.parseFloat(stcgRateRule.value) / 100; // Convert percentage to decimal
			const ltcgRate = Number.parseFloat(ltcgRateRule.value) / 100;
			const ltcgExemption = Number.parseFloat(ltcgExemptionRule.value);
			...
			res.json({
				...
				calculation: {
					...
					stcgTax: Number.parseFloat(stcgTax.toFixed(2)),
					ltcgTax: Number.parseFloat(ltcgTax.toFixed(2)),
					totalTax: Number.parseFloat(totalTax.toFixed(2)),
					breakdown: {
						stcg: {
							amount: stcgAmount,
							rate: Number.parseFloat(stcgRateRule.value),
							ratePercentage: `${stcgRateRule.value}%`,
							tax: Number.parseFloat(stcgTax.toFixed(2)),
						...
						ltcg: {
							...
							rate: Number.parseFloat(ltcgRateRule.value),
							ratePercentage: `${ltcgRateRule.value}%`,
							tax: Number.parseFloat(ltcgTax.toFixed(2)),
	...
	app.post("/api/tax/calculate-income-tax", async (req, res) => {
		try {
			...
			slabs.sort(
				(a: any, b: any) =>
					Number.parseFloat(a.minAmount || "0") -
					Number.parseFloat(b.minAmount || "0"),
			);
			...
			for (let i = 0; i < slabs.length; i++) {
				const slab = slabs[i];
				const minAmount = Number.parseFloat(slab.minAmount || "0");
				const maxAmount = slab.maxAmount
					? Number.parseFloat(slab.maxAmount)
					: Number.POSITIVE_INFINITY;
				const rate = Number.parseFloat(slab.value) / 100;
				...
				if (taxableIncome > minAmount) {
					...
					slabBreakdown.push({
						minAmount,
						maxAmount:
							maxAmount === Number.POSITIVE_INFINITY ? null : maxAmount,
						rate: Number.parseFloat(slab.value),
						ratePercentage: `${slab.value}%`,
						taxableAmount: Number.parseFloat(taxableInSlab.toFixed(2)),
						tax: Number.parseFloat(taxInSlab.toFixed(2)),
					});
		...
		try {
			...
			try {
				const activeItrCases = await db
					.select({
						id: schema.agentItrCases.id,
						status: schema.agentItrCases.status,
						caId: schema.agentItrCases.caId,
					})
				...
				hasExpertService =
					activeItrCases.length > 0 && activeItrCases[0].caId !== null;

				if (hasExpertService) {
					console.info(
						`[Tax Reminder] User ${userId} has active CA-assisted ITR case - eligible for free tier`,
					);
				}
			} catch (error: any) {
				console.warn(
					"[Tax Reminder] Failed to check expert service status:",
					error.message,
				);
			...
			res.json({
				...
				subscription: {
					...
					annualPrice: Number.parseFloat(subscription.annualPrice),
			...
			res
				.status(500)
				.json({ error: "Failed to create tax reminder subscription" });
		}
	});
	...
	app.get("/api/tax/reminder-subscription/:userId", async (req, res) => {
		try {
			...
			res
				.status(500)
				.json({ error: "Failed to fetch tax reminder subscription" });
		}
	});
	...
	app.post("/api/tax/calculate-portfolio-gains/:userId", async (req, res) => {
		try {
			...
			const { capitalGainsCalculator } = await import(
				"../services/capital-gains-calculator"
			);
			const gainsBreakdown =
				await capitalGainsCalculator.calculatePortfolioGains(userId);
	...
	app.get("/api/tax/capital-gains-reminders/:userId", async (req, res) => {
		try {
			...
			const reminders =
				await capitalGainsCalculator.getRemindersForUser(userId);
			...
			const formattedReminders = reminders.map((reminder: any) => ({
				...
				estimatedSTCG: Number.parseFloat(reminder.estimatedSTCG || "0"),
				estimatedLTCG: Number.parseFloat(reminder.estimatedLTCG || "0"),
				totalTaxLiability: Number.parseFloat(reminder.totalTaxLiability || "0"),
	...
	app.post(
		"/api/tax/generate-quarterly-reminders/:userId",
		async (req, res) => {
			try {
				const { userId } = req.params;
				...
				if (!financialYear) {
					return res
						.status(400)
						.json({ error: "Financial year is required (format: YYYY-YY)" });
				}
				...
				if (!subscription || subscription.subscriptionStatus !== "active") {
					return res.status(403).json({
						error:
							"Active tax reminder subscription required to generate reminders",
				...
				const reminders =
					await capitalGainsCalculator.generateQuarterlyReminders(
						userId,
						subscription.id,
						financialYear,
					);
