...
async function fetchQuote(symbol: string): Promise<{ changePercent: number; currentPrice: number } | null> {
  try {
  ...
  try {
    ...
    if (cached && cached.expiresAt > Date.now()) {
      const data = filterClientId
        ? { ...cached.data, alerts: cached.data.alerts.filter((a: any) => a.clients.some((c: any) => c.clientId === filterClientId)) }
        : cached.data;
    ...
    if (agentClients.length === 0) {
      return res.json({ generatedAt: new Date().toISOString(), alerts: [], cached: false });
    }
    ...
    const symbolClientMap = new Map<string, { symbol: string; name: string; clients: { clientId: string; clientName: string; holdingValue: number; phone?: string }[] }>();
    ...
    for (const h of compH) {
      ...
      if (!client) continue;
      if (!symbolClientMap.has(sym)) symbolClientMap.set(sym, { symbol: sym, name: h.assetName, clients: [] });
    ...
    for (const h of pfH) {
      ...
      if (!client) continue;
      if (!symbolClientMap.has(sym)) symbolClientMap.set(sym, { symbol: sym, name: h.name || sym, clients: [] });
      symbolClientMap.get(sym)!.clients.push({
        clientId: userId || "",
        clientName: `${client.firstName || ""} ${client.lastName || ""}`.trim(),
    ...
    const result = { generatedAt: new Date().toISOString(), alerts, cached: false };
