...
router.get("/public/proposal/:token", async (req: Request, res: Response) => {
	try {
		const proposal = await agentProspectWizardService.getProposalByToken(
			req.params.token,
		);
		if (!proposal) {
			return res
				.status(404)
				.json({ success: false, message: "Proposal not found or expired" });
		}
			...
			try {
				...
				if (prospect.convertedUserId) {
					portfolioConditions.push(
						eq(portfolios.userId, prospect.convertedUserId),
					);
				}
					...
					if (rawHoldings.length > 0) {
						...
						holdings = rawHoldings.map((h) => ({
							...
							averageCost: h.avgPrice
								? Number.parseFloat(h.avgPrice.toString())
								: undefined,
							currentValue: Number.parseFloat(
								h.currentValue?.toString() || "0",
							),
							investedValue: h.investedValue
								? Number.parseFloat(h.investedValue.toString())
								: undefined,
						...
						console.log(
							`[Agent Wizard] Loaded ${holdings.length} holdings from real portfolio for client ${prospect.convertedUserId} (prospect ${req.params.id})`,
						);
					}
				}
			} catch (fallbackErr) {
				console.warn(
					"[Agent Wizard] Real portfolio fallback failed, returning empty:",
					fallbackErr,
				);
			}
		}
