...
export function registerTaxPart1Routes(app: Express): void {
	...
	app.post("/api/tax/session", async (req, res) => {
		try {
			...
			if (!panNumber || !assessmentYear) {
				return res
					.status(400)
					.json({ error: "PAN number and assessment year are required" });
			}

			const existingSession = await taxOrchestrator.getSessionSummary(
				`${req.user.id}-${panNumber}-${assessmentYear}`,
			);
			...
			const session = await taxOrchestrator.createSession({
				...
				panNumber,
				assessmentYear,
				financialYear: `${Number.parseInt(assessmentYear.split("-")[0]) - 1}-${Number.parseInt(assessmentYear.split("-")[1]) - 1}`,
			});
	...
	app.post("/api/tax/session/:sessionId/initialize", async (req, res) => {
		try {
			const { sessionId } = req.params;
			const dataSources =
				await taxOrchestrator.initializeDataSources(sessionId);
	...
	app.post("/api/tax/session/:sessionId/optimize", async (req, res) => {
		try {
			const { sessionId } = req.params;
			const suggestions =
				await taxOrchestrator.generateOptimizationSuggestions(sessionId);
			...
			res
				.status(500)
				.json({ error: "Failed to generate optimization suggestions" });
		}
	});
	...
	app.post("/api/tax/session/:sessionId/file", async (req, res) => {
		try {
			...
			const filingRecord = await taxOrchestrator.submitFiling(
				sessionId,
				itrJson,
				verificationMethod,
			);
	...
	app.get(
		"/api/sandbox-itr/form-26as/:pan/:assessmentYear",
		async (req, res) => {
			try {
	...
	app.get("/api/sandbox-itr/entity-types", async (req, res) => {
		try {
			const entityTypes = {
				individual: {
					name: "Individual",
					description: "Resident/Non-resident individual",
					applicableForms: ["ITR-1", "ITR-2", "ITR-3", "ITR-4"],
				},
				huf: {
					name: "Hindu Undivided Family",
					description: "HUF entities",
					applicableForms: ["ITR-2", "ITR-3", "ITR-4"],
				},
				partnership: {
					name: "Partnership Firm",
					description: "Partnership firms and LLPs",
				...
				trust: {
					name: "Trust/Society",
					description: "Charitable trusts, educational institutions",
					applicableForms: ["ITR-7"],
				},
				local_authority: {
					name: "Local Authority",
					description: "Municipal corporations, panchayats",
					applicableForms: ["ITR-7"],
				},
			...
			res
				.status(500)
				.json({ success: false, message: "Failed to retrieve entity types" });
		}
	});
	...
	app.get("/api/sandbox-itr/test-data", async (_req, res) => {
		try {
			...
			res
				.status(500)
				.json({ success: false, message: "Failed to retrieve test data" });
		}
	});
