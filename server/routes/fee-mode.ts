...
router.get("/calculate-fees", async (req: Request, res: Response) => {
	try {
		...
		const orderValue = Number.parseFloat(req.query.orderValue as string);
		if (Number.isNaN(orderValue) || orderValue <= 0) {
			return res.status(400).json({ error: "Invalid order value" });
		}

		const feeBreakdown = await clientFeeModeService.calculateFees(
			orderValue,
			userId,
		);
		if (!feeBreakdown) {
...
router.get(
	"/admin/export/:clientId",
	requireAdmin,
	async (req: Request, res: Response) => {
		try {
			const { clientId } = req.params;
			const exportBundle =
				await clientFeeModeService.generateSebiExportBundle(clientId);
