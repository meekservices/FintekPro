...
export function registerMFMonthwiPart3Routes(app: Express): void {
	...
	app.post("/api/cdsl/edis/consent", async (req, res) => {
		try {
			const { boId, isin, quantity, clientCode, executionDate, tpin } =
				req.body;
	...
	app.post("/api/cdsl/pledge/create", async (req, res) => {
		try {
			const { boId, isin, quantity, pledgeeClientCode, pledgeReason, tpin } =
				req.body;
			...
			const pledgeData = {
				...
				pledgeValue: (Number.parseFloat(quantity) * 1200).toString(), // Simulated value
				closureDate: null,
	...
	app.post("/api/cdsl/elas/pledge", async (req, res) => {
		try {
			const { boId, securities, lenderCode, loanAmount, purpose, tpin } =
				req.body;
			...
			const elasPledge = {
				...
				pledgeDate: new Date().toISOString().split("T")[0],
				securities,
				eligibleAmount: (Number.parseFloat(loanAmount) * 0.8).toString(), // 80% LTV
				interestRate: "11.5%",
				tenure: "12 months",
			};
	...
	app.post("/api/cdsl/margin-pledge/create", async (req, res) => {
		try {
			...
			const marginPledge = {
				...
				status: "ACTIVE",
				pledgeDate: new Date().toISOString().split("T")[0],
				marginValue: (Number.parseFloat(quantity) * 900).toString(), // Simulated margin value
				haircut: "20%",
				availableMargin: (Number.parseFloat(quantity) * 720).toString(),
			};
	...
	app.post("/api/cdsl/early-payin", async (req, res) => {
		try {
			const { boId, isin, quantity, tradeDate, settlementCycle, tpin } =
				req.body;
			...
			const earlyPayin = {
				...
				status: "CONFIRMED",
				marginBenefit: "15%",
				benefitAmount: (Number.parseFloat(quantity) * 150).toString(),
	...
	app.post("/api/market/story/sentiment", async (req, res) => {
		try {
			...
			if (text.length > 5000) {
				return res
					.status(400)
					.json({ error: "Text is too long (max 5000 characters)" });
			}
