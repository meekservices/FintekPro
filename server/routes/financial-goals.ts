...
export function registerFinancialGoalsRoutes(app: Express): void {
	...
	app.post("/api/financial-goals", async (req, res) => {
		try {
			...
			if (!goalData.name || !goalData.targetAmount || !goalData.targetDate) {
				return res
					.status(400)
					.json({ error: "Name, target amount, and target date are required" });
			}
	...
	app.get("/api/recommendations/goal/:goalId", async (req, res) => {
		try {
			...
			const recommendations =
				await storage.generateGoalBasedRecommendations(goalId);
	...
	app.get(
		"/api/recommendations/portfolio/:portfolioId/rebalance",
		async (req: any, res) => {
			try {
				...
				const goals = await storage.getFinancialGoals(userId);
				const recommendations =
					await storage.generatePortfolioRebalanceRecommendations(
						portfolioId,
						goals,
					);
				...
				res
					.status(500)
					.json({ error: "Failed to generate rebalance recommendations" });
			}
		},
	);
		...
		try {
			...
			if (!verifiedName) {
				return res.status(404).json({
					...
					panNumber: panNumber,
					message:
						"PAN number not found in verified records. Please use Sandbox API for real-time PAN verification.",
					source: "local_lookup",
				});
			}
		...
		try {
			...
			const result = {
				...
				portfolioData: {
					...
					allocation: {
						equity:
							minimalData.investmentPreference === "aggressive"
								? 70
								: minimalData.investmentPreference === "balanced"
									? 50
									: 30,
						debt:
							minimalData.investmentPreference === "aggressive"
