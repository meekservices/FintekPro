...
export function registerBondTradingOrderPart3Part1Routes(app: Express): void {
	app.get("/api/aif/comprehensive", requireLevel2, async (req, res) => {
		try {
			...
			const amcStr =
				typeof amc === "string" ? amc : Array.isArray(amc) ? amc[0] : undefined;
			const categoryStr =
			...
			const subCategoryStr =
				typeof subCategory === "string"
					...
					: Array.isArray(subCategory)
						...
						: undefined;
			const riskRatingStr =
				typeof riskRating === "string"
					? riskRating
					: Array.isArray(riskRating)
						? riskRating[0]
						: undefined;
			...
			const enhancedStats = {
				...
				categoryBreakdown: {
					"Category I": allFundsData.filter((f) => f.category === "Category I")
						.length,
					"Category II": allFundsData.filter(
						(f) => f.category === "Category II",
					).length,
					"Category III": allFundsData.filter(
						(f) => f.category === "Category III",
					).length,
				},
				activeAMCs: new Set(
					allFundsData.map(
