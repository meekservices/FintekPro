...
import type { CorporateBond, FixedIncomeLog, StatusSummary } from '../types/fixedIncome';
...
router.get('/advisor/search', requireAuth, async (req: Request, res: Response): Promise<void> => {
  try {
    const {
      limit = '50',
      offset = '0'
    ...
    const bonds: CorporateBond[] = await db.select().from(corporateBonds)
      .where(
        and(
    ...
    const [totalRow] = await db.select({ count: sql<string>`count(*)` })
      .from(corporateBonds)
      .where(
        and(
          eq(corporateBonds.tradingStatus, 'active'),
...
router.get('/holdings', requireAuth, async (req: Request, res: Response): Promise<void> => {
  try {
...
router.get('/status/summary', requireAuth, async (_req: Request, res: Response): Promise<void> => {
  try {
    const summary: StatusSummary = await fixedIncomeStatusEngine.getStatusSummary();
...
router.get('/status/:isin', requireAuth, async (req: Request, res: Response): Promise<void> => {
  try {
...
router.post('/status/:isin/evaluate', requireAuth, requireRole(['admin']), async (req: Request, res: Response): Promise<void> => {
  try {
    const { isin } = req.params;
...
router.post('/status/refresh-all', requireAuth, requireRole(['admin']), async (_req: Request, res: Response): Promise<void> => {
  try {
...
router.get('/status/log', requireAuth, requireRole(['admin']), async (req: Request, res: Response): Promise<void> => {
  try {
    const { limit = '100', offset = '0' } = req.query;

    const logs: FixedIncomeLog[] = await db.select().from(fixedIncomeStatusLog)
...
router.get('/bonds/:type/:rating', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const bonds: CorporateBond[] = await db.select().from(corporateBonds)
      .where(
        and(
          ...
          rating !== 'all' ? eq(corporateBonds.creditRating, rating) : sql`1=1`
        )
      )
...
router.post('/admin/seed-universe', requireAuth, requireRole(['admin']), async (req: Request, res: Response): Promise<void> => {
  try {
...
router.get('/admin/universe-stats', requireAuth, requireRole(['admin']), async (_req: Request, res: Response): Promise<void> => {
  try {
...
router.get('/admin/audit-logs', requireAuth, requireRole(['admin']), async (_req: Request, res: Response): Promise<void> => {
  try {
    const logs: FixedIncomeLog[] = await db.select().from(fixedIncomeStatusLog)
      .orderBy(desc(fixedIncomeStatusLog.createdAt))
      .limit(100);
    ...
    console.error('Error fetching fixed income audit logs:', errorMessage(err));
