...
router.get("/dashboard/stats", async (req: Request, res: Response) => {
	try {
		...
		for (const app of applications) {
			...
			totalAmount += Number.parseFloat(app.requestedAmount || "0");
			if (app.status === "approved" || app.status === "disbursed") {
				approvedAmount += Number.parseFloat(app.requestedAmount || "0");
			}
			if (app.status === "disbursed") {
				disbursedAmount += Number.parseFloat(app.requestedAmount || "0");
			}
		}

		const bankStats: Record<
			string,
			{ submitted: number; approved: number; rejected: number; pending: number }
		> = {};
		for (const history of routingHistories) {
			if (!bankStats[history.bankCode]) {
				bankStats[history.bankCode] = {
					submitted: 0,
					approved: 0,
					rejected: 0,
					pending: 0,
			...
			if (history.bankStatus === "approved")
				bankStats[history.bankCode].approved++;
			else if (history.bankStatus === "rejected")
				bankStats[history.bankCode].rejected++;
		...
		const bankWiseStats = banks.map((bank) => ({
			...
			approvalRate: bankStats[bank.bankCode]
				? (
						(bankStats[bank.bankCode].approved /
							(bankStats[bank.bankCode].approved +
								bankStats[bank.bankCode].rejected)) *
							100 || 0
					).toFixed(1)
				: "0",
		}));
		...
		const completedApps = applications.filter((a) =>
			["approved", "rejected", "disbursed"].includes(a.status),
		);
				...
				overview: {
					...
					approvalRate:
						completedApps.length > 0
							? ((approvedApps.length / completedApps.length) * 100).toFixed(1)
							: 0,
...
router.get("/applications", async (req: Request, res: Response) => {
	try {
		...
		const query = db.select().from(dsaLoanApplications);
		const conditions = [];

		if (status)
			conditions.push(eq(dsaLoanApplications.status as any, status as string));
		if (loanType)
			conditions.push(eq(dsaLoanApplications.loanType, loanType as string));
		if (fromDate)
			conditions.push(
				gte(dsaLoanApplications.createdAt, new Date(fromDate as string)),
			);
		if (toDate)
			conditions.push(
				lte(dsaLoanApplications.createdAt, new Date(toDate as string)),
			);
		...
		if (originationMode)
			conditions.push(
				eq(
					(dsaLoanApplications as any).originationMode,
					originationMode as string,
				),
			);
		if (routingIntent)
			conditions.push(
				eq((dsaLoanApplications as any).routingIntent, routingIntent as string),
			);
		if (agentId)
			conditions.push(eq(dsaLoanApplications.agentId, agentId as string));
		if (bankCode)
			conditions.push(
				sql`${bankCode} = ANY(${dsaLoanApplications.routedBanks})`,
			);
...
router.get("/applications/:id", async (req: Request, res: Response) => {
	try {
		...
		const routingHistory = await dsaLoanService.getRoutingHistory(
			req.params.id,
		);
...
router.get("/eligibility-rules", async (req: Request, res: Response) => {
	try {
		...
		if (bankCode)
			conditions.push(eq(loanEligibilityRules.bankCode, bankCode as string));
		if (loanType)
			conditions.push(eq(loanEligibilityRules.loanType, loanType as string));
...
router.get("/audit-logs", async (req: Request, res: Response) => {
	try {
		const { applicationId, action, fromDate, toDate, limit, offset } =
			req.query;
		const conditions = [];

		if (applicationId)
			conditions.push(
				eq(dsaLoanAuditLogs.applicationId, applicationId as string),
			);
		if (action) conditions.push(eq(dsaLoanAuditLogs.action, action as string));
		if (fromDate)
			conditions.push(
				gte(dsaLoanAuditLogs.createdAt, new Date(fromDate as string)),
			);
		if (toDate)
			conditions.push(
				lte(dsaLoanAuditLogs.createdAt, new Date(toDate as string)),
			);
