...
export function calculateCapitalGains(holdings: NormalizedHolding[]) {
	...
	const stcg = { count: 0, totalValue: 0, taxableGain: 0, estimatedTax: 0 };
	const ltcg = {
		count: 0,
		totalValue: 0,
		taxableGain: 0,
	...
	const grandfathered = { count: 0, benefit: 0 };
	...
	for (const h of holdings) {
		...
		if (isLongTerm) {
			...
			estimatedTax = Math.max(0, gain) * 0.2;
...
export function calculatePortfolioHealthScore(
	...
	for (const h of holdings) {
		...
		assetTypes.set(
			assetType,
			(assetTypes.get(assetType) || 0) + (h.currentValue || 0),
		);
		const amc = h.name?.split(" ")[0] || "Unknown";
		amcs.set(amc, (amcs.get(amc) || 0) + (h.currentValue || 0));
	}

	const maxAssetConcentration =
		(Math.max(...Array.from(assetTypes.values())) / totalValue) * 100;
	...
	if (costEfficiency < 70)
		recommendations.push(
			"Look for lower-cost fund alternatives to reduce expenses",
		);
	if (amcs.size < 3)
		recommendations.push("Consider spreading investments across more AMCs");
...
export async function calculateExpenseRatioAnalysis(
	holdings: NormalizedHolding[],
) {
	const totalValue = holdings.reduce(
		(sum, h) => sum + (h.currentValue || 0),
	...
	const getKey = (h: NormalizedHolding) =>
		(h as any).isin || (h as any).schemeCode || h.name || "";

	// Helper to check if holding is a stock/equity
	const isEquity = (h: NormalizedHolding) =>
		h.assetType === "equity" || h.assetType === "stock";

	// Helper to check if holding is a mutual fund (strict - only MFs have expense ratios)
	const isMutualFund = (h: NormalizedHolding) =>
		h.assetType === "mutual_fund" || h.assetType === "etf";
	...
	for (const h of holdings) {
		...
		if (isEquity(h) || !isMutualFund(h)) {
			...
			if (terLookup) {
				...
				else if (terLookup.source === "category_default")
					categoryDefaultCount++;
				else fallbackCount++;
			}
		}
...
export function isGrowthPlan(name: string): boolean {
	...
	const hasDividendIndicator = dividendPatterns.some((p) =>
...
export async function calculateDividendProjection(
	holdings: NormalizedHolding[],
) {
	const totalValue = holdings.reduce(
		(sum, h) => sum + (h.currentValue || 0),
	...
	const stockYieldMap =
		await portfolioAnalyticsDataService.batchGetStockDividendYields(
			stockHoldings.map((h) => ({
				name: h.name || "",
				isin: (h as any).isin,
	...
	const dividendPayingValue = holdingsWithDividend.reduce(
		(sum, h) => sum + h.value,
		0,
	);
	...
	const totalNonDbCount =
		sectorDefaultCount + categoryDefaultCount + fallbackCount;
...
export function calculateRiskHeatmap(
	...
	const criticalCount = concentrationWarnings.filter(
		(w) => w.severity === "critical",
	).length;
	const warningCount = concentrationWarnings.filter(
...
export function guessSector(name: string): string {
	...
	if (
		...
		lower.includes("sun") ||
		lower.includes("cipla")
	)
		return "Healthcare";
	if (
		lower.includes("auto") ||
		lower.includes("maruti") ||
		lower.includes("tata motors")
	)
		return "Automobile";
	if (
	...
	if (
		...
		lower.includes("tata steel")
	)
		return "Metals";
	if (lower.includes("real") || lower.includes("infra"))
		return "Real Estate & Infra";
	if (lower.includes("small") || lower.includes("mid"))
...
export function calculateBenchmarkComparison(
	...
	return {
		...
		benchmarks: [
			{
				name: "Nifty 50",
				returns: { oneYear: 14.2, threeYear: 12.8, fiveYear: 11.5 },
			},
			{
				name: "Sensex",
				returns: { oneYear: 13.8, threeYear: 12.5, fiveYear: 11.2 },
			},
			{
				name: "Nifty Midcap 100",
				returns: { oneYear: 22.5, threeYear: 18.2, fiveYear: 15.8 },
			},
			{
				name: "Category Average",
				returns: { oneYear: 11.5, threeYear: 10.2, fiveYear: 9.8 },
			},
		],
		...
		dataSources: {
			...
			note:
				stockValue === 0
					? "No direct stock holdings - using MF beta approximation"
					: undefined,
		},
	};
}
		...
		scenarios: [
			{
				...
				marketChange: -20,
				portfolioImpact: -18,
				newValue: Math.round(totalValue * 0.82),
			},
			{
				name: "Correction (-10%)",
				marketChange: -10,
				portfolioImpact: -9,
				newValue: Math.round(totalValue * 0.91),
			},
			{
				name: "Bull Run (+20%)",
				marketChange: 20,
				portfolioImpact: 18,
				newValue: Math.round(totalValue * 1.18),
			},
			{
				name: "Strong Rally (+30%)",
				marketChange: 30,
				portfolioImpact: 27,
				newValue: Math.round(totalValue * 1.27),
			},
		],
		stressTestResult: {
			worstCase: Math.round(totalValue * 0.7),
...
export function generatePriorityRecommendations(
	...
	if (
		riskHeatmap?.concentrationWarnings?.some(
	...
	const stcgHoldings =
		capitalGains?.holdings?.filter(
			(h: any) => h.taxType === "STCG" && h.gain > 0,
		) || [];
	...
	const highTERHoldings = holdings.filter(
		(h) => ((h as any).expenseRatio || 0) > 1.5,
	);
...
export async function generateSipRecommendations(
	...
	if (
		...
		console.log(
			"[SIP] All selected categories are lumpsum-only — skipping SIP recommendations",
		);
		return [];
	}
	...
	const totalGoalSip = goals.reduce(
		(sum, g) => sum + (Number(g.monthlyContribution) || 0),
		0,
	);
	if (goalsProvided && totalGoalSip === 0) {
		console.log(
			"[SIP] Goals provided with zero monthly contribution — agent intends no SIP",
		);
		return [];
	}
	...
	const monthlyAmount =
		totalGoalSip > 0
			? totalGoalSip
			: analysis?.totalValue
				? Math.round((analysis.totalValue * 0.05) / 12)
				: 10000;

	const fundsByRisk: Record<
		string,
		Array<{
			fundName: string;
			category: string;
		...
		conservative: [
			{
				...
				suggestedAmount: Math.round(monthlyAmount * 0.4),
				expectedReturn: 7.5,
				riskLevel: "Low",
			...
			},
			{
				...
				suggestedAmount: Math.round(monthlyAmount * 0.35),
				expectedReturn: 10,
				riskLevel: "Moderate",
				rationale: "Dynamic equity-debt mix for stability",
			},
			{
				fundName: "Axis Bluechip Fund",
				category: "Equity - Large Cap",
				suggestedAmount: Math.round(monthlyAmount * 0.25),
				expectedReturn: 12,
				riskLevel: "Moderate",
				rationale: "Quality large caps for growth",
			},
		],
		moderate: [
			{
				...
				expectedReturn: 14,
				riskLevel: "Moderate",
				rationale: "Diversified equity with global exposure",
			},
			{
				fundName: "Mirae Asset Large Cap",
				category: "Equity - Large Cap",
				suggestedAmount: Math.round(monthlyAmount * 0.3),
				expectedReturn: 13,
				riskLevel: "Moderate",
				rationale: "Consistent large cap performer",
			},
			{
				fundName: "Kotak Emerging Equity",
				category: "Equity - Mid Cap",
				suggestedAmount: Math.round(monthlyAmount * 0.2),
				expectedReturn: 16,
				riskLevel: "High",
			...
			},
			{
				...
				suggestedAmount: Math.round(monthlyAmount * 0.15),
				expectedReturn: 8,
				riskLevel: "Low",
				rationale: "Portfolio stability component",
			},
		],
		aggressive: [
			{
				...
				expectedReturn: 18,
				riskLevel: "Very High",
				rationale: "High growth small cap exposure",
			},
			{
				...
				expectedReturn: 16,
				riskLevel: "High",
				rationale: "Quality mid caps for alpha",
			},
			{
				fundName: "Quant Multi Cap Fund",
				category: "Equity - Multi Cap",
				suggestedAmount: Math.round(monthlyAmount * 0.25),
				expectedReturn: 17,
				riskLevel: "High",
				rationale: "Momentum-based multi cap strategy",
			},
			{
				fundName: "UTI Nifty 50 Index",
				category: "Equity - Index",
				suggestedAmount: Math.round(monthlyAmount * 0.15),
				expectedReturn: 12,
				riskLevel: "Moderate",
				rationale: "Low-cost market returns",
			},
		],
		very_aggressive: [
			{
				fundName: "Quant Small Cap",
				category: "Equity - Small Cap",
				suggestedAmount: Math.round(monthlyAmount * 0.35),
				expectedReturn: 20,
				riskLevel: "Very High",
				rationale: "Aggressive small cap exposure",
			},
			{
				fundName: "Nippon India Small Cap",
				category: "Equity - Small Cap",
			...
			},
			{
				...
				expectedReturn: 16,
				riskLevel: "High",
				rationale: "Quality mid cap allocation",
			},
			{
				fundName: "Parag Parikh Flexi Cap",
				category: "Equity - Flexi Cap",
				...
				expectedReturn: 14,
				riskLevel: "Moderate",
				rationale: "Diversification anchor",
			},
		],
	};
	...
	for (const fund of recommendations) {
		...
		let reason = "";
		try {
			const dbResult = await schemeGovernanceService.checkEligibility(
				fund.fundName,
				"name",
			);
		...
		if (restricted) {
			console.log(
				`[SIP] Excluding ${fund.fundName} from SIP recommendations: ${reason}`,
			);
		} else {
			eligible.push(fund);
		}
	...
	if (eligible.length > 0 && eligible.length < recommendations.length) {
		...
		if (totalEligible > 0) {
			...
			eligible.forEach((fund) => {
				fund.suggestedAmount = Math.round(
					fund.suggestedAmount * redistributionFactor,
				);
			});
			const redistributedTotal = eligible.reduce(
				(sum, f) => sum + f.suggestedAmount,
				0,
			);
