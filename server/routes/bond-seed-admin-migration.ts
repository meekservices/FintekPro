...
migrationRouter.post("/sync-bonds", async (req: Request, res: Response) => {
	...
	const migrationSecret =
		process.env.MIGRATION_SECRET || "fintekpro-bond-sync-2026";
	...
	for (const bond of bonds) {
		try {
			await db
				...
				.values({
					...
					netYieldToMaturity:
						bond.netYieldToMaturity || bond.net_yield_to_maturity,
		...
		if ((inserted + skipped) % 500 === 0) {
			console.log(
				`[Migration] Progress: ${inserted + skipped}/${bonds.length}`,
			);
		}
	}
