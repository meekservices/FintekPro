...
async function generateRebalanceRecommendations(
	...
	const targetAllocation =
		DEFAULT_TARGET_ALLOCATION[profileKey] || DEFAULT_TARGET_ALLOCATION.moderate;

	const allocationByType: Record<
		string,
		{ internal: number; external: number; holdings: HoldingForRebalance[] }
	> = {};
	...
	const allTypes = new Set([
		...Object.keys(allocationByType),
		...Object.keys(targetAllocation),
	]);

	allTypes.forEach((assetType) => {
		const data = allocationByType[assetType] || {
			internal: 0,
			external: 0,
			holdings: [],
		};
...
router.post(
	"/api/ai/generate-rebalance-proposal",
	requireAuth,
	async (req: Request, res: Response) => {
		try {
			...
			const { includeExternal, portfolioId, unifiedHoldings } =
				generateRebalanceProposalSchema.parse(req.body);
			...
			if (unifiedHoldings && unifiedHoldings.length > 0) {
				...
				for (const portfolio of userPortfolios) {
					...
					for (const h of portHoldings) {
						...
						const quantity = Number.parseFloat(h.quantity?.toString() || "0");
			...
			try {
				...
				console.warn(
					"[AI Rebalancing] Failed to load risk profile:",
					e?.message,
				);
			}

			const totalValue =
				holdings.reduce((sum, h) => sum + (h.currentValue || 0), 0) || 1;

			const recommendations =
				await autonomousRebalancePlanner.evaluatePortfolio(
					totalValue,
					holdings.map((h) => ({
						asset_class: h.assetType,
			...
			if (goals.length === 0) {
				try {
					...
					goals = userGoals.map((g) => ({
						id: g.id,
						name: g.name,
						targetAmount: Number.parseFloat(g.targetAmount?.toString() || "0"),
...
router.post(
	"/api/ai/generate-retirement-proposal",
	requireAuth,
	async (req: Request, res: Response) => {
		try {
			const userId = (req.user as any)?.id;
			...
			const futureMonthlyExpenses =
				monthlyExpenses * (1 + inflationRate / 100) ** yearsToRetirement;
			const requiredCorpus = futureMonthlyExpenses * 12 * 25;

			const monthsToRetirement = yearsToRetirement * 12;
			const monthlyRate = expectedReturn / 100 / 12;
			const futureValueOfCurrentSavings =
				currentSavings * (1 + expectedReturn / 100) ** yearsToRetirement;
			const remainingCorpus = Math.max(
				0,
				requiredCorpus - futureValueOfCurrentSavings,
			);
			...
			if (yearsToRetirement <= 5) {
				recommendations = [
					{
						type: "debt_short_duration",
						allocation: 50,
						name: "Short Duration Fund",
					},
					{
						type: "hybrid_conservative",
						allocation: 30,
