...
router.post(
    ...
    `);
			...
			res
				.status(500)
				.json({
					error: (error as Error).message || "Failed to initiate eSign",
				});
		}
	},
);
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
				...
				(async () => {
					try {
						...
						const packSnapshot = {
							...
							signedAt:
								result.signatureData?.signedAt || new Date().toISOString(),
							provider: result.provider,
						};
						const auditHash = createHash("sha256")
							.update(JSON.stringify(packSnapshot))
							.digest("hex");
						await db.insert(regulatoryAuditPacks).values({
							...
							kycSnapshot: {
								...
								note: "KYC pre-verified at account creation",
							},
							suitabilitySnapshot: {
								verifiedAt: new Date().toISOString(),
								note: "Suitability assessed at onboarding",
							},
						...
						console.error(
							"[eSign] Failed to create regulatory audit pack:",
							e?.message,
						);
					}
				})();
			...
			res
				.status(500)
				.json({ error: (error as Error).message || "Failed to verify OTP" });
		}
	},
);
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			res
				.status(500)
				.json({ error: (error as Error).message || "Failed to resend OTP" });
		}
	},
);

router.get(
	"/api/esign/request/:transactionId",
	isAuthenticated,
	async (req: Request, res: Response) => {
		try {
			...
			res
				.status(500)
				.json({ error: (error as Error).message || "Failed to get status" });
		}
	},
);

router.get(
	"/api/esign/certificates",
	isAuthenticated,
	async (req: Request, res: Response) => {
		try {
			const userId = (req.user as any)?.id;
			...
			const certificates =
				await truthScreenESignService.getUserCertificates(userId);
...
router.get(
	"/api/esign/verify-certificate/:certificateSerial",
	async (req: Request, res: Response) => {
		try {
			const { certificateSerial } = req.params;

			const result =
				await truthScreenESignService.verifyCertificate(certificateSerial);
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			const hash =
				truthScreenESignService.generateDocumentHash(documentContent);
...
router.get(
	"/api/esign/download/:transactionId",
	isAuthenticated,
	async (req: Request, res: Response) => {
		try {
			const userId = (req.user as any)?.id;
...
router.get(
		...
		try {
			...
			if (!userRoles.some((r: string) => AUDIT_ROLES.includes(r))) {
				return res
					.status(403)
					.json({
						error: "Access denied. Compliance officer or admin role required.",
					});
			}
    ...
    `);
			...
			if (format === "csv") {
				...
				res.setHeader(
					"Content-Disposition",
					`attachment; filename="esign-audit-${fromDate}-to-${toDate}.csv"`,
				);
