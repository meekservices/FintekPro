...
export function registerSandboxITReportRoutes(app: Express): void {
			...
			try {
				...
				if (!pan || !financialYear) {
					return res
						...
						.json({
							success: false,
							message: "pan and financialYear are required",
						});
				}
				...
				if (outputFormat && !["json", "excel"].includes(outputFormat)) {
					return res
						...
						.json({
							success: false,
							message: 'outputFormat must be "json" or "excel"',
						});
				}
					...
					.json({
						...
						message:
							error instanceof Error
								? error.message
								: "Failed to submit Tax P&L report job",
					});
			}
		},
	);
	...
	app.get(
		...
		async (req: Request, res: Response) => {
			try {
				...
				const result = await sandboxTDSService.pollTaxPLReportJob(
					String(job_id),
				);
				return res.json(result);
			} catch (error) {
				...
				return res
					...
					.json({
						...
						message:
							error instanceof Error
								? error.message
								: "Failed to poll Tax P&L report job",
					});
			}
		},
	);
				...
				return res
					...
					.json({
						...
						message:
							error instanceof Error
								? error.message
								: "Failed to submit Capital Gains report job",
					});
			}
		},
	);
	...
	app.get(
		...
		async (req: Request, res: Response) => {
			try {
				...
				const result = await sandboxTDSService.pollCapitalGainsReportJob(
					String(job_id),
				);
				return res.json(result);
			} catch (error) {
				...
				return res
					...
					.json({
						...
						message:
							error instanceof Error
								? error.message
								: "Failed to poll Capital Gains report job",
					});
			}
		},
	);
	...
	console.log(
		"✅ IT Report routes registered (Tax P&L + Capital Gains async reports)",
	);
}
