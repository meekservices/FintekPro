...
router.post("/api/user/signatures", isAuthenticated, async (req, res) => {
	try {
		...
		if (error instanceof z.ZodError) {
			return res
				.status(400)
				.json({ error: "Invalid signature data", details: error.issues });
		}
...
router.patch("/api/user/signatures/:id", isAuthenticated, async (req, res) => {
	try {
		...
		const [existing] = await db
			.select()
			.from(userSignatures)
			.where(and(eq(userSignatures.id, id), eq(userSignatures.userId, userId)))
			.limit(1);
		...
		if (error instanceof z.ZodError) {
			return res
				.status(400)
				.json({ error: "Invalid update data", details: error.issues });
		}
...
router.post(
	"/api/user/signatures/:id/set-default",
	isAuthenticated,
	async (req, res) => {
		try {
			const userId = req.user?.id;
			...
			const [existing] = await db
				...
				.from(userSignatures)
				.where(
					and(eq(userSignatures.id, id), eq(userSignatures.userId, userId)),
				)
				.limit(1);
