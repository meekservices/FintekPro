		...
		try {
			...
			if (!isBuyer && !isSeller) {
				return apiResponse.forbidden(
					res,
					"You are not authorized to reject this deal",
				);
			}
			...
			if (deal.status !== "pending" && deal.status !== "awaiting_acceptance") {
				return apiResponse.badRequest(
					res,
					`Deal cannot be rejected in current status: ${deal.status}`,
				);
			}
			...
			const updatedDeal = await storage.updateUnlistedDeal(id, {
				...
				cancellationReason:
					reason || `Rejected by ${isBuyer ? "buyer" : "seller"}`,
			});
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (!proposedPrice || Number.isNaN(Number.parseFloat(proposedPrice))) {
			...
			if (!isBuyer && !isSeller) {
				return apiResponse.forbidden(
					res,
					"You are not authorized to make a counter offer",
				);
			}
			...
			if (deal.status !== "pending" && deal.status !== "awaiting_acceptance") {
				return apiResponse.badRequest(
					res,
					`Counter offers not allowed in current status: ${deal.status}`,
				);
			}
			...
			const updatedDeal = await storage.updateUnlistedDeal(id, {
				...
				totalValue: (
					Number.parseFloat(proposedPrice) * deal.quantity
				).toString(),
				buyerAccepted: false,
			...
			console.log(
				`[Deal ${id}] Counter-offer: ${isBuyer ? "Buyer" : "Seller"} proposed ₹${proposedPrice}`,
			);
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			const suggestion =
				await priceSuggestionService.calculateSuggestedPrice(companyId);
			...
			if (error.message === "Insufficient data to calculate price suggestion") {
				return apiResponse.badRequest(
					res,
					"Insufficient data to calculate price suggestion",
				);
			}

			return apiResponse.serverError(
				res,
				"Failed to calculate price suggestion",
			);
		}
	},
);
...
router.get(
	...
	async (req: Request, res: Response) => {
		try {
			...
			const priceSuggestionService = new PriceSuggestionService(storage);
			const suggestion =
				await priceSuggestionService.calculateSuggestedPrice(id);
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			const priceSuggestionService = new PriceSuggestionService(storage);
			const suggestions =
				await priceSuggestionService.calculateBatchSuggestions(companyIds);
			...
			return apiResponse.serverError(
				res,
				"Failed to calculate price suggestions",
			);
		}
	},
);
...
router.get(
	...
	async (req: Request, res: Response) => {
		try {
			...
			const allSellListings = await db
				.select()
				.from(sellListings)
				.where(eq(sellListings.status, "active"));

			// Get all active buy requests
			const allBuyRequests = await db
				.select()
				.from(buyRequests)
				.where(eq(buyRequests.status, "active"));

			// Group buy requests by company
			const buyRequestsByCompany = allBuyRequests.reduce(
				(acc: Record<string, BuyRequest[]>, buyRequest: BuyRequest) => {
			...
			for (const sellListing of allSellListings) {
				...
				const lastDeal = recentDeals.sort(
					(a: any, b: any) =>
				...
				try {
					priceSuggestion =
						await priceSuggestionService.calculateSuggestedPrice(
							sellListing.companyId,
						);
				} catch (error) {
					console.error(
						`Error calculating price suggestion for ${company.name}:`,
						error,
					);
				}

				// Find best matching buy request (highest max price)
				const bestBuyRequest = matchingBuyRequests.reduce(
					(best: BuyRequest, current: BuyRequest) => {
						const bestPrice = Number.parseFloat(best.maxPrice);
						const currentPrice = Number.parseFloat(current.maxPrice);
						return currentPrice > bestPrice ? current : best;
					},
				);

				// Calculate match score (0-100)
				const sellerLandingPrice = Number.parseFloat(sellListing.landingPrice);
				const buyerMaxPrice = Number.parseFloat(bestBuyRequest.maxPrice);
				const suggestedMidpoint =
					priceSuggestion?.suggestedPrice ||
					(sellerLandingPrice + buyerMaxPrice) / 2;
				...
				if (
					minMatchScore &&
					matchScore < Number.parseFloat(minMatchScore as string)
				) {
					continue;
				}
				...
				if (
					minPrice &&
					sellerLandingPrice < Number.parseFloat(minPrice as string)
				) {
					continue;
				}
				if (
					maxPrice &&
					sellerLandingPrice > Number.parseFloat(maxPrice as string)
				) {
					continue;
				}
			...
			const paginatedNegotiations = negotiations.slice(
				offset,
				offset + limitNum,
			);
	...
	async (req: Request, res: Response) => {
		try {
			...
			const companiesNeedingPricing = allCompanies.filter(
				(c) => c.status === "active" && (!c.draftBuyPrice || !c.draftSellPrice),
			);
			const companiesWithDraftPrices = allCompanies.filter(
				(c) =>
					c.pricingStatus === "draft" || c.pricingStatus === "pending_review",
			);

			// Get high-risk companies
			const highRiskCompanies = allCompanies.filter(
				(c) =>
					c.complianceStatus === "blocked" ||
					((c as any).riskScore && (c as any).riskScore > 70),
			);

			// Get all active sell listings
			const activeSellListings = await db
				.select()
				.from(sellListings)
				.where(eq(sellListings.status, "active"));

			// Get all active buy requests
			const activeBuyRequests = await db
				.select()
				.from(buyRequests)
				.where(eq(buyRequests.status, "active"));
			...
			const completedRecentDeals = recentDeals.filter(
				(d) =>
					d.status === "completed" &&
			...
			const tradingVolume = completedRecentDeals.reduce((sum, deal) => {
				const price = Number.parseFloat(deal.agreedPrice || "0");
...
router.get(
	...
	async (req: Request, res: Response) => {
		try {
			...
			const pageNum = Number.parseInt(page as string);
			const limitNum = Math.min(Number.parseInt(limit as string), 100);
