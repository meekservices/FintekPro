...
export function registerMFMonthwiPart2Part2Routes(app: Express): void {
	app.post("/api/nsdl/margin/pledge/create", async (req, res) => {
		try {
			const { accountNumber, isin, quantity, pledgeeCode, purpose, otp } =
				req.body;
			...
			const pledgeData = {
				...
				collateralValue: (Number.parseFloat(quantity) * 1500).toString(), // Simulated value
				haircut: "15%",
				eligibleValue: (Number.parseFloat(quantity) * 1275).toString(),
			};
		...
		try {
			...
			if (!accountNumber || !loanAmount || !collateralSecurities || !bankCode) {
				return res.status(400).json({
					status: "error",
					error:
						"Account number, loan amount, collateral securities, and bank code are required",
				});
			}
			...
			const loanApplication = {
				...
				interestRate: "12.5%",
				tenure: "12 months",
				eligibleLoanAmount: (Number.parseFloat(loanAmount) * 0.7).toString(),
			};
