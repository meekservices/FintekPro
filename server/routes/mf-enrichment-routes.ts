...
export function registerMFEnrichmentRoutes(app: Express) {
  app.get('/api/funds/list', async (req: Request, res: Response) => {
    try {
      const { category, planType, riskLevel, search, page = '1', limit = '50', sortBy = 'aum', sortOrder = 'desc' } = req.query;
      const pageNum = Math.max(1, parseInt(page as string) || 1);
      const limitNum = Math.min(100, Math.max(1, parseInt(limit as string) || 50));
      const offset = (pageNum - 1) * limitNum;

      const conditions: any[] = [eq(mutualFunds.schemeStatus, 'active')];
      if (category) conditions.push(eq(mutualFunds.category, category as string));
      if (planType) conditions.push(eq(mutualFunds.planType, planType as string));
      if (riskLevel) conditions.push(eq(mutualFunds.riskLevel, riskLevel as string));
      if (search) conditions.push(ilike(mutualFunds.schemeName, `%${search}%`));

      const whereClause = conditions.length > 1 ? and(...conditions) : conditions[0];

      const [countResult] = await db.select({ count: sql<number>`COUNT(*)` })
        .from(mutualFunds).where(whereClause);

      const sortColumn = sortBy === 'nav' ? mutualFunds.nav
        : sortBy === 'returns_1y' ? mutualFunds.returns1y
        : sortBy === 'returns_3y' ? mutualFunds.returns3y
        : sortBy === 'expense_ratio' ? mutualFunds.expenseRatio
        : sortBy === 'name' ? mutualFunds.schemeName
        : mutualFunds.aum;
  ...
  app.get('/api/funds/:schemeCode', async (req: Request, res: Response) => {
    try {
      ...
      const [fund] = await db.select().from(mutualFunds)
        .where(eq(mutualFunds.schemeCode, schemeCode)).limit(1);
  ...
  app.get('/api/funds/:schemeCode/aum-history', async (req: Request, res: Response) => {
    try {
      const { schemeCode } = req.params;
      const { days = '90' } = req.query;
      const daysNum = Math.min(365, parseInt(days as string) || 90);
  ...
  app.get('/api/funds/category/:category', async (req: Request, res: Response) => {
    try {
      ...
      const rules = await db.select().from(mfCategoryRules)
        .where(and(
          eq(mfCategoryRules.category, category),
          eq(mfCategoryRules.isActive, true)
        ));

      const rule = rules.length > 0 ? rules : await db.select().from(mfCategoryRules)
        .where(and(
  ...
  app.get('/api/funds/enrichment/audit', async (req: Request, res: Response) => {
    try {
      const { schemeCode, changeType, source, limit = '50' } = req.query;
      const limitNum = Math.min(200, parseInt(limit as string) || 50);

      const conditions: any[] = [];
      if (schemeCode) conditions.push(eq(mfEnrichmentAuditLogs.schemeCode, schemeCode as string));
      if (changeType) conditions.push(eq(mfEnrichmentAuditLogs.changeType, changeType as string));
      if (source) conditions.push(eq(mfEnrichmentAuditLogs.source, source as string));
  ...
  app.get('/api/funds/sebi/category-rules', async (_req: Request, res: Response) => {
    try {
      const rules = await db.select().from(mfCategoryRules)
  ...
  app.get('/api/funds/enrichment/null-stats', async (_req: Request, res: Response) => {
    try {
