...
router.get("/tables", requireAdmin, async (req: Request, res: Response) => {
    ...
    `);
		...
		const tables: TableInfo[] = (result.rows as any[]).map((row) => ({
			...
			rowCount: Number.parseInt(row.row_count) || 0,
			sizeBytes: Number.parseInt(row.size_bytes) || 0,
			sizeFormatted: formatBytes(Number.parseInt(row.size_bytes) || 0),
		}));
...
router.get(
	...
	async (req: Request, res: Response) => {
		try {
			...
			const totalRows =
				Number.parseInt((countResult.rows[0] as any).total) || 0;
    ...
    `);
		...
		res.json({
			...
			stats: {
				...
				tableCount:
					Number.parseInt((tableCountResult.rows[0] as any).count) || 0,
				activeConnections:
					Number.parseInt(
						(connectionResult.rows[0] as any).active_connections,
					) || 0,
			},
		});
	} catch (error: any) {
