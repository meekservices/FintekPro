...
router.get("/annotations/:documentId", async (req: Request, res: Response) => {
	try {
		...
		const annotations = await esignAIAnalysisService.getAnnotationsForDocument(
			documentId,
			{
...
router.get(
	"/annotations/detail/:annotationId",
	async (req: Request, res: Response) => {
		try {
			const annotationId = Number.parseInt(req.params.annotationId, 10);

			const result =
				await esignAIAnalysisService.getAnnotationWithReplies(annotationId);

			if (!result) {
				return res
					.status(404)
					.json({ success: false, error: "Annotation not found" });
			}
...
router.patch(
	"/annotations/:annotationId/status",
	requireAuth,
	async (req: Request, res: Response) => {
		try {
			const user = req.user as any;
			...
			const result = await esignAIAnalysisService.updateAnnotationStatus(
				annotationId,
				{
				...
				(async () => {
					try {
						...
						if (openCount === 0) {
							...
							const accepted = allCorrections
								.filter((a) => a.status === "accepted")
								.map((a) => a.title);
							const rejected = allCorrections
						...
						console.error(
							"[eSign AI] Version history write failed:",
							e?.message,
						);
					}
				})();
...
router.get(
	"/annotations/:annotationId/audit",
	async (req: Request, res: Response) => {
		try {
			const annotationId = Number.parseInt(req.params.annotationId, 10);

			const auditLog =
				await esignAIAnalysisService.getAnnotationAuditLog(annotationId);
...
router.get("/sessions", async (req: Request, res: Response) => {
	try {
		...
		const sessions = await esignAIAnalysisService.getAnalysisSessions(
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			const annotation = await esignAIAnalysisService.addManualAnnotation(
...
router.post(
	"/annotations/:annotationId/accept-all",
	requireAuth,
	async (req: Request, res: Response) => {
		try {
			...
			const annotations =
				await esignAIAnalysisService.getAnnotationsForDocument(documentId, {
					status: "open",
				});
