...
router.post("/sync/bse", async (req: Request, res: Response) => {
	try {
		...
		for (const type of [
			...
			feeProfiles[type] =
				await bondFeeCalibrationService.getProfileByInstrumentType(type);
		}
		...
		for (let i = 0; i < newBonds.length; i += batchSize) {
			...
			try {
				...
				synced += batch.length;
				console.log(
					`[BSE Sync] Batch ${Math.floor(i / batchSize) + 1}: Inserted ${batch.length} bonds`,
				);
			} catch (err) {
...
router.get("/refresh/status", async (req: Request, res: Response) => {
	try {
		...
		const [publishedCount] = await db
			.select({ count: count() })
			.from(bondCatalog)
			.where(eq(bondCatalog.status, "published"));
...
router.post("/unlisted", async (req: Request, res: Response) => {
	try {
		...
		if (!bondData.isin || !bondData.bondName || !bondData.issuerName) {
			return res
				.status(400)
				.json({ error: "ISIN, bond name, and issuer name are required" });
		}
		...
		if (existing.length > 0) {
			return res
				.status(400)
				.json({ error: "Bond with this ISIN already exists" });
		}
