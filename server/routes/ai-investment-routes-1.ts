		...
		if (clientInfo.isProspect) {
			...
			const holdingsWithMarketData = await Promise.all(
				holdings.map(async (holding) => {
					const symbol =
						holding.symbol ||
						holding.name?.replace(/[^A-Za-z0-9]/g, "").substring(0, 10);
					const [market] = symbol
						? await db
								.select()
								.from(marketData)
					...
					const daysToLTCG =
						holdingDays !== null && !isLTCG ? 365 - holdingDays : 0;
			...
			const totalValue = holdingsWithMarketData.reduce(
			...
			return res.json({
				...
				lastUpdated:
					clientInfo.prospect?.updatedAt?.toISOString() ||
...
router.post("/portfolio/manual-entry", async (req, res) => {
	try {
		...
		if (clientInfo.isProspect) {
			...
			const updatedHoldings = await prospectPortfolioSyncService.addHoldings(
				clientId,
				normalizedHoldings as any,
			);
		...
		res.status(500).json({
			success: false,
			error:
				error instanceof z.ZodError ? error.issues : "Failed to add holdings",
		});
	}
});
...
router.post(
	...
	async (req, res) => {
		try {
			...
			const importResult = await unifiedPortfolioImportService.importFromCSV(
				csvContent,
			...
			if (clientInfo.isProspect) {
				...
				await prospectPortfolioSyncService.addHoldings(
					clientId,
					normalizedHoldings as any,
				);
			...
			if (!portfolio) {
				[portfolio] = await db
					.insert(portfolios)
					.values(
						clientInfo.getPortfolioCreateValues(
							"CSV Uploaded Portfolio",
							"uploaded",
						),
					)
					.returning();
...
router.put("/portfolio/update/:holdingId", async (req, res) => {
	try {
		...
		if (clientId) {
			...
			if (clientInfo.isProspect) {
				...
				if (purchaseDate !== undefined)
					updates.purchaseDate = purchaseDate || undefined;
				if (folioNumber !== undefined)
					updates.folioNumber = folioNumber || undefined;
				...
				if (investedValue !== undefined)
					updates.investedValue = Number(investedValue);

				const updatedHoldings =
					await prospectPortfolioSyncService.updateHolding(
						clientId,
						holdingId,
						updates,
					);
		...
		if (currentValue !== undefined)
			updateData.currentValue = String(currentValue);
		if (investedValue !== undefined)
			updateData.investedValue = String(investedValue);
...
router.delete("/portfolio/holding/:holdingId", async (req, res) => {
	try {
		...
		if (clientId && typeof clientId === "string") {
			...
			if (clientInfo.isProspect) {
				const updatedHoldings =
					await prospectPortfolioSyncService.deleteHolding(clientId, holdingId);
				return res.json({
					success: true,
...
router.post("/ai/portfolio/analyze", async (req, res) => {
	try {
		...
		const analysis = await aiInvestmentService.analyzePortfolio(
			clientId,
			portfolioId,
		);
...
router.post("/ai/agent/talking-points", async (req, res) => {
	try {
		...
		const talkingPoints = await aiInvestmentService.generateTalkingPoints(
			clientId,
			analysisId,
		);
...
router.put("/ai/profit-pick/:pickId/approve", async (req, res) => {
	try {
		...
		const updated = await aiInvestmentService.approveProfitPick(
			pickId,
			agentId,
			{
				targetPrice,
