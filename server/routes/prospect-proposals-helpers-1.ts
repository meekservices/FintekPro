...
export async function resolveAgentName(
	...
	try {
		...
		if (lookupEmail) {
			const [ccAgent] = await db
				.select({ fullName: customerCareAgents.fullName })
...
export async function getStoreEligibleMutualFunds(
	options: {
		category?: string;
		riskLevel?: string;
		limit?: number;
...
export async function getStoreEligibleBonds(
...
export async function getStoreEligibleStocks(
	options: {
		limit?: number;
		sector?: string;
		marketCap?: string;
...
export function getExitLoadFromMetadata(
	product: any,
	productType: string,
): {
	...
	if (product.exitLoad !== undefined && product.exitLoad !== null) {
		...
		if (Array.isArray(product.exitLoad)) {
			...
			if (firstLoad?.load) {
				const loadPercent =
					Number.parseFloat(firstLoad.load.replace("%", "")) || 0;
				return {
		...
		const exitLoadValue = Number.parseFloat(product.exitLoad);
		if (!Number.isNaN(exitLoadValue) && exitLoadValue > 0) {
			return {
	...
	const defaultExitLoads: Record<
		string,
		{ applicable: boolean; percent: number; periodDays: number }
	> = {
	...
	if (category.includes("elss") || category.includes("tax")) {
		return {
			...
			exitLoadPercent: 0,
			exitLoadPeriodDays: 0,
		}; // ELSS has 3-year lock-in, no exit load
	}

	const defaults =
		defaultExitLoads[productType] || defaultExitLoads.mutual_fund;
	return {
...
export function deriveValuationMetrics(
	product: any,
	productType: string,
	returns1Y: number,
	returns3Y: number,
	...
	else if (category.includes("hybrid") || category.includes("balanced"))
		catKey = "hybrid";
	else if (
	...
	let pe = Number.parseFloat(product.pe || product.peRatio || "0");
	let pbRatio = Number.parseFloat(product.pb || product.pbRatio || "0");
	let roe = Number.parseFloat(product.roe || "0");
	let dividendYield = Number.parseFloat(
		product.dividendYield || product.divYield || "0",
	);
	let epsGrowth3Y = Number.parseFloat(
		product.epsGrowth || product.epsGrowth3Y || "0",
	);
	...
	if (!pe && productType !== "debt" && productType !== "bond") {
		...
		pe = Number.parseFloat((benchmark.pe * returnsFactor).toFixed(1));
	}

	// Derive PB from PE and ROE relationship (PB = PE * ROE / 100)
	if (!pbRatio && pe > 0) {
		const estimatedRoe = roe || benchmark.roe;
		pbRatio = Number.parseFloat(((pe * estimatedRoe) / 100).toFixed(2));
	} else if (!pbRatio) {
	...
	if (!roe) {
		...
		roe = Number.parseFloat(
			(benchmark.roe * (1 + (returns1Y - 12) / 40)).toFixed(1),
		);
	...
	if (!dividendYield) {
		...
		if (category.includes("value") || category.includes("dividend"))
			dividendYield *= 1.5;
	}

	// Derive EPS growth from 3Y returns
	if (!epsGrowth3Y) {
		epsGrowth3Y = Number.parseFloat((returns3Y * 0.85).toFixed(1)); // EPS growth typically ~85% of fund returns
	...
	if (aum) {
		...
		if (aumMatch) {
			aumValue = Number.parseFloat(aumMatch[0].replace(/,/g, ""));
	...
	const concentration =
		Number.parseFloat(product.top10Holdings || product.concentration || "0") ||
	...
	const priceVs52WH =
		productType === "stock"
			? Number.parseFloat(
					product.priceVs52WH || String(-10 + Math.random() * 20),
				)
			: 0;

	// Sector performance (YTD relative to Nifty)
	const sectorPerf = Number.parseFloat(
		product.sectorPerf || String(returns1Y - 12),
	);

	// IRR for AIFs
	const irr =
		productType === "aif"
			? Number.parseFloat(product.irr || String(returns3Y * 1.1))
			: undefined;

	// Guard against division by zero for debt/bond categories where benchmarks are 0
	const peVsCat =
		benchmark.pe > 0
			? Number.parseFloat(((pe / benchmark.pe - 1) * 100).toFixed(1))
			: 0;
	const pbVsCat =
		benchmark.pb > 0
			? Number.parseFloat(((pbRatio / benchmark.pb - 1) * 100).toFixed(1))
			: 0;
	const roeVsCat =
		benchmark.roe > 0
			? Number.parseFloat(((roe / benchmark.roe - 1) * 100).toFixed(1))
			: 0;

	return {
		pe: Number.parseFloat(pe.toFixed(1)),
		peVsCat,
		pbRatio: Number.parseFloat(pbRatio.toFixed(2)),
		pbVsCat,
		roe: Number.parseFloat(roe.toFixed(1)),
		roeVsCat,
		dividendYield: Number.parseFloat(dividendYield.toFixed(2)),
		epsGrowth3Y: Number.parseFloat(epsGrowth3Y.toFixed(1)),
		aum,
		aumCategory,
		downsideCapture: Math.max(50, Math.min(150, downsideCapture)),
		styleBox,
		concentration,
		priceVs52WH: Number.parseFloat(priceVs52WH.toFixed(1)),
		sectorPerf: Number.parseFloat(sectorPerf.toFixed(1)),
		irr,
	};
}

export async function generateAIEnhancedRationale(
	product: any,
	productType: string,
	recommendationType: "BUY" | "SELL" | "HOLD" | "SWITCH" = "BUY",
) {
	const baseResult = generateAnalyticalRationale(
		product,
		productType,
		recommendationType,
	);
	...
	try {
		const productName =
			product.schemeName || product.name || product.companyName || "Investment";
		const returns1Y = Number.parseFloat(
			product.returns1y || product.return1Y || "0",
		);
		...
		const { result, modelUsed } =
			await unifiedAIRecommendationEngine.runPrompt<string>({
				prompt,
				category: productType as any,
				cacheKey: `proposal-rationale:${productType}:${productName}:${recommendationType}`,
				cacheTtlMinutes: 60,
				systemPrompt:
					"You are an Indian SEBI-registered investment advisor generating brief, data-driven rationales.",
...
export function generateAnalyticalRationale(
	product: any,
	productType: string,
	recommendationType: "BUY" | "SELL" | "HOLD" | "SWITCH" = "BUY",
): {
	...
	const returns3Y = Number.parseFloat(
		product.returns3y || product.returns3Y || "0",
	);
	...
	const estimatedVolatility =
		productType === "bond" || productType === "debt"
			? 3.5
			: productType === "stock"
				? 22
				: productType === "pms" || productType === "aif"
					? 18
					: 15;
	const sharpeRatio =
		estimatedVolatility > 0
			? Number.parseFloat(
					((returns1Y - riskFreeRate) / estimatedVolatility).toFixed(2),
				)
			: 0;
			...
			: productType === "stock"
				...
				: productType === "pms"
					...
					: productType === "aif"
						? 14
						: 11;
	const alpha = Number.parseFloat((returns1Y - benchmarkReturn).toFixed(2));
	...
	const valuationMetrics = deriveValuationMetrics(
		product,
		productType,
		returns1Y,
		returns3Y,
	...
	if (product.categoryRank) {
		...
		const stars = Number.parseInt(product.rating) || 3;
		const rankEstimate =
			stars >= 5 ? "1-5" : stars >= 4 ? "6-15" : stars >= 3 ? "16-30" : "31+";
	...
	if (product.expenseRatio !== undefined && product.expenseRatio !== null) {
		expenseRatio = Number.parseFloat(product.expenseRatio);
	} else if (product.ter !== undefined && product.ter !== null) {
		expenseRatio = Number.parseFloat(product.ter);
	} else {
		expenseRatio =
			productType === "bond" || productType === "debt"
				? 0.35
	...
	let rationale = "";
	const category =
		product.category || product.schemeName?.split(" ")[0] || "Fund";
	const productName =
		product.schemeName ||
		product.name ||
		product.companyName ||
	...
	if (productType === "mutual_fund" || productType === "mf" || !productType) {
		...
		rationale = buildStockRationale(recommendationType, {
			...
			dividendYield,
			sectorPerf,
			priceVs52WH: valuationMetrics.priceVs52WH,
			sharpeRatio,
			alpha,
			returns1Y,
			categoryRank,
		...
		// AIF rationale
		rationale = buildAIFRationale(
			recommendationType,
			{
				irr: irr || returns3Y * 1.1,
				concentration,
				alpha,
				sharpeRatio,
				returns1Y,
		...
		rationale = buildDefaultRationale(
			recommendationType,
			{
				sharpeRatio,
				alpha,
...
export function buildMFRationale(
	recType: string,
	m: any,
	exitLoad: any,
): string {
	...
	if (recType === "BUY") {
			...
			const valuation =
				m.peVsCat < -10
					...
					: m.peVsCat > 10
						...
						: "fairly valued";
			parts.push(
				`Portfolio P/E of ${m.pe}x (${m.peVsCat > 0 ? "+" : ""}${m.peVsCat}% vs category) - ${valuation}`,
			);
		}
		...
		// Style & efficiency
		parts.push(
			`${m.styleBox} style with ${m.expenseRatio.toFixed(2)}% expense ratio`,
		);
	...
	if (recType === "SELL") {
		if (m.peVsCat > 15)
			parts.push(`Overvalued at ${m.pe}x P/E (${m.peVsCat}% above category)`);
		if (m.roeVsCat < -15)
			parts.push(`Weak ROE of ${m.roe}% vs category average`);
		if (m.sharpeRatio < 0.2)
			parts.push(`Poor risk-adjusted returns (Sharpe: ${m.sharpeRatio})`);
		if (m.alpha < -3)
			parts.push(
				`Underperforming benchmark by ${Math.abs(m.alpha).toFixed(1)}%`,
			);
		if (m.downsideCapture > 110)
			parts.push(`High downside risk (${m.downsideCapture}% capture)`);
		if (exitLoad.exitLoadApplicable) {
			parts.push(
				`Exit load: ${exitLoad.exitLoadPercent}% within ${exitLoad.exitLoadPeriodDays} days`,
			);
		}
		return parts.length > 0
			? parts.join(". ") + ". Consider reallocation."
			: "Performance concerns warrant exit. Consider alternatives.";
	}
	if (recType === "SWITCH") {
		parts.push(
			`Current valuation (P/E: ${m.pe}x, P/B: ${m.pbRatio}x) suggests better opportunities available`,
		);
		if (m.expenseRatio > 1.5)
			parts.push(`High expense ratio of ${m.expenseRatio.toFixed(2)}%`);
		if (m.alpha < 0)
			parts.push(
				`Underperforming benchmark by ${Math.abs(m.alpha).toFixed(1)}%`,
			);
		return (
			parts.join(". ") +
			". Switch to higher-rated alternative in same category."
		);
	}
	// HOLD
	return (
		`Balanced portfolio (P/E: ${m.pe}x, ROE: ${m.roe}%, P/B: ${m.pbRatio}x). ` +
