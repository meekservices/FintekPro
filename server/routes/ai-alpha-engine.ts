...
router.get("/backtest", requireAuth, async (req, res) => {
	try {
		...
		const windowMonths = Number.parseInt(req.query.windowMonths as string) || 6;
		const startDate = req.query.startDate as string | undefined;
		const endDate = req.query.endDate as string | undefined;
		const includeTransactionCosts =
			req.query.includeTransactionCosts !== "false";
...
router.get("/backtest/walk-forward", requireAuth, async (req, res) => {
	try {
		...
		res
			...
			.json({
				success: false,
				error: error.message || "Failed to run walk-forward backtest",
			});
	}
});
...
router.get("/backtest/history", requireAuth, async (req, res) => {
	try {
		...
		const history = await aiBacktestingEngine.getBacktestHistory(
			assetClass,
			limit,
		);
...
router.get("/regime/current", requireAuth, async (req, res) => {
	try {
		...
		res
			...
			.json({
				success: false,
				error: error.message || "Failed to fetch current regime",
			});
	}
});
...
router.get("/regime/detect", requireAdmin, async (req, res) => {
	try {
		...
		res
			...
			.json({
				success: false,
				error: error.message || "Failed to detect regime",
			});
	}
});
...
router.get("/regime/history", requireAuth, async (req, res) => {
	try {
		const days = Number.parseInt(req.query.days as string) || 90;
		...
		res
			...
			.json({
				success: false,
				error: error.message || "Failed to fetch regime history",
			});
	}
});
...
router.get("/regime/distribution", requireAuth, async (req, res) => {
	try {
		const days = Number.parseInt(req.query.days as string) || 90;
		const distribution =
			await aiRegimeDetectionEngine.getRegimeDistribution(days);
		...
		res
			...
			.json({
				success: false,
				error: error.message || "Failed to fetch regime distribution",
			});
	}
});
...
router.get("/picks/optimized", requireAuth, async (req, res) => {
	try {
		const maxPositions =
			Number.parseInt(req.query.maxPositions as string) || 10;
		const includeAlternatives = req.query.includeAlternatives !== "false";
		const maxWeightPerAsset =
			Number.parseFloat(req.query.maxWeightPerAsset as string) || 0.25;
		...
		res
			...
			.json({
				success: false,
				error: error.message || "Failed to optimize basket",
			});
	}
});
...
router.get("/picks/diversification", requireAuth, async (req, res) => {
	try {
		const diversification =
			await aiPortfolioOptimizer.getDiversificationScore();
		...
		res
			...
			.json({
				success: false,
				error: error.message || "Failed to fetch diversification analysis",
			});
	}
});
...
router.post("/models/:id/activate", requireAdmin, async (req, res) => {
	try {
		...
		if (Number.isNaN(modelId)) {
			return res
				.status(400)
				.json({ success: false, error: "Invalid model ID" });
		}
...
router.get("/status", async (_req, res) => {
	try {
		...
		const currentRegime = regimeRow
			? {
					label: regimeRow.regimeLabel,
					confidence: regimeRow.confidence,
					date: regimeRow.regimeDate,
				}
		...
		res
			...
			.json({
				success: false,
				error: error.message || "Failed to fetch AI engine status",
			});
	}
});
...
router.post("/scoring/train", requireAdmin, async (req, res) => {
	try {
		const { assetClass, targetDays, maxStumps, learningRate, nFolds } =
			req.body;
		const config = {
			assetClass: assetClass || undefined,
			targetDays: targetDays ? Number.parseInt(targetDays) : 7,
			maxStumps: maxStumps ? Number.parseInt(maxStumps) : 50,
			learningRate: learningRate ? Number.parseFloat(learningRate) : 0.1,
			nFolds: nFolds ? Number.parseInt(nFolds) : 5,
		};
		...
		res
			.status(500)
			.json({ success: false, error: error.message || "Training failed" });
	}
});
	...
	try {
		...
		if (!assetId || !assetClass || !features) {
			return res
				...
				.json({
					success: false,
					error: "assetId, assetClass, and features are required",
				});
		}
		const result = await aiMLScoringEngine.score(
			assetId,
			assetClass,
			features,
			regime,
		...
		if (!result) {
			return res.json({
				...
				result: null,
				message:
					"No active model for this asset class, using rule-based scoring",
			});
		}
		res.json({ success: true, result });
	} catch (error: any) {
		...
		res
			.status(500)
			.json({ success: false, error: error.message || "Prediction failed" });
	}
});

router.get(
	"/scoring/evaluate/:modelName/:modelVersion",
	requireAdmin,
	async (req, res) => {
		try {
			const { modelName, modelVersion } = req.params;
			const evaluation = await aiMLScoringEngine.evaluateModel(
				modelName,
				modelVersion,
			);
			...
			res
				.status(500)
				.json({ success: false, error: error.message || "Evaluation failed" });
		}
	},
);
...
router.get("/xai/explain/:pickId", requireAuth, async (req, res) => {
	try {
		const pickId = Number.parseInt(req.params.pickId);
...
router.get("/xai/similar/:pickId", requireAuth, async (req, res) => {
	try {
		...
		res
			...
			.json({
				success: false,
				error: error.message || "Failed to find similar patterns",
			});
	}
});

router.get(
	"/xai/feature-importance/:assetClass",
	requireAuth,
	async (req, res) => {
		try {
...
router.get("/xai/calibration", requireAuth, async (req, res) => {
	try {
		...
		res
			...
			.json({
				success: false,
				error: error.message || "Failed to get calibration",
			});
	}
});
...
router.post("/feedback/log", requireAuth, async (req, res) => {
	try {
		...
		const { pickId, interactionType, metadata, sessionId, deviceType } =
			req.body;
		if (!pickId || !interactionType) {
			return res
				...
				.json({
					success: false,
					error: "pickId and interactionType are required",
				});
		}
		await aiFeedbackEngine.logInteraction({
			userId,
			pickId: Number.parseInt(pickId),
			interactionType,
			metadata,
			sessionId,
			deviceType,
...
router.get("/picks/personalized", requireAuth, async (req, res) => {
	try {
		...
		res
			...
			.json({
				success: false,
				error: error.message || "Failed to get personalized picks",
			});
	}
});
...
router.get("/feedback/history", requireAuth, async (req, res) => {
	try {
		...
		res
			...
			.json({
				success: false,
				error: error.message || "Failed to get interaction history",
			});
	}
});
...
router.get("/governance/check", requireAdmin, async (req, res) => {
	try {
		...
		res
			...
			.json({
				success: false,
				error: error.message || "Governance check failed",
			});
	}
});

router.get(
	"/governance/model/:modelName/:modelVersion",
	requireAdmin,
	async (req, res) => {
		try {
			const { modelName, modelVersion } = req.params;
			const report = await aiModelGovernance.checkModelHealth(
				modelName,
				modelVersion,
			);
			...
			res
				...
				.json({
					success: false,
					error: error.message || "Failed to check model health",
				});
		}
	},
);
...
router.post(
	...
	async (req, res) => {
		try {
			...
			res
				.status(500)
				.json({ success: false, error: error.message || "Retrain failed" });
		}
	},
);

router.post(
	"/governance/rollback/:assetClass",
	requireAdmin,
	async (req, res) => {
		try {
			const { assetClass } = req.params;
			const { targetVersion } = req.body;
			const result = await aiModelGovernance.rollbackModel(
				assetClass,
				targetVersion,
			);
			res.json({ success: true, result });
			...
			res
				.status(500)
				.json({ success: false, error: error.message || "Rollback failed" });
		}
	},
);

router.get(
	"/governance/history/:assetClass",
	requireAdmin,
	async (req, res) => {
		try {
			const { assetClass } = req.params;
...
router.post("/governance/update-outcomes", requireAdmin, async (req, res) => {
	try {
		...
		res
			...
			.json({
				success: false,
				error: error.message || "Failed to update outcomes",
			});
	}
});
