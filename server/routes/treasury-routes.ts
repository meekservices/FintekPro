...
router.post("/entities/:id/accounts", isAuthenticated, async (req, res) => {
	try {
		const account = await treasuryService.linkBankAccount(
			req.params.id,
			req.body,
		);
...
router.get(
	"/entities/:id/consolidated-position",
	isAuthenticated,
	async (req, res) => {
...
router.get("/entities/:id/forecast", isAuthenticated, async (req, res) => {
	try {
		const { ForecastingService } = await import(
			"../../modules/forecasting/forecasting.service"
		);
		const forecastingService = new ForecastingService();

		const days = Number.parseInt(req.query.days as string) || 30;
		const forecast = await forecastingService.generateLiquidityForecast(
			req.params.id,
			days,
		);
...
router.get(
	...
	async (req, res) => {
		try {
			...
			const analysis = await forecastingService.generateAIAnalysis(
				req.params.id,
				forecast,
			);
