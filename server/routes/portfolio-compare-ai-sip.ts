...
export function registerPortfolioCompareAISIPRoutes(app: Express): void {
	app.post("/api/portfolios/compare", async (req: any, res) => {
		try {
			...
			const portfolioComparisonService = new PortfolioComparisonService(
				storage as any,
			);
	...
	app.get("/api/users/:userId/portfolio-comparisons", async (req, res) => {
		try {
			...
			const comparisons =
				await portfolioComparisonService.getUserComparisons(userId);
	...
	app.get("/api/amfi/sip-calculator", async (req, res) => {
		try {
			...
			const monthlyAmt = Number.parseFloat(String(monthly_investment)) || 5000;
			const tenureYears = Number.parseInt(String(tenure_years)) || 10;
			const annualReturn = Number.parseFloat(String(expected_return)) || 12;
	...
	app.get(
		"/api/ai/portfolios/:portfolioId/rebalancing-recommendations",
		requireAuth,
		async (req, res) => {
			try {
				...
				if (portfolio.userId !== userId) {
					return res
						.status(403)
						.json({ error: "Access denied: Portfolio not owned by user" });
				}
				...
				const portfolioData = {
					id: portfolioId,
					totalValue: Number.parseFloat(portfolio.totalValue || "0"),
					holdings: holdings.map((h) => ({
						symbol: h.symbol,
						quantity: Number.parseFloat(h.quantity),
						currentPrice: Number.parseFloat(h.avgPrice), // Using avg price as current price for now
						currentValue:
					...
					assetAllocation:
						assetAllocation?.map((a) => ({
							...
							percentage: Number.parseFloat(a.currentPercentage || "0"),
							currentValue: Number.parseFloat(a.currentValue || "0"),
						})) || [],
					performance: {
						totalGainLoss: Number.parseFloat(performance?.totalGainLoss || "0"),
						totalGainLossPercent: Number.parseFloat(
							performance?.totalGainLossPercent || "0",
						),
						dayChange: Number.parseFloat(performance?.dayChange || "0"),
						dayChangePercent: Number.parseFloat(
				...
				const aiRecommendations =
					await aiPortfolioService.generatePortfolioRebalancingRecommendations(
						portfolioData,
						userProfile as any,
					);
				...
				if (!aiRecommendations || aiRecommendations.length === 0) {
					return res.status(500).json({
						...
						message:
							"AI service returned empty recommendations. Please check OpenAI API configuration.",
					});
				}
				...
				console.error(
					"Error generating AI rebalancing recommendations:",
					error,
				);
				res.status(500).json({
					success: false,
	...
	app.post(
		"/api/ai/portfolios/:portfolioId/investment-proposal",
		requireAuth,
		async (req, res) => {
			try {
				const { portfolioId } = req.params;
				...
				if (financialGoals && financialGoals.length > 0) {
					(userProfile as any).financialGoals = financialGoals.map((goal) => ({
						...
						targetAmount: Number.parseFloat(goal.targetAmount),
				...
				const portfolioData = {
					...
					holdings: holdings.map((h) => ({
						symbol: h.symbol,
						quantity: Number.parseFloat(h.quantity),
						currentPrice: Number.parseFloat(h.avgPrice),
						currentValue:
							Number.parseFloat(h.quantity) * Number.parseFloat(h.avgPrice),
						investedValue:
							Number.parseFloat(h.quantity) * Number.parseFloat(h.avgPrice),
				...
				if (!aiProposal) {
					return res.status(500).json({
						...
						message:
							"AI service returned empty proposal. Please check OpenAI API configuration.",
					});
				}
			...
			res.json({
				insights: [
					...
					},
					{
						...
						description: "Potential tax savings of ₹15,000 identified",
						priority: "high",
						actionable: true,
					},
					{
						...
						description: "IT sector comprises 40% of your portfolio",
						priority: "low",
						actionable: false,
					},
				],
