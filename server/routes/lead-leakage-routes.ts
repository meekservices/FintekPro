...
const requireAgentOrPartner = (req: any, res: Response, next: any) => {
	if (
		!req.user ||
		!["agent", "partner", "admin", "superadmin", "tester"].includes(
			req.user.role,
		)
	)
...
export function registerLeadLeakageRoutes(app: Express) {
	...
	app.post(
		"/api/leads/:leadId/processing-mode",
		requireAgentOrPartner,
		async (req: any, res: Response) => {
			try {
		...
		async (req: any, res: Response) => {
			try {
				...
				if (req.user.role === "agent") {
					...
					leads = await leadRegistryService.getLeadsByPartner(
						req.user.partnerId || req.user.id,
					);
				}
				res.json(leads);
			} catch (error: any) {
	...
	app.post(
		...
		async (req: any, res: Response) => {
			try {
				...
				if (result.claim) {
					bankerConfirmationService
						.triggerBankerConfirmation(result.claim.claimId)
						.catch((err) => {
							console.error(
								"Failed to trigger banker confirmation email:",
								err,
							);
						});
				}
	...
	app.post(
		"/api/payout-claims/:claimId/proof",
		requireAuth,
		async (req: any, res: Response) => {
			try {
	...
	app.post(
		"/api/payout-claims/:claimId/banker-confirmation",
		requireAdmin,
		async (req: any, res: Response) => {
			try {
				...
				const result = await payoutClaimService.sendBankerConfirmation(
					req.params.claimId,
					parsed.data,
				);
				if (!result.success) {
	...
	app.post(
		"/api/payout-claims/:claimId/confirm-banker-reply",
		requireAdmin,
		async (req: any, res: Response) => {
			try {
	...
	app.post(
		"/api/payout-claims/:claimId/pdd-clearance",
		requireAdmin,
		async (req: any, res: Response) => {
			try {
	...
	app.post(
		"/api/payout-claims/:claimId/reject",
		requireAdmin,
		async (req: any, res: Response) => {
			try {
	...
	app.post(
		"/api/payout-claims/:claimId/clawback",
		requireAdmin,
		async (req: any, res: Response) => {
			try {
				const parsed = reasonSchema.safeParse(req.body);
	...
	app.get(
		...
		async (req: any, res: Response) => {
			try {
				...
				const proofs = await payoutClaimService.getProofsByClaimId(
					req.params.claimId,
				);
	...
	app.get(
		"/api/payout-claims/:claimId/proofs",
		requireAuth,
		async (req: any, res: Response) => {
			try {
				const proofs = await payoutClaimService.getProofsByClaimId(
	...
	app.post(
		"/api/admin/master-dsa-claims/:payoutClaimId/create",
		requireAdmin,
		async (req: any, res: Response) => {
			try {
				...
				const result = await masterDsaClaimService.createClaimFromPayout(
					req.params.payoutClaimId,
					req.user.id,
					{
						...parsed.data,
						claimedAmount: parsed.data.claimedAmount.toString(),
					},
				);
				if (!result.success) {
					return res.status(400).json({ error: result.error });
	...
	app.post(
		"/api/admin/master-dsa-claims/:dsaClaimId/attachments",
		requireAdmin,
		async (req: any, res: Response) => {
			try {
				...
				const result = await masterDsaClaimService.addAttachment(
					req.params.dsaClaimId,
					{
						...parsed.data,
						adminId: req.user.id,
					},
	...
	app.post(
		"/api/admin/master-dsa-claims/:dsaClaimId/submit",
		requireAdmin,
		async (req: any, res: Response) => {
			try {
				const result = await masterDsaClaimService.submitClaim(
					req.params.dsaClaimId,
					req.user.id,
				);
				if (!result.success) {
	...
	app.post(
		"/api/admin/master-dsa-claims/:dsaClaimId/send-email",
		requireAdmin,
		async (req: any, res: Response) => {
			try {
				const result = await masterDsaClaimService.sendClaimEmail(
					req.params.dsaClaimId,
					req.user.id,
				);
				if (!result.success) {
	...
	app.post(
		"/api/admin/master-dsa-claims/:dsaClaimId/status",
		requireAdmin,
		async (req: any, res: Response) => {
			try {
	...
	app.post(
		"/api/admin/master-dsa-claims/:dsaClaimId/payments",
		requireAdmin,
		async (req: any, res: Response) => {
			try {
				...
				const result = await masterDsaClaimService.recordPayment(
					req.params.dsaClaimId,
					{
						...parsed.data,
	...
	app.get(
		...
		async (req: any, res: Response) => {
			try {
				...
				const claims = await masterDsaClaimService.getAllClaims(
					status as string,
				);
				res.json(claims);
			} catch (error: any) {
	...
	app.get(
		"/api/admin/master-dsa-claims/:dsaClaimId",
		requireAdmin,
		async (req: any, res: Response) => {
			try {
				const result = await masterDsaClaimService.getClaimById(
					req.params.dsaClaimId,
				);
				if (!result.claim) {
	...
	app.get(
		"/api/admin/master-dsa-claims/by-payout/:payoutClaimId",
		requireAdmin,
		async (req: any, res: Response) => {
			try {
				const claim = await masterDsaClaimService.getClaimByPayoutId(
					req.params.payoutClaimId,
				);
				res.json({ claim });
			} catch (error: any) {
