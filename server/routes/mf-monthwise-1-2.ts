...
export function registerMFMonthwiPart1Part2Routes(app: Express): void {
		...
		try {
			...
			const analytics = {
				...
				portfolio_summary: {
					...
					total_investment: portfolios.reduce(
				...
				recommendations: [
					{
						type: "rebalancing",
						message:
							"Consider rebalancing your portfolio - equity allocation is high",
					},
					{
	...
	app.get("/api/cams/portfolio/:pan", async (req, res) => {
		try {
			...
			const portfolios = await camsApi.getInvestorPortfolio(
				pan,
				folio as string,
			);
		...
		try {
			...
			if (
				...
				return res.status(400).json({
					status: "error",
					error:
						"PAN, scheme code, amount, investor name, bank account, and IFSC are required",
				});
			}
		...
		try {
			...
			if (
				...
				return res.status(400).json({
					status: "error",
					error:
						"PAN, folio, scheme code, redemption type, bank account, and IFSC are required",
				});
			}
		...
		try {
			...
			if (
				...
				return res.status(400).json({
					status: "error",
					error:
						"PAN, scheme code, amount, frequency, start date, bank account, and IFSC are required",
				});
			}
