...
router.get(
	...
	async (req: Request, res: Response): Promise<void> => {
		try {
			...
			const snapshot = await alpacaMarketDataService.getSnapshot(
				symbol.toUpperCase(),
			);
			if (!snapshot) {
				res
	...
	async (req: Request, res: Response): Promise<void> => {
		try {
			...
			if (!symbols) {
				res
					.status(400)
					.json({ success: false, error: "symbols query param required" });
				return;
			}
			...
			if (symbolList.length > 100) {
				res
					.status(400)
					.json({ success: false, error: "Max 100 symbols per request" });
				return;
			}
		...
		try {
			...
			if (!validTimeframes.includes(timeframe)) {
				res
					...
					.json({
						success: false,
						error: `Invalid timeframe. Use one of: ${validTimeframes.join(", ")}`,
					});
				return;
			}
			...
			const barsMap = await alpacaMarketDataService.getBars(
				...
				Math.min(Number.parseInt(limit) || 365, 10000),
			);
	...
	async (req: Request, res: Response): Promise<void> => {
		try {
			...
			const symbolList = (symbols as string)
				...
				.filter(Boolean);
			const quotesMap =
				await alpacaMarketDataService.getLatestQuotes(symbolList);
	...
	async (req: Request, res: Response): Promise<void> => {
		try {
			...
			const symbolList = (symbols as string)
				...
				.filter(Boolean);
			const tradesMap =
				await alpacaMarketDataService.getLatestTrades(symbolList);
			const trades: Record<string, any> = {};
			tradesMap.forEach((t, sym) => {
				trades[sym] = t;
			});
	...
	try {
		...
		if (!data.consent || !data.lrsDeclaration) {
			res.status(400).json({
				success: false,
				error:
					"Both trade consent and LRS declaration must be acknowledged before placing order",
			});
			return;
		}
		...
		if (alpacaBrokerService.isConfigured()) {
			try {
				...
				if (accountInfo?.pattern_day_trader) {
					const equity = Number.parseFloat(accountInfo.equity || "0");
					if (equity < 25_000) {
						res.status(403).json({
							success: false,
							error:
								"Pattern Day Trader (PDT) restriction: your account equity is below $25,000. " +
			...
			try {
				...
				if (asset && !asset.fractionable) {
					res.status(400).json({
						success: false,
						error:
							`${data.symbol.toUpperCase()} is not eligible for fractional trading. ` +
		...
		await usTradingService.recordLrsDeclaration({
			...
			declarationText:
				"I declare this transaction is within my LRS limit and complies with FEMA regulations",
			declarationHash: crypto
				.createHash("sha256")
				.update(
					JSON.stringify({
						userId,
...
router.get("/orders", async (req: Request, res: Response): Promise<void> => {
	try {
		...
		const orders = await usTradingService.getOrders(
			userId,
		...
		const enrichedHoldings = await Promise.all(
			holdings.map(async (holding) => {
				...
				return {
					...
					marketValueInr: quote
						? Number.parseFloat(holding.quantity) * quote.price * fxRate
						: null,
				};
			}),
		);
			...
			(sum, h) =>
				...
				(h.currentPriceUsd
					? Number.parseFloat(h.quantity) *
						Number.parseFloat(h.currentPriceUsd.toString())
					: 0),
			0,
		);
...
router.delete(
	...
	async (req: Request, res: Response): Promise<void> => {
		try {
			...
			await usTradingService.removeFromWatchlist(
				userId,
