...
router.post("/applications", async (req: Request, res: Response) => {
	try {
		...
		if (parsed.clientMode === "existing" && parsed.clientId) {
			const isValidMapping = await validateAgentClientMapping(
				agentId,
				parsed.clientId,
			);
			if (!isValidMapping) {
				return res.status(403).json({
					success: false,
					error:
						"You are not authorized to create applications for this client. Please request mapping approval from admin.",
				});
			}
		}

		if (
			parsed.routingMode === "manual" &&
			(!parsed.targetBanks || parsed.targetBanks.length === 0)
		) {
			return res.status(400).json({
				success: false,
				error:
					"At least one bank must be selected when using manual routing mode.",
			});
		}
		...
		if (isIrisLoanType && parsed.applicantPan) {
			try {
					...
					new Promise((resolve) =>
						setTimeout(
							() =>
								resolve({
									success: false,
									warning: "Eligibility check timed out",
								}),
							8000,
						),
					),
				...
				irisEligibility = {
					success: false,
					warning: "IRIS eligibility check unavailable",
				};
			}
		}
		// ─── End IRIS Intercept ───────────────────────────────────────────────────
		...
		try {
			if (parsed.applicantPan && parsed.applicantPhone) {
				const loanTypeMap: Record<string, string> = {
					...
					las: "Loan Against Securities (IRIS)",
				};
				...
				if (leadResult.success) {
					...
					if (
						parsed.processingMode === "EXTERNAL_FINANCIER" &&
						parsed.financierName
					) {
						try {
		...
		const [application] = await db
			...
			.values({
				...
				leadRegistryId,
				routingMode:
					parsed.processingMode === "PLATFORM"
						? (parsed.routingMode as any)
						: undefined,
				targetBanks:
					parsed.processingMode === "PLATFORM" ? parsed.targetBanks || [] : [],
		...
		await logAgentAction({
			...
			newValue: {
				loanType: parsed.loanType,
				amount: parsed.requestedAmount,
				clientMode: parsed.clientMode,
				processingMode: parsed.processingMode,
				irisRouted: isIrisLoanType,
			},
			req,
		});
		...
		try {
			...
			if (connection) {
				...
				console.log(
					`[Zoho CRM] Loan lead ${applicationNumber} synced to Zoho Lead ${zohoLeadId}`,
				);
			}
		} catch (zohoError: any) {
			console.warn(
				"[Zoho CRM] Loan lead sync failed (non-blocking):",
				zohoError?.message,
			);
		}
		...
		res.status(201).json({
			...
			...(isIrisLoanType && {
				...
				irisNextSteps: {
					...
					disclaimer:
						"Loan Against Securities requires pledge of MF folios or demat securities via IRIS KFintech. Use the IRIS LAS endpoints to complete the pledge and loan application.",
				},
			}),
		});
	} catch (error: any) {
