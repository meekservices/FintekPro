...
export function registerLoanAdminPart1Routes(app: Express) {
	...
	app.get(
		"/api/admin/loan-marketplace/products",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
				...
				res
					.status(500)
					.json({ success: false, error: "Failed to fetch loan products" });
			}
		},
	);
	...
	app.get(
		"/api/admin/loan-marketplace/providers",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
				...
				res
					.status(500)
					.json({ success: false, error: "Failed to fetch loan providers" });
			}
		},
	);
	...
	app.get(
		"/api/admin/loan-marketplace/providers/:providerKey",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
				...
				res
					.status(500)
					.json({ success: false, error: "Failed to fetch provider" });
			}
		},
	);
	...
	app.get(
		"/api/admin/loan-marketplace/staff",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
				...
				if (providerId)
					conditions.push(
						eq(schema.lenderStaff.providerId, providerId as string),
					);
				if (status)
					conditions.push(eq(schema.lenderStaff.status, status as string));
				if (designation)
					conditions.push(
						eq(schema.lenderStaff.designation, designation as string),
					);
				...
				res
					.status(500)
					.json({ success: false, error: "Failed to fetch lender staff" });
			}
		},
	);
	...
	app.get(
		"/api/admin/loan-marketplace/staff/:staffId",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
				...
				res
					.status(500)
					.json({ success: false, error: "Failed to fetch staff member" });
			}
		},
	);
	...
	app.post(
		...
		async (req: any, res: Response) => {
			try {
				...
				res
					.status(500)
					.json({ success: false, error: "Failed to create lender staff" });
			}
		},
	);
	...
	app.post(
		"/api/admin/loan-marketplace/providers/:providerId/staff",
		requireAdmin,
		async (req: any, res: Response) => {
			try {
	...
	app.patch(
		...
		async (req: any, res: Response) => {
			try {
				...
				res
					.status(500)
					.json({ success: false, error: "Failed to update lender staff" });
			}
		},
	);
	...
	app.delete(
		...
		async (req: any, res: Response) => {
			try {
				...
				res
					.status(500)
					.json({ success: false, error: "Failed to delete lender staff" });
			}
		},
	);
	...
	app.post(
		"/api/admin/loan-marketplace/staff/:staffId/status-change",
		requireAdmin,
		async (req: any, res: Response) => {
			try {
				const { staffId } = req.params;
				...
				const [historyRecord] = await db
					.insert(schema.lenderStaffHistory)
					.values({
						staffId,
						...
						newProviderId:
							validatedData.newProviderId || currentStaff.providerId,
						newDesignation:
							validatedData.newDesignation || currentStaff.designation,
						newStatus: validatedData.newStatus,
						newBranchCode:
							validatedData.newBranchCode || currentStaff.branchCode,
						newReportsToId:
							validatedData.newReportsToId || currentStaff.reportsToId,
						effectiveDate: new Date(validatedData.effectiveDate),
						reason: validatedData.reason,
						remarks: validatedData.remarks,
						relievingDate: validatedData.relievingDate
							? new Date(validatedData.relievingDate)
							: undefined,
						lastWorkingDay: validatedData.lastWorkingDay
							? new Date(validatedData.lastWorkingDay)
							: undefined,
						isEligibleForRehire: validatedData.isEligibleForRehire,
						leaveType: validatedData.leaveType,
						leaveStartDate: validatedData.leaveStartDate
							? new Date(validatedData.leaveStartDate)
							: undefined,
						leaveEndDate: validatedData.leaveEndDate
							? new Date(validatedData.leaveEndDate)
							: undefined,
				...
				if (validatedData.newProviderId)
					updateData.providerId = validatedData.newProviderId;
				if (validatedData.newDesignation)
					updateData.designation = validatedData.newDesignation;
				if (validatedData.newBranchCode)
					updateData.branchCode = validatedData.newBranchCode;
				if (validatedData.newReportsToId)
					updateData.reportsToId = validatedData.newReportsToId;
				...
				if (validatedData.leadsReassignedTo) {
					...
					await db
						.update(schema.lenderStaffHistory)
				...
				res
					.status(500)
					.json({ success: false, error: "Failed to change staff status" });
			}
		},
	);
