...
export function registerLoanPart2Routes(app: Express) {
	...
	app.get(
		"/api/marketplace/applications/:applicationId",
		requireLevel1,
		async (req: any, res: any) => {
			try {
	...
	app.get(
		"/api/marketplace/applications/:applicationId/documents",
		requireLevel1,
		async (req: any, res: any) => {
			try {
				const { applicationId } = req.params;
				...
				const documents =
					await storage.getLoanApplicationDocuments(applicationId);
	...
	app.post(
		...
		async (req: any, res: any) => {
			try {
				...
				const document = await storage.addLoanApplicationDocument(
					applicationId,
					documentData,
				);
	...
	app.post(
		...
		async (req: any, res: any) => {
			try {
				...
				const { dsaLoanService } = await import(
					"../../services/dsa-loan-service"
				);
				...
				res.json({
					...
					data: result,
					message:
						"Background multi-bank routing initiated for better offer discovery",
				});
			} catch (error: any) {
...
export async function registerLoanProcessingRoutes(app: Express) {
	const { loanProcessingService } = await import(
	...
	app.post("/api/icici/loans/personal/eligibility", async (req, res) => {
		try {
			const { amount, monthlyIncome, employmentType, cibilScore, age } =
				req.body;
			...
			const icicioffer = result.offers.find(
				(offer: any) => offer.lenderId === "icici",
			);
	...
	app.post("/api/hdfc/loans/personal/eligibility", async (req, res) => {
		try {
			...
			const result = await loanProcessingService.checkLoanEligibility(request);
			const hdfcOffer = result.offers.find(
				(offer: any) => offer.lenderId === "hdfc",
			);
...
export function registerLoanComparisonRoutes(app: Express) {
	...
	app.post("/api/loan-comparison/generate", async (req, res) => {
		try {
			...
			const { amount, tenure, loanType, monthlyIncome, creditScore } =
	...
	app.get("/api/loan-comparison/offers", async (req, res) => {
		try {
			...
			const params = {
				amount: Number.parseInt(req.query.amount as string),
				tenure: Number.parseInt(req.query.tenure as string),
				loanType: req.query.loanType as string,
				monthlyIncome: Number.parseInt(req.query.monthlyIncome as string),
				creditScore: req.query.creditScore
					? Number.parseInt(req.query.creditScore as string)
					: undefined,
			};
			...
			const response = await fetch(
				`${req.protocol}://${req.get("host")}/api/loan-comparison/generate`,
				{
					method: "POST",
	...
	app.post("/api/loan-comparison/save", async (req, res) => {
		try {
			...
			if (
				!validatedData.selectedOffers ||
				(Array.isArray(validatedData.selectedOffers) &&
					validatedData.selectedOffers.length < 2)
			) {
				return res
					.status(400)
					.json({ error: "At least 2 offers must be selected for comparison" });
			}
