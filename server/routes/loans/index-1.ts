...
export function registerLoanPart1Routes(app: Express) {
		...
		async (req: any, res: any) => {
			try {
				...
				if (
					...
					!loanApplication.applicantDetails ||
				...
				const result =
					await iciciBankAPI.submitLoanApplication(loanApplication);
				...
				if (result.success && result.data) {
					const dbApplication = await storage.createICICILoanApplication({
						...
						expectedDecisionDate: result.data.expectedDecisionDate
							? new Date(result.data.expectedDecisionDate)
							: undefined,
					});
	...
	app.get(
		"/api/icici/loans/status/:applicationId",
		requireAuth,
		async (req: any, res: any) => {
			try {
	...
	app.post(
		"/api/icici/loans/eligibility",
		requireAuth,
		async (req: any, res: any) => {
			try {
				const { loanType, monthlyIncome, existingEmi, loanAmount, tenure } =
					req.body;
	...
	app.get(
		"/api/icici/loans/my-applications",
		requireAuth,
		async (req: any, res: any) => {
			try {
				const userId = req.user!.id;
				const applications =
					await storage.getICICILoanApplicationsByUser(userId);
	...
	app.get(
		"/api/marketplace/credit-profile",
		requireAuth,
		async (req: any, res: any) => {
			try {
				const userId = req.user!.id;
	...
	app.get(
		"/api/marketplace/loan-products/:productKey",
		requireLevel1,
		async (req: any, res: any) => {
			try {
	...
	app.get(
		"/api/marketplace/loan-providers",
		requireLevel1,
		async (req: any, res: any) => {
			try {
	...
	app.get(
		"/api/marketplace/loan-providers/:providerKey/products",
		requireLevel1,
		async (req: any, res: any) => {
			try {
			...
			try {
				...
				if (!productKey || !requestedAmount || !monthlyIncome) {
					return res.status(400).json({
						success: false,
						error:
							"Missing required fields: productKey, requestedAmount, monthlyIncome",
					});
				}
	...
	app.get(
		"/api/marketplace/loan-requests/:requestId/offers",
		requireLevel1,
		async (req: any, res: any) => {
			try {
				const { requestId } = req.params;
