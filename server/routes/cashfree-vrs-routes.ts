...
router.post('/driving-license', requireAdmin, async (req, res) => {
  ...
  if (!dlNumber || !dob) return res.status(400).json({ error: 'dlNumber and dob are required' });
...
router.post('/passport', requireAdmin, async (req, res) => {
  ...
  if (!fileNumber || !name || !dob) return res.status(400).json({ error: 'fileNumber, name, and dob are required' });
...
router.post('/udyam', requireAdmin, async (req, res) => {
  ...
  if (!udyamNumber) return res.status(400).json({ error: 'udyamNumber is required' });
...
router.post('/bank-account', requireAdmin, async (req, res) => {
  ...
  if (!bankAccount || !ifsc) return res.status(400).json({ error: 'bankAccount and ifsc are required' });
  const result = await verifyBankAccountV2({ bankAccount, ifsc, name, phoneNumber });
  res.status(result.success ? 200 : 400).json(result);
});
...
router.post('/upi-penny-drop', requireAdmin, async (req, res) => {
  ...
  if (!vpa) return res.status(400).json({ error: 'vpa (UPI address) is required' });
