		...
		try {
			...
			if (
				...
				return apiResponse.badRequest(
					res,
					"Risk disclosure acknowledgment is required. Please read and accept all mandatory disclosures before placing an order.",
				);
			}
			...
			if (!disclosureValidation.valid) {
				console.log(
					`[COMPLIANCE] buy_request_blocked: Missing risk disclosures | userId: ${req.user.id} | missing: ${disclosureValidation.missingDisclosures.join(", ")}`,
				);
				return apiResponse.badRequest(
					res,
					"All mandatory risk disclosures must be acknowledged before placing an order.",
					{
			...
			const maxPriceNum = Number.parseFloat(validatedData.maxPrice) || 0;
			...
			if (transactionValue >= ACCREDITED_INVESTOR_THRESHOLD) {
				...
				if (!profile?.accreditedInvestorType) {
					console.log(
						`[COMPLIANCE] unlisted_buy_blocked: High-value transaction (₹${transactionValue}) requires accredited investor status | userId: ${req.user.id}`,
					);
					return apiResponse.forbidden(
						res,
			...
			console.log(
				`[COMPLIANCE] unlisted_buy_request: { userId: '${req.user.id}', companyId: '${validatedData.companyId}', quantity: ${validatedData.quantity}, maxValue: ${transactionValue}, disclosureVersion: '${unlistedRiskDisclosureService.getDisclosureVersion()}', outcome: 'success' }`,
			);
	...
	async (req: Request, res: Response) => {
		try {
			...
			const existing = await db
				...
				.from(unlistedCart)
				.where(
					and(eq(unlistedCart.id, id), eq(unlistedCart.userId, req.user.id)),
				);
