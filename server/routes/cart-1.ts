...
export function registerCarPart1Routes(app: Express) {
	app.get("/api/cart", requireAuth, async (req, res) => {
		try {
			...
			const totalValue = cartItems.reduce((sum, item) => {
				const amount = Number.parseFloat(item.investmentAmount || "0");
				if (amount > 0) return sum + amount;
				if (item.product?.minimumInvestment) {
					return sum + Number.parseFloat(item.product.minimumInvestment);
				}
				return sum;
			}, 0);
	...
	app.post("/api/cart/items", requireAuth, async (req, res) => {
		try {
			...
			const count = [hasProduct, hasProposal, hasInvestment].filter(
				Boolean,
			).length;
			...
			if (itemType === "proposal" && !proposalId) {
				return res
					.status(400)
					.json({ error: "proposalId is required for proposal items" });
			}
			if (itemType === "investment" && !investmentId) {
				return res
					.status(400)
					.json({ error: "investmentId is required for investment items" });
			}
			...
			if (productId)
				whereConditions.push(eq(userCartItems.productId, productId));
			if (proposalId)
				whereConditions.push(eq(userCartItems.proposalId, proposalId));
			if (investmentId)
				whereConditions.push(eq(userCartItems.investmentId, investmentId));
	...
	app.put("/api/cart/items/:itemId", requireAuth, async (req, res) => {
		try {
			...
			const [cartItem] = await db
				...
				.where(and(eq(userCartItems.id, itemId), eq(userCart.userId, userId)));
	...
	app.delete("/api/cart/items/:itemId", requireAuth, async (req, res) => {
		try {
			...
			await db.delete(userCartItems).where(eq(userCartItems.id, itemId));
	...
	app.delete("/api/cart", requireAuth, async (req, res) => {
		try {
			...
			if (cart) {
				await db.delete(userCartItems).where(eq(userCartItems.cartId, cart.id));
			}
