...
export function registerCapitalGainPart1Routes(app: Express): void {
	app.post("/api/nsdl/capital-gains", async (req, res) => {
		try {
			...
			if (!accountNumber || !financialYear) {
				return res
					.status(400)
					.json({ error: "Account number and financial year are required" });
			}
	...
	app.post("/api/cdsl/capital-gains", async (req, res) => {
		try {
			...
			if (!boid || !financialYear) {
				return res
					.status(400)
					.json({ error: "BOID and financial year are required" });
			}
	...
	app.delete("/api/capital-gains/reports/:reportId", async (req, res) => {
		try {
			...
			await db
				.delete(capitalGainsReports)
				.where(eq(capitalGainsReports.id, reportId));
	...
	app.post("/api/capital-gains/sell-advice", async (req, res) => {
		try {
			...
			const advice =
				await proposalCapitalGainsService.generateTaxEfficientSellAdvice(
					holdings,
				);
			res.json(advice);
		} catch (error) {
	...
	app.post("/api/capital-gains/simulate-redemption", async (req, res) => {
		try {
			...
			if (!holding || !redemptionAmount) {
				return res
					.status(400)
					.json({ error: "Holding and redemption amount are required" });
			}
			...
			const purchaseNav =
				(holding.investedAmount || holding.currentValue * 0.85) / units;
			...
			const lotWiseSummary = proposalCapitalGainsService.calculateLotWiseTax(
				...
				lots.map((lot: any) => ({
					...
					purchaseValue:
						lot.purchaseValue ||
						(lot.purchaseNav || purchaseNav) * (lot.units || units),
			...
			const redemptionPlan =
				proposalCapitalGainsService.getOptimizedRedemptionPlan(
					lotWiseSummary,
					redemptionAmount,
					strategy,
				);
