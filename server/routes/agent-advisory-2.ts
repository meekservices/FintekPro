...
export function registerAgentAdvisoryPart2Routes(app: Express) {
	app.get(
      ...
      `);
        ...
        `);
					...
					formattedWizardProposals = (wizardProposalsResult.rows || []).map(
						(p: any) => ({
							id: p.id,
							clientId: null,
							title:
								p.proposal_title ||
								`Investment Proposal for ${p.prospect_name}`,
								...
								p.status === "shared"
									...
									: p.status === "viewed"
										...
										: p.status === "converted"
											...
											: "draft",
							investmentAmount:
								p.total_investment_amount || p.investment_amount || 0,
							createdAt: p.created_at,
							updatedAt: p.updated_at,
					...
					console.error(
						"Gracefully caught error fetching prospect_proposals:",
						err,
					);
				}
            ...
            `);
						...
						if (clientName === "Unknown" && proposal.title) {
							...
							if (dashMatch?.[1] && dashMatch[1].trim().length > 0) {
								...
								const forMatch = proposal.title.match(
									/for\s+(.+?)(?:\s*-|\s*$)/i,
								);
								if (forMatch?.[1]) {
	...
	app.get(
		"/api/agent/proposals/:proposalId/items",
		requireAgentPortal,
		async (req: Request, res: Response) => {
			try {
	...
	app.post(
		"/api/agent/proposals/:proposalId/notes",
		requireAgentPortal,
		async (req: Request, res: Response) => {
			try {
				const agentId = (req.user as any).id;
				...
				if (!noteType || !content) {
					return res
						.status(400)
						.json({ error: "Note type and content are required" });
				}
	...
	app.post(
		"/api/agent/proposals/:proposalId/share",
		requireAgentPortal,
		async (req: Request, res: Response) => {
			try {
				const agentId = (req.user as any).id;
	...
	app.delete(
		"/api/agent/proposals/:proposalId",
		requireAgentPortal,
		async (req: Request, res: Response) => {
			try {
				const agentId = (req.user as any).id;
				...
				if (investmentProposal) {
					...
					await db
						.delete(investmentProposalItems)
						.where(eq(investmentProposalItems.proposalId, proposalId));

					// Delete proposal shares
					await db
						.delete(proposalShares)
        ...
        `);
					...
					if (
						prospectProposalResult.rows &&
						prospectProposalResult.rows.length > 0
					) {
          ...
          `);
					...
					console.error(
						"Gracefully caught error deleting from prospect_proposals:",
						err,
					);
				}
	...
	app.post(
		"/api/proposal/view/:shareToken/action",
		async (req: Request, res: Response) => {
			try {
				...
				if (!share) {
					return res
						.status(404)
						.json({ error: "Proposal not found or link expired" });
				}

				if (
					share.shareTokenExpiresAt &&
					new Date(share.shareTokenExpiresAt) < new Date()
				) {
				...
				if (share.clientAction) {
					return res
						.status(400)
						.json({ error: "Action already taken on this proposal" });
				}
				...
				await db
					...
					.set({
						...
						...(action === "request_clarification"
							? ({ clientFeedback: clarificationNote } as any)
							: {}),
					} as any)
				...
				return res
					.status(400)
					.json({ error: error.message || "Failed to process action" });
			}
		},
	);

	app.get(
		"/api/proposal/view/:shareToken",
		async (req: Request, res: Response) => {
			try {
				const { shareToken } = req.params;
	...
	app.get(
		...
		async (req: Request, res: Response) => {
			try {
				...
				const conditions = [eq(agentComplianceAuditLogs.agentId, agentId)];

				if (actionCategory) {
					conditions.push(
						eq(
							agentComplianceAuditLogs.actionCategory,
				...
				res
					.status(500)
					.json({ error: "Failed to fetch compliance audit logs" });
			}
		},
	);

	app.post(
		"/api/agent/advisory-sessions/:sessionId/suitability-check",
		requireAgentPortal,
		async (req: Request, res: Response) => {
			try {
				const agentId = (req.user as any).id;
				...
				const result = await ProposalOrchestrator.runSuitabilityCheck(
					sessionId,
					agentId,
				);
				...
				res
					.status(500)
					.json({ error: error.message || "Failed to run suitability check" });
			}
		},
	);

	app.post(
		"/api/agent/advisory-sessions/:sessionId/optimize",
		requireAgentPortal,
		async (req: Request, res: Response) => {
			try {
				const agentId = (req.user as any).id;
				...
				const result = await ProposalOrchestrator.runOptimization(
					sessionId,
					agentId,
				);
				...
				res
					.status(500)
					.json({ error: error.message || "Failed to run optimization" });
			}
		},
	);

	app.get(
		"/api/agent/advisory-sessions/:sessionId/status",
		requireAgentPortal,
		async (req: Request, res: Response) => {
			try {
				const { sessionId } = req.params;
