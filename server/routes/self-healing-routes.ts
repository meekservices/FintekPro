...
import {
	...
	routeFixStrategy,
	type GuardedModule,
} from "../services/guarded-execution";
import { db } from "../db";
...
router.get("/status", requireAdmin, async (req: Request, res: Response) => {
	try {
		...
		res
			.status(500)
			.json({ error: "Failed to retrieve system health snapshot" });
	}
});
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (service) {
				...
				console.log(
					`[SelfHealing] Circuit breaker reset for service "${service}" by admin ${adminId}`,
				);
				res.json({
					success: true,
				...
				console.log(
					`[SelfHealing] All circuit breakers reset by admin ${adminId}: ${reset.join(", ")}`,
				);
				res.json({
					success: true,
					reset,
					message: `${reset.length} circuit breaker(s) reset`,
				});
			}
		} catch (err: any) {
...
router.get("/feedback", requireAdmin, async (req: Request, res: Response) => {
	try {
		const hours = Number.parseInt((req.query.hours as string) || "24", 10);
...
router.get("/actions", requireAdmin, (_req: Request, res: Response) => {
	const descriptions: Record<RecoveryAction, string> = {
		flush_lru_cache:
			"Flush the in-memory LRU / distributed cache (reduces memory pressure)",
		clear_sandbox_token:
			"Clear the cached Sandbox.co.in auth token — next request re-authenticates",
		reset_python_circuit:
			"Reset the Python sidecar circuit breaker so the next call probes the service",
		reset_all_circuits: "Reset ALL open circuit breakers at once",
		flush_ai_response_cache: "Clear the AI (Gemini) response cache",
		flush_distributed_cache:
			"Purge expired rows from the distributed DB cache table",
		gc_collect:
			"Force a Node.js garbage collection cycle (requires --expose-gc flag)",
	};
