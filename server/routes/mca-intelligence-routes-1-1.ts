...
const ingestFinancialsSchema = z.object({
	...
	financialYear: z
		.string()
		.regex(/^\d{4}-\d{2}$/, "Format: YYYY-YY (e.g., 2023-24)"),
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			const companyExists = await mcaIntelligenceService.companyExists(
				data.cin,
			);
			if (!companyExists) {
				return res.status(400).json({
		...
		try {
			...
			for (const fin of financials) {
				try {
					...
					const companyExists =
						ingestedCompanyCins.has(fin.cin) ||
						(await mcaIntelligenceService.companyExists(fin.cin));

					if (!companyExists) {
						results.financialsSkipped++;
						results.warnings.push(
							`Financials ${fin.cin}/${fin.financialYear}: Skipped - company not found. Ingest company first.`,
						);
						continue;
					}
					...
					results.errors.push(
						`Financials ${fin.cin}/${fin.financialYear}: ${err.message}`,
					);
				}
			}
			...
			await mcaIntelligenceService.logQuery({
				...
				success:
					results.companiesFailed === 0 && results.financialsFailed === 0,
			});

			// Determine overall success and confidence level
			const allSucceeded =
				results.companiesFailed === 0 &&
				results.financialsFailed === 0 &&
				results.financialsSkipped === 0;
			const partialSuccess =
				results.companiesIngested > 0 || results.financialsIngested > 0;
...
const profitableSchema = z.object({
	...
	sort_by: z
		.enum(["pat", "revenue", "patMargin", "roe"])
		.optional()
		.default("pat"),
});
