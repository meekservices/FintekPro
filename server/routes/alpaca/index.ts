...
import {
	...
	BarTimeframe,
} from "../../services/alpaca-market-data-service";
...
router.post("/trade/notional", async (req, res) => {
	...
	try {
		const order = await orderManager.placeOrder(userId, {
			symbol,
			notional: Number.parseFloat(notional),
			side: side || "buy",
			type: "market",
			time_in_force: "day",
		});
...
router.get("/market/best-buys", async (req, res) => {
	...
	if (!validProfiles.includes(riskProfile)) {
		return res
			...
			.json({
				success: false,
				message: `riskProfile must be one of: ${validProfiles.join(", ")}`,
			});
	}
	try {
		...
		res.json({
			...
			meta: {
				...
				version: "1.0",
				engine_version: result.modelVersion,
			},
		});
	} catch (error: any) {
...
router.get("/market/instruments", async (req, res) => {
	...
	try {
		const [stocks, etfs, fxRate] = await Promise.all([
			type !== "etfs"
				? alpacaMarketDataService.getPopularStocks()
				: Promise.resolve([]),
			type !== "stocks"
				? alpacaMarketDataService.getPopularETFs()
				: Promise.resolve([]),
			alpacaMarketDataService.getUsdInrRate(),
		]);
		const marketStatus = alpacaMarketDataService.getMarketStatus();
		const enrichWithInr = (items: any[]) =>
			items.map((i) => ({
				...i,
				priceInr: i.price
					? Number.parseFloat((i.price * fxRate).toFixed(2))
					: null,
			}));
		res.json({
			success: true,
			data: {
				stocks: enrichWithInr(stocks),
				etfs: enrichWithInr(etfs),
				fxRate,
				marketStatus,
				disclaimer:
					"Prices delayed up to 15 minutes. For informational purposes only.",
			},
...
router.post("/funding/bank/link", async (req, res) => {
	...
	try {
		...
		// 2. Link with Alpaca
		const relationship = await alpacaFundingService.linkBankWithPlaid(
			userId,
			processorToken,
			bankName,
		);
...
router.post("/funding/deposit", async (req, res) => {
	...
	try {
		const transfer = await alpacaFundingService.depositFunds(
			userId,
...
router.get("/social/profile/:code", async (req, res) => {
	try {
		const profile = await referralService.getProfileByReferralCode(
			req.params.code,
		);
