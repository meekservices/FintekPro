	...
	async (req: Request, res: Response) => {
		try {
			...
			return apiResponse.serverError(
				res,
				`Failed to preview MoneyControl data: ${error.message}`,
			);
		}
	},
);
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			return apiResponse.serverError(
				res,
				`Failed to import from MoneyControl: ${error.message}`,
			);
		}
	},
);
		...
		try {
			...
			if (existingByIsin) {
				return apiResponse.badRequest(
					res,
					`Company with ISIN ${isin} already exists: ${existingByIsin.name}`,
				);
			}
			...
			try {
				const credhiveResults =
					await credhiveService.searchCompanyByNameOrCIN(name);

				if (credhiveResults && credhiveResults.length > 0) {
					// Find best match - exact name match or first result
					const exactMatch = credhiveResults.find(
						(r) => (r as any).name.toLowerCase() === name.toLowerCase(),
					);
					...
					credhiveDetails =
						await credhiveService.getCompanyDetails(credhiveCompanyId);
					result.credhiveFound = true;
				}
			} catch (error: any) {
				console.warn(
					"CredHive search failed, creating company with basic data:",
					error.message,
				);
			}
			...
			if (credhiveDetails) {
				...
				if (credhiveDetails.total_shares) {
					companyData.totalShares =
						typeof credhiveDetails.total_shares === "number"
							? credhiveDetails.total_shares
							: Number.parseInt(credhiveDetails.total_shares, 10);
				}
			...
			if (credhiveCompanyId && result.credhiveData) {
				try {
					const financials = await credhiveService.getCompanyFinancials(
						credhiveCompanyId,
						5,
					);
					for (const fin of financials) {
						const dbFormat = (
							credhiveService as any
						).convertFinancialsToDbFormat(company.id, fin);
						await storage.createCompanyFinancials(dbFormat);
						result.credhiveData.financialsSynced++;
					}

					const ratios = await (credhiveService as any).getCompanyRatios(
						credhiveCompanyId,
						5,
					);
					for (const ratio of ratios) {
						const dbFormat = (credhiveService as any).convertRatiosToDbFormat(
							company.id,
							ratio,
						);
					...
					console.warn(
						"Failed to sync financials/ratios from CredHive:",
						error.message,
					);
				}
			}
			...
			if (price && price > 0) {
				try {
					...
					console.warn(
						"Failed to import price from MoneyControl:",
						error.message,
					);
				}
			}
			...
			return apiResponse.created(
				...
				`Company "${name}" created successfully` +
					...
					(result.priceImported
						? ` and price ₹${price?.toLocaleString("en-IN")}`
						: ""),
			);
		} catch (error: any) {
			...
			return apiResponse.serverError(
				res,
				`Failed to add company: ${error.message}`,
			);
		}
	},
);
...
router.get(
	...
	async (req: Request, res: Response) => {
		try {
			...
			return apiResponse.serverError(
				res,
				`Failed to search ISIN: ${error.message}`,
			);
		}
	},
);
	...
	async (req: Request, res: Response) => {
		try {
			...
			return apiResponse.success(
				res,
				result,
				`ISIN cache refreshed with ${result.recordCount} records`,
			);
		} catch (error: any) {
...
router.get("/listings/published", async (req: Request, res: Response) => {
	try {
		...
		const limitNum = Math.min(
			50,
			Math.max(1, Number.parseInt(limit as string) || 20),
		);
		...
		if (sector && typeof sector === "string") {
			filteredListings = filteredListings.filter(
				(l) => l.company?.sector?.toLowerCase() === sector.toLowerCase(),
			);
		}
...
router.post("/listings", requireLevel2, async (req: Request, res: Response) => {
	try {
		...
		if (
			...
			!Array.isArray(acknowledgedDisclosureIds)
		) {
			return apiResponse.badRequest(
				res,
				"Risk disclosure acknowledgment is required. Please read and accept all mandatory disclosures before creating a listing.",
			);
		}

		const disclosureValidation =
			unlistedRiskDisclosureService.validateAcknowledgment({
				acknowledgedDisclosureIds,
				userId: req.user!.id,
		...
		if (!disclosureValidation.valid) {
			console.log(
				`[COMPLIANCE] sell_listing_blocked: Missing risk disclosures | userId: ${req.user!.id} | missing: ${disclosureValidation.missingDisclosures.join(", ")}`,
			);
			return apiResponse.badRequest(
				res,
				"All mandatory risk disclosures must be acknowledged before creating a listing.",
				{
		...
		const company = await storage.getUnlistedCompanyById(
		...
		if (
			(company as any).listingStage === "listed" ||
		...
		const askPriceNum = Number.parseFloat(validatedData.askPrice) || 0;
		...
		if (transactionValue >= ACCREDITED_INVESTOR_THRESHOLD) {
			...
			if (!profile?.accreditedInvestorType) {
				console.log(
					`[COMPLIANCE] unlisted_sell_blocked: High-value transaction (₹${transactionValue}) requires accredited investor status | userId: ${req.user!.id}`,
				);
				return apiResponse.forbidden(
					res,
		...
		console.log(
			`[COMPLIANCE] unlisted_sell_listing: { userId: '${req.user!.id}', companyId: '${validatedData.companyId}', quantity: ${validatedData.quantity}, value: ${transactionValue}, disclosureVersion: '${unlistedRiskDisclosureService.getDisclosureVersion()}', outcome: 'success' }`,
		);
		...
		return apiResponse.created(
			res,
			listing,
			"Sell listing created successfully",
		);
	} catch (error: any) {
