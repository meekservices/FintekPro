...
function err(message: string, code = 'RECON_ERROR') {
  return { success: false, error: { error_code: code, message, retryable: false }, meta: { timestamp: new Date().toISOString(), version: API_VERSION } };
}
 ...
 */
router.get('/discrepancies', requireAdmin, async (req: Request, res: Response) => {
  try {
 ...
 */
router.get('/client/:clientId', requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    const discrepancies = (discrepancyResult as any).rows ?? discrepancyResult;
...
router.post('/run', requireAdmin, async (req: Request, res: Response) => {
  ...
  // Fire in background
  portfolioReconciliationEngine.reconcileAllClients().then(stats => {
    ...
    logger.error('[ReconAPI] On-demand reconciliation failed', { runId, message: e?.message });
  });

  return res.json(ok({
    runId,
    message: 'Portfolio reconciliation started in background. Check /summary in ~2 minutes.',
 ...
 */
router.post('/run/:clientId', requireAdmin, async (req: Request, res: Response) => {
  const { clientId } = req.params;
  const start = Date.now();

  try {
    logger.info('[ReconAPI] Single-client on-demand reconciliation triggered', {
      event: 'RECON_ADMIN_SINGLE_TRIGGER',
      user_id: (req as any).user?.id,
      target_client: clientId,
    });

    const result = await portfolioReconciliationEngine.reconcileClient(clientId);
    ...
    logger.error('[ReconAPI] Single-client reconciliation failed', { message: e.message });
    return res.status(500).json(err(e.message));
  }
});
 ...
 */
router.patch('/resolve/:id', requireAdmin, async (req: Request, res: Response) => {
  const { id } = req.params;
  const { resolution_note } = req.body;
  const adminId = (req as any).user?.id;

  if (!resolution_note) {
    return res.status(400).json(err('resolution_note is required', 'VALIDATION_ERROR'));
  }
  ...
  try {
    ...
    return res.json(ok({ id, resolved: true, resolvedAt: new Date().toISOString() }));
  } catch (e: any) {
