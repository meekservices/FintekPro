...
import {
	kfsGeneratorService,
	KFSLoanDetails,
} from "../services/kfs-generator-service";
import { z } from "zod";
...
router.get(
	...
	async (req: Request, res: Response) => {
		try {
			const application = await dsaLoanService.getApplicationByNumber(
...
router.post("/applications/:id/submit", async (req: Request, res: Response) => {
	try {
		const agentId = (req as any).user?.id;
		const application = await dsaLoanService.submitApplication(
			req.params.id,
			agentId,
		);
...
router.post(
	"/applications/:id/check-eligibility",
	async (req: Request, res: Response) => {
		try {
...
router.get(
	"/applications/:id/routing-history",
	async (req: Request, res: Response) => {
		try {
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			await dsaLoanService.updateBankResponse(
				req.params.routingId,
				parsed,
				actorId,
			);
...
router.get("/my-applications", async (req: Request, res: Response) => {
	try {
		...
		const applications = await dsaLoanService.getApplicationsByUser(
			userId,
			userPhone,
			userEmail,
		);
...
router.get("/applications/:id/kfs", async (req: Request, res: Response) => {
	try {
		...
		const approvedOffer = bankOffers.find(
			(o: any) => o.status === "approved" || o.status === "accepted",
		);
		...
		if (!approvedOffer) {
			return res.status(400).json({
				success: false,
				error:
					"No approved loan offer found for this application. KFS can only be generated for approved loans.",
			});
		}

		const loanDetails: KFSLoanDetails = {
			loanAmount: Number.parseFloat(
				approvedOffer.sanctionedAmount || application.requestedAmount,
			),
			interestRatePerAnnum: Number.parseFloat(
				approvedOffer.offeredInterestRate || "12",
			),
			tenureMonths: approvedOffer.offeredTenure || application.requestedTenure,
			processingFeePercent: Number.parseFloat(
				approvedOffer.processingFeePercent || "1",
			),
