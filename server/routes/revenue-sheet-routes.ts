	...
	if (!req.user) {
		...
		if (isDev) {
			req.user = {
				...
				roles: ["superadmin", "admin", "partner", "agent", "client"],
			};
		} else {
			return res
				.status(401)
...
router.get(
	...
	async (req: any, res: Response) => {
		try {
			...
			const month =
				Number.parseInt(req.query.month as string) || new Date().getMonth() + 1;
			const year =
				Number.parseInt(req.query.year as string) || new Date().getFullYear();
    ...
    `);
			...
			const claimCases = ((claimsRows as any).rows || claimsRows || []).map(
				(c: any) => ({
					id: c.id,
					date: c.created_at,
			...
			const ledgerCases = ((ledgerRows as any).rows || ledgerRows || []).map(
				(e: any) => ({
					id: e.ledger_id,
					date: e.created_at,
					clientName: "Investment Portfolio",
					productType:
						e.role === "TRAIL" ? "MF Trail" : "Investment Commission",
			...
			const summary = {
				...
				directCommission: allCases
					.filter(
						(c) =>
							c.productCategory !== "investments" ||
							c.productType !== "MF Trail",
					)
			...
			res.json({
				success: true,
				data: { cases: allCases, summary, month, year },
			});
		} catch (err: any) {
...
router.get(
    ...
    `);
			...
			const agentNetwork = ((agentRows as any).rows || agentRows || []).map(
				(a: any) => ({
					agentId: a.agent_id,
			...
			const uplineIncome = uplineRows.reduce(
				(s: number, e: any) => s + Number(e.amount),
				0,
			);
			const networkCommission = agentNetwork.reduce(
				(s: number, a: any) => s + a.agentCommission,
				0,
			);

			const summary = {
				totalAgents: agentNetwork.length,
				activeCases: agentNetwork.reduce(
					(s: number, a: any) => s + a.casesCount,
					0,
				),
				networkCommission,
				uplineIncome,
