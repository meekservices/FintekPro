...
router.post('/api/kyc/eaadhaar-digilocker/initiate', async (req, res) => {
  try {
    ...
    const result = await truthScreenEAadhaarDGLService.generateDigiLockerLink(transId);
    ...
    res.status(500).json({ success: false, message: 'Failed to initiate E-Aadhaar DigiLocker flow' });
  }
});

router.get('/api/kyc/eaadhaar-digilocker/status/:tsTransId', async (req, res) => {
  try {
    if (!req.user?.id) {
      return res.status(401).json({ success: false, message: 'Authentication required' });
    }
    ...
    res.status(500).json({ success: false, message: 'Failed to check E-Aadhaar DigiLocker status' });
  }
});
