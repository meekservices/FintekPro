...
router.get('/reits', async (req: Request, res: Response) => {
  try {
    const { sector, riskLevel, minYield, maxYield, aiSignal, sortBy, sortOrder } = req.query;
    ...
    if (minYield) {
      filteredReits = filteredReits.filter(r => parseFloat(r.distributionYield) >= parseFloat(minYield as string));
    }
    if (maxYield) {
      filteredReits = filteredReits.filter(r => parseFloat(r.distributionYield) <= parseFloat(maxYield as string));
    }
    
    if (sortBy) {
      const order = sortOrder === 'asc' ? 1 : -1;
      filteredReits.sort((a: any, b: any) => {
        const aVal = parseFloat(a[sortBy as string]) || 0;
        const bVal = parseFloat(b[sortBy as string]) || 0;
...
router.get('/reits/:symbol', async (req: Request, res: Response) => {
  try {
    ...
    const reit = SAMPLE_REITS.find(r => r.symbol.toLowerCase() === symbol.toLowerCase());
...
router.get('/invits', async (req: Request, res: Response) => {
  try {
    ...
    if (minYield) {
      filteredInvits = filteredInvits.filter(i => parseFloat(i.distributionYield) >= parseFloat(minYield as string));
    }
    if (maxYield) {
      filteredInvits = filteredInvits.filter(i => parseFloat(i.distributionYield) <= parseFloat(maxYield as string));
    }
...
router.get('/invits/:symbol', async (req: Request, res: Response) => {
  try {
    const { symbol } = req.params;
    const invit = SAMPLE_INVITS.find(i => i.symbol.toLowerCase() === symbol.toLowerCase());
...
router.get('/market-overview', async (req: Request, res: Response) => {
  try {
    const reitStats = {
      totalMarketCap: SAMPLE_REITS.reduce((sum, r) => sum + parseFloat(r.marketCap), 0),
      avgYield: (SAMPLE_REITS.reduce((sum, r) => sum + parseFloat(r.distributionYield), 0) / SAMPLE_REITS.length).toFixed(2),
      avgOccupancy: (SAMPLE_REITS.reduce((sum, r) => sum + parseFloat(r.occupancyRate || '0'), 0) / SAMPLE_REITS.length).toFixed(1),
      topPerformer: SAMPLE_REITS.reduce((max, r) => parseFloat(r.returns1Y || '0') > parseFloat(max.returns1Y || '0') ? r : max),
    };
    
    const invitStats = {
      totalMarketCap: SAMPLE_INVITS.reduce((sum, i) => sum + parseFloat(i.marketCap), 0),
      avgYield: (SAMPLE_INVITS.reduce((sum, i) => sum + parseFloat(i.distributionYield), 0) / SAMPLE_INVITS.length).toFixed(2),
      avgConcessionLife: (SAMPLE_INVITS.reduce((sum, i) => sum + parseFloat(i.concessionLife || '0'), 0) / SAMPLE_INVITS.length).toFixed(1),
      topPerformer: SAMPLE_INVITS.reduce((max, i) => parseFloat(i.returns1Y || '0') > parseFloat(max.returns1Y || '0') ? i : max),
    };
