 ...
 */
router.post(
	"/companies/:companyId/publish-to-store-with-prices",
	requireAdmin,
		...
		try {
			...
			if (!buyPrice || !sellPrice) {
				return apiResponse.badRequest(
					res,
					"Both buyPrice and sellPrice are required",
				);
			}
			...
			if (Number.isNaN(parsedBuyPrice) || Number.isNaN(parsedSellPrice)) {
			...
			if (existingProduct) {
				...
				const updatedProduct = await storage.updateStoreProduct(
					existingProduct.id,
					{
			...
			if (!unlistedCategory) {
				unlistedCategory = await storage.createStoreCategory({
					...
					description:
						"Pre-IPO and unlisted company shares for sophisticated investors",
					slug: "unlisted",
					icon: "TrendingUp",
					displayOrder: 10,
					isActive: true,
			...
			if (company.sector) {
				const sectorSlug = company.sector
					.toLowerCase()
					.replace(/[^a-z0-9]+/g, "-");
			...
			const productData = {
				...
				fullDescription:
					company.description ||
					`Invest in ${company.name}, an unlisted company in the ${company.sector || "Technology"} sector.`,
				categoryId: unlistedCategory.id,
				...
				minimumInvestment: company.minLotSize
					? String(Number(company.minLotSize) * parsedSellPrice)
					: "10000",
				riskLevel: "high",
 ...
 */
router.patch(
	"/admin/update-store-prices/:productId",
	requireAdmin,
	async (req: Request, res: Response) => {
		try {
