...
import {
	...
	mfReconciliationEntries,
	mfContractNotes,
	insertMfOrderSchema,
	insertMfFolioSchema,
	insertMfHoldingSchema,
	insertBankMandateSchema,
	insertMfOrderAuditLogSchema,
	insertSuitabilityAcknowledgementSchema,
	users,
	userProfiles,
	riskProfiles,
} from "@shared/schema";
...
router.post(
	"/api/mf-orders/:id/suitability-ack",
	isAuthenticated,
	async (req: Request, res: Response) => {
		try {
			...
			const [ack] = await db
				.insert(suitabilityAcknowledgements)
				.values({
					orderId: id,
					userId,
					clientRiskProfile,
					...
					acknowledgementText:
						acknowledgementText ||
						"I understand that this investment may not be suitable for my risk profile and I wish to proceed.",
			...
			console.error(
				"[MF Orders] Error creating suitability acknowledgement:",
				error,
			);
			res
				.status(500)
				.json({ error: "Failed to create suitability acknowledgement" });
		}
	},
);

router.get(
	"/api/mf-orders/:id/audit-log",
	isAuthenticated,
	async (req: Request, res: Response) => {
		try {
			const userId = (req.user as any)?.id;
...
router.get(
	"/api/mf-orders/:id/contract-note",
	isAuthenticated,
	async (req: Request, res: Response) => {
		try {
			const userId = (req.user as any)?.id;
			...
			if (
				!["executed", "partially_executed", "settled"].includes(
					order.order.status || "",
				)
			) {
				return res
					.status(400)
					.json({
						error: "Contract note available only for executed/settled orders",
					});
			}
			...
			const contractNote = {
				contractNoteNumber,
				tradeDate:
					order.order.navApplied ||
					(order.order as any).executedAt ||
				...
				disclaimer:
					"This contract note is electronically generated and is valid without signature. Please verify all details and report discrepancies within 24 hours.",
...
router.get(
	"/api/admin/mf-orders/reconciliation",
	isAuthenticated,
	async (req: Request, res: Response) => {
		try {
			const userRole = (req.user as any)?.role;
			...
			for (const order of orders) {
				summary.byStatus[order.status || "unknown"] =
					(summary.byStatus[order.status || "unknown"] || 0) + 1;
				summary.byOrderType[order.orderType || "unknown"] =
					(summary.byOrderType[order.orderType || "unknown"] || 0) + 1;
				...
				if (
					["pending_payment", "processing", "placed"].includes(
						order.status || "",
					)
				) {
			...
			console.error(
				"[MF Orders] Error generating reconciliation report:",
				error,
			);
			res
				.status(500)
				.json({ error: "Failed to generate reconciliation report" });
		}
	},
);

router.post(
	"/api/admin/mf-orders/:id/mark-settled",
	isAuthenticated,
	async (req: Request, res: Response) => {
		try {
			const userId = (req.user as any)?.id;
			const userRole = (req.user as any)?.role;
			const { id } = req.params;
			const { settlementReference, unitsAllotted, navApplied, remarks } =
				req.body;
			...
			if (
				!["executed", "partially_executed"].includes(existingOrder.status || "")
			) {
				return res
					.status(400)
					.json({ error: "Only executed orders can be marked as settled" });
			}
			...
			if (["buy", "lumpsum", "sip"].includes(existingOrder.orderType || "")) {
				const finalUnits = Number.parseFloat(
					unitsAllotted || existingOrder.units || "0",
				);
				const finalNav = Number.parseFloat(
					navApplied || existingOrder.navApplied || "0",
				);
				const investedValue = Number.parseFloat(existingOrder.amount || "0");
				...
				if (existingHolding) {
					...
					const newUnits =
						Number.parseFloat(existingHolding.units || "0") + finalUnits;
					const newInvested =
						Number.parseFloat(existingHolding.investedValue || "0") +
						investedValue;
				...
				["sell", "redemption"].includes(existingOrder.orderType || "")
			) {
				// Reduce units for sell orders
				const unitsToReduce = Number.parseFloat(
					unitsAllotted || existingOrder.units || "0",
				);
				...
				if (existingHolding) {
					const newUnits = Math.max(
						0,
						Number.parseFloat(existingHolding.units || "0") - unitsToReduce,
					);
					const finalNav = Number.parseFloat(
						navApplied ||
							existingOrder.navApplied ||
				...
				try {
					...
					await realizedGainsAggregationService.recalculateUserReminders(
						existingOrder.userId,
					);
					console.log(
						"[MF Orders] Capital gains calculated for sell order",
						id,
					);
				} catch (gainError) {
					console.error(
						"[MF Orders] Failed to calculate capital gains:",
						gainError,
					);
			...
			res.json({
				order: updatedOrder,
				message: "Order marked as settled successfully",
			});
		} catch (error) {
...
router.get(
	"/api/mf-orders/:id/settlement-status",
	isAuthenticated,
	async (req: Request, res: Response) => {
		try {
			const userId = (req.user as any)?.id;
			...
			if (order.navApplied) {
				...
				if (["buy", "lumpsum", "sip"].includes(order.orderType || "")) {
					...
					["sell", "redemption", "swp", "switch"].includes(
						order.orderType || "",
					)
				) {
					if (
						order.schemeName?.toLowerCase().includes("liquid") ||
						order.schemeName?.toLowerCase().includes("overnight")
					) {
						expectedSettlementDate.setDate(
							expectedSettlementDate.getDate() + 1,
						);
					} else {
						expectedSettlementDate.setDate(
							expectedSettlementDate.getDate() + 3,
						);
					}
				}
			}
			...
			const settlementTimeline = [
				...
				},
				{
					...
					status: (order as any).executedAt
						? "completed"
						: ["processing", "placed"].includes(order.status || "")
							? "in_progress"
							: "pending",
...
router.post(
	"/api/mf/batch/validate-credentials",
	requireRole(["agent", "admin", "compliance_officer"]),
	async (req: Request, res: Response) => {
		try {
			...
			if (error.name === "ZodError") {
				return res
					.status(400)
					.json({ error: "Invalid batch data", details: error.issues });
			}
...
router.get(
	"/api/mf/batch/preflight/:agentId",
	requireRole(["agent", "admin", "compliance_officer"]),
	async (req: Request, res: Response) => {
		try {
			...
			if (
				...
				return res
					.status(403)
					.json({ error: "Cannot check credentials for other agents" });
			}

			const preflightResult =
				await mfBatchCredentialValidator.preflightCheck(agentId);
...
router.get(
	"/api/admin/mf/batch/validation-status",
...
router.post(
	"/api/admin/mf/batch/clear-cache",
	requireRole(["admin"]),
	async (req: Request, res: Response) => {
		try {
