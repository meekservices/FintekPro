...
export function buildStockRationale(recType: string, m: any): string {
	...
	if (recType === "BUY") {
		...
		if (m.dividendYield > 1.5)
			parts.push(`Dividend yield: ${m.dividendYield}%`);
		if (m.sectorPerf > 0)
			parts.push(`Sector outperforming Nifty by ${m.sectorPerf}%`);
		if (m.priceVs52WH < -15)
			parts.push(
				`${Math.abs(m.priceVs52WH)}% below 52-week high - buying opportunity`,
			);
	...
	if (recType === "SELL") {
		...
		return parts.length > 0
			? parts.join(". ") + ". Book profits."
			: "Valuation stretched. Consider profit booking.";
	}
	if (recType === "SWITCH") {
...
export function buildPMSRationale(
	recType: string,
	m: any,
	exitLoad: any,
): string {
	if (recType === "BUY") {
		return (
			`Portfolio P/E: ${m.pe}x (${m.peVsCat > 0 ? "+" : ""}${m.peVsCat}% vs PMS avg). ` +
	...
	if (recType === "SELL") {
		...
		if (m.concentration > 70)
			r += `High concentration risk (${m.concentration}%). `;
		if (exitLoad.exitLoadApplicable)
			r += `Exit load: ${exitLoad.exitLoadPercent}%. `;
...
export function buildAIFRationale(
	recType: string,
	m: any,
	exitLoad: any,
): string {
	if (recType === "BUY") {
		return (
			`Targeted IRR: ${m.irr?.toFixed(1) || m.returns3Y}%. ` +
...
export function buildDefaultRationale(
	...
	if (recType === "BUY") {
		...
		if (m.expenseRatio < 0.5)
			parts.push(`Low cost: ${m.expenseRatio.toFixed(2)}%`);
		return (
			parts.join(". ") +
			`. ${m.categoryRank} in ${m.category}. Recommended for stable income.`
		);
	}
	if (recType === "SELL") {
...
export async function buildDynamicRecommendations(options: {
	...
	const [
		...
		includeStocks
			? getStoreEligibleStocks({ limit: 10, marketCap: "Large" })
			: Promise.resolve([]),
	...
	if (filteredAllocations["Large Cap"] && largeCaps.length > 0) {
		...
		recommendations.push({
			...
			recommendedAmount: Math.round(
				(totalAmount * filteredAllocations["Large Cap"]) / 100,
			),
			allocationPercentage: filteredAllocations["Large Cap"],
			investmentType: "lumpsum",
			returns1Y: Number.parseFloat(fund.returns1y || "0"),
			returns3Y: Number.parseFloat(fund.returns3y || "0"),
			returns5Y: Number.parseFloat(fund.returns5y || "0"),
	...
	if (filteredAllocations["Mid Cap"] && midCaps.length > 0) {
		...
		recommendations.push({
			...
			category: fund.category,
			recommendedAmount: Math.round(
				(totalAmount * filteredAllocations["Mid Cap"]) / 100,
			),
			allocationPercentage: filteredAllocations["Mid Cap"],
			investmentType: hasSIP ? "sip" : "lumpsum",
			...(hasSIP
				? {
						sipAmount: Math.round(
							(monthlyInvestment! * filteredAllocations["Mid Cap"]) / 100,
						),
					}
				: {}),
			returns1Y: Number.parseFloat(fund.returns1y || "0"),
	...
	if (filteredAllocations["Flexi Cap"] && flexiCaps.length > 0) {
		...
		recommendations.push({
			...
			category: fund.category,
			recommendedAmount: Math.round(
				(totalAmount * filteredAllocations["Flexi Cap"]) / 100,
			),
			...
			...(hasSIP
				? {
						sipAmount: Math.round(
							(monthlyInvestment! * filteredAllocations["Flexi Cap"]) / 100,
						),
					}
				: {}),
			returns1Y: Number.parseFloat(fund.returns1y || "0"),
	...
	if (
		(filteredAllocations.Debt || filteredAllocations["Corporate Bond"]) &&
		debtFunds.length > 0
	) {
		const allocation =
			filteredAllocations.Debt || filteredAllocations["Corporate Bond"];
	...
	if (filteredAllocations.Liquid && liquidFunds.length > 0) {
		...
		recommendations.push({
			...
			category: fund.category,
			recommendedAmount: Math.round(
				(totalAmount * filteredAllocations.Liquid) / 100,
			),
	...
	if (filteredAllocations.Bonds && bonds.length > 0) {
		...
		recommendations.push({
			...
			recommendedAmount: Math.round(
				(totalAmount * filteredAllocations.Bonds) / 100,
			),
			allocationPercentage: filteredAllocations.Bonds,
			investmentType: "lumpsum",
			returns1Y: Number.parseFloat(bond.couponRate || "0"),
	...
	if (filteredAllocations.Stocks && stocks.length > 0) {
		...
		const perStockAllocation =
			filteredAllocations.Stocks / stocksToInclude.length;
		...
		for (const stock of stocksToInclude) {
			...
			recommendations.push({
				...
				investmentType: "lumpsum",
				returns1Y: Number.parseFloat(stock.returns1Y || "0"),
				returns3Y: Number.parseFloat(stock.returns3Y || "0"),
				returns5Y: Number.parseFloat(stock.returns5Y || "0"),
				...
				selectionReason:
					stock.selectionNotes ||
					`Quality ${stock.marketCap || "large cap"} stock in ${stock.sector || "diversified"} sector for long-term wealth creation.`,
				recommendationType: "BUY",
				...analyticalData,
			});
		}
	...
	if (includePremium) {
		...
		if (filteredAllocations.PMS && pmsProducts.length > 0) {
			...
			recommendations.push({
				...
				recommendedAmount: Math.round(
					(totalAmount * filteredAllocations.PMS) / 100,
				),
				allocationPercentage: filteredAllocations.PMS,
				investmentType: "lumpsum",
				minInvestment: Number.parseFloat(pms.minInvestment || "5000000"),
				returns1Y: Number.parseFloat(pms.return1Y || "0"),
				riskRating: pms.riskScore
					? `Risk Score: ${pms.riskScore}`
					: "Moderately High",
		...
		if (filteredAllocations.AIF && aiFs.length > 0) {
			...
			recommendations.push({
				...
				recommendedAmount: Math.round(
					(totalAmount * filteredAllocations.AIF) / 100,
				),
				allocationPercentage: filteredAllocations.AIF,
				investmentType: "lumpsum",
				minInvestment: Number.parseFloat(aif.minInvestment || "10000000"),
				returns1Y: Number.parseFloat(aif.return1Y || "0"),
		...
		if (filteredAllocations.Alternatives && mlds.length > 0) {
			...
			recommendations.push({
				...
				recommendedAmount: Math.round(
					(totalAmount * filteredAllocations.Alternatives) / 100,
				),
				allocationPercentage: filteredAllocations.Alternatives,
				investmentType: "lumpsum",
				minInvestment: Number.parseFloat(mld.minInvestment || "1000000"),
				returns1Y: Number.parseFloat(mld.ytm || "0"),
				riskRating: mld.riskScore
					? `Risk Score: ${mld.riskScore}`
					: "Moderately High",
	...
	if (recommendations.length === 0) {
		recommendations.push({
			...
			riskRating: "N/A",
			selectionReason:
				"Please configure store-eligible mutual funds (Regular plan) to generate personalized recommendations.",
		});
	}
...
export async function trackProposalRecommendations(
	...
	const trackableRecs = recommendations.filter(
		(rec) =>
			rec.action === "BUY" ||
	...
	for (const rec of trackableRecs) {
		try {
			...
			console.log(
				`[AI Tracking] Recorded recommendation: ${productName} for proposal ${proposalId}`,
			);
		} catch (error) {
