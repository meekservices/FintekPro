	...
	try {
		...
		if (!userId || !kycStep || !productType) {
			res
				...
				.json({
					success: false,
					error: "userId, kycStep, and productType are required",
				});
			return;
		}
		...
		if (result.success) {
			const stepToType: Record<
				string,
				"pan" | "aadhaar" | "ckyc" | "bank" | "address"
			> = {
			...
			if (verificationType) {
				try {
					...
					const updatedProfile =
						await identityTokenService.updateVerificationStatus(
							userId,
							verificationType,
							{
								verified: true,
					...
					console.error(
						"[KYC-ENGINE-ROUTES] Failed to update identity profile:",
				...
				if (bankPayload.accountNo && bankPayload.ifsc) {
					try {
						...
						const existingAccount = activeAccounts.find(
							(a) => a.accountNumber === bankPayload.accountNo,
						);
						if (existingAccount) {
						...
						console.error(
							"[KYC-ENGINE-ROUTES] Failed to sync bank account:",
							errorMessage(bankErr),
						);
					}
				}
			}
...
router.get(
	...
	async (req: Request, res: Response): Promise<void> => {
		try {
			...
			console.error(
				"[KYC-ENGINE-ROUTES] Error in getFlowForProduct:",
				errorMessage(error),
			);
...
router.get(
	...
	async (req: Request, res: Response): Promise<void> => {
		try {
			...
			const chain = await kycOrchestrationEngine.getProviderChainForStep(
				kycStep,
				productType,
			);
			...
			console.error(
				"[KYC-ENGINE-ROUTES] Error in getProviderChainForStep:",
				errorMessage(error),
			);
			res
				.status(500)
				.json({ success: false, error: "Failed to get provider chain" });
		}
	},
);
...
router.get(
	...
	async (req: Request, res: Response): Promise<void> => {
		try {
			...
			console.error(
				"[KYC-ENGINE-ROUTES] Error in getOrCreateProfile:",
				errorMessage(error),
			);
			res
				.status(500)
				.json({ success: false, error: "Failed to get identity profile" });
		}
	},
);

router.get(
	"/identity/:userId/eligibility/:productType",
	async (req: Request, res: Response): Promise<void> => {
		try {
			const { userId, productType } = req.params;
			const eligibility = await identityTokenService.checkKycEligibility(
				userId,
				productType,
			);
			...
			console.error(
				"[KYC-ENGINE-ROUTES] Error in checkKycEligibility:",
				errorMessage(error),
			);
			res
				.status(500)
...
router.post(
	...
	async (req: Request, res: Response): Promise<void> => {
		try {
			...
			console.error(
				"[KYC-ENGINE-ROUTES] Error in declareFatca:",
				errorMessage(error),
			);
			res
				.status(500)
				.json({ success: false, error: "Failed to declare FATCA" });
		}
	},
);

router.post(
	"/identity/:userId/risk",
	async (req: Request, res: Response): Promise<void> => {
		try {
			const { userId } = req.params;
			const { category, score } = req.body as {
				category?: string;
				score?: number;
			};
			if (!category || score === undefined) {
				res
					.status(400)
					.json({ success: false, error: "category and score are required" });
				return;
			}
			...
			console.error(
				"[KYC-ENGINE-ROUTES] Error in assessRisk:",
				errorMessage(error),
			);
...
router.get(
	...
	async (_req: Request, res: Response): Promise<void> => {
		try {
			...
			console.error(
				"[KYC-ENGINE-ROUTES] Error in getPurposes:",
				errorMessage(error),
			);
			res
				.status(500)
				.json({ success: false, error: "Failed to get consent purposes" });
		}
	},
);
...
router.post("/consent", async (req: Request, res: Response): Promise<void> => {
	try {
		const { userId, consentType, consentGiven, ipAddress, userAgent } =
			req.body as {
				userId?: string;
				consentType?: string;
		...
		if (!userId || !consentType || consentGiven === undefined) {
			res
				...
				.json({
					success: false,
					error: "userId, consentType, and consentGiven are required",
				});
			return;
		}
		...
		console.error(
			"[KYC-ENGINE-ROUTES] Error in captureConsent:",
			errorMessage(error),
		);
		res
			.status(500)
			.json({ success: false, error: "Failed to capture consent" });
	}
});
...
router.get(
	...
	async (req: Request, res: Response): Promise<void> => {
		try {
			...
			console.error(
				"[KYC-ENGINE-ROUTES] Error in getActiveConsents:",
				errorMessage(error),
			);
			res
				.status(500)
				.json({ success: false, error: "Failed to get active consents" });
		}
	},
);
...
router.get(
	...
	async (req: Request, res: Response): Promise<void> => {
		try {
			...
			console.error(
				"[KYC-ENGINE-ROUTES] Error in getConsentHistory:",
				errorMessage(error),
			);
			res
				.status(500)
				.json({ success: false, error: "Failed to get consent history" });
		}
	},
);

router.post(
	"/consent/:userId/withdraw",
	async (req: Request, res: Response): Promise<void> => {
		try {
			const { userId } = req.params;
			const { consentType, reason } = req.body as {
				consentType?: string;
				reason?: string;
			};
			if (!consentType || !reason) {
				res
					...
					.json({
						success: false,
						error: "consentType and reason are required",
					});
				return;
			}
			...
			console.error(
				"[KYC-ENGINE-ROUTES] Error in withdrawConsent:",
				errorMessage(error),
			);
			res
				.status(500)
				.json({ success: false, error: "Failed to withdraw consent" });
		}
	},
);
...
router.get(
	...
	async (_req: Request, res: Response): Promise<void> => {
		try {
			...
			console.error(
				"[KYC-ENGINE-ROUTES] Error listing providers:",
				errorMessage(error),
			);
			res
				.status(500)
				.json({ success: false, error: "Failed to list providers" });
		}
	},
);
	...
	async (req: Request, res: Response): Promise<void> => {
		try {
			...
			if (pricePerCall !== undefined)
				updates.pricePerCall = String(pricePerCall);
			...
			console.error(
				"[KYC-ENGINE-ROUTES] Error updating provider:",
				errorMessage(error),
			);
			res
				.status(500)
...
router.get(
	...
	async (req: Request, res: Response): Promise<void> => {
		try {
			...
			console.error(
				"[KYC-ENGINE-ROUTES] Error getting provider metrics:",
				errorMessage(error),
			);
			res
				.status(500)
				.json({ success: false, error: "Failed to get provider metrics" });
		}
	},
);
		...
		try {
			...
			if (!kycStep || !providerId || !newPriority) {
				res
					...
					.json({
						success: false,
						error: "kycStep, providerId, and newPriority are required",
					});
				return;
			}
			await kycOrchestrationEngine.updateProviderPriority(
				kycStep,
				providerId,
				newPriority,
				updatedBy,
			...
			console.error(
				"[KYC-ENGINE-ROUTES] Error updating priority:",
				errorMessage(error),
			);
			res
				.status(500)
				.json({ success: false, error: "Failed to update priority" });
		}
	},
);
...
router.get(
	...
	async (_req: Request, res: Response): Promise<void> => {
		try {
			...
			console.error(
				"[KYC-ENGINE-ROUTES] Error listing products:",
				errorMessage(error),
			);
			res
				.status(500)
				.json({ success: false, error: "Failed to list products" });
		}
	},
);
...
router.patch(
	...
	async (req: Request, res: Response): Promise<void> => {
		try {
			...
			const updates: Record<string, any> = {};
			const { isEnabled, requiredKycLevel, requiredKycSteps, configuration } =
				req.body;
			if (isEnabled !== undefined) updates.isEnabled = isEnabled;
			if (requiredKycLevel !== undefined)
				updates.requiredKycLevel = requiredKycLevel;
			if (requiredKycSteps !== undefined)
				updates.requiredKycSteps = requiredKycSteps;
			...
			if (!updated) {
				res
					.status(404)
					.json({ success: false, error: "Product configuration not found" });
				return;
			}
			res.json({ success: true, product: updated });
		} catch (error: unknown) {
			console.error(
				"[KYC-ENGINE-ROUTES] Error updating product:",
				errorMessage(error),
			);
			res
				.status(500)
...
router.get(
	...
	async (_req: Request, res: Response): Promise<void> => {
		try {
			...
			console.error(
				"[KYC-ENGINE-ROUTES] Error listing brokers:",
				errorMessage(error),
			);
		...
		try {
			...
			if (!brokerCode || !brokerName || !brokerType) {
				res
					...
					.json({
						success: false,
						error: "brokerCode, brokerName, and brokerType are required",
					});
				return;
			}
			...
			console.error(
				"[KYC-ENGINE-ROUTES] Error creating broker:",
				errorMessage(error),
			);
			res
				.status(500)
				.json({ success: false, error: "Failed to create broker" });
		}
	},
);
	...
	async (req: Request, res: Response): Promise<void> => {
		try {
			...
			if (requiredEnvVars !== undefined)
				updates.requiredEnvVars = requiredEnvVars;
			if (supportedProducts !== undefined)
				updates.supportedProducts = supportedProducts;
			if (features !== undefined) updates.features = features;
			if (configuration !== undefined) updates.configuration = configuration;
			...
			if (!updated) {
				res
					.status(404)
					.json({ success: false, error: "Broker configuration not found" });
				return;
			}
			res.json({ success: true, broker: updated });
		} catch (error: unknown) {
			console.error(
				"[KYC-ENGINE-ROUTES] Error updating broker:",
				errorMessage(error),
			);
			res
				.status(500)
				.json({ success: false, error: "Failed to update broker" });
		}
	},
);
...
router.get(
	...
	async (req: Request, res: Response): Promise<void> => {
		try {
			...
			const limit = Number.parseInt(limitStr as string, 10) || 50;
			const offset = Number.parseInt(offsetStr as string, 10) || 0;

			const conditions: any[] = [];
			if (entityType)
				conditions.push(eq(platformAuditLogs.entityType, entityType as string));
			if (eventType)
				conditions.push(eq(platformAuditLogs.eventType, eventType as string));
			if (startDate)
				conditions.push(
					gte(platformAuditLogs.createdAt, new Date(startDate as string)),
				);
			if (endDate)
				conditions.push(
					lte(platformAuditLogs.createdAt, new Date(endDate as string)),
				);
			...
			console.error(
				"[KYC-ENGINE-ROUTES] Error getting audit logs:",
				errorMessage(error),
			);
			res
				.status(500)
				.json({ success: false, error: "Failed to get audit logs" });
		}
	},
);
...
router.get(
	...
	async (req: Request, res: Response): Promise<void> => {
		try {
			...
			const conditions: any[] = [];
			if (funnelType)
				conditions.push(eq(conversionFunnels.funnelType, funnelType as string));
			if (productType)
				conditions.push(
					eq(conversionFunnels.productType, productType as string),
				);
			if (startDate)
				conditions.push(
					gte(conversionFunnels.createdAt, new Date(startDate as string)),
				);
			if (endDate)
				conditions.push(
					lte(conversionFunnels.createdAt, new Date(endDate as string)),
				);
			...
			console.error(
				"[KYC-ENGINE-ROUTES] Error getting funnels:",
				errorMessage(error),
			);
			res
				.status(500)
				.json({ success: false, error: "Failed to get funnel data" });
		}
	},
);
	...
	async (_req: Request, res: Response): Promise<void> => {
		try {
			...
			const dashboard = providers.map((provider) => {
				const providerMetricsList = metricsResult.filter(
					(m) => m.providerId === provider.id,
				);
				const totalCalls = providerMetricsList.reduce(
					(sum, m) => sum + (m.totalCalls || 0),
					0,
				);
				const successCalls = providerMetricsList.reduce(
					(sum, m) => sum + (m.successfulCalls || 0),
					0,
				);
				const failedCalls = providerMetricsList.reduce(
					(sum, m) => sum + (m.failedCalls || 0),
					0,
				);
				const avgLatency =
					providerMetricsList.length > 0
						? providerMetricsList.reduce(
								(sum, m) => sum + (m.avgLatencyMs || 0),
								0,
							) / providerMetricsList.length
						: 0;
			...
			console.error(
				"[KYC-ENGINE-ROUTES] Error getting provider dashboard:",
				errorMessage(error),
			);
			res
				.status(500)
				.json({ success: false, error: "Failed to get provider dashboard" });
		}
	},
);
