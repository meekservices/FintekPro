	...
	async (req: Request, res: Response) => {
		try {
			...
			return apiResponse.serverError(
				res,
				"Failed to update buy request status",
			);
		}
	},
);
	...
	async (req: Request, res: Response) => {
		try {
			...
			await storage.updateUser(req.user.id, {
				unlistedWatchlist: updatedWatchlist,
			} as any);
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			console.log(
				`Express interest: User ${req.user.id} interested in ${company.name}`,
				{
					interestedQuantity,
					maxBudget,
					notes,
...
import {
	listingTransitionService,
	type ListingTransitionRequest,
} from "../services/listing-transition-service";
		...
		try {
			...
			for (let i = 0; i < companies.length; i++) {
				...
				try {
					...
					const existingByName = await storage.getUnlistedCompanyByName(
						row.name.trim(),
					);
					if (existingByName) {
						results.errors.push({
							row: i + 1,
							name: row.name,
							error: "Company already exists (by name)",
						});
						results.skipped++;
						continue;
					}
					...
					if (row.cin) {
						const existingByCin = await storage.getUnlistedCompanyByCIN(
							row.cin,
						);
						if (existingByCin) {
							results.errors.push({
								row: i + 1,
								name: row.name,
								error: "Company already exists (by CIN)",
							});
							results.skipped++;
							continue;
						}
					...
					await storage.createUnlistedCompany({
						...
						incorporationDate:
							row.incorporationDate || row.incorporation_date || null,
						paidUpCapital:
							row.paidUpCapital?.toString() ||
							row.paid_up_capital?.toString() ||
							null,
						authorizedCapital:
							row.authorizedCapital?.toString() ||
							row.authorized_capital?.toString() ||
							null,
						faceValue:
							row.faceValue?.toString() || row.face_value?.toString() || null,
						totalShares:
							Number.parseInt(row.totalShares || row.total_shares) || null,
		...
		try {
			...
			if (!companyId || !targetStage) {
				return apiResponse.badRequest(
					res,
					"Company ID and target stage are required",
				);
			}
			...
			const request: ListingTransitionRequest = {
				...
				exchange,
				stockSymbol,
				ipoPrice: ipoPrice ? Number.parseFloat(ipoPrice) : undefined,
				listPrice: listPrice ? Number.parseFloat(listPrice) : undefined,
				lotSize: lotSize ? Number.parseInt(lotSize) : undefined,
				notes,
...
router.get(
	...
	async (req: Request, res: Response) => {
		try {
			...
			const isValid = listingTransitionService.isValidTransition(
				currentStage,
				targetStage as any,
			);
			const requiredFields =
				listingTransitionService.getRequiredFieldsForTransition(
					targetStage as any,
				);
			const transactionRules = listingTransitionService.getTransactionRules(
			...
			return apiResponse.success(res, {
				...
				validTransitions: [
					...
					listingTransitionService.isValidTransition(
						currentStage,
						stage as any,
					),
				),
 ...
 */
router.get(
	"/admin/transition/pending",
	requireAdmin,
	async (req: Request, res: Response) => {
		try {
			const pendingCompanies =
				await listingTransitionService.getPendingListings();
...
async function autoPublishCompanyToStore(company: any): Promise<string | null> {
	try {
		...
		if (!resolvedPrice || Number.parseFloat(resolvedPrice) <= 0) {
			const mcPrice = await moneyControlReconciliation.lookupMarketPrice(
				company.name,
				company.isin,
			);
			if (mcPrice && mcPrice > 0) {
				...
				console.log(
					`[AutoPublish] ${company.name}: price from MoneyControl ₹${mcPrice}`,
				);
			}
		}
		...
		const priceNum = resolvedPrice ? Number.parseFloat(resolvedPrice) : null;
		...
		const product = await storage.createStoreProduct({
			...
			currency: "INR",
			minimumInvestment:
				company.minLotSize && priceNum
					? String(Number(company.minLotSize) * priceNum)
					: "10000",
			riskLevel:
				riskMapping[(company.riskRating || "high").toLowerCase()] || "high",
		...
		console.log(
			`[AutoPublish] ${company.name} auto-published to store as product ${product.id}`,
		);
