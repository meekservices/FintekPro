...
function deriveSipStatus(
	holding: any,
): "active" | "expiring" | "lapsed" | "paused" {
	const now = new Date();
	...
	if (meta?.mandateEndDate) {
		...
		const daysToEnd = Math.floor(
			(mandateEnd.getTime() - now.getTime()) / (1000 * 60 * 60 * 24),
		);
	...
	if (lastUpdated) {
		const daysSinceUpdate = Math.floor(
			(now.getTime() - new Date(lastUpdated).getTime()) / (1000 * 60 * 60 * 24),
		);
		const freq = (
		...
		const expectedDays = freq.includes("week")
			? 10
	...
	try {
		...
		for (const h of compHoldings) {
			...
			items.push({
				...
				marketValue: Number.parseFloat(String(h.marketValue || 0)),
		...
		for (const h of pfHoldings) {
			...
			if (!client) continue;
			const status = deriveSipStatus(h);
			const sipAmount = Number.parseFloat(String((h as any).sipAmount || 0));
			items.push({
				id: h.id,
				clientId: userId,
				clientName: `${client.firstName || ""} ${client.lastName || ""}`.trim(),
				...
				nextDebitDate: null,
				marketValue: Number.parseFloat(String(h.currentValue || 0)),
				source: "portfolio",
			});
		}
		...
		const summary = {
			...
			totalMonthlySipAmount: items
				.filter(
					(i) =>
						i.status === "active" &&
						i.frequency?.toLowerCase().includes("month"),
				)
		...
		items.sort(
			(a, b) =>
				(urgencyOrder[a.status as keyof typeof urgencyOrder] ?? 3) -
				(urgencyOrder[b.status as keyof typeof urgencyOrder] ?? 3),
		);
