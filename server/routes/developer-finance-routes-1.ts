...
router.get("/projects", readAccess, async (req: Request, res: Response) => {
	try {
		...
		const whereClause = agentId
			? eq(developerProjects.agentId, agentId)
			: undefined;
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			const parsed = insertProjectLandDetailsSchema.parse({
				...req.body,
				projectId: req.params.projectId,
			});
			const [land] = await db
				.insert(projectLandDetails)
				.values(parsed as any)
				.returning();
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			const parsed = insertProjectApprovalsSchema.parse({
				...req.body,
				projectId: req.params.projectId,
			});
...
router.post(
	"/projects/:projectId/cashflows",
	agentAccess,
	async (req: Request, res: Response) => {
		try {
			const parsed = insertProjectCashflowsSchema.parse({
				...req.body,
				projectId: req.params.projectId,
			});
...
router.post(
	"/projects/:projectId/cashflows/bulk",
	agentAccess,
	async (req: Request, res: Response) => {
		try {
			...
			if (!Array.isArray(rows) || rows.length === 0) {
				return res
					.status(400)
					.json({ success: false, error: "cashflows array required" });
			}
			const parsed = rows.map((r: any) =>
				insertProjectCashflowsSchema.parse({
					...r,
					projectId: req.params.projectId,
				}),
			);
...
router.delete(
	...
	async (req: Request, res: Response) => {
		try {
			await db
				.delete(projectCashflows)
				.where(eq(projectCashflows.id, req.params.id));
			res.json({ success: true });
		} catch (error: any) {
			res.status(500).json({ success: false, error: error.message });
		}
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			const parsed = insertDeveloperFinancialsSchema.parse({
				...req.body,
				projectId: req.params.projectId,
			});
			const [fin] = await db
				.insert(developerFinancials)
				.values(parsed as any)
				.returning();
