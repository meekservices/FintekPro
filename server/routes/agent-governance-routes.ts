...
router.get(
	"/api/agent/client-context/:clientId",
	requireAuth,
	async (req, res) => {
		try {
			...
			holdings.forEach((h) => {
				...
				if (assetClass.includes("equity") || assetClass.includes("stock")) {
					...
					assetClass.includes("debt") ||
					assetClass.includes("bond") ||
					assetClass.includes("fixed")
				) {
					allocation.debt += percentage;
				} else if (
					assetClass.includes("alternative") ||
					assetClass.includes("aif") ||
					assetClass.includes("pms")
				) {
...
router.get("/api/agent/clients", requireAuth, async (req, res) => {
	try {
		...
		const aumResults = await db
			...
			.innerJoin(portfolios, eq(portfolioHoldings.portfolioId, portfolios.id))
			.where(
				sql`${portfolios.userId} = ANY(ARRAY[${sql.join(
		...
		const aumMap = new Map(
			aumResults.map((r) => [r.userId, Number(r.totalAum) || 0]),
		);
...
router.post(
	"/api/agent/recommendation-drafts",
	requireAuth,
	async (req, res) => {
		try {
			...
			console.log(
				`[Agent Governance] Draft saved for client ${clientId} by agent ${agentId}`,
			);
			console.log(
				`Mode: ${mode}, Items: ${items?.length}, Overrides: ${overrides?.length}`,
			);
...
router.post(
	"/api/agent/share-recommendation",
	requireAuth,
	async (req, res) => {
		try {
			...
			console.log(
				`[Agent Governance] Recommendation shared with client ${clientId}`,
			);
	...
	try {
		...
		for (const override of overrides || []) {
			await db.insert(agentOverrideAuditLog).values({
				...
				originalMode: mode,
				overriddenMode:
					override.type === "mode_downgrade" ? override.newValue : undefined,
				ipAddress: req.ip,
		...
		console.log(
			`[Agent Governance] Proposal created for client ${clientId} with ${overrides?.length || 0} overrides`,
		);
		...
		const playbooks = [
			{
				...
				description:
					"Learn how to use Growth-Optimized mode responsibly with proper client disclosure and risk assessment.",
				modules: 5,
			...
			},
			{
				...
				description:
					"Best practices for discussing risk with clients and managing expectations.",
				modules: 4,
			...
			},
			{
				...
				description:
					"Understanding when and how to use overrides, and your accountability responsibilities.",
				modules: 3,
			...
			},
			{
				...
				description:
					"Essential compliance rules for investment advisors under SEBI regulations.",
				modules: 6,
...
router.post(
	"/api/agent/training/complete-module",
	requireAuth,
	async (req, res) => {
		try {
			...
			console.log(
				`[Agent Training] Module ${moduleId} of playbook ${playbookId} completed by agent ${agentId}`,
			);
...
router.post(
	"/api/agent/training/submit-quiz",
	requireAuth,
	async (req, res) => {
		try {
			...
			res.json({
				...
				passed,
				message: passed
					? "Congratulations! You have passed the certification quiz."
				...
				res.json([
					{
						...
						summary:
							"Guidelines for AI-powered investment recommendations and agent responsibilities.",
						isActive: true,
					},
					{
						id: "doc-2",
						...
						effectiveDate: "2025-01-01",
						summary:
							"Standards for assessing client suitability and risk profiling procedures.",
						isActive: true,
					},
					{
						id: "doc-3",
						...
						effectiveDate: "2025-01-01",
						summary:
							"Procedures ensuring human oversight in automated recommendation systems.",
						isActive: true,
					},
					{
						id: "doc-4",
						...
						effectiveDate: "2025-01-01",
						summary:
							"Rules governing when and how agents can override AI recommendations.",
						isActive: true,
					},
					{
						id: "doc-5",
						...
						effectiveDate: "2025-01-01",
						summary:
							"Technical documentation on AI recommendation explainability methods.",
						isActive: true,
					},
					{
						id: "doc-6",
						...
						effectiveDate: "2025-01-01",
						summary:
							"Governance framework for conducting A/B tests on recommendation algorithms.",
						isActive: true,
					},
				]);
			} else {
...
router.post(
	"/api/admin/inspection-evidence/export",
	requireAuth,
	async (req, res) => {
		try {
			...
			const evidenceRecord = {
				...
				agentOverrides: [],
				aiExplanationShown:
					"This recommendation is based on your moderate risk profile and 5-year investment horizon.",
				clientConsent: {
				...
				executionRecord: {
					...
					products: [
						{
							productId: "mf-1",
							productName: "HDFC Balanced Advantage Fund",
							allocation: 40,
						},
						{
							productId: "mf-2",
							productName: "ICICI Prudential Equity & Debt Fund",
							allocation: 30,
						},
						{
							productId: "bond-1",
							productName: "RBI Floating Rate Savings Bond",
							allocation: 30,
						},
					],
				},
...
router.get(
	"/api/admin/agent-oversight/:agentId",
	requireAuth,
	async (req, res) => {
		try {
			...
			const agentDetail = {
				...
				certifications: [
					{
						type: "growth_optimized",
						name: "Growth-Optimized Certification",
						status: "active",
