 ...
 */
router.get(
	"/admin/reconciliation/cache-status",
	requireAdmin,
	...
	async (req: Request, res: Response) => {
		try {
			...
			const existingCins = new Set(
				existingCompanies.map((c: UnlistedCompany) => c.cin).filter(Boolean),
			);
			...
			const [moneyControlResults, mcaResults, credhiveResults] =
				await Promise.allSettled([
					...
					(async () => {
						...
						try {
							const cached =
								await moneyControlReconciliation.getReconciliationSuggestions(
									false,
								);
							const allCompanies = cached.suggestions.map(
								(s) => s.externalCompany,
							);
							...
							sourceStatuses.moneycontrol.error = {
								...
								troubleshooting:
									"MoneyControl data fetch failed. This is a web scraping source and may be temporarily unavailable.",
								isRetryable: true,
							};
							return [];
						}
					...
					(async () => {
						...
						const isCinFormat =
							query.length === 21 &&
							/^[A-Z]{1}[0-9]{5}[A-Z]{2}[0-9]{4}[A-Z]{3}[0-9]{6}$/.test(query);
						...
						if (!isCinFormat) {
							...
							sourceStatuses.mca.error = {
								...
								troubleshooting:
									"MCA (Sandbox.co.in API) only supports direct CIN lookup, not company name search. Enter a valid 21-character CIN to search MCA.",
								isRetryable: false,
							};
							return [];
						}
						...
						const mcaResult =
							await mcaService.getCompanyByCINWithDetails(query);
					...
					(async () => {
						...
						if (credhiveResult.error) {
							sourceStatuses.credhive.error = {
								code: 503,
								message: credhiveResult.error,
								troubleshooting: "Credhive API unavailable",
								isRetryable: true,
							};
							return [];
						}
				...
				for (const company of moneyControlResults.value) {
					...
					const existing = isInFintekPro
						? existingCompanies.find(
								(c: UnlistedCompany) => c.isin === company.isin,
							)
						: null;
			...
			if (mcaResults.status === "fulfilled") {
				for (const company of mcaResults.value) {
					...
					const isInFintekPro = company.cin
						? existingCins.has(company.cin)
						: false;
					const existing = company.cin
						? existingCompanies.find(
								(c: UnlistedCompany) => c.cin === company.cin,
							)
						: null;
 ...
 */
router.get(
	"/admin/company-details/:source/:id",
	requireAdmin,
	async (req: Request, res: Response) => {
		try {
			...
			if (source === "credhive") {
				...
				const financials = await credhiveService
			...
			if (source === "moneycontrol") {
				const cached =
					await moneyControlReconciliation.getReconciliationSuggestions(false);
				const company = cached.suggestions.find(
					(s) => s.externalCompany.isin === id,
				);

				if (!company) {
					return apiResponse.notFound(
						res,
						"Company not found in MoneyControl cache",
					);
				}
			...
			return apiResponse.badRequest(
				res,
				'Invalid source. Use "mca", "moneycontrol", or "credhive"',
			);
		} catch (error: any) {
 ...
 */
router.get(
	"/admin/company-unified-data/:companyId",
	requireAdmin,
	async (req: Request, res: Response) => {
		try {
			...
			const unifiedData =
				await unifiedCompanyDataService.getCompanyData(companyId);
			const valuationData = await valuationService.getValuationData({
				companyId,
			});
			...
			return apiResponse.serverError(
				res,
				error.message || "Failed to fetch unified company data",
			);
		}
	},
);
...
const addToFintekProSchema = z.object({
	...
	isin: z
		.string()
		.regex(isinRegex, "Invalid ISIN format")
		.optional()
		.or(z.literal("")),
	cin: z
		.string()
	...
	buyPrice: z
		...
		.max(10000000, "Buy price too high")
		.optional(),
	sellPrice: z
		.number()
		.positive("Sell price must be positive")
		.max(10000000, "Sell price too high")
		.optional(),
		...
		try {
			...
			const existingCompany = existingCompanies.find(
				(c: UnlistedCompany) =>
					...
					(data.cin && c.cin === data.cin),
			);

			if (existingCompany) {
				return apiResponse.badRequest(
					res,
					"This company already exists in FintekPro",
					{
			...
			console.log(
				`[Add to FintekPro] Created new unlisted company: ${newCompany.id} - ${data.name} by admin ${userId}`,
			);

			return apiResponse.created(res, {
				message:
					"Company added to FintekPro successfully. You can now review it in the Unlisted Companies list and publish to store when ready.",
				company: newCompany,
			});
		} catch (error: any) {
			...
			return apiResponse.serverError(
				res,
				error.message || "Failed to add company to FintekPro",
			);
		}
	},
);

const publishCompanySchema = z
	.object({
		name: z
			.string()
			.min(1, "Company name is required")
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (!existingCompany) {
				...
				console.log(
					`[Publish] Created new unlisted company: ${existingCompany.id} - ${data.name}`,
				);
			}

			const allStoreProducts = await storage.getAllStoreProducts();
			const existingProduct = allStoreProducts.find(
				(p: any) =>
					p.category === "unlisted" &&
					p.sourceCompanyId === existingCompany!.id,
			);

			if (existingProduct) {
				return apiResponse.badRequest(
					res,
					"This company is already published to the store",
				);
			}
			...
			return apiResponse.serverError(
				res,
				error.message || "Failed to publish company",
			);
		}
	},
);
 ...
 */
router.post(
	"/companies/:companyId/save-draft-prices",
	requireAdmin,
	async (req: Request, res: Response) => {
		try {
			const { companyId } = req.params;
			...
			if (typeof buyPrice !== "number" || typeof sellPrice !== "number") {
				return apiResponse.badRequest(
					res,
					"buyPrice and sellPrice must be numbers",
				);
			}
			...
			return apiResponse.success(
				res,
				result,
				"Draft prices saved successfully",
			);
		} catch (error: any) {
 ...
 */
router.get(
	"/companies/:companyId/validate-prices",
	requireAdmin,
	async (req: Request, res: Response) => {
		try {
			const { companyId } = req.params;
			...
			if (Number.isNaN(buyPrice) || Number.isNaN(sellPrice)) {
				return apiResponse.badRequest(
					res,
					"buyPrice and sellPrice query parameters are required and must be numbers",
				);
			}

			const result = await unlistedPricingWorkflowService.validatePrices(
				companyId,
				buyPrice,
				sellPrice,
			);
