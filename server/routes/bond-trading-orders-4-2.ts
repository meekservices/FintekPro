...
export function registerBondTradingOrderPart4Part2Routes(app: Express): void {
	app.get("/api/amfi/nav-history/:scheme_code", async (req, res) => {
		try {
			...
			const analytics = {
				...
				periodReturn: (
					((navHistory[0].nav - navHistory[navHistory.length - 1].nav) /
				...
				avgNAV: (
					navHistory.reduce((sum, h) => sum + h.nav, 0) / navHistory.length
				).toFixed(4),
			};
	...
	app.get("/api/amfi/categories", async (req, res) => {
		try {
							...
							subcategories: [
								{
									...
									avgReturns1Y:
										cat.funds.length > 0
											? cat.funds.reduce(
													(sum, fund) => sum + (fund.returns["1Y"] || 0),
													0,
												) / cat.funds.length
											: 0,
									riskLevel: cat.riskLevel,
			...
			const summary = {
				...
				totalFunds: categories.reduce(
					(sum, cat) =>
						sum +
						cat.subcategories.reduce((subSum, sub) => subSum + sub.count, 0),
					0,
				),
				avgReturns1Y: (
					categories.reduce(
						(sum, cat) =>
							...
							cat.subcategories.reduce(
								(subSum, sub) => subSum + sub.avgReturns1Y * sub.count,
								0,
							),
						0,
					) /
	...
	app.post("/api/funds/compare", async (req, res) => {
		try {
			...
			const comparisonRecord = await db
				.insert(fundComparisons)
				.values({
