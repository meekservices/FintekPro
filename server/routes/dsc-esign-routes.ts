	...
	async (req: Request, res: Response, next: NextFunction) => {
		try {
			...
			const supportedAlgorithms =
				unifiedESignService.getDSCSupportedAlgorithms();
			...
			res.json({
				...
				description:
					config?.description ||
					"Digital Signature Certificate via USB Token or Smart Card",
				...
				requirements: {
					...
					Class3:
						"In-person identity verification - Physical presence required",
				},
...
router.post(
	...
	async (req: Request, res: Response, next: NextFunction) => {
		try {
			...
			const validation = await dscTokenESignService.validateCertificate(
				result.data as any,
			);
...
router.post(
	...
	async (req: Request, res: Response, next: NextFunction) => {
		try {
			...
			const response = await unifiedESignService.submitDSCSignature(
				result.data,
			);
...
router.post(
	...
	async (req: Request, res: Response, next: NextFunction) => {
		try {
			...
			const response = await unifiedESignService.cancelDSCSession(
				transactionId,
				userId,
				reason,
			);
