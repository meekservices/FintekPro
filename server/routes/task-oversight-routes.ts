...
router.get("/stats", async (req, res: Response) => {
	try {
		...
		const pendingTasks =
			(taskStats?.pending || 0) + (appointmentStats?.scheduled || 0);
		const overdueTasks =
			(taskStats?.overdue || 0) + (appointmentStats?.overdue || 0);
		const completedToday =
			(taskStats?.completedToday || 0) +
			(appointmentStats?.completedToday || 0);
		const dueToday =
			(taskStats?.dueToday || 0) + (appointmentStats?.dueToday || 0);
		...
		res
			...
			.json({
				success: false,
				message: "Failed to fetch task oversight stats",
			});
	}
});
...
router.get("/agents", async (req, res: Response) => {
    ...
    `);
		...
		const clientTaskStatsMap = new Map<
			string,
			{
				total: number;
				pending: number;
		...
		const agentOverviews: AgentTaskOverview[] = agents.map((agent) => {
			const aptStats = appointmentStatsMap.get(agent.id) || {
				total: 0,
				pending: 0,
				overdue: 0,
				completedToday: 0,
			};
			const taskStats = clientTaskStatsMap.get(agent.id) || {
				total: 0,
				pending: 0,
				overdue: 0,
				completedToday: 0,
		...
		res
			.status(500)
			.json({ success: false, message: "Failed to fetch agent task overview" });
	}
});
...
router.get("/alerts", async (req, res: Response) => {
	try {
		...
		const overdueAppointments = await db
			...
			.from(agentAppointments)
			.where(
				and(eq(agentAppointments.status, "scheduled"), sql`date < ${todayStr}`),
			)
		...
		const overdueTasks = await db
			...
			.where(
				or(
					...
					and(eq(clientTasks.status, "pending"), sql`due_date < ${todayStr}`),
				),
			)
		...
		if (agentIds.size > 0) {
			...
			const agentIdsSqlJoined = sql.join(
				agentIdArray.map((id) => sql`${id}`),
				sql`, `,
			);
		...
		for (const apt of overdueAppointments) {
			const daysOverdue = Math.floor(
				(today.getTime() - new Date(apt.date).getTime()) /
					(1000 * 60 * 60 * 24),
			);
			alerts.push({
				id: apt.id,
		...
		res
			.status(500)
			.json({ success: false, message: "Failed to fetch compliance alerts" });
	}
});
...
router.get("/breakdown", async (req, res: Response) => {
	try {
		...
		for (const item of appointmentTypeBreakdown) {
			const existingIdx = breakdown.findIndex(
				(b) =>
					b.name.toLowerCase() === formatTaskTypeName(item.type).toLowerCase(),
			);
			if (existingIdx >= 0) {
		...
		res
			.status(500)
			.json({ success: false, message: "Failed to fetch task type breakdown" });
	}
});
