...
export function registerSandboxGSTRoutes(app: Express): void {
	...
	app.post("/api/gst/auth/otp", async (req: Request, res: Response) => {
		try {
			...
			if (!username || !gstin)
				return bad(res, "username and gstin are required");
			ok(res, await sandboxGSTService.generateOtp(username, gstin));
		} catch (e) {
			err(res, e, "generateOtp");
		}
	});
	...
	app.post("/api/gst/auth/otp/verify", async (req: Request, res: Response) => {
		try {
			...
			if (!username || !gstin || !otp)
				return bad(res, "username, gstin and otp are required");
	 ...
	 */
	app.post(
		"/api/gst/auth/session/refresh",
		async (_req: Request, res: Response) => {
			try {
				ok(res, await sandboxGSTService.refreshSession());
			} catch (e) {
				err(res, e, "refreshSession");
			}
		},
	);
	 ...
	 */
	app.get(
		"/api/gst/gstr-2a/:section/:year/:month",
		async (req: Request, res: Response) => {
			try {
				...
				ok(
					res,
					await sandboxGSTService.gstr2aSection(section as any, year, month),
				);
			} catch (e) {
				err(res, e, "gstr2aSection");
			}
		},
	);
	 ...
	 */
	app.post(
		"/api/gst/gstr-2b/:year/:month/regenerate",
		async (req: Request, res: Response) => {
			try {
				const { year, month } = req.params;
				ok(res, await sandboxGSTService.gstr2bRegenerate(year, month));
			} catch (e) {
				err(res, e, "gstr2bRegenerate");
			}
		},
	);
	 ...
	 */
	app.post(
		"/api/gst/e-invoice/authenticate",
		async (req: Request, res: Response) => {
			try {
				const { username, password, gstin } = req.body;
				if (!username || !password || !gstin)
					return bad(res, "username, password and gstin are required");
				ok(
					res,
					await sandboxGSTService.eInvoiceAuthenticate(
						username,
						password,
						gstin,
					),
				...
				err(res, e, "eInvoiceAuthenticate");
			}
		},
	);
	 ...
	 */
	app.post(
		"/api/gst/e-invoice/invoice",
		async (req: Request, res: Response) => {
			try {
				if (!req.body || Object.keys(req.body).length === 0)
					return bad(res, "Invoice data is required");
				ok(res, await sandboxGSTService.eInvoiceGenerate(req.body));
			} catch (e) {
				err(res, e, "eInvoiceGenerate");
			}
		},
	);
	 ...
	 */
	app.post(
		"/api/gst/e-invoice/invoice/:irn/cancel",
		async (req: Request, res: Response) => {
			try {
				...
				ok(
					res,
					await sandboxGSTService.eInvoiceCancel(
						irn,
						String(cancelReason),
						cancelRemark || "",
					),
	 ...
	 */
	app.post(
		"/api/gst/e-invoice/pdf/generate",
		async (req: Request, res: Response) => {
			try {
				const { irn, signedQrCode } = req.body;
				if (!irn || !signedQrCode)
					return bad(res, "irn and signedQrCode are required");
				ok(res, await sandboxGSTService.eInvoiceGeneratePdf(irn, signedQrCode));
			} catch (e) {
				err(res, e, "eInvoiceGeneratePdf");
			}
		},
	);
	...
	app.get("/api/gst/e-invoice/invoice", async (req: Request, res: Response) => {
		try {
			const { docType, docNo, docDate, sellerGstin } = req.query as Record<
				string,
				string
			>;
			ok(
				res,
				await sandboxGSTService.eInvoiceGetByDocData({
					docType,
					docNo,
					docDate,
					sellerGstin,
			...
			err(res, e, "eInvoiceGetByDocData");
		}
	});
	 ...
	 */
	app.post(
		"/api/gst/e-invoice/invoice/:irn/e-way-bill",
		async (req: Request, res: Response) => {
			try {
				const { irn } = req.params;
				if (!req.body || Object.keys(req.body).length === 0)
					return bad(res, "E-Way Bill data is required");
				ok(res, await sandboxGSTService.eInvoiceGenerateEwb(irn, req.body));
			} catch (e) {
				err(res, e, "eInvoiceGenerateEwb");
			}
		},
	);
	 ...
	 */
	app.post(
		"/api/gst/e-invoice/gstin/search",
		async (req: Request, res: Response) => {
			try {
				const { gstin } = req.body;
				if (!gstin) return bad(res, "gstin is required");
	 ...
	 */
	app.post(
		"/api/gst/e-way-bill/authenticate",
		async (req: Request, res: Response) => {
			try {
				const { username, password, gstin } = req.body;
				if (!username || !password || !gstin)
				...
				ok(
					res,
					await sandboxGSTService.ewbAuthenticate(username, password, gstin),
				);
			} catch (e) {
				err(res, e, "ewbAuthenticate");
			}
		},
	);
	 ...
	 */
	app.get(
		"/api/gst/e-way-bill/bill/:ewb_no",
		async (req: Request, res: Response) => {
			try {
	 ...
	 */
	app.get(
		"/api/gst/e-way-bill/error-list",
		async (_req: Request, res: Response) => {
			try {
				ok(res, await sandboxGSTService.ewbGetErrorList());
			} catch (e) {
				err(res, e, "ewbGetErrorList");
			}
		},
	);
	 ...
	 */
	app.post(
		"/api/gst/e-way-bill/gstin/search",
		async (req: Request, res: Response) => {
			try {
				const { gstin } = req.body;
				if (!gstin) return bad(res, "gstin is required");
	 ...
	 */
	app.post(
		"/api/gst/e-way-bill/consignor/bill",
		async (req: Request, res: Response) => {
			try {
				if (!req.body || !req.body.docNo)
					return bad(res, "E-Way Bill data with docNo is required");
				ok(res, await sandboxGSTService.ewbConsignorGenerate(req.body));
			} catch (e) {
				err(res, e, "ewbConsignorGenerate");
			}
		},
	);
	 ...
	 */
	app.get(
		"/api/gst/e-way-bill/consignor/bills",
		async (req: Request, res: Response) => {
			try {
				const { date, type, fromDate, toDate } = req.query as Record<
					string,
					string
				>;
				ok(
					res,
					await sandboxGSTService.ewbConsignorGetByDate({
						date,
						type,
						fromDate,
						toDate,
				...
				err(res, e, "ewbConsignorGetByDate");
			}
		},
	);
	 ...
	 */
	app.post(
		"/api/gst/e-way-bill/consignor/bill/:ewb_no/cancel",
		async (req: Request, res: Response) => {
			try {
				...
				ok(
					res,
					await sandboxGSTService.ewbConsignorCancel(
						ewb_no,
						Number(cancelRsnCode),
						cancelRmrk || "",
					),
				);
			} catch (e) {
				err(res, e, "ewbConsignorCancel");
			}
		},
	);
	 ...
	 */
	app.post(
		"/api/gst/e-way-bill/consignor/bill/:ewb_no/extend",
		async (req: Request, res: Response) => {
			try {
				const { ewb_no } = req.params;
				...
				ok(
					res,
					await sandboxGSTService.ewbConsignorExtendValidity(ewb_no, req.body),
				);
			} catch (e) {
				err(res, e, "ewbConsignorExtendValidity");
			}
		},
	);
	 ...
	 */
	app.put(
		"/api/gst/e-way-bill/consignor/bill/:ewb_no/vehicle",
		async (req: Request, res: Response) => {
			try {
				const { ewb_no } = req.params;
				...
				ok(
					res,
					await sandboxGSTService.ewbConsignorUpdateVehicle(ewb_no, req.body),
				);
			} catch (e) {
				err(res, e, "ewbConsignorUpdateVehicle");
			}
		},
	);
	 ...
	 */
	app.put(
		"/api/gst/e-way-bill/consignor/bill/:ewb_no/transporter",
		async (req: Request, res: Response) => {
			try {
				const { ewb_no } = req.params;
				...
				ok(
					res,
					await sandboxGSTService.ewbConsignorUpdateTransporter(
						ewb_no,
						transporterId,
					),
				);
			} catch (e) {
				err(res, e, "ewbConsignorUpdateTransporter");
			}
		},
	);
	 ...
	 */
	app.post(
		"/api/gst/e-way-bill/consignor/consolidated-bill",
		async (req: Request, res: Response) => {
			try {
				if (!req.body.tripSheetEwbBills?.length)
					return bad(res, "tripSheetEwbBills (array) is required");
				ok(res, await sandboxGSTService.ewbConsignorConsolidate(req.body));
			} catch (e) {
				err(res, e, "ewbConsignorConsolidate");
			}
		},
	);
	 ...
	 */
	app.get(
		"/api/gst/e-way-bill/consignee/bills",
		async (req: Request, res: Response) => {
			try {
				const { fromDate, toDate } = req.query as Record<string, string>;
				ok(
					res,
					await sandboxGSTService.ewbConsigneeGetByDate({ fromDate, toDate }),
				);
			} catch (e) {
				err(res, e, "ewbConsigneeGetByDate");
			}
		},
	);
	 ...
	 */
	app.post(
		"/api/gst/e-way-bill/consignee/bill/:ewb_no/reject",
		async (req: Request, res: Response) => {
			try {
				ok(res, await sandboxGSTService.ewbConsigneeReject(req.params.ewb_no));
			} catch (e) {
				err(res, e, "ewbConsigneeReject");
			}
		},
	);
	 ...
	 */
	app.get(
		"/api/gst/e-way-bill/transporter/bills",
		async (req: Request, res: Response) => {
			try {
				const { fromDate, toDate, state } = req.query as Record<string, string>;
				ok(
					res,
					await sandboxGSTService.ewbTransporterGetByDateAndState({
						fromDate,
						toDate,
						state,
					}),
				...
				err(res, e, "ewbTransporterGetByDateAndState");
			}
		},
	);
	...
	app.get("/api/gst/status", (_req: Request, res: Response) => {
		res.json({
			...
			groups: {
				...
				gstr2b: ["POST /api/gst/gstr-2b/:year/:month/regenerate"],
				eInvoice: [
					"POST /api/gst/e-invoice/authenticate",
					"POST /api/gst/e-invoice/invoice",
					"GET  /api/gst/e-invoice/invoice/:irn",
	...
	console.log(
		"✅ Sandbox GST routes registered (Public / Auth / GSTR-2A / GSTR-2B / E-Invoice / E-Way Bill)",
	);
}
