...
router.post("/:id/approve", async (req, res) => {
  try {
    ...
    meetingNotificationService.sendNotification('approved', {
...
router.patch("/:id/reschedule", async (req, res) => {
  try {
    ...
    if (booking.clientId !== userId && booking.agentId !== userId) {
      return res.status(403).json({ error: "Not authorized to reschedule this meeting" });
    }

    if (booking.status === "cancelled" || booking.status === "completed") {
      return res.status(400).json({ error: "Cannot reschedule a cancelled or completed meeting" });
    }
    ...
    const participantEmails = [client?.email, agent?.email].filter(Boolean) as string[];
    ...
    meetingNotificationService.sendNotification('rescheduled', {
...
router.patch("/:id/complete", async (req, res) => {
  try {
    ...
    const userRoles = user.roles || [];
    if (!userRoles.some((r: string) => ["agent", "admin", "partner"].includes(r))) {
      return res.status(403).json({ error: "Only agents can mark meetings as completed" });
    }
    ...
    if (booking.agentId !== user.id) {
      return res.status(403).json({ error: "Only the assigned agent can complete this meeting" });
    }

    if (booking.status !== "confirmed") {
      return res.status(400).json({ error: "Only confirmed meetings can be marked as completed" });
    }
    ...
    meetingNotificationService.sendNotification('completed', {
      bookingId: updated.id,
      topic: updated.topic,
      description: updated.description || undefined,
      scheduledAt: updated.scheduledAt,
...
router.get("/admin/analytics", async (req, res) => {
  try {
    ...
    const stats = {
      ...
      averageDuration: Math.round(allMeetings.reduce((sum, m) => sum + (m.duration || 30), 0) / (allMeetings.length || 1)),
      completionRate: allMeetings.length > 0 
        ? Math.round((allMeetings.filter(m => m.status === "completed").length / allMeetings.length) * 100) 
        : 0,
    };
