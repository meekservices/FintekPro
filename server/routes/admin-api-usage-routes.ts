...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			const result = await apiUsageTrackingService.addProvider(
				validated,
				adminId,
			);
...
router.get(
	"/api/admin/api-usage/monthly-estimate",
	requireAuth,
	requireRole("admin"),
	async (req: Request, res: Response) => {
		try {
			...
			console.error(
				"[Admin API Usage] Error fetching monthly estimate:",
				error,
			);
