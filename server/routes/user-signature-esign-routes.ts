...
router.get(
	"/api/esign/user-signature/status",
			...
			if (!validated.signatureId && !validated.signatureDataUrl) {
				...
				if (!defaultSig) {
					return res.status(400).json({
						error:
							"No signature provided and no default signature found. Please create a signature first.",
					});
				}
			}
...
router.get(
	"/api/esign/user-signature/transaction/:transactionId",
	requireAuth,
	async (req: Request, res: Response) => {
		try {
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (!signatureDataUrl) {
				return res
					.status(400)
					.json({ error: "Signature data URL is required" });
			}

			const result =
				await userSignatureESignService.validateSignature(signatureDataUrl);
			res.json(result);
		} catch (error) {
...
router.get(
	"/api/esign/user-signature/available",
	requireAuth,
	async (req: Request, res: Response) => {
		try {
			const userId = (req.user as any)?.id;
