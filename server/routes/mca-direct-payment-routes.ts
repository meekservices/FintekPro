...
import { z } from "zod";
import {
	mcaDirectPaymentService,
	McaFeeType,
} from "../services/mca-direct-payment-service";
	...
	try {
		...
		if (result.success) {
			res.json({
				...
				message:
					"Payment initiated. Please complete payment on MCA portal and return to confirm.",
				instructions: [
...
router.post("/:paymentId/cancel", async (req: Request, res: Response) => {
	try {
		...
		if (!reason) {
			return res
				.status(400)
				.json({ success: false, error: "Cancellation reason is required" });
		}
	...
	try {
		...
		if (!fromDate || !toDate) {
			return res.status(400).json({
				success: false,
				error:
					"fromDate and toDate query parameters are required (YYYY-MM-DD format)",
			});
		}
