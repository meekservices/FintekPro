...
function extractParams(req: any) {
	return {
		riskScore: Number.parseInt(req.query.riskScore as string) || 50,
		horizon: Number.parseInt(req.query.horizon as string) || 5,
...
uniPortfolioRouter.get("/unified", async (req, res) => {
	...
	try {
		const snapshot = await uniPortfolioService.getSnapshot(
			req.user!.id,
			riskScore,
			horizon,
			segment,
		...
		logger.error(`[UniPortfolio API] Snapshot failed`, {
			event: "UNI_PORTFOLIO_API_ERROR",
			user_id: req.user!.id,
			error: err?.message,
		});
		res
	...
	try {
		...
		res.json({
			...
			data: {
				...
				confidence_score: snapshot.analysis.rebalancing.needsRebalance
					? 85
					: 70,
...
uniPortfolioRouter.post("/unified/refresh", async (req, res) => {
	...
	const { riskScore, horizon, segment } = extractParams(req);
	try {
		const snapshot = await uniPortfolioService.refresh(
			req.user!.id,
			riskScore,
			horizon,
			segment,
