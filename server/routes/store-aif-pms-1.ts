	...
	try {
		...
		if (riskScore) {
			const [minRisk, maxRisk] = (riskScore as string).includes("-")
				...
				: [
						Number.parseInt(riskScore as string),
						Number.parseInt(riskScore as string),
					];
		...
		if (minInvestment) {
			const minVal = Number.parseFloat(minInvestment as string);
			if (!Number.isNaN(minVal)) {
				conditions.push(
					sql`CAST(${aifMaster.minInvestment} AS NUMERIC) >= ${minVal}`,
				);
			}
		}

		if (maxInvestment) {
			const maxVal = Number.parseFloat(maxInvestment as string);
			if (!Number.isNaN(maxVal)) {
				conditions.push(
					sql`CAST(${aifMaster.minInvestment} AS NUMERIC) <= ${maxVal}`,
				);
			}
		}
...
router.patch("/aif/:id/publish", requireAdmin, async (req, res) => {
	try {
		...
		res.json({
			success: true,
			message: `AIF ${validation.data.isPublished ? "published" : "unpublished"} successfully`,
		});
	} catch (error: any) {
...
router.get("/pms", async (req, res) => {
	try {
		...
		if (minInvestment) {
			...
			if (!Number.isNaN(minVal)) {
				conditions.push(
					sql`CAST(${pmsMaster.minInvestment} AS NUMERIC) >= ${minVal}`,
				);
			}
		}
		...
		if (maxInvestment) {
			...
			if (!Number.isNaN(maxVal)) {
				conditions.push(
					sql`CAST(${pmsMaster.minInvestment} AS NUMERIC) <= ${maxVal}`,
				);
			}
		}
...
router.patch("/pms/:id/publish", requireAdmin, async (req, res) => {
	try {
		...
		res.json({
			success: true,
			message: `PMS ${validation.data.isPublished ? "published" : "unpublished"} successfully`,
		});
	} catch (error: any) {
...
router.get("/performance/fund/:id/monthwise", async (req, res) => {
	try {
		...
		yearsAgo.setFullYear(
			yearsAgo.getFullYear() - Number.parseInt(years as string),
		);
...
router.post("/admin/store/publish", requireAdmin, async (req, res) => {
	try {
		...
		res.json({
			success: true,
			message: `${fundType.toUpperCase()} ${publish ? "published" : "unpublished"} successfully`,
		});
	} catch (error: any) {
