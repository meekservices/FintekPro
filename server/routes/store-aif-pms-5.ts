	...
	try {
		...
		const [manager] = await db
			...
			.values({
				...
				fundsManaged: fundsManaged ? Number.parseInt(fundsManaged) : null,
...
router.patch("/fund-managers/:id", requireAdmin, async (req, res) => {
	try {
		...
		if (updates.experienceYears) {
			updates.experienceYears = Number.parseInt(updates.experienceYears);
		}
		if (updates.fundsManaged) {
			updates.fundsManaged = Number.parseInt(updates.fundsManaged);
		}
...
router.delete("/fund-managers/:id", requireAdmin, async (req, res) => {
	try {
		...
		const pmsCount = await db
			.select({ count: sql<number>`count(*)` })
			.from(pmsMaster)
			.where(eq(pmsMaster.managerId, id));
		const aifCount = await db
			.select({ count: sql<number>`count(*)` })
			.from(aifMaster)
			.where(eq(aifMaster.managerId, id));

		if ((pmsCount[0]?.count || 0) > 0 || (aifCount[0]?.count || 0) > 0) {
			return res.status(400).json({
				error:
					"Cannot delete fund manager with associated funds. Please reassign or remove funds first.",
			});
		}
...
router.post("/remittance/:id/upload", requireAuth, async (req, res) => {
	try {
		...
		if (error.name === "ZodError") {
			return res
				.status(400)
				.json({ error: "Invalid upload data", details: error.issues });
		}
...
router.get("/remittance/my", requireAuth, async (req, res) => {
	try {
		const user = req.user as any;
		const remittances = await externalRemittanceService.getRemittancesByUser(
			user.id,
		);
...
router.get("/remittance/order/:orderId", requireAuth, async (req, res) => {
	try {
		...
		const remittances =
			await externalRemittanceService.getRemittancesByOrder(orderId);
...
router.get(
	"/remittance/check/:productType/:orderId",
	requireAuth,
	async (req, res) => {
		try {
			...
			if (productType !== "aif" && productType !== "pms") {
				return res
					.status(400)
					.json({ error: "Invalid product type. Must be 'aif' or 'pms'" });
			}

			const status =
				await externalRemittanceService.checkRemittanceProofRequired(
...
router.get("/admin/remittance/pending", requireAdmin, async (req, res) => {
	try {
		const pendingRemittances =
			await externalRemittanceService.getPendingVerifications();
...
router.post("/admin/remittance/:id/verify", requireAdmin, async (req, res) => {
	try {
		...
		if (error.name === "ZodError") {
			return res
				.status(400)
				.json({ error: "Invalid verification data", details: error.issues });
		}
...
router.get("/recommendation-sync/preview", requireAdmin, async (req, res) => {
	try {
		const aifLimit = Number.parseInt(req.query.aifLimit as string) || 15;
		const pmsLimit = Number.parseInt(req.query.pmsLimit as string) || 15;

		console.log(
			`[Recommendation Sync] Preview requested: ${aifLimit} AIFs, ${pmsLimit} PMS`,
		);

		const preview = await aiRecommendationSyncService.previewSync(
			aifLimit,
			pmsLimit,
		);
...
router.post("/recommendation-sync/execute", requireAdmin, async (req, res) => {
	try {
		...
		console.log(
			`[Recommendation Sync] Execute requested: ${aifLimit} AIFs, ${pmsLimit} PMS`,
		);

		const result = await aiRecommendationSyncService.executeSync(
			aifLimit,
			pmsLimit,
		);
