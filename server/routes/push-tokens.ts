...
pushTokensRouter.post('/', requireMobileAuth, async (req: Request, res: Response) => {
  ...
  if (!token || !platform) {
    return res.status(400).json({
      success: false,
      error: { error_code: 'MISSING_FIELDS', message: 'token and platform are required', retryable: false }
    });
  }
  ...
  try {
    ...
    return res.json({ success: true, meta: { timestamp: new Date().toISOString(), version: '1.0' } });
  } catch (err: any) {
    // Table may not exist yet — handled gracefully
    if (err.message?.includes('push_tokens')) {
      console.warn('[PushTokens] Table not found — will be created on next schema repair');
      return res.json({ success: true, meta: { timestamp: new Date().toISOString(), version: '1.0' } });
    }
    return res.status(500).json({
      success: false,
      error: { error_code: 'TOKEN_SAVE_ERROR', message: 'Could not save push token', retryable: true }
    });
  }
});

// ── DELETE /api/push-tokens/:token ────────────────────────────────────────
pushTokensRouter.delete('/:token', requireMobileAuth, async (req: Request, res: Response) => {
  const mobileUser = (req as any).mobileUser;
  ...
  try {
    ...
    return res.status(500).json({
      success: false,
      error: { error_code: 'TOKEN_DELETE_ERROR', message: 'Could not deregister push token', retryable: true }
    });
  }
});
