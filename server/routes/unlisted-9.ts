 ...
 */
router.post(
	"/deals/:dealId/initiate-payment",
	requireAuth,
	requireLevel2,
	requireRiskDisclosure("buy"),
...
router.get(
	...
	async (req: Request, res: Response) => {
		try {
			...
			const totalValue = Number.parseFloat(deal.totalValue);
			const fees = unlistedEscrowService.calculateFees(totalValue);

			return apiResponse.success(res, {
				dealId,
				quantity: deal.quantity,
				pricePerShare: Number.parseFloat(deal.agreedPrice),
				totalValue,
				...fees,
			});
		} catch (error: any) {
 ...
 */
router.post(
	"/deals/:dealId/mark-transfer-pending",
	requireAuth,
	requireLevel2,
	requireRiskDisclosure("sell"),
	async (req: Request, res: Response) => {
		try {
			const { dealId } = req.params;
			const user = req.user as any;
			...
			const result = await unlistedEscrowService.markTransferPending(
				dealId,
				user.id,
				disSlipId,
			);
...
router.get("/payment/callback", async (req: Request, res: Response) => {
	try {
		...
		if (!order_id || !escrow_id || !deal_id) {
			return res.redirect(
				"/unlisted/my-orders?payment=error&message=Invalid callback parameters",
			);
		}
		...
		if (result.success && result.status === "escrowed") {
			return res.redirect(
				`/unlisted/my-orders?payment=success&deal=${deal_id}`,
			);
		}
		return res.redirect(
			`/unlisted/my-orders?payment=failed&deal=${deal_id}&status=${result.status}`,
		);
	} catch (error: any) {
 ...
 */
router.post(
	"/admin/deals/:dealId/release-escrow",
	requireAdmin,
	async (req: Request, res: Response) => {
		try {
			const { dealId } = req.params;
			...
			if (!result.success) {
				return apiResponse.badRequest(
					res,
					result.error || "Failed to initiate release approval",
				);
			}
			...
			try {
				...
				if (deal) {
					await auditLogArchivalService.archiveUnlistedMarketplaceEvent({
						...
						dealId,
						userId: adminUser.id,
						amount: Number.parseFloat((deal as any).totalAmount || "0"),
						metadata: {
							transferConfirmationId,
					...
					const amount = Number.parseFloat((deal as any).totalAmount || "0");
					if (amount >= 1000000) {
						await regulatoryReportingService.registerReportableEvent({
				...
				console.error(
					"Compliance logging failed for escrow release initiation:",
					complianceError,
				);
			}
			...
			return apiResponse.success(
				res,
				result,
				"Release approval initiated. Awaiting second admin approval.",
			);
		} catch (error: any) {
 ...
 */
router.post(
	"/admin/deals/:dealId/refund-escrow",
	requireAdmin,
	async (req: Request, res: Response) => {
		try {
			const { dealId } = req.params;
			...
			if (!result.success) {
				return apiResponse.badRequest(
					res,
					result.error || "Failed to initiate refund approval",
				);
			}
			...
			try {
				...
				console.error(
					"Compliance logging failed for escrow refund initiation:",
					complianceError,
				);
			}
			...
			return apiResponse.success(
				res,
				result,
				"Refund approval initiated. Awaiting second admin approval.",
			);
		} catch (error: any) {
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (documentType === "dis_slip" && deal.status === "escrowed") {
				await unlistedEscrowService.markTransferPending(
					dealId,
					user.id,
					document.id,
				);
 ...
 */
router.post(
	"/admin/escrow/initiate-approval",
	requireAuth,
	requireAdmin,
	async (req: Request, res: Response) => {
		try {
			...
			if (!dealId || !requestType) {
				return apiResponse.badRequest(
					res,
					"dealId and requestType are required",
				);
			}
			...
			if (!["release", "refund"].includes(requestType)) {
				return apiResponse.badRequest(
					res,
					'requestType must be "release" or "refund"',
				);
			}
			...
			if (!result.success) {
				return apiResponse.badRequest(
					res,
					result.error || "Failed to initiate approval",
				);
			}
			...
			return apiResponse.success(
				res,
				result,
				"Approval request created. Awaiting second admin approval.",
			);
		} catch (error: any) {
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (!approvalId || !action) {
				return apiResponse.badRequest(
					res,
					"approvalId and action are required",
				);
			}
			...
			if (!["approved", "rejected", "requested_info"].includes(action)) {
				return apiResponse.badRequest(
					res,
					'action must be "approved", "rejected", or "requested_info"',
				);
			}
 ...
 */
router.get(
	"/admin/escrow/pending-approvals",
	requireAuth,
	requireAdmin,
	async (req: Request, res: Response) => {
		try {
			const user = req.user as any;
			const approvals = await escrowMakerCheckerService.getPendingApprovals(
				user.id,
			);
 ...
 */
router.get(
	"/admin/escrow/deal/:dealId/history",
	requireAuth,
	requireAdmin,
	async (req: Request, res: Response) => {
		try {
			const { dealId } = req.params;
			const history =
				await escrowMakerCheckerService.getDealApprovalHistory(dealId);
