...
async function resolveClientType(clientId: string): Promise<{
	...
	return {
		...
		getPortfolioCreateValues: (name: string, source?: string) =>
			isProspect
				? {
						prospectId: clientId,
...
router.get("/talking-points/:clientId", async (req, res) => {
	try {
		const { clientId } = req.params;
		const talkingPoints =
			await aiInvestmentService.generateTalkingPoints(clientId);
...
router.post("/proposal", async (req, res) => {
	try {
		...
		if (!clientId || !picks?.length) {
			return res
				.status(400)
				.json({ error: "Client ID and picks are required" });
		}
...
function generatePortfolioInsights(
	...
	if (allocationEntries.length > 0) {
		const sortedAllocation = allocationEntries.sort(
			(a, b) => b[1].percent - a[1].percent,
		);
		const topAsset = sortedAllocation[0];
		if (topAsset && topAsset[1].percent > 50) {
			insights.push(
				`**Concentration Alert:** ${formatAssetClass(topAsset[0])} represents ${topAsset[1].percent.toFixed(1)}% of your portfolio. Consider diversifying to reduce risk.\n`,
			);
		} else if (sortedAllocation.length === 1) {
			insights.push(
				`Your portfolio is entirely in ${formatAssetClass(topAsset[0])}. Consider diversifying across asset classes for better risk management.\n`,
			);
		} else {
			insights.push(
				`Your portfolio is distributed across ${sortedAllocation.length} asset classes, with ${formatAssetClass(topAsset[0])} being the largest at ${topAsset[1].percent.toFixed(1)}%.\n`,
			);
		}
	}
	...
	if (gainers.length > 0) {
		...
		insights.push(
			`**Top Performer:** ${topGainer.name} with ${topGainer.gainLossPercent.toFixed(2)}% gains.\n`,
		);
	}
	...
	if (losers.length > 0) {
		...
		insights.push(
			`**Underperformer:** ${topLoser.name} with ${topLoser.gainLossPercent.toFixed(2)}% loss. Consider reviewing this position.\n`,
		);
	}
	...
	insights.push("\n### Risk Assessment\n");
	const equityPercent =
		(assetAllocation?.equity?.percent || 0) +
		(assetAllocation?.unlisted?.percent || 0);
	const debtPercent =
		(assetAllocation?.bond?.percent || 0) +
		(assetAllocation?.mld?.percent || 0);
	const mfPercent = assetAllocation?.mutual_fund?.percent || 0;

	if (equityPercent > 70) {
		insights.push(
			`**High Equity Exposure (${equityPercent.toFixed(1)}%):** Your portfolio has aggressive risk profile. Consider adding debt instruments for stability during market corrections.\n`,
		);
	} else if (debtPercent > 70) {
		insights.push(
			`**Conservative Allocation (${debtPercent.toFixed(1)}% debt):** While stable, you may be missing growth opportunities. Consider adding equity exposure based on your risk tolerance.\n`,
		);
	} else {
		insights.push(
			`Your portfolio has a balanced mix with ${equityPercent.toFixed(1)}% equity and ${debtPercent.toFixed(1)}% debt.\n`,
		);
	}
	...
	if (uniqueAssetClasses < 3) {
		insights.push(
			`**Limited Diversification:** Only ${uniqueAssetClasses} asset class(es). Consider adding mutual funds, bonds, or ETFs to improve diversification.\n`,
		);
	} else if (holdingsCount < 5) {
		insights.push(
			`**Concentrated Holdings:** Only ${holdingsCount} securities. A well-diversified portfolio typically has 10-15 holdings across sectors.\n`,
		);
	} else {
		insights.push(
			`Good diversification with ${holdingsCount} holdings across ${uniqueAssetClasses} asset classes.\n`,
		);
	}
	...
	if (!assetAllocation?.bond && !assetAllocation?.mld) {
		recommendations.push(
			"Add fixed-income instruments (bonds/MLDs) for portfolio stability",
		);
	}
	if (equityPercent > 80) {
		recommendations.push(
			"Consider profit booking in equity positions with significant gains",
		);
	}
	if (losers.length > 3) {
		recommendations.push(
	...
	if (holdingsCount > 30) {
		recommendations.push(
			"Portfolio may be over-diversified; consider consolidating similar holdings",
		);
	}
	if (mfPercent > 50) {
		recommendations.push(
			"Review mutual fund overlap to avoid duplicate holdings across schemes",
		);
	}
	...
	if (recommendations.length > 0) {
		...
		insights.push(
			"Your portfolio appears well-balanced. Continue monitoring and rebalance quarterly.\n",
		);
	}
		...
		if (portfolio) {
			...
			if (holdings.length > 0) {
				const avgReturn =
					holdings.reduce(
						(sum, h) =>
							sum + Number.parseFloat(h.returnPercentage?.toString() || "0"),
						0,
					) / holdings.length;
				portfolioReturns = {
...
router.post(
	"/admin/migrate-prospect-portfolios",
...
router.post(
	"/admin/migrate-prospect-portfolio/:prospectId",
	requireRole("admin", "superadmin"),
	async (req, res) => {
		try {
			const { prospectId } = req.params;
			const result =
				await prospectPortfolioSyncService.migrateToCurrentPortfolio(
					prospectId,
				);
			res.json({
				success: true,
