...
import {
	...
	clearParseCache,
	TransactionLot,
} from "../services/portfolio-parser";
import {
	holdingLotsStorageService,
	LotStorageInput,
} from "../services/holding-lots-storage-service";
...
const normalizeAssetType = (type: string | undefined) =>
	holdingNormalizationService.normalizeAssetType(type);
...
const deriveAllocation = (holdings: any[]) => {
	...
	return holdingNormalizationService.deriveAllocationFromHoldings(
		unifiedHoldings,
	);
};
...
async function upsertProspectPortfolio(
	...
	const unifiedHoldings: UnifiedHolding[] = holdings.map((h: any) => ({
		...
		assetType: holdingNormalizationService.normalizeAssetType(
			h.assetType || h.type || h.productType,
		),
	...
	if (userId && result.portfolioId) {
		...
		for (const h of holdings) {
			if (
				h.transactionLots &&
				Array.isArray(h.transactionLots) &&
				h.transactionLots.length > 0
			) {
		...
		if (lotsToStore.length > 0) {
			const storageResult =
				await holdingLotsStorageService.insertLots(lotsToStore);
			console.log(
				`[Portfolio Import] Stored ${storageResult.inserted} transaction lots for portfolio ${result.portfolioId}`,
			);
			if (storageResult.errors.length > 0) {
				console.warn(
					"[Portfolio Import] Lot storage errors:",
					storageResult.errors.slice(0, 3),
				);
			}
		}
	}
	...
	async (req: Request, res: Response) => {
		try {
			...
			await db
				...
				.set({
					uploadedPortfolio: {
						...
						fileName: req.file.originalname,
						fileType: fileType,
						parsedHoldings: snapshot.holdings.map(
							(h: any) =>
								({
									name: h.name,
									quantity: h.quantity,
			...
			res.json({
				...
				confidenceScore: importResult.confidenceScore,
				needsReview:
					snapshot.parsingStatus === "needs_review" ||
					importResult.needsManualReview,
			...
			res
				.status(500)
				.json({ error: error.message || "Failed to upload portfolio" });
		}
	},
);
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (blockedDomains.some((d: any) => urlObj.hostname.includes(d))) {
				return res
					.status(400)
					.json({ error: "Invalid URL - local addresses not allowed" });
			}
			...
			const importResult =
				await unifiedPortfolioImportService.importFromURL(importUrl);
			...
			res
				.status(500)
				.json({
					error: error.message || "Failed to import portfolio from URL",
				});
		}
	},
);
...
router.get(
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (hasUploadedPortfolio || hasFetchedPortfolio) {
				...
				const holdings = hasUploadedPortfolio
					? uploaded.parsedHoldings.map((h: any, idx: number) => ({
							...
							currentValue: Number.parseFloat(h.value) || 0,
							units: Number.parseFloat(h.quantity) || 0,
				...
				portfolio = {
					...
					brokerDetected: hasUploadedPortfolio
						? uploaded.brokerDetected
						: fetched?.source,
					importedAt: hasUploadedPortfolio
						? uploaded.uploadedAt
						: fetched?.fetchedAt,
					confidenceScore: hasUploadedPortfolio
						? uploaded.confidenceScore
						: fetched?.confidenceScore,
					parsingStatus: hasUploadedPortfolio
						? uploaded.parsingStatus
						: fetched?.parsingStatus,
			...
			res
				.status(500)
				.json({ error: error.message || "Failed to get portfolio" });
		}
	},
);
...
router.delete(
	...
	async (req: Request, res: Response) => {
		try {
			...
			res
				.status(500)
				.json({ error: error.message || "Failed to delete portfolio" });
		}
	},
);
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			console.log(
				"[Portfolio Import] Using unified service for parse-cas:",
				req.file.originalname,
			);
			...
			const totalInvested = holdings.reduce(
				(sum: any, h: any) => sum + h.investedValue,
				0,
			);
			const fundsCount = holdings.length;
			const tierBreakdown = importResult.tierBreakdown || {
				FULL: fundsCount,
				VALUATION_ONLY: 0,
				SUMMARY_PLACEHOLDER: 0,
			};
			const lotCounts = importResult.lotCounts || {
				withLots: 0,
				withMultipleLots: 0,
				withoutLots: fundsCount,
			};
...
router.get(
	...
	async (req: Request<{ prospectId: string }>, res: Response) => {
		try {
			...
			const totalValue = holdings.reduce(
				(sum: any, h: any) => sum + (Number(h.currentValue) || 0),
				0,
			);
			const totalInvested = holdings.reduce(
				(sum: any, h: any) => sum + (Number(h.investedValue) || 0),
				0,
			);
			...
			holdings.forEach((h: any) => {
				...
				allocationMap[type] =
					(allocationMap[type] || 0) + (Number(h.currentValue) || 0);
			});
