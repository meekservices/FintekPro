...
router.get("/methodology", async (req, res) => {
	try {
		...
		const documentation =
			financialMetricsRefreshScheduler.getMethodologyDocumentation();
		res.json({
			success: true,
			documentation,
...
router.post("/refresh/mutual-funds", async (req, res) => {
	try {
		...
		console.log(
			`[Admin] Triggering mutual fund refresh (limit: ${limit}) by ${user?.email}`,
		);
...
router.post("/refresh/stocks", async (req, res) => {
	try {
		...
		console.log(
			`[Admin] Triggering stock metrics refresh (limit: ${limit}) by ${user?.email}`,
		);
...
router.post("/refresh/mutual-fund/:schemeCode", async (req, res) => {
	try {
		...
		console.log(
			`[Admin] Refreshing metrics for scheme ${schemeCode} by ${user?.email}`,
		);
		...
		// Calculate returns
		const returns =
			await mutualFundMetricsService.calculateReturnsForScheme(schemeCode);

		// Update database
		const success =
			await mutualFundMetricsService.updateSchemeReturns(schemeCode);
...
router.post("/refresh/stock/:symbol", async (req, res) => {
	try {
		...
		console.log(
			`[Admin] Refreshing metrics for stock ${symbol} by ${user?.email}`,
		);
...
router.post("/refresh/all", async (req, res) => {
	try {
		...
		const result =
			await financialMetricsRefreshScheduler.triggerImmediateRefresh();
