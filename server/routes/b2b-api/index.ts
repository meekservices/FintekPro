...
b2bAdvisoryRouter.post("/generate", async (req: B2BRequest, res: Response) => {
	const { user_profile, market_state, is_partner_override, partner_ria_id } =
		req.body;
	...
	if (!user_profile || !market_state) {
		return res
			.status(400)
			.json({
				error: "Missing required payload keys (user_profile, market_state).",
			});
	}

	const tenantId = req.b2bClient!.id;
	const governanceMode = req.b2bClient!.governanceMode as
		| "STRICT"
		| "DELEGATED";
...
b2bSimulationRouter.post("/run", async (req: B2BRequest, res: Response) => {
	return res
		...
		.json({
			error:
				"APSE Direct Gateway integration pending Phase 2 deployment pipeline.",
		});
});
