...
import {
	...
	logProposalEvent,
	trackProposalRecommendations,
} from "./prospect-proposals-helpers";

const router = Router();
router.post(
	"/api/agent/prospect-proposals",
		...
		try {
			...
			if (!prospectName || !proposalType || !proposalTitle) {
				return res
					.status(400)
					.json({
						error: "Prospect name, proposal type, and title are required",
					});
			}
			...
			if (proposalType === "sample_portfolio" && !samplePortfolio) {
				return res
					.status(400)
					.json({
						error: "Sample portfolio data is required for portfolio analysis",
					});
			}
			...
			if (proposalType === "fresh_investment" && !investmentGoals) {
				return res
					...
					.json({
						error:
							"Investment goals are required for fresh investment proposals",
					});
			}
				...
				const [proposal] = await tx
					...
					.values({
						...
						agentName: user.firstName
							? `${user.firstName} ${user.lastName || ""}`.trim()
							: user.email?.split("@")[0] || user.email || null,
			...
			if (
				...
				trackProposalRecommendations(
					...
					console.error("[AI Tracking] Background tracking failed:", err),
				);
			}
			...
			res
				.status(500)
				.json({ error: error.message || "Failed to create proposal" });
		}
	},
);
