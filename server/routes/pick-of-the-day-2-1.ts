		...
		switch (category) {
			case "listed_stocks": {
				const stockResults = await db
					...
					.limit(20);
				results.push(
					...stockResults.map((s) => ({ ...s, type: "listed_stocks" })),
				);
				break;
			}
			...
			case "global_stocks": {
				const globalResults = await db
					...
					.limit(20);
				results.push(
					...globalResults.map((g) => ({ ...g, type: "global_stocks" })),
				);
				break;
			}
			...
			case "unlisted": {
				const unlistedResults = await db
					.select({
					...
					.limit(20);
				results.push(
					...unlistedResults.map((u) => ({ ...u, type: "unlisted" })),
				);
				break;
			}

			case "reits_invits": {
				const reitResults = await db
					.select({
						id: instrumentMaster.id,
						name: instrumentMaster.name,
						symbol: instrumentMaster.symbol,
					...
					.limit(20);
				results.push(
					...reitResults.map((r) => ({ ...r, type: "reits_invits" })),
				);
				break;
			}

			case "fixed_deposits": {
				const fdResults = await db
					.select({
						id: instrumentMaster.id,
						name: instrumentMaster.name,
						symbol: instrumentMaster.symbol,
					...
					.limit(20);
				results.push(
					...fdResults.map((f) => ({ ...f, type: "fixed_deposits" })),
				);
				break;
			}
		}
...
router.get("/watchlist", requireAuth, async (req, res) => {
	try {
		...
		res
			.status(500)
			.json({ success: false, error: "Failed to fetch watchlist" });
	}
});
...
router.post("/watchlist/add", requireAuth, async (req, res) => {
	try {
		...
		const userId = (req as any).user.id;
		const { pickId, notes, priceAlertEnabled, alertThreshold, alertType } =
			parseResult.data;
		...
		const existing = await db
			...
			.from(pickWatchlist)
			.where(
				and(eq(pickWatchlist.userId, userId), eq(pickWatchlist.pickId, pickId)),
			);

		if (existing.length > 0) {
			return res
				.status(400)
				.json({ success: false, error: "Pick already in watchlist" });
		}

		const [item] = await db
			.insert(pickWatchlist)
			.values({
				userId,
				pickId,
				notes,
...
router.delete("/watchlist/:pickId", requireAuth, async (req, res) => {
	try {
		...
		if (deleted.length === 0) {
			return res
				.status(404)
				.json({ success: false, error: "Pick not found in watchlist" });
		}
...
router.get("/diversification", requireAuth, async (req, res) => {
	try {
		...
		const sectorDistribution: Record<
			string,
			{ count: number; picks: typeof livePicks }
		> = {};
		...
		for (const pick of livePicks) {
			...
			categoryDistribution[pick.category] =
				(categoryDistribution[pick.category] || 0) + 1;
			timeHorizonDistribution[pick.timeHorizon || "medium_term"] =
				(timeHorizonDistribution[pick.timeHorizon || "medium_term"] || 0) + 1;
			riskDistribution[pick.riskLevel || "medium"] =
				(riskDistribution[pick.riskLevel || "medium"] || 0) + 1;
		}
		...
		const diversificationScore =
			totalPicks > 0
				? Math.min(
						100,
						Object.keys(sectorDistribution).length * 15 +
							Object.keys(categoryDistribution).length * 10,
					)
				: 0;
		...
		res.json({
			...
			analysis: {
				...
				riskDistribution,
				recommendations: getDiversificationRecommendations(
					sectorDistribution,
					categoryDistribution,
				),
			},
		...
		res
			.status(500)
			.json({ success: false, error: "Failed to analyze diversification" });
	}
});
...
function getDiversificationRecommendations(
	...
	if (sectorCount < 5) {
		recommendations.push(
			"Consider adding picks from more diverse sectors to reduce concentration risk",
		);
	}
	if (categoryCount < 4) {
		recommendations.push(
			"Portfolio lacks asset class diversity - consider adding bonds, ETFs, or global stocks",
		);
	}
	...
	for (const [sector, data] of Object.entries(sectors)) {
		if (data.count > 3) {
			recommendations.push(
				`High concentration in ${sector} sector (${data.count} picks) - consider rebalancing`,
			);
		}
	}
	...
	if (!categories.bonds && !categories.sgb) {
		recommendations.push(
			"No fixed income picks - consider adding bonds or SGBs for stability",
		);
	}
