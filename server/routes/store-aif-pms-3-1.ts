...
router.get("/portfolio/admin/all", requireAdmin, async (req, res) => {
	try {
		...
		// Fetch AIF holdings
		let aifHoldings: Array<{
			holding: typeof clientPortfolioAif.$inferSelect;
		...
		if (!type || type === "aif") {
			...
			if (status)
				aifConditions.push(
					eq(clientPortfolioAif.entryStatus, status as string),
				);
		...
		// Fetch PMS holdings
		let pmsHoldings: Array<{
			holding: typeof clientPortfolioPms.$inferSelect;
			client: typeof users.$inferSelect | null;
		}> = [];
		if (!type || type === "pms") {
			...
			if (status)
				pmsConditions.push(
					eq(clientPortfolioPms.entryStatus, status as string),
				);
		...
		if (!type || type === "mld") {
			...
			if (status)
				mldConditions.push(
					eq(clientPortfolioMld.entryStatus, status as string),
				);
		...
		res.json({
			...
			mld: mldHoldings.map((h) => ({
				...
				totalInvested:
					h.holding.quantity && h.holding.purchasePrice
						? String(
								Number(h.holding.quantity) * Number(h.holding.purchasePrice),
							)
						: null,
				client: h.client
					? {
			...
			summary: {
				...
				pendingApproval:
					aifHoldings.filter((h) => h.holding.entryStatus === "pending")
						.length +
					pmsHoldings.filter((h) => h.holding.entryStatus === "pending")
						.length +
...
router.put(
	"/portfolio/admin/approve/:type/:id",
	requireAdmin,
	async (req, res) => {
		try {
			...
			if (!["approve", "reject", "needs_review"].includes(action)) {
				return res
					.status(400)
					.json({
						error: "Invalid action. Must be approve, reject, or needs_review",
					});
			}
			...
			if (type === "aif") {
				...
				res
					.status(400)
					.json({ error: "Invalid type. Must be aif, pms, or mld" });
			}
		} catch (error: any) {
...
router.get("/aif/sebi/preview", requireAdmin, async (req, res) => {
	try {
		...
		console.log(
			`[SEBI AIF Import] Preview: ${listings.length} total, ${newCount} new, ${duplicateCount} duplicates`,
		);
...
router.post("/aif/sebi/import", requireAdmin, async (req, res) => {
	try {
		...
		console.log(
			`[SEBI AIF Import] Starting import of ${listings.length} AIFs...`,
		);
		...
		for (const listing of listings) {
			...
			try {
				...
				console.error(
					`[SEBI AIF Import] Error importing ${listing.registrationNo}:`,
					itemError.message,
				);
		...
		console.log(
			`[SEBI AIF Import] Completed: ${imported.length} imported, ${skipped.length} skipped, ${errors.length} errors`,
		);
...
router.get("/aif/seed/preview", requireAdmin, async (req, res) => {
	try {
		...
		const byCategory = {
			...
			"Category II": listings.filter((l) => l.category === "Category II")
				.length,
			"Category III": listings.filter((l) => l.category === "Category III")
				.length,
		};

		console.log(
			`[AIF Seed] Preview: ${listings.length} total, ${newCount} new, ${duplicateCount} duplicates`,
		);
