...
export function registerBondsMarkPart3Routes(app: Express): void {
	app.get(
		"/api/bonds/filter/rating",
		requireLevel2,
		async (req: Request, res: Response): Promise<void> => {
			try {
				const ratings = req.query.ratings
					? (req.query.ratings as string).split(",")
					: ["AAA"];
	...
	app.get(
		"/api/bonds/filter/yield",
		requireLevel2,
		async (req: Request, res: Response): Promise<void> => {
			try {
				const minYield = req.query.min
					? Number.parseFloat(req.query.min as string)
					: 0;
				const maxYield = req.query.max
					? Number.parseFloat(req.query.max as string)
					: 15;
	...
	app.get(
		"/api/bonds/filter/maturity",
		requireLevel2,
		async (req: Request, res: Response): Promise<void> => {
			try {
				const params = {
					minYears: req.query.minYears
						? Number.parseInt(req.query.minYears as string)
						: undefined,
					maxYears: req.query.maxYears
						? Number.parseInt(req.query.maxYears as string)
						: undefined,
					exactYears: req.query.exactYears
						? Number.parseInt(req.query.exactYears as string)
						: undefined,
				};
	...
	app.get(
		"/api/bonds/infrastructure-bonds",
		requireLevel2,
		async (req: Request, res: Response): Promise<void> => {
			try {
	...
	app.get(
		...
		async (req: Request, res: Response): Promise<void> => {
			try {
				...
				console.warn(
					`[NSE] Historical auctions fetch failed: ${errorMessage(error)}`,
				);
				res.status(500).json({
					status: "error",
					error: errorMessage(error),
	...
	app.get(
		...
		async (req: Request, res: Response): Promise<void> => {
			try {
				...
				const [nseData, bseData, mcxData, ncdexData, mseiData]: any[] =
					await Promise.all([
						nseResponse.json(),
						bseResponse.json(),
						mcxResponse.json(),
				...
				if (category && category !== "all") {
					allBonds = allBonds.filter((bond) => {
						const bondCategory =
							bond.segment?.toLowerCase() || bond.type?.toLowerCase() || "";
					...
					allBonds.length > 0
						? {
								...
								totalVolume: allBonds.reduce((sum, bond) => {
									const volumeStr = bond.volume
										? bond.volume.replace(/[₹,\sCr]/g, "")
										: "0";
								...
								averageYield: (
									allBonds.reduce(
										(sum, bond) => sum + (bond.currentYield || 0),
										0,
									) / allBonds.length
								).toFixed(2),
								topGainer: allBonds.reduce(
									(max, bond) =>
										(bond.changePercent || 0) > (max.changePercent || 0)
											? bond
											: max,
									allBonds[0],
								),
								mostTraded: allBonds.reduce((max, bond) => {
									const v1Str = bond.volume
										? bond.volume.replace(/[₹,\sCr]/g, "")
										: "0";
									const v2Str = max.volume
										? max.volume.replace(/[₹,\sCr]/g, "")
										: "0";
									const volume1 = Number.parseFloat(v1Str);
									const volume2 = Number.parseFloat(v2Str);
									return (Number.isNaN(volume1) ? 0 : volume1) >
										(Number.isNaN(volume2) ? 0 : volume2)
										? bond
										: max;
								}, allBonds[0]),
								segmentDistribution: {
									Government: allBonds.filter((b) => b.segment === "Government")
										.length,
									Corporate: allBonds.filter((b) => b.segment === "Corporate")
										.length,
									Agricultural: allBonds.filter(
										(b) => b.segment === "Agricultural",
									).length,
									Commodity: allBonds.filter((b) => b.segment === "Commodity")
										.length,
									Technology: allBonds.filter((b) => b.segment === "Technology")
										.length,
									Healthcare: allBonds.filter((b) => b.segment === "Healthcare")
										.length,
								},
							}
						: {
