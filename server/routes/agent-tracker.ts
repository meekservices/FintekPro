...
function classifyAsset(
	assetType: string,
): "equity" | "debt" | "gold" | "alternatives" | "cash" {
	const t = (assetType || "").toLowerCase();
	if (t.includes("gold") || t.includes("silver")) return "gold";
	if (t.includes("debt") || t.includes("bond") || t.includes("fd"))
		return "debt";
	if (t.includes("alt") || t.includes("aif") || t.includes("pms"))
		return "alternatives";
	...
	async (req: Request, res: Response): Promise<void> => {
		try {
			...
			const holdings = await db
				...
				.where(inArray(comprehensiveHoldings.userId, clientIds))
				.limit(5000);
			...
			const amcMap = new Map<
				string,
				{ aum: number; sipCount: number; trail: number }
			>();
			const clientAumMap = new Map<string, number>();

			for (const h of holdings) {
				const mv = Number.parseFloat(String(h.marketValue || 0));
				if (!mv || mv <= 0) continue;
				totalAUM += mv;
			...
			const sipHoldings = holdings.filter(
				(h) =>
					h.contributionFrequency ||
					(h.metadata as unknown as SipMetadata)?.sipAmount,
			);
			...
			for (const h of sipHoldings) {
				...
				const sipAmount = Number.parseFloat(
					String(meta?.sipAmount || meta?.amount || 0),
				);
			...
			const monthlyTrend = await db
				...
				.where(
					and(
						...
						gte(
							comprehensiveHoldings.holdingDate,
			...
			const trendMap = new Map(
				monthlyTrend.map((t) => [t.month, Number(t.totalAUM)]),
			);
			...
			for (let i = 5; i >= 0; i--) {
				...
				trend.push({
					month: key,
					aum: trendMap.get(key) ?? (i === 0 ? totalAUM : 0),
					label,
				});
			}
			...
			const irisConnected = await db
				.select({ id: users.id })
				.from(users)
				.where(and(inArray(users.id, clientIds), isNotNull(users.panNumber)));
	...
	try {
		...
		if (!irisKfintechService.isConfigured) {
			res.status(503).json({
				...
				message:
					"Set IRIS_USERNAME and IRIS_PASSWORD to enable live data fetch.",
			});
			return;
		}
		...
		res
			.status(500)
			.json({ error: "Failed to initiate IRIS request", detail: msg });
	}
});
...
router.post("/iris/verify", requireAuth, async (req, res): Promise<void> => {
	try {
		...
		if (!verifyResult.success) {
			res
				.status(400)
				.json({ error: verifyResult.message || "OTP verification failed" });
			return;
		}
		...
		res
			.status(500)
			.json({ error: "IRIS OTP verification failed", detail: msg });
	}
});
