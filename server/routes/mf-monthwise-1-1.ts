...
export function registerMFMonthwiPart1Part1Routes(app: Express): void {
	app.get(
		"/api/mutual-funds/:schemeCode/monthwise-performance",
		async (req, res) => {
			try {
				const { schemeCode } = req.params;
				const months = Number.parseInt(req.query.months as string) || 24;

				console.log(
					`[MFMonthwise] Fetching monthwise performance for scheme ${schemeCode}`,
				);

				// Import service dynamically to avoid circular dependencies
				const { mfMonthwisePerformanceService } = await import(
					"../services/mf-monthwise-performance-service"
				);

				// First try to get cached data
				let performance =
					await mfMonthwisePerformanceService.getMonthwisePerformance(
						schemeCode,
						months,
					);
				...
				if (performance.length === 0) {
					console.log(
						`[MFMonthwise] No cached data, calculating for scheme ${schemeCode}`,
					);
					const calcResult =
						await mfMonthwisePerformanceService.calculateAndStoreMonthlyReturns(
							schemeCode,
						);
	...
	app.post(
		"/api/mutual-funds/:schemeCode/monthwise-performance/refresh",
		async (req, res) => {
			try {
				const { schemeCode } = req.params;
	...
	app.get("/api/iris/scheme/:schemeCode/details", async (req, res) => {
		try {
			...
			console.error(
				`Error fetching IRIS scheme details for ${req.params.schemeCode}:`,
				error,
			);
			res.status(500).json({ status: "error", error: error.message });
		}
	...
	app.post("/api/iris/lumpsum-calculator", async (req, res) => {
		try {
			...
			const maturityAmount = amount * (1 + expectedReturn / 100) ** years;
				...
				schemeCodes.map(async (code) => {
					try {
						...
						const oneYearReturn = oneYearAgo
							? (
									((latest.nav - oneYearAgo.nav) / oneYearAgo.nav) *
									100
								).toFixed(2)
							: "N/A";
	...
	app.post("/api/iris/goal-planner", async (req, res) => {
		try {
			...
			const futureValue = goalAmount * (1 + inflation / 100) ** timeHorizon;
			const currentSavingsValue = currentSavings || 0;
			const remainingAmount =
				futureValue -
				currentSavingsValue * (1 + expectedReturn / 100) ** timeHorizon;
