...
export function registerPaymentPart2Routes(app: Express): void {
  ...
  app.get('/api/admin/compliance/client-money-segregation', async (req, res) => {
    try {
      if (!req.user?.role || !['admin', 'compliance_officer'].includes(req.user.role)) {
        return res.status(403).json({ message: 'Forbidden - Admin or Compliance Officer access required' });
      }

      const complianceArtifacts = clientMoneySegregationService.generateComplianceArtifacts();
      ...
      console.error('Error fetching client money segregation compliance:', error);
  ...
  app.get('/api/admin/compliance/payment-flows', async (req, res) => {
    try {
      ...
      const complianceStatus = clientMoneySegregationService.getComplianceStatus();
  ...
  app.get('/api/admin/compliance/regulatory-disclosure', async (req, res) => {
    try {
      ...
      const disclosure = clientMoneySegregationService.generateRegulatoryDisclosure();
  ...
  app.get('/api/admin/compliance/reconciliation/:reportId', async (req, res) => {
    try {
      if (!req.user?.role || !['admin', 'compliance_officer'].includes(req.user.role)) {
        return res.status(403).json({ message: 'Forbidden - Admin or Compliance Officer access required' });
      }
