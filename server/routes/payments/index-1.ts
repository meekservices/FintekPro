...
export function registerPaymentPart1Routes(app: Express): void {
	...
	app.post("/api/payments/create-order", async (req, res) => {
		...
		try {
			...
			if (idempotencyKey) {
				const { locked, cached } =
					await unifiedPaymentGateway.acquireIdempotencyLock(
						idempotencyKey,
						user.id,
					);
			...
			if (!orderResponse.success) {
				...
				if (idempotencyKey && idempotencyLockHeld) {
					await unifiedPaymentGateway.releaseIdempotencyLock(
						idempotencyKey,
						user.id,
					);
				...
				return res
					.status(400)
					.json({ message: orderResponse.message || "Order creation failed" });
			}
			...
			if (
				...
				unifiedPaymentGateway
					.releaseIdempotencyLock(idempotencyKey, userId)
					.catch(() => {});
			}
			...
			if (statusResult) {
				await storage.updateCashfreeTransaction(transaction.id, {
					...
					cashfreeOrderId:
						...
						((transaction as any).cashfreeOrderId as any),
					paymentMethod: statusResult.paymentMethod,
					completedAt:
						statusResult.orderStatus === "PAID" ? new Date() : undefined,
				});
			}
	...
	app.post("/api/payments/cashfree/webhook", async (req, res) => {
		try {
			...
			if (!isValidSignature) {
				console.error(
					"⚠️ Cashfree webhook signature verification FAILED - potential spoofing attempt",
				);
				complianceMonitor.logEvent({
					eventType: "security",
	...
	app.post("/api/payments/phonepe/callback", async (req, res) => {
		try {
			...
			const isValidSignature = phonePeService.verifyCallback(
				base64Response,
				checksum,
			);
			if (!isValidSignature) {
				console.error(
					"⚠️ PhonePe callback signature verification FAILED - potential spoofing attempt",
				);
				complianceMonitor.logEvent({
					eventType: "security",
			...
			if (!merchantTransactionId) {
				return res
					.status(400)
					.json({ message: "Missing merchantTransactionId" });
			}
	...
	app.get("/api/cashfree/transactions", async (req, res) => {
		try {
			...
			const transactions = await storage.getCashfreeTransactionsByUserId(
				(req.user as any)!.id,
			);
	...
	app.get("/api/payments/phonepe/status/:orderId", async (req, res) => {
		try {
			...
			const status = await phonePeService.checkOrderStatus(
