...
import {
	matchDealToProspects,
	getGeoIntelligence,
	type DealType,
} from "../services/aif-pms-prospect-matcher";
import { db } from "../db";
...
router.post("/match-prospects", async (req: Request, res: Response) => {
	try {
		...
		if (!dealId || !dealType)
			return res
				.status(400)
				.json({ error: "dealId and dealType (aif|pms) are required" });
		if (!["aif", "pms"].includes(dealType))
			return res.status(400).json({ error: "dealType must be 'aif' or 'pms'" });
		const result = await matchDealToProspects(
			dealId,
	...
	try {
		const [aifs, pmss] = await Promise.all([
			db
				...
				.from(aifMaster)
				.orderBy(desc(aifMaster.createdAt))
				.limit(50),
			db
				.select({
					id: pmsMaster.id,
				...
				.from(pmsMaster)
				.orderBy(desc(pmsMaster.createdAt))
				.limit(50),
		]);
	...
	try {
		const [prospectStats] = await db
			.select({
				...
				hot: sql<number>`count(*) filter (where lead_quality = 'hot')`.mapWith(
					Number,
				),
				warm: sql<number>`count(*) filter (where lead_quality = 'warm')`.mapWith(
					Number,
				),
		...
		res.json({
			...
			prospects: {
				...
				avgCompositeScore: Number.parseFloat(
					String(prospectStats.avgComposite || "0"),
				).toFixed(1),
				totalInvestableRupees: Number.parseFloat(
					String(prospectStats.totalInvestable || "0"),
				),
				totalNetworthRupees: Number.parseFloat(
					String(prospectStats.totalNetworth || "0"),
				),
			},
			deals: {
