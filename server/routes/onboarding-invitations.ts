...
router.post(
	"/api/agent/onboarding-invitations",
	async (req: Request, res: Response) => {
		try {
			...
			if (!clientEmail && !clientMobile) {
				return res
					.status(400)
					.json({ error: "Either email or mobile is required" });
			}
			...
			res
				.status(500)
				.json({ error: error.message || "Failed to create invitation" });
		}
	},
);
...
router.get(
	...
	async (req: Request, res: Response) => {
		try {
			...
			res
				.status(500)
				.json({ error: error.message || "Failed to list invitations" });
		}
	},
);
...
router.post(
	"/api/agent/onboarding-invitations/:id/resend",
	async (req: Request, res: Response) => {
		try {
			const user = req.user as any;
			if (!user) {
			...
			await db
				...
				.set({
					status:
						invitation.status === "expired" ? "pending" : invitation.status,
			...
			res
				.status(500)
				.json({ error: error.message || "Failed to resend invitation" });
		}
	},
);
...
router.get(
	"/api/agent/onboarding-invitations/stats",
	async (req: Request, res: Response) => {
		try {
			const user = req.user as any;
			if (!user) {
...
router.post(
	"/api/partner/onboarding-invitations",
	async (req: Request, res: Response) => {
		try {
			...
			if (!partner) {
				return res
					.status(401)
					.json({ error: "Partner authentication required" });
			}
...
router.post(
	"/api/partner/onboarding-invitations/:id/resend",
	async (req: Request, res: Response) => {
		try {
			const partner = await getPartnerFromAuth(req);
			if (!partner) {
...
router.get(
	"/api/partner/onboarding-invitations/stats",
	async (req: Request, res: Response) => {
		try {
			const partner = await getPartnerFromAuth(req);
			if (!partner) {
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (
				...
				return res
					.status(400)
					.json({ error: "Referral link has expired", valid: false });
			}
			...
			res
				.status(500)
				.json({ error: error.message || "Failed to validate referral" });
		}
	},
);
...
router.patch(
	"/api/onboarding/referrals/:code/progress",
	async (req: Request, res: Response) => {
		try {
			const { code } = req.params;
			const {
			...
			if (onboardingSessionId)
				updates.onboardingSessionId = onboardingSessionId;
