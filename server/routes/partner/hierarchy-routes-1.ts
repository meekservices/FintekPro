...
export function registerPartnerHierarchyPart1Routes(app: Express) {
	...
	app.post(
		"/api/partner-hierarchy/partners/approve",
		requireAdmin,
		async (req: any, res) => {
			try {
				...
				const result = await partnerHierarchyService.approvePartner(
				...
				await partnerAuditService.logPartnerApproval(
					req.user.id,
					parsed.data.partnerId,
					req.ip,
				);
	...
	// Partner Rejection
	app.post(
		"/api/partner-hierarchy/partners/reject",
		requireAdmin,
		async (req: any, res) => {
			try {
				...
				const result = await partnerHierarchyService.rejectPartner(
					parsed.data.partnerId,
					req.user.id,
					parsed.data.reason,
				);
				...
				await partnerAuditService.logPartnerRejection(
					req.user.id,
					parsed.data.partnerId,
					parsed.data.reason,
					req.ip,
	...
	// KYC Status Update
	app.post(
		"/api/partner-hierarchy/partners/kyc",
		requireAdmin,
		async (req: any, res) => {
			try {
				...
				const result = await partnerHierarchyService.updateKycStatus(
					parsed.data.partnerId,
					parsed.data.status,
				);
				if (!result.success) {
				...
				await partnerAuditService.logKycUpdate(
					req.user.id,
					parsed.data.partnerId,
					oldStatus,
					parsed.data.status,
				...
				res.json({
					success: true,
					message: `KYC status updated to ${parsed.data.status}`,
				});
			} catch (error: any) {
	...
	app.post(
		...
		async (req: any, res) => {
			try {
				const result = await partnerHierarchyService.suspendPartner(
					req.params.partnerId,
				);
				if (!result.success) {
					return res.status(400).json({ error: result.error });
				}
				await partnerAuditService.logPartnerSuspension(
					req.user.id,
					req.params.partnerId,
					req.ip,
				);
	...
	app.post(
		...
		async (req: any, res) => {
			try {
				const result = await partnerHierarchyService.terminatePartner(
					req.params.partnerId,
				);
				if (!result.success) {
					return res.status(400).json({ error: result.error });
				}
				await partnerAuditService.logPartnerTermination(
					req.user.id,
					req.params.partnerId,
					req.ip,
				);
	...
	app.get(
		"/api/partner-hierarchy/partners/pending",
		requireAdmin,
		async (req: any, res) => {
			try {
	...
	// Get partner by ID
	app.get(
		"/api/partner-hierarchy/partners/:partnerId",
		requirePartnerOrAdmin,
		async (req: any, res) => {
			try {
				const partner = await partnerHierarchyService.getPartnerById(
	...
	app.get(
		"/api/partner-hierarchy/partners/:partnerId/downline",
		requirePartnerOrAdmin,
		async (req: any, res) => {
			try {
				const isAdmin =
				...
				const downline = await partnerHierarchyService.getDownline(
					req.params.partnerId,
					maskPII,
				);
				res.json(downline);
	...
	app.get(
		"/api/partner-hierarchy/partners/:partnerId/tree",
		requirePartnerOrAdmin,
		async (req: any, res) => {
			try {
				const tree = await partnerHierarchyService.getPartnerTree(
					req.params.partnerId,
				);
				res.json(tree);
			} catch (error: any) {
	...
	app.post(
		"/api/partner-hierarchy/client-ownership",
		requirePartnerOrAdmin,
		async (req: any, res) => {
			try {
				...
				const result = await partnerHierarchyService.assignClientOwnership(
					parsed.data.clientId,
					parsed.data.ownerPartnerId,
				);
				if (!result.success) {
					return res.status(400).json({ error: result.error });
				}
	...
	app.post(
		"/api/partner-hierarchy/client-ownership/override",
		requireAdmin,
		async (req: any, res) => {
			try {
				...
				const oldOwnership = await partnerHierarchyService.getClientOwnership(
					parsed.data.clientId,
				);
	...
	app.get(
		"/api/partner-hierarchy/client-ownership/:clientId",
		requirePartnerOrAdmin,
		async (req: any, res) => {
			try {
				const ownership = await partnerHierarchyService.getClientOwnership(
					req.params.clientId,
				);
				if (!ownership) {
	...
	app.post(
		"/api/partner-hierarchy/commission-rules",
		requireAdmin,
		async (req: any, res) => {
			try {
	...
	app.patch(
		"/api/partner-hierarchy/commission-rules/:ruleId",
		requireAdmin,
		async (req: any, res) => {
			try {
				const result = await commissionWaterfallEngine.updateCommissionRule(
					req.params.ruleId,
					req.body,
				);
				if (!result.success) {
				...
				await partnerAuditService.logCommissionOverride(
					req.user.id,
					req.params.ruleId,
					req.body,
					req.ip,
	...
	app.post(
		"/api/partner-hierarchy/commission/process",
		requireAdmin,
		async (req: any, res) => {
			try {
				...
				const antiMLM = commissionWaterfallEngine.validateAntiMLM(
				...
				const result = await commissionWaterfallEngine.processCommission(
					parsed.data,
				);
				if (!result.success) {
					return res.status(400).json({ error: result.error });
