...
export function registerPartnerHierarchyPart2Routes(app: Express) {
	...
	app.get(
		"/api/partner-hierarchy/wallet/:partnerId",
		requirePartnerOrAdmin,
		async (req: any, res) => {
			try {
				const wallet = await commissionWaterfallEngine.getWallet(
	...
	app.get(
		"/api/partner-hierarchy/ledger/:partnerId",
		requirePartnerOrAdmin,
		async (req: any, res) => {
			try {
				const limit = Number.parseInt(req.query.limit as string) || 50;
				const entries = await commissionWaterfallEngine.getLedgerEntries(
					req.params.partnerId,
					limit,
				);
				res.json(entries);
	...
	app.post(
		"/api/partner-hierarchy/wallet/:partnerId/payout",
		requireAdmin,
		async (req: any, res) => {
			try {
				...
				const partner = await partnerHierarchyService.getPartnerById(
					req.params.partnerId,
				);
				if (!partner) {
				...
				if (partner.kycStatus !== "VERIFIED") {
					return res
						.status(400)
						.json({ error: "Partner KYC must be verified before payout" });
				}

				const result = await commissionWaterfallEngine.debitWallet(
					req.params.partnerId,
					amount,
				);
				if (!result.success) {
				...
				await partnerAuditService.logManualPayout(
					req.user.id,
					req.params.partnerId,
					amount,
					req.ip,
	...
	app.get(
		"/api/partner-hierarchy/audit-logs",
		requireAdmin,
		async (req: any, res) => {
			try {
	...
	app.get(
		"/api/partner-hierarchy/audit-logs/:entityType/:entityId",
		requireAdmin,
		async (req: any, res) => {
			try {
				const trail = await partnerAuditService.getEntityAuditTrail(
	...
	app.post(
		"/api/partner-hierarchy/integrity-check",
		requireAdmin,
		async (req: any, res) => {
			try {
				const { detectCycles, detectOrphans, detectDepthViolations } =
					await import("../../services/hierarchy-integrity-validator");
	...
	app.post(
		"/api/partner-hierarchy/payout-config",
		requireAdmin,
		async (req: any, res) => {
			try {
	...
	app.get(
		"/api/partner-hierarchy/payout-config/:productType",
		requirePartnerOrAdmin,
		async (req: any, res) => {
			try {
				const config = await commissionPayoutService.getConfigByProduct(
					req.params.productType,
				);
				if (!config) {
					return res
						.status(404)
						.json({ error: "No active config for this product type" });
				}
				res.json(config);
			} catch (error: any) {
	...
	app.post(
		"/api/partner-hierarchy/payout/process",
		requireAdmin,
		async (req: any, res) => {
			try {
				...
				const result = await commissionPayoutService.processTransaction(
					parsed.data,
				);
				if (!result.success) {
					return res.status(400).json({ error: result.error });
	...
	app.get(
		"/api/partner-hierarchy/payout/ledger/transaction/:transactionId",
		requirePartnerOrAdmin,
		async (req: any, res) => {
			try {
				const entries = await commissionPayoutService.getLedgerByTransaction(
	...
	app.get(
		"/api/partner-hierarchy/payout/ledger/partner/:partnerId",
		requirePartnerOrAdmin,
		async (req: any, res) => {
			try {
				const limit = Number.parseInt(req.query.limit as string) || 50;
				const entries = await commissionPayoutService.getLedgerByPartner(
					req.params.partnerId,
					limit,
				);
				res.json(entries);
				...
				res
					.status(500)
					.json({ error: "Failed to fetch partner ledger entries" });
			}
		},
	);
	...
	app.get(
		"/api/partner-hierarchy/payout-statement/:partnerId",
		requirePartnerOrAdmin,
		async (req: any, res) => {
			try {
				const { from_date, to_date, group_by } = req.query;
				const statement = await partnerStatementService.getPayoutStatement(
					req.params.partnerId,
					{
						fromDate: from_date as string,
						toDate: to_date as string,
						groupBy:
							(group_by as "transaction" | "day" | "month") || "transaction",
					},
				);
		...
		async (req: any, res) => {
			try {
				...
				if (req.query.transaction_id)
					filters.transactionId = req.query.transaction_id;
				if (req.query.limit)
					filters.limit = Number.parseInt(req.query.limit as string);
	...
	app.get(
		"/api/commission/dispute/:disputeId",
		requirePartnerOrAdmin,
		async (req: any, res) => {
			try {
				const dispute = await commissionDisputeService.getDisputeById(
	...
	app.patch(
		...
		async (req: any, res) => {
			try {
				...
				res.json({
					success: true,
					message: `Dispute status updated to ${parsed.data.status}`,
				});
			} catch (error: any) {
	...
	app.get(
		"/api/commission/reversals/:transactionId",
		requirePartnerOrAdmin,
		async (req: any, res) => {
			try {
				const reversals = await commissionDisputeService.getReversals(
					req.params.transactionId,
				);
				res.json(reversals);
			} catch (error: any) {
