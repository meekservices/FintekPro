...
export function registerPartnerPortalPart1Routes(app: Express): void {
	...
	const requirePartner = async (req: any, res: any, next: any) => {
		...
		if (!authHeader) {
			return res
				.status(401)
				.json({ message: "Partner authentication required" });
		}
	...
	app.put("/api/partner/products/:id", requirePartner, async (req, res) => {
		try {
			...
			updates.partnerId = undefined; // Prevent changing partner
			updates.id = undefined; // Prevent changing ID

			const updatedProduct = await partnerService.updateProduct(
				req.params.id,
				updates,
			);
	...
	app.get(
		"/api/partner/support/tickets/:id",
		requirePartner,
		async (req, res) => {
			try {
	...
	app.put(
		...
		async (req, res) => {
			try {
				...
				const updatedTicket = await partnerService.updateTicket(
					req.params.id,
					updates,
				);
	...
	app.post(
		"/api/partner/support/tickets/:id/messages",
		requirePartner,
		async (req, res) => {
			try {
				const ticket = await partnerService.getTicket(req.params.id);
	...
	app.post("/api/contact/submit", async (req, res) => {
		try {
			const { fullName, email, phone, company, inquiryType, subject, message } =
				req.body;
			...
			const ticket =
				await partnerService.createSupportTicket(contactTicketData);

			// Log the contact form submission for analytics
			console.log(
				`[CONTACT] New contact form submission: ${inquiryType} from ${email}`,
			);
	...
	app.post("/api/partner/register", async (req: any, res) => {
      ...
      `);
        ...
        `);
			...
			console.log(
				`[PartnerRegister] New partner application: ${email} (${partnerType}) — user ${userId}`,
			);
			res.json({
				success: true,
