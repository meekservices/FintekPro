...
export function registerAdminMiscRoutes(app: Express): void {
	...
	app.post("/api/admin/whatsapp/init", requireAdmin, async (_req, res) => {
		try {
			...
			res.json({
				success: true,
				message:
					"WhatsApp initialization started — visit /api/admin/whatsapp/qr to scan",
			});
		} catch (err: any) {
	...
	app.get(
		"/api/admin/settings/otp-priority",
		requireAdmin,
		async (_req, res) => {
			try {
				...
				const channels = (setting?.value as string[] | null) || [
					"email",
					"whatsapp",
					"sms",
				];
		...
		async (req, res) => {
			try {
				...
				if (
					...
					channels.length === 0 ||
					!channels.every((c) => valid.includes(c))
				) {
					return res
						.status(400)
						.json({
							success: false,
							error: "channels must be a non-empty array of email/whatsapp/sms",
						});
				}
				...
				if (unique.length !== 3) {
					return res
						...
						.json({
							success: false,
							error:
								"All three channels (email, whatsapp, sms) must be present exactly once",
						});
				}
				const { adminSettings } = await import("@shared/schema");
				...
				if (existing) {
					...
					await db.insert(adminSettings).values({
						...
						value: unique,
						description:
							"Global OTP delivery channel priority order (email/whatsapp/sms)",
						updatedBy: (req as any).user?.id,
					});
				}
	...
	const requireAgent = async (req: any, res: any, next: any) => {
		...
		if (!hasRole(req.user, ["agent", "partner", "admin", "superadmin"])) {
			return res
				.status(403)
				.json({ message: "Agent, partner, or admin access required" });
		}
	...
	app.post("/api/admin/migrate-user-id", requireAdmin, async (req, res) => {
		try {
			...
			if (!email || !newUserId) {
				return res
					.status(400)
					.json({ success: false, error: "email and newUserId are required" });
			}
			...
			res.json({
				success: true,
				message: `Updated user ${email} to ID ${newUserId}`,
			});
		} catch (err: any) {
			res.status(500).json({ success: false, error: err.message });
		}
