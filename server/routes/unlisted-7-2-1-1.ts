 ...
 */
router.post(
	"/admin/refresh-company-data/:companyId",
	requireAdmin,
	async (req: Request, res: Response) => {
		try {
			...
			try {
				const mcResult =
					await priceAggregationService.refreshMoneyControlPrice(companyId);
				results.push({
					source: "moneycontrol",
					status: mcResult ? "success" : "no_data",
				});
			} catch (mcError: any) {
				results.push({
					source: "moneycontrol",
			...
			if (companyData.cin && mcaService.isConfigured()) {
				try {
					console.log(
						`[Refresh] Fetching MCA data for CIN: ${companyData.cin}`,
					);
					...
					if (mcaData) {
						...
						await storage.updateUnlistedCompany(companyId, {
							...
							totalShares: companyUpdate.totalShares
								? Number.parseInt(companyUpdate.totalShares)
								: companyData.totalShares,
							description:
								companyUpdate.registeredAddress || companyData.description,
						...
						if (metrics.debtEquity !== undefined) {
							...
							try {
								...
								console.log(
									"[Refresh] Could not store MCA ratios:",
									ratioError,
								);
							}
						}
			...
			try {
				...
				if (!credhiveId && companyData.name) {
					console.log(
						`[Refresh] No CredHive ID, searching for: ${companyData.name}`,
					);
					const searchResults = await credhiveService.searchCompanyByNameOrCIN(
						companyData.name.substring(0, 50),
					);
					...
					if (searchResults.length > 0) {
						...
						console.log(
							`[Refresh] Found and linking CredHive company: ${credhiveId}`,
						);
						...
						if (searchResults[0].cin && !companyData.cin) {
							...
							console.log(
								`[Refresh] Auto-populated CIN: ${searchResults[0].cin}`,
							);
						}
				...
				if (credhiveId) {
					...
					if (companyDetails) {
						...
						if (companyDetails.cin && !companyData.cin) {
							...
							console.log(
								`[Refresh] Auto-populated CIN from CredHive details: ${companyDetails.cin}`,
							);
						}
						// Add capital info if available
						if (companyDetails.paid_up_capital) {
							updateData.paidUpCapital =
								companyDetails.paid_up_capital.toString();
						}
						if (companyDetails.authorized_capital) {
							updateData.authorizedCapital =
								companyDetails.authorized_capital.toString();
						}
						...
						if (
							...
							console.log(
								`[Refresh] Got ${companyDetails.directors.length} directors from CredHive`,
							);
						}
					}
					...
					for (const fin of financials) {
						try {
							const existingFin = await storage.getCompanyFinancialsByYear(
								companyId,
								fin.financial_year,
							);
							const finData = {
							...
							console.log(
								`[Refresh] Could not save financial for ${fin.financial_year}:`,
								finError,
							);
						}
					}
					console.log(
						`[Refresh] Saved ${financialsSaved} financials from CredHive`,
					);
					...
					for (const ratio of ratios) {
						try {
							const existingRatio = await storage.getCompanyRatiosByYear(
								companyId,
								ratio.financial_year,
							);
							const ratioData = {
								companyId,
							...
							console.log(
								`[Refresh] Could not save ratio for ${ratio.financial_year}:`,
								ratioError,
							);
						}
					}
					...
					results.push({
						source: "credhive",
						status: "skipped",
						error: "Could not find matching company in CredHive",
					});
				}
			} catch (p42Error: any) {
				results.push({
			...
			try {
				...
				const enriched = await dataEnrichmentService.enrichCompanyFinancials(
					companyId,
					fy,
					{
						forceRefresh: true,
				...
				if (enriched.sources.length > 0) {
					...
					const blockReason = enriched.auditTrail.find(
						(a: any) => a.action === "block",
					)?.reason;
					const bypassReason = enriched.auditTrail.find(
						(a: any) => a.action === "bypass",
					)?.reason;
					results.push({
						source: "data_enrichment",
						status: "no_data",
						data: {
							reason: blockReason || bypassReason || "No metrics fetched",
						},
					});
				}
			...
			return apiResponse.serverError(
				res,
				error.message || "Failed to refresh company data",
			);
		}
	},
);
	...
	async (req: Request, res: Response) => {
		try {
			...
			const needsSectorEnrich =
				!(companyData as any).sector ||
				(companyData as any).sector.toLowerCase().includes("unknown");
			const needsIndustryEnrich =
				!(companyData as any).industry ||
				(companyData as any).industry.toLowerCase().includes("unknown");
			...
			if (!currentCIN && companyData.name) {
				try {
					console.log(
						`[Auto-Enrich] No CIN available, searching CredHive by company name: ${companyData.name}`,
					);
					const credhiveResults =
						await credhiveService.searchCompanyByNameOrCIN(companyData.name);

					if (credhiveResults && credhiveResults.length > 0) {
						// Find best match by name similarity
						const searchNameLower = companyData.name
							.toLowerCase()
							.replace(/\s+/g, " ")
							.trim();
						...
						for (const result of credhiveResults) {
							const resultNameLower = (result as any).name
								.toLowerCase()
								.replace(/\s+/g, " ")
								.trim();
							...
							for (const word of searchWords) {
								if (
									resultWords.some(
										(rw: any) => rw.includes(word) || word.includes(rw),
									)
								) {
									matchScore++;
								}
							...
							const score =
								matchScore / Math.max(searchWords.length, resultWords.length);
							if (score > bestScore) {
								bestScore = score;
								bestMatch = result;
							}
						...
						if (bestMatch.cin && bestScore >= 0.4) {
							...
							console.log(
								`[Auto-Enrich] Found CIN ${bestMatch.cin} for "${companyData.name}" via CredHive (matched: "${bestMatch.name}", score: ${(bestScore * 100).toFixed(1)}%)`,
							);
							...
							if (
								bestMatch.name &&
								bestMatch.name.toLowerCase() !== companyData.name.toLowerCase()
							) {
								await storage.updateUnlistedCompany(companyId, {
									name: bestMatch.name,
								});
							...
							console.log(
								`[Auto-Enrich] CredHive search found ${credhiveResults.length} results but no confident match (best score: ${(bestScore * 100).toFixed(1)}%)`,
							);
						}
					} else {
						console.log(
							`[Auto-Enrich] Could not find CIN for "${companyData.name}" in CredHive search`,
						);
					}
				} catch (cinError: any) {
					console.error(
						"[Auto-Enrich] Error searching CredHive for CIN:",
						cinError.message,
					);
				}
			}
			...
			if (currentCIN && mcaService.isConfigured()) {
				try {
					...
					if (mcaData) {
						...
						console.log(
							"[Auto-Enrich] MCA raw data:",
							JSON.stringify(mcaData, null, 2),
						);
						console.log(
							"[Auto-Enrich] MCA company data:",
							JSON.stringify(mcaCompanyData, null, 2),
						);
						console.log(
							"[Auto-Enrich] Current company name:",
							companyData.name,
						);
						...
						if (mcaCompanyData.name) {
							...
							console.log(
								"[Auto-Enrich] Normalized MCA name:",
								normalizedMcaName,
							);
							console.log(
								"[Auto-Enrich] Normalized current name:",
								normalizedCurrentName,
							);
							console.log(
								"[Auto-Enrich] Names equal (case-insensitive)?",
								normalizedMcaName.toLowerCase() ===
									normalizedCurrentName.toLowerCase(),
							);
							console.log(
								"[Auto-Enrich] Names equal (exact)?",
							...
							if (
								...
								console.log(
									"[Auto-Enrich] Name will be updated to:",
									normalizedMcaName,
								);
							} else {
								console.log(
									"[Auto-Enrich] Name update skipped - names are identical",
								);
							}
						} else {
						...
						if (
							mcaCompanyData.authorizedCapital &&
						...
						if (
							mcaCompanyData.incorporationDate &&
						...
						if (Object.keys(updateData).length > 1) {
							...
							console.log(
								`[Auto-Enrich] Updated ${enrichedFields.length} fields from MCA for ${companyData.name}`,
							);
						}
					}
				} catch (mcaError: any) {
			...
			const stillNeedsSector =
				enrichedFields.every((f: any) => f.field !== "sector") &&
				needsSectorEnrich;
			const stillNeedsIndustry =
				enrichedFields.every((f: any) => f.field !== "industry") &&
				needsIndustryEnrich;

			if (
				(stillNeedsSector || stillNeedsIndustry) &&
				companyData.probe42CompanyId
			) {
				try {
					console.log(
						`[Auto-Enrich] Fetching CredHive data for ID: ${companyData.probe42CompanyId}`,
					);
					const credhiveDetails = await credhiveService.getCompanyDetails(
						companyData.probe42CompanyId,
					);
					...
					if (credhiveDetails) {
						...
						if (Object.keys(updateData).length > 1) {
							...
							if (
								enrichmentSource.includes("MCA") &&
								!enrichmentSource.includes("CredHive")
							) {
							...
							console.log(
								`[Auto-Enrich] Updated ${enrichedFields.length} fields from CredHive for ${companyData.name}`,
							);
						}
					}
				} catch (p42Error: any) {
			...
			const finalNeedsSector =
				enrichedFields.every((f: any) => f.field !== "sector") &&
				needsSectorEnrich;
			const finalNeedsIndustry =
				enrichedFields.every((f: any) => f.field !== "industry") &&
				needsIndustryEnrich;

			if ((finalNeedsSector || finalNeedsIndustry) && currentCIN) {
				try {
					const { classifyIndustryFromCIN } = await import(
						"../utils/nic-industry-classifier"
					);
					const nicClassification = classifyIndustryFromCIN(currentCIN);

					if (nicClassification) {
						console.log(
							`[Auto-Enrich] Using NIC-based classification for CIN ${currentCIN}: ${nicClassification.sector}/${nicClassification.industry}`,
						);
						const updateData: any = { lastSyncedAt: new Date() };
						...
						if (Object.keys(updateData).length > 1) {
							...
							console.log(
								`[Auto-Enrich] Updated ${enrichedFields.filter((f: any) => f.source.includes("NIC")).length} fields from NIC classification for ${companyData.name}`,
							);
						}
					}
				} catch (nicError: any) {
					console.error(
						"[Auto-Enrich] NIC classification error:",
						nicError.message,
					);
				}
			}
			...
			return apiResponse.serverError(
				res,
				error.message || "Failed to auto-enrich company data",
			);
		}
	},
);
