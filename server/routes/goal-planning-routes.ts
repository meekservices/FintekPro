...
import {
	...
	insertGoalInvestmentLinkSchema,
	GOAL_CATEGORIES,
} from "@shared/schema";
import { z } from "zod";
...
router.post("/calculate-sip", async (req: Request, res: Response) => {
	try {
		...
		const result = goalPlanningEngine.calculateSIPForGoal(
			...
			Number.parseFloat(currentSavings || "0"),
			Number.parseInt(monthsRemaining),
			Number.parseFloat(expectedReturnRate || "12"),
			Number.parseFloat(inflationRate || "6"),
		);
...
router.post("/calculate-lumpsum", async (req: Request, res: Response) => {
	try {
		const { targetAmount, yearsRemaining, expectedReturnRate, inflationRate } =
			req.body;

		const result = goalPlanningEngine.calculateLumpsumForGoal(
			Number.parseFloat(targetAmount),
			Number.parseFloat(yearsRemaining),
			Number.parseFloat(expectedReturnRate || "12"),
			Number.parseFloat(inflationRate || "6"),
		);
...
router.post("/calculate-step-up-sip", async (req: Request, res: Response) => {
	try {
		...
		const result = goalPlanningEngine.calculateSipWithStepUp(
			...
			Number.parseFloat(expectedReturnRate || "12"),
			Number.parseFloat(inflationRate || "6"),
			Number.parseFloat(annualStepUpPercent || "10"),
		);
...
router.get("/:goalId", async (req: Request, res: Response) => {
	try {
		const details = await goalPlanningEngine.getGoalWithDetails(
			req.params.goalId,
		);
		if (!details) {
...
router.patch("/:goalId", async (req: Request, res: Response) => {
	try {
		...
		const updated = await goalPlanningEngine.updateGoalProgress(
			req.params.goalId,
		);
		res.json(updated);
	} catch (error: any) {
