...
import { returnForecastingEngine, AssetReturns, CashFlow } from '../services/return-forecasting-engine';
import { z } from 'zod';
...
const assetReturnsSchema = z.object({
  assetId: z.string(),
  assetType: z.enum(['equity', 'mutual_fund', 'bond', 'fd', 'etf', 'real_estate', 'gold', 'alternative']),
  assetName: z.string(),
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        absoluteReturn: ((input.endingValue - input.beginningValue) / input.beginningValue) * 100
      }
    });
  } catch (error) {
...
router.post('/asset-metrics', async (req, res) => {
  try {
    ...
    const returnMetrics = returnForecastingEngine.calculateReturnMetrics(asset as AssetReturns);
    const yieldMetrics = returnForecastingEngine.calculateYieldMetrics(asset as AssetReturns);
    const riskAdjusted = returnForecastingEngine.calculateRiskAdjustedReturns(asset as AssetReturns);
...
router.post('/stress-test', async (req, res) => {
  try {
    const asset = assetReturnsSchema.parse(req.body);
    const stressTests = returnForecastingEngine.runStressTests(asset as AssetReturns);
    ...
    res.json({
      ...
      data: {
        ...
        stressTests,
        worstCase: stressTests.reduce((min, s) => s.projectedReturn < min.projectedReturn ? s : min),
        bestCase: stressTests.reduce((max, s) => s.projectedReturn > max.projectedReturn ? s : max)
      }
    });
  } catch (error) {
...
router.post('/drawdown', async (req, res) => {
  try {
    const { historicalReturns } = z.object({
      historicalReturns: z.array(z.number())
    }).parse(req.body);
    
    const drawdownMetrics = returnForecastingEngine.calculateDrawdownMetrics(historicalReturns);
...
router.post('/portfolio', async (req, res) => {
  try {
    ...
    const assetMetrics = input.assets.map(asset => ({
      ...
      metrics: returnForecastingEngine.calculateReturnMetrics(asset as AssetReturns)
    }));
...
router.get('/asset-classes', async (_req, res) => {
  try {
    const assetClasses = [
      { type: 'equity', name: 'Equity/Stocks', expectedReturn: 12.0, volatility: 18.0, riskLevel: 'high' },
      { type: 'mutual_fund', name: 'Mutual Funds', expectedReturn: 10.0, volatility: 14.0, riskLevel: 'medium-high' },
      { type: 'bond', name: 'Bonds', expectedReturn: 7.0, volatility: 4.0, riskLevel: 'low' },
      { type: 'fd', name: 'Fixed Deposits', expectedReturn: 6.5, volatility: 0.5, riskLevel: 'very-low' },
      { type: 'etf', name: 'ETFs', expectedReturn: 11.0, volatility: 16.0, riskLevel: 'medium-high' },
      { type: 'real_estate', name: 'Real Estate', expectedReturn: 8.0, volatility: 10.0, riskLevel: 'medium' },
      { type: 'gold', name: 'Gold', expectedReturn: 6.0, volatility: 12.0, riskLevel: 'medium' },
      { type: 'alternative', name: 'Alternatives', expectedReturn: 15.0, volatility: 25.0, riskLevel: 'very-high' }
    ];
    ...
    res.status(500).json({ success: false, error: 'Failed to fetch asset classes' });
  }
});
...
router.get('/stress-scenarios', async (_req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Failed to fetch stress scenarios' });
  }
});
