...
export function registerSystemAdminRoutes(app: Express): void {
  ...
  app.get('/api/admin/system/external-api-health', requireAdmin, (_req, res) => {
    try {
      ...
      const summary = {
        ...
        healthy: Object.values(health).filter((s) => s.state === 'CLOSED').length,
        degraded: Object.values(health).filter((s) => s.state === 'HALF_OPEN').length,
        down: Object.values(health).filter((s) => s.state === 'OPEN').length,
      };

      res.json({ success: true, summary, services: health });
    } catch (err) {
      logger.error('[SystemAdmin] Failed to get external API health', { error: String(err) });
    ...
    try {
      ...
      const audit = tableRows.map((t) => {
        ...
        return {
          ...
          bloatWarning: Number(stats.n_dead_tup) > Number(stats.n_live_tup) * 0.2,
        };
      });
      ...
      logger.error('[SystemAdmin] Failed to run DB audit', { error: String(err) });
  ...
  app.get('/api/admin/system/vcip-expiry', requireAdmin, async (req, res) => {
    try {
      ...
      logger.error('[SystemAdmin] V-CIP expiry check failed', { error: String(err) });
      res.status(500).json({ message: 'Failed to check V-CIP expiry' });
    }
  });

  // Re-KYC trigger for expired V-CIP users
  app.post('/api/admin/system/vcip-expiry/notify', requireAdmin, async (req, res) => {
    try {
      ...
      logger.error('[SystemAdmin] V-CIP notification failed', { error: String(err) });
