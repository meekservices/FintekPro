 ...
 */
function resolveClientId(req: Request): { clientId: string | null; forbidden: boolean } {
  ...
  if (role === 'admin' || role === 'agent') return { clientId, forbidden: false };
 ...
 */
router.get('/:clientId/portfolio/summary', requireAuth, async (req: Request, res: Response) => {
  ...
  try {
    ...
    const preferCapabilities = capabilitiesParam
      ? capabilitiesParam.split(',').map(c => c.trim()).filter(Boolean) as any[]
      : undefined;

    const result = await portfolioAggregator.getUnifiedPortfolio(clientId!, undefined, undefined, preferCapabilities);
    ...
    const sources = result.brokerResults.filter(b => b.success).map(b => b.brokerId);
    const responseData = {
      ...result.summary,
    ...
    logger.error(`[ClientPortfolioAPI] summary error`, { event: 'PORTFOLIO_SUMMARY_ERROR', user_id: clientId, message: e.message });
    return res.status(500).json(err(e.message ?? 'Failed to fetch portfolio summary'));
  }
});
 ...
 */
router.get('/:clientId/portfolio/holdings', requireAuth, async (req: Request, res: Response) => {
  const start = Date.now();
  const { clientId, forbidden } = resolveClientId(req);
  if (forbidden) return res.status(403).json(err('Access denied', 'FORBIDDEN'));
  ...
  try {
    ...
    const assetClassFilter = req.query.assetClass ? (req.query.assetClass as string).split(',') : null;
    ...
    if (assetClassFilter) {
      filtered = filtered.filter(h => assetClassFilter.some(ac => h.assetClass?.toLowerCase().includes(ac.toLowerCase()) || h.assetType?.toLowerCase().includes(ac.toLowerCase())));
    }
    if (sourceFilter) {
      filtered = filtered.filter(h => sourceFilter.some(s => (h as any).broker?.toLowerCase().includes(s.toLowerCase()) || (h as any).dataSource?.toLowerCase().includes(s.toLowerCase())));
    }
    ...
    const totalInvested = filtered.reduce((s, h) => s + (h.investedValue ?? h.currentValue ?? 0), 0);
    ...
    logger.error(`[ClientPortfolioAPI] holdings error`, { event: 'PORTFOLIO_HOLDINGS_ERROR', user_id: clientId, message: e.message });
 ...
 *   ?page=1&limit=50
 */
router.get('/:clientId/portfolio/transactions', requireAuth, async (req: Request, res: Response) => {
  const start = Date.now();
  const { clientId, forbidden } = resolveClientId(req);
  if (forbidden) return res.status(403).json(err('Access denied', 'FORBIDDEN'));
  ...
  try {
    ...
    const typeFilter = req.query.type ? (req.query.type as string).split(',') : null;
    const sourceFilter = req.query.source ? (req.query.source as string).split(',') : null;

    // Build query conditions
    const conditions: any[] = [eq(portfolioTransactions.clientId, clientId!)];
    if (fromDate) conditions.push(gte(portfolioTransactions.tradeDate, new Date(fromDate)));
    if (toDate) conditions.push(lte(portfolioTransactions.tradeDate, new Date(toDate)));
    ...
    if (typeFilter) filtered = filtered.filter(t => typeFilter.includes(t.transactionType ?? ''));
    if (sourceFilter) filtered = filtered.filter(t => sourceFilter.includes(t.source ?? ''));
    ...
    logger.error(`[ClientPortfolioAPI] transactions error`, { event: 'PORTFOLIO_TRANSACTIONS_ERROR', user_id: clientId, message: e.message });
    return res.status(500).json(err(e.message ?? 'Failed to fetch transactions'));
  }
});
 ...
 */
router.get('/:clientId/portfolio/asset-allocation', requireAuth, async (req: Request, res: Response) => {
  const start = Date.now();
  const { clientId, forbidden } = resolveClientId(req);
  if (forbidden) return res.status(403).json(err('Access denied', 'FORBIDDEN'));
  ...
  try {
    ...
    const allocation = Object.entries(weights).map(([assetClass, pct]) => ({
      assetClass,
    ...
    logger.error(`[ClientPortfolioAPI] allocation error`, { event: 'PORTFOLIO_ALLOCATION_ERROR', user_id: clientId, message: e.message });
    return res.status(500).json(err(e.message ?? 'Failed to fetch asset allocation'));
  }
});
 ...
 */
router.get('/:clientId/portfolio/performance', requireAuth, async (req: Request, res: Response) => {
  const start = Date.now();
  const { clientId, forbidden } = resolveClientId(req);
  if (forbidden) return res.status(403).json(err('Access denied', 'FORBIDDEN'));
  ...
  try {
    ...
    const byAssetClass: Record<string, { value: number; invested: number; count: number }> = {};
    for (const h of holdings) {
      const key = h.assetClass ?? h.assetType ?? 'OTHER';
      if (!byAssetClass[key]) byAssetClass[key] = { value: 0, invested: 0, count: 0 };
      byAssetClass[key].value += h.currentValue ?? 0;
      byAssetClass[key].invested += h.investedValue ?? h.currentValue ?? 0;
      byAssetClass[key].count++;
    }

    const assetClassPerformance = Object.entries(byAssetClass).map(([assetClass, d]) => ({
      assetClass,
      label: ASSET_CLASS_LABELS[assetClass] ?? assetClass,
      ...
      returnPct: d.invested > 0 ? Math.round(((d.value - d.invested) / d.invested) * 10000) / 100 : 0,
    ...
    return res.json(ok({
      ...
      disclaimer: 'Returns are calculated on a simple cost-vs-market-value basis. XIRR requires complete transaction history.',
      calculatedAt: new Date().toISOString(),
    }, ['comprehensiveHoldings', 'alpaca']));
  } catch (e: any) {
    logger.error(`[ClientPortfolioAPI] performance error`, { event: 'PORTFOLIO_PERFORMANCE_ERROR', user_id: clientId, message: e.message });
    return res.status(500).json(err(e.message ?? 'Failed to fetch performance'));
  }
});
 ...
 */
router.get('/:clientId/portfolio/tax', requireAuth, async (req: Request, res: Response) => {
  const start = Date.now();
  const { clientId, forbidden } = resolveClientId(req);
  if (forbidden) return res.status(403).json(err('Access denied', 'FORBIDDEN'));
  ...
  try {
    ...
    for (const h of holdings) {
      ...
      const purchaseDateRaw = (h as any).purchaseDate ?? (h as any).navDate ?? null;
      if (!purchaseDateRaw) {
        unclassified.push(h);
        continue;
      }
    ...
    const calcGain = (arr: typeof holdings) => arr.reduce((s, h) => s + ((h.currentValue ?? 0) - (h.investedValue ?? h.currentValue ?? 0)), 0);
    ...
    return res.json(ok({
      stcg: {
        ...
        taxRateNote: 'STCG on equity: 20% (post July 2024 budget), on debt/MF: as per income slab',
      },
      ltcg: {
        holdingCount: ltcgHoldings.length,
        estimatedGainInr: Math.round(ltcgGain * 100) / 100,
        taxRateNote: 'LTCG on equity >₹1.25L: 12.5% (post July 2024 budget)',
        exemptionLimitNote: '₹1,25,000 exemption on LTCG from equity and equity MF',
      },
      unclassified: {
      ...
      disclaimer: 'This is an estimate only. Actual tax liability depends on your income slab, grandfathering rules, indexation, and corporate actions. Consult your CA before filing.',
      calculatedAt: new Date().toISOString(),
    }, ['comprehensiveHoldings']));
  } catch (e: any) {
    logger.error(`[ClientPortfolioAPI] tax error`, { event: 'PORTFOLIO_TAX_ERROR', user_id: clientId, message: e.message });
    return res.status(500).json(err(e.message ?? 'Failed to fetch tax summary'));
  }
});
