 ...
 */
router.get(
	"/admin/reconciliation/moneycontrol",
	requireAdmin,
	async (req: Request, res: Response) => {
		try {
			...
			return apiResponse.serverError(
				res,
				error.message || "Failed to fetch reconciliation data",
			);
		}
	},
);
 ...
 */
router.post(
	"/admin/reconciliation/refresh",
	requireAdmin,
	async (req: Request, res: Response) => {
		try {
			const result =
				await moneyControlReconciliation.getReconciliationSuggestions(true);
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			const synced =
				await moneyControlReconciliation.syncCompanyFromMoneyControl(
					{
						...validatedCompany,
						scrapedAt: validatedCompany.scrapedAt
							? new Date(validatedCompany.scrapedAt)
							: new Date(),
					},
					(req.user as any)?.id,
				);
			...
			return apiResponse.serverError(
				res,
				error.message || "Failed to sync company",
			);
		}
	},
);
 ...
 */
router.post(
	"/admin/reconciliation/sync-batch",
	requireAdmin,
	async (req: Request, res: Response) => {
		try {
				...
				if (result.success) {
					validatedCompanies.push({
						...
						scrapedAt: result.data.scrapedAt
							? new Date(result.data.scrapedAt)
							: new Date(),
					});
				} else {
			...
			if (validatedCompanies.length === 0) {
				return apiResponse.badRequest(
					res,
					"No valid companies to sync",
					validationErrors,
				);
			}

			const result = await moneyControlReconciliation.syncMultipleCompanies(
				validatedCompanies,
				req.user?.id,
			);
			...
			return apiResponse.serverError(
				res,
				error.message || "Failed to batch sync companies",
			);
		}
	},
);
