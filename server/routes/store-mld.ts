...
router.get("/mld", async (req, res) => {
  try {
    const { issuer, underlying, payoffType, status, search, limit = "50", offset = "0" } = req.query;
    
    const conditions = [eq(mldMaster.isPublished, true)];
    
    if (issuer) conditions.push(eq(mldMaster.issuer, issuer as string));
    if (underlying) conditions.push(eq(mldMaster.underlying, underlying as string));
    if (payoffType) conditions.push(eq(mldMaster.payoffType, payoffType as string));
...
router.post("/admin/mld", requireAdmin, async (req, res) => {
  try {
    ...
    if (!mldData.isin || !mldData.name || !mldData.issuer || !mldData.underlying || !mldData.payoffType || !mldData.maturityDate) {
      return res.status(400).json({ 
        error: "Missing required fields: isin, name, issuer, underlying, payoffType, maturityDate" 
      });
    }
    ...
    if (error.code === "23505") {
      return res.status(400).json({ error: "MLD with this ISIN already exists" });
    }
