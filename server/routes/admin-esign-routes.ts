...
import {
	unifiedESignService,
	ESignProvider,
} from "../services/unified-esign-service";
...
router.get(
	"/api/admin/esign/active-provider",
	requireAuth,
	requireRole("admin"),
	async (req: Request, res: Response) => {
		try {
...
router.post(
	"/api/admin/esign/update-pricing",
	requireAuth,
	requireRole("admin"),
	async (req: Request, res: Response) => {
		try {
...
router.get(
	"/api/admin/esign/cheapest-provider",
	requireAuth,
	requireRole("admin"),
	async (req: Request, res: Response) => {
		try {
			const cheapestProvider =
				await unifiedESignService.getCheapestConfiguredProvider();
...
router.get(
	"/api/admin/esign/provider/:provider",
	requireAuth,
	requireRole("admin"),
	async (req: Request, res: Response) => {
		try {
			const { provider } = req.params;
			const config = unifiedESignService.getProviderConfig(
				provider as ESignProvider,
			);
