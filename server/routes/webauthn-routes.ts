...
router.post("/register/options", requireAuth, async (req, res) => {
	try {
		...
		if (!checkRateLimit(`reg:${userId}`, 5, 60_000)) {
			return res
				.status(429)
				.json({ error: "Too many registration attempts. Please wait." });
		}
		...
		res
			.status(500)
			.json({
				error: err.message || "Failed to generate registration options",
			});
	}
});
...
router.post("/register/verify", requireAuth, async (req, res) => {
	try {
		...
		res
			.status(400)
			.json({ error: err.message || "Registration verification failed" });
	}
});
...
router.post("/authenticate/options", requireAuth, async (req, res) => {
	try {
		...
		if (!checkRateLimit(`auth:${userId}:${ip}`, 10, 60_000)) {
			return res
				.status(429)
				.json({ error: "Too many authentication attempts. Please try later." });
		}
...
router.post("/authenticate/verify", requireAuth, async (req, res) => {
	try {
		...
		res.json({
			...
			risk: result.risk,
			stepUpRequired:
				result.risk.stepUpRequired !== "none"
					? result.risk.stepUpRequired
					: null,
		});
	} catch (err: any) {
		...
		res
			.status(400)
			.json({ error: err.message || "Authentication verification failed" });
	}
});
...
router.patch("/credentials/:id", requireAuth, async (req, res) => {
	try {
		...
		const updated = await webauthnService.renameCredential(
			req.user!.id,
			req.params.id,
			deviceName.trim(),
		);
