...
router.get("/my-documents", async (req: Request, res: Response) => {
	try {
		...
		const participantRecords = await db
			.select({
				...
				preferredSignatureMethod:
					proposalEsignParticipants.preferredSignatureMethod,
			})
		...
		const workflows = await db
			...
			.where(
				sql`${proposalEsignWorkflows.id} IN (${sql.join(
		...
		const documents = workflows.map((workflow) => {
			...
			const isPending =
				participant?.actionStatus === "pending" ||
				participant?.actionStatus === "waiting";
			const isCompleted =
			...
			let clientStatus: "pending" | "completed" | "declined" | "expired" =
				"pending";
			if (isDeclined) {
			...
			return {
				...
				createdByName: workflow.createdBy
					? creatorMap[workflow.createdBy]
					: "System",
		...
		const pendingCount = documents.filter(
			(d) => d.clientStatus === "pending",
		).length;
		const completedCount = documents.filter(
			(d) => d.clientStatus === "completed",
		).length;
