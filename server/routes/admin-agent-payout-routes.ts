...
router.get("/payout-claims", async (req: Request, res: Response) => {
	try {
		...
		if (fromDate) {
			conditions.push(
				gte(agentPayoutClaims.createdAt, new Date(fromDate as string)),
			);
		}
		if (toDate) {
			conditions.push(
				lte(agentPayoutClaims.createdAt, new Date(toDate as string)),
			);
		}
		...
		const claims = await db
			...
			.leftJoin(
				dsaLoanApplications,
				eq(agentPayoutClaims.applicationId, dsaLoanApplications.id),
			)
			.where(whereClause)
		...
		res.json({
			...
			data: {
				...
				agentName: claim.agent
					? `${claim.agent.firstName || ""} ${claim.agent.lastName || ""}`.trim()
					: "Unknown",
		...
		try {
			...
			const [updated] = await db
				...
				.set({
					...
					rejectionReason:
						parsed.action === "reject" ? parsed.rejectionReason : undefined,
					updatedAt: new Date(),
				})
				.where(eq(agentPayoutClaims.id, req.params.id))
				.returning();
	...
	try {
		...
		res.json({
			...
			data: {
				...
				underReview: {
					count: Number(underReview?.count || 0),
					amount: Number(underReview?.total || 0),
				},
				approved: {
					count: Number(approved?.count || 0),
					amount: Number(approved?.total || 0),
				},
				paid: {
					count: Number(paid?.count || 0),
					amount: Number(paid?.total || 0),
				},
				rejected: {
					count: Number(rejected?.count || 0),
					amount: Number(rejected?.total || 0),
				},
			},
		});
	} catch (error: any) {
...
router.get("/agent-loan-actions", async (req: Request, res: Response) => {
	try {
		...
		if (applicationId) {
			conditions.push(
				eq(agentLoanActions.applicationId, applicationId as string),
			);
		}
		if (actionType) {
