...
router.get("/listed-stocks", async (req: Request, res: Response) => {
	try {
		const stocks = await db
			.select()
			.from(listedStocks)
			.orderBy(listedStocks.symbol);
		res.json(stocks);
	} catch (error: any) {
		console.error("Error fetching listed stocks:", error);
		res
			.status(500)
			.json({ error: error.message || "Failed to fetch listed stocks" });
	}
});
...
router.post("/listed-stocks", async (req: Request, res: Response) => {
	try {
		...
		if (!symbol || !companyName) {
			return res
				.status(400)
				.json({ error: "Symbol and company name are required" });
		}
		...
		if (existingStock.length > 0) {
			return res
				.status(400)
				.json({ error: "Stock with this symbol already exists" });
		}
		...
		res
			.status(500)
			.json({ error: error.message || "Failed to add listed stock" });
	}
});

router.patch(
	"/listed-stocks/:id/publish",
	async (req: Request, res: Response) => {
		try {
			...
			res
				.status(500)
				.json({ error: error.message || "Failed to update listed stock" });
		}
	},
);

router.patch(
	"/listed-stocks/bulk-publish",
	async (req: Request, res: Response) => {
		try {
			...
			res
				.status(500)
				.json({
					error: error.message || "Failed to bulk update listed stocks",
				});
		}
	},
);
...
router.post(
	"/listed-stocks/enrich-dividends",
	async (req: Request, res: Response) => {
		try {
			const { limit = 100 } = req.body;

			const stocksToEnrich = await db
				.select({
					id: listedStocks.id,
			...
			if (stocksToEnrich.length === 0) {
				return res.json({
					success: true,
					message: "All stocks already have dividend yield data",
					updated: 0,
					total: 0,
				});
			}
			...
			for (let i = 0; i < stocksToEnrich.length; i += batchSize) {
				...
				for (let j = 0; j < batch.length; j++) {
					...
					const yieldValue =
						yfYield != null ? (yfYield * 100).toFixed(4) : "0.0000";
			...
			res
				.status(500)
				.json({ error: error.message || "Failed to enrich dividend yields" });
		}
	},
);
...
router.delete("/listed-stocks/:id", async (req: Request, res: Response) => {
	try {
		...
		res
			.status(500)
			.json({ error: error.message || "Failed to delete listed stock" });
	}
});
