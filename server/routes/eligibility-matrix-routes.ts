...
import {
	...
	ApplicantProfile,
} from "../services/eligibility-matrix-service";
import {
...
router.get("/rules", async (req: Request, res: Response) => {
	try {
		...
		let rules;
		if (productType) {
			rules = await eligibilityMatrixService.getRulesByProduct(
...
router.post("/routing", async (req: Request, res: Response) => {
	try {
		...
		const recommendation =
			await eligibilityMatrixService.getRoutingRecommendation(
				applicant as ApplicantProfile,
				productType,
			);
...
reconciliationRouter.get("/summary", async (req: Request, res: Response) => {
	try {
		const summary =
			await commissionReconciliationService.getReconciliationSummary();
		res.json({
			success: true,
			data: summary,
		});
...
reconciliationRouter.get(
	...
	async (req: Request, res: Response) => {
		try {
			const payments =
				await commissionReconciliationService.getUnmatchedPayments();
			res.json({
				success: true,
				data: payments,
				count: payments.length,
...
reconciliationRouter.get(
	...
	async (req: Request, res: Response) => {
		try {
			const payments =
				await commissionReconciliationService.getDisputedPayments();
			res.json({
				success: true,
				data: payments,
				count: payments.length,
...
reconciliationRouter.get("/overdue", async (req: Request, res: Response) => {
	try {
		...
		const daysOverdue = days ? Number.parseInt(days as string) : 30;
		const overdue =
			await commissionReconciliationService.getOverdueCommissions(daysOverdue);
		res.json({
			success: true,
			data: overdue,
...
reconciliationRouter.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			const result =
				await commissionReconciliationService.processPaymentStatement(
...
reconciliationRouter.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			await commissionReconciliationService.manualMatch(
				paymentId,
				commissionLedgerId,
				userId,
			);
...
reconciliationRouter.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			await commissionReconciliationService.resolveDispute(
				paymentId,
				resolution,
				finalStatus,
			);
