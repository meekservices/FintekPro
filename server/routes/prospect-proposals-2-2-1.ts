...
router.patch(
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (prospectMobile !== undefined)
				updateData.prospectMobile = prospectMobile;
			...
			if (currentAnalysis !== undefined)
				updateData.currentAnalysis = currentAnalysis;
			if (recommendations !== undefined)
				updateData.recommendations = recommendations;
			if (totalInvestmentAmount !== undefined)
				updateData.totalInvestmentAmount = totalInvestmentAmount?.toString();
			if (projectedReturns !== undefined)
				updateData.projectedReturns = projectedReturns?.toString();
			if (projectedValue !== undefined)
				updateData.projectedValue = projectedValue?.toString();
			if (targetAllocation !== undefined)
				updateData.targetAllocation = targetAllocation;
			if (samplePortfolio !== undefined)
				updateData.samplePortfolio = samplePortfolio;
			...
			res
				.status(500)
				.json({ error: error.message || "Failed to update proposal" });
		}
	},
);
