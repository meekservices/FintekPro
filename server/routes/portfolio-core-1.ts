...
export function registerPortfolioCorPart1Routes(app: Express): void {
  app.get("/api/portfolios", requireAuth, async (req: Request, res: Response): Promise<void> => {
    try {
  ...
  app.get("/api/portfolios/by-pan", requireAuth, async (req: Request, res: Response): Promise<void> => {
    try {
      const userId = (req as AuthRequest).user?.id;
      if (!userId) {
        res.status(401).json({ error: "Authentication required" });
      ...
      const portfoliosResult = await storage.getPortfoliosByUserPan(user.panNumber);
  ...
  app.get("/api/wealth-management/analysis", requireAuth, async (req: Request, res: Response): Promise<void> => {
    try {
      const userId = (req as AuthRequest).user?.id;
      if (!userId) {
        res.status(401).json({ error: "Authentication required" });
      ...
      console.error("Error fetching financial analysis:", errorMessage(error));
  ...
  app.get("/api/financial-obligations", requireAuth, async (req: Request, res: Response): Promise<void> => {
    try {
      const userId = (req as AuthRequest).user?.id;
      if (!userId) {
        res.status(401).json({ error: "Authentication required" });
      ...
      console.error("Error fetching financial obligations:", errorMessage(error));
      res.status(500).json({ error: "Failed to fetch financial obligations" });
    }
  });

  app.post("/api/financial-obligations", requireAuth, async (req: Request, res: Response): Promise<void> => {
    try {
      const userId = (req as AuthRequest).user?.id;
      if (!userId) {
        res.status(401).json({ error: "Authentication required" });
      ...
      console.error("Error creating financial obligation:", errorMessage(error));
      res.status(500).json({ error: "Failed to create financial obligation" });
    }
  });

  app.patch("/api/financial-obligations/:id", requireAuth, async (req: Request, res: Response): Promise<void> => {
    try {
      const { id } = req.params;
      const userId = (req as AuthRequest).user?.id;
      if (!userId) {
      ...
      console.error("Error updating financial obligation:", errorMessage(error));
      res.status(500).json({ error: "Failed to update financial obligation" });
    }
  });

  app.delete("/api/financial-obligations/:id", requireAuth, async (req: Request, res: Response): Promise<void> => {
    try {
      const { id } = req.params;
      const userId = (req as AuthRequest).user?.id;
      if (!userId) {
      ...
      console.error("Error deleting financial obligation:", errorMessage(error));
      res.status(500).json({ error: "Failed to delete financial obligation" });
    }
  });

  // Government Scheme Holdings endpoints
  app.get("/api/government-schemes/epf", requireAuth, async (req: Request, res: Response): Promise<void> => {
    try {
      const userId = (req as AuthRequest).user?.id;
      if (!userId) {
        res.status(401).json({ error: "Authentication required" });
  ...
  app.get("/api/government-schemes/ppf", requireAuth, async (req: Request, res: Response): Promise<void> => {
    try {
      const userId = (req as AuthRequest).user?.id;
      if (!userId) {
        res.status(401).json({ error: "Authentication required" });
  ...
  app.get("/api/government-schemes/eps", requireAuth, async (req: Request, res: Response): Promise<void> => {
    try {
      const userId = (req as AuthRequest).user?.id;
      if (!userId) {
        res.status(401).json({ error: "Authentication required" });
  ...
  app.get("/api/government-schemes/nps", requireAuth, async (_req: Request, res: Response): Promise<void> => {
    try {
      res.json({
        holdings: [],
        totalValue: 0,
  ...
  app.get("/api/government-schemes/apy", requireAuth, async (_req: Request, res: Response): Promise<void> => {
    try {
      res.json({
        holdings: [],
        totalValue: 0,
  ...
  app.get("/api/government-schemes/consent/:panNumber/:schemeType", requireAuth, async (req: Request, res: Response): Promise<void> => {
    try {
      const userId = (req as AuthRequest).user?.id;
      if (!userId) {
        res.status(401).json({ error: "Authentication required" });
  ...
  app.post("/api/government-schemes/consent", requireAuth, async (req: Request, res: Response): Promise<void> => {
    try {
      const userId = (req as AuthRequest).user?.id;
      if (!userId) {
        res.status(401).json({ error: "Authentication required" });
      ...
      const { panNumber, schemeType, purpose } = req.body as { panNumber: string; schemeType: string; purpose?: string };
  ...
  app.get("/api/government-schemes/consents", requireAuth, async (req: Request, res: Response): Promise<void> => {
    try {
      const userId = (req as AuthRequest).user?.id;
      if (!userId) {
        res.status(401).json({ error: "Authentication required" });
  ...
  app.delete("/api/government-schemes/consent/:panNumber/:schemeType", requireAuth, async (req: Request, res: Response): Promise<void> => {
    try {
      const userId = (req as AuthRequest).user?.id;
      if (!userId) {
        res.status(401).json({ error: "Authentication required" });
  ...
  app.get("/api/tasks/user", requireAuth, async (req: Request, res: Response): Promise<void> => {
    try {
      const userId = (req as AuthRequest).user?.id;
      if (!userId) {
        res.status(401).json({ error: "Authentication required" });
  ...
  app.post("/api/tasks/user", requireAuth, async (req: Request, res: Response): Promise<void> => {
    try {
      const userId = (req as AuthRequest).user?.id;
      if (!userId) {
        res.status(401).json({ error: "Authentication required" });
      ...
      const { title, description, type, priority, dueDate, actionLabel, actionRoute, metadata } = req.body as {
        title?: string;
        description?: string;
        type?: string;
        priority?: string;
      ...
      if (!title || !type || !dueDate) {
        res.status(400).json({ error: "Missing required fields: title, type, dueDate" });
        return;
      }
      const taskData = {
        userId,
  ...
  app.patch("/api/tasks/user/:taskId", requireAuth, async (req: Request, res: Response): Promise<void> => {
    try {
      const userId = (req as AuthRequest).user?.id;
      if (!userId) {
        res.status(401).json({ error: "Authentication required" });
  ...
  app.delete("/api/tasks/user/:taskId", requireAuth, async (req: Request, res: Response): Promise<void> => {
    try {
      const userId = (req as AuthRequest).user?.id;
      if (!userId) {
        res.status(401).json({ error: "Authentication required" });
  ...
  app.get("/api/loans/applications", requireAuth, async (req: Request, res: Response): Promise<void> => {
    try {
      const userId = (req as AuthRequest).user?.id;
      if (!userId) {
        res.status(401).json({ error: "Authentication required" });
