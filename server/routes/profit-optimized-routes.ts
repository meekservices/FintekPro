...
export function registerProfitOptimizedRoutes(app: Express) {
	app.post(
		"/api/recommendations/scored-basket",
		async (req: Request, res: Response) => {
			try {
				...
				const balancedProducts =
					mode !== RECOMMENDATION_MODE.BALANCED
						? profitOptimizedScoringEngine.scoreProducts(
								allProducts,
								clientProfile,
				...
				if (mode === RECOMMENDATION_MODE.GROWTH_OPTIMIZED) {
					comparison = profitOptimizedScoringEngine.compareWithBalanced(
						scoredProducts,
						balancedProducts,
					);
				}
				...
				const response = {
					...
					disclosures:
						mode === RECOMMENDATION_MODE.GROWTH_OPTIMIZED
							? {
	...
	app.get("/api/recommendations/modes", async (req: Request, res: Response) => {
		try {
			const killSwitchStatus =
				profitOptimizedScoringEngine.getKillSwitchStatus();
			...
			const modes = [
				{
					...
					name: "Conservative",
					description:
						"Emphasizes capital preservation with minimal risk exposure",
				...
				},
				{
					...
					name: "Balanced",
					description:
						"Balances suitability with growth potential for steady returns",
					weightings: { suitability: 0.7, upside: 0.3 },
					available: true,
					isDefault: true,
				},
				{
					...
					description:
						"Emphasizes growth opportunities within your risk profile",
					weightings: { suitability: 0.55, upside: 0.45 },
					available: !killSwitchStatus.active,
					disabledReason: killSwitchStatus.active
	...
	app.get(
		"/api/recommendations/overrides/:clientId",
		requireAuth,
		async (req: Request, res: Response) => {
			try {
				const { clientId } = req.params;
				const overrides =
					profitOptimizedScoringEngine.getOverridesForClient(clientId);
	...
	app.get(
		"/api/admin/recommendations/audit-logs",
		requireAuth,
		requireRole(["admin"]),
		async (req: Request, res: Response) => {
			try {
				...
				const logs = profitOptimizedScoringEngine.getAuditLogs(
				...
				const stats = {
					...
					byMode: {
						...
						growth_optimized: logs.filter((l) => l.mode === "growth_optimized")
							.length,
					},
					avgProductsRecommended:
						logs.length > 0
							? logs.reduce((sum, l) => sum + l.productsRecommended, 0) /
								logs.length
							: 0,
			...
			try {
				...
				if (action === "activate") {
					if (!reason) {
						return res
							.status(400)
							.json({ error: "Reason is required to activate kill switch" });
					}
					...
					return res
						.status(400)
						.json({ error: "Action must be 'activate' or 'deactivate'" });
				}
			} catch (error: any) {
	...
	app.get(
		...
		async (req: Request, res: Response) => {
			try {
				...
				console.error(
					"[PROFIT-OPTIMIZED] Get kill switch status error:",
					error,
				);
	...
	app.get(
		"/api/recommendations/score-breakdown/:productType",
		async (req: Request, res: Response) => {
			try {
				...
				const methodologies: Record<
					string,
					{ description: string; factors: { name: string; weight: number }[] }
				> = {
					STOCK: {
						description:
							"Stocks are scored based on return potential, price momentum, valuation metrics, and sector outlook",
						factors: [
							{ name: "Return Potential", weight: 0.3 },
							{ name: "Momentum Score", weight: 0.25 },
							{ name: "Valuation Score", weight: 0.25 },
							{ name: "Sector Score", weight: 0.2 },
						],
					},
					MF: {
						description:
							"Mutual funds are scored based on return potential, alpha generation, consistency, and ratings",
						factors: [
					...
					BOND: {
						description:
							"Bonds are scored based on yield, credit quality, duration risk, and spread opportunities",
						factors: [
							{ name: "Yield Score", weight: 0.4 },
							{ name: "Credit Score", weight: 0.25 },
							{ name: "Duration Score", weight: 0.2 },
							{ name: "Spread Score", weight: 0.15 },
						],
					},
					REIT: {
						description:
							"REITs/InvITs are scored based on dividend yield, NAV discount, occupancy, and distribution growth",
						factors: [
							{ name: "Yield Score", weight: 0.35 },
							{ name: "NAV Score", weight: 0.25 },
							{ name: "Occupancy Score", weight: 0.2 },
							{ name: "Growth Score", weight: 0.2 },
						],
					},
					INVIT: {
						description:
					...
					IPO: {
						description:
							"IPOs are scored based on subscription levels, valuation, sector momentum, and issuer quality",
						factors: [
							{ name: "Subscription Score", weight: 0.3 },
							{ name: "Valuation Score", weight: 0.3 },
							{ name: "Sector Score", weight: 0.2 },
							{ name: "Issuer Quality", weight: 0.2 },
						],
					},
					UNLISTED: {
						description:
							"Unlisted equities are scored based on IPO proximity, company growth, valuation, and liquidity premium",
						factors: [
							{ name: "IPO Proximity", weight: 0.3 },
							{ name: "Growth Score", weight: 0.3 },
							{ name: "Valuation Score", weight: 0.25 },
					...
					AIF: {
						description:
							"AIF/PMS are scored based on track record, strategy, alpha generation, and risk-adjusted returns",
						factors: [
							{ name: "Track Record", weight: 0.3 },
							{ name: "Strategy Score", weight: 0.25 },
							{ name: "Alpha Score", weight: 0.25 },
							{ name: "Risk-Adjusted Return", weight: 0.2 },
						],
					},
					PMS: {
						description:
					...
					MLD: {
						description:
							"MLDs are scored similar to bonds based on yield, credit quality, duration, and spread",
						factors: [
							{ name: "Yield Score", weight: 0.4 },
							{ name: "Credit Score", weight: 0.25 },
							{ name: "Duration Score", weight: 0.2 },
		...
		async (req: Request, res: Response) => {
			try {
				...
				for (const product of products as ScoredProduct[]) {
					const explanation =
						optimisticExplanationTemplates.generateExplanation(product, mode);
	...
	app.post("/api/ab-testing/assign", async (req: Request, res: Response) => {
		try {
			...
			if (!clientId || !clientProfile) {
				return res
					.status(400)
					.json({ error: "clientId and clientProfile are required" });
			}
			...
			res.json({
				...
				assignment,
				message: assignment
					? `Client assigned to Group ${assignment.group}`
					: "Client not eligible for experiment",
			});
		} catch (error: any) {
	...
	app.get(
		"/api/ab-testing/assignment/:clientId",
		async (req: Request, res: Response) => {
			try {
				const { clientId } = req.params;
		...
		async (req: Request, res: Response) => {
			try {
				...
				if (!clientId || !metricType || value === undefined) {
					return res
						.status(400)
						.json({ error: "clientId, metricType, and value are required" });
				}
	...
	app.get(
		"/api/admin/ab-testing/metrics",
		requireAuth,
		requireRole(["admin"]),
		async (req: Request, res: Response) => {
			try {
				const { experimentId } = req.query;
				const metrics = abTestingService.getExperimentMetrics(
	...
	app.post(
		"/api/admin/ab-testing/safety-thresholds",
		requireAuth,
		requireRole(["admin"]),
		async (req: Request, res: Response) => {
			try {
				const { maxDrawdown, maxComplaintRate, maxRestrictedAssetExposure } =
					req.body;

				const thresholds: any = {};
				if (maxDrawdown !== undefined) thresholds.maxDrawdown = maxDrawdown;
				if (maxComplaintRate !== undefined)
					thresholds.maxComplaintRate = maxComplaintRate;
				if (maxRestrictedAssetExposure !== undefined)
					thresholds.maxRestrictedAssetExposure = maxRestrictedAssetExposure;
	...
	app.post(
		"/api/admin/ab-testing/check-safety",
		requireAuth,
		requireRole(["admin"]),
		async (req: Request, res: Response) => {
			try {
