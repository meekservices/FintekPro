...
export function registerDLMRoutes(app: Express) {
	...
	app.get(
		...
		async (req: any, res: Response) => {
			try {
				...
				res
					.status(500)
					.json({ success: false, error: "Failed to list documents" });
			}
		},
	);
	...
	app.get(
		...
		async (req: any, res: Response) => {
			try {
				...
				const result =
					await DLMWorkflowService.getDocumentWithVersions(documentId);
		...
		async (req: any, res: Response) => {
			try {
				...
				if (assignedToRole !== undefined)
					updates.assignedToRole = assignedToRole;
	...
	// Create new version
	app.post(
		"/api/dlm/documents/:documentId/versions",
		requireAdmin,
		async (req: any, res: Response) => {
			try {
				const { documentId } = req.params;
				...
				res
					.status(500)
					.json({ success: false, error: "Failed to create version" });
			}
		},
	);
	...
	// Get version content
	app.get(
		"/api/dlm/documents/:documentId/versions/:versionId",
		requireAdmin,
		async (req: any, res: Response) => {
			try {
				...
				res
					.status(500)
					.json({ success: false, error: "Failed to fetch version" });
			}
		},
	);
	...
	app.post(
		"/api/dlm/documents/:documentId/transition",
		requireAdmin,
		async (req: any, res: Response) => {
			try {
				const { documentId } = req.params;
				...
				res
					.status(500)
					.json({ success: false, error: "Failed to transition document" });
			}
		},
	);
	...
	app.get(
		"/api/dlm/documents/:documentId/allowed-actions",
		requireAdmin,
		async (req: any, res: Response) => {
			try {
				const { documentId } = req.params;
				...
				res.json({
					success: true,
					data: { currentStatus: document.status, allowedActions: actions },
				});
			} catch (error: any) {
				console.error("Error getting allowed actions:", error);
				res
					.status(500)
					.json({ success: false, error: "Failed to get allowed actions" });
			}
		},
	);
	...
	app.get(
		"/api/dlm/documents/:documentId/audit",
		requireAdmin,
		async (req: any, res: Response) => {
			try {
				const { documentId } = req.params;
	...
	app.post(
		"/api/dlm/documents/:documentId/changes",
		requireAdmin,
		async (req: any, res: Response) => {
			try {
				const { documentId } = req.params;
				...
				res
					.status(500)
					.json({ success: false, error: "Failed to create tracked change" });
			}
		},
	);
	...
	app.get(
		...
		async (req: any, res: Response) => {
			try {
				...
				res
					.status(500)
					.json({ success: false, error: "Failed to fetch tracked changes" });
			}
		},
	);
		...
		async (req: any, res: Response) => {
			try {
				...
				if (!change) {
					return res
						.status(404)
						.json({ success: false, error: "Change not found" });
				}
				...
				res
					.status(500)
					.json({ success: false, error: "Failed to resolve tracked change" });
			}
		},
	);
	...
	// Add comment
	app.post(
		"/api/dlm/documents/:documentId/comments",
		requireAdmin,
		async (req: any, res: Response) => {
			try {
				const { documentId } = req.params;
	...
	app.get(
		...
		async (req: any, res: Response) => {
			try {
				...
				const comments = await DLMWorkflowService.getComments(
					documentId,
					versionId as string,
				);
	...
	// Resolve comment
	app.patch(
		"/api/dlm/comments/:commentId/resolve",
		requireAdmin,
		async (req: any, res: Response) => {
			try {
				...
				res
					.status(500)
					.json({ success: false, error: "Failed to resolve comment" });
			}
		},
	);
	...
	app.get("/api/dlm/stats", requireAdmin, async (req: any, res: Response) => {
		try {
			...
			const highRiskDocuments = await db
				.select()
				.from(schema.documents)
			...
			res.json({
				...
				data: {
					...
					entityCounts: Object.fromEntries(
						entityCounts.map((e) => [e.entityType, e.count]),
					),
					expiringDocuments,
					highRiskDocuments,
					recentActivity,
	...
	app.post(
		"/api/dlm/documents/:documentId/ai-review",
		requireAdmin,
		async (req: any, res: Response) => {
			try {
				const { documentId } = req.params;
				...
				res
					...
					.json({
						success: false,
						error: "Failed to run AI compliance review",
					});
			}
		},
	);
	...
	app.get(
		...
		async (req: any, res: Response) => {
			try {
				...
				const review = await DLMAIComplianceService.getReview(
					documentId,
					versionId as string,
				);
				...
				if (!review) {
					return res
						.status(404)
						.json({ success: false, error: "No AI review found" });
				}
				...
				res
					.status(500)
					.json({ success: false, error: "Failed to fetch AI review" });
			}
		},
	);
	...
	app.post(
		"/api/dlm/ai-reviews/:reviewId/acknowledge",
		requireAdmin,
		async (req: any, res: Response) => {
			try {
				...
				res
					.status(500)
					.json({ success: false, error: "Failed to acknowledge review" });
			}
		},
	);
	...
	app.post(
		"/api/dlm/documents/:documentId/compare-versions",
		requireAdmin,
		async (req: any, res: Response) => {
			try {
				const { documentId } = req.params;
				...
				if (!versionId1 || !versionId2) {
					return res
						.status(400)
						.json({ success: false, error: "Both version IDs required" });
				}
				...
				res
					.status(500)
					.json({ success: false, error: "Failed to compare versions" });
			}
		},
	);
	...
	app.post(
		"/api/dlm/suggest-clause",
		requireAdmin,
		async (req: any, res: Response) => {
			try {
				const { clauseCategory, entityType, agreementType, existingText } =
					req.body;
				...
				if (!clauseCategory || !entityType || !agreementType) {
					return res
						...
						.json({
							success: false,
							error:
								"clauseCategory, entityType, and agreementType are required",
						});
				}
				...
				res
					...
					.json({
						success: false,
						error: "Failed to generate clause suggestion",
					});
			}
		},
	);
