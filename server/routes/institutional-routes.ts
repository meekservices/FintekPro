...
router.get("/api/institutional/corporate-actions", async (req, res) => {
	try {
		...
		console.error(
			"[Institutional] Corporate actions fetch error:",
			error.message,
		);
		res
			.status(500)
			.json({ message: "Internal server error fetching corporate actions" });
	}
});
...
router.get("/api/institutional/credit-ratings", async (req, res) => {
	try {
		...
		res
			.status(500)
			.json({ message: "Internal server error fetching credit ratings" });
	}
});
...
router.get("/api/institutional/security-master", async (req, res) => {
	try {
      ...
      `);
		...
		console.error(
			"[Institutional] Security master fetch error:",
			error.message,
		);
		res
			.status(500)
			.json({ message: "Internal server error fetching security master" });
	}
});
