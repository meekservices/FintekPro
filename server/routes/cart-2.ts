...
export function registerCarPart2Routes(app: Express) {
	app.get(
		...
		async (req: any, res: any) => {
			try {
				const proposals = await storage.getProposalsByClientId(
					(req.user as any)!.id,
				);
				res.json(proposals);
			} catch (error) {
		...
		async (req: any, res: any) => {
			try {
				...
				const createdProposal =
					await storage.createInvestmentProposal(proposal);
	...
	app.put(
		...
		async (req: any, res: any) => {
			try {
				...
				const proposal = await storage.acceptProposal(
					proposalId,
					(req.user as any)!.id,
				);
				res.json(proposal);
	...
	app.put(
		...
		async (req: any, res: any) => {
			try {
				const { proposalId } = req.params;
				const proposal = await storage.rejectProposal(
					proposalId,
					(req.user as any)!.id,
				);
				res.json(proposal);
	...
	app.put(
		"/api/proposals/:proposalId/mark-viewed",
		requireClientOrHigher,
		async (req: any, res: any) => {
			try {
				const { proposalId } = req.params;
				const proposal = await storage.markProposalAsViewed(
					proposalId,
					(req.user as any)!.id,
				);
				res.json(proposal);
	...
	app.post(
		"/api/proposals/:proposalId/add-to-cart",
		requireClientOrHigher,
		async (req: any, res: any) => {
			try {
				const { proposalId } = req.params;
				const result = await storage.addProposalToCart(
					proposalId,
					(req.user as any)!.id,
				);
				res.json(result);
	...
	app.post(
		...
		async (req: any, res: any, next: any) => {
			try {
				...
				const proposalItems =
					await storage.getInvestmentProposalItems(proposalId);
				...
				const totalAmount = proposalItems.reduce(
					(sum: any, item: any) => sum + Number.parseFloat(item.amount || "0"),
					0,
				);
		...
		async (req: any, res: any) => {
			try {
				...
				if (proposal.clientId !== userId) {
					return res
						.status(403)
						.json({ error: "Not authorized to complete this proposal" });
				}
				...
				if (proposal.status !== "accepted") {
					return res
						.status(400)
						.json({ error: "Proposal must be accepted before completion" });
				}
	...
	app.get(
		...
		async (req: any, res: any) => {
			try {
				...
				const paymentStatus = await bseStarApi.checkPaymentStatus(
					userId,
					transNo,
				);
		...
		async (req, res) => {
			try {
				...
				if (!isAuthorized) {
					return res
						.status(403)
						.json({ error: "Not authorized to access this proposal" });
				}
				...
				if (!cart) {
					[cart] = await db.insert(userCart).values({ userId }).returning();
				}
				...
				for (const item of proposalItems) {
					try {
						...
						const investmentAmount =
							item.recommendedAmount?.toString() ||
							storeProduct.minimumInvestment?.toString();
						...
						if (existingItem) {
							...
							const [updatedItem] = await db
								...
								.set({
									...
									investmentAmount:
										investmentAmount || existingItem.investmentAmount,
								})
						...
						console.error(
							`Error processing proposal item ${item.productName}:`,
							itemError,
						);
						skippedItems.push({
							productName: item.productName,
				...
				res
					.status(500)
					.json({ error: "Failed to load proposal items to cart" });
			}
		},
	);
