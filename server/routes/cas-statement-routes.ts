...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			console.log(
				"[CAS Routes] Using unified service for parse:",
			...
			try {
				...
				return res.status(500).json({
					...
					message:
						"Transaction rows found in CAS but lost during processing. This is a critical parsing error.",
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			console.log(
				"[CAS Routes] Using unified service for import:",
				prospect.name,
			);
			const importResult = await unifiedPortfolioImportService.importFromPDF(
				req.file.buffer,
			...
			const storageResult =
				await portfolioStorageService.upsertProspectPortfolio(
					prospectId,
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			const parseResult = await unifiedPDFParser.extractTextSafe(
				req.file.buffer,
			);
			...
			const statementDateMatch = text.match(
				/As\s*on\s*(\d{1,2}[-\/][A-Za-z]{3}[-\/]\d{4})/i,
			);
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			console.log(
				"[CAS Routes] Generating tax analysis for:",
				req.file.originalname,
			);
			...
			if (result.reconciliation && !result.reconciliation.passed) {
				return res.status(400).json({
					success: false,
					error:
						"Tax analysis blocked - CAS reconciliation failed. Please verify the statement is complete.",
			...
			for (const holding of result.holdings) {
				...
				if (!holding.eligibleForTax) {
					console.log(
						"[CAS Routes] Skipping tax analysis for non-eligible holding:",
						holding.isin,
						"tier:",
						holding.holdingTier,
					);
				...
				const lotResult = result.lotLedger.results.find(
					(r) => r.isin === holding.isin,
				);
				...
				if (!holding.unitBalance || holding.unitBalance <= 0) {
					console.log(
						"[CAS Routes] Skipping tax analysis for holding with zero units:",
						holding.isin,
					);
					continue;
				}
				const currentNav =
					holding.nav || holding.marketValue / holding.unitBalance;
				const taxSummary =
					await lotTaxCalculatorService.calculateHoldingTaxSummary(
						lotResult.lots,
						currentNav,
						holding.schemeName,
					);
			...
			const totalExitLoad = taxAnalysis.reduce(
				(sum, t) => sum + t.exitLoadAmount,
				0,
			);
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			console.log(
				"[CAS Routes] Generating audit view for:",
				req.file.originalname,
			);
			...
			const auditView = {
				...
				flaggedHoldings: result.holdings
					.filter((h) => {
						const conf = result.holdingConfidence.get(
							`${h.isin}|${h.folioNumber}`,
						);
