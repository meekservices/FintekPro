...
import {
	mcaIntelligenceService,
	McaRole,
	McaQueryType,
} from "../services/mca-intelligence-service";
...
router.use((req: Request, res: Response, next: Function) => {
	console.log(
		`[MCA Router] ${req.method} ${req.path} - Full URL: ${req.originalUrl}`,
	);
	next();
});
...
function getMcaRole(req: Request): McaRole {
	...
	if (role === "compliance" || rolesLower.includes("compliance"))
		return "compliance";
	if (
		role === "ops" ||
		role === "operations" ||
		rolesLower.includes("ops") ||
		rolesLower.includes("operations")
	)
		return "ops";
	return "advisor";
}
...
function requireMcaAccess(action: "read" | "query" | "ingest" | "full") {
	return (req: Request, res: Response, next: Function) => {
		...
		const user = (req as any).user;
		console.log(
			`[MCA Access] Checking ${action} access - Role: ${role}, User: ${user?.email}, UserRole: ${user?.role}, UserRoles: ${JSON.stringify(user?.roles)}`,
		);
	...
	async (req: Request, res: Response) => {
		...
		try {
			...
			if (format === "csv") {
				...
				res.setHeader(
					"Content-Disposition",
					`attachment; filename=mca-audit-${new Date().toISOString().split("T")[0]}.csv`,
				);
				return res.send(csv);
			}
		...
		try {
			...
			res.json({
				...
				data: {
					...
					disclaimer:
						"Data sourced from MCA via Sandbox.co.in. This is cached data and may not reflect real-time MCA records.",
				},
			});
		} catch (error: any) {
...
router.get("/compliance-disclaimer", (req: Request, res: Response) => {
	res.json({
		...
		data: {
			disclaimer:
				"All company data displayed on this platform is sourced from the Ministry of Corporate Affairs (MCA) via authorized third-party APIs. This data is provided for informational purposes only and should not be construed as financial, legal, or investment advice.",
...
router.get(
	...
	async (req: Request, res: Response) => {
		try {
			const maxAgeDays = Number.parseInt(req.query.maxAgeDays as string) || 90;
			...
			const staleCompanies =
				await mcaIntelligenceService.getCompaniesNeedingRefresh({
					maxAgeDays,
					limit,
					onlyUnlisted,
				});
 ...
 */
router.post(
	"/enrichment/enrich/:cin",
	requireMcaAccess("ingest"),
	async (req: Request, res: Response) => {
		try {
			...
			console.log(
				`[MCA Enrichment] Manual enrichment triggered for ${cin} (force: ${forceRefresh})`,
			);

			const result = await mcaIntelligenceService.enrichCompanyData(cin, {
				forceRefresh,
			});
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			const invalidCins = cins.filter(
				(cin: string) => typeof cin !== "string" || cin.length !== 21,
			);
			...
			console.log(
				`[MCA Enrichment] Bulk enrichment triggered for ${cins.length} companies`,
			);
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			console.log(
				`[MCA Enrichment] Stale refresh triggered (maxAge: ${maxAgeDays}, limit: ${limit})`,
			);
			...
			console.error(
				"[MCA Enrichment] Error refreshing stale companies:",
				error,
			);
			res.status(500).json({
				success: false,
 ...
 */
router.get(
	"/enrichment/check-freshness/:cin",
	requireMcaAccess("read"),
	async (req: Request, res: Response) => {
		try {
			const { cin } = req.params;
