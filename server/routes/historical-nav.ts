...
router.get("/summary/:schemeCode", async (req, res) => {
  try {
    const { schemeCode } = req.params;
    const summary = await historicalNavService.getDataSummary(schemeCode, 'mutual_fund');
    res.json(summary);
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
...
router.get("/metrics/:schemeCode", async (req, res) => {
  try {
    ...
    const { ready, summary } = await historicalNavService.ensureData(schemeCode);
    ...
    const metrics = await historicalNavService.calculateMetrics(
      schemeCode,
      'mutual_fund',
      parseInt(periodYears as string)
    );
    
    res.json({
      schemeCode,
      periodYears: parseInt(periodYears as string),
      dataSummary: summary,
      metrics
    });
  } catch (error: any) {
...
router.post("/batch-fetch", async (req, res) => {
  try {
    ...
    const results = await Promise.all(
      schemeCodes.slice(0, 10).map(code => 
