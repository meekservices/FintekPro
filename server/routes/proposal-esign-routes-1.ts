...
router.get("/workflows/:workflowId", async (req: Request, res: Response) => {
	try {
		...
		const workflow =
			await proposalEsignWorkflowService.getWorkflowWithDetails(workflowId);
...
router.get(
	...
	async (req: Request, res: Response) => {
		try {
			...
			const workflow =
				await proposalEsignWorkflowService.getWorkflowByProposal(proposalId);

			if (!workflow) {
				return res
					.status(404)
					.json({ error: "No workflow found for this proposal" });
			}

			const auth = await requireWorkflowAccess(req, res, workflow.id);
			if (!auth) return;

			const details = await proposalEsignWorkflowService.getWorkflowWithDetails(
				workflow.id,
			);
...
router.get("/user/workflows", async (req: Request, res: Response) => {
	try {
		...
		const workflows = await proposalEsignWorkflowService.getWorkflowsForUser(
			user.id,
			user.role,
		);
...
router.get("/user/pending-approvals", async (req: Request, res: Response) => {
	try {
		...
		const pending = await proposalEsignWorkflowService.getPendingApprovals(
			user.id,
		);
...
router.post(
	"/workflows/:workflowId/participants",
	async (req: Request, res: Response) => {
		try {
			const { workflowId } = req.params;
			...
			if (!auth.isCreator && !auth.isAdmin) {
				return res
					.status(403)
					.json({
						error: "Only workflow creator or admin can add participants",
					});
			}
			...
			const participant = await proposalEsignWorkflowService.addParticipant({
				workflowId,
				...parsed.data,
				actionRequiredBy: parsed.data.actionRequiredBy
					? new Date(parsed.data.actionRequiredBy)
					: undefined,
			} as any);
		...
		try {
			...
			if (!auth.isCreator && !auth.isAdmin && !auth.participant?.canEdit) {
				return res
					...
					.json({
						error:
							"You do not have permission to create versions for this workflow",
					});
			}
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (!auth.isCreator && !auth.isAdmin && !auth.participant?.canEdit) {
				return res
					.status(403)
					.json({ error: "You do not have permission to edit this workflow" });
			}
			...
			res
				...
				.json({
					error:
						error instanceof Error ? error.message : "Failed to record edit",
				});
		}
	},
);
...
router.post("/edits/:editId/approve", async (req: Request, res: Response) => {
	try {
		...
		if (!auth.isElevated) {
			return res
				.status(403)
				.json({ error: "Only admin, agent, or partner can approve edits" });
		}
		...
		res
			...
			.json({
				error:
					error instanceof Error ? error.message : "Failed to approve edit",
			});
	}
});
...
router.post("/edits/:editId/reject", async (req: Request, res: Response) => {
	try {
		...
		if (!auth.isElevated) {
			return res
				.status(403)
				.json({ error: "Only admin, agent, or partner can reject edits" });
		}
		...
		res
			.status(500)
			.json({
				error: error instanceof Error ? error.message : "Failed to reject edit",
			});
	}
});
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			await proposalEsignWorkflowService.resolveCommentThread(
				commentId,
				user.id,
			);
