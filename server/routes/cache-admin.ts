	...
	try {
		...
		res.json({
			...
			data: {
				...metrics,
				description:
					"Request deduplication prevents duplicate in-flight API requests for the same resource",
				benefits: [
	...
	try {
		...
		res.json({
			...
			data: {
				...metrics,
				description:
					"AI response caching reduces Gemini API costs by caching similar recommendation requests",
				cachedTypes: [
		...
		res.json({
			...
			data: {
				...
				totalSavings: {
					...
					apiCallsSaved:
						dedupeMetrics.apiCallsSaved +
						aiMetrics.estimatedApiCallsSaved +
						onboardingMetrics.estimatedApiCallsSaved,
					estimatedMonthlySavingsUSD:
						(cacheStats.apiUsage.estimatedSavings || 0) +
						aiMetrics.estimatedCostSavingsUSD,
	...
	try {
		...
		res.json({
			...
			data: {
				...metrics,
				description:
					"Unified stock price caching from NSE/BSE with automatic fallback",
			},
		});
	} catch (error: any) {
	...
	try {
		...
		res.json({
			...
			data: {
				...metrics,
				description:
					"Caches verification results during KYC onboarding to reduce API calls",
			},
		});
	} catch (error: any) {
...
router.post("/batch-enrich", async (req, res) => {
	try {
		...
		console.log(
			`[BatchEnrich] Processing ${cins.length} companies${includeFinancials ? " with financials" : ""}`,
		);
		...
		if (includeFinancials) {
			financialsMap = await credhiveService.batchGetCompanyFinancials(
				cins,
				years,
			);
		}
