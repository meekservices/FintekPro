	...
	async (req: Request, res: Response) => {
		try {
			...
			const kycLevel = profile?.kycLevel
				? Number.parseInt(profile.kycLevel, 10)
				: 0;
			const kycVerified = kycLevel >= 2;
			const fatcaCompliant =
				profile?.fatcaDeclarationDate !== null &&
				profile?.fatcaDeclarationDate !== undefined;
			const riskProfileCompleted =
				riskData?.riskTolerance !== null &&
				riskData?.riskTolerance !== undefined;
			...
			const checks = {
				kycStatus: {
					...
					message: kycVerified
						...
						: "KYC Level 2 required for mutual fund transactions",
				},
				fatcaStatus: {
					passed: fatcaCompliant,
					declarationDate: profile?.fatcaDeclarationDate,
					message: fatcaCompliant
						? "FATCA declaration on file"
						: "FATCA declaration may be required by AMC",
				},
				riskProfile: {
					passed: riskProfileCompleted,
					profile: riskData?.riskTolerance,
					message: riskProfileCompleted
						? `Risk profile: ${riskData?.riskTolerance}`
						: "Risk profile assessment recommended",
				},
				amlPepStatus: {
					passed: profile?.amlStatus !== "flagged",
					message:
						profile?.amlStatus === "flagged"
							? "AML screening flagged - contact compliance"
							: "AML/PEP screening clear",
				},
				bankAccountLinked: {
					passed: true,
			...
			const overallPassed =
				checks.kycStatus.passed && checks.amlPepStatus.passed;
			const warnings = [];

			if (!checks.fatcaStatus.passed) warnings.push(checks.fatcaStatus.message);
			if (!checks.riskProfile.passed && orderType !== "sell")
				warnings.push(checks.riskProfile.message);
			...
			res.json({
				...
				requiresSuitabilityAck:
					!checks.riskProfile.passed && orderType !== "sell",
			});
		} catch (error) {
...
async function checkPreTradeCompliance(
	userId: string,
	orderType: string,
	hasSuitabilityAck: boolean = false,
) {
	const [profile] = await db
		.select()
		.from(userProfiles)
	...
	const fatcaCompliant =
		...
		profile?.fatcaDeclarationDate !== undefined;
	const riskProfileCompleted =
		riskData?.riskTolerance !== null && riskData?.riskTolerance !== undefined;
	const amlClear = profile?.amlStatus !== "flagged";

	if (!kycVerified) {
		errors.push(
			"KYC Level 2 or higher is required for mutual fund transactions",
		);
	}
	...
	if (!fatcaCompliant && isInvestmentOrder) {
		errors.push(
			"FATCA declaration is required for investment orders - please complete FATCA declaration",
		);
	}

	if (!riskProfileCompleted && isInvestmentOrder && !hasSuitabilityAck) {
		errors.push(
			"Risk profile assessment required - please complete risk profiling or provide suitability acknowledgement",
		);
	}
	...
	return {
		...
		warnings,
		flags,
		requiresSuitabilityAck: warnings.includes(
			"Risk profile not completed - suitability assessment may be required",
		),
	};
}
	...
	async (req: Request, res: Response) => {
		try {
			...
			res.status(201).json({
				order,
				message:
					"Order created successfully. Proceed to payment to complete the order.",
			});
		} catch (error) {
...
const PRIVILEGED_STATUS_CHANGES = [
	"placed",
	"confirmed",
	"processing",
	"settled",
	...
	async (req: Request, res: Response) => {
		try {
			...
			const isPrivilegedChange =
				PRIVILEGED_STATUS_CHANGES.includes(requestedStatus);
				...
				if (isCancellation) {
					if (!isOwnOrder) {
						return res
							.status(403)
							.json({
								error: "Access denied - cannot cancel other users orders",
							});
					}
					if (
						!["created", "pending_payment"].includes(existingOrder.status || "")
					) {
						return res
							.status(400)
							.json({ error: "Order cannot be cancelled after placement" });
					}
				} else if (isPrivilegedChange) {
					return res
						.status(403)
						.json({
							error: "Only authorized administrators can perform this action",
						});
				} else {
					return res.status(403).json({ error: "Access denied" });
				}
			...
			if (validated.navApplied)
				updateData.navApplied = validated.navApplied.toString();
			if (validated.unitsAllotted)
				updateData.unitsAllotted = validated.unitsAllotted.toString();
			if (validated.rtaReference)
				updateData.rtaReference = validated.rtaReference;
			if (validated.amcReference)
				updateData.amcReference = validated.amcReference;
			if (validated.bseOrderId) updateData.bseOrderId = validated.bseOrderId;
			if (validated.paymentReference)
				updateData.paymentReference = validated.paymentReference;
			if (validated.settlementReference)
				updateData.settlementReference = validated.settlementReference;
			...
			if (
				...
				["sell", "redeem", "switch"].includes(existingOrder.orderType || "")
			) {
				try {
					...
					console.log(
						"[MF Orders] Capital gains calculated on status change to settled:",
						id,
					);
				} catch (gainError) {
					console.error(
