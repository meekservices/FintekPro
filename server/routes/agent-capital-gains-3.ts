...
export function registerAgentCapitalGainPart3Routes(app: Express): void {
	...
	app.get("/api/supplier-products", requireAdmin, async (req, res) => {
		try {
			...
			const products = await (storage as any).getSupplierProducts(
		...
		try {
			...
			if (!supplierId || !productName || !price || !profitMargin) {
				return res
					...
					.json({
						error:
							"Supplier ID, product name, price, and profit margin are required",
					});
			}
	...
	app.get(
		"/api/products/:productId/optimal-supplier",
		requireAdmin,
		async (req, res) => {
			try {
				...
				if (!optimalSupplier) {
					return res
						.status(404)
						.json({ error: "No suppliers found for this product" });
				}
	...
	app.get(
		"/api/products/:productId/profit-analysis",
		requireAdmin,
		async (req, res) => {
			try {
				const { productId } = req.params;
	...
	app.get(
		"/api/products/:productId/supplier-comparison",
		requireAdmin,
		async (req, res) => {
			try {
				const { productId } = req.params;
				...
				res
					.status(500)
					.json({ error: "Failed to generate supplier comparison" });
			}
		},
	);
	...
	app.get("/api/product-performance", requireAdmin, async (req, res) => {
		try {
			...
			const metrics = await (storage as any).getProductPerformanceMetrics(
				productId as string,
			);
			res.json({ metrics });
		} catch (error) {
			...
			res
				.status(500)
				.json({ error: "Failed to fetch product performance metrics" });
		}
	});
		...
		try {
			...
			if (!productId || !salesVolume || !revenue) {
				return res
					.status(400)
					.json({
						error: "Product ID, sales volume, and revenue are required",
					});
			}
			...
			res
				.status(500)
				.json({ error: "Failed to create product performance metric" });
		}
	});
	...
	app.put(
		"/api/admin/client-assignments/:id",
		requireAdmin,
		async (req, res) => {
			try {
				const { id } = req.params;
	...
	app.post("/api/loans/eligibility", async (req, res) => {
		try {
			...
			const totalValue = holdings.reduce(
				(sum, holding) =>
					...
					Number.parseFloat(holding.quantity) *
						Number.parseFloat((holding as any).avgPrice),
				0,
			);

			// Calculate eligibility (typically 50-80% LTV for securities)
			const maxLoanAmount = totalValue * 0.75; // 75% LTV
			const isEligible = Number.parseFloat(requestedAmount) <= maxLoanAmount;
			...
			const eligibilityData = {
				...
				loanToValue: (
					(Number.parseFloat(requestedAmount) / totalValue) *
					100
				).toFixed(2),
				interestRate: "10.25", // Starting rate like 50Fin
				processingFee: Number.parseFloat(requestedAmount) * 0.01, // 1% processing fee
				eligibleAssets: holdings.filter((h) =>
					["equity", "mf"].includes(h.assetType),
				),
			};
		...
		try {
			...
			const apiKeys = {
				...
				OPENAI_API_KEY: process.env.OPENAI_API_KEY
					? "configured"
					: "not_configured",
				ICICI_BANK_API_KEY: process.env.ICICI_BANK_API_KEY
					? "configured"
					: "not_configured",
				HDFC_BANK_API_KEY: process.env.HDFC_BANK_API_KEY
					? "configured"
					: "not_configured",
				JM_FINANCIAL_API_KEY: process.env.JM_FINANCIAL_API_KEY
					? "configured"
					: "not_configured",
			};
			...
			res
				.status(500)
				.json({ success: false, error: "Failed to fetch API keys status" });
		}
	});
	...
	app.post("/api/admin/api-keys", requireAdmin, async (req, res) => {
		try {
			...
			res
				.status(500)
				.json({ success: false, error: "Failed to update API key" });
		}
	});
