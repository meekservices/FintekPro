...
router.get("/catalog", async (req: Request, res: Response) => {
	try {
		...
		if (!type || type === "all" || type === "government") {
			...
			const govBonds = await govQuery.limit(Number.parseInt(limit as string));
			bonds.push(
				...govBonds.map((b: any) => ({
					...b,
		...
		if (!type || type === "all" || type === "corporate") {
			...
			const corpBonds = await corpQuery.limit(Number.parseInt(limit as string));
			bonds.push(
				...corpBonds.map((b: any) => ({
					...b,
	...
	async (req: Request, res: Response) => {
		try {
			...
			const transactionValue =
				Number.parseFloat(askPrice) * Number.parseInt(quantity);
			...
			if (!tierCheck.eligible) {
				...
				return apiResponse.forbidden(
					res,
					`KYC tier requirement not met: ${tierCheck.reason}. Required tier: ${requiredTier}`,
				);
			}
			...
			if (requiredTier === "tier_3" || !isListed) {
				...
				const requiredDisclosures = disclosureResult.disclosures
				...
				if (
					...
					return apiResponse.badRequest(
						res,
						"Risk disclosure acknowledgments required for this bond type. Please acknowledge all mandatory risks before proceeding.",
					);
				}
			}
			...
			const [listing] = await db
				.insert(schema.bondSellListings)
				.values({
					sellerUserId: userId,
					instrumentType,
					governmentSecurityId,
...
router.get(
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (bondType) {
				conditions.push(
					eq(schema.bondBuyRequests.bondType, bondType as string),
				);
			}
			if (isin) {
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			// SEBI Compliance: Determine regulatory tier based on bond type and transaction value
			const transactionValue =
				Number.parseFloat(maxPrice) * Number.parseInt(quantity);
			const requiredTier = determineRegulatoryTier(
				bondType,
				transactionValue,
				isListed,
