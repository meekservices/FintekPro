...
import { m2pCardService, CREDIT_CARD_CATALOG } from '../services/m2p-card-service';
...
export function registerM2PCardRoutes(app: Express): void {
  ...
  app.get('/api/m2p/status', (_req: Request, res: Response) => {
    res.json({
      ...
      onboarding: {
        step1: 'Contact M2P: business@m2pfintech.com or https://m2pfintech.com/connect',
        step2: 'Sign NDA + Partnership Agreement',
        step3: 'Receive sandbox credentials (M2P_BASE_URL, M2P_API_KEY, M2P_SECRET_KEY, M2P_PROGRAM_ID)',
        step4: 'Set env vars and test in sandbox (no PKI needed for sandbox)',
        step5: 'Receive production PKI keys (M2P_PUBLIC_KEY, M2P_PRIVATE_KEY) + IP whitelist',
  ...
  app.get('/api/cards/products', async (req: Request, res: Response) => {
    try {
      ...
      if (minIncome !== undefined) {
        products = products.filter(p => p.eligibility.minIncome <= Number(minIncome));
      }
      if (search) {
      ...
      res.status(500).json({ success: false, message: error instanceof Error ? error.message : 'Failed to fetch card products' });
    }
  });
   ...
   */
  app.post('/api/cards/eligibility', requireAuth, async (req: Request, res: Response) => {
    try {
      const { pan, mobile, annualIncome, employmentType, creditScore } = req.body;

      if (!pan || !mobile || !annualIncome || !employmentType) {
        return res.status(400).json({
          success: false,
          message: 'pan, mobile, annualIncome and employmentType are required',
        });
      }
      if (!/^[A-Z]{5}[0-9]{4}[A-Z]{1}$/.test(pan)) {
        return res.status(400).json({ success: false, message: 'Invalid PAN format' });
      }
      if (!['salaried', 'self_employed', 'business'].includes(employmentType)) {
        return res.status(400).json({
          success: false,
          message: 'employmentType must be salaried, self_employed, or business',
        });
      }
      ...
      return res.status(500).json({ success: false, message: error instanceof Error ? error.message : 'Eligibility check failed' });
    }
  });
   ...
   */
  app.post('/api/cards/apply', requireAuth, async (req: Request, res: Response) => {
    try {
      if (!m2pCardService.isConfigured()) {
        return res.status(503).json({
          success: false,
          message: 'M2P API not configured. Contact business@m2pfintech.com to onboard and set M2P_BASE_URL, M2P_API_KEY, M2P_SECRET_KEY, M2P_PROGRAM_ID.',
        });
      }

      const { programId, customer, referenceId } = req.body;
      if (!programId || !customer) {
        return res.status(400).json({ success: false, message: 'programId and customer are required' });
      }

      const required = ['name', 'mobile', 'email', 'pan', 'dob', 'gender', 'annualIncome', 'employmentType', 'address'];
      const missing = required.filter(f => !customer[f]);
      if (missing.length > 0) {
        return res.status(400).json({ success: false, message: `Missing customer fields: ${missing.join(', ')}` });
      }

      const result = await m2pCardService.applyForCard({ programId, customer, referenceId });
      return res.json({ success: true, application: result });
    } catch (error) {
      console.error('[M2P Cards] Apply error:', error);
      return res.status(500).json({ success: false, message: error instanceof Error ? error.message : 'Card application failed' });
    }
  });
   ...
   */
  app.get('/api/cards/applications/:id', requireAuth, async (req: Request, res: Response) => {
    try {
      if (!m2pCardService.isConfigured()) {
        return res.status(503).json({ success: false, message: 'M2P API not configured' });
      }
      ...
      return res.status(500).json({ success: false, message: error instanceof Error ? error.message : 'Failed to get application status' });
    }
  });
   ...
   */
  app.get('/api/cards/my-cards', requireAuth, async (req: Request, res: Response) => {
    try {
      if (!m2pCardService.isConfigured()) {
        return res.status(503).json({ success: false, message: 'M2P API not configured', cards: [] });
      }
      const customerId = req.query.customerId as string;
      if (!customerId) {
        return res.status(400).json({ success: false, message: 'customerId query param required' });
      }
      const cards = await m2pCardService.listCards(customerId);
      return res.json({ success: true, cards });
    } catch (error) {
      console.error('[M2P Cards] List cards error:', error);
      return res.status(500).json({ success: false, message: error instanceof Error ? error.message : 'Failed to list cards' });
    }
  });
   ...
   * Get card details.
   */
  app.get('/api/cards/:cardId', requireAuth, async (req: Request, res: Response) => {
    try {
      if (!m2pCardService.isConfigured()) {
        return res.status(503).json({ success: false, message: 'M2P API not configured' });
      }
      ...
      return res.status(500).json({ success: false, message: error instanceof Error ? error.message : 'Failed to get card' });
    }
  });
   ...
   */
  app.post('/api/cards/:cardId/activate', requireAuth, async (req: Request, res: Response) => {
    try {
      if (!m2pCardService.isConfigured()) {
        return res.status(503).json({ success: false, message: 'M2P API not configured' });
      }
      ...
      return res.status(500).json({ success: false, message: error instanceof Error ? error.message : 'Failed to activate card' });
    }
  });
   ...
   * Body: { reason }
   */
  app.post('/api/cards/:cardId/block', requireAuth, async (req: Request, res: Response) => {
    try {
      if (!m2pCardService.isConfigured()) {
        return res.status(503).json({ success: false, message: 'M2P API not configured' });
      }
      ...
      const result = await m2pCardService.blockCard(req.params.cardId, reason);
      return res.json({ success: true, ...result });
    } catch (error) {
      console.error('[M2P Cards] Block card error:', error);
      return res.status(500).json({ success: false, message: error instanceof Error ? error.message : 'Failed to block card' });
    }
  });
   ...
   */
  app.post('/api/cards/:cardId/unblock', requireAuth, async (req: Request, res: Response) => {
    try {
      if (!m2pCardService.isConfigured()) {
        return res.status(503).json({ success: false, message: 'M2P API not configured' });
      }
      ...
      return res.status(500).json({ success: false, message: error instanceof Error ? error.message : 'Failed to unblock card' });
    }
  });
   ...
   */
  app.post('/api/cards/:cardId/replace', requireAuth, async (req: Request, res: Response) => {
    try {
      if (!m2pCardService.isConfigured()) {
        return res.status(503).json({ success: false, message: 'M2P API not configured' });
      }
      ...
      if (!['lost', 'damaged', 'expired'].includes(reason)) {
        return res.status(400).json({ success: false, message: 'reason must be lost, damaged, or expired' });
      }
      const result = await m2pCardService.replaceCard(req.params.cardId, reason);
      return res.json({ success: true, ...result });
    } catch (error) {
      console.error('[M2P Cards] Replace card error:', error);
      return res.status(500).json({ success: false, message: error instanceof Error ? error.message : 'Failed to request card replacement' });
    }
  });
   ...
   * Body: { limit, reason? }
   */
  app.put('/api/cards/:cardId/limit', requireAuth, async (req: Request, res: Response) => {
    try {
      if (!m2pCardService.isConfigured()) {
        return res.status(503).json({ success: false, message: 'M2P API not configured' });
      }
      ...
      if (!limit || isNaN(Number(limit))) {
        return res.status(400).json({ success: false, message: 'limit (number) is required' });
      }
      const result = await m2pCardService.updateCreditLimit(req.params.cardId, Number(limit), reason);
      return res.json({ success: true, ...result });
    } catch (error) {
      console.error('[M2P Cards] Update limit error:', error);
      return res.status(500).json({ success: false, message: error instanceof Error ? error.message : 'Failed to update credit limit' });
    }
  });
   ...
   */
  app.get('/api/cards/:cardId/transactions', requireAuth, async (req: Request, res: Response) => {
    try {
      if (!m2pCardService.isConfigured()) {
        return res.status(503).json({ success: false, message: 'M2P API not configured', transactions: [] });
      }
      const { from, to, type, limit, offset } = req.query;
      const result = await m2pCardService.listTransactions(req.params.cardId, {
      ...
      return res.status(500).json({ success: false, message: error instanceof Error ? error.message : 'Failed to fetch transactions' });
    }
  });
   ...
   * Body: { reason }
   */
  app.post('/api/cards/:cardId/transactions/:txId/dispute', requireAuth, async (req: Request, res: Response) => {
    try {
      if (!m2pCardService.isConfigured()) {
        return res.status(503).json({ success: false, message: 'M2P API not configured' });
      }
      const { reason } = req.body;
      if (!reason) return res.status(400).json({ success: false, message: 'reason is required' });
      const result = await m2pCardService.disputeTransaction(req.params.cardId, req.params.txId, reason);
      return res.json({ success: true, ...result });
    } catch (error) {
      console.error('[M2P Cards] Dispute error:', error);
      return res.status(500).json({ success: false, message: error instanceof Error ? error.message : 'Failed to raise dispute' });
    }
  });
   ...
   */
  app.get('/api/cards/:cardId/statements', requireAuth, async (req: Request, res: Response) => {
    try {
      if (!m2pCardService.isConfigured()) {
        return res.status(503).json({ success: false, message: 'M2P API not configured', statements: [] });
      }
      const statements = await m2pCardService.listStatements(req.params.cardId);
      return res.json({ success: true, statements });
    } catch (error) {
      console.error('[M2P Cards] Statements error:', error);
      return res.status(500).json({ success: false, message: error instanceof Error ? error.message : 'Failed to fetch statements' });
    }
  });
   ...
   */
  app.post('/api/cards/:cardId/statements/generate', requireAuth, async (req: Request, res: Response) => {
    try {
      if (!m2pCardService.isConfigured()) {
        return res.status(503).json({ success: false, message: 'M2P API not configured' });
      }
      const { billingMonth } = req.body;
      if (!billingMonth) return res.status(400).json({ success: false, message: 'billingMonth (YYYY-MM) required' });
      const result = await m2pCardService.generateStatement(req.params.cardId, billingMonth);
      return res.json({ success: true, ...result });
    } catch (error) {
      console.error('[M2P Cards] Generate statement error:', error);
      return res.status(500).json({ success: false, message: error instanceof Error ? error.message : 'Failed to generate statement' });
    }
  });
   ...
   */
  app.get('/api/cards/:cardId/statements/:sid', requireAuth, async (req: Request, res: Response) => {
    try {
      if (!m2pCardService.isConfigured()) {
        return res.status(503).json({ success: false, message: 'M2P API not configured' });
      }
      const statement = await m2pCardService.getStatement(req.params.cardId, req.params.sid);
      return res.json({ success: true, statement });
    } catch (error) {
      console.error('[M2P Cards] Get statement error:', error);
      return res.status(500).json({ success: false, message: error instanceof Error ? error.message : 'Failed to get statement' });
    }
  });
  ...
  app.post('/api/webhooks/m2p', async (req: Request, res: Response) => {
    try {
      ...
      if (!signature) {
        console.warn('[M2P Webhook] Missing x-m2p-signature header — rejecting');
      ...
      console.log(`[M2P Webhook] Event received: ${event.eventType} (${event.eventId})`);
      ...
      switch (event.eventType) {
        ...
        case 'card.activated':
          ...
          break;
        case 'card.blocked':
          console.log('[M2P Webhook] Card blocked:', event.data.cardId, '— reason:', event.data.reason);
          break;
        case 'card.replaced':
          console.log('[M2P Webhook] Card replaced — old:', event.data.oldCardId, '→ new:', event.data.newCardId);
          break;
        case 'transaction.approved':
          console.log('[M2P Webhook] Transaction approved:', event.data.transactionId, '₹', event.data.amount);
          break;
        case 'transaction.declined':
          console.log('[M2P Webhook] Transaction declined:', event.data.transactionId, '— reason:', event.data.declineReason);
          break;
        case 'statement.generated':
          console.log('[M2P Webhook] Statement generated:', event.data.statementId, 'for card:', event.data.cardId);
          break;
        case 'limit.updated':
          console.log('[M2P Webhook] Limit updated:', event.data.cardId, '→', event.data.newLimit);
          break;
        case 'application.approved':
          console.log('[M2P Webhook] Application approved:', event.data.applicationId, '— limit:', event.data.approvedLimit);
          break;
        case 'application.rejected':
          console.log('[M2P Webhook] Application rejected:', event.data.applicationId, '— reason:', event.data.rejectionReason);
          break;
        default:
  ...
  console.log('✅ M2P Credit Card routes registered (/api/cards/*, /api/webhooks/m2p, /api/m2p/status)');
}
