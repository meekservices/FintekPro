...
async function verifyFamilyMembership(req: any, res: any, next: any) {
	try {
		...
		if (!req.user?.id)
			return res.status(401).json({ message: "Authentication required" });
		const membership = await storage.checkFamilyMembership(
			targetId,
			req.user!.id,
		);
		if (!membership)
			return res
				.status(403)
				.json({ message: "Not a member of this family group" });
...
export function registerFamilyCollaborationRoutes(app: Express): void {
	...
	app.post("/api/families", requireAuth, async (req, res) => {
		try {
			...
			await storage.logFamilyActivity({
				...
				description: `Family group "${(familyGroup as any).name}" was created`,
			} as any);
	...
	app.patch(
		...
		async (req, res) => {
			try {
				if ((req as any).familyMember.role !== "admin")
					return res
						.status(403)
						.json({ message: "Only admins can update family group" });
				const updates = insertFamilyGroupSchema.partial().parse(req.body);
				const updatedFamily = await storage.updateFamilyGroup(
					req.params.id,
					updates,
				);
				await storage.logFamilyActivity({
		...
		async (req, res) => {
			try {
				...
				await storage.logFamilyActivity({
					...
					description: `User was invited to the family`,
				} as any);
				complianceMonitor.logEvent({
					userId: req.user!.id,
					eventType: "admin_action" as any,
					action: "invite_family_member",
					resource: req.params.familyId,
					outcome: "success",
					riskLevel: "medium",
				});
	...
	app.post(
		"/api/families/:familyId/members/:memberId/accept",
		requireAuth,
		async (req, res) => {
			try {
				const { familyId, memberId } = req.params;
				const member = await storage.acceptFamilyInvitation(
					memberId,
					req.user!.id,
				);
				await storage.logFamilyActivity({
	...
	app.post(
		...
		async (req, res) => {
			try {
				if ((req as any).familyMember.role === "viewer")
					return res
						.status(403)
						.json({ message: "Viewers cannot create goals" });
				...
				await storage.logFamilyActivity({
					...
					description: `New goal "${(goal as any).name}" was created`,
				} as any);
				complianceMonitor.logEvent({
					userId: req.user!.id,
					eventType: "admin_action" as any,
	...
	app.post(
		"/api/families/:familyId/goals/:goalId/contribute",
		requireAuth,
		verifyFamilyMembership,
		async (req, res) => {
			try {
				if ((req as any).familyMember.role === "viewer")
					return res
						.status(403)
						.json({ message: "Viewers cannot contribute to goals" });
	...
	app.get(
		"/api/families/:familyId/goals/:goalId/contributions",
		requireAuth,
		verifyFamilyMembership,
		async (req, res) => {
			try {
				const contributions = await storage.getGoalContributions(
					req.params.goalId,
				);
				complianceMonitor.logEvent({
					userId: req.user!.id,
	...
	app.post(
		"/api/families/:familyId/discussions",
		requireAuth,
		verifyFamilyMembership,
		async (req, res) => {
			try {
				if ((req as any).familyMember.role === "viewer")
					return res
						.status(403)
						.json({ message: "Viewers cannot create discussions" });
				...
				await storage.logFamilyActivity({
					...
					description: `Started a new discussion: "${(discussion as any).title}"`,
				} as any);
				complianceMonitor.logEvent({
					userId: req.user!.id,
					eventType: "admin_action" as any,
	...
	app.get(
		...
		async (req, res) => {
			try {
				const discussions = await storage.getFamilyDiscussions(
					req.params.familyId,
				);
				complianceMonitor.logEvent({
					userId: req.user!.id,
		...
		async (req, res) => {
			try {
				if ((req as any).familyMember.role !== "admin")
					return res
						.status(403)
						.json({ message: "Only admins can create budgets" });
				...
				await storage.logFamilyActivity({
					...
					description: `Created budget for "${(budget as any).category}"`,
				} as any);
				complianceMonitor.logEvent({
					userId: req.user!.id,
					eventType: "admin_action" as any,
