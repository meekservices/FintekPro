...
export function registerBondTradingOrderPart3Part2Routes(app: Express): void {
	...
	app.get("/api/aif/all-exchanges", requireLevel2, async (req, res) => {
		try {
			...
			const [nseData, bseData, mcxData, ncdexData, mseiData] =
				await Promise.all([
					nseResponse.json(),
					bseResponse.json(),
					mcxResponse.json(),
			...
			if (exchange && exchange !== "all") {
				allFunds = allFunds.filter(
					(fund) => fund.exchange.toLowerCase() === exchange.toLowerCase(),
				);
			}
			...
			const marketStats = {
				...
				totalAUM: allFunds.reduce((sum, fund) => {
					const aum = Number.parseFloat(fund.aum.replace(/[₹,\sCr]/g, ""));
					return sum + aum;
				}, 0),
				averageReturns: {
					"1Y": (
						allFunds.reduce((sum, fund) => sum + fund.returns["1Y"], 0) /
						allFunds.length
					).toFixed(1),
					"3Y": (
						allFunds.reduce((sum, fund) => sum + (fund.returns["3Y"] || 0), 0) /
						allFunds.filter((f) => f.returns["3Y"]).length
					).toFixed(1),
					"5Y": (
						allFunds.reduce((sum, fund) => sum + (fund.returns["5Y"] || 0), 0) /
						allFunds.filter((f) => f.returns["5Y"]).length
					).toFixed(1),
				},
				categoryDistribution: {
					"Category I": allFunds.filter((f) => f.category === "Category I")
						.length,
					"Category II": allFunds.filter((f) => f.category === "Category II")
						.length,
					"Category III": allFunds.filter((f) => f.category === "Category III")
						.length,
				},
				riskDistribution: {
					High: allFunds.filter((f) => f.riskRating?.includes("High")).length,
					Medium: allFunds.filter((f) => f.riskRating?.includes("Medium"))
						.length,
					Low: allFunds.filter((f) => f.riskRating?.includes("Low")).length,
				},
				topPerformer: allFunds.reduce(
					(max, fund) => (fund.returns["1Y"] > max.returns["1Y"] ? fund : max),
					allFunds[0],
				),
			};
	...
	app.get("/api/nsdl/holdings", async (req, res) => {
		try {
			...
			const nsdlHoldings = [
				{
					...
					totalCostValue: 600187.5,
					unrealizedGainLoss: 24812.5,
			...
			const filteredHoldings = isin
				? nsdlHoldings.filter((h) => h.isin === isin)
				: nsdlHoldings;
			...
			const summary = {
				...
				averageGainLossPercentage: (
					filteredHoldings.reduce((sum, h) => sum + h.gainLossPercentage, 0) /
					filteredHoldings.length
				).toFixed(2),
				totalPledgedValue: filteredHoldings.reduce(
					(sum, h) => sum + h.pledgedQuantity * h.currentPrice,
					0,
				),
			};
