	...
	try {
		...
		const whereClause = clientId
			? and(
					...
					eq(unifiedOrders.userId, clientId),
				)
			: eq(unifiedOrders.createdByAgentId, agentId);
	...
	try {
		...
		if (!consentConfirmed) {
			return res
				.status(400)
				.json({
					error: "Client consent must be confirmed before placing an order",
				});
		}
		if (!clientId)
		...
		if (!action)
			return res
				.status(400)
				.json({ error: "Order action (buy/sell) is required" });
