...
import { PortalType, PORTAL_BRAND_CONFIG, resolvePortalType } from '@shared/portal';
import { db } from '../db';
...
router.get('/api/system/portal-logo/:type', (req: Request, res: Response) => {
  ...
  const portalType = validTypes.includes(type) ? type as PortalType : PortalType.MAIN;
...
router.get('/api/admin/portal-access-logs', async (req: Request, res: Response) => {
  try {
    ...
    const logs = await db.select().from(portalAccessLog)
      .orderBy(sql`${portalAccessLog.accessedAt} DESC`)
      .limit(limit)
      .offset(offset);

    const total = await db.select({ count: sql<number>`count(*)` }).from(portalAccessLog);
...
export function registerPortalSystemRoutes(app: any) {
  app.use(router);
  console.log('✅ Portal System routes registered (portal-meta, portal-logo, portal-access-log)');
}
