	...
	async (req: Request, res: Response) => {
		try {
			...
			const pendingUsers = await db
				...
				.where(
					eq(
						users.appointmentStatus,
						APPOINTMENT_STATUSES.PENDING_ADMIN_APPROVAL,
					),
				)
				...
				pendingUsers.map(async (user) => {
					...
					if (user.appointmentInitiatedBy) {
						...
						if (initiator.length > 0) {
							initiatorName =
								`${initiator[0].firstName || ""} ${initiator[0].lastName || ""}`.trim() ||
								"Unknown";
						}
					}
					return {
...
router.post(
	"/api/admin/appointments/:userId/approve",
	async (req: Request, res: Response) => {
		try {
			...
			if (
				user.appointmentStatus !== APPOINTMENT_STATUSES.PENDING_ADMIN_APPROVAL
			) {
...
router.post(
	"/api/admin/appointments/:userId/reject",
	async (req: Request, res: Response) => {
		try {
			const { userId } = req.params;
...
router.post(
	"/api/admin/appointments/:userId/suspend",
	async (req: Request, res: Response) => {
		try {
			const { userId } = req.params;
			const { reason } = req.body;
			...
			if (!reason || reason.trim().length === 0) {
				return res
					.status(400)
					.json({ error: "Suspension reason is mandatory" });
			}
...
router.get(
	"/api/admin/appointments/audit-trail",
	async (req: Request, res: Response) => {
		try {
			const {
				userId,
...
router.get("/api/appointments/my-team", async (req: Request, res: Response) => {
	try {
		...
		res.json({
			teamMembers,
			statusCounts: {
				pending: teamMembers.filter(
					(m) =>
						m.appointmentStatus === APPOINTMENT_STATUSES.PENDING_ADMIN_APPROVAL,
				).length,
				active: teamMembers.filter(
					(m) => m.appointmentStatus === APPOINTMENT_STATUSES.ACTIVE,
				).length,
				rejected: teamMembers.filter(
					(m) => m.appointmentStatus === APPOINTMENT_STATUSES.REJECTED,
				).length,
				suspended: teamMembers.filter(
					(m) => m.appointmentStatus === APPOINTMENT_STATUSES.SUSPENDED,
				).length,
			},
		});
	} catch (error: any) {
...
router.post("/api/appointments/create", async (req: Request, res: Response) => {
	try {
		...
		const initiatorName = currentUser
