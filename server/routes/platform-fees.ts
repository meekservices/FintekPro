...
router.get("/", requireAdmin, async (req, res) => {
  try {
    ...
    let query = db.select().from(platformFeeConfig);
    ...
    const filtered = fees.filter(fee => {
      ...
      if (isActive !== undefined && fee.isActive !== (isActive === 'true')) return false;
      return true;
    });
    ...
    res.json({ 
      ...
      data: filtered,
      grouped,
      categories: ['regulatory', 'platform', 'advisory', 'document', 'convenience', 'value_added'],
    });
  } catch (error: any) {
...
router.post("/", requireAdmin, async (req, res) => {
  try {
    ...
    const [fee] = await db.insert(platformFeeConfig)
      .values({
        ...data,
        createdBy: userId,
        updatedBy: userId,
...
router.put("/:id", requireAdmin, async (req, res) => {
  try {
    ...
    const [fee] = await db.update(platformFeeConfig)
      .set({
        ...data,
        updatedAt: new Date(),
        updatedBy: userId,
