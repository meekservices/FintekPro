...
function classifyAssetClass(holding: any): string {
	...
	if (type === "gold" || type === "silver" || productType === "gold")
		return "gold";
	...
	if (type === "aif" || type === "pms" || type === "alternative")
		return "alternatives";
	return "equity";
}
...
router.get("/portfolio-drift", requireAuth, async (req, res) => {
	try {
		...
		if (agentClients.length === 0) {
			return res.json({
				clients: [],
				summary: { highDrift: 0, avgDrift: 0, rebalancedThisMonth: 0 },
			});
		}
		...
		const portfolioUserMap = new Map(
			allPortfolios.map((p) => [p.id, p.userId]),
		);
		const portfolioUpdatedMap = new Map(
			allPortfolios.map((p) => [p.userId || "", p.updatedAt]),
		);
		...
		const outcomes = await db
			...
			.where(
				and(
					eq(agentPortfolioOutcomes.agentId, agentId),
					inArray(agentPortfolioOutcomes.clientId, clientIds),
				),
			);
		...
		for (const client of agentClients) {
			...
			const totalValue = clientHoldings.reduce(
				(sum, h) => sum + Number.parseFloat(String(h.currentValue || 0)),
				0,
			);
			...
			for (const h of clientHoldings) {
				...
				const val = Number.parseFloat(String(h.currentValue || 0));
				currentAllocation[cls] =
					(currentAllocation[cls] || 0) + (val / totalValue) * 100;
			}
			...
			for (const cls of Object.keys(targetAllocation)) {
				driftScore += Math.abs(
					(currentAllocation[cls] || 0) - (targetAllocation[cls] || 0),
				);
			}

			const lastRebalancedAt = portfolioUpdatedMap.get(client.id) || null;
			const rebalancedThisMonth =
				lastRebalancedAt && new Date(lastRebalancedAt) >= startOfMonth;
			...
			result.push({
				...
				currentAllocation: Object.fromEntries(
					Object.entries(currentAllocation).map(([k, v]) => [
						k,
						Math.round(v * 10) / 10,
					]),
				),
		...
		const avgDrift =
			result.length > 0
				? Math.round(
						(result.reduce((s, r) => s + r.driftScore, 0) / result.length) * 10,
					) / 10
				: 0;
		const rebalancedThisMonth = result.filter(
			(r) => r.rebalancedThisMonth,
		).length;

		res.json({
			clients: result,
			summary: { highDrift, avgDrift, rebalancedThisMonth },
		});
	} catch (err) {
