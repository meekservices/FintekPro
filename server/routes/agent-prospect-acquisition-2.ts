...
export function registerAgentProspectAcquisitionPart2Routes(
	app: Express,
): void {
	app.post(
		"/api/agent/prospect-clients/:id/upload-portfolio",
		requireAgent,
		async (req: any, res) => {
			try {
				...
				if (!holdings || !Array.isArray(holdings)) {
					return res
						.status(400)
						.json({ message: "Holdings array is required" });
				}
		...
		async (req: any, res) => {
			try {
				...
				if (!portfolio) {
					return res
						.status(400)
						.json({ message: "No portfolio data available for analysis" });
				}

				const holdings =
					"holdings" in portfolio
						? portfolio.holdings
						: "parsedHoldings" in portfolio
...
8. riskScore - risk level score 0-100`;
				...
				try {
					...
					const fallbackAnalysis = {
						...
						assetAllocationBreakdown: {
							equity: { value: totalValue * 0.6, percentage: 60 },
							debt: { value: totalValue * 0.3, percentage: 30 },
							cash: { value: totalValue * 0.1, percentage: 10 },
						},
						concentrationRisk: {
							topHoldingConcentration: 35,
							sectorConcentration: { financials: 25, technology: 20 },
							alerts: ["High concentration in top 3 holdings"],
						},
						performanceVsBenchmark: {
						...
						externalVsFintekpro: {
							externalPercentage: 85,
							fintekproPercentage: 15,
						},
						gapAnalysis: [
							{
								gap: "No international diversification",
								severity: "medium",
								recommendation: "Add 10-15% international equity exposure",
							},
							{
								gap: "Limited fixed income allocation",
								severity: "low",
								recommendation: "Consider adding corporate bonds for stability",
							},
						],
						overallScore: 65,
						riskScore: 55,
	...
	app.get(
		"/api/agent/proposals/:id/interactions",
		requireAgent,
		async (req: any, res) => {
			try {
				const { id } = req.params;
				...
				res
					.status(500)
					.json({ message: "Failed to fetch proposal interactions" });
			}
		},
	);
		...
		async (req: any, res) => {
			try {
				...
				if (!type || !content || !senderType) {
					return res
						.status(400)
						.json({ message: "type, content, and senderType are required" });
				}
	...
	app.get(
		"/api/agent/proposals/:id/approval",
		requireAgent,
		async (req: any, res) => {
			try {
				const { id } = req.params;
				...
				if (!approval) {
					return res.json({
						...
						message: "No approval request found for this proposal",
					});
				}
		...
		async (req: any, res) => {
			try {
				...
				clientsWithPortfolio.forEach((c: any) => {
					if (
						c.fetchedPortfolio &&
						typeof c.fetchedPortfolio === "object" &&
						"totalValue" in c.fetchedPortfolio
					) {
						totalAUM += (c.fetchedPortfolio as any).totalValue || 0;
					}
					if (
						c.uploadedPortfolio &&
						typeof c.uploadedPortfolio === "object" &&
						"totalValue" in c.uploadedPortfolio
					) {
				...
				const proposalConversionRate =
					proposalTotal > 0
						? ((proposalConverted / proposalTotal) * 100).toFixed(1)
						: "0.0";
				...
				res.json({
					...
					proposals: {
						...
						conversionRate: Number.parseFloat(proposalConversionRate),
					},
				...
				res
					.status(500)
					.json({ message: "Failed to fetch acquisition metrics" });
			}
		},
	);
