...
router.patch(
	...
	async (req: Request, res: Response) => {
		try {
			...
			console.log(
				`[Admin] Updating company ${id} with fields:`,
		...
		try {
			...
			if (!Array.isArray(companyIds) || companyIds.length === 0) {
				return apiResponse.badRequest(
					res,
					"companyIds must be a non-empty array",
				);
			}
			...
			if (!["active", "inactive", "delisted"].includes(status)) {
				return apiResponse.badRequest(
					res,
					"Invalid status. Must be active, inactive, or delisted",
				);
			}
			...
			for (const companyId of companyIds) {
				try {
					...
					console.log(
						`[Admin Bulk] Updated company ${companyId} status to ${status}`,
					);
				} catch (err: any) {
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (!priceChange || !["fixed", "percentage"].includes(priceChange.mode)) {
				return apiResponse.badRequest(
					res,
					"Invalid priceChange. Must include mode (fixed or percentage) and value",
				);
			}

			if (
				typeof priceChange.value !== "number" ||
				Number.isNaN(priceChange.value)
			) {
				return apiResponse.badRequest(
					res,
					"priceChange.value must be a valid number",
				);
			}
			...
			for (const companyId of companyIds) {
				try {
					...
					if (newPrice < 0) {
						errors.push(
							`Company ${companyId}: calculated price would be negative`,
						);
						failedCount++;
						continue;
					}
					...
					console.log(
						`[Admin Bulk] Updated company ${companyId} price from ₹${currentPrice} to ₹${newPrice}`,
					);
				} catch (err: any) {
					errors.push(`Failed to update ${companyId}: ${err.message}`);
					failedCount++;
			...
			return apiResponse.serverError(
				res,
				"Failed to perform bulk price update",
			);
		}
	},
);
		...
		try {
			...
			for (const company of allCompanies) {
				...
				if (redFlags.length > 0) {
					highRiskCompanies++;
					if (
						redFlags.includes("negative_net_worth") ||
						redFlags.includes("very_high_leverage")
					) {
						criticalAlerts++;
					}
				}
			...
			const totalAlerts =
				criticalAlerts + blockedTrades + kycFailures + highRiskCompanies;
			...
			return apiResponse.serverError(
				res,
				"Failed to fetch compliance statistics",
			);
		}
	},
);
				...
				if (listing.status === "rejected") {
					...
					alerts.push({
						...
						userName: seller
							...
							: "Unknown",
						tradeValue:
							(Number.parseFloat(listing.askPrice) || 0) * listing.quantity,
				...
				if (request.status === "rejected") {
					...
					alerts.push({
						...
						userName: buyer
							...
							: "Unknown",
						tradeValue:
							(Number.parseFloat(request.maxPrice) || 0) * request.quantity,
...
router.get(
	...
	async (req: Request, res: Response) => {
		try {
			...
			const paginatedListings = enrichedListings.slice(
				offset,
				offset + limitNum,
			);
...
router.get(
	...
	async (req: Request, res: Response) => {
		try {
			...
			const paginatedRequests = enrichedRequests.slice(
				offset,
				offset + limitNum,
			);
