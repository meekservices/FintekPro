	...
	try {
		...
		if (!clientProfile || !product || !investmentAmount) {
			res.status(400).json({
				success: false,
				error:
					"Missing required fields: clientProfile, product, investmentAmount",
			});
			return;
		}
...
router.post("/explain/portfolio", async (req: Request, res: Response) => {
	try {
		...
		const explanation = explainabilityEngine.explainPortfolio(
			clientProfile,
			products,
		);
		...
		data: {
			disclosures: [
				...
				},
				{
					...
					content:
						"Investment advice is provided in accordance with SEBI (Investment Advisers) Regulations, 2013. We are a SEBI-registered Investment Adviser.",
				...
				},
				{
					...
					content:
						"We may receive commissions from product manufacturers. These commissions do not influence our recommendations which are based solely on suitability assessment.",
				...
				},
				{
					...
					content:
						"All investment recommendations are made based on a thorough suitability assessment considering your risk profile, investment objectives, and financial situation.",
					regulatoryReference:
	...
	async (_req: Request, res: Response) => {
		res.json({
			...
			data: {
				riskCategories: [
					{
						...
						color: "green",
						description:
							"Principal is relatively safe. Returns may be lower but more predictable.",
						suitableFor:
							"Conservative investors, retirement corpus, emergency funds",
						typicalProducts: [
							"Liquid funds",
							"Money market funds",
							"Government bonds",
					...
					},
					{
						...
						color: "lightgreen",
						description:
							"Relatively stable with occasional minor fluctuations.",
					...
					},
					{
						...
						color: "yellow",
						description:
							"Balanced risk-return profile with moderate fluctuations.",
						suitableFor:
							"Moderate risk investors, medium-term goals (3-5 years)",
						typicalProducts: [
							"Hybrid funds",
							"Dynamic bond funds",
					...
					},
					{
						...
						color: "orange",
						description:
							"Higher return potential with significant fluctuations possible.",
					...
					},
					{
						...
						color: "red",
						description:
							"High volatility with potential for both significant gains and losses.",
						suitableFor:
							"Very aggressive investors, long-term wealth creation (7+ years)",
						typicalProducts: [
							"Mid cap funds",
							"Small cap funds",
							"Sectoral funds",
				...
				regulatoryNote:
					"Risk categorization is based on SEBI guidelines for product labeling and riskometer.",
				lastUpdated: new Date().toISOString(),
			},
		});
	},
	...
	res.json({
		...
		data: {
			factors: [
				{
					...
					weight: 30,
					description:
						"Match between client risk appetite and product risk level",
					assessmentMethod:
						"Risk profiling questionnaire score compared to product riskometer",
				},
				{
					name: "Investment Horizon",
					weight: 25,
					description:
						"Client investment timeline vs product recommended holding period",
					assessmentMethod:
						"Years to goal compared to product lock-in and recommended duration",
				},
				{
					name: "Income Adequacy",
					weight: 20,
					description:
						"Client financial capacity to invest and sustain the investment",
				...
				},
				{
					...
					assessmentMethod:
						"KYC tier level comparison (Basic < Enhanced < Accredited)",
				},
				{
			...
			thresholds: {
				highlySuitable: {
					minScore: 85,
					description: "Strong match across all factors",
				},
				suitable: { minScore: 70, description: "Good match with minor gaps" },
				moderatelySuitable: {
					minScore: 50,
					description: "Acceptable but with reservations",
				},
				notSuitable: {
					minScore: 0,
					description: "Significant mismatches - not recommended",
				},
			},
		},
	});
...
router.get("/suitability/compliance", async (_req: Request, res: Response) => {
	res.json({
		...
		data: {
			...
			retentionPeriod:
				"5 years from date of advice or client relationship end, whichever is later",
		},
	});
});
		...
		data: {
			categories: [
				...
				},
				{
					...
					description:
						"Portfolio diversification benefit from adding this product",
					impactWeight: 15,
				},
				{
					code: "tax_efficiency",
				...
				},
				{
					...
					name: "Market Context",
					description:
						"Current market conditions favoring/disfavoring the product",
					impactWeight: 10,
				},
				{
					code: "regulatory",
...
router.post("/scenario/analyze", async (req: Request, res: Response) => {
	try {
		...
		const scenarios = [
			{
				...
				projectedValue:
					investmentAmount * (1 + (expectedReturn + vol * 0.7) / 100) ** years,
			...
			},
			{
				...
				projectedValue: investmentAmount * (1 + expectedReturn / 100) ** years,
			...
			},
			{
				...
				projectedValue:
					investmentAmount * (1 + (expectedReturn - vol * 0.8) / 100) ** years,
			...
			},
			{
				...
				projectedValue: Math.max(
					...
					investmentAmount * (1 + (expectedReturn - vol * 2) / 100) ** years,
				),
		...
		const weightedValue = scenarios.reduce(
			(sum, s) => sum + (s.projectedValue * s.probability) / 100,
			0,
		);
		...
		res.json({
			...
			data: {
				...
				riskMetrics: {
					valueAtRisk95: Math.round(
						investmentAmount * (1 - (vol * 1.65) / 100),
					),
...
router.post("/audit/log-explanation", async (req: Request, res: Response) => {
	try {
		...
		const auditEntry = {
			...
			suitabilityScore:
				explanation?.suitabilityExplanation?.suitabilityScore || 0,
			disclosuresProvided:
				explanation?.disclosures?.map((d: any) => d.type) || [],
