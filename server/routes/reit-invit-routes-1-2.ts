...
router.get("/eligibility/:userId", async (req: Request, res: Response) => {
	try {
		...
		const requirement =
			KYC_TIER_REQUIREMENTS[assetType as string] || KYC_TIER_REQUIREMENTS.reit;
		...
		res.json({
			...
			restrictions: eligible
				? []
				: ["Cannot place orders until KYC requirements are met"],
		});
	} catch (error) {
...
const orderSchema = z
	...
	.refine(
		(data) =>
			...
			(data.limitPrice !== undefined && data.limitPrice > 0),
		{
...
router.post("/orders", async (req: Request, res: Response) => {
	try {
		...
		let asset;
		if (assetType === "reit") {
			asset = SAMPLE_REITS.find(
				(r) => r.symbol.toLowerCase() === symbol.toLowerCase(),
			);
		} else {
			asset = SAMPLE_INVITS.find(
		...
		if (!asset) {
			return res
				...
				.json({
					success: false,
					error: `${assetType.toUpperCase()} not found`,
				});
		}
...
router.get("/holdings/:userId", async (req: Request, res: Response) => {
	try {
		...
		const totalGain = sampleHoldings.reduce(
			(sum, h) => sum + h.unrealizedGain,
			0,
		);
		const avgYield =
			sampleHoldings.length > 0
				? (
						sampleHoldings.reduce(
							(sum, h) => sum + Number.parseFloat(h.distributionYield),
							0,
						) / sampleHoldings.length
					).toFixed(2)
				: "0";
		...
		res.json({
			...
			summary: {
				...
				invitCount: sampleHoldings.filter((h) => h.assetType === "invit")
					.length,
			},
		});
	} catch (error) {
...
router.get("/store/reits/admin", async (req: Request, res: Response) => {
	try {
		const dbReits = await db
			.select()
			.from(reits)
			.orderBy(desc(reits.createdAt));
...
router.get("/store/invits/admin", async (req: Request, res: Response) => {
	try {
		const dbInvits = await db
			.select()
			.from(invits)
			.orderBy(desc(invits.createdAt));
