...
import {
	...
	type Region,
} from "../services/market-holiday-service";
	...
	try {
		...
		const options = {
			...
			instrumentTypes: instrumentTypes
				? (instrumentTypes as string).split(",")
				: undefined,
...
router.patch("/events/:id", async (req, res) => {
	try {
		const event = await financialCalendarService.updateEvent(
			req.params.id,
			req.body,
		);
...
router.get("/export/ical", async (req, res) => {
	try {
		...
		const icalFeed = await financialCalendarService.generateICalFeed({
			...
			months: months ? Number.parseInt(months as string, 10) : 6,
		});

		res.setHeader("Content-Type", "text/calendar; charset=utf-8");
		res.setHeader(
			"Content-Disposition",
			"attachment; filename=fintekpro-bond-calendar.ics",
		);
		res.send(icalFeed);
	} catch (error) {
...
router.get("/events/:id/google-calendar", async (req, res) => {
	try {
		...
		const googleCalendarUrl =
			financialCalendarService.generateGoogleCalendarUrl(event);
...
router.get("/holidays/bank", async (req, res) => {
	try {
		...
		const holidays = marketHolidayService.getUpcomingBankHolidays(
			Number.parseInt(count as string, 10),
			region as Region,
		);
		...
		res
			.status(500)
			.json({ success: false, error: "Failed to fetch bank holidays" });
	}
});
...
router.get("/holidays/trading", async (req, res) => {
	try {
		...
		res
			.status(500)
			.json({ success: false, error: "Failed to fetch trading holidays" });
	}
});
...
router.get("/holidays/indian-markets", async (req, res) => {
	try {
		...
		res
			...
			.json({
				success: false,
				error: "Failed to fetch Indian market holidays",
			});
	}
});
...
router.get("/holidays/month/:year/:month", async (req, res) => {
	try {
		...
		res
			.status(500)
			.json({ success: false, error: "Failed to fetch holidays for month" });
	}
});
...
router.get("/holidays/check/:date", async (req, res) => {
	try {
		...
		const bankHoliday = marketHolidayService.getBankHoliday(
			date,
			region as Region,
		);
		const tradingHoliday = marketHolidayService.getHoliday(
			date,
			exchange as any,
		);
		const intlHoliday = marketHolidayService.getInternationalHoliday(
			date,
			exchange as any,
		);
...
router.get("/holidays/exchanges/:region", async (req, res) => {
	try {
		...
		const exchanges = marketHolidayService.getExchangesByRegion(
			region as Region,
		);
		...
		res
			.status(500)
			.json({ success: false, error: "Failed to fetch exchanges" });
	}
});
