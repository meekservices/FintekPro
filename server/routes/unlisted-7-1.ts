 ...
 */
router.post(
	"/admin/companies/bulk-enrich-mca",
	requireAdmin,
	async (req: Request, res: Response) => {
		try {
			...
			const companies = await storage.getUnlistedCompanies({
				status: "active",
			});
			...
			console.log(
				`📊 Bulk MCA enrichment: Processing ${toEnrich.length} companies`,
			);
			...
			for (const company of toEnrich) {
				try {
					const result = await enrichUnlistedCompanyWithMCAData(
						company.id,
						company.cin!,
					);
					if (result.success) {
			...
			return apiResponse.serverError(
				res,
				error.message || "Failed bulk MCA enrichment",
			);
		}
	},
);
 ...
 */
router.get(
	"/admin/companies/:id/test-enrichment",
	requireAdmin,
	async (req: Request, res: Response) => {
		try {
			const { id } = req.params;
			...
			console.log(
				`  Overall confidence: ${(enriched.overallConfidence * 100).toFixed(1)}%`,
			);
			...
			return apiResponse.serverError(
				res,
				error.message || "Test enrichment failed",
			);
		}
	},
);
 ...
 */
router.get(
	"/admin/companies/:id/financials/:metric/provenance",
	requireAdmin,
	async (req: Request, res: Response) => {
		try {
			...
			const usageFlags = await dataEnrichmentService.getDataUsageFlags(
				...
				(financialYear as string) ||
					`FY${new Date().getFullYear() - 1}-${new Date().getFullYear().toString().slice(-2)}`,
			);
 ...
 */
router.get(
	"/admin/companies/:id/data-usage-flags",
	requireAdmin,
	async (req: Request, res: Response) => {
		try {
			const { id } = req.params;
...
router.get(
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (source === "nse") {
				const nseReports =
					await dataEnrichmentService.fetchNSEFinancialReports(symbol);
				return apiResponse.success(res, nseReports);
			}
			if (source === "bse") {
				const bseReports = await dataEnrichmentService.fetchBSEFinancialReports(
					symbol,
					scripcode as string | undefined,
				);
				return apiResponse.success(res, bseReports);
			}
			const unifiedReports =
				await dataEnrichmentService.fetchUnifiedFinancialReports(
					symbol,
					scripcode as string | undefined,
				);
...
router.put(
	...
	async (req: Request, res: Response) => {
		try {
			...
			console.log(
				`[Peers] Updated ${listedPeers.length} listed peers for company ${id}`,
			);
 ...
 */
router.post(
	"/admin/companies/:id/auto-fetch-peers",
	requireAdmin,
	async (req: Request, res: Response) => {
		try {
			const { id } = req.params;
			...
			console.log(
				`[Auto-Fetch Peers] Starting for ${companyData.name}, sector: ${companyData.sector}, industry: ${companyData.industry}`,
			);
			...
			if (!symbolToUse) {
				...
				for (const [key, symbol] of Object.entries(sectorToSymbolMap)) {
					if (
						...
						industry.includes(key.toLowerCase()) ||
			...
			try {
				const recommendations =
					await yahooFinance.recommendationsBySymbol(symbolToUse);
				recommendedSymbols =
					recommendations.recommendedSymbols?.slice(0, maxPeers) || [];
			} catch (recError: any) {
				console.log(
					`[Auto-Fetch Peers] Could not get recommendations: ${recError.message}`,
				);
				...
				const fallbackSymbols =
					Object.entries(sectorFallbacks).find(([key]) =>
						sector.includes(key.toLowerCase()),
					)?.[1] || sectorFallbacks.Technology;

				recommendedSymbols = fallbackSymbols
					.slice(0, maxPeers)
					.map((s, i) => ({ symbol: s, score: 0.5 - i * 0.05 }));
			}
				...
				try {
					...
					if (quote) {
						...
						try {
							...
							console.log(
								`[Auto-Fetch Peers] Could not fetch stats for ${symbol}`,
							);
						}
						...
						listedPeers.push({
							...
							roe: keyStats?.financialData?.returnOnEquity
								? keyStats.financialData.returnOnEquity * 100
								: undefined,
							roce: keyStats?.financialData?.returnOnAssets
								? keyStats.financialData.returnOnAssets * 100
								: undefined,
							debtEquity: keyStats?.financialData?.debtToEquity
								? keyStats.financialData.debtToEquity / 100
								: undefined,
							revenueGrowth: keyStats?.financialData?.revenueGrowth
								? keyStats.financialData.revenueGrowth * 100
								: undefined,
						});
					}
				} catch (quoteError: any) {
					console.log(
						`[Auto-Fetch Peers] Error fetching ${rec.symbol}: ${quoteError.message}`,
					);
				}
			}
			...
			if (listedPeers.length > 0) {
				...
				const existingPeers = Array.isArray(companyData.listedPeers)
					? companyData.listedPeers
					: [];
				const existingTickers = new Set(
					existingPeers.map((p: any) => p.ticker?.toUpperCase()),
				);
				const newPeers = listedPeers.filter(
					(p) => !existingTickers.has(p.ticker.toUpperCase()),
				);
				...
				console.log(
					`[Auto-Fetch Peers] Added ${newPeers.length} new peers for ${companyData.name}`,
				);
			...
			return apiResponse.serverError(
				res,
				`Failed to auto-fetch peers: ${error.message}`,
			);
		}
	},
);
