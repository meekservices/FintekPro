...
router.get("/zoho-oauth-url", async (req, res) => {
	try {
		...
		res.json({
			...
			oauthUrl,
			instructions:
				"Visit the URL to authorize Zoho Meeting. After authorization, copy the refresh token from the callback response and update ZOHO_REFRESH_TOKEN secret.",
		});
	} catch (error: any) {
		res.status(500).json({
			success: false,
...
router.get("/zoho-oauth-callback", async (req, res) => {
	try {
		...
		const tokens = await zohoMeetingService.exchangeCodeForTokens(
			code,
			redirectUri,
		);
		...
		res.json({
			success: true,
			message:
				"Authorization successful! Copy the refresh_token below and update your ZOHO_REFRESH_TOKEN secret.",
	...
	try {
		...
		if (
			...
			return res
				.status(403)
				.json({
					error: "Access denied. Only agents/partners can schedule meetings.",
				});
		}
		...
		res
			.status(500)
			.json({ error: error.message || "Failed to schedule meeting" });
	}
});
...
router.get("/", async (req, res) => {
	try {
		...
		const isAgent = userRoles.some((r: string) =>
			["agent", "admin", "partner"].includes(r),
		);
	...
	try {
		...
		if (booking[0].clientId !== userId && booking[0].agentId !== userId) {
			...
			if (!userRoles.includes("admin")) {
				return res
					.status(403)
					.json({ error: "Not authorized to cancel this meeting" });
			}
		}
...
router.patch("/:id/cancel", async (req, res) => {
	try {
		...
		if (booking[0].zohoMeetingId) {
			await videoConferencingService.cancelMeeting(booking[0].zohoMeetingId, undefined, userId);
		}
