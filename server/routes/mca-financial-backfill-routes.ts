...
import { z } from "zod";
import {
	mcaFinancialBackfillService,
	FinancialDataInput,
} from "../services/mca-financial-backfill-service";
...
router.get("/coverage/fields", async (req: Request, res: Response) => {
	try {
		const fieldStats =
			await mcaFinancialBackfillService.getFieldCoverageStats();
...
router.get("/coverage/needs-backfill", async (req: Request, res: Response) => {
	try {
		const companies =
			await mcaFinancialBackfillService.getCompaniesNeedingBackfill();
...
router.get("/company/:cin", async (req: Request, res: Response) => {
	try {
		...
		const summary =
			await mcaFinancialBackfillService.getFinancialSummaryByCIN(cin);
...
router.post("/backfill-bulk", async (req: Request, res: Response) => {
	try {
		...
		if (!Array.isArray(cins) || cins.length === 0) {
			return res
				.status(400)
				.json({ success: false, error: "cins array is required" });
		}

		if (cins.length > 50) {
			return res
				.status(400)
				.json({ success: false, error: "Maximum 50 CINs per request" });
		}

		const user = (req as any).user;
		const result = await mcaFinancialBackfillService.backfillBulk(
			cins,
			user?.email || "admin",
		);
...
router.post("/import-bulk", async (req: Request, res: Response) => {
	try {
		...
		if (!Array.isArray(data) || data.length === 0) {
			return res
				.status(400)
				.json({ success: false, error: "data array is required" });
		}

		if (data.length > 500) {
			return res
				.status(400)
				.json({ success: false, error: "Maximum 500 records per import" });
		}
...
router.post("/scheduler/trigger", async (req: Request, res: Response) => {
	try {
		const { cins } = req.body;
		const result =
			await mcaFinancialRefreshScheduler.triggerManualRefresh(cins);
		res.json({ success: true, ...result });
	} catch (error: any) {
		res.status(500).json({ success: false, error: error.message });
	}
