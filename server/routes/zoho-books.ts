...
import {
	zohoTransactionSyncService,
	ProductType,
} from "../services/zoho-transaction-sync-service";

// Role-based access middleware for admin and accounts team
const requireAdminOrAccounts = async (
	req: Request,
	res: Response,
	next: NextFunction,
) => {
	...
	if (!allowedRoles.includes(userRole.toLowerCase())) {
		return res
			.status(403)
			.json({ error: "Access denied. Admin or Accounts role required." });
	}
...
export function registerZohoBooksRoutes(app: Express) {
		...
		try {
			...
			if (!service) {
				return res.json({
					connected: false,
					message:
						"Zoho Books not configured. Please set ZOHO_CLIENT_ID, ZOHO_CLIENT_SECRET, ZOHO_REFRESH_TOKEN, and ZOHO_ZSOID.",
				});
			}
	...
	app.get(
		"/api/admin/zoho-books/bank-accounts/:id/transactions",
		async (req, res) => {
			try {
				const service = await getZohoBooksService();
				if (!service) {
	...
	app.get("/api/admin/zoho-books/reports/aging/:type", async (req, res) => {
		try {
			...
			const report = await service.getAgingReport(type, {
	...
	app.post(
		"/api/admin/zoho-books/sync/mutual-fund/:orderId",
		async (req, res) => {
			try {
				const result = await zohoTransactionSyncService.syncMutualFundOrder(
					req.params.orderId,
				);
				res.json(result);
			} catch (error: any) {
	...
	app.post("/api/admin/zoho-books/sync/bond/:orderId", async (req, res) => {
		try {
			const result = await zohoTransactionSyncService.syncBondOrder(
				req.params.orderId,
			);
			res.json(result);
		} catch (error: any) {
	...
	app.post(
		"/api/admin/zoho-books/sync/ipo/:applicationId",
		async (req, res) => {
			try {
				const result = await zohoTransactionSyncService.syncIPOApplication(
	...
	app.post("/api/admin/zoho-books/sync/unlisted/:dealId", async (req, res) => {
		try {
			const result = await zohoTransactionSyncService.syncUnlistedDeal(
				req.params.dealId,
			);
			res.json(result);
		} catch (error: any) {
	...
	app.post(
		"/api/admin/zoho-books/sync/store/:transactionId",
		async (req, res) => {
			try {
				const result = await zohoTransactionSyncService.syncStoreTransaction(
	...
	app.post("/api/admin/zoho-books/sync/store-batch", async (req, res) => {
		try {
			...
			const result =
				await zohoTransactionSyncService.syncPendingStoreTransactions(
	...
	app.get(
		"/api/admin/commission-reconciliation",
		requireAdminOrAccounts,
		async (req, res) => {
			try {
				...
				const summary = {
					...
					totalAmount: items
						.reduce((sum, i) => sum + Number.parseFloat(i.amount || "0"), 0)
						.toFixed(2),
					syncedAmount: items
			...
			try {
				...
				const summary = {
					totalPending: payouts
						...
						.reduce((sum, p) => sum + Number.parseFloat(p.netAmount || "0"), 0)
						.toFixed(2),
	...
	app.post(
		"/api/admin/commission-payouts/:id/approve",
		requireAdminOrAccounts,
		async (req, res) => {
			try {
	...
	app.get("/api/admin/payouts", requireAdminOrAccounts, async (req, res) => {
		try {
			...
			const { partnerSettlements, users, referralPayoutTransactions } =
			...
			const settlements = await db
				...
				.orderBy(desc(partnerSettlements.createdAt))
				.limit(100);
			...
			const payouts = await Promise.all(
				settlements.map(async (s) => {
					...
					const [user] = await db
						...
						.from(users)
						.where(eq(users.id, Number.parseInt(s.partnerId || "0")))
						.limit(1);
	...
	console.log(
		"   💰 Commission reconciliation: /api/admin/commission-reconciliation",
	);
}
