...
const INBOUND_PRODUCTS_SEED = [
	{
		...
		description:
			"Category III Alternative Investment Fund focused on listed Indian technology stocks with long-short strategies.",
		...
		taxImplications:
			"No capital gains tax in IFSC. Tax holiday for fund manager.",
		isPublished: true,
		isPremium: true,
		isLimited: false,
	},
	{
		...
		description:
			"Unlisted infrastructure investments across highways, ports, renewable energy and logistics.",
		category: "Alternative Investment Funds",
	...
	},
	{
		...
		description:
			"Early-stage technology investments in Indian startups. Focus on fintech, deeptech and climate-tech.",
		category: "Alternative Investment Funds",
		...
		taxImplications:
			"Long-term capital gains treatment. Carried interest taxed favorably.",
		isPublished: true,
		isPremium: true,
		isLimited: true,
	},
	{
		...
		description:
			"Listed REIT investing in Grade A commercial properties across Mumbai, Bengaluru and Hyderabad.",
		category: "Alternative Investment Funds",
		subcategory: "REIT",
		flowDirection: "inbound",
		regulatoryFramework: "IFSCA Fund Management",
		...
		taxImplications:
			"Dividend distribution taxed at 10% for foreign investors.",
		isPublished: true,
		isPremium: false,
		isLimited: false,
	},
	{
		...
		description:
			"Foreign currency fixed deposits with IFSC Banking Units. Various tenors from 3 months to 5 years.",
	...
	},
	{
		...
		description:
			"Access to Indian sovereign and top-rated corporate bonds listed on India International Exchange.",
	...
	},
	{
		...
		description:
			"Trade in international gold and silver markets through IIBX. Physical delivery available.",
	...
	},
	{
		...
		description:
			"US Dollar denominated life insurance with investment component. Global coverage.",
		...
		taxImplications:
			"Death benefit tax-free. Maturity taxable per investor jurisdiction.",
		isPublished: true,
		isPremium: true,
		isLimited: false,
	},
...
const OUTBOUND_PRODUCTS_SEED = [
	{
		...
		description:
			"Diversified global portfolio investing in US, European and Asian markets under LRS limits.",
		category: "Alternative Investment Funds",
		...
		regulatoryBenefits: [
			"Compliant with RBI LRS",
			"Tax-efficient IFSC structure",
		],
		eligibility: [
			"Resident Indians",
			"HNIs",
			"Retail investors with LRS quota",
		],
		complianceRequirements: [
			"PAN verification",
			"Form A2 declaration",
		...
		taxImplications:
			"Capital gains taxable in India. TCS applicable above ₹7L.",
		isPublished: true,
		isPremium: false,
		isLimited: false,
	},
	{
		...
		description:
			"Low-cost index fund tracking the S&P 500 with direct investment in US blue-chip stocks.",
		category: "Alternative Investment Funds",
	...
	},
	{
		...
		description:
			"Concentrated portfolio of leading global technology companies - FAANG, semiconductors, AI.",
		category: "Alternative Investment Funds",
		...
		complianceRequirements: [
			...
			"Form A2",
			"Risk profiling",
			"LRS utilization check",
		],
		taxImplications:
			"High-volatility asset. LTCG taxable at 20% with indexation.",
		isPublished: true,
		isPremium: true,
		isLimited: false,
	},
	{
		...
		description:
			"Physical gold-backed ETF providing exposure to international gold prices.",
		category: "Global Trading",
		...
		taxImplications:
			"Taxed as non-equity mutual fund. LTCG at 20% after 3 years.",
		isPublished: true,
		isPremium: false,
		isLimited: false,
	},
	{
		...
		description:
			"Investment-grade international bonds including US Treasuries and European sovereigns.",
		category: "Structured Products",
		...
		taxImplications:
			"Interest taxed at slab rate. LTCG at 20% with indexation.",
		isPublished: true,
		isPremium: false,
		isLimited: false,
	},
	{
		...
		description:
			"Diversified European equity exposure covering UK, Germany, France, Switzerland.",
		category: "Alternative Investment Funds",
		...
		taxImplications:
			"LTCG at 20% with indexation. Euro dividends taxable at slab rate.",
		isPublished: true,
		isPremium: false,
		isLimited: false,
	},
	{
		...
		description:
			"Exposure to high-growth emerging markets excluding India - China, Brazil, Indonesia.",
		category: "Alternative Investment Funds",
	...
	},
	{
		...
		description:
			"Investment in leading global pharma, biotech, medical devices companies.",
		category: "Alternative Investment Funds",
		subcategory: "Sector Fund",
		flowDirection: "outbound",
		regulatoryFramework: "FEMA LRS (Liberalised Remittance Scheme)",
	...
	},
	{
		...
		description:
			"Comprehensive family office setup and management services in GIFT City.",
		...
		taxImplications:
			"Structure-dependent tax treatment. Professional advice required.",
		isPublished: true,
		isPremium: true,
		isLimited: true,
	},
	{
		...
		description:
			"Special Purpose Vehicle for aircraft leasing operations with IFSC tax benefits.",
		...
		regulatoryBenefits: [
			"No withholding on lease rentals",
			"No MAT",
			"GST exemption",
		],
		eligibility: [
		...
		taxImplications:
			"Lease income exempt in IFSC. Depreciation benefits available.",
		isPublished: true,
		isPremium: true,
		isLimited: true,
	},
...
router.get("/", async (req: Request, res: Response) => {
	try {
		...
		const conditions = [eq(giftCityProducts.isPublished, true)];
...
router.post("/admin/bulk-seed", async (req: Request, res: Response) => {
	try {
		...
		switch (template) {
			...
			case "aif":
				productsToSeed = [
					...INBOUND_PRODUCTS_SEED,
					...OUTBOUND_PRODUCTS_SEED,
				].filter((p) => p.category === "Alternative Investment Funds");
				break;
			case "global-funds":
				productsToSeed = OUTBOUND_PRODUCTS_SEED.filter(
					(p) =>
						p.subcategory?.includes("Fund") || p.subcategory?.includes("ETF"),
				);
				break;
			default:
				return res
					...
					.json({
						error:
							"Invalid template. Valid options: inbound, outbound, all, aif, global-funds",
					});
		}
		...
		const newProducts = productsToSeed.filter(
...
router.post(
	"/admin/maintenance/refresh-pricing",
...
router.post(
	"/admin/maintenance/cleanup-duplicates",
	async (req: Request, res: Response) => {
		try {
...
router.put(
	...
	async (req: Request, res: Response) => {
		try {
			...
			res
				.status(400)
				.json({ error: error.message || "Failed to set interval" });
		}
	},
);
