...
const exitLoadSyncScheduler = {
	getStatus: async () => ({}),
	runFullEnrichment: async () => ({}),
};

export function registerUserProfileKYCPart2Routes(app: Express): void {
	app.post(
		"/api/admin/trigger-rekyc-reminders",
		requireAdmin,
		async (req, res) => {
			try {
				const { triggerReKYCRemindersManually } = await import(
					"../rekyc-cron"
				).catch(() => ({ triggerReKYCRemindersManually: async () => ({}) }));
	...
	app.post(
		"/api/admin/instrument-time-series/daily-update",
		requireAdmin,
		async (req, res) => {
			try {
				const { runDailyPriceUpdate } = await import(
					"../services/instrument-time-series/daily-price-updater"
				);
				...
				res
					...
					.json({
						success: false,
						error: error.message || "Daily price update failed",
					});
			}
		},
	);
	...
	app.post(
		"/api/admin/instrument-time-series/historical-backfill",
		requireAdmin,
		async (req, res) => {
			try {
				const batchSize = Number.parseInt(req.query.batchSize as string) || 5;
				const { runHistoricalBackfill } = await import(
					"../services/instrument-time-series/historical-backfill-service"
				);
				...
				res
					...
					.json({
						success: false,
						error: error.message || "Historical backfill failed",
					});
			}
		},
	);
	...
	app.get(
		"/api/admin/instrument-time-series/job-log",
		requireAdmin,
		async (req, res) => {
			try {
	...
	app.get(
		"/api/admin/instrument-time-series/retry-queue",
		requireAdmin,
		async (req, res) => {
			try {
	...
	app.get(
		"/api/admin/instrument-time-series/status",
		requireAdmin,
		async (req, res) => {
			try {
		...
		try {
			...
			if (
				...
				return res.status(400).json({
					error:
						"Missing required fields: PAN, first name, and last name are required",
				});
			}
			...
			const progressData = {
				...
				step2Data: onboardingData.aadhar
					? { aadhar: onboardingData.aadhar }
					: null,

				// Step 3: Account discovery (banking)
				step3AccountsDiscovered: !!(
					onboardingData.bankName && onboardingData.accountNumber
				),
				step3BankAccountsFound:
					onboardingData.bankName && onboardingData.accountNumber ? 1 : 0,
			...
			console.log(
				`[Onboarding] Smart KYC completed for user ${userId}, assigned tier: ${kycTier}`,
			);
			...
			res.json({
				...
				tierUpgrade:
					kycTier === "enhanced"
						? "Congratulations! You now have access to F&O, Commodities, and Global investments."
						: "Complete your financial profile to unlock Enhanced KYC and more investment options.",
			});
		} catch (error: any) {
	...
	app.get(
		"/api/profile/kyc-tier/requirements/:tier",
		requireClientOrHigher,
		async (req, res) => {
			try {
				const { getTierUpgradeRequirements } = await import(
					"../kyc-tier-service"
				);
				const userId = req.user!.id;
	...
	app.get(
		"/api/profile/kyc-tier/product-access",
		requireClientOrHigher,
		async (req, res) => {
			try {
	...
	app.post(
		"/api/profile/kyc-tier/upgrade-enhanced",
		requireClientOrHigher,
		async (req, res) => {
			try {
	...
	app.post(
		"/api/profile/kyc-tier/request-accredited",
		requireClientOrHigher,
		async (req, res) => {
			try {
				const { requestAccreditedInvestorVerification } = await import(
					"../kyc-tier-service"
				);
				const userId = req.user!.id;
				...
				const result = await requestAccreditedInvestorVerification(
					userId,
					verificationType,
				);
				...
				console.error(
					"Error requesting Accredited Investor verification:",
					error,
				);
				res.status(500).json({
					success: false,
	...
	app.post(
		"/api/profile/kyc-tier/verify-accredited",
		requireAdmin,
		async (req, res) => {
			try {
				const { verifyAccreditedInvestor } = await import(
					"../kyc-tier-service"
				);
				...
				const result = await verifyAccreditedInvestor(
					userId,
					verifiedBy,
					approved,
					rejectionReason,
	...
	app.get(
		"/api/profile/kyc-tier/product-prompt/:productCode",
		requireClientOrHigher,
		async (req, res) => {
			try {
