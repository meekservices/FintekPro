...
} from "../services/recommendation-products-service";
...
router.get("/", requireAdmin, async (req, res) => {
	try {
		...
		const query = db.select().from(recommendationProducts);
		const conditions = [];
...
router.get("/by-category", async (req, res) => {
	try {
		...
		if (!productType || !riskProfile) {
			return res
				.status(400)
				.json({ error: "productType and riskProfile are required" });
		}
		...
		console.error(
			"[RecommendationProducts] Error fetching by category:",
			error,
		);
		res.status(500).json({ error: error.message });
	}
	...
	try {
		...
		if (productType === "listed_stock") {
			sourceData = await db
				.select()
				.from(listedStocks)
				.where(eq(listedStocks.isPublished, true));
	...
	try {
		...
		const unlistedSeeds = [
			...
			},
			{
				...
				bseUrl: "https://www.bseindia.com/stock-share-price/hdb-financial-services-ltd/HDBFS/544429/",
				nseUrl: "https://www.nseindia.com/get-quotes/equity?symbol=HDBFS",
				sector: "NBFC",
				category: "Listed Stocks - NBFC",
				riskProfile: "moderate",
				riskLevel: "Moderate",
				returns1Y: "15.2",
				returns3Y: "18.5",
...
router.get("/stats", requireAdmin, async (req, res) => {
	try {
		const stats = await db
			...
			.from(recommendationProducts)
			.groupBy(
				recommendationProducts.productType,
				recommendationProducts.riskProfile,
			);
...
publicRouter.get("/with-rationale", async (req, res) => {
	try {
		...
		console.error(
			"[RecommendationProducts] Error fetching with rationale:",
			error,
		);
		res.status(500).json({ error: error.message });
	}
