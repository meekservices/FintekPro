...
import { sendPushToUser, sendPushToUsers, notify } from '../services/fcm-notification-service';
...
function requireAdminOrService(req: Request, res: Response, next: Function) {
  ...
  if (mobileUser?.roles?.some((r: string) => ['admin', 'super_admin'].includes(r))) {
    return next();
  }
  // Web session path
  const sessionUser = (req as any).user;
  if (sessionUser?.roles?.some((r: string) => ['admin', 'super_admin'].includes(r))) {
    return next();
  }
  ...
  return res.status(403).json({
    success: false,
    error: { error_code: 'FORBIDDEN', message: 'Admin access required', retryable: false }
  });
}

// ── POST /api/notifications/send ──────────────────────────────────────────
notificationsRouter.post('/send', requireAdminOrService, async (req: Request, res: Response) => {
  const { userId, userIds, title, body, data, imageUrl } = req.body;

  if (!title || !body) {
    return res.status(400).json({
      success: false,
      error: { error_code: 'MISSING_FIELDS', message: 'title and body are required', retryable: false }
    });
  }
  ...
  try {
    ...
    if (userIds && Array.isArray(userIds) && userIds.length > 0) {
      ...
      return res.status(400).json({
        success: false,
        error: { error_code: 'MISSING_TARGET', message: 'userId or userIds required', retryable: false }
      });
    }

    return res.json({
      success: result.success,
      data: { successCount: result.successCount, failureCount: result.failureCount, errors: result.errors },
      meta: { timestamp: new Date().toISOString(), version: '1.0' }
    });
  } catch (err: any) {
    return res.status(500).json({
      success: false,
      error: { error_code: 'PUSH_ERROR', message: err.message, retryable: true }
    });
  }
});
...
notificationsRouter.post('/test', requireAdminOrService, async (req: Request, res: Response) => {
  const sessionUser = (req as any).user;
  const mobileUser = (req as any).mobileUser;
  const userId = sessionUser?.id ?? mobileUser?.userId;

  if (!userId) {
    return res.status(401).json({ success: false, error: { error_code: 'UNAUTHENTICATED', message: 'Login required', retryable: false } });
  }
