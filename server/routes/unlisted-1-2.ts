		...
		try {
			...
			if (user?.role !== "admin" && user?.role !== "agent") {
				return apiResponse.forbidden(
					res,
					"Only Agents and Admins can trigger deep research",
				);
			}
			...
			const isStale =
				...
				Date.now() - new Date(company.lastSyncedAt).getTime() > STALE_THRESHOLD;

			if (!company || isStale || forceRefresh) {
				console.log(
					`[Unlisted Research] Syncing ${cin} from Credhive (reason: ${!company ? "new" : isStale ? "stale" : "forced"})`,
				);

				// Fetch profile
				const profileResp = await credhiveService.getCompanyProfile(
					cin,
					forceRefresh,
				);
				...
				const financialsResp = await credhiveService.getFinancials(
					cin,
					forceRefresh,
				);
				...
				for (const finData of financialsData) {
					...
					const existing = await storage.getCompanyFinancialsByYear(
						companyId,
						finData.financial_year,
					);
					if (existing) {
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (!cin) {
				return apiResponse.badRequest(
					res,
					"Company does not have a CIN — required for Credhive sync",
				);
			}
			...
			if (!isinResult.isin) {
				try {
					...
					const mcResult = await moneyControlScraper.searchISINByCompanyName(
						company.name,
					);
					...
					if (mcResult.isin && mcResult.matchScore >= 60) {
						...
						console.log(
							`[Unlisted Sync] Auto-populated ISIN ${mcResult.isin} from MoneyControl for ${company.name} (${mcResult.matchScore.toFixed(1)}% match)`,
						);
						...
						if (mcResult.price) {
							try {
								...
								console.log(
									`[Unlisted Sync] Also saved price ₹${mcResult.price} from MoneyControl`,
								);
							} catch (priceErr) {
								console.warn(
									"[Unlisted Sync] Failed to save MoneyControl price:",
									priceErr,
								);
							}
						}
						...
						console.log(
							`[Unlisted Sync] MoneyControl: No good match for "${company.name}" (best: ${mcResult.matchScore.toFixed(1)}%)`,
						);
					}
				} catch (mcError) {
					console.warn(
						"[Unlisted Sync] Failed to fetch ISIN from MoneyControl:",
						mcError,
					);
				}
			}
			...
			if (!isinResult.isin) {
				try {
					const { nsdlISINService } = await import(
						"../services/nsdl-isin-service"
					);
					// Search all security types for unlisted companies (they might be equity, preference, etc.)
					const nsdlResults = await nsdlISINService.searchByCompanyName(
						company.name,
						{
							securityType: "all",
							limit: 10,
					...
					console.log(
						`[Unlisted Sync] NSDL search for "${company.name}" returned ${nsdlResults.length} results`,
					);
					if (nsdlResults.length > 0) {
						console.log(
							`[Unlisted Sync] Best match: ${nsdlResults[0].issuerName} (${nsdlResults[0].matchScore}% match) - ISIN: ${nsdlResults[0].isin} - Type: ${nsdlResults[0].securityType}`,
						);
					}
					...
					const equityMatch = nsdlResults.find(
						(r) => r.securityType === "equity" && r.matchScore >= 60,
					);
					const bestMatch =
						equityMatch ||
						(nsdlResults.length > 0 && nsdlResults[0].matchScore >= 60
							? nsdlResults[0]
							: null);
					...
					if (bestMatch) {
						...
						console.log(
							`[Unlisted Sync] Auto-populated ISIN ${bestMatch.isin} from NSDL for ${company.name} (${bestMatch.matchScore}% match, type: ${bestMatch.securityType})`,
						);
					} else if (nsdlResults.length > 0) {
						console.log(
							`[Unlisted Sync] NSDL: Best match score (${nsdlResults[0].matchScore}%) below 60% threshold, skipping`,
						);
					}
				} catch (nsdlError) {
					console.warn(
						"[Unlisted Sync] Failed to fetch ISIN from NSDL:",
						nsdlError,
					);
			...
			for (const ratioData of ratiosData) {
				...
				const existing = await storage.getCompanyRatiosByYear(
					companyId,
					ratioData.financialYear,
				);
				if (existing) {
			...
			await storage.updateUnlistedCompany(companyId, {
				...
				incorporationDate:
					credhiveDetails.date_of_incorporation || company.incorporationDate,
				paidUpCapital:
					credhiveDetails.paid_up_capital?.toString() || company.paidUpCapital,
				authorizedCapital:
					credhiveDetails.authorized_capital?.toString() ||
			...
			const isAutoPopulated =
				isinResult.source === "moneycontrol" || isinResult.source === "nsdl";
			const sourceLabel =
				isinResult.source === "moneycontrol"
					? "MoneyControl"
					: isinResult.source === "nsdl"
						? "NSDL"
						: null;
			...
			return apiResponse.serverError(
				res,
				error.message || "Failed to sync company data",
			);
		}
	},
);
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (companies.length === 0) {
				return apiResponse.success(res, {
					success: true,
					message: "No companies with CIN to sync",
					results: [],
				});
			}

			console.log(
				`[Bulk Sync] Starting Credhive sync for ${companies.length} companies...`,
			);
			...
			for (const company of companies) {
				try {
					console.log(
						`[Bulk Sync] Processing: ${company.name} (${company.cin})`,
					);
					...
					if (!profileResp.success || !profileResp.data) {
						results.push({
							...
							success: false,
							credhiveLinked: false,
							message: "Not found on Credhive",
						});
						continue;
					}
					const credhiveDetails = profileResp.data;
					...
					if (!isin) {
						try {
							...
							const mcResult =
								await moneyControlScraper.searchISINByCompanyName(company.name);
							if (mcResult.isin && mcResult.matchScore >= 60) {
								isin = mcResult.isin;
								console.log(
									`[Bulk Sync] Auto-populated ISIN ${isin} from MoneyControl for ${company.name}`,
								);
								if (mcResult.price) {
									const today = new Date();
									today.setHours(0, 0, 0, 0);
							...
							console.warn(
								`[Bulk Sync] MoneyControl ISIN lookup failed for ${company.name}`,
							);
						}
					}
					...
					for (const finData of financialsData) {
						const dbFin = {
							...
							totalDebt: finData.total_debt?.toString() ?? null,
							operatingCashFlow:
								finData.operating_cash_flow?.toString() ?? null,
							dataSource: "credhive",
						};
						const existing = await storage.getCompanyFinancialsByYear(
							company.id,
						...
						existing
							? await storage.updateCompanyFinancials(existing.id, dbFin)
							: await storage.createCompanyFinancials(dbFin);
					}
					...
					for (const ratioData of ratiosData) {
						...
						existing
							? await storage.updateCompanyRatios(existing.id, dbRatio)
							: await storage.createCompanyRatios(dbRatio);
					}
					...
					await storage.updateUnlistedCompany(company.id, {
						...
						rocState: credhiveDetails.roc_state || company.rocState,
						incorporationDate:
							credhiveDetails.date_of_incorporation ||
							company.incorporationDate,
						paidUpCapital:
							credhiveDetails.paid_up_capital?.toString() ||
							company.paidUpCapital,
						authorizedCapital:
							credhiveDetails.authorized_capital?.toString() ||
							company.authorizedCapital,
						faceValue:
							credhiveDetails.face_value?.toString() || company.faceValue,
						totalShares: credhiveDetails.total_shares || company.totalShares,
						website: credhiveDetails.website || company.website,
						description: credhiveDetails.description || company.description,
					...
					console.error(
						`[Bulk Sync] Error syncing ${company.name}:`,
			...
			console.log(
				`[Bulk Sync] Completed: ${successCount} success, ${failedCount} failed`,
			);
	...
	async (req: Request, res: Response) => {
		try {
			...
			return apiResponse.created(
				res,
				priceHistory,
				"Price history entry added successfully",
			);
		} catch (error: any) {
 ...
 */
router.post(
	"/companies/:id/price-history/bulk",
	requireAdmin,
	async (req: Request, res: Response) => {
		try {
			const { id } = req.params;
