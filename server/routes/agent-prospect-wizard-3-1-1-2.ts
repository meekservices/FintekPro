...
router.post("/proposal-analytics", async (req: Request, res: Response) => {
	try {
		...
		if (requestedSections.includes("PRIORITY_RECOMMENDATIONS")) {
			const cgData =
				analytics.capitalGains?.data ||
				calculateCapitalGains(normalizedHoldings);
			const rhData =
				analytics.riskHeatmap?.data ||
				calculateRiskHeatmap(normalizedHoldings, totalValue);
			analytics.priorityRecommendations = {
				data: generatePriorityRecommendations(
					normalizedHoldings,
					riskProfile,
					cgData,
					rhData,
		...
		if (requestedSections.includes("SIP_RECOMMENDATIONS")) {
			analytics.sipRecommendations = {
				data: await generateSipRecommendations(
					riskProfile,
					analysis,
					Array.isArray(investmentGoals) ? investmentGoals : [],
					Array.isArray(selectedCategories) ? selectedCategories : undefined,
				),
		...
		console.error(
			"[Agent Wizard] Error calculating proposal analytics:",
			error,
		);
...
} from "./agent-prospect-wizard-3-1-1-analytics";
