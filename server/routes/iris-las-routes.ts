...
export function registerIrisLasRoutes(app: Express): void {
	 ...
	 */
	app.get(
		"/api/iris/las/mf/eligibility/:pan",
		requireAuth,
		async (req, res) => {
			...
			if (!parsed.success)
				return apiError(
					...
					400,
					"INVALID_PAN",
					parsed.error.issues[0]?.message ?? "Invalid PAN",
				);

			const folioNos = req.query.folioNos
				? String(req.query.folioNos).split(",")
				: undefined;
	 ...
	 */
	app.get(
		"/api/iris/las/securities/eligibility/:pan",
		requireAuth,
		async (req, res) => {
			const user = getUser(req);
			const pan = req.params.pan.toUpperCase();
	...
	app.post("/api/iris/las/mf/pledge", requireAuth, async (req, res) => {
		...
		const { pan, folioDetails, loanAmount, lenderCode, clientUserId } =
			parsed.data;
		...
		await wrap(res, user.id, "IRIS_LAS_MF_PLEDGE", () =>
			irisLasService.initiateMfPledge({
				userId: targetUserId,
				pan,
				folioDetails,
				loanAmount,
	...
	app.post("/api/iris/las/securities/pledge", requireAuth, async (req, res) => {
		...
		const { pan, dpId, securities, loanAmount, lenderCode, clientUserId } =
			parsed.data;
		const targetUserId = clientUserId ?? user.id;
		const agentId = clientUserId ? user.id : undefined;

		await wrap(res, user.id, "IRIS_LAS_SEC_PLEDGE", () =>
			irisLasService.initiateSecuritiesPledge({
				userId: targetUserId,
				pan,
				dpId,
				securities,
	 ...
	 */
	app.get(
		"/api/iris/las/pledge/:pledgeId/status",
		requireAuth,
		async (req, res) => {
			const user = getUser(req);
	...
	app.post("/api/iris/las/loan/apply", requireAuth, async (req, res) => {
		...
		if (!pan)
			return apiError(
				...
				400,
				"MISSING_PAN",
				"PAN is required for LAS. Please complete KYC first.",
			);
	 ...
	 */
	app.get(
		"/api/iris/las/loan/:loanId/status",
		requireAuth,
		async (req, res) => {
			const user = getUser(req);
	 ...
	 */
	app.get(
		"/api/iris/las/loan/statement/:pan",
		requireAuth,
		async (req, res) => {
			const user = getUser(req);
			const pan = req.params.pan.toUpperCase();
	 ...
	 */
	app.post(
		"/api/iris/las/pledge/:pledgeId/release",
		requireAuth,
		async (req, res) => {
			const user = getUser(req);
			...
			await wrap(res, user.id, "IRIS_LAS_PLEDGE_RELEASE", () =>
				irisLasService.releasePledge({
					userId: user.id,
					localPledgeId: req.params.pledgeId,
