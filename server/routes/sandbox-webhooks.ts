...
function validateSignature(
	...
	if (!secret) {
		console.warn(
			"[Sandbox Webhook] WEBHOOK_SECRET not configured - allowing webhook in development mode",
		);
	...
	if (!signature) {
		console.error(
			"[Sandbox Webhook] Signature missing but secret is configured - rejecting webhook",
		);
...
async function handleITCalculatorPnlDone(
	data: any,
	transactionId: string,
	subType: string,
): Promise<void> {
	console.log(
		`[Sandbox Webhook] IT Calculator PnL (${subType}) completed: ${transactionId}`,
	);
	if (data?.job_id)
		console.log(
			`[Sandbox Webhook] Job ID: ${data.job_id}, Status: ${data.status}`,
		);
	if (data?.url)
		console.log(`[Sandbox Webhook] Result URL available for download`);
}

async function handleITCalculatorTaxPnlDone(
	data: any,
	transactionId: string,
	subType: string,
): Promise<void> {
	console.log(
		`[Sandbox Webhook] IT Calculator Tax PnL (${subType}) completed: ${transactionId}`,
	);
	if (data?.job_id)
		console.log(
			`[Sandbox Webhook] Job ID: ${data.job_id}, Status: ${data.status}`,
...
async function handleITCalculatorITRDone(
	data: any,
	transactionId: string,
): Promise<void> {
	console.log(
		`[Sandbox Webhook] IT Calculator ITR completed: ${transactionId}`,
	);
	if (data?.job_id)
		console.log(
			`[Sandbox Webhook] Job ID: ${data.job_id}, Status: ${data.status}`,
...
async function handleITReportTaxPnlDone(
	data: any,
	transactionId: string,
	subType: string,
): Promise<void> {
	console.log(
		`[Sandbox Webhook] IT Report Tax PnL (${subType}) completed: ${transactionId}`,
	);
	if (data?.url) console.log(`[Sandbox Webhook] Report download URL available`);
}

async function handleITReportCapitalGainsDone(
	data: any,
	transactionId: string,
	subType: string,
): Promise<void> {
	console.log(
		`[Sandbox Webhook] IT Report Capital Gains (${subType}) completed: ${transactionId}`,
	);
	if (data?.url)
		console.log(`[Sandbox Webhook] Capital gains report URL available`);
}
...
async function handleITOcrForm16Done(
	data: any,
	transactionId: string,
): Promise<void> {
	console.log(
		`[Sandbox Webhook] IT OCR Form 16 extraction completed: ${transactionId}`,
	);
	if (data?.extraction_details) {
		console.log(
			`[Sandbox Webhook] Employer: ${data.extraction_details?.employer?.name || "N/A"}`,
		);
		console.log(`[Sandbox Webhook] Confidence: ${data.confidence || "N/A"}`);
	}
}

async function handleITOcrForm26ASDone(
	data: any,
	transactionId: string,
): Promise<void> {
	console.log(
		`[Sandbox Webhook] IT OCR Form 26AS extraction completed: ${transactionId}`,
	);
	if (data?.extraction_details) {
		console.log(
			`[Sandbox Webhook] TDS entries: ${data.extraction_details?.tds_entries?.length || 0}`,
		);
	}
}
...
async function handleTDSAnalyticsDone(
	data: any,
	transactionId: string,
	variant: string = "tds",
): Promise<void> {
	console.log(
		`[Sandbox Webhook] TDS Analytics (${variant.toUpperCase()}) completed: ${transactionId}`,
	);
	if (data?.job_id)
		console.log(
			`[Sandbox Webhook] Job ID: ${data.job_id}, Status: ${data.status}`,
...
async function handleTDSReportDone(
	data: any,
	transactionId: string,
	variant: string = "tds",
): Promise<void> {
	console.log(
		`[Sandbox Webhook] TDS Report (${variant.toUpperCase()}) prepared: ${transactionId}`,
	);
	if (data?.url) console.log(`[Sandbox Webhook] Report download URL available`);
}

async function handleTDSComplianceDone(
	data: any,
	transactionId: string,
	action: string,
): Promise<void> {
	console.log(
		`[Sandbox Webhook] TDS Compliance ${action} completed: ${transactionId}`,
	);
	if (data?.job_id)
		console.log(
			`[Sandbox Webhook] Job ID: ${data.job_id}, Status: ${data.status}`,
...
async function handleTDS206ABDone(
	data: any,
	transactionId: string,
): Promise<void> {
...
async function handleGSTAnalyticsDone(
	data: any,
	transactionId: string,
	variant: string = "reconciliation",
): Promise<void> {
	console.log(
		`[Sandbox Webhook] GST Analytics (${variant}) completed: ${transactionId}`,
	);
	if (data?.job_id)
		console.log(
			`[Sandbox Webhook] Job ID: ${data.job_id}, Status: ${data.status}`,
	...
	if (data?.url)
		console.log(`[Sandbox Webhook] Reconciliation report URL available`);
}
...
async function handleKYCVerificationDone(
	data: any,
	transactionId: string,
	method: string = "generic",
): Promise<void> {
	console.log(
		`[Sandbox Webhook] KYC Verification (${method}) completed: ${transactionId}`,
	);
	if (data?.verified !== undefined)
		console.log(`[Sandbox Webhook] Verified: ${data.verified}`);
}
...
export function registerSandboxWebhookRoutes(app: Express): void {
	...
	app.post("/api/webhooks/sandbox", async (req: Request, res: Response) => {
		...
		if (WEBHOOK_SECRET && !rawBody) {
			console.error(
				"[Sandbox Webhook] Raw body not captured - middleware may be missing",
			);
		...
		if (!validation.valid) {
			console.error(
				`[Sandbox Webhook] Signature validation failed: ${validation.reason}`,
			);
		...
		try {
			...
			console.log(
				`[Sandbox Webhook] Event: ${event}, Transaction: ${transaction_id}`,
			);

			if (processedWebhooks.has(transaction_id)) {
				console.log(
					`[Sandbox Webhook] Duplicate webhook ignored: ${transaction_id}`,
				);
			...
			switch (event) {
				...
				case "it.calculator.pnl.securities.domestic.done":
					await handleITCalculatorPnlDone(
						data,
						transaction_id,
						"domestic_securities",
					);
				...
				case "it.calculator.tax_pnl.securities.domestic.done":
					await handleITCalculatorTaxPnlDone(
						data,
						transaction_id,
						"domestic_securities",
					);
				...
				case "it.report.tax_pnl.securities.domestic.done":
					await handleITReportTaxPnlDone(
						data,
						transaction_id,
						"domestic_securities",
					);
				...
				case "it.report.capital_gains.securities.domestic.done":
					await handleITReportCapitalGainsDone(
						data,
						transaction_id,
						"domestic_securities",
					);
				...
				default:
					console.warn(
						`[Sandbox Webhook] Unhandled event type: ${event} — transaction: ${transaction_id}`,
					);
					console.log(
	...
	app.get("/api/sandbox/health", async (req: Request, res: Response) => {
		const services = {
			...
			itr: {
				status: "operational",
				latency: Math.floor(Math.random() * 100) + 60,
			},
			gst: {
				status: "operational",
				latency: Math.floor(Math.random() * 100) + 70,
			},
			kyc: {
				status: "operational",
			...
			mca: {
				...
				endpoints: ["company-master-data", "director-master-data"],
			},
			webhooks: {
				status: "operational",
		...
		res.json({
			...
			services,
			overallStatus: Object.values(services).every(
	...
	app.get(
		"/api/tds/quarter-due-dates/:financialYear",
		async (req: Request, res: Response) => {
	...
	app.post("/api/kyc/mca/company", async (req: Request, res: Response) => {
		...
		if (!cinRegex.test(cleanCin)) {
			return res.status(400).json({
				success: false,
				error:
					"Invalid CIN format. Expected format: U/L + 5 digits + 2 letters + 4 digits + 3 letters + 6 digits (e.g. U12345AB2010PTC123456)",
			});
		}
		...
		try {
			...
			res
				...
				.json({
					success: false,
					error: error.message || "MCA company verification failed",
				});
		}
	});
	...
	app.post("/api/kyc/mca/director", async (req: Request, res: Response) => {
		...
		if (!/^\d{8}$/.test(cleanDin)) {
			return res.status(400).json({
				success: false,
				error:
					"Invalid DIN format. DIN must be exactly 8 digits (e.g. 01234567)",
			});
		}
		...
		try {
			...
			res.json({
				success: true,
				data,
				message: "Director master data retrieved successfully",
			});
		} catch (error: any) {
			...
			res
				...
				.json({
					success: false,
					error: error.message || "MCA director verification failed",
				});
		}
	});
	...
	console.log(
		"✅ Additional Sandbox API routes registered (GST, KYC, MCA Company, MCA Director, TDS, Health)",
	);
}
