...
export function registerRiskProfilesPartnerAppPart2Routes(app: Express): void {
	...
	app.post("/api/ocr/form26as", async (req, res) => {
		try {
			...
			const result = await sandboxITRService.parseForm26AS(
				fileBuffer,
				fileName || "form26as.pdf",
			);
	...
	app.post("/api/ocr/parse-document", async (req, res) => {
		try {
			...
			switch (documentType?.toLowerCase()) {
				...
				case "form_16":
				case "form-16":
					result = await sandboxITRService.parseForm16(
						fileBuffer,
						fileName || "form16.pdf",
					);
					break;
				...
				case "form-26as":
				case "26as":
					result = await sandboxITRService.parseForm26AS(
						fileBuffer,
						fileName || "form26as.pdf",
					);
					break;
				default: {
					...
					if (
						lowerFileName.includes("form16") ||
						lowerFileName.includes("form-16") ||
						lowerFileName.includes("form_16")
					) {
						result = await sandboxITRService.parseForm16(fileBuffer, fileName);
					} else if (
						lowerFileName.includes("26as") ||
						lowerFileName.includes("form26")
					) {
						result = await sandboxITRService.parseForm26AS(
							fileBuffer,
							fileName,
	...
	app.post("/api/tax-data/generate-report", async (req, res) => {
		try {
			...
			if (!pan || !year || !format) {
				return res
					.status(400)
					.json({ error: "PAN, year, and format are required" });
			}
	...
	app.post("/api/tax-data/export", async (req, res) => {
		try {
			...
			if (!pan || !year || !platform) {
				return res
					.status(400)
					.json({ error: "PAN, year, and platform are required" });
			}
		...
		try {
			...
			const guide = guides[platform as keyof typeof guides] || {
				...
				steps: [
					...
					"Import to your preferred tax software",
				],
				videoUrl: null,
				supportUrl: null,
			};
	...
	app.post("/api/sandbox-itr/auto-populate", async (req, res) => {
		try {
			...
			if (!pan || !assessmentYear) {
				return res
					.status(400)
					.json({ error: "PAN and assessment year are required" });
			}
