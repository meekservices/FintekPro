	...
	try {
		...
		if (data.source === "agent") {
			...
			if (!isAgent) {
				return res
					.status(403)
					.json({ error: "Only agents can add items with 'agent' source" });
			}
		...
		const targetUserId =
			data.source !== "client" && req.body.targetUserId
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (!clientUserId || !Array.isArray(items) || items.length === 0) {
				return res
					.status(400)
					.json({ error: "clientUserId and items array are required" });
			}
...
router.patch("/:id", requireAuth, async (req: Request, res: Response) => {
	try {
		...
		if (req.body.targetPrice !== undefined)
			updates.targetPrice = req.body.targetPrice;
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (item.userId !== userId) {
				return res
					.status(403)
					.json({ error: "Only the client can approve this item" });
			}

			if (item.source === "client") {
				return res
					.status(400)
					.json({ error: "Client-added items don't need approval" });
			}
