	...
	async (req: Request, res: Response): Promise<void> => {
		try {
			...
			if (!consentSessionId || !userId) {
				res
					.status(400)
					.json({ error: "consentSessionId and userId are required" });
				return;
			}

			// Step 1: Fetch fresh data from AA
			const fetchResult = await (aaService as any).fetchAllData(
				consentSessionId,
			);
			...
			if (fetchResult.aggregatedData?.mutualFunds?.length) {
				const mfHoldings = fetchResult.aggregatedData.mutualFunds.map(
					(mf: any) => ({
						id: nanoid(),
			...
			if (fetchResult.aggregatedData?.dematHoldings?.length) {
				const dematHoldings = fetchResult.aggregatedData.dematHoldings.map(
					(h: any) => ({
						id: nanoid(),
			...
			if (useStaging && allHoldings.length > 0) {
				...
				const stagingResponse = await fetch(
					`${process.env.APP_URL || "http://localhost:5000"}/api/portfolio/staging/create`,
					{
						method: "POST",
				...
				res.json({
					...
					summary: {
						mutualFundsCount:
							fetchResult.aggregatedData?.mutualFunds?.length || 0,
						dematHoldingsCount:
							fetchResult.aggregatedData?.dematHoldings?.length || 0,
				...
				const clearResult =
					await kycPortfolioMigrationService.onAutoSyncRefresh(userId, "all");
				console.log(
					`[AA] Cleared ${clearResult.clearedCount} holdings before fetch`,
				);
				...
				const dematHoldings = allHoldings.filter(
					(h) => h.source === "aa_demat",
				);

				if (mfHoldings.length) {
					totalStored +=
						await kycPortfolioMigrationService.storeAAFetchedHoldings(
							userId,
							portfolio.id,
							mfHoldings,
							"aa_mf",
...
router.post("/callback", async (req: Request, res: Response): Promise<void> => {
	try {
		const { consentHandleId, status } = req.body as {
...
router.get(
	...
	async (req: Request, res: Response): Promise<void> => {
		try {
			...
			const summary =
				await unifiedHoldingsReaderService.getPortfolioSummary(userId);
			const clientType =
				await unifiedHoldingsReaderService.getClientType(userId);
...
router.post(
	...
	async (req: Request, res: Response): Promise<void> => {
		try {
			const { consentSessionId, userId, reason } = req.body as {
				consentSessionId?: string;
				userId?: string;
				reason?: string;
			};
