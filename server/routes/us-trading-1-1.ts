...
import type {
	...
	AlpacaBrokerAccountPayload,
	AxiosLikeError,
} from "../types/broker-types";
import {
	extractErrorMessage,
	resolveRiskTolerance,
	resolveInvestmentObjective,
} from "../types/broker-types";
	...
	async (_req: Request, res: Response): Promise<void> => {
		try {
			...
			const totalValueUSD = positions.reduce(
				(sum, p) => sum + Number.parseFloat(p.market_value || "0"),
				0,
			);
			const totalGainLossUSD = positions.reduce(
				(sum, p) => sum + Number.parseFloat(p.unrealized_pl || "0"),
				0,
			);
			res.json({
				configured: true,
				...
				positions: positions.map((p) => ({
					...
					avgPrice: Number.parseFloat(p.avg_entry_price),
					currentPrice: Number.parseFloat(p.current_price),
					marketValue: Number.parseFloat(p.market_value),
					gainLoss: Number.parseFloat(p.unrealized_pl),
					gainLossPercent: Number.parseFloat(p.unrealized_plpc) * 100,
					side: p.side,
					currency: "USD",
				})),
				totalValueUSD,
				...
				totalGainLossPercent:
					totalValueUSD > 0
						? (totalGainLossUSD / (totalValueUSD - totalGainLossUSD)) * 100
						: 0,
			});
		} catch (error: unknown) {
...
router.patch(
	...
	async (req: Request, res: Response): Promise<void> => {
		try {
			...
			const success = await usTradingService.setFeatureFlag(
				flagName,
				isEnabled,
				userId,
			);
			if (success) {
				...
				res
					.status(400)
					.json({ success: false, error: "Failed to update flag" });
			}
		} catch (error: unknown) {
			res.status(500).json({ success: false, error: errorMessage(error) });
		}
	...
	try {
		...
		const financialYearParam =
			typeof req.query.financialYear === "string"
				...
				: undefined;
		const usage = await usTradingService.getLrsUsage(
			userId,
			financialYearParam,
		);
...
router.post(
	...
	async (req: Request, res: Response): Promise<void> => {
		try {
			...
			const account = await usTradingService.createBrokerAccount({
				clientId: userId,
			});
			res.json({ success: true, account });
		} catch (error: unknown) {
		...
		try {
			...
			if (!identity || !contact || !disclosures || !agreements?.length) {
				res
					...
					.json({
						success: false,
						error:
							"Missing required fields: identity, contact, disclosures, agreements",
					});
				return;
			}
			...
			const alpacaPayload: AlpacaBrokerAccountPayload = {
				...
				contact: {
					...
					street_address: Array.isArray(contact.streetAddress)
				...
				agreements: (agreements as AgreementInput[]).map(
					(a: AgreementInput) => ({
						...
						revision:
							a.revision ??
							process.env.ALPACA_AGREEMENT_REVISION ??
							"04.2021.10",
					}),
				),
			...
			try {
				alpacaAccount =
					await alpacaBrokerService.createBrokerAccount(alpacaPayload);
			} catch (err: unknown) {
				const errMsg = extractErrorMessage(
					err,
					"Alpaca account creation failed",
				);
				...
				return res
					...
					.json({
						success: false,
						error: errMsg,
						alpacaError: axErr?.response?.data,
					});
			}
			...
			if (!alpacaAccount)
				return res
					.status(500)
					.json({ success: false, error: "Account creation returned no data" });
			...
			if (alpacaAccount.id && compliance.kycComplete) {
				try {
					...
					await usTradingService.updateBrokerAccount(userId, {
						cipSubmittedAt: new Date(),
					});
				} catch (cipErr: unknown) {
					// CIP failure is non-fatal — account still created
					console.warn(
						"[AccountApply] CIP submission failed:",
						(cipErr as Error)?.message,
					);
				}
			}
...
router.get(
	...
	async (req: Request, res: Response): Promise<void> => {
		try {
			...
			if (
				brokerAccount.alpacaAccountId &&
				["SUBMITTED", "APPROVAL_PENDING", "ACTION_REQUIRED"].includes(
					brokerAccount.alpacaStatus || "",
				)
			) {
				try {
					const liveAccount = await alpacaBrokerService.getAccount(
						brokerAccount.alpacaAccountId,
					);
					if (
						liveAccount?.status &&
						liveAccount.status !== brokerAccount.alpacaStatus
					) {
	...
	async (req: Request, res: Response): Promise<void> => {
		try {
			...
			if (!brokerAccount?.alpacaAccountId) {
				res
					.status(404)
					.json({ success: false, error: "No Alpaca account found" });
				return;
			}

			const tradingAccount = await alpacaBrokerService.getTradingAccount(
				brokerAccount.alpacaAccountId,
			);
			if (!tradingAccount) {
				res
					...
					.json({
						success: false,
						error: "Unable to fetch trading account from Alpaca",
					});
				return;
			}
...
router.get(
	...
	async (req: Request, res: Response): Promise<void> => {
		try {
			...
			const symbolList = (typeof symbols === "string" ? symbols : "")
				.split(",")
				.map((s) => s.trim().toUpperCase());
			const quotes =
				await alpacaMarketDataService.getMultipleQuotes(symbolList);
			const fxRate = await alpacaMarketDataService.getUsdInrRate();

			const resultQuotes = quotes
				? [...quotes.values()].map((quote) => ({
						...quote,
						priceInr: quote.price * fxRate,
					}))
				: [];
	...
	async (req: Request, res: Response): Promise<void> => {
		try {
			...
			const instructions = {
				...
				beneficiary_account_note:
					"Use the account number from your FintekPro Funding Wallet",
				currency: "USD",
				...
				tcs_rate_percent: 20,
				tcs_note:
					"Your AD bank will deduct 20% TCS on LRS remittance above ₹7 lakh/FY (Finance Act 2023 §206C(1G)). Claim credit when filing ITR.",
				...
				steps: [
					{
						...
						description:
							"Go to Funding → Wallet in FintekPro to get your dedicated USD beneficiary account number and routing details assigned by Alpaca.",
					},
					{
						step: 2,
						title: "Visit your bank's LRS / forex desk (AD Category-I bank)",
						description:
							"Authorised dealers: SBI, HDFC, ICICI, Axis, Kotak, YES Bank, etc. Online option: Wise, HDFC Remit, Thomas Cook.",
					},
					{
						step: 3,
						title: "Fill Form A2",
						description:
							"Your bank will provide Form A2 (FEMA declaration). Purpose: 'Overseas portfolio investment in listed US equities under LRS'. Purpose code: S0001.",
					},
					{
						step: 4,
						title: "Provide PAN and KYC",
						description:
							"Your PAN is mandatory for LRS. Bank will verify annual LRS utilization against your PAN. Ensure PAN is linked to Aadhaar.",
					},
					{
						step: 5,
						title: "Initiate SWIFT wire transfer",
						description:
							"Send the USD amount (after TCS deduction or net of bank charges) to your Alpaca beneficiary account. Use the SWIFT BIC of the intermediary bank shown above.",
					},
					{
						step: 6,
						title: "Track settlement",
						description:
							"International SWIFT transfers typically settle in 2–5 business days. Check your FintekPro Funding Wallet for the deposit to appear.",
					},
					{
						step: 7,
						title: "Maintain ITR records",
						description:
							"Report US assets in Schedule FA of your Indian ITR. US dividends and capital gains must be reported in Schedule FSI. Consult a CA.",
					},
				],
...
router.get(
	...
	async (req: Request, res: Response): Promise<void> => {
		try {
			...
			let usdInrRate = 84;
			try {
				usdInrRate = await currencyExchangeService.convertAmount(
					1,
					"USD",
					"INR",
				);
			...
			const usedUsd = allBrokerAccounts.reduce(
				(sum, a) => sum + Number.parseFloat(a.lrsUsedUsd ?? "0"),
				0,
			);
			...
			const tcsAmountInr = tcsApplies ? (usedInr - TCS_THRESHOLD_INR) * 0.2 : 0;
	...
	async (req: Request, res: Response): Promise<void> => {
		try {
			...
			const results = await alpacaMarketDataService.searchSymbols(
				rawQuery,
				typeof rawLimit === "string" ? Number.parseInt(rawLimit, 10) || 10 : 10,
			);
	...
	try {
		...
		if (
			...
			return res
				...
				.json({
					success: false,
					error:
						"Missing required fields: account_owner_name, bank_account_type, bank_account_number, bank_routing_number",
				});
		}
		if (!["CHECKING", "SAVINGS"].includes(bank_account_type)) {
			return res
				...
				.json({
					success: false,
					error: "bank_account_type must be CHECKING or SAVINGS",
				});
		}
...
router.delete(
	"/funding/ach-relationships/:relationshipId",
	async (req: Request, res: Response): Promise<void> => {
		try {
			const userId = (req as AuthRequest).user?.id;
			if (!userId) {
		...
		try {
			...
			if (!amount || !direction) {
				res
					...
					.json({
						success: false,
						error: "Missing required fields: amount, direction",
					});
				return;
			}
...
router.get(
	...
	async (req: Request, res: Response): Promise<void> => {
		try {
			...
			const brokerAccount = await usTradingService.getBrokerAccount(userId);
			const events = alpacaSseService.getRecentEvents(
				brokerAccount?.alpacaAccountId || undefined,
				50,
			);
