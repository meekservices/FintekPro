...
const updateProposalItemSchema = z.object({
	...
	status: z
		.enum(["pending", "approved", "rejected", "modified", "removed"])
		.optional(),
...
router.post("/diagnostics", async (req, res) => {
	try {
		...
		if (
			req.body.userId &&
			["agent", "partner", "admin", "superadmin"].some((r: string) =>
				(requestingUser?.roles || [requestingUser?.role]).includes(r),
			)
		) {
...
router.post("/generate", requireAgent, async (req, res) => {
	try {
		...
		if (!diagId) {
			const diagnostics =
				await aiProposalEngine.runPortfolioDiagnostics(clientId);
			diagId = diagnostics.id;
		}

		const recommendations = await aiProposalEngine.generateRecommendations(
			clientId,
			diagId,
		);
...
router.get("/proposals/agent/:agentId", requireAgent, async (req, res) => {
	try {
		...
		const requestingUser = (req as any).user;
		const isAdmin = ["admin", "superadmin"].some((r: string) =>
			(requestingUser?.roles || [requestingUser?.role]).includes(r),
		);
...
router.put("/proposals/:id/status", async (req, res) => {
	try {
		...
		const roles: string[] =
			...
			(requestingUser?.role ? [requestingUser.role] : []);
		const actorRole =
			roles.includes("admin") || roles.includes("superadmin")
				? "agent"
				: roles.some((r: string) => ["agent", "partner"].includes(r))
					? "agent"
					: "client";
		...
		const proposal = await aiProposalEngine.updateProposalStatus(
			id,
			status,
			actorId,
			actorRole,
...
router.put("/items/:id", requireAgent, async (req, res) => {
	try {
		...
		const item = await aiProposalEngine.updateProposalItem(
			id,
			validated,
			actorId,
			"agent",
...
router.post("/proposals/:id/finalize", async (req, res) => {
	try {
		...
		const proposal = await aiProposalEngine.finalizeProposalApproval(
			id,
			clientId,
		);
		res.json(proposal);
...
router.get("/stats", requireAdmin, async (req, res) => {
	try {
		...
		const [diagnosticsRows, proposalsRows] = await Promise.all([
			userId
				? db
						.select({ healthScore: portfolioDiagnostics.healthScore })
			...
			userId
				...
				: db.select({ status: aiProposals.status }).from(aiProposals),
		]);
		...
		const avgHealthScore =
			diagnosticsRows.length > 0
				? Math.round(
						diagnosticsRows.reduce((sum, d) => sum + (d.healthScore || 0), 0) /
