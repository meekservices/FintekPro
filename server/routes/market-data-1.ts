...
export function registerMarketDataPart1Routes(app: Express): void {
		...
		async (_req: Request, res: Response): Promise<void> => {
			try {
				res.json({
					...
					score: 65,
					summary:
						"Market shows resilience near resistance levels. Institutional interest remains steady.",
	...
	app.get(
		...
		async (_req: Request, res: Response): Promise<void> => {
			...
			try {
				const { data, cached, cacheAge } =
					await marketMoversCache.getMarketMovers();
				const duration = Date.now() - startTime;

				res.setHeader("X-Cache", cached ? "HIT" : "MISS");
				res.setHeader(
					"X-Cache-Age",
					Math.round((cacheAge ?? 0) / 1000).toString(),
				);
				res.setHeader("X-Response-Time", `${duration}ms`);

				console.log(
					`📊 [MarketMovers] Response in ${duration}ms (cache: ${cached ? "HIT" : "MISS"})`,
				);
				res.json(data);
			} catch (err: unknown) {
	...
	app.get(
		"/api/admin/cache/market-movers/metrics",
		requireAdmin,
		async (_req: Request, res: Response): Promise<void> => {
			try {
				...
				res.json({
					cache: "market-movers",
					...metrics,
					cacheAgeSeconds: metrics.cacheAge
						? Math.round(metrics.cacheAge / 1000)
						: null,
			...
			try {
				...
				try {
					if (process.env.FINNHUB_API_KEY) {
						const finnhubQuote = await finnhubService.getQuote(
							symbol.toUpperCase(),
						);
						const data = finnhubService.transformQuoteToMarketData(
							symbol.toUpperCase(),
							finnhubQuote,
						);
						res.json(data);
	...
	app.get(
		...
		async (req: Request, res: Response): Promise<void> => {
			try {
				...
				const toTs = to ? queryInt(to, 0) : Math.floor(Date.now() / 1000);
				...
				try {
					if (process.env.FINNHUB_API_KEY) {
						...
						const data =
							finnhubService.transformCandlesToMarketCandles(finnhubCandles);
						res.json(data);
						return;
					}
				} catch (err) {
				...
				try {
					...
					const yahooResponse = await fetch(
						`https://query1.finance.yahoo.com/v8/finance/chart/${symbol.toUpperCase()}?period1=${fromTs}&period2=${toTs}&interval=${interval}`,
					);
					const yahooData = (await yahooResponse.json()) as any;
	...
	app.get(
		"/api/market/enhanced-quote/:symbol",
		async (req: Request, res: Response): Promise<void> => {
			try {
				const { symbol } = req.params;
	...
	app.get(
		"/api/market/company-profile/:symbol",
		async (req: Request, res: Response): Promise<void> => {
			try {
				const { symbol } = req.params;
					...
					news = [
						{
							id: Date.now(),
							title:
								"Market Analysis: Technology Stocks Show Mixed Performance",
							summary:
								"Technology sector continues to show volatility as investors react to earnings reports and economic indicators.",
							url: "#",
						...
						},
						{
							...
							summary:
								"The Federal Reserve announced it will maintain current interest rates amid ongoing economic monitoring.",
							url: "#",
	...
	app.get(
		...
		async (_req: Request, res: Response): Promise<void> => {
			try {
				...
				const promises = globalIndices.map(
					async (index): Promise<IndexQuote> => {
						try {
			...
			try {
				...
				if (process.env.FINNHUB_API_KEY) {
					const finnhubCategory =
						category === "all" ? "general" : queryStr(category, "general");
				...
				const curatedNews = [
					{
						...
						datetime: now - 3600,
						headline:
							"RBI keeps repo rate unchanged at 6.50% amid easing inflation",
						image: "",
						source: "RBI",
						summary:
							"The Reserve Bank of India maintained its key lending rate at 6.50%, citing a stable inflation trajectory and resilient domestic growth outlook for FY2026-27.",
					...
					},
					{
						...
						datetime: now - 7200,
						headline:
							"SEBI introduces new framework for mutual fund light regulation",
						image: "",
						source: "SEBI",
						summary:
							"Markets regulator SEBI has proposed a lighter regulatory framework for passively managed mutual fund schemes to reduce compliance costs and boost passive investing.",
					...
					},
					{
						...
						category: "general",
						datetime: now - 10800,
						headline: "India GDP growth accelerates to 7.2% in Q3 FY26",
						image: "",
						source: "MoSPI",
						summary:
							"India's economy grew at 7.2% year-on-year in the October-December quarter, driven by robust services sector activity and strong urban consumption demand.",
					...
					},
					{
						...
						category: "general",
						datetime: now - 14400,
						headline: "Nifty 50 hits all-time high as FII inflows surge",
						image: "",
						source: "NSE",
						summary:
							"The benchmark Nifty 50 index touched a fresh lifetime high, supported by sustained foreign institutional investor inflows and positive global sentiment.",
					...
					},
					{
						...
						datetime: now - 18000,
						headline:
							"SIP contributions cross Rs 25,000 crore monthly milestone",
						image: "",
						source: "AMFI",
						summary:
							"Systematic Investment Plan contributions to mutual funds crossed the Rs 25,000 crore monthly mark for the first time, reflecting growing retail investor participation.",
					...
					},
					{
						...
						category: "general",
						datetime: now - 21600,
						headline: "Government bonds rally as 10-year yield falls below 7%",
						image: "",
						source: "RBI",
						summary:
							"Indian government bond prices rallied with the benchmark 10-year yield dropping below the 7% level, buoyed by expectations of monetary easing in the near term.",
						url: "https://rbi.org.in",
					},
					{
						id: 7,
						category: "general",
						datetime: now - 25200,
						headline: "Union Budget 2026: Key highlights for investors",
						image: "",
						source: "PIB",
						summary:
							"The Union Budget for FY2026-27 introduced several investor-friendly measures including enhanced deductions for long-term capital gains and simplified TDS provisions.",
					...
					},
					{
						...
						category: "general",
						datetime: now - 28800,
						headline: "Gold ETFs see record inflows amid global uncertainty",
						image: "",
						source: "AMFI",
						summary:
							"Gold exchange-traded funds in India witnessed record monthly inflows as investors sought safe-haven assets amid geopolitical tensions and volatile equity markets.",
						url: "https://amfiindia.com",
					},
				];
				...
				const categories = [
					{
						id: "all",
						name: "All News",
						description: "Complete market and financial news coverage",
					},
					{
						id: "market",
						name: "Market Updates",
						description: "Index movements and market trends",
					},
					{
						id: "technology",
						name: "Technology",
						description: "IT and technology sector news",
					},
					{
						id: "ipo",
						name: "IPOs & Primary Market",
						description: "New listings and public offerings",
					},
					{
						id: "mutual_funds",
						name: "Mutual Funds",
						description: "Fund performance and industry news",
					},
					{
						id: "bonds",
						name: "Fixed Income",
						description: "Bond market and interest rate news",
					},
					{
						id: "sector",
						name: "Sector Analysis",
						description: "Industry-specific updates and trends",
					},
					{
						id: "commodities",
						name: "Commodities",
						description: "Gold, oil, and commodity price movements",
					},
					{
						id: "regulatory",
						name: "Regulatory Updates",
						description: "SEBI, RBI, and government policy changes",
					},
					{
						id: "global",
						name: "Global Markets",
						description: "International market developments",
					},
					{
						id: "earnings",
						name: "Earnings Reports",
						description: "Company quarterly results and guidance",
					},
					{
						id: "analyst_update",
						name: "Analyst Research",
						description: "Research reports and rating changes",
					},
					{
		...
		async (req: Request, res: Response): Promise<void> => {
			try {
				...
				res.json({
					...
					results: [],
					message:
						"News search requires Finnhub API integration. Configure FINNHUB_API_KEY for live news search.",
				});
			} catch (err: unknown) {
