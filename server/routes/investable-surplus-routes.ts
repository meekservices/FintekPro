...
import { db } from "../db";
import { incomeStreams, financialObligations, emergencyFunds } from "@shared/schema";
...
const obligationInputSchema = z.object({
  ...
  totalOutstanding: z.string().or(z.number()).optional().transform((val) => val ? String(val) : undefined),
...
router.post("/obligations", async (req, res) => {
  try {
    ...
    const [obligation] = await db.insert(financialObligations).values(data).returning();
    res.json(obligation);
  } catch (error: any) {
    if (error instanceof z.ZodError) {
      res.status(400).json({ error: "Validation error", details: error.errors });
...
router.post("/emergency-fund", async (req, res) => {
  try {
    ...
    const monthlyExpenses = parseFloat(data.monthlyExpenses as string || "0");
    const requiredFund = monthlyExpenses * 6;
    const currentFund = parseFloat(data.currentEmergencyFund as string || "0");
...
router.get("/surplus/calculate/:userId", async (req, res) => {
  try {
    const { userId } = req.params;
    const result = await investableSurplusEngine.calculateInvestableSurplus(userId);
    res.json(result);
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
...
router.get("/segment/:userId", async (req, res) => {
  try {
    ...
    if (!segment) {
      ...
      const calculated = await investableSurplusEngine.determineClientSegment(userId);
...
router.post("/segment/recalculate/:userId", async (req, res) => {
  try {
    ...
    const surplusResult = await investableSurplusEngine.calculateInvestableSurplus(userId);
    
    // Determine segment
    const segmentResult = await investableSurplusEngine.determineClientSegment(userId, clientType);
...
router.get("/summary/:userId", async (req, res) => {
  try {
    ...
    const [
      ...
      db.select().from(incomeStreams).where(and(eq(incomeStreams.userId, userId), eq(incomeStreams.isActive, true))),
      db.select().from(financialObligations).where(and(eq(financialObligations.userId, userId), eq(financialObligations.isActive, true))),
    ...
    const liveCalculation = await investableSurplusEngine.calculateInvestableSurplus(userId);
