	...
	try {
		...
		if (id) {
			...
			if (!validation.success) {
				return res
					.status(400)
					.json({ error: "Invalid data", details: validation.error.issues });
			}
			await db
				.update(aifMaster)
...
router.post("/aif/compare", async (req, res) => {
	try {
		...
		if (
			!schemeIds ||
			!Array.isArray(schemeIds) ||
			schemeIds.length < 2 ||
			schemeIds.length > 3
		) {
			return res
				.status(400)
				.json({ error: "Please provide 2-3 scheme IDs to compare" });
		}
		...
		const schemes = await db
			...
			.where(or(...schemeIds.map((id: string) => eq(aifMaster.id, id))));
...
router.post("/pms/compare", async (req, res) => {
	try {
		...
		const schemes = await db
			...
			.where(or(...schemeIds.map((id: string) => eq(pmsMaster.id, id))));
...
router.get("/products/search", async (req, res) => {
	try {
		const { query = "", productType = "all", limit = "20" } = req.query;
		...
		const results: any[] = [];
		const limitNum = Math.min(Number.parseInt(limit as string) || 20, 50);
		...
		if (productType === "all" || productType === "mutual_fund") {
			...
			results.push(
				...mfResults.map((mf) => ({
					...
					issuer: mf.issuer,
					currentPrice: mf.nav ? Number.parseFloat(mf.nav) : null,
					returns1y: mf.returns1y ? Number.parseFloat(mf.returns1y) : null,
					returns3y: mf.returns3y ? Number.parseFloat(mf.returns3y) : null,
		...
		if (productType === "all" || productType === "pms") {
			...
			results.push(
				...pmsResults.map((pms) => ({
					...
					issuer: pms.issuer,
					currentPrice: pms.nav ? Number.parseFloat(pms.nav) : null,
					returns1y: pms.returns1y ? Number.parseFloat(pms.returns1y) : null,
					returns3y: pms.returns3y ? Number.parseFloat(pms.returns3y) : null,
		...
		if (productType === "all" || productType === "aif") {
			...
			results.push(
				...aifResults.map((aif) => ({
					...
					issuer: aif.issuer,
					currentPrice: aif.nav ? Number.parseFloat(aif.nav) : null,
					returns1y: aif.returns1y ? Number.parseFloat(aif.returns1y) : null,
					returns3y: aif.returns3y ? Number.parseFloat(aif.returns3y) : null,
			...
			const instrumentResults = await db
				...
				.where(
					and(
						...
						or(
							...assetClasses.map((ac) => eq(instrumentMaster.assetClass, ac)),
						),
						or(
		...
		if (productType === "mutual_fund") {
			...
			if (mf) {
				product = {
					...
					returns1y: mf.returns1y ? Number.parseFloat(mf.returns1y) : null,
					returns3y: mf.returns3y ? Number.parseFloat(mf.returns3y) : null,
					returns5y: mf.returns5y ? Number.parseFloat(mf.returns5y) : null,
					riskLevel: mf.riskLevel,
					identifier: mf.schemeCode,
					aum: mf.aum,
			...
			if (pms) {
				product = {
					...
					currentPrice: pms.latestNav ? Number.parseFloat(pms.latestNav) : null,
					returns1y: pms.return1Y ? Number.parseFloat(pms.return1Y) : null,
					returns3y: pms.return3Y ? Number.parseFloat(pms.return3Y) : null,
					riskLevel: pms.riskScore ? `Score: ${pms.riskScore}` : null,
					identifier: pms.id,
			...
			if (aif) {
				product = {
					...
					currentPrice: aif.latestNav ? Number.parseFloat(aif.latestNav) : null,
					returns1y: aif.return1Y ? Number.parseFloat(aif.return1Y) : null,
					returns3y: aif.return3Y ? Number.parseFloat(aif.return3Y) : null,
					riskLevel: aif.riskScore ? `Score: ${aif.riskScore}` : null,
					identifier: aif.id,
