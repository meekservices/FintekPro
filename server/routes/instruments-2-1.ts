...
router.post("/api/instruments/sync", async (req: Request, res: Response) => {
	try {
		...
		const synced = { mutualFunds: 0, bonds: 0, unlisted: 0, stocks: 0 };
			...
			try {
				await db
					...
					.values({
						...
						lastPrice:
							bond.cleanPrice?.toString() || bond.faceValue?.toString() || null,
		...
		for (const company of unlisted) {
			const isinToUse =
				company.isin || `UL${company.id.substring(0, 10).toUpperCase()}`;
		...
		res
			.status(500)
			.json({ error: error.message || "Failed to sync instruments" });
	}
});
