...
export function registerMarketDataPart2Routes(app: Express): void {
	app.get("/api/market/status", async (req, res) => {
		try {
			const { marketHolidayService } = await import(
				"../services/market-holiday-service"
			);
	...
	app.get("/api/market/holidays", async (req, res) => {
		try {
			...
			const targetExchange =
				(exchange as "NSE" | "BSE" | "MCX" | "NCDEX") || undefined;

			const holidays = marketHolidayService.getHolidaysForYear(
				targetYear,
				targetExchange,
			);
	...
	app.get("/api/market/holidays/upcoming", async (req, res) => {
		try {
			...
			const limit = count ? Number.parseInt(count as string) : 10;
			const targetExchange =
				(exchange as "NSE" | "BSE" | "MCX" | "NCDEX") || "NSE";
	...
	app.get("/api/market/trading-calendar", async (req, res) => {
		try {
			...
			const end =
				(endDate as string) || format(addDays(new Date(), 30), "yyyy-MM-dd");
			const targetExchange =
				(exchange as "NSE" | "BSE" | "MCX" | "NCDEX") || "NSE";

			const calendar = marketHolidayService.getTradingCalendar(
				start,
				end,
				targetExchange,
			);
	...
	app.get("/api/market/is-trading-day", async (req, res) => {
		try {
			...
			const isTradingDay = marketHolidayService.isTradingDay(
				targetDate,
				targetExchange,
			);
			const holiday = marketHolidayService.getHoliday(
				targetDate,
				targetExchange,
			);
			const isWeekend = marketHolidayService.isWeekendDay(targetDate);
			const nextTradingDay = marketHolidayService.getNextTradingDay(
				targetDate,
				targetExchange,
			);
		...
		try {
			...
			try {
				if (process.env.FINNHUB_API_KEY) {
					const profile = await finnhubService.getCompanyProfile(
						symbol.toUpperCase(),
					);
				...
				console.log(
					`Finnhub profile failed for ${symbol}, using static fallback`,
				);
			}
			...
			const name =
				companyNames[symbol.toUpperCase()] ?? `${symbol.toUpperCase()} Corp`;
			res.json({
				success: true,
				data: {
	...
	app.get("/api/market/earnings/:symbol", async (req, res) => {
		try {
			...
			res.json({
				...
				symbol: symbol.toUpperCase(),
				earnings: [
					{ period: "2023-Q4", actual: 1.25, estimate: 1.2, surprise: 4.17 },
					{ period: "2023-Q3", actual: 1.1, estimate: 1.05, surprise: 4.76 },
					{ period: "2023-Q2", actual: 1.02, estimate: 1.0, surprise: 2.0 },
				],
				source: "static-fallback",
			});
		} catch (error) {
	...
	app.get("/api/market/recommendations/:symbol", async (req, res) => {
		try {
			...
			res
				.status(500)
				.json({ error: "Failed to fetch analyst recommendations" });
		}
	});
