...
import {
	isinIntelligenceService,
	type ISINMetadata,
} from "../services/isin-intelligence-service";
...
router.post("/detect", isAuthenticated, async (req: Request, res: Response) => {
	try {
		...
		const result = await isinIntelligenceService.detectInstrument(
			cleanedISIN,
			metadata as ISINMetadata,
		);
		...
		try {
			...
			const results = await Promise.all(
				isins.map(async (item: { isin: string; metadata?: ISINMetadata }) => {
					...
					const cleanedISIN = isin.trim().toUpperCase();
					return isinIntelligenceService.detectInstrument(
						cleanedISIN,
						metadata,
					);
				}),
			...
			res.json({
				...
				summary: {
					...
					validated: results.filter((r) => r.validationStatus === "validated")
						.length,
					conflicts: results.filter((r) => r.validationStatus === "conflict")
						.length,
					unknown: results.filter((r) => r.validationStatus === "unknown")
						.length,
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			const cleanedISIN = isin.trim().toUpperCase();
			const validation = await isinIntelligenceService.validateISIN(
				cleanedISIN,
				(metadata as ISINMetadata) || {},
			);
	...
	async (req: Request, res: Response) => {
		try {
			...
			res.json({
				...
				correctedISIN: result.valid
					? cleanedISIN
					: cleanedISIN.slice(0, 11) + result.computedCheckDigit,
			});
		} catch (error: any) {
...
router.get(
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (!instrument) {
				const detection =
					await isinIntelligenceService.detectInstrument(cleanedISIN);
				return res.json({
					success: true,
					found: false,
					instrument: null,
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			let detection;
			if (autoDetect) {
				detection = await isinIntelligenceService.detectInstrument(
					cleanedISIN,
					metadata as ISINMetadata,
				);
			} else {
			...
			const instrument = await isinIntelligenceService.upsertInstrument(
				cleanedISIN,
				detection,
				metadata as ISINMetadata,
			);
	...
	async (req: Request, res: Response) => {
		const rules = {
			...
			INE: {
				...
				resolution:
					"Deep resolver based on issuer type and instrument attributes",
				edgeCases: [
					"Bank bonds (RBI)",
					"NBFC NCDs (dual)",
					"MLDs (structured)",
	...
	async (req: Request, res: Response) => {
		const edgeCases = {
			...
			PERPETUAL: {
				...
				detection:
					"INE + issuer_type = BANK + perpetual = TRUE + has maturity terms",
				assetClass: "Fixed Income",
...
router.get(
    ...
    `);
			...
			const totalRecords = coverageStats.rows.reduce(
				(sum: number, r: any) => sum + (r.total || 0),
				0,
			);
			const totalWithISIN = coverageStats.rows.reduce(
				(sum: number, r: any) => sum + (r.with_isin || 0),
				0,
			);
			const totalWithRegion = coverageStats.rows.reduce(
				(sum: number, r: any) => sum + (r.with_region || 0),
				0,
			);
			...
			res.json({
				...
				coverage: {
					overall: {
						...
						isinCoveragePct:
							totalRecords > 0
								? Math.round((totalWithISIN / totalRecords) * 1000) / 10
								: 0,
						regionCoveragePct:
							totalRecords > 0
								? Math.round((totalWithRegion / totalRecords) * 1000) / 10
								: 0,
					},
					byTable: coverageStats.rows.map((r: any) => ({
						...
						isinCoveragePct:
							r.total > 0 ? Math.round((r.with_isin / r.total) * 1000) / 10 : 0,
						regionCoveragePct:
							r.total > 0
								? Math.round((r.with_region / r.total) * 1000) / 10
								: 0,
					})),
