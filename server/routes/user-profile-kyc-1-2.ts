...
export function registerUserProfileKYCPart1Part2Routes(app: Express): void {
	...
	app.get("/api/kyc/notification-status", async (req, res) => {
		try {
			...
			const status =
				await kycUpgradeNotificationService.getPendingNotifications(userId);
	...
	app.post("/api/kyc/verify-pan", async (req, res) => {
		try {
			...
			console.log(
				`[KYC] PAN verify — provider order from admin/kyc-flow: ${providers.map((p: any) => `${p.providerId}(p${p.priority},${p.isConfigured ? "ready" : "not-configured"})`).join(" → ")}`,
			);
			...
			for (const provider of providers) {
				...
				try {
					if (provider.providerId === "cashfree") {
						const { CashfreePANService } = await import(
							"../services/cashfree-pan-service"
						);
						const result = await CashfreePANService.verifyPAN(
							normalizedPan,
							safeName,
						);
						...
						console.warn(
							`[KYC] cashfree returned unverified — ${result.message}`,
						);
					} else if (provider.providerId === "sandbox") {
						const { sandboxPANService } = await import("../sandbox-pan-api");
						const result = await sandboxPANService.verifyPAN(
							normalizedPan,
							safeName,
						);
						...
						const { TruthScreenCkycAdapter } = await import(
							"../services/adapters/truthscreen-ckyc-adapter"
						);
					...
					console.warn(
						`[KYC] ${provider.providerId} threw error, trying next provider:`,
	...
	app.get("/api/profile/kyc-verified-data", async (req, res) => {
		try {
			...
			const userId = req.user!.id;
			const { getVerifiedKYCProfile } = await import(
				"../services/verified-kyc-profile-service"
			);
	...
	app.post(
		...
		async (req, res) => {
			try {
				...
				res
					.status(500)
					.json({ success: false, error: "Failed to schedule reminders" });
			}
		},
	);
	...
	app.post(
		...
		async (req, res) => {
			try {
				...
				await kycUpgradeNotificationService.acknowledgeReminder(
					userId,
					reminderId,
				);
				...
				res
					.status(500)
					.json({ success: false, error: "Failed to acknowledge reminder" });
			}
		},
	);
	...
	app.get("/api/net-worth", requireClientOrHigher, async (req, res) => {
		try {
			...
			for (const portfolio of userPortfolios) {
				for (const holding of holdingsByPortfolio.get(portfolio.id) || []) {
					...
					const currentPrice = marketInfo?.price
						? Number.parseFloat(marketInfo.price.toString())
						: Number.parseFloat((holding as any).avgPrice.toString());
					const quantity = Number.parseFloat(holding.quantity.toString());
					...
					const asset = {
						...
						avgPrice: Number.parseFloat((holding as any).avgPrice.toString()),
						gainLoss:
							currentValue -
							Number.parseFloat((holding as any).avgPrice.toString()) *
								quantity,
			...
			const bankAccounts = await db.query.userBankAccounts.findMany({
				where: and(
					inArray(userBankAccounts.userId, targetUserIds),
					eq(userBankAccounts.isActive, true),
				),
			});

			// Note: We don't have real-time balance API, so we use cash from portfolios
			const totalCash = userPortfolios.reduce(
				(sum: any, p: any) =>
					sum + Number.parseFloat(p.cash?.toString() || "0"),
				0,
			);
			...
			const pendingOrders = await db.query.unifiedOrders.findMany({
				where: and(
					inArray(unifiedOrders.userId, targetUserIds),
					inArray(unifiedOrders.status, [
						"initiated",
						"payment_pending",
						"payment_completed",
						"processing",
			...
			const pendingInvestments = pendingOrders.map((order: any) => ({
				...
				amount: Number.parseFloat(order.amount.toString()),
				status: order.status,
				createdAt: order.createdAt,
			}));

			const totalPendingValue = pendingInvestments.reduce(
			...
			if (
				userProfile?.kycTier === "accredited_investor" &&
				userProfile.netWorthAmount
			) {
				declaredAssets = Number.parseFloat(
					userProfile.netWorthAmount.toString(),
				);
			}
			...
			const loans = await db.query.loanApplications.findMany({
				where: and(
					inArray(loanApplications.userId, targetUserIds),
					inArray(loanApplications.status, ["approved", "disbursed"]),
				),
			});
			...
			for (const loan of loans) {
				const approvedAmount = Number.parseFloat(
					loan.approvedAmount?.toString() || "0",
				);
				...
				const totalRepaid = repayments.reduce(
					(sum: any, r: any) =>
						sum + Number.parseFloat(r.paymentAmount?.toString() || "0"),
					0,
				);
			...
			const liquidAssetsValue = liquidAssets.reduce(
				(sum: any, a: any) => sum + a.value,
				0,
			);
			const semiLiquidAssetsValue = semiLiquidAssets.reduce(
				(sum: any, a: any) => sum + a.value,
				0,
			);
			const illiquidAssetsValue = illiquidAssets.reduce(
				(sum: any, a: any) => sum + a.value,
				0,
			);

			const liquidityRatio =
				totalAssets > 0 ? (liquidAssetsValue / totalAssets) * 100 : 0;
			...
			try {
				...
				if (expenseData.length > 0 && expenseData[0].expenseCount > 0) {
					const totalExpenses =
						Number.parseFloat(expenseData[0].totalExpenses) || 0;
					const monthsOfData = Math.min(
						6,
						Math.max(
							1,
							Math.ceil(
								(Date.now() - sixMonthsAgo.getTime()) /
									(30 * 24 * 60 * 60 * 1000),
							),
						),
					);
					...
					console.info(
						`[Net Worth] User ${userId}: Calculated emergency fund based on ${expenseData[0].expenseCount} expenses - ₹${recommendedEmergencyFund.toFixed(0)}`,
					);
				}
			} catch (error: any) {
				console.warn(
					"[Net Worth] Failed to calculate expenses from tracking system:",
					error.message,
				);
			}
			...
			res.json({
				...
				data: {
					summary: {
						...
						totalAssets: Number.parseFloat(totalAssets.toFixed(2)),
						totalLiabilities: Number.parseFloat(totalLiabilities.toFixed(2)),
						currency: "INR",
					...
					assets: {
						breakdown: {
							liquid: {
								value: Number.parseFloat(liquidAssetsValue.toFixed(2)),
								percentage:
									totalAssets > 0
										? Number.parseFloat(
												((liquidAssetsValue / totalAssets) * 100).toFixed(2),
											)
										: 0,
								items: liquidAssets,
							},
							semiLiquid: {
								value: Number.parseFloat(semiLiquidAssetsValue.toFixed(2)),
								percentage:
									totalAssets > 0
										? Number.parseFloat(
												((semiLiquidAssetsValue / totalAssets) * 100).toFixed(
													2,
												),
											)
										: 0,
							...
							illiquid: {
								value: Number.parseFloat(illiquidAssetsValue.toFixed(2)),
								percentage:
									totalAssets > 0
										? Number.parseFloat(
												((illiquidAssetsValue / totalAssets) * 100).toFixed(2),
											)
										: 0,
								items: illiquidAssets,
							},
							pending: {
								value: Number.parseFloat(totalPendingValue.toFixed(2)),
					...
					liabilities: {
						breakdown: {
							shortTerm: {
								value: shortTermLiabilities.reduce(
									(sum: any, l: any) => sum + l.outstandingAmount,
									0,
								),
								items: shortTermLiabilities,
							},
							longTerm: {
								value: longTermLiabilities.reduce(
									(sum: any, l: any) => sum + l.outstandingAmount,
									0,
								),
					...
					metrics: {
						liquidityRatio: Number.parseFloat(liquidityRatio.toFixed(2)),
						debtToAssetRatio: Number.parseFloat(debtToAssetRatio.toFixed(2)),
						emergencyFundGap: Math.max(
							0,
							recommendedEmergencyFund - liquidAssetsValue,
						),
	...
	app.get(
		"/api/profile/product-verification-status",
		requireClientOrHigher,
		async (req, res) => {
			try {
				const userId = req.user!.id;
				...
				const products = {
					mutualFunds: {
						name: "Mutual Funds",
						verified:
							kycStatus.currentLevel === "full" ||
							kycStatus.currentLevel === "enhanced",
						requiredLevel: "full",
