...
export function registerAgentAdvisoryPart4Routes(app: Express) {
	app.post(
		...
		async (req: Request, res: Response) => {
			try {
				...
				// Import services
				const { autoPopulationOrchestrator } = await import(
					"../services/auto-population-orchestrator"
				);
				...
				console.log(
					`🚀 Agent ${agentId} initiating auto-fetch for client ${clientId}`,
				);
				const populationResult =
					await autoPopulationOrchestrator.initiateFromKYC(
						clientId,
						"manual_refresh",
					);
						...
						if (holdings.length > 0) {
							...
							const portfolioData = {
								...
								holdings: holdings.map((h) => ({
									...
									currentPrice: Number.parseFloat(h.currentPrice || "0"),
									currentValue: Number.parseFloat(h.marketValue || "0"),
									investedValue: Number.parseFloat(h.investedValue || "0"),
									gainLoss: Number.parseFloat(h.gainLoss || "0"),
									gainLossPercent: Number.parseFloat(h.gainLossPercent || "0"),
								...
								performance: {
									totalGainLoss: holdings.reduce(
										(sum, h) => sum + Number.parseFloat(h.gainLoss || "0"),
										0,
									),
							...
							const userProfile = {
								age: client?.dateOfBirth
									...
									: 35,
								riskTolerance: ((client as any)?.riskProfile || "moderate") as
									| "conservative"
									| "moderate"
									| "aggressive",
							...
							const [recommendations, proposal] = await Promise.all([
								aiService.generatePortfolioRebalancingRecommendations(
									portfolioData as any,
									userProfile,
								),
								aiService.generateInvestmentProposal(
									portfolioData as any,
									userProfile,
									100000,
									clientId,
				...
				res
					.status(500)
					.json({
						error: "Failed to auto-fetch portfolio",
	...
	app.get(
		"/api/agent/client/:clientId/portfolio-analysis",
		requireAgentPortal,
		async (req: Request, res: Response) => {
			try {
				const agentId = (req.user as any).id;
				...
				if (holdings.length === 0) {
					return res.json({
						...
						hasHoldings: false,
						message:
							"No holdings found. Use auto-fetch to populate portfolio data.",
						holdings: [],
						aiAnalysis: null,
					});
				}
				...
				const totalValue = holdings.reduce(
					(sum, h) => sum + Number.parseFloat(h.marketValue || "0"),
					0,
				);
	...
	app.post(
		"/api/proposals/:proposalId/allocation-mode",
		requireAgentPortal,
		async (req: Request, res: Response) => {
			try {
				...
				if (!mode || !["AI_DRIVEN", "MANUAL"].includes(mode)) {
					return res
						.status(400)
						.json({ error: "Invalid mode. Must be AI_DRIVEN or MANUAL" });
				}
				const result = await ProposalOrchestrator.selectAllocationMode(
					proposalId,
					mode,
					agentId || "system",
				);
	...
	app.get(
		"/api/proposals/:proposalId/ai-allocation",
		requireAgentPortal,
		async (req: Request, res: Response) => {
			try {
				const { proposalId } = req.params;
				const result = await ProposalOrchestrator.suggestAiAllocation(
					proposalId,
					"system",
				);
				res.json(result);
		...
		async (req: Request, res: Response) => {
			try {
				...
				if (!allocationMode || !allocation || !Array.isArray(allocation)) {
					return res
						.status(400)
						.json({ error: "allocationMode and allocation array required" });
				}
	...
	app.get(
		"/api/proposals/:proposalId/locked-strategy",
		requireAgentPortal,
		async (req: Request, res: Response) => {
			try {
				const { proposalId } = req.params;
	...
	app.post(
		"/api/proposals/:proposalId/select-instruments",
		requireAgentPortal,
		async (req: Request, res: Response) => {
			try {
				const { proposalId } = req.params;
				...
				const result =
					await ProposalOrchestrator.selectInstrumentsWithinStrategy(
						proposalId,
						agentId || "system",
					);
				res.json(result);
	...
	app.post(
		...
		async (req: Request, res: Response) => {
			try {
				...
				const result = await ProposalOrchestrator.runFairBacktest(
					proposalId,
					oldHoldings,
					agentId || "system",
				);
		...
		async (req: Request, res: Response) => {
			try {
				...
				if (!oldAllocation || !Array.isArray(oldAllocation)) {
					return res
						.status(400)
						.json({ error: "oldAllocation array required" });
				}
				const result =
					await ProposalOrchestrator.generatePortfolioDifferenceSummary(
						proposalId,
						oldAllocation,
						agentId || "system",
					);
	...
	app.post(
		"/api/proposals/:proposalId/validate-override",
		requireAgentPortal,
		async (req: Request, res: Response) => {
			try {
				const { proposalId } = req.params;
				const { proposedAllocation, agentId } = req.body;
				const { ProposalVerdictNormalizer } = await import(
					"../services/proposal-verdict-normalizer"
				);
				const result =
					await ProposalVerdictNormalizer.validateAllocationOverride(
						proposalId,
						proposedAllocation,
						agentId || "system",
					);
	...
	app.get(
		"/api/proposals/:proposalId/strategy-integrity",
		requireAgentPortal,
		async (req: Request, res: Response) => {
			try {
				const { proposalId } = req.params;
				const { ProposalFlowGatekeeper } = await import(
					"../services/proposal-flow-gatekeeper"
				);
				const result =
					await ProposalFlowGatekeeper.validateStrategyIntegrity(proposalId);
				res.json(result);
			} catch (error: any) {
				res.status(500).json({ error: error.message });
			}
	...
	app.post(
		"/api/proposals/:proposalId/new-version",
		requireAgentPortal,
		async (req: Request, res: Response) => {
			try {
				const { proposalId } = req.params;
				const { newAllocation, allocationMode, agentId, changeReason } =
					req.body;
				if (
					!newAllocation ||
					!Array.isArray(newAllocation) ||
					!allocationMode
				) {
					return res
						.status(400)
						.json({ error: "newAllocation array and allocationMode required" });
				}
				const result =
					await ProposalOrchestrator.forceNewVersionOnAllocationChange(
						proposalId,
						newAllocation,
						allocationMode,
						agentId || "system",
						changeReason || "Allocation modified",
					);
				res.json(result);
			} catch (error: any) {
				res.status(500).json({ error: error.message });
	...
	console.log(
		"✅ Agent Advisory routes registered (including Proposal Builder Upgrade APIs)",
	);
}
