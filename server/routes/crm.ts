...
import { db } from "../db";
import { crmInteractions, crmOpportunities, crmTasks, crmClientTags, crmActivityLog, users } from "@shared/schema";
...
export function registerCrmRoutes(app: Express) {
  ...
  app.post("/api/crm/interactions", async (req, res) => {
    try {
      const [interaction] = await db.insert(crmInteractions).values(req.body).returning();
  ...
  app.post("/api/crm/opportunities", async (req, res) => {
    try {
      const [opportunity] = await db.insert(crmOpportunities).values(req.body).returning();
  ...
  app.patch("/api/crm/opportunities/:id/stage", async (req, res) => {
    try {
      ...
      const [opportunity] = await db.update(crmOpportunities)
  ...
  app.get("/api/crm/clients/:clientId/overview", async (req, res) => {
    try {
      ...
      const recentInteractions = await db.select().from(crmInteractions)
        .where(eq(crmInteractions.clientId, clientId))
        .orderBy(desc(crmInteractions.createdAt))
        .limit(10);
      ...
      const opportunities = await db.select().from(crmOpportunities)
        .where(and(eq(crmOpportunities.clientId, clientId), eq(crmOpportunities.status, 'open')));
      
      // Fetch pending tasks
      const pendingTasks = await db.select().from(crmTasks)
        .where(and(eq(crmTasks.clientId, clientId), eq(crmTasks.status, 'pending')));
      
      // Fetch tags
      const tags = await db.select().from(crmClientTags)
        .where(eq(crmClientTags.clientId, clientId));
      
      // Fetch activity timeline
      const activityTimeline = await db.select().from(crmActivityLog)
        .where(eq(crmActivityLog.clientId, clientId))
        .orderBy(desc(crmActivityLog.createdAt))
        .limit(20);
      
      // Calculate stats
      const totalOpportunityValue = opportunities.reduce((sum, o) => sum + Number(o.expectedAmount || 0), 0);
  ...
  app.post("/api/crm/clients/:clientId/tags", async (req, res) => {
    try {
      const { clientId } = req.params;
      const [tag] = await db.insert(crmClientTags)
