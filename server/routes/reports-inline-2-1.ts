...
export function registerReportsInline21Routes(app: Express): void {
	...
	app.post("/api/reports/sync-mf-to-portfolio", async (req, res) => {
		try {
			...
			if (!userId || !portfolioId) {
				return res
					.status(400)
					.json({ error: "userId and portfolioId are required" });
			}
			...
			const holdings = await db
				...
				.innerJoin(
					schema.mfFolios,
					eq(schema.mfHoldings.folioId, schema.mfFolios.id),
				)
			...
			for (const holding of holdings) {
				...
				const holdingUnits = Number.parseFloat(holding.units || "0");
			...
			res
				.status(500)
				.json({ error: "Failed to sync MF holdings to portfolio" });
		}
	});
	...
	app.post("/api/reports/sync-demat-to-portfolio", async (req, res) => {
		try {
			...
			res
				.status(500)
				.json({ error: "Failed to sync demat holdings to portfolio" });
		}
	});
	// Watchlist endpoints
	...
	app.post("/api/watchlists", async (req, res) => {
		try {
			...
			if (error instanceof z.ZodError) {
				res
					.status(400)
					.json({ error: "Invalid watchlist data", details: error.issues });
			} else {
	...
	app.get("/api/public/mutual-funds", async (req, res) => {
		try {
			...
			const conditions: any[] = [eq(mutualFunds.isPublished, true)];
			...
			if (sortBy === "nav") {
				orderBy =
					sortOrder === "asc" ? asc(mutualFunds.nav) : desc(mutualFunds.nav);
			} else if (sortBy === "returns1y") {
				orderBy =
					sortOrder === "asc"
						? asc(mutualFunds.returns1y)
						: desc(mutualFunds.returns1y);
			} else if (sortBy === "fundHouse") {
				orderBy =
					sortOrder === "asc"
						? asc(mutualFunds.fundHouse)
						: desc(mutualFunds.fundHouse);
			} else {
				orderBy =
					sortOrder === "asc"
						? asc(mutualFunds.schemeName)
						: desc(mutualFunds.schemeName);
			}
			...
			const fundHouses = await db
				.selectDistinct({ fundHouse: mutualFunds.fundHouse })
				.from(mutualFunds)
				.where(eq(mutualFunds.isPublished, true));

			// Get unique categories for filter dropdown
			const categories = await db
				.selectDistinct({ category: mutualFunds.category })
				.from(mutualFunds)
				.where(eq(mutualFunds.isPublished, true));
