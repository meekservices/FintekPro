...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			res
				.status(500)
				.json({ error: error.message || "Failed to parse portfolio" });
		}
	},
);
		...
		try {
			...
			if (!importResult.success || importResult.holdings.length === 0) {
				return res
					...
					.json({
						success: false,
						error: "No holdings found from URL",
						errors: importResult.errors,
					});
			}
			...
			res
				.status(500)
				.json({ error: error.message || "Failed to import from URL" });
		}
	},
);
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			res
				.status(500)
				.json({ error: error.message || "Failed to parse CAS statement" });
		}
	},
);
	...
	async (req: Request, res: Response) => {
		try {
			...
			const unifiedHoldings = holdings.map((h: any) => ({
				...
				avgCostPerUnit: Number.parseFloat(h.avgPrice || h.averagePrice || "0"),
	...
	async (req: Request, res: Response) => {
		try {
			...
			const totalInvested = holdings.reduce((sum, h) => {
				const qty = Number.parseFloat(h.quantity?.toString() || "0");
				const avgPrice = Number.parseFloat(h.avgPrice?.toString() || "0");
				return sum + qty * avgPrice;
			}, 0);

			const totalCurrentValue = holdings.reduce((sum, h) => {
				return sum + Number.parseFloat(h.currentValue?.toString() || "0");
			}, 0);
...
router.get(
	"/api/portfolio/unified-complete",
	requireAuth,
	async (req: Request, res: Response) => {
		try {
			const userId = (req.user as any)?.id;
			...
			for (const holding of allHoldings) {
				...
				const investedValue = Number.parseFloat(
			...
			const transactionsWithSource = await db
				.select({
				...
				.leftJoin(
					transactionReports,
					eq(transactionRecords.reportId, transactionReports.id),
				)
				.where(eq(transactionRecords.userId, userId))
				.orderBy(desc(transactionRecords.transactionDate))
				.limit(100);

			const formattedTransactions = transactionsWithSource.map(
				({ transaction: tx, source, assetType }) => ({
					id: tx.id,
					...
					units: Number.parseFloat(tx.units?.toString() || "0"),
					nav: Number.parseFloat(tx.nav?.toString() || "0"),
					amount: Number.parseFloat(tx.amount?.toString() || "0"),
					netAmount: Number.parseFloat(tx.netAmount?.toString() || "0"),
					stampDuty: Number.parseFloat(tx.stampDuty?.toString() || "0"),
					stt: Number.parseFloat(tx.stt?.toString() || "0"),
					tds: Number.parseFloat(tx.tds?.toString() || "0"),
...
router.get(
	"/api/agent/clients/:clientId/portfolio/unified",
	requireAuth,
