 ...
 */
router.post(
	"/admin/compliance/check-investor-limit",
	requireAdmin,
	async (req: Request, res: Response) => {
		try {
			...
			const result = await regulatoryComplianceService.checkInvestorLimit(
				companyId,
				userId,
			);
		...
		try {
			...
			if (!companyId || !sharesToSell) {
				return apiResponse.badRequest(
					res,
					"companyId and sharesToSell are required",
				);
			}

			const result = await regulatoryComplianceService.checkLockInStatus(
				companyId,
				userId,
				sharesToSell,
			);
 ...
 */
router.post(
	"/admin/compliance/check-company-status",
	requireAdmin,
	async (req: Request, res: Response) => {
		try {
			...
			const result =
				await regulatoryComplianceService.checkCompanyListingStatus(companyId);
			return apiResponse.success(res, {
 ...
 */
router.post(
	"/admin/compliance/verify-source-of-funds",
	requireAdmin,
	async (req: Request, res: Response) => {
		try {
			const { companyId, userId } = req.body;
			...
			const result =
				await regulatoryComplianceService.markSourceOfFundsVerified(
					companyId,
					userId,
					adminId,
				);
			return apiResponse.success(res, {
				message: result.success
					? "Source of funds verified successfully"
					: "Failed to verify source of funds",
				data: result,
			});
		} catch (error: any) {
	...
	async (req: Request, res: Response) => {
		try {
			...
			return apiResponse.success(
				res,
				result,
				"Valuation recorded successfully",
			);
		} catch (error: any) {
	...
	async (req: Request, res: Response) => {
		try {
			...
			return apiResponse.serverError(
				res,
				"Failed to log disclosure acknowledgment",
			);
		}
	},
);
	...
	async (req: Request, res: Response) => {
		try {
			...
			return apiResponse.success(
				res,
				report,
				`Staleness sweep complete: ${report.markedStale} newly marked stale`,
			);
		} catch (error: any) {
 ...
 */
router.get(
	"/companies/:id/proposal-modifiers",
	requireAuth,
	async (req: Request, res: Response) => {
		try {
			const { id } = req.params;
			const modifiers =
				unlistedValuationGovernanceService.getProposalModifiers(id);
			const latest =
				await unlistedValuationGovernanceService.getLatestValuation(id);
			return apiResponse.success(res, {
				companyId: id,
				...modifiers,
				latestValuation: latest,
...
router.get(
	...
	async (req: Request, res: Response) => {
		try {
			const report =
				await unlistedFinancialEnrichmentService.getFinancialHealthReport();
			return apiResponse.success(res, report);
		} catch (error: any) {
	...
	async (req: Request, res: Response) => {
		try {
			...
			return apiResponse.success(
				res,
				result,
				`Enrichment complete. FHS: ${result.fhs?.toFixed(3) ?? "N/A"}`,
			);
		} catch (error: any) {
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			const result =
				await unlistedFinancialEnrichmentService.enrichStaleBatch(limit);
			return apiResponse.success(
				res,
				result,
...
router.get(
	...
	async (req: Request, res: Response) => {
		try {
			...
			return apiResponse.serverError(
				res,
				"Failed to compute financial health score",
			);
		}
	},
);
