...
export function registerAgentAdvisoryPart1Routes(app: Express) {
		...
		async (req: Request, res: Response) => {
			try {
				...
				if (
					...
					return res
						.status(400)
						.json({ error: "Company name, email, and phone are required" });
				}
				...
				if (
					error.code === "23505" &&
					error.constraint?.includes("contact_email")
				) {
					return res
						.status(400)
						.json({ error: "A partner with this email already exists" });
				}
			...
			try {
				...
				if (includeProspects) {
					...
					const formattedLeads = leads.map((l) => {
						const nameParts = (l.contactName || l.companyName || "")
							.trim()
							.split(" ");
	...
	app.get(
		"/api/agent/portfolio-uploads/pending",
		requireAgentPortal,
		async (req: Request, res: Response) => {
			try {
				const agentId = (req.user as any).id;
		...
		async (req: Request, res: Response) => {
			try {
				...
				if (!clientId || !uploadType) {
					return res
						.status(400)
						.json({ error: "Client ID and upload type are required" });
				}
				...
				const [uploadRecord] = await db
					...
					.values({
						...
						rawData: file
							? { hasFile: true, fileName: file.originalname, size: file.size }
							: {},
				...
				console.log(
					`[Portfolio Upload] OTP for client ${clientId}: ${otp} (expires: ${otpExpiry})`,
				);
	...
	app.post(
		"/api/agent/portfolio-upload/:uploadId/confirm-otp",
		requireAgentPortal,
		async (req: Request, res: Response) => {
			try {
				const agentId = (req.user as any).id;
				...
				if (
					(upload as any).otpExpiry &&
					new Date((upload as any).otpExpiry) < new Date()
				) {
				...
				await logAgentAction({
					...
					newState: {
						status: "confirmed",
						confirmedAt: (updatedUpload as any).clientConfirmedAt,
					},
					ipAddress: req.ip,
					userAgent: req.headers["user-agent"],
				});
	...
	app.get(
		...
		async (req: Request, res: Response) => {
			try {
				...
				const conditions = [eq(advisorySessions.agentId, agentId)];
				if (clientId) {
		...
		async (req: Request, res: Response) => {
			try {
				...
				if (!clientId || !sessionPurpose) {
					return res
						.status(400)
						.json({ error: "Client ID and session purpose are required" });
				}
	...
	app.patch(
		"/api/agent/advisory-sessions/:sessionId/workflow",
		requireAgentPortal,
		async (req: Request, res: Response) => {
			try {
				const agentId = (req.user as any).id;
				...
				if (
					workflowState === "suitability_check" &&
					!agentDeclarationAcknowledged &&
					!existingSession.agentDeclarationAcknowledged
				) {
					return res
						.status(400)
						.json({
							error:
								"Agent must acknowledge declaration before proceeding to suitability check",
						});
				}

				if (
					workflowState === "client_sharing" &&
					!existingSession.optimizationCompleted
				) {
					return res
						.status(400)
						.json({
							error:
								"Optimization must be completed before sharing with client",
						});
				}
				...
				if (agentDeclarationAcknowledged !== undefined) {
					updateData.agentDeclarationAcknowledged =
						agentDeclarationAcknowledged;
