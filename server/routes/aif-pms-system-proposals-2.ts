...
export function registerAIFPMSSystemPart2Routes(app: Express): void {
	...
	app.post("/api/system/analyze-logs", async (req, res) => {
		try {
			...
			switch (logType) {
				case "system":
					analysis = await geminiService.analyzeSystemErrors(
						JSON.stringify(logs),
					);
					break;
				case "performance":
					analysis = await geminiService.analyzeApiPerformance(
						JSON.stringify(logs),
					);
					break;
				case "security":
					analysis = await geminiService.analyzeSecurityVulnerabilities(
						JSON.stringify(logs),
					);
					break;
				default:
	...
	app.get("/api/system/diagnostics/comprehensive", async (req, res) => {
		try {
			...
			const agentInstructions =
				await errorMonitor.generateReplitAgentInstructions();
			const healingActions = await errorMonitor.autoHeal();
			...
			res.json({
				...
				diagnostics: {
					...
					summary: {
						...
						criticalIssues: health.errors.filter(
	...
	app.get("/api/kfintech/sip/:pan", async (req, res) => {
		try {
			...
			const result = await kfintechApi.getSIPDetails(
				pan,
				folioNumber as string,
			);
	...
	app.post("/api/kfintech/transactions/purchase", async (req, res) => {
		try {
			...
			const result =
				await kfintechApi.createPurchaseTransaction(purchaseRequest);
	...
	app.post("/api/proposals", authenticateUser, async (req, res) => {
		try {
			...
			if (!hasRole(req.user, ["agent", "admin"])) {
				return res
					.status(403)
					.json({ error: "Only agents can create investment proposals" });
			}
		...
		async (req, res) => {
			try {
				...
				if (proposal.status === "approved" || proposal.status === "executed") {
					return res
						.status(400)
						.json({ error: "Cannot update approved or executed proposals" });
				}

				const updated = await storage.updateInvestmentProposal(
					proposalId,
					req.body,
				);
				res.json(updated);
	...
	app.post("/api/proposals/:id/approve", authenticateUser, async (req, res) => {
		try {
			...
			if (proposal.clientId !== userId) {
				return res
					.status(403)
					.json({ error: "You are not authorized to approve this proposal" });
			}

			if (
				proposal.status !== "waiting_client_approval" &&
	...
	app.post("/api/proposals/:id/reject", authenticateUser, async (req, res) => {
		try {
			...
			if (proposal.clientId !== userId) {
				return res
					.status(403)
					.json({ error: "You are not authorized to reject this proposal" });
			}
