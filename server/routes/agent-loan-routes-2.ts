	...
	async (req: Request, res: Response) => {
		try {
			const history = await db
				...
				.where(
					eq(agentLoanStatusHistory.applicationId, req.params.applicationId),
				)
	...
	try {
		...
		if (!application) {
			return res
				...
				.json({
					success: false,
					error: "Not authorized to add documents to this application",
				});
		}
...
router.get("/dashboard/stats", async (req: Request, res: Response) => {
	try {
		...
		applications.forEach((app) => {
			...
			stats.byLoanType[app.loanType] =
				(stats.byLoanType[app.loanType] || 0) + 1;
...
router.get("/disbursed", async (req: Request, res: Response) => {
	try {
		...
		const claimMap = new Map(
			existingClaims.map((c) => [c.applicationId, c.status]),
		);
