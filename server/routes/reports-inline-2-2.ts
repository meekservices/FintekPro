...
export function registerReportsInline22Routes(app: Express): void {
	app.get("/api/sebi/mutual-funds", async (req, res) => {
		try {
			...
			const pageNum = Number.parseInt((req.query.page as string) || "1");
			const limitNum = Number.parseInt((req.query.limit as string) || "50");
		...
		try {
			...
			if (nav_min) {
				filteredFunds = filteredFunds.filter(
					(fund) =>
						Number.parseFloat(fund.nav) >= Number.parseFloat(nav_min as string),
				);
			}
			...
			if (nav_max) {
				filteredFunds = filteredFunds.filter(
					(fund) =>
						Number.parseFloat(fund.nav) <= Number.parseFloat(nav_max as string),
				);
			}
	...
	app.get("/api/mutual-funds/popular", async (req, res) => {
		try {
			console.log(
				"📈 Fetching best performing mutual funds with MultiSource service...",
			);
			...
			if (popularFunds && popularFunds.length > 0) {
				console.log(
					`✅ MultiSource service found ${popularFunds.length} popular funds`,
				);
				...
				const formattedFunds = topFunds.map((fund) => ({
					...
					fundHouse: fund.fundHouse,
					category: fund.category,
					nav: String(Number.parseFloat(fund.currentNav || fund.nav || "0")),
					change: fund.change || "0",
					changePercent: fund.changePercent || "0",
				...
				for (const fund of topFunds) {
					try {
						...
						console.warn(
							`Failed to cache fund ${fund.schemeCode}:`,
							cacheError,
						);
					}
				}
			...
			console.warn(
				"⚠️ MultiSource service returned no popular funds, trying fallback...",
			);
			...
			try {
				...
				if (cachedFunds.length > 0) {
					...
					const formattedCached = topCachedFunds.map((fund, index) => ({
						...
						amc: fund.fundHouse,
						category: fund.category,
						nav: Number.parseFloat(String(fund.nav)) || 0,
						nav_date: fund.lastUpdated
							? fund.lastUpdated.toISOString().split("T")[0]
			...
			res.status(503).json({
				...
				message:
					"All data sources are currently unavailable. Please try again later.",
	...
	app.get("/api/mutual-funds/by-isin/:isin", async (req, res) => {
		try {
			...
			if (funds.length === 0) {
				return res
					.status(404)
					.json({ success: false, error: "Fund not found for this ISIN" });
			}
	...
	app.get("/api/mutual-funds/search", async (req, res) => {
		try {
			...
			if (searchResults.length > 0) {
				console.log(
					`✅ Query param search found ${searchResults.length} funds`,
				);
	...
	app.get("/api/mutual-funds/:schemeCode", async (req, res) => {
		try {
			...
			console.log(
				`📊 Fetching fund details for scheme: ${schemeCode} with MultiSource service`,
			);
			...
			const formattedFund = {
				...
				subCategory: fund.subCategory || fund.category,
				fundHouse: fund.fundHouse,
				nav: Number.parseFloat(fund.currentNav || "0"),
				navDate: fund.navDate,
				isin: fund.isin || "N/A",
				aum: fund.aum || "N/A",
				expenseRatio: fund.expenseRatio || "N/A",
      ...
      LEGACY END - to be removed */
			...
			console.error(
				`❌ Error fetching mutual fund ${req.params.schemeCode}:`,
				error,
			);
			res.status(500).json({
				success: false,
	...
	app.get("/api/mutual-funds/search/:query", async (req, res) => {
		try {
			...
			if (searchResults.length > 0) {
				console.log(
					`✅ MultiSource search found ${searchResults.length} funds`,
				);
			...
			console.error(
				`❌ Error searching mutual funds for "${req.params.query}":`,
				error,
			);
			res.status(500).json({
				success: false,
