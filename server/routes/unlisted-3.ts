...
router.delete(
	...
	async (req: Request, res: Response) => {
		try {
			...
			await db.delete(unlistedCart).where(eq(unlistedCart.userId, req.user.id));
	...
	async (req: Request, res: Response) => {
		try {
			...
			const totalValue = cartItems.reduce((sum, item) => {
				const price = Number.parseFloat(item.maxPrice) || 0;
			...
			if (totalValue >= ACCREDITED_INVESTOR_THRESHOLD) {
				...
				if (!profile?.accreditedInvestorType) {
					console.log(
						`[COMPLIANCE] cart_checkout_blocked: High-value transaction (₹${totalValue}) requires accredited investor status | userId: ${req.user.id}`,
					);
			...
			if (
				...
				return apiResponse.badRequest(
					res,
					"Risk disclosure acknowledgment is required for batch checkout",
				);
			}
			...
			for (const item of cartItems) {
				try {
					...
					console.log(
						`[COMPLIANCE] cart_checkout_buy_request: { userId: '${req.user.id}', companyId: '${item.companyId}', quantity: ${item.quantity}, buyRequestId: '${buyRequest.id}' }`,
					);
				} catch (itemError: any) {
					errors.push({
						companyId: item.companyId,
			...
			console.log(
				`[COMPLIANCE] cart_checkout_complete: { userId: '${req.user.id}', successCount: ${createdRequests.length}, errorCount: ${errors.length}, totalValue: ${totalValue} }`,
			);
...
router.get("/eligibility", async (req: Request, res: Response) => {
	try {
		...
		const eligibility = await unlistedEligibilityService.checkUserEligibility(
			req.user.id,
		);
...
router.get(
	...
	async (_req: Request, res: Response) => {
		try {
			const requirements =
				await unlistedEligibilityService.getEligibilityRequirements();
...
router.get("/eligibility/kyc-upgrade", async (req: Request, res: Response) => {
	try {
		...
		const upgradeStatus = await unlistedEligibilityService.getKYCUpgradeStatus(
			req.user.id,
		);
 ...
 */
router.get(
	"/risk-disclosures/company/:companyId",
	async (req: Request, res: Response) => {
		try {
			...
			const companyRisks =
				unlistedRiskDisclosureService.getCompanySpecificRisks({
					netWorth: (company as any).netWorth
						? Number.parseFloat((company as any).netWorth.toString())
						: undefined,
					debtEquityRatio: latestRatios?.debtEquity
						? Number.parseFloat(latestRatios.debtEquity.toString())
						: undefined,
					profitMargin: latestRatios?.marginPat
						? Number.parseFloat(latestRatios.marginPat.toString())
						: undefined,
			...
			return apiResponse.serverError(
				res,
				"Failed to fetch company risk disclosures",
			);
		}
	},
);
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (!Array.isArray(acknowledgedDisclosureIds)) {
				return apiResponse.badRequest(
					res,
					"acknowledgedDisclosureIds must be an array",
				);
			}
			...
			if (!validation.valid) {
				return apiResponse.badRequest(
					res,
					"Not all mandatory disclosures have been acknowledged",
					{
			...
			console.log(
				`[COMPLIANCE] risk_disclosure_acknowledged: { userId: '${req.user.id}', companyId: '${companyId}', tradeType: '${tradeType}', version: '${unlistedRiskDisclosureService.getDisclosureVersion()}' }`,
			);
			...
			return apiResponse.serverError(
				res,
				"Failed to validate risk disclosures",
			);
		}
	},
);
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (!isBuyer && !isSeller) {
				return apiResponse.forbidden(
					res,
					"You are not authorized to accept this deal",
				);
			}
			...
			if (deal.status !== "pending" && deal.status !== "awaiting_acceptance") {
				return apiResponse.badRequest(
					res,
					`Deal cannot be accepted in current status: ${deal.status}`,
				);
			}
			...
			if (isBuyer && deal.buyerAccepted) {
				return apiResponse.badRequest(
					res,
					"You have already accepted this deal",
				);
			}
			...
			if (
				...
				new Date() > new Date(deal.acceptanceDeadline)
			) {
			...
			if (
				...
				return apiResponse.badRequest(
					res,
					"Risk disclosure acknowledgment is required before accepting a deal",
					{
			...
			if (!disclosureValidation.valid) {
				console.log(
					`[COMPLIANCE] deal_accept_blocked: Missing risk disclosures | userId: ${userId} | dealId: ${id} | missing: ${disclosureValidation.missingDisclosures.join(", ")}`,
				);
				return apiResponse.badRequest(
					res,
					"All mandatory risk disclosures must be acknowledged before accepting the deal.",
					{
			...
			const otherPartyAccepted = isBuyer
				? deal.sellerAccepted
				: deal.buyerAccepted;
			...
			const totalValue = Number.parseFloat(deal.totalValue || "0");
			...
			if (totalValue >= 1000000) {
				await regulatoryReportingService.registerReportableEvent({
					...
					currency: "INR",
					riskIndicators:
						totalValue >= 5000000 ? ["high_value_transaction"] : [],
			...
			if (updateData.status === "confirmed") {
				console.log(
					`[Deal ${id}] Both parties accepted - deal confirmed, ready for payment`,
				);
			}
