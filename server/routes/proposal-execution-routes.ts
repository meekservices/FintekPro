...
router.get("/orders/pending-approval", async (req: Request, res: Response) => {
	try {
		...
		const orders = proposalExecutionEngine.getPendingApprovalOrders(
			role as any,
		);
		res.json({ success: true, data: orders });
	} catch (error: any) {
 ...
 */
router.get(
	"/order/:id/maker-checker-status",
	async (req: Request, res: Response) => {
		try {
			const status = proposalExecutionEngine.getMakerCheckerStatus(
				req.params.id,
			);
			if (!status) {
				res.status(404).json({ success: false, error: "Order not found" });
				...
				thresholds: [
					{
						...
						description: "Orders up to ₹1 Lakh - No approval required",
					},
					{
						tier: "elevated",
						minAmount: 100001,
						...
						requiresAuthorizer: false,
						description: "Orders ₹1L-5L - Checker approval required",
					},
					{
						tier: "high_value",
						minAmount: 500001,
						...
						requiresChecker: true,
						requiresAuthorizer: false,
						description: "Orders ₹5L-10L - Checker approval required",
					},
					{
						tier: "premium",
						minAmount: 1000001,
						...
						description:
							"Orders above ₹10L - Checker + Authorizer required (-1 indicates no upper limit)",
					},
				],
				description:
					"Maker-Checker approval workflow based on order value. Premium tier (>₹10L) requires both checker and authorizer approval.",
		...
		res.json({
			...
			data: [
				...
				},
				{
					method: "mandate",
					name: "E-Mandate",
					description: "Auto-debit mandate for SIP",
					instant: false,
				},
				{
					method: "neft",
