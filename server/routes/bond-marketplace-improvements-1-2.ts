 ...
 */
router.get(
	"/risk-disclosures/:instrumentType",
	async (req: Request, res: Response) => {
		try {
			const { instrumentType } = req.params;
			const { transactionValue, isListed } = req.query;

			const disclosures = getSEBIDisclosures(
				instrumentType,
				Number.parseFloat(transactionValue as string) || 0,
				isListed !== "false",
			);
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			const required = getSEBIDisclosures(
				instrumentType,
				transactionValue || 0,
				true,
			);
			const requiredCategories = required.disclosures
				.filter((d: any) => d.requiresExplicitAck)
				.map((d: any) => d.category);
			const acknowledged = disclosuresAcknowledged || [];

			const allAcknowledged = requiredCategories.every((cat: string) =>
				acknowledged.includes(cat),
			);

			// Record attestation
			const [attestation] = await db
				.insert(schema.bondRiskDisclosureAttestations)
				.values({
					userId,
					isin,
			...
			return apiResponse.success(res, {
				...
				message: allAcknowledged
					? "All risk disclosures acknowledged"
					: "Some required disclosures not acknowledged",
			});
		} catch (error: any) {
...
router.get("/data-freshness", async (_req: Request, res: Response) => {
	try {
		...
		const [latestGov] = await db
			.select({ lastUpdated: schema.governmentSecurities.lastUpdated })
			.from(schema.governmentSecurities)
			.orderBy(desc(schema.governmentSecurities.lastUpdated))
			.limit(1);

		const [latestCorp] = await db
			.select({ lastUpdated: schema.corporateBonds.lastUpdated })
		...
		const sources = [
			{
				...
				isStale: latestGov?.lastUpdated
					? now.getTime() - new Date(latestGov.lastUpdated).getTime() >
						freshnesThreshold
					: true,
				status: latestGov?.lastUpdated
					? now.getTime() - new Date(latestGov.lastUpdated).getTime() >
						freshnesThreshold
						? "stale"
						: "fresh"
			...
			},
			{
				...
				isStale: latestCorp?.lastUpdated
					? now.getTime() - new Date(latestCorp.lastUpdated).getTime() >
						freshnesThreshold
					: true,
				status: latestCorp?.lastUpdated
					? now.getTime() - new Date(latestCorp.lastUpdated).getTime() >
						freshnesThreshold
						? "stale"
						: "fresh"
		...
		return apiResponse.success(res, {
			...
			sources,
			overallStatus: sources.every((s) => s.status === "fresh")
				? "all_fresh"
...
function mapToFeeInstrumentType(
	rawType: string,
	isGovernment: boolean,
): BondFeeInstrumentType {
...
router.get("/net-yield/:isin", async (req: Request, res: Response) => {
	try {
		...
		if (govBond) {
			...
			grossYield = Number.parseFloat(govBond.yieldToMaturity || "0");
			...
			if (corpBond) {
				...
				grossYield = Number.parseFloat(corpBond.yieldToMaturity || "0");
		...
		const instrumentType = mapToFeeInstrumentType(
			rawInstrumentType,
			isGovernment,
		);
		...
		const now = new Date();
		const holdingPeriodYears = maturityDate
			? Math.max(
					0.25,
