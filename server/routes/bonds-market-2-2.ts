...
export function registerBondsMarkPart2Part2Routes(app: Express): void {
		...
		async (req: Request, res: Response): Promise<void> => {
			try {
				...
				if (exchange && exchange !== "all") {
					combinedBonds = combinedBonds.filter(
						(bond) => bond.exchange.toLowerCase() === exchange.toLowerCase(),
					);
				}
				...
				if (category && category !== "all") {
					combinedBonds = combinedBonds.filter((bond) => {
						...
						return (
							bondCategory.includes(category.toLowerCase()) ||
				...
				const marketStats =
					combinedBonds.length > 0
						? {
								...
								averageYield: (
									combinedBonds.reduce(
										(sum, bond) => sum + (bond.currentYield || 0),
										0,
									) / combinedBonds.length
								).toFixed(2),
								topGainer: combinedBonds.reduce(
									(max, bond) =>
	...
	app.get(
		"/api/bonds/search/advanced",
		requireLevel2,
		async (req: Request, res: Response): Promise<void> => {
			try {
				const filters = {
					creditRatings: req.query.creditRatings
						? (req.query.creditRatings as string).split(",")
						: undefined,
					minYield: req.query.minYield
						? Number.parseFloat(req.query.minYield as string)
						: undefined,
					maxYield: req.query.maxYield
						? Number.parseFloat(req.query.maxYield as string)
						: undefined,
					minMaturityYears: req.query.minMaturityYears
						? Number.parseInt(req.query.minMaturityYears as string)
						: undefined,
					maxMaturityYears: req.query.maxMaturityYears
						? Number.parseInt(req.query.maxMaturityYears as string)
						: undefined,
					bondTypes: req.query.bondTypes
						? (req.query.bondTypes as string).split(",")
						: undefined,
					issuers: req.query.issuers
						? (req.query.issuers as string).split(",")
						: undefined,
					minCouponRate: req.query.minCouponRate
						? Number.parseFloat(req.query.minCouponRate as string)
						: undefined,
					maxCouponRate: req.query.maxCouponRate
						? Number.parseFloat(req.query.maxCouponRate as string)
						: undefined,
					couponTypes: req.query.couponTypes
						? (req.query.couponTypes as string).split(",")
						: undefined,
