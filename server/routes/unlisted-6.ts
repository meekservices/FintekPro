 ...
 */
router.post(
	"/admin/publish-existing-to-store",
	requireAdmin,
	async (req: Request, res: Response) => {
		try {
			...
			const existingProduct =
				await storage.getStoreProductBySourceCompanyId(companyId);
			if (existingProduct) {
				return apiResponse.badRequest(
					res,
					"Company is already published to store",
					{
			...
			let unlistedCategory =
				await storage.getStoreCategoryBySlug("unlisted-stocks");
			...
			if (!unlistedCategory) {
				...
				unlistedCategory = await storage.createStoreCategory({
					...
					description:
						"Trade shares of unlisted companies before they go public. Enhanced KYC required.",
			...
			const sectorSlug = (company.sector || "others")
				.toLowerCase()
				.replace(/[^a-z0-9]+/g, "-");
			...
			const productData = {
				...
				fullDescription:
					company.description ||
					`Invest in ${company.name}, an unlisted company in the ${company.sector || "Technology"} sector. ${company.listingStage === "pre_ipo" ? "Pre-IPO opportunity." : "Growth stage investment."}`,
				categoryId: unlistedCategory.id,
				...
				currency: "INR",
				minimumInvestment: company.minLotSize
					? String(
							Number(company.minLotSize) *
								Number(company.currentPrice || company.lastPrice || 100),
						)
					: "10000",
				riskLevel:
					riskMapping[company.riskRating?.toLowerCase() || "high"] || "high",
				...
				eligibility: JSON.stringify({
					...
					description:
						"Enhanced/Accredited KYC tier required for unlisted stock trading",
				}),
				...
				inquiryMessage:
					"Contact our team for unlisted stock investment opportunities",
			...
			console.log(
				`Published ${company.name} to store as product ${product.id}`,
			);
 ...
 */
router.post(
	"/admin/sync-store-product/:companyId",
	requireAdmin,
	async (req: Request, res: Response) => {
		try {
			...
			if (!product) {
				return apiResponse.notFound(
					res,
					"Company is not published to store yet",
				);
			}
 ...
 */
router.get(
	"/admin/price-suggestions/:companyId",
	requireAdmin,
	async (req: Request, res: Response) => {
		try {
			// Check if user is admin
			...
			const suggestions =
				await priceAggregationService.getAggregatedPriceSuggestion(companyId);
			...
			return apiResponse.serverError(
				res,
				error.message || "Failed to fetch price suggestions",
			);
		}
	},
);
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			const suggestions =
				await priceAggregationService.getBatchPriceSuggestions(companyIds);
 ...
 */
router.post(
	"/admin/refresh-moneycontrol/:companyId",
	requireAdmin,
	async (req: Request, res: Response) => {
		try {
			// Check if user is admin
			...
			return apiResponse.serverError(
				res,
				error.message || "Failed to refresh MoneyControl price",
			);
		}
	},
);
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			const existingFinancials = await storage.getCompanyFinancialsByYear(
				id,
				financialYear,
			);
			...
			await storage.createUnlistedAuditLog({
				...
				previousData: existingFinancials
					? JSON.stringify(existingFinancials)
					: null,
			...
			console.log(
				`[ManualEntry] Admin ${req.user!.id} added financials for ${company.name} (${financialYear})`,
			);

			return apiResponse.success(res, {
				message: existingFinancials
 ...
 */
router.get(
	"/admin/companies/:id/data-quality",
	requireAdmin,
	async (req: Request, res: Response) => {
		try {
			const { id } = req.params;
 ...
 */
router.get(
	"/admin/companies/:id/identity-confidence",
	requireAdmin,
	async (req: Request, res: Response) => {
		try {
			const { id } = req.params;
			...
			return apiResponse.serverError(
				res,
				error.message || "Failed to fetch identity confidence",
			);
		}
	},
);
 ...
 */
router.post(
	"/admin/companies/:id/identity-confidence/recalculate",
	requireAdmin,
	async (req: Request, res: Response) => {
		try {
			const { id } = req.params;
			...
			if (!result.success) {
				return apiResponse.badRequest(
					res,
					result.error || "Failed to recalculate",
				);
			}
			...
			return apiResponse.serverError(
				res,
				"Failed to recalculate identity confidence",
			);
		}
	},
);
 ...
 */
router.post(
	"/admin/identity-confidence/batch-update",
	requireAdmin,
	async (req: Request, res: Response) => {
		try {
			...
			return apiResponse.serverError(
				res,
				"Failed to batch update identity confidence",
			);
		}
	},
);
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			return apiResponse.serverError(
				res,
				error.message || "Failed to enrich company data",
			);
		}
	},
);
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (!company.cin) {
				return apiResponse.badRequest(
					res,
					"Company CIN is required for MCA enrichment. Please set CIN first.",
				);
			}

			console.log(
				`📊 Starting MCA enrichment for ${company.name} (CIN: ${company.cin})`,
			);
			...
			return apiResponse.serverError(
				res,
				error.message || "Failed to enrich company with MCA data",
			);
		}
	},
);
