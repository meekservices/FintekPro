...
const proposalConfigSchema = z.object({
		...
		.refine(
			(data) => {
				const total =
					...
					(data.us_markets || 0) +
					(data.europe_markets || 0) +
					(data.asia_pacific_markets || 0) +
					(data.emerging_markets || 0) +
					(data.reit || 0) +
					(data.invit || 0) +
					(data.bonds || 0) +
					(data.listed_stocks || 0) +
					(data.unlisted_stocks || 0) +
					(data.etf || 0);
				return total === 100;
			},
	...
	coverPage: z.object({
		...
		date: z
			.string()
			.optional()
			.transform((v) => v || new Date().toLocaleDateString("en-IN")),
...
const detailedRecommendationSchema = z
	.object({
		action: z.string(),
...
router.get("/stats", async (req: Request, res: Response) => {
	try {
		...
		res.json({
			...
			totalDemoValue: Number.parseFloat(totalValue[0]?.sum || "0"),
			convertedValue: Number.parseFloat(convertedValue[0]?.sum || "0"),
		});
	} catch (error: any) {
...
agentDemoRouter.post("/generate-pdf", async (req: Request, res: Response) => {
	try {
		...
		if (!validationResult.success) {
			console.error(
				"[Generate PDF] Validation failed:",
		...
		if (clientId) {
			...
			if (client.length > 0) {
				const fullName =
					[client[0].firstName, client[0].lastName].filter(Boolean).join(" ") ||
					"Valued Client";
				clientData = {
					fullName,
		...
		const regulatorPdfConfig: Partial<ProposalPdfConfig> = {
			...
			proposedAllocation: {
				equity:
					...
					(config.assetAllocation.index || 0) +
					(config.assetAllocation.listed_stocks || 0) +
					(config.assetAllocation.etf || 0) +
					(config.assetAllocation.hybrid || 0) +
					(config.assetAllocation.unlisted_stocks || 0) +
				...
				debt:
					...
					(config.assetAllocation.bonds || 0),
				gold: config.assetAllocation.gold || 0,
				realestate:
					(config.assetAllocation.reit || 0) +
					(config.assetAllocation.invit || 0),
		...
		const pdfResult = await generateRegulatorGradePdf(
			regulatorPdfConfig as ProposalPdfConfig,
		);
			...
			await proposalAuditService.logEvent({
				...
				payloadAfter: {
					...
					sectionCustomizations:
						Object.keys(config.sectionCustomizations || {}).length > 0
		...
		const recommendationMap = [
			{
				...
				investType: "sip",
				reason: "Equity allocation for growth",
			},
			{
				key: "debt",
				type: "Debt MF",
				...
				investType: "lumpsum",
				reason: "Debt allocation for stability",
			},
			{
				key: "hybrid",
				type: "Hybrid MF",
				...
				investType: "sip",
				reason: "Hybrid allocation for balance",
			},
			{
				key: "gold",
				type: "Gold",
				name: "Gold ETF",
				investType: "lumpsum",
				reason: "Gold allocation for hedge",
			},
			{
				key: "index",
				type: "Index Fund",
				...
				investType: "sip",
				reason: "Index allocation for passive growth",
			},
			{
				key: "etf",
				type: "ETF",
				...
				investType: "lumpsum",
				reason: "ETF allocation for diversification",
			},
			{
				key: "us_markets",
				type: "US Markets",
			...
			},
			{
				...
				name: "Europe Fund",
				investType: "sip",
				reason: "European market exposure",
			},
			{
			...
			},
			{
				...
				investType: "sip",
				reason: "Emerging market exposure",
			},
			{
				key: "reit",
				type: "REIT",
			...
			},
			{
				...
				investType: "lumpsum",
				reason: "Infrastructure exposure",
			},
			{
				key: "bonds",
				type: "Bonds",
				name: "Direct Bond/NCD",
				investType: "lumpsum",
				reason: "Fixed income allocation",
			},
			{
				key: "listed_stocks",
				type: "Listed Stocks",
			...
			},
			{
				...
				name: "Pre-IPO Shares",
				investType: "lumpsum",
				reason: "Unlisted equity exposure",
			},
			{
				key: "cash",
				type: "Liquid",
		...
		const recommendations = recommendationMap
			...
			.map((r) => ({
				...
				productName: r.name,
				recommendedAmount:
					targetAmount * ((config.assetAllocation[r.key] || 0) / 100),
		...
		const proposalTitle =
			proposalName || `Investment Proposal - ${clientData.fullName}`;
