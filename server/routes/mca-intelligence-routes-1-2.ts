...
router.get(
	...
	async (req: Request, res: Response) => {
		...
		try {
			...
			const financials =
				await mcaIntelligenceService.getCompanyFinancialRatios(cin);
			...
			console.error("[MCA Routes] Financial ratios error:", {
				cin,
				error: error.message,
			});
...
router.get(
	...
	async (req: Request, res: Response) => {
		...
		try {
			const history = await mcaIntelligenceService.getFinancialHistory(
				cin,
				Math.min(limit, 20),
			);
			...
			console.error("[MCA Routes] Financial history error:", {
				cin,
				error: error.message,
			});
			res.status(500).json({
...
router.get(
	...
	async (req: Request, res: Response) => {
		...
		try {
			...
			console.error("[MCA Routes] Risk score error:", {
				cin,
				error: error.message,
			});
			res.status(500).json({
		...
		try {
			...
			res.json({
				...
				data: {
					...
					message:
						"MCA API requests are billed directly by Sandbox.co.in per API call",
				},
			});
		} catch (error: any) {
