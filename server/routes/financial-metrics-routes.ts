...
router.get("/stocks/:stockId", async (req: Request, res: Response) => {
	try {
		...
		const metrics =
			await financialMetricsRefreshService.getLatestStockMetrics(stockId);

		if (!metrics) {
			return res
				.status(404)
				.json({ error: "Metrics not found for this stock" });
		}
...
router.get("/stocks/:stockId/history", async (req: Request, res: Response) => {
	try {
		const { stockId } = req.params;
		const years = Number.parseInt(req.query.years as string) || 5;

		const metrics = await financialMetricsRefreshService.getStockMetricsHistory(
			stockId,
			years,
		);
		...
		console.error(
			"[FinancialMetrics] Error fetching stock metrics history:",
			error,
		);
...
router.get("/stocks/:stockId/trends", async (req: Request, res: Response) => {
	try {
		...
		const trends = await financialMetricsRefreshService.getMetricsTrend(
			stockId,
			metricNames,
			years,
		);
...
router.get("/symbol/:symbol", async (req: Request, res: Response) => {
	try {
		...
		const metrics = await financialMetricsRefreshService.getMetricsBySymbol(
			symbol.toUpperCase(),
		);
		...
		if (metrics.length === 0) {
			return res
				.status(404)
				.json({ error: "Metrics not found for this symbol" });
		}
		...
		console.error(
			"[FinancialMetrics] Error fetching metrics by symbol:",
			error,
		);
...
router.post("/compare", async (req: Request, res: Response) => {
	try {
		...
		const comparison = await financialMetricsRefreshService.compareStocks(
			stockIds,
			metricNames,
		);
...
router.get("/sector/:sector/averages", async (req: Request, res: Response) => {
	try {
		...
		const averages =
			await financialMetricsRefreshService.getSectorAverages(sector);
...
router.post("/refresh/:stockId", async (req: Request, res: Response) => {
	try {
		...
		const success =
			await financialMetricsRefreshService.refreshStockMetrics(stockId);
		...
		if (success) {
			...
			res
				.status(500)
				.json({ success: false, error: "Failed to refresh metrics" });
		}
	} catch (error) {
...
router.post("/refresh/batch", async (req: Request, res: Response) => {
	try {
		...
		const result =
			await financialMetricsRefreshService.refreshAllStockMetrics(batchSize);
...
router.get("/mutual-funds/:schemeCode", async (req: Request, res: Response) => {
	try {
		...
		if (!metrics) {
			return res
				.status(404)
				.json({ error: "Metrics not found for this scheme" });
		}
...
router.get(
	...
	async (req: Request, res: Response) => {
		try {
			...
			console.error(
				"[FinancialMetrics] Error fetching MF metrics history:",
				error,
			);
			res
				.status(500)
				.json({ error: "Failed to fetch mutual fund metrics history" });
		}
	},
);
...
router.get("/bonds/:isin/history", async (req: Request, res: Response) => {
	try {
		...
		console.error(
			"[FinancialMetrics] Error fetching bond metrics history:",
			error,
		);
...
router.get("/reits/:entityId", async (req: Request, res: Response) => {
	try {
		...
		if (!metrics) {
			return res
				.status(404)
				.json({ error: "Metrics not found for this REIT/InvIT" });
		}
...
router.get("/reits/:entityId/history", async (req: Request, res: Response) => {
	try {
		...
		console.error(
			"[FinancialMetrics] Error fetching REIT metrics history:",
			error,
		);
		res
			.status(500)
			.json({ error: "Failed to fetch REIT/InvIT metrics history" });
	}
});
...
router.get("/quality-scores/:stockId", async (req: Request, res: Response) => {
	try {
		...
		const qualityScores = {
			...
			piotroskiInterpretation: getPiotroskiInterpretation(
				metrics.piotroskiFScore,
			),
			altmanZScore: Number.parseFloat(metrics.altmanZScore?.toString() || "0"),
			altmanInterpretation: getAltmanInterpretation(
				Number.parseFloat(metrics.altmanZScore?.toString() || "0"),
			),
			beneishMScore: Number.parseFloat(
				metrics.beneishMScore?.toString() || "0",
			),
			accrualRatio: Number.parseFloat(metrics.accrualRatio?.toString() || "0"),
			earningsQuality: Number.parseFloat(
				metrics.earningsQuality?.toString() || "0",
			),
			earningsQualityInterpretation: getEarningsQualityInterpretation(
				Number.parseFloat(metrics.earningsQuality?.toString() || "0"),
			),
		};
	...
	async (req: Request, res: Response) => {
		try {
			...
			const sectorAverages = stock.broadSector
				? await financialMetricsRefreshService.getSectorAverages(
						stock.broadSector,
					)
				: {};
			...
			res.json({
				...
				valuation: {
					trailingPe: Number.parseFloat(metrics.trailingPe?.toString() || "0"),
					forwardPe: Number.parseFloat(metrics.forwardPe?.toString() || "0"),
					pegRatio: Number.parseFloat(metrics.pegRatio?.toString() || "0"),
					priceToBook: Number.parseFloat(
						metrics.priceToBook?.toString() || "0",
					),
					priceToSales: Number.parseFloat(
						metrics.priceToSales?.toString() || "0",
					),
					evToEbitda: Number.parseFloat(metrics.evToEbitda?.toString() || "0"),
				},
				profitability: {
					roe: Number.parseFloat(metrics.roe?.toString() || "0"),
					roce: Number.parseFloat(metrics.roce?.toString() || "0"),
					roic: Number.parseFloat(metrics.roic?.toString() || "0"),
					netMargin: Number.parseFloat(metrics.netMargin?.toString() || "0"),
					operatingMargin: Number.parseFloat(
						metrics.operatingMargin?.toString() || "0",
					),
				},
				growth: {
					revenueGrowthYoy: Number.parseFloat(
						metrics.revenueGrowthYoy?.toString() || "0",
					),
					epsGrowthYoy: Number.parseFloat(
						metrics.epsGrowthYoy?.toString() || "0",
					),
					revenueCagr3y: Number.parseFloat(
						metrics.revenueCagr3y?.toString() || "0",
					),
					epsCagr3y: Number.parseFloat(metrics.epsCagr3y?.toString() || "0"),
				},
				financial_health: {
					debtToEquity: Number.parseFloat(
						metrics.debtToEquity?.toString() || "0",
					),
					currentRatio: Number.parseFloat(
						metrics.currentRatio?.toString() || "0",
					),
					interestCoverage: Number.parseFloat(
						metrics.interestCoverage?.toString() || "0",
					),
					netDebtToEbitda: Number.parseFloat(
						metrics.netDebtToEbitda?.toString() || "0",
					),
				},
				quality: {
					piotroskiFScore: metrics.piotroskiFScore,
					altmanZScore: Number.parseFloat(
						metrics.altmanZScore?.toString() || "0",
					),
					earningsQuality: Number.parseFloat(
						metrics.earningsQuality?.toString() || "0",
					),
				...
				dividend: {
					yield: Number.parseFloat(metrics.dividendYield?.toString() || "0"),
					payoutRatio: Number.parseFloat(
						metrics.dividendPayoutRatio?.toString() || "0",
					),
			...
			console.error(
				"[FinancialMetrics] Error fetching recommendation data:",
				error,
			);
