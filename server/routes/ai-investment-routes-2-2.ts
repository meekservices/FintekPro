		...
		try {
			...
			res.json({
				...
				holdings: result.holdings.map((h: any) => ({
					...
					averagePrice:
						h.avgCostPerUnit || h.investedValue / (h.units || 1) || 0,
					...
					isinMatched:
						!!h.isin && h.confidenceScore && h.confidenceScore >= 0.7,
...
router.post("/portfolio/:clientId/import-previewed", async (req, res) => {
	try {
		...
		if (!clientInfo.isProspect && !clientInfo.isUser) {
			return res
				.status(404)
				.json({ error: "Client not found. Must be a valid prospect or user." });
		}
			...
			const normalizedHoldings = holdings.map((h: any) => ({
				...
				assetType:
					h.assetType === "mutual_fund"
						...
						: h.assetType || "equity",
				productType:
					h.productType || (h.assetType === "mutual_fund" ? "MF" : undefined),
				quantity: Number.parseFloat(h.quantity || h.units) || 0,
				averageCost: Number.parseFloat(h.averagePrice || h.avgPrice) || 0,
				currentValue: Number.parseFloat(h.value || h.currentValue) || 0,
				investedValue: Number.parseFloat(h.investedValue) || undefined,
				folioNumber: h.folioNumber,
				purchaseDate: h.purchaseDate,
			}));

			const updatedHoldings =
				await prospectPortfolioSyncService.replaceAllHoldings(
					clientId,
					normalizedHoldings,
				);
		...
		for (const holding of holdings) {
			const symbol =
				holding.symbol ||
				holding.name?.replace(/[^A-Za-z0-9]/g, "").substring(0, 20) ||
				"UNKNOWN";
			...
			await db.insert(portfolioHoldings).values({
				...
				productType:
					holding.productType ||
					(holding.assetType === "mutual_fund" ? "MF" : null),
		...
		const totalValue = holdings.reduce(
			(sum: number, h: any) => sum + (h.value || h.currentValue || 0),
			0,
		);
		await db
			.update(portfolios)
		...
		res
			.status(500)
			.json({ error: error.message || "Failed to import holdings" });
	}
});

// POST endpoint for CAS PDF upload (direct without preview)
router.post(
	"/portfolio/:clientId/upload-cas",
	upload.single("file"),
	async (req, res) => {
		try {
			const { clientId } = req.params;
			...
			if (!clientInfo.isProspect && !clientInfo.isUser) {
				return res
					.status(404)
					.json({
						error: "Client not found. Must be a valid prospect or user.",
					});
			}
			...
			if (!result.success || result.holdings.length === 0) {
				return res.status(400).json({
					error:
						result.errors?.join("; ") ||
						"Could not parse any holdings from the PDF",
				});
			}
			...
			if (clientInfo.isProspect) {
				const normalizedHoldings = result.holdings.map((h: any) => ({
					...
					symbol: (
						h.isin ||
						h.symbol ||
						h.name?.replace(/[^A-Za-z0-9]/g, "").substring(0, 20)
					)?.toUpperCase(),
					assetType: h.assetType || "mutual_fund",
					quantity: h.units || h.quantity || 0,
					averageCost:
			...
			for (const holding of result.holdings) {
				const symbol =
					holding.isin ||
					holding.symbol ||
					((holding as any).schemeName || holding.name || "")
						.replace(/[^A-Za-z0-9]/g, "")
						.substring(0, 20);
				...
				await db.insert(portfolioHoldings).values({
					...
					avgPrice: String(
						holding.avgCostPerUnit ||
							(holding.investedValue ?? 0) / ((holding as any).units || 1) ||
							0,
					),
...
router.post("/portfolio/validate-paste", async (req, res) => {
	try {
		...
		const result = await unifiedPortfolioImportService.importFromCSV(
			text,
			"clipboard-paste.csv",
		);
		...
		if (!result.success || result.holdings.length === 0) {
			return res.status(400).json({
				...
				error:
					result.errors?.[0] ||
					"Could not parse any holdings from the pasted data",
				errors: result.errors,
			});
		}
		...
		res.json({
			...
			totalPartial: result.holdings.filter((h: any) => !h.isin && h.symbol)
				.length,
			totalUnmatched: result.holdings.filter((h: any) => !h.isin && !h.symbol)
				.length,
			errors: result.errors,
		});
	} catch (error: any) {
		...
		res
			.status(500)
			.json({ error: error.message || "Failed to validate pasted data" });
	}
});

// In-memory import history store (per client, last 10 imports)
const importHistoryStore: Record<
	string,
	Array<{
		id: string;
		timestamp: string;
...
router.post("/portfolio/:clientId/import-history", async (req, res) => {
	try {
		...
		res
			.status(500)
			.json({ error: error.message || "Failed to record import history" });
	}
});
