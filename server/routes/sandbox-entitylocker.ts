...
export function registerEntityLockerRoutes(app: Express): void {
	 ...
	 */
	app.post(
		"/api/kyc/entitylocker/sessions/init",
		async (req: Request, res: Response) => {
			try {
				...
				if (
					...
					Number(consentExpiry) < Date.now() + 3600000
				) {
					return res.status(400).json({
						success: false,
				...
				return res.status(500).json({
					...
					message:
						error instanceof Error
							? error.message
							: "Failed to initiate EntityLocker session",
				});
			}
		},
	);
	...
	app.post(
		...
		async (req: Request, res: Response) => {
			try {
				...
				const result = await sandboxKYCService.fetchEntityLockerDocument(
					sessionId,
					documentType,
				);
				...
				return res.status(500).json({
					...
					message:
						error instanceof Error
							? error.message
							: "Failed to fetch EntityLocker document",
				});
			}
		},
	);
			...
			supported_document_types: [
				{
					...
					description:
						"GST registration certificate — GSTIN, legal name, trade name, registration date, address",
				},
				{
					type: "incorporation_certificate",
					description:
						"MCA/ROC incorporation certificate — CIN, company name, date of incorporation, registered address",
				},
				{
					type: "bank_account",
					description:
						"Business bank account — account number, IFSC, account holder name match, bank name",
				},
			],
			flow: [
				"1. POST /api/kyc/entitylocker/sessions/init → get { sessionId, authorizationUrl }",
	...
	console.log(
		"✅ EntityLocker (KYB) routes registered (/api/kyc/entitylocker/*)",
	);
}
