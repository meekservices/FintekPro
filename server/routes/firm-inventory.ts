...
import {
	firmInventorySyncService,
	FirmTxInput,
} from "../services/firm-inventory-sync-service";
import { db } from "../db";
...
export function registerFirmInventoryRoutes(app: Express) {
	// ── Holdings ───────────────────────────────────────────────────────────────
	app.get(
		"/api/admin/firm-inventory/holdings",
		requireAdmin,
		async (req, res) => {
			try {
				const holdings = await firmInventorySyncService.getAllHoldings();
				const totalCost = holdings.reduce(
					(s, h) => s + Number.parseFloat(h.totalCostValue ?? "0"),
					0,
				);
				const totalMV = holdings.reduce(
					(s, h) =>
						s +
						Number.parseFloat(h.currentMarketValue ?? h.totalCostValue ?? "0"),
					0,
				);
				res.json({
					holdings,
	...
	// ── Transactions ───────────────────────────────────────────────────────────
	app.get(
		"/api/admin/firm-inventory/transactions",
		requireAdmin,
		async (req, res) => {
			try {
				const page = Math.max(
			...
			try {
				...
				if (
					...
					return res
						...
						.json({
							error:
								"transactionType, securityName, quantity, totalValue, transactionDate are required",
						});
				}
	...
	// ── Zoho Books Balance ─────────────────────────────────────────────────────
	app.get(
		"/api/admin/firm-inventory/balance",
		requireAdmin,
		async (req, res) => {
			try {
	...
	app.post(
		...
		async (req, res) => {
			try {
				...
				const result =
					await firmInventorySyncService.syncTransactionToZohoBooks(id);
				res.json(result);
			} catch (e: any) {
				res.status(500).json({ error: e.message });
			}
	...
	app.post("/api/webhooks/zoho-books/firm-inventory", async (req, res) => {
		try {
			...
			console.log(
				`[FirmInventory webhook] event=${eventType} module=${module}`,
			);

			if (
				module === "bills" &&
				(eventType.includes("payment") || eventType.includes("paid"))
			) {
				await firmInventorySyncService.processZohoBooksBillPaid(eventId, {
					...
					total: Number.parseFloat(data.total || data.amount || "0"),
