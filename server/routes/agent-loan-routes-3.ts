	...
	try {
		...
		const conditions = [
			...
			or(
				...
				eq(bankerContacts.agentId, "system"),
			),
		];
...
router.post("/banker-contacts", async (req: Request, res: Response) => {
	try {
		...
		const [contact] = await db
			...
			.onConflictDoUpdate({
				target: [
					bankerContacts.agentId,
					bankerContacts.financierName,
					bankerContacts.bankerMobile,
				],
				set: {
...
router.put("/banker-contacts/:id", async (req: Request, res: Response) => {
	try {
		...
		const [updated] = await db
			...
			.where(
				and(
					eq(bankerContacts.id, req.params.id),
					or(
						eq(bankerContacts.agentId, agentId),
						eq(bankerContacts.agentId, "system"),
					),
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			const dsaCodeIdx = headers.findIndex(
				(h: string) => h.includes("dsa") || h === "code",
			);
			const financierIdx = headers.findIndex(
				(h: string) =>
					h.includes("institution") ||
					h.includes("financier") ||
					h.includes("bank"),
			);
			const productIdx = headers.findIndex((h: string) =>
				h.includes("product"),
			);
			...
			if (financierIdx === -1) {
				return res
					...
					.json({
						success: false,
						error: "Could not find financier/institution column in Excel",
					});
			}
			...
			for (let i = 1; i < rows.length; i++) {
				...
				const dsaCode =
					dsaCodeIdx >= 0 && row[dsaCodeIdx]
						? String(row[dsaCodeIdx]).trim()
						: undefined;
				const productRaw =
					productIdx >= 0 && row[productIdx]
				...
				const phone =
					phoneIdx >= 0 && row[phoneIdx]
						? String(row[phoneIdx]).trim().replace(/\D/g, "").slice(-10)
						: undefined;
			...
			console.log(
				`✅ [BankerContacts] Excel import: ${imported} imported, ${skipped} skipped`,
			);
			res.json({
				success: true,
				data: {
		...
		try {
			...
			if (!connection) {
				return res
					...
					.json({
						success: false,
						error: "No Zoho CRM connection found for this agent",
					});
			}
				...
				for (const contact of contacts) {
					try {
						...
						if (contact.zohoCrmId) {
							await apiClient.put(`/Contacts/${contact.zohoCrmId}`, {
								data: [zohoData],
							});
						} else {
							const resp: any = await apiClient.post("/Contacts", {
								data: [zohoData],
							});
						...
						console.warn(
							`[Zoho Sync] Failed to sync contact ${contact.bankerName}:`,
							syncErr.message,
						);
					}
				}
			...
			if (direction === "pull" || direction === "both") {
				try {
					const resp: any = await apiClient.get("/Contacts", {
						params: {
							fields: "id,Last_Name,Company,Phone,Email,Title,Description",
							per_page: "200",
						},
					});
						...
						try {
							await db
								...
								.values({
									...
									bankerMobile:
										zc.Phone?.replace(/\D/g, "").slice(-10) || undefined,
			...
			console.log(
				`✅ [BankerContacts] Zoho CRM sync complete: ${synced} records (direction: ${direction})`,
			);
...
router.get("/financier-suggestions", async (req: Request, res: Response) => {
	try {
		...
		let results = INDIAN_FINANCIERS.map((f) => ({
			...f,
		...
		if (agentId) {
			const contacts = await db
				.select({
					financierName: bankerContacts.financierName,
					dsaCode: bankerContacts.dsaCode,
				})
				.from(bankerContacts)
				.where(
					and(
			...
			for (const c of contacts) {
				if (
					c.financierName &&
					!existingNames.has(c.financierName.toLowerCase())
				) {
					results.push({
						name: c.financierName,
						type: "saved",
						source: "contact" as const,
						dsaCode: c.dsaCode || undefined,
					});
