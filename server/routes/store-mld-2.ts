...
router.delete(
	...
	async (req, res): Promise<void> => {
		try {
			...
			await db
				.delete(mldMonthwisePerformance)
	...
	try {
		...
		holdings.forEach((h) => {
			if (h.entryStatus === "approved") {
				totalInvested += Number.parseFloat(h.totalInvested || "0");
				totalCurrentValue += Number.parseFloat(
					h.currentValue || h.totalInvested || "0",
				);
				approvedCount++;
...
router.post("/portfolio/mld", requireAuth, async (req, res): Promise<void> => {
	try {
		...
		if (
			...
			res.status(400).json({
				error:
					"Missing required fields: isin, mldName, purchasePrice, purchaseDate, quantity",
			});
			return;
		}

		// Calculate derived fields
		const purchasePrice = Number.parseFloat(holdingData.purchasePrice);
		const quantity = Number.parseFloat(holdingData.quantity);
		const totalInvested = purchasePrice * quantity;
		const currentPrice = holdingData.currentPrice
			? Number.parseFloat(holdingData.currentPrice)
			: purchasePrice;
		...
		if (!mldId && holdingData.isin) {
			const [existingMld] = await db
				.select()
				.from(mldMaster)
				.where(eq(mldMaster.isin, holdingData.isin));
			if (existingMld) {
		...
		try {
			...
			if (
				...
				const purchasePrice = Number.parseFloat(
					updateData.purchasePrice || existing.purchasePrice || "0",
				);
				const quantity = Number.parseFloat(
					updateData.quantity || existing.quantity || "0",
				);
				const totalInvested = purchasePrice * quantity;
				...
				updateData.unrealizedGainLoss = (currentValue - totalInvested).toFixed(
					2,
				);
				updateData.unrealizedGainLossPercent =
					totalInvested > 0
...
router.get(
	...
	async (req, res): Promise<void> => {
		try {
			...
			const conditions = [];
			if (status)
				conditions.push(eq(clientPortfolioMld.entryStatus, status as string));
...
router.put(
	"/portfolio/admin/mld/:id/approve",
	requireAdmin,
	async (req, res): Promise<void> => {
		try {
			const { id } = req.params;
...
function calculateScenarioPayoffs(mld: MldMasterRecord): ScenarioPayoffs {
	const faceValue = Number.parseFloat(mld.faceValue || "1000000");
	const barrier = Number.parseFloat(mld.barrierLevel || "0.9");
	const participation = Number.parseFloat(mld.participationRate || "1");
	const cap = Number.parseFloat(mld.cap || "1.5");
	const floor = Number.parseFloat(mld.floor || "1");

	const scenarios: ScenarioPayoffs = {
		bull: { underlyingReturn: 0.2, payoff: 0, irr: 0 },
	...
	Object.keys(scenarios).forEach((key) => {
		...
		switch (mld.payoffType) {
			case "digital":
				payoffMultiple =
					scenario.underlyingReturn >= 0
						? Math.min(1 + scenario.underlyingReturn * participation, cap)
						: floor;
				break;
			case "barrier":
				payoffMultiple =
					scenario.underlyingReturn >= -(1 - barrier)
						? Math.min(1 + scenario.underlyingReturn * participation, cap)
						: floor;
				break;
			case "sharkfin":
				if (
					scenario.underlyingReturn > 0 &&
					scenario.underlyingReturn <= cap - 1
				) {
			...
			case "participation":
				payoffMultiple =
					floor + Math.max(0, scenario.underlyingReturn * participation);
				break;
			default:
		...
		const maturityYears = maturityDate
...
function calculateMldAnalytics(mld: MldMasterRecord) {
	...
	const yearsToMaturity = Math.max(
		(mDate.getTime() - Date.now()) / (365 * 24 * 60 * 60 * 1000),
		0.01,
	);
	...
	if (
		mld.rating &&
		["BBB", "BBB-", "BB+", "BB", "B"].some((r) => mld.rating!.includes(r))
	) {
	...
	return {
		...
		aiRecommendation:
			mld.aiRecommendation ||
			"Suitable for moderate risk investors seeking structured returns",
	};
}
	...
	for (let x = -30; x <= 40; x += 5) {
		...
		switch (mld.payoffType) {
			case "digital":
				payoffMultiple =
					underlyingReturn >= 0
						? Math.min(1 + underlyingReturn * participation, cap)
						: floor;
				break;
			case "barrier":
				payoffMultiple =
					underlyingReturn >= -(1 - barrier)
						? Math.min(1 + underlyingReturn * participation, cap)
						: floor;
				break;
			case "sharkfin":
		...
		try {
			...
			if (useSample === "true") {
				...
				if (listings.length === 0) {
					console.log(
						"[BSE Import] No live MLDs found, falling back to sample data",
					);
					listings = generateSampleMldListings();
				}
			}
			...
			const existingIsinSet = new Set(
				existingIsins.map((e) => normalizeIsin(e.isin)),
			);
			...
			const duplicateCount = previewListings.filter(
				(l) => l.isDuplicate,
			).length;
	...
	async (req, res): Promise<void> => {
		try {
			...
			if (useSample === "true") {
				...
				if (listings.length === 0) {
					console.log(
						"[NSE Import] No live MLDs found, falling back to sample data",
					);
					listings = generateSampleNseMldListings();
				}
			}
...
router.post(
	...
	async (req, res): Promise<void> => {
		try {
			...
			console.log(
				`[${source} Import] Starting import of ${listings.length} MLDs...`,
			);
				...
				try {
					...
					const [newMld] = await db
						...
						.values({
							...
							issueDate:
								listing.issueDate || new Date().toISOString().split("T")[0],
							maturityDate:
			...
			console.log(
				`[BSE Import] Completed: ${imported.length} imported, ${skipped.length} skipped, ${importErrors.length} errors`,
			);
