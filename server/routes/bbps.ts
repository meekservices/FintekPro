...
export async function registerBBPSRoutes(app: Express): Promise<void> {
	...
	digilockerService.initializeDigiLockerApp().catch((err: any) => {
		console.warn(
			"⚠️ DigiLocker optional service unavailable, using Sandbox API fallback",
		);
	});
			...
			try {
				...
				if (otpResponse.success && otpResponse.ref_id) {
					return res.json({
						...
						message:
							"OTP sent to Aadhaar-linked mobile. Please verify to fetch details.",
					});
				}
				...
				return res.status(503).json({
					success: false,
					error:
						"All verification services temporarily unavailable. Please enter details manually.",
					fallback: "manual",
				});
			}
		} catch (error: any) {
	...
	app.get("/api/digilocker/widget-config", async (req, res) => {
		try {
			...
			const config = await digilockerService.generateWidgetConfig(
			...
			res
				.status(500)
				.json({ error: "Failed to generate widget configuration" });
		}
	});
	...
	app.post("/api/digilocker/initiate-sharing", async (req, res) => {
		try {
			...
			try {
				const session = await sandboxKYCService.initiateDigiLockerSession(
					redirectUri,
					docTypes,
					flow,
				);
				...
				return res.json({
					...
					message:
						"Please complete the authentication in the DigiLocker window.",
				});
			} catch (sandboxErr: any) {
				if (
					sandboxErr.message?.includes("Test environment") ||
					sandboxErr.message?.includes("does not match")
				) {
					...
					console.log(
						"[DigiLocker] Sandbox test env - using simulated session:",
						mockSessionId,
					);
					return res.json({
						success: true,
						...
						message:
							"DigiLocker session created. In production, you will be redirected to authenticate with DigiLocker.",
						isSimulated: true,
					});
				}
				throw sandboxErr;
			...
			res.status(500).json({
				...
				message:
					error.message ||
					"Failed to initiate document sharing. Please try again.",
			});
		}
	});
	...
	app.get("/api/digilocker/session/:sessionId/status", async (req, res) => {
		try {
			...
			const status =
				await sandboxKYCService.getDigiLockerSessionStatus(sessionId);
	...
	app.post("/api/digilocker/fetch-document", async (req, res) => {
		try {
			...
			if (!sessionId || !documentType) {
				return res
					.status(400)
					.json({ error: "Session ID and document type are required" });
			}

			const document = await sandboxKYCService.fetchDigiLockerDocument(
				sessionId,
				documentType,
			);
	...
	app.post("/api/digilocker/share-document", async (req, res) => {
		try {
			...
			if (!uri || !docType || !txn) {
				return res
					.status(400)
					.json({ error: "Missing required document metadata" });
			}
			...
			const sharedDocument = await digilockerService.handleDocumentSharing(
				req.user.id,
				metadata,
			);
	...
	app.post("/api/digilocker/auto-populate-kyc", async (req, res) => {
		try {
			...
			try {
				...
				console.warn(
					"DigiLocker KYC failed, trying BSE Star fallback:",
					digilockerError,
				);
