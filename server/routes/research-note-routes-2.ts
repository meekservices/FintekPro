...
async function buildUnlistedReportData(cin: string): Promise<any> {
	...
	if (lookupCin && credhiveService.isAvailable()) {
		...
		if (profileRes.status === "fulfilled" && profileRes.value.success)
			credhiveProfile = profileRes.value.data;
		if (finRes.status === "fulfilled" && finRes.value.success)
			credhiveFinancials = finRes.value.data ?? [];
		if (dirRes.status === "fulfilled" && dirRes.value.success)
			credhiveDirectors = dirRes.value.data ?? [];
					...
					rows: [
						{
							...
							values: sortedStmts.map((s) =>
								s.revenue ? s.revenue / 1e7 : null,
							),
						},
						{
							label: "EBITDA (₹ Cr)",
							values: sortedStmts.map((s) =>
								s.ebitda ? s.ebitda / 1e7 : null,
							),
						},
						{
							label: "EBIT (₹ Cr)",
							values: sortedStmts.map((s) => (s.ebit ? s.ebit / 1e7 : null)),
						},
						{
							label: "PAT (₹ Cr)",
					...
					rows: [
						{
							...
							values: sortedStmts.map((s) =>
								s.total_assets ? s.total_assets / 1e7 : null,
							),
						},
						{
							label: "Networth (₹ Cr)",
							values: sortedStmts.map((s) =>
								s.networth ? s.networth / 1e7 : null,
							),
						},
						{
							label: "Total Debt (₹ Cr)",
							values: sortedStmts.map((s) =>
								s.total_debt ? s.total_debt / 1e7 : null,
							),
						},
						{
							label: "Share Capital (₹ Cr)",
							values: sortedStmts.map((s) =>
								s.share_capital ? s.share_capital / 1e7 : null,
							),
						},
					],
				}
					...
					rows: [
						{
							...
							values: sortedStmts.map((s) =>
								s.operating_cash_flow ? s.operating_cash_flow / 1e7 : null,
							),
						},
						{
							label: "Investing CF (₹ Cr)",
							values: sortedStmts.map((s) =>
								s.investing_cash_flow ? s.investing_cash_flow / 1e7 : null,
							),
						},
						{
							label: "Free CF (₹ Cr)",
							values: sortedStmts.map((s) =>
								s.free_cash_flow ? s.free_cash_flow / 1e7 : null,
							),
						},
					],
				}
	...
	const directors =
		credhiveDirectors.length > 0
			? credhiveDirectors
			: dbCompany?.directors || [];

	// ── 9. Build response (compatible with listed PreviewData + unlisted extras) ──
	const { ratios, valuation, thesis, risks, ratingScore, ratingLabel, fhs } =
		analytics;
	...
	const computedPE: number | null = (() => {
		...
		return Number.isFinite(v) && v > 0 ? Number.parseFloat(v.toFixed(1)) : null;
	})();

	// Price-to-Book = transaction price / book value per share
	const computedPB: number | null = (() => {
		if (!transactionPrice || !computedBookValue || computedBookValue <= 0)
			return null;
	...
	return {
		...
		cin: compCin,
		incorporationDate:
			credhiveProfile?.date_of_incorporation ||
			dbCompany?.incorporation_date ||
			null,
		...
		financials: {
			...
			faceValue: dbCompany?.face_value
				? Number.parseFloat(dbCompany.face_value)
				: null,
			vwap: null,
		...
		rating: {
			...
			score: ratingScore,
			breakdown: {
				fundamentals: Math.round(fhs * 0.5),
		...
		managementNote:
			directors.length > 0
				? `${companyName} is led by ${directors
						.slice(0, 3)
						.map((d: any) => d.name || d.din)
						.join(
							", ",
						)}${directors.length > 3 ? ` and ${directors.length - 3} others` : ""}.`
...
router.post("/preview-unlisted", async (req: Request, res: Response) => {
	try {
		...
		res
			.status(500)
			.json({
				error: err.message || "Failed to generate unlisted research data",
			});
	}
});
...
router.post("/generate/pdf-unlisted", async (req: Request, res: Response) => {
	try {
		...
		res.setHeader(
			"Content-Disposition",
			`attachment; filename="${safeName}_Unlisted_Report.pdf"`,
		);
		res.send(buffer);
	} catch (err: any) {
		res
			.status(500)
			.json({ error: err.message || "Failed to generate unlisted PDF" });
	}
});
...
router.post("/generate/ppt-unlisted", async (req: Request, res: Response) => {
	try {
		...
		res.setHeader(
			"Content-Disposition",
			`attachment; filename="${safeName}_Unlisted_Research.pptx"`,
		);
		res.send(buffer);
	} catch (err: any) {
		res
			.status(500)
			.json({ error: err.message || "Failed to generate unlisted PPT" });
	}
});
...
router.get("/sector-picks", async (req: Request, res: Response) => {
	try {
		...
		const maxResults = Math.min(Number.parseInt(limit) || 5, 10);
    ...
    `);
		...
		const scored = stocks.map((s: any) => {
			let score = 0;
			const roe = s.roe ? Number.parseFloat(s.roe) : null;
			const pe = s.pe_ratio ? Number.parseFloat(s.pe_ratio) : null;
			const revGrowth = s.revenue_growth
				? Number.parseFloat(s.revenue_growth)
				: null;
			const de = s.debt_to_equity ? Number.parseFloat(s.debt_to_equity) : null;
			const epsGrowth = s.earnings_growth
				? Number.parseFloat(s.earnings_growth)
				: null;
			const roce = s.roce ? Number.parseFloat(s.roce) : null;
			...
			return {
				...
				currentPrice: s.current_price
					? Number.parseFloat(s.current_price)
					: null,
				pe: pe,
				roe: roe,
		...
		const buys = scored
			.filter(
				(s) => s.recommendation === "STRONG BUY" || s.recommendation === "BUY",
			)
