...
export function registerSecurityMasterCreditRatingRoutes(app: Express): void {
    ...
    `);
			...
			res
				.status(500)
				.json({ message: "Internal server error searching securities" });
		}
	});
    ...
    `);
			...
			if (!results.rows || results.rows.length === 0) {
				return res
					.status(404)
					.json({ message: `Security with ISIN ${isin} not found` });
			}
			...
			res
				.status(500)
				.json({ message: "Internal server error fetching security data" });
		}
	});
	...
	app.get("/api/marketdata/symbol-map/:isin", async (req, res) => {
		try {
			...
			res
				.status(500)
				.json({ message: "Internal server error fetching symbol mappings" });
		}
	});
	...
	app.get("/api/marketdata/resolve-symbol", async (req, res) => {
		try {
			...
			if (!provider || !symbol) {
				return res
					.status(400)
					.json({ message: "Provider and symbol are required" });
			}
			const isin = await symbolMappingService.resolveSymbol(
				provider as string,
				symbol as string,
			);
			if (!isin) {
			...
			res
				.status(500)
				.json({ message: "Internal server error resolving symbol" });
		}
	});
	...
	app.post("/api/marketdata/symbol-map", async (req, res) => {
		try {
			...
			res
				.status(500)
				.json({ message: "Internal server error adding symbol mapping" });
		}
	});
          ...
          FROM government_securities WHERE isin = ${isin} LIMIT 1`,
			...
			if (gSec) {
				return res.json({
					...
					agency:
						gSec.issuer === "Government of India"
							? "GOI / RBI"
							: "State Government",
					ratingDate: null,
					...
					ratingNote:
						"Government securities carry sovereign (SOV) credit rating — the highest possible rating, backed by the Government of India's full faith and credit.",
				});
			}
			...
			if (stock) {
				return res.json({
					...
					ratingNote:
						"Traditional credit ratings (CRISIL/ICRA) are not issued for equity shares. Use the FintekPro Smart Rating for a proprietary multi-factor quality assessment.",
			...
			if (mf) {
				return res.json({
					...
					ratingSystem: "fintekpro_smart_rating",
					ratingNote:
						"Mutual funds are evaluated using the FintekPro Smart Rating — a proprietary 1–5 star system based on risk-adjusted returns, quality, liquidity, momentum, and valuation.",
					fintekProRating: mf.crisilRating,
					fintekProStars: mf.crisilRating,
					fintekProCategory: mf.crisilCategory,
					fintekProPercentile: mf.crisilPercentile
						? Number(mf.crisilPercentile)
						: null,
					fintekProOverallScore: mf.crisilOverallScore
						? Number(mf.crisilOverallScore)
						: null,
					evaluationDate: mf.crisilEvaluationDate,
					fundHouse: mf.fundHouse,
					category: mf.category,
					disclaimer:
						"FintekPro Smart Ratings are proprietary and not affiliated with CRISIL, ICRA, or any third-party agency.",
				});
			}
	...
	app.get("/api/marketdata/credit-rating/:isin/history", async (req, res) => {
		try {
			const history = await creditRatingsService.getRatingHistory(
				req.params.isin,
			);
			res.json(history);
		} catch (error) {
