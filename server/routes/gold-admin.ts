...
router.get("/sgb-issues", async (req: Request, res: Response) => {
	try {
		const issues = await db
			...
			.orderBy(sgbPrimaryIssues.issueOpenDate);
		res.json({ success: true, issues });
	} catch (error) {
		console.warn(
			`[Gold] SGB issues fetch failed: ${(error as any)?.message || "Unknown error"}`,
		);
		res.json({ success: true, issues: [] });
	}
});
...
router.get("/products", async (req: Request, res: Response) => {
	try {
		...
		console.warn(
			`[Gold] Products fetch failed: ${(error as any)?.message || "Unknown error"}`,
		);
		res.json({ success: true, products: [] });
	}
});
...
router.patch("/sgb-issues/:id/publish", async (req: Request, res: Response) => {
	try {
		...
		await db
			...
			.set({ isPublished })
			.where(eq(sgbPrimaryIssues.id, Number.parseInt(id)));
