	...
	switch (alertType) {
		case "market_price":
			triggerCondition = {
				...
				value: Number.parseFloat(targetValue),
			};
		...
		case "market_change":
			triggerCondition = {
				...
				value: Number.parseFloat(threshold),
		...
		case "spending_budget":
			triggerCondition = {
				type: "category_limit",
				value: Number.parseFloat(targetValue),
				threshold: Number.parseFloat(threshold || "80"),
			};
...
export function registerAlertSystemRoutes(app: Express): void {
	...
	app.get("/api/alerts", requireAuth, async (req, res) => {
		try {
			...
			let alerts = await storage.getUserAlerts(
				req.user!.id,
				category as string,
			);
			...
			if (status === "active") alerts = alerts.filter((a: any) => a.isActive);
				...
				alerts = alerts.filter((a: any) => !a.isActive);
			res.json(alerts);
		} catch (error) {
	...
	app.patch("/api/alerts/:id", requireAuth, async (req, res) => {
		try {
			...
			if (!alert || alert.userId !== req.user!.id)
				return res.status(404).json({ message: "Alert not found" });
			const updatedAlert = await storage.updateUserAlert(
				req.params.id,
				req.body,
			);
	...
	app.get("/api/expenses/by-category", async (_req, res) => {
		res.json({
			categories: [],
			total: 0,
			message: "Expense categories feature coming soon",
		});
	});
	...
	app.get("/api/insights", async (_req, res) => {
		res.json({
			insights: [],
			recommendations: [],
			message: "Financial insights feature coming soon",
		});
	});
}
