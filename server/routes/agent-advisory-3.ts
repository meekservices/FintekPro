...
export function registerAgentAdvisoryPart3Routes(app: Express) {
	app.post(
		"/api/agent/advisory-sessions/:sessionId/validate-transition",
		requireAgentPortal,
		async (req: Request, res: Response) => {
			try {
				...
				const validation =
					await ProposalOrchestrator.validateWorkflowTransition(
						sessionId,
						targetState,
					);
	...
	app.get(
		"/api/agent/treasury/eligible-clients",
		requireAgentPortal,
		async (req: Request, res: Response) => {
			try {
	...
	app.post(
			...
			try {
				...
				if (!clientId || !entityName || !totalCashAvailable) {
					return res.status(400).json({
						error:
							"Client ID, entity name, and total cash available are required",
					});
				}
      ...
      `)
				...
				if (!relationship) {
					return res
						.status(403)
						.json({ error: "Client is not assigned to you" });
				}
      ...
      `)
				...
				if (!eligibleTiers.includes(client.kyc_tier?.toLowerCase())) {
					return res.status(400).json({
						error:
							"Client requires Enhanced or Accredited KYC tier for treasury services",
	...
	app.patch(
		"/api/agent/treasury/mandates/:mandateId",
		requireAgentPortal,
		async (req: Request, res: Response) => {
			try {
				const agentId = (req.user as any).id;
      ...
      `)
				...
				if (!mandate) {
					return res
						.status(404)
						.json({ error: "Mandate not found or not accessible" });
				}

				const updateFields: Record<string, unknown> = {};
				if (totalCashAvailable !== undefined)
					updateFields.totalCashAvailable = String(totalCashAvailable);
				if (maxCreditRisk !== undefined)
					updateFields.maxCreditRisk = String(maxCreditRisk);
				if (maxDurationDays !== undefined)
					updateFields.maxDurationDays = Number(maxDurationDays);
				if (maxSingleCounterparty !== undefined)
					updateFields.maxSingleCounterparty = String(maxSingleCounterparty);
				if (capitalProtection !== undefined)
					updateFields.capitalProtection = Boolean(capitalProtection);
				if (liquidityManagement !== undefined)
					updateFields.liquidityManagement = Boolean(liquidityManagement);
				if (yieldEnhancement !== undefined)
					updateFields.yieldEnhancement = Boolean(yieldEnhancement);
				if (liabilityMatching !== undefined)
					updateFields.liabilityMatching = Boolean(liabilityMatching);
				if (makerCheckerEnabled !== undefined)
					updateFields.makerCheckerEnabled = Boolean(makerCheckerEnabled);
	...
	app.post(
		"/api/agent/treasury/mandates/:mandateId/deactivate",
		requireAgentPortal,
		async (req: Request, res: Response) => {
			try {
				const agentId = (req.user as any).id;
      ...
      `)
				...
				if (!mandate) {
					return res
						.status(404)
						.json({ error: "Active mandate not found or not accessible" });
				}
      ...
      `)
				...
				if (Number.parseInt(pendingProposals.count) > 0) {
					return res.status(400).json({
						error:
							"Cannot deactivate mandate with pending proposals. Please resolve pending proposals first.",
					});
				}
      ...
      `);
				...
				res
					.status(500)
					.json({ error: "Failed to deactivate treasury mandate" });
			}
		},
	);

	app.get(
		"/api/agent/treasury/clients",
		requireAgentPortal,
		async (req: Request, res: Response) => {
			try {
				const agentId = (req.user as any).id;
	...
	app.post(
		"/api/agent/treasury/proposals/generate",
		requireAgentPortal,
		async (req: Request, res: Response) => {
			try {
				const agentId = (req.user as any).id;
      ...
      `)
				...
				const totalCash = Number.parseFloat(mandate.total_cash_available) || 0;
				...
				const expectedTotalYield = recommendedAllocation
					.reduce((sum, a) => sum + (a.expectedYield * a.amount) / totalCash, 0)
					.toFixed(2);

				const initialStatus = makerCheckerEnabled
					? "pending_maker"
					: "pending_approval";
	...
	app.post(
		"/api/agent/treasury/proposals/:proposalId/maker-action",
		requireAgentPortal,
		async (req: Request, res: Response) => {
			try {
				const agentId = (req.user as any).id;
      ...
      `)
				...
				if (proposal.status !== "pending_maker") {
					return res
						.status(400)
						.json({ error: "Proposal is not pending maker approval" });
				}
        ...
        `);
				...
				await logAgentAction({
					...
					newState: {
						status: action === "approve" ? "pending_checker" : "rejected",
					},
				});
	...
	app.post(
		"/api/agent/treasury/proposals/:proposalId/single-approval",
		requireAgentPortal,
		async (req: Request, res: Response) => {
			try {
				const agentId = (req.user as any).id;
      ...
      `)
				...
				if (!makerCheckerDisabled) {
					await logAgentAction({
						...
						previousState: {
							makerCheckerEnabled: proposal.maker_checker_enabled,
						},
						newState: { blocked: true, reason: "maker_checker_required" },
					});
					return res.status(400).json({
						error:
							"This mandate requires maker-checker approval. Use maker/checker endpoints.",
				...
				if (proposal.status !== "pending_approval") {
					return res
						.status(400)
						.json({ error: "Proposal is not pending approval" });
				}
	...
	app.post(
		"/api/agent/treasury/proposals/:proposalId/checker-action",
		requireAgentPortal,
		async (req: Request, res: Response) => {
			try {
				const agentId = (req.user as any).id;
      ...
      `)
				...
				if (proposal.status !== "pending_checker") {
					return res
						.status(400)
						.json({ error: "Proposal is not pending checker approval" });
				}
				...
				if (proposal.maker_user_id === agentId) {
					return res
						...
						.json({
							error:
								"Checker cannot be the same as maker for maker-checker workflow",
						});
				}
