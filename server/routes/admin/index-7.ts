...
export function registerAdminPanelPart7Routes(app: Express): void {
	...
	app.get(
		"/api/admin/enrichment/mf/sebi-rules",
		requireAdmin,
		async (req, res) => {
			try {
	...
	app.get(
		"/api/admin/enrichment/mf/category-stats",
		requireAdmin,
		async (req, res) => {
			try {
				const { sebiCategoryEngine } = await import(
	...
	app.get(
		"/api/admin/enrichment/mf/audit-logs",
		requireAdmin,
		async (req, res) => {
			try {
	...
	app.get(
		"/api/admin/enrichment/mf/validate-categories",
		requireAdmin,
		async (req, res) => {
			try {
				const { sebiCategoryEngine } = await import(
				...
				const validation = await sebiCategoryEngine.validateAllSchemes(
					Number.parseInt(limit as string) || 500,
				);
	...
	app.get(
		"/api/admin/enrichment/stocks/stats",
		requireAdmin,
		async (req, res) => {
			try {
				...
				const stats =
					await stockFinancialEnrichmentService.getEnrichmentStats();
	...
	app.get(
		"/api/admin/enrichment/stocks/progress",
		requireAdmin,
		async (req, res) => {
			try {
				const { stockFinancialEnrichmentService } = await import(
	...
	app.post(
		"/api/admin/enrichment/stocks/run",
		requireAdmin,
		async (req, res) => {
			try {
				const { stockFinancialEnrichmentService } = await import(
				...
				stockFinancialEnrichmentService
					.enrichAllStocks({
						useYahoo,
						batchSize,
						maxYahooRequests,
					...
					.catch((err) => {
						console.error(
							"[Stock Enrichment Admin] Enrichment failed:",
							err.message,
						);
					});
				...
				res.json({
					success: true,
					message: "FMP enrichment started",
					status: "running",
					config: { useFmp, maxFmpStocks, includeReturns },
				});
			} catch (error: any) {
				res.status(500).json({ success: false, error: error.message });
			}
	...
	app.post(
		"/api/admin/enrichment/screener/run",
		requireAdmin,
		async (req: any, res) => {
			try {
				const limit = Math.min(Number(req.body?.limit) || 100, 500);
				const { runScreenerEnrichmentBatch } = await import(
					"../../cron-enrichment"
				);
				runScreenerEnrichmentBatch(limit, "AdminScreenerEnrich").catch(
					(e: any) =>
						console.warn("[AdminScreenerEnrich] Batch failed:", e?.message),
				);
				res.json({
					success: true,
					message: `Screener.in enrichment started for up to ${limit} pending stocks`,
					status: "running",
				});
			} catch (error: any) {
				res.status(500).json({ success: false, error: error.message });
	...
	app.get(
		"/api/admin/enrichment/screener/status",
		requireAdmin,
		async (_req, res) => {
			try {
				...
				const [statusResult, nullRevResult] = await Promise.all([
					pool.query(
						`SELECT enrichment_status, COUNT(*) as cnt FROM listed_stocks WHERE is_active = true GROUP BY enrichment_status ORDER BY cnt DESC`,
					),
					pool.query(
						`SELECT COUNT(*) as total, COUNT(revenue_growth) as has_revenue_growth, COUNT(debt_to_equity) as has_debt_equity, COUNT(earnings_growth) as has_earnings_growth, COUNT(roe) as has_roe FROM screener_financials`,
					),
				]);
				res.json({
					success: true,
					enrichmentStatus: statusResult.rows,
					screenerFinancials: nullRevResult.rows[0],
				});
			} catch (error: any) {
				res.status(500).json({ success: false, error: error.message });
			}
	...
	app.post(
		"/api/admin/enrichment/nav-metrics/run",
		requireAdmin,
		async (req, res) => {
			try {
				const { mfReturnsSyncService } = await import(
					"../../services/mf-returns-sync-service"
				);
				...
				mfReturnsSyncService.runBatchSync(maxFunds).catch((err) => {
					console.error(
						"[NAV Metrics Enrichment] Batch sync failed:",
						err.message,
					);
				});
				...
				res.json({
					success: true,
					message: "NAV metrics sync started",
					status: "running",
				});
			} catch (error: any) {
				res.status(500).json({ success: false, error: error.message });
	...
	app.get(
		"/api/admin/enrichment/nav-metrics/status",
		requireAdmin,
		async (req, res) => {
			try {
				const { mfReturnsSyncService } = await import(
		...
		try {
			...
			if (!hasProd()) {
				return res.status(400).json({
					success: false,
					error:
						"PRODUCTION_DATABASE_URL not configured. Enrichment writes require production database.",
			...
			Promise.all([
				mfExtendedEnrichmentService
					.enrichAllFunds({ forceRefresh: false })
					.then(() => console.log("[Enrichment All] MF TER/AUM done")),
				stockFinancialEnrichmentService
					.enrichAllStocks({
						useFmp: true,
						maxFmpStocks: 40,
					...
					.then(() =>
						console.log("[Enrichment All] Stock FMP enrichment done"),
					),
				mfReturnsSyncService
					.runBatchSync(200)
					.then(() => console.log("[Enrichment All] MF Returns done")),
			]).catch((err) => {
			...
			try {
				const { benchmarkSyncService } = await import(
					"../../services/benchmark-sync-service"
				);
				benchmarkSyncService
					.syncAllBenchmarks()
					.then((result) => {
						console.log(
							`[Enrichment All] Benchmarks done: ${result.synced} synced`,
						);
					})
					.catch((err) =>
						console.error(
							"[Enrichment All] Benchmark sync failed:",
							err.message,
						),
					);
			...
			try {
				const { amfiBenchmarkIngestionService } = await import(
					"../../services/amfi-benchmark-ingestion-service"
				);
				amfiBenchmarkIngestionService
					.syncAmfiSchemeBenchmarks()
					.then((result) => {
						console.log(
							`[Enrichment All] AMFI benchmarks done: ${result.normalized} normalized`,
						);
					})
					.catch((err) =>
						console.error(
							"[Enrichment All] AMFI benchmark failed:",
							err.message,
						),
					);
			...
			try {
				const { bseBenchmarkService } = await import(
					"../../services/bse-benchmark-service"
				);
				bseBenchmarkService
					.seedBseIndices()
					.then((result) => {
						console.log(
							`[Enrichment All] BSE indices seeded: ${result.seeded} new`,
						);
					})
					.catch((err) =>
						console.error("[Enrichment All] BSE seed failed:", err.message),
					);
			...
			res.json({
				success: true,
				message: "All enrichment jobs started",
				jobs: startedJobs,
			});
		} catch (error: any) {
			res.status(500).json({ success: false, error: error.message });
	...
	app.get(
		"/api/admin/enrichment/full-status",
		requireAdmin,
		async (req, res) => {
			try {
				...
				statuses.productionDb = {
					...
					message: hasProductionDb()
						? "Production database connected - enrichment will write to production"
						: "PRODUCTION_DATABASE_URL not set - enrichment writes will be blocked",
				};
				...
				try {
					...
					statuses.dataEnrichmentScheduler =
						dataEnrichmentScheduler.getStatus();
				} catch (e) {
					statuses.dataEnrichmentScheduler = { error: "not loaded" };
				}
				...
				try {
					...
					statuses.mfExtended = { error: "not loaded" };
				}
				...
				try {
					...
					statuses.stockFinancials =
						stockFinancialEnrichmentService.getProgress();
				} catch (e) {
					statuses.stockFinancials = { error: "not loaded" };
				}

				try {
					const { financialMetricsRefreshScheduler } = await import(
						"../../services/financial-metrics-refresh-scheduler"
					);
					statuses.metricsRefresh =
						financialMetricsRefreshScheduler.getStatus();
				} catch (e) {
					statuses.metricsRefresh = { error: "not loaded" };
				}

				try {
					const { historicalNavRefreshJob } = await import(
						"../../services/historical-nav-refresh-job"
					);
					statuses.historicalNav = await historicalNavRefreshJob.getStatus();
				} catch (e) {
					statuses.historicalNav = { error: "not loaded" };
				}
      ...
      `);
				...
				statuses.databaseGaps.dataSource = hasProductionDb()
					? "production"
					: "development";
	...
	app.get(
		"/api/admin/enrichment/extraction/stats",
		requireAdmin,
		async (req, res) => {
			try {
				const { mfExtendedDataExtractor } = await import(
					"../../services/mf-extended-data-extractor"
				);
	...
	app.get(
		"/api/admin/enrichment/extraction/progress",
		requireAdmin,
		async (req, res) => {
			try {
				const { mfExtendedDataExtractor } = await import(
	...
	app.post(
		"/api/admin/enrichment/extraction/run",
		requireAdmin,
		async (req, res) => {
			try {
				const { mfExtendedDataExtractor } = await import(
				...
				mfExtendedDataExtractor
					.extractAllFunds({ forceRefresh })
					.catch((err) => {
				...
				res.json({
					success: true,
					message: "Extended data extraction started",
				});
			} catch (error: any) {
				res.status(500).json({ success: false, error: error.message });
			}
	...
	app.get(
		"/api/admin/enrichment/scheduler/status",
		requireAdmin,
		async (req, res) => {
			try {
				const { dataEnrichmentScheduler } = await import(
	...
	app.post(
		"/api/admin/enrichment/scheduler/initialize",
		requireAdmin,
		async (req, res) => {
			try {
				const { dataEnrichmentScheduler } = await import(
	...
	import("../../utils/enrichment-guard").then(({ isProductionEnvironment }) => {
		if (isProductionEnvironment()) {
			import("../../services/data-enrichment-scheduler")
				.then(({ dataEnrichmentScheduler }) => {
					dataEnrichmentScheduler.initialize();
					console.log("✅ Data Enrichment Scheduler initialized");
				})
				.catch((err) => {
					console.warn(
						"⚠️ Failed to initialize Data Enrichment Scheduler:",
						err.message,
					);
				});
		} else {
			console.log(
				"⏭️ [DataEnrichment] Scheduler skipped (development mode - production only)",
			);
		}
	});
	...
	app.get(
		"/api/admin/data-providers/health",
		requireAdmin,
		async (req, res) => {
			try {
				const { getProviderRegistry } = await import(
					"../../services/screener/data-provider-registry"
				);
	...
	app.post(
		"/api/admin/data-providers/reset-metrics",
		requireAdmin,
		async (req, res) => {
			try {
				const { getProviderRegistry } = await import(
				...
				res.json({
					...
					message: providerName
						? `Metrics reset for ${providerName}`
						: "All metrics reset",
				});
			} catch (error: any) {
				res.status(500).json({ success: false, error: error.message });
	...
	app.get(
		"/api/admin/data-providers/test/:provider",
		requireAdmin,
		async (req, res) => {
			try {
				const { getProviderRegistry } = await import(
				...
				const { result: profile, provider } =
					await registry.getCompanyProfile(testSymbol);
	...
	app.get("/api/admin/data-providers/usage", requireAdmin, async (req, res) => {
		try {
			const { fmpUsageMonitor } = await import(
				"../../services/screener/fmp-usage-monitor"
			);
			const { getAlphaVantageProvider } = await import(
				"../../services/screener/alpha-vantage-provider"
			);
			...
			res.json({
				...
				usage: {
					fmp: {
						dailyCalls: (fmpStats as any).used,
						maxDaily: fmpStats.limit,
						remaining: fmpStats.remaining,
					},
	...
	app.get("/api/admin/signal-audit", requireAdmin, async (req, res) => {
		try {
			...
			res
				.status(500)
				.json({ success: false, error: "Failed to fetch signal audit log" });
		}
	});
	...
	app.get("/api/admin/governance-policy", requireAdmin, async (_req, res) => {
		try {
			...
			res
				.status(500)
				.json({ success: false, error: "Failed to fetch governance policy" });
		}
	});
	...
	app.patch("/api/admin/governance-policy", requireAdmin, async (req, res) => {
		try {
			...
			if (!ruleId) {
				return res
					.status(400)
					.json({ success: false, error: "ruleId is required" });
			}
			...
			res
				.status(500)
				.json({ success: false, error: "Failed to update governance policy" });
		}
	});
