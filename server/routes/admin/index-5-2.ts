...
export function registerAdminPanelPart5Sub2Routes(app: Express): void {
	...
	app.get(
		"/api/admin/audit/integrity-status",
		requireAdmin,
		async (req, res) => {
			try {
				const status = auditIntegrityChecker.getStatus();
				const failedVerifications =
					await auditIntegrityChecker.getFailedVerifications(20);
				...
				console.error(
					"[Audit Integrity] Error fetching status:",
					error.message,
				);
				res.status(500).json({
	...
	app.post(
		"/api/admin/audit/integrity-check",
		requireAdmin,
		async (req, res) => {
			try {
	...
	app.post(
		"/api/admin/audit/integrity-schedule",
		requireAdmin,
		async (req, res) => {
			try {
				...
				console.error(
					"[Audit Integrity] Schedule update failed:",
					error.message,
				);
				res.status(500).json({
	...
	app.post(
		"/api/admin/audit/integrity-failure/:failureId/review",
		requireAdmin,
		async (req: any, res) => {
			try {
				...
				const success = await auditIntegrityChecker.markVerificationReviewed(
					failureId,
					reviewedBy,
				);
				...
				console.error(
					"[Audit Integrity] Review marking failed:",
					error.message,
				);
				res.status(500).json({
	...
	app.get("/api/admin/mf-enrichment-status", requireAdmin, async (req, res) => {
      ...
      `);
			...
			const totalFunds = Number.parseInt(row.total) || 0;
			const enrichedFunds = Number.parseInt(row.with_returns) || 0;
			const pendingFunds = totalFunds - enrichedFunds;
			const progressPercentage =
				totalFunds > 0
					? Math.round((enrichedFunds / totalFunds) * 100 * 100) / 100
					: 0;
      ...
      `);
			...
			res.json({
				...
				stats: {
					...
					pendingFunds,
					progressPercentage,
					withNav: Number.parseInt(row.with_nav) || 0,
					withAum: Number.parseInt(row.with_aum) || 0,
					withTer: Number.parseInt(row.with_ter) || 0,
					withRisk: Number.parseInt(row.with_risk) || 0,
					withSharpe: Number.parseInt(row.with_sharpe) || 0,
				},
				syncStatus: {
				...
				recentlyEnriched: lastSyncedResult.rows.map((row: any) => ({
					...
					isin: row.isin || null,
					ter: row.expense_ratio ? Number.parseFloat(row.expense_ratio) : null,
					aum: row.aum ? Number.parseFloat(row.aum) : null,
					riskLevel: row.risk_level || null,
					returns1y: row.returns_1y ? Number.parseFloat(row.returns_1y) : null,
					returns3y: row.returns_3y ? Number.parseFloat(row.returns_3y) : null,
					returns5y: row.returns_5y ? Number.parseFloat(row.returns_5y) : null,
					sharpeRatio: row.sharpe_ratio
						? Number.parseFloat(row.sharpe_ratio)
						: null,
					sortinoRatio: row.sortino_ratio
						? Number.parseFloat(row.sortino_ratio)
						: null,
					standardDeviation: row.standard_deviation
						? Number.parseFloat(row.standard_deviation)
						: null,
					maxDrawdown: row.max_drawdown
						? Number.parseFloat(row.max_drawdown)
						: null,
					alpha: row.alpha ? Number.parseFloat(row.alpha) : null,
					beta: row.beta ? Number.parseFloat(row.beta) : null,
	...
	app.post(
		...
		async (req: any, res) => {
			try {
				...
				const batchLimit = Number.parseInt(req.body?.batchLimit) || 20;
				const clampedLimit = Math.min(Math.max(batchLimit, 1), 100); // Clamp between 1-100

				console.log(
					`[MF Enrichment] Admin-initiated sync started for ${clampedLimit} funds by ${req.user?.email || "admin"}`,
				);
				...
				syncPromise
					.then((result) => {
						console.log(
							`[MF Enrichment] Admin sync completed: ${result.successful}/${result.processed} successful, ${result.failed} failed`,
						);
					})
					.catch((err) => {
				...
				console.error(
					"[MF Enrichment] Manual sync trigger failed:",
					error.message,
				);
				res.status(500).json({
					success: false,
	...
	app.get("/api/admin/benchmarks", requireAdmin, async (req, res) => {
		try {
			...
			console.error(
				"[Benchmark Admin] Error fetching benchmarks:",
				error.message,
			);
	...
	app.post(
		...
		async (req: any, res) => {
			try {
				...
				console.log(
					`[Benchmark Admin] Manual sync triggered by ${req.user?.email || "admin"}`,
				);
				...
				syncPromise
					.then((result) => {
						console.log(
							`[Benchmark Admin] Sync completed: ${result.synced} synced, ${result.failed.length} failed`,
						);
					})
					.catch((err) => {
	...
	app.get(
		"/api/admin/mf-benchmark-mappings",
		requireAdmin,
		async (req, res) => {
			try {
				res.set("Cache-Control", "no-store, no-cache, must-revalidate");
      ...
      `);
				...
				for (const r of indexDistResult.rows as any[]) {
					byIndexCode[r.index_code] = Number.parseInt(r.count);
				}
				...
				res.json({
					...
					mappings,
					stats: {
						totalMappings: Number.parseInt(statsRow.total_mappings) || 0,
						autoMappings: Number.parseInt(statsRow.auto_mappings) || 0,
						manualOverrides: Number.parseInt(statsRow.manual_overrides) || 0,
						highConfidence: Number.parseInt(statsRow.high_confidence) || 0,
						byIndexCode,
					},
				...
				console.error(
					"[Benchmark Admin] Error fetching mappings:",
					error.message,
				);
				res.status(500).json({ success: false, error: error.message });
			}
	...
	app.post(
		"/api/admin/mf-benchmark-mappings/auto-map",
		requireAdmin,
		async (req: any, res) => {
			try {
				const { mfBenchmarkMappingService } = await import(
					"../../services/mf-benchmark-mapping-service"
				);

				const limit = Number.parseInt(req.body?.limit) || 500;
				console.log(
					`[Benchmark Admin] Auto-mapping triggered by ${req.user?.email || "admin"}`,
				);

				const result =
					await mfBenchmarkMappingService.autoMapUnmappedFunds(limit);
	...
	app.post(
		"/api/admin/mf-benchmark-mappings/override",
		requireAdmin,
		async (req: any, res) => {
			try {
				const { mfBenchmarkMappingService } = await import(
				...
				if (!isin || !indexCode) {
					return res
						.status(400)
						.json({ success: false, error: "Missing isin or indexCode" });
				}

				await mfBenchmarkMappingService.overrideBenchmark(
					isin,
					indexCode,
	...
	app.post(
		"/api/admin/mf-relative-metrics/recompute",
		requireAdmin,
		async (req: any, res) => {
			try {
				...
				const batchSize = Number.parseInt(req.body?.batchSize) || 50;
				console.log(
					`[Metrics Admin] Recompute triggered by ${req.user?.email || "admin"}`,
				);

				// Start async recompute
				const recomputePromise =
					mfRelativeMetricsEngine.recomputeAllMetrics(batchSize);
				...
				recomputePromise
					.then((result) => {
						console.log(
							`[Metrics Admin] Recompute completed: ${result.success}/${result.processed} success`,
						);
					})
					.catch((err) => {
