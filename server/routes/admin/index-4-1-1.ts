		...
		try {
			...
			try {
				...
				status.recommendations.push({
					severity: "critical",
					message:
						"Database connection failed - Check DATABASE_URL environment variable and database server status",
			...
			try {
				...
				status.recommendations.push({
					severity: "medium",
					message:
						"Yahoo Finance API is experiencing issues - Market data may be delayed",
					action:
						"Monitor API status and consider alternative data sources if issues persist",
				} as any);
			}
			...
			if (process.env.IIFL_APP_KEY) {
				try {
					...
					status.recommendations.push({
						...
						action:
							"Update IIFL_APP_KEY and IIFL_APP_SECRET environment variables with valid credentials",
					} as any);
				}
			} else {
				status.apis.iiflMarkets = {
				...
				status.recommendations.push({
					severity: "low",
					message:
						"IIFL Markets API not configured - Trading features are disabled",
					action:
						"Add IIFL_APP_KEY and IIFL_APP_SECRET environment variables to enable trading capabilities",
				} as any);
			}
			...
			try {
				status.apis.interactiveBrokers = {
					...
					lastChecked: new Date().toISOString(),
					details:
						"IB integration ready - Real-time trading capabilities available",
				};
			} catch (error) {
				status.apis.interactiveBrokers = {
					name: "Interactive Brokers API",
			...
			if (memoryUsagePercent > 85) {
				status.recommendations.push({
					...
					action:
						"Consider restarting the application or optimizing memory usage",
				} as any);
			}
			...
			const apiStatuses = Object.values(status.apis).map(
				(api: any) => api.status,
			);
			...
			(status.systemHealth as any).totalResponseTime =
...
System Security Data:`;
				analysisData = JSON.stringify(
					{ errors: systemErrors, apis: apiStatus },
					null,
					2,
				);
				break;
		...
		try {
			...
			return {
				...
				analysis: {
					summary:
						"AI analysis temporarily unavailable. Please check system configuration.",
					recommendations: [
	...
	async function analyzeWithGemini(prompt: string) {
		try {
			...
			throw new Error(
				`AI analysis failed: ${(error as any)?.message || error}`,
			);
		}
	}
	...
	async function getApiStatus() {
		const endpoints = [
			...
			},
			{
				...
				url: "/api/portfolios/holdings",
				category: "Portfolio Management",
				internal: true,
			},
			{
				name: "Portfolio Allocation",
				url: "/api/portfolios/allocation",
				category: "Portfolio Management",
				internal: true,
			},
			...
			},
			{
				...
				url: "/api/admin/api-status",
				category: "Admin APIs",
				internal: true,
			},
			{
				name: "Admin AI Analysis",
				url: "/api/admin/ai-analysis",
				category: "Admin APIs",
				internal: true,
			},
			{
				name: "Admin System Errors",
				url: "/api/admin/system-errors",
				category: "Admin APIs",
				internal: true,
			},
			{
		...
		return {
			...
			endpoints: results,
			categories: {
				"External APIs": results.filter(
					(r: any) => r.category === "External APIs",
				),
				"Market Data": results.filter((r: any) => r.category === "Market Data"),
				Authentication: results.filter(
					(r: any) => r.category === "Authentication",
				),
				"Portfolio Management": results.filter(
					(r: any) => r.category === "Portfolio Management",
				),
				"Admin APIs": results.filter((r: any) => r.category === "Admin APIs"),
				Database: results.filter((r: any) => r.category === "Database"),
				"Third Party Services": results.filter(
					(r: any) => r.category === "Third Party Services",
				),
			},
		};
	}
	...
	app.post("/api/ckyc", async (req, res) => {
		try {
			...
			if (useDigiLocker && req.user?.id) {
				try {
					const digilockerData = await digilockerService.autoPopulateKYCFields(
						(req.user as any)!.id,
					);
					...
					console.log(
						"✅ Auto-populated KYC data from DigiLocker for user:",
						(req.user as any)!.id,
					);
				} catch (digilockerError) {
					console.warn(
						"⚠️ DigiLocker auto-population failed, trying BSE Star fallback:",
						digilockerError,
					);
					...
					if (panNumber || bodyData.panNumber) {
						try {
							const { bseStarKYCService } = await import(
								"../../services/bse-star-kyc-service"
							);
							const bseData = await bseStarKYCService.autoPopulateKYC(
								panNumber || bodyData.panNumber,
							);
							...
							console.log(
								"✅ Auto-populated KYC data from BSE Star for user:",
								(req.user as any)!.id,
							);
						} catch (bseError) {
							console.error(
								"❌ BSE Star auto-population also failed:",
								bseError,
							);
			...
			const existingRecord = await storage.getCkycRecord(
				validatedData.userId ?? "",
			);

			let ckycRecord;
			if (existingRecord) {
				ckycRecord = await storage.updateCkycRecord(
					validatedData.userId ?? "",
					validatedData,
				);
			} else {
