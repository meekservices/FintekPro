...
export function registerAdminPanelPart6Routes(app: Express): void {
	...
	app.post(
		...
		async (req: any, res) => {
			try {
				...
				console.log(
					`[AMFI Admin] Sync triggered by ${req.user?.email || "admin"}`,
				);
				const result =
				...
				res
					...
					.json({
						success: false,
						error: error.message || "Unknown error during AMFI benchmark sync",
					});
			}
		},
	);
	...
	app.post(
		"/api/admin/amfi-benchmark/auto-map",
		requireAdmin,
		async (req: any, res) => {
			try {
				const { amfiBenchmarkIngestionService } = await import(
				...
				console.log(
					`[AMFI Admin] Auto-map from AMFI triggered by ${req.user?.email || "admin"}`,
				);
				...
				if (result.recompute.length > 0) {
					const { mfRelativeMetricsEngine } = await import(
						"../../services/mf-relative-metrics-engine"
					);
					mfRelativeMetricsEngine.recomputeAllMetrics(50).catch((err) => {
						console.error(
							`[AMFI Admin] Metrics recompute failed:`,
							err.message,
						);
					});
				}
	...
	app.get(
		"/api/admin/amfi-benchmark/conflicts",
		requireAdmin,
		async (req, res) => {
			try {
				const { amfiBenchmarkIngestionService } = await import(
	...
	app.post(
		"/api/admin/amfi-benchmark/resolve-conflict",
		requireAdmin,
		async (req: any, res) => {
			try {
				const { amfiBenchmarkIngestionService } = await import(
				...
				if (!isin || !resolution) {
					return res
						.status(400)
						.json({ success: false, error: "Missing isin or resolution" });
				}

				const adminId = req.user?.email || req.user?.id || "admin";
				const result = await amfiBenchmarkIngestionService.resolveConflict(
					isin,
					resolution,
					manualIndexCode,
					adminId,
				...
				res.json({
					success: result,
					message: result ? "Conflict resolved" : "Resolution failed",
				});
			} catch (error: any) {
				res.status(500).json({ success: false, error: error.message });
			}
	...
	app.get(
		"/api/admin/amfi-benchmark/history",
		requireAdmin,
		async (req, res) => {
			try {
				const { amfiBenchmarkIngestionService } = await import(
				...
				const history = await amfiBenchmarkIngestionService.getBenchmarkHistory(
					isin,
					limit,
				);
	...
	app.get("/api/admin/amfi-nav/status", requireAdmin, async (req, res) => {
		try {
			const { amfiNavScheduler } = await import(
				"../../services/amfi-nav-scheduler"
			);
			const { amfiOfficialNavService } = await import(
				"../../services/amfi-official-nav-service"
			);
	...
	app.post("/api/admin/amfi-nav/sync", requireAdmin, async (req: any, res) => {
		try {
			...
			console.log(
				`[AMFI NAV Admin] Manual sync triggered by ${req.user?.email || "admin"}`,
			);
	...
	app.post(
		"/api/admin/bse-benchmark/seed-indices",
		requireAdmin,
		async (req: any, res) => {
			try {
				...
				console.log(
					`[BSE Admin] Seed indices triggered by ${req.user?.email || "admin"}`,
				);
	...
	app.get(
		"/api/admin/bse-benchmark/resolve/:isin",
		requireAdmin,
		async (req, res) => {
			try {
				const { bseBenchmarkService } = await import(
	...
	app.post(
		"/api/admin/bse-benchmark/auto-map",
		requireAdmin,
		async (req: any, res) => {
			try {
				const { bseBenchmarkService } = await import(
				...
				console.log(
					`[BSE Admin] Auto-map with precedence triggered by ${req.user?.email || "admin"}`,
				);
	...
	app.get(
		"/api/admin/bse-benchmark/conflicts",
		requireAdmin,
		async (req, res) => {
			try {
				const { bseBenchmarkService } = await import(
	...
	app.post(
		"/api/admin/bse-benchmark/resolve-conflict",
		requireAdmin,
		async (req: any, res) => {
			try {
				const { bseBenchmarkService } = await import(
				...
				const adminId = req.user?.email || req.user?.id || "admin";
				const result = await bseBenchmarkService.resolveConflict(
					isin,
					resolution,
					manualIndexCode,
					adminId,
	...
	app.get(
		"/api/admin/bse-benchmark/lineage",
		requireAdmin,
		async (req, res) => {
			try {
				const { bseBenchmarkService } = await import(
				...
				const isin = req.query.isin as string | undefined;
				const limit = Number.parseInt(req.query.limit as string) || 50;
				const lineage = await bseBenchmarkService.getBenchmarkLineage(
					isin,
					limit,
				);
	...
	app.get(
		"/api/admin/enrichment/mf/progress",
		requireAdmin,
		async (req, res) => {
			try {
				const { mfExtendedEnrichmentService } = await import(
	...
	app.post("/api/admin/enrichment/mf/run", requireAdmin, async (req, res) => {
		try {
			...
			mfExtendedEnrichmentService
				.enrichAllFunds({ forceRefresh, batchSize })
				.catch((err) => {
					console.error(
						"[MF Enrichment Admin] Enrichment failed:",
						err.message,
					);
				});
	...
	app.get(
		"/api/admin/enrichment/mf/comprehensive/stats",
		requireAdmin,
		async (req, res) => {
			try {
	...
	app.get(
		"/api/admin/enrichment/mf/comprehensive/progress",
		requireAdmin,
		async (req, res) => {
			try {
				const { mfComprehensiveEnrichmentService } = await import(
	...
	app.post(
		"/api/admin/enrichment/mf/comprehensive/run",
		requireAdmin,
		async (req, res) => {
			try {
				const { mfComprehensiveEnrichmentService } = await import(
				...
				mfComprehensiveEnrichmentService
					.runComprehensiveEnrichment({ maxMfapiFunds, skipMfapi, batchSize })
					.catch((err) => {
				...
				res.json({
					success: true,
					message: "Comprehensive enrichment started",
					status: "running",
				});
			} catch (error: any) {
				res.status(500).json({ success: false, error: error.message });
	...
	app.get(
		"/api/admin/enrichment/mf/ratings/stats",
		requireAdmin,
		async (req, res) => {
			try {
				...
				res.json({
					...
					methodology: {
						description:
							"Proprietary multi-factor scoring engine using risk-adjusted returns, quality metrics, liquidity, momentum, and valuation analysis",
						scoringWeights: {
							riskAdjusted: 0.35,
							quality: 0.25,
							liquidity: 0.15,
						...
						disclaimer:
							"FintekPro Smart Ratings are proprietary calculations. Not affiliated with CRISIL, ICRA, or any third-party rating agency.",
					},
					coverage: {
						total,
						rated,
						...
						coveragePercent:
							total > 0 ? ((rated / total) * 100).toFixed(1) : "0",
					},
					distribution: {
					...
					averages: {
						overallScore: stats?.avgOverall
							? Number.parseFloat(Number(stats.avgOverall).toFixed(2))
							: null,
						percentile: stats?.avgPercentile
							? Number.parseFloat(Number(stats.avgPercentile).toFixed(2))
							: null,
					},
					dataSource: {
	...
	app.post(
		"/api/admin/enrichment/mf/ratings/run",
		requireAdmin,
		async (req, res) => {
			try {
				const { default: fintekProRatingService } = await import(
					"../../services/fintekpro-rating-service"
				);
				const { onlyNullRatings = true, maxFunds = 50000 } = req.body;

				fintekProRatingService
					.batchComputeAndPersist({
						onlyNullRatings,
						maxFunds,
						batchSize: 200,
					})
					.then((result) => {
						console.log(
							`[Admin] FintekPro Smart Rating batch complete: ${result.persisted} persisted, ${result.failed} failed`,
						);
					})
					.catch((err) => {
						console.error(
							"[Admin] FintekPro Smart Rating batch failed:",
							err.message,
						);
					});
	...
	app.get(
		"/api/admin/enrichment/mf/ratings/methodology",
		requireAdmin,
		async (req, res) => {
			res.json({
				success: true,
				...
				description:
					"Proprietary multi-factor rating engine for mutual fund evaluation",
				disclaimer:
					"FintekPro Smart Ratings are proprietary calculations based on publicly available fund data. They are NOT CRISIL ratings or any third-party agency ratings. Data source attribution: AMFI (official NAV feed), MFAPI (historical returns), FintekPro (proprietary scoring).",
				components: [
					{
						name: "Risk-Adjusted Returns Score",
						weight: 0.35,
						inputs: ["returns_1y", "returns_3y", "returns_5y", "expense_ratio"],
						method:
							"Net return above risk-free rate (6.5%), weighted average across time horizons",
					},
					{
						name: "Quality Score",
						weight: 0.25,
						inputs: ["fund_house", "aum", "existing_rating"],
						method:
							"Fund house reputation scoring (15 major AMCs), AUM scale bonus, rating history",
					},
					{
						...
						inputs: ["aum", "category"],
						method:
							"AUM-based liquidity tiers with category adjustments for large-cap preference",
					},
					{
						name: "Momentum Score",
						weight: 0.15,
						inputs: ["returns_1y", "returns_3y"],
						method: "60/40 weighted short-to-medium term performance momentum",
					},
					{
						name: "Valuation Score",
						weight: 0.1,
						inputs: ["expense_ratio", "category"],
						method:
							"Expense ratio efficiency scoring relative to category norms",
					},
				],
				starMapping: [
				...
				regulatoryCompliance: {
					...
					dataProvenance:
						"All data sourced from AMFI, MFAPI, and proprietary FintekPro calculations",
					auditTrail:
						"Full audit logging in mf_enrichment_audit_logs table with enrichment run IDs",
					noThirdPartyRatings:
						"No CRISIL, ICRA, or other third-party agency data used in rating computation",
				},
			});
		},
	);
	...
	app.post(
		"/api/admin/enrichment/mf/sebi-rules/seed",
		requireAdmin,
		async (req, res) => {
			try {
