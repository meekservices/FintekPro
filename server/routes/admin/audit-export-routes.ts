...
export function registerAuditExportRoutes(app: Express) {
  app.get("/api/admin/compliance/export", requireAdmin, async (req, res) => {
    try {
      ...
      const filters = [];
      if (startDate) filters.push(gte(immutableAuditLogs.timestamp, new Date(startDate as string)));
      if (endDate) filters.push(lte(immutableAuditLogs.timestamp, new Date(endDate as string)));
      ...
      if (format === 'csv') {
        const headers = ["ID", "Timestamp", "Event Type", "Action", "User ID", "Resource", "Checksum"];
  ...
  app.get("/api/admin/compliance/verify-integrity", requireAdmin, async (req, res) => {
    try {
