...
export function registerSEBIComplianceRoutes(app: Express): void {
	...
	app.get(
		"/api/admin/sebi/compliance-dashboard",
		requireAdmin,
		async (req, res) => {
			try {
      ...
      `);
				const namingFailures = Number.parseInt(
					(namingFailuresResult as any).rows?.[0]?.count || "0",
				);
      ...
      `);
				const glidePathInvalid = Number.parseInt(
					(glidePathInvalidResult as any).rows?.[0]?.count || "0",
				);
      ...
      `);
				const pendingCount = Number.parseInt(
					(pendingResult as any).rows?.[0]?.count || "0",
				);
      ...
      `);
				const lifecycleCount = Number.parseInt(
					(lifecycleCountResult as any).rows?.[0]?.count || "0",
				);
				...
				res.json({
					...
					summary: {
						...
						namingFailures,
						overlapPairsComputed: Number.parseInt(
							overlapStats.total_pairs || "0",
						),
						overlapBreachPairs: Number.parseInt(
							overlapStats.breach_pairs || "0",
						),
						glidePathInvalid,
	...
	app.get(
		"/api/admin/sebi/overlap-breaches",
		requireAdmin,
		async (req, res) => {
			try {
	...
	app.post(
		"/api/admin/sebi/overlap/recompute",
		requireAdmin,
		async (req, res) => {
			try {
				...
				const computeResult =
					await mfSebiOverlapService.computeAllOverlaps(filterByCategory);
	...
	app.get(
		"/api/admin/sebi/compliance/:schemeCode",
		requireAdmin,
		async (req, res) => {
			try {
	...
	app.post(
		"/api/admin/sebi/compliance/:schemeCode/transition",
		requireAdmin,
		async (req, res) => {
			try {
				const { schemeCode } = req.params;
				...
				if (!toStatus || !reason) {
					return res
						...
						.json({
							success: false,
							error: "toStatus and reason are required",
						});
				}
				...
				res.json({
					success: true,
					message: `Transitioned ${schemeCode}: ${result.fromStatus} → ${result.toStatus}`,
				});
			} catch (err: any) {
	...
	app.post(
		"/api/admin/sebi/validate/naming/all",
		requireAdmin,
		async (req, res) => {
			try {
				console.log("[SEBI Admin] Starting bulk naming validation...");
				const summary =
					await mfNamingComplianceService.runNamingValidationForAll();
				res.json({
					success: true,
	...
	app.post(
		"/api/admin/sebi/validate/lifecycle/all",
		requireAdmin,
		async (req, res) => {
			try {
				console.log(
					"[SEBI Admin] Starting bulk lifecycle glide path validation...",
				);
				const summary =
					await mfLifecycleGlidePathService.validateAllLifecycleSchemes();
				res.json({
					success: true,
	...
	app.get(
        ...
        `);
					...
					const total = Number.parseInt(
						(countResult as any).rows?.[0]?.count || "0",
					);
      ...
      `);
				const countResult = await db.execute(
					sql`SELECT COUNT(*) as count FROM mf_categorization_audit_log`,
				);
				const logs = (logsResult as any).rows || [];
				const total = Number.parseInt(
					(countResult as any).rows?.[0]?.count || "0",
	...
	app.post(
		"/api/admin/sebi/holdings/import",
		requireAdmin,
		async (req, res) => {
			try {
				const result =
					await mfSebiOverlapService.importHoldingsFromSchemeStockHoldings();
	...
	app.post(
		"/api/admin/subscription-sync/trigger",
		requireAdmin,
		async (req, res) => {
			try {
	...
	app.get(
		"/api/admin/subscription-sync/status",
		requireAdmin,
		async (req, res) => {
			try {
