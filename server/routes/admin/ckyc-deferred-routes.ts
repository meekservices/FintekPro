...
router.get("/dashboard-stats", async (req, res) => {
	try {
		...
		const blockedAttemptsCount =
			ckycEnvironmentService.getBlockedAttemptsCount();
		...
		console.error(
			"[CKYC Deferred Admin] Failed to get dashboard stats:",
			error,
		);
		res
			.status(500)
			.json({ success: false, error: "Failed to get dashboard stats" });
	}
});
...
router.get("/cases", async (req, res) => {
	try {
		...
		res
			.status(500)
			.json({ success: false, error: "Failed to get deferred cases" });
	}
});
...
router.get("/cases/:caseId", async (req, res) => {
	try {
		...
		res
			.status(500)
			.json({ success: false, error: "Failed to get case details" });
	}
});
...
router.post("/cases/:caseId/action", async (req, res) => {
	try {
		...
		res
			.status(500)
			.json({ success: false, error: "Failed to apply admin action" });
	}
});
...
router.post("/check-sla-breaches", async (req, res) => {
	try {
		...
		const { breachedCount, escalatedCount } =
			await ckycSlaEscalationService.triggerManualCheck();
		...
		res
			.status(500)
			.json({ success: false, error: "Failed to check SLA breaches" });
	}
});
...
router.get("/environment-compliance", async (req, res) => {
	try {
		...
		console.error(
			"[CKYC Deferred Admin] Failed to get compliance status:",
			error,
		);
		res
			.status(500)
			.json({ success: false, error: "Failed to get compliance status" });
	}
});
...
router.get("/cases/:caseId/journey", async (req, res) => {
	try {
		...
		console.error(
			"[CKYC Deferred Admin] Failed to reconstruct journey:",
			error,
		);
		res
			.status(500)
			.json({ success: false, error: "Failed to reconstruct CKYC journey" });
	}
});
...
router.get("/cases/:caseId/export", async (req, res) => {
	try {
		...
		res
			.status(500)
			.json({ success: false, error: "Failed to export audit trail" });
	}
});
...
router.get("/compliance-events", async (req, res) => {
	try {
		...
		console.error(
			"[CKYC Deferred Admin] Failed to get compliance events:",
			error,
		);
		res
			.status(500)
			.json({ success: false, error: "Failed to get compliance events" });
	}
});
