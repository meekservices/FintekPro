...
import {
	requireAdmin,
	requireSuperadmin as requireSuperAdmin,
	...
	app.get("/api/admin/db/table-audit", requireAdmin, async (_req, res) => {
		try {
			...
			const enriched = rows.map((row) => ({
				...
				rowCount: Number.parseInt(row.n_live_tup ?? "0", 10),
				deadTuples: Number.parseInt(row.n_dead_tup ?? "0", 10),
				deadTuplePct: Number.parseFloat(row.dead_tuple_pct ?? "0"),
				seqScans: Number.parseInt(row.seq_scan ?? "0", 10),
				idxScans: Number.parseInt(row.idx_scan ?? "0", 10),
			...
			logger.error("[DbGovernance] Table audit failed", {
				error: error.message,
			});
	...
	app.post(
		...
		async (req: any, res) => {
			...
			if (!tableName || typeof tableName !== "string") {
				return res
					.status(400)
					.json({ success: false, error: "tableName is required" });
			}
			...
			if (!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(tableName)) {
				return res
					.status(400)
					.json({ success: false, error: "Invalid table name" });
			}
         ...
         WHERE table_schema = 'public' AND table_name = $1`,
				...
				if (Number.parseInt(existsResult.rows[0]?.cnt ?? "0", 10) === 0) {
					return res
						.status(404)
						.json({
							success: false,
				...
				try {
					...
					await client.query(
						`ALTER TABLE public.${tableName} SET SCHEMA _archive`,
					);
					await client.query(
						`ALTER TABLE _archive.${tableName} RENAME TO ${archiveName}`,
					);
         ...
         VALUES
           (gen_random_uuid(), $1, 'write',
            ...
            'success', NOW())`,
				...
				logger.info(
					`[DbGovernance] Table archived: public.${tableName} → _archive.${archiveName}`,
					{
						archivedBy: adminId,
						reason: reason.trim(),
					},
				...
				logger.error("[DbGovernance] Archive operation failed", {
					error: error.message,
					tableName,
				});
				res.status(500).json({ success: false, error: error.message });
