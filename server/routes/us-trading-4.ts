...
router.delete(
	...
	async (req, res) => {
		try {
			...
			await alpacaBrokerService.deleteWatchlist(
...
router.get("/broker/corporate-actions/announcements", async (req, res) => {
	try {
		...
		const announcements = await alpacaBrokerService.getCorporateActionsNew({
			...
			limit: req.query.limit ? Number.parseInt(req.query.limit as string) : 100,
		});
		res.json({ success: true, announcements });
	} catch (error: any) {
		res
			.status(500)
			.json({ success: false, announcements: [], error: error.message });
	}
});
...
router.get("/broker/assets/:symbolOrId", async (req, res) => {
	try {
		...
		const asset = await alpacaBrokerService.getAsset(
			req.params.symbolOrId.toUpperCase(),
		);
		if (!asset)
...
router.get(
	"/broker/accounts/:accountId/funding-wallet",
	alpacaAccountGuard,
	async (req, res) => {
		try {
			if (!alpacaBrokerService.isConfigured())
			...
			const details = wallet
				? await alpacaBrokerService.getFundingWalletDetails(
						accountId,
						wallet.id,
					)
				: [];
			const transfers = wallet
				? await alpacaBrokerService.getFundingWalletTransfers(
						accountId,
						wallet.id,
					)
				: [];
...
router.post(
	"/broker/accounts/:accountId/funding-wallet/deposit-simulation",
	requireAdmin,
	async (req, res) => {
		try {
			if (!alpacaBrokerService.isConfigured())
			...
			if (!wallet)
				return res
					.status(404)
					.json({ error: "No funding wallet — create it first" });
			const result = await alpacaBrokerService.simulateFundingDeposit(
				accountId,
				wallet.id,
				Number.parseFloat(amount_usd),
			);
...
router.get(
	"/broker/accounts/:accountId/recipient-banks",
	alpacaAccountGuard,
	async (req, res) => {
		try {
			if (!alpacaBrokerService.isConfigured())
				return res.status(400).json({ error: "Not configured" });
			const banks = await alpacaBrokerService.listRecipientBanks(
				req.params.accountId,
			);
...
router.post(
	...
	async (req, res) => {
		try {
			...
			const bank = await alpacaBrokerService.createRecipientBank(
				req.params.accountId,
				data,
			);
...
router.delete(
	"/broker/accounts/:accountId/recipient-banks/:bankId",
	alpacaAccountGuard,
	async (req, res) => {
		try {
			if (!alpacaBrokerService.isConfigured())
				return res.status(400).json({ error: "Not configured" });
			await alpacaBrokerService.deleteRecipientBank(
				req.params.accountId,
				req.params.bankId,
			);
			res.json({ success: true });
...
router.post(
	"/broker/accounts/:accountId/wire-withdrawal",
	alpacaAccountGuard,
	async (req, res) => {
		try {
			if (!alpacaBrokerService.isConfigured())
			...
			if (!amount || !recipient_bank_id)
				return res
					.status(400)
					.json({ error: "amount and recipient_bank_id required" });
			const result = await alpacaBrokerService.createWireWithdrawal(
				req.params.accountId,
				{
...
router.get("/options/contracts/:symbolOrId", async (req, res) => {
	try {
		if (!alpacaBrokerService.isConfigured())
			return res.status(400).json({ error: "Not configured" });
		const contract = await alpacaBrokerService.getOptionContract(
...
router.get("/account/config", async (req, res) => {
	try {
		...
		if (!accountId)
			return res
				.status(400)
				.json({ error: "No Alpaca account linked to user" });
...
router.patch("/account/config", async (req, res) => {
	try {
		if (!alpacaBrokerService.isConfigured())
			return res.status(400).json({ error: "Not configured" });
		const accountId = (req as any).user?.alpacaAccountId;
		const config = await alpacaBrokerService.updateAccountConfig(
			req.body,
			accountId,
		);
		res.json({ success: true, config });
...
router.get(
	"/broker/accounts/:accountId/config",
	alpacaAccountGuard,
	async (req, res) => {
		try {
			if (!alpacaBrokerService.isConfigured())
				return res.status(400).json({ error: "Not configured" });
			const config = await alpacaBrokerService.getAccountConfig(
				req.params.accountId,
			);
			res.json({ success: true, config });
		} catch (error: any) {
...
router.post(
	"/broker/rebalancing/portfolios",
	requireAdmin,
	async (req, res) => {
		try {
			if (!alpacaBrokerService.isConfigured())
				return res.status(400).json({ error: "Not configured" });
			const portfolio = await alpacaBrokerService.createRebalancingPortfolio(
				req.body,
			);
...
router.get("/broker/rebalancing/portfolios/:portfolioId", async (req, res) => {
	try {
		if (!alpacaBrokerService.isConfigured())
			return res.status(400).json({ error: "Not configured" });
		const portfolio = await alpacaBrokerService.getRebalancingPortfolio(
...
router.patch(
	...
	async (req, res) => {
		try {
			if (!alpacaBrokerService.isConfigured())
				return res.status(400).json({ error: "Not configured" });
			const portfolio = await alpacaBrokerService.updateRebalancingPortfolio(
				req.params.portfolioId,
				req.body,
			);
			res.json({ success: true, portfolio });
...
router.get(
	"/broker/rebalancing/portfolios/:portfolioId/subscriptions",
	async (req, res) => {
		try {
			if (!alpacaBrokerService.isConfigured())
				return res.status(400).json({ error: "Not configured" });
			const subs = await alpacaBrokerService.listPortfolioSubscriptions(
				req.params.portfolioId,
			);
...
router.post(
	...
	async (req, res) => {
		try {
			...
			const sub = await alpacaBrokerService.subscribeAccountToPortfolio(
				req.params.portfolioId,
				account_id,
			);
...
router.delete(
	"/broker/rebalancing/portfolios/:portfolioId/subscriptions/:subscriptionId",
	async (req, res) => {
		try {
			if (!alpacaBrokerService.isConfigured())
				return res.status(400).json({ error: "Not configured" });
			await alpacaBrokerService.unsubscribeAccountFromPortfolio(
				req.params.portfolioId,
...
router.get(
	"/broker/rebalancing/portfolios/:portfolioId/runs",
	async (req, res) => {
		try {
			if (!alpacaBrokerService.isConfigured())
				return res.status(400).json({ error: "Not configured" });
			const runs = await alpacaBrokerService.listRebalancingRuns(
				req.params.portfolioId,
			);
...
router.post(
	...
	async (req, res) => {
		try {
			...
			const run = await alpacaBrokerService.createRebalancingRun(
				req.params.portfolioId,
				type,
			);
...
router.post(
	"/broker/accounts/:accountId/cash-interest/enroll",
	alpacaAccountGuard,
	async (req, res) => {
		try {
			if (!alpacaBrokerService.isConfigured())
			...
			if (!apr_tier_name)
				return res.status(400).json({ error: "apr_tier_name required" });
			const result = await alpacaBrokerService.enrollCashInterest(
				accountId,
				apr_tier_name,
			);
...
router.post(
	"/broker/accounts/:accountId/cash-interest/unenroll",
	async (req, res) => {
		try {
			if (!alpacaBrokerService.isConfigured())
				return res.status(400).json({ error: "Not configured" });
...
router.get(
	"/broker/accounts/:accountId/fpsl/status",
	alpacaAccountGuard,
	async (req, res) => {
		try {
			if (!alpacaBrokerService.isConfigured())
...
router.post(
	"/broker/accounts/:accountId/fpsl/enroll",
	alpacaAccountGuard,
	async (req, res) => {
		try {
			if (!alpacaBrokerService.isConfigured())
...
router.get(
	"/broker/accounts/:accountId/options/exercises",
	alpacaAccountGuard,
	async (req, res) => {
		try {
			if (!alpacaBrokerService.isConfigured()) {
			...
			const exercises = await alpacaBrokerService.listOptionsExercises(
				req.params.accountId,
				{
					symbol: req.query.symbol as string,
		...
		try {
			...
			if (!Array.isArray(contracts) || contracts.length === 0) {
				return res
					...
					.json({
						success: false,
						error: "contracts array is required (e.g. [{ symbol, qty }])",
					});
			}
			if (!["e", "a"].includes(type)) {
				return res
					...
					.json({
						success: false,
						error: "type must be 'e' (exercise) or 'a' (abandon)",
					});
			}
			const result = await alpacaBrokerService.exerciseOptions(
				req.params.accountId,
				contracts,
				type,
			);
...
router.get(
	"/broker/accounts/:accountId/restrictions",
	alpacaAccountGuard,
	async (req, res) => {
		try {
...
router.post(
	"/broker/accounts/:accountId/ach/:achId/verify",
	async (req, res) => {
		try {
			...
			const result = await alpacaBrokerService.verifyAchRelationship(
				req.params.accountId,
				req.params.achId,
				Number.parseFloat(amount1),
				Number.parseFloat(amount2),
			);
...
router.get("/broker/transfers", requireAdmin, async (req, res) => {
	try {
		...
		if (req.query.limit)
			params.limit = Number.parseInt(req.query.limit as string);
		if (req.query.offset)
			params.offset = Number.parseInt(req.query.offset as string);
