...
import { z } from "zod";
import {
	unifiedAadhaarService,
	AadhaarProvider,
} from "../services/unified-aadhaar-service";
...
router.get(
	"/api/admin/aadhaar/active-provider",
	requireAuth,
	requireRole("admin"),
	async (req: Request, res: Response) => {
		try {
...
router.post(
	"/api/admin/aadhaar/set-provider",
	requireAuth,
	requireRole("admin"),
	async (req: Request, res: Response) => {
		try {
...
router.get(
	"/api/admin/aadhaar/cheapest-provider",
	requireAuth,
	requireRole("admin"),
	async (req: Request, res: Response) => {
		try {
			const cheapestProvider =
				unifiedAadhaarService.getCheapestConfiguredProvider();
			...
			if (!cheapestProvider) {
				return res.json({
					...
					config: null,
					recommendation:
						"No providers are currently configured. Please configure at least one Aadhaar verification provider.",
				});
			}
...
router.get(
	"/api/admin/aadhaar/provider/:provider",
	requireAuth,
	requireRole("admin"),
	async (req: Request, res: Response) => {
		try {
			const { provider } = req.params;
			const config = unifiedAadhaarService.getProviderConfig(
				provider as AadhaarProvider,
			);
 ...
 */
router.post(
	"/api/verification/pan-aadhaar-linkage",
	requireAuth,
	async (req: Request, res: Response) => {
		try {
			...
			const linkageStatus =
				await TruthscreenAadhaarService.checkPanAadhaarLinkage(pan, aadhaar);
