...
const orderLimiter = rateLimit({
	...
	legacyHeaders: false,
	keyGenerator: (req) =>
		(req as AuthRequest).user?.id?.toString() || ipKeyGenerator(req.ip ?? ""),
	message: {
		success: false,
		error: "Too many orders — max 30/minute. Please slow down.",
		retryable: true,
	},
});
...
const fundingLimiter = rateLimit({
	...
	message: {
		success: false,
		error: "Too many funding actions — max 10/minute.",
		retryable: true,
	},
});
...
const complianceLimiter = rateLimit({
	...
	message: {
		success: false,
		error: "Too many compliance actions — max 5/minute.",
		retryable: false,
	},
});
...
router.get("/broker/accounts", requireAdmin, async (req, res) => {
	try {
		...
		const accounts = await alpacaBrokerService.listAccounts({
			...
			entities:
				(req.query.entities as string) || "identity,contact,disclosures",
		});
...
router.post("/broker/accounts", requireAdmin, async (req, res) => {
	try {
		if (!alpacaBrokerService.isConfigured()) {
			return res
				.status(400)
				.json({ success: false, error: "Alpaca Broker API not configured" });
		}
...
router.patch("/broker/accounts/:accountId", requireAdmin, async (req, res) => {
	try {
		...
		const account = await alpacaBrokerService.updateBrokerAccount(
			req.params.accountId,
			req.body,
		);
		res.json({ success: true, account });
...
router.get(
	"/broker/accounts/:accountId/trading",
	alpacaAccountGuard,
	async (req, res) => {
		try {
			if (!alpacaBrokerService.isConfigured()) {
			...
			const account = await alpacaBrokerService.getTradingAccount(
				req.params.accountId,
			);
			res.json({ success: true, account });
		} catch (error: any) {
...
router.post(
	...
	async (req, res) => {
		try {
			...
			const cip = await alpacaBrokerService.submitCip(
				req.params.accountId,
				req.body,
			);
...
router.get(
	"/broker/accounts/:accountId/documents",
	alpacaAccountGuard,
	async (req, res) => {
		try {
			if (!alpacaBrokerService.isConfigured()) {
			...
			const docs = await alpacaBrokerService.listDocuments(
				req.params.accountId,
				{
...
router.post(
	...
	async (req, res) => {
		try {
			...
			const doc = await alpacaBrokerService.uploadDocument(
				req.params.accountId,
				req.body,
			);
...
router.get(
	"/broker/accounts/:accountId/documents/:documentId/download",
	alpacaAccountGuard,
	async (req, res) => {
		try {
			if (!alpacaBrokerService.isConfigured()) {
			...
			const url = await alpacaBrokerService.downloadDocument(
				req.params.accountId,
...
router.get(
	"/broker/accounts/:accountId/ach-relationships",
	async (req, res) => {
		try {
			if (!alpacaBrokerService.isConfigured()) {
				return res
...
router.post(
	...
	async (req, res) => {
		try {
			...
			const relationship = await alpacaBrokerService.createAchRelationship(
				req.params.accountId,
				req.body,
			);
...
router.delete(
	"/broker/accounts/:accountId/ach-relationships/:achId",
	async (req, res) => {
		try {
			if (!alpacaBrokerService.isConfigured()) {
				return res
			...
			await alpacaBrokerService.deleteAchRelationship(
				req.params.accountId,
				req.params.achId,
			);
...
router.get("/broker/accounts/:accountId/transfers", async (req, res) => {
	try {
		...
		const transfers = await alpacaBrokerService.listTransfers(
			req.params.accountId,
			{
...
router.post(
	"/broker/accounts/:accountId/transfers",
	fundingLimiter,
	async (req, res) => {
		...
		try {
			...
			const transfer = await alpacaBrokerService.createTransfer(
				req.params.accountId,
				req.body,
			);
			await auditLog({
...
router.delete(
	"/broker/accounts/:accountId/transfers/:transferId",
	async (req, res) => {
		try {
			if (!alpacaBrokerService.isConfigured()) {
				return res
			...
			await alpacaBrokerService.cancelTransfer(
				req.params.accountId,
...
router.get("/broker/accounts/:accountId/activities", async (req, res) => {
	try {
		...
		const activities = await alpacaBrokerService.getAccountActivities(
			req.params.accountId,
			{
...
router.get("/broker/accounts/:accountId/orders", async (req, res) => {
	try {
		...
		const orders = await alpacaBrokerService.getOrders(
			...
			req.query.limit ? Number.parseInt(req.query.limit as string) : 50,
			req.params.accountId,
		);
...
router.post(
	"/broker/accounts/:accountId/orders",
	orderLimiter,
		...
		try {
			...
			const idempotencyKey =
				...
				crypto
					.createHash("sha256")
					.update(
						`${userId}-${accountId}-${req.body.symbol}-${req.body.side}-${req.body.qty || req.body.notional || ""}-${Date.now()}`,
					)
					.digest("hex")
					.slice(0, 36);
			...
			try {
				await orderAuditHook.before(req.body, userId?.toString());
			} catch {
				/* non-fatal */
			}
			...
			try {
				await orderAuditHook.after(order, userId?.toString());
			} catch {
				/* non-fatal */
			}
			...
			await auditLog({
				...
				details: {
					...
					compliance_gates_passed:
						res.locals.complianceResult?.gates?.length ?? 7,
					latency_ms: Date.now() - startMs,
					status: "accepted",
				},
			}).catch(() => {
				/* non-fatal — don't block order response */
			});
...
router.delete(
	"/broker/accounts/:accountId/orders/:orderId",
	orderLimiter,
	async (req, res) => {
		const userId = (req as AuthRequest).user?.id;
		try {
...
router.delete(
	"/broker/accounts/:accountId/positions/:symbol",
	async (req, res) => {
		try {
			if (!alpacaBrokerService.isConfigured()) {
				return res
...
router.get(
	"/broker/accounts/:accountId/portfolio/history",
	async (req, res) => {
		try {
			if (!alpacaBrokerService.isConfigured()) {
				return res
...
router.get("/broker/accounts/:accountId/watchlists", async (req, res) => {
	try {
		...
		const watchlists = await alpacaBrokerService.listWatchlists(
			req.params.accountId,
		);
...
router.post("/broker/accounts/:accountId/watchlists", async (req, res) => {
	try {
		...
		const watchlist = await alpacaBrokerService.createWatchlist(
			req.params.accountId,
			name,
			symbols || [],
		);
...
router.put(
	"/broker/accounts/:accountId/watchlists/:watchlistId",
	async (req, res) => {
		try {
			if (!alpacaBrokerService.isConfigured()) {
				return res
...
router.post(
	"/broker/accounts/:accountId/watchlists/:watchlistId/symbols",
	async (req, res) => {
		try {
			if (!alpacaBrokerService.isConfigured()) {
				return res
...
router.delete(
	"/broker/accounts/:accountId/watchlists/:watchlistId/symbols/:symbol",
	async (req, res) => {
		try {
			if (!alpacaBrokerService.isConfigured()) {
				return res
...
router.get(
	"/broker/accounts/:accountId/corporate-actions",
	async (req, res) => {
		try {
			if (!alpacaBrokerService.isConfigured()) {
				return res
			...
			const announcements =
				await alpacaBrokerService.getAccountCorporateActions(
					req.params.accountId,
					{
...
router.get(
	"/broker/corporate-actions/announcements/:announcementId",
	async (req, res) => {
		try {
			if (!alpacaBrokerService.isConfigured()) {
				return res
			...
			const announcement =
				await alpacaBrokerService.getCorporateActionAnnouncement(
...
router.get(
	"/broker/accounts/:accountId/corporate-actions/:announcementId/elections",
	async (req, res) => {
		try {
			if (!alpacaBrokerService.isConfigured()) {
				return res
		...
		try {
			...
			if (!election_type) {
				return res
					...
					.json({
						success: false,
						error: "election_type is required (cash|stock|mixed|none)",
					});
			}
...
router.get(
	"/broker/accounts/:accountId/positions/tax-lots",
	async (req, res) => {
		try {
			if (!alpacaBrokerService.isConfigured()) {
				return res
			...
			const lots = await alpacaBrokerService.getAllPositionTaxLots(
				req.params.accountId,
			);
...
router.get(
	"/broker/accounts/:accountId/positions/:symbol/tax-lots",
	async (req, res) => {
		try {
			if (!alpacaBrokerService.isConfigured()) {
				return res
 ...
 */
router.post(
	"/broker/accounts/:accountId/ach-relationships/:achId/verify",
	async (req, res) => {
		try {
			if (!alpacaBrokerService.isConfigured()) {
				return res
			...
			if (amount1 == null || amount2 == null) {
				return res
					.status(400)
					.json({ success: false, error: "amount1 and amount2 are required" });
			}
...
router.get("/broker/accounts/:accountId/restrictions", async (req, res) => {
	try {
		...
		const restrictions = await alpacaBrokerService.getAccountRestrictions(
			req.params.accountId,
		);
...
router.post(
	"/broker/accounts/:accountId/suspend",
	complianceLimiter,
	requireAdmin,
	async (req, res) => {
		const userId = (req as AuthRequest).user?.id;
		try {
			...
			if (!reason)
				return res
					...
					.json({
						success: false,
						error: "reason is required for account suspension",
					});
			const result = await alpacaBrokerService.suspendAccount(
				req.params.accountId,
				reason,
			);
			await auditLog({
...
router.post(
	"/broker/accounts/:accountId/reinstate",
	complianceLimiter,
	requireAdmin,
	async (req, res) => {
		const userId = (req as AuthRequest).user?.id;
		try {
			...
			const result = await alpacaBrokerService.reinstateAccount(
				req.params.accountId,
			);
			await auditLog({
...
router.get("/broker/accounts/:accountId/orders/:orderId", async (req, res) => {
	try {
		...
		const order = await alpacaBrokerService.getAccountOrder(
			req.params.accountId,
			req.params.orderId,
		);
		if (!order)
...
router.get("/broker/accounts/:accountId/trusted-contact", async (req, res) => {
	try {
		...
		const contact = await alpacaBrokerService.getTrustedContact(
			req.params.accountId,
		);
...
router.post("/broker/accounts/:accountId/trusted-contact", async (req, res) => {
	try {
		...
		const contact = await alpacaBrokerService.updateTrustedContact(
			req.params.accountId,
			req.body,
		);
		res.json({ success: true, contact });
...
router.delete(
	"/broker/accounts/:accountId/trusted-contact",
	async (req, res) => {
		try {
			if (!alpacaBrokerService.isConfigured()) {
				return res
...
router.get("/broker/accounts/:accountId/cash-interest", async (req, res) => {
	try {
		...
		const status = await alpacaBrokerService.getCashInterestStatus(
			req.params.accountId,
		);
...
router.post("/broker/journals/:journalId/reverse", async (req, res) => {
	try {
		...
		const result = await alpacaBrokerService.reverseJournal(
...
router.get("/broker/reports/:reportId/download", async (req, res) => {
	try {
		...
		if (!url)
			return res
				.status(404)
				.json({ success: false, error: "Report not available for download" });
		res.json({ success: true, url });
	} catch (error: any) {
		res.status(500).json({ success: false, error: error.message });
	}
...
router.delete(
	"/broker/rebalancing/portfolios/:portfolioId",
	async (req, res) => {
		try {
			if (!alpacaBrokerService.isConfigured()) {
				return res
			...
			await alpacaBrokerService.deleteRebalancingPortfolio(
...
router.get("/broker/lrs-summary", requireAdmin, async (req, res) => {
	try {
		...
		const summary = await db
			...
			.where(eq(usLrsDeclarations.financialYear, fy));

		res.json({
			success: true,
			summary: summary[0] || { totalUsed: 0, count: 0 },
		});
	} catch (error: any) {
		res.status(500).json({ success: false, error: error.message });
	}
