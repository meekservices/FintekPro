...
import {
	getBondRiskDisclosures,
	validateDisclosureAcknowledgments,
} from "../services/bond-risk-disclosures";
...
router.get(
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (status) {
				query = query.where(
					eq(schema.bondSellListings.status, status as string),
				) as any;
			}
...
router.get(
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (status) {
				query = query.where(
					eq(schema.bondBuyRequests.status, status as string),
				) as any;
			}
	...
	async (req: Request, res: Response) => {
		try {
			...
			const [updated] = await db
				.update(schema.bondSellListings)
				.set({
					status,
					complianceStatus:
						status === "active" ? "cleared" : listing.complianceStatus,
					updatedAt: new Date(),
				})
				.where(eq(schema.bondSellListings.id, id))
				.returning();
		...
		try {
			...
			if (sellListing.isin !== buyRequest.isin) {
				return apiResponse.badRequest(
					res,
					"ISIN mismatch between listing and request",
				);
			}

			// Calculate deal values
			const dirtyPrice =
				Number.parseFloat(agreedPrice) +
				Number.parseFloat(accruedInterest || "0");
			const totalValue = quantity * dirtyPrice;

			// Create deal
			const [deal] = await db
				.insert(schema.bondDeals)
				.values({
					sellListingId,
					buyRequestId,
			...
			// Update sell listing
			const newRemaining =
				(sellListing.quantityRemaining || sellListing.quantity) - quantity;
			await db
				.update(schema.bondSellListings)
				.set({
...
router.post("/orders", requireAuth, async (req: Request, res: Response) => {
	try {
		...
		if (!isin || !quantity || quantity < 1) {
			return apiResponse.badRequest(
				res,
				"ISIN and valid quantity are required",
			);
		}
		...
		if (govBond) {
			...
			faceValue = Number.parseFloat(govBond.faceValue?.toString() || "1000");
			...
			if (corpBond) {
				...
				faceValue = Number.parseFloat(corpBond.faceValue?.toString() || "1000");
		...
		if (!bondDetails) {
			return apiResponse.notFound(
				res,
				"Bond not found with the specified ISIN",
			);
		}
		...
		const orderPrice =
			price ||
			Number.parseFloat(
				bondDetails.currentPrice?.toString() ||
					bondDetails.lastPrice?.toString() ||
					"1000",
			);
...
router.get("/orders/:id", requireAuth, async (req: Request, res: Response) => {
	try {
		...
		const [order] = await db
			...
			.from(schema.bondOrders)
			.where(
				and(eq(schema.bondOrders.id, id), eq(schema.bondOrders.userId, userId)),
			);
...
router.get("/admin/stats", requireAuth, async (req: Request, res: Response) => {
	try {
		...
		// Get counts
		const [sellListingsCount] = await db
			.select({ count: sql<number>`count(*)` })
			.from(schema.bondSellListings);

		const [buyRequestsCount] = await db
			.select({ count: sql<number>`count(*)` })
			.from(schema.bondBuyRequests);
