...
export class LoanProcessingService {
	...
	constructor() {
		...
		this.iciciBankAPI = new ICICIBankAPI({
			...
			environment:
				(process.env.ICICI_BANK_ENVIRONMENT as
					| "sandbox"
					| "uat"
					| "production") || "sandbox",
		});
		...
		this.hdfcBankAPI = new HDFCBankAPI({
			...
			environment:
				(process.env.HDFC_BANK_ENVIRONMENT as
					| "sandbox"
					| "uat"
					| "production") || "sandbox",
		});
	...
	private async checkICICIEligibility(
		...
		// Calculate offer
		let interestRate = 10.5; // Base rate for ICICI
		...
		const emi =
			(amount * monthlyRate * (1 + monthlyRate) ** request.tenure) /
			((1 + monthlyRate) ** request.tenure - 1);
		...
		return {
			...
			specialOffers:
				cibilScore > 780
					? ["0.25% rate reduction for high CIBIL", "Free insurance"]
					: [],
			validityDays: 15,
			terms: [
	...
	private async checkHDFCEligibility(
		request: LoanEligibilityRequest,
	): Promise<LoanOffer> {
		...
		return {
			...
			specialOffers:
				amount > 500000 ? ["Priority processing", "Relationship rewards"] : [],
			validityDays: 20,
			terms: [
				"Salary slips",
	...
	private async checkTataCapitalEligibility(
		request: LoanEligibilityRequest,
	): Promise<LoanOffer> {
		...
		return {
			...
			tenure: request.tenure,
			eligibilityScore: this.calculateEligibilityScore(
				request.cibilScore || 750,
				result.emi / request.monthlyIncome,
	...
	private async checkBajajFinanceEligibility(
		request: LoanEligibilityRequest,
	): Promise<LoanOffer> {
		// Use existing Bajaj Finance API
		const result = this.bajajFinanceAPI.calculatePersonalLoan(
			request.amount,
			request.tenure,
		);
