...
export class ConstraintEngine {
   ...
   */
  public applyConstraints(allocations: AllocationOutput[], limits: SystemConstraints): AllocationOutput[] {
    let constrainedVector = [...allocations];
    ...
    if (excessWeight > 0) {
       const bondVector = constrainedVector.find(a => a.asset_class === "bonds");
       if (bondVector) {
  ...
  private normalizeWeights(allocations: AllocationOutput[]): AllocationOutput[] {
    ...
    return allocations.map(a => ({
      ...
      weight: parseFloat((a.weight / totalWeight).toFixed(4)) // Rescale to perfectly exact 1.0
    }));
  }
}
