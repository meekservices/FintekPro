...
export class RoutingEngine {
	...
	calculateModelRoutingScore(
		...
		if (context.query_type === "compliance" || context.query_type === "tax") {
			w2 = 0.4; // Massive Compliance Focus
			w1 = 0.3; // High Accuracy Focus
			w3 = 0.1; // Relax latency
			w4 = 0.05; // Relax cost
			w5 = 0.15; // Keep specialization
		} else if (context.latency_requirement === "low") {
			w3 = 0.5; // Priority zero: speed
			w2 = 0.2;
			w1 = 0.2;
			w4 = 0.05;
			w5 = 0.05;
	...
	private normalizeCost(cost: number): number {
		...
		if (cost >= 0.1) return 0; // Huge penalty
		return 100 - (cost / 0.1) * 100;
	}
	...
	private getSpecializationScore(
		...
		if (queryType === "investment" && model.specialization_weights.investment)
			return model.specialization_weights.investment * 100;
		if (queryType === "tax" && model.specialization_weights.tax)
			return model.specialization_weights.tax * 100;
		if (queryType === "compliance" && model.specialization_weights.compliance)
			return model.specialization_weights.compliance * 100;
