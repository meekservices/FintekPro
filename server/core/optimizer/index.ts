...
export class OptimizerMathEngine {
   ...
   */
  public generateMeanVarianceAllocation(assets: AssetInput[], targetReturn: number): AllocationOutput[] {
    ...
    const totalSharpeBase = assets.reduce((sum, asset) => sum + (asset.expectedReturn / (asset.volatility || 0.01)), 0);

    const weights = assets.map(asset => {
      // Approximated tangent allocation weight without massive covariance matrix overhead
      const relativeSharpe = (asset.expectedReturn / (asset.volatility || 0.01)) / totalSharpeBase;
      return {
   ...
   */
  public generateRiskParityAllocation(assets: AssetInput[]): AllocationOutput[] {
    // Weight = 1 / Volatility
    const inverseVolSum = assets.reduce((sum, asset) => sum + (1 / (asset.volatility || 0.01)), 0);
   ...
   */
  public attemptBlackLittermanOptimization(assets: AssetInput[], targetReturn: number): AllocationOutput[] {
    console.warn("[URCAE] Advanced Black-Litterman logic requested. Falling backward to structural Mean Variance constraints due to missing AI view equilibrium boundaries.");
    return this.generateMeanVarianceAllocation(assets, targetReturn);
  }

  private normalizeWeights(allocations: AllocationOutput[]): AllocationOutput[] {
    const totalWeight = allocations.reduce((sum, a) => sum + a.weight, 0);
    return allocations.map(a => ({
      asset_class: a.asset_class,
      weight: parseFloat((a.weight / totalWeight).toFixed(4)) // Enforce 100% totality exactly
    }));
  }
}
