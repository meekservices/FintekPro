...
import { db } from "./db";
import {
	sebiDepositoryParticipants,
	insertSebiDepositoryParticipantSchema,
} from "@shared/schema";
	...
	try {
		...
		if (!userId || !productCategory || !investmentAmount) {
			return res.status(400).json({
				success: false,
				error:
					"Missing required fields: userId, productCategory, investmentAmount",
			});
		}
...
router.post("/sebi/debt-listing", async (req: Request, res: Response) => {
	try {
		...
		const result = await sebiComplianceService.validateDebtListingCompliance(
			isin,
			bondData,
		);
		res.json({ success: true, data: result });
...
router.post("/demat/validate-account", async (req: Request, res: Response) => {
	try {
		...
		const result = await dematComplianceService.validateDematAccount(
			accountNumber,
			depository,
		);
		res.json({ success: true, data: result });
 ...
 */
router.post(
	"/demat/authorize-transaction",
	async (req: Request, res: Response) => {
		try {
			const { userId, transactionType, isin, quantity, dematAccountNumber } =
				req.body;
...
router.get("/demat/nominee/:userId", async (req: Request, res: Response) => {
	try {
		const { userId } = req.params;
		const result =
			await dematComplianceService.validateNomineeCompliance(userId);
		res.json({ success: true, data: result });
	} catch (error: any) {
...
router.get("/demat/report/:userId", async (req: Request, res: Response) => {
	try {
		const { userId } = req.params;
		const result =
			await dematComplianceService.generateComplianceReport(userId);
		res.json({ success: true, data: result });
	} catch (error: any) {
...
router.post("/pmla/cdd", async (req: Request, res: Response) => {
	try {
		const { userId, cddType, outcome, riskCategory, findings, nextReviewDate } =
			req.body;
...
router.post("/pmla/edd", async (req: Request, res: Response) => {
	try {
		const { userId, eddReason, measures, outcome, reviewedBy, findings } =
			req.body;
...
router.post("/pmla/fiu-report", async (req: Request, res: Response) => {
	try {
		const { userId, reportType, transactionIds, suspicionIndicators } =
			req.body;
...
router.post("/retention/archive-bulk", async (req: Request, res: Response) => {
	try {
		...
		const result = await orderRetentionService.archiveOldOrders(
			olderThanDays || 90,
		);
		res.json({ success: true, data: result });
	} catch (error: any) {
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (!result) {
				return res
					.status(404)
					.json({ success: false, error: "Archived order not found" });
			}
 ...
 */
router.post(
	"/retention/regulatory-request",
	async (req: Request, res: Response) => {
		try {
			const { requestType, requestedBy, userId, orderIds, dateRange } =
				req.body;
 ...
 */
router.get(
	"/retention/verify/:archiveId",
	async (req: Request, res: Response) => {
		try {
			const { archiveId } = req.params;
			const result =
				await orderRetentionService.verifyArchiveIntegrity(archiveId);
			res.json({ success: true, data: result });
		} catch (error: any) {
...
router.get("/dashboard/:userId", async (req: Request, res: Response) => {
	try {
		...
		const scores = [
			...
			fatcaCrsStatus.overallStatus === "compliant"
				? 100
				: fatcaCrsStatus.overallStatus === "action_required"
					? 60
					: fatcaCrsStatus.overallStatus === "pending"
						? 40
						: 20,
		];
...
router.get("/dp-registry", async (req: Request, res: Response) => {
	try {
		const { page = "1", limit = "50", depository, status, search } = req.query;

		const offset =
			(Number.parseInt(page as string) - 1) * Number.parseInt(limit as string);

		const query = db.select().from(sebiDepositoryParticipants);

		// Apply filters
		const conditions = [];
		if (depository) {
			conditions.push(
				eq(sebiDepositoryParticipants.depository, depository as string),
			);
		}
		if (status) {
		...
		if (search) {
			conditions.push(
				or(
					...
					like(
						sebiDepositoryParticipants.sebiRegistrationNumber,
						`%${search}%`,
					),
				),
			);
...
router.post("/dp-registry", async (req: Request, res: Response) => {
	try {
		// Validate input
		const validationResult = insertSebiDepositoryParticipantSchema.safeParse(
			req.body,
		);
		...
		const inserted = await db
			.insert(sebiDepositoryParticipants)
			.values(dpData)
			.returning();

		console.log(
			`[DP Registry] Added new DP: ${dpData.dpId} - ${dpData.dpName}`,
		);
	...
	async (req: Request, res: Response) => {
		try {
			...
			const updated = await db
				.update(sebiDepositoryParticipants)
				.set({
					status,
					statusReason,
...
router.get("/dp-registry-stats", async (req: Request, res: Response) => {
	try {
		const stats = await db
			...
			.from(sebiDepositoryParticipants)
			.groupBy(
				sebiDepositoryParticipants.depository,
				sebiDepositoryParticipants.status,
			);
