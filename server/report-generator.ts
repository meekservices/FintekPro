...
import { generatedReports, reportAccessLogs, clientStatements, aiTransactionTracking } from "@shared/schema";
...
export interface ReportRequest {
  userId: string;
  reportType: "transaction_history" | "account_statement" | "tax_report" | "capital_gains" | "dividend_income";
 ...
 */
export async function generateTransactionReport(request: ReportRequest): Promise<GeneratedReportData> {
  try {
    const { userId, reportType, reportFormat, dateFrom, dateTo, transactionTypes, filters } = request;
    ...
    // Apply date filters
    if (dateFrom && dateTo) {
      whereConditions.push(between(aiTransactionTracking.transactionDate, dateFrom, dateTo));
    }
    ...
    await db.insert(generatedReports).values({
      ...
      transactionTypes: transactionTypes ? sql`${JSON.stringify(transactionTypes)}::jsonb` : null,
...
export async function generateAccountStatement(
  ...
  try {
    ...
    const totalCredits = transactions
      .filter((t) => t.transactionCategory === "deposit" || t.transactionCategory === "salary")
      .reduce((sum, t) => sum + parseFloat(t.amount || "0"), 0);

    const totalDebits = transactions
      .filter((t) => t.transactionCategory === "withdrawal" || t.transactionCategory === "investment")
      .reduce((sum, t) => sum + parseFloat(t.amount || "0"), 0);
