...
export class ProductAccountService {
	...
	async getAccountsForProduct(
		...
		if (requirements.requiresBank && !bankAccountId) {
			...
			const verifiedAccount = bankAccounts.find(
				(acc) => (acc as any).isVerified && acc.isActive,
			);
		...
		if (requirements.requiresDemat && !dematAccountId) {
			...
			const verifiedAccount = filteredAccounts.find(
				(acc) => (acc as any).isVerified && acc.isActive,
			);
		...
		if (requirements.requiresBank) {
			if (!bankAccountId) {
				...
				if (!bankAccount) {
					...
					errors.push(
						"Bank account is not verified. Please complete verification first.",
					);
		...
		if (requirements.requiresDemat) {
			if (!dematAccountId) {
				...
				if (!dematAccount) {
					...
					errors.push(
						"Demat account is not verified. Please complete verification first.",
					);
	...
	getProductRequirements(productType: string): ProductRequirements {
		return (
			PRODUCT_REQUIREMENTS[productType] || {
				requiresBank: true,
				requiresDemat: false,
			}
		);
