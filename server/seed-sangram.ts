...
async function seedSangram() {
  ...
  const existingAgent = await db.select().from(customerCareAgents).where(eq(customerCareAgents.email, email)).limit(1);
  
  if (existingAgent.length > 0) {
    console.log("✅ Agent profile already exists in 'customerCareAgents' table.");
  } else {
