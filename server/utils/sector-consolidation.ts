...
export function mapToBroadSector(
	granularSector: string | null | undefined,
): BroadSector {
	...
	if (
		...
		normalizedSector.includes("finance") ||
		normalizedSector.includes("insurance")
	) {
		return "Banking & Finance";
	}
	if (
		normalizedSector.includes("pharma") ||
		normalizedSector.includes("health") ||
		normalizedSector.includes("hospital")
	) {
		return "Healthcare & Pharma";
	}
	if (
		normalizedSector.includes("software") ||
		normalizedSector.includes("it ") ||
		normalizedSector.includes("tech")
	) {
		return "Technology";
	}
	if (
		normalizedSector.includes("power") ||
		normalizedSector.includes("energy") ||
		normalizedSector.includes("oil") ||
		normalizedSector.includes("gas")
	) {
		return "Energy & Utilities";
	}
	if (
		normalizedSector.includes("steel") ||
		normalizedSector.includes("metal") ||
		normalizedSector.includes("mining")
	) {
		return "Metals & Mining";
	}
	if (
		normalizedSector.includes("chemical") ||
		normalizedSector.includes("fertilizer") ||
		normalizedSector.includes("pesticide")
	) {
		return "Chemicals";
	}
	if (
		normalizedSector.includes("cement") ||
		normalizedSector.includes("construct") ||
		normalizedSector.includes("infrastructure")
	) {
	...
	if (
		...
		normalizedSector.includes("property")
	) {
		return "Real Estate";
	}
	if (
		normalizedSector.includes("fmcg") ||
		normalizedSector.includes("consumer") ||
		normalizedSector.includes("retail") ||
		normalizedSector.includes("food")
	) {
		return "Consumer Goods & Retail";
	}
	if (
		normalizedSector.includes("auto") ||
		normalizedSector.includes("vehicle") ||
		normalizedSector.includes("industrial")
	) {
		return "Manufacturing";
	}
	if (
		normalizedSector.includes("hotel") ||
		normalizedSector.includes("media") ||
		normalizedSector.includes("telecom") ||
		normalizedSector.includes("logistics")
	) {
		return "Services";
	}
...
export function getBroadSectorStats(): Record<BroadSector, string[]> {
	const stats: Record<BroadSector, string[]> = {} as Record<
		BroadSector,
		string[]
	>;
 ...
 */
export function getSectorRiskLevel(
