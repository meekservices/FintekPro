...
export class UnlistedTradingError extends AppError {
	...
	protected getDefaultUserMessage(): string {
		return (
			UNLISTED_ERROR_MESSAGES[this.errorCode] ||
			"Unable to complete this trading action. Please try again."
		);
	}
}
...
export const UNLISTED_ERROR_MESSAGES: Record<UnlistedErrorCode, string> = {
	COMPANY_SUSPENDED:
		"Trading is temporarily suspended for this company. Check back later or contact support.",
	INSUFFICIENT_QUANTITY:
		"Not enough shares available to complete your order. Try reducing the quantity.",
	PRICE_MISMATCH:
		"The price has changed since you started. Please refresh and try again.",
	LISTING_EXPIRED:
		"This listing has expired. Please browse other available listings.",
	LISTING_NOT_FOUND:
		"This listing is no longer available. It may have been sold or cancelled.",
	DEAL_ALREADY_MATCHED:
		"This deal has already been matched with another party.",
	DEAL_CANCELLED: "This deal has been cancelled and cannot be modified.",
	SELF_TRADING_BLOCKED:
		"You cannot trade with yourself. Use a different account for selling.",
	COMPLIANCE_BLOCK:
		"This transaction requires additional verification. Please contact support.",
	HIGH_RISK_COMPANY:
		"This company has elevated risk factors. Please review the risk disclosures.",
	CART_LIMIT_EXCEEDED: "You can add up to 10 companies to your cart at once.",
	DUPLICATE_CART_ITEM:
		"This company is already in your cart. Update the existing item instead.",
	INVALID_PRICE_RANGE:
		"Your price is outside the acceptable range. Check current market prices.",
...
export class EscrowError extends AppError {
	...
	constructor(
		...
		const status = ["GATEWAY_ERROR", "TIMEOUT", "PROCESSING_ERROR"].includes(
			errorCode,
		)
			? 503
			: 400;
	...
	protected getDefaultUserMessage(): string {
		return (
			ESCROW_ERROR_MESSAGES[this.errorCode] ||
			"Payment processing issue. Please try again or use a different payment method."
		);
	}
}
...
export const ESCROW_ERROR_MESSAGES: Record<EscrowErrorCode, string> = {
	...
	ESCROW_NOT_FOUND:
		"Payment record not found. Please contact support if you believe this is an error.",
	...
	TRANSFER_NOT_CONFIRMED:
		"Share transfer has not been confirmed yet. Please wait for seller confirmation.",
	GATEWAY_ERROR:
	...
	REFUND_FAILED:
		"Refund could not be processed. Our team will manually process it within 3-5 business days.",
...
export class DocumentError extends AppError {
	...
	protected getDefaultUserMessage(): string {
		return (
			DOCUMENT_ERROR_MESSAGES[this.errorCode] ||
			"Document upload issue. Please try again."
		);
	}
}
...
export const DOCUMENT_ERROR_MESSAGES: Record<DocumentErrorCode, string> = {
	...
	VERIFICATION_PENDING:
		"Document is being verified. You will be notified once complete.",
...
export class DealLifecycleError extends AppError {
	...
	protected getDefaultUserMessage(): string {
		const statusMessages: Record<string, string> = {
			...
			shares_transferred:
				"Shares have been transferred. Awaiting final confirmation.",
		...
		return (
			statusMessages[this.currentStatus] ||
			`Deal is currently ${this.currentStatus} and cannot be modified.`
		);
	}
}
