	...
	if (!key) {
		...
		if (sessionSecret && sessionSecret.length >= 32) {
			if (process.env.NODE_ENV !== "production") {
				console.warn(
					"⚠️ [ENCRYPTION] Using SESSION_SECRET as field encryption key. Set FIELD_ENCRYPTION_KEY for stronger isolation.",
				);
			}
		...
		if (process.env.NODE_ENV === "development" || !process.env.NODE_ENV) {
			console.warn(
				"⚠️ [ENCRYPTION] Using default encryption key. Set FIELD_ENCRYPTION_KEY in production!",
			);
			return "dev-encryption-key-do-not-use-in-prod-32chars";
		}
		throw new EncryptionError(
			"FIELD_ENCRYPTION_KEY or SESSION_SECRET environment variable is not set",
		);
	}
 ...
 * Returns: { encrypted: string, maskedDisplay: string }
 */
export function encryptAadhaar(aadhaar: string): {
	encrypted: string;
	maskedDisplay: string;
} {
