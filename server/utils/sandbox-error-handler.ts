...
export function handleSandboxError(error: any): never {
	...
	console.error(
		`[Sandbox API Error] Status: ${statusCode}, Message: ${message}${transactionId ? `, Transaction ID: ${transactionId}` : ""}`,
	);
...
export function is403InsufficientPrivilege(error: any): boolean {
	return (
		...
		error.response?.data?.message
			?.toLowerCase()
			?.includes("insufficient privilege")
	);
}
...
export async function withRetry<T>(
	...
	for (let attempt = 0; attempt < maxRetries; attempt++) {
		try {
			...
			console.log(
				`[Sandbox API] Retry ${attempt + 1}/${maxRetries} after ${Math.round(delay)}ms`,
			);
