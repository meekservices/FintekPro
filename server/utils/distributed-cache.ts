...
class DistributedCache {
	...
	private async initRedis(): Promise<void> {
		...
		if (!redisUrl) {
			logger.info(
				"[DistributedCache] No REDIS_URL set — using local LRU cache",
			);
			return;
		}
		try {
			...
			if (!createClient) {
				logger.warn(
					"[DistributedCache] redis package not installed — using local LRU",
				);
				return;
			}
			...
			client.on("error", (err: any) => {
				if (this.usingRedis) {
					logger.warn(
						"[DistributedCache] Redis error — falling back to local LRU",
						{ error: String(err) },
					);
			...
			client.on("ready", () => {
				logger.info(
					"[DistributedCache] Redis connected — using distributed cache",
				);
