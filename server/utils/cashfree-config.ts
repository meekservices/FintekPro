...
function resolveEnvironment(productEnvKey: string): "PRODUCTION" | "SANDBOX" {
	const explicit =
		process.env[productEnvKey] || process.env.CASHFREE_ENVIRONMENT;
	if (explicit)
		return explicit.toUpperCase() === "PRODUCTION" ? "PRODUCTION" : "SANDBOX";
...
export function getCashfreeSecureIDBaseUrl(): string {
	if (process.env.CASHFREE_SECUREID_BASE_URL)
		return process.env.CASHFREE_SECUREID_BASE_URL.replace(/\/$/, "");
...
export function hasCashfreeSubscriptionsCredentials(): boolean {
	return !!(
		getCashfreeSubscriptionsAppId() && getCashfreeSubscriptionsSecretKey()
	);
}
	...
	return [
		{
			...
			configured: hasCashfreePGCredentials(),
			environment: pgEnv,
		},
		{
			product: "Secure ID",
		...
		{
			product: "Payouts",
			configured: hasCashfreePayoutsCredentials(),
			environment: pgEnv,
		},
		{
			product: "Subscriptions",
			configured: hasCashfreeSubscriptionsCredentials(),
			environment: pgEnv,
		},
		{
			product: "Cross Border",
			configured: hasCashfreeCrossBorderCredentials(),
			environment: pgEnv,
		},
		{
			product: "Auto Collect",
			configured: hasCashfreeAutoCollectCredentials(),
			environment: pgEnv,
		},
	];
}
