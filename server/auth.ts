...
import { registerAuthEventConsumers } from "./auth-event-consumers";
...
function generateOtp(): string {
  // Production ready OTP generation
  return randomInt(100000, 999999).toString();
}

/**
 * Ensures the session is stamped with the correct portal type based on the request subdomain.
 * This is crucial for multi-portal session isolation and security.
 */
function stampSessionPortal(req: any) {
  if (req.session) {
    ...
    if (hostname.startsWith('agent.') || hostname.includes('agent.fintekpro.com')) {
      req.session.portalType = 'agent';
    } else if (hostname.startsWith('partner.') || hostname.includes('partner.fintekpro.com')) {
      req.session.portalType = 'partner';
    } else if (hostname.startsWith('admin.') || hostname.includes('admin.fintekpro.com')) {
      req.session.portalType = 'admin';
    } else {
      req.session.portalType = 'main';
    }
    console.log(`[Session] Stamped portal type: ${req.session.portalType} for host: ${hostname}`);
  }
}

/**
 * Utility to determine OTP channel priority for a user
 */
async function getOtpChannelOrder(userId?: number): Promise<string[]> {
  try {
    // Default system priority
    const defaultOrder = ['email', 'whatsapp', 'sms'];
    
    // In the future, we can fetch user-specific preferences or admin global settings here
...
async function generateUniqueUserId(email: string, firstName?: string): Promise<string> {
  const prefix = "FTP";
  const year = new Date().getFullYear().toString().slice(-2);
  const random = randomInt(1000, 9999).toString();
  
  // Use first 3 chars of first name or 'USR'
  const namePart = firstName ? firstName.slice(0, 3).toUpperCase() : "USR";
  
  const userId = `${prefix}${year}${namePart}${random}`;
  
  // Check if it already exists (very unlikely but good to have)
  const existing = await storage.getUserByUserId(userId);
  if (existing) {
    return generateUniqueUserId(email, firstName);
  }
...
export function setupAuth(app: Express) {
  // Explicitly register event consumers on startup
  registerAuthEventConsumers();

  passport.use(
    new LocalStrategy(async (username, password, done) => {
      try {
        // Support login by email, mobile, or userId
        let user;
        if (username.includes("@")) {
          user = await storage.getUserByEmail(username);
        } else if (username.startsWith("FTP")) {
          user = await storage.getUserByUserId(username);
        } else {
          user = await storage.getUserByMobile(username);
        }
  ...
  // Health check endpoint for auth status
  ...
  // Register endpoint - Step 1: Request OTP
  app.post("/api/register", async (req, res) => {
    try {
      // Security Check: Block registration on admin portal
      const hostname = (req.headers["x-forwarded-host"] || req.hostname || req.get('host') || '').toString().toLowerCase();
      if (hostname.startsWith('admin.') || hostname.includes('admin.fintekpro.com')) {
        console.warn(`⚠️ [SECURITY] Registration attempt blocked from admin portal - Host: ${hostname}, IP: ${req.ip}`);
  ...
  app.post("/api/register/verify-otp", async (req, res) => {
    try {
      ...
      req.login(user, (err) => {
        ...
        return apiResponse.success(res, user, "Account created successfully");
      });
    } catch (error) {
  ...
  app.post("/api/register/resend-otp", async (req, res) => {
    try {
      ...
      if (!registrationToken) {
        return apiResponse.badRequest(res, "Registration token is required");
      }

      // Find OTP record by token in metadata
      const otpRecord = await db.query.otpVerifications.findFirst({
        where: sql`metadata->>'registrationToken' = ${registrationToken}`
      });

      if (!otpRecord) {
        return apiResponse.notFound(res, "Registration session not found");
      }
      ...
      // Resend OTP
      await emailService.sendRegistrationOTP(metadata.email, otp);
      await whatsappService.sendLoginOTP(metadata.mobile, otp);

      return apiResponse.success(res, {}, "Verification code resent successfully");
    } catch (error) {
  ...
  // Unified Login Endpoint (Step 1: Check Password)
  app.post("/api/login", async (req, res, next) => {
    try {
      // Passport's authenticate method
      ...
      passport.authenticate("local", async (err: any, user: any, info: any) => {
        ...
        if (!user) {
          console.log(`[Login] Failed attempt for: ${req.body.username}`);
        ...
        try {
          ...
          if (!user.isActive) {
            console.warn(`⚠️ [Login] Inactive user blocked: ${user.id}`);
            return apiResponse.forbidden(res, "Account is inactive. Please contact support.");
          }

          // Portals access control - Check if user has permission for the current subdomain
          const hostname = (req.headers["x-forwarded-host"] || req.hostname || req.get('host') || '').toString().toLowerCase();
          ...
          if (hostname.startsWith('admin.') || hostname.includes('admin.fintekpro.com')) {
            if (!isAdmin) {
              console.warn(`⚠️ [SECURITY] Non-admin access attempt blocked from admin portal - User: ${user.id}, Roles: ${userRoles}`);
              return apiResponse.forbidden(res, "Access denied. Admin privileges required.");
            }
          } else if (hostname.startsWith('agent.') || hostname.includes('agent.fintekpro.com')) {
            if (!isAgent && !isAdmin) {
              console.warn(`⚠️ [SECURITY] Non-agent access attempt blocked from agent portal - User: ${user.id}, Roles: ${userRoles}`);
              return apiResponse.forbidden(res, "Access denied. Agent profile required.");
            }
          } else if (hostname.startsWith('partner.') || hostname.includes('partner.fintekpro.com')) {
            if (!isPartner && !isAdmin) {
              console.warn(`⚠️ [SECURITY] Non-partner access attempt blocked from partner portal - User: ${user.id}, Roles: ${userRoles}`);
              return apiResponse.forbidden(res, "Access denied. Partner profile required.");
            }
          }

          // Step 2: Trigger OTP as a mandatory second layer for ALL logins
          const otp = generateOtp();
          const expiresAt = new Date(Date.now() + 10 * 60 * 1000); // 10 minutes for login verification
          
          // Tester account logic
          const isTesterAccount = user.email === 'tester@fintekpro.com' || user.mobile === '9999999999';
          
          // Determine where to send OTP (prioritize mobile for security)
          const otpDestination = user.mobile || user.email;
          const otpType = user.mobile ? "mobile" : "email";

          if (!otpDestination) {
            console.error(`❌ [Login] User ${user.id} has no email or mobile for OTP delivery`);
