...
function isTesterAccount(
	user: Pick<UserType, "userId" | "email" | "roles">,
): boolean {
	return Boolean(
...
function canUseFixedTesterOtp(
	user: Pick<UserType, "userId" | "email" | "roles">,
): boolean {
	return (
		(process.env.ALLOW_TESTER_BYPASS === "true" || !isProductionRuntime()) &&
		isTesterAccount(user)
	);
}

function getTargetPortal(req: any): string {
	return (req.query.portal ||
		req.headers["x-portal-context"] ||
		req.session?.targetPortal ||
		req.subdomain ||
		"main") as string;
}

function canAccessPortal(
	user: Pick<UserType, "roles">,
	targetPortal: string,
): boolean {
...
async function trustCurrentPinDevice(
	req: any,
	res: Response,
	userId: string,
): Promise<string> {
...
async function getOtpChannelOrder(userId: string): Promise<string[]> {
	try {
		...
		if (user && (user as any).preferredOtpChannel) {
			...
			const others = ["whatsapp", "email", "sms"].filter(
				(c) => c !== preferred,
			);
		...
		if (adminSettings?.defaultOtpChannel) {
		...
		console.warn(
			`[OTP_CONFIG] Error fetching channel order for user ${userId}, using defaults:`,
			error,
		);
	}
...
export function registerAuthRoutes(app: Express) {
	...
	app.post("/api/login", async (req, res, next) => {
		try {
			...
			passport.authenticate(
				"local",
				async (err: any, user: UserType | false, info: any) => {
					try {
						if (err) {
						...
						console.log(
							`[Login] User roles: ${(user.roles || []).join(", ")} | Target: ${targetPortal}`,
						);
						...
						if (!canAccessPortal(user as UserType, targetPortal)) {
							return apiResponse.forbidden(
								res,
								portalDeniedMessage(targetPortal),
							);
						}

						// 2. Test Account Bypass Logic
						const testerAccount = isTesterAccount(user as UserType);
						const fixedTesterOtpEnabled = canUseFixedTesterOtp(
							user as UserType,
						);

						if (testerAccount) {
							console.log(
								`🧪 Detected tester account: ${user.userId || user.email}. Fixed OTP enabled: ${fixedTesterOtpEnabled ? "yes" : "no"}`,
							);
						}
						...
						if (
							!fixedTesterOtpEnabled &&
							(user as UserType).isPinSet &&
							(user as UserType).loginPin
						) {
							const trusted = await isTrustedPinDevice(req, user.id);
							if (trusted) {
								console.log(
									`🔐 [Login] Trusted device detected for user ${user.id} — skipping OTP, requesting PIN`,
								);
						...
						if (!otpDestination) {
							console.error(
								`❌ [Login] User ${user.id} has no valid OTP destination (email/mobile)`,
							);
							return apiResponse.badRequest(
								res,
								"No valid OTP destination found for this account",
							);
						}

						// Store OTP for verification (using normalized identifier as the key)
						const normalizedOtpDestination =
							normalizeIdentifier(otpDestination);
						...
						if (fixedTesterOtpEnabled) {
							...
							console.log(
								`🧪 Skipping OTP delivery for test account - use OTP: ${otp}`,
							);
						} else {
							const channelOrder = await getOtpChannelOrder(user.id);
							console.log(
								`[OTP] Channel priority for ${user.id}: ${channelOrder.join(" → ")}`,
							);
							for (const channel of channelOrder) {
								if (channel === "email" && user.email) {
									...
									if (sent) {
										console.log(
											`✅ Login OTP sent via email to: ${user.email}`,
										);
										otpDelivered = true;
									...
									console.log(
										`⚠️ Email delivery failed, trying next channel...`,
									);
								} else if (channel === "whatsapp" && user.mobile) {
									const sent = await whatsappService.sendLoginOTP(
										user.mobile,
										otp,
									);
									if (sent) {
										console.log(
											`✅ Login OTP sent via WhatsApp to: ${user.mobile}`,
										);
										otpDelivered = true;
									...
									console.log(
										`⚠️ WhatsApp delivery failed, trying next channel...`,
									);
							...
							if (!otpDelivered) {
								console.error(
									`❌ [Login] All OTP delivery channels (${channelOrder.join(", ")}) failed for user ${user.id}`,
								);
							}
						}
						...
						if (!otpDelivered) {
							console.error(
								`❌ OTP delivery failed — no delivery channel available for this account`,
							);
							return res.status(503).json({
								error: "Unable to send OTP",
								message:
									"We could not reach you via SMS, WhatsApp, or email. Please contact support or try again later.",
							});
						}
						...
						return apiResponse.serverError(
							res,
							"Internal error during login callback",
						);
					}
				},
	...
	app.post("/api/register", async (req, res) => {
		try {
			const { fullName, email, mobile, password, portalType, username } =
				req.body;
	...
	app.post("/api/login/request-otp", async (req, res) => {
		try {
			...
			if (fixedTesterOtpEnabled) {
				sent = true;
				console.log(
					`🧪 Test account detected - skipping OTP delivery, use fixed OTP: ${otp}`,
				);
			} else {
				if (otpType === "email") {
					...
					sent =
						(await whatsappService.sendLoginOTP(identifier, otp)) ||
	...
	app.post("/api/login/verify-otp", async (req, res) => {
		try {
			...
			const normalizedIdentifier = normalizeIdentifier(identifier);
			console.log(
				`[VERIFY_OTP] Request: identifier=${identifier} (normalized=${normalizedIdentifier}), otp=${otp}`,
			);
			...
			if (!isValid && otpType === "email") {
				...
				if (userByEmail?.mobile) {
					console.log(
						`[VERIFY_OTP] OTP not found by email, trying mobile: ${userByEmail.mobile}`,
					);
			...
			if (pendingLoginUserId) {
				...
				console.log(
					`[VERIFY_OTP] Resolved pending login user: ${pendingLoginUserId}`,
				);
			}
			...
			if (!user) {
				console.error(
					`❌ [VERIFY_OTP] User not found after valid OTP: ${resolvedIdentifier}`,
				);
			...
			console.log(
				`[VERIFY_OTP] Found user ${user.id}. Updating login stats...`,
			);
			...
			if (!updatedUser) {
				console.error(
					`❌ [VERIFY_OTP] Failed to retrieve updated user data for ${user.id}`,
				);
				return apiResponse.serverError(
					res,
			...
			if (!req.session) {
				console.error(
					`❌ [VERIFY_OTP] Session not available for ${updatedUser.id}`,
				);
				return apiResponse.serverError(
					res,
			...
			const eitherVerified =
				updatedUser.isEmailVerified || updatedUser.isMobileVerified;
			...
			if (!eitherVerified && !fixedTesterOtpEnabled) {
				...
				console.warn(
					`[VERIFY_OTP] Neither email nor mobile verified for user ${updatedUser.id} after OTP — blocking login`,
				);
				return apiResponse.success(
					res,
					{
			...
			console.log(
				`[VERIFY_OTP] Finalizing login for user ${updatedUser.id}...`,
			);
			req.login(updatedUser, (loginErr) => {
				...
				(req.session as any).pendingLoginUserId = undefined;
				(req.session as any).pendingLoginOtpIdentifier = undefined;
				...
				req.session.save(async (saveErr) => {
					...
					try {
						pinDeviceToken = await trustCurrentPinDevice(
							req,
							res,
							updatedUser.id,
						);
						...
						console.warn(
							"⚠️ [VERIFY_OTP] Trusted device save failed (non-critical):",
							(deviceError as Error)?.message,
						);
					}

					console.log(
						`✅ [VERIFY_OTP] Success! User ${updatedUser.id} logged in. Session: ${req.sessionID}`,
					);
					...
					return apiResponse.success(
						...
						needsPinSetup
							? "Verification successful. Please set up your login PIN."
							: "Login successful",
					);
				});
			});
	...
	app.delete("/api/user/trusted-devices/:id", async (req, res) => {
		try {
			...
			console.log(
				`[TRUSTED_DEVICES_REVOKE] Device ${id} revoked by user ${req.user.id}`,
			);
	...
	app.post("/api/user/change-password", async (req, res) => {
		try {
			...
			if (!currentUser.password) {
				return apiResponse.badRequest(
					res,
					"No password set on this account. Use OTP login.",
				);
			}
	...
	app.post("/api/login/set-pin", async (req, res) => {
		try {
			...
			if (!currentUser.mobile || !currentUser.isMobileVerified) {
				return apiResponse.forbidden(
					res,
					"Verify your mobile number before setting a login PIN",
				);
			}
			...
			try {
				...
				console.warn(
					"⚠️ [PIN_SETUP] Trusted device save failed (non-critical):",
					(deviceError as Error)?.message,
				);
			}
			...
			return apiResponse.success(
				res,
				{ pinDeviceToken },
				"PIN set successfully. You can now use this for future logins.",
			);
		} catch (error) {
	...
	app.post("/api/login/verify-pin", async (req, res) => {
		try {
			...
			if (pendingLoginUserId) {
				user = await storage.getUser(pendingLoginUserId);
				console.log(
					`[PIN_VERIFY] Resolved pending login user from session: ${pendingLoginUserId}`,
				);
			}
			...
			if (!user || !user.isPinSet || !user.loginPin) {
				return apiResponse.unauthorized(
					res,
					"Invalid credentials or PIN not set",
				);
			}
			...
			req.login(user, (err) => {
				...
				req.session.save((saveErr) => {
					...
					console.log(
						`✅ [PIN_VERIFY] User ${user.id} logged in via PIN on trusted device`,
					);
					return apiResponse.success(
						res,
						{
	...
	app.post("/api/otp/send", async (req, res) => {
		try {
			...
			if (type !== "email" && type !== "mobile") {
				return apiResponse.badRequest(
					res,
					"Type must be either 'email' or 'mobile'",
				);
			}
	...
	app.post("/api/sessions/force-logout", async (req, res) => {
		try {
			...
			console.log(
				`[Force Logout] Terminating all sessions for user ID: ${user.id}`,
			);
	...
	app.post("/api/setup-pin", async (req, res) => {
		...
		try {
			...
			if (!currentUser.mobile || !currentUser.isMobileVerified) {
				return res
					.status(403)
					.send("Verify your mobile number before setting a login PIN");
			}
	...
	app.post("/api/logout", (req, res, next) => {
		...
		req.logout((err) => {
			...
			req.session.destroy((destroyErr) => {
				if (destroyErr) {
					console.error(
						`[LOGOUT] Session destroy error for session ${sessionId}:`,
						destroyErr,
					);
				...
				res.clearCookie(PIN_DEVICE_COOKIE, {
					path: "/",
					httpOnly: true,
					secure: inProd,
					sameSite: inProd ? "none" : "lax",
				...
				console.log(
					`[LOGOUT] Session ${sessionId} destroyed and cookie cleared.`,
				);
	...
	app.get("/api/user", (req, res) => {
		if (!req.isAuthenticated() || !req.user) {
			...
			console.warn(
				`[AUTH_401] /api/user | sid=${sid} | passport.user=${passportUser} | cookie=${hasCookie} | portal=${portalType} | isAuth=${req.isAuthenticated()}`,
			);
			return apiResponse.unauthorized(res);
		}
	...
	app.patch("/api/user/preferences", async (req, res) => {
		...
		try {
			...
			if (!navPosition || !validPositions.includes(navPosition)) {
				return apiResponse.badRequest(
					res,
					"Invalid nav position. Must be 'left', 'top', or 'bottom'",
				);
			}
	...
	app.post("/api/agent/pan-consent/record/:userId", async (req, res) => {
		try {
			...
			if (consent === undefined) {
				return apiResponse.badRequest(
					res,
					"Consent status (true/false) is required",
				);
			}
			...
			console.log(
				`[PAN_CONSENT] Recorded ${consent ? "CONSENT_GIVEN" : "CONSENT_WITHDRAWN"} for user ${user.id} by agent ${req.user.id}`,
			);
