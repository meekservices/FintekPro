  ...
  try {
    await executeWithRetry(
      async () => {
        attempts++;
        if (attempts < 3) {
          throw new ExternalServiceError('TestService', 'Temporary failure', undefined, true);
        }
        return { success: true };
      },
      {
    ...
    assert(attempts === 3, `Retry attempted exactly 3 times (actual: ${attempts})`);
    assert(duration >= 300, `Exponential backoff applied (duration: ${duration}ms)`);
  } catch (error) {
...
async function testRetryNonRetryableError() {
  ...
  try {
    ...
    assert(attempts === 1, `Non-retryable error attempted only once (actual: ${attempts})`);
...
async function testCircuitBreaker() {
  ...
  assert(breaker.getState() === 'OPEN', 'Circuit OPEN after threshold failures');
  ...
  try {
    ...
    assert(error.name === 'CircuitOpenError', 'CircuitOpenError thrown while OPEN');
  }
  ...
  assert(breaker.getState() === 'OPEN', 'Circuit reopens after HALF_OPEN failure');
...
async function testErrorNormalization() {
  ...
  for (const error of errors) {
    ...
    assert(typeof error.userMessage === 'string', `${error.name} has userMessage`);
    assert(typeof error.isRetryable === 'boolean', `${error.name} has isRetryable flag`);
  }
}
