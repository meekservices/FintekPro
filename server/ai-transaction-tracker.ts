...
class AITransactionTracker {
  ...
  async trackTransaction(transactionData: {
    ...
    try {
      ...
      const categorization = this.categorizeTransaction(transactionData, aiAnalysis);
      
      // Detect patterns and anomalies
      const patternAnalysis = await this.detectPatternsAndAnomalies(transactionData);
      
      // Check for compliance flags
      const complianceCheck = this.performComplianceCheck(transactionData, aiAnalysis);
      ...
      const transactionRecord: InsertAiTransactionTracking = {
        ...
        // API metadata
        apiSource: transactionData.isOnSiteTransaction ? 'internal_platform' : 'external_bank_api',
      ...
      const [savedTransaction] = await db.insert(aiTransactionTracking)
        .values(transactionRecord)
        .returning();

      // Generate alerts if necessary
      if (aiAnalysis.riskScore > 70 || patternAnalysis.anomalyScore > 80 || complianceCheck.requiresReview) {
        await this.generateAlert(savedTransaction, aiAnalysis, patternAnalysis, complianceCheck);
      }

      // Update user's transaction patterns in background
      this.updateUserTransactionPatterns(transactionData.userId).catch(console.error);
  ...
  async analyzeUserTransactions(userId: string, options: {
    fromDate?: Date;
    toDate?: Date;
    analysisType?: string;
  } = {}) {
    ...
    try {
      ...
      const fromFilter = fromDate || new Date(Date.now() - 30 * 24 * 60 * 60 * 1000); // 30 days ago
      ...
      const analysis = await this.performComprehensiveAnalysis(transactions, analysisType);
      ...
      const analysisRecord: InsertTransactionEnrichmentAnalysis = {
        ...
        averageMonthlyIncome: analysis.financial.averageMonthlyIncome.toString(),
        averageMonthlyExpense: analysis.financial.averageMonthlyExpense.toString(),
      ...
      const [savedAnalysis] = await db.insert(transactionEnrichmentAnalysis)
  ...
  private async analyzeTransactionWithAI(transactionData: any) {
    ...
    if (!transactionData.isOnSiteTransaction && !transactionData.bankName) riskScore += 25;
  ...
  private async detectPatternsAndAnomalies(transactionData: any) {
    ...
    if (historicalTransactions.length > 0) {
      ...
      const similarTransactions = historicalTransactions.filter(t => 
        Math.abs(parseFloat(t.amount) - amount) < amount * 0.1 && // Within 10% of amount
      ...
      const typicalTimeOfDay = this.getTypicalTransactionTime(historicalTransactions);
      const currentTimeOfDay = this.getTimeOfDay(transactionData.transactionDate);
  ...
  private performComplianceCheck(transactionData: any, aiAnalysis: any) {
    ...
    if (amount >= 1000000) { // ₹10 lakh or more
      amlFlag = true;
      status = 'flagged';
    ...
    if (transactionData.paymentMethod === 'cash' && amount >= 200000) { // ₹2 lakh cash
      amlFlag = true;
      status = 'flagged';
    ...
    if (transactionData.sourceType === 'international' || transactionData.destinationType === 'international') {
      status = 'under_review';
  ...
  private async generateAlert(transaction: any, aiAnalysis: any, patternAnalysis: any, complianceCheck: any) {
    ...
    if (complianceCheck.amlFlag) {
      ...
      severity = 'high';
      alertCategory = 'fraud';
    } else if (parseFloat(transaction.amount) > 500000) {
    ...
    const alertRecord: InsertTransactionAlert = {
      ...
      alertDescription: this.generateAlertDescription(alertType, transaction, aiAnalysis, patternAnalysis),
      riskScore: aiAnalysis.riskScore,
      ...
      thresholdExceeded: this.identifyThresholdsExceeded(transaction, aiAnalysis, patternAnalysis),
      ...
      followUpDate: severity === 'high' ? new Date(Date.now() + 24 * 60 * 60 * 1000) : undefined, // 24 hours
  ...
  private async performComprehensiveAnalysis(transactions: any[], analysisType: string) {
    const amounts = transactions.map(t => parseFloat(t.amount));
    const totalInflow = amounts.filter(amt => amt > 0).reduce((sum, amt) => sum + amt, 0);
    const totalOutflow = Math.abs(amounts.filter(amt => amt < 0).reduce((sum, amt) => sum + amt, 0));
    ...
    const financial = {
      ...
      totalOutflow,
      netCashFlow,
      averageMonthlyIncome: totalInflow / Math.max(1, this.getMonthsSpan(transactions)),
      averageMonthlyExpense: totalOutflow / Math.max(1, this.getMonthsSpan(transactions))
    };
    ...
    // Risk assessment
    const avgRiskScore = transactions.reduce((sum, t) => sum + (t.riskScore || 0), 0) / transactions.length;
    const highRiskTransactions = transactions.filter(t => (t.riskScore || 0) > 70).length;
    
    const risk = {
      riskFactors: this.identifyRiskFactors(transactions),
      riskScore: Math.round(avgRiskScore),
      riskCategory: avgRiskScore < 30 ? 'low' : avgRiskScore < 60 ? 'medium' : avgRiskScore < 80 ? 'high' : 'very_high',
      creditworthinessScore: Math.max(0, 850 - Math.round(avgRiskScore * 2)) // Inverse relationship
    };

    // Investment capacity analysis
    const investment = {
      disposableIncome: Math.max(0, financial.averageMonthlyIncome - financial.averageMonthlyExpense),
      investmentCapacity: Math.max(0, (financial.averageMonthlyIncome - financial.averageMonthlyExpense) * 0.7),
  ...
  private categorizeTransaction(transactionData: any, aiAnalysis: any) {
    ...
    if (amount > 0) {
      ...
      if (transactionData.transactionType === 'salary' || transactionData.merchantCategory === 'employer') {
      ...
      if (transactionData.merchantCategory) {
        if (['grocery', 'supermarket', 'pharmacy'].includes(transactionData.merchantCategory)) {
          expenseCategory = 'necessity';
        } else if (['restaurant', 'entertainment', 'travel'].includes(transactionData.merchantCategory)) {
          expenseCategory = 'lifestyle';
        } else if (['mutual_fund', 'stock', 'insurance'].includes(transactionData.merchantCategory)) {
          expenseCategory = 'investment';
        } else if (['loan_payment', 'emi'].includes(transactionData.merchantCategory)) {
          expenseCategory = 'loan_payment';
        } else if (['utility', 'telecom', 'internet'].includes(transactionData.merchantCategory)) {
  ...
  private analyzeSpendingBehavior(transactionData: any) {
    return {
      ...
      frequency: 'regular',
      avgAmount: parseFloat(transactionData.amount.toString())
    };
  }
  ...
  private identifyRiskIndicators(transactionData: any) {
    ...
    if (transactionData.paymentMethod === 'cash') indicators.push('cash_transaction');
    if (!transactionData.isOnSiteTransaction) indicators.push('external_transaction');
    if (this.getTimeOfDay(transactionData.transactionDate) === 'night') indicators.push('unusual_timing');
  ...
  private generateAlertTitle(alertType: string, transaction: any): string {
    switch (alertType) {
      case 'suspicious_activity':
        return `Suspicious Transaction Alert - ₹${parseFloat(transaction.amount).toLocaleString()}`;
      case 'large_transaction':
        return `Large Transaction Notification - ₹${parseFloat(transaction.amount).toLocaleString()}`;
  ...
  private generateAlertDescription(alertType: string, transaction: any, aiAnalysis: any, patternAnalysis: any): string {
  ...
  private identifyThresholdsExceeded(transaction: any, aiAnalysis: any, patternAnalysis: any) {
    const thresholds = [];
    const amount = parseFloat(transaction.amount);
    
    if (amount > 100000) thresholds.push({ threshold: 'amount_100k', value: amount });
    if (aiAnalysis.riskScore > 70) thresholds.push({ threshold: 'risk_score_70', value: aiAnalysis.riskScore });
    if (patternAnalysis.anomalyScore > 80) thresholds.push({ threshold: 'anomaly_score_80', value: patternAnalysis.anomalyScore });
  ...
  private getMonthsSpan(transactions: any[]): number {
    ...
    return Math.max(1, Math.ceil((latest.getTime() - earliest.getTime()) / (30 * 24 * 60 * 60 * 1000)));
  }
...
export const aiTransactionTrackerService = {
  ...
  async trackTransaction(req: Request, res: Response) {
    try {
      ...
      const result = await aiTransactionTracker.trackTransaction(transactionData);
  ...
  async analyzeTransactions(req: Request, res: Response) {
    try {
      ...
      const result = await aiTransactionTracker.analyzeUserTransactions(userId, options);
