		...
		try {
			...
			const portfolioData = {
				...
				holdings: holdings.map((h) => ({
					...
					currentPrice: Number.parseFloat(h.avgPrice ?? "0"), // Using avgPrice as currentPrice placeholder
					totalValue:
...
🤖 *Powered by FinanceHub AI*`;
	...
	async findInvestmentOpportunities(userId: string): Promise<{
		...
		const opportunities = this.generateOpportunitiesForRisk(
			riskLevel,
			marketInsights,
		);
	...
	private async generateOptimizationSuggestions(
		...
		const sectors = Array.from(
			new Set(portfolioData.holdings.map((h: any) => h.sector)),
		);
		if (sectors.length < 3) {
		...
		// Concentration risk
		const topHolding = portfolioData.holdings.reduce(
			(max: any, h: any) => (h.totalValue > (max?.totalValue || 0) ? h : max),
			null,
		);
	...
	private generateOpportunitiesForRisk(
		riskLevel: string,
		marketInsights: string,
	): string[] {
		const opportunities = [];
	...
	private getTargetAllocation(riskTolerance: string): any[] {
		...
		return (
			allocations[riskTolerance as keyof typeof allocations] ||
		...
		try {
			...
			if (!portfolios.length) {
				return {
					...
					aiSummary:
						"No portfolio found. Here are top-rated mutual funds to get started.",
				};
			}
