...
class PartnerService {
    ...
    const demoTickets: SupportTicket[] = [
      {
        ...
        description: "I want to know about the minimum investment amount for Growth Plus fund",
      ...
      },
      {
        ...
        description: "My loan application is pending for review for 5 days. Please provide update.",
        category: "billing",
        priority: "high",
        status: "in_progress",
        assignedTo: "central-test-user",
  ...
  async authenticatePartner(email: string, password: string): Promise<Partner | null> {
  ...
  async getProductsByPartner(partnerId: string): Promise<Product[]> {
    return Array.from(this.products.values()).filter(product => product.partnerId === partnerId);
  }
  ...
  async getTicketsByPartner(partnerId: string): Promise<SupportTicket[]> {
    return Array.from(this.supportTickets.values()).filter(ticket => ticket.assignedTo === partnerId);
  }
  ...
  async getPublicProducts(): Promise<Product[]> {
    return Array.from(this.products.values()).filter(product => product.isPublic && product.status === 'active');
  }

  async getProductsByCategory(category: string): Promise<Product[]> {
    return Array.from(this.products.values()).filter(product => 
      product.category === category && product.isPublic && product.status === 'active'
    );
  }
