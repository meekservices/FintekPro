...
const sampleCompanies = [
  {
    ...
    description: 'National Stock Exchange of India Limited (NSE) is the leading stock exchange of India, located in Mumbai. NSE was established in 1992 as the first dematerialized electronic exchange in the country. It is the 4th largest stock exchange in the world by equity trading volume.',
  ...
  },
  {
    ...
    description: 'Tata Technologies Limited is a global engineering and digital services company, focused on automotive, aerospace, and industrial sectors. A subsidiary of Tata Motors.',
  ...
  },
  {
    ...
    description: 'HDB Financial Services is a leading non-banking financial company (NBFC) and a subsidiary of HDFC Bank. It provides various loan products including vehicle loans, personal loans, and business loans.',
  ...
  },
  {
    ...
    description: 'Swiggy is an Indian online food ordering and delivery platform founded in 2014. It also operates Instamart for quick commerce grocery delivery.',
  ...
  },
  {
    ...
    description: 'PhonePe is a digital payments platform that provides UPI-based payments, mobile recharges, utility bill payments, and financial services. It is one of the largest digital payment platforms in India.',
...
export async function seedUnlistedMarketplace(userId: string) {
  ...
  for (const company of sampleCompanies) {
    try {
      ...
      const existing = await db.select().from(unlistedCompanies).where(sql`cin = ${company.cin}`);
      ...
      const [created] = await db.insert(unlistedCompanies).values({
        name: company.name,
        cin: company.cin,
        isin: company.isin,
  ...
  for (const financial of sampleFinancials) {
    ...
    try {
      ...
      console.log(`Created financials for company index ${financial.companyIndex}`);
    } catch (error) {
  ...
  const sellListingData = [
    { companyIndex: 0, quantity: 500, askPrice: '5800', landingPrice: '5600', floorPrice: '5400' }, // NSE
    { companyIndex: 0, quantity: 1000, askPrice: '5750', landingPrice: '5550', floorPrice: '5350' },
    { companyIndex: 1, quantity: 2000, askPrice: '1300', landingPrice: '1250', floorPrice: '1200' }, // Tata Tech
    { companyIndex: 2, quantity: 1500, askPrice: '1850', landingPrice: '1780', floorPrice: '1700' }, // HDB
    { companyIndex: 3, quantity: 5000, askPrice: '450', landingPrice: '420', floorPrice: '390' }, // Swiggy
    { companyIndex: 4, quantity: 3000, askPrice: '980', landingPrice: '930', floorPrice: '880' }, // PhonePe
  ];
  ...
  for (const listing of sellListingData) {
    ...
    try {
      ...
      console.log(`Created sell listing for company index ${listing.companyIndex}`);
    } catch (error) {
  ...
  for (const request of buyRequestData) {
    ...
    try {
      ...
      console.log(`Created buy request for company index ${request.companyIndex}`);
    } catch (error) {
