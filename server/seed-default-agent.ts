...
export async function seedDefaultAgent(): Promise<void> {
	try {
		...
		if (existingAgent.length > 0) {
			...
			if (!agent.arnCode || !agent.distributorId) {
				await db
					...
					.where(eq(customerCareAgents.email, "sangram.m@outlook.com"));
				console.log(
					"✅ Default agent (Sangram Kesari Mohanty) updated with ARN code and distributor ID",
				);
			} else {
				console.log(
					"✅ Default agent (Sangram Kesari Mohanty) already exists with complete information",
				);
			}
			return;
		}
		...
		console.log(
			"✅ Default agent (Sangram Kesari Mohanty) created successfully",
		);
	} catch (error) {
		console.error(
			"⚠️ Error seeding default agent:",
