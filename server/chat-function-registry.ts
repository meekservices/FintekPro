...
export class FunctionRegistry {
	...
	private registerFunctions() {
		...
		this.register({
			...
			description:
				"Get a comprehensive summary of the user's investment portfolio including total value, asset allocation, and top holdings.",
			category: "portfolio",
			parameters: [],
		...
		this.register({
			...
			description:
				"Get detailed list of all holdings in the user's portfolio with current prices and P&L.",
			category: "portfolio",
			parameters: [
				{
					name: "limit",
		...
		this.register({
			...
			description:
				"Get a list of recent buy/sell transactions from the user's portfolio.",
			category: "portfolio",
			parameters: [
				{
					name: "limit",
		...
		this.register({
			...
			description:
				"Get current market overview including index prices, top gainers, and losers.",
			category: "market",
			parameters: [],
			requiresConfirmation: false,
			execute: async (params, user) => {
		...
		this.register({
			...
			description:
				"Search for stocks, mutual funds, or other securities by name or symbol.",
			category: "market",
			parameters: [
				{
					name: "query",
		...
		this.register({
			...
			description:
				"Get user profile information including KYC status, risk profile, and financial goals.",
			category: "profile",
			parameters: [],
			requiresConfirmation: false,
			execute: async (params, user) => {
					...
					goals: goals.map((g: any) => ({
						...
						progress:
							g.currentAmount && g.targetAmount
								? Math.round(
										(Number.parseFloat(g.currentAmount) /
											Number.parseFloat(g.targetAmount)) *
											100,
									)
								: 0,
					})),
		...
		this.register({
			...
			description:
				"Create a buy or sell order for a mutual fund. Requires user confirmation.",
		...
		this.register({
			...
			description:
				"Rebalance portfolio to match target asset allocation. Requires user confirmation.",
			category: "transaction",
			parameters: [
				{
			...
			execute: async (params, user) => {
				...
				const suggestions = await storage.getRebalancingSuggestions(
					portfolios[0].id,
				);
		...
		this.register({
			...
			description:
				"Calculate expected returns for a Systematic Investment Plan (SIP).",
			category: "utility",
			parameters: [
				{
					name: "monthlyAmount",
	...
	async executeFunction(name: string, params: any, user: User): Promise<any> {
		...
		if (func.requiredRoles && func.requiredRoles.length > 0) {
			...
			const hasRequiredRole = func.requiredRoles.some((role) =>
				userRoles.includes(role),
			);
			if (!hasRequiredRole) {
				throw new Error(
					`Insufficient permissions. Required roles: ${func.requiredRoles.join(", ")}`,
				);
			}
		}
