...
import type {
	CredhiveFinancialStatement,
	CredhiveComplianceData,
...
function getSectorMultipleEbitda(sector: string | null | undefined): number {
	...
	const key = Object.keys(SECTOR_EV_EBITDA).find((k) =>
		sector.toLowerCase().includes(k.toLowerCase()),
	);
...
export function computeRatios(
	...
	const ebit =
		n(latest.ebit) ??
		(ebitda !== null && revenue !== null ? ebitda * 0.9 : null);

	const roe = safeDivide(pat, networth);
	const roa = safeDivide(pat, assets);
	const roce = safeDivide(
		ebit,
		assets !== null && debt !== null
			? assets - (n(latest.total_liabilities) ?? 0) + debt
			: assets,
	);
	...
	const interestCoverage =
		ebit !== null && interestEst !== null && interestEst > 0
	...
	const revenueCagr3Y =
		revenue !== null && threeRevenue !== null
			? cagr(revenue, threeRevenue, 2)
			: null;
	const patCagr3Y =
		pat !== null && threePat !== null && threePat > 0
			? cagr(pat, threePat, 2)
			: null;
...
export function computeEVEbitda(
	...
	const perShareValue =
		totalShares && totalShares > 0 ? equityValue / totalShares : null;
...
export function computeDCF(
	...
	if (baseFCF === null) {
		...
		const capex =
			n(latest.capex) ?? (n(latest.revenue) ? n(latest.revenue)! * 0.05 : null);
	...
	const projectedGrowthRate = Math.min(Math.max(rawGrowth, 0.05), 0.4); // cap 5%–40%
	...
	const pvTerminalValue = terminalValue / (1 + wacc) ** 5;
...
export function computeValuationRange(
	...
	const revenueMultiple = computeRevenueMultiple(
		statements,
		totalShares,
		sector,
	);
	...
	if (values.length > 0) {
		low = Math.min(...values) * 0.8;
		high = Math.max(...values) * 1.2;
		mid = values.reduce((a, b) => a + b, 0) / values.length;
	}

	const upside =
		mid !== null && transactionPrice && transactionPrice > 0
	...
	return {
		...
		revenueMultiple,
		method:
			methodParts.length > 0 ? methodParts.join(" + ") : "Insufficient Data",
	};
}
...
export function computeFHS(ratios: UnlistedRatios): number {
	...
	// ROE component (35%)
	if (ratios.roe !== null) {
		if (ratios.roe > 0.2) score += 35 * 1.0;
	...
	if (ratios.revenueGrowth !== null) {
		...
		else if (ratios.revenueGrowth > 0.1) score += 30 * 0.6;
		else if (ratios.revenueGrowth > 0) score += 30 * 0.3;
		else if (ratios.revenueGrowth < -0.1) score -= 15;
	}
...
export function generateUnlistedThesis(
	...
	if (ratios.roe !== null && ratios.roe > 0.15) {
		bullets.push(
			`Strong return on equity of ${(ratios.roe * 100).toFixed(1)}%, indicating efficient capital allocation`,
		);
	}
	if (ratios.revenueGrowth !== null && ratios.revenueGrowth > 0.15) {
		bullets.push(
			`Revenue growing at ${(ratios.revenueGrowth * 100).toFixed(1)}% YoY, well above industry average`,
		);
	}
	if (ratios.revenueCagr3Y !== null && ratios.revenueCagr3Y > 0.12) {
		bullets.push(
			`3-year revenue CAGR of ${(ratios.revenueCagr3Y * 100).toFixed(1)}% demonstrates consistent business momentum`,
		);
	}
	if (ratios.ebitdaMargin !== null && ratios.ebitdaMargin > 0.18) {
		bullets.push(
			`Healthy EBITDA margin of ${(ratios.ebitdaMargin * 100).toFixed(1)}% shows strong operating efficiency`,
		);
	}
	if (ratios.debtToEquity !== null && ratios.debtToEquity < 0.5) {
		bullets.push(
			`Conservative balance sheet with D/E of ${ratios.debtToEquity.toFixed(2)}x — well-positioned for growth capex`,
		);
	}
	if (valuation.upside !== null && valuation.upside > 0.2) {
		bullets.push(
			`Blended intrinsic value (${valuation.method}) suggests ${(valuation.upside * 100).toFixed(0)}% upside from current transaction price`,
		);
	}
	if (sector) {
		bullets.push(
			`Exposure to the ${sector} sector offers differentiated diversification beyond listed equities`,
		);
	}
	if (bullets.length < 3) {
		bullets.push(
			`${companyName} operates as a privately held entity, potentially positioned ahead of a future listing`,
		);
		bullets.push(
			`Unlisted status may offer entry at a discount to comparable listed peers`,
		);
	}
...
export function generateUnlistedRisks(
	...
	risks.push(
		"Liquidity risk: unlisted shares have no active secondary market; exit depends on Offer For Sale (OFS), buyback, or listing event",
	);
	risks.push(
		"Limited public disclosure: financials are MCA-filings based — may lag by 12–18 months; real-time data unavailable",
	);

	if (ratios.debtToEquity !== null && ratios.debtToEquity > 1.5) {
		risks.push(
			`Elevated leverage (D/E ${ratios.debtToEquity.toFixed(2)}x) increases refinancing risk in a rising interest rate environment`,
		);
	}
	if (ratios.revenueGrowth !== null && ratios.revenueGrowth < 0) {
		risks.push(
			`Revenue declined ${(Math.abs(ratios.revenueGrowth) * 100).toFixed(1)}% YoY — business contraction or cyclical headwind`,
		);
	}
	if (ratios.netMargin !== null && ratios.netMargin < 0) {
		risks.push(
			"Company is loss-making; path to profitability and cash flow generation needs monitoring",
		);
	}
	if (compliance) {
		if (
			compliance.overall_risk === "high" ||
			compliance.overall_risk === "critical"
		) {
			risks.push(
				`Elevated compliance risk (${compliance.overall_risk}): ${compliance.signals
					.slice(0, 2)
					.map((s) => s.description)
					.join("; ")}`,
			);
		...
		if (compliance.charges_count && compliance.charges_count > 2) {
			risks.push(
				`${compliance.charges_count} active charges registered against the company — review lender exposure`,
			);
		}
	}
	if (valuation.upside !== null && valuation.upside < 0) {
		risks.push(
			"Current transaction price appears to be above our blended intrinsic value estimate — downside risk to valuation",
		);
	}

	risks.push(
		"Regulatory risk: SEBI/RBI framework for unlisted securities trading may change; platform-specific compliance requirements apply",
	);
...
export function computeUnlistedRating(
	...
	score: number;
	label: "STRONG BUY" | "BUY" | "HOLD" | "SELL" | "STRONG SELL" | "REVIEW";
} {
...
export function runUnlistedAnalytics(
	...
	const ratios = computeRatios(statements);
	const valuation = computeValuationRange(
		statements,
		totalShares,
		transactionPrice,
		sector,
	...
	const thesis = generateUnlistedThesis(
		companyName,
		ratios,
		valuation,
		compliance,
