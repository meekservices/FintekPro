...
function displayTarget(data: ReportData): string {
	...
	if (data.priceTarget?.blended)
		return `${priceRs(data.priceTarget.blended)} (Est.)`;
	return "N/A";
}
...
export async function generatePPT(data: ReportData): Promise<Buffer> {
	...
	s1.addShape(ppt.ShapeType.line, {
		x: 0.4,
		y: 2.85,
		w: 6.8,
		h: 0,
	...
	s1.addText(
		`${data.symbol.replace(".NS", "").replace(".BO", "")}  ·  ${data.exchange}`,
		{ x: 0.4, y: 3.0, w: 7, h: 0.35, fontSize: 13, color: "CBD5E1" },
	);
	if (data.sector)
		s1.addText(data.sector, {
			x: 0.4,
			y: 3.45,
			w: 7,
			h: 0.3,
	...
	if (data.industry && data.industry !== data.sector)
		s1.addText(data.industry, {
			x: 0.4,
			y: 3.8,
			w: 7,
			h: 0.28,
	...
	s1.addText(`Composite Score: ${data.rating.score}/100`, {
		x: 7.7,
		y: 6.2,
		w: 4.8,
		h: 0.3,
	...
	s1.addText(
		`Institutional Research Note  ·  For Professional Use  ·  ${data.generatedAt}`,
		{
			x: 0.4,
			y: 7.05,
			w: 12,
	...
	s2.addText("Company & Sector Overview", {
		x: 0.4,
		y: 0.2,
		w: 12,
		h: 0.45,
	...
	s2.addShape(ppt.ShapeType.line, {
		x: 0.4,
		y: 0.7,
		w: 12,
		h: 0,
	...
	leftInfo.forEach(([label, value], i) => {
		...
		s2.addShape(ppt.ShapeType.rect, {
			x: 0.4,
			y,
			w: 5.8,
			h: 0.65,
	...
	s3.addText("Fundamentals — Revenue, EPS & Cash Flows", {
		x: 0.4,
		y: 0.2,
		w: 12,
		h: 0.45,
	...
	s3.addShape(ppt.ShapeType.line, {
		x: 0.4,
		y: 0.7,
		w: 12,
		h: 0,
	...
	const fundRows: [string, string, string | null, number | null][] = [
		...
		[
			"Revenue Growth (YoY)",
			pct(f.revenueGrowth),
			signStr(f.revenueGrowth),
			f.revenueGrowth,
		],
		["EPS", formatPrice(f.eps, f.currency), null, null],
		[
			"Earnings Growth (YoY)",
			pct(f.earningsGrowth),
			signStr(f.earningsGrowth),
			f.earningsGrowth,
		],
	...
	s4.addText("Financial Ratios vs Sector Average", {
		x: 0.4,
		y: 0.2,
		w: 12,
		h: 0.45,
	...
	s4.addShape(ppt.ShapeType.line, {
		x: 0.4,
		y: 0.7,
		w: 12,
		h: 0,
	...
	const ratioRows: Signal[] = [
		...
		},
		{
			...
			signal:
				(f as any).pbRatio && sa?.avgPB
					? (f as any).pbRatio < sa.avgPB
						? "Attractive"
						: (f as any).pbRatio > sa.avgPB * 1.2
							? "Premium"
							: "In-Line"
					: "N/A",
			signalCol:
		...
		},
		{
			label: "PEG Ratio",
			company:
				data.peg !== null && data.peg > 0
					? numFmt(data.peg)
					: f.earningsGrowth !== null && f.earningsGrowth <= 0
						? "N/M"
						: "N/A",
			sector: "—",
			signal:
		...
		},
		{
			...
			signal:
				f.debtToEquity !== null && sa?.avgDE !== null && sa?.avgDE !== undefined
					? f.debtToEquity < (sa.avgDE ?? 999)
						? "Conservative"
						: "Leveraged"
					: "N/A",
			signalCol:
	...
	// Header
	s4.addShape(ppt.ShapeType.rect, {
		x: 0.4,
		y: 0.82,
		w: 12,
		h: 0.42,
	...
	ratioRows.forEach(
		({ label, company, sector: secVal, signal, signalCol }, i) => {
			const y = 1.28 + i * 0.72;
			const bg = i % 2 === 0 ? WHITE : ALT_ROW;
			s4.addShape(ppt.ShapeType.rect, {
				x: 0.4,
	...
	if (sa?.stockCount) {
		s4.addText(
			`Sector average based on ${sa.stockCount} stocks in ${data.sector ?? "this sector"}`,
			{
				x: 0.4,
				y: 7.15,
				w: 12,
	...
	s5.addShape(ppt.ShapeType.line, {
		x: 0.4,
		y: 0.7,
		w: 12,
		h: 0,
	...
	// Header
	s5.addShape(ppt.ShapeType.rect, {
		x: 0.4,
		y: 0.82,
		w: 12,
		h: 0.42,
	...
	peerRows
		...
		.forEach(({ symbol, name, price, pe, pb, roe, marketCap, analystRating, numberOfAnalysts, isTarget }, i) => {
			...
			s5.addShape(ppt.ShapeType.rect, {
				...
				h: 0.65,
				fill: { color: bg },
				line: { color: border, width: isTarget ? 1.5 : 0.5 },
			});
			if (isTarget)
				s5.addShape(ppt.ShapeType.rect, {
					x: 0.4,
			...
			s5.addText(name.length > 28 ? name.slice(0, 28) + "…" : name, {
				x: 0.6,
				y: y + 0.15,
				w: 3.0,
				h: 0.35,
	...
	if (data.peers.length === 0) {
		s5.addText("Peer data unavailable for this sector.", {
			x: 0.4,
			y: 3.0,
			w: 12,
			h: 0.4,
	...
	if (data.plHistory && data.plHistory.rows.length > 0) {
		...
		s5b.addText("5-Year Financial History", {
			x: 0.4,
			y: 0.2,
			w: 12,
			h: 0.45,
		...
		s5b.addShape(ppt.ShapeType.line, {
			x: 0.4,
			y: 0.7,
			w: 12,
			h: 0,
		...
		const colW =
			pl.headers.length > 0 ? Math.min(1.8, 10.5 / pl.headers.length) : 1.8;
		const labelW = 12 - colW * pl.headers.length - 0.4;

		// Sub-header
		s5b.addText("Profit & Loss (₹ Crores)", {
			x: 0.4,
			y: 0.82,
			w: 6,
			h: 0.3,
		...
		if (data.salesCagr5Y !== null || data.profitCagr5Y !== null) {
			const cagrParts = [];
			if (data.salesCagr5Y !== null)
				cagrParts.push(
					`Rev 5Y CAGR: ${data.salesCagr5Y >= 0 ? "+" : ""}${(data.salesCagr5Y * 100).toFixed(1)}%`,
				);
			if (data.profitCagr5Y !== null)
				cagrParts.push(
					`PAT 5Y CAGR: ${data.profitCagr5Y >= 0 ? "+" : ""}${(data.profitCagr5Y * 100).toFixed(1)}%`,
				);
			s5b.addText(cagrParts.join("  ·  "), {
				x: 6.5,
				y: 0.84,
				w: 5.9,
				h: 0.28,
				fontSize: 10,
				color:
					data.salesCagr5Y !== null && data.salesCagr5Y >= 0
						? ACCENT_GREEN
						: ACCENT_RED,
				align: "right",
				bold: true,
		...
		// Header row
		const headerY = 1.15;
		s5b.addShape(ppt.ShapeType.rect, {
			x: 0.4,
			y: headerY,
			w: 12,
			h: 0.38,
		...
		if (data.quarterlyHistory && data.quarterlyHistory.rows.length > 0) {
			...
			if (qStartY < 6.5) {
				...
				s5b.addText("Recent Quarterly Results (₹ Crores)", {
					x: 0.4,
					y: qStartY,
					w: 12,
					h: 0.3,
				...
				const qColW =
					qh.headers.length > 0 ? Math.min(1.8, 10.5 / qh.headers.length) : 1.8;
				...
				qKeyRows.forEach((row, ri) => {
					...
					row.values.forEach((v, vi) => {
						...
						s5b.addText(display, {
							x: 0.4 + qLabelW + vi * qColW,
							y: qRY + 0.08,
							w: qColW,
							h: 0.28,
							fontSize: 9.5,
							bold: isLatest,
							color: isLatest ? "7c3aed" : DARK_TEXT,
							align: "right",
						});
					});
				});
	...
	const slideTitle = isInvITSymbol
		? "Unit Holding Pattern & Trust Governance"
		: "Shareholding Pattern & Corporate Governance";
	s6.addText(slideTitle, {
		x: 0.4,
		y: 0.2,
		w: 12,
		h: 0.45,
	...
	s6.addShape(ppt.ShapeType.line, {
		x: 0.4,
		y: 0.7,
		w: 12,
		h: 0,
	...
	if (sh) {
		...
		barItems.forEach((b) => {
			...
			s6.addShape(ppt.ShapeType.rect, {
				x: xCursor,
				y: 1.0,
				w: segW,
				h: 0.5,
		...
		barItems.forEach((b, i) => {
			...
			s6.addText(`${b.label}: ${b.pct!.toFixed(1)}%`, {
				x: x + 0.35,
				y: 1.65,
				w: 2.5,
				h: 0.25,
		...
		const qRows = [
			...
			[
				`${promoterLabel} %`,
				sh.promoterPrevPct !== null
					? `${sh.promoterPrevPct.toFixed(2)}%`
					: "N/A",
				sh.promoterPct !== null ? `${sh.promoterPct.toFixed(2)}%` : "N/A",
				sh.promoterChange !== null
					? sh.promoterChange >= 0
						? `+${sh.promoterChange.toFixed(2)}%`
						: `${sh.promoterChange.toFixed(2)}%`
					: "N/A",
			],
			[
				"FII / FPI %",
				"—",
				sh.fiiPct !== null ? `${sh.fiiPct.toFixed(2)}%` : "N/A",
				"—",
			],
			[
				"DII / MF %",
				"—",
				sh.diiPct !== null ? `${sh.diiPct.toFixed(2)}%` : "N/A",
				"—",
			],
		];
		...
		if (sh.pledgedPct !== null && sh.pledgedPct > 0) {
			...
			s6.addText(
				`⚠ Pledged ${isInvITSymbol ? "Units" : "Shares"}: ${sh.pledgedPct.toFixed(1)}%`,
				{
					x: 0.4,
					y: 4.9,
					w: 6,
		...
		if (sh.promoterChange !== null && sh.promoterChange < -1) {
			s6.addText(
				`⚠ ${promoterLabel} holding declined by ${Math.abs(sh.promoterChange).toFixed(1)}% QoQ — monitor for further movement`,
				{
					x: 0.4,
					y: 5.25,
					w: 12,
	...
	s6.addText("Management & Governance Note", {
		x: 0.4,
		y: 5.75,
		w: 12,
		h: 0.3,
	...
	s6.addText(data.managementNote, {
		x: 0.4,
		y: 6.1,
		w: 12,
		h: 0.7,
	...
	s7.addText("Industry Trends & Sector Outlook", {
		x: 0.4,
		y: 0.2,
		w: 12,
		h: 0.45,
	...
	s7.addShape(ppt.ShapeType.line, {
		x: 0.4,
		y: 0.7,
		w: 12,
		h: 0,
	...
	const commItems = comm
		? [
				{ label: "Industry Trends & Tailwinds", text: comm.industryTrends },
				{
					...
					text: comm.expansionPlans,
				},
				{ label: "Investor Outlook", text: comm.outlook },
			]
		: [
				{ label: "Industry Outlook", text: "Sector commentary unavailable." },
				{ label: "", text: "" },
				{ label: "", text: "" },
			];
	...
	commItems.forEach(({ label, text }, i) => {
		if (!label) return;
		const y = 0.9 + i * 1.9;
		s7.addShape(ppt.ShapeType.rect, {
			x: 0.4,
			y,
			w: 0.08,
			h: 1.6,
	...
	s7.addText(
		"AI-generated overview based on sector dynamics, company financial profile, and market context.",
		{
			x: 0.4,
			y: 6.85,
			w: 12,
	...
	s8.addText("Price Trend & Technical Analysis", {
		x: 0.4,
		y: 0.2,
		w: 12,
		h: 0.45,
	...
	s8.addShape(ppt.ShapeType.line, {
		x: 0.4,
		y: 0.7,
		w: 12,
		h: 0,
	...
	const lo = f.fiftyTwoWeekLow,
		hi = f.fiftyTwoWeekHigh,
		cur = f.price;
	if (lo && hi && cur) {
		const pos = Math.max(0, Math.min(1, (cur - lo) / (hi - lo)));
		s8.addShape(ppt.ShapeType.rect, {
			x: 1.0,
			y: 1.0,
			w: 10,
			h: 0.3,
		...
		s8.addText(`₹${Math.round(lo).toLocaleString("en-IN")}`, {
			x: 0.4,
			y: 1.4,
			w: 1.5,
			h: 0.25,
		...
		s8.addText(`CMP ₹${Math.round(cur).toLocaleString("en-IN")}`, {
			x: 1.0 + 10 * pos - 0.5,
			y: 0.62,
			w: 2.0,
			h: 0.25,
		...
		s8.addText(`₹${Math.round(hi).toLocaleString("en-IN")}`, {
			x: 10.5,
			y: 1.4,
			w: 2,
			h: 0.25,
		...
		s8.addText(data.weekRange52Position, {
			x: 0.4,
			y: 1.7,
			w: 12,
			h: 0.3,
	...
	const levels = [
		{
			label: "Stop Loss",
			value: priceRs(data.levels.stopLoss),
			color: ACCENT_RED,
		},
		{
			label: "Support",
			value: priceRs(data.levels.support),
			color: ACCENT_AMBER,
		},
		...
		{
			label: "Target 2",
			value: priceRs(data.levels.target2),
			color: ACCENT_GREEN,
		},
	];
	...
	returns.forEach(([label, val], i) => {
		...
		s8.addText(v !== null ? signStr(v) : "N/A", {
			x: 3.1,
			y: y + 0.1,
			w: 2.5,
			h: 0.28,
	...
	// Valuation details
	if (pt?.blended) {
		s8.addText("Price Target Derivation", {
			x: 6.5,
			y: 3.4,
			w: 6,
			h: 0.35,
		...
		ptRows.forEach(([label, val], i) => {
			...
			s8.addText(val, {
				...
				fontSize: 11,
				bold: true,
				color: i === 3 ? BRAND_COLOR : DARK_TEXT,
			});
		});
	}
	...
	s9.addText("Investment Thesis, Risks & Recommendation", {
		x: 0.4,
		y: 0.2,
		w: 12,
		h: 0.45,
	...
	s9.addShape(ppt.ShapeType.line, {
		x: 0.4,
		y: 0.7,
		w: 12,
		h: 0,
	...
	// Rating + Score strip
	s9.addShape(ppt.ShapeType.rect, {
		x: 0.4,
		y: 0.85,
		w: 12,
		h: 0.75,
	...
	s9.addText(
		`Score: ${data.rating.score}/100  ·  Fundamentals: ${data.rating.breakdown.fundamentals}  ·  Valuation: ${data.rating.breakdown.valuation}  ·  Momentum: ${data.rating.breakdown.momentum}`,
		{
			x: 6.5,
			y: 1.0,
			w: 5.8,
	...
	// Thesis
	s9.addText("Why We Recommend — Investment Thesis", {
		x: 0.4,
		y: 1.75,
		w: 12,
		h: 0.3,
	...
	s10.addText("FintekPro Research", {
		x: 0.5,
		y: 2.2,
		w: 12,
		h: 0.6,
	...
	s10.addText("Institutional Research  ·  Investment Advisory", {
		x: 0.5,
		y: 3.0,
		w: 12,
		h: 0.4,
...
export async function generatePDF(data: ReportData): Promise<Buffer> {
	return new Promise((resolve, reject) => {
		...
		const PW = W - 90;
		const f = data.financials;
		const rc = data.rating.rating.includes("BUY")
			? "#16a34a"
		...
		doc
			...
			.fontSize(18)
			.font("Helvetica-Bold")
			.text("FintekPro Research", 45, 16);
		doc
			.fontSize(10)
			.font("Helvetica")
			.fillColor("#94A3B8")
			.text("Institutional Research Note", 45, 42);
		doc
			.fontSize(9)
			.fillColor("#64748B")
			.text(data.generatedAt, W - 140, 42);
		...
		stats.forEach(({ label, value, isRating }, i) => {
			...
			doc
				.fillColor(isRating ? "#FFFFFF" : "#64748B")
				.fontSize(8)
				.font("Helvetica")
				.text(label, bx + 6, 142, { width: bw - 12 });
			doc
				.fillColor(isRating ? "#FFFFFF" : "#111827")
				.fontSize(isRating ? 13 : 14)
				.font("Helvetica-Bold")
				.text(value, bx + 6, 157, { width: bw - 12 });
		});
		...
		doc
			...
			.fontSize(13)
			.font("Helvetica-Bold")
			.text("Financial Highlights", 45, 207);
		...
		const rightRows = [
			...
			[
				"52W High / Low",
				`${formatPrice(f.fiftyTwoWeekHigh, f.currency)} / ${formatPrice(f.fiftyTwoWeekLow, f.currency)}`,
			],
		];
		...
		leftRows.forEach(([label, value], i) => {
			...
			doc
				...
				.fontSize(8.5)
				.font("Helvetica")
				.text(rl, 50 + PW / 2, ry + 5);
			doc
				.fillColor("#111827")
				.fontSize(9.5)
				.font("Helvetica-Bold")
				.text(rv, 45 + PW * 0.75, ry + 5, {
					width: PW / 4 - 5,
					align: "right",
				});
			ry += 19;
		...
		if (f.returns1M !== null || f.returns6M !== null || f.returns1Y !== null) {
			...
			[
				...
				doc
					...
					.fontSize(8.5)
					.font("Helvetica")
					.text(String(label), 45 + i * 80, ry);
				doc
					.fillColor(col)
					.fontSize(11)
					.font("Helvetica-Bold")
					.text(v !== null ? signStr(v) : "N/A", 45 + i * 80, ry + 12);
			});
			ry += 32;
		}
		...
		doc
			...
			.fontSize(10)
			.font("Helvetica-Bold")
			.text("FintekPro Research", 45, fY1 + 10);
		doc
			.fillColor("#475569")
			.fontSize(8)
			.font("Helvetica")
			.text("Institutional Research  ·  Investment Advisory", 45, fY1 + 25);
		doc
			.fillColor("#94A3B8")
			.fontSize(7)
			.font("Helvetica-Oblique")
			.text("Page 1 of 3  ·  For Professional Use Only", W - 160, fY1 + 18);
		...
		doc
			...
			.fontSize(11)
			.font("Helvetica-Bold")
			.text("FintekPro Research", 45, 9);
		doc
			.fillColor("#94A3B8")
			.fontSize(9)
			.font("Helvetica")
			.text(`${data.companyName} — Valuation, Peers & Governance`, 170, 10);
		...
		doc
			...
			.fontSize(13)
			.font("Helvetica-Bold")
			.text("Valuation Analysis — Price Target Derivation", 45, y2);
		y2 += 20;
		...
		if (data.priceTarget?.blended) {
			...
			const methods = [
				{
					label: "PE-Based",
					formula: `EPS (${formatPrice(f.eps, f.currency)}) × Target PE (${pt.targetPE?.toFixed(1)}x)`,
					result: priceRs(pt.peBased),
				},
				{
					label: "PB-Based",
					formula: `Book Value (${formatPrice((f as any).bookValue, f.currency)}) × Target PB (${pt.targetPB?.toFixed(1)}x)`,
					result: priceRs(pt.pbBased),
				},
			...
			methods.forEach(({ label, formula, result }, i) => {
				...
				doc
					...
					.fontSize(10)
					.font("Helvetica-Bold")
					.text(label, bx + 8, y2 + 5);
				doc
					.fillColor("#475569")
					.fontSize(8.5)
					.font("Helvetica")
					.text(formula, bx + 8, y2 + 20, { width: PW / 2 - 16 });
				doc
					.fillColor("#111827")
					.fontSize(12)
					.font("Helvetica-Bold")
					.text(result, bx + PW / 2 - 60, y2 + 20, {
						width: 50,
						align: "right",
					});
			});
			...
			doc
				...
				.fontSize(11)
				.font("Helvetica-Bold")
				.text(`Blended Price Target (${pt.method})`, 50, y2 + 5);
			doc
				.fillColor("#FFFFFF")
				.fontSize(16)
				.font("Helvetica-Bold")
				.text(priceRs(pt.blended), W - 130, y2 + 7, {
					width: 80,
					align: "right",
				});
			doc
				...
				.fontSize(10)
				.font("Helvetica")
				.text(`Upside: ${pt.upside}% from CMP`, 50, y2 + 19);
			y2 += 42;
			...
			scenarios.forEach(({ label, val, color }, i) => {
				...
				doc
					...
					.fontSize(9)
					.font("Helvetica-Bold")
					.text(label, sx, y2 + 6, { width: sw, align: "center" });
				doc
					.fillColor("#FFFFFF")
					.fontSize(14)
					.font("Helvetica-Bold")
					.text(val, sx, y2 + 18, { width: sw, align: "center" });
			});
			y2 += 46;
		}
		...
		doc
			...
			.fontSize(13)
			.font("Helvetica-Bold")
			.text("Financial Ratios vs Sector Average", 45, y2);
		y2 += 18;
		...
		peerRowsPDF
			...
			.forEach(({ name, symbol, price, pe, roe, mcap, analystRating, numberOfAnalysts, isTarget }, i) => {
				...
				doc.rect(45, y2, PW, 16).fill(bg);
				if (isTarget) {
					doc.rect(45, y2, 3, 16).fill("#1a56db");
				}
		...
		if (data.peers.length === 0) {
			doc
				...
				.fontSize(9)
				.font("Helvetica-Oblique")
				.text("Peer data unavailable for this sector.", 45, y2 + 4);
			y2 += 20;
		}
		...
		const promoterLabelPdf = isInvITPdf
			? "Sponsor Holding"
			: "Promoter Holding";
		const shSectionTitle = isInvITPdf
			? "Unit Holding Pattern & Governance"
			: "Shareholding Pattern & Governance";
		doc
			.fillColor("#1a56db")
			.fontSize(13)
			.font("Helvetica-Bold")
			.text(shSectionTitle, 45, y2);
		y2 += 18;
		const sh = data.shareholding;
		if (sh) {
			const shRows = [
				[
					...
					sh.promoterChange !== null
						? sh.promoterChange >= 0
							? `+${sh.promoterChange.toFixed(2)}% QoQ`
							: `${sh.promoterChange.toFixed(2)}% QoQ`
						: "",
				],
				[
				...
				[
					isInvITPdf ? "Institutional Investors" : "DII / Mutual Funds",
					sh.diiPct !== null ? `${sh.diiPct.toFixed(2)}%` : "N/A",
					sh.mutualFundPct !== null
						? `MF: ${sh.mutualFundPct.toFixed(2)}%`
						: "",
				],
				[
					"Public Unit Holders",
					sh.publicPct !== null ? `${sh.publicPct.toFixed(2)}%` : "N/A",
					sh.pledgedPct !== null && sh.pledgedPct > 0
						? `Pledged: ${sh.pledgedPct.toFixed(2)}%`
						: "",
				],
			];
			...
			doc
				...
				.fontSize(9)
				.font("Helvetica-Oblique")
				.text("Shareholding data unavailable.", 50, y2);
			y2 += 16;
		}
		y2 += 4;
		doc
			...
			.fontSize(8.5)
			.font("Helvetica-Oblique")
			.text(data.managementNote, 45, y2, { width: PW });
		...
		doc
			...
			.fontSize(9)
			.font("Helvetica-Bold")
			.text("FintekPro Research", 45, fY2 + 10);
		doc
			.fillColor("#94A3B8")
			.fontSize(7)
			.font("Helvetica-Oblique")
			.text("Page 2  ·  For Professional Use Only", W - 160, fY2 + 12);
		...
		if (data.plHistory && data.plHistory.rows.length > 0) {
			...
			doc
				...
				.fontSize(9)
				.font("Helvetica")
				.text(`${data.companyName} — Historical Financials`, 170, 10);
			...
			if (data.companyDescription) {
				doc
					...
					.fontSize(11)
					.font("Helvetica-Bold")
					.text("About the Company", 45, yH);
				yH += 16;
				doc
					.fillColor("#374151")
					.fontSize(8.5)
					.font("Helvetica")
					.text(data.companyDescription, 45, yH, { width: PW });
			...
			// CAGR badges
			const cagrItems = [
				data.salesCagr3Y !== null
					? `Revenue 3Y CAGR: ${data.salesCagr3Y >= 0 ? "+" : ""}${(data.salesCagr3Y * 100).toFixed(1)}%`
					: null,
				data.salesCagr5Y !== null
					? `Revenue 5Y CAGR: ${data.salesCagr5Y >= 0 ? "+" : ""}${(data.salesCagr5Y * 100).toFixed(1)}%`
					: null,
				data.profitCagr3Y !== null
					? `Profit 3Y CAGR: ${data.profitCagr3Y >= 0 ? "+" : ""}${(data.profitCagr3Y * 100).toFixed(1)}%`
					: null,
				data.profitCagr5Y !== null
					? `Profit 5Y CAGR: ${data.profitCagr5Y >= 0 ? "+" : ""}${(data.profitCagr5Y * 100).toFixed(1)}%`
					: null,
			...
			if (cagrItems.length > 0) {
				...
				cagrItems.forEach((label, i) => {
					...
					doc
						...
						.fontSize(7.5)
						.font("Helvetica")
						.text(label.split(":")[0], 50 + i * bw2, yH + 4, {
							width: bw2 - 10,
						});
					doc
						.fillColor(col)
						.fontSize(11)
						.font("Helvetica-Bold")
						.text(label.split(":")[1]?.trim() ?? "", 50 + i * bw2, yH + 14, {
							width: bw2 - 10,
						});
				});
				yH += 38;
			...
			doc
				...
				.fontSize(11)
				.font("Helvetica-Bold")
				.text("Multi-Year Profit & Loss (₹ Crores)", 45, yH);
			yH += 16;
			const pl2 = data.plHistory;
			const plColW2 =
				pl2.headers.length > 0
					? Math.floor((PW - 120) / pl2.headers.length)
					: 60;
			...
			doc
				...
				.fontSize(7.5)
				.font("Helvetica-Bold")
				.text("Metric", 50, yH + 4, { width: plLabelW2 });
			pl2.headers.forEach((h, hi) => {
				doc
					.fillColor("#FFFFFF")
					.fontSize(7.5)
					.font("Helvetica-Bold")
					.text(h, 45 + plLabelW2 + hi * plColW2, yH + 4, {
						width: plColW2,
						align: "right",
					});
			});
			...
			plRows2.forEach((row, ri) => {
				...
				doc
					...
					.fontSize(7.5)
					.font("Helvetica")
					.text(row.label, 50, yH + 3, { width: plLabelW2 });
				row.values.forEach((v, vi) => {
					const n = safeNum(v);
					const prev = vi > 0 ? safeNum(row.values[vi - 1]) : null;
					const col =
					...
					doc
						...
						.fontSize(8)
						.font("Helvetica-Bold")
						.text(display, 45 + plLabelW2 + vi * plColW2, yH + 3, {
							width: plColW2,
							align: "right",
						});
				});
			...
			if (data.quarterlyHistory && data.quarterlyHistory.rows.length > 0) {
				...
				doc
					...
					.fontSize(11)
					.font("Helvetica-Bold")
					.text("Quarterly Results (₹ Crores)", 45, yH);
				yH += 16;
				const qh2 = data.quarterlyHistory;
				const qColW2 =
					qh2.headers.length > 0
						? Math.floor((PW - 120) / qh2.headers.length)
						: 60;
				...
				doc
					...
					.fontSize(7.5)
					.font("Helvetica-Bold")
					.text("Metric", 50, yH + 4, { width: qLabelW2 });
				qh2.headers.forEach((h, hi) => {
					doc
						.fillColor("#FFFFFF")
						.fontSize(7.5)
						.font("Helvetica-Bold")
						.text(h, 45 + qLabelW2 + hi * qColW2, yH + 4, {
							width: qColW2,
							align: "right",
						});
				});
				...
				qRows2.forEach((row, ri) => {
					...
					doc
						...
						.fontSize(7.5)
						.font("Helvetica")
						.text(row.label, 50, yH + 3, { width: qLabelW2 });
					row.values.forEach((v, vi) => {
						const n = safeNum(v);
						const isLatest = vi === row.values.length - 1;
						const display =
							n === null
								...
								: isPercent
									...
									: isEps
										...
										: n.toLocaleString("en-IN", { maximumFractionDigits: 0 });
						doc
							.fillColor(isLatest ? "#7c3aed" : "#111827")
							.fontSize(8)
							.font(isLatest ? "Helvetica-Bold" : "Helvetica")
							.text(display, 45 + qLabelW2 + vi * qColW2, yH + 3, {
								width: qColW2,
								align: "right",
							});
					});
			...
			doc
				...
				.fontSize(9)
				.font("Helvetica-Bold")
				.text("FintekPro Research", 45, fYH + 10);
			doc
				.fillColor("#94A3B8")
				.fontSize(7)
				.font("Helvetica-Oblique")
				.text("Historical Data · For Professional Use Only", W - 200, fYH + 12);
		}
		...
		doc
			...
			.fontSize(9)
			.font("Helvetica")
			.text(`${data.companyName} — Thesis, Industry & Recommendation`, 170, 10);
		...
		doc
			...
			.fontSize(13)
			.font("Helvetica-Bold")
			.text("Industry Trends & Sector Outlook", 45, y3);
		y3 += 20;
		const comm = data.commentary;
		const commItems = comm
		...
		commItems.filter(Boolean).forEach((text, i) => {
			const labels = [
				...
				"Expansion & Strategic Initiatives",
				"Investor Outlook",
			];
			...
			doc
				...
				.fontSize(9.5)
				.font("Helvetica-Bold")
				.text(labels[i], 52, y3 + 2);
			doc
				.fillColor("#374151")
				.fontSize(9)
				.font("Helvetica")
				.text(text, 52, y3 + 15, { width: PW - 8 });
			y3 += 45;
		});
		doc
			.fillColor("#94A3B8")
			...
			.font("Helvetica-Oblique")
			.text(
				"AI-generated overview based on sector dynamics and company financial profile.",
				45,
				y3,
			);
		y3 += 18;
		...
		data.thesis.forEach((bullet, i) => {
			...
			doc
				...
				.fontSize(9)
				.font("Helvetica")
				.text(bullet, 65, y3 + 4, { width: PW - 20 });
			y3 += 30;
		});
		...
		doc
			...
			.fontSize(13)
			.font("Helvetica-Bold")
			.text("Investment Recommendation & Score Breakdown", 45, y3);
		y3 += 18;
		...
		doc
			...
			.fontSize(16)
			.font("Helvetica-Bold")
			.text(data.rating.rating, 45, y3 + 7, { width: PW / 2, align: "center" });
		doc
			.fillColor("#FFFFFF")
			.fontSize(10)
			.font("Helvetica")
			.text(`Composite Score: ${data.rating.score}/100`, 45 + PW / 2, y3 + 10, {
				width: PW / 2,
				align: "center",
			});
		y3 += 40;
		...
		const scores = [
			{
				label: "Fundamentals",
				score: data.rating.breakdown.fundamentals,
				weight: "40%",
			},
			{
				label: "Valuation",
				score: data.rating.breakdown.valuation,
				weight: "30%",
			},
			{
				label: "Momentum",
				score: data.rating.breakdown.momentum,
				weight: "30%",
			},
		];
		const bw3 = PW / 3;
		scores.forEach(({ label, score, weight }, i) => {
			...
			doc
				...
				.fontSize(8.5)
				.font("Helvetica")
				.text(`${label} (${weight})`, bx + 4, y3 + 5, { width: bw3 - 10 });
			const scoreCol =
				score >= 65 ? "#16a34a" : score >= 45 ? "#d97706" : "#dc2626";
			doc
				.fillColor(scoreCol)
				.fontSize(17)
				.font("Helvetica-Bold")
				.text(`${score}/100`, bx + 4, y3 + 20, {
					width: bw3 - 10,
					align: "center",
				});
		});
		...
		doc
			...
			.fontSize(7)
			.font("Helvetica-Oblique")
			.text(disclaimer, 45, y3, { width: PW });
		...
		doc
			...
			.fontSize(9)
			.font("Helvetica-Bold")
			.text("FintekPro Research", 45, fY3 + 10);
		doc
			.fillColor("#94A3B8")
			.fontSize(7)
			.font("Helvetica-Oblique")
			.text("Page 3 of 3  ·  For Professional Use Only", W - 160, fY3 + 12);
...
export async function generateOnePager(data: ReportData): Promise<Buffer> {
	return new Promise((resolve, reject) => {
		...
		doc
			...
			.fontSize(16)
			.font("Helvetica-Bold")
			.text("FintekPro Research", 40, 10);
		doc
			.fontSize(10)
			.font("Helvetica")
			.fillColor("#CBD5E1")
			.text("Quick Reference Note  ·  One Pager", 40, 32);
		doc
			.fillColor("#94A3B8")
			.fontSize(8)
			.font("Helvetica")
			.text(data.generatedAt, W - 130, 32);
		...
		doc
			...
			.fontSize(17)
			.font("Helvetica-Bold")
			.text(data.companyName, 40, 72);
		doc
			.fillColor("#475569")
			.fontSize(9)
			.font("Helvetica")
			.text(
				[data.symbol.replace(".NS", ""), data.sector, data.exchange]
					.filter(Boolean)
					.join("  ·  "),
				40,
				94,
		...
		stats.forEach(([label, value], i) => {
			...
			doc
				.rect(bx, 110, bw, 36)
				.fill(isRating ? rc : "#F8FAFC")
				.stroke("#E2E8F0");
			doc
				.fillColor(isRating ? "#FFFFFF" : "#64748B")
				.fontSize(7.5)
				.font("Helvetica")
				.text(label, bx + 4, 116, { width: bw - 8 });
			doc
				.fillColor(isRating ? "#FFFFFF" : "#111827")
				.fontSize(isRating ? 11 : 12)
				.font("Helvetica-Bold")
				.text(value, bx + 4, 126, { width: bw - 8 });
		});
		...
		const metrics = [
			...
			["ROCE", pct((f as any).roce)],
			["EPS", formatPrice(f.eps, f.currency)],
			["D/E Ratio", numFmt(f.debtToEquity)],
			["Rev Growth", pct(f.revenueGrowth)],
			["EPS Growth", pct(f.earningsGrowth)],
			["52W High", formatPrice(f.fiftyTwoWeekHigh, f.currency)],
			["Div Yield", pct(f.dividendYield)],
			["52W Low", formatPrice(f.fiftyTwoWeekLow, f.currency)],
			["Market Cap", formatMarketCap(f.marketCap, f.currency)],
		];
		let my = 162;
		metrics.forEach(([label, value], i) => {
			...
			doc
				...
				.fontSize(8)
				.font("Helvetica")
				.text(label, mx + 4, myi + 4);
			doc
				.fillColor("#111827")
				.fontSize(9)
				.font("Helvetica-Bold")
				.text(value, mx + PW / 4, myi + 4, {
					width: PW / 4 - 4,
					align: "right",
				});
			if (col === 1) my = myi + 17;
		...
		if (data.priceTarget?.blended) {
			...
			[
				...
				doc
					...
					.fontSize(8)
					.font("Helvetica-Bold")
					.text(label, sx, y + 5, { width: PW / 3, align: "center" });
				doc
					.fillColor("#FFFFFF")
					.fontSize(12)
					.font("Helvetica-Bold")
					.text(val, sx, y + 14, { width: PW / 3, align: "center" });
			});
			y += 36;
		}
		...
		data.thesis.slice(0, 2).forEach((bullet, i) => {
			...
			doc
				...
				.fontSize(8.5)
				.font("Helvetica")
				.text(`${i + 1}. ${bullet}`, 48, y + 2, { width: PW - 8 });
			y += 24;
		});
		...
		data.risks.slice(0, 2).forEach((risk, i) => {
			...
			doc
				...
				.fontSize(8.5)
				.font("Helvetica")
				.text(`${i + 1}. ${risk}`, 48, y + 2, { width: PW - 8 });
			y += 24;
		});
		...
		doc
			...
			.fontSize(9)
			.font("Helvetica")
			.text(levelItems.join("   ·   "), 40, y + 14, { width: PW });
		y += 32;
		...
		doc
			...
			.fontSize(9)
			.font("Helvetica-Oblique")
			.text(`52W Position: ${data.weekRange52Position}`, 40, y, { width: PW });
		if (f.returns1Y !== null) {
			...
			doc
				...
				.fontSize(9)
				.font("Helvetica-Bold")
				.text(`1Y Return: ${signStr(f.returns1Y)}`, W - 130, y);
		}
		y += 18;
		...
		doc
			...
			.fontSize(9)
			.font("Helvetica-Bold")
			.text("FintekPro Research", 40, fY + 8);
		doc
			.fillColor("#94A3B8")
			.fontSize(7)
			.font("Helvetica-Oblique")
			.text(
				"For informational use only. Not investment advice. Consult your financial advisor.",
				40,
				fY + 22,
				{ width: PW },
			);
