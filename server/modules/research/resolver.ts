 ...
 */
export async function searchExternal(q: string): Promise<ExternalSearchResult[]> {
  ...
  if (results.length < 5) {
    try {
      ...
      for (const s of quotes) {
        ...
        if (!["NSI", "BSE", "BSI", "BOM"].some(x => exchRaw.includes(x))) continue;
        seen.add(cleanSym);
        const isBse = rawSym.toUpperCase().endsWith(".BO") || ["BSE", "BSI", "BOM"].some(x => exchRaw.includes(x));
        results.push({
          symbol: cleanSym,
