...
export const TDSForm16InputSchema = z.object({
	...
	financialYear: z
		.string()
		.regex(/^\d{4}-\d{2}$/, "Invalid financial year format"),
	employees: z.array(
		z.object({
			pan: z.string(),
			name: z.string(),
...
> = {
	"194C": {
		rate: 1,
		thresholdIndividual: 30000,
		thresholdOther: 100000,
		description: "Contractor payments (Individual/HUF: 1%, Others: 2%)",
	},
	"194J": {
		rate: 10,
		thresholdIndividual: 30000,
		thresholdOther: 30000,
		description: "Professional/Technical services",
	},
	"194I": {
		rate: 10,
		thresholdIndividual: 240000,
		thresholdOther: 240000,
		description: "Rent (Plant/Equipment: 2%, Land/Building: 10%)",
	},
	"194A": {
		rate: 10,
		thresholdIndividual: 40000,
		thresholdOther: 50000,
		description: "Interest other than securities",
	},
	"194": {
		rate: 10,
		thresholdIndividual: 5000,
		thresholdOther: 5000,
		description: "Dividend",
	},
	"194H": {
		rate: 5,
		thresholdIndividual: 15000,
		thresholdOther: 15000,
		description: "Commission/Brokerage",
	},
	"194O": {
		rate: 1,
		thresholdIndividual: 500000,
		thresholdOther: 500000,
	...
	"194B": {
		rate: 30,
		thresholdIndividual: 10000,
		thresholdOther: 10000,
	...
	"194BB": {
		...
		thresholdIndividual: 10000,
		thresholdOther: 10000,
		description: "Horse racing winnings",
	},
	"194D": {
		rate: 5,
		thresholdIndividual: 15000,
	...
	"194N": {
		rate: 2,
		thresholdIndividual: 10000000,
		thresholdOther: 10000000,
...
class SandboxTDSService {
	...
	constructor() {
		...
		if (!this.apiKey || !this.apiSecret) {
			console.warn(
				"⚠️ Sandbox.co.in API credentials not configured. TDS services will use calculated data.",
			);
		}
	}
		...
		try {
			...
			if (response.status === 401) {
				...
				if (!retryResponse.ok) {
					...
					console.error("Sandbox TDS API retry error:", errorBody);
					return this.getCalculatedResponse(endpoint, data);
				}
	...
	private calculateOldRegimeTax(taxableIncome: number): {
		tax: number;
		slabs: any[];
	} {
		const slabs = [];
	...
	private calculateNewRegimeTax(taxableIncome: number): {
		tax: number;
		slabs: any[];
	} {
		const slabs = [];
		...
		for (const slab of newSlabs) {
			...
			slabs.push({
				slab: `${(prevLimit / 100000).toFixed(1)}L - ${slab.limit === Number.POSITIVE_INFINITY ? "∞" : (slab.limit / 100000).toFixed(1) + "L"}`,
				rate: slab.rate,
	...
	private calculateSalaryTDS(input: TDSSalaryInput): TDSCalculationResponse {
		try {
			...
			const grossIncome =
				...
				validated.bonus +
				validated.perquisites +
				validated.profitInLieu;
			...
			const { tax: basicTax, slabs: slabWise } =
				validated.taxRegime === "old"
			...
			const effectiveRate =
				grossIncome > 0 ? (finalTax / grossIncome) * 100 : 0;
	...
	private calculateNonSalaryTDS(
		input: TDSNonSalaryInput,
	): TDSNonSalaryResponse {
		try {
			const validated = TDSNonSalaryInputSchema.parse(input);

			const section =
				validated.section ||
				PAYMENT_TO_SECTION[validated.paymentType] ||
				"194J";
			const sectionInfo = TDS_RATES[section] || {
				rate: 10,
				thresholdIndividual: 30000,
				thresholdOther: 30000,
			...
			const threshold = validated.isIndividualHUF
				? sectionInfo.thresholdIndividual
				: sectionInfo.thresholdOther;
			...
			// Check threshold
			const applicableTDS =
				validated.thresholdExceeded || validated.amount > threshold;
			const tdsAmount = applicableTDS
				? Math.round(validated.amount * (tdsRate / 100))
				: 0;
	...
	private generateForm16Mock(input: TDSForm16Input): Form16Response {
		try {
			...
			return {
				success: false,
				message:
					error instanceof Error ? error.message : "Form 16 generation failed",
			};
		}
	}
	...
	private prepareTDSReturnMock(input: TDSReturnInput): TDSReturnResponse {
		try {
			...
			const challanTotal = validated.challanDetails.reduce(
				(sum, c) => sum + c.tdsAmount,
				0,
			);
			const deducteeTotal = validated.deducteeDetails.reduce(
			...
			if (Math.abs(challanTotal - deducteeTotal) > 1) {
				warnings.push(
					`Challan total (₹${challanTotal}) doesn't match deductee TDS total (₹${deducteeTotal})`,
				);
			}
	...
	async calculateSalaryTDSAPI(
		input: TDSSalaryInput,
	): Promise<TDSCalculationResponse> {
		try {
	...
	async calculateNonSalaryTDSAPI(
		input: TDSNonSalaryInput,
	): Promise<TDSNonSalaryResponse> {
		try {
	...
	async getTDSAnalytics(
		tan: string,
		financialYear: string,
	): Promise<TDSAnalyticsResponse> {
		try {
			const response = await this.makeAPICall(
	...
	async eFileTDSReturn(
		returnId: string,
		credentials: { userId: string; password: string },
	): Promise<TDSReturnResponse> {
		try {
			const response = await this.makeAPICall(
	...
	async calculateTDS(
		input: any,
	): Promise<TDSCalculationResponse | TDSNonSalaryResponse> {
		...
		const nonSalaryInput: TDSNonSalaryInput = {
			...
			paymentType:
				input.paymentType || this.getSectionPaymentType(input.section),
			amount: input.amount || 0,
			paymentDate: input.paymentDate || new Date().toISOString().split("T")[0],
			section: input.section,
			isIndividualHUF:
				input.isIndividualHUF ??
				(input.entityType === "individual" || input.entityType === "huf"),
	...
	private getSectionPaymentType(
		section?: string,
	): TDSNonSalaryInput["paymentType"] {
	...
	async pollTDSAnalyticsJob(jobId: string): Promise<any> {
		return this.makeAPICall(
			`/tds/analytics/potential-notices?job_id=${encodeURIComponent(jobId)}`,
			undefined,
			"GET",
		);
	}
	...
	async pollTCSAnalyticsJob(jobId: string): Promise<any> {
		return this.makeAPICall(
			`/tcs/analytics/potential-notices?job_id=${encodeURIComponent(jobId)}`,
			undefined,
			"GET",
		);
	}
	...
	async submitTDSReportJob(params: {
		...
		return this.makeAPICall(
			"/tds/reports/txt",
			{
				...
				quarter: params.quarter,
				tan: params.tan,
				...(params.previousReceiptNumber && {
					previous_receipt_number: params.previousReceiptNumber,
				}),
			},
			"POST",
		);
	...
	async pollTDSReportJob(jobId: string): Promise<any> {
		return this.makeAPICall(
			`/tds/reports/txt?job_id=${encodeURIComponent(jobId)}`,
			undefined,
			"GET",
		);
	}
	...
	async pollTCSReportJob(jobId: string): Promise<any> {
		return this.makeAPICall(
			`/tcs/reports/txt?job_id=${encodeURIComponent(jobId)}`,
			undefined,
			"GET",
		);
	}
			...
			`/tds/compliance/traces/deductors/forms/${certificateType}`,
			{
				...
				security_captcha: {
					...
					provisional_receipt_number:
						params.securityCaptcha.provisionalReceiptNumber,
					challan_amount: params.securityCaptcha.challanAmount,
					unique_pan_amount_combination_for_challan:
						params.securityCaptcha.uniquePanAmountCombination,
				},
	...
	async pollFVUGenerateJob(jobId: string): Promise<any> {
		return this.makeAPICall(
			`/tds/compliance/fvu/generate?job_id=${encodeURIComponent(jobId)}`,
			undefined,
			"GET",
		);
	}
	...
	async pollEFileJob(jobId: string): Promise<any> {
		return this.makeAPICall(
			`/tds/compliance/e-file?job_id=${encodeURIComponent(jobId)}`,
			undefined,
			"GET",
		);
	}
	...
	async submitSalaryTDSAnalyticsJob(params: {
		...
		return this.makeAPICall(
			"/tds/analytics/salary-payments/potential-notices",
			{
	...
	async pollSalaryTDSAnalyticsJob(jobId: string): Promise<any> {
		return this.makeAPICall(
			`/tds/analytics/salary-payments/potential-notices?job_id=${encodeURIComponent(jobId)}`,
			undefined,
			"GET",
		);
	}
	...
	async submitNRITDSAnalyticsJob(params: {
		...
		return this.makeAPICall(
			"/tds/analytics/nri-payments/potential-notices",
			{
	...
	async pollNRITDSAnalyticsJob(jobId: string): Promise<any> {
		return this.makeAPICall(
			`/tds/analytics/nri-payments/potential-notices?job_id=${encodeURIComponent(jobId)}`,
			undefined,
			"GET",
		);
	}
	...
	async pollTaxPLReportJob(jobId: string): Promise<any> {
		return this.makeAPICall(
			`/it/report/tax-profit-loss?job_id=${encodeURIComponent(jobId)}`,
			undefined,
			"GET",
		);
	}
	...
	async pollCapitalGainsReportJob(jobId: string): Promise<any> {
		return this.makeAPICall(
			`/it/report/capital-gains?job_id=${encodeURIComponent(jobId)}`,
			undefined,
			"GET",
		);
	}

	getTDSFormTypes(): Array<{
		form: string;
		description: string;
...
export function buildTDSSheetJSON(
				...
				blocks: [
					{
						...
						rows: data.payments.map((p) => [
							...
							p.tdsAmount,
							p.surcharge,
							p.healthAndEducationCess,
							p.deductionDate,
							p.reasonForLowerDeduction || "",
							p.certificateNumber || "",
