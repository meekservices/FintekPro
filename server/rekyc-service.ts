...
export async function getKYCStatus(userId: string): Promise<KYCStatus> {
	...
	const verifiedBank = await db.query.userBankAccounts.findFirst({
		where: (accounts, { eq, and }) =>
			and(eq(accounts.userId, userId), eq(accounts.isVerified, true)),
	});
	...
	const currentLevel: "none" | "basic" | "enhanced" = hasLevel2Requirements
		? "enhanced"
		: hasLevel1Requirements
			? "basic"
			: "none";
	...
	const lastUpdated =
		(profile as any).kycLastUpdatedDate ||
		profile.profileCompletedAt ||
		profile.createdAt;

	if (!dueDate && lastUpdated) {
		dueDate = calculateKYCDueDate(
			lastUpdated,
			(profile as any).riskCategory || "low",
		);
	}
...
export function shouldSendReKYCReminder(
	...
	shouldSend: boolean;
	reminderType: "60_day" | "30_day" | "15_day" | null;
} {
...
export async function updateKYCDueDate(userId: string) {
	...
	const lastUpdated =
		(profile as any).kycLastUpdatedDate || profile.createdAt || new Date();
	const dueDate = calculateKYCDueDate(
		lastUpdated,
		(profile as any).riskCategory || "low",
	);
...
export async function resetReKYCProcess(userId: string) {
	...
	const newDueDate = calculateKYCDueDate(
		now,
		(profile as any).riskCategory || "low",
	);
