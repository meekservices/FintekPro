...
export class TurtlefinAPI {
	...
	constructor(config?: Partial<TurtlefinConfig>) {
		...
		console.log(
			`✅ Turtlefin API initialized in ${this.config.environment} mode`,
		);
	}
	...
	async getPolicyDocument(
		...
		try {
			...
			const response = await this.client.get(
				`/policies/${policyNumber}/document`,
			);
		...
		const mockPolicies: TurtlefinPolicy[] = [
			{
				...
				status: "active",
				nominees: [{ name: "Spouse Name", relation: "Spouse", share: 100 }],
				coverageDetails: {
					baseCover: 2500000,
					riderCovers: [
	...
	private generateMockPolicyDetails(policyNumber: string): TurtlefinPolicy {
		return {
			...
			coverageDetails: {
				baseCover: 2500000,
				riderCovers: [{ name: "Accidental Death Benefit", sumAssured: 500000 }],
			},
		};
	}
	 ...
	 */
	private generateMockPremiumQuotes(
		params: PremiumCalculationParams,
	): PremiumQuote[] {
	...
	async healthCheck(): Promise<{ status: "ok" | "error"; message: string }> {
		try {
			if (this.config.environment === "sandbox") {
				return {
					status: "ok",
					message: "Turtlefin API (Sandbox) is operational",
				};
			}
