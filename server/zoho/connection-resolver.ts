...
export class ZohoConnectionResolver {
	static async bootstrapFromEnvVars(): Promise<ResolvedConnection | null> {
		...
		try {
			...
			console.log(
				"[ZohoConnectionResolver] No DB connections found, bootstrapping from env vars...",
			);
			...
			if (!tokenResponse?.access_token) {
				console.error(
					"[ZohoConnectionResolver] Bootstrap failed: no access token returned",
				);
				return null;
			}
			...
			if (!encryptedAccessToken || !encryptedRefreshToken) {
				console.error(
					"[ZohoConnectionResolver] Encryption service unavailable, aborting bootstrap for security",
				);
				return null;
			}
			...
			console.log(
				`[ZohoConnectionResolver] Bootstrap successful, connection ID: ${connection.id}`,
			);
			return {
			...
			console.error(
				"[ZohoConnectionResolver] Bootstrap from env vars failed:",
				error.message,
			);
			return null;
		}
	...
	static async resolveForAgent(
		...
		try {
			...
			console.warn(
				"ZohoConnectionResolver: Error resolving connection:",
				error,
			);
			return null;
		}
	...
	static async getMasterConnection(): Promise<ResolvedConnection | null> {
		try {
			...
			console.warn(
				"ZohoConnectionResolver: Error getting master connection:",
				error,
			);
			return null;
		}
	 ...
	 */
	static async getMasterAgentZohoAccountId(
		connectionId: string,
	): Promise<string | null> {
		try {
				...
				const [masterAgentMapping] = await db
					...
					.where(
						and(
							...
							eq(
								zohoEntityMappings.fintekproEntityId,
								connection.masterAgentId,
							),
						),
					)
					.limit(1);
			...
			console.warn(
				"ZohoConnectionResolver: Error getting master agent Zoho Account ID:",
				error,
			);
			return null;
		}
