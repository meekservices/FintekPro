...
class ZohoRateLimiter {
	...
	async consumeTokens(connectionId: string, cost: number = 1): Promise<void> {
		...
		if (bucket.tokens < cost) {
			throw new Error(
				`Rate limit exceeded for connection ${connectionId}. Available: ${Math.floor(bucket.tokens)}, Required: ${cost}`,
			);
		}
	...
	async waitForTokens(
		...
		while (!(await this.canMakeRequest(connectionId, cost))) {
			if (Date.now() - startTime > maxWaitMs) {
				throw new Error(
					`Rate limit wait timeout exceeded for connection ${connectionId}`,
				);
			}
