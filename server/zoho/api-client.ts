...
export class ZohoApiClient {
			...
			async (error) => {
				...
				if (isAuthError && !originalRequest._retry) {
					...
					try {
						...
						const newAccessToken = await this.oauthService.forceRefreshToken(
							this.connectionId,
						);
	...
	private getServiceBaseUrl(): string {
		...
		return (
			serviceUrls[this.service] || `https://www.zohoapis.${this.dataCenter}`
		);
	}
	...
	private async logApiRequest(
		...
		try {
			...
			if (msg.includes("foreign key constraint")) {
				console.warn(
					`[Zoho API] Skipping log (connection ${this.connectionId} not in DB)`,
				);
			} else {
	...
	private async logApiError(error: any): Promise<void> {
		...
		if (!this.connectionId) {
			console.error(
				`[Zoho API] Error (no connection): ${this.service} - ${error.message}`,
			);
			return;
		}
		...
		try {
			...
			if (msg.includes("foreign key constraint")) {
				console.warn(
					`[Zoho API] Skipping error log (connection ${this.connectionId} FK missing): ${this.service} - ${error.message}`,
				);
			} else {
