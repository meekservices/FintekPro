...
export function startZohoSyncScheduler(): void {
	if (!isSyncEnabled()) {
		console.log(
			"[ZohoSyncScheduler] Skipped - sync not enabled (set ZOHO_SYNC_ENABLED=true or run in production)",
		);
		return;
	}
	...
	incrementalTask = cron.schedule(
		...
		async () => {
			...
			try {
				...
				console.log(
					`[ZohoSyncScheduler] Incremental sync done: ${report.summary.totalProcessed} processed, ${report.summary.totalUpdated} updated, ${report.summary.totalConflicts} conflicts, ${report.summary.totalErrors} errors`,
				);
			} catch (error: any) {
				console.error(
					"[ZohoSyncScheduler] Incremental sync failed:",
					error.message,
				);
			}
		},
		...
		async () => {
			try {
				...
				if (result.processed > 0) {
					console.log(
						`[ZohoSyncScheduler] Webhook processing: ${result.succeeded} ok, ${result.failed} failed, ${result.deadLettered} dead-lettered`,
					);
				}
			} catch (error: any) {
				console.error(
					"[ZohoSyncScheduler] Webhook processing failed:",
					error.message,
				);
			}
		},
	...
	reconciliationTask = cron.schedule(
		...
		async () => {
			...
			try {
				...
				const totalConflicts = report.modules.reduce(
					(s, m) => s + m.conflictCount,
					0,
				);
				const totalUnmapped = report.modules.reduce(
					(s, m) => s + m.unmappedCount,
					0,
				);

				if (totalConflicts > 0 || report.financialDiscrepancies.length > 0) {
					console.warn(
						`[ZohoSyncScheduler] RECONCILIATION ALERT: ${totalConflicts} conflicts, ${report.financialDiscrepancies.length} financial discrepancies, ${totalUnmapped} unmapped records`,
					);
				} else {
					console.log(
						`[ZohoSyncScheduler] Reconciliation clean: ${totalUnmapped} unmapped records, 0 conflicts`,
					);
				}
			} catch (error: any) {
				console.error(
					"[ZohoSyncScheduler] Reconciliation failed:",
					error.message,
				);
			}
		},
	...
	console.log(
		"[ZohoSyncScheduler] Scheduled: incremental sync every 30min, webhook processing every 2min, reconciliation daily at 6:00 AM IST",
	);
}
