...
export class ZohoWebhookProcessor {
	...
	async processPendingEvents(limit: number = 50): Promise<{
		...
		if (!isSyncEnabled()) {
			console.log(
				"[WebhookProcessor] Skipped - sync not enabled (set ZOHO_SYNC_ENABLED=true or run in production)",
			);
			return {
				processed: 0,
				succeeded: 0,
		...
		if (_processingLock) {
			if (_lockAcquiredAt && Date.now() - _lockAcquiredAt > LOCK_TIMEOUT_MS) {
				console.warn(
					"[WebhookProcessor] Lock held for >10m — force-releasing stale lock",
				);
				_processingLock = false;
				_lockAcquiredAt = null;
			} else {
				console.log(
					"[WebhookProcessor] Skipped - another processing batch is already running",
				);
				return {
					processed: 0,
					succeeded: 0,
		...
		for (const event of pendingEvents) {
				...
				if (alreadyProcessed && alreadyProcessed.id !== event.id) {
					await db
						...
						.set({
							status: "completed",
							processedAt: now,
							processingError: "duplicate_skipped",
							updatedAt: now,
						})
			...
			try {
				...
				if (isDeadLetter) {
					...
					console.error(
						`[WebhookProcessor] DEAD_LETTER event ${event.id}: ${error.message}`,
					);
				} else {
					failed++;
					const backoffMs = 2 ** retryCount * 60000;
	...
	private async processCRMEvent(
		...
		if (module === "contacts" || module === "contact") {
			return this.processCRMContactEvent(
				event,
				payload,
				recordId,
				existingMapping,
		...
		if (module === "leads" || module === "lead") {
			return this.processCRMLeadEvent(
				event,
				payload,
				recordId,
				existingMapping,
		...
		if (module === "deals" || module === "deal") {
			return this.processCRMDealEvent(
				event,
				payload,
				recordId,
				existingMapping,
		...
		return {
			eventId: event.id,
			success: true,
			action: `skipped_crm_module_${module}`,
		};
	}
			...
			if (eventType.includes("create")) {
				...
				if (!contactData) {
					return {
						...
						success: false,
						action: "fetch_failed",
						error: "Could not fetch contact from Zoho",
					};
				}
				...
				if (!email && !mobile) {
					return {
						eventId: event.id,
						success: true,
						action: "skipped_no_contact_info",
					};
				}
				...
				if (email) {
					...
					if (existing) {
						...
						return {
							eventId: event.id,
							success: true,
							action: "mapped_existing_prospect",
						};
					}
				}
				...
				return {
					eventId: event.id,
					success: true,
					action: "new_contact_logged",
				};
			}
			return {
				eventId: event.id,
				success: true,
				action: "skipped_unmapped_contact",
			};
		}
		...
		if (eventType.includes("update")) {
			...
			if (!contactData) {
				return {
					...
					success: false,
					action: "fetch_failed",
					error: "Could not fetch updated contact",
				};
			}
			...
			if (fintekproType === "prospect") {
				...
				if (allowedFields.includes("email") && contactData.Email)
					updateData.email = contactData.Email.toLowerCase().trim();
				if (
					allowedFields.includes("mobile") &&
					(contactData.Mobile || contactData.Phone)
				)
					updateData.mobile = contactData.Mobile || contactData.Phone;
				...
				if (
					...
					updateData.phone = contactData.Mobile || contactData.Phone;
		...
		if (eventType.includes("update")) {
			...
			if (!leadData) {
				return {
					...
					success: false,
					action: "fetch_failed",
					error: "Could not fetch updated lead",
				};
			}
			...
			if (fintekproType === "prospect") {
				const updateData: any = { updatedAt: new Date() };
				if (allowedFields.includes("email") && leadData.Email)
					updateData.email = leadData.Email.toLowerCase().trim();
				if (
					allowedFields.includes("mobile") &&
					(leadData.Mobile || leadData.Phone)
				)
					updateData.mobile = leadData.Mobile || leadData.Phone;
				if (allowedFields.includes("name")) {
			...
			return {
				eventId: event.id,
				success: true,
				action: `updated_${fintekproType}_from_lead`,
			};
		}
		...
		if (eventType.includes("update")) {
			...
			if (!dealData) {
				return {
					...
					success: false,
					action: "fetch_failed",
					error: "Could not fetch updated deal",
				};
			}
			...
			if (existingMapping.fintekproEntityType === "partner_commission") {
				...
				const protectedFields =
					FIELD_AUTHORITY.partner_commission?.fintekpro || [];
				...
				if (dealData.Amount && protectedFields.includes("commissionAmount")) {
					...
					const currentAmount = Number.parseFloat(
						currentCommission?.commissionAmount?.toString() || "0",
					);
					const zohoAmount = Number.parseFloat(dealData.Amount.toString());
				...
				return {
					eventId: event.id,
					success: true,
					action: hasFinancialConflict
						? "commission_updated_with_conflict"
						: "updated_commission_from_deal",
					conflictDetected: hasFinancialConflict,
					conflictResolution: hasFinancialConflict
						? "fintekpro_wins_financial_field_protected"
						: undefined,
				};
			}
			...
			await this.logSync(
				"update",
				existingMapping.fintekproEntityType,
				"from_zoho",
				"CRM",
				"Deals",
				true,
	...
	private async processBooksEvent(
		event: any,
		payload: any,
	): Promise<ProcessingResult> {
		const eventType = (
		...
		return {
			eventId: event.id,
			success: true,
			action: `skipped_books_${module}`,
		};
	}

	private async processBooksInvoiceEvent(
		event: any,
		payload: any,
		eventType: string,
	): Promise<ProcessingResult> {
		...
		await this.logSync(
			eventType.includes("create") ? "create" : "update",
			"invoice",
			"from_zoho",
			"Books",
			"Invoices",
	...
	private async processBooksPaymentEvent(
		event: any,
		payload: any,
		eventType: string,
	): Promise<ProcessingResult> {
	...
	private async processSignEvent(
		event: any,
		payload: any,
	): Promise<ProcessingResult> {
		const actionType =
			payload.action_type || payload.event_type || event.eventType || "";
		const requestId = payload.requests?.request_id || payload.request_id;
		if (!requestId)
			return {
				eventId: event.id,
				success: true,
				action: "skipped_no_request_id",
			};
		...
		const signStatus =
			actionType.includes("Completed") || actionType.includes("completed")
				? "completed"
	...
	private async processMeetingEvent(
		event: any,
		payload: any,
	): Promise<ProcessingResult> {
		...
		const meetingStatus = eventType.includes("ended")
			? "ended"
		...
		return {
			eventId: event.id,
			success: true,
			action: `meeting_${meetingStatus}`,
		};
	}

	private async fetchCRMRecord(
		module: string,
		recordId: string,
	): Promise<any | null> {
		try {
			...
			console.error(
				`[WebhookProcessor] Failed to fetch ${module}/${recordId}:`,
				error.message,
			);
			return null;
		}
	...
	private async logConflict(
		...
		try {
			...
			console.warn(
				`[WebhookProcessor] CONFLICT on ${mapping.fintekproEntityType}/${mapping.fintekproEntityId} field "${field}": FintekPro=${fintekproValue}, Zoho=${zohoValue} => ${resolution}`,
			);
		} catch (e) {
	...
	async getProcessingStats(): Promise<{
		...
		stats.forEach((s) => {
			sc[s.status || "unknown"] = Number(s.count);
		});
	...
	async retryDeadLetterEvent(eventId: string): Promise<boolean> {
		const [event] = await db
			...
			.where(
				and(
					eq(zohoWebhookEvents.id, eventId),
					eq(zohoWebhookEvents.status, "dead_letter"),
				),
			)
			.limit(1);
