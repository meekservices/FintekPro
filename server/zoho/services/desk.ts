...
export interface ZohoDeskTicket {
  ...
  contactId?: string;
  contact?: { id: string; lastName?: string; firstName?: string; email?: string };
  departmentId?: string;
...
export class ZohoDeskService {
  ...
  async getAgents(params?: { limit?: number; from?: number }): Promise<ZohoDeskAgent[]> {
   ...
   */
  async searchTickets(keyword: string, params?: { limit?: number; from?: number }): Promise<ZohoDeskTicket[]> {
  ...
  async escalateTicket(ticketId: string, reason?: string): Promise<boolean> {
    await this.client.post(`/tickets/${ticketId}/escalate`, { comment: reason });
    return true;
  }
 ...
 */
export async function getZohoDeskService(dataCenter: string = 'in'): Promise<ZohoDeskService | null> {
  try {
