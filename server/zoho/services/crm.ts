...
export class ZohoCRMService {
	...
	async getLeads(limit: number = 100): Promise<ZohoCRMLead[]> {
		try {
			const response = await this.apiClient.get("/Leads", {
				fields:
					"First_Name,Last_Name,Email,Phone,Mobile,Company,Designation,Lead_Source,Lead_Status,Industry,Description,Tag,Created_Time,Modified_Time",
	...
	async getContacts(limit: number = 100): Promise<ZohoCRMContact[]> {
		try {
			const response = await this.apiClient.get("/Contacts", {
				fields:
					"First_Name,Last_Name,Email,Phone,Mobile,Account_Name,Title,Department,Mailing_Street,Mailing_City,Mailing_State,Mailing_Zip,Mailing_Country,Description,Lead_Source,Tag,Created_Time,Modified_Time",
				per_page: Math.min(limit, 200),
				sort_by: "Created_Time",
				sort_order: "desc",
			});
	...
	async updateContact(
		contactId: string,
		contactData: Partial<ZohoCRMContact>,
	): Promise<boolean> {
		try {
	...
	async bulkSyncPartnersToAccounts(partnerIds: string[]): Promise<void> {
		...
		console.log(
			`Bulk sync completed: ${succeeded} succeeded, ${failed} failed`,
		);
	}
	...
	async createCommissionDeal(commissionId: string): Promise<string> {
		...
		const dealData: ZohoCRMDeal = {
			...
			Amount: Number.parseFloat(commission.commissionAmount.toString()),
			Stage: this.mapCommissionStatusToStage(commission.status ?? ""),
			Closing_Date: commission.settlementId
				? new Date(commission.settlementId).toISOString().split("T")[0]
				: undefined,
	 ...
	 */
	async updateCommissionDealStage(
		commissionId: string,
		status: string,
	): Promise<void> {
			...
			if (existingMapping) {
				...
				await db
					...
					.set({
						...
						parentZohoRecordId:
							prospectData.masterAgentZohoAccountId ||
							existingMapping.parentZohoRecordId,
	...
	async syncLoanLeadToCRM(loanData: {
		...
		if (loanData.processingMode === "EXTERNAL_FINANCIER") {
			if (loanData.financierName)
				descriptionParts.push(`Financier/Bank: ${loanData.financierName}`);
			if (loanData.bankerName)
				descriptionParts.push(`Banker Contact: ${loanData.bankerName}`);
			if (loanData.bankerMobile)
				descriptionParts.push(`Banker Mobile: ${loanData.bankerMobile}`);
			if (loanData.bankerEmail)
				descriptionParts.push(`Banker Email: ${loanData.bankerEmail}`);
		}
		...
		const leadData: ZohoCRMLead = {
			...
			Tag: [
				"FintekPro Loan Lead",
				loanData.loanType,
				loanData.processingMode === "EXTERNAL_FINANCIER"
					? "Bank Direct"
					: "Platform Processed",
			],
		...
		if (zohoRecordId) {
			...
			if (existingMapping) {
				await db
					...
					.set({
						...
						parentZohoRecordId:
							loanData.masterAgentZohoAccountId ||
							existingMapping.parentZohoRecordId,
			...
			console.log(
				`✅ [Zoho CRM] Loan lead ${loanData.applicationNumber} synced to Zoho Lead ${zohoRecordId} (banker: ${loanData.financierName || "N/A"})`,
			);
		}
	...
	async fetchAllContacts(
		...
		try {
			...
			console.error(
				"[Zoho CRM Import] Failed to fetch contacts:",
				error.message,
			);
	...
	async getImportPreview(): Promise<{
		...
		const [contactsCount, leadsCount] = await Promise.all([
			this.apiClient
				.get("/Contacts", { per_page: 1, page: 1 })
				.then((r) => r.data?.info?.count || 0)
				.catch(() => 0),
			this.apiClient
				.get("/Leads", { per_page: 1, page: 1 })
				.then((r) => r.data?.info?.count || 0)
				.catch(() => 0),
		]);
	...
	async importContactsAsProspects(options: {
		...
		if (!isProduction) {
			console.log(
				"[Zoho CRM Import] DRY RUN - Production import disabled in development",
			);
		}
		...
		while (hasMore) {
			const { contacts, hasMore: more } = await this.fetchAllContacts({
				page,
				limit: batchSize,
			});
			hasMore = more;
			...
			console.log(
				`[Zoho CRM Import] Page ${page - 1}: ${imported} imported, ${duplicates} duplicates, ${skipped} skipped`,
			);
		}
	...
	async importLeadsAsProspects(options: {
		...
		while (hasMore) {
			const { leads, hasMore: more } = await this.fetchAllLeads({
				page,
				limit: batchSize,
			});
			hasMore = more;
			...
			console.log(
				`[Zoho CRM Import] Leads Page ${page - 1}: ${imported} imported, ${duplicates} duplicates, ${skipped} skipped`,
			);
		}
