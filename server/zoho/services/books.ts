...
export class ZohoBooksService {
	...
	async findOrCreateContact(params: {
		...
		const existing = contacts.items.find(
			(c) =>
				c.contact_name.toLowerCase() === params.contact_name.toLowerCase() ||
				(params.email &&
	...
	async getOrCreateCOGSAccount(): Promise<{
		account_id: string;
		account_name: string;
	}> {
		...
		const cogsAccount = accounts.find(
			(acc) =>
				acc.account_name.toLowerCase().includes("cost of goods") ||
		...
		return {
			...
			account_name:
				...
				"Cost of Goods Sold - Bond Inventory",
		};
	}
	...
	async getDashboardSummary(): Promise<{
		...
		try {
			...
			const totalReceivables = unpaidInvoices.items.reduce(
			...
			const totalPayables = overdueBills.items.reduce(
				(sum, bill) => sum + (bill.balance || 0),
				0,
			);
			const overduePayables = overdueBills.items.reduce(
				(sum, bill) => sum + (bill.balance || 0),
				0,
			);
...
export async function getZohoBooksService(
	dataCenter: string = "in",
): Promise<ZohoBooksService | null> {
	try {
		...
		const organizationId =
			process.env.ZOHO_BOOKS_ORGANIZATION_ID || process.env.ZOHO_ZSOID || "";
