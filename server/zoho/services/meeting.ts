...
export class ZohoMeetingService {
	...
	private getBasePath(): string {
		if (!this.zsoid) {
			throw new Error(
				"ZSOID not configured - cannot make Meeting API requests",
			);
		}
	...
	async getMeetings(options?: {
		...
		if (options?.fromDate)
			params.from_date = options.fromDate.toISOString().split("T")[0];
		if (options?.toDate)
			params.to_date = options.toDate.toISOString().split("T")[0];

		const response = await this.client.get(
			`${this.getBasePath()}/sessions.json`,
			{ params },
		);
	...
	async getMeetingDetails(meetingKey: string): Promise<ZohoMeeting | null> {
		const response = await this.client.get(
			`${this.getBasePath()}/sessions/${meetingKey}.json`,
		);
	...
	async updateMeeting(
		...
		if (updates.startTime)
			payload.session.start_time = updates.startTime.toISOString();
	...
	async inviteParticipants(
		...
		const response = await this.client.post(
			`${this.getBasePath()}/sessions/${meetingKey}/participants.json`,
			{
	...
	async getMeetingParticipants(
		meetingKey: string,
	): Promise<ZohoMeetingParticipant[]> {
		const response = await this.client.get(
			`${this.getBasePath()}/sessions/${meetingKey}/participants.json`,
		);
	...
	async getMeetingRecording(meetingKey: string): Promise<string | null> {
		const response = await this.client.get(
			`${this.getBasePath()}/sessions/${meetingKey}/recording.json`,
		);
	...
	async getWebinarDetails(webinarKey: string): Promise<ZohoWebinar | null> {
		const response = await this.client.get(
			`${this.getBasePath()}/webinar/${webinarKey}.json`,
		);
		return response.data?.session || null;
	}
	...
	async registerForWebinar(
		webinarKey: string,
		registrant: ZohoWebinarRegistrant,
	): Promise<string> {
		const response = await this.client.post(
			`${this.getBasePath()}/webinar/${webinarKey}/registrants.json`,
			{
				registrant: {
	...
	async getWebinarAttendees(
		webinarKey: string,
	): Promise<ZohoWebinarRegistrant[]> {
		const response = await this.client.get(
			`${this.getBasePath()}/webinar/${webinarKey}/attendees.json`,
		);
...
export const createZohoMeetingService = (
	connectionId: string,
