...
export class ZohoSyncOrchestrator {
	...
	private acquireLock(holder: string): boolean {
		if (_syncLock) {
			console.log(
				`[SyncOrchestrator] Lock held by "${_syncLockHolder}", skipping "${holder}"`,
			);
			return false;
		}
		_syncLock = true;
	...
	async runFullSync(): Promise<FullSyncReport> {
		if (!isSyncEnabled()) {
			console.log(
				"[SyncOrchestrator] Full sync skipped - sync not enabled (set ZOHO_SYNC_ENABLED=true or run in production)",
			);
		...
		try {
			...
			console.log(
				`[SyncOrchestrator] Starting full sync at ${startedAt.toISOString()}`,
			);

			const webhookResult =
				await this.webhookProcessor.processPendingEvents(100);
			console.log(
				`[SyncOrchestrator] Webhook processing: ${webhookResult.succeeded} succeeded, ${webhookResult.failed} failed, ${webhookResult.deadLettered} dead-lettered`,
			);
			...
			for (const config of SYNC_MODULES) {
				...
				try {
					...
					console.log(
						`[SyncOrchestrator] ${config.zohoModule}: ${result.recordsProcessed} processed, ${result.recordsUpdated} updated, ${result.conflicts} conflicts, ${result.errors.length} errors`,
					);
				} catch (error: any) {
					console.error(
						`[SyncOrchestrator] ${config.zohoModule} sync failed:`,
						error.message,
					);
			...
			const report: FullSyncReport = {
				...
				summary: {
					totalProcessed: moduleResults.reduce(
						(s, r) => s + r.recordsProcessed,
						0,
					),
			...
			console.log(
				`[SyncOrchestrator] Full sync completed in ${report.totalDurationMs}ms`,
			);
			return report;
		} finally {
			this.releaseLock();
	...
	async runIncrementalSync(): Promise<FullSyncReport> {
		if (!isSyncEnabled()) {
			console.log(
				"[SyncOrchestrator] Incremental sync skipped - sync not enabled",
			);
			return this.emptyReport();
		}
		...
		try {
			const startedAt = new Date();
			console.log(
				`[SyncOrchestrator] Starting incremental sync at ${startedAt.toISOString()}`,
			);

			const webhookResult =
				await this.webhookProcessor.processPendingEvents(50);
			...
			for (const config of SYNC_MODULES) {
				...
				if (connectionInvalidated) {
					moduleResults.push(
						this.errorResult(config, "Connection invalidated"),
					);
					continue;
				}
				...
				try {
					...
					if (isConnectionGone) {
						...
						console.warn(
							`[SyncOrchestrator] Connection ${this.connectionId} OAuth token invalid — marking inactive to stop retry loops`,
						);
						try {
							await db
							...
							console.warn(
								"[SyncOrchestrator] Could not mark connection inactive:",
								dbErr.message,
							);
						}
						moduleResults.push(this.errorResult(config, error.message));
						...
						console.error(
							`[SyncOrchestrator] Incremental ${config.zohoModule} failed:`,
							error.message,
						);
						moduleResults.push(this.errorResult(config, error.message));
					}
	...
	private async syncModuleIncremental(
	...
	private async pullFromZoho(
		...
		while (hasMore && page <= MAX_PAGES) {
			try {
				...
				const response = await this.crmClient.get(
					`/${config.zohoModule}`,
					params,
				);
				...
				await db
					...
					.set({
						conflictData: {
							...
							resolution:
								config.conflictStrategy === "per_field"
									? "per_field_authority"
									: config.conflictStrategy,
						},
						updatedAt: new Date(),
	...
	private detectConflict(mapping: any, zohoRecord: any): boolean {
		...
		const zohoModified = zohoRecord.Modified_Time
			? new Date(zohoRecord.Modified_Time)
			: null;
		if (!zohoModified) return false;

		const lastSync = new Date(mapping.lastSyncedAt);
		const fintekproModified = mapping.updatedAt
			? new Date(mapping.updatedAt)
			: lastSync;

		const zohoChangedSinceSync = zohoModified.getTime() > lastSync.getTime();
		const fintekproChangedSinceSync =
			fintekproModified.getTime() > lastSync.getTime() + SAFETY_WINDOW_MS;
	...
	private resolveConflict(
		...
		return zohoModified.getTime() >= fintekproModified.getTime()
			? "zoho"
			: "fintekpro";
	}
	...
	private async applyPerFieldUpdate(
		...
		if (fintekproType === "prospect") {
			...
			if (zohoAllowed.has("email") && zohoRecord.Email)
				updateData.email = zohoRecord.Email.toLowerCase().trim();
			if (zohoAllowed.has("mobile") && (zohoRecord.Mobile || zohoRecord.Phone))
				updateData.mobile = zohoRecord.Mobile || zohoRecord.Phone;
			if (zohoAllowed.has("name")) {
				const name = [zohoRecord.First_Name, zohoRecord.Last_Name]
					.filter(Boolean)
					.join(" ")
					.trim();
			...
			if (Object.keys(updateData).length > 1) {
				await db
					...
					.set(updateData)
					.where(eq(prospectClients.id, fintekproId));
			}
		} else if (fintekproType === "user") {
			const updateData: any = {};
			if (zohoAllowed.has("email") && zohoRecord.Email)
				updateData.email = zohoRecord.Email.toLowerCase().trim();
			if (zohoAllowed.has("phone") && (zohoRecord.Mobile || zohoRecord.Phone))
				updateData.phone = zohoRecord.Mobile || zohoRecord.Phone;
			if (zohoAllowed.has("fullName")) {
				const fullName = [zohoRecord.First_Name, zohoRecord.Last_Name]
					.filter(Boolean)
					.join(" ")
					.trim();
			...
			if (zohoRecord.Amount && fintekproProtected.has("commissionAmount")) {
				...
				const currentAmt = Number.parseFloat(
					current?.commissionAmount?.toString() || "0",
				);
				const zohoAmt = Number.parseFloat(zohoRecord.Amount.toString());

				if (currentAmt !== zohoAmt) {
					hasFinancialConflict = true;
					await this.logFieldConflict(
						mapping,
						"commissionAmount",
						currentAmt,
						zohoAmt,
	...
	private async applyZohoUpdate(
		...
		if (fintekproType === "prospect") {
			...
			if (zohoRecord.Email)
				updateData.email = zohoRecord.Email.toLowerCase().trim();
			if (zohoRecord.Mobile || zohoRecord.Phone)
				updateData.mobile = zohoRecord.Mobile || zohoRecord.Phone;
			const name = [zohoRecord.First_Name, zohoRecord.Last_Name]
				.filter(Boolean)
				.join(" ")
	...
	private async logFieldConflict(
		...
		try {
			...
			console.warn(
				`[SyncOrchestrator] FIELD CONFLICT on ${mapping.fintekproEntityType}/${mapping.fintekproEntityId} "${field}": FintekPro=${fintekproValue}, Zoho=${zohoValue} => FintekPro wins (financial field protected)`,
			);
		} catch (e) {
	...
	private async logFullSync(report: FullSyncReport): Promise<void> {
		try {
			await db.insert(zohoSyncLogs).values({
				...
				recordsSucceeded:
					report.summary.totalCreated + report.summary.totalUpdated,
	...
	async runReconciliation(): Promise<{
		...
		const [prospectConflicts] = await db
			.select({ count: count() })
			.from(zohoEntityMappings)
			.where(
				and(
		...
		const [prospectLastSync] = await db
			.select({ lastSync: sql<Date>`max(${zohoEntityMappings.lastSyncedAt})` })
			.from(zohoEntityMappings)
			.where(
				and(
					eq(zohoEntityMappings.connectionId, this.connectionId),
		...
		const [commissionCount] = await db
			.select({ count: count() })
			.from(partnerCommissions);
		const [commissionMapped] = await db
			.select({ count: count() })
			.from(zohoEntityMappings)
			.where(
				and(
		...
		const [commissionConflicts] = await db
			.select({ count: count() })
			.from(zohoEntityMappings)
			.where(
				and(
		...
		const [commissionLastSync] = await db
			.select({ lastSync: sql<Date>`max(${zohoEntityMappings.lastSyncedAt})` })
			.from(zohoEntityMappings)
			.where(
				and(
		...
		modules.push({
			...
			unmappedCount:
				(commissionCount?.count || 0) - (commissionMapped?.count || 0),
			conflictCount: commissionConflicts?.count || 0,
			lastSync: commissionLastSync?.lastSync || null,
		});

		const financialDiscrepancies: Array<{
			entityId: string;
			field: string;
			fintekproValue: any;
			zohoValue: any;
		...
		const conflictMappings = await db
			.select()
			.from(zohoEntityMappings)
			.where(
				and(
