...
interface ZohoSignDocument {
  request_id?: string;
  request_name: string;
  request_status?: 'draft' | 'inprogress' | 'completed' | 'declined' | 'expired' | 'recalled';
  owner_email?: string;
  created_time?: string;
...
interface ZohoSignField {
  field_type: 'signature' | 'initials' | 'textfield' | 'date' | 'checkbox' | 'dropdown' | 'email' | 'name' | 'company';
  field_name: string;
...
export class ZohoSignService {
  ...
  async getDocuments(options?: {
    status?: 'draft' | 'inprogress' | 'completed' | 'declined' | 'expired' | 'recalled';
    page?: number;
    limit?: number;
  ...
  async getDocumentDetails(requestId: string): Promise<ZohoSignDocument | null> {
    const response = await this.client.get(`/api/v1/requests/${requestId}`);
    return response.data?.requests || null;
  }

  async createSignRequest(options: CreateSignRequestOptions): Promise<ZohoSignDocument> {
  ...
  async downloadSignedDocument(requestId: string): Promise<Buffer> {
    const response = await this.client.get(`/api/v1/requests/${requestId}/pdf`, {
  ...
  async getTemplateDetails(templateId: string): Promise<ZohoSignTemplate | null> {
  ...
  async createFromTemplate(templateId: string, options: {
    ...
    const response = await this.client.post(`/api/v1/templates/${templateId}/createdocument`, payload);
  ...
  async createKYCSignRequest(options: {
    clientName: string;
    clientEmail: string;
    documentType: 'kyc_form' | 'investment_agreement' | 'nominee_form' | 'fatca_declaration';
  ...
  async getCompletedKYCDocuments(clientEmail: string): Promise<ZohoSignDocument[]> {
    const allDocs = await this.getDocuments({ status: 'completed' });
    
    return allDocs.filter(doc => 
      doc.actions?.some(action => 
...
export const createZohoSignService = (connectionId: string, dataCenter: string = 'in') => {
