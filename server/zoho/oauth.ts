...
export class ZohoOAuthService {
	...
	async refreshAccessToken(refreshToken: string): Promise<ZohoTokenResponse> {
		try {
			...
			if (!response.data?.access_token) {
				console.error(
					"[Zoho OAuth] Invalid refresh response - no access_token:",
					response.data,
				);
				throw new Error(
					"Zoho returned invalid token response: " +
						JSON.stringify(response.data),
				);
			}
	...
	async revokeToken(token: string): Promise<void> {
		try {
			await axios.post(`${this.baseAuthUrl}/oauth/v2/token/revoke`, null, {
				params: {
					token,
				},
			});
	...
	async saveConnection(
		...
		const expiresInMs =
			tokenResponse.expires_in && typeof tokenResponse.expires_in === "number"
	...
	async getValidAccessToken(connectionId: string): Promise<string> {
		...
		try {
			...
			if (connection.refreshToken?.startsWith("1000.")) {
				console.log(
					"[Zoho OAuth] Using unencrypted refresh token (legacy format)",
				);
				decryptedRefreshToken = connection.refreshToken;
			} else {
				decryptedRefreshToken = encryptionService.decrypt(
					connection.refreshToken,
				);
			}
		} catch (decryptError: any) {
			console.error(
				`[Zoho OAuth] Decrypt Refresh Token Error for connection ${connectionId}:`,
			...
			if (decryptError instanceof DecryptionError) {
				console.warn(
					`[Zoho OAuth] Decryption failed for connection ${connectionId}. Marking as error.`,
				);
			...
			if (connection.refreshToken?.includes(".")) {
				...
				throw new Error(
					"Failed to decrypt Zoho refresh token and no fallback available. Re-authentication required.",
				);
			}
		}
		...
		if (now.getTime() + expiryBuffer >= expiresAt.getTime()) {
			...
			const isLegacyFormat =
				connection.accessToken?.startsWith("1000.") ||
				connection.refreshToken?.startsWith("1000.");

			let accessTokenToStore: string;
			if (isLegacyFormat) {
				console.log(
					"[Zoho OAuth] Storing new access token in legacy format (unencrypted)",
				);
				console.log(
					"[Zoho OAuth] Token starts with:",
		...
		try {
			if (connection.accessToken?.startsWith("1000.")) {
				console.log(
					"[Zoho OAuth] Using unencrypted access token (legacy format)",
				);
			...
			console.error(
				`[Zoho OAuth] Access token decrypt error for connection ${connectionId}:`,
				decryptError.message,
			);
			if (connection.accessToken?.includes(".")) {
				...
				throw new Error(
					"Failed to decrypt Zoho access token. Token refresh might be needed.",
				);
			}
		}
	...
	async forceRefreshToken(connectionId: string): Promise<string> {
		console.log(
			"[Zoho OAuth] Force refreshing token for connection:",
			connectionId,
		);
		...
		try {
			...
			console.error(
				`[Zoho OAuth] Force refresh decrypt error for connection ${connectionId}:`,
				decryptError.message,
			);
			if (connection.refreshToken?.includes(".")) {
				decryptedRefreshToken = connection.refreshToken;
				...
				throw new Error(
					"Failed to decrypt Zoho refresh token for force refresh.",
				);
			}
		}
