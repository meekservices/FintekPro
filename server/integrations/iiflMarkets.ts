...
export class IIFLMarketsAPI {
	...
	constructor(config: IIFLConfig) {
		...
		this.httpClient.interceptors.response.use(
			...
			(error: AxiosError) => {
				...
				throw new Error(
					`IIFL Markets API Error: ${JSON.stringify(errorMessage)}`,
				);
			},
		);
	}
	...
	async getUserSession(clientId: string, authCode: string): Promise<string> {
		...
		const response = await this.httpClient.post<IIFLResponse<void>>(
			"/getusersession",
			{
				checkSum,
			},
		...
		throw new Error(
			`Failed to get user session: ${response.data.message || "Unknown error"}`,
		);
	}
	...
	async getProfile(): Promise<IIFLUserProfile> {
		...
		const response =
			await this.httpClient.get<IIFLResponse<IIFLUserProfile>>("/profile");
	...
	async getLimits(): Promise<IIFLLimits> {
		...
		const response =
			await this.httpClient.get<IIFLResponse<IIFLLimits>>("/limits");
		...
		throw new Error(
			`Failed to get limits: ${response.data.message || "Unknown error"}`,
		);
	}
	...
	async logout(): Promise<void> {
		...
		const response =
			await this.httpClient.post<
				IIFLResponse<{ status: string; message: string }>
			>("/profile/logout");
		...
		if (response.data.status === "Ok") {
			...
			throw new Error(
				`Failed to logout: ${response.data.message || "Unknown error"}`,
			);
		}
	}
	 ...
	 */
	async getPreOrderMargin(
		request: IIFLMarginRequest,
	): Promise<IIFLPreOrderMargin> {
		this.ensureAuthenticated();

		const response = await this.httpClient.post<
			IIFLPreOrderMargin | IIFLResponse<IIFLPreOrderMargin>
		>("/preordermargin", {
			...
			...(request.slTriggerPrice && {
				slTriggerPrice: String(request.slTriggerPrice),
			}),
			...(request.slLegPrice && { slLegPrice: String(request.slLegPrice) }),
			...(request.targetLegPrice && {
				targetLegPrice: String(request.targetLegPrice),
			}),
		});
		...
		if (data.status === "Not_Ok") {
			throw new Error(
				`Failed to get pre-order margin: ${data.message || "Unknown error"}`,
			);
		}
		if (data.result) {
			return data.result;
	...
	async getSpanExposureMargin(
		...
		const response = await this.httpClient.post<
			IIFLSpanExposure | IIFLResponse<IIFLSpanExposure>
		>("/spanexposure", requests);

		// Handle both wrapped and direct response formats
		const data = response.data as any;
		if (data.status === "Not_Ok") {
			throw new Error(
				`Failed to get SPAN/exposure margin: ${data.message || "Unknown error"}`,
			);
		}
		if (data.result) {
			return data.result;
	 ...
	 */
	async placeOrder(
		orders: IIFLOrderRequest | IIFLOrderRequest[],
		...
		const payload = orderArray.map((order) => ({
			...
			...(order.slTriggerPrice && {
				slTriggerPrice: String(order.slTriggerPrice),
			}),
			...(order.slLegPrice && { slLegPrice: String(order.slLegPrice) }),
			...(order.targetLegPrice && {
				targetLegPrice: String(order.targetLegPrice),
			}),
			...(order.disclosedQuantity && {
				disclosedQuantity: String(order.disclosedQuantity),
			}),
			...(order.marketProtectionPercent && {
				marketProtectionPercent: String(order.marketProtectionPercent),
			}),
	...
	async modifyOrder(
		...
		if (modifications.quantity !== undefined)
			payload.quantity = String(modifications.quantity);
		if (modifications.price !== undefined)
			payload.price = String(modifications.price);
		if (modifications.orderType) payload.orderType = modifications.orderType;
		if (modifications.slTriggerPrice !== undefined)
			payload.slTriggerPrice = String(modifications.slTriggerPrice);
		if (modifications.disclosedQuantity !== undefined)
			payload.disclosedQuantity = String(modifications.disclosedQuantity);
		if (modifications.validity) payload.validity = modifications.validity;

		const response = await this.httpClient.put<IIFLResponse<IIFLOrderResult>>(
		...
		throw new Error(
			`Failed to modify order: ${response.data.message || "Unknown error"}`,
		);
	}
	...
	async cancelOrder(brokerOrderId: string): Promise<IIFLOrderResult> {
		...
		throw new Error(
			`Failed to cancel order: ${response.data.message || "Unknown error"}`,
		);
	}
	...
	async getOrderBook(): Promise<IIFLOrderBookEntry[]> {
		...
		const response =
			await this.httpClient.get<IIFLResponse<IIFLOrderBookEntry[]>>("/orders");
		...
		throw new Error(
			`Failed to get order book: ${response.data.message || "Unknown error"}`,
		);
	}
	...
	async getOrderHistory(brokerOrderId: string): Promise<IIFLOrderBookEntry[]> {
		...
		const response = await this.httpClient.get<
			IIFLOrderBookEntry[] | IIFLResponse<IIFLOrderBookEntry[]>
		>(`/orders/${brokerOrderId}`);
		...
		if (data.status === "Not_Ok") {
			throw new Error(
				`Failed to get order history: ${data.message || "Unknown error"}`,
			);
		}
		if (data.result) {
			return data.result || [];
	...
	async getTradeBook(): Promise<IIFLTradeBookEntry[]> {
		...
		const response = await this.httpClient.get<
			IIFLTradeBookEntry[] | IIFLResponse<IIFLTradeBookEntry[]>
		>("/trades");
		...
		if (data.status === "Not_Ok") {
			throw new Error(
				`Failed to get trade book: ${data.message || "Unknown error"}`,
			);
		}
		if (data.result) {
			return data.result || [];
	...
	async getHoldings(): Promise<IIFLHolding[]> {
		...
		const response = await this.httpClient.get<
			IIFLHolding[] | IIFLResponse<IIFLHolding[]>
		>("/holdings");
		...
		if (data.status === "Not_Ok") {
			throw new Error(
				`Failed to get holdings: ${data.message || "Unknown error"}`,
			);
		}
		if (data.result) {
			return data.result || [];
	...
	async getPositions(): Promise<IIFLPosition[]> {
		...
		const response = await this.httpClient.get<
			IIFLPosition[] | IIFLResponse<IIFLPosition[]>
		>("/positions");
	...
	async getHistoricalData(
		...
		const response = await this.httpClient.post<IIFLResponse<any[]>>(
		...
		if (response.data.status === "Ok" && response.data.result) {
			...
			return response.data.result.map((item: any) => ({
				...
				open: Number.parseFloat(item[1]) || item.open,
				high: Number.parseFloat(item[2]) || item.high,
				low: Number.parseFloat(item[3]) || item.low,
				close: Number.parseFloat(item[4]) || item.close,
				volume: Number.parseInt(item[5]) || item.volume,
			}));
		}

		throw new Error(
			`Failed to get historical data: ${response.data.message || "Unknown error"}`,
		);
	}
	...
	async getMarketQuotes(
		...
		const response = await this.httpClient.post<
			IIFLResponse<IIFLMarketQuote[]>
		>("/marketdata/marketquotes", instruments);
		...
		throw new Error(
			`Failed to get market quotes: ${response.data.message || "Unknown error"}`,
		);
	}
	...
	async getMarketDepth(
		...
		const response = await this.httpClient.post<IIFLResponse<IIFLMarketDepth>>(
		...
		throw new Error(
			`Failed to get market depth: ${response.data.message || "Unknown error"}`,
		);
	}
	...
	async getOpenInterest(
		...
		const response = await this.httpClient.post<IIFLResponse<IIFLOpenInterest>>(
		...
		throw new Error(
			`Failed to get open interest: ${response.data.message || "Unknown error"}`,
		);
	}
	...
	async searchInstruments(
		...
		const exchanges = exchange
			? [exchange]
			: (["NSEEQ", "BSEEQ", "NSEFO", "BSEFO"] as IIFLExchange[]);
	...
	async getInstrumentBySymbol(
		...
		try {
			...
			return (
				instruments.find(
					(inst) =>
						inst.tradingSymbol?.toUpperCase() === tradingSymbol.toUpperCase(),
				) || null
			);
		} catch {
			return null;
	...
	private ensureAuthenticated(): void {
		if (!this.userSession) {
			throw new Error(
				"Not authenticated. Please call getUserSession() first or setUserSession().",
			);
		}
	}
	...
	static mapOrderStatus(
		iiflStatus: string,
	): "open" | "complete" | "rejected" | "cancelled" | "pending" {
...
export function getIIFLMarketsAPI(): IIFLMarketsAPI {
	if (!iiflMarketsInstance) {
		...
		if (!appKey || !appSecret) {
			throw new Error(
				"IIFL Markets API credentials not configured. Please set IIFL_APP_KEY and IIFL_APP_SECRET environment variables.",
			);
		}
