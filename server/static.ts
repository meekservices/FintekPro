...
export function serveStatic(app: Express) {
	...
	logBootProgress(
		`Step 1.5: Initializing static file server (distPath: ${distPath})`,
	);
	...
	if (
		...
		console.log(
			`✅ [Static] Serving frontend assets from production path: ${distPath}`,
		);
		app.use(
		...
		console.warn(
			`⚠️ [Static] Production build not found at ${distPath}. Falling back to ${publicPath}`,
		);
		app.use(
		...
		console.error(
			`❌ [Static] No static asset directory found! Checked ${distPath} and ${publicPath}`,
		);
	}
}
	...
	app.use("*", (req, res) => {
		...
		if (fs.existsSync(indexPath)) {
			res.sendFile(indexPath, (err) => {
				if (err) {
					...
					if (!res.headersSent) {
						res
							.status(500)
							.send("Internal Server Error: Failed to serve frontend.");
					}
				}
			});
		} else {
			res
				.status(404)
				.send(
					`Frontend application index.html not found. Deployment appears incomplete. (Checked: ${indexPath})`,
				);
		}
	});
}
