...
async function runTests() {
	...
	try {
		...
		try {
			const result = await sandboxPANService.validatePANComprehensive(
				"ABCDE1234F",
				"John Doe",
			);
			console.log(
	...
	try {
		...
		// Check initial state
		const check1 = otpThrottleService.canRequestOTP(
			testSessionId,
			testUserId,
			testAadhaarHash,
		);
		...
		// Record attempts
		otpThrottleService.recordAttempt(
			testSessionId,
			testUserId,
			testAadhaarHash,
		);
		...
		const check2 = otpThrottleService.canRequestOTP(
			testSessionId,
			testUserId,
			testAadhaarHash,
		);
		...
		const check3 = otpThrottleService.canRequestOTP(
			testSessionId,
			testUserId,
			testAadhaarHash,
		);
	...
	try {
		...
		console.log(
			"  ✓ Accreditation granted, expires:",
			investor.expiresAt.toISOString().split("T")[0],
		);
		...
		const accessResult = accreditedInvestorService.checkProductAccess(
			testUserId,
			"aif",
		);
	...
	try {
		...
		console.log(
			...
			fingerprint.fingerprintId.substring(0, 16) + "...",
		);
	...
	try {
		...
		console.log(
			"  ✓ New risk category:",
			version2.computedProfile.riskCategory,
		);
	...
	try {
		...
		console.log(
			"  ✓ Regulatory refs:",
			aadhaarConsent?.regulatoryReferences.join(", "),
		);
		...
		console.log(
			"  ✓ Consent recorded:",
			record?.consentId?.substring(0, 16) + "...",
		);

		// Verify consent
		const userConsents =
			pmlaConsentService.getUserConsents("consent-test-user");
