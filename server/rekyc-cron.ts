...
import { getUsersRequiringReminders, incrementReminderCount } from "./rekyc-service";
import { db } from "./db";
...
export async function processReKYCReminders(): Promise<{
  ...
  console.log(`[Re-KYC Cron] Found ${usersNeedingReminders.length} users requiring reminders`);
  ...
  for (const userInfo of usersNeedingReminders) {
    try {
      ...
      if (!user) {
        console.warn(`[Re-KYC Cron] User ${userInfo.userId} not found in users table`);
        errors++;
        continue;
      }
      ...
      console.error(`[Re-KYC Cron] Error processing reminder for user ${userInfo.userId}:`, error);
      errors++;
    }
  }
...
export function initReKYCCron() {
  ...
  cron.schedule("0 9 * * *", async () => {
    console.log("[Re-KYC Cron] Scheduled task triggered at", new Date().toISOString());
    await processReKYCReminders();
  });

  console.log("✅ Re-KYC reminder cron job initialized (runs daily at 9:00 AM)");
}
