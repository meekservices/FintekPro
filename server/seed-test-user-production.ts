...
async function generateUniqueUserId(prodDb: any, email: string): Promise<string> {
  const emailLocalPart = email.split("@")[0] || "";
  const alphabeticChars = emailLocalPart.replace(/[^a-zA-Z]/g, "").toUpperCase();
  const prefix = alphabeticChars.length >= 3 ? alphabeticChars.substring(0, 3) : "FTP";
...
async function seedProductionTestUser(): Promise<void> {
  ...
  try {
    ...
    if (existingUser.length > 0) {
      ...
      console.log("✅ Test user already exists — password reset and roles updated in PRODUCTION");
    } else {
    ...
    console.error("❌ Error seeding production test user:", error instanceof Error ? error.message : error);
  } finally {
    await pool.end();
  }
}
