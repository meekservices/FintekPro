...
} from "./services/credhive-service";
...
export function registerMarketingRoutes(app: Express) {
	...
	app.get(
		...
		async (req: Request, res: Response) => {
			try {
				...
				if (type)
					conditions.push(eq(marketingCampaigns.campaignType, String(type)));
				if (status)
					conditions.push(eq(marketingCampaigns.status, String(status)));
	 ...
	 */
	app.post(
		"/api/admin/marketing/campaigns/:id/send-email",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
				const [campaign] = await db
				...
				if (!zohoCampaignKey) {
					return apiResponse.serverError(
						res,
						"Failed to create campaign in Zoho",
					);
				}
	 ...
	 */
	app.post(
		"/api/admin/marketing/campaigns/:id/send-whatsapp",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
				const [campaign] = await db
				...
				if (campaign.campaignType !== "whatsapp") {
					return apiResponse.badRequest(
						res,
						"Campaign is not a WhatsApp campaign",
					);
				}
				...
				const { irisSent, twilioSent, failed } =
					await whatsappDispatcher.sendBulk(dispatchList);
	 ...
	 */
	app.post(
		"/api/admin/marketing/campaigns/:id/sync-analytics",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
				const [campaign] = await db
				...
				if (campaign.campaignType === "email" && campaign.zohoCampaignId) {
					...
					if (stats) {
						await db
							...
							.set({
								sentCount:
									stats.sentCount != null ? Number(stats.sentCount) : null,
								deliveredCount:
									stats.deliveredCount != null
										? Number(stats.deliveredCount)
										: null,
								openedCount:
									stats.openedCount != null ? Number(stats.openedCount) : null,
								clickedCount:
									stats.clickedCount != null
										? Number(stats.clickedCount)
										: null,
								bouncedCount:
									stats.bouncedCount != null
										? Number(stats.bouncedCount)
										: null,
								unsubscribedCount:
									stats.unsubscribedCount != null
										? Number(stats.unsubscribedCount)
										: null,
								updatedAt: new Date(),
							})
							.where(eq(marketingCampaigns.id, req.params.id));
	 ...
	 */
	app.get(
		"/api/admin/marketing/campaigns/:id/recipients",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
				const recipients = await db
	 ...
	 */
	app.post(
		"/api/admin/marketing/leads/search",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
				const { minRevenue, minProfit, credhiveScore, minEbitda, riskLevel } =
					req.body;

				const hasFinancialFilters =
					minRevenue || minProfit || credhiveScore || minEbitda || riskLevel;
				...
				if (!result.available) {
					...
					if (nameStartsWith) {
						conditions.push(
							ilike(prospectLeads.companyName, `${nameStartsWith}%`),
						);
					}
					if (city) {
					...
					if (minRevenue) {
						conditions.push(
							gte(prospectLeads.annualRevenue, minRevenue.toString()),
						);
					}
					...
					return res.json({
						companies: localLeads.map((l) => ({
							...l,
							authorizedCapital: l.authorizedCapital
								? Number.parseFloat(l.authorizedCapital)
								: null,
							paidUpCapital: l.paidUpCapital
								? Number.parseFloat(l.paidUpCapital)
								: null,
						})),
						...
						error: result.error,
						fallbackMessage:
							"Showing results from local database. Credhive API is unavailable.",
					});
				}

				const normalizedCompanies = result.companies.map((c: any) =>
					normalizeCompanyResult(c),
				);
				...
				return res.json({
					...
					fallbackMessage:
						"Credhive API is currently unavailable. You can still create B2B leads manually in the Prospect Dashboard or import from Zoho CRM.",
				});
			}
		},
	);
	 ...
	 */
	app.post(
		"/api/admin/marketing/leads/director-search",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
				...
				if (!directorName || directorName.trim().length < 3) {
					return apiResponse.badRequest(
						res,
						"Director name must be at least 3 characters",
					);
				}

				const result = await credhiveService.searchDirectorsByName(
					directorName.trim(),
					{ page, limit },
				);
				...
				for (let i = 0; i < allCINs.length; i += BATCH_SIZE) {
					...
					await Promise.all(
						batch.map(async (cin) => {
							const enriched =
								await credhiveService.enrichDirectorCompanyData(cin);
							if (enriched) {
				...
				const enrichmentRate =
					allCINs.length > 0
						...
						: "0";
				console.log(
					`📊 Director search enrichment: ${enrichedCompanyMap.size}/${allCINs.length} (${enrichmentRate}%) companies enriched`,
				);

				// Merge enriched data back into director results
				const enrichedDirectors = result.directors.map((director) => ({
					...director,
					companies: director.companies.map(
	 ...
	 */
	app.get(
		"/api/admin/marketing/leads/company/:cin",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
				const companyRes = await credhiveService.getCompanyDetails(
					req.params.cin,
				);
	 ...
	 */
	app.post(
		"/api/admin/marketing/leads/enrich-preview",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
				...
				const enrichedData = credhiveService.extractEnrichmentData(
					enrichment,
				...
				const response = {
					...
					paidUpCapital:
						enrichedData.paidUpCapital || company?.paidUpCapital || null,
					authorizedCapital:
					...
					enrichmentSources:
						((enrichment as Record<string, unknown>)
							.enrichmentSources as unknown[]) ?? [],
					apiAccessIssues: enrichedData.apiAccessIssues || [],
					isEnriched: true,
				};

				console.log(
					`✅ Enrichment preview complete for ${cin}: score ${response.enrichmentScore}%`,
				);
				res.json(response);
			} catch (error) {
	 ...
	 */
	app.post(
		"/api/admin/marketing/leads/import",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
				const { cin, companyName: requestCompanyName } = req.body;
				...
				if (!finalCompanyName) {
					return apiResponse.badRequest(
						res,
						"Company name is required. Please try again.",
					);
				}

				// Calculate lead score with enrichment bonus
				let leadScore = company
					? credhiveService.calculateLeadScore(company)
					: 10;
				leadScore = Math.min(
					100,
					leadScore + Math.floor(Number(enrichedData.enrichmentScore ?? 0) / 5),
				);
				...
				const [lead] = await db
					...
					.values({
						...
						pincode: company?.pincode || null,
						paidUpCapital:
							company?.paidUpCapital?.toString() ||
							enrichedData.paidUpCapital?.toString() ||
							null,
						authorizedCapital:
							company?.authorizedCapital?.toString() ||
							enrichedData.authorizedCapital?.toString() ||
							null,
						annualRevenue:
							company?.financials?.[0]?.revenue?.toString() || null,
						netProfit: company?.financials?.[0]?.netProfit?.toString() || null,
						ebitda: company?.financials?.[0]?.ebitda?.toString() || null,
						totalAssets:
							company?.financials?.[0]?.totalAssets?.toString() || null,
						debtToEquityRatio:
							company?.financials?.[0]?.debtToEquityRatio?.toString() || null,
						currentRatio:
							company?.financials?.[0]?.currentRatio?.toString() || null,
						roe: company?.financials?.[0]?.roe?.toString() || null,
						probe42Score: null,
						directors: (enrichedData.directors as any)?.length
							? (enrichedData.directors as unknown[])
							: (company?.directors as unknown[] | undefined),
						authorizedSignatories: company?.authorizedSignatories as
							| unknown[]
							| undefined,
						leadScore,
						leadQuality,
						...
						openChargesCount: enrichedData.openChargesCount || null,
						totalChargesAmount:
							enrichedData.totalChargesAmount?.toString() || null,
						chargeHolders:
							(enrichedData.chargeHolders as unknown[] | undefined) ?? null,
						suitFiledCasesCount:
							(enrichedData.suitFiledCases as number | null) ?? null,
						activeLegalCases:
							(enrichedData.activeLegalCases as number | null) ?? null,
						riskIndicators:
							(enrichedData.riskIndicators as unknown[] | undefined) ?? null,
						enrichmentScore: enrichedData.enrichmentScore || null,
						enrichmentSources:
							((enrichment as Record<string, unknown>).enrichmentSources as
								| unknown[]
								| undefined) ?? null,
						enrichmentData: {
							...enrichment,
						...
						incorporationDate: company?.incorporationDate || null,
						companyType:
							company?.companyType || enrichedData.entityType || null,
						companyClass: company?.companyClass || null,
				...
				console.log(
					`✅ Lead imported with ${Array.isArray(sourcesArr) ? sourcesArr.length : 0}/10 data sources: ${cin}`,
				);
	...
	app.get(
		...
		async (req: Request, res: Response) => {
			try {
				...
				const query = db.select().from(prospectLeads);

				const conditions: SQL[] = [];
				if (status) conditions.push(eq(prospectLeads.status, String(status)));
				if (quality)
					conditions.push(eq(prospectLeads.leadQuality, String(quality)));
				if (assignedTo)
					conditions.push(eq(prospectLeads.assignedTo, String(assignedTo)));
	 ...
	 */
	app.post(
		"/api/admin/marketing/leads/:id/activities",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
				...
				await db
					...
					.set({
						...
						nextFollowUpAt: req.body.nextActionDate
							? new Date(req.body.nextActionDate)
							: null,
						updatedAt: new Date(),
					})
					.where(eq(prospectLeads.id, req.params.id));
	 ...
	 */
	app.post(
		"/api/admin/marketing/client-intelligence/verify/:userId",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
				const { cin } = req.body;
	 ...
	 */
	app.get(
		"/api/admin/marketing/client-intelligence/:userId",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
				const [intelligence] = await db
				...
				if (!intelligence) {
					return apiResponse.notFound(
						res,
						"No intelligence data found for this client",
					);
				}
				...
				return apiResponse.serverError(
					res,
					"Failed to fetch client intelligence",
				);
			}
		},
	);
	 ...
	 */
	app.get(
		"/api/admin/marketing/client-intelligence",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
				const { riskLevel } = req.query;

				const query = db.select().from(clientIntelligence).$dynamic();

				const results = riskLevel
					? await query.where(
							eq(clientIntelligence.riskLevel, String(riskLevel)),
						)
					: await query;
	 ...
	 */
	app.get(
		"/api/admin/marketing/dashboard/stats",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
				// Campaign stats
				...
				res.json({
					...
					performance: {
						...
						openRate:
							campaignPerformance[0]?.totalSent > 0
								? (
										((campaignPerformance[0]?.totalOpened || 0) /
											campaignPerformance[0].totalSent) *
										100
									).toFixed(2)
								: "0.00",
						clickRate:
							campaignPerformance[0]?.totalSent > 0
								? (
										((campaignPerformance[0]?.totalClicked || 0) /
											campaignPerformance[0].totalSent) *
										100
									).toFixed(2)
								: "0.00",
	 ...
	 */
	app.get(
		"/api/admin/marketing/dashboard/recent-activity",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
	 ...
	 */
	app.get(
		"/api/admin/marketing/sms/status",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
		...
		async (req: Request, res: Response) => {
			try {
				...
				if (productType && details) {
					result = await smsMarketingService.sendPromotionalSMS(
						mobile,
						productType,
						details,
					);
					...
					return apiResponse.badRequest(
						res,
						"Message or productType with details is required",
					);
				}
	...
	app.post(
		...
		async (req: Request, res: Response) => {
			try {
				...
				const result = await smsMarketingService.sendBulkSMS(
					recipients,
					messageTemplate,
					campaignId,
				);
	 ...
	 */
	app.post(
		"/api/admin/marketing/sms/campaign",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
	 ...
	 */
	app.get(
		"/api/admin/marketing/whatsapp/templates",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
	 ...
	 */
	app.post(
		"/api/admin/marketing/whatsapp/send",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
	 ...
	 */
	app.post(
		"/api/admin/marketing/whatsapp/ipo-alert",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
				...
				if (!mobile || !companyName || !openDate) {
					return apiResponse.badRequest(
						res,
						"Mobile, companyName, and openDate are required",
					);
				}
	 ...
	 */
	app.post(
		"/api/admin/marketing/whatsapp/promotion",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
				...
				if (!mobile || !offerTitle) {
					return apiResponse.badRequest(
						res,
						"Mobile and offerTitle are required",
					);
				}
	 ...
	 */
	app.post(
		"/api/admin/marketing/whatsapp/bulk",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
	 ...
	 */
	app.post(
		"/api/admin/marketing/consent/opt-out",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
	 ...
	 */
	app.post(
		"/api/admin/marketing/consent/opt-in",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
				...
				if (userId) {
					...
					updateResult = await db
						...
						.set({ marketingConsent: true })
						.where(
							sql`REPLACE(${users.mobile}, '+', '') LIKE ${"%" + cleaned.slice(-10)}`,
						)
						.returning();
				}
	 ...
	 */
	app.get(
		"/api/admin/marketing/audience/eligible",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
	 ...
	 */
	app.get(
		"/api/admin/marketing/audience/stats",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
	 ...
	 */
	app.get(
		"/api/admin/marketing/audience/all",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
				const {
	 ...
	 */
	app.post(
		"/api/admin/marketing/multi-channel/bulk",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
				...
				if (channels.whatsapp) {
					const whatsappRecipients = recipients.filter(
					...
					results.whatsapp = {
						total: whatsappRecipients.length,
						sent: whatsappSent,
				...
				if (channels.sms) {
					...
					const smsMessage =
						templateMessages[templateType] ||
						`${variables.custom_message || "Greetings from FintekPro!"}`;

					for (const recipient of smsRecipients) {
						try {
							const personalizedMessage = smsMessage.replace(
								/\{name\}/g,
								recipient.name || "Valued Customer",
							);
							await (
								smsMarketingService as unknown as {
									sendSMS: (to: string, msg: string) => Promise<void>;
								}
							).sendSMS(recipient.mobile, personalizedMessage);
							smsSent++;
						} catch (e) {
							smsFailed++;
						}
				...
				if (channels.email) {
					...
					const subject =
						templateSubjects[templateType] || "Greetings from FintekPro";
            ...
            `;
				...
				const totalSent =
					(results.whatsapp?.sent || 0) +
					(results.sms?.sent || 0) +
					(results.email?.sent || 0);
				const totalFailed =
					(results.whatsapp?.failed || 0) +
					(results.sms?.failed || 0) +
				...
				const templateDisplayName = templateType
					.replace(/_/g, " ")
				...
				const [savedCampaign] = await db
					...
					.values({
						...
						description:
							...
							`Bulk ${templateDisplayName} campaign`,
						campaignType: campaignType,
						status:
							totalFailed === 0 ? "sent" : totalSent > 0 ? "sent" : "failed",
				...
				console.log(
					`[Campaign] Saved campaign ${savedCampaign.id}: ${totalSent} sent, ${totalFailed} failed`,
				);
	 ...
	 */
	app.get(
		"/api/admin/marketing/whatsapp/status",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
		...
		async (req: Request, res: Response) => {
			try {
				...
				const hotLeads = allLeads.filter(
					(l) => l.status === "qualified" || (l.leadScore && l.leadScore >= 70),
				).length;
				...
				const totalOpened = allCampaigns.reduce(
					(sum, c) => sum + (c.openedCount || 0),
					0,
				);
				const totalClicked = allCampaigns.reduce(
	...
	app.get(
		...
		async (req: Request, res: Response) => {
			try {
				...
				return apiResponse.serverError(
					res,
					"Failed to get client intelligence",
				);
			}
		},
	);
	 ...
	 */
	app.post(
		"/api/admin/marketing/intelligence/sync-all",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
					...
					try {
						...
						if (existing.length === 0) {
							...
							await db.insert(clientIntelligence).values({
								...
								probe42Score: null,
								updatedAt: new Date(),
							} as Parameters<typeof db.insert>[0] extends { values: infer V }
								? V
								: never);
							syncedCount++;
						} else {
							...
							await db
								...
								.set({ synced: true, updatedAt: new Date() } as Parameters<
									typeof db.update
								>[0] extends { set: infer S }
									? S
									: never)
								.where(eq(clientIntelligence.userId, user.id));
							syncedCount++;
						...
						console.error(
							`Failed to sync intelligence for user ${user.id}:`,
							err,
						);
					}
				}
				...
				return apiResponse.serverError(
					res,
					"Failed to sync all client intelligence",
				);
			}
		},
	);
	 ...
	 */
	app.post(
		"/api/admin/marketing/intelligence/:userId/sync",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
				...
				res.json({
					success: true,
					message: "Client intelligence synced successfully",
				});
			} catch (error: any) {
				console.error("Error syncing client intelligence:", error);
				return apiResponse.serverError(
					res,
					"Failed to sync client intelligence",
				);
			}
		},
	);
	...
	async function getMarketingAnalytics(periodStr: string) {
		...
		return campaigns.map((c) => ({
			...
			revenue: c.clickedCount
				? `₹${((c.clickedCount || 0) * 250).toLocaleString()}`
				: undefined,
			recordedAt:
	...
	app.get(
		...
		async (req: Request, res: Response) => {
			try {
				...
				return apiResponse.serverError(
					res,
					"Failed to get marketing analytics",
				);
			}
		},
	);
	 ...
	 */
	app.get(
		"/api/admin/marketing/analytics/:period",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
	 ...
	 */
	app.get(
		"/api/admin/festival-marketing/campaigns",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
				const campaigns = await db
				...
				return apiResponse.serverError(
					res,
					"Failed to fetch festival campaigns",
				);
			}
		},
	);
	 ...
	 */
	app.post(
		"/api/admin/festival-marketing/send-bulk",
		requireAdmin,
		async (req: Request, res: Response) => {
			try {
				...
				> = {
					diwali: {
						name: "Diwali",
						message:
							"Wishing you a Happy Diwali! May this festival of lights bring joy, prosperity, and success to you and your family.",
					},
					holi: {
						name: "Holi",
						message:
							"Happy Holi! May your life be filled with vibrant colors of happiness, love, and prosperity.",
					},
					eid: {
						name: "Eid",
						message:
							"Eid Mubarak! Wishing you and your family a blessed celebration filled with peace and happiness.",
					},
					christmas: {
						name: "Christmas",
						message:
							"Merry Christmas! Wishing you joy, peace, and wonderful blessings this holiday season.",
					},
					"ganesh-chaturthi": {
						name: "Ganesh Chaturthi",
						message:
							"Happy Ganesh Chaturthi! May Lord Ganesha remove all obstacles and shower you with wisdom.",
					},
					"durga-puja": {
						name: "Durga Puja",
						message:
							"Happy Durga Puja! May Goddess Durga bless you with strength and prosperity.",
					},
					onam: {
						name: "Onam",
						message:
							"Happy Onam! Wishing you a harvest of happiness, health, and prosperity.",
					},
					pongal: {
						name: "Pongal",
						message:
							"Happy Pongal! May this harvest festival bring abundance and joy to you.",
					},
					"new-year": {
						name: "New Year",
						message:
							"Happy New Year! Wishing you a year filled with new hopes, joys, and success.",
					},
					ugadi: {
						name: "Ugadi",
						message:
							"Happy Ugadi! May this new year usher in new hopes and opportunities.",
					},
					vishu: {
						name: "Vishu",
						message:
							"Happy Vishu! Wishing you a golden year filled with happiness and prosperity.",
					},
					bihu: {
						name: "Bihu",
						message:
							"Happy Bihu! May this harvest festival bring you joy and new beginnings.",
					},
					baisakhi: {
						name: "Baisakhi",
						message:
							"Happy Baisakhi! May the spirit of Baisakhi bring you abundance and prosperity.",
					},
					lohri: {
						name: "Lohri",
						message:
							"Happy Lohri! May the warmth of Lohri bring love and happiness to your life.",
					},
					"makar-sankranti": {
						name: "Makar Sankranti",
						message:
							"Happy Makar Sankranti! May your life soar high with success like colorful kites.",
					},
					"raksha-bandhan": {
						name: "Raksha Bandhan",
						message:
							"Happy Raksha Bandhan! Celebrating the beautiful bond of love and protection.",
					},
					navratri: {
						name: "Navratri",
						message:
							"Happy Navratri! May the divine blessings bring you strength and prosperity.",
					},
				};
				...
				console.log(
					`📧 Festival Campaign Created: ${festival.name} - ${recipientCount} recipients via ${channel}`,
				);
				...
				return apiResponse.serverError(
					res,
					"Failed to send festival greetings",
				);
			}
		},
	);
	...
	app.get(
		...
		async (req: Request, res: Response) => {
			try {
				...
				const isMaskedPhone = (ph: string | null) =>
					!ph || ph.startsWith("+XXXX");
				...
				for (const contact of raw) {
					...
					const key =
						contact.phone && !isMaskedPhone(contact.phone)
							? `phone:${contact.phone}`
							: `email:${contact.email}`;
	 ...
	 */
	app.post(
		"/api/agent/marketing/upload-greeting-image",
		async (req: Request, res: Response) => {
			try {
				if (!req.user)
					return res.status(401).json({ error: "Authentication required" });
				...
				if (!imageBase64 || !festivalId) {
					return res
						.status(400)
						.json({ error: "imageBase64 and festivalId are required" });
				}
				...
				if (publicSearchPaths.length === 0) {
					return res
						.status(500)
						.json({ error: "Object storage not configured" });
				}
	...
	app.post(
		"/api/agent/marketing/send-greetings",
		async (req: Request, res: Response) => {
			try {
				if (!req.user) {
					return res.status(401).json({ error: "Authentication required" });
				...
				const { festivalId, clientIds, channel, customMessage, imageUrl } =
					req.body;

				if (!festivalId || !clientIds?.length) {
					return apiResponse.badRequest(
						res,
						"Festival ID and client IDs are required",
					);
				}
				...
				const [leadsRows, prospectsRows] = await Promise.all([
					...
					db
						.select()
						.from(prospectClients)
						.where(sql`${prospectClients.id} = ANY(${clientIds})`),
				]);
				...
				console.log(
					`📧 Agent ${req.user.id} sending ${festivalId} greetings to ${clientIds.length} clients via ${channel}`,
				);
				...
				if (channel === "email" && emailClients.length > 0) {
					try {
						...
						console.log(
							`✅ Zoho Campaigns: Sent ${festivalId} greetings via campaign ${zohoCampaignKey}`,
						);
					} catch (zohoError) {
						console.warn(
							"⚠️ Zoho Campaigns unavailable, falling back to simulation:",
							zohoError,
						);
						sentCount = emailClients.length;
					}
					...
					const greetingText =
						customMessage ||
						`${festivalData.emoji} Happy ${festivalData.name}! Warm wishes from ${agentName}.`;
				...
				res.json({
					...
					...(channel === "whatsapp"
						? { irisSent, twilioSent, failedCount }
						: {}),
					meta: {
	 ...
	 */
	app.patch(
		"/api/agent/marketing/contacts/:id/email",
		async (req: Request, res: Response) => {
			try {
				if (!req.user)
					return res.status(401).json({ error: "Authentication required" });
					...
					await db
						...
						.where(
							and(
								...
								eq(prospectClients.agentId, req.user.id),
							),
						);
				} else {
					await db
						...
						.where(
							and(
								eq(prospectLeads.id, id),
								eq(prospectLeads.assignedTo, req.user.id),
							),
						);
				}
	 ...
	 */
	app.get(
		"/api/agent/marketing/greeting-history",
		async (req: Request, res: Response) => {
			try {
				if (!req.user)
					return res.status(401).json({ error: "Authentication required" });
				...
				const history = rows.map((r) => {
					...
					const descMatch =
						r.description?.match(
							/^Sent (\S+) festival greetings to (\d+) .* via (\S+)/,
						) || [];
					return {
						id: r.id,
						festivalId: descMatch[1] || "unknown",
						clientCount: Number.parseInt(descMatch[2] || "0", 10),
	 ...
	 */
	app.post(
		"/api/agent/marketing/share-pick",
		async (req: Request, res: Response) => {
			try {
				if (!req.user)
					return res.status(401).json({ error: "Authentication required" });
				...
				if (!pickId || !clientIds?.length || !channel) {
					return apiResponse.badRequest(
						res,
						"pickId, clientIds and channel are required",
					);
				}
				...
				const [pick] = await db
					.select()
					.from(dailyPicks)
					.where(eqOp(dailyPicks.id, Number.parseInt(pickId, 10)))
					.limit(1);
				...
				if (contacts.length === 0)
					return apiResponse.badRequest(res, "No valid contacts found");

				const direction =
					pick.recoPrice && pick.targetPrice
				...
				const upside =
					pick.recoPrice && pick.targetPrice
						? (
								((Number(pick.targetPrice) - Number(pick.recoPrice)) /
									Number(pick.recoPrice)) *
								100
							).toFixed(1)
						: null;
				...
				if (channel === "whatsapp") {
					...
					const text = [
						...
						pick.rationale
							? `Rationale: ${pick.rationale.slice(0, 200)}`
							: null,
	...
	function getFestivalData(festivalId: string): FestivalData {
		const festivals: Record<string, FestivalData> = {
			diwali: {
				...
				gradient: "linear-gradient(135deg, #1a0a2e 0%, #4a2c6a 100%)",
				primaryColor: "#ffd700",
			},
			holi: {
				name: "Holi",
				emoji: "🎨",
				message:
					"May your life be filled with vibrant colors of happiness, love, and prosperity",
				gradient: "linear-gradient(135deg, #ff6b6b 0%, #5f27cd 100%)",
				primaryColor: "#ffffff",
			},
			eid: {
				name: "Eid",
				emoji: "🌙",
				message:
					"Wishing you and your family a blessed Eid filled with peace, happiness, and prosperity",
			...
			christmas: {
				...
				emoji: "🎄",
				message:
					"Wishing you a Merry Christmas filled with love, joy, and wonderful blessings",
			...
			"ganesh-chaturthi": {
				...
				gradient: "linear-gradient(135deg, #ff5722 0%, #ffab40 100%)",
				primaryColor: "#ffffff",
			},
			"durga-puja": {
				name: "Durga Puja",
				emoji: "🪷",
				message:
					"May Goddess Durga bless you with strength, courage, and happiness",
				gradient: "linear-gradient(135deg, #d32f2f 0%, #ffc107 100%)",
				primaryColor: "#ffffff",
			},
			onam: {
				name: "Onam",
				emoji: "🌸",
				message:
					"Wishing you a harvest of happiness, health, and prosperity this Onam",
				gradient: "linear-gradient(135deg, #f57c00 0%, #ffc107 100%)",
				primaryColor: "#ffffff",
			},
			pongal: {
				name: "Pongal",
				emoji: "🌾",
				message:
					"May this Pongal bring abundant harvest of happiness and prosperity to you",
			...
			"new-year": {
				...
				emoji: "🎆",
				message:
					"Wishing you a year filled with new hopes, dreams, and wonderful opportunities",
				gradient: "linear-gradient(135deg, #1a237e 0%, #7b1fa2 100%)",
				primaryColor: "#ffd700",
			},
			"independence-day": {
				name: "Independence Day",
				emoji: "🇮🇳",
				message: "Wishing you a Happy Independence Day! Jai Hind",
				gradient:
			...
			"republic-day": {
				name: "Republic Day",
				emoji: "🇮🇳",
				message: "Wishing you a Happy Republic Day! Celebrate our Constitution",
				gradient:
					"linear-gradient(135deg, #ff9933 0%, #ffffff 50%, #138808 100%)",
				primaryColor: "#000080",
			},
...
<body style="margin: 0; padding: 0; font-family: Arial, sans-serif;">
  <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
		...
		async (req: Request, res: Response) => {
			try {
				...
				if (!templateType && !customMessage) {
					return apiResponse.badRequest(
						res,
						"Template type or custom message is required",
					);
				}
	...
	smsMarketingService.isAvailable().then((smsAvailable) => {
		console.log(
			"   📱 SMS Marketing: " + (smsAvailable ? "Active" : "Not configured"),
		);
	});
	console.log(
		"   💬 WhatsApp Marketing: " +
			(whatsAppMarketingService.isAvailable() ? "Active" : "Not configured"),
	);
}
