...
export async function analyzeApiPerformance(
...
export async function generateReplitAgentInstructions(
...
export async function analyzeCodeErrors(
	codeErrors: string,
	filePath: string,
): Promise<any> {
...
export async function callGemini<T = Record<string, unknown>>(
	...
	for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
		try {
			...
			console.warn(
				`[CopilotAI] Attempt ${attempt + 1} failed — retrying in ${200 * 2 ** attempt}ms`,
				lastError.message,
			);
...
export async function analyzePortfolio(portfolioData: {
	...
	const gainLossPct =
		portfolioData.gainLossPercent ?? portfolioData.gainLoss ?? 0;
	...
	for (const h of holdings) {
		const w =
			h.weight ?? (totalValue > 0 ? ((h.value ?? 0) / totalValue) * 100 : 0);
	...
	try {
		const prompt = `Act as a senior SEBI-registered Investment Adviser. Analyze the following portfolio (Indian market context).\n\nPortfolio:\n- Total Value: ₹${totalValue.toLocaleString("en-IN")}\n- Returns: ${gainStr}\n- Asset Mix: ${equityWeight.toFixed(1)}% Equity, ${debtWeight.toFixed(1)}% Debt, ${cashWeight.toFixed(1)}% Cash\n- Top Holdings: ${holdings
			.slice(0, 5)
			.map((h) => `${h.name} (${h.weight?.toFixed(1)}%)`)
			.join(
				", ",
			...
			[
				{
					role: "system",
					content:
						"You are a senior portfolio strategist specialising in technical analysis and regulatory compliance.",
				},
		...
		console.error(
			"[GeminiService] Portfolio analysis AI call failed — falling back to local calculation:",
			error,
		);
		const recommendations: string[] = [];
		if (equityWeight > 80)
			recommendations.push(
				"Equity > 80% — rebalance into debt/liquid instruments.",
			);
		if (equityWeight < 20 && equityWeight > 0)
			recommendations.push(
				"Conservative equity — consider index fund SIPs for long-term growth.",
			);
		if (debtWeight < 15)
			recommendations.push(
				"Low debt — add quality debt/liquid funds as portfolio cushion.",
			);
		if (gainLossPct < -15)
			recommendations.push(
				"Significant drawdown — review fundamental quality of core holdings.",
			);
		if (holdingCount < 5 && holdingCount > 0)
			recommendations.push(
				"Concentrated portfolio — diversify across sectors and asset classes.",
			);
		if (cashWeight > 30)
			recommendations.push(
				"High cash — stagger deployment into equity or debt based on valuations.",
			);
		while (recommendations.length < 3) {
			recommendations.push(
				"Maintain strategic allocation; review performance quarterly.",
			);
		}
		return {
			analysis: localAnalysis + " [Fallback]",
 ...
 */
export async function generateInvestmentStory(
	stockSymbol: string,
	priceData: unknown,
): Promise<string> {
