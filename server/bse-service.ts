...
class BSEService {
	...
	async getAllSchemes(): Promise<
		{ schemeCode: string; schemeName: string; isin?: string }[]
	> {
		return this.getCachedData(
				...
				try {
					...
					if (response.data && Array.isArray(response.data)) {
						return response.data.map((fund: any) => ({
							schemeCode:
								fund.scheme_code || fund.schemeCode || String(fund.id),
			...
			24 * 60 * 60 * 1000,
		); // 24 hour cache for schemes list
	}

	/**
	 * Get hardcoded popular scheme codes as fallback
	 */
	private getPopularSchemeCodes(): {
		schemeCode: string;
		schemeName: string;
		isin?: string;
	}[] {
		...
		return this.getCachedData(`fund_${schemeCode}`, async () => {
			try {
				...
				return {
					...
					isin: fundInfo.isin || "",
					currentNav: Number.parseFloat(latestNav.nav),
				...
				console.warn(
					`Primary BSE API failed for ${schemeCode}, trying fallback:`,
					error,
				);
	 ...
	 */
	private async getFundDataFallback(
		schemeCode: string,
	): Promise<BSEFundData | null> {
		try {
			...
			if (scheme) {
				return {
					...
					riskLevel: this.categorizeRiskLevel(
						this.inferCategory(scheme.schemeName),
					),
					rating: 4,
					exitLoad: "1% if redeemed within 365 days",
				};
	 ...
	 */
	private async fetchNavFromAMFIFallback(
		schemeCode: string,
	): Promise<BSEHistoricalNav[]> {
		try {
			...
			return data.data
				...
				.map((item: any) => ({
					...
					nav: Number.parseFloat(item.nav) || 0,
				}))
	...
	async calculatePerformance(
		...
		const findNavForDate = (targetDate: Date, maxDaysBack: number = 7) => {
			return navHistory.find((nav) => {
				...
				const daysDiff =
					Math.abs(targetDate.getTime() - navDate.getTime()) /
					(1000 * 60 * 60 * 24);
	...
	async getFundPerformance(
		schemeCode: string,
	): Promise<BSEFundPerformance | null> {
		try {
		...
		for (let i = 0; i < popularSchemes.length; i += batchSize) {
			...
			const batchPromises = batch.map(async (scheme) => {
				try {
					...
					console.warn(
						`Failed to fetch performance for ${scheme.schemeCode}:`,
						error,
					);
					return null;
				}
	...
	async searchFunds(query: string): Promise<BSEFundSearch> {
		...
		try {
			...
			const validResults = results
				.filter(
					(result): result is PromiseFulfilledResult<BSEFundPerformance> =>
	...
	private categorizeRiskLevel(
		...
		if (
			normalizedCategory.includes("large cap") ||
			normalizedCategory.includes("bluechip")
		) {
			return "Moderate";
		}
		if (
			normalizedCategory.includes("mid cap") ||
			normalizedCategory.includes("balanced")
		) {
			return "High";
		}
		return "Very High";
	...
	private inferCategory(schemeName: string): string {
		...
		if (name.includes("flexi") || name.includes("multi cap")) {
			return "Multi Cap Fund";
		}
		if (name.includes("debt") || name.includes("bond")) {
			return "Debt Fund";
		}
		if (name.includes("tax") || name.includes("elss")) {
			return "ELSS";
		}
	...
	private normalizeCategoryName(category: string): string {
		...
		if (normalized.includes("multi cap") || normalized.includes("flexi")) {
