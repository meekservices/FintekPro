import { db } from "./db";
import { bondOrders, fixedIncomeSettlements, bondHoldings } from "@shared/schema";
 ...
 */
export function calculateSettlementDate(orderDate: Date, cycle: 'T+0' | 'T+1' | 'T+2'): Date {
...
export async function createBondOrder(params: {
  ...
  const expectedSettlementDate = calculateSettlementDate(orderDate, settlementCycle);
  ...
  console.log(`[Bond Order] Created order ${orderNumber} - Settlement ${settlementCycle} expected on ${settlementDateStr}`);
...
export async function updateOrderStatus(
  ...
  console.log(`[Bond Order] ${orderId} status updated to: ${status} - ${message}`);
}

/**
 * Process payment confirmation and move to settlement
 */
export async function confirmPayment(orderId: string, paymentReference: string): Promise<void> {
  await updateOrderStatus(orderId, 'confirmed', 'Payment confirmed, awaiting settlement', {
    paymentReference,
...
export async function creditToDemat(
  ...
  await updateOrderStatus(orderId, 'credited', 'Units credited to Demat account', {
    ...creditDetails,
    creditedAt: new Date()
  });
  ...
  if (order) {
    const purchaseDateStr = order.orderDate ? new Date(order.orderDate).toISOString().split('T')[0] : now.toISOString().split('T')[0];
    const maturityDateStr = order.settlementDate || new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
...
export async function getOrderLifecycle(orderId: string): Promise<{
  ...
  const [order] = await db.select().from(bondOrders).where(eq(bondOrders.id, orderId));
  const [settlement] = await db.select().from(fixedIncomeSettlements).where(eq(fixedIncomeSettlements.orderId, orderId));
  ...
  if (order) {
    ...
    if (order.orderStatus === 'confirmed' || order.orderStatus === 'settlement' || 
        order.orderStatus === 'allotted' || order.orderStatus === 'credited') {
      timeline.push({
        status: 'confirmed',
    ...
    if (order.orderStatus === 'settlement' || order.orderStatus === 'allotted' || order.orderStatus === 'credited') {
      timeline.push({
        status: 'settlement',
        timestamp: order.lastUpdated || new Date(),
...
export async function processSettlements(): Promise<void> {
  ...
  console.log(`[Settlement Cron] Processing ${pendingSettlements.length} pending settlements`);

  for (const settlement of pendingSettlements) {
    try {
      const quantity = settlement.quantity || 0;
      const settlementValue = parseFloat(settlement.settlementValue || '0');
      ...
      console.log(`[Settlement Cron] Completed settlement for order ${settlement.orderId}`);
    } catch (error) {
      console.error(`[Settlement Cron] Error processing settlement ${settlement.id}:`, error);
    }
  }
}
