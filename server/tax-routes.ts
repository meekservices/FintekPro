...
function getUserTaxRole(req: Request): TaxRole {
	...
	if (sessionRole === "ca" || sessionRole === "chartered_accountant")
		return "ca";
	...
	if (reqUser) {
		const userRoles: string[] =
			reqUser.roles || (reqUser.role ? [reqUser.role] : []);
...
function logTaxAudit(
	entry: Omit<TaxAuditEntry, "id" | "timestamp" | "hash" | "previousHash">,
) {
	...
	console.log(
		`[TAX AUDIT] ${auditEntry.action} on ${auditEntry.entityType}:${auditEntry.entityId} by user ${auditEntry.userId} (${auditEntry.userRole})`,
	);
	return auditEntry;
}
...
function shouldResetApprovalOnEdit(currentStatus: string): boolean {
	return ["ca_approved", "approved", "signed", "submitted", "filed"].includes(
		currentStatus,
	);
}

function handleImmutableEdit(
	draft: any,
	userId: number,
	userRole: TaxRole,
): { allowed: boolean; resetTo?: string; warning?: string } {
	...
	if (status === "paid") {
		logTaxAudit({
			...
			userRole,
			changesDescription:
				"Payment status reset due to draft modification after payment",
		});
		return {
			allowed: true,
			resetTo: "draft",
			warning:
				"Draft modified after payment. Payment has been voided and you will need to pay again.",
		};
	}
	...
	if (status === "preview") {
		...
		return {
			...
			resetTo: "draft",
			warning:
				"Draft modified after preview. You will need to preview and lock again.",
		};
	}
	...
	if (["ca_approved", "approved"].includes(status)) {
		logTaxAudit({
			...
			userRole,
			changesDescription:
				"CA approval reset due to draft modification after approval",
		});
		return {
			allowed: true,
			resetTo: "pending_review",
			warning:
				"Draft modified after CA approval. The return will need to be re-approved.",
		};
	}
...
function determinePANType(pan: string): { type: PANType; description: string } {
	...
	switch (fourthChar) {
		...
		case "J":
			return {
				type: "artificial_juridical_person",
...
function isNRI(userProfile: any): boolean {
	...
	return (
		...
		(userProfile.countryOfResidence &&
			userProfile.countryOfResidence !== "India")
	);
}
...
router.get("/pan-context", async (req: Request, res: Response) => {
	try {
		...
		if (
			session?.panContext &&
			Date.now() - session.panContext.timestamp < 300000
		) {
		...
		const panContext: PANContext = {
			...
			entityDescription:
				finalType === "nri" ? "Non-Resident Indian (NRI)" : description,
		};
...
function getEligibleITRForms(
	panType: PANType,
): Array<{ form: string; name: string; description: string }> {
	switch (panType) {
		case "individual":
			return [
				{
				...
				},
				{
					...
					description: "Capital gains, multiple properties, foreign income",
				},
				{
					form: "ITR-3",
				...
				},
				{
					form: "ITR-4",
					name: "Sugam",
					description: "Presumptive income for small businesses",
				},
			];
		case "nri":
			return [
				...
				},
				{
					form: "ITR-3",
					name: "Business/Profession",
					description: "Income from business or profession in India",
				},
			];
		case "huf":
			return [
				{
					form: "ITR-2",
					name: "For HUFs",
					description: "Capital gains, multiple properties",
				},
				{
					form: "ITR-3",
					name: "Business/Profession",
			...
			return [
				{
					form: "ITR-5",
					name: "For Firms/LLPs",
					description: "Partnership firms, LLPs, AOPs, BOIs",
				},
			];
		case "company":
			return [
				{
					form: "ITR-6",
					name: "For Companies",
					description: "Companies other than those claiming exemption u/s 11",
				},
			];
		case "trust":
		case "aop":
			...
			return [
				{
					form: "ITR-5",
					name: "For AOPs/BOIs",
					description: "Association of Persons, Body of Individuals",
				},
				{
					form: "ITR-7",
					name: "For Trusts",
		...
		try {
			if (!sandboxITRService.isConfigured()) {
				return res.status(503).json({
					...
					error: "SANDBOX_API_NOT_CONFIGURED",
					message:
						"Form 16 parsing requires Sandbox.co.in API. Not configured.",
				});
			}
			...
			if (!file || !file.buffer) {
				return res
					...
					.json({
						success: false,
						message: "No file uploaded. Please upload a Form 16 PDF or image.",
					});
			}

			const result = await sandboxITRService.parseForm16(
				file.buffer,
				file.originalname || "form16.pdf",
			);
			...
			if (result.success && result.data) {
				...
				res.json({
					success: false,
					message: result.message || "Could not parse Form 16",
				});
			}
		} catch (error) {
			...
			res.status(500).json({
				...
				error: "PARSE_FAILED",
				message:
					error instanceof Error ? error.message : "Form 16 parsing failed",
			});
		}
	},
);
...
router.post("/itr/eligible-form", async (req: Request, res: Response) => {
	try {
		...
		const result = sandboxITRService.getSuitableITRForm(
			incomeDetails,
		...
		res
			.status(500)
			.json({ success: false, error: "Form determination failed" });
	}
});

router.get(
	"/form26as/:pan/:assessmentYear",
	async (req: Request, res: Response) => {
		try {
			const reqUser = (req as any).user;
			const session = (req as any).session;
	...
	try {
		...
		if (draftData.businessDetails?.isPresumptive) {
			...
			if (bd.businessType === "business" && bd.grossTurnover > 30000000) {
				blockingErrors.push(
					"Section 44AD: Turnover exceeds ₹3 Cr limit. Presumptive taxation ineligible.",
				);
			}
			if (
				bd.businessType === "business" &&
				bd.presumptiveIncome44AD > 0 &&
				bd.grossTurnover > 0
			) {
				const minDeemed = Math.round(bd.grossTurnover * 0.06);
				if (bd.presumptiveIncome44AD < minDeemed) {
					blockingErrors.push(
						`Section 44AD: Deemed profit ₹${bd.presumptiveIncome44AD} is below minimum 6% of turnover (₹${minDeemed}).`,
					);
				}
			}
			if (bd.businessType === "profession" && bd.grossReceipts > 7500000) {
				blockingErrors.push(
					"Section 44ADA: Gross receipts exceed ₹75 lakhs limit. Presumptive taxation ineligible.",
				);
			}
			if (
				bd.businessType === "profession" &&
				bd.presumptiveIncome44ADA > 0 &&
				bd.grossReceipts > 0
			) {
				const minDeemed = Math.round(bd.grossReceipts * 0.5);
				if (bd.presumptiveIncome44ADA < minDeemed) {
					blockingErrors.push(
						`Section 44ADA: Deemed profit ₹${bd.presumptiveIncome44ADA} is below minimum 50% of receipts (₹${minDeemed}).`,
					);
				}
			}
			if (bd.businessType === "transport" && bd.vehicleCount > 10) {
				blockingErrors.push(
					"Section 44AE: Vehicle count exceeds 10. Presumptive taxation ineligible.",
				);
			}
			if (blockingErrors.length > 0) {
				...
				return res
					.status(422)
					.json({
						error: "Presumptive taxation validation failed",
		...
		const existingDraftKey = Array.from(itrDraftStorage.entries()).find(
			([_, d]) =>
				d.userId === userId &&
				d.pan === draftData.pan &&
				d.assessmentYear === draftData.assessmentYear,
		);
		...
		if (existingDraftKey) {
			...
			const immutabilityCheck = handleImmutableEdit(
				existingDraft,
				userId,
				userRole,
			);
...
router.get("/itr/drafts", async (req: Request, res: Response) => {
	try {
		...
		const userDrafts = Array.from(itrDraftStorage.values()).filter(
			(d) => d.userId === userId,
		);
...
router.get("/itr/draft/:id", async (req: Request, res: Response) => {
	try {
		...
		const draftId = Number.parseInt(req.params.id);
...
router.get("/notices", async (req: Request, res: Response) => {
	try {
		...
		const userNotices = Array.from(taxNoticesStorage.values()).filter(
			(n) => n.userId === userId,
		);
...
router.delete("/expert-cases/:id", async (req: Request, res: Response) => {
	try {
		...
		if (expertCase.status !== "draft" && expertCase.status !== "submitted") {
			return res
				.status(400)
				.json({ error: "Cannot delete case after it has been assigned" });
		}
...
router.post("/itr/payment", async (req: Request, res: Response) => {
	try {
		...
		const draft = itrDraftStorage.get(Number.parseInt(draftId));
		if (draft && draft.userId === userId) {
			itrDraftStorage.set(Number.parseInt(draftId), {
				...draft,
				status: "paid",
...
router.post("/itr/verify/send-otp", async (req: Request, res: Response) => {
	try {
		...
		verificationStorage.set(Number.parseInt(draftId), {
			method,
			referenceId,
...
router.post("/itr/verify/submit", async (req: Request, res: Response) => {
	try {
		...
		if (draft.status !== "paid" && draft.status !== "preview") {
			console.log(
				`Draft status: ${draft.status}, proceeding with verification for demo`,
			);
		}
		...
		logTaxAudit({
			entityType: "itr_draft",
			entityId: Number.parseInt(draftId),
			action: "ITR_FILED",
			previousStatus: draft.status,
			newStatus: "filed",
			userId,
		...
		verificationStorage.set(Number.parseInt(draftId), {
			...verificationStorage.get(Number.parseInt(draftId)),
			status: "verified",
			acknowledgementNumber,
			filingDate,
...
router.get(
	"/itr/verification-status/:draftId",
	async (req: Request, res: Response) => {
		try {
			const reqUser = (req as any).user;
			const session = (req as any).session;
			...
			const draftId = Number.parseInt(req.params.draftId);
...
router.get(
	"/audit-log",
	requirePermission("canViewAllCases"),
	async (req: Request, res: Response) => {
		try {
			const { entityType, entityId, limit = 100 } = req.query;

			let logs = Array.from(taxAuditLog.values()).sort(
				(a, b) =>
...
router.post(
	"/agent/cases/:caseId/action",
	requirePermission("canSubmitForReview"),
	async (req: Request, res: Response) => {
		try {
			const { caseId } = req.params;
			const { action, notes } = req.body;

			const draft = itrDraftStorage.get(Number.parseInt(caseId));
			...
			itrDraftStorage.set(Number.parseInt(caseId), {
				...draft,
				status: newStatus,
				agentNotes: notes,
				updatedAt: new Date().toISOString(),
...
router.post("/itr/questionnaire-log", async (req: Request, res: Response) => {
	try {
		...
		res
			.status(500)
			.json({ success: false, error: "Questionnaire logging failed" });
	}
});
	...
	try {
		if (!sandboxITRService.isConfigured()) {
			return res.status(503).json({
				...
				error: "SANDBOX_API_NOT_CONFIGURED",
				message:
					"Tax regime comparison requires Sandbox.co.in API. Not configured.",
			});
		}
		...
		const [oldRegimeResult, newRegimeResult] = await Promise.all([
			sandboxITRService.calculateTaxFromWizard({
				...data,
				taxRegime: "old",
			} as any),
			sandboxITRService.calculateTaxFromWizard({
		...
		const oldTax =
			oldRegimeResult.data?.taxPayable ??
			oldRegimeResult.data?.taxLiability ??
			0;
		const newTax =
			newRegimeResult.data?.taxPayable ??
			newRegimeResult.data?.taxLiability ??
			0;
		...
		res.status(500).json({
			...
			message:
				error instanceof Error ? error.message : "Regime comparison failed",
		});
	}
});

router.get(
	"/itr/audit-trail/:entityId",
	async (req: Request, res: Response) => {
		try {
			const reqUser = (req as any).user;
			const session = (req as any).session;
			...
			res
				.status(500)
				.json({ success: false, error: "Failed to fetch audit trail" });
		}
	},
);
...
router.get("/itr/audit-verify", async (req: Request, res: Response) => {
	try {
		const entries = Array.from(taxAuditLog.values()).sort(
			(a, b) => a.id - b.id,
		);
...
router.post("/itr/e-verify", async (req: Request, res: Response) => {
	try {
		...
		if (!method || !acknowledgmentNumber) {
			return res
				.status(400)
				.json({ error: "method and acknowledgmentNumber are required" });
		}
		...
		const result = await sandboxITRService.eVerifyITR(
...
const BROKER_REGISTRY: BrokerInfo[] = [
	{
		...
		fileFormatHint: "Tax P&L report from Console > Reports > Tax P&L",
		assetTypes: ["equity", "fno", "commodity", "currency"],
		pdfType: "broker_zerodha",
		isSupported: true,
	},
	{
		id: "groww",
		...
		fileFormatHint: "P&L report from Groww Dashboard > Reports",
		assetTypes: ["equity", "mutual_fund", "fno"],
		pdfType: "broker_groww",
		isSupported: true,
	},
	{
		id: "angelone",
		...
		category: "stock_broker",
		supportedFormats: ["pdf", "xlsx"],
		fileFormatHint: "Tax P&L from Angel One > My Account > Tax Report",
		assetTypes: ["equity", "fno", "commodity"],
		pdfType: "broker_angelone",
		isSupported: true,
	},
	{
		id: "icici_direct",
		...
		category: "stock_broker",
		supportedFormats: ["pdf", "xlsx"],
		fileFormatHint: "Capital Gains Statement from ICICI Direct > Tax Center",
		assetTypes: ["equity", "mutual_fund", "fno"],
		pdfType: "broker_icici",
		isSupported: true,
	},
	{
		id: "hdfc_securities",
		...
		category: "stock_broker",
		supportedFormats: ["pdf", "xlsx"],
		fileFormatHint: "Capital Gains report from HDFC Securities > Reports",
		assetTypes: ["equity", "fno"],
		pdfType: "broker_hdfc",
		isSupported: true,
	},
	{
		...
		category: "stock_broker",
		supportedFormats: ["pdf", "xlsx"],
		fileFormatHint: "Tax statement from Kotak Securities > Reports > Tax P&L",
		assetTypes: ["equity", "fno"],
		pdfType: "broker_kotak",
		isSupported: true,
	},
	{
		id: "upstox",
		...
		category: "stock_broker",
		supportedFormats: ["pdf", "xlsx", "csv"],
		fileFormatHint: "Tax P&L from Upstox Pro > Reports > Tax Report",
		assetTypes: ["equity", "fno", "commodity"],
		pdfType: "broker_upstox",
		isSupported: true,
	},
	{
		id: "5paisa",
		...
		category: "stock_broker",
		supportedFormats: ["pdf", "xlsx"],
		fileFormatHint: "Capital Gains report from 5Paisa > Reports",
		assetTypes: ["equity", "fno"],
		pdfType: "broker_5paisa",
		isSupported: true,
	},
	{
		id: "motilal_oswal",
		...
		category: "stock_broker",
		supportedFormats: ["pdf", "xlsx"],
		fileFormatHint: "Tax report from Motilal Oswal > Reports > Capital Gains",
		assetTypes: ["equity", "fno"],
		pdfType: "broker_motilal",
		isSupported: true,
	},
	{
		id: "axis_direct",
		...
		category: "stock_broker",
		supportedFormats: ["pdf", "xlsx"],
		fileFormatHint: "Capital Gains Statement from Axis Direct > Reports",
		assetTypes: ["equity", "mutual_fund", "fno"],
		pdfType: "broker_axis",
		isSupported: true,
	},
	{
		...
		category: "stock_broker",
		supportedFormats: ["pdf", "xlsx"],
		fileFormatHint: "Tax P&L from IIFL > Reports > Tax Statement",
		assetTypes: ["equity", "fno"],
		pdfType: "broker_iifl",
		isSupported: true,
	},
	{
		...
		category: "stock_broker",
		supportedFormats: ["pdf", "xlsx"],
		fileFormatHint: "Capital Gains Statement from Sharekhan > Reports",
		assetTypes: ["equity", "mutual_fund", "fno"],
		pdfType: "broker_sharekhan",
		isSupported: true,
	},
	{
		id: "anand_rathi",
		...
		fileFormatHint: "Tax P&L report from Anand Rathi trade platform",
		assetTypes: ["equity", "fno"],
		pdfType: "broker_anand_rathi",
		isSupported: true,
	},
	{
		id: "fyers",
		...
		category: "stock_broker",
		supportedFormats: ["pdf", "xlsx", "csv"],
		fileFormatHint: "Tax P&L from Fyers > Reports > Tax Report",
		assetTypes: ["equity", "fno"],
		pdfType: "broker_fyers",
		isSupported: true,
	},
	{
		id: "geojit",
		...
		category: "stock_broker",
		supportedFormats: ["pdf", "xlsx"],
		fileFormatHint: "Capital Gains from Geojit > Back Office > Reports",
		assetTypes: ["equity", "fno"],
		pdfType: "broker_geojit",
		isSupported: true,
	},
	{
		id: "idbi_securities",
		name: "IDBI Securities",
		category: "stock_broker",
		supportedFormats: ["xlsx", "csv"],
		fileFormatHint: "Capital Gains report from IDBI Securities portal",
		assetTypes: ["equity"],
		pdfType: "broker_idbi",
		isSupported: true,
	},
	{
		...
		category: "stock_broker",
		supportedFormats: ["pdf", "xlsx"],
		fileFormatHint: "Tax report from SBI Securities > Reports > Capital Gains",
		assetTypes: ["equity", "fno"],
		pdfType: "broker_sbi",
		isSupported: true,
	},
	{
		...
		category: "stock_broker",
		supportedFormats: ["xlsx", "csv"],
		fileFormatHint: "Capital Gains from SMC Global trade platform",
		assetTypes: ["equity", "fno", "commodity"],
		pdfType: "broker_smc",
		isSupported: true,
	},
	{
		...
		category: "stock_broker",
		supportedFormats: ["xlsx", "csv"],
		fileFormatHint: "Tax P&L from Religare Broking platform",
		assetTypes: ["equity", "fno"],
		pdfType: "broker_religare",
		isSupported: true,
	},
	{
		id: "yes_securities",
		...
		category: "stock_broker",
		supportedFormats: ["xlsx", "csv"],
		fileFormatHint: "Capital Gains from Yes Securities back office",
		assetTypes: ["equity"],
		pdfType: "broker_yes",
		isSupported: true,
	},
	{
		...
		category: "stock_broker",
		supportedFormats: ["pdf", "xlsx"],
		fileFormatHint: "Tax report from Ventura > Reports > Capital Gains",
		assetTypes: ["equity", "fno"],
		pdfType: "broker_ventura",
		isSupported: true,
	},
	{
		id: "mastertrust",
		...
		category: "stock_broker",
		supportedFormats: ["xlsx", "csv"],
		fileFormatHint: "P&L statement from Mastertrust back office",
		assetTypes: ["equity", "fno"],
		pdfType: "broker_mastertrust",
		isSupported: true,
	},
	{
		id: "paytm_money",
		...
		category: "stock_broker",
		supportedFormats: ["pdf", "xlsx"],
		fileFormatHint: "Capital Gains from Paytm Money > Reports > Tax P&L",
		assetTypes: ["equity", "mutual_fund"],
		pdfType: "aggregator_paytm",
		isSupported: true,
	},
	{
		id: "funds_india",
		...
		category: "aggregator",
		supportedFormats: ["xlsx", "csv"],
		fileFormatHint: "Capital Gains statement from Funds India portal",
		assetTypes: ["mutual_fund"],
		pdfType: "aggregator_fundsindia",
		isSupported: true,
	},
	{
		id: "cams",
		...
		fileFormatHint:
			"Consolidated Account Statement (CAS) from mycams.camsonline.com",
		assetTypes: ["mutual_fund"],
		pdfType: "cas_cams",
		isSupported: true,
	},
	{
		...
		category: "fund_house",
		supportedFormats: ["pdf"],
		fileFormatHint: "CAS from KFintech (mfs.kfintech.com)",
		assetTypes: ["mutual_fund"],
		pdfType: "cas_kfintech",
		isSupported: true,
	},
	{
		id: "iris",
		...
		category: "fund_house",
		supportedFormats: ["pdf"],
		fileFormatHint: "CAS from IRIS / KFintech",
		assetTypes: ["mutual_fund"],
		pdfType: "cas_combined",
		isSupported: true,
	},
	{
		id: "mfcentral",
		name: "MF Central / IRIS",
		category: "fund_house",
		supportedFormats: ["pdf"],
		fileFormatHint: "Combined CAS from mfcentral.com or IRIS (CAMS + KFintech)",
		assetTypes: ["mutual_fund"],
		pdfType: "cas_combined",
		isSupported: true,
	},
	{
		...
		category: "aggregator",
		supportedFormats: ["pdf", "xlsx"],
		fileFormatHint: "Capital Gains from INDmoney app > Reports",
		assetTypes: ["mutual_fund"],
		pdfType: "aggregator_indmoney",
		isSupported: true,
	},
	{
		id: "kuvera",
		...
		category: "aggregator",
		supportedFormats: ["pdf", "xlsx"],
		fileFormatHint: "Tax report from Kuvera > Reports > Capital Gains",
		assetTypes: ["mutual_fund"],
		pdfType: "aggregator_kuvera",
		isSupported: true,
	},
	{
		id: "etmoney",
		...
		category: "aggregator",
		supportedFormats: ["pdf"],
		fileFormatHint: "Capital Gains statement from ET Money app",
		assetTypes: ["mutual_fund"],
		pdfType: "aggregator_etmoney",
		isSupported: true,
	},
	{
		id: "vested_us",
		...
		category: "us_stocks",
		supportedFormats: ["xlsx", "csv"],
		fileFormatHint: "Capital Gains report from Vested > Tax Center",
		assetTypes: ["us_equity"],
		pdfType: "broker_vested_us",
		isSupported: true,
	},
	{
		id: "indmoney_us",
		...
		category: "us_stocks",
		supportedFormats: ["xlsx", "csv"],
		fileFormatHint: "US Stock Capital Gains from INDmoney > Reports",
		assetTypes: ["us_equity"],
		pdfType: "broker_indmoney_us",
		isSupported: true,
	},
	{
		id: "groww_us",
		name: "Groww (US Stocks)",
		category: "us_stocks",
		supportedFormats: ["xlsx", "csv"],
		fileFormatHint: "US Stock P&L from Groww > Reports > US Stocks",
		assetTypes: ["us_equity"],
		pdfType: "broker_groww_us",
		isSupported: true,
	},
	{
		id: "template",
		name: "FintekPro Template",
		category: "aggregator",
		supportedFormats: ["xlsx", "csv"],
		fileFormatHint:
			"Download our Excel template, fill in your transactions, and upload",
		assetTypes: ["equity", "mutual_fund", "fno", "property", "other"],
		pdfType: "template",
		isSupported: true,
	},
];
...
router.get("/capital-gains/brokers", (_req: Request, res: Response) => {
	res.json({
		...
		categories: {
			stock_broker: BROKER_REGISTRY.filter((b) => b.category === "stock_broker")
				.length,
			fund_house: BROKER_REGISTRY.filter((b) => b.category === "fund_house")
				.length,
			aggregator: BROKER_REGISTRY.filter((b) => b.category === "aggregator")
				.length,
			us_stocks: BROKER_REGISTRY.filter((b) => b.category === "us_stocks")
				.length,
		},
	});
});
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (!brokerId || !assessmentYear) {
				return res
					.status(400)
					.json({ error: "brokerId and assessmentYear are required" });
			}
			...
			const transactions: CapitalGainsUploadRecord["transactions"] = [];
			const summary: CapitalGainsUploadRecord["summary"] = {
				totalSTCG: 0,
				totalLTCG: 0,
				totalSTCL: 0,
				totalLTCL: 0,
			...
			if (fileExt === "pdf") {
				try {
					const { unifiedPDFParser } = await import(
						"./services/unified-pdf-parser"
					);
					const parseResult = await (unifiedPDFParser as any).parseBuffer(
						file.buffer,
					);
					...
					if (parseResult.success) {
						...
						for (const holding of parseResult.holdings) {
							...
							const buyValue =
								holding.investedValue || holding.units * (holding.nav || 0);
							...
							for (const lot of parseResult.holdingLots) {
								if (lot.status === "redeemed" && lot.redemptionNav) {
									...
									const holdingDays = Math.floor(
										(new Date(lot.redemptionDate || "").getTime() -
											new Date(lot.purchaseDate).getTime()) /
											(1000 * 60 * 60 * 24),
									);
									const isLTCG = holdingDays > 365;
						...
						parseWarnings.push(
							"PDF parsing returned errors: " +
								(parseResult.errors || []).join(", "),
						);
					}
				} catch (parseErr) {
					parseWarnings.push(
						`PDF parse failed: ${parseErr instanceof Error ? parseErr.message : "Unknown error"}. Consider using Excel template.`,
					);
				}
			} else if (fileExt === "xlsx" || fileExt === "csv") {
				parseConfidence = 0.5;
				parseWarnings.push(
					`${fileExt.toUpperCase()} file detected. Auto-parsing will attempt column mapping. Please verify extracted data.`,
				);
				try {
					...
					if (lines.length > 1) {
						...
						parseWarnings.push(
							`Found ${lines.length - 1} rows. Column auto-detection applied.`,
						);
					}
				} catch {
	...
	(req: Request, res: Response) => {
		...
		const uploads = Array.from(capitalGainsUploads.values())
			.filter(
				(u) =>
					u.userId === userId &&
					(!assessmentYear || u.assessmentYear === assessmentYear),
			)
			.map((u) => ({
				id: u.id,
		...
		const aggregated = {
			...
			totalTransactions: uploads.reduce(
...
router.get(
	...
	(req: Request, res: Response) => {
		const userId =
			...
			(req as any).session?.userId ||
			(req as any).session?.user?.id;
		const uploadId = Number.parseInt(req.params.id);
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			const processedEntries = entries.map((entry, idx) => {
				...
				const holdingDays =
					entry.buyDate && entry.sellDate
						? Math.floor(
								(new Date(entry.sellDate).getTime() -
									new Date(entry.buyDate).getTime()) /
									(1000 * 60 * 60 * 24),
							)
						: 0;
			...
			const summary = {
				totalSTCG: processedEntries
					.filter((e) => e.gainType === "STCG" && e.gainLoss > 0)
					.reduce((s, e) => s + e.gainLoss, 0),
				totalLTCG: processedEntries
					.filter((e) => e.gainType === "LTCG" && e.gainLoss > 0)
					.reduce((s, e) => s + e.gainLoss, 0),
				totalExemptions: processedEntries.reduce(
					(s, e) => s + e.exemptionAmount,
					0,
				),
...
router.get(
	...
	(req: Request, res: Response) => {
		...
		const entries = Array.from(manualCapitalGains.values()).filter(
			(e) =>
				e.userId === userId &&
				(!assessmentYear || e.assessmentYear === assessmentYear),
		);
		...
		const aggregated = {
			...
			totalExemptions: entries.reduce(
				(s, e) => s + e.summary.totalExemptions,
				0,
			),
...
router.get(
	...
	(req: Request, res: Response) => {
		...
		const uploads = Array.from(capitalGainsUploads.values()).filter(
			(u) =>
				u.userId === userId &&
				(!assessmentYear || u.assessmentYear === assessmentYear),
		);
		const manuals = Array.from(manualCapitalGains.values()).filter(
			(e) =>
				e.userId === userId &&
				(!assessmentYear || e.assessmentYear === assessmentYear),
		);
		...
		const manualExemptions = manuals.reduce(
			(s, e) => s + e.summary.totalExemptions,
			0,
		);
		...
		const totalTransactions =
			uploads.reduce((s, u) => s + u.transactions.length, 0) +
			...
			summary: {
				...
				sources: [
					...uploads.map((u) => ({
						...
						broker: u.brokerName,
						stcg: u.summary.netSTCG,
						ltcg: u.summary.netLTCG,
						txCount: u.transactions.length,
					})),
					...manuals.map((e) => ({
						type: "manual" as const,
						assetType: e.assetType,
						stcg: e.summary.totalSTCG,
						ltcg: e.summary.totalLTCG,
						txCount: e.entries.length,
					})),
				],
			},
			auditTrail: {
		...
		try {
			...
			for (let i = 1; i < lines.length; i++) {
				...
				if (broker === "zerodha") {
					...
					buyDate =
						cols[headerMap.buy_date ?? headerMap.purchase_date ?? 2] || "";
					sellDate =
						cols[headerMap.sell_date ?? headerMap.sale_date ?? 3] || "";
					qty = Number(cols[headerMap.quantity ?? headerMap.qty ?? 4]) || 0;
					buyPrice =
						Number(
							cols[
								headerMap.buy_price ??
									headerMap.purchase_price ??
									headerMap.buy_avg ??
									5
							],
						) || 0;
					sellPrice =
						Number(
							cols[
								headerMap.sell_price ??
									headerMap.sale_price ??
									headerMap.sell_avg ??
									6
							],
						) || 0;
					...
					symbol =
						cols[headerMap.scheme_name ?? headerMap.fund_name ?? 0] || "";
					isin = cols[headerMap.isin ?? headerMap.isin_no ?? 1] || "";
					buyDate =
						cols[headerMap.allotment_date ?? headerMap.purchase_date ?? 2] ||
						"";
					sellDate =
						cols[headerMap.redemption_date ?? headerMap.sale_date ?? 3] || "";
					qty = Number(cols[headerMap.units ?? headerMap.quantity ?? 4]) || 0;
					buyPrice =
						Number(cols[headerMap.purchase_nav ?? headerMap.buy_price ?? 5]) ||
						0;
					sellPrice =
						Number(
							cols[headerMap.redemption_nav ?? headerMap.sell_price ?? 6],
						) || 0;
				} else {
					symbol = cols[0] || "";
					isin = cols[1] || "";
				...
				const holdingDays = Math.floor(
					(sellDateObj.getTime() - buyDateObj.getTime()) /
						(1000 * 60 * 60 * 24),
				);
			...
			res
				...
				.json({
					success: false,
					message: err.message || "Failed to parse broker file",
				});
		}
	},
);
	...
	async (req: Request, res: Response) => {
		try {
			...
			try {
				...
				return res
					...
					.json({
						success: false,
						message:
							"Invalid JSON file. Please upload a valid ITR JSON from the IT portal.",
					});
			}

			const itrData = parsed.ITR || parsed;
			const formType =
				Object.keys(itrData).find((k) => k.startsWith("ITR")) || "Unknown";

			const personalInfo =
				itrData[formType]?.PersonalInfo || itrData.PersonalInfo || {};
			const incomeDetails =
				itrData[formType]?.IncomeDeductions || itrData.IncomeDeductions || {};
			const taxPaid = itrData[formType]?.TaxPaid || itrData.TaxPaid || {};
			const schedCG =
				itrData[formType]?.ScheduleCGFor23 ||
				itrData[formType]?.ScheduleCG ||
				{};
			...
			const extractedData = {
				formType,
				assessmentYear:
					personalInfo?.AssesseeName?.AssessmentYear || personalInfo?.AY || "",
				...
				capitalGains: {
					stcg:
						schedCG?.ShortTerm?.TotalSTCG ||
						partBTI?.CapGain?.ShortTerm?.TotalSTCG ||
						0,
					ltcg:
						schedCG?.LongTerm?.TotalLTCG ||
						partBTI?.CapGain?.LongTerm?.TotalLTCG ||
						0,
				},
				otherSources: {
				...
				deductions: {
					...
					totalDeductions:
						incomeDetails?.DeductUndChapVIA?.TotalChapVIADeductions ||
						partBTI?.TotalChapVIADeductions ||
						0,
				},
				taxPaid: {
					...
					selfAssessment:
						taxPaid?.SelfAssessment || taxPaid?.TotalSelfAssessmentTax || 0,
				},
				totals: {
			...
			res.json({
				success: true,
				data: extractedData,
				message: `Successfully parsed ${formType} JSON`,
			});
		} catch (err: any) {
			res
				.status(500)
				.json({
					success: false,
					message: err.message || "Failed to parse ITR JSON",
				});
		}
	},
);
	...
	try {
		...
		if (!pan || !assessmentYear || !itrForm) {
			return res
				...
				.json({
					success: false,
					message: "PAN, assessment year, and ITR form are required",
				});
		}
		...
		const itrJSON: any = {
			[itrForm]: {
				...
				Form_ITR: {
					FormName: itrForm,
					Description: `Income Tax Return - ${itrForm}`,
				...
				PersonalInfo: {
					...
					PAN: pan,
					FilingStatus: {
						ReturnFileSec: data.filingSection || "139(1)",
						OptOutNewTaxRegime: data.taxRegime === "old" ? "Y" : "N",
					},
					ResidentialStatus:
						data.residentialStatus === "resident"
							? "RES"
				...
				IncomeDeductions: {
					...
					IncomeFromSal: Math.max(
						0,
						(data.salaryDetails?.grossSalary || 0) -
							(data.salaryDetails?.standardDeduction || 0),
					),
				},
				PartB_TI: {
					...
					CapGain: {
						ShortTerm: {
							TotalSTCG: data.capitalGainsDetails?.shortTermGains || 0,
						},
						LongTerm: {
							TotalLTCG: data.capitalGainsDetails?.longTermGains || 0,
						},
					},
		...
		res
			...
			.json({
				success: false,
				message: err.message || "Failed to generate ITR JSON",
			});
	}
});

router.post(
	"/api/tax/export/computation-sheet",
	async (req: Request, res: Response) => {
		try {
			...
			if (!pan || !assessmentYear) {
				return res
					...
					.json({
						success: false,
						message: "PAN and assessment year are required",
					});
			}
			...
			const computationSheet = {
				...
				pan,
				assessmentYear,
				previousYear: `${Number.parseInt(assessmentYear.split("-")[0]) - 1}-${assessmentYear.split("-")[0].slice(2)}`,
				sections: [
					{
						...
						items: [
							{
								label: "Gross Salary",
								amount: data.salaryDetails?.grossSalary || 0,
							},
							{
								label: "Less: Standard Deduction u/s 16(ia)",
								amount: -(data.salaryDetails?.standardDeduction || 0),
							},
							{
								label: "Net Salary",
								amount: Math.max(
					...
					},
					{
						...
						items: [
							{
								label: "Net Income from House Property",
								amount: data.housePropertyIncome || 0,
								isBold: true,
							},
						],
					},
						...
						items: [
							{
								...
								amount: data.capitalGainsDetails?.shortTermGains || 0,
							},
							{
								label: "Long Term Capital Gains",
								amount: data.capitalGainsDetails?.longTermGains || 0,
							},
							{
								label: "Total Capital Gains",
								amount:
									(data.capitalGainsDetails?.shortTermGains || 0) +
									(data.capitalGainsDetails?.longTermGains || 0),
								isBold: true,
							},
						],
					},
						...
						items: [
							{
								...
								amount: data.otherIncomeDetails?.interestIncome || 0,
							},
							{
								label: "Dividend Income",
								amount: data.otherIncomeDetails?.dividendIncome || 0,
							},
							{
								label: "Other Sources",
								amount: data.otherIncomeDetails?.otherSources || 0,
							},
							{
								label: "Total Other Income",
								amount:
									(data.otherIncomeDetails?.interestIncome || 0) +
									(data.otherIncomeDetails?.dividendIncome || 0) +
									(data.otherIncomeDetails?.otherSources || 0),
								isBold: true,
							},
						],
					},
						...
						items: [
							{
								...
								amount: data.grossTotalIncome || 0,
								isBold: true,
							},
						],
					},
					...
					},
					{
						...
						items: [
							...
							},
							{
								...
								amount: data.totalTaxLiability || 0,
								isBold: true,
							},
							...
							{ label: "Less: Advance Tax", amount: -(data.advanceTax || 0) },
							{
								...
								amount: -(data.selfAssessmentTax || 0),
							},
							{
								label: data.refundDue > 0 ? "REFUND DUE" : "TAX PAYABLE",
								amount:
									data.refundDue > 0 ? data.refundDue : data.taxPayable || 0,
								isBold: true,
							},
						],
					},
			...
			res
				...
				.json({
					success: false,
					message: err.message || "Failed to generate computation sheet",
				});
		}
	},
);
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			const { pan, name, email, phone, itrForm, assessmentYear, status } =
				req.body;
			if (!pan || !name)
				return res
					.status(400)
					.json({ success: false, message: "PAN and name required" });
...
router.patch(
	"/api/tax/practice/clients/:clientId",
	async (req: Request, res: Response) => {
		try {
...
router.post("/api/tax/challan/prepare", async (req: Request, res: Response) => {
	try {
		const { pan, assessmentYear, taxAmount, challanType, paymentMode } =
			req.body;
		if (!pan || !taxAmount)
			return res
				.status(400)
				.json({ success: false, message: "PAN and tax amount required" });
		...
		const dueDateObj = dueDate
			? new Date(dueDate)
			: new Date(`${(assessmentYear || "2025-26").split("-")[0]}-07-31`);
		const filingDateObj = filingDate ? new Date(filingDate) : new Date();
		const monthsLate234A = Math.max(
			0,
			Math.ceil(
				(filingDateObj.getTime() - dueDateObj.getTime()) /
					(30.44 * 24 * 60 * 60 * 1000),
			),
		);
		const interest234A =
			monthsLate234A > 0 ? Math.round(amt * 0.01 * monthsLate234A) : 0;

		const advTaxPaid = Number(advanceTaxPaid) || 0;
		const shortfall234B = Math.max(0, amt - advTaxPaid);
		const assessedTaxDue = amt * 0.9;
		const months234B =
			shortfall234B > 0 && advTaxPaid < assessedTaxDue
				? Math.max(
						0,
						Math.ceil(
							(filingDateObj.getTime() -
								new Date(
									`${(assessmentYear || "2025-26").split("-")[0]}-03-31`,
								).getTime()) /
								(30.44 * 24 * 60 * 60 * 1000),
						),
					)
				: 0;
		const interest234B =
			months234B > 0 ? Math.round(shortfall234B * 0.01 * months234B) : 0;
		...
		if (amt > 10000) {
			...
			const q4Due = amt;
			if (q1Paid < q1Due)
				interest234C += Math.round((q1Due - q1Paid) * 0.01 * 3);
			if (q1Paid + q2Paid < q2Due)
				interest234C += Math.round((q2Due - q1Paid - q2Paid) * 0.01 * 3);
			if (q1Paid + q2Paid + q3Paid < q3Due)
				interest234C += Math.round(
					(q3Due - q1Paid - q2Paid - q3Paid) * 0.01 * 3,
				);
			if (q1Paid + q2Paid + q3Paid + q4Paid < q4Due)
				interest234C += Math.round(
					(q4Due - q1Paid - q2Paid - q3Paid - q4Paid) * 0.01 * 1,
				);
		}
		...
		const surcharge = computeSurcharge(
			computeSlabTax(amt, NEW_REGIME_SLABS),
			amt,
			NEW_REGIME_SLABS,
		);
		...
		const challanData = {
			...
			interestBreakdown: {
				...
				"234C_note":
					"1% per month for shortfall in quarterly advance tax installments (15%/45%/75%/100%)",
				"234F_reason":
					fee234F > 0 ? "Filed after due date" : "Filed on or before due date",
			},
		};
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (!employeeName || !pan)
				return res
					.status(400)
					.json({ success: false, message: "Employee name and PAN required" });
			...
			const form12BB = {
				...
				employee: {
					name: employeeName,
					pan,
					designation: declarations?.designation || "",
				},
...
router.post("/api/tax/calculator/hra", async (req: Request, res: Response) => {
	try {
		const { basicSalary, daReceived, hraReceived, rentPaid, metroCity } =
			req.body;
		if (!basicSalary)
			return res
				.status(400)
		...
		const exemption2 = isMetro
			? Math.round((basic + da) * 0.5)
			: Math.round((basic + da) * 0.4);
		const exemption3 = Math.max(0, rent - Math.round((basic + da) * 0.1));
...
router.post(
	"/api/tax/calculator/form10e",
	async (req: Request, res: Response) => {
		try {
			const { currentYearArrears, totalIncome, arrearYears } = req.body;
			if (!currentYearArrears || !totalIncome)
				return res
					.status(400)
					.json({
						success: false,
						message: "Arrears and total income required",
					});
			...
			const taxOnWithAverage = computeSimpleTax(
				incomeWithoutArrears + averageArrears,
			);
			const additionalTaxOnAverage = taxOnWithAverage - taxOnWithout;
			const totalAdditionalTax = additionalTaxOnAverage * years;

			const reliefUs89 = Math.max(
				0,
				taxOnTotal - taxOnWithout - totalAdditionalTax,
			);
			...
			res.json({
				...
				data: {
					totalIncome: income,
					arrearsReceived: arrears,
					incomeWithoutArrears,
					taxOnTotal,
					taxOnWithout,
					averageArrears,
					taxOnWithAverage,
					additionalTaxOnAverage,
					totalAdditionalTax,
					reliefUs89,
					arrearYears: years,
...
function computeSurcharge(
	...
	for (let i = thresholds.length - 1; i >= 0; i--) {
		if (income > thresholds[i]) {
			...
			const maxTaxPlusSurcharge =
				taxAtThreshold + surchargeAtThreshold + excessIncome;
...
function computeFullTax(
	...
	const slabs =
		regime === "old"
			? age >= 80
				? OLD_REGIME_SLABS_ABOVE80
				: age >= 60
					? OLD_REGIME_SLABS_60TO80
		...
		try {
			...
			if (taxRegime === "old") {
				...
				if (used80C < 150000) {
					suggestions.push({
						section: "80C",
						potential: 150000 - used80C,
						taxSaving: Math.round((150000 - used80C) * 0.3),
						description:
							"Invest in ELSS, PPF, or NSC to maximize Section 80C deduction",
					});
				}
				...
				if (used80D < max80D) {
					suggestions.push({
						section: "80D",
						potential: max80D - used80D,
						taxSaving: Math.round((max80D - used80D) * 0.3),
				...
				if (used80CCD1B < 50000) {
					suggestions.push({
						section: "80CCD(1B)",
						potential: 50000 - used80CCD1B,
						taxSaving: Math.round((50000 - used80CCD1B) * 0.3),
			...
			if (income > 500000) {
				...
				if (taxRegime === "old" && newTax < oldTax) {
					suggestions.push({
						section: "Regime",
						potential: oldTax - newTax,
						taxSaving: oldTax - newTax,
						description:
							"Switch to New Regime for lower tax (fewer deductions but lower rates)",
					});
				} else if (taxRegime === "new" && oldTax < newTax) {
					suggestions.push({
						section: "Regime",
						potential: newTax - oldTax,
						taxSaving: newTax - oldTax,
						description:
							"Switch to Old Regime and maximize deductions for lower tax",
					});
				}
			}
			...
			res.json({
				...
				data: {
					...
					currentTax: computeSimpleTax(income),
				},
			});
		} catch (err: any) {
			res.status(500).json({ success: false, message: err.message });
	...
	async (req: Request, res: Response) => {
		try {
			if (!req.file)
				return res
					.status(400)
					.json({ success: false, message: "No AIS/TIS file uploaded" });
			const content = req.file.buffer.toString("utf-8");
			let parsed: any;
			try {
				parsed = JSON.parse(content);
				...
				return res
					...
					.json({
						success: false,
						message:
							"Invalid JSON format. Please upload the AIS/TIS JSON downloaded from the IT portal.",
					});
			}
			...
			const extractedData: any = {
				...
				foreignRemittances: [] as any[],
				mutualFundPurchases: [] as any[],
				pan: parsed.pan || parsed.PAN || "",
				assessmentYear: parsed.assessmentYear || parsed.ay || "",
			};

			const sections =
				parsed.data || parsed.sections || parsed.TaxPayerInfo?.SFTInfo || [];
			if (Array.isArray(sections)) {
				for (const section of sections) {
					const sftType =
						section.sftType || section.type || section.infoCategory || "";
					const amount = Number(
						section.amount || section.value || section.transactionValue || 0,
					);
					if (sftType.includes("TDS") || section.tdsSection) {
						extractedData.tdsEntries.push({
							tan: section.tan || section.deductorTAN || "",
							deductorName: section.deductorName || "",
							amount,
							section: section.tdsSection || "192",
							quarter: section.quarter || "",
						});
					} else if (
						sftType.includes("SFT-005") ||
						sftType.includes("INTEREST")
					) {
						extractedData.interestIncome += amount;
						extractedData.sftEntries.push({
							type: "interest",
							source: section.reportingEntity || "",
							amount,
						});
					} else if (
						sftType.includes("SFT-011") ||
						sftType.includes("DIVIDEND")
					) {
						...
						sftType.includes("SFT-013") ||
						sftType.includes("PROPERTY")
					) {
						extractedData.propertyTransactions.push({
							description: section.description || "Property",
							amount,
							date: section.transactionDate || "",
						});
					} else if (
						sftType.includes("SFT-008") ||
						sftType.includes("MUTUAL_FUND")
					) {
						extractedData.mutualFundPurchases.push({
							scheme: section.schemeName || section.description || "",
							amount,
							date: section.transactionDate || "",
						});
					} else if (
						sftType.includes("FOREIGN") ||
						sftType.includes("REMITTANCE")
					) {
						extractedData.foreignRemittances.push({
							purpose: section.purpose || "",
							amount,
							country: section.country || "",
						});
					} else if (amount > 1000000) {
						extractedData.highValueTransactions.push({
							type: sftType,
			...
			if (parsed.salary || parsed.salaryIncome)
				extractedData.salaryIncome = Number(
					parsed.salary || parsed.salaryIncome,
				);
			...
			res
				...
				.json({
					success: false,
					message: err.message || "Failed to parse AIS/TIS data",
				});
		}
	},
);
...
router.post("/api/tax/reconcile/26as", async (req: Request, res: Response) => {
	try {
		...
		if (!tdsEntered || !tds26AS)
			return res
				.status(400)
				.json({
					success: false,
					message: "Both entered TDS and 26AS TDS data required",
				});
		...
		Array.from(enteredMap.entries()).forEach(([key, entered]) => {
			...
			if (!as26Entry) {
				missing26AS.push({
					...entered,
					issue: "TDS entry not found in 26AS — may be rejected by CPC",
				});
			} else {
				const diff = Math.abs(
					Number(entered.amount) - Number(as26Entry.amount),
				);
				if (diff > 1) {
					mismatches.push({
						tan: entered.tan,
						section: entered.section,
						deductorName: entered.deductorName || as26Entry.deductorName,
						enteredAmount: Number(entered.amount),
						amount26AS: Number(as26Entry.amount),
						difference: diff,
						recommendation:
							Number(entered.amount) > Number(as26Entry.amount)
								? "Reduce to 26AS amount to avoid mismatch notice"
								: "Consider claiming 26AS amount",
					});
				} else {
					matched.push({
						tan: entered.tan,
		...
		Array.from(as26Map.entries()).forEach(([key, as26Entry]) => {
			if (!enteredMap.has(key)) {
				missingEntry.push({
					...as26Entry,
					issue:
						"TDS in 26AS but not claimed — you may be leaving money on the table",
				});
			}
		});

		const totalEntered = tdsEntered.reduce(
			(s: number, e: any) => s + Number(e.amount || 0),
			0,
		);
		const total26AS = tds26AS.reduce(
			(s: number, e: any) => s + Number(e.amount || 0),
			0,
		);
		...
		res.json({
			...
			data: {
				...
				summary: {
					totalEntered,
					total26AS,
					difference: Math.abs(totalEntered - total26AS),
					matchedCount: matched.length,
					mismatchCount: mismatches.length,
					missing26ASCount: missing26AS.length,
					missingEntryCount: missingEntry.length,
					status:
						mismatches.length === 0 && missing26AS.length === 0
							? "RECONCILED"
							: "DISCREPANCIES_FOUND",
					recommendation:
						mismatches.length > 0
							? "Fix mismatched amounts before filing to avoid intimation u/s 143(1)"
							: missing26AS.length > 0
								? "Remove unclaimed TDS not in 26AS"
								: "All TDS entries reconciled",
				},
			},
		});
	} catch (err: any) {
...
const SUPPORTED_BROKERS = [
	...
	{
		id: "coin_zerodha",
		name: "Coin by Zerodha (MF)",
		format: "CSV",
		columns: ["Fund", "ISIN", "Date", "Units", "NAV", "Amount", "Type"],
	},
	{
		id: "iris",
		name: "IRIS (KFintech)",
		format: "JSON/PDF",
		columns: ["Scheme", "ISIN", "Units", "Amount"],
	},
	{
		id: "mfcentral",
		name: "MFCentral / IRIS (CAS)",
		format: "PDF/CSV",
		columns: ["Scheme", "ISIN", "Date", "Units", "NAV", "Amount"],
	},
];
...
router.get("/api/tax/brokers/supported", (_req: Request, res: Response) => {
	res.json({
		success: true,
		data: SUPPORTED_BROKERS,
		total: SUPPORTED_BROKERS.length,
	});
});

router.post(
	"/api/tax/import/broker-cg-v2",
	upload.single("file"),
	async (req: Request, res: Response) => {
		try {
			if (!req.file)
			...
			if (lines.length < 2)
				return res
					.status(400)
					.json({ success: false, message: "File has insufficient data rows" });

			const header = lines[0].toLowerCase();
			let detectedBroker = brokerId;
			if (brokerId === "generic" || brokerId === "auto") {
				if (header.includes("trade_type") && header.includes("symbol"))
					detectedBroker = "zerodha";
				else if (header.includes("stock name") && header.includes("buy date"))
					detectedBroker = "groww";
				else if (header.includes("scrip name") && header.includes("isin no"))
					detectedBroker = "iifl_securities";
				else if (header.includes("scrip") && header.includes("buy avg"))
					detectedBroker = "hdfc_securities";
				else if (header.includes("script name"))
					detectedBroker = "kotak_securities";
				else if (header.includes("scheme") || header.includes("fund"))
					detectedBroker = "cams";
			...
			for (let i = 1; i < lines.length; i++) {
				...
				try {
					...
					holdingDays = Math.floor(
						(sd.getTime() - bd.getTime()) / (1000 * 60 * 60 * 24),
					);
				} catch {}
			...
			res
				...
				.json({
					success: false,
					message: err.message || "Failed to parse broker statement",
				});
		}
	},
);
...
router.post("/api/tax/efile/direct", async (req: Request, res: Response) => {
	try {
		...
		if (!pan || !itrForm || !itrData)
			return res
				.status(400)
				.json({
					success: false,
					message: "PAN, ITR form, and ITR data required",
				});

		const firstName =
			personalInfo?.firstName || itrData?.personalInfo?.firstName;
		const lastName = personalInfo?.lastName || itrData?.personalInfo?.lastName;
		const dateOfBirth =
			personalInfo?.dateOfBirth || itrData?.personalInfo?.dateOfBirth;
		...
		const acctNumber =
			bankDetails?.accountNumber || itrData?.bankDetails?.accountNumber;
		...
		if (!aadhar || aadhar.length !== 12)
			missingFields.push("aadhar (12-digit Aadhaar number)");
		if (
			!address?.line1 ||
			!address?.city ||
			!address?.state ||
			...
			missingFields.push("address (line1, city, state, pincode)");
		...
		const formData = {
			...
			incomeDetails: {
				salaryIncome: Number(
					itrData?.salaryDetails?.grossSalary || itrData?.salaryIncome || 0,
				),
				businessIncome: Number(itrData?.businessIncome || 0),
				capitalGains:
					Number(itrData?.capitalGainsDetails?.shortTermGains || 0) +
					Number(itrData?.capitalGainsDetails?.longTermGains || 0),
				otherIncome: Number(itrData?.otherIncomeDetails?.otherIncome || 0),
				interestIncome: Number(
					itrData?.otherIncomeDetails?.interestIncome || 0,
				),
				rentalIncome: Number(itrData?.housePropertyIncome || 0),
				dividendIncome: Number(
					itrData?.otherIncomeDetails?.dividendIncome || 0,
				),
			},
			deductions: {
				...
				homeLoanInterest: Number(
					itrData?.deductionDetails?.homeLoanInterest || 0,
				),
				standardDeduction: Number(
					itrData?.deductionDetails?.standardDeduction || 75000,
				),
				professionalTax: Number(
					itrData?.deductionDetails?.professionalTax || 0,
				),
				otherDeductions: Number(
					itrData?.deductionDetails?.otherDeductions || 0,
				),
			},
			taxPayments: {
				...
				selfAssessmentTax: Number(
					itrData?.taxPaymentDetails?.selfAssessmentTax || 0,
				),
			},
			bankDetails: {
		...
		if (eVerificationMethod && result.data?.acknowledgmentNumber) {
			...
			(result.data as any).eVerificationStatus = eVerifyResult.success
				? "VERIFIED"
				: "PENDING";
		...
		try {
			res.json({
				...
				data: {
					...
					generatedAt: new Date().toISOString(),
					instructions:
						"If not e-verified within 30 days, please send signed ITR-V to CPC, Post Bag No. 1, Electronic City Post Office, Bengaluru — 560100, Karnataka",
...
// ==========================================
router.post(
	"/api/tax/challan/prepare-extended",
	async (req: Request, res: Response) => {
		try {
			const {
				pan,
			...
			if (challanType === "tds_tcs" || challanType === "281") {
				challanNo = "281";
				majorHead = "0020";
				minorHead = natureOfPayment === "salary" ? "200" : "800";
			...
			res.json({
				...
				data: {
					...
					instructions:
						challanNo === "281"
							? "Use TAN for TDS/TCS payments"
							: challanNo === "282"
								? "For gift tax, wealth tax, and other direct taxes"
...
const BSR_CODES: Record<string, string> = {
	"0002": "State Bank of India",
	"0003": "SBI (Associate)",
	"0004": "Union Bank of India",
	"0005": "Indian Bank",
	"0006": "Canara Bank",
	"0008": "Bank of Baroda",
	"0009": "Punjab National Bank",
	"0010": "Central Bank of India",
	"0011": "Bank of India",
	"0012": "Indian Overseas Bank",
	"0016": "HDFC Bank",
	"0018": "ICICI Bank",
	"0022": "Axis Bank",
	"0024": "Kotak Mahindra Bank",
	"0028": "Yes Bank",
	"0032": "IDBI Bank",
	"0229": "RBL Bank",
};
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (!code || code.length !== 11)
				return res
					.status(400)
					.json({ success: false, message: "IFSC code must be 11 characters" });
			...
			res.json({
				...
				data: {
					ifsc: code,
					bank: bankCode,
					branch: `Branch for ${code}`,
					address: "",
					city: "",
					state: "",
					note: "Lookup via external API unavailable, showing partial data",
				},
			});
		} catch (err: any) {
			res.status(500).json({ success: false, message: err.message });
...
router.get("/api/tax/lookup/bsr/:code", async (req: Request, res: Response) => {
	try {
		...
		if (!code)
			return res
				.status(400)
				.json({ success: false, message: "BSR code required" });
		...
		res.json({
			...
			data: {
				bsrCode: code,
				bankName,
				description: `BSR Code ${code} — ${bankName}`,
			},
		});
	} catch (err: any) {
		res.status(500).json({ success: false, message: err.message });
	...
	try {
		...
		if (!tan || tan.length !== 10)
			return res
				...
				.json({
					success: false,
					message: "TAN must be 10 characters (e.g., DELH12345A)",
				});
		if (/iris|kfintech|mf\s*central|mfcentral\.com/i.test(text))
			return "aggregator_mfcentral";
		if (!/^[A-Z]{4}\d{5}[A-Z]$/.test(tan))
			return res
				.status(400)
				.json({ success: false, message: "Invalid TAN format" });
		...
		res.json({
			...
			data: {
				tan,
				city: cityMap[cityCode] || cityCode,
				entityType: tan[3],
				isValid: true,
				format: "AAAA99999A",
			},
	...
	try {
		...
		const aoCodes = [
			...
			},
			{
				...
				rangeCode: "2",
				aoNumber: "1",
				jurisdiction: "Circle 2(1), Mumbai",
			},
			{
				city: "Chennai",
				areaCode: "CHE",
			...
			},
			{
				...
				rangeCode: "1",
				aoNumber: "1",
				jurisdiction: "Ward 1(1), Bengaluru",
			},
			{
				city: "Kolkata",
				areaCode: "KOL",
			...
			},
			{
				...
				rangeCode: "1",
				aoNumber: "1",
				jurisdiction: "Ward 1(1), Hyderabad",
			},
		];
		...
		if (city)
			filtered = filtered.filter((a) =>
				a.city.toLowerCase().includes(String(city).toLowerCase()),
			);
		if (areaCode)
			filtered = filtered.filter(
				(a) => a.areaCode === String(areaCode).toUpperCase(),
			);
		if (rangeCode)
			filtered = filtered.filter((a) => a.rangeCode === String(rangeCode));
		if (aoType)
			filtered = filtered.filter(
				(a) => a.aoType === String(aoType).toUpperCase(),
			);
		...
		res.json({
			...
			aoTypes: { W: "Ward", C: "Circle", R: "Range" },
		});
	} catch (err: any) {
		res.status(500).json({ success: false, message: err.message });
	}
		...
		try {
			...
			if (
				...
				return res
					...
					.json({
						success: false,
						message: "Asset class and transactions array required",
					});
			}
			...
			if (!validClasses.includes(assetClass)) {
				return res
					...
					.json({
						success: false,
						message: `Invalid asset class. Valid: ${validClasses.join(", ")}`,
					});
			}

			const result = await sandboxITRService.calculateTaxPnL(
				assetClass,
				transactions,
			);
			if (!result.success) {
...
// ==========================================
router.post(
	"/api/tax/calculate/indexed-cost",
	async (req: Request, res: Response) => {
		try {
			...
			if (!items || !Array.isArray(items) || items.length === 0) {
				return res
					...
					.json({
						success: false,
						message:
							"Items array required with acquisitionCost, acquisitionYear, saleYear",
					});
			}
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (!pan)
				return res
					.status(400)
					.json({ success: false, message: "PAN required" });

			const result = await sandboxITRService.getCapitalGainsReport(
				pan,
...
router.post("/api/tax/share/email", async (req: Request, res: Response) => {
	try {
		...
		const docLabels: Record<string, string> = {
			itr_v: "ITR-V Acknowledgment",
			computation: "Computation Sheet",
			itr_json: "ITR JSON",
			form_26as: "Form 26AS",
			ais_tis: "AIS/TIS Statement",
...
router.post("/api/tax/share/whatsapp", async (req: Request, res: Response) => {
	try {
		const { phoneNumber, documentType, pan, assessmentYear, message } =
			req.body;
		if (!phoneNumber)
			return res
				.status(400)
		...
		try {
			...
			if (sent) {
				deliveryMethod =
					result.provider === "iris" ? "iris_whatsapp" : "twilio_whatsapp";
			}
		} catch {
			try {
		...
		const whatsappRecord = {
			...
			deliveryMethod,
			whatsappUrl: !sent
				? `https://wa.me/${phoneNumber.replace(/\D/g, "")}?text=${encodeURIComponent(msgToSend)}`
				: undefined,
		};
...
// ==========================================
router.post(
	"/api/tax/export/computation-pdf",
	async (req: Request, res: Response) => {
		try {
			...
			const salaryIncome =
				Number(data?.salaryDetails?.grossSalary || 0) -
				Number(data?.salaryDetails?.standardDeduction || 75000);
			...
			const deductions =
				...
				sec80D +
				sec80G +
				Number(data?.deductionDetails?.otherDeductions || 0);
			const tds = Number(data?.taxPaymentDetails?.tdsDeducted || 0);
			const advanceTax = Number(data?.taxPaymentDetails?.advanceTaxPaid || 0);
			const selfAssessmentTax = Number(
				data?.taxPaymentDetails?.selfAssessmentTax || 0,
			);
			...
			try {
				const calcResult = await sandboxITRService.calculateTaxFromWizard({
					...
					capitalGainsExemptions: Number(
						data?.capitalGainsDetails?.exemptions || 0,
					),
					...
					selfAssessmentTax,
					standardDeduction: Number(
						data?.salaryDetails?.standardDeduction || 75000,
					),
					professionalTax: Number(data?.salaryDetails?.professionalTax || 0),
					homeLoanInterest: Number(
						data?.deductionDetails?.homeLoanInterest || 0,
					),
				});
				...
				console.warn(
					"[Computation] Sandbox API unavailable, using local calculation:",
					apiErr,
				);
			}

			const taxableIncome =
				taxResult?.taxableIncome ?? Math.max(0, grossTotal - deductions);
			...
			const refund =
				totalPaid > totalTaxLiability ? totalPaid - totalTaxLiability : 0;
			...
			const pdfContent = {
				...
				sections: [
					{
						heading: "1. INCOME FROM SALARY",
						items: [
							{
								label: "Gross Salary",
								amount: Number(data?.salaryDetails?.grossSalary || 0),
							},
							{
								label: "Less: Standard Deduction u/s 16(ia)",
								amount: Number(data?.salaryDetails?.standardDeduction || 75000),
								isDeduction: true,
							},
							{
								label: "Net Salary Income",
								amount: salaryIncome,
								isBold: true,
							},
						],
					...
					{
						heading: "2. INCOME FROM HOUSE PROPERTY",
						items: [{ label: "Net Income from HP", amount: hpIncome }],
					},
					{
						heading: "3. CAPITAL GAINS",
						items: [
					...
					{
						heading: "4. INCOME FROM OTHER SOURCES",
						items: [
					...
					{
						heading: "5. GROSS TOTAL INCOME",
						items: [
							{
								label: "Gross Total Income",
								amount: taxResult?.totalIncome ?? grossTotal,
								isBold: true,
							},
						],
					},
					{
						...
						items: [
							...
							{
								label: "Total Deductions",
								amount: taxResult?.totalDeductions ?? deductions,
								isBold: true,
								isDeduction: true,
							},
						],
					...
					{
						heading: "7. TOTAL TAXABLE INCOME",
						items: [
							{
								label: "Total Taxable Income",
					...
					{
						heading: "8. TAX COMPUTATION",
						items: [
							{
								label: `Tax on Total Income (${data?.taxRegime || "new"} regime)`,
								amount: tax,
							},
...
router.get("/api/tax/refund/status", async (req: Request, res: Response) => {
	try {
		...
		if (acknowledgmentNumber) {
			const apiResult = await sandboxITRService.getITRStatus(
				...
				const allStages = [
					{
						...
						date: apiResult.data.filingDate || null,
					},
					{
						stage: "Return Verified (e-Verification)",
						status:
							completedSteps >= 2
								? "completed"
								: completedSteps === 1
									? "in_progress"
									: "pending",
						date: apiResult.data.verificationDate || null,
					},
					{
						stage: "Return Processed at CPC",
						status:
							completedSteps >= 3
								? "completed"
								: completedSteps === 2
					...
					{
						stage: "Intimation u/s 143(1) Sent",
						status:
							completedSteps >= 4
								? "completed"
								: completedSteps === 3
					...
					},
					{
						stage: "Refund Issued",
						status:
							apiResult.data.refundStatus === "Issued"
								? "completed"
								: apiResult.data.refundStatus === "Processed"
									? "in_progress"
									: "pending",
						date: null,
					},
...
router.get("/api/tax/deadlines", async (_req: Request, res: Response) => {
	try {
		const deadlines = [
			{
				form: "ITR-1/ITR-4",
				category: "Individual/HUF (No Audit)",
				deadline: "2025-07-31",
				status: "upcoming",
				description: "Last date for non-audit cases",
				penalty: "₹5,000 after due date (₹1,000 if income ≤ ₹5L)",
			},
			{
				form: "ITR-2/ITR-3",
				category: "Individual (CG/Business)",
				deadline: "2025-07-31",
				status: "upcoming",
				description: "Capital gains / business income",
				penalty: "₹5,000 after due date",
			},
			{
				form: "ITR-5/ITR-6/ITR-7",
				category: "Firms/Companies/Trusts (Audit)",
				deadline: "2025-10-31",
				status: "upcoming",
				description: "Audit cases u/s 44AB",
			...
			},
			{
				...
				penalty: "0.5% of turnover or ₹1,50,000",
			},
			{
				form: "ITR-U",
				category: "Updated Return",
				deadline: "2027-03-31",
				status: "upcoming",
				description: "Updated return for AY 2025-26 (within 24 months)",
				penalty: "25% additional tax (12m) / 50% (24m)",
			},
			{
				form: "Advance Tax - Q1",
				...
				deadline: "2025-06-15",
				status: "upcoming",
				description: "15% of estimated tax",
			...
			},
			{
				...
				deadline: "2025-09-15",
				status: "upcoming",
				description: "45% of estimated tax",
				penalty: "Interest u/s 234C",
			},
			{
				form: "Advance Tax - Q3",
				...
				deadline: "2025-12-15",
				status: "upcoming",
				description: "75% of estimated tax",
				penalty: "Interest u/s 234C",
			},
			{
				form: "Advance Tax - Q4",
				...
				deadline: "2026-03-15",
				status: "upcoming",
				description: "100% of estimated tax",
				penalty: "Interest u/s 234C",
			},
			{
				form: "Belated Return",
				...
				deadline: "2025-12-31",
				status: "upcoming",
				description: "Last date for belated/revised return u/s 139(4)",
				penalty: "₹5,000 + loss of carry-forward",
			},
		];
...
router.post("/api/tax/generate/itrv", async (req: Request, res: Response) => {
	try {
		...
		const taxableIncome = Number(
			incomeData?.taxableIncome ||
				Math.max(0, grossTotalIncome - totalDeductions),
		);
		...
		const itrV = {
			...
			bankDetails: incomeData?.bankDetails || {
				bankName: "",
				accountNumber: "",
				ifscCode: "",
				accountType: "Savings",
			...
			note: ackNumber
				? "Sandbox API did not return ITR-V. Showing structured data. Re-try with valid acknowledgment number."
				: "No acknowledgment number provided. File ITR first to generate official ITR-V.",
			generatedAt: new Date().toISOString(),
...
router.post("/api/tax/history/save", async (req: Request, res: Response) => {
	try {
		...
		if (!pan || !assessmentYear)
			return res
				.status(400)
				.json({ success: false, message: "PAN and assessment year required" });
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (!lastName || !firstName || !dateOfBirth)
				return res
					.status(400)
					.json({ success: false, message: "Name and date of birth required" });
			...
			const form49A = {
				...
				identityProof: {
					...
					documentType: aadhaarNumber ? "Aadhaar Card" : "Other",
				},
				fee: {
					amount: applicationType === "correction" ? 107 : 107,
					currency: "INR",
					gst: 19,
					total: 126,
				},
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (!pan || !data)
				return res
					.status(400)
					.json({ success: false, message: "PAN and data required" });
			...
			if (pan && !/^[A-Z]{5}\d{4}[A-Z]$/.test(pan))
				errors.push({
					code: "E002",
					field: "pan",
					message: "PAN does not match standard format AAAAA9999A",
					severity: "error",
				});

			const salary = Number(data?.salaryDetails?.grossSalary || 0);
			const stdDeduction = Number(data?.salaryDetails?.standardDeduction || 0);
			if (salary > 0 && stdDeduction > 75000)
				errors.push({
					code: "E003",
					field: "standardDeduction",
					message: "Standard deduction cannot exceed ₹75,000 (Budget 2024)",
					severity: "error",
				});
			if (salary > 0 && !data?.salaryDetails?.employerTAN)
				warnings.push({
					code: "W001",
					field: "employerTAN",
					message:
						"Employer TAN not provided — TDS credit may not be processed",
					severity: "warning",
				});

			const sec80C = Number(data?.deductionDetails?.section80C || 0);
			if (sec80C > 150000 && data?.taxRegime === "old")
				errors.push({
					code: "E004",
					field: "section80C",
			...
			if (sec80D > 100000)
				errors.push({
					code: "E005",
					field: "section80D",
					message: "Section 80D deduction exceeds maximum limit of ₹1,00,000",
					severity: "error",
				});

			if (data?.taxRegime === "new" && sec80C > 0)
				warnings.push({
					code: "W002",
					field: "regime",
					message:
						"Chapter VI-A deductions (80C, 80D, etc.) are not available under New Regime",
					severity: "warning",
				});

			const stcg = Number(data?.capitalGainsDetails?.shortTermGains || 0);
			const ltcg = Number(data?.capitalGainsDetails?.longTermGains || 0);
			if ((stcg !== 0 || ltcg !== 0) && itrForm === "ITR-1")
				errors.push({
					code: "E006",
					field: "itrForm",
					message: "Capital gains not allowed in ITR-1 — use ITR-2 or ITR-3",
					severity: "error",
				});

			const businessInc = Number(data?.businessIncome || 0);
			if (businessInc !== 0 && itrForm === "ITR-1")
				errors.push({
					code: "E008",
					field: "itrForm",
					message:
						"Business/professional income not allowed in ITR-1 — use ITR-3 or ITR-4",
					severity: "error",
				});

			const agriIncome = Number(data?.agriculturalIncome || 0);
			if (agriIncome > 5000 && itrForm === "ITR-1")
				errors.push({
					code: "E009",
					field: "itrForm",
					message:
						"Agricultural income exceeding ₹5,000 not allowed in ITR-1 — use ITR-2",
					severity: "error",
				});

			const housePropertyCount = Number(
				data?.housePropertyCount || data?.numberOfHouseProperties || 0,
			);
			if (housePropertyCount > 1 && itrForm === "ITR-1")
				errors.push({
					code: "E010",
					field: "itrForm",
					message: "Multiple house properties not allowed in ITR-1 — use ITR-2",
					severity: "error",
				});

			const sec80GG = Number(data?.deductionDetails?.section80GG || 0);
			if (sec80GG > 0 && data?.taxRegime === "old") {
				const maxGG = Math.min(
					60000,
					Math.round(Number(data?.grossTotalIncome || 0) * 0.25),
				);
				if (sec80GG > maxGG)
					errors.push({
						code: "E011",
						field: "section80GG",
						message: `Section 80GG deduction cannot exceed ₹60,000/year or 25% of total income (max: ₹${maxGG.toLocaleString("en-IN")})`,
						severity: "error",
					});
			}
			...
			const grossTotal =
				...
				stcg +
				ltcg +
				Number(data?.otherIncomeDetails?.interestIncome || 0);
			if (tds > grossTotal * 0.5)
				warnings.push({
					code: "W003",
					field: "tds",
					message: "TDS exceeds 50% of gross income — verify 26AS data",
					severity: "warning",
				});

			if (
				data?.residentialStatus === "NRI" &&
				data?.deductionDetails?.section80C > 0 &&
				data?.taxRegime === "old"
			) {
				warnings.push({
					code: "W004",
					field: "nri_deductions",
					message:
						"NRIs may have limited Chapter VI-A deductions — verify eligibility",
					severity: "warning",
				});
			}

			const grossIncome = Number(data?.grossTotalIncome || grossTotal);
			if (grossIncome > 5000000 && itrForm === "ITR-1")
				warnings.push({
					code: "W005",
					field: "itrForm",
					message:
						"Income exceeds ₹50 lakhs — Schedule AL (Assets & Liabilities) may be required (use ITR-2)",
					severity: "warning",
				});

			if (
				data?.taxPaymentDetails?.selfAssessmentTaxPaid > 0 &&
				!data?.taxPaymentDetails?.selfAssessmentChallanDate
			) {
				warnings.push({
					code: "W006",
					field: "challanDate",
					message:
						"Self-assessment tax challan date not provided — needed for 234B/C computation",
					severity: "warning",
				});
			}
			...
			if (hpIncome < -200000)
				errors.push({
					...
					field: "houseProperty",
					message:
						"House property loss set-off capped at ₹2,00,000 per year (excess carries forward)",
					severity: "error",
				});
			...
			if (data?.bankDetails?.accountNumber && !data?.bankDetails?.ifscCode) {
				warnings.push({
					code: "W007",
					field: "bankIfsc",
					message: "Bank IFSC code missing — required for refund credit",
					severity: "warning",
				});
			}
			...
			if (data?.residentialStatus === "NRI" && itrForm === "ITR-1") {
				errors.push({
					code: "E012",
					field: "itrForm",
					message: "Non-residents cannot file ITR-1 — use ITR-2 or higher",
					severity: "error",
				});
			}
			...
			if (data?.isDirectorInCompany && itrForm === "ITR-1") {
				errors.push({
					code: "E013",
					field: "itrForm",
					message: "Directors of companies cannot file ITR-1 — use ITR-2",
					severity: "error",
				});
			}
			...
			if (data?.hasForeignAssets && itrForm === "ITR-1") {
				errors.push({
					...
					field: "itrForm",
					message:
						"Taxpayers with foreign assets/income cannot file ITR-1 — use ITR-2 with Schedule FA",
					severity: "error",
				});
			}
			...
			info.push({
				code: "I001",
				message: `Filing ${itrForm || "ITR-1"} for AY ${assessmentYear || "2025-26"}`,
			});
			if (grossIncome > 2500000)
				info.push({
					code: "I002",
					message: "Return is mandatory (income exceeds basic exemption limit)",
				});
	...
	async (req: Request, res: Response) => {
		try {
			...
			res.json({
				...
				data: {
					results,
					summary: { total: clients.length, imported: success, failed },
				},
			});
		} catch (err: any) {
			res.status(500).json({ success: false, message: err.message });
