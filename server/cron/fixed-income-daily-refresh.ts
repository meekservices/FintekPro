...
export async function runDailyFixedIncomeRefresh(): Promise<{
	...
	console.log(
		`[FixedIncomeRefresh] Starting daily refresh at ${new Date().toISOString()}`,
	);
...
export function scheduleFixedIncomeRefresh(
	cronHour: number = 6,
	cronMinute: number = 0,
): void {
	const scheduleNextRun = () => {
		...
		console.log(
			`[FixedIncomeRefresh] Next refresh scheduled for ${targetTime.toISOString()}`,
		);
	...
	scheduleNextRun();
	console.log(
		`[FixedIncomeRefresh] Daily refresh scheduler initialized for ${cronHour}:${cronMinute.toString().padStart(2, "0")} IST`,
	);
}
