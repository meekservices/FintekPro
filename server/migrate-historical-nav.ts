...
async function main() {
	...
	try {
		const devSchemes = await devDb
			.selectDistinct({ identifier: historicalNavData.identifier })
			.from(historicalNavData);
		const prodSchemes = await prodDb
			.selectDistinct({ identifier: historicalNavData.identifier })
			.from(historicalNavData);

		const prodSet = new Set(prodSchemes.map((r) => r.identifier));
		const missing = devSchemes
			.map((r) => r.identifier)
			.filter((s) => !prodSet.has(s));
		...
		for (let chunk = 0; chunk < missing.length; chunk += CHUNK) {
			...
			const rows = await devDb
				...
				.orderBy(
					asc(historicalNavData.identifier),
					asc(historicalNavData.date),
				);
			...
			console.log(
				`   Schemes ${done}/${missing.length}: ${totalMigrated} rows, ${totalErrors} errors`,
			);
		}

		const [{ cnt }] = await prodDb
			.select({ cnt: count() })
			.from(historicalNavData);
		console.log(
