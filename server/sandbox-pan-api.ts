...
class SandboxPANService {
	...
	constructor() {
		...
		if (!hasSandboxCredentials()) {
			console.warn(
				"⚠️ [Sandbox PAN API] API credentials not configured. Mock mode will be used in sandbox environment only.",
			);
		} else {
			const env = this.isTestEnvironment ? "TEST" : "PRODUCTION";
			console.log(
				`✅ [Sandbox PAN API] Initialized (${env} environment → ${this.baseUrl})`,
			);
		}
	}
	...
	private async authenticate(): Promise<string> {
		try {
			...
			console.error(
				"❌ [Sandbox PAN API] Authentication failed:",
				error.message,
			);
			throw new AppError(
	...
	async verifyPAN(
		...
		fullName?: string,
		dateOfBirth?: string,
	): Promise<SandboxPANResponse> {
		try {
			const panRegex = /^[A-Z]{5}[0-9]{4}[A-Z]{1}$/;
			if (!panRegex.test(panNumber)) {
				throw new AppError(
					"Invalid PAN format. PAN must be in format: ABCDE1234F",
					400,
					"INVALID_PAN_FORMAT",
				);
			}
			...
			if (!hasSandboxCredentials()) {
				if (kycEnvironmentService.isSandbox()) {
					console.log(
						"🔧 [Sandbox PAN API] No credentials, using mock (sandbox mode)",
					);
			...
			try {
				...
				if (kycEnvironmentService.isSandbox()) {
					console.warn(
						`⚠️ [Sandbox PAN API] Auth failed (${authError.message}), falling back to mock (KYC sandbox mode)`,
					);
					return this.mockPANVerification(panNumber, fullName);
				}
				throw authError;
			...
			if (
				formattedDOB?.includes("-") &&
				formattedDOB.match(/^\d{4}-\d{2}-\d{2}$/)
			) {
			...
			const resolvedData: any = responseData?.pan_number
				? responseData
			...
			const presentNameFields = nameFields.filter(
				(f) => resolvedData?.[f] != null && resolvedData[f] !== "",
			);
			console.log(
				`[Sandbox PAN API] Response fields: ${Object.keys(resolvedData || {}).join(", ")} | Name fields present: ${presentNameFields.map((f) => `${f}="${resolvedData[f]}"`).join(", ") || "NONE"}`,
			);

			// Resolve name from whichever field the API populated
			const resolvedName =
				resolvedData?.full_name ||
				resolvedData?.name ||
				resolvedData?.name_on_card ||
				resolvedData?.name_pan_card ||
				(resolvedData?.first_name && resolvedData?.last_name
					? `${resolvedData.first_name} ${resolvedData.last_name}`.trim()
					: null) ||
				resolvedData?.first_name ||
				resolvedData?.holder_name ||
				null;
			...
			if (normalizedResponse.status === "success" && normalizedResponse.data) {
				console.log(
					`✅ [Sandbox PAN API] PAN verified successfully: ${this.maskPAN(panNumber)} name="${resolvedName || "NOT_RETURNED"}" (${this.isTestEnvironment ? "TEST" : "LIVE"})`,
				);
				return normalizedResponse;
			}
			console.error(
				"❌ [Sandbox PAN API] PAN verification failed:",
			...
			if (axios.isAxiosError(error)) {
				...
				if (
					...
					console.warn(
						"⚠️ [Sandbox PAN API] PAN API not subscribed on test account, falling back to mock verification",
					);
					return this.mockPANVerification(panNumber, fullName);
				}
				...
				if (
					statusCode === 401 ||
					statusCode === 403 ||
					(statusCode === 400 && errorMessage?.includes("Authorization"))
				) {
					clearSandboxToken();
					try {
						...
						return {
							...
							data: retryData?.pan_number
								? retryData
								: retryData?.data || retryData,
						};
					} catch (retryError) {
						console.error(
							"❌ [Sandbox PAN API] Retry also failed:",
							retryError,
						);
						if (kycEnvironmentService.isSandbox()) {
							console.warn(
								"⚠️ [Sandbox PAN API] Auth retry failed, falling back to mock (KYC sandbox mode)",
							);
							return this.mockPANVerification(panNumber, fullName);
						}
					}
				...
				if (kycEnvironmentService.isSandbox()) {
					console.warn(
						`⚠️ [Sandbox PAN API] API returned HTTP ${statusCode} ("${errorMessage}") in KYC sandbox mode — falling back to mock`,
					);
					return this.mockPANVerification(panNumber, fullName);
				}
	...
	private mockPANVerification(
		...
		console.log(
			"🔧 [Sandbox PAN API] Using MOCK PAN verification for:",
			this.maskPAN(panNumber),
		);
	...
	async checkPANAadhaarLinkage(
		...
		try {
			...
			if (!hasSandboxCredentials()) {
				...
				throw new AppError(
					"PAN-Aadhaar linkage service not configured",
					503,
					"SANDBOX_NOT_CONFIGURED",
				);
			}
			...
			try {
				...
				if (kycEnvironmentService.isSandbox()) {
					console.warn(
						`⚠️ [Sandbox PAN API] Auth failed for linkage check (${authError.message}), falling back to mock (KYC sandbox mode)`,
					);
					return this.mockPANAadhaarLinkage(panNumber);
				}
				throw authError;
			...
			console.log(
				`✅ [Sandbox PAN API] PAN-Aadhaar linkage checked: ${this.maskPAN(panNumber)} (${this.isTestEnvironment ? "TEST" : "LIVE"})`,
			);
			return {
				status: "success" as const,
				data: linkageData as PANAadhaarLinkageResponse["data"],
			};
		} catch (error) {
			...
			if (axios.isAxiosError(error)) {
				...
				const errorMessage =
					axiosError.response?.data?.message || axiosError.message;
				console.error("❌ [Sandbox PAN API] Linkage check error:", {
					status: statusCode,
					message: errorMessage,
				});
				...
				if (
					...
					console.warn(
						"⚠️ [Sandbox PAN API] PAN-Aadhaar linkage API not subscribed on test account, falling back to mock",
					);
					return this.mockPANAadhaarLinkage(panNumber);
				}
				...
				if (
					...
					if (kycEnvironmentService.isSandbox()) {
						console.warn(
							"⚠️ [Sandbox PAN API] Linkage auth failed (401/403), falling back to mock (KYC sandbox mode)",
						);
						return this.mockPANAadhaarLinkage(panNumber);
					}
					throw new AppError(
				...
				if (kycEnvironmentService.isSandbox()) {
					console.warn(
						`⚠️ [Sandbox PAN API] Linkage check HTTP ${statusCode} ("${errorMessage}") in KYC sandbox mode — falling back to mock`,
					);
					return this.mockPANAadhaarLinkage(panNumber);
				}
			...
			console.error(
				"❌ [Sandbox PAN API] Unexpected linkage check error:",
				error,
			);
			throw new AppError(
				"PAN-Aadhaar linkage check failed",
				500,
				"SANDBOX_API_ERROR",
			);
		}
	...
	private mockPANAadhaarLinkage(panNumber: string): PANAadhaarLinkageResponse {
		console.log(
			"🔧 [Sandbox PAN API] Using MOCK PAN-Aadhaar linkage for:",
			this.maskPAN(panNumber),
		);
	...
	async validatePANComprehensive(
		panNumber: string,
		fullName?: string,
		dateOfBirth?: string,
	): Promise<{
		...
		try {
			...
			if (this.isPANInoperative(panData.status)) {
				errors.push(
					"PAN is inoperative. Please link your PAN with Aadhaar on the Income Tax portal.",
				);
				return {
					isValid: false,
					isOperative: false,
			...
			try {
				...
				if (!linkageData?.aadhaar_linked) {
					warnings.push(
						"PAN is not linked with Aadhaar. You may proceed, but linking is recommended for compliance.",
					);
				}
			} catch (linkageError) {
				console.warn(
					"⚠️ [Sandbox PAN API] Linkage check failed, proceeding without linkage data:",
					linkageError,
				);
				warnings.push(
					"PAN-Aadhaar linkage status could not be verified. Please check on the Income Tax portal.",
				);
			}
			...
			console.error(
				"❌ [Sandbox PAN API] Comprehensive validation error:",
				error,
			);
			if (error instanceof AppError) {
