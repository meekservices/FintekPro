...
export const chatSessions = pgTable("chat_sessions", {
  ...
  title: varchar("title").notNull(),
  sessionType: varchar("session_type").notNull(), // 'general', 'portfolio_analysis', 'tax_advice', 'transaction'
...
// Chat messages - stores individual messages in sessions
export const chatMessages = pgTable("chat_messages", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  sessionId: varchar("session_id").references(() => chatSessions.id).notNull(),
  ...
  functionCall: jsonb("function_call"), // {name: string, arguments: object}
  functionResponse: jsonb("function_response"), // Response from function execution
...
// Chat actions - stores pending actions requiring user confirmation
export const chatActions = pgTable("chat_actions", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  sessionId: varchar("session_id").references(() => chatSessions.id).notNull(),
  ...
  functionName: varchar("function_name").notNull(), // 'execute_transaction', 'rebalance_portfolio', etc.
  actionParams: jsonb("action_params").notNull(), // Parameters for the action
  
  status: varchar("status").notNull().default("pending"), // 'pending', 'confirmed', 'rejected', 'executed', 'failed'
  ...
  index("idx_chat_actions_session").on(table.sessionId),
  index("idx_chat_actions_user").on(table.userId),
  index("idx_chat_actions_status").on(table.status),
]);

// Zod schemas for chat tables
export const insertChatSessionSchema = createInsertSchema(chatSessions).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
