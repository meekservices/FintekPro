...
- **Root cause of "SERVER unexpectedly disconnected" — fully fixed (two-part fix)**:
  - **Part 1 — Schema isolation**: heliumdb has 755 tables + 85 sequences in the `public` schema. When drizzle-kit introspected `public`, it generated DROP SEQUENCE for 80 sequences not in schema-stub → `error:` in output → Replit's panel crashed. Fix: `drizzle.config.ts` uses `schemaFilter: ["drizzle_kit_managed"]` — an isolated schema with only the 6 stub tables and 5 sequences. drizzle-kit NEVER sees `public` → zero DROP statements.
  - **Part 2 — SSL disabled**: Helium (Replit's managed PostgreSQL) runs locally and does NOT support SSL connections (unlike legacy Neon which required SSL). Without `ssl: false` in `dbCredentials`, the pg driver attempts an SSL handshake which causes "the socket disconnecting unexpectedly" — this IS the "SERVER unexpectedly disconnected" error. Fix: `drizzle.config.ts` now has `ssl: false` in `dbCredentials`. This matches Replit's own upgrade docs: https://docs.replit.com/cloud-services/storage-and-databases/database-upgrade.
