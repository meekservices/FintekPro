...
- **NEVER re-add `postgresql-16` to Replit modules** — it activates the database diff panel which can crash on publish if drizzle-kit hangs. The `postgresql-16` module was intentionally removed. The app connects to Neon via `PRODUCTION_DATABASE_URL` directly (see `server/db.ts`).
- **`drizzle.config.ts` uses `shared/schema-stub.ts`** — a fully self-contained file with 6 table definitions and 38 pgEnum declarations. It does NOT import from `shared/schema.ts` (1.5MB / 32k lines → OOM-crashes drizzle-kit). See the "drizzle-kit Schema Diff Configuration" section below for full details.
- **`drizzle-kit push` completes in ~3s** when the DB is under normal load. If the DB is under heavy load, `connect_timeout=15` and `statement_timeout=15000` in `drizzle.config.ts` ensure the process fails fast (within 15s) rather than hanging indefinitely and triggering "SERVER unexpectedly disconnected".
