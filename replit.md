...
A Scheme Master Governance System (`server/services/scheme-governance-service.ts`) provides ISIN-based fund identification with DB-driven transaction eligibility rules. Key components include: `scheme_rename_log` for automatic scheme name change detection during AMFI sync, `scheme_transaction_rules` for DB-driven purchase/SIP restrictions (replacing hardcoded registries), `proposal_audit_log` for per-fund eligibility audit trails on every proposal, and `proposal_versions` for immutable proposal snapshots. The system uses a DB-first with hardcoded-fallback pattern for zero-downtime migration. Admin endpoints (`/api/admin/scheme-governance/*`) provide rename tracking, transaction rule management, compliance reports, ISIN resolution, and proposal audit/version history.

### System Design Choices
FintekPro uses a subdomain-based portal architecture for Admin, Partner, Agent, and Client portals with role-based access control. A Multi-Portal Brand Segregation System (`shared/portal.ts`, `server/routes/portal-system.ts`, `client/src/components/portal/`) provides portal-specific branding (logos, colors, taglines) via a PortalType enum (MAIN, PARTNER, AGENT, ADMIN), a `/api/system/portal-meta` endpoint returning branding config based on subdomain detection, dynamically generated SVG logos per portal, a `PortalLogo.tsx` component with `usePortalMeta()` hook, and a `PortalThemeProvider.tsx` that applies CSS custom properties per portal. Session-based portal validation (`stampSessionPortal` + `validateSessionPortal` middleware) prevents cross-portal session reuse. A `portal_access_log` table provides SEBI/RBI audit trail for portal access. A Financial Metrics Engine provides 40+ derived ratios. It utilizes a Centralized Service Registry pattern for singleton management. A Staggered Startup System prevents resource contention, and Fast Boot Optimization ensures quick server responsiveness. A Regulatory Gaps Tracker monitors compliance across various regulators.
