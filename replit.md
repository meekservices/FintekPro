...
The frontend leverages Wouter for routing, TanStack Query for state management, and React Hook Form with Zod for form handling, all built with Vite. The backend is developed using Express.js with TypeScript, connected to a PostgreSQL database via Drizzle ORM and Drizzle Kit for migrations. A RESTful API pattern is implemented with centralized error handling. Authentication includes a simplified system with mandatory two-factor OTP verification (email/SMS/WhatsApp) and unified login supporting email, mobile, or userId, utilizing Passport.js. **Password Reset Flow** (2025-10-25): Secure forgot password functionality using existing OTP infrastructure with `type="password_reset"`, 5-minute expiry, multi-channel OTP delivery (email primary, SMS/WhatsApp fallback), generic responses preventing user enumeration, and automatic OTP cleanup after successful reset. **PAN Verification Migration** (2025-10-27): Complete migration from Sandbox.co.in to Cashfree Verification Suite API for all PAN verification flows (Client KYC, Corporate KYC, NRI KYC). Includes type normalization (uppercase "INDIVIDUAL"/"BUSINESS" → TitleCase "Individual"/"Business"), enhanced response data (Aadhaar seeding status, name match scores, PAN status), and consistent error handling across all user types. Sandbox.co.in now reserved exclusively for ITR filing functionality.

**KYC Profile Dashboard & Product Eligibility** (2025-10-27): Integrated verified KYC profile display into existing KYC dashboard with real-time product eligibility matrix based on tiered KYC system. Upon Smart KYC completion (post-Aadhaar verification), verified data (PAN, full name, DOB) is automatically transferred from `kycVerificationSession` to both `users` and `userProfiles` tables. Dashboard displays: (1) Verified Profile Card showing masked PAN (format: ABCDE****F), full name, tier badge, and verification date with edit button for future use; (2) Smart Upgrade CTA for basic/enhanced tier users with tier-specific messaging, hidden for accredited investors; (3) Product Eligibility Matrix with visual green/gray indicators showing accessible vs locked investment products (mutual funds, F&O, AIF, PMS, etc.) based on current tier. Data is stored in PostgreSQL (Neon serverless driver), with Drizzle ORM ensuring type-safe queries and schemas for various financial entities.
