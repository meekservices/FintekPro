...
11. **Cookie Configuration Fix** (November 9, 2025): Changed session and CSRF cookies from `SameSite=None` to `SameSite=Lax` to fix production login failures caused by browser cookie blocking. `SameSite=Lax` provides better browser compatibility for same-domain requests (fintekpro.com → fintekpro.com) while maintaining cross-subdomain cookie sharing via `domain: ".fintekpro.com"`. Both cookies maintain secure flags (`secure: true` in production, `httpOnly: true` for session). Architect-verified PASS.
