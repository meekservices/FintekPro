...
- **NEVER re-add `postgresql-16` to Replit modules** — this is the root cause of "Failed to check for database diff: SERVER unexpectedly disconnected". When `postgresql-16` is in modules, Replit activates its DB diff panel which runs `drizzle-kit studio` as a persistent server. Studio connects to the local DATABASE_URL which proxies to the same 755-table Neon DB. When the Replit browser client connects to studio and triggers full DB introspection, studio OOMs and crashes → Replit shows "SERVER unexpectedly disconnected". The `postgresql-16` module has been intentionally removed. The app connects to Neon via `PRODUCTION_DATABASE_URL` exclusively (see `server/db.ts` — DATABASE_URL is never used by the app).
- **`drizzle.config.ts` uses `DATABASE_URL` (Replit-managed heliumdb, local)** — NOT `PRODUCTION_DATABASE_URL`. The 6 tables in schema-stub.ts are already synced to heliumdb, so the diff is always empty and fast. Using `PRODUCTION_DATABASE_URL` here would be dangerous: agent_notifications.id is uuid in schema-stub but serial in the live Neon DB → drizzle-kit would generate destructive ALTER TABLE against production. See the "drizzle-kit Schema Diff Configuration" section below for full details.
- **`drizzle-kit push` completes in ~3s**. `connect_timeout=15` and `statement_timeout=15000` in `drizzle.config.ts` provide hard bounds if the DB is under heavy load.
...
- **drizzle-kit check** always passes cleanly. **drizzle-kit push** completes in ~3s and exits with code 0 but may print a PostgreSQL error about sequences — this is safe to ignore (no DB changes applied). The sequence issue arises because drizzle-kit queries ALL sequences in the `public` schema (not filtered by tablesFilter) and tries to DROP ones not in the schema; PostgreSQL correctly refuses (dependent objects). This is a known drizzle-kit design limitation with no workaround short of patching the binary.
- **drizzle.config.ts appends `connect_timeout=15` and `options=-c statement_timeout=15000`** to the DB URL. These hard limits ensure drizzle-kit always exits within ~15s even when the DB is under heavy load (e.g., 54+ active connections from the production server), preventing the indefinite hang that causes "SERVER unexpectedly disconnected" in the Replit database diff tool.
