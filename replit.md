...
A Consolidated MF Metrics System enforces a strict separation between the two mutual fund tables: `mutual_funds` holds only API-fetched fields (NAV, AUM, expense ratio, basic returns from AMFI, min SIP/lumpsum, exit load, ISIN, benchmark mapping, fund metadata). `mutual_fund_metrics` holds exclusively calculated/derived fields (alpha, beta, sharpe, sortino, stddev, treynor, information ratio, max drawdown, CAGR, SIP returns, XIRR, capture ratios, VaR, portfolio PE/PB, percentile rank) per fiscal year (FY25-26 format). The 12 calculated metric columns (alpha, beta, sharpeRatio, sortinoRatio, standardDeviation, treynorRatio, informationRatio, maxDrawdown, alphaAvailable, betaAvailable, treynorAvailable, informationRatioAvailable) have been permanently removed from `mutual_funds`. All reading code for risk metrics uses LEFT JOIN with `mutual_fund_metrics` (latest fiscal year per scheme). The MF Comprehensive Enrichment Pipeline writes calculated metrics exclusively to `mutual_fund_metrics` via upsert on (scheme_code, fiscal_year). Returns in `prospect-proposals.ts` and fund listing use COALESCE(mutual_funds.returns1y, mutual_fund_metrics.return1y) for maximum coverage. The two tables share no duplicate columns (aum/expenseRatio in both serve different purposes: current vs period-specific).
