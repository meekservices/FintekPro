...
- **Agent Onboarding & Management System**: SEBI-compliant agent onboarding with ARN/EUIN validation, hierarchical commission distribution (master-agent → sub-agent), document verification workflow, and automated TDS calculation (10%). Features secure API endpoints with role-based access control, ownership verification, and comprehensive audit logging.
