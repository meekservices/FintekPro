...
A **Unified OCR Service** (`server/services/unified-ocr-service.ts`) provides central document text extraction for the entire platform: Gemini Vision (`gemini-2.0-flash`) handles scanned PDFs and images (JPG/PNG/WEBP/HEIC) with 6 domain-tuned prompts (financial_statement, tax_document, kyc_document, bank_statement, invoice, general); Sandbox.co.in handles structured ITR extraction (Form 16, Form 26AS). Exposed as `GET /api/ocr/status` and `POST /api/ocr/extract-text`. The **Unified PDF Parser** (`unified-pdf-parser.ts`) now has an automatic OCR fallback: when pdf-parse returns `EMPTY_CONTENT` (scanned/image-based PDFs), it transparently routes to Gemini Vision and continues through the same pipeline (type detection, layout analysis, semantic extraction). `SafeExtractResult` surfaces `ocrFallbackUsed` and `ocrConfidence` flags. The `enableOCRFallback` option (default `true`) and `ocrHint` control the behaviour per call.
