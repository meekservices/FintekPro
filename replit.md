...
FintekPro integrates a Comprehensive Zoho Ecosystem (CRM, Books, Campaigns, Meeting, Sign). It features a Profit-Optimized AI Recommendation Engine and a Unified AI Recommendation Engine. A Stock Enrichment System consolidates listed stocks, and an ISIN Intelligence Layer provides automatic instrument classification. The Agent Knowledge Hub provides market intelligence and a Gemini-powered Daily AI Market Brief Engine. A **DB-First Stock Screener** (`server/services/screener/`) uses FMP with rate-limited data ingestion (250 API calls/day, auto-stop at 220), a derived metrics scoring engine (Growth/Quality/Value/Risk/Composite scores, FintekRating 1-5 stars), and seeds from the existing `listed_stocks` table for zero-cost development testing. FMP enrichment routes are production-only by default. DB tables: `screener_stocks`, `screener_financials`, `screener_price_history`, `screener_derived_metrics`, `fmp_usage_log`. Admin endpoints: `/api/screener/admin/seed-from-db`, `/api/screener/admin/enrich/*`, `/api/screener/admin/recalculate-metrics`.
