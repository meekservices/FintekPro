...
**Production Authentication Security** (November 2025): KYC wizard component implements client-side authentication guards that check user authentication status via `useAuth()` hook before rendering. Unauthenticated users are automatically redirected to `/login` with clear messaging. The wizard defers all data fetching (progress queries) until authentication is confirmed via `enabled: !!user` flag. Loading states prevent premature rendering. The PAN verification endpoint (`/api/kyc/wizard/verify-pan`) now derives the customer's full name from persisted user profile data (firstName, middleName, lastName) instead of accepting empty strings, ensuring the Sandbox.co.in PAN API receives proper input parameters. The endpoint gracefully handles name lookup failures and continues verification with available data.
