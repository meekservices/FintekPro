...
A **Multi-Level Partner Hierarchy System** enables hierarchical partner onboarding (Partner → Sub-Partner → Agent) with controlled delegation, commission waterfall, client ownership protection, and audit-ready compliance. The `partners` table is extended with hierarchy fields (`parentPartnerId`, `partnerLevel` L1/L2/L3, `hierarchyPartnerType` MASTER/SUB/AGENT, `hierarchyStatus`, `kycStatus`, `approvalStatus`, `agreementId`). Six new tables support the system: `partner_hierarchy_agreements` (formal agreements), `partner_commission_rules` (waterfall percentages per product type), `partner_wallets` (balance tracking), `partner_commission_ledger` (immutable ledger entries), `partner_audit_logs` (regulatory audit trail), and `partner_client_ownership` (immutable client ownership after first transaction). Services: `PartnerHierarchyService` (creation, approval, downline, ownership), `CommissionWaterfallEngine` (tree-based commission resolution, wallet credits, anti-MLM safeguards), `PartnerAuditService` (immutable logging), `HierarchyIntegrityValidator` (cycle/orphan/depth detection). API routes under `/api/partner-hierarchy/*` in `server/routes/partner/hierarchy-routes.ts`.
