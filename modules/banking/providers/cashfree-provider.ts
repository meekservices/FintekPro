...
export class CashfreeProvider implements IBankingProvider {
  ...
  async getTransactions(accountId: string, startDate: Date, endDate: Date): Promise<BankTransaction[]> {
    const token = await this.getAuthToken();
    // Cashfree Payouts uses a different endpoint for transaction history
  ...
  async initiateTransfer(request: TransferRequest): Promise<TransferResponse> {
    const token = await this.getAuthToken();
    const payload = {
      beneId: `BENE_${Date.now()}`, // Or use existing bene
    ...
    // First add beneficiary if not exists (omitted for brevity)
    // Then request transfer
    const response = await axios.post(`${this.baseUrl}/requestTransfer`, payload, {
      headers: { Authorization: `Bearer ${token}` },
    });
  ...
  async verifyBeneficiary(accountNumber: string, ifscCode: string): Promise<boolean> {
    ...
    try {
      const response = await axios.post(`${this.baseUrl}/validation/bankDetails`, payload, {
        headers: { Authorization: `Bearer ${token}` },
      });
      return response.data.status === 'SUCCESS' && response.data.data.accountStatus === 'VALID';
    } catch (e) {
      return false;
    }
  }
